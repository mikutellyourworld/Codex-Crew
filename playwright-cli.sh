...
#
#   curl -fsSL https://raw.githubusercontent.com/mikutellyourworld/Codex-Crew/main/playwright-cli.sh | sh
#   curl -fsSL https://raw.githubusercontent.com/mikutellyourworld/Codex-Crew/main/playwright-cli.sh | sh -s -- --version 0.1.18
#
# Read before you run (many enterprises forbid piping a script into a shell):
#   curl -fsSLO https://raw.githubusercontent.com/mikutellyourworld/Codex-Crew/main/playwright-cli.sh
...
Environment:
  CODEXCREW_HOME                 data home (default ~/.codex-crew)
  CODEXCREW_PLAYWRIGHT_CLI_HOME  overrides --prefix
  CODEXCREW_NPM_REGISTRY         overrides --registry
  CODEXCREW_NODE_BIN_DIR         an existing Node bin dir to reuse
...
# flag form is refused, so CODEXCREW_NPM_REGISTRY stays a working escape.
REGISTRY="${CODEXCREW_NPM_REGISTRY:-$PUBLIC_NPM_REGISTRY}"
REGISTRY_FROM_FLAG=0
DOWNLOAD_HOST_FROM_FLAG=0
ISOLATED_NPMRC=0

# The package itself declares engines.node >= 18, but the floor that matters is
# the one Codex Crew's own browsing requires of this CLI: MIN_NODE_MAJOR in
# src/codex_crew/browser_cli/install.py, currently 20. Accepting 18 here would
...
NODE_STAMP_NAME=".codexcrew-playwright-cli-node"
...
DATA_HOME="${CODEXCREW_HOME:-${HOME:-}/.codex-crew}"
PREFIX="${CODEXCREW_PLAYWRIGHT_CLI_HOME:-$DATA_HOME/playwright-cli}"
...
if [ -z "${HOME:-}" ] && [ -z "${CODEXCREW_HOME:-}" ] \
   && { [ "$PREFIX" = "/.codex-crew/playwright-cli" ] || [ "$BIN_DIR" = "/.local/bin" ]; }; then
...
else _registry_label="CODEXCREW_NPM_REGISTRY"; fi
...
[ "$REGISTRY_FROM_FLAG" = 0 ] || _reject_url_credential "--registry" "$REGISTRY" \
  "Pass it as CODEXCREW_NPM_REGISTRY in the environment instead, or run 'npm login --registry <url>' first."
...
_try_node() { # candidate node path
  ...
  # CODEXCREW_NODE_BIN_DIR or the ensure-node.sh marker; a PATH lookup cannot
  ...
  # candidate funnels through -- $CODEXCREW_NODE_BIN_DIR, the marker file and a
...
# earlier, then one the caller named, then the toolchain Codex Crew's own
# ensure-node.sh recorded (so the two installers share a single download), then
# whatever is on PATH.
_resolve_node() {
  _try_node "$PREFIX/node/bin/node" && return 0
  if [ -n "${CODEXCREW_NODE_BIN_DIR:-}" ]; then
    _try_node "$CODEXCREW_NODE_BIN_DIR/node" && return 0
  fi
