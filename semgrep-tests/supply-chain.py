...
def download_then_parse(url):
    ...
    # ok: codexcrew.download-then-exec
    resp = requests.get(url)
    return resp.json()
...
def download_then_subprocess(fetch):
    ...
    # ruleid: codexcrew.download-then-subprocess
    download_path = fetch()
...
def run_pinned_binary(config_path):
    ...
    # ok: codexcrew.download-then-subprocess
