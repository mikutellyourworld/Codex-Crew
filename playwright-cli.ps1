...
#
#   irm https://raw.githubusercontent.com/mikutellyourworld/Codex-Crew/main/playwright-cli.ps1 | iex
#
# Read before you run (many enterprises forbid piping a script into a shell):
#   irm https://raw.githubusercontent.com/mikutellyourworld/Codex-Crew/main/playwright-cli.ps1 -OutFile playwright-cli.ps1
...
# Codex Crew's browsing requires of this CLI: MIN_NODE_MAJOR in
# src/codex_crew/browser_cli/install.py. Accepting less would install a CLI the
...
$NodeStampName = ".codexcrew-playwright-cli-node"
...
if ([string]::IsNullOrWhiteSpace($Registry)) {
    $Registry = if ($env:CODEXCREW_NPM_REGISTRY) { $env:CODEXCREW_NPM_REGISTRY } else { $PublicNpmRegistry }
}
...
if ($PSBoundParameters.ContainsKey("Registry")) {
    Deny-UrlCredential "-Registry" $Registry ("Set CODEXCREW_NPM_REGISTRY in the " +
...
$dataHome = if ($env:CODEXCREW_HOME) { $env:CODEXCREW_HOME } else { Join-Path $HOME ".codex-crew" }
if ([string]::IsNullOrWhiteSpace($Prefix)) {
    $Prefix = if ($env:CODEXCREW_PLAYWRIGHT_CLI_HOME) {
        $env:CODEXCREW_PLAYWRIGHT_CLI_HOME
    } else {
...
function Try-Node([string]$Candidate) {
    ...
    # wrapper. Reachable via CODEXCREW_NODE_BIN_DIR, the node-bin-dir marker, or a
    ...
    # here -- $env:CODEXCREW_NODE_BIN_DIR, the marker file and a PATH lookup can
    # each be relative.
...
function Resolve-Node {
    ...
    if ($env:CODEXCREW_NODE_BIN_DIR -and (Try-Node (Join-Path $env:CODEXCREW_NODE_BIN_DIR "node.exe"))) { return $true }
