# Codex Crew - Windows build targets (pip + npm/vite + pytest).
#
...
function Get-DataHome {
    if ($env:CODEXCREW_HOME) { return $env:CODEXCREW_HOME }
    # Never the legacy ~/.codexcrew: the one-time data-home migration deletes it,
    # so writing there would resurrect it on every build.
    return (Join-Path $env:USERPROFILE ".codex-crew")
}
...
function Invoke-Frontend {
    ...
    $staged = Join-Path $RepoRoot "src\codex_crew\static\dist"
    Remove-PathForce $staged
    New-Item -ItemType Directory -Force -Path (Join-Path $RepoRoot "src\codex_crew\static") | Out-Null
    Copy-Item -Recurse -Path (Join-Path $RepoRoot "website\dist") -Destination $staged
    Write-Ok "dashboard staged into src\codex_crew\static\dist"
}
...
function Invoke-Backend {
    ...
    # CODEXCREW_SKIP_FRONTEND: the frontend target already staged the dist, and
    # the editable install must not try to rebuild it.
    $prev = $env:CODEXCREW_SKIP_FRONTEND
    $env:CODEXCREW_SKIP_FRONTEND = "1"
    try {
        Invoke-Step $VenvPython @("-m", "pip", "install", "--prefer-binary", "-e", ".[dev]") `
            "pip install -e .[dev]"
    } finally {
        $env:CODEXCREW_SKIP_FRONTEND = $prev
    }
...
function Invoke-Clean {
    ...
    foreach ($p in @("build", "dist", "src\codex_crew\static\dist", "website\dist",
