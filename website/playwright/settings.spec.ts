...
test.describe('Settings Page', () => {
  ...
  test('config mutation round-trip: recent_tint_count via Display stepper', async ({ page, request }) => {
    ...
    const before = await (await request.get('/api/config/codexcrew')).json()
    ...
    await expect.poll(async () => {
      const cfg = await (await request.get('/api/config/codexcrew')).json()
    ...
    await request.fetch('/api/config/codexcrew', {
      method: 'PATCH',
