 ...
 * Skills, Steering, Hooks, Prompts. Default tab is "crews" (CodexCrewAgentsPage).
 *
...
test.describe('Capabilities Page — /capabilities', () => {
  ...
  test('renders the page title and default Agents tab heading', async ({ page }) => {
    ...
    // page-subtitle testid on this route. CodexCrewAgentsPage renders the same
  ...
  test('crews tab renders the crew roster as cards', async ({ page }) => {
    ...
    // The minimal fixture seeds one crew bound to the "codexcrew" agent template.
    ...
    const seeded = cards.filter({ hasText: 'codexcrew' }).first()
  ...
  test('create and delete crew round-trip via the editor panel', async ({ page, request }) => {
    ...
    try {
      ...
      await page.getByRole('option', { name: 'codexcrew', exact: true }).click()
