...
const HARNESS_GATEWAY = !!process.env.CODEXCREW_E2E_EPHEMERAL
...
test.describe('Logs Page', () => {
  test.beforeEach(async ({ page, request }) => {
    test.skip(
      !HARNESS_GATEWAY,
      'persisting a DEBUG log level requires the ephemeral harness gateway (CODEXCREW_E2E_EPHEMERAL)',
    )
  ...
  test('search filter shows match count when term matches log lines', async ({ page }) => {
    ...
    // Every log line contains "codex_crew" in the logger name field.
    // Type the search term
    const filterInput = page.getByLabel('Filter logs')
    await filterInput.fill('codex_crew')
