...
describe('McpTab Integration Tests', () => {
        ...
        return HttpResponse.json([
          {
            ...
            presence: { codexcrew: true, kiroGlobal: true, ccGlobal: false },
          },
          {
            ...
            enabled: false,
            tools: [],
            presence: { codexcrew: false, kiroGlobal: false, ccGlobal: false },
          },
        ])
      })
    )
        ...
        return HttpResponse.json([
          {
            ...
            disabledTools: [],
            command: 'node test',
            presence: { codexcrew: true, kiroGlobal: false, ccGlobal: false },
          },
        ])
      })
    )
  ...
  it('stages CodexCrew scope off and commits it via Apply', async () => {
    const user = userEvent.setup()
    ...
    // Find the builder-mcp row and click its CodexCrew scope badge
    ...
    const mcBadge = within(builderRow as HTMLElement).getByRole('button', {
      name: /Codex Crew.*click to disable/i,
    })
    ...
    await waitFor(() => {
      ...
      expect(applyPayload.changes[0].codexcrew).toBe(false)
    })
  })
