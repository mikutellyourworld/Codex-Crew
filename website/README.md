...
React + TypeScript + Vite single-page app for the Codex Crew dashboard. Built assets
are emitted to `dist/` and copied into the Python package at
`../src/codex_crew/static/dist/` so the gateway can serve them.

## Develop

```bash
npm install          # install dependencies (public npm registry)
npm run dev          # Vite dev server on http://localhost:3000 (proxies API to the gateway on :5486)
```
...
```bash
rm -rf ../src/codex_crew/static/dist && cp -r dist ../src/codex_crew/static/dist
```
