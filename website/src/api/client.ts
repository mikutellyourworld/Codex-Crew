...
export type McpShareRecommendation = {
  strength: string
  // Two axes, not one verdict. `recommendStub` is the safe half — Codex Crew's
...
export const SEARCH_MIN_CHARS = 2  // backend session search threshold (must match codex_crew.history.SEARCH_MIN_CHARS)
...
export interface DeniedCommandRule {
   ...
   *  an always-on floor built into Codex Crew; 'policy' = governance-pinned;
   *  null/absent = freely toggleable. Additive — `pinned` keeps its
   *  governance-only meaning. */
  lock_reason?: 'floor' | 'policy' | null
  /** Where the rule came from: 'builtin' = shipped in Codex Crew's catalog;
 ...
 * `PATCH /api/config/codexcrew` validates against); `installed` is a machine fact.
...
export interface OpenAICompatibleProfilesResponse {
  active: string
...
function showSessionExpiredBanner(lead?: string): void {
  ...
  code.textContent = 'codexcrew token'
 ...
 * a session whose token was minted before `CODEXCREW_OWNER_ID` was configured.
...
export interface KiroPrerequisiteStatus {
  ...
  /**
   * Codex Crew's own agent spec files missing from the kiro-cli agents directory.
  ...
  /**
   * Codex Crew's own specs that are PRESENT on disk but which the installed
   * kiro-cli refuses to load. Presence and acceptance are different questions: a
   * rejected spec is dropped from kiro-cli's agent table, so `--agent codexcrew`
   * resolves to the default agent with none of Codex Crew's MCP servers — the
  ...
  /**
   * Whether the installed kiro-cli exposes the `acp` subcommand every Codex Crew
   ...
   * `login_command`, Codex Crew also runs this FOR the owner via the update-cli
...
/** Skip keeps CodexCrew's item; rename installs alongside; overwrite replaces it
...
export const api = {
  ...
  codexcrewAgents: (sessionKey?: string) =>
    fetch('/api/agents', {
      headers: sessionKey ? { 'X-Session-Key': sessionKey } : { ..._sk },
    }).then(j),
  /** The model a new session on this CodexCrew agent would run on. Empty
  ...
  // against, exactly as it does for codexcrewAgents. Without it the server can
  ...
  codexcrewConfig: () => fetch('/api/config/codexcrew').then(j),
  saveKirocrewConfig: (agent: object) => put('/api/config/codexcrew', { agent }).then(j) as Promise<{ ok?: boolean; restart_required?: boolean; error?: string }>,
  patchConfig: (path: string, value: unknown) => fetch('/api/config/codexcrew', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ path, value }) }).then(j),
  ...
  selectOpenAIProfile: (id: string) =>
    put('/api/openai-profiles/active', { id }).then(j) as Promise<OpenAICompatibleProfilesResponse>,
  deleteOpenAIProfile: (id: string) =>
    del(`/api/openai-profiles/${encodeURIComponent(id)}`).then(j) as Promise<OpenAICompatibleProfilesResponse>,
