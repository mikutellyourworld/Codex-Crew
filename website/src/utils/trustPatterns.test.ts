...
describe('truncateCommandLabel — label only, never the pattern', () => {
  ...
  it('renders an ordinary long command IN FULL — the ceiling only guards pathological input', () => {
    ...
    const cmd = 'gh api repos/mikutellyourworld/Codex-Crew/contents/website/src/config/production.json --jq .content.sha'
  ...
  it('distinguishes two commands that COLLIDED under the old 64 budget', () => {
    ...
    const head = 'gh api repos/mikutellyourworld/Codex-Crew/contents/'
  ...
  it('keeps two commands distinguishable when they share a prefix LONGER than the budget', () => {
    ...
    const base = `gh api repos/mikutellyourworld/Codex-Crew/contents/${'deeply/nested/path/segment/'.repeat(9)}`
  ...
  it('can still collide PAST the ceiling — the residual risk is documented, not hidden', () => {
    ...
    const head = `gh api repos/mikutellyourworld/Codex-Crew/contents/${'deeply/nested/path/segment/'.repeat(6)}`.slice(0, 170)
