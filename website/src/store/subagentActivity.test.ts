...
describe('selectSubagentActivityCount', () => {
  ...
  it('counts started agents in the active slot', () => {
    ...
    store.dispatch(sseSubagentSpawn({ slot: 'a', id: 'x1', task: 't', agent: 'codexcrew' }))
    store.dispatch(sseSubagentSpawn({ slot: 'a', id: 'x2', task: 't', agent: 'codexcrew' }))
  ...
  it('counts agents in background slots too — the whole point of a rail dot', () => {
    const store = makeStore()
    store.dispatch(setActiveSlot('a'))
    store.dispatch(sseSubagentSpawn({ slot: 'b', id: 'y1', task: 't', agent: 'codexcrew' }))
