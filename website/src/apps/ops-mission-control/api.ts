...
export interface RotationInfo {
   ...
   * operator would be showing them Codex Crew's cron plumbing on an ops board, and the
 ...
 * fixes differ (flip a toggle / enter a channel / configure Codex Crew's Slack).
 */
export interface SlackOutStatus {
  enabled: boolean
  channel: string
  /** Whether Codex Crew's OWN Slack client exists — this app stores no token. */
...
export const opsApi = {
   ...
   * not run Codex Crew can be handed, which is why it is worth a round trip.
   *
   ...
   * exists, and the UI must never synthesize it: `CODEXCREW_HOME` moves the data directory,
   ...
   * exists beside the Slack one. Per-channel muting is NOT here: Codex Crew owns that
