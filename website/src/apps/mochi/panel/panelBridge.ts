...
// Slot keys are CodexCrew-internal and never shown to the user.
 ...
 * `codex_crew.trust_patterns` module), and they arrive on this
...
// process's GatewayManager.spawn(); the "Start CodexCrew" button launched it. None of that maps onto a builtin: the panel is SERVED BY the gateway, so
...
// "Start CodexCrew" button to "Reconnect" — see ChatPanel's banner.)
 ...
 * — it waits for the frame. CodexCrew core does NOT echo a normal send over the
 ...
 * closed=true)`), so the conversation reappears in CodexCrew's session list with
...
/** Open the CodexCrew dashboard in the system browser (main supplies the URL). */
...
export async function respondApproval(
  ...
  try {
    ...
    if (!res.ok) {
      // A stale pre-owner session (signed in before CODEXCREW_OWNER_ID was set)
...
// CodexCrew" survives as a real feature, and core owns it (/api/instances/*).
