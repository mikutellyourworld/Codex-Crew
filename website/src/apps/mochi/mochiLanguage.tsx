 ...
 *
 *   ''        -> follow CodexCrew (the shared `mc-lang` the dashboard persists)
 *   '<code>'  -> Mochi overrides it for its own windows only
 *
 * `initI18n(undefined)` already falls back to `readStoredLanguage()`, and Mochi's
 * windows are same-origin with the dashboard, so "follow CodexCrew" needs no
 ...
 * argument it resolves the shared `mc-lang`, i.e. whatever CodexCrew is set to;
...
export function MochiLocalized({
  ...
  useEffect(() => {
    ...
    const apply = (value: unknown) => {
      ...
      // Empty means "follow CodexCrew", which is what the shared key holds.
