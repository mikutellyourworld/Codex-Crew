 ...
 * (so `git diff` against the pristine copy stays readable when CodexCrew's Mochi
 ...
 *     devForceUpdateUI, devReinstall — CodexCrew owns updates.
 ...
 *   - themes: broadcastTheme — Mochi follows the CodexCrew theme.
...
interface UnimplementedUpstream {
  /** Self-updater — CodexCrew owns updates. */
  ...
  /** Themes — Mochi follows the CodexCrew theme. */
...
export function nestConfig(s: MochiSettings): NestedConfig {
  ...
  return {
    mochi: {
      ...
      // Owned by CodexCrew core (Settings -> Chat), not by Mochi. Reported as the
 ...
 */
const HOST_MANAGED_MCP = new Set(['codexcrew-cron', 'codexcrew-core'])
 ...
 * CodexCrew's OWN mechanism (`captureScreen` + `SnipOverlay`, with
 ...
 *
 * Two steps, in this order and for a reason. The chat SLOT belongs to CodexCrew
 ...
 * slot left the conversation (and its preview) in CodexCrew's session list, so a
