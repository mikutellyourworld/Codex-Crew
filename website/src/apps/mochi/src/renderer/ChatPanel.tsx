...
export const ChatPanel: React.FC<ChatPanelProps> = ({ onToggleWatch, watchPanelVisible, onTogglePinned, pinnedPanelVisible, pinnedFileCount }) => {
  ...
  useEffect(() => {
    // Load history from CodexCrew slot
      ...
      {ctxMenu && (
        <ContextMenu
          ...
          /* SUBTRACTED four rows, each of which rendered but did nothing:
             ...
             app manager owns that and `codexcrew doctor` covers the host), quit
             (CodexCrew owns the app lifecycle; the pet is disabled from the App
 ...
 * Ported from CodexCrew's MarkdownRenderer — handles incomplete code fences
...
const mdComponents: Components = {
  ...
  img: (p) => {
    ...
    //
    // This used to be gated on `.codex-crew/screenshots/` only, so every UPLOADED
...
function isLocalFilePath(src: unknown): src is string {
  ...
  return /^\/(?:Users|home|tmp|var|private|etc)\//.test(src) || src.includes('.codex-crew/')
}
