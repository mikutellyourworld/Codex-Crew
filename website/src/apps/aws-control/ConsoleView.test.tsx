...
describe('UsagePane', () => {
  ...
  it('renders the storage block (bucket line + meter) only when the drive exists', async () => {
    ...
    expect(within(storage).getByTestId('drive-bucket')).toHaveTextContent('codexcrew-drive-abc123')
...
describe('SetupCard', () => {
  it('previews the bootstrap payload, then confirms and creates the bucket', async () => {
    vi.mocked(awsControlApi.driveBootstrapPreview).mockResolvedValue({
      preview: true, account: ACCOUNT.account, region: 'us-west-2', resource: 'codexcrew-drive-abc123',
    })
    vi.mocked(awsControlApi.driveBootstrapConfirm).mockResolvedValue({ created: true, bucket: 'codexcrew-drive-abc123' })
    ...
    expect(await screen.findByTestId('drive-preview')).toHaveTextContent('codexcrew-drive-abc123')
