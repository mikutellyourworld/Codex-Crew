...
import AgentSelector, { type CodexCrewAgent } from '../components/AgentSelector'
...
export default function WebhooksPage() {
  ...
  const agentsQuery = useQuery<CodexCrewAgent[]>({
    queryKey: ['agents-installed'],
    queryFn: async () => {
      const response = await Promise.resolve(api.agentsInstalled?.())
      return Array.isArray(response) ? response as CodexCrewAgent[] : []
    },
    retry: false,
  })
        ...
        actions={plane === 'sources' && selectedToken
          ? (
            <Btn
              ...
              onClick={() => sendTest.mutate(selectedToken.agent || 'codexcrew')}
            >
              <Play size={14} />{' '}
