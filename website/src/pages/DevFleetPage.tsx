/**
 * Dev Fleet — worktree management page ported to CodexCrew SPA.
...
export default function DevFleetPage() {
    ...
    const ok = await askConfirm(
      ...
      cancellingStage
        ...
        : canRestart
          ...
          : i18nT('pages.devFleetPage.stages_the_code_behind_the_live_dashboard_manual', { cmd: fleet?.manual_restart || 'codexcrew restart' }),
      cancellingStage
  ...
  // Its own state, not an error: the backend found no Codex Crew checkout to
  ...
  function rowButtons(w: Worktree): ReactNode[] {
    if (w.is_main) {
      ...
      if (fleet?.build_pending) {
        ...
        out.push(<Badge key="bp" variant="warn" title={i18nT('pages.devFleetPage.build_pending_restart_gateway_to_apply_codexcrew')}>{i18nT('pages.devFleetPage.build_pending')}</Badge>)
      }
      return out
    }
    const out: ReactNode[] = []
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '16px 84px minmax(0,1fr) 64px 48px 44px 212px', gap: 8, alignItems: 'center', padding: '5px 0', borderTop: '1px solid var(--border)', minHeight: 30, minWidth: 640 } as CSSProperties}>
          ...
          <div style={{ minWidth: 0, display: 'flex', alignItems: 'baseline', gap: 6, whiteSpace: 'nowrap', overflow: 'hidden' } as CSSProperties}>
            ...
            {w.is_staged ? <Badge variant="warn" className="text-[10px] px-1.5 py-0" title={stagedWorktree?.name === w.name
              ? i18nT('pages.devFleetPage.cutover_staged_run_cmd_to_finish_or_cancel', { cmd: fleet?.manual_restart || 'codexcrew restart' })
              : i18nT('pages.devFleetPage.cutover_staged_run_the_restart_command_to_finish', { cmd: fleet?.manual_restart || 'codexcrew restart' })}>{i18nT('pages.devFleetPage.restart_pending')}</Badge> : null}
