...
describe('scanSensitive', () => {
  ...
  it('flags private key blocks, home paths, and loopback URLs', () => {
    ...
    expect(scanSensitive('open http://localhost:5486/settings')).toContain('internal_url')
    ...
    expect(scanSensitive('local http://[::1]:5486/chat')).toContain('internal_url')
  ...
  it('dedupes kinds and stays quiet on ordinary technical prose', () => {
    ...
    expect(scanSensitive('Codex Crew triaged 47 issues and opened PR #7202, CI green.')).toEqual([])
    // Public URLs and relative paths are not findings.
    expect(scanSensitive('https://github.com/mikutellyourworld/Codex-Crew src/index.ts')).toEqual([])
  })
})
