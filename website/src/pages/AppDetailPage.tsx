  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (installed) {
        ...
        // "Codex Crew · Developer Tools" in the list and "codexcrew · Productivity"
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {app.installed && isSelfManaged && !isBuiltin && (
                <>
                  ...
                  {canUninstall && <Btn danger onClick={() => handleAction('uninstall')} disabled={actionLoading === 'uninstall'} title={i18nT('pages.appDetailPage.removes_codexcrew_metadata_only_the_app_itself_is')}><Trash2 size={14} /> {i18nT('pages.appDetailPage.uninstall')}</Btn>}
                </>
              )}
              {app.installed && !isSelfManaged && !isBuiltin && (
                <>
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-4 mt-4">
          ...
          <Card>
            ...
            <div className="grid gap-1.5 mt-2 text-[13px] text-muted">
              ...
              {app.manifest?.minCodexCrewVersion && <div>{i18nT('pages.appDetailPage.min_codexcrew_v')}{app.manifest.minCodexCrewVersion}</div>}
