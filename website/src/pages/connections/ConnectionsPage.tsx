...
export function connectionStateFor(
  ...
  // Codex Crew stores no credential — so a remote OAuth server answers it with 401
...
export default function ConnectionsPage({ servicesEnabled = false }: { servicesEnabled?: boolean } = {}) {
  ...
  const connect = async (provider: ConnectionProvider, existing?: McpServer) => run(provider, 'connect', async () => {
    ...
    if (existing) {
      ...
      // CodexCrew-managed scope. mcpToggle would write the GLOBAL mcp.json
      // (creating an empty stub for codexcrew-scoped names), so use the
      // scope-preserving apply instead: codexcrew on, every observed global
      // scope passed through unchanged (the backend defaults kiroGlobal to
      // false when omitted).
      if (!existing.enabled) {
        const reenable: McpApplyChange = { name: existing.name, codexcrew: true }
        for (const [scope, present] of Object.entries(existing.presence ?? {})) {
          if (scope !== 'codexcrew' && scope.endsWith('Global')) reenable[scope as `${string}Global`] = !!present
        }
