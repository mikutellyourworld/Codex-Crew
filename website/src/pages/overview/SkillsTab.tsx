 ...
 *  through `skills.extra_paths` still reports `source: 'codexcrew'`, but
...
const canControlInjection = (s: Skill) =>
  s.source === 'codexcrew' && !s.always && s.owned !== false
...
function sourceLabel(source: Skill['source']): string | null {
  switch (source) {
    ...
    default: return null  // codexcrew — the default home, no badge needed
  }
}
...
export default function SkillsTab() {
  ...
  // Two groups: skills CodexCrew can edit (codexcrew + kiro-cli's own dirs) and
        ...
        <div className={PANE_SHELL_CLASS}>
          ...
          {showDetail && <div className="flex-1 min-w-0 flex flex-col border border-border rounded-md bg-card overflow-hidden">
            {!selectedSkill ? (
              ...
              <div className="flex flex-col h-full min-h-0">
                {/* Detail header: name, source badge, Edit/Delete (codexcrew only) */}
                ...
                <div className="flex items-center justify-between gap-2 flex-wrap px-4 py-2.5 border-b border-border shrink-0">
                  ...
                  {selectedSkill.source === 'codexcrew' && (
                    <div className="flex gap-2 shrink-0">
