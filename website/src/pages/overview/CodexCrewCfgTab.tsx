...
type CodexCrewAgentCfg = Omit<CodexCrewAgent, 'name'>
interface WorkspaceCfg { dir: string }
interface MemoryStoreCfg { description: string; embedding_provider: string }
interface CodexCrewCfg {
  agents: Record<string, CodexCrewAgentCfg>
...
export default function CodexCrewCfgTab() {
  const provider = useProvider()
  const queryClient = useQueryClient()
  const { data: cfg = null, error: queryErr } = useQuery<CodexCrewCfg>({
  ...
  const patchMut = useMutation({
    ...
    onSuccess: (updated) => { queryClient.setQueryData(['codexcrewConfig'], updated) },
    onError: (e: Error) => {
      setSaveErr(e.message)
      setTimeout(() => setSaveErr(''), 4000)
      queryClient.invalidateQueries({ queryKey: ['codexcrewConfig'] })
      setRev(r => r + 1)
    },
  })
  ...
  return (
    <>
      {/* Agents */}
      <Card>
        <CardTitle><Bot className="lucide-inline" /> {i18nT('pages.overview.kiroCrewCfgTab.codexcrew_agents')} <InfoTip text={i18nT('pages.overview.kiroCrewCfgTab.named_agent_definitions', { label: provider.labels.agentTemplateField.toLowerCase() })} /></CardTitle>
        {agents.length === 0 ? (
      ...
      {/* Subagent Settings */}
      <SubagentSettings cfg={cfg} onSaved={() => queryClient.invalidateQueries({ queryKey: ['codexcrewConfig'] })} />
      ...
      <Card>
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-2 max-[600px]:grid-cols-1">
          ...
          <CfgSelect key={`enforce-${rev}`} label={i18nT('pages.overview.kiroCrewCfgTab.enforce_denied_commands')} path="agent.enforce_denied_commands" value={cfg.agent.enforce_denied_commands ?? 'all'} options={['all', 'codexcrew']} hint={i18nT('pages.overview.kiroCrewCfgTab.immediate_all_enforces_on_every_agent_codexcrew_o')} onSave={save} />
...
function SubagentSettings({ cfg, onSaved }: { cfg: CodexCrewCfg; onSaved: () => void }) {
