...
// A scope key: the core scopes 'codexcrew' / 'kiroGlobal', or a provider scope
...
const DEFAULT_PRESENCE: McpScopePresence = { codexcrew: true, kiroGlobal: false }
 ...
 * kiro-cli holds (Codex Crew keeps no credentials), so a remote OAuth server
  ...
  const apply = useMutation({
    mutationFn: async () => {
      ...
      for (const s of servers) {
        ...
        change.codexcrew = eff.codexcrew
    ...
    <Card>
      ...
      {pendingCount > 0 && (
        <div className="mb-3 p-3 rounded border border-[var(--warn)] bg-warn/20 flex items-center justify-between">
          <div className="text-[13px] text-[var(--warn)]">
            ...
            <span className="ml-2 text-muted">{i18nT('pages.overview.mcpTab.apply_commits_to_codexcrew_mcp_json_provider_glob')}</span>
          </div>
      ...
      {isLoading ? <ContentSkeleton rows={6} /> : (
        ...
        <table ref={attachMcpTable} className="w-full border-collapse table-striped"><thead><tr>
          ...
          <th className="text-left text-muted text-[12px] uppercase tracking-[.04em] px-2.5 py-2 border-b border-border font-medium">{i18nT('pages.overview.mcpTab.codexcrew')}</th>
            ...
            return (
              <tr key={s.name} className={`group/mcprow hover:bg-bg-hover transition-colors align-top ${rowBorder}`} style={{ opacity: pendingUninstall ? 0.5 : 1 }}>
                ...
                <td className="px-2.5 py-2 border-b border-border text-sm whitespace-nowrap">
                  <ScopeBadge
                    label={i18nT('pages.overview.mcpTab.codexcrew')}
                    scope="codexcrew"
                    active={eff.codexcrew}
                    pendingChange={!pendingUninstall && !!p?.scopes && 'codexcrew' in p.scopes}
                    disabled={pendingUninstall}
                    onClick={() => toggleScope(s.name, 'codexcrew', !eff.codexcrew, base)}
                  />
                </td>
                <td className="px-2.5 py-2 border-b border-border text-sm whitespace-nowrap">
