        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-1">
              ...
              <InfoTip text={i18nT('pages.kiroCrewAgentsPage.subdirectory_inside_codex_crew_where_this_workspa')} />
            </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex h-[54px] min-w-0 flex-1 flex-col justify-center">
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            {agent.source && agent.source !== 'codexcrew' && <SourceBadge source={agent.source} />}
          </div>
 ...
 */
function describeCrew(agent: CodexCrewAgent, isDefault: boolean): { text: string; placeholder: boolean } {
...
export default function CodexCrewAgentsPage({ embedded }: { embedded?: boolean } = {}) {
  const provider = useProvider()
  const dispatch = useAppDispatch()
  const navigate = useNavigate()
  const { data: agentsData, refetch: refetchAgents, error: agentsError } = useQuery({
    queryKey: ['codexcrew-agents'],
    queryFn: () => api.codexcrewAgents(),
  })
  ...
  const agents = useMemo<CodexCrewAgent[]>(() => agentsData?.agents || [], [agentsData])
  ...
  const kiroAgentOptions = Array.isArray(installedAgents) ? installedAgents.map((x: { name: string }) => x.name).filter(Boolean) : ['codexcrew']
  ...
  const { data: codexcrewCfg, error: cfgError } = useQuery({
    queryKey: ['codexcrewConfig'],
    queryFn: () => api.codexcrewConfig(),
  })
  const memoryStoreOptions = codexcrewCfg?.memory_stores ? Object.keys(codexcrewCfg.memory_stores) : ['default']
  // The three option lists above fall back to a built-in default when their
  // fetch fails, so without this the editor would offer `default` / `codexcrew`
  ...
  // Starts UNSELECTED, not at the built-in 'codexcrew'. Pre-filling the built-in
  ...
  const openEdit = useCallback((a: CodexCrewAgent) => {
    sheetEpoch.current += 1
    setError(''); setSheetHint('')
    setConfirmDelete(false)
  ...
  const create = () => {
    ...
    // 'codexcrew' default: that default is what silently turns a new crew into an
  ...
  const sharedKind = (a: CodexCrewAgent): SharedKind => {
