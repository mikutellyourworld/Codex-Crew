        ...
        <div ref={dndRow ? setNodeRef : undefined} {...(dndRow ? listeners : {})}
          ...
          <div className="flex-1 min-w-0 overflow-hidden">
            <div className={`session-agent-label ${ROW_META_CLS} font-semibold truncate flex items-center gap-1 ${agentColor}`}>
              ...
              {s.clean_mode
                ? <span className="text-accent" title={i18nT('pages.chatSidebar.clean_agent_only_no_codexcrew_context_or_mcp')}><Droplet size={10} /></span>
                : <>
...
function ChatSidebar({
  ...
  // codexcrewConfig query); recomputes when the slots or the configured count change.
  const { data: mcCfg } = useQuery({ queryKey: ['codexcrewConfig'], queryFn: () => api.codexcrewConfig() })
