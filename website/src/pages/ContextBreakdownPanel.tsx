...
export interface ContextTrace {
  ...
  user_chars: number
  // The model context CodexCrew did NOT inject (kiro-cli's own prompt + tool
...
function ContextBreakdownCard({ trace }: { trace: ContextTrace }) {
  ...
  const codexcrewAdded = Math.max(0, trace.injected_chars - trace.user_chars)
  ...
  // Whole-window summary: aggregate blocks + the estimated non-CodexCrew remainder.
  ...
  return (
    <div className="border border-border bg-card rounded-xl overflow-hidden">
      ...
      <div className="grid grid-cols-3 border-b border-border">
        ...
        />
        <Stat
          label={i18nT('pages.contextBreakdown.strip_codexcrew_added')}
          value={fmtPct(pctOf(codexcrewAdded))}
          sub={i18nT('pages.contextBreakdown.strip_codexcrew_added_sub', { chars: fmtN(codexcrewAdded) })}
        />
        <Stat
