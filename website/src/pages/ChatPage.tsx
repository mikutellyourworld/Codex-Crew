...
import type { CodexCrewAgent } from '../components/AgentSelector'
...
export function ChatHeaderMenu({ activeSlot, agent, onReveal, onRename, mode }: {
  ...
  // Tool Search mode for this session's MCP tools (shared ['codexcrewConfig']
...
export default function ChatPage({ mode, embedded, embedMode, popout, noUrlSync }: { mode?: string; embedded?: boolean; embedMode?: 'chat' | 'sessions'; popout?: boolean; noUrlSync?: boolean } = {}) {
  ...
  const effectiveAgents = useMemo<CodexCrewAgent[]>(() => {
    if (!remoteCrew.isRemote) return installedAgents
    // The peer's roster carries the four fields its picker renders. The rest of
    // CodexCrewAgent describes bindings that only mean something on the machine
 ...
 // forensic attribution. Set on widget pre-fill, consumed
  ...
  const { data: sttCfg } = useQuery({
    queryKey: ['sttConfig'],
    queryFn: () => api.sttConfig() as Promise<{ streaming?: boolean; enabled?: boolean; dictation_panel?: boolean; enter_sends?: boolean; available?: boolean; provider?: string }>,
  })
  ...
  // Same "explicit boolean, undefined means not loaded" reasoning as above, and
  // the same default-on: the dictation panel advertises "Enter to send", so the
  // binding that makes that true is the standard behaviour, not an opt-in.
  const sttEnterSends = sttCfg?.enter_sends !== false
  ...
  const handleFileOpen = useCallback(async (filePath: string, opts?: { replaceId?: string; line?: number; endLine?: number; diffMode?: boolean; canReplace?: () => boolean }) => {
    ...
    try { window.dispatchEvent(new CustomEvent('codexcrew-file-open', { detail: { path: filePath } })) } catch { /* ignore */ }
    if ((window as unknown as { __codexcrewPluginHandlesFiles?: boolean }).__codexcrewPluginHandlesFiles) return
    try {
  ...
  const handleOpenDiff = useCallback((filePath: string, modified: string, original: string) => {
    ...
    try {
      window.dispatchEvent(new CustomEvent('codexcrew-file-open', {
 ...
 // composer instead of auto-submitting. Auto-submitting would be a
  ...
  //   • marker (`codexcrew:preview`, explicit agent intent) → also OPEN the tab,
  ...
  useEffect(() => {
    ...
    window.addEventListener('codexcrew-tool-call', onTool)
    return () => window.removeEventListener('codexcrew-tool-call', onTool)
  }, [dispatch])
