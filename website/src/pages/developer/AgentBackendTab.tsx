...
/**
 * Public backend ids, verbatim from `acp/types.py`. Legacy and edition-specific
 * ids may still arrive from the server and are added dynamically below.
 */
const CODEX = 'codex'
 ...
 */
const NAMED = [CODEX, OPENAI_COMPATIBLE]
 ...
 * dead reaches a screen reader instead of living in visual proximity only. An
 * empty legacy id uses the explicit `kiro` fallback so the DOM id stays valid.
 */
 ...
 *
 * The public core previously exposed only legacy Kiro selection. This control
 * presents the public Codex-first backend set and still accepts edition-specific
 ...
 * `PATCH /api/config/codexcrew` validates against. So the enabled options and the
 ...
 *
 * The component stays in the developer module so the developer page and the
 * normal Settings > Secrets page share one implementation. Changing the backend
 * affects new sessions only, which the control states directly.
 */
export function AgentBackendTab() {
  ...
  const patchMut = useMutation({
    ...
    onSuccess: () => {
      setSaveError('')
      qc.invalidateQueries({ queryKey: ['codexcrewConfig'] })
    },
   ...
   *
   * `?? CODEX` is right for a config that genuinely omits the key because Codex
   * is the shipped public default. It is wrong for a read that FAILED: the value
   * is then unknown, and guessing would paint a backend as selected while the
   * actual configuration is unavailable.
  ...
  const current = cfgQ.data?.agent?.acp_backend ?? CODEX
  ...
  const caveat = (value: string): string => {
    ...
    if (value === OPENAI_COMPATIBLE) return i18nT('pages.developer.agentBackendTab.openai_keys_in_settings')
    return ''
  }
  ...
  const NAME: Record<string, string> = {
    ...
    [OPENAI_COMPATIBLE]: i18nT('pages.developer.agentBackendTab.openai_compatible'),
  }

  const ICON: Record<string, React.ReactNode> = {
    [CODEX]: <Terminal size={14} />,
    [OPENAI_COMPATIBLE]: <Boxes size={14} />,
  }
  ...
  const status = (value: string): string => {
    ...
    if (value === CODEX) return i18nT('pages.developer.agentBackendTab.default_all_features_supported')
