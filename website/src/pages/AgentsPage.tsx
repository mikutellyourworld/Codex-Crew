            ...
            return (
              <div key={s.key}>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <div className="text-[13px] text-muted font-mono">{s.agent && s.agent !== 'codexcrew' ? <span className={`mr-1.5 ${installed.find(a => a.name === s.agent)?.source === 'codexcrew' ? 'text-accent' : 'text-aim'}`}>{s.agent}</span> : null}{shortModel(s.model)}</div>
                </div>
...
export default function AgentsPage({ embedded }: { embedded?: boolean } = {}) {
  ...
  const { data: installed = [], isPending: installedLoading, error: installedError, refetch: refetchInstalled } = useQuery({
    ...
    queryFn: async () => {
      ...
      ;(a as InstalledAgent[]).sort((x, y) => {
        if (x.name === 'codexcrew') return -1; if (y.name === 'codexcrew') return 1
        if (x.name === 'codexcrew-lite') return -1; if (y.name === 'codexcrew-lite') return 1
   ...
   *  (`CodexCrewAgentsPage.tsx`): one shared QueryClient keeps one value per
  ...
  const { data: crewsData, isError: crewsFailed, refetch: refetchCrews } = useQuery<{ agents?: CodexCrewAgent[]; default_agent?: string }>({
  ...
  const blockedBy: 'owner' | 'default' | 'crews' | 'unloaded' | null =
    !listed ? 'owner'
      : (listed.source === 'codexcrew' || listed.source === 'package') ? 'owner'
  ...
  const renderRow = (a: InstalledAgent) => (
    ...
    <Btn
      ...
      /* Named explicitly rather than by its composed text: the metadata line
         ...
         "codexcrew claude-opus-5 3 2" buries the one word that identifies the
        ...
        {installedLoading ? (
          ...
          <div className="card-glow border border-border bg-card rounded-lg mb-4 shadow-sm py-6">
            <EmptyState
              ...
              subtitle={i18nT('pages.agentsPage.templates_come_with_codexcrew_or_with_an_app_you_i')}
            />
          </div>
        ) : (
