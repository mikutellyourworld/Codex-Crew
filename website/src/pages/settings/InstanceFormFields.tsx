...
export const DEFAULT_REMOTE_PORT = '5486'
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      ...
      <label htmlFor={`${idPrefix}-remote-port`} className="flex flex-col gap-1 text-[13px] text-muted">
        ...
        <input id={`${idPrefix}-remote-port`} aria-label={i18nT('pages.settings.instancesPanel.remote_port')} className={inputCls} value={values.remotePort} onChange={e => set('remotePort', e.target.value)} placeholder="5486" inputMode="numeric" />
        <span className="text-[12px] text-muted leading-snug">
      ...
      <label htmlFor={`${idPrefix}-remote-bin`} className="flex flex-col gap-1 text-[13px] text-muted sm:col-span-2">
        {i18nT('pages.settings.instancesPanel.remote_codexcrew_path')} <span className="text-muted-strong">{i18nT('pages.settings.instancesPanel.optional')}</span>
        <input
          id={`${idPrefix}-remote-bin`}
          aria-label={i18nT('pages.settings.instancesPanel.remote_codexcrew_path')}
          className={inputCls}
          value={values.remoteBin}
          onChange={e => set('remoteBin', e.target.value)}
          placeholder={i18nT('pages.settings.instancesPanel.home_you_local_bin_codexcrew_leave_blank_for_stan')}
        />
        <span className="text-[12px] text-muted leading-snug">
          {i18nT('pages.settings.instancesPanel.only_needed_if')} <code className="text-text">{i18nT('pages.settings.instancesPanel.codexcrew')}</code> {i18nT('pages.settings.instancesPanel.is_installed_somewhere_non_standard_on_the_remot')} <code className="text-text">{i18nT('pages.settings.instancesPanel.command_v_codexcrew')}</code>{' '}
          {i18nT('pages.settings.instancesPanel.commonly')} <code className="text-text">{i18nT('pages.settings.instancesPanel.local_bin_codexcrew')}</code>{i18nT('pages.settings.instancesPanel.use_an_absolute_path_no')} <code className="text-text">~</code>).
        </span>
      </label>
    </div>
  )
