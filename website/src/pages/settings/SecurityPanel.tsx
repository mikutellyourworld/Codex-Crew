...
function YoloDurationCard() {
  ...
  const { data, isError: cfgError } = useQuery<KirocrewCfgShape>({ queryKey: ['codexcrewConfig'], queryFn: api.codexcrewConfig })
  ...
  const save = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['codexcrewConfig'] }),
  })
        ...
        {isLoading ? (
          ...
          <div className="flex items-start gap-2.5 py-3 mt-1 rounded-md bg-bg-elevated border border-border px-3">
            ...
            <div>
              ...
              <div className="text-[12px] text-muted mt-0.5 leading-relaxed">{i18nT('pages.settings.securityPanel.no_policy_or_host_profile_restricts_the_host_sur')} <code className="font-mono text-[11px]">{i18nT('pages.settings.securityPanel.codex_crew_security_policy_json')}</code> {i18nT('pages.settings.securityPanel.and_per_surface')} <code className="font-mono text-[11px]">{i18nT('pages.settings.securityPanel.profiles_json')}</code>.</div>
            </div>
          </div>
        ) : (
          <>
 ...
 * since `config.local.json` is user-owned and never written by Codex Crew), and
    ...
    <SettingsSection title={i18nT('pages.settings.securityPanel.live_security_posture')}>
      ...
      <SettingsCard>
        ...
        <StatusRow icon={<Lock size={14} />} label={i18nT('pages.settings.securityPanel.process_sandbox')} value={i18nT('pages.settings.securityPanel.standard')} variant="ok"
          href={`${CODE_BASE}/src/codex_crew/sandbox.py`} />
        <StatusRow
...
function DeniedCommandsSection({ draft, onDraftChange, noteDraft, onNoteDraftChange }: { draft: string; onDraftChange: (next: string) => void; noteDraft: string; onNoteDraftChange: (next: string) => void }) {
  ...
  const confirmBody = !confirm ? '' : confirm.kind === 'disable-all'
    ? i18nT('pages.settings.securityPanel.disabling_all_built_in_denies_removes_codexcrew_s')
    ...
    <SettingsSection title={i18nT('pages.settings.securityPanel.denied_commands')}>
      ...
      >
        ...
        <label className="flex items-center gap-2.5 mt-4 cursor-pointer">
          ...
          <span className="text-[13px] text-text">{i18nT('pages.settings.securityPanel.i_understand_this_weakens_codexcrew_s_protection')}</span>
        </label>
      </Modal>
    </SettingsSection>
  )
    ...
    <SettingsSection title={i18nT('pages.settings.securityPanel.defense_in_depth_architecture')}>
      <SettingsCard>
        <div className="text-[12px] text-muted mb-3 leading-relaxed">
          {i18nT('pages.settings.securityPanel.codexcrew_implements_6_security_layers_each_layer')}
        </div>
        <div className="divide-y divide-border">
...
export function SecurityPanel({ basePath }: { basePath?: string } = {}) {
  ...
  const { data: cfg, isError: cfgError } = useQuery<KirocrewCfgShape>({ queryKey: ['codexcrewConfig'], queryFn: api.codexcrewConfig })
