...
vi.mock('../developer/AgentBackendTab', () => ({
  AgentBackendTab: () => null,
}))
vi.mock('./OpenAIProfilesPanel', () => ({
  OpenAIProfilesPanel: () => <div>OpenAI-Compatible Providers</div>,
}))
...
beforeEach(() => {
  ...
  localStorage.setItem('codex_crew_token', 'test-token')
  installFetch()
})
...
describe('SecretsPanel', () => {
  it('renders the section heading and description', async () => {
    ...
    expect(await screen.findByText('Agent backend')).toBeInTheDocument()
  })
...
describe('SecretsPanel session key', () => {
   ...
   * `localStorage['codex_crew_token']` — a key nothing in the app ever writes —
  ...
  it('sends the dashboard:ui session key on both the list GET and a mutating POST', async () => {
    ...
    localStorage.setItem('codex_crew_token', 'SHOULD-BE-IGNORED')
    installFetch()
