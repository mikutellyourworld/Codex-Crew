  ...
  return (
    <>
      ...
      <SettingsCard index={cardIndex}>
        ...
        {/* Paired with the panel toggle because it is the panel's own "Enter to
            send" hint this makes true. Starting a recording leaves focus on the
            mic button, so a composer-scoped Enter handler never fires — this
            binds Enter at the document level while dictation is live, the same
            way Escape already cancels from anywhere. Off means Enter inserts a
            newline while recording instead. */}
        <SettingsToggle label={i18nT('pages.settings.sttSettings.enter_sends_while_dictating')} description={i18nT('pages.settings.sttSettings.let_enter_send_the_message_while_dictation_is_live')} checked={stt.enter_sends !== false} onChange={v => set({ enter_sends: v })} disabled={saving} />
