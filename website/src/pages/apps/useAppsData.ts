...
export default function useAppsData(): AppsData {
  ...
  const browseApps: RegistryApp[] = useMemo(() => {
    ...
    return registry
      .filter(r => {
        ...
        // `minClientVersion`-aware "needs a newer Codex Crew" state exists to say
  ...
  const sources: SourceRow[] = useMemo(() => {
    ...
    if (builtinCount > 0) rows.push({ name: '__builtin__', label: i18nT('pages.appsPage.built_in_codexcrew'), count: builtinCount, builtin: true })
    ...
    if (coreCount > 0) rows.push({ name: '__core__', label: i18nT('pages.appsPage.codexcrew_registry'), count: coreCount, builtin: true })
    return rows
