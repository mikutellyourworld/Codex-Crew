 ...
 *
 * The optional crew-avatar art is NOT duplicated here. `crew-avatar-mark.svg` is imported
 ...
 *
 * What is taken from the asset is GEOMETRY, not presentation. `crew-avatar-mark.svg`
...
import markSvg from '../assets/crew-avatar-mark.svg?raw'
...
export function markPaths(svg: string): [string, string, string] {
  ...
  if (found.length !== 3) {
    throw new Error(
      `crew-avatar-mark.svg must hold exactly 3 paths (body, eye, eye); found ${found.length}`,
    )
  }
