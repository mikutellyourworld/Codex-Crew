 ...
 * `codex_crew/effort.py` so the UI and server agree on levels and per-model
...
/** Transports whose active backend may accept a reasoning-effort level. */
export const REASONING_EFFORT_PROVIDERS = new Set(['acp'])

/**
 * Per-model effort capability — mirrors the backend `model_supports_effort`
 * (codex_crew/effort.py): effort is available on Fable/Opus/Sonnet and GPT-5.x
