 ...
 */
const mc = (window as { codexcrew?: { isElectron?: boolean; platform?: string; linuxFrameless?: boolean } }).codexcrew
 ...
 * above) so a test can stub `window.codexcrew` per-case, exactly as `pathForFile`
 * does.
 */
export function electronPlatform(): string | undefined {
  return (window as { codexcrew?: { platform?: string } }).codexcrew?.platform
}
 ...
 * `window.codexcrew` per-case without import-order coupling.
 */
export function pathForFile(file: File): string {
  const k = (window as { codexcrew?: { getPathForFile?: (f: File) => string } }).codexcrew
  try {
 ...
 */
export const DESKTOP_APP_REQUIRED_LABEL = 'Requires the CodexCrew desktop app'
