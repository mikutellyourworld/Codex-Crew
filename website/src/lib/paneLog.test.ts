...
describe('safePaneUrl', () => {
  ...
  it('marks a non-token query without echoing it', () => {
    expect(safePaneUrl('http://localhost:5486/?foo=1')).toBe('http://localhost:5486/?<query>')
  })
  ...
  it('passes a query-less URL through untouched', () => {
    expect(safePaneUrl('http://localhost:5486/')).toBe('http://localhost:5486/')
  })
})
...
describe('frameDocumentState', () => {
  ...
  it('redacts a token even when the frame is readable', () => {
    const el = {
      contentWindow: { location: { href: 'http://localhost:5486/?token=abc' } },
    } as unknown as HTMLIFrameElement
    expect(frameDocumentState(el)).toBe('http://localhost:5486/?token=<redacted>')
  })
