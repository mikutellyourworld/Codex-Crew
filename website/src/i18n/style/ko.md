...
- **Half-width stays inside code**: commands, paths, filenames and extensions
  (`~/.codex-crew`, `.ics`), identifiers and config keys
...
- **A Latin run and the Hangul that follows it take a space** when that Hangul is
  a noun rather than a 조사: `MCP 서버`, `Git 저장소`, `Codex Crew 실행`. Never
...
beginning with a particle draws it detached from its noun — `Codex Crew 을(를)
...
`CodexCrew` / `Codex Crew`, `Kiro`, `Slack`, `Discord`, `MCP`, `GitHub`, `Playwright`, etc.
