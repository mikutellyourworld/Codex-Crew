export interface StatusData {
   ...
   * `src/codex_crew/release_channel.py`) rather than leaving the dashboard to
...
export interface Skill {
  ...
  /** False when the SKILL.md lives outside the directory Codex Crew owns (e.g. a
   ...
   *  native ``skill://`` loader (it may still load via CodexCrew text-injection). */
...
export interface DiscoveredMcpServer {
  ...
  /** "" if unknown. */
  repo_url: string
  /** Cross-referenced against CodexCrew's configured servers. */
  installed: boolean
...
export interface McpServer {
  ...
  /** True when the entry lives in CodexCrew's own mcp.json — the scope the
   *  Edit JSON action reads and writes (consent-disabled rows included). */
  codexcrewManaged?: boolean
 ...
 *   the agent spec's own servers, not just the ones Codex Crew injects.
 */
export interface McpSessionReport {
  /** Server names Codex Crew put on the wire for this session. */
  configured: string[]
...
export interface ArtifactPublication {
  ...
  provider?: string
  /** Sync authority: 'mirror' (CodexCrew-authoritative) | 'live' (remote CRDT). */
  collab_mode?: 'mirror' | 'live'
  visibility: 'PRIVATE' | 'SHARED' | 'PUBLIC'
  shared_with: string[]
  auto_sync: boolean
  last_synced_codexcrew_version: number
  /** Maps CodexCrew version (as string) -> provider version number. */
