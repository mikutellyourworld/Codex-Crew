 ...
 * `$CODEXCREW_HOME/config.json`. Config is read live, so no gateway restart is
 * needed.
 *
 ...
 * from one predicate and one `['codexcrewConfig']` cache entry is what keeps them
...
/** Live flag value, off the shared `['codexcrewConfig']` query cache. */
export function useConnectionsUiEnabled(): boolean {
  const { data } = useQuery({ queryKey: ['codexcrewConfig'], queryFn: () => api.codexcrewConfig() })
