...
/** Narrow view of GET /api/config/codexcrew — only the fields the composer needs
...
export class AcpAdapter implements ProviderAdapter {
  ...
  async resolveModel(agentName: string): Promise<string> {
    ...
    // chain is four tiers deep (the CodexCrew agent's own model, the bound kiro
    ...
    //
    // `agentName` is a CodexCrew agent name (a "crew"), not a kiro agent
  ...
  /** CodexCrew's configured default model (Settings → Chat → Default Model).
  ...
  async resolveDefaultModel(): Promise<string> {
    try {
      const c = (await api.codexcrewConfig()) as KirocrewAgentConfig
  ...
  /** CodexCrew's configured default reasoning effort (Settings → Chat). '' means
  ...
  async fetchUsage(): Promise<NormalizedUsage> {
    ...
    const today = s.today || {}
    const thisWeek = s.this_week || {}
    const thisMonth = s.this_month || {}
    const b = data.billing || {}
    return {
      sessions: {
        total: s.total_sessions ?? 0,
        today: { sessions: today.sessions ?? 0, messages: today.messages ?? 0, toolCalls: today.tool_calls ?? 0 },
        thisWeek: { sessions: thisWeek.sessions ?? 0, messages: thisWeek.messages ?? 0, toolCalls: thisWeek.tool_calls ?? 0 },
        thisMonth: { sessions: thisMonth.sessions ?? 0, messages: thisMonth.messages ?? 0, toolCalls: thisMonth.tool_calls ?? 0 },
