// ACP is the single transport layer, with Codex and named
// OpenAI-compatible profiles selected as harnesses behind it. This remains a
// single-member union so the adapter interface below type-checks for consumers.
...
export interface ProviderAdapter {
  ...
  /** The model a NEW session on this CodexCrew agent would run on ('' = the
   *  backend picks). Takes a CodexCrew agent name, not a kiro agent template:
