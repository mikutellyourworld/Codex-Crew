...
export const SHELL_COMMANDS = [
  { shell: 'macos', command: 'export CODEXCREW_TELEMETRY_DISABLED=1' },
  { shell: 'powershell', command: "$env:CODEXCREW_TELEMETRY_DISABLED = '1'" },
  { shell: 'cmd', command: 'set CODEXCREW_TELEMETRY_DISABLED=1' },
 ...
 * `codexcrew telemetry disable` persists), so the choice survives restarts and
      ...
      {!govOverride && envOverride && (
        <p className="text-[12px] text-muted mt-1">
          <Trans
            ...
            components={{
              settingRef: <SettingRef kind="env" configKey={statusQ.data?.env_var ?? 'CODEXCREW_TELEMETRY_DISABLED'} envIntent="unset" />,
            }}
          />
        </p>
      )}
