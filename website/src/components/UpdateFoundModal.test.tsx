...
function withNudgeConfig(record: Record<string, unknown> | undefined) {
  mockedApi.codexcrewConfig.mockResolvedValue({ dashboard: { update_nudge: record } } as never)
}
...
beforeEach(() => {
  mockedApi.codexcrewConfig.mockReset()
...
describe('UpdateFoundModal — desktop source', () => {
  it('renders nothing without an update state', async () => {
    ...
    expect(mockedApi.codexcrewConfig).not.toHaveBeenCalled()
  })
  ...
  it('stays closed until the persisted record has loaded', async () => {
    ...
    mockedApi.codexcrewConfig.mockReturnValue(new Promise(r => { resolve = r }) as never)
    await mount(found)
