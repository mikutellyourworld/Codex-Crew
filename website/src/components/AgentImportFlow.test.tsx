...
describe('AgentImportFlow', () => {
  ...
  it('reviews merge-only handling and sends the selected source categories', async () => {
    ...
    expect(
      screen.getByText(/existing Codex Crew setup is never overwritten by default/i),
    ).toBeInTheDocument()
  ...
  it('preserves the actionable auth-recovery copy instead of offering a retry', async () => {
    ...
    vi.mocked(api.onboardingImportApply).mockRejectedValueOnce(
      new ApiError(
        403,
        'Session expired. Run codexcrew token, then use the banner to sign back in.',
    ...
    const alert = await screen.findByRole('alert')
    expect(alert).toHaveTextContent('Session expired. Run codexcrew token')
