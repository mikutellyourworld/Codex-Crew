 ...
 *
 * Also reports slot titles to the plugin via `window.__codexcrewReportSlotTitle`
 ...
 */
export default function CodexCrewNavBridge() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('codexcrew-soft-navigate', handler)
    return () => window.removeEventListener('codexcrew-soft-navigate', handler)
  }, [navigate])

  // Report slot titles to the plugin whenever slots change
  useEffect(() => {
    const fn = (window as unknown as { __codexcrewReportSlotTitle?: (slug: string, title: string) => void }).__codexcrewReportSlotTitle
