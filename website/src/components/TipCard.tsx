...
// Tip docs live in the repo at src/codex_crew/docs/ (same base the Security and
// Discord settings panels link to).
const DOCS_BASE = 'https://github.com/mikutellyourworld/Codex-Crew/blob/main/src/codex_crew/docs'
...
export function useTipTrigger(isRunning: boolean, suppressed = false, slotKey: string | null = null, blocked = false) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const lastShown = safeGetItem('codexcrew.tips.lastShownAt')
  ...
  useEffect(() => {
    if (tip && enabled && isRunning && !suppressed && !blocked && !shownThisTurnRef.current) {
      ...
      safeSetItem('codexcrew.tips.lastShownAt', String(Date.now()))
