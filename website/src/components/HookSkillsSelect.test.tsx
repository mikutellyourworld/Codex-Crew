...
const mockSkills = [
  { key: 'codexcrew-dev/prepare-pr', name: 'prepare-pr', description: 'PR workflow' },
...
describe('HookSkillsSelect', () => {
  ...
  it('renders selected skills as read-only chips with names from catalog', async () => {
    renderWithProviders(
      <HookSkillsSelect selected={['codexcrew-dev/prepare-pr', 'dev-fleet/pod-e2e']} onChange={vi.fn()} />,
    )
    await waitFor(() => {
  ...
  it('renders chips without inline buttons — removal lives in the dropdown Selected section', async () => {
    renderWithProviders(
      <HookSkillsSelect selected={['codexcrew-dev/prepare-pr']} onChange={vi.fn()} />,
    )
  ...
  it('renders multiple selected chips maintaining order', async () => {
    renderWithProviders(
      <HookSkillsSelect
        selected={['widgets/theme-pack', 'codexcrew-dev/prepare-pr', 'dev-fleet/pod-e2e']}
        onChange={vi.fn()}
      />,
    )
    await waitFor(() => {
