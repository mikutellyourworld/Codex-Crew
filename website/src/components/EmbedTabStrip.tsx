...
function loadTabs(activeSlot: string | null): { tabs: Tab[]; index: number } {
  // Priority: sessionStorage > window.__codexcrewTabs > activeSlot > empty
  try {
  ...
  const injected = w.__codexcrewTabs
  if (injected?.length) return { tabs: injected.map(s => ({ slug: s })), index: w.__codexcrewActiveTabIndex ?? 0 }
...
export default function EmbedTabStrip() {
  ...
  const persist = useCallback((newTabs: Tab[], newIndex: number) => {
    ...
    window.dispatchEvent(new CustomEvent('codexcrew-tab-update', {
  ...
  useEffect(() => {
    ...
    window.addEventListener('codexcrew-tab-state', handler)
    return () => window.removeEventListener('codexcrew-tab-state', handler)
  }, [persist, navigate])
