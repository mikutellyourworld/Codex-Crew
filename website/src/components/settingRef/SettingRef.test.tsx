...
describe('SettingRef component', () => {
  ...
  describe('kind: env (set mode - default)', () => {
    it('renders a button (popover trigger) with env var name', () => {
      const { container } = renderRef({ configKey: 'CODEXCREW_EMBED_MODEL_PATH', kind: 'env' })
      const button = container.querySelector('button')
      expect(button).not.toBeNull()
      expect(container.textContent).toContain('CODEXCREW_EMBED_MODEL_PATH')
    })

    it('does not render a Link', () => {
      const { container } = renderRef({ configKey: 'CODEXCREW_DEBUG', kind: 'env' })
      const link = container.querySelector('a')
      expect(link).toBeNull()
    })
    ...
    it('shows set description text', () => {
      const { container } = renderRef({ configKey: 'MY_VAR', kind: 'env' })
      const bodyText = openPopover(container)
      expect(bodyText).toContain('Set this environment variable before starting Codex Crew.')
    })
  })
  ...
  describe('kind: env (unset mode)', () => {
    ...
    it('shows unset description text', () => {
      const { container } = renderRef({ configKey: 'MY_VAR', kind: 'env', envIntent: 'unset' })
      const bodyText = openPopover(container)
      expect(bodyText).toContain('Unset this environment variable before starting Codex Crew.')
    })
  })
