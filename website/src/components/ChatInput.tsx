...
interface ChatInputProps {
  ...
  /**
   * Let Enter send while dictation is live, bound at the document level so it
   * works with focus on the mic button (stt.enter_sends, default on). Off makes
   * Enter insert a newline mid-dictation instead.
   */
  voiceEnterSends?: boolean
...
function ChatInput({
  ...
  // Enter SENDS while dictation is live, from anywhere — the mirror of the
  // Escape handler above, and for exactly the same reason: starting a recording
  // means clicking the mic button, so focus sits on that button and the
  // textarea's own onKeyDown never fires. The dictation panel advertises
  // "Esc to cancel, Enter to send"; Escape was bound at the document level to
  // make its half true and Enter was not, so the hint was half a lie — the key
  // did nothing and the message sat there unsent.
  //
  // Gated on `voiceEnterSends` (stt.enter_sends, default on) so a user who wants
  // Enter to insert a newline mid-dictation can have that instead.
  //
  // The same yields as Escape, for the same reasons: BUBBLE phase so an inner
  // control consumes the key first, `defaultPrevented` to respect one that did,
  // the three hand-rolled picker refs that close without preventing default, and
  // the `[role="dialog"]` probe so a modal/palette owns Enter while it is up.
  // Additionally it ignores an Enter the textarea itself will handle: that path
  // already sends, and consuming the key here too would send twice.
  useEffect(() => {
    if (!voiceRecording || !voiceEnterSends) return
    const handler = (e: KeyboardEvent) => {
      // Honour the user's send binding rather than hardcoding bare Enter, so
      // ctrl-enter mode does not gain a second, looser way to send.
      const isSendKey = sendOnEnter === 'ctrl-enter'
        ? (e.key === 'Enter' && (e.metaKey || e.ctrlKey))
        : (e.key === 'Enter' && !e.shiftKey && !e.metaKey && !e.ctrlKey && !e.altKey)
      if (!isSendKey || e.isComposing || e.defaultPrevented) return
      // The composer already owns this key when it has focus.
      ...
      e.preventDefault()
      e.stopPropagation()
      // Gated on `connected` to match the Send button and the textarea path —
      // send() bails when offline, and clearing the draft there would lose it.
      if (connected) fireComposer()
    }
    document.addEventListener('keydown', handler)
    return () => document.removeEventListener('keydown', handler)
  }, [voiceRecording, voiceEnterSends, sendOnEnter, connected, fireComposer])
