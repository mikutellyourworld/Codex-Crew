...
//
// Forcing matters because Codex Crew no longer performs setup — the user installs
      ...
      <div className={PANEL_CLASS}>
        <ShellAside
          copy={{
            ...
            panelBody:
              asideBody
              || i18nT('components.kiroPrerequisiteGate.install_kiro_cli_sign_in_once_and_codex_crew_will'),
            panelFootnote: i18nT(
      ...
      <>
        ...
        <p className="mt-3 max-w-lg text-sm leading-relaxed text-muted">
          {authRequired
            ...
            : i18nT('components.kiroPrerequisiteGate.ask_the_codex_crew_owner_to_install_kiro_cli_and')}
        </p>
...
const SETUP_COMPLETE_KEY = 'codexcrew:kiro-setup-complete'
...
const SANDBOX_DOCS_URL =
  'https://github.com/mikutellyourworld/Codex-Crew/blob/main/docs/guides/install.md' +
 ...
 * also offered `aa-exec -p codexcrew-userns` for a hand-started gateway, which is
      ...
      return (
        <ul className="mt-2 list-none space-y-3">
          <li className="text-sm leading-relaxed text-muted">
            ...
            <CopyCommand>
              <code>codexcrew service install</code>
            </CopyCommand>
          </li>
        </ul>
      )
...
function SandboxRemedy({ remedy, transient }: { remedy: string; transient: boolean }) {
  ...
  return (
    <div className="mt-4 w-full max-w-lg text-left">
      ...
      <p className="mt-2 text-sm leading-relaxed text-muted">
        {i18nT('components.kiroPrerequisiteGate.run_codexcrew_doctor_on_the_gateway_host_for_a_ful')}
      </p>
      <CopyCommand>
        <code>codexcrew doctor</code>
      </CopyCommand>
      <a
...
function SandboxUnavailable({
  ...
  const body =
    failureKind === 'transient'
      ...
      : failureKind === 'foreign_sandbox'
        ? i18nT('components.kiroPrerequisiteGate.another_sandbox_already_confines_codex_crew_so_it')
  ...
  return (
    <SetupShell
      ...
      asideBody={i18nT('components.kiroPrerequisiteGate.codex_crew_isolates_the_agent_in_an_os_level_sand')}
      footer={
        <Btn type="button" disabled={retrying} onClick={onRetry}>
          <RefreshCw className={`lucide-inline ${retrying ? 'animate-spin' : ''}`} />
          {i18nT('components.kiroPrerequisiteGate.check_again')}
...
function CliOutdated({
  ...
  // subcommand Codex Crew launches every session through — so it would fail at
  // session-create rather than here. The remedy is an UPDATE in place, not a
  // reinstall, and unlike the install/sign-in steps Codex Crew CAN run this one
  ...
  return (
    ...
    >
      <>
        ...
        {/* Codex Crew runs the update for the user via the button below, but the
...
function AgentSpecsMissing({
  ...
  return (
    <SetupShell
      ...
      asideBody={i18nT('components.kiroPrerequisiteGate.codex_crew_installs_the_agent_specs_kiro_cli_load')}
    >
      <>
        <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-danger/10 text-danger">
          <AlertTriangle className="lucide-inline" />
        ...
        <h1 className="mt-2 text-3xl font-bold tracking-tight text-text-strong">
          {i18nT('components.kiroPrerequisiteGate.codex_crew_s_agent_specs_are_not_installed')}
        </h1>
        <p className="mt-3 max-w-lg text-sm leading-relaxed text-muted">
          {i18nT('components.kiroPrerequisiteGate.codex_crew_writes_its_own_agent_specs_where_kiro')}
        </p>
        <div className="mt-5 w-full max-w-lg text-left">
          <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-muted">
  ...
  return (
    ...
    >
      <>
        ...
        <h1 className="mt-2 text-3xl font-bold tracking-tight text-text-strong">
          {i18nT('components.kiroPrerequisiteGate.kiro_cli_will_not_load_codex_crew_s_agent_specs')}
        </h1>
        <p className="mt-3 max-w-lg text-sm leading-relaxed text-muted">
        ...
        <p className="mt-4 max-w-lg text-[13px] leading-relaxed text-muted">
          {i18nT('components.kiroPrerequisiteGate.repair_rewrites_the_specs_codex_crew_owns')}
        </p>
        <ul className="mt-3 w-full max-w-lg list-none space-y-2 text-left">
          ...
          <li className="text-sm leading-relaxed text-muted">
            ...
            <CopyCommand>
              <code>codexcrew setup --agent-only --clean</code>
            </CopyCommand>
          </li>
        </ul>
...
export default function KiroPrerequisiteGate({ children }: { children: ReactNode }) {
  ...
  // Codex Crew's own agent specs are absent, so kiro-cli answers every
  ...
  // so `--agent codexcrew` resolves to the default agent with none of Codex Crew's
  ...
  // update the CLI in place, which Codex Crew runs for the user here. Ordered
        ...
        <>
          <div className="mb-7">
            ...
            <p className="mt-2 max-w-xl text-sm leading-relaxed text-muted">
              {i18nT('components.kiroPrerequisiteGate.codex_crew_uses_kiro_cli_as_its_agent_engine_comp')}{' '}
          ...
          <Card className={!status.installed ? 'border-accent/60 shadow-[0_10px_35px_var(--accent-glow)]' : ''}>
            ...
            {/* A link, not a button: Codex Crew does not install Kiro CLI. Kiro's
          ...
          <Card className={status.installed && !status.authenticated ? 'border-accent/60 shadow-[0_10px_35px_var(--accent-glow)]' : ''}>
            ...
            {/* Rendered VERBATIM from the backend constants, never catalog
                ...
                Codex Crew does not run them; the footer's Check again reads the
                result.
                ...
                guess. Codex Crew does not detect which one applies — that would
