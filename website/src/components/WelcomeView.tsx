...
export default function WelcomeView({
  ...
  // welcome screen matches the active theme. Falls back to the terminal mark
  ...
  const brandMark = brandLogo
    ...
    : <TerminalMascot size={64} className="drop-shadow-lg shrink-0 animate-float" />
    ...
    <div className="flex flex-col items-center w-full gap-6 px-8">
      ...
      {mode === 'orchestrator' && (
        <button
          ...
          onClick={() => setInput('Create a plan to analyze Codex Crew code package and report file count by major components')}
        >
          {i18nT('components.welcomeView.try_create_a_plan_to_analyze_codexcrew_code_packa')}
        </button>
      )}
          ...
          {anonOpen && createPortal(
            ...
            >
              ...
              {/* Clean is a peer option in this group, but it is NOT a memory
                  ...
                  the agent runs with its own identity only, no CodexCrew context
              ...
              {onToggleClean && (
                ...
                >
                  ...
                  <div className="text-[11px] text-muted leading-snug">{i18nT('components.welcomeView.agent_only_no_codexcrew_context_or_mcp')}</div>
                </button>
              )}
            </div>,
            document.body
