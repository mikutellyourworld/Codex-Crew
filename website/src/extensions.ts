 ...
 * `extensions.tsx` (in its own repo, at `$CODEXCREW_EDITION_DIR`) is where the
 ...
 *
 * The core OWNS this file and imports the `virtual:codexcrew-edition` module,
 * which the `editionExtensionPlugin` in `vite.config.ts` resolves to:
 *   - an INERT empty module in the stock OSS build (`CODEXCREW_EDITION_DIR`
 *     unset) — the stock build registers nothing, byte-identical to no seam;
 *   - the downstream edition's OWN composition root
 *     (`$CODEXCREW_EDITION_DIR/extensions.tsx`) when that env var points at an
...
// the edition's composition root when CODEXCREW_EDITION_DIR is set.
import 'virtual:codexcrew-edition'
