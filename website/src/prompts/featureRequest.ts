export const FEATURE_REQUEST_URL = 'https://github.com/mikutellyourworld/Codex-Crew/issues/new'
...
export const FEATURE_REQUEST_PROMPT_FALLBACK = [
  ...
  '`gh label list --repo mikutellyourworld/Codex-Crew --limit 100`',
  ...
  "3. Direct creation via `gh issue create --repo mikutellyourworld/Codex-Crew --title \"$TITLE\" --body-file <file>` with one `--label '<name>'` flag per chosen label, single-quoted so a `$` or backtick in a label name stays literal (needs gh auth; fall back to option 2 on auth errors).",
  '',
