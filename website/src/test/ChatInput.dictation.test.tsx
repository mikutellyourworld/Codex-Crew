...
describe('ChatInput — Enter sends while dictating', () => {
  /**
   * The reported bug: the dictation panel says "Esc to cancel, Enter to send",
   * Escape works from anywhere (document-level listener) and Enter did not,
   * because Enter was only bound to the textarea's own onKeyDown. Starting a
   * recording means clicking the mic button, so the moment focus is anywhere but
   * the composer — the mic button, the device picker in the panel, a stray click
   * — the advertised key did nothing and the message sat there unsent.
   *
   * Focus is moved OFF the textarea in each case below on purpose: that is the
   * condition the textarea-scoped handler cannot cover, so a test that left
   * focus in the composer would pass with the fix reverted.
   */
  const dictating = {
    voiceRecording: true,
  ...
  it('sends on Enter when focus is not in the composer', () => {
    const onSend = vi.fn()
    renderWithProviders(
      <ChatInput {...base} onSend={onSend} value="ship it" {...dictating} voiceEnterSends />,
    )
    // The panel focuses the composer on mount; take focus away to reproduce.
  ...
  it('does nothing on Enter when the setting is off', () => {
    const onSend = vi.fn()
    renderWithProviders(
      <ChatInput {...base} onSend={onSend} value="ship it" {...dictating} voiceEnterSends={false} />,
    )
    ;(document.activeElement as HTMLElement | null)?.blur()
  ...
  it('ignores Shift+Enter, which is the newline binding, not the send binding', () => {
    const onSend = vi.fn()
    renderWithProviders(
      <ChatInput {...base} onSend={onSend} value="ship it" {...dictating} voiceEnterSends />,
    )
  ...
  it('does not bind Enter when no recording is in flight', () => {
    // Otherwise a stray Enter anywhere in the dashboard would send the draft.
    const onSend = vi.fn()
    renderWithProviders(<ChatInput {...base} onSend={onSend} value="ship it" voiceEnterSends />)
    ;(document.activeElement as HTMLElement | null)?.blur()
  ...
  it('yields Enter to a dialog that is up', () => {
    // Escape already defers to the topmost dismissible surface; Enter must too,
    // or confirming a modal would also fire the composer underneath it.
    const onSend = vi.fn()
    renderWithProviders(
      <ChatInput {...base} onSend={onSend} value="ship it" {...dictating} voiceEnterSends />,
    )
