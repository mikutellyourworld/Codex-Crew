...
describe('applyBrandingToHtml', () => {
  it('patches the title text', () => {
    ...
    expect(out).not.toContain('<title>Codex Crew</title>')
  })

  it('preserves attributes on a future <title> tag', () => {
    const out = applyBrandingToHtml('<title lang="en">Codex Crew</title>', { title: 'Acme Crew' })
...
describe('verifyOverlayBytes', () => {
  ...
  it('throws loudly when a dist file has stock bytes (precedence flipped)', () => {
    ...
    const readFile = fakeFs({
      'edition/public/manifest.json': '{"name":"Acme"}',
      'dist/manifest.json': '{"name":"Codex Crew"}',
    })
    expect(() =>
      verifyOverlayBytes({
        distDir: 'dist',
