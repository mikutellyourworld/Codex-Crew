...
describe('pickFeatured', () => {
  ...
  it('fills remaining slots from fallback when fewer than 3 flagged', () => {
    const apps = [
      ...
      app({ name: 'verified', author: 'codexcrew' }),
      app({ name: 'plain' }),
    ]
...
describe('provenance helpers', () => {
  it('labels built-ins, tagged registries, and core entries', () => {
    ...
    expect(sourceLabel({})).toBe('Codex Crew registry')
  })

  it('verifies built-ins and codexcrew-authored core apps only', () => {
    expect(isVerified({ origin: 'builtin', author: 'x' })).toBe(true)
    expect(isVerified({ author: 'CodexCrew' })).toBe(true)
    expect(isVerified({ author: 'random' })).toBe(false)
  })

  it('never lets an EXTERNAL registry self-award the verified mark', () => {
    // An external app.json claiming author "CodexCrew" must not be badged as
    // first-party — the badge sits next to an Install that runs setup code
    // with gateway privileges.
    expect(isVerified({ author: 'CodexCrew', _registry: 'evil-registry' })).toBe(false)
    expect(isVerified({ author: 'codexcrew', _registry: 'kirodotdev-labs' })).toBe(false)
  })
...
describe('server-computed trust fields (issue #580)', () => {
  it('isVerified prefers the server verified field over client derivation', () => {
    ...
    expect(isVerified({ author: 'CodexCrew', verified: false })).toBe(false)  // brand-ok: author-spoof fixture
  ...
  it('sourceLabel prefers the server provenance field', () => {
    ...
    expect(sourceLabel({ provenance: 'official', origin: 'builtin' })).toBe('Codex Crew registry')
  ...
  it('sourceLabel still accepts the pre-migration "core" spelling', () => {
    ...
    expect(sourceLabel({ provenance: 'core', origin: 'builtin' })).toBe('Codex Crew registry')
    expect(sourceLabel({ provenance: 'core' })).toBe('Codex Crew registry')
  })
  ...
  it('legacy rows (older gateway, no server fields) derive exactly as before', () => {
    ...
    expect(isVerified({ author: 'CodexCrew' })).toBe(true)  // brand-ok: author-spoof fixture
    expect(isVerified({ origin: 'builtin', author: 'x' })).toBe(true)
    expect(isVerified({ author: 'random' })).toBe(false)
    expect(sourceLabel({ origin: 'builtin' })).toBe('Built-in')
    expect(sourceLabel({})).toBe('Codex Crew registry')
