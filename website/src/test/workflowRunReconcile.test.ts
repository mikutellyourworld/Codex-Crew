...
function liveRun(store: Store, run_id = 'wf_000025', session_key = 'dashboard:chat-1') {
  store.dispatch(sseWorkflowEvent({ run_id, session_key, type: 'run_started', data: { name: 'Codex Crew perf investigation' } }))
  store.dispatch(sseWorkflowEvent({ run_id, type: 'phase_started', data: { title: 'synthesize' } }))
  store.dispatch(sseWorkflowEvent({ run_id, type: 'log', data: { message: 'Starting Codex Crew performance investigation' } }))
}
...
describe('reconcileWorkflowRuns', () => {
  ...
  it('clears a spinner left behind by a missed terminal frame', () => {
    ...
    store.dispatch(reconcileWorkflowRuns([
      { run_id: 'wf_000025', name: 'Codex Crew perf investigation', status: 'finished', session_key: 'dashboard:chat-1' },
    ]))
  ...
  it('lets the live stream own progress, filling only what never arrived', () => {
    ...
    expect(runs(store)['wf_000025']).toMatchObject({ phase: 'synthesize', lastLog: 'Starting Codex Crew performance investigation' })
