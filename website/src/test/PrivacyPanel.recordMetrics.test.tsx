...
const collection = (over: Record<string, unknown> = {}) => ({
  enabled: false,
  env_pinned: false,
  env_var: 'CODEXCREW_TELEMETRY',
  overlay_override: false,
  otlp_configured: false,
  metrics_dir: '/home/u/.codex-crew/metrics',
  ...over,
})
...
describe('PrivacyPanel metric-recording switch', () => {
  ...
  it('disables itself and names the variable when an env var pins the setting', async () => {
    mount({ env_pinned: true, env_var: 'CODEXCREW_TELEMETRY' })
    const sw = await screen.findByRole('switch', { name: LABEL })
    ...
    // CODEXCREW_TELEMETRY_DISABLED shell commands, which a bare variable-name
    // match would hit instead.
    expect(screen.getByText(/CODEXCREW_TELEMETRY environment variable is pinning/)).toBeInTheDocument()
    await userEvent.click(sw)
  ...
  it('shows one note, not two, when both the env var and an overlay are set', async () => {
    ...
    expect(
      await screen.findByText(/CODEXCREW_TELEMETRY environment variable is pinning/),
    ).toBeInTheDocument()
  ...
  it('shows the egress fact even when an env var also pins the switch', async () => {
    ...
    mount({ enabled: true, env_pinned: true, env_var: 'CODEXCREW_TELEMETRY', otlp_configured: true })
    const sw = await screen.findByRole('switch', { name: LABEL })
    // Both notes are announced, not just the first. Assert through the idrefs
    // rather than by text: the panel also prints CODEXCREW_TELEMETRY_DISABLED in
    ...
    expect(texts.some(t => /CODEXCREW_TELEMETRY/.test(t))).toBe(true)
  ...
  it('announces WHY it is disabled, not just that it is', async () => {
    ...
    expect(note?.textContent ?? '').toMatch(/CODEXCREW_TELEMETRY environment variable/)
  })
