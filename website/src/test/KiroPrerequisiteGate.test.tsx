...
describe('KiroPrerequisiteGate', () => {
  ...
  it('forces a real host probe on the blocking gate, not a latched read', async () => {
    ...
    await screen.findByText(/Codex Crew uses Kiro CLI/)
  ...
  it('lifts the gate as soon as detection reports a signed-in CLI', async () => {
    ...
    expect(await screen.findByText(/Codex Crew uses Kiro CLI/)).toBeInTheDocument()
  ...
  it('sends the user to Kiro CLI setup instead of installing anything', async () => {
    // Codex Crew does not install Kiro CLI. A missing CLI must offer a link to
  ...
  it('exposes no way to start a sign-in from the dashboard', async () => {
    // Codex Crew does not authenticate for the user: there is no device-flow
  ...
  it('blocks an ESTABLISHED install when the agent specs are missing', async () => {
    ...
    vi.mocked(api.kiroPrerequisite).mockResolvedValue(status({
      ...
      missing_agent_specs: ['codexcrew.json', 'codexcrew-lite.json'],
    }))
    ...
    expect(await screen.findByText("Codex Crew's agent specs are not installed")).toBeInTheDocument()
    ...
    expect(screen.getByText(/codexcrew\.json/)).toBeInTheDocument()
    expect(screen.getByText(/codexcrew-lite\.json/)).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Check again' })).toBeEnabled()
  })
  ...
  it('gates on a spec that is PRESENT but which kiro-cli refuses', async () => {
    ...
    // there, while kiro-cli drops it from its agent table, so Codex Crew's agent
    ...
    vi.mocked(api.kiroPrerequisite).mockResolvedValue(status({
      ...
      repair_required: true,
      missing_agent_specs: [],
      rejected_agent_specs: ['codexcrew.json'],
      agent_spec_rejection_detail:
        'Error: Json supplied at /home/u/.kiro/agents/codexcrew.json is invalid: '
    ...
    expect(
      await screen.findByText("Kiro CLI will not load Codex Crew's agent specs"),
    ).toBeInTheDocument()
    expect(screen.queryByText('Dashboard loaded')).not.toBeInTheDocument()
  ...
  it('does not promise that checking again rewrites a rejected spec', async () => {
    ...
    expect(screen.getByText(/Update Codex Crew\./)).toBeInTheDocument()
    ...
    const command = screen.getByText('codexcrew setup --agent-only --clean')
  ...
  it('shows the missing-specs card, not the rejected one, when a spec is absent', async () => {
    ...
    vi.mocked(api.kiroPrerequisite).mockResolvedValue(status({
      ...
      authenticated: true,
      initial_setup_complete: true,
      missing_agent_specs: ['codexcrew.json'],
      rejected_agent_specs: ['codexcrew-lite.json'],
    }))
    ...
    expect(
      await screen.findByText("Codex Crew's agent specs are not installed"),
    ).toBeInTheDocument()
    expect(
      screen.queryByText("Kiro CLI will not load Codex Crew's agent specs"),
  ...
  it('shows NO sign-in chrome when an established install is signed out', async () => {
    ...
    expect(await screen.findByText('Dashboard loaded')).toBeInTheDocument()
    expect(screen.queryByText('Codex Crew needs Kiro sign-in.')).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: 'Sign in to Kiro' })).not.toBeInTheDocument()
  ...
  it('remembers a returning user across a cold start with an erroring gateway', async () => {
    ...
    localStorage.setItem('codexcrew:kiro-setup-complete', '1')
    vi.mocked(api.kiroPrerequisite).mockRejectedValue(new ApiError(500, 'Probe failed'))
  ...
  it('records first-run completion so later cold starts skip setup chrome', async () => {
    ...
    expect(await screen.findByText('Dashboard loaded')).toBeInTheDocument()
    await waitFor(() =>
      expect(localStorage.getItem('codexcrew:kiro-setup-complete')).toBe('1'),
    )
  })
  ...
  it('tells a transient sandbox failure apart from a host verdict', async () => {
    ...
    expect(screen.queryByText('codexcrew service install')).not.toBeInTheDocument()
  })
  ...
  it('names the AppArmor mechanism and the command that fixes it', async () => {
    ...
    expect(screen.getByText('codexcrew service install')).toBeInTheDocument()
    ...
    expect(screen.getByText('codexcrew doctor')).toBeInTheDocument()
  ...
  it('copies a command to the clipboard when its block is clicked', async () => {
    ...
    const command = await screen.findByText('codexcrew service install')
    ...
    await waitFor(() =>
      expect(copyToClipboard).toHaveBeenCalledWith('codexcrew service install'),
    )
  })
  ...
  it('still points at doctor when the mechanism is unknown', async () => {
    ...
    expect(await screen.findByText('codexcrew doctor')).toBeInTheDocument()
    expect(screen.queryByText('How to fix')).not.toBeInTheDocument()
    expect(screen.queryByText('codexcrew service install')).not.toBeInTheDocument()
