 ...
 *   3. else returns None.
 *
 * `codexcrewAgents()` sent no `X-Session-Key` at all, so step 1 could never
...
describe('useAgents session scoping', () => {
  it('forwards the slot key so the server can resolve project-local agents', async () => {
    ...
    expect(mockApi.codexcrewAgents).toHaveBeenCalledWith('chat-2-1786309747')
  })
  ...
  it('re-fetches when the focused slot changes, since project scope is per slot', async () => {
    ...
    rerender({ sk: 'chat-2' })
    await waitFor(() => expect(mockApi.codexcrewAgents).toHaveBeenCalledWith('chat-2'))
  })
  ...
  it('clears the previous slot\'s roster while the new scope\'s fetch is in flight', async () => {
    ...
    resolveSecond({ agents: [{ name: 'other-project-agent', scope: 'project' }], default_agent: 'codexcrew' })
  ...
  it('omits the key on surfaces with no slot context (Channels, Schedule)', async () => {
    ...
    await waitFor(() => expect(mockApi.codexcrewAgents).toHaveBeenCalled())
    expect(mockApi.codexcrewAgents).toHaveBeenCalledWith(undefined)
  })
})
