...
// Plugin bridge global installed on window (mirrors CodexCrewNavBridge.tsx).
type BridgeWindow = { __codexcrewReportSlotTitle?: (slug: string, title: string) => void }
...
import CodexCrewNavBridge from '../components/CodexCrewNavBridge'
...
describe('EmbedTabStrip', () => {
  ...
  it('renders tab from sessionStorage', () => {
    sessionStorage.setItem('codexcrew-embed-tabs', JSON.stringify(['chat-1']))
    sessionStorage.setItem('codexcrew-embed-active-index', '0')
    wrap(<EmbedTabStrip />)
    expect(screen.getByText('First Chat')).toBeTruthy()
  })
  ...
  it('clicking a tab navigates to its chat', () => {
    sessionStorage.setItem('codexcrew-embed-tabs', JSON.stringify(['chat-1', 'chat-2']))
    sessionStorage.setItem('codexcrew-embed-active-index', '0')
    wrap(<EmbedTabStrip />)
  ...
  it('dispatches codexcrew-tab-update on tab change', () => {
    const handler = vi.fn()
    window.addEventListener('codexcrew-tab-update', handler)
    sessionStorage.setItem('codexcrew-embed-tabs', JSON.stringify(['chat-1']))
    sessionStorage.setItem('codexcrew-embed-active-index', '0')
    wrap(<EmbedTabStrip />)
    fireEvent.click(screen.getByLabelText('New tab'))
    expect(handler).toHaveBeenCalled()
    window.removeEventListener('codexcrew-tab-update', handler)
  })

  it('navigates on mount to restore active tab URL', () => {
    sessionStorage.setItem('codexcrew-embed-tabs', JSON.stringify(['chat-2']))
    sessionStorage.setItem('codexcrew-embed-active-index', '0')
    wrap(<EmbedTabStrip />)
  ...
  it('responds to codexcrew-tab-state event from plugin', () => {
    wrap(<EmbedTabStrip />)
    act(() => {
      window.dispatchEvent(new CustomEvent('codexcrew-tab-state', {
  ...
  it('slot deletion removes tab and navigates when active tab deleted', () => {
    sessionStorage.setItem('codexcrew-embed-tabs', JSON.stringify(['chat-1', 'chat-3']))
    sessionStorage.setItem('codexcrew-embed-active-index', '1') // chat-3 is active
  ...
  it('mount navigates to sessions when active tab is empty slug', () => {
    sessionStorage.setItem('codexcrew-embed-tabs', JSON.stringify(['']))
    sessionStorage.setItem('codexcrew-embed-active-index', '0')
    wrap(<EmbedTabStrip />)
...
describe('CodexCrewNavBridge', () => {
  it('renders nothing (returns null)', () => {
    const { container } = wrap(<CodexCrewNavBridge />)
    expect(container.innerHTML).toBe('')
  })

  it('navigates on codexcrew-soft-navigate event', () => {
    wrap(<CodexCrewNavBridge />)
    act(() => {
      window.dispatchEvent(new CustomEvent('codexcrew-soft-navigate', {
  ...
  it('ignores events without url', () => {
    wrap(<CodexCrewNavBridge />)
    act(() => {
      window.dispatchEvent(new CustomEvent('codexcrew-soft-navigate', { detail: {} }))
    })
