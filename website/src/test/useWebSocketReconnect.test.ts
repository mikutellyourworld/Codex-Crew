...
describe('useWebSocket reconnect unread suppression', () => {
  ...
  it('clears stale subagents before authoritative snapshot replay', () => {
    ...
    testStore.dispatch(sseSubagentSpawn({ slot: 'chat-active', id: 'stale-active', task: 'Old active task', agent: 'codexcrew' }))
    testStore.dispatch(sseSubagentSpawn({ slot: 'chat-other', id: 'stale-background', task: 'Old background task', agent: 'codexcrew' }))
    ...
    act(() => {
      ws1.simulateMessage({
        ...
        data: { id: 'live-1', slot: 'chat-active', task: 'Live task', agent: 'codexcrew', streaming: '', last_tool: '', started: Date.now() / 1000 },
      })
    })
    ...
    act(() => {
      ws2.simulateMessage({
        type: 'subagent_snapshot',
        data: { id: 'live-2', slot: 'chat-active', task: 'Current live task', agent: 'codexcrew', streaming: '', last_tool: '', started: Date.now() / 1000 },
      })
    })
  ...
  it('preserves pending spawn-approval cards through the reconnect clear', () => {
    ...
    testStore.dispatch(sseSubagentSpawn({ slot: 'chat-active', id: 'stale-running', task: 'Old task', agent: 'codexcrew' }))
