...
const PROMPT = '~/work/CodexCrew (feat/x) » '
...
describe('shellEscape', () => {
  it('leaves a safe name untouched', () => {
    expect(shellEscape('CodexCrew-terminal_v2.1')).toBe('CodexCrew-terminal_v2.1')
  })
...
describe('commonPrefix', () => {
  it('returns the shared leading text', () => {
    expect(commonPrefix(['CodexCrew', 'KiroCli'])).toBe('KiroC')
    expect(commonPrefix(['docs', 'doctor', 'dock'])).toBe('doc')
  })

  it('returns the whole name for a single entry', () => {
    expect(commonPrefix(['CodexCrew'])).toBe('CodexCrew')
  })
...
describe('buildInsertion', () => {
  it('appends only the missing suffix in the common case', () => {
    expect(buildInsertion('Kiro', 'CodexCrew', '/')).toEqual({ erase: 0, text: 'Crew/' })
  })

  it('rewrites the token when the typed case differs from disk', () => {
    // Matching is case-insensitive, so `kiro` can select `CodexCrew`; sending
    // only the suffix would leave a path that does not exist.
    expect(buildInsertion('kiro', 'CodexCrew', '/')).toEqual({ erase: 4, text: 'CodexCrew/' })
  })
