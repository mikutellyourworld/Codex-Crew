...
describe('the rail while Learning is open', () => {
  beforeEach(() => {
    ...
    mockApi.namespaces.mockResolvedValue({
      namespaces: [
        ...
        { name: 'codexcrew', patterns: 5, candidate: 1, active: false },
      ],
      active: ['default'],
    })
    mockApi.settings.mockResolvedValue({
  ...
  it('lists the namespaces with their pattern counts', async () => {
    mount()
    expect(await screen.findByRole('button', { name: 'Read namespace codexcrew' }))
      .toBeTruthy()
  ...
  it('reads the namespace you pick', async () => {
    ...
    mount()
    await userEvent.click(await screen.findByRole('button', { name: 'Read namespace codexcrew' }))
    await waitFor(() => expect(mockApi.learnings).toHaveBeenCalledWith('codexcrew'))
    // Title and guidance both carry it.
    expect((await screen.findAllByText(/Rule in codexcrew/)).length).toBeGreaterThan(0)
  })
  ...
  it('says whether the namespace being read is actually loaded', async () => {
    ...
    await screen.findByRole('button', { name: 'Read namespace codexcrew' })
    expect(await screen.findByText(/^loaded during reviews$/)).toBeTruthy()
    await userEvent.click(screen.getByRole('button', { name: 'Read namespace codexcrew' }))
  ...
  it('keeps active independent of what you are reading', async () => {
    ...
    await userEvent.click(screen.getByRole('checkbox', {
      name: /Load namespace codexcrew during reviews/,
    }))
    await waitFor(() => expect(mockApi.putSettings).toHaveBeenCalledWith({
      active_namespaces: ['default', 'codexcrew'],
    }))
  })
  ...
  it('offers no delete for the default namespace', async () => {
    mount()
    await screen.findByRole('button', { name: 'Read namespace codexcrew' })
    expect(screen.getByRole('button', { name: /Delete namespace codexcrew/ })).toBeTruthy()
