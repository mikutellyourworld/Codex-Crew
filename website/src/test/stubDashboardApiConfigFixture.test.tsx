/**
 * The shared screenshot stub's /api/config/codexcrew fixture (#3696).
 *
 * `website/scripts/lib/stub-dashboard-api.mjs` used to answer every path
 * matching /config/ from a catch-all that returns `{}`. `/api/config/codexcrew`
 * matched, so `CodexCrewCfgTab` got `cfg.agents === undefined`,
 ...
 * render. A future field added to `CodexCrewCfg` and read unguarded (the exact
 * shape of the original bug) fails here rather than silently blanking ~140
 * capture harnesses.
 */
import { describe, it, expect, vi, beforeEach } from 'vitest'
import { screen } from '@testing-library/react'
import CodexCrewCfgTab from '../pages/overview/CodexCrewCfgTab'
import { renderWithProviders } from './helpers'
import { api } from '../api/client'
import {
  CODEXCREW_CONFIG_FIXTURE,
...
beforeEach(() => {
  ...
  m.codexcrewConfig = vi.fn().mockResolvedValue(CODEXCREW_CONFIG_FIXTURE)
  m.patchConfig = vi.fn().mockResolvedValue(CODEXCREW_CONFIG_FIXTURE)
...
describe('stub-dashboard-api config fixture', () => {
  ...
  it('populates every table the tab renders, not just the headers', async () => {
    renderWithProviders(<CodexCrewCfgTab />)
    await screen.findByText('Codex Crew Agents')
  ...
  it('spells the workspace directory the way the component reads it', async () => {
    ...
    expect(Object.values(CODEXCREW_CONFIG_FIXTURE.workspaces)[0]).toHaveProperty('dir')
    renderWithProviders(<CodexCrewCfgTab />)
    await screen.findByText('Codex Crew Agents')
    expect(screen.getByText('~/.codex-crew/workspace')).toBeInTheDocument()
  })
  ...
  it('carries the nested objects the tab dereferences unguarded', () => {
    ...
    expect(CODEXCREW_CONFIG_FIXTURE.agent).toBeTypeOf('object')
    expect(CODEXCREW_CONFIG_FIXTURE.session).toBeTypeOf('object')
    expect(CODEXCREW_CONFIG_FIXTURE.memory).toBeTypeOf('object')
    expect(CODEXCREW_CONFIG_FIXTURE.auto_update).toBeTypeOf('boolean')
    for (const key of ['agents', 'workspaces', 'memory_stores'] as const) {
      expect(CODEXCREW_CONFIG_FIXTURE[key]).toBeTypeOf('object')
      expect(CODEXCREW_CONFIG_FIXTURE[key]).not.toBeNull()
    }
    for (const key of ['default_agent', 'default_workspace', 'default_memory_store'] as const) {
      expect(CODEXCREW_CONFIG_FIXTURE[key]).toBeTypeOf('string')
    }
  })
  ...
  it('names each default as a key that actually exists', async () => {
    ...
    expect(CODEXCREW_CONFIG_FIXTURE.agents).toHaveProperty(CODEXCREW_CONFIG_FIXTURE.default_agent)
    expect(CODEXCREW_CONFIG_FIXTURE.workspaces).toHaveProperty(CODEXCREW_CONFIG_FIXTURE.default_workspace)
    expect(CODEXCREW_CONFIG_FIXTURE.memory_stores).toHaveProperty(CODEXCREW_CONFIG_FIXTURE.default_memory_store)
    renderWithProviders(<CodexCrewCfgTab />)
    await screen.findByText('Codex Crew Agents')
