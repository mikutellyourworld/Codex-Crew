 ...
 * - the Codex CLI status modal and its setup guidance
...
vi.mock('../pages/CodexCrewAgentsPage', () => ({ default: () => null }))
...
vi.mock('../hooks/useAgents', () => ({ useAgents: vi.fn(() => ({ agents: [{ name: 'codexcrew' }], defaultAgent: 'codexcrew' })) }))
...
beforeEach(() => {
  ...
  vi.mocked(api.acpBackends).mockResolvedValue({ backends: [] } as never)
})
...
describe('App — applying an update', () => {
  it('replaces the changelog with the progress overlay', async () => {
    ...
    expect(await screen.findByText('Updating Codex Crew…')).toBeInTheDocument()
    expect(screen.queryByRole('dialog', { name: 'Changelog' })).toBeNull()
  ...
  it('surfaces the server-supplied reason when applying fails', async () => {
    ...
    expect(screen.queryByText('Updating Codex Crew…')).toBeNull()
...
describe('App — update progress overlay', () => {
  it('marks earlier steps done and times the active one', async () => {
    ...
    await screen.findByText('Updating Codex Crew…')
...
describe('App — Codex CLI status modal', () => {
  beforeEach(() => { localStorage.setItem('mc-last-version', '0.4.0') })

  it('shows setup guidance when the Codex adapter is missing', async () => {
    vi.mocked(api.acpBackends).mockResolvedValue({
      backends: [{
        id: 'codex', policy_id: 'codex', selectable: true, installed: 'missing',
    ...
    fireEvent.click(await screen.findByTitle('Primary agent backend'))
    const dialog = await screen.findByRole('dialog', { name: 'Codex CLI' })
    expect(await within(dialog).findByText('Codex CLI needs setup')).toBeInTheDocument()
    expect(within(dialog).getByText('codex-acp')).toBeInTheDocument()
    expect(within(dialog).getByText('npm install -g @agentclientprotocol/codex-acp')).toBeInTheDocument()
    expect(within(dialog).queryByText(/Kiro Account|credit usage/i)).not.toBeInTheDocument()
  })
})
