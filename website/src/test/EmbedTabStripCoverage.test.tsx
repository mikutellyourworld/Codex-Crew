/**
 * Coverage-focused tests for EmbedTabStrip — the tab bar the Codex Crew IDE
 ...
 *   - the plugin-pushed `codexcrew-tab-state` event landing on a Sessions tab
...
const STORAGE_KEY = 'codexcrew-embed-tabs'
const STORAGE_INDEX_KEY = 'codexcrew-embed-active-index'

/** Tab state the host plugin injects onto `window`. */
interface EmbedTabsWindow extends Window {
  __codexcrewTabs?: string[]
  __codexcrewActiveTabIndex?: number
}
...
beforeEach(() => {
  ...
  delete tabsWindow.__codexcrewTabs
  delete tabsWindow.__codexcrewActiveTabIndex
})
...
describe('EmbedTabStrip — initial tab resolution', () => {
  it('adopts tabs injected on window by the host plugin', () => {
    tabsWindow.__codexcrewTabs = ['chat-2', 'chat-3']
    tabsWindow.__codexcrewActiveTabIndex = 1
...
describe('EmbedTabStrip — plugin-pushed tab state', () => {
  it('navigates to the sessions list when the pushed active tab is empty', () => {
    ...
    act(() => {
      window.dispatchEvent(new CustomEvent('codexcrew-tab-state', { detail: { tabs: ['', 'chat-2'], activeIndex: 0 } }))
    })
  ...
  it('defaults the pushed active index to the first tab when it is not a number', () => {
    ...
    wrap(makeStore())
    act(() => {
      window.dispatchEvent(new CustomEvent('codexcrew-tab-state', { detail: { tabs: ['chat-3', 'chat-2'] } }))
    })
  ...
  it('ignores a pushed payload whose tabs field is not an array', () => {
    ...
    wrap(makeStore())
    act(() => {
      window.dispatchEvent(new CustomEvent('codexcrew-tab-state', { detail: { tabs: 'chat-2' } }))
    })
    expect(tabTitles()).toEqual(['First Chat'])
  })
})
