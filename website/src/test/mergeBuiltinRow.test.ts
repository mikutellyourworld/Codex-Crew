 ...
 * preferred the local manifest, so one app could read "Codex Crew ·
 * Developer Tools" in the list and "codexcrew · Productivity" one click later.
...
describe('mergeBuiltinRow', () => {
  ...
  it('never invents an author when neither source states one', () => {
    // The previous browse-side chain ended in a hardcoded 'codexcrew', which
  ...
  it('never invents an author when neither source states one', () => {
    // The browse chain used to hardcode `|| 'codexcrew'`, asserting authorship no
