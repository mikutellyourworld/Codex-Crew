...
vi.mock('../api/client', () => ({
  api: {
    ...
    defaultAgent: vi.fn().mockResolvedValue({ default_agent: 'codexcrew' }),
...
describe('SchedulePage roster failure wiring (#5990)', () => {
  beforeEach(async () => {
    ...
    vi.mocked(api).defaultAgent.mockResolvedValue({ default_agent: 'codexcrew' })
  ...
  it('hands the picker a failure it can act on when /api/agents rejects', async () => {
    ...
    expect(screen.getByLabelText('Switch agent')).toHaveTextContent('codexcrew')
