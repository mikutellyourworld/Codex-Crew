...
vi.mock('../hooks/useAgents', () => ({ useAgents: () => ({ agents: [{ name: 'codexcrew' }, { name: 'researcher' }], defaultAgent: 'codexcrew' }) }))
...
function makeStore() {
  return configureStore({
    ...
    preloadedState: {
      dashboard: {
        status: null,
        slots: [{ key: 'slot-a', messages: 0, running: false, mode: '', agent: 'codexcrew', model: 'claude-opus-5', project: '/home/user/old-proj', pending_approval: false, waiting_for_input: false, last_activity_ts: undefined }],
...
describe('ChatPage — switch labels update without a slot-list round trip (#4523)', { timeout: 15_000 }, () => {
  ...
  it('agent pick writes agent AND workspace to the store without a slot-list round trip (#5120)', async () => {
    const store = await renderChat()
    const slotsFetchesBeforePick = vi.mocked(api.chatSlots).mock.calls.length
    const chip = screen.getByTitle('Agent: codexcrew')
    await act(async () => { fireEvent.click(chip) })
  ...
  it('keeps the pre-switch agent when the agent switch fails (#5120)', async () => {
    ...
    expect(store.getState().dashboard.slots.find(s => s.key === 'slot-a')?.agent).toBe('codexcrew')
    expect(screen.getByTitle('Agent: codexcrew')).toBeTruthy()
  })
})
