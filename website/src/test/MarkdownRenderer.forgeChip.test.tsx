...
describe('MarkdownRenderer forge chips — the four URL shapes', () => {
  it('chips a GitHub pull request URL as owner/repo#N with the GitHub mark', () => {
    render(<MarkdownRenderer content="See https://github.com/mikutellyourworld/Codex-Crew/pull/2579 for the fix" />)
    const link = screen.getByRole('link')
    expect(link).toHaveAttribute('href', 'https://github.com/mikutellyourworld/Codex-Crew/pull/2579')
    expect(link).toHaveAttribute('target', '_blank')
    expect(link.getAttribute('rel')).toBe('noopener noreferrer')
    expect(screen.getByText('mikutellyourworld/Codex-Crew#2579')).toBeTruthy()
    expect(link.querySelector('[data-provider-mark="github"]')).toBeTruthy()
  })

  it('chips a GitHub issue URL', () => {
    render(<MarkdownRenderer content="https://github.com/mikutellyourworld/Codex-Crew/issues/45" />)
    expect(screen.getByText('mikutellyourworld/Codex-Crew#45')).toBeTruthy()
...
describe('MarkdownRenderer forge chips — security and fallthrough', () => {
  it('does NOT chip a lookalike hostname', () => {
    render(<MarkdownRenderer content="https://evil-github.com.attacker.test/mikutellyourworld/Codex-Crew/pull/1" />)
    const link = screen.getByRole('link')
  ...
  it('does NOT chip a credential-bearing URL, even on the real host', () => {
    render(<MarkdownRenderer content="https://user:pass@github.com/mikutellyourworld/Codex-Crew/pull/1" />)
    expect(screen.queryByText('mikutellyourworld/Codex-Crew#1')).toBeNull()
  ...
  it('falls through to a plain anchor for malformed forge paths', () => {
    render(
      <MarkdownRenderer content={'https://github.com/mikutellyourworld/Codex-Crew/pull/abc and https://github.com/mikutellyourworld/Codex-Crew and https://gitlab.com/acme/widgets/merge_requests/7'} />,
    )
  ...
  it('keeps the authored href — fragments and query params are not rewritten', () => {
    const authored = 'https://github.com/mikutellyourworld/Codex-Crew/pull/2579?diff=split#issuecomment-1'
...
describe('MarkdownRenderer forge chips — render contexts', () => {
  it('chips with link previews OFF and issues no network request', () => {
    ...
    render(<MarkdownRenderer content="https://github.com/mikutellyourworld/Codex-Crew/pull/2579" softBreaks compactImages />)
    expect(screen.getByText('mikutellyourworld/Codex-Crew#2579')).toBeTruthy()
