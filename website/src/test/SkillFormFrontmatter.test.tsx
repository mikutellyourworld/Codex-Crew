...
describe('structured editor preserves unmodelled frontmatter', () => {
  ...
  it('round-trips repo_scope, which gates where the skill may match', () => {
    const out = assembleSkillContent(parseSkillContent(RAW, 'codexcrew-dev/worktree-dev'))
    expect(parseFrontmatter(out).meta.repo_scope).toBe('src/codex_crew')
  })
  ...
  it('still writes the keys the form owns', () => {
    const data = parseSkillContent(RAW, 'codexcrew-dev/worktree-dev')
  ...
  it('does not let a top-level comment corrupt an unmodelled scalar', () => {
    const raw = ['---', 'name: s', 'repo_scope: src/codex_crew', '# scoped on purpose', '---', '', '# Body'].join('\n')
    const out = assembleSkillContent(parseSkillContent(raw, 's'))
    expect(parseFrontmatter(out).meta.repo_scope).toBe('src/codex_crew')
    expect(out).toContain('repo_scope: src/codex_crew\n# scoped on purpose')
  })
  ...
  it('still derives a name when the file declares none', () => {
    ...
    const data = parseSkillContent(raw, 'codexcrew-dev/derived-key')
