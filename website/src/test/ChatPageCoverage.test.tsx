...
vi.mock('../hooks/useAgents', () => ({
  useAgents: () => ({ agents: [{ name: 'codexcrew' }, { name: 'reviewer' }], defaultAgent: 'codexcrew' }),
}))
...
describe('ChatPage default-agent footer row', () => {
  it('points the row at the session\'s own agent and writes that one', async () => {
    ...
    // useAgents mock above returns defaultAgent: 'codexcrew'), not the
    // literal 'default' placeholder the pre-fix fallback rendered.
    expect(defaultAgentRowProps!.agentName).toBe('codexcrew')
    act(() => { defaultAgentRowProps!.onSetDefault() })
    await waitFor(() => expect(setDefault).toHaveBeenCalledWith('codexcrew'))
  })
})
