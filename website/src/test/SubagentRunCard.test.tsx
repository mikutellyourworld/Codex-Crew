...
const SPAWN_OUTPUT = [
  ...
  '  1713e7d0 (codexcrew): INVESTIGATION ONLY -- trace the backend signals',
  '  5c15adde (codexcrew): INVESTIGATION ONLY -- trace the sidebar data flow',
  '  aa5da49b (codexcrew): INVESTIGATION ONLY -- trace the chat transcript cards',
  '',
...
function agent(id: string, status: SubagentActivity['status']): SubagentActivity {
  return {
    id, task: 't', agent: 'codexcrew', status, streaming: '', lastTool: '',
    ...
    content: [
      {
        ...
        text:
          ...
          '  e5f6a7b8 (codexcrew): Print the working directory and return it.\n',
      },
    ],
  })

  it('unwraps the envelope and recovers the header + agent ids', () => {
    const msg = { role: 'tool', content: '🔧 Running: @codexcrew-core/spawn_run', cls: '', meta: { output: ENVELOPE } } as ChatMessage
...
describe('SubagentRunCard — wave total comes from the header count', () => {
  ...
  it('counts every member of a staggered wave now that queued ids are real', () => {
    ...
    //     4fbc9f4b (codexcrew): RESEARCH …
    //     q1 (codexcrew): RESEARCH …
    ...
    const output =
      'Spawned 2 subagent(s). Results will arrive as completion events:\n' +
      '  4fbc9f4b (codexcrew): RESEARCH: is react-i18next\u2019s <Trans> still current\n' +
      '  8b2f1e3b (codexcrew): RESEARCH: what is the current React version\n'
