...
describe('useTouchedFiles', () => {
  ...
  it('persists to localStorage', () => {
    const { result } = renderHook(() => useTouchedFiles('test-session'))
    act(() => result.current.addFile('/tmp/a.ts', 'tool'))
    const stored = JSON.parse(localStorage.getItem('codexcrew:touched-files:test-session')!)
    expect(stored).toHaveLength(1)
    expect(stored[0].path).toBe('/tmp/a.ts')
  })

  it('loads from localStorage on mount', () => {
    localStorage.setItem('codexcrew:touched-files:s1', JSON.stringify([{ path: '/x.ts', ts: 1, source: 'tool' }]))
