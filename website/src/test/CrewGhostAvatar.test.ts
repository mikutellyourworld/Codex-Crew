 ...
 *    `src/assets/crew-avatar-mark.svg`, so the generator cannot drift from the art;
...
const MARK = join(__dirname, '..', 'assets', 'crew-avatar-mark.svg')
...
describe('kiroGhost style', () => {
  ...
  it('renders a local data URI with no remix claim in its metadata', () => {
    const svg = createAvatar(kiroGhost, { seed: 'codexcrew' }).toString()
    ...
    expect(createAvatar(kiroGhost, { seed: 'codexcrew' }).toDataUri()).toMatch(
      /^data:image\/svg\+xml/,
    )
  })
  ...
  it('is deterministic and distinct across seeds', () => {
    ...
    expect(createAvatar(kiroGhost, { seed: 'codexcrew' }).toString()).not.toBe(a)
  })
  ...
  it('pins the draw order', () => {
    ...
    expect(traits('codexcrew')).toEqual({
      eyes: 'wide',
      brows: 'angry',
      mouth: 'cat',
      accessory: 'cap',
