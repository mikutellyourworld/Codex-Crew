...
vi.mock('../pages/CodexCrewAgentsPage', () => ({ default: () => <div data-testid="mc-agents-page">MCAgentsPage</div> }))
...
vi.mock('../hooks/useAgents', () => ({ useAgents: vi.fn(() => ({ agents: [{ name: 'codexcrew' }, { name: 'reviewer' }, { name: 'oracle' }], defaultAgent: 'codexcrew' })) }))
...
vi.mock('../api/client', () => ({
  api: {
    ...
    beaconStatus: vi.fn().mockResolvedValue({
      ...
      env_override: false,
      env_var: 'CODEXCREW_TELEMETRY_DISABLED',
    }),
...
describe('App routing', () => {
  ...
  it('migrates legacy browser-only onboarding before applying server defaults', async () => {
    ...
    expect(screen.queryByRole('dialog', { name: 'Welcome to Codex Crew' })).not.toBeInTheDocument()
  })
  ...
  it('renders nav items', () => {
    ...
    expect(screen.getByLabelText('Star Codex Crew on GitHub')).toBeInTheDocument()
    expect(
  ...
  it('refetches the Apps nav when the gateway reconnects (post-update recovery)', async () => {
    ...
    // /api/apps once on mount, and right after a `codexcrew update` restart that
    ...
    // Simulate a `codexcrew update` restart: the WS connects, drops, reconnects.
  ...
  it('renders Codex Crew branding', () => {
  ...
  it('uses installed theme branding in the left rail and browser favicon', async () => {
    ...
    try {
      ...
      await waitFor(() => expect(brand).toHaveTextContent('CODEX CREW'))
  ...
  it('lets the brand toggle expand the rail while preview expand mode is active', () => {
    ...
    act(() => {
      window.dispatchEvent(new CustomEvent('codexcrew-preview-expand', { detail: { expanded: true } }))
    })
    expect(within(nav).getByRole('button', { name: 'Expand sidebar' })).toBeInTheDocument()
    ...
    act(() => {
      window.dispatchEvent(new CustomEvent('codexcrew-preview-expand', { detail: { expanded: false } }))
    })
    expect(within(nav).getByRole('button', { name: 'Collapse sidebar' })).toBeInTheDocument()
    localStorage.removeItem('mc-nav')
  })
...
describe('onCycleAgent keyboard shortcut', () => {
  it('cycles to next agent when Alt+Shift+A is pressed', async () => {
    ...
    store.dispatch({ type: 'dashboard/sseSlots', payload: [{ key: 'slot-1', messages: 0, running: false, agent: 'codexcrew' }] })
  ...
  it.each([
    ...
    store.dispatch({ type: 'dashboard/sseSlots', payload: [{ key: 'slot-1', messages: 0, running: true, agent: 'codexcrew' }] })
    store.dispatch({ type: 'chat/setActiveSlot', payload: 'slot-1' })
    renderWithProviders(<App />, { route: '/chat' })
...
describe('onCycleAgent edge cases', () => {
  it('does not cycle agent when installedAgents is empty', async () => {
    ...
    expect(api.chatSlotAgent).not.toHaveBeenCalled()
    useAgentsMock.mockReturnValue({ agents: [{ name: 'codexcrew' }, { name: 'reviewer' }, { name: 'oracle' }], defaultAgent: 'codexcrew' })
  })
})
...
describe('onCycleReasoningEffort keyboard shortcut (#5120)', () => {
  it('steps a burst from the in-flight target and writes the adjudicated level', async () => {
    ...
    store.dispatch({ type: 'dashboard/sseSlots', payload: [{ key: 'slot-1', messages: 0, running: false, agent: 'codexcrew', reasoning_effort: 'max' }] })
    store.dispatch({ type: 'chat/setActiveSlot', payload: 'slot-1' })
    renderWithProviders(<App />, { route: '/chat' })
  ...
  it('cycles backward on Alt+Shift+C and writes the store', async () => {
    ...
    const { store } = await import('../store')
    ;(api.chatSlotReasoningEffort as ReturnType<typeof vi.fn>).mockClear()
    store.dispatch({ type: 'dashboard/sseSlots', payload: [{ key: 'slot-1', messages: 0, running: false, agent: 'codexcrew', reasoning_effort: 'low' }] })
    store.dispatch({ type: 'chat/setActiveSlot', payload: 'slot-1' })
    renderWithProviders(<App />, { route: '/chat' })
    await act(async () => {
...
describe('onCycleApprovalMode and onCyclePrevAgent shortcuts', () => {
  ...
  it('cycles agent backward on Alt+Shift+Z', async () => {
    ...
    expect(api.chatSlotAgent).toHaveBeenCalledWith('slot-1', 'codexcrew')
  })
...
describe('Codex CLI status pill', () => {
  it('replaces the Kiro account and credit surface', async () => {
    const { api } = await import('../api/client')
    vi.mocked(api.acpBackends).mockResolvedValue({
      backends: [{
        id: 'codex', policy_id: 'codex', selectable: true, installed: 'installed',
        missing_components: [], install_command: '', restart_required: false,
      }],
    })
    renderWithProviders(<App />, { route: '/chat' })

    const pill = await screen.findByTitle('Primary agent backend')
    expect(pill).toHaveTextContent('Codex CLI')
    ...
    expect(await screen.findByText('Ready on this machine')).toBeInTheDocument()
    expect(screen.queryByText(/credit usage/i)).not.toBeInTheDocument()
  })

  it('closes the Codex CLI modal on Escape and restores focus', async () => {
    const { api } = await import('../api/client')
    ...
    await waitFor(() => expect(screen.queryByRole('dialog', { name: 'Codex CLI' })).not.toBeInTheDocument())
