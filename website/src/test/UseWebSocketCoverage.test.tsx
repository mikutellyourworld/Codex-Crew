...
describe('useWebSocket frame router', () => {
  ...
  const slotFixture = (key: string, extra: Partial<ChatSlot> = {}): ChatSlot => ({
    key, title: key, agent: 'codexcrew', ...extra,
  } as ChatSlot)
  ...
  it('threads the whole subagent lifecycle into one card', () => {
    const { ws } = mount()
    act(() => {
      ws.simulateMessage({ type: 'subagent_spawn', data: { slot: ACTIVE, id: 'ag-1', task: 'Read specs', agent: 'codexcrew' } })
  ...
  it('re-broadcasts every channel frame on one window event', () => {
    ...
    window.addEventListener('codexcrew-channel', listener)
    try {
      act(() => {
        for (const type of [
      ...
      window.removeEventListener('codexcrew-channel', listener)
    }
  })
  ...
  it('re-broadcasts tool_call so the Browser panel can open on a browse', () => {
    ...
    try {
      ...
      window.removeEventListener('codexcrew-tool-call', onTool)
    }
  })
  ...
  it('re-broadcasts cron history and the computer-use mirror frame', () => {
    ...
    window.addEventListener('codexcrew-browser-frame', onBrowser)
    window.addEventListener('codexcrew-computer-use-frame', onComputer)
    try {
      act(() => {
      ...
      window.removeEventListener('codexcrew-browser-frame', onBrowser)
      window.removeEventListener('codexcrew-computer-use-frame', onComputer)
    }
  })
...
describe('useWebSocket slots reconcile', () => {
  it('prunes per-slot caches for sessions the authoritative list dropped', () => {
    ...
    testStore.dispatch(sseSlots([{ key: ACTIVE, title: ACTIVE, agent: 'codexcrew' } as ChatSlot]))
