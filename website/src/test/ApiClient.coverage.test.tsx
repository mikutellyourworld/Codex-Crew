...
describe('client transport', () => {
  it('exposes the backend search threshold as a shared constant', () => {
    // Must match codex_crew.history.SEARCH_MIN_CHARS — the pages gate typing on it.
...
describe('session-expired banner', () => {
  ...
  it('shows the banner only once the refresh comes back terminal (401)', async () => {
    ...
    expect(el.querySelector('code')?.textContent).toBe('codexcrew token')
  ...
  describe('pasting a token', () => {
    ...
    it('extracts the token out of a pasted `codexcrew token` URL', async () => {
...
describe('query-string builders', () => {
  ...
  it('effortLevels and mcpActive scope to a slot/agent when given one', async () => {
    ...
    await api.mcpActive('codexcrew')
    expect(call(3).url).toBe('/api/mcp/active?agent=codexcrew')
  })
  ...
  it('fileSearch scopes to a project and forwards the abort signal', async () => {
    ...
    await api.fileSearch('cli', 'codexcrew', ctl.signal)
    expect(call().url).toBe('/api/file-search?q=cli&project=codexcrew')
...
describe('request bodies with conditionally-omitted keys', () => {
  ...
  it('createChatFolder folds the modal settings into the create body', async () => {
    ...
    await api.createChatFolder('Work', 'p1', { project_dir: '/r', default_agent: 'codexcrew', color: 'blue' })
    expect(call(1).body).toEqual({
      name: 'Work', parent_id: 'p1', project_dir: '/r', default_agent: 'codexcrew', color: 'blue',
    })
  })
