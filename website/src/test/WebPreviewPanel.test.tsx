...
describe('isolatePreviewHost', () => {
  ...
  it('isolates a same-host *.localhost dashboard (e.g. codexcrew.localhost) to 127.0.0.1', () => {
    expect(isolatePreviewHost('http://codexcrew.localhost:5173/', 'codexcrew.localhost'))
  ...
  it('leaves a preview host that already differs from the dashboard host', () => {
    ...
    expect(isolatePreviewHost('http://localhost:5173/', 'codexcrew.localhost')).toBe('http://localhost:5173/')
  })
...
describe('isDashboardOrigin', () => {
  it('matches the gateway across the two loopback aliases (same port)', () => {
    ...
    expect(isDashboardOrigin('http://127.0.0.1:6776/', 'http://codexcrew.localhost:6776/')).toBe(true)
  })
...
describe('WebPreviewPanel', () => {
  ...
  it('dispatches a snip request when the crop button is clicked', () => {
    ...
    window.addEventListener('codexcrew-web-preview-snip', handler)
    try {
      renderWithProviders(<WebPreviewPanel sessionKey="sess-1" />)
      fireEvent.click(screen.getByLabelText('Screenshot an area into the chat'))
      expect(fired).toBe(true)
    } finally {
      window.removeEventListener('codexcrew-web-preview-snip', handler)
    }
  })
  ...
  it('broadcasts preview-expand true/false as the expand button toggles', () => {
    ...
    window.addEventListener('codexcrew-preview-expand', handler)
    try {
      renderWithProviders(<WebPreviewPanel sessionKey="sess-1" />)
      ...
      window.removeEventListener('codexcrew-preview-expand', handler)
    }
  })
})
