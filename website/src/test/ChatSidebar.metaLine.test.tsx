...
const TAGS = [
  { id: 'codexcrew', name: 'backend', color: '#6b7280', order: 5, status: false },
...
const SLOTS: ChatSlot[] = [
  { key: 'k-tag', title: 'has-a-tag', running: false, messages: 2, agent: 'codexcrew', tags: ['codexcrew'], last_ts: LAST_TS },
  { key: 'k-bare', title: 'no-tags', running: false, messages: 2, agent: 'codexcrew', tags: [], last_ts: LAST_TS },
  { key: 'k-pin', title: 'is-pinned', running: false, messages: 2, agent: 'codexcrew', tags: ['themes'], pinned: true, last_ts: LAST_TS },
  ...
  { key: 'k-mixed', title: 'status-plus-identity', running: false, messages: 2, agent: 'codexcrew', tags: ['review', 'codexcrew'], last_ts: LAST_TS },
...
describe('chat sidebar — session row meta line', () => {
  it('renders the session\'s tag after the agent name, separated by "·"', () => {
    ...
    expect(line.textContent).toContain('codexcrew')   // agent
  ...
  it('renders no separate chip row — tags live only in the meta line', () => {
    ...
    const { container } = renderSidebar()
    const line = metaLineFor(container, 'has-a-tag')
    expect(line.querySelector('[data-testid="slot-tag-codexcrew"]')).toBeTruthy()
    ...
    expect(content.querySelectorAll('[data-testid="slot-tag-codexcrew"]')).toHaveLength(1)
  })
