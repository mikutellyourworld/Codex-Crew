...
describe('PullRequestPanel', () => {
  ...
  it('shows an actionable warning when the local GitHub CLI is not logged in', async () => {
    ...
    expect(alert).toHaveTextContent('Codex Crew uses your local provider CLI')
  ...
  it('preserves trusted-install guidance when the local CLI is unavailable', async () => {
    mockApi.pullRequestSource.mockRejectedValueOnce(
      new Error('{"error":"The local GitHub CLI (gh) was not found in a trusted system location. Install a root-owned `gh` at `/usr/local/libexec/codexcrew/gh`, run `gh auth login`, then retry."}'),
    )
    ...
    expect(alert).toHaveTextContent('/usr/local/libexec/codexcrew/gh')
    expect(alert).toHaveTextContent('gh auth login')
