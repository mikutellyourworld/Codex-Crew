...
describe('useWebSocket folder-tree generation invalidation', () => {
  ...
  it('invalidates when the generation moves — an agent created a folder', () => {
    ...
    act(() => {
      ws.simulateMessage({
        type: 'slots',
        data: [{ key: 'chat-1', title: 's', agent: 'codexcrew', folder_id: 'f2' }],
  ...
  it('does NOT invalidate when the generation is unchanged (optimistic edits survive)', () => {
    ...
    act(() => { ws.simulateMessage({ type: 'slots', data: [{ key: 'a', title: 'x', agent: 'codexcrew' }], foldersGeneration: 4 }) })
    act(() => { ws.simulateMessage({ type: 'slots', data: [{ key: 'b', title: 'y', agent: 'codexcrew' }], foldersGeneration: 4 }) })
    act(() => { ws.simulateMessage({ type: 'slots', data: [{ key: 'c', title: 'z', agent: 'codexcrew' }], foldersGeneration: 4 }) })
