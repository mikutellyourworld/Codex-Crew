...
describe('JobForm script/command edit path', () => {
  it('jobKindOf derives the execution kind from the carried field', () => {
    expect(jobKindOf(makeJob({ script: '~/.codexcrew/crons/f.py:run' }))).toBe('script')
  ...
  it('parseJobDefaults seeds jobKind=script for a script cron with empty message', () => {
    const result = parseJobDefaults(makeJob({ script: '~/.codexcrew/crons/nba_progress_nudge.py:run' }))
  ...
  it('buildBody does NOT require a message for a script cron and omits message/agent', () => {
    let error = ''
    const f = { ...parseJobDefaults(makeJob({ script: '~/.codexcrew/crons/f.py:run' })) }
  ...
  it('buildBody changing only the schedule of a script cron still saves (interval mode)', () => {
    let error = ''
    const base = parseJobDefaults(makeJob({ script: '~/.codexcrew/crons/f.py:run' }))
  ...
  it('buildBody requires a name for every kind', () => {
    let error = ''
    const f = { ...parseJobDefaults(makeJob({ script: '~/.codexcrew/crons/f.py:run', name: '' })) }
    const body = buildBody(f, 'UTC', e => { error = e })
    expect(body).toBeNull()
  ...
  it('renders the script value read-only and hides the Agent selector in the detail panel', () => {
    renderWithProviders(
      <JobForm
        job={makeJob({ script: '~/.codexcrew/crons/nba_progress_nudge.py:run' })}
        agents={[{ name: 'codexcrew', description: '' } as never]}
    ...
    // Script value shown
    expect(screen.getByText('~/.codexcrew/crons/nba_progress_nudge.py:run')).toBeInTheDocument()
