...
describe('gauge instruments', () => {
  it('renders a gauge row under Gauges with its latest value, not zero', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('codexcrew.process.threads.os')).toBeInTheDocument()
    })
    const gaugeRow = screen.getByText('codexcrew.process.threads.os').closest('div')!
    ...
    const counterRow = screen.getByText('codexcrew.mcp.warm_pool.acquire').closest('div')!
  ...
  it('formats byte gauges with units and renders the per-PID breakdown', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('codexcrew.process.memory.rss_bytes')).toBeInTheDocument()
    })
