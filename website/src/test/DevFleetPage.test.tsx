...
describe('DevFleetPage', () => {
  ...
  it('shows a setup state, not a discovery error, when no checkout was found', async () => {
    ...
    expect(screen.getByText('No Codex Crew checkout found')).toBeInTheDocument()
    expect(screen.getByText(/CODEXCREW_DEVFLEET_REPO=/)).toBeInTheDocument()
  ...
  it('warns when the install serving the dashboard is not the managed checkout', async () => {
    mockFleet({
      serving_install_reason: 'this dashboard is served by the install at /Applications/CodexCrew.app/Contents/Resources/backend-dist/codexcrew-backend-arm64/lib/python3.12/site-packages/codex_crew, which is not inside the checkout Dev Fleet manages (/Users/dev/codexcrew).',
...
describe('DevFleetPage restart handshake', () => {
  ...
  it('keeps a failed restart on the page instead of only in a toast', async () => {
    ...
    const RESTART_ERR = "loaded launchd restart contract is outdated; re-run `codexcrew service install`"
    const FLEET_LIVE = {
      gateway_service_active: true,
      worktrees: [
        { name: 'main', is_main: true, running: false, has_dist: true, behind: 0, is_live: true, path: '/wt/main' },
    ...
    expect(banner.textContent).toContain('codexcrew service install')
    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation((url) => {
      ...
      if (u.includes('/make-live')) {
        return Promise.resolve(new Response(JSON.stringify({
          ...
          manual_restart: 'sudo systemctl restart codexcrew',
          notice: 'main is now the live target. Run `sudo systemctl restart codexcrew` to finish the cutover.',
        }), { status: 200 }))
      }
  ...
  it('marks a staged worktree restart-pending instead of live', async () => {
    ...
    const FLEET = {
      ...
      manual_restart: 'codexcrew restart',
      worktrees: [
