 ...
 * sends `wait`, the pooled gateway namespaces it `codexcrew-core___wait`, and one
 ...
 *
 * Frontend mirror of `src/codex_crew/acp/liveness.py::is_wait_tool`. The two have
...
describe('isWaitToolTitle', () => {
  it('matches every transport shape of the wait tool title', () => {
    ...
    expect(isWaitToolTitle('codexcrew-core___wait')).toBe(true) // pooled gateway
  ...
  it('is case-insensitive and ignores surrounding whitespace', () => {
    ...
    expect(isWaitToolTitle('Wait')).toBe(true)
    expect(isWaitToolTitle(' wait ')).toBe(true)
    expect(isWaitToolTitle('\tCodexCrew-Core___Wait\n')).toBe(true)
  })
  ...
  it('accepts a namespace prefix and an mcp suffix together', () => {
    ...
    expect(isWaitToolTitle('codexcrew-core___wait (mcp)')).toBe(true)
