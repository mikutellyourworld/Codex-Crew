...
const server = (name: string): McpServer => ({
  name, command: `${name}-cmd`, status: 'ok', source: 'codexcrew', enabled: true, tools: ['t1'],
})
...
beforeEach(() => {
  ...
  mockApi.codexcrewConfig.mockResolvedValue({})
})
...
describe('McpTab needs_auth status', () => {
  ...
  it('offers an in-place Sign in on a resolvable managed row when connections_ui is on', async () => {
    mockApi.codexcrewConfig.mockResolvedValue({ connections_ui: true })
    mockApi.mcpServers.mockResolvedValue([
      {
        ...remote('needs_auth'),
        name: 'notion',
  ...
  it('FIX 1: falls back to the chat prose on a resolvable managed row when connections_ui is OFF', async () => {
    ...
    mockApi.codexcrewConfig.mockResolvedValue({}) // flag off (also the default)
    mockApi.mcpServers.mockResolvedValue([
      {
        ...remote('needs_auth'),
        name: 'notion',
