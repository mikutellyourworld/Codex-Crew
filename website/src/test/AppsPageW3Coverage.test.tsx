...
const BUILTIN_OFF = {
  ...
  manifest: {
    ...
    author: 'codexcrew', tags: ['fun'], ui: { pages: [{ route: '/pets', label: 'Pets', icon: 'Cat' }] },
  },
}
...
const REGISTRY_APPS = [
  {
    name: 'oncall-radar', displayName: 'Oncall Radar', author: 'codexcrew',
 ...
 */
const builtinServerRow = (name: string, displayName: string, author = 'codexcrew') => ({
...
describe('AppsPage — uninstall dialog', () => {
  ...
  it('warns about a self-managed app whose resources live outside Codex Crew', async () => {
...
describe('AppsPage — editorial layer wiring', () => {
  ...
  it('enables from the lead card and Gets from a row card', async () => {
    ...
    listRegistry.mockResolvedValue({
      apps: [
        {
          name: 'zeta-app', displayName: 'Zeta App', author: 'codexcrew', description: 'Later in the alphabet.',
          version: '1.0.0', tags: ['github'], installed: false, provenance: 'core',
        },
        {
          name: 'zulu-utility', displayName: 'Zulu Utility', author: 'codexcrew', description: 'Fills the second row slot.',
          version: '1.0.0', tags: ['github'], installed: false, provenance: 'core',
        },
...
describe('AppsPage — sources rail', () => {
  it('counts built-ins, a stale registry, and the core registry separately', async () => {
    listRegistry.mockResolvedValue({
      apps: [
        { name: 'core-app', displayName: 'Core App', author: 'codexcrew', description: 'From the core file.', version: '1.0.0', tags: ['github'], installed: false, provenance: 'core' },
    ...
    expect(within(rail).getByText('Built-in · codexcrew')).toBeInTheDocument()
    ...
    expect(within(rail).getByText('Codex Crew registry')).toBeInTheDocument()
