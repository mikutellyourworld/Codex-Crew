...
const MANIFEST = {
  alias: 'tester',
  manifest: 'display_information:\n  name: CodexCrew-tester\n',  // brand-ok: product emits CodexCrew-<alias> (slack-manifest.yaml)
...
describe('SlackPanel manifest card', () => {
  it('links the one-click create URL and copies the YAML, reverting the pill after 1.5s', async () => {
    ...
    expect(screen.getByText(/named CodexCrew-tester/)).toBeInTheDocument()  // brand-ok: product emits CodexCrew-<alias> (slack-manifest.yaml)
  ...
  it('inertly disables create + copy while the manifest is unavailable', async () => {
    ...
    expect(screen.getByText(/named CodexCrew-you/)).toBeInTheDocument()  // brand-ok: product emits CodexCrew-<alias> (slack-manifest.yaml)
...
describe('SlackPanel read-only session', () => {
  it('drops every writer and shows stored previews only', async () => {
    ...
    expect(
      screen.getByText(
        'Slack settings are managed on the machine running Codex Crew and are read-only from remote sessions.',
      ),
    ).toBeInTheDocument()
...
describe('SlackPanel save payload', () => {
  it('trims the identity fields, forwards both toggles, and reports success', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('codexcrew'), { target: { value: ' crew ' } })
