...
const agents: AgentItem[] = [
  { name: 'codexcrew', source: 'codexcrew', description: 'Main agent' },
  { name: 'builtin', source: 'builtin' },
]

describe('AgentDropdownList', () => {
  it('renders all agents', () => {
    render(<AgentDropdownList agents={agents} activeAgent="codexcrew" defaultAgent="codexcrew" onSelect={() => {}} />)
    expect(screen.getAllByText('codexcrew').length).toBeGreaterThan(0)
  ...
  it('calls onSelect with the agent name when clicked', () => {
    ...
    const btn = Array.from(document.querySelectorAll('button')).find(
      b => b.querySelector('.font-mono')?.textContent === 'codexcrew'
    )
    fireEvent.click(btn!)
    expect(onSelect).toHaveBeenCalledWith('codexcrew')
  })
...
describe('AgentDropdownList default-agent affordance', () => {
  it('labels the default agent with a Default pill instead of its source badge', () => {
    render(<AgentDropdownList agents={agents} activeAgent="" defaultAgent="codexcrew" onSelect={() => {}} />)
