 ...
 * resolved value -- except `codexcrewAgents`, which is the failure under test.
 */
...
const member = (over: Raw = {}): Raw => ({
  id: 'a1', role: 'Researcher', agent_name: 'codexcrew',
...
beforeEach(() => {
  ...
  vi.mocked(api).codexcrewAgents = vi.fn().mockRejectedValue(new Error('gateway restarting'))
})
...
describe('ChannelPage Add Agent roster failure (#5990)', () => {
  ...
  it('recovers the roster when the retry succeeds', async () => {
    ...
    vi.mocked(api).codexcrewAgents = vi.fn().mockResolvedValue({ agents: ROSTER, default_agent: 'codexcrew' })
