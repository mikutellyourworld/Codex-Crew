...
const AGENTS = [
  {
    name: 'reviewer', kiro_agent: 'codexcrew', workspace: 'default', memory_store: 'default',
    model: '', description: 'Reviews code changes', source: 'user',
  },
  {
    name: 'oncall', kiro_agent: 'codexcrew', workspace: 'default', memory_store: 'default',
...
describe('request examples and responsive shell', () => {
  it('ties the collapsed request example to each source signing policy', async () => {
    ...
    expect(signed).toContain('X-CodexCrew-Signature: sha256=$SIG')
    ...
    expect(bearer).not.toContain('X-CodexCrew-Signature')
  })
