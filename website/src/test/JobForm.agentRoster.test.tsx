...
vi.mock('../api/client', () => ({
  api: {
    ...
    codexcrewAgents: vi.fn().mockResolvedValue({ agents: [], default_agent: '' }),
  },
}))
 ...
 */
const roster: CodexCrewAgent[] = [
  { name: 'codexcrew', kiro_agent: 'codexcrew', workspace: 'default', memory_store: 'default', description: 'built-in', source: 'codexcrew' },
...
describe('cron JobForm agent selector roster (#5990)', () => {
  it('offers the WHOLE roster in the side-panel (vertical) edit form, not just the default', () => {
    renderWithProviders(
      <JobForm job={messageJob()} agents={roster} defaultAgent="codexcrew" onSaved={() => {}} layout="vertical" />,
    )
  ...
  it('offers the WHOLE roster in the inline (horizontal) create form', () => {
    renderWithProviders(
      <JobForm agents={roster} defaultAgent="codexcrew" onSaved={() => {}} layout="horizontal" />,
    )
    const options = openRosterOptions()
    expect(options).toHaveLength(roster.length)
    for (const a of roster) {
  ...
  it('honours a stored NON-default agent and still offers the rest of the roster', () => {
    renderWithProviders(
      <JobForm job={messageJob({ agent: 'oncall' })} agents={roster} defaultAgent="codexcrew" onSaved={() => {}} layout="vertical" />,
    )
...
describe('cron JobForm agent selector roster failure (#5990)', () => {
  it('says the roster failed to load, rather than claiming there are no matches', () => {
    renderWithProviders(
      <JobForm job={messageJob()} agents={[]} defaultAgent="codexcrew" rosterFailure={{ reloading: false, onReload: () => {} }} onSaved={() => {}} layout="vertical" />,
    )
    fireEvent.click(screen.getByLabelText('Switch agent'))
  ...
  it('offers a retry that re-runs the roster fetch', () => {
    ...
    renderWithProviders(
      <JobForm job={messageJob()} agents={[]} defaultAgent="codexcrew" rosterFailure={{ reloading: false, onReload: onReloadRoster }} onSaved={() => {}} layout="vertical" />,
    )
    fireEvent.click(screen.getByLabelText('Switch agent'))
  ...
  it('reports the failure on the inline create form too', () => {
    renderWithProviders(
      <JobForm agents={[]} defaultAgent="codexcrew" rosterFailure={{ reloading: false, onReload: () => {} }} onSaved={() => {}} layout="horizontal" />,
    )
    fireEvent.click(screen.getByLabelText('Switch agent'))
  ...
  it('keeps a roster it still holds usable when a later refresh failed', () => {
    renderWithProviders(
      <JobForm job={messageJob()} agents={roster} defaultAgent="codexcrew" rosterFailure={{ reloading: false, onReload: () => {} }} onSaved={() => {}} layout="vertical" />,
    )
    const options = openRosterOptions()
  ...
  it('shows the retry as in flight, so a retry that fails again is not silent', () => {
    renderWithProviders(
      <JobForm job={messageJob()} agents={[]} defaultAgent="codexcrew" rosterFailure={{ reloading: true, onReload: () => {} }} onSaved={() => {}} layout="vertical" />,
    )
    fireEvent.click(screen.getByLabelText('Switch agent'))
