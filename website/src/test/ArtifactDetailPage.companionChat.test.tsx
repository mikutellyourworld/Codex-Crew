...
describe('ArtifactDetailPage companion chat', () => {
  ...
  it('navigates away when the artifact is deleted elsewhere', async () => {
    ...
    await waitForLoaded()
    act(() => {
      window.dispatchEvent(new CustomEvent('codexcrew:artifact-deleted', { detail: { slug: 'cr-queue' } }))
    })
  ...
  it('ignores a delete event for a different artifact', async () => {
    ...
    await waitForLoaded()
    act(() => {
      window.dispatchEvent(new CustomEvent('codexcrew:artifact-deleted', { detail: { slug: 'other' } }))
    })
