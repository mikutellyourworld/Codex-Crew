...
const ISSUE_URL = `https://github.com/mikutellyourworld/Codex-Crew/issues/new?${LONG_BENIGN_QUERY}`
...
describe('sanitizeExfiltrationUrls: the reported false positive', () => {
  ...
  it('keeps it when the host case differs', () => {
    ...
    expectKept(`https://GitHub.com/mikutellyourworld/Codex-Crew/issues/new?${LONG_BENIGN_QUERY}`)
  })
...
describe('sanitizeExfiltrationUrls: every span of the validated shape is load-bearing', () => {
  it('redacts an unvalidated path at the same host', () => {
    expectRedacted(`https://github.com/mikutellyourworld/Codex-Crew/settings?${LONG_BENIGN_QUERY}`)
  })
...
describe('sanitizeExfiltrationUrls: no pattern signal is waived', () => {
  ...
  const validPrefix = 'https://github.com/mikutellyourworld/Codex-Crew/issues/new?body='
...
describe('sanitizeExfiltrationUrls: the `+`-encoded prose body is still redacted', () => {
  ...
  const PLUS_PROSE_URL =
    'https://github.com/mikutellyourworld/Codex-Crew/issues/new?title=Dashboard+chat+drops+long+links' +
