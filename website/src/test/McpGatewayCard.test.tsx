...
describe('backendRowKey', () => {
  ...
  it('falls back to the row index when pid is null so keys stay unique', () => {
    ...
    // pid alone would collide to "codexcrew-core-null" and trigger React key warnings.
    const a = backendRowKey({ server: 'codexcrew-core', pid: null }, 0)
    const b = backendRowKey({ server: 'codexcrew-core', pid: null }, 1)
    expect(a).toBe('codexcrew-core-i0')
    expect(b).toBe('codexcrew-core-i1')
    expect(a).not.toBe(b)
  })
})
