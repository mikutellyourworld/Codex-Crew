...
const TERM: DictionaryTerm = { correct: 'Codex Crew', aliases: ['kiro cru', 'kero crew'] }
...
describe('Meetings SettingsView — speech dictionary', () => {
  ...
  it('splits the aliases on commas, submits on Enter, and clears both fields', async () => {
    ...
    fireEvent.change(correct, { target: { value: '  Codex Crew  ' } })
    fireEvent.keyDown(correct, { key: 'Enter' })

    await waitFor(() =>
      expect(apiMocks.addTerm).toHaveBeenCalledWith('Codex Crew', ['kiro cru', 'kero crew']),
    )
    ...
    expect(
      await screen.findByRole('button', { name: 'Remove Codex Crew' }, { timeout: 5_000 }),
    ).toBeInTheDocument()
  })
  ...
  it('reports a correction the backend rejected', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Correct term'), { target: { value: 'Codex Crew' } })
    fireEvent.click(screen.getByRole('button', { name: 'Add' }))
  ...
  it('removes a correction and falls back to the empty state', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Remove Codex Crew' }))

    await waitFor(() => expect(apiMocks.removeTerm).toHaveBeenCalledWith('Codex Crew'))
    expect(await screen.findByTestId('empty-state-title')).toHaveTextContent('No corrections yet')
  })
})
