  ...
  const spy = vi.fn(async (input: unknown, init?: { method?: string }) => {
    ...
    if (url.includes('/api/update/arm') && init?.method === 'POST') {
      ...
      return json({
        ...
        expires_in: 600, approve_command: 'codexcrew update approve',
      })
    }
    if (url.includes('/api/update/arm')) {
      return json({ armed: true, request_id: 'r1', version: '9.9.9', expires_in: 590, approve_command: 'codexcrew update approve' })
    }
...
describe('AboutPanel in-app update (arm + approve)', () => {
  ...
  it('renders the Update button for an armable install and arms on click', async () => {
    ...
    expect(screen.getByTestId('approve-command').textContent).toContain('codexcrew update approve')
    ...
    try {
      ...
      spy.mockImplementation(async (input: unknown, init?: { method?: string }) => {
        ...
        if (url.includes('/api/update/arm') && init?.method === 'POST') {
          return {
            ok: true, status: 200,
            json: async () => ({ ok: true, armed: true, request_id: 'r1', version: '9.9.9', expires_in: 600, approve_command: 'codexcrew update approve' }),
