...
const BUILTIN = {
  ...
  skills: ['prepare-pr'],
  mcp_servers: ['codexcrew-core'],
  filename: 'codexcrew.json',
}
const USER_TEMPLATE = {
  name: 'reviewer',
...
const BUILTIN_DETAIL = {
  ...BUILTIN,
  prompt: 'file://~/.codex-crew/prompts/codexcrew.md',
  tools: ['fs_read', 'fs_write'],
  allowedTools: ['fs_read'],
  mcpServers: { 'codexcrew-core': {} },
...
beforeEach(() => {
  ...
  mockApi.defaultAgent.mockResolvedValue({ default_agent: 'codexcrew' })
  ...
  mockApi.codexcrewAgents.mockResolvedValue({
    agents: [
      { name: 'default', kiro_agent: 'codexcrew', workspace: 'default', memory_store: 'default', description: '', source: 'user' },
      { name: 'oncall', kiro_agent: 'codexcrew', workspace: 'oncall', memory_store: 'oncall', description: '', source: 'user' },
  ...
  mockApi.agentDetail.mockImplementation((name: string) => {
    if (name === 'codexcrew') return Promise.resolve(BUILTIN_DETAIL)
...
describe('agent templates inspector — tabs', () => {
  it('gives the prompt and the guardrails a pane each instead of one scroll box', async () => {
    ...
    expect(await screen.findByText(/prompts\/codexcrew\.md/)).toBeInTheDocument()
...
describe('agent templates inspector — shared crews cache', () => {
  /* The Crews tab owns ['codexcrew-agents', <trigger>] and stores the whole
  ...
  it('reads and leaves the cache in the shape the Crews tab stores', async () => {
    ...
    qc.setQueryData(['codexcrew-agents'], {
      agents: [{ name: 'default', kiro_agent: 'codexcrew', workspace: 'default', memory_store: 'default', description: '', source: 'user' }],
      default_agent: 'default',
    })
    ...
    expect(await screen.findByText('Used By')).toBeInTheDocument()
    expect(screen.getByText('default')).toBeInTheDocument()
    const cached = qc.getQueryData(['codexcrew-agents'])
...
describe('agent templates inspector — delete', () => {
  ...
  it('withholds delete until the crew roster has actually loaded', async () => {
    ...
    mockApi.codexcrewAgents.mockImplementation(() => new Promise(resolve => { landCrews = resolve }))
    renderPage()
    await open('reviewer')
  ...
  it('surfaces a server refusal and re-syncs instead of failing silently', async () => {
    ...
    await waitFor(() => expect(mockApi.codexcrewAgents.mock.calls.length).toBeGreaterThan(1))
  ...
  it('withholds delete when a reference REFETCH fails, leaving stale data behind', async () => {
    ...
    const qc = new QueryClient({ defaultOptions: { queries: { retry: false } } })
    qc.setQueryData(['codexcrew-agents'], { agents: [], default_agent: '' })

    renderPage(qc)
    await open('scratch')
    expect(await screen.findByTestId('delete-template')).toBeInTheDocument()

    mockApi.codexcrewAgents.mockRejectedValue(new Error('network'))
    await qc.refetchQueries({ queryKey: ['codexcrew-agents'] })
