...
const RUNNING_JOB = {
  ...
  steps: [
    ...
    { key: 'install', label: 'Installing Codex Crew', state: 'active' as const },
...
describe('RemoteCrewPanel', () => {
  it('never offers the plain-machine delete to a cloud crew while the launch history is still loading', async () => {
    ...
    expect(await screen.findByText('Launched by Codex Crew')).toBeInTheDocument()
    await openRowMenu(u)
    expect(screen.getByRole('menuitem', { name: 'Stop Codex Crew Cloud (kc-3f9a)' })).toBeInTheDocument()
  ...
  it('does not offer a one-click Remove to an SSM crew it cannot identify', async () => {
    ...
    await openRowMenu(u)
    await u.click(screen.getByRole('menuitem', { name: /Remove Codex Crew Cloud/i }))
  ...
  it('still lists the crews when the gateway cannot do cloud provisioning at all', async () => {
    ...
    expect(screen.getByText(/Codex Crew Cloud/)).toBeInTheDocument()
  ...
  it('offers Start so Stop is not a one-way door', async () => {
    ...
    await openRowMenu(u)
    await u.click(await screen.findByRole('menuitem', { name: /^Start Codex Crew Cloud/i }))
  ...
  it('shows progress on the button that was clicked', async () => {
    ...
    await openRowMenu(u)
    await u.click(await screen.findByRole('menuitem', { name: /^Stop Codex Crew Cloud/i }))
    // While in flight the clicked button reports progress rather than still saying "Stop".
    await waitFor(() => expect(screen.getByRole('button', { name: /^Stop Codex Crew Cloud/i })).toHaveTextContent('…'))
    release({ ok: true })
  })
  ...
  it('shows a Deleting… state after the delete is accepted, instead of leaving the row untouched', async () => {
    ...
    await openRowMenu(u)
    await u.click(await screen.findByRole('menuitem', { name: /^Delete Codex Crew Cloud/i }))
  ...
  it('distinguishes cloud crews from hand-added machines, and shows an in-progress launch', async () => {
    ...
    await openRowMenu(u, /More actions for Codex Crew Cloud/i)
    expect(screen.getByRole('menuitem', { name: 'Stop Codex Crew Cloud (kc-3f9a)' })).toBeInTheDocument()
  ...
  it('drops the retry when the load failed because the session no longer authenticates', async () => {
    ...
    const denial = new ApiError(403, 'Session expired. Run codexcrew token …')
    ;(denial as unknown as { authRequired: boolean }).authRequired = true
    vi.mocked(api.listInstances).mockRejectedValue(denial)
    vi.mocked(api.cloudLaunches).mockResolvedValue({ jobs: [] })
    renderWithProviders(<RemoteCrewPanel />)

    expect(await screen.findByText(/codexcrew token/i)).toBeInTheDocument()
  ...
  it('launches a cloud crew when prerequisites pass and shows the progress card', async () => {
    ...
    expect(await screen.findByText('Installing Codex Crew')).toBeInTheDocument()
  })
