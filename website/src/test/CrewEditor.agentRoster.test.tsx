...
/* The transport under the REAL `useAgents`: rejecting `codexcrewAgents` is the
...
const ACTIVE = { owner: 'kirodotdev', repo: 'CodexCrew' } // brand-ok: the repository name
...
const ROSTER = [
  { name: 'codexcrew', source: 'kiro', description: 'The default agent' },
  { name: 'codexcrew-crew', source: 'kiro', description: 'Issue worker' },
]
...
describe('CrewEditor roster failure wiring (#7656)', () => {
  beforeEach(async () => {
    ...
    vi.mocked(api).codexcrewAgents.mockRejectedValue(new Error('gateway restarting'))
  })
  ...
  it('retry re-fetches the roster and clears the error line', async () => {
    ...
    vi.mocked(api).codexcrewAgents.mockResolvedValue({ agents: ROSTER, default_agent: 'codexcrew' })
    fireEvent.click(agentField().getByText(i18nT(`${K}.agent_roster_retry`)))

    await waitFor(() => {
      expect(agentField().getByRole('option', { name: 'codexcrew-crew' })).toBeInTheDocument()
    })
