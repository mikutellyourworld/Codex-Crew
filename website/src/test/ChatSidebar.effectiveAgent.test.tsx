...
const SLOTS: ChatSlot[] = [
  // The divergence: bound to a `mochi` that nothing dispatches, so `codexcrew`
  // takes the turns.
  { key: 'k-diverged', title: 'diverged', running: false, messages: 2, agent: 'mochi', effective_agent: 'codexcrew', tags: [], last_ts: LAST_TS },
...
describe('chat sidebar — effective-agent marker', () => {
  it('names the agent that actually answers when it differs', () => {
    ...
    expect(marker!.textContent).toContain('codexcrew')
  })
