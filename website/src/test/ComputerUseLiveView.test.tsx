...
function pushFrame(data: string, extra: Record<string, unknown> = {}) {
  window.dispatchEvent(
    new CustomEvent('codexcrew-computer-use-frame', {
...
function pushToggle() {
  window.dispatchEvent(new CustomEvent('codexcrew-toggle-computer-use-live'))
}
...
describe('ComputerUseLiveView', () => {
  ...
  it('stays hidden for a frame that carries no image data', async () => {
    ...
    await act(async () => {
      window.dispatchEvent(
        new CustomEvent('codexcrew-computer-use-frame', { detail: { format: 'jpeg' } }),
      )
    })
    expect(container.firstChild).toBeNull()
  })
