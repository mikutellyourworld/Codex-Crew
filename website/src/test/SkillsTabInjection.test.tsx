...
describe('injection control on the Skills page', () => {
  ...
  it('writes the opt-out when the toggle is switched off', async () => {
    ...
    await waitFor(() =>
      expect(mockApi.setSkillInjectOnTrigger).toHaveBeenCalledWith('codexcrew-commands', false))
  })
  ...
  it('writes the opt-in when an opted-out skill is switched back on', async () => {
    ...
    await waitFor(() =>
      expect(mockApi.setSkillInjectOnTrigger).toHaveBeenCalledWith('codexcrew-commands', true))
  })
  ...
  it('hides the control for a skill outside the directory Codex Crew owns', async () => {
    /* A `skills.extra_paths` skill still reports source `codexcrew`, but the
