...
describe('a provider that needs attention', () => {
  ...
  it('rewrites the endpoint and re-enables a disabled entry, because reconnect IS consent', async () => {
    mcpServers.mockResolvedValue([server({
      status: 'error',
      enabled: false,
      presence: { codexcrew: false, kiroGlobal: true, ccGlobal: false },
    })])
    mount()
    ...
    // Global scopes are passed through unchanged; only Codex Crew's own is turned on.
    expect(mcpApply).toHaveBeenCalledWith([{ name: 'notion', codexcrew: true, kiroGlobal: true, ccGlobal: false }])
  })
