...
describe('subagent reducers', () => {
  ...
  it('sseSubagentSpawn carries the resolved model, and later frames never blank a known model (#3582)', () => {
    ...
    let state = reducer(withSlot, sseSubagentSpawn({ slot: 'slot-1', id: 'a1', task: 't', agent: 'codexcrew', model: 'claude-opus-4.8' }))
    ...
    let s2 = reducer(withSlot, sseSubagentSpawn({ slot: 'slot-1', id: 'a2', task: 't', agent: 'codexcrew', model: 'gpt-5.6-sol' }))
  ...
  it('sseSubagentSnapshot restores the model on reconnect', () => {
    const state = reducer(withSlot, sseSubagentSnapshot({
      id: 'a1', slot: 'slot-1', task: 't', agent: 'codexcrew', model: 'claude-opus-4.8',
  ...
  it('sseSubagentSnapshot preserves a live retrying flag on reconnect (#7472-adjacent)', () => {
    ...
    let state = reducer(withSlot, sseSubagentSpawn({ slot: 'slot-1', id: 'a1', task: 't', agent: 'codexcrew' }))
    ...
    state = reducer(state, sseSubagentSnapshot({
      id: 'a1', slot: 'slot-1', task: 't', agent: 'codexcrew',
      streaming: '', last_tool: '', started: 1,
    }))
    expect(state.subagents['a1'].retrying).toBe(true)
  })
  ...
  it('sseSubagentSpawn preserves existing streaming text from pending', () => {
    ...
    state = reducer(state, sseSubagentSpawn({ slot: 'slot-1', id: 'a1', task: 'task', agent: 'codexcrew' }))
    expect(state.subagents['a1'].status).toBe('running')
  ...
  it('sseSubagentSpawn copies task onto pending card (empty approval-derived task)', () => {
    ...
    state = reducer(state, sseSubagentSpawn({ slot: 'slot-1', id: 'a1', task: 'scan package X', agent: 'codexcrew' }))
    expect(state.subagents['a1'].status).toBe('running')
...
describe('approval_resolved and toolLog mutations', () => {
  ...
  it('sseToolActivity is_update merges into existing entry by tool_call_id', () => {
    ...
    expect(state.toolLog).toHaveLength(1)
    state = reducer(state, sseToolActivity({ slot: 'slot-1', tool: 'List CodexCrew modules', kind: 'execute', purpose: '', input_preview: '{"command":"ls"}', tool_call_id: 'tc-bash-1', is_update: true }))
    expect(state.toolLog).toHaveLength(1)
    expect(state.toolLog[0].text).toBe('List CodexCrew modules')
...
describe('selectSlotPendingSpawnApprovals', () => {
  ...
  it('drops the approval once the sub-agent starts running', () => {
    ...
    state = reducer(state, sseSubagentSpawn({ slot: 'slot-1', id: 'a1', task: 't', agent: 'codexcrew' }))
