/**
 * CodexCrewCfgTab — the Codex Crew config table on the developer page.
 *
...
function seed(cfg: Cfg = CFG, patched: Cfg = CFG) {
  ...
  m.codexcrewConfig = vi.fn().mockResolvedValue(cfg)
...
async function renderTab() {
  const view = renderWithProviders(<CodexCrewCfgTab />)
...
// ── Query boundaries ─────────────────────────────────────────────────────
describe('CodexCrewCfgTab — query boundaries', () => {
  it('shows a skeleton until the config resolves', async () => {
    const m = vi.mocked(api)
    let release: (v: unknown) => void = () => {}
    m.codexcrewConfig = vi.fn().mockReturnValue(new Promise((res) => { release = res }))

    const { container } = renderWithProviders(<CodexCrewCfgTab />)
    expect(container.querySelector('.skeleton')).not.toBeNull()
    expect(screen.queryByText('Codex Crew Agents')).toBeNull()
  ...
  it('renders an Error rejection by its message', async () => {
    const m = vi.mocked(api)
    m.codexcrewConfig = vi.fn().mockRejectedValue(new Error('config file unreadable'))
  ...
  it('stringifies a non-Error rejection', async () => {
    const m = vi.mocked(api)
    m.codexcrewConfig = vi.fn().mockRejectedValue('gateway offline')
...
describe('CodexCrewCfgTab — tables', () => {
...
describe('CodexCrewCfgTab — numeric rows', () => {
...
// ── CfgSelect + CfgToggle ────────────────────────────────────────────────
describe('CodexCrewCfgTab — select and toggle rows', () => {
  ...
  it('surfaces a failed patch in both cards that host the save banner', async () => {
    ...
    await waitFor(() => expect(m.codexcrewConfig).toHaveBeenCalledTimes(2))
  })
...
// ── SubagentSettings ─────────────────────────────────────────────────────
describe('CodexCrewCfgTab — subagent settings', () => {
  ...
  it('shows a rejection returned in the payload', async () => {
    ...
    expect(m.codexcrewConfig).toHaveBeenCalledTimes(1)
  })
