...
const mockApi = api as unknown as {
  codexcrewAgents: ReturnType<typeof vi.fn>
...
beforeEach(() => {
  ...
  mockApi.codexcrewAgents.mockResolvedValue({
    agents: [{ name: 'codexcrew', scope: 'global' }],
...
describe('useAgents project scoping', () => {
  it('re-fetches when the same slot is pointed at a project', async () => {
    ...
    await waitFor(() => expect(mockApi.codexcrewAgents).toHaveBeenCalledTimes(1))

    mockApi.codexcrewAgents.mockResolvedValue({
      agents: [
        { name: 'codexcrew', scope: 'global' },
  ...
  it('still sends only the session key — the server derives project scope', async () => {
    ...
    for (const call of mockApi.codexcrewAgents.mock.calls) {
  ...
  it('clears the old project\'s roster while the new project\'s fetch is in flight', async () => {
    ...
    mockApi.codexcrewAgents.mockImplementationOnce(
    ...
    resolveSecond({ agents: [{ name: 'two-agent', scope: 'project' }], default_agent: 'codexcrew' })
    await waitFor(() => expect(result.current.agents).toHaveLength(1))
  ...
  it('does not clear the roster on a same-project refresh (no flicker)', async () => {
    ...
    mockApi.codexcrewAgents.mockImplementationOnce(() => new Promise(() => {}))
    rerender({ trig: 1 })
  ...
  it('treats an omitted project as its own scope, without refetch churn', async () => {
    ...
    rerender()
    expect(mockApi.codexcrewAgents).toHaveBeenCalledTimes(1)
  })
  it('waits for the one-time sync even when the scope changes while it is in flight', async () => {
    ...
    rerender({ dir: '/repo/service' })
    expect(mockApi.codexcrewAgents).not.toHaveBeenCalled()
  ...
  it('still fetches when the one-time sync fails', async () => {
    ...
    await waitFor(() => expect(mockApi.codexcrewAgents).toHaveBeenCalledWith('chat-1'))
  })
})
