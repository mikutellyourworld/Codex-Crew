...
describe('AboutPanel gateway update check', () => {
  ...
  it('a check that could not run reports the failure, not "up to date"', async () => {
    stubFetch({ check_status: 'failed', update_available: null, error_code: 'feed_unreachable', managed_by: 'codexcrew' })
    mountWeb()
    await pressCheck()
  ...
  it('reports up to date only when a comparison actually completed', async () => {
    stubFetch({ check_status: 'succeeded', update_available: false, error_code: null, managed_by: 'codexcrew' })
    mountWeb()
    await pressCheck()
  ...
  it('the available-version line shows the folded display value, keeping the raw stamp off screen', async () => {
    ...
    stubFetch({
      ...
      remediation: { kind: 'command', message: '', command: 'codexcrew update' },
    })
    mountWeb()
    await pressCheck()
