...
const AGENTS = [{ name: 'codexcrew' }, { name: 'codexcrew-dev' }]
...
describe('FolderConfigModal', () => {
  ...
  it('keeps an uninstalled agent selectable so Save cannot wipe it', async () => {
    ...
    // <option> used to sit.
    expect(await openAgents()).toEqual(['None', 'retired-agent (not installed)', 'codexcrew', 'codexcrew-dev'])
  ...
  it('does not flag an installed agent as uninstalled', async () => {
    const f = folder('f1', { name: 'Payments', default_agent: 'codexcrew-dev' })
    open({ mode: 'edit', folder: f, folders: [f] })
    expect(agentTrigger()).toHaveTextContent('codexcrew-dev')
    ...
    // pass vacuously.
    expect(await openAgents()).toEqual(['None', 'codexcrew', 'codexcrew-dev'])
  ...
  it('clearing the agent back to inherit submits an empty string', async () => {
    ...
    const f = folder('f1', { name: 'Payments', default_agent: 'codexcrew-dev' })
    const { onSubmit } = open({ mode: 'edit', folder: f, folders: [f], globalDefaultAgent: 'codexcrew' })
    // With a global default the top row names it, so the user can see what
    // "inherit" actually resolves to.
    expect(await openAgents()).toEqual(['Inherit (codexcrew)', 'codexcrew', 'codexcrew-dev'])
    fireEvent.keyDown(document, { key: 'Escape' })
    await waitFor(() => expect(screen.queryByRole('option')).toBeNull())
    await pickAgent('Inherit (codexcrew)')
    expect(agentTrigger()).toHaveTextContent('Inherit (codexcrew)')
    fireEvent.click(screen.getByTestId('folder-config-submit'))
  ...
  describe('edit mode', () => {
    const existing = folder('f1', {
      name: 'Payments', project_dir: '/repo/pay', default_agent: 'codexcrew-dev',
    })
