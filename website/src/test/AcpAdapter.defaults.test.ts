// The kiro agent FILE's model (~/.kiro/agents/codexcrew.json → e.g.
...
// CodexCrew agent's own model, the bound kiro agent's pin, the global fallback,
...
const codexcrewConfig = api.codexcrewConfig as unknown as ReturnType<typeof vi.fn>
...
describe('AcpAdapter.resolveModel — delegates to the backend resolver', () => {
  ...
  it('passes the CodexCrew agent name through, not a kiro template name', async () => {
  ...
  it('does NOT re-derive the precedence client-side', async () => {
    ...
    expect(codexcrewConfig).not.toHaveBeenCalled()
  })
...
describe('AcpAdapter.resolveDefaultEffort', () => {
  ...
  it('returns the configured default effort', async () => {
    codexcrewConfig.mockResolvedValue({ agent: { reasoning_effort: 'high' } })
