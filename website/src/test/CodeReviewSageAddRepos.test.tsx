...
describe('pasting a pull request link', () => {
  beforeEach(() => {
    ...
    api.recentRepos.mockResolvedValue({ repos: [], pinned: [], gh_ready: true })
    api.myRepos.mockResolvedValue({ repos: [], pinned: [], gh_ready: true })
    api.repoPrs.mockResolvedValue({ repo: 'mikutellyourworld/Codex-Crew', prs: [], count: 0 })
    api.settings.mockResolvedValue({
      settings: { review: { concurrency: 2 } }, pool: null, reviewer: null,
    })
  })
  ...
  it('sends the pull request URL as-is', async () => {
    ...
    api.pinRepoUrl.mockResolvedValue({
      ok: true,
      repos: [{ owner: 'kirodotdev', repo: 'CodexCrew' }],  // brand-ok: literal repository name
      added: { owner: 'kirodotdev', repo: 'CodexCrew' },  // brand-ok: literal repository name
      pull_request: {
        owner: 'kirodotdev', repo: 'CodexCrew', number: 777,  // brand-ok: literal repository name
        url: 'https://github.com/mikutellyourworld/Codex-Crew/pull/777',
        change_id: 'GH-kirodotdev-CodexCrew-777',
      },
    })
    mount()
    const field = await screen.findByLabelText(/Repository or pull request URL/i)
    await userEvent.type(field, 'https://github.com/mikutellyourworld/Codex-Crew/pull/777')
  ...
  it('opens the pasted pull request instead of leaving you to find it', async () => {
    ...
    api.pullRequestSource?.mockResolvedValue?.({
      provider: 'github', url: 'https://github.com/mikutellyourworld/Codex-Crew/pull/777',
    ...
    expect(screen.getByTestId('probe').textContent)
      .toContain('repo:mikutellyourworld/Codex-Crew')
  })
