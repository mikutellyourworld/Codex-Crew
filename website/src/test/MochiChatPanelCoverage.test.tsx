...
describe('ChatPanel gateway start', () => {
  it('reports the start as under way and re-offers it once the window lapses', async () => {
    ...
    backendOnline = false
    await renderPanel()
    const start = await screen.findByRole('button', { name: 'Start Codex Crew' }, { timeout: 5000 })
    ...
    expect(screen.queryByRole('button', { name: 'Start Codex Crew' })).not.toBeInTheDocument()
    ...
    expect(await screen.findByRole('button', { name: 'Start Codex Crew' })).toBeInTheDocument()
  })
})
...
describe('ChatPanel pointer feedback', () => {
  ...
  it.each([
    ...
    stubGlobal('codexcrew', { isElectron: true, platform })
    history = [turn('assistant', 'Check `src/app/notes.md` please.')]
    await renderPanel()
