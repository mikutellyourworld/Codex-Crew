...
describe('requestPayloadPath', () => {
  it('builds the canonical queue file path from dataDir and requestId', () => {
    const p = requestPayloadPath('/home/user/.codex-crew/apps/design-tweak', 'req-abc')
    expect(p).toBe('/home/user/.codex-crew/apps/design-tweak/queue/req-abc.json')
  })
...
describe('fetchHealth', () => {
  it('GETs /health and returns the parsed response', async () => {
    const payload = { status: 'ok', dataDir: '/home/.codex-crew/apps/design-tweak' }
    const fetchMock = vi.fn(async () => jsonResponse(payload))
    vi.stubGlobal('fetch', fetchMock)
