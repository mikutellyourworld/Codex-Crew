...
describe('TerminalCompletion', () => {
  it('opens a menu for `cd ../` and lists directories', async () => {
    const fetchMock = mockComplete([{ name: 'CodexCrew', dir: true }, { name: 'notes', dir: true }])
    ...
    expect(screen.getByText('CodexCrew/')).toBeInTheDocument()
  ...
  it('emphasises a mid-name match and replaces the typed fragment', async () => {
    vi.stubGlobal('fetch', mockComplete(
      [{ name: 'CodexCrew-terminal-completion', dir: true, at: 9 }], 'termi', '/work',
    ))
    ...
    expect(row).toHaveTextContent('CodexCrew-terminal-completion/')
    ...
    expect(sent).toEqual(['\x7f'.repeat(5) + 'CodexCrew-terminal-completion/'])
  })
