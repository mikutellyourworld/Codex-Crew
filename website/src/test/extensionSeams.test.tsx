...
describe('composition root — stock extensions.ts is empty', () => {
  // extensions.ts is core-OWNED: it imports the `virtual:codexcrew-edition`
  // module (resolved by editionExtensionPlugin to an inert stub in the stock
  // build, or the edition's own composition root when CODEXCREW_EDITION_DIR is
  ...
  it('has an empty body (edition import + export {} + comments only)', () => {
    ...
    const code = src
      ...
      .trim()
    expect(code).toBe("import 'virtual:codexcrew-edition'\n\nexport {}")
  })
