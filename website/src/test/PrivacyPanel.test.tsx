...
const RECEIPT_DISCLOSURE = "Official app slug · per-app anonymous token (not linkable across apps) · fresh/update flag · Codex Crew version"
...
const CONTROL_COMMANDS = [
  'codexcrew telemetry status',
  'codexcrew telemetry disable',
  'export CODEXCREW_TELEMETRY_DISABLED=1',
  "$env:CODEXCREW_TELEMETRY_DISABLED = '1'",
  'set CODEXCREW_TELEMETRY_DISABLED=1',
] as const

const SHELL_COMMANDS = [
  ['macOS / Linux', 'export CODEXCREW_TELEMETRY_DISABLED=1'],
  ['Windows PowerShell', "$env:CODEXCREW_TELEMETRY_DISABLED = '1'"],
  ['Windows Command Prompt', 'set CODEXCREW_TELEMETRY_DISABLED=1'],
] as const
...
describe('PrivacyPanel', () => {
  ...
  it('disables the switch and explains when the env var pins telemetry off', async () => {
    beaconStatus.mockResolvedValue({
      ...
      enabled: false,
      would_send: false,
      reason: 'opted out via CODEXCREW_TELEMETRY_DISABLED',
      env_override: true,
    })
    renderWithProviders(<PrivacyPanel />)
    ...
    expect(
      await screen.findByLabelText(/Environment variable CODEXCREW_TELEMETRY_DISABLED/),
    ).toBeInTheDocument()
  ...
  it('warns when the stored flag is on but nothing is actually being sent', async () => {
    beaconStatus.mockResolvedValue({
      ...ON,
      would_send: false,
      reason: 'non-default CODEXCREW_HOME (dev home / pod / preview)',
