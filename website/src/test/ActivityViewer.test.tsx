...
describe('ActivityViewer', () => {
  ...
  it('Resources hides links present in the Changes tab (sources) and keeps the rest', () => {
    ...
    const prUrl = 'https://github.com/mikutellyourworld/Codex-Crew/pull/42'
    render(
      <Provider store={store}>
        <QueryClientProvider client={qc}>
          <ActivityViewer
            ...
            sources={[{ url: prUrl, provider: 'github', number: 42, repo: 'CodexCrew' }]}
            navLinks={[
              ...
              { url: 'https://git.example.com/packages/CodexCrew', type: 'other', label: 'CodexCrew repo', msgIdx: 0 },
            ]}
          />
        </QueryClientProvider>
      </Provider>,
    ...
    expect(screen.getByText('CodexCrew repo')).toBeInTheDocument()
...
describe('ActivityViewer — Artifacts tab', () => {
  ...
  it('keeps the subagent card header on one line in a narrow rail', () => {
      ...
      <Provider store={store}>
        <QueryClientProvider client={qc}>
          <ActivityViewer
            ...
            subagents={{
              s1: {
                id: 's1', task: 'READ-ONLY RESEARCH', agent: 'codexcrew', status: 'tool',
    ...
    // the Cancel button.
    const chip = screen.getByText('codexcrew')
