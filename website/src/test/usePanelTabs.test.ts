...
describe('usePanelTabs', () => {
  ...
  it('keeps a folder tab while files open in separate de-duplicated tabs', () => {
    const { result } = renderHook(() => usePanelTabs())
    act(() => result.current.openFolder('/Users/me/workspace/CodexCrew', 'chat-a'))
    expect(result.current.activeTab).toMatchObject({
      id: 'folder:/Users/me/workspace/CodexCrew', kind: 'folder', title: 'CodexCrew', slot: 'chat-a',
    })
    // Re-opening the same directory focuses the existing tab, not a duplicate.
    act(() => result.current.openFolder('/Users/me/workspace/CodexCrew'))
    expect(result.current.tabs).toHaveLength(1)

    // A file opens beside the tree instead of replacing it, and becomes active.
    act(() => result.current.openFile('/Users/me/workspace/CodexCrew/README.md', 'v1'))
    expect(result.current.tabs.map(t => t.id)).toEqual([
      'folder:/Users/me/workspace/CodexCrew',
      'file:/Users/me/workspace/CodexCrew/README.md',
    ])
    expect(result.current.activeId).toBe('file:/Users/me/workspace/CodexCrew/README.md')

    // Opening the same path again activates and refreshes the existing tab.
    act(() => result.current.openFile('/Users/me/workspace/CodexCrew/README.md', 'v2'))
