/**
 * Crew roster (CodexCrewAgentsPage) — card grid, compact table, editor dialog.
 *
...
const mockApi = vi.hoisted(() => ({
  codexcrewAgents: vi.fn(),
  agentsInstalled: vi.fn(),
  workspaces: vi.fn(),
  codexcrewConfig: vi.fn(),
...
const AGENTS_RESPONSE = { agents: [DEFAULT_CREW, OTHER_CREW], default_agent: 'codexcrew' }
const WORKSPACES_RESPONSE = {
  workspaces: [{ name: 'default' }, { name: 'core-ws' }, { name: 'oncall' }],
}
const INSTALLED_RESPONSE = [{ name: 'codexcrew' }, { name: 'oncall-agent' }]
const CONFIG_RESPONSE = { memory_stores: { default: {}, 'core-mem': {}, 'oncall-mem': {} } }

beforeEach(() => {
  vi.clearAllMocks()
  mockApi.codexcrewAgents.mockResolvedValue(AGENTS_RESPONSE)
  mockApi.agentsInstalled.mockResolvedValue(INSTALLED_RESPONSE)
  mockApi.workspaces.mockResolvedValue(WORKSPACES_RESPONSE)
  mockApi.codexcrewConfig.mockResolvedValue(CONFIG_RESPONSE)
  mockApi.agentResolvedModel.mockResolvedValue({ model: '', pinned: false, kiro_agent: 'codexcrew' })
...
async function renderRoster(expectCards = 2) {
  ...
  await waitFor(() => expect(mockApi.codexcrewConfig).toHaveBeenCalled())
  return rendered
}
...
describe('crew roster — cards', () => {
  it('renders one card per crew and badges only the default one', async () => {
    ...
    const defaultCard = crewCard('codexcrew')
  ...
  it('flags only the store that a second crew also points at', async () => {
    ...
    mockApi.codexcrewAgents.mockResolvedValue({
      agents: [
    ...
    for (const name of ['codexcrew', 'oncall']) {
...
describe('crew roster — filtering', () => {
  it('narrows the visible cards', async () => {
    ...
    expect(screen.queryByRole('button', { name: 'Edit agent codexcrew' })).not.toBeInTheDocument()
  })
  ...
  it('shows the zero-crew empty state when there are no crews at all', async () => {
    mockApi.codexcrewAgents.mockResolvedValue({ agents: [], default_agent: '' })
    renderPage()
    await waitFor(() =>
  ...
  it('does not flash the empty state while an invalidateQueries-driven refetch is in flight', async () => {
    ...
    // roster query key is stable (`['codexcrew-agents']`) and refetches are
    ...
    act(() => { qc.invalidateQueries({ queryKey: ['codexcrew-agents'] }) })

    // The prior roster must remain on screen throughout the pending refetch —
    // no empty state, cards intact.
    await waitFor(() => expect(mockApi.codexcrewAgents).toHaveBeenCalledTimes(2))
...
describe('crew roster — description', () => {
  ...
  it('does not put an empty title on a crew with no description', async () => {
    ...
    const filler = within(crewCard('codexcrew')).getByText('Used for all new chats')
  ...
  it('falls back to the same text in the card and the row', async () => {
    ...
    const defaultRow = screen.getByRole('button', { name: 'Edit agent codexcrew' }).closest('tr')!
...
describe('crew editor — create', () => {
  ...
  it('refuses a crew with no Agent Template chosen, without calling the api', async () => {
    ...
    // The template used to be pre-filled with 'codexcrew', so a crew created
...
describe('crew editor — stale writes', () => {
  it('does not close the panel when a write for a DIFFERENT crew lands', async () => {
    ...
    const sheetB = await openEditor('codexcrew')
    resolveA({ ok: true })

    // B survives, and A's outcome is not reported against it.
    await waitFor(() => expect(mockApi.codexcrewAgents).toHaveBeenCalled())
    expect(screen.getByRole('dialog', { name: 'Edit agent codexcrew' })).toBeInTheDocument()
  ...
  it('does not report a stale write\u2019s error against the crew now open', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('dialog', { name: 'Edit agent codexcrew' })).toBeInTheDocument())
  ...
  it('does not navigate away when a stale chat request completes', async () => {
    ...
    const replacement = await openEditor('codexcrew')
...
describe('crew roster — default crew bar', () => {
  it('names the current default and switches it on pick, with no Save step', async () => {
    ...
    expect(picker).toHaveTextContent('codexcrew')
  ...
  it('is hidden when there is nothing to choose between', async () => {
    mockApi.codexcrewAgents.mockResolvedValue({ agents: [DEFAULT_CREW], default_agent: 'codexcrew' })
...
describe('crew editor — delete', () => {
  ...
  it('hides the danger zone on the default crew', async () => {
    await renderRoster()
    const sheet = await openEditor('codexcrew')
...
describe('CrewAvatar', () => {
  ...
  it('renders a decorative img backed by a local data URI', async () => {
    const { img, src } = renderAvatar('codexcrew')
  ...
  it('is deterministic per seed and distinct across seeds', async () => {
    ...
    const other = renderAvatar('codexcrew')
