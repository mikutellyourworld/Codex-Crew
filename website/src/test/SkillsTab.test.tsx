...
describe('SkillsTab', () => {
  it('renders a row per skill with its loaded_by_agents pill', async () => {
    mockApi.skills.mockResolvedValue([
      {
        key: 'foo', name: 'foo', description: 'a foo skill', source: 'codexcrew',
        loaded_by_agents: ['codexcrew', 'codexcrew-lite'],
      },
    ])
    renderWithQuery()
  ...
  it('shows singular form when exactly one agent loads the skill', async () => {
    mockApi.skills.mockResolvedValue([
      { key: 'solo', name: 'solo', description: 'lone', source: 'codexcrew', loaded_by_agents: ['only-one'] },
    ])
    renderWithQuery()
  ...
  it('selected row has no border (regression: selection should not draw a border)', async () => {
    mockApi.skills.mockResolvedValue([
      { key: 'a', name: 'a', description: 'first', source: 'codexcrew', loaded_by_agents: [] },
      { key: 'b', name: 'b', description: 'second', source: 'codexcrew', loaded_by_agents: [] },
    ])
    renderWithQuery()
  ...
  it('omits the pill when loaded_by_agents is empty', async () => {
    mockApi.skills.mockResolvedValue([
      { key: 'unloaded', name: 'unloaded', description: 'no one', source: 'codexcrew', loaded_by_agents: [] },
    ])
    renderWithQuery()
  ...
  it('auto-selects the first skill and renders the directory browser (no modal)', async () => {
    mockApi.skills.mockResolvedValue([
      { key: 'demo', name: 'demo', description: 'demo skill', source: 'codexcrew', loaded_by_agents: [] },
    ])
    renderWithQuery()
  ...
  it('switches the browser when another skill row is clicked', async () => {
    mockApi.skills.mockResolvedValue([
      { key: 'first', name: 'first', description: 'one', source: 'codexcrew', loaded_by_agents: [] },
      { key: 'second', name: 'second', description: 'two', source: 'codexcrew', loaded_by_agents: [] },
    ])
    renderWithQuery()
    ...
    mockApi.skills.mockResolvedValue([
      {
        ...
        description: 'has agents', source: 'codexcrew',
  ...
  it('Delete button confirms and dispatches the deleteSkill mutation', async () => {
    mockApi.skills.mockResolvedValue([
      { key: 'doomed', name: 'doomed', description: 'will go', source: 'codexcrew', loaded_by_agents: [] },
    ])
  ...
  it('Edit button enters edit mode for codexcrew-sourced skills', async () => {
    mockApi.skills.mockResolvedValue([
      { key: 'editable', name: 'editable', description: 'fixme', source: 'codexcrew', loaded_by_agents: [] },
    ])
  ...
  it('preserves edit mode when the edited skill is filtered out (no data loss)', async () => {
    ...
    mockApi.skills.mockResolvedValue([
      { key: 'editable', name: 'editable', description: 'fixme', source: 'codexcrew', loaded_by_agents: [] },
      { key: 'other', name: 'other', description: 'second', source: 'codexcrew', loaded_by_agents: [] },
    ])
    mockApi.skill.mockResolvedValue({
      name: 'editable',
      content: '---\nname: editable\ndescription: fixme\n---\nbody text',
...
describe('SkillsTab create gate and coded refusal', () => {
  ...
  const ONE_SKILL = [
    { key: 'existing', name: 'existing', description: 'here already', source: 'codexcrew', loaded_by_agents: [] },
  ]
