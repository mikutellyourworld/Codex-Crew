...
describe('AboutPanel download states', () => {
  ...
  it('offers a platform-correct manual download once the update is staged', async () => {
    const { setState } = mountWithStates({ downloadUrl: 'https://download.crew.kiro.dev/desktop/nightly/latest/CodexCrew.dmg' })
    ...
    expect(link.getAttribute('href')).toBe(
      'https://download.crew.kiro.dev/desktop/nightly/latest/CodexCrew.dmg',
    )
  ...
  it('renders whatever lane the main process resolved (Linux AppImage)', async () => {
    const { setState } = mountWithStates({ downloadUrl: 'https://download.crew.kiro.dev/desktop/stable/latest/CodexCrew-x86_64.AppImage' })
    setState({ state: 'downloaded', version: '9.9.9' })
    const link = (await screen.findByTestId('update-manual-fallback')).querySelector('a')
    expect(link?.getAttribute('href')).toBe(
      'https://download.crew.kiro.dev/desktop/stable/latest/CodexCrew-x86_64.AppImage',
    )
  })
  ...
  it('keeps an INSTALL failure in the card, labelled honestly, with the escape hatch', async () => {
    const { setState } = mountWithStates({
      downloadUrl: 'https://download.crew.kiro.dev/desktop/nightly/latest/CodexCrew.dmg',
    })
