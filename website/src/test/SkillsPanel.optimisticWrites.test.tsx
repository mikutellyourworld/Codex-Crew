 ...
 * (#6890). The panel PATCHes sibling paths of the shared ['codexcrewConfig']
...
const { patchConfigMock, codexcrewConfigMock } = vi.hoisted(() => ({
...
vi.mock('../api/client', () => ({
  api: { codexcrewConfig: codexcrewConfigMock, patchConfig: patchConfigMock },
}))
...
describe('SkillsPanel — optimistic toggle via the per-path overlay', () => {
  beforeEach(() => {
    ...
    codexcrewConfigMock.mockImplementation(() =>
      Promise.resolve({ skills: { auto_create_from_sessions: false, approval_required: true } }),
    )
  })
  ...
  it('flips the toggle immediately while the PATCH is in flight, then keeps the refetched value', async () => {
    ...
    await waitFor(() => expect(codexcrewConfigMock).toHaveBeenCalledTimes(1))
