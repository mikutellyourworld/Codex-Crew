...
describe('safeSetItem', () => {
  ...
  it('escalates past tier 1 to evict mc-paste-store-v1 when height caches are absent', () => {
    // The real-world failure mode behind the looping CODEXCREW boot reveal: the
  ...
  it('reclaims touched-files lists as part of tier 2', () => {
    window.localStorage.setItem('codexcrew:touched-files:chat-1-100', '["a.ts"]')
    ...
    const spy = vi.spyOn(Storage.prototype, 'setItem').mockImplementation(function (
      ...
      if (this.getItem('codexcrew:touched-files:chat-1-100') !== null
        && !key.startsWith('codexcrew:touched-files:')) {
        throw quotaError()
      }
      real.call(this, key, value)
    })
    ...
    expect(safeSetItem('k', 'v')).toBe(true)
    expect(window.localStorage.getItem('codexcrew:touched-files:chat-1-100')).toBeNull()
    spy.mockRestore()
  })
  ...
  it('preserves the :toolClearedAt watermark while reclaiming touched-files lists', () => {
    ...
    window.localStorage.setItem('codexcrew:touched-files:chat-1-100', '["a.ts"]')
    window.localStorage.setItem('codexcrew:touched-files:chat-1-100:toolClearedAt', '1700000000000')
    ...
    expect(window.localStorage.getItem('codexcrew:touched-files:chat-1-100:toolClearedAt'))
      .toBe('1700000000000')
    spy.mockRestore()
  })
