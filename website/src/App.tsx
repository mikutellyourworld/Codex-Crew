...
import { Rocket, Bell, Code, RefreshCw, Package, Download, Hammer, XCircle, Check, AlertTriangle, CheckCircle, X, AudioWaveform, ChevronUp, MoreHorizontal, ArrowLeftToLine, Compass, LayoutGrid, Fullscreen, Menu, SquareTerminal, Bot, Smartphone, Search as SearchIcon } from 'lucide-react'
...
// `/api/apps` once on mount; right after a `codexcrew update` the gateway is
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-bg/80 backdrop-blur-sm animate-rise">
      <div className="bg-card border border-border rounded-xl p-8 max-w-md w-full mx-4 shadow-xl text-center">
        ...
        <div className="text-lg font-bold text-text-strong mb-2">{i18nT('app.updating_codexcrew')}</div>
...
export default function App() {
  ...
  // `codexcrew token` URL) -- showing the loud pulsing "Offline" pill on top
  ...
  const refreshAppNav = useCallback((attempt = 0) => {
    ...
    api.listApps()
      ...
      .catch(() => {
        ...
        // `codexcrew update`, or the cold apps-dir scan) used to be swallowed
  ...
  // drop — e.g. a `codexcrew update` restart disconnects then reconnects the
  ...
  return (
      ...
      >
        ...
        <div ref={metricsGroupRef} className={`tb-right relative${updateAvailable ? ' tb-has-update' : ''}`}>
          ...
          {(() => {
            ...
            // Primary-backend segment. The account/status popover is Codex-owned
            // in the public build, so no Kiro account or credit data appears here.
            const codexTitle = i18nT('components.codexAccountModal.primary_backend')
            segments.push(
              <button
                key="codex-status"
                className={seg}
                onClick={() => setCodexStatusOpen(true)}
                title={codexTitle}
                aria-label={codexTitle}
              >
                <Code size={12} />
                {!isMobile && (
                  <span className="tb-drop-usage text-[11px] whitespace-nowrap">
              ...
              <div {...(effectiveCollapsed ? { inert: '' } : {})} className={`overflow-hidden transition-all duration-200 ${effectiveCollapsed ? 'max-h-0 opacity-0' : 'max-h-16 opacity-100 mt-1'}`}>
                <div className="flex items-center border-t border-border pl-3 pr-0.5 pt-2.5 pb-0.5 whitespace-nowrap">
                  ...
                  <div className="rail-community-links flex items-center gap-[5px] flex-1 min-w-0 ml-1.5 text-[12px]">
                    <a href="https://github.com/mikutellyourworld/Codex-Crew" target="_blank" rel="noopener noreferrer" title={i18nT('app.star_codexcrew_on_github')} aria-label={i18nT('app.star_codexcrew_on_github')} className="shrink-0 rounded text-muted hover:text-text transition-colors">{i18nT('app.star_us')}</a>
    ...
    <CodexAccountModal
      open={codexStatusOpen}
      onClose={() => setCodexStatusOpen(false)}
    />
    <QuickSearchSurface
      owners={slotOwners}
