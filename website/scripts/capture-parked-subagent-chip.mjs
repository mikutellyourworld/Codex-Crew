...
const LAUNCH_OUTPUT = [
  ...
  `  ${RUNNING_ID} (codexcrew): audit the retry ladder in the uploader`,
  `  ${PARKED_ID} (codexcrew): ${PARKED_TASK}`,
].join('\n')
...
async function main() {
  ...
  await send('subagent_spawn', { slot: SLOT, id: RUNNING_ID, task: 'audit the retry ladder in the uploader', agent: 'codexcrew' })
