...
const SLOTS = [{
  ...
  agent: 'codexcrew', mode: '', modified: Math.floor(Date.now() / 1000),
...
const extra = async (path, route) => {
  ...
  if (path === '/api/config/codexcrew') {
    await json(route, {
      agents: { codexcrew: { kiro_agent: 'codexcrew', workspace: 'default', memory_store: 'default' } },
      default_agent: 'codexcrew',
      agent: { default_agent: 'codexcrew', provider: 'acp', model: 'auto' },
      session: { timeout_secs: 1800, pool_size: 2 },
    })
    return true
  }
  if (path === '/api/agent/config') { await json(route, { name: 'codexcrew', mcpServers: {} }); return true }
  return false
}
