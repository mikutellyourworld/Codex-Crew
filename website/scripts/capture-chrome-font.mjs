...
// CodexCrew user actually sees there.
...
const AGENTS = [
  {
    id: '1713e7d0',
    task: 'You are auditing the GitHub PR history of the repo mikutellyourworld/Codex-Crew for a translation gap.',
    tool: 'gh pr list --repo mikutellyourworld/Codex-Crew --state all --limit 700 --json number,title,state',
    tool_count: 5,
  },
  {
    id: '5c15adde',
...
async function main() {
  ...
  await page.route('**/api/**', async route => {
    ...
    if (path === '/api/agents') {
      return json(route, {
        agents: [{ name: 'default', kiro_agent: 'codexcrew', description: 'Default crew agent' }],
        default_agent: 'default',
      })
    }
    if (path.startsWith('/api/agents/detail/')) return json(route, { name: 'codexcrew', model: 'claude-opus-5', skills: [] })
    ...
    if (path === '/api/spawn') {
      return json(route, {
        agents: AGENTS.map(a => ({ id: a.id, task: a.task, done: false, parent: `dashboard:${SLOT}`, agent: 'codexcrew' })),
      })
    }
  ...
  async function pushWave() {
    ...
    for (const a of AGENTS) {
      await push('subagent_spawn', { slot: SLOT, id: a.id, task: a.task, agent: 'codexcrew' }, 150)
