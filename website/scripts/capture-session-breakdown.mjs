 ...
 * Output: <outDir> || $CODEXCREW_SCRATCH || os.tmpdir()/session-tree-cap/
...
// Output dir: first CLI arg, else $CODEXCREW_SCRATCH, else the OS temp dir —
// never a hardcoded personal path.
const OUT = process.argv[2] || join(process.env.CODEXCREW_SCRATCH || tmpdir(), 'session-tree-cap')
...
function makeTrace(slot, turnBlocks, peakFrac) {
  ...
  return {
    ...
    // The model context Codex Crew did NOT inject (kiro-cli prompt + tool catalog).
