...
const ITEMS = [
  {
    ...
    prompt:
      'In src/codex_crew/dashboard/ws.py, apply the same token-bucket limiter added to the upload handler to the WebSocket upgrade path. Reuse the limiter helper rather than duplicating it, cap concurrent sockets per caller, and add tests for the reject path.',
  ...
  },
  {
    ...
    prompt:
      'Document the upload rate-limiter defaults and their config keys in src/codex_crew/docs/configuration.md, including how to raise them for a trusted deployment.',
