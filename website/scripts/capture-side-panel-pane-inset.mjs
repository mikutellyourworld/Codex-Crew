...
const MCP = {
  servers: {
    'codexcrew-core': { command: 'codexcrew', args: ['mcp'], disabled: false },
    'codexcrew-cron': { command: 'codexcrew', args: ['mcp-cron'], disabled: false },
  },
}
const WORKSPACES = [
  { name: 'default', path: '/home/user/.codex-crew/workspace', active: true, sessions: 3 },
  { name: 'research', path: '/home/user/.codex-crew/workspaces/research', active: false, sessions: 0 },
]
const AGENTS = [
  { name: 'codexcrew', description: 'Autonomous personal AI agent', source: 'codexcrew', model: 'auto', mcp_servers: ['codexcrew-core'], filename: 'codexcrew.json', skills: [] },
...
await stubDashboardApi(page, {
  ...
  extra: (path, route) => {
    ...
    if (path === '/api/config/default-agent') {
      return json(route, { default_agent: 'codexcrew' }), true
    }
    if (path === '/api/models') {
