...
const OUT = process.argv[2] || (process.env.CODEXCREW_SCRATCH || '/tmp') + '/older-sessions-hint'
...
const slot = (key, title, folder_id, ageHours, extra = {}) => ({
  key, title, messages: 4, running: false, agent: 'codexcrew',
...
const history = [
  { key: 'h-1', title: 'Bedrock payments research', modified: (NOW - 3 * 86400_000) / 1000, agent: 'codexcrew', messages: 12 },
  { key: 'h-2', title: 'Theme pack debugging', modified: (NOW - 5 * 86400_000) / 1000, agent: 'codexcrew', messages: 7 },
]
