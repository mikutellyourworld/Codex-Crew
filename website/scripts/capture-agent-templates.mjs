...
const INSTALLED = [
  {
    ...
    mcp_servers: ['codexcrew-core', 'playwright'],
...
const DETAIL = {
  codexcrew: {
    ...
    mcpServers: { 'codexcrew-core': {}, playwright: { args: ['--include-tools', 'browser_navigate,browser_click'] } },
    toolsSettings: {
      execute_bash: {
        deniedCommands: [
          'rm\\s+-rf\\s+/',
...
function templatesApi(installed) {
  return async (path, route) => {
    ...
    if (path === '/api/config/default-agent') return json(route, { default_agent: 'codexcrew' }), true
