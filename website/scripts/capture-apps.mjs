...
const registryApps = [
  A('code-review-sage', 'Code Review Sage', 'codexcrew', 'Self-evolving deep code reviewer for GitHub PRs with a prioritized focus report.', ['code-review', 'github'], { featured: 1, version: '3.2.0' }),
  A('oncall-radar', 'Oncall Radar', 'codexcrew', 'Oncall operations dashboard — track tickets, pipelines, risks, MCMs, shift handoffs and maintain runbooks.', ['oncall', 'tickets'], { featured: 2, version: '2.1.0' }),
  A('auto-research', 'Research Lab', 'codexcrew', 'Autonomous research campaigns powered by autonudge — pre-flight validation, live findings, stagnation detection.', ['research', 'autonudge'], { featured: 3, installed: true, enabled: true, version: '1.4.0' }),
  A('issue-radar', 'Issue Radar', 'codexcrew', 'An issue triage assistant that remembers. Browse, filter, and triage GitHub issues with AI-suggested labels.', ['github', 'issue-triage'], { _registry: REG }),
  ...
  A('workflows', 'Workflows', 'codexcrew', 'Author, run, and watch dynamic workflows — agent-authored Python scripts that orchestrate CodexCrew agents.', ['workflows', 'automation'], { installed: true, enabled: true }),
  A('code-reviewer', 'Code Reviewer', 'codexcrew', 'Review local git changes with a CRUX-like diff viewer, inline comments, and an IntelliJ-style Git panel.', ['code-review', 'git'], { _registry: REG }),
...
const I = (name, displayName, origin, over = {}) => {
  ...
  return {
    ...
    manifest: { name, version: '1.0.0', displayName, description: src.description || '', author: src.author || 'codexcrew', tags: src.tags || [] },
    ...over,
  }
}
const installedApps = [
  ...
  I('aws-control', 'AWS Control', 'builtin', {
    enabled: false,
    manifest: {
      name: 'aws-control', version: '1.0.0', displayName: 'AWS Control', author: 'codexcrew',
  ...
  I('channels', 'Channels', 'builtin', {
    enabled: false,
    manifest: {
      name: 'channels', version: '1.0.0', displayName: 'Channels', author: 'codexcrew',
...
await page.route('**/api/**', async route => {
  ...
  if (path === '/api/dashboard/branding') return json(route, { bot_name: 'Codex Crew', avatar: '' })
