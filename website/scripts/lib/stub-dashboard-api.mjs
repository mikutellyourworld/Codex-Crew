...
/**
 * The `/api/config/codexcrew` body, matching `CodexCrewCfg` in
 * `website/src/pages/overview/CodexCrewCfgTab.tsx`.
 *
 ...
 */
export const CODEXCREW_CONFIG_FIXTURE = {
  agents: {
    codexcrew: { kiro_agent: 'codexcrew', workspace: 'default', memory_store: 'default' },
  },
  default_agent: 'codexcrew',
  workspaces: { default: { dir: '~/.codex-crew/workspace' } },
  ...
  agent: {
    default_agent: 'codexcrew', provider: 'acp', model: 'auto',
  ...
  session: { timeout_secs: 1800, pool_size: 2, pool_agent: 'codexcrew', pool_ttl_secs: 600 },
  memory: { embedding_provider: 'local' },
  auto_update: true,
}

/** The `/api/agent/config` body — the per-agent MCP view. */
export const AGENT_CONFIG_FIXTURE = { name: 'codexcrew', mcpServers: {} }

/**
 * The `/api/knowledge/stats` body, matching `get_stats` in
 * `src/codex_crew/dashboard/handlers/knowledge.py`.
 *
 * Named ahead of the catch-all for the same reason as CODEXCREW_CONFIG_FIXTURE,
...
export async function stubDashboardApi(page, opts = {}) {
  const {
    ...
    // "Codex Crew"`). It must stay TWO WORDS: the nav brand row accents the last
  ...
  await page.route('**/api/**', async route => {
    ...
    if (path === '/api/agents' || path === '/api/chat/agents') {
      return json(route, [{ name: 'codexcrew', source: 'builtin' }, { name: 'oncall', source: 'aim' }])
    }
    ...
    // CODEXCREW_CONFIG_FIXTURE for why).
    if (path === '/api/config/codexcrew') return json(route, CODEXCREW_CONFIG_FIXTURE)
    ...
    // for /api/config/codexcrew was silent AND fatal, and a harness author with
