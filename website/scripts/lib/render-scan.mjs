...
export const ALWAYS_LATIN = [
  // glossary.json `dnt`, plus the spaced display name `Codex Crew`, which is NOT a
  ...
  'AWS', 'Discord', 'Docker', 'Git', 'GitHub', 'GitLab', 'JSON', 'Kiro', 'Codex Crew',
  'CodexCrew',
...
export function dntViolations(text, terms) {
  ...
  for (const term of terms) {
    ...
    while (m) {
      ...
      // `PLAYWRIGHT_MCP_EXTENSION_TOKEN`, `CODEXCREW_OWNER_ID`. Both are env var
...
export function scanDocument(opts) {
  ...
  if (mode === 'real' && opts.dnt && opts.dnt.length) {
    ...
    for (let n = walker.nextNode(); n; n = walker.nextNode()) {
      ...
      // this, the agent named `codexcrew` reads as a mangled `CodexCrew` on every
