  ...
  await context.route('**/api/**', async route => {
    ...
    if (path === '/api/chat/slots') {
      ...
      if (route.request().method() === 'POST') {
        return json(route, { key: 'fixture-chat', title: 'New Session…', agent: 'codexcrew' })
      }
      return json(route, [])
    }
    ...
    if (path === '/api/dashboard/branding') return json(route, { bot_name: 'Codex Crew', avatar: '/logo.png' })
    ...
    if (path === '/api/agents/installed') return json(route, [{ name: 'codexcrew' }])
    if (path === '/api/workspaces') return json(route, { workspaces: [{ name: 'default' }] })
    if (path === '/api/chat/agents') return json(route, [{ name: 'codexcrew', source: 'builtin' }])
