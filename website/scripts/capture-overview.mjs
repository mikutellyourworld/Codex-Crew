...
const PROJECT = '/home/codexcrew/workspace'
...
fixedApi.set('/api/dashboard/branding', { bot_name: 'Codex Crew', avatar: '/logo.png' })
...
await page.route('**/api/**', async route => {
  ...
  if (path === '/api/codexcrew-config' || path === '/api/config/codexcrew') return json(route, {
    // Complete CodexCrewCfg shape (CodexCrewCfgTab enumerates every section).
    agents: { codexcrew: { provider: 'kiroacp', model: 'auto', approval_mode: 'reads' } },
    ...
    agent: { default_agent: 'codexcrew', provider: 'kiroacp', model: 'auto', approval_mode: 'reads', sandbox: 'auto', subagent_max_turns: 60, max_subagents: 8, subagent_auto_max: 4, conductor_skill: false, tool_search: true, max_channels: 5, max_channel_agents: 2, enforce_denied_commands: 'on' },
    session: { timeout_secs: 900, pool_size: 2, pool_agent: 'codexcrew', pool_ttl_secs: 900 },
    memory: { embedding_provider: 'local' },
    auto_update: true,
  })
  if (path === '/api/agent-config' || path === '/api/agent/config') return json(route, {
    name: 'codexcrew', provider: 'kiroacp', tools: ['fs_read', 'fs_write', 'execute_bash'], mcpServers: { 'playwright-mcp': {} },
  })
  // Provider usage — codexcrew provider raw payload (normalized client-side).
