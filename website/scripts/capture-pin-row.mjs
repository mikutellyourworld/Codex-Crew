...
async function main() {
  ...
  await page.route('**/api/**', async route => {
    ...
    if (path === '/api/agents') {
      return json(route, {
        agents: [{ name: AGENT, kiro_agent: 'codexcrew', description: 'Default crew agent' }],
        default_agent: AGENT,
      })
    }
    if (path.startsWith('/api/agents/detail/')) return json(route, { name: 'codexcrew', model: MODEL, skills: [] })
