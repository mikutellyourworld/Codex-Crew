...
const SLOTS = [
  {
    ...
    agent: 'codexcrew', modified: now, last_ts: iso(now), folder_id: '',
    last_message: 'Reading ui/dialog.tsx.',
  },
  {
    key: 'chat-b', title: 'App Store 内建行改为来自 catalog', running: false, messages: 12,
    agent: 'codexcrew', modified: now - 420, last_ts: iso(now - 420), folder_id: '',
  ...
  if (path.startsWith('/api/instances')) {
    await json(route, {
      instances: [
        ...
        { id: 'lab', name: 'homelab', url: 'http://lab:5486', active: false, unread: 2 },
      ],
      active: 'local',
    })
    return true
