...
const CAUSES = [
  ...
  },
  {
    ...
    pr: 42,
    error: 'the reviewer cannot run: the ACP runtime (codex_crew.acp.runtime) is not'
  ...
  },
  {
    ...
    error: "Agent spec 'code-review-sage-reviewer' is not installed: kiro-cli found no"
      ...
      + ' until it is restored — repair with `codexcrew setup --agent-only --clean`,'
      + ' then restart the gateway.',
    expect: 'repair with `codexcrew setup --agent-only --clean`',
    verbatim: true,
  },
  {
