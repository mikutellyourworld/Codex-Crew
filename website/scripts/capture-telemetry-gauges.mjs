 ...
 *
 * The PR adds codexcrew.process.* observable gauges; the API reports them as
...
const PROJECT = '/home/user/.codex-crew/workspace'
...
const telemetry = {
  ...
  window_days: 14,
  shard_count: 42,
  metrics_dir: '/home/user/.codex-crew/metrics',
  startup: {
  ...
  cost: null,
  other: [
    { name: 'codexcrew.mcp.backend.acquire.duration', kind: 'histogram', ...stat({ count: 1134 }) },
    { name: 'codexcrew.mcp.warm_pool.acquire', kind: 'counter', total: 1134,
      by_attr: { 'result=hit': 1031, 'result=miss': 103 } },
    // The new process gauges — point-in-time readings, `latest` not `total`.
    { name: 'codexcrew.process.threads.python', kind: 'gauge', latest: 49, by_attr: {} },
    { name: 'codexcrew.process.threads.os', kind: 'gauge', latest: 96, by_attr: {} },
    { name: 'codexcrew.process.open_fds', kind: 'gauge', latest: 144, by_attr: {} },
    ...
    { name: 'codexcrew.process.memory.rss_bytes', kind: 'gauge', latest: 4402341888,
      by_attr: { 'pid=5346': 4402341888, 'pid=9121': 287309824 } },
    { name: 'codexcrew.process.memory.peak_rss_bytes', kind: 'gauge', latest: 4617089024, by_attr: {} },
    { name: 'codexcrew.process.cpu.seconds', kind: 'counter', total: 5123.4, by_attr: {} },
  ],
}
...
async function main() {
  ...
  const gauges = page.getByText('codexcrew.process.threads.os')
  await gauges.waitFor({ timeout: 10000 })
  const card = page.locator('text=codexcrew.process.threads.os').locator(
