 ...
 * chains (src/codex_crew/dashboard/handlers) collapse into one row — which is
...
const TREE_PATHS = [
  ...
  'src/codex_crew/dashboard/handlers/files.py',
  'src/codex_crew/dashboard/handlers/sessions.py',
  'src/codex_crew/dashboard/server.py',
...
async function main() {
   ...
   *   codexcrew:comment-hint-dismissed    the markdown comment-tip banner would
  ...
  async function load(tabs, activeId, opts = {}) {
    await page.addInitScript(([slot, project, tabsJson, railOpen, panelW, railW, diffSplit]) => {
      ...
      localStorage.setItem('codexcrew:comment-hint-dismissed', '1')
  ...
  ], [await assertTreeRows('10-files-tab-all-files', ['AGENTS.md', 'README.md', 'package.json', 'docs', 'website', 'codex_crew'])])
