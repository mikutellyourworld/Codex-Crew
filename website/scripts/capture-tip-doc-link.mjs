...
const TIPS = {
  'subagent-parallelism': {
    ...
    body: '`agent.max_subagents` caps concurrent subagents. **0 = auto-size** to your machine\'s memory and CPU; pin a number with `codexcrew config set agent.max_subagents 4`.',
    why: '',
    doc: '',
...
async function main() {
  ...
  async function load(theme, tip) {
    ...
    await page.addInitScript(slot => {
      ...
      localStorage.removeItem('codexcrew.tips.lastShownAt')
    }, SLOT)
