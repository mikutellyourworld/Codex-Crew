...
const MEMBERS = [
  { name: 'radar', slug: 'radar', bound: true, slot_key: 'member-radar', running: true, kiro_agent: 'codexcrew-autofix', workspace: 'autofix', memory_store: 'default', model: '', last_active_ts: 1000, last_message: 'Six new issues: four covered by open PRs.' },
  { name: 'scout', slug: 'scout', bound: false, slot_key: '', running: false, kiro_agent: 'codexcrew-research', workspace: 'default', memory_store: 'scout-own', model: 'claude-opus-5' },
  { name: 'fixer', slug: 'fixer', bound: true, slot_key: 'member-fixer', running: false, kiro_agent: 'codexcrew', workspace: 'default', memory_store: 'default', model: '', last_active_ts: 900, last_message: 'Two PRs opened for the queue.' },
  { name: 'scribe', slug: 'scribe', bound: false, slot_key: '', running: false, kiro_agent: 'codexcrew-lite', workspace: 'docs', memory_store: 'default', model: '' },
]
...
async function newPage(theme, viewport = { width: 1280, height: 820 }) {
  ...
  await page.route(u => new URL(u).pathname.startsWith('/api/'), route => {
    ...
    if (path === '/api/members') {
      return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ members: MEMBERS, default_agent: 'codexcrew' }) })
    }
    if (path === '/api/crons') {
    ...
    if (path === '/api/agents') {
      return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ agents: [], default_agent: 'codexcrew' }) })
    }
...
// 02 — thread open + drawer (dark)
{
  ...
  check('02-thread drawer config', /codexcrew-autofix/.test(drawer || ''), 'agent template shown')
