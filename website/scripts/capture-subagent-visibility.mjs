...
const SPAWN_OUTPUT = [
  ...
  ...AGENTS.map(a => `  ${a.id} (codexcrew): ${a.task}`),
  '',
...
async function main() {
  ...
  await page.route('**/api/**', async route => {
    ...
    if (path === '/api/spawn') {
      return json(route, {
        agents: AGENTS.map(a => ({
          id: a.id, task: a.task, done: false, parent: `dashboard:${SLOT}`, agent: 'codexcrew',
        })),
      })
    }
  ...
  async function startAgent(i) {
    ...
    await push('subagent_spawn', { slot: SLOT, id: AGENTS[i].id, task: AGENTS[i].task, agent: 'codexcrew' }, 150)
