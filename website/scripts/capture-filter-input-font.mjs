...
const AGENTS = [
  { name: 'default', kiro_agent: 'codexcrew', description: 'Default crew agent' },
  { name: 'reviewer', kiro_agent: 'codexcrew', description: 'Reads a diff and reports findings' },
  { name: 'researcher', kiro_agent: 'codexcrew', description: 'Gathers evidence before a decision' },
  { name: 'translator', kiro_agent: 'codexcrew', description: 'Keeps the locale catalogs in parity' },
]
...
async function main() {
  ...
  await page.route('**/api/**', async route => {
    ...
    if (path.startsWith('/api/agents/detail/')) return json(route, { name: 'codexcrew', model: SHELF_MODEL, skills: [] })
