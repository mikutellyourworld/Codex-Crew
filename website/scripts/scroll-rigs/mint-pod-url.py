...
Usage: python3 mint-pod-url.py <pod-worktree-name> <session-key> [codexcrew-bin]

Writes the URL to $CODEXCREW_SCRATCH/kc-pod-url.txt (or $TMPDIR, else the
...
POD = sys.argv[1]
KEY = sys.argv[2]
KC = sys.argv[3] if len(sys.argv) > 3 else "codexcrew"
...
if not kc_bin:
    raise SystemExit(f"codexcrew binary not found: {KC!r}")
...
# A directory this user owns. $CODEXCREW_SCRATCH is reclaimed with the session;
# the shared /tmp is deliberately NOT a fallback (see the write below).
OUT_DIR = os.environ.get("CODEXCREW_SCRATCH") or os.environ.get("TMPDIR") or "."
