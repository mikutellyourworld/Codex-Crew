 ...
 *     on the wire. Only the two diverged rows gain "· answered by codexcrew"; the
...
const DIVERGED = { 'chat-mochi': 'codexcrew', 'chat-research': 'codexcrew' }

const baseSlots = [
  mkSlot('chat-mochi', 'Pet chat — feeding schedule', 'mochi'),
  mkSlot('chat-review', 'Review PR 6104', 'codexcrew'),
  mkSlot('chat-research', 'Compare vector stores', 'research-bot'),
  mkSlot('chat-triage', 'Triage needs-triage queue', 'codexcrew'),
  mkSlot('chat-notes', 'Draft weekly notes', 'writer'),
  mkSlot('chat-sweep', 'Dependency sweep', 'codexcrew'),
]
...
async function main() {
  ...
  try {
    ...
    // ── Frame 2: with the field — exactly the diverged rows are marked ─────
    {
      ...
      if (!text || !text.includes('codexcrew')) {
    ...
    // ── Frame 3: minimum width — the marker yields, the timestamp survives ──
    {
      ...
      const crowded = afterSlots.map(s =>
        DIVERGED[s.key] ? { ...s, effective_agent: 'codexcrew-research-orchestrator' } : s,
      )
