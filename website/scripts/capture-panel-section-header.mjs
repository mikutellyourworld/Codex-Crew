...
const SESSION_ARTIFACTS = [
  ...
  { slug: 'link-unfurl-contract-md', name: 'link-unfurl-contract.md', kind: 'markdown', pinned: false, source_path: '/Users/diwm/.codex-crew/workspace/scratch/link-unfurl-contract.md' },
]
const LIBRARY_ONLY = [
...
async function main() {
  ...
  async function load(theme) {
    ...
    await page.addInitScript(({ t, slot }) => {
      ...
      localStorage.setItem('codexcrew:touched-files:' + slot, JSON.stringify([
        { path: '/Users/diwm/.codex-crew/workspace/scratch/link-unfurl-contract.md', ts: Date.now(), lastWrite: Date.now(), source: 'tool' },
        { path: '/Users/diwm/.codex-crew/workspace/scratch/notes.ts', ts: Date.now(), source: 'tool' },
      ]))
