...
await page.route('**/api/**', async route => {
  ...
  if (/\/api\/(slack|discord|telegram|webex|wecom)\/config$/.test(p)) {
    return json(route, {
      ...
      command: 'codexcrew', allow_all_users: false, ws_url: '',
    })
  }
  ...
  if (p === '/api/dashboard/branding') return json(route, { bot_name: 'Codex Crew', avatar: '' })
  ...
  if (p === '/api/codexcrew-config' || p === '/api/config/codexcrew') return json(route, {
    ...
    agent: { default_agent: 'codexcrew', provider: 'kiroacp', model: 'auto' },
