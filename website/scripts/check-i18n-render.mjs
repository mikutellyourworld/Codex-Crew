 ...
 * fix. The shared stub's defaults (`user: 'owner'`, agents `codexcrew`/`oncall`) are
...
const FIXTURE_OVERRIDES = async (language, path, route) => {
  ...
  // `CodexCrewCfgTab` does `Object.entries(cfg.agents)` and `VoicePanel`'s provider
 ...
 * `../../../../src/codex_crew/connections/registry.json` — and an allowlist of the
...
function buildBaseBundle(sha) {
  ...
  try {
    ...
    // `../../../../src/codex_crew/connections/registry.json` (#1229/#1287), so a
    // `website`-only archive produced a base tree whose vite build died with
    // `Could not resolve "../../../../src/codex_crew/connections/registry.json"`
