# CodexCrewWebsite — Agent Guidelines
...
React + TypeScript + Vite SPA for the Codex Crew dashboard. Built assets go to
`dist/` and are staged into `../src/codex_crew/static/dist/` so the gateway serves
them.
...
- **Edition composition is fail-closed:** it needs `CODEXCREW_EDITION_DIR` **and**
  `CODEXCREW_ALLOW_EDITION=1`. Never set the latter in a release or publish job. A
