/**
 * The AppArmor grant a DIRECTLY LAUNCHED Codex Crew needs, and why the app cannot
 * just apply it itself.
 *
 * `codexcrew service install` installs a NAMED AppArmor profile and lets systemd
 ...
 * @param {string} [opts.cliBin] - absolute path to the bundled `codexcrew` CLI.
 *   REQUIRED in practice: this persona installed no CLI, so there is no
 *   `codexcrew` on their PATH and a bare command would be `command not found`.
...
function describeSandboxProfileNeed({ platform, env, readSysctl, cliBin }) {
  ...
  // printing `codexcrew` would hand the affected user `command not found` and
  // leave them with only the sandbox opt-out.
  const cli = String(cliBin || "").trim() || "codexcrew";
 ...
 * is printed for a human to paste, so `Codex-Crew-$(...).AppImage` must not have
