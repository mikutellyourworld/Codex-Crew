...
// all of them keeps `CODEXCREW_DEBUG=1` the single answer to "turn profiling on".
...
function nativeLoggingSwitches(logPath, env = process.env) {
  ...
  // CODEXCREW_DEBUG opt-in as the rest of the profiling surface and is OFF on a
...
function rotateNativeLog(logPath, { fs, log = () => {} } = {}) {
  ...
  try {
    ...
    // `src/codex_crew/atomic_write.py`.
  ...
  if (typeof startCrashReporter === "function") {
    try {
      startCrashReporter({
        ...
        // Codex Crew does not phone home (website/src/rum.ts is a no-op in the
