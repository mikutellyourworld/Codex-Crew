...
test("local gateway: no token, URL unchanged from the pre-switching shape", () => {
  assert.strictEqual(
    mochiPageUrl("http://localhost:5486", "pet.html"),
    "http://localhost:5486/app-windows/mochi/pet.html",
  );
  // An explicitly empty token is the `self` case, not "append nothing weird".
  assert.strictEqual(
    mochiPageUrl("http://localhost:5486", "panel.html", ""),
    "http://localhost:5486/app-windows/mochi/panel.html",
  );
});
...
test("a trailing slash on the origin does not produce a double slash", () => {
  assert.strictEqual(
    mochiPageUrl("http://localhost:5486/", "avatar.html"),
    "http://localhost:5486/app-windows/mochi/avatar.html",
  );
});
