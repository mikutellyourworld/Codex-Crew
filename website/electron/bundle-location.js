...
function describeLocation(location, { bundleWritable = true } = {}) {
  ...
  if (location === "translocated") {
    return "macOS is running Codex Crew from a temporary read-only copy (App Translocation), "
      + "so it cannot install its own updates.";
  }
  return "Codex Crew is running from a read-only disk image, so it cannot install its own updates.";
}
...
function describeLinuxInstall(kind, { imageWritable = true, packageFormat = "" } = {}) {
  ...
  if (kind === "package") {
    return "Codex Crew cannot tell which package format this install came from, so it "
      + "cannot fetch the right update. Update it through your package manager.";
  }
  return "Codex Crew cannot write to the folder holding its AppImage, so it cannot replace "
