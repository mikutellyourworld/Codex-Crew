...
const CODEXCREW_EXE_NAMES = new Set(["codexcrew", "codexcrew-backend"]);
...
function executableSelector(tokens) {
  ...
  for (let index = 1; ; index++) {
    ...
    if (CODEXCREW_EXE_NAMES.has(name) || PYTHON_EXE_RE.test(name)) return { name, next: index };
...
/**
 * Match only a Codex Crew executable, or a Python process whose first execution
 * selector invokes the `codex_crew` module or a Codex Crew script. Later process
...
function isKirocrewCommand(commandLine, { trustedExecutablePaths = [] } = {}) {
  ...
  if (CODEXCREW_EXE_NAMES.has(selector.name)) return true;
  ...
  while (index < tokens.length) {
    const token = tokens[index];
    if (token === "-m") return tokens[index + 1] === "codex_crew";
  ...
  return /[\\/]/.test(script || "") && CODEXCREW_EXE_NAMES.has(executableName(script));
}
...
function postShutdown({
  ...
  if (!secretList.length) {
    try {
      const s = fsMod.readFileSync(pathMod.join(codexcrewHome, ".local_secret"), "utf8");
      secretList = [s];
    } catch { /* none readable */ }
  }
...
async function stopGatewayGracefully(
  ...
  await new Promise((resolve) => {
    ...
    postShutdownFn({ backendUrl, codexcrewHome, secrets, httpMod, fsMod, pathMod }).then((ok) => {
...
/**
 * Force-stop whatever CodexCrew process is LISTENing on `port`, then VERIFY the
 ...
 * died), plus `survivors` (the CodexCrew PIDs we tried to kill that are still
 * holding the port) and `foreignHolder` (a non-CodexCrew process still owns it).
...
async function forceStopPort(
  ...
  // Only signal PIDs we can positively identify as CodexCrew — never SIGKILL an
  ...
  for (const pid of owners) {
    ...
    if (ours) {
      ...
      if (await isServiceManaged(pid, getPpid)) {
        serviceHolder = true;
        log(`force-stop: SKIP pid=${pid} — service-managed CodexCrew gateway (${cmd.slice(0, 80)})`);
        continue;
      }
      try {
      ...
      log(`force-stop: SKIP pid=${pid} — not a CodexCrew process (${cmd.slice(0, 80)})`);
    }
  }
  ...
  if (survivors.length) {
    ...
    log(`force-stop: port :${port} held by a non-CodexCrew process we won't kill — respawn would fail to bind`);
  }
 ...
 * gateway from a remote one reached through a port-forward: `ssh -L 5486:...`
 * makes a gateway on another machine answer on `localhost:5486` with the same
 ...
 * CodexCrew process is a reason NOT to evict.
 *   "codexcrew" — a local LISTEN owner matching isKirocrewCommand. Only this
 ...
 * @returns {Promise<"codexcrew"|"foreign"|"none"|"unknown">}
 */
async function classifyPortOwner(
  ...
  for (const pid of pids) {
    ...
    if (ours) {
      if (await isServiceManaged(pid, getPpid)) {
        log(`port-owner: :${port} held by SERVICE-MANAGED CodexCrew pid=${pid} (${cmd.slice(0, 80)}) — reuse, never evict`);
        return "service";
      }
      log(`port-owner: :${port} held by local CodexCrew pid=${pid} (${cmd.slice(0, 80)})`);
      return "codexcrew";
    }
    log(`port-owner: :${port} held by NON-CodexCrew pid=${pid} (${cmd.slice(0, 80)})`);
  }
  return "foreign";
}
