# CodexCrew Desktop (Electron)

Desktop shell for the Codex Crew web dashboard on macOS, Linux, and Windows
(Windows is in preview — see the build note below). It automatically starts
`codexcrew gateway` and connects to `localhost:5486`.
...
2. Launch `codexcrew gateway` when needed
3. Show a loading screen while the backend boots. An owned Windows gateway has
   no hard wall-clock deadline: the shell keeps polling while its child is alive,
   so Defender scans or a large persisted home cannot turn a slow successful boot
   into a false “can't reach” dialog. A child that exits still fails immediately
   with its launch-log cause.
...
sudo rm -rf /Applications/CodexCrew.app
sudo cp -R "$APP_DIR/CodexCrew.app" /Applications/CodexCrew.app
```

Launch via Spotlight (Cmd+Space → "CodexCrew"), Dock, or `open /Applications/CodexCrew.app`.
...
- The result is an assisted (non-one-click, per-user) NSIS installer,
  `CodexCrew Setup <version>.exe` (nightly builds:
  `CodexCrew Nightly Setup <version>.exe`), in `website/electron/dist/`.
- The pinned electron-builder NSIS template is patched during `npm install` to
  expose Codex Crew's staged-payload publish hook. On a normal same-volume
...
- The native welcome/finish sidebar and the header used on intermediate pages
  carry the Codex Crew logo and ghost artwork. The standard NSIS controls and
...
launchctl stop dev.codexcrew.gateway
launchctl start dev.codexcrew.gateway
```
...
launchctl unload ~/Library/LaunchAgents/dev.codexcrew.gateway.plist 2>/dev/null
rm -f ~/Library/LaunchAgents/dev.codexcrew.gateway.plist
```
...
1. An SSH tunnel forwarding the remote gateway port to localhost:
   ```bash
   ssh -L 5486:localhost:5486 YOUR_HOST.example.com
   ```
   Or use a macOS LaunchAgent (see `../../docs/guides/assets/`).

2. `codexcrew` installed on the remote host. The default auto-discovers across common
...
1. The modal shows which port it's configuring (e.g. "Remote host for :5486")
2. Enter your remote host's hostname or SSH config alias (e.g. `myhost.example.com` or `clouddesk`)
3. Leave the binary path at the default unless you installed codexcrew somewhere
   unusual. The default tries, in order:
   - `~/.toolbox/bin/codexcrew` (toolbox install — recommended)
   - `~/.local/bin/codexcrew` (install.sh / source install)
   - `~/.codexcrew-app/.venv/bin/codexcrew` (one-liner installer venv)
...
5. Optionally set a **Remote PATH** if codexcrew needs additional directories
   (default: `~/.toolbox/bin:/usr/bin:/bin`)
6. Click Save. Leave hostname empty to clear (use local token for that port).

**Multi-instance example:**
- Tab 1 on `:5486` — local gateway, no remote host needed
- Tab 2 on `:7778` — SSH tunnel to another host, remote host configured

The app will SSH into the configured remote host and run `codexcrew token` on
each launch to get a fresh JWT — no manual paste required.

### Token flow (per tab)

```
1. Try local ~/.codex-crew/.local_secret → /api/token/local on the tab's port
   (with a temporary ~/.codexcrew read fallback during one-time migration)
2. If remote host configured for this port:
   SSH: export PATH=<remotePath> CODEXCREW_PORT=<port>; <bin> token
...
`~/Library/Application Support/CodexCrew/config.json`:
...
| "codexcrew binary not found in any of …" | Install Codex Crew through a [supported install path](../../docs/guides/install.md#install-paths), or set a custom path |
...
| Token fetched but 403 | Gateway may need restart — `ssh host systemctl --user restart codexcrew` |
