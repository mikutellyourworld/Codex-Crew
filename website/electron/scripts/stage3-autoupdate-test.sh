...
#    falls back to a PATH `codexcrew`. This script therefore stubs an EMPTY
...
#    Have `codexcrew` on PATH (toolbox install) so a gateway can start. We
#    isolate with a temp CODEXCREW_HOME + the BETA flavor (port 7788) so this
...
#
# Env overrides: PORT (8799), OUT (/tmp/codexcrew-ota), NODE (node).
# ============================================================================
...
# find-bin.js falls back to a PATH `codexcrew`, so an empty dir is sufficient.
...
if [ ! -d backend-dist ]; then
  ...
  command -v codexcrew >/dev/null || echo "    WARNING: no codexcrew on PATH — the app may not reach initAutoUpdate"
fi
...
# install with:
#
#   Code signature at URL .../CodexCrew.app/ did not pass validation:
...
OTA_IDENTITY="CodexCrew OTA Test Signing"
...
Run the installed app pointed at the local feed (new terminal, or here):
  ...
  CODEXCREW_UPDATE_FEED="http://127.0.0.1:$PORT/feed" \\
  CODEXCREW_FLAVOR=beta \\
  CODEXCREW_HOME="$OUT/home" \\
