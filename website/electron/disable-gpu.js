...
// up. The backend Gateway is fine and still listening on :5486; only the
...
// argv from a second launch, so `CodexCrew.exe --disable-gpu` does nothing once
...
// keep hardware acceleration. It is opt-in via `CODEXCREW_DISABLE_GPU`
// (mirroring the existing `CODEXCREW_*` env conventions: CODEXCREW_HOME,
// CODEXCREW_PORT, CODEXCREW_DEBUG) or the `--disable-gpu` command-line flag.
//
...
/** Truthy env values, matching how the rest of the shell reads CODEXCREW_* flags. */
 ...
 *   1. `CODEXCREW_DISABLE_GPU` env set to a truthy value (1/true/yes/on).
...
function shouldDisableGpu({ env = process.env, argv = process.argv } = {}) {
  const raw = env && env.CODEXCREW_DISABLE_GPU;
