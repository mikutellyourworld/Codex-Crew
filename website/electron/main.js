...
const store = new Store({
  defaults: {
    remoteHost: "",
    codexcrewBinPath: DEFAULT_REMOTE_BIN,
    remoteHosts: {},
    sshTimeoutMs: 20000,
    windowState: null,
    globalHotkey: null,
...
const CODEXCREW_HOME = resolveHome();

function resolvePort() {
  const raw = process.env.CODEXCREW_PORT;
  if (raw) {
    const parsed = parseInt(raw, 10);
    if (isNaN(parsed) || parsed < 1 || parsed > 65535) {
      console.warn('Invalid CODEXCREW_PORT="' + raw + '", falling back to 5486');
      return 5486;
    }
    return parsed;
  }
  ...
  const configuredPort = findConfiguredDashboardPort(fs, path, [CODEXCREW_HOME]);
  ...
  console.debug("No usable dashboard.url port in the data home, falling back to 5486");
  return 5486;
}
...
if (process.platform === "win32") {
  const appUserModelId = identityFamily(app.getVersion()) === "nightly"
    ? "io.github.mikutellyourworld.codexcrew.nightly"
    : "io.github.mikutellyourworld.codexcrew";
...
function scanCrashArtifacts() {
  ...
  try {
    crashScan = collectCrashReports({
      ...
      // `codexcrew-desktop` (and the nightly channel overrides it again), while
      // `getName()` is `Codex Crew`. Off darwin a minidump is our only crash
...
async function offerRelocationIfUnupdatable() {
  ...
  try {
    ({ response } = await dialog.showMessageBox({
      type: "warning",
      title: "Move Codex Crew to Applications?",
  ...
  try {
    await dialog.showMessageBox({
      type: "error",
      message: "Could not move Codex Crew automatically.",
      detail: "Drag Codex Crew into your Applications folder, then reopen it from there.",
      buttons: ["OK"],
    });
  } catch {
