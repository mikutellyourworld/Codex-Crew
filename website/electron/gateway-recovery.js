...
//   "reused-local"   — adopted a local same-family Codex Crew process.
 ...
 * local LISTEN owner ("codexcrew"/"service"); anything less (tunnel, no visible
 ...
 *                               ("codexcrew" | "service" | "other" | "none" | …).
 * @returns {"reused-service" | "reused-local" | "none"}
 */
function classifyAdoptedGateway({ reason, localOwner }) {
  const local = reason === "same-family" && (localOwner === "codexcrew" || localOwner === "service");
...
function unrecoverableGatewayDialog({
  ...
  return {
    title: probeFailed
      ? `Codex Crew: can't verify what's using port ${port}`
      : held
        ? `Codex Crew: port ${port} is in use`
        : `Codex Crew: backend stuck on port ${port}`,
    message: probeFailed
      ? `Codex Crew could not verify which process owns port ${port}, so it did not `
        + `risk terminating an unrelated process or starting a second gateway. `
        + `Quit and reopen Codex Crew to try again. If the port is still blocked, `
        + `restart your computer.`
      : held
        ? `Another process is holding port ${port}, so Codex Crew can't reconnect to `
        ...
        : `The Codex Crew backend is wedged and cannot be stopped. It is in an `
    ...
    portConflict: false,
    primaryAction: "quit",
    primaryLabel: isPrimaryWindow ? "Quit Codex Crew" : "Close",
 ...
 *                                      codexcrew at all must never relaunch.
