...
// Migrate legacy single-host config (remoteHost + codexcrewBinPath) to the
...
function migrateRemoteHostConfig(store, port) {
  ...
  if (legacy && Object.keys(store.get("remoteHosts") || {}).length === 0) {
    const bin = store.get("codexcrewBinPath") || DEFAULT_REMOTE_BIN;
    store.set("remoteHosts", { [port]: { host: legacy, binPath: bin } });
    store.delete("remoteHost");
    store.delete("codexcrewBinPath");
    return true;
  }
  return false;
}
