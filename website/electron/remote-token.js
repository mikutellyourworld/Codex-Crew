...
// dev desktop over SSH to fetch a CodexCrew dashboard token.
//
// Split out from main.js so the shell-construction logic can be unit-tested
// without spinning up Electron.

const DEFAULT_REMOTE_BIN = "~/.local/bin/codexcrew";
...
const REMOTE_BIN_CANDIDATES = [
  "~/.toolbox/bin/codexcrew",           // toolbox install (recommended per wiki)
...
function buildCandidateTokenCommand(candidates, { port, remotePath } = {}) {
  ...
  const portExport = port ? ` CODEXCREW_PORT=${port}` : "";
  ...
  return [
    ...
    '  fi;',
    'done;',
    `echo "codexcrew binary not found in any of: ${candidates.join(", ")}" >&2;`,
    'exit 127',
  ].join(" ");
}
...
// Extract the JWT from a `codexcrew token` URL. The command prints:
//   http://localhost:5486?token=eyJ...
