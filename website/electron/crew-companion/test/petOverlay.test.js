...
test("opens one overlay per display, covering each display's full bounds", () => {
  ...
  try {
    ...
    overlay.setOverlayTarget("http://localhost:5486", "");
...
test("the page URL mirrors the file layout, and omits an empty credential", () => {
  ...
  try {
    ...
    assert.strictEqual(
      pageUrl.companionPageUrl("http://localhost:5486", "pet.html"),
      "http://localhost:5486/app-windows/crew-companion/pet.html",
    );
    assert.strictEqual(
      pageUrl.companionPageUrl("http://localhost:5486/", "pet.html", "abc"),
      "http://localhost:5486/app-windows/crew-companion/pet.html?token=abc",
    );
  } finally {
    stub.restore();
  }
...
test("an inconclusive probe leaves the windows exactly as they are", async () => {
  ...
  try {
    ...
    overlay.setOverlayTarget("http://localhost:5486", "cred");
    overlay.openPetWindow();
...
test("no credential is unknown, not disabled", async () => {
  ...
  try {
    ...
    index.initCrewCompanion({
      backendUrl: "http://localhost:5486",
...
test("showing an overlay re-asserts the host's Dock presence on macOS", () => {
  ...
  try {
    ...
    // opening the companion makes Codex Crew's Dock icon disappear.
    assert.ok(
