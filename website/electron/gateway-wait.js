...
// Windows packaged startup has no honest fixed ceiling: Defender scans, a large
// persisted home, and edition startup hooks can all keep a healthy child alive
// past any guessed deadline. Keep polling while the child lives; its exit/error
// still wins immediately through `getFailure`, and closing the window cancels
// the wait.
//
// ponytail: child liveness is the only cross-platform startup-progress signal.
// Replace this unbounded wait with milestone-stall detection if the backend later
// exposes durable startup milestones.
const WINDOWS_LOCAL_GATEWAY_WAIT_MS = Number.POSITIVE_INFINITY;
...
function describeGatewayFailure(failure) {
  ...
  if (failure.disabled) {
    if (failure.remoteHost) {
      ...
      return `Nothing is answering at ${target} and Codex Crew is set not to `
        ...
        + "with CODEXCREW_PORT set to a port that has no remote host configured, "
        + "then choose Start Local Gateway when prompted.";
    }
    return `No gateway is answering on port ${failure.port}, and Codex Crew is set `
  ...
  if (failure.signal === "SIGKILL") {
    return "The gateway was killed on launch (SIGKILL). On macOS this usually "
      ...
      + "xattr -cr <path to CodexCrew.app>";
  }
  if (failure.signal) {
 ...
 * something already holds it (a wedged or other CodexCrew gateway). This is a
