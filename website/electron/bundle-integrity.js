 ...
 * @param {string} backendRoot  the `…/backend-dist/codexcrew-backend` directory
...
function describeIncompleteBundle(missing) {
  ...
  return `Codex Crew's bundled Python runtime is still being installed — ${detail} `
 ...
 * `aiohttp` or `codex_crew.foo`) is never excused as "still installing". And only
...
function shouldReclassifyAsInstalling({
  ...
  // a PATH `codexcrew` failing on a stdlib import is a broken environment, not an
