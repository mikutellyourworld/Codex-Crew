...
describe("electron-builder files list", () => {
  ...
  it("lists the baked EXTERNALLY-MANAGED marker so a build that places it packs it into app.asar", () => {
    // build-desktop.sh copies CODEXCREW_MANAGED_INSTALL_MARKER here; without this
    ...
    assert.match(script, /CODEXCREW_MANAGED_INSTALL_MARKER/);
...
describe("macOS bundle naming", () => {
  ...
  it("keeps CFBundleName aligned with productName for Electron helpers", () => {
    assert.equal(pkg.build.productName, "CodexCrew");
    assert.equal(
  ...
  it("uses CFBundleDisplayName for spaced stable and nightly names", () => {
    assert.equal(extendInfo.CFBundleDisplayName, "Codex Crew");
    assert.match(
      buildScript,
...
describe("first-download installer design contract", () => {
  ...
  it("ships the Windows startup caches generated after the platform prune", () => {
    ...
    assert.match(
      buildScript,
      /precompile_windows\.py" \\\r?\n\s*--root "\$out" --module codex_crew\.cli_server/
    );
  });
  ...
  it("bundles the voice runtime, downloads models on demand, and gates supported targets", () => {
    ...
    assert.match(buildScript, /from codex_crew\.stt\.engine import probe/);
    assert.match(buildScript, /from codex_crew\.transcribe import _packaged_ffmpeg_version_probe/);
    ...
    assert.match(windowsBuilder, /"codexcrew\[voice-aws\] @ \$root_uri"/);
    assert.match(windowsBuilder, /"codexcrew\[voice\] @ \$root_uri"/);
    assert.doesNotMatch(windowsBuilder, /"\$ROOT\[voice(?:-aws)?\]"/);
    assert.doesNotMatch(windowsBuilder, /if\s+!\s+[\s\S]*?codexcrew\[voice\]/);
  });
  ...
  it("uses the Codex Crew terminal mark across release surfaces", () => {
    ...
    const siteLogo = normalize(
      fs.readFileSync(path.join(REPO_ROOT, "site", "public", "codexcrew-logo.svg"), "utf8")
    );
    ...
    for (const source of [loading, siteLogo, dmgSource, sidebarSource, headerSource]) {
      assert.ok(source.includes("#7c3aed"));
      assert.ok(source.includes("#c9ff38"));
      assert.ok(!source.includes(String.fromCodePoint(0x1F47B)));
    }
    for (const source of [siteLogo, dmgSource, sidebarSource, headerSource]) {
      assert.ok(source.indexOf("#4a5256") >= 0 || source.indexOf("#3f474b") >= 0);
      assert.ok(source.lastIndexOf("#7c3aed") > source.indexOf("#4a5256") ||
        source.lastIndexOf("#7c3aed") > source.indexOf("#3f474b"));
    }
  });
...
describe("uninstall data preservation contract", () => {
  ...
  it("keeps the data home out of the Windows uninstaller's reach", () => {
    ...
    // Electron userData dir on uninstall, and the CodexCrew home under
    // ~/.codex-crew is user data that survives an uninstall by design.
    assert.notEqual(
  ...
  it("uses an assisted installer so nightly installs beside stable", () => {
    ...
    // app-builder-lib only uses productFilename ("CodexCrew" / "CodexCrew
  ...
  it("gives nightly its own Linux package identity so it installs beside stable", () => {
    ...
    assert.equal(electronPkg.build.deb.packageName, "codexcrew");
    assert.equal(electronPkg.build.rpm.packageName, "codexcrew");
    assert.equal(electronPkg.build.linux.executableName, "codexcrew-desktop");
    assert.equal(electronPkg.desktopName, "codexcrew-desktop.desktop");
  ...
  it("reclaims the updater cache the generated uninstaller cannot reach", () => {
    ...
    // ~/.codex-crew. The electron-updater cache lives under $LOCALAPPDATA and so
  ...
  it("keeps the uninstaller away from the Codex Crew data home", () => {
  ...
  it("gives nightly its own per-user state so an uninstall cannot cross channels", () => {
    ...
    assert.ok(
      buildScript.includes("-c.extraMetadata.name=codexcrew-desktop-nightly"),
    ...
    assert.equal(electronPkg.name, "codexcrew-desktop");
  });
  ...
  it("gives nightly its own Windows appId so a channel update cannot orphan the other's shortcuts", () => {
    ...
    // io.github.mikutellyourworld.codexcrew.nightly for a nightly stamp). Leaving the PACKAGED
    ...
    assert.ok(
      buildScript.includes("-c.win.appId=io.github.mikutellyourworld.codexcrew.nightly"),
    ...
    assert.ok(
      !buildScript.includes("-c.appId=io.github.mikutellyourworld.codexcrew.nightly"),
    ...
    assert.ok(
      main.includes("io.github.mikutellyourworld.codexcrew.nightly"),
    ...
    assert.equal(electronPkg.build.appId, "io.github.mikutellyourworld.codexcrew");
  });
});
