...
test("an explicit falsey value reads as off, not as merely-set", () => {
  for (const raw of ["0", "false", "no", "off", ""]) {
    assert.strictEqual(profilingEnabled({ CODEXCREW_DEBUG: raw }), false, `${raw} should be off`);
  }
  for (const raw of ["1", "true", "YES", " on "]) {
    assert.strictEqual(profilingEnabled({ CODEXCREW_DEBUG: raw }), true, `${raw} should be on`);
  }
});
