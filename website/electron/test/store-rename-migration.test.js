...
// rename ("codexcrew-electron-mac" -> "codexcrew-desktop" / "-nightly").
//
...
// %APPDATA%\codexcrew-electron-mac\config.json still held
//
//     "updateChannel": "stable",  "windowState": {...},  "themeAccent": "#8e48ff"
//
// while the freshly created codexcrew-desktop-nightly\config.json had
...
const DEFAULTS = {
  remoteHost: "",
  codexcrewBinPath: "~/.local/bin/codexcrew",
  remoteHosts: {},
  sshTimeoutMs: 20000,
  windowState: null,
  globalHotkey: null,
...
test("seeds every orphaned setting into a store that does not exist yet", () => {
  ...
  const legacy = {
    ...
    remoteHosts: { 5486: { host: "box", binPath: "/usr/bin/codexcrew" } },
    sshTimeoutMs: 30000,
...
test("carries the PRE-remoteHosts single-host keys so the chained migration still runs", () => {
  ...
  // codexcrewBinPath) into the per-port remoteHosts map, and it reads those keys from
  ...
  assert.strictEqual(
    seedRenamedStore(userData, {
      readLegacy: () => ({ remoteHost: "box.example", codexcrewBinPath: "/opt/bin/codexcrew" }),
    }),
    true
  );
  const store = openStore(userData);
  assert.strictEqual(store.get("remoteHost"), "box.example");
  assert.strictEqual(store.get("codexcrewBinPath"), "/opt/bin/codexcrew");

  // ...and the chained migration can now do its job.
  const { migrateRemoteHostConfig } = require("../host-config");
  assert.strictEqual(migrateRemoteHostConfig(store, 5486), true);
  assert.deepStrictEqual(store.get("remoteHosts"), {
    5486: { host: "box.example", binPath: "/opt/bin/codexcrew" },
  });
});
...
test("NEVER touches a destination that already exists", () => {
  ...
  assert.strictEqual(
    seedRenamedStore(userData, {
      readLegacy: () => ({ remoteHosts: { 5486: { host: "old-box" } }, updateChannel: "insider" }),
    }),
    false,
...
test("a CORRUPTED legacy store is final and loud, never retried", () => {
  ...
  const userData = path.join(parent, "codexcrew-desktop-nightly");
  fs.mkdirSync(userData, { recursive: true });
...
test("locates the legacy store beside the CURRENT one, on every platform", () => {
  ...
  const cases = [
    ["win32", "C:\\Users\\jane\\AppData\\Roaming\\codexcrew-desktop"],
    ["darwin", "/Users/jane/Library/Application Support/codexcrew-desktop"],
    ["linux", "/home/jane/.config/codexcrew-desktop-nightly"],
  ];
  for (const [platform, userData] of cases) {
    ...
    assert.ok(!file.includes("codexcrew-desktop"), `${platform}: must not point at the CURRENT store`);
...
test("the legacy directory name is the pre-rename npm package name", () => {
  ...
  assert.strictEqual(LEGACY_STORE_NAME, "codexcrew-electron-mac");
});
...
test("seeds Mochi's machine store across the rename, and the store reads it back", () => {
  ...
  assert.strictEqual(store.get("mochi.petInstance"), "remote-5486");
...
test("locates the legacy Mochi store beside the legacy config.json", () => {
  const file = legacyStoreFile(
    "/Users/jane/Library/Application Support/codexcrew-desktop",
    "mochi-machine.json"
  );
