...
// Helper: temp CODEXCREW_HOME containing a .local_secret file.
...
test("postShutdown returns true on 200 with correct secret", async () => {
  ...
  try {
    const ok = await postShutdown({ backendUrl: `http://127.0.0.1:${port}`, codexcrewHome: home });
...
test("postShutdown returns false when no secret file exists", async () => {
  ...
  const ok = await postShutdown({ backendUrl: "http://127.0.0.1:1", codexcrewHome: home });
  assert.strictEqual(ok, false);
...
test("stopGatewayGracefully: happy path — endpoint exits process, no signal needed", async () => {
  ...
  try {
    await stopGatewayGracefully(proc, {
      backendUrl: `http://127.0.0.1:${port}`, codexcrewHome: home, timeoutMs: 10000,
    });
...
test("stopGatewayGracefully: SIGKILL fallback when SIGTERM ignored", { skip: process.platform === "win32" ? "no real signals on Windows: kill(SIGTERM) is TerminateProcess, which cannot be ignored" : false }, async () => {
  ...
  try {
    await stopGatewayGracefully(proc, {
      backendUrl: `http://127.0.0.1:${port}`, codexcrewHome: home, timeoutMs: 800,
    });
...
test("stopGatewayGracefully: no-op on already-dead process", async () => {
  ...
  await stopGatewayGracefully(proc, { backendUrl: "http://127.0.0.1:1", codexcrewHome: "/nope", timeoutMs: 500 });
  assert.ok(true);
});
...
function fakeDeps({ listenSeq, command = "python -m codex_crew gateway", onKill = () => {} }) {
  let i = 0;
  const killed = [];
  return {
    killed,
...
test("forceStopPort: never signals a non-CodexCrew owner", async () => {
...
test("forceStopPort: foreign owner that vanishes during verify reports freed", async () => {
  // A non-CodexCrew owner we skip, but the port frees on its own before we finish
...
test("forceStopPort: freed reflects real port state even after killing our target", async () => {
  ...
  const deps = {
    ...
    getCommand: async () => "python -m codex_crew gateway",
    kill: () => {},
    sleep: async () => {},
    verifyTimeoutMs: 1000,
    pollIntervalMs: 250,
...
test("forceStopPort: awaits an asynchronous Windows kill before verifying", async () => {
  ...
  const r = await forceStopPort(7788, {
    ...
    getCommand: async () => "C:\\bundle\\codexcrew.exe gateway",
    kill: async () => { killFinished = true; },
    sleep: async () => {},
    isKirocrew: (command) => command.includes("codexcrew.exe"),
  });
  assert.strictEqual(r.killed, 1);
  assert.strictEqual(r.freed, true);
});
...
// outcome except a positively identified local CodexCrew process must be
// treated as "not ours" by callers.

test("classifyPortOwner: local CodexCrew gateway is ours", async () => {
  const owner = await classifyPortOwner(5486, {
    getListenPids: async () => [4242],
    getCommand: async () => "python -m codex_crew gateway --port 5486",
  });
  assert.strictEqual(owner, "codexcrew");
});
...
test("classifyPortOwner: an app-spawned gateway (real ppid) stays 'codexcrew'", async () => {
  const owner = await classifyPortOwner(5486, {
    getListenPids: async () => [4242],
    getCommand: async () => "python -m codex_crew gateway --port 5486",
    getPpid: async () => "  3310\n",
...
test("forceStopPort: never SIGKILLs a service-managed gateway", async () => {
  const killed = [];
  const res = await forceStopPort(5486, {
    getListenPids: async () => [4242],
    getCommand: async () => "python -m codex_crew gateway --port 5486",
    getPpid: async () => "1",
...
test("classifyPortOwner: an ssh -L forward is foreign, not ours", async () => {
  ...
  const owner = await classifyPortOwner(5486, {
    ...
    getCommand: async () => "ssh -NL 5486:localhost:5486 dev-dsk-example.amazon.com",
  });
...
test("classifyPortOwner: ours wins when a mixed set holds the port", async () => {
  const owner = await classifyPortOwner(5486, {
    ...
    getCommand: async (pid) => (pid === 4242 ? "codexcrew gateway" : "ssh -NL 5486:localhost:5486 host"),
  });
  assert.strictEqual(owner, "codexcrew");
});

test("classifyPortOwner and forceStopPort share one CodexCrew matcher", async () => {
  // Drift between the two would let one mis-target a stranger's process.
  assert.ok(isKirocrewCommand("python -m codex_crew gateway"));
  assert.ok(isKirocrewCommand("/Applications/CodexCrew.app/.../codexcrew"));
  assert.ok(!isKirocrewCommand("ssh -NL 5486:localhost:5486 host"));
  assert.ok(!isKirocrewCommand("ssh -NL 5486:localhost:5486 codexcrew"));
  // The matcher keys on the executable/module TOKEN, not a path substring:
  // an unrelated process merely living under a `codexcrew` home dir is foreign.
  assert.ok(!isKirocrewCommand("C:\\Users\\codexcrew\\OtherApp\\server.exe --port 5486"));
  assert.ok(!isKirocrewCommand("/home/codexcrew/some-other-server --port 5486"));
  assert.ok(!isKirocrewCommand("C:\\Users\\codexcrew\\python.exe -m http.server 5486"));
  assert.ok(!isKirocrewCommand("python app.py C:\\tmp\\codexcrew"));
  assert.ok(!isKirocrewCommand("python app.py -m codex_crew"));
  ...
  assert.ok(isKirocrewCommand("/Users/Jane Doe/Apps/CodexCrew.app/Contents/Resources/bin/codexcrew gateway --port 5486"));
  assert.ok(isKirocrewCommand("/Users/Jane Doe/venv/bin/python -m codex_crew gateway"));
  assert.ok(!isKirocrewCommand("/tmp/evil --spoof /usr/local/bin/codexcrew"));
  assert.ok(!isKirocrewCommand("/tmp/evil /usr/local/bin/codexcrew"));
  assert.ok(!isKirocrewCommand("/tmp/dir with space/evil codexcrew --port 5486"));
  // Windows identity is path-bound: a matching basename at any other location
  // remains foreign and can never authorize taskkill.
  const trustedCli = "C:\\Program Files\\CodexCrew\\codexcrew.exe";
  const trustedBackend = "C:\\Program Files\\CodexCrew\\codexcrew-backend.exe";
  const trustedPython = "C:\\Program Files\\CodexCrew\\python.exe";
  ...
  assert.ok(isKirocrewCommand(`"${trustedPython}" -m codex_crew gateway`, {
    trustedExecutablePaths: [trustedPython],
  }));
  assert.ok(!isKirocrewCommand("C:\\Temp\\codexcrew.exe gateway", {
    trustedExecutablePaths: [trustedCli],
  }));
});
