...
const DASHBOARD_ORIGIN = "http://localhost:5486";
...
describe("frame-load-log URL redaction", () => {
  ...
  it("marks a non-token query without dropping the fact there was one", () => {
    assert.equal(safeUrl("http://localhost:5486/?foo=1"), "http://localhost:5486/?<query>");
  });

  it("passes through a URL with no query untouched", () => {
    assert.equal(safeUrl("http://localhost:5486/"), "http://localhost:5486/");
  });
});
...
describe("frame-load-log formatting", () => {
  it("distinguishes a crew pane from the dashboard itself", () => {
    ...
    assert.match(
      formatFrameNavigate({ url: "http://localhost:5486/", httpResponseCode: 200, isMainFrame: true }),
      /main/,
    );
  });
  ...
  it("stays silent for a same-document navigation, which carries no load info", () => {
    assert.equal(
      formatFrameStartNavigation({ url: "http://localhost:5486/chat", isInPlace: true }),
      "",
...
describe("frame-load-log attachment", () => {
  ...
  it("logs a started navigation and skips the SPA's in-place ones", () => {
    ...
    contents.emit("did-start-navigation", {}, "http://localhost:5486/chat", true, true);
...
describe("frame-load-log console trust boundary", () => {
  ...
  it("reduces the loaded URL to the origin Chromium reports, not a prefix of it", () => {
    assert.equal(normalizeTrustedOrigin("http://localhost:5486/?token=secret"), DASHBOARD_ORIGIN);
    ...
    assert.equal(
      isTrustedFrameMessage({ parent: null, origin: "http://localhost:54860" }, DASHBOARD_ORIGIN),
      false,
    );
  });
  ...
  it("disables the journal entirely when no trusted origin was configured", () => {
    ...
    assert.equal(lines.length, 1);
    assert.match(lines[0], /untrusted frame http:\/\/localhost:5486/);
  });
  ...
  it("does not budget the dashboard's own top-frame navigations", () => {
    ...
    for (let i = 0; i < UNTRUSTED_LOG_BUDGET + 50; i += 1) {
      contents.emit("did-frame-navigate", {}, `http://localhost:5486/r${i}`, 200, "OK", true);
    }
