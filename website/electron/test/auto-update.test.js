...
test("CONTRACT: absolute artifact urls pass through newUrlFromBase unchanged (pointer/bytes split)", () => {
  ...
  const absolute = "https://download.crew.kiro.dev/desktop/nightly/0.1.0-nightly.20260728t112233/CodexCrew-arm64.dmg";
...
function makeDeps(opts = {}) {
  const {
    ...
    resourcesPath = "/Applications/Codex Crew.app/Contents/Resources",
    bundleWritable = true,
...
test("externally-managed getInfo carries the marker metadata and kills the switcher", () => {
  const { deps } = makeDeps({
    ...
    externallyManaged: { managedBy: "internal-registry", updateCommand: "pkgtool update codexcrew" },
  });
  const info = initAutoUpdate(deps).getInfo();
  assert.strictEqual(info.managedBy, "internal-registry");
  assert.strictEqual(info.updateCommand, "pkgtool update codexcrew");
...
test("managed check() with updateCommand+checkCommand emits found with the printed version", async (t) => {
  const { deps, states } = makeDeps({
    ...
    externallyManaged: {
      ...
      updateCommand: "pkgtool update codexcrew",
      checkCommand: "pkgtool check codexcrew",
    },
  });
  ...
  await u.check();
  assert.deepStrictEqual(commands, ["pkgtool check codexcrew"], "check must shell the checkCommand");
...
test("managed install() runs updateCommand then relaunch+exit", async (t) => {
  ...
  const exits = [];
  const { deps, states } = makeDeps({
    externallyManaged: { managedBy: "m", updateCommand: "pkgtool update codexcrew", checkCommand: "check" },
  });
  deps.app.relaunch = () => relaunches.push(true);
  ...
  await u.check();
  await u.install();
  assert.ok(commands.includes("pkgtool update codexcrew"), "install must shell the updateCommand");
...
test("managed auto-on-restart: pref true + found arms before-quit that runs updateCommand", async (t) => {
  ...
  quit.fn(event);
  await new Promise((r) => setImmediate(() => setImmediate(r)));
  assert.ok(commands.includes("pkgtool update codexcrew"), "before-quit must run the updateCommand");
...
test("readExternallyManaged: JSON marker carries metadata", (t) => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ managedBy: "internal-registry", updateCommand: "pkgtool update codexcrew" }),
  );
...
test("readExternallyManaged: env override points at a marker file", (t) => {
  ...
  const got = readExternallyManaged({
    env: { CODEXCREW_EXTERNALLY_MANAGED: marker },
...
test("baked marker: trusted as code -- the ownership probe is never consulted", (t) => {
  ...
  const { baked, resources } = bakedFixture(t, JSON.stringify({
    ...
    updateCommand: "/opt/toolbox/bin/toolbox update codexcrew",
    checkCommand: "/opt/toolbox/bin/codexcrew-update-check",
  }));
  let probed = 0;
  assert.deepStrictEqual(readExternallyManaged({
    env: {},
...
test("baked marker: the dev/test env seam still wins, and stays a LOOSE read", (t) => {
  // CODEXCREW_EXTERNALLY_MANAGED (unpackaged only) names the file the harness
  ...
  let probedPath = "";
  assert.deepStrictEqual(readExternallyManaged({
    env: { CODEXCREW_EXTERNALLY_MANAGED: envMarker },
    isPackaged: false,
    resourcesPath: resources,
...
test("readExternallyManaged: a PACKAGED app ignores CODEXCREW_EXTERNALLY_MANAGED", (t) => {
  const fs = require("node:fs");
  const os = require("node:os");
  const path = require("node:path");
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "kc-ext-"));
...
test("managed command env is CONSTRUCTED: nothing the shell reads as code is inherited", async (t) => {
  ...
  assert.strictEqual(env.CODEXCREW_MANAGED_ARGV0, process.execPath);
...
test("translocated bundle returns disabled:'translocated' and never arms the updater", () => {
  const { deps, calls } = makeDeps({
    resourcesPath: "/private/var/folders/ab/cd/d/AppTranslocation/UUID/d/Codex Crew.app/Contents/Resources",
  });
  const u = initAutoUpdate(deps);
...
test("read-only volume returns disabled:'volume' and never arms the updater", () => {
  const { deps, calls } = makeDeps({
    resourcesPath: "/Volumes/Codex Crew 1.0.0/Codex Crew.app/Contents/Resources",
...
test("WRITABLE volume still arms: an external disk is not a read-only image", () => {
  ...
  const { deps, calls } = makeDeps({
    resourcesPath: "/Volumes/External SSD/Codex Crew.app/Contents/Resources",
    bundleWritable: true,
  });
  const u = initAutoUpdate(deps);
  assert.strictEqual(u.disabled, undefined);
...
test("guard is macOS-only: a linux /Volumes-shaped path still arms", () => {
  ...
  const { deps, calls } = makeDeps({
    osPlatform: "linux",
    resourcesPath: "/Volumes/whatever/Codex Crew.app/Contents/Resources",
    bundleWritable: false,
  });
  const u = initAutoUpdate(deps);
  assert.strictEqual(u.disabled, undefined);
