...
test("a mounted volume is 'volume' — the path alone does not condemn it", () => {
  const p = "/Volumes/CodexCrew Nightly/CodexCrew Nightly.app/Contents/Resources";
  assert.equal(classifyBundleLocation(p, DARWIN), "volume");
});

test("Gatekeeper App Translocation is 'translocated'", () => {
  const p = "/private/var/folders/ab/cd/d/AppTranslocation/DEAD-BEEF/d/CodexCrew.app/Contents/Resources";
  assert.equal(classifyBundleLocation(p, DARWIN), "translocated");
});

test("a normal install is 'applications'", () => {
  assert.equal(
    classifyBundleLocation("/Applications/CodexCrew Nightly.app/Contents/Resources", DARWIN),
    "applications",
  );
  assert.equal(
    classifyBundleLocation("/Users/someone/Applications/CodexCrew.app/Contents/Resources", DARWIN),
    "applications",
  );
});

test("an unusual but writable path is 'other', not a problem", () => {
  const p = "/Users/someone/Desktop/CodexCrew.app/Contents/Resources";
...
test("translocation wins over the volume prefix", () => {
  ...
  const p = "/Volumes/x/d/AppTranslocation/UUID/d/CodexCrew.app/Contents/Resources";
  assert.equal(classifyBundleLocation(p, DARWIN), "translocated");
});

test("non-darwin platforms do not get macOS-only verdicts", () => {
  assert.equal(classifyBundleLocation("C:\\Program Files\\CodexCrew", { platform: "win32" }), "other");
  assert.equal(classifyBundleLocation("/opt/codexcrew", { platform: "linux" }), "other");
});
...
test("containingDirForBundle strips Contents/Resources and the .app", () => {
  assert.equal(
    containingDirForBundle("/Applications/CodexCrew.app/Contents/Resources"),
    "/Applications",
  );
  assert.equal(
    containingDirForBundle("/Volumes/CodexCrew Nightly/CodexCrew Nightly.app/Contents/Resources"),
    "/Volumes/CodexCrew Nightly",
  );
});
...
test("$APPIMAGE identifies an AppImage", () => {
  assert.equal(
    classifyLinuxInstall({ appImagePath: "/home/u/Applications/CodexCrew-x86_64.AppImage" }),
    "appimage",
  );
});
...
test("an /opt resourcesPath is a package install even with no other signal", () => {
  ...
  assert.equal(classifyLinuxInstall({ resourcesPath: "/opt/CodexCrew/resources" }), "package");
});
