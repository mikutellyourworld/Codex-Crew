...
function wslEvent(url = "http://localhost:5486/system/services") {
  return {
...
test("factory requires an integer primary port", () => {
  ...
  assert.throws(
    () => createIpcRegistrar({ ...dependencies, port: "5486" }),
    { message: "createIpcRegistrar: port is required" },
  );
});
...
test("wsl:detect allows owned and service-managed primary gateways without reshaping", async () => {
  for (const portOwner of ["codexcrew", "service"]) {
    const result = {
      available: true,
      distros: [{
        name: "Ubuntu",
