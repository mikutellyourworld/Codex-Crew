...
describe("migrateRemoteHostConfig", () => {
  it("migrates legacy remoteHost to remoteHosts[port]", () => {
    const store = mockStore({ remoteHost: "myhost.corp.example.com", codexcrewBinPath: "~/.local/bin/codexcrew", remoteHosts: {} });
    const result = migrateRemoteHostConfig(store, 7778);
    assert.equal(result, true);
    assert.deepEqual(store._data.remoteHosts, { 7778: { host: "myhost.corp.example.com", binPath: "~/.local/bin/codexcrew" } });
    assert.equal(store._data.remoteHost, undefined);
    assert.equal(store._data.codexcrewBinPath, undefined);
  });

  it("uses DEFAULT_REMOTE_BIN when codexcrewBinPath is empty", () => {
    const store = mockStore({ remoteHost: "host.com", codexcrewBinPath: "", remoteHosts: {} });
    migrateRemoteHostConfig(store, 7777);
    assert.equal(store._data.remoteHosts[7777].binPath, "~/.local/bin/codexcrew");
  });
...
describe("remoteHostPort", () => {
  ...
  it("skips an entry whose host was cleared", () => {
    const store = mockStore({
      remoteHosts: {
        "5477": { host: "", binPath: "~/.local/bin/codexcrew" },
        "7778": { host: "a.example.com" },
      },
    });
    assert.equal(remoteHostPort(store), 7778);
...
describe("isSelectablePort", () => {
  ...
  it("refuses anything that is not a port number in range", () => {
    for (const value of [0, -1, 65536, 1.5, NaN, null, undefined, "5486"]) {
      assert.equal(isSelectablePort(value), false, String(value));
    }
  });

  it("accepts the ordinary gateway ports", () => {
    for (const value of [1, 443, 5486, 7778, 65535]) {
