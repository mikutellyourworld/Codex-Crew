...
const TOKEN_PROMPT = "file:///app/resources/token-prompt.html?port=5486&kind=own";
const DASHBOARD = "http://localhost:5486/?token=abc123";
const DASH_ROUTE_A = "http://localhost:5486/settings";
const DASH_ROUTE_B = "http://localhost:5486/sessions/42";
...
describe("isTransientShellPage", () => {
  ...
  it("never matches the dashboard, even when a dashboard URL mentions loading.html", () => {
    ...
    assert.equal(isTransientShellPage("http://localhost:5486/loading.html"), false);
    assert.equal(isTransientShellPage("http://localhost:5486/?page=loading.html"), false);
  });
...
describe("armSplashHistoryClear (simulated handoffs)", () => {
  ...
  it("dashboard-only reloads (token retry) touch nothing", () => {
    ...
    fake.navigate(DASHBOARD);
    const removedSoFar = fake.removedIndexes.length;
    fake.navigate("http://localhost:5486/?token=rotated"); // token-retry reload
