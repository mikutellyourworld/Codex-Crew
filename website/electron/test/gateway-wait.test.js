...
test("Windows local startup keeps waiting while its child is alive", async () => {
  let polls = 0;
  const maxWaitMs = gatewayWaitTimeoutMs({ platform: "win32", watchSpawn: true });
  const { p } = harness({
    // 720 failed 500ms polls model a six-minute packaged startup — longer than
    // the former 120s deadline that showed a false can't-reach dialog while the
    // same child continued booting successfully.
    checkBackend: () => (++polls <= 720
  ...
  await p;
  assert.strictEqual(polls, 721);
  assert.strictEqual(maxWaitMs, Number.POSITIVE_INFINITY);
});
...
test("describeGatewayFailure: an incomplete bundle passes its message through bare", () => {
  const msg = "Codex Crew's bundled Python runtime is still being installed — retry.";
...
test("describeGatewayFailure: the disabled case names the port and both ways out", () => {
  const s = describeGatewayFailure({ disabled: true, port: 5486 });
  assert.match(s, /5486/);
...
test("describeGatewayFailure: an empty remoteHost keeps the local-start wording", () => {
  ...
  for (const failure of [
    { disabled: true, port: 5486 },
    { disabled: true, port: 5486, remoteHost: "" },
  ]) {
...
test("isPortInUse detects a port-bind failure", () => {
  assert.strictEqual(
    isPortInUse("17:57:53 ERROR codex_crew.dashboard.server: Port 7788 already in use -- is another CodexCrew gateway running?"),
    true,
  );
