...
describe("classifyAdoptedGateway", () => {
  ...
  it("classifies a same-family codexcrew-owned holder as reused-local", () => {
    assert.equal(classifyAdoptedGateway({ reason: "same-family", localOwner: "codexcrew" }), "reused-local");
  });
  ...
  it("stays none without the same-family health identification", () => {
    assert.equal(classifyAdoptedGateway({ reason: "healthy", localOwner: "codexcrew" }), "none");
  ...
  it("composes with chooseRecoveryStrategy end to end", () => {
    const local = classifyAdoptedGateway({ reason: "same-family", localOwner: "codexcrew" });
...
describe("snapshotPortPids", () => {
  it("uses the Windows listener probe so recovery can wait for gateway.lock", async () => {
    ...
    assert.deepEqual(calls, [["windows", 5486]]);
  });
...
describe("unrecoverableGatewayDialog", () => {
  it("offers a real quit action for an unkillable primary gateway", () => {
    ...
    assert.equal(model.title, "Codex Crew: backend stuck on port 5486");
    assert.equal(model.primaryAction, "quit");
    assert.equal(model.primaryLabel, "Quit Codex Crew");
  ...
  it("tells a probe-failure user to reopen before restarting", () => {
    ...
    assert.equal(model.title, "Codex Crew: can't verify what's using port 5486");
    assert.equal(model.primaryAction, "quit");
    assert.equal(model.primaryLabel, "Close");
    assert.equal(model.showQuitButton, false);
    assert.match(model.message, /Quit and reopen Codex Crew to try again/);
  ...
  it("tells a user to quit an unowned process that still holds the port", () => {
    ...
    assert.equal(model.title, "Codex Crew: port 5486 is in use");
    assert.equal(model.primaryAction, "quit");
    assert.equal(model.primaryLabel, "Quit Codex Crew");
    assert.equal(model.showQuitButton, false);
    assert.match(model.message, /Quit the process using port 5486/);
...
describe("shouldReresolveBackend", () => {
  ...
  it("pins the watchdog's exit status so the two sides cannot drift apart", () => {
    const backend = fs.readFileSync(
      path.join(__dirname, "..", "..", "..", "src", "codex_crew", "dashboard", "stale_asset_watchdog.py"),
      "utf8",
    );
  ...
  // A dev checkout with no codexcrew anywhere spawns the bare PATH name and gets
