...
// different CodexCrew identity families; every ambiguous case preserves the
...
test("same family reuses: prod shell, prod gateway", () => {
  const d = decideGatewayAction("0.1.0", { ok: true, app: "codexcrew", version: "0.1.0-insider.3" });
...
test("same family reuses: nightly shell, nightly gateway (relaunch)", () => {
  const d = decideGatewayAction("0.1.0-nightly.20260722120000", {
    ok: true, app: "codexcrew", version: "0.1.0-nightly.20260721000000",
  });
  assert.equal(d.action, "reuse");
});

// A local CodexCrew LISTEN owner is what makes an eviction legitimate. Passing
// it explicitly in the cross-family cases keeps those tests about FAMILY logic.
const LOCAL = { localOwner: "codexcrew" };

test("cross family prompts: nightly shell over prod gateway", () => {
  const d = decideGatewayAction("0.2.0-nightly.20260722120000", { ok: true, app: "codexcrew", version: "0.1.0" }, LOCAL);
...
test("cross family prompts: prod shell over nightly gateway", () => {
  const d = decideGatewayAction("0.1.0", { ok: true, app: "codexcrew", version: "0.1.0-nightly.20260722120000" }, LOCAL);
  assert.equal(d.action, "takeover-prompt");
...
test("non-codexcrew responder on the port reuses (never evict a stranger)", () => {
...
test("unclassifiable own version never evicts", () => {
  assert.equal(decideGatewayAction("", { ok: true, app: "codexcrew", version: "0.1.0" }).action, "reuse");
});

// ── Locality: a cross-family payload alone must never authorise an eviction ──
// An `ssh -L 5486:localhost:5486 host` forward makes a REMOTE gateway answer
...
// reuse, because the port is not held by a local CodexCrew process.
const CROSS_FAMILY_HEALTH = { ok: true, app: "codexcrew", version: "0.1.0" };
...
test("a genuine local rival install still prompts (cross-app mutex preserved)", () => {
  ...
  // on ONE machine sharing ~/.codex-crew and :5486. The guard must keep working.
  const d = decideGatewayAction(NIGHTLY_SHELL, CROSS_FAMILY_HEALTH, { localOwner: "codexcrew" });
  assert.equal(d.action, "takeover-prompt");
  assert.equal(d.otherFamily, "prod");
});
...
test("locality is checked only after family — same-family still short-circuits", () => {
  ...
  const d = decideGatewayAction("0.1.0", { ok: true, app: "codexcrew", version: "0.1.0-insider.3" }, { localOwner: "foreign" });
  assert.equal(d.action, "reuse");
...
test("FAMILY_META separates display names from quit-by-name targets", () => {
  ...
  assert.equal(FAMILY_META.prod.appName, "CodexCrew");
  assert.equal(FAMILY_META.nightly.appName, "CodexCrew Nightly");
  assert.equal(FAMILY_META.prod.displayName, "Codex Crew");
  assert.equal(FAMILY_META.nightly.displayName, "Codex Crew Nightly");
});
