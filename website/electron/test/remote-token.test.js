...
describe("REMOTE_BIN_CANDIDATES", () => {
  it("lists the toolbox path first (most common install)", () => {
    assert.equal(REMOTE_BIN_CANDIDATES[0], "~/.toolbox/bin/codexcrew");
  });
...
describe("buildCandidateTokenCommand", () => {
  ...
  it("includes CODEXCREW_PORT when port option is provided", () => {
    const cmd = buildCandidateTokenCommand(REMOTE_BIN_CANDIDATES, { port: "7778" });
    assert.match(cmd, /CODEXCREW_PORT=7778/);
  });
...
describe("buildRemoteTokenCommand", () => {
  ...
  it("respects a user-customized binPath", () => {
    const cmd = buildRemoteTokenCommand("/opt/custom/codexcrew");
    assert.doesNotMatch(cmd, /for b in /);
    assert.match(cmd, /"\/opt\/custom\/codexcrew" token/);
  });

  it("includes CODEXCREW_PORT for custom binPath", () => {
    const cmd = buildRemoteTokenCommand("/opt/custom/codexcrew", { port: "7778" });
    assert.match(cmd, /CODEXCREW_PORT=7778/);
    assert.match(cmd, /"\/opt\/custom\/codexcrew" token/);
  });
  ...
  it("rewrites a leading ~/ to $HOME/ so it expands inside double quotes", () => {
    ...
    const cmd = buildRemoteTokenCommand("~/apps/codexcrew");
    assert.match(cmd, /"\$HOME\/apps\/codexcrew" token/);
    assert.doesNotMatch(cmd, /"~\//);
  });

  it("leaves absolute and $HOME-prefixed paths untouched", () => {
    assert.match(buildRemoteTokenCommand("/opt/x/codexcrew"), /"\/opt\/x\/codexcrew" token/);
    assert.match(buildRemoteTokenCommand("$HOME/x/codexcrew"), /"\$HOME\/x\/codexcrew" token/);
  });

  it("passes port through to candidate command", () => {
    const cmd = buildRemoteTokenCommand(DEFAULT_REMOTE_BIN, { port: "8080" });
    assert.match(cmd, /CODEXCREW_PORT=8080/);
  });
...
describe("parseTokenFromStdout", () => {
  it("extracts token from standard URL", () => {
    const url = "http://localhost:5486?token=eyJhbGciOiJIUzI1NiJ9";
