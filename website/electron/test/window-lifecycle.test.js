...
describe("session security registration", () => {
  it("registers every default and browser-partition policy exactly once", async () => {
    ...
    const dashboard = { getURL: () => "http://localhost:5486/chat" };
    let micGranted = null;
    ...
    assert.equal(
      calls.defaultCheck[0](null, "media", "http://localhost:5486", {
        mediaType: "audio",
      }),
      true,
    );
    assert.equal(
      calls.defaultCheck[0](dashboard, "media", "http://localhost:5486", {
        mediaType: "video",
      }),
      false,
    );
