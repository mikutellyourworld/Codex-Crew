...
describe("findKirocrewBin", () => {
  it("returns the bundled backend launcher (backend-dist/.../bin/codexcrew) when it exists", () => {
    const venvLayout = path.join(RESOURCES, "backend-dist", "codexcrew-backend", "bin", "codexcrew");
  ...
  it("returns ~/.toolbox/bin/codexcrew when bundled paths don't exist", () => {
    const toolboxBin = path.join(HOME, ".toolbox", "bin", "codexcrew");
    const fakeFs = only(toolboxBin);
    const result = findKirocrewBin(fakeFs, fakeOs, path, RESOURCES, DIRNAME);
    assert.equal(result, toolboxBin);
  });

  it("returns ~/.local/bin/codexcrew when bundled and toolbox paths don't exist", () => {
    const localBin = path.join(HOME, ".local", "bin", "codexcrew");
    const fakeFs = only(localBin);
    const result = findKirocrewBin(fakeFs, fakeOs, path, RESOURCES, DIRNAME);
    assert.equal(result, localBin);
  });

  it("returns ~/.codexcrew-app/.venv/bin/codexcrew when only venv binary exists", () => {
    const venvBin = path.join(HOME, ".codexcrew-app", ".venv", "bin", "codexcrew");
    const fakeFs = only(venvBin);
    const result = findKirocrewBin(fakeFs, fakeOs, path, RESOURCES, DIRNAME);
    assert.equal(result, venvBin);
  });

  it("returns ../bin/codexcrew relative to dirname when only that path exists", () => {
    const binPath = path.resolve(DIRNAME, "..", "bin", "codexcrew");
    const fakeFs = only(binPath);
    const result = findKirocrewBin(fakeFs, fakeOs, path, RESOURCES, DIRNAME);
    assert.equal(result, binPath);
  });

  it("falls back to bare 'codexcrew' when no candidates are executable (POSIX)", () => {
    const result = findKirocrewBin(none, fakeOs, path, RESOURCES, DIRNAME, "x64", false);
    assert.equal(result, "codexcrew");
  });

  it("falls back to bare 'codexcrew.exe' when no candidates are executable (Windows)", () => {
    const result = findKirocrewBin(none, fakeOs, path, RESOURCES, DIRNAME, "x64", true);
    assert.equal(result, "codexcrew.exe");
  });

  it("finds the venv Scripts\\codexcrew.exe two levels up from electron/ on Windows", () => {
    const venvExe = path.resolve(DIRNAME, "..", "..", ".venv", "Scripts", "codexcrew.exe");
  ...
  it("finds the bundled Windows backend Scripts\\codexcrew.exe on Windows", () => {
    const bundledExe = path.join(
      RESOURCES, "backend-dist", "codexcrew-backend", "Scripts", "codexcrew.exe"
    );
  ...
  // bin\codexcrew.cmd, and the `pip install` that populates the tree also drops
  // a console-script Scripts\codexcrew.exe. Every Windows case above uses
  ...
  it("prefers the relocatable bin\\codexcrew.cmd over the bundle's Scripts\\codexcrew.exe", () => {
    ...
    // points at a build-agent path like D:\a\CodexCrew\... that does not exist on
    ...
    const cmd = path.join(RESOURCES, "backend-dist", "codexcrew-backend", "bin", "codexcrew.cmd");
    const exe = path.join(RESOURCES, "backend-dist", "codexcrew-backend", "Scripts", "codexcrew.exe");
    const result = findKirocrewBin(present(cmd, exe), fakeOs, path, RESOURCES, DIRNAME, "x64", true);
    assert.equal(result, cmd);
  });

  it("prefers the bundled launcher over a checkout virtual environment", () => {
    const cmd = path.join(RESOURCES, "backend-dist", "codexcrew-backend", "bin", "codexcrew.cmd");
    const checkoutExe = path.resolve(DIRNAME, "..", "..", ".venv", "Scripts", "codexcrew.exe");
    const result = findKirocrewBin(
      present(cmd, checkoutExe), fakeOs, path, RESOURCES, DIRNAME, "x64", true
    );
    assert.equal(result, cmd);
  });

  it("prefers the bundle's Scripts\\codexcrew.exe over a user-level install", () => {
    // A packaged app must run the backend it shipped with, never whatever
    // happens to be installed on the machine.
    const exe = path.join(RESOURCES, "backend-dist", "codexcrew-backend", "Scripts", "codexcrew.exe");
    const userLocal = path.join(HOME, ".local", "bin", "codexcrew.exe");
    const result = findKirocrewBin(
  ...
  it("still finds a user-level codexcrew.exe when nothing is bundled", () => {
    const userLocal = path.join(HOME, ".local", "bin", "codexcrew.exe");
    const result = findKirocrewBin(
  ...
  it("returns first match when multiple candidates exist", () => {
    const bundled = path.join(RESOURCES, "backend-dist", "codexcrew-backend", "bin", "codexcrew");
    const localBin = path.join(HOME, ".local", "bin", "codexcrew");
    const fakeFs = {
  ...
  it("resolves dirname-relative dev path correctly", () => {
    const devBin = path.resolve(DIRNAME, "backend-dist", "codexcrew-backend", "bin", "codexcrew");
  ...
  const archBin = (arch) =>
    path.join(RESOURCES, "backend-dist", `codexcrew-backend-${arch}`, "bin", "codexcrew");
  ...
  it("prefers the arch-suffixed tree over the unsuffixed layout when both exist", () => {
    const unsuffixed = path.join(RESOURCES, "backend-dist", "codexcrew-backend", "bin", "codexcrew");
  ...
  it("skips arch-suffixed candidates cleanly for an unmapped arch (e.g. 'ia32')", () => {
    ...
    assert.equal(result, unsuffixed);
    assert.deepStrictEqual(probed.filter((p) => p.includes("codexcrew-backend-")), []);
  });
});
