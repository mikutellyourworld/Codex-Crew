...
const APP = "CodexCrew"; // brand-ok: build.productName, an on-disk identifier
...
const EXEC = "codexcrew-desktop";
const NIGHTLY_EXEC = "codexcrew-desktop-nightly";
...
describe("crashLogPath / crashStatePath", () => {
  it("keeps both files in the logs directory beside chromium.log", () => {
    assert.equal(crashLogPath("/logs/Codex Crew"), path.join("/logs/Codex Crew", CRASH_LOG_BASENAME));
    assert.equal(crashStatePath("/logs/Codex Crew"), path.join("/logs/Codex Crew", CRASH_STATE_BASENAME));
  });
...
describe("isOwnModule", () => {
  ...
  it("ignores spacing and .exe so one app name serves every platform", () => {
    assert.equal(isOwnModule(`C:\\Program Files\\${APP}\\${APP}.exe`, "Codex Crew"), true);
  });
  ...
  // enumerated set: on Linux and Windows the binary is `codexcrew-desktop`, which
  // is neither equal to `Codex Crew` nor a helper of it. Off darwin a minidump is
  ...
  it("accepts the executable name, which the display name does not imply", () => {
    assert.equal(isOwnModule(`/opt/${APP}/${EXEC}`, ["Codex Crew", EXEC]), true);
    assert.equal(isOwnModule(`C:\\Program Files\\${APP}\\${EXEC}.exe`, ["Codex Crew", EXEC]), true);
    // The nightly channel renames the binary again, and `codexcrew-desktop-nightly`
    // extends `codexcrewnightly` in no direction at all — which is precisely why
    // the name has to be passed in rather than computed from the display name.
    assert.equal(isOwnModule(`/opt/${APP}/${NIGHTLY_EXEC}`, ["Codex Crew Nightly", NIGHTLY_EXEC]), true);
  });
  ...
  it("still refuses a sibling channel's binary once two names are in play", () => {
    assert.equal(isOwnModule(`/opt/${APP}/${NIGHTLY_EXEC}`, ["Codex Crew", EXEC]), false);
    assert.equal(isOwnModule(`/opt/${APP}/${EXEC}`, ["Codex Crew Nightly", NIGHTLY_EXEC]), false);
    assert.equal(isOwnModule(`${EXEC}-helper-x`, ["Codex Crew", EXEC]), false);
  });
...
describe("ipsBelongsToApp", () => {
  it("matches our reports by name before anything is opened", () => {
    ...
    assert.equal(ipsBelongsToApp(`${APP}-2026-09-03-101530.ips`, "Codex Crew"), true);
  });
...
describe("collectCrashReports — bounds and failure", () => {
  ...
  it("acknowledges a genuinely foreign dump, which is why the name must be right", () => {
    ...
    const scan = collectCrashReports({
      ...withBaseline(fs), appName: "Codex Crew", execName: EXEC, diagnosticReportsDir: "",
    });
    assert.deepEqual(scan.newCrashes, []);
