...
test("manualDownloadUrl: per-platform artifact on the byte host", () => {
  assert.strictEqual(
    ...
    `${DOWNLOAD_BASE}/desktop/nightly/latest/CodexCrew.dmg`,
  );
  assert.strictEqual(
    manualDownloadUrl("stable", "linux", "x64"),
    `${DOWNLOAD_BASE}/desktop/stable/latest/CodexCrew-x86_64.AppImage`,
  );
  assert.strictEqual(
    manualDownloadUrl("insider", "darwin"),
    `${DOWNLOAD_BASE}/desktop/insider/latest/CodexCrew.dmg`,
  );
  assert.strictEqual(
    manualDownloadUrl("nightly", "win32", "x64"),
    `${DOWNLOAD_BASE}/desktop/nightly/latest/CodexCrew-Setup.exe`,
  );
});
...
test("manualDownloadUrl: Windows publishes x64 on every known channel", () => {
  ...
  assert.strictEqual(
    ...
    `${DOWNLOAD_BASE}/desktop/insider/latest/CodexCrew-Setup.exe`,
  );
  ...
  assert.strictEqual(
    ...
    `${DOWNLOAD_BASE}/desktop/stable/latest/CodexCrew-Setup.exe`,
  );
  ...
  assert.strictEqual(
    ...
    `${DOWNLOAD_BASE}/desktop/stable/latest/CodexCrew.dmg`,
  );
});
...
test("manualDownloadUrl: Linux picks the AppImage for the running arch", () => {
  ...
  assert.strictEqual(
    ...
    `${DOWNLOAD_BASE}/desktop/stable/latest/CodexCrew-aarch64.AppImage`,
  );
  assert.strictEqual(
    manualDownloadUrl("stable", "linux", "x64"),
    `${DOWNLOAD_BASE}/desktop/stable/latest/CodexCrew-x86_64.AppImage`,
...
test("manualDownloadUrl: the link matches the format they installed", () => {
  ...
  assert.strictEqual(
    ...
    `${DOWNLOAD_BASE}/desktop/stable/latest/CodexCrew-x86_64.deb`,
  );
  assert.strictEqual(
    manualDownloadUrl("stable", "linux", "arm64", "rpm"),
    `${DOWNLOAD_BASE}/desktop/stable/latest/CodexCrew-aarch64.rpm`,
  );
