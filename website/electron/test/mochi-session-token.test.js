...
describe("borrowSessionToken", () => {
  it("returns the mc_token_<port> cookie value for the backend's port", async () => {
    ...
    const electronSession = {
      cookies: {
        get(filter) {
          seen.push(filter);
          return Promise.resolve([{ name: "mc_token_5486", value: "session-cookie-value" }]);
        },
      },
    };
    ...
    assert.deepEqual(seen, [{ url: "http://localhost:5486", name: "mc_token_5486" }]);
  });
  ...
  it("fails closed when there is no session/cookie API at all", async () => {
    assert.equal(
      await borrowSessionToken({ electronSession: null, backendUrl: "http://localhost:5486" }),
      "",
    );
    assert.equal(
      await borrowSessionToken({ electronSession: {}, backendUrl: "http://localhost:5486" }),
      "",
    );
  });
