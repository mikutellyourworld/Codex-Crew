...
test("our own local gateway points at THIS machine", () => {
  assert.equal(classifyAuthBlock({ localOwner: "codexcrew" }), "local");
});

test("a confirmed tunnel (ssh owns the socket) points at the OTHER machine", () => {
  // The reported scenario: `ssh -NL 5486:localhost:5486 dev-host`. The remote
...
test("a configured remote host is decisive, whatever the socket says", () => {
  assert.equal(
    classifyAuthBlock({ localOwner: "codexcrew", remoteHost: "dev-host.example.com" }),
    "foreign",
  );
});
...
test("an empty remoteHost does not force the foreign verdict", () => {
  ...
  assert.equal(classifyAuthBlock({ localOwner: "codexcrew", remoteHost: "" }), "local");
});
...
test("defaultedPort resolves a scheme-default URL to a real port", () => {
  ...
  // :5486 — describing and submitting to a different gateway than the one that
  // just returned 403.
...
test("defaultedPort passes an explicit port through unchanged", () => {
  assert.equal(defaultedPort("http://localhost:5486/"), "5486");
