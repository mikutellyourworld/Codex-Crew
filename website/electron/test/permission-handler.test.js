...
const APP = wcAt("http://localhost:5486/chat/x?token=abc");
const ORIGIN = "http://localhost:5486";
...
describe("isAppOrigin — two sources", () => {
  ...
  it("FALLS BACK to the origin string when webContents is null", () => {
    ...
    assert.equal(isAppOrigin(undefined, "http://localhost:5486/"), true);
...
describe("createPermissionCheckHandler", () => {
  it("GRANTS the real observed payload: live frame, NO mediaType at all", () => {
    ...
    //   CHECK media wcIsNull=false origin=http://localhost:5486/ mediaType=undefined
    ...
    const observed = {
      embeddingOrigin: "http://localhost:5486/",
      isMainFrame: true,
      requestingUrl: "http://localhost:5486/chat/x?token=redacted&sid=chat-70",
    };
    assert.equal(h(APP, "media", "http://localhost:5486/", observed), true);
    // …and the follow-up check that did carry mediaType.
    assert.equal(h(APP, "media", "http://localhost:5486/", { ...observed, mediaType: "audio" }), true);
  });
