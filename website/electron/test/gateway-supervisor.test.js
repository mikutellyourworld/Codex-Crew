...
function harness(overrides = {}) {
  ...
  const port = overrides.port ?? 5486;
  const processRef = overrides.processRef || {
    platform: "test",
    arch: "x64",
    execPath: "/virtual/codexcrew-desktop",
    env: { CODEXCREW_HOME: "/virtual/codexcrew-home" },
...
test("entryUrl preserves an initial path/query and encodes the token once", () => {
  ...
  assert.strictEqual(result.origin, "http://localhost:5486");
...
test("entryUrl omits the token parameter when no token is available", () => {
  const { supervisor } = harness();
  const result = new URL(supervisor.entryUrl("http://localhost:5486", "/settings"));
...
test("gateway receives Electron as the packaged Codex adapter node runner", async () => {
  const { supervisor, spawnCalls } = harness();

  assert.strictEqual(await supervisor.start(), true);
  assert.strictEqual(
    spawnCalls[0][2].env.CODEXCREW_NODE_EXECUTABLE,
    "/virtual/codexcrew-desktop",
  );
});
...
const APP_EXEC_PATH = "/virtual/Applications/CodexCrew.app/Contents/MacOS/CodexCrew";
...
const BUNDLED_BIN = "/virtual/resources/backend-dist/codexcrew-backend-x64/bin/codexcrew";
...
test("a second stale exit starts a fresh copy of the app and exits only once its gateway is serving", async () => {
  ...
  assert.ok(state.statuses.includes("status:Restarting Codex Crew to finish the update…"),
  ...
  assert.ok(logs.some((line) => line.includes("is serving on :5486") && line.includes("exiting this instance")));
  assert.deepStrictEqual(timers.pending, [], "the deadline is disarmed once the successor is confirmed");
  assert.ok(state.statuses.filter((entry) => entry === "status:Restarting Codex Crew to finish the update…").length >= 2,
...
test("a successor that never serves within the bound is stopped and this instance stays", async () => {
  ...
  assert.strictEqual(state.lockRequests, 1, "the single-instance lock is taken back");
  assert.ok(logs.some((line) => line.includes("did not answer on :5486 within 60s")));
...
test("a pruned bundle re-probes once, then surfaces the failure when the app executable is gone too", async () => {
  ...
  assert.strictEqual(spawnCalls.length, 2);
  assert.strictEqual(spawnCalls[1][0], "codexcrew");
