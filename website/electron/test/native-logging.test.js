...
describe("nativeLogPath / previousNativeLogPath", () => {
  it("sits next to the other launch logs in the logs directory", () => {
    assert.equal(nativeLogPath("/logs/Codex Crew"), path.join("/logs/Codex Crew", NATIVE_LOG_BASENAME));
  });
...
describe("nativeLoggingSwitches", () => {
  ...
  it("uses the exact Chromium switch names", () => {
    assert.deepEqual(nativeLoggingSwitches("/tmp/c.log", { CODEXCREW_DEBUG: "1" }), [
      ["enable-logging", "file"],
      ["log-file", "/tmp/c.log"],
  ...
  // capture crash logs, so the switch rides the CODEXCREW_DEBUG opt-in while the
  ...
  it("treats an explicit falsey debug value as off", () => {
    const names = nativeLoggingSwitches("/tmp/c.log", { CODEXCREW_DEBUG: "0" }).map(([n]) => n);
...
describe("initNativeLogging", () => {
  function harness(over = {}) {
    ...
    const result = initNativeLogging({
      ...
      // carry CODEXCREW_DEBUG.
      env: { CODEXCREW_DEBUG: "1" },
      ...over,
      fs,
    });
