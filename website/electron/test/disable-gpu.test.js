...
test("shouldDisableGpu: truthy CODEXCREW_DISABLE_GPU values enable it", () => {
  for (const v of ["1", "true", "TRUE", "yes", "on", " On "]) {
    assert.equal(
      shouldDisableGpu({ env: { CODEXCREW_DISABLE_GPU: v }, argv: [] }),
      true,
...
test("initGpuPolicy: a throwing appendSwitch does not abort the rest", () => {
  ...
  const res = initGpuPolicy({
    ...
    env: { CODEXCREW_DISABLE_GPU: "true" },
    argv: [],
  });
  assert.equal(res.disabled, true);
