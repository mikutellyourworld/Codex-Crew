...
describe("BINPATH_RE", () => {
  it("accepts valid binary paths", () => {
    assert.ok(BINPATH_RE.test("~/.local/bin/codexcrew"));
    assert.ok(BINPATH_RE.test("/usr/local/bin/codexcrew"));
    assert.ok(BINPATH_RE.test("codexcrew"));
    assert.ok(BINPATH_RE.test("$HOME/.local/bin/codexcrew"));
  });
  ...
  it("rejects paths with spaces", () => {
    assert.ok(!BINPATH_RE.test("/opt/my tools/codexcrew"));
    assert.ok(!BINPATH_RE.test("codexcrew token"));
  });

  it("accepts paths with single dots (current dir)", () => {
    assert.ok(BINPATH_RE.test("./bin/codexcrew"));
    assert.ok(BINPATH_RE.test("/usr/./bin/codexcrew"));
  });
...
describe("validateRemoteSettings", () => {
  it("returns null for valid settings", () => {
    assert.equal(validateRemoteSettings("myhost.corp.example.com", "~/.local/bin/codexcrew"), null);
    assert.equal(validateRemoteSettings("myhost.corp.example.com", "$HOME/.local/bin/codexcrew"), null);
  });

  it("returns null for empty host (skips remote)", () => {
    assert.equal(validateRemoteSettings("", "~/.local/bin/codexcrew"), null);
  });
  ...
  it("accepts SSH config alias as hostname", () => {
    assert.equal(validateRemoteSettings("clouddesk", "/usr/bin/codexcrew"), null);
    assert.equal(validateRemoteSettings("dev-box", "~/.toolbox/bin/codexcrew"), null);
  });

  it("rejects hostname with SSH option injection", () => {
    const err = validateRemoteSettings("-oProxyCommand=evil", "/usr/bin/codexcrew");
    assert.ok(err);
  });
  ...
  it("rejects bin path with spaces", () => {
    const err = validateRemoteSettings("host.example.com", "/opt/my tools/codexcrew");
    assert.ok(err);
  });
  ...
  it("rejects hostname with consecutive dots", () => {
    assert.ok(validateRemoteSettings("a..b", "/usr/bin/codexcrew"));
  });

  it("rejects hostname over 253 chars", () => {
    const long = "a" + ".bb".repeat(84) + ".c";  // > 253 chars
    assert.ok(validateRemoteSettings(long, "/usr/bin/codexcrew"));
  });
