...
test("windowsGatewayExecutablePaths resolves the executables a launcher can spawn", () => {
  assert.deepStrictEqual(
    windowsGatewayExecutablePaths(
      "D:\\CodexCrew\\resources\\backend-dist\\codexcrew-backend\\bin\\codexcrew.cmd"
    ),
    ["D:\\CodexCrew\\resources\\backend-dist\\codexcrew-backend\\python.exe"]
  );
  assert.deepStrictEqual(
    windowsGatewayExecutablePaths("D:\\venv\\Scripts\\codexcrew.exe"),
    [
      "D:\\venv\\Scripts\\codexcrew.exe",
      "D:\\venv\\Scripts\\python.exe",
      "D:\\venv\\python.exe",
    ]
  );
  assert.deepStrictEqual(
    windowsGatewayExecutablePaths("codexcrew.exe", { pathEnv: "" }),
    []
  );
});

test("windowsGatewayExecutablePaths resolves a PATH-launched executable", () => {
  const expected = "D:\\Codex Crew\\Scripts\\codexcrew.exe";
  const probed = [];
  const resolved = windowsGatewayExecutablePaths("codexcrew.exe", {
    pathEnv: 'C:\\Other;"D:\\Codex Crew\\Scripts"',
  ...
  assert.deepStrictEqual(resolved, [
    expected,
    "D:\\Codex Crew\\Scripts\\python.exe",
    "D:\\Codex Crew\\python.exe",
  ]);
  assert.deepStrictEqual(probed, [
    "C:\\Other\\codexcrew.exe",
    expected,
  ]);
});

test("a venv launcher trusts the Python listener it delegates to", () => {
  const launcher = "D:\\venv\\Scripts\\codexcrew.exe";
  const trustedExecutablePaths = windowsGatewayExecutablePaths(launcher);
  assert.strictEqual(
    isKirocrewCommand(
      '"D:\\venv\\Scripts\\python.exe" '
        + '"D:\\venv\\Scripts\\python.exe" -s -m codex_crew gateway',
...
test("parseNetstatListenPids finds IPv4 and IPv6 listeners without reading state text", () => {
  const output = [
    ...
    "  TCP    127.0.0.1:5486        0.0.0.0:0              ABHÖREN         4242",
    "  TCP    [::1]:5486            [::]:0                 ÉCOUTE          4343",
    "  TCP    0.0.0.0:5486          0.0.0.0:0              ESCUCHANDO      4242",
  ].join("\r\n");
  assert.deepStrictEqual(parseNetstatListenPids(output, 5486), [4242, 4343]);
});

test("parseNetstatListenPids requires the exact local port and a wildcard peer", () => {
  const output = [
    "  TCP    127.0.0.1:54860       0.0.0.0:0              LISTENING       1111",
    "  TCP    127.0.0.1:5486        10.0.0.5:443           ESTABLISHED     2222",
    "  UDP    127.0.0.1:5486        *:*                                    3333",
  ].join("\r\n");
  assert.deepStrictEqual(parseNetstatListenPids(output, 5486), []);
});
...
test("windowsListenPids queries both address families with a bounded timeout", async () => {
  ...
  const execFileFn = (command, args, options, callback) => {
    ...
    callback(null, "TCP  [::1]:5486  [::]:0  ECOUTE  4343");
  };
  const pids = await windowsListenPids(5486, {
    execFileFn,
    timeoutMs: 1234,
    tools: TEST_TOOLS,
  });
...
test("windowsListenPids rejects netstat errors and timeouts", async () => {
  ...
  await assert.rejects(
    windowsListenPids(5486, { execFileFn, tools: TEST_TOOLS }),
    /netstat timed out/
  );
});
...
test("windowsProcessCommand prefers PowerShell command-line output", async () => {
  ...
  const execFileFn = (command, args, options, callback) => {
    ...
    callback(null, '"C:\\Program Files\\CodexCrew\\codexcrew.exe" gateway\r\n');
  };
  const command = await windowsProcessCommand(4242, {
    execFileFn,
    tools: TEST_TOOLS,
  });
  assert.strictEqual(command, '"C:\\Program Files\\CodexCrew\\codexcrew.exe" gateway');
...
test("windowsProcessCommand falls back to WMIC and unwraps its value", async () => {
  ...
  const execFileFn = (command, args, _options, callback) => {
    ...
    callback(
      ...
      "\r\nExecutablePath=C:\\Python\\python.exe\r\n"
        + "CommandLine=C:\\Python\\python.exe -m codex_crew gateway\r\n\r\n"
    );
  };
  const command = await windowsProcessCommand(4242, {
    execFileFn,
  ...
  assert.strictEqual(
    command,
    '"C:\\Python\\python.exe" C:\\Python\\python.exe -m codex_crew gateway'
  );
  assert.deepStrictEqual(
...
test("windowsProcessCommand fails closed when WMIC cannot prove the executable path", async () => {
  const execFileFn = (command, _args, _options, callback) => {
    ...
    callback(null, "CommandLine=codexcrew gateway\r\n");
  };
  assert.strictEqual(
    await windowsProcessCommand(4242, { execFileFn, tools: TEST_TOOLS }),
    ""
...
test("isKirocrewCommand accepts Windows executable and module shapes", () => {
  ...
  const trustedBackend = "C:\\bundle\\codexcrew-backend.exe";
  ...
  assert.strictEqual(
    isKirocrewCommand(`${trustedPython} -s -m codex_crew gateway`, {
  ...
  assert.strictEqual(
    isKirocrewCommand(
      `${trustedPython} C:\\venv\\Scripts\\codexcrew gateway`,
      { trustedExecutablePaths: [trustedPython] }
    ),
    true
  );
  assert.strictEqual(
    isKirocrewCommand(
      `"${trustedPython}" ${trustedPython} -s -m codex_crew gateway`,
      { trustedExecutablePaths: [trustedPython] }
    ),
    true
  );
...
test("isKirocrewCommand rejects Windows path substrings and unrelated Python", () => {
  assert.strictEqual(
    isKirocrewCommand("C:\\Users\\codexcrew\\OtherApp\\server.exe --port 5486"),
    false
  );
  assert.strictEqual(
    isKirocrewCommand("C:\\Users\\codexcrew\\python.exe -m http.server 5486"),
    false
  );
  assert.strictEqual(
    isKirocrewCommand("C:\\Python\\python.exe app.py codexcrew"),
    false
  );
  assert.strictEqual(
    isKirocrewCommand("C:\\Python\\python.exe app.py C:\\tmp\\codexcrew"),
    false
  );
  assert.strictEqual(
    isKirocrewCommand("C:\\Python\\python.exe app.py -m codex_crew"),
    false
  );
  assert.strictEqual(
    isKirocrewCommand("C:\\Temp\\codexcrew.exe gateway", {
      trustedExecutablePaths: [
        "C:\\Program Files\\CodexCrew\\codexcrew.exe",
      ],
    }),
    false
  );
...
test("isKirocrewCommand rejects SSH aliases and remote gateway commands", () => {
  assert.strictEqual(
    isKirocrewCommand("C:\\Windows\\System32\\OpenSSH\\ssh.exe -NL 5486:localhost:5486 codexcrew"),
    false
  );
  assert.strictEqual(
    isKirocrewCommand("ssh.exe host python.exe -m codex_crew gateway"),
    false
  );
});
...
test("Windows force-stop refuses an SSH holder even when its alias is codexcrew", async () => {
  const killed = [];
  const result = await forceStopPort(5486, {
    getListenPids: async () => [909],
    getCommand: async () =>
      "C:\\Windows\\System32\\OpenSSH\\ssh.exe -NL 5486:localhost:5486 codexcrew",
...
test("Windows force-stop refuses unrelated Python with a later codexcrew path", async () => {
  const killed = [];
  const result = await forceStopPort(5486, {
    getListenPids: async () => [910],
    getCommand: async () => "C:\\Python\\python.exe app.py C:\\tmp\\codexcrew",
    kill: async (pid) => killed.push(pid),
    sleep: async () => {},
    failClosedOnProbeError: true,
  });
...
test("Windows force-stop refuses an untrusted codexcrew.exe basename", async () => {
  const killed = [];
  const trustedCli = "C:\\Program Files\\CodexCrew\\codexcrew.exe";
  const result = await forceStopPort(5486, {
    getListenPids: async () => [911],
    getCommand: async () => "C:\\Temp\\codexcrew.exe gateway",
    kill: async (pid) => killed.push(pid),
    sleep: async () => {},
