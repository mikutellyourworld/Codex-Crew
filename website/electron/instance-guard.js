 ...
 *
 * The nightly app ("CodexCrew Nightly.app") and the production app
 * ("CodexCrew.app", stable ⇄ insider via the update channel) are separate
 * INSTALLS sharing ONE bundle identifier (io.github.mikutellyourworld.codexcrew — Finder
 ...
 * (~/.codex-crew), and ONE gateway port (5486). That makes the port a mutex:
 * only one CodexCrew-family gateway may run at a time. Electron's
 * requestSingleInstanceLock is keyed on userData (per productName), so it
 * cannot stop "CodexCrew Nightly" launching while "CodexCrew" runs — this
 ...
 * AND the port is held by a local CodexCrew process. That last input is what
...
const FAMILY_META = {
  ...
  prod: { appName: "CodexCrew", displayName: "Codex Crew" },
  nightly: { appName: "CodexCrew Nightly", displayName: "Codex Crew Nightly" },
};
 ...
 * @param {object} [opts]
 * @param {"codexcrew"|"foreign"|"none"|"unknown"} [opts.localOwner="unknown"]
...
function decideGatewayAction(ownVersion, remoteHealth, { localOwner = "unknown" } = {}) {
  ...
  // Legacy (pre-identity health payload) or non-codexcrew responder on the
  // port: keep the historical reuse behavior. The guard turns on organically
  // once both sides carry the identity fields.
  if (!remoteHealth || remoteHealth.app !== "codexcrew" || !remoteHealth.version) {
  ...
  // Cross-family: the payload says "a rival CodexCrew owns the port". That is
  ...
  // local CodexCrew LISTEN owner authorises the eviction; a tunnel ("foreign",
