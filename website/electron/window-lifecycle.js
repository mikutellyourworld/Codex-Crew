...
const BROWSER_PARTITION = "persist:codexcrew-browser";
...
function createWindowLifecycle(options) {
  ...
  function setupWindowContents(win, windowBackendUrl) {
    ...
    function applyTitle() {
      ...
      if (!IS_WIN) {
        ...
        win.setTitle(`Codex Crew ${suffix}`);
        return;
      }
      ...
      if (!suffix && windowPort && String(windowPort) !== "5486") {
        suffix = `[:${windowPort}]`;
      }
      win.setTitle(suffix ? `Codex Crew ${suffix}` : "Codex Crew");
    }
          ...
          if (!document.getElementById('electron-linux-controls')) {
            ...
            const mk = (cls, label, action) => {
              ...
              button.addEventListener(
                'click',
                () => window.codexcrew?.windowControl?.(action),
              );
              return button;
            };
            wrap.append(
  ...
  async function promptRemoteHost() {
    ...
    const html = `<!DOCTYPE html><html><head><style>
      ...
      <label>codexcrew binary path</label>
  ...
  async function openNewConnectionWindow() {
    ...
    const html = `<!DOCTYPE html><html><head><style>
      ...
      <div class="hint">Connect to a Codex Crew gateway running on another port</div>
  ...
  function renameCurrentWindow() {
    ...
    getDashboardThemeVars().then((vars) => {
      ...
      const html = `<!DOCTYPE html><html><head><style>
        ...
        <input id="n" value="${esc(currentTitle.replace(/^Codex ?Crew /g, ""))}" autofocus>
  ...
  function showScreenPermissionDialog() {
    ...
    dialog.showMessageBox({
      ...
      message: "Allow Codex Crew to capture the screen",
      detail:
        "The screen-snip tool needs macOS Screen Recording permission. "
        + "Open System Settings › Privacy & Security › Screen Recording, "
        + "enable Codex Crew, then try the snip again.",
    ...
    dialog.showMessageBox({
      ...
      detail: restricted
        ...
        : "Voice input needs macOS Microphone permission, and macOS will not "
          ...
          + "& Security › Microphone, enable Codex Crew, then try the mic again.",
