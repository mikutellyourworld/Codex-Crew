 ...
 *
 * The ONE CodexCrew-specific concern vs. a plain Electron app is unchanged: the
...
// The public 0.7 build keeps automatic updates disabled until signed release
// artifacts and metadata are published from the repository.
//
...
// artifact URLs may be absolute. `new URL(fileUrl, base)` ignores the base when
 ...
 *
 * Lookup order: the `CODEXCREW_EXTERNALLY_MANAGED` env var (a path to a marker
 * file, or any other non-empty value to mark the install managed with no
 * metadata — the test-harness seam, mirroring `CODEXCREW_UPDATE_FEED`, and
 ...
 * `CODEXCREW_MANAGED_INSTALL_MARKER` names one; read on a PACKAGED build only,
...
function readExternallyManaged({
  ...
  try {
    ...
    const override = (!isPackaged && env && env.CODEXCREW_EXTERNALLY_MANAGED) || "";
    if (override) {
...
const DEFAULT_FEED_BASE = "https://github.com/mikutellyourworld/Codex-Crew/releases/latest/download";
 ...
 */
// Public 0.7 does not claim an update lane until signed release artifacts exist.
...
const DOWNLOAD_BASE = "https://github.com/mikutellyourworld/Codex-Crew/releases/latest/download";
 ...
 * update harness (CODEXCREW_UPDATE_FEED=http://127.0.0.1:PORT/feed) works;
 ...
 * in the CodexCrew home directory, never inside the app bundle.
 *
...
function manualDownloadUrl(channel, osPlatform, osArch = process.arch, linuxFormat = "") {
  ...
  const linuxFile = linuxArch ? `CodexCrew-${linuxArch}.${linuxExt}` : null; // brand-ok
  ...
  const windowsFile = { x64: "CodexCrew-Setup.exe" }[osArch];
...
function initAutoUpdate(deps) {
  const {
    ...
    feedBase = process.env.CODEXCREW_UPDATE_FEED || DEFAULT_FEED_BASE,
    onUpdateState = null,
    log = console,
  } = deps;
  ...
  if (managed) {
    ...
    const managedEnv = () => {
      ...
      // CODEXCREW_MANAGED_ARGV0 is DERIVED, not inherited: process.execPath is
      ...
      const e = {
        PATH: managedPath(),
        PYTHONNOUSERSITE: "1",
        CODEXCREW_MANAGED_ARGV0: process.execPath,
      };
  ...
  function notifyWindowsInstallHandoff() {
    ...
    try {
      new Notification({
        title: "Installing Codex Crew update",
        ...
        body: "If Codex Crew doesn’t reopen after the installer finishes, open it from the Start menu.",
      }).show();
  ...
  async function promptInstall(versionName, notes) {
    const handoffDetail = osPlatform === "win32"
      ? "Installing can take several minutes. Codex Crew will close, show Windows installation progress, and reopen automatically."
      : "Installing can take several minutes. Codex Crew will close and reopen automatically when the update is complete.";
    const { response } = await dialog.showMessageBox({
      type: "info",
      buttons: ["Install Update & Restart App", "Later"],
      defaultId: 0,
      cancelId: 1,
      title: "Codex Crew update ready",
      message: `Codex Crew ${versionName || ""} is ready to install.`.trim(),
      detail:
        (notes || "").slice(0, 500) +
        `\n\n${handoffDetail}`,
    });
    if (response === 0) {
      ...
      try {
        new Notification({
          ...
          body: "Codex Crew will finish updating the next time you quit.",
        }).show();
      } catch { /* notifications optional */ }
    }
  }
