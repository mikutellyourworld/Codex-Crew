...
const RESTARTING_STATUS = "Restarting Codex Crew to finish the update…";
...
function createGatewaySupervisor({
  ...
  const CODEXCREW_HOME = home || resolveHome();
  ...
  async function resolveGatewayConflict(rebindDepth = 0) {
    ...
    const { response } = await dialog.showMessageBox({
      ...
      message: `${other.displayName} (${decision.otherVersion}) is already running with your Codex Crew data.`,
      detail: canTakeover
        ? `Only one Codex Crew app can use ~/.codex-crew at a time. Quit ${other.displayName} and continue here?`
        : `Only one Codex Crew app can use ~/.codex-crew at a time. Quit ${other.displayName}, then reopen this app.`,
  ...
  function startGateway() {
    reresolveAttempts = 0;
    glog(`launch: port=${PORT} home=${CODEXCREW_HOME} packaged=${app.isPackaged} resourcesPath=${processObj.resourcesPath || "(none)"} log=${gatewayLogPath()}`);
  ...
  function spawnGateway(resolve) {
    ...
    // bytecode cache, honoring an explicit CODEXCREW_HOME without touching the
    // deprecated legacy directory on a clean install.
    const codexcrewDir = processObj.env.CODEXCREW_HOME || canonicalHome();
    try {
      fs.mkdirSync(codexcrewDir, { recursive: true, mode: 0o700 });
    } catch (error) {
      glog(`WARN failed to create codexcrew dir ${codexcrewDir}: ${error.message}`);
    }
    ...
    // CODEXCREW_PORT would let the child re-derive a port which differs from the
    // shell URL and from the gateway's own frame-ancestor claim.
    const { CODEXCREW_PORT: _ignored, ...cleanEnv } = processObj.env;
    ...
    if (bin.endsWith("codexcrew.cmd")) {
      const pythonExe = path.resolve(path.dirname(bin), "..", "python.exe");
      if (fs.existsSync(pythonExe)) {
        spawnBin = pythonExe;
        spawnArgs = ["-s", "-m", "codex_crew", ...spawnArgs];
      } else {
    ...
    const child = spawn(spawnBin, spawnArgs, {
      ...
      env: buildGatewayEnvironment({
        ...
        CODEXCREW_NODE_EXECUTABLE: processObj.execPath,
        CODEXCREW_PROJECT_DIR: IS_WIN
          ? resolveProjectDir()
          : path.resolve(dirname, ".."),
        ...gatewayBytecodeEnvironment(
          processObj.platform,
          path.join(codexcrewDir, "cache", "pycache"),
          app.isPackaged,
        ),
      }),
    });
    ...
    child.on("exit", (code, signal) => {
      ...
      if (signal === "SIGKILL" && IS_MAC) {
        glog("HINT: SIGKILL on a freshly-spawned bundled binary almost always means macOS Gatekeeper blocked an unsigned/quarantined nested executable. On the recipient's Mac run: xattr -cr <path to CodexCrew.app>");
      }
      if (gatewayProcess !== child) return;
      const giveUp = () => {
  ...
  async function fetchLocalToken(targetBackendUrl = BACKEND_URL) {
    // Re-resolve the home at call time so a CODEXCREW_HOME change after Electron
  ...
  async function recoverWedgedGateway(window, { userInitiated = false } = {}) {
    ...
    if (strategy === "reconnect-bounded") {
      glog("liveness: backend unresponsive on an adopted local Codex Crew gateway — bounded wait, then respawn");
      if (!window || window.isDestroyed() || quitting()) return;
  ...
  async function showLoadingThenConnect(
    ...
    try {
      ...
      if (failureKind === "installing") {
        title = "Codex Crew — installation still finishing";
        message = error.failure?.incompleteBundle
          ? error.message
          : describeIncompleteBundle([]);
      } else if (localGatewayOff) {
        title = remoteTarget
          ? `Codex Crew — nothing answering at ${remoteTarget}:${remoteTargetPort}`
          : `Codex Crew — no gateway on port ${PORT}`;
        message = error.message;
      } else if (portConflict) {
        title = `Codex Crew — port ${PORT} already in use`;
        message = `Another Codex Crew gateway is already using port ${PORT} (it may be wedged). `
          + `Force-stop it and retry, or quit. From a terminal you can also run: `
          + `codexcrew stop --port ${PORT}`;
      } else if (failedToStart) {
        title = "Codex Crew — gateway failed to start";
        message = error.message;
      } else {
        title = "Codex Crew — can't reach the gateway";
        message = "Could not connect to the Codex Crew backend. Make sure "
          + "'codexcrew gateway' is running, or check codexcrew doctor.";
      }
