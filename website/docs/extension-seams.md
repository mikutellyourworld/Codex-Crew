...
body is one side-effect import of the `virtual:codexcrew-edition` module plus
...
- an **inert empty module** in the stock OSS build (`CODEXCREW_EDITION_DIR` unset),
  so the stock build registers nothing and is byte-identical to having no seam;
- the **edition's own** `$CODEXCREW_EDITION_DIR/extensions.tsx` (or `.ts`) when that
...
Resolution is eager, so a misconfigured `CODEXCREW_EDITION_DIR` (set but with no
...
Edition composition needs **two** env vars, not one. `CODEXCREW_EDITION_DIR` alone
throws: the plugin also requires `CODEXCREW_ALLOW_EDITION=1`.
...
stray or inherited `CODEXCREW_EDITION_DIR` fails the build instead of silently
...
`CODEXCREW_ALLOW_EDITION=1` in a release or publish job.
...
than synthesizing it**: forcing `CODEXCREW_ALLOW_EDITION=1` would defeat the
...
Both inputs are inert when absent, and the stock build (no `CODEXCREW_EDITION_DIR`)
...
```jsonc
{
  "extends": "../CodexCrew/website/tsconfig.app.json",
  "compilerOptions": {
    "noEmit": true,
...
`src/codex_crew/dashboard/handlers/source_providers.py`, under the same id. The
...
defaulting to `Codex Crew`, so the stock build renders unchanged text.
