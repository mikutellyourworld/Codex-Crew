...
`"formatVersion": 1` (integer). CodexCrew rejects packs with a missing value or
an unknown major with an explicit "this pack requires a newer version of
CodexCrew" error. Author against the current major; breaking manifest changes
bump it.
...
- Backend: `_THEME_CSS_VARS_SET` (built from `_THEME_CSS_VARS`) in
  `src/codex_crew/dashboard/theme_validate.py`
...
falls back independently to its configured Codex Crew branding. Compiled edition
...
| Option | Resolves to |
|---|---|
| Sans | the pack's `sans` face, else Codex Crew's own proportional stack |
| Mono | the pack's `mono` face, else Codex Crew's own monospace stack |
...
`_THEME_CSS_FORBIDDEN` in `src/codex_crew/dashboard/theme_validate.py` rejects a
