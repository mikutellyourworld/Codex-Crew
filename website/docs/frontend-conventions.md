...
`src/codexcrew-ui/index.ts` re-exports the subset that apps may import as
`@codexcrew/ui`. Adding a primitive there makes it app-facing API, so add
deliberately.
...
| `instances` | the known Codex Crew instances a user can switch between |
