...
`interpolation.defaultVariables` with the stock value `Codex Crew`, so the stock
...
- **Attribution and data-egress copy keeps the literal too.** A string whose
  ...
  `app.star_codexcrew_on_github` wraps a hardcoded upstream repo URL, and the
...
- **Wire-format identifiers keep their unspaced literal.** The generated
  Slack app name (`CodexCrew-{{alias}}`) and the webhook signature headers
  (`X-CodexCrew-Timestamp` / `X-CodexCrew-Signature`) are fixed by the backend,
...
`src/codex_crew/apps/builtins/<app>/app.json` on the **Python** side, and the App Store
...
*replaces* the English. `codexcrew app list` prints `displayName` straight to a terminal
