 ...
 * composed through the `CODEXCREW_EDITION_DIR` seam (see `editionExtensionPlugin`
 ...
 * no build error. Gated on the same `CODEXCREW_ALLOW_EDITION=1` opt-in as the
...
if (process.env.CODEXCREW_EDITION_DIR && process.env.CODEXCREW_ALLOW_EDITION === '1') {
  content.push(`${process.env.CODEXCREW_EDITION_DIR}/**/*.{ts,tsx}`)
}
