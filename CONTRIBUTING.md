# Contributing to Codex Crew

Thanks for your interest in contributing! Codex Crew is an open-source project and
we welcome issues and pull requests.

## Reporting Bugs and Requesting Features

Open a [GitHub issue](https://github.com/mikutellyourworld/Codex-Crew/issues). Before you
...
your operating system, and anything unusual about how Codex Crew is installed or
...
Start with the [open issues](https://github.com/mikutellyourworld/Codex-Crew/issues). Issues
...
npm install
npm run build
cp -r dist ../src/codex_crew/static/dist
...
codexcrew setup               # data dir, agent backend (channels connect later)
codexcrew doctor              # verify everything works
codexcrew gateway             # start server (dashboard + messaging channels)
```

The dashboard is at `http://localhost:5486`.
...
**Messaging channels are optional**: the default `codexcrew setup` configures
...
`codexcrew setup --slack` for the guided Slack path.

## Development Skills (agents and humans)

The contributor workflow is codified as agent-loadable skills in
[`src/codex_crew/builtin_skills/codexcrew-dev/`](src/codex_crew/builtin_skills/codexcrew-dev/)
— the canonical definition of how code gets written, tested, and reviewed here:

- **`codexcrew-worktree-dev`** — the HARD RULE workflow: every change in a git
...
An agent contributing to Codex Crew loads this suite and follows the same
...
`src/codex_crew/static/dist/` and served by the backend.
...
After building, copy `website/dist` into `src/codex_crew/static/dist/` so the
...
CODEXCREW_HOME=.codexcrew-dev CODEXCREW_PORT=6777 codexcrew gateway
```
...
|---------|---------|---------|
| `CODEXCREW_HOME` | Config/data directory override | `~/.codex-crew` |
| `CODEXCREW_PORT` | Dashboard port override | `5486` |
| `CODEXCREW_KIRO_BIN` | Explicit path to the `kiro-cli` binary (overrides PATH auto-detection) | auto-detected |

If you don't need to run production and dev side by side, omit `CODEXCREW_PORT` —
...
| File | Field |
|------|-------|
| `src/codex_crew/__init__.py` | `__version__` — the source of truth |
...
| File | Purpose |
|------|---------|
| `src/codex_crew/cli.py` | CLI entrypoint (argparse) |
| `src/codex_crew/session.py` | Conversation session management |
| `src/codex_crew/providers/` | LLM provider layer. ACP only — `agent.provider` is fixed to `acp` |
| `src/codex_crew/acp/client.py` | ACP JSON-RPC client (stdio) |
| `src/codex_crew/slack/gateway.py` | Slack Socket Mode gateway |
| `src/codex_crew/slack/handler.py` | Message handling, tool approval |
| `src/codex_crew/dashboard/` | Web dashboard (aiohttp backend) |
| `src/codex_crew/mcp_core.py` | MCP tools: spawn, learn, task, wait, hook, send_message, file_send |
| `src/codex_crew/mcp_cron.py` | MCP tools: cron scheduling |
| `src/codex_crew/context.py` | Context builder (memory, skills, history) |
| `src/codex_crew/subagent.py` | Subagent lifecycle and timeout |
| `src/codex_crew/autonudge.py` | Reactive same-session self-nudge service |
| `src/codex_crew/snapshot.py` | Portable snapshot and restore |
| `src/codex_crew/apps/` | App Kit platform (manifest, manager, registry, routes) |
| `src/codex_crew/eval/` | Multi-session eval harness |
...
`src/codex_crew/docs/` filenames-are-an-API caveat are in
[The rule for changing docs](docs/README.md#the-rule-for-changing-docs).

## Extending Codex Crew

- **Skills** — drop markdown files in `skills/` or `~/.codex-crew/skills/`. See [skills/README.md](skills/README.md) for the full format reference
- **MCP tools** — add to `mcp_core.py` or `mcp_cron.py`. Every LLM-facing command must have an MCP tool
- **Hooks** — configure in `~/.codex-crew/config.json`
- **Lessons** — self-learned from corrections, stored in `~/.codex-crew/lessons.jsonl`
...
taste. See Development Skills above: `codexcrew-worktree-dev` covers building and
...
(`src/codex_crew/builtin_skills/codexcrew-dev/babysit/SKILL.md`) only tells one
...
Open a [GitHub issue](https://github.com/mikutellyourworld/Codex-Crew/issues) or start a
...
Codex Crew is licensed under the Apache License 2.0. See [LICENSE](LICENSE) for the
