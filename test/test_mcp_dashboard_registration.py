"""Managed-MCP registration for ``codexcrew-dashboard``, and why it is its own server.

The dashboard-control tools are deliberately NOT in ``codexcrew-core``. Core is the
...
* **The default agent's spec does not carry the server**, in ``mcpServers`` or as
  an ``@codexcrew-dashboard`` ref in ``tools``. kiro-cli loads a server only when
...
from codex_crew import agent, mcp_cleanup, mcp_discovery, onboarding_import

DASH_SERVER = "codexcrew-dashboard"
...
class TestRegistryParity:
    def test_named_in_every_managed_registry(self) -> None:
        ...
        assert DASH_SERVER in mcp_cleanup.CODEXCREW_BIN_MCP_SERVERS
...
class TestDoctorTreatsItAsAssignedNotMissing:
    """`codexcrew doctor` must not undo the assignment, in either direction."""
    ...
    def test_a_half_grant_is_reported_not_repaired(self, tmp_path: Any, capsys: Any) -> None:
        ...
        spec = {
            "mcpServers": {
                n: {"command": "/usr/local/bin/codexcrew", "args": [f"mcp-{n.split('-', 1)[1]}"]}
                for n in mcp_cleanup.CODEXCREW_BIN_MCP_SERVERS
            },
    ...
    def test_a_ref_without_an_entry_is_also_reported(self, tmp_path: Any, capsys: Any) -> None:
        ...
        spec = {
            ...
            "tools": [f"@{n}" for n in mcp_cleanup.CODEXCREW_BIN_MCP_SERVERS],
            "allowedTools": [],
        }
        spec_path.write_text(json.dumps(spec), encoding="utf-8")
        issues: list[str] = []
...
class TestTheDefaultAgentIsNotGrantedTheSet:
    ...
    def test_the_always_on_servers_are_still_granted(self) -> None:
        ...
        assert "codexcrew-core" in mcp
        assert "codexcrew-cron" in mcp
            ...
            json.dumps(
                {
                    "mcpServers": {
                        **{
                            n: {
                                "command": "/usr/local/bin/codexcrew",
...
class TestTheNameAloneIsNotOwnership:
    """A global entry under an opt-in name is never Codex Crew's to delete.

    ``clean_stale_managed_mcp`` reclaims entries an OLDER INSTALL METHOD wrote
    to the user's global ``mcp.json``. No version of Codex Crew ever writes an
