...
from codex_crew import agent_backend_governance as abg
...
from codex_crew.platform.governance import SCOPE_CATALOG, parse_policy
...
class TestScopeRegistration:
    ...
    def test_codex_is_the_governance_floor(self):
        assert FLOOR == "codex"
        assert acp_backends.POLICY_ID_BY_BACKEND[FLOOR] == "codex"
...
class TestRegistryRecompute:
    ...
    def test_registering_writes_the_baseline_too(self, monkeypatch):
        ...
        acp_backends.register_selectable_backend("openai_compatible")
        assert "openai_compatible" in acp_backends.registered_backends()
        assert "openai_compatible" in acp_backends.selectable_backends()
...
class TestAudit:
    ...
    def test_an_audit_failure_never_breaks_the_narrowing(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.sel.sel", _boom)
        assert abg.narrow_selectable_backends() == ["kas"]
...
class TestWhenAPolicyChangeBinds:
    ...
    cannot: its single gate runs inside ``CodexCrewConfig.load()`` and reading a ceiling
...
class TestNoSecondGate:
    ...
    def test_the_provider_factory_path_gained_no_governance_call(self):
        ...
        src = inspect.getsource(CodexCrewConfig.create_provider_factory)
    ...
    def test_the_config_load_path_never_reads_the_platform_context(self):
        # H3: the one gate runs inside CodexCrewConfig.load(); a ceiling read there
        ...
        assert not [m for m in imported if m.startswith("codex_crew.platform")]
        called = {
            node.func.id
            for node in ast.walk(tree)
