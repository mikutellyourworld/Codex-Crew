"""Tests for the extracted ``codex_crew.config.validation`` module.
...
from codex_crew.config import loader as _loader_module
...
class TestValidateConfigData:
    ...
    def test_save_stamped_meta_key_is_not_reported_as_unrecognized(
        ...
        # ``meta`` is written by CodexCrewConfig.save() itself, so warning about
        # it means every launch scolds the user for CodexCrew's own bookkeeping —
        # on stdout of `codexcrew token`, whose URL the user has to copy.
        data = {
    ...
    def test_warning_logger_name_is_loader_not_validation(self) -> None:
        ...
        assert validation.logger.name == "codex_crew.config.loader"
...
class TestLoaderBackCompatReexport:
    """The C3 symbols remain importable from ``codex_crew.config.loader``."""
