...
Per-test config isolation comes from the autouse CODEXCREW_HOME fixture in
...
def test_defaults_to_off() -> None:
    ...
    assert CodexCrewConfig.load().stt.endpointing is False
...
def test_explicit_true_is_honoured() -> None:
    ...
    assert CodexCrewConfig.load().stt.endpointing is True
...
def test_non_bool_in_config_file_falls_back_to_off() -> None:
    ...
    for bogus in ("true", "false", 1, 0, None, [], {}):
        ...
        loaded = CodexCrewConfig.load().stt.endpointing
...
async def test_complete_verdict_emits_endpoint_frame(monkeypatch) -> None:
    monkeypatch.setattr(
        "codex_crew.dashboard.stt_stream.run_bg_oneliner",
...
async def test_newer_final_supersedes_the_debounced_one(monkeypatch) -> None:
    ...
    monkeypatch.setattr("codex_crew.dashboard.stt_stream.run_bg_oneliner", bg)
    ws = _fake_ws()
...
async def test_single_flight_collision_reruns_not_drops(monkeypatch) -> None:
    ...
    monkeypatch.setattr("codex_crew.dashboard.stt_stream.run_bg_oneliner", fake)
    ws = _fake_ws()
    ep = stt_stream._Endpointer(ws, object(), debounce=0.0, timeout=1.0)
