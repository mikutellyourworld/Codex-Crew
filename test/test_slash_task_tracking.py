...
from codex_crew.slack.events import _handle_slash, register_slash_command
...
def _make_orch() -> MagicMock:
    ...
    orch.slack_command = "codexcrew"  # _handle_slash compares cmd to f"/{slack_command}"
    return orch
...
async def test_slash_handler_task_is_retained(tmp_path, monkeypatch) -> None:
    ...
    try:
        ...
        orch = _make_orch()
        await _handle_slash(orch, {"command": "/codexcrew", "user_id": "U1", "text": "zzztest do a thing", "response_url": "https://example.test/r"})
...
async def test_unauthorized_reply_task_is_retained(tmp_path, monkeypatch) -> None:
    ...
    events._bg_tasks.clear()
    orch = _make_orch()
    await _handle_slash(orch, {"command": "/codexcrew", "user_id": "U_BAD", "text": "status", "response_url": "https://example.test/r"})
...
async def test_owner_not_configured_reply_retained(tmp_path, monkeypatch) -> None:
    ...
    events._bg_tasks.clear()
    await _handle_slash(orch, {"command": "/codexcrew", "user_id": "U1", "text": "status", "response_url": "https://example.test/r"})
    assert len(events._bg_tasks) >= 1
    await asyncio.gather(*list(events._bg_tasks), return_exceptions=True)
    await asyncio.sleep(0)
    events._bg_tasks.clear()
...
async def test_unknown_subcommand_help_reply_retained(tmp_path, monkeypatch) -> None:
    ...
    orch = _make_orch()
    events._bg_tasks.clear()
    await _handle_slash(orch, {"command": "/codexcrew", "user_id": "U1", "text": "no_such_subcmd_xyz", "response_url": "https://example.test/r"})
    assert len(events._bg_tasks) >= 1
    await asyncio.gather(*list(events._bg_tasks), return_exceptions=True)
    await asyncio.sleep(0)
    events._bg_tasks.clear()
...
async def test_user_mention_disabled_reply_retained(tmp_path, monkeypatch) -> None:
    ...
    await _handle_slash(
        orch,
        {"command": "/codexcrew", "user_id": "U1", "text": "<@U99999>", "response_url": "https://example.test/r"},
    )
    assert len(events._bg_tasks) >= 1
    await asyncio.gather(*list(events._bg_tasks), return_exceptions=True)
    await asyncio.sleep(0)
...
async def test_channel_mention_track_tasks_retained(tmp_path, monkeypatch) -> None:
    ...
    await _handle_slash(
        orch,
        {"command": "/codexcrew", "user_id": "U1", "text": "<#C12345|general>", "response_url": "https://example.test/r"},
    )
...
async def test_raising_handler_task_still_self_cleans(tmp_path, monkeypatch) -> None:
    ...
    try:
        ...
        await _handle_slash(
            orch,
            {"command": "/codexcrew", "user_id": "U1", "text": "zzzboom", "response_url": "https://example.test/r"},
        )
