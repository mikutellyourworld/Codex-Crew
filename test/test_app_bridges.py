"""Tests for codex_crew.apps.bridges — resource registration bridges."""
...
from codex_crew.apps.manager import APP_MANIFEST_FILENAME, install_app
...
def app_env(tmp_path, monkeypatch):
    """Set up isolated CODEXCREW_HOME and KIRO agents dir."""
    ...
    import codex_crew.apps.bridges as bridges_mod
    import codex_crew.apps.execution as execution_mod
...
class TestAgentRegistration:
    ...
    def test_a_dangling_at_grant_logs_a_warning_and_still_registers(
        ...
        broken agent.
        """
        from codex_crew.apps import bridges as bridges_mod
        ...
        with caplog.at_level("WARNING", logger="codex_crew.apps.bridges"):
            registered = _register_agents("test-app", manifest, app_root)
    ...
    def test_a_legacy_symlink_is_still_replaced(self, tmp_path, app_env):
        """A symlink from an older CodexCrew is dropped and replaced with a real file."""
        src = _make_app_source(tmp_path)
        install_app(src)
        manifest = AppManifest.from_json_file(
            app_env["home"] / "apps" / "test-app" / APP_MANIFEST_FILENAME
    ...
    def test_register_mcp_strips_a_governed_autoapprove(self, tmp_path, app_env, monkeypatch):
        ...
        from codex_crew.apps import bridges as bridges_mod
        from codex_crew.platform import governance as gov
    ...
    def test_agent_name_with_path_separator_is_refused(self, tmp_path, app_env):
        ...
        # outside the agents dir (e.g. ~/.codex-crew/config.json).
...
class TestSkillRegistration:
    def test_register_skills(self, tmp_path, app_env):
        ...
        # A link exists under ~/.codex-crew/skills/test-app/my-skill: a symlink on
...
class TestCronRegistration:
    ...
    def test_a_non_list_manifest_does_not_break_registration(self, tmp_path, app_env):
        ...
        from codex_crew.apps.bridges import register_app_crons_with_service
        ...
        with patch("codex_crew.apps.bridges.CronSDK", return_value=mock_sdk):
...
class TestTopLevel:
    ...
    def test_register_app_resources_app_skips_all(self, tmp_path, app_env, monkeypatch):
        ...
        """
        import codex_crew.apps.bridges as bmod
        ...
        from codex_crew.apps.manager import register_external_app
...
class TestMCPRegistration:
    ...
    def test_reregister_app_mcp_servers_overwrites_with_live_port(
        ...
        import codex_crew.apps.backend as backend_mod
        import codex_crew.apps.bridges as bmod
        from codex_crew.apps.bridges import reregister_app_mcp_servers
...
class TestStdioInterpreterResolution:
    ...
    def test_a_gateway_module_server_is_pinned_to_the_gateway_interpreter(
        ...
        # A stdio server that runs Codex Crew's OWN code (`-m codex_crew...`) must
        # run under the gateway's interpreter even when the app has a venv: app
        # venvs are created without --system-site-packages, so codex_crew is not
        ...
        entry = self._register_stdio(
            tmp_path, app_env, monkeypatch,
            {"command": "python3", "args": ["-s", "-m", "codex_crew.apps.builtins.x.server"]},
            setup=_fake_venv_python,
        )
        assert entry["command"] == sys.executable
    ...
    def test_a_script_argument_named_dash_m_does_not_trigger_the_gateway_pin(
        ...
        # `python3 server.py -m codex_crew.mode`: the -m belongs to the SCRIPT
        ...
        entry = self._register_stdio(
            tmp_path, app_env, monkeypatch,
            {"command": "python3", "args": ["server.py", "-m", "codex_crew.mode"]},
            setup=lambda root: created.append(_fake_venv_python(root)),
        )
        assert entry["command"] == str(created[0])
    ...
    def test_interpreter_options_before_dash_m_still_trigger_the_pin(
        ...
        entry = self._register_stdio(
            tmp_path, app_env, monkeypatch,
            {"command": "python3", "args": ["-s", "-u", "-m", "codex_crew.apps.x"]},
            setup=_fake_venv_python,
        )
        assert entry["command"] == sys.executable
    ...
    def test_separate_value_options_do_not_break_the_pin_scan(
        ...
        entry = self._register_stdio(
            tmp_path, app_env, monkeypatch,
            {"command": "python3",
             "args": ["-X", "dev", "-W", "ignore", "-m", "codex_crew.apps.x"]},
            setup=_fake_venv_python,
        )
        assert entry["command"] == sys.executable
    ...
    def test_an_attached_module_spelling_triggers_the_pin(
        ...
        entry = self._register_stdio(
            tmp_path, app_env, monkeypatch,
            {"command": "python3", "args": ["-mcodex_crew.apps.x"]},
            setup=_fake_venv_python,
        )
        assert entry["command"] == sys.executable
    ...
    def test_a_dash_c_program_never_triggers_the_pin(
        ...
        entry = self._register_stdio(
            tmp_path, app_env, monkeypatch,
            {"command": "python3",
             "args": ["-cimport server", "-m", "codex_crew.x"]},
            setup=lambda root: created.append(_fake_venv_python(root)),
        )
        assert entry["command"] == str(created[0])
    ...
    def test_the_host_cli_pin_still_wins(self, tmp_path, app_env, monkeypatch):
        ...
        # `codexcrew` is pinned to `sys.executable -m codex_crew` by
        # _pin_host_cli_command BEFORE stdio resolution; the venv must not
        # override that (the host CLI is gateway code, not app code).
        entry = self._register_stdio(
            tmp_path, app_env, monkeypatch,
            {"command": "codexcrew", "args": ["app", "mcp", "test-app"]},
            setup=_fake_venv_python,
        )
        assert entry["command"] == sys.executable
        assert entry["args"][:3] == ["-s", "-m", "codex_crew"]
...
class TestBackendSharesTheInterpreterPolicy:
    ...
    def test_venv_present_resolves_to_the_venv_interpreter(self, tmp_path):
        from codex_crew.apps.interpreter import resolve_app_python
...
class TestCronServiceBridge:
    ...
    def _write_app_crons(self, tmp_path, app_name, cron_defs):
        ...
        app_dir = tmp_path / "codexcrew-home" / "apps" / app_name
        ...
        cron_defs = [
            {
                ...
                "agent": "",
                "message": "",
                "command": "python3 ~/.codexcrew/apps/test-app/scripts/collect.py",
                "script": "",
                "app": "test-app",
                "agent_sequence": [],
                "env": {},
    ...
    def test_rejects_invalid_script_path(self, tmp_path, app_env, monkeypatch):
        """Scripts outside ~/.codexcrew/crons/ are rejected at registration."""
        from unittest.mock import MagicMock, patch
    ...
    def test_register_crons_serializes_all_fields(self, tmp_path, app_env):
        ...
        from codex_crew.apps.bridges import _register_crons, load_app_cron_defs
        ...
        from codex_crew.apps.manifest import CronEntry
...
class TestCronServiceDeregister:
    ...
    def test_returns_zero_when_no_cron_service(self, tmp_path, app_env):
        from codex_crew.apps.bridges import deregister_app_crons_from_service
...
class TestBuiltinAgentNamesAreNamespaced:
    ...
    def _builtin_dirs(self):
        ...
        import codex_crew.apps.builtins as builtins_pkg
...
class TestUserAgentEditsSurviveRefresh:
    ...
    def test_user_keys_win_and_owned_keys_are_refreshed(self) -> None:
        from codex_crew.apps.bridges import _preserve_user_agent_edits
    ...
    def test_a_corrupt_prior_file_does_not_fail_the_refresh(self, tmp_path) -> None:
        ...
        preserve", not "abort"."""
        from codex_crew.apps.bridges import _read_agent_config
    ...
    def test_the_owned_key_set_names_every_field_the_framework_derives(self) -> None:
        ...
        """
        from codex_crew.apps.bridges import _FRAMEWORK_OWNED_AGENT_KEYS
    ...
    def test_every_template_key_is_a_decided_key(self) -> None:
        ...
        root = _REPO_ROOT / "src/codex_crew/apps/builtins"
...
class TestBuiltinDeclaredResourcesActuallyRegister:
    ...
    def test_builtin_dict_carries_every_declarative_manifest_field(self):
        ...
        from codex_crew.apps.discovery import _manifest_to_builtin_dict
        from codex_crew.apps.manifest import AppManifest
    ...
    def test_resource_root_for_builtin_is_the_package_dir(self, app_env):
        ...
        from codex_crew.apps.bridges import _app_resource_root
        from codex_crew.apps.discovery import _get_builtins_dir
        from codex_crew.apps.manager import get_app, register_builtin_apps
...
class TestAppEventBusIsActuallyWired:
    ...
    def test_dashboard_state_exposes_the_broadcast_method_the_gateway_passes(self):
        ...
        from codex_crew.dashboard import server as server_mod
    ...
    def test_context_has_no_event_bus_without_a_broadcast_fn(self, tmp_path):
        from codex_crew.apps.context import build_app_context
...
class TestAppPromptPathIsContained:
    ...
    def test_a_path_outside_the_app_is_dropped(self, tmp_path, monkeypatch):
        from codex_crew.apps import bridges, manager
...
class TestRebuildPreservesTheLiveMcpSpec:
    ...
    def test_live_registered_spec_wins_over_the_manifest(self, monkeypatch):
        ...
        import codex_crew.apps.bridges as bridges
        import codex_crew.apps.manager as manager
...
class TestMcpEnableHandlersOffloadTheSync:
    ...
    def test_handlers_offload_sync_to_agent(self) -> None:
        ...
        src = (_REPO_ROOT / "src/codex_crew/dashboard/handlers/mcp.py").read_text(encoding="utf-8")
...
class TestRegisterPrunesUpgradedAwayResources:
    ...
    def test_stale_app_agent_and_server_are_pruned(self, tmp_path, app_env) -> None:
        ...
        from codex_crew.apps.manager import enable_app
...
class TestMalformedConfigIsNotClobbered:
    """A read-modify-write of an EXISTING-but-unreadable codexcrew.json must
    ...
    def test_strict_read_raises_on_malformed_existing(self, tmp_path, monkeypatch):
        ...
        mcp_path = tmp_path / "codexcrew.json"
        monkeypatch.setattr(bmod, "_mcp_json_path", lambda: mcp_path)
...
class TestShippedAgentTemplatesAreRenderedByTheGateway:
    ...
    def test_placeholders_resolve_to_gateway_computed_values(self, tmp_path, app_env):
        from codex_crew.apps.bridges import _placeholder_values
    ...
    def test_a_template_is_rendered_into_the_data_home(self, tmp_path, app_env):
        from codex_crew.apps.bridges import _render_shipped_agent
...
class TestRefreshAppAgentsReportsIoFailures:
    ...
    def test_an_io_failure_is_collected(self, monkeypatch, tmp_path):
        import codex_crew.apps.bridges as brmod
...
class TestDemotionKeepsBackendIndependentServers:
    ...
    def test_the_unhealthy_reconcile_scrubs_http_and_keeps_stdio(
        ...
        import codex_crew.apps.backend as bmod
        import codex_crew.apps.bridges as brmod
