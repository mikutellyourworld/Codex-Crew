...
from codex_crew import resource_status
from codex_crew.dashboard import cautious_boot
from codex_crew.dashboard.cautious_boot import (
    MAX_DELAY_SECS,
    MILD_DELAY_SECS,
...
from codex_crew.dashboard.crash_dump_store import DUMP_PREFIX, DUMP_SUFFIX
...
class _Cfg:
    """Minimal stand-in for CodexCrewConfig — only what _evaluate reads."""
...
def _create_stacked_dump(dumps_dir: Path, *, age_secs: float = 0.0) -> Path:
    ...
    p.write_text(
        "# Codex Crew loop-stall crash dump — opened 20260810T010000Z\n"
        "# PID: 12345\n"
...
def _create_header_only_dump(dumps_dir: Path) -> Path:
    ...
    p.write_text(
        "# Codex Crew loop-stall crash dump — opened 20260810T020000Z\n"
        "# PID: 12345\n"
        "# If thread stacks appear below, the event loop wedged and faulthandler fired.\n"
        "\n"
    )
...
class TestConfigKey:
    ...
    def test_loader_rejects_non_bool(self):
        from codex_crew.config.loader import _safe_bool
