"""Coverage tests for ``codex_crew.dashboard.handlers.files``.
...
from codex_crew.dashboard.handlers import files as files_mod
...
def mock_sel():
    ...
    with patch("codex_crew.dashboard.handlers.files._sel") as m:
        instance = MagicMock()
...
class TestFileRaw:
    ...
    async def test_forbidden_path_is_400(self, mock_sel):
        with patch("codex_crew.dashboard.handlers._validate_dashboard_path", return_value=None):
            async with TestClient(TestServer(self._client_app())) as client:
    ...
    async def test_sensitive_path_is_403(self, tmp_path, mock_sel):
        ...
        with patch("codex_crew.dashboard.handlers.files.is_sensitive_path", return_value=True):
            async with TestClient(TestServer(self._client_app())) as client:
                resp = await client.get(f"/api/file-raw?path={f}")
                assert resp.status == 403
    ...
    async def test_symlink_is_refused_by_o_nofollow(self, tmp_path, mock_sel):
        ...
        with patch(
            "codex_crew.dashboard.handlers._validate_dashboard_path", return_value=str(link)
        ):
            async with TestClient(TestServer(self._client_app())) as client:
...
def outbox(tmp_path):
    ...
    odir = base / "outbox"
    odir.mkdir()
    with patch("codex_crew.config.loader.outbox_dir", return_value=odir):
        try:
            yield odir
        finally:
...
class TestOutboxDownload:
    ...
    async def test_oversize_file_is_413(self, outbox, mock_sel):
        ...
        with patch(
            "codex_crew.hooks.safe_read_file_bytes",
    ...
    async def test_unreadable_file_is_403(self, outbox, mock_sel):
        ...
        with patch("codex_crew.hooks.safe_read_file_bytes", return_value=None):
...
class TestRevealPath:
    ...
    async def test_open_action_hands_the_file_itself_to_the_launcher(
        ...
        with patch("sys.platform", "linux"), \
             patch("codex_crew.dashboard.handlers.files.platform_compat.open_with_default_app", return_value=True) as launch:
            async with TestClient(TestServer(self._client_app())) as client:
                resp = await client.post(
    ...
    async def test_reveal_hands_the_file_itself_to_the_helper(
        ...
        with patch("sys.platform", "darwin"), \
             patch("codex_crew.dashboard.handlers.files.platform_compat.reveal_in_file_manager", return_value=True) as reveal:
            async with TestClient(TestServer(self._client_app())) as client:
    ...
    async def test_a_launcher_that_does_not_start_falls_back_to_the_clipboard(
        ...
        for platform, action in (("darwin", "reveal"), ("linux", "reveal"),
            ...
            with patch("sys.platform", platform), \
                 patch("codex_crew.dashboard.handlers.files.platform_compat.reveal_in_file_manager", return_value=False), \
                 patch("codex_crew.dashboard.handlers.files.platform_compat.open_with_default_app", return_value=False):
                async with TestClient(TestServer(self._client_app())) as client:
                    resp = await client.post(
...
def cfg_file(tmp_path):
    ...
    with patch("codex_crew.config.loader.config_path", return_value=p):
        yield p
...
class TestDashboardConfigPut:
    ...
    async def test_cancellation_mid_load_is_still_audited(self, mock_sel):
        ...
        for method, tool in (("PUT", "dashboard_config_write"),
            ...
            with patch(
                "codex_crew.dashboard.handlers.source_providers."
