...
import codex_crew.dashboard.handlers._shared as _shared
...
def _isolate_config():
    # Warm the platform context BEFORE patching CodexCrewConfig.load to raise —
    ...
    current_context()
    with patch.object(_shared.CodexCrewConfig, "load", side_effect=_no_extra_paths):
        yield
...
def test_resolve_skill_root_resolves_edition_nested_key(tmp_path, monkeypatch):
    ...
    empty_codexcrew = tmp_path / "codexcrew_skills"
    empty_codexcrew.mkdir()
    monkeypatch.setattr(_shared, "skills_dir", lambda: empty_codexcrew)
...
def test_resolve_skill_root_still_prefers_codexcrew_root(tmp_path, monkeypatch):
    mc_root = tmp_path / "codexcrew_skills"
...
def test_resolve_skill_root_finds_skill_in_extra_paths(tmp_path, monkeypatch):
    ...
    empty_mc = tmp_path / "codexcrew_skills"
    empty_mc.mkdir()
    ...
    with patch.object(_shared.CodexCrewConfig, "load", return_value=_FakeConfig()):
...
def test_resolve_skill_root_extra_paths_take_precedence_over_edition(tmp_path, monkeypatch):
    ...
    # (codexcrew -> extra_paths -> edition roots).
    extra_root = tmp_path / "extra_skills"
...
def test_package_row_matched_by_exact_key():
    from codex_crew.dashboard.handlers.prompts import _match_package_row
...
def test_ambiguous_leaf_name_refuses_rather_than_serving_the_wrong_file(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="codex_crew.dashboard.handlers.prompts"):
...
def test_same_relative_path_in_two_roots_refuses_to_guess(tmp_path, monkeypatch, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="codex_crew.dashboard.handlers._shared"):
...
def test_symlink_loop_root_does_not_break_key_enumeration(tmp_path, monkeypatch):
    ...
    """
    data_home = tmp_path / "codexcrew_skills"
    data_home.mkdir()
