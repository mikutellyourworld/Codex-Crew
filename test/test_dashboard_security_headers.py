...
from codex_crew.dashboard.server import _apply_security_headers
...
class TestApplySecurityHeaders:
    ...
    def test_frame_ancestors_trusts_token_embed_parent(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.server.token_embed_parent_port",
            lambda token: 5486 if token else None,
        )
        ...
        assert "http://localhost:5486" in frame_anc
        assert "http://127.0.0.1:5486" in frame_anc
    ...
    def test_frame_ancestors_reads_embed_parent_from_session_cookie(self, monkeypatch) -> None:
        ...
        from codex_crew.dashboard.state import _DEFAULT_PORT
        ...
        def _fake_reader(token: str) -> int | None:
            seen["token"] = token
            return 5486 if token else None

        monkeypatch.setattr("codex_crew.dashboard.server.token_embed_parent_port", _fake_reader)
    ...
    def test_frame_ancestors_prefers_request_stashed_claim(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.server.token_embed_parent_port", lambda token: None
        )
        request = make_mocked_request("GET", "/?token=revoked-link-token")
        request["embed_parent_port"] = "5486"  # what the middleware stashed
        resp = _make_response()
        _apply_security_headers(resp, _make_app(with_instances=True), request=request)
        csp = resp.headers["Content-Security-Policy"]
        frame_anc = next(d for d in csp.split(";") if d.strip().startswith("frame-ancestors"))
    ...
    def test_shell_csp_ancestors_go_through_the_same_validator(self, monkeypatch) -> None:
        ...
        """
        monkeypatch.setattr(
            "codex_crew.dashboard.server._extra_frame_ancestors",
            lambda request, app: ["http://[::1]:5486", "http://localhost:5486"],
        )
        request = make_mocked_request("GET", "/")
        resp = _make_response()
        _apply_security_headers(resp, _make_app(with_instances=True), request=request)
