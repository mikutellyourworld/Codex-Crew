...
from codex_crew.snapshot import (
    COMPONENTS,
    ComponentRefused,
    Purpose,
    SecretPolicy,
...
def _snapshot(out: Path, extra: list[str]) -> Path:
    ...
    tars = sorted(out.glob("codexcrew-snapshot-*.tar.gz"), key=lambda p: p.stat().st_mtime)
    assert tars
    return tars[-1]
...
def _extract(tarball: Path, into: Path) -> Path:
    ...
    # A selective bundle's root is `codexcrew-partial-`, which is how released
...
class TestPolicyDeclarations:
    ...
    def test_component_spec_cannot_be_built_without_a_policy(self):
        from codex_crew.snapshot import ComponentSpec
...
class TestSelectiveStaging:
    ...
    def test_a_share_bundle_refuses_crons(self, src, tmp_path, capsys):
        ...
        assert not list(out.glob("codexcrew-snapshot-*.tar.gz"))
