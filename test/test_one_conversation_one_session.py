...
from codex_crew.history import ConversationLog, _safe_key
...
DISCORD_KEY = "discord:codexcrew:direct:123456"
DISCORD_STEM = "discord_codexcrew_direct_123456"
...
def log(tmp_path, monkeypatch):
    monkeypatch.setattr("codex_crew.history.config_dir", lambda: tmp_path)
...
def _real_get_or_create(st):
    ...
    from codex_crew.dashboard.state import _ChatSlot, _normalize_slot_key
...
class TestA4TheTabStaysCurrent:
    ...
    def test_surfacing_publishes_the_tab_to_the_surface_registry(self, state, log, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_utils.set_dashboard_surfaced",
...
class TestA6NoLostOrOutOfOrderTurn:
    ...
    def test_a_busy_session_is_reported_busy_from_either_spelling(self):
        ...
        from codex_crew.session import SessionManager, _Session
...
class TestForeignAppendsInterleaveChronologically:
    ...
    def test_a_channel_line_from_between_two_window_turns_lands_between_them(self):
        from codex_crew.dashboard.chat_persistence import _interleave_foreign_lines
