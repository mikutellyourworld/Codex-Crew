...
* :mod:`codex_crew.peer_resolve` — the shared /proc ancestry walk (extracted
...
from codex_crew.mcp_gateway.socketsec import PeerCredResult
...
def test_signed_only_refuses_forged_unsigned_mapping(
    ...
    from codex_crew import session_pid_sig as sps
...
def _unix_http_request(
    ...
    s.connect(sock_path)
    try:
        lines = [f"GET {path} HTTP/1.1", "Host: localhost:5486", "Connection: close"]
...
def test_check_origin_trusts_unix_transport_without_origin() -> None:
    ...
    from codex_crew.dashboard.origin import check_origin
...
async def test_start_unix_site_binds_and_removes_stale(
    ...
    monkeypatch.setattr(
        "codex_crew.dashboard.server.dashboard_socket_path",
...
async def test_start_unix_site_bind_failure_degrades(
    ...
    monkeypatch.setattr("codex_crew.dashboard.server.dashboard_socket_path", lambda port: squatter)
    app = web.Application()
    runner = web.AppRunner(app)
    await runner.setup()
    try:
