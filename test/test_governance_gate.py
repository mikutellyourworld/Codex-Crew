...
CodexCrew to refuse the call at its own host gate, EVEN IF the kiro agent config
...
def test_profile_denies_mcp_tool_kiro_would_grant(profiles_dir, monkeypatch):
    ...
    # surface denies the whole @codexcrew-cron server.
    ...
    _write(
        ...
        "cron",
        {
            ...
            "mcp": {"mode": "deny", "deny": ["@codexcrew-cron"]},
        },
    )
    hooks = HookManager(HooksConfig(auto_approve_tools=["*"]))  # kiro would auto-approve!
    result = hooks.on_tool_call("mcp__codexcrew-cron__cron_add", session_key="cron:job-1:run-1")
