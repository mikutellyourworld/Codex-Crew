"""Tests for /api/agents frequency ordering (api_codexcrew_agents).
...
def _fake_config(names):
    """A stand-in CodexCrewConfig: ordered agents dict + default_agent."""
...
def _make_agents_app(state) -> web.Application:
    from codex_crew.dashboard.handlers.agents import api_codexcrew_agents
...
class TestProjectScopeRoster:
    ...
    async def test_project_agent_appears_with_project_scope(self, tmp_path, monkeypatch):
        ...
        from codex_crew.agent_discovery import clear_project_agent_cache
