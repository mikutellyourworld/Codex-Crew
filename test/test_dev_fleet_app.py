...
import codex_crew.apps.builtins.dev_fleet.server as mod
from codex_crew import platform_compat
from codex_crew.apps.builtins.dev_fleet import fleet_state as fleet_state_mod
from codex_crew.apps.builtins.dev_fleet import http_api as http_api_mod
from codex_crew.apps.builtins.dev_fleet import live as live_mod
from codex_crew.apps.builtins.dev_fleet import npm_preflight
from codex_crew.apps.builtins.dev_fleet import repository as repository_mod
from codex_crew.apps.builtins.dev_fleet import runtime as runtime_mod
from codex_crew.apps.builtins.dev_fleet import worktree_ops as worktree_ops_mod
...
# (vs ``.venv\\Scripts\\codexcrew.exe``), systemctl/launchctl service probing, and
...
def test_parse_worktree_porcelain_basic():
    from codex_crew.apps.builtins.dev_fleet.server import _parse_worktree_porcelain

    raw = textwrap.dedent("""\
        worktree /home/user/codexcrew
        HEAD abc1234567890abcdef1234567890abcdef123456
        branch refs/heads/main

        worktree /home/user/codexcrew-wt-feature-x
        HEAD def4567890abcdef1234567890abcdef12345678
        branch refs/heads/feature-x

        worktree /home/user/codexcrew-wt-detached
    ...
    assert entries[0]["path"] == "/home/user/codexcrew"
    assert entries[0]["branch"] == "main"
    assert entries[1]["path"] == "/home/user/codexcrew-wt-feature-x"
...
def test_parse_worktree_porcelain_captures_locked():
    ...
    raw = textwrap.dedent("""\
        ...
        worktree /home/user/codexcrew-wt-held
        HEAD def4567890abcdef1234567890abcdef12345678
        branch refs/heads/held
        locked keeping this for the repro

        worktree /home/user/codexcrew-wt-bare-lock
        HEAD def4567890abcdef1234567890abcdef12345678
        detached
        locked
    """)
...
def test_parse_worktree_porcelain_captures_prunable():
    ...
    raw = textwrap.dedent("""\
        ...
        worktree /home/user/codexcrew-wt-deleted
        HEAD def4567890abcdef1234567890abcdef12345678
        branch refs/heads/gone
        prunable gitdir file points to non-existent location

        worktree /home/user/codexcrew-wt-bare-flag
        HEAD def4567890abcdef1234567890abcdef12345678
        detached
        prunable
...
async def test_discover_worktrees_drops_prunable():
    ...
    stdout = textwrap.dedent("""\
        ...
        worktree /home/user/codexcrew-wt-alive
        HEAD def4567890abcdef1234567890abcdef12345678
    ...
    assert paths == ["/home/user/codexcrew", "/home/user/codexcrew-wt-alive"]
...
async def test_discover_worktrees_keeps_prunable_main():
    ...
    assert [e["path"] for e in entries] == [
        "/home/user/codexcrew", "/home/user/codexcrew-wt-alive",
    ]
    assert entries[0]["is_main"] is True
...
def test_pr_status_merged():
    from codex_crew.apps.builtins.dev_fleet.server import _is_pr_merged
...
async def test_git_ahead_counts_plus_lines():
    from codex_crew.apps.builtins.dev_fleet.server import _git_ahead

    cherry_output = "+ abc1234\n+ def5678\n- ghi9012\n"
    with patch("codex_crew.apps.builtins.dev_fleet.repository._git", new_callable=AsyncMock) as mock_git:
...
async def test_remove_proceeds_when_session_bus_absent():
    ...
    import codex_crew.apps.builtins.dev_fleet.server as mod
    from codex_crew.pod.runtime import PodBackendAbsent
...
async def test_upstream_remote_reads_config():
    ...
    repository_mod._UPSTREAM_REMOTE = None
    with patch.object(runtime_mod, "_run_cmd", new_callable=AsyncMock, return_value=(0, "codexcrew\n", "")):
        result = await mod._upstream_remote()
...
async def test_sync_script_emits_step_markers():
    ...
    """
    import codex_crew.apps.builtins.dev_fleet.server as mod
    from codex_crew.apps.builtins.dev_fleet import sync_runner
    ...
    with patch.object(repository_mod, "_git", new_callable=AsyncMock, return_value="main"), \
         ...
         patch("codex_crew.apps.builtins.dev_fleet.worktree_ops.sandboxed_spawn_argv",
...
# The probe itself moved to codex_crew.dep_sync with the substitute it feeds, and
...
async def _run_sync(mod, locked):
    ...
    sit in a Codex Crew checkout. Assertions that quote the repo path must use
...
async def test_sync_substitutes_a_dependency_only_install_when_a_script_is_locked():
    ...
    result, cmd, steps = await _run_sync(mod, [r"C:\repo\.venv\Scripts\codexcrew.exe"])
    ...
    # It must NOT be run as `-m codex_crew...dep_sync`. That would import the
...
@pytest.mark.asyncio
@pytest.mark.parametrize("locked", [[], [r"C:\repo\.venv\Scripts\codexcrew.exe"]],
...
async def test_repo_owner_name_ssh():
    ...
    with patch.object(runtime_mod, "_run_cmd", new_callable=AsyncMock, return_value=(0, "git@github.com:mikutellyourworld/Codex-Crew.git\n", "")):
        result = await mod._repo_owner_name()
    assert result == "mikutellyourworld/Codex-Crew"
...
async def test_repo_owner_name_https():
    ...
    fleet_state_mod._OWNER_REPO = None
    with patch.object(runtime_mod, "_run_cmd", new_callable=AsyncMock, return_value=(0, "https://github.com/mikutellyourworld/Codex-Crew.git\n", "")):
        result = await mod._repo_owner_name()
    assert result == "mikutellyourworld/Codex-Crew"
...
async def test_worktree_remove_force_must_be_bool():
    ...
    from codex_crew.apps.builtins.dev_fleet.server import api_dev_fleet_worktree_remove
    ...
    with patch("codex_crew.apps.builtins.dev_fleet.repository._valid_worktree_names", new_callable=AsyncMock, return_value={"test-wt"}):
...
def test_redact_applied():
    from codex_crew.apps.builtins.dev_fleet.server import _redact
...
async def test_disk_name_derivation_from_path():
    ...
    fake_worktrees = [
        {"path": "/home/user/codexcrew", "is_main": True},
        {"path": "/home/user/codexcrew-wt-feature-x", "is_main": False},
    ]
    ...
    assert "codexcrew" in mod._DISK["per"] or "codexcrew-wt-feature-x" in mod._DISK["per"]
...
async def test_mutation_denied_emits_sel_event(monkeypatch):
    ...
    with patch(
        "codex_crew.apps.builtins.dev_fleet.repository._valid_worktree_names",
...
async def test_mutation_success_emits_sel_event(monkeypatch):
    ...
    from codex_crew.apps.builtins.dev_fleet.server import api_dev_fleet_pod_down
    ...
    with patch(
        "codex_crew.apps.builtins.dev_fleet.repository._find_worktree",
        new_callable=AsyncMock, return_value=({"name": "feature-x"}, None),
    ), patch(
        "codex_crew.apps.builtins.dev_fleet.worktree_ops._pod_down",
...
async def test_mutation_ok_false_audited_as_denied(monkeypatch):
    ...
    with patch(
        ...
        "codex_crew.apps.builtins.dev_fleet.worktree_ops._worktree_remove",
...
def test_escalation_cleanup_skips_symlinked_app_dir(tmp_path, monkeypatch):
    ...
    import codex_crew.apps.manager as mgr
...
async def test_pod_guard_rejects_foreign_pinned_checkout(monkeypatch, tmp_path):
    ...
    ours = tmp_path / "repo-a" / "codexcrew-wt-feature"
    ours.mkdir(parents=True)
    foreign = tmp_path / "repo-b" / "codexcrew-wt-feature"
    ...
    with patch.object(repository_mod, "_find_worktree", new_callable=AsyncMock,
        ...
        err = await mod._pod_checkout_guard("codexcrew-wt-feature")
...
async def test_pod_guard_allows_matching_or_unpinned(monkeypatch, tmp_path):
    ...
    ours = tmp_path / "codexcrew-wt-feature"
    ours.mkdir()
    ...
    with patch.object(repository_mod, "_find_worktree", new_callable=AsyncMock,
        ...
        assert await mod._pod_checkout_guard("codexcrew-wt-feature") is None
...
def test_build_env_excludes_credentials(monkeypatch):
    ...
    assert env["CODEXCREW_POD_REPO"] == mod.MAIN_REPO

    benv = mod._build_env()
    assert "SLACK_BOT_TOKEN" not in benv
    assert "CODEXCREW_POD_REPO" not in benv
...
async def test_pod_guard_denies_active_unpinned_pod(tmp_path):
    ...
    with patch.object(repository_mod, "_find_worktree", new_callable=AsyncMock,
         ...
         patch.object(runtime_mod, "_load_cfg", return_value=object()), \
         patch.object(worktree_ops_mod, "_read_pin_strict", return_value=(False, None)), \
         patch.object(runtime_mod.rt, "active_names", return_value={"codexcrew-wt-feature"}):
        err = await mod._pod_checkout_guard("codexcrew-wt-feature")
    assert err is not None
...
def _sign_request(secret: str, method: str, path: str, body: bytes = b"") -> dict:
    ...
    return {"X-CodexCrew-Proxy": f"{ts}:{sig}"}
...
async def test_discover_worktrees_sandbox_error_keeps_remedy():
    ...
    stderr = (
        ...
        "already inside a macOS Seatbelt sandbox that CodexCrew did not create. "
        'Set {"sandbox": false} in ~/.kiro/settings/amazon-internal.json so '
        "CodexCrew's own profile owns isolation, then restart the gateway."
    )
...
async def test_discover_worktrees_missing_repo_raises_actionable_error(tmp_path):
    ...
    CODEXCREW_PROJECT_DIR points at the app bundle and discovery falls through
    to the hardcoded ~/codexcrew) that empty state told users they had no
    ...
    assert "CODEXCREW_DEVFLEET_REPO" in msg  # names the remedy
...
async def test_discover_worktrees_unresolved_git_blames_host_not_repo(tmp_path):
    ...
    naming CODEXCREW_DEVFLEET_BIN_GIT, the override that is the actual remedy.
    """
    repo = tmp_path / "repo"
    (repo / ".git").mkdir(parents=True)
    ...
    assert "CODEXCREW_DEVFLEET_BIN_GIT" in msg  # names the remedy
...
async def test_sync_unresolved_git_names_override_not_path(monkeypatch):
    ...
    assert "CODEXCREW_DEVFLEET_BIN_GIT" in res["error"]
...
def test_bin_override_var_derivation():
    ...
    assert mod._bin_override_var("git") == "CODEXCREW_DEVFLEET_BIN_GIT"
    assert mod._bin_override_var("some-tool") == "CODEXCREW_DEVFLEET_BIN_SOME_TOOL"
    assert "CODEXCREW_DEVFLEET_BIN_GIT" in mod._unresolved_tool_message("git")
...
async def test_hmac_invalid_signature_returns_401():
    ...
    with patch.object(http_api_mod, "_load_app_secret", return_value="secret"):
        async with TestClient(TestServer(app)) as client:
            ts = str(int(time.time()))
            headers = {"X-CodexCrew-Proxy": f"{ts}:badbadbadbad"}
            resp = await client.get("/api/fleet", headers=headers)
            assert resp.status == 401
            body = await resp.json()
...
async def test_hmac_expired_timestamp_returns_401():
    ...
    with patch.object(http_api_mod, "_load_app_secret", return_value=secret):
        async with TestClient(TestServer(app)) as client:
            ...
            sig = _hmac_mod.new(secret.encode(), msg.encode(), hashlib.sha256).hexdigest()
            headers = {"X-CodexCrew-Proxy": f"{old_ts}:{sig}"}
            resp = await client.get("/api/fleet", headers=headers)
            assert resp.status == 401
            body = await resp.json()
...
async def test_fleet_handler_missing_repo_returns_error_payload():
    ...
    async def boom():
        raise RuntimeError(
            "main checkout not found: /nope/codexcrew is missing or not a git "
            "checkout. Set CODEXCREW_DEVFLEET_REPO to your Codex Crew checkout, "
            "or clone it to ~/codexcrew."
        )
    ...
    with patch.object(fleet_state_mod, "_fleet_refresh", new=boom), \
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            assert "CODEXCREW_DEVFLEET_REPO" in body["error"]
...
def _make_checkout(root: Path) -> Path:
    """Create a directory carrying every Codex Crew checkout marker."""
    (root / ".git").mkdir(parents=True)
    (root / "src" / "codex_crew").mkdir(parents=True)
    (root / "pyproject.toml").write_text("[project]\nname = 'kiro-crew'\n")
    return root


def test_is_codexcrew_checkout_requires_every_marker(tmp_path):
    ...
    assert mod._is_codexcrew_checkout(str(bare)) is False
    assert mod._is_codexcrew_checkout(str(_make_checkout(tmp_path / "codexcrew"))) is True
    assert mod._is_codexcrew_checkout("") is False
...
def test_default_main_repo_returns_empty_when_nothing_is_found(monkeypatch):
    ...
    """
    monkeypatch.delenv("CODEXCREW_DEVFLEET_REPO", raising=False)
...
def test_default_main_repo_skips_a_project_dir_that_is_not_codexcrew(monkeypatch, tmp_path):
    """A project directory that is some other git repo is not adopted."""
    other = tmp_path / "other"
    (other / ".git").mkdir(parents=True)
    monkeypatch.delenv("CODEXCREW_DEVFLEET_REPO", raising=False)
    monkeypatch.setenv("CODEXCREW_PROJECT_DIR", str(other))
    with patch.object(repository_mod, "_own_source_checkout", return_value=None):
        assert mod._default_main_repo() == ""
...
def test_default_main_repo_falls_back_to_the_running_checkout(monkeypatch, tmp_path):
    ...
    own = _make_checkout(tmp_path / "codexcrew")
    monkeypatch.delenv("CODEXCREW_DEVFLEET_REPO", raising=False)
    monkeypatch.delenv("CODEXCREW_PROJECT_DIR", raising=False)
...
def test_discover_main_repo_takes_an_explicit_path_verbatim(monkeypatch):
    ...
    monkeypatch.setenv("CODEXCREW_DEVFLEET_REPO", "/typo/codexcrew")
    with patch.object(repository_mod, "_load_dev_fleet_cfg", return_value={"repo_path": "/opt/kc"}):
        assert mod._discover_main_repo() == "/typo/codexcrew"
...
def test_discover_main_repo_finds_a_conventional_clone_location(monkeypatch, tmp_path):
    ...
    home = tmp_path / "home"
    checkout = _make_checkout(home / "Repos" / "CodexCrew")  # brand-ok: clone dir name
    monkeypatch.delenv("CODEXCREW_DEVFLEET_REPO", raising=False)
    monkeypatch.delenv("CODEXCREW_PROJECT_DIR", raising=False)
...
def test_discover_main_repo_uses_the_filesystem_spelling(monkeypatch, tmp_path):
    ...
    home = tmp_path / "home"
    checkout = _make_checkout(home / "repos" / "CODEXCREW")
    monkeypatch.delenv("CODEXCREW_DEVFLEET_REPO", raising=False)
    monkeypatch.delenv("CODEXCREW_PROJECT_DIR", raising=False)
    with patch.object(repository_mod, "_load_dev_fleet_cfg", return_value={}), \
         patch.object(repository_mod, "_own_source_checkout", return_value=None), \
...
def test_discover_main_repo_ignores_an_unmarked_conventional_location(monkeypatch, tmp_path):
    """An empty ~/codexcrew directory does not count as a checkout."""
    home = tmp_path / "home"
    (home / "codexcrew").mkdir(parents=True)
    monkeypatch.delenv("CODEXCREW_DEVFLEET_REPO", raising=False)
    monkeypatch.delenv("CODEXCREW_PROJECT_DIR", raising=False)
    with patch.object(repository_mod, "_load_dev_fleet_cfg", return_value={}), \
         patch.object(repository_mod, "_own_source_checkout", return_value=None), \
...
async def test_fleet_handler_reports_needs_setup_without_an_error():
    ...
    async def unconfigured():
        raise mod.RepoNotConfigured("no Codex Crew checkout found to manage")
...
@pytest.mark.asyncio
async def test_discover_worktrees_refuses_a_configured_non_codexcrew_repo():
    ...
    with patch.object(repository_mod, "MAIN_REPO", "/some/other/repo"), \
         patch.object(repository_mod, "_REPO_INVALID_MSG", "not a Codex Crew checkout: /some/other/repo ..."), \
         patch.object(runtime_mod, "_run_cmd", new=run):
    ...
    assert "not a Codex Crew checkout" in str(exc.value)


@pytest.mark.asyncio
async def test_auto_prune_reaper_idles_for_a_configured_non_codexcrew_repo():
    ...
    sel = MagicMock()
    with patch.object(repository_mod, "MAIN_REPO", "/some/other/repo"), \
         patch.object(repository_mod, "_REPO_INVALID_MSG", "not a Codex Crew checkout: ..."), \
...
@pytest.mark.asyncio
async def test_sync_refuses_a_configured_non_codexcrew_repo():
    ...
    assert res["ok"] is False
    assert "not a Codex Crew checkout" in res["error"]


@pytest.mark.asyncio
async def test_status_refresher_idles_for_a_configured_non_codexcrew_repo():
    run = AsyncMock(return_value=(0, "", ""))
    with patch.object(repository_mod, "MAIN_REPO", "/some/other/repo"), \
         patch.object(repository_mod, "_REPO_INVALID_MSG", "not a Codex Crew checkout: /some/other/repo ..."), \
         patch.object(runtime_mod, "_run_cmd", new=run):
...
async def test_discover_worktrees_proceeds_for_a_validated_checkout():
    with patch.object(repository_mod, "MAIN_REPO", "/good/codexcrew"), \
         patch.object(repository_mod, "_REPO_INVALID_MSG", None), \
         patch.object(runtime_mod, "_run_cmd", new=AsyncMock(
             return_value=(0, "worktree /good/codexcrew\nbranch refs/heads/main\n", "")
         )):
        entries = await mod._discover_worktrees()
...
async def test_sync_refuses_without_a_repo():
    with patch.object(repository_mod, "MAIN_REPO", ""):
        res = await mod._sync_start_locked()
    assert res["ok"] is False
    assert "no Codex Crew checkout" in res["error"]
...
def test_repo_source_hint_names_the_env_var(monkeypatch):
    monkeypatch.setenv("CODEXCREW_DEVFLEET_REPO", "/typo/codexcrew")
    assert "CODEXCREW_DEVFLEET_REPO" in mod._repo_source_hint()
...
def test_repo_source_hint_offers_both_when_neither_is_set(monkeypatch):
    ...
    with patch.object(repository_mod, "_load_dev_fleet_cfg", return_value={}):
        hint = mod._repo_source_hint()
    assert "CODEXCREW_DEVFLEET_REPO" in hint and "config.json" in hint
...
def test_backend_spawn_env_includes_codexcrew_home(monkeypatch):
    """The app backend must resolve the SAME config home as the gateway:
    minimal_env() strips CODEXCREW_HOME, so spawn must re-inject it or the
    backend reads the wrong .app_secret and every proxied call 401s."""
    import codex_crew.apps.registry as registry

    monkeypatch.setenv("CODEXCREW_HOME", "/tmp/some-pod-home")
    env = registry.minimal_env(CODEXCREW_HOME="/tmp/some-pod-home")
    assert env["CODEXCREW_HOME"] == "/tmp/some-pod-home"
    # And the bare strip behavior that motivated the fix:
    assert "CODEXCREW_HOME" not in registry.minimal_env()


def test_backend_spawn_env_passes_project_dir(monkeypatch):
    """CODEXCREW_PROJECT_DIR is a platform var like CODEXCREW_HOME — backends
    (e.g. dev-fleet worktree discovery) need the gateway's source checkout."""
    import inspect

    import codex_crew.apps.backend as backend_mod

    src = inspect.getsource(backend_mod)
    assert 'CODEXCREW_HOME=str(config_dir())' in src
    assert '"CODEXCREW_PROJECT_DIR"' in src
...
def _mk_make_live_wt(tmp_path, *, venv: bool = False, dist: bool = False,
                     venv_exec: bool = True):
    """Build a fake worktree dir with optional .venv/bin/codexcrew and built dist.

    When ``venv`` is set the fake ``.venv/bin/codexcrew`` is created **executable**
    ...
    if dist:
        dd = wt / "src" / "codex_crew" / "static" / "dist"
...
async def test_make_live_dry_run_plan(monkeypatch, tmp_path):
    ...
    assert plan["exec"] == str(wt / ".venv" / "bin" / "codexcrew")
...
async def test_make_live_missing_venv(monkeypatch, tmp_path):
    """No .venv/bin/codexcrew -> actionable Provision error."""
...
async def test_make_live_venv_not_executable(monkeypatch, tmp_path):
    """.venv/bin/codexcrew present but NOT executable -> a distinct, actionable
...
async def test_live_worktree_path_reads_working_directory_with_spaces(monkeypatch):
    ...
    spacey = "/home/u/my worktrees/codexcrew-wt-feat"
...
async def test_live_worktree_path_falls_back_to_execstart(monkeypatch, tmp_path):
    ...
    checkout = tmp_path / "codexcrew-wt-feat"
    (checkout / ".venv" / "bin").mkdir(parents=True)
    exe = checkout / ".venv" / "bin" / "codexcrew"
...
async def test_make_live_already_live_space_path(monkeypatch, tmp_path):
    ...
    wt = tmp_path / "my worktrees" / "codexcrew-wt-feat"
...
def test_in_pod_tristate(monkeypatch, tmp_path):
    ...
    import codex_crew.config.loader as cfg_loader

    pod_home = tmp_path / ".codexcrew-pods" / "codexcrew-wt-x"
    pod_home.mkdir(parents=True)
    monkeypatch.setattr(cfg_loader, "config_dir", lambda: pod_home)
    assert mod._in_pod() is True

    live_home = tmp_path / ".codexcrew"
    live_home.mkdir()
...
async def test_live_user_unit_status_darwin_agent_not_indirected(monkeypatch, tmp_path):
    ...
    plist.write_text("<string>/usr/local/bin/codexcrew</string>")
    monkeypatch.setattr(
...
async def test_live_user_unit_status_ok_and_missing(monkeypatch):
    ...
    async def cat_missing(cmd, **kw):
        return (1, "", "No files found for codexcrew-gateway.service.")
...
async def test_make_live_escapes_special_char_worktree(monkeypatch, tmp_path):
    ...
    name = 'codexcrew-wt feat'
    wt = tmp_path / name
    (wt / ".venv" / "bin").mkdir(parents=True)
    _kc = wt / ".venv" / "bin" / "codexcrew"
...
async def test_live_worktree_path_prefers_pointer_over_service(monkeypatch, tmp_path):
    ...
    target_wt = tmp_path / "codexcrew-wt-new"
    target_wt.mkdir(parents=True)
    (target_wt / ".venv" / "bin").mkdir(parents=True)
    (target_wt / ".venv" / "bin" / "codexcrew").write_text("#!/bin/sh\n")
    (target_wt / ".venv" / "bin" / "codexcrew").chmod(0o755)
    (target_wt / "src" / "codex_crew").mkdir(parents=True)
...
async def test_live_worktree_path_reports_running_image_while_staged(monkeypatch, tmp_path):
    ...
    (target_wt / ".venv" / "bin" / "codexcrew").chmod(0o755)
    (target_wt / "src" / "codex_crew").mkdir(parents=True)
    running_wt = tmp_path / "codexcrew-running"
...
async def test_cancel_invalid_target_is_a_refusal_not_a_crash(monkeypatch, tmp_path):
    ...
    def explode(_checkout):
        raise live_mod.live_target.InvalidTarget("no src/codex_crew in target")
...
async def test_loaded_but_inactive_user_unit_is_not_drivable(monkeypatch):
    ...
    assert notice.index("codexcrew restart") < notice.index("not running")
...
def test_running_checkout_resolves_this_checkout():
    ...
    assert (got / "src" / "codex_crew").is_dir()
...
async def test_sync_stages_dist_on_a_stock_checkout(monkeypatch):
    ...
    src/codex_crew/static/dist; without this step Pull+Build reports success and
...
async def test_sync_never_stages_dist_on_an_edition_checkout(monkeypatch):
    ...
    CODEXCREW_EDITION_DIR / CODEXCREW_ALLOW_EDITION, so on an edition composition
...
async def test_upstream_remote_rejects_option_injection(monkeypatch):
    ...
    assert await mod._upstream_remote() == "codexcrew"
...
def test_find_cli_is_module_invocation_only():
    """No filesystem resolution: a planted `codexcrew` shim must never become
    the pod CLI. Always our interpreter + the RUNNABLE ``codex_crew`` package
    entry (its __main__), never ``codex_crew.cli`` (no __main__ guard -> #220)."""
    import sys as _sys

    assert mod._find_cli() == [_sys.executable, "-m", "codex_crew"]
...
async def test_hmac_no_secret_always_denies(monkeypatch):
    ...
    monkeypatch.setenv("CODEXCREW_DEVFLEET_INSECURE", "1")
    app = mod.create_app()
    async with TestClient(TestServer(app)) as client:
        resp = await client.get("/api/fleet")
        assert resp.status == 401
...
async def test_hmac_invalid_signature_denial_is_audited(monkeypatch):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        r2 = await client.get("/api/fleet", headers={"X-CodexCrew-Proxy": "junk"})
        ts = str(int(time.time()))
        r3 = await client.get(
            "/api/fleet", headers={"X-CodexCrew-Proxy": f"{ts}:deadbeef"})
...
def test_strict_sandbox_hides_gh_config():
    ...
    import codex_crew.sandbox as sandbox_mod
...
def test_build_preexec_raises_nofile_ceiling(monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.security.apply_resource_limits", fake_apply)
...
def test_build_preexec_tolerates_malformed_config(monkeypatch):
    ...
    monkeypatch.setattr(sandbox_mod, "_BUILD_RESOURCE_PREEXEC", sandbox_mod._UNSET)
    monkeypatch.setattr(
        "codex_crew.config.loader._raw_config",
...
def test_gateway_unit_resolves_pod_instance(monkeypatch, tmp_path):
    ...
    pod_home = tmp_path / ".codexcrew-pods" / "codexcrew-wt-feature"
    pod_home.mkdir(parents=True)
    monkeypatch.setenv("CODEXCREW_HOME", str(pod_home))
    from codex_crew.config import loader as cfg_loader
    if hasattr(cfg_loader, "_config_dir_cache"):
        monkeypatch.setattr(cfg_loader, "_config_dir_cache", None, raising=False)
    assert mod._gateway_unit_name() == "codexcrew-pod@codexcrew-wt-feature.service"


def test_gateway_unit_resolves_live_outside_pods(monkeypatch, tmp_path):
    home = tmp_path / ".codexcrew"
    home.mkdir(parents=True)
    monkeypatch.setenv("CODEXCREW_HOME", str(home))
    assert mod._gateway_unit_name() == "codexcrew-gateway.service"
...
def test_parse_html_repo_base_variants():
    ...
    assert p("git@github.com:mikutellyourworld/Codex-Crew.git") == "https://github.com/mikutellyourworld/Codex-Crew"
    assert p("https://github.com/mikutellyourworld/Codex-Crew.git") == "https://github.com/mikutellyourworld/Codex-Crew"
    assert p("https://github.com/mikutellyourworld/Codex-Crew") == "https://github.com/mikutellyourworld/Codex-Crew"
    assert p("ssh://git@github.com/mikutellyourworld/Codex-Crew.git") == "https://github.com/mikutellyourworld/Codex-Crew"
    assert p("") is None
...
async def test_build_context_parses_pr_body_and_commits():
    ...
    with patch.object(repository_mod, "_upstream_remote", new_callable=AsyncMock, return_value="origin"), \
         ...
         patch.object(fleet_state_mod, "_html_repo_base", new_callable=AsyncMock,
                      return_value="https://github.com/mikutellyourworld/Codex-Crew"), \
    ...
    assert ctx["issues"][0]["url"] == "https://github.com/mikutellyourworld/Codex-Crew/issues/147"
...
async def test_fleet_payload_preserves_ordinary_main_repo_path():
    ordinary = "/home/user/oss/CodexCrew"
...
async def test_build_state_is_reported_even_where_pods_cannot_run(tmp_path):
    ...
    exe = binp / ("codexcrew.exe" if platform_compat.IS_WINDOWS else "codexcrew")
    exe.write_text("#!/bin/sh\n")
    exe.chmod(0o755)
    (wt / "src" / "codex_crew" / "static" / "dist").mkdir(parents=True)
...
async def test_main_checkout_build_state_is_probed(tmp_path):
    ...
    exe.write_text("#!/bin/sh\n")
    exe.chmod(0o755)
    (main_co / "src" / "codex_crew" / "static" / "dist").mkdir(parents=True)
...
# =============================================================================
def test_find_cli_targets_codex_crew_package():
    """_find_cli must invoke the ``codex_crew`` package (its __main__), not
    ``codex_crew.cli`` — the latter has no __main__ guard and no-ops silently."""
    import sys

    assert mod._find_cli() == [sys.executable, "-m", "codex_crew"]


def test_codex_crew_module_entry_actually_runs():
    """The entry point _find_cli uses must actually run main() and emit output.

    Guards the root cause of #220: ``python -m codex_crew.cli`` imported the
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "codex_crew", "--help"],
...
async def test_pod_down_fails_closed_when_still_active():
    ...
    with patch.object(worktree_ops_mod, "_pod_checkout_guard", new_callable=AsyncMock, return_value=None), \
         ...
         patch.object(runtime_mod, "_load_cfg", return_value=object()), \
         patch.object(runtime_mod, "_POD_AVAILABLE", True), \
         patch.object(runtime_mod.rt, "active_names", return_value={"codexcrew-wt-x"}):
        result = await mod._pod_down("codexcrew-wt-x")
    assert result["ok"] is False
...
async def test_pod_up_fails_closed_when_not_active():
    ...
    with patch.object(worktree_ops_mod, "_pod_checkout_guard", new_callable=AsyncMock, return_value=None), \
         ...
         patch.object(runtime_mod, "_POD_AVAILABLE", True), \
         patch.object(runtime_mod.rt, "active_names", return_value=set()):
        result = await mod._pod_up("codexcrew-wt-x")
    assert result["ok"] is False
...
def test_register_skills_creates_symlinks_for_bundled_skills(tmp_path, monkeypatch):
    ...
    codexcrew-worktree-dev is deliberately NOT bundled here: the canonical copy
    ...
    """
    from codex_crew.apps.bridges import _register_skills
    from codex_crew.apps.manifest import AppManifest

    fake_config = tmp_path / "config"
    fake_config.mkdir()
    monkeypatch.setattr(
        "codex_crew.apps.bridges.config_dir", lambda: fake_config
    )

    app_root = Path(__file__).resolve().parent.parent / (
        "src/codex_crew/apps/builtins/dev_fleet"
    )
    ...
    assert not (app_root / "skills" / "codexcrew-worktree-dev").exists()
...
def test_register_skills_tolerates_missing_feature_demo_recording(tmp_path, monkeypatch):
    ...
    manifest = AppManifest(
        ...
        skills=[
            "skills/pod-e2e",
            "skills/codexcrew-worktree-dev",  # no longer bundled — must not crash
    ...
    assert "codexcrew-worktree-dev" not in registered_names
...
def test_own_checkout_path_resolves_this_worktree():
    ...
    from pathlib import Path
    assert (Path(own) / "src" / "codex_crew").is_dir()
...
def test_parse_step_marker_index_and_label():
    from codex_crew.apps.builtins.dev_fleet.server import _parse_step_marker
...
def test_declared_platforms_all_resolve_to_a_real_sys_platform():
    ...
    """
    from codex_crew.apps.manifest import PlatformConfig
...
def test_kill_tree_reaps_a_descendant_that_escaped_the_process_group():
    ...
    from codex_crew.apps.builtins.dev_fleet import server as dev
...
async def test_removal_audit_log_emitted(caplog):
    ...
    with (
        ...
        patch.object(runtime_mod, "_run_cmd", new_callable=AsyncMock, return_value=(0, "", "")),
        patch.object(repository_mod, "_upstream_remote", new_callable=AsyncMock, return_value="origin"),
        caplog.at_level(logging.INFO, logger="codex_crew.apps.builtins.dev_fleet.server"),
    ):
        result = await mod._worktree_remove("feat-x", force=False)
...
async def test_pod_provision_registers_disk_invalidation(monkeypatch):
    ...
    import codex_crew.apps.builtins.dev_fleet.worktree_ops as wt_ops
    ...
    monkeypatch.setattr(runtime_mod, "_start_run", fake_start_run)
    monkeypatch.setattr(runtime_mod, "_find_cli", lambda: ["codexcrew"])
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(return_value=(["codexcrew", "pod", "provision", "feat-x"], {}, None)),
    )
...
def _mk_kcbin(tmp_path: Path, name: str = "codexcrew") -> Path:
...
def _fg(tmp_path: Path, *, port: int = 7777, pid: int = 4242,
    ...
    from codex_crew.apps.builtins.dev_fleet import gateway_service as gs
...
async def test_foreground_backend_binary_resolution(tmp_path):
    ...
    (an agent can plant a `codexcrew` in ~/.local/bin); invalid recorded
    ...
    assert await fg.status() == "no_codexcrew_binary"
    ok, err = await fg.restart_detached()
    assert not ok and "resolved" in err and spawned == []
    # A launcher that is not basenamed codexcrew is refused even when executable
    ...
    fg, _ = _fg(tmp_path, launcher="bin/codexcrew")
    assert await fg.status() == "no_codexcrew_binary"
...
def test_default_confinement_detects_sandbox_marker(monkeypatch):
    """CODEXCREW_SANDBOX_ACTIVE — the launcher-exported in-sandbox marker — is
...
async def test_make_live_foreground_last_resort_cutover(monkeypatch, tmp_path):
    ...
    `codexcrew restart` established, start_id (the pre-restart pid) returned,
...
async def test_sync_never_runs_an_operator_supplied_command(monkeypatch):
    ...
    """
    monkeypatch.setenv("CODEXCREW_DEVFLEET_NPM_AUTH_REPAIR", "/bin/sh -c true")
    monkeypatch.setattr(worktree_ops_mod.frontend, "edition_configured", lambda: False)
    _, cmd, steps = await _run_sync(mod, [])
    ...
    for token in ("CODEXCREW_DEVFLEET_NPM_AUTH_REPAIR", "npm_auth_repair"):
...
async def test_only_the_preflight_step_may_assert_a_diagnosis(monkeypatch):
    ...
    # the runner needs to import nothing from codex_crew and the two cannot
...
async def test_probe_and_merge_consume_one_immutable_commit(monkeypatch):
    ...
    assert pinned.startswith("refs/codexcrew/sync-base-"), pinned
...
async def test_prune_clears_dead_sync_refs_and_spares_live_ones(tmp_path, monkeypatch):
    ...
    dead = f"refs/codexcrew/sync-base-{dead_pid}"
    live_other = "refs/codexcrew/sync-base-1"
    foreign = "refs/codexcrew/something-else"
    ...
    remaining = set(
        git("for-each-ref", "--format=%(refname)", "refs/codexcrew/").splitlines()
    )
...
async def test_preflight_runs_a_snapshot_not_the_editable_source(monkeypatch):
    ...
    ``sys.path``, so an untracked ``codex_crew/`` package at its root would win;
    ``-I`` fixes that. But the install is EDITABLE, so ``import codex_crew...``
    ...
    Verified end to end in $CODEXCREW_SCRATCH/shadow_probe.py: a planted shadow
    ...
    assert "/src/codex_crew/" not in script[0], (
...
async def test_a_full_tmpdir_refuses_the_sync_instead_of_raising(
    ...
    leaked = list(tmp_path.glob("codexcrew-npm-preflight-*"))
    assert not leaked, (
...
async def test_every_stash_presence_gate_uses_lexists(monkeypatch):
    ...
    Verified end to end in $CODEXCREW_SCRATCH/runner_probe.py (scenarios 8 and 9);
