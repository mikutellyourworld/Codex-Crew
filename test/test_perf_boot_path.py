...
from codex_crew.dashboard import handlers_channel
...
def _probe(snippet: str) -> dict:
    ...
    # parse_dashboard_url honours CODEXCREW_PORT; keep the probe deterministic.
...
class TestOtelSdkImportIsDeferred:
    ...
    def test_provider_import_does_not_load_otel_sdk(self) -> None:
        result = _probe(
            "import json, sys\n"
            "import codex_crew.metrics.provider as p\n"
            "print(json.dumps({\n"
    ...
    def test_enabled_recorder_still_loads_the_sdk(self, tmp_path: Path) -> None:
        ...
        result = _probe(
            "import json, sys\n"
            "import codex_crew.metrics.provider as p\n"
            f"import os; os.environ['CODEXCREW_HOME'] = {str(tmp_path)!r}\n"
            "os.environ['CODEXCREW_TELEMETRY'] = '1'\n"
            "rec = p.get_recorder()\n"
            "print(json.dumps({\n"
            "    'sdk': 'opentelemetry.sdk.metrics' in sys.modules,\n"
            "    'enabled': rec.enabled,\n"
...
class TestConfigDirMemo:
    ...
    def test_override_change_is_honoured(
        ...
        """The memo must not pin a stale home when CODEXCREW_HOME is repointed —
        pods, worktrees and the test suite all repoint it at runtime."""
        monkeypatch.setattr(paths, "_config_dir_memo", None, raising=False)
        a, b = tmp_path / "a", tmp_path / "b"
        monkeypatch.setenv("CODEXCREW_HOME", str(a))
        assert paths.config_dir() == a.resolve()
        assert paths.config_dir() == a.resolve()  # memo hit
        monkeypatch.setenv("CODEXCREW_HOME", str(b))
...
class TestDashboardImportIsLeaf:
    ...
    def test_importing_urls_pulls_no_handler_tree_and_no_aiohttp(self) -> None:
        result = _probe(
            ...
            "print(json.dumps({\n"
            "    'server': 'codex_crew.dashboard.server' in sys.modules,\n"
            "    'handlers': 'codex_crew.dashboard.handlers' in sys.modules,\n"
    ...
    def test_importing_origin_pulls_no_handler_tree(self) -> None:
        result = _probe(
            "import json, sys\n"
            "import codex_crew.dashboard.origin as o\n"
            "print(json.dumps({\n"
            "    'server': 'codex_crew.dashboard.server' in sys.modules,\n"
            "    'handlers': 'codex_crew.dashboard.handlers' in sys.modules,\n"
            "    'aiohttp': 'aiohttp' in sys.modules,\n"
    ...
    def test_lazy_package_attributes_still_resolve(self) -> None:
        result = _probe(
            "import json\n"
            "from codex_crew.dashboard import (start_dashboard, start_api_server,\n"
            "    DashboardState, _ChatSlot, _fmt_duration)\n"
            "import codex_crew.dashboard as d\n"
            "print(json.dumps({\n"
    ...
    def test_memory_handler_does_not_import_vector_memory(self) -> None:
        ...
        result = _probe(
            "import json, sys\n"
            "import codex_crew.dashboard.handlers.memory  # noqa: F401\n"
            "print(json.dumps({\n"
            "    'vector_memory': 'codex_crew.vector_memory' in sys.modules,\n"
...
class TestFolderWatcherScanQueryCount:
    ...
    async def test_last_seen_is_still_written(self, tmp_path: Path) -> None:
        ...
        from codex_crew.knowledge.folder_watcher import FolderWatcher
        from codex_crew.knowledge.store import KnowledgeStore
...
class TestOptionalMcpServersAreNotImportedByTheCli:
    """`codexcrew gateway` boots through ``cli``, so a module-scope import of an
    ...
    def test_importing_cli_does_not_pull_the_mcp_server_modules(self) -> None:
        got = _probe(
            ...
            "print(json.dumps({\n"
            "    'dashboard': 'codex_crew.mcp_dashboard' in sys.modules,\n"
            "    'computer': 'codex_crew.mcp_computer' in sys.modules,\n"
            "}))\n"
        )
        assert got["dashboard"] is False, (
            "codex_crew.mcp_dashboard is imported at cli module scope — move it "
