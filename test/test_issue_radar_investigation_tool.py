...
from codex_crew.apps.builtins.issue_radar.backend import routes as ir_routes
...
class TestToolRegistration(unittest.TestCase):
    def test_schema_is_registered_so_bad_args_do_not_crash_the_server(self):
        ...
        # and kills the whole codexcrew-core server (see test_mcp_core_arg_crash).
...
class TestFindingsAreRedactedBeforePersisting(unittest.TestCase):
    ...
    def test_ordinary_prose_is_left_alone(self):
        ...
        captured, _ = self._call(
            ...
            summary="See https://github.com/mikutellyourworld/Codex-Crew/issues/1039 for the thread.",
        )
        findings = captured["body"]["findings"]
        assert findings["root_cause"] == "off-by-one in _parse_item_number"
        assert "github.com/mikutellyourworld/Codex-Crew/issues/1039" in findings["summary"]
