"""Tests for ``codex_crew.service.live_target`` — the live-target pointer.
...
from codex_crew.service.live_target import (
    _FILENAME,
    EXEC_MARKER,
    InvalidTarget,
    maybe_reexec,
...
def _isolate_pointer(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "codex_crew.config.loader.config_dir", lambda: tmp_path
    )
...
def _place_entry_point(checkout: Path, mode: int = 0o755) -> Path:
    ...
    (``.venv/bin/codexcrew`` vs ``.venv/Scripts/codexcrew.exe``) instead of pinning
...
def _make_valid_checkout(tmp_path: Path) -> Path:
    ...
    checkout.mkdir()
    (checkout / "src" / "codex_crew").mkdir(parents=True)
...
class TestValidate:
    ...
    def test_rejects_directory_lacking_src_codex_crew(self, tmp_path):
        checkout = tmp_path / "co"
        checkout.mkdir()
        _place_entry_point(checkout)
        with pytest.raises(InvalidTarget, match="no src/codex_crew"):
            validate(str(checkout))
...
class TestSnapshotRestore:
    ...
    def test_restore_none_does_not_harden_a_deleted_pointer(self, tmp_path, monkeypatch):
        ...
        hardened: list = []
        monkeypatch.setattr(
            "codex_crew.platform_compat.restrict_to_owner", lambda path: hardened.append(path))
        path = pointer_path()
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text("anything")
    ...
    def test_restore_returns_false_when_hardening_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.platform_compat.restrict_to_owner", boom)
        path = pointer_path()
        path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_restore_returns_false_on_oserror(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.service.live_target.pointer_path",
...
class TestMaybeReexec:
    ...
    def test_exec_path_argv_and_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["/usr/bin/codexcrew", "gateway", "--port", "5486"])
        monkeypatch.delenv(EXEC_MARKER, raising=False)
        ...
        maybe_reexec(["gateway", "--port", "5486"])
        ...
        assert exec_argv == [str(kcbin), "gateway", "--port", "5486"]
        assert exec_env[EXEC_MARKER] == "1"
        assert exec_env["CODEXCREW_PROJECT_DIR"] == str(resolved)
    ...
    def test_execve_oserror_is_swallowed(self, tmp_path, monkeypatch):
        ...
        checkout = _make_valid_checkout(tmp_path)
        write_target(checkout)
        monkeypatch.setattr(sys, "argv", ["/other/bin/codexcrew", "gateway"])
        monkeypatch.delenv(EXEC_MARKER, raising=False)
...
class TestSecurityRatchet:
    ...
    def test_live_target_json_in_sensitive_home_dirs_both_prefixes(self):
        ...
        # Must be at least .codex-crew and .codexcrew
