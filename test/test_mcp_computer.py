"""The ``codexcrew-computer`` MCP server — a THIN SHIM (``mcp_computer.py``).
...
from codex_crew import mcp_computer, mcp_core
from codex_crew.computer_use import backend as cu_backend
from codex_crew.computer_use import index as cu_index
from codex_crew.computer_use import policy as cu_policy
from codex_crew.computer_use import service as cu_service
from codex_crew.computer_use import tools as cu_tools
...
from codex_crew.testing.fake_computer_use import (
...
from codex_crew.validation import MCP_COMPUTER_SCHEMAS, ValidationError
...
def home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect ``CODEXCREW_HOME`` so the keystone lands in a tmp dir.

    Mandatory, not hygiene: without it a developer's real
    ``~/.codex-crew/computer_use.json`` would decide whether these tests see the
    feature as enabled.
    """
...
def ceiling(home: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    ``~/.codex-crew/profiles`` cannot decide the outcome, and the profile store cache
...
def test_oversized_text_is_rejected():
    ...
    from codex_crew.computer_use.types import MAX_TYPE_TEXT_LEN
...
async def test_gateway_dispatch_runs_on_the_bounded_subprocess_pool(
    ...
    monkeypatch.setattr("codex_crew.computer_use.tools.dispatch_tool", lambda *a, **k: "ok")
...
def test_server_identity_is_slash_free_and_stable():
    ...
    assert mcp_computer.SERVER_NAME == "codexcrew-computer"
...
def test_call_tool_emits_a_sel_invocation_on_validation_failure(
    ...
    """
    _enable(keystone)
    with patch("codex_crew.mcp_shared.sel") as sel_factory:
        recorder = MagicMock()
...
def test_ttl_expiry_names_the_age(
    ...
    """
    _enable(keystone)
    from codex_crew.computer_use.types import SNAPSHOT_TTL_SECS
...
def test_no_result_ever_carries_an_image_content_block(
    ...
    every tool's output.
    """
    from codex_crew.validation import build_tool_response
...
def test_get_state_relays_a_path_not_bytes(keystone: Path, fake_backend: FakeComputerUseBackend):
    ...
    from codex_crew.testing.fake_computer_use import FAKE_JPEG_BYTES
...
def test_sel_records_a_tool_invocation_on_success(
    ...
    """
    _enable(keystone)
    with patch("codex_crew.computer_use.tools.sel") as sel_factory:
        recorder = MagicMock()
        sel_factory.return_value = recorder
...
def test_read_only_and_mutating_sets_partition_the_tools():
    ...
    ``computer_end_turn`` is neither: it drops CodexCrew's OWN cache and touches no
...
class TestCursorMotionIsWiredToThePointerPath:
    ...
    def motions(self, monkeypatch) -> list:
        ...
        from codex_crew.computer_use import overlay as overlay_mod
    ...
    def test_an_app_post_click_never_consults_the_pointer_permit(
        ...
        real-pointer path.
        """
        from codex_crew.computer_use import gate as cu_gate
    ...
    def test_a_permitted_pointer_move_is_audited_by_method(
        ...
        monkeypatch.setattr("codex_crew.sel.sel", lambda: _Sel())
        _enable(keystone)
        _get_state()
...
class TestDragTool:
    ...
    def test_drag_is_refused_for_a_blocked_app(
        ...
        Retargeted from a terminal (no longer refused) onto CodexCrew's own window,
        which stays refused because driving our own Settings UI would route around
        the keystone that holds the primary enable.
        """
        _enable(keystone)
        result = _dispatch(TOOL_DRAG, app="Codex Crew", from_x=1, from_y=2, to_x=3, to_y=4)
        assert result.startswith(ERROR_PREFIX)
        assert not [args for name, args in fake_backend.calls if name == "drag"]
...
class TestTheSkillContractMatchesTheRuntime:
    ...
    _SKILL = (
        Path(__file__).resolve().parents[1] / "src/codex_crew/builtin_skills/computer-use/SKILL.md"
    )
    ...
    def test_the_skill_documents_the_truncated_screenshot_suppression(self):
        ...
        """
        from codex_crew.computer_use.types import TRUNCATED_WINDOW_NOTE
...
class TestUnresolvedSessionsAreNamespaced:
    ...
    **GPT's prescribed fix was to refuse an empty key. Not taken** — that is exactly
    ...
    def test_two_unresolved_sessions_do_not_share_a_snapshot_slot(self):
        ...
        from codex_crew.computer_use.index import SnapshotIndex
        from codex_crew.computer_use.types import ElementRec, Snapshot
...
class TestTheDriftWalkHonoursTheSnapshotBudget:
    ...
    def _wide_tree(count: int = 1500):
        from codex_crew.testing.fake_computer_use import FakeNode
