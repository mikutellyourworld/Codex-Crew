...
_BASELINE_PATH = pathlib.Path(
    os.environ.get("CODEXCREW_ERROR_CODE_BASELINE", _REPO_ROOT / "error-code-baseline.json")
)
...
class _Finding:
    ...
    def __str__(self) -> str:  # pragma: no cover - diagnostic only
        return f"src/codex_crew/{self.path}:{self.lineno}"
...
def test_baseline_file_is_parseable_and_shaped() -> None:
    ...
    for path, counts in baseline["files"].items():
        assert not path.startswith("/"), f"baseline paths are relative to src/codex_crew: {path}"
...
def test_no_new_error_response_without_a_code() -> None:
    ...
    for path, counts in actual.items():
        ...
        for bucket, count in counts.items():
            ...
            regressions.append(f"  src/codex_crew/{path}: {bucket} {cap} -> {count} (lines {lines})")
...
def test_baseline_is_not_stale() -> None:
    ...
    for path, counts in baseline.items():
        ...
        for bucket, cap in counts.items():
            ...
            if count < cap:
                stale.append(f"  src/codex_crew/{path}: {bucket} {cap} -> {count}")
