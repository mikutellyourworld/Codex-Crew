...
class TestSafeReadFileBytes:
    def test_reads_normal_file(self, tmp_path):
        ...
        with patch("codex_crew.hooks.is_sensitive_path", return_value=False):
    ...
    def test_rejects_sensitive_path(self, tmp_path):
        ...
        with patch("codex_crew.hooks.is_sensitive_path", return_value=True):
    ...
    def test_raises_file_too_large(self, tmp_path):
        ...
        with patch("codex_crew.hooks.MAX_FILE_BYTES", 10):
            with patch("codex_crew.hooks.is_sensitive_path", return_value=False):
...
class TestOutboxDir:
    def test_creates_and_returns_outbox(self, tmp_path):
        with patch("codex_crew.config.loader.workspace_root", return_value=tmp_path):
            from codex_crew.config.loader import outbox_dir
