"""Additional tests for codex_crew.sandbox — wrap_argv, profiles, env scrubbing."""
...
def clean_backend(monkeypatch):
    ...
    CodexCrew's OWN backend selection / fail-closed path — never reach the code
    ...
    Clears ``CODEXCREW_SANDBOX_ACTIVE`` to prevent the "already inside sandbox"
    ...
    """
    monkeypatch.delenv("CODEXCREW_SANDBOX_ACTIVE", raising=False)
    monkeypatch.setattr(
        "codex_crew.sandbox._KIRO_INTERNAL_SETTINGS_PATH",
        "/nonexistent/codexcrew-test/amazon-internal.json",
    )
...
class TestDetectBackend:
    ...
    @patch("codex_crew.sandbox._probe_unshare", return_value=False)
    @patch("codex_crew.sandbox._probe_sandbox_exec", return_value=False)
    def test_no_backend_available(self, mock_sb, mock_ns):
        result = detect_backend(config_mode="auto")
        assert result == "none"

    @patch("codex_crew.sandbox._probe_unshare", return_value=True)
    def test_linux_namespace(self, mock_ns):
        result = detect_backend(config_mode="auto")
        assert result == "namespace"

    @patch("codex_crew.sandbox._probe_unshare", return_value=False)
    @patch("codex_crew.sandbox._probe_sandbox_exec", return_value=True)
...
class TestWrapArgv:
    @patch("codex_crew.sandbox._allow_unsandboxed_exec", return_value=True)
    @patch("codex_crew.sandbox.detect_backend", return_value="none")
    ...
    @patch("codex_crew.sandbox.detect_backend", return_value="namespace")
    @patch("codex_crew.sandbox.namespace_argv")
    ...
    @patch("codex_crew.sandbox.detect_backend", return_value="sandbox-exec")
    @patch("codex_crew.sandbox.sandbox_exec_argv")
    ...
    @patch("codex_crew.sandbox.detect_backend")
    def test_inside_sandbox_passes_through(self, mock_detect, monkeypatch):
        # Inside an existing CodexCrew sandbox, nested unshare is seccomp-denied,
        ...
        # CODEXCREW_SANDBOX_ACTIVE marker (not the dual-purpose CODEXCREW_HOST_PID).
    ...
    def test_inside_sandbox_passthrough_survives_sel_failure(self, mock_detect, monkeypatch):
        ...
        monkeypatch.setattr(sandbox_mod, "_macos_sandbox_state", lambda: None)
        argv = ["kiro-cli", "acp"]
        with patch("codex_crew.sel.sel", side_effect=OSError("SEL transport down")):
            result, cleanup = wrap_argv(argv, mode="strict")
        assert result == argv
        assert cleanup is None
        mock_detect.assert_not_called()
    ...
    def test_host_pid_alone_does_not_pass_through(self, mock_detect, monkeypatch):
        # Deny-by-default: CODEXCREW_HOST_PID is dual-purpose session-identity
        # plumbing, so it must NOT by itself open the nested-sandbox passthrough.
        # Only the explicit CODEXCREW_SANDBOX_ACTIVE marker does.
        monkeypatch.delenv("CODEXCREW_SANDBOX_ACTIVE", raising=False)
        monkeypatch.setenv("CODEXCREW_HOST_PID", "12345")
        with patch("codex_crew.sandbox._allow_unsandboxed_exec", return_value=True):
            result, cleanup = wrap_argv(["kiro-cli"], mode="strict")
    ...
    def test_outside_sandbox_does_not_pass_through(self, mock_detect, monkeypatch):
        ...
        monkeypatch.delenv("CODEXCREW_SANDBOX_ACTIVE", raising=False)
        monkeypatch.delenv("CODEXCREW_HOST_PID", raising=False)
        with patch("codex_crew.sandbox._allow_unsandboxed_exec", return_value=True):
            result, cleanup = wrap_argv(["kiro-cli"], mode="strict")
        mock_detect.assert_called_once()
...
class TestBuildSeatbeltProfile:
    ...
    def test_every_mode_seals_voice_runtime_from_agents(self, level):
        ...
        home = str(Path.home())
        for relative in (".codex-crew/run/voice-runtime", ".codexcrew/run/voice-runtime"):
    ...
    def test_voice_guard_refusal_names_both_paths_when_workspace_contains_runtime(
        ...
        assert (
            "Pick a project subdirectory that does not contain the Codex Crew data home."
            in message
        )
    ...
    def test_bound_session_target_is_read_off_the_descriptor(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "codex_crew.sandbox.fd_real_path", lambda _fd: "/canonical/workspace"
        )
    ...
    def test_bound_session_target_fails_closed_when_the_name_cannot_be_read(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.sandbox.fd_real_path", lambda _fd: None)
...
class TestBuildLauncherScript:
    ...
    def test_launcher_refuses_when_seccomp_cannot_be_installed(self):
        ...
        Both ``_inside_codexcrew_sandbox`` and
    ...
    def test_a_file_valued_hidden_path_reaches_the_file_loop(self, tmp_path):
        ...
        (``sel_hmac.key``, ``token_signing.key``, ``.codex-crew/.env`` are files), and
    ...
    def test_the_builder_does_not_stat_the_hidden_paths(self):
        ...
        from codex_crew import sandbox
    ...
    @_POSIX_ONLY
    def test_launcher_has_no_unimportable_codex_crew_refs(self):
        """The launcher runs as a standalone ~/.codexcrew/run script with the
        launcher dir scrubbed from sys.path, so it CANNOT import codex_crew.
        ...
        for level in ("strict", "standard", "cc"):
            ...
            # explanatory comment naming platform_compat/codex_crew) don't
            ...
            forbidden = used_modules & {"platform_compat", "codex_crew", "logger", "logging"}
            assert (
                not forbidden
...
class TestSignalBroadcastGuard:
    """seccomp kill(-1) broadcast denial + CODEXCREW_HOST_PID export.
    ...
    def test_launcher_script_exports_host_pid(self):
        """Static: launcher exports CODEXCREW_HOST_PID before fork so the
        whole subtree can resolve session_pid files by the recorded pid."""
        script = _build_launcher_script("standard")
        assert 'os.environ["CODEXCREW_HOST_PID"] = str(os.getpid())' in script
        # Must appear in main() BEFORE the fork so the child inherits it.
        assert script.index("CODEXCREW_HOST_PID") < script.index("os.fork()")
    ...
    def test_kill_broadcast_denied_targeted_allowed_e2e(self, tmp_path):
        ...
        ``os.kill(own_pid, 0)`` succeeds and CODEXCREW_HOST_PID is present.
        ...
        import codex_crew.sandbox as _sb
        ...
        probe.write_text(
            ...
            "os.kill(os.getpid(), 0)\n"
            "print('TARGETED_OK')\n"
            "print('HOSTPID_' + ('SET' if os.environ.get('CODEXCREW_HOST_PID', '').isdigit() else 'MISSING'))\n"
        )
        launcher = tmp_path / "launcher.py"
        launcher.write_text(_build_launcher_script("standard"))
        result = subprocess.run(
...
class TestSandboxExecArgv:
    def test_exports_the_in_sandbox_marker(self):
        ...
        CodexCrew's own sandbox already confines it, tries to nest, and gets EPERM
    ...
    @patch.dict(os.environ, {"PYTHONPATH": "/opt/codexcrew/site-packages", "PYTHONHOME": "/opt/py"})
    def test_strips_python_env_when_requested(self):
        # A foreign Python subprocess (kiro-cli's MCP servers, e.g. ord-mcp) must
        # NOT inherit CodexCrew's PYTHONPATH/PYTHONHOME, or it prepends CodexCrew's
        # site-packages to sys.path and imports CodexCrew's fastmcp/cryptography
    ...
    def test_preserves_python_env_by_default(self):
        # CodexCrew's OWN sandboxed Python subprocesses (cron scripts, app
        # backends, code-review workers) import codex_crew via PYTHONPATH, so it
...
class TestNamespaceArgv:
    @patch("codex_crew.sandbox._resolve_agent_executable", return_value="/usr/local/bin/kiro-cli")
    ...
    @patch("codex_crew.sandbox._resolve_agent_executable", return_value="/bin/true")
...
class TestPinnedEnvBin:
    ...
    def test_pins_absolute_path_from_trusted_system_bin(self):
        with patch(
            "codex_crew.platform_compat.trusted_system_bin",
    ...
    def test_falls_back_to_canonical_location_not_bare_name(self):
        with patch("codex_crew.platform_compat.trusted_system_bin", return_value=None):
...
class TestAgentExecutableResolver:
    ...
    def test_edition_resolver_can_replace_executable(self):
        ...
        with patch("codex_crew.sandbox.current_context", return_value=context):
    ...
    def test_composition_failure_propagates(self):
        ...
        context.agent_executable = resolver
        with (
            patch("codex_crew.sandbox.current_context", return_value=context),
...
class TestSandboxNoWarningWhenExpected:
    ...
    @patch("codex_crew.sandbox._allow_unsandboxed_exec", return_value=True)
    @patch("codex_crew.sandbox._allow_no_isolation", return_value=True)
    @patch("codex_crew.sandbox.detect_backend", return_value="none")
    def test_no_sandbox_opted_in_logs_info_not_warning(
        ...
        with caplog.at_level(logging.DEBUG, logger="codex_crew.sandbox"):
...
class TestCleanupStaleSandboxProfiles:
    ...
    def test_removes_dead_pid_profile(self, tmp_path):
        ...
        from codex_crew.sandbox import cleanup_stale_sandbox_profiles

        run_dir = tmp_path / ".codexcrew" / "run"
        run_dir.mkdir(parents=True)
        stale_file = run_dir / "codexcrew_sandbox_99999_abc123.sb"
        stale_file.write_text("(version 1)")

        with patch("codex_crew.sandbox.config_dir", return_value=tmp_path / ".codexcrew"):
            with patch("codex_crew.sandbox.platform_compat.pid_exists", return_value=False):
    ...
    def test_reclaims_retired_acp_snapshot_tree(self, tmp_path):
        ...
        CodexCrew used to copy the whole ~100 MB kiro-cli binary per ACP spawn
        ...
        matches codexcrew_sandbox_* files; the tree is on the agent's
        ...
        with patch("codex_crew.sandbox.config_dir", return_value=home):
            removed = cleanup_stale_sandbox_profiles(legacy_dir=str(tmp_path / "nonexistent"))
    ...
    def test_preserves_live_pid_profile(self, tmp_path):
        ...
        run_dir = tmp_path / ".codexcrew" / "run"
        run_dir.mkdir(parents=True)
        live_file = run_dir / f"codexcrew_sandbox_{os.getpid()}_xyz789.sb"
    ...
    def test_ignores_non_sandbox_files(self, tmp_path):
        """Files not matching codexcrew_sandbox_*.sb pattern are left alone."""
        from codex_crew.sandbox import cleanup_stale_sandbox_profiles
...
class TestResourceLimitPreexec:
    ...
    def _reset_cache(self):
        import codex_crew.sandbox as sb
    ...
    def test_config_read_failure_falls_back_to_defaults(self):
        ...
        self._reset_cache()
        try:
            with patch("codex_crew.config.loader._raw_config", side_effect=RuntimeError("boom")):
    ...
    def test_non_posix_returns_none(self):
        ...
        self._reset_cache()
        try:
            with patch("codex_crew.sandbox.os.name", "nt"):
...
class TestCgroupScopeArgv:
    ...
    def test_available_prepends_systemd_scope_with_limits(self):
        ...
        try:
            with (
                patch("codex_crew.sandbox._probe_cgroup_scope", return_value=(True, "ok")),
                patch(
                    "codex_crew.sandbox._cgroup_limits_from_config",
                    return_value=(8192, 8192, 50, 0),
                ),
                patch("codex_crew.sandbox._cpu_controller_delegated", return_value=True),
            ):
    ...
    def test_no_cpu_properties_without_cpu_delegation(self):
        ...
        try:
            with (
                ...
                patch("codex_crew.sandbox._cpu_controller_delegated", return_value=False),
            ):
                out = sb.cgroup_scope_argv(["kiro-cli", "chat"])
            assert os.path.basename(out[0]) == "systemd-run"
            assert "TasksMax=8192" in out
    ...
    def test_config_defaults_when_absent_or_zero(self):
        ...
        try:
            ...
            with patch("codex_crew.config.loader._raw_config", return_value={}):
                procs, mem, weight, quota = sb._cgroup_limits_from_config()
    ...
    def test_real_pids_max_enforced_when_available(self):
        ...
        try:
            ...
            with patch(
                "codex_crew.sandbox._cgroup_limits_from_config", return_value=(20, 8192, 50, 0)
            ):
...
class TestAgentsSliceLimits:
    ...
    codexcrew-agents.slice — the parent of every per-spawn scope — so N
    ...
    def test_applies_runtime_property_once_idempotent(self):
        ...
        try:
            ...
            with (
                patch("codex_crew.sandbox._probe_cgroup_scope", return_value=(True, "ok")),
                patch("codex_crew.sandbox._slice_limits_from_config", return_value=(10000, 32768)),
                patch(
                    "codex_crew.platform_compat.trusted_system_bin",
                    return_value="/usr/bin/systemctl",
                ),
                patch("codex_crew.sandbox.subprocess.run", run_mock),
            ):
    ...
    def test_no_trusted_systemctl_means_no_apply(self):
        ...
        try:
            ...
            with (
                patch("codex_crew.sandbox._probe_cgroup_scope", return_value=(True, "ok")),
                patch("codex_crew.platform_compat.trusted_system_bin", return_value=None),
                patch("codex_crew.sandbox.subprocess.run", run_mock),
            ):
    ...
    def test_skipped_when_unavailable_and_no_second_warning(self, caplog):
        ...
        try:
            run_mock = MagicMock()
            with (
                patch("codex_crew.sandbox._probe_cgroup_scope", return_value=(False, "not Linux")),
                patch("codex_crew.sandbox.subprocess.run", run_mock),
                caplog.at_level(logging.WARNING, logger="codex_crew.sandbox"),
            ):
                assert sb.ensure_agents_slice_limits() is False
    ...
    def _fake_slice(self, tmp_path, *, oom_kill=0, local_max=0, current=100, mem_max="1000"):
        d = tmp_path / "codexcrew-agents.slice"
    ...
    def test_slice_pressure_seeds_then_reports_new_kills(self, tmp_path):
        ...
        try:
            ...
            with patch("codex_crew.sandbox._agents_slice_cgroup_dir", return_value=d):
    ...
    def test_slice_pressure_none_when_slice_absent(self):
        ...
        self._reset()
        try:
            with patch("codex_crew.sandbox._agents_slice_cgroup_dir", return_value=None):
                assert sb.check_agents_slice_pressure() is None
        finally:
            self._reset()
    ...
    def test_slice_pressure_self_heals_a_vanished_ceiling(self, tmp_path):
        ...
        try:
            ...
            with (
                patch("codex_crew.sandbox._agents_slice_cgroup_dir", return_value=d),
                patch("codex_crew.sandbox.ensure_agents_slice_limits", ensure_mock),
            ):
    ...
    def test_real_scope_nests_under_agents_slice(self):
        ...
        through codexcrew-agents.slice — the structural premise of the
        ...
        try:
            ...
            if not available:
                pytest.skip("no cgroup v2 delegation on this host")
            with patch(
                "codex_crew.sandbox._cgroup_limits_from_config", return_value=(50, 512, 50, 0)
            ):
                argv = sb.cgroup_scope_argv(
                    [
                        sys.executable,
            ...
            assert "/codexcrew-agents.slice/" in cg_path
...
class TestCgroupScopeBusEnv:
    ...
    def test_unset_env_argv_prefix_and_absence(self):
        ...
        with patch("codex_crew.sandbox.os.path.isfile", return_value=False):
    ...
    def test_sandboxed_spawn_argv_forwards_bus_but_child_cannot_keep_it(self):
        ...
        try:
            with (
                patch("codex_crew.sandbox.wrap_argv", return_value=(["gh", "pr", "view"], None)),
                patch("codex_crew.sandbox._probe_cgroup_scope", return_value=(True, "ok")),
                patch(
                    "codex_crew.sandbox._cgroup_limits_from_config",
                    return_value=(8192, 8192, 50, 0),
                ...
                patch("codex_crew.sandbox._cpu_controller_delegated", return_value=False),
                patch(
                    "codex_crew.sandbox._unset_env_argv",
    ...
    def test_no_env_binary_fails_closed_without_leaking_bus(self, caplog):
        ...
        try:
            with (
                patch("codex_crew.sandbox.wrap_argv", return_value=(["gh"], None)),
                patch("codex_crew.sandbox._probe_cgroup_scope", return_value=(True, "ok")),
                patch(
                    "codex_crew.sandbox._cgroup_limits_from_config",
                    return_value=(8192, 8192, 50, 0),
                ...
                patch("codex_crew.sandbox._cpu_controller_delegated", return_value=False),
                patch("codex_crew.sandbox._unset_env_argv", return_value=None),
                patch.dict(
                    os.environ, {"XDG_RUNTIME_DIR": "/run/user/4242"}, clear=False
                ),
...
class TestKiroInternalSandboxExclusion:
    ...
    def _write_settings(self, tmp_path, monkeypatch, content: str | None):
        ...
        monkeypatch.setattr("codex_crew.sandbox._KIRO_INTERNAL_SETTINGS_PATH", str(p))
        return p

    # --- kiro_internal_sandbox_enabled() helper ---

    def test_absent_file_is_disabled(self, tmp_path, monkeypatch):
        from codex_crew.sandbox import kiro_internal_sandbox_enabled
    ...
    def test_darwin_kiro_spawn_delegates(self, tmp_path, monkeypatch):
        ...
        self._write_settings(tmp_path, monkeypatch, '{"sandbox": true}')
        monkeypatch.setattr("codex_crew.sandbox.sys.platform", "darwin")
        with patch("codex_crew.sandbox.detect_backend") as mock_detect:
    ...
    def test_darwin_non_kiro_spawn_stays_wrapped(self, tmp_path, monkeypatch):
        ...
        with (
            patch("codex_crew.sandbox.detect_backend", return_value="sandbox-exec"),
            patch(
                "codex_crew.sandbox.sandbox_exec_argv",
    ...
    def test_darwin_kiro_disabled_stays_wrapped(self, tmp_path, monkeypatch):
        """kiro sandbox OFF -> CodexCrew's seatbelt ON (the inverse rule)."""
        self._write_settings(tmp_path, monkeypatch, '{"sandbox": false}')
        monkeypatch.setattr("codex_crew.sandbox.sys.platform", "darwin")
        with (
            patch("codex_crew.sandbox.detect_backend", return_value="sandbox-exec"),
    ...
    def test_linux_unaffected(self, tmp_path, monkeypatch):
        ...
        self._write_settings(tmp_path, monkeypatch, '{"sandbox": true}')
        monkeypatch.setattr("codex_crew.sandbox.sys.platform", "linux")
        with (
            patch("codex_crew.sandbox.detect_backend", return_value="namespace"),
            patch(
                "codex_crew.sandbox.namespace_argv",
                return_value=[
                    sys.executable,
                    *sandbox_mod._LAUNCHER_INTERPRETER_FLAGS,
                    "/tmp/launcher.py",
    ...
    def test_windows_explicit_kiro_backend_delegates_before_backend_probe(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.sandbox.sys.platform", "win32")
        launch = r"C:\Program Files\Kiro\kiro-cli.exe"
        with (
            patch("codex_crew.sel.sel", return_value=MagicMock()),
            patch("codex_crew.sandbox.detect_backend") as mock_detect,
            patch("codex_crew.sandbox.kiro_internal_sandbox_enabled", return_value=True) as mock_cap,
        ):
            argv, cleanup = wrap_argv(
                [launch, "acp"],
                mode="auto",
    ...
    def test_windows_kiro_sandbox_disabled_fails_closed(self, monkeypatch):
        ...
        """
        monkeypatch.setattr("codex_crew.sandbox.sys.platform", "win32")
        monkeypatch.setattr("codex_crew.sandbox._allow_unsandboxed_exec", lambda: False)
        with (
            patch("codex_crew.sandbox.kiro_internal_sandbox_enabled", return_value=False),
            patch("codex_crew.sandbox.detect_backend", return_value="none") as mock_detect,
            patch("codex_crew.sel.sel", return_value=MagicMock()),
    ...
    def test_windows_kiro_sandbox_disabled_honours_explicit_opt_in(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.sandbox.sys.platform", "win32")
        monkeypatch.setattr("codex_crew.sandbox._allow_unsandboxed_exec", lambda: True)
        launch = r"C:\Program Files\Kiro\kiro-cli.exe"
        with (
            patch("codex_crew.sandbox.kiro_internal_sandbox_enabled", return_value=False),
            patch("codex_crew.sandbox.detect_backend", return_value="none") as mock_detect,
    ...
    def test_windows_sel_failure_refuses_delegation(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.sandbox._allow_unsandboxed_exec", lambda: False)
        with (
            patch("codex_crew.sel.sel", side_effect=RuntimeError("audit down")),
            patch("codex_crew.sandbox.detect_backend", return_value="none") as mock_detect,
            pytest.raises(sandbox_mod.SandboxUnavailableError),
        ):
            wrap_argv(
    ...
    def test_sel_failure_refuses_delegation_falls_back_to_seatbelt(self, tmp_path, monkeypatch):
        ...
        is refused and the spawn falls back to CodexCrew's own seatbelt."""
        self._write_settings(tmp_path, monkeypatch, '{"sandbox": true}')
        monkeypatch.setattr("codex_crew.sandbox.sys.platform", "darwin")
        with (
            patch("codex_crew.sel.sel", side_effect=RuntimeError("audit down")),
    ...
    def test_symlink_to_sensitive_path_is_disabled(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            raise
        monkeypatch.setattr("codex_crew.sandbox._KIRO_INTERNAL_SETTINGS_PATH", str(link))
        assert kiro_internal_sandbox_enabled() is False
    ...
    def test_sel_failure_does_not_burn_warn_once_flag(self, tmp_path, monkeypatch, caplog):
        ...
        self._write_settings(tmp_path, monkeypatch, '{"sandbox": true}')
        monkeypatch.setattr("codex_crew.sandbox.sys.platform", "darwin")
        monkeypatch.setattr("codex_crew.sandbox._kiro_delegation_warned", False)
        ...
        with caplog.at_level(logging.WARNING, logger="codex_crew.sandbox"):
            with patch("codex_crew.sel.sel", return_value=MagicMock()):
                argv, cleanup = wrap_argv(["kiro-cli", "acp"], mode="auto")
        assert "sandbox-exec" not in argv
        assert cleanup is None
...
class TestMacOsNestingDetection:
    ...
    — because CodexCrew's own seatbelt had already confined the process tree.
    ...
    def test_foreign_outer_sandbox_fails_closed_with_actionable_guidance(
        ...
        # Nested under a sandbox CodexCrew did NOT create (no marker): its profile
    ...
    def test_available_backend_still_wraps(self, mock_detect, monkeypatch):
        ...
        monkeypatch.delenv("CODEXCREW_SANDBOX_ACTIVE", raising=False)
        monkeypatch.setattr(sandbox_mod, "_macos_sandbox_state", lambda: True)
        with patch("codex_crew.sandbox.sandbox_exec_argv") as mock_sb:
...
class TestAgentSliceMemoryHigh:
    ...
    ceiling on codexcrew-agents.slice — bounding the SUM of all concurrent agent
    ...
    def test_spawn_path_schedules_reconciliation_off_thread(self):
        ...
        try:
            with patch(
                "codex_crew.sandbox._ensure_agent_slice_memory_high",
    ...
    def test_thread_start_failure_disarms_without_aborting_the_spawn(self) -> None:
        ...
        try:
            with patch(
                "codex_crew.sandbox.threading.Thread",
    ...
    def test_applies_host_default_via_systemctl_runtime(self):
        ...
        try:
            with (
                patch("codex_crew.sandbox._default_slice_memory_high_mb", return_value=2048),
                patch("codex_crew.sandbox.platform_compat.trusted_system_bin", return_value="/usr/bin/systemctl"),
                patch("codex_crew.sandbox.subprocess.run") as run,
            ):
    ...
    def test_steady_state_is_a_noop(self):
        ...
        try:
            with (
                patch(
                    "codex_crew.sandbox._default_slice_memory_high_mb",
                    return_value=2048,
                ),
                patch("codex_crew.sandbox.platform_compat.trusted_system_bin", return_value="/usr/bin/systemctl"),
                patch("codex_crew.sandbox.subprocess.run") as run,
    ...
    def test_missing_systemctl_warns_and_disarms_without_raising(self, caplog):
        ...
        try:
            with (
                patch("codex_crew.sandbox._default_slice_memory_high_mb", return_value=2048),
                patch("codex_crew.sandbox.platform_compat.trusted_system_bin", return_value=None),
            ):
                with caplog.at_level(logging.WARNING):
                    sb._ensure_agent_slice_memory_high()
            assert sb._SLICE_MEMHIGH_DISABLED is True
    ...
    def test_cgroup_scope_argv_reconciles_when_available(self):
        ...
        try:
            with (
                ...
                patch("codex_crew.sandbox._cpu_controller_delegated", return_value=False),
                patch(
                    "codex_crew.sandbox._reconcile_slice_memory_high_off_thread"
                ) as ensure,
            ):
                out = sb.cgroup_scope_argv(["kiro-cli", "chat"])
            ...
            assert any(
                a.startswith("--slice=codexcrew-agents") and a.endswith(".slice") for a in out
            ), out
        finally:
            sb._CGROUP_SCOPE_PROBE = None
    ...
    def test_cgroup_scope_argv_skips_reconcile_when_unavailable(self):
        ...
        try:
            with (
                ...
                patch("codex_crew.sandbox._ensure_agent_slice_memory_high") as ensure,
            ):
                out = sb.cgroup_scope_argv(["git", "status"])
    ...
    def test_worker_checks_pressure_after_reconcile(self):
        ...
        try:
            with (
                patch("codex_crew.sandbox._ensure_agent_slice_memory_high") as ensure,
                patch(
                    "codex_crew.sandbox._check_slice_memory_pressure",
    ...
    def test_pressure_warns_once_per_climbing_episode(self, caplog):
        ...
        try:
            ...
            with patch(
                "codex_crew.sandbox._slice_memory_events_high",
    ...
    def test_pressure_first_read_baselines_without_warning(self, caplog):
        ...
        self._reset()
        try:
            with patch("codex_crew.sandbox._slice_memory_events_high", return_value=42):
                with caplog.at_level(logging.WARNING):
    ...
    def test_pressure_counter_reset_rebaselines_silently(self, caplog):
        ...
        try:
            with caplog.at_level(logging.WARNING):
                with patch("codex_crew.sandbox._slice_memory_events_high", return_value=2):
                    sb._check_slice_memory_pressure()
                assert sb._SLICE_MEMHIGH_EVENTS_SEEN == 2
                assert sb._SLICE_MEMHIGH_CLIMB_WARNED is False
                with patch("codex_crew.sandbox._slice_memory_events_high", return_value=4):
                    sb._check_slice_memory_pressure()
            warns = [r for r in caplog.records if "memory.events" in r.getMessage()]
            assert len(warns) == 1
    ...
    def test_pressure_unreadable_is_silent_and_keeps_state(self, caplog):
        ...
        sb._SLICE_MEMHIGH_EVENTS_SEEN = 5
        try:
            with patch("codex_crew.sandbox._slice_memory_events_high", return_value=None):
                with caplog.at_level(logging.WARNING):
                    sb._check_slice_memory_pressure()
            assert not [r for r in caplog.records if "memory.events" in r.getMessage()]
    ...
    def test_slice_memory_events_high_reads_counter(self, tmp_path):
        ...
        with patch("codex_crew.sandbox._agents_slice_cgroup_dir", return_value=tmp_path):
            assert sb._slice_memory_events_high() == 42
        with patch("codex_crew.sandbox._agents_slice_cgroup_dir", return_value=None):
            assert sb._slice_memory_events_high() is None
        empty = tmp_path / "no-events"
        empty.mkdir()
        with patch("codex_crew.sandbox._agents_slice_cgroup_dir", return_value=empty):
            assert sb._slice_memory_events_high() is None
        evt.write_text("low 0\nhigh notanumber\n", encoding="utf-8")
        with patch("codex_crew.sandbox._agents_slice_cgroup_dir", return_value=tmp_path):
            assert sb._slice_memory_events_high() is None
    ...
    def test_slice_memory_events_high_resolves_dash_hierarchy(self, tmp_path):
        ...
        under codexcrew.slice (dash-hierarchy), NOT directly under
        ...
        nested = tmp_path / "codexcrew.slice" / sb._CGROUP_AGENTS_SLICE
