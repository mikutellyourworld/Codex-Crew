...
class TestPipelineConductorInstaller:
    ...
    def test_identity_and_charter(self, tmp_path, monkeypatch):
        ...
        assert data["name"] == "codexcrew-pipeline-conductor"
    ...
    def test_dashboard_grants_are_create_and_read_only(self, tmp_path, monkeypatch):
        ...
        assert "@codexcrew-dashboard/session_create" in allowed
        assert "@codexcrew-dashboard/session_read_message" in allowed
        assert "@codexcrew-dashboard/chat_folder_tree" in allowed
        assert "@codexcrew-dashboard/chat_folder_create" in allowed
        for gated in (
            "@codexcrew-dashboard/session_send",
            "@codexcrew-dashboard/session_stop",
            "@codexcrew-dashboard/chat_folder_move_session",
            "@codexcrew-dashboard",
            "execute_bash",
        ):
            assert gated not in allowed, gated
        assert "@codexcrew-dashboard" in data["tools"]  # mounted, so gated verbs still work
    ...
    def test_core_grants_are_named_verbs_never_the_whole_server(self, tmp_path, monkeypatch):
        ...
        data = self._install(tmp_path, monkeypatch)
        allowed = set(data["allowedTools"])
        assert "@codexcrew-core/monitor_start" in allowed
        assert "@codexcrew-core/resource_status" in allowed
        assert "@codexcrew-core/session_ledger_record" in allowed
        for gated in (
            "@codexcrew-core",
            "@codexcrew-core/task_run",
            "@codexcrew-core/workflow_run",
            "@codexcrew-core/cron_add",
            "@codexcrew-core/spawn_run",
        ):
            assert gated not in allowed, gated
        assert "@codexcrew-core" in data["tools"]

    def test_mcp_servers_are_narrowed(self, tmp_path, monkeypatch):
        """Only codexcrew-core and the hand-built codexcrew-dashboard entry ship;
...
class TestFleetProbe:
    ...
    def _config(self, tmp_path: Path, monkeypatch, sessions: list[str], **extra) -> Path:
        ...
        under $CODEXCREW_HOME/sessions, /proc behind the test-only env seam --
        neither is a config key (containment: the config is agent-authored)."""
        (tmp_path / "sessions").mkdir(exist_ok=True)
        monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path))
        monkeypatch.setenv("CODEXCREW_PROBE_PROC_ROOT", str(tmp_path / "no-proc"))
    ...
    def test_banned_process_scan_reports_matches(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setenv("CODEXCREW_PROBE_PROC_ROOT", str(proc))
        out = self._run(mod, cfg, capsys)
    ...
    def test_a_windows_path_spelling_still_matches_its_fleet_root(self, tmp_path, monkeypatch):
        ...
        """
        mod = self._mod()
        root = r"D:\a\CodexCrew\fleet\wt-a"
