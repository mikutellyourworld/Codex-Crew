...
``codexcrew-computer`` stdio shim forwards to.
...
from codex_crew.dashboard.handlers import computer_use as cu_api
...
def home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect CODEXCREW_HOME so the keystone + config writes land in a tmp dir."""
...
def no_agent_spec_rewrite(monkeypatch):
    ...
    gateway start). That rebuild writes ``~/.kiro/agents/codexcrew.json``, which is
    machine-wide and deliberately NOT under the ``CODEXCREW_HOME`` this module's
...
def no_real_probe(monkeypatch):
    """Never spawn the real ``codexcrew computer doctor`` child in CI.
...
class TestConfigGet:
    ...
    async def test_macos_platform_reports_the_probed_grants(self, home):
        ...
        with patch("codex_crew.computer_use.backend.get_shared_backend", return_value=backend):
            async with _client() as client:
                body = await (await client.get("/api/computer-use/config")).json()
    ...
    async def test_unsupported_platform_reports_reason_and_skips_the_probe(self, home, monkeypatch):
        from codex_crew.computer_use.backend import UnsupportedBackend
...
class TestConfigSave:
    ...
    async def test_limits_write_preserves_unrelated_config_sections(self, config_file: Path):
        ...
        # ``CodexCrewConfig.to_dict()``, so an unrelated section keeps whatever the
...
class TestInvoke:
    ...
    async def test_forwards_the_call_and_returns_the_dispatcher_text(self, home, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.computer_use.tools.dispatch_tool", _fake_dispatch)
        async with _client() as client:
    ...
    async def test_malformed_requests_are_400(self, body, home, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.computer_use.tools.dispatch_tool", _boom)
        async with _client() as client:
    ...
    async def test_a_raising_dispatcher_becomes_error_text_not_a_5xx(self, home, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.computer_use.tools.dispatch_tool", _raise)
        async with _client() as client:
            resp = await client.post(
                "/api/computer-use/invoke",
...
class TestInvokeSecretEnforcement:
    ...
    def _authed_client(secret: str) -> TestClient:
        from codex_crew.dashboard.server import _STRICT_INTERNAL_API_PATHS
        from codex_crew.dashboard.token_auth import token_auth_middleware
    ...
    def _never_dispatch(self, monkeypatch):
        ...
        reached: list[str] = []
        monkeypatch.setattr(
            "codex_crew.computer_use.tools.dispatch_tool",
...
class TestConfigSection:
    def test_config_section_has_no_enabled_field(self):
        ...
        """
        from codex_crew.config.loader import ComputerUseConfig
    ...
    def test_section_is_known_and_round_trips(self, home):
        from codex_crew.config.loader import _KNOWN_CONFIG_SECTIONS, CodexCrewConfig

        assert "computer_use" in _KNOWN_CONFIG_SECTIONS
        assert "computer_use" in CodexCrewConfig.load().to_dict()

    def test_state_path_is_on_the_keystone_floor(self):
        from codex_crew.config.loader import computer_use_state_path
        ...
        for command in (
            "cat ~/.codex-crew/computer_use.json",
            "echo x > ~/.codex-crew/computer_use.json",
            "tee ~/.codex-crew/computer_use.json",
        ):
...
class TestFrameIngress:
    ...
    async def test_a_non_loopback_post_is_refused(self, home, mock_sel):
        ...
        with patch("codex_crew.dashboard.origin.is_loopback", return_value=False):
    ...
    async def test_a_loopback_post_without_the_verified_secret_is_refused(self, home, mock_sel):
        ...
        delivered: list = []
        app["state"].deliver_ws_owners = lambda *a: delivered.append(a)
        with patch("codex_crew.dashboard.origin.is_loopback", return_value=True):
            async with TestClient(TestServer(app)) as client:
                resp = await client.post("/api/computer-use/frame", json=_VALID_FRAME)
        assert resp.status == 403
        assert delivered == []
...
class TestFramePayload:
    ...
    def test_a_minimal_valid_frame(self):
        from codex_crew.computer_use.screencast import build_frame_payload
    ...
    def test_an_oversized_frame_is_rejected(self):
        ...
        from codex_crew.computer_use.screencast import (
            MAX_FRAME_B64_CHARS,
            build_frame_payload,
        )
    ...
    def test_dimensions_are_integers_within_the_encoder_ceiling(self):
        from codex_crew.computer_use.screencast import build_frame_payload
        from codex_crew.computer_use.types import MAX_SCREENSHOT_MAX_PX
...
class TestLiveViewSuppression:
    ...
    def _snapshot(self, *, secure: bool = False):
        from codex_crew.computer_use.types import AppRef, Snapshot
    ...
    def test_the_frame_scope_is_restored_and_never_leaks_across_dispatches(self, home):
        ...
        assert screencast.active_scope() is None
        with screencast.frame_scope(session_key="dashboard:slot1", agent="codexcrew"):
...
class TestEnableRestartsSessions:
    ...
    def _spy(monkeypatch) -> list:
        ...
        """
        calls: list = []
        import codex_crew.dashboard.handlers.sessions as sessions_mod
    ...
    async def test_the_flip_REBUILDS_the_agent_spec(self, state_file, monkeypatch):
        ...
        ``agent._computer_use_spec_gate`` keeps ``codexcrew-computer`` out of the
    ...
    async def test_patching_the_agent_module_REACHES_the_handler(self, state_file, monkeypatch):
        ...
        so patching ``codex_crew.agent`` would no longer reach this call site — and
        ...
        assert resp.status == 200
        assert ran == ["stub"], (
            "patching codex_crew.agent no longer reaches the handler's call site — the "
...
class TestErrorCodes:
    ...
    * the stdio shim reaches ``/invoke`` through ``mcp_core._http_error_body``,
    ...
    def test_no_refusal_in_this_module_is_prose_only(self) -> None:
        ...
        assert missing == [], (
            "these src/codex_crew/dashboard/handlers/computer_use.py lines refuse with "
