...
``codex_crew.agent.__file__`` to a fabricated checkout under pytest's tmp dir.
...
# really occur -- codexcrew-wt-example/ via _make_linked_worktree() and a plain
# CodexCrew/ clone dir in test_does_not_decline_from_an_ordinary_clone.
PHANTOM_PATHS = [
    pytest.param(
        "/tmp/pytest-of-runner/pytest-0/popen-gw0/"
        "test_does_not_decline_from_an_0/CodexCrew/src/codex_crew/agent.py",
        id="ci-ordinary-clone",
    ),
    pytest.param(
        "/tmp/pytest-of-runner/pytest-0/popen-gw0/"
        "test_declines_0/codexcrew-wt-example/src/codex_crew/agent.py",
...
# checked out at /home/runner/work/CodexCrew/CodexCrew, so a "*/CodexCrew/*" glob
# would omit the entire package and turn the coverage gate green on nothing.
REAL_SOURCE_PATHS = [
    pytest.param(
        "/home/runner/work/CodexCrew/CodexCrew/src/codex_crew/agent.py",
        id="ci-checkout",
    ),
    pytest.param("/home/dev/src/CodexCrew/src/codex_crew/agent.py", id="local-checkout"),
]
