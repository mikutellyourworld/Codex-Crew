...
read-write open recovers the log into the main file and unlinks the log, so `codexcrew
...
import pytest
from test_snapshot import _setup_fake_codexcrew
...
class TestStagingDoesNotWriteToTheLiveDatabase:
    ...
    def test_the_command_never_writes_to_the_live_database(
        ...
        removed by then. That checkpoint was the only write `codexcrew snapshot` made to live
    ...
    def test_a_symlinked_data_home_does_not_traceback(
        ...
        ends in `.resolve()`, so a `CODEXCREW_HOME` override can never present a symlinked
        ...
        real_home.mkdir()
        _setup_fake_codexcrew(real_home)
...
class TestACorruptCoreDatabaseFailsInsteadOfPruningAGoodBackup:
    ...
    def test_the_failure_happens_before_any_backup_is_pruned(
        ...
        out = tmp_path / "out"
        out.mkdir()
        previous = out / "codexcrew-snapshot-20260101-000000.tgz"
    ...
    def test_a_tree_database_that_is_not_sqlite_still_rides_as_bytes(
        ...
        over it would be an outage rather than a safeguard. `_setup_fake_codexcrew` leaves a
