...
import codex_crew.acp.client as acp_client
...
from codex_crew.acp.liveness import (
    EVIDENCE_SAMPLING,
    VERDICT_DEAD,
    VERDICT_UNKNOWN,
    VERDICT_WORKING,
...
class TestVendoredClaudeAcp:
    ...
    def _make_vendored(self, root: Path, *, with_deps: bool = True) -> Path:
        ...
        """
        from codex_crew.acp.client import _CLAUDE_ACP_DEP_MARKER
    ...
    def test_finds_vendored_in_pkg_vendor_dir(self, tmp_path, monkeypatch):
        ...
        # (sibling-website detection / CODEXCREW_PROJECT_DIR) is not consulted.
        monkeypatch.delenv("CODEXCREW_PROJECT_DIR", raising=False)
        pkg_dir = tmp_path / "site-packages" / "codex_crew"
    ...
    def test_finds_vendored_under_project_dir(self, tmp_path, monkeypatch):
        # CODEXCREW_PROJECT_DIR/node_modules holds the adapter; pkg_dir has none.
        pkg_dir = tmp_path / "site-packages" / "codex_crew"
        pkg_dir.mkdir(parents=True)
        entry = self._make_vendored(tmp_path / "proj" / "node_modules")
        (tmp_path / "proj").mkdir(exist_ok=True)
        monkeypatch.setenv("CODEXCREW_PROJECT_DIR", str(tmp_path / "proj"))
        assert _resolve_vendored_claude_acp(pkg_dir=pkg_dir) == str(entry)

    def test_returns_none_when_absent(self, tmp_path, monkeypatch):
        # Isolated pkg_dir with no _vendor and a project dir with no adapter.
        monkeypatch.setenv("CODEXCREW_PROJECT_DIR", str(tmp_path / "empty"))
    ...
    def test_skips_incomplete_copy_missing_deps(self, tmp_path, monkeypatch):
        ...
        # falling through to a complete copy under CODEXCREW_PROJECT_DIR.
        pkg_dir = tmp_path / "site-packages" / "codex_crew"
        pkg_dir.mkdir(parents=True)
...
class TestAcpClientSessionKey:
    ...
    async def test_spawn_sets_env_with_session_key(self, tmp_path):
        ...
        with (
            patch("codex_crew.acp.client._resolve_kiro_bin", return_value="/usr/bin/kiro-cli"),
            patch(
                "codex_crew.acp.client.wrap_argv", return_value=(["/usr/bin/kiro-cli", "acp"], None)
            ),
            ...
            assert env["CODEXCREW_SESSION_KEY"] == "test-key"
    ...
    async def test_spawn_sets_env_with_channel_id(self, tmp_path):
        ...
        with (
            ...
            env = call_kwargs.kwargs.get("env") or call_kwargs[1].get("env")
            assert env is not None
            assert env["CODEXCREW_CHANNEL_ID"] == "C0ABC123"
            assert env["CODEXCREW_SESSION_KEY"] == "k"
    ...
    async def test_spawn_forwards_claude_config_dir_from_extra_env(self, tmp_path):
        ...
        with (
            patch(
                "codex_crew.acp.client._resolve_claude_acp_bin",
    ...
    async def test_spawn_gives_each_process_its_own_browser_session(self, tmp_path):
        ...
        for _ in range(2):
            ...
            with (
                ...
                patch("asyncio.create_subprocess_exec", new_callable=AsyncMock) as mock_exec,
                patch("codex_crew.acp.client.browser_socket_env", side_effect=_lifecycle_env),
                patch("codex_crew.session._track_pid"),
                patch("codex_crew.session._track_session_pid"),
            ):
                mock_proc = MagicMock()
    ...
    async def test_spawn_no_channel_id_env_absent(self, tmp_path):
        ...
        with (
            ...
            env = call_kwargs.kwargs.get("env") or call_kwargs[1].get("env")
            assert env is not None
            assert "CODEXCREW_CHANNEL_ID" not in env

        await _stop_stderr_drain(client)

    @pytest.mark.asyncio
    async def test_spawn_channel_id_only_no_session_key(self, tmp_path):
        clean_env = {k: v for k, v in os.environ.items() if k != "CODEXCREW_SESSION_KEY"}
        ...
        with (
            ...
            assert env is not None
            assert env["CODEXCREW_CHANNEL_ID"] == "C0ABC123"
            assert "CODEXCREW_SESSION_KEY" not in env
...
async def test_failed_live_spawn_cleanup_releases_workspace_when_kill_is_cancelled(tmp_path):
    ...
    with patch("codex_crew.acp.client.release_bound_agent_workspace", side_effect=record_release):
...
class TestAcpClientBackendSelection:
    ...
    def _reset_claude_cache(self):
        import codex_crew.acp.client as _mod

        _mod._claude_acp_argv_cache = _mod._UNRESOLVED
        _mod._codex_acp_argv_cache = _mod._UNRESOLVED
        yield
        _mod._claude_acp_argv_cache = _mod._UNRESOLVED
        _mod._codex_acp_argv_cache = _mod._UNRESOLVED
    ...
    def _no_cgroup_scope(self):
        ...
        with patch(
            "codex_crew.sandbox._probe_cgroup_scope",
    ...
    async def test_spawn_codex_installs_native_tool_gate(self, tmp_path, monkeypatch):
        ...
        with (
            ...
            assert wrap.call_args.kwargs["native_tool_gate"] is True
            assert wrap.call_args.kwargs.get("extra_hidden_dirs", ()) == ()
            env = mock_exec.call_args.kwargs["env"]
            merged = json.loads(env["CODEX_CONFIG"])
            assert merged["model"] == "gpt-test"
            assert merged["features"] == {"shell": True, "hooks": True}
    ...
    async def test_spawn_claude_backend_missing_bin_raises(self, tmp_path):
        client = AcpClient(work_dir=tmp_path, acp_backend=ACP_BACKEND_CLAUDE)
        with (
            patch("codex_crew.acp.client._resolve_claude_acp_bin", return_value=(None, "")),
    ...
    async def test_spawn_kiro_missing_bin_reports_only_resolver_search_dirs(self, tmp_path):
        ...
        client = AcpClient(work_dir=tmp_path)
        with (
            patch("codex_crew.acp.client._resolve_kiro_bin", return_value=None),
            patch("codex_crew.acp.client.known_kiro_cli_dirs", return_value=searched),
        ):
    ...
    async def test_spawn_kiro_missing_bin_reports_the_environment_it_searched(
        ...
        """
        from codex_crew.acp import client as client_mod
        ...
        with (
            patch(
                "codex_crew.acp.client._resolve_kiro_bin",
                side_effect=_resolve_then_change_path,
            ),
            patch("codex_crew.acp.client.known_kiro_cli_dirs", side_effect=_spy_dirs),
        ):
    ...
    async def test_resolve_kiro_bin_defaults_keep_every_other_caller_unchanged(self, tmp_path):
        ...
        with patch("codex_crew.acp.client.resolve_kiro_cli", return_value=None) as resolve:
...
class TestResolveClaudeAcpBin:
    def test_env_override_wins(self, tmp_path, monkeypatch):
        from codex_crew.acp import client as client_mod
        from codex_crew.acp.client import _resolve_claude_acp_bin
...
class TestResolveCodexAcpBin:
    def test_packaged_adapter_uses_electron_node_runner(self, tmp_path, monkeypatch):
        from codex_crew.acp import client as client_mod

        node_modules = tmp_path / "node_modules"
        entry = node_modules / client_mod._CODEX_ACP_PKG_ENTRY
        entry.parent.mkdir(parents=True)
        entry.write_text("// fake adapter\n", encoding="utf-8")
        (node_modules / client_mod._CODEX_ACP_DEP_MARKER).mkdir(parents=True)
        electron = tmp_path / "codexcrew-desktop.exe"
        electron.write_bytes(b"fake electron runtime")

        monkeypatch.setenv("CODEXCREW_NODE_EXECUTABLE", str(electron))
        monkeypatch.setattr(client_mod, "_vendored_acp_roots", lambda: [node_modules])
        monkeypatch.setattr(client_mod, "_mise_which", lambda tool: None)
        monkeypatch.setattr(client_mod, "_mise_node_installs_dir", lambda: tmp_path / "mise")
        monkeypatch.setattr(client_mod.shutil, "which", lambda name, path=None: None)

        argv, _search_path = client_mod._resolve_codex_acp_bin()

        assert argv == [str(electron.resolve()), str(entry.resolve())]
...
class TestMiseWhich:
    def test_returns_path_on_success(self, tmp_path, monkeypatch):
        from codex_crew.acp import client as client_mod
        from codex_crew.acp.client import _mise_which
...
class TestAcpClientStaleTurn:
    ...
    async def test_recent_stderr_activity_prevents_stale_turn(self, tmp_path, caplog):
        ...
        with patch("codex_crew.acp.client._STALE_TURN_TIMEOUT", 0.2):
            with caplog.at_level("WARNING", logger="codex_crew.acp.client"):
                actions = []
    ...
    async def test_genuine_silence_still_triggers_stale_turn(self, tmp_path, caplog):
        ...
        with patch("codex_crew.acp.client._STALE_TURN_TIMEOUT", 0.05):
            with caplog.at_level("WARNING", logger="codex_crew.acp.client"):
                actions = []
...
class TestAcpClientStaleTurnOracleGate:
    ...
    async def test_working_verdict_defers_stale_turn(self, tmp_path, caplog):
        ...
        with patch("codex_crew.acp.client._STALE_TURN_TIMEOUT", 0.1):
            with caplog.at_level("WARNING", logger="codex_crew.acp.client"):
                actions = []
    ...
    async def test_real_oracle_movement_defers_via_fake_proc(self, tmp_path, caplog):
        ...
        with patch("codex_crew.acp.client._STALE_TURN_TIMEOUT", 0.05):
            with patch("codex_crew.acp.client._READ_TIMEOUT", 0.02):
                with caplog.at_level("WARNING", logger="codex_crew.acp.client"):
                    actions = []
    ...
    async def test_consult_reports_unknown_when_the_submission_itself_fails(self, tmp_path):
        ...
        with patch(
            "codex_crew.acp.client.subprocess_executor",
    ...
    async def test_reset_state_releases_a_consult_from_the_dead_generation(self, tmp_path):
        ...
        with (
            patch("codex_crew.session_pid._pid_gone_or_unmanaged", return_value=True),
            patch("codex_crew.session._untrack_pid"),
            patch("codex_crew.session._untrack_session_pid"),
        ):
            client._reset_state()
        client._pid = 5353
    ...
    async def test_the_submitted_walk_is_bound_to_the_oracle_it_sampled(self, tmp_path):
        ...
        with (
            patch("codex_crew.acp.client.subprocess_executor", return_value=pool),
            patch("codex_crew.acp.client.asyncio.wait_for", _fast_timeout),
        ):
    ...
    async def test_pending_consult_exception_is_consumed_without_a_reset(self, tmp_path):
        ...
        with (
            patch("codex_crew.acp.client.subprocess_executor", return_value=pool),
            patch("codex_crew.acp.client.asyncio.wait_for", _always_times_out),
        ):
            assert await client._consult_liveness_model_wait() == (
                VERDICT_UNKNOWN,
                "oracle offload error",
    ...
    async def test_cancelled_consult_still_consumes_a_later_failure(self, tmp_path):
        ...
        with patch("codex_crew.acp.client.subprocess_executor", return_value=pool):
...
class TestStaleTurnProbeAccounting:
    ...
    * the oracle's baseline-priming answer (``EVIDENCE_SAMPLING``) counted as a
    ...
    async def test_priming_verdict_defers_then_real_evidence_reaps(
        ...
        with patch("codex_crew.acp.client._STALE_TURN_TIMEOUT", 0.0):
            with caplog.at_level("WARNING", logger="codex_crew.acp.client"):
...
class TestAcpClientTrackMetadata:
    ...
    def test_backfill_clamps_malformed_pct(self, tmp_path, monkeypatch):
        ...
        import codex_crew.acp.client as c
        from codex_crew.acp.types import JsonRpcMessage
...
class TestAcpClientTrackUsageUpdate:
    ...
    def test_tokens_carry_forward_across_prompt_reset(self, tmp_path):
        ...
        from codex_crew.acp.types import AcpPromptStats
...
class TestGetChildPids:
    def test_nonexistent_pid(self):
        from codex_crew.acp.client import _get_child_pids
    ...
    def test_recursive_children(self, monkeypatch):
        import codex_crew.acp.client as client_mod
        from codex_crew.acp.client import _get_child_pids
...
class TestIsOurChild:
    def test_nonexistent_pid(self):
        from codex_crew.acp.client import _is_our_child
...
class TestKillEscapedChildren:
    def test_empty_dict(self):
        from codex_crew.acp.client import _kill_escaped_children
...
class TestReadBasename:
    def test_reads_basename_from_ps(self, monkeypatch):
        ...
        import codex_crew.acp.client as client_mod
        from codex_crew.acp.client import _read_basename
...
class TestProcessMessage:
    ...
    def test_compaction_status_classified(self):
        ...
        from codex_crew.acp.types import METHOD_COMPACTION_STATUS, JsonRpcMessage
    ...
    def test_clear_status_classified(self):
        client = self._make_client()
        from codex_crew.acp.types import METHOD_CLEAR_STATUS, JsonRpcMessage
    ...
    def test_agent_switched_classified(self):
        client = self._make_client()
        from codex_crew.acp.types import METHOD_AGENT_SWITCHED, JsonRpcMessage
    ...
    def test_subagent_list_update_classified(self):
        client = self._make_client()
        from codex_crew.acp.types import METHOD_SUBAGENT_LIST_UPDATE, JsonRpcMessage
    ...
    def test_kiro_session_update_classified(self):
        client = self._make_client()
        from codex_crew.acp.types import METHOD_KIRO_SESSION_UPDATE, JsonRpcMessage
    ...
    def test_mcp_oauth_request_classified(self):
        client = self._make_client()
        from codex_crew.acp.types import METHOD_MCP_OAUTH_REQUEST, JsonRpcMessage
    ...
    def test_permission_request_with_colliding_id_not_complete(self):
        ...
        client = self._make_client()
        from codex_crew.acp.types import METHOD_REQUEST_PERMISSION, JsonRpcMessage
...
class TestStreamEventsExtension:
    ...
    async def test_tool_interrupted_marker_send_message_stream_returns(self):
        ...
        from codex_crew.acp.types import UPDATE_AGENT_MESSAGE_CHUNK, JsonRpcMessage
    ...
    async def test_tool_interrupted_sel_contract(self):
        ...
        This test patches codex_crew.sel.sel so the real helper runs, protecting
        ...
        with patch("codex_crew.sel.sel") as mock_sel:
    ...
    async def test_tool_interrupted_all_paths_log_and_return_promptly(
        ...
        caplog.set_level(logging.WARNING, logger="codex_crew.acp.client")
...
class TestDrainStderrRedaction:
    ...
    async def test_raw_stored_redacted_logged(self):
        ...
        with patch("codex_crew.acp.client.logger") as mock_logger:
...
class TestDrainStderrSuppression:
    ...
    async def test_suppressed_lines_still_advance_liveness(self):
        ...
        with patch("codex_crew.acp.client.logger"):
            await client._drain_stderr(reader)
...
class TestEnsureReadyRetryOnAcpError:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("sess-created", AcpError("set_mode failed after session/new")),
        ],
        ids=("session-new-timeout", "post-create-failure"),
    )
    async def test_does_not_retry_an_ambiguous_session_outcome(self, tmp_path, session_id, error):
        client = AcpClient(work_dir=tmp_path)
        spawn_count = 0
        ...
        async def fake_spawn():
            ...
            spawn_count += 1
            client._process = MagicMock(returncode=None, pid=100 + spawn_count, stderr=None)
...
class TestMakeUnifiedDiff:
    ...
    def test_truncation_cuts_at_line_boundary_and_is_marked(self):
        ...
        from codex_crew.acp._dispatch import DIFF_TRUNCATION_MARK
...
class TestDeriveEditDiff:
    ...
    def test_create_content_becomes_addition_diff(self):
        from codex_crew.acp._dispatch import derive_edit_diff
...
class TestStopReasonPopulated:
    ...
    async def test_stop_reason_populated_on_complete(self):
        ...
        from codex_crew.acp.types import EVENT_COMPLETE, JsonRpcMessage
...
class TestSendPipeErrors:
    ...
    async def test_stale_eligible_cleared_by_tool_call(self):
        ...
        from codex_crew.acp.client import AcpTimeoutError
        from codex_crew.acp.types import (
            UPDATE_AGENT_MESSAGE_CHUNK,
            UPDATE_TOOL_CALL,
            JsonRpcMessage,
...
class TestKillProcess:
    ...
    async def test_sigterm_success(self):
        ...
        with (
            ...
            patch("codex_crew.acp.client._get_child_pids", return_value=[]),
            patch("codex_crew.acp.client._kill_escaped_children") as mock_esc,
        ):
    ...
    async def test_sigterm_timeout_then_sigkill(self):
        ...
        with (
            ...
            patch("codex_crew.acp.client._get_child_pids", return_value=[]),
            patch("codex_crew.acp.client._kill_escaped_children"),
        ):
            await client._kill_process()
    ...
    async def test_kill_process_awaits_async_variant_not_sync(self):
        ...
        with (
            patch(
                "codex_crew.platform_compat.kill_process_tree_async",
                new_callable=AsyncMock,
            ) as mock_async,
            patch(
                "codex_crew.platform_compat.kill_process_tree",
...
class TestResetStateExtended:
    ...
    def test_untracks_pids(self):
        ...
        with (
            patch("codex_crew.session_pid._pid_gone_or_unmanaged", return_value=True),
            patch("codex_crew.session._untrack_child_pids") as mock_uc,
            patch("codex_crew.session._untrack_pid") as mock_up,
            patch("codex_crew.session._untrack_session_pid") as mock_usp,
        ):
            client._reset_state()
    ...
    def test_retains_live_root_pid_survivor(self):
        ...
        with (
            patch("codex_crew.session_pid._pid_gone_or_unmanaged", return_value=False),
            patch("codex_crew.session._untrack_pid") as mock_up,
            patch("codex_crew.session._untrack_session_pid") as mock_usp,
        ):
            client._reset_state()
    ...
    def test_untracks_dead_children_retains_live(self):
        ...
        with (
            patch("codex_crew.session_pid._pid_gone_or_unmanaged", side_effect=_gone),
            patch("codex_crew.session._untrack_child_pids") as mock_uc,
        ):
            client._reset_state()
...
class TestInitializeSession:
    ...
    @pytest.mark.asyncio
    async def test_session_resume_timeout_does_not_create_a_new_session(
        self, tmp_path, monkeypatch
    ):
        """A missing load response is ambiguous, so session/new must not follow it."""
        kiro_home = tmp_path / "kiro-home"
        monkeypatch.setenv("KIRO_HOME", str(kiro_home))
        session_dir = kiro_home / "sessions" / "cli"
        session_dir.mkdir(parents=True)
        (session_dir / "slow-sess.json").write_text("{}", encoding="utf-8")

        client = self._make_client(tmp_path)
        client._resume_session_id = "slow-sess"
        ...
        async def fake_wait(req_id, timeout=50.0, *, method="", expected_mcp=None):
            ...
            raise AcpTimeoutError(message="session/load timed out", method="session/load")
...
class TestWaitForResponse:
    ...
    @pytest.mark.asyncio
    async def test_timeout_carries_the_request_method(self, tmp_path):
        client = AcpClient(work_dir=tmp_path)
        client._read_message = AsyncMock(return_value=None)

        with pytest.raises(AcpTimeoutError) as exc_info:
            await client._wait_for_response(1, timeout=0.1, method="session/new")

        assert exc_info.value.method == "session/new"
    ...
    async def test_shutdown_event_raises(self, tmp_path):
        ...
        with patch("codex_crew.shutdown_event") as mock_ev:
...
class TestDispatchEventsExtended:
    ...
    async def test_permission_event_yielded(self):
        from codex_crew.acp.types import EVENT_COMPLETE, EVENT_PERMISSION_REQUEST, JsonRpcMessage
    ...
    async def test_tool_event_yielded(self):
        from codex_crew.acp.types import EVENT_COMPLETE, EVENT_TOOL_CALL, JsonRpcMessage
    ...
    async def test_extract_agent_from_result(self):
        ...
        from codex_crew.acp.types import EVENT_AGENT_SWITCHED, EVENT_COMPLETE, JsonRpcMessage
...
class TestSendCommand:
    ...
    async def test_send_command_redacts_urls(self, tmp_path):
        ...
        with patch(
            "codex_crew.acp.client.redact_exfiltration_urls",
...
class TestCancelSession:
    ...
    async def test_cancel_raises_grace_window_to_budget(self, tmp_path):
        ...
        from codex_crew.acp.client import _CANCEL_GRACE_SECS
...
class TestWaitForCompaction:
    ...
    async def test_buffers_non_compaction_notifications(self, tmp_path, monkeypatch):
        ...
        client = AcpClient(work_dir=tmp_path)
        client._model = "some-model"
        from codex_crew.acp.types import METHOD_COMPACTION_STATUS, METHOD_METADATA, JsonRpcMessage
...
class TestExtractToolEvent:
    ...
    def test_basic_tool_call(self):
        client = AcpClient()
        from codex_crew.acp.types import EVENT_TOOL_CALL, JsonRpcMessage
...
class TestBuildPermissionEvent:
    ...
    def test_basic_permission(self):
        client = AcpClient()
        from codex_crew.acp.types import EVENT_PERMISSION_REQUEST, JsonRpcMessage
    ...
    def test_every_deny_vocabulary_value_round_trips(self):
        ...
        from codex_crew.acp._dispatch import (
            _DENY_BEHAVIORS,
            _DENY_OPTION_IDS,
            reject_option_id,
        )
    ...
    def test_contradictory_kind_wins_over_deny_metadata(self):
        ...
        from codex_crew.acp._dispatch import reject_option_id
...
class TestApproveTool:
    ...
    async def test_always_uses_recorded_optionid(self, tmp_path):
        from codex_crew.acp.types import OUTCOME_SELECTED
...
class TestRejectTool:
    ...
    async def test_falls_back_to_cancelled_when_no_reject_option(self, tmp_path):
        ...
        from codex_crew.acp.types import OUTCOME_CANCELLED
...
class TestToolStallWatchdog:
    ...
    async def test_dispatched_tool_with_no_data_exits(self, tmp_path, monkeypatch):
        from codex_crew.acp import client as acp_client
...
class TestSendMessageStreamBranches:
    ...
    async def test_metadata_tracked(self):
        from codex_crew.acp.types import METHOD_METADATA, JsonRpcMessage
...
class TestExtractToolCallUpdate:
    ...
    def test_unrenderable_content_entries_are_logged_with_shape(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="codex_crew.acp.client"):
            assert client._extract_tool_call_update(msg) is None
...
class TestDispatchToolResultContentShapes:
    ...
    def test_bare_text_block_in_content_is_accepted(self):
        from codex_crew.acp._dispatch import _build_tool_result_event
    ...
    def test_unrenderable_content_entries_are_logged_with_shape(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="codex_crew.acp._dispatch"):
            assert (
    ...
    def test_a_newline_bearing_tool_id_cannot_forge_a_log_line(self, caplog):
        ...
        forged = "tc-1\nWARNING codex_crew: drive deleted by owner"
    ...
    def test_every_documented_known_content_type_is_renderable_or_silent(self):
        ...
        from codex_crew.acp._dispatch import _KNOWN_TOOL_CONTENT_TYPES, unrenderable_content_shapes
    ...
    def test_a_numeric_tool_id_does_not_crash_the_turn(self, caplog):
        ...
        from codex_crew.acp._dispatch import _build_tool_result_event, redacted_tool_id
    ...
    def test_both_diagnostics_are_bounded(self):
        ...
        from codex_crew.acp._dispatch import redacted_tool_id, unrenderable_content_shapes
    ...
    def test_collection_stops_early_rather_than_building_the_whole_join(self):
        ...
        """
        from codex_crew.acp._dispatch import unrenderable_content_shapes
...
class TestExtractToolCallRefinement:
    ...
    def test_prefers_rawinput_description_over_title(self):
        ...
        # is the human-readable purpose ("List CodexCrew dashboard module
        ...
        msg = self._make_msg(
            {
                ...
                "rawInput": {
                    ...
                    "description": "List CodexCrew dashboard module files",
                },
            }
        )
        event = client._extract_tool_call_refinement(msg)
        assert event is not None
        assert event.title == "List CodexCrew dashboard module files"
...
class TestCaptureAvailableModels:
    ...
    def test_capture_delegates_to_canonical_parser(self, monkeypatch):
        ...
        ``codex_crew.acp.client.parse_advertised_models`` instead.
        """
        from codex_crew.acp import session_handle as sh
...
class TestWaitForResponseActivityDeadline:
    ...
    async def test_streaming_notifications_extend_deadline(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.acp.client.time.monotonic", fake_monotonic)
...
class TestFormatAcpError:
    ...
    def test_model_not_available_rewrite(self):
        ...
        # Must NOT point at ~/.claude/settings.json — CodexCrew never reads it;
    ...
    def test_advertised_model_still_reads_as_capacity(self):
        from codex_crew.acp.client import _is_transient_raw_error
...
class TestIsTransientRawError:
    ...
    def test_model_capacity_rejection_shapes_are_transient(self):
        """Every shape a model-capacity rejection arrives in must be retryable.

        The backend reports one capacity failure under several renderings — a
        CamelCase exception name, a structured reason code, the nested 5xx
        exception, and user-facing prose. Any rendering left unmatched reads as
        terminal, which strands the turn with no output instead of retrying or
        falling back to the configured second model.
        """
        from codex_crew.acp.client import _is_transient_raw_error

        # Word-boundary anchors cannot see "Throttle" inside a CamelCase name,
        # so this has to match as a named exception.
        assert _is_transient_raw_error({"data": "ModelThrottleError"}) is True
        # Machine reason code, which survives prose rewording.
        assert _is_transient_raw_error({"data": "MODEL_TEMPORARILY_UNAVAILABLE"}) is True
        # The nested cause the throttle wrapper carries.
        assert _is_transient_raw_error({"data": "InternalServerException"}) is True
        # The backend's own phrasing, which names neither throttle nor rate limit.
        assert (
            _is_transient_raw_error(
                {"data": "The model you've selected is experiencing a high volume of traffic."}
            )
            is True
        )

    def test_capacity_shapes_lose_to_terminal_classes(self):
        """Capacity wording must not reclaim an error a retry cannot fix.

        Usage-limit and auth rejections are checked ahead of the throttle and
        5xx families, so a co-occurring capacity token must leave them terminal
        rather than re-arming the retry ladder against a spent allowance or a
        rejected credential.
        """
        from codex_crew.acp.client import _is_transient_raw_error
        ...
        assert (
            _is_transient_raw_error(
                {
                    "data": "You have reached your monthly usage limit",
                    "message": "ModelThrottleError",
                }
            )
            is False
        )
        assert (
            _is_transient_raw_error(
                {"data": "AccessDeniedException", "message": "MODEL_TEMPORARILY_UNAVAILABLE"}
            )
            is False
        )
    ...
    def test_raise_acp_error_carries_transient_flag_and_formatted_message(self):
        ...
        from codex_crew.acp.client import AcpError, _raise_acp_error
...
class TestResolveKiroBinEnvOverride:
    """_resolve_kiro_bin honors the CODEXCREW_KIRO_BIN override for environments
    (e.g. AgentSpaces/DevSpaces) where the toolbox shim is broken."""

    def test_env_override_used_when_valid(self, tmp_path):
        from codex_crew.acp.client import _resolve_kiro_bin
        from codex_crew.kiro_prerequisite import TrustedAcpExecutableSnapshot
        ...
        with (
            patch(
                "codex_crew.acp.client.resolve_kiro_cli",
                return_value=str(fake),
            ),
            patch(
                "codex_crew.kiro_prerequisite.snapshot_trusted_acp_executable",
    ...
    def test_resolver_returns_installed_path_without_copying(self, tmp_path):
        ...
        from codex_crew.acp import client as client_module
    ...
    async def test_spawn_launches_self_updated_override(self, tmp_path):
        from codex_crew import sandbox as sandbox_module
        from codex_crew.acp import client as client_module
        from codex_crew.kiro_prerequisite import KiroPrerequisiteService
        ...
        mock_exec = AsyncMock()
        with (
            patch.dict("os.environ", {"CODEXCREW_KIRO_BIN": str(fake)}),
    ...
    def test_env_override_ignored_when_missing_file(self, tmp_path):
        ...
        with (
            patch.dict("os.environ", {"CODEXCREW_KIRO_BIN": missing}),
            patch(
                "codex_crew.acp.client.resolve_kiro_cli",
                return_value=None,
            ),
    ...
    def test_env_override_ignored_when_not_executable(self, tmp_path):
        ...
        with (
            patch.dict("os.environ", {"CODEXCREW_KIRO_BIN": str(nonexec)}),
            patch(
                "codex_crew.acp.client.resolve_kiro_cli",
                return_value=None,
            ),
    ...
    def test_no_env_falls_through(self):
        ...
        env = {k: v for k, v in os.environ.items() if k != "CODEXCREW_KIRO_BIN"}
        with (
...
class TestTrackMetadataWindowResolution:
    ...
    def test_kiro_cached_non_registry_model_backfills_real_window(self):
        ...
        import codex_crew.model_registry as mr
...
class TestSubstitutionAdvisory:
    ...
    requested model for another. The substitute is already live, so CodexCrew
...
class TestAcpClientIsShellSignal:
    ...
    def test_permission_event_does_not_promote_title_on_identity_cache_miss(self, tmp_path):
        ...
        from codex_crew.acp.client import AcpClient
        from codex_crew.acp.types import JsonRpcMessage
        from codex_crew.trust_patterns import approval_command
    ...
    def test_end_to_end_long_shell_title_passes_validation(self, tmp_path):
        ...
        from codex_crew.acp.client import AcpClient
        from codex_crew.dashboard.chat_utils import _validate_tool_name
    ...
    def test_is_shell_kind_is_none_safe(self):
        ...
        from codex_crew.acp.client import _is_shell_kind
    ...
    def test_is_shell_propagates_through_to_llm_event(self):
        ...
        from codex_crew.acp.types import AcpEvent
        from codex_crew.providers.acp import AcpProvider
        from codex_crew.providers.base import EVENT_PERMISSION_REQUEST
...
class TestSpawnEnvScrub:
    ...
    async def test_client_spawn_scrubs_sensitive_env_on_default_auto(self, monkeypatch):
        ...
        monkeypatch.setenv("CODEXCREW_OWNER_ID", "U_FAKE_OWNER")
        ...
        monkeypatch.setenv("CODEXCREW_UNRELATED_KEEPME", "keep-this-value")
        ...
        assert env.get("CODEXCREW_UNRELATED_KEEPME") == "keep-this-value"
...
class TestModelEntitlementPreflight:
    ...
    def _client(advertised, model, is_claude=False):
        from codex_crew.acp.client import ACP_BACKEND_CLAUDE, AcpClient
    ...
    async def test_startup_withholds_unusable_model(self):
        ...
        """
        from codex_crew.acp.client import DEFAULT_MODEL
...
class TestCompactionFailureDetail:
    ...
    def test_prefers_a_named_reason(self):
        from codex_crew.acp.client import compaction_failure_detail
    ...
    def test_is_bounded_and_redacted(self):
        from codex_crew.acp.client import _COMPACTION_DETAIL_MAX_CHARS, compaction_failure_detail
...
class TestKasSummarizationFailureLogging:
    ...
    @staticmethod
    def _bare_handle():
        from codex_crew.acp.session_handle import AcpSessionHandle
    ...
    def test_the_frame_is_redacted_before_it_reaches_the_log(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="codex_crew.acp.session_handle"):
...
class TestCompactionVerdictReachesTheConsumer:
    ...
    def test_the_session_provider_forwards_the_verdict_from_its_handle(self):
        ...
        from codex_crew.acp.session_provider import AcpSessionProvider
...
class TestCompactionFailureIsTransient:
    ...
    def test_a_throttled_summarization_call_is_transient(self):
        from codex_crew.acp.client import compaction_failure_is_transient
