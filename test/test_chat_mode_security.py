...
``api_chat_mode`` (``src/codex_crew/dashboard/chat_handlers.py``) carried three
...
from codex_crew.dashboard.chat_handlers import api_chat_mode
from codex_crew.safety_override import (
    reset_singleton,
)
from codex_crew.safety_override import safety_override as real_safety_override
...
def _hermetic_home(tmp_path, monkeypatch):
    """Redirect CODEXCREW_HOME so SEL writes never touch the developer's home."""
...
async def test_trust_reads_unknown_slot_is_400_and_revokes_nothing(state) -> None:
    ...
    with patch("codex_crew.dashboard.chat_handlers.safety_override", return_value=override):
...
async def test_rejected_trust_reads_does_not_touch_existing_slots(state) -> None:
    ...
    state.get_or_create_slot("s2")
    with patch(
        "codex_crew.dashboard.chat_handlers.safety_override",
