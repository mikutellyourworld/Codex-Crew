...
``codexcrew doctor`` and a silent bot. The answer is derived from descriptor DATA
...
from codex_crew.channels import ChannelReadiness, builtin_channel_descriptors, channel_readiness
...
def _credentials_the_gateway_requires() -> dict[str, set[str]]:
    ...
    src = (Path(codex_crew_pkg.__file__).parent / "slack" / "gateway.py").read_text(encoding="utf-8")
    tree = ast.parse(src)
...
class TestTheServicesPageCoversEveryChannel:
    ...
    def _label_map_keys() -> set[str]:
        ...
        # From THIS file, not from the package: `codex_crew.__file__` points into
