...
from codex_crew.vector_memory import (
    _HAS_FAISS,
    _HAS_NUMPY,
    _MMR_MAX_POOL,
    SemanticRejectCode,
...
class TestLogRejectEvent:
    ...
    def test_audited_reject_set_is_bounded(self, tmp_path: Path) -> None:
        ...
        store = VectorMemoryStore(db_path=tmp_path / "mem.db")
        store.init()
        with patch("codex_crew.vector_memory._MAX_AUDITED_REJECTS", 0):
            with patch.object(store, "_log_event") as mock_log:
...
class TestConflictResolution:
    ...
    def test_preserving_writes_respect_cap_across_store_instances(
        ...
        monkeypatch.setattr("codex_crew.vector_memory.uuid4", synchronized_uuid)
...
class TestSchemaInit:
    ...
    def test_creation_routes_through_restrict_to_owner(
        ...
        """
        from codex_crew import vector_memory as vm
...
class TestFaissDimMismatch:
    ...
    def test_mismatched_dim_skipped_not_crashed(self, tmp_path: Path, monkeypatch) -> None:
        ...
        import codex_crew.vector_memory as vm_mod
...
class TestEmbeddingDimPlumbing:
    ...
    def test_learn_cmd_passes_embedding_dim(self, tmp_path: Path, monkeypatch) -> None:
        ...
        with (
            patch("codex_crew.cli_commands.CodexCrewConfig.load", return_value=mock_cfg),
    ...
    def test_dashboard_fallback_passes_embedding_dim(self, tmp_path: Path, monkeypatch) -> None:
        ...
        import codex_crew.dashboard.handlers.memory as mem_mod
        ...
        # _get_vector_store does lazy `from codex_crew.config.loader import CodexCrewConfig`
        # and `from codex_crew.vector_memory import VectorMemoryStore` inside the function.
        ...
        monkeypatch.setattr(loader_mod, "CodexCrewConfig", mock_config_cls)
...
class TestDedupThresholdPlumbing:
    ...
    def _find_unplumbed_sites(tree, where: str = "") -> list[str]:
        ...
        Structural rather than behavioural for the gateway and `codexcrew run`:
    ...
    def test_production_sites_pass_the_configured_threshold(self) -> None:
        ...
        pkg_root = _Path(inspect.getfile(codex_crew)).parent
        violations: list[str] = []
...
class TestDbLockGuard:
    ...
    def test_every_db_statement_is_lock_serialized(self) -> None:
        ...
        from codex_crew import vector_memory
...
class TestHandlerOffload1947:
    ...
    calls), and the scan covers every module in the ``codex_crew`` package.
    """
    ...
    def _package_root() -> Path:
        ...
        return Path(codex_crew.__file__).resolve().parent
...
class TestSemanticWriteTimeEmbedding:
    ...
    def test_write_time_embed_uses_bulk_priority(self, tmp_path: Path) -> None:
        from codex_crew.embeddings import PRIORITY_BULK
...
class TestPromotionSkipIsObservable:
    ...
    def test_rejected_promotion_is_logged_and_counted(self, tmp_path: Path, caplog) -> None:
        ...
        with (
            ...
            caplog.at_level(logging.INFO, logger="codex_crew.vector_memory"),
        ):
...
class TestSanitizeDecayRates:
    ...
    def test_ignores_non_numeric_with_warning(self, caplog: pytest.LogCaptureFixture) -> None:
        with caplog.at_level(logging.WARNING, logger="codex_crew.vector_memory"):
...
class TestDedupThresholdLoaderValidation:
    ...
    def _memory_section(self, tmp_path: Path, monkeypatch, raw_value: object):
        ...
        return CodexCrewConfig.load().memory
