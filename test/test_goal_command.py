...
import codex_crew.dashboard.chat_runner as chat_runner


def _make_slot(key: str = "slot-1", agent: str = "codexcrew") -> MagicMock:
    slot = MagicMock()
    slot.key = key
    slot.agent = agent
...
async def test_arm_default_budget(
    ...
    # ~/.codex-crew). data_home() reads CODEXCREW_HOME (pinned elsewhere by
    # conftest), so redirect it to track the patched home and keep the
    # ~/.codexcrew/goal-stop layout this test builds authoritative.
    monkeypatch.setattr(chat_runner, "data_home", lambda: tmp_path / ".codexcrew")
    slot, state = _make_slot(key="a/b:c"), _make_state()
    stale_sentinel = tmp_path / ".codexcrew" / "goal-stop" / "a_b_c.stop"
