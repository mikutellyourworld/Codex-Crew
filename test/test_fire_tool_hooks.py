...
class TestScriptHookStoreFire:
    ...
    async def test_fire_emits_subagent_id_when_set(self, fire_store: ScriptHookStore):
        with patch("codex_crew.hooks.run_script_hook", new_callable=AsyncMock) as mock_run:
...
class TestScriptHookStoreStopContext:
    ...
    The env var ``CODEXCREW_HOOK_CONTEXT`` is capped at 500 chars (ARG_MAX
...
class TestRunScriptHookStopEnvCap:
    ...
    async def test_stop_env_context_capped_but_stdin_full(self) -> None:
        """Stop hook: CODEXCREW_HOOK_CONTEXT env is capped at 500; stdin JSON is full.
        ...
        with (
            patch("codex_crew.sandbox.wrap_argv", lambda argv, *a, **k: (argv, None)),
            patch("codex_crew.sandbox.cgroup_scope_argv", lambda argv: argv),
        ...
        env_ctx = captured["env"]["CODEXCREW_HOOK_CONTEXT"]
