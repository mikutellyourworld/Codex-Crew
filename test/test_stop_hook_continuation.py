...
from codex_crew.dashboard.chat_utils import SYNTHETIC_RECOVERY_KIND, is_synthetic_recovery_item
...
from codex_crew.hooks import HOOK_EVENT_STOP, ScriptHookStore
...
class TestRunnerWiring:
    ...
    async def test_a_stop_during_pre_turn_await_suppresses_the_continuation(self, tmp_path) -> None:
        ...
        with patch(
            "codex_crew.dashboard.chat_runner.expire_slack_options",
...
def _sandbox_backend_available() -> bool:
    ...
    """
    try:
        from codex_crew.sandbox import detect_backend
...
class TestStopHookContinuationCount:
    ...
    async def _fire_capturing_event(tmp_path, event: str, **fire_kwargs) -> dict:
        ...
        with patch("codex_crew.hooks.run_script_hook", side_effect=_capture):
    ...
    async def test_non_stop_events_carry_no_continuation_fields(self, tmp_path) -> None:
        ...
        from codex_crew.hooks import HOOK_EVENT_USER_PROMPT_SUBMIT
...
class TestStopHookNudgeCap:
    ...
    def _capped_load(cap: int):
        """A CodexCrewConfig.load replacement pinning the cap, fresh per call.
    ...
    async def _run_blocking_turn(self, tmp_path, cap: int, start_depth: int):
        ...
        with (
            patch(
                "codex_crew.dashboard.chat_runner.CodexCrewConfig.load",
    ...
    async def test_pending_queued_continuations_count_against_the_cap(self, tmp_path) -> None:
        ...
        """
        from codex_crew.dashboard.chat_utils import SYNTHETIC_RECOVERY_KIND
