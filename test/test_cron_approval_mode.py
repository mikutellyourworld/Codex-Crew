...
class TestCronApprovalModeGateway:
    ...
    def _run_cron_callback(self, approval_mode: str) -> dict:
        ...
        with patch("codex_crew.slack.gateway.stream_and_collect", fake_stream), patch(
            "codex_crew.slack.gateway.CronService"
        ) as mock_cron_cls:
...
class TestCronApprovalModeExtraEnv:
    """Cron dispatch injects CODEXCREW_APPROVAL_MODE into the spawned process
    ...
    def _run_cron_callback_capture_get_or_create(
        ...
        with patch("codex_crew.slack.gateway.stream_and_collect", AsyncMock(return_value="done")), patch(
            "codex_crew.slack.gateway.CronService"
        ) as mock_cron_cls:
    ...
    def test_auto_mode_injects_approval_mode_env_var(self) -> None:
        ...
        assert captured["extra_env"] == {"CODEXCREW_APPROVAL_MODE": "auto"}
    ...
    def test_interactive_job_cannot_smuggle_auto_via_job_env(self) -> None:
        # Security: CODEXCREW_APPROVAL_MODE is a RESERVED control var. An
        ...
        captured = self._run_cron_callback_capture_get_or_create(
            "", job_env={"CODEXCREW_APPROVAL_MODE": "auto", "SOME_OTHER_VAR": "x"}
        )
        assert captured["extra_env"] == {"SOME_OTHER_VAR": "x"}
    ...
    def test_auto_job_env_does_not_double_inject(self) -> None:
        ...
        captured = self._run_cron_callback_capture_get_or_create(
            "auto", job_env={"CODEXCREW_APPROVAL_MODE": "auto"}
        )
        assert captured["extra_env"] == {"CODEXCREW_APPROVAL_MODE": "auto"}
...
class TestCronApprovalModeValidation:
    ...
    def _simulate_tool_call(self, tool_name: str, arguments: dict) -> str:
        from codex_crew.mcp_cron import _call_tool

        return _call_tool(tool_name, arguments)

    def test_valid_auto(self) -> None:
        with patch("codex_crew.mcp_cron.CronService") as mock_svc:
            job = MagicMock()
            job.id = "v1"
            job.name = "test"
...
class TestSubagentInheritsPolicy:
    ...
    def _run_inner_and_capture(self, parent_policy: str, parent_session_key: str = "parent-key") -> dict:
        ...
        from codex_crew.subagent import SubagentInfo, SubagentManager
    ...
    def test_empty_policy_flows_to_child_session(self) -> None:
        ...
        with patch(
            "codex_crew.subagent.CodexCrewConfig.load",
    ...
    def _run_inner_with_tool_event(self, parent_policy: str, on_tool_approval=None, parent_session_key: str = "parent-key") -> MagicMock:
        ...
        from codex_crew.hooks import TOOL_ALLOW, ToolHookResult
        from codex_crew.providers.base import EVENT_COMPLETE, EVENT_PERMISSION_REQUEST, LLMEvent
        from codex_crew.subagent import SubagentInfo, SubagentManager
        ...
        with patch("codex_crew.subagent.sel"):
            asyncio.run(runner._run_inner(info, "subagent:sub1"))
        return mock_client
...
class TestCronSubagentInjection:
    ...
    def _init_and_get_done_cb(self, gw):  # type: ignore[no-untyped-def]
        ...
        with patch("codex_crew.slack.gateway.SubagentManager") as mock_cls:
    ...
    def _patches(self, stream_rv="ok"):  # type: ignore[no-untyped-def]
        return (
            ...
            patch(
                "codex_crew.slack.gateway.redact_exfiltration_urls",
                return_value=(stream_rv, False),
            ),
            patch(
                "codex_crew.slack.gateway.redact_credentials",
                return_value=(stream_rv, False),
            ),
        )
...
class TestNoCronsFlag:
    ...
    def test_no_crons_skips_start(self) -> None:
        ...
        with patch("codex_crew.slack.gateway.CronService") as mock_cls:
            svc = MagicMock()
            svc.start = AsyncMock()
    ...
    def test_default_starts_crons_after_app_reconcile(self) -> None:
        ...
        with patch("codex_crew.slack.gateway.CronService") as mock_cls, patch(
            "codex_crew.apps.bridges.reconcile_app_crons_for_execution",
    ...
    def test_run_gateway_passes_no_crons(self) -> None:
        ...
        with (
            ...
            patch("codex_crew.slack.gateway.ensure_agents_slice_limits", return_value=True),
            patch("codex_crew.slack.gateway.GatewayOrchestrator") as mock_cls,
        ):
            mock_orch = MagicMock()
    ...
    def test_cli_gateway_passes_no_crons(self) -> None:
        ...
        with patch("codex_crew.cli_server.config_path") as mock_cp, patch(
            "codex_crew.cli_server.CodexCrewConfig"
        ) as mock_cfg_cls, patch(
            "codex_crew.cli_server.run_gateway", new_callable=AsyncMock
        ) as mock_run:
    ...
    def test_cli_argparse_no_crons_flag(self) -> None:
        ...
        with patch.object(sys, "argv", ["codexcrew", "gateway", "--no-crons"]):
            from codex_crew.cli import main

            with patch("codex_crew.cli_server._gateway", new_callable=AsyncMock) as mock_gw, patch(
                "codex_crew.cli.asyncio"
            ) as mock_asyncio:
...
class TestSubagentRoleModelForcesDedicatedPath:
    ...
    def _run(self, *, role_models=None, role_efforts=None):
        from codex_crew.config.loader import AgentConfig, CodexCrewConfig
        from codex_crew.providers.base import EVENT_COMPLETE, LLMEvent
        from codex_crew.subagent import SubagentInfo, SubagentManager
        ...
        ), patch("codex_crew.config.loader.CodexCrewConfig.load", classmethod(lambda c: cfg)):
            asyncio.run(runner._run_inner(info, "subagent:sub1"))
