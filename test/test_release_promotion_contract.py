...
PROMOTION_ARTIFACT = "CodexCrew-notarized-stable-${{ needs.version.outputs.version }}"
PROMOTION_ARTIFACT_FORMAT = (
    "format('CodexCrew-notarized-stable-{0}', needs.version.outputs.version)"
)
...
def test_stable_rebuild_is_the_default_and_promotion_is_opt_in_per_version() -> None:
    ...
    stamp in the wheel name, the feed, ``pip show`` and ``codexcrew --version``.
...
def test_the_windows_installer_is_chosen_per_mode_not_by_extension() -> None:
    ...
    own bytes sit in the gated bundle under ``CodexCrew-Setup.exe``.
    ...
    assert '"${NOTARIZED_DIR}/CodexCrew-Setup.exe"' in assemble
...
def test_github_release_selects_explicit_versioned_macos_handoff() -> None:
    ...
    assert (
        'NOTARIZED_DIR="artifacts/CodexCrew-notarized-${{ needs.version.outputs.channel }}-'
        '${{ needs.version.outputs.version }}"' in assemble
    )
    assert "CodexCrew-notarized-stable-promotion" not in assemble
    assert "*CodexCrew-notarized-*" not in assemble
