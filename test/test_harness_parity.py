...
Codex Crew drives one first-class harness, ``kiro-cli``, and adapts the others.
...
from codex_crew import acp_backends
from codex_crew.acp import client as acp_client
from codex_crew.acp import runtime as acp_runtime
...
from codex_crew.config.loader import AgentConfig, _normalize_acp_backend
from codex_crew.providers import acp as providers_acp
...
def test_codex_is_the_default_backend() -> None:
    """H1: configuring nothing yields the primary Codex harness."""
    assert _field_default("acp_backend") == ACP_BACKEND_CODEX


def test_codex_is_always_selectable() -> None:
    """H1: the Codex harness is never gated behind a preview flag or an edition.
...
def test_unselectable_backend_degrades_to_codex(persisted: object) -> None:
    """H3: an unusable persisted value degrades to Codex and never raises.
    ...
    if persisted not in selectable_backends():
        assert resolved == ACP_BACKEND_CODEX
...
def test_registering_a_backend_makes_it_survive_load() -> None:
    ...
    try:
        acp_backends._baseline.discard(ACP_BACKEND_OPENAI_COMPATIBLE)
        acp_backends._selectable.discard(ACP_BACKEND_OPENAI_COMPATIBLE)
        assert _normalize_acp_backend(ACP_BACKEND_OPENAI_COMPATIBLE) == ACP_BACKEND_CODEX

        acp_backends.register_selectable_backend(ACP_BACKEND_OPENAI_COMPATIBLE)
        assert (
            _normalize_acp_backend(ACP_BACKEND_OPENAI_COMPATIBLE)
            == ACP_BACKEND_OPENAI_COMPATIBLE
        )
    finally:
    ...
    assert _normalize_acp_backend(ACP_BACKEND_CLAUDE) == ACP_BACKEND_CODEX
...
def test_config_load_never_reads_the_platform_context(monkeypatch) -> None:
    ...
    from inside ``CodexCrewConfig.load()`` re-enters that load and recurses to the
    ...
    """
    from codex_crew.platform import context as pc
...
def test_session_sharing_is_opt_in() -> None:
    ...
    assert ACP_BACKENDS_SESSION_SHARING == frozenset()
...
def test_steer_capability_declares_its_stamp() -> None:
    ...
    """
    from codex_crew.acp.session_provider import AcpSessionProvider  # noqa: F401
    from codex_crew.providers.acp import AcpProvider  # noqa: F401
...
def test_is_kiro_cli_is_positive() -> None:
    ...
    assert ACP_BACKENDS_INTERNAL_SANDBOX == frozenset()
...
def test_every_known_backend_has_a_label() -> None:
    ...
    labels = {
        ...
        ACP_BACKEND_OPENAI_COMPATIBLE: PROVIDER_LABEL_OPENAI_COMPATIBLE,
    }
...
def test_codex_is_selectable_and_answerable() -> None:
    ...
    """
    from codex_crew.agent_sdk.backend_install import _PROBES
...
def test_codex_tool_calls_are_gated_before_it_is_offered() -> None:
    ...
    """
    from codex_crew import acp_tool_gate
...
def test_effort_channel_is_opt_in() -> None:
    ...
    assert ACP_BACKENDS_KIRO_SLASH_COMMANDS == frozenset()
