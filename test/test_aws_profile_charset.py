...
shape into ``codex_crew.constants.AWS_PROFILE_NAME_RE`` as the single source of
truth. These tests pin every consumer:

* ``codex_crew.cloud.ec2._PROFILE_SPEC`` (EC2 wizard — aliases
  ``profiles.PROFILE_SPEC``)
* ``codex_crew.deploy.profiles._PROFILE_RE`` (deploy profile registry + the
  ``aws configure list-profiles`` discovery filter)
* ``codex_crew.deploy.handlers._PROFILE_SPEC`` (deploy-web HTTP boundary —
  aliases ``profiles.PROFILE_SPEC``)
* ``codex_crew.validation._WM_PROFILE_RE`` (workspace-manager webapp_metadata)
* ``codex_crew.instances.validation._AWS_PROFILE_RE`` (SSM tunnel inputs;
  ``instances.registry`` aliases it for its early record check)
* ``codex_crew.aws_consent._PROFILE_RE`` (identity probe — a DELIBERATE
...
from codex_crew import aws_consent, constants
from codex_crew import validation as validation_mod
from codex_crew.cloud import aws as cloud_aws
from codex_crew.cloud import ec2
from codex_crew.deploy import engine as engine_mod
from codex_crew.deploy import handlers
from codex_crew.deploy import profiles as profiles_mod
from codex_crew.instances import registry as instances_registry
from codex_crew.instances import validation as instances_validation
