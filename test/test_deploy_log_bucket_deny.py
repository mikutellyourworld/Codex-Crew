...
CWE-732: the S3BucketLevel/S3ObjectLevel Allow grants use the codexcrew-deploy-*
wildcard, which also matches the audit bucket codexcrew-deploy-logs-* (CloudTrail
...
from codex_crew.deploy.iam import policy_document
...
def test_policy_denies_audit_log_bucket_tamper():
    ...
    for tier in ("static", "fullstack"):
        ...
        assert res and all("codexcrew-deploy-logs-" in r for r in res), res
