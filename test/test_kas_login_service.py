...
from codex_crew.auth.login import device
from codex_crew.auth.login.device import DeviceAuthorization
from codex_crew.auth.service import KasLoginService, UnknownLoginError, _parse_provider
from codex_crew.auth.store import KasToken, SocialProvider, TokenStore
...
async def test_poll_authorized_store_write_failure_is_error(tmp_path, monkeypatch):
    ...
    def _boom(_token):
        from codex_crew.auth.store import TokenStoreError
...
from codex_crew.auth.login import builder_id  # noqa: E402
from codex_crew.auth.service import MissingStartUrlError  # noqa: E402
...
async def test_begin_oidc_rejects_url_metacharacter_regions(tmp_path):
    ...
    """
    service, _ = _service(tmp_path)
    from codex_crew.auth.service import InvalidRegionError
