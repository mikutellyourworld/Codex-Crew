...
def test_multiple_agents_sorted(agents_dir):
    ...
    _write_agent(agents_dir, "codexcrew.json", {"name": "codexcrew"})
    assert _get_agent_names() == ["alpha", "codexcrew", "zebra"]
...
def test_non_utf8_agent_file_does_not_crash(agents_dir):
    ...
    old (json.JSONDecodeError, OSError) except and killed the `/codexcrew
