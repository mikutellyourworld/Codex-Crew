...
def _load_with_session(session_block: dict) -> CodexCrewConfig:
...
def test_a_threshold_change_reaches_an_already_live_manager() -> None:
    ...
    ``codexcrew config set``, which never contacts the gateway -- take effect
    without a restart.
    """
    from codex_crew.config.loader import publish_autocompact_pct
...
def test_load_publishes_the_threshold_it_resolved() -> None:
    ...
    """
    from codex_crew.config.loader import published_autocompact_pct
...
def test_an_older_load_cannot_republish_over_a_newer_one() -> None:
    ...
    already changed.
    """
    from codex_crew.config.loader import publish_autocompact_pct, published_autocompact_pct
    ...
    newer = CodexCrewConfig()
    newer.session.autocompact_pct = 55.0
    older = CodexCrewConfig()
