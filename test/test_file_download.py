...
async def test_text_file_redacted_blocks_download(tmp_path, mock_sel):
    ...
    redact_path = "codex_crew.dashboard.handlers.files.redact"
    with patch("codex_crew.dashboard.handlers._validate_dashboard_path", return_value=str(f)), \
...
async def test_oversize_file_rejected(tmp_path, mock_sel):
    ...
    with patch("codex_crew.dashboard.handlers._validate_dashboard_path", return_value=str(f)), \
         patch("codex_crew.dashboard.handlers.files._MAX_UPLOAD_BYTES", 512):
        async with TestClient(TestServer(_make_app())) as client:
            resp = await client.get(f"/api/file-download?path={f}")
...
async def test_resolve_relative_path_within_project(tmp_path, mock_sel, monkeypatch):
    ...
    with patch("codex_crew.dashboard.handlers._validate_dashboard_path", return_value=str(f.resolve())):
        async with TestClient(TestServer(_make_app())) as client:
