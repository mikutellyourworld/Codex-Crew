...
class TestConfigOverlayLoad:
    ...
    def test_local_overlay_merges_on_load(self, tmp_path: Path) -> None:
        ...
        with patch("codex_crew.config.loader.config_dir", return_value=config_dir):
    ...
    def test_world_writable_warning_fires(self, tmp_path: Path, caplog) -> None:
        ...
        with (
            ...
            caplog.at_level(logging.WARNING, logger="codex_crew.config.loader"),
        ):
            cfg = CodexCrewConfig.load()
...
class TestDictSetCreate:
    ...
    def test_creates_intermediate_dicts(self) -> None:
        from codex_crew.cli_config import _dict_set_create
...
class TestCliConfigSetLocal:
    ...
    def test_local_set_writes_to_config_local_json(self, tmp_path: Path) -> None:
        ...
        with patch(
            "codex_crew.cli_config.config_local_path", return_value=config_dir / "config.local.json"
        ):
            with patch("codex_crew.cli_config.sel"):
                _config_cmd(args)
