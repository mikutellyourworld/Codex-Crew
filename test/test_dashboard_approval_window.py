...
from codex_crew.dashboard import turn_dispatch as td
...
def cfg(monkeypatch: pytest.MonkeyPatch):
    ...
    def _apply(*, window: int, turn: int = 7200) -> None:
        monkeypatch.setattr(
            td.CodexCrewConfig, "load", staticmethod(lambda: _Cfg(window=window, turn=turn))
        )
...
class TestResolver:
    ...
    def test_falls_back_when_config_unavailable(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(td.CodexCrewConfig, "load", staticmethod(_boom))
