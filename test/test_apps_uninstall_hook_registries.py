...
import codex_crew.apps.teardown as teardown
...
class TestUninstallDropsTheHooks:
    async def _uninstall(self) -> None:
        ...
        with (
            ...
            patch(
                "codex_crew.apps.routes.uninstall_app",
            ...
            patch(
                "codex_crew.apps.teardown.on_app_disable",
                new_callable=AsyncMock,
                return_value=None,
            ),
            patch("codex_crew.apps.routes.sel", return_value=MagicMock()),
            patch(
                "codex_crew.apps.routes.classify_and_clean_for_uninstall",
                return_value={"removable": [], "shared": [], "userInstalled": []},
            ),
            patch(
                "codex_crew.apps.routes.clean_dependencies",
                new_callable=AsyncMock,
                return_value=[],
            ),
        ):
