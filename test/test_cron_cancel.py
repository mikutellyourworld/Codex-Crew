...
from codex_crew.cron_history import CronHistoryStore
...
from codex_crew.dashboard.handlers.cron import api_cron_cancel
...
class TestCronServiceCancel:
    ...
    async def test_cancel_agent_job_resets_session_and_records_history(
        ...
        with patch("codex_crew.sel.sel") as mock_sel, patch.object(svc, "_save"):
    ...
    async def test_cancel_script_job_kills_subprocess_not_session(
        ...
        job = _make_job("script1", script="~/.codexcrew/crons/x.py:run")
        svc._jobs = [job]
        ...
        with patch(
            "codex_crew.cron_script.kill_running_process", return_value=True
        ) as mock_kill, patch("codex_crew.sel.sel"), patch.object(svc, "_save"):
    ...
    async def test_cancel_does_not_touch_consecutive_failures(
        ...
        with patch("codex_crew.sel.sel"), patch.object(svc, "_save"):
...
class TestSubprocessRegistry:
    ...
    def test_run_command_sandboxed_can_be_cancelled_mid_run(self) -> None:
        ...
        with patch(
            "codex_crew.cron_script.wrap_argv", side_effect=lambda argv, mode: (argv, None)
        ), patch(
            "codex_crew.cron_script.cgroup_scope_argv", side_effect=lambda argv: argv
        ), patch(
            ...
            "codex_crew.cron_script._resolve_command_shell", return_value="/bin/sh"
        ):
    ...
    def test_run_command_without_job_id_not_registered(self, posix_test_shell) -> None:
        ...
        with patch(
            ...
            "codex_crew.cron_script._resolve_command_shell", return_value=posix_test_shell
        ):
