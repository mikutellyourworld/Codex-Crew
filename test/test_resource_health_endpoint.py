...
from codex_crew.dashboard import handlers_system as hs
from codex_crew.resource_status import ResourceStatus
...
async def test_api_system_includes_resource_posture(monkeypatch):
    ...
    with patch(
        "codex_crew.dashboard.handlers_system._resource_probe",
        return_value=_MOCK_STATUS,
        create=True,
    ):
        # Patch at the module level where the lazy import will resolve
        monkeypatch.setattr(
            "codex_crew.resource_status.probe", lambda cfg=None: _MOCK_STATUS
        )
        monkeypatch.setattr(
            "codex_crew.subagent.compute_max_subagents", lambda cfg: 7
        )
...
async def test_api_system_resource_posture_fallback_on_probe_failure(monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.resource_status.probe", _failing_probe)
...
async def test_api_system_resource_posture_ample(monkeypatch):
    ...
    monkeypatch.setattr(
        "codex_crew.resource_status.probe", lambda cfg=None: ample_status
    )
    monkeypatch.setattr(
        "codex_crew.subagent.compute_max_subagents", lambda cfg: 11
    )
