...
from codex_crew.mcp_gateway import hazards, shareability
from codex_crew.mcp_gateway import verdict_cache as vc
from codex_crew.mcp_gateway.hashing import ENV_SCRUB_PREFIXES
from codex_crew.mcp_gateway.record_json import finite_float
from codex_crew.mcp_gateway.shareability import ShareEvidence, Strength, assess
...
class TestDisqualifiers:
    ...
    def test_a_session_bound_server_is_never_recommended(self) -> None:
        ...
        ``codexcrew-cron`` is the real shape: one of ours, and still reading its
        ...
        verdict = assess(
            ShareEvidence(
                name="codexcrew-cron", session_bound_by_construction=True, probe_ok=True
            )
        )
    ...
    def test_a_managed_server_that_consumes_the_caller_block_is_not_disqualified(self) -> None:
        """Regression: ``codexcrew-core`` was disqualified for being ours.
...
class TestMeasurementCanEarnAVerdict:
    ...
    ``codexcrew.caller-identity`` is our own extension and the MCP base protocol has
...
class TestNameCollisionDoesNotInheritEvidence:
    ...
    def test_a_stored_row_is_served_by_name(self, tmp_path, monkeypatch) -> None:
        from codex_crew.dashboard.handlers import mcp as handler
...
class TestNoBlockingRecordIoOnTheLoop:
    ...
    RECORD_MODULES = (
        "codex_crew.mcp_gateway.hazards",
        "codex_crew.mcp_gateway.verdict_cache",
        "codex_crew.mcp_gateway.evaluate",
    )
    ...
    SITES = (
        ("codex_crew.dashboard.handlers.mcp", "api_mcp_gateway_servers"),
        ("codex_crew.dashboard.handlers.mcp", "_evaluate_shareability"),
        ("codex_crew.mcp_gateway.evaluate", "evaluate_new_servers"),
    )
    ...
    def _blocking_helpers(cls) -> set[str]:
        ...
        while True:
            ...
            # name alone flagged ``CodexCrewConfig.load()`` — an unrelated loader
    ...
    def test_the_key_derivation_refuses_to_run_on_the_loop(self) -> None:
        ...
        from codex_crew.mcp_gateway import evaluate
    ...
    def test_gatewayd_installs_the_sink_off_loop(self) -> None:
        ...
        from codex_crew.mcp_gateway import gatewayd
    ...
    def test_the_expensive_modules_stay_off_the_gateway_boot_path(self) -> None:
        ...
        banned = {
            "codex_crew.mcp_gateway.evaluate",
            "codex_crew.mcp_gateway.preflight",
            "codex_crew.mcp_gateway.stub",
        }
    ...
    def test_the_new_modules_have_no_function_local_imports(self) -> None:
        ...
        from codex_crew.mcp_gateway import evaluate, preflight, seed, verdict_cache
    ...
    def test_assess_server_is_pure(self) -> None:
        ...
        for forbidden in ("load_cache", "load_ledger", "CodexCrewConfig", "open("):
...
class TestHazardInvalidation:
    ...
    def test_the_recording_site_stamps_the_pool_key(self) -> None:
        ...
        """
        from codex_crew.mcp_gateway import backend as backend_mod
...
class TestRecordsDirIsNotGatedOnABroker:
    ...
    def test_unconfigured_socket_still_resolves_a_directory(self, monkeypatch, tmp_path) -> None:
        from codex_crew.mcp_gateway.rewriter import records_dir, runtime_dir
    ...
    def test_the_default_socket_lives_in_that_same_directory(self, monkeypatch, tmp_path) -> None:
        from codex_crew.mcp_gateway.rewriter import default_socket_path, runtime_dir
...
class TestBackendRecordsHazards:
    ...
    def _backend(server_name: str, *, exclusive_token: str = "", refcount: int = 2) -> object:
        ...
        """
        from codex_crew.mcp_gateway.backend import Backend
    ...
    def test_both_observation_sites_call_the_recorder(self) -> None:
        ...
        import codex_crew.mcp_gateway.backend as backend_mod
