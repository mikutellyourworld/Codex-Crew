...
from codex_crew.dashboard.handlers import api_send_message
from codex_crew.discord.client import DISCORD_CHUNK_LIMIT
from codex_crew.discord.transport import DiscordTransport
from codex_crew.mcp_core import _call_tool
from codex_crew.mcp_tools.messaging import (
    _CHANNEL_SESSIONS,
    _SESSION_TARGETS,
    _SLACK_ONLY_FIELDS,
    schemas,
)
from codex_crew.messaging.transport import ConfiguredChannelTarget, TransportCapabilities
from codex_crew.platform.governance import Decision
from codex_crew.validation import SEND_MESSAGE_SCHEMA, ValidationError, validate_tool_args
...
def cron_caller():
    ...
    with patch.dict("os.environ", {"CODEXCREW_SESSION_KEY": CRON_CALLER}):
        yield
...
def test_discord_session_is_vetted_on_discord_and_not_on_slack(cron_caller) -> None:
    ...
    """
    with (
        patch("codex_crew.mcp_core._post") as post,
        patch("codex_crew.mcp_core._vet_channel_governance", return_value=None) as vet,
    ):
        post.return_value = {"ok": True, "delivered_to": "discord"}
...
def test_governance_denial_stops_the_discord_send(cron_caller) -> None:
    with (
        patch("codex_crew.mcp_core._post") as post,
        patch(
            "codex_crew.mcp_core._vet_channel_governance",
...
def test_a_channel_addressed_send_does_not_claim_a_dm_or_notification(cron_caller) -> None:
    ...
    """
    with patch(
        "codex_crew.mcp_core._post", return_value={"ok": True, "delivered_to": "webex", "parts": 1}
    ):
        result = _call_tool(
            "send_message",
...
def test_an_addressed_send_is_refused_without_a_verifiable_identity() -> None:
    ...
    """
    with patch.dict("os.environ", {}, clear=True), patch("codex_crew.mcp_core._post") as post:
        result = _call_tool(
            "send_message",
            {"text": "hi", "channel_type": "webex", "target_id": "room:Y2lzY29z"},
        )
...
def test_a_bare_send_still_works_without_a_verifiable_identity() -> None:
    ...
    with (
        ...
        patch("codex_crew.mcp_core._post", return_value={"ok": True}) as post,
    ):
...
@pytest.fixture
def audit():
    with patch("codex_crew.sel.sel") as factory:
        recorder = MagicMock()
...
async def test_a_body_caller_session_is_ignored_without_internal_auth() -> None:
    ...
    with patch.object(
        __import__("codex_crew.dashboard.handlers.messaging", fromlist=["_send_to_channel_target"]),
...
async def test_a_non_cron_channel_session_vets_on_the_headers_attested_identity() -> None:
    ...
    module = __import__("codex_crew.dashboard.handlers.messaging", fromlist=["_deliver_channel_dm"])
