...
F1: fullstack apigateway mutations (PUT/PATCH/DELETE) must be gated on the
    codexcrew:managed resource tag — the deploy profile must not be able to
...
TEMPLATES = REPO / "src" / "codex_crew" / "deploy" / "skills" / "artifact-deploy" / "templates"
...
class TestF1ApiGwTagCondition:
    def test_mutations_are_tag_conditioned(self):
        ...
        assert cond["aws:ResourceTag/codexcrew:managed"] == "true"
    ...
    def test_templates_tag_the_api(self):
        ...
        for name in ("app-apigw.yaml", "app-apigw-ddb.yaml"):
            ...
            assert tags.get("codexcrew:managed") == "true", (
                f"{name}: HttpApi must carry codexcrew:managed=true or the "
