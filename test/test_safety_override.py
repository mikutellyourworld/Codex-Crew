"""Tests for codex_crew.safety_override — time-limited safety override (YOLO replacement)."""
...
class TestActivation:
    def test_activate_from_slack(self, override: SafetyOverride) -> None:
        with patch("codex_crew.safety_override.sel") as mock_sel:
...
class TestExpiry:
    ...
    def test_expiry_logs_sel_event(self, override: SafetyOverride) -> None:
        ...
        with patch("codex_crew.safety_override.sel", return_value=mock_sel_instance):
            override.activate("slack", ttl=1)
        # Force expiry
        override._expires_at = time.monotonic() - 1
        mock_sel_instance2 = MagicMock()
        with patch("codex_crew.safety_override.sel", return_value=mock_sel_instance2):
            override.is_active()
...
class TestSelFaultTolerance:
    ...
    def test_renew_does_not_resurrect_grant_deactivated_during_audit(
        ...
        with patch("codex_crew.safety_override.sel", return_value=sink):
            result = override.renew("slack")
    ...
    def test_sel_import_error_rolls_back_activate(self, override: SafetyOverride) -> None:
        ...
        with patch("codex_crew.safety_override.sel", side_effect=ImportError("no sel")):
            result = override.activate("slack")
        assert result.active is False
        assert not override.is_active()
    ...
    def test_activate_denied_with_real_async_sel_unwritable(
        ...
        monkeypatch.setattr("codex_crew.safety_override.sel", lambda: real_sel)
...
class TestScopedGrants:
    ...
    def test_scope_expires_and_is_purged(self, override: SafetyOverride) -> None:
        with patch("codex_crew.safety_override.sel") as mock_sel:
            ...
            with patch("codex_crew.safety_override.time.monotonic", return_value=time.monotonic() + 120):
    ...
    def test_renew_scoped_extends_expiry(self, override: SafetyOverride) -> None:
        with patch("codex_crew.safety_override.sel") as mock_sel:
            mock_sel.return_value = MagicMock()
            base = time.monotonic()
            with patch("codex_crew.safety_override.time.monotonic", return_value=base):
                override.activate_scoped("s", source="dashboard", ttl=100)
            # 90s later a tool call renews it — expiry slides forward.
            with patch("codex_crew.safety_override.time.monotonic", return_value=base + 90):
...
class TestDeclaredGrantGovernance:
    ...
    def test_permitted_when_ungoverned(self) -> None:
        from codex_crew.safety_override import declared_grant_permitted
    ...
    def test_denied_when_policy_denies_permanent(self) -> None:
        ...
        with patch(
            "codex_crew.platform.governance_profiles.governance_permits", return_value=denied
        ):
    ...
    def test_asks_the_host_profile_fail_closed(self) -> None:
        ...
        with patch(
            "codex_crew.platform.governance_profiles.governance_permits", return_value=permitted
        ) as gp:
    ...
    def test_denied_policy_falls_back_to_adhoc_ttl(self) -> None:
        ...
        with patch("codex_crew.safety_override.sel") as mock_sel, patch.object(
...
class TestUntilShutdownDuration:
    ...
    def test_governance_can_forbid_it(self) -> None:
        ...
        denied = MagicMock()
        denied.permitted = False
        with patch("codex_crew.config.loader.CodexCrewConfig.load", return_value=cfg), patch(
            "codex_crew.platform.governance_profiles.governance_permits", return_value=denied
        ):
...
class TestRenamedConfigKey:
    ...
    def _load(agent: dict) -> bool:
        from codex_crew.config.loader import _read_skip_permissions
    ...
    def test_round_trips_through_save(self, tmp_path: pathlib.Path) -> None:
        ...
        with patch("codex_crew.config.loader.config_path", return_value=cfg_file), patch(
            "codex_crew.config.loader.config_local_path", return_value=local
        ):
            ...
            cfg.save()
            assert CodexCrewConfig.load().agent.dangerously_skip_permissions is True
...
class TestGrantLifetimeCopy:
    ...
    @staticmethod
    def _helpers():
        from codex_crew.slack.handler import describe_grant_lifetime, describe_new_grant
...
class TestStatusDoesNotBlockTheEventLoop:
    ...
    def test_helper_is_fail_soft(self) -> None:
        ...
        from codex_crew.dashboard.handlers_system import _yolo_duration_fields

        with patch(
            "codex_crew.dashboard.handlers_system.CodexCrewConfig.load",
            side_effect=RuntimeError("unreadable"),
        ), patch(
            "codex_crew.dashboard.handlers_system.until_shutdown_permitted",
