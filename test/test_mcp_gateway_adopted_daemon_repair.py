...
Every test that starts a real daemon isolates CODEXCREW_HOME and the working
...
from codex_crew.mcp_gateway import manager as mgr
from codex_crew.mcp_gateway import transport
from codex_crew.mcp_gateway.gatewayd import resolvable_target_stems

_POSIX_ONLY = pytest.mark.skipif(
    sys.platform == "win32", reason="observes a socket file and lock being released"
)

_TARGET_PREFIXES = ("CODEXCREW_MCP_TARGET_", "MC_MCP_TARGET_")
...
def _only_targets(monkeypatch: pytest.MonkeyPatch, mapping: dict[str, str]) -> None:
    ...
    drops every variable the suite set for isolation -- including CODEXCREW_HOME,
...
class TestApplyStandDown:
    def test_stands_down_when_it_cannot_resolve_a_needed_stem(
        ...
        _only_targets(monkeypatch, {"CODEXCREW_MCP_TARGET_PDF": "npx -y server-pdf"})
        stop = asyncio.Event()
        reply = gw._apply_stand_down({"type": "stand-down", "need": ["CODEXCREW_CORE", "PDF"]}, stop)
        assert reply["type"] == "standing-down"
        assert reply["missing"] == ["CODEXCREW_CORE"]
    ...
    def test_a_superset_daemon_is_fit_and_refuses(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        _only_targets(
            monkeypatch,
            {
                "CODEXCREW_MCP_TARGET_PDF": "npx -y server-pdf",
...
class TestAdoptedDriftRecheck:
    ...
    async def test_a_fit_adopted_daemon_is_left_alone(
        ...
        manager = _manager(tmp_path, {"CODEXCREW_MCP_TARGET_CORE": "run core"})
...
class TestElection:
    ...
    async def test_a_fit_incumbent_is_adopted_with_no_stand_down(
        ...
        manager = _manager(tmp_path, {"CODEXCREW_MCP_TARGET_PDF": "npx -y server-pdf"})
        monkeypatch.setattr(
            manager,
            "_ping_payload",
    ...
    async def test_a_stale_incumbent_that_yields_is_replaced(
        ...
        manager = _manager(tmp_path, {"CODEXCREW_MCP_TARGET_CORE": "codexcrew mcp-core"})
        monkeypatch.setattr(
            manager,
            "_ping_payload",
            AsyncMock(return_value={"type": "pong", "targets": ["PDF"]}),
...
def _isolate(monkeypatch: pytest.MonkeyPatch, home: Path) -> Path:
    ...
    CODEXCREW_HOME because the suite does not set it and
...
async def test_the_stand_down_frame_is_wired_into_the_serving_daemon(
    ...
    try:
        await _await_endpoint(sock)
        reply = await _round_trip(sock, {"type": "stand-down", "need": ["CODEXCREW_CORE"]})
        assert reply["type"] == "standing-down"
...
async def test_a_stale_survivor_is_repaired_end_to_end(
    ...
    survivor = await asyncio.create_subprocess_exec(
        sys.executable,
        "-m",
        "codex_crew.mcp_gateway.gatewayd",
        "--socket",
        str(sock),
        "--idle-timeout-secs",
        "60",
    ...
    try:
        ...
        # The starting gateway's config now wants codexcrew-core stubbed too.
        wanted = {
        ...
        assert "CODEXCREW_CORE" in pong["targets"], "the new daemon covers the new stem"
    finally:
...
async def test_a_covering_survivor_is_still_adopted_end_to_end(
    ...
    _isolate(monkeypatch, tmp_path / "home")
    sock = short_sock_dir / "gw.sock"
    targets = {"CODEXCREW_MCP_TARGET_PDF": "npx -y server-pdf --stdio"}
...
async def test_stand_down_is_recorded_in_the_security_event_log(
    ...
    gw._apply_stand_down({"type": "stand-down", "need": ["CODEXCREW_CORE"]}, asyncio.Event())
