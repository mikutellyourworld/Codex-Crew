...
``codexcrew.caller-identity`` and appends the ``sessionKey`` of every
...
def main() -> int:
    ...
    # ``codexcrew.caller-identity`` -- gatewayd injects the block only into a
    ...
    for line in sys.stdin:
        ...
        if method == "tools/call" and (caller_log or tenant_log):
            ...
            block = meta.get("codexcrew.caller") or {}
            tenant = meta.get("codexcrew.tenant") or {}
        ...
        if caller_log or tenant_log:
            capabilities["experimental"] = {"codexcrew.caller-identity": {"schemaVersion": 1}}
        reply = {
            "jsonrpc": "2.0",
            "id": msg.get("id"),
            "result": {
