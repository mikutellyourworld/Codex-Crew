...
TEMPLATES_DIR = Path(__file__).resolve().parent.parent / "src" / "codex_crew" / "deploy" / "skills" / "artifact-deploy" / "templates"
...
class TestEmailRedactionInLogs:
    ...
    @pytest.mark.parametrize("module_path", [
        "src/codex_crew/teams/transport_dispatch.py",
        "src/codex_crew/webex/transport_dispatch.py",
    ])
...
class TestSageRoutesErrorDisclosure:
    ...
    def test_no_raw_exception_in_502_response(self):
        src = (Path(__file__).resolve().parent.parent
               / "src" / "codex_crew" / "apps" / "builtins" / "code_review_sage" / "backend" / "routes.py")
        text = src.read_text(encoding="utf-8")
...
class TestWeakCryptography:
    ...
    def test_sage_learning_no_sha1(self):
        src = (Path(__file__).resolve().parent.parent
               / "src" / "codex_crew" / "apps" / "builtins" / "code_review_sage" / "sage_lib" / "learning.py")
        text = src.read_text(encoding="utf-8")
...
class TestEngineDeployBucketLogging:
    ...
    def test_hardening_lives_in_one_place(self):
        ...
        src = (Path(__file__).resolve().parent.parent
               / "src" / "codex_crew" / "deploy" / "engine.py")
        text = src.read_text(encoding="utf-8")
