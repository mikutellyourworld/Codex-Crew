"""Tests for codex_crew.dashboard.port_reclaim.
...
async def test_lsof_unavailable_returns_unavailable() -> None:
    outcome = await pr.reclaim_stale_gateway_port(
        5486, list_listeners=lambda _p: None, probe_healthy=_unhealthy
    )
...
async def test_no_listeners_returns_no_holder() -> None:
    outcome = await pr.reclaim_stale_gateway_port(
        5486, list_listeners=lambda _p: [], probe_healthy=_unhealthy
    )
...
async def test_foreign_holder_is_never_touched() -> None:
    ...
    outcome = await pr.reclaim_stale_gateway_port(
        ...
        is_codexcrew=lambda _pid: False,  # not a CodexCrew process
...
@pytest.mark.asyncio
async def test_stale_codexcrew_gateway_is_reclaimed() -> None:
    killed: list[int] = []
    ...
    outcome = await pr.reclaim_stale_gateway_port(
        ...
        is_codexcrew=lambda pid: pid in (1234, 5678),
...
def test_listeners_windows_uses_find_listening_pids(monkeypatch) -> None:
    ...
    assert pr._listeners_on_port(5486) == [4242]
