...
from codex_crew.channel import CHANNEL_AGENT_BLOCKED_TOOLS, _stream_task
from codex_crew.providers.base import EVENT_COMPLETE, EVENT_PERMISSION_REQUEST
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("send_message", True),
        ("send_notification (codexcrew-core)", True),
        ("codexcrew-core___send_message", True),
        ("mcp__codexcrew-core__send_message", True),  # canonical MCP prefix (round 20)
...
def test_blocked_tool_matcher_precision(rendered, expected):
    from codex_crew.channel import _blocked_tool_named
...
def test_every_session_control_tool_is_contained():
    ...
    """
    from codex_crew.mcp_dashboard import SESSION_CONTROL_TOOLS
