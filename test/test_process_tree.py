...
class TestGetChildPidsVisitedSet:
    def test_cycle_terminates(self):
        ...
        with patch("codex_crew.acp.client._direct_children", side_effect=fake_direct):
            result = _get_child_pids(1)
        assert result == [2]
        assert call_count <= 3

    def test_self_loop(self):
        with patch("codex_crew.acp.client._direct_children", return_value=[42]):
            assert _get_child_pids(42) == []

    def test_diamond_deduplicates(self):
        tree = {1: [2, 3], 2: [4], 3: [4]}
        with patch("codex_crew.acp.client._direct_children", side_effect=lambda p: tree.get(p, [])):
    ...
    def test_no_children(self):
        with patch("codex_crew.acp.client._direct_children", return_value=[]):
...
class TestKillEscapedChildren:
    ...
    def test_kills_verified_child(self):
        ...
        with (
            ...
            patch("codex_crew.acp.client._is_our_child", return_value=True),
        ):
    ...
    def test_skips_recycled_pid(self):
        ...
        with (
            patch("os.kill", side_effect=fake_kill),
            patch("codex_crew.acp.client._is_our_child", return_value=False),
        ):
            _kill_escaped_children({42: 100})
...
class TestIsOurChild:
    ...
    def _force_linux(self):
        with patch("codex_crew.acp.client.sys") as mock_sys:
            mock_sys.platform = "linux"
            yield

    def test_rejects_missing_proc(self):
        with (
            patch("codex_crew.acp.client._get_start_time", return_value=1),
            patch("codex_crew.acp.client._read_basename", return_value=None),
        ):
    ...
    def test_rejects_unknown_binary(self):
        with (
            patch("codex_crew.acp.client._get_start_time", return_value=1),
            patch("codex_crew.acp.client._read_basename", return_value=b"postgres"),
        ):
            # recorded basename was "node" but live binary is "postgres" (recycled)
            assert _is_our_child(999, expected_start=1, expected_basename=b"node") is False

    def test_rejects_start_time_mismatch(self):
        with patch("codex_crew.acp.client._get_start_time", return_value=200):
            assert _is_our_child(999, expected_start=100) is False

    def test_accepts_matching_kiro(self):
        with (
            patch("codex_crew.acp.client._get_start_time", return_value=100),
            patch("codex_crew.acp.client._read_basename", return_value=b"kiro-cli"),
        ):
            assert _is_our_child(999, expected_start=100, expected_basename=b"kiro-cli") is True

    def test_accepts_mcp_in_name(self):
        with (
            patch("codex_crew.acp.client._get_start_time", return_value=50),
            patch("codex_crew.acp.client._read_basename", return_value=b"builder-mcp"),
        ):
...
class TestDirectChildren:
    def test_proc_children_parsed(self):
        with (
            patch("codex_crew.acp.client.sys") as mock_sys,
            patch("codex_crew.acp.client.Path") as mock_path_cls,
        ):
            mock_sys.platform = "linux"
            mock_path = MagicMock()
...
class TestSnapshotProcessTree:
    ...
    async def test_tracks_all_descendants(self, tmp_path):
        ...
        with (
            patch("codex_crew.acp.client._get_child_pids", return_value=[200, 300, 400]),
            patch("codex_crew.acp.client._get_start_time", side_effect=lambda p: p * 10),
            patch("codex_crew.acp.client._read_basename", side_effect=lambda p: f"proc{p}".encode()),
            patch("codex_crew.session_pid.config_dir", return_value=tmp_path),
        ):
    ...
    async def test_no_descendants_no_tracking(self):
        ...
        with patch("codex_crew.acp.client._get_child_pids", return_value=[]):
            await client._snapshot_process_tree()
    ...
    async def test_merges_early_and_late_snapshots(self, tmp_path):
        ...
        with (
            patch("codex_crew.acp.client._get_child_pids", return_value=[200, 300]),
            patch("codex_crew.acp.client._get_start_time", side_effect=lambda p: p * 10),
            patch("codex_crew.acp.client._read_basename", side_effect=lambda p: f"proc{p}".encode()),
            patch("codex_crew.session_pid.config_dir", return_value=tmp_path),
        ):
...
class TestPidTracking:
    def test_track_and_untrack(self, tmp_path):
        ...
        with patch("codex_crew.session_pid._pid_file_path", return_value=pid_file):
            _track_pid(100)
            _track_pid(200)
            _track_pid(300)
