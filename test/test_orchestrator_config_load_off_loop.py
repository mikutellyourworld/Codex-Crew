...
That read goes through ``CodexCrewConfig.load()``, which stats and reads
...
def _write_config(payload: dict[str, Any]) -> None:
    """Put a real config.json in the per-test CODEXCREW_HOME.
...
def _record_config_loads(monkeypatch: Any, threads: list[int], *, raises: bool = False) -> None:
    ...
    ``chat_orchestrator`` binds ``CodexCrewConfig`` at import, so replacing that
    ...
    monkeypatch.setattr(
        "codex_crew.dashboard.chat_orchestrator.CodexCrewConfig",
...
def _gate_config_load(monkeypatch: Any) -> tuple[asyncio.Event, asyncio.Event]:
    ...
    monkeypatch.setattr(
        "codex_crew.dashboard.chat_orchestrator.asyncio.to_thread",
        gated_to_thread,
    )
...
async def _press_stop(state: MagicMock, slot: _ChatSlot) -> None:
    ...
    from codex_crew.dashboard.chat_handlers import api_chat_slot_stop
    ...
    with (
        patch("codex_crew.dashboard.chat_handlers.sel"),
        patch("codex_crew.dashboard.chat_handlers._reject_pending_approvals"),
    ):
...
async def _press_plan_cancel(state: MagicMock, slot: _ChatSlot) -> dict[str, Any]:
    ...
    from codex_crew.dashboard.chat_orchestrator import api_chat_plan_action
...
async def test_plan_cancel_during_the_config_load_does_not_start_the_plan(
    ...
    monkeypatch.setattr("codex_crew.dashboard.chat_orchestrator.sel", MagicMock())
    ...
    monkeypatch.setattr("codex_crew.dashboard.chat_orchestrator._run_chat", _fake_run_chat)
...
async def test_a_plan_started_after_a_cancel_still_runs(monkeypatch: Any) -> None:
    ...
    from codex_crew.dashboard.chat import _stage_loop
    from codex_crew.dashboard.chat_title import _reset_auto_run_for_new_plan
...
async def test_a_message_queued_during_the_config_load_is_still_handed_off(
    ...
    monkeypatch.setattr(
        "codex_crew.dashboard.chat_orchestrator._start_next_queued_turn", _fake_handoff
    )
