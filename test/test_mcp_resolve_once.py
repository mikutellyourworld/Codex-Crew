...
from codex_crew.mcp_gateway import resolve_once as R
...
class TestLaunchSpecs:
    def test_dedupes_the_disambiguated_twin(self) -> None:
        ...
        out = R.launch_specs(
            {
                "CODEXCREW_MCP_TARGET_FOO": "npx -y foo@1.0.0",
                "CODEXCREW_MCP_TARGET_FOO__deadbeef": "npx -y foo@1.0.0",
            }
        )
    ...
    def test_unparsable_and_empty_are_skipped(self) -> None:
        assert R.launch_specs({"CODEXCREW_MCP_TARGET_A": '"unterminated'}) == []
        assert R.launch_specs({"CODEXCREW_MCP_TARGET_B": "   "}) == []
...
class TestEnsureAndPrefetch:
    ...
    async def test_prefetch_reports_per_package_outcomes(self, tmp_path, monkeypatch) -> None:
        ...
        outcomes = await R.prefetch(
            home,
            {
                "CODEXCREW_MCP_TARGET_A": "npx -y foo@1.0.0",
                "CODEXCREW_MCP_TARGET_B": "uvx not-npm",
            },
        )
    ...
    async def test_prefetch_survives_a_raising_target(self, tmp_path, monkeypatch) -> None:
        ...
        outcomes = await R.prefetch(str(tmp_path), {"CODEXCREW_MCP_TARGET_A": "npx -y foo@latest"})
...
class TestSubstitutionActuallyHappens:
    ...
    def test_a_realistic_inherited_env_still_gets_the_store(self, tmp_path, monkeypatch) -> None:
        from codex_crew.mcp_gateway import gatewayd as G
