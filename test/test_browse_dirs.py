...
from codex_crew.dashboard.handlers import api_browse_dirs
from codex_crew.dashboard.handlers.files import _browse_dirs_sync
...
class TestBrowseDirs:
    ...
    async def test_scan_does_not_run_on_the_event_loop(self, tmp_path, mock_sel):
        ...
        with patch("codex_crew.dashboard.handlers.files._browse_dirs_sync", spy):
            async with TestClient(TestServer(_make_app())) as client:
                resp = await client.get(f"/api/browse-dirs?path={tmp_path}")
