...
async def _delete(tmp_path, name: str, cfg: CodexCrewConfig):
    with patch("codex_crew.agent.KIRO_AGENTS_DIR", tmp_path), patch.object(
        CodexCrewConfig, "load", staticmethod(lambda: cfg)
    ):
...
async def test_delete_refuses_a_crew_bound_template(tmp_path):
    f = _agent_file(tmp_path, "scratch")
    cfg = CodexCrewConfig()
    cfg.agents = {"researcher": CodexCrewAgentConfig(kiro_agent="scratch")}
...
async def test_delete_allows_an_unreferenced_template(tmp_path):
    f = _agent_file(tmp_path, "scratch")
    cfg = CodexCrewConfig()
    cfg.agent.default_agent = "codexcrew"
    cfg.default_agent = "codexcrew"
    cfg.agents = {"researcher": CodexCrewAgentConfig(kiro_agent="codexcrew")}
...
async def test_delete_guard_reads_config_off_the_event_loop(tmp_path):
    ...
    f = _agent_file(tmp_path, "scratch")
    with patch("codex_crew.agent.KIRO_AGENTS_DIR", tmp_path), patch.object(
        CodexCrewConfig, "load", staticmethod(_load)
    ):
