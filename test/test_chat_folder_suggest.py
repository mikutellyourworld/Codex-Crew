...
from codex_crew.dashboard import chat_folder_suggest as fs
...
def test_build_prompt_numbers_folders_and_marks_empty_ones() -> None:
    prompt = fs._build_prompt(
        ...
        labels=["Codex Crew › i18n", "Errands"],
        samples=[["Pseudolocale gate", "Catalog sync"], []],
    )
    assert "1. Codex Crew › i18n — contains: \"Pseudolocale gate\"; \"Catalog sync\"" in prompt
...
def _run(state, slot, *, reply="1", enabled=True):
    ...
    with (
        patch.object(fs.CodexCrewConfig, "load", staticmethod(lambda: cfg)),
...
_FOLDERS = [{"id": "f1", "name": "Codex Crew", "order": 0}]
...
def test_suggests_and_delivers_to_owner_sockets() -> None:
    ...
    assert payload["folder_name"] == "Codex Crew"
