...
async def test_a_boundary_ahead_of_the_window_does_not_duplicate_persisted_turns(
    ...
    monkeypatch.setattr("codex_crew.dashboard.chat_fork.save_slot_off_loop", fake_flush)
...
async def test_a_rewind_landing_during_the_pending_rewrite_save_is_not_erased(
    ...
    for i in range(10):
        slot.append("user" if i % 2 == 0 else "assistant", f"m{i}", "msg")
    slot.drain()
    from codex_crew.dashboard import chat_fork as chat_fork_mod
    from codex_crew.dashboard.chat import _save_slot_to_history
...
def _archived_contents(log):
    ...
    from codex_crew.history import _archive_dir
