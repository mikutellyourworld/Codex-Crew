...
def test_slot_root_defaults_under_cache_not_codexcrew_home(
    ...
    monkeypatch.setenv("CODEXCREW_HOME", "/tmp/some-unrelated-home")
    resolved = ct._slot_root()
    assert resolved == pathlib.Path.home() / ".cache" / "codexcrew" / "test-slots"
...
def test_the_xdist_env_var_is_honoured_as_a_ceiling(
    ...
    is where the variable would otherwise have been read. Codex Crew itself seeds
