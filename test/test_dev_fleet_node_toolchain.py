...
tier — but Codex Crew's own installer puts node under ``$HOME``, so:

* Dev Fleet "Pull + build main" answered ``no trusted executable for 'npm'``;
* ``codexcrew pod provision`` raised an unhandled ``FileNotFoundError: 'npm'``.
...
def test_toolchain_bin_finds_a_home_resident_npm(node_dir):
    """The exact case _trusted_bin rejects, and the one Codex Crew's own
    installer creates."""
    with (
...
def test_provision_reports_a_remedy_instead_of_raising(tmp_path, monkeypatch, capsys):
    ...
    assert "CODEXCREW_NODE_BIN_DIR" in err
