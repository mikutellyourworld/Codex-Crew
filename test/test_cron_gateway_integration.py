...
import codex_crew.executors as ex
...
def _make_gw():
    from codex_crew.slack.gateway import GatewayOrchestrator
...
def _make_script_job(**overrides):
    defaults = dict(
        ...
        script="~/.codexcrew/crons/monitor.py:run",
    )
...
async def _run_script_callback(gw, job, script_result=None, vet_reason=None, side_effect=None):
    ...
    with (
        patch("codex_crew.slack.gateway.CronService") as mock_cron_cls,
        patch("codex_crew.slack.gateway.run_script_sandboxed", **mock_kw) as mock_run,
        patch("codex_crew.slack.gateway.vet_job_at_fire_time", return_value=vet_reason),
        patch("codex_crew.slack.gateway.sel"),
    ):
...
async def _run_command_callback(gw, job, cmd_result=None, side_effect=None, vet_reason=None):
    ...
    with (
        patch("codex_crew.slack.gateway.CronService") as mock_cron_cls,
        patch("codex_crew.slack.gateway.run_command_sandboxed", **mock_kw) as mock_run,
        gate,
        patch("codex_crew.slack.gateway.sel"),
    ):
...
class TestScriptExecution:
    ...
    async def test_done_busy_store_defers_removal_not_dropped(self):
        ...
        from codex_crew.cron import CronStoreBusy
        ...
        with (
            patch("codex_crew.slack.gateway.CronService") as mock_cron_cls,
            patch(
                "codex_crew.slack.gateway.run_script_sandboxed",
                return_value={"status": "done", "message": "CR merged"},
            ),
            patch("codex_crew.slack.gateway.vet_job_at_fire_time", return_value=None),
            patch("codex_crew.slack.gateway.sel"),
        ):
    ...
    async def test_concurrent_guard_skips(self):
        ...
        with (
            patch("codex_crew.slack.gateway.CronService") as mock_cron_cls,
            patch("codex_crew.slack.gateway.run_script_sandboxed") as mock_run,
            patch("codex_crew.slack.gateway.sel"),
        ):
...
class TestCommandExecution:
    ...
    async def test_fire_time_command_deny_blocks_execution(self):
        ...
        job = _make_command_job()
        with (
            patch("codex_crew.mcp_cron._vet_cron_capability_governance", return_value=None),
            patch(
                "codex_crew.mcp_cron._vet_command_governance",
    ...
    async def test_fire_time_capability_deny_blocks_execution(self):
        ...
        job = _make_command_job()
        with patch(
            "codex_crew.mcp_cron._vet_cron_capability_governance",
    ...
    async def test_fire_time_governance_allow_still_executes(self):
        ...
        with (
            patch("codex_crew.mcp_cron._vet_cron_capability_governance", return_value=None),
            patch("codex_crew.mcp_cron._vet_command_governance", return_value=None),
        ):
            result, mock_run = await _run_command_callback(
                gw, job, {"status": "ok", "output": "hello\n", "exit_code": 0}
            )
...
class TestFireTimeGatesScriptAndMessage:
    ...
    async def _run_script_real_vet(self, gw, job, script_result, cap=None, script_vet=None):
        ...
        with (
            patch("codex_crew.slack.gateway.CronService") as mock_cron_cls,
            patch(
                "codex_crew.slack.gateway.run_script_sandboxed", return_value=script_result
            ) as mock_run,
            patch("codex_crew.mcp_cron._vet_cron_capability_governance", return_value=cap),
            patch("codex_crew.mcp_cron.resolve_script_path", return_value=("/tmp/x.py", "run")),
            patch("codex_crew.mcp_cron._vet_script_file", return_value=script_vet) as mock_vet_file,
            patch("codex_crew.slack.gateway.sel") as mock_sel,
        ):
...
class TestFireTimeAuditTrail:
    ...
    def test_allowed_fire_emits_governance_decision(self):
        ...
        with (
            patch("codex_crew.mcp_cron._vet_cron_capability_governance", return_value=None),
            patch("codex_crew.mcp_cron._vet_command_governance", return_value=None),
            patch("codex_crew.mcp_cron.sel") as mock_sel,
        ):
    ...
    def test_script_body_deny_emits_scoped_decision(self):
        ...
        with (
            ...
            patch("codex_crew.mcp_cron.resolve_script_path", return_value=("/tmp/x.py", "run")),
            patch(
                "codex_crew.mcp_cron._vet_script_file", return_value="Error: cron script blocked: x"
            ),
            patch("codex_crew.mcp_cron.sel") as mock_sel,
        ):
            assert vet_job_at_fire_time(job) is not None
...
async def _run_llm_callback(gw, job, *, get_or_create_side_effect=None):
    ...
    with (
        patch("codex_crew.slack.gateway.CronService") as mock_cron_cls,
        patch("codex_crew.slack.gateway.run_in_embed_pool", _embed_mock),
        patch("codex_crew.slack.gateway.stream_and_collect", _stream_mock),
        patch("codex_crew.slack.gateway.sel"),
        patch("codex_crew.slack.gateway.build_cron_session_context") as mock_ctx,
    ):
...
class TestThrottleFallbackCronWiring:
    ...
    async def test_chain_is_threaded_into_stream_and_collect(self):
        ...
        job = _make_llm_job(model="")
        with patch(
            "codex_crew.slack.gateway.configured_fallback_chain",
    ...
    async def test_fallback_served_run_blanks_pinned_model_in_usage_row(self):
        ...
        with patch(
            "codex_crew.slack.gateway.persist_token_record_async", new_callable=AsyncMock
        ) as persist_mock:
            await _run_llm_callback(gw, job, get_or_create_side_effect=_side_effect)
    ...
    def test_annotate_noop_without_marker(self):
        ...
        from codex_crew.slack.gateway import _annotate_model_fallback
    ...
    def test_gateway_annotator_is_the_shared_body(self):
        ...
        from codex_crew.llm_helpers import annotate_model_fallback
        from codex_crew.slack.gateway import _annotate_model_fallback
    ...
    async def test_chain_exhaustion_story_reaches_the_failure_alert(self):
        ...
        from codex_crew.llm_helpers import FALLBACK_STORY_ATTR
        ...
        from codex_crew.slack.gateway import _CRON_FAILURE_DETAIL_CAP
...
class TestCronUsageRow:
    ...
    async def test_llm_cron_persists_usage_row_with_surface(self):
        ...
        with (
            patch("codex_crew.slack.gateway.persist_token_record_async", persist),
            patch(
                "codex_crew.slack.gateway.read_context_tokens",
    ...
    async def test_cron_row_records_resolved_agent_not_requested(self):
        ...
        with (
            ...
            patch(
                "codex_crew.slack.gateway.read_effective_agent",
                MagicMock(return_value="codexcrew"),
                create=True,
            ),
        ):
            await _run_llm_callback(gw, job)
        ...
        persist.assert_awaited_once()
        assert persist.await_args.kwargs["agent"] == "codexcrew"
    ...
    async def test_script_cron_writes_no_usage_row(self):
        ...
        persist = AsyncMock()
        with patch("codex_crew.slack.gateway.persist_token_record_async", persist):
            await _run_script_callback(gw, job, {"status": "ok"})
...
def test_shutdown_cancel_keeps_the_last_completed_result(tmp_path) -> None:
    ...
    from codex_crew.cron import CronJob, CronSchedule, CronService
...
async def _run_script_callback_behind_a_busy_worker(gw, job, script_result, hold_secs):
    ...
    with (
        ...
        patch("codex_crew.slack.gateway.vet_job_at_fire_time", side_effect=_vet),
        patch("codex_crew.slack.gateway.sel"),
    ):
...
class TestCronPoolQueueWait:
    ...
    async def test_repeated_starvation_never_auto_pauses_a_healthy_script(self):
        ...
        an exception.
        """
        from codex_crew.cron import _AUTO_PAUSE_THRESHOLD
    ...
    async def _one_reaper_sweep(svc, job, elapsed_secs):
        ...
        with patch("codex_crew.cron._REAPER_INTERVAL", 0.01):
    ...
    async def _message_one_shot_run(
        ...
        *,
        ...
        with (
            ...
            gate_work = (
                patch(
                    "codex_crew.slack.gateway.vet_job_at_fire_time",
            ...
            gate_stall = (
                patch("codex_crew.slack.gateway.run_in_cron_gate_pool", _stalled_gate)
                if cancel_gate
                else nullcontext()
            )
    ...
    async def test_the_execution_deadline_accounts_for_the_gate_bound(self, tmp_path):
        ...
        import codex_crew.cron as cron_mod
    ...
    async def test_a_returned_verdict_clears_the_retention_marker(self, tmp_path):
        ...
        that was never made.
        """
        from codex_crew.slack.gateway import _await_cron_fire_time_gate
        ...
        for verdict in (None, "denied by policy"):
            ...
            with patch("codex_crew.slack.gateway.run_in_cron_gate_pool", _verdict_gate):
    ...
    async def test_gate_starvation_clears_the_carried_result_only_for_command_and_script(
        ...
        with patch("codex_crew.slack.gateway.run_in_cron_gate_pool", _starved_gate):
...
class TestClaimBackstopRetainsAnUnstartedOneShot:
    ...
    * ``claim()`` was granted, the payload started, and the subprocess overran.
    ...
    async def test_backstop_retains_only_the_run_that_never_started(
        ...
        gw = _make_gw()
        with patch("codex_crew.slack.gateway.run_in_cron_pool", self._pool(grant_claim=grant_claim)):
    ...
    async def test_a_deny_leaves_retention_to_fire_time_denied(self, tmp_path):
        ...
        gw = _make_gw()
        with patch("codex_crew.slack.gateway.run_in_cron_pool", _denying_pool):
            await _run_command_callback(
...
class TestACancellationAtTheClaimAwaitKeepsItsOneShot:
    ...
    async def test_command_cancellation_retains_only_the_run_that_never_started(
        ...
        with patch(
            "codex_crew.slack.gateway.run_in_cron_pool",
    ...
    async def test_script_cancellation_retains_only_the_run_that_never_started(
        ...
        svc, created = self._one_shot(tmp_path, script="~/.codexcrew/crons/monitor.py:run")
        gw = _make_gw()
