...
from codex_crew.computer_use import policy
from codex_crew.computer_use import service as cu_service
from codex_crew.computer_use import tools
...
def fake_computer_backend(tmp_path, monkeypatch):
    ...
    ``CODEXCREW_HOME`` is redirected first: the dispatcher refuses everything before
    reaching a driver unless the keystone says enabled, and a developer's real
    ``~/.codex-crew`` must never decide a test's outcome.
    """
...
class TestLaunchResolutionTrust:
    ...
    **Runs on every platform, and that is what makes the coverage honest.**
    ...
    def _protected(monkeypatch, root, *, writable=False):
        ...
        """
        from codex_crew.computer_use import launch_windows
    ...
    def _entry(key: str, executable: str):
        from codex_crew.computer_use.launch_windows import InstalledApp
    ...
    def test_the_protected_roots_do_NOT_come_from_the_environment(self, monkeypatch, tmp_path):
        ...
        * reading the install roots from ``os.environ`` (fixed by
          :func:`~codex_crew.computer_use.launch_windows._install_roots_from_registry`);
...
class TestMacOSCatalogAndResolution:
    ...
    def _roots(monkeypatch, *roots):
        from codex_crew.computer_use import launch_macos
...
class TestMacOSBundleVerification:
    ...
    @staticmethod
    def _app():
        from codex_crew.computer_use.launch_macos import InstalledApp
    ...
    def test_the_probed_directories_are_FIXED_not_read_from_the_bundle(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.computer_use.apps_macos.read_bundle_plist",
...
class TestResolvedIdentityReachesThePolicy:
    ...
    def _run(identity, *, denied="", found=None):
        ...
        *found* is what ``find`` answers AFTER the spawn (it always answers ``None`` before,
        ...
        """
        from codex_crew.computer_use import backend as be
        from codex_crew.computer_use.policy import PolicyConfig, check_app
    ...
    def test_EVERY_driver_forwards_its_platform_identity_supplier(self, driver_mod):
        ...
        module = importlib.import_module(f"codex_crew.computer_use.{driver_mod}")
    ...
    def test_the_macos_key_is_the_BUNDLE_ID(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.computer_use.apps_macos.bundle_identity_at",
...
class TestLaunchDispatch:
    ...
    def test_a_denied_BUNDLE_ID_is_refused_before_the_process_exists(self, fake_computer_backend):
        ...
        undone, and refusing afterwards only stops Codex Crew driving a process it has
        already started.
    ...
    def test_the_operators_deny_rule_matches_the_OS_IDENTITY_not_just_the_name(
        ...
        revision: with ``extra_denied_apps: ["dev.codexcrew.fake.draw"]`` the launch
        succeeded.
        ...
        (tmp_path / "computer_use.json").write_text(
            json.dumps({"enabled": True, "extra_denied_apps": ["dev.codexcrew.fake.draw"]}),
            encoding="utf-8",
        )
        out = _launch("Fake Draw")
        assert out.startswith(ERROR_PREFIX)
    ...
    def test_the_resolved_check_does_not_turn_an_ALLOW_list_into_a_refusal(self):
        ...
        """
        who = LaunchIdentity(display="Fake Draw", key="dev.codexcrew.fake.draw")
        for spelling in ("fake draw", "dev.codexcrew.fake.draw"):
    ...
    def test_the_tool_is_in_the_mutating_set_not_the_read_only_one(self):
        ...
        # a tree.
        from codex_crew.computer_use.types import MUTATING_TOOLS, READ_ONLY_TOOLS
    ...
    def test_the_schema_accepts_no_path_or_argument_field(self):
        ...
        # would notice.
        from codex_crew.validation import MCP_COMPUTER_SCHEMAS
    ...
    def test_the_advertised_schema_matches_the_validator(self):
        ...
        from codex_crew.mcp_computer import _tool_definitions
...
def test_the_denylist_probe_shape_reaches_the_self_target_rule():
    ...
    # to Codex Crew's own dashboard.
    probe = AppRef(name="Codex Crew", pid=0, bundle_id="Codex Crew", window_title="Codex Crew")
...
def test_launch_windows_imports_no_platform_module_at_MODULE_SCOPE():
    ...
    break EVERY test that transitively touches ``codex_crew`` on the Linux CI fleet.
...
def test_the_fake_launch_catalog_is_disjoint_from_the_running_list():
    ...
    from codex_crew.testing.fake_computer_use import FAKE_APPS, FAKE_LAUNCHABLE
