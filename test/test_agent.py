...
from codex_crew import agent_state
from codex_crew import atomic_write as aw
from codex_crew.agent import _MANAGED_MCP_ENTRY_KEYS, install_agent, migrate_agent_specs
...
def _run_install(tmp_path: Path, cfg_dir: Path, managed_mcps: dict | None = None, **kwargs) -> Path:  # type: ignore[return]
    ...
    patches = [
        ...
        patch("codex_crew.agent.shutil.which", side_effect=lambda c, **kw: c),
...
class TestInstallAgent:
    def test_fresh_install_generates_from_defaults(self, tmp_path: Path):
        """No existing codexcrew.json → config built from defaults."""
    ...
    def test_fresh_install_preserves_safe_managed_server_overrides(self, tmp_path: Path):
        ...
        cfg_dir = _bundled_defaults(tmp_path)
        user_home = tmp_path / "codexcrew_home"
        user_home.mkdir()
            ...
            json.dumps(
                {
                    "mcpServers": {
                        "codexcrew-core": {
                            ...
                            "timeout": 45,
                            "env": {"MINE": "keep", "CODEXCREW_HOME": "/tmp/wrong"},
        ...
        path = _run_install(tmp_path, cfg_dir)
        entry = json.loads(path.read_text(encoding="utf-8"))["mcpServers"]["codexcrew-core"]

        assert entry["command"] == "/usr/bin/codexcrew"
        assert entry["args"] == ["mcp-core"]
        assert "url" not in entry
        assert entry["timeout"] == 45
        assert entry["env"]["MINE"] == "keep"
        assert entry["env"].get("CODEXCREW_HOME") != "/tmp/wrong"
    ...
    def test_fresh_install_drops_unsupported_entry_keys(self, tmp_path: Path):
        ...
        take every Codex Crew tool down with it. Preserving the user's
    ...
    def test_refresh_drops_unsupported_entry_keys(self, tmp_path: Path):
        ...
        kiro_dir = tmp_path / "kiro_agents"
        kiro_dir.mkdir(exist_ok=True)
        (kiro_dir / "codexcrew.json").write_text(
            json.dumps(
                {
    ...
    def test_fresh_install_strips_launcher_exec_env_from_a_managed_entry(self, tmp_path: Path):
        ...
        with patch("codex_crew.agent._valid_override_home", return_value=None):
            path = _run_install(tmp_path, cfg_dir)
        entry = json.loads(path.read_text(encoding="utf-8"))["mcpServers"]["codexcrew-core"]
    ...
    def test_fresh_install_drops_a_non_finite_timeout(self, tmp_path: Path, literal: str):
        ...
        user_home.mkdir()
        (user_home / "agent.json").write_text(
            '{"mcpServers": {"codexcrew-core": {"timeout": ' + literal + "}}}"
        )

        path = _run_install(tmp_path, cfg_dir)
        raw = path.read_text(encoding="utf-8")
        entry = json.loads(raw)["mcpServers"]["codexcrew-core"]
    ...
    def test_fresh_install_drops_a_boolean_timeout(self, tmp_path: Path):
        ...
        user_home.mkdir()
        (user_home / "agent.json").write_text(
            json.dumps({"mcpServers": {"codexcrew-core": {"timeout": True}}})
        )
    ...
    def test_fresh_install_strips_reserved_control_env_keys(self, tmp_path: Path):
        ...
        ``CODEXCREW_APPROVAL_MODE`` is read straight out of os.environ by
        ...
        seed it onto codexcrew-core's env, a clean rebuild would hand every
        ...
        the one a name list cannot state: two whole NAMESPACES, ``CODEXCREW_``
        and ``PYTHON``, are refused by PREFIX and case-INSENSITIVELY.
        ``CODEXCREW_CLI`` is asserted because ``mcp_cron._caller_is_cli()`` reads
        it as "skip per-session ownership entirely";
        ``PYTHONBREAKPOINT`` because it takes a dotted callable and imports it;
        ``CODEXCREW_NOT_YET_INVENTED`` and ``PYTHONNOTYETINVENTED`` stand for the
            ...
            json.dumps(
                {
                    "mcpServers": {
                        "codexcrew-core": {
                            "env": {
                                "MINE": "keep",
                                "CODEXCREW_APPROVAL_MODE": "auto",
                                "CODEXCREW_SESSION_KEY": "forged",
                                "CODEXCREW_INTERNAL_SECRET": "forged",
                                "CODEXCREW_PRINCIPAL": "forged",
                                "CODEXCREW_CLI": "1",
                                "CODEXCREW_NOT_YET_INVENTED": "forged",
        ...
        assert entry["env"]["MINE"] == "keep"
        for reserved in (
            "CODEXCREW_APPROVAL_MODE",
            "CODEXCREW_SESSION_KEY",
            "CODEXCREW_INTERNAL_SECRET",
            "CODEXCREW_PRINCIPAL",
            "CODEXCREW_CLI",
            "CODEXCREW_NOT_YET_INVENTED",
    ...
    def test_refresh_strips_reserved_control_env_keys(self, tmp_path: Path):
        """The refresh path (existing codexcrew.json) scrubs the same reserved keys.
        ...
        (kiro_dir / "codexcrew.json").write_text(json.dumps(existing))
    ...
    def test_fresh_install_drops_malformed_non_dict_env(self, tmp_path: Path):
        ...
        user_home.mkdir()
        (user_home / "agent.json").write_text(
            json.dumps({"mcpServers": {"codexcrew-core": {"env": "not-a-dict"}}})
        )
        ...
        # remaining ``env`` key is only Codex Crew's own CODEXCREW_HOME pin (the
        # test harness's tmp_path counts as an override home).
        assert set(entry.get("env", {})) <= {"CODEXCREW_HOME"}
    ...
    def test_fresh_install_pins_our_data_home_against_a_declared_home(
        ...
        no ``env`` at all, so that an existing user's ``codexcrew.json`` is not
        ...
        with patch("codex_crew.agent._valid_override_home", return_value=None):
            path = _run_install(tmp_path, cfg_dir)
        entry = json.loads(path.read_text(encoding="utf-8"))["mcpServers"]["codexcrew-cron"]
    ...
    def test_existing_config_preserves_user_model(self, tmp_path: Path):
        """Existing codexcrew.json → user's model choice survives restart."""
        cfg_dir = _bundled_defaults(tmp_path)
        kiro_dir = tmp_path / "kiro_agents"
        kiro_dir.mkdir(exist_ok=True)
        existing = {
    ...
    def test_fresh_install_marks_model_managed(self, tmp_path: Path):
        ...
        assert agent_state.get_model_managed("codexcrew") is True
    ...
    def test_legacy_config_without_marker_frozen(self, tmp_path: Path):
        ...
        assert "model_managed" not in config
        assert agent_state.get_model_managed("codexcrew") is None
    ...
    def test_explicitly_frozen_config_not_tracked(self, tmp_path: Path):
        ...
        assert "model_managed" not in config
        assert agent_state.get_model_managed("codexcrew") is False
    ...
    def test_existing_config_refreshes_security_fields(self, tmp_path: Path):
        ...
        denial moved to CodexCrew's own hooks.py PreToolUse gate). A stale
    ...
    def test_existing_config_refreshes_dynamic_mcp_servers(self, tmp_path: Path):
        """codexcrew-cron and codexcrew-core commands are always refreshed."""
        cfg_dir = _bundled_defaults(tmp_path)
        kiro_dir = tmp_path / "kiro_agents"
        kiro_dir.mkdir(exist_ok=True)
        existing = {
            ...
            "allowedTools": [],
            "mcpServers": {
                "codexcrew-cron": {"command": "/old/path/codexcrew", "args": ["mcp-cron"]},
            },
        }
        (kiro_dir / "codexcrew.json").write_text(json.dumps(existing))
        ...
        path = _run_install(tmp_path, cfg_dir)
        config = json.loads(path.read_text(encoding="utf-8"))
        assert config["mcpServers"]["codexcrew-cron"]["command"] == "/usr/bin/codexcrew"
        assert config["mcpServers"]["codexcrew-core"]["command"] == "/usr/bin/codexcrew"
    ...
    def test_existing_config_preserves_mcp_auto_approve(self, tmp_path: Path):
        ...
        path = _run_install(tmp_path, cfg_dir)
        config = json.loads(path.read_text(encoding="utf-8"))
        # codexcrew-cron/core: command refreshed, autoApprove preserved
        assert config["mcpServers"]["codexcrew-cron"]["command"] == "/usr/bin/codexcrew"
        assert config["mcpServers"]["codexcrew-cron"]["autoApprove"] == ["cron_list", "cron_add"]
        assert config["mcpServers"]["codexcrew-core"]["autoApprove"] == ["learn_list"]
    ...
    def test_codexcrew_mcp_json_overrides_kiro_mcp(self, tmp_path: Path):
        """~/.codexcrew/mcp.json overrides ~/.kiro/settings/mcp.json for codexcrew agent."""
        cfg_dir = _bundled_defaults(tmp_path)
        kiro_dir = tmp_path / "kiro_agents"
        kiro_dir.mkdir(exist_ok=True)
        ...
        (kiro_dir / "codexcrew.json").write_text(json.dumps(existing))
        # codexcrew mcp.json overrides args (removes --include-tools)
        mc_home = tmp_path / "codexcrew_home"
        ...
        path = _run_install(tmp_path, cfg_dir)
        config = json.loads(path.read_text(encoding="utf-8"))
        # builder-mcp args overridden by codexcrew mcp.json (no tool-tag injection
        # on a public install — that Amazon-specific wiring was removed)
        assert config["mcpServers"]["builder-mcp"]["args"] == []
        # autoApprove preserved (not in codexcrew mcp.json)
        assert config["mcpServers"]["builder-mcp"]["autoApprove"] == ["ReadInternalWebsites"]
        # new server added from codexcrew mcp.json
    ...
    def test_new_managed_server_seeds_auto_approve_on_refresh(self, tmp_path: Path):
        ...
        kiro_dir = tmp_path / "kiro_agents"
        kiro_dir.mkdir(exist_ok=True)
        # Existing config has codexcrew-cron/core but NOT playwright-mcp
        existing = {
            "model": "claude-user-custom",
            "tools": [],
            "allowedTools": [],
            "mcpServers": {
                "codexcrew-cron": {"command": "/old/codexcrew", "args": ["mcp-cron"]},
                "codexcrew-core": {"command": "/old/codexcrew", "args": ["mcp-core"]},
            },
            "toolsSettings": {"execute_bash": {"deniedCommands": []}},
            "hooks": {},
        }
    ...
    def test_corrupt_existing_falls_back_to_defaults(self, tmp_path: Path):
        """Corrupt codexcrew.json → falls back to build_agent_config()."""
        cfg_dir = _bundled_defaults(tmp_path)
        kiro_dir = tmp_path / "kiro_agents"
        kiro_dir.mkdir(exist_ok=True)
        (kiro_dir / "codexcrew.json").write_text("not valid json{{{")
    ...
    def test_missing_bundled_defaults_raises_when_existing_config_present(self, tmp_path: Path):
        ...
        kiro_dir = tmp_path / "kiro_agents"
        kiro_dir.mkdir()
        (kiro_dir / "codexcrew.json").write_text(json.dumps({"model": "x", "mcpServers": {}}))
...
class TestAtomicJsonWrite:
    ...
    def test_preserves_existing_permissions(self, tmp_path: Path):
        from codex_crew.agent import _atomic_json_write
...
class TestAllSkillPathsLocalSymlinks:
    ...
    def test_resolves_local_symlink_with_skills_parent(self, tmp_path: Path):
        ...
        from codex_crew.agent import _all_skill_paths
        ...
        with patch("codex_crew.agent.Path.home", return_value=tmp_path):
            with patch("codex_crew.agent._project_dir", return_value=None):
    ...
    def test_skips_broken_symlink(self, tmp_path: Path):
        ...
        with patch("codex_crew.agent.Path.home", return_value=tmp_path):
            with patch("codex_crew.agent._project_dir", return_value=None):
                with patch("codex_crew.agent.logger") as mock_logger:
                    paths = _all_skill_paths()
...
class TestResolveKirocrewBin:
    """Tests for lazy codexcrew binary resolution."""
    ...
    def no_interpreter_scripts(self, tmp_path: Path):
        ...
        prefix legitimately holds a ``bin/codexcrew`` whenever the suite runs
    ...
    def test_prefers_interpreter_scripts_dir_over_path(self, tmp_path: Path):
        ...
        PATH and pick up whatever ``codexcrew`` an unrelated earlier install had
        left there, then cache it as the command for the built-in MCP servers.

        The launcher is created through ``_codexcrew_bin_subpath`` rather than at
        a hardcoded ``bin/codexcrew``, so the layout is the one this OS actually
        looks for (``Scripts\\codexcrew.exe`` on Windows).
        """
        import codex_crew.agent as agent_mod
        from codex_crew.agent import _codexcrew_bin_subpath, _resolve_codexcrew_bin

        root = tmp_path / "runtime"
        pkg_dir = root / "lib" / "python3.12" / "site-packages" / "codex_crew"
        ...
        own_bin = _codexcrew_bin_subpath(prefix)
        ...
        # ``codexcrew`` that happens to exist on the host cannot be selected.
        _real_works = agent_mod._launcher_works

        def _scoped_works(p: Path) -> bool:
            return str(p).startswith(str(tmp_path)) and _real_works(p)

        old_val = agent_mod._CODEXCREW_BIN
        try:
            agent_mod._CODEXCREW_BIN = None
            with ExitStack() as stack:
                stack.enter_context(patch.dict("sys.modules", {"codex_crew": mock_mc}))
                stack.enter_context(
                    patch("codex_crew.agent._launcher_works", side_effect=_scoped_works)
                )
                stack.enter_context(patch("sys.exec_prefix", str(prefix)))
                stack.enter_context(
                    patch(
                        "shutil.which",
                        side_effect=lambda c: str(path_bin) if c == "codexcrew" else None,
                    )
                )
                result = _resolve_codexcrew_bin()
            assert result == str(own_bin)
            assert result != str(path_bin)
        finally:
            agent_mod._CODEXCREW_BIN = old_val

    def test_finds_bin_in_parent_hierarchy(self, tmp_path: Path):
        """Walks up from package dir to find bin/codexcrew."""
        import codex_crew.agent as agent_mod
        from codex_crew.agent import _resolve_codexcrew_bin

        # Create structure: venv/lib/python3.x/site-packages/codex_crew
        #                   venv/bin/codexcrew
        venv = tmp_path / "venv"
        pkg_dir = venv / "lib" / "python3.11" / "site-packages" / "codex_crew"
        pkg_dir.mkdir(parents=True)
        (pkg_dir / "__init__.py").write_text("")

        bin_dir = venv / "bin"
        bin_dir.mkdir()
        codexcrew_bin = bin_dir / "codexcrew"
        codexcrew_bin.write_text("#!/bin/bash\necho codexcrew")
        codexcrew_bin.chmod(0o755)

        # Mock codex_crew.__file__ to point to our fake package
        mock_mc = unittest.mock.MagicMock()
        mock_mc.__file__ = str(pkg_dir / "__init__.py")
        ...
        try:
            agent_mod._CODEXCREW_BIN = None
            with patch.dict("sys.modules", {"codex_crew": mock_mc}):
                result = _resolve_codexcrew_bin()
            assert result == str(codexcrew_bin)
        finally:
            agent_mod._CODEXCREW_BIN = old_val
    ...
    def test_falls_back_to_shutil_which(self, tmp_path: Path, no_interpreter_scripts):
        ...
        mock_mc = unittest.mock.MagicMock()
        mock_mc.__file__ = str(tmp_path / "codex_crew" / "__init__.py")
        (tmp_path / "codex_crew").mkdir()
        (tmp_path / "codex_crew" / "__init__.py").write_text("")

        # Create the fallback binary so _usable() validation passes
        fallback_bin = tmp_path / "usr_local_bin_codexcrew"
        fallback_bin.write_text("#!/bin/sh\n")
        fallback_bin.chmod(0o755)

        # Selective isfile mock: only the explicit fallback passes validation;
        # any real /bin/codexcrew the walk might find gets rejected.
    ...
    def test_returns_codexcrew_when_not_found(self, tmp_path: Path, no_interpreter_scripts):
        """Returns 'codexcrew' string when not found anywhere."""
        import codex_crew.agent as agent_mod
        from codex_crew.agent import _resolve_codexcrew_bin

        mock_mc = unittest.mock.MagicMock()
        mock_mc.__file__ = str(tmp_path / "codex_crew" / "__init__.py")
        (tmp_path / "codex_crew").mkdir(exist_ok=True)
        (tmp_path / "codex_crew" / "__init__.py").write_text("")
    ...
    def test_skips_stale_shutil_which_result(self, tmp_path: Path, no_interpreter_scripts):
        ...
        def _which(cmd: str) -> str | None:
            ...
            if cmd == "codexcrew":
                return "/home/user/.local/bin/codexcrew-DELETED"
            return None
        ...
        try:
            ...
            with patch.dict("sys.modules", {"codex_crew": mock_mc}):
                ...
                with patch("os.path.isfile", return_value=False):
                    with patch("shutil.which", side_effect=_which):
                        result = _resolve_codexcrew_bin()
            # Must NOT cache the stale path — falls through to bare 'codexcrew'
            assert result == "codexcrew"
            assert agent_mod._CODEXCREW_BIN is None  # didn't cache fallback
        finally:
            agent_mod._CODEXCREW_BIN = old_val

    def test_walk_and_path_miss_falls_back_to_bare(self, tmp_path: Path, no_interpreter_scripts):
        """When the bin walk and PATH both miss, fall back to bare 'codexcrew'.
        ...
        codexcrew_bin = runtime_farm / "bin" / "codexcrew"
        codexcrew_bin.write_text("#!/bin/sh\n")
        codexcrew_bin.chmod(0o755)
        ...
        try:
            ...
            with patch.dict("sys.modules", {"codex_crew": mock_mc}):
                ...
                def _fake_isfile(p):
                    return p == str(codexcrew_bin) and _real_isfile(p)
            ...
            assert result == "codexcrew"
            assert agent_mod._CODEXCREW_BIN is None
        finally:
            agent_mod._CODEXCREW_BIN = old_val
                ...
                with patch("os.path.isfile", return_value=False):
                    with patch("shutil.which", side_effect=_which):
                        with patch(
                            ...
                            result = _resolve_codexcrew_bin()
            # Falls through to bare 'codexcrew', doesn't crash
            assert result == "codexcrew"
            assert agent_mod._CODEXCREW_BIN is None
        finally:
            agent_mod._CODEXCREW_BIN = old_val
    ...
    def test_caches_result(self):
        """Result is cached in global _CODEXCREW_BIN."""
        import codex_crew.agent as agent_mod
        from codex_crew.agent import _resolve_codexcrew_bin

        old_val = agent_mod._CODEXCREW_BIN
        try:
            agent_mod._CODEXCREW_BIN = "/cached/codexcrew"
            result = _resolve_codexcrew_bin()
            assert result == "/cached/codexcrew"
        finally:
            agent_mod._CODEXCREW_BIN = old_val
    ...
    def test_accepts_any_readable_bin_from_walk(self, tmp_path: Path):
        ...
        readable ``bin/codexcrew`` discovered by the walk is used as-is.
        """
        import codex_crew.agent as agent_mod
        from codex_crew.agent import _resolve_codexcrew_bin

        venv = tmp_path / "venv"
        pkg_dir = venv / "lib" / "python3.10" / "site-packages" / "codex_crew"
        pkg_dir.mkdir(parents=True)
        (pkg_dir / "__init__.py").write_text("")
        ...
        fallback_bin = tmp_path / "toolbox" / "bin" / "codexcrew"
            ...
            with patch.dict("sys.modules", {"codex_crew": mock_mc}):
                with patch("shutil.which") as mock_which:
                    mock_which.side_effect = lambda cmd, **kw: (
                        str(fallback_bin) if cmd == "codexcrew" else None
                    )
                    result = _resolve_codexcrew_bin()
            # The walk finds venv/bin/codexcrew and accepts it directly.
            assert result == str(codexcrew_bin)
        finally:
            agent_mod._CODEXCREW_BIN = old_val
    ...
    def test_accepts_apollo_binary_with_envroot(self, tmp_path: Path):
        ...
        # Create env/runtime/.envroot + env/runtime/bin/codexcrew (Apollo)
        runtime = tmp_path / "env" / "runtime"
        (runtime / "lib" / "python3.10" / "site-packages" / "codex_crew").mkdir(parents=True)
        (runtime / "lib" / "python3.10" / "site-packages" / "codex_crew" / "__init__.py").write_text(
            ""
        )
        ...
        mock_mc.__file__ = str(
            runtime / "lib" / "python3.10" / "site-packages" / "codex_crew" / "__init__.py"
        )
    ...
    def test_accepts_wrapper_bin_from_walk(self, tmp_path: Path):
        """A shell-wrapper bin/codexcrew found by the walk is accepted as-is.
        ...
        pkg_dir = project / "src" / "codex_crew"
        pkg_dir.mkdir(parents=True)
        (pkg_dir / "__init__.py").write_text("")

        bin_dir = project / "bin"
        bin_dir.mkdir()
        wrapper = bin_dir / "codexcrew"
        wrapper.write_text('#!/bin/sh\nexec codexcrew "$@"\n')
        wrapper.chmod(0o755)

        # A PATH fallback that should NOT be chosen.
        fallback_bin = tmp_path / "local" / "bin" / "codexcrew"
        fallback_bin.parent.mkdir(parents=True)
        fallback_bin.write_text("#!/bin/sh\n")
        fallback_bin.chmod(0o755)
        ...
        try:
            ...
            with patch.dict("sys.modules", {"codex_crew": mock_mc}):
                with patch("shutil.which") as mock_which:
                    ...
                    result = _resolve_codexcrew_bin()
            # The walk finds project/bin/codexcrew and accepts it directly.
            assert result == str(wrapper)
        finally:
            agent_mod._CODEXCREW_BIN = old_val

    def test_accepts_brazil_wrapper_inside_workspace(self, tmp_path: Path):
        """Accepts bin/codexcrew with brazil-runtime-exec when packageInfo exists."""
        from codex_crew.agent import _bin_is_usable
        ...
        bin_dir.mkdir()
        brazil_wrapper = bin_dir / "codexcrew"
        brazil_wrapper.write_text('#!/bin/sh\nexec brazil-runtime-exec codexcrew "$@"\n')
    ...
    def test_prefers_venv_bin_over_project_bin(self, tmp_path: Path):
        """Resolves .venv/bin/codexcrew before bin/codexcrew in the same tree."""
        import codex_crew.agent as agent_mod
        from codex_crew.agent import _resolve_codexcrew_bin

        # Create structure: project/src/codex_crew + project/.venv/bin/codexcrew
        # + project/bin/codexcrew (Brazil wrapper)
        project = tmp_path / "project"
        pkg_dir = project / "src" / "codex_crew"
        pkg_dir.mkdir(parents=True)
        (pkg_dir / "__init__.py").write_text("")

        # .venv/bin/codexcrew — the preferred candidate
        venv_bin = project / ".venv" / "bin" / "codexcrew"
        venv_bin.parent.mkdir(parents=True)
        venv_bin.write_text('#!/bin/sh\nexec python -m codex_crew "$@"\n')
        venv_bin.chmod(0o755)

        # bin/codexcrew — Brazil wrapper (should be skipped)
        bin_dir = project / "bin"
        bin_dir.mkdir()
        brazil_wrapper = bin_dir / "codexcrew"
        brazil_wrapper.write_text('#!/bin/sh\nexec brazil-runtime-exec codexcrew "$@"\n')
        ...
        try:
            ...
            with patch.dict("sys.modules", {"codex_crew": mock_mc}):
                result = _resolve_codexcrew_bin()
            # Should prefer .venv/bin/codexcrew over bin/codexcrew
    ...
    def test_venv_install_falls_through_to_step1(self, tmp_path: Path):
        ...
        # Simulate pip-into-venv: .venv/lib/python3.x/site-packages/codex_crew/
        venv_root = tmp_path / ".venv"
        pkg_dir = venv_root / "lib" / "python3.13" / "site-packages" / "codex_crew"
        pkg_dir.mkdir(parents=True)
        (pkg_dir / "__init__.py").write_text("")
        (venv_root / "pyvenv.cfg").write_text("home = /usr/bin\n")

        # .venv/bin/codexcrew exists (step 1 should find it)
        venv_bin = venv_root / "bin" / "codexcrew"
        venv_bin.parent.mkdir(parents=True)
        venv_bin.write_text('#!/bin/sh\nexec python -m codex_crew "$@"\n')
        venv_bin.chmod(0o755)
        ...
        try:
            ...
            with patch.dict("sys.modules", {"codex_crew": mock_mc}):
                result = _resolve_codexcrew_bin()
            # Step 0 breaks at pyvenv.cfg; step 1 finds bin/codexcrew
            assert result == str(venv_bin)
        finally:
            agent_mod._CODEXCREW_BIN = old_val
...
class TestKirocrewBinSubpath:
    ...
    On Windows the resolver must prefer the relocatable ``bin\\codexcrew.cmd``
    shim over the pip-generated ``Scripts\\codexcrew.exe``: inside the shipped
    ...
    def test_posix_returns_bin_codexcrew(self, tmp_path: Path, monkeypatch):
        from codex_crew import platform_compat
        from codex_crew.agent import _codexcrew_bin_subpath

        monkeypatch.setattr(platform_compat, "IS_WINDOWS", False)
        # Even with a stray codexcrew.cmd present, POSIX resolution is unchanged.
        (tmp_path / "bin").mkdir()
        (tmp_path / "bin" / "codexcrew.cmd").write_text("@echo off\n")
        assert _codexcrew_bin_subpath(tmp_path) == tmp_path / "bin" / "codexcrew"
    ...
    def test_windows_prefers_relocatable_cmd_shim(self, tmp_path: Path, monkeypatch):
        ...
        monkeypatch.setattr(platform_compat, "IS_WINDOWS", True)
        (tmp_path / "bin").mkdir()
        cmd_shim = tmp_path / "bin" / "codexcrew.cmd"
        cmd_shim.write_text('@echo off\r\n"%~dp0..\\python.exe" -s -m codex_crew %*\r\n')
        (tmp_path / "Scripts").mkdir()
        (tmp_path / "Scripts" / "codexcrew.exe").write_bytes(b"MZ")
        assert _codexcrew_bin_subpath(tmp_path) == cmd_shim

    def test_windows_falls_back_to_scripts_exe_without_cmd(self, tmp_path: Path, monkeypatch):
        """Plain pip install: no .cmd shim -> Scripts/codexcrew.exe as before."""
        from codex_crew import platform_compat
        from codex_crew.agent import _codexcrew_bin_subpath
        ...
        (tmp_path / "Scripts").mkdir()
        (tmp_path / "Scripts" / "codexcrew.exe").write_bytes(b"MZ")
        expected = tmp_path / "Scripts" / "codexcrew.exe"
        assert _codexcrew_bin_subpath(tmp_path) == expected

    def test_windows_cmd_must_be_a_file(self, tmp_path: Path, monkeypatch):
        """A directory named codexcrew.cmd does not shadow the .exe fallback."""
        from codex_crew import platform_compat
        from codex_crew.agent import _codexcrew_bin_subpath

        monkeypatch.setattr(platform_compat, "IS_WINDOWS", True)
        (tmp_path / "bin" / "codexcrew.cmd").mkdir(parents=True)
        expected = tmp_path / "Scripts" / "codexcrew.exe"
        assert _codexcrew_bin_subpath(tmp_path) == expected
    ...
    def test_resolver_walk_finds_cmd_shim_in_bundle_layout(self, tmp_path: Path, monkeypatch):
        ...
        resolving the co-present ``Scripts\\codexcrew.exe`` instead of the
        ...
        # <root>/Lib/site-packages/codex_crew + <root>/bin/codexcrew.cmd
        # + the pip-dropped <root>/Scripts/codexcrew.exe (non-relocatable).
        root = tmp_path / "codexcrew-backend"
        pkg_dir = root / "Lib" / "site-packages" / "codex_crew"
        pkg_dir.mkdir(parents=True)
        (pkg_dir / "__init__.py").write_text("")
        (root / "bin").mkdir()
        ...
        (root / "Scripts").mkdir()
        dead_exe = root / "Scripts" / "codexcrew.exe"
...
class TestKirocrewMcpInvocation:
    ...
    Regression: when ``_resolve_codexcrew_bin`` cannot find a usable
    standalone binary (e.g. the gateway runs as a systemd user service and
    ``codexcrew`` is not on the service PATH), the built-in cron/core servers
    must still get a runnable command instead of the bare ``"codexcrew"``
    sentinel, which fails validation and drops them from ``codexcrew.json`` on
    every refresh.
    """

    def test_uses_standalone_binary_when_resolved(self):
        from codex_crew.agent import _codexcrew_mcp_invocation

        with patch("codex_crew.agent._resolve_codexcrew_bin", return_value="/opt/bin/codexcrew"):
            cmd, args = _codexcrew_mcp_invocation("mcp-cron")
        assert cmd == "/opt/bin/codexcrew"
        assert args == ["mcp-cron"]

    def test_falls_back_to_interpreter_module_when_unresolved(self):
        from codex_crew.agent import _codexcrew_mcp_invocation

        # Bare "codexcrew" is the unresolved sentinel from _resolve_codexcrew_bin.
        with patch("codex_crew.agent._resolve_codexcrew_bin", return_value="codexcrew"):
            cmd, args = _codexcrew_mcp_invocation("mcp-core")
        assert cmd == sys.executable
        assert args == ["-m", "codex_crew", "mcp-core"]

    def test_unwraps_cmd_shim_to_sibling_interpreter(self, tmp_path: Path):
        """A resolved bin/codexcrew.cmd is never emitted verbatim (#4439).

        Mirrors website/electron/main.js: the shim is unwrapped to
        ``<root>/python.exe -s -m codex_crew <sub>`` so kiro-cli spawns the
        ...
        assert args == ["-P", "-s", "-m", "codex_crew", "mcp-cron"]
...
class TestKiroHooksMerge:
    ...
    def test_user_hooks_dedup_against_bundled(self, tmp_path: Path):
        ...
        from codex_crew.agent import _merge_kiro_hooks
    ...
    def test_validate_rejects_relative_path(self, tmp_path: Path):
        from codex_crew.agent import _validate_hook_command
    ...
    def test_validate_rejects_symlink_to_sensitive(self, tmp_path: Path):
        ...
        with patch("codex_crew.agent.is_sensitive_path", side_effect=lambda p: ".ssh" in p):
    ...
    def test_merge_kiro_hooks_max_per_event_limit(self, tmp_path: Path):
        ...
        from codex_crew.agent import _MAX_USER_HOOKS_PER_EVENT, _merge_kiro_hooks
    ...
    def test_merge_rejects_oversized_matcher(self, tmp_path: Path):
        ...
        from codex_crew.agent import _MAX_MATCHER_LEN, _merge_kiro_hooks
    ...
    def test_merge_global_hooks_limit(self, tmp_path: Path):
        ...
        from codex_crew.agent import _MAX_TOTAL_USER_HOOKS, _merge_kiro_hooks
...
class TestKiroHooksFiltering:
    """Tests that CodexCrew-internal hook keys are stripped from kiro-cli agent config."""

    def test_auto_approve_tools_stripped_from_agent_config(self, tmp_path: Path):
        """auto_approve_tools must not appear in codexcrew.json (kiro-cli rejects it)."""
        from codex_crew.agent import _kiro_hooks_only

        hooks = {
            "auto_approve_tools": ["codexcrew browse *"],
    ...
    def test_auto_deny_tools_stripped_from_agent_config(self):
        """auto_deny_tools must not appear in codexcrew.json."""
        from codex_crew.agent import _kiro_hooks_only
    ...
    def test_only_valid_kiro_events_preserved(self):
        ...
        from codex_crew.agent import _VALID_HOOK_EVENTS, _kiro_hooks_only
    ...
    def test_bundled_defaults_hook_keys_are_classified(self):
        ...
        (Codex-Crew-internal), never neither (#3362 fail-loud guard)."""
        from codex_crew.agent import _BUNDLED_CFG_DIR, _load_json
        ...
        assert bundled_hook_keys == {"auto_approve_tools", "postToolUse"}, (
            ...
            "via _VALID_HOOK_EVENTS) or Codex-Crew-internal (add to "
    ...
    def test_sanitize_agent_hooks_repairs_owned_files_subtractively(self, tmp_path: Path):
        """The repair removes only Codex Crew's legacy key from every owned spec."""
        from codex_crew.agent import _hooks_sanitized_mtimes, _sanitize_agent_hooks
        from codex_crew.agent_files import OWNED_KIRO_AGENT_FILES
        ...
        with patch("codex_crew.agent.KIRO_AGENTS_DIR", kiro_dir):
    ...
    @pytest.mark.parametrize(
        "filename", ["other-tool.json", "codexcrew-custom.json", "sample-app--worker.json"]
    )
    ...
    def test_sanitize_agent_hooks_skips_clean_file(self, tmp_path: Path):
        ...
        (kiro_dir / "codexcrew.json").write_text(json.dumps(clean_config))
        original_mtime = (kiro_dir / "codexcrew.json").stat().st_mtime

        _hooks_sanitized_mtimes.clear()
        with patch("codex_crew.agent.KIRO_AGENTS_DIR", kiro_dir):
            _sanitize_agent_hooks()

        assert (kiro_dir / "codexcrew.json").stat().st_mtime == original_mtime
...
class TestToolBloatFixes:
    ...
    def test_fresh_install_adds_managed_mcp_to_tools_only(self, tmp_path: Path):
        ...
        path = _run_install(tmp_path, cfg_dir)
        config = json.loads(path.read_text(encoding="utf-8"))
        assert "@codexcrew-cron" in config["tools"]
        assert "@codexcrew-core" in config["tools"]
        assert "@codexcrew-cron" not in config["allowedTools"]
        assert "@codexcrew-core" not in config["allowedTools"]
    ...
    def test_a_malformed_store_value_does_not_confer_ownership(self, tmp_path: Path):
        ...
        ``codexcrew_mcp`` value entirely, so it contributes no hints and cannot be
        the source of truth for any — yet a bare `name in codexcrew_mcp` test
    ...
    def test_an_agent_config_only_server_keeps_its_oauth_hints_verbatim(self, tmp_path: Path):
        ...
        ``kiro-cli mcp add --agent codexcrew``, or hand-edited in. No mcp.json scope
    ...
    def test_non_dict_json_treated_as_fresh_install(self, tmp_path: Path):
        ...
        kiro_dir = tmp_path / "kiro_agents"
        kiro_dir.mkdir(exist_ok=True)
        (kiro_dir / "codexcrew.json").write_text('"just a string"')
...
class TestKiroHooksAutoimport:
    ...
    def test_kiro_hooks_autoimport_loads_executable_scripts(self, tmp_path: Path):
        ...
        from codex_crew.agent import _autoimport_kiro_hooks
    ...
    def test_kiro_hooks_autoimport_skips_non_executable(self, tmp_path: Path, caplog):
        ...
        with caplog.at_level(logging.INFO, logger="codex_crew.agent"):
            result = _autoimport_kiro_hooks(hooks_dir)
    ...
    def test_kiro_hooks_autoimport_dedupes_with_explicit_config(self, tmp_path: Path):
        ...
        from codex_crew.agent import _apply_user_kiro_hooks
    ...
    def test_kiro_hooks_autoimport_respects_total_limit(self, tmp_path: Path, caplog):
        ...
        from codex_crew.agent import _MAX_TOTAL_USER_HOOKS, _apply_user_kiro_hooks
        ...
        with caplog.at_level(logging.WARNING, logger="codex_crew.agent"):
            _apply_user_kiro_hooks(config, mc_cfg)
    ...
    def test_kiro_hooks_per_event_cap_emits_sel_audit(self, tmp_path: Path, monkeypatch, caplog):
        ...
        from codex_crew import agent as _agent_mod
        from codex_crew.agent import _MAX_USER_HOOKS_PER_EVENT, _apply_user_kiro_hooks
    ...
    def test_kiro_hooks_autoimport_missing_dir_is_noop(self, tmp_path: Path, caplog):
        ...
        with caplog.at_level(logging.DEBUG, logger="codex_crew.agent"):
            result = _autoimport_kiro_hooks(missing)

        assert result == {}
        # Scope to the codex_crew.agent logger: under `pytest -n auto`, a leaked
        ...
        warnings = [
            ...
            if r.levelno >= logging.WARNING and r.name == "codex_crew.agent"
        ]
        assert warnings == []
    ...
    def test_kiro_hooks_autoimport_rejects_dir_equal_to_home(
        ...
        monkeypatch.setattr(
            "codex_crew.agent._DEFAULT_KIRO_HOOKS_DIR",
            tmp_path / ".kiro" / "hooks",
        )
...
class TestRefreshDynamicFieldsStripsStaleUrl:
    ...
    def test_stale_url_and_headers_removed_from_managed_entry(self):
        from codex_crew.agent import _refresh_dynamic_fields

        config = {
            "mcpServers": {
                "codexcrew-core": {
                    "url": "http://localhost:5486/api/mcp/core",
                    "headers": {"X-Stale": "1"},
                },
                "codexcrew-cron": {"url": "http://localhost:5486/api/mcp/cron"},
            }
        }
        _refresh_dynamic_fields(config)
        for name, args in (("codexcrew-core", ["mcp-core"]), ("codexcrew-cron", ["mcp-cron"])):
            ...
            # ``-m codex_crew.__main__`` before the same managed subcommand.
    ...
    def test_non_managed_server_oauth_hints_preserved(self):
        """scopes/clientId are passthrough — the runtime, not Codex Crew, uses them."""
        from codex_crew.agent import _refresh_dynamic_fields
...
class TestMigrateAgentSpecs:
    """migrate_agent_specs lifts CodexCrew bookkeeping keys into the sidecar."""
    ...
    def test_strips_and_lifts_keys(self, tmp_path: Path):
        ...
        with patch("codex_crew.agent.KIRO_AGENTS_DIR", kiro):
    ...
    def test_installed_spec_is_schema_clean(self, tmp_path: Path):
        """After install the written spec carries no CodexCrew bookkeeping keys."""
        cfg_dir = _bundled_defaults(tmp_path)
        path = _run_install(tmp_path, cfg_dir)
        config = json.loads(path.read_text(encoding="utf-8"))
        assert "model_managed" not in config
...
class TestRebuildReconcileRetainsEnabledAppServers:
    """The final codexcrew.json reconcile must not delete an ENABLED app's
    ...
    ``install_agent`` gated on ``is_codexcrew_json`` (the written path equalling
...
class TestMcpMergePriority:
    def test_the_authorship_marker_never_reaches_the_rendered_spec(self, tmp_path: Path):
        ...
        The marker records that Codex Crew wrote an entry into a file it does NOT
        ...
        """
        from codex_crew.mcp_provenance import MARKER_KEY, stamp
    ...
    def test_codexcrew_override_does_not_corrupt_global_fallback(self, tmp_path: Path):
        """Regression (review-bot): a codexcrew mcp.json override carrying an
        ...
        config = _run_install_mcp_merge(
            ...
            cc_servers={},
            kiro_servers={"srv": {"command": kiro_cmd}},
            codexcrew_servers={"srv": {"command": "bare-unresolvable"}},
...
class TestRefreshDynamicFieldsSyncsConfigModel:
    ...
    def test_explicit_config_model_overrides_managed_default(self, tmp_path: Path):
        ...
        agent_state.set_model_managed("codexcrew", True)
        mc = self._write_mc_config(tmp_path, "claude-opus-4.8")
        config = {"name": "codexcrew", "model": "stale-from-install"}
        with patch("codex_crew.agent._mc_config_path", return_value=mc):
            _refresh_dynamic_fields(config)
    ...
    def test_auto_sentinel_does_not_clobber_agent_model(self, tmp_path: Path):
        ...
        config = {"name": "codexcrew", "model": "claude-haiku-4.5"}
        with patch("codex_crew.agent._mc_config_path", return_value=mc):
            _refresh_dynamic_fields(config)
    ...
    def test_no_config_model_leaves_agent_model_untouched(self, tmp_path: Path):
        ...
        config = {"name": "codexcrew", "model": "claude-sonnet-4.6"}
        with patch("codex_crew.agent._mc_config_path", return_value=mc):
            _refresh_dynamic_fields(config)
...
class TestResetAgentModel:
    ...
    def test_clear_model_pin_drops_the_pin_and_resumes_tracking(self):
        from codex_crew.agent import clear_model_pin

        config = {"name": "codexcrew", "model": "claude-opus-4.8"}
        clear_model_pin(config, "codexcrew")
    ...
    def test_reset_writes_the_spec_and_reports_the_previous_model(
        ...
        self._spec(tmp_path, "codexcrew", {"name": "codexcrew", "model": "claude-opus-4.8"})
        monkeypatch.setattr(agent_mod, "kiro_agents_dir_path", lambda: tmp_path)

        spec_path, previous = agent_mod.reset_agent_model("codexcrew")

        assert previous == "claude-opus-4.8"
        assert json.loads(spec_path.read_text(encoding="utf-8")) == {"name": "codexcrew"}
        assert agent_state.get_model_managed("codexcrew") is True
    ...
    def test_reset_overrides_an_explicit_freeze(self, tmp_path: Path, monkeypatch):
        ...
        agent_state.set_model_managed("codexcrew", False)
        self._spec(tmp_path, "codexcrew", {"name": "codexcrew", "model": "claude-haiku-4.5"})
        monkeypatch.setattr(agent_mod, "kiro_agents_dir_path", lambda: tmp_path)

        agent_mod.reset_agent_model("codexcrew")
        assert agent_state.get_model_managed("codexcrew") is True
    ...
    def test_reset_never_writes_bookkeeping_into_the_spec(self, tmp_path: Path, monkeypatch):
        ...
        self._spec(
            tmp_path,
            "codexcrew",
            {"name": "codexcrew", "model": "claude-opus-4.8", "cc_model": "claude-sonnet-4.6"},
        )
        monkeypatch.setattr(agent_mod, "kiro_agents_dir_path", lambda: tmp_path)

        spec_path, _ = agent_mod.reset_agent_model("codexcrew")
        written = json.loads(spec_path.read_text(encoding="utf-8"))
        assert "cc_model" not in written and "model_managed" not in written
        assert agent_state.get_cc_model("codexcrew") == "claude-sonnet-4.6"
...
class TestSpecPathRefusesSymlinks:
    ...
    def test_a_symlinked_spec_is_refused_and_the_target_is_not_copied(
        ...
        link = agents / "codexcrew.json"
        link.symlink_to(secret)
        monkeypatch.setattr(agent_mod, "kiro_agents_dir_path", lambda: agents)

        assert agent_mod.agent_spec_path("codexcrew") is None
        with pytest.raises(FileNotFoundError):
            agent_mod.reset_agent_model("codexcrew")
    ...
    def test_a_plain_spec_is_still_accepted(self, tmp_path: Path, monkeypatch):
        ...
        agents = tmp_path / "agents"
        agents.mkdir()
        (agents / "codexcrew.json").write_text(json.dumps({"name": "codexcrew"}), encoding="utf-8")
        monkeypatch.setattr(agent_mod, "kiro_agents_dir_path", lambda: agents)

        assert agent_mod.agent_spec_path("codexcrew") == agents / "codexcrew.json"
...
class TestSpecReadsAreSizeCapped:
    ...
    def test_an_oversized_spec_is_refused_by_resolver_and_reset(self, tmp_path: Path, monkeypatch):
        import codex_crew.agent as agent_mod
        from codex_crew import hooks

        monkeypatch.setattr(hooks, "MAX_FILE_BYTES", 256)
        agents = tmp_path / "agents"
        agents.mkdir()
        (agents / "codexcrew.json").write_text(
            json.dumps({"name": "codexcrew", "model": "m", "pad": "x" * 1024}), encoding="utf-8"
        )
        monkeypatch.setattr(agent_mod, "kiro_agents_dir_path", lambda: agents)
    ...
    def test_a_normal_sized_spec_under_the_same_cap_still_resets(
        ...
        agents.mkdir()
        (agents / "codexcrew.json").write_text(
            json.dumps({"name": "codexcrew", "model": "claude-opus-4.8"}), encoding="utf-8"
        )
        monkeypatch.setattr(agent_mod, "kiro_agents_dir_path", lambda: agents)

        _, previous = agent_mod.reset_agent_model("codexcrew")
        assert previous == "claude-opus-4.8"
...
class TestResetOutputEscapesUntrustedPaths:
    ...
    def test_the_success_line_escapes_the_path(self, monkeypatch, capsys):
        import codex_crew.cli_commands as cli_commands
        ...
        cli_commands._agent_reset_model(argparse.Namespace(agent="codexcrew"))
    ...
    def test_the_ambiguity_refusal_escapes_both_paths(self, tmp_path: Path, monkeypatch):
        ...
        (agents / "elsewhere.json").write_text(
            json.dumps({"name": "codexcrew"}), encoding="utf-8"
        )
        monkeypatch.setattr(agent_mod, "kiro_agents_dir_path", lambda: agents)
...
class TestSelHookRejectedRedaction:
    ...
    def test_credential_straddling_the_cut_is_not_leaked(self, monkeypatch) -> None:
        from codex_crew.agent import _sel_hook_rejected
