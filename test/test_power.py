...
from codex_crew import platform_compat, power
...
def _patch_prevent_sleep_flag(monkeypatch, enabled: bool):  # type: ignore[no-untyped-def]
    ...
    monkeypatch.setattr(server.CodexCrewConfig, "load", classmethod(lambda cls: _Cfg()))
    return server
...
def test_prevent_sleep_is_editable_and_defaults_off():
    ...
    assert CodexCrewConfig().dashboard.prevent_sleep is False
