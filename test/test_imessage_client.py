"""Tests for codex_crew.imessage.client (watch resume, dedupe, capability probe).
...
from codex_crew.imessage import client as client_mod
from codex_crew.imessage.client import (
    DEDUPE_WINDOW,
    ECHO_TTL_S,
    ECHO_WINDOW,
    WATCH_BUFFER_LIMIT,
...
class TestOwnEchoLedger:
    ...
    async def test_every_suppression_is_logged_not_only_the_first(
        ...
        imc = await _client(tmp_path)
        peers[0].default_result = {"ok": True}
        with caplog.at_level(logging.DEBUG, logger="codex_crew.imessage.client"):
            for i in range(3):
