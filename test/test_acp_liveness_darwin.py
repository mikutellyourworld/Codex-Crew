...
from codex_crew.acp import liveness
...
def _mcp_tool(clock: _Clock, tool_name: str = "use_subagent") -> ToolCallState:
    return ToolCallState(
        title="codexcrew-core___use_subagent",
        command='{"task": "x"}',
        dispatch_ts=clock.t,
...
def test_observable_tree_with_only_old_descendants_is_tagged_absent(tmp_path):
    clock = _Clock()
    backend = FakeBackend()
    backend.add(201, "python -m codex_crew.mcp_gateway.stub --server github", started=0.0)
    backend.add(202, "python -m codex_crew.mcp_gateway.stub --server slack", started=0.0)
    oracle = _oracle(backend, clock, tmp_path)
...
def test_undatable_descendant_keeps_the_full_window(tmp_path):
    ...
    clock = _Clock()
    backend = FakeBackend()
    backend.add(201, "python -m codex_crew.mcp_gateway.stub --server github", started=None)
    oracle = _oracle(backend, clock, tmp_path)
    tool = _shell_tool("ls /some/dir | grep needle | wc -l", clock)
...
def test_mcp_subtree_cpu_delta_is_working_with_the_cpu_only_label(tmp_path):
    clock = _Clock()
    backend = FakeBackend()
    backend.add(210, "python -m codex_crew.mcp_gateway.stub --server github", cpu=5_000_000_000)
