...
the per-test ``CODEXCREW_HOME`` that Codex Crew's conftest pins.
"""
...
from codex_crew.mcp_gateway.backend import Backend, BackendGone
from codex_crew.mcp_gateway.hashing import hash_effective_env
from codex_crew.mcp_gateway.pool import BackendPool, BackendUnavailable, PoolKey
...
class TestDeclaredNonSecretEnv:
    ...
    def test_unreadable_config_falls_back_to_the_default_overlay_dir(self, monkeypatch):
        monkeypatch.setattr(
            gw.CodexCrewConfig, "load", classmethod(lambda cls: (_ for _ in ()).throw(OSError("nope")))
        )
...
class TestEnvTargetResolver:
    def test_unmapped_server_returns_none(self, monkeypatch):
        ...
        monkeypatch.delenv("CODEXCREW_MCP_TARGET_UNMAPPED_MCP", raising=False)
    ...
    def test_whitespace_only_spec_returns_none(self, monkeypatch):
        ...
        monkeypatch.setenv("CODEXCREW_MCP_TARGET_BLANK_MCP", "   ")
        assert gw.env_target_resolver(key) is None

    def test_legacy_prefix_is_still_accepted(self, monkeypatch):
        key = _pool_key(server="legacy-mcp")
        monkeypatch.delenv("CODEXCREW_MCP_TARGET_LEGACY_MCP", raising=False)
    ...
    def test_python_env_prefixes_are_stripped_from_spawned_env(self, monkeypatch):
        ...
        monkeypatch.delenv("CODEXCREW_MCP_TARGET_PYENV_MCP", raising=False)
...
class TestMetricEmitters:
    def test_backend_acquire_metric_carries_the_warm_attribute(self, monkeypatch):
        ...
        assert rec.histogram.call_args.args[0] == "codexcrew.mcp.backend.acquire.duration"
    ...
    def test_lazy_load_metrics_also_emit_the_acquire_histogram(self, monkeypatch):
        ...
        assert "codexcrew.mcp.lazy_load.duration" in names
        assert "codexcrew.mcp.backend.acquire.duration" in names
        assert rec.counter.call_args.args[0] == "codexcrew.mcp.lazy_load.count"
