...
def _cfg(**kw: object) -> CodexCrewConfig:
    defaults: dict = {
    ...
    defaults.update(kw)
    return CodexCrewConfig(**defaults)


_SEL = "codex_crew.sel.sel"
_LOAD = "codex_crew.config.loader.CodexCrewConfig.load"
_CFGDIR = "codex_crew.config.loader.config_dir"
_DATAHOME = "codex_crew.dashboard.handlers.files.data_home"
...
class TestDeleteHandler:
    ...
    async def test_delete_referenced_returns_409(self) -> None:
        cfg = _cfg(
            ...
            agents={"bot": CodexCrewAgentConfig(workspace="staging")},
        )
        with patch(_LOAD, return_value=cfg):
