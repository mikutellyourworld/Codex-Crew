...
from codex_crew.dashboard.handlers import api_sessions_search
...
class TestSessionsSearchHandler:
    ...
    async def test_title_is_redacted(self, tmp_path, monkeypatch):
        ...
        # Patched on `codex_crew.security`, not on the `handlers` re-exports: the
        ...
        monkeypatch.setattr("codex_crew.security.redact_credentials", _fake_redact_creds)
        monkeypatch.setattr("codex_crew.security.redact_exfiltration_urls", _fake_redact_urls)
        async with TestClient(TestServer(_make_app(log))) as client:
