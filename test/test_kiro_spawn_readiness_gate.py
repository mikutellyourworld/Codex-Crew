...
from codex_crew.dashboard import kiro_readiness
from codex_crew.dashboard.handlers import agents, sessions
from codex_crew.kiro_prerequisite import KiroPrerequisiteService

_RESOLVE_TARGET = "codex_crew.acp.client._resolve_kiro_bin_for_spawn"
...
async def test_refused_call_is_visible_in_the_log(
    ...
    with caplog.at_level("WARNING", logger="codex_crew.dashboard.kiro_readiness"):
    ...
    refusals = [
        ...
        if r.name == "codex_crew.dashboard.kiro_readiness" and r.levelname == "WARNING"
    ]
...
async def test_a_newline_in_the_path_cannot_forge_a_log_line(
    ...
    assert resp.status == 503
    records = [r for r in caplog.records if r.name == "codex_crew.dashboard.kiro_readiness"]
