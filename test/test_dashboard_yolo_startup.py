...
from codex_crew.dashboard.server import _apply_startup_yolo
from codex_crew.dashboard.state import DashboardState
from codex_crew.safety_override import SafetyOverride, reset_singleton, safety_override
...
def test_startup_seeds_the_adhoc_ttl_even_when_yolo_is_off() -> None:
    ...
    with patch("codex_crew.safety_override.sel"), patch(
        "codex_crew.config.loader.CodexCrewConfig.load", return_value=cfg
    ):
...
def test_apply_startup_yolo_handles_exception_gracefully() -> None:
    ...
    state = _make_state()
    with patch(
        "codex_crew.dashboard.server.grant_declared_yolo",
