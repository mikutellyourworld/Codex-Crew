...
from codex_crew.cli_commands import _cron
from codex_crew.cli_doctor import _doctor
from codex_crew.cli_server import _update
...
def _write_agent_config(path: Path, *, tools: list[str], allowed: list[str], servers: dict) -> None:
    """Write a ``codexcrew.json`` agent config with the given managed
...
def _healthy_agent_file(path: Path) -> None:
    """Write a ``codexcrew.json`` whose managed MCP servers are all present
    ...
    (``mcp_cleanup.CODEXCREW_BIN_MCP_SERVERS``) rather than hardcoded: doctor
    ...
    added — which is exactly how these fixtures broke when ``codexcrew-computer``
    landed.
    """
    from codex_crew.mcp_cleanup import CODEXCREW_BIN_MCP_SERVERS

    refs = [f"@{name}" for name in CODEXCREW_BIN_MCP_SERVERS]
    _write_agent_config(
        path,
        tools=refs,
        allowed=refs,
        servers={
            name: {
                # sys.executable, not a literal like /usr/local/bin/codexcrew:
                ...
                # The subcommand is the server name minus the "codexcrew-" prefix
                # ("codexcrew-core" -> "mcp-core"), matching the real invocation.
                "args": [f"mcp-{name.split('-', 1)[1]}"],
            }
            for name in CODEXCREW_BIN_MCP_SERVERS
        },
    )
...
def _pin_default_config(monkeypatch) -> None:
    ...
    ``_doctor()`` calls the real ``CodexCrewConfig.load()`` / ``load_credentials()``,
    which read the shared ``~/.codexcrew`` config at runtime. ``CodexCrewConfig.save()``
    ...
    def _pristine() -> CodexCrewConfig:
        cfg = CodexCrewConfig()
        cfg.stt.enabled = False
        return cfg

    monkeypatch.setattr(CodexCrewConfig, "load", classmethod(lambda cls: _pristine()))
    monkeypatch.setattr(CodexCrewConfig, "load_credentials", lambda self: {})
...
class TestDoctor:
    ...
    def _hermetic_embeddings_runtime(self, monkeypatch):
        ...
        import codex_crew.cli_doctor as _doc
    ...
    def test_doctor_with_kiro(self, tmp_path):
        agent_file = tmp_path / "codexcrew.json"
        ...
        with (
            patch(
                "codex_crew.cli_doctor.shutil.which",
                side_effect=lambda b, **_kw: f"/usr/local/bin/{b}",
            ),
            patch("codex_crew.cli_doctor.KIRO_AGENTS_DIR", tmp_path),
            patch("codex_crew.cli_doctor.subprocess.run", return_value=mock_run),
            patch("urllib.request.urlopen", side_effect=urllib.error.URLError("no gateway")),
            patch("codex_crew.cli_doctor.is_local_only", return_value=True),
            patch("codex_crew.cli_doctor.config_dir", return_value=tmp_path),
            patch("codex_crew.cli_doctor.probe_server", side_effect=_noop_probe_server),
        ):
            _doctor()
    ...
    def _stt_config(monkeypatch, **fields):
        ...
        monkeypatch.setattr(CodexCrewConfig, "load", classmethod(lambda cls: _cfg()))
    ...
    def test_doctor_windows_missing_stt_engine_and_ffmpeg_is_non_fatal(self, tmp_path, monkeypatch):
        ...
        with (
            patch("codex_crew.cli_doctor.shutil.which", side_effect=_which),
            patch("codex_crew.cli_doctor.KIRO_AGENTS_DIR", tmp_path),
            patch("codex_crew.cli_doctor.subprocess.run", return_value=mock_run),
            patch("urllib.request.urlopen", side_effect=urllib.error.URLError("no gateway")),
            patch("codex_crew.cli_doctor.is_local_only", return_value=True),
    ...
    def test_doctor_reports_platform_boot_error_without_crashing(self, tmp_path, capsys):
        ...
        from codex_crew.platform import PlatformCompositionError

        agent_file = tmp_path / "codexcrew.json"
        _healthy_agent_file(agent_file)
        mock_run = MagicMock(returncode=0, stdout="kiro-cli 1.0.0", stderr="")
        boot_err = PlatformCompositionError(
            "profile=amazon resolved no companion; set CODEXCREW_PROFILE=standalone"
        )
        with (
            patch(
                "codex_crew.cli_doctor.shutil.which",
        ...
        assert "CODEXCREW_PROFILE=standalone" in out

    def test_doctor_without_kiro(self, tmp_path):
        with (
            patch("codex_crew.cli_doctor.shutil.which", return_value=None),
            patch("urllib.request.urlopen", side_effect=urllib.error.URLError("no gateway")),
            patch("codex_crew.cli_doctor.is_local_only", return_value=True),
            patch("codex_crew.cli_doctor.config_dir", return_value=tmp_path),
        ):
    ...
    def test_doctor_remote_shows_ssh_tunnel_hint(self, tmp_path, capsys):
        ...
        with (
            ...
            patch("urllib.request.urlopen", side_effect=urllib.error.URLError("no gateway")),
            patch("codex_crew.cli_doctor.is_local_only", return_value=False),
            patch("codex_crew.cli_doctor.config_dir", return_value=tmp_path),
            patch("codex_crew.cli_doctor.machine_hostname", return_value="myhost"),
        ):
            try:
                _doctor()
            except SystemExit:
    ...
    def test_doctor_slack_workspace_allowed_ok(self, tmp_path, capsys):
        ...
        with (
            ...
            patch("codex_crew.cli_doctor.config_dir", return_value=tmp_path),
            patch("codex_crew.cli_doctor.probe_server", side_effect=_noop_probe_server),
            patch("codex_crew.cli_doctor.CodexCrewConfig.load_credentials", return_value=slack_creds),
            patch("codex_crew.slack.enterprise.validate_enterprise", return_value=True) as mock_ve,
        ):
            _doctor()
        out = capsys.readouterr().out
    ...
    def test_doctor_slack_workspace_not_allowed_flags_issue(self, tmp_path, capsys):
        ...
        with (
            ...
            patch("codex_crew.cli_doctor.probe_server", side_effect=_noop_probe_server),
            patch("codex_crew.cli_doctor.CodexCrewConfig.load_credentials", return_value=slack_creds),
            patch("codex_crew.slack.enterprise.validate_enterprise", return_value=False),
        ):
            with pytest.raises(SystemExit) as exc:
                _doctor()
        assert exc.value.code == 1
...
class TestSetupWorkspaceDir:
    ...
    def test_uses_saved_path_as_default(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.cli_setup._workspace_dir_file", lambda: ws_file)
        with patch("builtins.input", return_value=str(custom_dir)) as mock_input:
            from codex_crew.cli_setup import _setup_workspace_dir
...
def _patch_path():
    ...
    return patch("codex_crew.cli_server.Path", return_value=mock_path_inst)
...
def _git_resolvable():
    ...
    """
    with patch(
        "codex_crew.platform.update_capability.trusted_system_bin",
...
class TestUpdateFailures:
    ...
    def test_git_fetch_failure_exits(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CODEXCREW_PROJECT_DIR", str(tmp_path))
        monkeypatch.setattr(
            "codex_crew.platform.update_capability.running_from_checkout",
            lambda root, **kw: True,
        )
    ...
    def test_pip_install_failure_exits(self, tmp_path, monkeypatch):
        ...
        with (
            _patch_path(),
            _git_resolvable(),
            patch("codex_crew.cli_server.shutil.which", return_value=None),
            patch("codex_crew.cli_server.build_frontend_sync"),
            patch("codex_crew.cli._ensure_node"),
            ...
            patch("codex_crew.dep_sync.locked_console_scripts", return_value=[]),
            patch("codex_crew.dep_sync.venv_not_mapped_to", return_value=None),
...
class TestCronCli:
    def test_cron_add_with_channel(self, tmp_path):
        with (
            patch("codex_crew.cli_commands.CronService") as mock_svc_cls,
            patch("codex_crew.cli_commands.sel"),
        ):
    ...
    def test_cron_add_with_approval_mode(self, tmp_path):
        with (
            patch("codex_crew.cli_commands.CronService") as mock_svc_cls,
            patch("codex_crew.cli_commands.sel") as mock_sel,
        ):
            mock_svc = mock_svc_cls.return_value
            mock_job = MagicMock()
            mock_job.id = "ghi"
    ...
    def test_cron_update_whitespace_channel_skipped(self, tmp_path, capsys):
        with patch("codex_crew.cli_commands.CronService") as mock_svc_cls:
            mock_svc = mock_svc_cls.return_value
    ...
    def test_cron_update_every_and_cron_exclusive(self, tmp_path, capsys):
        with patch("codex_crew.cli_commands.CronService"):
            args = argparse.Namespace(
                cron_action="update",
                job_id="job1",
                name=None,
    ...
    def test_cli_argparse_cron_add_agent_flag(self) -> None:
        """`codexcrew cron add ... --agent NAME` parses into args.agent."""
        import sys
        ...
        with patch.object(sys, "argv", argv), patch("codex_crew.cli_commands._cron") as mock_cron:
    ...
    def test_cli_argparse_cron_update_agent_flag(self) -> None:
        """`codexcrew cron update <id> --agent NAME` parses into args.agent."""
        import sys
...
class TestPortEnvValidatedAtEntry:
    """`main()` rejects an unusable CODEXCREW_PORT before any subcommand runs.

    Type alone is not enough. 70000 parses as an int, so a type-only check let
    `CODEXCREW_PORT=70000 codexcrew service install` bake an unbindable port into
    ...
    def test_out_of_range_port_exits_before_dispatch(self, monkeypatch, capsys):
        ...
        for bad in ("70000", "0", "-1"):
            monkeypatch.setenv("CODEXCREW_PORT", bad)
            dispatched = []
            with (
                patch.object(sys, "argv", ["codexcrew", "cron", "list"]),
                patch("codex_crew.cli_commands._cron", lambda _ns: dispatched.append(True)),
            ):
                from codex_crew.cli import main
    ...
    def test_in_range_port_is_accepted(self, monkeypatch):
        ...
        monkeypatch.setenv("CODEXCREW_PORT", "5477")
        dispatched = []
        with (
            patch.object(sys, "argv", ["codexcrew", "cron", "list"]),
            patch("codex_crew.cli_commands._cron", lambda _ns: dispatched.append(True)),
...
class TestSandboxActiveMarkerCleared:
    """cli.main() must drop an INHERITED CODEXCREW_SANDBOX_ACTIVE marker.
    ...
    def test_main_clears_inherited_sandbox_active_marker(self, monkeypatch):
        ...
        monkeypatch.setenv("CODEXCREW_SANDBOX_ACTIVE", "1")
        ...
        monkeypatch.setenv("CODEXCREW_SANDBOX_LEVEL", "strict")
        ...
        argv = ["codexcrew", "cron", "list"]
        seen = {}

        def _capture(_ns):
            seen["marker"] = os.environ.get("CODEXCREW_SANDBOX_ACTIVE")
            seen["level"] = os.environ.get("CODEXCREW_SANDBOX_LEVEL")

        with patch.object(sys, "argv", argv), patch("codex_crew.cli_commands._cron", _capture):
            from codex_crew.cli import main

            main()
        assert seen.get("marker") is None
        assert seen.get("level") is None
        assert os.environ.get("CODEXCREW_SANDBOX_ACTIVE") is None
        assert os.environ.get("CODEXCREW_SANDBOX_LEVEL") is None
...
class TestDirectCliOverrideAttestation:
    def test_agent_command_pins_override_before_jail_gate(self, tmp_path, monkeypatch):
        ...
        from codex_crew import cli, kiro_prerequisite
        ...
        monkeypatch.setenv("CODEXCREW_HOME", str(data_home))
        monkeypatch.setenv("CODEXCREW_KIRO_BIN", str(executable))
        monkeypatch.setattr(cli, "boot_platform", lambda *_args, **_kwargs: None)
        monkeypatch.setattr(cli.sys, "argv", ["codexcrew", "chat", "--no-jail"])
...
class TestSetupTimezone:
    def test_auto_detect_from_tz_env(self, monkeypatch):
        ...
        from codex_crew.cli_setup import _detect_system_timezone
    ...
    def test_windows_uses_tzlocal_when_no_posix_signal(self, monkeypatch):
        ...
        from codex_crew import cli_setup
    ...
    def test_input_or_skip_returns_none_on_empty_and_raises_on_eof(self, monkeypatch):
        ...
        from codex_crew.cli_setup import _input_or_skip, _SetupAborted
    ...
    def test_timezone_retry_eof_propagates_setup_aborted(self, tmp_path, monkeypatch):
        ...
        cfg_file.write_text("{}")
        monkeypatch.setattr("codex_crew.cli_setup.config_path", lambda: cfg_file)

        from codex_crew.cli_setup import _setup_timezone, _SetupAborted
        ...
        with patch("builtins.input", _input):
            with patch("codex_crew.cli_setup._detect_system_timezone", return_value=""):
                with pytest.raises(_SetupAborted):
                    _setup_timezone()
    ...
    def test_auto_detect_from_symlink(self, tmp_path, monkeypatch):
        ...
        from codex_crew.cli_setup import _setup_timezone

        with patch("builtins.input", return_value="") as mock_input:
            with patch(
                "codex_crew.cli_setup._detect_system_timezone",
...
class TestGetAlias:
    ...
    def test_returns_user_env(self, monkeypatch):
        ...
        from codex_crew.cli_setup import _get_alias
...
class TestManifest:
    ...
    def test_packaged_manifest_declares_complete_oauth_scopes(self):
        ...
        from codex_crew import slack_manifest
    ...
    def _patch_template(
        self, content="name: CodexCrew-{{ALIAS}}\ndisplay_name: CodexCrew-{{ALIAS}}\n"
    ):
        ...
        return patch("codex_crew.slack_manifest._pkg_files", return_value=mock_resource)

    def test_renders_alias_to_stdout(self, capsys):
        with self._patch_template():
            from codex_crew.cli_setup import _manifest

            _manifest(alias="alice")
        out = capsys.readouterr().out
        assert "CodexCrew-alice" in out
        assert "{{ALIAS}}" not in out

    def test_writes_to_output_file(self, tmp_path):
        out_file = tmp_path / "sub" / "out.yaml"
        with self._patch_template("name: CodexCrew-{{ALIAS}}\n"):
            from codex_crew.cli_setup import _manifest

            _manifest(alias="bob", output=str(out_file))
        assert out_file.exists()
        assert "CodexCrew-bob" in out_file.read_text(encoding="utf-8")
    ...
    def test_exits_when_template_missing(self):
        ...
        with patch("codex_crew.slack_manifest._pkg_files", return_value=mock_resource):
            from codex_crew.cli_setup import _manifest
...
class TestLogout:
    ...
    def test_logout_success(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.cli_server.read_local_secret", lambda _port: "test-secret")

        from codex_crew.cli_server import _logout
        ...
        with patch("codex_crew.cli_server.loopback_urlopen", return_value=mock_resp):
            _logout(5486)  # Should not raise

    def test_logout_gateway_not_running(self, tmp_path, monkeypatch):
        """Missing secret file means gateway not running."""
        monkeypatch.setattr("codex_crew.cli_server.read_local_secret", lambda _port: "")
    ...
    def test_logout_http_error(self, tmp_path, monkeypatch):
        ...
        with patch(
            "codex_crew.cli_server.loopback_urlopen",
    ...
    def test_logout_connection_error(self, tmp_path, monkeypatch):
        ...
        secret_file = tmp_path / ".codexcrew" / ".local_secret"
...
class TestStatus:
    ...
    def test_status_auth_required(self, capsys):
        ...
        from codex_crew.cli_server import _status

        with patch(
            "codex_crew.cli_server.loopback_urlopen",
            side_effect=urllib.error.HTTPError(
                "http://127.0.0.1:5486/api/status", 403, "Forbidden", {}, None
            ),
        ):
    ...
    def test_status_other_http_error(self, capsys):
        ...
        with patch(
            "codex_crew.cli_server.loopback_urlopen",
            side_effect=urllib.error.HTTPError(
                "http://127.0.0.1:5486/api/status", 500, "Internal Server Error", {}, None
            ),
        ):
            _status(self._make_args())
        out = capsys.readouterr().out
    ...
    def test_status_unexpected_exception(self, capsys):
        ...
        with patch("codex_crew.cli_server.loopback_urlopen", side_effect=RuntimeError("unexpected")):
            _status(self._make_args())
        out = capsys.readouterr().out
        assert "running" in out
...
class TestIsKirocrewProcess:
    """Tests for _is_codexcrew_process helper.
    ...
    process whose image name is ``python``/``python.exe`` (the venv codexcrew.exe
    re-exec) is still classified by its command line.
    """

    def _cmdline(self, value):
        return patch(
            "codex_crew.cli_server.platform_compat.process_command_line", return_value=value
        )

    def test_returns_true_for_codexcrew(self):
        from codex_crew.cli_server import _is_codexcrew_process

        with self._cmdline("python3 -m codex_crew.dashboard"):
            assert _is_codexcrew_process(1234) is True

    def test_returns_true_for_codexcrew_binary(self):
        from codex_crew.cli_server import _is_codexcrew_process

        with self._cmdline("/usr/bin/codexcrew start"):
            assert _is_codexcrew_process(1234) is True

    def test_returns_true_for_module_gateway_form(self):
        """Regression: the real service launch form
        ``python -m codex_crew gateway`` must be recognized. Previously the
        matcher only accepted the dotted ``codex_crew.gateway`` form, so
        ``codexcrew stop`` no-op'd on service installs.

        Patched through the cross-platform ``process_command_line`` seam (the
        Windows port routes _is_codexcrew_process through platform_compat rather
        than calling ``subprocess.check_output`` directly)."""
        from codex_crew.cli_server import _is_codexcrew_process

        real = (
            "/Users/x/.toolbox/tools/codexcrew/3.1.0/bin/../python3.10/bin/"
            "python3.10 -m codex_crew gateway\n"
        )
        with self._cmdline(real):
            assert _is_codexcrew_process(54842) is True

    def test_returns_true_for_windows_python_reexec(self):
        # The venv codexcrew.exe re-execs python.exe, so the gateway cmdline reads
        # `python.exe ...\Scripts\codexcrew.exe gateway` (or `-m codex_crew gateway`).
        # The token-parser (_args_look_like_codexcrew) recognizes the console-script
        # basename ("codexcrew"/"codexcrew.exe") and the "-m codex_crew gateway" module
        # form; assert both Windows re-exec shapes match.
        from codex_crew.cli_server import _is_codexcrew_process

        with patch("codex_crew.cli_server.platform_compat.IS_WINDOWS", True):
            with self._cmdline(
                r'"C:\Program Files\Python312\python.exe" '
                r'"D:\U\.codexcrew\.venv\Scripts\codexcrew.exe" gateway --no-open'
            ):
                assert _is_codexcrew_process(1234) is True
            with self._cmdline(r"C:\Python312\python.exe -m codex_crew gateway"):
                assert _is_codexcrew_process(1234) is True
    ...
    def test_returns_false_for_unrelated(self):
        ...
        with self._cmdline("nginx: worker process"):
            assert _is_codexcrew_process(1234) is False

    def test_returns_false_for_broad_match(self):
        """Editing a codexcrew file should NOT match — only gateway entry points."""
        from codex_crew.cli_server import _is_codexcrew_process

        with self._cmdline("vim /tmp/codexcrew-notes.txt"):
            assert _is_codexcrew_process(1234) is False

    def test_returns_false_when_cmdline_unavailable(self):
        # process_command_line returns "" on any failure (dead PID, WMI/ps error);
        # _is_codexcrew_process must then fail closed (False), never raise.
        from codex_crew.cli_server import _is_codexcrew_process
...
class TestArgsLookLikeKirocrew:
    """Rigorous tests for the pure command-line classifier ``_args_look_like_codexcrew``.
    ...
    matched — a false positive would let ``codexcrew stop`` SIGTERM an unrelated
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "/Users/x/.toolbox/tools/codexcrew/3.1.0/bin/../python3.10/bin/python3.10 -m codex_crew gateway",
            "python3 -m codex_crew gateway",
            "python -m codex_crew dashboard",
            "python3.10 -m codex_crew start",
            ...
            "Python.app/Contents/MacOS/Python -m codex_crew gateway --no-open",
            "Python -m codex_crew gateway",
            # Subcommand followed by trailing flags.
            "python -m codex_crew gateway --no-open --port 7777",
            # Legacy dotted-submodule form.
            "python3 -m codex_crew.gateway",
            "python3 -m codex_crew.dashboard",
            # Console-script wrapper form.
            "/usr/local/bin/codexcrew gateway",
            "/Users/x/.toolbox/bin/codexcrew start",
            "codexcrew dashboard",
        ],
    )
    def test_matches_server_launch_forms(self, args):
        from codex_crew.cli_server import _args_look_like_codexcrew

        assert _args_look_like_codexcrew(args) is True
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "vim /tmp/codexcrew-notes.txt",  # editing a file named codexcrew*
            "cat /var/log/codex_crew_gateway.log",  # reading a codexcrew log file
            "python -m codex_crew",  # bare module, no server subcommand
            "python -m codex_crew run /tmp/spec.md",  # task runner — NOT a port-bound server
            "python -m codex_crew run gateway",  # "gateway" is a file arg to run, not the subcommand
            "python -m codex_crew run start",  # "start" is a file arg to run, not the subcommand
            "python -m codex_crew_other gateway",  # different package named codex_crew_other
            "/usr/bin/codexcrew",  # wrapper with no subcommand
            "grep -m codex_crew gateway somefile",  # "-m" is grep's flag (no python interpreter)
        ],
    )
    def test_rejects_non_server_processes(self, args):
        from codex_crew.cli_server import _args_look_like_codexcrew

        assert _args_look_like_codexcrew(args) is False
    ...
    def test_unbalanced_quotes_do_not_raise(self):
        ...
        assert _args_look_like_codexcrew('python -m codex_crew gateway "') is True
    ...
    def test_subcommand_match_is_exact_case(self):
        ...
        assert _args_look_like_codexcrew("python -m codex_crew GATEWAY") is False
...
class TestStop:
    ...
    def _mock_sel(self):
        mock = MagicMock()
        return patch("codex_crew.cli_commands.sel", return_value=mock)
    ...
    def _no_service(self):
        ...
        # test host happens to have ``codexcrew.service`` installed.
        #
        ...
        # host.
        with (
            patch("codex_crew.cli_server.service_controller.stop_service", return_value=False),
            patch(
                "codex_crew.cli_server.platform_compat.listening_pid_tool_available",
                return_value=True,
            ),
        ):
            yield
    ...
    def _ports(self, pids):
        return patch("codex_crew.cli_server.platform_compat.find_listening_pids", return_value=pids)
    ...
    def test_no_process_on_port(self, capsys):
        ...
        from codex_crew.cli_server import _stop

        with self._mock_sel(), self._ports([]):
            with pytest.raises(SystemExit) as exc:
                _stop(5486)
            assert exc.value.code == 1
        assert "No Codex Crew gateway" in capsys.readouterr().out
    ...
    def _tool_absent(self, unpinned_at):
        ...
        return (
            ...
            patch("codex_crew.cli_server.platform_compat.listening_pid_tool", return_value="lsof"),
            patch(
                "codex_crew.cli_server.platform_compat.tool_outside_trusted_dirs",
    ...
    def test_a_tool_outside_the_pin_is_not_reported_as_missing(self, capsys):
        ...
        with (
            patch("codex_crew.cli_server.sel", return_value=mock_sel),
            self._ports([]),
            available,
            tool,
            unpinned,
    ...
    def test_no_codexcrew_process(self, capsys):
        # A listener exists but its cmdline isn't a codexcrew gateway → refuse to kill.
        from codex_crew.cli_server import _stop
    ...
    def test_successful_stop(self, capsys):
        ...
        ctx = [
            self._mock_sel(),
            self._ports([1234]),
            self._cmdline("python3 -m codex_crew.dashboard"),
            patch("time.sleep"),
            patch("codex_crew.cli_server.platform_compat.pid_exists", return_value=False),
        ]
        if sys.platform == "win32":
            ctx.append(
                patch("codex_crew.cli_server.platform_compat.kill_process_tree", return_value=True)
            )
        else:
    ...
    def test_explicit_port_bypasses_service_short_circuit(self, capsys):
        ...
        with (
            self._mock_sel(),
            patch(
                "codex_crew.cli_server.service_controller.stop_service",
                return_value=True,
...
class TestWaitForPidsExit:
    ...
    def test_empty_pid_list_returns_immediately(self):
        from codex_crew.cli_server import _wait_for_pids_exit
    ...
    def test_zero_timeout_still_probes_once(self):
        ...
        with patch("codex_crew.cli_server._pid_exited", return_value=False) as mock_exited:
    ...
    def test_returns_only_the_pids_still_alive(self):
        ...
        with patch("codex_crew.cli_server._pid_exited", side_effect=lambda p: p != 9):
...
class TestRestart:
    ...
    def _fast_restart_ready(self, monkeypatch):
        ...
        """
        from codex_crew import cli_server

        monkeypatch.setattr("codex_crew.cli_server._RESTART_READY_TIMEOUT", 0)
        monkeypatch.setattr(
            "codex_crew.cli_server._wait_gateway_ready",
    ...
    def _no_active_service(self):
        ...
        # depend on whether the BUILD HOST runs a codexcrew service. Pin it
        # False; the denied-branch tests override it per-test.
        with patch(
            "codex_crew.cli_server.service_controller.is_service_active",
            return_value=False,
        ):
            yield
    ...
    def _mock_sel(self):
        return patch("codex_crew.cli_server.sel", return_value=MagicMock())

    def test_service_active_restarts_via_controller(self, capsys):
        from codex_crew.cli_server import _restart

        with (
            self._mock_sel(),
            patch(
                "codex_crew.cli_server.service_controller.restart_service",
                return_value=True,
            ) as mock_restart,
            patch("codex_crew.cli_server._spawn_detached_gateway") as mock_spawn,
            patch("codex_crew.cli_server.platform_compat.find_listening_pids") as mock_ports,
        ):
            _restart(None)
    ...
    def test_active_service_restart_denied_fails_loud_with_remedy(self, capsys):
        ...
        with (
            ...
            patch(
                "codex_crew.cli_server.service_controller.manual_restart_hint",
                return_value="sudo systemctl restart codexcrew",
            ),
            patch("codex_crew.cli_server.platform_compat.find_listening_pids") as mock_ports,
            patch("codex_crew.cli_server._stop") as mock_stop,
            patch("codex_crew.cli_server._spawn_detached_gateway") as mock_spawn,
        ):
            with pytest.raises(SystemExit) as exc:
        ...
        assert "sudo systemctl restart codexcrew" in out
    ...
    def test_inactive_service_still_falls_through_after_refused_restart(self):
        ...
        with (
            ...
            patch(
                "codex_crew.cli_server.platform_compat.find_listening_pids",
                return_value=[],
            ),
            patch(
                "codex_crew.cli_server._spawn_detached_gateway",
    ...
    def test_no_service_no_running_gateway_spawns_fresh(self, capsys):
        ...
        # ``codexcrew restart`` after the gateway died, they should still
    ...
    def test_toctou_stop_systemexit_is_swallowed_so_spawn_proceeds(self, capsys):
        ...
        with (
            ...
            patch("codex_crew.cli_server._stop", side_effect=SystemExit(1)) as mock_stop,
            patch(
                "codex_crew.cli_server._spawn_detached_gateway",
    ...
    def test_waits_for_incumbent_to_exit_before_spawning(self):
        ...
        The incumbent holds the ``CODEXCREW_HOME`` flock for its whole graceful
        ...
        with (
            ...
            patch("codex_crew.cli_server._is_codexcrew_process", return_value=True),
            patch("codex_crew.cli_server._stop"),
            patch("codex_crew.cli_server._pid_exited", side_effect=fake_pid_exited),
            patch("codex_crew.cli_server._print_token_url"),
            patch(
                "codex_crew.cli_server._spawn_detached_gateway",
                return_value=self._fake_proc(5678),
            ) as mock_spawn,
    ...
    def test_refuses_to_spawn_when_incumbent_never_exits(self, capsys):
        ...
        with (
            ...
            patch("codex_crew.cli_server._is_codexcrew_process", return_value=True),
            patch("codex_crew.cli_server._stop"),
            patch("codex_crew.cli_server._pid_exited", return_value=False),
            patch("codex_crew.cli_server._RESTART_STOP_TIMEOUT", 0),
            patch("codex_crew.cli_server._spawn_detached_gateway") as mock_spawn,
        ):
            with pytest.raises(SystemExit) as exc:
                cli_server._restart(None)
    ...
    def test_unrelated_port_listener_is_not_waited_on(self):
        """A non-CodexCrew listener must never gate the restart.
        ...
        refuse, so the wait set is filtered by ``_is_codexcrew_process``.
        """
        from codex_crew import cli_server
        ...
        with (
            ...
            patch("codex_crew.cli_server._is_codexcrew_process", return_value=False),
            patch("codex_crew.cli_server._stop") as mock_stop,
            patch("codex_crew.cli_server._pid_exited", return_value=False) as mock_exited,
            patch("codex_crew.cli_server._print_token_url"),
            patch(
                "codex_crew.cli_server._spawn_detached_gateway",
                return_value=self._fake_proc(5678),
        ...
        # _stop still runs (it owns the "not a CodexCrew gateway" diagnostic)...
        mock_stop.assert_called_once_with(None)
    ...
    def test_spawn_detached_gateway_binds_requested_port(self, tmp_path, monkeypatch):
        ...
        just did), an unparameterised spawn lets the replacement bind 5486 while
        the parent waits on 6776 and prints a 6776 URL.
        """
        from codex_crew.cli_server import _spawn_detached_gateway

        monkeypatch.setattr("codex_crew.cli_server.config_dir", lambda: tmp_path)
        proc = MagicMock(pid=4321)
        with (
            patch("shutil.which", return_value="/usr/local/bin/codexcrew"),
            patch("codex_crew.cli_server.subprocess.Popen", return_value=proc) as mock_popen,
        ):
            _spawn_detached_gateway(6776)
        assert mock_popen.call_args.args[0] == [
            "/usr/local/bin/codexcrew",
            "gateway",
            "--port",
            "6776",
        ]
    ...
    def test_restart_passes_resolved_port_to_spawn(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.cli_server.config_dir", lambda: tmp_path)
        with (
            patch("codex_crew.cli_server.resolve_client_port", return_value=6776),
            patch("codex_crew.cli_server.service_controller.restart_service", return_value=False),
            patch("codex_crew.cli_server.platform_compat.find_listening_pids", return_value=[]),
            patch(
                "codex_crew.cli_server.platform_compat.listening_pid_tool_available",
                return_value=True,
            ),
    ...
    def test_spawn_detached_gateway_uses_codexcrew_bin(self, tmp_path, monkeypatch):
        # When ``codexcrew`` is on PATH, the detached child must invoke it
        ...
        assert argv == ["/usr/local/bin/codexcrew", "gateway"]
    ...
    def test_spawn_detached_gateway_prefers_own_console_script(self, tmp_path, monkeypatch):
        # Two `codexcrew` scripts can exist at once — an edition composes this
    ...
    def test_spawn_detached_gateway_ignores_non_codexcrew_argv0(self, tmp_path, monkeypatch):
        # argv[0] is only trusted when it names the codexcrew console script.
        # Anything else (a test runner, a wrapper, `python -m codex_crew restart`)
        ...
        with (
            ...
            _spawn_detached_gateway()
        assert mock_popen.call_args.args[0] == ["/usr/local/bin/codexcrew", "gateway"]
    ...
    def test_spawn_detached_gateway_ignores_missing_argv0_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.cli_server.config_dir", lambda: tmp_path)
        monkeypatch.setattr(sys, "argv", [str(tmp_path / "ghost" / "codexcrew"), "restart"])
    ...
    def test_spawn_detached_gateway_absolutizes_relative_argv0(self, tmp_path, monkeypatch):
        # Regression: `cd ~/checkout && .venv/bin/codexcrew restart`. shutil.which()
        ...
        monkeypatch.setattr("codex_crew.cli_server.config_dir", lambda: tmp_path)
        # Use the real per-platform venv console-script layout: `bin/codexcrew` on
        # POSIX, `Scripts\codexcrew.exe` on Windows. shutil.which() only accepts a
        ...
        if sys.platform == "win32":
            ...
            script_name = "codexcrew.exe"
        else:
            venv_bin = tmp_path / ".venv" / "bin"
            script_name = "codexcrew"
        ...
        with patch("codex_crew.cli_server.subprocess.Popen", return_value=proc) as mock_popen:
            _spawn_detached_gateway()
    ...
    def test_spawn_detached_gateway_falls_back_to_python_m(self, tmp_path, monkeypatch):
        # Dev/Brazil-workspace installs may not have ``codexcrew`` on
        # PATH globally. Fall back to ``python -m codex_crew`` so the
        ...
        assert argv[1:] == ["-m", "codex_crew", "gateway"]
...
class TestRestartReadinessVerdict:
    ...
    the ``CODEXCREW_HOME`` ownership guard refused (exit 1, milliseconds later)
    ...
    def _drive(self, *, poll, ready_status, marker_pid, timeout=None):
        ...
        stack = [
            ...
            patch("codex_crew.cli_server.run_marker.read_pid", side_effect=marker_pid),
            patch("codex_crew.cli_server._gateway_owns_port", return_value=True),
            patch("codex_crew.cli_server._probe_gateway_ready", return_value=ready_status),
            patch(
                "codex_crew.cli_server._spawn_detached_gateway",
                return_value=MagicMock(pid=4321, poll=MagicMock(return_value=poll)),
            ),
            patch("codex_crew.cli_server._print_token_url"),
            patch("codex_crew.cli_server._RESTART_READY_TIMEOUT", 0 if timeout is None else timeout),
        ]
    ...
    def test_replacement_that_dies_is_reported_as_a_failure(self, capsys):
        ...
        assert "4321" in out
        assert "codexcrew logs -f" in out
...
class TestResolveClientPort:
    ...
    def test_cli_flag_wins(self, monkeypatch, tmp_path):
        ...
        from codex_crew.cli_server import resolve_client_port

        monkeypatch.setenv("CODEXCREW_PORT", "9999")
        mock_cfg = MagicMock()
        mock_cfg.dashboard.url = "http://localhost:8888"
        with patch("codex_crew.cli_server.CodexCrewConfig.load", return_value=mock_cfg):
            assert resolve_client_port(12345) == 12345

    def test_env_var_used_when_no_cli(self, monkeypatch):
        """CODEXCREW_PORT env var wins over config when no --port passed."""
        from codex_crew.cli_server import resolve_client_port

        monkeypatch.setenv("CODEXCREW_PORT", "6777")
        mock_cfg = MagicMock()
        mock_cfg.dashboard.url = "http://localhost:8888"
        with patch("codex_crew.cli_server.CodexCrewConfig.load", return_value=mock_cfg):
    ...
    def test_invalid_env_var_falls_through_to_config(self, monkeypatch):
        """A garbage CODEXCREW_PORT must not crash; the helper falls through."""
        from codex_crew.cli_server import resolve_client_port

        monkeypatch.setenv("CODEXCREW_PORT", "not-a-number")
        mock_cfg = MagicMock()
    ...
    def test_config_url_used_when_no_cli_no_env(self, monkeypatch):
        ...
        monkeypatch.delenv("CODEXCREW_PORT", raising=False)
        mock_cfg = MagicMock()
        mock_cfg.dashboard.url = "http://localhost:7778"
        with patch("codex_crew.cli_server.CodexCrewConfig.load", return_value=mock_cfg):
            assert resolve_client_port(None) == 7778
    ...
    def test_config_url_hostname_only_falls_through_to_default(self, monkeypatch):
        """A dashboard.url without an explicit port must fall through to 5486."""
        from codex_crew.cli_server import resolve_client_port
        ...
        with (
            patch("codex_crew.cli_server.CodexCrewConfig.load", return_value=mock_cfg),
            # No gateway advertising itself — isolate from the dev box's markers.
            patch("codex_crew.cli_server.run_marker.marker_ports", return_value=[]),
        ):
            ...
            assert resolve_client_port(None) == 5486

    def test_empty_config_falls_through_to_default(self, monkeypatch):
        """No env, empty dashboard.url → 5486."""
        from codex_crew.cli_server import resolve_client_port
    ...
    def test_config_load_failure_falls_through_to_default(self, monkeypatch):
        ...
        monkeypatch.delenv("CODEXCREW_PORT", raising=False)
        with patch("codex_crew.cli_server.CodexCrewConfig.load", side_effect=RuntimeError("boom")):
            assert resolve_client_port(None) == 5486
...
class TestResolveClientPortRunMarker:
    ...
    read that marker instead of assuming 5486 — but only when a verified
    CodexCrew gateway process holds the port, and only when exactly one does.
    """
    ...
    def _cfg(self, url):
        ...
        return patch("codex_crew.cli_server.CodexCrewConfig.load", return_value=mock_cfg)

    def _markers(self, ports):
        return patch("codex_crew.cli_server.run_marker.marker_ports", return_value=ports)

    def _owned(self, ports):
        """Pretend a verified CodexCrew gateway listens on each of *ports*."""
        return patch(
            "codex_crew.cli_server._gateway_owns_port", side_effect=lambda p: p in set(ports)
        )

    def test_sole_owned_marker_used_when_nothing_configured(self):
        """The bug: one gateway on 6776, no config → must not return 5486."""
        from codex_crew.cli_server import resolve_client_port
    ...
    def test_portless_config_url_falls_through_to_marker(self):
        ...
        parse_dashboard_url() substitutes 5486 for a portless URL, which would
    ...
    def test_discovery_failure_falls_through_to_default(self):
        ...
        with (
            self._cfg(""),
            patch(
                "codex_crew.cli_server.run_marker.marker_ports",
...
class TestGatewayOwnsPort:
    ...
    `/tmp/codexcrew gateway`.
    """

    def _sidecar(self, pid):
        return patch("codex_crew.cli_server.run_marker.read_pid", return_value=pid)

    def _listeners(self, pids):
        return patch("codex_crew.cli_server.platform_compat.find_listening_pids", return_value=pids)

    def _owner(self, uid):
        return patch("codex_crew.cli_server.platform_compat.process_owner_uid", return_value=uid)

    def _me(self):
        return os.getuid() if hasattr(os, "getuid") else 0

    def _posix(self, value=True):
        return patch("codex_crew.cli_server.platform_compat.IS_POSIX", value)

    def test_true_when_recorded_pid_holds_the_port_and_is_ours(self):
        from codex_crew.cli_server import _gateway_owns_port
        ...
        with (
            ...
            patch("codex_crew.cli_server._is_codexcrew_process", return_value=True),
            patch("codex_crew.cli_server.os.getuid", return_value=self._me(), create=True),
        ):
    ...
    def test_denies_on_non_posix(self):
        ...
        / CODEXCREW_PORT, which is where it was before this fallback existed.
        """
        from codex_crew.cli_server import _gateway_owns_port
    ...
    def test_false_when_pid_is_not_a_codexcrew_process(self):
...
class TestEnsurePrerequisites:
    ...
    def test_returns_true_when_all_satisfied(self):
        from codex_crew.cli_setup import _ensure_prerequisites

        with (
            patch("codex_crew.cli_setup.shutil.which", return_value="/usr/bin/kiro-cli"),
            patch("codex_crew.cli_doctor.subprocess.run", return_value=MagicMock(returncode=0)),
        ):
    ...
    def test_returns_true_when_optional_kiro_absent(self):
        ...
        with patch("codex_crew.cli_setup.shutil.which", return_value=None):
            assert _ensure_prerequisites() is True
...
class TestDoctorStaleProjectDir:
    ...
    def test_doctor_detects_stale_project_dir(self, tmp_path, capsys):
        ...
        agent_file = tmp_path / "codexcrew.json"
        agent_data = {
            "tools": ["@codexcrew-core", "@codexcrew-cron"],
            "allowedTools": ["@codexcrew-core", "@codexcrew-cron"],
            "mcpServers": {
                "codexcrew-core": {"command": "/usr/local/bin/codexcrew", "args": ["mcp-core"]},
                "codexcrew-cron": {"command": "/usr/local/bin/codexcrew", "args": ["mcp-cron"]},
            },
        }
        ...
        with (
            ...
            patch.dict(
                "os.environ",
                {"CODEXCREW_PROJECT_DIR": "", "SLACK_APP_TOKEN": "", "SLACK_BOT_TOKEN": ""},
                clear=False,
            ),
        ):
            with pytest.raises(SystemExit):
...
class TestDoctorMcpTools:
    ...
    The helper live-probes only the managed servers (`codexcrew-core`,
    `codexcrew-cron`) via `probe_server`; tests monkey-patch that call so
    ...
    def _mock_probe(self, results: dict[str, tuple[str, list[str], str]]):
        ...
        from codex_crew.mcp_discovery import McpServerInfo
        ...
        return patch("codex_crew.cli_doctor.probe_server", side_effect=fake)

    def test_success_shows_tool_counts(self, tmp_path, capsys):
        from codex_crew.cli_doctor import _doctor_mcp_tools

        agent_path = tmp_path / "codexcrew.json"
        ...
        with self._mock_probe(
            {
                "codexcrew-core": ("ok", ["spawn_run", "learn_add", "task_run"], ""),
                "codexcrew-cron": ("ok", ["cron_add"], ""),
            }
        ):
            _doctor_mcp_tools(agent_path, issues)
        out = capsys.readouterr().out
        assert "@codexcrew-core: ✅ 3 tools" in out
        assert "@codexcrew-cron: ✅ 1 tool" in out
        assert issues == []
    ...
    def test_failure_shows_error_head_and_indented_stderr(self, tmp_path, capsys):
        ...
        _write_agent_config(
            agent_path,
            tools=["@codexcrew-core", "@codexcrew-cron"],
            allowed=["@codexcrew-core", "@codexcrew-cron"],
            servers={
                "codexcrew-core": {"command": "/bin/codexcrew", "args": ["mcp-core"]},
                "codexcrew-cron": {"command": "/bin/codexcrew", "args": ["mcp-cron"]},
            },
        )
        issues: list[str] = []
        fail_err = (
            "no response\n"
            "stderr: Directory isn't within a workspace: '/home/u/.codexcrew-app' "
            "(Amazon::Brazil::Cli::FindupException)"
        )
        with self._mock_probe(
            {
                "codexcrew-core": ("error", [], fail_err),
                "codexcrew-cron": ("ok", [], ""),
            }
        ):
            _doctor_mcp_tools(agent_path, issues)
        ...
        assert "@codexcrew-core: ❌ no response" in out
        assert "      stderr: Directory isn't within a workspace" in out
        assert "FindupException" in out
        assert "@codexcrew-cron: ✅ 0 tools" in out
        assert "@codexcrew-core probe" in issues
        # Healthy server must not pollute the issue list.
        assert "@codexcrew-cron probe" not in issues
    ...
    def test_missing_mcp_server_cannot_auto_fix(self, tmp_path, capsys):
        ...
        with self._mock_probe({}) as probe_mock:
            _doctor_mcp_tools(agent_path, issues)
        out = capsys.readouterr().out
        assert "@codexcrew-core: ❌ missing from mcpServers" in out
        assert "@codexcrew-cron: ❌ missing from mcpServers" in out
        assert "re-run `codexcrew setup`" in out
        assert "@codexcrew-core config" in issues
        assert "@codexcrew-cron config" in issues
    ...
    def test_auto_fix_adds_missing_tools_and_allowed(self, tmp_path, capsys):
        ...
        assert updated["tools"] == ["@codexcrew-cron", "@codexcrew-core"]
        assert updated["allowedTools"] == ["@codexcrew-cron", "@codexcrew-core"]

    def test_auto_fix_never_blanket_allows_computer_use(self, tmp_path, capsys):
        """**Doctor must never add ``@codexcrew-computer`` to ``allowedTools``.**
        ...
        gated_open = dict(agent._MANAGED_MCP_SERVERS["codexcrew-computer"])
        gated_open["spec_gate"] = lambda: True
        issues: list[str] = []
        with (
            patch.dict(agent._MANAGED_MCP_SERVERS, {"codexcrew-computer": gated_open}),
            self._mock_probe({}),
        ):
            _doctor_mcp_tools(agent_path, issues)

        updated = json.loads(agent_path.read_text(encoding="utf-8"))
        assert "@codexcrew-computer" in updated["tools"]
        assert "@codexcrew-computer" not in updated["allowedTools"]
        assert not [t for t in updated["allowedTools"] if t.startswith("@codexcrew-computer/")]
        # The other managed servers are unaffected — the carve-out is scoped.
        assert "@codexcrew-core" in updated["allowedTools"]
        assert "@codexcrew-cron" in updated["allowedTools"]
    ...
    def test_auto_fix_preserves_a_user_made_computer_use_grant(self, tmp_path, capsys):
        ...
        with (
            ...
            _doctor_mcp_tools(agent_path, issues)
        updated = json.loads(agent_path.read_text(encoding="utf-8"))
        assert "@codexcrew-computer" in updated["allowedTools"]
    ...
    def test_probe_exception_does_not_crash(self, tmp_path, capsys):
        ...
        issues: list[str] = []
        with patch(
            "codex_crew.cli_doctor.probe_server",
    ...
    def test_only_managed_servers_are_probed(self, tmp_path, capsys):
        ...
        keeps doctor output focused on CodexCrew's own servers and avoids
        ...
        _write_agent_config(
            agent_path,
            tools=["@codexcrew-core", "@codexcrew-cron", "@builder-mcp"],
            allowed=["@codexcrew-core", "@codexcrew-cron", "@builder-mcp"],
            servers={
                "codexcrew-core": {"command": "/bin/codexcrew", "args": ["mcp-core"]},
                "codexcrew-cron": {"command": "/bin/codexcrew", "args": ["mcp-cron"]},
        ...
        with patch("codex_crew.cli_doctor.probe_server", side_effect=recording_probe):
            _doctor_mcp_tools(agent_path, issues)
        assert probed_names == ["codexcrew-cron", "codexcrew-core"]
        out = capsys.readouterr().out
        assert "@builder-mcp" not in out

    def test_malformed_agent_config_does_not_crash(self, tmp_path, capsys):
        """If codexcrew.json is truncated or otherwise unparseable, doctor
        ...
        agent_path.write_text('{"tools": ["@codexcrew-c')
...
class TestDoctorMcpSpecGate:
    ...
    ``@codexcrew-computer: missing from mcpServers`` and `codexcrew setup` could
    ...
    def _pin_gate(self, gate):
        """Return a patch pinning ``codexcrew-computer``'s spec gate to *gate*."""
        from codex_crew import agent

        pinned = dict(agent._MANAGED_MCP_SERVERS["codexcrew-computer"])
        pinned["spec_gate"] = gate
        return patch.dict(agent._MANAGED_MCP_SERVERS, {"codexcrew-computer": pinned})
    ...
    def _config_without_computer(self, path: Path) -> None:
        ...
        from codex_crew.mcp_cleanup import ALWAYS_ON_BIN_MCP_SERVERS

        present = [n for n in ALWAYS_ON_BIN_MCP_SERVERS if n != "codexcrew-computer"]
        _write_agent_config(
            path,
    ...
    def test_gate_closed_absent_entry_is_informational_not_an_issue(self, tmp_path, capsys):
        ...
        with self._pin_gate(lambda: False), self._mock_probe(probed):
            _doctor_mcp_tools(agent_path, issues)
        out = capsys.readouterr().out
        assert "@codexcrew-computer: ℹ️  gated off on this host" in out
        assert "feature disabled or no driver" in out
        assert "@codexcrew-computer: ❌" not in out
        assert issues == []
        ...
        assert "codexcrew-computer" not in probed
    ...
    def test_gate_open_absent_entry_keeps_the_hard_error(self, tmp_path, capsys):
        ...
        with self._pin_gate(lambda: True), self._mock_probe([]):
            _doctor_mcp_tools(agent_path, issues)
        out = capsys.readouterr().out
        assert "@codexcrew-computer: ❌ missing from mcpServers (re-run `codexcrew setup`)" in out
        assert "@codexcrew-computer config" in issues
    ...
    def test_gate_raising_is_treated_as_open(self, tmp_path, capsys):
        ...
        with self._pin_gate(explode), self._mock_probe([]):
            _doctor_mcp_tools(agent_path, issues)
        out = capsys.readouterr().out
        assert "@codexcrew-computer: ❌ missing from mcpServers" in out
        assert "@codexcrew-computer config" in issues
    ...
    def test_shipped_gate_swallowing_internal_errors_reads_as_closed(self, tmp_path, capsys):
        ...
        with (
            patch(
                "codex_crew.computer_use.enable_state.is_enabled",
    ...
    def test_gate_closed_stale_entry_is_not_mounted_or_probed(self, tmp_path, capsys):
        ...
        _write_agent_config(
            agent_path,
            tools=[f"@{n}" for n in present if n != "codexcrew-computer"],
            allowed=[],
            servers={
                n: {"command": sys.executable, "args": [f"mcp-{n.split('-', 1)[1]}"]}
                for n in present
        ...
        assert "codexcrew-core" in probed and "codexcrew-cron" in probed
        updated = json.loads(agent_path.read_text(encoding="utf-8"))
        assert "@codexcrew-computer" not in updated["tools"]
        assert "@codexcrew-computer" not in updated["allowedTools"]
    ...
    def test_governance_denominator_skips_an_absent_gated_off_server(
        ...
        "re-run `codexcrew setup --agent-only`" loop in this section.
        """
        from codex_crew import cli_doctor
        from codex_crew.mcp_cleanup import ALWAYS_ON_BIN_MCP_SERVERS
        ...
        monkeypatch.setattr(cli_doctor.CodexCrewConfig, "load", staticmethod(lambda: _Cfg()))
        spec_path = tmp_path / "codexcrew.json"
...
class TestDoctorStt:
    ...
    and the exit status their ``codexcrew doctor && codexcrew gateway`` chain
    depends on.
    """
    ...
    def _model_on_disk(monkeypatch, present):
        ...
        Patched on the ``codex_crew.stt`` namespace the doctor actually reads. The
    ...
    def _report(self, tmp_path, capsys, *, ffmpeg=True, modules=None):
        ...
        """
        _healthy_agent_file(tmp_path / "codexcrew.json")
        mock_run = MagicMock(returncode=0, stdout="kiro-cli 1.0.0", stderr="")
        ...
        packaged_decoder = "/codexcrew-bundle/Resources/ffmpeg" if ffmpeg else None

        code = 0
        with (
            patch("codex_crew.cli_doctor.shutil.which", side_effect=_which),
            patch(
                "codex_crew.transcribe._packaged_ffmpeg_resource",
            ...
            patch("codex_crew.cli_doctor.config_dir", return_value=tmp_path),
            patch("codex_crew.cli_doctor.probe_server", side_effect=_noop_probe_server),
            patch("codex_crew.slack.enterprise.validate_enterprise", return_value=True),
    ...
    def test_doctor_stt_ffmpeg_is_a_prerequisite_of_every_provider(
        ...
        assert "reinstall Codex Crew" not in out
        assert "❌ Fix these issues: " in out
    ...
    def test_doctor_bundled_desktop_never_requests_a_system_ffmpeg_install(
        ...
        section = self._stt_section(out)
        assert "reinstall Codex Crew (the bundled audio decoder is missing)" in section
    ...
    def test_doctor_stt_transcribe_amazon_transcribe_missing(self, tmp_path, capsys, monkeypatch):
        ...
        # The cloud dependencies by name: `pip install codexcrew[voice]` resolves
    ...
    def test_doctor_stt_apple_unsupported_host_names_its_code(self, tmp_path, capsys, monkeypatch):
        ...
        import codex_crew.cli_doctor as _doc
        from codex_crew.transcribe import CODE_APPLE_UNSUPPORTED
...
class TestConfigDirOverride:
    """Tests that CLI functions respect CODEXCREW_HOME env var via config_dir()."""
    ...
    def test_project_dir_file_uses_config_dir(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.cli.config_dir", lambda: tmp_path)

        from codex_crew.cli import _project_dir_file
    ...
    def test_detect_project_dir_reads_from_config_dir(self, tmp_path, monkeypatch):
        ...
        proj.mkdir()
        (proj / "skills").mkdir()
        (proj / "src" / "codex_crew").mkdir(parents=True)

        config_home = tmp_path / "custom_config"
        config_home.mkdir()
        (config_home / "project_dir").write_text(str(proj) + "\n")

        monkeypatch.setattr("codex_crew.cli.config_dir", lambda: config_home)
        monkeypatch.chdir(tmp_path)  # CWD has no project markers

        from codex_crew.cli import _detect_project_dir
    ...
    def test_detect_project_dir_no_agents_dir(self, tmp_path, monkeypatch):
        ...
        Regression guard: agent config was consolidated into src/codex_crew/config/
        ...
        """
        proj = tmp_path / "CodexCrew"
        (proj / "skills").mkdir(parents=True)
        (proj / "src" / "codex_crew").mkdir(parents=True)
        assert not (proj / "agents").exists()

        monkeypatch.setattr("codex_crew.cli.config_dir", lambda: tmp_path / "cfg")
        (tmp_path / "cfg").mkdir()
    ...
    def test_logout_reads_secret_for_listener_port(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.cli_server.read_local_secret", read_secret)
        ...
        with patch("codex_crew.cli_server.loopback_urlopen", return_value=mock_resp):
            _logout(5486)
        read_secret.assert_called_once_with(5486)

    def test_setup_slack_tokens_writes_to_config_dir(self, tmp_path, monkeypatch):
        """_setup_slack_tokens writes .env to config_dir(), not ~/.codexcrew."""
        monkeypatch.setattr("codex_crew.cli_setup.env_path", lambda: tmp_path / ".env")

        from codex_crew.cli_setup import _setup_slack_tokens
    ...
    def test_setup_slack_tokens_locks_env_to_owner(self, tmp_path, monkeypatch):
        ...
        import codex_crew.cli_setup as cs
        from codex_crew import platform_compat as pc

        env_file = tmp_path / ".env"
        monkeypatch.setattr("codex_crew.cli_setup.env_path", lambda: env_file)
        inputs = iter(["y", "xapp-test", "xoxb-test", "U12345"])
        monkeypatch.setattr("builtins.input", lambda _: next(inputs))
    ...
    def test_setup_slack_tokens_aborts_when_shared_env_lock_is_held(
        ...
        import codex_crew.cli_setup as cs
        from codex_crew import platform_compat as pc
        from codex_crew.secrets.migrate import _env_lock_path
        ...
        # Simulate another writer (e.g. `codexcrew secrets import`) holding the
...
class TestSetupChannelGating:
    """`codexcrew setup` runs the Slack steps only with --slack.
    ...
    def _run_setup(self, monkeypatch, tmp_path, **kwargs):
        ...
        calls: list[str] = []
        monkeypatch.delenv("CODEXCREW_PROJECT_DIR", raising=False)
        # Imported inside _setup_impl — patch at their source modules.
        monkeypatch.setattr(
            "codex_crew.agent.install_agent", lambda clean=False: tmp_path / "agent.json"
        )
        ...
        monkeypatch.setattr("codex_crew.agent.ensure_codexcrew_on_path", lambda *a, **k: None)
        monkeypatch.setattr("codex_crew.mcp_cleanup.clean_stale_managed_mcp", lambda: [])
        ...
        monkeypatch.setattr(
            cs, "CodexCrewConfig", MagicMock(load=MagicMock(side_effect=RuntimeError("no config")))
        )
    ...
    def test_the_whatsapp_flag_reaches_setup_from_the_command_line(self):
        ...
        ``codexcrew setup --whatsapp`` exits 2 instead of running anything."""
        import sys

        argv = ["codexcrew", "setup", "--whatsapp"]
        with patch.object(sys, "argv", argv), patch("codex_crew.cli._setup") as mock_setup:
            from codex_crew.cli import main
...
class TestSpawnCliAuth:
    """``codexcrew spawn`` attaches X-Internal-Secret on every gateway call.
    ...
    def test_internal_secret_reads_local_secret_file(self, tmp_path, monkeypatch):
        monkeypatch.setattr("codex_crew.cli_commands.read_local_secret", lambda _port: "abc123")

        from codex_crew.cli_commands import _internal_secret

        assert _internal_secret(5486) == "abc123"

    def test_internal_secret_returns_empty_when_missing(self, tmp_path, monkeypatch):
        monkeypatch.setattr("codex_crew.cli_commands.read_local_secret", lambda _port: "")

        from codex_crew.cli_commands import _internal_secret

        assert _internal_secret(5486) == ""

    def test_spawn_list_sends_internal_secret_header(self, tmp_path, monkeypatch, capsys):
        monkeypatch.setattr(
            "codex_crew.cli_commands.read_local_secret", lambda _port: "test-secret-xyz"
        )
        ...
        monkeypatch.setattr("codex_crew.cli_commands.loopback_urlopen", fake_urlopen)

        from codex_crew.cli_commands import _spawn

        args = argparse.Namespace(spawn_action="list", port=5486)
        _spawn(args)

        assert len(captured) == 1
        req = captured[0]
        assert req.full_url == "http://localhost:5486/api/spawn"
    ...
    def test_spawn_run_sends_internal_secret_header(self, tmp_path, monkeypatch, capsys):
        monkeypatch.setattr(
            "codex_crew.cli_commands.read_local_secret", lambda _port: "run-secret-abc"
        )
        ...
        from codex_crew.cli_commands import _spawn_run

        args = argparse.Namespace(task="do thing", fire_and_forget=True, port=5486)
        _spawn_run(args, "http://localhost:5486")
...
class TestArtifactCli:
    ...
    def test_save_parses_an_explicit_slug(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            "codex_crew.cli_commands._artifact",
    ...
    def test_save_refuses_sensitive_content_file(
        ...
        from codex_crew.cli_commands import _artifact

        monkeypatch.setattr("codex_crew.cli_commands.is_sensitive_path", lambda _p: True)
        # Surface any HTTP call as a fatal so we can prove the function exited
        # at the security check, not at the network layer.
        monkeypatch.setattr(
            "codex_crew.cli_commands.loopback_urlopen",
...
class TestMcpBuiltinDispatch:
    ...
    def test_mcp_builtin_dispatches_to_module(self, monkeypatch):
        ...
        """
        import codex_crew.cli as cli_mod
        ...
        monkeypatch.setattr(sys, "argv", ["codexcrew", f"mcp-{builtin_name}"])
        with patch("importlib.import_module", return_value=mock_module) as mock_import:
            cli_mod.main()

        mock_import.assert_called_once_with(f"codex_crew.apps.builtins.{builtin_name}.mcp_server")
...
class TestSeedDispatch:
    ...
    def test_seed_calls_seed_cmd(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["codexcrew", "gateway", "--seed", "demo"])
        mock_seed = MagicMock(return_value=0)
        gateway_call = object()
        mock_gateway = MagicMock(return_value=gateway_call)
        with (
            patch("codex_crew.cli.seed_cmd", mock_seed),
            patch("codex_crew.cli_server._gateway", mock_gateway),
            patch("codex_crew.cli.asyncio.run") as mock_run,
        ):
            from codex_crew.cli import main
    ...
    def test_seed_nonzero_exits(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["codexcrew", "gateway", "--seed", "bad"])
        mock_seed = MagicMock(return_value=1)
        with patch("codex_crew.cli.seed_cmd", mock_seed):
            from codex_crew.cli import main
    ...
    def test_no_seed_skips_seed_cmd(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["codexcrew", "gateway"])
        mock_seed = MagicMock()
        gateway_call = object()
        mock_gateway = MagicMock(return_value=gateway_call)
        with (
    ...
    def test_seed_with_replace_flag(self, monkeypatch):
        ...
        monkeypatch.setattr(
            sys,
            "argv",
            ["codexcrew", "gateway", "--seed", "demo", "--seed-replace"],
        )
        mock_seed = MagicMock(return_value=0)
        gateway_call = object()
        mock_gateway = MagicMock(return_value=gateway_call)
...
class TestDoctorEmbeddings:
    ...
    def _run_doctor(
        ...
        *,
        ...
        agent_file = tmp_path / "codexcrew.json"
        _healthy_agent_file(agent_file)
        import codex_crew.cli_doctor as doc
        ...
        with (
            patch("codex_crew.cli_doctor.KIRO_AGENTS_DIR", tmp_path),
            patch("codex_crew.cli_doctor.subprocess.run", return_value=default_run),
    ...
    def test_doctor_blames_the_override_dir_not_the_bundled_tree(
        ...
        The libs load from the operator's directory, so "reinstall Codex Crew"
...
class TestWaitGatewayReady:
    ...
    def test_dead_child_short_circuits_with_its_exit_status(self):
        ...
        with (
            patch("codex_crew.cli_server._probe_gateway_ready", probe),
            patch("codex_crew.cli_server.time.sleep") as mock_sleep,
        ):
    ...
    def test_zero_timeout_still_probes_once(self):
        ...
        with (
            patch("codex_crew.cli_server._probe_gateway_ready", probe),
            patch("codex_crew.cli_server._replacement_is_serving", return_value=True),
        ):
    ...
    def test_not_ready_within_deadline_is_a_timeout(self):
        ...
        with patch("codex_crew.cli_server._probe_gateway_ready", return_value=503):
    ...
    def test_polls_until_the_replacement_answers(self):
        ...
        with (
            patch("codex_crew.cli_server._probe_gateway_ready", side_effect=[0, 503, 200]),
            patch("codex_crew.cli_server._replacement_is_serving", return_value=True),
            patch("codex_crew.cli_server.time.sleep") as mock_sleep,
        ):
            verdict = cli_server._wait_gateway_ready(
    ...
    def test_child_that_exits_during_the_last_probe_is_reported_as_died(self):
        ...
        with (
            patch("codex_crew.cli_server._probe_gateway_ready", return_value=503),
    ...
    def test_missing_marker_is_never_the_replacement(self):
        ...
        with (
            patch("codex_crew.cli_server.run_marker.read_pid", return_value=None),
            patch("codex_crew.cli_server.platform_compat.IS_POSIX", False),
        ):
    ...
    def test_unchanged_marker_pid_is_the_old_gateway(self):
        ...
        with patch("codex_crew.cli_server.run_marker.read_pid", return_value=1234):
            assert cli_server._replacement_is_serving(7777, 1234) is False
    ...
    def test_changed_marker_pid_that_owns_the_port_is_the_replacement(self):
        ...
        with (
            patch("codex_crew.cli_server.run_marker.read_pid", return_value=4321),
            patch("codex_crew.cli_server.platform_compat.IS_POSIX", True),
            patch(
                "codex_crew.cli_server.platform_compat.listening_pid_tool_available",
                return_value=True,
            ),
            patch("codex_crew.cli_server._gateway_owns_port", return_value=True) as mock_owns,
        ):
    ...
    def test_non_posix_degrades_to_the_marker_comparison(self):
        ...
        with (
            patch("codex_crew.cli_server.run_marker.read_pid", return_value=4321),
            patch("codex_crew.cli_server.platform_compat.IS_POSIX", False),
            patch("codex_crew.cli_server._gateway_owns_port", return_value=False) as mock_owns,
        ):
            assert cli_server._replacement_is_serving(7777, 1234) is True
    ...
    def test_probe_targets_api_ready_not_api_health(self):
        ...
        with patch("codex_crew.cli_server.loopback_urlopen", return_value=resp) as mock_open:
    ...
    def test_probe_reports_zero_when_unreachable(self):
        ...
        with patch(
            "codex_crew.cli_server.loopback_urlopen", side_effect=urllib.error.URLError("down")
        ):
            assert cli_server._probe_gateway_ready(7777) == 0
    ...
    def test_probe_reports_the_http_status_of_a_not_ready_gateway(self):
        ...
        with patch("codex_crew.cli_server.loopback_urlopen", side_effect=err):
...
class TestPrintTokenUrl:
    ...
    def test_prints_token_on_success(self, tmp_path, capsys, monkeypatch):
        from codex_crew.cli_server import _print_token_url

        monkeypatch.setattr("codex_crew.cli_server.read_local_secret", lambda _port: "test-secret")
        monkeypatch.setattr(
            "codex_crew.cli_server.CodexCrewConfig.load",
        ...
        monkeypatch.setattr(
            "codex_crew.cli_server.resolve_dashboard_host",
    ...
    def test_prints_custom_origin(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.cli_server.CodexCrewConfig.load",
            lambda: MagicMock(dashboard=MagicMock(url="http://codexcrew.dev:7777")),
        )
        monkeypatch.setattr(
            "codex_crew.cli_server.dashboard_origin", lambda u: "http://codexcrew.dev:7777"
        )
        ...
        out = capsys.readouterr().out
        assert "http://codexcrew.dev:7777/?token=xyz789" in out
...
class TestInstallPidfdChildWatcher:
    ...
    def _install_then_spawn_in_child(expected_watcher: str) -> None:
        ...
        ``codexcrew gateway`` -- so this deterministically exercises the real
        ...
        code = textwrap.dedent("""
            import asyncio
            from codex_crew.cli import _install_child_watcher
        ...
        # Propagate the runtime's import path so the child can import codex_crew
    ...
    def test_installs_safe_watcher_on_macos(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.cli.sys.platform", "darwin")
        ...
        monkeypatch.setattr("codex_crew.cli.os.pidfd_open", _boom, raising=False)
    ...
    def test_sets_watcher_on_linux(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.cli.sys.platform", "linux")
        ...
        closed = []
        monkeypatch.setattr(
            "codex_crew.cli.os.pidfd_open", lambda pid: opened.append(pid) or 4242, raising=False
        )
        monkeypatch.setattr("codex_crew.cli.os.close", lambda fd: closed.append(fd))
        called_with = []
    ...
    def test_falls_back_to_safe_watcher_on_old_kernel(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.cli.os.pidfd_open", _no_pidfd, raising=False)
        installed = []
    ...
    def test_falls_back_to_safe_watcher_when_pidfd_open_missing(self, monkeypatch) -> None:
        ...
        monkeypatch.delattr("codex_crew.cli.os.pidfd_open", raising=False)
        installed = []
        monkeypatch.setattr(asyncio, "set_child_watcher", lambda w: installed.append(w))
        monkeypatch.setattr(asyncio, "SafeChildWatcher", lambda: "fake-safe-watcher")
...
class TestChildWatcherApiRemoved:
    ...
    thing ``codexcrew gateway`` did was raise ``AttributeError: module 'asyncio'
    ...
    def test_noop_when_child_watcher_api_removed(self, monkeypatch, platform) -> None:
        ...
        monkeypatch.setattr("codex_crew.cli.sys.platform", platform)
        ...
        # stubbing os.close: `codex_crew.cli.os` IS the shared os module, so a
        # stub there would also be seen by asyncio's own internals.
        monkeypatch.setattr(
            "codex_crew.cli.os.pidfd_open",
...
class TestTokenCommand:
    """Tests for the ``codexcrew token`` command handler (``_token``)."""
    ...
    def test_prints_loopback_only(self, tmp_path, capsys, monkeypatch):
        from codex_crew.cli_server import _token
        ...
        monkeypatch.setattr("codex_crew.cli_server.dashboard_origin", lambda u: "")
    ...
    def test_separates_custom_origin_with_blank_line(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.cli_server.CodexCrewConfig.load",
            lambda: MagicMock(dashboard=MagicMock(url="https://codexcrew.dev:7777")),
        )
        monkeypatch.setattr(
            "codex_crew.cli_server.dashboard_origin", lambda u: "https://codexcrew.dev:7777"
        )
        monkeypatch.setattr(
            "codex_crew.cli_server.resolve_dashboard_host",
            lambda local_only=True: "canonical-host.invalid",
        ...
        custom_line = "https://codexcrew.dev:7777/?token=xyz789"
    ...
    # (codex_crew.instances.token_mint.mint_remote_token) over SSH. Error prose on
    ...
    def _stub_token_env(self, tmp_path, monkeypatch, *, secret: bool = True) -> None:
        ...
        monkeypatch.setattr("codex_crew.cli_server.read_local_secret", lambda _port: value)
        monkeypatch.setattr(
            "codex_crew.cli_server.CodexCrewConfig.load",
            lambda: MagicMock(dashboard=MagicMock(url="")),
        )
    ...
    def test_unreachable_gateway_error_goes_to_stderr(self, tmp_path, capsys, monkeypatch):
        ...
        self._stub_token_env(tmp_path, monkeypatch)
        with patch(
            "codex_crew.cli_server.loopback_urlopen", side_effect=urllib.error.URLError("refused")
        ):
            with pytest.raises(SystemExit) as excinfo:
                _token(argparse.Namespace(ttl="1h", port=7777))
        assert excinfo.value.code == 1
    ...
    def test_empty_token_error_goes_to_stderr(self, tmp_path, capsys, monkeypatch):
        ...
        self._stub_token_env(tmp_path, monkeypatch)
        with patch(
            "codex_crew.cli_server.loopback_urlopen", return_value=self._mock_token_response("")
        ):
            with pytest.raises(SystemExit) as excinfo:
                _token(argparse.Namespace(ttl="1h", port=7777))
        assert excinfo.value.code == 1
...
class TestBannerBranding:
    ...
    pre-rename name that reached users on `codexcrew` with no args, in the chat
    REPL, and at the top of every `codexcrew cloud` run.
    """
    ...
    def test_main_banner_is_codex_crew(self):
        from codex_crew.cli import BANNER
    ...
    def test_banner_is_single_sourced(self):
        ...
        """
        from codex_crew.cli import BANNER as MAIN
        from codex_crew.cli_chat import BANNER as CHAT
        from codex_crew.constants import BANNER as CANON
    ...
    def test_no_reinlined_banner_literal(self):
        ...
        import codex_crew.cli as cli_mod
        import codex_crew.cli_chat as chat_mod

        for mod in (cli_mod, chat_mod):
            src = Path(mod.__file__).read_text(encoding="utf-8")
            assert "BANNER = r" not in src, f"{mod.__name__} re-inlined the banner literal"

    def test_cloud_banner_is_codex_crew_cloud(self):
        from codex_crew.cloud.ui import BANNER
    ...
    def test_no_kiroclaw_spelling_anywhere_in_banners(self):
        from codex_crew.cloud.ui import BANNER as CLOUD
        from codex_crew.constants import BANNER as MAIN
...
class TestChatPermissionRequest:
    """`codexcrew chat` must ANSWER a permission request, and answering one is an
    ...
    def _fresh_stdin_state(self, monkeypatch):
        ...
        import codex_crew.cli_chat as cli_chat
    ...
    def _event(
        *,
        ...
        """
        from codex_crew.providers.base import EVENT_PERMISSION_REQUEST, LLMEvent
    ...
    def _direct_tool_call(client, *, path="notes.md", tool_call_id="tc-direct"):
        ...
        from codex_crew.acp.types import JsonRpcMessage
    ...
    class _GatedProvider:
        ...
        async def stream(self, message):
            from codex_crew.providers.base import EVENT_COMPLETE, EVENT_TEXT_CHUNK, LLMEvent
    ...
    def _gate():
        ...
        import codex_crew.cli_chat as cli_chat
        from codex_crew.hooks import HookManager, HooksConfig
    ...
    async def test_direct_client_non_shell_permission_can_be_allowed(self, monkeypatch, tmp_path):
        ...
        from codex_crew.acp.types import ACP_BACKEND_CLAUDE
    ...
    async def test_canonical_mcp_identity_is_forwarded_and_denies(self, monkeypatch):
        ...
        import codex_crew.cli_chat as cli_chat
        import codex_crew.hooks as hooks_mod
        from codex_crew.providers.base import EVENT_PERMISSION_REQUEST, LLMEvent
...
"""
    ...
    def test_denial_notice_survives_a_non_utf8_redirected_stderr(self, tmp_path, io_encoding):
        ...
        env["CODEXCREW_HOME"] = str(tmp_path / "home")
...
class TestChatProviderShutdown:
    ...
    def _patch(self, monkeypatch, provider):
        ...
        monkeypatch.setattr(CodexCrewConfig, "load", staticmethod(lambda: CodexCrewConfig()))
        monkeypatch.setattr(
