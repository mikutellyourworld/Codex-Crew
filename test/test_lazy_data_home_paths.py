...
``CODEXCREW_HOME`` on *every* call. Binding one of them to a module-level constant
freezes whichever home happened to be active when that module was first
imported, which silently breaks:

* **pod isolation** -- a pod exports its own ``CODEXCREW_HOME``;
* **the lazy default-home resolution** (``~/.codex-crew``), which is deliberately
  resolved late and cached;
* **test isolation** -- the autouse ``_isolate_codexcrew_home`` fixture in
...
def _transitive_path_factories() -> set[str]:
    ...
    ``Path``-returning function elsewhere in ``src/codex_crew/`` that calls
...
_CONFIG_DIR_ACCESSORS = [
    ("codex_crew.dashboard.handlers.usage", "_TOKEN_USAGE_DIR", "_token_usage_dir"),
    ("codex_crew.cron", "_DEFAULT_DIR", "_default_dir"),
    ("codex_crew.subagent_persistence", "_SUBAGENTS_DIR", "_subagents_dir"),
    ("codex_crew.dashboard.handlers.files", "_UPLOAD_DIR", "_upload_dir"),
    ("codex_crew.dashboard.handlers.files", "_SCREENSHOT_DIR", "_screenshot_dir"),
    ("codex_crew.dashboard.handlers.hooks", "_HOOK_STORE_PATH", "_hook_store_path"),
    ("codex_crew.dashboard.handlers.mcp", "_CODEXCREW_MCP_JSON", "_codexcrew_mcp_json"),
    ("codex_crew.slack.sessions_view", "_SESSIONS_DIR", "_sessions_dir"),
    ("codex_crew.apps.builtins.auto_research.handlers", "RESEARCH_DIR", "research_dir"),
    ("codex_crew.apps.builtins.auto_research.handlers", "DB_PATH", "db_path"),
]


class TestAccessorsFollowTheLiveHome:
    """A post-import ``CODEXCREW_HOME`` change must redirect every accessor.

    ``config_dir()`` returns a ``$CODEXCREW_HOME`` override immediately, ahead of
    ...
    def test_every_accessor_follows_a_post_import_home_change(self, tmp_path, monkeypatch):
        ...
        assert not stranded, (
            "accessor did not follow CODEXCREW_HOME set after import (issue #874):\n  "
            + "\n  ".join(stranded)
        )
    ...
    def test_accessor_tracks_a_second_change(self, tmp_path, monkeypatch):
        ...
        mod = importlib.import_module("codex_crew.dashboard.handlers.usage")
        first = tmp_path / "home-1"
        monkeypatch.setenv("CODEXCREW_HOME", str(first))
        one = mod._token_usage_dir()
        second = tmp_path / "home-2"
        monkeypatch.setenv("CODEXCREW_HOME", str(second))
    ...
    def test_override_hook_still_wins(self, tmp_path, monkeypatch):
        ...
        mod = importlib.import_module("codex_crew.dashboard.handlers.usage")
        monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path / "ignored-home"))
...
class TestResolutionDoesNotRepeatStartupMaintenance:
    ...
    def test_override_is_never_cached(self, tmp_path, monkeypatch):
        """A CODEXCREW_HOME set after import must still be honoured (#874).
    ...
    def test_invalid_override_does_not_reopen_the_maintenance_path(
        ...
        monkeypatch.setenv("CODEXCREW_HOME", tmp_path.anchor)
    ...
    def test_valid_override_still_delegates(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CODEXCREW_HOME", str(good))
...
class TestConfigDirMemoIsNotServedAfterTheHomeIsCleared:
    ...
    The entry is keyed on ``(raw CODEXCREW_HOME, resolved home)`` and the suite's
    ...
    def test_entry_from_a_bypassed_resolution_is_not_served_later(
        ...
        # ``~/.codexcrew.breadcrumb`` on the machine running the suite.
