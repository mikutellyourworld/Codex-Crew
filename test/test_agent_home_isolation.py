...
from codex_crew.config.paths import kiro_agents_dir, kiro_home

REPO_ROOT = Path(__file__).resolve().parents[1]
SRC = REPO_ROOT / "src" / "codex_crew"
...
def _no_overrides(monkeypatch) -> None:
    """No KIRO_HOME and no CODEXCREW_HOME — the plain-install baseline.

    Both must be cleared: with a CODEXCREW_HOME override active AND the code
    ...
    monkeypatch.delenv("CODEXCREW_POD", raising=False)
...
def test_kiro_home_matches_codexcrew_home_safety_rules():
    """Both overrides share one predicate, so they refuse the same targets."""
    from codex_crew.config.paths import _is_unsafe_home
...
def test_macos_private_etc_is_refused():
    ...
    # CODEXCREW_HOME=/etc/codexcrew through on macOS alone — the two platforms
    # would disagree about the same override.
    assert _is_unsafe_home(Path("/etc/codexcrew").resolve())
    assert _is_unsafe_home(Path("/private/etc/codexcrew"))
...
def test_under_system_tmp_answers_on_the_call_time_root():
    ...
    from codex_crew.config.paths import _under_system_tmp

    root = Path(tempfile.gettempdir()).resolve()
    assert _under_system_tmp(root)
    assert _under_system_tmp(root / "kc-task-1234" / "repo" / "src")
    assert not _under_system_tmp(Path("/durable-install/CodexCrew").resolve())
...
def _make_linked_worktree(tmp_path: Path) -> Path:
    ...
    wt = tmp_path / "codexcrew-wt-example"
    (wt / "src" / "codex_crew").mkdir(parents=True)
    (wt / ".git").write_text(
        "gitdir: /somewhere/CodexCrew/.git/worktrees/codexcrew-wt-example\n",
        encoding="utf-8",
    )
    return wt
...
def test_private_target_is_never_declined(monkeypatch, tmp_path):
    ...
    This is what keeps the guard from breaking CodexCrew's own suite: development
    ...
    monkeypatch.setattr(agent, "__file__", str(wt / "src" / "codex_crew" / "agent.py"))
...
def test_agent_home_inside_own_data_home_is_private(monkeypatch, tmp_path):
    ...
    """
    from codex_crew import agent
    from codex_crew.config.paths import isolated_agents_dir

    own_home = tmp_path / "wt" / ".codexcrew-dev"
    own_home.mkdir(parents=True)
    agents = isolated_agents_dir(own_home)

    wt = _make_linked_worktree(tmp_path)
    monkeypatch.setattr(agent, "__file__", str(wt / "src" / "codex_crew" / "agent.py"))
    monkeypatch.setenv("CODEXCREW_HOME", str(own_home))
...
def test_data_home_that_is_an_ancestor_does_not_make_shared_private(monkeypatch, tmp_path):
    ...
    ``CODEXCREW_HOME=$HOME`` the machine-wide ``~/.kiro/agents`` sits beneath the
    ...
    monkeypatch.setenv("CODEXCREW_HOME", str(fake_home))
...
def test_declines_from_a_clone_under_the_temp_dir(monkeypatch, tmp_path):
    ...
    with tempfile.TemporaryDirectory(prefix="kc-clone-") as scratch_name:
        ...
        (clone / "src" / "codex_crew").mkdir(parents=True)
        (clone / ".git").mkdir()  # a DIRECTORY -> ordinary clone, not a worktree
        monkeypatch.setattr(agent, "__file__", str(clone / "src" / "codex_crew" / "agent.py"))
        _pretend_target_is_shared(monkeypatch, agent, shared)
...
def test_does_not_decline_from_a_temp_clone_when_no_spec_exists(monkeypatch, tmp_path):
    ...
    spec at all -- every turn then fails with ``Mode 'codexcrew' not found``.
    """
    import tempfile
...
def test_does_not_decline_from_an_appimage_runtime_mount(monkeypatch, tmp_path):
    ...
    with tempfile.TemporaryDirectory(prefix="kc-appimage-") as scratch_name:
        ...
        (mount / "usr" / "lib" / "codex_crew").mkdir(parents=True)
        monkeypatch.setattr(
            agent, "__file__", str(mount / "usr" / "lib" / "codex_crew" / "agent.py")
        )
        _pretend_target_is_shared(monkeypatch, agent, tmp_path / "agents")
...
def test_under_system_tmp_covers_posix_tmp_when_tmpdir_points_elsewhere(monkeypatch, tmp_path):
    ...
    from codex_crew.config import paths as paths_mod
...
def test_under_system_tmp_still_answers_yes_for_an_appimage_mount():
    ...
    from codex_crew.config.paths import _in_ephemeral_tree, _under_system_tmp
...
def test_does_not_decline_from_a_durable_clone(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("KIRO_HOME", raising=False)
    monkeypatch.delenv("CODEXCREW_HOME", raising=False)
    durable = Path("/durable-install/CodexCrew/src/codex_crew/agent.py")
...
def test_pod_env_gives_the_pod_its_own_homes():
    ...
    transcripts somewhere CodexCrew never reads and lose session resume. With
    ...
    from codex_crew.pod.runtime import build_pod_env

    cfg = PodConfig.load()
    home = Path("/tmp/codexcrew-pods/example")
    env = build_pod_env(cfg, home, 7811, Path("/workplace/example"))

    assert env["KIRO_HOME"] == str(home / "kiro")
    assert env["CODEXCREW_HOME"] == str(home)
...
def test_sessions_dir_follows_kiro_home(monkeypatch, tmp_path):
    ...
    CodexCrew kept reading the machine-wide path, an instance with its own agent
    ...
    """
    from codex_crew.config.paths import kiro_agents_dir, kiro_sessions_dir
