...
its approve/deny verdict -- lazily imports ``codex_crew.slack.gateway`` for
``_is_read_only_tool``. That module imports ``codex_crew.channels``, which
...
def test_the_approval_hook_import_chain_loads_without_cryptography() -> None:
    proc = _run("""
        ...
        import codex_crew.channels
        import codex_crew.wecom.media
        import codex_crew.weixin.media
        import codex_crew.secrets.vault
...
def test_only_decrypting_media_needs_cryptography() -> None:
    ...
    proc = _run("""
        from codex_crew.wecom import media as wecom_media
        from codex_crew.weixin import media as weixin_media
