...
from codex_crew.cloud import aws, iam
...
class TestPolicyDocument:
    ...
    def test_put_role_policy_is_tag_scoped(self):
        # PutRolePolicy must be gated on aws:ResourceTag/codexcrew:managed=true (in
        ...
        # codexcrew-ec2-* role. The role is tagged atomically at CreateRole, so the
    ...
    def test_put_role_policy_and_passrole_not_tag_scoped_regression(self):
        # Guard: both PutRolePolicy and PassRole on a codexcrew-ec2-* role ARN must
        ...
        for sid in ("IamPutRolePolicyForInstance", "IamPassRoleToEc2"):
            ...
            ), f"{sid} lost its aws:ResourceTag/codexcrew:managed gate"

    def test_tag_role_gated_on_existing_managed_tag(self):
        # iam:TagRole must be a SEPARATE statement gated on aws:ResourceTag/
        # codexcrew:managed=true — NOT unconditioned, and NOT in IamRoleForInstance.
        # If it were unconditioned, a leaked launcher credential could tag a
        # pre-existing UNBOUNDED codexcrew-ec2-* role codexcrew:managed=true and
    ...
    def test_tag_role_not_unconditioned_anywhere(self):
        ...
        # aws:ResourceTag/codexcrew:managed=true gate — an unconditioned TagRole
        ...
        for st in iam.policy_document()["Statement"]:
            if "iam:TagRole" in st.get("Action", []):
                ...
                assert se.get(f"aws:ResourceTag/{iam.MANAGED_TAG_KEY}") == "true", (
                    ...
                    "aws:ResourceTag/codexcrew:managed=true gate"
                )
    ...
    def test_cloudformation_mutation_scoped_to_codexcrew_stacks(self):
        st = next(
        ...
        # scoped to codexcrew-* stacks, not "*"
    ...
    def test_changeset_actions_scoped_to_changeset_and_stack_arns(self):
        ...
        # present, still codexcrew-*-scoped.
        st = next(
        ...
        assert all(iam.STACK_PREFIX in r for r in res)  # all codexcrew-*-scoped

    def test_stack_prefix_matches_ec2(self):
        from codex_crew.cloud import ec2
    ...
    def test_create_role_requires_boundary_with_arnlike(self):
        ...
        # codexcrew-ec2-* role MUST carry our permissions boundary. The condition
    ...
    def test_put_role_policy_scoped_no_dead_boundary_condition(self):
        ...
        # carries the aws:ResourceTag/codexcrew:managed gate (see
    ...
    def test_boundary_document_shape(self):
        ...
        assert s3["Resource"] == "arn:aws:s3:::codexcrew-src-123456789012-*/*"
        # roundtrips as JSON
        assert json.loads(iam.boundary_policy_json("123456789012")) == doc

    def test_authorize_security_group_is_tag_gated(self):
        # SG rule mutation must be gated to codexcrew:managed=true SGs so a leaked
    ...
    def test_run_instances_request_tag_gated_on_instance_only(self):
        # ec2:RunInstances must require aws:RequestTag/codexcrew:managed=true, but
    ...
    def test_ssm_session_and_sendcommand_gated_to_managed_instances(self):
        # The RCE-adjacent verbs must be tag-scoped to CodexCrew instances so a
    ...
    def test_create_tags_only_on_create(self):
        ...
        # can't tag arbitrary existing resources as codexcrew:managed=true and
    ...
    def test_attach_role_policy_pinned_to_ssm_core(self):
        ...
        # to a codexcrew-ec2-* role and pass it to EC2 (full escalation).
...
class TestAgentDenyListForCloudVerbs:
    ...
    CodexCrew's own PreToolUse gate (``security.is_denied`` via the ported
    ...
    def test_launcher_creation_verbs_denied(self):
        ...
        # allowed.
        must_deny = [
            "aws cloudformation deploy --template-file t --stack-name codexcrew-x",
            ...
            "aws iam create-role --role-name codexcrew-ec2-x",
            ...
            "aws iam create-policy --policy-name codexcrew-ec2-boundary --policy-document {}",
    ...
    def test_s3api_write_verbs_denied(self):
        # The launcher IAM grants s3:PutObject to codexcrew-src-* buckets; if the
    ...
    def test_codexcrew_cloud_wrapper_denied(self):
        # `codexcrew cloud destroy` is a wrapper that internally runs
        ...
        for cmd in (
            "codexcrew cloud destroy --yes --tag kc-1",
            "codexcrew cloud stop",
            "kiro-crew cloud launch",
            "codexcrew cloud connect",  # mints/prints a dashboard token
            "codexcrew cloud tunnel",
            "codexcrew cloud login",
        ):
            assert self._denied(cmd), f"codexcrew cloud wrapper not denied: {cmd!r}"
        # read-only observation stays allowed
        for allowed in ("codexcrew cloud list", "codexcrew cloud status"):
