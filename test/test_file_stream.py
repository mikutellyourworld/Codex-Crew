...
from codex_crew.dashboard.handlers import api_file_stream
...
async def test_resolve_serves_relative_path_from_project_dir(tmp_path, mock_sel, monkeypatch):
    ...
    with patch("codex_crew.dashboard.handlers._validate_dashboard_path", side_effect=lambda p: p):
        async with TestClient(TestServer(_make_app())) as client:
...
async def test_text_credentials_behind_forged_magic_refused(tmp_path, mock_sel):
    ...
    with patch("codex_crew.dashboard.handlers._validate_dashboard_path", return_value=str(f)), \
         patch("codex_crew.dashboard.handlers.files.redact", side_effect=lambda t: "[REDACTED]"):
        async with TestClient(TestServer(_make_app())) as client:
            resp = await client.get(f"/api/file-stream?path={f}")
            assert resp.status == 400
...
async def test_invalid_utf8_magic_cannot_skip_the_scan(tmp_path, mock_sel):
    ...
    with patch("codex_crew.dashboard.handlers._validate_dashboard_path", return_value=str(f)), \
         patch("codex_crew.dashboard.handlers.files.redact", side_effect=_flag_credentials):
        async with TestClient(TestServer(_make_app())) as client:
            resp = await client.get(f"/api/file-stream?path={f}")
            assert resp.status == 400
            assert (await resp.json())["code"] == "content_redacted"
...
async def test_binary_media_is_not_affected_by_the_probe(tmp_path, mock_sel):
    ...
    f.write_bytes(_MP4_BYTES)
    with patch("codex_crew.dashboard.handlers._validate_dashboard_path", return_value=str(f)), \
         patch("codex_crew.dashboard.handlers.files.redact", side_effect=lambda t: t):
        async with TestClient(TestServer(_make_app())) as client:
            resp = await client.get(f"/api/file-stream?path={f}")
            assert resp.status == 200
            assert await resp.read() == _MP4_BYTES
...
async def test_missing_file_404(tmp_path, mock_sel):
    ...
    with patch("codex_crew.dashboard.handlers._validate_dashboard_path", return_value=str(missing)):
        async with TestClient(TestServer(_make_app())) as client:
...
async def test_symlink_refused_403(tmp_path, mock_sel):
    ...
    with patch("codex_crew.dashboard.handlers._validate_dashboard_path", return_value=str(link)):
        async with TestClient(TestServer(_make_app())) as client:
...
async def test_oversize_file_413_via_fstat_not_read(tmp_path, mock_sel):
    ...
    f.write_bytes(_MP4_BYTES)
    with patch("codex_crew.dashboard.handlers._validate_dashboard_path", return_value=str(f)), \
         patch("codex_crew.dashboard.handlers.files._STREAM_MAX_BYTES", 10):
        async with TestClient(TestServer(_make_app())) as client:
            resp = await client.get(f"/api/file-stream?path={f}")
...
def test_resolve_passes_windows_absolute_shapes_unaltered(monkeypatch):
    ...
    monkeypatch.setenv("CODEXCREW_PROJECT_DIR", "/proj")
    unc = "\\\\evil-host\\share\\x.mp4"
    with patch(
        "codex_crew.dashboard.handlers.files.os.path.realpath",
