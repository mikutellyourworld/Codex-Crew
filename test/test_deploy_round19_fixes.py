...
def test_f1_sanitize_response_redacts_credential_in_error():
    ...
    from codex_crew.deploy.handlers import _sanitize_response
...
def test_f2_hardlink_rejected_in_staging(tmp_path):
    ...
    from codex_crew.deploy.handlers import _stage_tree_safe
    src = tmp_path / "site"
    src.mkdir()
...
def _load_attach_backend():
    spec_path = (
        ...
        / "src/codex_crew/deploy/skills/artifact-deploy/scripts/attach_backend.py"
    )
...
def test_f4_reaper_dist_tag_mismatch_skips(capsys):
    """Distribution with mismatched codexcrew:site tag is skipped, not deleted."""
    from unittest.mock import patch as _patch
    spec_path = (
        Path(__file__).resolve().parent.parent
        / "src/codex_crew/deploy/skills/artifact-deploy/scripts/reaper_lambda/index.py"
    )
    ...
    mock_cf.list_tags_for_resource.return_value = {
        "Tags": {"Items": [{"Key": "codexcrew:site", "Value": "OTHER-SLUG"}]}
    }
...
def test_f5_dedup_validates_webapp_metadata():
    ...
    from codex_crew.dashboard.handlers.artifacts import _validate_inbound_webapp_metadata
...
def test_f5_dedup_kind_conflict_code_exists():
    ...
    from codex_crew.dashboard.handlers import artifacts as art_mod
