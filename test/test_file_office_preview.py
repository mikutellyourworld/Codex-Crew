...
from codex_crew.dashboard.handlers import api_file_office_preview
from codex_crew.dashboard.handlers import files as files_mod
from codex_crew.dashboard.handlers.files import _MAX_UPLOAD_BYTES, _OFFICE_PREVIEW_CAP
...
async def test_sensitive_path_403(tmp_path, mock_sel):
    ...
    with (
        ...
        patch(
            "codex_crew.dashboard.handlers.files.is_sensitive_path",
            return_value=True,
        ),
    ):
        async with TestClient(TestServer(_make_app())) as client:
...
async def test_resolve_uses_shared_helper(tmp_path, mock_sel):
    ...
    with (
        patch.dict(os.environ, {"CODEXCREW_PROJECT_DIR": str(f.parent)}),
        patch("codex_crew.dashboard.handlers._validate_dashboard_path", return_value=str(f)),
    ):
        async with TestClient(TestServer(_make_app())) as client:
...
async def test_resolve_outside_project_denied_and_audited(tmp_path, mock_sel):
    proj = tmp_path / "proj"
    proj.mkdir()
    with patch.dict(os.environ, {"CODEXCREW_PROJECT_DIR": str(proj)}):
        async with TestClient(TestServer(_make_app())) as client:
...
async def test_extraction_reads_through_the_prefix_fd_and_closes_it(tmp_path, mock_sel):
    ...
    with (
        patch("codex_crew.dashboard.handlers._validate_dashboard_path", return_value=str(f)),
        patch("codex_crew.dashboard.handlers.files.extract_text", side_effect=_spy),
    ):
        async with TestClient(TestServer(_make_app())) as client:
            resp = await client.get(f"/api/file-office-preview?path={f}")
            assert resp.status == 200
...
async def test_open_envelope_is_the_shared_prefix_and_runs_off_the_loop(tmp_path, mock_sel):
    ...
    with (
        patch("codex_crew.dashboard.handlers._validate_dashboard_path", return_value=str(f)),
        patch("codex_crew.dashboard.handlers.files._open_checked_file", side_effect=_spy),
    ):
        async with TestClient(TestServer(_make_app())) as client:
            resp = await client.get(f"/api/file-office-preview?path={f}")
            assert resp.status == 200
