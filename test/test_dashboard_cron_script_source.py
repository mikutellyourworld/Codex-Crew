...
def crons_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr("codex_crew.cron_script.config_dir", lambda: tmp_path)
    monkeypatch.setattr("codex_crew.dashboard.handlers.cron.config_dir", lambda: tmp_path)
    return tmp_path
...
def stub_sel():
    ...
    """
    with patch("codex_crew.dashboard.handlers.cron._sel") as sel_fn:
        recorder = MagicMock()
