...
from codex_crew.safety_override import SafetyOverride, reset_singleton
...
def _quiet_sel():
    ...
    return patch("codex_crew.safety_override.sel", return_value=MagicMock())
...
class TestWhichLoopsCount:
    def test_only_active_loops_are_counted(self) -> None:
        ...
        with patch("codex_crew.dashboard.server._autonudge_get", return_value=svc):
            assert len(_armed_unattended_loops()) == 1

    def test_no_service_means_no_loops(self) -> None:
        with patch("codex_crew.dashboard.server._autonudge_get", return_value=None):
...
class TestBothSurfacesAreNotified:
    ...
    def _drive(self, state, svc):
        async def _run():
            with patch("codex_crew.dashboard.server._autonudge_get", return_value=svc):
                with patch("codex_crew.dashboard.server._dm_owner") as dm:
...
class TestTheExpiryCallbackFiresExactlyOnce:
    ...
    def test_repeated_is_active_calls_fire_the_callback_once(self) -> None:
        ...
        with _quiet_sel(), patch(
            "codex_crew.safety_override.time.monotonic", return_value=anchor + 700
        ):
            for _ in range(5):
...
class TestPolicyRevocationSuppressesTheUnFollowableRemedy:
    ...
    def test_policy_source_drops_the_remedy_and_names_the_cause(self) -> None:
        from codex_crew.safety_override import POLICY_REVOKED_SOURCE
...
class TestTheNoticeHasItsOwnChannel:
    ...
    def test_the_kind_routes_to_its_own_channel_not_the_fallback(self) -> None:
        ...
        """
        from codex_crew.notifications.bus import _FALLBACK_CHANNEL, payload_from_legacy
    ...
    def test_it_is_default_priority_not_critical(self) -> None:
        ...
        """
        from codex_crew.notifications.bus import SYSTEM_CHANNELS
