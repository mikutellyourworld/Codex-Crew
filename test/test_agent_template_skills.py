...
def fake_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "codex_crew.agent_discovery._KIRO_AGENTS_DIR", tmp_path / ".kiro" / "agents"
    )
    return tmp_path
...
class TestEnumerateSkillCatalog:
    ...
    def test_sensitive_root_is_skipped(self, fake_home, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.handlers._shared._skill_key_roots",
...
class TestPatchRejectionLeavesStateIntact:
    ...
    def test_unknown_skill_does_not_freeze_the_model(self, fake_home, monkeypatch):
        ...
        from codex_crew.dashboard.handlers import agents as agents_handlers
        ...
        monkeypatch.setattr("codex_crew.agent.KIRO_AGENTS_DIR", d, raising=False)
...
class TestExtraSkillPathsAreAbsolute:
    def test_relative_extra_path_is_made_absolute(self, fake_home, monkeypatch):
        ...
        from codex_crew.dashboard.handlers._shared import _skill_key_roots
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.handlers._shared.CodexCrewConfig",
...
class TestSessionContextGate:
    ...
    def test_mapped_codexcrew_is_scoped_not_full_catalog(self, fake_home):
        """The mapping bounds the codexcrew agent too: before this feature it
        always received the entire catalog."""
        skills_root = fake_home / "skills"
        _make_skill(skills_root, "alpha")
        _make_skill(skills_root, "beta")
        d = _agents_dir(fake_home)
        (d / "codexcrew.json").write_text(
            json.dumps(
                {
                    "name": "codexcrew",
                    "resources": [f"skill://{(skills_root / 'alpha' / 'SKILL.md').as_posix()}"],
        ...
        ctx = self._builder(fake_home, skills_root).build_session_context(
            agent="codexcrew", provider_type="claude_code"
        )
        assert "alpha" in ctx
        assert "beta" not in ctx

    def test_mapped_codexcrew_on_kiro_defers_to_native_load(self, fake_home):
        """On the kiro backend the mapped SKILL.md files are loaded by kiro-cli
        from ``resources``, so CodexCrew must not inject them a second time."""
        skills_root = fake_home / "skills"
        _make_skill(skills_root, "alpha")
        _make_skill(skills_root, "beta")
        d = _agents_dir(fake_home)
        ...
        ctx = self._builder(fake_home, skills_root).build_session_context(
            agent="codexcrew", provider_type="acp"
        )
        assert "[Skills:]" not in ctx

    def test_unmapped_codexcrew_still_gets_everything(self, fake_home):
        skills_root = fake_home / "skills"
        _make_skill(skills_root, "alpha")
        _make_skill(skills_root, "beta")
