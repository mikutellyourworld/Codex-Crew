...
from codex_crew.dashboard.handlers.files import api_channel_upload_file, api_slack_upload_file
...
class TestFileUploadChannel:
    ...
    async def test_upload_to_tracked_channel(self, tmp_path, outbox_file):
        ...
        with patch(
            "codex_crew.config.loader.outbox_dir",
            return_value=outbox_file.parent,
        ), patch(
            "codex_crew.config.loader.workspace_root",
            return_value=tmp_path,
        ), patch(
            "codex_crew.dashboard.handlers.files.is_tracked_channel",
            return_value=True,
        ):
...
class TestFileUploadBinary:
    ...
    async def test_binary_audio_uploaded_to_slack(self, tmp_path):
        ...
        with patch(
            ...
            "codex_crew.dashboard.handlers.files._sel",
...
class TestFileUploadSlotThreading:
    ...
    async def test_slot_thread_ts_used_when_body_empty(self, tmp_path):
        ...
        with patch(
            "codex_crew.config.loader.outbox_dir", return_value=outbox
        ), patch(
            "codex_crew.config.loader.workspace_root", return_value=tmp_path
        ), patch(
            "codex_crew.dashboard.handlers.files.is_tracked_channel", return_value=False
        ):
            async with TestClient(TestServer(app)) as client:
                resp = await client.post(
                    "/api/slack/upload-file",
    ...
    async def test_explicit_thread_ts_takes_priority_over_slot(self, tmp_path):
        ...
        with patch(
            ...
            "codex_crew.dashboard.handlers.files.is_tracked_channel", return_value=True
        ):
            async with TestClient(TestServer(app)) as client:
                resp = await client.post(
                    "/api/slack/upload-file",
...
class TestChannelUploadEndpoint:
    ...
    async def test_a_restricted_session_gets_no_native_delivery(self, tmp_path, outbox_file):
        ...
        app = self._app()
        with patch(
            "codex_crew.dashboard.chat_runner._resolve_mirror_target",
            return_value=(self._link("telegram", "42"), transport),
        ), patch(
            "codex_crew.messaging.upload_gate.uploads_restricted",
    ...
    async def test_outbound_text_is_redacted_in_display_form(self, tmp_path, outbox_file):
        ...
        with patch(
            ...
            "codex_crew.config.loader.outbox_dir", return_value=outbox_file.parent
        ), patch(
            "codex_crew.config.loader.workspace_root", return_value=tmp_path
        ):
            async with TestClient(TestServer(app)) as client:
    ...
    async def test_discord_destination_gets_send_document(self, tmp_path, outbox_pdf):
        ...
        with patch(
            ...
            "codex_crew.config.loader.outbox_dir", return_value=outbox_pdf.parent
        ), patch(
            "codex_crew.config.loader.workspace_root", return_value=tmp_path
        ):
            async with TestClient(TestServer(app)) as client:
        ...
        from codex_crew.messaging.outbound_files import upload_filename
    ...
    async def test_path_outside_allowed_roots_is_denied(self, tmp_path):
        ...
        with patch(
            ...
            "codex_crew.config.loader.workspace_root", return_value=workspace
        ):
            async with TestClient(TestServer(app)) as client:
                resp = await client.post(
                    "/api/channel/upload-file",
...
class TestSlackUploadAuthorizationRungs:
    ...
    async def test_a_governance_denied_channels_scope_denies_the_slack_leg(
        ...
        with patch(
            ...
            "codex_crew.dashboard.handlers.files._sel", return_value=sel
        ), patch(
            "codex_crew.dashboard.upload_destination.vet_and_audit",
...
class TestDestinationOracleEquivalence:
    ...
    async def test_a_dm_from_the_session_map_never_consults_the_tracking_probe(
        ...
        with patch(
            ...
            "codex_crew.dashboard.handlers.files.is_tracked_channel", new=probe
        ):
    ...
    async def test_the_admission_gate_runs_before_any_destination_work(self, tmp_path):
        ...
        with patch(
            "codex_crew.config.loader.outbox_dir", return_value=tmp_path / "outbox"
        ), patch(
            "codex_crew.config.loader.workspace_root", return_value=tmp_path / "workspace"
        ):
            resp, body = await self._post_slack(
                app,
    ...
    async def test_both_legs_resolve_through_the_one_oracle_module(self):
        ...
        from codex_crew.dashboard import upload_destination
        from codex_crew.dashboard.handlers import files
