...
from codex_crew.providers.acp import AcpProvider, provider_label
...
class TestBackendPredicates:
    ...
    def test_exactly_one_predicate_holds_for_every_known_backend(self, backend):
        ...
        held = [
            ...
            provider.is_kimi_backend,
            provider.is_openai_compatible_backend,
        ]
        assert sum(held) == 1
...
class TestProviderLabel:
    ...
    def test_labels_each_backend(self):
        ...
        assert provider_label(_build_provider(ACP_BACKEND_KIMI)) == PROVIDER_LABEL_KIMI
        assert (
            provider_label(_build_provider(ACP_BACKEND_OPENAI_COMPATIBLE))
            == PROVIDER_LABEL_OPENAI_COMPATIBLE
        )
...
class TestConfigThreading:
    ...
    def test_default_config_is_codex(self):
        cfg = CodexCrewConfig()
...
def _load_agent_config(agent_data: dict, tmp_path: Path) -> CodexCrewConfig:
...
class TestConfigRoundTrip:
    ...
    def test_the_field_is_consumed_on_load(self, tmp_path):
        ...
        """
        cfg = _load_agent_config({"acp_backend": ACP_BACKEND_KIMI, "streaming": False}, tmp_path)
        assert cfg.agent.acp_backend == ACP_BACKEND_KIMI
        assert cfg.agent.streaming is False

    def test_absent_key_loads_as_the_default(self, tmp_path):
        assert _load_agent_config({}, tmp_path).agent.acp_backend == ACP_BACKEND_CODEX

    def test_kimi_survives_a_load_from_disk(self, tmp_path):
        ...
        """
        cfg = _load_agent_config({"acp_backend": ACP_BACKEND_KIMI}, tmp_path)
        assert cfg.agent.acp_backend == ACP_BACKEND_KIMI
        assert cfg.to_dict()["agent"]["acp_backend"] == ACP_BACKEND_KIMI
        provider = cfg.create_provider_factory()(session_key="test:rt", agent="")
        assert provider.is_kimi_backend is True
...
class TestNoImportCycle:
    ...
    module-scope import of ``codex_crew.acp.types`` from here is a cycle: reaching
    that module executes the ``codex_crew.acp`` package init, which imports the
    ...
    ``codex_crew.acp`` already — so this asserts it in a fresh interpreter.
    """

    def test_config_loader_imports_alone(self):
        proc = subprocess.run(
            [sys.executable, "-c", "import codex_crew.config.loader"],
            capture_output=True,
            text=True,
            timeout=120,
        )
    ...
    def test_the_backend_normalizer_works_in_that_interpreter(self):
        ...
        proc = subprocess.run(
            [
                sys.executable,
                "-c",
                "import codex_crew.config.loader as m;"
        ...
        assert proc.stdout.strip().split("|") == ["codex", "codex"]
