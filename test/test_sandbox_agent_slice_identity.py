...
and a host routinely runs several at once, because ``codexcrew pod up`` creates
    ...
    def _call(argv=None):
        ...
        try:
            with (
                ...
                patch("codex_crew.sandbox._reconcile_slice_memory_high_off_thread"),
                patch(
...
class TestAgentSliceIdentity:
    def test_two_instances_do_not_share_a_slice(self, wrapped, monkeypatch, tmp_path):
        ...
        """
        monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path / "instance-a"))
        slice_a = _slice_arg(wrapped())
        monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path / "instance-b"))
    ...
    def test_slice_nests_under_the_aggregate_parent(self, wrapped, monkeypatch, tmp_path):
        ...
        systemd's dash-hierarchy makes ``codexcrew-agents-<tok>.slice`` a child
        of ``codexcrew-agents.slice``, so the MemoryHigh / MemoryMax applied to
    ...
    def test_unresolvable_home_degrades_to_the_shared_slice(self, wrapped, caplog):
        ...
        with patch(
            "codex_crew.sandbox._instance_slice_token",
...
class TestSlicePressureScanFindsNestedScopes:
    ...
    def _slice_tree(self, tmp_path, *, nested: bool):
        ...
        root = tmp_path / "codexcrew-agents.slice"
        holder = root / "codexcrew-agents-abc123.slice" if nested else root
    ...
    def test_victim_scope_is_named(self, tmp_path, nested):
        ...
        try:
            ...
            with (
                patch("codex_crew.sandbox._agents_slice_cgroup_dir", return_value=root),
                patch("codex_crew.sandbox._SLICE_LIMITS_APPLIED", False),
            ):
