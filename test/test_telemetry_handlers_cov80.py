...
from codex_crew.dashboard.handlers import telemetry as h
...
def _patch_config(monkeypatch: pytest.MonkeyPatch, cfg: Any) -> None:
    ...
    monkeypatch.setattr(h, "CodexCrewConfig", loader)
...
async def test_startup_route_reports_posture_and_all_four_blocks(
    ...
    monkeypatch.setattr(
        ...
        lambda: {
            ...
            "turn": {"count": 1},
            "other": [{"name": "codexcrew.x", "kind": "counter"}],
            "shard_count": 3,
        },
    )
    ...
    assert payload["other"][0]["name"] == "codexcrew.x"
...
def _beacon_stub(monkeypatch: pytest.MonkeyPatch, info: dict, *, env_opted_out: bool) -> MagicMock:
    ...
    stub.DISABLE_ENV = "CODEXCREW_TELEMETRY_DISABLED"
...
async def test_beacon_status_reports_stored_flag_and_effective_verdict(
    ...
    assert payload["env_var"] == "CODEXCREW_TELEMETRY_DISABLED"
