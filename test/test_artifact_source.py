"""Tests for :mod:`codex_crew.artifact_source` — the copy-vs-link decision.
...
from codex_crew.artifact_source import (
    COPY,
    LINK,
    classify_source,
    disposable_roots,
...
class TestNonGitProjectsLink:
    ...
    def test_a_marked_filesystem_root_is_never_a_project_root(
        ...
        """
        from codex_crew import artifact_source as mod
    ...
    def test_home_is_never_a_project_root(
        ...
        CodexCrew's own data home is ``~/.codex-crew``, so ``~/.kiro`` exists for
...
class TestClassifySourceProjectWalk:
    ...
    def test_git_FILE_worktree_counts_as_repo(self, narrow_tempdir, tmp_path: Path) -> None:
        ...
        # the CodexCrew development layout itself.
        wt = tmp_path / "codexcrew-wt-feature"
        wt.mkdir()
        (wt / ".git").write_text("gitdir: /workplace/user/CodexCrew/.git/worktrees/feature\n")
