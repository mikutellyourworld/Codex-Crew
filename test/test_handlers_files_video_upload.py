...
from codex_crew.dashboard.handlers.files import api_upload_file
...
def upload_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr("codex_crew.dashboard.handlers.files._UPLOAD_DIR", target)
    return target
...
async def test_video_is_exempt_from_the_document_cap(
    ...
    """
    monkeypatch.setattr("codex_crew.dashboard.handlers.files._MAX_UPLOAD_BYTES", 64)
...
async def test_video_over_its_own_cap_is_refused_and_leaves_nothing(
    ...
    """
    monkeypatch.setattr("codex_crew.dashboard.handlers.files._MAX_VIDEO_UPLOAD_BYTES", 512)
