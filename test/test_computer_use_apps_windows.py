...
from codex_crew.computer_use import apps_windows, policy
from codex_crew.computer_use import windows_ffi as ffi
...
class TestTransientPopupsAreNotApplications:
    ...
    def test_a_DENIED_popup_is_still_listed_so_it_can_be_refused(self, monkeypatch) -> None:
        ...
        apps = apps_windows.list_apps()
        assert [app.window_title for app in apps] == ["Codex Crew"]
...
class TestListApps:
    ...
    def test_codexcrews_own_window_is_refused_at_the_ENFORCEMENT_layer(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            ...
            "window_list",
            lambda: (
                _info(hwnd=1, title="Codex Crew", exe="CodexCrew Nightly.exe"),
                _info(hwnd=2, title="Notepad", exe="notepad.exe"),
            ),
        )
        by_title = {a.window_title: a for a in apps_windows.list_apps()}
        assert policy.check_app(by_title["Codex Crew"], PolicyConfig()) is not None
