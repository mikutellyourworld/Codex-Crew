...
from codex_crew.dashboard.handlers.files import api_file_diff
...
async def test_empty_path_returns_empty():
    ...
    req = _req("")
    with patch("codex_crew.dashboard.handlers.files._sel", return_value=_mock_sel()):
...
async def test_sensitive_path_returns_403():
    ...
    with patch("codex_crew.dashboard.handlers.files.is_sensitive_path", return_value=True), \
         patch("codex_crew.dashboard.handlers.files.os.path.isfile", return_value=True), \
         patch("codex_crew.dashboard.handlers.files._sel", return_value=_mock_sel()):
        resp = await api_file_diff(req)
...
async def test_failed_git_diff_reports_error_not_clean(tmp_path):
    ...
    with patch("codex_crew.dashboard.handlers.files._sel", return_value=_mock_sel()), \
         patch("codex_crew.dashboard.handlers.files.subprocess.run", side_effect=failing_diff_run):
        req = _req(str(f))
        resp = await api_file_diff(req)
    assert resp.status == 200
    body = json.loads(resp.body)
...
async def test_timeout_after_preflight_reports_error_not_not_git(tmp_path):
    ...
    with patch("codex_crew.dashboard.handlers.files._sel", return_value=_mock_sel()), \
         patch("codex_crew.dashboard.handlers.files.subprocess.run", side_effect=slow_diff_run):
        req = _req(str(f))
        resp = await api_file_diff(req)
    assert resp.status == 200
    body = json.loads(resp.body)
...
async def test_git_output_is_decoded_as_utf8(tmp_path):
    ...
    with patch("codex_crew.dashboard.handlers.files._sel", return_value=_mock_sel()), \
         patch("codex_crew.dashboard.handlers.files.subprocess.run", side_effect=spy_run):
...
async def test_timeout_returns_not_git(tmp_path):
    ...
    with patch("codex_crew.dashboard.handlers.files._sel", return_value=_mock_sel()), \
         patch("codex_crew.dashboard.handlers.files.subprocess.run", side_effect=timeout_run), \
...
async def test_sel_audit_logging_on_success(tmp_path):
    ...
    mock_sel = _mock_sel()
    with patch("codex_crew.dashboard.handlers.files._sel", return_value=mock_sel), \
         patch("codex_crew.dashboard.handlers.files.is_sensitive_path", return_value=False), \
         patch("codex_crew.dashboard.handlers.files.subprocess.run", side_effect=subprocess.CalledProcessError(1, "git")):
        req = _req(str(f))
        await api_file_diff(req)
