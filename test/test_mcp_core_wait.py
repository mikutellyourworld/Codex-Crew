...
from codex_crew.mcp_core import _call_tool, _post
...
def test_spawn_run_error():
    ...
    with patch("codex_crew.mcp_core._post") as mock_post, \
         patch("codex_crew.mcp_core._resolve_session_key", return_value="dashboard:chat-1"):
...
def test_post_marks_transport_errors_as_uncertain():
    ...
    with patch("codex_crew.mcp_core._resolve_session_key", return_value=""), \
         patch("codex_crew.mcp_core._internal_secret", return_value="secret"), \
         patch("codex_crew.mcp_core.loopback_urlopen", side_effect=TimeoutError("timed out")):
...
def test_post_marks_connection_refusal_as_definite_failure():
    ...
    with patch("codex_crew.mcp_core._resolve_session_key", return_value=""), \
         patch("codex_crew.mcp_core._internal_secret", return_value="secret"), \
         patch("codex_crew.mcp_core.loopback_urlopen", side_effect=refused):
...
class TestSpawnRunApprovalModeForwarding:
    ...
    CODEXCREW_APPROVAL_MODE env var to /api/spawn, so a cron running with
    ...
    def test_forwards_approval_mode_auto_from_env(self):
        with patch("codex_crew.mcp_core._post") as mock_post, \
                patch.dict("os.environ", {"CODEXCREW_APPROVAL_MODE": "auto"}):
            mock_post.return_value = {"id": "abc123"}
    ...
    def test_omits_approval_mode_when_env_unset(self):
        with patch("codex_crew.mcp_core._post") as mock_post, \
            ...
            os.environ.pop("CODEXCREW_APPROVAL_MODE", None)
            mock_post.return_value = {"id": "abc123"}
            _call_tool("spawn_run", {"task": "test task"})
