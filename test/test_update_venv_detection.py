...
class TestVenvPipInstall:
    ...
    async def test_returns_true_on_success(self, monkeypatch, tmp_path) -> None:
        ...
        from codex_crew.dashboard.handlers.updates import _venv_pip_install
    ...
    async def test_substitution_notice_is_progress_not_an_error(
        ...
        def fake_sync(repo, target_py, emit=None, timeout=None):
            emit(r"dep-sync: C:\v\codexcrew.exe locked; substituting", False)
            return 0
...
class TestRestartGateway:
    ...
    async def test_invalid_executable_pushes_error_and_returns(
        ...
        from codex_crew.dashboard.handlers.updates import _restart_gateway
    ...
    async def test_save_history_failure_is_swallowed(
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.chat.save_all_slots_to_history", raising_save
        )
        monkeypatch.setattr("os.execv", lambda *a, **k: execv_called.append(a))
        monkeypatch.setattr(
            "asyncio.sleep", AsyncMock(return_value=None)
    ...
    async def test_session_close_failure_is_swallowed(
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.chat.save_all_slots_to_history", lambda s: None
        )
        monkeypatch.setattr("os.execv", lambda *a, **k: execv_called.append(a))
        monkeypatch.setattr(
            "asyncio.sleep", AsyncMock(return_value=None)
    ...
    async def test_concurrent_restart_is_coalesced_before_session_drain(
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat.save_all_slots_to_history", lambda s: None)
        monkeypatch.setattr("os.execv", lambda *a, **k: execv_called.append(a))
...
class TestApiUpdateApplyVenvDispatch:
    ...
    async def test_pip_path_invokes_pip_install_then_restarts(
        ...
        monkeypatch.setattr("codex_crew.env.is_toolbox_install", lambda: False)
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.handlers.updates._venv_pip_install", fake_pip
        )
        monkeypatch.setattr(
            "codex_crew.dashboard.handlers.updates._restart_gateway", fake_restart
        )
        ...
        from codex_crew.dashboard.handlers.updates import api_update_apply
