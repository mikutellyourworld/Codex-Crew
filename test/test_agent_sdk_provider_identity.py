...
1. **The provider-identity axis** — ``agent.provider``, asked through
   :func:`codex_crew.agent_sdk.provider_identity.is_claude_code`. The eleven
...
from codex_crew.agent_sdk.provider_identity import (
    PROVIDER_ACP,
...
def test_module_is_dependency_free() -> None:
    """No ``codex_crew`` imports, so this module cannot be the one that closes a cycle.
    ...
    assert [m for m in imported if m.startswith("codex_crew")] == []


def test_importing_this_module_does_not_load_the_acp_package() -> None:
    """Importing the predicate must not pull ``codex_crew.acp`` into the process.
    ...
    load is ``codex_crew.acp`` itself, which stays out only because
    ...
    probe = (
        ...
        "loaded = {\n"
        "    'backend_install': 'codex_crew.agent_sdk.backend_install' in sys.modules,\n"
        "    'drivers_acp': 'codex_crew.agent_sdk.drivers.acp' in sys.modules,\n"
    ...
    assert loaded["acp"] is False, (
        "importing agent_sdk.provider_identity loaded codex_crew.acp; "
