...
from codex_crew.config.loader import ACTIVATION_ALWAYS, CodexCrewConfig, MessagingConfig
from codex_crew.messaging.outbound_files import ExtractLimits, OutboundFile, Rejection
...
from codex_crew.slack.format import SLACK_MSG_LIMIT, TRUNCATION_NOTICE
from codex_crew.slack.handler import _THINKING
from codex_crew.slack.renderer import SlackRenderer
...
def _quiet_sel():
    ...
    fake = MagicMock()
    with patch("codex_crew.slack.renderer.sel", return_value=fake):
...
class TestOutboundUploads:
    ...
    async def test_an_oversize_file_is_refused_visibly(self, tmp_path) -> None:
        ...
        with patch("codex_crew.slack.renderer.UPLOAD_LIMITS", tiny):
...
class TestFenceSafeSplitting:
    ...
    async def test_the_splitter_runs_off_the_event_loop(self) -> None:
        ...
        slack = _FakeSlack(streaming=False)
        renderer = _renderer(slack)
        with patch("codex_crew.slack.renderer.split_markdown_safe", _record):
            await _turn(renderer, _fenced_answer())
...
async def _route_voice(
    ...
    with patch("codex_crew.slack.events.is_allowed_user", return_value=True):
        with patch("codex_crew.slack.events.stt_available", return_value=available):
            with patch(
...
class TestVoiceMemoRejections:
    ...
    async def test_the_notes_read_exactly_as_the_other_channels_do(self, monkeypatch) -> None:
        ...
        from codex_crew.messaging.attachments import IngestResult, transcribe_audio_attachments
