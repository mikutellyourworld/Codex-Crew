...
from codex_crew.config import config_path
from codex_crew.config.loader import DEFAULT_MODEL, CodexCrewConfig, resolve_effective_model
...
def shipped(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    def _write(model: str | None) -> Path:
        ...
        monkeypatch.setattr("codex_crew.agent._shipped_defaults", lambda: path)
        return path
...
class TestResolverOracle:
    ...
    def test_round_trip_concrete_then_auto_reports_auto(
        ...
        specs.mkdir()
        monkeypatch.setattr("codex_crew.config.loader.kiro_agents_dir", lambda: specs)
...
def _load_config() -> CodexCrewConfig:
