...
from codex_crew.dashboard.state import DashboardState, _ChatSlot
...
def _make_app(state):
    ...
    from codex_crew.dashboard.handlers import api_lessons_create, api_lessons_delete
...
class TestConsolidation:
    def test_consolidation_not_triggered_for_restricted(self, tmp_path):
        ...
        from codex_crew.dashboard.chat import _maybe_consolidate

        state = _make_state(tmp_path)
        state.consolidator = MagicMock()
        slot = state.get_or_create_slot("e1", memory_mode="incognito")

        with patch("codex_crew.dashboard.chat_utils.sel") as mock_sel:
...
class TestSlotAPI:
    ...
    async def test_create_incognito_slot_via_api(self, tmp_path, monkeypatch):
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_persistence.CodexCrewConfig.load",
...
class TestLessonsGate:
    ...
    async def test_learn_add_allowed_for_persistent_session(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        monkeypatch.setattr(
            "codex_crew.dashboard.handlers._get_memory",
    ...
    async def test_learn_add_allowed_for_channel_namespace_session(self, tmp_path, monkeypatch):
        ...
        resolves the ``X-Session-Key`` (e.g. ``telegram:codexcrew:forum:…``), but
        ...
        for channel_key in (
            "telegram:codexcrew:forum:-1004326574849:18:gen3",
            "discord:codexcrew:dm:123456789:gen1",
            "webex:codexcrew:dm:user@example.com",
            "wecom:codexcrew:dm:wuser",
        ):
            async with TestClient(TestServer(_make_app(state))) as client:
                resp = await client.post(
                    "/api/lessons",
...
class TestMcpCoreSessionKeyPassthrough:
    def test_learn_add_sends_session_key_header(self):
        with (
            patch("codex_crew.mcp_core.loopback_urlopen") as mock_urlopen,
            patch.dict("os.environ", {"CODEXCREW_SESSION_KEY": "dashboard:e1"}),
        ):
            mock_resp = MagicMock()
            ...
            from codex_crew.mcp_core import _post
    ...
    def test_learn_add_no_session_key_header_when_unset(self):
        with (
            patch("codex_crew.mcp_core.loopback_urlopen") as mock_urlopen,
            patch("codex_crew.mcp_core._resolve_session_key", return_value=""),
        ):
            mock_resp = MagicMock()
    ...
    def test_post_surfaces_http_error_json_body(self):
        ...
        with (
            patch("codex_crew.mcp_core.loopback_urlopen") as mock_urlopen,
            patch("codex_crew.mcp_core._resolve_session_key", return_value="1781215864.487849"),
        ):
    ...
    def test_learn_add_tool_maps_unknown_session_to_friendly_message(self):
        ...
        with (
            ...
            from codex_crew.mcp_core import _call_tool_inner
...
class TestSoftGatePrompt:
    def test_incognito_prefix_injected(self):
        from codex_crew.dashboard.chat import _apply_incognito_prefix
...
class TestSessionSlotRecovery:
    ...
    subprocess holds a stale CODEXCREW_SESSION_KEY env var that maps to a swept
    slot.
    """

    @pytest.fixture(autouse=True)
    def _persisted_history_dir_tracks_patched_home(self, monkeypatch):
        """Point ``_shared.config_dir()`` at ``<patched home>/.codexcrew``.

        The data home moved from ``~/.codexcrew`` to ``~/.codex-crew``
        (``config_dir()``); ``_session_has_persisted_history`` now probes
        ``config_dir()/sessions`` rather than ``Path.home()/".codexcrew"/"sessions"``.
        Each test here patches ``Path.home()`` and seeds JSONL under
        ``<home>/.codexcrew/sessions``, but ``config_dir()`` reads ``CODEXCREW_HOME``
        (a different tmp dir pinned by conftest), so redirect ``_shared``'s
        ``config_dir`` to ``Path.home()/".codexcrew"`` (lazy, tracks the per-test
        ``Path.home`` patch) to keep the seeded layout authoritative. Applied
        first so a test's own patches still win.
        """
        monkeypatch.setattr(
            "codex_crew.dashboard.handlers._shared.config_dir",
            lambda: Path.home() / ".codexcrew",
        )

    def _write_sessions_jsonl(self, tmp_path, stem: str) -> None:
        sess_dir = tmp_path / ".codexcrew" / "sessions"
    ...
    async def test_learn_add_rejects_forged_cron_key_without_jsonl(
        ...
        (tmp_path / ".codexcrew" / "sessions").mkdir(parents=True, exist_ok=True)
    ...
    async def test_learn_add_rejects_path_traversal_in_cron_slot_name(
        ...
        # "../escape" → sess_dir/../escape.jsonl → ~/.codexcrew/escape.jsonl
        (tmp_path / ".codexcrew" / "escape.jsonl").write_text("{}\n")
    ...
    def test_session_has_persisted_history_unit(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("pathlib.Path.home", lambda: tmp_path)
        from codex_crew.dashboard.handlers._shared import _session_has_persisted_history
    ...
    async def test_learn_add_audits_live_slot_allow_path(self, tmp_path, monkeypatch):
        ...
        with patch("codex_crew.dashboard.handlers.cron._sel") as mock_sel:
            async with TestClient(TestServer(_make_app(state))) as client:
                resp = await client.post(
                    "/api/lessons",
                    json={"rule": "remember this", "category": "knowledge"},
...
class TestArchivedRestrictedSessionRecovery:
    ...
    async def test_learn_add_denied_for_archived_restricted_session(
        ...
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        monkeypatch.setattr("codex_crew.dashboard.handlers._shared.config_dir", lambda: tmp_path)
        from codex_crew.dashboard.handlers._shared import _session_has_persisted_history
    ...
    def test_persisted_memory_mode_reader(self, tmp_path, monkeypatch, mode, expected):
        monkeypatch.setattr("codex_crew.dashboard.handlers._shared.config_dir", lambda: tmp_path)
        from codex_crew.dashboard.handlers._shared import _persisted_session_memory_mode
    ...
    def test_whitespace_or_case_variant_modes_do_not_fail_open(
        ...
        monkeypatch.setattr("codex_crew.dashboard.handlers._shared.config_dir", lambda: tmp_path)
        from codex_crew.dashboard.handlers._shared import _persisted_session_memory_mode
        from codex_crew.history import INCOGNITO_MEMORY_MODES
...
class TestDurableSlackFlagsAtHttpGate:
    ...
    def durable(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.session_map._KIRO_SESSIONS_DIR", tmp_path / "kiro")
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        monkeypatch.setattr("codex_crew.dashboard.handlers._shared.config_dir", lambda: tmp_path)
        from codex_crew.slack import handler as _h
...
class TestLessonsDeleteGate:
    ...
    def _persisted_history_dir_tracks_patched_home(self, monkeypatch):
        ...
        ``<home>/.codexcrew/sessions`` layout authoritative for the probe."""
        monkeypatch.setattr(
            "codex_crew.dashboard.handlers._shared.config_dir",
            lambda: Path.home() / ".codexcrew",
        )
    ...
    def _deletable_state(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.handlers.cron._get_memory",
            MagicMock(return_value=MagicMock(vector_store=None)),
        )
        state = _make_state(tmp_path)
    ...
    async def test_delete_allowed_for_channel_namespace(self, tmp_path, monkeypatch):
        ...
        async with TestClient(TestServer(_make_app(state))) as client:
            resp = await client.delete(
                "/api/lessons",
                json={"rule": "x"},
                headers={"X-Session-Key": "telegram:codexcrew:forum:-1004326574849:18:gen3"},
            )
            assert resp.status == 200
...
class TestSharedRecognitionGate:
    ...
    async def test_create_and_delete_call_the_same_gate(self, tmp_path, monkeypatch):
        from codex_crew.dashboard.handlers import cron as cron_handlers
        from codex_crew.history import is_incognito_transcript
    ...
    async def test_delete_unknown_session_carries_machine_code(
        ...
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        monkeypatch.setattr(
            "codex_crew.dashboard.handlers._shared.config_dir", lambda: tmp_path
        )
    ...
    def test_http_error_body_preserves_machine_code(self):
        ...
        from codex_crew.mcp_core import _http_error_body
    ...
    def test_learn_remove_dispatches_on_code_not_wording(self, monkeypatch):
        ...
        from codex_crew.mcp_tools.learn import learn_remove

        monkeypatch.setattr(
            "codex_crew.mcp_core._delete",
...
class TestMemoryRoutesSessionGate:
    ...
    def _memory_state(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.handlers.memory._get_vector_store",
