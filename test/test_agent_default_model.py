...
The default model is stored per CodexCrew agent (``agents.<name>.model`` in
config.json), NOT on the kiro agent spec files under ``~/.kiro/agents`` — several
CodexCrew agents can bind the same spec, and CodexCrew regenerates most of those
...
Precedence under test, highest first:
  ...
  2. the bound kiro agent's pinned ``model`` (not for the built-in "codexcrew")
...
class TestNonStringModelInConfig:
    ...
    def test_non_string_model_loads_as_inherit(self, bad: object) -> None:
        cfg = _load_from_dict(
            {
                "agents": {"oncall": {"kiro_agent": "codexcrew", "model": bad}},
    ...
    def test_resolution_survives_a_non_string_model(self) -> None:
        ...
        cfg = _load_from_dict(
            {
                "agents": {"oncall": {"kiro_agent": "codexcrew", "model": 123}},
                "default_agent": "oncall",
            }
        )
...
class TestApiDoesNotStringifyModels:
    ...
    def _handler_source() -> str:
        ...
        from codex_crew.dashboard.handlers import agents as agents_mod
...
class TestPerAgentModelStorage:
    ...
    def test_model_parsed_from_config(self) -> None:
        cfg = _load_from_dict(
            {
                "agents": {"oncall": {"kiro_agent": "codexcrew", "model": "claude-opus-5"}},
                "default_agent": "oncall",
            }
        )
    ...
    def test_model_defaults_to_inherit_when_absent(self) -> None:
        ...
        cfg = _load_from_dict(
            {"agents": {"oncall": {"kiro_agent": "codexcrew"}}, "default_agent": "oncall"}
        )
        assert cfg.agents["oncall"].model == ""
    ...
    def test_null_model_coerces_to_empty_string(self) -> None:
        cfg = _load_from_dict(
            {
                "agents": {"oncall": {"kiro_agent": "codexcrew", "model": None}},
                "default_agent": "oncall",
            }
        )
        assert cfg.agents["oncall"].model == ""
    ...
    def test_bindings_normalize_an_auto_pin(self) -> None:
        ...
        cfg = _load_from_dict(
            {
                "agents": {"oncall": {"kiro_agent": "codexcrew", "model": "auto"}},
                "default_agent": "oncall",
            }
        )
        assert resolve_agent_bindings(cfg, "oncall").model == ""
    ...
    def test_two_agents_on_one_template_hold_distinct_models(self) -> None:
        ...
        cfg = _load_from_dict(
            {
                "agents": {
                    "a": {"kiro_agent": "codexcrew", "model": "claude-opus-5"},
                    "b": {"kiro_agent": "codexcrew", "model": "claude-sonnet-4.6"},
                },
                "default_agent": "a",
            }
        )
...
def specs_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr("codex_crew.config.loader.kiro_agents_dir", lambda: d)
    return d


def _cfg(agents: dict, global_model: str) -> CodexCrewConfig:
    cfg = _load_from_dict({"agents": {"seed": {}}, "default_agent": "seed"})
    cfg.agent.model = global_model
    cfg.agents = {n: CodexCrewAgentConfig(**a) for n, a in agents.items()}
...
class TestEffectiveModelPrecedence:
    ...
    def test_agent_model_outranks_the_global(self, specs_dir: Path) -> None:
        cfg = _cfg({"crew": {"kiro_agent": "codexcrew", "model": "claude-opus-5"}}, "claude-haiku-4.5")
    ...
    def test_global_is_the_fallback_when_nothing_pins(self, specs_dir: Path) -> None:
        cfg = _cfg({"crew": {"kiro_agent": "codexcrew", "model": ""}}, "claude-haiku-4.5")
    ...
    def test_global_reaches_a_named_template_that_pins_nothing(self, specs_dir: Path) -> None:
        """Previously the global was skipped entirely for non-codexcrew templates,
    ...
    def test_auto_global_is_never_returned_verbatim(self, specs_dir: Path) -> None:
        ...
        cfg = _cfg({"crew": {"kiro_agent": "codexcrew", "model": ""}}, "auto")
    ...
    def test_unknown_agent_falls_back_to_the_default_agent(self, specs_dir: Path) -> None:
        cfg = _cfg({"crew": {"kiro_agent": "codexcrew", "model": "claude-opus-5"}}, "")
...
class TestSessionModelCoversEverySurface:
    ...
    resolved kiro template name; Slack threads and cron jobs pass a CodexCrew
    agent name), so both namespaces must work.
    """

    def test_crew_name_resolves_its_own_model(self, specs_dir: Path) -> None:
        cfg = _cfg({"oncall": {"kiro_agent": "codexcrew", "model": "claude-opus-5"}}, "claude-haiku-4.5")
    ...
    def test_template_name_still_behaves_as_before(self, specs_dir: Path) -> None:
        ...
        cfg = _cfg({"oncall": {"kiro_agent": "codexcrew", "model": "claude-opus-5"}}, "")
