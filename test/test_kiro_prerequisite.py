...
from codex_crew import _process_group_supervisor as supervisor
from codex_crew import kiro_prerequisite as prerequisite_module
from codex_crew import platform_compat
from codex_crew.agent_files import AGENT_FILENAME
from codex_crew.dashboard.chat_handlers import api_chat_slot_create
from codex_crew.dashboard.chat_regenerate import (
    api_chat_slot_edit_resend,
    api_chat_slot_regenerate,
)
from codex_crew.dashboard.chat_rewind import api_chat_slot_rewind
from codex_crew.dashboard.chat_runner import _run_chat
from codex_crew.dashboard.handlers.kiro_prerequisite import (
    api_kiro_prerequisite_repair_specs,
    api_kiro_prerequisite_status,
    api_kiro_prerequisite_update_cli,
)
from codex_crew.dashboard.kiro_readiness import kiro_session_ready
from codex_crew.kiro_cli import resolve_kiro_cli
from codex_crew.kiro_prerequisite import (
...
def _isolate_agent_specs(tmp_path_factory, monkeypatch):
    ...
    live in a MACHINE-WIDE directory that no ``CODEXCREW_HOME`` override isolates,
    ...
    """
    from codex_crew import agent as agent_module
    from codex_crew.agent_files import REQUIRED_KIRO_AGENT_FILES
...
def _agents_dir_never_the_real_home(
    ...
    import codex_crew.kiro_prerequisite as kiro_prerequisite_module
...
class TestKiroPrerequisiteHelpers:
    ...
    def test_launches_packaged_fake_backend_by_the_ordinary_path(
        ...
        import codex_crew.testing as kc_testing
    ...
    def test_anchors_relative_candidate_to_an_absolute_path(
        ...
        # An operator CODEXCREW_KIRO_BIN=./kiro-cli resolves against the GATEWAY's
...
class TestKiroPrerequisiteWorkflow:
    ...
    async def test_identity_probe_runs_against_real_home_like_acp(
        ...
        # tool registry lives in the real home is detected. Only Codex Crew's own
    ...
    async def test_windows_override_that_runs_is_used_for_setup(
        ...
        service = KiroPrerequisiteService(
            ...
            environ={
                ...
                "PATH": "",
                "ProgramFiles": str(tmp_path / "Program Files"),
                "CODEXCREW_KIRO_BIN": str(planted),
            },
            home=tmp_path,
            process_runner=run,
            audit_writer=_no_audit,
    ...
    async def test_missing_windows_override_does_not_shadow_program_files_candidate(
        ...
        service = KiroPrerequisiteService(
            ...
            environ={
                ...
                "PATH": "",
                "ProgramFiles": str(tmp_path / "Program Files"),
                "CODEXCREW_KIRO_BIN": str(missing),
            },
            home=tmp_path,
            process_runner=run,
            audit_writer=_no_audit,
    ...
    async def test_probe_process_routes_planted_binary_through_sandbox(
        ...
        monkeypatch.setattr(
            "codex_crew.kiro_prerequisite.sandboxed_spawn_argv",
            sandbox,
        )
    ...
    async def test_process_timeout_escalates_while_supervisor_anchors_group(
        ...
        monkeypatch.setattr(
            "codex_crew.kiro_prerequisite._prepare_sandboxed_spawn",
            passthrough_sandbox,
        )
        monkeypatch.setattr(
            "codex_crew.kiro_prerequisite._TERMINATION_GRACE_SECS",
            0.001,
        )
...
class TestKiroPrerequisiteHandlers:
    ...
    async def test_dashboard_user_reads_status_and_no_setup_verb_exists(
        ...
        # The owner can READ readiness. Neither setup step is a Codex Crew verb:
    ...
    async def test_stale_not_ready_does_not_park_synthesis(
        ...
        from codex_crew.dashboard.state import SUBAGENT_SYNTHESIS_PROMPT
        from codex_crew.providers.base import EVENT_COMPLETE, EVENT_TEXT_CHUNK, LLMEvent
...
class TestTimedOutProbeIsNotAMissingBinary:
    ...
    async def test_a_persistent_timeout_warns_once_not_once_per_probe(
        ...
        with caplog.at_level("DEBUG", logger="codex_crew.kiro_prerequisite"):
            for _ in range(6):
                status = await service.snapshot(force=True)
...
class TestCodexCrewNeverSetsUpKiroCli:
    """Codex Crew DETECTS Kiro CLI. It never installs it and never signs in.
...
class TestAgentSpecsNarrowReadiness:
    ...
    Codex Crew's own agent specs (``~/.kiro/agents/codexcrew*.json``) were not an
...
class TestRejectedAgentSpecsNarrowReadiness:
    ...
    ``--agent codexcrew`` resolves to the default agent with none of Codex Crew's
    ...
    * output that is NOT a schema rejection (a sandbox denial, a vanished file)
    ...
    async def test_a_rejected_required_spec_blocks_readiness_and_names_the_reason(
        ...
        (agents / "codexcrew.json").write_text("{}", encoding="utf-8")
        reason = (
            f"Error: Json supplied at {agents / 'codexcrew.json'} is invalid: "
        ...
        assert status["rejected_agent_specs"] == ["codexcrew.json"]
        assert status["ready"] is False
        assert status["repair_required"] is True
    ...
    async def test_a_signed_out_cli_is_not_asked_about_specs(
        ...
        """
        spec = self._spec_dir(tmp_path) / "codexcrew.json"
    ...
    async def test_an_mcp_server_with_no_command_is_reported_without_a_spawn(
        ...
        (agents / "codexcrew.json").write_text(
            json.dumps({"name": "codexcrew", "mcpServers": {"broken": {"args": ["x"]}}}),
            encoding="utf-8",
        )
        calls: list[list[str]] = []
    ...
    async def test_a_spec_the_bounded_reader_rejects_fails_open(
        ...
        (agents / "codexcrew.json").symlink_to(real)
        calls: list[list[str]] = []
    ...
    async def test_a_launchable_mcp_server_still_reaches_the_binary(
        ...
        (agents / "codexcrew.json").write_text(
            json.dumps(
                {"name": "codexcrew", "mcpServers": {"ok": {"command": "uvx", "args": []}}}
            ),
            encoding="utf-8",
        )
        calls: list[list[str]] = []
    ...
    async def test_a_rejected_spec_is_re_probed_but_never_rewritten(
        ...
        await service.snapshot(force=True)
        assert service._status.rejected_agent_specs == ["codexcrew.json"]
    ...
    async def test_repair_declines_when_nothing_is_wrong(
        ...
        agents = self._spec_dir(tmp_path)
        (agents / "codexcrew.json").write_text("{}", encoding="utf-8")
        (agents / "codexcrew-lite.json").write_text("{}", encoding="utf-8")
        rebuilt: list[str] = []
...
class TestAgentSpecRepair:
    ...
    async def test_silent_no_op_rebuild_is_reported_as_a_failure(
        ...
        assert "codexcrew setup --agent-only --clean" in status["agent_spec_repair_error"]
        assert status["ready"] is False
    ...
    async def test_does_not_rebuild_over_a_present_main_spec(
        ...
        """A present codexcrew.json is never regenerated by this route.
        ...
        """
        from codex_crew import agent as agent_module
        from codex_crew.agent_files import LITE_AGENT_FILENAME

        agents = self._agents_dir(tmp_path, monkeypatch)
        (agents / AGENT_FILENAME).write_text('{"name": "codexcrew"}', encoding="utf-8")
        calls: list[int] = []
        monkeypatch.setattr(
            agent_module, "rebuild_agent_config", lambda: calls.append(1)
        )
