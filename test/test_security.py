...
from codex_crew import cron_inflight, security
from codex_crew.security import (
    _SECRET_KEY_LEN,
...
class TestRedactCredentials:
    ...
    def test_link_token_signature_is_43_chars(self) -> None:
        ...
        """
        from codex_crew.dashboard.token_auth import _sign
    ...
    def test_link_token_payload_clears_the_96_char_floor(self) -> None:
        ...
        from codex_crew.dashboard.token_auth import generate_token
        from codex_crew.security import _CREDENTIAL_PATTERNS
...
class TestSecretGateOrderIsVerdictNeutral:
    ...
    SOURCES = (
        ...
        "src/codex_crew/security/redaction/Handler2/Manager3/Factory4/Builder5x",
...
class TestShannonEntropyIsBitIdentical:
    ...
    :func:`~codex_crew.security._looks_like_secret_key`, so it decides whether a
...
class TestSandboxDeniedCommands:
    ...
    enforced solely at CodexCrew's own ``hooks.py`` PreToolUse gate, whose
    ...
    def _is_denied(cmd: str) -> bool:
        from codex_crew.security import is_denied
...
class TestKiroCliBundledDeniedCommands:
    ...
    (``self-protection-kill``) enforced at CodexCrew's own ``hooks.py`` PreToolUse
    ...
    Regression tests for the ``kill``/``codexcrew`` pattern false positive,
    narrowed in two steps.

    Step 1 (word boundaries): the original pattern ``.*kill.*kiro.?crew.*``
    matched any command whose argv contained ``~/.codexcrew/skills/...``
    (because ``skills`` contains the substring ``kill``) followed by
    ``codexcrew`` anywhere.  Anchoring the kill word on word boundaries
    ...
    *mention* the product anywhere, in any role (a file being restored, a log
    ...
    def test_pkill_codexcrew_blocked(self) -> None:
        assert self._is_denied("pkill codexcrew")

    def test_kill_codexcrew_pid_blocked(self) -> None:
        assert self._is_denied("kill -9 $(pgrep codexcrew)")

    def test_killall_codexcrew_blocked(self) -> None:
        assert self._is_denied("sudo killall codexcrew")

    def test_kill_codex_crew_hyphenated_blocked(self) -> None:
    ...
    def test_kill_pidof_substitution_blocked(self) -> None:
        ...
        assert self._is_denied("kill $(pidof codexcrew)")

    def test_kill_pidfile_substitution_blocked(self) -> None:
        assert self._is_denied("kill $(cat /var/run/codexcrew.pid)")

    def test_kill_backtick_substitution_blocked(self) -> None:
        assert self._is_denied("kill `pgrep codexcrew`")

    # --- skill-dir false positives: must be allowed ---

    def test_skill_create_sh_codexcrew_domain_allowed(self) -> None:
        """The brazil-workspace skill scaffold must not be blocked."""
        cmd = "/Users/user/.codexcrew/skills/brazil-workspace/create.sh --domain codexcrew"
        assert not self._is_denied(cmd)

    def test_skills_dir_listing_allowed(self) -> None:
        assert not self._is_denied("ls ~/.codexcrew/skills/")

    def test_skill_run_with_codexcrew_arg_allowed(self) -> None:
        cmd = "/Users/user/.codexcrew/skills/coder/run.sh codexcrew --dry-run"
        assert not self._is_denied(cmd)

    def test_bash_skill_script_allowed(self) -> None:
        assert not self._is_denied("bash ~/.codexcrew/skills/something.sh")

    def test_cat_codexcrew_config_allowed(self) -> None:
        # "cat" has no "kill" word anywhere — must not match.
        assert not self._is_denied("cat ~/.codexcrew/config.json")

    # --- incidental-mention false positives: must be allowed ---
    # A bare `kill` takes PIDs, so none of these can aim at a codexcrew process
    ...
    def test_kill_pid_then_restore_config_file_allowed(self) -> None:
        cmd = "kill 12345 && cp /tmp/bk/codexcrew.json ~/.kiro/agents/"
        assert not self._is_denied(cmd)

    def test_kill_pid_then_diff_config_file_allowed(self) -> None:
        cmd = "kill $PID; diff /tmp/bk/codexcrew.json ~/.kiro/agents/codexcrew.json"
        assert not self._is_denied(cmd)

    def test_kill_pid_with_trailing_comment_allowed(self) -> None:
        assert not self._is_denied("kill $PID  # stop the stray codexcrew instance")

    def test_kill_pid_piped_to_codexcrew_log_allowed(self) -> None:
        assert not self._is_denied("kill 12345 | tee /tmp/codexcrew.log")
...
class TestBuiltinDenyPatterns:
    ...
    def test_two_pass_evaluates_all_deny_patterns(self, monkeypatch) -> None:
        ...
        """
        import codex_crew.security as security_module
...
class TestOAuthAuthorizationUrlRedaction:
    ...
    def test_unrecognized_parameter_name_is_omitted_from_diagnostic_and_log(
        ...
        assert diagnostic.rule == "exfil_query_length"
        assert diagnostic.parameter is None
        with caplog.at_level("WARNING", logger="codex_crew.security"):
            assert oauth_url_contains_credential(url) is True
    ...
    def test_credential_surviving_the_decode_budget_fails_closed(self) -> None:
        ...
        from codex_crew.security import _MAX_URL_DECODE_PASSES
...
class TestOperatorOAuthEndpointExtension:
    ...
    def test_missing_file_yields_empty_set(self, ext_home: Path) -> None:
        from codex_crew.security import _load_operator_oauth_endpoints
    ...
    @pytest.mark.parametrize("prefix", [".codex-crew", ".codexcrew"])
    def test_extension_file_is_sensitive_under_every_home_prefix(self, prefix: str) -> None:
        from codex_crew.security import is_sensitive_write_path
    ...
    def test_bash_write_and_read_both_blocked(self) -> None:
        for cmd in (
            "echo x > ~/.codex-crew/oauth_endpoints.json",
            "tee ~/.codex-crew/oauth_endpoints.json",
            "cp evil ~/.codex-crew/oauth_endpoints.json",
            "cat ~/.codex-crew/oauth_endpoints.json",
            "cat ~/.codexcrew/oauth_endpoints.json",
        ):
...
class TestRedactExfiltrationUrls:
    ...
    def test_external_long_query_redacted(self) -> None:
        ...
        from codex_crew.security import redact_exfiltration_urls
    ...
    def test_s3_presigned_url_scan_clean(self) -> None:
        ...
        from codex_crew.security import scan_exfiltration_urls
...
class TestExfilExactHostExemption:
    ...
    class _StubCredentialPolicy:
        ...
        def redact(self, text: str) -> str:
            from codex_crew.security import redact
    ...
    def _install_exempt_hosts(self, hosts: "frozenset[str]") -> None:
        ...
        from codex_crew.platform.context import set_context

        base = build_default_context(CodexCrewConfig())
    ...
    def test_mixed_case_exempted_host_preserved(self) -> None:
        ...
        from codex_crew.security import redact_exfiltration_urls, scan_exfiltration_urls
    ...
    def test_unbooted_path_does_no_context_resolution(self) -> None:
        ...
        from codex_crew.platform import context as context_mod
        from codex_crew.platform.context import reset_context
        from codex_crew.security import redact

        calls: list[str] = []
        real_current = context_mod.current_context
        real_load = CodexCrewConfig.load

        with _pytest.MonkeyPatch.context() as mp:
            mp.setenv("CODEXCREW_PROFILE", "enterprise")
            reset_context()
            ...
            mp.setattr(CodexCrewConfig, "load", _spy_load)
            try:
    ...
    def test_composition_error_degrades_to_full_redaction(self) -> None:
        ...
        from codex_crew.config import CodexCrewConfig
        from codex_crew.platform.bootstrap import build_default_context
        from codex_crew.platform.context import PlatformCompositionError, set_context
        from codex_crew.security import redact_exfiltration_urls
    ...
    def test_unbooted_nonstandalone_profile_still_redacts(self) -> None:
        ...
        ``CODEXCREW_PROFILE=enterprise`` ``current_context()`` fail-closes, and
        ...
        from codex_crew.platform.context import (
...
class TestIsSensitivePath:
    ...
    def test_codexcrew_env(self) -> None:
        # The data home moved to ~/.codex-crew; the legacy ~/.codexcrew stays gated
        # (migration leaves a rollback copy that still holds real secret bytes).
        assert is_sensitive_path("~/.codex-crew/.env") is True
        assert is_sensitive_path("~/.codexcrew/.env") is True
    ...
    def test_browser_auth_cookie_paths(self) -> None:
        ...
        home = str(Path.home())
        assert is_sensitive_path("~/.codex-crew/browser-cookies.txt") is True
        assert is_sensitive_path("~/.codex-crew/playwright-storage-state.json") is True
        assert is_sensitive_path(f"{home}/.codex-crew/browser-cookies.txt") is True
        assert is_sensitive_path(f"{home}/.codex-crew/playwright-storage-state.json") is True
        # Legacy pre-move home is still gated.
        assert is_sensitive_path("~/.codexcrew/browser-cookies.txt") is True
        assert is_sensitive_path(f"{home}/.codexcrew/playwright-storage-state.json") is True
    ...
    def test_sel_hmac_key(self) -> None:
        ...
        assert is_sensitive_path("~/.codex-crew/sel_hmac.key") is True
        assert is_sensitive_path("~/.codexcrew/sel_hmac.key") is True
        assert is_sensitive_path("~/.codex-crew/trust") is True
        assert is_sensitive_path("~/.codex-crew/trust/sel_hmac.key") is True
        assert is_sensitive_path("~/.codexcrew/trust") is True
        assert is_sensitive_path("~/.codexcrew/trust/sel_hmac.key") is True
    ...
    def test_security_events_log(self) -> None:
        ...
        assert is_sensitive_path("~/.codex-crew/security_events.jsonl") is True
        assert is_sensitive_path("~/.codexcrew/security_events.jsonl") is True
    ...
    def test_rotated_security_event_segments(self) -> None:
        ...
        assert is_sensitive_path("~/.codex-crew/security_events.d") is True
        assert (
            is_sensitive_path(
                "~/.codex-crew/security_events.d/security_events-000001-20260821T045139Z.jsonl"
            )
            is True
        )
        assert is_sensitive_path("~/.codexcrew/security_events.d") is True
        assert (
            is_sensitive_path(
                "~/.codexcrew/security_events.d/security_events-000001-20260821T045139Z.jsonl"
            )
            is True
        )
    ...
    def test_sel_files_absolute_path(self) -> None:
        home = str(Path.home())
        assert is_sensitive_path(f"{home}/.codex-crew/sel_hmac.key") is True
        assert is_sensitive_path(f"{home}/.codex-crew/trust/sel_hmac.key") is True
        assert is_sensitive_path(f"{home}/.codex-crew/security_events.jsonl") is True
        assert is_sensitive_path(f"{home}/.codexcrew/sel_hmac.key") is True
        assert is_sensitive_path(f"{home}/.codexcrew/trust/sel_hmac.key") is True
        assert is_sensitive_path(f"{home}/.codexcrew/security_events.jsonl") is True
    ...
    def test_app_admission_policy(self) -> None:
        ...
        home = str(Path.home())
        assert is_sensitive_path("~/.codex-crew/app_admission.json") is True
        assert is_sensitive_path(f"{home}/.codex-crew/app_admission.json") is True
        assert is_sensitive_path("~/.codexcrew/app_admission.json") is True
    ...
    def test_token_signing_key(self) -> None:
        ...
        # HMAC key above.
        assert is_sensitive_path("~/.codex-crew/token_signing.key") is True
        assert is_sensitive_path("~/.codexcrew/token_signing.key") is True
    ...
    def test_refresh_chains_json(self) -> None:
        ...
        assert is_sensitive_path("~/.codex-crew/refresh_chains.json") is True
        assert is_sensitive_path("~/.codexcrew/refresh_chains.json") is True
    ...
    def test_local_secret(self) -> None:
        ...
        assert is_sensitive_path("~/.codex-crew/.local_secret") is True
        assert is_sensitive_path("~/.codexcrew/.local_secret") is True

    def test_kiro_cli_binary_attestation(self) -> None:
        assert is_sensitive_path("~/.codex-crew/.kiro_cli_binary_trust.json") is True
        assert is_sensitive_path("~/.codexcrew/.kiro_cli_binary_trust.json") is True

    def test_kiro_auth_staging_parent(self) -> None:
        assert is_sensitive_path("~/.codex-crew-auth-staging") is True
        assert is_sensitive_path("~/.codex-crew-auth-staging/auth-123/token.json") is True

    def test_dashboard_secrets_absolute_path(self) -> None:
        home = str(Path.home())
        assert is_sensitive_path(f"{home}/.codex-crew/token_signing.key") is True
        assert is_sensitive_path(f"{home}/.codex-crew/refresh_chains.json") is True
        assert is_sensitive_path(f"{home}/.codex-crew/.local_secret") is True
        assert is_sensitive_path(f"{home}/.codexcrew/token_signing.key") is True
        assert is_sensitive_path(f"{home}/.codexcrew/refresh_chains.json") is True
        assert is_sensitive_path(f"{home}/.codexcrew/.local_secret") is True
    ...
    def test_non_sel_crew_file_not_blocked(self) -> None:
        ...
        assert is_sensitive_path("~/.codex-crew/config.json") is False
        assert is_sensitive_path("~/.codex-crew/sessions.db") is False
        assert is_sensitive_path("~/.codexcrew/config.json") is False
        assert is_sensitive_path("~/.codexcrew/sessions.db") is False
...
class TestKeystonePublishArtifacts:
    ...
    def test_the_write_gate_stays_a_superset(self) -> None:
        ...
        assert is_sensitive_write_path("~/.codex-crew/tmpAB12CD34.tmp") is True
        assert is_sensitive_write_path("~/.codex-crew/.policy.lock") is True
    ...
    @pytest.mark.parametrize(
        "command",
        [
            "cat ~/.codex-crew/tmpAB12CD34.tmp",
            "cat ~/.codex-crew/.policy.lock",
            "cat ~/.codex-crew/computer_use.json.tmp",
            "cat ~/.codex-crew/ops_mission_control_secrets.json.lock",
            "cat $HOME/.codexcrew/tmpAB12CD34.tmp",
            # Verb-independent: a redirect and a copy are caught without enumerating
            # write verbs.
            "echo pwned > ~/.codex-crew/tmpAB12CD34.tmp",
            "cp /tmp/evil ~/.codex-crew/tmpAB12CD34.tmp",
            # Windows-native spellings, which the POSIX tokenizing passes cannot see.
            r"type C:\Users\u\.codex-crew\tmpAB12CD34.tmp",
            r"type C:\Users\u\.codex-crew\.policy.lock",
            r"type %USERPROFILE%\.codex-crew\tmpAB12CD34.tmp",
            r"python -c \"open(r'C:\Users\u\.codex-crew\tmpAB12CD34.tmp','w')\"",
        ],
    )
    ...
    def test_a_shell_metacharacter_does_not_end_the_fence(self, terminator: str) -> None:
        ...
        """
        posix = f"cat ~/.codex-crew/tmpAB12CD34.tmp{terminator}"
        win = rf"type C:\Users\u\.codex-crew\tmpAB12CD34.tmp{terminator}"
    ...
    def test_the_keystone_leaf_shares_that_terminator(self, terminator: str) -> None:
        ...
        """
        assert (
            is_sensitive_bash_command(rf"type C:\Users\u\.codex-crew\computer_use.json{terminator}")
            is not None
        )
        assert (
            is_sensitive_bash_command(rf"type C:\Users\u\.codex-crew\.env{terminator}") is not None
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            r"Get-Content C:\Users\u\.codex-crew\computer_use.json$null",
            r"type C:\Users\u\.codex-crew\.env$null",
            r"type C:\Users\u\.codex-crew\tmpAB12CD34.tmp$null",
            r"type C:\Users\u\.codex-crew\.policy.lock$null",
            r"Get-Content %USERPROFILE%\.codex-crew\token_signing.key$env:x",
        ],
    )
    ...
    def test_the_dollar_addition_does_not_over_block(self) -> None:
        ...
        assert is_sensitive_bash_command("cat ~/.codex-crew/config.json") is None
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            r"type C:\Users\u\.codex-crew\.\tmpAB12CD34.tmp",
            r"type C:\Users\u\.codex-crew\.\.policy.lock",
            r"type C:\Users\u\.codex-crew\x\..\tmpAB12CD34.tmp",
            "cat ~/.codex-crew/./tmpAB12CD34.tmp",
            "cat ~/.codex-crew/x/../tmpAB12CD34.tmp",
    ...
    def test_the_leaf_branch_covers_the_noop_segment_but_not_the_trailing_dot(self) -> None:
        ...
        accept forward slashes, that refused ``ls -d ~/.codex-crew/backup.tar`` -- a
        ...
        """
        assert (
            is_sensitive_bash_command(r"type C:\Users\u\.codex-crew\.\computer_use.json") is not None
        )
        # The pre-existing gap, asserted so a future change to the terminator is a
        # deliberate decision rather than a surprise.
        assert is_sensitive_bash_command(r"type C:\Users\u\.codex-crew\computer_use.json.") is None
        # ...and the read that constrains it stays allowed.
        assert is_sensitive_bash_command("ls -d ~/.codex-crew/backup.tar") is None
    ...
    def test_the_alias_tolerance_still_rejects_a_different_file(self) -> None:
        ...
        """
        assert is_sensitive_bash_command("cat ~/.codex-crew/tmpAB12CD34.tmpx") is None
        assert is_sensitive_bash_command("cat ~/.codex-crew/tmpAB12CD34.tmp-old") is None
    ...
    def test_the_parent_is_matched_by_equality_not_prefix(self) -> None:
        ...
        """
        assert is_sensitive_path("~/.codex-crew/sub/deeper/x.tmp") is False
        assert is_sensitive_bash_command("cat ~/.codex-crew/sub/deeper/x.tmp") is None
    ...
    def test_a_directory_without_a_keystone_leaf_is_out_of_scope(self) -> None:
        ...
        """
        assert is_sensitive_path("~/.codex-crew/deploy/pending-deploys.lock") is False

    def test_the_leaves_themselves_are_still_fenced(self) -> None:
        """No regression: the artifact clause is additive."""
        assert is_sensitive_path("~/.codex-crew/computer_use.json") is True
        assert is_sensitive_path("~/.codex-crew/security_policy.json") is True
        assert is_sensitive_path("~/.codex-crew/.env") is True
        assert is_sensitive_path("~/.codex-crew/webhooks/tokens.json") is True

    def test_a_relocated_crew_home_is_covered(self, tmp_path, monkeypatch) -> None:
        """CODEXCREW_HOME re-anchoring is inherited, not reimplemented.

        The keystone leaves live directly under a custom ``CODEXCREW_HOME``, so the
        ...
        relocated.mkdir()
        monkeypatch.setenv("CODEXCREW_HOME", str(relocated))
...
class TestHomeDirTargetsCache:
    ...
    def test_codexcrew_home_change_is_not_deferred_by_ttl(self, monkeypatch, tmp_path) -> None:
        """A changed CODEXCREW_HOME must re-key immediately, not after the TTL.

        This is the security-relevant property: the keystone secrets live under
        CODEXCREW_HOME, so a stale target set built for the OLD home would stop
        ...
        monkeypatch.setenv("CODEXCREW_HOME", str(home_a))
        targets_a = set(security._home_dir_targets(security._SENSITIVE_HOME_DIRS))
        monkeypatch.setenv("CODEXCREW_HOME", str(home_b))
    ...
    def test_cache_dict_is_bounded(self, monkeypatch, tmp_path) -> None:
        ...
        self._clear()
        for i in range(200):
            monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path / f"h{i}"))
            security._home_dir_targets(security._SENSITIVE_HOME_DIRS)
...
class TestEnvDumpGrepAwsNarrowing:
    ...
    RESIDUAL_OVER_BLOCK = (
        ...
        "env | grep CODEXCREW && echo AWS_SECRET",
    ...
    ALLOWED = (
        ...
        # No filter at all.
        "env | cut -d= -f1 | sort",
        "docker exec codexcrew printenv CODEXCREW_PORT",
        "printenv | wc -l",
    ...
    def _keystone(cmd: str) -> bool:
        from codex_crew.security import _check_env_credential_access
    ...
    def test_catalog_rule_is_published_not_silently_disabled(self, rule_id: str) -> None:
        ...
        from codex_crew.security import is_safe_user_regex
...
class TestIsSensitiveBashCommand:
    ...
    def test_bare_filename_after_cd_is_resolved_against_the_cd_target(self) -> None:
        ...
        moved to.
        """
        assert is_sensitive_bash_command("cd ~/.codex-crew && cat token_signing.key") is not None
        assert is_sensitive_bash_command("cd ~/.codex-crew; cat token_signing.key") is not None
        ...
        assert (
            is_sensitive_bash_command("cd $HOME/.codex-crew && awk 1 token_signing.key") is not None
        )
    ...
    def test_chained_relative_cd_resolves_against_prior_base(self) -> None:
        ...
        # cd ~/.kiro && cd crew → base should be ~/.codex-crew, not bare "crew"
        assert (
    ...
    def test_quoted_separator_does_not_retarget_the_cd_base(self) -> None:
        ...
        assert (
            is_sensitive_bash_command(
                "cd ~/.codex-crew && echo 'x; cd /tmp' && cat token_signing.key"
            )
            is not None
        )
        assert (
    ...
    def test_separator_in_a_command_substitution_does_not_retarget_the_cd_base(self) -> None:
        ...
        assert (
            is_sensitive_bash_command(
                "cd ~/.codex-crew && echo $(true; cd /tmp) && cat token_signing.key"
            )
            is not None
        )
        assert (
    ...
    def test_assignment_only_counts_as_a_prefix(self) -> None:
        ...
        assert (
            is_sensitive_bash_command(
                "V=$HOME; echo V=/tmp; cd $V/.codex-crew; cat token_signing.key"
            )
            is not None
        )
        assert (
    ...
    def test_cd_dash_returns_to_the_previous_directory(self) -> None:
        ...
        assert (
            is_sensitive_bash_command("cd ~/.codex-crew; cd /tmp; cd -; cat token_signing.key")
            is not None
        )
        assert (
    ...
    def test_parameter_expansion_resolves_like_a_plain_reference(self) -> None:
        ...
        # As a `cd` target.
        assert (
            is_sensitive_bash_command("D=$HOME/.codex-crew; cd ${D:-/tmp}; cat token_signing.key")
            is not None
        )
    ...
    def test_a_masked_substitution_still_shows_the_path_inside_it(self) -> None:
        ...
        Masking keeps `cd "$(printf %s ~)/.codex-crew"` as one token so its tail
        ...
        assert (
            is_sensitive_bash_command('cd "$(printf %s ~)/.codex-crew" && cat token_signing.key')
            is not None
        )
    ...
    def test_an_append_assignment_builds_on_the_recorded_value(self) -> None:
        ...
        `$HOME/.codex-crew`, and the read after the `cd` resolved against the
        wrong directory.
        """
        assert (
    ...
    def test_a_substitutions_own_text_can_name_the_path(self) -> None:
        ...
        `~/crew` — benign — while bash entered `~/.codex-crew` and read the key.
        """
        assert (
    ...
    def test_a_cd_into_a_directory_that_holds_a_secret_taints(self) -> None:
        """`~/.codex-crew` is not sensitive itself — only its leaves are.
        ...
        """
        assert (
            is_sensitive_bash_command('bash -c "cd ~/.codex-crew; cat token_signing.key"')
            is not None
        )
        assert (
            is_sensitive_bash_command("cd ~/.codex-crew; false && cd /tmp; cat token_signing.key")
            is not None
        )
    ...
    def test_a_later_cd_does_not_erase_a_sensitive_one(self) -> None:
        ...
        assert (
            is_sensitive_bash_command(
                "H=$HOME; D=$H/.codex-crew; cd $D; false && cd /tmp; cat token_signing.key"
            )
            is not None
        )
    ...
    def test_a_declaration_builtin_is_an_assignment(self) -> None:
        ...
        """
        assert (
            is_sensitive_bash_command("export D=$HOME/.codex-crew; cd $D; cat token_signing.key")
            is not None
        )
    ...
    def test_an_assignment_keeps_the_operator_form_literal(self) -> None:
        ...
        """
        assert (
            is_sensitive_bash_command("X=x; D=${X:+$HOME/.codex-crew}; cd $D; cat token_signing.key")
            is not None
        )
        assert (
    ...
    def test_the_reserved_placeholder_name_is_refused_in_every_spelling(self) -> None:
        ...
        """
        from codex_crew.security import _SUBST_PLACEHOLDER_NAME, _SUBST_PLACEHOLDER_NAME_RE
    ...
    def test_command_substitution_is_an_unresolved_value(self) -> None:
        ...
        assert (
            is_sensitive_bash_command("D=$(printf %s ~); cd $D/.codex-crew && cat token_signing.key")
            is not None
        )
    ...
    def test_pushd_tracks_directory(self) -> None:
        ...
        assert is_sensitive_bash_command("pushd ~/.codex-crew && cat token_signing.key") is not None
    ...
    def test_cat_sel_hmac_key_blocked(self) -> None:
        ...
        result = is_sensitive_bash_command("cat ~/.codex-crew/sel_hmac.key")
        assert result is not None and "blocked" in result.lower()
        legacy = is_sensitive_bash_command("cat ~/.codexcrew/sel_hmac.key")
        assert legacy is not None and "blocked" in legacy.lower()
        # The key's real home since the trust/ relocation.
        trust = is_sensitive_bash_command("cat ~/.codex-crew/trust/sel_hmac.key")
        assert trust is not None and "blocked" in trust.lower()
        trust_legacy = is_sensitive_bash_command("cat ~/.codexcrew/trust/sel_hmac.key")
        assert trust_legacy is not None and "blocked" in trust_legacy.lower()

    def test_cat_security_events_log_blocked(self) -> None:
        result = is_sensitive_bash_command("cat ~/.codex-crew/security_events.jsonl")
        assert result is not None and "blocked" in result.lower()
        legacy = is_sensitive_bash_command("cat ~/.codexcrew/security_events.jsonl")
        assert legacy is not None and "blocked" in legacy.lower()
    ...
    def test_cat_rotated_security_event_segment_blocked(self) -> None:
        ...
        rotated = is_sensitive_bash_command(
            "cat ~/.codex-crew/security_events.d/security_events-000001-20260821T045139Z.jsonl"
        )
        assert rotated is not None and "blocked" in rotated.lower()
        legacy = is_sensitive_bash_command(
            "cat ~/.codexcrew/security_events.d/security_events-000001-20260821T045139Z.jsonl"
        )
        assert legacy is not None and "blocked" in legacy.lower()
    ...
    def test_write_app_admission_policy_blocked(self) -> None:
        ...
        tee = is_sensitive_bash_command("echo '{}' | tee ~/.codex-crew/app_admission.json")
        assert tee is not None and "blocked" in tee.lower()
        rm = is_sensitive_bash_command("rm -f ~/.codex-crew/app_admission.json")
        assert rm is not None and "blocked" in rm.lower()
        legacy = is_sensitive_bash_command("rm -f ~/.codexcrew/app_admission.json")
        assert legacy is not None and "blocked" in legacy.lower()
    ...
    def test_git_write_verbs_on_sensitive_path_blocked(self) -> None:
        ...
        assert is_sensitive_bash_command("git mv x ~/.codex-crew/profiles/p.json") is not None
        assert is_sensitive_bash_command("git mv x ~/.codexcrew/profiles/p.json") is not None
    ...
    def test_extract_into_trust_root_subdir_blocked(self) -> None:
        ...
        assert is_sensitive_bash_command("tar -xf evil.tar -C ~/.codex-crew/foo/") is not None
        assert is_sensitive_bash_command("unzip -d ~/.codex-crew/foo/ evil.zip") is not None
        assert is_sensitive_bash_command("tar -xf e.tar -C ~/.codex-crew") is not None
        # Legacy pre-move home is still gated.
        assert is_sensitive_bash_command("tar -xf evil.tar -C ~/.codexcrew/foo/") is not None
        assert is_sensitive_bash_command("tar -xf e.tar -C ~/.codexcrew") is not None
    ...
    def test_readonly_listing_of_crew_home_allowed(self) -> None:
        ...
        assert is_sensitive_bash_command("ls -d ~/.codex-crew/skills") is None
        assert is_sensitive_bash_command("ls -d ~/.codex-crew") is None
        assert is_sensitive_bash_command("ls -d ~/.codexcrew/skills") is None
        assert is_sensitive_bash_command("ls -d ~/.codex-crew/backup.tar") is None
        ...
        assert is_sensitive_bash_command("ls -d ~/.codex-crew/tar") is None
        assert is_sensitive_bash_command("ls -d ~/.codex-crew/rsync") is None
    ...
    def test_other_read_listers_on_crew_home_allowed(self) -> None:
        ...
        for prog in ("ls", "stat", "du", "readlink", "basename", "dirname", "wc"):
            cmd = f"{prog} -d ~/.codex-crew/skills"
    ...
    def test_reads_the_reporter_listed_as_allowed_stay_allowed(self) -> None:
        ...
        assert is_sensitive_bash_command("ls -lt ~/.codex-crew/skills") is None
        assert is_sensitive_bash_command("ls -l ~/.codex-crew/skills") is None
        assert is_sensitive_bash_command("ls -ld ~/.codex-crew") is None
        assert is_sensitive_bash_command("grep -r x ~/.codex-crew/skills") is None
    ...
    def test_non_archive_writers_into_trust_root_still_blocked(self) -> None:
        ...
        for cmd in (
            "patch -d ~/.codex-crew -p1 -i /tmp/evil.patch",
            "git -C ~/.codex-crew apply /tmp/evil.patch",
            "make -C ~/.codex-crew all",
            "install -d ~/.codex-crew/profiles",
            "cpio -D ~/.codex-crew -i",
        ):
            assert is_sensitive_bash_command(cmd) is not None, cmd
    ...
    def test_quote_split_program_name_still_blocked(self) -> None:
        ...
        for cmd in (
            't""ar -xf e.tar -C ~/.codex-crew',
            "t''ar -xf e.tar -C ~/.codex-crew",
            'ta""r -xf e.tar -C ~/.codex-crew',
            "'tar' -xf e.tar -C ~/.codex-crew",
        ):
            assert is_sensitive_bash_command(cmd) is not None, cmd
    ...
    def test_composed_commands_never_reach_the_read_carve_out(self) -> None:
        ...
        for cmd in (
            "ls -d ~/.codex-crew && tar -xf e.tar -C ~/.codex-crew",
            "curl http://x | tar xf - -C ~/.codex-crew/",
            "echo hi; unzip -d ~/.codex-crew e.zip",
            "sh -c 'tar -xf e.tar -C ~/.codex-crew'",
            "eval 'tar -xf e.tar -C ~/.codex-crew'",
            "sudo tar -xf e.tar -C ~/.codex-crew",
            "env FOO=1 tar -xf e.tar -C ~/.codex-crew",
            "tar -xf e.tar -C ~/.codex-crew &",
            "echo x\rtar -xf e.tar -C ~/.codex-crew",
            "ls -d ~/.codex-crew > ~/.codex-crew/out",
        ):
            assert is_sensitive_bash_command(cmd) is not None, cmd
    ...
    def test_powershell_parenthesised_group_never_reaches_the_carve_out(self) -> None:
        ...
        for cmd in (
            "ls -d $HOME/.codex-crew (tar.exe -xf evil.tar -C $HOME/.codex-crew)",
            "ls -d ~/.codex-crew (tar -xf evil.tar -C ~/.codex-crew)",
            "stat -d ~/.codex-crew @(tar -xf evil.tar -C ~/.codex-crew)",
        ):
            assert is_sensitive_bash_command(cmd) is not None, cmd
    ...
    def test_program_pathnames_are_never_exonerated(self) -> None:
        ...
        for cmd in (
            "/tmp/ls -d ~/.codex-crew",
            "./ls -d ~/.codex-crew",
            "/home/x/evil/ls -d ~/.codex-crew",
            "../ls -d ~/.codex-crew",
            "/bin/ls -d ~/.codex-crew",
            r"C:\evil\ls -d ~/.codex-crew",
        ):
            assert is_sensitive_bash_command(cmd) is not None, cmd
    ...
    def test_home_variable_is_the_only_dollar_form_exonerated(self) -> None:
        ...
        assert _is_bare_trust_root_read("ls -d $HOME/.codex-crew") is True
        assert _is_bare_trust_root_read("ls -d $HOME") is True
        assert _is_bare_trust_root_read("ls -d ${HOME}/.codex-crew") is False
        assert _is_bare_trust_root_read("ls -d $HOMEX/.codex-crew") is False
        assert _is_bare_trust_root_read("ls -d $(echo ~/.codex-crew)") is False
    ...
    def test_no_write_capable_program_is_ever_a_read_lister(self) -> None:
        ...
        from codex_crew.security import _TRUST_ROOT_READ_LISTERS
    ...
    def test_normal_crew_access_not_overblocked(self) -> None:
        ...
        assert is_sensitive_bash_command("cat ~/.codex-crew/config.json") is None
        assert is_sensitive_bash_command("sqlite3 ~/.codex-crew/sessions.db .tables") is None
        assert is_sensitive_bash_command("cat ~/.codexcrew/config.json") is None
        assert is_sensitive_bash_command("sqlite3 ~/.codexcrew/sessions.db .tables") is None
    ...
    def test_imds_shortform_encodings_blocked(self) -> None:
        from codex_crew.security import _check_imds_access, canonicalize_ip
    ...
    def test_imds_plainform_still_blocked(self) -> None:
        from codex_crew.security import _check_imds_access
    ...
    def test_variable_indirection_denied(self) -> None:
        ...
        cmd = "F=security_policy.json; cat ~/.codex-crew/$F"
...
class TestChdirVerbSpellings:
    ...
    def test_every_chdir_verb_moves_the_base(self, verb: str) -> None:
        ...
        assert security.is_sensitive_bash_command(f"{verb} $HOME; cat .codex-crew/token_signing.key")
    ...
    def test_every_chdir_verb_into_a_fenced_dir_taints_the_read(self, verb: str) -> None:
        ...
        assert security.is_sensitive_bash_command(f"{verb} ~/.codex-crew; cat token_signing.key")
    ...
    def test_cmd_exe_drive_switch_is_not_the_target(self) -> None:
        ...
        be the crew home, and discarding it turned `CODEXCREW_HOME=/d` plus
    ...
    def test_powershell_call_operator_prefix_is_unwrapped(self) -> None:
        ...
        assert security.is_sensitive_bash_command(
            amp + " chdir %USERPROFILE%; type .codex-crew/token_signing.key"
        )
...
class TestNativeHomeEntryThenFencedRead:
    ...
    def test_bare_chdir_lands_in_the_home_directory(self) -> None:
        ...
        assert security.is_sensitive_bash_command(
            "cd " + self.AMP + self.AMP + " cat .codex-crew/token_signing.key"
        )
    ...
    def test_lookalike_directory_names_are_not_fenced(self) -> None:
        ...
        assert security.is_sensitive_bash_command("cd ~; cat .codexcrewnotes") is None
    ...
    def test_longer_filename_ending_in_a_fenced_name(self) -> None:
        ...
        assert security.is_sensitive_bash_command("cd ~; cat my.codex-crew/x") is None
...
class TestKeystoneVariableLeafNativeSpellings:
    ...
    ``~/.codex-crew`` is not fenced as a directory -- only its leaves are -- so a
    read whose filename is a variable (``cat "$HOME/.codex-crew/$F"``) can only be
    ...
    CREW_HOMES = (".codex-crew", ".codexcrew")
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            # let `…\.codex-crew\$(Write-Output security_policy.json)` read the
    ...
    def test_an_absolute_home_spelled_with_backslashes_is_refused(self) -> None:
        ...
        assert security.is_sensitive_bash_command(f"type {home}\\.codexcrew\\$F")
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            'D=$HOME; cat "$D/.codex-crew/$F"',
            'export D=$HOME; type "$D\\.codexcrew\\$F"',
    ...
    @pytest.mark.parametrize(
        "command",
        (
            'cat "$UNKNOWN/.codex-crew/$F"',
            'cat "$UNKNOWN\\.kiro\\crew\\$F"',
            'cat "$(get_home)/.codex-crew/$F"',
            'cat "${SOMEDIR}/.codexcrew/$F"',
        ),
    )
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            "cat ~/.codex-crew/skills/$NAME/SKILL.md",
            "cat ~/.codex-crew/workspace/$PROJ/notes.md",
            ...
            "cat ~/.codex-crew/skills/$(ls)/SKILL.md",
            "echo ${My Var}",
...
class TestWindowsPathShapes:
    ...
    def test_cross_drive_forward_slash_token_stays_path_like(self) -> None:
        # CODEXCREW_HOME may legitimately live on another drive, and its
        ...
        with patch.object(security.Path, "home", return_value=Path("C:\\Users\\u")):
            assert security._is_path_like("D:/codexcrew/security_policy.json")

    def test_codexcrew_home_drive_anchors_backslash_recognition(self, monkeypatch) -> None:
        # A BACKSLASH spelling under a cross-drive CODEXCREW_HOME must also be
        # recognized: the keystone leaves are re-anchored under that root, so
        # its drive is an anchor alongside the user home's.
        from unittest.mock import patch

        monkeypatch.setenv("CODEXCREW_HOME", "D:\\crew")
        with patch.object(security.Path, "home", return_value=Path("C:\\Users\\u")):
    ...
    def test_drive_shapes_are_inert_on_posix_homes(self, monkeypatch) -> None:
        # With a POSIX home and no drive-lettered CODEXCREW_HOME, no anchor
        # root has a drive, so drive/UNC tokens are not path-like at all --
        # no behavior change for POSIX workflows. (CODEXCREW_HOME must be
...
class TestWindowsSeparatorRuns:
    ...
    def test_the_write_protected_leaves_tolerate_a_run(self, leaf: str) -> None:
        for prefix in (".kiro\\crew", ".codexcrew"):
    ...
    def test_a_run_does_not_smuggle_an_extraction_into_the_trust_root(self) -> None:
        ...
        for cmd in (
            "tar -xf evil.tar -C $HOME//.codex-crew",
            "tar -xf evil.tar -C ~//.kiro//crew",
            "tar -xzf evil.tar -C $HOME/\\.codex-crew",
        ):
            assert is_sensitive_bash_command(cmd) is not None, cmd
    ...
    def test_a_unc_path_with_an_interior_run_is_still_fenced(self) -> None:
        ...
        for cmd in (
            ...
            r'type "\\server\share\.codex-crew\\security_policy.json"',
            r'type "//server/share/.kiro//crew/security_policy.json"',
    ...
    def test_the_unchanged_unc_spelling_still_matches(self) -> None:
        ...
        cmd = r'type "\\server\share\.codex-crew\security_policy.json"'
        assert is_sensitive_bash_command(cmd) is not None
...
class TestBareTokenProtectedLeaves:
    ...
    around all of them: after ``cd ~/.codex-crew`` a relative ``echo forged >
    ...
    def test_relative_redirect_after_cd_is_blocked(self) -> None:
        for leaf in security._BARE_TOKEN_PROTECTED_LEAVES:
            for cmd in (
                f"cd ~/.codex-crew && echo forged > {leaf}",
                f"cd $HOME/.codex-crew; echo forged >> {leaf}",
                # no space between the operator and the target
                f"cd ~/.codexcrew && echo forged >{leaf}",
                f"cd ~/.codex-crew && echo forged > '{leaf}'",
            ):
                assert is_sensitive_bash_command(cmd) is not None, cmd
    ...
    def test_unrelated_names_and_crew_content_stay_allowed(self) -> None:
        ...
        assert is_sensitive_bash_command("touch ~/.codex-crew/sessions.db") is None
        assert is_sensitive_bash_command("touch ~/.codexcrew/sessions.db") is None
        assert is_sensitive_bash_command("cat ~/.codex-crew/config.json") is None
        for leaf in security._BARE_TOKEN_PROTECTED_LEAVES:
...
class TestKiroAgentsDirWriteProtection:
    ...
    def test_directory_is_tail_of_kiro_agents_dir(
        ...
        monkeypatch.delenv("CODEXCREW_HOME", raising=False)
        from codex_crew.config.paths import kiro_agents_dir
    ...
    def test_kiro_home_override_is_covered_on_the_tool_gate(self, tmp_path, monkeypatch) -> None:
        ...
        # CODEXCREW_HOME re-anchors the crew secrets). The default ~/.kiro/agents
...
class TestDeniedCommandsKeystone:
    ...
    ``~/.codexcrew/denied_commands.json`` on ``_SENSITIVE_HOME_DIRS`` — a full
    ...
    def test_keystone_path_is_sensitive(self) -> None:
        from codex_crew.security import is_sensitive_path

        assert is_sensitive_path("~/.codexcrew/denied_commands.json") is True
    ...
    def test_bash_write_and_read_both_blocked(self) -> None:
        ...
        for cmd in (
            "echo x > ~/.codexcrew/denied_commands.json",
            "tee ~/.codexcrew/denied_commands.json",
            "cp evil ~/.codexcrew/denied_commands.json",
            "cat ~/.codexcrew/denied_commands.json",
            "python -c open ~/.codexcrew/denied_commands.json",
        ):
            assert is_sensitive_bash_command(cmd) is not None, cmd
...
class TestStreamRedactor:
    ...
    @staticmethod
    def _run(chunks):
        from codex_crew.security import StreamRedactor
    ...
    def test_long_unbroken_run_is_capped_no_data_loss(self) -> None:
        ...
        from codex_crew.security import _STREAM_HOLDBACK_MAX, StreamRedactor
    ...
    def test_bearer_anchor_respects_holdback_cap_no_unbounded_buffer(self) -> None:
        ...
        """
        from codex_crew.security import _STREAM_HOLDBACK_JWT_MAX, StreamRedactor
...
class TestScanMemoryImportGuard:
    ...
    def test_non_importerror_degrades_to_empty(self, monkeypatch) -> None:
        ...
        from codex_crew.security import scan_memory

        real_import = builtins.__import__

        def fake_import(name, *args, **kwargs):
            if name == "codex_crew.vector_memory" or name.endswith(".vector_memory"):
...
class TestApplyResourceLimits:
    ...
    def test_bias_helper_writes_oom_score_adj(self) -> None:
        ...
        from codex_crew.security import _bias_child_oom_score
        ...
        with (
            patch("codex_crew.security.sys.platform", "linux"),
            patch("codex_crew.security.os.open", side_effect=fake_open),
            patch("codex_crew.security.os.write", return_value=4) as mwrite,
            patch("codex_crew.security.os.close") as mclose,
        ):
    ...
    def test_bias_helper_swallows_oserror(self) -> None:
        ...
        with (
            patch("codex_crew.security.sys.platform", "linux"),
            patch("codex_crew.security.os.open", side_effect=OSError("denied")),
        ):
    ...
    def test_bias_helper_noop_off_linux(self) -> None:
        ...
        with (
            patch("codex_crew.security.sys.platform", "darwin"),
            patch("codex_crew.security.os.open") as mopen,
        ):
            _bias_child_oom_score()
    ...
    def test_none_resource_module_is_noop(self, monkeypatch) -> None:
        ...
        import codex_crew.security as sec
...
class TestCodexCrewSlackAppCreateLink:
    """Codex Crew's OWN Slack app-create deep link survives the exfil redactor.

    ``codexcrew manifest --url`` and ``GET /api/slack/manifest`` emit
...
class TestDashboardLinkTokenAcrossHostForms:
    ...
    def test_token_is_redacted_on_every_host_form(self) -> None:
        from codex_crew.security import redact_credentials
...
class TestCronStoreProtection:
    ...
    def test_leaf_membership(self) -> None:
        ...
        # un-fence them.
        from codex_crew.security import _CREW_SECRET_LEAVES
    ...
    def test_bash_write_and_read_both_blocked(self) -> None:
        for cmd in (
            "echo x > ~/.codex-crew/crons.json",
            "tee ~/.codex-crew/crons.json",
            "cp evil ~/.codex-crew/crons.json",
            'sed -i \'s/"approval_mode": ""/"approval_mode": "auto"/\' ~/.codex-crew/crons.json',
            "cat ~/.codex-crew/crons.json",
            "echo x > ~/.codex-crew/cron-history/_index.jsonl",
            "cat ~/.codexcrew/crons.json",
            # A forged marker, and erasing one, are both the breaker's problem.
            'echo {"job_id":"x","pid":1,"started_at":0} > ~/.codex-crew/cron-running/x.json',
            "rm ~/.codex-crew/cron-running/x.json",
            "cat ~/.codex-crew/cron-running/x.json",
        ):
            assert is_sensitive_bash_command(cmd) is not None, cmd

    def test_sibling_cron_names_are_not_over_blocked(self) -> None:
        from codex_crew.security import is_sensitive_path, is_sensitive_write_path
        ...
        assert is_sensitive_path("~/.codex-crew/workspace/crons.json.bak") is False
...
class TestModelWeightsAreWriteProtected:
    ...
    conftest pins ``CODEXCREW_HOME`` to a per-test temp directory, which is deliberately
    ...
    MODEL_PATHS = (
        "~/.codex-crew/models/whisper/ggml-base.bin",
        "~/.codex-crew/models/qwen3-embedding-0.6b.gguf",
        "~/.codexcrew/models/whisper/ggml-base.bin",
    )
    ...
    def test_reads_stay_allowed_at_the_tool_gate(self, path: str) -> None:
        ...
        `codexcrew doctor` both read the directory to report what is installed, and the
    ...
    @pytest.mark.parametrize(
        "command",
        (
            "cd ~/.codex-crew/models/whisper && cp /tmp/evil.bin ggml-base.bin",
            "cd ~/.codexcrew/models ; echo x > a.gguf",
            "cd ~/.codexcrew/models/ ; echo x > a.gguf",
            "cd ~/.codexcrew/models/whisper; echo x > a.gguf",
        ),
    )
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            "cd ~/.codex-crew/models; cp /tmp/evil.bin ggml-base.bin",
            "cd ~/.codex-crew/models && cp /tmp/evil.bin ggml-base.bin",
            "cd ~/.codex-crew && echo x > models/ggml-base.bin",
            "cd ~/.kiro && cp /tmp/evil.bin crew/models/ggml-base.bin",
            "cd ~/.codex-crew/models; dd if=/tmp/evil of=ggml-large-v3-turbo.bin",
            "cd ~/.codex-crew/models; mv /tmp/evil ggml-tiny.bin",
            "cd ~/.codex-crew/models; ln -sf /tmp/evil ggml-base.bin",
            "cd ~/.codex-crew/models; python -c \"open('ggml-small.bin','wb')\"",
            ...
            "cd ~/.codex-crew/models; cp /tmp/evil.bin ggml-base.bin.tmp",
            "cd ~/.codex-crew/models; cp /tmp/evil.bin GGML-BASE.BIN",
            ...
            "cd ~/.codex-crew/models; tar -xf /tmp/evil.tar",
            "cd ~/.codex-crew/models; unzip /tmp/evil.zip",
        ),
    )
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            "cd ~/.aws;",
            "cd ~/.ssh;",
            "cd ~/.codex-crew/profiles;",
            "cd ~/.codex-crew/models;",
            "(cd ~/.aws)",
            "cd ~/.codex-crew/models|x",
        ),
    )
...
class TestPublishFloorNestedPayloads:
    ...
    def test_end_of_options_terminator_does_not_hide_the_payload(self) -> None:
        ...
        """
        from codex_crew.security import _shell_payload_sources, is_denied
    ...
    def test_the_eval_join_stays_linear(self) -> None:
        ...
        from codex_crew.security import _nested_shell_payloads
    ...
    def test_a_join_produced_frame_does_not_join_again(self) -> None:
        ...
        """
        from codex_crew.security import _deny_segment_views, _shell_payload_walk
    ...
    def test_source_arguments_are_not_joined(self) -> None:
        ...
        """
        from codex_crew.security import _nested_shell_payloads, normalize_shell_command
    ...
    def test_self_protection_floor_shares_the_walk(self) -> None:
        ...
        for cmd in (
            "bash -c 'codexcrew token'",
            "bash -c 'codexcrew restart'",
            "cat <(codexcrew token)",
        ):
            assert is_denied(cmd) is not None, cmd

    def test_payload_sources_and_frames_agree(self) -> None:
        from codex_crew.security import _self_token_frames, _shell_payload_sources
...
class TestGluedShellCommandPayloadExtraction:
    ...
    def test_every_glued_spelling_yields_the_spaced_payload(self) -> None:
        ...
        from codex_crew.security import _nested_shell_payloads, _shell_tokens
...
class TestGitPublishSubshellGluing:
    ...
    def test_glued_subshell_push_reaches_protected_branch_check(self) -> None:
        from codex_crew.security import _is_push_to_protected_branch
    ...
    def test_glued_operator_on_the_ref_is_not_part_of_the_name(self) -> None:
        ...
        """
        from codex_crew.security import _is_push_to_protected_branch, is_denied
    ...
    def test_cut_at_operator_preserves_a_quoted_ref(self) -> None:
        from codex_crew.security import _cut_at_operator
    ...
    def test_a_quoted_program_still_anchors_the_push(self) -> None:
        ...
        """
        from codex_crew.security import _git_push_args, is_denied
    ...
    def test_the_anchor_view_does_not_double_dequote_the_refs(self) -> None:
        ...
        """
        from codex_crew.security import _git_push_args, _is_push_to_protected_branch
...
class TestMaskedSubstitutionKeepsAdjacentLiterals:
    ...
    def test_unvalued_placeholder_is_brace_delimited(self) -> None:
        from codex_crew.security import _SUBST_PLACEHOLDER_NAME, _mask_substitutions
    ...
    def test_valued_placeholder_stays_bare(self) -> None:
        ...
        """
        from codex_crew.security import _mask_substitutions_valued
...
class TestIdentityAuthStoreFence:
    ...
    def test_leaf_membership_uses_the_canonical_filename_constant(self) -> None:
        ...
        from codex_crew.identity_stores import (
            AUTH_SQLITE_DB,
    ...
    def test_absolute_home_spelling_is_refused(self) -> None:
        home = os.path.expanduser("~")
        assert is_sensitive_bash_command(f"cat {home}/.codex-crew/data.sqlite3") is not None
    ...
    def test_unrelated_databases_are_not_over_blocked(self) -> None:
        ...
        for cmd in (
            ...
            "cat ~/.codex-crew/config.json",
            "cat ~/.codex-crew/sessions.db",
            "cat ~/.codex-crew/memory.db",
        ):
            assert is_sensitive_bash_command(cmd) is None, cmd
...
class TestFindTraversalReachesFence:
    ...
    path". ``find ~/.codex-crew -name .env -exec cat {} +`` has no such token: the
    ...
    @pytest.mark.parametrize(
        "command",
        (
            "find ~/.codex-crew -name '.env' -exec cat {} +",
            "find ~/.codex-crew -name 'token_signing.key' -exec cat {} +",
            "find ~/.codex-crew -name 'computer_use.json' -exec cat {} +",
            "find ~/.codex-crew -name '*.lock' | xargs cat",
        ),
    )
    ...
    @pytest.mark.parametrize(
        ...
        (
            "find ~/.codex-crew -name '.env' -exec cat {} +",
            "find ~/.codex-crew -name '.env' -exec cat {} ;",
            "find ~/.codex-crew -name '.env' -execdir cat {} +",
            "find ~/.codex-crew -name '.env' -ok cat {} ;",
            "find ~/.codex-crew -name '.env' -okdir cat {} ;",
            "find ~/.codex-crew -name '.env' -delete",
            "find ~/.codex-crew -name '.env' -fprint /tmp/leak",
            "find ~/.codex-crew -name '.env' -fls /tmp/leak",
            "find ~/.codex-crew -name '.env' -fprintf /tmp/leak '%p'",
            "find ~/.codex-crew -name '.env' -exec sh -c 'cat \"$1\"' _ {} ;",
            # the pipe is the delivery, so xargs' own flag grammar never matters
            "find ~/.codex-crew -name '.env' | xargs cat",
            "find ~/.codex-crew -name '.env' -print0 | xargs -0 cat",
            "find ~/.codex-crew -name '.env' | xargs -I{} cat {}",
            "find ~/.codex-crew -name '.env' | xargs -n1 -P4 head -c 100",
            "find ~/.codex-crew -name '.env' | while read f; do cat $f; done",
            "cat $(find ~/.codex-crew -name '.env')",
            "cat `find ~/.codex-crew -name '.env'`",
            "cat < <(find ~/.codex-crew -name '.env')",
            "find ~/.codex-crew -name '.env' > /tmp/leak",
        ),
    )
    ...
    def test_an_unknown_primary_is_treated_as_delivery(self) -> None:
        ...
        assert security.is_sensitive_bash_command(
            "find ~/.codex-crew -name '.env' -exceedingly-new-primary cat {} +"
        )
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            # leaf is the runtime wildcard -- the `~/.codex-crew/$F` shape
            "find ~/.codex-crew -type f -exec cat {} +",
            "find ~/.codex-crew -name '*' -exec cat {} +",
            "find ~/.codex-crew -name 'tmp*.tmp' -exec cat {} +",
            "find ~/.codexcrew -name '.env' -exec cat {} +",
            "find ~/.codex-crew/workspace/md-notebook -type f -exec cat {} +",
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            "find ~ -wholename '*/.codex-crew/.env' -exec cat {} +",
    ...
    @pytest.mark.parametrize(
        "command",
        (
            "find -L ~/.codex-crew -name '.env' -exec cat {} +",
            "find -H -P ~/.codex-crew -name '.env' -exec cat {} +",
            "find -D tree ~/.codex-crew -name '.env' -exec cat {} +",
            "find -O3 ~/.codex-crew -name '.env' -exec cat {} +",
            "find /tmp ~/.codex-crew -name '.env' -exec cat {} +",
            "/usr/bin/find ~/.codex-crew -name '.env' -exec cat {} +",
            "fi''nd ~/.codex-crew -name '.env' -exec cat {} +",
            "find $HOME/.codex-crew -name '.env' -exec cat {} +",
            "find ${HOME}/.codex-crew -name '.env' -exec cat {} +",
            "find ~/.codex-crew -mindepth 1 -maxdepth 2 -type f -name '.env' -exec cat {} +",
            "find ~/.codex-crew '(' -name '.env' -o -name '*.key' ')' -exec cat {} +",
            "ls /tmp && find ~/.codex-crew -name '.env' -exec cat {} +",
        ),
    )
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            "find ~/.codex-crew/workspace -name '*.md' -exec cat {} +",
            "find ~/.codex-crew/workspace/memory -name 'projects.md' -exec cat {} +",
            "find ~/.codex-crew/skills -name 'SKILL.md' | xargs head -5",
            # listing only: no command receives a match
            "find ~ -name '.env'",
            "find ~ -name 'credentials' -print",
            "find ~/.codex-crew -type f -name '*.lock' -printf '%p\\n'",
            "find ~/.codex-crew -name '.env' -ls",
    ...
    def test_the_delivery_requirement_is_what_bounds_the_change(self) -> None:
        ...
        listed = "find ~/.codex-crew -type f -name '*.lock'"
    ...
    def test_the_gate_is_not_verb_aware(self) -> None:
        ...
        for child in ("cat", "head", "python3", "base64", "cp", "rm", "totally-unknown"):
            command = f"find ~/.codex-crew -name '.env' -exec {child} {{}} +"
            assert security.is_sensitive_bash_command(command), command
    ...
    def test_the_pass_only_ever_adds_denials(self) -> None:
        ...
        for command in (
            "cat ~/.aws/credentials",
            "cat ~/.codex-crew/token_signing.key",
            "cd ~/.codex-crew && cat .env",
            "find ~/.ssh -type f",
        ):
            assert security.is_sensitive_bash_command(command), command
    ...
    def test_the_regex_over_block_is_bounded_by_the_root(self, tmp_path) -> None:
        ...
        relocates ``CODEXCREW_HOME`` under the pytest base temp dir, which on CI lives
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            "find ~/.codex-crew -name .env|xargs cat",
            "find ~/.codex-crew -name .env|head -1",
            "find ~/.codex-crew -type f|xargs cat",
            "find ~/.codex-crew -type f>/tmp/leak",
            "find ~/.codex-crew -type f>>/tmp/leak",
            "find ~/.codex-crew -name .env -exec cat {} ;",
    ...
    def test_a_nested_whole_directory_store_is_probed(self, monkeypatch, tmp_path) -> None:
        ...
        ``~/.codex-crew`` out, since that directory is a leaf PARENT and not an entry.
        """
        home = tmp_path / "home"
    ...
    def test_an_operator_glued_to_the_program_word(self, prefix: str) -> None:
        ...
        """
        command = f"{prefix}find ~/.codex-crew -name '.env' -exec cat {{}} +"
        assert security.is_sensitive_bash_command(command), command
    ...
    def test_a_glued_pipe_after_the_traversal_is_still_delivery(self) -> None:
        ...
        assert security.is_sensitive_bash_command("find ~/.codex-crew -type f|xargs cat")
        assert security.is_sensitive_bash_command("find ~/.codex-crew -name '.env'|head -1")

    @pytest.mark.parametrize(
        "command",
        (
            "find ~/.codex-crew -type f; cat notes | less",
            "find ~/.codex-crew -type f && echo done | tee log",
            "find ~/.codex-crew -name '*.lock' ; echo $(date) > /tmp/stamp",
            "find ~/.codex-crew -type f & wait",
        ),
    )
    ...
    def test_the_same_traversal_with_its_own_pipe_is_denied(self) -> None:
        ...
        assert security.is_sensitive_bash_command("find ~/.codex-crew -type f | less")
        assert security.is_sensitive_bash_command("find ~/.codex-crew -type f > /tmp/leak")
    ...
    def test_a_pattern_that_will_not_compile_fails_closed(self, pattern_flag: str) -> None:
        ...
        *maybe* answers yes.
        """
        command = f"find ~/.codex-crew {pattern_flag} -exec cat {{}} +"
        assert security.is_sensitive_bash_command(command), command
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            'bash -c "find ~/.codex-crew -name .env -exec cat {} +"',
            "sh -c 'find ~/.codex-crew -name .env -exec cat {} +'",
            'bash -c "find ~/.codex-crew -name .env | xargs cat"',
            "zsh -c 'find ~ -name token_signing.key -exec cat {} +'",
            "eval 'find ~/.codex-crew -name .env -exec cat {} +'",
            ...
            "bash -c \"sh -c 'find ~/.codex-crew -name .env -exec cat {} +'\"",
            # the payload is captured because the WRAPPER is
            "cat $(bash -c 'find ~/.codex-crew -name .env')",
        ),
    )
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            "cat ` find ~/.codex-crew -name .env `",
            "cat < <( find ~/.codex-crew -name .env )",
            "head -c 80 $(   find ~/.codex-crew -name .env   )",
            # nested one substitution deep
            "cat $(echo $( find ~/.codex-crew -name .env ))",
    ...
    def test_a_glob_expanded_program_word_still_names_find(self, program: str) -> None:
        ...
        """
        command = f"{program} ~/.codex-crew -name '.env' -exec cat {{}} +"
        assert security.is_sensitive_bash_command(command), command
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            "printf '%s\\0' ~/.codex-crew | find -files0-from - -name .env -exec cat {} +",
    ...
    def test_the_measured_false_positive_families_stay_allowed(self, monkeypatch, tmp_path) -> None:
        ...
        """
        home = tmp_path / "home"
        (home / ".codexcrew" / "run").mkdir(parents=True)
        (home / ".codexcrew" / "run" / "wrapper.py").write_text("print(1)\n")
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            "F=find; $F ~/.codex-crew -name .env -exec cat {} +",
            "F=find; ${F} ~/.codex-crew -name .env -exec cat {} +",
            ...
            'D=~/.codex-crew; find "$D" -type f -exec cat {} +',
            'D=~/.codex-crew; find "$D" -name .env -exec cat {} +',
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            "find >/tmp/leak ~/.codex-crew -type f",
            "find > /tmp/leak ~/.codex-crew -type f",
            "find >>/tmp/leak ~/.codex-crew -type f",
            "find 2>/tmp/leak ~/.codex-crew -type f",
            "find >/tmp/leak ~/.codex-crew -name .env",
    ...
    def test_a_leading_redirect_does_not_become_a_traversal_root(self) -> None:
        ...
        tokens = security.normalize_shell_command(
            security._find_space_unquoted_operators("find >/tmp/leak ~/.codex-crew -type f")
        )
    ...
    def test_a_command_nesting_more_substitutions_than_the_budget_is_refused(self) -> None:
        ...
        nested = "cat $(bash -c 'find ~/.codex-crew -type f -exec cat {} +')"
    ...
    def test_flat_backtick_spans_are_not_refused_for_the_budget(self) -> None:
        ...
        hidden = prose + " ; find ~/.codex-crew -name '.env' -exec cat {} +"
    ...
    def test_the_descoped_computed_operand_boundary_is_what_the_docs_claim(self) -> None:
        ...
        for command in (
            ...
            "$(printf find) ~/.codex-crew -type f -exec cat {} +",
            "`echo find` ~/.codex-crew -type f -exec cat {} +",
    ...
    def test_what_the_pass_does_resolve_from_the_text_alone(self) -> None:
        ...
        """
        for command in (
            "F=find; $F ~/.codex-crew -type f -exec cat {} +",
            "F=fin; ${F}d ~/.codex-crew -type f -exec cat {} +",
            "f?nd ~/.codex-crew -type f -exec cat {} +",
            'bash -c "find ~/.codex-crew -type f -exec cat {} +"',
            'echo "$(find ~/.codex-crew -type f -exec cat {} +)"',
        ):
            assert security.is_sensitive_bash_command(command), command
    ...
    def test_every_redirect_spelling_is_delivery_not_a_control_operator(
        ...
        """
        command = f"find ~/.codex-crew -type f {spelling}"
        assert security.is_sensitive_bash_command(command), command
    ...
    def test_a_bsd_preroot_option_does_not_hide_the_root(self, option: str) -> None:
        ...
        """
        command = f"find {option} ~/.codex-crew -type f -exec cat {{}} +"
        assert security.is_sensitive_bash_command(command), command
    ...
    def test_the_bsd_root_option_supplies_a_root_rather_than_hiding_one(self) -> None:
        ...
        """
        for command in (
            "find -f ~/.codex-crew -type f -exec cat {} +",
            "find -E -f ~/.codex-crew -type f -exec cat {} +",
            "find -f ~/.codex-crew -f /tmp -type f -exec cat {} +",
        ):
            assert security.is_sensitive_bash_command(command), command
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            "F=fin; ${F}d ~/.codex-crew -type f -exec cat {} +",
            "F=fin; ${F}d ~/.codex-crew -type f | xargs cat",
            "P=f; Q=ind; ${P}${Q} ~/.codex-crew -type f -exec cat {} +",
        ),
    )
    ...
    def test_a_leading_redirect_still_delivers_when_nothing_bounds_it(self) -> None:
        ...
        assert security.is_sensitive_bash_command("find >/tmp/out ~/.codex-crew -type f")
