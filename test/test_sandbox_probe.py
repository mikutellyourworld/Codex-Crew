...
from codex_crew.sandbox import _probe_sandbox_exec
...
@patch("codex_crew.sandbox.sys")
@patch("codex_crew.sandbox.os.path.exists", return_value=False)
...
@patch("codex_crew.sandbox.sys")
@patch("codex_crew.sandbox.os.path.exists", return_value=True)
@patch("codex_crew.sandbox.subprocess.run")
...
@patch("codex_crew.sandbox.sys")
@patch("codex_crew.sandbox.os.path.exists", side_effect=[True, False])
@patch("codex_crew.sandbox.subprocess.run")
...
@patch("codex_crew.sandbox.sys")
@patch("codex_crew.sandbox.os.path.exists", return_value=True)
@patch("codex_crew.sandbox.subprocess.run", side_effect=OSError("timeout"))
...
"""
    ...
    def test_the_shim_imports_nothing_first_party(self):
        """It runs under ``-I -S``: no site directory, so a codex_crew import would fail."""
        assert "codex_crew" not in sb._PROBE_SHIM_CODE
        for line in sb._PROBE_SHIM_CODE.splitlines():
            if line.startswith(("import ", "from ")):
                assert "codex_crew" not in line, line
