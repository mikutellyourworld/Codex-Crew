...
from codex_crew.dashboard import chat_delivery as cd
from codex_crew.dashboard import create_rate_limit
from codex_crew.dashboard import session_control as sc
from codex_crew.dashboard import stop_retry
from codex_crew.dashboard.chat_utils import slot_history_key
from codex_crew.dashboard.handlers import session_control as handlers_sc
...
# it — it still resolves ``CodexCrewConfig`` through module globals, so patching
...
class TestTheRoutesRequireTheInternalSecret:
    ...
    def test_send_with_the_secret_delivers_to_the_target(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_runner._run_chat", _fake_run_chat)
...
def test_the_switch_is_on_by_default_and_an_explicit_false_still_disables():
    ...
    switch: the tools come from the `codexcrew-dashboard` MCP server, so an agent
    ...
    * **Malformed.** ``bool("false")`` is ``True``, so a plain coercion loads a
    ...
    Asserted on the source rather than through ``CodexCrewConfig.load()``:
...
def test_a_config_read_that_raises_disables_the_feature(monkeypatch):
    ...
    monkeypatch.setattr(sc, "CodexCrewConfig", _Exploding)
...
def test_nothing_suspends_between_the_stop_gate_and_the_stop(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.dashboard.chat_handlers.stop_slot_turn", _fake_stop)
...
def test_the_config_warm_is_the_last_suspension_before_the_stop_gate(tmp_path, monkeypatch):
    ...
    `session_control_enabled` is synchronous and its `CodexCrewConfig.load()`
...
def test_a_retried_stop_keeps_the_queue_instead_of_escalating(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.dashboard.chat_handlers.sel", lambda: MagicMock())
...
def test_the_stop_button_still_escalates_on_a_second_press(tmp_path, monkeypatch):
    ...
    """
    from codex_crew.dashboard.chat_handlers import stop_slot_turn
...
def test_session_control_routes_are_strict_internal():
    ...
    from codex_crew.dashboard.server import _STRICT_INTERNAL_API_PATHS, _register_mcp_routes
...
def test_create_schema_bounds_the_folder_reference():
    ...
    """
    from codex_crew.validation import SESSION_CREATE_SCHEMA, ValidationError, validate_tool_args
...
def test_the_empty_window_merge_mirrors_the_full_saves_slot_owned_fields(tmp_path):
    ...
    """
    from codex_crew.dashboard.chat_persistence import save_slot_off_loop
    from codex_crew.history import SLOT_OWNED_META_KEYS
...
def test_creation_never_loads_the_config_on_the_event_loop():
    ...
    `CodexCrewConfig.load()` is synchronous filesystem work. Called directly from a
    ...
    assert "CodexCrewConfig.load()" not in code, "the config load must not run inline on the loop"
    assert (
        "await asyncio.to_thread(CodexCrewConfig.load)" in code
...
def test_an_identical_steer_is_refused_while_the_first_is_still_in_flight(tmp_path):
    ...
    """
    from codex_crew.dashboard import chat_delivery
...
def test_session_control_is_not_imported_on_the_gateway_boot_path():
    ...
    for line in src.splitlines():
        if line.startswith("from codex_crew.dashboard.handlers import"):
...
def test_slot_cap_has_one_owning_constant() -> None:
    ...
    """
    from codex_crew.dashboard import chat_fork
    from codex_crew.dashboard import session_control as sc_mod
    from codex_crew.dashboard import session_transfer
    from codex_crew.dashboard import state as state_mod
...
def test_close_maps_a_close_failure_to_its_code(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.dashboard.chat_handlers.close_slot", _boom)
...
def test_close_authorizes_then_acts_with_nothing_in_between(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sc, "sel", _sel)
    monkeypatch.setattr(sc, "authorize_target", _authorize)
    monkeypatch.setattr("codex_crew.dashboard.chat_handlers.close_slot", _fake_close)
