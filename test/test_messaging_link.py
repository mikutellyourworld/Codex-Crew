...
class TestBuildDmSessionKey:
    def test_per_channel_peer_is_channel_first(self) -> None:
        assert (
            build_dm_session_key("telegram", "codexcrew", "123")
            == "telegram:codexcrew:direct:123"
        )
    ...
    def test_default_scope_is_per_channel_peer(self) -> None:
        ...
        assert build_dm_session_key(
            "telegram", "codexcrew", "123"
        ) == build_dm_session_key(
            "telegram", "codexcrew", "123", dm_scope=DM_SCOPE_PER_CHANNEL_PEER
        )
    ...
    def test_unified_collapses_channel_and_user(self) -> None:
        a = build_dm_session_key("telegram", "codexcrew", "1", dm_scope=DM_SCOPE_UNIFIED)
        b = build_dm_session_key("wecom", "codexcrew", "999", dm_scope=DM_SCOPE_UNIFIED)
        assert a == b == "unified:codexcrew"
    ...
    def test_unified_scopes_only_direct_dms_not_forum(self) -> None:
        ...
        assert key == "telegram:codexcrew:forum:-1001234567890:5"
...
class TestBindOriginMirror:
    def test_an_unbound_conversation_is_bound_to_itself(self) -> None:
        sess = _Sessions()
        assert bind_origin_mirror(sess, key="discord:codexcrew:direct:u1", location=_HERE) is True
        assert sess.mirror_links == {"discord:codexcrew:direct:u1": _HERE}
    ...
    def test_a_unified_bucket_is_never_bound(self) -> None:
        ...
        """
        sess = _Sessions()
        assert bind_origin_mirror(sess, key="unified:codexcrew", location=_HERE) is False
        assert bind_origin_mirror(sess, key="unified:codexcrew:gen4", location=_HERE) is False
    ...
    def test_a_per_channel_bucket_under_the_same_config_is_bound(self) -> None:
        ...
        sess = _Sessions()
        assert (
            bind_origin_mirror(sess, key="discord:codexcrew:group:t9", location=_HERE) is True
        )
...
_KEY = "telegram:codexcrew:direct:7"
...
class TestUnbindReasonVocabulary:
    ...
    def test_no_call_site_passes_a_bare_reason_literal(self) -> None:
        ...
        pkg = Path(codex_crew.__file__).resolve().parent
        offenders = [
