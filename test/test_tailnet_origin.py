...
from codex_crew.dashboard.urls import build_allowed_hosts, build_allowed_origins
...
class TestSpawnHardening:
    ...
    def test_windows_candidate_must_pass_acl_validation(self, monkeypatch) -> None:
        ...
        with patch(
            "codex_crew.github_runner.validate_provider_executable",
...
class TestOriginSet:
    def test_absent_by_default(self) -> None:
        ...
        contributed = build_allowed_origins(5486, local_only=True, tailnet_host=_GOOD)
        assert contributed - baseline == {f"https://{_GOOD}"}

    def test_contributed_as_https_without_a_port(self) -> None:
        origins = build_allowed_origins(5486, local_only=True, tailnet_host=_GOOD)
        assert f"https://{_GOOD}" in origins
        assert f"https://{_GOOD}:5486" not in origins
