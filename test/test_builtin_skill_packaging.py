...
`_ensure_builtin_skills` copies `src/codex_crew/builtin_skills/` into a user's
`~/.codex-crew/skills/`, on every distribution. Top-level `skills/` is reachable
only through `_project_skills_dir()`, which reads `CODEXCREW_PROJECT_DIR` — a
...
import codex_crew
from codex_crew.skills import _RELOCATED_SKILLS
...
class TestPackagedSkillsReferenceOnlyPackagedSkills:
    ...
    @pytest.mark.parametrize(
        "referenced",
        sorted({"codexcrew-worktree-dev", "babysit"}),
    )
