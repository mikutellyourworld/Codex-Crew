"""Per-session MCP report accumulation (:mod:`codex_crew.acp.mcp_session_report`)."""
...
from codex_crew.acp.mcp_session_report import (
    _BUCKET_CAP,
    _ERROR_CAP,
    _NAME_CAP,
    McpSessionReport,
...
class TestEmptyState:
    ...
    def test_roster_alone_makes_it_non_empty(self):
        r = McpSessionReport()
        r.begin_session([{"name": "codexcrew-core"}])
        assert r.empty is False
        payload = r.payload()
        assert payload is not None
        assert payload["configured"] == ["codexcrew-core"]
...
class TestSupersetSemantics:
    def test_a_reported_server_need_not_be_in_the_roster(self):
        ...
        # Codex Crew injects, so reports are a SUPERSET of the roster. This is
