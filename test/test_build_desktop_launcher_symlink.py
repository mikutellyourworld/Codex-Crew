"""Regression guard for the bundled ``codexcrew`` launcher's symlink resolution.

Background
----------
``packaging/build-desktop.sh`` writes a small bash launcher into every backend
bundle at ``<bundle>/bin/codexcrew``.  It derives its own directory in order to
...
exactly such a symlink at ``~/.local/bin/codexcrew`` on every start
(``agent.ensure_codexcrew_on_path``), so on a packaged install ``$DIR`` became
``~/.local/bin`` and the launcher exec'd a non-existent
``~/.local/bin/python3.12`` — every ``codexcrew ...`` invocation from a shell
...
pytestmark = pytest.mark.skipif(
    os.name == "nt",
    reason="the POSIX launcher is a bash script; Windows ships codexcrew.cmd instead",
)
...
def _extract_launcher() -> str:
    ...
    m = re.search(
        r"cat > \"\$out/bin/codexcrew\" <<'LAUNCH'\n(.*?)\nLAUNCH\n",
        text,
        re.DOTALL,
    )
...
def test_resolves_when_invoked_directly(tmp_path):
    ...
    bundle = tmp_path / "codexcrew-backend-arm64"
...
def test_resolves_through_absolute_symlink(tmp_path):
    ...
    """
    bundle = tmp_path / "app" / "backend-dist" / "codexcrew-backend-arm64"
    launcher = _make_bundle(bundle)

    # Mirrors ~/.local/bin/codexcrew -> .../backend-dist/.../bin/codexcrew.
    local_bin = tmp_path / "home" / ".local" / "bin"
    local_bin.mkdir(parents=True)
    shim = local_bin / "codexcrew"
...
def test_resolves_through_relative_symlink(tmp_path):
    ...
    shim_dir.mkdir()
    shim = shim_dir / "codexcrew"
    # Relative to shim_dir: ../bundle/bin/codexcrew
    shim.symlink_to(Path("..") / "bundle" / "bin" / "codexcrew")
...
def test_resolves_through_symlink_chain(tmp_path):
    ...
    ``~/.local/bin/codexcrew``, which points at the bundle — so a single
    ...
    hop1_dir.mkdir()
    hop1 = hop1_dir / "codexcrew"
    hop1.symlink_to(launcher)

    hop2_dir = tmp_path / "hop2"
    hop2_dir.mkdir()
    hop2 = hop2_dir / "codexcrew"
...
def test_forwards_arguments_through_symlink(tmp_path):
    ...
    Asserts the module invocation contract (``-s -m codex_crew``) and that a
    quoted argument containing a space survives as ONE argument.
    """
    bundle = tmp_path / "bundle"
    launcher = _make_bundle(bundle)
    shim = tmp_path / "codexcrew"
    shim.symlink_to(launcher)

    proc = _invoke(shim, "config", "set", "a b")

    assert proc.returncode == 0, proc.stderr
    assert "ARGS=-s -m codex_crew config set a b" in proc.stdout, proc.stdout
    ...
    proc = _invoke(shim, "config", "set", "a b")
    assert proc.returncode == 0, proc.stderr
    # -s, -m, codex_crew, config, set, "a b" == 6
