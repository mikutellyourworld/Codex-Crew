...
from codex_crew.apps.builtins.papyrus.backend import latex, procio, store
...
class TestRunHelper:
    ...
    async def test_hides_codexcrews_own_trust_root_from_the_compiler(
        self, project: Path
    ) -> None:
        """Strict mode alone is NOT enough — it misses CodexCrew's own secrets.

        ``_STRICT_DIRS`` covers third-party credential locations plus
        ``~/.codex-crew/.env``, but the gateway's ``.local_secret``,
        ``sel_hmac.key``, ``security_policy.json`` and ``profiles/`` sit beside it
        and are NOT in that list. TeX reads files, so
        ``\\verbatiminput{~/.codex-crew/.local_secret}`` would typeset the gateway's
...
class TestSensitivePathsFollowTheLiveDataHome:
    """`sensitive_home_dirs()` names paths relative to `$HOME`, so its `.codex-crew/*`
    entries describe the DEFAULT data home.

    With `CODEXCREW_HOME` pointed elsewhere — a dev instance, a pod, an operator who
    ...
    def test_a_custom_data_home_is_covered(
        ...
        monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path / "moved-home"))
...
class TestTheCompileLinkScanRefusesJunctionsToo:
    ...
    name Codex Crew owns has to refuse a junction too, or the refusal is POSIX-only".
