...
_AMBIENT_READ_ALLOWLIST = frozenset(
    {
        ...
        ("forward_declared_env_enabled", "config-import:codex_crew.config.loader"),
        ...
        ("pool_identity_env_keys", "config-import:codex_crew.config.loader"),
    }
)
...
def _module_config_names(tree: ast.Module) -> set[str]:
    """Module-level names bound by importing from ``codex_crew.config*``.

    A module-scope ``from codex_crew.config.x import Y`` followed by ``Y.load()``
    ...
    for node in tree.body:
        if isinstance(node, ast.ImportFrom):
            if (node.module or "").startswith("codex_crew.config"):
                for alias in node.names:
                    names.add(alias.asname or alias.name)
        elif isinstance(node, ast.Import):
            for alias in node.names:
                if alias.name.startswith("codex_crew.config"):
...
def test_unresolved_bare_command_is_reprobed_and_install_invalidates(
    ...
    spec = json.loads((src / "agent-0.json").read_text())
    spec["mcpServers"]["srv"]["command"] = "codexcrew-test-definitely-missing-cmd"
    (src / "agent-0.json").write_text(json.dumps(spec))
    ...
    exe_name = (
        "codexcrew-test-definitely-missing-cmd.bat"
        if os.name == "nt"
        else "codexcrew-test-definitely-missing-cmd"
    )
...
def test_resolved_binary_removal_invalidates(
    ...
    exe_name = (
        "codexcrew-test-vanishing-cmd.bat" if os.name == "nt" else "codexcrew-test-vanishing-cmd"
    )
    exe = bin_dir / exe_name
    exe.write_text("#!/bin/sh\nexit 0\n")
    exe.chmod(0o755)
    ...
    src = _mk_tree(tmp_path, n_agents=1, with_env=False)
    spec = json.loads((src / "agent-0.json").read_text())
    spec["mcpServers"]["srv"]["command"] = "codexcrew-test-vanishing-cmd"
    (src / "agent-0.json").write_text(json.dumps(spec))
...
def test_a_deleted_overlay_is_rebuilt_even_on_a_settings_read_failure(
    ...
    assert servers["srv"].get("_codexcrew_mcp_gateway_wrapped") is True
...
def test_a_vanished_target_binary_refuses_the_keep(
    ...
    bin_dir.mkdir()
    monkeypatch.setenv("PATH", str(bin_dir) + os.pathsep + os.environ.get("PATH", ""))
    exe_name = "codexcrew-test-vanishing-cmd" + (".bat" if os.name == "nt" else "")
    exe = bin_dir / exe_name
    exe.write_text("#!/bin/sh\nexit 0\n")
    exe.chmod(0o755)
...
def test_legacy_overlay_acl_is_retightened_when_vouched(
    ...
    from codex_crew.mcp_gateway.rewriter import _stat_sig
...
def test_lockdown_failure_falls_through_to_full_rewrite(
    ...
    monkeypatch.setattr("codex_crew.mcp_gateway.rewriter.platform_compat.restrict_to_owner", failing)
    before = rewrite_counter["n"]
    _rewrite(tmp_path)
    fail["on"] = False
...
def test_fingerprint_carries_no_command_material(tmp_path: Path) -> None:
    ...
    assert "CODEXCREW_MCP_TARGET" not in fp.read_text()
