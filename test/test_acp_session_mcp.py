...
from codex_crew.acp import session_mcp
from codex_crew.acp.client import AcpClient
from codex_crew.acp.types import ACP_BACKEND_CLAUDE
from codex_crew.providers.mirrors import claude_code as claude_mirror
from codex_crew.providers.mirrors import registry as mirrors_registry
from codex_crew.providers.mirrors.claude_code import ClaudeCodeMirror

_CORE = {"command": "/opt/codexcrew", "args": ["mcp-core"]}
_CRON = {"command": "/opt/codexcrew", "args": ["mcp-cron"]}
...
def agents_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda name: {"codexcrew-core": dict(_CORE), "codexcrew-cron": dict(_CRON)}.get(name),
    )
...
def _write_spec(agents_dir: Path, *, servers: dict, tools: list | None) -> None:
    spec: dict = {"name": "codexcrew", "mcpServers": servers}
    if tools is not None:
        spec["tools"] = tools
    (agents_dir / "codexcrew.json").write_text(json.dumps(spec), encoding="utf-8")
...
def _write_project_spec(project_dir: Path, *, servers: dict, tools: list | None) -> None:
    ...
    if tools is not None:
        spec["tools"] = tools
    (agents / "codexcrew.json").write_text(json.dumps(spec), encoding="utf-8")
...
class TestElementShape:
    def test_stdio_entry_carries_env_array_and_explicit_type(self, agents_dir):
        ...
        foo = _by_name(session_mcp.session_mcp_servers("codexcrew"))["foo"]
        assert foo == {
            "name": "foo",
            "command": "/bin/foo",
            "args": ["--x"],
    ...
    def test_url_entry_defaults_to_http_with_headers_array(self, agents_dir):
        ...
        remote = _by_name(session_mcp.session_mcp_servers("codexcrew"))["remote"]
        assert remote == {
            "name": "remote",
    ...
    def test_non_sequence_args_does_not_raise(self, agents_dir, bad_args):
        ...
        assert _by_name(session_mcp.session_mcp_servers("codexcrew"))["foo"]["args"] == []

    def test_entry_with_no_transport_is_skipped(self, agents_dir):
        _write_spec(agents_dir, servers={"broken": {"args": ["--x"]}}, tools=["@broken"])
        assert "broken" not in _by_name(session_mcp.session_mcp_servers("codexcrew"))
...
class TestMounting:
    def test_server_not_referenced_by_tools_is_withheld(self, agents_dir):
        ...
        names = _by_name(session_mcp.session_mcp_servers("codexcrew"))
    ...
    def test_tool_scoped_reference_mounts_the_server(self, agents_dir):
        ...
        assert "foo" in _by_name(session_mcp.session_mcp_servers("codexcrew"))
    ...
    def test_a_spec_without_tools_mounts_nothing(self, agents_dir):
        ...
        assert "foo" not in _by_name(session_mcp.session_mcp_servers("codexcrew"))
    ...
    def test_registry_pointer_is_withheld_outside_registry_mode(self, agents_dir):
        ...
        assert "governed" not in _by_name(session_mcp.session_mcp_servers("codexcrew"))
    ...
    def test_registry_mode_still_keeps_crews_own_control_plane(self, agents_dir, monkeypatch):
        ...
        ``codexcrew-core``/``codexcrew-cron`` are re-derived from the managed source
        ...
        _write_spec(
            ...
            tools=["@marked", "@codexcrew-core", "@codexcrew-cron"],
        )
        monkeypatch.setattr(session_mcp, "_mcp_registry_mode", lambda: True)
        names = _by_name(session_mcp.session_mcp_servers("codexcrew"))
        assert "codexcrew-core" in names
    ...
    def test_an_unreadable_registry_mode_is_read_as_governed(self, agents_dir, monkeypatch):
        ...
        assert "local_only" not in _by_name(session_mcp.session_mcp_servers("codexcrew"))
    ...
    def test_a_project_only_agents_allowlist_is_honoured(self, tmp_path, agents_dir):
        ...
        names = _by_name(session_mcp.session_mcp_servers("codexcrew", work_dir=tmp_path))
        assert "proj" in names
        # tools names only @proj, and the control plane is not exempt from it.
        assert "codexcrew-core" not in names
        assert "codexcrew-cron" not in names
    ...
    def test_a_project_only_agent_with_no_work_dir_still_finds_nothing(self, tmp_path, agents_dir):
        ...
        assert "proj" not in _by_name(session_mcp.session_mcp_servers("codexcrew"))
    ...
    def test_a_project_only_agents_disabled_tools_reach_the_deny_rules(self, tmp_path, agents_dir):
        ...
        assert session_mcp.session_mcp_deny_rules("codexcrew", work_dir=tmp_path) == [
            "mcp__proj__danger"
        ]
        # And without the checkout it is silently lost -- the defect, pinned.
        assert session_mcp.session_mcp_deny_rules("codexcrew") == []
    ...
    def test_a_stubbed_server_yields_to_its_broker_stub(self, agents_dir):
        ...
        names = _by_name(
            session_mcp.session_mcp_servers("codexcrew", stub_server_names=frozenset({"pooled"}))
        )
    ...
    def test_a_stubbed_control_plane_server_also_yields(self, agents_dir):
        ...
        # drop has to run after that re-add or a pooled codexcrew-core comes back.
        names = _by_name(
            session_mcp.session_mcp_servers(
                "codexcrew", stub_server_names=frozenset({"codexcrew-core"})
            )
        )
        assert "codexcrew-core" not in names
        assert "codexcrew-cron" in names
...
class TestDenyRules:
    ...
    def test_malformed_spec_yields_no_rules(self, agents_dir):
        (agents_dir / "codexcrew.json").write_text("{not json", encoding="utf-8")
        assert session_mcp.session_mcp_deny_rules("codexcrew") == []
...
class TestControlPlane:
    def test_loaded_when_no_spec_exists(self, agents_dir):
        names = _by_name(session_mcp.session_mcp_servers("codexcrew"))
        assert set(names) == {"codexcrew-core", "codexcrew-cron"}
        assert names["codexcrew-core"]["args"] == ["mcp-core"]

    def test_loaded_when_the_spec_is_malformed(self, agents_dir):
        (agents_dir / "codexcrew.json").write_text("{not json", encoding="utf-8")
        assert set(_by_name(session_mcp.session_mcp_servers("codexcrew"))) == {
            "codexcrew-core",
            "codexcrew-cron",
        }
    ...
    def test_stale_spec_command_is_refreshed_from_the_managed_source(self, agents_dir):
        _write_spec(
            agents_dir,
            servers={"codexcrew-core": {"command": "/gone/codexcrew", "args": ["mcp-core"]}},
            tools=["@codexcrew-core"],
        )
        core = _by_name(session_mcp.session_mcp_servers("codexcrew"))["codexcrew-core"]
        assert core["command"] == "/opt/codexcrew"
    ...
    def test_a_spec_that_drops_the_reference_still_drops_the_server(self, agents_dir):
        ...
        _write_spec(agents_dir, servers={"foo": {"command": "/bin/foo"}}, tools=["@foo"])
        assert "codexcrew-core" not in _by_name(session_mcp.session_mcp_servers("codexcrew"))
...
class TestClientSeam:
    ...
    def test_claude_backend_translates_the_spec(self, tmp_path, agents_dir):
        _write_spec(agents_dir, servers={"foo": {"command": "/bin/foo"}}, tools=["@foo"])
        client = self._seeded(tmp_path, agent="codexcrew", acp_backend=ACP_BACKEND_CLAUDE)
    ...
    def test_neither_gate_is_an_identity_check(self, tmp_path, agents_dir, monkeypatch):
        ...
        """
        _write_spec(agents_dir, servers={"foo": {"command": "/bin/foo"}}, tools=["@foo"])
        client = self._seeded(tmp_path, agent="codexcrew")
        assert client._session_mcp_servers() == []
    ...
    def test_the_shared_call_site_reads_no_disk_for_kiro(self, tmp_path, agents_dir, monkeypatch):
        ...
        client = AcpClient(work_dir=tmp_path, agent="codexcrew")
    ...
    def test_no_tools_reach_a_session_whose_permissions_crew_does_not_own(
        ...
        client = AcpClient(work_dir=tmp_path, agent="codexcrew", acp_backend=ACP_BACKEND_CLAUDE)
...
class TestLocalSettingsSeed:
    ...
    :mod:`codex_crew.acp.seed_provenance` -- lives in
    ...
    def test_disabled_tools_reach_the_settings_deny_list(self, tmp_path, agents_dir):
        ...
        client = self._client(tmp_path, agent="codexcrew")
        client._write_claude_local_settings()
        data = json.loads((tmp_path / ".claude" / "settings.local.json").read_text())
