"""Coverage tests for codex_crew.apps.job_routes — the shared ``_jobs/*`` HTTP
surface mounted once for every app.

Everything runs in-process against ``aiohttp``'s ``TestServer`` with
``CODEXCREW_HOME`` pointed at ``tmp_path`` and the guards monkeypatched — no
...
import codex_crew.apps.job_routes as jr
import codex_crew.apps.job_sdk as js
import codex_crew.dashboard.handlers._shared as shared
import codex_crew.dashboard.handlers.source_providers as sp
from codex_crew.apps.job_sdk import JobError, JobSDK, forget_sdk, get_sdk, register_sdk
...
async def test_the_interruption_fields_reach_the_client(
    ...
    """
    from codex_crew.apps.job_sdk import (
