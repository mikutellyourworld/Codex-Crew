...
from codex_crew.acp.types import TurnUsage
from codex_crew.dashboard.chat_runner import _tool_call_ws_payload
from codex_crew.dashboard.state import (
    _MAX_SLOT_MESSAGES,
...
class TestChatSlot:
    ...
    def test_jira_issue_links_scanned_from_user_messages(self):
        ...
        already-open tab, since a restart without a ``codex_crew.__version__``
    ...
    def test_jira_issue_links_reevaluated_when_jira_allowlist_loads(self, monkeypatch):
        ...
        from codex_crew.dashboard.handlers import source_providers as sp
    ...
    def test_pr_source_links_stop_parsing_one_message_at_the_cap(self, monkeypatch):
        ...
        from codex_crew.dashboard.handlers import source_providers
    ...
    def test_issue_links_never_inherit_a_chip_status(self, monkeypatch):
        ...
        from codex_crew.dashboard import state as state_module
...
class TestBroadcastCompactionResultBackoff:
    ...
    def _failed_event(title: str = ""):
        from codex_crew.providers.base import LLMEvent
    ...
    def test_first_n_failures_shown_as_is(self, tmp_path, monkeypatch):
        from codex_crew.dashboard.chat_utils import (
    ...
    def test_cooldown_elapsed_shows_collapsed_streak_message(self, tmp_path, monkeypatch):
        import codex_crew.dashboard.chat_utils as chat_utils
    ...
    def test_enriched_title_replaces_unknown_error(self, tmp_path, monkeypatch):
        ...
        "unknown error"."""
        from codex_crew.dashboard.chat_utils import _broadcast_compaction_result
...
class TestApiChatDrainOnDisconnect:
    ...
    async def test_sse_reader_drains_pending_on_cancel(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_handlers._run_chat", fake_run_chat)
...
class TestApiChatMemoryModeForwarding:
    ...
    async def test_mismatched_memory_mode_on_existing_slot_returns_409(self, tmp_path, monkeypatch):
        ...
        mock_sel = MagicMock()
        monkeypatch.setattr("codex_crew.dashboard.chat_handlers.sel", lambda: mock_sel)
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        state = _make_state(tmp_path)
...
class TestApiChatModeForwarding:
    ...
    async def _post_chat(self, state, body):
        ...
        with pytest.MonkeyPatch.context() as mp:
            mp.setattr("codex_crew.dashboard.chat_handlers._run_chat", fake_run_chat)
            async with TestClient(TestServer(_make_app(state))) as client:
...
class TestSlotDetailPagination:
    ...
    async def test_foreign_disk_row_does_not_consume_the_unflushed_tail(
        ...
        """
        from codex_crew.dashboard.chat_persistence import _save_slot_to_history
    ...
    async def test_non_string_message_id_does_not_crash_the_bounded_read(
        ...
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        monkeypatch.setattr("codex_crew.dashboard.chat_handlers._run_chat", AsyncMock())
        state = _make_state(tmp_path)
    ...
    def test_id_less_boundary_matches_a_row_redacted_on_load(self):
        ...
        from codex_crew.dashboard.chat_handlers import _append_unflushed_tail
    ...
    async def test_hydrated_slot_over_an_all_id_transcript_is_not_served_twice(
        ...
        """
        from codex_crew.dashboard.cron_inject import hydrate_slot_from_history
    ...
    async def test_save_redacted_row_does_not_duplicate_the_persisted_suffix(
        ...
        """
        from codex_crew.dashboard.chat_persistence import (
    ...
    async def test_bounded_read_runs_the_tail_match_off_the_event_loop(self, tmp_path, monkeypatch):
        ...
        import codex_crew.dashboard.chat_handlers as ch
...
class TestSaveDoesNotResurrectDeletedSession:
    ...
    async def test_transfer_bundle_refuses_a_deleted_session(self, tmp_path, monkeypatch):
        ...
        from codex_crew.dashboard import session_transfer as st
        from codex_crew.dashboard.chat_persistence import _save_slot_to_history
    ...
    def test_channel_surfaced_slot_records_the_disk_identity(self, tmp_path, monkeypatch):
        ...
        the new transcript.
        """
        from codex_crew.dashboard import channel_slots
        from codex_crew.dashboard.chat_persistence import _save_slot_to_history
    ...
    async def test_fork_rolls_back_a_delete_landing_during_the_destination_save(
        ...
        after assembly.
        """
        from codex_crew.dashboard import chat_fork as fork_mod
        from codex_crew.dashboard.chat_persistence import _save_slot_to_history
    ...
    def test_probe_catches_a_delete_landing_between_its_stat_and_metadata_read(
        ...
        """
        from codex_crew.dashboard.chat_persistence import session_was_deleted
...
class TestSlotLifecycle:
    ...
    async def test_approve_resolves_future(self, tmp_path, monkeypatch):
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        monkeypatch.setattr("codex_crew.dashboard.chat.sel", lambda: MagicMock())
        state = _make_state(tmp_path)
        slot = state.get_or_create_slot("s1")
...
class TestPrepareMessages:
    def test_queued_preserved_done_stripped(self):
        ...
        from codex_crew.dashboard.chat import _prepare_messages
    ...
    def test_queued_placeholder_removed_on_processing(self):
        ...
        from codex_crew.dashboard.chat import _remove_queued_by_id
...
class TestKiroReadinessQueueHandoff:
    ...
    async def test_dequeued_turn_runs_without_a_readiness_probe(
        ...
        """
        from codex_crew.dashboard.chat import _run_chat
...
class TestHistorySaveOnClose:
    ...
    def test_close_saves_mode_to_history(self, tmp_path, monkeypatch):
        ...
        from codex_crew.dashboard.chat import _save_slot_to_history
...
class TestInMemoryAuthority:
    ...
    def test_append_only_colliding_ts_no_duplicate(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.history.datetime", _FrozenDateTime)
    ...
    def test_flush_then_finalized_reply_persists(self, tmp_path, monkeypatch):
        ...
        from codex_crew.dashboard.chat import _flush_segment, _save_slot_to_history
    ...
    def test_failed_rewrite_does_not_strand_or_lose(self, tmp_path, monkeypatch):
        ...
        """
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        from codex_crew.dashboard import chat_persistence
        from codex_crew.dashboard.chat import _save_slot_to_history
...
class TestBlockedSlashCommands:
    ...
    def test_quit_is_blocked(self):
        from codex_crew.dashboard.chat import _BLOCKED_SLASH_COMMANDS
    ...
    def test_blocked_is_subset_of_slash(self):
        from codex_crew.dashboard.chat import _BLOCKED_SLASH_COMMANDS, _SLASH_COMMANDS
...
class TestTitleGenerationSessionLeak:
    ...
    async def test_background_session_destroyed_on_stream_error(self, tmp_path):
        from codex_crew.dashboard.chat import _generate_title_via_kiro
    ...
    async def test_permission_request_rejected_during_title_gen(self, tmp_path):
        from codex_crew.dashboard.chat import _generate_title_via_kiro
        from codex_crew.providers.base import (
            EVENT_COMPLETE,
...
class TestFlushSegment:
    ...
    def test_flush_segment_persists_and_broadcasts(self, tmp_path, monkeypatch):
        ...
        from codex_crew.dashboard.chat import _flush_segment
    ...
    def test_flush_segment_schedules_widget_registration(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_runner.register_widgets_off_loop", _fake_register
        )
...
class TestRunChatCompactDeferredWait:
    ...
    async def test_claude_backend_skips_wait_for_compaction(self, tmp_path, monkeypatch):
        ...
        from codex_crew.providers.base import EVENT_COMPLETE, LLMEvent
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_runner.is_claude_backend", lambda _provider: True
        )
    ...
    async def test_kiro_backend_still_waits_for_compaction(self, tmp_path, monkeypatch):
        ...
        state.sessions.get_or_create = AsyncMock(return_value=(client, True, False))
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_runner.is_claude_backend", lambda _provider: False
        )
    ...
    async def test_failed_compaction_reason_is_length_capped(self, tmp_path, monkeypatch):
        ...
        from codex_crew.dashboard.chat_runner import _COMPACT_FAIL_REASON_MAX_CHARS
...
class TestTokenPersistenceBackfill:
    ...
    async def test_safe_complete_reports_structured_monitor_usage(self, tmp_path, monkeypatch):
        ...
        from codex_crew.dashboard.chat import _run_chat
        from codex_crew.monitoring.completion import MonitorCompletionHook
        from codex_crew.monitoring.models import (
        ...
        state.sessions.get_or_create = AsyncMock(return_value=(client, True, False))
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_runner.generate_session_summary",
    ...
    async def test_synthetic_timeout_complete_does_not_report_monitor_usage(
        ...
        from codex_crew.dashboard.chat import _run_chat
        from codex_crew.monitoring.completion import MonitorCompletionHook
        from codex_crew.monitoring.models import MonitorActionCompletion
        from codex_crew.providers.base import EVENT_COMPLETE, EVENT_TEXT_CHUNK, LLMEvent
    ...
    async def test_monitor_shutdown_gate_precedes_acceptance(self, tmp_path, monkeypatch):
        ...
        from codex_crew.dashboard.chat import _run_chat
        from codex_crew.monitoring.completion import MonitorCompletionHook
        from codex_crew.session import SessionClosingError
    ...
    async def test_raw_complete_survives_cancellation_during_token_persistence(
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_runner.persist_token_record_async",
            _cancel_persistence,
        )
        completions: list[MonitorActionCompletion] = []
    ...
    async def test_late_backfill_populates_model_for_cc_session(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_runner.CodexCrewConfig.load", lambda: _cc_cfg)
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_runner.is_claude_backend", lambda _client: True
        )
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_runner.persist_token_record_async", _fake_persist
        )
...
class TestTokenUsageSurface:
    ...
    async def test_completion_uses_effective_session_identity(
        ...
        persist = AsyncMock()
        monkeypatch.setattr("codex_crew.dashboard.chat_runner.persist_token_record_async", persist)
...
class TestKiroBackfillProfileGuard:
    ...
    def test_predicate_flags_bedrock_profile_ids(self):
        from codex_crew.dashboard.chat_runner import _is_bedrock_profile_id
    ...
    def test_kiro_profile_id_is_dropped(self):
        from codex_crew.dashboard.chat_runner import _backfill_canonical_model
...
class TestPinnedModelWithheld:
    ...
    def test_pin_absent_from_advertised_is_withheld(self):
        from codex_crew.dashboard.chat_runner import _pinned_model_verdict
    ...
    def test_every_session_teardown_drops_the_verdict(self):
        ...
        from codex_crew.dashboard import chat_handlers, chat_runner
    ...
    async def test_reset_chokepoint_forgets_the_verdict(self):
        ...
        from codex_crew.dashboard.chat_handlers import _reset_slot_session
        ...
        with patch("codex_crew.dashboard.chat_handlers._unblock_pending_waits"):
    ...
    async def test_run_chat_survives_an_unreadable_config_on_a_pinned_slot(
        ...
        `CodexCrewConfig.load()` (slash-command detection) would abort the turn
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_runner.CodexCrewConfig.load", load_then_break)
...
class TestRuntimeWiring:
    ...
    async def test_api_chat_slot_agent_response_includes_workspace(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat.CodexCrewConfig.load", lambda: mock_cfg)
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_handlers.CodexCrewConfig.load", lambda: mock_cfg
        )
        monkeypatch.setattr(
            "codex_crew.dashboard.chat.resolve_agent_bindings",
            lambda cfg, name, project_dir=None: mock_bindings,
        )
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_handlers.resolve_agent_bindings",
            lambda cfg, name, project_dir=None: mock_bindings,
        )
        monkeypatch.setattr(
            "codex_crew.dashboard.chat._workspace_name_for_dir",
            lambda cfg, ws_dir: "oncall-ws",
        )
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_handlers._workspace_name_for_dir",
            lambda cfg, ws_dir: "oncall-ws",
        )
    ...
    async def test_api_chat_slot_agent_failed_resolution_reports_slot_workspace(
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat.CodexCrewConfig.load", _boom)
        monkeypatch.setattr("codex_crew.dashboard.chat_handlers.CodexCrewConfig.load", _boom)
    ...
    async def test_api_chat_slot_agent_updates_project_dir(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_handlers.default_project_dir",
    ...
    async def test_api_chat_slot_agent_keeps_project_for_project_agent(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_handlers.warm_project_agent_names", AsyncMock()
        )
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_handlers.cached_project_agent_names",
    ...
    async def test_run_chat_passes_memory_store_to_build_message(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_runner.CodexCrewConfig.load", lambda: mock_cfg)
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_runner.resolve_agent_bindings",
            lambda cfg, name, project_dir=None: mock_bindings,
        )
...
class TestRunChatToolBoundarySegments:
    ...
    async def test_tool_boundary_splits_segments(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat.config_dir", lambda: tmp_path)
        monkeypatch.setattr("codex_crew.dashboard.chat.sel", lambda: MagicMock())
...
class TestRunChatToolCallUpdate:
    ...
    async def test_refinement_logs_sel_audit_event(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_runner.sel", lambda: _FakeSel())
...
class TestRunChatModelRefusal:
    ...
    async def test_stale_recover_notice_is_tagged_and_emits_one_frame(self, tmp_path, monkeypatch):
        ...
        from codex_crew.acp.types import STOP_REASON_STALE_RECOVER
        from codex_crew.dashboard.chat_utils import TRANSIENT_RETRY_KIND
        from codex_crew.providers.base import EVENT_COMPLETE, LLMEvent
    ...
    async def test_refusal_shows_declined_card_and_does_not_retry(self, tmp_path, monkeypatch):
        from codex_crew.acp.types import STOP_REASON_REFUSAL
        from codex_crew.providers.base import EVENT_COMPLETE, LLMEvent
...
class TestApiChatModePropagation:
    ...
    async def test_normal_mode_clears_policy(self, tmp_path, monkeypatch):
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        from codex_crew.safety_override import safety_override
    ...
    def test_yolo_auto_expires_and_clears_untrusted_policies(self, tmp_path, monkeypatch):
        ...
        with patch("codex_crew.safety_override.sel"):
    ...
    async def test_trust_mode_propagates_approval_policy_to_session(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        monkeypatch.setattr("codex_crew.slack.handler.is_yolo_mode", lambda: False)
        state = _make_state(tmp_path)
        state.push_slots_update = MagicMock()
        state.get_or_create_slot("s1")
...
class TestApiChatModeGlobalOverrideScope:
    ...
    async def test_until_shutdown_grant_is_ad_hoc_and_protected(self, tmp_path, monkeypatch):
        ...
        every operator who picked "until Codex Crew restarts".
        """
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        from codex_crew.safety_override import safety_override
...
class TestApiChatAgentPassing:
    ...
    async def test_invalid_agent_name_rejected(self, tmp_path, monkeypatch):
        ...
        state = _make_state(tmp_path)
        with patch("codex_crew.dashboard.chat_handlers._emit_agent_assignment") as mock_emit:
            async with TestClient(TestServer(_make_app(state))) as client:
                resp = await client.post(
                    "/api/chat?ws=1",
...
class TestPlanAction:
    ...
    async def test_go_shows_go_label(self, tmp_path, monkeypatch):
        ...
        with pytest.MonkeyPatch.context() as m:
            m.setattr("codex_crew.dashboard.chat_orchestrator._stage_loop", AsyncMock())
            async with TestClient(TestServer(_make_app(state))) as client:
                resp = await client.post(
    ...
    async def test_busy_go_queues_with_stamp_and_human_provenance(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        from codex_crew.dashboard import session_control as sc
...
class TestPlanValidationStuck:
    ...
    def test_strip_plan_markers_clears_has_plan(self):
        ...
        from codex_crew.context_management import (
            strip_plan_markers,
...
class TestOrchestratorPlanGateArming:
    ...
    async def test_stage_loop_sets_and_clears_in_stage_execution(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        monkeypatch.setattr("codex_crew.dashboard.chat.config_dir", lambda: tmp_path)
        monkeypatch.setattr("codex_crew.dashboard.chat_orchestrator.config_dir", lambda: tmp_path)
        from codex_crew.dashboard.chat import _stage_loop
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_orchestrator._run_chat", _rec)
    ...
    async def test_stage_loop_clamps_when_plan_shrinks(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_orchestrator._run_chat", _shrink)
    ...
    async def test_stage_loop_times_out_a_stage_that_swallows_cancellation(
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat.config_dir", lambda: tmp_path)
        monkeypatch.setattr("codex_crew.dashboard.chat_orchestrator.config_dir", lambda: tmp_path)
        from codex_crew.context_management import OrchestrationTracker
        from codex_crew.dashboard.chat import _stage_loop
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_orchestrator._run_chat", _hang_and_swallow)
    ...
    async def test_stage_loop_disabled_timeout_does_not_abort_instantly(
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_orchestrator._run_chat", _ok)
...
class TestPlanExecutionViaButton:
    ...
    async def test_go_button_triggers_stage_loop(self, tmp_path, monkeypatch):
        ...
        mock_loop = AsyncMock()
        with pytest.MonkeyPatch.context() as m:
            m.setattr("codex_crew.dashboard.chat_orchestrator._stage_loop", mock_loop)
            async with TestClient(TestServer(_make_app(state))) as client:
                resp = await client.post(
...
class TestWidgetOriginAutoRunGuard:
    ...
    async def test_widget_origin_go_all_denied(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_handlers._stage_loop", stage_loop_mock)
        monkeypatch.setattr("codex_crew.dashboard.chat_handlers._run_chat", run_chat_mock)
...
class TestPythonStageLoop:
    ...
    def _isolate_config_dir(self, tmp_path, monkeypatch):
        ...
        ``~/.codexcrew/sessions/`` dir, and parallel (xdist) runs then race on the
        ...
        for module in ("state", "chat", "chat_orchestrator"):
            monkeypatch.setattr(f"codex_crew.dashboard.{module}.config_dir", lambda: tmp_path)
    ...
    async def test_go_single_stage_then_stops(self, tmp_path, monkeypatch):
        ...
        run_chat_mock = AsyncMock()
        monkeypatch.setattr("codex_crew.dashboard.chat_orchestrator._run_chat", run_chat_mock)
    ...
    async def test_cancel_stops_loop(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_orchestrator._run_chat", _mock_run_chat)
    ...
    async def test_orchestrating_flag_set_and_held_queue_drained(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_orchestrator._start_next_queued_turn", start_next
        )
    ...
    async def test_signed_out_cli_holds_queue(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_orchestrator._run_chat", _auth_run_chat)
        start_next = AsyncMock(return_value=False)
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_orchestrator._start_next_queued_turn", start_next
        )
    ...
    async def test_go_button_uses_stage_loop(self, tmp_path, monkeypatch):
        ...
        stage_loop_mock = AsyncMock()
        monkeypatch.setattr("codex_crew.dashboard.chat_orchestrator._stage_loop", stage_loop_mock)
    ...
    async def test_build_stage_context_includes_goal_and_status(self):
        ...
        from codex_crew.context_management import OrchestrationTracker
        from codex_crew.dashboard.chat import _build_stage_context
    ...
    async def test_run_chat_error_stops_loop(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_orchestrator._run_chat", _exploding_run_chat)
    ...
    async def test_subagent_wait_loop(self, tmp_path, monkeypatch):
        ...
        run_chat_mock = AsyncMock()
        monkeypatch.setattr("codex_crew.dashboard.chat_orchestrator._run_chat", run_chat_mock)
        monkeypatch.setattr("codex_crew.dashboard.chat_orchestrator.asyncio.sleep", AsyncMock())
    ...
    async def test_previous_result_paths_compaction(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat.is_sensitive_path", lambda p: False)
        from codex_crew.context_management import OrchestrationTracker
        from codex_crew.dashboard.chat import _previous_result_paths
...
class TestStageFailureEscalation:
    ...
    def test_stage_round_limit_triggers_escalation(self):
        ...
        from codex_crew.context_management import MAX_STAGE_ROUNDS, OrchestrationTracker
...
class TestPromptBusyRecovery:
    ...
    async def test_prompt_busy_resets_session_and_requeues(self, tmp_path: Path) -> None:
        from codex_crew.acp.client import AcpError
        from codex_crew.dashboard.chat import _run_chat
...
class TestSlotTaskNoneGuard:
    ...
    async def test_delete_with_real_task_cancels(self, tmp_path: Path) -> None:
        ...
        async with TestClient(TestServer(_make_app(state))) as client:
            with patch("codex_crew.dashboard.chat_handlers.save_slot_off_loop"):
                resp = await client.delete("/api/chat/slots/s1")
            assert resp.status == 200
...
class TestBulkCleanup:
    ...
    async def test_cleanup_rollback_on_save_failure(self, tmp_path, monkeypatch):
        ...
        with patch(
            "codex_crew.dashboard.chat_handlers.save_slot_off_loop",
...
class TestHistoryKeyFor:
    ...
    def test_already_canonical(self):
        from codex_crew.dashboard.chat import _history_key_for
...
class TestFolderCRUD:
    ...
    async def test_create_folder_accepts_default_agent(self, tmp_path, monkeypatch):
        ...
        async with TestClient(TestServer(app)) as client:
            resp = await client.post(
                "/api/chat/folders",
                json={"name": "Payments", "default_agent": "codexcrew-dev"},
            )
            assert resp.status == 201
            data = await resp.json()
            assert data["default_agent"] == "codexcrew-dev"
    ...
    def test_folder_color_palette_matches_frontend_catalog(self):
        ...
        from codex_crew.dashboard.chat_folders import _FOLDER_COLOR_PALETTE
    ...
    async def test_slot_create_inherits_nearest_folder_project(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_handlers.schedule_eager_spawn",
    ...
    def test_resolve_folder_project_dir_terminates_on_cycle(self):
        from codex_crew.dashboard.chat_folders import _resolve_folder_project_dir
...
class TestFolderTags:
    ...
    def _app_with_tag_delete(state):
        ...
        from codex_crew.dashboard.chat_tags import api_chat_tag_delete
    ...
    async def test_folder_tag_writes_hold_the_tags_write_lock(self, tmp_path, monkeypatch):
        ...
        from codex_crew.dashboard.chat_tags import tags_write_lock
...
class TestGenerateEmojiForName:
    ...
    async def test_redaction_applied(self, tmp_path, monkeypatch):
        ...
        from codex_crew.dashboard.chat_folders import generate_emoji_for_name
        ...
        monkeypatch.setattr("codex_crew.providers.base.EVENT_TEXT_CHUNK", "text_chunk")
        monkeypatch.setattr("codex_crew.providers.base.EVENT_COMPLETE", "complete")
        monkeypatch.setattr("codex_crew.providers.base.EVENT_PERMISSION_REQUEST", "permission")
        ...
        with (
            patch(
                "codex_crew.dashboard.chat_folders.redact_exfiltration_urls",
                return_value=("🔥", False),
            ) as mock_url,
            patch(
                "codex_crew.dashboard.chat_folders.redact_credentials", return_value=("🔥", False)
            ) as mock_cred,
        ):
...
class TestNewPlanResetsAutoRun:
    ...
    def test_has_plan_resets_auto_run(self):
        ...
        from codex_crew.dashboard.chat import _reset_auto_run_for_new_plan
...
class TestRegenerateAndVariants:
    ...
    async def test_regenerate_truncates_and_stashes_variant(self, tmp_path, monkeypatch):
        ...
        with patch("codex_crew.dashboard.chat_regenerate._run_chat", new=_capture):
            async with TestClient(TestServer(_make_app(state))) as client:
    ...
    async def test_regenerate_passes_hint_to_run_chat(self, tmp_path, monkeypatch):
        ...
        slot.drain()
        mock_run = AsyncMock()
        with patch("codex_crew.dashboard.chat_regenerate._run_chat", new=mock_run):
            async with TestClient(TestServer(_make_app(state))) as client:
                resp = await client.post("/api/chat/slots/s1/regenerate")
                assert resp.status == 200
    ...
    async def test_regenerate_caps_variants(self, tmp_path, monkeypatch):
        ...
        from codex_crew.dashboard.chat import _MAX_VARIANTS
    ...
    async def test_regenerate_persists_to_disk(self, tmp_path, monkeypatch):
        ...
        from codex_crew.dashboard.chat import _history_key_for, _save_slot_to_history

        _save_slot_to_history(state, slot)
        with patch("codex_crew.dashboard.chat_regenerate._run_chat", new=AsyncMock()):
            async with TestClient(TestServer(_make_app(state))) as client:
                resp = await client.post("/api/chat/slots/s1/regenerate")
                assert resp.status == 200
    ...
    async def test_edit_resend_non_dict_body_is_400_not_500(self, tmp_path, monkeypatch):
        ...
        from codex_crew.dashboard.chat import api_chat_slot_edit_resend
    ...
    async def test_regenerate_clears_pending_on_task_error(self, tmp_path, monkeypatch):
        ...
        with patch("codex_crew.dashboard.chat_regenerate._run_chat", new=_boom):
            async with TestClient(TestServer(_make_app(state))) as client:
                resp = await client.post("/api/chat/slots/s1/regenerate")
                assert resp.status == 200
    ...
    async def test_restore_preserves_variants(self, tmp_path, monkeypatch):
        ...
        slot.messages[-1]["variant_idx"] = 1
        slot.drain()
        from codex_crew.dashboard.chat import _save_slot_to_history, restore_recent_sessions
    ...
    async def test_regenerate_clears_pending_on_cancel(self, tmp_path, monkeypatch):
        ...
        with patch("codex_crew.dashboard.chat_regenerate._run_chat", new=_hang):
            async with TestClient(TestServer(_make_app(state))) as client:
                resp = await client.post("/api/chat/slots/s1/regenerate")
                assert resp.status == 200
                assert slot._pending_variants != []
...
class TestForkSlot:
    ...
    async def test_fork_aborts_and_keeps_dirty_when_source_save_fails(self, tmp_path):
        ...
        app = _make_app(state)
        async with TestClient(TestServer(app)) as client:
            with patch("codex_crew.dashboard.chat_fork.save_slot_off_loop", failing_save):
                resp = await client.post("/api/chat/slots/src/fork", json={})
    ...
    async def test_fork_emits_sel_audit_event(self, tmp_path, monkeypatch):
        ...
        mock_sel = MagicMock()
        monkeypatch.setattr("codex_crew.dashboard.chat_fork.sel", lambda: mock_sel)
    ...
    async def test_fork_does_not_clone_parent_kiro_session_id(self, tmp_path, monkeypatch):
        ...
        """
        from codex_crew.session import SessionMap

        monkeypatch.setattr("codex_crew.session_map.config_dir", lambda: tmp_path)
    ...
    async def test_fork_rejects_when_slot_cap_reached(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_fork.MAX_LIVE_SLOTS", 3)
    ...
    async def test_fork_at_index_spans_chained_session_files(self, tmp_path):
        ...
        """
        from codex_crew.dashboard.chat_utils import _history_key_for
...
class TestColorTheme:
    ...
    async def test_color_theme_set_on_slot(self, tmp_path, monkeypatch):
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        state = _make_state(tmp_path)
        with patch("codex_crew.dashboard.chat_handlers._run_chat", new=AsyncMock()):
            async with TestClient(TestServer(_make_app(state))) as client:
                resp = await client.post(
                    "/api/chat?ws=1",
...
class TestInstalledPackConsentInjection:
    ...
    def test_matching_sha_injects(self):
        from codex_crew.dashboard.chat import _maybe_inject_persona

        with patch(
            "codex_crew.dashboard.chat_utils._installed_theme_persona",
    ...
    def test_normalizer_fail_closed_and_salvage(self):
        ...
        from codex_crew.validation import normalize_theme_consent_sha
...
class TestStopReasonCancelled:
    ...
    async def test_handler_stop_reason_cancelled_skips_record_success(self, tmp_path, monkeypatch):
        ...
        from codex_crew.acp.types import STOP_REASON_CANCELLED
        from codex_crew.dashboard.chat import _run_chat
        from codex_crew.providers.base import EVENT_COMPLETE, EVENT_TEXT_CHUNK, LLMEvent
    ...
    async def test_handler_stop_reason_end_turn_preserves_existing_behavior(
        ...
        from codex_crew.acp.types import STOP_REASON_END_TURN
        from codex_crew.dashboard.chat import _run_chat
        from codex_crew.providers.base import EVENT_COMPLETE, EVENT_TEXT_CHUNK, LLMEvent
    ...
    async def test_monitor_wake_turn_does_not_write_user_memory(self, tmp_path, monkeypatch):
        from codex_crew.acp.types import STOP_REASON_END_TURN
        from codex_crew.dashboard.chat import _run_chat
        from codex_crew.dashboard.state import MONITOR_WAKE_PREFIX
        from codex_crew.providers.base import EVENT_COMPLETE, EVENT_TEXT_CHUNK, LLMEvent
...
class TestStopDuringSessionPrep:
    ...
    def _make_state_and_slot(self, tmp_path):
        from codex_crew.dashboard.chat_runner import _run_chat
...
class TestAcpProcessDiedRecovery:
    ...
    async def test_retry_at_depth_0_requeues_message(self, tmp_path: Path) -> None:
        ...
        from codex_crew.acp.client import AcpProcessDied
    ...
    async def test_partial_assistant_text_redacted(self, tmp_path: Path) -> None:
        ...
        from codex_crew.providers.base import EVENT_TEXT_CHUNK, LLMEvent
        ...
        with patch(
            "codex_crew.dashboard.chat_runner._start_next_queued_turn",
    ...
    async def test_prompt_busy_requeue_does_not_claim_a_lost_connection(
        ...
        """
        from codex_crew.dashboard.chat_runner import PromptBusyExhaustedError
        from codex_crew.dashboard.chat_utils import (
            _BUSY_RECOVER_MSG,
            SYNTHETIC_RECOVERY_KIND,
            RecoveryPayload,
    ...
    async def test_a_recovery_turns_reply_still_reaches_the_linked_thread(
        ...
        """
        from codex_crew.dashboard.chat_utils import _CONN_RECOVER_MSG
        from codex_crew.providers.base import EVENT_TEXT_CHUNK, LLMEvent
    ...
    async def test_should_suppress_requeue_helper(self, tmp_path: Path) -> None:
        ...
        from codex_crew.dashboard.chat_runner import _should_suppress_requeue
    ...
    async def test_retry_requeues_via_queue_insert(self, tmp_path: Path) -> None:
        ...
        from codex_crew.acp.client import AcpProcessDied
        from codex_crew.dashboard.state import _ChatSlot
    ...
    async def test_retry_requeues_with_consumption_callback(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            _patch("codex_crew.dashboard.chat_runner._finish_queue_cycle"),
        ):
            await _run_chat(
                state,
                slot,
    ...
    async def test_prompt_busy_exhausted_depth_gt0_shows_please_retry(self, tmp_path: Path) -> None:
        ...
        from codex_crew.llm_helpers import PromptBusyExhaustedError
...
class TestEmptyResponseRetry:
    ...
    async def test_first_empty_response_requeues_message(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch("codex_crew.dashboard.chat_runner.save_slot_off_loop") as mock_save,
            patch("codex_crew.dashboard.chat_runner._maybe_consolidate") as mock_consolidate,
            patch("codex_crew.dashboard.chat_runner._flush_file_changes") as mock_flush,
            patch(
                "codex_crew.dashboard.chat_runner._start_next_queued_turn",
                new=AsyncMock(return_value=False),
            ),
    ...
    async def test_second_empty_response_auto_continues(self, tmp_path: Path) -> None:
        ...
        typing "continue"."""
        from codex_crew.dashboard.chat_runner import _EMPTY_AUTO_CONTINUE_MSG
    ...
    async def test_second_empty_flag_off_shows_notice(self, tmp_path: Path) -> None:
        ...
        # flag as false in a config file and point CodexCrewConfig.load() at it,
        ...
        with patch("codex_crew.config.loader.config_path", return_value=cfg_file):
            await _run_chat(state, slot, "test message")
    ...
    async def test_compaction_turn_no_empty_response_error(self, tmp_path: Path) -> None:
        ...
        from codex_crew.providers.base import EVENT_COMPACTION_STATUS, EVENT_COMPLETE, LLMEvent
    ...
    async def test_clear_turn_no_empty_response_error(self, tmp_path: Path) -> None:
        ...
        from codex_crew.providers.base import EVENT_CLEAR_STATUS, EVENT_COMPLETE, LLMEvent
    ...
    async def test_agent_switch_turn_no_empty_response_error(self, tmp_path: Path) -> None:
        ...
        from codex_crew.providers.base import EVENT_AGENT_SWITCHED, EVENT_COMPLETE, LLMEvent
...
class TestProductiveTurnNeverReplaysVerbatim:
    ...
    def _thinking_only_stream(client):
        from codex_crew.acp.types import STOP_REASON_END_TURN
        from codex_crew.providers.base import EVENT_COMPLETE, EVENT_THINKING_CHUNK, LLMEvent
    ...
    async def test_tool_only_turn_does_not_replay_verbatim(self, tmp_path: Path) -> None:
        ...
        from codex_crew.dashboard.chat_utils import _ACTIVITY_NO_REPLY_CONTINUE_MSG
...
class TestEmptyTurnDiagnostics:
    ...
    def test_normalize_stop_reason_never_returns_a_raw_backend_string(self) -> None:
        ...
        ``chat_utils`` may not import ``codex_crew.acp.types`` (the agent-SDK
        ...
        """
        from codex_crew.acp.types import (
    ...
    async def test_provider_empty_and_no_terminal_event_report_distinct_causes(
        ...
        with caplog.at_level(logging.WARNING, logger="codex_crew.dashboard.chat_runner"):
    ...
    async def test_the_warning_carries_no_content_and_no_billing_amounts(
        ...
        """
        from codex_crew.acp.types import STOP_REASON_END_TURN, TurnUsage
        from codex_crew.providers.base import (
            EVENT_COMPLETE,
            EVENT_TEXT_CHUNK,
            EVENT_TOOL_CALL,
...
class TestExpandDollarSkills:
    ...
    def _state_with_skills(self, tmp_path, monkeypatch, *skills):
        ...
        from codex_crew.platform.defaults import DefaultMcpToolingProvider
        from codex_crew.skills import SkillsLoader
    ...
    def test_no_dollar_short_circuits(self, tmp_path, monkeypatch):
        from codex_crew.dashboard.chat_runner import _expand_dollar_skills
    ...
    def test_unresolved_token_no_chip(self, tmp_path, monkeypatch):
        from codex_crew.dashboard import chat_runner
...
class TestRunChatTransientRetry:
    ...
    async def test_posttoken_recovery_turn_refailure_does_not_loop(self, tmp_path, monkeypatch):
        ...
        from codex_crew.acp.client import AcpError
        from codex_crew.dashboard.chat import _run_chat
        from codex_crew.dashboard.chat_runner import _POSTTOKEN_RECOVER_MSG
        from codex_crew.providers.base import EVENT_TEXT_CHUNK, LLMEvent
    ...
    async def test_transient_exhausts_budget_then_clean_error_resumable(
        ...
        from codex_crew.acp.client import AcpError
        from codex_crew.dashboard.chat import _run_chat
        from codex_crew.llm_helpers import TRANSIENT_RETRIES
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_runner._agent_fallback_chain", lambda: ())
        client = self._client(_stream)
        self._wire_sessions(state, client)
        slot = state.get_or_create_slot("s1")
        slot._titled = True
    ...
    async def test_poisoned_conversation_discarded_on_second_exhausted_cycle(
        ...
        with (
            ...
            patch(
                "codex_crew.dashboard.chat_runner.run_bg_oneliner",
                new_callable=AsyncMock,
                return_value="OK",
            ) as canary,
        ):
    ...
    async def test_thinking_only_failures_never_accrue_discard_streak(self, tmp_path, monkeypatch):
        ...
        from codex_crew.acp.client import AcpError
        from codex_crew.dashboard.chat import _run_chat
        from codex_crew.providers.base import EVENT_THINKING_CHUNK, LLMEvent
...
class TestRunChatModelFallback:
    ...
    async def test_fallback_swap_serialises_on_the_pick_lock(self, tmp_path, monkeypatch):
        ...
        from codex_crew.dashboard.chat_runner import _fallback_swap_for_turn

        monkeypatch.setattr(
            "codex_crew.dashboard.chat_runner._agent_fallback_chain",
    ...
    async def test_budget_exhaustion_swaps_and_recovers(self, tmp_path, monkeypatch):
        ...
        from codex_crew.dashboard.handlers.usage import read_turn_model
    ...
    async def test_candidate_budget_routes_through_shared_rewind_body(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_runner.fallback_rewound_transient_budget",
    ...
    async def test_restore_probe_skipped_when_session_moved_on(self, tmp_path, monkeypatch):
        ...
        from codex_crew.llm_helpers import TURN_FALLBACK_ATTR
...
class TestSlotProbeWrapsSharedRestoreBody:
    ...
    async def test_delegates_slot_state_to_the_shared_body(self):
        ...
        with patch(
            "codex_crew.dashboard.chat_runner.probe_fallback_restore", new_callable=AsyncMock
        ) as probe:
...
class TestSlotsGetWarmsGitLabAllowlist:
    ...
    async def test_cold_get_returns_the_authorized_self_hosted_link(self, monkeypatch) -> None:
        ...
        from codex_crew.dashboard.chat_handlers import api_chat_slots
        from codex_crew.dashboard.handlers import source_providers as sp
...
class TestBulkModelSwitch:
    ...
    def _app(state: DashboardState) -> web.Application:
        from codex_crew.dashboard.chat import api_chat_slots_model
...
class TestSlotModelGuard:
    ...
    @staticmethod
    def _app(state: DashboardState) -> web.Application:
        from codex_crew.dashboard.chat import api_chat_slot_model
...
class TestSlotModelLiveSwitch:
    ...
    def _provider(
        *,
        ...
        from codex_crew.providers.acp import AcpProvider
    ...
    async def test_unavailable_model_is_4xx_and_keeps_the_session(self, tmp_path):
        ...
        """
        from codex_crew.acp.client import AcpModelUnavailable
...
class TestForkSlotTail:
    ...
    def _tail_fork_enabled(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_fork.CodexCrewConfig.load", lambda: mock_cfg)
...
class TestSessionReload:
    ...
    async def test_reload_resets_notices_and_respawns(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_handlers.schedule_eager_spawn", eager)
    ...
    def test_system_notice_kinds_backend_frontend_parity(self):
        ...
        from codex_crew.dashboard.system_notices import SYSTEM_NOTICE_KINDS
...
class TestCloseBroadcastDurability:
    ...
    def _instrument(state, slot, monkeypatch, frames=None):
        ...
        from codex_crew.dashboard import chat_handlers
