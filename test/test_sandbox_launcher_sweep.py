...
Verifies:
  (a) namespace_argv() writes the launcher to ~/.codexcrew/run/ with PID in name
...
def fake_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    """Redirect HOME so the sandbox run dir resolves under ``tmp_path/.codexcrew``.

    The run dir moved from ``os.path.expanduser("~")/".codexcrew"/"run"`` to
    ``config_dir()/run`` (data home now ``~/.codex-crew``). ``config_dir()`` reads
    ``CODEXCREW_HOME`` (pinned to a different tmp dir by conftest), so also
    redirect ``sandbox.config_dir`` to ``tmp_path/".codexcrew"`` — keeping the
    ``.codexcrew/run`` layout these tests assert. ``expanduser``/``HOME`` are still
    patched for the non-run-dir ``~`` lookups in this module.
    """
    monkeypatch.setenv("HOME", str(tmp_path))
    (tmp_path / ".codexcrew").mkdir()
    monkeypatch.setattr("os.path.expanduser", lambda p: str(tmp_path) + p[1:] if p.startswith("~") else p)
    monkeypatch.setattr("codex_crew.sandbox.config_dir", lambda: tmp_path / ".codexcrew")
    return tmp_path
...
def _isolated_legacy_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    any stale codexcrew_sandbox_*.py files on the host inflate removal counts.
    Tests that exercise the legacy sweep pass legacy_dir= explicitly.
    """
    empty = tmp_path / "isolated_legacy"
    empty.mkdir(exist_ok=True)
    monkeypatch.setattr("codex_crew.sandbox._LEGACY_LAUNCHER_DIR", str(empty))
    return empty


class TestNamespaceArgvPlacement:
    """namespace_argv() launcher lands in ~/.codexcrew/run/ with PID."""
    ...
    def test_launcher_in_run_dir(self, _mock_detect, fake_home: Path):
        run_dir = fake_home / ".codexcrew" / "run"
    ...
    def test_launcher_has_pid_in_name(self, _mock_detect, fake_home: Path):
        ...
        # Filename pattern: codexcrew_sandbox_{pid}_{random}.py
        assert launcher.name.startswith(f"codexcrew_sandbox_{os.getpid()}_")
...
class TestRunDirMode:
    ...
    def test_makedirs_failure_falls_back(self, fake_home: Path, monkeypatch: pytest.MonkeyPatch):
        ...
        codexcrew_dir = fake_home / ".codexcrew"
        codexcrew_dir.mkdir(parents=True, exist_ok=True)
        # Put a regular file where "run" dir should be
        (codexcrew_dir / "run").write_text("blocker")
...
class TestCleanupSweep:
    ...
    def test_removes_dead_pid_py(self, fake_home: Path):
        ...
        dead_file = run_dir / "codexcrew_sandbox_99999999_abc123.py"
    ...
    def test_removes_dead_pid_sb(self, fake_home: Path):
        run_dir = fake_home / ".codexcrew" / "run"
        run_dir.mkdir(parents=True)
        dead_file = run_dir / "codexcrew_sandbox_99999999_xyz789.sb"
    ...
    def test_removes_old_mtime_live_pid(self, fake_home: Path):
        ...
        live_file = run_dir / f"codexcrew_sandbox_{os.getpid()}_old123.py"
    ...
    def test_keeps_fresh_live_pid(self, fake_home: Path):
        ...
        run_dir = fake_home / ".codexcrew" / "run"
        run_dir.mkdir(parents=True)
        live_file = run_dir / f"codexcrew_sandbox_{os.getpid()}_fresh123.py"
    ...
    def test_keeps_live_pid_sb(self, fake_home: Path):
        run_dir = fake_home / ".codexcrew" / "run"
        run_dir.mkdir(parents=True)
        live_file = run_dir / f"codexcrew_sandbox_{os.getpid()}_live456.sb"
    ...
    def test_overflow_error_resilience(self, fake_home: Path):
        ...
        bad_file = run_dir / f"codexcrew_sandbox_{huge_pid}_x.py"
        bad_file.write_text("# overflow")
        # Also add a normal dead-PID file to confirm sweep continues
        normal_dead = run_dir / "codexcrew_sandbox_99999999_y.py"
    ...
    def test_legacy_sweep_ignores_non_py(self, fake_home: Path, tmp_path: Path):
        ...
        legacy_dir.mkdir()
        non_py = legacy_dir / "codexcrew_sandbox_abc.txt"
    ...
    def test_ignores_nonconforming_filenames(self, fake_home: Path):
        ...
        # Wrong suffix
        f2 = run_dir / "codexcrew_sandbox_99999999_abc.txt"
        f2.write_text("# unrelated")
        # No PID (no underscore separator)
        f3 = run_dir / "codexcrew_sandbox_nopid.py"
        f3.write_text("# unrelated")
        # Right prefix, right suffix, but non-digit PID
        f4 = run_dir / "codexcrew_sandbox_notapid_abc.py"
    ...
    def test_no_run_dir_is_noop(self, fake_home: Path):
        """If ~/.codexcrew/run/ doesn't exist, no crash."""
