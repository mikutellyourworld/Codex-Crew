"""Tests for codex_crew.dashboard.handlers.secrets."""
...
from codex_crew.dashboard.handlers.secrets import _sanitize_for_log, setup_secrets_routes
...
class TestApiSecretsList:
    ...
    async def test_lists_names_sorted(self, vault_dir: Path) -> None:
        ...
        with patch("codex_crew.dashboard.handlers.secrets.config_dir", return_value=str(vault_dir)):
    ...
    async def test_empty_vault(self, empty_vault_dir: Path) -> None:
        ...
        with patch(
            "codex_crew.dashboard.handlers.secrets.config_dir", return_value=str(empty_vault_dir)
        ):
            async with TestClient(TestServer(app)) as client:
                resp = await client.get("/api/secrets")
                assert resp.status == 200
...
class TestApiSecretsSet:
    ...
    async def test_stores_secret(self, tmp_path: Path) -> None:
        ...
        with patch("codex_crew.dashboard.handlers.secrets.config_dir", return_value=str(tmp_path)):
            async with TestClient(TestServer(app)) as client:
...
class TestApiSecretsOwnerAuthorization:
    ...
    async def test_denial_is_audited(self, vault_dir: Path) -> None:
        ...
        sel = MagicMock()
        with (
            patch("codex_crew.dashboard.handlers.secrets.config_dir", return_value=str(vault_dir)),
            patch("codex_crew.dashboard.handlers.secrets._sel", return_value=sel),
        ):
            async with TestClient(TestServer(app)) as client:
                resp = await client.post(
                    "/api/secrets",
...
class TestApiSecretsLogInjection:
    ...
    async def test_newline_in_set_name_is_escaped_in_log(
        ...
        with patch("codex_crew.dashboard.handlers.secrets.config_dir", return_value=str(tmp_path)):
            async with TestClient(TestServer(app)) as client:
                with caplog.at_level(logging.INFO, logger="codex_crew.dashboard.handlers.secrets"):
...
class TestApiSecretsSetInputValidation:
    ...
    async def test_rejects_wrong_types_with_400(
        ...
        with patch(
            "codex_crew.dashboard.handlers.secrets.config_dir",
