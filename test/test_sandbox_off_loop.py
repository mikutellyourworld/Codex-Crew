...
AST guard test: scans ``src/codex_crew/`` for bare ``run_in_executor`` AND
...
from codex_crew.sandbox import (  # noqa: E402
...
class TestSandboxOffLoopCancellation:
    ...
    async def test_cancel_during_hop_drops_launcher(self, tmp_path):
        ...
        with patch("codex_crew.sandbox.sandboxed_spawn_argv", side_effect=fake):
    ...
    async def test_the_callers_executor_is_the_one_used(self):
        ...
        with ThreadPoolExecutor(max_workers=1, thread_name_prefix="pinned-pool") as pool:
            ...
            with patch("codex_crew.sandbox.sandboxed_spawn_argv", side_effect=_fake):
