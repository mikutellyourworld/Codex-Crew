...
from codex_crew.acp.session_handle import AcpSessionHandle, WatchdogSettings
...
from codex_crew.config.loader import WatchdogConfig
...
async def test_working_verdict_never_cancels_tool_at_any_idle():
    ...
    from codex_crew.acp.liveness import ToolCallState
...
def _cfg_with_agent_overrides(monkeypatch, agents: dict) -> None:
    """Patch CodexCrewConfig.load() with a real default config carrying *agents*."""
    from codex_crew.config.loader import CodexCrewConfig

    cfg = CodexCrewConfig()
    cfg.agents = agents
    monkeypatch.setattr(CodexCrewConfig, "load", classmethod(lambda cls: cfg))
...
def test_per_agent_override_narrows_watchdog_snapshot(monkeypatch):
    ...
    _cfg_with_agent_overrides(monkeypatch, {
        "pr-reviewer": CodexCrewAgentConfig(
...
def test_per_agent_override_zero_inherits_global(monkeypatch):
    ...
    _cfg_with_agent_overrides(monkeypatch, {
        "builder": CodexCrewAgentConfig(kiro_agent="builder-kiro"),
    })
...
def test_shared_binding_cannot_collide_canonical_names(monkeypatch):
    ...
    _cfg_with_agent_overrides(monkeypatch, {
        "a": CodexCrewAgentConfig(kiro_agent="shared", watchdog_tool_stall_suspect_secs=60.0),
        "b": CodexCrewAgentConfig(kiro_agent="shared", watchdog_tool_stall_suspect_secs=120.0),
    })
...
async def test_a_real_submission_is_recorded_and_gates_the_next_tick():
    ...
    with (
        patch("codex_crew.acp.session_handle.subprocess_executor", return_value=pool),
        patch("codex_crew.acp.session_handle.asyncio.wait_for", _fast_timeout),
    ):
        assert await handle._consult_oracle_offloaded(model_wait=False) == (
            VERDICT_UNKNOWN,
...
async def test_failed_submission_reports_unknown_without_latching_the_gate():
    ...
    with patch(
        "codex_crew.acp.session_handle.subprocess_executor",
...
async def test_cancelled_consult_still_consumes_a_later_failure():
    ...
    with patch("codex_crew.acp.session_handle.subprocess_executor", return_value=pool):
