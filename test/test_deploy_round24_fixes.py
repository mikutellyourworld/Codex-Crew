"""Regression tests for Round-24 advisories (CodexCrew PR #6).
...
def test_f1_pending_schema_carries_override_flag(tmp_path, monkeypatch):
    from codex_crew.deploy import pending
...
def test_f1_mcp_blocked_path_distinguishes_credential():
    src = (_ROOT / "src/codex_crew/mcp_tools/artifacts.py").read_text(encoding="utf-8")
...
def test_f2_reaper_quarantines_on_every_identity_mismatch():
    src = (
        _ROOT / "src/codex_crew/deploy/skills/artifact-deploy/scripts/"
