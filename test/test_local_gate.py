...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("src/codex_crew/gateway.py", (False, False, True)),
...
def test_changed_files_keeps_both_rename_endpoints(gate, monkeypatch) -> None:
    ...
    def fake_run(argv, **_kwargs):
        ...
        if argv[:2] == ["git", "diff"]:
            ...
            return _Proc("src/codex_crew/gateway.py\n")
        assert "--no-renames" in argv, "porcelain status must not fold renames"
        return _Proc('R  src/codex_crew/moved.py -> temp-screenshots/f/moved.png\n')

    monkeypatch.setattr(gate.subprocess, "run", fake_run)
    paths = gate.changed_files("main")
    assert paths is not None
    assert "src/codex_crew/moved.py" in paths
...
def test_classify_mixed_diff_sets_both_flags(gate) -> None:
    frontend, meta, backend = gate.classify(
        ["website/src/App.tsx", "src/codex_crew/gateway.py"]
    )
...
def test_both_surfaces_runs_full(gate, monkeypatch) -> None:
    monkeypatch.setattr(
        gate, "changed_files",
        lambda base: ["website/src/App.tsx", "src/codex_crew/gateway.py"],
    )
    assert _is_full(gate.build_plan(_args()))
...
def test_backend_only_diff_narrows_frontend(gate, monkeypatch) -> None:
    monkeypatch.setattr(gate, "changed_files", lambda base: ["src/codex_crew/gateway.py"])
    monkeypatch.setattr(
        gate, "selector_must_run",
