"""Additional coverage for ``codex_crew.slack.gateway``.
...
from codex_crew import session_directive
from codex_crew import subagent as _sa
from codex_crew.acp.types import EVENT_COMPLETE, EVENT_TOOL_CALL, EVENT_TOOL_RESULT, AcpEvent
from codex_crew.autonudge import AutoNudgeService, NudgeLoop
from codex_crew.config.loader import CodexCrewConfig
from codex_crew.monitoring import models as monitor_models
...
from codex_crew.session import SessionBusyError, SessionClosingError
...
def _discord_transport(*, authorized: bool = True, current_key: str | None = None) -> MagicMock:
    ...
    dispatcher.current_session_key = MagicMock(
        return_value=current_key if current_key is not None else "discord:codexcrew:direct:U9"
    )
...
_DKEY = "discord:codexcrew:direct:U9"
...
class TestFireDiscordNudge:
    ...
    async def test_unsupported_key_shape_retires_loop(self):
        ...
        assert await orch._fire_discord_nudge(_loop("discord:codexcrew:channel")) is False
    ...
    async def test_rotated_session_retires_loop(self):
        ...
        transport = _discord_transport(current_key="discord:codexcrew:direct:U9:gen2")
        orch = _discord_orchestrator(transport)
        assert await orch._fire_discord_nudge(_loop(_DKEY)) is False
        orch.autonudge_svc.remove.assert_awaited_once_with("loop-1")
    ...
    async def test_structured_delivery_distinguishes_busy_and_unavailable(self):
        ...
        unavailable = _discord_orchestrator(
            _discord_transport(current_key="discord:codexcrew:direct:U9:gen2")
        )
...
class TestFireSlackNudgeGuards:
    ...
    async def test_structured_turn_applies_monitor_stop_directive(
        ...
        orch.autonudge_svc = service
        monkeypatch.setattr("codex_crew.autonudge.get_instance", lambda: service)
...
class TestAutonudgeRouterAndObserver:
    ...
    async def _wire(self, orch: Any):
        with patch("codex_crew.slack.gateway.autonudge_enabled", return_value=True):
            with (
                patch("codex_crew.slack.gateway.AutoNudgeService") as mock_svc,
                patch("codex_crew.monitoring.controller.MonitorController") as mock_controller,
            ):
                inst = MagicMock()
...
class TestInitMcpGateway:
    ...
    async def test_nothing_routed_returns_without_touching_platform_probe(self):
        ...
        with patch("codex_crew.slack.gateway.is_gateway_supported") as probe:
    ...
    async def test_a_routed_server_starts_the_broker_with_sharing_off(self):
        ...
        with patch("codex_crew.slack.gateway.is_gateway_supported", return_value=False) as probe:
            await orch._init_mcp_gateway()
    ...
    async def test_turning_sharing_off_restarts_rather_than_stops_when_routed(self):
        ...
        * it must be a RESTART, not a no-op — the rewriter reads the sharing flag
        ...
        with patch("codex_crew.config.loader.CodexCrewConfig.load", return_value=orch._cfg):
            with (
    ...
    async def test_unsupported_platform_returns_early(self):
        ...
        with patch("codex_crew.slack.gateway.is_gateway_supported", return_value=False):
            with patch("codex_crew.slack.gateway.rewrite_agents") as rewriter:
                await orch._init_mcp_gateway()
    ...
    async def test_rewriter_failure_falls_back_to_per_session_mcp(self, tmp_path):
        ...
        orch._cfg.mcp_gateway.enabled = True
        with (
            patch("codex_crew.slack.gateway.is_gateway_supported", return_value=True),
            patch("codex_crew.slack.gateway.resolve_overlay_dir", return_value=tmp_path / "overlay"),
            patch("codex_crew.slack.gateway.default_socket_path", return_value=tmp_path / "gw.sock"),
            patch("codex_crew.slack.gateway.kiro_agents_dir", return_value=tmp_path / "agents"),
            patch("codex_crew.slack.gateway.rewrite_agents", side_effect=RuntimeError("bad spec")),
            patch("codex_crew.slack.gateway.GatewayManager") as mgr_cls,
        ):
            await orch._init_mcp_gateway()
    ...
    async def test_successful_start_records_the_manager(self, tmp_path):
        ...
        with (
            ...
            patch("codex_crew.slack.gateway.kiro_agents_dir", return_value=tmp_path / "agents"),
            patch(
                "codex_crew.slack.gateway.rewrite_agents",
                return_value=(None, {"MC_MCP_TARGET_X": "1"}),
            ),
            patch("codex_crew.slack.gateway.GatewayManager", return_value=manager),
        ):
            await orch._init_mcp_gateway()
    ...
    async def test_the_started_set_is_recorded_and_the_override_wins(self, tmp_path):
        ...
        with (
            ...
            patch("codex_crew.slack.gateway.default_socket_path", return_value=tmp_path / "gw.sock"),
            patch("codex_crew.slack.gateway.kiro_agents_dir", return_value=tmp_path / "agents"),
            patch("codex_crew.slack.gateway.rewrite_agents", return_value=(None, {})) as rewriter,
            patch("codex_crew.slack.gateway.GatewayManager", return_value=manager),
        ):
    ...
    async def test_the_ready_log_counts_the_served_set_not_the_configured_one(
        ...
        with (
            ...
            patch("codex_crew.slack.gateway.default_socket_path", return_value=tmp_path / "gw.sock"),
            patch("codex_crew.slack.gateway.kiro_agents_dir", return_value=tmp_path / "agents"),
            patch("codex_crew.slack.gateway.rewrite_agents", return_value=(None, {})),
            patch("codex_crew.slack.gateway.GatewayManager", return_value=manager),
        ):
            with caplog.at_level(logging.INFO, logger="codex_crew.slack.gateway"):
                await orch._init_mcp_gateway(stub_servers=frozenset({"alpha-mcp"}))
...
class TestFireDashboardNudgeDispatch:
    ...
    async def test_idle_slot_appends_nudge_and_spawns_a_turn(self, monkeypatch):
        ...
        task = MagicMock()
        monkeypatch.setattr("codex_crew.dashboard.chat._run_chat", MagicMock(return_value="CORO"))
...
class TestDigestChunkSize:
    """``CODEXCREW_SUBAGENT_DIGEST_CHUNK_SIZE`` parse guard: never crash import."""

    def test_default_when_unset(self, monkeypatch):
        monkeypatch.delenv("CODEXCREW_SUBAGENT_DIGEST_CHUNK_SIZE", raising=False)
        assert gw._digest_chunk_size() == 10

    def test_explicit_value_is_honoured(self, monkeypatch):
        monkeypatch.setenv("CODEXCREW_SUBAGENT_DIGEST_CHUNK_SIZE", "25")
        assert gw._digest_chunk_size() == 25

    def test_malformed_value_falls_back_to_default(self, monkeypatch):
        monkeypatch.setenv("CODEXCREW_SUBAGENT_DIGEST_CHUNK_SIZE", "not-a-number")
        assert gw._digest_chunk_size() == 10

    def test_value_is_clamped_to_a_sane_range(self, monkeypatch):
        monkeypatch.setenv("CODEXCREW_SUBAGENT_DIGEST_CHUNK_SIZE", "0")
        assert gw._digest_chunk_size() == 1
        monkeypatch.setenv("CODEXCREW_SUBAGENT_DIGEST_CHUNK_SIZE", "99999")
        assert gw._digest_chunk_size() == 1000


class TestDigestHoldSecs:
    """``CODEXCREW_SUBAGENT_DIGEST_HOLD_SECS`` parse guard (issue #2215): the
    ...
    def test_default_when_unset(self, monkeypatch):
        monkeypatch.delenv("CODEXCREW_SUBAGENT_DIGEST_HOLD_SECS", raising=False)
        assert _sa._digest_hold_secs() == 120.0

    def test_explicit_value_is_honoured(self, monkeypatch):
        monkeypatch.setenv("CODEXCREW_SUBAGENT_DIGEST_HOLD_SECS", "45.5")
        assert _sa._digest_hold_secs() == 45.5

    def test_malformed_value_falls_back_to_default(self, monkeypatch):
        monkeypatch.setenv("CODEXCREW_SUBAGENT_DIGEST_HOLD_SECS", "not-a-number")
        assert _sa._digest_hold_secs() == 120.0

    def test_zero_and_negative_disable_the_deadline(self, monkeypatch):
        monkeypatch.setenv("CODEXCREW_SUBAGENT_DIGEST_HOLD_SECS", "0")
        assert _sa._digest_hold_secs() == 0.0
        monkeypatch.setenv("CODEXCREW_SUBAGENT_DIGEST_HOLD_SECS", "-30")
        assert _sa._digest_hold_secs() == 0.0
    ...
    def test_clamped_to_the_per_agent_hard_ceiling(self, monkeypatch):
        ...
        monkeypatch.setenv("CODEXCREW_SUBAGENT_DIGEST_HOLD_SECS", "999999")
    ...
    def test_nan_is_malformed_input_not_a_deadline(self, monkeypatch):
        ...
        for spelling in ("nan", "NaN", "-nan"):
            monkeypatch.setenv("CODEXCREW_SUBAGENT_DIGEST_HOLD_SECS", spelling)
    ...
    def test_infinity_is_clamped_not_leaked(self, monkeypatch):
        ...
        monkeypatch.setenv("CODEXCREW_SUBAGENT_DIGEST_HOLD_SECS", "inf")
        assert _sa._digest_hold_secs() == float(_sa._TIMEOUT_SECS)
        monkeypatch.setenv("CODEXCREW_SUBAGENT_DIGEST_HOLD_SECS", "-inf")
        assert _sa._digest_hold_secs() == 0.0
