"""Coverage tests for codex_crew.memory — module path helpers, legacy-migration
...
def fake_config_dir(tmp_path, monkeypatch):
    """Point codex_crew.memory's config_dir() at an isolated tmp dir."""
    root = tmp_path / "cfg"
    root.mkdir()
    monkeypatch.setattr("codex_crew.memory.config_dir", lambda: root)
    return root
