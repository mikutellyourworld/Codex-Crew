...
from codex_crew.platform.governance import _SCOPE_ALIASES, SCOPE_CATALOG, parse_policy
...
class TestScopeAttribution:
    ...
    def test_unavailable_response_still_carries_the_field(self, profiles_dir, monkeypatch):
        ...
        import codex_crew.dashboard.handlers.security as sec
...
class TestEndpoint:
    ...
    async def test_display_fails_safe_on_resolution_error(self, profiles_dir, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.platform.governance_profiles.resolve_active_scope", _boom)
        resp = await self._call()
        assert resp.status == 200
        payload = json.loads(resp.body)
