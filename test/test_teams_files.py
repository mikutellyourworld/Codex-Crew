...
from codex_crew.teams.attachments import TEAMS_FILE_DOWNLOAD_INFO
from codex_crew.teams.client import (
    TEAMS_MAX_DOWNLOAD_BYTES,
    TeamsClient,
    TeamsInbound,
    TeamsSendError,
)
from codex_crew.teams.client import resolve_addresses as _real_resolve_addresses
from codex_crew.teams.renderer import TeamsRenderer
from codex_crew.teams.transport import TEAMS_CAPABILITIES, TeamsTransport
...
def _no_real_dns(monkeypatch) -> None:
    ...
    monkeypatch.setattr("codex_crew.teams.client.resolve_addresses", _fake)
...
class TestResolvedAddressVetting:
    ...
    async def test_a_public_name_resolving_inward_is_refused(
        ...
        monkeypatch.setattr("codex_crew.teams.client.resolve_addresses", _resolves_inward)
        client, session = _client([_FakeResponse(chunks=[PNG_BYTES])])
        with pytest.raises(ValueError):
    ...
    async def test_an_unparseable_resolved_address_still_refuses(
        ...
        monkeypatch.setattr("codex_crew.teams.client.resolve_addresses", _garbage)
        client, session = _client([_FakeResponse(chunks=[PNG_BYTES])])
        with pytest.raises(ValueError):
            await client.download_inbound_file("https://harmless.example/dl", str(tmp_path / "f"))
        assert session.calls == []
    ...
    async def test_one_bad_answer_among_several_refuses_the_whole_host(
        ...
        monkeypatch.setattr("codex_crew.teams.client.resolve_addresses", _mixed)
        client, session = _client([_FakeResponse(chunks=[PNG_BYTES])])
        with pytest.raises(ValueError):
    ...
    async def test_an_unresolvable_host_refuses_rather_than_raising_oserror(
        ...
        monkeypatch.setattr("codex_crew.teams.client.resolve_addresses", _fails)
        client, _session = _client([])
    ...
    async def test_a_redirect_hop_is_vetted_too(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.teams.client.resolve_addresses", _second_hop_is_internal)
        client, session = _client(
...
class TestOutboundSeal:
    ...
    def test_a_secret_the_length_cap_would_slice_is_still_caught(self) -> None:
        ...
        from codex_crew.teams.attachments import _MAX_INLINE_NAME_CHARS, inline_image_name
    ...
    async def test_a_plain_answer_never_touches_the_filesystem(self, tmp_path, monkeypatch) -> None:
        ...
        called: list[str] = []
        monkeypatch.setattr(
            "codex_crew.teams.renderer._persisted_upload_root",
    ...
    async def test_the_audit_carries_counts_and_reason_codes_only(
        ...
        fake = _FakeSel()
        monkeypatch.setattr("codex_crew.teams.renderer.sel", lambda: fake)
        chart = tmp_path / "chart.png"
        chart.write_bytes(PNG_BYTES)
...
class TestInboundGating:
    ...
    async def test_a_group_scope_is_denied_before_any_fetch(self, monkeypatch) -> None:
        fake = _FakeSel()
        monkeypatch.setattr("codex_crew.teams.transport.sel", lambda: fake)
        client = _IngestClient()
...
class TestMidTurnFiles:
    ...
    async def test_a_file_only_message_reaches_the_turn(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.teams.transport_dispatch.drive_turn", recorder)
        client = _IngestClient()
    ...
    async def test_a_body_echo_only_activity_runs_no_turn(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.teams.transport_dispatch.drive_turn", _drive)
...
class TestTheSocketDialsTheVettedAddress:
    ...
    async def test_the_resolver_serves_only_what_was_pinned(self) -> None:
        from codex_crew.teams.client import _VettedResolver
    ...
    async def test_the_pin_map_is_bounded(self) -> None:
        ...
        from codex_crew.teams.client import _PINNED_HOSTS_MAX, _VettedResolver
    ...
    async def test_a_host_the_vet_refuses_is_never_pinned(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.teams.client.resolve_addresses", _inward)
