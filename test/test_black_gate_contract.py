...
`AGENTS.md` listed `black src/codex_crew test` as a gate to run before committing.
...
def test_no_document_tells_a_contributor_to_reformat_the_whole_tree(doc: str) -> None:
    # `black src/codex_crew test` is the exact command that buries a diff under
    ...
    for line in text.splitlines():
        ...
        if not stripped.startswith("black "):
            continue
        assert "src/codex_crew test" not in stripped and "src/ test/" not in stripped, (
