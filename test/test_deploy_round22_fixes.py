"""Regression tests for Round-22 findings (CodexCrew PR #6).
...
F2: teardown.sh verifies the stack's codexcrew:site + codexcrew:managed tags
    before delete-stack (the direct-teardown twin of the reaper gate — R21).
"""
from pathlib import Path

_SCRIPTS = Path(__file__).resolve().parents[1] / (
    "src/codex_crew/deploy/skills/artifact-deploy/scripts"
)
...
def test_f2_teardown_gates_stack_deletion_on_identity_tags():
    ...
    assert "codexcrew:site" in src
    assert "codexcrew:managed" in src
    # The tag check precedes the delete-stack call.
    assert src.index("codexcrew:site") < src.index(
        'delete-stack --stack-name "codexcrew-deploy-app-$SLUG"'
    )
