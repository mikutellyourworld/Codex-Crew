...
from codex_crew.dashboard.handlers import weixin_qr as qr
...
def test_stage_config_preserves_unrelated_sections(tmp_path, monkeypatch):
    ...
    cfg.write_text(json.dumps({"slack": {"command": "codexcrew"}, "agent": {"model": "auto"}}))
    ...
    assert data["slack"] == {"command": "codexcrew"}  # untouched
...
#    used to cover this file moved to codex_crew.qr, which made the gap visible.
...
def test_commit_credential_aborts_when_env_lock_is_held(tmp_path, monkeypatch):
    ...
    """
    import codex_crew.platform_compat as _pc
