...
Local backlog f-20260818-08. Everything under ``src/codex_crew/builtin_skills/``
installs on every machine that installs Codex Crew, so a skill body naming THIS
...
SKILL_REL = "src/codex_crew/builtin_skills"
...
class TestTheMarkerSet:
    ...
    @pytest.mark.parametrize(
        "text",
        [
            "File it on mikutellyourworld/Codex-Crew please.",
            "Edit `src/codex_crew/dashboard/state.py`.",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "Contract: [theming](https://github.com/mikutellyourworld/Codex-Crew/blob/main/x.md)",
        ],
    )
    ...
    def test_a_bare_slug_next_to_a_url_is_still_flagged(self, gate) -> None:
        ...
        hits = gate.marker_hits(
            "See https://github.com/mikutellyourworld/Codex-Crew/blob/main/x.md, "
            "then file it on mikutellyourworld/Codex-Crew.\n"
        )
    ...
    @pytest.mark.parametrize(
        "text",
        [
            "Run `codexcrew pod up mypod --provision`.",
            "Scripts must live under `~/.codex-crew/crons/`.",
    ...
    def test_each_hit_names_its_class_and_line(self, gate) -> None:
        ...
        hits = gate.marker_hits("intro\n`src/codex_crew/a.py` and `test/test_a.py`\n")
    ...
    def test_two_markers_on_one_line_both_report(self, gate) -> None:
        assert len(gate.marker_hits("`src/codex_crew/a.py` vs `src/codex_crew/b.py`\n")) == 2
...
class TestTheExemptionIsNotInheritable:
    def test_the_family_is_exempt(self, gate, tmp_path: Path) -> None:
        ...
        _write(tmp_path, "codexcrew-dev/prepare-pr/SKILL.md", "Edit `src/codex_crew/x.py`.\n")
    ...
    def test_a_nested_reference_file_is_exempt_too(self, gate, tmp_path: Path) -> None:
        _write(
            tmp_path,
            "codexcrew-dev/prepare-pr/references/gate-floor.md",
    ...
    def test_a_repository_agnostic_family_is_scanned(self, gate, tmp_path: Path) -> None:
        _write(tmp_path, "widgets/SKILL.md", "Edit `src/codex_crew/x.py`.\n")
    ...
    def test_a_nested_family_directory_does_not_inherit_the_exemption(
        ...
        _write(tmp_path, "widgets/codexcrew-dev/SKILL.md", "Edit `src/codex_crew/x.py`.\n")
        assert list(gate.scan_tree(tmp_path)[0]) == [f"{SKILL_REL}/widgets/codexcrew-dev/SKILL.md"]

    def test_a_look_alike_sibling_does_not_inherit_the_exemption(
        self, gate, tmp_path: Path
    ) -> None:
        _write(tmp_path, "codexcrew-devtools/SKILL.md", "Edit `src/codex_crew/x.py`.\n")
        assert list(gate.scan_tree(tmp_path)[0]) == [f"{SKILL_REL}/codexcrew-devtools/SKILL.md"]

    def test_a_path_outside_the_skill_root_is_never_exempt(self, gate) -> None:
        assert gate.is_exempt(Path("docs/codexcrew-dev/notes.md")) is False
...
class TestAnnotationsCannotBeForged:
    ...
    def test_the_marker_text_is_escaped_too(self, gate, capsys) -> None:
        ...
        assert gate._report({"a.md": [(1, "repo-slug", "mikutellyourworld/Codex-Crew%0A")]}, []) == 1
    ...
    def test_a_real_newline_named_file_is_scanned_and_annotated_safely(
        ...
        (d / "a\n::error::forged.md").write_text("Edit `src/codex_crew/x.py`.\n", encoding="utf-8")
        found, unscannable = gate.scan_tree(tmp_path)
...
class TestTheGateFailsOnAMarker:
    def test_a_single_marker_fails(self, gate, capsys) -> None:
        current = {"a.md": [(1, "checkout-path", "src/codex_crew/a.py")]}
