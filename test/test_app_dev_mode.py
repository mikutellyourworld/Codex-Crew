"""Tests for codex_crew.apps.dev_mode — app dev-mode live reload."""
...
from codex_crew.apps.dev_mode import (
    _read_dev_sentinel,
...
def test_set_dev_mode_rejects_traversal(tmp_path, monkeypatch):
    ...
    would escape ~/.codexcrew/apps/ and read/overwrite an external installed.json.
    """
    _setup_env(tmp_path, monkeypatch)
...
def test_uninstall_clears_dev_sentinel(tmp_path, monkeypatch):
    ...
    """
    from codex_crew.apps.manager import uninstall_app
...
async def test_watch_loop_picks_up_out_of_process_toggle(tmp_path, monkeypatch):
    ...
    Simulates the out-of-process `codexcrew app dev` CLI: the sentinel changes,
...
def test_confirm_flag_refused_when_kernel_reports_confinement(tmp_path, monkeypatch):
    ...
    environment — ``env -u CODEXCREW_SANDBOX_ACTIVE`` does not launder an
