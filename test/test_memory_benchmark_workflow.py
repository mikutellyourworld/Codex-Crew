...
extract each step's script and execute it for real with `git`/`gh`/`codexcrew`
...
def test_the_digest_guard_rejects_a_non_hex_value(tmp_path: Path) -> None:
    ...
    broken = script.replace(
        "DIGEST=\"$(python3 -c 'from codex_crew import embeddings; "
...
def test_first_run_reports_bootstrap_instead_of_a_delta(tmp_path: Path) -> None:
    ...
    stubs = _stub_dir(tmp_path, codexcrew='echo "SHOULD NOT RUN"; exit 1')
...
def test_an_existing_baseline_is_actually_compared(tmp_path: Path) -> None:
    ...
    stubs = _stub_dir(tmp_path, codexcrew='echo "COMPARED $*"')
...
def test_the_download_step_does_not_inherit_the_test_suite_skip_flag() -> None:
    """`CODEXCREW_SKIP_MODEL_DOWNLOAD` exists so unit tests never pull 639 MB.
    ...
    assert step["env"]["CODEXCREW_SKIP_MODEL_DOWNLOAD"] == ""
