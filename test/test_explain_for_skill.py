...
SKILL_DIR = ROOT / "src" / "codex_crew" / "builtin_skills" / "explain-for"
...
def loader(tmp_path: Path) -> SkillsLoader:
    ...
    **``config`` is passed explicitly, and the fixture is function-scoped.**
    ``SkillsLoader(config=None)`` falls back to ``CodexCrewConfig.load()``, which
    resolves ``config_dir()`` — and that CREATES the operator's real data home as
    a side effect (see ``_isolate_codexcrew_home`` in the rootdir ``conftest.py``),
    ...
    return SkillsLoader(
        ...
        config=CodexCrewConfig(skills=SkillsConfig(max_triggered=1)),
    )
