...
from codex_crew.constants import SUBAGENT_COMPLETION_META_KEY
...
class TestMetaHelperShapes:
    ...
    def test_single_carries_outcome_agent_and_task(self) -> None:
        m = single_completion_meta(
            agent_id="a1", outcome=OUTCOME_OK, agent_name="codexcrew", task="add a label"
        )
        assert m == {
            "kind": "single",
            "agentId": "a1",
...
class TestDrainStampsMetaOntoRow:
    ...
    async def test_single_completion_meta_lands_on_the_subagent_row(self, tmp_path) -> None:
        ...
        with patch("codex_crew.dashboard.chat_runner.spawn_guarded_turn") as spawn:
