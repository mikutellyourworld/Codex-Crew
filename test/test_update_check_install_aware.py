...
from codex_crew.platform import update_capability, update_layout, update_provider
from codex_crew.platform.update_provider import CommandProvider, UpdateCheckResult
...
class TestChannelResolution:
    ...
    def test_remediation_command_pins_https(self, monkeypatch):
        ...
        # overridable via CODEXCREW_CDN_BASE. Without --proto '=https' an http://
        ...
        monkeypatch.setenv("CODEXCREW_CDN_BASE", "http://evil.example")
        capability = update_capability.derive_capability(install_root="", dist="wheel")
        assert capability.remediation is not None
    ...
    def test_cdn_override_moves_check_and_command_together(self, monkeypatch):
        monkeypatch.setenv("CODEXCREW_CDN_BASE", "https://cdn.example/")
...
class TestWheelInstallCheck:
    def test_reports_available_against_the_channel_feed(self, monkeypatch):
        ...
        assert info["managed_by"] == "codexcrew"
...
class TestGitCheckoutStillWorks:
    ...
    def _git_install(self, monkeypatch, tmp_path):
        ...
        # running them does not load codex_crew from tmp_path, so the provenance
    ...
    def test_detects_a_prerelease_bump_the_old_comparator_missed(self, _git_install, monkeypatch):
        ...
        assert updates.remediation_command(info) == "codexcrew update"
...
class TestAutoApplyGuard:
            ...
            with patch.object(handlers, "_do_update_check", new_callable=AsyncMock):
                with patch("codex_crew.config.CodexCrewConfig.load", return_value=cfg):
                    with patch(
                        ...
                        with patch("codex_crew.slack.gateway.distribution", return_value=dist):
    ...
    def test_a_failed_check_does_not_claim_up_to_date(self, capsys):
        orch = self._run(
            {"update_available": None, "error_code": "feed_unreachable", "managed_by": "codexcrew"},
            auto_update=True,
        )
        orch._auto_apply_update.assert_not_awaited()
