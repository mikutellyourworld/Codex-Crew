...
from codex_crew.auth.login import portal
...
from codex_crew.auth.shape import Transport
from codex_crew.auth.store import TokenStore
...
async def test_loopback_timeout_reports_expired_with_fallback_code(
    ...
    monkeypatch.setattr("codex_crew.auth.service.LOOPBACK_TIMEOUT_SECS", 0.05)
    service, _ = _service(tmp_path)
    begin = await service.begin_loopback("google")
    result = await _poll_until_terminal(service, begin["login_id"])
