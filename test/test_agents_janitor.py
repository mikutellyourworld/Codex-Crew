...
Covers the hard safety contract in :mod:`codex_crew.agents_janitor`:
...
from codex_crew.agents_janitor import (
...
def test_sweep_survives_zeroed_dirent_identity(tmp_path: Path, monkeypatch) -> None:
    ...
    """
    import codex_crew.agents_janitor as aj
...
def test_removes_aged_own_mkstemp_residue(tmp_path: Path) -> None:
    # Codex Crew's own _atomic_json_write leaves tmp<random>.tmp on a crash.
...
def test_backups_left_alone_when_gated_off(tmp_path: Path) -> None:
    ...
    # well past the backup window, because their retention is not Codex Crew's to
