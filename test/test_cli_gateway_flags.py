"""Tests for `codexcrew gateway` composable CLI flags.
...
from codex_crew.cli import _resolve_gateway_args
...
def _ns(**kwargs) -> argparse.Namespace:
    ...
    Mirrors what the CLI parser produces for `codexcrew gateway`. Tests
...
class TestApprovalYoloSafetyRail:
    ...
    def test_yolo_refused_without_codexcrew_home(self, monkeypatch, capsys):
        monkeypatch.delenv("CODEXCREW_HOME", raising=False)
        with pytest.raises(SystemExit) as exc:
            _resolve_gateway_args(_ns(approval="yolo"))
        assert exc.value.code == 2
        captured = capsys.readouterr()
        assert "CODEXCREW_HOME must be explicitly set" in captured.err

    def test_yolo_refused_when_codexcrew_home_empty_string(self, monkeypatch, capsys):
    ...
    def test_yolo_refused_when_resolves_to_default_home(self, monkeypatch, capsys):
        # Point CODEXCREW_HOME at the literal default; rail must catch it.
        monkeypatch.setenv("CODEXCREW_HOME", str(Path.home() / ".kiro" / "crew"))
        with pytest.raises(SystemExit) as exc:
            _resolve_gateway_args(_ns(approval="yolo"))
        assert exc.value.code == 2
        captured = capsys.readouterr()
    ...
    def test_yolo_refused_via_tilde_expansion(self, monkeypatch, capsys):
        # `~/.codex-crew` expands then resolves to the same path as
        # Path.home() / .kiro / crew.
        monkeypatch.setenv("CODEXCREW_HOME", "~/.codex-crew")
        with pytest.raises(SystemExit) as exc:
            _resolve_gateway_args(_ns(approval="yolo"))
        assert exc.value.code == 2
    ...
    def test_yolo_refused_when_resolves_to_legacy_home(self, monkeypatch, capsys):
        ...
        # LEGACY ~/.codexcrew too, not just ~/.codex-crew. On an unmigrated/downgraded
        # install the legacy home still holds the LIVE data, so yolo against it is
        # exactly as destructive as against the new home.
        monkeypatch.setenv("CODEXCREW_HOME", str(Path.home() / ".codexcrew"))
        with pytest.raises(SystemExit) as exc:
            _resolve_gateway_args(_ns(approval="yolo"))
        assert exc.value.code == 2
        captured = capsys.readouterr()
