...
class TestResolveContradictions:
    ...
    async def test_contradictory_verdict_returns_key(self):
        from codex_crew.dashboard.handlers.cron import _resolve_contradictions
    ...
    async def test_rejects_tool_calls_and_still_classifies(self):
        ...
        verdict still lands."""
        from codex_crew.dashboard.handlers import cron
        ...
        with patch("codex_crew.llm_helpers._sel") as mock_sel:
...
class TestResolveAndSupersede:
    ...
    async def test_deletes_contradicted_keys(self):
        from codex_crew.dashboard.handlers.cron import _resolve_and_supersede

        state = MagicMock()
        vs = MagicMock()
        candidates = [{"key": "lesson.old", "rule": "Use X", "similarity": 0.6}]
        with patch(
            "codex_crew.dashboard.handlers.cron._resolve_contradictions",
...
class TestLessonStorageShape:
    ...
    def test_the_envelope_does_not_shrink_accepted_rule_capacity(self, tmp_path):
        ...
        see it)."""
        from codex_crew.vector_memory import _MAX_VALUE_BYTES
...
class TestNormalizeLessonCategory:
    ...
    def test_strict_clamps_unknown_label_to_knowledge(self):
        from codex_crew.validation import normalize_lesson_category
