...
from codex_crew.dashboard.chat_folders import _KNOWN_INTERNAL_CALLERS
...
def recorded(monkeypatch: Any) -> _RecordingSel:
    rec = _RecordingSel()
    monkeypatch.setattr("codex_crew.dashboard.chat_folders.sel", lambda: rec)
    return rec
...
class TestFolderAuditOrigin:
    ...
    async def test_caller_header_without_secret_is_still_dashboard(
        ...
        try:
            resp = await client.post(
                "/api/chat/folders",
                json={"name": "Sneaky"},
                headers={"X-Internal-Caller": "codexcrew-dashboard"},
            )
            assert resp.status == 201
        finally:
            await client.close()
    ...
    async def test_mcp_create_is_audited_as_declared_caller(
        ...
        try:
            resp = await client.post(
                ...
                headers={
                    ...
                    "X-Internal-Caller": "codexcrew-dashboard",
                },
            )
            assert resp.status == 201
        finally:
        ...
        assert event["caller"] == "codexcrew-dashboard"
    ...
    async def test_internal_write_without_caller_header_is_unknown_internal(
        ...
        client = await _client(state)
        try:
            with caplog.at_level(logging.WARNING, logger="codex_crew.dashboard.chat_folders"):
                resp = await client.post(
                    "/api/chat/folders",
                    json={"name": "Mystery"},
...
class TestKnownCallerRatchet:
    def test_known_internal_callers_exact_list(self) -> None:
        ...
        that conscious edit.
        """
        assert _KNOWN_INTERNAL_CALLERS == frozenset({"codexcrew-dashboard"})
    ...
    def test_dashboard_server_name_is_a_known_caller(self) -> None:
        ...
        ``unknown-internal``."""
        from codex_crew.mcp_dashboard import SERVER_NAME
