...
def _setup_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "codex_crew.apps.routes.config_path",
        lambda: str(cfg),
    )
    # Mock SEL
    mock_sel = MagicMock()
    monkeypatch.setattr("codex_crew.apps.routes.sel", lambda: mock_sel)
...
class TestPutRegistries:
    ...
    async def test_new_url_host_emits_trust_grant_event(self, tmp_path, monkeypatch):
        ...
        _setup_env(tmp_path, monkeypatch)
        from codex_crew.apps import routes as routes_mod
...
class TestPutRegistriesValidation:
    ...
    async def test_rejects_blocked_repo(self, tmp_path, monkeypatch):
        ...
        async with TestClient(TestServer(_make_app())) as client:
            resp = await client.put(
                "/api/apps/registries",
                json={"registries": [{"repo": "CodexCrew"}]},
            )
            assert resp.status == 400
            data = await resp.json()
