...
Found while re-measuring: the baseline log carried
    ...
    WARNING codex_crew.vector_memory: Episodic write rejected: blocked content patterns
...
from codex_crew.eval.bench.ingest import IngestConfig, IngestReport, ingest_instance
from codex_crew.vector_memory import _contains_injection
...
def test_the_warning_names_the_injection_screen_and_not_only_dedup() -> None:
    ...
    from codex_crew.eval.bench.run import _ingest_warnings
...
def test_the_real_corpus_rate_is_the_documented_one() -> None:
    ...
    from codex_crew.eval.bench.adapters.locomo import load_locomo_file
    from codex_crew.eval.bench.datasets import cache_dir
    from codex_crew.eval.bench.ingest import fragment_text
