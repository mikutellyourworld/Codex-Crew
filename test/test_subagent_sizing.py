...
from codex_crew.subagent import compute_max_subagents, resolve_max_subagents
...
def _no_learned_cost(monkeypatch):
    """Isolate from the machine's learned-cost store (~/.codexcrew/subagents/
...
def _cfg(
    *,
    ...
    """Minimal duck-typed stand-in for CodexCrewConfig (agent + session)."""
...
class TestQueuedDepthWiring:
    ...
    def test_spawn_queue_branch_emits_depth(self, monkeypatch) -> None:
        ...
        import codex_crew.subagent as sub
...
class TestSampleLiveCosts:
    ...
    def _agent(self):
        ...
        info = SubagentInfo(id="a1", task="t", agent="codexcrew")
        info._pid = 4242
        return info
    ...
    def _sample(rss_kb: int = -1, jiffies: int = 0):
        ...
        from codex_crew.platform_compat import SubtreeSample
    ...
    def test_session_shared_agents_record_averaged_share(self, monkeypatch) -> None:
        ...
        a = SubagentInfo(id="a1", task="t", agent="codexcrew")
        a._pid = 4242
        a._session_sharing = True
        b = SubagentInfo(id="a2", task="t", agent="codexcrew")
        b._pid = 4242
...
class TestReadIntFile:
    def test_reads_int(self, tmp_path) -> None:
        from codex_crew.subagent import _read_int_file
...
class TestLastSampleAndMemoryRows:
    ...
    def _agent(self, **kw):
        ...
        info = SubagentInfo(id=kw.pop("id", "a1"), task=kw.pop("task", "t"), agent="codexcrew")
        info._pid = 4242
        for k, v in kw.items():
            setattr(info, k, v)
        return info
