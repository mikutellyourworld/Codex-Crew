...
import codex_crew
from codex_crew import jsonl_util, platform_compat
from codex_crew.image_artifacts import MAX_IMAGE_BYTES_PER_MESSAGE
...
class TestBoundedRecordReaders:
    ...
    def test_skip_reader_logs_once_and_keeps_going(self, tmp_path, caplog):
        ...
        with caplog.at_level(logging.DEBUG, logger="codex_crew.jsonl_util"):
            with open(path, "rb") as fh:
...
class TestNoUnboundedHandleIteration:
    ...
    def test_every_handle_iteration_is_bounded_or_excused(self):
        src = Path(codex_crew.__file__).parent
