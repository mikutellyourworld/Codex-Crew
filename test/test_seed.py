"""Tests for ``codex_crew.seed`` — Phase 1.A walking skeleton.

Scope: one fixture (``empty``), one guardrail (``CODEXCREW_HOME`` unset), plus a
...
# A test here spawns a real `python -m codex_crew gateway --help` child interpreter;
...
def test_seed_empty_happy_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """``codexcrew seed --fixture empty`` writes fixture.yaml into $CODEXCREW_HOME."""
    # copytree refuses an existing dst, so don't pre-create it.
    target = tmp_path / "home"
    monkeypatch.setenv("CODEXCREW_HOME", str(target))
...
def test_seed_unset_home_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    """Unset $CODEXCREW_HOME raises SeedError with exit code 2."""
    ...
    assert "CODEXCREW_HOME" in str(excinfo.value)
...
def test_seed_path_traversal_rejected(
    ...
    ``copytree`` the entire ``tests_fixtures/`` tree into ``$CODEXCREW_HOME``.
...
def test_seed_cli_flag_registered(tmp_path: Path) -> None:
    ...
    env["CODEXCREW_PROJECT_DIR"] = str(repo_root)

    result = subprocess.run(
        [sys.executable, "-m", "codex_crew", "gateway", "--help"],
        env=env,
        capture_output=True,
        text=True,
        timeout=30,
...
def test_seed_cmd_emits_sel_audit_on_success(
    ...
    # ``$CODEXCREW_HOME`` value (per pre-review finding on leakage).
...
def test_seed_cmd_emits_sel_audit_on_rail_denied(
    ...
    """Unset $CODEXCREW_HOME guardrail emits ``outcome="denied"`` with ``guardrail`` tag."""
    monkeypatch.delenv("CODEXCREW_HOME", raising=False)
...
@patch("codex_crew.seed.sel", side_effect=OSError("read-only HOME"))
...
def test_seed_cmd_safe_audit_logs_warning_on_swallowed_failure(
    ...
    ``codex_crew.seed`` whenever ``sel()`` raises.  Regression guard
    ...
    args = type("Args", (), {"seed": "empty"})()
    with caplog.at_level(logging.WARNING, logger="codex_crew.seed"):
        rc = seed_mod.seed_cmd(args)
    ...
    audit_records = [
        ...
        if r.name == "codex_crew.seed" and r.levelno == logging.WARNING
    ]
...
def test_seed_main_home_rail_refuses(
    ...
    """``$CODEXCREW_HOME=~/.codexcrew`` exits 2 with 'refusing to seed main
    gateway home' message, even when the path doesn't exist yet.

    PRD acceptance test 4. Monkeypatches ``$HOME`` so ``Path.home() /
    '.codexcrew'`` points into ``tmp_path`` — catching developers who set
    ``CODEXCREW_HOME`` to their real main home would be the worst possible
    test failure mode.
    """
    fake_home = tmp_path / "fake_home"
    fake_home.mkdir()
    monkeypatch.setenv("HOME", str(fake_home))
    target = fake_home / ".codexcrew"
    monkeypatch.setenv("CODEXCREW_HOME", str(target))
...
def test_seed_new_home_rail_refuses(
    ...
    """``$CODEXCREW_HOME=~/.codex-crew`` (the post-move default home) is refused
    too — the guardrail protects the CURRENT main gateway home, not just the
    pre-move legacy ``~/.codexcrew``.
    """
    fake_home = tmp_path / "fake_home"
    fake_home.mkdir()
    monkeypatch.setenv("HOME", str(fake_home))
...
def test_seed_main_home_rail_refuses_even_with_replace(
    ...
    test of this CR — ``--seed-replace`` on ``$CODEXCREW_HOME=~/.codexcrew``
...
def test_seed_main_home_rail_catches_symlink(
    ...
    """Symlinked ``$CODEXCREW_HOME`` resolving to ``~/.codexcrew`` is caught.

    PRD acceptance test 6. Developer might symlink ``~/dev-home -> ~/.codexcrew``
    and point ``$CODEXCREW_HOME`` at the symlink; the resolved comparison
    must still hit the main-home guardrail.
    """
    fake_home = tmp_path / "fake_home"
    fake_home.mkdir()
    real_main = fake_home / ".codexcrew"
    real_main.mkdir()
    (real_main / "user_data.txt").write_text("preserved")
    symlinked_target = fake_home / "dev-home"
    symlinked_target.symlink_to(real_main)
    monkeypatch.setenv("HOME", str(fake_home))
    monkeypatch.setenv("CODEXCREW_HOME", str(symlinked_target))
...
def test_seed_replace_refuses_symlinked_target(
    ...
    """``--seed-replace`` on a symlinked $CODEXCREW_HOME refuses, preventing
    ...
    case where $CODEXCREW_HOME symlinks to a non-main-home directory.
    """
    ...
    monkeypatch.setenv("CODEXCREW_HOME", str(link))
...
def test_seed_refuses_symlinked_nonempty_target_without_replace(
    ...
    """Symlinked non-empty $CODEXCREW_HOME fails fast with SYMLINK_REPLACE —
...
def test_seed_empty_existing_dir_succeeds_without_replace(
    ...
    """An empty pre-existing $CODEXCREW_HOME is accepted (no --seed-replace needed).
...
def test_seed_resolve_failure_fails_closed(
    ...
    live gateway state on a broken-symlink-chain ``$CODEXCREW_HOME``.
    ...
    assert "cannot resolve $CODEXCREW_HOME" in str(excinfo.value)
...
def test_seed_audit_uses_rail_tag_not_raw_path(
    ...
    with patch("codex_crew.seed.sel") as mock_sel:
        if setup == "main_home":
            fake_home = tmp_path / "fake_home"
            fake_home.mkdir()
            target = fake_home / ".codexcrew"
...
def test_seed_regular_file_target_rejected(
    ...
    """``$CODEXCREW_HOME`` pointing at a regular file raises GUARDRAIL_NOT_A_DIRECTORY.

    Regression guard (review nit #43): before this fix, a
    regular-file ``$CODEXCREW_HOME`` reached ``dst.iterdir()`` and raised a
    ...
    monkeypatch.setenv("CODEXCREW_HOME", str(target_file))
...
def test_seed_empty_symlink_target_rejected(
    ...
    """Empty-but-existing symlinked ``$CODEXCREW_HOME`` raises GUARDRAIL_SYMLINK_EMPTY.
...
def test_seed_symlink_to_file_rejected(
    ...
    """``$CODEXCREW_HOME`` as a symlink pointing at a regular file raises
...
def test_seed_dangling_symlink_rejected(
    ...
    """``$CODEXCREW_HOME`` as a symlink to a nonexistent target raises
