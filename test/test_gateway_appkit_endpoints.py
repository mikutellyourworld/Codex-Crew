...
from codex_crew.apps.manager import APP_MANIFEST_FILENAME
from codex_crew.apps.routes import register_app_routes
from codex_crew.dashboard import chat_persistence, chat_runner
from codex_crew.dashboard.chat import api_chat_slot_context, api_chat_slot_note
from codex_crew.dashboard.chat_handlers import _MAX_DEFERRED_NOTES
from codex_crew.dashboard.chat_persistence import save_slot_off_loop
from codex_crew.dashboard.chat_runner import drain_pending_context
from codex_crew.dashboard.chat_utils import (
    _history_key_for,
    effective_session_key,
    slot_history_key,
)
from codex_crew.dashboard.handlers import api_mcp_server_detail
from codex_crew.dashboard.state import _MAX_PENDING_CONTEXT, DashboardState, _ChatSlot
...
def mcp_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.mcp._GLOBAL_MCP_JSON", mcp_json
    )
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.mcp._MCP_LOCK_PATH",
        mcp_json.with_suffix(".lock"),
    )
    # Stub _sync_mcp_to_agent to avoid touching real agent config
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.mcp._sync_mcp_to_agent",
        lambda *a, **kw: None,
    )
    return mcp_json
...
class TestReverseProxy:
    ...
    def _proxy_env(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
        ...
        bmod._allocated_ports.clear()
        # Clear secret cache
        from codex_crew.apps.routes import _app_secret_cache
    ...
    async def test_path_traversal_rejected(self, monkeypatch):
        ...
        from codex_crew.apps.routes import handle_app_api_proxy
    ...
    async def test_disabled_app_proxy_rejected(self):
        ...
        with patch("codex_crew.apps.routes.sel") as mock_sel:
            resp = await handle_app_api_proxy(request)
    ...
    async def test_same_app_token_not_rejected_by_cross_app_guard(self, monkeypatch):
        ...
        import codex_crew.apps.routes as rmod
        from codex_crew.apps.routes import handle_app_api_proxy
    ...
    async def test_hmac_header_present_and_valid(self, monkeypatch):
        """Proxy request includes X-CodexCrew-Proxy with valid HMAC."""
        import hashlib
        ...
        try:
            ...
            proxy_header = received_headers.get("x-codexcrew-proxy", "")
            assert proxy_header, "X-CodexCrew-Proxy header missing"
    ...
    async def test_hmac_includes_percent_encoded_query_string_with_spaces(self, monkeypatch):
        ...
        async def echo_handler(request: web.Request) -> web.Response:
            for k, v in request.headers.items():
                received_headers[k.lower()] = v
            auth_hdr = request.headers.get("x-codexcrew-proxy", "")
...
class TestSSRFGuard:
    ...
    def test_rejects_gateway_own_port(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
        ...
        manifest = {
            ...
            "mcpServers": {
                "self-ref": {"url": "http://localhost:5486/api/lessons"}
            },
        }
        (app_dir / APP_MANIFEST_FILENAME).write_text(json.dumps(manifest))
        installed = {
        ...
        from codex_crew.apps.routes import _resolve_app_backend_url
...
class TestNoteEndpoint:
    ...
    async def test_app_token_denied_on_an_unbound_channel_origin_slot(self, tmp_path: Path):
        ...
        events: list[dict] = []
        with patch(
            "codex_crew.dashboard.chat_handlers.sel",
    ...
    def test_every_turn_start_seam_flushes_above_its_own_row(self):
        ...
        from codex_crew.dashboard import chat_orchestrator, chat_runner
    ...
    def test_every_queue_drain_seam_flushes_before_starting_the_successor(self):
        ...
        root = Path(codex_crew.__file__).resolve().parent
    ...
    async def test_the_stage_exit_leaves_a_note_held_while_a_turn_runs(
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_orchestrator._run_chat", _mock_run_chat
        )
    ...
    def _stage_slot(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, timeout: int = 0):
        ...
        from codex_crew.dashboard.chat_orchestrator import OrchestrationTracker
    ...
    async def test_a_cancelled_stage_still_writes_a_held_note(
        ...
        """
        from codex_crew.dashboard import chat_orchestrator
        from codex_crew.dashboard.chat import _stage_loop
...
class TestCleanupPersistsHeldNotes:
    ...
    async def test_cleanup_does_not_feed_a_held_note_to_the_doomed_turn(
        ...
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        monkeypatch.setattr(
            "codex_crew.autonudge.get_instance",
    ...
    async def test_a_failed_archive_reports_the_turn_it_already_cancelled(
        ...
        try:
            with patch(
                "codex_crew.dashboard.chat_handlers.save_slot_off_loop", new=_failing_save
            ):
                async with self._make_client(state) as client:
                    resp = await client.post(
                        "/api/chat/slots/cleanup", json={"max_inactive_days": 3}
...
class TestPersistenceRebindDenialIsAudited:
    ...
    def test_a_save_path_rebind_drop_records_a_sel_denial(
        ...
        with patch(
            "codex_crew.dashboard.chat_persistence.sel",
            lambda: SimpleNamespace(log_api_access=lambda **kw: events.append(kw)),
        ):
...
class TestUninstallAppSourcesCleanup:
    ...
    async def test_registry_app_sources_cleaned(self):
        ...
        # ~/.codexcrew/app-sources/{name}/ holding a checked-out repo).
...
class TestRegistryInstallStream:
    ...
    async def test_catalog_outage_streams_fail_closed_error(
        ...
        monkeypatch.setattr(
            "codex_crew.apps.official_catalog.inventory_for_install", _outage,
        )
        async with self._make_client() as client:
            resp = await client.post(
                "/api/apps/registry/install-stream",
    ...
    async def test_streams_log_lines_then_done(
        ...
        monkeypatch.setattr(
            "codex_crew.apps.routes.install_from_registry", _fake_install,
        )
        # Stub register_app to avoid touching real bridges
        monkeypatch.setattr(
            "codex_crew.apps.routes.register_app",
