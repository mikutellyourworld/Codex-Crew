...
def pid_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr("codex_crew.session_pid._pid_file_path", lambda: p)
    return p
...
def session_pid_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr("codex_crew.session_pid._session_pid_file_path", lambda: p)
    return p


class TestTrackUntrack:
    def test_track_pid_creates_file(self, pid_file: Path) -> None:
        from codex_crew.session_pid import _track_pid
    ...
    def test_untrack_pid(self, pid_file: Path) -> None:
        from codex_crew.session_pid import _track_pid, _untrack_pid
    ...
    def test_untrack_session_pid(
        ...
        """
        from codex_crew.session_pid import _track_session_pid, _untrack_session_pid

        monkeypatch.setattr("codex_crew.session_pid._pid_start_token", lambda p: token)
    ...
    def test_untrack_session_pid_missing_file(self, session_pid_file: Path) -> None:
        from codex_crew.session_pid import _untrack_session_pid
    ...
    def test_track_child_pids_with_parent(self, pid_file: Path) -> None:
        from codex_crew.session_pid import _track_child_pids
    ...
    def test_untrack_child_pids(self, pid_file: Path) -> None:
        from codex_crew.session_pid import _track_child_pids, _untrack_child_pids
    ...
    def test_untrack_child_pids_preserves_bare_pid(self, pid_file: Path) -> None:
        ...
        from codex_crew.session_pid import _track_child_pids, _track_pid, _untrack_child_pids
...
class TestCleanupOrphanedMcpServers:
    def test_dead_child_pruned(self, pid_file: Path) -> None:
        ...
        from codex_crew.session_pid import _cleanup_orphaned_mcp_servers
    ...
    def test_alive_child_with_alive_parent_survives(self, pid_file: Path) -> None:
        ...
        with patch("codex_crew.session_pid.platform_compat.pid_exists", side_effect=fake_pid_exists):
    ...
    def test_alive_child_with_dead_parent_killed(self, pid_file: Path) -> None:
        ...
        with (
            patch("codex_crew.session_pid.platform_compat.pid_exists", side_effect=fake_pid_exists),
            patch("codex_crew.session_pid.platform_compat.kill_pid"),
            patch("codex_crew.platform_compat.get_ppid", return_value=1),
        ):
            killed = _cleanup_orphaned_mcp_servers()
    ...
    def test_alive_child_with_dead_parent_pid_reused(self, pid_file: Path) -> None:
        ...
        with (
            patch("codex_crew.session_pid.platform_compat.pid_exists", side_effect=fake_pid_exists),
            patch("codex_crew.platform_compat.get_ppid", return_value=5555),
        ):
            killed = _cleanup_orphaned_mcp_servers()
    ...
    def test_bare_pid_dead_pruned(self, pid_file: Path) -> None:
        ...
        with patch("codex_crew.session_pid.platform_compat.pid_exists", return_value=False):
            killed = _cleanup_orphaned_mcp_servers()
        assert killed == 0
        assert "99999" not in pid_file.read_text(encoding="utf-8")
    ...
    def test_bare_pid_alive_kept(self, pid_file: Path) -> None:
        ...
        with patch("codex_crew.session_pid.platform_compat.pid_exists", return_value=True):
            killed = _cleanup_orphaned_mcp_servers()
        assert killed == 0
...
class TestCleanupOrphanedSessions:
    ...
    def test_preserves_non_kiro_pids(self, session_pid_file: Path) -> None:
        ...
        from codex_crew.session_pid import cleanup_orphaned_sessions
        ...
        with (
            patch(
                "codex_crew.session_pid._is_managed_agent_process", side_effect=lambda p: p == 99998
            ),
            patch("codex_crew.session_pid._cleanup_orphaned_mcp_servers", return_value=0),
            patch("codex_crew.session_pid._pid_in_spawn_grace", return_value=False),
            patch(
                "codex_crew.session_pid.platform_compat.pid_liveness",
                return_value=platform_compat.PID_ALIVE,
            ),
            patch("codex_crew.session_pid.platform_compat.pid_exists", return_value=True),
            patch("codex_crew.session_pid.platform_compat.kill_pid"),
        ):
    ...
    def test_kiro_pids_killed(self, session_pid_file: Path) -> None:
        ...
        with (
            patch("codex_crew.session_pid._is_managed_agent_process", return_value=True),
            ...
            patch("codex_crew.session_pid.platform_compat.pid_exists", return_value=True),
            patch("codex_crew.session_pid.platform_compat.kill_pid", side_effect=fake_kill),
            patch("codex_crew.session_pid._pid_in_spawn_grace", return_value=False),
            patch("codex_crew.session_pid._cleanup_orphaned_mcp_servers", return_value=0),
        ):
            cleanup_orphaned_sessions()
    ...
    def test_malformed_pid_files_deleted(
        ...
        monkeypatch.setattr("codex_crew.session_pid.config_dir", lambda: tmp_path)
...
class TestResetStateUntracksParentPid:
    def test_reset_state_untracks_parent_pid(self) -> None:
        ...
        with patch("codex_crew.session._untrack_pid") as mock_untrack:
            client._reset_state()
...
class TestFindOrphanMcpCandidates:
    ...
    def test_excludes_pids_in_active_set(self) -> None:
        ...
        from codex_crew.session_pid import find_orphan_mcp_candidates

        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[100, 200]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=b"codexcrew_sandbox_abc.py"),
        ):
    ...
    def test_vanished_pid_logs_one_line_without_traceback(
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[22620]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(
                Path,
                "read_bytes",
            ...
            caplog.at_level("DEBUG", logger="codex_crew.session_pid"),
        ):
            mock_sys.platform = "linux"
    ...
    def test_vanished_pid_on_macos_ps_exit_logs_no_traceback(
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[9140]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch(
                "codex_crew.session_pid.subprocess.check_output",
    ...
    def test_unexpected_probe_error_keeps_traceback(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[555]),
            patch("codex_crew.session_pid.sys") as mock_sys,
    ...
    def test_excludes_non_codexcrew_processes(self) -> None:
        """Orphans without known MCP entrypoint markers are skipped."""
        from codex_crew.session_pid import find_orphan_mcp_candidates

        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[300]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(
                Path, "read_bytes", return_value=b"/usr/bin/python3\x00some_other_script.py"
            ),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=300.0),
        ):
            mock_sys.platform = "linux"
            result = find_orphan_mcp_candidates(active_pids=set())
    ...
    def test_excludes_non_entrypoint_vim_grep(self) -> None:
        """Non-Python processes mentioning codexcrew in args (e.g. vim, grep) are skipped."""
        from codex_crew.session_pid import find_orphan_mcp_candidates

        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[350]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=b"vim\x00/tmp/codexcrew_sandbox_abc.log"),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=300.0),
        ):
            mock_sys.platform = "linux"
    ...
    def test_excludes_peer_gateway(self, tmp_path: Path) -> None:
        ...
        cmdline = (
            b"python3\x00-m\x00codex_crew.mcp_gateway.gatewayd"
            b"\x00--socket\x00" + os.fsencode(str(live_sock))
        )
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[360]),
            patch("codex_crew.session_pid.sys") as mock_sys,
    ...
    def test_includes_codexcrew_orphan_not_in_active(self) -> None:
        """Orphaned process with sandbox wrapper entrypoint and not in active set is a candidate."""
        from codex_crew.session_pid import find_orphan_mcp_candidates

        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[400]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(
                Path,
                "read_bytes",
                return_value=b"python3\x00/tmp/codexcrew_sandbox_xyz.py",
            ),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=300.0),
        ):
    ...
    def test_excludes_own_pid(self) -> None:
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[999]),
    ...
    def test_does_not_match_builder_mcp(self) -> None:
        """builder-mcp is NOT a CodexCrew-spawned process in this public fork.
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[410]),
            patch("codex_crew.session_pid.sys") as mock_sys,
    ...
    def test_matches_macos_space_separated_cmdline(self) -> None:
        ...
        def mock_check_output(cmd, **kwargs):
            ...
            if "etime=" in cmd and "command=" in cmd:
                return b"   05:00 python3 /tmp/codexcrew_sandbox_xyz.py"
            return b""

        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[420]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch(
    ...
    def test_skips_young_processes(self) -> None:
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[450]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(
                Path,
                "read_bytes",
                return_value=b"python3\x00/tmp/codexcrew_sandbox_new.py",
            ),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=50.0),
        ):
            mock_sys.platform = "linux"
            result = find_orphan_mcp_candidates(active_pids=set())
...
class TestKillOrphanMcps:
    ...
    def _stable_root_identity(self) -> Iterator[None]:
        ...
        """
        with patch("codex_crew.session_pid._pid_start_token", return_value="tok-stable"):
            yield

    def test_uses_killpg_when_pgid_differs(self) -> None:
        """If orphan is its own group leader, kill via killpg."""
        from codex_crew.session_pid import kill_orphan_mcps
        ...
        with (
            ...
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=b"python3\x00codexcrew_sandbox_x.py"),
        ):
            mock_sys.platform = "linux"
    ...
    def test_respects_max_kill_cap(self) -> None:
        ...
        from codex_crew.session_pid import _ORPHAN_SWEEP_MAX_KILLS, kill_orphan_mcps
    ...
    def test_macos_subprocess_error_does_not_abort_loop(self) -> None:
        ...
        def mock_check_output(cmd, **kwargs):
            ...
            return b"python3 /tmp/codexcrew_sandbox_x.py"
...
class TestParseEtime:
    ...
    def test_minutes_seconds(self) -> None:
        from codex_crew.session_pid import _parse_etime
...
class TestOurOrphanPids:
    ...
    def test_linux_proc_scan_finds_init_and_subreaper_children(self) -> None:
        ...
        """
        from codex_crew.session_pid import _our_orphan_pids
        ...
        with (
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch("codex_crew.session_pid.Path", return_value=proc_root),
...
class TestLinuxPidAge:
    ...
    def _patch_proc(stat_line: str, uptime: str = "10000.0 9000.0"):
        ...
        return patch("codex_crew.session_pid.Path", side_effect=fake_path)
    ...
    def test_age_with_spaces_and_parens_in_comm(self) -> None:
        ...
        """
        from codex_crew.session_pid import _linux_pid_age
...
class TestIsManagedAgentProcess:
    def test_self_pid_not_managed(self) -> None:
        ...
        """
        from codex_crew.session_pid import _is_managed_agent_process
...
class TestSyncKillProvider:
    def test_no_pid_returns_early(self) -> None:
        ...
        from codex_crew.session_pid import _sync_kill_provider
        ...
        with patch("codex_crew.session_pid.platform_compat.kill_pid") as mock_kill:
    ...
    def test_posix_sigterm_then_sigkill(self) -> None:
        ...
        try:
            ...
            with patch(
                "codex_crew.session_pid.platform_compat.kill_pid",
...
class TestCleanupOrphanedMcpServersExtra:
    ...
    def test_orphan_kill_oserror_swallowed(self, pid_file: Path) -> None:
        ...
        with (
            patch(
                "codex_crew.session_pid.platform_compat.pid_exists",
...
class TestPidGoneOrUnmanaged:
    ...
    def test_dead_pid_is_safe_to_untrack(self) -> None:
        from codex_crew.session_pid import _pid_gone_or_unmanaged

        with patch(
            "codex_crew.platform_compat.pid_liveness",
...
class TestMarkedMcpLauncherPredicates:
    ...
    def test_matches_npx_playwright_null_separated(self) -> None:
        from codex_crew.session_pid import _is_marked_mcp_launcher
    ...
    def test_rejects_peer_gateway(self) -> None:
        ...
        cmdline = b"python3\x00-m\x00codex_crew.mcp_gateway.gatewayd\x00mcp start-server"
    ...
    def test_sweepable_requires_env_marker_for_marked_launcher(self) -> None:
        ...
        from codex_crew.session_pid import _is_sweepable_orphan_mcp

        cmdline = b"npx\x00@playwright/mcp\x00--headless"
        with patch("codex_crew.session_pid._env_has_codexcrew_marker", return_value=False):
    ...
    def test_sweepable_with_env_marker(self) -> None:
        ...
        cmdline = b"npx\x00@playwright/mcp\x00--headless"
        with patch("codex_crew.session_pid._env_has_codexcrew_marker", return_value=True):
    ...
    def test_fingerprinted_cmdline_never_reads_environ(self) -> None:
        ...
        with patch("codex_crew.session_pid._env_has_codexcrew_marker") as mock_env:
            assert _is_sweepable_orphan_mcp(1, b"codexcrew_sandbox_abc\x00--stdio") is True
...
class TestEnvHasKirocrewMarker:
    ...
    def test_non_linux_fails_closed(self) -> None:
        from codex_crew.session_pid import _env_has_codexcrew_marker

        with patch("codex_crew.session_pid.sys") as mock_sys:
            mock_sys.platform = "darwin"
            assert _env_has_codexcrew_marker(os.getpid()) is False
    ...
    def test_read_failure_fails_closed(self) -> None:
        ...
        with (
            ...
            mock_sys.platform = "linux"
            assert _env_has_codexcrew_marker(1) is False
    ...
    def test_real_child_with_marker(self) -> None:
        ...
        env = {**os.environ, CODEXCREW_SPAWNED_ENV: CODEXCREW_SPAWNED_VALUE}
        proc = subprocess.Popen(["sleep", "30"], env=env)
        try:
            deadline = time.monotonic() + 5
            while time.monotonic() < deadline:
                if _env_has_codexcrew_marker(proc.pid):
                    break
                time.sleep(0.05)
            assert _env_has_codexcrew_marker(proc.pid) is True
        finally:
            proc.kill()
            proc.wait()
    ...
    def test_real_child_without_marker(self) -> None:
        from codex_crew.constants import CODEXCREW_SPAWNED_ENV
        from codex_crew.session_pid import _env_has_codexcrew_marker

        env = {k: v for k, v in os.environ.items() if k != CODEXCREW_SPAWNED_ENV}
        proc = subprocess.Popen(["sleep", "30"], env=env)
        try:
            assert _env_has_codexcrew_marker(proc.pid) is False
        finally:
            proc.kill()
            proc.wait()
...
class TestMarkedLauncherSweepIntegration:
    ...
    def test_find_includes_marked_npx_orphan(self) -> None:
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[700]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=b"npx\x00@playwright/mcp\x00--headless"),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=300.0),
            patch("codex_crew.session_pid._env_has_codexcrew_marker", return_value=True),
            patch(
                "codex_crew.session_pid._work_orphan_session_leader_alive",
                return_value=False,
            ),
        ):
            mock_sys.platform = "linux"
    ...
    def test_find_excludes_unmarked_npx_orphan(self) -> None:
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[710]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=b"npx\x00@playwright/mcp\x00--headless"),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=300.0),
            patch("codex_crew.session_pid._env_has_codexcrew_marker", return_value=False),
        ):
            mock_sys.platform = "linux"
            result = find_orphan_mcp_candidates(active_pids=set())
...
class TestIsSweepableOrphanWork:
    ...
    def test_marked_orphaned_old_work_process_is_sweepable(self) -> None:
        from codex_crew.session_pid import _is_sweepable_orphan_work
    ...
    def test_live_session_leader_blocks_sweep(self) -> None:
        ...
        with (
            ...
            patch("codex_crew.session_pid._env_has_codexcrew_marker") as mock_env,
        ):
    ...
    def test_unreadable_sid_fails_closed(self) -> None:
        ...
        from codex_crew.session_pid import _work_orphan_session_leader_alive

        with patch("codex_crew.session_pid._linux_pid_sid", return_value=-1):
    ...
    def test_self_session_leader_fails_closed(self) -> None:
        ...
        with patch("codex_crew.session_pid._linux_pid_sid", return_value=1234):
            assert _work_orphan_session_leader_alive(1234) is True
    ...
    def test_dead_leader_means_session_ended(self) -> None:
        ...
        with patch("codex_crew.session_pid._linux_pid_sid", side_effect=fake_sid):
    ...
    def test_bootstrap_payload_as_free_arg_is_not_sweepable(self) -> None:
        ...
        from codex_crew.session_pid import _work_sweep_cmdline_is_test_runner
    ...
    def test_mcp_floor_is_not_enough_for_work_class(self) -> None:
        ...
        from codex_crew.session_pid import (
    ...
    def test_unmarked_work_process_is_not_sweepable(self) -> None:
        """No CODEXCREW_SPAWNED environ marker — a user's own pytest is safe."""
        from codex_crew.session_pid import _is_sweepable_orphan_work
    ...
    def test_gateway_entrypoint_is_not_sweepable(self) -> None:
        ...
        with patch("codex_crew.session_pid._env_has_codexcrew_marker", return_value=True):
            assert (
                _is_sweepable_orphan_work(
                    1234, b"python3\x00-m\x00codex_crew.mcp_gateway.gatewayd", 700.0
                )
                is False
            )
...
class TestWorkOrphanSweepIntegration:
    ...
    def test_find_includes_marked_old_work_orphan(self) -> None:
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[900]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=self._PYTEST_CMDLINE),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=700.0),
            patch("codex_crew.session_pid._env_has_codexcrew_marker", return_value=True),
            patch(
                "codex_crew.session_pid._work_orphan_session_leader_alive",
                return_value=False,
    ...
    def test_find_excludes_young_marked_work_orphan(self) -> None:
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[901]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=self._PYTEST_CMDLINE),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=300.0),
    ...
    def test_find_excludes_unmarked_work_orphan(self) -> None:
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[902]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=self._PYTEST_CMDLINE),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=700.0),
    ...
    def test_find_excludes_marked_kiro_cli_orphan(self) -> None:
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[903]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(
    ...
    def test_kill_sweeps_whole_subtree_leaf_first(self) -> None:
        ...
        with (
            ...
            patch("codex_crew.acp.client._get_child_pids", return_value=[911, 912, 913]),
            patch(
                "codex_crew.session_pid.platform_compat.kill_pid",
    ...
    def test_kill_reverify_skips_now_young_or_unmarked(self) -> None:
        ...
        with (
            ...
            patch("codex_crew.session_pid._linux_pid_age", return_value=700.0),
            patch("codex_crew.session_pid._env_has_codexcrew_marker", return_value=False),
            patch("codex_crew.session_pid.platform_compat.kill_pid") as mock_kill,
        ):
            mock_sys.platform = "linux"
    ...
    def test_subtree_kill_respects_global_cap(self) -> None:
        ...
        with (
            patch("codex_crew.session_pid._ORPHAN_SWEEP_MAX_KILLS", 3),
            patch("os.getpgrp", return_value=1000),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=self._PYTEST_CMDLINE),
            ...
            patch(
                "codex_crew.acp.client._get_child_pids",
...
class TestSpawnedMarkerInjection:
    """Every provider/MCP spawn site injects the CODEXCREW_SPAWNED marker."""

    def test_sandboxed_spawn_argv_injects_marker(self) -> None:
        from codex_crew.constants import CODEXCREW_SPAWNED_ENV, CODEXCREW_SPAWNED_VALUE
        from codex_crew.sandbox import sandboxed_spawn_argv

        with (
            patch("codex_crew.sandbox.wrap_argv", return_value=(["echo"], None)),
            patch("codex_crew.sandbox.cgroup_scope_argv", side_effect=lambda a: a),
        ):
            _, env, _ = sandboxed_spawn_argv(["echo"], env={"PATH": "/bin"})

        assert env.get(CODEXCREW_SPAWNED_ENV) == CODEXCREW_SPAWNED_VALUE
    ...
    def test_spawn_site_source_registry(self) -> None:
        ...
        src_root = Path(__file__).resolve().parent.parent / "src" / "codex_crew"
        spawn_sites = [
            "sandbox.py",
            "acp/runtime.py",
            "acp/client.py",
        ...
        for rel in spawn_sites:
            ...
            assert "CODEXCREW_SPAWNED_ENV" in content, (
                f"{rel} no longer injects the CODEXCREW_SPAWNED marker — "
...
class TestPidStartTokenIdentityGuard:
    def test_track_session_pid_records_start_token(
        ...
        from codex_crew.session_pid import _track_session_pid

        monkeypatch.setattr("codex_crew.session_pid._pid_start_token", lambda p: "tok123")
    ...
    def test_track_session_pid_falls_back_when_token_unavailable(
        ...
        monkeypatch.setattr("codex_crew.session_pid._pid_start_token", lambda p: None)
        _track_session_pid(4242)
    ...
    def test_recycled_pid_is_pruned_not_killed(self, session_pid_file: Path) -> None:
        ...
        with (
            ...
            patch("codex_crew.session_pid._pid_start_token", return_value="newtoken"),
            patch(
                "codex_crew.session_pid.platform_compat.pid_liveness",
                return_value=platform_compat.PID_ALIVE,
            ),
    ...
    def test_unreadable_token_retains_entry(self, session_pid_file: Path) -> None:
        ...
        with (
            ...
            patch("codex_crew.session_pid._pid_start_token", return_value=None),
            patch(
                "codex_crew.session_pid.platform_compat.pid_liveness",
                return_value=platform_compat.PID_ALIVE,
            ),
    ...
    def test_session_roots_unreadable_token_retains_entry(self, session_pid_file: Path) -> None:
        ...
        from codex_crew.session_pid import cleanup_orphaned_session_roots
        ...
        with (
            patch("codex_crew.session_pid._is_managed_agent_process", return_value=True),
            patch("codex_crew.session_pid._pid_start_token", return_value=None),
            patch("codex_crew.session_pid.platform_compat.pid_liveness", side_effect=fake_liveness),
            patch("codex_crew.session_pid.platform_compat.get_ppid", return_value=1),
            patch("codex_crew.session_pid.platform_compat.pid_exists", return_value=True),
            patch(
                "codex_crew.session_pid.platform_compat.kill_pid",
                side_effect=lambda p, s: kills.append((p, s)),
    ...
    def test_session_roots_subreaper_reparent_still_killed(self, session_pid_file: Path) -> None:
        ...
        with (
            patch("codex_crew.session_pid._is_managed_agent_process", return_value=True),
            patch("codex_crew.session_pid._pid_start_token", return_value="sametoken"),
            patch("codex_crew.session_pid.platform_compat.pid_liveness", side_effect=fake_liveness),
            # The subreaper that adopted the orphan -- neither init(1), nor the
            # dead gateway PID, nor the -1 probe-failure sentinel.
            patch("codex_crew.session_pid.platform_compat.get_ppid", return_value=7447),
            patch("codex_crew.session_pid.platform_compat.pid_exists", return_value=True),
            patch(
                "codex_crew.session_pid.platform_compat.kill_pid",
                side_effect=lambda p, s: kills.append((p, s)),
...
class TestSpawnGraceCrossPlatform:
    ...
    def test_grace_applies_on_macos(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        import codex_crew.session_pid as sp
...
class TestPidFileRewriteIsAtomic:
    ...
    def test_write_back_failure_preserves_every_entry(
        ...
        from codex_crew.session_pid import _write_back_pid_file

        original = "100:200:tokA\n101:201:tokB\n102:202:tokC\n"
        session_pid_file.write_text(original, encoding="utf-8")
        monkeypatch.setattr("codex_crew.atomic_write.replace_with_retry", self._fail_rename)

        with caplog.at_level(logging.ERROR, logger="codex_crew.session_pid"):
    ...
    def test_untrack_pid_failure_preserves_every_entry(
        ...
        from codex_crew.session_pid import _untrack_pid
    ...
    def test_untrack_child_pids_failure_preserves_every_entry(
        ...
        from codex_crew.session_pid import _untrack_child_pids
    ...
    async def test_windows_loop_sharing_violation_does_not_abort_caller(
        ...
        original = "100:200:tokA\n101:201:tokB\n"
        session_pid_file.write_text(original, encoding="utf-8")
        monkeypatch.setattr("codex_crew.platform_compat.IS_WINDOWS", True)
        monkeypatch.setattr("codex_crew.atomic_write.os.replace", _sharing_violation)
...
class TestTrackedAgentPids:
    ...
    def test_no_files_yields_empty_set(self, pid_file: Path, session_pid_file: Path) -> None:
        from codex_crew.session_pid import _tracked_agent_pids
...
class TestIsUntrackedManagedAgentOrphan:
    ...
    def test_untracked_marked_runtime_is_detected(self) -> None:
        from codex_crew.session_pid import _is_untracked_managed_agent_orphan
    ...
    def test_peer_gateway_is_not_detected(self) -> None:
        ...
        cmdline = b"kiro-cli\x00-m\x00codex_crew.mcp_gateway.gatewayd"
        with patch("codex_crew.session_pid._env_has_codexcrew_marker", return_value=True):
...
class TestUntrackedRuntimeReportIntegration:
    ...
    def test_reports_at_error_and_never_returns_as_candidate(
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[4242]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=_agent_cmdline()),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=3600.0),
            patch("codex_crew.session_pid._env_has_codexcrew_marker", return_value=True),
            caplog.at_level(logging.ERROR, logger="codex_crew.session_pid"),
        ):
            mock_sys.platform = "linux"
            result = find_orphan_mcp_candidates(active_pids=set())
    ...
    def test_argv0_control_characters_cannot_forge_log_lines(
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[4848]),
            patch("codex_crew.session_pid.sys") as mock_sys,
    ...
    def test_report_is_logged_once_across_repeated_sweeps(
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[4343]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=_agent_cmdline()),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=3600.0),
    ...
    def test_vanished_pid_re_arms_the_report(
        ...
        with (
            ...
            mock_sys.platform = "linux"
            with patch("codex_crew.session_pid._our_orphan_pids", return_value=[4444]):
                find_orphan_mcp_candidates(active_pids=set())
            with patch("codex_crew.session_pid._our_orphan_pids", return_value=[]):
                find_orphan_mcp_candidates(active_pids=set())
    ...
    def test_tracked_runtime_is_not_reported(
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[4545]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=_agent_cmdline()),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=3600.0),
    ...
    def test_recycled_owner_pid_does_not_suppress_the_report(
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[4747]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=_agent_cmdline()),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=3600.0),
    ...
    def test_young_orphan_is_not_reported(
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[4646]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=_agent_cmdline()),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=5.0),
            patch("codex_crew.session_pid._env_has_codexcrew_marker", return_value=True),
            caplog.at_level(logging.ERROR, logger="codex_crew.session_pid"),
        ):
            mock_sys.platform = "linux"
...
class TestBrowserDaemonSessionArg:
    ...
    def test_extracts_generated_session_name(self) -> None:
        from codex_crew.session_pid import _browser_daemon_session_arg

        assert _browser_daemon_session_arg(_DAEMON_CMDLINE) == b"kc-1a2b3c4d"

    def test_rejects_operator_named_session(self) -> None:
        """Only Codex-Crew-generated ``kc-<8hex>`` names are ever sweepable."""
        from codex_crew.session_pid import _browser_daemon_session_arg
    ...
    def test_session_env_name_matches_launch_module(self) -> None:
        ...
        from codex_crew.browser_cli.launch import SESSION_ENV
        from codex_crew.session_pid import _BROWSER_SESSION_ENV
...
class TestBrowserDaemonOrphanSweep:
    ...
    def test_dead_owner_daemon_is_a_candidate(self) -> None:
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[800]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=_DAEMON_CMDLINE),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=900.0),
            patch("codex_crew.session_pid._env_has_codexcrew_marker", return_value=True),
            patch("codex_crew.session_pid._env_value", return_value=b"kc-1a2b3c4d"),
            patch(
                "codex_crew.session_pid._browser_session_owner_alive",
                return_value=False,
            ),
        ):
            mock_sys.platform = "linux"
    ...
    def test_live_owner_daemon_is_never_a_candidate(self) -> None:
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[801]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=_DAEMON_CMDLINE),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=900.0),
    ...
    def test_operator_session_daemon_is_never_a_candidate(self) -> None:
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[802]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=_OPERATOR_DAEMON_CMDLINE),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=900.0),
            patch("codex_crew.session_pid._env_has_codexcrew_marker", return_value=True),
            patch("codex_crew.session_pid._env_value", return_value=b"chrome"),
            patch(
                "codex_crew.session_pid._browser_session_owner_alive",
                return_value=False,
            ),
    ...
    def test_unmarked_daemon_is_never_a_candidate(self) -> None:
        """No ``CODEXCREW_SPAWNED`` marker means we did not spawn this tree."""
        from codex_crew.session_pid import find_orphan_mcp_candidates

        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[803]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=_DAEMON_CMDLINE),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=900.0),
    ...
    def test_young_daemon_is_never_a_candidate(self) -> None:
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[804]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=_DAEMON_CMDLINE),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=200.0),
            patch("codex_crew.session_pid._env_has_codexcrew_marker", return_value=True),
            patch("codex_crew.session_pid._env_value", return_value=b"kc-1a2b3c4d"),
            patch(
                "codex_crew.session_pid._browser_session_owner_alive",
    ...
    def test_env_argv_session_mismatch_is_never_a_candidate(self) -> None:
        ...
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[805]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=_DAEMON_CMDLINE),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=900.0),
            patch("codex_crew.session_pid._env_has_codexcrew_marker", return_value=True),
            patch("codex_crew.session_pid._env_value", return_value=b"kc-99999999"),
            patch(
                "codex_crew.session_pid._browser_session_owner_alive",
                return_value=False,
            ),
...
class TestBrowserSessionOwnerAlive:
    ...
    def test_live_peer_holding_the_session_reads_as_alive(self) -> None:
        from codex_crew import session_pid as sp
