...
:mod:`codex_crew.mcp_gateway.transport` was written to provide.
...
async def test_two_stubs_on_one_backend_are_told_apart(tmp_path: Path, short_sock_dir) -> None:
    ...
    The unit tests around ``codexcrew-cron`` prove the server CONSUMES the caller
    ...
    ``codexcrew.caller-identity`` -- and that capability is parsed off a REAL
...
async def test_backends_hosting_stub_covers_exclusive() -> None:
    ...
    from codex_crew.mcp_gateway.pool import BackendPool
