...
def _make_app() -> web.Application:
    from codex_crew.dashboard.handlers import api_beacon_status
...
class TestBeaconStatusEndpoint:
    ...
    async def test_stored_flag_on_but_suppressed_reports_both(
        ...
        assert body["enabled"] is True
        assert body["would_send"] is False
        assert "CODEXCREW_HOME" in body["reason"]
    ...
    async def test_overlay_entry_is_flagged(self, _neutral_env, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "codex_crew.config.loader.config_dir", lambda: _neutral_env, raising=False
        )
        async with TestClient(TestServer(_make_app())) as c:
            body = await _get(c)
