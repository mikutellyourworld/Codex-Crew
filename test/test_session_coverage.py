"""Coverage for ``codex_crew.session`` edges the behavioural suites skip.
...
from codex_crew.acp.types import ACP_BACKEND_CLAUDE, ACP_BACKEND_KIRO
...
class TestSessionModel:
    ...
    def test_crew_pin_wins_verbatim(self, cfg) -> None:
        ...
        cfg.agent.model = "sonnet"
        cfg.agents["research"] = CodexCrewAgentConfig(model="opus")
    ...
    def test_crew_inherit_spelling_is_not_a_pin(self, cfg) -> None:
        ...
        cfg.agent.model = "sonnet"
        cfg.agents["research"] = CodexCrewAgentConfig(model="auto")
    ...
    def test_crew_that_defers_continues_on_its_bound_template(self, cfg, monkeypatch) -> None:
        ...
        cfg.agent.model = "sonnet"
        cfg.agents["research"] = CodexCrewAgentConfig(model="", kiro_agent="tmpl")
    ...
    def test_base_agent_name_skips_the_per_agent_lookup(self, cfg, monkeypatch) -> None:
        """``codexcrew`` is the built-in template; globbing the agents dir for it
        ...
        assert _session_model(cfg, "codexcrew") == "sonnet"
        assert calls == []
...
class TestDetectProviderSwitch:
    ...
    def test_different_provider_with_a_stored_sid_is_a_switch(self) -> None:
        with patch("codex_crew.session.sel") as sel_mock:
...
class TestAdoptProvider:
    ...
    def test_non_resumed_first_turn_states_survive_adoption(self, start) -> None:
        ...
        ``is_new=True`` and re-inject Codex Crew history onto a live
...
class TestSendAbortForSession:
    ...
    async def test_runtime_info_drives_the_abort(self, mgr) -> None:
        ...
        with patch("codex_crew.session.schedule_abort") as abort:
    ...
    async def test_a_failing_audit_does_not_block_the_abort(self, mgr) -> None:
        ...
        with patch("codex_crew.session.sel", side_effect=RuntimeError("sel down")):
            with patch("codex_crew.session.schedule_abort") as abort:
                await mgr._send_abort_for_session("d1", sess)
        abort.assert_called_once()
...
class TestOrphanMcpHook:
    ...
    async def test_a_sweep_result_is_logged(self, mgr, caplog) -> None:
        with patch("codex_crew.session._cleanup_orphaned_mcp_servers", return_value=3):
    ...
    async def test_a_clean_sweep_logs_nothing(self, mgr, caplog) -> None:
        with patch("codex_crew.session._cleanup_orphaned_mcp_servers", return_value=0):
            with caplog.at_level(logging.INFO, logger="codex_crew.session"):
                await mgr._orphan_mcp_hook()
    ...
    async def test_a_sweep_failure_is_swallowed(self, mgr) -> None:
        with patch(
            "codex_crew.session._cleanup_orphaned_mcp_servers",
