...
class TestSlackThreadTemporary:
    ...
    def test_is_thread_temporary_false_by_default(self):
        from codex_crew.slack.handler import is_thread_temporary

        assert is_thread_temporary("unknown-key") is False

    def test_mark_temporary(self):
        from codex_crew.slack.handler import _mark_temporary, is_thread_temporary
    ...
    # slack.handler into codex_crew.messaging.privacy_mode).
...
class TestTemporaryCommand:
    ...
    # (the implementation moved into codex_crew.messaging.privacy_mode).

    @pytest.mark.asyncio
    async def test_temporary_modifier_idempotent(self):
        from codex_crew.slack.handler import _apply_temporary_modifier, _mark_temporary
...
class TestIsRestrictedSession:
    ...
    def test_dashboard_temporary_slot(self):
        from codex_crew.dashboard.handlers import _is_restricted_session
        from codex_crew.dashboard.state import _ChatSlot
    ...
    def test_slack_temporary_thread(self):
        from codex_crew.dashboard.handlers import _is_restricted_session
        from codex_crew.slack.handler import _mark_temporary
...
class TestMcpSessionKeyPlumbing:
    @patch.dict("os.environ", {"CODEXCREW_SESSION_KEY": "dashboard:chat-1-tmp"})
    @patch("codex_crew.mcp_core._post")
    ...
    @patch.dict("os.environ", {"CODEXCREW_SESSION_KEY": "dashboard:chat-1-tmp"})
    @patch("codex_crew.mcp_core._delete")
    ...
    @patch.dict("os.environ", {"CODEXCREW_SESSION_KEY": "dashboard:chat-1-tmp"})
    @patch("codex_crew.mcp_core._get")
