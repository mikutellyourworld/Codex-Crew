...
def test_lifts_when_unset():
    config = {"name": "codexcrew", "model_managed": True, "cc_model": "claude-sonnet-4.6"}

    changed = agent_state.lift_and_strip_bookkeeping(config, "codexcrew")
...
def test_non_string_cc_model_discarded_not_lifted(caplog):
    ...
    assert changed is True
    assert "cc_model" not in config
    assert agent_state.get_cc_model("codexcrew") is None
    assert "non-string cc_model" in caplog.text


def test_returns_false_when_neither_key_present():
    config = {"name": "codexcrew", "model": "auto"}

    changed = agent_state.lift_and_strip_bookkeeping(config, "codexcrew")

    assert changed is False
    assert config == {"name": "codexcrew", "model": "auto"}
