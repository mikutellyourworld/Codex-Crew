...
is a line in ``CodexCrewConfig.load()`` pulling it out of the JSON section. Miss
...
#: Config section name -> the attribute on ``CodexCrewConfig`` holding it. The
...
def test_every_scalar_channel_field_survives_a_load(
    ...
    assert not unread, (
        ...
        f"`{section}_data.get(...)` line to CodexCrewConfig.load(), or list the "
...
def test_an_unreadable_activation_degrades_to_the_narrower_mode(
    ...
    assert CodexCrewConfig.load().telegram.forum_activation == "mention"
...
def test_an_absent_activation_still_takes_the_documented_default(
    ...
    assert CodexCrewConfig.load().telegram.forum_activation == "always"
...
def test_telegram_does_not_advertise_an_activation_it_cannot_express() -> None:
    ...
    """
    from codex_crew.config.loader import _VALID_ACTIVATIONS, TELEGRAM_ACTIVATIONS
...
def test_telegram_show_thinking_specifically_round_trips(
    ...
    assert CodexCrewConfig.load().telegram.show_thinking is True
