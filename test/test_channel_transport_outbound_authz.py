...
from codex_crew.messaging.transport import MessagingTransport
...
def _overrides_hook(channel: str, class_name: str) -> bool:
    ...
    path = Path(codex_crew_pkg.__file__).parent / channel / "transport.py"
    tree = ast.parse(path.read_text(encoding="utf-8"), filename=str(path))
    for node in tree.body:
...
class TestOtherTransportsThatCanAnswer:
    def test_imessage_matches_the_handle_roster(self) -> None:
        from codex_crew.imessage.transport import IMessageTransport
    ...
    def _teams(allowed: list[str], *, owner: str = "", conversation: str = "") -> Any:
        ...
        """
        from codex_crew.teams.transport import TeamsTransport
    ...
    async def test_teams_warm_routes_precedes_registration_in_the_gateway(self) -> None:
        ...
        from codex_crew.teams import gateway as teams_gateway
    ...
    def _weixin(self, policy: str, allowed: list[str]) -> Any:
        from codex_crew.weixin.transport import WeixinTransport
    ...
    def test_wecom_mirrors_authorize(self) -> None:
        from codex_crew.wecom.transport import WeComTransport
...
class TestPrincipalAnsweredTransports:
    ...
    def _discord(self, allowed: list[str], threads: list[str] | None = None) -> Any:
        from codex_crew.discord.transport import DiscordTransport
    ...
    def _webex(
        ...
        *,
        ...
        from codex_crew.webex.transport import WebexTransport
...
class TestSessionPrincipalExtraction:
    ...
    def _principal(self, key: str) -> str:
        from codex_crew.dashboard.chat_runner import _session_principal

        return _session_principal(key)

    def test_a_direct_dm_key_names_its_peer(self) -> None:
        assert self._principal("discord:codexcrew:direct:42") == "42"
    ...
    def test_a_generation_suffix_does_not_hide_the_peer(self) -> None:
        ...
        assert self._principal("discord:codexcrew:direct:42:gen3") == "42"
    ...
    def test_a_forum_route_names_no_single_principal(self) -> None:
        ...
        """
        assert self._principal("telegram:codexcrew:forum:-100123:7") == ""
    ...
    def test_a_unified_bucket_names_no_principal(self) -> None:
        ...
        """
        assert self._principal("unified:codexcrew") == ""
        assert self._principal("unified:codexcrew:gen2") == ""
    ...
    def test_a_webex_space_key_and_its_transport_agree(self) -> None:
        ...
        """
        from codex_crew.messaging.link import CHAT_TYPE_FORUM, build_dm_session_key
        from codex_crew.webex.transport import WebexTransport

        room = "Y2lzY29zcGFyazovL3Jvb20xMjM"
        key = build_dm_session_key("webex", "codexcrew", f"space:{room}", chat_type=CHAT_TYPE_FORUM)
...
class TestTheLadderConsultsTheTransport:
    ...
    def _resolve(
        self, transport: Any, link: ChannelLink, key: str = "telegram:codexcrew:direct:111"
    ) -> Any:
    ...
    def test_the_link_is_passed_through_verbatim(self) -> None:
        ...
        self._resolve(transport, link, key="telegram:codexcrew:forum:-100123:7")
    ...
    def test_the_session_principal_reaches_the_transport(self) -> None:
        ...
        self._resolve(transport, link, key="discord:codexcrew:direct:42")
    ...
    def test_a_refusal_is_audited(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_runner.sel",
