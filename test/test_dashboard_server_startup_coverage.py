"""Coverage tests for ``codex_crew.dashboard.server``'s startup path.
...
reach: the body of :func:`~codex_crew.dashboard.server.start_dashboard` itself —
...
operator's REAL ``~/.kiro/settings/mcp.json``, outside ``CODEXCREW_HOME``), the
terminal reaper (shells out to ``ps``) and the MCP probe. ``CODEXCREW_HOME`` is
...
from codex_crew.dashboard import server as srv
...
def _neutralise_outside_process_work(monkeypatch) -> dict[str, Any]:
    ...
    """
    import codex_crew.apps.dev_mode as dev_mode
    import codex_crew.kiro_prerequisite as kiro_prereq
    ...
    spies: dict[str, Any] = {
        ...
        # here whose target is outside CODEXCREW_HOME.
...
async def _start_dashboard(tmp_path: Path, monkeypatch, **kwargs: Any) -> Any:
    ...
    import codex_crew.dashboard.state as _st
...
class TestStartDashboardWiring:
    ...
    @pytest.mark.asyncio
    async def test_optional_tunnel_does_not_delay_dashboard_readiness(
        self, tmp_path: Path, monkeypatch
    ) -> None:
        """A remote-auth wait cannot hold the local desktop behind its splash."""
        entered = asyncio.Event()
        cancelled = asyncio.Event()

        async def _blocked_tunnel(**_kwargs: Any) -> None:
            entered.set()
            try:
        ...
        provider = SimpleNamespace(stop=AsyncMock(), enabled=lambda: True)
        monkeypatch.setattr(srv, "setup_tunnel", _blocked_tunnel)
        monkeypatch.setattr(
            srv,
            "current_context",
            lambda: SimpleNamespace(
                ...
                dashboard=SimpleNamespace(
                    contribute_routes=lambda _app: None,
        ...
        runner, state, _spies = await asyncio.wait_for(
            _start_dashboard(tmp_path, monkeypatch), timeout=5
        )
        task = state._tunnel_start_task
        try:
    ...
    @pytest.mark.asyncio
    async def test_noncooperative_tunnel_start_cannot_wedge_shutdown_or_publish_late(
        self, tmp_path: Path, monkeypatch
    ) -> None:
        """The cleanup ceiling holds even when provider startup swallows cancel."""
        entered = asyncio.Event()
        release = asyncio.Event()
        provider_stop_seen = asyncio.Event()
        late_manager = SimpleNamespace(stop=AsyncMock())

        async def _stubborn_tunnel(**_kwargs: Any) -> Any:
            entered.set()
            try:
                await asyncio.Future()
            except asyncio.CancelledError:
                # Simulate a provider that finishes its own retry before it
                # honors cancellation and returns a newly-live manager late.
                await release.wait()
                return late_manager

        async def _stop_provider() -> None:
            provider_stop_seen.set()

        provider = SimpleNamespace(stop=AsyncMock(side_effect=_stop_provider), enabled=lambda: True)
        monkeypatch.setattr(srv, "_TUNNEL_STOP_TIMEOUT_SECS", 0.01)
        monkeypatch.setattr(srv, "setup_tunnel", _stubborn_tunnel)
        monkeypatch.setattr(
            srv,
            "current_context",
            lambda: SimpleNamespace(
        ...
        try:
            await asyncio.wait_for(provider_stop_seen.wait(), timeout=1)
            ...
            assert state.tunnel_manager is None
            late_manager.stop.assert_awaited_once()
        finally:
            release.set()
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("manager_present", [False, True])
    async def test_noncooperative_tunnel_stop_cannot_wedge_cleanup(
        self, monkeypatch, manager_present: bool
    ) -> None:
        """The hard stop bound covers both manager and direct-provider paths."""
        entered = asyncio.Event()
        release = asyncio.Event()

        async def _stubborn_stop() -> None:
            entered.set()
            try:
                await asyncio.Future()
            except asyncio.CancelledError:
        ...
        stopper = SimpleNamespace(stop=AsyncMock(side_effect=_stubborn_stop))
        provider = stopper if not manager_present else SimpleNamespace(stop=AsyncMock())
        state = SimpleNamespace(
            tunnel_manager=stopper if manager_present else None,
            _tunnel_start_task=None,
        ...
        monkeypatch.setattr(
            srv,
            "current_context",
            lambda: SimpleNamespace(tunnel=provider),
        )
        app = web.Application()
        srv._wire_tunnel_shutdown(app, state)
        ...
        try:
            ...
            assert state._tunnel_shutting_down is True
        finally:
            release.set()
            await asyncio.gather(cleanup, return_exceptions=True)
    ...
    async def test_a_configured_url_joins_the_csrf_origin_set(self, tmp_path, monkeypatch) -> None:
        ...
        """
        url = "http://dash.example.com:5486"
