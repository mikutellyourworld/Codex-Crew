...
from codex_crew.metrics import provider
...
def _own_home(monkeypatch, tmp_path):
    ...
    would otherwise decide the outcome -- an order-dependent pass. ``CODEXCREW_HOME``
...
def test_attrs_carry_identity_version_and_environment():
    ...
    assert attrs["service.name"] == "codexcrew"
    ...
    assert attrs["service.version"] == beacon.release(codex_crew.__version__)
...
def test_first_build_on_a_fresh_install_exports_the_id(tmp_path, monkeypatch):
    ...
    try:
        ...
        rec.counter("codexcrew.session.idle_expired", attrs={"turn_active": False})
        provider_shutdown()
...
def test_a_failed_mint_costs_only_the_attribute(tmp_path, monkeypatch):
    ...
    try:
        ...
        assert resource_attrs["service.name"] == "codexcrew"
...
def test_exported_shard_carries_the_resource(tmp_path, monkeypatch):
    ...
    try:
        ...
        assert resource_attrs["service.name"] == "codexcrew"
        assert resource_attrs["service.version"] == beacon.release(codex_crew.__version__)
