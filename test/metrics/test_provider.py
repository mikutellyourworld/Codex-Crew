"""Tests for codex_crew.metrics.provider — consent gate + recorder singleton."""
...
from codex_crew.config.loader import CodexCrewConfig, TelemetryConfig
from codex_crew.metrics.provider import MetricsRecorder, get_recorder, reset_for_testing
from codex_crew.metrics.provider import shutdown as provider_shutdown


def _patch_config(monkeypatch, **tel_kwargs):
    fake = CodexCrewConfig(telemetry=TelemetryConfig(**tel_kwargs))
    monkeypatch.setattr(CodexCrewConfig, "load", classmethod(lambda cls: fake))
    # Keep the consent gate deterministic: a stray CODEXCREW_TELEMETRY in the
...
def test_enabled_builds_live_recorder(tmp_path, monkeypatch):
    ...
    try:
        ...
        rec.histogram("codexcrew.session.startup.duration", 1.0, unit="ms")
    finally:
        reset_for_testing()
...
def _worker_event(monkeypatch):
    ...
    """
    import codex_crew.metrics.provider as provider_mod
...
def test_no_egress_destination_by_default():
    ...
    from codex_crew.metrics.provider import _otlp_destinations
    from codex_crew.platform.defaults import DefaultTelemetryProvider
...
def test_otlp_reader_degrades_without_logging_endpoint(monkeypatch, caplog):
    ...
    from codex_crew.config.loader import TelemetryConfig
    from codex_crew.metrics.provider import _build_otlp_reader
...
def test_env_var_opts_in_when_config_disabled(tmp_path, monkeypatch):
    """CODEXCREW_TELEMETRY=1 enables LOCAL telemetry even if the config flag is off."""
    reset_for_testing()
...
def test_env_var_opts_out_when_config_enabled(monkeypatch):
    """CODEXCREW_TELEMETRY=0 force-disables telemetry even if the config flag is on."""
    reset_for_testing()
...
def test_env_var_blank_defers_to_config(monkeypatch):
    ...
    from codex_crew.metrics.provider import _consent_enabled
...
class TestConsentRecheck:
    ...
    `codexcrew config set telemetry.enabled true` writes config.json from a separate
    ...
    def test_hot_path_does_not_read_config_every_call(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setattr(CodexCrewConfig, "load", classmethod(lambda cls: counting_load()))
            for _ in range(50):
                get_recorder()
            assert reads["n"] == 0
        finally:
    ...
    def test_unreadable_config_keeps_the_live_recorder(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setattr(CodexCrewConfig, "load", classmethod(lambda cls: _boom()))
            self._elapse_window(monkeypatch)
    ...
    def test_get_recorder_never_reads_config_on_the_calling_thread(self, monkeypatch):
        ...
        `CodexCrewConfig.load()` is a fingerprint-cache hit in the steady state
        ...
        try:
            ...
            monkeypatch.setattr(CodexCrewConfig, "load", classmethod(lambda cls: _tracking_load()))
            done = _worker_event(monkeypatch)
            self._elapse_window(monkeypatch)
            get_recorder()
    ...
    def test_env_pin_still_wins_after_a_config_edit(self, tmp_path, monkeypatch):
        ...
        try:
            assert get_recorder().enabled is False
            fake = CodexCrewConfig(telemetry=TelemetryConfig(enabled=True, local_dir=str(tmp_path)))
            monkeypatch.setattr(CodexCrewConfig, "load", classmethod(lambda cls: fake))
            monkeypatch.setenv("CODEXCREW_TELEMETRY", "0")
            self._elapse_window(monkeypatch)
