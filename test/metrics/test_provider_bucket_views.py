...
session startup, so the first ``codexcrew.turn.duration`` sample ever recorded
...
from codex_crew.dashboard.handlers.telemetry import _Hist
from codex_crew.metrics import provider as provider_mod
from codex_crew.metrics.provider import (
    _CREDIT_BUCKETS,
    _FAST_BUCKETS_MS,
...
_NAME_RE = re.compile(r'"(codexcrew\.[a-z0-9_.]*\.duration)"')
...
def _emitted_histogram_units() -> dict[str, set[str]]:
    ...
    suffix. That is not a property of the code: ``codexcrew.embed.queue_wait`` and
    ``codexcrew.embed.inference`` are millisecond histograms with no such suffix,
    and a non-duration histogram (``codexcrew.turn.credits``) has no reason to
...
class TestCompleteness:
    def test_source_scan_finds_the_known_instruments(self):
        ...
        assert "codexcrew.turn.duration" in names
        assert "codexcrew.session.startup.duration" in names
        assert len(names) >= 7
...
class TestNonDurationHistograms:
    ...
    1. **Explicit buckets.** `codexcrew.turn.credits` spans 0.03 to 658 in real
    ...
    def test_emitted_scan_finds_both_families(self):
        ...
        assert units.get("codexcrew.turn.duration") == {"ms"}
        assert units.get("codexcrew.turn.credits") == {"credit"}
        assert units.get("codexcrew.turn.cost_usd") == {"usd"}
        # The pair that motivated the unit-based check: ms, no `.duration` suffix.
        assert units.get("codexcrew.embed.queue_wait") == {"ms"}
...
class TestViewWiring:
    ...
    def test_long_turn_does_not_land_in_the_overflow_bucket(self):
        ...
        mp.get_meter("t").create_histogram(
            "codexcrew.turn.duration", unit="ms"
        ).record(227589)

        (dp,) = self._points(reader, "codexcrew.turn.duration")
    ...
    def test_no_duplicate_streams_per_instrument(self):
        ...
        meter.create_histogram("codexcrew.turn.duration", unit="ms").record(5000)
        meter.create_histogram(
            "codexcrew.session.startup.duration", unit="ms"
        ).record(4400)

        assert len(self._points(reader, "codexcrew.turn.duration")) == 1
        assert len(self._points(reader, "codexcrew.session.startup.duration")) == 1

    def test_each_instrument_gets_its_own_family(self):
        mp, reader = self._provider()
        meter = mp.get_meter("t")
        meter.create_histogram("codexcrew.turn.duration", unit="ms").record(60000)
        meter.create_histogram(
            "codexcrew.mcp.backend.acquire.duration", unit="ms"
        ).record(1)

        (turn,) = self._points(reader, "codexcrew.turn.duration")
        (fast,) = self._points(reader, "codexcrew.mcp.backend.acquire.duration")
    ...
    def test_billing_histograms_get_their_own_non_ms_bounds(self):
        ...
        mp, reader = self._provider()
        meter = mp.get_meter("t")
        meter.create_histogram("codexcrew.turn.credits", unit="credit").record(6.76)
        meter.create_histogram("codexcrew.turn.cost_usd", unit="usd").record(0.0032)

        (credits,) = self._points(reader, "codexcrew.turn.credits")
        (cost,) = self._points(reader, "codexcrew.turn.cost_usd")
...
class TestAggregatorReadsRealPercentiles:
    ...
    def test_turn_percentiles_are_no_longer_pinned_to_the_ceiling(self):
        from codex_crew.dashboard.handlers.telemetry import _pct_from_buckets
