"""Tests for codex_crew.metrics.recorder — the MetricsRecorder facade."""
...
class TestNoOpRecorder:
    def test_disabled_recorder_is_noop(self):
        ...
        rec.counter("codexcrew.x")
        rec.histogram("codexcrew.y", 1.0)
        rec.up_down_counter("codexcrew.z", 1)


class TestRecord:
    def test_counter_recorded(self):
        rec, reader = _recorder_with_reader()
        rec.counter("codexcrew.test.count", 3, attrs={"ok": True})
        names = [n for n, _ in _data_points(reader)]
        assert "codexcrew.test.count" in names

    def test_histogram_recorded(self):
        rec, reader = _recorder_with_reader()
        rec.histogram("codexcrew.test.dur", 12.5, unit="ms")
        names = [n for n, _ in _data_points(reader)]
        assert "codexcrew.test.dur" in names
...
class TestGuardrails:
    def test_app_cannot_spoof_core_namespace(self):
        ...
        rec, reader = _recorder_with_reader()
        rec.counter("codexcrew.evil", app_id="my_app")  # invalid namespace
