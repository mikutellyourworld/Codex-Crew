"""Tests for codexcrew.turn.duration, emitted at turn completion.
...
def _turn_call(rec):
    calls = [c for c in rec.calls if c["name"] == "codexcrew.turn.duration"]
    assert calls, "codexcrew.turn.duration histogram must be emitted"
    return calls[-1]
