...
Covers ``codex_crew.metrics.http_metrics``:

* ``codexcrew.gateway.boot.duration``   — boot-to-ready histogram
* ``codexcrew.gateway.request.duration`` — per-route latency histogram
...
class TestBootToReady:
    def test_emits_histogram_with_privacy_safe_labels(self):
        ...
        with patch(
            "codex_crew.metrics.http_metrics.get_recorder", return_value=rec
        ):
    ...
    def test_recorder_failure_swallowed(self):
        ...
        with patch(
            "codex_crew.metrics.http_metrics.get_recorder", return_value=boom
        ):
            # Must not raise.
...
async def test_middleware_records_template_not_concrete_path():
    rec = _CapturingRecorder()
    with patch("codex_crew.metrics.http_metrics.get_recorder", return_value=rec):
        app = _build_app(rec)
...
async def test_telemetry_failure_never_breaks_request():
    boom = MagicMock()
    boom.histogram.side_effect = RuntimeError("telemetry down")
    with patch("codex_crew.metrics.http_metrics.get_recorder", return_value=boom):
        app = _build_app(None)
        async with TestClient(TestServer(app)) as client:
...
async def test_bounded_cardinality_under_many_distinct_ids():
    ...
    with patch(
        "codex_crew.metrics.http_metrics.get_recorder", return_value=real_recorder
    ):
        app = _build_app(None)
        async with TestClient(TestServer(app)) as client:
            for i in range(100):
...
def test_server_module_wires_http_metrics():
    ...
    import codex_crew.dashboard.server as srv
