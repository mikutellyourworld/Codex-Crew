"""Tests for the codexcrew.inventory.* observable instrument contract.
...
from codex_crew.metrics import inventory_gauges as ig
...
def test_monitor_loops_counts_armed_loops_only(tmp_path):
    ...
    with patch("codex_crew.autonudge.get_instance", return_value=service):
...
def test_monitor_loops_service_absent_yields_none_but_running_empty_yields_zero(tmp_path):
    ...
    with patch("codex_crew.autonudge.get_instance", return_value=None):
        assert ig.read_active_monitor_loops() is None
    with patch("codex_crew.autonudge.get_instance", return_value=_nudge_service(tmp_path, [])):
...
def test_knowledge_documents_counts_sources(tmp_path):
    ...
    ig.reset_for_testing()
    try:
        with patch("codex_crew.config.paths.config_dir", return_value=tmp_path):
...
def test_knowledge_probe_opens_read_only(tmp_path):
    ...
    try:
        with (
            patch("codex_crew.config.paths.config_dir", return_value=tmp_path),
...
def test_lessons_counts_saved_lessons(tmp_path):
    from codex_crew.learn import Lesson, LessonStore
...
def test_mcp_classes_split_managed_from_user_added():
    from codex_crew.mcp_discovery import _MANAGED_SERVER_NAMES
    ...
    with patch("codex_crew.mcp_discovery._load_agent_config", return_value=agent_cfg):
        with patch("codex_crew.mcp_discovery._load_mcp_json_by_source", return_value=by_source):
...
def test_mcp_classes_empty_roster_yields_none():
    with patch("codex_crew.mcp_discovery._load_agent_config", return_value={}):
        with patch("codex_crew.mcp_discovery._load_mcp_json_by_source", return_value={}):
...
def test_no_mcp_server_name_reaches_any_attribute():
    ...
    with patch(
        "codex_crew.mcp_discovery._load_agent_config",
...
def test_reporter_claim_is_unconditional():
    ...
    under ``CODEXCREW_AUTONUDGE=0``, and one env var darkened all eight gauges.
...
def test_live_build_registers_inventory_gauges(monkeypatch):
    ...
    try:
        with patch("codex_crew.metrics.inventory_gauges.register_inventory_gauges") as register:
...
def test_gauge_registration_failure_keeps_telemetry_alive(monkeypatch):
    ...
    try:
        with patch(
            "codex_crew.metrics.inventory_gauges.register_inventory_gauges",
...
def test_inventory_failure_does_not_cost_the_process_gauges(monkeypatch):
    ...
    try:
        with patch(
            ...
            with patch("codex_crew.metrics.process_gauges.register_process_gauges") as process:
                rec = provider_mod.get_recorder()
                assert rec.enabled
