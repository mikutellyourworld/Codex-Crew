...
from codex_crew.dashboard.handlers.telemetry import api_usage_turns
from codex_crew.dashboard.handlers.usage import TURN_USAGE_FIELDS, slot_turn_usage
...
class TestAppIsolation:
    ...
    def _quiet_sel(self, monkeypatch):
        ...
        from codex_crew.dashboard.handlers import telemetry as tele_mod
...
class TestWriteSiteStamping:
    ...
    def test_chat_runner_stamps_the_slots_app(self):
        src = (self._ROOT / "src/codex_crew/dashboard/chat_runner.py").read_text(encoding="utf-8")
        assert 'app=getattr(slot, "_app", "") or ""' in src

    def test_subagent_completion_stamps_the_dispatching_app(self):
        src = (self._ROOT / "src/codex_crew/subagent_manager/run.py").read_text(encoding="utf-8")
        tree = ast.parse(src)
        calls = [
            node
