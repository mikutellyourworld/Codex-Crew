...
(``pip install "codexcrew[otlp]"``) that a default dev/CI environment does not
have:
...
from codex_crew.metrics import process_gauges as pg
from codex_crew.metrics import provider as pm
...
#: CUMULATIVE series Codex Crew exported; they are observable GAUGES now, so they
...
def test_service_name_identifies_the_product_on_the_wire(exported):
    data, _ = exported
    for serialized in data.resources:
        assert serialized.get("service.name") == "codexcrew"
...
def test_no_instrument_exports_cumulative(exported):
    """The ratchet: not one Codex Crew series is CUMULATIVE on either destination.
...
requires_otlp = pytest.mark.skipif(
    not _otlp_available(),
    reason='OTLP wire tier needs the optional extra: pip install "codexcrew[otlp]"',
)
