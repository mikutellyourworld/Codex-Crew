"""Tests for the codexcrew.process.* observable instrument contract.
...
def test_every_instrument_is_a_gauge_not_a_sum():
    ...
    only cumulative series Codex Crew exported, and one cumulative series forces
...
def test_live_build_registers_process_gauges(monkeypatch):
    ...
    try:
        with patch("codex_crew.metrics.process_gauges.register_process_gauges") as register:
...
def test_gauge_registration_failure_keeps_telemetry_alive(monkeypatch):
    ...
    try:
        with patch(
            "codex_crew.metrics.process_gauges.register_process_gauges",
