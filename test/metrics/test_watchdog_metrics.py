"""Tests for the watchdog telemetry (codexcrew.watchdog.*).
...
def _action_calls(rec, action=None):
    calls = [c for c in rec.counters if c["name"] == "codexcrew.watchdog.action"]
...
def _idle_calls(rec):
    return [h for h in rec.histograms if h["name"] == "codexcrew.watchdog.idle.duration"]
...
class TestEmitWatchdogMetric:
    ...
    def test_emit_failure_never_raises(self):
        handle = _handle()
        with patch(
            "codex_crew.metrics.provider.get_recorder", side_effect=RuntimeError("boom")
        ):
...
async def test_first_working_deferral_always_logged_regardless_of_host_uptime():
    ...
    with patch("codex_crew.acp.session_handle.time") as mock_time:
...
# ── codexcrew.watchdog.recovery.outcome (chat_runner) ─────────────────────────
...
def _run_recovery(mechanism, outcome, attempts):
    ...
    with patch("codex_crew.dashboard.chat_runner.get_recorder", return_value=rec):
        chat_runner._emit_recovery_outcome(mechanism, outcome, attempts)
    return rec


def _recovery_calls(rec):
    return [c for c in rec.counters if c["name"] == "codexcrew.watchdog.recovery.outcome"]
...
class TestRecoveryOutcome:
    ...
    def test_emit_failure_never_raises(self):
        ...
        with patch(
            "codex_crew.dashboard.chat_runner.get_recorder", side_effect=RuntimeError("boom")
        ):
