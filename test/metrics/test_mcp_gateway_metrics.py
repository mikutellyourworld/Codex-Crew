...
* ``codexcrew.mcp.warm_pool.acquire`` — via ``HotKeyStore.record_outcome``
* ``codexcrew.mcp.backend.acquire.duration`` — via ``gatewayd._emit_backend_acquire_metric``
* ``codexcrew.mcp.lazy_load.{count,duration}`` — via ``gatewayd._emit_lazy_load_metrics``
...
# ---------------------------------------------------------------------------
# codexcrew.mcp.warm_pool.acquire — prewarm.HotKeyStore.record_outcome
# ---------------------------------------------------------------------------
...
class TestWarmPoolAcquireCounter:
    ...
    def _make_store(self, tmp_path):
        from codex_crew.mcp_gateway.prewarm import HotKeyStore
    ...
    def test_hit_emits_counter(self, tmp_path):
        ...
        with patch("codex_crew.mcp_gateway.prewarm.get_recorder", return_value=rec):
            store.record_outcome(hit=True)
        assert rec.counters == [("codexcrew.mcp.warm_pool.acquire", {"result": "hit"})]
    ...
    def test_miss_emits_counter(self, tmp_path):
        ...
        assert rec.counters == [("codexcrew.mcp.warm_pool.acquire", {"result": "miss"})]
...
# ---------------------------------------------------------------------------
# codexcrew.mcp.backend.acquire.duration — gatewayd._emit_backend_acquire_metric
# ---------------------------------------------------------------------------
...
class TestBackendAcquireMetric:
    ...
    def test_warm_hit(self):
        from codex_crew.mcp_gateway.gatewayd import _emit_backend_acquire_metric

        rec = _CapturingRecorder()
        with patch("codex_crew.mcp_gateway.gatewayd.get_recorder", return_value=rec):
            _emit_backend_acquire_metric(12.5, warm=True)
        assert rec.histograms == [
            ("codexcrew.mcp.backend.acquire.duration", 12.5, "ms", {"warm": True})
        ]
    ...
    def test_cold_spawn(self):
        ...
        assert rec.histograms == [
            ("codexcrew.mcp.backend.acquire.duration", 340.0, "ms", {"warm": False})
        ]


# ---------------------------------------------------------------------------
# codexcrew.mcp.lazy_load.{count,duration} — gatewayd._emit_lazy_load_metrics
# ---------------------------------------------------------------------------
...
class TestLazyLoadMetrics:
    ...
    def test_emits_count_duration_and_acquire(self):
        from codex_crew.mcp_gateway.gatewayd import _emit_lazy_load_metrics
        ...
        assert rec.counters == [("codexcrew.mcp.lazy_load.count", {"transport": "stdio"})]
        ...
        assert by_name["codexcrew.mcp.lazy_load.duration"] == (
            "codexcrew.mcp.lazy_load.duration", 250.0, "ms", {"transport": "stdio"},
        )
        assert by_name["codexcrew.mcp.backend.acquire.duration"] == (
            "codexcrew.mcp.backend.acquire.duration", 250.0, "ms", {"warm": False},
        )
