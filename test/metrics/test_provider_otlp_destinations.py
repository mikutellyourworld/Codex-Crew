...
from codex_crew.metrics.provider import get_recorder, reset_for_testing
...
def _fake_readers(monkeypatch, *, fail_on=None):
    ...
    Patched on ``codex_crew.metrics.provider`` where the names are looked up, not
...
class TestTemporalityIsACoreDecision:
    ...
    def test_the_otlp_leg_gets_the_map_the_local_sink_uses(self, tmp_path, monkeypatch):
        ...
        from codex_crew.metrics.local_exporter import JsonlMetricExporter
        from codex_crew.metrics.temporality import TEMPORALITY_ENV_VAR
...
class TestEgressPostureIsSingleSourced:
    ...
    def test_a_provider_destination_counts_as_egress_with_no_config_endpoint(self):
        from codex_crew.metrics.provider import otlp_egress_active
    ...
    def test_a_provider_without_the_method_answers_no_not_unknown(self, monkeypatch):
        ...
        from codex_crew.dashboard.handlers import telemetry as telemetry_handlers
        from codex_crew.metrics.provider import _otlp_destinations, otlp_egress_active
