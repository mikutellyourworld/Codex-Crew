...
from codex_crew.messaging.link import TELEMETRY_CHANNELS, telemetry_channel_of
from codex_crew.session import POOL_DECISIONS
...
class TestStartupAttrs:
    ...
    def test_resume_outcome_counter_emitted_when_present(self):
        ...
        assert ("codexcrew.session.resume.outcome", {
...
class TestPrefixDrift:
    ...
    def test_every_session_manager_prefix_has_a_label(self):
        from codex_crew import session as session_mod
...
class TestPoolDecisions:
    ...
    def test_unmodelled_decision_folds_to_other(self):
        ...
        with patch("codex_crew.session.get_recorder", return_value=rec):
