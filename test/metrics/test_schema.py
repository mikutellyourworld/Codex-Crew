"""Tests for codex_crew.metrics.schema — tel-01 foundation contracts."""

import base64

import pytest

from codex_crew.metrics.schema import (
    MAX_ATTR_COUNT,
    MAX_ATTR_VALUE_LEN,
    redact,
    validate_attrs,
...
# ---------------------------------------------------------------------------
# C4 — Namespace validation: app cannot spoof codexcrew.*
# ---------------------------------------------------------------------------
...
class TestValidateName:
    ...
    def test_app_cannot_spoof_codexcrew_namespace(self):
        """Apps MUST NOT be able to emit metrics under codexcrew.*"""
        with pytest.raises(ValueError, match="cannot emit.*codexcrew"):
            validate_name("codexcrew.session.count", app_id="my_app")
    ...
    def test_core_valid_codexcrew_name(self):
        """Core caller can use codexcrew.* namespace."""
        result = validate_name("codexcrew.session.duration")
        assert result == "codexcrew.session.duration"
