...
subprocess calls in :mod:`codex_crew.service.linux` and
:mod:`codex_crew.service.macos` are mocked.
"""
...
from codex_crew.service import common, controller
from codex_crew.service.common import (
    LAUNCHD_LABEL,
    SERVICE_NAME,
    Platform,
    current_platform,
...
class TestPlatformDetection:
    def test_linux_with_systemctl_returns_systemd(self):
        with patch("codex_crew.service.common.sys") as mock_sys, patch(
            "codex_crew.service.common.shutil.which",
    ...
    def test_linux_without_systemctl_returns_unsupported(self):
        with patch("codex_crew.service.common.sys") as mock_sys, patch(
            "codex_crew.service.common.shutil.which", return_value=None
        ):
            mock_sys.platform = "linux"
...
class TestManagedServiceMarkerDetection:
    ...
    def test_launchd_definition_reads_environment_dictionary(self, monkeypatch, tmp_path):
        path = tmp_path / "dev.codexcrew.gateway.plist"
        ...
        path.write_bytes(plistlib.dumps({"Label": "dev.codexcrew.gateway"}))
        assert controller.installed_service_has_managed_marker() is False
        path.write_bytes(
            plistlib.dumps(
                {"EnvironmentVariables": {"CODEXCREW_SERVICE_MANAGED": "1"}}
            )
        )
        assert controller.installed_service_has_managed_marker() is True
...
class TestShutdownBudget:
    def test_service_deadline_covers_gateway_grace(self):
        from codex_crew.gateway_shutdown_budget import (
...
class TestLinuxUnitRendering:
    """The rendered systemd unit should reference the resolved codexcrew bin."""

    def test_render_unit_includes_exec_start(self, tmp_path, monkeypatch):
        from codex_crew.service import linux as svc_linux
        ...
        with patch(
            "codex_crew.service.common.shutil.which",
            return_value="/home/u/.toolbox/bin/codexcrew",
        ), patch(
            "codex_crew.service.linux.subprocess.run", return_value=gid_result
        ):
        ...
        assert 'ExecStart="/home/u/.toolbox/bin/codexcrew" gateway --no-open' in unit
    ...
    def test_render_unit_carries_the_session_bus_environment(self, monkeypatch):
        ...
        with patch(
            "codex_crew.service.common.shutil.which",
            return_value="/usr/local/bin/codexcrew",
        ), patch(
            "codex_crew.service.linux.subprocess.run", return_value=gid_result
        ), patch.object(
        ...
        assert 'Environment="CODEXCREW_SERVICE_MANAGED=1"\n' in unit
    ...
    def test_session_bus_is_systemd_only_not_in_the_shared_environment(self):
        ...
        from codex_crew.service.common import service_environment

        keys = set(service_environment("/home/tester"))
        assert "XDG_RUNTIME_DIR" not in keys
        assert "DBUS_SESSION_BUS_ADDRESS" not in keys
        assert service_environment("/home/tester")["CODEXCREW_SERVICE_MANAGED"] == "1"
    ...
    def test_render_unit_falls_back_to_argv0_when_codexcrew_not_on_path(self, monkeypatch):
        from codex_crew.service import linux as svc_linux

        monkeypatch.setenv("USER", "tester")
        with patch("codex_crew.service.common.shutil.which", return_value=None), patch.object(
            sys, "argv", ["/some/path/codexcrew"]
        ):
            unit = svc_linux.render_unit()
        # argv[0] is realpathed; just check the unit references *something*
        # that ends in the (quoted) codexcrew executable followed by gateway.
        assert 'codexcrew" gateway' in unit
    ...
    def test_install_writes_unit_via_sudo_install_and_invokes_systemctl(
        ...
        with patch(
            ...
            "codex_crew.service.linux.subprocess.run", return_value=ok
        ) as run:
            svc_linux.install()

        # Four things must happen:
        # 1) `sudo install -m 0644 -o root -g root <tmp> /etc/systemd/system/codexcrew.service`
        # 2) `sudo systemctl daemon-reload`
        # 3) `sudo systemctl enable codexcrew.service`
        # 4) `sudo systemctl restart codexcrew.service`
    ...
    def test_install_raises_with_clear_error_when_sudo_install_fails(
        ...
        with patch(
            ...
            "codex_crew.service.linux.subprocess.run", return_value=install_failed
        ):
    ...
    def test_uninstall_is_idempotent_when_unit_missing(self, tmp_path, monkeypatch):
        ...
        with patch("codex_crew.service.linux.subprocess.run") as run:
            svc_linux.uninstall()
...
class TestLinuxPrivilegeResolution:
    ...
    def test_current_user_prefers_sudo_user_over_root(self, monkeypatch):
        """`sudo codexcrew service install` must target the human behind sudo,
    ...
    def test_unit_home_matches_the_resolved_user_not_process_home(self, monkeypatch):
        ...
        with patch(
            "codex_crew.service.common.shutil.which", return_value="/usr/local/bin/codexcrew"
        ), patch("codex_crew.service.linux.subprocess.run", return_value=gid):
            unit = svc_linux.render_unit()
    ...
    def test_sudo_run_survives_missing_sudo_binary(self, monkeypatch):
        ...
        res = svc_linux._sudo_run("systemctl", "restart", "codexcrew.service")
...
class TestLinuxEnvironmentFile:
    ...
    CODEXCREW_PORT on the service and it did not change the port'."""
    ...
    def test_seed_env_file_creates_when_absent(self, tmp_path, monkeypatch):
        ...
        env_file = tmp_path / "codexcrew" / "codexcrew.env"
        ...
        assert "#CODEXCREW_PORT=" in written["contents"]
    ...
    def test_seed_env_file_never_clobbers_operator_edits(self, tmp_path, monkeypatch):
        ...
        env_file.write_text("CODEXCREW_PORT=5477\n")
        monkeypatch.setattr(svc_linux, "ENV_DIR", env_file.parent)
        monkeypatch.setattr(svc_linux, "ENV_FILE_PATH", env_file)
        ...
        assert env_file.read_text() == "CODEXCREW_PORT=5477\n"

    def test_seed_env_file_is_non_fatal_when_probe_denied(self, tmp_path, monkeypatch):
        """A pre-existing root-only /etc/codexcrew must not abort install: the
    ...
    def test_uninstall_preserves_an_operator_edited_env_file(self, tmp_path, monkeypatch):
        ...
        unit = tmp_path / "codexcrew.service"
...
class TestLinuxServiceWritesUtf8:
    ...
    def _capture_staged_bytes(self, call, contents: str) -> bytes:
        ...
        with (
            patch("codex_crew.service.linux.subprocess.run", side_effect=_fake_run),
            patch("codex_crew.service.linux._privilege_prefix", return_value=["sudo"]),
        ):
            call(contents)
        return staged["raw"]
    ...
    def test_a_non_ascii_home_reaches_the_unit_writer(self, monkeypatch):
        ...
        with (
            patch(
                "codex_crew.service.common.shutil.which", return_value="/home/usuario/bin/codexcrew"
            ),
            patch("codex_crew.service.linux.subprocess.run", return_value=gid_result),
            patch("codex_crew.service.linux._home_for_user", return_value="/home/usuario"),
        ):
            unit = svc_linux.render_unit()
    ...
    def test_the_install_file_write_stages_utf8_bytes(self, tmp_path):
        ...
        contents = "# Codex Crew — überschrift\nCODEXCREW_PORT=5477\n"
...
class TestMacOSPlistRendering:
    def test_plist_and_unit_never_auto_open_a_browser(self, monkeypatch, tmp_path):
        ...
        """
        from codex_crew.service import linux as svc_linux
        from codex_crew.service import macos as svc_macos

        monkeypatch.setattr(svc_macos, "LIVE_PROGRAM", tmp_path / "live-gateway")
        with patch(
            "codex_crew.service.common.shutil.which", return_value="/opt/homebrew/bin/codexcrew"
        ):
        ...
        with patch(
            ...
            unit = svc_linux.render_unit()
        assert 'ExecStart="/usr/local/bin/codexcrew" gateway --no-open' in unit
    ...
    def test_render_plist_runs_the_live_program_not_the_resolved_bin(self, monkeypatch, tmp_path):
        ...
        with patch(
            "codex_crew.service.common.shutil.which",
            return_value="/opt/homebrew/bin/codexcrew",
        ):
            plist = svc_macos.render_plist()
        assert f"<string>{LAUNCHD_LABEL}</string>" in plist
        assert f"<string>{link}</string>" in plist
        assert "/opt/homebrew/bin/codexcrew" not in plist
    ...
    def test_render_plist_pins_bounded_graceful_restart_contract(self, tmp_path):
        from codex_crew.gateway_shutdown_budget import TOTAL_SHUTDOWN_BUDGET_SECS
        from codex_crew.service import macos as svc_macos
    ...
    def test_install_writes_plist_and_loads(self, tmp_path, monkeypatch):
        ...
        with patch(
            "codex_crew.service.common.shutil.which",
            return_value="/opt/homebrew/bin/codexcrew",
        ), patch("codex_crew.service.macos.subprocess.run", return_value=run) as proc:
            svc_macos.install()
...
class TestControllerDispatch:
    def test_install_unsupported_returns_2(self):
        from codex_crew.service import controller

        with patch(
            "codex_crew.service.controller.current_platform",
    ...
    def test_manual_restart_hint_systemd_names_sudo_systemctl(self):
        ...
        with patch(
            ...
            "codex_crew.service.common.current_platform",
            return_value=Platform.SYSTEMD,
        ):
            hint = controller.manual_restart_hint()
        assert hint == "sudo systemctl restart codexcrew"
    ...
    def test_manual_restart_hint_launchd_names_a_recovery_pair(self):
        ...
        from codex_crew.service import controller
        from codex_crew.service import macos as svc_macos
        from codex_crew.service.common import LAUNCHD_LABEL

        with patch(
            "codex_crew.service.controller.current_platform",
            return_value=Platform.LAUNCHD,
        ), patch("codex_crew.service.controller.os.getuid", return_value=501, create=True):
            hint = controller.manual_restart_hint()
    ...
    def test_manual_restart_hint_never_circular(self):
        ...
        for plat in (Platform.SYSTEMD, Platform.LAUNCHD, Platform.UNSUPPORTED):
            with patch(
                ...
                assert controller.manual_restart_hint() != "codexcrew restart"
    ...
    def test_status_routes_to_systemd_active(self, capsys):
        ...
        with patch(
            ...
            svc_linux, "status", return_value="● codexcrew.service\n"
        ), patch.object(svc_linux, "is_active", return_value=True):
            rc = controller.service_status()
        assert rc == 0
        assert "codexcrew.service" in capsys.readouterr().out
...
class TestLinuxControlPaths:
    ...
    def test_uninstall_runs_full_teardown_when_unit_exists(self, tmp_path, monkeypatch):
        ...
        unit_path = tmp_path / "codexcrew.service"
    ...
    def test_is_active_returns_true_when_systemctl_says_active(self):
        ...
        with patch(
            "codex_crew.service.linux.subprocess.run", return_value=active_result
        ) as run:
    ...
    def test_is_active_returns_false_when_inactive(self):
        ...
        with patch(
            "codex_crew.service.linux.subprocess.run", return_value=inactive_result
        ):
    ...
    def test_restart_returns_false_on_nonzero_exit(self):
        ...
        with patch(
            "codex_crew.service.linux.subprocess.run", return_value=failed
        ):
            assert svc_linux.restart() is False
    ...
    def test_status_returns_systemctl_output(self):
        ...
        result = MagicMock(
            returncode=0, stdout="● codexcrew.service - active\n", stderr=""
        )
        with patch(
            "codex_crew.service.linux.subprocess.run", return_value=result
        ) as run:
            out = svc_linux.status()
        assert "codexcrew.service" in out
    ...
    def test_install_propagates_failure_at_daemon_reload(self, monkeypatch):
        ...
        with patch(
            ...
            "codex_crew.service.linux.subprocess.run", side_effect=responder
        ):
            with pytest.raises(svc_linux.ServiceInstallError) as exc_info:
                svc_linux.install()
    ...
    def test_current_group_falls_back_to_username_when_id_fails(self, monkeypatch):
        ...
        with patch(
            "codex_crew.service.linux.subprocess.run",
...
class TestMacOSControlPaths:
    ...
    def test_install_unloads_existing_plist_before_writing(self, tmp_path, monkeypatch):
        ...
        with patch(
            ...
            "codex_crew.service.macos.subprocess.run", return_value=ok
        ) as run:
            svc_macos.install()
    ...
    def test_uninstall_idempotent_when_plist_missing(self, tmp_path, monkeypatch):
        ...
        with patch("codex_crew.service.macos.subprocess.run") as run:
            svc_macos.uninstall()
        run.assert_not_called()
    ...
    def test_is_active_returns_false_when_launchctl_errors(self):
        ...
        with patch("codex_crew.service.macos.subprocess.run", return_value=not_loaded):
    ...
    def test_is_active_returns_true_with_pid_in_output(self):
        ...
        loaded = MagicMock(
            returncode=0,
            stdout='{\n\t"PID" = 1234;\n\t"Label" = "dev.codexcrew.gateway";\n}\n',
            stderr="",
        )
        with patch("codex_crew.service.macos.subprocess.run", return_value=loaded):
    ...
    def test_is_active_returns_true_when_loaded_without_pid_line(self):
        ...
        with patch(
            "codex_crew.service.macos.subprocess.run", return_value=loaded_no_pid
        ):
            assert svc_macos.is_active() is True
    ...
    def test_restart_is_false_when_launchd_rejects_it(self, tmp_path, monkeypatch):
        ...
        with patch("codex_crew.service.macos.subprocess.run", return_value=bad):
    ...
    def test_codexcrew_bin_falls_back_to_argv0(self, monkeypatch):
        """If `codexcrew` is not on PATH, codexcrew_bin should resolve
        sys.argv[0] rather than crash."""
        from codex_crew.service import common as svc_common

        monkeypatch.setattr(sys, "argv", ["/some/path/codexcrew"])
        with patch("codex_crew.service.common.shutil.which", return_value=None):
            assert "codexcrew" in svc_common.codexcrew_bin()
...
class TestRestartCommandHint:
    ...
    hardcoding ``systemctl --user restart codexcrew``, which fails on the
    ...
    def test_launchd_returns_service_aware_cli(self, monkeypatch):
        ...
        assert svc_common.restart_command_hint() == "codexcrew restart"
...
class TestKirocrewBinOverride:
    def test_service_bin_override_wins_over_which(self, monkeypatch):
        monkeypatch.setenv("CODEXCREW_SERVICE_BIN", "/opt/wrapper/codexcrew")
        with patch(
            "codex_crew.service.common.shutil.which",
            return_value="/usr/local/bin/codexcrew",
        ):
            assert codexcrew_bin() == "/opt/wrapper/codexcrew"

    def test_falls_back_to_which_when_override_unset(self, monkeypatch):
        monkeypatch.delenv("CODEXCREW_SERVICE_BIN", raising=False)
        with patch(
            "codex_crew.service.common.shutil.which",
            return_value="/usr/local/bin/codexcrew",
        ):
            assert codexcrew_bin() == "/usr/local/bin/codexcrew"

    def test_blank_override_is_ignored(self, monkeypatch):
        monkeypatch.setenv("CODEXCREW_SERVICE_BIN", "   ")
        with patch(
            "codex_crew.service.common.shutil.which",
            return_value="/usr/local/bin/codexcrew",
        ):
    ...
    def test_relative_override_is_made_absolute(self, monkeypatch):
        ...
        monkeypatch.setenv("CODEXCREW_SERVICE_BIN", "./.venv/bin/codexcrew")
        result = codexcrew_bin()
        assert os.path.isabs(result)
        assert result == os.path.abspath("./.venv/bin/codexcrew")
...
class TestServiceEnvironment:
    ...
    def test_always_sets_home_path_and_locale(self, monkeypatch):
        ...
        monkeypatch.delenv("CODEXCREW_KIRO_BIN", raising=False)
    ...
    def test_propagates_port_only_when_set(self, monkeypatch):
        """CODEXCREW_PORT reaches the installed service.

        It is the ONLY input DASHBOARD_PORT reads, so a service definition that
        cannot carry it can only ever bind the default 5486 — broken by
        construction on any host where that port is taken, which includes every
        host running Codex Crew's own instance tunnel (it pins
        local_port == remote_port).
        """
        monkeypatch.delenv("CODEXCREW_PORT", raising=False)
        assert "CODEXCREW_PORT" not in service_environment("/home/tester")
        monkeypatch.setenv("CODEXCREW_PORT", "5477")
        assert service_environment("/home/tester")["CODEXCREW_PORT"] == "5477"
    ...
    def test_port_reaches_both_rendered_service_definitions(self, monkeypatch, tmp_path):
        ...
        assert "<key>CODEXCREW_PORT</key>" in envs and "<string>5477</string>" in envs
        assert "<key>CODEXCREW_SERVICE_MANAGED</key>" in envs
        ...
        with patch(
            ...
            unit = svc_linux.render_unit()
        assert "CODEXCREW_PORT=5477" in unit
        assert "CODEXCREW_SERVICE_MANAGED=1" in unit

    def test_propagates_kiro_bin_pin_only_when_set(self, monkeypatch):
        monkeypatch.delenv("CODEXCREW_KIRO_BIN", raising=False)
        assert "CODEXCREW_KIRO_BIN" not in service_environment("/home/tester")
        monkeypatch.setenv("CODEXCREW_KIRO_BIN", "/opt/shim/kiro-cli")
        env = service_environment("/home/tester")
        assert env["CODEXCREW_KIRO_BIN"] == "/opt/shim/kiro-cli"
    ...
    def test_kiro_bin_pin_is_absolutized(self, monkeypatch):
        ...
        monkeypatch.setenv("CODEXCREW_KIRO_BIN", "./kiro-cli")
        env = service_environment("/home/tester")
        assert os.path.isabs(env["CODEXCREW_KIRO_BIN"])
        assert env["CODEXCREW_KIRO_BIN"] == os.path.abspath("./kiro-cli")
    ...
    def test_plist_includes_locale_and_kiro_bin(self, monkeypatch):
        ...
        assert "<key>CODEXCREW_KIRO_BIN</key>" in plist
    ...
    def test_unit_includes_locale_and_kiro_bin(self, monkeypatch):
        ...
        assert 'Environment="CODEXCREW_KIRO_BIN=/opt/shim/kiro-cli"\n' in unit

    def test_unit_quotes_spaced_program_and_env(self, monkeypatch):
        # A spaced CODEXCREW_SERVICE_BIN / CODEXCREW_KIRO_BIN must not split the
        # ExecStart exec (203/EXEC) or truncate the env value at the space.
        from codex_crew.service import linux as svc_linux

        monkeypatch.setenv("USER", "tester")
        monkeypatch.setenv("CODEXCREW_SERVICE_BIN", "/opt/Codex Crew/codexcrew")
        monkeypatch.setenv("CODEXCREW_KIRO_BIN", "/opt/Codex Crew/kiro-cli")
        unit = svc_linux.render_unit()
        assert 'ExecStart="/opt/Codex Crew/codexcrew" gateway' in unit
        assert 'Environment="CODEXCREW_KIRO_BIN=/opt/Codex Crew/kiro-cli"\n' in unit
        # The bare unquoted forms must NOT appear (would break systemd parsing).
        assert "ExecStart=/opt/Codex Crew/codexcrew gateway" not in unit
    ...
    def test_unit_escapes_percent_specifiers(self, monkeypatch):
        ...
        monkeypatch.setenv("USER", "tester")
        monkeypatch.setenv("CODEXCREW_SERVICE_BIN", "/opt/100%/codexcrew")
        unit = svc_linux.render_unit()
        assert 'ExecStart="/opt/100%%/codexcrew" gateway' in unit
        # The single-% form must NOT survive (systemd would treat %/ as a
        # specifier). Guard against a bare "/opt/100%/codexcrew" in ExecStart.
        assert "/opt/100%/codexcrew" not in unit

    def test_sd_quote_escape_order(self):
        from codex_crew.service.linux import _sd_quote
    ...
    def test_render_unit_rejects_newline_injection(self, monkeypatch):
        ...
        monkeypatch.setenv("USER", "tester")
        monkeypatch.setenv(
            "CODEXCREW_SERVICE_BIN", "/opt/x/codexcrew\nUser=root\nExecStart=/evil"
        )
        with pytest.raises(ValueError):
    ...
    def test_live_program_quotes_a_spaced_override(self, monkeypatch, tmp_path):
        ...
        # Compared against what codexcrew_bin() actually returned rather than a
        ...
        monkeypatch.setattr(svc_macos, "LIVE_PROGRAM", link)
        monkeypatch.setenv("CODEXCREW_SERVICE_BIN", "/opt/Codex Crew/codexcrew")
        resolved = svc_macos.codexcrew_bin()
    ...
    def test_live_program_escapes_a_single_quote_in_the_path(self):
        ...
        script = svc_macos.render_live_program("/opt/it's/codexcrew")
        assert """exec '/opt/it'\\''s/codexcrew' "$@\"""" in script
    ...
    def test_write_live_program_is_atomic_and_leaves_no_temp_files(self, monkeypatch, tmp_path):
        ...
        link = tmp_path / "live-gateway"
        monkeypatch.setattr(svc_macos, "LIVE_PROGRAM", link)
        svc_macos.write_live_program(svc_macos.render_live_program("/first/codexcrew"))
        svc_macos.write_live_program(svc_macos.render_live_program("/second/codexcrew"))
        assert "'/second/codexcrew'" in link.read_text()
        assert "'/first/codexcrew'" not in link.read_text()
...
class TestEnsureLiveProgram:
    ...
    def _agent(self, monkeypatch, tmp_path, *, indirected=True, fmt=None):
        ...
        plist = tmp_path / "dev.codexcrew.gateway.plist"
        target = str(launcher) if indirected else "/usr/local/bin/codexcrew"
        ...
        plist.write_bytes(plistlib.dumps(
            {
                "Label": "dev.codexcrew.gateway",
                "EnvironmentVariables": {"CODEXCREW_PORT": "5477"},
    ...
    def test_reads_a_binary_plist_rather_than_assuming_utf8_text(
        ...
        monkeypatch.setenv(
            "CODEXCREW_SERVICE_BIN", str(self._exe(tmp_path / "bin" / "codexcrew"))
        )
    ...
    def test_restores_a_deleted_launcher_and_leaves_the_plist_untouched(
        ...
        svc, launcher, plist = self._agent(monkeypatch, tmp_path)
        pinned = self._exe(tmp_path / "opt" / "codexcrew")
        monkeypatch.setenv("CODEXCREW_SERVICE_BIN", str(pinned))
        # Read the target back off the resolver rather than restating a literal:
        # codexcrew_bin() absolutizes, so the path SHAPE differs per platform
        # (Windows resolves a rooted POSIX path to a drive-qualified one).
        resolved = svc.codexcrew_bin()
    ...
    def test_refuses_to_write_a_launcher_that_execs_a_non_executable(
        ...
        """`python -m codex_crew` with no console script resolves to `__main__.py`.
        ...
        monkeypatch.setenv("CODEXCREW_SERVICE_BIN", str(not_exec))
    ...
    def test_refuses_rather_than_falling_back_to_path(self, monkeypatch, tmp_path):
        ...
        `codexcrew` ahead of this one would be persisted into the agent — the
        mismatch this repair exists to end, recreated by the repair.
        """
        svc, launcher, _plist = self._agent(monkeypatch, tmp_path)
        monkeypatch.delenv("CODEXCREW_SERVICE_BIN", raising=False)
        stray = self._exe(tmp_path / "stray" / "codexcrew")
        from codex_crew.service import common as svc_common
        monkeypatch.setattr(svc_common.shutil, "which", lambda _n: str(stray))
        # An interpreter directory with NO codexcrew beside it.
        bare = tmp_path / "bare"
        bare.mkdir()
        monkeypatch.setattr(svc.sys, "executable", str(bare / "python"))

        with pytest.raises(OSError, match="no codexcrew console script"):
            svc.ensure_live_program()
    ...
    def test_targets_the_repairing_install_not_whatever_path_finds(
        ...
        """A stray `codexcrew` earlier on PATH must not be baked into the launcher.
        ...
        monkeypatch.delenv("CODEXCREW_SERVICE_BIN", raising=False)
        stray = self._exe(tmp_path / "stray" / "codexcrew")
        # codexcrew_bin() lives in service.common and resolves through ITS shutil.
        from codex_crew.service import common as svc_common
        monkeypatch.setattr(svc_common.shutil, "which", lambda _n: str(stray))
        # The console script that ships beside the running interpreter.
        mine = self._exe(
            tmp_path / "mine" / ("codexcrew.exe" if os.name == "nt" else "codexcrew")
        )
    ...
    def test_an_explicit_service_bin_override_still_wins(self, monkeypatch, tmp_path):
        ...
        svc, launcher, _plist = self._agent(monkeypatch, tmp_path)
        pinned = self._exe(tmp_path / "pinned" / "codexcrew")
        monkeypatch.setenv("CODEXCREW_SERVICE_BIN", str(pinned))
        resolved = svc.codexcrew_bin()
    ...
    def test_is_a_noop_when_the_launcher_is_already_there(self, monkeypatch, tmp_path):
        ...
        svc, launcher, _plist = self._agent(monkeypatch, tmp_path)
        launcher.write_text("#!/bin/sh\ncd '/wt/live' || exit 1\nexec '/wt/live/.venv/bin/codexcrew' \"$@\"\n")
...
class TestLauncherReconcileIsProductionOnly:
    ...
    LIVE_PROGRAM is a per-user path that CODEXCREW_HOME does not scope, so a dev,
    ...
    def test_an_isolated_home_does_not_reconcile(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path / ".codexcrew-dev"))
        monkeypatch.setattr(cli_server.sys, "platform", "darwin")
    ...
    def test_the_desktop_bundle_never_reconciles(self, monkeypatch, tmp_path):
        ...
        """
        from codex_crew import cli_server, platform_compat
...
class TestAppArmorGate:
    ...
    *without* the restriction and must be left completely alone.
    ...
    def _gate(monkeypatch, *, lsm="apparmor,capability", sysctl="1", parser="/usr/sbin/apparmor_parser", version=(5, 0)):
        from codex_crew.service import apparmor as aa
...
class TestAppArmorProfileRendering:
    ...
    _EXEC = Path("/opt/codexcrew-venv/bin/codexcrew")
    ...
    def test_documents_that_removal_rebreaks_the_sandbox(self):
        ...
        text = aa.render_profile("4.0", self._EXEC)
        assert "Managed by Codex Crew" in text
...
class TestAppArmorInstall:
    ...
    _EXEC = "/opt/codexcrew-venv/bin/codexcrew"
    ...
    def test_exec_path_attaches_the_profile_to_the_resolved_launcher(
        ...
        launcher = durable_dir / "codexcrew"
        launcher.write_text("#!/bin/sh\n")
    ...
    def test_an_unresolvable_exec_path_is_a_clean_non_fatal_skip(self, monkeypatch):
        ...
        outcome = aa.install(
            write, run, lambda *_a: (0, ""), 1000, 1000,
            exec_path="/nonexistent/path/codexcrew",
        )
...
class TestAppArmorUnitDirective:
    ...
    def test_no_directive_by_default(self, monkeypatch):
        ...
        with patch(
            "codex_crew.service.common.shutil.which", return_value="/usr/bin/codexcrew"
        ), patch("codex_crew.service.linux.subprocess.run", return_value=gid):
            unit = svc_linux.render_unit()
...
class TestInstallApparmorProfileAttachesToTheLauncher:
    ...
    def test_passes_codexcrew_bin_as_exec_path_and_threads_expected_uid(self, monkeypatch):
        from codex_crew.service import apparmor as aa
        from codex_crew.service import linux as svc_linux

        monkeypatch.setattr(svc_linux, "codexcrew_bin", lambda: "/opt/codexcrew-venv/bin/codexcrew")
        calls = []
        ...
        assert kwargs["exec_path"] == "/opt/codexcrew-venv/bin/codexcrew"
...
class TestAppArmorNeverFailsTheInstall:
    ...
    def _patched(monkeypatch, outcome):
        from codex_crew.service import controller
        from codex_crew.service.common import Platform
    ...
    def test_install_still_succeeds_when_the_profile_fails(self, monkeypatch, capsys):
        from codex_crew.service.apparmor import ProfileOutcome
        ...
        assert "codexcrew service installed and started" in out
...
class TestEnforcementVerificationIsSafeAndFaithful:
    ...
    def test_uses_a_trusted_python_never_the_user_writable_venv(self, monkeypatch):
        ...
        assert "codex_crew" not in " ".join(argv)
...
class TestLauncherExecPathIsSafeToAttach:
    ...
    def test_rejects_a_relative_path(self):
        ...
        resolved, problem = aa.validate_exec_path("codexcrew.AppImage")
    ...
    def test_rejects_a_world_writable_location(self, tmp_path, monkeypatch):
        ...
        app = tmp_path / "codexcrew.AppImage"
    ...
    @pytest.mark.parametrize(
        "path",
        [
            "/home/user/AppImages/codexcrew.AppImage",
            "/opt/CodexCrew/codexcrew",
            "/usr/bin/codexcrew-desktop",
    ...
    def test_resolves_before_validating_so_a_symlink_cannot_smuggle_a_grant(
        ...
        link = tmp_path / "codexcrew.AppImage"
    ...
    def test_accepts_a_durable_path_with_a_space(self, durable_dir):
        ...
        app = durable_dir / "Codex Crew.AppImage"
        app.write_text("#!/bin/sh\n")
...
class TestExpectedUidOverride:
    ...
    ``codexcrew service install`` itself runs as root or under ``sudo``."""

    def test_a_file_owned_by_the_expected_uid_is_accepted(self, tmp_path):
        from codex_crew.service import apparmor as aa

        app = tmp_path / "codexcrew"
        app.write_text("#!/bin/sh\n")
    ...
    def test_a_file_owned_by_the_installer_but_not_the_expected_account_is_refused(
        ...
        codexcrew service install``), but that is not the account the SERVICE
...
class TestLauncherProfileRendering:
    ...
    def _render(self, path="/home/u/Apps/codexcrew.AppImage", abi="4.0"):
        from codex_crew.service import apparmor as aa
    ...
    def test_attaches_to_the_given_path_in_quotes(self):
        ...
        text = self._render("/home/u/My Apps/codexcrew.AppImage")

        assert (
            f'profile {aa.LAUNCHER_PROFILE_NAME} "/home/u/My Apps/codexcrew.AppImage" '
    ...
    def test_explains_that_moving_the_file_breaks_it(self):
        ...
        assert "codexcrew sandbox status" in text
    ...
    def test_round_trips_through_the_attachment_parser(self, monkeypatch, tmp_path):
        ...
        profile.write_text(self._render("/home/u/Apps/codexcrew.AppImage"))
        monkeypatch.setattr(aa, "LAUNCHER_PROFILE_PATH", profile)

        assert aa.installed_attachment() == "/home/u/Apps/codexcrew.AppImage"
...
class TestLauncherStatusTellsTheTruth:
    ...
    def test_unaffected_host_is_reported_as_fine(self, monkeypatch):
        ...
        ok, detail = aa.launcher_status("/home/u/Apps/codexcrew.AppImage")
    ...
    def test_missing_profile_on_an_appimage_launch_names_the_command(self, monkeypatch):
        ...
        assert ok is False
        assert "codexcrew sandbox install-profile" in detail
    ...
    def test_missing_profile_without_an_appimage_points_at_the_service(self, monkeypatch):
        ...
        assert ok is False
        assert "codexcrew service install" in detail
    ...
    def test_a_moved_appimage_is_reported_as_not_covered(self, monkeypatch, durable_dir):
        ...
        app = durable_dir / "codexcrew.AppImage"
        app.write_text("#!/bin/sh\n")
        self._restricted(monkeypatch, aa)
        monkeypatch.setattr(aa, "installed_attachment", lambda: "/old/place/codexcrew.AppImage")

        ok, detail = aa.launcher_status(str(app))

        assert ok is False
        assert "/old/place/codexcrew.AppImage" in detail
...
class TestLauncherInstallIsFailSoftAndHonest:
    ...
    def test_warns_about_a_conflicting_hand_written_profile(self, monkeypatch, durable_dir):
        ...
        self._ready(monkeypatch, aa)
        monkeypatch.setattr(
            aa, "conflicting_attachment", lambda _p: "/etc/apparmor.d/codexcrew"
        )
        ...
        assert "/etc/apparmor.d/codexcrew" in outcome.message
    ...
    def test_default_exec_path_reads_appimage(self, monkeypatch):
        ...
        monkeypatch.setenv("APPIMAGE", "/home/u/Apps/codexcrew.AppImage")
        assert aa.default_exec_path() == "/home/u/Apps/codexcrew.AppImage"
...
class TestSandboxProfileControllerDispatch:
    ...
    def test_install_returns_nonzero_when_the_outcome_is_not_ok(self, capsys):
        from codex_crew.service import apparmor, controller, linux
    ...
    def test_status_exit_code_is_the_answer(self):
        from codex_crew.service import apparmor, controller
...
class TestHeadlessApiKeyDoctorReport:
    ...
    def test_reports_when_a_service_is_installed(self, monkeypatch, capsys, tmp_path):
        issues = self._warn(monkeypatch, tmp_path / "codexcrew.service")
        out = capsys.readouterr().out
    ...
    def test_the_report_cannot_make_doctor_exit_nonzero(self, monkeypatch, tmp_path):
        ...
        assert self._warn(monkeypatch, tmp_path / "codexcrew.service") == []
    ...
    def test_silent_when_the_helper_has_nothing_to_say(
        ...
        issues = self._warn(monkeypatch, tmp_path / "codexcrew.service", warning="")
        assert capsys.readouterr().out == ""
        assert issues == []
    ...
    def test_a_failing_probe_cannot_break_doctor(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(
            ...
            "installed_unit_path",
            lambda: tmp_path / "codexcrew.service",
        )
    ...
    def test_doctor_never_echoes_the_credential_value(
        ...
        issues = self._warn(
            monkeypatch, tmp_path / "codexcrew.service", warning=real()
        )
...
class TestHeadlessApiKeyWarning:
    ...
    def test_custom_home_caveat_only_when_home_is_overridden(
        ...
        assert "CODEXCREW_HOME" not in plain
        with_home = common.headless_auth_warning(
            {self.API_KEY: self.SECRET, "CODEXCREW_HOME": "/srv/crew"}
        )
        assert "CODEXCREW_HOME" in with_home
...
_APPIMAGE = PurePosixPath("/opt/CodexCrew.AppImage")
...
class TestAppArmorConflictingAttachment:
    ...
    def test_our_own_profile_is_never_the_conflict(self, monkeypatch, tmp_path):
        aa = self._dir(monkeypatch, tmp_path)
        (tmp_path / aa.LAUNCHER_PROFILE_NAME).write_text('"/opt/CodexCrew.AppImage" {}')
        assert aa.conflicting_attachment(_APPIMAGE) is None

    @pytest.mark.parametrize(
        "body",
        [
            '"/opt/CodexCrew.AppImage" flags=(attach_disconnected) {}',
            "profile local /opt/CodexCrew.AppImage {}",
            "profile local\n/opt/CodexCrew.AppImage",
        ],
    )
    def test_each_attachment_spelling_is_found(self, body: str, monkeypatch, tmp_path):
        aa = self._dir(monkeypatch, tmp_path)
        other = tmp_path / "local-codexcrew"
    ...
    def test_an_unreadable_file_is_skipped_not_fatal(self, monkeypatch, tmp_path):
        ...
        good.write_text('"/opt/CodexCrew.AppImage" {}')
...
class TestAppArmorLauncherUninstall:
    ...
    def test_it_unloads_then_removes(self, monkeypatch, tmp_path):
        ...
        path = tmp_path / "codexcrew-launcher"
