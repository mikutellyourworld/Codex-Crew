...
from codex_crew.acp.types import ACP_BACKEND_CLAUDE, AcpEvent, TurnUsage
from codex_crew.providers.acp import AcpProvider


def _build_provider(backend: str) -> AcpProvider:
    with patch("codex_crew.providers.acp.AcpClient"):
...
class TestEffortControl:
    ...
    async def test_kiro_change_effort_pushes_slash_command_and_overlay(self):
        ...
        with patch("codex_crew.providers.acp._write_cli_overlay") as wco:
    ...
    async def test_kiro_clear_effort_no_default_returns_false_for_reset(self):
        ...
        with patch("codex_crew.providers.acp._clear_cli_overlay_effort") as cco:
...
class TestStartKiroRuntimeResume:
    ...
    def _kiro_provider(self, model="auto"):
        ...
        provider._client._agent = "codexcrew"
    ...
    async def _run_start(self, provider, resume_sid, file_exists, load_raises=False):
        ...
        with (
            patch("codex_crew.providers.acp.AcpRuntime", return_value=mock_runtime),
            patch(
                "codex_crew.providers.acp.AcpSessionProvider",
    ...
    async def test_failed_load_falls_back_to_create_session(self):
        # An explicit session/load rejection proves no resume landed, so a fresh
        # session is safe.
        provider = self._kiro_provider()
    ...
    @pytest.mark.asyncio
    async def test_timed_out_load_never_creates_a_second_session(self):
        provider = self._kiro_provider()
        provider._client._resume_session_id = "abc-123"

        runtime = MagicMock(pid=4321)
        runtime.spawn = AsyncMock()
        runtime.load_session = AsyncMock(
            side_effect=AcpRequestTimeout("session/load timed out after 90s")
        )
        runtime.create_session = AsyncMock()
        runtime.kill = AsyncMock()

        with (
            patch("codex_crew.providers.acp.AcpRuntime", return_value=runtime),
    ...
    async def test_create_session_failure_kills_orphaned_runtime(self):
        ...
        with (
            patch("codex_crew.providers.acp.AcpRuntime", return_value=mock_runtime),
            patch("codex_crew.providers.acp.AcpSessionProvider"),
        ):
...
class TestKiroStartupMetric:
    ...
    ``codexcrew.session.startup.duration`` tagged ``backend=kiro`` with a phase
    ...
    async def _start(self, provider, *, spawn_exc=None):
        ...
        with (
            ...
            patch("codex_crew.metrics.provider.get_recorder", return_value=rec),
        ):
            await provider._start_kiro_runtime()
        return rec
    ...
    def _phases(self, rec):
        ...
        for name, _ in rec.calls:
            assert name == "codexcrew.session.startup.duration"
    ...
    async def test_auth_required_outcome(self):
        from codex_crew.acp.runtime import AcpRuntimeError
...
class TestFixBDeadRuntimeRespawn:
    ...
    async def test_dead_runtime_respawned_and_create_session_on_new(self):
        ...
        with (
            patch(
                "codex_crew.providers.acp.AcpRuntime",
...
class TestLoadSessionWithRetry:
    ...
    async def test_fast_path_success_no_retry(self):
        ...
        with patch("codex_crew.providers.acp.asyncio.sleep", new=AsyncMock()) as sleep_mock:
    ...
    async def test_persistent_lock_exhausts_and_falls_back(self):
        from codex_crew.providers.acp import _RESUME_MAX_ATTEMPTS
    ...
    async def test_timeout_does_not_fall_back(self):
        provider = _build_provider(backend="")
        timeout = AcpRequestTimeout("session/load timed out after 90s")
        rt = self._runtime(AsyncMock(side_effect=timeout))

        with pytest.raises(AcpRequestTimeout) as exc_info:
            await provider._load_session_with_retry(rt, "/s.json", "sid", None, None)
...
def test_to_llm_event_preserves_provenance_flags():
    ...
    from codex_crew.acp.types import EVENT_PERMISSION_REQUEST, AcpEvent
    from codex_crew.providers.acp import AcpProvider
