...
from codex_crew.tunnel.manager import TunnelManager, TunnelState, TunnelStatus


@pytest.fixture
def manager():
    """Create a TunnelManager with test defaults."""
    return TunnelManager(port=5486, name_mode="username", name_override=None)


class TestTunnelName:
    def test_username_mode(self, manager: TunnelManager):
        assert manager._tunnel_name() == "codexcrew"

    def test_hash_mode(self):
        mgr = TunnelManager(port=5486, name_mode="hash")
        name = mgr._tunnel_name()
        assert name.startswith("codexcrew-")
        assert len(name) == len("codexcrew-") + 8  # 8-char hash

    def test_override(self):
        mgr = TunnelManager(port=5486, name_override="my-custom-tunnel")
...
class TestTunnelStatusEndpoint:
    ...
    async def test_disabled_when_no_manager(self):
        from codex_crew.dashboard.handlers.tunnel import api_tunnel_status
...
class TestPresignedLinkIntegration:
    def test_set_tunnel_url(self):
        from codex_crew.tunnel import get_tunnel_url, set_tunnel_url

        set_tunnel_url("https://codexcrew-gsanc.tunnels.corp.amazon.com")
        assert get_tunnel_url() == "https://codexcrew-gsanc.tunnels.corp.amazon.com"
...
class TestConfigIntegration:
    def test_tunnel_config_defaults(self):
        from codex_crew.config.loader import TunnelConfig
    ...
    def test_tunnel_config_loads_from_json(self, tmp_path):
        ...
        with patch("codex_crew.config.loader.config_path", return_value=config_file):
...
class TestSetupTunnel:
    ...
    async def test_starts_when_token_auth_present(self):
        ...
        with patch("codex_crew.tunnel.setup.TunnelManager") as mock_tm:
            mock_mgr = AsyncMock()
    ...
    async def test_no_tunnel_defaults_to_off(self):
        ...
        from codex_crew.tunnel import publish_disabled, set_publish_disabled
        from codex_crew.tunnel.setup import setup_tunnel
    ...
    async def test_connect_callback_adds_origin(self):
        ...
        with patch("codex_crew.tunnel.setup.TunnelManager", side_effect=capture_tm):
            await setup_tunnel(
                middlewares=[mw],
        ...
        await captured_on_connect("https://gsanc-codexcrew.tunnels.dev")
        assert "https://gsanc-codexcrew.tunnels.dev" in allowed_origins
        assert get_tunnel_url() == "https://gsanc-codexcrew.tunnels.dev"
        set_tunnel_url("")
...
class TestStartLogsDisabledNotice:
    ...
    async def test_start_logs_oss_disabled_notice(self, manager: TunnelManager):
        ...
        with patch("codex_crew.tunnel.manager.logger") as mock_log:
            await manager.start()
...
class TestNoTunnelIsProcessScoped:
    ...
    def test_the_declared_default_allows_publishing(self):
        ...
        import codex_crew.tunnel as tunnel_mod
    ...
    async def test_the_on_demand_link_path_refuses_to_provision(self):
        ...
        does not keep.
        """
        from codex_crew import slack as _slack_pkg  # noqa: F401  (package import)
        from codex_crew.slack import allowlist as al
        from codex_crew.tunnel import set_publish_disabled
        ...
        try:
            with (
                ...
                patch(
                    "codex_crew.platform.current_context",
                    return_value=MagicMock(tunnel=MagicMock(ensure_available=ensure)),
                ),
            ):
                cfg = MagicMock()
                cfg.slack.use_tunnel_url = True
                cfg.dashboard.url = "http://127.0.0.1:5486"
                with patch.object(al.CodexCrewConfig, "load", return_value=cfg):
...
class TestAllowlistTunnelBranch:
    def test_send_dashboard_link_uses_tunnel_url(self):
        ...
        set_tunnel_url("https://gsanc-codexcrew.tunnels.dev")
        try:
            url = get_tunnel_url()
            assert url == "https://gsanc-codexcrew.tunnels.dev"
...
def _install_tunnel_provider(provider):
    ...
    from codex_crew.config.loader import CodexCrewConfig
    from codex_crew.platform import build_default_context, set_context

    ctx = build_default_context(CodexCrewConfig())
...
def _cleanup_registration_order() -> list[str]:
    ...
    from codex_crew.dashboard import server
