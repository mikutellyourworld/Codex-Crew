...
from codex_crew.constants import CHANNEL_SESSION_NAMESPACES
from codex_crew.mcp_tools.messaging import _CHANNEL_SESSIONS, schemas
...
def test_the_channel_session_roster_is_the_gateways_minus_owner_inference_gaps() -> None:
    ...
    that #6514 was.
    """
    from codex_crew.constants import CHANNEL_OWNER_DM_NAMESPACES, CHANNEL_SEND_NAMESPACES
    from codex_crew.dashboard.handlers.messaging import _SEND_MESSAGE_CHANNEL_TYPES
...
def test_no_owner_dm_channel_advertises_learned_identities() -> None:
    ...
    from codex_crew.constants import CHANNEL_OWNER_DM_NAMESPACES

    learned = re.compile(r"_(known|learned|warm|seen|peers|authorized)[a-z_]*")
    offenders = {}
    for channel in CHANNEL_OWNER_DM_NAMESPACES:
        try:
            module = importlib.import_module(f"codex_crew.{channel}.transport")
...
def test_a_channel_that_cannot_dm_proactively_is_in_the_roster_but_yields_no_target() -> None:
    ...
    """
    from codex_crew.dashboard.handlers.messaging import _owner_dm_target
    from codex_crew.feishu.transport import FeishuTransport
...
def test_the_roster_move_did_not_change_the_roster() -> None:
    ...
    """
    from codex_crew.constants import CHANNEL_SESSION_NAMESPACES as canonical
    from codex_crew.messaging.link import CHANNEL_SESSION_NAMESPACES as re_exported
...
def test_a_channel_session_forwards_a_strict_caller_session_when_one_exists() -> None:
    ...
    from codex_crew.mcp_tools import messaging as tool
...
def test_a_channel_session_without_a_strict_key_is_refused() -> None:
    ...
    It costs no legitimate caller: the gateway injects ``CODEXCREW_SESSION_KEY``
