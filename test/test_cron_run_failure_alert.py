...
Regression cover for mikutellyourworld/Codex-Crew#4157: the script and command cron
...
from codex_crew.slack.gateway import _FAILURE_REMINDER_SECS

# The reason from the issue: a startup guard raising before the script body runs.
CONFLICT = (
    "data-home conflict: completion marker present at /home/u/.codex-crew but a "
    "non-empty legacy home /home/u/.codexcrew also exists"
)
...
def _script_job(**overrides):
    defaults = dict(
        ...
        script="~/.codex-crew/crons/monitor.py:run",
        channel="C123",
    )
...
async def _run_script(gw, job, script_result=None, *, vet_reason=None, side_effect=None):
    ...
    with (
        ...
        patch("codex_crew.slack.gateway.run_script_sandboxed", **mock_kw),
...
async def _run_command(gw, job, cmd_result=None, *, vet_reason=None, side_effect=None):
    ...
    with (
        patch("codex_crew.slack.gateway.CronService") as mock_cron_cls,
        patch("codex_crew.slack.gateway.run_command_sandboxed", **mock_kw),
        gate,
        patch("codex_crew.slack.gateway.sel"),
    ):
...
async def _run_message_callback_raising(gw, job, exc):
    ...
    with (
        patch("codex_crew.slack.gateway.stream_and_collect", fake_stream),
        patch("codex_crew.slack.gateway.CronService") as mock_cron_cls,
        patch("codex_crew.slack.gateway.sel"),
