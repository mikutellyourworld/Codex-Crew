...
from codex_crew.acp.client import _OVERSIZE_DRAIN_MAX_BYTES
from codex_crew.acp.runtime import (
    _REQUEST_TIMEOUT,
...
def _fast_no_report_ceiling(monkeypatch):
    ...
    """
    import codex_crew.acp.session_handle as sh
...
async def test_missing_agent_spec_error_reaches_caller_actionable(tmp_path):
    ...
    task = await _start_reader(rt)
    try:
        with patch("codex_crew.acp.runtime.kiro_agents_dir", return_value=tmp_path):
            _feed(
                reader,
                {
                    "id": 7,
    ...
    assert "'codexcrew.json'" in text  # the file that is missing
    assert str(tmp_path) in text  # where it was looked for
    assert "codexcrew setup --agent-only --clean" in text  # the repair
...
def test_runtime_reuses_clients_oversize_drain_helper():
    ...
    import codex_crew.acp.runtime as runtime_mod
...
def test_runtime_uses_clients_augmented_kiro_bin_resolver():
    ...
    AcpClient (honours CODEXCREW_KIRO_BIN + augmented_path so a non-login gateway
...
async def test_runtime_missing_kiro_bin_reports_the_directories_it_searched(backend):
    ...
    ``%ProgramFiles%\\Kiro-Cli`` and the ``CODEXCREW_KIRO_BIN`` override, none of
    which is PATH.

    That gap is what produced #6497. A Windows reporter read "not found in PATH",
    ran ``where kiro-cli``, got nothing, saw the gateway's OWN ``codexcrew.exe``
    in the app bundle, and concluded the agent CLI had been renamed and this
    lookup left stale. It had not been: ``codexcrew`` is Codex Crew's own console
    ...
    assert "CODEXCREW_KIRO_BIN" in message
...
# that at the same severity and shape as a crash made `codexcrew logs` misreport
...
def _neuter_kill_side_effects(monkeypatch, proc):
    ...
    import codex_crew.acp.runtime as rt_mod
...
async def test_deliberate_kill_logs_info_and_still_fails_pending_futures(caplog, monkeypatch):
    ...
    with caplog.at_level(logging.INFO, logger="codex_crew.acp.runtime"):
...
async def test_is_stale_none_when_fresh_and_small(monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.acp.runtime._get_rss_tree_mb", _boom)
...
async def test_is_stale_none_when_old_but_small_rss(monkeypatch):
    ...
    rt._max_rss_mb = 500.0
    monkeypatch.setattr("codex_crew.acp.runtime._get_rss_tree_mb", lambda pid: 10.0)
    assert await rt._is_stale() is None
...
async def test_is_stale_rss_when_tree_over_threshold(monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.acp.runtime._get_rss_tree_mb", lambda pid: 250.0)
...
def test_get_rss_mb_real_process():
    ...
    from codex_crew.acp.runtime import _get_rss_mb
...
def test_get_rss_tree_mb_real_process():
    ...
    """
    from codex_crew.acp.runtime import _get_rss_tree_mb
...
async def test_prompt_resets_turn_done_when_cancelled_while_building_blocks():
    ...
    with patch(
        "codex_crew.acp.session_handle.build_prompt_blocks",
...
async def test_handle_set_mode():
    ...
    q = _register(rt, "sA")
    handle = AcpSessionHandle("sA", q["sA"], rt)
    await handle.set_mode("codexcrew-lite")
    sent = json.loads(proc.stdin.write.call_args.args[0].decode())
    assert sent["method"] == "session/set_mode"
    assert sent["params"]["modeId"] == "codexcrew-lite"
...
async def test_dispatch_tool_call_and_result():
    ...
    from codex_crew.acp.types import EVENT_TOOL_CALL, EVENT_TOOL_RESULT
...
async def test_tool_stall_cancels_session_not_runtime(monkeypatch):
    ...
    from codex_crew.acp.session_handle import WatchdogSettings
    from codex_crew.acp.types import EVENT_COMPLETE
...
async def test_dispatch_thinking_chunk():
    ...
    from codex_crew.acp.types import EVENT_THINKING_CHUNK
...
async def test_dispatch_compaction_completed_resets_context_stats():
    ...
    from codex_crew.acp.types import METHOD_COMPACTION_STATUS, AcpPromptStats
...
async def test_wait_for_compaction_cached_result_applies_post_compaction_metadata():
    ...
    from codex_crew.acp.types import AcpPromptStats, JsonRpcMessage
...
async def test_dispatch_agent_switched():
    ...
    from codex_crew.acp.types import EVENT_AGENT_SWITCHED, METHOD_AGENT_SWITCHED
    ...
    try:
        ...
        assert len(sw) == 1 and sw[0].text == "codexcrew-lite"
    finally:
        await _stop_reader(task)
...
async def test_dispatch_mcp_oauth_request():
    ...
    from codex_crew.acp.types import EVENT_MCP_OAUTH_REQUEST, METHOD_MCP_OAUTH_REQUEST
...
async def test_dispatch_mcp_server_initialized():
    ...
    from codex_crew.acp.types import EVENT_MCP_SERVER_INITIALIZED, METHOD_MCP_SERVER_INITIALIZED
...
async def test_dispatch_mcp_server_init_failure():
    ...
    from codex_crew.acp.types import EVENT_MCP_SERVER_INIT_FAILURE, METHOD_MCP_SERVER_INIT_FAILURE
...
async def test_dispatch_tool_call_update_raw_output():
    ...
    from codex_crew.acp.types import EVENT_TOOL_RESULT
...
async def test_dispatch_tool_call_update_refinement():
    ...
    from codex_crew.acp.types import EVENT_TOOL_CALL_UPDATE
...
async def test_dispatch_metadata_credits():
    ...
    from codex_crew.acp.types import EVENT_COMPLETE, METHOD_METADATA
...
async def test_dispatch_metadata_credits_robust():
    ...
    from codex_crew.acp.types import METHOD_METADATA
...
async def test_dispatch_subagent_list():
    ...
    from codex_crew.acp.types import EVENT_SUBAGENT_LIST, METHOD_SUBAGENT_LIST_UPDATE
...
async def test_dispatch_subagent_activity_tool():
    ...
    """
    from codex_crew.acp.types import EVENT_SUBAGENT_ACTIVITY, METHOD_KIRO_SESSION_UPDATE
...
async def test_interleaved_tool_calls_routed_per_session():
    ...
    """
    from codex_crew.acp.types import EVENT_TOOL_CALL
...
class TestAcpRuntimePidTracking:
    ...
    async def test_kill_untracks_pid(self, monkeypatch):
        ...
        # runtime.py imports these at module top (from codex_crew.session_pid
...
class TestAcpRuntimeLoadSession:
    ...
    async def test_load_session_sends_direct_session_load_params(self, monkeypatch):
        ...
        async def _fake_send(method, params, timeout=None):
            ...
            if method == METHOD_SESSION_LOAD:
                return {"modes": {"currentModeId": "codexcrew"}, "models": []}
            return {}
    ...
    async def test_load_session_without_modes_raises_and_unregisters(self, monkeypatch):
        ...
        with pytest.raises(AcpRuntimeError):
            await rt.load_session("/f.json", "sid-y", agent="codexcrew")
    ...
    async def test_load_session_reinjects_the_kas_agent_definition(self, monkeypatch):
        ...
        """
        from codex_crew.acp._dispatch import build_session_new_params
        ...
        await rt.load_session("", "sid-kas", cwd="/work", agent="codexcrew")

        load_params = sent[0][1]
        assert load_params["_meta"]["kiro"]["customAgents"] == [
            {"id": "codexcrew", "prompt": "p", "tools": []}
        ]
        ...
        assert (
            ...
            == build_session_new_params(
                "/work", kas_custom_agents=[{"id": "codexcrew", "prompt": "p", "tools": []}]
            )["_meta"]["kiro"]
        )
    ...
    async def test_load_session_keeps_the_transcript_path_alongside_the_agents(self, monkeypatch):
        ...
        await rt.load_session("/t.json", "sid-both", cwd="/work", agent="codexcrew")
    ...
    async def test_the_kiro_resume_path_never_reaches_the_adapter(self, monkeypatch):
        ...
        await rt.load_session("/t.json", "sid-kiro", cwd="/work", agent="codexcrew")
    ...
    async def test_load_session_params_match_acp_client(self, monkeypatch):
        ...
        monkeypatch.setattr(rt, "_send_and_await", _fake_send)
        await rt.load_session("/k/sid.json", "sid", cwd="/w", agent="codexcrew")
    ...
    async def test_load_session_unregisters_queue_when_set_mode_fails(self, monkeypatch):
        ...
        with pytest.raises(AcpRuntimeError):
            await rt.load_session("/k/sid.json", "sid-z", cwd="/w", agent="codexcrew")
    ...
    async def test_load_session_redeclares_pooled_stubs(self, tmp_path, monkeypatch):
        ...
        from codex_crew.mcp_gateway.rewriter import _WRAPPER_MARKER

        overlay = tmp_path / "agents"
        overlay.mkdir()
        (overlay / "codexcrew.json").write_text(
            json.dumps(
                {
                    "name": "codexcrew",
                    "mcpServers": {
        ...
        await rt.load_session("/k/sid.json", "sid-r", cwd="/w", agent="codexcrew")
        ...
        await rt.create_session(cwd="/w", agent="codexcrew")
    ...
    async def test_load_session_resolves_stubs_off_the_event_loop(self, monkeypatch):
        ...
        monkeypatch.setattr(rt, "_send_and_await", _fake_send)
        await rt.load_session("/k/sid.json", "sid-t", cwd="/w", agent="codexcrew")
...
async def test_create_session_registers_queue_on_success(monkeypatch):
    ...
    handle = await rt.create_session(cwd="/w", agent="codexcrew")
...
async def test_drain_init_records_the_session_mcp_report():
    ...
    q = _register(rt, "sA")
    handle = AcpSessionHandle("sA", q["sA"], rt)
    q["sA"].put_nowait(_mcp_initialized_frame("sA", "codexcrew-core"))
    ...
    assert payload["ready"] == ["codexcrew-core"]
...
async def test_create_session_records_the_wire_roster(monkeypatch):
    ...
    monkeypatch.setattr(rt, "_send_and_await", _fake_send)
    servers = [{"name": "codexcrew-core", "command": "x"}, {"name": "codexcrew-cron"}]

    handle = await rt.create_session(cwd="/w", agent="codexcrew", mcp_servers=servers)

    assert handle.mcp_session_report().configured == ("codexcrew-core", "codexcrew-cron")
...
async def test_mcp_free_runtime_skips_no_report_ceiling(monkeypatch):
    ...
    with patch.object(AcpSessionHandle, "drain_init", _spy):
        await rt.create_session(cwd="/w", agent="codexcrew-lite", mcp_servers=[])
    ...
    try:
        ...
        with patch.object(rt, "_send_and_await", _fake_send):
            with patch.object(AcpSessionHandle, "drain_init", AsyncMock()) as mock_drain:
                handle = await rt.create_session(cwd="/w", agent="codexcrew", mcp_servers=[])
...
async def test_stream_translates_auth_failure_to_auth_required():
    ...
    from codex_crew.acp.client import AcpAuthRequired
    from codex_crew.acp.session_provider import AcpSessionProvider
...
def test_store_session_config_syncs_effort_levels(monkeypatch):
    ...
    calls = []
    fake = types.ModuleType("codex_crew.dashboard.chat_persistence")
    fake.update_reasoning_effort_values = lambda levels: calls.append(levels)
    monkeypatch.setitem(sys.modules, "codex_crew.dashboard.chat_persistence", fake)
    rt = MagicMock()
    rt.is_alive.return_value = True
    h = AcpSessionHandle("sA", asyncio.Queue(), rt)
...
async def test_stale_turn_probes_then_signals_recovery():
    ...
    from codex_crew.acp.types import EVENT_COMPLETE, STOP_REASON_STALE_RECOVER
...
def test_build_permission_event_sets_raw_tool_params():
    ...
    hides the path."""
    from codex_crew.acp._dispatch import build_permission_event
    from codex_crew.acp.types import METHOD_REQUEST_PERMISSION
...
def test_mark_dead_unregisters_protected_pid():
    ...
    from codex_crew.session_pid import _protected_pids, register_protected_pid
...
def test_protected_runtime_pid_lands_in_sweep_active_set():
    ...
    the CodexCrew analog of the upstream project's end-to-end guard.
    """
...
async def test_unknown_session_drops_aggregate_into_one_counted_record(caplog):
    ...
    task = await _start_reader(rt)
    try:
        with caplog.at_level(logging.DEBUG, logger="codex_crew.acp.runtime"):
            for _ in range(5):
                _feed(reader, {"method": "session/update", "params": {"sessionId": "ghost"}})
            await _drain(reader)
...
class TestToolPurposeExtraction:
    ...
    def test_snake_case_key(self):
        from codex_crew.acp._dispatch import _build_tool_call_event
    ...
    def test_blank_and_non_string_values_ignored(self):
        from codex_crew.acp._dispatch import extract_tool_purpose
...
async def test_create_session_sets_mode_when_agent_is_advertised():
    ...
    resp = _new_resp(
        {"currentModeId": "codexcrew", "availableModes": [{"id": "codexcrew"}, {"id": "ops"}]}
    )
...
async def test_create_session_fails_closed_when_agent_not_advertised():
    ...
    with patch.object(AcpSessionHandle, "drain_init", AsyncMock()):
        with pytest.raises(AcpRuntimeError, match="not available"):
            await rt.create_session(agent="codexcrew", mcp_servers=[])
    methods = [c.args[0] for c in rt._send_and_await.call_args_list]
...
async def test_create_session_fails_closed_when_available_modes_empty():
    ...
    rt, _, _ = _make_runtime()
    rt._finish_session_init = MagicMock(return_value=[])  # type: ignore[method-assign]
    resp = _new_resp({"currentModeId": "codexcrew", "availableModes": []})
    rt._send_and_await = AsyncMock(side_effect=[resp, {}])  # type: ignore[method-assign]
    with patch.object(AcpSessionHandle, "drain_init", AsyncMock()):
        with pytest.raises(AcpRuntimeError, match="not available"):
            await rt.create_session(agent="codexcrew", mcp_servers=[])
...
def test_mode_available_helper():
    ...
    membership test."""
    from codex_crew.acp.runtime import AcpRuntime

    assert AcpRuntime._mode_available("codexcrew", _new_resp(None)) is True
    assert (
        AcpRuntime._mode_available("codexcrew", _new_resp({"availableModes": [{"id": "codexcrew"}]}))
        is True
    )
    assert (
        AcpRuntime._mode_available("codexcrew", _new_resp({"availableModes": [{"id": "default"}]}))
        is False
    )
    # Present-but-empty availableModes → advertised, agent absent → fail closed.
    assert AcpRuntime._mode_available("codexcrew", _new_resp({"availableModes": []})) is False
    # A modes dict WITHOUT an availableModes list → not advertised → attempt.
    assert AcpRuntime._mode_available("codexcrew", _new_resp({"currentModeId": "x"})) is True
...
def test_parse_session_modes_shapes():
    ...
    from codex_crew.acp._dispatch import parse_session_modes
    ...
    assert ids == ["codexcrew", "ops", "code-reviewer"]
    assert current == "codexcrew"
...
async def test_session_load_call_site_passes_budget_above_request_timeout(monkeypatch):
    ...
    async def _fake_send(method, params, timeout=None):
        if method == METHOD_SESSION_LOAD:
            seen["timeout"] = timeout
            return {"modes": {"currentModeId": "codexcrew"}}
        return {}
...
async def test_session_start_budget_follows_config(monkeypatch):
    ...
    monkeypatch.setattr(CodexCrewConfig, "load", classmethod(lambda cls: fake_cfg))
...
def test_runtime_construction_never_touches_config(monkeypatch):
    """AcpRuntime.__init__ runs on the event loop; CodexCrewConfig.load() is a
    ...
    monkeypatch.setattr(CodexCrewConfig, "load", classmethod(_boom))
    rt = AcpRuntime(work_dir="/tmp")
    assert rt._session_start_timeout is None
...
def test_resolve_session_start_timeout_floors_and_falls_back(monkeypatch):
    ...
    from codex_crew.acp.runtime import _resolve_session_start_timeout
    from codex_crew.config.loader import CodexCrewConfig
    ...
    monkeypatch.setattr(CodexCrewConfig, "load", classmethod(lambda cls: low_cfg))
...
async def test_unroutable_permission_request_is_auto_rejected(caplog):
    ...
    task = await _start_reader(rt)
    try:
        with caplog.at_level(logging.WARNING, logger="codex_crew.acp.runtime"):
            _feed(
                reader,
                {
                    "jsonrpc": "2.0",
...
def test_remote_mcp_empty_rawinput_keeps_identity_through_dispatch():
    ...
    from codex_crew.acp._dispatch import build_permission_event, parse_session_update
...
async def test_notice_yield_abandonment_clears_turn_state():
    ...
    from codex_crew.acp.types import EVENT_SUBAGENT_ACTIVITY
...
async def test_prompt_warns_when_the_stream_ends_without_a_terminal_event(caplog):
    ...
    from codex_crew.acp.types import EVENT_TEXT_CHUNK, AcpEvent
...
def test_missing_kind_is_not_a_resolved_shell_classification():
    ...
    from codex_crew.acp._dispatch import _build_tool_call_event, build_permission_event
    from codex_crew.acp.types import METHOD_REQUEST_PERMISSION
