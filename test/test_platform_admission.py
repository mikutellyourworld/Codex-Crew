"""Tests for plugin admission control (codex_crew.platform.admission)."""
...
from codex_crew.platform import discovery as discovery_mod
...
from codex_crew.platform.discovery import PluginAdmissionError, discover_companion_context
...
def patch_manifest(monkeypatch):
    ...
    def _set(manifest):
        monkeypatch.setattr(
            "codex_crew.platform.admission._read_plugin_manifest",
            lambda ep: manifest,
        )
...
class TestPolicyLoading:
    def test_no_policy_fails_closed(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "codex_crew.platform.admission._policy_default_path", lambda: _nope
        )
        from codex_crew.platform.admission import load_admission_policy
    ...
    def test_seed_then_load_is_open(self, monkeypatch, tmp_path):
        ...
        import codex_crew.platform.admission as adm
    ...
    def test_integrity_mismatch_is_advisory_not_deny(self, monkeypatch, tmp_path, caplog):
        ...
        import codex_crew.platform.admission as adm
        from codex_crew.platform import governance_health as gh
    ...
    def test_policy_round_trip(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("CODEXCREW_ADMISSION_POLICY", str(p))
        from codex_crew.platform.admission import load_admission_policy
...
class TestPolicySignatureTrustRoot:
    ...
    def test_fail_closed_default_leaves_policy_signature_advisory(self, monkeypatch, tmp_path):
        ...
        monkeypatch.delenv("CODEXCREW_ADMISSION_POLICY", raising=False)
        monkeypatch.setattr(
            "codex_crew.platform.admission._policy_default_path", lambda: tmp_path / "nope.json"
        )
        from codex_crew.platform.admission import load_admission_policy
...
class TestSharedSigningPrimitives:
    ...
    def test_hmac_signature_matches_stdlib(self):
        from codex_crew.platform.admission import hmac_signature
...
class TestReadPolicyTrustRoot:
    ...
    def test_reads_flag_and_keys_from_env_path(self, monkeypatch, tmp_path):
        from codex_crew.platform.admission import read_policy_trust_root
    ...
    def test_missing_trust_root_stays_permissive(self, monkeypatch, tmp_path):
        ...
        from codex_crew.platform import admission as adm_mod
        from codex_crew.platform.admission import read_policy_trust_root

        monkeypatch.setenv("CODEXCREW_ADMISSION_POLICY", str(tmp_path / "gone.json"))
    ...
    def test_absent_policy_returns_permissive_not_fail_closed(self, monkeypatch, tmp_path):
        ...
        import codex_crew.platform.admission as adm_mod
        from codex_crew.platform.admission import read_policy_trust_root
