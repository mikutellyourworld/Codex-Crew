"""``codex_crew.__main__`` — the ordering contract of the ``python -m`` entry point.

The module is tiny but load-bearing, and its own docstring states the invariant
that makes it so: ``_ensure_ssl_certs()`` must run BEFORE ``codex_crew.cli`` is
...
# (``codex_crew.cli`` applies the same SSL fix on import), so importing it here
# costs nothing and starts no CLI.
import codex_crew.__main__  # noqa: F401

# Imported eagerly for a different reason: ``codex_crew.cli`` calls
...
# file exists to pin.
from codex_crew import cli as cli_mod
...
def order(monkeypatch: pytest.MonkeyPatch) -> list[str]:
    ...
    calls: list[str] = []
    monkeypatch.setattr(
        "codex_crew.platform_compat.ensure_utf8_console",
        lambda: calls.append("utf8"),
    )
    monkeypatch.setattr(
        "codex_crew._ssl_compat._ensure_ssl_certs",
...
def test_importing_the_module_applies_both_fixes_in_order(
    ...
    monkeypatch.delitem(sys.modules, "codex_crew.__main__", raising=False)
    module = importlib.import_module("codex_crew.__main__")
...
def test_executed_as_a_plain_module_does_not_start_the_cli(order: list[str]) -> None:
    ...
    namespace = _exec_entrypoint("codex_crew.__main__")
    assert order == ["utf8", "ssl"]
