...
Uses injected temp directories to avoid touching the real ~/.codexcrew/logs/.
...
from codex_crew.dashboard import crash_dump_store
...
def _create_header_only_dump(
    ...
    p = dumps_dir / name
    p.write_text(
        "# CodexCrew loop-stall crash dump — opened 20260717T010000Z\n"
        + pid_line
...
def _create_stacked_dump(dumps_dir: Path, name: str) -> Path:
    ...
    p = dumps_dir / name
    p.write_text(
        "# CodexCrew loop-stall crash dump — opened 20260717T020000Z\n"
        ...
        "    self._real_close()\n"
        '  File "/home/user/.codexcrew/src/codex_crew/acp/client.py", line 312, in _teardown\n'
        "    self._sock.close()\n"
        '  File "/home/user/.codexcrew/src/codex_crew/dashboard/server.py", line 800, in _cleanup\n'
...
def test_open_dump_file_creates_file(dumps_dir: Path) -> None:
    with opened_dump_file(dumps_dir) as f:
        ...
        assert "CodexCrew loop-stall crash dump" in content
...
def _create_multi_thread_dump(dumps_dir: Path, name: str, *, idle_workers: int = 3) -> Path:
    ...
    main_block = (
        ...
        '  File "/home/user/.codexcrew/src/codex_crew/dashboard/server.py", line 246 in _should_prevent_sleep\n'
        '  File "/usr/lib/python3.12/asyncio/base_events.py", line 645 in run_forever\n'
        '  File "/home/user/.codexcrew/src/codex_crew/cli.py", line 2046 in main\n'
    )
    p = dumps_dir / name
    p.write_text(
...
def test_dump_first_stack_lines_surfaces_wedged_thread(dumps_dir: Path) -> None:
    ...
    assert "_should_prevent_sleep" in body  # the wedged thread's Codex Crew frame
...
def test_dump_first_stack_lines_prefers_current_thread_marker(dumps_dir: Path) -> None:
    ...
    p.write_text(
        ...
        '  File "/home/user/.codexcrew/src/codex_crew/dashboard/state.py", line 100 in _flush\n'
...
def test_watchdog_soft_only_dump_is_written_to_discoverable_file(
    ...
    from codex_crew.dashboard import loop_watchdog
...
def test_dump_replay_lines_basic(dumps_dir: Path) -> None:
    ...
    from codex_crew.dashboard.crash_dump_store import dump_replay_lines
...
def test_sweep_keeps_own_pid_file(dumps_dir: Path) -> None:
    ...
    p.write_text(
        "# CodexCrew loop-stall crash dump — opened 20260717T030000Z\n"  # brand-ok: mirrors production dump header
...
def test_sweep_keeps_header_only_dump_without_pid_line(dumps_dir: Path) -> None:
    ...
    p.write_text("# CodexCrew loop-stall crash dump — opened 20260717T040000Z\n\n")  # brand-ok: mirrors production dump header
    removed = sweep_stale_dumps(dumps_dir, is_pid_alive=_dead_pid)
    assert removed == 0
    assert p.exists()
...
def test_sweep_treats_oversized_pid_as_unparseable(dumps_dir: Path) -> None:
    ...
    for name, digits in (("20260717T050000Z", "9" * 5000), ("20260717T060000Z", str(2**31))):
        ...
        p.write_text(
            "# CodexCrew loop-stall crash dump — opened 20260717T050000Z\n"  # brand-ok: mirrors production dump header
            f"# PID: {digits}\n"
            "\n"
        )
    removed = sweep_stale_dumps(dumps_dir, is_pid_alive=_dead_pid)
...
def test_sweep_reads_only_a_bounded_prefix(dumps_dir: Path) -> None:
    ...
    with p.open("w") as f:
        f.write("# CodexCrew loop-stall crash dump — opened 20260717T080000Z\n")  # brand-ok: mirrors production dump header
        f.write("# PID: 1\n\n")
...
def test_owner_alive_detects_pid_reuse_via_start_id(dumps_dir: Path) -> None:
    ...
    p.write_text(
        "# CodexCrew loop-stall crash dump — opened 20260717T110000Z\n"  # brand-ok: mirrors production dump header
