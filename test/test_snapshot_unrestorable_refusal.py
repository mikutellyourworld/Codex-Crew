...
class TestANewArchiveIsBoundedBeforeSuccessAndPrune:
    ...
    def test_nothing_is_pruned_when_the_new_archive_is_refused(
        ...
        for name in (
            "codexcrew-snapshot-20250101T000000Z.tar.gz",
            "codexcrew-snapshot-20250102T000000Z.tar.gz",
        ):
            (out / name).write_bytes(b"older bundle")
        before = sorted(p.name for p in out.glob("codexcrew-snapshot-*.tar.gz"))
        ...
        after = sorted(p.name for p in out.glob("codexcrew-snapshot-*.tar.gz"))
    ...
    def test_a_normal_snapshot_still_succeeds(self, home, tmp_path, capsys):
        ...
        # A memory-only (selective) bundle is named `codexcrew-partial-*`; a complete one
        # keeps `codexcrew-snapshot-*`. Match either — the property is that a bundle was
        # written, not which prefix it carries.
        assert list(out.glob("codexcrew-*.tar.gz"))
