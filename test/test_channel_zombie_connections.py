...
from codex_crew.discord.client import _WS_HEARTBEAT_SECS, DiscordClient
from codex_crew.messaging.transport import InboundMessage
from codex_crew.weixin.client import ContextTokenStore
...
async def test_heartbeat_loop_death_closes_ws_and_warns(caplog):
    ...
    ws = _FailingWs()
    with caplog.at_level(logging.WARNING, logger="codex_crew.discord.client"):
...
async def test_a_sustained_timeout_streak_warns_exactly_once(tmp_path, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="codex_crew.weixin.transport"):
