"""`codexcrew doctor` Sandbox section — honest verdicts from an unconfined shell.
...
codexcrew-userns AppArmor profile is ATTACHED to the resolved codexcrew launcher
...
confine the gateway's sandbox probe). A `codexcrew doctor` invocation that did
...
from codex_crew import cli_doctor, sandbox
...
def _resolve_launcher(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    """Make ``codexcrew_bin()`` resolve to a real file under *tmp_path* and
    ...
    installed codexcrew service would leak its own unit contents into these
    ...
    launcher = tmp_path / "codexcrew"
    launcher.write_text("#!/bin/sh\n")
    monkeypatch.setattr(service_linux, "codexcrew_bin", lambda: str(launcher))
    monkeypatch.setattr(service_linux, "UNIT_PATH", tmp_path / "codexcrew.service")
...
class TestProfileAbsent:
    ...
    def test_reports_broken_and_names_the_install_command(
        ...
        assert "codexcrew service install" in out
        assert "cannot be verified from this shell" not in out
        assert issues
...
class TestServiceProfileApplies:
    ...
    def test_leftover_unit_directive_defeats_a_matching_attachment(
        ...
        (tmp_path / "codexcrew.service").write_text(
            "[Service]\nAppArmorProfile=codexcrew-userns\n", encoding="utf-8"
        )
    ...
    def test_unit_without_directive_leaves_the_attachment_verdict(
        ...
        (tmp_path / "codexcrew.service").write_text(
            "[Service]\nExecStart=/opt/codexcrew-venv/bin/codexcrew gateway\n",
            encoding="utf-8",
        )
        assert cli_doctor._service_profile_applies(profile, apparmor.PROFILE_NAME)
    ...
    def test_undecodable_unit_bytes_do_not_crash_the_verdict(
        ...
        (tmp_path / "codexcrew.service").write_bytes(
            b"[Service]\n\xff\xfe garbage \xff\nExecStart=/x\n"
        )
        assert cli_doctor._service_profile_applies(profile, apparmor.PROFILE_NAME)
        (tmp_path / "codexcrew.service").write_bytes(
    ...
    def test_an_unresolvable_codexcrew_bin_does_not_apply(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        """``codexcrew_bin()`` can point at a path that no longer exists (an
        uninstalled or moved venv); that must read as "not applied", not raise."""
        monkeypatch.setattr(service_linux, "codexcrew_bin", lambda: "/nonexistent/codexcrew")
        profile = tmp_path / "profile"
        profile.write_text(
            f'profile {apparmor.PROFILE_NAME} "/nonexistent/codexcrew" flags=(unconfined) {{}}\n',
            encoding="utf-8",
        )
        assert not cli_doctor._service_profile_applies(profile, apparmor.PROFILE_NAME)
