...
Codex Crew reaches KAS through kiro-cli's own ACP relay
...
:mod:`codex_crew.acp.kas_transport` for why, and for the frame-parity measurement
...
class TestAuthOwnership:
    ...
    def test_the_kas_auth_module_is_gone(self):
        ...
        with pytest.raises(ModuleNotFoundError):
            __import__("codex_crew.acp.kas_auth")
...
class TestSandboxClassification:
    ...
    async def test_kas_is_not_classified_as_kiro_cli(self, kas_stub, tmp_path):
        ...
        with patch("codex_crew.acp.runtime.wrap_argv", side_effect=fake_wrap):
    ...
    async def test_kiro_still_classified_as_kiro_cli(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.acp.runtime._resolve_kiro_bin_for_spawn", fake_bin)
        monkeypatch.setattr("codex_crew.acp.runtime.ensure_agent_materialized", lambda _agent: None)
...
class TestKasInvocation:
    ...
    async def test_missing_kiro_cli_fails_with_an_actionable_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.acp.runtime._resolve_kiro_bin_for_spawn", no_bin)
        runtime = AcpRuntime(
