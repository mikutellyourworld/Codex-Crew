"""Coverage for the Codex Crew gateway's deterministic cron-execution paths.
...
socket and no write outside the per-test ``CODEXCREW_HOME`` (pinned by
...
from codex_crew.cron import CronJob, CronStoreBusy
    ...
    *,
    ...
    with ExitStack() as stack:
        for patcher in (
            ...
            patch("codex_crew.apps.bridges.reconcile_app_crons_for_execution", AsyncMock()),
        ):
    ...
    async def test_persist_turn_row_swallows_failure(self):
        ...
        with patch.object(gw, "read_context_tokens", side_effect=RuntimeError("no provider")):
            await gw._persist_turn_row(
                ...
                provider="anthropic",
                surface="heartbeat",
                agent_fallback=lambda: "codexcrew",
                t0=0.0,
            )
...
class TestTheVetIsAnAccountedBudgetTerm:
    ...
    def _budgets(job: Any) -> tuple[float, float]:
        ...
        from codex_crew.cron import effective_wake_budget
        from codex_crew.executors import cron_gate_budget
    ...
    def test_every_term_spent_inside_the_run_budget_is_accounted_for(self):
        ...
        """
        from codex_crew import cron as cron_mod
...
class TestChannelReplyLink:
    ...
    def test_no_session_manager_returns_none(self):
        orch = _make_orchestrator()
        orch.sessions = None
        assert orch._channel_reply_link("discord:codexcrew:direct:U9") is None
    ...
    def test_link_getter_failure_falls_through_to_stored_channel(self):
        ...
        resolved = orch._channel_reply_link("discord:codexcrew:direct:U9")
    ...
    def test_unified_bucket_validates_stored_namespace(self, stored):
        ...
        orch = _make_orchestrator()
        orch.sessions = _channel_sessions(stored=stored)
        assert orch._channel_reply_link("unified:codexcrew:direct:U9") is None
    ...
    def test_group_session_never_takes_the_stored_rung(self):
        ...
        assert orch._channel_reply_link("discord:codexcrew:group:C1") is None
    ...
    def test_origin_link_wins_and_needs_no_dm_resolution(self):
        ...
        assert orch._channel_reply_link("discord:codexcrew:direct:U9") == (origin, False)
...
class TestDeliverChannelReply:
    ...
    async def test_blank_text_is_not_delivered(self):
        orch = _make_orchestrator()
        orch.dashboard_state = _mock_dashboard_state()
        assert await orch._deliver_channel_reply("discord:codexcrew:direct:U9", "   ") is False

    @pytest.mark.asyncio
    async def test_no_dashboard_state_is_not_delivered(self):
        orch = _make_orchestrator()
        orch.dashboard_state = None
        assert await orch._deliver_channel_reply("discord:codexcrew:direct:U9", "hi") is False
    ...
    async def test_target_resolution_failure_degrades_to_false(self):
        ...
        with patch.object(gw, "_resolve_channel_target", _boom):
            delivered = await orch._deliver_channel_reply(
                "discord:codexcrew:direct:U9", "hi", resolved_link=(link, False)
            )
    ...
    async def test_a_long_fenced_result_keeps_every_code_line_inside_its_fence(self):
        ...
        """
        from codex_crew.messaging.split import FENCE_OUTSIDE, iter_fence_lines
        ...
        with patch.object(
            ...
            delivered = await orch._deliver_channel_reply(
                "discord:codexcrew:direct:U9", self._FENCED, resolved_link=(link, False)
            )
...
_TG_KEY = "telegram:codexcrew:direct:U9"
