...
and ``codex_crew.messaging`` still imports nothing from the surfaces built on it.
"""
...
import codex_crew.messaging.commands as commands
...
from codex_crew.messaging.queue_receipt import ReceiptQueue, receipt_text
...
class TestRunYoloCommand:
    ...
    def test_a_failed_activation_is_audited_as_denied(
        ...
        from codex_crew.safety_override import ActivationResult
...
class TestLayering:
    ...
    _ALLOWED_SURFACE_EDGES = {
        ("dispatch.py", "codex_crew.dashboard.session_directive_apply"),
    }
        ...
        for path in sorted(pkg.glob("*.py")):
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ImportFrom):
                    ...
                    continue
                if "codex_crew.dashboard" not in module and "codex_crew.slack" not in module:
                    continue
...
class TestCron:
    ...
    async def test_every_channel_forwards_delete_attribution_not_only_slack(self) -> None:
        ...
        with patch("codex_crew.messaging.commands.sel") as mock_sel:
...
class TestTaskSpecPathIsGated:
    ...
    async def test_a_sensitive_path_is_refused_before_the_runner_sees_it(
        ...
        monkeypatch.setattr(
            "codex_crew.hooks.is_sensitive_path",
...
class TestSlackKeywordCarriesTheSessionKey:
    ...
    async def test_the_slack_handler_forwards_the_session_key(self) -> None:
        from codex_crew.slack import handler as sh
        ...
        with pytest.MonkeyPatch.context() as mp:
            ...
            out = await sh._handle_run_command(
                ...
                "C1",
                "1.2",
                session_key="slack:codexcrew:direct:U1",
            )
        assert out == "started"
        assert seen and seen[0]["session_key"] == "slack:codexcrew:direct:U1"
...
class TestListsHostState:
    ...
    def test_the_spawn_listing_really_does_ignore_the_session(self) -> None:
        ...
        out = commands.spawn_task_reply("list", manager, "telegram:codexcrew:direct:7")
