...
_ROOT = 500
_ROOT_CMDLINE = b"python3\x00codexcrew_sandbox_x.py"
...
class TestOrphanMcpSubtreeReap:
    ...
    def _run(
        ...
        *,
        ...
        with (
            ...
            patch("codex_crew.session_pid._build_child_map", return_value={}),
            patch("codex_crew.session_pid._orphan_descendants", return_value=descendants),
            patch(
                "codex_crew.session_pid._pid_cmdline",
                side_effect=lambda pid: argv_map.get(pid, b"npm\x00exec\x00pkg@latest"),
            ),
            patch(
                "codex_crew.session_pid._pid_start_token",
            ...
            patch(
                "codex_crew.session_pid._env_has_codexcrew_marker",
    ...
    def test_descendant_without_env_marker_is_spared(self) -> None:
        """Positive identity per member: no CODEXCREW_SPAWNED, no kill."""
        killed = self._run(
            _pairs(_UNMARKED_INTERMEDIATE, _MARKED_LEAF),
            marked={_ROOT, _MARKED_LEAF},  # intermediate is NOT Codex-Crew-spawned
        )
    ...
    def test_peer_gateway_descendant_is_spared(self) -> None:
        ...
        killed = self._run(
            ...
            argv={gateway_pid: b"python3\x00-m\x00codex_crew.cli\x00gateway"},
        )
    ...
    def test_subtree_counts_against_the_global_kill_cap(self) -> None:
        ...
        """
        from codex_crew.session_pid import _ORPHAN_SWEEP_MAX_KILLS
    ...
    def test_subtree_enumerated_before_root_dies(self) -> None:
        ...
        with (
            ...
            patch("codex_crew.session_pid._build_child_map", return_value={}),
            patch(
                "codex_crew.session_pid._orphan_descendants",
            ...
            patch("codex_crew.session_pid._pid_start_token", return_value="tok-root"),
    ...
    def test_child_map_is_built_once_per_sweep(self) -> None:
        ...
        with (
            ...
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", lambda self: _ROOT_CMDLINE),
            patch("codex_crew.session_pid._build_child_map", return_value={}) as mock_map,
            patch("codex_crew.session_pid._orphan_descendants", return_value=[]),
            patch("codex_crew.session_pid.platform_compat.kill_pid"),
        ):
            mock_sys.platform = "linux"
    ...
    def test_child_map_not_built_when_no_mcp_orphan_is_confirmed(self) -> None:
        ...
        with (
            ...
            patch("codex_crew.session_pid._build_child_map") as mock_map,
        ):
            mock_sys.platform = "linux"
            kill_orphan_mcps([_ROOT])
...
class TestPidRecycleGuard:
    ...
    the kernel hands that PID to a NEW Codex-Crew-spawned worker, and the stale
    ...
    def _kill(self, walk_token: str | None, live_token: str | None) -> list[int]:
        from codex_crew.session_pid import _kill_orphan_mcp_descendants

        killed: list[int] = []
        with (
            patch("os.getpid", return_value=1),
            patch("os.getpgrp", return_value=1000),
            patch("codex_crew.session_pid._pid_cmdline", return_value=b"npm\x00exec\x00p@latest"),
            patch("codex_crew.session_pid._env_has_codexcrew_marker", return_value=True),
            patch("codex_crew.session_pid._pid_start_token", return_value=live_token),
            patch(
                "codex_crew.session_pid.platform_compat.kill_pid",
...
class TestOrphanMcpSubtreeHelper:
    ...
    def _kill_one(self, **overrides: object) -> int:
        ...
        with (
            ...
            patch("os.getpgrp", return_value=1000),
            patch("codex_crew.session_pid._pid_cmdline", return_value=b"npm\x00exec\x00p@latest"),
            patch("codex_crew.session_pid._env_has_codexcrew_marker", return_value=marker),
            patch("codex_crew.session_pid._pid_start_token", return_value="tok-502"),
            patch(
                "codex_crew.session_pid.platform_compat.kill_pid",
                side_effect=overrides.get("kill_side_effect"),
            ),
            patch("codex_crew.session_pid._sel_orphan_mcp_subtree_kill") as mock_sel,
        ):
...
class TestOrphanDescendantWalk:
    ...
    def test_walks_depth_first_preorder(self) -> None:
        from codex_crew.session_pid import _orphan_descendants
        ...
        with (
            patch("codex_crew.session_pid._prune_from_orphan_walk", return_value=False),
            patch(
                "codex_crew.session_pid._pid_parent_and_token",
    ...
    def test_childless_root_yields_nothing(self) -> None:
        ...
        with patch("codex_crew.session_pid._prune_from_orphan_walk", return_value=False):
...
class TestGatewaySubtreePrune:
    ...
    CODEXCREW_SPAWNED and no gateway marker of their own, so each passed the
    ...
    _ARGV = {
        _GATEWAY_PID: b"python3\x00-m\x00codex_crew.cli\x00gateway",
        # The worker's OWN argv carries no gateway marker -- this is the trap.
        _WORKER_PID: b"python3\x00-m\x00codex_crew.mcp_gateway.stub\x00--server\x00x",
    ...
    def test_unreadable_argv_prunes_rather_than_descends(self) -> None:
        ...
        from codex_crew.session_pid import _prune_from_orphan_walk

        with patch("codex_crew.session_pid._pid_cmdline", return_value=b""):
...
class TestPidCmdline:
    ...
    def test_off_linux_returns_empty_without_spawning(self) -> None:
        ...
        from codex_crew.session_pid import _pid_cmdline
...
class TestStaleMapEdge:
    ...
    def _walk(self, live_ppid_of_recycled: int | None) -> list[int]:
        ...
        with (
            patch("codex_crew.session_pid._prune_from_orphan_walk", return_value=False),
            patch("codex_crew.session_pid._pid_parent_and_token", side_effect=_stat),
        ):
            return [pid for pid, _ in _orphan_descendants(self._ROOT_PID, self._MAP)]
...
class TestPidParentAndToken:
    ...
    def test_reads_ppid_and_starttime_from_one_stat(self) -> None:
        from codex_crew.session_pid import _pid_parent_and_token
...
class TestRootRecycleGuard:
    ...
    def _sweep(
        ...
        *,
        ...
        with (
            ...
            patch("codex_crew.session_pid._pid_start_token", side_effect=_token),
            patch("codex_crew.session_pid._pid_cmdline", return_value=b"npm\x00exec\x00p@latest"),
            patch("codex_crew.session_pid._env_has_codexcrew_marker", return_value=True),
            patch(
                "codex_crew.session_pid.platform_compat.kill_pid",
    ...
    def test_identity_is_captured_before_the_subtree_scan(self) -> None:
        ...
        with (
            ...
            patch.object(Path, "read_bytes", lambda self: _ROOT_CMDLINE),
            patch(
                "codex_crew.session_pid._build_child_map",
...
class TestBudgetExhaustionSparesRoot:
    ...
    def _run_with(self, descendant_count: int) -> dict[str, list[object]]:
        ...
        with (
            ...
            patch.object(Path, "read_bytes", lambda self: _ROOT_CMDLINE),
            patch("codex_crew.session_pid._build_child_map", return_value={}),
            patch("codex_crew.session_pid._orphan_descendants", return_value=_pairs(*pids)),
            # Must agree with _pairs(): a constant here reads as "recycled" for
            # every descendant and silently skips the whole subtree.
            patch(
                "codex_crew.session_pid._pid_start_token",
    ...
    def _sweep(
        ...
        *,
        ...
        with (
            ...
            patch("codex_crew.session_pid._build_child_map", return_value={}),
            patch("codex_crew.session_pid._orphan_descendants", return_value=[]),
            patch("codex_crew.session_pid._pid_start_token", return_value="tok-stable"),
            patch(
                "codex_crew.session_pid._is_sweepable_orphan_mcp",
