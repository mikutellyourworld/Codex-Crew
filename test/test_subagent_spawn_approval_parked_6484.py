...
class TestSpawnParkedOnApprovalIsObservable:
    ...
    async def test_parked_run_logs_under_its_run_id(self, caplog) -> None:  # type: ignore[no-untyped-def]
        ...
        The report's dead-end was "``codexcrew logs`` contained no error or
        ...
        # record is emitted under ``codex_crew.subagent`` rather than under
...
class TestParkedRunIsVisibleOnBothReadPaths:
    ...
    A blocking ``codexcrew spawn run`` polls the SINGLE-run status endpoint
    ...
    def _payload_flag(self, *, awaiting: bool, exec_started: float | None) -> bool:
        ...
        from codex_crew.dashboard.handlers.messaging import _awaiting_spawn_approval
    ...
    def test_cli_poll_announces_the_wait_once(self) -> None:
        ...
        import codex_crew.cli_commands as cli
