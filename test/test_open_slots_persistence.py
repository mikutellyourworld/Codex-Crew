...
Path resolution goes through ``codex_crew.config.loader.config_dir`` (the
...
honors ``CODEXCREW_HOME``. These tests set ``CODEXCREW_HOME`` to ``tmp_path``
...
def test_persist_honors_codexcrew_home_env(tmp_path, monkeypatch):
    """Snapshot lands in CODEXCREW_HOME, not ~/.codexcrew -- proves the env-var path."""
    custom_home = tmp_path / "custom-codexcrew-home"
    monkeypatch.setenv("CODEXCREW_HOME", str(custom_home))
    state = _make_state(tmp_path / "sessions")
    state.get_or_create_slot("chat-1-foo")
    state._persist_open_slots()
...
def test_persist_open_slots_handles_write_failure_gracefully(tmp_path, monkeypatch):
    ...
    state = _make_state(tmp_path / "sessions")
    state.get_or_create_slot("chat-1-foo")
    with patch("codex_crew.atomic_write.os.fchmod", side_effect=OSError("read-only filesystem")):
        # Should not raise
        state._persist_open_slots()
...
def test_restore_open_slots_rolls_back_partial_slot_on_rehydrate_failure(tmp_path, monkeypatch):
    ...
    from codex_crew.dashboard import chat_persistence as cp_mod
...
def test_restart_restore_paths_converge_on_one_slot(tmp_path, monkeypatch):
    ...
    """
    from codex_crew.dashboard.chat_persistence import restore_recent_sessions
...
def test_restore_reads_transcript_before_backfilling_tab_id(tmp_path, monkeypatch):
    ...
    import codex_crew.dashboard.chat_persistence as chat_persistence
...
def test_flush_during_async_restore_does_not_truncate_snapshot(tmp_path, monkeypatch):
    ...
    with patch(
        "codex_crew.dashboard.chat_persistence.asyncio.sleep", side_effect=flushing_sleep
    ):
        restored = asyncio.run(restore_open_slots_async(state2))
...
def test_restoring_flag_cleared_even_if_restore_raises(tmp_path, monkeypatch):
    ...
    state2 = _make_state(tmp_path / "sessions")
    with patch(
        "codex_crew.dashboard.chat_persistence._build_kiro_model_map",
...
def test_read_messages_reraises_after_exhausting_retries(tmp_path, monkeypatch):
    ...
    """
    from codex_crew.history import _METADATA_READ_ATTEMPTS
...
def test_metadata_failure_is_reported_above_debug(tmp_path, monkeypatch, caplog):
    ...
    with caplog.at_level("WARNING", logger="codex_crew.dashboard.chat_persistence"):
...
def test_missing_created_at_falls_through_to_restoring(tmp_path, monkeypatch):
    ...
    from codex_crew.dashboard import chat_persistence as cp
