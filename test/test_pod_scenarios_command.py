"""Tests for ``codexcrew pod scenarios`` discovery."""
    ...
    env.update(
        {
            ...
            "CODEXCREW_PROJECT_DIR": str(_REPO),
            "PYTHONPATH": _SRC,
        }
    )
    return subprocess.run(
        [sys.executable, "-m", "codex_crew", "pod", "scenarios", *args],
        capture_output=True,
        text=True,
        encoding="utf-8",
        cwd=tmp_path,
...
def test_human_output_lists_sorted_names_and_first_sentences(tmp_path: Path) -> None:
    ...
    assert rows["empty"].split(maxsplit=1)[1].strip() == (
        "A CODEXCREW_HOME with nothing in it but this manifest."
    )
    # The hint names whichever fixture sorts first, so it moves with the registry.
    assert hint == ("seed one with: codexcrew pod up <worktree> --seed " + next(iter(rows)))
...
def test_json_output_contains_complete_literal_descriptions(tmp_path: Path) -> None:
    ...
    assert described["empty"].startswith(
        "A CODEXCREW_HOME with nothing in it but this manifest. The gateway writes its"
    )
    assert "\n" in described["empty"]
...
def test_output_absorbs_an_added_fixture_without_moving_known_rows(
    ...
    assert hint == "seed one with: codexcrew pod up <worktree> --seed apps-installed"
