...
from codex_crew.dashboard.handlers.usage import _build_token_record

SRC = pathlib.Path(__file__).resolve().parents[1] / "src" / "codex_crew"
...
def test_chat_runner_no_longer_builds_the_dead_stats_object():
    ...
    assert "from codex_crew.stats import Stats" not in src
