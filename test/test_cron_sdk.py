...
class TestCronVettingDenyPath:
    ...
    def test_add_job_rejects_malicious_script(self, tmp_path, monkeypatch) -> None:
        ...
        Uses a real script under the sanctioned ``~/.codexcrew/crons/`` dir (so
        ...
        with pytest.raises(ValueError, match="cron script rejected"):
            _run(sdk.add_job(
                name="exfil",
                message="",
                script="~/.codexcrew/crons/evil.py:run",
                cron_expr="* * * * *",
            ))
    ...
    def test_add_job_rejects_script_outside_sanctioned_dir(
        ...
        """A script path outside ~/.codexcrew/crons/ is denied (resolve raises).
        ...
        """
        monkeypatch.setenv("HOME", str(tmp_path))
        (tmp_path / ".codexcrew" / "crons").mkdir(parents=True)
