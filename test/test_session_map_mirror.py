...
from codex_crew.session_map import (
    MIRROR_OPT_OUT_FLAG,
...
class TestPrunePreservesMirror:
    ...
    def test_a_live_sid_with_a_mirror_is_left_alone(self, tmp_path):
        ...
        with patch("codex_crew.session_map.config_dir", return_value=tmp_path):
            ...
            sm.set_mirror_link(key, link, accepts_inbound=True)
            with patch("codex_crew.session_map._kiro_sessions_dir", return_value=tmp_path):
...
class TestLegacyDashboardSpelling:
    ...
    CHANNEL = "telegram:codexcrew:direct:7"
    LEGACY = "dashboard:telegram_codexcrew_direct_7"
...
class TestBatchedSave:
    ...
    def test_a_sequence_writes_once(self, session_map):
        ...
        with patch.object(session_map, "_write", side_effect=lambda: writes.append(1)):
            with session_map.batched_save():
                session_map.set_mirror_link("telegram:codexcrew:direct:7", self.LINK)
                session_map.set_flag("telegram:codexcrew:direct:7", MIRROR_OPT_OUT_FLAG, True)
                session_map.set("telegram:codexcrew:direct:7", "sid-1")
        assert writes == [1]
    ...
    def test_a_block_that_mutates_nothing_writes_nothing(self, session_map):
        ...
        with patch.object(session_map, "_write", side_effect=lambda: writes.append(1)):
            with session_map.batched_save():
                session_map.get_mirror_link("telegram:codexcrew:direct:7")
        assert writes == []
...
class TestAutomaticMirrorOptOut:
    ...
    def test_opt_out_survives_a_reload(self, session_map, tmp_path):
        ...
        with patch("codex_crew.session_map.config_dir", return_value=tmp_path):
            reloaded = SessionMap()
        assert reloaded.get_flag("telegram:codexcrew:direct:7", MIRROR_OPT_OUT_FLAG) is True
    ...
    def test_the_refusal_is_keyed_by_the_durable_bucket_not_the_generation(self):
        ...
        """
        assert _opt_out_key("telegram:codexcrew:direct:7:gen3") == "telegram:codexcrew:direct:7"
        assert _opt_out_key("telegram:codexcrew:direct:7") == "telegram:codexcrew:direct:7"
        assert (
            _opt_out_key("telegram:codexcrew:forum:-100123:5:gen9")
            == "telegram:codexcrew:forum:-100123:5"
        )
    ...
    def test_the_suffix_is_stripped_even_when_the_key_does_not_parse(self):
        ...
        """
        assert _opt_out_key("unified:codexcrew:gen3") == "unified:codexcrew"
        assert _opt_out_key("unified:codexcrew") == "unified:codexcrew"
        # A trailing segment that merely starts with "gen" is not a generation.
        assert _opt_out_key("telegram:codexcrew:direct:general") == (
            "telegram:codexcrew:direct:general"
        )
    ...
    def test_every_generation_shares_one_flag_row(self, session_map):
        ...
        for gen in ("", ":gen1", ":gen2", ":gen7"):
            session_map.set_flag(
                _opt_out_key(f"telegram:codexcrew:direct:7{gen}"), MIRROR_OPT_OUT_FLAG, True
            )
        flagged = [k for k, e in session_map._data.items() if e.get("flags")]
        assert flagged == ["telegram:codexcrew:direct:7"]
    ...
    def test_a_refusal_stored_under_the_old_generation_key_is_still_honoured(self, session_map):
        ...
        key = "telegram:codexcrew:direct:7:gen3"
        session_map.set_flag(key, MIRROR_OPT_OUT_FLAG, True)
    ...
    def test_reading_a_legacy_refusal_promotes_it_to_the_bucket(self, session_map):
        ...
        """
        mgr = _manager_over(session_map)
        session_map.set_flag("telegram:codexcrew:direct:7:gen3", MIRROR_OPT_OUT_FLAG, True)
        assert mgr.mirror_opt_out("telegram:codexcrew:direct:7:gen3") is True
        # Promoted to the bucket, and the generation row retired with it.
        assert session_map.get_flag("telegram:codexcrew:direct:7", MIRROR_OPT_OUT_FLAG) is True
        assert session_map.get_flag("telegram:codexcrew:direct:7:gen3", MIRROR_OPT_OUT_FLAG) is False
        # And it now survives the rotation that would have dropped it.
        assert mgr.mirror_opt_out("telegram:codexcrew:direct:7:gen4") is True
    ...
    def test_a_session_scoped_flag_does_not_make_an_entry_immortal(self, session_map):
        ...
        for flag in ("temporary", "incognito"):
            key = f"slack:codexcrew:{flag}"
            session_map.set_flag(key, flag, True)
        assert session_map.prune() == 2
        assert session_map.get_flag("slack:codexcrew:temporary", "temporary") is False
        assert session_map.get_flag("slack:codexcrew:incognito", "incognito") is False
...
def sel_events():
    ...
    with patch("codex_crew.session_map.sel") as fake_sel:
...
class TestPruneRemovesThroughTheChokePoint:
    ...
    def test_a_pruned_binding_is_audited_and_announced(
        ...
        monkeypatch.setattr("codex_crew.session_map._survives_prune", lambda entry: False)
    ...
    async def test_startup_collection_defers_the_write_off_loop(self, session_map):
        ...
        """
        import codex_crew.session_map as mod
...
class TestAnnouncementIsBestEffort:
    ...
    def test_listener_exception_is_swallowed_and_logged_at_warning(self, session_map, caplog):
        ...
        try:
            with caplog.at_level(logging.WARNING, logger="codex_crew.session_map"):
                session_map.set_mirror_link("dashboard:chat-1", INBOUND_LINK, accepts_inbound=True)
                assert session_map.clear_mirror_link("dashboard:chat-1") is True
        finally:
            set_unbind_listener(None)
...
class TestAuditDoesNotBlockTheLoop:
    ...
    async def test_the_loop_keeps_beating_while_the_audit_runs(self, session_map, unbind_calls):
        ...
        session_map.set_mirror_link("dashboard:chat-1", INBOUND_LINK, accepts_inbound=True)
        try:
            with patch("codex_crew.session_map.sel", _blocking_sel):
    ...
    async def test_an_audit_failure_is_isolated_and_logged_at_warning(self, session_map, caplog):
        session_map.set_mirror_link("dashboard:chat-1", INBOUND_LINK, accepts_inbound=True)
        with caplog.at_level(logging.WARNING, logger="codex_crew.session_map"):
            with patch("codex_crew.session_map.sel", side_effect=RuntimeError("sel down")):
    ...
    def test_off_loop_the_audit_runs_inline_exactly_once(self, session_map):
        ...
        session_map.set_mirror_link("dashboard:chat-1", INBOUND_LINK, accepts_inbound=True)
        with patch("codex_crew.session_map.sel", return_value=fake):
            session_map.clear_mirror_link("dashboard:chat-1", reason="dashboard_unlink")
