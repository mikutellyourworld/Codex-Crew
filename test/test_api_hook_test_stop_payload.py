...
``assistant_text`` key (the CODEXCREW_HOOK_CONTEXT env var is capped at 500).
...
from codex_crew.dashboard.handlers.hooks import api_hook_test
from codex_crew.hooks import HOOK_EVENT_STOP, HOOK_EVENT_USER_PROMPT_SUBMIT, ScriptHookStore
...
async def test_stop_hook_test_supplies_full_assistant_text(tmp_path):
    ...
    with (
        patch("codex_crew.hooks.run_script_hook", new_callable=AsyncMock, return_value=fake_result) as mock_run,
        patch("codex_crew.dashboard.handlers.hooks._sel", return_value=MagicMock()),
    ):
...
async def test_hook_test_redacts_context_in_sel_metadata(tmp_path):
    ...
    with (
        patch(
            "codex_crew.hooks.run_script_hook",
            new_callable=AsyncMock,
            return_value=fake_result,
        ) as mock_run,
        patch("codex_crew.dashboard.handlers.hooks._sel", return_value=audit),
    ):
