...
from codex_crew import beacon, diagnostics
from codex_crew.dashboard.handlers import diagnostics as dh
from codex_crew.diagnostics import BundleResult

_GATEWAY = (
    "09:00 boot ok\n"
    "Authorization: Bearer sk-ant-SECRETtoken1234567890abcXYZ\n"
    "Set-Cookie: mc_token_5486=supersecretcookievalueABCDEF123456\n"
    "Set-Cookie: mc_refresh_5486=REFRESHsecretVALUE9876543210\n"
...
def _isolate(monkeypatch, home: Path) -> None:
    ...
    monkeypatch.setattr("codex_crew.diagnostics.config_dir", lambda: home)
...
def test_issue_url_is_well_formed(tmp_path, monkeypatch):
    ...
    assert r.github_issue_url.startswith(
        "https://github.com/mikutellyourworld/Codex-Crew/issues/new?"
    )
...
class TestTerminalIssueUrlSurvivesRedaction:
    """The link `codexcrew doctor` PRINTS must reach the user intact.
    ...
    def _result(self, tmp_path):  # type: ignore[no-untyped-def]
        from codex_crew import diagnostics

        name = "codexcrew-diagnostics-20260811.zip"
...
def _stub_sel(monkeypatch) -> MagicMock:
    ...
    sel = MagicMock()
    monkeypatch.setattr("codex_crew.dashboard.handlers.sel", lambda: sel)
    return sel


def test_download_rejects_path_traversal(tmp_path, monkeypatch):
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.diagnostics.config_dir", lambda: tmp_path
    )
...
def test_collect_handler_returns_download_url(tmp_path, monkeypatch):
    ...
    fake = BundleResult(
        ...
        github_issue_url="https://github.com/mikutellyourworld/Codex-Crew/issues/new?title=x",
    )
...
def test_old_bundles_are_pruned(tmp_path):
    ...
    base = time.time()
    for i in range(6):
        p = out / f"codexcrew-diagnostics-2026010{i}-000000.zip"
        p.write_bytes(b"x")
        os.utime(p, (base + i, base + i))  # distinct mtimes; newest = i==5
    diagnostics._prune_old_bundles(out, keep=3)
    kept = {p.name for p in out.glob("codexcrew-diagnostics-*.zip")}
    assert len(kept) == 3
    assert "codexcrew-diagnostics-20260105-000000.zip" in kept  # newest kept
    assert "codexcrew-diagnostics-20260100-000000.zip" not in kept  # oldest pruned
