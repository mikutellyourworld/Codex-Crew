...
from codex_crew.dashboard import session_memory as sm
...
def stub_proc(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(sm, "_read_cmdline", lambda pid: "python -m codex_crew.mcp_gateway.stub")
...
async def test_background_session_records_the_agent_it_runs_as() -> None:
    ...
    from codex_crew.session import BACKGROUND_AGENT, BACKGROUND_KEY, SessionManager
...
def test_runtime_pids_includes_runtimes_held_only_on_the_manager() -> None:
    ...
    whole runtime each from the host total, understating "Codex Crew Used" by the
...
def test_slot_spend_sums_credits_and_counts_turns(
    ...
    from codex_crew.dashboard.handlers import usage
