...
async def _context(slot, tracker, stage_idx):
    ...
    """
    from codex_crew.dashboard.chat_orchestrator import _build_stage_context
...
def _fixture(tmp_path, monkeypatch, *, content="stage one output"):
    ...
    monkeypatch.setattr(
        "codex_crew.dashboard.chat_orchestrator.is_sensitive_path", lambda p: False
    )
...
async def test_sensitive_path_is_not_read(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "codex_crew.dashboard.chat_orchestrator.is_sensitive_path", lambda p: True
    )
