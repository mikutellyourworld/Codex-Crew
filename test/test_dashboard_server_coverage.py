"""Coverage tests for the residual branches of ``codex_crew.dashboard.server``.
...
class TestShouldPreventSleep:
    ...
    async def test_config_read_failure_allows_sleep(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(srv, "CodexCrewConfig", loader)
...
class TestPrecomputeTelemetry:
    ...
    def test_every_stage_failure_is_swallowed(self, monkeypatch) -> None:
        import codex_crew.dashboard.handlers_system as hs
...
class TestWriteSecretFileDescriptorCleanup:
    ...
    def test_permission_hardening_failure_closes_fd_and_removes_file(
        ...
        with patch("codex_crew.dashboard.server.os.close", _spy):
    ...
    def test_close_failure_does_not_mask_the_original_error(
        ...
        with patch("codex_crew.dashboard.server.os.close", _close_then_fail):
            with pytest.raises(OSError, match="chmod denied"):
                srv._write_secret_file(secret_path, "s")
...
class TestConnectionsWarmLifecycle:
    ...
    async def test_kick_tracks_scavenging_without_blocking_and_cleanup_retires_runtime(
        ...
        from codex_crew.connections import warm
...
class TestSttHooks:
    ...
    async def test_shutdown_does_not_build_an_engine_that_never_existed(self, monkeypatch) -> None:
        ...
        monkeypatch.delitem(sys.modules, "codex_crew.stt.engine", raising=False)
        for hook in app.on_cleanup:
            await hook(app)
        assert (
            "codex_crew.stt.engine" not in sys.modules
