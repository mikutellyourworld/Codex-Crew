...
from codex_crew.apps.builtins.issue_radar.backend.errors import sanitize_cli_stderr
from codex_crew.context import _neutralize_structural_markers
from codex_crew.dashboard import openai_compat
...
class TestDeployBucketDurability:
    ...
    def _argvs(self):
        with mock.patch.object(engine, "_checked") as checked:
            engine._harden_bucket("codexcrew-web-example", "p", "TagSet=[]")
    ...
    def test_teardown_is_not_version_aware(self):
        ...
        src = (Path(__file__).resolve().parent.parent
               / "src" / "codex_crew" / "deploy" / "engine.py").read_text(encoding="utf-8")
...
class TestMdNotebookErrorRedaction:
    def test_absolute_paths_are_stripped(self):
        ...
        from codex_crew.apps.builtins.md_notebook import server as md_server
        exc = FileNotFoundError(
            "[Errno 2] No such file or directory: '/home/alice/.codex-crew/vaults/v1'"
        )
    ...
    def test_both_middleware_branches_route_through_the_helper(self):
        ...
        src = (Path(__file__).resolve().parent.parent / "src" / "codex_crew" / "apps"
