"""Unit tests for codex_crew.platform_compat — the cross-platform shim that lets
CodexCrew run natively on Windows alongside macOS/Linux.
...
class TestReexecPythonModule:
    def test_windows_uses_space_free_argv0(self, monkeypatch):
        executable = (
            r"C:\Users\alice\AppData\Local\Programs\CodexCrew Nightly"
            r"\resources\backend-dist\codexcrew-backend\python.exe"
        )
        calls = []
        ...
        pc.reexec_python_module("codex_crew", ["gateway", "--port", "5486"])

        assert calls == [
            (
                executable,
                ["python.exe", "-m", "codex_crew", "gateway", "--port", "5486"],
            )
        ]
    ...
    def test_posix_preserves_full_argv0_and_pins_utf8(self, monkeypatch):
        executable = "/opt/Codex Crew/bin/python3"
        calls = []
        ...
        pc.reexec_python_module("codex_crew", ["gateway"])

        assert calls == [(executable, [executable, "-m", "codex_crew", "gateway"])]
        assert os.environ["PYTHONUTF8"] == "1"
        assert os.environ["PYTHONIOENCODING"] == "utf-8:backslashreplace"
    ...
    def test_reexec_successor_survives_hostile_parent_encoding(self, tmp_path):
        ...
        probe.write_text(
            ...
            "if os.environ.get('_CODEXCREW_UTF8_REEXEC_PROBE') == '1':\n"
            "    print('>_ restarted')\n"
            "else:\n"
            "    os.environ['_CODEXCREW_UTF8_REEXEC_PROBE'] = '1'\n"
        ...
        assert ">_ restarted".encode() in result.stdout
...
class TestUtf8Console:
    ...
    def test_emoji_print_does_not_raise_after_call(self, capsys):
        # The bug this guards: CodexCrew prints non-ASCII glyphs everywhere, and on
        ...
        pc.ensure_utf8_console()
        print("中文 CodexCrew 日本語")  # non-cp1252-encodable glyphs
...
class TestProcessIdentityPosix:
    ...
    def test_process_matches_true_for_a_child_with_a_known_token(self):
        ...
        token = "codexcrew-procmatch-probe"
...
class TestProcessArgvMatchesExact:
    ...
    def test_exact_argv_matches_and_near_misses_do_not(self):
        if pc.IS_POSIX:
            ...
            child, argv = self._spawn("codexcrew-argvexact-probe")
        try:
            if pc.IS_POSIX:
...
class TestRestrictToOwner:
    """Fail-loud owner-only lockdown used by every ~/.codexcrew secret writer.
...
class TestFindListeningPidsErrors:
    ...
    def test_windows_finds_ipv6_listener_via_netstat(self, monkeypatch):
        ...
        # so `codexcrew stop` / `codexcrew restart` silently no-op'd when the
...
class TestKillAsyncVariants:
    ...
    Windows ``taskkill`` spawn to :func:`codex_crew.executors.subprocess_executor`
    ...
    def test_windows_kill_pid_async_offloads_via_subprocess_executor(self, monkeypatch):
        ...
        # module namespace (top-level `from codex_crew.executors import ...`)
...
def _pin_warnings(caplog):
    ...
    return [r for r in caplog.records if r.name == "codex_crew.platform_compat"]
...
def test_a_tool_installed_outside_the_trusted_dirs_is_diagnosable(tmp_path, monkeypatch, caplog):
    ...
    operator sees only ``codexcrew stop`` no-opping and a prompt to install a
    ...
    with caplog.at_level(logging.WARNING, logger="codex_crew.platform_compat"):
        assert pc.trusted_system_bin("definitely-not-a-system-tool") is None
