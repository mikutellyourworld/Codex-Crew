...
def _capture_subagent_done(gateway):
    ...
    with patch("codex_crew.slack.gateway.SubagentManager") as mock_mgr_cls:
...
class TestSubagentDoneCronRouting:
    ...
    async def test_injects_into_cron_session(self) -> None:
        ...
        info.done = True
        with (
            patch("codex_crew.slack.gateway.redact_exfiltration_urls", return_value=("", False)),
            patch("codex_crew.slack.gateway.redact_credentials", return_value=("", False)),
            patch(
    ...
    async def test_retries_on_acp_error(self) -> None:
        ...
        with (
            patch("codex_crew.slack.gateway.redact_exfiltration_urls", return_value=("", False)),
            patch("codex_crew.slack.gateway.redact_credentials", return_value=("", False)),
            patch("codex_crew.slack.gateway.stream_and_collect", side_effect=flaky_stream),
...
class TestSubagentDoneCancelsBeforeRelease:
    ...
    async def test_cron_injection_cancels_before_release(self) -> None:
        ...
        p1, p2 = self._patches()
        with (
            patch("codex_crew.slack.gateway.stream_and_collect", new_callable=AsyncMock, return_value="ok"),
            p1, p2,
        ):
            await subagent_done(info)
    ...
    async def test_slack_injection_cancels_before_release(self) -> None:
        ...
        with (
            ...
            patch("codex_crew.slack.gateway.render_for_slack", return_value=["ok"]),
            p1, p2,
        ):
            await subagent_done(info)
        assert call_order == ["cancel", "release"]
...
class TestDashboardInjectionRoutesRunChat:
    ...
    def _patches(self):
        return (
            patch("codex_crew.slack.gateway.redact_exfiltration_urls", side_effect=lambda s: (s, False)),
            patch("codex_crew.slack.gateway.redact_credentials", side_effect=lambda s: (s, False)),
        )
    ...
    async def test_routes_through_run_chat_when_idle(self) -> None:
        ...
        p1, p2 = self._patches()
        with (
            patch("codex_crew.slack.gateway._run_chat", new_callable=AsyncMock),
            p1, p2,
        ):
            await subagent_done(info)
    ...
    async def test_busy_slot_awaits_then_injects(self) -> None:
        ...
        with (
            patch("codex_crew.slack.gateway._run_chat", _mock_run_chat),
            p1, p2,
        ):
            await subagent_done(info)
    ...
    async def test_busy_slot_timeout_queues_result(self) -> None:
        ...
        p1, p2 = self._patches()
        with (
            patch("codex_crew.slack.gateway.INJECTION_TIMEOUT", 0.01),
            p1, p2,
        ):
            await subagent_done(info)
...
class TestCronCallbackDashboardChat:
    ...
    def test_persistent_session_with_slot_calls_inject(self) -> None:
        ...
        with patch(
            "codex_crew.slack.gateway.inject_cron_result_to_dashboard"
        ) as mock_inject:
    ...
    def test_inject_fires_on_dedup_suppression(self) -> None:
        ...
        with patch(
            ...
            from codex_crew.slack.gateway import _result_hash
    ...
    def test_hide_in_chat_notify_meta_omits_slot_even_with_existing_slot(self) -> None:
        ...
        gateway.dashboard_state.has_slot = MagicMock(return_value=True)
        job = _make_job(persistent_session=True, hide_in_chat=True)
        with patch("codex_crew.slack.gateway.inject_cron_result_to_dashboard"):
            _run_callback(gateway, job, stream_result="hidden output")
