...
from codex_crew.cron_script import (
    Done,
    Report,
    ScriptContext,
    Skip,
...
def _crons_dir_tracks_patched_home(monkeypatch):
    """Keep ``cron_script.config_dir()`` pointed at ``<patched home>/.codexcrew``.

    The data home moved from the top-level ``~/.codexcrew`` to ``~/.codex-crew``
    (``config_dir()``), and ``resolve_script_path`` now derives its allowed
    ``crons/`` dir from ``config_dir()`` rather than ``Path.home()/".codexcrew"``.
    These tests patch ``Path.home()`` per-test and write scripts under
    ``<home>/.codexcrew/crons`` — but ``config_dir()`` reads ``CODEXCREW_HOME``
    (pinned to a *different* tmp dir by the conftest ``_isolate_codexcrew_home``
    fixture), so without this redirect the allowed dir would never match.
    Redirect ``config_dir`` to ``Path.home()/".codexcrew"`` (evaluated lazily, so
    it tracks whatever ``Path.home()`` each test patches) — preserving the
    existing ``.codexcrew/crons`` layout the tests build. Tests that patch
    ``cron_script.config_dir`` themselves still win (applied later).
    """
    monkeypatch.setattr(
        "codex_crew.cron_script.config_dir", lambda: Path.home() / ".codexcrew"
    )
...
class TestResolveScriptPath:
    ...
    def test_valid_path(self, tmp_path):
        crons_dir = tmp_path / ".codexcrew" / "crons"
    ...
    def test_file_not_found_raises(self, tmp_path):
        with patch("pathlib.Path.home", return_value=tmp_path):
            with pytest.raises(FileNotFoundError):
                resolve_script_path(str(tmp_path / ".codexcrew/crons/missing.py") + ":run")
    ...
    def test_tilde_expansion(self, tmp_path):
        ...
        with patch("pathlib.Path.home", return_value=tmp_path), patch.dict(
            ...
            file_path, func_name = resolve_script_path("~/.codexcrew/crons/monitor.py:check")
...
class TestRunCommandSandboxed:
    ...
    def _passthrough_sandbox(self, monkeypatch, posix_test_shell):
        ...
        """
        monkeypatch.setattr(
            "codex_crew.cron_script.wrap_argv", lambda argv, **k: (list(argv), None)
        )
        ...
        monkeypatch.setattr(
            "codex_crew.cron_script._resolve_command_shell", lambda: posix_test_shell
        )
...
class TestCronSandboxUnavailableIsStructuredNotRaised:
    ...
    def _sandbox_refuses(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.cron_script.wrap_argv", _raise)
        ...
        monkeypatch.setattr("codex_crew.cron_script._resolve_command_shell", lambda: "sh")
    ...
    def test_script_cron_returns_error_with_remedy(self, _sandbox_refuses, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.cron_script.resolve_script_path",
...
class TestCommandCronShellResolution:
    ...
    def test_posix_strict_sh_is_accepted(self, monkeypatch):
        ...
        written against."""
        from codex_crew import cron_script
    ...
    def test_shell_probe_detects_brace_expansion(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.cron_script.wrap_argv", lambda argv, **k: (argv, None)
        )
...
class TestRunScriptSandboxed:
    ...
    @pytest.fixture(autouse=True)
    def _passthrough_sandbox(self, monkeypatch, posix_test_shell):
        """Bypass OS-sandbox wrap and ensure subprocess can import codex_crew.
        ...
        PYTHONPATH is not set so the subprocess can't import codex_crew. Inject the
    ...
    def test_ok_status(self, tmp_path):
        script_path = self._write_script(
            tmp_path,
            """
from codex_crew.cron_script import Skip, Done
def run(ctx):
...
class TestResolveMcpServer:
    ...
    def test_returns_argv_and_env_for_valid_config(self, tmp_path):
        ...
        (agents_dir / "codexcrew.json").write_text(json.dumps(config))
        with patch("pathlib.Path.home", return_value=tmp_path):
    ...
    def _write_env_config(tmp_path, env_block):
        ...
        (agents_dir / "codexcrew.json").write_text(
            json.dumps(
...
class TestMcpToolClient:
    ...
    def test_init_fails_for_unknown_server(self, tmp_path):
        from codex_crew.cron_script import McpToolClient
    ...
    def test_init_passes_spec_env_to_popen_and_scrubs_secrets(self, monkeypatch):
        ...
        monkeypatch.setenv("CODEXCREW_PROBE_VAR", "keep-me")
        ...
        with patch(
            "codex_crew.cron_script._resolve_mcp_server",
            return_value=(("some-mcp", "--mode", "stdio"), spec_env),
        ), patch(
            "codex_crew.cron_script.wrap_argv",
            return_value=(["some-mcp", "--mode", "stdio"], None),
        ), patch(
            "codex_crew.cron_script.cgroup_scope_argv",
            side_effect=lambda argv: argv,
        ), patch(
            "codex_crew.cron_script.popen_limited", return_value=mock_proc
        ) as mock_popen:
        ...
        assert passed_env.get("CODEXCREW_PROBE_VAR") == "keep-me"
    ...
    def test_init_leaves_an_operator_command_argv0_for_the_spec_path_to_resolve(
        ...
        """
        from codex_crew.cron_script import McpToolClient, _resolve_mcp_server
        ...
        with patch(
            ...
            "codex_crew.cron_script.popen_limited", return_value=self._mock_handshake_proc()
        ) as mock_popen:
            client = McpToolClient("path-server")
            client.close()
    ...
    def test_init_drops_loader_injection_keys_from_spec_env(self, monkeypatch):
        ...
        with patch(
            ...
            "codex_crew.cron_script.wrap_argv", return_value=(["some-mcp"], None)
        ), patch(
            "codex_crew.cron_script.cgroup_scope_argv", side_effect=lambda argv: list(argv)
        ), patch(
            "codex_crew.cron_script.popen_limited", return_value=self._mock_handshake_proc()
        ) as mock_popen:
            client = McpToolClient("path-server")
    ...
    def test_init_drops_reserved_codexcrew_namespace_from_spec_env(self, monkeypatch):
        """SECURITY regression: a spec ``env`` must not forge caller identity.

        The server this bridge most often spawns is ``codexcrew-cron`` itself,
        whose ownership checks resolve the calling session from
        ``CODEXCREW_SESSION_KEY``/``CODEXCREW_HOST_PID`` when no gateway caller
        ...
        ``CODEXCREW_CLI`` is asserted here too, as history rather than as a live
        ...
        ``CODEXCREW_OWNER_ID``, and the loader prefixes cover ``LD_*``/``PYTHON*``.
        And unlike those, the OS sandbox is not a mitigation at all -- confinement
        bounds what the child may touch, not whose jobs Codex Crew believes it
        owns. Hence the reserved-namespace deny in the shared sanitizer.
        """
        from codex_crew.cron_script import McpToolClient, _resolve_mcp_server

        _resolve_mcp_server.cache_clear()
        for key in ("CODEXCREW_CLI", "CODEXCREW_SESSION_KEY", "CODEXCREW_HOST_PID"):
            monkeypatch.delenv(key, raising=False)
        ...
        with patch(
            ...
            client = McpToolClient("codexcrew-cron")
            client.close()
    ...
    def test_an_identity_key_cannot_be_forged_through_a_cron_spec_env_block(
        ...
        The key under test used to be ``CODEXCREW_CLI``, an ambient admin flag
        whose consumer #6624 deleted outright -- it had no producer in ``src/``,
        so there was no claim to re-ground. ``CODEXCREW_SESSION_KEY`` is the right
        ...
        (``sh -c 'CODEXCREW_SESSION_KEY=... exec codexcrew-cron'``), so it is a
        ...
        Codex Crew believes it owns. Closing it needs consumer-side vouching
        rather than another deny-list; naming the boundary here keeps the
        assertion honest about which half is proven.
        """
        from codex_crew.cron_script import McpToolClient, _resolve_mcp_server
        from codex_crew.mcp_core import _resolve_session_key_strict
        ...
        monkeypatch.setenv("CODEXCREW_SESSION_KEY", forged)
        assert _resolve_session_key_strict() == forged
        monkeypatch.delenv("CODEXCREW_SESSION_KEY")

        with patch(
            "codex_crew.cron_script._resolve_mcp_server",
            return_value=(("some-mcp",), {"CODEXCREW_SESSION_KEY": forged}),
        ), patch(
            "codex_crew.cron_script.wrap_argv", return_value=(["some-mcp"], None)
        ), patch(
            "codex_crew.cron_script.cgroup_scope_argv", side_effect=lambda argv: list(argv)
...
class TestScriptContextNotify:
    ...
    def test_notify_success(self):
        ...
        with patch("codex_crew.cron_script.loopback_urlopen", return_value=mock_response):
    ...
    def test_notify_redacts_credentials(self):
        ...
        mock_response.__exit__ = MagicMock(return_value=False)
        with patch(
            "codex_crew.cron_script.loopback_urlopen", return_value=mock_response
        ) as mock_urlopen:
...
class TestRunScriptSandboxedEdgeCases:
    ...
    def test_report_status(self, tmp_path):
        script_path = self._write_script(
            tmp_path,
            """
from codex_crew.cron_script import Report
def run(ctx):
...
""",
    ...
    def test_script_with_imports(self, tmp_path):
        script_path = self._write_script(
            ...
            """
import os
from codex_crew.cron_script import Done
def run(ctx):
...
class TestResolveMcpServerAimFallback:
    ...
    def test_aim_fallback_path(self, tmp_path):
        # No codexcrew.json, but a fallback agent spec exists
        _resolve_mcp_server.cache_clear()
        agents_dir = tmp_path / ".kiro" / "agents"
        agents_dir.mkdir(parents=True)
        config = {"mcpServers": {"builder-mcp": {"command": "npx", "args": ["-y", "builder-mcp"]}}}
        (agents_dir / "my-codexcrew-agent.json").write_text(json.dumps(config))
        with patch("pathlib.Path.home", return_value=tmp_path):
...
class TestScriptContextCallToolSuccess:
    ...
    def test_call_tool_success_path(self, tmp_path):
        ...
        with patch("codex_crew.cron_script.McpToolClient", return_value=mock_client), patch(
            "codex_crew.cron_script.sel"
        ) as mock_sel:
...
class TestScriptContextPost:
    ...
    def test_post_failure_returns_error(self):
        ...
        ctx = ScriptContext(job=job)
        with patch(
            "codex_crew.cron_script.loopback_urlopen", side_effect=Exception("connection refused")
        ):
            result = ctx._post("/api/deliver", {"text": "hello"})
...
class TestMcpToolClientInitFailures:
    ...
    def test_popen_exception_cleans_sandbox(self, tmp_path):
        ...
        with patch("pathlib.Path.home", return_value=tmp_path), patch(
            "codex_crew.cron_script.wrap_argv", return_value=(["false"], str(cleanup))
...
class TestResolveScriptPathSensitive:
    ...
    def test_sensitive_path_blocked(self, tmp_path):
        ...
        script.write_text("def run(ctx): pass")
        with patch("pathlib.Path.home", return_value=tmp_path), patch(
            "codex_crew.cron_script.is_sensitive_path", return_value=True
        ):
...
class TestRunScriptSandboxedErrorPaths:
    ...
    def test_nonzero_exit_no_stdout(self, tmp_path):
        ...
        with patch(
            "codex_crew.cron_script.resolve_script_path", return_value=("/f.py", "run")
        ), patch("codex_crew.cron_script.wrap_argv", return_value=(["true"], None)), patch(
...
class TestMcpCronHandlerPaths:
    ...
    def test_cron_list_shows_error(self, tmp_path):
        ...
        from codex_crew.cron import CronJob, CronSchedule
        from codex_crew.mcp_cron import _call_tool_inner
        ...
        job.session_key = os.environ["CODEXCREW_SESSION_KEY"]
        with patch("codex_crew.mcp_cron.config_dir", return_value=tmp_path), patch(
            "codex_crew.mcp_cron.CronService"
        ) as mock_svc:
    ...
    def test_cron_add_invalid_script_path(self, tmp_path):
        ...
        with patch("codex_crew.mcp_cron.config_dir", return_value=tmp_path):
    ...
    def test_cron_add_with_command(self, tmp_path):
        ...
        with patch("codex_crew.mcp_cron.config_dir", return_value=tmp_path), patch(
            "codex_crew.mcp_cron.CronService"
        ) as mock_svc, patch("codex_crew.mcp_cron._resolve_session_key", return_value=""):
    ...
    def test_cron_add_with_script(self, tmp_path):
        ...
        with patch("codex_crew.mcp_cron.config_dir", return_value=tmp_path), patch(
            "codex_crew.mcp_cron.CronService"
        ) as mock_svc, patch("codex_crew.mcp_cron._resolve_session_key", return_value=""), patch(
...
class TestKillBroadcastGuard:
    ...
    def test_mock_pid_refused(self):
        from codex_crew.cron_script import _resolve_safe_pgid
    ...
    def test_kill_proc_group_never_calls_killpg_for_mock(self):
        ...
        """
        from codex_crew.cron_script import _kill_proc_group
...
class TestResolveInternalSecret:
    ...
    def test_uses_env_when_set(self, tmp_path):
        with patch.dict(os.environ, {"CODEXCREW_INTERNAL_SECRET": "fromenv"}), patch(
            "codex_crew.config.loader.config_dir", return_value=tmp_path
        ):
            (tmp_path / ".local_secret").write_text("fromfile")
            assert _resolve_internal_secret(5486) == "fromenv"

    def test_falls_back_to_local_secret_file(self, tmp_path):
        (tmp_path / ".local_secret").write_text("filesecret\n")
        env = {k: v for k, v in os.environ.items() if k != "CODEXCREW_INTERNAL_SECRET"}
        with patch.dict(os.environ, env, clear=True), patch(
            "codex_crew.config.loader.config_dir", return_value=tmp_path
        ):
            assert _resolve_internal_secret(5486) == "filesecret"
    ...
    def test_empty_when_neither_present(self, tmp_path):
        ...
        with patch.dict(os.environ, env, clear=True), patch(
            ...
            assert _resolve_internal_secret(5486) == ""

    def test_env_empty_string_falls_back_to_file(self, tmp_path):
        (tmp_path / ".local_secret").write_text("filesecret")
        with patch.dict(os.environ, {"CODEXCREW_INTERNAL_SECRET": ""}), patch(
            "codex_crew.config.loader.config_dir", return_value=tmp_path
        ):
            assert _resolve_internal_secret(5486) == "filesecret"
    ...
    def test_run_script_writes_local_secret_to_sandbox_when_env_unset(self, tmp_path):
        ...
        def fake_popen(argv, **kwargs):
            sf = kwargs.get("env", {}).get("_CODEXCREW_SECRET_FILE")
        ...
        with patch.dict(os.environ, env, clear=True), patch(
            "codex_crew.config.loader.config_dir", return_value=tmp_path
        ), patch("codex_crew.cron_script.resolve_script_path", return_value=("/f.py", "run")), patch(
            "codex_crew.cron_script.wrap_argv", return_value=(["true"], None)
        ), patch(
...
class TestPostKillDrainTimeoutHardening:
    ...
    def test_command_drain_timeout_closes_pipes_and_keeps_contract(self, monkeypatch):
        ...
        with patch("subprocess.Popen", return_value=mock_proc), patch(
            "codex_crew.platform_compat.kill_process_tree", return_value=True
        ):
