...
3. **The scan cannot be walked around.** ``TYPE_CHECKING``-only imports,
   ...
   forbidden package (``from codex_crew import acp``) are all in scope, and the
...
def test_the_sdk_package_exists_and_is_exempt(gate):
    ...
    assert (ROOT / "src" / "codex_crew" / "agent_sdk" / "__init__.py").is_file()
    assert gate._is_exempt("src/codex_crew/agent_sdk/drivers/acp.py")
...
def test_the_forbidden_roots_include_the_re_export_channel(gate):
    """Watching only codex_crew.acp would let providers launder ACP shapes.
    ...
    """
    assert "codex_crew.acp" in gate.FORBIDDEN_ROOTS
    assert "codex_crew.providers" in gate.FORBIDDEN_ROOTS
...
def test_a_violation_outside_the_baseline_is_caught(gate):
    ...
    hits = gate._violations_in_source(
        "src/codex_crew/dashboard/brand_new.py",
...
def test_a_relative_import_resolves_to_its_absolute_target(gate):
    """`from ..acp.types import x` inside codex_crew is the same violation."""
    hits = gate._violations_in_source(
        "src/codex_crew/dashboard/brand_new.py",
...
def test_a_neighbouring_name_is_not_a_violation(gate):
    """`codex_crew.acp_backends` is a leaf that imports no ACP; prefix-matching it
    would flag four consumers for depending on the module that exists to keep
    them off the ACP package."""
    hits = gate._violations_in_source(
        "src/codex_crew/session.py",
...
def test_a_bound_name_that_is_the_forbidden_package_is_a_violation(gate):
    """`from codex_crew import acp` binds the forbidden package by NAME.

    The from-target is `codex_crew`, which matches no forbidden root, so a gate
    ...
    for source in (
        "from codex_crew import acp\n",
        "from codex_crew import providers\n",
        "from codex_crew import acp as _a\n",
        "from .. import acp\n",
    ):
...
def test_a_bound_name_that_is_an_unrelated_sibling_is_clean(gate):
    """The rule must not widen to every `from codex_crew import ...` line."""
    rel = "src/codex_crew/dashboard/probe.py"
    for source in (
        "from codex_crew import session\n",
        "from codex_crew import config\n",
        "from codex_crew import *\n",
    ):
...
def test_every_dynamic_import_spelling_is_a_violation(gate):
    ...
    spellings = {
        ...
        "canonical dunder": "__import__('codex_crew.acp.client')\n",
        "module alias": ("import importlib as _il\n_il.import_module('codex_crew.acp.types')\n"),
        "bare from-import": (
            "from importlib import import_module\nimport_module('codex_crew.acp')\n"
        ),
        "renamed from-import": (
            "from importlib import import_module as _im\n_im('codex_crew.acp')\n"
        ),
        "bound by assignment": (
            "import importlib\n_im = importlib.import_module\n_im('codex_crew.providers')\n"
        ),
        "keyword name": ("import importlib\nimportlib.import_module(name='codex_crew.acp')\n"),
        "dunder fromlist tuple": "__import__('codex_crew', fromlist=('acp',))\n",
        "dunder fromlist list": "__import__('codex_crew', fromlist=['providers'])\n",
        "dunder fromlist set": "__import__('codex_crew', fromlist={'acp'})\n",
...
def test_a_positional_package_resolves_a_relative_dynamic_import(gate):
    ...
    With `package` unread, `import_module(".acp", "codex_crew")` fell through to
    resolving against the CALLING file's package. From a nested consumer that
    lands on `codex_crew.dashboard.acp` -- not a forbidden root -- so the file read
    as clean while importing `codex_crew.acp`.
    """
    rel = "src/codex_crew/dashboard/probe.py"
    flagged = {
        "positional package": ("import importlib\nimportlib.import_module('.acp', 'codex_crew')\n"),
        "positional package, two dots": (
            "import importlib\n" "importlib.import_module('..providers', 'codex_crew.dashboard')\n"
        ),
        "keyword package": (
            "import importlib\nimportlib.import_module('.acp', package='codex_crew')\n"
        ),
    ...
    clean = {
        "positional package on a sibling": (
            "import importlib\nimportlib.import_module('.userns', 'codex_crew.sandbox')\n"
        ),
...
def test_dynamic_import_of_an_unrelated_module_is_clean(gate):
    ...
    rel = "src/codex_crew/dashboard/probe.py"
    clean = {
        "unrelated module": ("import importlib\nimportlib.import_module('codex_crew.session')\n"),
        "unrelated fromlist": "__import__('codex_crew', fromlist=('session',))\n",
        "star fromlist": "__import__('codex_crew', fromlist=('*',))\n",
        "fromlist bare string": "__import__('codex_crew', fromlist='acp')\n",
        "non-literal target": (
...
def test_an_undecidable_fromlist_under_the_boundary_is_a_violation(gate):
    ...
    undecidable = {
        ...
        "dict unpack": "__import__('codex_crew', fromlist={**{'acp': True}})\n",
        "concatenation": "__import__('codex_crew', fromlist=('acp',) + ())\n",
        "repetition": "__import__('codex_crew', fromlist=('acp',) * 1)\n",
        "opaque call": "__import__('codex_crew', fromlist=frozenset({'acp'}))\n",
        "comprehension": "__import__('codex_crew', fromlist=[m for m in ('acp',)])\n",
        "generator": "__import__('codex_crew', fromlist=(m for m in ('acp',)))\n",
        "name reference": "f = ['acp']\n__import__('codex_crew', fromlist=f)\n",
        "element concat": "__import__('codex_crew', fromlist=('a' + 'cp',))\n",
    }
    missed = [
        label
...
def test_the_invariant_does_not_flag_a_sibling_package(gate):
    """`codex_crew.sandbox` is a sibling of the boundary, not an ancestor of it.

    The tree really does call `__import__('codex_crew.sandbox',
    ...
    rel = "src/codex_crew/dashboard/probe.py"
    clean = {
        "real in-tree usage": ("__import__('codex_crew.sandbox', fromlist=['userns_available'])\n"),
        "opaque under a sibling": ("f = compute()\n__import__('codex_crew.sandbox', fromlist=f)\n"),
        "decidable and forbidden-free": ("__import__('codex_crew', fromlist=('session',))\n"),
        "empty": "__import__('codex_crew', fromlist=())\n",
        "non-strings only": "__import__('codex_crew', fromlist=(1, 2))\n",
        "bare string iterates characters": ("__import__('codex_crew', fromlist='acp')\n"),
    }
    flagged = [label for label, source in clean.items() if gate._violations_in_source(rel, source)]
...
def test_an_empty_module_name_is_a_relative_import_not_a_missing_one(gate):
    ...
    under `codex_crew` really imports `codex_crew.acp`.
    """
    rel = "src/codex_crew/dashboard/probe.py"
    relative = {
...
def test_an_empty_name_that_imports_nothing_stays_clean(gate):
    ...
    `codex_crew.dashboard.acp`, which is not the boundary. Flagging either would be
...
def test_growth_in_an_untouched_file_is_not_this_prs_problem(gate):
    ...
    """
    rel = "src/codex_crew/dashboard/untouched.py"
    violations = {rel: {10: "codex_crew.acp", 11: "codex_crew.acp"}}
    baseline = {rel: 1}

    _, grown, added_line_offenders, _ = gate._verdicts(
        violations, baseline, {"src/codex_crew/somewhere/else.py"}, {}
    )
...
def test_every_verdict_is_scope_gated(gate):
    ...
    rels = {
        "new": "src/codex_crew/dashboard/brand_new.py",
        "grown": "src/codex_crew/dashboard/grew.py",
        "shrunk": "src/codex_crew/dashboard/shrank.py",
    }
    violations = {
        rels["new"]: {1: "codex_crew.acp"},
        rels["grown"]: {1: "codex_crew.acp", 2: "codex_crew.acp"},
        rels["shrunk"]: {1: "codex_crew.acp"},
    }
