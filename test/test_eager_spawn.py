...
from codex_crew.dashboard.chat_runner import _eager_spawn, schedule_eager_spawn
from codex_crew.dashboard.state import DashboardState, _ChatSlot
from codex_crew.session import FirstTurnState
...
class TestScheduleEagerSpawn:
    def test_config_loader_parses_eager_spawn(self, tmp_path):
        ...
        The dataclass field alone is not enough: CodexCrewConfig.load() builds
        ...
        def _load(data: dict, name: str):
            ...
            with unittest.mock.patch("codex_crew.config.loader.config_path", return_value=tmp):
                return chat_runner.CodexCrewConfig.load()
    ...
    async def test_noop_when_flag_disabled(self):
        ...
        with patch.object(chat_runner.CodexCrewConfig, "load", _cfg(False)):
    ...
    async def test_noop_when_config_unreadable(self):
        ...
        state = _mock_state(slot)
        with patch.object(
            chat_runner.CodexCrewConfig, "load", MagicMock(side_effect=OSError("boom"))
        ):
            schedule_eager_spawn(state, slot)
        assert slot._eager_spawn_task is None
    ...
    async def test_newer_signal_cancels_older_task(self):
        slot = _ChatSlot("t1")
        state = _mock_state(slot)
        with patch.object(chat_runner.CodexCrewConfig, "load", _cfg(True)):
            schedule_eager_spawn(state, slot)
...
class TestEagerSpawn:
    ...
    async def test_creates_session_with_slot_bindings_and_releases(self, tmp_path):
        ...
        bindings.model = ""
        with (
            patch.object(chat_runner.CodexCrewConfig, "load", _cfg(True)),
    ...
    async def test_resumable_key_refusal_is_clean(self):
        ...
        from codex_crew.session import SpeculativeResumeRefused
...
class TestTtftMetric:
    """codexcrew.chat.first_token.duration — user message → first visible token."""

    def test_emits_histogram_with_attribution_attrs(self):
        rec = MagicMock()
        with patch("codex_crew.metrics.provider.get_recorder", return_value=rec):
        ...
        assert name == "codexcrew.chat.first_token.duration"
    ...
    def test_recorder_failure_is_swallowed(self):
        ...
        with patch("codex_crew.metrics.provider.get_recorder", side_effect=RuntimeError("boom")):
...
class TestSpeculativeGetOrCreate:
    ...
    async def test_speculative_refuses_resumable_key(self, cfg, tmp_path, monkeypatch):
        ...
        never exercised.
        """
        from codex_crew.session import SessionManager, SpeculativeResumeRefused

        sessions_dir = tmp_path / "kiro-sessions"
        sessions_dir.mkdir()
        monkeypatch.setattr("codex_crew.session_map._kiro_sessions_dir", lambda: sessions_dir)
        sid = "prior-sid-1234"
...
class TestProjectSetWiring:
    ...
    async def test_agent_switch_schedules_eager_spawn(self):
        ...
        from codex_crew.dashboard.chat import api_chat_slot_agent
        ...
        with (
            patch(
                "codex_crew.dashboard.chat_handlers._reset_slot_session",
                new=AsyncMock(),
            ),
            patch("codex_crew.dashboard.chat_handlers.save_slot_off_loop", new=AsyncMock()),
            patch("codex_crew.dashboard.chat_handlers.schedule_eager_spawn") as sched,
        ):
            async with TestClient(TestServer(app)) as client:
                resp = await client.post("/api/chat/slots/t1/agent", json={"agent": "codexcrew"})
    ...
    async def test_project_change_schedules_eager_spawn(self, tmp_path):
        ...
        from codex_crew.dashboard.chat import api_chat_slot_project
        ...
        with (
            patch("codex_crew.dashboard.chat_handlers._save_recent_project"),
            patch("codex_crew.dashboard.chat_handlers.schedule_eager_spawn") as sched,
        ):
            async with TestClient(TestServer(app)) as client:
...
class TestSpeculativeResumeHandover:
    ...
    async def test_opt_in_does_not_refuse_resumable_key(self, cfg, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.providers.acp.AcpProvider", object)
    ...
    async def test_resumed_observation_armed_and_consumed_by_real_claimant(
        ...
        from codex_crew.session import SessionManager  # noqa: F811
...
class TestSlotFocusedFrame:
    ...
    async def test_resumable_focus_schedules_resume_prefetch(self):
        from codex_crew.dashboard.ws import _handle_slot_focused
...
class TestSpecResumeFallbackMapGuard:
    ...
    async def test_provider_switch_fallback_never_persists_empty_sid(
        ...
        monkeypatch.setattr("codex_crew.session.detect_provider_switch", lambda *a: True)
        mgr = SessionManager(cfg, provider_factory=factory)
