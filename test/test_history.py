...
from codex_crew import history, history_search
from codex_crew.history import (
...
class TestRecentFromSource:
    ...
    def test_recent_from_source_sorted_by_ts(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.history.datetime", _IncDateTime)
...
class TestSearchSessions:
    ...
    def test_matches_query_with_json_escaped_chars(self, tmp_path):
        ...
        Regression: file paths like ``src\\codex_crew`` are stored in JSONL
        as ``src\\\\codex_crew`` (escaped).  A raw-line substring fast-path
        would miss them; parsing every line ensures the needle is compared
        against the un-escaped ``content`` value.
        """
        log = ConversationLog(base_dir=tmp_path)
        log.append("alpha", "user", r"edited src\codex_crew\history.py today")
        results = log.search_sessions(r"src\codex_crew")
        assert [s["key"] for s in results] == ["alpha"]
    ...
    def test_scan_window_caps_files_scored(self, tmp_path, monkeypatch):
        ...
        """
        monkeypatch.setattr("codex_crew.history._SEARCH_SCAN_WINDOW", 2)
        log = ConversationLog(base_dir=tmp_path)
...
class TestCjkSearch:
    ...
    def test_mixed_script_token_splits_at_script_boundary(self, tmp_path):
        """"codexcrew部署" matches a doc where the ASCII and CJK parts sit apart."""
        self._write_cjk_session(tmp_path, "hit", "codexcrew 的部署流程记录")
        log = ConversationLog(base_dir=tmp_path)

        results = log.search_sessions("codexcrew部署")
...
class TestForgeReferenceSearch:
    ...
    def _corpus(tmp_path) -> ConversationLog:
        ...
        log.append(
            "url_only",
            "assistant",
            "opened https://github.com/mikutellyourworld/Codex-Crew/pull/4411 for app sync",
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "pr4411",
            "pull/4411",
            "https://github.com/mikutellyourworld/Codex-Crew/pull/4411",
            "https://github.com/mikutellyourworld/Codex-Crew/pull/4411/files",
            "mikutellyourworld/Codex-Crew#4411",
            "(#4411)",
            "#4411.",
        ],
        ids=[
    ...
    def test_repo_slug_breaks_a_cross_repo_tie(self, tmp_path):
        ...
        """
        log = ConversationLog(base_dir=tmp_path)
        log.append("named_repo", "assistant", "kirodotdev/codexcrew#4411 needed a rebase")
        log.append("other_repo", "assistant", "looked at #4411 in the vendor tree")

        keys = [
            s["key"]
            for s in log.search_sessions("https://github.com/kirodotdev/codexcrew/pull/4411", 10)
        ]
    ...
    def test_a_sigil_captured_repo_also_ranks(self, tmp_path):
        ...
        keys = [s["key"] for s in log.search_sessions("kirodotdev/codexcrew#4411", 10)]
    ...
    def test_a_repo_name_ending_in_a_digit_still_matches(self, tmp_path):
        ...
        """
        log = ConversationLog(base_dir=tmp_path)
        log.append("digit_repo", "assistant", "see codexcrew2#4411 for the fix")

        assert {s["key"] for s in log.search_sessions("codexcrew2#4411", 10)} == {"digit_repo"}
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "issues/42",
            "merge_requests/12",
            "kirodotdev/codexcrew#4411",
            "codexcrew2#4411",
            "https://github.com/kirodotdev/codexcrew/pull/4411",
...
class TestProviderSearchRefSeam:
    ...
    def test_built_ins_win_and_the_resolver_is_never_asked(self):
        ...
        for query in ("#4411", "pull/4411", "https://github.com/mikutellyourworld/Codex-Crew/pull/4411"):
...
class TestArchive:
    def test_rotate_archives_dropped_lines(self, tmp_path, monkeypatch):
        monkeypatch.setattr("codex_crew.history._SESSION_MAX_BYTES", 100)
        monkeypatch.setattr("codex_crew.history._SESSION_KEEP_LINES", 3)
        log = ConversationLog(base_dir=tmp_path)
        for i in range(20):
    ...
    def test_cleanup_old_archives(self, tmp_path):
        ...
        import codex_crew.history as history_mod
        from codex_crew.history import _cleanup_old_archives
    ...
    def test_archive_empty_lines_noop(self, tmp_path):
        from codex_crew.history import _archive_lines
    ...
    def test_cleanup_throttled_skips_config_load(self, tmp_path, monkeypatch):
        ...
        Config resolution (CodexCrewConfig.load — a disk read + parse) is
    ...
    def test_safe_key_sanitizes_unsafe_chars(self, tmp_path):
        ...
        from codex_crew.history import _archive_lines, _safe_key
    ...
    def test_multiple_rotations_produce_multiple_archives(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.history._SESSION_MAX_BYTES", 200)
        monkeypatch.setattr("codex_crew.history._SESSION_KEEP_LINES", 2)
        log = ConversationLog(base_dir=tmp_path)
        for _ in range(3):
...
class TestConsolidationToolPolicy:
    ...
    kiro scopes the codexcrew-lite session to tools:[] via set_mode, but the
    Claude Code backend skips set_mode and injects the full codexcrew-core/cron
    ...
    async def test_call_llm_rejects_tools(self):
        from codex_crew.llm_helpers import ToolApprovalPolicy
        ...
        with patch("codex_crew.history.stream_and_collect_json", side_effect=_fake_scj):
...
class TestConsolidationDoesNotBlockLoop:
    ...
    def test_save_lessons_caps_oversized_list(self):
        ...
        from codex_crew.vector_memory import _MAX_LESSONS_PER_CONSOLIDATION
...
class TestStopEventContextInjection:
    ...
    def test_context_injection_stop_event(self, tmp_path):
        ...
        from codex_crew.context import _build_stop_event_notes
...
class TestCancelledTurnPreambleInstruction:
    ...
    def test_preamble_forbids_standalone_acknowledgment(self, tmp_path):
        ...
        from codex_crew.context import build_cancelled_turn_preamble
...
class TestAutoSkillHelpers:
    ...
    def test_count_tool_call_messages(self):
        from codex_crew.history import _count_tool_call_messages
    ...
    def test_session_touched_sensitive_true_for_aws(self):
        from codex_crew.history import _session_touched_sensitive
...
class TestAutoSkillSELAudit:
    ...
    async def test_refine_namespace_lock_rejection_emits_sel(self, tmp_path):
        ...
        with patch.object(consolidator, "_call_llm", side_effect=fake_llm):
            with patch("codex_crew.history.sel") as mock_sel:
...
class TestAutoSkillSELAuditCompleteness:
    ...
    async def test_refine_empty_after_redaction_emits_sel(self, tmp_path):
        ...
        from codex_crew.memory import MemoryStore
        from codex_crew.skills import AutoSkillProvenance, SkillsLoader
...
class TestConsolidationPromptJsonShape:
    ...
    def test_new_skill_prompt_shape_quotes_balanced(self):
        ...
        from codex_crew.history import HistoryConsolidator
...
class TestLRUCache:
    ...
    def test_hit_returns_value(self):
        from codex_crew.history import _LRUCache
...
class TestMetadataReadSurvivesATransientSharingViolation:
    ...
    def test_a_persistent_violation_still_reports_absence_but_warns(
        ...
        with caplog.at_level(logging.WARNING, logger="codex_crew.history"):
...
class TestConsolidationValueGuard:
    ...
    def test_value_absent_item_is_logged_and_counted(self, tmp_path, caplog) -> None:
        ...
        with caplog.at_level(logging.INFO, logger="codex_crew.history"):
            c._write_structured_memory(
                {"semantic": [{"key": "project.alpha.status", "confidence": 1.0}]}, "sess-1"
            )
