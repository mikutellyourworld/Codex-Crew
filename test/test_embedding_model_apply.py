...
from codex_crew.embeddings import ReembedProgress, reembed_progress, validate_custom_model_path
...
class TestGatedCandidateLifecycle:
    ...
    def test_candidate_is_built_gated_and_adopts_its_width(self, tmp_path) -> None:
        from codex_crew.embeddings import build_gated_candidate
    ...
    def test_retire_is_terminal_so_a_stale_holder_cannot_resurrect_it(self, tmp_path) -> None:
        ...
        singleton.
        """
        from codex_crew.embeddings import LlamaCppEmbedder
    ...
    def test_adopt_mode_refuses_an_unreadable_width(self, tmp_path, monkeypatch) -> None:
        ...
        import codex_crew.embeddings as emb
...
class TestEnvOverrideRefusal:
    """With CODEXCREW_EMBED_MODEL_PATH set, applying from the dashboard is refused.
    ...
    def test_handler_checks_the_env_var_before_arming_progress(self) -> None:
        ...
        from codex_crew.dashboard.handlers import memory as mem

        src = inspect.getsource(mem.api_memory_embedding_model)
        env_at = src.find("CODEXCREW_EMBED_MODEL_PATH")
    ...
    def test_resolve_takes_path_from_env_but_dim_from_config(self, monkeypatch, tmp_path) -> None:
        ...
        monkeypatch.setenv("CODEXCREW_EMBED_MODEL_PATH", str(model))
        monkeypatch.setattr(
...
class TestKnowledgeIngestSignatureBinding:
    ...
    def test_signature_is_captured_before_the_embed(self) -> None:
        ...
        from codex_crew.knowledge.ingestion import IngestionPipeline
    ...
    def test_the_batch_path_still_binds_sig_explicitly(self) -> None:
        ...
        from codex_crew.knowledge.ingestion import _write_item_embedding
...
class TestNonObjectJsonBody:
    ...
    def test_the_guard_runs_before_the_first_field_read(self) -> None:
        ...
        from codex_crew.dashboard.handlers.memory import api_memory_embedding_model
...
class TestWidthIsRestoredOnRollback:
    ...
    def test_every_rollback_branch_restores_the_width(self) -> None:
        ...
        from codex_crew.dashboard.handlers.memory import _apply_embedding_model
...
class TestRevertToBundledIsGatedToo:
    ...
    def test_bundled_candidate_is_gated_with_a_known_width(self) -> None:
        from codex_crew.embeddings import build_gated_bundled, bundled_embedding_dim
...
class TestConfigWriteHasNoOrphanWindow:
    ...
    async def test_a_slow_lock_still_produces_a_consistent_pair(
        ...
        from codex_crew.dashboard.handlers.memory import _write_embed_model_config

        cfg = tmp_path / "config.json"
        cfg.write_text("{}", encoding="utf-8")
        monkeypatch.setattr(
            "codex_crew.dashboard.handlers.memory.config_path", lambda: cfg, raising=False
        )
...
class TestValidateCustomModelPath:
    ...
    def test_rejects_protected_location(self, tmp_path: Path, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "codex_crew.embeddings.is_sensitive_path", lambda p, base_dir=None: True
        )
...
class TestConfigWrite:
    ...
    async def test_writes_path_into_the_memory_section(self, tmp_path: Path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.dashboard.handlers.memory.config_path", lambda: cfg)
