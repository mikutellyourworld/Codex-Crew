...
def test_subagent_effort_helper(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        classmethod(
            lambda cls: CodexCrewConfig(agent=AgentConfig(role_efforts={"subagent": "low"}))
        ),
    )
...
class TestBackgroundWiring:
    def test_background_model_defaults_auto(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "codex_crew.config.loader.CodexCrewConfig.load",
            classmethod(lambda cls: CodexCrewConfig(agent=AgentConfig())),
        )
...
class TestSubagentWiring:
    ...
    def test_pin_is_used(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "codex_crew.config.loader.CodexCrewConfig.load",
            classmethod(
                lambda cls: CodexCrewConfig(agent=AgentConfig(role_models={"subagent": "haiku-4.5"}))
            ),
        )
