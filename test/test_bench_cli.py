"""``codexcrew bench`` CLI behaviour that a reviewer flagged and a test must pin.
...
from codex_crew.cli_bench import _load_report, bench_cmd
...
# `codexcrew bench compare ~/.aws/credentials x.json` is a reachable invocation.
...
def test_importing_cli_bench_does_not_drag_the_vector_store_into_the_boot_path() -> None:
    ...
    code = textwrap.dedent(
        """
        import json, sys
        import codex_crew.cli_bench  # noqa: F401
        print(json.dumps({
            "vector_memory": "codex_crew.vector_memory" in sys.modules,
            "bench": "codex_crew.eval.bench" in sys.modules,
