...
def owned_home(tmp_path, monkeypatch):
    """A Codex Crew data home under *tmp_path*, so the walk has a trust anchor."""
...
def test_secret_write_allows_a_symlinked_data_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CODEXCREW_HOME", str(linked_home))
...
def test_outside_owned_roots_still_refuses_the_planted_chain(tmp_path, monkeypatch):
    """A destination outside every Codex Crew root keeps a best-effort check.
    ...
    """
    monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path / "datahome"))
...
def test_relocated_data_home_under_the_kiro_home_still_writes(tmp_path, monkeypatch):
    ...
    The default layout nests two owned roots: the data home ``~/.codex-crew``
