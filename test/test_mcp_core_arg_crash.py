"""Regression tests: every internally-validated codexcrew-core MCP tool must be
...
the whole codexcrew-core server), but the workflow_* tools that call
...
terminating every codexcrew-core tool for the session (and, when the backend is
    ...
    def test_all_internally_validated_tools_are_registered(self):
        ...
        for tool, _ in _PREVIOUSLY_UNGATED:
            assert tool in MCP_CORE_SCHEMAS, (
                ...
                f"— a bad arg would crash the codexcrew-core stdio loop"
            )
