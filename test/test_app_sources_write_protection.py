...
``~/.codex-crew/app-sources/{name}`` is the persistent tree every installed app
...
_PREFIXES = ("~/.codex-crew", "~/.codexcrew")
...
class TestAppSourcesWriteProtection:
    ...
    def test_sibling_data_home_paths_unaffected(self) -> None:
        ...
        assert is_sensitive_write_path("~/.codex-crew/app-sources-backup/x.py") is False
        # An installed app's DATA dir is a different tree and stays writable —
        # apps persist state there through the agent's own tools.
        assert is_sensitive_write_path("~/.codex-crew/apps/notes/data/notes.json") is False
