"""Tests for /codexcrew restart slash command."""
...
from codex_crew.config.loader import ACTIVATION_ALWAYS, ChannelConfig, CodexCrewConfig
from codex_crew.slack.events import SeenCache, _route_message
from codex_crew.slack.handler import set_owner_id


@pytest.fixture(autouse=True)
def _mock_sel():
    _sel = MagicMock()
    with patch("codex_crew.slack.events.sel", return_value=_sel):
        yield _sel
...
class TestRestartSlashCommand:
    ...
    async def test_non_owner_denied(self):
        from codex_crew.slack.events import _handle_restart
    ...
    async def test_owner_exits_even_if_session_close_hangs(self, _mock_sel):
        ...
        with (
            patch.dict("os.environ", {"INVOCATION_ID": "abc"}),
            patch("os._exit") as mock_exit,
            patch("codex_crew.slack.events.asyncio.wait_for", _timeout_first),
        ):
            await _handle_restart(orch, "U_OWNER", "", respond)
    ...
    async def test_dashboard_state_saved_before_restart(self, _mock_sel):
        ...
        with (
            patch.dict("os.environ", {"INVOCATION_ID": "abc"}),
            patch("os._exit") as mock_exit,
            patch("codex_crew.dashboard.chat.save_all_slots_to_history") as mock_save,
        ):
            await _handle_restart(orch, "U_OWNER", "", respond)
...
class TestRestartBangAlias:
    ...
    async def test_bang_restart_routes_to_handler(self):
        ...
        with (
            patch("codex_crew.slack.events.handle_message", new_callable=AsyncMock) as mock_hm,
            patch("codex_crew.slack.events._handle_restart", new_callable=AsyncMock) as mock_restart,
    ...
    async def test_bang_restart_in_alias_map(self):
        ...
        from codex_crew.slack.handler import _BANG_TO_SLASH

        assert _BANG_TO_SLASH.get("!restart") == "/codexcrew restart"
