...
from codex_crew.slack.handler import _list_all_agent_names, _resolve_cc_agent_name
...
class TestListAllAgentNames:
    ...
    def test_empty_dir(self, tmp_path: Path) -> None:
        ...
        with unittest.mock.patch("codex_crew.slack.handler.Path.home", return_value=tmp_path):
    ...
    def test_codexcrew_lite_excluded(self, tmp_path: Path) -> None:
        """The codexcrew-lite agent is excluded from listings."""
        cc_dir = tmp_path / "cc-plugins"
        _write_agent_md(cc_dir, "core", "codexcrew-lite")
        ...
        assert "codexcrew-lite" not in result
        assert "useful-agent" in result

    def test_codexcrew_lite_excluded_from_kiro_agents(self, tmp_path: Path) -> None:
        """A ~/.kiro/agents/codexcrew-lite.json must ALSO be hidden.

        Regression guard for the review-bot finding: the codexcrew-lite filter was
        only applied to cc-plugins agents, so a kiro-side codexcrew-lite.json
        ...
        with unittest.mock.patch("codex_crew.slack.handler.Path.home", return_value=tmp_path):
            kiro_agents = tmp_path / ".kiro" / "agents"
            kiro_agents.mkdir(parents=True)
            (kiro_agents / "codexcrew-lite.json").write_text(
                '{"name": "codexcrew-lite"}', encoding="utf-8"
            )
