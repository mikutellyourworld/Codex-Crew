...
from codex_crew.acp.runtime import AcpWorkspaceBindingError
from codex_crew.acp.types import ACP_BACKEND_KAS, ACP_BACKEND_KIRO, AcpPromptStats
...
def _raw_sid(mgr, key: str):
    ...
    """
    from codex_crew.session_map import canonical_key
...
class TestWorkflowPoolStateless:
    ...
    def test_wf_pool_prefix_is_stateless(self):
        from codex_crew.session import _STATELESS_PREFIXES
...
class TestHeartbeatStateless:
    ...
    async def test_heartbeat_key_skips_resume_lookup(self, cfg):
        ...
        from codex_crew.session import HEARTBEAT_KEY
...
class TestStopTurn:
    ...
    async def test_eager_respawn_failure_logged(self, cfg, caplog):
        ...
        with patch.object(
            ...
            with caplog.at_level(logging.DEBUG, logger="codex_crew.session"):
...
class TestCompactCallback:
    ...
    async def test_set_compact_callback_warns_on_replace(self, cfg, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="codex_crew.session"):
            mgr.set_compact_callback(AsyncMock())
    ...
    async def test_compact_session_defers_when_turn_never_drains(self, cfg, caplog, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.session.COMPACT_WAIT_TIMEOUT_SECS", 0.1)
        mgr = SessionManager(cfg, provider_factory=_mock_provider_factory())
    ...
    async def test_compact_session_callback_exception_is_logged(self, cfg, caplog):
        ...
        with caplog.at_level(logging.ERROR, logger="codex_crew.session"):
...
class TestResetWithPid:
    ...
    async def test_reset_with_pid_survives_shutdown_force_kills(self, cfg):
        ...
        with (
            ...
            patch("codex_crew.acp.client._get_start_time", return_value=None),
        ):
            await mgr.reset("k1")
    ...
    async def test_reset_child_sweep(self, cfg):
        ...
        with (
            patch("os.kill", side_effect=ProcessLookupError),
            patch("codex_crew.acp.client._get_child_pids", return_value=[333]),
            patch("codex_crew.acp.client._get_start_time", return_value=3000),
            patch("codex_crew.acp.client._read_basename", return_value=b"node"),
            patch("codex_crew.acp.client._kill_escaped_children") as mock_sweep,
        ):
            await mgr.reset("k1")
...
class TestCheckContextUsage:
    ...
    async def test_warning_fires_one_margin_below_the_threshold(self, cfg, caplog):
        ...
        number moves.
        """
        from codex_crew.config.loader import CONTEXT_WARN_MARGIN_PCT
        ...
        with caplog.at_level(logging.WARNING, logger="codex_crew.session"):
            mgr.check_context_usage("k1", provider)
        assert any(
            f"{warn_at:.0f}%" in r.message for r in caplog.records if r.name == "codex_crew.session"
        )
        await mgr.close_all()
    ...
    async def test_no_warning_just_below_the_margin(self, cfg, caplog):
        ...
        assert not [
            ...
            if r.levelno >= logging.WARNING and r.name == "codex_crew.session"
        ]
        await mgr.close_all()
...
class TestContextInfo:
    ...
    async def test_context_info_with_acp_provider(self, cfg):
        ...
        from codex_crew.session import _Session
        ...
        mock_provider.client._agent = "codexcrew"
    ...
    def test_resolve_agent_model_from_file(self, cfg, tmp_path):
        ...
        with patch("codex_crew.agent.KIRO_AGENTS_DIR", tmp_path):
    ...
    def test_resolve_agent_model_refuses_a_link_to_a_sensitive_target(self, tmp_path, monkeypatch):
        ...
        from codex_crew import agent_discovery
        ...
        with patch("codex_crew.agent.KIRO_AGENTS_DIR", agents):
...
class TestWarmPoolInternals:
    ...
    async def test_claim_from_pool_matching_agent(self, cfg):
        mgr = SessionManager(cfg, provider_factory=_mock_provider_factory())
        mgr._pool_agent = "codexcrew"
        mock_p = AsyncMock()
        mgr._warm_pool.put_nowait((mock_p, 100.0))
        result = mgr._claim_from_pool("codexcrew")
...
class TestCleanupLoop:
    ...
    ``~/.codexcrew`` PID file) made each test take ~10-20s of wall-clock and
    ...
    async def test_cleanup_loop_calls_expire_idle(self, cfg):
        ...
        with (
            ...
            patch("codex_crew.session._cleanup_orphaned_mcp_servers", return_value=0),
            patch("codex_crew.session.cleanup_stale_sandbox_profiles", return_value=0),
            patch("codex_crew.session._collect_active_pids", return_value=({}, True)),
            patch("codex_crew.session._periodic_pid_sweep", return_value=([], [])),
            patch("codex_crew.session._kill_confirmed_and_writeback", return_value=0),
            patch("codex_crew.session.cleanup_orphaned_session_roots", return_value=0),
            patch("codex_crew.session.find_orphan_mcp_candidates", return_value=[]),
            patch("codex_crew.session.shutdown_event") as mock_event,
        ):
    ...
    async def test_cleanup_loop_shutdown_signal(self, cfg):
        ...
        with patch("codex_crew.session.shutdown_event") as mock_event:
    ...
    async def test_cleanup_loop_runs_sandbox_sweep_via_executor(self, cfg, caplog):
        ...
        with (
            ...
            patch("codex_crew.session._cleanup_orphaned_mcp_servers", return_value=0),
            patch(
                "codex_crew.session.cleanup_stale_sandbox_profiles", side_effect=_fake_sweep
            ) as mock_sweep,
            patch("codex_crew.session._collect_active_pids", return_value=({}, True)),
            patch("codex_crew.session._periodic_pid_sweep", return_value=([], [])),
            patch("codex_crew.session._kill_confirmed_and_writeback", return_value=0),
            ...
            mock_event.wait = AsyncMock(side_effect=asyncio.TimeoutError)
            with caplog.at_level(logging.INFO, logger="codex_crew.session"):
                await mgr._cleanup_loop()
...
class TestClaudeBackendCompaction:
    ...
    async def test_compact_session_claude_runs_in_place(self, cfg):
        ...
        with patch("codex_crew.session._is_claude_backend", return_value=True):
            await mgr._compact_session("k1", 92.0)
    ...
    async def test_compact_session_claude_failure_keeps_session(self, cfg, caplog):
        ...
        with (
            patch("codex_crew.session._is_claude_backend", return_value=True),
            caplog.at_level(logging.ERROR, logger="codex_crew.session"),
        ):
            await mgr._compact_session("k1", 92.0)
    ...
    async def test_get_or_create_reuses_healthy_replacement_during_recycle(self, cfg):
        ...
        from codex_crew.session import FirstTurnState, _Session
...
class TestKiroInPlaceCompaction:
    ...
    async def test_inplace_midstream_terminal_status_skips_async_wait(self, cfg):
        ...
        from codex_crew.acp.types import EVENT_COMPACTION_STATUS, AcpEvent
...
class TestCompactTimeout:
    ...
    async def test_compact_timeout_sets_cooldown_and_fires_failure(self, cfg):
        ...
        with (
            ...
            patch("codex_crew.session.COMPACT_WAIT_TIMEOUT_SECS", 0.05),
        ):
            await mgr._compact_session("k1", 92.0)
...
class TestGetOrCreatePoolClaim:
    ...
    async def test_claims_from_pool_on_new_session(self, cfg):
        ...
        mock_pooled.client._agent = "codexcrew"
        ...
        provider, is_new, _ = await mgr.get_or_create("dashboard:slot1", agent="codexcrew")
...
class TestGetBgSessionRecycle:
    ...
    async def test_recycles_stale_idle_runtime(self, cfg):
        ...
        with patch("codex_crew.acp.runtime.AcpRuntime", side_effect=[rt2]):
    ...
    async def test_does_not_recycle_stale_runtime_with_active_sessions(self, cfg):
        ...
        with patch(
            "codex_crew.acp.runtime.AcpRuntime",
...
class TestGetBgSessionBackendSwitch:
    ...
    async def test_runtime_spawns_under_the_configured_backend(self, cfg, backend):
        ...
        with patch("codex_crew.acp.runtime.AcpRuntime", return_value=rt) as ctor:
            result = await mgr.get_bg_session()
    ...
    async def test_recycles_an_idle_runtime_spawned_under_a_different_backend(self, cfg):
        ...
        with patch("codex_crew.acp.runtime.AcpRuntime", return_value=rt2) as ctor:
            result = await mgr.get_bg_session()
    ...
    async def test_a_parked_runtime_is_reaped_once_its_handles_drain(self, cfg):
        ...
        rt = self._fresh_runtime()
        with patch("codex_crew.acp.runtime.AcpRuntime", return_value=rt):
...
class TestRetireStaleBackendBgRuntime:
    ...
    async def test_refresh_defaults_triggers_the_retirement(self, cfg):
        ...
        with (
            ...
            patch("codex_crew.session.build_provider_factory", return_value=MagicMock()),
            patch("codex_crew.session.CodexCrewConfig.load", return_value=cfg),
        ):
...
class TestOpenTaskSession:
    ...
    async def test_run_shares_one_runtime_across_sessions(self, cfg):
        ...
        p1, new1, res1 = await mgr.open_task_session(
            parent, "taskrunner:run1:decompose", agent="codexcrew"
        )
        p2, new2, res2 = await mgr.open_task_session(
            parent, "taskrunner:run1:task0", agent="codexcrew"
        )
    ...
    async def test_open_task_session_registers_under_key(self, cfg):
        ...
        provider, is_new, _resumed = await mgr.open_task_session(parent, key, agent="codexcrew")
...
class TestLoadRecoveryHistoryReplay:
    ...
    the new slot for CodexCrew conversation_log replay on the first prompt so the
...
class TestIneffectiveCompactionCooldown:
    ...
    async def test_claude_ineffective_sets_cooldown(self, cfg, caplog):
        ...
        with (
            patch("codex_crew.session._is_claude_backend", return_value=True),
            caplog.at_level(logging.WARNING, logger="codex_crew.session"),
        ):
            await mgr._compact_session("k1", 92.0)
    ...
    async def test_repeated_ineffective_keeps_damping_until_breaker_resets(self, cfg):
        ...
        from codex_crew.session import _CIRCUIT_BREAKER_THRESHOLD
