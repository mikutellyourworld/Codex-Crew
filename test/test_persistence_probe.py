...
from codex_crew.platform_compat import probe_file_persistence
...
class TestProbeFilePersistence:
    ...
    def test_replace_failure_reports_replace_step(
        ...
        monkeypatch.setattr("codex_crew.atomic_write.replace_with_retry", _no_replace)
        msg = probe_file_persistence(tmp_path)
        assert msg is not None
    ...
    def test_probe_uses_the_production_replace_primitive(
        ...
        from codex_crew import atomic_write
        ...
        monkeypatch.setattr("codex_crew.atomic_write.replace_with_retry", _spy)
        assert probe_file_persistence(tmp_path) is None
        assert len(calls) == 1
...
class TestGatewayPreflightWiring:
    ...
    def _orchestrator(self):  # type: ignore[no-untyped-def]
        ...
        return GatewayOrchestrator(CodexCrewConfig(), no_dashboard=True, no_crons=True)
