...
def sandbox(fake_home, monkeypatch):
    ...
    monkeypatch.setattr(mcp_mod, "_CODEXCREW_MCP_JSON", fake_home / "codexcrew.mcp.json")
    ...
    return SimpleNamespace(
        home=fake_home,
        codexcrew_json=fake_home / "codexcrew.mcp.json",
        rebuild=rebuild,
        mcp_mod=mcp_mod,
    )
...
def fake_sel(monkeypatch):
    ...
    from codex_crew.dashboard.handlers import mcp_custom as mod
...
def _written(sandbox) -> dict:
    ...
    if not sandbox.codexcrew_json.exists():
        return {}
    return json.loads(sandbox.codexcrew_json.read_text(encoding="utf-8")).get("mcpServers", {})
...
class TestCustomAdd:
    ...
    async def test_store_is_owner_only_after_credential_write(self, sandbox, fake_sel):
        ...
        try:
            resp = await client.post("/api/mcp/custom", json={"servers": {"remote": spec}})
            assert resp.status == 200
            mode = stat.S_IMODE(sandbox.codexcrew_json.stat().st_mode)
    ...
    async def test_collision_409_lists_conflicts_and_writes_nothing(self, sandbox, fake_sel):
        sandbox.codexcrew_json.write_text(json.dumps({"mcpServers": {"weather": dict(_STDIO)}}))
        client = await _client()
        try:
            resp = await client.post(
...
class TestCustomUpdate:
    async def test_replaces_spec_and_preserves_disabled_state(self, sandbox, fake_sel):
        ...
        sandbox.codexcrew_json.write_text(json.dumps({"mcpServers": {"weather": entry}}))
        client = await _client()
        try:
    ...
    async def test_put_can_author_headers_when_none_stored(self, sandbox, fake_sel):
        ...
        sandbox.codexcrew_json.write_text(json.dumps({"mcpServers": {"remote": entry}}))
        client = await _client()
        try:
    ...
    async def test_put_rejects_redaction_marker_as_fresh_header_value(self, sandbox, fake_sel):
        ...
        from codex_crew.mcp_discovery import MCP_REDACTED_HEADER_VALUE
...
class TestCustomGet:
    ...
    async def test_oauth_hints_round_trip_without_echoing_authorization(
        ...
        sandbox.codexcrew_json.write_text(json.dumps({"mcpServers": {"github": entry}}))
        client = await _client()
        try:
...
class TestMalformedConfigNeverClobbered:
    ...
    async def test_add_refuses_to_write_over_malformed_file(self, sandbox, fake_sel):
        sandbox.codexcrew_json.write_text("{not json")
        client = await _client()
        try:
            resp = await client.post(
            ...
            assert sandbox.codexcrew_json.read_text(encoding="utf-8") == "{not json"
        finally:
            await client.close()

    async def test_add_refuses_non_object_mcpservers(self, sandbox, fake_sel):
        sandbox.codexcrew_json.write_text(json.dumps({"mcpServers": ["broken"]}))
        client = await _client()
        try:
            resp = await client.post(
                "/api/mcp/custom", json={"servers": {"weather": dict(_STDIO)}}
            ...
            assert resp.status == 500
            assert json.loads(sandbox.codexcrew_json.read_text(encoding="utf-8"))["mcpServers"] == ["broken"]
        finally:
            await client.close()

    async def test_add_preserves_existing_entries(self, sandbox, fake_sel):
        sandbox.codexcrew_json.write_text(
        ...
        try:
            ...
            assert resp.status == 200
            servers = json.loads(sandbox.codexcrew_json.read_text(encoding="utf-8"))["mcpServers"]
...
class TestCarriedKeyRoundTrip:
    ...
    def _seed(self, sandbox) -> None:
        sandbox.codexcrew_json.write_text(json.dumps({"mcpServers": {"weather": self._ENTRY}}))
    ...
    async def test_unmodified_get_payload_saves_and_preserves_keys(self, sandbox, fake_sel):
        ...
        try:
            ...
            assert resp.status == 200
            data = json.loads(sandbox.codexcrew_json.read_text(encoding="utf-8"))
    ...
    async def test_the_authorship_marker_is_the_one_carried_key_not_preserved(
        ...
        exception: it records that Codex Crew wrote an entry into a file it does
        ...
        """
        from codex_crew.mcp_provenance import MARKER_KEY
        ...
        try:
            ...
            resp = await client.put("/api/mcp/custom/weather", json={"spec": got["spec"]})
            assert resp.status == 200
            entry = json.loads(sandbox.codexcrew_json.read_text(encoding="utf-8"))["mcpServers"][
                "weather"
            ]
    ...
    async def test_removing_carried_key_still_preserves_it(self, sandbox, fake_sel):
        ...
        try:
            ...
            assert resp.status == 200
            entry = json.loads(sandbox.codexcrew_json.read_text(encoding="utf-8"))["mcpServers"]["weather"]
    ...
    async def test_clearing_oauth_hints_defeats_a_carried_wire_sibling(self, sandbox, fake_sel):
        ...
        """
        from codex_crew.mcp_utils import kiro_entry_client_id, kiro_entry_scopes
    ...
    async def test_clearing_oauth_hints_also_drops_the_nested_wire_sibling(
        ...
        from codex_crew.mcp_utils import kiro_entry_scopes
    ...
    async def test_editing_a_nested_client_id_persists(self, sandbox, fake_sel):
        ...
        from codex_crew.mcp_utils import kiro_entry_client_id
    ...
    async def test_adding_a_non_hint_oauth_subkey_is_refused_not_dropped(self, sandbox, fake_sel):
        ...
        """
        url = "https://mcp.acme.com/mcp"
        sandbox.codexcrew_json.write_text(json.dumps({"mcpServers": {"weather": {"url": url}}}))
        client = await _client()
        try:
            resp = await client.put(
                "/api/mcp/custom/weather",
...
class TestServersListSurfacesCustomAdds:
    ...
    must appear in ``GET /api/mcp`` as a disabled, CodexCrew-managed row.
    ...
    async def test_disabled_custom_add_appears_in_servers_list(
        ...
        # Route discovery at the sandboxed CodexCrew scope file only.
        monkeypatch.setattr(
            "codex_crew.mcp_discovery._MCP_JSON_PATHS", (sandbox.codexcrew_json,)
        )
        ...
        app["state"] = state
        from codex_crew.dashboard.handlers import mcp_custom as custom_mod
        ...
        try:
            ...
            assert row["codexcrewManaged"] is True
        finally:
            await client.close()
