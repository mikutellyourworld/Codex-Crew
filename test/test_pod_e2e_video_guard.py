...
_SKILL = (
    ...
    / "src/codex_crew/apps/builtins/dev_fleet/skills/pod-e2e"
)
...
class _FakePage:
    ...
    def text_content(self, _sel):
        return "CodexCrew dashboard shell rendered fine"
...
def test_verdict_is_truncated_on_every_invocation():
    ...
    CODEXCREW_PW_PY). So it must run unconditionally, right after mkdir.
    """
    ...
    launch = _index('CODEXCREW_POD_TOKEN="$TOKEN" "${PW_CMD[@]}"')
...
def test_missing_playwright_interpreter_fails_the_run():
    ...
    The FE phase used to `warn` when CODEXCREW_PW_PY was unset, so the summary
    ...
    assert "CODEXCREW_PW_PY" in branch
...
def test_missing_playwright_message_names_the_pinned_version_and_the_opt_out():
    ...
    assert "export CODEXCREW_PW_PY=" in body
