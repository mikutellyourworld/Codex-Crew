"""Unit tests for the ``codexcrew agent`` CLI subcommand group.
...
class TestAgentList:
    """Test ``codexcrew agent list`` output format."""
    ...
    def test_list_output_format(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        with (
            ...
            unittest.mock.patch("sys.argv", ["codexcrew", "agent", "list"]),
        ):
            main()
...
class TestAgentCreate:
    ...
    def test_create_with_defaults(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        with (
            ...
            unittest.mock.patch(
                "sys.argv",
                ["codexcrew", "agent", "create", "--name", "research"],
            ),
        ):
            main()
        ...
        assert saved["agents"]["research"]["kiro_agent"] == "codexcrew"
    ...
    def test_create_duplicate_exits_nonzero(
        ...
        with (
            ...
            unittest.mock.patch(
                "sys.argv",
                ["codexcrew", "agent", "create", "--name", "default"],
            ),
...
class TestAgentUpdate:
    ...
    def test_update_nonexistent_exits_nonzero(
        ...
        with (
            ...
            unittest.mock.patch(
                "sys.argv",
                ["codexcrew", "agent", "update", "nonexistent", "--kiro-agent", "x"],
            ),
            pytest.raises(SystemExit) as exc_info,
        ):
            main()
...
class TestAgentDelete:
    ...
    def test_delete_default_agent_exits_nonzero(
        ...
        with (
            ...
            unittest.mock.patch(
                "sys.argv",
                ["codexcrew", "agent", "delete", "default"],
            ),
            pytest.raises(SystemExit) as exc_info,
        ):
            main()
...
class TestAgentResetModel:
    """``codexcrew agent reset-model`` -- the explicit way back to the default.
    ...
    def _isolated_spec(self, tmp_path: Path, monkeypatch, model: str | None) -> Path:
        ...
        body: dict = {"name": "codexcrew", "tools": ["fs_read"]}
        if model is not None:
            body["model"] = model
        spec = agents_dir / "codexcrew.json"
    ...
    def test_reset_clears_the_pin_and_keeps_the_rest_of_the_spec(
        ...
        with unittest.mock.patch("sys.argv", ["codexcrew", "agent", "reset-model"]):
            main()
    ...
    def test_reset_exits_nonzero_for_an_unknown_agent(self, tmp_path: Path, monkeypatch) -> None:
        ...
        with unittest.mock.patch(
            "sys.argv", ["codexcrew", "agent", "reset-model", "--agent", "nope"]
        ):
