...
from codex_crew.config.loader import CodexCrewConfig, SttConfig
...
def _make_app() -> web.Application:
    from codex_crew.dashboard import stt_stream
...
def _cfg(**kwargs) -> CodexCrewConfig:
    stt = SttConfig(
    ...
    return CodexCrewConfig(stt=stt)
...
class TestGuards:
    ...
    async def test_rejects_when_streaming_disabled(self, monkeypatch):
        monkeypatch.setattr(
            "codex_crew.dashboard.stt_stream.CodexCrewConfig.load",
            classmethod(lambda cls: _cfg(streaming=False)),
        )
        monkeypatch.setattr("codex_crew.dashboard.stt_stream.check_origin", lambda r, require: True)
    ...
    async def test_rejects_a_provider_that_cannot_stream(self, monkeypatch):
        ...
        ``CodexCrewConfig.load`` degrades it to ``local``, which is exactly why the
    ...
    async def test_rejects_bad_origin(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.stt_stream.check_origin", lambda r, require: False)
        async with TestClient(TestServer(_make_app())) as client:
            resp = await client.get("/api/ws/stt")
    ...
    async def test_bad_origin_emits_sel_rejection_audit(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.stt_stream.check_origin", lambda r, require: False)
        fake_sel = MagicMock()
        monkeypatch.setattr("codex_crew.dashboard.stt_stream.sel", lambda: fake_sel)
        async with TestClient(TestServer(_make_app())) as client:
            resp = await client.get("/api/ws/stt")
            assert resp.status == 403
    ...
    async def test_rejects_when_concurrent_cap_reached(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.stt_stream.check_origin", lambda r, require: True)
        monkeypatch.setattr("codex_crew.dashboard.stt_stream._MAX_CONCURRENT_SESSIONS", 1)
        monkeypatch.setattr("codex_crew.dashboard.stt_stream._active_sessions", 1)
        async with TestClient(TestServer(_make_app())) as client:
            resp = await client.get("/api/ws/stt")
            assert resp.status == 503
...
class TestAppleStreamingSession:
    ...
    def _install(self, monkeypatch, *, session=None, start_error="", feed_ok=True, avail=None):
        ...
        *avail* is what the double reports from ``availability()``, which the error
        ...
        """
        from codex_crew import apple_speech as real_apple_speech
        ...
        # BOTH, deliberately. `_run_apple_session` does `from codex_crew import
        # apple_speech`, which resolves the ATTRIBUTE on the already-imported
        # `codex_crew` package rather than consulting sys.modules — so patching
        ...
        monkeypatch.setitem(sys.modules, "codex_crew.apple_speech", fake_module)
        monkeypatch.setattr("codex_crew.apple_speech", fake_module, raising=False)
        return events, fed
    ...
    async def test_duration_cap_fires_for_a_client_that_sends_nothing(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.stt_stream._MAX_STREAM_DURATION_SECS", 0.05)
        async with TestClient(TestServer(_make_app())) as client:
    ...
    async def test_cap_teardown_is_audited_as_a_timeout(self, monkeypatch):
        ...
        outcomes: list[str] = []
        monkeypatch.setattr(
            "codex_crew.dashboard.stt_stream._emit_end_audit",
...
class TestStreamLifecycle:
    ...
    def _install_stubs(self, monkeypatch, *, fail_start=False):
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.stt_stream.TranscribeStreamingClient",
            lambda **kw: client,
        )
    ...
    async def test_import_error_emits_sel_end_audit(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.stt_stream.check_origin", lambda r, require: True)
        monkeypatch.setattr("codex_crew.dashboard.stt_stream.TranscribeStreamingClient", None)
        calls: list[dict] = []
        fake_sel = MagicMock()
        fake_sel.log_api_access = lambda **kw: calls.append(kw)
        monkeypatch.setattr("codex_crew.dashboard.stt_stream.sel", lambda: fake_sel)
    ...
    async def test_consent_refusal_reaches_the_client_and_is_audited(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.stt_stream.aws_consent.authorize", _refuse, raising=True
        )
        calls: list[dict] = []
        fake_sel = MagicMock()
        fake_sel.log_api_access = lambda **kw: calls.append(kw)
...
class TestConfigPutRoundTrip:
    ...
    async def test_put_persists_streaming(self, tmp_path, monkeypatch):
        ...
        """
        # CODEXCREW_HOME redirects both config_dir() and config_path() in a
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            # Assert CodexCrewConfig.load() correctly deserializes — guards
...
class TestSttLanguageCodes:
    ...
    def test_korean_is_offered(self):
        ...
        """
        from codex_crew.dashboard.handlers.core import _STT_LANGUAGE_CODES
...
class TestDefensiveGuards:
    ...
    async def test_guard_audit_sel_failure_preserves_status_code(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.stt_stream.sel", _raising_sel)
        async with TestClient(TestServer(_make_app())) as client:
            resp = await client.get("/api/ws/stt")
