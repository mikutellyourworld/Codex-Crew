...
* Pure-function unit tests for :mod:`codex_crew.mcp_gateway.apps`
...
Interception is gated by ``CODEXCREW_MCP_APPS`` and MUST be a no-op when off.
"""
...
from codex_crew.mcp_apps_render import find_marker
from codex_crew.mcp_caller import CallerContext
from codex_crew.mcp_gateway import apps
...
class TestAppendMarker:
    def test_prepends_to_first_text_item(self):
        ...
        assert out["content"][0]["text"] == "[codexcrew-mcp-app:abc123] hello"
    ...
    def test_skips_non_text_finds_first_text(self):
        ...
        assert out["content"][1]["text"] == "[codexcrew-mcp-app:id9] caption"
    ...
    def test_no_text_item_prepends_new_item(self):
        ...
        assert out["content"][0] == {"type": "text", "text": "[codexcrew-mcp-app:zz]"}
        assert out["content"][1] == {"type": "image", "data": "..."}

    def test_empty_content_appends_new_item(self):
        out = append_marker({"content": []}, "q")
        assert out["content"] == [{"type": "text", "text": "[codexcrew-mcp-app:q]"}]

    def test_missing_content_key_appends_new_item(self):
        out = append_marker({}, "m")
        assert out["content"] == [{"type": "text", "text": "[codexcrew-mcp-app:m]"}]
...
class TestSpoolDirAgreement:
    def test_writer_and_reader_agree_under_codexcrew_home(self, tmp_path, monkeypatch):
        """Regression (found in live pod testing): the gateway writer once
        hardcoded ``Path.home()`` while the dashboard reader used
        ``config_dir()`` — under CODEXCREW_HOME (pods) the writer spooled into
        the LIVE plane's home and the reader found nothing. Both sides MUST
        resolve identically with only CODEXCREW_HOME set (no spool override)."""
        from codex_crew import mcp_apps_render
        monkeypatch.delenv(apps.SPOOL_ENV, raising=False)
        monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path / "isolated-home"))
...
async def test_unreadable_visibility_drop_warns_declared_drop_informs(apps_flag_on, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="codex_crew.mcp_gateway.backend"):
        assert await backend._maybe_intercept_ui_result(_listing_pending("s1"), msg) is False
...
async def test_visibility_withhold_is_sel_audited(apps_flag_on, monkeypatch):
    ...
    monkeypatch.setattr(
        "codex_crew.mcp_gateway.backend.SecurityEventLog", lambda: _FakeSel()
    )
    backend = _make_backend()
...
async def test_sel_failure_does_not_break_the_filter(apps_flag_on, monkeypatch):
    ...
    monkeypatch.setattr(
        "codex_crew.mcp_gateway.backend.SecurityEventLog", lambda: _ExplodingSel()
    )
    backend = _make_backend()
    msg = _listing_msg()
    assert await backend._maybe_intercept_ui_result(_listing_pending("s1"), msg) is False
...
async def test_interception_end_to_end(apps_flag_on, spool_tmp):
    ...
    assert text.startswith("[codexcrew-mcp-app:")
...
async def test_resources_read_timeout_delivers_original(apps_flag_on, spool_tmp, monkeypatch):
    ...
    monkeypatch.setattr(
        "codex_crew.mcp_gateway.backend._APPS_RESOURCE_READ_TIMEOUT_SECS", 0.05
    )
    backend = _make_backend()
    inbox = await backend.attach_stub("s1")
    await backend.forward_from_stub(
...
class TestInterceptDecision:
    ...
    def _pending(self, method: str = "tools/call", tool: str = "draw"):
        from codex_crew.mcp_gateway.backend import _PendingRequest
        return _PendingRequest(
    ...
    async def test_is_error_result_never_intercepts_either_form(self, spool_tmp, monkeypatch):
        ...
        from codex_crew.mcp_gateway.backend import MCP_APPS_ENV_FLAG
        monkeypatch.setenv(MCP_APPS_ENV_FLAG, "1")
        backend = _make_backend()
    ...
    async def test_app_call_forward_never_reintercepted(self, spool_tmp, monkeypatch):
        ...
        from codex_crew.mcp_gateway.backend import MCP_APPS_ENV_FLAG, _PendingRequest
        monkeypatch.setenv(MCP_APPS_ENV_FLAG, "1")
        backend = _make_backend()
        backend._apps_declared_uris = {"draw": "ui://fake/app.html"}
        pending = _PendingRequest(
