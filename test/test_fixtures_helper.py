"""Tests for ``codex_crew.testing.fixtures``.
...
def test_seeded_home_restores_previous_env(monkeypatch: pytest.MonkeyPatch) -> None:
    """Exiting the context manager restores the prior CODEXCREW_HOME."""
    monkeypatch.setenv("CODEXCREW_HOME", "/tmp/sentinel-previous")
    with seeded_home("empty") as home:
        # Inside: env points at the tempdir, not the sentinel.
        assert os.environ["CODEXCREW_HOME"] == str(home)
    # After: sentinel is back.
    assert os.environ["CODEXCREW_HOME"] == "/tmp/sentinel-previous"


def test_seeded_home_unsets_env_when_not_previously_set(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Exiting unsets CODEXCREW_HOME when it wasn't set on entry."""
    monkeypatch.delenv("CODEXCREW_HOME", raising=False)
    with seeded_home("empty"):
        assert "CODEXCREW_HOME" in os.environ
    assert "CODEXCREW_HOME" not in os.environ
...
def test_seeded_home_restores_env_on_seed_failure(
    ...
    leave ``CODEXCREW_HOME`` dangling if seed() raised after we set it.
    """
    monkeypatch.setenv("CODEXCREW_HOME", "/tmp/sentinel-restore-on-fail")
    with pytest.raises(seed_mod.SeedError):
        with seeded_home("not-a-real-fixture"):
            pytest.fail("unreachable")
    assert os.environ["CODEXCREW_HOME"] == "/tmp/sentinel-restore-on-fail"
...
def test_seeded_home_populated_fixture_has_starter_session(
    ...
    ``$CODEXCREW_HOME/sessions/``, so shipping a well-formed JSONL file
    there makes the sessions tab non-empty on first boot — the headline
    value of ``codexcrew gateway --seed <fixture>`` for e2e dashboard
    testing.
    ...
    with seeded_home(fixture_name) as home:
        ...
        # ``codex_crew.dashboard.chat_persistence.restore_recent_sessions``:
...
def test_seeded_home_importable_without_pytest(
    ...
    wheel ships ``codex_crew.testing`` without pytest in its dependencies,
    so any hard ``import pytest`` at module top breaks the contract that
    ``from codex_crew.testing import seeded_home`` works from any test
    harness.
    ...
    monkeypatch.delitem(sys.modules, "codex_crew.testing.fixtures", raising=False)
    monkeypatch.delitem(sys.modules, "codex_crew.testing", raising=False)

    fresh = importlib.import_module("codex_crew.testing.fixtures")
