...
from codex_crew.session_summary import (
    count_user_turns,
...
class TestGating:
    ...
    async def test_disabled_returns_false_without_calling_the_model(self, env, monkeypatch):
        ...
        ok = await chat_summary.generate_session_summary(state, slot, cfg=CodexCrewConfig())
        assert ok is False
        assert called == []
...
class TestShouldSummarizeForceMatrix:
    ...
    def test_force_does_not_lift_disabled(self):
        ...
        override it."""
        cfg = CodexCrewConfig()  # session_summary.enabled defaults to False
        slot = _GateSlot()
...
class TestForcedGeneration:
    ...
    async def test_force_does_not_override_the_off_switch(self, env, monkeypatch):
        ...
        assert (
            await chat_summary.generate_session_summary(
                state, slot, cfg=CodexCrewConfig(), force=True
            )
            is False
        )
        assert called == []
