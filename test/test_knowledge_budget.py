...
from codex_crew.config.loader import KnowledgeConfig
...
class TestEmbedRateLimiter:
    def test_zero_rate_is_noop(self):
        from codex_crew.knowledge.ingestion import EmbedRateLimiter
    ...
    def test_high_rate_does_not_block(self):
        ...
        with patch("codex_crew.knowledge.ingestion.asyncio.sleep", fake_sleep):
            asyncio.run(limiter.acquire())
    ...
    def test_get_embed_rate_limiter_reads_config(self):
        import codex_crew.knowledge.ingestion as ing_mod
        from codex_crew.knowledge.ingestion import get_embed_rate_limiter

        # Reset the singleton
        ing_mod._embed_rate_limiter = None
        with patch("codex_crew.config.loader.CodexCrewConfig.load") as mock_load:
...
class TestSweepChunkBudget:
    def test_sweep_budget_read_from_config(self):
        ...
        with patch("codex_crew.knowledge.watcher.CodexCrewConfig") as mock_cfg:
...
class TestPoolSizeConfig:
    def test_default_pool_size(self):
        from codex_crew.knowledge.llm_pool import DEFAULT_POOL_SIZE, _get_pool_size
        assert _get_pool_size({}) == DEFAULT_POOL_SIZE

    def test_configured_pool_size(self):
        from codex_crew.knowledge.llm_pool import _get_pool_size
...
class TestExtractionModelResolution:
    def test_empty_extraction_model_uses_agent_model(self):
        ...
        with patch("codex_crew.config.loader.CodexCrewConfig.load") as mock_load:
            ...
            with patch("codex_crew.agent._atomic_json_write") as mock_write:
                with patch("codex_crew.agent.kiro_agents_dir_path") as mock_path:
                    mock_path.return_value = Path("/tmp/agents")
                    from codex_crew.agent import _install_knowledge_agent
