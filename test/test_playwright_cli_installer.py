...
test controls, `HOME` and `CODEXCREW_HOME` point into `tmp_path`, and no test
...
def test_a_registry_url_carrying_credentials_is_never_printed(tmp_path: Path, stubs: Path) -> None:
    ...
    result = _run(
        ...
        "--version",
        "0.1.18",
        extra_env={"CODEXCREW_NPM_REGISTRY": f"https://bob:{secret}@npm.internal.example/"},
    )
...
def test_a_credential_containing_an_at_sign_is_fully_redacted(tmp_path: Path, stubs: Path) -> None:
    ...
    result = _run(
        ...
        "--version",
        "0.1.18",
        extra_env={"CODEXCREW_NPM_REGISTRY": registry},
    )
    assert result.returncode == EXIT_CODES["registry_auth"]
    combined = result.stdout + result.stderr
...
def test_the_node_floor_matches_what_the_product_requires_of_this_cli() -> None:
    """These installers provision the CLI that Codex Crew's own browsing drives
    ...
    """
    from codex_crew.browser_cli.install import MIN_NODE_MAJOR as product_floor
...
def test_replacing_a_bootstrapped_node_never_leaves_the_prefix_without_one(
    ...
    (node_dir / ".codexcrew-playwright-cli-node").write_text("")
...
def test_the_windows_install_command_cannot_run_a_stale_local_script() -> None:
    ...
    """
    import codex_crew.browser_cli.install as install_mod
    from codex_crew.browser_cli.install import _standalone_install_command
...
def test_the_environment_route_still_accepts_a_credential(tmp_path: Path, stubs: Path) -> None:
    ...
    result = _run(
        ...
        "0.1.18",
        "--dry-run",
        extra_env={"CODEXCREW_NPM_REGISTRY": f"https://u:{secret}@npm.internal.example/"},
    )
    assert result.returncode == 0, result.stdout + result.stderr
    assert secret not in result.stdout + result.stderr
...
def test_a_node_tree_this_installer_wrote_is_replaced(
    ...
    node_dir = tmp_path / "datahome" / "playwright-cli" / "node"
    assert (node_dir / ".codexcrew-playwright-cli-node").exists()
...
def test_the_dry_run_plan_redacts_every_url_it_prints(tmp_path: Path, stubs: Path) -> None:
    ...
    result = _run(
        ...
        extra_env={
            ...
            "CODEXCREW_NPM_REGISTRY": "https://ru:reg-token-1@npm.internal.example/",
