"""Coverage tests for :mod:`codex_crew.slack.events`.

Focus areas that the existing Slack suites leave untouched:

* the built-in ``/codexcrew <sub-command>`` handlers (dashboard, agent, voice,
...
client, and ``codex_crew.slack.events.sel`` patched so no audit file is written.
"""
...
from codex_crew.slack import events as ev
...
def _mock_sel():
    ...
    fake = MagicMock()
    with patch("codex_crew.slack.events.sel", return_value=fake):
        yield fake
...
def _isolated_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path / ".kiro" / "crew"))
...
class TestSpawnTracked:
    ...
    async def test_failure_is_logged_not_raised(self, caplog):
        ...
        with caplog.at_level("DEBUG", logger="codex_crew.slack.events"):
...
class TestBuildHelpText:
    def test_lists_registered_subcommands_and_channel_hint(self):
        text = ev._build_help_text("codexcrew")
        assert text.startswith("*Available commands:*")
        assert "`/codexcrew dashboard`" in text
        assert "`/codexcrew #channel`" in text
    ...
    def test_description_less_command_renders_bare(self):
        ...
        try:
            line = "• `/codexcrew zzcovtmp`"
            assert line in ev._build_help_text("codexcrew")
        finally:
...
class TestHandleDashboard:
    ...
    async def test_default_ttl_sends_link(self):
        ...
        respond = AsyncMock()
        with patch(
            "codex_crew.slack.events.send_dashboard_link",
...
class TestHandleAgent:
    ...
    async def test_non_owner_denied(self):
        respond = AsyncMock()
        with patch("codex_crew.slack.handler.is_owner", return_value=False):
    ...
    async def test_off_resets_default_agent(self):
        respond = AsyncMock()
        with patch("codex_crew.slack.handler.is_owner", return_value=True):
            with patch("codex_crew.slack.handler._set_default_agent") as setter:
    ...
    async def test_known_agent_switches(self):
        respond = AsyncMock()
        with patch("codex_crew.slack.handler.is_owner", return_value=True):
            with patch("codex_crew.slack.handler._resolve_agent_name", return_value="scout"):
                with patch("codex_crew.slack.handler._set_default_agent") as setter:
    ...
    async def test_unknown_agent_falls_through_to_selector(self, _isolated_home):
        ...
        respond = AsyncMock()
        with patch("codex_crew.slack.handler.is_owner", return_value=True):
            with patch("codex_crew.slack.handler._resolve_agent_name", return_value=""):
                with patch("codex_crew.slack.handler._get_default_agent", return_value="scout"):
    ...
    async def test_selector_defaults_to_off_when_no_agents_dir(self):
        respond = AsyncMock()
        with patch("codex_crew.slack.handler.is_owner", return_value=True):
            with patch("codex_crew.slack.handler._get_default_agent", return_value=""):
...
def _yolo_env():
    ...
    so = MagicMock()
    with patch("codex_crew.slack.events.safety_override", return_value=so):
        with patch(
            "codex_crew.slack.events.describe_grant_lifetime",
            return_value="expires in 6h",
        ):
            with patch("codex_crew.slack.events.is_owner", return_value=True):
                yield so
...
class TestHandleYolo:
    ...
    async def test_non_owner_denied(self):
        respond = AsyncMock()
        with patch("codex_crew.slack.events.is_owner", return_value=False):
    ...
    async def test_off_disables(self, _yolo_env, _mock_sel):
        ...
        orch.dashboard_state = MagicMock()
        respond = AsyncMock()
        with patch("codex_crew.slack.handler.disable_yolo") as disable:
...
# ---------------------------------------------------------------------------
# /codexcrew config, users, channels
# ---------------------------------------------------------------------------
...
class TestGetAgentNamesGuards:
    ...
    def test_permission_error_falls_back_to_stem_and_audits(self, _isolated_home, _mock_sel):
        ...
        with patch(
            "codex_crew.slack.events.safe_read_file",
...
class _SocketPatches:
    ...
    def __enter__(self) -> _SocketPatches:
        for name in (
            ...
            p = patch(f"codex_crew.slack.events.{name}")
            self.setters[name] = p.start()
            self._stack.append(p)
        for target, new in (
            ("codex_crew.slack.events.WSSocketModeClient", self.client_cls),
            ("codex_crew.slack.events.AsyncWebClient", MagicMock()),
        ):
            p = patch(target, new)
            p.start()
            self._stack.append(p)
        ...
        p = patch("codex_crew.slack.events.current_context", ctx)
        p.start()
        self._stack.append(p)
        return self
...
class TestOnEventDispatch:
    ...
    async def test_ack_failure_short_circuits(self):
        ...
        with patch(
            "codex_crew.slack.events.dispatch_interactive", new_callable=AsyncMock
        ) as dispatch:
    ...
    async def test_slash_command_is_dispatched(self):
        orch = _socket_orch()
        on_event = _install_on_event(orch, ev.SeenCache())
        with patch("codex_crew.slack.events._handle_slash", new_callable=AsyncMock) as slash:
            await on_event(_client(), _req("slash_commands", {"command": "/codexcrew"}))
            await _drain(orch)
        slash.assert_awaited_once_with(orch, {"command": "/codexcrew"})

    @pytest.mark.asyncio
    async def test_unknown_envelope_type_ignored(self):
        orch = _socket_orch()
        on_event = _install_on_event(orch, ev.SeenCache())
        with patch("codex_crew.slack.events._route_message", new_callable=AsyncMock) as route:
    ...
    async def test_member_joined_channel_routed_to_prompt(self):
        ...
        with patch("codex_crew.slack.events._maybe_prompt_owner") as prompt:
    ...
    async def test_home_tab_published_for_allowed_user(self, _mock_sel):
        ...
        with patch("codex_crew.slack.events.is_allowed_user", return_value=True):
            with patch(
                "codex_crew.slack.events._publish_home_tab", new_callable=AsyncMock
            ) as publish:
                await on_event(_client(), _req("events_api", payload))
                await asyncio.sleep(0)
    ...
    async def test_home_tab_denied_for_unauthorized_user(self, _mock_sel):
        ...
        with patch("codex_crew.slack.events.is_allowed_user", return_value=False):
            with patch(
                "codex_crew.slack.events._publish_home_tab", new_callable=AsyncMock
            ) as publish:
                await on_event(_client(), _req("events_api", payload))
    ...
    async def test_home_tab_other_tab_ignored(self):
        ...
        with patch("codex_crew.slack.events._publish_home_tab", new_callable=AsyncMock) as publish:
            await on_event(_client(), _req("events_api", payload))
        publish.assert_not_called()
    ...
    async def test_message_deleted_is_delegated(self):
        ...
        with patch(
            "codex_crew.slack.events._handle_message_deleted", new_callable=AsyncMock
        ) as deleted:
    ...
    async def test_trusted_bot_routed_with_flag(self):
        ...
        with patch("codex_crew.slack.events.validated_self_bot_id", return_value="B_SELF"):
            with patch("codex_crew.slack.events._route_message", new_callable=AsyncMock) as route:
                await on_event(_client(), _req("events_api", {"event": event}))
    ...
    async def test_allowlisted_peer_denied_when_self_id_unknown(self, _mock_sel):
        ...
        with patch("codex_crew.slack.events.validated_self_bot_id", return_value=""):
            with patch("codex_crew.slack.events._route_message", new_callable=AsyncMock) as route:
                await on_event(_client(), _req("events_api", {"event": event}))
        route.assert_not_called()
        kw = _mock_sel.log_api_access.call_args.kwargs
...
class TestHandleSlash:
    ...
    async def test_unauthorized_caller_denied(self, _mock_sel):
        orch = _make_orch()
        payload = {"command": "/codexcrew", "user_id": "U_BAD", "text": "status"}
        with patch("codex_crew.slack.events.is_allowed_user", return_value=False):
    ...
    async def test_owner_not_configured_reports(self):
        ...
        orch._owner_id = ""
        posted: list[dict] = []
        payload = {"command": "/codexcrew", "user_id": "U_OWNER", "text": "status"}
        with patch("codex_crew.slack.events.is_allowed_user", return_value=True):
    ...
    async def test_user_mention_fallback_refuses_multi_user(self):
        orch = _make_orch()
        posted: list[dict] = []
        payload = {"command": "/codexcrew", "user_id": "U_OWNER", "text": "<@U123|bob>"}
        with patch("codex_crew.slack.events.is_allowed_user", return_value=True):
            with _capture_respond(posted):
                await ev._handle_slash(orch, payload)
                await _drain(orch)
    ...
    async def test_channel_mention_fallback_sends_track_request(self):
        orch = _make_orch()
        posted: list[dict] = []
        payload = {"command": "/codexcrew", "user_id": "U_OWNER", "text": "<#C123|general>"}
        with patch("codex_crew.slack.events.is_allowed_user", return_value=True):
            with patch(
                "codex_crew.slack.events.prompt_track_channel", new_callable=AsyncMock
            ) as prompt:
                with _capture_respond(posted):
                    await ev._handle_slash(orch, payload)
                    await _drain(orch)
    ...
    async def test_channel_mention_without_name_uses_secret(self):
        orch = _make_orch()
        posted: list[dict] = []
        payload = {"command": "/codexcrew", "user_id": "U_OWNER", "text": "<#C123>"}
        with patch("codex_crew.slack.events.is_allowed_user", return_value=True):
            with patch("codex_crew.slack.events.prompt_track_channel", new_callable=AsyncMock):
                with _capture_respond(posted):
                    await ev._handle_slash(orch, payload)
                    await _drain(orch)
    ...
    async def test_unknown_subcommand_returns_help(self):
        orch = _make_orch()
        posted: list[dict] = []
        payload = {"command": "/codexcrew", "user_id": "U_OWNER", "text": "flibbertigibbet"}
        with patch("codex_crew.slack.events.is_allowed_user", return_value=True):
            with _capture_respond(posted):
                await ev._handle_slash(orch, payload)
                await _drain(orch)
    ...
    async def test_respond_without_response_url_posts_nothing(self):
        orch = _make_orch()
        posted: list[dict] = []
        payload = {"command": "/codexcrew", "user_id": "U_OWNER", "text": "nope"}
        with patch("codex_crew.slack.events.is_allowed_user", return_value=True):
    ...
    async def test_respond_post_failure_is_swallowed(self):
        ...
        with patch("codex_crew.slack.events.is_allowed_user", return_value=True):
            with patch("codex_crew.slack.events.aiohttp.ClientSession", return_value=session):
                await ev._handle_slash(orch, payload)
                await _drain(orch)
...
class _capture_respond:
    ...
    def __enter__(self) -> _capture_respond:
        ...
        p = patch("codex_crew.slack.events.aiohttp.ClientSession", _Session)
        p.start()
        ...
        p2 = patch("codex_crew.slack.events._handle_slash", _wrapped)
        p2.start()
...
class TestTranscribeWithReaction:
    ...
    async def test_reaction_added_and_removed(self):
        ...
        slack = AsyncMock()
        with patch(
            "codex_crew.slack.events._transcribe_files",
            new_callable=AsyncMock,
            return_value=["hello"],
        ):
...
class TestTranscribeFiles:
    ...
    async def test_successful_transcription(self, _mock_sel):
        ...
        with patch(
            "codex_crew.slack.events.transcribe_audio",
            new_callable=AsyncMock,
    ...
    async def test_temp_unlink_failure_is_tolerated(self, tmp_path, monkeypatch):
        ...
        with patch(
            ...
            with patch("codex_crew.slack.events.os.unlink", side_effect=OSError("locked")) as unlink:
...
class TestResolveApprovalMode:
    def test_yolo_forces_auto(self):
        ...
        with patch("codex_crew.slack.events.is_yolo_mode", return_value=True):
    ...
    def test_cli_flag_wins_over_config(self):
        ...
        with patch("codex_crew.slack.events.is_yolo_mode", return_value=False):
            assert ev._resolve_approval_mode(orch) == ev.APPROVAL_AUTO
...
class TestRouteMessageGuards:
    ...
    async def test_activation_off_drops_plain_message(self, _mock_sel):
        ...
        with patch("codex_crew.slack.events.is_allowed_user", return_value=True):
            with patch("codex_crew.slack.events.handle_message", new_callable=AsyncMock) as hm:
    ...
    async def test_channels_governance_denial_blocks_message(self, _mock_sel):
        ...
        with patch("codex_crew.slack.events.is_allowed_user", return_value=True):
            with patch(
                "codex_crew.slack.events.channel_inbound_permitted",
                new_callable=AsyncMock,
                return_value=False,
            ):
                with patch("codex_crew.slack.events.handle_message", new_callable=AsyncMock) as hm:
    ...
    async def test_missing_channel_history_is_reported_not_fatal(self, caplog):
        ...
        with patch("codex_crew.slack.events.is_allowed_user", return_value=True):
            with patch("codex_crew.slack.events.handle_message", new_callable=AsyncMock) as hm:
                with caplog.at_level("ERROR", logger="codex_crew.slack.events"):
                    await ev._route_message(orch, _event(), ev.SeenCache())
                    await _drain(orch)
        hm.assert_awaited_once()
    ...
    async def test_handle_message_task_creation_failure_is_contained(self):
        ...
        with patch("codex_crew.slack.events.is_allowed_user", return_value=True):
            ...
            with patch(
                "codex_crew.slack.events.handle_message",
...
class TestRouteMessageStopCommand:
    ...
    async def test_unauthorized_stop_denied(self, _mock_sel):
        orch = _make_orch()
        with patch("codex_crew.slack.events.is_allowed_user", side_effect=[True, False]):
            with patch("codex_crew.slack.events.is_owner", return_value=False):
                await ev._route_message(orch, _event(text="!stop"), ev.SeenCache())
    ...
    async def test_bang_restart_delegates_to_restart_handler(self):
        ...
        with patch("codex_crew.slack.events.is_allowed_user", return_value=True):
            with patch("codex_crew.slack.events._handle_restart", new=_fake_restart):
...
class TestRouteMessageAttachments:
    ...
    async def test_the_availability_probe_runs_off_the_event_loop(self):
        ...
        with patch("codex_crew.slack.events.is_allowed_user", return_value=True):
            with patch(
                "codex_crew.slack.events.stt_available", side_effect=_probe_watching_the_loop
            ):
                with patch(
                    "codex_crew.slack.events._transcribe_with_reaction",
                    side_effect=_let_the_loop_breathe,
                ):
                    with patch(
                        "codex_crew.slack.events.process_slack_files",
                        new_callable=AsyncMock,
                        return_value=([], []),
                    ):
                        with patch(
                            "codex_crew.slack.events.handle_message", new_callable=AsyncMock
                        ) as hm:
    ...
    async def test_voice_transcript_is_prefixed(self):
        ...
        files = [{"mimetype": "audio/webm", "url_private": "https://x.invalid/a.webm"}]
        with patch("codex_crew.slack.events.is_allowed_user", return_value=True):
            with patch("codex_crew.slack.events.stt_available", return_value=True):
                with patch(
                    "codex_crew.slack.events._transcribe_with_reaction",
                    new_callable=AsyncMock,
                    return_value=["spoken"],
    ...
    async def test_image_and_text_blocks_are_appended(self, tmp_path):
        ...
        with patch("codex_crew.slack.events.is_allowed_user", return_value=True):
            with patch("codex_crew.slack.events.stt_available", return_value=False):
                with patch(
                    "codex_crew.slack.events.process_slack_files",
                    new_callable=AsyncMock,
...
class TestRouteMessageTransportPath:
    ...
    async def test_transport_path_dispatches_and_drains_queue(self):
        ...
        with patch("codex_crew.slack.events.is_allowed_user", return_value=True):
            with patch(
                "codex_crew.slack.events.handle_message_transport", new_callable=AsyncMock
            ) as transport:
                with patch(
                    "codex_crew.slack.events._dispatch_queued", new_callable=AsyncMock
                ) as dispatch:
                    with patch.object(CodexCrewConfig, "load", return_value=CodexCrewConfig()):
                        await ev._route_message(orch, _event(), ev.SeenCache())
                        await _drain(orch)
                        await _drain(orch)
    ...
    async def test_transport_drain_falls_back_to_pending_queue(self):
        ...
        with patch("codex_crew.slack.events.is_allowed_user", return_value=True):
            with patch("codex_crew.slack.events.handle_message_transport", new_callable=AsyncMock):
                with patch(
                    "codex_crew.slack.events._dispatch_queued", new_callable=AsyncMock
                ) as dispatch:
                    with patch.object(CodexCrewConfig, "load", return_value=CodexCrewConfig()):
...
class TestRouteMessageNativeDrain:
    ...
    async def test_native_done_callback_drains_session_queue(self):
        ...
        orch.sessions.dequeue = MagicMock(side_effect=lambda _k: queued.pop(0) if queued else None)
        with patch("codex_crew.slack.events.is_allowed_user", return_value=True):
            with patch("codex_crew.slack.events.handle_message", new_callable=AsyncMock):
                with patch(
                    "codex_crew.slack.events._dispatch_queued", new_callable=AsyncMock
                ) as dispatch:
                    await ev._route_message(orch, _event(), ev.SeenCache())
...
class TestRouteMessageEnterpriseAndObserve:
    ...
    async def test_enterprise_origin_mismatch_rejects(self, _mock_sel):
        ...
        with patch("codex_crew.slack.events.current_context", ctx):
            with patch("codex_crew.slack.events.handle_message", new_callable=AsyncMock) as hm:
                await ev._route_message(orch, _event(), ev.SeenCache())
        hm.assert_not_called()
...
class TestHandleStatus:
    ...
    async def test_reports_stats_summary_with_identity_line(self):
        respond = AsyncMock()
        with patch("codex_crew.slack.events.Stats") as stats:
...
class TestHandleSlashRespondBlocks:
    ...
    async def test_without_response_url_nothing_is_posted(self):
        ...
        try:
            payload = {"command": "/codexcrew", "user_id": "U_OWNER", "text": "zzcovnourl"}
            with patch("codex_crew.slack.events.is_allowed_user", return_value=True):
                with _capture_respond(posted, response_url=""):
                    await ev._handle_slash(orch, payload)
                    await _drain(orch)
