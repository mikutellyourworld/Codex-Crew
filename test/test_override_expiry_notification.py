...
def test_dispatch_skipped_when_disabled() -> None:
    ...
    state = _make_state()
    factory = MagicMock()
    with patch("codex_crew.dashboard.server.CodexCrewConfig.load", return_value=_cfg(False)):
...
def test_dispatch_schedules_when_enabled() -> None:
    ...
    async def _run() -> bool:
        ...
        with patch("codex_crew.dashboard.server.CodexCrewConfig.load", return_value=_cfg(True)):
...
class TestOverrideExpiryDmWording:
    ...
    ``approval_modes`` gate refuses the very ``/codexcrew yolo`` the generic text
    ...
    _LEGACY_TEXT = (
        ...
        "Reply `/codexcrew yolo` to re-authorize."
    )
    ...
    def test_policy_source_names_policy_and_omits_the_rearm_instruction(self) -> None:
        ...
        from codex_crew.dashboard.server import _override_expiry_dm_text
        ...
        assert "/codexcrew yolo" not in text
    ...
    def test_the_slack_notify_path_composes_source_into_the_dm(self) -> None:
        ...
        from codex_crew.dashboard.server import _notify_slack_override_expired
        from codex_crew.safety_override import POLICY_REVOKED_SOURCE
        ...
        assert "/codexcrew yolo" not in body
...
class TestDmOwner:
    ...
    def test_redacts_before_posting(self) -> None:
        ...
        with (
            patch(
                "codex_crew.dashboard.server.redact_exfiltration_urls",
                return_value=("no-exfil", []),
            ) as m_exfil,
            patch(
                "codex_crew.dashboard.server.redact_credentials",
