...
from codex_crew.dashboard.handlers.messaging import api_notification_agent_push
from codex_crew.dashboard.state import DashboardState, sweep_expired_notifications
...
def _make_state(monkeypatch, tmp_path) -> DashboardState:
    ...
    monkeypatch.setattr("codex_crew.notifications.settings.config_dir", lambda: tmp_path)
...
class TestSendNotificationToolIdentity:
    ...
    def test_governance_allowed_sel_record_on_permit(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.sel.sel", lambda: sel_mock)
        assert (
    ...
    def test_governance_error_fails_closed_for_send_notification(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.platform.governance_profiles.governance_permits", _boom
        )
        posted: list = []
        monkeypatch.setattr(
...
class TestAgentPushEndpoint:
    ...
    async def test_cookie_callers_denied(self, monkeypatch, tmp_path):
        ...
        sel_mock = MagicMock()
        monkeypatch.setattr(
            "codex_crew.dashboard.handlers.messaging._sel", lambda: sel_mock
        )
    ...
    async def test_persist_failure_returns_500(self, monkeypatch, tmp_path):
        ...
        state = _make_state(monkeypatch, tmp_path)
        monkeypatch.setattr(
            "codex_crew.dashboard.state._persist_notification", lambda note: False
        )
        async with TestClient(TestServer(self._app(state))) as client:
            resp = await client.post(
...
class TestSendNotificationSchemaRegistry:
    ...
    def test_missing_title_rejected_before_tool_body(self):
        ...
        with _patch("codex_crew.mcp_core._post") as mock_post:
...
class TestRound17Fixes:
    ...
    def test_strict_resolver_accepts_gateway_caller_context(self, monkeypatch):
        ...
        # CODEXCREW_SESSION_KEY nor CODEXCREW_HOST_PID. The gateway-injected
        # per-call caller context (pooled topology) must resolve instead of
        # failing closed.
        from codex_crew import mcp_caller, mcp_core
    ...
    def test_channel_agent_denied_at_mcp_dispatch(self, monkeypatch, tool):
        # GPT 5.6 round 17 HIGH: auto-approved codexcrew-core calls emit no
