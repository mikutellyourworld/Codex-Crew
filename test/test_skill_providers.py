...
from codex_crew.skill_providers.base import ProviderRegistry, SkillSearchResult
from codex_crew.skill_providers.skillsh import (
    SkillsShConfig,
    SkillsShProvider,
    _github_raw_url,
    _is_allowed_host,
...
class TestSkillsShProvider:
    ...
    async def test_search_parses_response(self):
        ...
        with patch(
            "codex_crew.skill_providers.skillsh._sync_fetch_json",
...
class TestProviderRegistry:
    ...
    async def test_search_provider_timeout_returns_empty(self):
        ...
        with patch.object(p, "search", side_effect=slow_search), \
             patch("codex_crew.skill_providers.base._SEARCH_TIMEOUT_SECS", 0.05):
            results = await reg.search("test")
            assert results == []
...
class TestSkillsShTagsCoercion:
    ...
    async def test_null_tags_coerced_to_empty_list(self):
        ...
        with patch("codex_crew.skill_providers.skillsh._sync_fetch_json", return_value=payload):
...
class TestIsInternalUrlSSRF:
    ...
    def test_blocked_internal_ip_emits_sel_audit(self):
        ...
        with patch("codex_crew.skill_providers.skillsh._audit_ssrf_blocked") as m:
