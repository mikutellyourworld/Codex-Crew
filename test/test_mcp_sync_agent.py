...
from codex_crew.dashboard.handlers.mcp import _mcp_hot_reload_active as _REAL_GATE
from codex_crew.mcp_provenance import without_marker
...
def _owner_caller(monkeypatch):
    ...
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.source_providers.is_owner_dashboard_request",
        lambda request: True,
    )
...
def _no_hot_reload(monkeypatch):
    ...
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.mcp._mcp_hot_reload_active",
...
def mcp_env(tmp_path: Path):
    ...
    agent_cfg = tmp_path / "codexcrew.json"
    ...
    with (
        patch("codex_crew.dashboard.handlers.mcp._GLOBAL_MCP_JSON", mcp_json),
        patch(
            "codex_crew.dashboard.handlers.agents._installed_agent_config", return_value=agent_cfg
        ),
    ):
...
async def _sync_one_remote(remote, mcp_env, *, owned: bool = True, marked: bool = True) -> dict:
    ...
    ``owned`` marks the name as managed by Codex Crew, which is the regime every caller
    ...
    """
    from codex_crew.dashboard.handlers.mcp import api_mcp_sync
    from codex_crew.mcp_discovery import SCOPE_CODEXCREW
    from codex_crew.mcp_provenance import stamp
    ...
    with (
        patch("codex_crew.mcp_discovery.discover_servers_to_sync", return_value=[remote]),
        patch("codex_crew.mcp_discovery.sync_to_agent_config", return_value=True),
        patch("codex_crew.mcp_discovery.register_servers_for_cc"),
        patch(
            "codex_crew.mcp_discovery._load_mcp_json_by_source",
            return_value={SCOPE_CODEXCREW: _store},
        ),
        patch("codex_crew.dashboard.handlers.mcp._get_mcp_lock") as mock_lock,
        patch("codex_crew.dashboard.handlers.mcp._write_mcp_json") as mock_write,
        patch("codex_crew.dashboard.handlers.mcp._sync_mcp_to_agent_batch"),
        patch(
            "codex_crew.dashboard.handlers.sessions._reset_all_sessions",
...
class TestGlobalWritesAreOwnershipGated:
    ...
    def _own(names: set[str]):
        """Patch the store scope so ``codexcrew_managed_names`` sees exactly ``names``."""
        from codex_crew.mcp_discovery import SCOPE_CODEXCREW

        return patch(
            "codex_crew.mcp_discovery._load_mcp_json_by_source",
            return_value={SCOPE_CODEXCREW: {n: {"url": "https://store"} for n in names}},
        )
    ...
    async def _kiro_global(self, *, managed: bool, mcp_env, marked: bool | None = None) -> dict:
        ...
        remote = McpServerInfo(
            name="handmade",
            url="https://codexcrew.example.com/mcp",
            scopes=["codexcrew:scope"],
            client_id="codexcrew-client",
            source="discovered",
        )
    ...
    def _cc_sidecar(self, *, managed: bool, tmp_path) -> dict:
        ...
        from codex_crew.mcp_discovery import McpServerInfo, register_servers_for_cc
    ...
    async def test_a_managed_name_still_syncs_at_every_global_write_site(
        ...
        assert entry["url"] == "https://codexcrew.example.com/mcp"
        assert entry["oauthScopes"] == ["codexcrew:scope"]
        assert entry["oauth"]["clientId"] == "codexcrew-client"
        assert entry["oauth"]["issuer"] == "https://user-issuer", "sub-key still survives"

        cc = self._cc_sidecar(managed=True, tmp_path=tmp_path)
        assert cc["scopes"] == ["codexcrew:scope"]
        assert cc["clientId"] == "codexcrew-client"
    ...
    def test_a_malformed_store_value_does_not_make_a_name_managed(self, tmp_path):
        ...
        from codex_crew.mcp_discovery import SCOPE_CODEXCREW, codexcrew_managed_names

        with patch(
            "codex_crew.mcp_discovery._load_mcp_json_by_source",
            return_value={SCOPE_CODEXCREW: {"good": {"url": "https://x"}, "bad": "not-a-dict"}},
        ):
            assert codexcrew_managed_names() == {"good"}
...
class TestExactNameCollisionIsDecidedByTheMarker:
    ...
    * HARMFUL: the user hand-authored a global server at A whose name collides
    ...
    recorded rather than inferred: our writes carry ``x-codexcrew``, and an entry
    ...
    async def test_stripping_the_marker_reclaims_the_entry_for_good(self, mcp_env):
        ...
        """
        from codex_crew.mcp_provenance import is_marked, stamp, without_marker
    ...
    async def test_a_marked_entry_does_not_re_write_on_every_sync(self, mcp_env):
        ...
        """
        from codex_crew.mcp_provenance import is_marked, stamp
    ...
    def test_an_unmanaged_remote_keeps_its_sidecar_entry(self, tmp_path, monkeypatch):
        ...
        """
        from codex_crew import mcp_discovery as md
    ...
    def test_a_managed_remote_re_syncs_to_the_sidecar_only_when_marked(
        ...
        """
        from codex_crew import mcp_discovery as md
        from codex_crew.mcp_provenance import stamp, without_marker
        ...
        monkeypatch.setattr(
            ...
            "_load_mcp_json_by_source",
            lambda: {
                md.SCOPE_CODEXCREW: {"owned": store_entry},
    ...
    def test_a_remote_with_no_sidecar_entry_is_still_registered(self, tmp_path):
        ...
        """
        from codex_crew.mcp_discovery import McpServerInfo, register_servers_for_cc
        from codex_crew.mcp_provenance import is_marked, without_marker
...
class TestSidecarStdioWritesAreGatedToo:
    ...
    def test_a_colliding_hand_authored_stdio_entry_is_preserved(self, tmp_path, caplog):
        ...
        sidecar = tmp_path / "cc.json"
        with caplog.at_level(logging.WARNING, logger="codex_crew.mcp_provenance"):
    ...
    def test_an_absent_stdio_name_is_created_stamped(self, tmp_path):
        from codex_crew.mcp_provenance import is_marked
...
class TestSyncMcpToAgent:
    def test_enable_adds_server_and_tool_refs(self, mcp_env):
        agent_cfg, _ = mcp_env
        from codex_crew.dashboard.handlers.mcp import _sync_mcp_to_agent
...
class TestSyncMcpToAgentBatch:
    def test_enable_adds_multiple_servers(self, mcp_env):
        agent_cfg, _ = mcp_env
        from codex_crew.dashboard.handlers.mcp import _sync_mcp_to_agent_batch
...
class TestAimMcpInstallSync:
    ...
    @staticmethod
    def _mgr(ok: bool):
        from codex_crew.platform.interfaces import CapabilityResult
    ...
    async def test_install_calls_sync(self):
        ...
        with (
            patch(
                "codex_crew.dashboard.handlers.agents._capability_manager",
                return_value=self._mgr(ok=True),
            ),
            patch("codex_crew.dashboard.handlers.mcp._sync_mcp_to_agent") as mock_sync,
        ):
...
class TestApiMcpSyncToolsUpdate:
    ...
    async def test_sync_adds_tools_for_discovered_servers(self, mcp_env):
        ...
        with (
            patch(
                "codex_crew.mcp_discovery.discover_servers_to_sync",
                return_value=[mock_server],
            ),
            patch(
                "codex_crew.mcp_discovery.sync_to_agent_config",
                return_value=True,
            ),
            patch("codex_crew.mcp_discovery.register_servers_for_cc"),
            patch("codex_crew.dashboard.handlers.mcp._get_mcp_lock") as mock_lock,
            patch("codex_crew.dashboard.handlers.mcp._write_mcp_json"),
            patch(
                "codex_crew.dashboard.handlers.mcp._sync_mcp_to_agent_batch",
            ) as mock_batch,
            patch(
                "codex_crew.dashboard.handlers.sessions._reset_all_sessions",
                new_callable=AsyncMock,
    ...
    def test_discovery_can_deliver_a_non_dict_header_map(self, tmp_path: Path):
        ...
        from codex_crew.mcp_discovery import discover_servers_to_sync

        with (
            patch(
                "codex_crew.mcp_discovery._load_mcp_json",
                return_value={"remote": {"url": "https://mcp.example.com/v1", "headers": "bad"}},
            ),
            patch("codex_crew.mcp_discovery._load_agent_config", return_value={"mcpServers": {}}),
        ):
    ...
    async def test_sync_omits_a_malformed_scope_list_instead_of_truncating_it(self, mcp_env):
        ...
        mcp_json.write_text(json.dumps(data))
        from codex_crew.mcp_discovery import _spec_scopes
...
class TestApiMcpSyncHotReload:
    ...
    async def _run(
        ...
        monkeypatch.setattr("codex_crew.dashboard.handlers.mcp._mcp_hot_reload_active", _REAL_GATE)
        sessions = MagicMock()
        ...
        with (
            patch("codex_crew.mcp_discovery.discover_servers_to_sync", return_value=[]),
            patch("codex_crew.mcp_discovery.sync_to_agent_config", return_value=False),
            patch("codex_crew.dashboard.handlers.mcp._sync_mcp_to_agent_batch"),
            patch("codex_crew.dashboard.handlers.sessions._reset_all_sessions", reset),
        ):
            resp = await api_mcp_sync(req)
        assert resp.status == 200
...
class TestOffloadedSyncHoldsTheConfigLock:
    """`_sync_mcp_to_agent*` does a read-modify-write of codexcrew.json. Offloading
    ...
    def test_every_offloaded_sync_is_under_the_config_lock(self) -> None:
        ...
        lines = (
            (_REPO_ROOT / "src/codex_crew/dashboard/handlers/mcp.py").read_text(encoding="utf-8").splitlines()
        )
        offenders: list[str] = []
...
class TestSyncSharesTheFileLockWithBridges:
    """The dashboard sync and bridges' app-MCP registration both RMW codexcrew.json.
    ...
    def test_both_sync_funcs_hold_the_mcp_file_lock(self) -> None:
        ...
        from codex_crew.dashboard.handlers import mcp
...
class TestSyncStripsGovernedAutoApprove:
    """Copying a global MCP server into codexcrew.json must not carry a governed
...
class TestSyncStripsPreExistingGovernedAutoApprove:
    ...
    in codexcrew.json (re-enable, or a spec written before the ceiling): the copy
    branch only runs for a brand-new alias."""

    def test_existing_entry_autoapprove_is_stripped(self, mcp_env, monkeypatch):
        agent_cfg, _ = mcp_env
        import codex_crew.dashboard.handlers.mcp as mcp
        from codex_crew.dashboard.handlers.mcp import _sync_mcp_to_agent, mcp_server_alias
...
class TestCapabilityInstallOffloadsTheLockedSync:
    ...
    codexcrew.json RMW. Called directly on the event loop it freezes the gateway
    ...
    def test_discover_capability_install_offloads(self):
        from codex_crew.dashboard.handlers import mcp_discover
