...
from codex_crew.config.loader import CodexCrewConfig, SessionSummaryConfig
from codex_crew.dashboard import chat_handlers, chat_summary
from codex_crew.dashboard.chat import api_chat_slot_summary, api_chat_slot_summary_generate
...
def _pin_flag(monkeypatch, enabled: bool) -> None:
    ...
    monkeypatch.setattr(chat_handlers.CodexCrewConfig, "load", staticmethod(_load))
...
class TestSummaryEndpoint:
    ...
    async def test_the_endpoint_never_generates(self, tmp_path, monkeypatch):
        ...
        from codex_crew.dashboard import chat_summary
...
class TestPanelGateDrift:
    ...
    async def test_each_mirrored_gate_refuses_on_both_sides(self):
        ...
        cases: list[tuple[str, CodexCrewConfig, _ChatSlot]] = []
        ...
        def _cfg(enabled: bool = True) -> CodexCrewConfig:
            cfg = CodexCrewConfig()
...
class TestGenerateState:
    ...
    async def test_unavailable_when_the_feature_is_off(self):
        cfg = CodexCrewConfig()  # enabled defaults to False
        slot = _ChatSlot("s1")
        slot.messages = [
            {"role": "user", "content": "one"},
            {"role": "user", "content": "two"},
