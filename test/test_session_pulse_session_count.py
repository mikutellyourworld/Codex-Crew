...
from codex_crew.dashboard import session_pulse_counter as spc
from codex_crew.dashboard.state import DashboardState, SlotOrigin
from codex_crew.history import ConversationLog

_SRC = Path(__file__).resolve().parents[1] / "src" / "codex_crew"
...
def _isolated_counter(tmp_path, monkeypatch: pytest.MonkeyPatch):
    ...
    import codex_crew.dashboard.state as state_mod
...
def _opted_in_call_counts() -> dict[str, int]:
    """Map of module path (relative to src/codex_crew) -> number of
...
def test_only_human_request_paths_opt_in() -> None:
    ...
    # (chat_fork.py). This sweeps every module under src/codex_crew, so an
