...
``codexcrew-core`` stub registered, pre-flighted ``ensure_backend``, was refused
...
from codex_crew.mcp_gateway.gatewayd import _TargetUnknown, resolvable_target_stems
from codex_crew.mcp_gateway.manager import GatewayManager, GatewaySpec
from codex_crew.mcp_gateway.stub import (
...
def test_stems_strip_the_prefix_and_the_args_hash() -> None:
    ...
    env = {
        "CODEXCREW_MCP_TARGET_CODEXCREW_CORE": "codexcrew mcp-core",
        "CODEXCREW_MCP_TARGET_CODEXCREW_CORE__61774e2023ff185e": "codexcrew mcp-core",
        "CODEXCREW_MCP_TARGET_PDF": "npx -y server-pdf --stdio",
        "PATH": "/usr/bin",
        "CODEXCREW_HOME": "/home/x/.codex-crew",
    }
    assert resolvable_target_stems(env) == ["CODEXCREW_CORE", "PDF"]
...
def test_required_stems_come_from_the_spec_the_rewriter_just_computed() -> None:
    ...
    mgr = _manager(
        {
            "CODEXCREW_MCP_TARGET_CODEXCREW_CORE": "codexcrew mcp-core",
            "CODEXCREW_MCP_TARGET_CODEXCREW_CORE__deadbeef": "codexcrew mcp-core",
            "CODEXCREW_MCP_TARGET_EXCALIDRAW": "node x.js",
        }
    )
    assert mgr._required_target_stems() == {"CODEXCREW_CORE", "EXCALIDRAW"}
...
def test_a_superset_daemon_is_also_silent(caplog) -> None:
    ...
    with caplog.at_level(logging.WARNING):
        assert (
            mgr._adoption_drift({"type": "pong", "targets": ["PDF", "EXCALIDRAW", "CODEXCREW_CORE"]})
            == []
        )
    assert caplog.text == ""
...
def test_a_daemon_that_cannot_report_is_flagged_as_unverifiable(caplog) -> None:
    ...
    mgr = _manager({"CODEXCREW_MCP_TARGET_CODEXCREW_CORE": "codexcrew mcp-core"})
    with caplog.at_level(logging.WARNING):
        assert mgr._adoption_drift({"type": "pong"}) == ["CODEXCREW_CORE"]
...
def test_the_stub_side_string_matches_the_daemon_that_sends_it() -> None:
    ...
    reason = str(
        _TargetUnknown(
            "no target mapping for server 'codexcrew-core'; "
...
def test_the_legacy_env_prefix_spelling_does_not_break_the_match() -> None:
    ...
    ``CODEXCREW_MCP_TARGET_``), so matching the whole sentence would miss exactly
    the old daemons this exists for."""
    for spelling in ("MC_MCP_TARGET_", "CODEXCREW_MCP_TARGET_"):
...
class _Args:
    server = "codexcrew-core"
    agent = "codexcrew"
    channel_id = ""
    target_command = "/x/codexcrew"


def _counts_in(tmp_path, monkeypatch, records):
    """Write `records` through the real writer, then aggregate them."""
    monkeypatch.setattr(
        "codex_crew.mcp_gateway.stub._fallback_log_path",
        lambda: tmp_path / "stub_fallback.jsonl",
    )
    for reason, terminal in records:
        log_fallback(reason, "uuid-x", "codexcrew:codexcrew-core", _Args(), terminal=terminal)
...
def test_a_terminal_record_does_not_inflate_the_fallback_rate(tmp_path, monkeypatch):
    ...
    assert c["total"] == 1
    assert c["by_server"] == {"codexcrew-core": 1}
    assert c["by_reason"] == {"at capacity": 1}
    assert c["terminal_total"] == 1
    assert c["terminal_by_server"] == {"codexcrew-core": 1}
...
def test_a_record_predating_the_flag_still_counts_as_a_fallback(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.mcp_gateway.stub._fallback_log_path", lambda: log)
    c = fallback_counts()
    assert c["total"] == 1
    assert c["terminal_total"] == 0
