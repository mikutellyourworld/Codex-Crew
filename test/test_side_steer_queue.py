...
from codex_crew.dashboard.side_state import MAX_SIDE_QUEUE, SideState
from codex_crew.dashboard.ws import broadcast_side_queue, broadcast_side_result
...
def _install_gated_stream(
    ...
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.side.stream_and_collect", _fake_stream
    )
    return seen
...
async def test_a_stale_task_cannot_drain_a_newer_sides_queue(tmp_path, monkeypatch):
    ...
    """
    from codex_crew.dashboard.handlers.side import _drain_side_queue
    ...
    dispatched: list[str] = []
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.side._dispatch_side_turn",
...
async def test_a_failed_drain_puts_the_entry_back_instead_of_dropping_it(
    ...
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.side._dispatch_side_turn", _boom
    )
...
async def test_a_stale_consumption_echo_cannot_settle_a_replacement_sidecar(
    ...
    """
    from codex_crew.dashboard.handlers.side import _run_side_turn
...
async def test_ledger_capacity_evicts_terminal_entries_but_never_a_pending_one(tmp_path):
    ...
    """
    from codex_crew.dashboard.side_state import (
        MAX_STEER_LEDGER,
        STEER_CONSUMED,
        STEER_PENDING,
    )
...
async def test_an_unproven_steer_is_not_committed_so_it_cannot_duplicate(tmp_path):
    ...
    """
    from codex_crew.dashboard.side_state import STEER_PENDING
...
async def test_the_head_insert_is_bounded_like_every_other_queue_write(tmp_path):
    ...
    from codex_crew.dashboard.side_state import MAX_SIDE_QUEUE
...
def test_every_queued_steer_response_hands_back_a_correlation_handle():
    ...
    from codex_crew.dashboard.handlers import side as side_mod
...
async def test_a_requeued_steer_card_names_the_steer_it_came_from(tmp_path, monkeypatch):
    ...
    """
    from codex_crew.dashboard.handlers.side import _requeue_unconsumed_side_steers
...
async def test_a_new_submit_does_not_overtake_a_queue_on_an_idle_side(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.side._dispatch_side_turn", _fake_dispatch
    )
...
async def test_an_auth_failure_holds_the_queue_instead_of_draining_it(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.side.stream_and_collect", _auth_fails
    )
    monkeypatch.setattr(
        "codex_crew.dashboard.chat_runner._mark_kiro_signed_out", lambda *_a, **_k: None
    )
...
def test_a_bare_kas_echo_lets_the_sidecar_render_its_steer():
    ...
    """
    from codex_crew.dashboard.side_state import STEER_CONSUMED, SideState
