"""Tests for the ``codex_crew._bootstrap`` console-entry self-heal.

The bootstrap closes the git-pull gap for editable installs: a commit that
adds a runtime dependency must not leave ``codexcrew`` dying on a raw
...
from codex_crew import _bootstrap
...
def test_self_heal_runs_on_windows_through_the_dependency_only_path(monkeypatch, tmp_path):
    ...
    ``codexcrew.exe`` — but a dependency install never touches that wrapper, and a
    ...
    monkeypatch.setattr(
        dep_sync, "locked_console_scripts", lambda target: [r"C:\v\Scripts\codexcrew.exe"]
    )
    monkeypatch.setattr(
