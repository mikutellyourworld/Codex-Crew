...
Covers :func:`codex_crew.resource_status.admission_check` (critical refuses;
...
from codex_crew import resource_status as rs
...
def _cfg(pressure: float = 4.0, critical: float = 2.0, gate: bool = True) -> SimpleNamespace:
    """Minimal stand-in for CodexCrewConfig exposing the gate's config surface."""
...
class TestCronAdmissionDeferral:
    ...
    async def test_critical_defers_then_runs_on_recovery(
        ...
        with caplog.at_level(logging.INFO, logger="codex_crew.cron"):
            with patch("codex_crew.cron.admission_check", return_value=_refused()):
                await svc._on_timer()
                await svc._on_timer()
        ...
        with patch("codex_crew.cron.admission_check", return_value=_admitted()):
            await svc._on_timer()
...
class TestSpawnAdmissionGate:
    ...
    def test_spawn_refused_when_critical(self) -> None:
        ...
        mgr = self._mgr()
        with patch(
            "codex_crew.subagent.check_memory_available", return_value=(True, 8.0)
        ), patch("codex_crew.subagent.CodexCrewConfig") as mock_cfg, patch(
            "codex_crew.subagent.cached_admission_check", return_value=_refused()
        ), patch(
            "codex_crew.subagent.sel"
        ) as mock_sel:
    ...
    def test_spawn_proceeds_past_gate_when_admitted(self) -> None:
        ...
        with patch(
            ...
            "codex_crew.subagent.cached_admission_check", return_value=_admitted()
        ), patch(
            "codex_crew.subagent.validate_cwd", return_value=("", "not allowed")
        ), patch(
            "codex_crew.subagent.sel"
        ) as mock_sel:
            mock_cfg.load.return_value.agent.spawn_min_memory_gb = 4.0
...
def _load_from_dict(data: dict, tmp_path: Path) -> CodexCrewConfig:
    """Write *data* to a config file under *tmp_path* and load it."""
    tmp = tmp_path / "config.json"
    tmp.write_text(json.dumps(data))
    with unittest.mock.patch(
        "codex_crew.config.loader.config_path", return_value=tmp
    ):
...
class TestCronExprPassthrough:
    ...
    async def test_job_claimed_during_admission_await_is_not_double_fired(
        ...
        with patch("codex_crew.cron.admission_check", side_effect=claiming_check):
            await svc._on_timer()
    ...
    async def test_manual_run_completed_during_await_is_not_double_fired(
        ...
        with patch("codex_crew.cron.admission_check", side_effect=completing_check):
            await svc._on_timer()
    ...
    async def test_job_edited_during_await_dispatches_live_object(
        ...
        with patch("codex_crew.cron.admission_check", side_effect=editing_check):
            await svc._on_timer()
    ...
    async def test_cron_expr_job_runs_normally_under_critical(
        ...
        with (
            patch("codex_crew.cron.admission_check", return_value=_refused()),
            patch("codex_crew.cron.cron_expr_matches", return_value=True),
        ):
            await svc._on_timer()
    ...
    async def test_deferral_episode_floors_timer_delay(
        ...
        from codex_crew.cron import _TIMER_POLL_SECS
    ...
    async def test_interval_edited_to_cron_during_await_still_fires(
        ...
        with (
            patch("codex_crew.cron.admission_check", side_effect=editing_check),
            patch("codex_crew.cron.cron_expr_matches", return_value=True),
        ):
            await svc._on_timer()
