"""Tests for codex_crew.apps.backend — backend process management."""
...
def _sandbox_can_spawn() -> bool:
    ...
    """
    try:
        from codex_crew import sandbox as _sb
...
class TestBackendLifecycle:
    ...
    def test_backend_entrypoint_escapes_app_root(self, tmp_path, app_env, caplog):
        ...
        from codex_crew.apps.backend import _start_app_backend_body
        from codex_crew.apps.manager import app_dir, get_app_manifest
    ...
    def test_third_party_backend_refused_when_gate_off(
        ...
        monkeypatch.setattr(
            "codex_crew.apps.execution.third_party_execution_allowed", lambda: False
        )
        monkeypatch.setattr(
    ...
    def test_shipped_builtin_module_backend_not_blocked_by_gate(
            ...
            json.dumps(
                {
                    ...
                    "backend": {
                        "entryPoint": "codex_crew.apps.builtins.file_explorer.server",
                        "port": "auto",
                    },
                }
            )
        ...
        def _capture_wrap(cmd, **kwargs):
            ...
            captured["first_party_fixed_argv"] = kwargs.get("first_party_fixed_argv")
            return cmd, None
        ...
        assert captured["cmd"][:3] == [
            sys.executable,
            "-m",
            "codex_crew.apps.builtins.file_explorer.server",
        ]
        assert captured["first_party_fixed_argv"] is True
...
class TestShellDispatch:
    ...
    def _dispatch_cmd(
        ...
        import codex_crew.apps.backend as bmod
        from codex_crew.apps.manager import get_app_manifest
...
class TestBootAdmissionRevet:
    """start_enabled_app_backends re-vets admission at boot (CodexCrew parity).
...
# =============================================================================
# CODEXCREW_DEVFLEET_REPO forwarding (silent-empty-fleet fix)
# =============================================================================
...
def test_devfleet_repo_survives_the_app_backend_env_allowlist(monkeypatch):
    ...
    ``CODEXCREW_DEVFLEET_REPO`` is dev-fleet's highest-priority repo discovery
    ...
    ``CODEXCREW_PROJECT_DIR``, which packaged installs point at the app bundle
    with no ``.git``, and a hardcoded ``~/codexcrew`` fallback).
    """
    ...
    monkeypatch.setenv("CODEXCREW_DEVFLEET_REPO", "/opt/checkouts/codexcrew")
    # The generic allowlist does NOT carry it — that is the trap this guards.
    assert "CODEXCREW_DEVFLEET_REPO" not in minimal_env()

    # apps/backend.py must therefore add it to the explicit platform extras
    # (same mechanism that carries CODEXCREW_PROJECT_DIR and the
    # CODEXCREW_DEVFLEET_BIN_* trusted-binary overrides).
    body = Path(bmod.__file__).read_text()
    assert (
        '_platform_extra["CODEXCREW_DEVFLEET_REPO"]' in body
    ), "the CODEXCREW_DEVFLEET_REPO override no longer reaches app backends"
...
def test_devfleet_repo_env_wins_repo_discovery(monkeypatch, tmp_path):
    ...
    from codex_crew.apps.builtins.dev_fleet import server as dfmod
    ...
    assert dfmod._default_main_repo() == "/opt/checkouts/codexcrew"

    # Without the override the chain falls through to PROJECT_DIR, which is
    # adopted only because it carries the Codex Crew checkout markers.
    monkeypatch.delenv("CODEXCREW_DEVFLEET_REPO")
...
class TestTheCacheOnlyChildCanSeeTheCacheItMustBootFrom:
    ...
    def test_an_unconfined_host_says_so_once_and_names_the_control(
        ...
        try:
            with caplog.at_level(logging.WARNING, logger="codex_crew.apps.backend"):
...
class TestPromotionRequiresAConfirmedEnabledApp:
    ...
    `codexcrew app disable` runs in its OWN process: it deregisters the app's resources
...
class TestPromotionIsVerifiedAfterTheWrite:
    ...
    `codexcrew app disable` runs in another process, so there is no lock to share. Ordering
