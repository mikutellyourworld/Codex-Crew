...
(``src/codex_crew/apps/builtins/code_review_sage/tests/``), but several handlers
...
as a package), ``CODEXCREW_HOME`` is pointed at a per-test tmp dir so nothing
...
_APP_ROOT = (Path(__file__).resolve().parent.parent / "src" / "codex_crew" / "apps"
...
class _SageRoutesBase(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        self._old_home = os.environ.get("CODEXCREW_HOME")
        os.environ["CODEXCREW_HOME"] = self.tmp
        # config_dir() memoizes the resolved data home for the process; reset it
        # so this test's CODEXCREW_HOME wins over whatever an earlier test cached.
        import codex_crew.config.paths as _paths
        self._paths = _paths
    ...
    def tearDown(self):
        ...
        if self._old_home is None:
            ...
            os.environ["CODEXCREW_HOME"] = self._old_home
...
class TestSettingsHandler(_SageRoutesBase):
    def _patch_audit(self):
        return unittest.mock.patch("codex_crew.sel.sel")
    ...
    async def test_audit_failure_never_breaks_the_response(self):
        with unittest.mock.patch("codex_crew.sel.sel", side_effect=RuntimeError("no sel")):
            resp = await self.mod._handle_settings(
