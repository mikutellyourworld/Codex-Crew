"""Tests for codex_crew.apps.dependency_ledger — reference-counted dependency tracking."""
...
def _clear_ledger() -> None:
    ...
    from codex_crew.apps.dependency_ledger import _ledger_path
    path = _ledger_path()
    if path.is_file():
        path.unlink()
...
class TestDeclaredCapabilityKeys:
    def test_reads_capabilities_key(self):
        from codex_crew.apps.dependency_ledger import declared_capability_keys
...
class TestUncleanableTypesArePreserved:
    ...
    Deleting the row for something CodexCrew cannot actually uninstall leaves the
    ...
    def test_is_uncleanable_accepts_every_spelling(self):
        from codex_crew.apps.dependency_ledger import _is_uncleanable
