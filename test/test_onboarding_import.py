...
def _api() -> ModuleType:
    try:
        return importlib.import_module("codex_crew.onboarding_import")
    except ModuleNotFoundError:
        pytest.fail("codex_crew.onboarding_import is not implemented")
...
class TestApply:
    ...
    def test_mcp_import_uses_the_shared_dashboard_lock(
        ...
        api = _api()
        mcp_handlers = importlib.import_module("codex_crew.dashboard.handlers.mcp")
        entered: list[bool] = []
    ...
    def test_mcp_import_rejects_enabled_effective_name_collisions(
        ...
        installed_path = home / ".kiro" / "agents" / "codexcrew.json"
        mcp_handlers = importlib.import_module("codex_crew.dashboard.handlers.mcp")
    ...
    def test_mcp_import_rejects_edition_scope_collisions(
        ...
        mcp_discovery = importlib.import_module("codex_crew.mcp_discovery")
        monkeypatch.setattr(
            mcp_discovery,
            "_extra_scopes",
    ...
    def test_mcp_import_rejects_edition_provided_server_collision(
        ...
        platform_context = importlib.import_module("codex_crew.platform.context")
    ...
    def test_mcp_secret_fields_reject_the_entire_server_definition(self, tmp_path: Path) -> None:
            ...
            json.dumps(
                {
                    "mcpServers": {
                        ...
                        "codexcrew-core": {"command": "foreign-managed"},
                    }
                }
            ),
            encoding="utf-8",
        ...
        assert "codexcrew-core" not in written["mcpServers"]
    ...
    def test_clean_skill_assets_preserve_large_content_and_whitespace(
        ...
        atomic_write_module = importlib.import_module("codex_crew.atomic_write")
    ...
    def test_episodic_import_never_replaces_a_similar_native_memory(
        ...
        api = _api()
        vector_memory = importlib.import_module("codex_crew.vector_memory")
...
class TestReviewFindings:
    ...
    def test_instruction_cap_is_shared_across_sources(self, tmp_path: Path) -> None:
        ...
        destination = tmp_path / "destination"
        result = api.apply_import(plan, data_home=destination)
        from codex_crew.learn import _MAX_LESSONS_TOTAL, LessonStore
