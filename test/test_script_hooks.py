...
_ECHO_HOOK_EVENT = "echo %CODEXCREW_HOOK_EVENT%" if _IS_WINDOWS else "echo $CODEXCREW_HOOK_EVENT"
...
class TestCappedScriptHookOutput:
    ...
    async def test_reader_keeps_only_the_cap_but_drains_to_eof(self):
        from codex_crew.hooks import _read_capped_stream
    ...
    def test_decode_marks_a_multibyte_boundary(self):
        from codex_crew.hooks import _HOOK_TRUNCATION_MARKER, _decode_capped
    ...
    async def test_cancellation_observes_both_reader_tasks(self):
        from codex_crew.hooks import _communicate_capped
    ...
    async def test_real_hook_caps_and_marks_both_streams(self, tmp_path, monkeypatch):
        from codex_crew.hooks import _HOOK_STREAM_CAP_BYTES, _HOOK_TRUNCATION_MARKER
        ...
        monkeypatch.setattr("codex_crew.sandbox.wrap_argv", lambda argv, **k: (list(argv), None))
        monkeypatch.setattr("codex_crew.sandbox.cgroup_scope_argv", lambda argv: list(argv))
...
class TestRunScriptHook:
    ...
    def _passthrough_sandbox(self, monkeypatch):
        # run_script_hook uses a lazy `from codex_crew.sandbox import wrap_argv`
    ...
    def test_subprocess_env_excludes_gateway_credentials(self, monkeypatch):
        from codex_crew.hooks import _hook_subprocess_env
        ...
        monkeypatch.setenv("CODEXCREW_HOME", "/custom/data/home")
        hook = ScriptHook(
            id="env-safe",
            name="env-safe",
            event=HOOK_EVENT_USER_PROMPT_SUBMIT,
        ...
        # The data-home override must survive so a hook invoking the codexcrew
        # CLI resolves the gateway's home, not the default (it is a path, not a
        # credential).
        assert env["CODEXCREW_HOME"] == "/custom/data/home"
        assert env["CODEXCREW_HOOK_EVENT"] == HOOK_EVENT_USER_PROMPT_SUBMIT
        assert env["CODEXCREW_HOOK_CONTEXT"] == "prompt text"
    ...
    async def test_subprocess_routes_allowlist_through_safe_spawn_funnel(self, monkeypatch):
        ...
        proc.returncode = 0
        monkeypatch.setattr("codex_crew.sandbox.sandboxed_spawn_argv", safe_spawn)
        create = AsyncMock(return_value=proc)
        monkeypatch.setattr("codex_crew.sandbox.create_subprocess_limited", create)
...
class TestRunScriptHookSpawnForm:
    ...
    async def test_command_reaches_the_shell_verbatim(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.sandbox.wrap_argv", lambda argv, **k: (list(argv), None))
        monkeypatch.setattr("codex_crew.sandbox.cgroup_scope_argv", lambda argv: list(argv))
        monkeypatch.setattr("codex_crew.sandbox.create_subprocess_limited", fake_exec)
    ...
    async def test_a_wrapping_sandbox_wins_over_the_shell_spawn(self, monkeypatch):
        ...
        """
        monkeypatch.setattr(
            "codex_crew.sandbox.wrap_argv", lambda argv, **k: (["sandbox-exec", *argv], None)
        )
