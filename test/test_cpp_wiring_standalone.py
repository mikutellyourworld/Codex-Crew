...
With NO companion installed and NO ``CODEXCREW_PROFILE`` override, the active
...
from codex_crew import sandbox, security
...
def test_boot_standalone_no_signals(cfg: CodexCrewConfig, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "codex_crew.platform.profile.Path.home",
...
def test_boot_platform_is_idempotent(cfg: CodexCrewConfig, monkeypatch) -> None:
...
def test_default_agent_runtime_delegates_to_agent_first_run_setup(monkeypatch) -> None:
    ...
    calls: list[tuple] = []
    monkeypatch.setattr(
        "codex_crew.agent.run_first_run_setup",
...
def test_gateway_first_run_setup_routes_through_the_seam(monkeypatch) -> None:
    ...
    from codex_crew.platform import build_default_context, reset_context, set_context
    ...
    monkeypatch.setattr("codex_crew.agent.run_first_run_setup", lambda: seen.append("core-direct"))
