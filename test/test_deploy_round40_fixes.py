...
F2: the ``.codexcrew-deploy.json`` control manifest lives beside published
...
HANDLERS = (REPO / "src" / "codex_crew" / "deploy" / "handlers.py").read_text(encoding="utf-8")
BASE_STACK = (
    REPO / "src" / "codex_crew" / "deploy" / "skills" / "artifact-deploy"
...
class TestF1EmptyProfileName:
    def test_validate_field_gap_this_guards(self):
        ...
        from codex_crew.deploy.handlers import _PROFILE_SPEC
...
class TestF2ManifestNotPublic:
    def _policy_statements(self):
        with mock.patch.object(engine, "_checked") as checked:
            engine.put_oac_bucket_policy(
                "codexcrew-web-example", "arn:aws:cloudfront::123:distribution/EX1", "p")
    ...
    def test_engine_policy_denies_cloudfront_manifest_reads(self):
        ...
        res = deny["Resource"]
        assert any(r.endswith("/*/.codexcrew-deploy.json") for r in res), (
    ...
    def test_base_stack_policy_denies_cloudfront_manifest_reads(self):
        ...
        assert "/*/.codexcrew-deploy.json" in block
