...
from codex_crew.platform.defaults import DefaultTipsProvider
...
from codex_crew.tips_pool import (
    PUBLIC_POOL_ID,
    WITHHELD_POOL_ID,
    CatalogEntry,
    TipsPool,
...
def _install(provider: object, *, profile: str = PROFILE_STANDALONE) -> None:
    ...
    ctx = build_default_context(CodexCrewConfig.load(), profile=profile)
...
class TestPoolIdentity:
    ...
    def test_default_context_composes_the_default_provider(self) -> None:
        ...
        ctx = build_default_context(CodexCrewConfig.load())
...
class TestCrossPoolReconciliation:
    ...
    async def test_colliding_id_cannot_dismiss_the_previous_pools_doc(self, tmp_path: Path) -> None:
        ...
        from codex_crew.tips import api_tips_feedback
        ...
        with patch.dict(os.environ, {"CODEXCREW_HOME": str(tmp_path)}):
            ...
            with mpatch("codex_crew.tips.CodexCrewConfig") as mock_cfg:
        ...
        from codex_crew.tips import _scoped
    ...
    async def test_stale_public_tips_are_dropped_on_first_load(self, tmp_path: Path) -> None:
        ...
        with patch.dict(os.environ, {"CODEXCREW_HOME": str(tmp_path)}):
            ...
            from codex_crew.tips import _state_path
    ...
    def test_pool_id_round_trips_through_disk(self, tmp_path: Path) -> None:
        from codex_crew.tips import _load_state
    ...
    def test_missing_or_bad_pool_id_loads_as_public(self, tmp_path: Path) -> None:
        from codex_crew.tips import _load_state, _state_path
...
class TestGeneratedTipContainment:
    ...
    def test_anchoring_runs_after_link_sanitization(self) -> None:
        from codex_crew.tips import _clean_catalog_entry, _parse_tips
    ...
    async def test_generation_anchors_against_the_edition_catalog(self, tmp_path: Path) -> None:
        ...
        from codex_crew.tips import generate_tips
        ...
        with patch.dict(os.environ, {"CODEXCREW_HOME": str(tmp_path)}):
            ...
            with (
                mpatch("codex_crew.tips.CodexCrewConfig") as mock_cfg,
                mpatch("codex_crew.tips.run_bg_oneliner", AsyncMock(return_value=payload)),
                mpatch("codex_crew.tips._build_context", return_value=""),
            ):
                mock_cfg.load.return_value = cfg
...
class TestScopedDismissalIdentity:
    ...
    def test_public_dismissal_does_not_suppress_an_edition_tip(self) -> None:
        from codex_crew.tips import _is_eligible
    ...
    def test_edition_dismissal_still_suppresses_its_own_tip(self) -> None:
        from codex_crew.tips import _is_eligible, _scoped
    ...
    def test_dismissal_is_honoured_again_after_switching_back(self) -> None:
        from codex_crew.tips import _is_eligible, _reconcile_pool
    ...
    def test_prompt_sees_only_the_active_pools_dismissals_unscoped(self) -> None:
        from codex_crew.tips import _active_pool_dismissals, _scoped
    ...
    async def test_dismissing_an_edition_tip_scopes_what_it_writes(self, tmp_path: Path) -> None:
        ...
        from codex_crew.tips import _is_eligible, api_tips_feedback
...
class TestPoolEntryValidation:
    ...
    async def test_sanitized_pool_serves_without_error(self, tmp_path: Path) -> None:
        ...
        from codex_crew.tips import api_tips_next
        ...
        with patch.dict(os.environ, {"CODEXCREW_HOME": str(tmp_path)}):
            ...
            with (
                mpatch("codex_crew.tips.CodexCrewConfig") as mock_cfg,
                mpatch("codex_crew.tips.maybe_refresh", _noop),
            ):
                mock_cfg.load.return_value = cfg
