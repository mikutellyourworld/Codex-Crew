...
from codex_crew.mcp_gateway.backend import Backend, BackendGone, _PendingRequest
from codex_crew.mcp_gateway.pool import BackendUnavailable, PoolAtCapacity, PoolKey
...
class TestControlFrames:
    ...
    async def test_pong_reports_the_target_map_so_an_adopter_can_check_it(
        ...
        monkeypatch.setenv("CODEXCREW_MCP_TARGET_CODEXCREW_CORE", "codexcrew mcp-core")
        writer = _FakeWriter()
        await _handle(_ScriptedReader({"type": "ping"}), writer, _fake_pool())
        assert "CODEXCREW_CORE" in writer.frames()[0]["targets"]
    ...
    async def test_register_naming_a_reserved_stub_prefix_is_rejected(self, peer_ok):
        ...
        at registration."""
        from codex_crew.mcp_gateway.backend import INTERNAL_STUB_PREFIXES
