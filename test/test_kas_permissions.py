...
from codex_crew.acp.kas_permissions import (
    CAPABILITY_BY_TOOL,
    WITHHELD_FROM_AUTO_APPROVE,
    allowed_tools_to_permissions,
)
from codex_crew.agent import _seed_kas_permissions
...
class TestTheShellAndFilesystemFamiliesAreRefused:
    ...
    def test_the_refusal_is_reported_at_info_because_it_reverses_the_spec(self, caplog):
        ...
        with caplog.at_level("INFO", logger="codex_crew.acp.kas_permissions"):
            allowed_tools_to_permissions(["execute_bash"], agent_id="codexcrew")
...
class TestMcpEntries:
    def test_a_bare_server_becomes_a_one_level_glob(self):
        policy = allowed_tools_to_permissions(["@codexcrew-core"])
        assert _rule(policy, "mcp")["match"] == ["codexcrew-core/*"]

    def test_a_named_action_stays_exact(self):
        policy = allowed_tools_to_permissions(["@codexcrew-cron/cron_list"])
        assert _rule(policy, "mcp")["match"] == ["codexcrew-cron/cron_list"]
    ...
    def test_a_server_wildcard_absorbs_its_own_per_tool_entries(self):
        ...
        policy = allowed_tools_to_permissions(
            ["@codexcrew-cron/cron_list", "@codexcrew-cron/cron_pause", "@codexcrew-cron"]
        )
        assert _rule(policy, "mcp")["match"] == ["codexcrew-cron/*"]
...
class TestTranslationNeverWidensAGrant:
    ...
    def test_a_glob_does_not_suppress_the_literal_entries_beside_it(self):
        policy = allowed_tools_to_permissions(["@*", "@codexcrew-core", "web_fetch"])
        assert _rule(policy, "mcp")["match"] == ["codexcrew-core/*"]
        assert _rule(policy, "web_fetch")["effect"] == "allow"

    def test_a_rejected_glob_is_explainable_from_the_log(self, caplog):
        with caplog.at_level("DEBUG", logger="codex_crew.acp.kas_permissions"):
...
class TestUnclassifiableEntriesFailClosed:
    ...
    def test_the_names_are_reported_so_a_missing_grant_is_explainable(self, caplog):
        ...
        with caplog.at_level("DEBUG", logger="codex_crew.acp.kas_permissions"):
            allowed_tools_to_permissions(["introspect"], agent_id="codexcrew")
...
class TestTheRealAllowlist:
    ...
    ALLOWED = [
        ...
        "session",
        "report",
        "@codexcrew-cron/cron_list",
        "@codexcrew-cron/cron_pause",
        "@codexcrew-core",
        "@notes-mcp",
        "@tickets-mcp",
        "@codexcrew-cron",
...
class TestTheDiskWriter:
    ...
    def _config(**over) -> dict:
        base: dict = {"name": "codexcrew", "tools": ["fs_read"], "allowedTools": ["web_fetch"]}
        base.update(over)
        return base
    ...
    def test_a_stale_block_is_the_accepted_cost_and_the_wire_covers_it(self):
        ...
        _seed_kas_permissions(config)
        assert config["permissions"]["rules"] == [{"capability": "web_fetch", "effect": "allow"}]
        assert to_client_custom_agent("codexcrew", {**config, "prompt": "p"}, "p")[
            "permissions"
