...
Codex Crew's supported installer (``install.sh --mise`` / ``ensure-node.sh``)
...
see the very node Codex Crew installed for them: ``codexcrew pod provision`` died
...
from codex_crew import env as env_mod
...
def isolated_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("CODEXCREW_NODE_BIN_DIR", raising=False)
...
def test_env_override_wins(isolated_home, tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CODEXCREW_NODE_BIN_DIR", str(override))
    assert env_mod.node_bin_dirs()[0] == str(override)


def test_relative_env_override_is_ignored(isolated_home, monkeypatch):
    monkeypatch.setenv("CODEXCREW_NODE_BIN_DIR", "some/relative/bin")
...
def test_env_override_carrying_a_path_separator_is_rejected(isolated_home, monkeypatch):
    ...
    """
    monkeypatch.setenv("CODEXCREW_NODE_BIN_DIR", f"/opt/a/bin{os.pathsep}/tmp/evil/bin")
...
def test_duplicate_spellings_collapse(isolated_home, monkeypatch):
    ...
    monkeypatch.setenv("CODEXCREW_NODE_BIN_DIR", str(d) + os.sep + ".")
    dirs = env_mod.node_bin_dirs()
...
def test_ensure_node_glibc217_tree_is_found_without_the_marker(isolated_home):
    """The tree Codex Crew's own installer unpacks must not depend on the marker.

    ``ensure-node.sh`` writes ``<data-home>/node-bin-dir`` after installing, but a
    tree installed under a different CODEXCREW_HOME (or a marker since deleted)
