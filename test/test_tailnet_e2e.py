...
with real exit codes. The command under test is the real `codexcrew tailnet`
...
def env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CODEXCREW_PORT", str(_DASH_PORT))
    return {"cli": cli, "state": state, "home": home}


def _run_cli(*argv: str) -> int:
    """Invoke the real `codexcrew` entry point in-process; return its exit code."""
    from codex_crew.cli import main

    with patch.object(sys, "argv", ["codexcrew", *argv]):
        try:
            main()
