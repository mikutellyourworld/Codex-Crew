...
from codex_crew import name_grant, task_executor
...
from codex_crew.hooks import TOOL_AUTO_APPROVE, HookManager, ToolHookResult
from codex_crew.messaging import (
    APPROVAL_INTERACTIVE,
    APPROVAL_TRUST_READS,
    TransportCapabilities,
    TurnDriver,
)
from codex_crew.messaging.renderer import Renderer
from codex_crew.providers.base import LLMEvent
from codex_crew.task_models import Project, Task
...
class TestLoopSafetyPins:
    ...
    def test_every_surface_shares_the_one_off_loop_entry_point(self):
        ...
        from codex_crew import llm_helpers, subagent, task_executor
        from codex_crew.dashboard import chat_runner
        from codex_crew.discord import transport_dispatch as discord_dispatch
        from codex_crew.messaging import driver
        from codex_crew.slack import handler as slack_handler
        from codex_crew.slack import transport_dispatch as slack_dispatch
        from codex_crew.telegram import transport_dispatch as telegram_dispatch
    ...
    def test_the_channel_gates_stay_loop_bound_and_check_free(self):
        ...
        from codex_crew.discord import transport_dispatch as discord_dispatch
        from codex_crew.messaging import dispatch
        from codex_crew.slack import transport_dispatch as slack_dispatch
        from codex_crew.telegram import transport_dispatch as telegram_dispatch
...
async def _execute_one_task(tmp_path, provider, *, on_tool_approval):
    ...
    with patch.object(task_executor.CodexCrewConfig, "load") as cfg:
...
class TestSubagentSurface:
    ...
    async def test_refused_grant_falls_to_the_headless_fail_closed_reject(self, monkeypatch):
        ...
        with (
            ...
            patch("codex_crew.subagent.update_state"),
            patch("codex_crew.subagent.create_agent_folder", MagicMock()),
        ):
    ...
    async def test_the_decline_is_audited(self, monkeypatch):
        ...
        with (
            patch("codex_crew.subagent.Stats"),
            patch("codex_crew.subagent.sel") as sel_factory,
            patch("codex_crew.subagent.update_state"),
            patch("codex_crew.subagent.create_agent_folder", MagicMock()),
        ):
            await manager._run_inner(info, "subagent:ng01")
...
class TestTurnDriverSurface:
    ...
    def test_the_decline_is_audited(self, monkeypatch):
        from codex_crew.messaging import driver as driver_mod
...
class TestSpawnRungEventIdentity:
    ...
    def test_genuine_spawn_run_event_rides_the_rung(self):
        from codex_crew.messaging.dispatch import build_auto_approve

        p = _ScriptedProvider(
            self._events(
                tool_name="spawn_run",
                mcp_server_name="codexcrew-core",
    ...
    def test_no_surface_keeps_a_title_only_spawn_check(self):
        ...
        from codex_crew.discord import transport_dispatch as discord_dispatch
        from codex_crew.messaging import dispatch, driver
        from codex_crew.slack import handler as slack_handler
        from codex_crew.slack import transport_dispatch as slack_dispatch
        from codex_crew.telegram import transport_dispatch as telegram_dispatch
...
class TestStreamAndCollectSurface:
    ...
    def _run(self, monkeypatch, refusal, *, approver):
        from codex_crew import llm_helpers
        from codex_crew.hooks import ToolHookResult
        from codex_crew.providers.base import EVENT_COMPLETE as PB_COMPLETE
    ...
    def test_the_decline_is_audited(self, monkeypatch):
        ...
        # `codex_crew.sel`, so the observable seam is the source module.
