"""Tests for codex_crew.snapshot — snapshot and restore."""
...
from codex_crew import snapshot as snapshot_mod
from codex_crew.jsonl_util import OversizedRecord, UndecodableRecord, UnreadableRecord
from codex_crew.snapshot import restore_main, snapshot_main
...
def _no_gateway(monkeypatch):
    ...
    """
    monkeypatch.setenv("CODEXCREW_ASSUME_GATEWAY_RUNNING", "0")


def _setup_fake_codexcrew(d: Path) -> None:
    """Create a realistic fake ~/.codexcrew directory."""
    for sub in (
    ...
    (d / "workspace_dir").write_text("/home/user/.codexcrew/workspace")
...
def _make_snapshot(src: Path, out: Path, extra_args: list[str] | None = None) -> Path:
    """Create a snapshot and return the tarball path. Caller must set CODEXCREW_HOME.
    ...
    snapshot_main(args)
    tarballs = sorted(
        out.glob("codexcrew-snapshot-*.tar.gz"), key=lambda p: p.stat().st_mtime, reverse=True
    )
...
def env(tmp_path, monkeypatch):
    ...
    src = tmp_path / "src"
    out = tmp_path / "out"
    _setup_fake_codexcrew(src)
    monkeypatch.setenv("CODEXCREW_HOME", str(src))
...
class TestSnapshot:
    def test_creates_valid_tarball(self, env):
        ...
        snaps = [d for d in extract.iterdir() if d.name.startswith("codexcrew-snapshot-")]
        assert snaps
        snap = snaps[0]
    ...
    def test_db_content_survives(self, env):
        ...
        with tarfile.open(str(tarball)) as tar:
            tar.extractall(extract, filter=lambda t, _d="": t)
        snap = next(d for d in extract.iterdir() if d.name.startswith("codexcrew-snapshot-"))
    ...
    def test_keep_prunes(self, env, monkeypatch):
        ...
        for i in range(3):
            (out2 / f"codexcrew-snapshot-2026010{i}T000000Z.tar.gz").write_text("fake")
        monkeypatch.setenv("CODEXCREW_HOME", str(src))
        snapshot_main([str(out2), "--keep", "2"] + unpinnable_argv())
        total = len(list(out2.glob("codexcrew-snapshot-*.tar.gz")))
        assert total == 2
    ...
    def test_list(self, env, capsys, monkeypatch):
        ...
        assert "codexcrew-snapshot-" in capsys.readouterr().out
...
class TestRestoreDryRun:
    def test_dry_run(self, env, capsys, monkeypatch):
        ...
        fresh.mkdir()
        monkeypatch.setenv("CODEXCREW_HOME", str(fresh))
...
class TestRestoreReplace:
    ...
    def test_replace_backs_up(self, env, monkeypatch):
        ...
        _setup_fake_codexcrew(existing)
        (existing / "workspace/original.md").write_text("original")
        monkeypatch.setenv("CODEXCREW_HOME", str(existing))
        ...
        # However the fake setup may not create it -- check what _setup_fake_codexcrew does.
...
class TestRestoreMerge:
    def test_merge_memory_dedup(self, env, monkeypatch):
        ...
        conn.commit()
        conn.close()
        monkeypatch.setenv("CODEXCREW_HOME", str(dst))
...
class TestSecurity:
    def test_data_filter_drops_sel_hmac_key_at_trust_path(self):
        ...
        from codex_crew.snapshot import _data_filter
    ...
    def test_path_traversal_filtered(self, env, capsys, monkeypatch):
        ...
        with tarfile.open(str(evil_tar), "w:gz") as tar:
            ...
            info = tarfile.TarInfo(name="codexcrew-snapshot-20260101T000000Z/")
            info.type = tarfile.DIRTYPE
            tar.addfile(info)
            # Add traversal entry — will be filtered
            info2 = tarfile.TarInfo(name="codexcrew-snapshot-20260101T000000Z/../../../etc/passwd")
            info2.size = 0
            tar.addfile(info2)
    ...
    def test_hardlink_filtered(self, env, capsys, monkeypatch):
        ...
        with tarfile.open(str(evil_tar), "w:gz") as tar:
            ...
            info.type = tarfile.DIRTYPE
            tar.addfile(info)
            info2 = tarfile.TarInfo(name="codexcrew-snapshot-20260101T000000Z/evil")
            info2.type = tarfile.LNKTYPE
            info2.linkname = "codexcrew-snapshot-20260101T000000Z/memory.db"
            tar.addfile(info2)
...
class TestParsedNamespace:
    ...
    def test_snapshot_via_parsed_namespace(self, env, monkeypatch):
        ...
        assert ret == 0
        assert list(out.glob("codexcrew-snapshot-*.tar.gz"))
...
class TestGatewayRunningRefusal:
    def test_restore_refused_when_gateway_running(self, env, capsys, monkeypatch):
        ...
        fresh.mkdir()
        monkeypatch.setenv("CODEXCREW_HOME", str(fresh))
        monkeypatch.setenv("CODEXCREW_ASSUME_GATEWAY_RUNNING", "1")
...
class TestEmptyKirocrewDir:
    def test_snapshot_empty_dir(self, tmp_path, monkeypatch):
        """Snapshot succeeds on an empty ~/.codexcrew directory."""
        empty = tmp_path / "empty_mc"
        empty.mkdir()
        out = tmp_path / "empty_out"
        monkeypatch.setenv("CODEXCREW_HOME", str(empty))
...
class TestConcurrentSnapshot:
    def test_concurrent_snapshots_unique(self, env, monkeypatch):
        ...
        time.sleep(1.1)
        snapshot_main([str(out)] + unpinnable_argv())
        tarballs = list(out.glob("codexcrew-snapshot-*.tar.gz"))
...
class TestTheArchiveIsLockedDownBeforeItIsPublished:
    ...
    def test_the_lockdown_runs_before_the_archive_is_published(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.platform_compat.restrict_to_owner", _recording)
        snapshot_main([str(out)] + unpinnable_argv())

        published = sorted(out.glob("codexcrew-snapshot-*.tar.gz"))
    ...
    def test_a_failed_lockdown_publishes_no_archive(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.platform_compat.restrict_to_owner",
        ...
        assert not list(
            out.glob("codexcrew-snapshot-*.tar.gz")
    ...
    def test_a_successful_snapshot_is_still_owner_only(self, tmp_path, monkeypatch):
        ...
        snapshot_main([str(out)] + unpinnable_argv())
        tarball = sorted(out.glob("codexcrew-snapshot-*.tar.gz"))[0]
        assert tarball.is_file()
        if os.name == "posix":
...
class TestNotificationMergeWriteSideContract:
    ...
    def test_an_archive_derived_path_cannot_forge_terminal_output(self, tmp_path, capsys):
        ...
        """
        hostile = tmp_path / "codexcrew-snapshot-\x1b[2K\x1b[1Aevil"
        hostile.mkdir()
    ...
    def test_the_residual_copy_failure_also_escapes_its_archive_path(
        ...
        hostile = tmp_path / "codexcrew-snapshot-\x1b[2K-forged"
        hostile.mkdir()
        src = hostile / "notifications.jsonl"
        dst = tmp_path / "live.jsonl"
...
class TestNotificationCopyWhenNoLiveFileExists(_NotificationCopyFixtures):
    ...
    def test_the_reader_loads_every_record_the_copy_installs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CODEXCREW_HOME", str(home2))
        self._merge(clean, home2)
        from codex_crew.dashboard import state as dashboard_state
    ...
    def test_a_note_delivered_during_the_copy_survives_the_READER(self, tmp_path, monkeypatch):
        ...
        # the patched `CODEXCREW_HOME` while the append is still queued, so the worker
    ...
    def test_an_import_failure_that_is_not_ImportError_is_not_swallowed(
        ...
        def failing(name, globals=None, locals=None, fromlist=(), level=0):
            if name == "codex_crew.dashboard" and "state" in (fromlist or ()):
    ...
    def test_an_unimportable_dashboard_falls_back_instead_of_failing(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setitem(sys.modules, "codex_crew.dashboard.state", None)
        snapshot_mod._copy_notifications(snap / "notifications.jsonl", home / "notifications.jsonl")
        assert (home / "notifications.jsonl").read_bytes() == self.GOOD
