"""Additional coverage for ``codex_crew.slack.handler``.
...
from codex_crew.acp.client import AcpProcessDied, AcpPromptBusy, AcpTimeoutError
...
from codex_crew.hooks import TOOL_ALLOW, TOOL_AUTO_APPROVE, TOOL_DENY, ToolHookResult
from codex_crew.slack.handler import handle_interaction, handle_message
...
class TestResolveAgentNameProject:
    ...
    def test_non_matching_project_agent_does_not_short_circuit(self, tmp_path):
        ...
        # No project match and (in an isolated CODEXCREW_HOME) no user agent either.
...
class TestHandleInteractionAuthReChecks:
    ...
    async def test_late_trust_refused_when_session_map_lookup_fails(self, monkeypatch, owner):
        import codex_crew.session as session_mod
...
class TestLinkedThreadRouting:
    ...
    async def test_idle_slot_starts_a_dashboard_turn(self, monkeypatch, owner):
        import codex_crew.dashboard.chat as chat_mod
