...
from codex_crew.apps.bridges import _apply_agent_mcp_policy
from codex_crew.apps.builtins.mochi.agent_policy import (
    BG_AGENT,
    CHAT_AGENT,
    build_policy,
    write_policy,
...
class TestBuildPolicy:
    def test_audience_maps_chat_and_bg_to_real_agent_names(self, monkeypatch):
        monkeypatch.setattr(
            "codex_crew.apps.builtins.mochi.agent_policy._ambient_servers", lambda: {}
        )
        pol = build_policy(
            {
                "extraMcpServers": [
    ...
    def test_ungranted_ambient_server_is_neutralized_with_its_real_tools(self, monkeypatch):
        monkeypatch.setattr(
            "codex_crew.apps.builtins.mochi.agent_policy._ambient_servers",
    ...
    def test_discovery_failure_does_not_raise(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("codex_crew.mcp_discovery.list_servers", boom)
    ...
    def test_builtin_core_cron_grants_are_foreground_only(self, monkeypatch):
        ...
        in codexcrew-core / codexcrew-cron. Leaving those to the fail-closed
        ...
        monkeypatch.setattr(
            "codex_crew.apps.builtins.mochi.agent_policy._ambient_servers",
            lambda: {"codexcrew-core": ["spawn_run"], "codexcrew-cron": ["cron_add"]},
        )
        pol = build_policy({"extraMcpServers": []})
        chat, bg = pol["agents"][CHAT_AGENT], pol["agents"][BG_AGENT]
        # foreground: both granted, neither neutralized
        assert "codexcrew-core" in chat["servers"]
        assert "codexcrew-cron" in chat["servers"]
        assert "codexcrew-core" not in chat["neutralize"]
        assert "codexcrew-cron" not in chat["neutralize"]
        ...
        assert chat["servers"]["codexcrew-core"]["mountOnly"] is True
        assert chat["servers"]["codexcrew-cron"]["mountOnly"] is True
        # background: NEITHER granted, BOTH denied (a subagent must not spawn)
        assert "codexcrew-core" not in bg["servers"]
        assert "codexcrew-cron" not in bg["servers"]
        assert "codexcrew-core" in bg["neutralize"]
        assert "codexcrew-cron" in bg["neutralize"]
    ...
    def test_user_grant_still_wins_over_builtin_floor(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.apps.builtins.mochi.agent_policy._ambient_servers",
            lambda: {"codexcrew-core": ["spawn_run"]},
        )
        pol = build_policy(
            {
                "extraMcpServers": [
                    {"name": "codexcrew-core", "agents": ["chat"], "autoApprove": ["spawn_run"]},
                ]
            }
        )
        assert pol["agents"][CHAT_AGENT]["servers"]["codexcrew-core"]["autoApprove"] == ["spawn_run"]
...
class TestQueuedPetActionIsExecutable:
    ...
    The MCP server runs as a SEPARATE process (``codexcrew app mcp mochi``) and can
    ...
    def _queue(self, monkeypatch, tmp_path, args):
        from codex_crew.apps.builtins.mochi import mcp_server as ms

        monkeypatch.setattr(ms, "_data_dir", lambda: tmp_path)
        ms._tool_perform_pet_action(args)
        from codex_crew.apps.builtins.mochi import queue_file as qf
...
class TestAmbientEnumerationFailsClosed:
    ...
    def _discovery_raises(monkeypatch):
        ...
        def boom(name, *args, **kwargs):
            if name == "codex_crew.mcp_discovery":
...
class TestPolicyMaterializationFailsClosed:
    ...
    def test_a_raising_refresh_is_not_swallowed(self, tmp_path, monkeypatch):
        from codex_crew.apps.builtins.mochi import agent_policy as ap

        monkeypatch.setattr(ap, "_ambient_servers", lambda: {})
        monkeypatch.setattr(
            "codex_crew.apps.bridges.refresh_app_agents",
    ...
    def test_a_partial_refresh_is_a_failure(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(ap, "_ambient_servers", lambda: {})
        monkeypatch.setattr("codex_crew.apps.bridges.refresh_app_agents", lambda _app: ["mochi"])
        monkeypatch.setattr(
            "codex_crew.apps.bridges.get_app_manifest",
    ...
    def test_no_registered_agents_is_not_a_failure(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(ap, "_ambient_servers", lambda: {})
        monkeypatch.setattr("codex_crew.apps.bridges.refresh_app_agents", lambda _app: [])
        monkeypatch.setattr(
            "codex_crew.apps.bridges.get_app_manifest", lambda _app: self._manifest([])
        )
