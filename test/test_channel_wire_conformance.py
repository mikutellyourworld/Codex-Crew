...
network calls, so it must never run in the normal suite or on a fork PR:
    ...
    CODEXCREW_WIRE_PROBE=1 \\
    CODEXCREW_WIRE_PROBE_WEIXIN_TOKEN=<bot token> \\
...
from codex_crew.testing.channel_fixtures import assert_same_shape, load_fixture

# The fixtures root lives in the TEST tree, so the layout coupling lives here
# too -- codex_crew.testing.channel_fixtures ships in the wheel and deliberately
...
_PROBE_ENABLED = os.environ.get("CODEXCREW_WIRE_PROBE") == "1"

pytestmark = pytest.mark.skipif(
    not _PROBE_ENABLED,
    reason=(
        "live vendor conformance is opt-in: set CODEXCREW_WIRE_PROBE=1 plus the "
...
class TestWeixinConformance:
    def test_get_bot_qrcode_still_matches_the_recorded_shape(self) -> None:
        ...
        token = _require("CODEXCREW_WIRE_PROBE_WEIXIN_TOKEN")
