...
the gateway-injected caller block, ``CODEXCREW_SESSION_KEY``, or
``CODEXCREW_HOST_PID`` plus its signed sidecar. A script cron had none of them:
...
So ``ctx.call_tool("codexcrew-cron", "cron_trigger", ...)`` reached the handler
...
including agent crons: the launcher injects ``CODEXCREW_SESSION_KEY=cron:<job>``
...
from codex_crew.cron_script import McpToolClient, ScriptContext, run_script_sandboxed
...
def _capture_launcher_env(job_id: str) -> dict[str, str]:
    ...
    with (
        patch("codex_crew.cron_script.resolve_script_path", return_value=("/f.py", "run")),
        patch("codex_crew.cron_script.wrap_argv", return_value=(["true"], None)),
        patch("codex_crew.cron_script._resolve_internal_secret", return_value="s"),
        patch("codex_crew.cron_script.popen_limited", side_effect=fake_popen),
    ):
...
def _no_ambient_identity(monkeypatch):
    ...
    """
    for key in ("CODEXCREW_SESSION_KEY", "CODEXCREW_HOST_PID", "CODEXCREW_CLI"):
...
class TestLauncherInjectsIdentity:
    def test_child_env_carries_the_jobs_session_key(self):
        ...
        assert env.get("CODEXCREW_SESSION_KEY") == EXPECTED_KEY
    ...
    def test_key_is_the_one_scriptcontext_presents_over_http(self):
        ...
        with patch.dict(os.environ, {"_CODEXCREW_DIAL_PORT": "5486"}):
        ...
        with patch("codex_crew.cron_script.loopback_urlopen", side_effect=fake_urlopen):
            ctx._post("/api/send-message", {"text": "x"})
        assert captured["key"] == env["CODEXCREW_SESSION_KEY"]
    ...
    def test_a_forged_inherited_key_is_overwritten_not_kept(self, monkeypatch):
        ...
        monkeypatch.setenv("CODEXCREW_SESSION_KEY", "dashboard:someone-else")
        env = _capture_launcher_env(JOB_ID)
        assert env["CODEXCREW_SESSION_KEY"] == EXPECTED_KEY
...
class TestBridgePinsIdentityOnTheServerSpawn:
    def _spawn_env(self, session_key: str, spec_env: dict[str, str] | None = None):
        ...
        with (
            ...
            patch("codex_crew.cron_script.wrap_argv", return_value=(["some-mcp"], None)),
            patch("codex_crew.cron_script.cgroup_scope_argv", side_effect=lambda argv: list(argv)),
            patch(
                "codex_crew.cron_script.popen_limited", return_value=_handshake_proc()
            ) as mock_popen,
        ):
            client = McpToolClient("codexcrew-cron", session_key=session_key)
            client.close()
    ...
    def test_script_rewriting_its_own_environ_cannot_change_the_spawned_identity(self, monkeypatch):
        ...
        user code owns. A script that rewrites ``CODEXCREW_SESSION_KEY`` before
        ``ctx.call_tool`` must still spawn the server as ITS job.
        """
        monkeypatch.setenv("CODEXCREW_SESSION_KEY", "dashboard:victim")
    ...
    def test_spec_env_block_still_cannot_supply_the_key(self):
        ...
        env = self._spawn_env(EXPECTED_KEY, {"CODEXCREW_SESSION_KEY": "dashboard:victim"})
        assert env["CODEXCREW_SESSION_KEY"] == EXPECTED_KEY
    ...
    def test_call_tool_passes_the_jobs_key_to_the_bridge(self):
        ...
        with patch("codex_crew.cron_script.McpToolClient", return_value=fake_client) as ctor:
            ctx.call_tool("codexcrew-cron", "cron_list", {})
        ctor.assert_called_once_with("codexcrew-cron", session_key=EXPECTED_KEY)
...
class TestTheRealConsumerAcceptsIt:
    ...
    def test_cron_authz_gate_sees_the_job_not_an_unidentified_caller(self):
        from codex_crew.mcp_cron import _authz_session_key
