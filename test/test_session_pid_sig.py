"""Tests for :mod:`codex_crew.session_pid_sig` — signed session_pid publication.
...
from codex_crew import platform_compat, session_pid_sig

SESSION_KEY = "dashboard:chat-7-123456"
LOGGER_NAME = "codex_crew.session_pid_sig"
...
class TestVerify:
    def test_round_trip(self, cfg):
        session_pid_sig.publish_session_pid(4242, SESSION_KEY)
        assert session_pid_sig.verify_session_pid(4242) == SESSION_KEY
        # str pid (as read from CODEXCREW_HOST_PID) verifies identically.
        assert session_pid_sig.verify_session_pid("4242") == SESSION_KEY
...
class TestSigningHealth:
    """The diagnostic surface (`codexcrew doctor`) asks proactively; publication
    ...
    def test_never_constructs_the_sel_singleton(self, cfg):
        ...
        with patch("codex_crew.sel.SecurityEventLog") as sel_cls:
