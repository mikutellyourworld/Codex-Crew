"""Guards `codexcrew doctor`'s Discord section and the intent probe behind it.
...
* **Degrading to "unknown".** The probe is a network call inside a diagnostic.
  ...
  `codexcrew doctor` produces no report at the moment the operator needs one.
...
from codex_crew.config.loader import DiscordConfig, CodexCrewConfig
from codex_crew.discord import intent_probe
...
def _cfg(**kwargs: Any) -> CodexCrewConfig:
    cfg = CodexCrewConfig()
...
def _thread_cfg() -> CodexCrewConfig:
    return _cfg(
        enabled=True,
        allowed_user_ids=[_USER_ID],
        allowed_thread_ids=[_THREAD_ID],
...
class TestDoctorLiveConnection:
    ...
    def test_not_connected_without_a_reason_points_at_a_restart(self, monkeypatch, capsys) -> None:
        ...
        assert "codexcrew" in out and "restart" in out
        assert issues == []
