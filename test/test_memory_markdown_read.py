...
Covers ``codexcrew memory show`` (the documented-but-previously-missing
command) and ``codexcrew memory export --include-markdown``, plus the
...
class TestGuardedReadRobustness:
    ...
    def test_concurrent_rewrite_retries_to_consistent_version(
        ...
        from codex_crew import memory as memory_mod
...
class TestMemoryShowCli:
    ...
    def test_store_reads_where_the_consolidator_writes(self) -> None:
        ...
        from codex_crew.memory import MemoryStore, workspace_dir
...
# ── codexcrew memory export --include-markdown ──
...
class TestMemoryCliWiring:
    def test_memory_show_arguments_parse(self) -> None:
        ...
        with (
            ...
            patch("codex_crew.cli_commands._memory_cmd") as mock_cmd,
        ):
    ...
    def test_memory_show_target_optional_defaults(self) -> None:
        with (
            patch.object(sys, "argv", ["codexcrew", "memory", "show"]),
            patch("codex_crew.cli_commands._memory_cmd") as mock_cmd,
        ):
            from codex_crew.cli import main
    ...
    def test_memory_export_include_markdown_defaults_off(self) -> None:
        with (
            patch.object(sys, "argv", ["codexcrew", "memory", "export"]),
            patch("codex_crew.cli_commands._memory_cmd") as mock_cmd,
        ):
            from codex_crew.cli import main
...
class TestLiteralFtsQuery:
    ...
    def test_an_embedded_quote_is_escaped_not_injected(self) -> None:
        from codex_crew.memory import _fts5_literal_query
...
class TestOneEscapingDialect:
    ...
    def test_memory_and_knowledge_share_the_quoting_primitive(self) -> None:
        from codex_crew._sqlite_compat import fts5_quote_tokens
