...
def _patch_sel():
    mock_sel = MagicMock()
    with patch("codex_crew.dashboard.chat_handlers.sel", return_value=mock_sel):
        yield mock_sel
...
class TestSteerConsumedClears:
    ...
    def test_snapshot_settles_only_contained_steers(self, tmp_path, monkeypatch):
        from codex_crew.dashboard.chat_runner import _settle_consumed_steers
...
class TestSteerRegisteredBeforeAwait:
    ...
    async def test_failed_steer_already_requeued_by_finally_skips_fallback(
        ...
        async def _steer(message):
            ...
            from codex_crew.dashboard.chat_runner import _requeue_unconsumed_steers
...
class TestProductionWiring:
    ...
    def test_steer_handler_registers_before_await(self):
        ...
        import codex_crew.dashboard.chat_delivery as cd
...
class TestSteerLifecycleState:
    ...
    async def test_a_written_steer_row_is_not_marked_consumed(
        ...
        from codex_crew.dashboard.chat_delivery import STEER_STEERED, steer_into_running_turn
    ...
    async def test_a_consumed_echo_promotes_the_row(self, tmp_path, monkeypatch, _patch_sel):
        ...
        from codex_crew.dashboard.chat_delivery import steer_into_running_turn
        from codex_crew.dashboard.chat_runner import _settle_consumed_steers
    ...
    async def test_two_steers_with_identical_sanitized_content_are_left_alone(
        ...
        from codex_crew.dashboard.chat_delivery import sanitize_outbound, steer_into_running_turn
        from codex_crew.dashboard.chat_runner import _mark_steer_row_state
