...
from codex_crew.eval.bench.retrieval import RetrievalAggregate
...
def test_the_two_exclusion_reasons_are_counted_separately() -> None:
    ...
    from codex_crew.eval.bench.retrieval import aggregate
...
def test_the_summary_names_the_reason_that_actually_applies() -> None:
    ...
    from codex_crew.eval.bench.run import _excluded_phrase
...
def test_the_json_report_carries_both_counts() -> None:
    ...
    source = Path(
        __import__("codex_crew.eval.bench.run", fromlist=["run"]).__file__ or ""
