"""``codexcrew doctor`` reports Claude Code as a real optional agent backend.
...
from codex_crew.agent_sdk import INSTALLED, MISSING, UNKNOWN, BackendInstallState
...
def _report(monkeypatch, state, *, on_path: bool = True) -> str:
    ...
    monkeypatch.setattr("codex_crew.agent_sdk.probe_backend", _probe)
    monkeypatch.setattr(
        cli_doctor.shutil,
        "which",
