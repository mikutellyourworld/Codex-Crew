...
from codex_crew.knowledge.dedup import (
    DocRef,
    _match_reason,
    dedup_document,
    dedup_sweep,
...
class TestPriority:
    ...
    def test_format_rank_ordering(self):
        from codex_crew.knowledge.dedup import _format_rank
...
class TestOneDocumentManyLocations:
    ...
    def test_an_adopted_aggregate_document_stays_visible_to_the_duplicate_gate(self, tmp_path):
        ...
        """
        from codex_crew.knowledge.agent_source import find_document_by_hash
        store = _mk_store(tmp_path)
        try:
    ...
    def test_two_identical_artifacts_stay_independent_documents(self, tmp_path):
        ...
        """
        from codex_crew.knowledge.dedup import enumerate_docs
        store = _mk_store(tmp_path)
        try:
    ...
    def test_certain_only_applies_exact_matches_and_only_reports_fuzzy(self, tmp_path):
        ...
        from codex_crew.knowledge.dedup import DedupAction, DocRef, dedup_sweep
        store = _mk_store(tmp_path)
        ...
        try:
            ...
            import codex_crew.knowledge.dedup as dd
    ...
    def test_a_co_owned_loser_leaves_no_second_copy(self, tmp_path):
        ...
        store = _mk_store(tmp_path)
        try:
            from codex_crew.knowledge.dedup import _collapse_doc
