...
from codex_crew.dashboard.server import _start_site
...
async def test_start_site_retries_then_succeeds() -> None:
    ...
    with patch("codex_crew.dashboard.server.asyncio.sleep", new_callable=AsyncMock):
...
async def test_start_site_logs_warning_once() -> None:
    ...
    with (
        patch("codex_crew.dashboard.server.asyncio.sleep", new_callable=AsyncMock),
        patch("codex_crew.dashboard.server.logger") as mock_logger,
    ):
