...
SRC = Path(__file__).parent.parent / "src" / "codex_crew"
...
class TestDeployArtifactSchemaRegistration:
    def test_schema_registered_in_mcp_core_schemas(self):
        from codex_crew.validation import MCP_CORE_SCHEMAS
    ...
    def test_valid_input_passes_schema(self):
        from codex_crew.validation import DEPLOY_ARTIFACT_SCHEMA, validate_tool_args
    ...
    def test_redundant_inline_validate_removed_from_mcp_core(self):
        # The inline `from codex_crew.validation import DEPLOY_ARTIFACT_SCHEMA`
        # inside the deploy_artifact branch should be gone.
        assert "from codex_crew.validation import DEPLOY_ARTIFACT_SCHEMA" not in MCP_CORE
...
class TestDeploySkillsCopyNotSymlink:
    ...
    def test_skills_discoverable_after_copy(self):
        ...
        from codex_crew.skills import _iter_skill_files
