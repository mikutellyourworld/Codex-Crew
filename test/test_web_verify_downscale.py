...
class DownscaleTest(unittest.TestCase):
    ...
    def test_bundled_python_is_the_launcher_s_sibling(self) -> None:
        """bin/codexcrew -> bin/python, resolved THROUGH the launcher's symlink.
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            (bindir / "codexcrew").write_text("#!/bin/sh\n")
            (bindir / "python").write_text("#!/bin/sh\n")
            shim = str(Path(td) / "shim" / "codexcrew")
            with mock.patch.object(mod.shutil, "which", return_value=shim), \
                    mock.patch.object(mod.os.path, "realpath", return_value=str(bindir / "codexcrew")), \
    ...
    def test_bundled_python_uses_the_exe_suffix_on_windows(self) -> None:
        """Windows: Scripts/codexcrew.exe -> Scripts/python.exe. Only the suffix
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            (scripts / "codexcrew.exe").write_text("")
            (scripts / "python.exe").write_text("")
            with mock.patch.object(mod.shutil, "which", return_value=str(scripts / "codexcrew.exe")), \
    ...
    def test_cap_matches_the_prompt_block_ceiling(self) -> None:
        ...
        from codex_crew.acp.prompt_blocks import MAX_IMAGE_EDGE_PX
