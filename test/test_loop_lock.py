"""Tests for ``codex_crew.loop_lock.LoopBoundLock`` and its CI guard (#4800).
...
def test_concurrent_loops_emit_one_warning(caplog) -> None:
    ...
    with caplog.at_level(logging.WARNING, logger="codex_crew.loop_lock"):
...
def test_converted_registries_hand_out_loop_bound_locks() -> None:
    ...
    from codex_crew.dashboard.handlers import worktree
...
def test_guard_tree_is_clean(tmp_path) -> None:
    """The real run over src/codex_crew finds nothing — the declaration-form
    conversion is total."""
    proc = subprocess.run(
