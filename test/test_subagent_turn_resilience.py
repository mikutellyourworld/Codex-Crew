...
from codex_crew.providers.base import EVENT_COMPLETE, EVENT_TEXT_CHUNK
...
async def test_transient_error_pretoken_retries_same_prompt():
    ...
    with patch("codex_crew.subagent.transient_retry_delay", return_value=0.0):
...
async def test_transient_budget_exhausted_propagates():
    ...
    mgr = _manager(_mock_sessions(stream_factory))
    with (
        patch("codex_crew.subagent.transient_retry_delay", return_value=0.0),
        # fallback_model="" (disabled): this test pins the PRE-FEATURE budget
        # behavior — the default is now "auto", which would walk the chain.
        patch("codex_crew.subagent.configured_fallback_chain", return_value=()),
    ):
        info = await _spawn_and_wait(mgr)
...
async def test_throttle_fallback_chain_swaps_model_and_annotates():
    ...
    with (
        patch("codex_crew.subagent.transient_retry_delay", return_value=0.0),
        patch("codex_crew.subagent.configured_fallback_chain", return_value=("fb-1",)),
    ):
        info = await _spawn_and_wait(mgr)
...
async def test_throttle_fallback_chain_exhausted_propagates():
    ...
    from codex_crew.llm_helpers import FALLBACK_CANDIDATE_ATTEMPTS
...
async def test_throttle_fallback_ladder_routes_through_shared_budget_body():
    ...
    test_llm_helpers.py)."""
    from codex_crew.llm_helpers import FallbackState
...
async def test_throttle_fallback_story_survives_a_verbose_error():
    ...
    from codex_crew.subagent import _MAX_ERROR_DETAIL_LEN
...
async def test_user_cancel_is_neutral_stopped_with_partial():
    ...
    with patch("codex_crew.subagent.Stats") as stats, patch("codex_crew.subagent.sel"):
...
async def test_unexpected_cancel_after_tool_activity_finalizes_without_respawn():
    ...
    from codex_crew.acp.client import EVENT_TOOL_CALL
...
async def test_cancel_recovery_text_only_respawn_gets_resume_preamble():
    ...
    from codex_crew.subagent import _CANCEL_RESUME_PREFIX
...
async def test_delivered_orphan_survives_audit_and_tombstone_failures():
    ...
    with (
        patch("codex_crew.subagent.has_dashboard_surface", return_value=True),
        patch("codex_crew.subagent.sel", return_value=audit),
        patch(
            "codex_crew.subagent.write_tombstone", side_effect=OSError("disk unavailable")
        ) as write_tombstone,
    ):
...
async def test_cancel_recovery_failure_emits_done_and_delivers():
    ...
    with (
        ...
        patch("codex_crew.subagent._RECOVERY_SLOT_WAIT_SECS", 0.4),
    ):
...
def test_no_raw_cancel_outside_chokepoint():
    ...
    import codex_crew.subagent as subagent_mod
...
async def test_reconcile_multiple_orphans_sends_single_digest_dm():
    ...
    with (
        patch("codex_crew.subagent.list_orphans", return_value=orphans),
        patch("codex_crew.subagent.write_tombstone"),
        patch("codex_crew.subagent.sel"),
    ):
