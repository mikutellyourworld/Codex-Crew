...
``src/codex_crew/docs/discord-integration.md`` publishes for operators to paste.
...
from codex_crew.discord import install_url

_APP_ID = "123456789012345678"

_DOC = Path(__file__).resolve().parents[1] / "src" / "codex_crew" / "docs" / "discord-integration.md"
...
#: Discord's SEND_MESSAGES bit, deliberately NOT requested: a Codex Crew turn
...
class TestDocAgreement:
    ...
    def test_doc_url_matches_the_built_url(self) -> None:
        ...
        # the same URL, parameter for parameter, that Codex Crew builds.
        doc_urls = [
            line.strip()
