"""Tests for the per-crew-member space (``$CODEXCREW_HOME/members/<slug>/``).

``CODEXCREW_HOME`` is pinned to a per-test tmp dir by the autouse
``_isolate_codexcrew_home`` fixture, so every path here resolves under tmp.
"""
...
from codex_crew import members
...
class TestRecordActivity:
    ...
    def test_reports_failure_instead_of_raising(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.members.member_dir", lambda _s: (_ for _ in ()).throw(OSError("boom"))
        )
...
class TestRecordActivityRotation:
    ...
    def small_cap(self, monkeypatch):
        monkeypatch.setattr("codex_crew.members._ACTIVITY_LOG_MAX_BYTES", self.CAP)
