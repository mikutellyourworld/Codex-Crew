...
class TestUpDownCounter:
    def test_up_down_counter_records_a_negative_delta(self) -> None:
        ...
        rec.up_down_counter("codexcrew.pool.live", 3, attrs={"kind": "backend"})
        rec.up_down_counter("codexcrew.pool.live", -2)

        matching = [dp for name, dp in _points(reader) if name == "codexcrew.pool.live"]
    ...
    def test_second_emit_reuses_the_cached_instrument(self) -> None:
        ...
        rec.up_down_counter("codexcrew.pool.live", 1)
        rec.up_down_counter("codexcrew.pool.live", -1)
        assert created == ["codexcrew.pool.live"]
        assert added == [1, -1]
...
class TestFailuresAreSwallowed:
    def test_invalid_name_is_logged_not_raised(self, caplog: Any) -> None:
        ...
        with caplog.at_level(logging.WARNING, logger="codex_crew.metrics.recorder"):
    ...
    def test_instrument_add_failure_is_logged_not_raised(self, caplog: Any) -> None:
        ...
        with caplog.at_level(logging.WARNING, logger="codex_crew.metrics.recorder"):
            rec.up_down_counter("codexcrew.pool.live", 1)
            rec.counter("codexcrew.pool.spawns")
            rec.histogram("codexcrew.pool.latency", 5.0)
    ...
    def test_app_metric_cannot_spoof_the_core_namespace(self, caplog: Any) -> None:
        """``app_id`` set + a ``codexcrew.`` name is a namespace violation, and the
        facade must drop it rather than emit it."""
        rec, reader = _recorder_with_reader()
        with caplog.at_level(logging.WARNING, logger="codex_crew.metrics.recorder"):
            rec.up_down_counter("codexcrew.core.thing", 1, app_id="some_app")
        assert not [n for n, _ in _points(reader) if n == "codexcrew.core.thing"]
