...
async def test_registry_fresh_reinstall_checks_retained_startup_before_clone(
    ...
    from codex_crew.apps import hooks_integration
...
class TestApplyTrustFields:
    ...
    def test_external_entry_is_never_verified_despite_spoofed_fields(self):
        ...
        entry = {
            ...
            "author": "CodexCrew",       # brand-ok: author-spoof fixture
    ...
    def test_core_codexcrew_index_author_is_verified(self):
        """``verified`` derives from the INDEX-declared author snapshot
        (``_index_author``, taken by ``list_registry`` pre-merge)."""
        entry = {"name": "good-app", "_index_author": "CodexCrew"}  # brand-ok: author-spoof fixture
        (out,) = registry._apply_trust_fields([entry])
        assert out["provenance"] == "official"
        assert out["verified"] is True

    def test_manifest_author_alone_never_mints_verified(self):
        """A third-party core repo publishing ``"author": "codexcrew"`` in its
        app.json gains nothing: the merged ``author`` display field is not
        consulted, only the pre-merge index snapshot is."""
        entry = {"name": "sneaky", "author": "CodexCrew"}  # merged, no snapshot  # brand-ok: author-spoof fixture
        (out,) = registry._apply_trust_fields([entry])
        assert out["verified"] is False
        entry = {"name": "sneaky2", "author": "CodexCrew", "_index_author": "third-party"}  # brand-ok: author-spoof fixture
        (out,) = registry._apply_trust_fields([entry])
        assert out["verified"] is False
    ...
    def test_index_author_snapshot_never_leaks_into_payload(self):
        entry = {"name": "x", "_index_author": "CodexCrew"}  # brand-ok: author-spoof fixture
        (out,) = registry._apply_trust_fields([entry])
    ...
    def test_two_word_org_spelling_is_verified(self):
        ...
        entry = {"name": "spec-builder", "_index_author": "Codex Crew"}
        (out,) = registry._apply_trust_fields([entry])
        assert out["verified"] is True
    ...
    def test_folding_does_not_grant_the_mark_to_an_external_row(self):
        ...
        entry = {"name": "app", "_registry": "labs", "_index_author": "Codex Crew"}
        (out,) = registry._apply_trust_fields([entry])
        assert out["provenance"] == "external"
        assert out["verified"] is False
    ...
    def test_near_miss_author_is_not_verified(self):
        ...
        for name in ("codex crews", "kiro-crew", "codexcrew labs", "crew kiro"):
    ...
    async def test_list_registry_stamps_trust_fields(self, monkeypatch):
        ...
        ``author: "codexcrew"`` are all neutralized."""
        core = {"name": "core-app", "author": "CodexCrew", "featured": 1}  # brand-ok: author-spoof fixture
        ...
        preseed = {"name": "preseed-app", "_index_author": "CodexCrew"}  # brand-ok: author-spoof fixture
        ext = {
            "name": "ext-app",
            "_registry": "labs",
            "author": "CodexCrew",  # brand-ok: author-spoof fixture
        ...
        async def _fake_resolve(entry):
            ...
            if entry["name"] == "sneaky-app":
                return {**entry, "author": "CodexCrew"}  # brand-ok: author-spoof fixture
            return entry
...
async def test_python_build_uses_the_running_interpreter_not_path_pip(tmp_path, monkeypatch):
    ...
    routinely NOT the gateway's: ``bin/codexcrew`` execs ``.venv/bin/codexcrew`` without
...
class TestMergeManifestProjectsRegistryKeys:
    ...
    def test_index_author_snapshot_survives_the_merge(self):
        ...
        entry = {"name": "demo-app", "_index_author": "Codex Crew"}
        out = registry._merge_manifest(entry, self.MANIFEST)
        assert out["_index_author"] == "Codex Crew"
