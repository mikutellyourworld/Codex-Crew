"""Coverage tests for ``codex_crew.subagent`` helpers and manager bookkeeping.
...
from codex_crew import subagent as sa
...
class TestVetSpawnGovernance:
    def test_permitted_returns_none(self) -> None:
        with patch(
            "codex_crew.platform.governance_profiles.governance_permits",
    ...
    def test_agent_scope_denied(self) -> None:
        ...
        with patch(
            "codex_crew.platform.governance_profiles.governance_permits", side_effect=_permits
        ):
    ...
    def test_other_error_fails_closed_and_audits(self) -> None:
        ...
        with (
            ...
            patch(
                "codex_crew.platform.governance_profiles.audit_governance_degraded",
                audit,
            ),
        ):
            reason = sa._vet_spawn_governance("dash:1", "scout")
...
class TestSubagentRolePins:
    def test_model_pin_returns_normalized_value(self) -> None:
        ...
        with (
            patch("codex_crew.config.loader.CodexCrewConfig.load", return_value=cfg),
            patch("codex_crew.config.loader.normalize_agent_model", side_effect=str.strip),
        ):
            assert sa._subagent_default_model() == "sonnet"

    def test_model_pin_never_raises(self) -> None:
        with patch(
            "codex_crew.config.loader.CodexCrewConfig.load", side_effect=RuntimeError("no config")
        ):
...
class TestDigestHoldSecs:
    ...
    def test_nan_is_malformed_not_a_deadline(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("CODEXCREW_SUBAGENT_DIGEST_HOLD_SECS", "nan")
...
class TestNotifyInjectionFailed:
    def test_parent_without_a_tab_is_skipped(self) -> None:
        mgr = _manager(on_event=AsyncMock())
        with patch("codex_crew.dashboard.chat_utils.dashboard_slot_key", return_value=""):
    ...
    async def test_queues_failure_for_the_next_turn(self, tmp_path: Path) -> None:
        ...
        with patch("codex_crew.dashboard.chat_utils.dashboard_slot_key", return_value="slot-1"):
    ...
    def test_lookup_failure_is_swallowed(self) -> None:
        mgr = _manager(on_event=AsyncMock())
        with patch(
            "codex_crew.dashboard.chat_utils.dashboard_slot_key",
...
class TestShouldUseSessionSharing:
    def test_disabled_by_config(self) -> None:
        ...
        with patch("codex_crew.subagent.CodexCrewConfig.load", return_value=cfg):
    ...
    def test_config_failure_fails_closed(self) -> None:
        mgr = _manager()
        with patch("codex_crew.subagent.CodexCrewConfig.load", side_effect=RuntimeError):
            assert mgr._should_use_session_sharing(_info(parent_session_key="dash:1")) is False
...
class TestIsCcProvider:
    def test_delegates_to_backend_probe(self) -> None:
        with patch("codex_crew.providers.acp.is_claude_backend", return_value=True):
            assert SubagentManager._is_cc_provider(object()) is True

    def test_non_claude_backend(self) -> None:
        with patch("codex_crew.providers.acp.is_claude_backend", return_value=False):
