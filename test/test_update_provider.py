"""Tests for codex_crew.platform.update_provider."""
...
from codex_crew.platform import update_provider
...
class TestResolveProvider:
    ...
    def test_platform_only_policy_is_still_a_provider(self) -> None:
        ...
        with patch("codex_crew.platform.governance.active_update_pins", return_value=pins):
            provider = resolve_provider()
        assert isinstance(provider, CommandProvider)
...
class TestPlatformHelpers:
    ...
    def test_shell_exec_args_posix(self) -> None:
        ...
        with patch.object(sys, "platform", "linux"):
            with patch(
                "codex_crew.platform.update_provider.trusted_system_bin",
                return_value="/bin/sh",
            ):
...
class TestUpdatePinsCommandFields:
    ...
    def test_from_dict_command_non_string_raises(self) -> None:
        from codex_crew.platform.governance import PlatformCompositionError
...
class TestCommandProviderNoShellAndTimeout:
    ...
    async def test_check_no_trusted_shell(self) -> None:
        ...
        with patch(
            "codex_crew.platform.update_provider._shell_exec_args",
            return_value=None,
        ):
            result = await p.check()
    ...
    async def test_check_timeout_kills_proc(self) -> None:
        ...
        with (
            ...
            patch(
                "codex_crew.platform.update_provider.trusted_system_path",
    ...
    # version of this test stubbed the whole ``codex_crew.security`` module and
    ...
    def _install_policy(policy) -> None:
        ...
        from codex_crew.platform import PROFILE_ENTERPRISE, build_default_context, set_context

        set_context(
            dataclasses.replace(
                build_default_context(CodexCrewConfig(), profile=PROFILE_ENTERPRISE),
                credentials=policy,
            )
        )
    ...
    async def test_apply_failure_withholds_stderr_when_composition_fails(
        ...
        """
        from codex_crew.platform import PlatformCompositionError, reset_context
        from codex_crew.platform.context import LOG_WITHHELD_PLACEHOLDER
...
class TestCancellationKillsUpdaterChild:
    ...
    async def test_kill_and_reap_kills_the_whole_tree(self) -> None:
        ...
        proc.stderr = _stream(b"")
        proc.wait = AsyncMock(return_value=proc.returncode)
        with patch("codex_crew.platform_compat.kill_process_tree_async", AsyncMock()) as tree:
    ...
    async def test_kill_and_reap_bounds_the_reap(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with patch("codex_crew.platform_compat.kill_process_tree_async", AsyncMock()):
...
class TestManualEntryPointsHonourPolicy:
    """`POST /api/update` and `codexcrew update` must not run the built-in
    ...
    async def test_apply_policy_update_delegates_when_configured(self) -> None:
        ...
        with (
            patch("codex_crew.platform.governance.active_update_pins", return_value=pins),
...
class TestInstallerNeverLandsOnDisk:
    ...
    def test_download_failure_still_fails_the_command(self, tmp_path) -> None:
        ...
        from codex_crew.platform.update_layout import wheel_update_command
        from codex_crew.platform.update_provider import _shell_exec_args
...
class TestRedactionHappensBeforeTruncation:
    ...
    def test_both_sites_redact_before_slicing(self) -> None:
        ...
        from codex_crew.platform import update_provider as provider_mod
