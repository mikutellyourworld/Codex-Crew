...
def sandbox(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> SimpleNamespace:
    ...
    The agent-config sync (``codexcrew.json`` read-modify-write) belongs to
    ...
    monkeypatch.setattr(mcp_mod, "_CODEXCREW_MCP_JSON", tmp_path / "crew" / "mcp.json")
...
def _known(monkeypatch: pytest.MonkeyPatch, *names: str) -> None:
    ...
    import codex_crew.mcp_discovery as disc
...
class TestActive:
    ...
    async def test_default_agent_uses_the_global_scope_and_prepends_builtins(
        ...
        assert rows[0]["name"].startswith("codexcrew-")

    @pytest.mark.asyncio
    async def test_agent_alias_resolves_to_the_global_scope(
        self, sandbox: SimpleNamespace, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """A Codex Crew agent name bound to ``codexcrew`` reads the global scope."""
        import codex_crew.config.loader as loader

        monkeypatch.setattr(
            loader,
            "resolve_agent_bindings",
            lambda cfg, name: SimpleNamespace(kiro_agent="codexcrew"),
        )
        ...
        assert {r["name"] for r in _payload(resp)} >= {"on", "codexcrew-core"}
...
class TestLocalOverlayOwnsTheStubKeys:
    ...
    async def test_toggling_sharing_keeps_an_overlay_owned_allowlist(
        ...
        """
        from codex_crew.config.loader import config_local_path, config_path
...
def routed_allowlist(monkeypatch: pytest.MonkeyPatch):
    """Pin ``CodexCrewConfig.load().mcp_gateway.stub_servers``."""
    import codex_crew.config.loader as loader
    ...
    def _set(names: list[str], *, enabled: bool = False, forward_declared_env: bool = False) -> None:
        ...
        monkeypatch.setattr(loader.CodexCrewConfig, "load", staticmethod(lambda: cfg))
...
def _seed_probe(monkeypatch, *names: str) -> None:
    ...
    meta = SimpleNamespace(
        status="ok",
        capabilities={"experimental": {"codexcrew.caller-identity": {}}},
...
class TestGatewayServers:
    ...
    async def test_the_write_resolves_eligibility_against_the_state_it_writes_under(
        ...
        # ``codex_crew.config.loader`` inside the function body, so patching this
        ...
        from codex_crew.mcp_gateway.shareability import ShareVerdict, Strength
    ...
    async def test_denylisted_server_can_never_be_pooled(
        ...
        from codex_crew.mcp_gateway import rewriter
...
class TestStubEligibility:
    ...
    def test_a_denylisted_server_is_never_eligible(
        ...
        """
        import codex_crew.mcp_gateway.rewriter as rewriter_mod
    ...
    def test_a_measurement_of_a_replaced_command_does_not_count(
        ...
        from codex_crew.mcp_gateway.verdict_cache import CachedPreflight, load_cache
