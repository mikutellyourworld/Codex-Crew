...
def _mock_run_chat(monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.dashboard.chat_rewind._run_chat", run_chat_mock)
...
class TestRewindSlot:
    ...
    async def test_rewind_failed_save_keeps_live_and_persisted_branch_during_flush(
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_rewind._save_slot_to_history", _wait_then_fail
        )
        app = _make_app(state)
        async with TestClient(TestServer(app)) as client:
    ...
    async def test_rewind_rejects_when_the_boundary_cannot_be_saved(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_rewind._save_slot_to_history", _fail_save)
        app = _make_app(state)
        async with TestClient(TestServer(app)) as client:
            resp = await client.post(
                "/api/chat/slots/src/rewind",
    ...
    async def test_rewind_rejects_when_the_save_is_refused(self, tmp_path, monkeypatch):
        ...
        state.sessions._session_map.get = MagicMock(return_value="")
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_rewind._save_slot_to_history",
    ...
    async def test_rewind_pairs_the_snapshot_with_the_frozen_prefix_boundary(
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_rewind._save_slot_to_history", _record_pairing
        )
    ...
    async def test_rewind_refuses_when_a_cap_trim_moves_the_frozen_prefix(
        ...
        """
        monkeypatch.setattr("codex_crew.dashboard.state._MAX_SLOT_MESSAGES", 4)
        state = _make_state(tmp_path)
        slot = _populate_slot(state)
        state.sessions._session_map.get = MagicMock(return_value="")
    ...
    async def test_rewind_cancelled_mid_save_still_commits_the_landed_rewrite(
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_rewind._save_slot_to_history", _gated_save)
    ...
    async def test_rewind_deletes_orphan_kiro_session(self, tmp_path):
        ...
        app = _make_app(state)
        with patch(
            "codex_crew.dashboard.chat_rewind._delete_orphan_kiro_session",
            new=AsyncMock(),
        ) as mock_delete:
            async with TestClient(TestServer(app)) as client:
                resp = await client.post(
