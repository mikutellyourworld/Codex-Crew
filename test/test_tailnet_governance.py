...
class TestScopeCatalog:
    ...
    def test_row_is_registered_as_a_default_on_capability(self) -> None:
        from codex_crew.platform.governance import CAPABILITY, SCOPE_CATALOG
    ...
    def test_a_policy_can_actually_express_the_pin(self) -> None:
        ...
        from codex_crew.platform.governance import CapabilityGate, parse_policy
...
class TestGovernancePin:
    ...
    def test_a_profile_layer_deny_on_a_governed_host_is_also_not_a_pin(
        ...
        from codex_crew.platform import governance_profiles as gp
        from codex_crew.platform.governance import CapabilityGate, Profile
...
def _status_app(host: str = "", resolved_at: int = 0) -> web.Application:
    from codex_crew.dashboard.handlers import api_tailnet_status
...
async def _get_status(monkeypatch, *, enabled: bool, pinned: bool, host: str, resolved_at: int = 0):
    from codex_crew.dashboard.handlers import tailnet as handler_mod

    # The handler resolves the probe through ``codex_crew.dashboard.tailnet``, so
...
class TestStatusState:
    ...
    def test_state_precedence_is_a_pure_function(self) -> None:
        from codex_crew.dashboard.handlers.tailnet import _derive_state
...
def _patch_app() -> web.Application:
    from codex_crew.dashboard.handlers import api_codexcrew_config_patch

    app = web.Application()
    app.router.add_patch("/api/config/codexcrew", api_codexcrew_config_patch)
    return app
...
def tmp_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.config.loader.config_path", lambda: cfg_path)
    return cfg_path
...
class TestPatchChokepoint:
    ...
    async def test_enable_is_refused_with_403(self, tmp_config, monkeypatch) -> None:
        ...
        async with TestClient(TestServer(_patch_app())) as c:
            resp = await c.patch(
                "/api/config/codexcrew", json={"path": "dashboard.tailscale.enabled", "value": True}
            )
    ...
    async def test_disable_is_still_allowed_under_a_pin(self, tmp_config, monkeypatch) -> None:
        ...
        async with TestClient(TestServer(_patch_app())) as c:
            resp = await c.patch(
                "/api/config/codexcrew", json={"path": "dashboard.tailscale.enabled", "value": False}
            )
            assert resp.status == 200
    ...
    async def test_non_bool_is_rejected(self, tmp_config) -> None:
        async with TestClient(TestServer(_patch_app())) as c:
            resp = await c.patch(
                "/api/config/codexcrew", json={"path": "dashboard.tailscale.enabled", "value": "on"}
            )
            assert resp.status == 400
...
class TestCliChokepoint:
    """``codexcrew config set`` is a second write path, and ``--local`` writes the
...
class TestBothStartupSitesStash:
    ...
    def test_the_route_is_registered(self) -> None:
        ...
        from codex_crew.dashboard import routes as routes_pkg
        from codex_crew.dashboard import server as server_mod
...
class TestRevokingAPublishedOriginWhenTheCeilingTightens:
    ...
    def test_a_failed_withdrawal_is_reported_and_does_not_raise(self, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.ERROR, logger="codex_crew.dashboard.tailnet_serve"):
