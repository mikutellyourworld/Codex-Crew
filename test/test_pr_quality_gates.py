...
class TestCrossPlatform:
    ...
    def test_excludes_vendor_and_compat_module(self):
        wf = _read("cross-platform.yml")
        assert ":(exclude)src/codex_crew/_vendor/**" in wf
        assert ":(exclude)src/codex_crew/platform_compat.py" in wf
...
class TestPrScopeMeasureLogic:
    ...
    def test_measured_diff_reports_scope(self, tmp_path):
        result, summary = self._run_measure(
            tmp_path,
            files_out="src/codex_crew/session/state.py\n",
            numstat_out="10\t2\tsrc/codex_crew/session/state.py\n",
        )
        assert result.returncode == 0, result.stdout + result.stderr
