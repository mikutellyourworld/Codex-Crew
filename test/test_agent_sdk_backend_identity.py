...
:func:`codex_crew.agent_sdk.backend_identity.is_claude_backend_name`; the client
...
from codex_crew.acp_backends import ACP_BACKEND_CLAUDE, ACP_BACKEND_KAS, ACP_BACKEND_KIRO
...
def test_module_reaches_only_the_leaf_constants() -> None:
    ...
    ``acp_backends`` is a documented leaf. An import of ``codex_crew.acp`` or
    ``codex_crew.providers`` here would put the consolidation point inside the
    ...
    assert [m for m in imported if m.startswith("codex_crew")] == ["codex_crew.acp_backends"]
...
def test_importing_this_module_does_not_load_acp_or_providers() -> None:
    ...
    probe = (
        ...
        "loaded = {\n"
        "    'acp': 'codex_crew.acp' in sys.modules,\n"
        "    'providers': 'codex_crew.providers' in sys.modules,\n"
        "    'acp_backends': 'codex_crew.acp_backends' in sys.modules,\n"
        "}\n"
        "print(repr(loaded))\n"
    )
    ...
    assert loaded["acp"] is False, "importing the helper loaded codex_crew.acp"
    assert loaded["providers"] is False, "importing the helper loaded codex_crew.providers"
...
def test_acp_client_keeps_its_own_comparison() -> None:
    ...
    ``agent_sdk`` sits above ``codex_crew.acp``, so routing the client through the
...
def test_the_property_and_the_session_predicate_agree(backend: str, expected: bool) -> None:
    ...
    from codex_crew.session import _is_claude_backend
