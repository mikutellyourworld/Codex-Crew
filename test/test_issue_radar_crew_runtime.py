...
from codex_crew.apps.builtins.issue_radar.backend import crew_runtime as cr
from codex_crew.apps.builtins.issue_radar.backend import crew_store as cs
from codex_crew.apps.builtins.issue_radar.backend import provider
from codex_crew.apps.builtins.issue_radar.backend import store as store_mod
from codex_crew.apps.builtins.issue_radar.backend import watch as watch_mod
...
class TestCrewStoreScoping(unittest.TestCase):
    ...
    def test_no_unscoped_crew_store_call_survives(self):
        from codex_crew.apps.builtins.issue_radar.backend import crew_routes
...
class TestSession(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_session_key_agent_workspace_and_model_come_from_the_record(self):
        crew = _crew(self.root, agent="codexcrew", model="claude-opus-5")
        state = _FakeState()
        ...
        self.assertEqual(created["agent"], "codexcrew")
    ...
    async def test_a_grant_whose_audit_fails_is_never_usable(self):
        ...
        crew = _crew(self.root, unattended=True)
        with mock.patch(
            "codex_crew.safety_override.sel", side_effect=OSError("SEL unavailable")
        ):
            slot = await cr.ensure_crew_session(_FakeState(), OWNER, REPO, crew)
        self.assertFalse(_effectively_trusted(slot))
...
class TestDismissal(unittest.IsolatedAsyncioTestCase):
    ...
    def _teardown_mod(self):
        ...
        """
        from codex_crew.apps import teardown
    ...
    async def test_the_watcher_registers_the_hook_before_it_ever_sweeps(self):
        ...
        """
        from codex_crew.apps.builtins.issue_radar.backend import watch
...
class TestDisablingTheAppRevokesInline(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_the_sweep_still_suspends_a_flag_flipped_behind_our_back(self):
        """The BACKSTOP. ``codexcrew app disable`` runs in another process and an
...
class TestSelRootIsolation(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        if getattr(sel_mod._default_dir, "__module__", "") == "codex_crew.sel":
