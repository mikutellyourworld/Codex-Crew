...
from codex_crew.dashboard.handlers.sessions import api_approval_resolve
...
from codex_crew.hooks import HOOK_EVENT_PRE_TOOL_USE, ScriptHookResult, ToolHookResult
...
@contextmanager
def _patch_stats():
    with patch("codex_crew.dashboard.chat.sel") as mock_sel:
...
class TestApprovalModes:
    ...
    async def test_auto_approve_still_fires_pretooluse_script_hook(self, tmp_path):
        ...
        from codex_crew.hooks import HOOK_EVENT_PRE_TOOL_USE
...
class TestDenyOnce:
    ...
    async def test_both_audit_event_types_agree_on_the_denial(self, tmp_path):
        ...
        with patch("codex_crew.dashboard.state.sel") as mock_sel:
            mock_sel.return_value = MagicMock()
    ...
    async def test_deny_once_is_distinguishable_in_the_audit_log(self, tmp_path):
        ...
        with patch("codex_crew.dashboard.chat_runner.sel") as mock_sel:
            mock_sel.return_value = MagicMock()
            with _patch_stats():
                await _run_chat(state, slot, "hello")
            await _drain(answerer)
...
class TestBackgroundApprovalDenyFast:
    ...
    async def test_background_uses_short_window_and_denies(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.state.asyncio.wait_for", _fake_wait_for)
...
class TestRefusalRecovery:
    ...
    the turn via kiro-cli's tool-interrupted marker. CodexCrew should hand the
...
class TestDenyRowTitleRedaction:
    ...
    async def test_trust_reads_invalid_name_redacts(self, tmp_path):
        ...
        with patch("codex_crew.dashboard.chat_runner.sel") as mock_sel, _no_refusal:
            audit = MagicMock()
            mock_sel.return_value = audit
