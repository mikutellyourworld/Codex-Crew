...
``kiro_agent`` used to default to ``"codexcrew"`` when a create request omitted
...
* ``"codexcrew"`` stays a perfectly legal explicit CHOICE, because a crew that
...
def _fake_config():
    """A stand-in CodexCrewConfig recording whether save() was reached."""
    saved: list[bool] = []
...
def _make_app() -> web.Application:
    from codex_crew.dashboard.handlers.agents import api_codexcrew_agents_create
...
async def _post(body, cfg, installed=(), spy=None):
    ...
    *spy* to observe whether the listing was consulted at all.
    ...
    with (
        ...
        patch(
            "codex_crew.dashboard.handlers.agents.list_agents",
            new=_list_agents,
        ),
        patch(
            "codex_crew.dashboard.handlers.agents._sel",
...
class TestExplicitTemplateStillWorks:
    @pytest.mark.asyncio
    async def test_builtin_codexcrew_is_a_legal_choice(self):
        """Only the silent DEFAULT is refused — the value itself is legitimate."""
        cfg = _fake_config()
        status, data = await _post(
            {"name": "researcher", "kiro_agent": "codexcrew", "workspace": "research"},
            cfg,
            installed=("codexcrew",),
        )
        assert status == 200
        assert data["ok"] is True
        assert cfg.agents["researcher"].kiro_agent == "codexcrew"
