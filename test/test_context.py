...
from codex_crew.context import ContextBuilder, _neutralize_structural_markers
from codex_crew.hooks import ContextRule, HookManager, HooksConfig
...
class TestContextBuilder:
    ...
    def test_build_message_injects_folder_breadcrumb(self, tmp_path):
        ...
        msg, _ = builder.build_message(
            "hello", is_new_session=False, folder_path="CodexCrew › Backend"
        )
        assert "[FOLDER]" in msg
        assert "CodexCrew › Backend" in msg
    ...
    def test_hook_modify(self, tmp_path):
        from codex_crew.hooks import TransformHook
    ...
    def test_history_budget_limits_total_chars(self, tmp_path):
        ...
        from codex_crew.context import _HISTORY_BUDGET_CHARS
        from codex_crew.history import ConversationLog
...
class TestGetMemoryForVectorStore:
    ...
    def test_nondefault_store_shares_vector_store(self, tmp_path, monkeypatch):
        ...
        import codex_crew.context as ctx_mod
...
class TestCompressAssistantMessage:
    ...
    def test_small_code_block_preserved(self):
        from codex_crew.context import _compress_assistant_message
...
class TestDocsSection:
    def test_docs_section_present_when_docs_exist(self, tmp_path, monkeypatch):
        ...
        from codex_crew import context as ctx_mod
    ...
    def test_docs_injected_for_codexcrew_agent(self, tmp_path, monkeypatch):
        """build_session_context includes docs for the default codexcrew agent."""
        from codex_crew import context as ctx_mod
    ...
    def test_docs_not_injected_for_custom_agent(self, tmp_path, monkeypatch):
        """build_session_context skips docs for custom (non-codexcrew) agents."""
        from codex_crew import context as ctx_mod
...
class TestCompressThreadHistory:
    ...
    async def test_returns_none_when_no_history(self, tmp_path):
        from codex_crew.context import compress_thread_history
        from codex_crew.history import ConversationLog
    ...
    async def test_long_transcript_calls_llm(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.llm_helpers.stream_and_collect",
...
class TestRuntimeDisplayName:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("dashboard:chat-1-100", "CodexCrew dashboard"),
            ("dashboard_chat-1-100", "CodexCrew dashboard"),
            ("cron:daily", "CodexCrew cron job"),
            ("cron_076ab486", "CodexCrew cron job"),
            ("subagent:abc-123", "CodexCrew subagent"),
            ("taskrunner:proj:task1", "CodexCrew task runner"),
            ("_bg", "CodexCrew background"),
            ("_hb", "CodexCrew heartbeat"),
            ("cli_chat", "CLI terminal"),
            ("slack:1234567890.123456", "Slack"),
            ("discord:codexcrew:direct:474737235959480320", "Discord"),
            ("discord_codexcrew_direct_474737235959480320", "Discord"),
            ("telegram:codexcrew:direct:123", "Telegram"),
            ("wecom:codexcrew:direct:user@example.com", "WeCom"),
            ("weixin:codexcrew:direct:wxid", "Weixin"),
            ("webex:codexcrew:direct:user@example.com", "Webex"),
            ("teams:codexcrew:direct:user@example.com", "Microsoft Teams"),
            ("1234567890.123456", "Slack"),
        ],
    )
    def test_runtime_display_name(self, session_key, expected_runtime):
        from codex_crew.context import _runtime_display_name
    ...
    def test_agent_identity_injected_with_session_key(self, tmp_path):
        ...
        assert "[RUNTIME] CodexCrew dashboard" in ctx
    ...
    def test_agent_defaults_to_codexcrew(self, tmp_path):
        """Agent label defaults to 'codexcrew' when agent param is None."""
        builder = ContextBuilder(memory=MemoryStore(workspace=tmp_path))
        ctx = builder.build_session_context("dashboard:chat-1")
        assert "[CURRENT AGENT] codexcrew" in ctx
    ...
    def test_explicit_runtime_source_overrides_stable_session_key(self, tmp_path):
        ...
        assert "[RUNTIME] CodexCrew dashboard" not in ctx
...
class TestMultibyteSanitization:
    ...
    def test_multibyte_table_covers_all_chars(self):
        ...
        from codex_crew.context import _MULTIBYTE_TABLE
...
class TestCurrentDateTimezone:
    """[CURRENT DATE] injection must honour CodexCrewConfig.timezone, so LLMs
    ...
    def test_current_date_uses_configured_timezone(self, tmp_path):
        ...
        with patch("codex_crew.cron.published_config_timezone", return_value="Asia/Tokyo"):
            ctx = builder.build_session_context()
    ...
    def test_current_date_falls_back_to_utc_when_config_empty(self, tmp_path):
        builder = self._make_builder(tmp_path)
        with patch("codex_crew.cron.published_config_timezone", return_value=""):
            ctx = builder.build_session_context()
        date_line = [ln for ln in ctx.splitlines() if ln.startswith("[CURRENT DATE]")][0]
...
class TestLoadSteeringResources:
    ...
    def test_skips_sensitive_paths(self, tmp_path):
        ...
        (agents_dir / "codexcrew.json").write_text(json.dumps({"resources": ["file://.ssh/*.md"]}))
...
class TestLessonsCap:
    def test_over_cap_injects_error_block(self, tmp_path):
        from codex_crew.context import _LESSONS_CAP
        from codex_crew.learn import Lesson
...
class TestKeepVisibleMarkerRule:
    ...
    def test_marker_documented_in_dashboard_rules_only(self):
        from codex_crew.context import _CRITICAL_RULES, _CRITICAL_RULES_CHANNEL
