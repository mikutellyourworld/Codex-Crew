...
import codex_crew.autonudge_authz as autonudge_authz
...
async def test_authz_rejects_spoofed_discord_session() -> None:
    ...
    disp = FakeDiscordDispatcher(
        authorized={"42"}, current={"42": "discord:codexcrew:direct:42"}
    )
    loop, error, status = await authorize_and_add_nudge(
        svc=svc,
        state=_state(discord=disp),
        slot_key="discord:codexcrew:direct:99",  # someone else's user id
        message="pwn",
        source="workflow",
    )
...
async def test_author_prompt_advertises_only_wired_primitives() -> None:
    ...
    """
    from codex_crew.workflows.service import _AUTHOR_SYSTEM
