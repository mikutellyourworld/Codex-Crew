"""Tests for codex_crew.imessage.transport_dispatch (IMessageDispatcher)."""
...
from codex_crew.imessage.transport_dispatch import IMessageDispatcher
...
def _permit(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codex_crew.imessage.transport_dispatch.inbound_permitted", _yes)
...
class TestCommandIntercept:
    ...
    async def test_a_governance_deny_drops_the_message_silently(
        ...
        monkeypatch.setattr("codex_crew.imessage.transport_dispatch.inbound_permitted", _no)
        dispatcher, client, _ = _dispatcher()
        await dispatcher.handle_message(_inbound("/help"))
        assert client.sent == []
...
class TestSessionIdentity:
    ...
    def test_the_agent_falls_back_to_the_canonical_codexcrew_agent(self) -> None:
        # Otherwise the session loads kiro-cli's bare default and has no
        # spawn_run / cron tools.
        dispatcher, _, _ = _dispatcher()
        assert dispatcher._resolve_agent() == "codexcrew"
