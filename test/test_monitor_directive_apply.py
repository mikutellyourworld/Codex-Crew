...
async def test_disabled_structured_monitor_directives_are_audited_as_denied(kind):
    audit = MagicMock()
    with (
        patch("codex_crew.autonudge.get_instance", return_value=None),
        patch("codex_crew.sel.sel", return_value=audit),
    ):
...
async def test_unsupported_structured_monitor_stop_is_audited_as_denied(tmp_path):
    ...
    audit = MagicMock()
    with (
        patch("codex_crew.autonudge.get_instance", return_value=service),
        patch("codex_crew.dashboard.session_directive_apply._audit", audit),
    ):
        result = await apply_session_directive(
            SimpleNamespace(),
...
async def test_webex_structured_watch_is_refused_by_authoritative_consumer(tmp_path):
    service = AutoNudgeService(base_dir=tmp_path)
    audit = MagicMock()
    session_key = "webex:codexcrew:direct:operator@example.com"
    with (
        patch("codex_crew.autonudge.get_instance", return_value=service),
        patch("codex_crew.dashboard.session_directive_apply._audit", audit),
    ):
...
async def test_refused_structured_monitor_stop_is_audited_as_denied(tmp_path):
    ...
    with (
        patch("codex_crew.autonudge.get_instance", return_value=service),
        patch(
            "codex_crew.autonudge_authz.authorize_and_stop_monitor",
...
async def test_watch_update_and_stop_are_authoritative_and_owned(tmp_path):
    ...
    with (
        patch("codex_crew.autonudge.get_instance", return_value=service),
        patch("codex_crew.autonudge_authz.sel", return_value=audit),
    ):
...
async def test_identity_update_conflict_is_a_controlled_authorizer_denial(tmp_path):
    ...
    with patch("codex_crew.autonudge_authz.sel", return_value=MagicMock()):
...
async def test_banner_cannot_silently_patch_a_structured_monitor(tmp_path):
    ...
    with (
        patch("codex_crew.autonudge.get_instance", return_value=service),
        patch("codex_crew.autonudge_authz.sel", return_value=MagicMock()),
    ):
        result = await apply_session_directive(
            state,
            slot,
