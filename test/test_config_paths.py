"""Tests for the pure path-primitives leaf ``codex_crew.config.paths``.
...
1. The path primitives behave identically to their historical
   ``codex_crew.config.loader`` definitions (back-compat).
2. ``codex_crew.config.paths`` is a genuine leaf — importing it pulls in **no**
   ``codex_crew`` modules (in particular not the heavy ``config.loader``), so the
...
class TestConfigDir:
    """``config_dir()`` resolves ~/.codex-crew, honoring CODEXCREW_HOME."""
    ...
    def _reset_resolved_home(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        # patched Path.home / CODEXCREW_HOME rather than a value another test cached.
        monkeypatch.setattr(paths, "_resolved_home", None)

    def test_default_is_home_dotcodex_crew(
    ...
    def test_codexcrew_home_override(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    def test_codexcrew_home_system_dir_is_ignored(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        # A system directory must be refused and fall back to ~/.codex-crew.
        monkeypatch.setenv("CODEXCREW_HOME", "/usr")
        monkeypatch.setattr(Path, "home", classmethod(lambda cls: tmp_path))
        result = paths.config_dir()
        assert result == tmp_path / ".kiro" / "crew"
...
class TestConfigPackageDir:
    """``config_package_dir()`` points at the installed ``codex_crew/config/``."""
...
class TestLeafPurity:
    ...
    Importing ``codex_crew.config.paths`` in a fresh interpreter must NOT import
    ``codex_crew.config.loader`` (or any other ``codex_crew`` submodule). Run in a
    subprocess so the already-warm modules in this test process don't mask a
    regression.
    """

    def test_importing_paths_pulls_no_codex_crew_modules(self) -> None:
        code = (
            "import sys\n"
            "import codex_crew.config.paths\n"
            "leaked = sorted(\n"
            "    m for m in sys.modules\n"
            "    if m.startswith('codex_crew')\n"
            "    and m not in {'codex_crew', 'codex_crew.config', 'codex_crew.config.paths'}\n"
            ")\n"
            "print(','.join(leaked))\n"
        )
        import os

        # Ensure codex_crew is importable in the subprocess on local dev runs
        ...
        assert leaked == [], f"config.paths leaf leaked codex_crew modules: {leaked}"


class TestBackCompatReexport:
    """All primitives remain importable from ``codex_crew.config.loader``."""
    ...
    def test_config_package_lazy_surface(self) -> None:
        # `from codex_crew.config import X` still resolves the public surface
        # without eagerly importing the loader at package import time.
        import codex_crew.config as cfg

        assert cfg.config_dir is paths.config_dir
        assert cfg.CodexCrewConfig.__name__ == "CodexCrewConfig"
