"""Hang-resilience telemetry — the codexcrew.* series added after the silent
...
from codex_crew.acp.runtime import AcpRuntime, JsonRpcMessage
from codex_crew.acp.session_handle import AcpSessionHandle
from codex_crew.metrics import events as metric_events
...
def recorded(monkeypatch):
    ...
    for mod in (
        "codex_crew.metrics.events",
        "codex_crew.acp.runtime",
        "codex_crew.acp.session_handle",
        "codex_crew.subagent",
        "codex_crew.session",
        "codex_crew.dashboard.chat_runner",
    ):
...
def test_emit_counter_never_raises():
    ...
    with patch(
        "codex_crew.metrics.provider.get_recorder",
        side_effect=RuntimeError("recorder down"),
    ):
        metric_events.emit_counter("codexcrew.test", {"a": 1})  # no raise
...
def test_dashboard_ceiling_emits_timeout_cause(recorded):
    ...
    from codex_crew.dashboard import turn_dispatch
