...
class TestSpawnSubAgents:
    def test_spawns_agents_and_collects_results(self):
        with patch("codex_crew.mcp_core._post") as mock_post, \
             patch("codex_crew.mcp_core._get") as mock_get, \
             patch("codex_crew.mcp_core.sel"), \
             patch.dict("os.environ", {"CODEXCREW_SESSION_KEY": "sess1"}):
    ...
    def test_returns_error_for_empty_agents(self):
        with patch("codex_crew.mcp_core.sel"), \
             patch.dict("os.environ", {"CODEXCREW_SESSION_KEY": "s"}):
    ...
    def test_reports_timed_out_agents(self):
        with patch("codex_crew.mcp_core._post") as mock_post, \
             patch("codex_crew.mcp_core._get") as mock_get, \
             patch("codex_crew.mcp_core.time") as mock_time, \
             patch("codex_crew.mcp_core.sel"), \
             patch.dict("os.environ", {"CODEXCREW_SESSION_KEY": "s"}):
            mock_post.return_value = {"id": "a1"}
    ...
    def test_max_wait_configurable_via_env(self):
        ...
        with patch("codex_crew.mcp_core._post") as mock_post, \
             ...
             patch("codex_crew.mcp_core.sel"), \
             patch.dict("os.environ", {"CODEXCREW_SESSION_KEY": "s",
                                       "CODEXCREW_SPAWN_SUB_AGENTS_MAX_WAIT": "120"}):
            mock_post.return_value = {"id": "a1"}
            mock_get.return_value = {"done": False, "agent": "slow"}
...
class TestSpawnSubAgentsSummarization:
    ...
    def test_large_result_summarized_with_path(self):
        ...
        with patch("codex_crew.mcp_core._post") as mock_post, \
             patch("codex_crew.mcp_core._get") as mock_get, \
             patch("codex_crew.mcp_core.sel"), \
             patch("codex_crew.mcp_core.summarize_result") as mock_summarize, \
             patch.dict("os.environ", {"CODEXCREW_SESSION_KEY": "s"}):
            ...
            mock_summarize.return_value = (
                "Full transcript: /home/user/.codexcrew/subagents/agent123/result.txt\n"
    ...
    def test_invalid_max_wait_env_falls_back(self):
        with patch("codex_crew.mcp_core._post") as mock_post, \
             ...
             patch.dict("os.environ", {
                 "CODEXCREW_SESSION_KEY": "s",
                 "CODEXCREW_SPAWN_SUB_AGENTS_MAX_WAIT": "not-a-number",
             }):
            mock_post.return_value = {"id": "a1"}
            mock_get.return_value = {"done": True, "agent": "w", "result": "ok"}
...
class TestSpawnList:
    def test_spawn_list_renders_running_and_done_agents(self):
        with patch("codex_crew.mcp_core._get") as mock_get, \
             patch("codex_crew.mcp_core.list_agents", return_value=[]), \
             patch("codex_crew.mcp_core.sel"), \
             patch.dict("os.environ", {"CODEXCREW_SESSION_KEY": "s"}):
...
class TestSpawnSubAgentsAuditOwner:
    ...
    def test_unresolved_owner_is_marked_in_audit_records(self):
        with patch("codex_crew.mcp_core._post") as mock_post, \
             ...
             patch("codex_crew.mcp_core.sel") as mock_sel:
            mock_post.return_value = {"id": "a1"}
            mock_get.return_value = {"done": True, "agent": "w", "result": "ok"}
    ...
    def test_resolved_owner_is_recorded_verbatim(self):
        ...
        with patch("codex_crew.mcp_core._post") as mock_post, \
             patch("codex_crew.mcp_core._get") as mock_get, \
             patch("codex_crew.mcp_core._resolve_session_key", return_value="dashboard:tab7"), \
             patch("codex_crew.mcp_core.sel") as mock_sel:
            mock_post.return_value = {"id": "a1"}
            mock_get.return_value = {"done": True, "agent": "w", "result": "ok"}
