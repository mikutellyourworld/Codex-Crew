"""Tests for codexcrew-lite duplicate agent prevention."""
...
from codex_crew.agent import _LITE_AGENT_FILENAME, _install_lite_agent_fallback
...
AIM_LITE_FILENAME = "CodexCrewAICapabilities-codexcrew-lite.json"


class TestLiteAgentDuplicate:
    """Prevent duplicate codexcrew-lite agent configs (bare + AIM-installed)."""
    ...
    def test_fallback_writes_even_when_legacy_aim_file_present(self, agents_dir: Path) -> None:
        ...
        present — the bare ``codexcrew-lite.json`` is always written for the
        claude_code provider's cheap background agent.
        """
        aim_file = agents_dir / AIM_LITE_FILENAME
        aim_file.write_text(json.dumps({"name": "codexcrew-lite"}))
