...
from codex_crew.dashboard import server as dashboard_server
from codex_crew.dashboard import token_auth
...
class TestPerPortCredentialFile:
    def test_path_is_keyed_by_port_and_sits_beside_the_marker(self, home: Path) -> None:
        assert run_marker.secret_path(5486).name == "gateway-5486.secret"
        assert run_marker.secret_path(5486).parent == run_marker.marker_path(5486).parent

    def test_read_returns_empty_when_absent(self, home: Path) -> None:
        assert run_marker.read_secret(5486) == ""

    def test_read_does_not_create_the_run_dir(self, home: Path) -> None:
        run_marker.read_secret(5486)
    ...
    def test_cleared_with_the_marker_so_a_dead_generation_leaves_no_credential(
        ...
        run_marker.write_marker(5486)
        dashboard_server._write_secret_file(run_marker.secret_path(5486), "deadbeef")
        run_marker.clear_marker(5486)
        assert run_marker.read_secret(5486) == ""
...
class TestSharedFileIsNotClobberedWhileASiblingServes:
    def test_shared_file_written_when_this_is_the_only_gateway(self, home: Path) -> None:
        ...
        with mock.patch.object(dashboard_server, "_live_sibling_port", return_value=None):
            dashboard_server._write_instance_credentials(shared, 5486, "mine")
        assert shared.read_text() == "mine"
        assert run_marker.read_secret(5486) == "mine"

    def test_shared_file_preserved_when_another_gateway_is_live(self, home: Path) -> None:
        shared = home / ".local_secret"
        shared.write_text("incumbent")
        with mock.patch.object(dashboard_server, "_live_sibling_port", return_value=5486):
    ...
    def test_a_stale_marker_is_not_a_sibling(self, home: Path) -> None:
        ...
        run_marker.write_marker(5486)
        with mock.patch(
            "codex_crew.port_resolution._gateway_owns_port", return_value=False
        ):
    ...
    def test_a_verified_live_marker_is_a_sibling(self, home: Path) -> None:
        run_marker.write_marker(5486)
        with mock.patch(
            "codex_crew.port_resolution._gateway_owns_port", return_value=True
        ):
            assert dashboard_server._live_sibling_port(7811) == 5486
    ...
    def test_own_port_is_never_its_own_sibling(self, home: Path) -> None:
        ...
        with mock.patch(
            ...
            assert dashboard_server._live_sibling_port(5486) is None
...
class TestClientReadsTheCredentialForThePortItDials:
    def test_per_port_beats_the_shared_file(self, home: Path) -> None:
        from codex_crew import mcp_core

        (home / ".local_secret").write_text("newcomer-that-replaced-the-file")
        dashboard_server._write_secret_file(run_marker.secret_path(5486), "owner-of-5486")
        with mock.patch.object(mcp_core, "_api_port", return_value=5486):
            assert mcp_core._internal_secret() == "owner-of-5486"
    ...
    def test_two_generations_in_one_home_no_longer_collide(self, home: Path) -> None:
        ...
        Incumbent owns 5486. A second gateway starts in the same home on 7811.
        ...
        shared = home / ".local_secret"
        with mock.patch.object(dashboard_server, "_live_sibling_port", return_value=None):
            dashboard_server._write_instance_credentials(shared, 5486, "incumbent")
        with mock.patch.object(dashboard_server, "_live_sibling_port", return_value=5486):
            dashboard_server._write_instance_credentials(shared, 7811, "newcomer")
...
class TestPruneKeepsALiveSibling:
    def test_a_false_ownership_answer_does_not_take_the_credential_with_it(
        ...
        """
        run_marker.write_marker(5486)
        dashboard_server._write_secret_file(run_marker.secret_path(5486), "incumbent")
        with mock.patch(
            "codex_crew.port_resolution._gateway_owns_port", return_value=False
        ):
            run_marker.prune_markers(keep_port=7811)
        assert run_marker.marker_ports() == []
        assert run_marker.read_secret(5486) == "incumbent"

    def test_an_unverifiable_host_also_keeps_the_credential(self, home: Path) -> None:
        run_marker.write_marker(5486)
        dashboard_server._write_secret_file(run_marker.secret_path(5486), "incumbent")
        with mock.patch(
            "codex_crew.port_resolution._gateway_owns_port",
    ...
    def test_live_sibling_is_kept(self, home: Path) -> None:
        ...
        run_marker.write_marker(5486)
        dashboard_server._write_secret_file(run_marker.secret_path(5486), "alive")
        with mock.patch(
            "codex_crew.port_resolution._gateway_owns_port", return_value=True
        ):
            run_marker.prune_markers(keep_port=7811)
        assert run_marker.marker_ports() == [5486]
        assert run_marker.read_secret(5486) == "alive"
...
class TestEphemeralBindPublishesUnderTheRealPort:
    ...
    def test_declared_port_is_used_when_non_zero(self) -> None:
        runner = self._Runner([("127.0.0.1", 5486)])
        assert dashboard_server._resolved_bound_port(runner, 5486) == 5486
    ...
    def test_a_unix_socket_address_is_not_mistaken_for_a_port(self) -> None:
        runner = self._Runner(["/run/user/1000/codexcrew/gateway.sock", ("127.0.0.1", 41234)])
        assert dashboard_server._resolved_bound_port(runner, 0) == 41234

    def test_zero_when_no_tcp_address_is_readable(self) -> None:
        runner = self._Runner(["/run/user/1000/codexcrew/gateway.sock"])
...
class TestEveryToolGetsTheExplanation:
    ...
    def test_auth_mismatch_is_rewritten_for_every_caller(self) -> None:
        ...
        assert "wrong Codex Crew instance" in out["error"]
    ...
    def test_a_plain_forbidden_is_not_misdiagnosed(self) -> None:
        ...
        assert "wrong Codex Crew instance" not in out["error"]
        assert out["error"] == "Forbidden"

    def test_learn_add_surfaces_the_rewritten_message(self) -> None:
        from codex_crew.mcp_tools import learn
        ...
        assert "wrong Codex Crew instance" in out
...
class TestTheSharedHelperOwnsThePairing:
    ...
    def test_no_caller_relies_on_ambient_port_resolution(self) -> None:
        ...
        src = pathlib.Path(__file__).resolve().parent.parent / "src" / "codex_crew"
        offenders = []
    ...
    def test_mcp_core_delegates_rather_than_reimplementing(self, home: Path) -> None:
        ...
        with mock.patch.object(mcp_core, "_api_port", return_value=7811), mock.patch(
            "codex_crew.mcp_core.read_local_secret", return_value="from-helper"
        ) as helper:
    ...
    def test_cron_trigger_pairs_its_credential_with_the_port(self, home: Path) -> None:
        from codex_crew import cron_trigger
    ...
    def test_sage_probe_only_considers_ports_this_process_can_claim(self) -> None:
        ...
        a stranger.
        """
        from codex_crew.apps.builtins.code_review_sage.sage_lib import review_driver
        ...
        with mock.patch.dict(os.environ, {"CODEXCREW_PORT": "7811"}, clear=True):
            with mock.patch.object(
                review_driver.store, "crew_home", return_value=Path("/nonexistent")
            ):
        ...
        with mock.patch.dict(
            os.environ,
            {"CODEXCREW_BOUND_PORT": "7899", "CODEXCREW_PORT": "5486"},
            clear=True,
        ):
            with mock.patch.object(
                review_driver.store, "crew_home", return_value=Path("/nonexistent")
            ):
                assert review_driver._candidate_ports() == [7899, 5486]
    ...
    def test_cron_child_dials_the_port_its_credential_was_minted_for(self) -> None:
        ...
        its own port it reads CODEXCREW_PORT, which is 5486 on a `--port auto` gateway
        ...
        with mock.patch.dict(
            os.environ, {"_CODEXCREW_DIAL_PORT": "7899", "CODEXCREW_PORT": "5486"}
        ):
        ...
        with mock.patch.dict(os.environ, {"CODEXCREW_PORT": "5486"}, clear=True):
            assert cron_script.ScriptContext(job=job)._port == 5486
...
class TestFrameRelayNeverCredentialsASiblingGateway:
    ...
    the opposite case. ``parse_dashboard_url`` reads ``CODEXCREW_PORT`` then
    ``dashboard.url``, and on a ``--port auto`` gateway neither names the port that
    was actually bound -- only ``CODEXCREW_BOUND_PORT`` does. So both the ingress URL
    and the credential resolved to 5486, a SIBLING on a multi-gateway host, and the
    ...
    def test_the_credential_matches_the_port_the_frame_is_posted_to(
        ...
        monkeypatch.setenv("CODEXCREW_BOUND_PORT", "7811")
        monkeypatch.delenv("CODEXCREW_PORT", raising=False)
        dashboard_server._write_secret_file(run_marker.secret_path(5486), "sibling-secret")
    ...
    def test_a_default_port_install_still_gets_its_credential(
        ...
        ``CODEXCREW_BOUND_PORT`` to offer, so the guard never fired in the scenario
        ...
        """
        from codex_crew.computer_use import screencast
        from codex_crew.port_resolution import resolve_client_port_ex
    ...
    def test_an_inherited_codexcrew_port_does_not_win_over_the_bound_port(
        self, home: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """The generic client resolver's own ordering is the hazard here.

        ``resolve_client_port_ex`` reads ``CODEXCREW_PORT`` BEFORE
        ``CODEXCREW_BOUND_PORT``, which is right for a CLI client aiming at a chosen
        instance and wrong for code running inside the gateway. A shell that
        exported ``CODEXCREW_PORT=5486`` and then started a second gateway with
        ``--port auto`` leaves both set, and the frame would carry 5486's own valid
        credential -- so that sibling ACCEPTS the capture rather than refusing it.
        """
        from codex_crew.computer_use import screencast

        monkeypatch.setenv("CODEXCREW_PORT", "5486")  # inherited, names the sibling
        monkeypatch.setenv("CODEXCREW_BOUND_PORT", "7811")  # what we actually bound
        dashboard_server._write_secret_file(run_marker.secret_path(5486), "sibling-secret")
        dashboard_server._write_secret_file(run_marker.secret_path(7811), "our-secret")
    ...
    def test_codexcrew_port_still_decides_when_no_port_was_bound(
        self, home: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_a_malformed_bound_port_falls_through_instead_of_raising(
        ...
        monkeypatch.setenv("CODEXCREW_BOUND_PORT", "not-a-port")
        monkeypatch.setenv("CODEXCREW_PORT", "6777")
        assert ":6777" in screencast._ingress_url()
...
class TestCronDialsTheGatewayItRunsUnderNotASibling:
    ...
    ``_CODEXCREW_DIAL_PORT`` so credential and dial target cannot diverge. The
    hazard is the same one screencast had: the generic resolver reads
    ``CODEXCREW_PORT`` before ``CODEXCREW_BOUND_PORT``, so an inherited
    ``CODEXCREW_PORT=5486`` beside a ``--port auto`` gateway dials 5486 -- a
    ...
    def test_bound_port_wins_over_an_inherited_codexcrew_port(
        self, home: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from codex_crew import cron_script

        monkeypatch.setenv("CODEXCREW_PORT", "5486")  # inherited, the sibling
        monkeypatch.setenv("CODEXCREW_BOUND_PORT", "7811")  # what we actually bound
    ...
    def test_the_credential_is_read_for_the_dialed_port(
        ...
        monkeypatch.setenv("CODEXCREW_PORT", "5486")
        monkeypatch.setenv("CODEXCREW_BOUND_PORT", "7811")
        dashboard_server._write_secret_file(run_marker.secret_path(5486), "sibling-secret")
        dashboard_server._write_secret_file(run_marker.secret_path(7811), "our-secret")
        # The caller resolves the dial port once and passes it in; the credential
        # must be the one for that port, never the inherited-CODEXCREW_PORT sibling.
...
class TestTheServingResolverIsTheOneGatewaySideChokepoint:
    ...
    The client resolver reads CODEXCREW_PORT first, which is right for a CLI client
    ...
    def test_bound_port_beats_an_inherited_codexcrew_port(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from codex_crew.port_resolution import resolve_serving_port

        monkeypatch.setenv("CODEXCREW_PORT", "5486")
        monkeypatch.setenv("CODEXCREW_BOUND_PORT", "7811")
        assert resolve_serving_port() == 7811

    def test_codexcrew_port_still_decides_with_no_bound_port(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from codex_crew.port_resolution import resolve_serving_port
    ...
    def test_every_in_gateway_consumer_routes_through_it(self) -> None:
        ...
        src = Path(__file__).resolve().parents[1] / "src" / "codex_crew"
        offenders = []
        for rel in (
...
class TestReviewDriverDoesNotGuessASiblingPort:
    """The report-write base must be a port a source NAMED, never an invented 5486.
    ...
    NO source names one, guessing 5486 and dialing it with 5486's credential is how a
    review artifact gets created or pruned in whatever sibling owns that port.
    """

    def test_no_named_port_fails_closed_instead_of_guessing_5486(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from codex_crew.apps.builtins.code_review_sage.sage_lib import review_driver
        ...
        assert "5486" not in review_driver._gateway_base()
