...
from codex_crew import atomic_write as atomic_write_mod
from codex_crew import platform_compat
from codex_crew.apps.builtins.md_notebook import git_ops
...
def fixtures(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, _seed_template: Path):
    ...
    # so isolate CODEXCREW_HOME too or tests would touch the real ~/.codex-crew.
    monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path / "crew"))
    monkeypatch.setenv("CODEXCREW_PROXY_SECRET", SECRET)
    monkeypatch.setenv("MD_NOTEBOOK_NO_PICKER", "1")
    from codex_crew.apps.builtins.md_notebook import server as server_mod
...
async def test_tampered_signature_is_rejected(fixtures) -> None:
    ...
    async with TestClient(TestServer(app)) as raw:
        resp = await raw.get(
            "/api/vaults", headers={"X-CodexCrew-Proxy": f"{int(time.time())}:deadbeef"}
        )
        assert resp.status == 401
...
def test_path_contains_sensitive_flags_ancestors_of_credentials(
    ...
    # A custom CODEXCREW_HOME re-anchors the crew secret leaves — a folder
    # containing THAT must be refused as well (the Notes PAT lives there).
    crew = tmp_path / "elsewhere" / "notes" / "crew"
    monkeypatch.setenv("CODEXCREW_HOME", str(crew))
...
def test_default_home_follows_codexcrew_home(monkeypatch, tmp_path: Path) -> None:
    """An isolated instance keeps its own vaults instead of the production ones.

    Deriving the data root from ``Path.home()`` ignored ``CODEXCREW_HOME``, so a
    dev gateway (``CODEXCREW_HOME=.codexcrew-dev``) would load and edit the real
    ~/.codex-crew notes. ``_default_home`` now routes through ``config_dir()``.
    """
    from codex_crew.apps.builtins.md_notebook import server as server_mod

    iso = tmp_path / "iso-home"
    monkeypatch.setenv("CODEXCREW_HOME", str(iso))
...
def test_notebook_pat_is_behind_the_sensitive_path_floor() -> None:
    ...
    assert is_sensitive_path("~/.codex-crew/workspace/md-notebook/pat") is True
    # config_dir() can resolve to the legacy `.codexcrew` data-home on a migration
    # fallback, and HOME follows it — the token must be protected there too.
    assert is_sensitive_path("~/.codexcrew/workspace/md-notebook/pat") is True
    ...
    assert is_sensitive_path("~/.codex-crew/workspace/md-notebook/vaults.json") is True
    assert is_sensitive_path("~/.codexcrew/workspace/md-notebook/vaults.json") is True
    ...
    assert is_sensitive_path("~/.codex-crew/workspace/md-notebook/settings.json") is True
    assert is_sensitive_path("~/.codexcrew/workspace/md-notebook/settings.json") is True
    # Notes themselves must stay readable — the floor covers the token, not the vault.
    assert is_sensitive_path("~/.codex-crew/workspace/md-notebook/vaults/v1/note.md") is False
...
def loop_syncer(monkeypatch: pytest.MonkeyPatch):
    ...
    """
    from codex_crew.apps.builtins.md_notebook import syncer as syncer_mod
