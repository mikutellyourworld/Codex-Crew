...
every long-lived MCP stdio server (``codexcrew mcp-core`` / ``mcp-cron`` /
...
each name, cutting a fresh ``import codex_crew.cli`` to ~0.5 s / ~54 MB.

The tests here keep it that way:

1. **Ratchet** — a fresh interpreter that imports ``codex_crew.cli`` must not
...
2. **Dispatch integrity** — every function-local ``from codex_crew.* import``
...
_CLI_PY = _SRC / "codex_crew" / "cli.py"

#: Modules that must NOT load as a side effect of ``import codex_crew.cli``.
...
_BANNED_AFTER_CLI_IMPORT = (
    "codex_crew.cli_commands",
    "codex_crew.cli_server",
    "codex_crew.slack.gateway",
    "codex_crew.dashboard.state",
    "codex_crew.vector_memory",
    "numpy",
)


def test_cli_import_does_not_load_heavy_modules() -> None:
    """Ratchet: ``import codex_crew.cli`` must stay free of the deferred modules.
    ...
    assert res.returncode == 0, f"import codex_crew.cli failed:\n{res.stderr}"
    present = ast.literal_eval(res.stdout.strip())
    assert present == [], (
        f"module-scope import of codex_crew.cli loaded deferred modules: {present}. "
...
def _local_imports_in_main() -> list[tuple[str, str]]:
    """(module, name) for every function-local codex_crew import inside main()."""
    ...
    for node in ast.walk(main_fn):
        if isinstance(node, ast.ImportFrom) and node.module and (
            node.module == "codex_crew" or node.module.startswith("codex_crew.")
        ):
...
def test_main_contains_the_deferred_dispatch_imports() -> None:
    ...
    assert "codex_crew.cli_commands" in modules
    assert "codex_crew.cli_server" in modules
    assert "codex_crew.dashboard.state" in modules
...
def _stdio_roundtrip(subcommand: str, tmp_path: Path) -> list[str]:
    """Start ``codexcrew <subcommand>`` over stdio; return the tools/list names."""
    proc = subprocess.Popen(
        [sys.executable, "-m", "codex_crew", subcommand],
        ...
        env={
            **os.environ,
            "CODEXCREW_HOME": str(tmp_path / "home"),
        },
    )
    try:
        init = {
...
def test_boot_platform_runs_before_mcp_core_dispatch(monkeypatch) -> None:
    ...
    fake_mcp_core = types.ModuleType("codex_crew.mcp_core")
    fake_mcp_core.run_mcp_core_server = lambda: order.append("dispatch")  # type: ignore[attr-defined]
    monkeypatch.setitem(sys.modules, "codex_crew.mcp_core", fake_mcp_core)

    monkeypatch.setattr(cli.sys, "argv", ["codexcrew", "mcp-core"])
    cli.main()
