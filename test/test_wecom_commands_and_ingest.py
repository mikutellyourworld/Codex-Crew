...
from codex_crew.messaging.attachments import Attachment, IngestResult
from codex_crew.messaging.attachments import cleanup as cleanup_attachments
from codex_crew.wecom.client import WeComInbound
from codex_crew.wecom.commands import (
    COMMAND_SPEC,
    build_help_text,
...
from codex_crew.wecom.transport_dispatch import WeComDispatcher
...
def _permit_inbound(monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.wecom.transport_dispatch.inbound_permitted", _permit)
...
class TestCommandCatalogue:
    ...
    async def test_every_advertised_command_is_actually_intercepted(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.wecom.transport_dispatch.drive_turn", fake_drive_turn)
...
class TestLinkCommandsStayOffTheLoop:
    ...
    async def test_unlink_offloads_both_batching_calls(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "codex_crew.wecom.transport_dispatch.release_conversation_location", fake_release
        )
        d = _dispatcher(sessions, FakeClient())
...
class TestCleanupCoversEveryAwaitAfterIngest:
    ...
    async def test_a_cancelled_origin_bind_still_deletes_the_plaintext(
        ...
        monkeypatch.setattr(
            "codex_crew.wecom.transport_dispatch.process_wecom_attachments", fake_process
        )
        ...
        monkeypatch.setattr("codex_crew.wecom.transport_dispatch.bind_origin_mirror", cancelled_bind)
...
class TestTurnPathStaysOffTheLoop:
    ...
    async def test_the_origin_bind_is_offloaded(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.wecom.transport_dispatch.bind_origin_mirror", recording_bind)
        d = _dispatcher(FakeSessions(), FakeClient())

        await d._bind_origin_mirror("wecom:codexcrew:direct:Wei", _inbound("hi"))
...
class TestIngestMedia:
    ...
    async def test_an_unreadable_attachment_keeps_the_message_and_says_so(
        ...
        monkeypatch.setattr("codex_crew.wecom.transport_dispatch.process_wecom_attachments", boom)
...
class TestProcessWeComAttachments:
    ...
    async def test_no_pairs_does_no_network_work(self) -> None:
        from codex_crew.wecom.attachments import process_wecom_attachments
    ...
    async def test_each_item_is_downloaded_with_its_own_key_and_written_decrypted(
        ...
        from codex_crew.wecom import attachments as mod
    ...
    def test_the_blocking_write_helper_writes_exactly_the_bytes(self, tmp_path) -> None:
        from codex_crew.wecom.attachments import _write_bytes
    ...
    async def test_the_cancellation_cleanup_runs_OFF_the_event_loop(
        ...
        async def recording_cleanup(paths):
            import codex_crew.messaging.attachments as shared
    ...
    def test_the_text_budget_is_deliberately_below_the_transport_ceiling(self) -> None:
        ...
        from codex_crew.wecom.attachments import WECOM_INGEST_LIMITS as lim
    ...
    def test_every_transport_ceiling_matches_wecoms_documented_maximum(self) -> None:
        from codex_crew.wecom.attachments import WECOM_INGEST_LIMITS as lim
        from codex_crew.wecom.media import MAX_MEDIA_BYTES
        ...
        from codex_crew.wecom.media import WECOM_MAX_PLAINTEXT_BYTES
