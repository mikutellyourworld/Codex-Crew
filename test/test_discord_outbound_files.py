...
from codex_crew.discord.renderer import _UPLOAD_LIMITS, DiscordRenderer
from codex_crew.discord.transport import DISCORD_CAPABILITIES, DiscordTransport
from codex_crew.messaging import outbound_files as outbound
...
def _shrink_file_cap(monkeypatch: pytest.MonkeyPatch, cap: int = 64) -> None:
    monkeypatch.setattr("codex_crew.discord.renderer._UPLOAD_LIMITS", replace(_UPLOAD_LIMITS, max_file_bytes=cap))
...
class TestDescriptionRedaction:
    ...
    def test_a_display_reassembled_credential_is_redacted(self, streamed: str) -> None:
        from codex_crew.messaging.outbound_files import unescape_md
    ...
    def test_redaction_runs_before_the_length_cap(self) -> None:
        from codex_crew.discord.client import _safe_description
