...
NOT EXECUTED IN THE INTEGRATIONS_ONLY SANDBOX. Importing ``codex_crew.skills`` pulls
``codex_crew.cron`` -> ``croniter`` and ``codex_crew.vector_memory`` ->
...
def test_delete_refuses_a_symlinked_skill_dir(loader, tmp_path):
    ...
    from codex_crew.platform_compat import symlink_or_junction
