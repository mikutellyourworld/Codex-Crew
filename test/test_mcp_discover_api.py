...
CodexCrew scope, SEL-logged), and every documented error status.
...
def sandbox(fake_home, monkeypatch):
    ...
    import codex_crew.mcp_discovery as discovery_mod
...
def fake_sel(monkeypatch):
    ...
    from codex_crew.dashboard.handlers import mcp_discover as mod
...
class TestDiscoverInstall:
    async def test_official_install_happy_path(self, sandbox, fake_sel, reset_registry):
        ...
        try:
            ...
            # The translated spec landed in the CodexCrew scope — DISABLED,
            # because required env is unset: enabling after configuration is
            # the user's informed-consent step (review finding).
            written = json.loads(sandbox.codexcrew_json.read_text(encoding="utf-8"))
