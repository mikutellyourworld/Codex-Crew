...
The sweep entry point (:func:`codex_crew.session_pid.kill_orphan_mcps`) is
...
def _gatewayd_cmdline(sock) -> bytes:  # type: ignore[no-untyped-def]
    return f"python\x00-m\x00codex_crew.mcp_gateway.gatewayd\x00--unix={sock}".encode()
...
class TestGatewaydKillRoutesThroughShim:
    ...
    def test_liveness_probe_uses_pid_exists_not_signal_zero(self) -> None:
        ...
        with (
            ...
            patch("codex_crew.session_pid.os.kill") as raw_kill,
        ):
