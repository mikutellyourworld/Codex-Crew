...
#: code".
_FIXTURE_ESCAPES = {"*/pytest-of-*/*", "*/codexcrew-wt-example/*"}
...
#: `skipif(not __import__('codex_crew.sandbox', fromlist=['userns_available']).userns_available()`,
...
def test_omit_patterns_do_not_swallow_product_code() -> None:
    ...
    A pattern like ``*/codex_crew/*`` would omit all real source and turn the coverage
...
def _capability_guarded_test_files() -> set[str]:
    ...
    roots = [REPO_ROOT / "test", REPO_ROOT / "src" / "codex_crew" / "apps" / "builtins"]
    found: set[str] = set()
    for root in roots:
...
def test_every_capability_guarded_suite_runs_in_the_sandbox_job() -> None:
    ...
    sandbox lane, so 85 assertions (the ~/.codex-crew keystone among them) executed nowhere
