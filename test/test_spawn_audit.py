...
Every subprocess spawn in ``src/codex_crew`` must be either
...
* Operator-invoked CLI / setup / doctor / self-update (fixed argv against our
  own install: git pull, pip, npm, kiro-cli/codexcrew update,
...
_SRC_ROOT = Path(__file__).resolve().parent.parent / "src" / "codex_crew"
...
BENIGN_SPAWNS: frozenset[str] = frozenset(
    {
        ...
        # py-spy attach for `codexcrew perf sample --pid`: fixed list-argv (no
        ...
        # behind CODEXCREW_DEBUG and reachable only from the CLI.
        ...
        # service manager): a detached `codexcrew restart --port <marker port>`,
        # fixed argv whose binary is validated (basenamed codexcrew, absolute,
        ...
        # save (off the event loop) or the `codexcrew browse setup` CLI. Fixed
        ...
        # path floor fences from the agent, and ``codexcrew doctor`` is reachable
        ...
        # (`<this interpreter> -m codex_crew setup --agent-only`) built from
        ...
        # codex_crew.computer_use.overlay_proc`, a fixed argv built from
        ...
        # Computer-use permission probe: `<our own codexcrew binary> computer
        ...
        # body. The binary is resolved by `agent._codexcrew_mcp_invocation`, i.e.
        # the SAME install as the running gateway (or `sys.executable -m
        # codex_crew`), never a PATH shim the agent could plant. It exists as a
        ...
        # `diff` list-argv (no shell=True) run in CODEXCREW_PROJECT_DIR, an operator
        ...
        # <script>` argv, script path derived from CODEXCREW_PROJECT_DIR / the
        ...
        # (CODEXCREW_PROJECT_DIR, an operator environment value) and the
        ...
        "instances/token_mint.py::run_remote_codexcrew",
        ...
        # variable is the path, which comes from CODEXCREW_PROJECT_DIR — an
        ...
        # `launchctl <verb> gui/<uid>/dev.codexcrew.pod.<name>`. Argv is a fixed
        ...
        # --runtime codexcrew-agents.slice MemoryMax=<N>M MemorySwapMax=0
        ...
        # set-property --runtime codexcrew-agents.slice MemoryHigh=<value>`, where
        ...
        # and the whole flow runs from `codexcrew service install` on a TTY, not
        ...
        # venv and symlink ~/.local/bin/codexcrew.
        ...
        # command: the argv is a fixed toolchain path or the helper Codex Crew itself
...
# First-party fixed-argv spawn sites: every call site in ``src/codex_crew`` that
...
FIRST_PARTY_SPAWNS: frozenset[str] = frozenset(
    {
        # A module-style app backend earns this only when
        # ``shipped_builtin_module_path`` resolves the target inside the exact
        # shipped app package. The command is then the gateway's immutable
        # ``sys.executable -m <shipped module>`` tuple; file backends and forged
        # names compute False and retain the ordinary fail-closed sandbox path.
        ...
        # (``agent._codexcrew_mcp_invocation`` + ``agent._managed_mcp_env``, the
...
def test_every_spawn_is_routed_or_allowlisted():
    ...
    assert not unexpected, (
        "New unrouted subprocess spawn(s) found in src/codex_crew:\n  "
        + "\n  ".join(sorted(unexpected))
        + "\n\nRoute agent-influenced spawns through "
        "codex_crew.sandbox.sandboxed_spawn_argv (OS sandbox + scrubbed env), "
...
def test_every_routed_spawn_applies_resource_limits():
    ...
    assert not missing, (
        ...
        + "\n  ".join(sorted(missing))
        + "\n\nPass preexec_fn=codex_crew.sandbox.resource_limit_preexec() to the "
...
def test_every_routed_spawn_applies_cgroup_scope():
    ...
    assert not missing, (
        ...
        + "\n  ".join(sorted(missing))
        + "\n\nWrap the final argv with codex_crew.sandbox.cgroup_scope_argv() "
...
def test_bundled_skill_assets_are_not_imported():
    ...
    asset_modules = {
        "codex_crew." + p.relative_to(_SRC_ROOT).with_suffix("").as_posix().replace("/", ".")
        for p in assets
    }
    asset_packages = {
        "codex_crew." + p.relative_to(_SRC_ROOT).parent.as_posix().replace("/", ".") for p in assets
    }
    ...
    assert not offenders, (
        ...
        "shared logic into a real module under src/codex_crew (where the spawn "
