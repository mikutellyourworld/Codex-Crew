...
from codex_crew.apps import registry as reg_mod
...
from codex_crew.config.loader import ExternalRegistryConfig, CodexCrewConfig
from codex_crew.platform.bootstrap import build_default_context
from codex_crew.platform.defaults import DefaultAppsLoader
...
def _with_config(monkeypatch, registries):
    """Pin what ``CodexCrewConfig.load()`` reports for ``registries``.
    ...
    monkeypatch.setattr(loader_mod.CodexCrewConfig, "load", staticmethod(lambda: cfg))
...
class TestPinnedRegistries:
    ...
    def test_config_failure_degrades_to_pinned(self, monkeypatch):
        ...
        monkeypatch.setattr(loader_mod.CodexCrewConfig, "load", staticmethod(_boom))
        _with_loader(monkeypatch, [{"name": "official", "repo": FORGE}])
        rows = _effective_registries()
...
class TestTrustTier:
    ...
    def test_the_tier_survives_a_config_disk_round_trip(self, monkeypatch, tmp_path):
        ...
        loaded = loader_mod.CodexCrewConfig.load()
