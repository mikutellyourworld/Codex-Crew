...
from codex_crew.apps.builtins.design_tweak.backend import server
...
class TestChildEnv:
    ...
    def test_proxy_secret_stripped(self, monkeypatch, tmp_path):
        monkeypatch.setenv("CODEXCREW_PROXY_SECRET", "s3cr3t-hmac-key")
        env = server._child_env(tmp_path)
        assert "CODEXCREW_PROXY_SECRET" not in env
    ...
    @pytest.mark.parametrize(
        ...
        [
            "CODEXCREW_HOME",
            "CODEXCREW_APP_NAME",
            "CODEXCREW_APP_DATA_DIR",
            "CODEXCREW_PROJECT_DIR",
            "CODEXCREW_DEVFLEET_BIN_GIT",
            "CODEX_CREW_ANYTHING",
        ],
    )
    def test_codexcrew_capability_vars_stripped(self, monkeypatch, tmp_path, name):
...
class TestProjectSecretsAreNeverServed:
    ...
    `is_sensitive_path` is HOME-relative — it covers `~/.aws` and Codex Crew's data
...
class TestCodexCrewInternalTreesAreNeverServed:
    """Registering `~` must not expose Codex Crew's OWN secrets.
    ...
    "~/.codex-crew")` is False and everything unlisted under it was servable —
    ...
    def test_app_secret_under_the_crew_home_is_refused(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(server, "_CODEXCREW_INTERNAL_DIRS", (os.path.realpath(home / ".kiro"),))

        code, _ctype, body = server._static_response(
            str(home), "/.codex-crew/apps/design-tweak/.app_secret", "/p/"
        )
        assert code == 403
    ...
    def test_chat_transcripts_under_the_crew_home_are_refused(self, tmp_path, monkeypatch):
        ...
        code, _ctype, body = server._static_response(
            str(home), "/.codex-crew/history/2026-08-03.jsonl", "/p/"
        )
        assert code == 403
    ...
    def test_the_legacy_data_home_is_refused_too(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(server, "_CODEXCREW_INTERNAL_DIRS", (os.path.realpath(legacy),))

        code, _ctype, body = server._static_response(str(home), "/.codexcrew/.env", "/p/")
        assert code == 403
    ...
    def test_the_barrier_is_wired_into_the_static_sink(self):
        ...
        assert "_is_codexcrew_internal" in src
...
class TestDataHomeDefault:
    def test_default_home_is_codex_crew(self, tmp_path, monkeypatch):
        """With no env override the data dir lands under ~/.codex-crew.

        The pre-move `~/.codexcrew` home is dead; loading a fresh copy of the
        module proves the fallback, rather than asserting on source text.
        """
        for var in ("CODEXCREW_APP_DATA_DIR", "CODEXCREW_APP_DATA", "CODEXCREW_HOME"):
        ...
        assert ".codexcrew" not in str(mod.DATA_DIR)
...
class TestDevProxyStripsCredentials:
    ...
    def test_relay_drops_upstream_set_cookie(self):
        ...
        upstream = [
            ...
            ("Set-Cookie", "codexcrew_session=attacker; Path=/"),
    ...
    def test_websocket_handshake_strips_set_cookie(self):
        ...
        raw_head = (
            ...
            b"Set-Cookie: codexcrew_session=attacker; Path=/\r\n"
    ...
    def test_relay_http_drops_credentials(self, header):
        ...
        incoming = {
            header: "codexcrew_token=super-secret",
            "Accept": "text/html",
    ...
    def test_websocket_handshake_replay_drops_credentials(self):
        ...
        incoming = {
            "Host": "127.0.0.1:5486",
            "Cookie": "codexcrew_token=super-secret",
            "Upgrade": "websocket",
...
class TestStaticPreviewServer:
    ...
    def test_forwards_no_credentials_and_sets_none(self, static_preview):
        ...
        status, headers, body = get(
            "/p1/",
            headers={"Cookie": "codexcrew_token=super-secret", "Authorization": "Bearer x"},
        )
        assert status == 200
        assert b"super-secret" not in body
...
class TestRedirectStaysOnTheProxy:
    ...
    def test_loopback_on_a_different_port_is_untouched(self):
        ...
        """
        p = self._probe()
        assert p._keep_redirect_local("http://127.0.0.1:5486/chat") == "http://127.0.0.1:5486/chat"
...
class TestCustomCrewHomeIsRefused:
    ...
    def test_default_home_is_covered_by_the_kiro_entry(self):
        ...
        assert server._is_codexcrew_internal(home / ".kiro" / "crew" / "history" / "x.jsonl")

    def test_the_resolved_crew_home_is_listed(self):
        """Structural: the resolution must land in the tuple, not hold by accident."""
        listed = {os.path.realpath(p) for p in server._CODEXCREW_INTERNAL_DIRS}
    ...
    def test_a_sibling_of_the_home_is_not_over_blocked(self):
        ...
        home = Path(os.path.realpath(os.path.expanduser("~")))
        assert not server._is_codexcrew_internal(home / ".kiro-backup" / "notes.md")
    ...
    def test_custom_home_resolution_honours_the_env_var(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("CODEXCREW_APP_DATA_DIR", raising=False)
        monkeypatch.delenv("CODEXCREW_APP_DATA", raising=False)

        reloaded = importlib.reload(server)
        try:
            assert reloaded._is_codexcrew_internal(relocated / "history" / "chat.jsonl")
            assert reloaded._is_codexcrew_internal(relocated / "sessions.db")
        finally:
