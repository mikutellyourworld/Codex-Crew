...
from codex_crew.session import SessionManager, _Session, unlink_queued_temp_paths
...
class TestDispatchQueued:
    ...
    async def test_removes_reaction_and_calls_handler(self):
        from codex_crew.slack.events import _dispatch_queued
        ...
        with patch("codex_crew.slack.events.handle_message", new_callable=AsyncMock) as mock_hm:
...
def _make_route_orch() -> MagicMock:
    ...
    # to True, so a bare CodexCrewConfig() sends _route_message down
...
_ROUTE_PATCHES = [
    ...
    patch("codex_crew.slack.enterprise.check_message_origin", return_value=True),
]
...
async def _settle_handler_tasks(orch: MagicMock, *, rounds: int = 20) -> None:
    ...
    CodexCrewConfig.load() disk reads while building its coroutine), so it holds
...
class TestQueueRouting:
    ...
    async def test_busy_session_enqueues_with_force(self):
        from codex_crew.slack.events import SeenCache, _route_message
...
class TestOnDoneDrain:
    ...
    async def test_drains_session_queue_after_task(self):
        ...
        with patch("codex_crew.slack.events.handle_message", new_callable=AsyncMock) as mock_hm, \
             patch("codex_crew.slack.events.handle_message_transport", new_callable=AsyncMock) as mock_tr, \
             patch("codex_crew.slack.events.is_allowed_user", return_value=True), \
             patch("codex_crew.slack.enterprise.check_message_origin", return_value=True):
            await _route_message(orch, event, SeenCache(), is_mention=True)
...
class TestQueuedMessageImagePaths:
    ...
    async def test_dispatch_queued_unlinks_images_after_turn(self, tmp_path):
        import codex_crew.slack.events as events
