...
class TestHeartbeatHooksCarryDeniedState:
    ...
    def test_scoped_hooks_snapshot_live_denied_state(self) -> None:
        from codex_crew.hooks import HookManager, HooksConfig, UserDeniedPattern
...
class TestIsHeartbeatSafeTool:
    ...
    def test_mcp_server_prefix_stripped(self) -> None:
        ...
        assert _is_heartbeat_safe_tool("mcp__codexcrew-core__learn_list")
        assert _is_heartbeat_safe_tool("mcp__codexcrew-core__spawn_list")
        assert _is_heartbeat_safe_tool("mcp__codexcrew-core__local_knowledge_search")
        assert _is_heartbeat_safe_tool("mcp__codexcrew-cron__cron_list")
    ...
    def test_mcp_prefix_write_tool_still_rejected(self) -> None:
        ...
        assert not _is_heartbeat_safe_tool("mcp__codexcrew-core__send_message")
        assert not _is_heartbeat_safe_tool("mcp__builder-mcp__CodeReviewWriteActions")
        assert not _is_heartbeat_safe_tool("mcp__codexcrew-core__cron_add")
    ...
    def test_running_prefix_with_at_server_slash_tool(self) -> None:
        ...
        assert _is_heartbeat_safe_tool("Running: @codexcrew-core/learn_list")
        assert _is_heartbeat_safe_tool("Running: @codexcrew-core/spawn_list")
        assert _is_heartbeat_safe_tool("Running: @codexcrew-cron/cron_list")

    def test_at_server_slash_tool_without_running_prefix(self) -> None:
        """@server/Tool without a status prefix must also normalize."""
        assert _is_heartbeat_safe_tool("@codexcrew-core/artifact_list")
        assert _is_heartbeat_safe_tool("@codexcrew-core/local_knowledge_search")
    ...
    def test_running_prefix_write_tool_still_rejected(self) -> None:
        ...
        assert not _is_heartbeat_safe_tool("Running: @codexcrew-core/send_message")
    ...
    def test_at_server_slash_write_tool_rejected(self) -> None:
        ...
        assert not _is_heartbeat_safe_tool("@codexcrew-core/send_message")
        assert not _is_heartbeat_safe_tool("@codexcrew-core/cron_add")
...
class TestEditionHeartbeatAllowlist:
    ...
    def _install_gate_extra(self, monkeypatch, extra):
        ...
        from codex_crew.platform import build_default_context
        ...
        set_context(ctx)
        monkeypatch.setattr("codex_crew.platform.bootstrap._BOOTED", True, raising=False)
...
class TestHeartbeatApproval:
    ...
    async def test_approves_allowlisted_tool(self, orchestrator, monkeypatch) -> None:
        sel_mock = MagicMock()
        monkeypatch.setattr("codex_crew.slack.gateway.sel", lambda: sel_mock)
        ...
        assert kwargs["agent"] == "codexcrew-heartbeat"
    ...
    async def test_dashboard_log_warning_redacts_llm_title(
        ...
        — CodexCrew logs surface in the dashboard. Per security-controls
        ...
        with caplog.at_level(logging.WARNING, logger="codex_crew.slack.gateway"):
    ...
    async def test_approve_fails_closed_with_real_async_sel_unwritable(
        ...
        monkeypatch.setattr("codex_crew.slack.gateway.sel", lambda: real_sel)
...
class TestHeartbeatHooks:
    ...
    def test_drops_user_auto_approve_tools(self) -> None:
        ...
        from codex_crew.hooks import TOOL_AUTO_APPROVE
        from codex_crew.slack.gateway import _build_heartbeat_hooks
    ...
    def test_preserves_user_auto_deny_tools(self) -> None:
        ...
        """
        from codex_crew.hooks import TOOL_DENY
        from codex_crew.slack.gateway import _build_heartbeat_hooks
    ...
    def test_does_not_inherit_bundled_auto_approve(self) -> None:
        """The bundled ``codexcrew browse *`` patterns from HooksConfig.from_dict
        ...
        """
        from codex_crew.hooks import TOOL_AUTO_APPROVE, HookManager, HooksConfig
        from codex_crew.slack.gateway import _build_heartbeat_hooks

        # User config goes through from_dict → bundled patterns merged in.
        user = HookManager(HooksConfig.from_dict({}))
        hb = _build_heartbeat_hooks(user)
        # The bundled "codexcrew browse *" pattern would auto-approve this in
        # the user-scoped hooks, but must NOT in the heartbeat scope.
        assert hb.on_tool_call("Running: codexcrew browse foo").action != TOOL_AUTO_APPROVE
