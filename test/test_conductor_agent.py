...
from codex_crew.agent_files import CONDUCTOR_AGENT_FILENAME, OWNED_KIRO_AGENT_FILES
from codex_crew.skills import _BUILTIN_SKILLS_DIR

SKILL_DIR = (
    Path(__file__).resolve().parents[1] / "src" / "codex_crew" / "builtin_skills" / "goal-conductor"
)
...
class TestConductorInstaller:
    def _install(self, tmp_path, monkeypatch, *, may_auto_approve=None):
        ...
        monkeypatch.setattr(
            ...
            lambda: {
                ...
                "prompt": "file://x",
                "mcpServers": {
                    "codexcrew-core": {"command": "/resolved/codexcrew", "args": ["mcp-core"]},
                    "builder-mcp": {"command": "/x/builder", "args": []},
                },
                "tools": ["fs_write", "@codexcrew-core"],
                "allowedTools": ["@codexcrew-core"],
            },
        )
        monkeypatch.setattr(
            agent,
            "_codexcrew_mcp_invocation",
            lambda sub: ("/resolved/codexcrew", [sub]),
        )
    ...
    def test_identity_and_charter(self, tmp_path, monkeypatch):
        ...
        assert data["name"] == "codexcrew-conductor"
    ...
    def test_prompt_names_the_tools_it_expects_to_be_used(self, tmp_path, monkeypatch):
        ...
        ``@codexcrew-core`` puts 74 registered tools in front of this agent. Naming
    ...
    def test_no_write_tool_and_shell_not_preapproved(self, tmp_path, monkeypatch):
        ...
        ``@codexcrew-core`` and ``@codexcrew-dashboard`` are MOUNTED whole but never
        ...
        assert "@codexcrew-dashboard" in data["tools"]
        assert "@codexcrew-dashboard" not in data["allowedTools"]
        assert "@codexcrew-core" in data["tools"]
        assert "@codexcrew-core" not in data["allowedTools"]
    ...
    def test_only_create_and_read_verbs_are_auto_approved(self, tmp_path, monkeypatch):
        ...
        for verb in (
            ...
            assert f"@codexcrew-dashboard/{verb}" not in granted, verb
        for verb in (
            "chat_folder_tree",
            "chat_folder_create",
            "session_create",
            ...
            assert f"@codexcrew-dashboard/{verb}" in granted, verb
    ...
    def test_no_dashboard_verb_is_granted_outside_the_named_set(self, tmp_path, monkeypatch):
        ...
        """
        granted = self._install(tmp_path, monkeypatch)["allowedTools"]
        dashboard = {g for g in granted if g.startswith("@codexcrew-dashboard")}
        assert dashboard == {
        ...
        assert "@codexcrew-dashboard" not in granted
    ...
    def test_core_grants_are_named_verbs_never_the_whole_server(self, tmp_path, monkeypatch):
        ...
        just as well against the server-wide ``@codexcrew-core`` this replaced, so
        ...
        """
        granted = self._install(tmp_path, monkeypatch)["allowedTools"]
        core = {g for g in granted if g.startswith("@codexcrew-core")}
        assert core == {
        ...
        assert "@codexcrew-core" not in granted
        for denied in (
            ...
            assert f"@codexcrew-core/{denied}" not in granted, denied
        # And the projected KAS policy must deny them too, not just the kiro-cli
        # allowlist: a ``codexcrew-core/*`` pattern here would re-grant every one
        ...
        assert "codexcrew-core/*" not in match
        for denied in ("task_run", "spawn_run", "set_project", "browser", "artifact_save"):
            assert f"codexcrew-core/{denied}" not in match, denied
    ...
    def test_template_grants_never_leak_into_the_conductor_list(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: {
                ...
                "tools": ["fs_read", "@codexcrew-core"],
                # A template list carrying floor builtins — as if the build-time
                # filter did not exist. None of it may survive the replacement.
                "allowedTools": ["fs_read", "code", "glob", "grep", "@codexcrew-core"],
            },
        )
        monkeypatch.setattr(
            agent, "_codexcrew_mcp_invocation", lambda sub: ("/resolved/codexcrew", [sub])
        )
    ...
    def test_mcp_surface_is_narrowed_to_core_plus_dashboard(self, tmp_path, monkeypatch):
        ...
        data = self._install(tmp_path, monkeypatch)
        assert set(data["mcpServers"]) == {"codexcrew-core", "codexcrew-dashboard"}
        assert data["mcpServers"]["codexcrew-dashboard"]["args"] == ["mcp-dashboard"]
    ...
    def test_dashboard_entry_omits_managed_metadata_on_a_default_install(
        ...
        repo-root conftest pins ``CODEXCREW_HOME`` for every test, so the real
        ...
        dash = self._install(tmp_path, monkeypatch)["mcpServers"]["codexcrew-dashboard"]
        assert dash == {"command": "/resolved/codexcrew", "args": ["mcp-dashboard"]}
    ...
    def test_dashboard_entry_carries_managed_server_metadata(self, tmp_path, monkeypatch):
        ...
        ``CODEXCREW_HOME`` pin the shim reads the default data home while the
        ...
        monkeypatch.setattr(agent, "_managed_mcp_env", lambda: {"CODEXCREW_HOME": "/tmp/override"})
        data = self._install(tmp_path, monkeypatch)
        dash = data["mcpServers"]["codexcrew-dashboard"]
        assert dash["type"] == "registry"
        assert dash["env"] == {"CODEXCREW_HOME": "/tmp/override"}
    ...
    def test_grants_pass_through_the_governance_ceiling(self, tmp_path, monkeypatch):
        ...
        no bare ``@codexcrew-core`` entry left for it to have an opinion about.
        """
        data = self._install(
            tmp_path,
            monkeypatch,
            may_auto_approve=lambda ref: ref != "@codexcrew-core/monitor_start",
        )
        assert "@codexcrew-core" in data["tools"]
        assert "@codexcrew-core" not in data["allowedTools"]
        assert "@codexcrew-core/monitor_start" not in data["allowedTools"]
        # Every sibling grant survives — the ceiling removed one verb, not the set.
        assert "@codexcrew-core/monitor_update" in data["allowedTools"]
    ...
    def test_kas_permissions_are_derived_from_the_filtered_grants(self, tmp_path, monkeypatch):
        ...
        rather than ``codexcrew-core/*`` / ``codexcrew-dashboard/*`` wildcards, which
        ...
        assert "codexcrew-core/*" not in data["permissions"]["rules"][0]["match"]
        assert "codexcrew-dashboard/*" not in data["permissions"]["rules"][0]["match"]
            ...
            "rules": [
                {
                    "capability": "mcp",
                    "match": [
                        *(r for r in core_resources if r != "codexcrew-core/monitor_start"),
                        *dashboard_resources,
                    ],
                    "effect": "allow",
                }
    ...
    def test_withholding_a_grant_is_audit_logged(self, tmp_path, monkeypatch):
        ...
        "@codexcrew-core was withheld" could not tell which of 14 grants he lost.
        """
        calls: list[dict] = []
        ...
        self._install(
            tmp_path, monkeypatch, may_auto_approve=lambda ref: ref != "@codexcrew-core/select_crew"
        )
        withheld = [c for c in calls if c.get("operation") == "mcp_auto_approve_withheld"]
        assert len(withheld) == 1
        assert "@codexcrew-core/select_crew" in withheld[0]["resources"]
    ...
    def test_audit_failure_does_not_break_the_install(self, tmp_path, monkeypatch):
        ...
        assert "@codexcrew-core/select_crew" not in data["allowedTools"]
        assert data["allowedTools"] == [
            "session",
            "report",
            "tool_search",
    ...
    def test_spec_is_registered_as_codexcrew_owned(self):
...
class TestAcceptEvaluatorInvariant:
    ...
    because the script runs as an approved wrapper and Codex Crew's
