"""The shared hardened gh runner (``codex_crew.github_runner``).
...
* resolver precedence (caller override → ``CODEXCREW_GH_BIN`` → candidates),
...
from codex_crew import github_runner as runner  # noqa: E402


@pytest.fixture(autouse=True)
def _clean_runner_state(monkeypatch):
    runner.reset_cache()
    monkeypatch.delenv("CODEXCREW_GH_BIN", raising=False)
    monkeypatch.delenv("_CODEXCREW_GH_PREVALIDATED", raising=False)
    monkeypatch.delenv("CODEXCREW_PROVIDER_BIN_STRICT", raising=False)
...
class TestResolveGh:
    ...
    def test_caller_override_wins_over_generic_and_candidates(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("CODEXCREW_TEST_GH", caller)
        monkeypatch.setenv("CODEXCREW_GH_BIN", generic)
        monkeypatch.setattr(
            runner, "PROVIDER_EXECUTABLE_CANDIDATES", {"gh": (candidate,), "glab": ()}
        )

        assert runner.resolve_gh(override_env="CODEXCREW_TEST_GH") == caller

    def test_generic_override_wins_when_caller_var_is_unset(self, monkeypatch, tmp_path):
        generic = _fake_gh(tmp_path / "generic-bin")
        candidate = _fake_gh(tmp_path / "candidate-bin")
        monkeypatch.delenv("CODEXCREW_TEST_GH", raising=False)
        monkeypatch.setenv("CODEXCREW_GH_BIN", generic)
        monkeypatch.setattr(
            runner, "PROVIDER_EXECUTABLE_CANDIDATES", {"gh": (candidate,), "glab": ()}
        )

        assert runner.resolve_gh(override_env="CODEXCREW_TEST_GH") == generic
    ...
    def test_a_set_but_empty_override_fails_loudly(self, monkeypatch, tmp_path):
        ...
        """
        candidate = _fake_gh(tmp_path / "candidate-bin")
        monkeypatch.setenv("CODEXCREW_TEST_GH", "")
        monkeypatch.setattr(
            runner, "PROVIDER_EXECUTABLE_CANDIDATES", {"gh": (candidate,), "glab": ()}
        )

        with pytest.raises(runner.SetupError, match="CODEXCREW_TEST_GH.*path must be absolute"):
            runner.resolve_gh(override_env="CODEXCREW_TEST_GH")

    def test_a_wrong_override_names_the_variable(self, monkeypatch, tmp_path):
        monkeypatch.setenv("CODEXCREW_TEST_GH", str(tmp_path / "missing-gh"))

        with pytest.raises(runner.SetupError, match="CODEXCREW_TEST_GH.*failed validation"):
            runner.resolve_gh(override_env="CODEXCREW_TEST_GH")

    def test_strict_mode_refuses_path_hits(self, monkeypatch, tmp_path):
        planted = _fake_gh(tmp_path / "user-bin")
        monkeypatch.setenv("CODEXCREW_PROVIDER_BIN_STRICT", "1")
        monkeypatch.setenv("PATH", str(tmp_path / "user-bin"))
        monkeypatch.setattr(
            runner,
            "PROVIDER_EXECUTABLE_CANDIDATES",
            {"gh": ("/nonexistent-codexcrew/gh",), "glab": ()},
        )
    ...
    def test_missing_gh_message_gives_install_guidance(self, monkeypatch):
        ...
        assert "CODEXCREW_TEST_GH" in message
    ...
    def test_a_changed_override_value_is_not_served_from_cache(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("CODEXCREW_TEST_GH", first)
        assert runner.resolve_gh(override_env="CODEXCREW_TEST_GH") == first
        monkeypatch.setenv("CODEXCREW_TEST_GH", second)
        assert runner.resolve_gh(override_env="CODEXCREW_TEST_GH") == second
...
class TestGhEnv:
    def test_polluted_gateway_env_never_reaches_the_child(self, monkeypatch):
        ...
        polluted = {
            ...
            "CODEXCREW_INTERNAL_TOKEN": "internal",
            "GH_TOKEN": "gho_token",
        ...
        for secret_key in (
            ...
            "SSH_AUTH_SOCK", "SSH_AGENT_PID", "GIT_SSH_COMMAND", "CODEXCREW_INTERNAL_TOKEN",
        ):
...
class TestRunGh:
    ...
    def test_audit_event_carries_the_caller_namespace(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.sel.sel", lambda: _FakeSel())
    ...
    def test_unavailable_audit_refuses_the_spawn(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.sel.sel", mock.Mock(side_effect=RuntimeError("sel down"))
        )
    ...
    def test_outcome_audit_failure_never_breaks_the_call(self, monkeypatch):
        ...
        flaky = _FlakySel()
        monkeypatch.setattr("codex_crew.sel.sel", lambda: flaky)
...
class TestReExports:
    ...
    def test_github_client_url_parser_is_the_shared_function(self):
        from codex_crew.apps.builtins.issue_radar.backend import github_client
    ...
    def test_repo_url_error_is_one_class_across_layers(self):
        from codex_crew.apps.builtins.issue_radar.backend import errors, github_client
