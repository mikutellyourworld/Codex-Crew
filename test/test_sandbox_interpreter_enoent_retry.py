"""Tests for codex_crew.sandbox — launcher-interpreter ENOENT spawn retry.
...
from codex_crew.sandbox import _INTERPRETER_ENOENT_DELAYS, _is_transient_interpreter_enoent
...
class TestSpawnToRegisteredIsAtomic:
    ...
    def test_finish_spawn_registers_and_clears_the_spawning_mark(self):
        cron = importlib.import_module("codex_crew.cron_script")
        self._clear(cron)
...
class TestAnOverlappingWakeCostsNoFailureStrike:
    ...
    def test_the_consumer_treats_skipped_as_non_counting(self):
        ...
        gateway = pathlib.Path(
            importlib.import_module("codex_crew.slack.gateway").__file__
...
class TestARefusedOverlapIsNotPersistedAsASuccessfulRun:
    ...
    def _dispatch_source(self) -> str:
        return pathlib.Path(importlib.import_module("codex_crew.slack.gateway").__file__).read_text(
            encoding="utf-8"
        )
    ...
    def test_execute_really_does_treat_a_non_error_status_as_success(self):
        ...
        """
        cron_src = pathlib.Path(importlib.import_module("codex_crew.cron").__file__).read_text(
            encoding="utf-8"
        )
...
class TestSpawnOnlyCancellationIsReportedHonestly:
    ...
    def _cron_source() -> str:
        return pathlib.Path(importlib.import_module("codex_crew.cron").__file__).read_text(
            encoding="utf-8"
        )
    ...
    def test_the_control_flow_reader_is_gated_on_the_job_being_an_agent_job(self):
        ...
        cron_script = importlib.import_module("codex_crew.cron_script")
        claimers = {
            node.name
