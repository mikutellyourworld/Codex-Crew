...
from codex_crew.mcp_gateway.hashing import is_secret_env_key
...
class TestSettingsInjection:
    ...
    def test_alias_spelling_is_honoured(self) -> None:
        ...
        from codex_crew.mcp_gateway.rewriter import mcp_server_alias
    ...
    def test_end_to_end_no_settings_overlay_and_real_settings_untouched(
        ...
        """
        from codex_crew.mcp_gateway.rewriter import rewrite_agents

        source_dir = tmp_path / "agents"
        source_dir.mkdir()
        (source_dir / "codexcrew.json").write_text(
            json.dumps({"name": "codexcrew", "mcpServers": {}}), encoding="utf-8"
        )
        ...
        overlay = json.loads((overlay_dir / "codexcrew.json").read_text(encoding="utf-8"))
...
def test_private_server_with_declared_env_is_not_warned_about(tmp_path: Path, caplog) -> None:
    ...
    with caplog.at_level(logging.WARNING, logger="codex_crew.mcp_gateway.rewriter"):
...
def test_unresolvable_bare_command_is_not_stubbed(tmp_path: Path, caplog) -> None:
    ...
    spec = {
        ...
        "mcpServers": {
            "ghost": {
                "command": "codexcrew-test-definitely-missing-cmd",
                "args": ["--serve"],
                "poolable": True,
            },
        },
    ...
    assert wrapped == 0
    assert _WRAPPER_MARKER not in entry
    assert entry.get("command") == "codexcrew-test-definitely-missing-cmd"
...
def test_spec_env_path_wins_over_augmented_host_path(tmp_path: Path, monkeypatch) -> None:
    ...
    from codex_crew.mcp_gateway import rewriter as _rw
...
def test_non_string_env_path_does_not_abort_the_rewrite(tmp_path: Path) -> None:
    ...
    resolved = _rw._resolve_target_command(
        "codexcrew-test-definitely-missing-cmd", {"PATH": 7}, None
    )
...
def test_rewriter_calls_restrict_to_owner_on_windows(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("codex_crew.mcp_gateway.rewriter.platform_compat.IS_POSIX", False)
    monkeypatch.setattr("codex_crew.mcp_gateway.rewriter.platform_compat.IS_WINDOWS", True)
    # Forwarding ON or the env-declaring fixture is declassified (issue #3495
    # cause B) and no sidecar write happens at all.
    monkeypatch.setattr("codex_crew.mcp_gateway.rewriter.forward_declared_env_enabled", lambda: True)
    with (
        patch(
            "codex_crew.mcp_gateway.rewriter.platform_compat.restrict_to_owner",
            side_effect=_mock_restrict,
        ),
        patch(
            "codex_crew.mcp_gateway.rewriter.platform_compat.make_owner_only_dir",
...
def test_stub_fingerprint_is_the_module_on_the_launch_line(tmp_path: Path) -> None:
    ...
    from codex_crew.dashboard import session_memory
    from codex_crew.mcp_gateway import STUB_MODULE, rewriter
