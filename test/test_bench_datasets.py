...
from codex_crew.eval.bench.datasets import (
    SPECS,
    CorpusFetchError,
    DatasetSpec,
    cache_dir,
...
def cache(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setenv("CODEXCREW_BENCH_CACHE", str(tmp_path))
...
def test_cache_override_wins_over_xdg(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CODEXCREW_BENCH_CACHE", str(tmp_path / "explicit"))
...
def test_cache_dir_falls_back_to_xdg_then_home(
    ...
    """Never under CODEXCREW_HOME: `codexcrew snapshot` must not swallow the corpora."""
    monkeypatch.delenv("CODEXCREW_BENCH_CACHE", raising=False)
    monkeypatch.setenv("XDG_CACHE_HOME", str(tmp_path / "xdg"))
    assert cache_dir() == tmp_path / "xdg" / "codexcrew" / "bench-data"
    monkeypatch.delenv("XDG_CACHE_HOME", raising=False)
    assert cache_dir() == Path.home() / ".cache" / "codexcrew" / "bench-data"
...
def test_missing_file_with_downloads_disabled_names_the_fetch_command(cache: Path) -> None:
    ...
    msg = str(exc.value)
    assert "codexcrew bench fetch longmemeval_s" in msg
