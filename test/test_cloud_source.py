...
from codex_crew.cloud import aws, source
...
class TestRepoRoot:
    ...
    def test_repo_root_fails_closed_when_no_marker(self, monkeypatch, tmp_path):
        ...
        fake_module = tmp_path / "site-packages" / "codex_crew" / "cloud" / "source.py"
...
class TestBuildTarball:
    ...
    def test_tar_fallback_excludes_secrets(self, tmp_path):
        ...
        (tmp_path / "install.sh").write_text("echo hi\n")
        (tmp_path / ".codexcrew-dev").mkdir()
        (tmp_path / ".codexcrew-dev" / "config.json").write_text('{"token":"secret"}\n')
        ...
        try:
            ...
            assert not any(".codexcrew-dev" in n for n in names)
    ...
    def test_tar_fallback_excludes_custom_codexcrew_home(self, monkeypatch, tmp_path):
        # Dev mode can set CODEXCREW_HOME to a custom-named dir at the repo root;
        # the tarfile fallback must exclude it by its actual name (not just the
        # hardcoded .codexcrew* entries) so its data/secrets don't ship to S3.
        (tmp_path / "install.sh").write_text("echo hi\n")
    ...
    def test_tar_fallback_excludes_nested_custom_home(self, monkeypatch, tmp_path):
        # A custom CODEXCREW_HOME nested below the repo root (root/data/kc-home)
    ...
    def test_custom_home_not_excluded_when_outside_repo(self, monkeypatch, tmp_path):
        # An absolute ~/.codexcrew OUTSIDE the repo root isn't in the tarball
        ...
        repo = tmp_path / "repo"
        repo.mkdir()
        outside = tmp_path / "home" / ".codexcrew"
        monkeypatch.setenv("CODEXCREW_HOME", str(outside))
    ...
    def test_refilter_corrupt_archive_cleans_up_temp(self, tmp_path, monkeypatch):
        ...
        import codex_crew.cloud.source as _src
...
class TestBucketNaming:
    def test_bucket_name(self, monkeypatch):
        ...
        assert source.bucket_name("dev", "us-east-1") == "codexcrew-src-814959995281-us-east-1"
...
class TestEnsureBucket:
    def test_reuses_existing(self, monkeypatch):
        ...
        assert b == "codexcrew-src-123-us-east-1"
    ...
    def test_creates_when_missing(self, monkeypatch):
        ...
        assert b == "codexcrew-src-123-us-west-2"
        assert created["n"] >= 1
...
_ACCT = "123456789012"
_BUCKET = f"codexcrew-src-{_ACCT}-us-east-1"
...
class TestAccountFromBucket:
    def test_extracts_12_digit_account(self):
        ...
        assert source._account_from_bucket("codexcrew-src-123456789012-ap-south-1") == "123456789012"

    def test_rejects_unknown_fallback_and_bad_shapes(self):
        # The `codexcrew-src-unknown-*` fallback (account didn't resolve) yields ""
        # so callers fail closed instead of pinning a bogus owner.
        assert source._account_from_bucket("codexcrew-src-unknown-us-east-1") == ""
        assert source._account_from_bucket("codexcrew-src-123-us-east-1") == ""  # too short
...
class TestUploadDelete:
    def test_upload_source(self, monkeypatch, tmp_path):
        ...
        assert key == "kc-1/codexcrew-src.tar.gz"
    ...
    def test_upload_source_fails_closed_when_owner_underivable(self, monkeypatch, tmp_path):
        ...
        # `codexcrew-src-unknown-*`), we must NOT upload without an owner pin.
        monkeypatch.setattr(source, "ensure_bucket", lambda *a: "codexcrew-src-unknown-us-east-1")
    ...
    def test_delete_source(self, monkeypatch):
        ...
        assert "--key" in rm["args"] and "kc-1/codexcrew-src.tar.gz" in rm["args"]
        assert "--expected-bucket-owner" in rm["args"] and _ACCT in rm["args"]
        assert res["removed"] is True
        assert res["error"] == ""
        assert res["uri"] == f"s3://{_BUCKET}/kc-1/codexcrew-src.tar.gz"
    ...
    def test_delete_source_fails_closed_when_owner_underivable(self, monkeypatch):
        ...
        monkeypatch.setattr(source, "bucket_name", lambda *a: "codexcrew-src-unknown-us-east-1")
        monkeypatch.setattr(
