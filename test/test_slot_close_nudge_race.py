...
from codex_crew import autonudge
from codex_crew.apps.builtins.issue_radar.backend import crew_runtime
from codex_crew.autonudge import AutoNudgeService, NudgeAdmissionRefused
from codex_crew.dashboard import chat_handlers as handlers
from codex_crew.monitoring.models import MonitorBudgets, MonitorOutcome
...
async def test_a_failed_persist_takes_the_app_dismissal_back(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("codex_crew.apps.teardown.notify_slot_closed", _told)
    monkeypatch.setattr("codex_crew.apps.teardown.notify_slot_close_undone", _undo)
    monkeypatch.setattr(handlers, "save_slot_off_loop", _persist)
...
async def test_an_app_that_cannot_be_told_aborts_the_close(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(handlers, "save_slot_off_loop", _persist)
    monkeypatch.setattr("codex_crew.apps.teardown.notify_slot_closed", _hook_fails)
...
async def test_app_close_retires_a_loop_armed_during_the_close_hook(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("codex_crew.apps.teardown.notify_slot_closed", _hook)
    close = asyncio.create_task(handlers.api_chat_slot_delete(_Req(state, NAME)))
