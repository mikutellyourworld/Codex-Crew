...
from codex_crew.dashboard.chat import restore_recent_sessions
...
def _make_config_app(tmp_path):
    ...
    from codex_crew.dashboard.handlers import api_dashboard_config
...
class TestRestoreRecentSessions:
    ...
    def test_channel_rebuild_window_computes_the_durable_prefix_count(self, tmp_path, monkeypatch):
        ...
        from codex_crew.dashboard.channel_slots import _rebuild_window
    ...
    def test_redacts_credentials_in_restored_messages(self, tmp_path, monkeypatch):
        ...
        from codex_crew.dashboard.chat_utils import _prepare_messages
        ...
        from codex_crew.dashboard.chat_persistence import _build_history_prefix
        from codex_crew.dashboard.side_context import _format_parent_snapshot
...
class TestDashboardConfigAPI:
    ...
    async def test_get_defaults(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.config.loader.config_path", lambda: tmp_path / "nonexistent.json"
        )
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
...
class TestRehydrateSlotFromHistory:
    ...
    def test_returns_none_when_session_not_on_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        from codex_crew.dashboard.chat import _rehydrate_slot_from_history
    ...
    def test_rehydrates_slot_with_metadata_and_messages(self, tmp_path, monkeypatch):
        ...
        _acp_cfg = CodexCrewConfig()
        _acp_cfg.agent.provider = "acp"
        monkeypatch.setattr(CodexCrewConfig, "load", classmethod(lambda cls: _acp_cfg))
        _write_session(
            tmp_path,
    ...
    def test_rehydrate_canonicalizes_model_for_claude_code(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        _cc_cfg = CodexCrewConfig()
        _cc_cfg.agent.provider = "claude_code"
        monkeypatch.setattr(CodexCrewConfig, "load", classmethod(lambda cls: _cc_cfg))
        _write_session(
            tmp_path,
            "dashboard_ccchat",
            [{"role": "user", "content": "hi", "ts": "2026-03-23T10:00:00"}],
...
class TestAsyncRestoreRecentSessionsOffLoop:
    ...
    async def test_matches_the_sync_driver(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        from codex_crew.dashboard.chat_persistence import restore_recent_sessions_async
        ...
        for name in ("dashboard_par1", "dashboard_par2"):
            _write_session(
                ...
                name,
                [{"role": "user", "content": "hi", "ts": "2026-03-23T10:00:00"}],
                meta={"title": name, "agent": "codexcrew"},
            )
            (tmp_path / f"{name}.jsonl").touch()
