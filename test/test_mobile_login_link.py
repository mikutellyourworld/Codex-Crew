...
from codex_crew.dashboard.handlers import _shared, auth_mobile
...
def _call(request: MagicMock, *, valid_origin: bool = True):
    with patch("codex_crew.dashboard.handlers.auth_mobile.check_origin", return_value=valid_origin):
...
def test_mobile_link_refuses_a_restricted_session():
    ...
    state = MagicMock()
    with patch(
        "codex_crew.dashboard.handlers.auth_mobile._is_restricted_session", return_value=True
    ):
