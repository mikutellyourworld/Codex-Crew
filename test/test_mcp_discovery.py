...
from codex_crew.mcp_discovery import (
    SCOPE_CC_GLOBAL,
    SCOPE_KIRO_GLOBAL,
    SCOPE_CODEXCREW,
    McpServerInfo,
...
def _passthrough_sandbox(monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.mcp_discovery.sandboxed_spawn_argv", _passthrough)
...
class TestListServers:
    ...
    def test_list_merges_installed_config(self, tmp_path, monkeypatch) -> None:
        """defaults.json has no mcpServers; installed codexcrew.json does."""
        agent_dir = tmp_path / "agents"
        agent_dir.mkdir()
        (agent_dir / "defaults.json").write_text(json.dumps({"name": "codexcrew"}))
        monkeypatch.setenv("CODEXCREW_PROJECT_DIR", str(tmp_path))
        kiro_dir = tmp_path / ".kiro" / "agents"
        kiro_dir.mkdir(parents=True)
        installed = {"mcpServers": {"codexcrew-cron": {"command": "codexcrew", "args": ["mcp-cron"]}}}
        (kiro_dir / "codexcrew.json").write_text(json.dumps(installed))
        monkeypatch.setattr("codex_crew.mcp_discovery.Path.home", lambda: tmp_path)
        ...
        monkeypatch.setattr("codex_crew.mcp_discovery.kiro_agents_dir", lambda: kiro_dir)
        monkeypatch.setattr("codex_crew.mcp_discovery._MCP_JSON_PATHS", (tmp_path / "nope.json",))
        servers = list_servers()
        names = {s.name for s in servers}
        assert "codexcrew-cron" in names
    ...
    def test_mcp_json_servers_merged(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.mcp_discovery._MCP_JSON_PATHS", (mcp_json,))
        servers = list_servers()
        names = {s.name for s in servers}
    ...
    def test_list_skips_disabled_servers(self, tmp_path, monkeypatch) -> None:
        ...
        (agent_dir / "defaults.json").write_text(json.dumps(cfg))
        monkeypatch.setenv("CODEXCREW_PROJECT_DIR", str(tmp_path))
        monkeypatch.setattr("codex_crew.mcp_discovery._MCP_JSON_PATHS", (tmp_path / "x",))
        monkeypatch.setattr("codex_crew.mcp_discovery.Path.home", lambda: tmp_path)
        servers = list_servers()
        names = {s.name for s in servers}
    ...
    def test_list_surfaces_codexcrew_disabled_servers_as_disabled_rows(
        self, tmp_path, monkeypatch
    ) -> None:
        """CodexCrew-scope disabled entries get a row marked disabled.

        Consent-disabled installs/custom adds land with ``disabled: true``
        in the CodexCrew scope; the table's enable action is the consent
        ...
        assert by_name["inactive"].presence["codexcrew"] is False

    def test_codexcrew_disabled_row_survives_agent_mirror(self, tmp_path, monkeypatch) -> None:
    ...
    async def test_probe_all_never_probes_disabled_rows(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.mcp_discovery.probe_server", fake_probe)
        from codex_crew.mcp_discovery import probe_all
    ...
    def test_dashboard_disabled_server_keeps_a_disabled_row(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setenv("CODEXCREW_PROJECT_DIR", str(tmp_path))
        monkeypatch.setattr(
            "codex_crew.mcp_discovery._MCP_SOURCES",
            ((store, SCOPE_CODEXCREW), (kiro_mcp, SCOPE_KIRO_GLOBAL)),
        )
        monkeypatch.setattr("codex_crew.mcp_discovery._MCP_JSON_PATHS", (store, kiro_mcp))
        monkeypatch.setattr("codex_crew.mcp_discovery.Path.home", lambda: tmp_path)
    ...
    def test_mcp_json_merges_multiple_files(self, tmp_path, monkeypatch) -> None:
        ...
        codexcrew_mcp = tmp_path / "codexcrew_mcp.json"
        codexcrew_mcp.write_text(
            json.dumps(
                {"mcpServers": {"shared": {"command": "codexcrew"}, "mc-only": {"command": "m"}}}
            )
        )
        monkeypatch.setattr("codex_crew.mcp_discovery._MCP_JSON_PATHS", (kiro_mcp, codexcrew_mcp))
    ...
    def test_mcp_json_malformed_file_skipped(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.mcp_discovery._MCP_JSON_PATHS", (bad, good))
    ...
    def test_mcp_json_permission_error_skipped(self, tmp_path, monkeypatch) -> None:
        ...
        good = tmp_path / "good.json"
        good.write_text(json.dumps({"mcpServers": {"srv": {"command": "x"}}}))
        monkeypatch.setattr("codex_crew.mcp_discovery._MCP_JSON_PATHS", (blocked, good))

        original = __import__("codex_crew.hooks", fromlist=["safe_read_file"]).safe_read_file
        ...
        monkeypatch.setattr("codex_crew.mcp_discovery.safe_read_file", _mock_safe_read)
...
class TestExtraScopeSeam:
    ...
    def test_oss_default_scans_kiro_only(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.mcp_discovery._extra_scope_sources", lambda: [])
    ...
    def test_companion_scope_is_scanned(self, tmp_path, monkeypatch) -> None:
        ...
        cc.write_text(json.dumps({"mcpServers": {"companion-srv": {"command": "x"}}}))
        monkeypatch.setattr(
            "codex_crew.mcp_discovery._extra_scope_sources", lambda: [(cc, "ccGlobal")]
        )
    ...
    def test_non_cc_scope_reported_in_presence(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "codex_crew.mcp_discovery._extra_scope_sources",
    ...
    def test_seam_scope_ranks_below_kiro_global(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "codex_crew.mcp_discovery._MCP_SOURCES", ((kiro_mcp, SCOPE_KIRO_GLOBAL),)
        )
        monkeypatch.setattr("codex_crew.mcp_discovery._MCP_JSON_PATHS", (kiro_mcp,))
        monkeypatch.setattr(
            "codex_crew.mcp_discovery._extra_scope_sources", lambda: [(cc_mcp, "ccGlobal")]
        )
...
class TestDiscoverNew:
    ...
    def test_discover_skips_existing_with_expanded_path(self, tmp_path, monkeypatch) -> None:
        ...
        forever.
        """
        from codex_crew.env import spec_env_path
...
class TestCommandsDiverged:
    def test_identical_commands(self) -> None:
        from codex_crew.mcp_discovery import _commands_diverged
...
class TestSyncToAgentConfig:
    def test_sync_never_launches_kiro_cli(self, tmp_path, monkeypatch) -> None:
        ...
        kiro_dir = tmp_path / ".kiro" / "agents"
        kiro_dir.mkdir(parents=True)
        config_path = kiro_dir / "codexcrew.json"
        config_path.write_text(json.dumps({"mcpServers": {}, "tools": [], "allowedTools": []}))

        install_called = []
        monkeypatch.setattr(
            "codex_crew.agent.install_agent",
...
class TestProbeRemote:
    ...
    async def test_probe_remote_ok(self) -> None:
        ...
        with patch("codex_crew.mcp_discovery.aiohttp.ClientSession", return_value=mock_session):
    ...
    def test_needs_authorization_predicate(self) -> None:
        ...
        from codex_crew.mcp_discovery import _needs_authorization
    ...
    def test_a_real_oauth_challenge_is_recognised(self) -> None:
        ...
        from codex_crew.mcp_discovery import _is_bearer_challenge
    ...
    async def test_a_tokenless_401_records_the_challenge_and_an_absent_grant(self) -> None:
        ...
        with patch("codex_crew.mcp_discovery.aiohttp.ClientSession", return_value=mock_session), patch(
            "codex_crew.mcp_discovery._runtime_grant_present", AsyncMock(return_value=False)
        ):
            result = await _probe_remote(server)
    ...
    async def test_an_existing_runtime_grant_is_reported_alongside_needs_auth(self) -> None:
        ...
        with patch("codex_crew.mcp_discovery.aiohttp.ClientSession", return_value=mock_session), patch(
            "codex_crew.mcp_discovery._runtime_grant_present", AsyncMock(return_value=True)
        ):
            result = await _probe_remote(server)
    ...
    async def test_a_rejected_static_credential_still_records_the_oauth_challenge(self) -> None:
        ...
        with patch("codex_crew.mcp_discovery.aiohttp.ClientSession", return_value=mock_session), patch(
            "codex_crew.mcp_discovery._runtime_grant_present", grant_lookup
        ):
            result = await _probe_remote(server)
    ...
    def test_the_probe_cache_round_trips_the_authorization_evidence(self) -> None:
        ...
        from codex_crew.mcp_discovery import _cache_probe, probe_metadata
    ...
    async def test_an_unobservable_grant_is_omitted_rather_than_reported_absent(self) -> None:
        ...
        with patch("codex_crew.mcp_discovery.aiohttp.ClientSession", return_value=mock_session), patch(
            "codex_crew.mcp_discovery._runtime_grant_present", AsyncMock(return_value=None)
        ):
            result = await _probe_remote(server)
    ...
    async def test_the_probe_asks_for_the_absent_grant_to_be_audited(self) -> None:
        ...
        """
        from codex_crew.mcp_discovery import _runtime_grant_present

        lookup = AsyncMock(return_value=False)
        with patch("codex_crew.mcp_discovery.grant_observed", lookup):
    ...
    def test_importing_this_module_does_not_pull_in_the_mint_engine(self) -> None:
        ...
        """
        probe = (
            "import sys; import codex_crew.mcp_discovery;"
            " print('MINT' if 'codex_crew.connections.mint' in sys.modules else 'CLEAN')"
        )
        out = subprocess.run(
    ...
    async def test_a_credential_bearing_url_never_reaches_the_log(self, caplog) -> None:
        ...
        with patch("codex_crew.mcp_discovery.grant_observed", AsyncMock(return_value=None)):
            with caplog.at_level(logging.DEBUG, logger="codex_crew.mcp_discovery"):
    ...
    async def test_probe_dispatches_to_remote(self) -> None:
        ...
        with patch("codex_crew.mcp_discovery._probe_remote", new_callable=AsyncMock) as mock_remote:
...
class TestProbeServerConsentGate:
    ...
    async def test_disabled_stdio_server_is_never_spawned(self) -> None:
        ...
        with (
            patch("codex_crew.mcp_discovery.shutil.which", return_value="/bin/true"),
            patch(
                "codex_crew.mcp_discovery.create_subprocess_limited",
    ...
    async def test_refusal_does_not_clobber_cached_tools(self) -> None:
        ...
        with patch("codex_crew.mcp_discovery.shutil.which", return_value="/bin/true"):
...
class TestProbeTempContainment:
    ...
    async def test_spec_declared_temp_suppresses_probe_containment(self, tmp_path) -> None:
        ...
        from codex_crew.mcp_gateway import backend_tmp as bt
    ...
    async def test_probe_tmp_allocated_before_wrap_and_carved_out(
        ...
        monkeypatch.setattr("codex_crew.mcp_discovery.sandboxed_spawn_argv", _wrap)
    ...
    @pytest.mark.asyncio
    async def test_probe_drops_reserved_codexcrew_namespace_from_spec_env(
        self, tmp_path, monkeypatch
    ) -> None:
        ...
        only ``cron_script`` had a ``CODEXCREW_*`` deny (``_CRON_ENV_DENY``, via
        ``CODEXCREW_OWNER_ID``) -- the probe applied none. Putting the
        ...
        monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path / "real-home"))
        monkeypatch.delenv("CODEXCREW_CLI", raising=False)
        ...
        server = McpServerInfo(
            ...
            env={
                "CODEXCREW_CLI": "1",
                "CODEXCREW_SESSION_KEY": "some-other-session",
                "CODEXCREW_HOME": str(tmp_path / "attacker-home"),
                "MCP_TOKEN": "keep-me",
            },
        )
        with patch(
        ...
        captured = spawn_mock.call_args.kwargs["env"]
        assert "CODEXCREW_CLI" not in captured
        assert "CODEXCREW_SESSION_KEY" not in captured
        # Inherited value survives; the spec's override of it does not.
        assert captured["CODEXCREW_HOME"] == str(tmp_path / "real-home")
...
class TestProbeServerProcessCleanup:
    ...
    async def test_graceful_stdin_close(self) -> None:
        ...
        with (
            patch("codex_crew.mcp_discovery.asyncio.create_subprocess_exec", return_value=proc),
            patch("codex_crew.mcp_discovery.shutil.which", return_value="/usr/bin/echo"),
        ):
    ...
    async def test_probe_spawns_with_the_expanded_path(self, monkeypatch) -> None:
        ...
        with (
            patch("codex_crew.mcp_discovery.asyncio.create_subprocess_exec", side_effect=_spawn),
            patch("codex_crew.mcp_discovery.shutil.which", return_value="/usr/bin/echo"),
        ):
            proc.stdout = AsyncMock()
            proc.stdout.readline = AsyncMock(return_value=b"")
...
class TestInstallAgentRemote:
    ...
    def test_install_preserves_remote_server(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.agent.KIRO_AGENTS_DIR", kiro_dir)
        monkeypatch.setattr(
            "codex_crew.agent._KIRO_MCP_JSON", tmp_path / "nonexistent_kiro_mcp.json"
        )
        monkeypatch.setattr("codex_crew.agent._CC_MCP_JSON", tmp_path / "nonexistent_cc.json")
        monkeypatch.setattr("codex_crew.agent._CODEXCREW_BIN", "/usr/bin/codexcrew")
        monkeypatch.setattr("shutil.which", lambda cmd, path=None: None)

        install_agent()

        data = json.loads((kiro_dir / "codexcrew.json").read_text(encoding="utf-8"))
    ...
    def test_install_merges_kiro_mcp_json(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.agent.KIRO_AGENTS_DIR", kiro_dir)
        monkeypatch.setattr("codex_crew.agent._KIRO_MCP_JSON", settings_dir / "mcp.json")
        monkeypatch.setattr("codex_crew.agent._CC_MCP_JSON", tmp_path / "nonexistent_cc.json")
        monkeypatch.setattr("codex_crew.agent._CODEXCREW_BIN", "/usr/bin/codexcrew")
        monkeypatch.setattr("shutil.which", lambda cmd, path=None: None)
...
class TestGetProbeTimeout:
    ...
    def test_get_probe_timeout_reads_config(self) -> None:
        ...
        from codex_crew.mcp_discovery import _get_probe_timeout
        ...
        with patch("codex_crew.config.loader.CodexCrewConfig", mock_cls):
    ...
    def test_get_probe_timeout_fallback(self) -> None:
        ...
        from codex_crew.mcp_discovery import _PROBE_TIMEOUT_SECS, _get_probe_timeout
...
class TestProbeServerTimeout:
    ...
    async def test_probe_server_timeout_on_tools_list(self) -> None:
        ...
        with (
            ...
            patch("codex_crew.config.loader.CodexCrewConfig") as mock_cls,
        ):
            mock_cfg = MagicMock()
...
class TestFixStaleManagedCommand:
    ...
    The managed invocation is delegated to ``_codexcrew_mcp_invocation`` (the
    single source of truth), which returns a runnable ``(command, args)`` —
    either a standalone ``codexcrew`` binary (POSIX ``bin/codexcrew`` / Windows
    ``Scripts\\codexcrew.exe``) or the ``<interpreter> -m codex_crew <sub>``
    ...
    args and spawned a bare ``codexcrew`` that isn't on PATH — the Windows
    ``command not found: codexcrew`` regression)."""

    @pytest.fixture(autouse=True)
    def _reset_cache(self):
        import codex_crew.mcp_discovery as _d
    ...
    def test_rewrites_command_and_args_from_invocation(self):
        """Both command and args come from _codexcrew_mcp_invocation."""
        from codex_crew.mcp_discovery import _fix_stale_managed_command

        spec = {"command": "/stale/bin/codexcrew", "args": ["mcp-core"]}
        with patch(
            "codex_crew.agent._codexcrew_mcp_invocation",
            return_value=("/usr/local/bin/codexcrew", ["mcp-core"]),
        ) as inv:
            _fix_stale_managed_command("codexcrew-core", spec)
        inv.assert_called_once_with("mcp-core")
        assert spec["command"] == "/usr/local/bin/codexcrew"
        assert spec["args"] == ["mcp-core"]

    def test_applies_python_dash_m_fallback_with_args(self):
        """When no standalone binary resolves, the python -m codex_crew fallback
        (command + its args) is applied — regression for Windows where rewriting
        the command alone left a bare 'codexcrew' that isn't on PATH."""
        from codex_crew.mcp_discovery import _fix_stale_managed_command

        spec = {"command": "codexcrew", "args": []}
        with patch(
            "codex_crew.agent._codexcrew_mcp_invocation",
            return_value=("/venv/Scripts/python.exe", ["-m", "codex_crew", "mcp-cron"]),
        ):
            _fix_stale_managed_command("codexcrew-cron", spec)
        assert spec["command"] == "/venv/Scripts/python.exe"
        assert spec["args"] == ["-m", "codex_crew", "mcp-cron"]

    def test_maps_each_managed_server_to_its_subcommand(self):
        from codex_crew.mcp_discovery import _fix_stale_managed_command

        for name, sub in (("codexcrew-core", "mcp-core"), ("codexcrew-cron", "mcp-cron")):
            spec = {"command": "x", "args": []}
            with patch(
                "codex_crew.agent._codexcrew_mcp_invocation", return_value=("/bin/codexcrew", [sub])
            ) as inv:
    ...
    def test_skips_non_managed_server(self):
        ...
        with patch("codex_crew.agent._codexcrew_mcp_invocation") as inv:
    ...
    def test_caches_resolution_across_calls(self):
        ...
        with patch(
            "codex_crew.agent._codexcrew_mcp_invocation", return_value=("/bin/codexcrew", ["mcp-core"])
        ) as inv:
            _fix_stale_managed_command("codexcrew-core", {"command": "x", "args": []})
            _fix_stale_managed_command("codexcrew-core", {"command": "y", "args": []})
    ...
    def test_resolution_failure_leaves_spec_untouched(self):
        ...
        spec = {"command": "/old/codexcrew", "args": ["mcp-core"]}
        with patch("codex_crew.agent._codexcrew_mcp_invocation", side_effect=RuntimeError("boom")):
            _fix_stale_managed_command("codexcrew-core", spec)
        assert spec["command"] == "/old/codexcrew"
...
class TestSharedServerToolsRegistration:
    ...
    def test_shared_servers_added_to_tools_and_allowedtools(self, tmp_path, monkeypatch) -> None:
        ...
        from codex_crew.agent import rebuild_agent_config
...
class TestProbeServerStderrCapture:
    ...
    async def test_stderr_captured_when_child_exits_before_response(self, tmp_path) -> None:
        ...
        from codex_crew.mcp_discovery import probe_server
        ...
        with patch("codex_crew.config.loader.CodexCrewConfig") as mock_cls:
            mock_cfg = MagicMock()
    ...
    async def test_long_probe_error_keeps_remedy_sentence(self, monkeypatch) -> None:
        ...
        """
        from codex_crew.mcp_discovery import _PROBE_ERROR_MAX_CHARS, probe_server
        ...
        long_msg = (
            ...
            + ("x" * 300)
            + " set agent.sandbox_allow_unsandboxed_exec=true in ~/.codex-crew/config.json"
        )
        ...
        monkeypatch.setattr(
            "codex_crew.mcp_discovery.shutil.which", lambda *a, **k: "/usr/bin/srv"
        )

        def boom(*_a: object, **_k: object) -> object:
            raise RuntimeError(long_msg)

        monkeypatch.setattr("codex_crew.mcp_discovery.sandboxed_spawn_argv", boom)
        result = await probe_server(server)
...
class TestProbeStdioMalformedResponse:
    ...
    def test_init_error_as_string_does_not_crash(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("shutil.which", lambda cmd, path=None: "/usr/bin/srv")
        with patch(
            "codex_crew.mcp_discovery.asyncio.create_subprocess_exec",
...
class TestReadStdioJsonrpcResponse:
    ...
    async def test_banner_flood_capped(self) -> None:
        ...
        from codex_crew.mcp_discovery import _MAX_BANNER_LINES
...
class TestProbeServerBannerTolerance:
    ...
    async def test_leading_banner_does_not_fail_probe(self) -> None:
        ...
        with (
            patch("codex_crew.mcp_discovery.asyncio.create_subprocess_exec", return_value=proc),
            patch("codex_crew.mcp_discovery.shutil.which", return_value="/usr/bin/local-chorus-mcp"),
        ):
            result = await probe_server(server)
...
class TestProbeGroupReap:
    ...
    async def test_spawn_uses_start_new_session_on_posix(self) -> None:
        ...
        with (
            ...
            patch("codex_crew.mcp_discovery.shutil.which", return_value="/usr/bin/echo"),
            patch("codex_crew.mcp_discovery.os.killpg"),
        ):
            await probe_server(server)
    ...
    async def test_teardown_reaps_process_group(self) -> None:
        ...
        with (
            ...
            patch("codex_crew.mcp_discovery.shutil.which", return_value="/usr/bin/echo"),
            patch("codex_crew.mcp_discovery.os.killpg") as mock_killpg,
        ):
            await probe_server(server)
    ...
    async def test_teardown_tolerates_empty_group(self) -> None:
        ...
        with (
            ...
            patch("codex_crew.mcp_discovery.shutil.which", return_value="/usr/bin/echo"),
            patch("codex_crew.mcp_discovery.os.killpg", side_effect=ProcessLookupError),
        ):
            result = await probe_server(server)
    ...
    async def test_real_grandchild_is_reaped(self, monkeypatch, tmp_path) -> None:
        ...
        monkeypatch.setattr("codex_crew.mcp_discovery._get_probe_timeout", lambda: 1)
        ...
        monkeypatch.setattr("codex_crew.mcp_discovery._PROBE_TEARDOWN_WAIT_SECS", 0.5)
...
class TestDisabledIsCrossScope:
    ...
    merge only marked rows introduced from the Codex Crew scope. A server also
    ...
    async def test_probe_server_refuses_a_cross_scope_disabled_row(
        ...
        monkeypatch.setattr("codex_crew.mcp_discovery.create_subprocess_limited", _no_spawn)
...
class TestProbeSandboxUnavailable:
    ...
    async def test_sandbox_refusal_is_reported_as_a_probe_limitation(self, monkeypatch) -> None:
        import codex_crew.mcp_discovery as md
        from codex_crew.sandbox import SandboxUnavailableError
        ...
        with patch("codex_crew.mcp_discovery.sandboxed_spawn_argv", _refuse), patch(
            "codex_crew.mcp_discovery.shutil.which", return_value="/usr/bin/node"
        ):
            result = await probe_server(server)
    ...
    async def test_a_managed_server_is_still_spawned_when_the_sandbox_works(self) -> None:
        ...
        stale ("command not found: codexcrew; the built-in cron/core tools then never
        ...
        server = McpServerInfo(name="codexcrew-core", command="codexcrew", args=["mcp-core"])
        with patch("codex_crew.mcp_discovery.sandboxed_spawn_argv", _wrap), patch(
            "codex_crew.mcp_discovery.shutil.which", return_value="/usr/bin/codexcrew"
        ):
            await probe_server(server)
    ...
    async def test_a_managed_server_falls_back_to_its_declaration_with_no_backend(
        ...
        for name, expect_tools in (("codexcrew-core", True), ("codexcrew-cron", True)):
            server = McpServerInfo(name=name, command="codexcrew", args=["mcp-x"])
            with patch("codex_crew.mcp_discovery.sandboxed_spawn_argv", _refuse), patch(
                "codex_crew.mcp_discovery.shutil.which", return_value="/usr/bin/codexcrew"
            ):
                result = await probe_server(server)
    ...
    async def test_the_remedy_paragraph_is_logged_once_per_server(
        ...
        with caplog.at_level(logging.WARNING, logger=md.logger.name):
            md._warn_probe_sandbox_unavailable_once("codexcrew-core")
            md._warn_probe_sandbox_unavailable_once("codexcrew-core")
...
class TestFirstPartyManagedArgv:
    ...
    _INVOCATION = ("/opt/codexcrew/bin/codexcrew", ["mcp-core"])

    def _patch_invocation(self, monkeypatch) -> None:
        import codex_crew.mcp_discovery as md

        monkeypatch.setattr(
            md, "_resolved_managed_invocation", {"codexcrew-core": self._INVOCATION}
        )
        # Default install: the package-derived managed env is empty.
        monkeypatch.setattr("codex_crew.agent._managed_mcp_env", lambda: {})
    ...
    def test_self_derived_managed_argv_is_first_party(self, monkeypatch) -> None:
        ...
        assert md._is_first_party_managed_argv(
            "codexcrew-core", self._INVOCATION[0], list(self._INVOCATION[1]), {}
        )
    ...
    def test_customized_command_under_a_managed_name_is_not(self, monkeypatch) -> None:
        ...
        assert not md._is_first_party_managed_argv(
            "codexcrew-core", "/home/user/evil-shim", list(self._INVOCATION[1]), {}
        )
        assert not md._is_first_party_managed_argv(
            "codexcrew-core", self._INVOCATION[0], ["mcp-core", "--extra"], {}
        )
    ...
    def test_the_package_derived_home_pin_still_matches(self, monkeypatch) -> None:
        ...
        the ``CODEXCREW_HOME`` pin this package derived — that must still count
        as first-party, and any EXTRA key alongside it must not."""
        import codex_crew.mcp_discovery as md

        self._patch_invocation(monkeypatch)
        pin = {"CODEXCREW_HOME": "/data/override-home"}
        monkeypatch.setattr("codex_crew.agent._managed_mcp_env", lambda: dict(pin))
        assert md._is_first_party_managed_argv(
            "codexcrew-core", self._INVOCATION[0], list(self._INVOCATION[1]), dict(pin)
        )
        assert not md._is_first_party_managed_argv(
            "codexcrew-core",
            self._INVOCATION[0],
    ...
    def test_the_interpreter_fallback_is_never_first_party(self, monkeypatch) -> None:
        """`python -m codex_crew` prepends the child's CWD to sys.path (3.10 has
        no -P), so a planted `codex_crew/` tree in an untrusted cwd would shadow
        the install — only a resolved console-script binary qualifies."""
        import sys

        import codex_crew.mcp_discovery as md

        fallback = (sys.executable, ["-m", "codex_crew", "mcp-core"])
        monkeypatch.setattr(md, "_resolved_managed_invocation", {"codexcrew-core": fallback})
        monkeypatch.setattr("codex_crew.agent._managed_mcp_env", lambda: {})
        assert not md._is_first_party_managed_argv(
            "codexcrew-core", fallback[0], list(fallback[1]), {}
        )
    ...
    def test_resolution_failure_fails_toward_not_first_party(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.agent._codexcrew_mcp_invocation", _boom)
        assert not md._is_first_party_managed_argv(
            "codexcrew-core", self._INVOCATION[0], list(self._INVOCATION[1]), {}
        )
    ...
    async def test_probe_passes_the_flag_for_a_self_derived_managed_server(
        ...
        server = McpServerInfo(
            name="codexcrew-core", command=self._INVOCATION[0], args=list(self._INVOCATION[1])
        )
        with patch("codex_crew.mcp_discovery.sandboxed_spawn_argv", _capture), patch(
            "codex_crew.mcp_discovery.shutil.which", return_value=self._INVOCATION[0]
        ):
            await probe_server(server)
