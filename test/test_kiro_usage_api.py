"""Tests for codex_crew.dashboard.handlers.kiro_usage_api — the direct RTS
...
import codex_crew.dashboard.handlers.kiro_usage_api as api
...
class TestLoadBearerToken:
    def test_reads_access_token_from_json(self):
        ...
        with patch("codex_crew.hooks.safe_read_file_internal", side_effect=fake_read), \
    ...
    def test_skips_expired_json_token(self):
        ...
        with patch("codex_crew.hooks.safe_read_file_internal", return_value=blob), \
             patch.object(api, "_CLI_SQLITE_DBS", ()), \
             patch.object(api, "_OTHER_SQLITE_DBS", ()):
    ...
    def test_missing_sources_return_none(self, tmp_path):
        with patch("codex_crew.hooks.safe_read_file_internal", return_value=None), \
    ...
    def test_falls_through_expired_json_to_sqlite(self, tmp_path):
        ...
        con.commit()
        con.close()
        with patch("codex_crew.hooks.safe_read_file_internal", return_value=stale), \
             patch("codex_crew.hooks.emit_internal_read_audit", return_value=True), \
    ...
    def test_sqlite_token_denied_when_audit_cannot_be_recorded(self, tmp_path):
        ...
        con.close()
        with patch("codex_crew.hooks.safe_read_file_internal", return_value=None), \
             patch("codex_crew.hooks.emit_internal_read_audit", return_value=False), \
             patch.object(api, "_CLI_SQLITE_DBS", (db,)), \
             patch.object(api, "_OTHER_SQLITE_DBS", ()):
            assert api._load_bearer_token() is None
    ...
    def test_skips_expired_sqlite_token(self, tmp_path):
        ...
        con.close()
        with patch("codex_crew.hooks.safe_read_file_internal", return_value=None), \
             patch("codex_crew.hooks.emit_internal_read_audit", return_value=True) as audit, \
             patch.object(api, "_CLI_SQLITE_DBS", (db,)), \
             patch.object(api, "_OTHER_SQLITE_DBS", ()):
            assert api._load_bearer_token() is None
    ...
    def test_sqlite_oserror_fails_closed(self, tmp_path):
        ...
        with patch("codex_crew.hooks.emit_internal_read_audit", return_value=True):
            now = datetime.now(timezone.utc)
    ...
    def test_json_token_non_dict_ignored(self):
        ...
        with patch("codex_crew.hooks.safe_read_file_internal", return_value=b"[]"), \
             patch.object(api, "_CLI_SQLITE_DBS", ()), \
             patch.object(api, "_OTHER_SQLITE_DBS", ()):
            assert api._load_bearer_token() is None
    ...
    def test_sqlite_opened_but_no_token_still_audits(self, tmp_path):
        ...
        con.commit()
        con.close()
        with patch("codex_crew.hooks.emit_internal_read_audit", return_value=True) as audit:
