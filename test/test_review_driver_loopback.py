...
calls ``_probe`` once per candidate port (``CODEXCREW_PORT``, the configured port,
...
class TestReviewDriverLoopbackCallsIgnoreTheProxy:
    ...
    def test_guarded_import_fallback_does_not_proxy_the_secret(self, listeners):
        ...
        ``codex_crew.loopback_http`` poisoned to ImportError, so the ``except``
        ...
        with mock.patch.dict(sys.modules, {"codex_crew.loopback_http": None}):
