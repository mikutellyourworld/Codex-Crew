...
from codex_crew.mcp_core import WAIT_PING_SECS, _call_tool
...
class _Clock:
    ...
    distinguish calls originating from the code-under-test (the ``codexcrew-core``
...
def _run_wait(reply_fn, *, seconds: int = MIN_WAIT, reason: str = "test", identified: bool = True):
    ...
    with (
        patch("codex_crew.mcp_core._post", side_effect=_fake_post),
        patch(
