...
from codex_crew.apps import job_sdk
...
class TestEveryUploadRefusalIsAudited:
    ...
    def _arrange(stack: ExitStack, **over: Any) -> mock.MagicMock:
        ...
        stack.enter_context(
            mock.patch(
                "codex_crew.apps.manager.is_app_enabled", return_value=over.get("enabled", True)
            )
        )
        granted = stack.enter_context(mock.patch("codex_crew.aws_consent.is_granted"))
        granted.return_value = over.get("granted", (True, ""))
        grant = stack.enter_context(mock.patch("codex_crew.aws_consent.read_grant"))
...
class TestRunnerAuthorization:
    def test_a_run_started_off_the_generic_surface_is_still_consent_gated(self, sdk):
        ...
        def fake_snapshot(argv):
            (Path(argv[0]) / "codexcrew-snapshot-20260101T000000Z.tar.gz").write_bytes(b"x")
            return 0
        ...
        with (
            ...
            mock.patch("codex_crew.aws_consent.is_granted", return_value=(False, "not confirmed")),
    ...
    def test_the_manifest_declares_the_jobs_grant(self):
        ...
        manifest = json.loads(
            (
                ...
                / "src/codex_crew/apps/builtins/aws_control/app.json"
...
class TestLedgerSurvives:
    def test_a_successful_run_still_writes_the_app_ledger(self, sdk):
        ...
        def fake_snapshot(argv):
            (Path(argv[0]) / "codexcrew-snapshot-20260101T000000Z.tar.gz").write_bytes(b"x" * 11)
            return 0
