...
from codex_crew.dashboard.chat import _dequeue_next_message
...
class TestDequeueNextMessage:
    ...
    def test_user_pasted_recovery_text_is_plain_user_content(self):
        ...
        from codex_crew.dashboard.chat_utils import _EMPTY_AUTO_CONTINUE_MSG
...
class TestDashboardConfigMergeQueued:
    ...
    async def test_get_includes_merge_queued_messages(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.config.loader.config_path", lambda: tmp_path / "config.json"
        )
    ...
    async def test_put_persists_merge_queued_messages(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.config.loader.config_path", lambda: cfg_file
        )
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        with patch("codex_crew.sel.sel") as mock_sel:
            mock_sel.return_value = MagicMock()
