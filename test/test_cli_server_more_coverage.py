"""Coverage for the thinly-tested command dispatchers in ``codex_crew.cli_server``.
...
class _SelRecorder:
    """Stand-in for :func:`codex_crew.sel.sel` that records audit calls."""
...
class TestResolveClientPortExFallThrough:
    ...
    def test_garbage_codexcrew_port_falls_to_default(self, monkeypatch) -> None:
    ...
    def test_garbage_bound_port_falls_to_default(self, monkeypatch) -> None:
        ...
        monkeypatch.setenv("CODEXCREW_BOUND_PORT", "")
        monkeypatch.setenv("CODEXCREW_BOUND_PORT", "12x4")
        assert cli_server.resolve_client_port_ex(None) == (_DEFAULT_PORT, False)

    def test_garbage_codexcrew_port_still_honours_bound_port(self, monkeypatch) -> None:
        """The fall-through lands on the NEXT source, not straight on the default."""
        monkeypatch.setenv("CODEXCREW_PORT", "oops")
        monkeypatch.setenv("CODEXCREW_BOUND_PORT", "9931")
...
class TestProbeDashboardHealth:
    ...
    def test_stale_marker_warns_on_stderr(self, monkeypatch, capsys) -> None:
        ...
        cli_server._probe_dashboard_health(5486)
    ...
    def test_network_error_is_swallowed(self, monkeypatch, capsys) -> None:
        ...
        cli_server._probe_dashboard_health(5486)  # must not raise
        assert capsys.readouterr().err == ""
...
class TestStopViaService:
    ...
    def test_service_stop_short_circuits_port_scan(self, monkeypatch, sel_rec, capsys) -> None:
        monkeypatch.setattr(cli_server, "resolve_client_port", lambda p: 5486)
        ...
        assert "Stopped codexcrew service" in capsys.readouterr().out
    ...
    def test_explicit_port_bypasses_the_service(self, monkeypatch, sel_rec, capsys) -> None:
        ...
        assert exc.value.code == 1
        assert "No Codex Crew gateway currently running on port 8123" in capsys.readouterr().out
...
class TestStopOnWindows:
    ...
    def _windows(self, monkeypatch):
        ...
        monkeypatch.setattr(cli_server, "_is_codexcrew_process", lambda pid: True)
...
class TestServiceCmd:
    ...
    def test_unknown_action_prints_usage_and_returns_2(self, sel_rec, capsys) -> None:
        ...
        assert "Usage: codexcrew service" in capsys.readouterr().err
...
class TestSandboxCmd:
    ...
    def test_unknown_action_prints_usage_and_returns_2(self, sel_rec, capsys) -> None:
        ...
        assert "Usage: codexcrew sandbox" in capsys.readouterr().err
        assert sel_rec.calls == []
...
class TestGateway:
    ...
    def gw(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(cli_server, "CodexCrewConfig", _Cfg)
        monkeypatch.setattr("codex_crew.cli._node_ok", lambda: True)
    ...
    def test_missing_dist_warns_but_still_starts(self, gw, monkeypatch, caplog) -> None:
        ...
        with caplog.at_level("WARNING", logger="codex_crew.cli_server"):
    ...
    def test_stale_node_triggers_ensure_node(self, gw, monkeypatch) -> None:
        calls: list[str] = []
        monkeypatch.setattr("codex_crew.cli._node_ok", lambda: False)
        monkeypatch.setattr("codex_crew.cli._ensure_node", lambda *a: calls.append("ensured"))
        asyncio.run(cli_server._gateway(no_dashboard=True))
    ...
    def test_mise_activation_is_logged_when_it_changes_the_env(
        ...
        with caplog.at_level("INFO", logger="codex_crew.cli_server"):
            asyncio.run(cli_server._gateway(no_dashboard=True))
...
def taskrunner_env(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(CodexCrewConfig, "load", classmethod(lambda cls: cls()))
    monkeypatch.setattr(cli_server, "CodexCrewConfig", CodexCrewConfig)
...
def git_checkout(monkeypatch, tmp_path):
    """A CODEXCREW_PROJECT_DIR that looks like a git checkout, with git stubbed out."""
    proj = tmp_path / "proj"
    ...
    monkeypatch.setenv("CODEXCREW_PROJECT_DIR", str(proj))
    ...
    monkeypatch.setattr(
        "codex_crew.platform.update_governance.resolve_remote_url",
        lambda p, remote="", branch="": "https://github.com/mikutellyourworld/Codex-Crew.git",
    )
    monkeypatch.setattr(
        "codex_crew.platform.update_governance.update_blocked_reason", lambda url: ""
    )
    ...
    monkeypatch.setattr("codex_crew.cli._ensure_node", lambda *a: None)
    ...
    # they assert. `codexcrew update`'s own substitute behaviour is covered in
...
class TestUpdateGitPath:
    ...
    def test_detached_head_is_treated_as_mainline_and_pin_is_checked_first(
        ...
        monkeypatch.setattr(
            "codex_crew.platform.update_governance.update_blocked_reason",
    ...
    def test_diverged_checkout_refuses_without_force(
        ...
        assert "codexcrew update --force" in out
        assert not any(c[:2] == ["git", "reset"] for c in stub.calls)
    ...
    def test_local_changes_confirmed_proceeds_to_reset(
        ...
        cli_server._update()
        assert any(c[:2] == ["git", "reset"] for c in stub.calls)
        assert "Codex Crew updated!" in capsys.readouterr().out
    ...
    def test_success_updates_kiro_cli_and_refreshes_agent_config(
        ...
        cli_server._update()
        out = capsys.readouterr().out
        assert "Codex Crew updated!" in out
...
class TestUpdateSubprocessHardening:
    ...
    def test_fetch_timeout_takes_the_existing_failure_branch(
        ...
        through ``codexcrew update`` instead of the SystemExit(1) the fetch
...
class TestUpdateWheelDispatch:
    ...
    def test_layout_is_built_from_the_distribution(self, monkeypatch, capsys) -> None:
        ...
        monkeypatch.setattr("codex_crew.platform.update_capability.distribution", lambda: "wheel")
        monkeypatch.setattr("codex_crew.cli_server.distribution", lambda: "wheel")
    ...
    def test_unknown_distribution_defaults_to_wheel(self, monkeypatch) -> None:
        monkeypatch.delenv("CODEXCREW_PROJECT_DIR", raising=False)
        monkeypatch.setattr("codex_crew.platform.update_capability.distribution", lambda: "")
        monkeypatch.setattr("codex_crew.cli_server.distribution", lambda: "")
        seen: list[InstallLayout] = []
        monkeypatch.setattr(cli_server, "_update_wheel", lambda layout: seen.append(layout))
        cli_server._update()
        assert seen[0].kind == "wheel"
...
def wheel_feed(monkeypatch):
    ...
    monkeypatch.setattr(
        "codex_crew.platform.update_layout.cdn_bases",
        lambda: ("https://cdn.example.com", "https://cdn.example.com"),
    )
    monkeypatch.setattr("codex_crew.platform.update_layout.release_channel", lambda: "stable")
    monkeypatch.setattr(
        "codex_crew.platform.update_layout.wheel_update_command",
...
class TestUpdateWheelFeedValidation:
    ...
    def test_channel_mismatch_is_refused(self, wheel_feed, capsys) -> None:
        wheel_feed(
            b'{"schema": "codexcrew-cli-artifact-manifest-v1", '
    ...
    def test_missing_version_is_refused(self, wheel_feed, capsys) -> None:
        wheel_feed(b'{"schema": "codexcrew-cli-artifact-manifest-v1", "channel": "stable"}')
        with pytest.raises(SystemExit) as exc:
            cli_server._update_wheel(_LAYOUT)
        assert exc.value.code == 1
...
class TestUpdateWheelInstaller:
    ...
    def test_shadow_failure_redacts_credentialed_urls(self, monkeypatch, capsys) -> None:
        ...
        CODEXCREW_CDN_BASE either would land credentials in terminal history.
        """
        from codex_crew.platform import wheel_engine
        from codex_crew.platform.wheel_engine import WheelUpdateError
    ...
    def test_success_reports_the_new_version_and_restart_hint(self, monkeypatch, capsys) -> None:
        ...
        assert "codexcrew restart" in out
    ...
    def test_already_latest_returns_without_running_the_installer(
        ...
        wheel_feed(
            b'{"schema": "codexcrew-cli-artifact-manifest-v1", "channel": "stable", '
