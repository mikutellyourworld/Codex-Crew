...
class TestExecutionDecision:
    ...
    def test_environment_variable_cannot_override_absent_policy(
        ...
        home.mkdir()
        monkeypatch.setenv("CODEXCREW_HOME", str(home))
        monkeypatch.setenv("CODEXCREW_APPS_ALLOW_THIRD_PARTY", "true")
        assert third_party_execution_allowed() is False
    ...
    def test_config_load_failure_fails_closed(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(CodexCrewConfig, "load", classmethod(_raise))
        assert execution.third_party_execution_allowed() is False
    ...
    def test_edition_manifest_source_builtin_is_admitted_with_containment(
        ...
        import codex_crew.platform as platform_mod
        from codex_crew.apps import execution
    ...
    def test_builtin_app_names_requires_builtin_owned_install(
        ...
        import codex_crew.platform as platform_mod
        from codex_crew.apps import execution
        from codex_crew.apps.manager import InstalledApp, _write_installed
...
class TestLaunchAndLifecycleBoundary:
    ...
    async def test_disabled_app_cannot_open_even_when_execution_is_admitted(
        ...
        import codex_crew.apps.routes as routes
        from codex_crew.apps import execution
    ...
    async def test_lifecycle_script_default_off_has_no_process_side_effect(
        ...
        import codex_crew.apps.routes as routes
        from codex_crew.apps import lifecycle_scripts
    ...
    async def test_lifecycle_script_runs_after_explicit_admission(
        ...
        from codex_crew.apps import execution, lifecycle_scripts
...
class TestTrustedGrantBounds:
    ...
    def test_name_at_the_cap_is_still_a_grant(self, tmp_path, monkeypatch) -> None:
        from codex_crew.apps.execution import _MAX_GRANT_NAME_LEN, trusted_app_names
    ...
    def test_over_long_list_is_truncated_not_honoured_whole(
        ...
        from codex_crew.apps.execution import _MAX_GRANT_ENTRIES, trusted_app_names
    ...
    def test_unknown_legacy_name_grant_cannot_claim_fresh_local_source(
        ...
        monkeypatch.setattr("codex_crew.apps.manager.get_app", lambda name: None)
    ...
    def test_installed_legacy_local_grant_remains_migration_compatible(
        ...
        monkeypatch.setattr(
            "codex_crew.apps.manager.get_app",
    ...
    def test_runtime_legacy_repository_never_consults_catalog(
        ...
        from codex_crew.apps import execution, registry
        from codex_crew.config.loader import CodexCrewConfig
    ...
    def test_config_derived_grant_name_never_enters_failure_logs(
        ...
        from codex_crew.apps import execution, manager, registry
