...
from codex_crew.dashboard.origin import mark_audit_claimed
...
async def test_health_returns_ok_with_identity() -> None:
    ...
    ~/.codexcrew and the gateway port, so the shell must be able to tell
    WHICH CodexCrew-family gateway owns the port."""
...
async def test_rebound_loopback_health_omits_build_identity() -> None:
    ...
    req.app = {"allowed_origins": {"http://localhost:5486"}}
...
async def test_direct_local_health_with_served_host_keeps_identity() -> None:
    ...
    req = _probe_req(headers={"Host": "127.0.0.1:5486"})
    req.app = {"allowed_origins": {"http://localhost:5486"}}
    resp = await core_mod.api_health(req)
    body = json.loads(resp.body)
    assert body["app"] == "codexcrew"
...
def _req_with_state(
    ...
    req = MagicMock(spec=web.Request)
    req.headers = {"Host": "127.0.0.1:5486" if host_allowed else "attacker.example"}
...
async def test_public_probe_contract_frozen_minimal_anonymous_surface_and_statuses() -> None:
    ...
    shutdown.set()
    state.ready = True
    with patch("codex_crew.shutdown_event", shutdown):
        response = await core_mod.api_ready(serving)
        assert response.status == 503
        assert json.loads(response.body)["ready"] is False
...
# function-local `from codex_crew import shutdown_event`, so patching the source
...
async def test_ready_returns_503_during_shutdown() -> None:
    ...
    state = MagicMock()
    state.sessions = MagicMock()
    with patch("codex_crew.shutdown_event", ev):
        resp = await core_mod.api_ready(_req_with_state(state))
    assert resp.status == 503
    body = json.loads(resp.body)
    assert body["ready"] is False
...
async def test_disallowed_host_probes_pass_through_middleware_chain() -> None:
    ...
    async with TestClient(TestServer(_host_barrier_app())) as client:
        for path in ("/api/health", "/api/live"):
            resp = await client.get(path, headers={"Host": "10.42.7.13:5486"})
        ...
        resp = await client.get("/api/ready", headers={"Host": "10.42.7.13:5486"})
...
async def test_allowed_host_non_probe_passes_host_barrier() -> None:
    ...
    async with TestClient(TestServer(_host_barrier_app())) as client:
        resp = await client.get("/api/sessions", headers={"Host": "localhost:5486"})
        assert resp.status == 200
...
def test_every_middleware_denial_is_audited_off_the_loop() -> None:
    ...
    for module, path in (
        ("codex_crew.dashboard.stt_stream", "stt_stream"),
        ("codex_crew.dashboard.handlers.terminal", "handlers/terminal"),
    ):
...
def test_api_server_resolves_bind_address_via_shared_helper() -> None:
    ...
    resolve its TCP bind through bind_address_for (CODEXCREW_BIND-aware,
