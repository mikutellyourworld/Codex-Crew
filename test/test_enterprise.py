"""Tests for codex_crew.slack.enterprise — workspace validation.
...
def _reset_module_state(tmp_path, monkeypatch):
    ...
    runs against an isolated, initially-empty ``CODEXCREW_HOME`` to avoid coupling
...
def test_degraded_read_does_not_admit_caller_supplied_ids(tmp_path):
    ...
    The caller passes ``extra_ids`` from its OWN ``CodexCrewConfig.load()``-derived
...
def test_clean_read_does_not_admit_caller_supplied_ids(tmp_path):
    ...
    ``extra_ids`` is the caller's earlier ``CodexCrewConfig.load()`` snapshot of
...
#
# CodexCrewConfig.load() degrades a torn/corrupt config to a *defaults* object
...
def test_reader_entry_filter_does_not_drift_from_the_loader(tmp_path):
    """Pin `_read_allowlist`'s entry filter to `CodexCrewConfig.load()`'s.
    ...
    via_loader = set(CodexCrewConfig.load().slack.allowed_enterprise_ids)
