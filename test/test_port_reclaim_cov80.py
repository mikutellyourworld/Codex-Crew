"""Coverage for the platform-specific edges of :mod:`codex_crew.dashboard.port_reclaim`.
...
import codex_crew.cli_server as cli_server
from codex_crew.dashboard import port_reclaim as pr
...
def test_listeners_posix_parses_and_dedupes_lsof_output(monkeypatch) -> None:
    ...
    assert pr._listeners_on_port(5486) == [1234, 5678]
    ...
    assert calls == [["/usr/bin/lsof", "-ti", "TCP:5486", "-sTCP:LISTEN"]]
...
def test_listeners_posix_lsof_missing_returns_none(monkeypatch) -> None:
    ...
    assert pr._listeners_on_port(5486) is None
...
def test_listeners_posix_no_match_returns_empty_list(monkeypatch) -> None:
    ...
    assert pr._listeners_on_port(5486) == []
...
async def test_default_identity_check_is_the_shared_cli_server_gate(monkeypatch) -> None:
    """With no injected checker, reclaim uses the same gate as `codexcrew stop`."""
    asked: list[int] = []

    def _is_codexcrew(pid: int) -> bool:
        asked.append(pid)
        return False

    monkeypatch.setattr(cli_server, "_is_codexcrew_process", _is_codexcrew)
    monkeypatch.setattr(pr.platform_compat, "process_thread_count", lambda _pid: 4)

    outcome = await pr.reclaim_stale_gateway_port(
        5486, list_listeners=lambda _p: [7777], probe_healthy=_unhealthy
    )
