...
``codexcrew.json`` (``mint._write_mint_agent_spec``,
...
from codex_crew.agent import _install_heartbeat_agent, migrate_agent_specs
from codex_crew.agent_files import AGENT_FILENAME, HEARTBEAT_AGENT_FILENAME
from codex_crew.connections import mint
from codex_crew.dashboard.chat_persistence import _build_kiro_model_map
...
def agents_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.agent.KIRO_AGENTS_DIR", tmp_path)
    return tmp_path
...
def identity_bindings(monkeypatch):
    """Bind every Codex Crew agent name to a same-named kiro agent.

    Without this the real resolver maps an unknown name onto the ``codexcrew``
...
def agents_dir_resolver(agents_dir, monkeypatch):
    ...
    the next one.
    """
    from codex_crew.config import paths as config_paths
    from codex_crew.cron_script import _resolve_mcp_server
...
class TestFindServerSpecAnywhere:
    ...
    def test_refused_agent_spec_contributes_no_spec(self, agents_dir_resolver, kind):
        _plant_refused(
            agents_dir_resolver,
            AGENT_FILENAME,
            {"name": "codexcrew", "mcpServers": {"phantom-hr": {"command": "x"}}},
            kind,
        )
    ...
    def test_valid_agent_spec_still_found_under_the_same_cap(self, agents_dir_resolver):
        _plant(
            agents_dir_resolver,
            AGENT_FILENAME,
            {"name": "codexcrew", "mcpServers": {"real-hr": {"command": "x", "disabled": True}}},
        )
...
class TestResolveMcpServer:
    ...
    def test_refused_agent_spec_resolves_to_no_server(self, agents_dir_resolver, kind):
        ...
        _plant_refused(
            agents_dir_resolver,
            AGENT_FILENAME,
            {"name": "codexcrew", "mcpServers": {"srv-hr": {"command": "c", "args": ["a"]}}},
            kind,
        )
...
class TestLoadAgentConfig:
    ...
    def test_refused_agent_spec_contributes_no_servers(self, agents_dir_resolver, kind):
        from codex_crew.mcp_discovery import _load_agent_config

        _plant_refused(
            agents_dir_resolver,
            AGENT_FILENAME,
            {"name": "codexcrew", "mcpServers": {"phantom-md": {"command": "x"}}},
            kind,
        )
    ...
    def test_valid_agent_spec_still_merges_under_the_same_cap(self, agents_dir_resolver):
        ...
        _plant(
            agents_dir_resolver,
            AGENT_FILENAME,
            {"name": "codexcrew", "mcpServers": {"real-md": {"command": "x"}}},
        )
...
class TestLoadSteeringResources:
    ...
    def test_refused_agent_spec_loads_no_steering(self, agents_dir_resolver, monkeypatch, kind):
        from codex_crew.context import _load_steering_resources

        self._plant_steering(agents_dir_resolver, monkeypatch)
        _plant_refused(
            agents_dir_resolver,
            AGENT_FILENAME,
            {"name": "codexcrew", "resources": ["file://steer.md"]},
            kind,
        )
...
class TestDenialAuditNeverRaises:
    ...
    def test_reader_audit_failure_still_degrades_to_absent(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level("WARNING", logger="codex_crew.agent_discovery"):
...
class TestSensitiveSymlinkGuard:
    ...
    def test_link_to_a_sensitive_target_is_refused(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.agent.KIRO_AGENTS_DIR", agents)
...
class TestWriteMintAgentSpec:
    ...
    spawns ``kiro-cli --agent codexcrew``, and the child would reload the very
...
class TestInstallHeartbeatAgent:
    ...
    def heartbeat_env(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.agent._background_agent_model", lambda: "auto")
        monkeypatch.setattr("codex_crew.agent.agent_state.set_cc_model", lambda *a, **k: None)
    ...
    def test_refused_main_spec_yields_no_mcp_servers(self, agents_dir, heartbeat_env, kind):
        _plant_refused(
            agents_dir,
            AGENT_FILENAME,
            {"mcpServers": {"codexcrew-core": {"command": "x"}}},
            kind,
        )
    ...
    def test_valid_main_spec_still_feeds_the_heartbeat_agent(self, agents_dir, heartbeat_env):
        _plant(
            agents_dir,
            AGENT_FILENAME,
            {"mcpServers": {"codexcrew-core": {"command": "x", "args": ["--include-tools", "a"]}}},
        )

        _install_heartbeat_agent()

        written = json.loads((agents_dir / HEARTBEAT_AGENT_FILENAME).read_text(encoding="utf-8"))
        assert written["mcpServers"]["codexcrew-core"]["args"] == []
        assert written["tools"] == ["@codexcrew-core"]
...
class TestProjectNamesDenialAttribution:
    ...
    def test_default_call_shape_emits_exactly_the_historical_event(self, tmp_path, monkeypatch):
        """The compatibility defaults preserve the pre-attribution event."""
        from codex_crew.agent_discovery import project_agent_names
    ...
    def test_labelled_call_attributes_the_denial_to_that_surface(self, tmp_path, monkeypatch):
        ...
        result = project_agent_names(
            project_dir, operation="api_codexcrew_agents", source="dashboard"
        )
        assert result == frozenset()
        assert len(events) == 1
        assert events[0]["operation"] == "api_codexcrew_agents"
...
_EXPECTED_CALL_SITE_LABELS: dict[str, list[tuple[str, str]]] = {
    ...
    "codex_crew/cli_doctor.py": [("doctor", "cli"), ("doctor", "cli"), ("doctor", "cli")],
    "codex_crew/config/loader.py": [("load_config", "unknown")],
    "codex_crew/connections/mint.py": [
        ("connections_mint", "dashboard"),
        ("connections_mint", "dashboard"),
    ],
    "codex_crew/connections/warm.py": [
        ("connections_warm_mint", "dashboard"),
        ("connections_warm_mint", "dashboard"),
    ],
    "codex_crew/context.py": [("steering_resources", "unknown")],
    "codex_crew/cron_script.py": [("cron_resolve_mcp_server", "cron")],
    "codex_crew/dashboard/handlers/agents.py": [
        ("api_agent_detail", "dashboard"),
        ("api_agent_detail", "dashboard"),
        ("api_agents_sync", "dashboard"),
    ],
    "codex_crew/dashboard/handlers/hooks.py": [("api_kiro_hooks", "dashboard")],
    ...
    "codex_crew/mcp_discovery.py": [("mcp_discovery_agent_config", "unknown")],
...
_EXPECTED_PROJECT_NAMES_CALL_SITE_LABELS: dict[str, list[tuple[str, str]]] = {
    "codex_crew/agent_discovery.py": [("forward:operation", "forward:source")],
    "codex_crew/config/loader.py": [("project_declares_agent", "unknown")],
    "codex_crew/dashboard/handlers/agents.py": [("api_codexcrew_agents", "dashboard")],
}
...
_EXPECTED_WARM_CALL_SITE_LABELS: dict[str, list[tuple[str, str]]] = {
    "codex_crew/dashboard/chat_handlers.py": [
        ("api_chat", "dashboard"),
        ("api_chat_slot_agent", "dashboard"),
    ],
    "codex_crew/dashboard/chat_runner.py": [("chat_turn", "unknown")],
    "codex_crew/dashboard/handlers/side.py": [("side_panel", "dashboard")],
    "codex_crew/spawn_warm.py": [("spawn_warm", "unknown")],
}
