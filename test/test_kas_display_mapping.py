...
def test_current_mode_update_emits_on_first_and_on_change() -> None:
    ...
    events = _update(handle, {"sessionUpdate": "current_mode_update", "currentModeId": "codexcrew"})
    assert len(events) == 1 and events[0].kind == EVENT_AGENT_SWITCHED and events[0].text == "codexcrew"
...
def test_current_mode_update_reassert_is_noop() -> None:
    handle = _handle(ACP_BACKEND_KAS)
    first = _update(handle, {"sessionUpdate": "current_mode_update", "currentModeId": "codexcrew"})
    assert len(first) == 1  # first frame emits
    # A re-assert of the already-current mode must not echo a spurious switch.
    assert _update(handle, {"sessionUpdate": "current_mode_update", "currentModeId": "codexcrew"}) == []
