...
from codex_crew import config as codex_crew_config
from codex_crew.acp import kas_agents
from codex_crew.acp.kas_agents import (
...
def _spec(**over):
    base = {
        ...
        "prompt": "You are Kiro.",
        "tools": ["fs_read", "fs_write", "@codexcrew-core"],
...
class TestRequiredFields:
    ...
    def test_id_and_prompt_are_emitted(self):
        out = to_client_custom_agent("codexcrew", _spec(), "You are Kiro.")
        assert out["id"] == "codexcrew"
    ...
    def test_empty_prompt_is_refused(self):
        with pytest.raises(KasAgentTranslationError):
            to_client_custom_agent("codexcrew", _spec(), "   ")
...
class TestToolsFailClosed:
    ...
    def test_list_is_passed_through(self):
        ...
        assert out["tools"] == ["fs_read", "fs_write", "@codexcrew-core"]

    def test_mcp_server_shorthand_survives(self):
        """KAS tags every MCP tool ``@<server>``, so Crew's existing syntax works."""
        out = to_client_custom_agent("a", _spec(tools=["@codexcrew-cron"]), "p")
        assert out["tools"] == ["@codexcrew-cron"]
...
class TestTheCeilingIsReAskedAtProjectionTime:
    ...
    def test_the_withholding_is_reported_so_a_missing_grant_is_explainable(
        ...
        monkeypatch.setattr(kas_agents, "may_skip_gate_now", lambda ref: False)
        with caplog.at_level("INFO", logger="codex_crew.acp.kas_agents"):
            to_client_custom_agent("codexcrew", _spec(allowedTools=["web_fetch"]), "p")
    ...
    def test_the_withhold_is_recorded_in_the_security_event_log(self, monkeypatch):
        ...
        to_client_custom_agent("codexcrew", _spec(allowedTools=["@denied-srv"]), "p")
        ...
        assert "codexcrew" in events[0]["resources"]
...
class TestKeysTheWireCannotCarry:
    ...
    def test_the_keys_are_named(self, caplog):
        ...
        with caplog.at_level("DEBUG", logger="codex_crew.acp.kas_agents"):
            to_client_custom_agent("codexcrew", spec, "p")
    ...
    def test_it_does_not_warn_on_every_session(self, caplog):
        ...
        with caplog.at_level("WARNING"):
            to_client_custom_agent("codexcrew", _spec(toolsSettings={"x": 1}), "p")
    ...
    def test_nothing_logged_when_the_spec_has_none(self, caplog):
        with caplog.at_level("DEBUG", logger="codex_crew.acp.kas_agents"):
            to_client_custom_agent("codexcrew", _spec(), "p")
...
class TestPromptResolution:
    ...
    def test_empty_prompt_falls_back_to_the_kas_constant(self, bad, tmp_path, caplog):
        ...
        # intentionally prompt-less agent (e.g. codexcrew-lite ships "prompt": ""),
        # so the projection substitutes the small inline fallback constant
        # instead of crashing the session.
        out = resolve_prompt({"prompt": bad}, agent_id="codexcrew-lite", agents_dir=tmp_path)
    ...
    def test_build_projects_a_prompt_less_spec_with_the_fallback(self, tmp_path):
        (tmp_path / "codexcrew-lite.json").write_text(
            json.dumps({"name": "codexcrew-lite", "tools": [], "prompt": ""}), encoding="utf-8"
        )
        agents = build_kas_custom_agents(tmp_path, "codexcrew-lite")
...
class TestAgainstTheRealBundledSpec:
    ...
    @staticmethod
    def _bundled() -> dict:
        path = Path(codex_crew_config.__file__).resolve().parent / "defaults.json"
    ...
    def test_the_bundled_template_carries_refs_but_declares_no_servers(self):
        ...
        ``defaults.json`` ships ``@codexcrew-*`` refs with NO ``mcpServers`` key:
    ...
    def test_every_mcp_ref_resolves_to_a_declaration_on_a_materialized_spec(self):
        ...
        spec["mcpServers"] = {
            name: {"command": "/opt/codexcrew", "args": [f"mcp-{name.split('-')[-1]}"]}
            for name in refs
        }
...
class TestMcpServersProjection:
    ...
    ``tools: ["@codexcrew-core", ...]`` and no definition of what that names.
    """

    def test_declared_server_is_projected(self):
        out = to_client_custom_agent("a", _spec(), "p")
        assert out["mcpServers"] == {"codexcrew-core": {"command": "x"}}
    ...
    def test_stubbed_names_are_withheld(self):
        ...
        out = to_client_custom_agent(
            "a",
            _spec(mcpServers={"codexcrew-core": {"command": "x"}, "other": {"command": "y"}}),
            "p",
            stub_server_names=frozenset({"codexcrew-core"}),
        )
        assert out["mcpServers"] == {"other": {"command": "y"}}

    def test_all_names_stubbed_emits_nothing(self):
        out = to_client_custom_agent(
            "a", _spec(), "p", stub_server_names=frozenset({"codexcrew-core"})
        )
    ...
    def test_auto_approve_is_stripped_from_a_managed_server_too(self):
        out = to_client_custom_agent(
            "a",
            _spec(mcpServers={"codexcrew-core": {"command": "x", "autoApprove": ["t"]}}),
            "p",
        )
        assert "autoApprove" not in out["mcpServers"]["codexcrew-core"]

    def test_managed_server_keeps_the_one_env_key_it_needs(self):
        """Crew's own env pins CODEXCREW_HOME; dropping it would have the shims
        read a different data home than the gateway."""
        out = to_client_custom_agent(
            "a",
            _spec(
                mcpServers={
                    "codexcrew-core": {"command": "x", "env": {"CODEXCREW_HOME": "/h"}}
                }
            ),
            "p",
        )
        assert out["mcpServers"]["codexcrew-core"]["env"] == {"CODEXCREW_HOME": "/h"}
    ...
    def test_a_hand_added_env_key_under_a_managed_name_is_withheld(self):
        ...
        CODEXCREW_HOME survives; the neighbouring secret does not reach the wire.
        """
        out = to_client_custom_agent(
            "a",
            _spec(
                mcpServers={
                    "codexcrew-core": {
                        "command": "x",
                        "env": {"CODEXCREW_HOME": "/h", "OPENAI_API_KEY": "sk-live"},
                    }
                }
            ),
            "p",
    ...
    def test_a_managed_env_of_only_secrets_leaves_no_env_at_all(self):
        out = to_client_custom_agent(
            "a",
            _spec(mcpServers={"codexcrew-cron": {"command": "x", "env": {"T": "s"}}}),
            "p",
        )
        entry = out["mcpServers"]["codexcrew-cron"]
    ...
    def test_a_malformed_managed_env_is_dropped_not_filtered(self):
        ...
        out = to_client_custom_agent(
            "a",
            _spec(mcpServers={"codexcrew-core": {"command": "x", "env": "TOKEN=s"}}),
            "p",
        )
        assert out["mcpServers"]["codexcrew-core"] == {"command": "x"}
            ...
            to_client_custom_agent(
                ...
                _spec(
                    mcpServers={
                        "codexcrew-core": {
                            "command": "x",
                            "env": {"CODEXCREW_HOME": "/h", "SECRET_TOKEN": "sekrit"},
                        }
                    }
                ),
                "p",
    ...
    def test_wrapper_marker_never_reaches_the_wire(self):
        ...
        out = to_client_custom_agent(
            ...
            _spec(
                mcpServers={
                    "srv": {"command": "x", "_codexcrew_mcp_gateway_wrapped": True},
                }
            ),
            "p",
        )
...
class TestRuntimeSuppliesTheStubbedSet:
    ...
    def _runtime(monkeypatch, overlay, seen):
        from codex_crew.acp import runtime as runtime_mod
    ...
    async def test_the_overlay_set_is_forwarded(self, monkeypatch):
        ...
        monkeypatch.setattr(
            runtime_mod, "injection_server_names", lambda _o, _a: frozenset({"codexcrew-core"})
        )

        await rt._kas_custom_agents("codexcrew")

        assert seen == [frozenset({"codexcrew-core"})]
    ...
    async def test_an_unreadable_overlay_still_yields_an_agent(self, monkeypatch):
        ...
        out = await rt._kas_custom_agents("codexcrew")
