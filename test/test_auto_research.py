...
from codex_crew.apps.builtins.auto_research.handlers import (
    DEFAULT_DEPTH_DECAY,
...
def _isolate(tmp_path: Path):
    ...
    with (
        patch(
            "codex_crew.apps.builtins.auto_research.handlers.DB_PATH",
            tmp_path / "test.db",
        ),
        patch(
            "codex_crew.apps.builtins.auto_research.handlers.RESEARCH_DIR",
            tmp_path / "research",
        ),
    ):
        yield tmp_path
...
class TestPathValidation:
    ...
    def test_worker_slot_key_contract(self):
        from codex_crew.apps.builtins.auto_research.session_keys import (
    ...
    def test_safe_dir_rejects_invalid(self, tmp_path: Path):
        with patch("codex_crew.apps.builtins.auto_research.handlers.RESEARCH_DIR", tmp_path):
...
class TestValidation:
    ...
    def test_overlength_model_rejected_not_truncated(self):
        ...
        from codex_crew.apps.builtins.auto_research.handlers import _campaign_model
...
class TestCycleFileMatching:
    ...
    def test_non_cycle_files_ignored(self, tmp_path: Path):
        ...
        with patch("codex_crew.apps.builtins.auto_research.handlers.RESEARCH_DIR", tmp_path):
            d = self._findings_dir(tmp_path)
            (d / "cycle_000.json").write_text(json.dumps({"cycle": 0, "new_findings_count": 1}))
            (d / "01-kiroom-vs-codexcrew.md").write_text("# not a cycle file")
...
class TestCRUD:
    ...
    def test_delete_missing(self):
        from codex_crew.apps.builtins.auto_research.handlers import delete_campaign
...
class TestStalledCampaignVerdict:
    ...
    def test_verified_finding_completes(self, _isolate: Path):
        from codex_crew.apps.builtins.auto_research.handlers import _stalled_campaign_verdict
    ...
    def test_stop_tombstone_is_preferred_over_worker_marker(
        ...
        from codex_crew.apps.builtins.auto_research import handlers as h
        from codex_crew.autonudge import AUTONUDGE_STOP_REASON
    ...
    def test_clear_marker_symlink_removes_link_not_target(self, _isolate: Path):
        ...
        from codex_crew.apps.builtins.auto_research.handlers import _clear_worker_done_marker
...
class TestRedaction:
    def test_redact_finding_with_security_module(self):
        from codex_crew.apps.builtins.auto_research.handlers import _redact_finding
...
class TestAudit:
    def test_audit_does_not_crash(self):
        from codex_crew.apps.builtins.auto_research.handlers import _audit
...
class TestRequireAuth:
    def test_returns_none_when_user_present(self):
        ...
        from codex_crew.apps.builtins.auto_research.handlers import _require_auth
...
class TestHTTPHandlers:
    ...
    def app(self, tmp_path: Path):
        ...
        from codex_crew.apps.builtins.auto_research.handlers import register_routes
    ...
    async def test_validate(self, app, tmp_path: Path):
        ...
        with (
            patch("codex_crew.apps.builtins.auto_research.handlers.DB_PATH", tmp_path / "t.db"),
            patch("codex_crew.apps.builtins.auto_research.handlers.RESEARCH_DIR", tmp_path / "r"),
        ):
    ...
    async def test_resume_cannot_be_overwritten_by_slow_terminal_sidecar(
        ...
        from codex_crew.apps.builtins.auto_research import handlers as h
        from codex_crew.autonudge import AUTONUDGE_STOP_REASON, AutoNudgeService
    ...
    async def test_stale_settlement_skips_replacement_run_effects(
        ...
        from codex_crew.apps.builtins.auto_research import handlers as h
        from codex_crew.autonudge import AutoNudgeService
    ...
    async def test_add_question(self, app, tmp_path: Path):
        ...
        from codex_crew.apps.builtins.auto_research import handlers as h
        with patch("codex_crew.apps.builtins.auto_research.handlers.DB_PATH", tmp_path / "t.db"), \
             patch("codex_crew.apps.builtins.auto_research.handlers.RESEARCH_DIR", tmp_path / "r"):
            async with TestClient(TestServer(app)) as c:
    ...
    async def test_to_artifact(self, app, tmp_path: Path):
        ...
        from codex_crew.apps.builtins.auto_research import handlers as h
        from codex_crew.artifacts import ArtifactStore
        ...
        with patch("codex_crew.apps.builtins.auto_research.handlers.DB_PATH", tmp_path / "t.db"), \
             patch("codex_crew.apps.builtins.auto_research.handlers.RESEARCH_DIR", tmp_path / "r"), \
             patch("codex_crew.apps.builtins.auto_research.handlers.LLMPool",
                   lambda *a, **k: _RaisingPool()), \
             patch("codex_crew.apps.builtins.auto_research.handlers.ArtifactStore",
...
class TestRedactCampaignFields:
    def test_redacts_sub_questions_and_sources_json(self):
        from codex_crew.apps.builtins.auto_research.handlers import _redact_campaign
...
class TestLoopLaunch:
    ...
    async def test_launch_arms_autonudge(self, monkeypatch):
        ...
        assert state.get_or_create_slot.call_args.kwargs["agent"] == "codexcrew-research"
...
# --- codexcrew-research core agent install ---


class TestResearchAgentInstall:
    def test_installs_codexcrew_research(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            agent,
            "build_agent_config",
            lambda: {"name": "codexcrew", "prompt": "file://x", "mcpServers": {}, "tools": []},
        )
        agent._install_research_agent()
        data = json.loads((tmp_path / "codexcrew-research.json").read_text(encoding="utf-8"))
        assert data["name"] == "codexcrew-research"
    ...
    def _floor_builtins() -> set[str]:
        ...
        """
        from codex_crew.platform.governance import _FLOOR_GATE_SCOPES, BUILTIN_TOOL_SCOPES
    ...
    def _install_real(self, monkeypatch, tmp_path) -> dict:
        ...
        monkeypatch.setattr(agent, "KIRO_AGENTS_DIR", tmp_path)
        agent._install_research_agent()
        return json.loads((tmp_path / "codexcrew-research.json").read_text(encoding="utf-8"))
...
class TestClarificationQuestions:
    def test_pending_question_surfaced(self):
        from codex_crew.apps.builtins.auto_research.handlers import _campaign_dir
...
class TestForkAndGrillTreeHTTP:
    ...
    def test_fork_name_helper(self):
        from codex_crew.apps.builtins.auto_research.handlers import _fork_name
...
class TestGrillParse:
    def test_parses_clarifier_and_research(self):
        from codex_crew.apps.builtins.auto_research.handlers import _parse_grill_nodes
    ...
    def test_node_depth(self):
        from codex_crew.apps.builtins.auto_research.handlers import _node_depth
...
class TestGrillSuggestedCycles:
    def test_suggested_max_cycles(self):
        from codex_crew.apps.builtins.auto_research.handlers import validate_campaign
...
class TestGrillHTTP:
    ...
    def app(self, tmp_path: Path):
        ...
        with patch(
            "codex_crew.apps.builtins.auto_research.handlers.DB_PATH", tmp_path / "t.db"
        ), patch(
            "codex_crew.apps.builtins.auto_research.handlers.RESEARCH_DIR", tmp_path / "r"
        ):
            a = web.Application(middlewares=[_inject_user])
            register_routes(a)
            yield a
    ...
    async def test_expand_requires_auth(self):
        from codex_crew.apps.builtins.auto_research.handlers import _handle_grill_expand
...
class TestPromptTrustBoundary:
    ...
    def test_fence_untrusted_wraps_with_unique_nonce(self):
        from codex_crew.apps.builtins.auto_research.handlers import _fence_untrusted
    ...
    def test_workflow_source_fences_untrusted_and_still_validates(self):
        from codex_crew.apps.builtins.auto_research.workflow_template import (
            RESEARCH_WORKFLOW_SOURCE,
        )
        from codex_crew.workflows.validate import validate
