...
from codex_crew.cron import CronService, cron_job_id_from_session_key, cron_session_key_is_stable
...
class TestOwnerUnusableCaveat:
    ...
    def test_a_subagent_caller_is_told_the_parent_cannot_manage_the_job(self):
        ...
        assert "codexcrew cron adopt abc12345" in out
...
class TestEphemeralAuthorityCaveat:
    ...
    def test_a_command_or_script_job_is_never_warned(self, persistent):
        ...
        A script cron is launched with ``CODEXCREW_SESSION_KEY=cron:<job_id>``
...
class TestCronAddSurfacesTheCaveat:
    ...
    def test_a_subagent_still_gets_its_job_and_also_gets_told(self, monkeypatch):
        monkeypatch.setenv("CODEXCREW_SESSION_KEY", "subagent:conv-42")
        ...
        assert f"codexcrew cron adopt {jobs[0].id}" in result

    def test_an_ephemeral_agent_job_is_flagged_at_creation(self, monkeypatch):
        monkeypatch.setenv("CODEXCREW_SESSION_KEY", "dashboard:chat-9")
        monkeypatch.delenv("CODEXCREW_CHANNEL_ID", raising=False)
        name = _unique_name()
...
class TestRefusalsNameTheWayBack:
    ...
    def test_scoped_empty_denies_being_an_empty_registry(self):
        ...
        assert "codexcrew cron adopt" in _SCOPED_EMPTY
