"""Coverage tests for :mod:`codex_crew.cli_commands` subcommand dispatch.
...
by patching ``codex_crew.cli_commands.loopback_urlopen``, and every filesystem write lands in
``tmp_path``.

Follows the style already established by ``test_cli.py`` (``argparse.Namespace``
+ ``patch("codex_crew.cli_commands.<name>")``) and ``test_workspace_crud_cli.py``
...
from codex_crew import cli_commands as cc
from codex_crew import sel as sel_mod
from codex_crew.config.loader import CodexCrewAgentConfig, CodexCrewConfig, WorkspaceConfig
from codex_crew.cron import CronSchedule
from codex_crew.eval.scenario import AssertionType
from codex_crew.security import BUILTIN_DENIED_RULES
from codex_crew.vector_memory import LessonWriteOutcome, LessonWriteResult
...
def _cfg_with(
    *,
    agents: dict[str, CodexCrewAgentConfig] | None = None,
...
class TestSmallHelpers:
    def test_internal_secret_reads_file(self, tmp_path: Path) -> None:
        ...
        with patch("codex_crew.cli_commands.read_local_secret", return_value="s3cr3t") as read:
    ...
    def test_internal_secret_missing_file_is_empty(self, tmp_path: Path) -> None:
        ...
        with patch("codex_crew.cli_commands.read_local_secret", return_value=""):
    ...
    def test_format_schedule_delegates_for_every(self) -> None:
        ...
        with patch("codex_crew.cli_commands.format_schedule", return_value="every 5m") as fmt:
...
class TestWorkspaceDirGuard:
    ...
    def test_relative_name_inside_home_is_accepted(self, tmp_path: Path) -> None:
        with patch("codex_crew.cli_commands.config_dir", return_value=tmp_path):
    ...
    def test_sensitive_target_is_refused(self, tmp_path: Path) -> None:
        with (
            patch("codex_crew.cli_commands.config_dir", return_value=tmp_path),
            patch("codex_crew.cli_commands.is_sensitive_path", return_value=True),
        ):
...
class TestSpawnCli:
    def test_list_prints_agents_with_status_glyphs(
        ...
        with (
            patch("codex_crew.cli_commands._internal_secret", return_value="s"),
            patch("codex_crew.cli_commands.loopback_urlopen", return_value=_FakeResponse(payload)),
        ):
    ...
    def test_list_empty_says_so(self, capsys: pytest.CaptureFixture[str]) -> None:
        with (
            patch("codex_crew.cli_commands._internal_secret", return_value=""),
            patch(
    ...
    def test_list_http_error_with_json_body_prints_server_error(
        ...
        with (
            patch("codex_crew.cli_commands._internal_secret", return_value=""),
            patch("codex_crew.cli_commands.loopback_urlopen", side_effect=err),
    ...
    def test_list_http_error_with_opaque_body_prints_status(
        ...
        with (
            patch("codex_crew.cli_commands._internal_secret", return_value=""),
            patch(
                "codex_crew.cli_commands.loopback_urlopen", side_effect=_http_error(503, b"<html>")
            ),
    ...
    def test_unknown_action_prints_usage(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        assert "Usage: codexcrew spawn" in capsys.readouterr().out
    ...
    def test_run_blocking_polls_until_done_then_prints_result(
        ...
        with (
            ...
            patch("codex_crew.cli_commands.loopback_urlopen", side_effect=responses),
        ):
    ...
    def test_run_unreachable_gateway_exits_1(self, capsys: pytest.CaptureFixture[str]) -> None:
        with (
            patch("codex_crew.cli_commands._internal_secret", return_value=""),
            patch("codex_crew.cli_commands.loopback_urlopen", side_effect=OSError("no route")),
            pytest.raises(SystemExit),
        ):
...
class TestAppCli:
    def test_install_success_reports_registrations(
        ...
        with (
            patch("codex_crew.cli_commands.install_app", return_value=_result(True, name="demo")),
            patch(
                "codex_crew.cli_commands.register_app",
        ...
        assert "partial" in out and "codexcrew app enable demo" in out

    def test_install_failure_exits_1(self, capsys: pytest.CaptureFixture[str]) -> None:
        with (
            patch(
                "codex_crew.cli_commands.install_app",
    ...
    def test_list_empty(self, capsys: pytest.CaptureFixture[str]) -> None:
        with patch("codex_crew.cli_commands.list_apps", return_value=[]):
    ...
    def test_list_renders_enabled_and_disabled_rows(
        ...
        with patch("codex_crew.cli_commands.list_apps", return_value=apps):
            cc._handle_app(_ns(app_action="list"))
        out = capsys.readouterr().out
    ...
    def test_enable_success_counts_registrations(self, capsys: pytest.CaptureFixture[str]) -> None:
        with (
            patch("codex_crew.cli_commands.enable_app", return_value=_result(True, message="on")),
            patch(
                "codex_crew.cli_commands.register_app",
    ...
    def test_enable_failure_exits_1(self) -> None:
        with (
            patch("codex_crew.cli_commands.enable_app", return_value=_result(False)),
            pytest.raises(SystemExit) as exc,
        ):
            cc._handle_app(_ns(app_action="enable", name="demo"))
        assert exc.value.code == 1
    ...
    def test_disable_cleans_crons_then_deregisters(
        ...
        with (
            patch("codex_crew.cli_commands._cleanup_app_crons_from_scheduler") as cleanup,
            patch("codex_crew.cli_commands.deregister_app") as dereg,
            patch("codex_crew.cli_commands.disable_app", return_value=_result(True, message="off")),
        ):
    ...
    def test_disable_flips_the_flag_before_deregistering(self) -> None:
        ...
        order: list[str] = []
        with (
            patch("codex_crew.cli_commands._cleanup_app_crons_from_scheduler"),
            patch(
                "codex_crew.cli_commands.deregister_app",
                side_effect=lambda n: order.append("deregister"),
            ),
            patch(
                "codex_crew.cli_commands.disable_app",
    ...
    def test_disable_failure_exits_1(self) -> None:
        with (
            patch("codex_crew.cli_commands._cleanup_app_crons_from_scheduler"),
            patch("codex_crew.cli_commands.deregister_app"),
            patch("codex_crew.cli_commands.disable_app", return_value=_result(False)),
            pytest.raises(SystemExit),
        ):
            cc._handle_app(_ns(app_action="disable", name="demo"))
    ...
    def test_uninstall_maps_purge_flag_to_keep_data(
        ...
        with (
            patch("codex_crew.cli_commands._cleanup_app_crons_from_scheduler"),
            patch("codex_crew.cli_commands.deregister_app"),
            patch("codex_crew.cli_commands.uninstall_app", return_value=_result(True)) as uninstall,
        ):
    ...
    def test_uninstall_failure_exits_1(self) -> None:
        with (
            patch("codex_crew.cli_commands._cleanup_app_crons_from_scheduler"),
            patch("codex_crew.cli_commands.deregister_app"),
            patch("codex_crew.cli_commands.uninstall_app", return_value=_result(False)),
            pytest.raises(SystemExit),
        ):
    ...
    def test_dev_on_prints_live_reload_hint(self, capsys: pytest.CaptureFixture[str]) -> None:
        with patch("codex_crew.apps.dev_mode.set_dev_mode", return_value={"ok": True}):
    ...
    def test_dev_off_restores_caching(self, capsys: pytest.CaptureFixture[str]) -> None:
        with patch("codex_crew.apps.dev_mode.set_dev_mode", return_value={"ok": True}) as setter:
    ...
    def test_dev_confirmation_refusal_reaches_stderr(
        ...
        refusal = {
            "error": "needs confirmation: run `codexcrew app dev demo --confirm-out-of-install-root`",
            "code": "dev_mode_out_of_install_confirmation_required",
        }
        with (
            patch("codex_crew.apps.dev_mode.set_dev_mode", return_value=refusal) as setter,
            pytest.raises(SystemExit) as exc,
        ):
            cc._handle_app(
    ...
    def test_dev_error_exits_1(self, capsys: pytest.CaptureFixture[str]) -> None:
        with (
            patch("codex_crew.apps.dev_mode.set_dev_mode", return_value={"error": "no such app"}),
            pytest.raises(SystemExit) as exc,
        ):
            cc._handle_app(_ns(app_action="dev", name="demo", off=False))
        assert exc.value.code == 1
    ...
    def test_info_prints_json(self, capsys: pytest.CaptureFixture[str]) -> None:
        with patch("codex_crew.cli_commands.get_app", return_value={"name": "demo", "v": 1}):
    ...
    def test_info_missing_exits_1(self, capsys: pytest.CaptureFixture[str]) -> None:
        with (
            patch("codex_crew.cli_commands.get_app", return_value=None),
            pytest.raises(SystemExit) as exc,
        ):
    ...
    def test_init_scaffolds_and_prints_next_steps(
        ...
        with patch("codex_crew.cli_commands.scaffold_app", return_value=target) as scaffold:
            cc._handle_app(
                _ns(
                    app_action="init",
                    name="my-app",
    ...
    def test_init_without_ui_skips_npm_hint(
        ...
        with patch("codex_crew.cli_commands.scaffold_app", return_value=tmp_path / "a"):
            cc._handle_app(
    ...
    def test_mcp_action_delegates_to_server_runner(self) -> None:
        with patch("codex_crew.cli_commands._run_app_mcp_server") as runner:
            cc._handle_app(_ns(app_action="mcp", name="demo"))
        runner.assert_called_once_with("demo")

    def test_unknown_action_prints_usage(self, capsys: pytest.CaptureFixture[str]) -> None:
        cc._handle_app(_ns(app_action=None))
        assert "Usage: codexcrew app" in capsys.readouterr().out
...
class TestRunAppMcpServer:
    def test_missing_module_exits_1_on_stderr(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        target = "codex_crew.apps.builtins.my_app.mcp_server"
        with (
            patch(
    ...
    def test_missing_parent_package_exits_1(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        parent = "codex_crew.apps.builtins.my_app"
        with (
            patch(
                "importlib.import_module",
...
class TestCleanupAppCrons:
    ...
    def test_reports_removed_count_and_audits(
        ...
        with (
            patch("codex_crew.cli_commands.config_dir", return_value=tmp_path),
            patch("codex_crew.cli_commands.CronService"),
            patch("codex_crew.cli_commands.deregister_app_crons_from_service", new=_two),
            patch("codex_crew.cli_commands.sel") as sel,
        ):
    ...
    def test_zero_removed_prints_nothing(
        ...
        with (
            patch("codex_crew.cli_commands.config_dir", return_value=tmp_path),
            patch("codex_crew.cli_commands.CronService"),
            patch("codex_crew.cli_commands.deregister_app_crons_from_service", new=self._zero),
    ...
    def test_failure_is_audited_then_reraised(self, tmp_path: Path) -> None:
        ...
        with (
            patch("codex_crew.cli_commands.config_dir", return_value=tmp_path),
            patch("codex_crew.cli_commands.CronService"),
            patch("codex_crew.cli_commands.deregister_app_crons_from_service", new=_boom),
            patch("codex_crew.cli_commands.sel") as sel,
...
class TestAgentCli:
    def test_list_marks_default(self, capsys: pytest.CaptureFixture[str]) -> None:
        cfg = _cfg_with(
            agents={
                "default": CodexCrewAgentConfig(kiro_agent="codexcrew"),
                "other": CodexCrewAgentConfig(kiro_agent="alt"),
            }
        )
        with patch.object(CodexCrewConfig, "load", return_value=cfg):
    ...
    def test_create_duplicate_exits_1_without_saving(
        ...
        cfg = _cfg_with(agents={"dup": CodexCrewAgentConfig()})
        with (
            patch.object(CodexCrewConfig, "load", return_value=cfg),
            pytest.raises(SystemExit) as exc,
        ):
            cc._handle_agent(
                _ns(
    ...
    def test_update_applies_only_provided_fields(self) -> None:
        cfg = _cfg_with(
            agents={"a": CodexCrewAgentConfig(kiro_agent="old", workspace="ws0", memory_store="m0")}
        )
        with patch.object(CodexCrewConfig, "load", return_value=cfg):
            cc._handle_agent(
                _ns(
    ...
    def test_update_all_fields(self) -> None:
        cfg = _cfg_with(agents={"a": CodexCrewAgentConfig()})
        with patch.object(CodexCrewConfig, "load", return_value=cfg):
            cc._handle_agent(
                _ns(
                    agent_action="update",
    ...
    def test_delete_removes_non_default(self, capsys: pytest.CaptureFixture[str]) -> None:
        cfg = _cfg_with(
            agents={"default": CodexCrewAgentConfig(), "spare": CodexCrewAgentConfig()},
        )
        with patch.object(CodexCrewConfig, "load", return_value=cfg):
    ...
    def test_delete_default_is_refused(self, capsys: pytest.CaptureFixture[str]) -> None:
        cfg = _cfg_with(agents={"default": CodexCrewAgentConfig()})
        with (
            patch.object(CodexCrewConfig, "load", return_value=cfg),
            pytest.raises(SystemExit) as exc,
        ):
    ...
    def test_unknown_action_prints_usage(self, capsys: pytest.CaptureFixture[str]) -> None:
        with patch.object(CodexCrewConfig, "load", return_value=_cfg_with()):
            cc._handle_agent(_ns(agent_action=None))
        assert "Usage: codexcrew agent" in capsys.readouterr().out
...
class TestWorkspaceCopyFrom:
    def _base(self) -> CodexCrewConfig:
        return _cfg_with(
            workspaces={
    ...
    def test_copy_from_unknown_source_exits_1(self, tmp_path: Path) -> None:
        with (
            patch("codex_crew.cli_commands.config_dir", return_value=tmp_path),
            patch.object(CodexCrewConfig, "load", return_value=self._base()),
            patch("codex_crew.cli_commands.sel"),
            pytest.raises(SystemExit) as exc,
        ):
    ...
    def test_copy_from_skips_sensitive_entries(self, tmp_path: Path) -> None:
        ...
        with (
            patch("codex_crew.cli_commands.config_dir", return_value=tmp_path),
            patch.object(CodexCrewConfig, "load", return_value=self._base()),
            patch("codex_crew.cli_commands.is_sensitive_path", side_effect=_sensitive),
            patch("codex_crew.cli_commands.sel"),
        ):
            cc._handle_workspace(
...
class TestSecurityCli:
    ...
    def test_audit_clean_reports_history_only(
        ...
        with (
            patch("codex_crew.cli_commands.config_dir", return_value=tmp_path),
            patch("codex_crew.cli_commands.scan_history", return_value=[]),
            patch("codex_crew.cli_commands.scan_memory", return_value=[]),
        ):
    ...
    def test_audit_reports_history_and_memory_findings(
        ...
        with (
            patch("codex_crew.cli_commands.config_dir", return_value=tmp_path),
            patch("codex_crew.cli_commands.scan_history", return_value=history),
            patch("codex_crew.cli_commands.scan_memory", return_value=memory),
        ):
            cc._security(_ns(sec_action="audit"))
        out = capsys.readouterr().out
    ...
    def test_events_empty(self, capsys: pytest.CaptureFixture[str]) -> None:
        with patch("codex_crew.cli_commands.sel") as sel:
    ...
    def test_unknown_action_prints_usage(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        assert "Usage: codexcrew security" in capsys.readouterr().out
...
class TestPolicyCli:
    def test_show_without_policy(self, capsys: pytest.CaptureFixture[str]) -> None:
        with patch(
            "codex_crew.platform.context.current_context",
    ...
    def test_validate_without_policy_or_profiles_dir(
        ...
        with (
            ...
            patch(
                "codex_crew.platform.governance_profiles._profiles_dir",
    ...
    def test_validate_flags_invalid_profile(
        ...
        with (
            ...
            patch("codex_crew.platform.governance_profiles._profiles_dir", return_value=pdir),
            patch("codex_crew.platform.governance_profiles.get_store_profile", side_effect=_get),
        ):
            cc._policy(_ns(policy_action="validate"))
        out = capsys.readouterr().out
    ...
    def test_explain_unknown_scope_lists_catalog(self, capsys: pytest.CaptureFixture[str]) -> None:
        with (
            ...
            patch(
                "codex_crew.platform.governance.SCOPE_CATALOG",
    ...
    def test_explain_known_scope_prints_verdicts(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        with (
            ...
            patch(
                "codex_crew.platform.governance_profiles.resolve_active_scope",
                return_value=SimpleNamespace(name="prof"),
            ),
            patch("codex_crew.platform.governance.resolve", return_value=decision),
            patch("codex_crew.platform.governance.gate_decision", return_value=gate),
        ):
            cc._policy(
                _ns(
                    policy_action="explain",
    ...
    def test_explain_without_active_profile(self, capsys: pytest.CaptureFixture[str]) -> None:
        with (
            ...
            patch("codex_crew.platform.governance.SCOPE_CATALOG", {"s": object()}),
            patch(
                "codex_crew.platform.governance_profiles.resolve_active_scope",
                return_value=None,
            ),
            patch(
                "codex_crew.platform.governance.resolve",
                return_value=SimpleNamespace(
                    permitted=True, rule="allow", layer=None, reason="default"
                ),
            ),
            patch(
                "codex_crew.platform.governance.gate_decision",
    ...
    def test_profile_missing(self, capsys: pytest.CaptureFixture[str]) -> None:
        with (
            ...
            patch("codex_crew.platform.governance_profiles.get_store_profile", return_value=None),
        ):
    ...
    def test_profile_prints_bind_and_scopes(self, capsys: pytest.CaptureFixture[str]) -> None:
        prof = SimpleNamespace(
            name="team",
            bind=SimpleNamespace(type="agent", id="codexcrew"),
            extends="base",
            controls={"capabilities.telemetry": "off"},
        )
        with (
            ...
            patch("codex_crew.platform.governance_profiles.get_store_profile", return_value=prof),
        ):
            cc._policy(_ns(policy_action="profile", name="team"))
        out = capsys.readouterr().out
        assert "bind=agent:codexcrew" in out and "extends=base" in out
        assert "capabilities.telemetry: off" in out
    ...
    def test_unknown_action_prints_usage(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        assert "Usage: codexcrew policy" in capsys.readouterr().out
...
class _LearnHarness:
    ...
    def __init__(self) -> None:
        ...
        self._patches = [
            patch("codex_crew.cli_commands.VectorMemoryStore", return_value=self.vs),
            patch("codex_crew.cli_commands.LessonStore", return_value=self.jsonl),
            patch.object(CodexCrewConfig, "load", return_value=CodexCrewConfig()),
        ]
...
class TestLearnCli:
    ...
    def test_unknown_action_prints_usage_and_closes_store(
        ...
        h.vs.close.assert_called_once()
        assert "Usage: codexcrew learn" in capsys.readouterr().out
...
class _MemHarness:
    def __init__(self) -> None:
        ...
        self._patches = [
            patch("codex_crew.cli_commands.VectorMemoryStore", return_value=self.store),
            patch.object(CodexCrewConfig, "load", return_value=CodexCrewConfig()),
        ]
...
class TestMemoryCli:
    ...
    def test_audit_with_and_without_findings(self, capsys: pytest.CaptureFixture[str]) -> None:
        with _MemHarness(), patch("codex_crew.cli_commands.scan_memory", return_value=[]):
        ...
        with _MemHarness(), patch("codex_crew.cli_commands.scan_memory", return_value=findings):
            cc._memory_cmd(_ns(mem_action="audit"))
    ...
    def test_import_requires_a_file_argument(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        assert "Usage: codexcrew memory import" in capsys.readouterr().out
    ...
    def test_unknown_action_prints_usage_and_closes(
        ...
        assert "Usage: codexcrew memory" in capsys.readouterr().out
...
class _ArtifactHarness:
    ...
    def __enter__(self) -> _ArtifactHarness:
        self._patches = [
            patch.object(CodexCrewConfig, "load", return_value=CodexCrewConfig()),
            patch("codex_crew.cli_commands.parse_dashboard_url", return_value=("localhost", 5486)),
            patch("codex_crew.cli_commands._internal_secret", return_value="s"),
            patch("codex_crew.cli_commands.loopback_urlopen", side_effect=self._responses),
        ]
...
class TestArtifactCli:
    ...
    def test_save_warns_when_the_slug_was_suffixed(
        ...
        assert "codexcrew artifact update run-summary" in captured.err
    ...
    def test_unknown_action_exits_2(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        assert exc.value.code == 2
        assert "Usage: codexcrew artifact" in capsys.readouterr().err


class TestPodDispatch:
    def test_pod_delegates_to_verb_layer(self) -> None:
        args = _ns(pod_action="ls")
        with patch("codex_crew.pod.cli.dispatch") as dispatch:
            cc._pod(args)
...
class TestTelemetryCli:
    def test_status_is_read_only(self, capsys: pytest.CaptureFixture[str]) -> None:
        with (
            patch.object(CodexCrewConfig, "load", return_value=CodexCrewConfig()),
            patch("codex_crew.cli_commands.beacon") as beacon,
        ):
    ...
    def test_disable_writes_config_and_acks_privacy(
        ...
        effective = CodexCrewConfig()
        effective.telemetry.beacon_enabled = False
        with (
            patch.object(CodexCrewConfig, "load", return_value=effective),
            patch("codex_crew.cli_commands.config_path", return_value=path),
            patch("codex_crew.cli_commands.beacon") as beacon,
        ):
            beacon.DISABLE_ENV = "CODEXCREW_NO_BEACON"
    ...
    def test_unreadable_config_exits_1(
        ...
        with (
            patch.object(CodexCrewConfig, "load", return_value=CodexCrewConfig()),
            patch("codex_crew.cli_commands.config_path", return_value=path),
            patch("codex_crew.cli_commands.beacon"),
            pytest.raises(SystemExit) as exc,
        ):
            cc._telemetry(_ns(telemetry_action="disable"))
        assert exc.value.code == 1
    ...
    def test_overlay_shadowing_the_write_is_reported_as_failure(
        ...
        still_on = CodexCrewConfig()
        still_on.telemetry.beacon_enabled = True
        with (
            patch.object(CodexCrewConfig, "load", return_value=still_on),
            patch("codex_crew.cli_commands.config_path", return_value=path),
            patch("codex_crew.cli_commands.beacon") as beacon,
            pytest.raises(SystemExit) as exc,
        ):
    ...
    def test_write_failure_exits_1(
        ...
        with (
            ...
            patch("codex_crew.cli_commands.config_path", return_value=path),
            patch("codex_crew.cli_commands.beacon"),
            patch("codex_crew.config.loader.atomic_write", side_effect=OSError("disk full")),
            pytest.raises(SystemExit) as exc,
        ):
            cc._telemetry(_ns(telemetry_action="disable"))
        assert exc.value.code == 1
    ...
    def test_effective_check_failure_does_not_mask_the_write(
        ...
        with (
            patch.object(
                CodexCrewConfig,
                "load",
                side_effect=[CodexCrewConfig(), RuntimeError("bad config")],
            ),
            patch("codex_crew.cli_commands.config_path", return_value=path),
            patch("codex_crew.cli_commands.beacon") as beacon,
        ):
...
class _EvalHarness:
    ...
    def __enter__(self) -> _EvalHarness:
        self._patches = [
            patch.object(CodexCrewConfig, "load", return_value=CodexCrewConfig()),
            patch("codex_crew.cli_commands.build_provider_factory", return_value=MagicMock()),
            patch("codex_crew.cli_commands.EvalRunner", return_value=self.runner),
            patch("codex_crew.cli_commands.LLMJudge", return_value=self.judge),
            patch("codex_crew.cli_commands.format_results", return_value="## Report"),
            patch("codex_crew.cli_commands.score_by_dimension", return_value={}),
        ]
        for p in self._patches:
            p.start()
        return self
...
class TestRunEval:
    ...
    async def test_default_runs_smoke_test_and_writes_both_reports(
        ...
        with (
            ...
            patch("codex_crew.cli_commands.load_scenario", return_value=_scenario()) as loader,
        ):
    ...
    async def test_all_scenarios_loads_the_whole_directory(
        ...
        with (
            ...
            patch("codex_crew.cli_commands.load_scenarios", return_value=scenarios) as loader,
        ):
    ...
    async def test_named_scenario_is_resolved_by_extension(
        ...
        with (
            _EvalHarness([_scenario_result(True)]),
            patch("codex_crew.cli_commands.load_scenario", return_value=_scenario("named")) as ld,
        ):
            await cc._run_eval(
    ...
    async def test_dimension_summary_marks_pass_and_fail_rates(
        ...
        with (
            _EvalHarness([_scenario_result(True)]),
            patch("codex_crew.cli_commands.load_scenario", return_value=_scenario()),
            patch("codex_crew.cli_commands.score_by_dimension", return_value=dims),
        ):
            await cc._run_eval(_ns(all_scenarios=False, scenarios=None, judge=False))
        out = capsys.readouterr().out
    ...
    async def test_judge_failure_marks_assertion_failed_without_aborting(
        ...
        with (
            _EvalHarness([_scenario_result(True, turn=turn)]) as h,
            patch(
                "codex_crew.cli_commands.load_scenario",
...
class TestDevConfirmFlagNoAbbreviation:
    ...
    def test_abbreviated_confirm_flag_is_rejected(self, capsys) -> None:
        ...
        for abbrev in ("--confirm", "--confirm-out-of-install-roo", "--c"):
            argv = ["codexcrew", "app", "dev", "demo", abbrev]
            with (
                patch.object(sys, "argv", argv),
                patch("codex_crew.cli_commands._handle_app") as handler,
            ):
    ...
    def test_literal_confirm_flag_still_parses(self) -> None:
        ...
        argv = ["codexcrew", "app", "dev", "demo", "--confirm-out-of-install-root"]
        with (
            patch.object(sys, "argv", argv),
            patch("codex_crew.cli_commands._handle_app") as handler,
        ):
