...
from codex_crew.dashboard.handlers.files import api_reveal_path
...
async def test_reveal_path_no_crash(mock_sel, tmp_path):
    ...
    f.write_text("hi")
    with patch(
        "codex_crew.dashboard.handlers.files.is_direct_local_request", return_value=True
    ), patch("codex_crew.dashboard.handlers.files.platform_compat."
...
async def test_reveal_path_sensitive_denied(mock_sel):
    ...
    with patch(
        "codex_crew.dashboard.handlers.files.is_sensitive_path", return_value=True
    ):
        async with TestClient(TestServer(_make_app())) as client:
            resp = await client.post(
                "/api/reveal",
...
async def test_reveal_remote_request_returns_copy_fallback(mock_sel, tmp_path):
    ...
    f.write_text("hello")
    with patch(
        "codex_crew.dashboard.handlers.files.is_direct_local_request", return_value=False
