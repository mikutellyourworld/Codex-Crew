...
from codex_crew.apps.builtins.file_explorer import server
...
def patch_allowed_roots(tmp_tree):
    ...
    def mock_is_sensitive(path_str):
        """Check sensitive dirs by path component (excluding .codexcrew which is handled
        granularly)."""
...
class TestIsSensitive:
    ...
    def test_codexcrew_is_sensitive(self, tmp_tree):
        assert server._is_sensitive(tmp_tree / ".codexcrew" / ".env") is True
    ...
    def test_crew_home_nonsafe_file_is_sensitive(self, tmp_tree):
        ...
        assert server._is_sensitive(tmp_tree / ".codexcrew" / "config.json") is True
    ...
    def test_crew_home_marker_match_is_case_insensitive(self, tmp_tree):
        ...
        # ~/.KIRO/crew/config.json opens the same inode as ~/.codex-crew but
        ...
        for variant in (
            ...
            tmp_tree / ".CODEXCREW" / "config.json",
        ):
...
class TestKirocrewGranularSensitive:
    """Regression tests for .codexcrew granular sensitive path policy.

    .codexcrew/workspace/, uploads/, skills/, artifacts/ etc. should be accessible.
    .codexcrew/config.json, sessions/, *.key should be blocked.
    """

    @pytest.fixture(autouse=True)
    def codexcrew_tree(self, tmp_tree):
        """Create a .codexcrew directory structure for testing."""
        mc = tmp_tree / ".codexcrew"
        mc.mkdir()
        # Safe subdirs
        (mc / "workspace").mkdir()
    ...
    def test_workspace_notes_accessible(self, tmp_tree):
        """User notes under .codexcrew/workspace/ should be readable."""
        p = server._safe_path(str(tmp_tree / ".codexcrew" / "workspace" / "notes" / "update.md"))
        assert p.exists()

    def test_uploads_accessible(self, tmp_tree):
        """Uploaded images should be accessible."""
        p = server._safe_path(str(tmp_tree / ".codexcrew" / "uploads" / "image.png"))
        assert p.exists()

    def test_skills_accessible(self, tmp_tree):
        """Skills directory should be accessible."""
        p = server._safe_path(str(tmp_tree / ".codexcrew" / "skills" / "my-skill" / "SKILL.md"))
        assert p.exists()

    def test_artifacts_accessible(self, tmp_tree):
        """Artifacts should be accessible."""
        p = server._safe_path(str(tmp_tree / ".codexcrew" / "artifacts" / "data.json"))
        assert p.exists()

    def test_config_json_blocked(self, tmp_tree):
        """config.json contains tokens — must be blocked."""
        with pytest.raises(server.PathError) as exc_info:
            server._safe_path(str(tmp_tree / ".codexcrew" / "config.json"))
        assert exc_info.value.status == 403

    def test_key_files_blocked(self, tmp_tree):
        """Signing keys must be blocked."""
        with pytest.raises(server.PathError) as exc_info:
            server._safe_path(str(tmp_tree / ".codexcrew" / "sel_hmac.key"))
        assert exc_info.value.status == 403

    def test_sessions_blocked(self, tmp_tree):
        """Sessions directory (auth tokens) must be blocked."""
        with pytest.raises(server.PathError) as exc_info:
            server._safe_path(str(tmp_tree / ".codexcrew" / "sessions" / "sess-001.json"))
        assert exc_info.value.status == 403

    def test_memory_db_blocked(self, tmp_tree):
        """memory.db contains full conversation transcripts — must be blocked."""
        with pytest.raises(server.PathError) as exc_info:
            server._safe_path(str(tmp_tree / ".codexcrew" / "memory.db"))
        assert exc_info.value.status == 403

    def test_codexcrew_root_listing_blocked_by_safe_path(self, tmp_tree):
        """Listing .codexcrew/ root is blocked at _safe_path level (deny-by-default).
        Tree/complete handlers use _codexcrew_safe_children() instead."""
        with pytest.raises(server.PathError) as exc_info:
            server._safe_path(str(tmp_tree / ".codexcrew"))
        assert exc_info.value.status == 403

    def test_codexcrew_safe_children_returns_only_safe_subdirs(self, tmp_tree):
        """_codexcrew_safe_children() exposes only allowlisted dirs."""
        mc = tmp_tree / ".codexcrew"
        entries = server._codexcrew_safe_children(mc)
        names = {e["name"] for e in entries}
        # Only dirs in _CODEXCREW_SAFE_SUBDIRS should appear
    ...
    # ~/.codexcrew/security_policy.json, profiles/, admission_policy.json are
    ...
    def test_profiles_not_in_safe_children_or_listing(self, tmp_tree):
        ...
        mc = tmp_tree / ".codexcrew"
        assert "profiles" not in server._CODEXCREW_SAFE_SUBDIRS
        child_names = {e["name"] for e in server._codexcrew_safe_children(mc)}
...
class TestAutoSdeRound1Findings:
    ...
    #15 security-controls: listing .codexcrew/ root must not leak sensitive
        entry NAMES (config.json, *.key, memory.db, sessions/).
    #17 rg allowlist side-effect: searching a root OUTSIDE .codexcrew must not
        restrict results to .codexcrew safe subdirs (non-negated globs
        allowlist-restrict ripgrep); searching INSIDE a safe subdir adds no
        .codexcrew globs at all.
    ...
    def test_root_listing_hides_sensitive_names(self, tmp_tree):
        """#15: /tree of .codexcrew root shows ONLY safe subdirs — no
        config.json / *.key / sessions / memory.db names."""
        entries, _ = server._list_dir(tmp_tree / ".codexcrew", depth=1)
        names = {e["name"] for e in entries}
        assert "workspace" in names
    ...
    def test_python_search_outside_codexcrew_finds_project_files(self, tmp_tree):
        """#17: searching the project root must return matches OUTSIDE
        .codexcrew (the old glob set silently excluded them under rg)."""
        results = server._search_python(tmp_tree, "needle", "", "")
    ...
    def test_python_search_never_surfaces_codexcrew_root_files(self, tmp_tree):
        """#15/defense: .codexcrew root files (config.json) never appear in
        search results even when the walk passes through .codexcrew."""
        results = server._search_python(tmp_tree, "token123", "", "")
        assert results == [], f".codexcrew root file content leaked: {results}"

    def test_rg_glob_set_has_no_nonnegated_codexcrew_globs(self, tmp_tree):
        ...
        ~/.codex-crew and the pre-move legacy ~/.codexcrew — so both variants
        ...
        crew_globs = [g for g in globs if ".codex-crew" in g or ".codexcrew" in g]
        assert crew_globs == [
            "!**/.codex-crew/**",
            "!**/.codexcrew/**",
        ], crew_globs
    ...
    def test_rg_no_codexcrew_globs_when_root_inside_safe_subdir(self, tmp_tree):
        """#17: searching inside .codexcrew/workspace adds no .codexcrew globs
        ...
        with patch.object(server, "wrap_argv", side_effect=fake_wrap):
            try:
                server._search_rg(tmp_tree / ".codexcrew" / "workspace", "needle", "", "")
            except Exception:
                pass
        cmd = captured.get("cmd")
        assert cmd, "wrap_argv never called — _search_rg did not build an rg command"
        globs = [cmd[i + 1] for i, a in enumerate(cmd[:-1]) if a == "--glob"]
        assert not any(".codexcrew" in g for g in globs), globs
    ...
    def test_codexcrew_root_outside_allowed_roots_denied(self, tmp_tree, monkeypatch):
        """Security: a .codexcrew dir outside ALLOWED_ROOTS must be denied even
        ...
        with tempfile.TemporaryDirectory() as attacker_dir:
            fake_mc = Path(attacker_dir) / ".codexcrew"
            fake_mc.mkdir()
            ...
            # _expand + name==".codexcrew" + is_dir() all pass, but
            # ALLOWED_ROOTS gate must still block.
            expanded = server._expand(str(fake_mc))
            assert expanded.name == ".codexcrew"
...
def test_main_boots_platform_before_serving(monkeypatch):
    ...
    monkeypatch.setattr(server.CodexCrewConfig, "load", classmethod(lambda cls: SimpleNamespace()))
