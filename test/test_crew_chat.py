...
import codex_crew.crew_chat as crew_mod
from codex_crew.crew_chat import CrewOrchestrator, CrewStore
...
def _slot(key: str = "s1", agent: str = "codexcrew") -> MagicMock:
    slot = MagicMock()
    slot.key = key
    slot.agent = agent
...
def _slot_save(side_effect: BaseException | None = None):
    ...
    """
    return patch(
        "codex_crew.crew_chat.save_slot_off_loop",
...
class TestIngest:
    ...
    async def test_dispatch_warms_the_agent_cache_before_spawning(self) -> None:
        ...
        with patch("codex_crew.crew_chat.warm_project_agents_for_spawn", new=warm), \
             patch.object(orch, "_post"):
...
class TestGptRoundFive:
    ...
    def test_the_gateway_boot_path_does_not_import_crew(self) -> None:
        ...
        Every module-level `from codex_crew.crew_chat import ...` this branch added
        sat on the gateway's import graph (gateway -> codex_crew.dashboard ->
        ...
        """
        probe = (
            "import sys, codex_crew.slack.gateway as g;"
            "print(g.__file__);"
            "print('codex_crew.crew_chat' in sys.modules)"
        )
    ...
    async def test_dispatch_runs_in_the_slots_own_project(self) -> None:
        ...
        warmed: list[str] = []
        with patch("codex_crew.crew_chat.warm_project_agents_for_spawn",
    ...
    async def test_crew_chat_does_not_import_the_dashboard_handler_tree(self) -> None:
        ...
        prog = (
            "import sys\n"
            "import codex_crew.crew_chat  # noqa: F401\n"
            "bad = [m for m in sys.modules if m.startswith('codex_crew.dashboard.handlers')]\n"
            "print('|'.join(sorted(bad)))\n"
        )
        # Resolve the tree UNDER TEST, not whatever `codex_crew` the interpreter
...
class TestModePlumbing:
    def test_valid_modes_include_crew(self) -> None:
        from codex_crew.dashboard.chat_folders import _VALID_MODES
...
class TestCrewNameResolvesToTemplate:
    ...
    defect only ever showed on the default crew (`default` -> `codexcrew`) — which
    is the crew every session starts on, making crew mode unusable by default.
    """

    def _patches(self, mapping: dict[str, str]):
        return (
            patch.object(crew_mod.CodexCrewConfig, "load", staticmethod(
    ...
    async def test_default_crew_dispatches_as_its_template(self) -> None:
        cfg_patch, bind_patch = self._patches({"default": "codexcrew"})
        subagents = MagicMock()
        subagents.spawn = MagicMock(return_value=_spawn_info("r1"))
        orch = _orch(subagents=subagents)
        ...
        # "default".
        assert subagents.spawn.call_args.kwargs["agent"] == "codexcrew"
    ...
    async def test_empty_agent_resolves_to_the_default_binding(self) -> None:
        ...
        def _resolve(_cfg, agent_name=None, project_dir=None):  # type: ignore[no-untyped-def]
            ...
            return MagicMock(kiro_agent="codexcrew", requested_resolved=True)

        cfg_patch = patch.object(crew_mod.CodexCrewConfig, "load", staticmethod(
            lambda: _crew_config({"default": "codexcrew"})))
    ...
    async def test_resolution_failure_falls_back_to_the_crew_name(self) -> None:
        ...
        st = orch._store("s1")
        e = st.add_msg("build X")
        boom = patch.object(crew_mod.CodexCrewConfig, "load", staticmethod(
...
class TestUnknownCrewNameStaysFailClosed:
    ...
    async def test_unknown_crew_returns_the_raw_name_not_the_default(self) -> None:
        cfg_patch = patch.object(
            crew_mod.CodexCrewConfig, "load",
            staticmethod(lambda: _crew_config({"default": "codexcrew"})))
        bind_patch = patch.object(
            crew_mod, "resolve_agent_bindings", self._unknown_bindings("codexcrew"))
        orch = _orch()
        with cfg_patch, bind_patch:
            resolved = await orch._dispatch_agent(_slot(agent="ghost-crew"))
        # NOT "codexcrew": the default binding must not be dispatched under an
