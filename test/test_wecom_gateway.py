...
from codex_crew.wecom.gateway import (
    _allowed_userids,
...
def _never_open_a_socket(monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.wecom.client.WeComClient.start", fake_start)
    return started
...
class TestStartup:
    ...
    async def test_a_startup_failure_is_swallowed_and_reported_on_the_badge(
        ...
        monkeypatch.setattr("codex_crew.wecom.client.WeComClient.start", boom)
        state = FakeState()
...
class TestSkipReasonWarning:
    ...
    def test_enabled_without_credentials_warns_once_naming_both(self, caplog) -> None:
        with caplog.at_level(logging.WARNING, logger="codex_crew.wecom.gateway"):
    ...
    def test_a_disabled_channel_logs_nothing_at_all(self, caplog) -> None:
        with caplog.at_level(logging.DEBUG, logger="codex_crew.wecom.gateway"):
            warn_if_wecom_uncredentialed(False, "", "")
        assert [r for r in caplog.records if r.name == "codex_crew.wecom.gateway"] == []
