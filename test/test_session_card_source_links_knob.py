...
import codex_crew.dashboard.handlers.source_providers as sp
from codex_crew.config.loader import CodexCrewConfig
from codex_crew.dashboard.chat_handlers import api_chat_slot_source_links
...
class TestSnapshotRead:
    ...
    def test_never_reads_config_on_the_event_loop(self, monkeypatch) -> None:
        ...
        This runs once per slots push, and ``CodexCrewConfig.load()`` stats, reads,
...
class TestCheckRefreshFeed:
    ...
    def test_the_turn_boundary_refresh_spawns_no_provider_call(self, tmp_path, monkeypatch) -> None:
        ...
        with patch(
            "codex_crew.dashboard.handlers.source_providers.request_check_refresh_now"
        ) as refresh:
...
class TestExpandEndpoint:
    ...
    async def test_unknown_slot_404s(self) -> None:
        ...
        with (
            patch(
                "codex_crew.dashboard.handlers.source_providers.ensure_gitlab_hosts_loaded",
                return_value=None,
            ),
...
class TestConfigField:
    def test_default_is_on_so_no_install_loses_its_chips(self) -> None:
        assert CodexCrewConfig().dashboard.session_card_source_links is True
    ...
    def test_save_load_round_trip(self, cfg_file) -> None:
        ...
        assert CodexCrewConfig.load().dashboard.session_card_source_links is False
    ...
    def test_a_hand_edited_non_bool_stays_on(self, cfg_file) -> None:
        ...
        assert CodexCrewConfig.load().dashboard.session_card_source_links is True
...
def mock_sel():
    try:
        import codex_crew.dashboard.handlers  # noqa: F401
    except ImportError:
        pytest.skip("dashboard handler deps not available locally")
    m = MagicMock()
    m.log_tool_invocation = MagicMock()
    with patch("codex_crew.dashboard.handlers.sel", return_value=m):
        yield m
...
def handler_app(cfg_file, mock_sel):
    from codex_crew.dashboard.handlers.files import api_dashboard_config
