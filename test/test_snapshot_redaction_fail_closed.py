...
from codex_crew import snapshot_redact as redact
...
class TestTheUploadPathDoesNotConsultConfigAtAll:
    def test_the_switch_is_read_through_the_redactor_not_the_config(self):
        ...
        assert "CodexCrewConfig" not in src, "a config read came back to the upload path"
        assert "from codex_crew.config.loader import" not in src
        assert not hasattr(
            snap, "CodexCrewConfig"
