...
import codex_crew.discord.transport_dispatch as td_mod
...
from codex_crew.discord.attachments import process_discord_attachments
from codex_crew.discord.client import (
...
from codex_crew.discord.transport import (
    DISCORD_CAPABILITIES,
    DiscordInboundMessage,
    DiscordTransport,
)
from codex_crew.discord.transport_dispatch import (
    _STEER_ACK_EMOJI,
    DiscordDispatcher,
)
from codex_crew.messaging.attachments import cleanup
...
from codex_crew.session import SessionManager, _opt_out_key
...
class TestRotationSplitting:
    ...
    def _renderer(
        ...
        monkeypatch.setattr("codex_crew.discord.renderer._EDIT_THROTTLE_S", 1e9)
        return r, cli
    ...
    async def test_pathological_rotation_work_is_offloaded(
        ...
        from codex_crew.discord import renderer as renderer_module
...
class TestDiscordAttachmentAdapter:
    ...
    async def test_audio_is_returned_for_transcription(
        ...
        monkeypatch.setattr("codex_crew.transcribe.is_available", lambda: True)
        monkeypatch.setattr("codex_crew.transcribe.transcribe_audio", _transcribe)
    ...
    async def test_stt_availability_check_runs_off_loop(
        ...
        monkeypatch.setattr("codex_crew.transcribe.is_available", _available)
        result = await process_discord_attachments(
            client,  # type: ignore[arg-type]
            [
                {
...
class TestPublicInjectionSurface:
    ...
    def test_current_session_key_matches_inbound_derivation(self) -> None:
        d, _cli, _sess = _dispatcher({"42"}, default_agent="codexcrew")
...
class TestTransportReceive:
    ...
    async def test_allowed_user_in_unapproved_thread_is_audited(
        ...
        monkeypatch.setattr(
            "codex_crew.discord.transport.sel",
    ...
    async def test_channels_governance_denial_blocks_thread_creation(
        ...
        monkeypatch.setattr("codex_crew.discord.transport.channel_inbound_permitted", _denied)
        t, dispatched, client = self._transport(["u1"], allowed_channels=["c1"])
        await t.receive(
            DiscordInbound(
                channel_id="c1",
...
class TestRenderer:
    ...
    async def test_prompt_choice_sends_separate_approval_message(self) -> None:
        ...
        from codex_crew.messaging.renderer import new_approval_nonce
...
class TestApprovalDecider:
    ...
    async def test_timeout_records_the_stall_for_a_bound_loop(self, monkeypatch) -> None:
        ...
        from codex_crew.discord import renderer as _rend
...
class TestDispatcher:
    ...
    async def test_monitor_wake_losing_race_at_session_claim_returns_busy(
        ...
        manager = SessionManager(CodexCrewConfig(), provider_factory=_factory)
    ...
    async def test_monitor_wake_pre_turn_refusal_is_unavailable(
        ...
        monkeypatch.setattr(
            "codex_crew.discord.transport_dispatch.channel_inbound_permitted", _denied
        )
        result = await d.handle_message(
            self._msg("[Monitor wake]"),
            interpret_commands=False,
    ...
    async def test_monitor_stop_directive_before_safe_completion_keeps_accounting(
        ...
        monkeypatch.setattr(
            "codex_crew.autonudge_authz.sel",
    ...
    async def test_session_released_even_when_renderer_close_raises(self, monkeypatch) -> None:
        ...
        from codex_crew.discord.renderer import DiscordRenderer
    ...
    async def test_text_and_image_reach_prompt_then_temp_is_cleaned(
        ...
        monkeypatch.setattr(
            "codex_crew.discord.transport_dispatch.cleanup_attachments",
            _cleanup,
        )
        d, cli, _ = _dispatcher({"u1"})
        url = "https://cdn.discordapp.com/attachments/c/m/photo.png"
    ...
    async def test_command_like_caption_does_not_discard_attachment(self) -> None:
        ...
        assert "Codex Crew — Discord" not in "\n".join(text for text, _ in cli.sent)
...
class TestCommandSurfaceParity:
    ...
    async def test_every_catalogued_command_is_answered_over_slash(self, name: str) -> None:
        ...
        with (
            mock.patch("codex_crew.dashboard.token_auth.generate_token", return_value="TKN"),
...
class TestRenderTogglesAreWiredPerTurn:
    ...
    async def test_the_dispatcher_reads_the_toggles_fresh_for_each_turn(self) -> None:
        ...
        with (
            ...
            mock.patch("codex_crew.config.loader.CodexCrewConfig.load") as load,
        ):
    ...
    def test_an_unreadable_config_keeps_the_shipped_defaults(self) -> None:
        ...
        (the quiet one)."""
        d, _cli, _sess = _dispatcher({"u1"})
        with mock.patch("codex_crew.config.loader.CodexCrewConfig.load", side_effect=OSError):
...
class TestReviewFindingRegressions:
    ...
    async def test_an_ambiguous_allowlist_gets_no_owner_dm(self) -> None:
        ...
        from codex_crew.dashboard.handlers.messaging import _owner_dm_target
    ...
    def test_the_install_url_grants_thread_creation(self) -> None:
        ...
        from codex_crew.discord.install_url import (
