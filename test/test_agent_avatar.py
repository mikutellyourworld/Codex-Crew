"""Tests for the per-crew avatar override on CodexCrewAgentConfig.
...
from codex_crew.config.sections import _safe_avatar


def _load_from_dict(data: dict) -> CodexCrewConfig:
...
class TestCodexCrewAgentConfigAvatar:
    """avatar field on CodexCrewAgentConfig."""

    def test_default_empty(self):
        assert CodexCrewAgentConfig().avatar == {}

    def test_default_is_not_shared_between_instances(self):
        a, b = CodexCrewAgentConfig(), CodexCrewAgentConfig()
        a.avatar["kind"] = "ghost"
        assert b.avatar == {}

    def test_serializes_in_asdict(self):
        d = dataclasses.asdict(CodexCrewAgentConfig(avatar=_GHOST))
        assert d["avatar"] == _GHOST

    def test_empty_serializes(self):
        d = dataclasses.asdict(CodexCrewAgentConfig())
        assert d["avatar"] == {}
...
class TestAvatarLoadRoundTrip:
    ...
    def test_round_trips_through_to_dict(self):
        cfg = CodexCrewConfig()
        cfg.agents["radar"] = CodexCrewAgentConfig(avatar=_GHOST)
        assert cfg.to_dict()["agents"]["radar"]["avatar"] == _GHOST

    def test_loads_from_agents_section(self):
        cfg = _load_from_dict({"agents": {"radar": {"kiro_agent": "codexcrew", "avatar": _GHOST}}})
        assert cfg.agents["radar"].avatar == _GHOST

    def test_junk_avatar_collapses_on_load(self):
        cfg = _load_from_dict({"agents": {"radar": {"kiro_agent": "codexcrew", "avatar": "ghost"}}})
...
class TestAvatarEndpoints:
    ...
    def seeded_agent(self):
        ...
        cfg.agents["existing"] = CodexCrewAgentConfig(kiro_agent="codexcrew")
        cfg.save()
        return "existing"
    ...
    async def test_update_persists_a_valid_override(self, seeded_agent):
        ...
        assert CodexCrewConfig.load().agents[seeded_agent].avatar == _GHOST
    ...
    async def test_update_refuses_junk_with_a_code(self, seeded_agent):
        ...
        assert CodexCrewConfig.load().agents[seeded_agent].avatar == {}
    ...
    async def test_create_accepts_an_override(self):
        ...
        async with TestClient(TestServer(self._app())) as client:
            resp = await client.post(
                "/api/agents",
                json={"name": "radar2", "kiro_agent": "codexcrew", "avatar": _GHOST},
            )
            assert resp.status == 200
        assert CodexCrewConfig.load().agents["radar2"].avatar == _GHOST
    ...
    async def test_create_refuses_junk_with_a_code(self):
        ...
        async with TestClient(TestServer(self._app())) as client:
            resp = await client.post(
                "/api/agents",
                json={"name": "radar3", "kiro_agent": "codexcrew", "avatar": ["x"]},
            )
            assert resp.status == 400
            assert (await resp.json())["code"] == "invalid_avatar"
        assert "radar3" not in CodexCrewConfig.load().agents
...
class TestImageBodyComplete:
    ...
    def test_complete_fixtures_pass(self):
        from codex_crew.dashboard.handlers.agents import _image_body_complete
...
class TestUploadedAvatarEndpoints:
    ...
    async def test_upload_rejects_truncated_image(self, seeded_agent):
        ...
        from codex_crew.dashboard.handlers.agents import _pending_avatar_path
    ...
    def _app():
        ...
        app = web.Application()
        app.router.add_get("/api/agents/{name}/avatar", api_codexcrew_agent_avatar_get)
        app.router.add_post("/api/agents/{name}/avatar", api_codexcrew_agent_avatar_upload)
        app.router.add_put("/api/agents/{name}", api_codexcrew_agent_update)
    ...
    def seeded_agent(self):
        ...
        cfg.agents["other"] = CodexCrewAgentConfig(kiro_agent="codexcrew")
    ...
    def _stored(name: str):
        ...
        """
        from codex_crew.dashboard.handlers.agents import _avatar_variant_paths
    ...
    async def test_upload_then_commit_then_get_roundtrip(self, seeded_agent):
        ...
        async with TestClient(TestServer(self._app())) as client:
            ...
            stored = CodexCrewConfig.load().agents[seeded_agent].avatar
    ...
    async def test_upload_caps_size(self, seeded_agent, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.handlers.agents._AVATAR_MAX_BYTES", 128)
        async with TestClient(TestServer(self._app())) as client:
            resp = await client.post(
    ...
    async def test_get_requires_config_to_select_the_image(self, seeded_agent):
        ...
        from codex_crew.dashboard.handlers.agents import _avatar_stem, _avatars_dir
    ...
    async def test_mistyped_ghost_traits_never_delete_the_saved_picture(self, seeded_agent):
        ...
        async with TestClient(TestServer(self._app())) as client:
            ...
            assert got.status == 200
            assert await got.read() == _PNG
        assert CodexCrewConfig.load().agents[seeded_agent].avatar.get("kind") == "image"
    ...
    async def test_install_never_touches_the_committed_file(self, seeded_agent, monkeypatch):
        ...
        import codex_crew.dashboard.handlers.agents as handlers_agents
    ...
    async def test_failed_save_rolls_back_without_touching_the_committed_file(
        ...
        async with TestClient(TestServer(self._app())) as client:
            ...
            real_save = handlers_agents.CodexCrewConfig.save
            monkeypatch.setattr(
                handlers_agents.CodexCrewConfig,
                "save",
            ...
            assert unlinked == []
            monkeypatch.setattr(handlers_agents.CodexCrewConfig, "save", real_save)
            got = await client.get(f"/api/agents/{seeded_agent}/avatar")
            assert got.status == 200
            assert await got.read() == _PNG
