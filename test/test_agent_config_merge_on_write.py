...
from codex_crew.dashboard.handlers import api_agent_config
...
async def _put(
    ...
    *,
    ...
    if apps is None:
        apps = {"demo": ("notes",)}
    installed = tmp_path / "codexcrew.json"
    ...
    def _census(project_dirs=()):
        if unreadable_scope:
            ...
            return ({"codexcrew": {}}, ("codexcrew",))
        return ({"codexcrew": {name: {"url": "scope"} for name in scope_names}}, ())
    ...
    patches = [
        patch("codex_crew.dashboard.handlers._installed_agent_config", return_value=installed),
        patch("codex_crew.dashboard.handlers._find_agent_config", return_value=defaults),
        patch("codex_crew.dashboard.handlers._reset_all_sessions", new_callable=AsyncMock),
        patch("codex_crew.dashboard.handlers.config_path", return_value=mc_cfg),
        patch(
            "codex_crew.dashboard.handlers.agents.get_shipped_tools",
        ...
        patch("codex_crew.dashboard.handlers.agents._agent_file_lock", file_lock),
        ...
        patch(
            "codex_crew.dashboard.handlers.agents.apps_dir",
            return_value=_handler_apps_root,
        ),
        patch("codex_crew.apps.manager.apps_dir", return_value=_apps_root),
        patch("codex_crew.dashboard.handlers.agents._registration_source", _reg_source),
        patch("codex_crew.agent._MANAGED_MCP_SERVERS", _managed_map),
        patch("codex_crew.agent._extra_mcp_servers", return_value=dict(extra_managed or {})),
        patch("codex_crew.dashboard.handlers.mcp._get_mcp_lock", lock),
        patch("codex_crew.connections.ownership.spec_census", _census),
    ]
    if not govern:
        patches.append(
            patch(
                "codex_crew.platform.governance.sanitize_agent_config_governance",
                lambda config: None,
            )
        )
...
async def test_app_bridge_registered_during_the_lock_wait_survives_the_put(tmp_path):
    ...
    response, written = await _put(
        ...
        on_disk={"name": "codexcrew", "mcpServers": {"weather": {"url": "w"}}},
        submitted={"name": "codexcrew", "mcpServers": {"weather": {"url": "w"}}},
...
async def test_direct_client_entry_deletes_on_a_sequential_add_then_remove(tmp_path):
    ...
    response, written = await _put(
        tmp_path,
        on_disk={"name": "codexcrew", "mcpServers": {"myserver": {"command": "mine"}}},
        submitted={"name": "codexcrew", "mcpServers": {}},
...
async def test_app_owned_entry_absent_from_the_snapshot_is_preserved(tmp_path):
    ...
    response, written = await _put(
        tmp_path,
        on_disk={"name": "codexcrew", "mcpServers": {"demo:notes": {"command": "notes-mcp"}}},
        submitted={"name": "codexcrew", "mcpServers": {}},
        scope_names=frozenset(),
    )
...
async def test_a_scope_declared_name_with_no_proven_owner_is_deleted(tmp_path):
    ...
    response, written = await _put(
        tmp_path,
        on_disk={"name": "codexcrew", "mcpServers": {"demo:custom": {"command": "mine"}}},
        submitted={"name": "codexcrew", "mcpServers": {}},
...
async def test_a_namespaced_entry_of_an_uninstalled_app_is_deleted(tmp_path):
    ...
    response, written = await _put(
        tmp_path,
        on_disk={"name": "codexcrew", "mcpServers": {"nosuchapp:thing": {"command": "x"}}},
        submitted={"name": "codexcrew", "mcpServers": {}},
        apps={"demo": ("notes",)},
    )
...
async def test_host_managed_entry_is_preserved(tmp_path):
    ...
    justification: a fresh rebuild re-adds ``codexcrew-core`` unconditionally, so
    ...
    response, written = await _put(
        tmp_path,
        on_disk={"name": "codexcrew", "mcpServers": {"codexcrew-core": {"command": "c"}}},
        submitted={"name": "codexcrew", "mcpServers": {}},
        managed=("codexcrew-core",),
    )

    assert response.status == 200
    assert written["mcpServers"] == {"codexcrew-core": {"command": "c"}}
...
async def test_an_opt_in_managed_server_omitted_from_the_snapshot_is_deleted(tmp_path):
    ...
    ``codexcrew-dashboard`` carries ``opt_in``, which makes it an assignable set
    ...
    response, written = await _put(
        tmp_path,
        on_disk={"name": "codexcrew", "mcpServers": {"codexcrew-dashboard": {"command": "d"}}},
        submitted={"name": "codexcrew", "mcpServers": {}},
        managed_specs={"codexcrew-dashboard": {"opt_in": True}},
    )
...
async def test_a_gate_closed_managed_server_omitted_from_the_snapshot_is_deleted(tmp_path):
    ...
    ``codexcrew-computer``'s gate is consulted at emission time, and both spec
    ...
    response, written = await _put(
        tmp_path,
        on_disk={"name": "codexcrew", "mcpServers": {"codexcrew-computer": {"command": "u"}}},
        submitted={"name": "codexcrew", "mcpServers": {}},
        managed_specs={"codexcrew-computer": {"spec_gate": lambda: False}},
    )
...
async def test_a_gate_open_managed_server_is_still_preserved(tmp_path):
    ...
    response, written = await _put(
        ...
        on_disk={"name": "codexcrew", "mcpServers": {"codexcrew-computer": {"command": "u"}}},
        submitted={"name": "codexcrew", "mcpServers": {}},
        managed_specs={"codexcrew-computer": {"spec_gate": lambda: True}},
    )

    assert response.status == 200
    assert written["mcpServers"] == {"codexcrew-computer": {"command": "u"}}
...
async def test_a_managed_server_whose_gate_raises_is_deleted(tmp_path):
    ...
    response, written = await _put(
        ...
        on_disk={"name": "codexcrew", "mcpServers": {"codexcrew-computer": {"command": "u"}}},
        submitted={"name": "codexcrew", "mcpServers": {}},
        managed_specs={"codexcrew-computer": {"spec_gate": _explode}},
    )
...
async def test_an_edition_contributed_server_is_preserved(tmp_path):
    ...
    response, written = await _put(
        tmp_path,
        on_disk={"name": "codexcrew", "mcpServers": {"internal-mcp": {"command": "i"}}},
        submitted={"name": "codexcrew", "mcpServers": {}},
...
async def test_a_namespaced_edition_extra_is_preserved_by_host_ownership(tmp_path):
    ...
    response, written = await _put(
        tmp_path,
        on_disk={"name": "codexcrew", "mcpServers": {"vendor:tools": {"command": "v"}}},
        submitted={"name": "codexcrew", "mcpServers": {}},
...
async def test_client_owned_entry_present_in_the_snapshot_is_updated(tmp_path):
    ...
    response, written = await _put(
        tmp_path,
        on_disk={"name": "codexcrew", "mcpServers": {"weather": {"url": "old"}}},
        submitted={"name": "codexcrew", "mcpServers": {"weather": {"url": "new"}}},
        scope_names=frozenset({"weather"}),
    )
...
async def test_app_owned_entry_present_in_the_snapshot_is_updated(tmp_path):
    ...
    response, written = await _put(
        tmp_path,
        on_disk={"name": "codexcrew", "mcpServers": {"demo:notes": {"command": "old"}}},
        submitted={"name": "codexcrew", "mcpServers": {"demo:notes": {"command": "new"}}},
        scope_names=frozenset(),
    )
...
async def test_non_mcp_keys_are_still_replaced_wholesale(tmp_path):
    ...
    response, written = await _put(
        tmp_path,
        on_disk={"name": "codexcrew", "prompt": "old", "tools": ["a"], "mcpServers": {}},
        submitted={"name": "codexcrew", "tools": ["b"]},
        scope_names=frozenset(),
    )
...
async def test_preserved_entries_go_through_the_governance_filter(tmp_path, monkeypatch):
    ...
    """
    import codex_crew.platform.governance as gov
...
async def test_a_stale_snapshot_cannot_resurrect_an_uninstalled_apps_bridge(tmp_path):
    ...
    response, written = await _put(
        tmp_path,
        on_disk={"name": "codexcrew", "mcpServers": {"plainuser": {"url": "u"}}},
        submitted={
            "name": "codexcrew",
            "mcpServers": {
...
async def test_a_stale_snapshot_cannot_resurrect_a_disabled_apps_bridge(tmp_path):
    ...
    response, written = await _put(
        tmp_path,
        on_disk={"name": "codexcrew", "mcpServers": {}},
        submitted={"name": "codexcrew", "mcpServers": {"demo:notes": {"command": "notes-mcp"}}},
        apps={"demo": ("notes",)},
        enabled=False,
        scope_names=frozenset({"demo:notes"}),
    )
...
async def test_a_plain_client_entry_the_client_adds_is_still_persisted(tmp_path):
    ...
    response, written = await _put(
        tmp_path,
        on_disk={"name": "codexcrew", "mcpServers": {}},
        submitted={"name": "codexcrew", "mcpServers": {"myserver": {"command": "mine"}}},
    )
...
async def test_a_namespaced_edition_extra_the_client_resubmits_is_left_alone(tmp_path):
    ...
    response, written = await _put(
        tmp_path,
        on_disk={"name": "codexcrew", "mcpServers": {}},
        submitted={"name": "codexcrew", "mcpServers": {"edition:extra": {"command": "x"}}},
...
async def test_a_readable_spec_with_no_servers_still_drops_a_namespaced_addition(tmp_path):
    ...
    response, written = await _put(
        tmp_path,
        on_disk={"name": "codexcrew", "mcpServers": {}},
        submitted={"name": "codexcrew", "mcpServers": {"demoapp:tool": {"command": "t"}}},
        apps={},
    )
...
async def test_a_spec_with_no_mcpservers_key_still_drops_a_namespaced_addition(tmp_path):
    ...
    response, written = await _put(
        tmp_path,
        on_disk={"name": "codexcrew"},  # readable, and carries no mcpServers at all
        submitted={"name": "codexcrew", "mcpServers": {"demo:notes": {"command": "notes-mcp"}}},
        apps={"demo": ("notes",)},
        enabled=False,
    )
...
async def test_a_malformed_mcpservers_value_on_disk_decides_nothing(tmp_path):
    ...
    response, written = await _put(
        tmp_path,
        on_disk={"name": "codexcrew", "mcpServers": "not an object"},
        submitted={"name": "codexcrew", "mcpServers": {"demoapp:tool": {"command": "t"}}},
        apps={},
    )
...
async def test_the_app_namespace_region_decides_every_axis_it_claims_to(tmp_path):
    ...
    * CONTENT, name on BOTH sides -> the SUBMISSION decides. **This cell is
    ...
    """
    submitted_only = {"name": "codexcrew", "mcpServers": {"demo:ghost": {"command": "ghost"}}}
    matrix = [
        (
            "existence / absent from the submission -> on disk decides",
            {"demo:notes": {"command": "live"}},
            {"name": "codexcrew", "mcpServers": {}},
            {"demo:notes": {"command": "live"}},
        ...
        (
            ...
            {"demo:notes": {"command": "live"}},
            {"name": "codexcrew", "mcpServers": {"demo:notes": {"command": "stale"}}},
    ...
    for axis, on_disk, submitted, expected, why in matrix:
        response, written = await _put(
            tmp_path,
            on_disk={"name": "codexcrew", "mcpServers": on_disk},
            submitted=submitted,
        )
...
async def test_a_stale_snapshot_reverts_a_live_http_bridge_to_a_dead_url(tmp_path):
    ...
    response, written = await _put(
        tmp_path,
        on_disk={"name": "codexcrew", "mcpServers": {"demo:notes": {"url": live}}},
        submitted={"name": "codexcrew", "mcpServers": {"demo:notes": {"url": illustrative}}},
    )
