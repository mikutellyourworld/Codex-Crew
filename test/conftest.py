...
from codex_crew.safety_override import reset_singleton as _reset_safety_override
from codex_crew.safety_override import reset_yolo_policy_cache as _reset_yolo_policy_cache
from codex_crew.slack.client import SlackClientOps
from codex_crew.slack.handler import _PHASE_EMOJIS, _build_phase_emojis
...
# ── Windows CI ──────────────────────────────────────────────────────────
# The backend runs natively on Windows (codex_crew.platform_compat), but a
...
from codex_crew import platform_compat  # noqa: E402
...
#: ``src/codex_crew/apps/builtins/auto_improvement/tests/``, and a hook rooted here never
...
def _release_stt_engine():
    ...
    ``codex_crew.stt.engine`` keeps ONE recogniser per process on purpose: the
...
def _reset_degraded_config_observations():
    ...
    ``codex_crew.config.loader`` remembers every malformed config section it has
    ...
    """
    from codex_crew.config.loader import reset_degraded_observations
...
#: ``_isolation_root`` / ``_isolation_dirs`` / ``_isolate_codexcrew_home`` live in the
...
#: ``src/codex_crew/apps/builtins/*/tests/`` and never see this file. The fixtures
...
def _disable_dev_fleet_background_tasks(monkeypatch):
    ...
    """
    monkeypatch.setenv("CODEXCREW_DEVFLEET_NO_BACKGROUND", "1")
...
def _isolate_kiro_window_cache():
    ...
    * **Import-time host leak** — ``model_registry`` calls ``_load_kiro_windows()``
    ...
    own body.
    """
    import codex_crew.model_registry as _mr
...
def _isolate_message_entry_cache():
    ...
    once per process from whatever CODEXCREW_HOME the first caller saw, so a test
    that resolved them under its own home would otherwise leak its bounds into
    every later test's cache behaviour.
    """
    from codex_crew.dashboard import chat_persistence as _cp
...
def _reset_options_control_state():
    ...
    ``codex_crew.slack.outbound`` holds two process-global maps keyed by
    ...
    """
    from codex_crew.slack import outbound
...
def _enterprise_bypass(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codex_crew.slack.enterprise._validated_team_id", "TTEST")
    monkeypatch.setattr("codex_crew.slack.enterprise._validated_enterprise_id", "ETEST")
    monkeypatch.setattr("codex_crew.slack.enterprise._allowed_team_ids", {"TTEST"})
...
def _fake_computer_use_backend():
    ...
    the ``codex_crew.testing.fake_computer_use`` import) on all ~16k tests would
    ...
    it fail.
    """
    from codex_crew.computer_use.backend import (
        register_computer_use_backend,
        reset_shared_backend,
    )
    from codex_crew.testing.fake_computer_use import FakeComputerUseBackend
...
def _reset_platform_context(monkeypatch):
    ...
    Also pins ``CODEXCREW_PROFILE=standalone`` by default so a dev box that has a
    ...
    """
    from codex_crew.platform.bootstrap import _reset_boot_state
    from codex_crew.platform.context import reset_context

    monkeypatch.setenv("CODEXCREW_PROFILE", "standalone")
    reset_context()
    _reset_boot_state()
    yield
    reset_context()
...
def _no_release_feed_network(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    env with no ``CODEXCREW_PROJECT_DIR`` takes the feed branch by definition.
    ...
    async def _refuse(url: str) -> tuple[int, bytes]:
        raise AssertionError(
            ...
            "codex_crew.dashboard.handlers.updates._fetch_feed_bytes instead"
        )

    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.updates._fetch_feed_bytes", _refuse, raising=True
    )
...
def _no_live_catalog_network(monkeypatch: pytest.MonkeyPatch):
    ...
    def _refuse(req: object) -> None:
        ...
        raise AssertionError(
            ...
            "codex_crew.apps.official_catalog.fetch_document (or a higher "
...
def named_cron_caller(monkeypatch):
    ...
    monkeypatch.setenv("CODEXCREW_SESSION_KEY", key)
    return key
...
def healthy_host_memory(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    import codex_crew.resource_status as resource_status
    import codex_crew.subagent as subagent
...
def _reset_create_rate_limit_buckets():
    ...
    ``codex_crew.dashboard.create_rate_limit`` keeps its per-(verb, caller)
    ...
    """
    from codex_crew.dashboard import create_rate_limit
...
def gateway_posts(request, monkeypatch):
    ...
    avoided a live write because this conftest's ``CODEXCREW_HOME`` pin makes
