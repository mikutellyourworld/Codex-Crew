...
class TestNativeCardRegistry:
    ...
    def test_register_creates_dict_and_entry(self):
        from codex_crew.dashboard.chat_runner import _register_native_card
    ...
    def test_unregister_noop_without_registry(self):
        from codex_crew.dashboard.chat_runner import _unregister_native_card
...
class TestNativeCardFeedRedaction:
    ...
    def test_feed_joined_and_truncated(self):
        from codex_crew.dashboard.chat_runner import _native_card_feed
    ...
    def test_feed_applies_both_redactions(self):
        ...
        with (
            patch(
                "codex_crew.dashboard.chat_runner.redact_exfiltration_urls",
                return_value=("URLS_REDACTED", 0),
            ) as m_urls,
            patch(
                "codex_crew.dashboard.chat_runner.redact_credentials",
...
class TestNativeCancelHandler:
    ...
    async def test_cancel_native_card_broadcasts_done_and_audits(self):
        ...
        from codex_crew.dashboard.handlers.messaging import api_spawn_delete

        state = _make_state()
        state._native_cards = {"native:s1": {"slot": "slot-1", "session_id": "s1", "started": 0}}
        with patch("codex_crew.dashboard.handlers.messaging._sel") as m_sel:
    ...
    async def test_cancel_native_card_survives_sel_failure(self):
        ...
        state._native_cards = {"native:s1": {"slot": "slot-1", "session_id": "s1", "started": 0}}
        with patch(
            "codex_crew.dashboard.handlers.messaging._sel",
...
class TestNativeAutoApproveLogSanitization:
    ...
    def test_newline_is_escaped_not_forged(self) -> None:
        from codex_crew.dashboard.chat_runner import _safe_native_crew_debug_title
