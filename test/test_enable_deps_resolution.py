...
def _mock_dashboard_server():
    ...
    if "codex_crew.dashboard.server" not in sys.modules:
        sys.modules["codex_crew.dashboard.server"] = MagicMock()
    yield
...
class TestEnableDepsResolution:
    ...
    async def test_dependencies_resolved_on_enable(self) -> None:
        ...
        from codex_crew.apps.dependencies import DependencyResult
        ...
        with (
            patch("codex_crew.apps.routes.get_app", return_value=fake_app_info),
            patch(
                "codex_crew.apps.routes.enable_app",
                return_value=MagicMock(ok=True, to_dict=lambda: {"ok": True}),
            ),
            patch("codex_crew.apps.routes.register_app", return_value=MagicMock(to_dict=lambda: {})),
            patch("codex_crew.apps.routes.start_app_backend", return_value=None),
            patch("codex_crew.apps.routes.on_app_enable", new_callable=AsyncMock, return_value=None),
            patch("codex_crew.apps.routes.sel", return_value=MagicMock()),
            patch(
                "codex_crew.apps.routes._resolve_deps",
                new_callable=AsyncMock,
                return_value=mock_dep_result,
            ) as mock_resolve,
        ):
            from codex_crew.apps.routes import handle_enable_app
    ...
    async def test_no_dependencies_skips_resolution(self) -> None:
        ...
        with (
            ...
            patch("codex_crew.apps.routes.on_app_enable", new_callable=AsyncMock, return_value=None),
            patch("codex_crew.apps.routes.sel", return_value=MagicMock()),
            patch("codex_crew.apps.routes._resolve_deps", new_callable=AsyncMock) as mock_resolve,
        ):
            from codex_crew.apps.routes import handle_enable_app
    ...
    async def test_deps_resolved_before_on_enable_script(self) -> None:
        ...
        with (
            ...
            patch("codex_crew.apps.routes.register_app", return_value=MagicMock(to_dict=lambda: {})),
            patch("codex_crew.apps.routes.start_app_backend", return_value=None),
            patch("codex_crew.apps.routes._run_lifecycle_script", side_effect=mock_script),
            patch("codex_crew.apps.routes.on_app_enable", new_callable=AsyncMock, return_value=None),
            patch("codex_crew.apps.routes.sel", return_value=MagicMock()),
            patch("codex_crew.apps.routes._resolve_deps", side_effect=mock_resolve),
        ):
            from codex_crew.apps.routes import handle_enable_app
...
class TestClientInstallOnEnableIsAdvisory:
    ...
    async def test_client_app_stays_enabled_when_desktop_app_missing(self) -> None:
        ...
        with (
            patch("codex_crew.apps.routes.get_app", return_value=self._client_app()),
            patch(
                "codex_crew.apps.routes.enable_app",
                return_value=MagicMock(ok=True, to_dict=lambda: {"ok": True}),
            ),
            ...
            patch("codex_crew.apps.routes.sel", return_value=MagicMock()),
            patch(
                "codex_crew.apps.routes._run_lifecycle_script",
                new_callable=AsyncMock,
                return_value=failed_script,
            ),
            patch("codex_crew.apps.routes.disable_app") as mock_disable,
            patch("codex_crew.apps.routes.sys") as mock_sys,
        ):
    ...
    async def test_client_app_script_skipped_on_unsupported_os(self) -> None:
        ...
        with (
            ...
            patch("codex_crew.apps.routes.sel", return_value=MagicMock()),
            patch(
                "codex_crew.apps.routes._run_lifecycle_script", new_callable=AsyncMock
            ) as mock_script,
            patch("codex_crew.apps.routes.disable_app") as mock_disable,
            patch("codex_crew.apps.routes.sys") as mock_sys,
        ):
    ...
    def test_a_malformed_platform_block_cannot_break_enable(self, platform_value) -> None:
        ...
        """
        from codex_crew.apps.routes import _client_install_manifest
    ...
    async def test_server_app_onenable_failure_still_rolls_back(self) -> None:
        ...
        with (
            patch("codex_crew.apps.routes.get_app", return_value=server_app),
            patch(
                "codex_crew.apps.routes.enable_app",
                return_value=MagicMock(ok=True, to_dict=lambda: {"ok": True}),
            ),
            patch("codex_crew.apps.routes.register_app", return_value=MagicMock(to_dict=lambda: {})),
            patch("codex_crew.apps.routes.start_app_backend", return_value=None),
            patch("codex_crew.apps.routes.stop_app_backend"),
            patch("codex_crew.apps.routes.deregister_app") as mock_dereg,
            patch("codex_crew.apps.routes.on_app_enable", new_callable=AsyncMock, return_value=None),
            patch("codex_crew.apps.routes.sel", return_value=MagicMock()),
            patch(
                "codex_crew.apps.routes._run_lifecycle_script",
