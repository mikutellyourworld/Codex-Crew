...
every ``codex_crew`` record into the same file the rotating file handler
...
def _pristine_logging():
    ...
    ``_setup_cli_logging`` mutates the root and ``codex_crew`` loggers
    ...
    kc = logging.getLogger("codex_crew")
...
class TestSetupCliLoggingDetached:
    ...
    def _detached(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.cli._fd_targets_file", lambda fd, path: True)
        self.redirect = MagicMock()
        monkeypatch.setattr("codex_crew.cli._redirect_fds_to", self.redirect)
    ...
    def test_queue_handler_on_root_not_codex_crew(self):
        _setup_cli_logging("gateway", 1)
        ...
        for logger in (logging.getLogger(), logging.getLogger("codex_crew")):
    ...
    def test_codex_crew_record_written_exactly_once(self):
        _setup_cli_logging("gateway", 1)
        logging.getLogger("codex_crew.test_doublewrite").warning("sentinel-record")
    ...
    def test_handler_level_capped_at_warning_for_third_party(self, monkeypatch):
        """A stricter persisted codex_crew level must not gag third-party
        WARNINGs on the shared root handler (codex_crew records stay filtered
        at the codex_crew logger itself)."""
        from codex_crew.config import CodexCrewConfig

        cfg = CodexCrewConfig.load()
        cfg.agent.log_level = "ERROR"
        monkeypatch.setattr("codex_crew.cli.CodexCrewConfig.load", staticmethod(lambda: cfg))
        ...
        assert logging.getLogger("codex_crew").level == logging.ERROR
...
class TestSetupCliLoggingForeground:
    ...
    def test_queue_handler_on_codex_crew_logger(self):
        _setup_cli_logging("gateway", 1)
        kc_qhs = [
            h
            for h in logging.getLogger("codex_crew").handlers
    ...
    def test_record_written_once_to_file(self):
        _setup_cli_logging("gateway", 1)
        logging.getLogger("codex_crew.test_foreground").warning("fg-sentinel")
        _stop_log_queue_listener()  # deterministic drain to disk
        text = (config_dir() / "gateway.log").read_text(encoding="utf-8")
...
class TestQueueOffLoop:
    ...
    def _foreground(self, monkeypatch):
        monkeypatch.setattr("codex_crew.cli._fd_targets_file", lambda fd, path: False)
        monkeypatch.setattr("codex_crew.cli._redirect_fds_to", MagicMock())
    ...
    def test_file_emit_runs_on_listener_thread(self):
        ...
        logging.getLogger("codex_crew.offloop").warning("off-loop-sentinel")
    ...
    def test_stop_drains_pending_records(self):
        _setup_cli_logging("gateway", 1)
        log = logging.getLogger("codex_crew.drain")
        for i in range(50):
    ...
    def test_short_lived_command_keeps_sync_handler(self):
        ...
        logging.getLogger("codex_crew.short").warning("short-lived-record")
    ...
    def test_bounded_stop_still_drains_when_healthy(self):
        ...
        _setup_cli_logging("gateway", 1)
        logging.getLogger("codex_crew.bounded").warning("bounded-record")
    ...
    def test_bounded_stop_gives_up_on_wedged_handler(self):
        ...
        logging.getLogger("codex_crew.wedged").warning("wedged-record")
        t0 = time.monotonic()
...
class TestDrainBeforeHardExit:
    ...
    async def test_queued_tail_reaches_disk(self):
        ...
        _setup_cli_logging("gateway", 1)
        logging.getLogger("codex_crew.shutdown").warning("shutdown-tail-record")
    ...
    async def test_wedged_handler_does_not_delay_the_exit(self):
        ...
        fh.emit = wedged_emit  # type: ignore[method-assign]
        logging.getLogger("codex_crew.wedged").warning("wedged-shutdown-record")
        t0 = time.monotonic()
...
class TestEveryGatewayHardExitDrainsTheQueue:
    ...
    drains it. ``src/codex_crew/slack/gateway.py`` and ``.../events.py`` are the
    ...
    _MODULES = (
        Path(__file__).resolve().parents[1] / "src/codex_crew/slack/gateway.py",
        Path(__file__).resolve().parents[1] / "src/codex_crew/slack/events.py",
    )
