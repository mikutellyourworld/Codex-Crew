...
def test_pool_key_field_set_is_exactly_the_twelve_dimensions() -> None:
    ...
    ``CODEXCREW_PRINCIPAL`` source, so it always collapsed to the OS user and
...
class TestChannelIsNotAPoolDimension:
    ...
    delivered to backends PER CALL via ``_meta.codexcrew.caller`` instead, so a
    ...
    def test_legacy_user_identity_is_not_a_pool_dimension(self) -> None:
        ...
        anything — nothing populated ``CODEXCREW_PRINCIPAL``, so it always
