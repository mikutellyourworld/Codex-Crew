"""Regression tests for Round-21 findings (CodexCrew PR #6).

F1: the app-arch (CloudFormation stack) reap path verifies the stack's
    codexcrew:site identity tag before deletion, matching the R19 engine-arch
...
_SKILL = Path(__file__).resolve().parents[1] / (
    "src/codex_crew/deploy/skills/artifact-deploy"
)
...
def test_f1_deploy_backend_tags_stack_at_creation():
    ...
    assert '--tags "codexcrew:site=$SLUG"' in src
...
def test_f1_shell_reaper_gates_stack_deletion():
    ...
    assert src.count("codexcrew:site") >= 2
...
def _validate(url: str):
    from codex_crew.validation import _validate_webapp_metadata_shape
