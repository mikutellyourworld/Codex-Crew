"""Tests for ``codex_crew.testing.harness`` — gateway-spawning context manager.
...
the end-to-end test is gated behind ``CODEXCREW_HARNESS_INTEGRATION``.
"""
...
from codex_crew.testing.harness import (
...
def test_ready_line_raises_on_timeout_with_stderr() -> None:
    ...
    assert "CODEXCREW_HARNESS_READY_TIMEOUT" in str(exc.value)
...
def test_terminate_falls_back_to_sigkill() -> None:
    ...
    try:
        ...
        with patch("codex_crew.testing.harness.TERMINATE_GRACE_SECONDS", 0.5):
            _terminate_process_group(proc)
...
def test_resolve_workspace_src_finds_package() -> None:
    ...
    assert (src / "codex_crew" / "__init__.py").exists()
...
def test_spawn_feature_gateway_happy_path() -> None:
    ...
    with (
        patch("codex_crew.testing.harness.subprocess.Popen", side_effect=fake_popen),
        patch(
            "codex_crew.testing.harness._terminate_process_group",
    ...
    # Spawn invokes ``codexcrew gateway --test-mode --seed <fixture>`` —
    ...
    assert captured_env["CODEXCREW_FAKE_ACP_TEST_MODE"] == "1"
...
def test_spawn_feature_gateway_isolates_the_agent_spec_home() -> None:
    ...
    ``CODEXCREW_HOME`` isolated (the data home) and ``KIRO_HOME`` left at the default,
    ...
    with (
        patch("codex_crew.testing.harness.subprocess.Popen", side_effect=fake_popen),
        patch("codex_crew.testing.harness._terminate_process_group"),
    ):
        with spawn_feature_gateway(fixture="empty") as handle:
            ...
            assert captured_env["CODEXCREW_HOME"] == str(data_home)
...
def test_parallel_spawns_get_distinct_homes_and_ports() -> None:
    ...
    CODEXCREW_HOME or port". Mocks Popen so each call returns a fake
    ...
    with (
        patch(
            "codex_crew.testing.harness.subprocess.Popen",
...
@pytest.mark.skipif(
    not os.environ.get("CODEXCREW_HARNESS_INTEGRATION"),
    reason=(
        "Real-gateway integration test. Set CODEXCREW_HARNESS_INTEGRATION=1 to run. "
