...
class TestSchemaShape:
    ...
    def test_a_fraction_survives_the_FULL_loader_path_and_truncates(self):
        """The truncation rule has to hold through ``CodexCrewConfig.load()``, not
...
class TestParseRule:
    ...
    def test_a_negative_fraction_does_not_disable_the_nofile_ceiling(self):
        ...
        from codex_crew.security import _RLIMIT_DEFAULTS, resource_limit_spec
    ...
    def test_a_refusal_is_logged_once_per_key(self, caplog):
        ...
        with caplog.at_level("WARNING", logger="codex_crew.config.loader"):
            for _ in range(3):
...
class TestSliceConsumer:
    ...
    def _limits(raw: dict):
        ...
        with unittest.mock.patch(
            "codex_crew.config.loader._raw_config", return_value={"resource_limits": raw}
        ):
...
class TestXdistConsumer:
    @staticmethod
    def _cap(raw: dict):
        import codex_crew.resource_status as rs
...
class TestOverlayOwnership:
    ...
    def _save_with_overlay(tmp_path: Path, base: dict, overlay: dict) -> dict:
        ...
        with (
            ...
            unittest.mock.patch("codex_crew.config.loader.config_local_path", return_value=local),
        ):
