...
class TestPolicyConfig:
    ...
    def test_extra_denied_can_only_add(self):
        ...
        # Retargeted onto ``codexcrew_self`` — the ONE entry the floor still carries
        ...
        ours = AppRef(name="Codex Crew", pid=1, bundle_id="dev.kiro.crew")
...
class TestKeystoneProtection:
    ...
    @pytest.mark.parametrize(
        "path",
        [
            "~/.codex-crew/computer_use.json",
            "~/.codexcrew/computer_use.json",
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "less ~/.codex-crew/computer_use.json",
            "echo '{\"enabled\":true}' > ~/.codex-crew/computer_use.json",
            "echo x >> ~/.codex-crew/computer_use.json",
            "tee ~/.codex-crew/computer_use.json",
            "cp /tmp/evil.json ~/.codex-crew/computer_use.json",
            "python -c \"open('~/.codex-crew/computer_use.json','w')\"",
        ],
    )
    ...
    def test_archive_extraction_into_the_trust_root_is_blocked(self):
        ...
        # ever naming it.
        for command in (
            "tar -xf payload.tar -C ~/.codex-crew",
            "unzip payload.zip -d ~/.codex-crew",
        ):
            assert security.is_sensitive_bash_command(command), command
