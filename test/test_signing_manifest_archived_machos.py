...
def _bundle(tmp_path: pathlib.Path) -> pathlib.Path:
    binaries = tmp_path / "CodexCrew.app" / "Contents" / "Resources" / "binaries"
...
def test_a_clean_bundle_reports_nothing(generator, tmp_path):
    ...
    assert generator.find_archived_machos(str(tmp_path / "CodexCrew.app")) == []
...
def test_gzip_sealed_macho_is_reported(generator, tmp_path):
    ...
    errors = generator.find_archived_machos(str(tmp_path / "CodexCrew.app"))
...
def test_zip_and_tar_members_are_inspected_too(generator, tmp_path):
    ...
    reported = " ".join(generator.find_archived_machos(str(tmp_path / "CodexCrew.app")))
