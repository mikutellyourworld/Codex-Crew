"""Tests for the `codexcrew doctor` OS-aware fix hints.

Guards _os_fix_hint: it returns the macOS Homebrew command on Darwin and the
Linux/AL2023 guidance otherwise, so `codexcrew doctor` never prints a brew
...
from codex_crew import cli_doctor, cron
...
class TestManagedServicePolicyDoctor:
    ...
    def test_stale_service_names_the_one_time_fix(self, monkeypatch, capsys):
        ...
        assert "codexcrew service install" in output
...
class TestFixHint:
    """OS-aware `codexcrew doctor` fix hints."""
...
class TestFfmpegLinuxHintResolvable:
    ...
    def test_hint_offers_the_managed_store_download(self) -> None:
        ...
        from codex_crew.stt import decoder
...
class TestDataHome:
    """`codexcrew doctor` Data Home section — location + leftover legacy home."""

    def test_legacy_present_default_path_says_not_the_data_home(
        self, monkeypatch, tmp_path: Path, capsys
    ) -> None:
        # A leftover top-level ~/.codexcrew on the default path is not the data
        # home — the doctor notes it as safe to delete, never as active state.
        monkeypatch.setattr(Path, "home", classmethod(lambda cls: tmp_path))
        monkeypatch.delenv("CODEXCREW_HOME", raising=False)  # default-path case
    ...
    def test_legacy_override_points_at_legacy_says_active_not_ignored(
        ...
        # CODEXCREW_HOME=~/.codexcrew makes the legacy dir the ACTIVE home, not
        # ignored debris — the doctor must not mislabel the home the process is
        # actually using (GPT 5.6 MEDIUM).
        monkeypatch.setattr(Path, "home", classmethod(lambda cls: tmp_path))
        legacy = tmp_path / cli_doctor.LEGACY_CONFIG_DIR_NAME
        legacy.mkdir()
        monkeypatch.setenv("CODEXCREW_HOME", str(legacy))
    ...
    def test_legacy_with_venv_is_never_advised_deletable(
        ...
        # An older wheel install could nest its managed venv inside ~/.codexcrew,
...
class TestPodSessionBus:
    """`codexcrew doctor` Pods section — the systemd --user session bus.
    ...
    def test_missing_bus_is_reported_but_never_blocks(
        ...
        # such host is told its setup is broken (and `codexcrew doctor` starts
        # exiting 1 in CI).
...
class TestTrustRoot:
    """`codexcrew doctor` reports whether session identities can be signed.
...
class TestMemoryPressure:
    """`codexcrew doctor` Memory Pressure section — freeze-preparedness verdict.
...
class TestDoctorKas:
    """`codexcrew doctor` KAS backend section — gated on acp_backend == kas.
    ...
    def _patch_cfg(self, monkeypatch, backend: str) -> None:
        monkeypatch.setattr(
            cli_doctor.CodexCrewConfig, "load", classmethod(lambda cls: self._Cfg(backend))
        )
...
class TestPathLauncherOwnership:
    """`codexcrew doctor` names which install owns the `codexcrew` command.
    ...
    def test_matching_launcher_is_reported_clean(self, monkeypatch, tmp_path, capsys) -> None:
        exe = tmp_path / "opt" / "bin" / "codexcrew"
        exe.parent.mkdir(parents=True)
        exe.write_text("")
        monkeypatch.setattr(cli_doctor.shutil, "which", lambda c, **kw: str(exe))
        monkeypatch.setattr("codex_crew.agent._resolve_codexcrew_bin", lambda: str(exe))

        cli_doctor._doctor_path_launcher()

        out = capsys.readouterr().out
        assert "codexcrew CLI: ✅" in out
        assert "different install" not in out

    def test_divergent_launcher_names_both_paths(self, monkeypatch, tmp_path, capsys) -> None:
        wheel = tmp_path / "crew-venv" / "bin" / "codexcrew"
        wheel.parent.mkdir(parents=True)
        wheel.write_text("")
        package = tmp_path / "opt" / "CodexCrew" / "codexcrew"  # brand-ok: real /opt path
        package.parent.mkdir(parents=True)
        package.write_text("")
        monkeypatch.setattr(cli_doctor.shutil, "which", lambda c, **kw: str(wheel))
        monkeypatch.setattr("codex_crew.agent._resolve_codexcrew_bin", lambda: str(package))

        cli_doctor._doctor_path_launcher()

        out = capsys.readouterr().out
        assert "⚠ codexcrew CLI on PATH belongs to a different install" in out
    ...
    def test_unresolvable_install_does_not_cry_wolf(self, monkeypatch, tmp_path, capsys) -> None:
        """A bare "codexcrew" sentinel is not a path, so there is nothing to
        compare and no divergence to claim."""
        found = tmp_path / "bin" / "codexcrew"
        found.parent.mkdir(parents=True)
        found.write_text("")
        monkeypatch.setattr(cli_doctor.shutil, "which", lambda c, **kw: str(found))
        monkeypatch.setattr("codex_crew.agent._resolve_codexcrew_bin", lambda: "codexcrew")
...
class TestSourceCheckout:
    """`codexcrew doctor` Source Checkout section — stale/off-branch source tree.
...
class TestEffectiveModelSection:
    """`codexcrew doctor`'s Model section (#2559).
    ...
    def test_spec_pin_decides_when_the_global_defers(self, capsys) -> None:
        ...
        assert "codexcrew agent reset-model" in out
    ...
    def test_project_local_spec_is_flagged_as_shadowing(self, capsys) -> None:
        """kiro-cli resolves <project>/.kiro/agents FIRST and Codex Crew's own
        resolver never reads it, so that file can decide what actually runs while
        every Codex Crew surface reports something else."""
        from codex_crew.agent import AGENT_FILENAME
    ...
    def _bind_custom_agent(self, cfg, name: str):
        ...
        cfg.agents["default"] = CodexCrewAgentConfig(kiro_agent=name)
        return cfg
    ...
    def test_a_bound_custom_agent_is_attributed_to_its_own_spec(self, capsys) -> None:
        ...
        Reading codexcrew.json in both cases attributed the pin to the wrong file
        ...
        assert "codexcrew agent reset-model --agent 'custom-agent'" in out
    ...
    def test_control_sequences_in_a_project_spec_are_escaped(self, capsys) -> None:
        ...
        (project / ".kiro" / "agents").mkdir(parents=True)
        (project / ".kiro" / "agents" / AGENT_FILENAME).write_text(
            json.dumps({"name": "codexcrew", "model": "x\x1b[31mred"}), encoding="utf-8"
        )
        issues: list[str] = []
    ...
    def test_a_control_bearing_project_filename_is_escaped(self, monkeypatch, capsys) -> None:
        ...
        """
        hostile = Path("/tmp/proj/.kiro/agents/codexcrew\x1b[2J.json")
        self._install_spec(None)
        real_reader = cli_doctor._read_agent_spec
        monkeypatch.setattr(cli_doctor, "project_agent_files", lambda d: [hostile])
        monkeypatch.setattr(cli_doctor, "project_agent_name", lambda p: "codexcrew")
...
class TestWhatsAppSection:
    """`codexcrew doctor`'s WhatsApp Integration section.
    ...
    def _extra(monkeypatch, present: bool) -> None:
        monkeypatch.setattr(
            "codex_crew.whatsapp.client.neonize_available", lambda: present
        )

    @staticmethod
    def _pair(home: Path) -> Path:
        from codex_crew.whatsapp.client import default_db_path
    ...
    def test_an_unpaired_store_warns_but_never_fails_doctor(
        ...
        `codexcrew doctor && codexcrew gateway` chain at the one moment the operator
...
class TestCronHealth:
    """`codexcrew doctor` Cron Jobs section — auto-paused / errored jobs.
    ...
    def test_auto_paused_job_is_reported_with_resume_hint(
        ...
        assert "codexcrew cron resume <id>" in out
    ...
    def test_errored_job_is_reported_with_trigger_hint(
        ...
        assert "codexcrew cron trigger <id>" in out
