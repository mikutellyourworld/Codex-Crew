...
from codex_crew.dashboard.handlers import mobile_connect
from codex_crew.platform.defaults import DefaultMobileConnectProvider
from codex_crew.platform.governance import SCOPE_CATALOG
from codex_crew.platform.interfaces import MobileConnectMethod
...
class TestGovernedMethods:
    def _with_provider(self, methods, denied: set[str] = frozenset()):
        ...
        with (
            patch("codex_crew.platform.context.current_context", return_value=ctx),
            patch(
    ...
    def test_degraded_seam_read_hides_the_entry(self) -> None:
        ...
        with patch("codex_crew.platform.context.current_context", return_value=ctx):
...
def _call_methods(request: MagicMock):
    with patch("codex_crew.dashboard.handlers.mobile_connect.check_origin", return_value=True):
...
class TestMintEndpointsReCheck:
    def test_mobile_link_mint_refused_when_method_denied(self) -> None:
        ...
        from codex_crew.dashboard.handlers import auth_mobile

        request = MagicMock(spec=web.Request)
        request.get.side_effect = {"user": "alice", "app": ""}.get
        request.headers = {"Origin": ""}
        with (
            patch("codex_crew.dashboard.handlers.auth_mobile.check_origin", return_value=True),
            patch(
                "codex_crew.dashboard.handlers.auth_mobile.mint_denied_reason",
    ...
    def test_tailnet_qr_mint_refused_when_method_denied(self) -> None:
        from codex_crew.dashboard.handlers import tailnet_mobile
        ...
        with (
            ...
            patch(
                "codex_crew.dashboard.handlers.tailnet_mobile.mint_denied_reason",
                return_value="mobile connect disabled by policy",
            ),
