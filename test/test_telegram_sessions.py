...
from codex_crew.messaging.link import UNBIND_REASON_UNSPECIFIED, ChannelLink
from codex_crew.messaging.renderer import session_provenance_tag
from codex_crew.messaging.session_resume import SETTLE_NOTHING, RoutingDecision
from codex_crew.messaging.session_trust import clear_trusted_sessions
from codex_crew.session import _opt_out_key
...
from codex_crew.telegram.renderer import TelegramApprovalDecider
from codex_crew.telegram.session_resume import _ROUTE_OWNER_REFUSAL
...
def _config() -> Any:
    return SimpleNamespace(
        ...
        agent=SimpleNamespace(default_agent="codexcrew"),
...
def _resume_module() -> Any:
    return importlib.import_module("codex_crew.telegram.session_resume")
...
class TestTelegramSessionPicker:
    ...
    async def test_unreadable_expectation_store_answers_the_press(self, tmp_path: Any) -> None:
        ...
        """
        from codex_crew.messaging.resume_expectation import ExpectationStoreError
    ...
    async def test_unified_dm_scope_still_gets_one_click_takeover(self, tmp_path: Any) -> None:
        ...
        dispatcher, client, sessions, _ = _dispatcher(tmp_path)
        link = dispatcher._session_resume.link_for(7, None)
        unified_native = "unified:codexcrew:direct:7"
    ...
    async def test_a_foreign_native_key_grants_nothing(self, tmp_path: Any) -> None:
        ...
        await dispatcher._session_resume.choose(
            client,
            _callback(data, message_id=message_id),
            native_key="unified:codexcrew:direct:999",
        )
...
class TestTelegramColdResume:
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param("default", "codexcrew", id="default-sentinel"),
            pytest.param("Auto", "codexcrew", id="auto-sentinel"),
            pytest.param("", "codexcrew", id="absent"),
        ],
    )
    @pytest.mark.asyncio
...
class TestTelegramRestrictedResumedSession:
    ...
    async def test_an_affirmative_incognito_marker_still_denies(self, tmp_path: Any) -> None:
        ...
        from codex_crew.messaging import upload_gate
