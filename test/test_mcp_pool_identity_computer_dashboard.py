"""``codexcrew-computer`` and ``codexcrew-dashboard`` resolve the calling session
from the injected caller block.

The exact state #4622 described and fixed for ``codexcrew-cron``: both servers
...
advertised ``codexcrew.caller-identity``, and ``mcp_gateway/backend.py`` strips
...
from codex_crew import mcp_computer, mcp_dashboard
