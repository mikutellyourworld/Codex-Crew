...
from codex_crew.apps.builtins.issue_radar.backend import github_client as gh

MARKER = (
    "<!-- codexcrew-crew id=c_7f3a phase=implementing pr=2271 "
...
class UpdateIssueCommentTest(unittest.TestCase):
    ...
    def test_patches_the_flat_comment_path_with_the_body_on_stdin(self):
        ...
        with mock.patch.object(gh, "_gh_run", return_value=_proc(json.dumps(payload))) as run:
            out = gh.update_issue_comment("o", "r", 9001, f">_ on this\n\n{MARKER}")
        ...
        self.assertEqual(sent, {"body": f">_ on this\n\n{MARKER}"})
...
class FindCrewClaimTest(unittest.TestCase):
    ...
    def test_parses_every_field_of_a_well_formed_marker(self):
        rows = [_comment_row(9001, f">_ **Andromeda** is on this\n\n{MARKER}")]
    ...
    def test_the_brief_sentinel_is_not_a_claim(self):
        # `codexcrew-crew-brief` shares the marker's prefix; matching it would make
        # every session that echoes the brief look like a claim.
        rows = [_comment_row(1, "<!-- codexcrew-crew-brief v1 -->")]
        self.assertEqual(gh.find_crew_claim(rows), [])

    def test_unknown_keys_are_ignored(self):
        marker = (
            "<!-- codexcrew-crew id=c_7f3a phase=claimed pr=7 "
    ...
    def test_absent_optional_fields_read_as_empty_not_missing(self):
        claim = gh.find_crew_claim([_comment_row(5, "<!-- codexcrew-crew id=c_1 -->")])[0]
    ...
    def test_a_marker_with_no_id_is_still_reported_as_a_claim(self):
        ...
        claim = gh.find_crew_claim([_comment_row(5, "<!-- codexcrew-crew phase=claimed -->")])[0]
        self.assertEqual(claim["crew_id"], "")
        self.assertEqual(gh.find_crew_claim([_comment_row(5, "<!-- codexcrew-crew phase=x -->")],
                                            crew_id="c_7f3a"), [])

    def test_a_non_numeric_pr_is_none_not_a_crash(self):
        marker = "<!-- codexcrew-crew id=c_1 pr=none updated=2026-08-08T20:44:12Z -->"
    ...
    def test_a_valid_stamp_with_fractional_seconds_is_accepted(self):
        marker = "<!-- codexcrew-crew id=c_1 updated=2026-08-08T20:44:12.501Z -->"
    ...
    def test_a_malformed_stamp_reads_as_unparseable_not_fresh(self):
        ...
        for bad in (
            ...
            marker = f"<!-- codexcrew-crew id=c_1 updated={bad} -->"
            claim = gh.find_crew_claim([_comment_row(5, marker)])[0]
    ...
    def test_claims_are_ordered_by_comment_id_ascending(self):
        rows = [
            _comment_row(9003, "<!-- codexcrew-crew id=c_c -->", created="2026-08-08T20:00:00Z"),
            _comment_row(9001, "<!-- codexcrew-crew id=c_a -->", created="2026-08-08T18:00:00Z"),
            _comment_row(9002, "<!-- codexcrew-crew id=c_b -->", created="2026-08-08T19:00:00Z"),
        ]
    ...
    def test_an_unknown_comment_id_sorts_last_so_it_cannot_win_a_collision(self):
        rows = [
            _comment_row(None, "<!-- codexcrew-crew id=c_ghost -->"),
            _comment_row(9002, "<!-- codexcrew-crew id=c_b -->"),
            _comment_row(9001, "<!-- codexcrew-crew id=c_a -->"),
        ]
        self.assertEqual([c["comment_id"] for c in gh.find_crew_claim(rows)],
                         [9001, 9002, None])
    ...
    def test_a_non_int_comment_id_is_normalized_to_none(self):
        for raw in ("9001", True, 9001.0, {"id": 9001}):
            claim = gh.find_crew_claim([_comment_row(raw, "<!-- codexcrew-crew id=c_1 -->")])[0]
    ...
    def test_crew_id_filters_to_one_crews_own_claims(self):
        rows = [
            _comment_row(9001, "<!-- codexcrew-crew id=c_other phase=claimed -->"),
    ...
    def test_the_first_marker_wins_when_a_body_carries_two(self):
        body = f"{MARKER}\nquoted example:\n<!-- codexcrew-crew id=c_zzz phase=resolved -->"
