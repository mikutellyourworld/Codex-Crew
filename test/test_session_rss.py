...
from codex_crew import session, session_pid
...
class TestGetSessionRssMb:
    ...
    def test_macos_returns_zero(self) -> None:
        ...
        with patch("codex_crew.session_pid.sys.platform", "darwin"), patch(
            "codex_crew.session_pid.platform_compat.IS_WINDOWS", False
        ):
    ...
    def test_windows_measures_the_tree_rather_than_returning_zero(self) -> None:
        ...
        """
        with patch("codex_crew.session_pid.sys.platform", "win32"), patch(
            "codex_crew.session_pid.platform_compat.IS_WINDOWS", True
        ), patch(
            "codex_crew.session_pid.platform_compat.proc_rss_tree_mb_for_pid",
            return_value=512.7,
        ):
    ...
    def test_accumulates_root_plus_descendants(self) -> None:
        ...
        with patch("codex_crew.session_pid.sys.platform", "linux"), patch(
            "codex_crew.session_pid._build_child_map", return_value=child_map
        ), patch(
            "codex_crew.session_pid._read_rss_pages",
    ...
    def test_rss_mb_from_tree_uses_shared_map(self) -> None:
        ...
        with patch(
            "codex_crew.session_pid._read_rss_pages",
            side_effect=lambda p, proc_root=None: pages.get(p, 0),
        ), patch("codex_crew.session_pid._build_child_map") as bm:
...
class TestProcParsingPrimitives:
    ...
    def test_get_session_rss_mb_end_to_end_real_parse(self, tmp_path) -> None:
        ...
        with patch("codex_crew.session_pid.sys.platform", "linux"):
...
class TestRssThresholdCheck:
    ...
    async def test_disabled_by_default_is_noop(self) -> None:
        ...
        with patch("codex_crew.session._build_child_map", return_value={}), patch(
            "codex_crew.session._rss_mb_from_tree", return_value=99999
        ) as g:
    ...
    async def test_over_threshold_session_is_reset(self) -> None:
        ...
        with patch("codex_crew.session._build_child_map", return_value={}), patch(
            "codex_crew.session._rss_mb_from_tree", return_value=2048
        ):
            await manager._rss_threshold_check()
    ...
    async def test_child_map_built_once_per_tick(self) -> None:
        ...
        manager.reset = AsyncMock(return_value=True)
        manager.get_pid = MagicMock(return_value=4242)
        with patch("codex_crew.session._build_child_map", return_value={}) as bm, patch(
            "codex_crew.session._rss_mb_from_tree", return_value=2048
        ) as rt:
            await manager._rss_threshold_check()
    ...
    async def test_windows_measures_per_candidate_without_a_shared_map(self) -> None:
        ...
        with patch.object(session.platform_compat, "IS_WINDOWS", True), patch(
            "codex_crew.session._build_child_map",
            side_effect=AssertionError("must not build a raw Toolhelp parent map"),
        ), patch("codex_crew.session.get_session_rss_mb", return_value=2048) as gs:
            await manager._rss_threshold_check()
    ...
    async def test_no_notification_when_under_threshold(self) -> None:
        ...
        manager.set_recycle_callback(cb)
        with patch("codex_crew.session._build_child_map", return_value={}), patch(
            "codex_crew.session._rss_mb_from_tree", return_value=512
        ):
            await manager._rss_threshold_check()
    ...
    async def test_busy_session_is_skipped(self) -> None:
        ...
        manager.get_pid = MagicMock(return_value=4242)
        with patch("codex_crew.session._build_child_map", return_value={}), patch(
            "codex_crew.session._rss_mb_from_tree", return_value=999999
        ) as g:
            await manager._rss_threshold_check()
    ...
    async def test_persistent_key_is_protected(self) -> None:
        from codex_crew.session import _PERSISTENT_KEYS
    ...
    async def test_channel_key_is_protected(self) -> None:
        ...
        from codex_crew.session import _CHANNEL_PREFIX
