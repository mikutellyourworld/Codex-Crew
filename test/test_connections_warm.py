...
from codex_crew.connections import tool_aliases, warm
from codex_crew.connections.mint import _mints
from codex_crew.connections.registry import Provider
...
def test_the_warm_sweep_refuses_a_cold_mint_spec_that_shares_the_prefix():
    ...
    for cold in ("codexcrew-mint-warm-foo-4821-9ab3c1de", "codexcrew-mint-warm-Foo-4821-9ab3c1de"):
...
def test_the_warm_sweep_drops_a_warm_spec_absent_from_the_plan_and_keeps_the_rest():
    assert warm._is_stale_warm_spec("codexcrew-mint-warm-notion", frozenset()) is True
    assert (
        warm._is_stale_warm_spec(
            "codexcrew-mint-warm-notion", frozenset({"codexcrew-mint-warm-notion"})
        )
        is False
    )
...
#
# Warm spec names are FIXED and predictable (``codexcrew-mint-warm-<alias>``), and they live in
...
def test_the_sweep_refuses_to_unlink_a_foreign_file_at_a_warm_spec_path(_agents_dir: Path):
    ...
    planted, body = _foreign_spec(_agents_dir, "codexcrew-mint-warm-notion")
...
def test_a_mimic_carrying_only_our_generic_defaults_survives_sweep_and_teardown(
    ...
    planted, body = _mimic_spec(_agents_dir, "codexcrew-mint-warm-notion")
...
def test_the_judge_reads_a_mimic_as_foreign_and_our_own_writer_output_as_ours(
    ...
    mimic, _ = _mimic_spec(_agents_dir, "codexcrew-mint-warm-mimic")
    assert warm._warm_spec_is_foreign(mimic) is True

    ours = _agents_dir / "codexcrew-mint-warm-ours.json"
    ours.write_text(
        json.dumps(warm._warm_spec_body("codexcrew-mint-warm-ours", {}, "probe")),
        encoding="utf-8",
    )
...
def test_a_dangling_symlink_at_the_spec_path_reads_as_foreign(_agents_dir: Path):
    ...
    away from it."""
    planted = _agents_dir / "codexcrew-mint-warm-notion.json"
...
def test_the_sweep_still_unlinks_a_stale_spec_a_warm_plan_did_write(_agents_dir: Path):
    ...
    stale = _agents_dir / "codexcrew-mint-warm-gone.json"
    stale.write_text(
        json.dumps(warm._warm_spec_body("codexcrew-mint-warm-gone", {}, "stale")),
        encoding="utf-8",
    )
...
def test_an_unreadable_file_at_a_warm_spec_path_is_left_alone(_agents_dir: Path):
    ...
    path = _agents_dir / "codexcrew-mint-warm-notion.json"
...
#
# ``_warm_spec_plan`` read ``codexcrew.json`` with a raw ``_load_json``, which follows a
...
# denial. #6736 migrated the other ``codexcrew.json`` readers; this one was added after.
...
async def _rearm_registry():
    ...
    :func:`~codex_crew.connections.warm._settle_invalidation_rearms` exists to close -- a test
