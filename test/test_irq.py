...
from codex_crew.irq import _dedupe_key as dedupe_key
from codex_crew.irq import (
    load_state,
)
from codex_crew.irq import poll as irq_poll
from codex_crew.irq import (
    run,
    sanitize_label,
    state_path,
...
class _FakeClock:
    ...
    Installed over the ``time`` name that :mod:`codex_crew.irq` reads (see
    ...
    def __getattr__(self, name: str) -> object:
        raise AttributeError(
            f"_FakeClock does not fake time.{name}; codex_crew.irq grew a clock "
...
def _deterministic_clock(monkeypatch):
    """Give ``codex_crew.irq`` a clock that only this module can move.

    The patch targets the module attribute ``codex_crew.irq.time`` -- the name
    ...
    monkeypatch.setattr("codex_crew.irq.time", _clock)
    return _clock
...
def test_unwritable_state_delivers_instead_of_swallowing_the_window():
    ...
    import codex_crew.irq as irq_mod
...
def test_a_kernel_that_returns_without_a_verdict_falls_back(monkeypatch):
    monkeypatch.setattr("codex_crew.irq.run", lambda *a, **k: None)
