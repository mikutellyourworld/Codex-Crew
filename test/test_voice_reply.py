...
from codex_crew.voice_reply import (
...
def _capture_mkstemp(monkeypatch) -> list[str]:
    ...
    monkeypatch.setattr("codex_crew.voice_reply.tempfile.mkstemp", recording_mkstemp)
    return allocated
...
_SANDBOX_REMEDY = (
    ...
    "agent.sandbox_allow_unsandboxed_exec=true in ~/.codex-crew/config.json."
)
...
def _patch_aws_on_path(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "codex_crew.voice_reply.shutil.which",
    ...
    monkeypatch.setattr("codex_crew.voice_reply.resolve_aws_bin", lambda: "aws")
...
def _no_argv_prefixers(monkeypatch):
    ...
    * ``create_subprocess_limited`` — prepends an RLIMIT shim that re-``exec``s
    ...
    """
    monkeypatch.setattr("codex_crew.voice_reply.cgroup_scope_argv", lambda argv: list(argv))
    monkeypatch.setattr(
        "codex_crew.voice_reply.create_subprocess_limited",
...
class TestIsAvailable:
    def test_polly_available_when_aws_on_path(self) -> None:
        with patch("codex_crew.voice_reply.shutil.which", return_value="/usr/bin/aws"):
            assert is_available(PROVIDER_POLLY) is True

    def test_polly_unavailable_when_aws_missing(self) -> None:
        with patch("codex_crew.voice_reply.shutil.which", return_value=None):
    ...
    def test_piper_unavailable_when_binary_missing(self, tmp_path) -> None:
        ...
        with patch(
            "codex_crew.voice_reply._resolve_piper_binary", return_value=None,
        ):
            assert is_available(
    ...
    def test_piper_unavailable_when_model_empty(self, tmp_path) -> None:
        ...
        with patch(
            "codex_crew.voice_reply._resolve_piper_binary", return_value=str(bin_path),
        ):
...
class TestResolvePollyCli:
    ...
    def test_resolved_absolutely_under_minimal_path(self, monkeypatch, tmp_path) -> None:
        ...
        from codex_crew import github_runner, voice_reply
    ...
    def test_none_when_cli_absent_everywhere(self, monkeypatch, tmp_path) -> None:
        ...
        from codex_crew import voice_reply
        from codex_crew.deploy import engine
...
class TestResolvePiperBinary:
    ...
    def test_falls_back_to_path(self, tmp_path) -> None:
        with patch(
            "codex_crew.voice_reply.shutil.which", return_value="/usr/local/bin/piper",
...
class TestSynthesizePiper:
    ...
    async def test_binary_not_found_returns_none(self) -> None:
        with patch("codex_crew.voice_reply._resolve_piper_binary", return_value=None):
    ...
    async def test_success_returns_wav_path(self, tmp_path) -> None:
        ...
        with patch(
            "codex_crew.voice_reply._resolve_piper_binary", return_value=str(bin_path),
        ), patch("codex_crew.voice_reply.wrap_argv", side_effect=fake_wrap), patch(
    ...
    async def test_nonzero_returncode_unlinks_and_returns_none(self, tmp_path) -> None:
        ...
        with patch(
            ...
            "codex_crew.voice_reply.wrap_argv", side_effect=lambda c, mode: (c, None),
        ), patch(
    ...
    async def test_sandbox_unavailable_returns_none_and_unlinks(
        ...
        monkeypatch.setattr("codex_crew.voice_reply.tempfile.mkstemp", tracking_mkstemp)

        def refuse(cmd, mode):
            raise SandboxUnavailableError(_SANDBOX_REMEDY, "no_backend", "not Linux")

        monkeypatch.setattr("codex_crew.voice_reply.wrap_argv", refuse)
        monkeypatch.setattr(
            "codex_crew.voice_reply._resolve_piper_binary", lambda *a, **k: str(bin_path)
        )

        with caplog.at_level("ERROR", logger="codex_crew.voice_reply"):
            assert await _synthesize_piper("hello", piper_model=str(model)) is None
    ...
    async def test_sandbox_cleanup_unlinked(self, tmp_path) -> None:
        ...
        with patch(
            ...
            "codex_crew.voice_reply.wrap_argv",
...
class TestSynthesizePolly:
    ...
    def _passthrough_sandbox(self, monkeypatch, _polly_consented):
        ...
        monkeypatch.setattr(
            "codex_crew.voice_reply.wrap_argv", lambda argv, **k: (list(argv), None)
        )
    ...
    async def test_applies_wrap_argv_sandbox(self, tmp_path) -> None:
        ...
        with patch(
            "codex_crew.voice_reply.wrap_argv", side_effect=fake_wrap,
        ), patch("asyncio.create_subprocess_exec", side_effect=fake_exec):
    ...
    async def test_aws_cli_missing_short_circuits_before_spawn(self, monkeypatch) -> None:
        ...
        """
        monkeypatch.setattr(
            "codex_crew.voice_reply.shutil.which", lambda name, *a, **k: None
        )
        spawned = {"n": 0}
...
class TestSynthesizeSpeechDispatcher:
    ...
    async def test_polly_dispatch(self) -> None:
        with patch(
            "codex_crew.voice_reply._synthesize_polly",
            new=AsyncMock(return_value="/tmp/out.mp3"),
        ) as mock_polly, patch(
            "codex_crew.voice_reply._synthesize_piper",
    ...
    async def test_redacts_credentials_before_synthesis(self) -> None:
        ...
        with patch(
            "codex_crew.voice_reply._synthesize_polly", side_effect=capture_polly,
        ):
...
class TestVoiceReplyEndToEnd:
    ...
    async def test_synthesis_fails_returns_false(self) -> None:
        client = MagicMock()
        with patch(
            "codex_crew.voice_reply.synthesize_speech",
            new=AsyncMock(return_value=None),
        ):
...
class TestStreamingVoiceReply:
    ...
    async def test_redacts_credentials_before_synthesis(self, tmp_path) -> None:
        from codex_crew.voice_reply import streaming_voice_reply
        ...
        with patch(
            "codex_crew.voice_reply._synthesize_polly", side_effect=fake_polly,
        ):
    ...
    async def test_skips_sentences_with_failed_synth(self, tmp_path) -> None:
        ...
        with patch(
            "codex_crew.voice_reply._synthesize_polly", side_effect=alternating,
        ):
            collected = []
...
def _pin_mkstemp(monkeypatch, owned) -> None:
    ...
    monkeypatch.setattr("codex_crew.voice_reply.tempfile.mkstemp", fake_mkstemp)
...
def _track_unlink(monkeypatch, owned, events: list[str]) -> None:
    ...
    monkeypatch.setattr("codex_crew.voice_reply.os.unlink", tracked)
...
class TestSynthesizePiperCancelOwnership:
    ...
    def _piper_env(tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.voice_reply._resolve_piper_binary", lambda cfg: str(bin_path)
        )
        monkeypatch.setattr(
            "codex_crew.voice_reply.wrap_argv", lambda c, mode: (list(c), None)
        )
        return model, owned
    ...
    async def test_locked_wav_does_not_replace_the_cancellation(
        ...
        monkeypatch.setattr("codex_crew.voice_reply.os.unlink", locked_unlink)
        with patch(
            "asyncio.create_subprocess_exec", return_value=_CancelOnceProc(events)
        ):
            with pytest.raises(asyncio.CancelledError):
