...
from codex_crew import hooks, mcp_grant
from codex_crew.connections import mint
from codex_crew.dashboard.handlers import connections
...
def _isolated_mint(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    ...
    cache_dir.mkdir()
    monkeypatch.setattr("codex_crew.agent.kiro_agents_dir_path", lambda: agents_dir)
...
def test_the_session_is_promptless_model_free_and_mounts_one_server():
    body = mint._mint_spec_body("codexcrew-mint-notion", {"notion": {"url": _URL}}, "desc")
...
async def test_the_mint_runs_on_a_dedicated_single_server_spec():
    ...
    assert agent.startswith(f"codexcrew-mint-notion-{os.getpid()}-")
    await mint._dispose_mint(mint._mints["notion"])
...
def test_cleanup_never_touches_a_file_it_did_not_record(
    ...
    lookalikes = [
        tmp_path / f"codexcrew-mint-notion-{os.getpid()}-abcdef12.json",
        tmp_path / "codexcrew-mint-notion-1234-deadbeef.json",
        tmp_path / "codexcrew-mint-notion.json",
    ]
...
def test_releasing_a_spec_drops_its_row(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    manifest = tmp_path / "mint-specs.json"
    monkeypatch.setattr(mint, "_mint_manifest_path", lambda: manifest)
    mine = tmp_path / f"codexcrew-mint-notion-{os.getpid()}-abcdef12.json"
...
def _mint_shaped(name_dir: Path, alias: str = "notion") -> Path:
    return name_dir / f"codexcrew-mint-{alias}-{os.getpid()}-abcdef12.json"
...
def test_a_failed_release_unlink_keeps_its_manifest_row(monkeypatch):
    ...
    spec = agents_dir / "codexcrew-mint-notion-4242-abcdef01.json"
...
def test_a_successful_release_drops_its_manifest_row():
    agents_dir = mint._agent.kiro_agents_dir_path()
    spec = agents_dir / "codexcrew-mint-notion-4243-abcdef02.json"
    spec.write_text("{}", encoding="utf-8")
    mint._write_mint_manifest({str(spec): time.time()})
...
def test_a_hard_stop_between_the_row_and_the_file_leaves_only_a_harmless_row(monkeypatch):
    ...
    def _die_on_the_spec(path, data, *a, **k):
        ...
        if Path(path).name.startswith("codexcrew-mint-"):
            raise _HardStop()
...
def test_a_row_survives_a_failed_unlink_so_a_later_sweep_retries(monkeypatch):
    ...
    agents_dir = mint._agent.kiro_agents_dir_path()
    orphan = agents_dir / "codexcrew-mint-notion-4242-abcdef01.json"
...
def test_a_spec_whose_row_cannot_be_recorded_is_not_left_behind(monkeypatch, tmp_path):
    ...
    assert not list(agents_dir.glob("codexcrew-mint-*.json"))
...
def test_a_spec_name_is_unique_per_flow(tmp_path: Path):
    ...
    assert first != second
    assert first.startswith(f"codexcrew-mint-notion-{os.getpid()}-")


def test_removal_deletes_only_the_exact_path_it_is_given(tmp_path: Path):
    mine = tmp_path / f"codexcrew-mint-notion-{os.getpid()}-abcdef12.json"
    sibling = tmp_path / f"codexcrew-mint-notion-{os.getpid() + 1}-beefcafe.json"
...
def test_removal_is_a_no_op_without_a_path(tmp_path: Path):
    handmade = tmp_path / "codexcrew-mint-notion.json"
...
def test_a_provider_absent_from_the_main_spec_falls_back_to_the_managed_agent(
    ...
    monkeypatch.setattr("codex_crew.agent.kiro_agents_dir_path", lambda: tmp_path)
    (tmp_path / "codexcrew.json").write_text(json.dumps({"mcpServers": {}}), encoding="utf-8")

    assert mint._write_mint_agent_spec("notion") == ("codexcrew", "")
...
def test_writing_a_spec_lands_a_uniquely_named_file(
    ...
    monkeypatch.setattr("codex_crew.agent.kiro_agents_dir_path", lambda: tmp_path)
    (tmp_path / "codexcrew.json").write_text(
        json.dumps({"mcpServers": {"notion": {"url": _URL}}}), encoding="utf-8"
    )

    name, path = mint._write_mint_agent_spec("notion")

    assert name.startswith(f"codexcrew-mint-notion-{os.getpid()}-")
...
def test_writing_refuses_to_overwrite_an_existing_path(
    ...
    monkeypatch.setattr(mint, "_mint_spec_name", lambda alias: "codexcrew-mint-fixed-1-aaaaaaaa")
    (tmp_path / "codexcrew-mint-fixed-1-aaaaaaaa.json").write_text("PRECIOUS", encoding="utf-8")

    with pytest.raises(FileExistsError):
        mint._write_mint_agent_spec("notion")

    assert (tmp_path / "codexcrew-mint-fixed-1-aaaaaaaa.json").read_text(
        encoding="utf-8"
    ) == "PRECIOUS"
...
async def test_the_unaudited_warning_names_the_key_not_the_url(
    ...
    with caplog.at_level(logging.WARNING, logger="codex_crew.mcp_grant"):
...
async def test_the_agents_dir_read_runs_off_the_loop(
    ...
    (_isolated_mint / "codexcrew.json").write_text(json.dumps({"mcpServers": {}}), encoding="utf-8")
    seen: list[int] = []
    monkeypatch.setattr(
...
async def test_the_data_home_resolution_runs_off_the_loop(monkeypatch: pytest.MonkeyPatch):
    # Resolving the data home CREATES it under a CODEXCREW_HOME override, so it is a
...
def test_the_handlers_package_does_not_import_the_mint_engine():
    ...
    """
    probe = (
        "import sys; import codex_crew.dashboard.handlers;"
...
def test_a_row_whose_parent_is_a_symlink_is_never_unlinked(tmp_path, monkeypatch):
    ...
    victim = agents_dir / "codexcrew-mint-notion-4242-abcdef01.json"
...
def test_a_relative_row_is_never_unlinked():
    ...
    assert mint._is_reapable_spec("codexcrew-mint-notion-4242-abcdef01.json") is False
...
def test_row_tokens_do_not_repeat_across_a_gateway_restart():
    ...
    probe = (
        ...
        # mint engine it defers. A cold `import codex_crew.connections.mint` trips a
        # pre-existing cycle in codex_crew.session_pid that production never hits.
        "import codex_crew.dashboard.handlers;"
        "from codex_crew.connections.mint import _new_mint_token as t; print(t())"
    )
