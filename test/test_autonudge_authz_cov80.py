"""Coverage for ``codex_crew.autonudge_authz`` guard clauses and audit fallbacks.
...
from codex_crew import autonudge_authz
from codex_crew.autonudge import MonitorUpdateConflict
from codex_crew.autonudge_authz import (
...
from codex_crew.monitoring.models import MAX_MONITOR_WAKE_INSTRUCTIONS_CHARS, MonitorState
...
async def test_add_denies_discord_when_the_transport_is_not_running(audits: list[dict]) -> None:
    ...
    loop, error, status = await authorize_and_add_nudge(
        ...
        slot_key="discord:codexcrew:direct:42",
        message="watch",
        source="dashboard",
    )
...
async def test_add_denies_webex_when_the_transport_is_not_running(audits: list[dict]) -> None:
    ...
    loop, error, status = await authorize_and_add_nudge(
        svc=svc,
        state=_state(transports={}),
        slot_key="webex:codexcrew:direct:user@example.com",
        message="watch",
        source="dashboard",
    )
...
async def test_add_denies_a_non_dm_webex_session(audits: list[dict]) -> None:
    ...
    loop, error, status = await authorize_and_add_nudge(
        ...
        slot_key="webex:codexcrew:forum:space:ROOM1",
        message="watch",
        source="dashboard",
    )
...
async def test_add_denies_a_webex_user_off_the_allowlist(audits: list[dict]) -> None:
    ...
    loop, error, status = await authorize_and_add_nudge(
        svc=svc,
        state=_state(transports={"webex": transport}),
        slot_key="webex:codexcrew:direct:intruder@example.com",
        message="watch",
        source="dashboard",
    )
...
async def test_add_denies_a_webex_key_that_is_not_the_users_current_session(
    ...
    transport = SimpleNamespace(
        dispatcher=SimpleNamespace(
            current_session_key=lambda _e: "webex:codexcrew:direct:user@example.com:gen3"
        ),
        is_authorized=lambda _e: True,
    )
    loop, error, status = await authorize_and_add_nudge(
...
async def test_add_denies_a_non_dm_discord_session(audits: list[dict]) -> None:
    ...
    dispatcher = SimpleNamespace(
        ...
        current_session_key=lambda uid: "discord:codexcrew:guild:42",
    )
    loop, error, status = await authorize_and_add_nudge(
        svc=svc,
        state=_state(transports={"discord": SimpleNamespace(dispatcher=dispatcher)}),
        slot_key="discord:codexcrew:guild:42",
        message="watch",
        source="dashboard",
    )
    assert loop is None and status == 400 and "DM sessions only" in error
...
@pytest.mark.parametrize(
    ...
    [
        "telegram:9001",
        "whatsapp:codexcrew:direct:15550100",
        "unified:codexcrew",
