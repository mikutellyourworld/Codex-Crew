...
from codex_crew.dashboard.chat_compaction_notice import (
...
def _permit_governance():
    ...
    with (
        patch("codex_crew.dashboard.chat_compaction_notice.vet_and_audit", return_value=permit),
        patch("codex_crew.platform.governance_profiles.vet_and_audit", return_value=permit),
    ):
        yield
...
class TestSlackDelivery:
    ...
    async def test_governance_denial_blocks_the_post(self) -> None:
        ...
        with patch(
            "codex_crew.dashboard.chat_compaction_notice.vet_and_audit",
...
class TestTransportDelivery:
    async def test_sends_to_origin_conversation(self) -> None:
        ...
        with _permit_governance():
            await deliver_channel_compaction_notice(
                state, "discord:codexcrew:direct:u1", 93.0, success=True
            )
    ...
    async def test_unified_bucket_uses_resolved_channel_for_hint(self) -> None:
        ...
        with _permit_governance():
            await deliver_channel_compaction_notice(
                state, "unified:codexcrew:direct:u1", 90.0, success=False
            )
    ...
    async def test_a_mirror_only_conversation_still_gets_the_notice(self) -> None:
        ...
        with _permit_governance():
            await deliver_channel_compaction_notice(
                state, "telegram:codexcrew:direct:7", 91.0, success=False
            )
    ...
    async def test_origin_wins_when_both_are_bound(self) -> None:
        ...
        with _permit_governance():
            await deliver_channel_compaction_notice(
                state, "discord:codexcrew:direct:u1", 90.0, success=True
            )
...
class TestOriginLinkStore:
    ...
    def _manager(self, tmp_path):
        ...
        with patch("codex_crew.session_map.config_dir", return_value=tmp_path):
            return SessionManager(CodexCrewConfig())

    def test_round_trip(self, tmp_path) -> None:
        mgr = self._manager(tmp_path)
        link = ChannelLink("discord", channel_id="chan-9")
        mgr.set_origin_link("discord:codexcrew:direct:u1", link)
        assert mgr.get_origin_link("discord:codexcrew:direct:u1") == link
    ...
    def test_bounded_against_unevicted_growth(self, tmp_path) -> None:
        from codex_crew.session import _MAX_ORIGIN_LINKS
