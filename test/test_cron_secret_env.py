...
def cron_home(monkeypatch, tmp_path):
    ...
    ``<home>/.codexcrew/crons`` and the vault lives beside them, so the
    ...
    back to a per-test tmp dir — never the operator's real ``~/.codexcrew``,
    ...
    """
    real_home = Path.home()
    fallback = tmp_path / "codexcrew-home-fallback"

    def _dir() -> Path:
        home = Path.home()
        if home == real_home:
            return fallback
        return home / ".codexcrew"

    monkeypatch.setattr("codex_crew.cron_script.config_dir", _dir)
    return _dir
...
class TestValidateSecretEnvGrant:
    ...
    def test_protected_names_rejected(self, key):
        # A leading-underscore name (_CODEXCREW_*) fails the grammar check
...
class TestSecretEnvPrecheck:
    ...
    def test_resolves_from_vault(self, tmp_path):
        SecretVault(tmp_path / ".codexcrew").set_sync("slack-sandbox", "xoxb-123")
        script = _make_script(tmp_path, "def run(ctx): pass\n")
        spec = str(script) + ":run"
        with patch("pathlib.Path.home", return_value=tmp_path):
...
class TestRunCommandSandboxedSecretEnv:
    ...
    def _passthrough_sandbox(self, monkeypatch, posix_test_shell):
        monkeypatch.setattr("codex_crew.cron_script.wrap_argv", lambda argv, **k: (list(argv), None))
        monkeypatch.setattr(
...
class TestRunScriptSandboxedSecretEnv:
    ...
    def test_unsandboxed_host_refuses_granted_run(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.cron_script.wrap_argv", lambda argv, **k: (list(argv), None))
        SecretVault(tmp_path / ".codexcrew").set_sync("slack-sandbox", "xoxb-script")
        script = _make_script(tmp_path, "def run(ctx): pass\n")
        spec = str(script) + ":run"
    ...
    def test_granted_run_uses_strict_sandbox_mode(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.cron_script.wrap_argv", recording_wrap)
        SecretVault(tmp_path / ".codexcrew").set_sync("slack-sandbox", "xoxb-script")
        script = _make_script(tmp_path, "def run(ctx): pass\n")
        spec = str(script) + ":run"
        grant = {"MY_SANDBOX_TOKEN": "slack-sandbox"}
    ...
    def test_granted_launcher_lives_in_private_dir_and_strips_it(self, tmp_path, monkeypatch):
        ...
        assert Path(launcher_dir).name.startswith("codexcrew_cron_pin_")
    ...
    def test_granted_keys_stripped_from_descendant_env(self, tmp_path):
        ...
        script = _make_script(
            tmp_path,
            "import os\n"
            "from codex_crew.cron_script import Done, _clean_cron_env\n"
            "def run(ctx):\n"
...
class TestCronServiceSecretEnvGate:
    ...
    def test_refused_approval_leaves_active_grant_valid(self, tmp_path, cron_home):
        ...
        from codex_crew.cron_script import peek_grant_epoch
    ...
    def test_revoked_grant_cannot_be_replayed_from_store(self, tmp_path, cron_home):
        ...
        from codex_crew.cron_script import bump_grant_epoch
    ...
    def test_cleared_then_deleted_grant_cannot_be_replayed(self, tmp_path, cron_home):
        ...
        from codex_crew.cron_script import commit_grant_epoch, peek_grant_epoch
    ...
    def test_commit_grant_epoch_cas_refuses_stale(self, tmp_path, cron_home):
        ...
        from codex_crew.cron_script import bump_grant_epoch, commit_grant_epoch, peek_grant_epoch
    ...
    def test_failed_epoch_bump_aborts_deletion(self, tmp_path, cron_home):
        ...
        job, spec, pin = self._script_job(svc, cron_home, grant={"MY_TOKEN": "slack-sandbox"})
        svc.update_job(job.id, secret_env={"MY_TOKEN": "slack-sandbox"}, secret_env_pin=pin)
        with patch("codex_crew.cron_script._write_grant_epochs", side_effect=OSError("read-only")):
    ...
    def test_epoch_bumps_are_atomic_across_threads(self, tmp_path, cron_home):
        ...
        from codex_crew.cron_script import _grant_epoch, bump_grant_epoch
...
class TestInjectionDefenseInDepth:
    def test_protected_keys_skipped_at_injection(self):
        ...
        kept = _filter_grant_env({"_CODEXCREW_SECRET_FILE": "evil", "OK_TOKEN": "fine"})
    ...
    def test_granted_script_cannot_import_live_crons_sibling(self, tmp_path):
        ...
        with patch("pathlib.Path.home", return_value=tmp_path):
            ...
            with patch(
                ...
                SecretVault(tmp_path / ".codexcrew").set_sync("slack-sandbox", "xoxb-1")
                result = run_script_sandboxed(
                    spec,
                    "job1",
                    timeout=120,
...
class TestMcpSecretRequest:
    ...
    def _svc_and_job(self, cron_home=None, *, agent: bool = False):
        from codex_crew.config.loader import config_dir as loader_config_dir
    ...
    def test_request_records_pending_not_active(self, cron_home):
        from codex_crew import mcp_cron
...
class TestGrantEndpointPendingFlow:
    ...
    def _app_and_svc(self):
        ...
        from codex_crew.config.loader import config_dir as loader_config_dir
        from codex_crew.dashboard.handlers.cron import api_cron_secret_grant
    ...
    async def test_approve_promotes_pending_to_active(self, cron_home):
        ...
        with patch("codex_crew.dashboard.handlers.cron._sel"):
    ...
    async def test_approve_racing_bump_conflicts_and_kills_pin(self, cron_home):
        ...
        from codex_crew.config.loader import config_dir as loader_config_dir
        from codex_crew.cron_script import _secret_env_precheck, bump_grant_epoch
        ...
        with (
            patch("codex_crew.dashboard.handlers.cron._sel"),
    ...
    async def test_approve_epoch_commit_failure_keeps_pending(self, cron_home):
        ...
        with (
            patch("codex_crew.dashboard.handlers.cron._sel"),
            patch(
                "codex_crew.dashboard.handlers.cron.commit_grant_epoch",
    ...
    async def test_revoke_epoch_bump_failure_is_structured_503(self, cron_home):
        ...
        with (
            patch("codex_crew.dashboard.handlers.cron._sel"),
            patch(
                "codex_crew.dashboard.handlers.cron.bump_grant_epoch",
    ...
    async def test_approve_refused_when_audit_unwritable(self, cron_home):
        ...
        SecretVault(loader_config_dir()).set_sync("slack-sandbox", "xoxb-1")
        jid = self._request_pending(svc, cron_home)
        with patch("codex_crew.dashboard.handlers.cron._sel") as sel_mock:
    ...
    async def test_unreadable_store_is_structured_on_approve_and_deny(self, cron_home):
        ...
        from codex_crew.config.loader import config_dir as loader_config_dir
        from codex_crew.cron import CronStoreUnreadable
    ...
    def test_owner_view_grant_map_is_redacted(self):
        ...
        from codex_crew.dashboard.handlers.cron import _redacted_grant_map
...
class TestGrantBodySizeCap:
    ...
    def test_grant_cap_never_exceeds_preview_cap(self):
        from codex_crew.cron_script import _MAX_GRANTED_SCRIPT_BYTES
        from codex_crew.dashboard.handlers.cron import _SCRIPT_SOURCE_MAX_BYTES
...
class TestGrantValueScrubbing:
    ...
    def _run(self, tmp_path, body):
        SecretVault(tmp_path / ".codexcrew").set_sync("slack-sandbox", "xoxb-leakyvalue")
    ...
    def test_short_secret_is_scrubbed_too(self, tmp_path):
        ...
        SecretVault(tmp_path / ".codexcrew").set_sync("pin-secret", "7q1")
        script = _make_script(
            tmp_path,
            "import os\n"
            "def run(ctx):\n"
    ...
    def test_status_token_is_not_scrubbed_when_a_vault_value_equals_it(self, tmp_path):
        ...
        SecretVault(tmp_path / ".codexcrew").set_sync("word-secret", "done")
        script = _make_script(
            tmp_path,
            "import os\n"
            "from codex_crew.cron_script import Done\n"
