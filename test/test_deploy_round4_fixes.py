...
SCRIPTS_DIR = Path(__file__).parent.parent / "src" / "codex_crew" / "deploy" / "skills" / "artifact-deploy" / "scripts"
...
class TestTeardownProfileValidation:
    ...
    async def test_bad_region_rejected(self):
        ...
        from codex_crew.deploy.profiles import REGION_SPEC
        from codex_crew.validation import ValidationError, validate_field
...
class TestIAMOACPolicy:
    ...
    def test_oac_statement_no_tag_condition(self):
        ...
        from codex_crew.deploy.iam import policy_json
...
class TestDeployWebCompatRedirect:
    ...
    def _make_app() -> web.Application:
        ...
        from codex_crew.deploy.handlers import register_routes
        register_routes(app)
