...
class TestFormatSchedule:
    ...
    def test_cron_expr_human_readable(self, monkeypatch) -> None:
        from codex_crew.cron import CronSchedule, format_schedule

        monkeypatch.setattr(
            "codex_crew.cron.CodexCrewConfig.load",
    ...
    def test_at_timestamp_today(self, monkeypatch, _utc_tz) -> None:
        ...
        monkeypatch.setattr("codex_crew.cron.datetime", type("D", (datetime,), {
    ...
    def test_config_timezone_fallback(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.cron.CodexCrewConfig.load", staticmethod(_record_load))
        monkeypatch.setattr(
            "codex_crew.cron.published_config_timezone", lambda: "America/New_York"
        )
        s = CronSchedule(kind="cron", cron_expr="0 22 * * 1-5")
...
class TestTimezoneScheduling:
    ...
    def test_job_tz_empty_returns_utc(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.cron.published_config_timezone", lambda: "")
    ...
    def test_job_tz_never_loads_the_config_file(self, monkeypatch) -> None:
        ...
        ``CodexCrewConfig.load()`` here stats and validates ``config.json`` on
        ...
        monkeypatch.setattr("codex_crew.cron.CodexCrewConfig.load", staticmethod(_record_load))
        monkeypatch.setattr(
            "codex_crew.cron.published_config_timezone", lambda: "America/Toronto"
        )
    ...
    def test_get_local_tz_never_loads_the_config_file(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.cron.CodexCrewConfig.load", staticmethod(_record_load))
        monkeypatch.setattr("codex_crew.cron.published_config_timezone", lambda: "Asia/Tokyo")
