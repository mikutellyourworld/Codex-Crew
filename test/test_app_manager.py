"""Tests for codex_crew.apps.manager — App lifecycle management."""
...
def app_home(tmp_path, monkeypatch):
    """Set CODEXCREW_HOME to a temp directory for isolated testing."""
...
class TestUnportableAppName:
    ...
    def test_builtin_registration_refuses_it(self):
        from codex_crew.apps.manager import _validate_builtin_app
...
class TestUninstall:
    ...
    def test_install_rejects_unsafe_app_name(self, tmp_path, app_home, monkeypatch):
        ...
        sel_calls = []
        monkeypatch.setattr(
            "codex_crew.apps.manager.sel",
            lambda: type(
                "FakeSel", (), {"log_api_access": lambda self, **kw: sel_calls.append(kw)}
            )(),
        )
        monkeypatch.setattr(
            "codex_crew.apps.manager._check_path_safety",
            lambda name: False,
        )
        result = install_app(src)
        assert not result.ok
...
class TestAppAdmission:
    ...
    def test_enable_builtin_exempt_under_require_signature(self, tmp_path, app_home):
        ...
        from codex_crew.apps.manager import _write_installed
    ...
    def test_non_ascii_signature_is_clean_deny(self):
        ...
        from codex_crew.apps.admission import AppAdmissionPolicy, _signature_valid
        from codex_crew.apps.manifest import AppManifest
...
class TestInstalledApp:
    ...
    def test_migrate_managed_codexcrew(self):
        """Old managed='codexcrew' with no source → defaults to registry."""
        meta = InstalledApp.from_dict({"name": "old", "managed": "codexcrew"})
        assert meta.origin == "registry"
        assert meta.resources == "gateway"
        assert meta.lifecycle == "gateway"
        assert meta.schemaVersion == 2

    def test_migrate_managed_codexcrew_local_source(self):
        """Old managed='codexcrew' with filesystem source → origin='local'."""
        meta = InstalledApp.from_dict(
            {
                "name": "old",
                "managed": "codexcrew",
    ...
    def test_migrate_managed_codexcrew_registry_source(self):
        """Old managed='codexcrew' with registry: source → origin='registry'."""
        meta = InstalledApp.from_dict(
            {
                "name": "old",
                "managed": "codexcrew",
    ...
    def test_uninstall_locked_rejected(self, tmp_path, app_home):
        ...
        from codex_crew.apps.manager import register_builtin_apps
...
class TestCleanupMigratedBuiltin:
    ...
    def test_idempotent_already_gone(self, tmp_path, monkeypatch):
        ...
        from codex_crew.apps import manager
        from codex_crew.apps.manager import cleanup_migrated_builtin
...
class TestCopyAppTree:
    def test_symlink_escaping_source_root_omitted(self, tmp_path, app_home):
        ...
        from codex_crew.apps.manager import app_dir
    ...
    def test_lifecycle_lock_is_per_app(self):
        from codex_crew.apps.manager import app_lifecycle_lock
    ...
    def test_install_bookkeeping_failure_keeps_provisional_repository_provenance(
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.token_auth.write_app_secret",
    ...
    async def test_registry_context_is_task_local_across_to_thread(self):
        ...
        from codex_crew.apps.manager import _effective_source_repository
    ...
    def test_legacy_name_grant_cannot_update_to_repository_code(
        ...
        from codex_crew.apps.manager import update_app
        from codex_crew.config.loader import _invalidate_config_cache
    ...
    def test_update_preserves_data_and_secret(self, tmp_path, app_home):
        from codex_crew.apps.manager import app_dir, update_app
...
def _ship_test_builtin(monkeypatch, root, manifest_data):
    ...
    from codex_crew.apps import execution
...
class TestEnabledStateTellsUnreadableFromNotInstalled:
    ...
    def test_a_dangling_windows_junction_ancestor_is_unknown(self, app_home, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.apps.manager.is_link_or_junction",
...
class TestBootSkillReconcile:
    ...
    def test_reconcile_creates_missing_skill_symlinks(self, tmp_path, monkeypatch):
        ...
        from codex_crew.apps.bridges import reconcile_app_skills
...
class TestBuiltinSecretForMcpServers:
    ...
    def test_declares_backend_helper(self):
        from codex_crew.apps.manager import _app_declares_backend
        ...
        assert not _app_declares_backend(
            {"mcpServers": {"x": {"url": "http://127.0.0.1:5486/mcp"}}}
        )
...
class TestBuiltinDoesNotClobberUserInstall:
    ...
    def test_origin_and_lifecycle_survive(self, tmp_path, app_home, monkeypatch):
        ...
        """
        from codex_crew.apps.manager import _read_installed
    ...
    def test_helper_classifies_both_cases(self):
        from codex_crew.apps.manager import InstalledApp, _builtin_owns_install
...
class TestMalformedMcpUrlIsSkippedNotFatal:
    ...
    def test_malformed_urls_return_none_and_do_not_raise(self):
        from codex_crew.apps.manager import resolve_mcp_backend_url
...
class TestRegisterExternalPreservesServerProvenance:
    ...
    def _seed_registry_app(cls) -> None:
        from codex_crew.apps.manager import set_app_provenance
