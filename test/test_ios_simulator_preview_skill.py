...
1. **The launcher ships and is discoverable.** The skill is the only bundled
   skill outside ``codexcrew-dev/`` with a ``scripts/`` payload, so a packaging
...
class TestSkillFilesShip:
    ...
    def test_documented_launcher_path_resolves_via_skill_dir(self, skill_md: str) -> None:
        """The documented path must honor CODEXCREW_HOME (same convention as
        prepare-pr), not hardcode a home that a relocated install won't have."""
        assert '"${CODEXCREW_HOME:-$HOME/.codex-crew}/skills/ios-simulator-preview"' in skill_md
...
class TestReviewFindings:
    ...
    def test_state_dir_honors_codexcrew_home(self, launcher: str) -> None:
        """Hardcoding ~/.codex-crew makes a dev instance
        (CODEXCREW_HOME=~/.codexcrew-dev) share pidfile state with a production
        install, so one's `stop` reaches into the other's mirrors."""
        assert 'os.environ.get("CODEXCREW_HOME")' in launcher
    ...
    def test_vendor_cleanup_is_gated_on_local_ownership(self, launcher: str) -> None:
        ...
        CODEXCREW_HOME would tear down a live mirror another home owns for the
        same device."""
    ...
    def test_relative_data_home_is_refused_not_silently_resolved(self, launcher: str) -> None:
        ...
        in, so a relative CODEXCREW_HOME would resolve per-caller: a `start` from
    ...
    def test_relative_data_home_errors_at_runtime(self, tmp_path: Path) -> None:
        ...
        p = subprocess.run(
            ...
            env={**os.environ, "CODEXCREW_HOME": "relhome"},
        )
        assert p.returncode == 1
        assert "Traceback" not in p.stderr, p.stderr
    ...
    def test_tilde_data_home_is_expanded_at_runtime(self, tmp_path: Path) -> None:
        ...
        p = subprocess.run(
            ...
            env={**os.environ, "HOME": str(fake_home), "CODEXCREW_HOME": "~/crewhome"},
        )
        assert p.returncode == 0, p.stderr
