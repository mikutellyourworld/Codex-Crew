...
class TestCronAddChannelCapture:
    def test_cron_add_captures_channel_from_env(self, monkeypatch, tmp_path):
        """CODEXCREW_CHANNEL_ID env var is used as job channel."""
        monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path))
        monkeypatch.setenv("CODEXCREW_CHANNEL_ID", "C0ABC123")
    ...
    def test_cron_add_no_env_channel_is_none(self, monkeypatch, tmp_path):
        """Without CODEXCREW_CHANNEL_ID, job channel is None (DM fallback)."""
        monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path))
    ...
    def test_cron_respects_codexcrew_home(self, monkeypatch, tmp_path):
        """CronService uses CODEXCREW_HOME when set, not the default ~/.codexcrew."""
        monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path))
        monkeypatch.delenv("CODEXCREW_CHANNEL_ID", raising=False)
        ...
        # Job should be in tmp_path, not ~/.codexcrew
        crons_file = tmp_path / "crons.json"
        assert crons_file.exists(), "crons.json not written to CODEXCREW_HOME directory"
...
class TestCronAddPersistenceOwner:
    ...
    def test_cron_add_persists_all_fields_in_one_save(self, monkeypatch, tmp_path):
        monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path))
        monkeypatch.setenv("CODEXCREW_SESSION_KEY", "dashboard:slot9")
        monkeypatch.delenv("CODEXCREW_CHANNEL_ID", raising=False)
