...
from codex_crew import embeddings
from codex_crew.apps import registry as app_registry
...
# A *second* internal git host NOT in CodexCrew's Default trusted set, so the
...
_ENT_RUM_CONFIG = {
    ...
    "applicationId": "codexcrew-enterprise",
    "region": "us-east-1",
}
_ENT_FEATURE_APPS = [
    "enterprise-tickets",
...
class _EnterpriseRegistryPolicy:
    def public_git_hosts(self):
        # The CodexCrew Default trusted set PLUS an extra internal git host.
...
def test_edition_builtin_apps_discovered_from_manifest_sources(enterprise_ctx) -> None:
    from codex_crew.apps.manager import _edition_builtin_apps
...
def test_edition_bundled_app_names_reflect_enterprise(enterprise_ctx) -> None:
    from codex_crew.apps.manager import _edition_bundled_app_names
...
def test_context_redact_runs_enterprise_policy(enterprise_ctx) -> None:
    ...
    from codex_crew import agent, mcp_core
...
def test_register_acp_backends_called_once_at_boot(monkeypatch, manifest_dir) -> None:
    ...
    import codex_crew.platform.bootstrap as bootstrap
