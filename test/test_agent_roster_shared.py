...
from codex_crew.mcp_tools import spawn as spawn_tools
...
class TestHelperContract:
    ...
    def test_the_reserved_pair_is_excluded_by_default(self) -> None:
        ...
        shown, _ = sa.visible_agent_names(["codexcrew", "codexcrew-conductor", "scout"])
        assert shown == ["scout"]
    ...
    def test_an_empty_exclusion_lists_the_reserved_pair(self) -> None:
        ...
        shown, _ = sa.visible_agent_names(["codexcrew", "scout"], exclude=())
        assert shown == ["codexcrew", "scout"]
...
class TestRenderedStringsAreUnchanged:
    ...
    def test_the_refusal_reads_the_same(self) -> None:
        with patch.object(sa, "list_agents", return_value=_agents("codexcrew", "scout", "probe")):
    ...
    def test_the_empty_refusal_reads_the_same(self) -> None:
        with patch.object(sa, "list_agents", return_value=_agents("codexcrew")):
    ...
    def test_the_parameter_description_reads_the_same(self) -> None:
        with patch.object(
            spawn_tools.mcp_core, "list_agents", return_value=_agents("scout", "codexcrew", "probe")
        ):
    ...
    def test_spawn_list_still_lists_every_name_unbounded(self) -> None:
        ...
        with (
            patch.object(spawn_tools.mcp_core, "_get", return_value={"agents": []}),
            patch.object(
                spawn_tools.mcp_core, "list_agents", return_value=_agents("codexcrew", *many)
            ),
        ):
            out = spawn_tools.spawn_list("spawn_list", {})
        assert out.endswith("\nAvailable agents: " + ", ".join(["codexcrew", *many]))
...
class TestExclusionIsInheritedNotRespelled:
    def test_the_helper_default_is_the_shared_constant(self) -> None:
        ...
        assert sa.UNADVERTISED_AGENTS == frozenset(
            {"codexcrew", "codexcrew-conductor", "codexcrew-pipeline-conductor"}
        )
