...
from codex_crew.slack import events as events_mod
from codex_crew.slack.events import _publish_home_tab
...
class TestPublishHomeTabHappyPath:
    @pytest.mark.asyncio
    @patch("codex_crew.slack.events.is_yolo_mode", return_value=False)
    @patch("codex_crew.slack.events.format_schedule", return_value="every 5m")
    @patch(
        "codex_crew.sso_status.get_sso_status_line",
    ...
    async def test_builds_valid_home_view(self, _mw, _fmt, _yolo):
        ...
        blocks = view["blocks"]
        text = str(blocks)
        assert "Codex Crew Status" in text
    ...
    @pytest.mark.asyncio
    @patch("codex_crew.slack.events.is_yolo_mode", return_value=True)
    @patch("codex_crew.slack.events.format_schedule", return_value="daily")
    @patch(
        "codex_crew.sso_status.get_sso_status_line",
        new_callable=AsyncMock,
        return_value="*SSO:* ✅ 5.0h remaining",
...
class TestViewTypeAlwaysHome:
    @pytest.mark.asyncio
    @patch("codex_crew.slack.events.is_yolo_mode", return_value=False)
    @patch("codex_crew.slack.events.format_schedule", return_value="*")
...
class TestPublishHomeTabCapabilities:
    ...
    @patch("codex_crew.slack.events.is_yolo_mode", return_value=False)
    @patch(
        "codex_crew.slack.events.list_servers",
        return_value=[SimpleNamespace(name="builder-mcp"), SimpleNamespace(name="codexcrew-core")],
    )
    @patch(
        "codex_crew.slack.events._get_skills_loader",
    )
    @patch(
        "codex_crew.sso_status.get_sso_status_line",
        new_callable=AsyncMock,
    ...
    @pytest.mark.asyncio
    @patch("codex_crew.slack.events.is_yolo_mode", return_value=False)
    @patch("codex_crew.slack.events.list_servers", return_value=[])
    @patch("codex_crew.slack.events._get_skills_loader")
    @patch(
        "codex_crew.sso_status.get_sso_status_line",
        new_callable=AsyncMock,
        return_value="*SSO:* ✅ 5.0h remaining",
    ...
    @pytest.mark.asyncio
    @patch("codex_crew.slack.events.is_yolo_mode", return_value=False)
    @patch("codex_crew.slack.events.list_servers", side_effect=RuntimeError("boom"))
    @patch(
        "codex_crew.sso_status.get_sso_status_line",
        new_callable=AsyncMock,
        return_value="*SSO:* ✅ 5.0h remaining",
...
class TestPublishHomeTabUptime:
    ...
    @pytest.mark.asyncio
    @patch("codex_crew.slack.events.is_yolo_mode", return_value=False)
    @patch("codex_crew.slack.events.Stats")
    @patch("codex_crew.slack.events.list_servers", return_value=[])
    @patch("codex_crew.slack.events._get_skills_loader")
    @patch(
        "codex_crew.sso_status.get_sso_status_line",
...
class TestPublishHomeTabSessions:
    ...
    def _grant_home_tab_auth(self, monkeypatch):
        ...
        """
        monkeypatch.setattr("codex_crew.slack.events.is_owner", lambda _: True)
        monkeypatch.setattr(
            "codex_crew.slack.events.is_allowed_user", lambda _: True
        )
    ...
    async def test_section_header_always_rendered(self, _mw, _fmt, _yolo, tmp_path, monkeypatch):
        ...
        sess_dir.mkdir()
        monkeypatch.setattr("codex_crew.slack.sessions_view._SESSIONS_DIR", sess_dir)
    ...
    async def test_unauthorized_user_blocked_with_denied_audit(
        ...
        orch = _make_orch()
        with (
            patch("codex_crew.slack.events.is_owner", return_value=False),
            patch("codex_crew.slack.events.is_allowed_user", return_value=False),
            patch("codex_crew.slack.events.sel") as mock_sel,
        ):
            mock_sel.return_value.log_api_access = MagicMock()
