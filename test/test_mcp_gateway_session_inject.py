...
def test_injects_only_wrapped_stub_entries(tmp_path):
    overlay = _write_overlay(tmp_path, "codexcrew", {
        "pooled": _stub(),
        "unpooled": {"command": "npx", "args": ["-y", "srv"], "env": {"TOKEN": "s3cr3t"}},
    })
    out = pooled_session_servers(overlay, "codexcrew")
...
def test_unpooled_server_env_is_never_transmitted(tmp_path):
    ...
    assert pooled_session_servers(overlay, "codexcrew") == []
    assert "s3cr3t" not in json.dumps(pooled_session_servers(overlay, "codexcrew"))
...
def test_stub_name_is_preserved_so_it_shadows_the_spec_entry(tmp_path):
    ...
    overlay = _write_overlay(tmp_path, "codexcrew", {"builder-mcp": _stub()})
    (entry,) = pooled_session_servers(overlay, "codexcrew")
...
def test_entries_are_name_sorted_for_deterministic_params(tmp_path):
    ...
    assert [e["name"] for e in pooled_session_servers(overlay, "codexcrew")] == [
        "alpha", "mid", "zeta",
    ]
...
def test_marker_is_stripped_from_the_injected_element(tmp_path):
    ...
    overlay = _write_overlay(tmp_path, "codexcrew", {"pooled": _stub()})
    (entry,) = pooled_session_servers(overlay, "codexcrew")
...
def test_non_string_args_are_coerced(tmp_path):
    overlay = _write_overlay(tmp_path, "codexcrew", {"pooled": _stub(args=["ok", 7])})
    (entry,) = pooled_session_servers(overlay, "codexcrew")
...
def test_commandless_stub_does_not_suppress_others(tmp_path):
    ...
    assert [e["name"] for e in pooled_session_servers(overlay, "codexcrew")] == ["fine"]
...
def test_disabled_gateway_injects_nothing():
    assert pooled_session_servers(None, "codexcrew") == []
...
def test_absent_overlay_spec_is_not_an_error(tmp_path):
    assert pooled_session_servers(tmp_path / "nope", "codexcrew") == []


def test_corrupt_overlay_degrades_to_unpooled(tmp_path):
    overlay = tmp_path / "agents"
    overlay.mkdir()
    (overlay / "codexcrew.json").write_text("{not json", encoding="utf-8")
    assert pooled_session_servers(overlay, "codexcrew") == []
...
def test_malformed_spec_shapes_degrade_to_unpooled(tmp_path, body):
    ...
    (overlay / "codexcrew.json").write_text(body, encoding="utf-8")
    assert pooled_session_servers(overlay, "codexcrew") == []


def test_non_dict_server_entry_is_skipped(tmp_path):
    overlay = _write_overlay(tmp_path, "codexcrew", {"bad": "x", "good": _stub()})
    assert [e["name"] for e in pooled_session_servers(overlay, "codexcrew")] == ["good"]
...
for line in sys.stdin:
    ...
    if request.get("method") == "initialize":
        ...
        result = {
            ...
            "capabilities": {},
            "serverInfo": {"name": "codexcrew-precedence-probe", "version": "1"},
        }
