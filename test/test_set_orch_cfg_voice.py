...
from codex_crew.slack import handler as handler_mod
from codex_crew.slack.handler import _vc, load_voice_reply_config, set_orch_cfg
...
def test_provider_typo_falls_back_to_local_with_warning(tmp_path, monkeypatch, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="codex_crew.slack.handler"):
        set_orch_cfg(SimpleNamespace())
    assert _vc.provider == "piper"
    assert any(
