...
class TestMcpMatcher:
    def test_server_grant_covers_all_tools(self):
        r = ScopedRuleset(mode=MODE_DENY, deny=("@codexcrew-cron",), matcher="mcp")
        assert not r.permits("@codexcrew-cron/cron_add").permitted
        assert not r.permits("@codexcrew-cron").permitted
        assert r.permits("@codexcrew-core/spawn_run").permitted

    def test_tool_level_deny_is_specific(self):
        r = ScopedRuleset(mode=MODE_DENY, deny=("@codexcrew-cron/cron_remove_all",), matcher="mcp")
        assert not r.permits("@codexcrew-cron/cron_remove_all").permitted
        assert r.permits("@codexcrew-cron/cron_add").permitted

    def test_title_to_ref_conversion(self):
        assert mcp_title_to_ref("mcp__codexcrew-cron__cron_add") == "@codexcrew-cron/cron_add"
...
class TestLoader:
    def test_absent_returns_none(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "codex_crew.platform.governance._policy_home_path", lambda: _nope
        )
    ...
    def test_env_path_wins(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("CODEXCREW_SECURITY_POLICY", str(p))
    ...
    def test_unreadable_env_fails_closed(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("CODEXCREW_SECURITY_POLICY", str(bad))
        with pytest.raises(PlatformCompositionError):
            load_security_policy()
    ...
    def test_home_path_used_when_no_env(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "codex_crew.platform.governance._policy_home_path", lambda: home
        )
        ceiling = load_security_policy()
        assert ceiling is not None
...
class TestConformanceVectors:
    ...
    def ceiling(self):
        return parse_policy(
            _policy_body(
                ...
                mcp={"mode": "deny", "deny": ["@codexcrew-cron/cron_remove_all"]},
    ...
    def test_e3_mcp_tool_deny_specific(self, ceiling, profile):
        assert not resolve(ceiling, profile, "mcp", "@codexcrew-cron/cron_remove_all").permitted
        assert resolve(ceiling, profile, "mcp", "@codexcrew-cron/cron_add").permitted
...
def _patch_trust(monkeypatch, *, require: bool, keys: dict):
    ...
    monkeypatch.setattr(
        "codex_crew.platform.governance._policy_trust_settings",
...
def _real_trust_file(monkeypatch, tmp_path, *, require: bool, keys: dict):
    ...
    monkeypatch.setenv("CODEXCREW_ADMISSION_POLICY", str(adm))
...
class TestPolicySigningPayload:
    ...
    def test_shares_admission_canonicalization(self):
        ...
        from codex_crew.platform.admission import canonical_signing_bytes
...
class TestPolicySignatureOptIn:
    ...
    def test_require_on_marks_governance_health_incident(self, monkeypatch, tmp_path):
        from codex_crew.platform import governance_health
    ...
    def test_home_tier_is_verified_too(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("codex_crew.platform.governance._policy_home_path", lambda: home)
    ...
    def test_bundled_tier_is_advisory_when_require_off(self, monkeypatch, tmp_path):
        ...
        monkeypatch.delenv("CODEXCREW_SECURITY_POLICY", raising=False)
        monkeypatch.setattr(
            "codex_crew.platform.governance._policy_home_path", lambda: tmp_path / "nope.json"
        )
        _patch_trust(monkeypatch, require=False, keys={})
...
class TestPolicySignatureAbsenceGate:
    ...
    def test_a_broken_trust_root_reads_as_no_optin_by_design(
        ...
        predictable, reported by ``codexcrew doctor``.
        """
        bad = tmp_path / "admission_policy.json"
        bad.write_text(shape)
        monkeypatch.setenv("CODEXCREW_ADMISSION_POLICY", str(bad))
        assert_policy_signature_satisfied(None)
        assert_policy_signature_satisfied(
            parse_policy(_policy_body(), signature_state=SIGNATURE_UNSIGNED)
        )
    ...
    def test_absent_admission_file_is_a_noop(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("CODEXCREW_ADMISSION_POLICY", str(tmp_path / "missing.json"))
        assert_policy_signature_satisfied(None)
        assert_policy_signature_satisfied(
            parse_policy(_policy_body(), signature_state=SIGNATURE_UNSIGNED)
        )
    ...
    def test_absent_admission_policy_keeps_verification_advisory(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "codex_crew.platform.admission._policy_default_path",
    ...
    def test_raising_trust_root_keeps_verification_advisory(self, monkeypatch, tmp_path):
        monkeypatch.setattr(
            "codex_crew.platform.admission.read_policy_trust_root",
    ...
    def test_trust_settings_read_from_admission_policy_file(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("CODEXCREW_ADMISSION_POLICY", str(adm))
        from codex_crew.platform.governance import _policy_trust_settings
...
class TestPolicyShowReporting:
    """`codexcrew policy show` must distinguish the three provenance states."""
...
class TestCapabilityOmissionIsUngoverned:
    ...
    ``codexcrew policy validate`` reporting a partially-governed block.
    """
...
class TestValidateReportsUngovernedCapabilities:
    """`codexcrew policy validate` must surface a partially-governed block.
