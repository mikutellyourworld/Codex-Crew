...
from codex_crew.dashboard.state import SlotOrigin
from codex_crew.dashboard.token_auth import app_token_path_allowed
from codex_crew.dashboard.ws_event_scope import (
...
def _clear_module_caches():
    ...
    """
    from codex_crew.dashboard import ws_event_scope as _wes
...
def _clear_ws_scope_module_caches(monkeypatch):
    ...
    """
    from codex_crew.dashboard import ws_event_scope as mod
...
class TestSlotScopedCrossApp:
    ...
    def test_cross_app_allowed_when_target_opts_in(self):
        ...
        with patch("codex_crew.dashboard.ws_event_scope.get_app_manifest", return_value=manifest):
            assert ws_event_allowed(
...
class TestGlobalEvents:
    ...
    def test_channel_settings_is_gated_by_owner_not_by_note_source(self):
        ...
        from codex_crew.dashboard.ws_event_scope import _SOURCE_FILTERED_EVENTS
    ...
    def test_channel_owner_helper_matches_the_handler_convention(self):
        ...
        from codex_crew.dashboard.ws_event_scope import notification_channel_owner
...
class TestImplicitAllow:
    ...
    def test_api_ws_implicit_allow_emits_no_exception(self):
        ...
        # at module load; patching codex_crew.sel.sel would not affect that binding).
        with patch(
            "codex_crew.dashboard.token_auth._sel_fn",
...
def test_app_token_path_allowed_implicit_ws() -> None:
    ...
    permissions.api declaration: every app that uses CodexCrewClient needs it to
...
class TestEmptyAppDeniedClosed:
    ...
    def test_empty_app_deny_emits_audit(self):
        ...
        from codex_crew.dashboard import ws_event_scope as _wes
        state = _make_state()
        with patch("codex_crew.sel.sel") as sel_mock:
...
class TestScopeCheckExceptionAudited:
    ...
    def test_scope_check_exception_emits_audit(self):
        ...
        with patch(
            "codex_crew.dashboard.ws_event_scope.ws_event_allowed",
            side_effect=RuntimeError("boom"),
        ), patch("codex_crew.sel.sel") as sel_mock:
...
class TestLiveScopeNarrowing:
    ...
    def test_log_subscriber_send_rechecks_the_live_scope(self):
        ...
        from codex_crew.dashboard import ws_event_scope as mod
        from codex_crew.dashboard.handlers import updates as upd
        from codex_crew.dashboard.state import DashboardState
    ...
    def test_source_guard_status_frame_withholds_checkout_identity(self):
        ...
        src = (
            ...
            / "src" / "codex_crew" / "dashboard" / "ws.py"
        ...
        state_src = (
            Path(__file__).resolve().parents[1]
            / "src" / "codex_crew" / "dashboard" / "state.py"
        ).read_text(encoding="utf-8")
...
class TestGrantsAreAudited:
    ...
    def test_repeated_grants_are_deduplicated(self):
        ...
        with patch("codex_crew.sel.sel") as sel_factory:
            for _ in range(5):
                ws_event_allowed(
                    "chat_delta", {"slot": "s1"},
                    app=self.APP, allowed_events=frozenset(), state=state,
...
class TestDisabledAppLosesTheOwnSlotDefault:
    ...
    def test_slots_repush_filter_drops_own_slot_when_disabled(self):
        ...
        from codex_crew.dashboard.ws_event_scope import filter_slots_for_app
        state = self._own_slot_state()
        rows = [{"key": "s1"}]
    ...
    def test_subagent_batch_filter_drops_own_items_when_disabled(self):
        ...
        from codex_crew.dashboard.ws_event_scope import filter_subagent_batch_for_app
        state = self._own_slot_state()
        items = [{"slot": "s1", "id": "a"}]
...
class TestExposeToCacheNeverBlocksTheLoop:
    ...
    def test_source_guard_connect_manifest_read_is_offloaded(self):
        ...
        """
        from codex_crew.dashboard import ws_event_scope as mod  # noqa: F401
        src = (
            Path(__file__).resolve().parents[1]
            / "src" / "codex_crew" / "dashboard" / "ws.py"
        ).read_text(encoding="utf-8")
        ...
        body = re.sub(r"from codex_crew[^)]*?\)", "", src, flags=re.S)
    ...
    def test_source_guard_manifest_loader_has_no_internal_cache(self):
        ...
        from codex_crew.apps import manager
...
class TestSubscribeHandlerGates:
    ...
    def _ws_source(self) -> str:
        import codex_crew.dashboard.ws as _ws
...
class TestExposeToAppsCache:
    ...
    def test_repeated_calls_hit_cache_and_do_not_reread_manifest(self):
        ...
        manifest.permissions.exposeToApps = ["monitor-app"]
        with patch(
            "codex_crew.dashboard.ws_event_scope.get_app_manifest",
    ...
    def test_disabled_target_exposes_to_nobody(self):
        ...
        with patch(
            ...
            "codex_crew.dashboard.ws_event_scope.is_app_enabled", return_value=False
        ):
            state = MagicMock()
...
class TestEventTableCompleteness:
    ...
    def test_fire_event_names_are_classified(self):
        ...
        loud error.
        """
        source_root = Path(__file__).resolve().parents[1] / "src" / "codex_crew"
    ...
    def _classified() -> frozenset[str]:
        from codex_crew.dashboard import ws_event_scope as m
    ...
    def test_every_broadcast_event_name_is_classified(self):
        ...
        root = Path(codex_crew.__file__).parent
...
class TestUntaggedOriginIsNotUser:
    ...
    def test_untagged_non_app_slot_is_not_user(self):
        from codex_crew.dashboard.state import SlotOrigin, request_slot_origin

        src = Path(
            __import__("codex_crew.dashboard.state", fromlist=["x"]).__file__
        ).read_text(encoding="utf-8")
    ...
    def test_every_handler_slot_creation_declares_an_origin(self):
        ...
        """
        import codex_crew.dashboard.chat_handlers as _ch
    ...
    def test_cron_injection_declares_cron(self):
        import codex_crew.dashboard.cron_inject as _ci
    ...
    def test_rehydrate_restores_the_persisted_origin(self):
        ...
        import codex_crew.dashboard.chat_persistence as _cp
...
class TestWildcardDeclaration:
    ...
    def test_wildcard_expands_instead_of_passing_through(self):
        from codex_crew.dashboard.ws_event_scope import build_allowed_event_set
...
class TestNotificationSourceParsing:
    ...
    def test_constants_match_the_real_emitters(self):
        ...
        from codex_crew.dashboard import ws_event_scope as wes
        from codex_crew.notifications import bus
        ...
        push = Path(
            __import__(
                "codex_crew.dashboard.handlers.notifications_push", fromlist=["x"]
            ).__file__
        ).read_text(encoding="utf-8")
        assert f'source=f"{wes._APP_SOURCE_PREFIX}{{app_name}}"' in push

    def test_parses_the_app_identity_out_of_the_prefix(self):
        from codex_crew.dashboard.ws_event_scope import notification_source_app
...
class TestAppEventFrames:
    ...
    def test_constant_matches_the_event_bus(self):
        ...
        from codex_crew.apps.event_bus import APP_EVENT_WS_TYPE
        from codex_crew.dashboard.ws_event_scope import _APP_EVENT_WS_TYPE
...
class TestApprovalResolvedCarriesSlot:
    ...
    def test_slot_level_resolution_carries_owning_slot(self):
        ...
        async def _run():
            ...
            with patch.object(state, "broadcast_ws") as bws, patch(
                "codex_crew.dashboard.state.sel"
            ):
    ...
    def test_every_approval_resolved_producer_sends_a_slot(self):
        ...
        roots = [
            Path(__file__).resolve().parents[1] / "src" / "codex_crew" / "dashboard" / f
            for f in (
...
class TestApiStatusRequiresDeclaration:
    def test_declared_app_still_reaches_api_status(self):
        ...
        with patch(
            "codex_crew.dashboard.token_auth._app_api_allowlist",
    ...
    def test_undeclared_app_is_denied_api_status(self):
        with patch(
            "codex_crew.dashboard.token_auth._app_api_allowlist", return_value=()
        ):
            assert app_token_path_allowed("mochi-pet", "/api/status") is False
...
class TestGlobalEventWithSlotDoesNotBypassScope:
    ...
    def test_slack_heartbeat_really_passes_a_slot_in_meta(self):
        ...
        gw = (
            Path(__file__).resolve().parents[1]
            / "src" / "codex_crew" / "slack" / "gateway.py"
        ).read_text(encoding="utf-8")
    ...
    def test_meta_merge_is_flat_so_slot_lands_top_level(self):
        ...
        bus = (
            Path(__file__).resolve().parents[1]
            / "src" / "codex_crew" / "notifications" / "bus.py"
        ).read_text(encoding="utf-8")
...
class TestSubagentBatchFrames:
    ...
    def test_batch_events_really_exist_with_per_item_slots(self):
        ...
        src = (
            Path(__file__).resolve().parents[1]
            / "src" / "codex_crew" / "subagent_scale.py"
        ).read_text(encoding="utf-8")
    ...
    def test_batch_item_key_map_matches_the_emitter(self):
        ...
        from codex_crew.dashboard.ws_event_scope import _SUBAGENT_BATCH_ITEM_KEY
...
class TestDirectSendGrantsAreAudited:
    ...
    def _drive_connect(self, monkeypatch, *, declared: list[str], yolo: bool):
        ...
        from codex_crew.dashboard import ws as dashboard_ws
        from codex_crew.dashboard import ws_event_scope
