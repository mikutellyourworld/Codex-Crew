...
from codex_crew import platform_compat, security
from codex_crew.hooks import validate_file_path
from codex_crew.platform import governance
from codex_crew.platform import governance_health as health
from codex_crew.platform import governance_profiles as gp
from codex_crew.platform import policy_distribution as pd
from codex_crew.platform.context import PlatformCompositionError, current_context

#: The import the boot tier must NOT perform on a host that configures no source.
_ENGINE_MODULE = "codex_crew.platform.policy_distribution"
...
def _hermetic_governance_globals(monkeypatch):
    ...
    """
    monkeypatch.delenv("CODEXCREW_SECURITY_POLICY", raising=False)
    monkeypatch.delenv("CODEXCREW_ADMISSION_POLICY", raising=False)
...
class TestTransport:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "file:///etc/codexcrew/policy.json",
        ],
    )
...
class TestLoadSecurityPolicyPrecedence:
    def test_the_local_env_file_outranks_a_configured_central_source(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("CODEXCREW_SECURITY_POLICY", str(local))
        monkeypatch.setenv(
...
class TestLiveRefresh:
    ...
    def test_a_304_adopts_a_cache_another_process_moved_ahead(self, transport, install_ceiling):
        ...
        an app backend's boot, and the ``codexcrew policy fetch`` the operator guide
...
_CACHE_PATHS = [
    ...
    for prefix in (".codex-crew", ".codexcrew")
    for leaf in (pd._CACHE_DOC_LEAF, pd._CACHE_META_LEAF)
]
...
class TestTierOneOutranksTheCentralSource:
    ...
    def test_a_local_policy_file_stops_the_refresh_before_it_fetches(
        ...
        pd._record_installed(_body("running", distribution={"source": source}))
        monkeypatch.setenv(
            "CODEXCREW_SECURITY_POLICY", str(_write_policy(tmp_path / "rollback.json", "rollback"))
        )
    ...
    def test_an_env_path_that_does_not_exist_is_not_tier_one(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("CODEXCREW_SECURITY_POLICY", str(tmp_path / "gone.json"))
...
class TestAFileSourceMustNotBeAgentWritable:
    ...
    def test_the_message_names_the_channel_designed_for_a_local_file(self, tmp_path):
        ...
        path = _write_policy(tmp_path / "policy.json", "published")
        path.chmod(0o644)
        with pytest.raises(PlatformCompositionError, match="CODEXCREW_SECURITY_POLICY"):
            pd.fetch_once(governance.PolicyDistribution(source=path.as_uri()))
...
class TestTheDetailNeverNamesTheEndpoint:
    """``codexcrew policy fetch`` is an ordinary shell command an agent can run.
    ...
    def test_the_log_never_carries_a_traceback_naming_the_source(self, caplog):
        ...
        with caplog.at_level(logging.DEBUG, logger="codex_crew.platform.policy_distribution"):
    ...
    def test_an_echoed_request_header_is_substituted_by_value(self, transport, monkeypatch):
        ...
        ``CODEXCREW_POLICY_HEADERS`` is deliberately arbitrary: an opaque ``X-Fleet-Key``
...
class TestTheHostnameAloneIsRedacted:
    ...
    def test_a_host_only_message_is_redacted(self, message):
        source = "https://config.corp.example/codexcrew/security_policy.json"
...
class TestEndpointAuthoredTextIsSanitised:
    ...
    error page — would carry that credential into ``codexcrew policy fetch`` and into
    ``GET /api/logs``.
    """
...
class TestTheCacheIsPublishedBeforeTheCeilingIsInstalled:
    def test_a_concurrent_child_cannot_read_the_retired_ceiling(self, transport, install_ceiling):
        ...
        import codex_crew.platform.context as ctx_mod
...
class TestTheSignatureAuditLabelIsRedacted:
    ...
    def test_the_agent_reachable_log_line_carries_neither_label_nor_reason(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="codex_crew.platform.governance"):
...
class TestTheFetchCommandDoesNotOverclaim:
    ...
    ``codexcrew policy fetch`` exits immediately — so a bare "applied" would claim
    ...
    def _run(capsys, *, interval, monkeypatch):
        from codex_crew import cli_commands
        from codex_crew.platform import policy_distribution as dist_mod
...
class TestAMaterialisedControlIsReDerivedAfterAnInstall:
    ...
    def test_a_failing_hook_is_reported_as_a_governance_incident(
        ...
        with caplog.at_level(logging.ERROR, logger="codex_crew.platform.policy_distribution"):
            assert pd.refresh_now().status == pd.REFRESH_APPLIED
...
class TestAllowedToolsIsReDerivedWhenTheCeilingTightens:
    ...
    short-circuits inside the harness and never reaches Codex Crew's own PreToolUse gate.
    """

    @staticmethod
    def _hook(monkeypatch):
        from codex_crew import agent as agent_mod
    ...
    def test_it_is_registered_as_a_post_install_hook(self):
        ...
        import codex_crew.slack.gateway as gw
...
class TestAPublishedSourceMayNotCarryACredential:
    ...
    was already that the per-machine credential travels in ``CODEXCREW_POLICY_HEADERS``,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "file:///etc/codexcrew/security_policy.json",
...
class TestAnUnconditional304IsNotSuccess:
    ...
    anyway has answered nothing. Reporting that as UNCHANGED made ``codexcrew policy fetch
...
class TestTheCacheIsHiddenFromAgentSubprocesses:
    ...
    def test_a_relocated_data_home_is_masked_by_its_resolved_path(self, monkeypatch, tmp_path):
        """Those entries are ``$HOME``-relative, and ``CODEXCREW_HOME`` moves the cache.
    ...
    def test_the_default_layout_adds_no_duplicate_rule(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("CODEXCREW_HOME", str(home / ".kiro" / "crew"))
    ...
    def test_a_symlinked_home_costs_a_redundant_rule_never_a_missing_one(
        ...
        monkeypatch.setenv("CODEXCREW_HOME", str(real / ".kiro" / "crew"))
        ...
        assert f".codex-crew/{pd.CACHE_DIR_LEAF}" in sandbox._STANDARD_DIRS
...
class TestAnExposedCacheIsStillReadOnly:
    ...
    def test_it_holds_for_every_spelling_the_dir_lists_carry(self, prefix):
        """Including the legacy ``~/.codexcrew`` entry the deny lists must keep covering."""
        from pathlib import Path as _Path
    ...
    def test_a_sibling_directory_is_not_mistaken_for_the_cache(self):
        ...
        assert not sandbox._is_policy_cache_dir("/home/u/.codex-crew")
        assert not sandbox._is_policy_cache_dir("/home/u/.codex-crew/policy_cache_backup")
        assert sandbox._is_policy_cache_dir("/home/u/.codex-crew/policy_cache/")
...
class TestPosture:
    ...
    def test_the_url_appears_nowhere_in_the_serialized_posture(self, install_ceiling):
        ...
        source = f"https://{host}/codexcrew/policy.json"
        install_ceiling(
            governance.parse_policy(
                _doc(
                    "running",
    ...
    def test_a_misconfigured_source_reports_a_code_and_still_no_url(self, monkeypatch):
        ...
        host = "config.corp.example"
        monkeypatch.setenv(pd.POLICY_URL_ENV, f"https://{host}/codexcrew/policy.json")
        monkeypatch.setenv(pd.POLICY_REFRESH_ENV, "15m")
        posture = pd.distribution_posture()
