...
from codex_crew.config.loader import ConfigReadError, read_config_for_update
...
class TestNoFailOpenConfigWriters:
    ...
    def test_no_except_handler_defaults_config_to_empty_dict(self):
        ...
        root = pathlib.Path(__file__).resolve().parents[1] / "src" / "codex_crew"
...
class TestWriteConfigAtomically:
    ...
    def test_preserves_existing_mode(self, tmp_path):
        ...
        from codex_crew.config.loader import write_config_atomically
    ...
    def test_does_not_spawn_a_subprocess_on_the_event_loop(self, tmp_path, monkeypatch):
        ...
        This function runs inside async request handlers and CodexCrewConfig.save(),
    ...
    def test_a_synchronous_caller_gets_the_dacl_on_a_non_local_volume(self, tmp_path, monkeypatch):
        # The CLI and startup paths (cli_setup, cli_chat, CodexCrewConfig.save from
    ...
    def test_the_predicate_answers_for_the_thread_that_actually_writes(self):
        ...
        """
        from codex_crew.atomic_write import on_event_loop
...
class TestEveryConfigWriterIsLocked:
    ...
    **What it does NOT cover.** Only calls to ``write_config_atomically``. A
    ...
    ``codex_crew.agent._atomic_json_write`` or :meth:`CodexCrewConfig.save`
    ...
    #: definition, and ``CodexCrewConfig.save`` is the head of the second family
