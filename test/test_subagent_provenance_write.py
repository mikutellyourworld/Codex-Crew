...
async def test_provenance_written_once_before_the_spawn_event() -> None:
    ...
    with (
        ...
        patch("codex_crew.subagent.update_state", side_effect=_spy_update),
...
async def test_provenance_write_retries_once_on_transient_failure() -> None:
    ...
    with (
        patch("codex_crew.subagent.Stats"),
        patch("codex_crew.subagent.sel"),
        patch("codex_crew.subagent.update_state", side_effect=_flaky_update),
        patch.object(manager, "_fire_event", _spy_fire),
    ):
        await manager._run_inner(info, f"subagent:{info.id}")
...
async def test_provenance_write_retries_on_silently_skipped_merge() -> None:
    ...
    with (
        patch("codex_crew.subagent.Stats"),
        patch("codex_crew.subagent.sel"),
        patch("codex_crew.subagent.update_state", side_effect=_skippy_update),
    ):
        await manager._run_inner(info, f"subagent:{info.id}")
...
async def test_per_turn_diagnostics_write_is_drained_on_cancellation() -> None:
    ...
    with (
        patch("codex_crew.subagent.Stats"),
        patch("codex_crew.subagent.sel"),
        patch("codex_crew.subagent.update_state", return_value=True),
        patch("codex_crew.subagent.asyncio.to_thread", side_effect=_gated_to_thread),
    ):
...
async def test_per_turn_diagnostics_drain_is_bounded() -> None:
    ...
    with (
        ...
        patch("codex_crew.subagent.sel"),
        patch("codex_crew.subagent.update_state", return_value=True),
        patch("codex_crew.subagent.asyncio.to_thread", side_effect=_wedged_to_thread),
        patch("codex_crew.subagent._STATE_DRAIN_TIMEOUT", 0.0),
    ):
        task = asyncio.ensure_future(manager._run_inner(info, f"subagent:{info.id}"))
        try:
            await entered.wait()
...
async def test_unpinned_spawn_records_requested_model_auto() -> None:
    ...
    with (
        ...
        patch("codex_crew.subagent._subagent_default_model", return_value=""),
    ):
        await manager._run_inner(info, f"subagent:{info.id}")
...
async def test_pid_and_session_records_are_written_off_loop() -> None:
    ...
    with (
        patch("codex_crew.subagent.Stats"),
        patch("codex_crew.subagent.sel"),
        patch("codex_crew.subagent.update_state", side_effect=_spy),
    ):
        await manager._run_inner(info, f"subagent:{info.id}")
...
async def test_shared_session_pid_write_is_drained_on_cancellation() -> None:
    ...
    with (
        patch("codex_crew.subagent.SubagentManager._get_parent_runtime", return_value=runtime),
        patch("codex_crew.subagent.AcpSessionProvider", MagicMock()),
        patch("codex_crew.subagent.update_state", return_value=True),
        patch("codex_crew.subagent.asyncio.to_thread", side_effect=_gated_to_thread),
    ):
