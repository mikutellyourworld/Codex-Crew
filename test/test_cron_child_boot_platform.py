...
from codex_crew.cron_script import run_script_sandboxed
from codex_crew.platform import PROFILE_ENTERPRISE
...
def _cron_home(tmp_path, monkeypatch):
    ...
    and ``config_dir()`` resolves from ``CODEXCREW_HOME`` -- not from
...
def _spawns_real_child(monkeypatch):
    ...
    import ``codex_crew`` when the checkout is not a packaged install.
    """
...
def _capture_launcher(script_path: str) -> str:
    ...
    with patch("codex_crew.cron_script.wrap_argv", _capture):
...
def _child_profile(monkeypatch, profile: str) -> None:
    ...
    Setting ``CODEXCREW_PROFILE`` on the test process itself would make the
...
class TestLauncherPreambleOrdering:
    ...
    def test_launcher_boots_the_platform(self, _cron_home):
        ...
        assert "from codex_crew.platform.bootstrap import boot_platform" in launcher
        assert "boot_platform(CodexCrewConfig.load())" in launcher
    ...
    def test_boot_is_after_syspath_strip_and_before_scriptcontext(self, _cron_home):
        ...
        boot = launcher.index("boot_platform(CodexCrewConfig.load())")
        script_context = launcher.index("from codex_crew.cron_script import ScriptContext")
...
class TestChildArrivesBooted:
    ...
    PROBE = (
        "import json\n"
        "def run(ctx):\n"
        "    from codex_crew.platform import bootstrap\n"
...
def test_launcher_is_profile_independent(_cron_home, monkeypatch, profile):
    ...
    assert "boot_platform(CodexCrewConfig.load())" in _capture_launcher(script_path)
