...
from codex_crew.apps.builtins.issue_radar.backend import gitlab_client as gl
...
def _isolate(monkeypatch, tmp_path):
    """No inherited state: CODEXCREW_HOME under tmp_path, no cached binary, no SEL."""
...
def test_allowed_hosts_fails_closed_on_a_broken_config(monkeypatch):
    ...
    monkeypatch.setattr(gl, "CodexCrewConfig", SimpleNamespace(load=_boom))
...
def test_glab_bin_override_is_validated_and_cached(monkeypatch):
    ...
    monkeypatch.setenv("CODEXCREW_ISSUE_RADAR_GLAB", "/opt/glab")
...
def test_glab_bin_override_failure_is_a_setup_error(monkeypatch):
    ...
    assert "CODEXCREW_ISSUE_RADAR_GLAB" in str(excinfo.value)
...
def test_glab_bin_skips_untrusted_candidates_and_reports_the_last_check(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("CODEXCREW_ISSUE_RADAR_GLAB", raising=False)
