...
from codex_crew.dashboard.handlers import api_browse_files
from codex_crew.dashboard.handlers.files import _browse_files_sync
...
class TestBrowseFiles:
    ...
    async def test_build_artifact_dirs_skipped(self, tmp_path, mock_sel):
        for d in ["node_modules", "__pycache__", ".cache", "build", "dist", ".next", ".codexcrew"]:
            (tmp_path / d).mkdir()
        (tmp_path / "src").mkdir()
        async with TestClient(TestServer(_make_app())) as client:
            resp = await client.get(f"/api/browse-files?path={tmp_path}")
    ...
    async def test_entry_unstattable_mtime_degrades_to_zero(self, tmp_path, mock_sel):
        ...
        with patch(
            "codex_crew.dashboard.handlers.files.os.scandir",
    ...
    async def test_scan_does_not_run_on_the_event_loop(self, tmp_path, mock_sel):
        ...
        with patch("codex_crew.dashboard.handlers.files._browse_files_sync", spy):
            async with TestClient(TestServer(_make_app())) as client:
                resp = await client.get(f"/api/browse-files?path={tmp_path}")
                assert resp.status == 200
                data = await resp.json()
