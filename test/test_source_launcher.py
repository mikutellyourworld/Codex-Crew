...
_SOURCE_LAUNCHER = _REPO_ROOT / "bin" / "codexcrew"
...
def _copy_launcher(tmp_path: Path) -> tuple[Path, Path]:
    ...
    install_root = tmp_path / "Codex Crew checkout"
    launcher = install_root / "bin" / "codexcrew"
...
def test_launcher_delegates_to_venv_without_rewriting_pythonpath(tmp_path: Path) -> None:
    ...
    venv_entry = install_root / ".venv" / "bin" / "codexcrew"
    ...
    env = os.environ.copy()
    env.pop("CODEXCREW_PROJECT_DIR", None)
    env.update(
        {
            "CODEXCREW_LAUNCH_CAPTURE": str(capture_path),
...
def test_launcher_without_venv_explains_source_install(tmp_path: Path) -> None:
    ...
    assert f"Codex Crew virtual environment not found at {install_root}/.venv" in result.stderr
