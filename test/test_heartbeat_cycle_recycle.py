...
from codex_crew.heartbeat import HeartbeatService, heartbeat_path
from codex_crew.session import HEARTBEAT_KEY


def _make_cfg():
    """Minimal CodexCrewConfig stub for SessionManager unit tests."""
    cfg = MagicMock()
...
def heartbeat_file(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.heartbeat.workspace_dir", lambda: tmp_path)
...
class TestOnCycleEndCallback:
    ...
    async def test_failure_log_carries_the_fallback_story(self, heartbeat_file, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="codex_crew.heartbeat"):
            await svc._process_heartbeat_file()
...
class TestRecycleHeartbeat:
    ...
    async def test_recycles_healthy_session(self):
        ...
        from codex_crew.session import FirstTurnState, SessionManager, _Session
...
class TestHeartbeatAgentInstall:
    """``_install_heartbeat_agent`` writes the codexcrew-heartbeat agent
    config with a minimal MCP surface (per code review)."""

    def test_installs_with_minimal_mcp_servers(self, tmp_path, monkeypatch):
        """Heartbeat agent gets codexcrew-core only on public installs — NOT
        codexcrew-cron / governance / etc.  This is the
        ...
        kiro_dir.mkdir()
        # Seed a main codexcrew.json with multiple mcp servers — the heartbeat
        # installer should pick out only the one it needs (codexcrew-core).
        main_config = {
            "name": "codexcrew",
            "mcpServers": {
                "builder-mcp": {"command": "/bin/builder-mcp", "args": ["x"]},
                "codexcrew-core": {"command": "/bin/mc", "args": ["mcp-core"]},
                "codexcrew-cron": {"command": "/bin/mc", "args": ["mcp-cron"]},
                "playwright-mcp": {"command": "/bin/playwright-mcp", "args": []},
            },
        }
        (kiro_dir / "codexcrew.json").write_text(json.dumps(main_config))

        monkeypatch.setattr(agent_mod, "KIRO_AGENTS_DIR", kiro_dir)

        agent_mod._install_heartbeat_agent()

        path = kiro_dir / "codexcrew-heartbeat.json"
        assert path.exists()
        config = json.loads(path.read_text(encoding="utf-8"))

        assert config["name"] == "codexcrew-heartbeat"
        # Minimal MCP surface — only codexcrew-core on public installs
        # (builder-mcp is Amazon-internal and omitted).
        assert set(config["mcpServers"].keys()) == {"codexcrew-core"}
    ...
    def test_strips_include_tools_filters_from_main_config(self, tmp_path, monkeypatch):
        """The main codexcrew config may narrow a server via ``--include-tools``
        ...
        ``HEARTBEAT_SAFE_TOOLS`` is the sole gate. (Asserted on codexcrew-core —
        ...
        config = json.loads((kiro_dir / "codexcrew-heartbeat.json").read_text(encoding="utf-8"))
        core_args = config["mcpServers"]["codexcrew-core"]["args"]
    ...
    def test_install_resilient_when_main_config_missing(self, tmp_path, monkeypatch):
        """First-run scenario: codexcrew.json may not exist yet when the
        ...
        # call will re-seed when codexcrew.json appears).
    ...
    def test_tools_derived_from_resolved_mcp_servers(self, tmp_path, monkeypatch):
        ...
        The public installer only pulls ``codexcrew-core``; the Amazon-internal
        ``builder-mcp`` is never carried over.  So a main config that has
        builder-mcp but NOT codexcrew-core resolves to an empty toolbelt — and
        ...
        kiro_dir = tmp_path / "agents"
        kiro_dir.mkdir()
        # Main config has builder-mcp but NOT codexcrew-core.
        main_config = {
            "name": "codexcrew",
            "mcpServers": {
        ...
        config = json.loads((kiro_dir / "codexcrew-heartbeat.json").read_text(encoding="utf-8"))
        # builder-mcp is omitted on public installs; codexcrew-core absent here.
        assert config["mcpServers"] == {}
