...
from codex_crew import publish_provider as pp
from codex_crew.publish import personal_drive as pd
from codex_crew.publish_provider import Capability, CapabilityNotSupportedError, PublishError
...
def test_drive_does_not_carry_the_deploy_site_tag(monkeypatch, payload):
    """The deploy engine groups sites by codexcrew:site and SKIPS resources without it.
...
def test_oversized_artifact_is_refused_with_its_size(monkeypatch, tmp_path):
    from codex_crew.artifacts import MAX_CONTENT_BYTES
...
def test_externally_disabled_is_reported_on_a_public_publish(monkeypatch, payload):
    ...
    assert "not a state Codex Crew sets" in res.notice
...
def test_the_drive_never_claims_the_site_name_a_user_would_destroy(monkeypatch, payload):
    """The engine hard-codes codexcrew:site on the create call and it can only be removed
...
def test_a_discovery_miss_is_not_confirmed_absence(monkeypatch):
    ...
    from codex_crew.publish_provider import DriveNotFound
...
def test_boot_does_not_import_the_publish_stack(monkeypatch):
    ...
    ``sys.modules`` entry. `codex_crew.publish` stays imported and keeps `personal_drive`
    bound on it, so `from codex_crew.publish import personal_drive` would resolve that
    ...
    import codex_crew.publish as publish_pkg
    from codex_crew.platform.defaults import DefaultPublishRegistry

    original = sys.modules.pop("codex_crew.publish.personal_drive", None)
    ...
    try:
        ...
        assert (
            "codex_crew.publish.personal_drive" not in sys.modules
        ), "bootstrap imported the provider module; the import must live inside the factory"
    finally:
        if original is not None:
            sys.modules["codex_crew.publish.personal_drive"] = original
        if had_attr:
...
def test_the_distribution_is_never_born_with_the_deploy_site_tag(monkeypatch, payload):
    """Carrying codexcrew:site the pooled distribution shows on the deploy site surface,
...
def test_the_artifact_store_is_never_imported_at_module_scope():
    """Ratchet, not decoration. ``codex_crew.artifacts`` resolves config while it is
    still importing, which installs the platform context, which registers this provider,
    which imports this module. A module-scope ``from codex_crew.artifacts import ...``
    ...
    offenders = [
        ...
        and (node.module or "").startswith("codex_crew.artifacts")
    ]
