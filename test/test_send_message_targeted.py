...
# Ensure codex_crew.slack.handler is importable for patching even when
# heavy transitive deps (cron_descriptor, etc.) aren't installed.
if "codex_crew.slack.handler" not in sys.modules:
    _stub = types.ModuleType("codex_crew.slack.handler")
    ...
    sys.modules["codex_crew.slack.handler"] = _stub

from codex_crew.dashboard.handlers import api_send_message, api_slack_profile  # noqa: E402
from codex_crew.messaging.link import ChannelLink  # noqa: E402
from codex_crew.telegram.client import TELEGRAM_MAX_TEXT  # noqa: E402
...
class TestTargetedChannel:
    ...
    async def test_channel_delivers_directly(self, mock_sel):
        ...
        with patch("codex_crew.slack.handler.is_tracked_channel", return_value=True):
    ...
    async def test_untracked_channel_returns_403(self, mock_sel):
        ...
        with patch("codex_crew.slack.handler.is_tracked_channel", return_value=False):
            async with TestClient(TestServer(app)) as client:
                resp = await client.post(
                    "/api/send-message",
...
class TestTargetedUser:
    ...
    async def test_user_opens_dm_and_delivers(self, mock_sel):
        ...
        with patch("codex_crew.slack.handler.is_allowed_user", return_value=True):
            async with TestClient(TestServer(app)) as client:
                resp = await client.post(
                    "/api/send-message",
    ...
    async def test_disallowed_user_returns_403(self, mock_sel):
        ...
        with patch("codex_crew.slack.handler.is_allowed_user", return_value=False):
            async with TestClient(TestServer(app)) as client:
                resp = await client.post(
                    "/api/send-message",
...
_TG_KEY = "telegram:codexcrew:dm:99887766"
...
class TestChannelTextIsNotTheNotificationText:
    ...
    async def test_session_closed_suffix_stays_on_the_bell(self, mock_sel):
        ...
        with (
            _governance(True),
            patch(
                "codex_crew.dashboard.handlers.messaging.rehydrate_slot_from_history_async",
                return_value=None,
            ),
        ):
            async with TestClient(TestServer(app)) as client:
...
def _call_tool(args):
    from codex_crew.mcp_tools.messaging import send_message
