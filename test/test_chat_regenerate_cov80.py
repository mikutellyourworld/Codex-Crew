...
:mod:`codex_crew.dashboard.chat_regenerate`.
...
from codex_crew.dashboard import chat_regenerate
...
async def test_readiness_latch_blocks_before_the_truncation(state) -> None:
    ...
    with patch(
        "codex_crew.dashboard.chat_regenerate.reject_if_kiro_unverified",
...
async def test_regenerate_survives_a_history_write_failure(state, caplog) -> None:
    ...
    with (
        patch(
            "codex_crew.dashboard.chat_regenerate._save_slot_to_history",
            side_effect=OSError("disk full"),
        ),
        patch("codex_crew.dashboard.chat_regenerate._run_chat", new=AsyncMock()),
    ):
...
async def test_unconsumed_variants_are_discarded_with_a_warning(state, caplog) -> None:
    ...
    with patch(
        "codex_crew.dashboard.chat_regenerate._run_chat", new=AsyncMock()
...
async def test_edit_resend_by_ts_truncates_and_resends(state) -> None:
    ...
    with patch("codex_crew.dashboard.chat_regenerate._run_chat", new=run):
        async with _client(state) as client:
            resp = await client.post(
...
async def test_edit_resend_redacts_the_edited_content(state) -> None:
    ...
    with patch("codex_crew.dashboard.chat_regenerate._run_chat", new=AsyncMock()) as run:
        async with _client(state) as client:
            resp = await client.post(
                "/api/chat/slots/s1/edit-resend",
...
async def test_edit_resend_rejects_when_the_history_save_raises(state, caplog) -> None:
    ...
    with (
        patch(
            "codex_crew.dashboard.chat_persistence._save_slot_to_history",
            side_effect=OSError("disk full"),
        ),
        patch("codex_crew.dashboard.chat_regenerate._run_chat", new=run),
    ):
        with caplog.at_level("WARNING"):
            async with _client(state) as client:
                resp = await client.post(
...
async def test_edit_resend_rejects_when_the_native_boundary_cannot_be_discarded(state) -> None:
    ...
    with (
        patch("codex_crew.dashboard.chat_persistence._save_slot_to_history") as save,
        patch("codex_crew.dashboard.chat_regenerate._run_chat", new=run),
    ):
        async with _client(state) as client:
            resp = await client.post(
...
async def test_edit_resend_discards_the_native_conversation_before_persisting(state) -> None:
    ...
    run = AsyncMock()
    with (
        patch("codex_crew.dashboard.chat_persistence._save_slot_to_history", side_effect=_save),
        patch("codex_crew.dashboard.chat_regenerate._run_chat", new=run),
    ):
        async with _client(state) as client:
            resp = await client.post(
...
async def test_edit_resend_logs_a_failing_background_turn(state, caplog) -> None:
    ...
    with patch(
        "codex_crew.dashboard.chat_regenerate._run_chat",
...
async def test_edit_resend_refusal_leaves_the_live_pending_and_cards_alone(state) -> None:
    ...
    with (
        patch("codex_crew.dashboard.chat_persistence._save_slot_to_history"),
        patch("codex_crew.dashboard.chat_regenerate._run_chat", new=AsyncMock()),
    ):
        async with _client(state) as client:
            resp = await client.post(
...
async def test_edit_resend_abort_hands_a_diverted_send_to_the_queue_drain(state) -> None:
    ...
    with (
        patch("codex_crew.dashboard.chat_regenerate._start_next_queued_turn", new=drain),
        patch("codex_crew.dashboard.chat_regenerate._run_chat", new=AsyncMock()) as run,
    ):
        async with _client(state) as client:
            resp = await client.post(
                "/api/chat/slots/s1/edit-resend", json={"index": 0, "content": "edited"}
