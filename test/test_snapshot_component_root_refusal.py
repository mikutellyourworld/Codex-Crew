...
class TestASelectiveBundleIsRefusedByOlderRestores:
    ...
    extracted root to start with `codexcrew-snapshot-`, so a different name turns silent
    ...
    def test_a_selective_bundle_root_is_named_partial(self, home, tmp_path):
        ...
        assert root.startswith("codexcrew-partial-"), root
        assert not root.startswith("codexcrew-snapshot-")
    ...
    def test_a_complete_bundle_keeps_the_original_root(self, home, tmp_path):
        ...
        assert root.startswith("codexcrew-snapshot-"), root
