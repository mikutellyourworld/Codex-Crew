...
from codex_crew.testing.fake_computer_use import FAKE_FILES_APP, FakeComputerUseBackend

# Every abstract method a driver must implement, as (name, args) pairs the
# ``UnsupportedBackend`` uniformity test drives.
_APP = AppRef(name="Probe", pid=1, bundle_id="dev.codexcrew.probe")
...
def test_package_import_is_side_effect_free():
    ...
    merely imports ``codex_crew``, breaking collection on the Linux fleet.
    """
...
class TestPlatformSelection:
    ...
    def test_macos_driver_import_failure_degrades_instead_of_raising(self, monkeypatch):
        ...
        def _fail(name, *args, **kwargs):
            if name == "codex_crew.computer_use.macos_driver":
