"""Tests for codex_crew.apps.hook_reconcile — reload app hooks on out-of-process CLI mutation.
...
import codex_crew.apps.hook_reconcile as hr
import codex_crew.apps.hooks_integration as hi
import codex_crew.apps.teardown as teardown


@pytest.fixture(autouse=True)
def _isolate_home(tmp_path, monkeypatch):
    """Pin CODEXCREW_HOME so hook_signature's stats resolve under a throwaway dir,
...
async def test_denied_app_teardown_runs_no_shutdown_hook(_harness):
    ...
    import codex_crew.apps.hook_reconcile as _hr
