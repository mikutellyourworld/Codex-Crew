...
from codex_crew.subagent import _TURN_LIMIT, SubagentManager
...
class TestSpawnWithoutApprovalCallback:
    ...
    async def test_spawn_denied_without_callback(self) -> None:
        ...
        # Act
        with patch("codex_crew.subagent.Stats"), patch("codex_crew.subagent.sel"):
...
class TestSpawnWithApprovalCallback:
    ...
    async def test_rejected_spawn_logs_sel_rejection(self) -> None:
        ...
        # Act
        with patch("codex_crew.subagent.Stats"), patch("codex_crew.subagent.sel") as mock_sel:
            info = manager.spawn("rejected task")
            assert info is not None
            await manager._tasks[info.id]
...
class TestSubagentReaper:
    ...
    async def test_reaper_kills_expired_subagent(self) -> None:
        ...
        from codex_crew.subagent import _TIMEOUT_SECS, SubagentInfo
    ...
    async def test_reaper_skips_completed_subagents(self) -> None:
        ...
        with patch("codex_crew.subagent.Stats"), patch("codex_crew.subagent.sel"), patch(
    ...
    async def test_reaper_handles_reset_timeout(self) -> None:
        ...
        with patch("codex_crew.subagent.Stats"), patch("codex_crew.subagent.sel"), patch(
            "codex_crew.subagent._RESET_TIMEOUT", 0.1
...
class TestConfigurableTimeout:
    ...
    async def test_default_timeout_fallback(self) -> None:
        ...
        from codex_crew.subagent import _TIMEOUT_SECS
...
class TestOnDoneTimeout:
    ...
    async def test_run_on_done_timeout_fires_injection_failed(self) -> None:
        ...
        with patch("codex_crew.subagent.Stats"), patch("codex_crew.subagent.sel"), patch(
            "codex_crew.subagent._ON_DONE_TIMEOUT", 0.1
        ):
...
class TestTimeoutContext:
    ...
    def test_basic_with_elapsed(self) -> None:
        from codex_crew.subagent import SubagentInfo, _timeout_context
    ...
    def test_redaction_called(self) -> None:
        ...
        with _patch("codex_crew.subagent._redact", return_value="[REDACTED]") as mock_redact:
            ctx = _timeout_context(info, include_elapsed=False)
...
class TestEffectiveTurnLimit:
    ...
    def test_falls_back_to_hardcoded_limit(self) -> None:
        from codex_crew.subagent import _TURN_LIMIT, SubagentInfo
...
class TestCheckMemoryAvailable:
    ...
    def test_sufficient_memory(self, tmp_path):
        ...
        from codex_crew.subagent import check_memory_available
    ...
    def test_sensitive_path_rejected(self, tmp_path):
        ...
        with patch("codex_crew.subagent.safe_read_file", side_effect=PermissionError("blocked")):
...
class TestSpawnMemoryGuard:
    ...
    def test_spawn_refused_low_memory(self):
        ...
        with patch("codex_crew.subagent.check_memory_available", return_value=(False, 2.5)), \
             patch("codex_crew.subagent.CodexCrewConfig") as mock_cfg, \
             patch("codex_crew.subagent.sel") as mock_sel:
...
class TestSpawnEmptyTaskGuard:
    ...
    def test_spawn_rejected_empty_task(self):
        ...
        mgr = self._mgr()
        with patch("codex_crew.subagent.sel") as mock_sel:
            mock_sel.return_value.log_tool_invocation = MagicMock()
    ...
    def test_spawn_rejection_survives_sel_failure(self):
        ...
        mgr = self._mgr()
        with patch("codex_crew.subagent.sel", side_effect=RuntimeError("sel down")):
            info = mgr.spawn(task="", parent_session_key="sess-1")
...
class TestCompletionKeepHelper:
    ...
    def test_short_text_unchanged(self) -> None:
        ...
        from codex_crew.context_management import apply_completion_keep
...
class TestSubagentManagerCompletionKeepWiring:
    ...
    async def test_default_completion_keep_is_head(self) -> None:
        ...
        from codex_crew.context_management import COMPLETION_KEEP_DEFAULT_CHARS
...
class TestCompletionKeepLoader:
    ...
    def test_load_unknown_completion_keep_raises(self) -> None:
        from codex_crew.config.loader import _validated_completion_keep
...
class TestSubagentUsageRow:
    ...
    async def test_subagent_turn_persists_usage_row_with_surface(self) -> None:
        from codex_crew.acp.types import AcpEvent, TurnUsage
        from codex_crew.providers.base import EVENT_COMPLETE
        from codex_crew.subagent import SubagentInfo
        ...
        persist = AsyncMock()
        with patch("codex_crew.subagent.Stats"), patch("codex_crew.subagent.sel"), patch(
            "codex_crew.dashboard.handlers.usage.persist_token_record_async", persist
        ), patch(
            "codex_crew.dashboard.handlers.usage.read_context_tokens",
    ...
    async def test_shared_runtime_agent_does_not_override_spawn_agent(self) -> None:
        ...
        with patch("codex_crew.subagent.Stats"), patch("codex_crew.subagent.sel"), patch(
            ...
            "codex_crew.dashboard.handlers.usage.read_effective_agent",
...
class TestIdentityTrustedChildParentPolicyAuto:
    ...
    def _manager_and_stream(self, event):
        from codex_crew.hooks import ToolHookResult
        from codex_crew.subagent import SubagentInfo, SubagentManager
    ...
    async def test_identity_trusted_child_approved_under_parent_policy_auto(self) -> None:
        ...
        with patch("codex_crew.subagent.Stats"), patch("codex_crew.subagent.sel"), patch(
            "codex_crew.subagent.update_state"
        ), patch("codex_crew.subagent.create_agent_folder", MagicMock(), create=True):
...
class TestChildEscalationLimit:
    ...
    async def test_escalation_limit_bail_answers_triggering_request(self) -> None:
        from codex_crew.hooks import TOOL_AUTO_APPROVE, ToolHookResult
        from codex_crew.providers.base import EVENT_PERMISSION_REQUEST, LLMEvent
        from codex_crew.subagent import SubagentInfo, SubagentManager
