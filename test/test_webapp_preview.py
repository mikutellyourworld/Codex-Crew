...
from codex_crew.dashboard.handlers import webapp_preview as wp
from codex_crew.deploy.webapp_types import webapp_metadata_from_dict
...
def env(tmp_path: Path, monkeypatch):
    ...
    (app_dir / "public" / ".codexcrew-deploy.json").write_text("{}")
...
class TestRound2Fixes:
    ...
    async def test_frame_ancestors_uses_self_plus_embedding_ancestors(self, env) -> None:
        ...
        resp = await wp.serve_artifact_app_file(
            _req(
                ...
                headers={"Host": "localhost:5486"},
            )
        )
        csp = resp.headers["Content-Security-Policy"]
        assert "frame-ancestors 'self'" in csp
        assert "localhost:5486" not in csp, "a rewritable Host reached the header"
    ...
    async def test_an_injecting_ancestor_cannot_reach_the_header(self, env, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            ...
            lambda request, app: ["http://localhost:5486; default-src *"],
        )
        store, app_dir = env
...
class TestRound3Fixes:
    ...
    def test_write_time_app_dir_validation(self) -> None:
        ...
        from codex_crew.validation import ValidationError, _validate_webapp_metadata_shape
