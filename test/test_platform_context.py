"""Tests for the Composed Platform Providers contract (codex_crew.platform)."""
...
from codex_crew.platform.context import PlatformContext
...
class TestDefaultContext:
    ...
    def test_build_default_context_is_standalone(self, cfg: CodexCrewConfig) -> None:
    ...
    def test_default_adapters_match_legacy_behavior(self, cfg: CodexCrewConfig) -> None:
        ctx = build_default_context(cfg)
        # Each Default* adapter reproduces today's module-level value.
        from codex_crew import agent, embeddings, sandbox
        from codex_crew.apps import registry
    ...
    def test_default_security_is_baseline_only(self, cfg: CodexCrewConfig) -> None:
        ctx = build_default_context(cfg)
        assert isinstance(ctx.security, PolicyAuthority)
        assert set(ctx.security.effective_patterns()) == set(BASELINE_DENY)

    def test_default_credential_redaction_delegates(self, cfg: CodexCrewConfig) -> None:
        ctx = build_default_context(cfg)
...
class TestProfileResolution:
    def test_env_override_standalone(self, cfg: CodexCrewConfig, monkeypatch) -> None:
        monkeypatch.setenv("CODEXCREW_PROFILE", "standalone")
        assert resolve_profile(cfg, entry_points=[object()]) == PROFILE_STANDALONE

    def test_env_override_enterprise(self, cfg: CodexCrewConfig, monkeypatch) -> None:
        monkeypatch.setenv("CODEXCREW_PROFILE", "enterprise")
        assert resolve_profile(cfg, entry_points=[]) == PROFILE_ENTERPRISE

    def test_env_override_legacy_alias(self, cfg: CodexCrewConfig, monkeypatch) -> None:
        # A legacy edition value still resolves to the enterprise profile
        # (back-compat alias) rather than falling back to standalone.
        monkeypatch.setenv("CODEXCREW_PROFILE", "amazon")
        assert resolve_profile(cfg, entry_points=[]) == PROFILE_ENTERPRISE

    def test_unknown_env_falls_back_to_standalone(self, cfg: CodexCrewConfig, monkeypatch) -> None:
        # An unknown CODEXCREW_PROFILE value returns standalone immediately,
        # before any identity/entry-point signal is consulted.
        monkeypatch.setenv("CODEXCREW_PROFILE", "bogus")
        assert resolve_profile(cfg, entry_points=[]) == PROFILE_STANDALONE

    def test_entry_points_take_precedence_over_marker(
        self, cfg: CodexCrewConfig, monkeypatch
    ) -> None:
        ...
        monkeypatch.delenv("CODEXCREW_PROFILE", raising=False)
    ...
    def test_marker_stat_ignored_without_probe_optin(
        ...
        monkeypatch.delenv("CODEXCREW_PROFILE", raising=False)
        monkeypatch.delenv("CODEXCREW_SSO_PROFILE_PROBE", raising=False)
        monkeypatch.setattr("codex_crew.platform.profile.Path.home", lambda: _FakeHome(True))
        assert resolve_profile(cfg, entry_points=[]) == PROFILE_STANDALONE
    ...
    def test_marker_stat_triggers_enterprise_when_probe_opted_in(
        ...
        monkeypatch.delenv("CODEXCREW_PROFILE", raising=False)
        monkeypatch.setenv("CODEXCREW_SSO_PROFILE_PROBE", "1")
        monkeypatch.setattr("codex_crew.platform.profile.Path.home", lambda: _FakeHome(True))
        assert resolve_profile(cfg, entry_points=[]) == PROFILE_ENTERPRISE
    ...
    def test_legacy_probe_env_still_triggers_enterprise(
        ...
        monkeypatch.delenv("CODEXCREW_PROFILE", raising=False)
        monkeypatch.delenv("CODEXCREW_SSO_PROFILE_PROBE", raising=False)
        monkeypatch.setenv("CODEXCREW_MIDWAY_PROFILE_PROBE", "1")
        monkeypatch.setattr("codex_crew.platform.profile.Path.home", lambda: _FakeHome(True))
        assert resolve_profile(cfg, entry_points=[]) == PROFILE_ENTERPRISE

    def test_no_signals_is_standalone(self, cfg: CodexCrewConfig, monkeypatch) -> None:
        monkeypatch.delenv("CODEXCREW_PROFILE", raising=False)
        monkeypatch.setenv("CODEXCREW_SSO_PROFILE_PROBE", "1")
        monkeypatch.setattr("codex_crew.platform.profile.Path.home", lambda: _FakeHome(False))
        assert resolve_profile(cfg, entry_points=[]) == PROFILE_STANDALONE
...
class TestBootstrapAndDiscovery:
    def test_bootstrap_standalone(self, cfg: CodexCrewConfig, monkeypatch) -> None:
        monkeypatch.setenv("CODEXCREW_PROFILE", "standalone")
    ...
    def test_bootstrap_enterprise_without_companion_fails_closed(
        ...
        monkeypatch.setattr("codex_crew.platform.bootstrap.plugin_entry_points", lambda: [])
        monkeypatch.setattr("codex_crew.platform.discovery.plugin_entry_points", lambda: [])
        with pytest.raises(PlatformCompositionError):
            bootstrap_context(cfg)

    def test_contract_version_mismatch_rejected(self, cfg: CodexCrewConfig, monkeypatch) -> None:
        import dataclasses

        from codex_crew.platform import bootstrap as bootstrap_mod
...
class TestRedactLogViaContext:
    ...
    @staticmethod
    def _install(policy, cfg: CodexCrewConfig):
        import dataclasses

        from codex_crew.platform import set_context
    ...
    def _restore_context(self):
        ...
        from codex_crew.platform import reset_context

        yield
        reset_context()

    def test_companion_shape_is_scrubbed_on_top_of_the_baseline(self, cfg: CodexCrewConfig) -> None:
        from codex_crew.platform.context import redact_log_via_context
    ...
    def test_converged_log_sites_do_not_drift_back_to_the_baseline(self) -> None:
        ...
        root = pathlib.Path(codex_crew.__file__).parent
