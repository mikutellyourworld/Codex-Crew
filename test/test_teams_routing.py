...
from codex_crew.teams.client import TeamsClient, TeamsSendError
...
class TestTheCredentialOnlyGoesToConnectorHosts:
    ...
    def test_the_predicate_is_deny_by_default(self, url: str, allowed: bool) -> None:
        from codex_crew.teams.client import connector_host_allowed
...
class TestTheRoutingStoreIsOnTheKeystoneFloor:
    ...
    def test_the_store_path_is_sensitive_both_ways(self) -> None:
        ...
        from codex_crew.teams.service_urls import STORE_DIRNAME, STORE_FILENAME
    ...
    def test_the_default_path_is_the_one_the_leaf_registers(self, tmp_path, monkeypatch) -> None:
        ...
        """
        from codex_crew.security import is_sensitive_path
        from codex_crew.teams.service_urls import ServiceUrlStore
        ...
        assert is_sensitive_path(str(Path.home() / ".codex-crew" / store.path().parent.name))
    ...
    def test_the_atomic_write_temp_sibling_is_covered_too(self) -> None:
        ...
        from codex_crew.security import is_sensitive_path
        from codex_crew.teams.service_urls import STORE_DIRNAME

        temp = Path.home() / ".codex-crew" / STORE_DIRNAME / "tmpAb3Kd9Zq.tmp"
    ...
    def test_a_shell_write_to_the_store_is_refused(self) -> None:
        ...
        target = f"~/.codex-crew/{STORE_DIRNAME}/{STORE_FILENAME}"
        for command in (
            f"cat {target}",
            f"echo '{{}}' > {target}",
            f"tee {target}",
            # The temp sibling the rename would publish.
            f"echo '{{}}' > ~/.codex-crew/{STORE_DIRNAME}/tmpAb3Kd9Zq.tmp",
        ):
