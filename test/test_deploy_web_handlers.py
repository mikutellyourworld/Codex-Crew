...
from codex_crew.deploy import engine, handlers
from codex_crew.deploy import profiles as profiles_mod
...
def _isolate_config(tmp_path: Path, monkeypatch):
    ...
    import codex_crew.deploy as _deploy_pkg
    from codex_crew.deploy import pending as _pending_mod
    from codex_crew.deploy import profiles as _profiles_mod
...
def _mock_base_stack(monkeypatch):
    ...
    _base_outputs = _json.dumps([{"OutputKey": "BucketName", "OutputValue": "codexcrew-base-bucket"}])
...
def test_deploy_info_finding_overridable(monkeypatch):
    ...
    def fake_deploy(site_id, src_dir, profile, region):
        ...
        return {"site_id": site_id, "url": "https://d.cloudfront.net/", "reused": False,
                "bucket": "codexcrew-web-x", "distribution_id": "D1", "status": "InProgress"}
...
def test_deploy_missing_artifact_404(monkeypatch):
    _set_profile(monkeypatch)
    from codex_crew.artifacts import ArtifactNotFoundError
...
def test_recall_preview_then_confirm(monkeypatch):
    _set_profile(monkeypatch)
    site = {"bucket": "codexcrew-web-x", "distribution_id": "D1", "distribution_arn": "arn"}
...
def test_destroy_preview_echoes_resources(monkeypatch):
    _set_profile(monkeypatch)
    site = {"bucket": "codexcrew-web-x", "distribution_id": "D1"}
    monkeypatch.setattr(engine, "find_site_by_tag", lambda sid, p, r=None: site)
    ...
    assert "codexcrew-web-x" in payload["message"] and "D1" in payload["message"]
...
def test_expire_manifest_uses_art_slug_not_meta_slug(monkeypatch):
    ...
    for cmd in s3_cmd:
        ...
        assert "real-slug/.codexcrew-deploy.json" in s3_args_str
...
def test_deploy_rejects_on_windows(monkeypatch):
    ...
    from codex_crew.deploy import handlers
...
def test_teardown_reaper_present_tombstones(monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.dashboard.handlers.artifacts._serialize",
...
def test_do_deploy_writes_manifest_with_ttl(tmp_path, monkeypatch):
    """After successful deploy, _do_deploy writes .codexcrew-deploy.json with TTL."""
    ...
    manifest_calls = [c for c in captured_s3_calls if ".codexcrew-deploy.json" in " ".join(c)]
    assert len(manifest_calls) == 1
    assert "s3://test-bucket/my-ttl-app/.codexcrew-deploy.json" in " ".join(manifest_calls[0])
...
def test_deploy_manifest_written_to_base_stack_bucket(monkeypatch):
    ...
    def fake_deploy(site_id, src_dir, profile, region):
        return {"site_id": site_id, "url": "https://d.cloudfront.net/", "reused": False,
                "bucket": "codexcrew-web-random123", "distribution_id": "D1", "status": "Deployed"}

    def fake_run_aws(cmd, profile, timeout=15):
        import json as _j
        if "describe-stacks" in cmd and "codexcrew-deploy-base" in cmd:
            return (0, _j.dumps([
    ...
    s3_dest = s3_calls[0][3]  # "s3://shared-base-bucket/mysite/.codexcrew-deploy.json"
    assert "shared-base-bucket" in s3_dest
    assert "codexcrew-web-random123" not in s3_dest
    assert "mysite/.codexcrew-deploy.json" in s3_dest
...
def test_deploy_manifest_includes_engine_arch_fields(monkeypatch, tmp_path):
    ...
    def fake_deploy(site_id, src_dir, profile, region):
        return {"site_id": site_id, "bucket": "codexcrew-web-abc123",
                "distribution_id": "E1234XYZ", "url": "https://d.cf.net/", "status": "Deployed"}

    def fake_run_aws(args, profile, timeout=30):
        # Capture the manifest write
        if args[:2] == ["s3", "cp"] and ".codexcrew-deploy.json" in str(args):
            import json as _json
    ...
    assert man["bucket"] == "codexcrew-web-abc123"
...
def test_reaper_engine_arch_happy_path(monkeypatch):
    ...
    reaper_path = str(Path(__file__).resolve().parent.parent /
                      "src/codex_crew/deploy/skills/artifact-deploy/scripts/reaper_lambda")
    ...
    mock_cf.list_tags_for_resource.return_value = {
        "Tags": {"Items": [{"Key": "codexcrew:site", "Value": "mysite"}]}
    }
    mock_s3.get_bucket_tagging.return_value = {
        "TagSet": [{"Key": "codexcrew:site", "Value": "mysite"}]
    }
    ...
    mock_s3.delete_bucket.assert_called_once_with(
        Bucket="codexcrew-web-mysite", ExpectedBucketOwner="123456789012"
    )
...
def test_reaper_engine_arch_distribution_not_disabled_retries(monkeypatch):
    ...
    mock_cf.list_tags_for_resource.return_value = {
        "Tags": {"Items": [{"Key": "codexcrew:site", "Value": "retrysite"}]}
    }
    ...
    man = {
        "slug": "retrysite",
        "arch": "engine",
        "bucket": "codexcrew-web-retrysite",
...
def test_internal_secret_strips_confirm_enforces_preview():
    ...
    async def scenario():
        ...
        import codex_crew.deploy.engine as _eng
...
def test_migrated_builtin_cleans_hyphenated_name(tmp_path, monkeypatch):
    ...
    from codex_crew.apps import manager as mgr
...
def test_allowed_local_roots_no_bare_tmp(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(CodexCrewConfig, "load", lambda: type("C", (), {
