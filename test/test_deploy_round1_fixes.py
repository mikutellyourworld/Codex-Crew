...
def test_staging_root_uses_config_dir(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.deploy.handlers.config_dir", lambda: tmp_path)
...
def test_staging_root_rejects_symlink(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.deploy.handlers.config_dir", lambda: fake_config)
...
def test_claim_pending_returns_entry(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.deploy.pending.config_dir", lambda: tmp_path)
...
def test_manifest_failure_triggers_rollback(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.deploy.engine.recall", mock_recall)
