...
import codex_crew.embeddings as embeddings_mod
...
class TestPlatformLibsDirname:
    ...
    def test_supported_platforms(
        ...
        monkeypatch.setattr("codex_crew.embeddings.sys.platform", platform_str)
        monkeypatch.setattr("codex_crew.embeddings.platform.machine", lambda: machine)
...
class TestModelPaths:
    def test_models_dir_under_config_dir(self, tmp_path: Path, monkeypatch) -> None:
        monkeypatch.setattr("codex_crew.embeddings.config_dir", lambda: tmp_path)
...
class TestLlamaCppEmbedder:
    ...
    def test_embed_returns_vector(self, tmp_path: Path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.embeddings._load_llama_class", lambda: fake_cls)
    ...
    def test_load_failure_sets_cooldown(self, tmp_path: Path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.embeddings._load_llama_class", lambda: fail_cls)
        emb = self._embedder(tmp_path)
    ...
    def test_load_returns_none_when_llama_class_unavailable(
        ...
        monkeypatch.setattr("codex_crew.embeddings._load_llama_class", lambda: None)
        emb = self._embedder(tmp_path)
        assert emb.wait_ready(timeout=5) is False
        assert emb.embed("hello") is None
    ...
    def test_worker_is_bound_to_its_own_queue(self, tmp_path: Path, monkeypatch) -> None:
        ...
        fake_cls = _make_fake_llama_class()
        monkeypatch.setattr("codex_crew.embeddings._load_llama_class", lambda: fake_cls)
        monkeypatch.setattr("codex_crew.embeddings._INFER_STOP_TIMEOUT_SECS", 0.0)
        emb = self._embedder(tmp_path)
        assert emb.wait_ready(timeout=5)
...
class TestModelDownloadManager:
    ...
    def _download_env(self, monkeypatch):
        ...
        monkeypatch.delenv("CODEXCREW_SKIP_MODEL_DOWNLOAD", raising=False)
        ...
        monkeypatch.setattr("codex_crew.embeddings.urllib.request.urlopen", _no_network)
    ...
    async def test_successful_download_installs_model(self, tmp_path: Path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.embeddings.urllib.request.urlopen", fake_urlopen)
        monkeypatch.setattr(
            "codex_crew.embeddings._GGUF_SHA256", hashlib.sha256(_model_bytes()).hexdigest()
        )
    ...
    async def test_env_url_override_wins(self, tmp_path: Path, monkeypatch) -> None:
        monkeypatch.setenv("CODEXCREW_EMBED_MODEL_URL", "https://mirror.example/custom.gguf")
        fake_urlopen, state = _fake_urlopen_factory()
        monkeypatch.setattr("codex_crew.embeddings.urllib.request.urlopen", fake_urlopen)
        monkeypatch.setattr(
            "codex_crew.embeddings._GGUF_SHA256", hashlib.sha256(_model_bytes()).hexdigest()
    ...
    async def test_sha_mismatch_retries_then_fails(self, tmp_path: Path, monkeypatch) -> None:
        fake_urlopen, state = _fake_urlopen_factory()
        monkeypatch.setattr("codex_crew.embeddings.urllib.request.urlopen", fake_urlopen)
        monkeypatch.setattr("codex_crew.embeddings._GGUF_SHA256", "0" * 64)
        sleep_mock = AsyncMock()
        monkeypatch.setattr("codex_crew.embeddings.asyncio.sleep", sleep_mock)
        mgr = self._mgr(tmp_path)
    ...
    async def test_too_small_download_fails(self, tmp_path: Path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.embeddings.urllib.request.urlopen", fake_urlopen)
        monkeypatch.setattr(
            "codex_crew.embeddings._GGUF_SHA256", hashlib.sha256(tiny).hexdigest()
        )
        mgr = self._mgr(tmp_path)
    ...
    async def test_salvages_legacy_ollama_blob_without_network(
        ...
        monkeypatch.setattr("codex_crew.embeddings._GGUF_SHA256", digest)
...
class TestMakeSyncEmbedFn:
    ...
    def fake_embedder(self, monkeypatch) -> _FakeSharedEmbedder:
        ...
        monkeypatch.setattr("codex_crew.embeddings.get_shared_embedder", lambda: fake)
        return fake
...
class TestSingletons:
    ...
    def test_reset_shared_embedder_closes_model(self, tmp_path: Path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.embeddings._load_llama_class", lambda: fake_cls)
        model = _write_model_file(tmp_path / "model.gguf")
        monkeypatch.setattr("codex_crew.embeddings.default_model_path", lambda: model)
...
class TestBundledWindowsMsvcRuntimeGate:
    ...
    def test_the_probe_is_a_no_op_off_windows(self, monkeypatch) -> None:
        monkeypatch.setattr("codex_crew.embeddings.sys.platform", "linux")
    ...
    def test_every_named_runtime_dll_is_probed(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.embeddings.sys.platform", "win32")
        asked: list[str] = []
