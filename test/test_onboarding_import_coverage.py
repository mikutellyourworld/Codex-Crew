"""Branch coverage for :mod:`codex_crew.onboarding_import` helpers.
...
class TestConfigProjection:
    ...
    def test_managed_and_invalid_server_names_are_diagnosed_apart(self, tmp_path: Path) -> None:
        ...
        api._add_mcp_configs(
            scan,
            [{"mcpServers": {"codexcrew-core": {"command": "x"}, "..": {"command": "y"}}}],
        )
...
class TestMcpWriter:
    ...
    def test_overwrite_cannot_claim_a_name_it_does_not_hold(
        ...
        module = importlib.import_module("codex_crew.mcp_discovery")
...
class TestLineageMemoryDatabase:
    ...
    def test_a_clean_episode_lands_with_a_clamped_importance(self, tmp_path: Path) -> None:
        ...
        _lineage_db(
            scan.root / "memory.db",
            episodic=[("e1", "the dashboard listens on port 5486", 9.0, 0, "", None)],
        )
        ...
        assert scan.items["memories"][0].payload == {
            "kind": "episodic",
            "text": "the dashboard listens on port 5486",
            "importance": 1.0,
        }
