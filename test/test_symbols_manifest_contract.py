...
def _symbolize(tmp_path: Path, manifest: Path) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        ...
        env={
            ...
            "HOME": str(tmp_path),
            "TMPDIR": str(tmp_path),
            "CODEXCREW_SYMBOL_CACHE": str(cache),
        },
        **UTF8_TEXT,
    )
...
def _ips_report(tmp_path: Path, version: str, uuid: str, arch: str) -> Path:
    ...
    """
    path = tmp_path / "Codex Crew-2026-09-04-120000.ips"
    header = {"app_name": "Codex Crew", "timestamp": "2026-09-04 12:00:00.00 +0000"}
    payload = {
        "usedImages": [
            {"name": "dyld", "path": "/usr/lib/dyld", "uuid": "0" * 36, "arch": arch},
            {
                "name": "Electron Framework",
                "path": "/Applications/Codex Crew.app/Contents/Frameworks/"
