...
from codex_crew.cron import CronJob, CronSchedule, CronService, format_schedule
...
class TestDashboardCronsTimezone:
    ...
    async def test_api_crons_uses_job_timezone(self, monkeypatch, tmp_path) -> None:
        from codex_crew.dashboard.handlers.cron import api_crons
...
class TestMcpCronListTimezone:
    ...
    def _calls_as_the_fixture_owner(self, monkeypatch) -> None:
        ...
        monkeypatch.setenv("CODEXCREW_SESSION_KEY", _TZ_OWNER)
    ...
    def test_cron_list_uses_job_timezone(self, tmp_path) -> None:
        ...
        with patch("codex_crew.mcp_cron.CronService") as mock_cls, \
                patch("codex_crew.mcp_cron.get_local_tz", return_value=mock_tz):
...
class TestSlackCronListTimezone:
    ...
    def test_uses_job_timezone(self, tmp_path) -> None:
        ...
        with patch(
            "codex_crew.messaging.commands.get_local_tz",
            return_value=("America/New_York", ZoneInfo("America/New_York")),
        ), patch("codex_crew.messaging.commands.compute_next_run_ts", return_value=None):
