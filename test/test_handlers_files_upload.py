...
from codex_crew.dashboard.handlers.files import _write_file_restricted, api_upload_file
...
def upload_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    pollute the real ``~/.codexcrew/uploads/`` and don't race other tests."""
    target = tmp_path / "uploads"
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.files._UPLOAD_DIR",
        target,
    )
    return target
...
async def test_upload_docx_emits_match_true_diagnostic(
    ...
    with caplog.at_level(
        logging.INFO, logger="codex_crew.dashboard.handlers.files",
    ):
