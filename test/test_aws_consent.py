...
class TestProviderDefaultIsLocal:
    ...
    def test_dataclass_default_is_piper(self):
        from codex_crew.slack.handler import _VoiceConfig
        from codex_crew.voice_reply import DEFAULT_PROVIDER, PROVIDER_PIPER
    ...
    def test_absent_provider_key_loads_as_piper(self, home):
        ...
        from codex_crew.slack.handler import _vc, load_voice_reply_config
        from codex_crew.voice_reply import PROVIDER_PIPER
...
class TestGrantIsOnTheKeystoneFloor:
    ...
    def test_leaf_is_fenced_for_read_and_write(self):
        from codex_crew.config.loader import aws_consent_path
        ...
        assert is_sensitive_path("~/.codex-crew/aws_service_consent.json") is True
        for command in (
            "cat ~/.codex-crew/aws_service_consent.json",
            "echo x > ~/.codex-crew/aws_service_consent.json",
            "tee ~/.codex-crew/aws_service_consent.json",
        ):
    ...
    def test_a_failed_payload_write_preserves_the_previous_store(self, home, monkeypatch):
        ...
        # revert the home fixture's CODEXCREW_HOME (same monkeypatch instance).
        assert aws_consent_path().read_bytes() == before, "the previous store was altered"
        assert (
            aws_consent.read_grant(aws_consent.SERVICE_POLLY) is not None
...
class TestPollySynthesisRefuses:
    ...
    def test_no_grant_means_no_subprocess(self, home):
        ...
        with (
            ...
            patch("codex_crew.sandbox.create_subprocess_limited") as limited,
        ):
            result = asyncio.run(
...
class TestConsentEndpointRequiresTheOwner:
    ...
    def test_get_refuses_a_non_owner_messaging_user(self, home):
        ...
        from codex_crew.dashboard.handlers import aws_consent as handler
...
class TestThereIsNoCliGrantSurface:
    ...
    An earlier revision shipped a ``codexcrew aws-consent grant`` verb behind an
    env-var guard. Review found that single layer bypassable (an in-process agent
    can unset the variable, and unlike ``codexcrew cloud`` the verb was not also
    ...
    def test_cli_commands_has_no_consent_helpers(self):
        ...
        import codex_crew.cli_commands as cc
...
class TestConsentDecisionsAreAudited:
    ...
    def test_an_audit_failure_does_not_break_the_gate(self, home):
        with patch("codex_crew.sel.sel", side_effect=RuntimeError("sel down")):
...
class TestDescribeVoicesEndpoint:
    ...
    def test_polly_without_consent_returns_an_empty_list_without_calling_aws(self, home):
        ...
        """
        from codex_crew.dashboard import chat_voice
        from codex_crew.voice_reply import PROVIDER_POLLY
