...
def isolated_state(tmp_path: Path):
    ...
    with patch(
        "codex_crew.dashboard.refresh_tokens._state_singleton",
        mgr,
    ):
        yield mgr
...
def test_tr_u_15c_handler_rate_limiter_caps_per_ip():
    ...
    """
    from codex_crew.dashboard.handlers import auth_refresh
...
def test_tr_u_15f_rate_buckets_evict_stale_ips():
    ...
    """
    from codex_crew.dashboard.handlers import auth_refresh as ar
...
def test_cookie_jar_needs_pruning_threshold():
    ...
    from codex_crew.dashboard.refresh_tokens import COOKIE_JAR_PRUNE_THRESHOLD_BYTES
...
def test_tr_u_23_logout_revokes_chain_and_clears_cookies(
    ...
    with patch("codex_crew.dashboard.handlers.auth_refresh.check_origin", return_value=True):
        import asyncio
...
def test_tr_u_25_secure_flag_only_on_https():
    ...
    finding #5 on. Forward-compatible for CodexCrew OSS behind
    ...
    https_req.host = "codexcrew.example.com"
    https_req.headers = {}
...
def test_tr_u_25b_secure_flag_via_forwarded_proto_over_tunnel():
    ...
    tun_req.scheme = "http"
    tun_req.host = "codexcrew.example.com"
...
def test_tr_u_26_refresh_cookie_path_covers_logout():
    ...
    from codex_crew.dashboard import refresh_tokens
    from codex_crew.dashboard.handlers import auth_refresh as ar
...
def test_tr_u_27_logout_revokes_access_cookie(tmp_path, monkeypatch):
    ...
    import codex_crew.dashboard.revocation_gen as rg
    import codex_crew.dashboard.token_auth as ta
    from codex_crew.dashboard.handlers import auth_refresh as ar
    from codex_crew.dashboard.token_auth import generate_token, validate_token

    # Isolate BOTH the refresh store and the token_auth revoked-nonce store to
    # tmp dirs so nothing touches the real ~/.codexcrew.
    ...
    with patch(
        "codex_crew.dashboard.refresh_tokens._state_singleton", refresh_state
    ), patch(
        "codex_crew.dashboard.handlers.auth_refresh.check_origin", return_value=True
    ):
        resp = asyncio.run(ar.api_auth_logout(request))
...
def test_refresh_expires_foreign_port_cookies_keeps_current(
    ...
    with patch(
        ...
        "codex_crew.dashboard.handlers.auth_refresh._rate_limited", return_value=False
    ):
...
#
# `codexcrew logout` (revoke_all_sessions) bumps the persisted revocation
...
def test_tr_u_28_revoke_all_sessions_kills_refresh_token(
    ...
    """A refresh token minted before `codexcrew logout` must be rejected.
    ...
    """
    import codex_crew.dashboard.token_auth as ta
    from codex_crew.dashboard.token_auth import revoke_all_sessions
    ...
    revoke_all_sessions()  # operator `codexcrew logout`
...
def test_tr_u_30_legacy_payload_without_gen_fails_closed(
    ...
    """
    import codex_crew.dashboard.refresh_tokens as rt
...
def test_tr_u_35_bump_persist_failure_leaves_counter_unchanged(
    ...
    monkeypatch.setattr("codex_crew.config.loader.config_dir", lambda: blocker)
...
def test_tr_u_36_unreadable_counter_file_logs_recovery(
    ...
    assert "re-enables unexpired sessions revoked by codexcrew logout" in caplog.text
