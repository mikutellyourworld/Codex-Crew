    ...
    env.update(
        {
            ...
            "GITHUB_REPOSITORY": "mikutellyourworld/Codex-Crew",
        }
    )
    proc = subprocess.run(
        ["bash", "-c", step["run"]],
...
def test_the_check_run_is_published_against_the_pr_head(tmp_path: Path) -> None:
    ...
    assert proc.returncode == 0, proc.stderr
    assert "repos/mikutellyourworld/Codex-Crew/check-runs" in args
...
def test_my_own_row_is_patched_not_duplicated(tmp_path: Path) -> None:
    ...
    assert "repos/mikutellyourworld/Codex-Crew/check-runs/424242" in args
