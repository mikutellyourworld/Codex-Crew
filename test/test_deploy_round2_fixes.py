...
async def test_artifact_branch_uses_single_thread_hop(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("codex_crew.deploy.handlers._HAS_ARTIFACTS", True)
    monkeypatch.setattr("codex_crew.deploy.handlers.get_default_store", lambda: FakeStore())
    monkeypatch.setattr("codex_crew.deploy.handlers._stage_artifact_html",
                        lambda kind, content, name: ([], str(tmp_path), 42))
    monkeypatch.setattr("codex_crew.deploy.handlers._resolve_profile", _fake_resolve)
    monkeypatch.setattr("codex_crew.deploy.handlers.validate_field", lambda v, spec: v)
...
def test_scan_line_numbers_correct():
    ...
    from codex_crew.deploy.scan import scan_content
...
def test_scan_truncates_at_500():
    ...
    from codex_crew.deploy.scan import _MAX_FINDINGS, scan_content
...
def test_deploy_sh_reaper_check_before_upload():
    ...
    script = Path(__file__).parent.parent / "src" / "codex_crew" / "deploy" / \
        "skills" / "artifact-deploy" / "scripts" / "deploy.sh"
    content = script.read_text(encoding="utf-8")
    # The reaper check must come BEFORE the s3 sync
    reaper_pos = content.find("codexcrew-deploy-reaper")
...
def test_reaper_lambda_has_oac_cleanup():
    ...
    spec = importlib.util.spec_from_file_location(
        "reaper_lambda",
        str(Path(__file__).parent.parent / "src" / "codex_crew" / "deploy" /
...
def test_reaper_yaml_has_oac_permissions():
    ...
    template = Path(__file__).parent.parent / "src" / "codex_crew" / "deploy" / \
...
def test_handlers_manifest_includes_oac_id():
    ...
    source = Path(__file__).parent.parent / "src" / "codex_crew" / "deploy" / "handlers.py"
