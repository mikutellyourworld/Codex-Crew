"""Tests for codex_crew.apps.version — parsing and the min-version install gate.
...
from codex_crew.apps.version import check_min_version, parse_version
...
class TestCheckMinVersion:
    def test_gate_rejects_on_insider_rc_stamp(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.__version__", "0.4.0rc3")
    ...
    def test_gate_rejects_on_bare_stamp(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("codex_crew.__version__", "0.4.0")
