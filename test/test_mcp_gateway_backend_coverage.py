"""Behavioural coverage for :mod:`codex_crew.mcp_gateway.backend`.
...
driving the parts of :class:`~codex_crew.mcp_gateway.backend.Backend` that had no
direct test:
...
class TestSubscriptionResponseHardening:
    ...
    async def test_orphan_drop_log_omits_the_uri(
        ...
        with caplog.at_level(logging.DEBUG, logger="codex_crew.mcp_gateway.backend"):
            await backend._route_backend_line(_line({
                "method": "notifications/resources/updated",
...
class TestSpawnBackend:
    ...
    async def test_spawn_marks_env_and_wires_pipes(self, fake_spawn: dict[str, Any]) -> None:
        ...
        assert env[backend_mod.CODEXCREW_SPAWNED_ENV] == backend_mod.CODEXCREW_SPAWNED_VALUE
...
class TestSendInitialize:
    ...
    async def test_success_seeds_cache_and_detects_capability(self) -> None:
        ...
        assert request["params"]["clientInfo"]["name"] == "codexcrew-gateway"
...
class TestPumpStderr:
    ...
    async def test_drains_until_eof(self) -> None:
        ...
        await _pump_stderr(reader, "codexcrew:example-mcp")
