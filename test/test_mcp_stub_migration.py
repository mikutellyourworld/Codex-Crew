...
from codex_crew.config.loader import CodexCrewConfig, _resolve_stub_servers


def _load_from_dict(data: object, tmp_path: Path) -> CodexCrewConfig:
    """Write *data* into the test's own tmp_path and load via CodexCrewConfig.load().
...
class TestThroughTheLoader:
    """The resolver wired into ``CodexCrewConfig.load``, which is what ships."""
