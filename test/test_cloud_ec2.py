...
from codex_crew.cloud import aws, ec2, sizes
...
class TestSubTemplateSyntax:
    ...
    def test_source_fetch_failure_is_not_asserted_to_be_an_install_failure(self):
        ...
        # step it cannot know.
        text = ec2.load_template()
        assert 'fail "codexcrew source fetch or install failed"' in text
        assert 'fail "codexcrew install.sh failed"' not in text, (
...
class TestValidation:
    def test_valid_tag(self):
        assert ec2.validate_tag("codexcrew-7f3a") == "codexcrew-7f3a"
    ...
    def test_tag_length_capped_for_iam_role_name(self):
        # codexcrew-ec2-<tag> must fit IAM's 64-char role-name limit; 13-char
        # prefix + tag <= 64 => tag <= 51.
        assert len("codexcrew-ec2-") + 51 == 64
    ...
    def test_stack_name(self):
        assert ec2.stack_name("abc") == "codexcrew-abc"
    ...
    def test_cidr_wide_range_warns(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="codex_crew.cloud.ec2"):
    ...
    def test_repo_ref_charset(self):
        from codex_crew.validation import validate_field
...
class TestTemplate:
    ...
    def test_source_read_grant_pinned_to_derived_arn_not_params(self):
        ...
        text = ec2.load_template()
        derived = (
            "arn:aws:s3:::codexcrew-src-${AWS::AccountId}-${AWS::Region}"
            "/${StackTag}/codexcrew-src.tar.gz"
        )
    ...
    def test_boundary_is_referenced_by_param_not_created_per_launch(self):
        ...
        assert "codexcrew-ec2-boundary-${StackTag}" not in text
    ...
    def test_permissions_boundary_arn_param_pattern(self):
        ...
        assert "codexcrew-ec2-boundary" in block.group(0)
    ...
    def test_stacktag_pattern_matches_cli_length_cap(self):
        ...
        # the CLI _TAG_RE: the role name "codexcrew-ec2-${StackTag}" + IAM's 64-char
    ...
    def test_bootstrap_verifies_dashboard_built_before_success(self):
        ...
        text = ec2.load_template()
        assert "src/codex_crew/static/dist/index.html" in text
    ...
    def test_bootstrap_requires_the_frontend_build(self):
        ...
        text = ec2.load_template()
        assert "CODEXCREW_REQUIRE_FRONTEND=1" in text
    ...
    def test_bootstrap_hands_off_to_persistent_oneshot_before_heavy_work(self):
        ...
        text = ec2.load_template()
        handoff = text.index("BOOTSTRAP_SCRIPT=/usr/local/sbin/codexcrew-bootstrap")
        ...
        assert "ExecStart=/usr/local/sbin/codexcrew-bootstrap --resume" in text
        assert "RemainAfterExit=yes" in text
        assert "WantedBy=multi-user.target" in text
        assert "systemctl enable codexcrew-bootstrap.service" in text
        assert "systemctl start --no-block codexcrew-bootstrap.service" in text
    ...
    def test_bootstrap_resume_cannot_clobber_a_running_gateway(self):
        ...
        text = ec2.load_template()
        install_guard = text.index('if [ ! -f "$INSTALL_DONE" ]; then')
        stop_gateway = text.index("systemctl stop codexcrew.service")
        replace_checkout = text.index("rm -rf codexcrew && mkdir codexcrew")
        install_done = text.index('touch "$INSTALL_DONE"')
        start_gateway = text.index("systemctl start codexcrew.service")
        signal = text.index("/opt/aws/bin/cfn-signal -e 0")
        assert (
    ...
    def test_failure_reason_pipeline_replica_yields_clean_reason(self):
        ...
        log_lines = [b"padding line %d" % i for i in range(25)] + [
            ...
            b'INSTALLING CODEXCREW AND DEPENDENCIES',
        ...
        noise = re.compile(rb"Installing (npm|codexcrew and) depend|building React app", re.I)
        ...
        assert b"CODEXCREW AND" not in reason and b"React App" not in reason
...
_BOUNDARY_ARN = "arn:aws:iam::123456789012:policy/codexcrew-ec2-boundary"
...
class TestBuildDeployArgv:
    def test_core_argv(self):
        ...
        assert "--stack-name" in argv and "codexcrew-t1" in argv
        ...
        assert "codexcrew:managed=true" in argv
        assert "codexcrew:instance=t1" in argv
    ...
    def test_source_params_included_when_set(self):
        ...
        argv = ec2.build_deploy_argv(
            ...
            subnet_id="s",
            permissions_boundary_arn=_BOUNDARY_ARN,
            source_bucket="codexcrew-src-123-us-east-1",
            source_key="t1/codexcrew-src.tar.gz",
        )
        assert "SourceBucket=codexcrew-src-123-us-east-1" in argv
        assert "SourceKey=t1/codexcrew-src.tar.gz" in argv
...
class TestDeployDryRun:
    def test_dry_run_returns_argv_without_aws(self, monkeypatch):
        import codex_crew.cloud.source as source_mod
    ...
    def test_deploy_uploads_source_and_passes_params(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda tag, profile="", region="": (
                "codexcrew-src-1-us-east-1",
                f"{tag}/codexcrew-src.tar.gz",
            ),
        )
        ...
        assert "SourceBucket=codexcrew-src-1-us-east-1" in captured["argv"]
        # IGW-routed subnet -> the public IP is required for egress
        assert "AssociatePublicIp=true" in captured["argv"]
        assert "SourceKey=t1/codexcrew-src.tar.gz" in captured["argv"]
...
class TestStatusAndList:
    def _stack(self, status="CREATE_COMPLETE", instance="i-0abc"):
        return {
            "StackName": "codexcrew-t1",
            "StackStatus": status,
            "Tags": [{"Key": "codexcrew:managed", "Value": "true"}],
            "Outputs": [
    ...
    def test_find_stack_raises_on_untagged_name_collision(self, monkeypatch):
        # A stack merely NAMED codexcrew-<tag> but not tagged managed=true is a
        ...
        monkeypatch.setattr(
            ...
            lambda *a, **k: (
                0,
                json.dumps(
                    {"Stacks": [{"StackName": "codexcrew-t1", "StackStatus": "CREATE_COMPLETE"}]}
                ),
                "",
            ),
        )
    ...
    def test_find_stack_raises_on_instance_tag_mismatch(self, monkeypatch):
        # A managed stack named codexcrew-t1 but whose codexcrew:instance tag is a
                        ...
                        "Stacks": [
                            {
                                ...
                                "Tags": [
                                    {"Key": "codexcrew:managed", "Value": "true"},
                                    {"Key": "codexcrew:instance", "Value": "somethingelse"},
                                ],
                            }
                        ]
                    }
    ...
    def test_find_stack_ok_when_instance_tag_matches(self, monkeypatch):
                        ...
                        "Stacks": [
                            {
                                ...
                                "Tags": [
                                    {"Key": "codexcrew:managed", "Value": "true"},
                                    {"Key": "codexcrew:instance", "Value": "t1"},
                                ],
                            }
                        ]
                    }
        ...
        assert st is not None and st["StackName"] == "codexcrew-t1"
                ...
                "ResourceTagMappingList": [
                    {
                        ...
                        "Tags": [{"Key": "codexcrew:instance", "Value": "t1"}],
                    },
                ]
            }
            ...
            return {
                "ResourceTagMappingList": [
                    ...
                    },
                    {
                        ...
                        "Tags": [{"Key": "codexcrew:instance", "Value": "t0"}],
                    },
                ]
            }
    ...
    def test_list_stacks_filters_codexcrew_prefix(self, monkeypatch):
        captured = {}
        ...
        def fake_json(args, profile="", region="", *, action, timeout=aws.DEFAULT_TIMEOUT):
            ...
            return {
                "StackSummaries": [
                    {"StackName": "codexcrew-kc-b", "StackStatus": "CREATE_COMPLETE"},
                    {"StackName": "other", "StackStatus": "CREATE_COMPLETE"},
                    {"StackName": "codexcrew-kc-a", "StackStatus": "UPDATE_COMPLETE"},
                ]
            }
        ...
        assert rows == [
            {"tag": "kc-a", "stack_name": "codexcrew-kc-a", "stack_status": "UPDATE_COMPLETE"},
            {"tag": "kc-b", "stack_name": "codexcrew-kc-b", "stack_status": "CREATE_COMPLETE"},
        ]
...
class TestStackEventsAndFailures:
    _EVENTS = {
        "StackEvents": [
            # newest-first, as the API returns them
            {
                ...
                "'codexcrew install.sh failed' :: ...node-rc=1|No match for nodejs",
            },
            {
                "EventId": "e2",
            ...
            {
                "EventId": "e1",
                "LogicalResourceId": "codexcrew-t1",
    ...
    def test_get_stack_failures_keeps_generic_when_no_specific(self, monkeypatch):
        ...
        fails = ec2.get_stack_failures("t1", "dev", "us-east-1")
        assert len(fails) == 1
        assert fails[0]["resource"] == "codexcrew-t1"
            ...
            lambda *a, **k: [
                {
                    ...
                    "reason": "codexcrew install.sh failed :: node-rc=1",
                }
            ],
        )
...
class TestHumanActionGuard:
    """Mutating cloud ops must refuse from an agent session (CODEXCREW_SESSION_KEY
    ...
    def test_mutations_denied_under_agent_session(self, monkeypatch):
        monkeypatch.setenv("CODEXCREW_SESSION_KEY", "sess-123")
        monkeypatch.setattr(
    ...
    def test_mutations_allowed_without_session_key(self, monkeypatch):
        # Human terminal: no CODEXCREW_SESSION_KEY -> the guard is a no-op.
        monkeypatch.delenv("CODEXCREW_SESSION_KEY", raising=False)
...
class TestDestroy:
    ...
    def test_dry_run(self, monkeypatch):
        ...
        r = ec2.destroy("t1", "dev", "us-east-1", dry_run=True)
        assert r["dry_run"] is True
        assert r["argv"] == ["cloudformation", "delete-stack", "--stack-name", "codexcrew-t1"]
    ...
    def test_destroy_deletes_and_waits(self, monkeypatch):
        monkeypatch.setattr(ec2, "find_stack", lambda *a, **k: {"StackName": "codexcrew-t1"})
        calls = {}
        monkeypatch.setattr(
            aws,
            "checked",
