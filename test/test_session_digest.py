"""Tests for codex_crew.session_digest."""
...
from codex_crew import session_digest
from codex_crew.session_digest import SessionDigest, _collapse_whitespace, digest
...
class TestFirstMessageExtraction:
    ...
    def test_basic_first_message(self, sessions_dir: Path, cli_dir: Path) -> None:
        ...
        with (
            patch("codex_crew.session_digest.data_home", return_value=sessions_dir.parent),
            patch("codex_crew.session_digest.kiro_sessions_dir", return_value=cli_dir),
        ):
...
class TestBoundedRecords:
    ...
    def test_over_cap_transcript_record_is_skipped(self, sessions_dir: Path, cli_dir: Path) -> None:
        ...
        with (
            patch("codex_crew.session_digest._RECORD_CAP", 200, create=True),
            patch("codex_crew.session_digest.data_home", return_value=sessions_dir.parent),
            patch("codex_crew.session_digest.kiro_sessions_dir", return_value=cli_dir),
        ):
    ...
    def test_transcript_handle_is_never_read_unbounded(
        ...
        with (
            patch("codex_crew.session_digest.open", _spy_open),
            patch("codex_crew.session_digest.data_home", return_value=sessions_dir.parent),
            patch("codex_crew.session_digest.kiro_sessions_dir", return_value=cli_dir),
        ):
    ...
    def test_record_exactly_at_cap_survives(self, sessions_dir: Path, cli_dir: Path) -> None:
        ...
        with (
            patch("codex_crew.session_digest._RECORD_CAP", cap, create=True),
            patch("codex_crew.session_digest.data_home", return_value=sessions_dir.parent),
            patch("codex_crew.session_digest.kiro_sessions_dir", return_value=cli_dir),
        ):
    ...
    def test_crlf_terminated_records_still_parse(self, sessions_dir: Path, cli_dir: Path) -> None:
        ...
        with (
            patch("codex_crew.session_digest._RECORD_CAP", 4096, create=True),
            patch("codex_crew.session_digest.data_home", return_value=sessions_dir.parent),
            patch("codex_crew.session_digest.kiro_sessions_dir", return_value=cli_dir),
        ):
