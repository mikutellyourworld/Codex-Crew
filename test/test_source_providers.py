...
from codex_crew import github_runner
from codex_crew.dashboard.handlers import source_providers as source
from codex_crew.sandbox import spawn_shim_argv
...
def test_parse_github_pull_request() -> None:
    ref = source.parse_source_url("https://github.com/mikutellyourworld/Codex-Crew/pull/58?tab=checks")
    assert ref.provider == "github"
    assert ref.owner == "kirodotdev"
    assert ref.repo == "CodexCrew"
    assert ref.number == 58
    assert ref.url == "https://github.com/mikutellyourworld/Codex-Crew/pull/58"
...
def test_provider_executable_rejects_agent_writable_tree(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setenv("CODEXCREW_PROJECT_DIR", str(project))
    monkeypatch.setenv("CODEXCREW_GH_BIN", str(shim))
...
def test_provider_executable_candidates_append_path_hits(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(
        ...
        {"gh": ("/usr/local/libexec/codexcrew/gh",), "glab": ("/usr/bin/glab",)},
    )

    candidates = source.provider_executable_candidates("gh")

    assert candidates[0] == "/usr/local/libexec/codexcrew/gh"
...
def test_provider_executable_not_found_gives_install_guidance(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        github_runner,
        "PROVIDER_EXECUTABLE_CANDIDATES",
        {"gh": ("/nonexistent-codexcrew/gh",), "glab": ("/nonexistent-codexcrew/glab",)},
    )
    ...
    assert "CODEXCREW_GH_BIN" in message
...
def test_provider_executable_strict_mode_asks_for_a_root_owned_copy(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        "PROVIDER_EXECUTABLE_CANDIDATES",
        {
            "gh": ("/usr/local/libexec/codexcrew/gh",),
            "glab": ("/usr/local/libexec/codexcrew/glab",),
        },
    )
    monkeypatch.setattr(
        source,
    ...
    message = str(excinfo.value)
    assert "CODEXCREW_PROVIDER_BIN_STRICT" in message
    assert 'sudo cp "$(command -v gh)" /usr/local/libexec/codexcrew/gh' in message
    assert "gh auth login" in message


def test_provider_executable_rejects_relative_override(monkeypatch) -> None:
    monkeypatch.setenv("CODEXCREW_GH_BIN", "workspace/bin/gh")
...
def test_provider_executable_accepts_user_owned_install(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setenv("CODEXCREW_GH_BIN", str(executable))
...
def test_provider_executable_accepts_symlinked_install(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.delenv("CODEXCREW_PROVIDER_BIN_STRICT", raising=False)
    monkeypatch.setattr(github_runner, "agent_writable_roots", lambda: ())
    monkeypatch.setenv("CODEXCREW_GH_BIN", str(link))
...
async def test_gitlab_allowlist_never_reads_config_on_the_event_loop(monkeypatch) -> None:
    """CodexCrewConfig.load() stats/reads/parses config files, so it must only run
...
async def test_local_token_uses_configured_owner_subject(monkeypatch) -> None:
    from codex_crew.dashboard.handlers import core
...
async def test_local_token_carries_embed_parent_port_claim(monkeypatch) -> None:
    ...
    async with TestClient(TestServer(app)) as client:
        response = await client.get(
            "/api/token/local?ttl=15m&embed_parent_port=5486",
            headers={"X-Local-Secret": "local-secret"},
        )
        assert response.status == 200

    generate.assert_called_once_with(
        "U_OWNER", ttl_seconds=900, extra={"embed_parent_port": "5486"}
    )
...
def test_parse_github_issue_url() -> None:
    ref = source.parse_source_url("https://github.com/mikutellyourworld/Codex-Crew/issues/58#issue-1")
    assert ref.provider == "github"
    assert ref.owner == "kirodotdev"
    assert ref.repo == "CodexCrew"
    assert ref.number == 58
    assert ref.kind == "issue"
    assert ref.url == "https://github.com/mikutellyourworld/Codex-Crew/issues/58"
...
class TestGetJiraAuth:
    ...
    def test_returns_none_when_no_config(self, monkeypatch):
        ...
        monkeypatch.setattr(source, "CodexCrewConfig", FakeConfig)
    ...
    def test_raises_value_error_on_config_load_failure(self, monkeypatch):
        ...
        monkeypatch.setattr(source, "CodexCrewConfig", BrokenConfig)
