...
@pytest.mark.asyncio
async def test_close_all_keeps_cross_boundary_teardown_order(cfg: CodexCrewConfig) -> None:
    events: list[str] = []
...
@pytest.mark.asyncio
async def test_hard_stop_orders_abort_reset_respawn_and_hook(cfg: CodexCrewConfig) -> None:
    events: list[str] = []
    manager = SessionManager(cfg, provider_factory=lambda **_: _provider())
    session = _Session(
        provider=_provider(),
    ...
    with patch("codex_crew.session.asyncio.create_task", side_effect=create_task):
...
@pytest.mark.asyncio
async def test_soft_stop_marks_cancelled_context_before_hook(cfg: CodexCrewConfig) -> None:
    manager = SessionManager(cfg, provider_factory=lambda **_: _provider())
    provider = _provider()
...
async def test_identity_retirement_preserves_map_and_drops_transient_state(
    ...
    with patch("codex_crew.session._provider_uses_kiro_identity_store", return_value=True):
