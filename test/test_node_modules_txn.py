...
from codex_crew import node_modules_txn as nmt
from codex_crew.node_modules_txn import BACKUP_SUFFIX, NodeModulesBackup
...
def _clean_edition_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("CODEXCREW_ALLOW_EDITION", raising=False)
...
class TestSyncBuild:
    ...
    def test_restores_the_tree_on_keyboard_interrupt(
        ...
        # Ctrl-C during `codexcrew update`. KeyboardInterrupt is a BaseException,
...
def test_the_backup_suffix_matches_dev_fleets_runner() -> None:
    ...
    codex_crew, because it executes while the repository is being merged
    ...
    it HERE is fine -- the no-codex_crew-imports rule constrains what the runner
    imports, not who imports the runner.)
    """
    from codex_crew.apps.builtins.dev_fleet.sync_runner import _BACKUP_SUFFIX

    assert BACKUP_SUFFIX == ".codexcrew-sync-backup"
