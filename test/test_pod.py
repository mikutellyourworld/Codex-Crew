...
from codex_crew.pod import cli as pod_cli
from codex_crew.pod import provision as prov
from codex_crew.pod import runtime as rt
from codex_crew.pod import unit as unit_mod
from codex_crew.pod.config import (
    DEFAULT_BASE_PORT,
    DEFAULT_LIVE_PORT,
    DEFAULT_UNIT_PREFIX,
    PodConfig,
...
def _isolate_pod_host_state(tmp_path_factory, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    Two pod paths resolve through ``Path.home()`` rather than ``CODEXCREW_HOME``, so
    ...
    ``codexcrew-pod@.service`` with this test's tmpdir as
    ``Environment=CODEXCREW_POD_ROOT=``. Every later ``pod up`` on the host then dies
...
def _ready_worktree(
    ...
    """Build a flat codexcrew worktree checkout (repo root == worktree dir).
    ...
    if venv:
        ...
        b.chmod(0o755)
    if dist:
        (co / "src" / "codex_crew" / "static" / "dist").mkdir(parents=True, exist_ok=True)
    if flags:
        cli = co / "src" / "codex_crew" / "cli.py"
...
class TestConfig:
    def test_defaults(self, monkeypatch: pytest.MonkeyPatch) -> None:
        for k in list(os.environ):
            if k.startswith("CODEXCREW_POD_"):
                monkeypatch.delenv(k, raising=False)
        c = PodConfig.load()
        assert c.base_port == DEFAULT_BASE_PORT
        assert c.live_port == DEFAULT_LIVE_PORT == 5486
        assert c.unit_prefix == DEFAULT_UNIT_PREFIX == "codexcrew-pod"
    ...
    def test_env_overrides_build_a_hermetic_plane(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("CODEXCREW_POD_UNIT_PREFIX", "codexcrew-podtest")
        monkeypatch.setenv("CODEXCREW_POD_BASE_PORT", "7300")
        monkeypatch.setenv("CODEXCREW_POD_ROOT", "/tmp/podtest-root")
        monkeypatch.setenv("CODEXCREW_POD_LIVE_PORT", "9999")
        c = PodConfig.load()
        assert c.unit_prefix == "codexcrew-podtest"
        assert c.base_port == 7300
        assert c.pod_root == Path("/tmp/podtest-root")
        assert c.live_port == 9999
        assert rt.pod_unit(c, "foo") == "codexcrew-podtest@foo.service"
...
class TestPortDerivation:
    ...
    def test_pinned_port_wins(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("CODEXCREW_POD_ENV_DIR", str(tmp_path))
        c = PodConfig.load()
...
class TestPortAllocation:
    ...
    def _plane(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> PodConfig:
        monkeypatch.setenv("CODEXCREW_POD_ENV_DIR", str(tmp_path / "env"))
        monkeypatch.setenv("CODEXCREW_POD_ROOT", str(tmp_path / "pods"))
        return PodConfig.load()
...
class TestWorktreeResolution:
    ...
    def test_git_worktrees_parses_porcelain(self, monkeypatch: pytest.MonkeyPatch) -> None:
        out = (
            ...
            "worktree /repo/codexcrew-wt-foo\nHEAD bbb\nbranch refs/heads/feat/foo\n\n"
        )
        ...
        assert wts["codexcrew-wt-foo"] == Path("/repo/codexcrew-wt-foo")
        assert wts["feat/foo"] == Path("/repo/codexcrew-wt-foo")  # branch match
    ...
    def test_resolve_root_fallback(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("CODEXCREW_POD_ENV_DIR", str(tmp_path / "env"))
        monkeypatch.setenv("CODEXCREW_POD_WORKTREES_ROOT", str(tmp_path / "wts"))
        c = PodConfig.load()
...
class TestUnitRendering:
    ...
    def test_env_block_pins_nondefaults(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("CODEXCREW_POD_ROOT", "/tmp/hermetic-pods")
        monkeypatch.setenv("CODEXCREW_POD_REPO", "/tmp/some-repo")
        txt = unit_mod.render_unit(PodConfig.load())
        assert 'Environment="CODEXCREW_POD_ROOT=/tmp/hermetic-pods"' in txt
        assert 'Environment="CODEXCREW_POD_REPO=/tmp/some-repo"' in txt

    def test_env_block_quotes_spaces(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("CODEXCREW_POD_ROOT", "/tmp/pod root")
        cfg = PodConfig.load()
        txt = unit_mod.render_unit(cfg)
        root_line = next(line for line in txt.splitlines() if "CODEXCREW_POD_ROOT=" in line)
    ...
    def test_unit_path_uses_prefix(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("CODEXCREW_POD_UNIT_PREFIX", "codexcrew-podtest")
        assert unit_mod.unit_path(PodConfig.load()).name == "codexcrew-podtest@.service"
...
class TestSeedSanitization:
    ...
    def test_adds_tunnel_when_absent(self, tmp_path: Path) -> None:
        seed = self._write(tmp_path / "s", {"dashboard": {"port": 5486}})
        out = rt.sanitized_seed_config(seed)
        assert out is not None and out["tunnel"]["enabled"] is False
    ...
    def test_refuses_sensitive_seed_path(
        ...
        monkeypatch.setattr("codex_crew.security.is_sensitive_path", lambda p, base_dir=None: True)
        assert rt.sanitized_seed_config(seed) is None
    ...
    def test_disabled_sections_cover_every_rostered_channel(self) -> None:
        ...
        section_fields = {f.name: f for f in dataclasses.fields(CodexCrewConfig)}
        exempt: set[str] = set()
        ungated: list[str] = []
...
class TestPodEnvCredentialScrub:
    ...
    def test_scrubs_every_channel_credential_the_loader_knows(
        ...
        credential and a pod runs agent turns, and ``CODEXCREW_OWNER_ID`` is the pod
    ...
    def test_bind_is_pinned_to_the_loopback_every_pod_client_dials(
        ...
        """An inherited CODEXCREW_BIND must never reach the pod gateway.
        ...
        """
        monkeypatch.setenv("CODEXCREW_BIND", inherited)
        env = rt.build_pod_env(cfg, tmp_path / "home", 7999, tmp_path / "co")
        assert env["CODEXCREW_BIND"] == "127.0.0.1"
...
class TestProvision:
    ...
    def test_venv_bin_follows_the_platform_venv_layout(self, tmp_path: Path) -> None:
        ...
        if platform_compat.IS_WINDOWS:
            assert got == tmp_path / ".venv" / "Scripts" / "codexcrew.exe"
        else:
            assert got == tmp_path / ".venv" / "bin" / "codexcrew"
...
class TestProvisionBuildPaths:
    ...
    def test_build_dist_npm_then_stages(
        ...
        assert (co / "src" / "codex_crew" / "static" / "dist" / "index.html").is_file()
...
class TestPodEnv:
    def test_scrubs_slack_and_nonaws_tokens_keeps_aws(
        ...
        assert env["CODEXCREW_PORT"] == "7999"
        assert env["CODEXCREW_HOME"].endswith("home")
        assert env["CODEXCREW_PROJECT_DIR"].endswith("co")
...
class TestPodConfigWrite:
    ...
    def test_a_failed_lockdown_publishes_no_config(self, tmp_path: Path, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "codex_crew.atomic_write.platform_compat.restrict_to_owner",
...
class TestCleanupHome:
    ...
    def test_refuses_dotdot_escape(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        pods = tmp_path / "pods"
        monkeypatch.setenv("CODEXCREW_POD_ROOT", str(pods))
        c = PodConfig.load()
...
class TestLinuxTeardownOrdering:
    ...
    def _stale_unit_with_failing_reload(
        ...
        monkeypatch.setattr(rt.unit_mod, "_codexcrew_argv", lambda: (sys.executable,))
        issued: list[str] = []
    ...
    def test_the_stop_trusts_systemd_not_the_unit_file(
        ...
        def _systemctl(*args: str, **kwargs: object) -> subprocess.CompletedProcess:
            ...
            if args[0] == "show":
                ...
                return _cp(stdout="{ path=/usr/bin/codexcrew ; argv[]=pod _cleanup x }\n")
            return _cp()
    ...
    def test_both_teardown_messages_name_the_same_path(
        ...
        monkeypatch.setenv("CODEXCREW_POD_ROOT", str(link))
        c = PodConfig.load()
    ...
    def test_a_process_outliving_the_drain_blocks_the_delete_entirely(
        ...
        home = c.home_dir("demo")
        home.mkdir(parents=True)
        (home / "codexcrew.db").write_text("a writer still has this")
        deleted: list[str] = []
        monkeypatch.setattr(rt, "systemctl", lambda *a, **k: _cp())
        ...
        assert (home / "codexcrew.db").read_text() == "a writer still has this"
...
class TestOrphanHomes:
    ...
    def test_ls_surfaces_them_with_the_reclaim_command(
        ...
        assert "codexcrew pod down orphan" in out
...
class TestDownReclaimsResidue:
    ...
    def test_down_on_a_never_used_name_is_still_a_no_op(
        ...
        monkeypatch.setattr(
            rt,
            "systemctl",
            lambda *a, **k: _cp(returncode=5, stderr="Unit codexcrew-pod@demo.service not loaded."),
        )
        monkeypatch.setattr(pod_cli, "_audit", lambda *a, **k: None)
...
class TestHostStateIsFenced:
    ...
    codexcrew-pod@.service` with a test's tmpdir as
    `Environment=CODEXCREW_POD_ROOT=`, after which every `pod up` died with "no
...
class TestRuntimeHelpers:
    ...
    def test_active_names_parses_units(
        ...
        out = (
            "codexcrew-pod@alpha.service    loaded active running x\n"
            "codexcrew-pod@beta-two.service loaded active running y\n"
    ...
    def test_mint_token_reads_secret_and_posts(
        ...
        monkeypatch.setenv("CODEXCREW_POD_ROOT", str(tmp_path))
        c = PodConfig.load()
        home = c.home_dir("demo")
        home.mkdir(parents=True)
...
class TestUnitExecSelfHeal:
    """The unit bakes an absolute codexcrew path; a pruned worktree leaves it
    ...
    def test_dangling_binary_detected(self, tmp_path, monkeypatch):
        ...
        cfg = self._cfg_with_unit(
            tmp_path,
            monkeypatch,
            f"ExecStart={(tmp_path / 'gone/.venv/bin/codexcrew').as_posix()} pod _run %i",
        )
    ...
    def test_what_this_build_renders_is_current(self, cfg, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(unit_mod, "_codexcrew_argv", lambda: (sys.executable,))
        unit_mod.install_unit(cfg)
    ...
    def test_spaced_executable_is_quoted_and_current(self, cfg, monkeypatch, tmp_path):
        ...
        exe = tmp_path / "venv with spaces" / "codexcrew"
        exe.parent.mkdir()
        exe.write_text("#!/bin/sh\n")
        exe.chmod(0o755)
        monkeypatch.setattr(unit_mod, "unit_path", lambda c: tmp_path / "pod@.service")
        monkeypatch.setattr(unit_mod, "_codexcrew_argv", lambda: (str(exe),))
    ...
    def test_module_invocation_form_passes(self, tmp_path, monkeypatch):
        ...
        cfg = self._cfg_with_unit(
            tmp_path,
            monkeypatch,
            "ExecStart=python3 -m codex_crew pod _run %i",
        )
        assert unit_mod.unit_exec_ok(cfg) is True
...
class TestSessionBus:
    ...
    A process descended from a systemd SYSTEM unit — how ``codexcrew service
...
class TestBootTimeSettings:
    ...
    ``codexcrew pod _run <name>`` with no flags. On systemd one template unit is
    ...
    def _booted_argv(
        ...
        monkeypatch.setenv("CODEXCREW_POD_ENV_DIR", str(root / "env"))
        monkeypatch.setenv("CODEXCREW_POD_ROOT", str(root / "pods"))
        c = PodConfig.load()
    ...
    def test_boot_omits_the_flag_for_a_checkout_that_cannot_parse_it(
        ...
        # CodexCrewConfig.load() deep-merges config.local.json OVER config.json with
    ...
    def test_the_probe_accepts_this_very_repo(self) -> None:
        ...
        assert (repo_root / "src" / "codex_crew" / "cli.py").is_file(), repo_root
    ...
    def test_the_probe_ignores_a_comment_only_mention(self, tmp_path: Path) -> None:
        ...
        co = tmp_path / "co"
        (co / "src" / "codex_crew").mkdir(parents=True)
        cli = co / "src" / "codex_crew" / "cli.py"
