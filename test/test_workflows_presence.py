...
Touching ``codex_crew/workflows/**`` without a matching test must fail the build
...
WORKFLOWS_DIR = TEST_DIR.parent / "src" / "codex_crew" / "workflows"
PKG = "codex_crew.workflows"
...
def _referenced_paths(tree: ast.Module) -> set[str]:
    ...
    ``from codex_crew.workflows.dsl import parallel`` registers ``...workflows.dsl``.
    """
    paths: set[str] = set()
