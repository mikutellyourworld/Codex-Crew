"""``codexcrew config defaults`` -- review, adopt, or affirm superseded defaults.
...
from codex_crew.config import superseded_defaults as SD
...
def _run(args: argparse.Namespace, config_dir: Path) -> None:
    ...
    with (
        ...
        patch("codex_crew.config.loader.config_path", return_value=config_dir / "config.json"),
...
def test_adopt_leaves_a_value_that_changed_since_it_was_listed(home):
    ...
    from codex_crew import cli_config
    from codex_crew.cli_config import _config_cmd
    ...
    with (
        patch("codex_crew.cli_config.config_path", return_value=home / "config.json"),
        patch(
            "codex_crew.cli_config.config_local_path",
            return_value=home / "config.local.json",
        ),
        patch("codex_crew.config.loader.config_dir", return_value=home),
        patch("codex_crew.cli_config.sel"),
...
def test_keep_records_the_stored_values_and_silences_the_report(home, capsys):
    ...
    with patch("codex_crew.config.loader.config_dir", return_value=home):
...
def test_a_coerced_value_cannot_be_kept(retired, capsys):
    ...
    with patch("codex_crew.config.loader.config_dir", return_value=retired):
        assert SD.acked_superseded() == {}
