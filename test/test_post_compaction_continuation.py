...
from codex_crew.acp._dispatch import parse_claude_compaction_notice
...
from codex_crew.acp_backends import ACP_BACKEND_CLAUDE, ACP_BACKEND_KIRO
from codex_crew.dashboard.chat_utils import (
    _COMPACTION_CONTINUE_MSG,
    should_continue_after_compaction,
)
from codex_crew.dashboard.state import COMPACTION_RECOVERY_PREFIX
...
def test_the_dispatch_path_flags_the_notice_chunk_it_forwards() -> None:
    ...
    """
    src = (
        pathlib.Path(__file__).resolve().parents[1] / "src" / "codex_crew" / "acp" / "client.py"
    ).read_text(encoding="utf-8")
...
def test_answer_text_only_removes_each_recorded_chunk_once() -> None:
    ...
    from codex_crew.dashboard.chat_runner import _answer_text_only
