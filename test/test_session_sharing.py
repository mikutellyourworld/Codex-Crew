...
def _mock_sessions(*, sharing_eligible: bool = True) -> MagicMock:
    ...
    sessions.get_agent = MagicMock(return_value="codexcrew")
    ...
    sessions._pool_cwd = "/home/user/.codexcrew/workspace"
...
def _cfg_patch(session_sharing: bool = True):
    """Patch CodexCrewConfig.load() to return session_sharing flag."""
    cfg = MagicMock()
    ...
    return patch("codex_crew.subagent.CodexCrewConfig.load", return_value=cfg)
...
class TestSessionSharingSpawn:
    ...
    async def test_shared_identity_persistence_failure_keeps_live_handle(
        ...
        with patch(
            "codex_crew.subagent.asyncio.to_thread",
            AsyncMock(side_effect=identity_error),
        ), patch("codex_crew.subagent.update_state", side_effect=OSError("disk full")):
...
class TestSessionSharingParentReset:
    ...
    async def test_get_subagent_runtime_retries_once_on_spawn_failure(self, monkeypatch):
        ...
        sm = SessionManager(CodexCrewConfig.load())
        sm._get_session_agent = lambda k: "codexcrew"  # type: ignore[assignment]
        ...
        monkeypatch.setattr("codex_crew.acp.runtime.AcpRuntime", _FlakyRuntime)
    ...
    async def test_get_subagent_runtime_raises_after_retries_exhausted(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.acp.runtime.AcpRuntime", _DeadRuntime)
