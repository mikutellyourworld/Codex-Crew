...
CodexCrew is developed on macOS/Linux but must pass the ``Backend Tests
...
Guidance:
    ...
    def test_no_dupe_under_colliding_ts(tmp_path):
        with colliding_clock("codex_crew.history"):
            ...  # every datetime.now() in codex_crew.history returns ONE instant
...
def colliding_clock(
    ...
    does ``from datetime import datetime`` (e.g. ``"codex_crew.history"``).
