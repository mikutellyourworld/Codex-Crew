...
from codex_crew.messaging.link import ChannelLink, legacy_dashboard_mirror_key
...
def _make_app(state):
    ...
    from codex_crew.dashboard.chat_slack import api_chat_slot_slack_pause
...
class TestTheSendPathHonoursIt:
    def test_predicates_fail_open_on_an_unstubbed_session_manager(self):
        ...
        silently dead.
        """
        from codex_crew.dashboard.chat_utils import mirror_is_paused, slack_mirror_is_paused
...
class TestEndpoints:
    ...
    async def test_disconnect_survives_a_denied_courtesy_note(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_slack.vet_and_audit",
    ...
    async def test_an_origin_disconnect_never_notifies_the_mirror(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_mirror._resolve_mirror_target", resolve)
