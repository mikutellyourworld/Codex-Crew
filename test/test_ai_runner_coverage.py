...
``subprocess``. Writes are confined to ``tmp_path`` (``CODEXCREW_HOME``,
...
from codex_crew.apps.builtins.auto_improvement import profiles as profiles_mod
...
from codex_crew.apps.builtins.auto_improvement.spine import agent_runner as agent_runner_mod
from codex_crew.apps.builtins.auto_improvement.spine import driver as driver_mod
...
class TestRedactActivity:
    def test_recurses_into_nested_events(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("codex_crew.security.redact", lambda text: text.replace("s3cret", "***"))
    ...
    def test_passthrough_for_non_text_scalars(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("codex_crew.security.redact", lambda text: text)
    ...
    def test_fails_closed_when_the_redactor_raises(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.security.redact", _boom)
    ...
    def test_fails_closed_for_a_bare_string_when_the_redactor_cannot_be_imported(
        ...
        monkeypatch.setitem(sys.modules, "codex_crew.security", None)
...
class TestCredentialConfinement:
    ...
    def _sandbox(self, monkeypatch: pytest.MonkeyPatch, mode: Any) -> None:
        ...
        monkeypatch.setattr(
            CodexCrewConfig, "load", staticmethod(lambda: SimpleNamespace(sandbox=mode))
        )
...
class TestProgressSinks:
    ...
    def test_agent_activity_is_tagged_and_redacted(
        ...
        monkeypatch.setattr("codex_crew.security.redact", lambda text: text.replace("k3y", "***"))
