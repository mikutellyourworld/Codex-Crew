...
from codex_crew.apps.builtins.issue_radar.backend import routes, store
...
def _req(query: dict | None = None):
    ...
    stub fails the repo-wide ``mypy src/codex_crew/`` gate.
    """
...
class TestGhSetupClassification(unittest.TestCase):
    ...
    def test_rejected_override_binary_is_a_setup_error(self):
        ...
        try:
            with mock.patch.dict("os.environ", {"CODEXCREW_ISSUE_RADAR_GH": "/fake/prefix/bin/gh"}), \
                 mock.patch(
