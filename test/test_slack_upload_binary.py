...
from codex_crew.dashboard.handlers import api_slack_upload_file
...
def workspace(tmp_path):
    ...
    ws.mkdir()
    with patch("codex_crew.config.loader.workspace_root", return_value=ws), \
         patch("codex_crew.config.loader.outbox_dir", return_value=tmp_path / "outbox"):
...
class TestSlackUploadBinary:
    ...
    async def test_pdf_upload_accepted(self, workspace, mock_sel):
        ...
        with patch("codex_crew.dashboard.handlers.files.is_tracked_channel", return_value=True):
