...
from codex_crew.wecom import media as media_mod
from codex_crew.wecom.attachments import to_attachments
from codex_crew.wecom.media import (
    WeComMediaError,
    _vet_media_url,
    decode_aes_key,
    decrypt_media,
...
class TestDownloadCaps:
    ...
    def test_a_missing_url_is_refused_before_any_request(self) -> None:
        with pytest.raises(WeComMediaError, match="no url"):
            asyncio.run(
                __import__("codex_crew.wecom.media", fromlist=["x"]).download_media(None, "", "k")
            )
