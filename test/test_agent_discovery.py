...
_DISCOVERY_LOGGER = "codex_crew.agent_discovery"
...
class TestProjectScopeDiscovery:
    ...
    def test_sensitive_project_dir_yields_no_agents(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.agent_discovery.is_sensitive_path",
...
class TestSpecModelCoercion:
    ...
    def test_edition_supplied_row_is_coerced(self, tmp_path: Path, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "codex_crew.platform.context.safe_context_call",
            lambda *_a, **_kw: [
...
class TestListAgentsDedup:
    ...
    def test_aim_codexcrew_package_source(self, tmp_path: Path) -> None:
        """A package-installed agent (e.g. CodexCrewAICapabilities) gets source='package'."""
        agents_dir = tmp_path / "agents"
        agents_dir.mkdir()
        (agents_dir / "CodexCrewAICapabilities-myskill.json").write_text(
    ...
    def test_local_twin_of_same_package_does_not_warn(
        ...
        with caplog.at_level(logging.DEBUG, logger="codex_crew.agent_discovery"):
            agents = list_agents(agents_dir=agents_dir)
    ...
    def test_cross_package_duplicate_still_warns(
        ...
        with caplog.at_level(logging.WARNING, logger="codex_crew.agent_discovery"):
            agents = list_agents(agents_dir=agents_dir)
        dupes = [a for a in agents if a.name == "myagent"]
        assert len(dupes) == 1
...
class TestSystematicScanFailureWarning:
    ...
    def test_row_construction_failure_counts_as_unparsed(self, fake_home, caplog, monkeypatch):
        ...
        import codex_crew.agent_discovery as mod
