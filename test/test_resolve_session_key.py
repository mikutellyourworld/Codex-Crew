...
from codex_crew.mcp_core import _get_ppid, _ppid_via_libproc, _resolve_session_key


class TestResolveSessionKey:
    def test_env_var_takes_priority(self):
        """Env var is returned immediately without file I/O."""
        with patch.dict("os.environ", {"CODEXCREW_SESSION_KEY": "dashboard:chat-1-123"}):
    ...
    def test_fresh_session_returns_env_without_walking_pids(self):
        ...
        (AcpClient._spawn sets CODEXCREW_SESSION_KEY), so resolution returns it
        ...
        with patch.dict(
            "os.environ", {"CODEXCREW_SESSION_KEY": "dashboard:chat-9-999"}
        ), patch("codex_crew.mcp_core._get_ppid") as mock_ppid:
    ...
    def test_warm_pool_session_resolves_via_pid_file(self, tmp_path):
        ...
        env = {k: v for k, v in os.environ.items() if k != "CODEXCREW_SESSION_KEY"}
        with patch.dict("os.environ", env, clear=True), patch(
            "codex_crew.mcp_core.config_dir", return_value=tmp_path
        ):
    ...
    def test_ancestor_walk_finds_grandparent(self, tmp_path):
        ...
        with patch.dict("os.environ", env, clear=True), patch(
            ...
            "codex_crew.mcp_core._get_ppid", side_effect=fake_get_ppid
        ):
    ...
    def test_stops_on_ppid_failure(self, tmp_path):
        ...
        with patch.dict("os.environ", env, clear=True), patch(
            ...
            "codex_crew.mcp_core._get_ppid", return_value=0
        ):
            assert _resolve_session_key() == ""
    ...
    def test_symlinked_pid_file_refused(self, tmp_path):
        ...
        with patch.dict("os.environ", env, clear=True), patch(
            "codex_crew.mcp_core.config_dir", return_value=tmp_path
        ), patch("codex_crew.mcp_core._get_ppid", return_value=0):
            assert _resolve_session_key() == ""
...
class TestGetPpid:
    ...
    def test_macos_prefers_libproc_over_ps(self):
        ...
        with patch("platform.system", return_value="Darwin"), patch(
            "codex_crew.mcp_core._ppid_via_libproc", return_value=42
    ...
    def test_macos_falls_back_to_ps_when_libproc_yields_zero(self):
        ...
        with patch("platform.system", return_value="Darwin"), patch(
            "codex_crew.mcp_core._ppid_via_libproc", return_value=0
