...
from codex_crew.cloud import ec2, iam, login, ssm, wizard
from codex_crew.cloud.config import CloudConfig
...
def _patch_post_launch(monkeypatch, *, logged_in: bool = True) -> dict[str, list[str]]:
    ...
    def fake_register(instance_id, *, name, profile="", region="", remote_port=5486):
...
class TestLaunchResume:
        ...
        def fake_describe(tag, *_a, **_k):
            if tag == "kc-broken":
                return {
                    "tag": tag,
                    "exists": True,
                    "stack_name": "codexcrew-kc-broken",
    ...
    def test_hold_tunnel_false_closes_and_returns(self, monkeypatch, capsys):
        # Embedded in `codexcrew setup`, the wizard must NOT block on the
    ...
    def test_interactive_existing_stack_can_create_new_installation(self, monkeypatch, capsys):
        ...
        assert save_calls == [("dev", "us-west-2", "kc-new")]
        assert choices[0][0] == "Existing CodexCrew cloud deployment"
    ...
    def test_interactive_multiple_discovered_stacks_can_choose_one(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            "list_stacks",
            lambda *_a, **_k: [
                {"tag": "kc-a", "stack_name": "codexcrew-kc-a"},
                {"tag": "kc-b", "stack_name": "codexcrew-kc-b"},
            ],
        )
        monkeypatch.setattr(
            ec2,
        ...
        assert choices == [
            (
                "Existing CodexCrew cloud deployments",
                [
                    ("Keep kc-a", "codexcrew-kc-a"),
                    ("Keep kc-b", "codexcrew-kc-b"),
    ...
    def test_multiple_discovered_stacks_fail_safe(self, monkeypatch, capsys):
        ...
        assert "multiple existing CodexCrew cloud stacks found" in out
