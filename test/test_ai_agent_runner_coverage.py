...
from codex_crew.apps.builtins.auto_improvement.spine import agent_runner as R
...
def fake_sel(monkeypatch):
    sel = _FakeSel()
    monkeypatch.setattr("codex_crew.sel.sel", lambda: sel)
    return sel
...
def _wire_hooks(monkeypatch, *, action: str, reason: str = ""):
    ...
    monkeypatch.setattr(
        R, "CodexCrewConfig", SimpleNamespace(load=lambda: SimpleNamespace(hooks={"x": 1}))
    )
...
def test_governance_denial_fails_closed_when_the_hook_layer_breaks(monkeypatch):
    monkeypatch.setattr(
        R, "CodexCrewConfig", SimpleNamespace(load=lambda: SimpleNamespace(hooks={}))
    )
...
def test_audit_fallback_tool_redacts_and_truncates_the_target(monkeypatch, fake_sel):
    monkeypatch.setattr("codex_crew.security.redact", lambda t: "R:" + t)
...
def test_audit_fallback_tool_survives_an_unwritable_log(broken_sel, monkeypatch):
    monkeypatch.setattr("codex_crew.security.redact", lambda t: t)
...
def test_run_json_encodes_a_non_string_result():
    ...
    with pytest.MonkeyPatch.context() as mp:
        mp.setattr("codex_crew.sel.sel", lambda: _FakeSel())
...
def test_session_runner_unavailable_when_config_load_raises(monkeypatch):
    ...
    monkeypatch.setattr(R, "CodexCrewConfig", SimpleNamespace(load=_boom))
    assert R.SessionAgentRunner.available() is False
...
def test_ensure_agent_registered_never_blocks_a_run_on_a_path_failure(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("codex_crew.config.paths.kiro_agents_dir", _boom)
...
def test_session_run_reports_no_configured_provider(monkeypatch):
    ...
    res = R.SessionAgentRunner().run("prompt")
    assert res.ok is False
    assert res.error == "no Codex Crew provider configured"
