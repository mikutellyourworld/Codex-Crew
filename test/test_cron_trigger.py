...
class TestCronTriggerMCP:
    ...
    def test_trigger_success(self, mock_dashboard, monkeypatch):
        port, cfg_dir = mock_dashboard
        monkeypatch.setenv("CODEXCREW_HOME", str(cfg_dir))
...
class TestCronTriggerCLI:
    ...
    def test_cli_subcommand_registered(self):
        """codexcrew cron trigger is a recognized subcommand with job_id argument."""
        ...
        with _patch("codex_crew.cli_commands.config_dir", return_value=Path("/tmp/nonexistent")):
            _cron(args)
    ...
    def test_cli_trigger_success(self, mock_dashboard, capsys):
        ...
        with _patch("codex_crew.cli_commands.config_dir", return_value=cfg_dir), \
             _patch(
                 "codex_crew.cli_commands.resolve_client_port_ex",
    ...
    def test_cli_trigger_invalid_id(self, capsys):
        ...
        loader.DASHBOARD_PORT = 19999
        try:
            with _patch("codex_crew.cli_commands.config_dir", return_value=Path("/tmp")):
                _cron(args)
        finally:
            loader.DASHBOARD_PORT = orig_port
...
class TestCronTriggerToolDefinition:
    ...
    def test_tool_listed(self):
        from codex_crew.mcp_cron import _list_tools
        tools = _list_tools()
