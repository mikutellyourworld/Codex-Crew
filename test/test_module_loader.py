...
from codex_crew.apps.module_loader import (
    _module_namespace,
...
def _explicit_third_party_execution_admission(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "codex_crew.apps.execution.third_party_execution_allowed", lambda: True
    )
...
""")
    ...
    def test_third_party_load_warns_once(self, tmp_path: Path, caplog) -> None:
        ...
        import codex_crew.apps.module_loader as ml
...
""")
        ...
        assert "_codexcrew_app_app-a.backend.routes" in sys.modules
        assert "_codexcrew_app_app-b.backend.routes" in sys.modules
...
""")
        ...
        assert key == "_codexcrew_app_my-app.handlers"
...
class TestModuleUnload:
    ...
    **Validates: Requirements 1.3 (deregistration completeness)**
    ...
    def test_unload_removes_all_app_modules(self, tmp_path: Path) -> None:
        ...
        assert "_codexcrew_app_test-app.mod_a" not in sys.modules
        assert "_codexcrew_app_test-app.sub.mod_b" not in sys.modules
        assert "_codexcrew_app_test-app.sub" not in sys.modules
        assert "_codexcrew_app_test-app" not in sys.modules
...
""")
        ...
        assert (
            sys.modules["_codexcrew_app_iso-a.backend.config"]
            is not sys.modules["_codexcrew_app_iso-b.backend.config"]
        )
...
""")
        ...
        assert "_codexcrew_app_broken-app" not in sys.modules
        assert "_codexcrew_app_broken-app.backend" not in sys.modules
...
""")
        ...
        assert "_codexcrew_app_mixed-app.backend.routes" in sys.modules
        assert "_codexcrew_app_mixed-app.backend.config" in sys.modules
        assert "_codexcrew_app_mixed-app.backend.hooks" not in sys.modules
        assert func(None) == 11
...
""")
        ...
        parent = sys.modules["_codexcrew_app_residue-app.backend"]
        assert "_codexcrew_app_residue-app.backend.render" not in sys.modules
...
""")
        ...
        key = "_codexcrew_app_two-hooks-app.backend.hooks"
        first_module = sys.modules[key]
        parent = sys.modules["_codexcrew_app_two-hooks-app.backend"]
...
def test_deploy_skill_install_copy_fallback(tmp_path, monkeypatch):
    ...
    import codex_crew.deploy as deploy_pkg
...
def test_deploy_skill_install_preserves_user_placed_dir(tmp_path, monkeypatch):
    """A user-placed directory without .codexcrew-managed marker is never removed."""
    from pathlib import Path
    ...
    # No .codexcrew-managed marker — should survive
    def _no_symlink(self, target, *a, **kw):
        raise OSError("symlink not permitted")
    monkeypatch.setattr(Path, "symlink_to", _no_symlink)
...
def test_deploy_skill_install_replaces_managed_dir(tmp_path, monkeypatch):
    """A directory WITH .codexcrew-managed marker is replaced on refresh."""
    from pathlib import Path
    ...
    managed_dir.mkdir()
    (managed_dir / ".codexcrew-managed").write_text("")
    ...
    assert (managed_dir / ".codexcrew-managed").exists()
