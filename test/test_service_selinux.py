...
from codex_crew.service import selinux as sel
from codex_crew.service.common import SERVICE_NAME
...
class TestUserScopeUnitRendering:
    ...
    def _render(monkeypatch, user_scope):
        ...
        with patch(
            ...
            return_value="/home/tester/.local/bin/codexcrew",
        ):
...
class TestStartFailureCoversWhatThePreflightCannotProve:
    ...
    A `CODEXCREW_SERVICE_BIN` override naming a system-labelled WRAPPER that later
    ...
    def test_enforcing_host_gets_the_selinux_hypothesis_and_remedy(self, monkeypatch):
        ...
        with (
            patch(
                "codex_crew.service.common.shutil.which",
                return_value="/usr/local/bin/codexcrew-wrapper",
            ),
            patch("codex_crew.service.linux.subprocess.run", side_effect=run),
        ):
        ...
        assert "/usr/local/bin/codexcrew-wrapper" in flat
        ...
        assert "CODEXCREW_UNIT" not in msg
...
class TestInstallRefusesAnUnstartableSystemUnit:
    ...
    def test_install_raises_before_touching_the_host(self, monkeypatch):
        ...
        with (
            ...
            patch("codex_crew.service.linux.subprocess.run", return_value=ok) as run,
        ):
            with pytest.raises(svc_linux.ServiceInstallError) as exc:
                svc_linux.install()
    ...
    def test_refusal_carries_a_pasteable_user_unit(self, monkeypatch):
        ...
        # Every command named must exist. `codexcrew service` has only
        # install/uninstall/status, so the remedy may not invent a subcommand.
        assert "codexcrew service print-unit" not in msg
    ...
    def test_refusal_names_only_real_service_subcommands(self, monkeypatch):
        ...
        for token in str(exc.value).split("codexcrew service ")[1:]:
    ...
    def test_gate_is_asked_about_the_unit_exec_path(self, monkeypatch):
        ...
        with (
            ...
            patch("codex_crew.service.linux.subprocess.run", return_value=ok),
        ):
            svc_linux.install()

        assert asked == ["/home/tester/.local/bin/codexcrew"]
