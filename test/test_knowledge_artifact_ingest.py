...
from codex_crew.knowledge import artifact_ingest
from codex_crew.knowledge.artifact_ingest import (
...
from codex_crew.knowledge.ingestion import DUPLICATE_JOB_STATUS, IngestionPipeline
...
class TestIngestArtifact:
    ...
    async def test_file_backed_sensitive_source_path_skipped(
        ...
        monkeypatch.setattr(
            "codex_crew.knowledge.artifact_ingest.is_sensitive_path", lambda p: True)
        assert await ingest_artifact(pipeline, art_store, art.slug, sid, DEFAULT_KINDS) is None
        assert _contents(kstore, sid) == []
...
class TestKnowledgeConfigDefaults:
    def test_auto_ingest_defaults_off(self):
        ...
        from codex_crew.config.loader import CodexCrewConfig, KnowledgeConfig
        kc = KnowledgeConfig()
        assert kc.auto_ingest_artifacts is False
        assert kc.auto_ingest_artifact_kinds == ["markdown", "text", "html", "json"]
        assert CodexCrewConfig().knowledge.auto_ingest_artifacts is False
...
class TestGroupLabelAndRename:
    ...
    async def test_attach_file_paths_labels_artifact_items_by_name(
        ...
        from codex_crew.dashboard.handlers.knowledge import _attach_file_paths
        sid, _ = ensure_artifact_source(kstore)
...
def test_removing_a_deduped_artifact_releases_its_claim_on_the_winner(tmp_path):
    ...
    """
    from codex_crew.knowledge.artifact_ingest import remove_artifact
    from codex_crew.knowledge.store import KnowledgeStore
