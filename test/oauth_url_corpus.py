...
rejected by CodexCrew's MCP OAuth-banner safety check.

When an MCP server asks CodexCrew to authenticate, kiro-cli forwards the
provider's *authorization* (consent) URL and CodexCrew renders it as a
...
**Adding a provider:** when CodexCrew gains/observes a new MCP OAuth provider,
add a representative authorize URL here.  If any param it uses isn't yet in
``_OAUTH_QUERY_PARAMS`` (codex_crew/security.py), add it there too
