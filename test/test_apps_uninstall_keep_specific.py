...
from codex_crew.apps.dependency_ledger import canonical_dep_key
...
class TestUninstallKeepSpecificParsing:
    async def _run(self, body: object) -> MagicMock:
        ...
        with (
            patch("codex_crew.apps.routes.get_app", return_value=fake_app),
            patch("codex_crew.apps.routes.uninstall_app", return_value=MagicMock(
                ok=True, to_dict=lambda: {"ok": True})),
            patch("codex_crew.apps.routes.stop_app_backend", return_value=None),
            patch("codex_crew.apps.routes.deregister_app", return_value=None),
            patch("codex_crew.apps.teardown.on_app_disable", new_callable=AsyncMock,
                  return_value=None),
            patch("codex_crew.apps.routes.sel", return_value=MagicMock()),
            patch("codex_crew.apps.routes.classify_and_clean_for_uninstall",
                  return_value={"removable": [], "shared": [], "userInstalled": []}) as m,
            patch("codex_crew.apps.routes.clean_dependencies", new_callable=AsyncMock,
                  return_value=[]),
        ):
            from codex_crew.apps.routes import handle_uninstall_app
