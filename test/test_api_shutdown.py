...
from codex_crew.dashboard.handlers.core import api_shutdown
...
async def test_shutdown_rejects_non_loopback(app):
    ...
    # it calls _h.is_loopback (where _h is the codex_crew.dashboard.handlers
    # package), so patch the name there, not on core.
    with patch("codex_crew.dashboard.handlers.core._sel") as mock_sel, \
         patch("codex_crew.dashboard.handlers.is_loopback", return_value=False):
...
async def test_shutdown_success_sets_event(app, shutdown_event):
    ...
    # `from codex_crew import shutdown_event`, so patch the source attribute.
    with patch("codex_crew.dashboard.handlers.core._sel") as mock_sel, \
         patch("codex_crew.shutdown_event", shutdown_event):
        mock_sel.return_value = MagicMock()
        async with TestClient(TestServer(app)) as c:
            resp = await c.post(
                "/api/shutdown",
