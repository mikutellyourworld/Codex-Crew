...
class TestSessionSummaryDefaults:
    def test_disabled_on_a_fresh_config(self):
        ...
        assert CodexCrewConfig().session_summary.enabled is False
...
class TestSessionSummaryParsing:
    """The section is parsed by ``CodexCrewConfig.load()``, which reads config.json."""
    ...
    def test_section_is_serialized_so_save_round_trips(self):
        assert "session_summary" in CodexCrewConfig().to_dict()
