...
from codex_crew.acp.kas_agents import _KAS_FALLBACK_PROMPT
...
def mode_stub(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "codex_crew.acp.runtime._resolve_kiro_bin_for_spawn", fake_bin
    )
...
def crew_agent(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.acp.runtime.kiro_agents_dir", lambda: agents_dir)
    monkeypatch.setattr("codex_crew.acp.runtime.ensure_agent_materialized", lambda _a: True)
    return agents_dir
...
class TestModeBinding:
    ...
    async def test_prompt_is_inlined_not_sent_as_a_file_uri(
        ...
        (crew_agent / "codexcrew.json").write_text(
            json.dumps(
                {"name": "codexcrew", "prompt": f"file://{prompt_file}", "tools": ["fs_read"]}
            ),
            encoding="utf-8",
        )
        runtime = AcpRuntime(
        ...
        try:
            await runtime.spawn()
            await runtime.create_session(cwd=tmp_path / "ws2", agent="codexcrew")
        finally:
            await runtime.kill()
    ...
    async def test_a_prompt_less_agent_falls_back_to_the_kas_prompt(
        ...
        one. Crew's own prompt-less utility agents (e.g. ``codexcrew-lite``, which
        ...
        """
        (crew_agent / "codexcrew.json").write_text(
            json.dumps({"name": "codexcrew", "tools": ["fs_read"], "prompt": ""}),
            encoding="utf-8",
        )
        runtime = AcpRuntime(
        ...
        try:
            await runtime.spawn()
            await runtime.create_session(cwd=tmp_path / "ws3", agent="codexcrew")
        finally:
            await runtime.kill()
...
class TestKiroPathUntouched:
    ...
    async def test_no_custom_agents_for_the_kiro_backend(self, mode_stub, crew_agent, tmp_path):
        ...
        assert await runtime._kas_custom_agents("codexcrew") is None
