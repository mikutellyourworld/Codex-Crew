...
def test_sync_runner_imports_only_stdlib():
    ...
    from codex_crew -- otherwise a by-path snapshot would drag the package chain
    ...
    offenders = [m for m in imported if m.split(".")[0] in {"codex_crew", "codexcrew"}]
...
class TestBootstrap:
    ...
    def _bootstrap_cmd(self, snap, digest, *runner_args):
        ...
        from codex_crew.apps.builtins.dev_fleet.worktree_ops import (
