"""Tests for :mod:`codex_crew.dashboard.handlers.artifacts`.
...
def patch_restricted(monkeypatch):
    ...
    from codex_crew.dashboard.handlers import artifacts as art_handlers
...
def linkable_project(tmp_path: Path, monkeypatch):
    ...
    ``codex_crew.artifact_source``): a disposable file is snapshotted and NO
    ...
    """
    from codex_crew import artifact_source
...
class TestCreate:
    ...
    async def test_artifact_error_fallback_returns_500(
        ...
        # errors are 500.
        from codex_crew.artifacts import ArtifactError
    ...
    async def test_promotion_of_an_oversized_source_is_refused_not_a_500(
        ...
        from codex_crew.dashboard.handlers import artifacts as _handlers
...
class TestDelete:
    ...
    async def test_withdraws_the_published_copy_before_deleting_locally(
        ...
        from codex_crew.artifacts import ArtifactPublication
        ...
        monkeypatch.setattr("codex_crew.publish_sync.delete_for_artifact", _withdraw)
    ...
    async def test_a_recreate_during_the_withdrawal_is_not_deleted(
        ...
        """
        import codex_crew.publish_sync as _ps
        from codex_crew.artifacts import ArtifactPublication
    ...
    async def test_an_unregistered_destination_refuses_the_delete_and_the_handler_adds_no_guard(
        ...
        monkeypatch.setattr("codex_crew.publish_sync.delete_for_artifact", _boom)
        ...
        monkeypatch.setattr("codex_crew.publish_sync.delete_for_artifact", real_withdraw)
        resp = await api_artifact_delete(_request(match={"slug": "x"}))
    ...
    async def test_a_reachable_withdrawal_failure_keeps_the_artifact_and_its_handle(
        ...
        monkeypatch.setattr("codex_crew.publish_sync.delete_for_artifact", _reachable_but_rejected)
        resp = await api_artifact_delete(_request(match={"slug": "x"}))
        assert resp.status == 502
    ...
    async def test_an_unreachable_destination_refuses_the_delete(
        ...
        monkeypatch.setattr("codex_crew.publish_sync.delete_for_artifact", _unreachable)
        resp = await api_artifact_delete(_request(match={"slug": "x"}))
        assert resp.status == 502
        assert (isolated_store.root / "x").exists()
        assert isolated_store.get("x").publication is not None
    ...
    async def test_a_confirmed_gone_destination_lets_the_delete_through(
        ...
        monkeypatch.setattr("codex_crew.publish_sync.delete_for_artifact", _gone)
        resp = await api_artifact_delete(_request(match={"slug": "x"}))
        assert resp.status == 200
        assert not (isolated_store.root / "x").exists()
...
class TestRecordEvent:
    ...
    async def test_referenced_event_recorded_with_metadata(
        ...
        from codex_crew.dashboard.handlers.artifacts import api_artifact_record_event
...
class TestRelocate:
    ...
    async def test_outside_home_denied(
        ...
        # — mirroring production's ~/.codex-crew/artifacts — so "outside home"
...
class TestTeardown:
    ...
    @pytest.mark.asyncio
    async def test_restricted_session_denied(self, isolated_store, patch_restricted) -> None:
        from codex_crew.deploy.handlers import _handle_teardown
    ...
    async def test_manifest_expiry_success(self, isolated_store, patch_restricted, monkeypatch) -> None:
        ...
        for call in s3_calls:
            assert any("expire-app/.codexcrew-deploy.json" in a for a in call[0])
