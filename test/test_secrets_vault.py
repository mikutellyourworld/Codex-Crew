"""Tests for codex_crew.secrets.vault — encrypted vault store."""
...
from codex_crew.secrets.vault import SecretValue, SecretVault
...
def test_restrict_to_owner_called_on_read(tmp_path, monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(
        "codex_crew.secrets.vault.restrict_to_owner",
...
# positive for the Codex Crew agent path: `.vault` is registered as a keystone
...
# `python -c "open('~/.codex-crew/.vault/...')"`. These tests prove that narrower
...
def test_keystone_denies_agent_reads_of_the_vault(tmp_path, monkeypatch) -> None:
    ...
    crew home via CODEXCREW_HOME so the keystone-leaf expansion applies, then
...
def test_vault_dir_is_hidden_by_the_os_sandbox() -> None:
    ...
    for mode_list in (
        ...
        assert (
            ".codex-crew/.vault" in mode_list
        ), "the vault dir must be OS-sandbox-hidden in every mode"
        assert ".codexcrew/.vault" in mode_list, "the legacy vault dir path must also be hidden"
