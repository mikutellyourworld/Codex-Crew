...
def _seed_language(language: str) -> None:
    ...
    conftest pins CODEXCREW_HOME to a per-test tmp dir, so config_path()
    resolves inside it and CodexCrewConfig.load() picks this file up.
    """
    p = config_path()
...
class TestUiLanguageSection:
    ...
    def test_non_str_value_does_not_raise(self, tmp_path, monkeypatch):
        ...
        cfg = MagicMock()
        monkeypatch.setattr("codex_crew.context.CodexCrewConfig.load", lambda: cfg)
        ctx = _builder(tmp_path).build_session_context()
        assert "[UI LANGUAGE]" not in ctx
...
class TestCatalogDriftGate:
    def test_backend_set_matches_the_frontend_registry(self):
        ...
        On failure: edit _UI_LANGUAGE_CATALOGS in src/codex_crew/context.py to
        ...
        assert _UI_LANGUAGE_CATALOGS == shipped, (
            ...
            "Update _UI_LANGUAGE_CATALOGS in src/codex_crew/context.py."
        )
