...
``codex_crew.dashboard.handlers.taskrunner`` — plus the redaction applied to
...
from codex_crew.acp.types import EVENT_COMPLETE, EVENT_PERMISSION_REQUEST, EVENT_TEXT_CHUNK, AcpEvent
...
from codex_crew.taskrunner import Step, StepStatus, TaskRun
...
def _stub_sel():
    ...
    with patch("codex_crew.dashboard.handlers.taskrunner._sel") as sel:
...
class TestStart:
    ...
    async def test_sensitive_path_is_403(self, tmp_path: Path) -> None:
        ...
        runner = _runner(tmp_path)
        with patch("codex_crew.dashboard.handlers.taskrunner.is_sensitive_path", return_value=True):
            resp = await api_taskrunner_start(
...
class TestExportYaml:
    ...
    async def test_serializer_failure_is_generic_500(self, tmp_path: Path) -> None:
        ...
        with patch(
            "codex_crew.dashboard.handlers.taskrunner.plan_to_yaml",
...
class TestToChat:
    ...
    def _stub_run_chat(self):
        ...
        with patch("codex_crew.dashboard.chat._run_chat", new=_noop):
            yield
...
class TestRefineStart:
    ...
    async def test_starts_background_task_and_cancels_previous(self) -> None:
        ...
        with patch("codex_crew.dashboard.handlers.taskrunner._run_refine", new=_noop):
