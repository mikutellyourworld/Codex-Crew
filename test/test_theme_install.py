...
Exercises the pure helpers in ``codex_crew.dashboard.theme_validate`` and the
theme HTTP handlers in ``codex_crew.dashboard.handlers.themes`` (plus the
...
from codex_crew.dashboard.handlers.themes import (
    _atomic_write_theme_json,
    _clone_github,
    _copy_installed_theme,
)
from codex_crew.dashboard.theme_validate import (
    _THEME_LOADER_ICONS,
...
class TestFormatVersion:
    ...
    newer CodexCrew' message rather than an opaque downstream error."""
    ...
    def test_newer_format_version_rejected(self, tmp_path: Path) -> None:
        ...
        assert "requires a newer version of Codex Crew" in err
...
class TestThemeInstallGovernanceAudit:
    ...
    def test_allowed_and_denied_emit_sel(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        from codex_crew.dashboard.handlers import themes as th
...
class TestCopyInstalledTheme:
    ...
    def test_oversized_source_rejected_by_copy_budget(self, tmp_path: Path) -> None:
        ...
        from codex_crew.dashboard.theme_validate import _THEME_TOTAL_BYTES_BY_LEVEL
    ...
    def test_source_containing_themes_dir_rejected(
        ...
        import codex_crew.dashboard.handlers.themes as th_mod
        import codex_crew.dashboard.theme_validate as tv_mod
...
class TestInstalledThemePersona:
    ...
    def test_reads_persona(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        from codex_crew.dashboard.chat_utils import _installed_theme_persona
...
class TestDeleteLock:
    ...
    def test_delete_blocks_while_install_lock_held(self, tmp_path, monkeypatch):
        ...
        import codex_crew.dashboard.theme_validate as tv_mod
        from codex_crew.dashboard.handlers import themes as themes_mod
...
class TestServingReadNolink:
    ...
    def test_regular_asset_served(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        from codex_crew.dashboard.handlers.themes import _read_theme_bytes_nolink
...
class TestCssParserCorpus:
    ...
    def test_install_verdicts_match_corpus(self) -> None:
        from codex_crew.dashboard.theme_validate import _validate_overrides_css
