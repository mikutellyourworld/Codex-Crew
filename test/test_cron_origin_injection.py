...
from codex_crew.cron import CronJob  # noqa: E402
from codex_crew.cron_script import ScriptContext  # noqa: E402
from codex_crew.dashboard.handlers import api_send_message  # noqa: E402
...
async def test_mid_plan_stage_window_queues_not_concurrent_turn(mock_sel):
    ...
    state = _state_with_job(slot=slot)
    with (
        patch("codex_crew.dashboard.chat_runner._run_chat") as run_chat,
        patch("codex_crew.dashboard.turn_dispatch.spawn_guarded_turn") as spawn,
    ):
        async with TestClient(TestServer(_make_app(state))) as c:
            resp = await c.post(
                "/api/send-message",
