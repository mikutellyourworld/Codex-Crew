...
from codex_crew.platform.context import CONTRACT_VERSION, PlatformCompositionError
...
def test_standalone_no_policy_is_none(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("codex_crew.platform.governance._policy_home_path", lambda: _nope)
...
def test_unreadable_policy_aborts_boot(monkeypatch, tmp_path):
    ...
    with pytest.raises(PlatformCompositionError):
        build_default_context(CodexCrewConfig.load())
...
def test_looser_profile_ordinal_aborts_boot(monkeypatch, tmp_path):
    ...
    try:
        ...
        with pytest.raises(PlatformCompositionError):
            bootstrap.bootstrap_context(CodexCrewConfig.load())
    finally:
        gp.reset_store()
...
def test_profile_within_ceiling_boots(monkeypatch, tmp_path):
    ...
    try:
        ...
        bootstrap.bootstrap_context(CodexCrewConfig.load())  # no raise
    finally:
        gp.reset_store()
