...
a corporate installer) names its marker through ``CODEXCREW_MANAGED_INSTALL_MARKER``.
...
def _run(tmp_path: Path, marker_env: str | None) -> tuple[subprocess.CompletedProcess, Path]:
    ...
    env = {k: v for k, v in os.environ.items() if k != "CODEXCREW_MANAGED_INSTALL_MARKER"}
    if marker_env is not None:
        env["CODEXCREW_MANAGED_INSTALL_MARKER"] = marker_env
...
def test_malformed_marker_fails_the_build(tmp_path: Path, body: str, reason: str) -> None:
    ...
    assert "CODEXCREW_MANAGED_INSTALL_MARKER rejected" in proc.stderr
