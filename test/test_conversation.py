...
class TestMaxGeneration:
    def test_none_returns_minus_one(self, session_map):
        assert session_map.max_generation("telegram:codexcrew:direct:U") == -1

    def test_base_only_returns_zero(self, session_map):
        session_map.set("telegram:codexcrew:direct:U", "sid0", provider="claude_code")
        assert session_map.max_generation("telegram:codexcrew:direct:U") == 0

    def test_highest_generation(self, session_map):
        b = "telegram:codexcrew:direct:U"
    ...
    def test_ignores_other_buckets(self, session_map):
        session_map.set("telegram:codexcrew:direct:U:gen5", "sid", provider="claude_code")
        # A different (prefix-lookalike) user must not leak in.
        assert session_map.max_generation("telegram:codexcrew:direct:U2") == -1
        assert session_map.max_generation("telegram:codexcrew:direct:U") == 5
