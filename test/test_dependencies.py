"""Tests for codex_crew.apps.dependencies — dependency resolution."""
...
from codex_crew.apps.dependencies import (
    DependencyResult,
    _get_dep_id,
    _get_managed_by,
    clean_dependencies,
...
from codex_crew.apps.manifest import CapabilityDependencies, Dependencies
from codex_crew.platform.interfaces import CapabilityResult


@pytest.fixture(autouse=True)
def _dep_home(tmp_path, monkeypatch):
    monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path / "codexcrew-home"))
...
def fake_manager(monkeypatch):
    ...
    def _install(**kwargs):
        ...
        monkeypatch.setattr(
            "codex_crew.apps.dependencies._capability_manager",
...
class TestCapabilityManagerAccessor:
    ...
    def test_reads_context_and_preserves_bound(self, monkeypatch):
        ...
        from codex_crew.apps import dependencies as deps_mod
        from codex_crew.platform import context as platform_context
        from codex_crew.platform.capability_bound import BoundedCapabilityManager
...
class TestCleanDependenciesFailurePaths:
    ...
    async def test_unavailable_manager_cleans_nothing(self, monkeypatch):
        monkeypatch.setattr("codex_crew.apps.dependencies._capability_manager", lambda: None)
        cleaned = await clean_dependencies(
    ...
    async def test_raising_op_is_swallowed(self, monkeypatch):
        ...
        mgr = _Exploding()
        monkeypatch.setattr("codex_crew.apps.dependencies._capability_manager", lambda: mgr)
        cleaned = await clean_dependencies(
            "test-app", [{"id": "capability/mcp/x", "type": "capability.mcp"}],
        )
        assert cleaned == []
...
class TestLedgerTypeRecorded:
    async def test_install_records_canonical_type(self, fake_manager):
        ...
        from codex_crew.apps.dependency_ledger import get_entry
