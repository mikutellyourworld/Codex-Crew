...
from codex_crew.dashboard.handlers.messaging import _mask_secret, _write_env_updates
...
def test_write_env_updates_handles_missing_file(tmp_path: Path, monkeypatch) -> None:
    ...
    _write_env_updates({"CODEXCREW_OWNER_ID": "U0123ABC456"})

    assert env.read_text(encoding="utf-8").strip() == "CODEXCREW_OWNER_ID=U0123ABC456"


def test_save_denies_non_loopback(monkeypatch) -> None:
    """Config writes are loopback-only: remote sessions are read-only."""
    import codex_crew.dashboard.handlers.messaging as mod
...
def test_save_syncs_process_environ(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("CODEXCREW_OWNER_ID", "U0123ABC456")
    ...
    assert "CODEXCREW_OWNER_ID" not in os.environ  # cleared key removed
...
def test_manifest_endpoint_renders_alias_and_url(monkeypatch) -> None:
    ...
    assert body["alias"] == "codexcrew"  # $USER must NOT leak as the default
    ...
    assert "CodexCrew-myteam" in body["manifest"]
...
def test_restart_required_only_on_actual_change(tmp_path, monkeypatch) -> None:
    ...
    env.write_text("", encoding="utf-8")
    cfg = tmp_path / "config.json"
    cfg.write_text('{"slack": {"command": "codexcrew"}}', encoding="utf-8")
    monkeypatch.setattr(loader, "env_path", lambda: env)
    monkeypatch.setattr(loader, "config_path", lambda: cfg)
    monkeypatch.setattr(mod, "is_direct_local_request", lambda req: True)
    ...
    body = asyncio.run(_run({"command": "codexcrew", "owner_id": "", "reactions_enabled": True}))
...
def test_teams_config_json_write_failure_leaves_consistent_pair(tmp_path, monkeypatch) -> None:
    ...
    # `from codex_crew.agent import _atomic_json_write`.  Patch the source module
