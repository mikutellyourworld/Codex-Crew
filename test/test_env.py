...
import codex_crew.env as env_mod
...
def _patch_statfns(monkeypatch, spec, *, uid=4242):
    """Patch os.getuid/os.lstat/os.stat in codex_crew.env for a ccache test."""
    monkeypatch.setattr("codex_crew.env.os.getuid", lambda: uid)
    lstat, stat_fn = _fake_statfns(spec)
    monkeypatch.setattr("codex_crew.env.os.lstat", lstat)
    monkeypatch.setattr("codex_crew.env.os.stat", stat_fn)
...
class TestAugmentedPath:
    ...
    def test_appends_running_interpreter_bin_dir_last(self, monkeypatch) -> None:
        ...
        on ``$PATH``, so ``shutil.which("codexcrew")`` silently returns ``None``
        and every user-configured MCP that spawns the ``codexcrew`` wrapper
        (e.g. ``codexcrew-core``) is dropped. Appending ``sys.executable``'s
...
class TestContributedDirsNeverReachTheTrustedRuntime:
    ...
    def test_kiro_cli_search_dirs_exclude_contributed_dirs(self, tmp_path) -> None:
        from codex_crew.kiro_cli import known_kiro_cli_dirs
...
class TestResolveKrb5Ccname:
    def test_prefers_uid_ccache(self, monkeypatch) -> None:
        monkeypatch.setattr("codex_crew.env.sys.platform", "linux")
        monkeypatch.setattr("codex_crew.env.platform_compat.IS_LINUX", True)
    ...
    def test_preserves_kcm_scheme(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.env.sys.platform", "darwin")
        monkeypatch.setattr("codex_crew.env.platform_compat.IS_LINUX", False)
        _patch_statfns(monkeypatch, {"/tmp/krb5cc_4242": ("reg", 4242)})
        env = {"KRB5CCNAME": "KCM:"}
        resolve_krb5_ccname(env)
    ...
    def test_logs_resolved_path_on_success(self, monkeypatch, caplog) -> None:
        ...
        _patch_statfns(monkeypatch, {"/tmp/krb5cc_4242": ("reg", 4242)})
        env: dict[str, str] = {}
        with caplog.at_level(logging.DEBUG, logger="codex_crew.env"):
            resolve_krb5_ccname(env)
    ...
    def test_never_calls_getuid_when_not_linux(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.env.os.getuid", _getuid_boom, raising=False)
        env: dict[str, str] = {}
        resolve_krb5_ccname(env)
        assert calls == []
        assert "KRB5CCNAME" not in env
...
class TestActivateMise:
    def test_noop_when_mise_absent(self, monkeypatch) -> None:
        monkeypatch.setattr("codex_crew.env._mise_bin", lambda: None)
        env: dict[str, str] = {"PATH": "/usr/bin"}
        assert activate_mise(env) == []
        assert env == {"PATH": "/usr/bin"}

    def test_noop_when_disabled_via_env(self, monkeypatch) -> None:
        # CODEXCREW_NO_MISE escape hatch short-circuits before mise is invoked.
        called = {"n": 0}
        monkeypatch.setattr("codex_crew.env._mise_bin", lambda: called.__setitem__("n", 1))
        env = {"PATH": "/usr/bin", "CODEXCREW_NO_MISE": "1"}
        assert activate_mise(env) == []
        assert called["n"] == 0  # _mise_bin never consulted

    def test_merges_path_and_added_vars(self, monkeypatch) -> None:
        monkeypatch.setattr("codex_crew.env._mise_bin", lambda: "/home/u/.local/bin/mise")
        payload = json.dumps(
            {
        ...
        monkeypatch.setattr("codex_crew.env.subprocess.run", _fake_run(stdout=payload))
        env = {"PATH": "/usr/bin"}
    ...
    def test_skips_unchanged_vars(self, monkeypatch) -> None:
        monkeypatch.setattr("codex_crew.env._mise_bin", lambda: "/m")
    ...
    def test_nonzero_exit_is_noop(self, monkeypatch) -> None:
        monkeypatch.setattr("codex_crew.env._mise_bin", lambda: "/m")
        monkeypatch.setattr(
            "codex_crew.env.subprocess.run", _fake_run(returncode=1, stderr="boom")
        )
        env = {"PATH": "/usr/bin"}
        assert activate_mise(env) == []
        assert env == {"PATH": "/usr/bin"}
    ...
    def test_unparsable_json_is_noop(self, monkeypatch) -> None:
        monkeypatch.setattr("codex_crew.env._mise_bin", lambda: "/m")
        monkeypatch.setattr("codex_crew.env.subprocess.run", _fake_run(stdout="not json{"))
        env = {"PATH": "/usr/bin"}
        assert activate_mise(env) == []

    def test_non_dict_json_is_noop(self, monkeypatch) -> None:
        monkeypatch.setattr("codex_crew.env._mise_bin", lambda: "/m")
        monkeypatch.setattr("codex_crew.env.subprocess.run", _fake_run(stdout="[1, 2, 3]"))
        env = {"PATH": "/usr/bin"}
        assert activate_mise(env) == []
    ...
    def test_subprocess_failure_is_swallowed(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.env.subprocess.run", _boom)
        env = {"PATH": "/usr/bin"}
        assert activate_mise(env) == []
        assert env == {"PATH": "/usr/bin"}
...
class TestGitBuildInfo:
    """codex_crew.env.git_build_info reports the running checkout's branch+sha."""
    ...
    def test_reads_branch_and_commit(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.env.subprocess.run", _run)
    ...
    def test_fails_open_on_nonzero_exit(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setenv("CODEXCREW_PROJECT_DIR", str(tmp_path))
        monkeypatch.setattr(
            "codex_crew.env.subprocess.run", _fake_run(returncode=128, stderr="fatal")
        )
...
class TestSanitizeSpecEnv:
    ...
    def test_python_env_is_dropped(self) -> None:
        ...
        Codex Crew's Linux sandbox launcher is itself a Python process
...
class TestSanitizeSpecEnvReservedNamespace:
    """SECURITY: a config-declared ``env`` may not author the ``CODEXCREW_``
    ...
    def test_caller_identity_keys_are_dropped(self) -> None:
        ...
        ``CODEXCREW_SESSION_KEY``/``CODEXCREW_HOST_PID`` are two of the three
        sources ``_resolve_session_key_strict`` accepts *on the grounds that an
        agent cannot write them*; ``CODEXCREW_OWNER_ID`` is the Slack owner.
        ``CODEXCREW_CLI`` was the cron admin-bypass flag, whose consumer #6624
        ...
        out = env_mod.sanitize_spec_env(
            [
                ("CODEXCREW_CLI", "1"),
                ("CODEXCREW_SESSION_KEY", "victim-session"),
                ("CODEXCREW_HOST_PID", "4242"),
                ("CODEXCREW_CHANNEL_ID", "C0DEADBEEF"),
                ("CODEXCREW_OWNER_ID", "UATTACKER"),
                ("CODEXCREW_INTERNAL_SECRET", "s3cret"),
                ("MCP_TOKEN", "keep-me"),
                ("PATH", "/opt/helper/bin"),
            ]
        )
    ...
    def test_whole_namespace_is_denied_not_a_key_list(self) -> None:
        ...
        out = env_mod.sanitize_spec_env(
            [
                ("CODEXCREW_SANDBOX_LEVEL", "none"),
                ("CODEXCREW_APPROVAL_MODE", "yolo"),
                ("CODEXCREW_NOT_A_REAL_VAR_YET", "x"),
                ("OK", "1"),
            ]
        )
        assert out == {"OK": "1"}
    ...
    def test_matching_is_case_insensitive(self) -> None:
        """Windows env names are case-insensitive, so ``codexcrew_cli`` reaches
        ``os.environ.get("CODEXCREW_CLI")`` there exactly like the upper spelling.
        """
        out = env_mod.sanitize_spec_env(
            [("codexcrew_cli", "1"), ("CodexCrew_Session_Key", "v"), ("OK", "1")]
        )
        assert out == {"OK": "1"}
    ...
    def test_denying_the_overlay_cannot_strip_an_inherited_value(self) -> None:
        ...
        spec on top, so a gateway-authored ``CODEXCREW_*`` value is inherited
        ...
        value.
        """
        inherited = {"CODEXCREW_HOME": "/real/home", "CODEXCREW_CLI": ""}
        child = dict(inherited)
        child.update(env_mod.sanitize_spec_env([("CODEXCREW_CLI", "1"), ("OK", "1")]))
        assert child["CODEXCREW_HOME"] == "/real/home"
        assert child["CODEXCREW_CLI"] == ""
        assert child["OK"] == "1"
...
class TestDeniedSpecEnvKeys:
    ...
    def test_reserved_namespace_is_deliberately_not_reported(self) -> None:
        ...
        for ``CODEXCREW_CLI``: it is not a launcher-execution channel, and a
        ...
        the loader class.
        """
        env = {"CODEXCREW_CLI": "1", "PYTHONPATH": "/srv", "TOKEN": "t"}
        assert env_mod.denied_spec_env_keys(env) == ["PYTHONPATH"]
        assert "CODEXCREW_CLI" not in env_mod.sanitize_spec_env(
