...
from codex_crew.config.loader import _sanitize_bot_name
...
class TestBotNameSubstitution:
    ...
    def test_empty_defaults_from_config(self):
        ...
        with patch("codex_crew.context.CodexCrewConfig.load") as mock_cfg:
        ...
        # When provider is claude_code, default bot_name is "CodexCrew"
        with patch("codex_crew.context.CodexCrewConfig.load") as mock_cfg:
            mock_cfg.return_value.agent.provider = "claude_code"
            ctx = ContextBuilder(bot_name="")
            assert ctx._substitute_bot_name("You are {bot_name}.") == "You are CodexCrew."
