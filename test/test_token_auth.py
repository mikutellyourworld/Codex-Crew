...
from codex_crew.dashboard.refresh_tokens import (
...
def clear_nonces(tmp_path, monkeypatch):
    ...
    is not written to the real ~/.codexcrew, resets the in-process gen to 0, and
    clears the nonce store. Uses _state.clear_all() (not revoke_all_sessions)
    so the gen isn't bumped between unrelated tests.
    """
    import codex_crew.dashboard.revocation_gen as _rg
    import codex_crew.dashboard.token_auth as _ta

    monkeypatch.setattr("codex_crew.config.loader.config_dir", lambda: tmp_path)
...
def test_expired_token_rejected() -> None:
    ...
    with patch("codex_crew.dashboard.token_auth.time") as mock_time:
...
async def test_middleware_accepts_valid_token(via: str) -> None:
    ...
    if via == "cookie":
        ...
        mark_consumed(token)
        req = _make_request(cookies={"mc_token_5486": token})
    else:
...
async def test_cookie_set_on_query_param_auth() -> None:
    ...
    cookie_header = resp.cookies.get("mc_token_5486")
...
async def test_embed_parent_port_claim_survives_session_exchange() -> None:
    ...
    mw = token_auth_middleware()
    token = generate_token("embeduser", ttl_seconds=300, extra={"embed_parent_port": "5486"})
    req = _make_request(query={"token": token}, remote="10.0.0.1")
    ...
    assert token_embed_parent_port(cookie_header.value) == 5486
    ...
    req.__setitem__.assert_any_call("embed_parent_port", "5486")
...
async def test_no_refresh_claim_survives_session_exchange() -> None:
    ...
    from codex_crew.dashboard.token_auth import _b64url_decode
...
async def test_cookie_not_reset_when_present() -> None:
    ...
    assert "mc_token_5486" not in resp.cookies
...
async def test_expired_query_token_falls_back_to_valid_cookie() -> None:
    ...
    req = _make_request(query={"token": stale}, cookies={"mc_token_5486": cookie})
    resp = await mw(req, _ok_handler)
    assert resp.status == 200
...
async def test_expired_query_token_with_expired_cookie_denied() -> None:
    ...
    req = _make_request(
        path="/api/status", query={"token": stale}, cookies={"mc_token_5486": dead_cookie}
    )
    resp = await mw(req, _ok_handler)
    assert resp.status in (401, 403)
...
async def test_valid_query_token_still_wins_over_cookie() -> None:
    ...
    bind_token_ip(cookie, "127.0.0.1")
    mark_consumed(cookie)
    req = _make_request(query={"token": fresh}, cookies={"mc_token_5486": cookie})
    resp = await mw(req, _ok_handler)
    assert resp.status == 200
    assert resp.cookies.get("mc_token_5486") is not None
...
async def test_internal_path_expired_query_token_falls_back_to_cookie() -> None:
    ...
    req = _make_request(
        path="/api/spawn", query={"token": stale}, cookies={"mc_token_5486": cookie}
    )
    resp = await mw(req, _ok_handler)
    assert resp.status == 200
...
async def test_expired_app_token_does_not_fall_back_to_user_cookie() -> None:
    ...
    req = _make_request(
        path="/api/status", query={"token": stale_app}, cookies={"mc_token_5486": cookie}
    )
    resp = await mw(req, _ok_handler)
    assert resp.status in (401, 403)
...
async def test_internal_path_expired_app_token_does_not_fall_back_to_cookie() -> None:
    ...
    mw = token_auth_middleware(internal_paths=frozenset({"/api/spawn"}), internal_secret="s")
    req = _make_request(
        path="/api/spawn", query={"token": stale_app}, cookies={"mc_token_5486": cookie}
    )
    resp = await mw(req, _ok_handler)
    assert resp.status in (401, 403)
...
async def test_cookie_named_by_host_port_under_tunnel() -> None:
    """Server on 5486 reached via tunneled localhost:7778 -> cookie is
    mc_token_7778 (Host port), not mc_token_5486 (server port). Lets two
    same-server-port instances coexist without colliding in the localhost jar."""
    mw = token_auth_middleware()  # default server port 5486
...
async def test_cookie_server_port_name_denied_when_host_differs() -> None:
    ...
    req = _make_request(
        path="/api/status", cookies={"mc_token_5486": token}, headers={"Host": "localhost:7778"}
    )
    resp = await mw(req, _ok_handler)
    assert resp.status != 200
...
#
# These three carry NO dashboard token: the `codexcrew` CLI authenticates to them
...
# handler ever runs — which is exactly how `codexcrew logout` broke.
...
def test_cli_local_secret_endpoints_are_in_bypass_exact() -> None:
    ...
    import codex_crew.dashboard.token_auth as ta
...
def test_apps_server_routes_are_excluded_from_shell() -> None:
    ...
    import codex_crew.apps.routes as ar
    import codex_crew.dashboard.token_auth as ta
...
async def test_internal_path_non_loopback_cookie_auth_when_not_local_only() -> None:
    ...
    req = _make_request(path="/api/spawn", remote="10.0.0.1", cookies={"mc_token_5486": token})
    resp = await mw(req, _ok_handler)
    assert resp.status == 200
...
async def test_sessions_summarize_is_registered_internal_path() -> None:
    ...
    from codex_crew.dashboard.server import _STRICT_INTERNAL_API_PATHS
...
async def test_strict_path_non_loopback_still_hard_denied() -> None:
    ...
    mark_consumed(token)
    req = _make_request(
        path="/api/send-message", remote="10.0.0.1", cookies={"mc_token_5486": token}
    )
    resp = await mw(req, _ok_handler)
    assert resp.status == 403
...
def test_cookie_auth_survives_nonce_store_wipe() -> None:
    ...
    """
    from codex_crew.dashboard.token_auth import _state
...
def test_revoke_all_sessions_kills_established_cookie() -> None:
    """Explicit revoke (codexcrew logout) MUST end established cookie sessions.
...
def test_signing_secret_persisted_across_loads(tmp_path, monkeypatch) -> None:
    ...
    """
    from codex_crew.dashboard import token_auth as ta
...
def test_signing_secret_concurrent_first_init_converges(tmp_path, monkeypatch) -> None:
    ...
    """
    from codex_crew.dashboard import token_secret as ts
...
def test_signing_secret_staging_file_is_covered_by_the_keystone_fence(
    ...
    the real crew-home prefixes, so pointing ``CODEXCREW_HOME`` at a temp dir would
...
async def test_non_api_path_gets_html_403() -> None:
    ...
    assert b"codexcrew token" in resp.body
...
async def test_spa_shell_served_with_expired_cookie() -> None:
    ...
    mw = token_auth_middleware(spa_shell_handler=_shell_handler)
    req = _make_request(
        path="/", cookies={"mc_token_5486": "garbage.invalid.token"}, remote="10.0.0.1"
    )
    resp = await mw(req, _ok_handler)
    assert resp.status == 200
...
def test_apps_routes_get_paths_are_matched_by_the_apps_spa_regex() -> None:
    ...
    import codex_crew.apps.routes as app_routes
    import codex_crew.dashboard.token_auth as ta
...
async def test_shell_bypass_does_not_preempt_ip_mismatch() -> None:
    ...
    mark_consumed(token)
    req = _make_request(
        path="/", cookies={"mc_token_5486": token}, remote="10.0.0.99"
...
async def test_dashboard_url_host_selection(dashboard_url: str, expected_host: str) -> None:
    ...
    from codex_crew.slack.handler import _handle_slash_command
    ...
    expected_port = 8080 if dashboard_url else 5486

    # Unset CODEXCREW_PORT so parse_dashboard_url (which reads os.environ at
    # call time) uses the port from the URL or the hard-coded default.
    with (
        patch("codex_crew.slack.allowlist.CodexCrewConfig.load", return_value=mock_cfg),
        patch("codex_crew.dashboard.origin.socket.gethostname", return_value="myhostname"),
        patch("codex_crew.dashboard.origin.socket.gethostbyname", return_value="10.0.0.1"),
        patch("codex_crew.dashboard.origin.socket.getaddrinfo", side_effect=socket.gaierror),
        patch.dict(os.environ, {}, CODEXCREW_PORT=""),
        patch("codex_crew.slack.allowlist.sel") as mock_sel,
    ):
...
async def test_api_logout_success_from_loopback() -> None:
    ...
    from codex_crew.dashboard.handlers import api_logout
...
async def test_api_logout_rejects_non_loopback() -> None:
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        with patch("codex_crew.dashboard.handlers.is_loopback", return_value=False):
            resp = await client.post(
                "/api/logout",
                json={},
                headers={"X-Local-Secret": "test-secret-123"},
...
async def test_api_logout_success_revokes_sessions() -> None:
    ...
    with patch("codex_crew.dashboard.token_auth.revoke_all_sessions") as mock_revoke:
        async with TestClient(TestServer(app)) as client:
            resp = await client.post(
                "/api/logout",
                json={},
...
async def test_different_ports_use_different_cookie_names() -> None:
    ...
    mw_a = token_auth_middleware(port=5486)
    ...
    assert "mc_token_5486" not in resp_b.cookies
...
def test_revoked_store_locks_the_file_down_to_its_owner(tmp_path, monkeypatch) -> None:
    ...
    from codex_crew.dashboard import token_auth as token_auth_mod
...
def test_app_owns_path_boundaries() -> None:
    ...
    assert not _app_owns_path("foo", "/api/config/codexcrew")
...
def test_reserved_app_path_segments_stay_in_sync() -> None:
    ...
    """
    from codex_crew.apps.manifest import RESERVED_APP_PATH_SEGMENTS as manifest_segments
    from codex_crew.dashboard.token_auth import RESERVED_APP_PATH_SEGMENTS as token_auth_segments
...
async def test_url_token_exchanged_for_distinct_session_cookie() -> None:
    ...
    cookie = resp.cookies.get("mc_token_5486")
    assert cookie is not None
    # Distinct credential …
...
async def test_leaked_link_token_rejected_as_cookie_after_exchange() -> None:
    ...
    req2 = _make_request(cookies={"mc_token_5486": url_token}, remote="10.0.0.9")
    resp2 = await mw(req2, _ok_handler)
...
async def test_link_token_still_reusable_via_query_param_after_exchange() -> None:
    ...
    resp1 = await mw(req1, _ok_handler)
    assert resp1.status == 200
    first_cookie = resp1.cookies.get("mc_token_5486").value
    ...
    resp2 = await mw(req2, _ok_handler)
    assert resp2.status == 200
    second_cookie = resp2.cookies.get("mc_token_5486").value
...
async def test_served_shell_is_auth_independent() -> None:
    ...
    authed = _make_request(
        path="/",
        cookies={"mc_token_5486": "someminted.token.value"},
        remote="10.0.0.1",
    )
...
async def test_index_serves_guidance_when_bundle_missing(tmp_path, monkeypatch) -> None:
    ...
    assert "restarting Codex Crew" in body
    assert "codexcrew restart" in body
...
def test_extract_numeric_claim_returns_float_session_exp() -> None:
    ...
    from codex_crew.dashboard.token_auth import extract_numeric_claim
    ...
    from codex_crew.dashboard.token_auth import extract_claims_from_token
...
def test_cookie_session_mint_does_not_evict_link_nonce() -> None:
    ...
    from codex_crew.dashboard.token_auth import MAX_CONCURRENT_NONCES, validate_token
...
def test_api_auth_refresh_mints_session_token_without_nonce_registration() -> None:
    ...
    from codex_crew.dashboard.handlers.auth_refresh import api_auth_refresh
...
def test_token_embed_parent_port_roundtrip() -> None:
    ...
    tok = generate_token("local-app", extra={"embed_parent_port": "5486"})
    assert token_embed_parent_port(tok) == 5486
...
def test_token_embed_parent_port_absent_forged_or_oob() -> None:
    ...
    forged = generate_token("local-app", extra={"embed_parent_port": "5486"}) + "x"
...
def test_start_paths_warm_auth_singletons_off_loop() -> None:
    ...
    from codex_crew.dashboard import server as _srv
...
def test_ambiguous_app_and_window_names_cannot_collide(tmp_path) -> None:
    ...
    """
    from codex_crew.dashboard.server import discover_app_window_entries
...
def test_app_window_entries_register_route_and_exclusion(tmp_path) -> None:
    ...
    from codex_crew.dashboard.server import (
...
def _tailnet_trust(**kw):
    from codex_crew.dashboard.tailnet import TailnetTrust
...
def _tailnet_env(monkeypatch):
    ...
    from codex_crew.dashboard import tailnet
...
async def test_verified_peer_session_pins_to_identity_key(_tailnet_env) -> None:
    ...
    from codex_crew.dashboard import token_auth as _ta
    ...
    from codex_crew.dashboard.token_auth import proxied_pin_observed
...
async def test_node_scope_replay_from_another_node_denied_with_device_reason(
    ...
    from codex_crew.dashboard.token_auth import bind_token_peer
    ...
    mark_consumed(token)
    resp = await mw(_peer_request(cookies={"mc_token_5486": token}), _ok_handler)
    assert resp.status == 403
...
async def test_require_peer_link_refuses_daemon_failure_before_cookie_exchange(
    ...
    assert resp.cookies.get("mc_token_5486") is None
...
async def test_require_peer_link_exchanges_for_verified_allowed_peer(_tailnet_env) -> None:
    ...
    refresh = resp.cookies.get(refresh_cookie_name("5486"))
...
async def test_plain_ip_mismatch_reason_is_preserved(_tailnet_env) -> None:
    ...
    bind_token_ip(token, "10.0.0.1")
    mark_consumed(token)
    req = _make_request(cookies={"mc_token_5486": token}, remote="192.168.1.9")
    resp = await mw(req, _ok_handler)
    assert resp.status == 403
...
async def test_refresh_cookie_alone_still_reaches_the_allowlist_deny(_tailnet_env) -> None:
    ...
    _tailnet_env(_whois_payload(login="mallory@example.com"))
    mw = token_auth_middleware(tailnet_trust=_tailnet_trust())
    req = _peer_request(cookies={"mc_refresh_5486": "whatever"})
...
async def test_internal_mixed_path_enforces_the_peer_pin(_tailnet_env) -> None:
    ...
    bind_token_peer(token, "ts:node:you@example.com|phone.tail.ts.net")
    mark_consumed(token)
    req = _peer_request(cookies={"mc_token_5486": token})
    req.path = "/api/spawn"
    resp = await mw(req, _ok_handler)
    assert resp.status == 403
    assert b"device identity mismatch" in resp.body
...
async def test_restart_first_use_repins_verified_peer_cookie(_tailnet_env) -> None:
    ...
    resp2 = await mw(
        _peer_request(forwarded="100.64.0.6", cookies={"mc_token_5486": token}), _ok_handler
    )
    assert resp2.status == 403
...
async def test_restart_signed_require_peer_cookie_rehydrates_only_for_original_device(
    ...
    assert token not in _ta._state._peer_bindings
    response = await mw(_peer_request(cookies={"mc_token_5486": token}), _ok_handler)
    ...
    set_whois(_whois_payload(node="other-node.tail.ts.net"))
    replay = await mw(
        _peer_request(forwarded="100.64.0.6", cookies={"mc_token_5486": token}),
        _ok_handler,
    )
...
async def test_restart_repin_covers_internal_mixed_paths(_tailnet_env) -> None:
    ...
    set_whois(_whois_payload(node="other-node.tail.ts.net"))
    req2 = _peer_request(forwarded="100.64.0.6", cookies={"mc_token_5486": token})
    req2.path = "/api/spawn"
    resp2 = await mw(req2, _ok_handler)
    assert resp2.status == 403
    assert b"device identity mismatch" in resp2.body
...
async def test_restart_unbound_cookie_without_peer_keeps_todays_semantics(
    ...
    token = generate_token("user", ttl_seconds=300)
    mark_consumed(token)
    resp = await mw(_make_request(cookies={"mc_token_5486": token}), _ok_handler)
    assert resp.status == 200
    assert token not in _ta._state._peer_bindings
...
def test_app_token_path_allowed_implicit_ws():
    ...
    alongside.
    """
    from codex_crew.dashboard.token_auth import app_token_path_allowed
...
async def test_no_refresh_link_expires_a_refresh_cookie_the_browser_already_had() -> None:
    ...
    stale = refresh_cookie_name("5486")
    resp = await mw(
        _make_request(
            query={"token": token},
