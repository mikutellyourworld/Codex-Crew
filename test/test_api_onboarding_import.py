...
def _handler_module():
    return importlib.import_module("codex_crew.dashboard.handlers.onboarding_import")
...
async def test_state_persists_import_onboarded(monkeypatch, tmp_path) -> None:
    ...
    config = Config()
    monkeypatch.setattr(module.CodexCrewConfig, "load", lambda: config)
    monkeypatch.setattr(module, "_sel", lambda: audit)
...
async def test_state_failure_is_generic_and_credential_free(monkeypatch) -> None:
    module = _handler_module()
    audit = _AuditLog()
    private_detail = "/Users/alice/.codex-crew/config.json"

    def fail_load():
        raise OSError(private_detail)

    monkeypatch.setattr(module.CodexCrewConfig, "load", fail_load)
    monkeypatch.setattr(module, "_sel", lambda: audit)
...
async def test_apply_response_never_leaks_restore_or_rename_paths(monkeypatch) -> None:
    ...
    module = _handler_module()
    secret_path = "/Users/alice/private/.codex-crew/imports/replaced/20260728T000000Z"
...
async def test_scan_never_writes(monkeypatch, tmp_path) -> None:
    ...
    module = _handler_module()
    import codex_crew.onboarding_import as backend
...
def test_backfill_embeddings_waits_for_the_model_before_sweeping(monkeypatch) -> None:
    ...
    module = _handler_module()
    embeddings = importlib.import_module("codex_crew.embeddings")
    swept: list[bool] = []
...
def test_the_two_source_id_patterns_cannot_drift() -> None:
    ...
    """
    module = _handler_module()
    backend = importlib.import_module("codex_crew.onboarding_import")
