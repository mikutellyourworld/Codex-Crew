...
nothing Codex Crew does to enable browsing writes, rewrites, or deletes
...
from codex_crew.browser_cli import install, launch, snapshots, view
...
class TestOperatorConfigIsNotOurs:
    ...
    Codex Crew writes its OWN config under the data home and points
    ...
    def test_the_package_has_exactly_one_destructive_operation(self):
        ...
        #   launch.py -- generates Codex Crew's OWN launch config under the data
        ...
        # browser channel, which Codex Crew does not install.
...
class TestAppTokensCannotArmBrowsing:
    ...
    def test_the_token_write_refuses_an_app_token(self):
        from codex_crew.dashboard.handlers import messaging as msg
...
class TestTokenIsOwnerRestricted:
    ...
    def test_set_token_calls_atomic_write_with_restrict_to_owner(
        ...
        from codex_crew.browser_cli import token
...
class TestViewSubprocessesReceiveNodeEnv:
    ...
    def test_spawn_passes_env_to_popen(self, monkeypatch: pytest.MonkeyPatch):
        ...
        from codex_crew.browser_cli import view as view_mod
