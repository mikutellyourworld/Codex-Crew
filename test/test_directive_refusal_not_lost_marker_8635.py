...
def published(monkeypatch) -> list[tuple[str, dict]]:
    ...
    that is the operator's own live gateway (measured: `http://127.0.0.1:5486`),
    and the request carries a real-looking session key. Under this suite the
    gateway refuses it (the conftest ``CODEXCREW_HOME`` pin makes the client read a
    different instance credential, so the POST comes back
    ``this client authenticated against the wrong Codex Crew instance``), but a
