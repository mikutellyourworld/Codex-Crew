...
Both are run by ``codexcrew setup`` without asking, and neither is exercised by
...
* ``_fix_shell_profiles`` REWRITES ``~/.zshrc`` and friends in place. It must
  ...
  re-source.
* ``_find_electron_dir`` resolves the desktop-app sources. ``CODEXCREW_PROJECT_DIR``
...
from codex_crew.cli_setup import (
    _find_electron_dir,
    _fix_shell_profiles,
...
_STALE = 'export PATH="$HOME/.codexcrew-app/bin:$PATH"\n'
...
class TestFixShellProfiles:
    ...
    def test_a_marker_line_without_path_is_left_alone(self, home: Path) -> None:
        ...
        bashrc = home / ".bashrc"
        original = "# see ~/.codexcrew-app for notes\nexport EDITOR=vi\n"
...
class TestSetupSlashCommand:
    ...
    def cfg_file(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
        ...
        monkeypatch.setattr("codex_crew.cli_setup.config_path", lambda: target)
        return target

    @staticmethod
    def _answer(monkeypatch: pytest.MonkeyPatch, value: str | None) -> None:
        monkeypatch.setattr("codex_crew.cli_setup._input_or_skip", lambda prompt: value)
    ...
    def test_an_illegal_character_falls_back_to_the_current_name(
        ...
        _setup_slash_command()
        assert self._saved(cfg_file) == "codexcrew"
...
class TestFindElectronDir:
    def test_the_project_dir_env_var_wins(
        ...
        monkeypatch.setenv("CODEXCREW_PROJECT_DIR", str(root))
    ...
    def test_an_env_var_pointing_nowhere_is_ignored_not_returned(
        ...
        monkeypatch.setenv("CODEXCREW_PROJECT_DIR", str(tmp_path / "does-not-exist"))
...
class TestSetupWhatsApp:
    """``_setup_whatsapp``: the guided WhatsApp opt-in (`codexcrew setup --whatsapp`).
    ...
    def cfg_file(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
        ...
        monkeypatch.setattr("codex_crew.whatsapp.client.neonize_available", lambda: True)
        return target
    ...
    def test_a_missing_extra_is_named_with_its_install_command(
        ...
        monkeypatch.setattr("codex_crew.whatsapp.client.neonize_available", lambda: False)
        self._answer(monkeypatch, "y")
