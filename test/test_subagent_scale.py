...
class TestBatchIdentity:
    def test_digest_chunk_size_env_guarded(self):
        """A malformed CODEXCREW_SUBAGENT_DIGEST_CHUNK_SIZE must never crash
        ...
        from codex_crew.slack.gateway import _digest_chunk_size

        with _patch.dict(os.environ, {"CODEXCREW_SUBAGENT_DIGEST_CHUNK_SIZE": "foo"}):
            assert _digest_chunk_size() == 10
        with _patch.dict(os.environ, {"CODEXCREW_SUBAGENT_DIGEST_CHUNK_SIZE": "-5"}):
            assert _digest_chunk_size() == 1  # clamped to positive
        with _patch.dict(os.environ, {"CODEXCREW_SUBAGENT_DIGEST_CHUNK_SIZE": "25"}):
    ...
    async def test_reaper_stuck_wave_sweep_reconciles(self):
        ...
        from codex_crew.subagent import _WAVE_STUCK_SECS
...
def _make_orchestrator():
    ...
    cfg = CodexCrewConfig()
    with patch.object(cfg, "load_credentials", return_value={"CODEXCREW_OWNER_ID": "U_OWNER"}):
...
class TestWaveDigest:
    ...
    async def test_large_wave_delivers_chunked_digests(self):
        ...
        with patch("codex_crew.slack.gateway._run_chat", side_effect=_fake_run_chat), \
                patch("codex_crew.subagent_persistence.mark_delivered"):
    ...
    async def test_digest_chunks_inject_in_fifo_order_despite_delayed_dispatch_hop(self):
        ...
        with patch("codex_crew.slack.gateway._run_chat", side_effect=_fake_run_chat), \
                patch("codex_crew.subagent_persistence.mark_delivered"), \
                patch(
                    "codex_crew.dashboard.turn_dispatch.chat_turn_timeout_secs",
                    _held_resolver,
                ):
            for i in range(total):
    ...
    async def test_held_members_marked_delivered_only_at_digest(self):
        ...
        marked: list[str] = []
        with patch("codex_crew.slack.gateway._run_chat", _consuming_run_chat), \
                patch("codex_crew.subagent_persistence.mark_delivered",
    ...
    async def test_run_loop_settles_held_ids_after_on_done(self):
        ...
        with patch("codex_crew.subagent.mark_delivered", side_effect=marked.append):
        ...
        from codex_crew.subagent_manager.terminal import TerminalCoordinator
    ...
    async def test_holds_settle_only_after_the_injection_turn_confirms(self):
        ...
        marked: list[str] = []
        with patch("codex_crew.slack.gateway._run_chat", _gated_run_chat), \
                patch("codex_crew.subagent_persistence.mark_delivered",
                      side_effect=marked.append):
    ...
    async def test_an_auth_required_turn_is_not_a_confirmed_hand_off(self):
        ...
        marked: list[str] = []
        with patch("codex_crew.slack.gateway._run_chat", _auth_required_run_chat), \
                patch("codex_crew.subagent_persistence.mark_delivered",
                      side_effect=marked.append):
            for i, m in enumerate(members[:10]):
                mgr.batch_members_pending = MagicMock(return_value=True)
    ...
    async def test_a_failed_injection_turn_leaves_holds_recoverable(self):
        ...
        marked: list[str] = []
        with patch("codex_crew.slack.gateway._run_chat", _failing_run_chat), \
                patch("codex_crew.subagent_persistence.mark_delivered",
                      side_effect=marked.append):
            for i, m in enumerate(members[:10]):
                mgr.batch_members_pending = MagicMock(return_value=True)
...
class TestDigestHoldDeadline:
    ...
    def test_expired_hold_forces_flush(self):
        ...
        from codex_crew.subagent import DIGEST_HOLD_SECS
    ...
    def test_deadline_zero_disables_sweep(self):
        """``CODEXCREW_SUBAGENT_DIGEST_HOLD_SECS=0`` is the documented opt-out
        ...
        mgr._agents["h0"] = m
        with patch("codex_crew.subagent.DIGEST_HOLD_SECS", 0.0), \
    ...
    async def test_straggler_wave_delivers_partial_digest_end_to_end(self):
        ...
        with patch("codex_crew.slack.gateway._run_chat", side_effect=_fake_run_chat), \
                patch("codex_crew.subagent_persistence.mark_delivered"), \
                patch("codex_crew.subagent.mark_delivered"):
            for m in finished:
                await on_done(m)
                await asyncio.sleep(0)
            ...
            hold = getattr(__import__(
                "codex_crew.subagent", fromlist=["DIGEST_HOLD_SECS"]
