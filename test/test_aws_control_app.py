...
from codex_crew import aws_consent
from codex_crew.apps.builtins.aws_control.backend import accounts as accounts_mod
from codex_crew.apps.builtins.aws_control.backend import routes as routes_mod
...
class TestConsentExtension:
    ...
    def test_effective_target_resolves_deploy_registry_default(self):
        from codex_crew.dashboard.handlers import aws_consent as consent_handlers
        from codex_crew.deploy import profiles as deploy_profiles
...
class TestStorageValidation:
    def test_good_keys_pass(self):
        from codex_crew.apps.builtins.aws_control.backend import storage
    ...
    def test_find_drive_refuses_ambiguity(self):
        from codex_crew.apps.builtins.aws_control.backend import storage
        from codex_crew.deploy.engine import AWSError

        two = json.dumps(
            {
                "ResourceTagMappingList": [
                    {"ResourceARN": "arn:aws:s3:::codexcrew-drive-0123456789ab"},
                    {"ResourceARN": "arn:aws:s3:::codexcrew-drive-ba9876543210"},
                ]
            }
        )
    ...
    def test_find_drive_ignores_foreign_naming(self):
        ...
        # STARTS with our prefix ("codexcrew-drive-company-data") is adopted --
        # only the complete prefix+12-hex scheme new_bucket_name() produces.
        payload = json.dumps(
            {
                "ResourceTagMappingList": [
                    {"ResourceARN": "arn:aws:s3:::stolen-tag-bucket"},
                    {"ResourceARN": "arn:aws:s3:::codexcrew-drive-company-data"},
                    {"ResourceARN": "arn:aws:s3:::codexcrew-drive-0123456789ab"},
                ]
            }
        )
...
class TestDriveGuards:
    ...
    def test_bootstrap_with_confirm_creates_once(self):
        ...
        with (
            ...
            mock.patch.object(routes_mod.storage_mod, "find_drive", return_value=None),
            mock.patch.object(
                routes_mod.storage_mod, "create_drive", return_value="codexcrew-drive-abc"
            ) as create,
        ):
            req = _request("POST", f"/drive/{ACCOUNT}/bootstrap", match_info={"account": ACCOUNT})
        ...
        assert _payload(resp) == {"created": True, "bucket": "codexcrew-drive-abc"}
    ...
    def test_every_mutation_refuses_a_restricted_session(self):
        ...
        with (
            mock.patch.object(routes_mod, "is_app_enabled", return_value=True),
            mock.patch(
                "codex_crew.dashboard.handlers._shared._is_restricted_session",
                return_value=True,
            ),
        ):
    ...
    def test_upload_over_the_cap_is_refused_by_header(self):
        ...
        with (
            ...
            mock.patch.object(
                routes_mod.storage_mod, "find_drive", return_value="codexcrew-drive-abc"
            ),
        ):
            req = _request(
                "POST",
...
class TestShares:
    ...
    def _isolated_store(self, tmp_path, monkeypatch):
        from codex_crew.apps.builtins.aws_control.backend import shares
...
class TestCosts:
    ...
    def _isolated_cache(self, tmp_path, monkeypatch):
        from codex_crew.apps.builtins.aws_control.backend import costs
...
class TestBackup:
    ...
    def _isolated_state(self, tmp_path, monkeypatch):
        from codex_crew.apps.builtins.aws_control.backend import backup
...
class TestDriveIamTier:
    def test_drive_tier_is_self_contained_and_scoped(self):
        from codex_crew.deploy import iam
        ...
        for statement in doc["Statement"]:
            if statement["Sid"].startswith("DriveBucket") or statement["Sid"].startswith(
                ...
                for arn in statement["Resource"]:
                    ...
                    assert arn.startswith("arn:*:s3:::codexcrew-drive-"), arn
        ...
        assert backup["Resource"] == ["arn:*:s3:::codexcrew-drive-*/backup/*"]
        ...
        assert objects["Resource"] == [
            "arn:*:s3:::codexcrew-drive-*/drive/*",
            "arn:*:s3:::codexcrew-drive-*/artifacts/*",
        ]
        # No deploy-web statements leak into the drive tier.
        assert not any("codexcrew-web" in json.dumps(s) for s in doc["Statement"])
...
class TestRound3Hardening:
    ...
    def test_concurrent_bootstrap_confirms_create_exactly_one_drive(self):
        ...
        def create(profile, region, account):
            ...
            return f"codexcrew-drive-{len(created)}"

        # First discovery sees nothing; once created, discovery finds it.
        def find(profile, region, *, account):
            return f"codexcrew-drive-{len(created)}" if created else None
...
class TestLibraryScan:
    def test_credential_bearing_artifact_is_refused(self, tmp_path, monkeypatch):
        ...
        from codex_crew.apps.builtins.aws_control.backend import library
...
class TestRound17Hardening:
    ...
    def test_archive_does_not_walk_a_symlinked_directory(self, tmp_path):
        from codex_crew.apps.builtins.aws_control.backend import backup as _bk
...
class TestRound21Partition:
    ...
    def test_drive_is_discovered_on_every_partition(self, partition):
        ...
        name = "codexcrew-drive-abc123def456"
        payload = json.dumps(
        ...
        payload = json.dumps(
            {
                "ResourceTagMappingList": [
                    {
                        "ResourceARN": "arn:aws:dynamodb:us-west-2:1:table/codexcrew-drive-abc123def456"
                    }
                ]
            }
        )
    ...
    def test_drive_policy_arns_are_partition_neutral(self):
        ...
        assert all("codexcrew-drive-" in a for a in arns)
...
class TestRound22Hardening:
    ...
    def test_teardown_refuses_an_upload_that_has_not_started(self):
        ...
        with (
            mock.patch(
                "codex_crew.deploy.engine._checked",
                return_value=json.dumps({"Account": ACCOUNT}),
            ),
            mock.patch("codex_crew.apps.manager.is_app_enabled", return_value=True),
            mock.patch("codex_crew.aws_consent.is_granted", return_value=(True, "")),
            mock.patch(
                "codex_crew.aws_consent.read_grant",
    ...
    def test_nightly_backup_emits_sel_records(self):
        from codex_crew.apps.builtins.aws_control import hooks
        ...
        with (
            ...
            mock.patch.object(
                hooks.storage_mod, "find_drive", return_value="codexcrew-drive-abc123def456"
            ),
            mock.patch.object(
                hooks.backup_mod,
                "run_snapshot_backup",
...
class TestRound26Hardening:
    ...
    def _authorizing_env(self, grant_account: str):
        ...
        return (
            ...
            mock.patch("codex_crew.apps.manager.is_app_enabled", return_value=True),
            mock.patch("codex_crew.aws_consent.is_granted", return_value=(True, "")),
            mock.patch("codex_crew.aws_consent.read_grant", return_value=grant),
        )
    ...
    def test_grant_withdrawn_mid_build_refuses_the_upload(self):
        ...
        with (
            ...
            mock.patch("codex_crew.apps.manager.is_app_enabled", return_value=True),
            mock.patch("codex_crew.aws_consent.is_granted", return_value=(True, "")),
            mock.patch("codex_crew.aws_consent.read_grant", return_value=None),
        ):
            backup.clear_stop()
...
class TestBootstrapReauthorizes:
    ...
    def test_the_app_disabled_mid_create_refuses_and_creates_nothing(self):
        ...
        with (
            ...
            mock.patch.object(
                routes_mod.storage_mod, "create_drive", return_value="codexcrew-drive-abc123def456"
            ) as create,
            mock.patch.object(routes_mod, "_audit") as audit,
        ):
            resp = asyncio.run(
    ...
    def test_a_stable_connection_still_creates(self):
        ...
        assert _payload(resp) == {"created": True, "bucket": "codexcrew-drive-abc123def456"}
        create.assert_called_once()
...
class TestRound29Hardening:
    ...
    def test_configured_region_refuses_a_value_that_is_not_a_region(self):
        ...
        # being trusted.
        from codex_crew.apps.builtins.aws_control.backend import accounts
