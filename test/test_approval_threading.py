...
from codex_crew.llm_helpers import LLMEvent
...
class TestApprovalThreading:
    ...
    async def test_uses_stored_thread_ts(self) -> None:
        ...
        with patch("codex_crew.slack.handler.is_yolo_mode", return_value=False), patch(
            "codex_crew.slack.handler._PendingApproval", return_value=_pre_approved_pending()
        ):
...
class TestIsDmReflectsDestination:
    ...
    async def test_channel_thread_sets_is_dm_false(self) -> None:
        ...
        with patch("codex_crew.slack.handler.is_yolo_mode", return_value=False), patch(
            ...
            "codex_crew.slack.handler._build_approval_blocks", wraps=None
        ) as mock_blocks:
...
class TestApprovalModeSelAudit:
    ...
    async def test_yolo_emits_sel_audit_on_approve(self) -> None:
        ...
        with patch("codex_crew.slack.gateway.sel", return_value=mock_sel), patch(
            "codex_crew.slack.handler.is_yolo_mode", return_value=False
        ), patch("codex_crew.slack.handler._PendingApproval", return_value=_pre_approved_pending()):
    ...
    async def test_yolo_returns_true_even_if_sel_raises(self) -> None:
        ...
        with patch("codex_crew.slack.gateway.sel", broken_sel), patch(
            "codex_crew.slack.handler.is_yolo_mode", return_value=False
        ), patch("codex_crew.slack.handler._PendingApproval", return_value=_pre_approved_pending()):
            approve_fn = gateway._interactive_approval("cron")
