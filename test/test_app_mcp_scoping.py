"""App MCP servers land in CodexCrew's agent config, never the shared kiro file.
...
def fake_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr(bridges, "_mcp_json_path", lambda: agents / "codexcrew.json")
...
class TestGrantVersusGovernance:
    ...
    def _policy(self, monkeypatch: pytest.MonkeyPatch, denied: bool):
        from codex_crew.apps import bridges as bmod
    ...
    def test_builtin_probe_is_quiet_without_a_ceiling(
        ...
        from codex_crew.platform import context as ctxmod
        from codex_crew.platform import governance as gmod
    ...
    def test_a_per_tool_rule_also_declines_auto_approve(self) -> None:
        ...
        from codex_crew.platform.governance import _ceiling_mentions_mcp_server
...
class TestRegistrationTarget:
    ...
    def test_registration_preserves_other_agent_config_fields(self, fake_home: Path) -> None:
        ...
        bridges._mcp_json_path().write_text(
            json.dumps({"name": "codexcrew", "includeMcpJson": False, "tools": ["fs_read"]})
        )
...
class TestRebuildSurvival:
    ...
    def test_collect_returns_enabled_app_servers(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            "codex_crew.apps.manager.list_apps", lambda: [{"name": "mochi"}], raising=False
        )
        monkeypatch.setattr("codex_crew.apps.manager.is_app_enabled", lambda n: True, raising=False)
        monkeypatch.setattr(
            "codex_crew.apps.manager.get_app_manifest",
    ...
    def test_disabled_app_contributes_nothing(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.apps.manager.is_app_enabled", lambda n: False, raising=False)
    ...
    def test_one_broken_manifest_does_not_drop_the_others(
        ...
        monkeypatch.setattr(
            "codex_crew.apps.manager.list_apps",
            lambda: [{"name": "broken"}, {"name": "good"}],
            raising=False,
        )
        monkeypatch.setattr("codex_crew.apps.manager.is_app_enabled", lambda n: True, raising=False)
        monkeypatch.setattr(
            "codex_crew.apps.manager.get_app_manifest", _manifest_or_boom, raising=False
        )
    ...
    def test_self_managed_http_url_is_preserved(
        ...
        monkeypatch.setattr(
            "codex_crew.apps.manager.list_apps", lambda: [{"name": "companion"}], raising=False
        )
        monkeypatch.setattr("codex_crew.apps.manager.is_app_enabled", lambda n: True, raising=False)
        monkeypatch.setattr(
            "codex_crew.apps.manager.get_app_manifest",
        ...
        monkeypatch.setattr(
            "codex_crew.apps.bridges.registered_app_mcp_servers", lambda: {}, raising=False
        )
    ...
    def test_gateway_managed_http_url_without_live_port_is_skipped(
        ...
        monkeypatch.setattr(
            "codex_crew.apps.manager.list_apps", lambda: [{"name": "hosted"}], raising=False
        )
        monkeypatch.setattr("codex_crew.apps.manager.is_app_enabled", lambda n: True, raising=False)
        monkeypatch.setattr(
            "codex_crew.apps.manager.get_app_manifest",
...
class TestCeilingProbesFailClosed:
    ...
    def test_mcp_probe_declines_when_the_evaluator_raises(
        ...
        monkeypatch.setattr("codex_crew.platform.governance.gate_decision", _boom)
        monkeypatch.setattr(
...
class TestBothWritePointsConsultTheCeiling:
    ...
    def test_the_predicate_is_the_only_implementation(self) -> None:
        ...
        from codex_crew import agent, cli_doctor
        from codex_crew.apps import bridges
        from codex_crew.dashboard.handlers import mcp as mcp_handler
    ...
    def test_every_mapped_scope_exists_in_the_catalog(self) -> None:
        ...
        from codex_crew.platform.governance import BUILTIN_TOOL_SCOPES, SCOPE_CATALOG
...
class TestAnEmptyAllowlistIsTheStrictestCeiling:
    ...
    def test_empty_allowlist_withholds_auto_approve(self) -> None:
        from codex_crew.platform.governance import ScopedRuleset, may_skip_gate
    ...
    def test_an_unrecognized_shape_is_not_proof_of_safety(self) -> None:
        from codex_crew.platform.governance import may_skip_gate
...
class TestManifestAutoApproveCannotSelfGrantAnExemption:
    ...
    rather than by CodexCrew or the user.
    """
...
class TestACapabilityGrantNeedsALiteralTrue:
    ...
    def test_only_a_real_true_grants(self, value: Any) -> None:
        from codex_crew.apps.manifest import Permissions
    ...
    def test_a_restriction_fails_the_other_way(self) -> None:
        ...
        """
        from codex_crew.apps.admission import AppAdmissionPolicy
...
class TestTemplateGrantsAreCeilingFilteredAtBuild:
    ...
    ``rebuild_agent_config``'s final pass covers only ``codexcrew.json``. A
    ...
    def test_the_filter_is_idempotent_so_rebuilds_are_unchanged(self) -> None:
        ...
        so a fresh rebuild's ``codexcrew.json`` is byte-for-byte what it was
...
class TestTheCodeToolIsGovernedForWrites:
    ...
    def test_code_is_in_the_map(self) -> None:
        from codex_crew.platform.governance import BUILTIN_TOOL_SCOPES
...
class TestProfileOnlyGovernanceWithholdsAutoApprove:
    ...
    def test_a_profile_deny_withholds_even_without_a_ceiling(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.platform.context.current_context", lambda: _Ctx())
...
class TestKirocrewJsonHasOneSerializedWriter:
    """codexcrew.json is written by BOTH the regenerating rebuild and the app-MCP
    ...
    def test_rebuild_writes_codexcrew_json_under_the_shared_mcp_lock(self) -> None:
        import inspect

        from codex_crew import agent

        src = inspect.getsource(agent.rebuild_agent_config)
        assert "with _mcp_lock():" in src, "the codexcrew.json write must hold bridges' lock"
...
class TestQueuedSpawnKeepsAppIdentity:
    ...
    def test_the_queued_params_include_app(self) -> None:
        ...
        from codex_crew.subagent_manager.admission import SpawnAdmissionCoordinator
