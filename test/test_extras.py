...
``codexcrew`` is not published on any index, so ``pip install "codexcrew[feishu]"``
...
def _expand(
    ...
    for req in declared[name]:
        nested = re.match(r"^codexcrew\s*\[([^\]]+)\]$", req, re.IGNORECASE)
...
def test_no_surface_ever_names_this_project(extra: str) -> None:
    """Nothing the user is asked to run may mention ``codexcrew[...]``.
    ...
    resolve. The interpreter path itself may legitimately contain "codexcrew" (a
    ...
    assert "codexcrew[" not in rendered.lower()
