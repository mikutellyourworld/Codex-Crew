...
from codex_crew.computer_use import macos_ffi, snapshot_macos
from codex_crew.computer_use.types import ComputerUseUnsupported, SnapshotRequest
...
def test_cannot_complete_triggers_the_opt_in_and_exactly_one_retry(fakes: _Fakes):
    ...
    Obsidian, CodexCrew's own desktop app) looks permanently empty. Without the
...
def test_window_list_parses_entries_and_releases_the_array(fakes: _Fakes):
    ...
    entry = fakes.new_dict(
        {
            ...
            macos_ffi.CG_WINDOW_NAME: fakes.new_string("CodexCrew — GitHub"),
    ...
    assert windows[0].title == "CodexCrew — GitHub"
...
def test_post_key_applies_a_modifier_mask_verbatim(fakes: _Fakes):
    ...
    from codex_crew.computer_use import keymap
...
def test_shots_dir_default_uses_the_platform_tempdir(fakes: _Fakes):
    ...
    from codex_crew.computer_use.types import SCREENSHOT_DIR_NAME
