...
from codex_crew.dashboard.handlers.agents import api_agent_detail
...
async def test_patch_explicit_model_freezes(tmp_path):
    cfg = tmp_path / "codexcrew.json"
    cfg.write_text(json.dumps({"name": "codexcrew", "model": "claude-old", "model_managed": True}))
    request = _patch_request("codexcrew", {"model": "claude-new"})
...
async def test_patch_clear_model_resumes_tracking(tmp_path):
    cfg = tmp_path / "codexcrew.json"
    cfg.write_text(
        json.dumps({"name": "codexcrew", "model": "claude-pinned", "model_managed": False})
    )
    request = _patch_request("codexcrew", {"model": ""})
...
async def test_patch_without_model_lifts_stale_bookkeeping_keys(tmp_path):
    ...
    cfg = tmp_path / "codexcrew.json"
    cfg.write_text(
        json.dumps({"name": "codexcrew", "model_managed": False, "cc_model": "claude-sonnet-4.6"})
    )
    request = _patch_request("codexcrew", {})
