...
import codex_crew.validation as validation
from codex_crew import security, security_posture
from codex_crew import sel as _sel_mod
from codex_crew.security_posture import (
    _CONTROLS,
    PostureItem,
...
class TestDerivation:
    ...
    def test_every_mcp_schema_registry_is_covered_by_the_posture_view(self, snapshot):
        ...
        """
        from codex_crew import security_posture
    ...
    def test_token_auth_ttls_come_from_the_enforcing_module(self, snapshot):
        from codex_crew.dashboard.token_auth import LINK_WINDOW_SECS, MAX_SESSION_TTL_SECS
...
class TestSensitivePathClassification:
    def test_crew_owned_paths_are_labelled_as_trust_roots(self, snapshot):
        ...
        assert "trust root" in by_label["~/.codex-crew/security_policy.json"]
    ...
    def test_classification_is_a_path_boundary_match(self, monkeypatch):
        ...
        # mislabelled as ours.
        monkeypatch.setattr(
            security, "sensitive_home_dirs", lambda: (".codexcrew-notes/x", ".codex-crew/.env")
        )
        items = security_posture._sensitive_path_items()
        by_label = {i.label: i.detail for i in items}
        assert "Third-party" in by_label["~/.codexcrew-notes/x"]
        assert "trust root" in by_label["~/.codex-crew/.env"]
...
class TestEndpoint:
    ...
    async def test_handler_serves_the_snapshot(self):
        ...
        from codex_crew.dashboard.handlers.core import api_security_posture
    ...
    def test_route_is_actually_registered_on_the_router(self):
        ...
        src = inspect.getsource(server) + "".join(
            inspect.getsource(importlib.import_module(f"codex_crew.dashboard.routes.{name}"))
    ...
    async def test_counts_only_path_agrees_with_the_full_snapshot(self):
        ...
        """
        from codex_crew.security_posture import posture_counts, posture_counts_async
