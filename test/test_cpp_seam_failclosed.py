...
from codex_crew.config.loader import CodexCrewConfig, build_provider_factory
...
@pytest.fixture
def cfg() -> CodexCrewConfig:
    return CodexCrewConfig()


# Context isolation (reset before+after every test, CODEXCREW_PROFILE=standalone)
...
def _jail_env_isolation(monkeypatch):
    ...
    monkeypatch.delenv("CODEXCREW_JAILED", raising=False)
    monkeypatch.delenv("CODEXCREW_NO_JAIL", raising=False)
...
def stub_child_argv(monkeypatch):
    ...
    monkeypatch.setattr(cli, "_child_argv", lambda: ["codexcrew", "chat"])
    return ["codexcrew", "chat"]
...
def _install_jail(cfg: CodexCrewConfig, jail: Any, *, jail_mode: str = "auto") -> None:
...
def test_public_default_jail_on_runs_in_process(cfg: CodexCrewConfig) -> None:
...
def test_public_default_jail_auto_runs_in_process(cfg: CodexCrewConfig) -> None:
...
def test_backend_returns_none_on_mode_fails_closed(cfg: CodexCrewConfig) -> None:
...
def test_backend_raises_on_mode_fails_closed(cfg: CodexCrewConfig) -> None:
...
def test_backend_raises_auto_mode_degrades(cfg: CodexCrewConfig) -> None:
...
def test_backend_returns_none_auto_mode_degrades(cfg: CodexCrewConfig) -> None:
...
def test_flaky_available_on_mode_fails_closed(cfg: CodexCrewConfig) -> None:
...
def test_flaky_available_auto_mode_degrades(cfg: CodexCrewConfig) -> None:
...
def test_composition_error_propagates_through_gate(cfg: CodexCrewConfig, monkeypatch) -> None:
...
def test_successful_reexec_propagates_rc(cfg: CodexCrewConfig, monkeypatch) -> None:
    ...
    assert jail.calls[0][0] == ("codexcrew", "chat")


# ── R2: --no-jail and CODEXCREW_NO_JAIL both force off (skip a present backend) ──


def test_no_jail_flag_skips_present_backend(cfg: CodexCrewConfig) -> None:
...
def test_env_no_jail_skips_present_backend(cfg: CodexCrewConfig, monkeypatch) -> None:
    """CODEXCREW_NO_JAIL=1 is the documented env-only bypass and IS read at the gate."""
    jail = _RealJailReturns(0)
    _install_jail(cfg, jail, jail_mode="on")
    monkeypatch.setenv("CODEXCREW_NO_JAIL", "1")
    cli._jail_reexec_gate("chat", no_jail_flag=False)  # no SystemExit
    assert jail.calls == []


@pytest.mark.parametrize("falsey", ["0", "false", "no", "", " "])
def test_env_no_jail_falsey_does_not_disable(cfg: CodexCrewConfig, monkeypatch, falsey) -> None:
    """CODEXCREW_NO_JAIL=0/false/no must NOT disable the jail (truthy-set, not bool()).
    ...
    """
    _install_jail(cfg, _RealJailReturnsNone(), jail_mode="on")
    monkeypatch.setenv("CODEXCREW_NO_JAIL", falsey)
    with pytest.raises(SystemExit) as ei:
        cli._jail_reexec_gate("chat", no_jail_flag=False)
    assert ei.value.code == 2
...
def test_env_no_jail_truthy_variants_disable(cfg: CodexCrewConfig, monkeypatch, truthy) -> None:
    """Case/space-insensitive truthy values DO disable the jail (the .strip().lower() path)."""
    jail = _RealJailReturns(0)
    _install_jail(cfg, jail, jail_mode="on")
    monkeypatch.setenv("CODEXCREW_NO_JAIL", truthy)
    cli._jail_reexec_gate("chat", no_jail_flag=False)  # no SystemExit
    assert jail.calls == []  # backend never invoked
...
def test_off_mode_skips_present_backend(cfg: CodexCrewConfig) -> None:
...
def test_garbage_mode_normalizes_to_auto(cfg: CodexCrewConfig) -> None:
...
def test_reentry_marker_short_circuits(cfg: CodexCrewConfig, monkeypatch, marker) -> None:
    """CODEXCREW_JAILED PRESENT (any non-empty value) → gate returns without re-jailing,
...
def test_empty_marker_does_not_short_circuit(cfg: CodexCrewConfig, monkeypatch) -> None:
    """An EMPTY CODEXCREW_JAILED is NOT 'already jailed' (presence = non-empty), so the
...
def test_marker_restored_after_degrade(cfg: CodexCrewConfig, monkeypatch) -> None:
    """try/finally invariant: the gate must NOT leak CODEXCREW_JAILED after the backend
...
def test_marker_restored_before_on_mode_exit(cfg: CodexCrewConfig, monkeypatch) -> None:
...
# ── R5: _child_argv reuses _resolve_codexcrew_bin incl. the sentinel branch ──


def test_child_argv_sentinel_falls_back_to_module(monkeypatch) -> None:
    """When _resolve_codexcrew_bin returns the bare 'codexcrew' sentinel (no usable
    binary), _child_argv falls back to ``python -m codex_crew`` with sys.argv[1:]."""
    import codex_crew.agent as agent_mod

    monkeypatch.setattr(agent_mod, "_resolve_codexcrew_bin", lambda: "codexcrew")
    monkeypatch.setattr(cli.sys, "argv", ["codexcrew", "chat", "--model", "x"])
    assert cli._child_argv() == [cli.sys.executable, "-m", "codex_crew", "chat", "--model", "x"]
...
def test_child_argv_resolved_path_used(monkeypatch) -> None:
    ...
    monkeypatch.setattr(agent_mod, "_resolve_codexcrew_bin", lambda: "/opt/venv/bin/codexcrew")
    monkeypatch.setattr(cli.sys, "argv", ["codexcrew", "run", "TASK.md"])
    assert cli._child_argv() == ["/opt/venv/bin/codexcrew", "run", "TASK.md"]
...
def test_no_jail_flag_accepted_in_both_orderings(monkeypatch, argv) -> None:
    """`codexcrew --no-jail <cmd>` AND `codexcrew <cmd> --no-jail` both set no_jail=True
    ...
    monkeypatch.setattr(cli.sys, "argv", ["codexcrew", *argv])
    with pytest.raises(SystemExit) as ei:
        cli.main()
    assert ei.value.code == 0
...
def test_no_jail_default_false_when_absent(monkeypatch) -> None:
    ...
    monkeypatch.setattr(cli, "boot_platform", lambda *a, **k: None)
    monkeypatch.setattr(cli, "_jail_reexec_gate", _fake_gate)
    monkeypatch.setattr(cli.sys, "argv", ["codexcrew", "chat"])
    with pytest.raises(SystemExit):
        cli.main()
...
def test_build_provider_factory_degrades_to_public(cfg: CodexCrewConfig) -> None:
...
def test_build_provider_factory_reraises_composition_error(cfg: CodexCrewConfig) -> None:
...
def test_build_provider_factory_default_delegates_to_cfg(cfg: CodexCrewConfig) -> None:
...
def test_default_dashboard_contributor_is_noop(cfg: CodexCrewConfig) -> None:
    from codex_crew.platform.defaults import DefaultDashboardContributor
...
@pytest.mark.asyncio
async def test_default_dashboard_services_symmetric_app(cfg: CodexCrewConfig) -> None:
...
def test_default_jail_provider_noop() -> None:
    from codex_crew.platform.defaults import DefaultJailProvider

    j = DefaultJailProvider()
    assert j.available() is False
    assert j.maybe_reexec_into_jail(["codexcrew", "chat"], "on") is None
...
def _merge_connectors(cfg: CodexCrewConfig):
    """Mirror of the production connector-merge (built-ins first, then the seam)."""
    from codex_crew.platform import current_context, safe_context_call
...
def test_knowledge_extra_connectors_merges_after_builtins(cfg: CodexCrewConfig) -> None:
    base = build_default_context(cfg, profile=PROFILE_ENTERPRISE)
    extra = {"quip": object()}
...
def test_knowledge_extra_connectors_degrades_to_builtins(cfg: CodexCrewConfig) -> None:
    base = build_default_context(cfg, profile=PROFILE_ENTERPRISE)
...
def test_knowledge_extra_connectors_reraises_composition_error(cfg: CodexCrewConfig) -> None:
    base = build_default_context(cfg, profile=PROFILE_ENTERPRISE)
    set_context(
...
def test_default_knowledge_provider_empty(cfg: CodexCrewConfig) -> None:
    from codex_crew.platform.defaults import DefaultKnowledgeProvider
...
def test_dashboard_sso_login_handler_degrades_to_stub(cfg: CodexCrewConfig) -> None:
    from codex_crew.platform import current_context, safe_context_call
...
def test_dashboard_contribute_routes_reraises_composition_error(cfg: CodexCrewConfig) -> None:
    from codex_crew.platform import current_context, safe_context_call
...
@pytest.mark.asyncio
async def test_dashboard_start_services_degrades(cfg: CodexCrewConfig) -> None:
    from codex_crew.platform import async_safe_context_call, current_context
...
@pytest.mark.asyncio
async def test_dashboard_start_services_reraises_composition_error(cfg: CodexCrewConfig) -> None:
    from codex_crew.platform import async_safe_context_call, current_context
...
def test_knowledge_extra_connectors_site_uses_safe_context_call() -> None:
    ...
    src = _read_source("codex_crew.dashboard.handlers.knowledge")
...
def test_dashboard_contributor_sites_use_safe_context_call() -> None:
    ...
    src = _read_source("codex_crew.dashboard.server")
    ...
    src += _read_source("codex_crew.dashboard.routes.connections")
