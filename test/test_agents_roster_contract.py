...
which made its response contract "every field ``CodexCrewAgentConfig`` has now,
...
so a field added to ``CodexCrewAgentConfig`` fails here rather than silently
...
from codex_crew.config.sections import CodexCrewAgentConfig, _safe_avatar
...
# (``website/src/components/AgentSelector.tsx`` declares the ``CodexCrewAgent``
...
def _make_app() -> web.Application:
    ...
    from codex_crew.dashboard.handlers import api_codexcrew_agents

    app = web.Application()
    app.router.add_get("/api/agents", api_codexcrew_agents)
    return app
...
class TestRosterRowKeySet:
    ...
    async def test_project_row_ships_the_same_key_set(self, monkeypatch) -> None:
        ...
        """
        monkeypatch.setattr(
            "codex_crew.dashboard.handlers.agents.active_project_dir",
            lambda state, key: "/probe/project",
        )
        monkeypatch.setattr(
            "codex_crew.dashboard.handlers.agents.project_agent_names",
...
class TestRosterRowIsAnAllowlistNotASpread:
    ...
    def test_record_field_added_later_is_not_shipped_by_omission(self) -> None:
        """Every ``CodexCrewAgentConfig`` field is either allowlisted or withheld.
        ...
        """
        record_fields = {f.name for f in dataclasses.fields(CodexCrewAgentConfig)}
        unclassified = record_fields - ROSTER_ROW_KEYS - WITHHELD_RECORD_FIELDS
        assert not unclassified, (
            f"CodexCrewAgentConfig grew {sorted(unclassified)}. GET /api/agents is an "
    ...
    def test_an_attribute_the_allowlist_does_not_name_is_dropped(self) -> None:
        ...
        def _clone(f: dataclasses.Field) -> dataclasses.Field:
            ...
            ``CodexCrewAgentConfig()`` instantiates fine on a clean tree.
            """
        ...
        grown = dataclasses.make_dataclass(
            "GrownAgentConfig",
            [(f.name, f.type, _clone(f)) for f in dataclasses.fields(CodexCrewAgentConfig)]
            + [("secret_future_field", str, dataclasses.field(default="must-not-ship"))],
        )
        row = _agent_roster_row("grown", "global", cast(CodexCrewAgentConfig, grown()), redact=False)
        assert set(row) == ROSTER_ROW_KEYS
...
class TestUnshowableValuesAreMasked:
    ...
    def _full(self) -> CodexCrewAgentConfig:
        return CodexCrewAgentConfig(
    ...
    def test_owner_keeps_name_addressable_but_app_token_does_not_need_it(self) -> None:
        ...
        are readable unmasked from ``GET /api/config/codexcrew`` as the ``agents``
        ...
        g = _agent_roster_row(f"crew-{self.PROBE}", "global", CodexCrewAgentConfig(), redact=False)
        assert g["name"] == f"crew-{self.PROBE}"
        assert g["scope"] == "global"
        # Global + app token: masked, because an app cannot reach those routes.
        ga = _agent_roster_row(f"crew-{self.PROBE}", "global", CodexCrewAgentConfig(), redact=True)
        assert _carries_mask(ga["name"])
        # Project: masked for BOTH callers, because nothing addresses a project row.
        for redact in (False, True):
            p = _agent_roster_row(
                f"crew-{self.PROBE}", "project", CodexCrewAgentConfig(), redact=redact
            )
        ...
        for scope in ("global", "project"):
            for redact in (False, True):
                row = _agent_roster_row("my-agent", scope, CodexCrewAgentConfig(), redact=redact)
    ...
    def test_benign_content_is_never_altered(self, redact: bool) -> None:
        ...
        cfg = CodexCrewAgentConfig(description="a plain crew", triggers="use for triage")
        row = _agent_roster_row("codexcrew", "global", cfg, redact=redact)
        assert row["description"] == "a plain crew"
        assert row["triggers"] == "use for triage"
        assert row["name"] == "codexcrew"
    ...
    def test_both_caller_classes_ship_the_same_key_set(self) -> None:
        ...
        cfg = CodexCrewAgentConfig(description="d", triggers="t")
        ...
        row = _agent_roster_row(
            ...
            cast(
                ...
                types.SimpleNamespace(
                    **{
                        **{f.name: "" for f in dataclasses.fields(CodexCrewAgentConfig)},
...
class TestCredentialShapedNamesAreRefusedAtCreation:
    ...
    def test_the_create_rule_is_keyed_to_the_read_rule(self) -> None:
        ...
        for benign in ("oncall", "codexcrew", "crew-7", "release manager"):
    ...
    async def test_creation_refuses_a_credential_shaped_name(self) -> None:
        ...
        try:
            with unittest.mock.patch("codex_crew.config.loader.config_path", return_value=tmp):
                from codex_crew.dashboard.handlers import api_codexcrew_agents_create
                ...
                async with TestClient(TestServer(app)) as client:
                    resp = await client.post(
                        "/api/agents",
                        json={"name": self.PROBE, "kiro_agent": "codexcrew"},
                    )
...
class TestCallerClassIsTheOwnerPredicate:
    ...
    async def test_an_echoed_avatar_does_not_overwrite_a_masked_trait(self) -> None:
        ...
        try:
            with (
                unittest.mock.patch("codex_crew.config.loader.config_path", return_value=tmp),
                unittest.mock.patch(
...
class TestMaskIsTreatedAsUnchangedOnWrite:
    ...
    def test_the_sentinel_is_the_one_the_config_endpoint_uses(self) -> None:
        ...
        row = _agent_roster_row(
            "probe", "global", CodexCrewAgentConfig(description=f"see {self.PROBE}"), redact=False
        )
