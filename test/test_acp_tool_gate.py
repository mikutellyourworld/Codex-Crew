...
from codex_crew import acp_tool_gate as gate
...
def test_every_session_config_harness_names_its_option() -> None:
    ...
    """
    from codex_crew.acp_backends import ACP_BACKEND_ROUTING
...
def test_mask_reanchors_a_relocated_credential(
    ...
    who relocated a store with ``CLAUDE_CONFIG_DIR`` (or ``CODEXCREW_HOME``) kept the
...
def test_sandbox_off_is_allowed_for_an_unenforced_harness(
    ...
    monkeypatch.setattr(sandbox, "_governance_sandbox_floor", lambda: None)
    monkeypatch.setattr(sandbox, "_inside_codexcrew_sandbox", lambda: False)
...
def test_env_root_override_is_read_verbatim(
    ...
    ``_valid_override_home`` takes ``CODEXCREW_HOME`` raw and ``config_dir`` mkdirs
    ...
    monkeypatch.setenv("CODEXCREW_HOME", raw)
    expected = str(pathlib.Path(raw).expanduser().resolve())
    assert security._resolved_env_root("CODEXCREW_HOME") == expected
    monkeypatch.setenv("CODEXCREW_HOME", "")
    assert security._resolved_env_root("CODEXCREW_HOME") is None
...
def test_native_pre_tool_gate_replaces_the_missing_windows_mask(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A verified native gate closes the passive-read gap before tool execution.

    Codex 0.153.3+ invokes PreToolUse for every native tool, including reads that
    do not raise an ACP permission request. Once Codex Crew injects its own hook,
    the OS credential mask is defense in depth rather than the only control, so a
    Windows host with no outer sandbox backend can start the adapter.
    """
    from codex_crew import sandbox
    ...
    gate.enforce_sandbox_floor(ACP_BACKEND_CODEX, "standard", native_tool_gate=True)


def test_native_gate_keeps_the_outer_mask_on_supported_hosts(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The Windows exception must not remove defense in depth elsewhere."""
    from codex_crew.acp import client

    original_platform = client.sys.platform
    monkeypatch.setattr(client.sys, "platform", "linux")
    monkeypatch.setattr(gate, "enforce_sandbox_floor", lambda *args, **kwargs: None)
    monkeypatch.setattr(gate, "adapter_hidden_credential_dirs", lambda backend: ("/masked",))

    try:
        assert client._sandbox_preflight(ACP_BACKEND_CODEX, "standard", native_tool_gate=True) == (
            "/masked",
        )
    finally:
        client.sys.platform = original_platform
...
def test_no_backend_verdict_ignores_the_mutable_opt_in() -> None:
    ...
    for opted_in in (True, False):
        with (
            ...
            _mock.patch.object(sandbox, "_inside_codexcrew_sandbox", lambda: False),
...
def test_nested_sandbox_passthrough_refuses_an_enforced_adapter(
    ...
    """Inside an existing Codex Crew sandbox the mask is never applied either.
    ...
    monkeypatch.setattr(sandbox, "_governance_sandbox_floor", lambda: None)
    monkeypatch.setattr(sandbox, "_inside_codexcrew_sandbox", lambda: True)
