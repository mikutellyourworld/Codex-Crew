"""Tests for codex_crew.secrets.migrate — plaintext .env → vault importer."""
...
def _migrate(config_dir: Path, ep: Path, **kwargs):
    ...
    """
    with (
        patch("codex_crew.secrets.migrate.env_path", return_value=ep),
        patch("codex_crew.secrets.migrate.config_dir", return_value=config_dir),
    ):
...
def test_stale_plaintext_behind_secret_ref_not_queued(tmp_path: Path) -> None:
    ...
    # would have left it.
    from codex_crew.secrets.migrate import migrate_env_secrets as _m  # noqa: F401
...
def test_lock_held_by_another_process_aborts_cleanly(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "codex_crew.secrets.migrate.platform_compat.try_acquire_lock",
...
def test_vault_entry_replaced_between_store_and_rewrite_aborts(tmp_path: Path, monkeypatch) -> None:
    ...
    from codex_crew.secrets.vault import SecretValue
...
def test_vault_delete_during_held_lock_cannot_strand_plaintext(tmp_path: Path, monkeypatch) -> None:
    ...
    from codex_crew.secrets.vault import SecretVault
    ...
    import codex_crew.atomic_write as _atomic_mod
    import codex_crew.secrets.migrate as _migrate_mod
...
def test_atomic_write_failure_rolls_back_vault_entries(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("codex_crew.secrets.migrate.atomic_write", _boom)
...
def test_rollback_delete_failure_records_orphaned_key_and_propagates_error(
    ...
    with caplog.at_level(logging.WARNING, logger="codex_crew.secrets.migrate"):
        with pytest.raises(OSError, match="disk full"):
            _migrate(config_dir_path, ep, dry_run=False)
...
class TestRemediationNamesOnlyRealCliSubcommands:
    """Every ``codexcrew secrets <verb>`` a shipped message tells an operator to
    run must be a verb the CLI actually registers.

    The defect this pins (#6889): the importer's remediation strings named
    ``codexcrew secrets set`` and ``codexcrew secrets rm``, but the ``secrets``
    ...
    def _verb_is_registered(verb: str, monkeypatch, tmp_path, capsys) -> bool:
        """True when ``codexcrew secrets <verb> --help`` parses.
        ...
        * ``main()`` unconditionally pops ``CODEXCREW_SANDBOX_ACTIVE`` and
          ``CODEXCREW_SANDBOX_LEVEL`` to refuse an inherited sandbox-bypass
        ...
        for marker in ("CODEXCREW_SANDBOX_ACTIVE", "CODEXCREW_SANDBOX_LEVEL"):
            monkeypatch.delenv(marker, raising=False)
        monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path / "data"))
        ...
        monkeypatch.setattr(sys, "argv", ["codexcrew", "secrets", verb, "--help"])
    ...
    def test_the_scan_finds_something(self):
        ...
        assert self._referenced_verbs(), "no `codexcrew secrets <verb>` reference found under src/"
    ...
    def test_every_referenced_verb_is_registered(self, monkeypatch, tmp_path, capsys):
        ...
        assert not dead, (
            "shipped remediation text tells the operator to run a `codexcrew secrets` "
            f"subcommand the CLI does not register: {dead}. Point the message at a real "
            "surface (Settings > Secrets in the dashboard, or `codexcrew secrets import`)."
        )
