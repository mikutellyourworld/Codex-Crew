...
3. the deliberate departure from ``history.py``: ``CODEXCREW_DEV_MODE`` alone
...
from codex_crew import on_loop_db
from codex_crew.knowledge.store import _ON_LOOP_DB_GUARD, KnowledgeStore
from codex_crew.on_loop_db import (
    STORE_STRICT_ENV,
    STRICT_ENV,
    OnLoopDBGuard,
    OnLoopStoreError,
...
class TestOnLoopGuard:
    ...
    async def test_on_loop_db_raises_under_strict(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CODEXCREW_STRICT_ON_LOOP_STORE", "1")
    ...
    async def test_on_loop_db_warns_in_production_mode(self, tmp_path, monkeypatch, caplog):
        ...
        monkeypatch.setenv("CODEXCREW_STRICT_ON_LOOP_STORE", "0")
        st = await asyncio.to_thread(KnowledgeStore, str(tmp_path / "k.db"))
        with caplog.at_level("WARNING", logger="codex_crew.on_loop_db"):
...
class TestDevModeDeparture:
    """This store deliberately does NOT let ``CODEXCREW_DEV_MODE`` alone raise.
    ...
    response -- unset ``CODEXCREW_DEV_MODE`` -- would silence history.py's guard
    ...
    async def test_dev_mode_alone_warns_but_does_not_raise(self, tmp_path, monkeypatch, caplog):
        monkeypatch.delenv("CODEXCREW_STRICT_ON_LOOP_STORE", raising=False)
...
class TestSharedSwitchCannotArmThisStore:
    ...
    ``CODEXCREW_STRICT_ON_LOOP_PERSIST=1`` into the e2e gateway, scoped when they
...
class TestConstructionOnLoopIsSanctioned:
    ...
    def _guard_records(caplog):
        ...
        return [r for r in caplog.records if r.name == "codex_crew.on_loop_db"]
...
class TestStrictSwitchDoesNotDrift:
    ...
    CASES = [
        {},
        {"CODEXCREW_STRICT_ON_LOOP_PERSIST": "1"},
        {"CODEXCREW_STRICT_ON_LOOP_PERSIST": "true"},
        {"CODEXCREW_STRICT_ON_LOOP_PERSIST": "0"},
        {"CODEXCREW_STRICT_ON_LOOP_PERSIST": "off"},
        {"CODEXCREW_DEV_MODE": "1"},
        {"CODEXCREW_DEV_MODE": "yes"},
        {"CODEXCREW_DEV_MODE": "0"},
        # explicit falsy must force-disable even under dev mode, in both readers
        {"CODEXCREW_DEV_MODE": "1", "CODEXCREW_STRICT_ON_LOOP_PERSIST": "0"},
        {"CODEXCREW_DEV_MODE": "1", "CODEXCREW_STRICT_ON_LOOP_PERSIST": "1"},
        # an unrecognised value is neither truthy nor falsy in either reader
        {"CODEXCREW_STRICT_ON_LOOP_PERSIST": "maybe"},
        {"CODEXCREW_DEV_MODE": "1", "CODEXCREW_STRICT_ON_LOOP_PERSIST": "maybe"},
    ]

    def test_agrees_with_history_when_dev_mode_is_armed(self, monkeypatch):
        from codex_crew.history import on_loop_persist_strict

        for env in self.CASES:
            for var in ("CODEXCREW_STRICT_ON_LOOP_PERSIST", "CODEXCREW_DEV_MODE"):
    ...
    def test_excluding_dev_mode_differs_only_on_the_dev_mode_arm(self, monkeypatch):
        ...
        for env in self.CASES:
            ...
            for key, value in env.items():
                monkeypatch.setenv(key, value)
            explicit = env.get("CODEXCREW_STRICT_ON_LOOP_PERSIST", "").strip().lower()
