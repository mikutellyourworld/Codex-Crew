"""Tests for codex_crew.webex.client (WebexClient, low-level layer)."""
...
from codex_crew.messaging.split import chunk_utf8_bytes
from codex_crew.webex import client as webex_client
from codex_crew.webex.client import (
    WEBEX_MAX_TEXT,
    WebexClient,
    WebexInbound,
    hydra_id,
...
class TestRedeliveryHandling:
    ...
    async def test_the_dedup_memory_is_bounded(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.webex.client._DEDUP_WINDOW", 4)
        c = _client()
        self._wired(monkeypatch, c)
...
class TestClusterRouting:
    ...
    def test_a_cluster_round_trips_through_the_id(self) -> None:
        from codex_crew.webex.client import cluster_of, hydra_id
    ...
    def test_a_non_hydra_value_yields_no_cluster(self, bad: str) -> None:
        from codex_crew.webex.client import cluster_of
    ...
    async def test_a_readable_press_is_dispatched_and_acked(
        ...
        async def _api(_method: str, path: str, _payload, **_kw):
            if path.startswith("/attachment/actions/"):
                return {"inputs": {"codexcrew_kind": "options"}, "roomId": "R1", "personId": "P1"}
        ...
        assert len(received) == 1
        assert received[0].card_inputs == {"codexcrew_kind": "options"}
...
class TestReadLoopFrameDispatch:
    ...
    async def test_malformed_binary_frame_does_not_crash_the_loop(
        ...
        with caplog.at_level(logging.DEBUG, logger="codex_crew.webex.client"):
            await c._connect_and_serve()
