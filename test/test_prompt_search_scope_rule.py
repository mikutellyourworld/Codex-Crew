...
CodexCrew's only lever over the kiro-cli-native ``grep``/``glob`` tools is the
...
CONFIG_DIR = Path(__file__).resolve().parents[1] / "src" / "codex_crew" / "config"
...
@pytest.mark.parametrize("name", PROMPTS)
def test_prompt_requires_same_turn_follow_through(name: str) -> None:
    text = (CONFIG_DIR / name).read_text(encoding="utf-8")
    assert "invoke the corresponding tool in the same turn" in text
    assert "Never end on an acknowledgement or announced next step" in text
