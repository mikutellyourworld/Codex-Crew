...
def test_absent_key_and_absent_section_are_not_drift():
    ...
    assert CodexCrewConfig().mcp_gateway.forward_declared_env is True
...
def test_every_registered_key_ends_at_the_live_default():
    ...
    for dotted, entry in newest.items():
        ...
        live = getattr(getattr(CodexCrewConfig(), section), field)
        ...
        assert any(
            f.name == field for f in dc_fields(getattr(CodexCrewConfig(), section))
...
def test_dropping_a_key_makes_the_current_default_apply(tmp_path, monkeypatch):
    ...
    assert CodexCrewConfig.load().session.autocompact_pct == 70.0
...
def test_many_drifted_keys_produce_one_warning_line(tmp_path, monkeypatch, caplog):
    ...
    for key in (
        ...
        assert key in text
    assert "codexcrew config defaults" in text
...
def test_doctor_offers_the_commands_when_something_is_unacked(tmp_path, monkeypatch, capsys):
    ...
    assert "codexcrew config defaults --adopt" in out
    assert "codexcrew config defaults --keep" in out
    assert issues == []
