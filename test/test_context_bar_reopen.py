...
def _isolate_snapshot_file(tmp_path, monkeypatch):
    ...
    ~/.codex-crew/context_snapshots.json and a stray entry there would change
...
def test_repeating_the_same_reading_leaves_nothing_to_flush(tmp_path, monkeypatch):
    ...
    state.broadcast_context_usage("s1", {"slot": "s1", "pct": 11.4})
    writes = MagicMock()
    monkeypatch.setattr("codex_crew.dashboard.state.atomic_write", writes)
    _flush(state)
...
def test_failed_write_is_retried_on_the_next_flush(tmp_path, monkeypatch):
    ...
    from codex_crew.dashboard.state import atomic_write as real_atomic_write

    monkeypatch.setattr(
        "codex_crew.dashboard.state.atomic_write",
        MagicMock(side_effect=OSError("disk full")),
    )
    _flush(state)  # must swallow, not raise

    monkeypatch.setattr("codex_crew.dashboard.state.atomic_write", real_atomic_write)
    _flush(state)
...
def test_overlapping_flushes_cannot_roll_the_file_back(tmp_path, monkeypatch):
    ...
    from codex_crew.dashboard.state import atomic_write as real_write
    ...
    monkeypatch.setattr("codex_crew.dashboard.state.atomic_write", stalled_first_write)
