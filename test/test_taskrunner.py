...
import codex_crew.taskrunner as taskrunner_module
from conftest import requires_git
from codex_crew.task_models import PROGRESS_FILE
...
from codex_crew.workflows.store import WorkflowRunStore
...
class TestWorkflowRunIntegration:
    ...
    async def test_concurrent_plans_get_distinct_ids_when_the_clock_repeats(
        ...
        with patch("codex_crew.taskrunner.time.time_ns", return_value=123):
...
class TestParseSteps:
    ...
    def test_unparseable_error_log_is_bounded_and_redacted(self, caplog) -> None:
        ...
        with caplog.at_level(logging.ERROR, logger="codex_crew.task_planner"):
            assert runner._parse_tasks(text) == []
...
class TestSessionRecovery:
    ...
    async def test_mid_stream_context_overflow_exceeds_max_recoveries(self, tmp_path: Path) -> None:
        ...
        from codex_crew.providers.base import LLMEvent
        from codex_crew.task_executor import MAX_RECOVERIES
...
class TestWatchdog:
    ...
    async def test_watchdog_stall_notification(self) -> None:
        ...
        with patch("codex_crew.taskrunner.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
...
class TestSelfReview:
    ...
    async def testself_review_fails(self, tmp_path: Path) -> None:
        ...
        with patch(
            "codex_crew.task_executor.stream_and_collect_json",
...
class TestEdgeCases:
    ...
    async def testself_review_fail_then_retry_succeeds(self, tmp_path: Path) -> None:
        ...
        with patch("codex_crew.task_executor.self_review", side_effect=_review_once):
            success = await runner._execute_single_task(run, step, "key")
...
class TestGitCoord:
    ...
    async def test_init_workspace_existing_repo(self, tmp_path: Path) -> None:
        ...
        assert run.branch_name == "codexcrew/task/wt_test"
        assert run.worktree_path != ""
...
class TestWorkspaceDirValidation:
    ...
    def test_resolve_helper_blank_and_normal(self, tmp_path):
        from codex_crew.taskrunner import _resolve_workspace_dir
...
class TestMaxParallelStepsClamp:
    ...
    def _cap(self, value):
        ...
        with patch("codex_crew.taskrunner.compute_max_subagents", return_value=9):
    ...
    def test_compute_failure_falls_back_to_legacy_default(self):
        sessions = _make_mock_sessions()
        with patch("codex_crew.taskrunner.compute_max_subagents", side_effect=RuntimeError("boom")):
...
class TestSemaphoreParallelScheduling:
    ...
    async def test_semaphore_caps_concurrency_at_knob(
        ...
        """
        monkeypatch.setattr("codex_crew.taskrunner.compute_max_subagents", lambda _cfg: 64)
        sessions = _make_mock_sessions()
        runner = TaskRunner(
    ...
    async def test_host_ceiling_still_caps_the_knob(
        ...
        """
        monkeypatch.setattr("codex_crew.taskrunner.compute_max_subagents", lambda _cfg: 2)
        runner = TaskRunner(
            sessions=_make_mock_sessions(),
            auto_test=False,
            work_dir=tmp_path,
...
class TestNotifySessionKey:
    ...
    async def test_session_aware_sink_receives_the_originating_key(self, tmp_path: Path) -> None:
        ...
        runner, run = await self._start(tmp_path, _sink, "telegram:codexcrew:direct:U9")
        await runner._notify("Task 1 requires approval", "run the deploy?", run=run)

        assert seen == [("[spec] Task 1 requires approval", "telegram:codexcrew:direct:U9")]
    ...
    async def test_legacy_three_arg_sink_is_still_notified(self, tmp_path: Path) -> None:
        ...
        runner, run = await self._start(tmp_path, _legacy, "telegram:codexcrew:direct:U9")
        await runner._notify("Task 1 requires approval", "run the deploy?", run=run)
    ...
    async def test_delete_forgets_the_originating_key(self, tmp_path: Path) -> None:
        ...
        runner, run = await self._start(tmp_path, AsyncMock(), "telegram:codexcrew:direct:U9")
