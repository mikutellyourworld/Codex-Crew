...
class TestBusySlotQueueEntry:
    ...
    async def test_entry_carries_the_client_send_id(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_handlers._run_chat", MagicMock())
...
class TestDrainedRow:
    ...
    async def test_merged_row_names_every_send_id(self, tmp_path, monkeypatch):
        ...
        from codex_crew.dashboard import chat_runner
        from codex_crew.dashboard.chat_delivery import queue_for_next_turn
        ...
        monkeypatch.setattr(chat_runner, "CodexCrewConfig", cfg)
        await _drain_once(state, slot)
