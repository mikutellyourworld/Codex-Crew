...
from codex_crew.acp import session_handle
from codex_crew.acp.client import _DEFAULT_PROMPT_TIMEOUT
...
from codex_crew.config.loader import AgentConfig, CodexCrewConfig, WatchdogConfig
from codex_crew.constants import CHAT_TURN_TIMEOUT

_LOGGER_NAME = "codex_crew.acp.session_handle"
...
def _fake_config(*, turn_timeout: float = CHAT_TURN_TIMEOUT, **watchdog: float) -> CodexCrewConfig:
...
def _load_with(monkeypatch: pytest.MonkeyPatch, cfg: CodexCrewConfig) -> WatchdogSettings:
...
def test_load_failure_still_yields_defaults(monkeypatch: pytest.MonkeyPatch) -> None:
    def _boom(cls: type) -> CodexCrewConfig:
