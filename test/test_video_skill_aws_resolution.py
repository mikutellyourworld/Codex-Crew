...
_REFS = (
    _REPO_ROOT / "src/codex_crew/apps/builtins/dev_fleet/skills/feature-demo-recording/references"
)
...
class TestDepsSpeechProbe:
    ...
    def test_resolver_degrades_to_bare_name_without_codex_crew(self, monkeypatch):
        # A None entry makes ``from codex_crew.deploy.engine import ...`` raise
        # ImportError, which is the standalone-interpreter environment the
        # doctor must keep diagnosing instead of crashing on import.
        monkeypatch.setitem(sys.modules, "codex_crew.deploy.engine", None)
