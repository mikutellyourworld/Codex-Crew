...
async def _call(store, query: dict) -> dict:
    ...
    with patch(
        "codex_crew.dashboard.handlers.knowledge._store", return_value=store
    ):
...
async def test_list_items_source_id_filters_search_results(seeded):
    ...
    with (
        patch("codex_crew.dashboard.handlers.knowledge._store", return_value=store),
        patch(
            "codex_crew.dashboard.handlers.knowledge.HybridRetriever",
...
async def test_search_candidate_loading_runs_off_the_event_loop(seeded):
    ...
    with (
        ...
        patch(
            "codex_crew.dashboard.handlers.knowledge.asyncio.to_thread",
