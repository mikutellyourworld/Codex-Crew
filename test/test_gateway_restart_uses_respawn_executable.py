...
``codex_crew.platform.wheel_engine.respawn_executable``; these tests pin the
...
def _make_orchestrator() -> GatewayOrchestrator:
    cfg = CodexCrewConfig()
    with patch.object(cfg, "load_credentials", return_value={"CODEXCREW_OWNER_ID": "U1"}):
...
def exec_seams(monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.platform.wheel_engine.respawn_executable", respawn)
    reexec = MagicMock()
    monkeypatch.setattr("codex_crew.platform_compat.reexec_python_module", reexec)
...
class TestResolverIsLoadedBeforeTheApply:
    ...
    def _resolver_import_lines(fn: ast.AST) -> list[int]:
        return [
            ...
            and node.module == "codex_crew.platform.wheel_engine"
...
class TestAutoApplyWheelUpdate:
    ...
    async def test_successful_install_execs_the_respawn_interpreter(
        ...
        monkeypatch.setattr(
            "codex_crew.platform.update_governance.update_blocked_reason", lambda _base: None
        )
        monkeypatch.setattr("codex_crew.slack.gateway.sys.platform", "linux")
        monkeypatch.setattr("codex_crew.platform_compat.trusted_system_bin", lambda name: "/bin/sh")
        monkeypatch.setattr(
...
class TestAutoApplyGitUpdate:
    ...
    async def test_successful_update_execs_the_respawn_interpreter(
        ...
        monkeypatch.setattr("codex_crew.platform_compat.trusted_git_bin", lambda: "/usr/bin/git")
        ...
        monkeypatch.setattr(
            "codex_crew.platform.update_governance.update_blocked_reason", lambda _url: None
        )
