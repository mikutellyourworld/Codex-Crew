...
def test_authenticated_shutdown_request_uses_fixed_loopback_endpoint() -> None:
    ...
    with (
        patch("codex_crew.cli_server.run_marker.read_secret", return_value="local-secret"),
        patch("codex_crew.cli_server.loopback_urlopen", return_value=context) as mock_open,
    ):
...
def test_authenticated_shutdown_request_fails_closed_without_secret() -> None:
    ...
    with (
        patch("codex_crew.cli_server.run_marker.read_secret", return_value=""),
        patch("codex_crew.cli_server.loopback_urlopen") as mock_open,
    ):
...
def test_authenticated_shutdown_request_rejects_non_object_response() -> None:
    ...
    with (
        patch("codex_crew.cli_server.run_marker.read_secret", return_value="local-secret"),
        patch("codex_crew.cli_server.loopback_urlopen", return_value=context),
    ):
        assert _request_gateway_shutdown(7777) is False
...
def test_stop_uses_authenticated_shutdown_when_listener_lookup_is_blind(capsys) -> None:
    ...
    with (
        ...
        patch("codex_crew.cli_server._request_gateway_shutdown", return_value=True),
...
def test_restart_refuses_spawn_after_blind_authenticated_shutdown(
    ...
    mock_sel = MagicMock()
    with (
        patch("codex_crew.cli_server.resolve_client_port", return_value=7777),
        patch(
        ...
        patch("codex_crew.cli_server.run_marker.read_pid", return_value=marker_pid),
        patch(
            "codex_crew.cli_server.platform_compat.find_listening_pids",
            return_value=[],
        ),
        ...
        patch("codex_crew.cli_server._report_authenticated_shutdown", return_value=True),
