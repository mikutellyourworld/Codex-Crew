...
from codex_crew.mcp_cron import (
    _call_tool_inner,
...
BENIGN_LOOKALIKE_COMMANDS = [
    ...
    "TZ=UTC date",
    "TZ=UTC LANG=C date",
    "PYTHONUNBUFFERED=1 python3 ~/.codex-crew/crons/report.py",
...
BENIGN_COMMANDS = [
    ...
    "ls -la /tmp",
    "git status",
    "python3 ~/.codex-crew/crons/report.py",
...
class TestCronAddCommandGuard:
    def test_malicious_command_rejected_and_not_persisted(self, monkeypatch, tmp_path):
        ...
        monkeypatch.delenv("CODEXCREW_CHANNEL_ID", raising=False)
...
BENIGN_SCRIPTS_WITH_A_SEPARATOR_RUN = [
    ...
    'import re\nSCRUB = re.compile(r"/home/\\\\S*/\\\\.codex-crew/security_policy.json")\n',
...
COMMANDS_WITH_A_SEPARATOR_RUN = [
    ...
    r"tar -xf evil.tar -C $HOME//.codex-crew",
...
def test_authenticity_follows_the_module_through_an_alias():
    ...
    """
    from codex_crew.security import _re_module_is_authentic
...
def test_a_compile_result_bound_where_it_cannot_be_tracked_forfeits_exoneration():
    ...
    """
    from codex_crew.security import _compile_result_is_untrackable
...
def test_the_recovery_guard_denies_by_default_rather_than_enumerating():
    ...
    """
    from codex_crew.security import _compiled_name_escapes
...
def test_only_compile_results_are_tracked_as_compiled_patterns():
    ...
    """
    from codex_crew.security import _compiled_pattern_names
...
def test_call_spelled_reextraction_is_caught_like_the_dotted_spelling():
    ...
    """
    from codex_crew.security import _pattern_reextracted
...
def test_a_walrus_in_the_pattern_slot_forfeits_the_exoneration():
    ...
    """
    from codex_crew.security import _enclosing_call_slot
...
def test_fence_layer_checks_the_value_not_only_its_collapsed_copies():
    ...
    """
    from codex_crew.security import _fence_hit_in_collapsed, _separator_collapsed_variants
...
def test_shell_path_skips_only_the_scan_pass_1_already_did():
    ...
    assert is_sensitive_bash_command("tar -xf evil.tar -C $HOME//.codex-crew") is not None
...
def test_vet_script_contents_survives_a_deeply_nested_expression():
    ...
    """
    from codex_crew.security import _sensitive_run_in_source_literals
...
def test_a_callable_replacement_forfeits_the_pattern_slot():
    ...
    """
    fenced = r"/home/\\S*/\\.codex-crew/security_policy.json"
...
class TestCronAddScriptGuard:
    ...
    def _setup_home(self, monkeypatch, tmp_path):
        ...
        # CODEXCREW_HOME=tmp_path, config_dir() returns tmp_path, so the allowed
        # crons dir is tmp_path/crons. CODEXCREW_HOME also drives the CronService
        # store.
        monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path))
        monkeypatch.delenv("CODEXCREW_CHANNEL_ID", raising=False)
...
def test_run_command_uses_cc_sandbox(monkeypatch):
    ...
    """
    import codex_crew.cron_script as cs
...
def test_defaults_allowedtools_excludes_cron_add():
    import codex_crew
    defaults_path = Path(codex_crew.__file__).parent / "config" / "defaults.json"
    ...
    assert "@codexcrew-cron" not in allowed
    # cron_add / cron_update must NOT be auto-approved.
    assert "@codexcrew-cron/cron_add" not in allowed
    assert "@codexcrew-cron/cron_update" not in allowed
    # Safe read/manage tools remain auto-approved for the autonomous UX.
    assert "@codexcrew-cron/cron_list" in allowed
    # cron remains a usable capability (still declared in tools).
    assert "@codexcrew-cron" in cfg["tools"]
...
def test_blocked_command_emits_sel_denial(monkeypatch, tmp_path):
    ...
    import codex_crew.mcp_cron as mcp_cron_mod
