...
class TestContextMatches:
    ...
    def test_regex_complex_pattern(self):
        assert _context_matches(
            r"github\.com/mikutellyourworld/Codex-Crew|fix CI|ship it",
            "regex",
...
class TestScriptHookMatcherMode:
    ...
    def test_to_dict_includes_skills(self):
        hook = ScriptHook(name="test", skills=["codexcrew-dev/prepare-pr", "dev-fleet/pod-e2e"])
        d = hook.to_dict()
        assert d["skills"] == ["codexcrew-dev/prepare-pr", "dev-fleet/pod-e2e"]
        ...
        store.create(
            {
                ...
                "matcher_mode": "regex",
                "skills": ["codexcrew-dev/prepare-pr", "dev-fleet/pod-e2e"],
            }
        )
        ...
        store.create(
            {
                ...
                "matcher": r"\bPR\b|worktree",
                "matcher_mode": "regex",
                "skills": ["codexcrew-dev/prepare-pr"],
            }
        )
...
def test_hook_create_requires_command_or_skills():
    ...
    from codex_crew.validation import ValidationError, _validate_hook_has_action
...
class TestRegexValidationAtSave:
    ...
    def test_valid_regex_passes(self):
        from codex_crew.validation import _validate_hook_regex

        _validate_hook_regex({"matcher": r"\bPR\b", "matcher_mode": "regex"})

    def test_invalid_regex_rejected(self):
        from codex_crew.validation import ValidationError, _validate_hook_regex
...
class TestSkillsOnlyDeadConfigValidation:
    ...
    def test_skills_only_on_user_prompt_allowed(self):
        from codex_crew.validation import _validate_hook_has_action
