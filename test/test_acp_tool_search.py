...
on demand instead of sending every spec each turn. CodexCrew exposes it via the
...
class TestInitWiring:
    def test_kiro_enabled_applies_on_init(self):
        with patch("codex_crew.providers.acp.AcpClient") as mock_client, patch.object(
...
class TestConfigField:
    ...
    def test_load_reads_false_from_config(self, tmp_path):
        ...
        with unittest.mock.patch(
            "codex_crew.config.loader.config_path", return_value=cfg_file
        ):
