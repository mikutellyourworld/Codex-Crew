...
Gateway side (:mod:`codex_crew.mcp_gateway.app_call`) is tested against the
...
Dashboard side (:mod:`codex_crew.dashboard.handlers.mcp_apps`) is tested with
...
from codex_crew.dashboard.handlers import mcp_apps as mcp_apps_handlers
from codex_crew.mcp_apps_render import load_spool
from codex_crew.mcp_gateway import apps
from codex_crew.mcp_gateway.app_call import handle_app_call
from codex_crew.mcp_gateway.apps import write_spool
from codex_crew.mcp_gateway.backend import MCP_APPS_ENV_FLAG, Backend
from codex_crew.mcp_gateway.pool import BackendPool, PoolKey
...
async def test_app_call_cancellation_is_audited(apps_flag_on, spool_tmp, monkeypatch):
    ...
    import codex_crew.mcp_gateway.app_call as app_call_mod
...
async def test_app_call_denied_by_governance_mcp_scope(apps_flag_on, spool_tmp, tmp_path, monkeypatch):
    ...
    authorities). Policy loaded via the real CODEXCREW_SECURITY_POLICY seam."""
    ...
    monkeypatch.setenv("CODEXCREW_SECURITY_POLICY", str(policy))
    live = await _spawn_pooled_server()
    try:
        spool_id = _spool_record()
        reply = await handle_app_call(live.pool, {
...
async def test_app_call_denied_when_the_reloaded_policy_signature_is_bad(
    ...
    from codex_crew.platform.governance import policy_signing_payload
...
async def test_app_call_still_denies_model_only_tool(apps_flag_on, spool_tmp):
    ...
    live = await _spawn_pooled_server()
    try:
        from codex_crew.mcp_gateway import app_call as app_call_mod
