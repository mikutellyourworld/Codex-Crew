...
from codex_crew.messaging import session_resume as core
from codex_crew.messaging.link import ChannelLink
from codex_crew.session_map import ConversationOwnershipConflict
from codex_crew.teams.cards import KIND_SESSION
from codex_crew.teams.client import TeamsInbound, TeamsSendError
from codex_crew.teams.transport_dispatch import TeamsDispatcher
...
def _dispatcher(
    ...
    d = TeamsDispatcher(
        ...
        cfg=SimpleNamespace(
            ...
            agent=SimpleNamespace(default_agent="codexcrew", approval_mode="interactive"),
...
class TestOwnerOnly:
    ...
    async def test_a_non_owner_is_refused_and_audited(self, monkeypatch) -> None:
        events: list[dict] = []
        monkeypatch.setattr(
            "codex_crew.messaging.session_resume.sel",
...
class TestRoutingComesFirst:
    ...
    async def test_stop_targets_the_resumed_session_not_the_native_one(self) -> None:
        ...
        import codex_crew.teams.transport_dispatch as mod
...
class TestAClickInAResumedConversation:
    ...
    async def test_approving_resolves_the_resumed_sessions_prompt(self) -> None:
        ...
        from codex_crew.teams.cards import DECISION_APPROVE, KIND_APPROVAL
    ...
    async def test_a_chip_runs_the_label_the_resumed_turn_offered(self, monkeypatch) -> None:
        from codex_crew.teams.cards import KIND_OPTION
    ...
    async def test_a_turn_that_started_before_the_bind_still_resolves(self) -> None:
        ...
        """
        from codex_crew.teams.approvals import TeamsApprovalDecider
        from codex_crew.teams.cards import DECISION_DENY, KIND_APPROVAL
...
class TestSharedWithDiscord:
    def test_the_expectation_stores_are_separate_files(self) -> None:
        ...
        """
        from codex_crew.messaging.resume_expectation import store_filename

        assert store_filename("teams") != store_filename("discord")

    def test_teams_and_discord_use_one_picker_bind_controller(self) -> None:
        from codex_crew.discord.session_resume import DiscordSessionResume
        from codex_crew.teams.session_resume import TeamsSessionResume
    ...
    async def test_controller_uses_the_surface_expectation_identity(self) -> None:
        ...
        """
        from codex_crew.teams.session_resume import TeamsSessionResume, _TeamsResumeSurface
    ...
    def test_the_teams_card_kind_is_distinct_from_the_other_two(self) -> None:
        ...
        from codex_crew.teams.cards import KIND_APPROVAL, KIND_OPTION
...
async def test_sessions_is_in_the_command_table_and_help() -> None:
    ...
    from codex_crew.teams.commands import COMMAND_SPEC, build_help_text, parse_command
