"""``codex_crew/BUILD_VERSION`` -- a distribution's build stamp over ``__version__``.
...
``/api/status``, ``/api/health``, ``codexcrew --version`` and the diagnostics
report all read ``codex_crew.__version__``, several through an import-time copy
(``dashboard/handlers/updates.py::_local_version``, ``ws.py``) that nothing
after import can reach. The stamp is a file BESIDE ``codex_crew/__init__.py``,
...
import codex_crew
from codex_crew import _BUILD_VERSION_FILENAME as BUILD_VERSION_FILENAME
from codex_crew import _build_version_override as build_version_override

REPO_ROOT = Path(__file__).resolve().parents[1]
SRC = REPO_ROOT / "src"
PKG_DIR = SRC / "codex_crew"
...
def test_no_file_keeps_the_base(tmp_path) -> None:
    assert codex_crew._apply_build_version_file("0.6.0", str(tmp_path)) == "0.6.0"


def test_no_package_dir_keeps_the_base() -> None:
    assert codex_crew._apply_build_version_file("0.6.0", "") == "0.6.0"
    assert codex_crew._apply_build_version_file("0.6.0", None) == "0.6.0"
...
def test_a_build_of_the_base_is_applied(tmp_path) -> None:
    ...
    assert codex_crew._apply_build_version_file("0.6.0", str(tmp_path)) == "0.6.0.12"
...
def test_a_foreign_value_is_refused_with_one_warning(tmp_path, caplog) -> None:
    ...
    with caplog.at_level("WARNING", logger="codex_crew"):
        assert codex_crew._apply_build_version_file("0.6.0", str(tmp_path)) == "0.6.0"
...
def test_an_oversized_file_is_refused_not_truncated(tmp_path, caplog) -> None:
    ...
    assert codex_crew._apply_build_version_file("0.6.0", str(tmp_path)) == "0.6.0.1234567890"
...
def _stamped_package_root(tmp_path: Path, stamp: str | None) -> Path:
    """A ``codex_crew`` package that IS the source tree's, with the file beside it.

    ``__init__.py`` is a byte copy of the real one (the code under test) plus a
    ``__path__`` line that points submodule resolution back at ``src/codex_crew``,
    so ``codex_crew.dashboard...`` imports the real modules while
    ``codex_crew.__file__`` -- the anchor the stamp is read beside -- is the
...
def _fresh_interpreter(tmp_path: Path, stamp: str | None, code: str) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [sys.executable, "-c", f"PKG = {str(root / 'codex_crew')!r}\n" + code],
        capture_output=True,
        text=True,
        encoding="utf-8",
        env=env,
...
import os
import codex_crew
from codex_crew.dashboard.handlers import updates
from codex_crew.dashboard import ws
from codex_crew.dashboard import handlers_system
from codex_crew import release_channel
from codex_crew.platform import update_governance
assert os.path.dirname(codex_crew.__file__) == PKG, codex_crew.__file__
assert os.path.dirname(updates.__file__) != PKG  # the REAL dashboard module
print("version", codex_crew.__version__)
print("updates", updates._local_version)
print("ws", ws._local_version)
print("display", updates.status_update_fields()["version_display"])
print("health", handlers_system.codex_crew.__version__)
...
def test_the_governance_floor_sees_the_stamp(tmp_path) -> None:
    ...
    code = (
        "import codex_crew\n"
        "from codex_crew.dashboard.handlers import updates\n"
        "from codex_crew.platform import update_governance as ug\n"
        "print('version', codex_crew.__version__)\n"
...
def test_a_child_leaves_no_bytecode_behind(tmp_path) -> None:
    proc = _fresh_interpreter(tmp_path, "0.0.0", "import codex_crew")
    assert proc.returncode == 0, proc.stderr
...
def test_changelog_marks_the_release_row_for_a_stamped_build() -> None:
    ...
    from codex_crew.changelog import build_release_list, running_release
...
def test_channel_move_compares_by_release_not_by_stamp() -> None:
    ...
    from codex_crew.changelog import release_of_build
    from codex_crew.dashboard.handlers.updates import _is_newer
...
def test_the_file_is_not_shipped_by_this_repo() -> None:
    ...
    assert f"src/codex_crew/{BUILD_VERSION_FILENAME}" in (REPO_ROOT / ".gitignore").read_text(
        encoding="utf-8"
    )
