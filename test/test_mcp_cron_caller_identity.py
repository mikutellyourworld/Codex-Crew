"""``codexcrew-cron`` resolves the calling session from the injected caller block.
...
from codex_crew import mcp_cron, mcp_shared, session_pid_sig
from codex_crew.cron import CronService
from codex_crew.mcp_caller import CallerContext, set_current_caller
from codex_crew.mcp_cron import _authz_session_key, _call_tool_inner
...
def _isolate(monkeypatch, tmp_path):
    ...
    """
    monkeypatch.setattr("codex_crew.cron._DEFAULT_DIR", tmp_path)
...
def test_the_caller_block_beats_the_process_environment(monkeypatch) -> None:
    ...
    """
    monkeypatch.setenv("CODEXCREW_SESSION_KEY", "dashboard:from-env")
...
#: like ``sh -c 'CODEXCREW_CLI=1 exec codexcrew-cron'`` -- differ only in which
...
def test_the_channel_default_also_comes_from_the_caller_block() -> None:
    """``CODEXCREW_CHANNEL_ID`` had the same defect as the session key.
...
def test_an_ownerless_row_is_outside_every_sessions_scope() -> None:
    ...
    Every creation path that has no session to name writes one -- ``codexcrew cron
