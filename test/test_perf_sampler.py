"""Tests for the debug-only performance sampler (codexcrew perf sample)."""
...
from codex_crew import cli_perf, perf_sampler
...
class TestGate:
    ...
    def test_other_values_stay_disabled(self, value):
        ...
        # CODEXCREW_DEBUG=0 silently enable profiling.
...
class TestParserWiring:
    def test_sample_flags_do_not_shadow_the_subcommand_dest(self):
        ...
        do nothing. Pin it.
        """
        parser = argparse.ArgumentParser(prog="codexcrew")
...
class TestStackSampler:
    ...
    def test_context_manager_stops_the_thread(self):
        ...
        assert not any(
            t.name == "codexcrew-perf-sampler" for t in __import__("threading").enumerate()
        )
...
class TestPySpy:
    ...
    def test_unavailable_message_explains_macos_and_the_dependency(self):
        ...
        # Names py-spy itself. `pip install codexcrew[perf]` cannot resolve --
        ...
        assert "codexcrew[" not in message
...
class TestResolveCallable:
    ...
    def test_rejects_non_callable(self):
        with pytest.raises(TypeError):
            cli_perf._resolve_callable("codex_crew.perf_sampler:DEBUG_ENV_VAR")

    def test_resolves_a_dotted_attribute(self):
        resolved = cli_perf._resolve_callable("codex_crew.perf_sampler:StackSampler.start")
