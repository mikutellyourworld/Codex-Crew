...
from codex_crew.dashboard.handlers.knowledge import ingest_file
...
@pytest.fixture
def mock_sel():
    with patch("codex_crew.dashboard.handlers.knowledge.sel") as m:
...
async def test_docx_zip_bomb_member_count_rejected_before_parse(mock_sel):
    ...
    with patch("codex_crew.dashboard.handlers.knowledge._MAX_INGEST_ARCHIVE_MEMBERS", 2):
        status, body = await _post(
            app, data, "bomb.docx",
            "application/vnd.openxmlformats-officedocument.wordprocessingml.document")
    assert status == 400, body
...
async def test_docx_zip_bomb_uncompressed_size_rejected_before_parse(mock_sel):
    ...
    with patch("codex_crew.dashboard.handlers.knowledge._MAX_INGEST_ARCHIVE_UNCOMPRESSED", 1):
        status, body = await _post(
            app, data, "bomb.docx",
            "application/vnd.openxmlformats-officedocument.wordprocessingml.document")
    assert status == 400, body
