...
def _stub_config(default_agent: str) -> CodexCrewConfig:
...
async def test_unloadable_config_still_creates_with_empty_agent(
    ...
    monkeypatch.setattr(chat_handlers, "CodexCrewConfig", SimpleNamespace(load=_boom))
...
def _alias_config(**aliases: Any) -> CodexCrewConfig:
    ...
    """
    from codex_crew.config.loader import CodexCrewAgentConfig, MemoryStoreConfig

    cfg = CodexCrewConfig()
    cfg.agents = {name: CodexCrewAgentConfig(**fields) for name, fields in aliases.items()}
...
class TestSameBindingGuard:
    ...
    async def test_different_memory_store_still_409s(
        ...
        cfg = _alias_config(
            **{
                "alias-a": {"kiro_agent": "codexcrew", "memory_store": "store-a"},
                "alias-b": {"kiro_agent": "codexcrew", "memory_store": "store-b"},
            }
        )
        monkeypatch.setattr(chat_handlers, "CodexCrewConfig", SimpleNamespace(load=lambda: cfg))
    ...
    async def test_identical_bindings_allowed_and_audited(
        ...
        cfg = _alias_config(
            **{
                "alias-a": {"kiro_agent": "codexcrew"},
                "alias-b": {"kiro_agent": "codexcrew"},
            }
        )
        monkeypatch.setattr(chat_handlers, "CodexCrewConfig", SimpleNamespace(load=lambda: cfg))
    ...
    async def test_project_agent_slot_still_409s_on_default_alias(
        ...
        cfg = _alias_config(**{"codexcrew": {"kiro_agent": "codexcrew"}})
        monkeypatch.setattr(chat_handlers, "CodexCrewConfig", SimpleNamespace(load=lambda: cfg))
        ...
        resp, _ = await _post_chat(
            dashboard_state, {"message": "hi", "slot": "proj-slot", "agent": "codexcrew"}
        )
        assert resp.status == 409
        assert events == ["denied_mismatch"]
