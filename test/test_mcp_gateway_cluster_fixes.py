...
from codex_crew.mcp_gateway.session_servers import (
...
class TestEnableRebuildsFactory:
    ...
    async def test_enable_calls_refresh_defaults(self) -> None:
        ...
        with patch.object(type(orch), "_init_mcp_gateway", new_callable=AsyncMock) as mock_init:
            ...
            with patch("codex_crew.slack.gateway.CodexCrewConfig") as mock_cfg:
...
class TestNamingMigration:
    """New naming emits CODEXCREW_* identifiers while still reading legacy
    MC_* / _mc_* values from overlays written by prior versions."""

    def test_new_marker_value_is_codexcrew_prefixed(self) -> None:
        assert "codexcrew" in _WRAPPER_MARKER.lower()
    ...
    def test_gatewayd_resolver_accepts_legacy_env_key(self) -> None:
        ...
        prefix when the new CODEXCREW_MCP_TARGET_ is absent."""
        from codex_crew.mcp_gateway.gatewayd import env_target_resolver
        from codex_crew.mcp_gateway.pool import PoolKey

        pk = PoolKey(
            server_name="test-srv",
            agent_name="codexcrew",
        ...
        with patch.dict(os.environ, env_patch, clear=False):
            ...
            os.environ.pop("CODEXCREW_MCP_TARGET_TEST_SRV", None)
            os.environ.pop("CODEXCREW_MCP_TARGET_TEST_SRV__abc123", None)
    ...
    def test_gatewayd_resolver_prefers_new_env_key(self) -> None:
        ...
        env_patch = {
            "CODEXCREW_MCP_TARGET_TEST_SRV": "/usr/bin/new-srv --stdio",
    ...
    def test_stub_accepts_codexcrew_mcp_socket_env(self) -> None:
        """The stub should prefer CODEXCREW_MCP_SOCKET over MC_MCP_SOCKET."""
        from codex_crew.mcp_gateway.stub import _parse_args

        with patch.dict(os.environ, {
            "CODEXCREW_MCP_SOCKET": "/tmp/new.sock",
    ...
    def test_stub_falls_back_to_legacy_mc_mcp_socket(self) -> None:
        ...
        env = dict(os.environ)
        env.pop("CODEXCREW_MCP_SOCKET", None)
    ...
    def test_rewriter_emits_new_env_key_prefix(self, tmp_path: Path) -> None:
        """rewrite_agents must populate target_env with CODEXCREW_MCP_TARGET_
        ...
        # Should have at least one CODEXCREW_MCP_TARGET_ key
        assert any(k.startswith("CODEXCREW_MCP_TARGET_") for k in target_env), (
            f"Expected CODEXCREW_MCP_TARGET_ prefix in target_env keys: {list(target_env.keys())}"
        )
    ...
    def test_idempotent_rewrite_upgrades_legacy_marker(self, tmp_path: Path) -> None:
        ...
        from codex_crew.mcp_gateway.rewriter import _rewrite_single_spec
