...
class TestMcpProcessCountLinux:
    ...
    def test_counts_all_signatures(self) -> None:
        fake_procs = {
            "10": b"python3\x00/tmp/codexcrew_sandbox_abc.py",
            "20": b"kiro-cli\x00acp\x00--agent\x00codexcrew",
        ...
        with (
            patch("codex_crew.dashboard.handlers_system.sys") as mock_sys,
            patch("codex_crew.dashboard.handlers_system.os.getpid", return_value=99999),
            patch("codex_crew.dashboard.handlers_system.os.listdir", side_effect=fake_listdir),
            ...
            patch(
                "codex_crew.dashboard.handlers_system.subprocess.check_output",
                return_value="%CPU\n0.0\n",
            ),
        ):
    ...
    def test_excludes_self(self) -> None:
        ...
        with (
            patch("codex_crew.dashboard.handlers_system.sys") as mock_sys,
            patch("codex_crew.dashboard.handlers_system.os.getpid", return_value=10),
            patch("codex_crew.dashboard.handlers_system.os.listdir", side_effect=fake_listdir),
            patch.object(Path, "read_bytes", fake_read_bytes),
...
class TestMcpProcessCountMacOS:
    ...
    def test_counts_all_signatures(self) -> None:
        ps_output = (
            "  PID COMMAND\n"
            "   10 python3 /tmp/codexcrew_sandbox_abc.py\n"
            "   20 kiro-cli acp --agent codexcrew\n"
    ...
    def test_excludes_self(self) -> None:
        ...
        with (
            patch("codex_crew.dashboard.handlers_system.sys") as mock_sys,
            patch("codex_crew.dashboard.handlers_system.os.getpid", return_value=42),
            patch(
                "codex_crew.dashboard.handlers_system.subprocess.check_output",
                return_value=ps_output,
            ),
    ...
    def test_ps_failure_returns_zeros(self) -> None:
        with (
            patch("codex_crew.dashboard.handlers_system.sys") as mock_sys,
            patch("codex_crew.dashboard.handlers_system.os.getpid", return_value=1),
            patch(
                "codex_crew.dashboard.handlers_system.subprocess.check_output",
