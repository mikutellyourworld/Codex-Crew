...
from codex_crew.config.loader import CodexCrewAgentConfig, CodexCrewConfig, coerce_effort
...
class TestCrewPinnedEffort:
    ...
    def _cfg(self, **crews: str) -> CodexCrewConfig:
        cfg = CodexCrewConfig()
        for name, effort in crews.items():
            cfg.agents[name] = CodexCrewAgentConfig(kiro_agent="codexcrew", reasoning_effort=effort)
        return cfg
...
class TestResolveSessionEffort:
    ...
    def _cfg(self, *, chat: str = "", background: str = "", crew_pin: str = "") -> CodexCrewConfig:
        cfg = CodexCrewConfig()
        cfg.agent.reasoning_effort = chat
        if background:
            cfg.agent.role_efforts = {"background": background}
        cfg.agents["worker"] = CodexCrewAgentConfig(
            kiro_agent="codexcrew-lite", reasoning_effort=crew_pin
        )
        return cfg

    def test_a_background_agent_takes_the_role_effort_not_the_chat_default(self):
        cfg = self._cfg(chat="low", background="high")
        assert cfg.resolve_session_effort("codexcrew-lite", "worker") == "high"
    ...
    def test_a_crew_named_after_a_background_agent_follows_its_own_binding(self):
        ...
        called codexcrew-lite while bound to a normal template runs that template,
        so it takes the chat default."""
        cfg = self._cfg(chat="low", background="high")
        cfg.agents["codexcrew-lite"] = CodexCrewAgentConfig(kiro_agent="codexcrew")
        assert cfg.resolve_session_effort("codexcrew-lite") == "low"
    ...
    def test_the_bare_background_key_still_resolves_the_role_effort(self):
        ...
        cfg = self._cfg(chat="low", background="high")
        assert cfg.resolve_session_effort("codexcrew-heartbeat", "") == "high"

    def test_a_normal_agent_takes_the_chat_default(self):
        cfg = self._cfg(chat="low", background="high")
        assert cfg.resolve_session_effort("codexcrew", "") == "low"

    def test_a_crew_pin_outranks_the_role_effort(self):
        cfg = self._cfg(chat="low", background="high", crew_pin="max")
        assert cfg.resolve_session_effort("codexcrew-lite", "worker") == "max"

    def test_empty_when_no_tier_pins_anything(self):
        assert self._cfg().resolve_session_effort("codexcrew", "") == ""
    ...
    def test_the_factory_and_the_resolver_cannot_disagree(self):
        ...
        src = inspect.getsource(loader.CodexCrewConfig.create_provider_factory)
        assert "resolve_session_effort" in src
        # The background-agent pair lives in ONE place now.
        assert '"codexcrew-lite"' not in src


class TestBackgroundWorkerAgents:
    def test_names_the_two_role_agents(self):
        from codex_crew.config.loader import BACKGROUND_WORKER_AGENTS

        assert BACKGROUND_WORKER_AGENTS == ("codexcrew-lite", "codexcrew-heartbeat")
...
class TestConfigLoadCoercesTheCrewPin:
    def test_a_valid_level_round_trips_through_save_and_load(self):
        ...
        cfg.agents["reviewer"] = CodexCrewAgentConfig(
            kiro_agent="codexcrew", reasoning_effort="xhigh"
        )
        cfg.save()

        assert CodexCrewConfig.load().agents["reviewer"].reasoning_effort == "xhigh"

    def test_a_hand_edited_junk_level_loads_as_inherit(self):
        cfg = CodexCrewConfig.load()
        cfg.agents["reviewer"] = CodexCrewAgentConfig(kiro_agent="codexcrew")
        cfg.save()
        ...
        assert CodexCrewConfig.load().agents["reviewer"].reasoning_effort == ""
...
class TestFactoryAppliesTheCrewPin:
    ...
    def _capture(self, *, crews: dict[str, str], config_effort: str = "", **factory_call):
        ...
        for name, effort in crews.items():
            cfg.agents[name] = CodexCrewAgentConfig(kiro_agent="codexcrew", reasoning_effort=effort)
        with patch("codex_crew.providers.acp.AcpProvider") as mock_provider:
    ...
    def test_a_pin_on_a_background_crew_beats_its_role_default(self):
        """``codexcrew-lite`` resolves the "background" role effort by name. A pin
        ...
        cfg.agents["codexcrew-lite"] = CodexCrewAgentConfig(
            kiro_agent="codexcrew-lite", reasoning_effort="high"
        )
        with patch("codex_crew.providers.acp.AcpProvider") as mock_provider:
            mock_provider.return_value = MagicMock()
            factory = cfg.create_provider_factory()
            factory(
                ...
                agent="codexcrew-lite",
                crew_agent="codexcrew-lite",
                model_override="claude-opus-4.7",
            )
...
class TestWarmPoolBypassesACrewPin:
    ...
    def _service(self, cfg: CodexCrewConfig):
        from codex_crew.session_allocation import SessionAllocationService
    ...
    def _cfg(self, effort: str) -> CodexCrewConfig:
        cfg = CodexCrewConfig()
        cfg.agents["reviewer"] = CodexCrewAgentConfig(kiro_agent="codexcrew", reasoning_effort=effort)
        return cfg
    ...
    def test_the_pool_decision_consults_the_probe(self):
        ...
        from codex_crew import session_allocation
...
class TestConcurrentRefreshesInstallInOrder:
    ...
    async def test_the_older_load_cannot_install_last(self):
        ...
        with (
            ...
            patch("codex_crew.session.CodexCrewConfig.load", side_effect=_staggered_load),
        ):
...
def _crud_app() -> web.Application:
    ...
    app = web.Application()
    app.router.add_post("/api/agents", api_codexcrew_agents_create)
    app.router.add_put("/api/agents/{name}", api_codexcrew_agent_update)
    return app
...
def seeded_agent():
    ...
    cfg = CodexCrewConfig.load()
    cfg.agents["existing"] = CodexCrewAgentConfig(
        kiro_agent="codexcrew", workspace="default", memory_store="default"
    )
    cfg.save()
    return "existing"
...
class TestApiValidatesTheCrewPin:
    ...
    async def test_create_stores_a_valid_level(self):
        async with TestClient(TestServer(_crud_app())) as client:
            resp = await client.post(
                "/api/agents",
                json={"name": "reviewer", "kiro_agent": "codexcrew", "reasoning_effort": "xhigh"},
            )
    ...
    async def test_create_refuses_an_unknown_level_and_stores_nothing(self):
        async with TestClient(TestServer(_crud_app())) as client:
            resp = await client.post(
                "/api/agents",
                json={"name": "reviewer", "kiro_agent": "codexcrew", "reasoning_effort": "ultra"},
            )
        ...
        assert "reviewer" not in CodexCrewConfig.load().agents

    @pytest.mark.asyncio
    async def test_create_without_the_field_defaults_to_inherit(self):
        async with TestClient(TestServer(_crud_app())) as client:
            resp = await client.post(
                "/api/agents", json={"name": "reviewer", "kiro_agent": "codexcrew"}
            )
            assert resp.status == 200
    ...
    async def test_update_stores_a_valid_level(self, seeded_agent):
        ...
        assert CodexCrewConfig.load().agents[seeded_agent].reasoning_effort == "high"
    ...
    async def test_update_clears_a_pin_with_the_empty_string(self, seeded_agent):
        ...
        assert CodexCrewConfig.load().agents[seeded_agent].reasoning_effort == ""
    ...
    async def test_update_refuses_an_unknown_level_and_writes_nothing(self, seeded_agent):
        ...
        stored = CodexCrewConfig.load().agents[seeded_agent]
...
class TestResolvedEndpointReportsEffort:
    ...
    async def test_reports_a_crew_pin_as_pinned(self, seeded_agent):
        from codex_crew.dashboard.handlers import api_codexcrew_agent_resolved_model
        ...
        app = web.Application()
        app.router.add_get("/api/agents/resolved-model", api_codexcrew_agent_resolved_model)
    ...
    async def test_reports_the_role_effort_for_a_background_agent_crew(self, seeded_agent):
        ...
        cfg = CodexCrewConfig.load()
        cfg.agents[seeded_agent].kiro_agent = "codexcrew-lite"
        cfg.agent.reasoning_effort = "low"
...
class TestSavingAPinRefreshesSessionDefaults:
    ...
    def _app(self, refreshed: list[str]) -> web.Application:
        ...
        app.router.add_post("/api/agents", api_codexcrew_agents_create)
        app.router.add_put("/api/agents/{name}", api_codexcrew_agent_update)
        app.router.add_delete("/api/agents/{name}", api_codexcrew_agent_delete)
    ...
    async def test_rebinding_kiro_agent_refreshes_even_with_no_pin(self, seeded_agent):
        ...
        async with TestClient(TestServer(self._app(refreshed))) as client:
            resp = await client.put(
                f"/api/agents/{seeded_agent}", json={"kiro_agent": "codexcrew-lite"}
            )
            assert resp.status == 200
        assert refreshed == ["yes"]
    ...
    async def test_create_refreshes_even_without_a_pin(self):
        ...
        async with TestClient(TestServer(self._app(refreshed))) as client:
            resp = await client.post(
                "/api/agents", json={"name": "worker", "kiro_agent": "codexcrew-lite"}
            )
            assert resp.status == 200
        assert refreshed == ["yes"]
    ...
    def test_the_trigger_is_derived_from_the_resolver_inputs(self):
        ...
        from codex_crew.config.loader import CodexCrewAgentConfig as Crew
        from codex_crew.dashboard.handlers.agents import _effort_inputs

        assert _effort_inputs(None) is None
        base = Crew(kiro_agent="codexcrew", reasoning_effort="high")
        assert _effort_inputs(base) == ("codexcrew", "high")
        # A junk stored level normalizes, so a no-op save of junk is not a change.
        assert _effort_inputs(Crew(kiro_agent="codexcrew", reasoning_effort="ultra")) == (
            "codexcrew",
            "",
        )
        # A field the chain does NOT read must not trigger a pool drain.
        assert _effort_inputs(
            Crew(kiro_agent="codexcrew", reasoning_effort="high", description="x")
        ) == _effort_inputs(base)
    ...
    async def test_a_failing_refresh_does_not_fail_the_save(self, seeded_agent):
        ...
        from codex_crew.dashboard.handlers import api_codexcrew_agent_update
