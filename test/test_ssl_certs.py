...
from codex_crew import _ssl_compat
from codex_crew._ssl_compat import _CA_CANDIDATES, _ensure_ssl_certs
...
class TestEnsureSslCerts:
    ...
    def test_sets_env_from_first_existing_candidate(self, monkeypatch, tmp_path):
        ...
        with (
            ...
            patch("codex_crew._ssl_compat._CA_CANDIDATES", candidates),
        ):
            _ensure_ssl_certs()
    ...
    def test_cli_invokes_ensure_ssl_certs(self):
        ...
        mock_fn = MagicMock()
        with _patch("codex_crew._ssl_compat._ensure_ssl_certs", mock_fn):
            import codex_crew.cli

            importlib.reload(codex_crew.cli)
    ...
    def test_macos_injection_still_exports_child_env(self, monkeypatch, tmp_path):
        ...
        with (
            ...
            patch.dict(sys.modules, {"certifi": mock_certifi}),
            patch("ssl.get_default_verify_paths", return_value=mock_paths),
            patch("codex_crew._ssl_compat._CA_CANDIDATES", ("/nonexistent/a.pem",)),
        ):
            _ensure_ssl_certs()
    ...
    def test_macos_injection_failure_falls_back(self, monkeypatch, tmp_path, caplog):
        ...
        with (
            patch.object(_ssl_compat, "truststore", mock_truststore),
            patch("ssl.get_default_verify_paths", return_value=mock_paths),
            caplog.at_level("WARNING", logger="codex_crew._ssl_compat"),
        ):
            _ensure_ssl_certs()
    ...
    def test_gatewayd_invokes_ensure_ssl_certs(self):
        ...
        mock_fn = MagicMock()
        with patch("codex_crew._ssl_compat._ensure_ssl_certs", mock_fn):
            import codex_crew.mcp_gateway.gatewayd

            importlib.reload(codex_crew.mcp_gateway.gatewayd)
        mock_fn.assert_called()
