...
from codex_crew.dashboard.chat import _expand_prompt_mention, _run_chat
...
from codex_crew.dashboard.handlers import prompts as _prompts_mod
...
def _isolate_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.agent._project_dir", lambda: None)
    # Clear prompt cache between tests
    import codex_crew.dashboard.handlers as h
...
def aim_dir(tmp_path, monkeypatch):
    ...
    base.mkdir()
    from codex_crew.platform.defaults import DefaultPromptSourceProvider
...
def mock_sel(monkeypatch):
    ...
    m = MagicMock()
    monkeypatch.setattr("codex_crew.dashboard.chat.sel", lambda: m)
    monkeypatch.setattr("codex_crew.dashboard.handlers.sel", lambda: m)
    return m
...
def block_sensitive_reads(monkeypatch):
    ...
    """
    monkeypatch.setattr("codex_crew.dashboard.chat_runner.is_sensitive_path", lambda p: True)
    monkeypatch.setattr("codex_crew.hooks.is_sensitive_path", lambda p: True)


@pytest.fixture()
def block_sensitive_discovery(monkeypatch):
    """Refuse every path at the DISCOVERY gate — the one `_prompt_dir_entry` applies."""
    monkeypatch.setattr("codex_crew.dashboard.handlers.is_sensitive_path", lambda p: True)
...
class TestListAimPrompts:
    ...
    def test_sensitive_sop_symlink_skipped(self, aim_dir, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.handlers.is_sensitive_path",
...
class TestUserPromptDiscoveryGate:
    ...
    def test_the_mention_read_refuses_an_escaped_path_it_is_handed(self, tmp_path, monkeypatch):
        ...
        (d / "creds.md").symlink_to(secret)
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_runner.is_sensitive_path", lambda p: "elsewhere" in str(p)
        )
        monkeypatch.setattr("codex_crew.hooks.is_sensitive_path", lambda p: "elsewhere" in str(p))
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_runner._find_prompt",
...
class TestExpandPromptMention:
    ...
    def test_list_error_returns_original(self, monkeypatch):
        monkeypatch.setattr(
            "codex_crew.dashboard.handlers._find_prompt",
...
class TestPromptReadsGoThroughTheDescriptorGate:
    ...
    def test_a_sensitive_description_target_is_audited_as_blocked(
        ...
        monkeypatch_target = "codex_crew.hooks.is_sensitive_path"
        pkg = aim_dir / "Pkg"
        pkg.mkdir()
    ...
    def test_a_prompt_resolving_onto_a_sensitive_target_is_refused(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.hooks.is_sensitive_path", lambda p: "credential-store" in str(p)
        )
        proj, d = self._checkout_prompts(tmp_path)
        (d / "creds.md").symlink_to(secret)
    ...
    def _swap_leaf_inside_the_gate(self, monkeypatch, entry: Path, secret: Path) -> None:
        from codex_crew import hooks as _hooks
...
class TestRunChatPrompts:
    ...
    def test_slash_get_ok(self, aim_dir, mock_sel, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_runner._run_chat", _mock_run_chat)
    ...
    def test_at_prompt_blocked(self, aim_dir, mock_sel, monkeypatch):
        ...
        captured = []
        slot = MagicMock(key="t", agent="codexcrew", model=None, _trust=False, _queue=[])
...
class TestUnscopedDetailReadStaysOffTheEventLoop:
    ...
    def test_the_read_gate_runs_on_the_resolution_s_own_worker_thread(
        ...
        reading the file.
        """
        import codex_crew.hooks as hooks
...
class TestPromptWriteHardening:
    ...
    def test_update_audits_a_filesystem_failure(self, tmp_path, mock_sel, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.handlers.prompts.verified_replace_file_nolink", _boom
        )
        resp = asyncio.run(
            api_prompt_detail(
...
class TestScopedReadOversizeRace:
    ...
    def test_gate_oversize_maps_to_a_coded_413(self, tmp_path, mock_sel, monkeypatch):
        ...
        import codex_crew.dashboard.handlers.prompts as mod
...
class TestUpdateCarriesAccessControlMetadata:
    ...
    def test_extended_attributes_survive_an_edit(self, tmp_path, mock_sel):
        ...
        try:
            os.setxattr(str(path), "user.codexcrew_test", b"keep-me")
        ...
        assert os.getxattr(str(path), "user.codexcrew_test") == b"keep-me"
...
class TestARootSwappedAfterValidationPublishesNothing:
    ...
    def test_a_mention_read_is_not_redirected_by_a_root_swapped_after_the_mint(
        ...
        from codex_crew.dashboard import chat_runner as _cr
