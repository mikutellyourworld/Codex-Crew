...
from codex_crew.dashboard.crash_dump_store import claim_dump_notification
...
class TestClaimDumpNotification:
    ...
    def test_marker_is_not_mistaken_for_a_dump(self, tmp_path) -> None:
        ...
        from codex_crew.dashboard.crash_dump_store import _list_dumps
...
class TestLoopStallBudgetConfig:
    def test_default_remains_automatic_until_launch(self) -> None:
        assert CodexCrewConfig().dashboard.loop_stall_exit_after_secs is None

    def test_managed_service_uses_the_wider_budget(self) -> None:
        assert resolve_loop_stall_exit_after({}, {"CODEXCREW_SERVICE_MANAGED": "1"}) == 90
        ...
        assert resolve_loop_stall_exit_after({}, {"CODEXCREW_SERVICE_MANAGED": "0"}) == 25
    ...
    def test_managed_marker_is_consumed_before_children_are_started(self) -> None:
        ...
        assert launch_environment == {"CODEXCREW_SERVICE_MANAGED": "1"}
    ...
    def test_operator_budget_is_preserved_for_managed_services(self) -> None:
        ...
        assert resolve_loop_stall_exit_after(explicit, {"CODEXCREW_SERVICE_MANAGED": "1"}) == 25

    def test_watchdog_arms_with_the_managed_service_budget(self) -> None:
        arms: list[float] = []
        watchdog = LoopStallWatchdog(
            exit_after=resolve_loop_stall_exit_after({}, {"CODEXCREW_SERVICE_MANAGED": "1"}),
    ...
    def test_automatic_default_survives_unrelated_full_config_save(
        ...
        try:
            assert load_loop_stall_exit_after({"CODEXCREW_SERVICE_MANAGED": "1"}) == 90
    ...
    def test_legacy_materialized_desktop_default_is_reported_not_rewritten(
        ...
        _invalidate_config_cache()
        try:
            assert CodexCrewConfig.load().dashboard.loop_stall_exit_after_secs == 25
            assert load_loop_stall_exit_after({"CODEXCREW_SERVICE_MANAGED": "1"}) == 25
            saved = json.loads((tmp_path / "config.json").read_text(encoding="utf-8"))
    ...
    def test_local_operator_budget_overrides_base_and_managed_default(
        ...
        assert load_loop_stall_exit_after({"CODEXCREW_SERVICE_MANAGED": "1"}) == 60
    ...
    def test_configured_value_is_read(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.config.loader.config_dir", lambda: cfg_dir)
        _invalidate_config_cache()
        try:
            assert CodexCrewConfig.load().dashboard.loop_stall_exit_after_secs == 60
        finally:
            _invalidate_config_cache()
...
class TestChatTurnCeilingConfig:
    def test_default_preserves_existing_behaviour(self, tmp_path, monkeypatch) -> None:
        ...
        behave differently from ``CodexCrewConfig()``.
        """
        from codex_crew.config.loader import AgentConfig, _invalidate_config_cache

        (tmp_path / "config.json").write_text(json.dumps({"agent": {}}), encoding="utf-8")
        monkeypatch.setattr("codex_crew.config.loader.config_dir", lambda: tmp_path)
        _invalidate_config_cache()
        try:
            loaded = CodexCrewConfig.load().agent.chat_turn_timeout_secs
        finally:
            _invalidate_config_cache()
        assert loaded == AgentConfig().chat_turn_timeout_secs
        assert CodexCrewConfig().agent.chat_turn_timeout_secs == AgentConfig().chat_turn_timeout_secs

    def test_out_of_range_values_are_clamped(self) -> None:
        from codex_crew.config.loader import CHAT_TURN_TIMEOUT_MAX, CHAT_TURN_TIMEOUT_MIN
