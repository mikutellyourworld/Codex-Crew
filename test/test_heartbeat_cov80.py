...
def workspace(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr("codex_crew.heartbeat.workspace_dir", lambda: tmp_path / "ws")
    return tmp_path / "ws"
...
def inline_executor(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("codex_crew.heartbeat.maintenance_executor", lambda: None)
...
class TestLoop:
    ...
    async def test_loop_returns_when_shutdown_is_already_set(
        ...
        event = asyncio.Event()
        event.set()
        monkeypatch.setattr("codex_crew.heartbeat.shutdown_event", event)
        svc = HeartbeatService(_memory(), interval=3600)
        beats: list[int] = []
...
class TestBeat:
    ...
    async def test_daily_tick_prunes_history_with_configured_retention(
        ...
        monkeypatch.setattr("codex_crew.heartbeat.CodexCrewConfig.load", staticmethod(lambda: cfg))
        sel_obj = MagicMock()
        monkeypatch.setattr("codex_crew.heartbeat.sel", lambda: sel_obj)
