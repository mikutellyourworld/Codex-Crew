...
These tests drive the real script with a stubbed ``codexcrew`` on PATH and
...
def _run_script(
    ...
    """Run do-restart.sh against a stub ``codexcrew`` that exits *exit_status*.
...
def test_dashboard_token_urls_are_redacted_from_the_log(tmp_path: Path) -> None:
    ...
    probe = 'echo "🔑 http://localhost:5486?token=SECRET123&x=1 and http://h/?a=1&token=T2"\n'
...
def test_attempt_specific_status_file_override(tmp_path: Path) -> None:
    ...
    # when the scheduler passes CODEXCREW_RESTART_STATUS_FILE, the verdict
    ...
    env["CODEXCREW_HOME"] = str(crew_home)
    env["CODEXCREW_RESTART_DELAY"] = "0"
    env["CODEXCREW_RESTART_STATUS_FILE"] = str(attempt)
    proc = subprocess.run(
        ["bash", str(SCRIPT)],
        cwd=tmp_path,
        env=env,
...
def test_nonconforming_status_paths_fall_back_to_the_shared_default(tmp_path: Path) -> None:
    ...
    for evil in evil_paths:
        ...
        env["CODEXCREW_HOME"] = str(crew_home)
        env["CODEXCREW_RESTART_DELAY"] = "0"
        env["CODEXCREW_RESTART_STATUS_FILE"] = evil
        proc = subprocess.run(
            ["bash", str(SCRIPT)],
            cwd=tmp_path,
            env=env,
