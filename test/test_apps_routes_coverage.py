"""Coverage tests for codex_crew.apps.routes — validation, denial and error paths.
...
``CODEXCREW_HOME`` pointed at ``tmp_path``. No git, no network egress, no
...
import codex_crew.apps.routes as routes_mod
...
from codex_crew.apps.routes import (
...
def _setup_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolate CODEXCREW_HOME and neutralize out-of-process side effects."""
...
async def test_get_app_keeps_genuinely_local_app_repositoryless(
    ...
    monkeypatch.setattr("codex_crew.apps.registry.get_registry_app", _must_not_resolve)
...
class TestInstallValidation:
    ...
    async def test_min_version_gate_rejects_before_copying(
        ...
        home = _setup_env(tmp_path, monkeypatch)
        src = _make_app_source(tmp_path, minCodexCrewVersion="999.0.0")
        async with TestClient(TestServer(_make_app())) as client:
...
class TestRegisterExternal:
    ...
    async def test_public_registration_cannot_mint_builtin_ownership(
        ...
        from codex_crew.apps.manager import _builtin_owns_install, _read_installed
    ...
    async def test_registry_owned_refresh_preserves_server_provenance(
        ...
        from codex_crew.apps.manager import _read_installed, set_app_provenance
...
class TestUpdateApp:
    ...
    async def test_app_token_cannot_replace_repository_bound_code_from_local_source(
        ...
        from codex_crew.apps.manager import get_app, set_app_provenance
        from codex_crew.config.loader import _invalidate_config_cache
        from codex_crew.dashboard.token_auth import generate_token, token_auth_middleware
...
class TestDisableBranches:
    ...
    async def test_failed_on_disable_warns_but_still_disables(
        ...
        from codex_crew.apps import teardown as teardown_mod
...
class TestFetchGitBlobCredentialPosture:
    ...
    async def test_ambiguous_git_target_refuses_before_ssrf_gate_or_clone(
        ...
        import codex_crew.apps.registry as reg_mod
...
async def test_api_proxy_signs_and_forwards_to_backend(
    ...
    ``X-CodexCrew-Proxy`` HMAC (validated with the app's own secret by the
    ...
    assert verify_proxy_request(
        seen["headers"]["X-CodexCrew-Proxy"],
        method="POST",
        target="/api/echo?a=1",
        body=b'{"hello":1}',
        secret="proxy-key",
...
def test_on_disable_hook_resolves_hyphenated_builtin_names():
    ...
    for module_name in hyphenated:
        ...
        importlib.import_module(f"codex_crew.apps.builtins.{module_name}")
