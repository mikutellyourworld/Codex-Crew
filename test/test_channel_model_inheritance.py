...
from codex_crew.session import SessionManager, _model_fallback
...
class TestModelFallbackToGlobalConfig:
    ...
    async def test_channel_agent_call_pattern(self, cfg):
        ...
        # would default to 'auto' (Sonnet) for non-codexcrew agents.
        assert captured["model_override"] == "claude-opus-4.6"
        await mgr.close_all()

    @pytest.mark.asyncio
    async def test_codexcrew_agent_uses_global(self, cfg):
        """The default 'codexcrew' agent is excluded from per-agent resolution
        and inherits the global model (it intentionally tracks the global)."""
        captured: dict = {}
        mgr = SessionManager(cfg, provider_factory=_capturing_factory(captured))
        await mgr.get_or_create("codexcrew-sess", agent="codexcrew")
        assert captured["model_override"] == "claude-opus-4.6"
        await mgr.close_all()
...
class TestResolveNamedAgentModel:
    """Real-file coverage for CodexCrewConfig._resolve_named_agent_model.
    ...
    def test_reads_model_by_name_field(self, tmp_path):
        ...
        assert (
            CodexCrewConfig._resolve_named_agent_model("foo-agent", agents_dir=tmp_path)
            == "claude-sonnet-3"
        )
    ...
    def test_reads_model_by_filename_stem(self, tmp_path):
        ...
        assert (
            CodexCrewConfig._resolve_named_agent_model("bar", agents_dir=tmp_path)
            == "claude-haiku-4.5"
        )

    def test_returns_empty_when_not_found(self, tmp_path):
        (tmp_path / "bar.json").write_text(json.dumps({"model": "x"}))
        assert CodexCrewConfig._resolve_named_agent_model("nope", agents_dir=tmp_path) == ""

    def test_returns_empty_for_empty_agent(self, tmp_path):
        assert CodexCrewConfig._resolve_named_agent_model("", agents_dir=tmp_path) == ""
    ...
    def test_skips_non_dict_json(self, tmp_path):
        ...
        assert CodexCrewConfig._resolve_named_agent_model("weird", agents_dir=tmp_path) == ""
    ...
    def test_skips_malformed_json_and_finds_valid(self, tmp_path):
        ...
        assert (
            CodexCrewConfig._resolve_named_agent_model("good", agents_dir=tmp_path)
            == "claude-opus-4.8"
        )
    ...
    def test_malformed_json_only_returns_empty(self, tmp_path):
        ...
        (tmp_path / "broken.json").write_text("{not valid json")
        assert (
            CodexCrewConfig._resolve_named_agent_model("broken", agents_dir=tmp_path) == ""
        )

    def test_match_with_no_model_field_returns_empty(self, tmp_path):
        (tmp_path / "nomodel.json").write_text(json.dumps({"name": "nomodel"}))
        assert (
            CodexCrewConfig._resolve_named_agent_model("nomodel", agents_dir=tmp_path) == ""
        )
...
class TestAgentSpecReadsAreHardened:
    ...
    def test_named_resolver_refuses_an_oversized_spec(self, tmp_path, monkeypatch):
        ...
        from codex_crew.hooks import FileTooLargeError
        ...
        assert CodexCrewConfig._resolve_named_agent_model("huge", agents_dir=tmp_path) == ""
    ...
    def test_named_resolver_refuses_a_link_into_a_sensitive_path(
        ...
        assert CodexCrewConfig._resolve_named_agent_model("link", agents_dir=tmp_path) == ""
        assert (
            CodexCrewConfig._resolve_named_agent_model("direct", agents_dir=tmp_path)
            == "fine"
        )
    ...
    def test_installed_spec_resolver_reads_through_the_hardened_reader(
        ...
        # The installed codexcrew.json path goes through the same reader: a
        ...
        (tmp_path / "codexcrew.json").write_text(json.dumps({"model": "pinned"}))
        ...
        result = CodexCrewConfig._resolve_agent_model()
