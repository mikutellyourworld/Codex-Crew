...
from codex_crew.acp.client import model_is_unusable, resolve_pin_spelling
...
def test_api_models_returns_only_entitled_rows(tmp_path):
    ...
    with (
        patch.object(
            agents.CodexCrewConfig,
            "load",
            return_value=SimpleNamespace(agent=SimpleNamespace(provider="kiro")),
        ),
        patch("codex_crew.acp.client._resolve_kiro_bin_for_spawn", return_value="/usr/bin/kiro-cli"),
