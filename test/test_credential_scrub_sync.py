...
refuses to re-inject them while ``_CODEXCREW_CREDS_SCRUBBED=1``. ``KIRO_API_KEY``
...
from codex_crew.acp.client import _resolve_spawn_env
...
class TestInjectKiroCliApiKey:
    def test_injects_from_env_file_when_absent(self, tmp_path: Path, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "codex_crew.config.loader.env_path", lambda: env_file
        )
    ...
    def test_noop_when_unset_everywhere(self, tmp_path: Path, monkeypatch) -> None:
        monkeypatch.setattr(
            "codex_crew.config.loader.env_path", lambda: tmp_path / "missing.env"
        )
        env: dict[str, str] = {}
        inject_kiro_cli_api_key(env)
...
class TestJiraTokenScrubGuard:
    ...
    def test_loader_skips_jira_token_when_scrubbed(self, monkeypatch, tmp_path) -> None:
        ...
        when _CODEXCREW_CREDS_SCRUBBED=1."""
        import os
        ...
        monkeypatch.setattr(
            "codex_crew.config.loader.config_dir", lambda: config_dir
        )
