...
from codex_crew.dashboard.handlers import worktree as wt_mod
from codex_crew.dashboard.handlers.worktree import (
...
from codex_crew.validation import FOLLOWUP_BRANCH_RE, is_valid_followup_branch
...
class TestNoRepositoryCodeExecution:
    ...
    async def test_hooks_path_is_not_a_repo_writable_location(self, repo, tmp_path):
        ...
        repository, so pointing it at `.git/codexcrew-no-hooks` left the
        ...
        planted = repo / ".git" / "codexcrew-no-hooks"
    ...
    def test_hooks_sink_is_a_non_directory_device(self):
        ...
        """
        from codex_crew.dashboard.handlers import worktree as wt
...
class TestIdempotentReentry:
    ...
    async def test_failed_add_leaves_no_branch_or_directory(self, repo):
        ...
        async with TestClient(TestServer(_make_app(str(repo)))) as client:
            with patch("codex_crew.dashboard.handlers.worktree._run_git", side_effect=fail_add):
                resp = await client.post(
    ...
    async def test_unresolvable_base_creates_nothing(self, repo):
        ...
        async with TestClient(TestServer(_make_app(str(repo)))) as client:
            with patch(
                "codex_crew.dashboard.handlers.worktree._resolve_base_ref",
...
class TestConcurrencySafety:
    ...
    def test_cleanup_survives_a_failed_worktree_listing(self, repo):
        ...
        with patch("codex_crew.dashboard.handlers.worktree._worktree_branches", return_value=None):
    ...
    def test_cleanup_deletes_the_branch_after_an_rmtree_fallback(self, repo):
        ...
        with patch(
            "codex_crew.dashboard.handlers.worktree._run_git", side_effect=_fail_worktree_remove
        ):
            _cleanup_partial(
    ...
    async def test_unreadable_worktree_list_is_a_503_not_a_create(self, repo):
        ...
        async with TestClient(TestServer(_make_app(str(repo)))) as client:
            with patch(
                "codex_crew.dashboard.handlers.worktree._worktree_branches", return_value=None
            ):
                resp = await client.post(
...
class TestCheckoutFilters:
    ...
    def test_probe_failure_fails_closed(self, repo):
        ...
        with patch("codex_crew.dashboard.handlers.worktree._run_git", return_value=failed):
...
class TestLauncherAdvisoryIsNotARefusal:
    ...
    def test_the_launcher_emits_no_severity_the_classifier_does_not_know(self):
        ...
        """
        from codex_crew.sandbox import _build_launcher_script
