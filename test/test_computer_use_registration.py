"""Managed-MCP registration for ``codexcrew-computer`` — and the parity tax.
...
* **The managed spec carries NO ``autoApprove`` key.** ``mcp_gateway/rewriter.py``
  ...
  feature.
* **A fresh install adds ``@codexcrew-computer`` to ``tools`` but NOT to
  ...
  ``@codexcrew-core`` — which is exactly why computer use is its own server rather
  than riding inside that one.

Every test patches ``codex_crew.agent``'s module globals with the
...
from codex_crew import agent, agent_state, mcp_cleanup, mcp_discovery, onboarding_import
from codex_crew.agent import install_agent
from codex_crew.platform_compat import IS_POSIX

CU_SERVER = "codexcrew-computer"
CU_REF = f"@{CU_SERVER}"
...
_MANAGED = {
    "codexcrew-cron": {"invocation_fn": lambda: ("/usr/bin/codexcrew", ["mcp-cron"])},
    "codexcrew-core": {"invocation_fn": lambda: ("/usr/bin/codexcrew", ["mcp-core"])},
    CU_SERVER: {"invocation_fn": lambda: ("/usr/bin/codexcrew", [CU_SUBCOMMAND])},
}
...
_GATED_MANAGED = {
    ...
    "codexcrew-core": {"invocation_fn": lambda: ("/usr/bin/codexcrew", ["mcp-core"])},
    CU_SERVER: {
        "invocation_fn": lambda: ("/usr/bin/codexcrew", [CU_SUBCOMMAND]),
...
def _bundled_defaults(tmp_path: Path) -> Path:
    ...
    Mirrors the shipped file's shape for the keys under test: ``@codexcrew-computer``
        ...
        json.dumps(
            {
                ...
                "tools": ["ReadFile", "@codexcrew-cron", "@codexcrew-core", CU_REF],
                "allowedTools": ["ReadFile", "@codexcrew-core"],
                "mcpServers": {},
...
def _run_install(tmp_path: Path, cfg_dir: Path, *, managed: "dict | None" = None, **kwargs) -> Path:
    ...
    *managed* overrides the staged managed-server set. It defaults to ``_MANAGED``
    ...
    patches = [
        ...
        patch("codex_crew.agent._user_dir", lambda: tmp_path / "home"),
...
def test_managed_spec_is_built_with_a_resolvable_command(tmp_path: Path):
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(patch("codex_crew.agent._project_dir", return_value=None))
        stack.enter_context(
            patch("codex_crew.agent._mc_config_path", return_value=tmp_path / "none.json")
        )
        config = agent.build_agent_config()
    spec = config["mcpServers"][CU_SERVER]
    assert spec["command"] == "/usr/bin/codexcrew"
...
def test_real_managed_entry_uses_the_shared_invocation_resolver():
    """The shipped row resolves its command through ``_codexcrew_mcp_invocation``.
    ...
    ``<interpreter> -m codex_crew``), and the permission-probe shell-out in the
...
def test_fresh_install_adds_the_ref_to_tools_but_not_allowed_tools(tmp_path: Path):
    ...
    ``config/defaults.json`` blanket-allows ``@codexcrew-core``, so riding inside it
...
def _existing_config(tmp_path: Path, cu_spec: dict) -> Path:
    """Seed an existing ``codexcrew.json`` whose computer-use entry is *cu_spec*."""
    kiro_dir = tmp_path / "kiro_agents"
    kiro_dir.mkdir(exist_ok=True)
    path = kiro_dir / "codexcrew.json"
    path.write_text(
        json.dumps(
            {
...
def test_refresh_re_resolves_a_stale_command(tmp_path: Path):
    ...
    cfg_dir = _bundled_defaults(tmp_path)
    _existing_config(tmp_path, {"command": "/old/dead/path/codexcrew", "args": [CU_SUBCOMMAND]})
    config = _installed(_run_install(tmp_path, cfg_dir))
    assert config["mcpServers"][CU_SERVER]["command"] == "/usr/bin/codexcrew"
...
def test_refresh_does_not_add_the_ref_to_allowed_tools(tmp_path: Path):
    ...
    """
    cfg_dir = _bundled_defaults(tmp_path)
    _existing_config(tmp_path, {"command": "/usr/bin/codexcrew", "args": [CU_SUBCOMMAND]})
    config = _installed(_run_install(tmp_path, cfg_dir))
    assert CU_REF not in config["allowedTools"]
    assert not [t for t in config["allowedTools"] if t.startswith(f"{CU_REF}/")]
...
def test_refresh_adds_the_tools_ref_to_an_upgrading_config(tmp_path: Path):
    """**An UPGRADING install must gain ``@codexcrew-computer`` in ``tools``.**
    ...
    """
    cfg_dir = _bundled_defaults(tmp_path)
    path = _existing_config(tmp_path, {"command": "/usr/bin/codexcrew", "args": [CU_SUBCOMMAND]})
...
def test_refresh_does_not_re_add_other_managed_refs(tmp_path: Path):
    ...
    who removed ``@codexcrew-cron`` does not get it silently restored. Scoping the
    carve-out is what keeps "the user controls their tool lists" true.
    """
    cfg_dir = _bundled_defaults(tmp_path)
    _existing_config(tmp_path, {"command": "/usr/bin/codexcrew", "args": [CU_SUBCOMMAND]})
    config = _installed(_run_install(tmp_path, cfg_dir))
    assert "@codexcrew-cron" not in config["tools"]
    assert "@codexcrew-core" not in config["tools"]
...
def test_managed_server_name_appears_in_every_registry():
    ...
    install-method change (and silently drops ``codexcrew doctor``'s probe, which
    ...
    assert CU_SERVER in mcp_cleanup.CODEXCREW_BIN_MCP_SERVERS
    ...
    from codex_crew.dashboard.handlers import mcp as mcp_handlers
...
def test_cleanup_tuple_is_ordered_and_covers_every_managed_server():
    """``CODEXCREW_BIN_MCP_SERVERS`` is a TUPLE (deterministic doctor order).

    ``cli_doctor`` iterates it to probe each server, so a set would make the
    doctor's output order vary between runs.
    """
    assert isinstance(mcp_cleanup.CODEXCREW_BIN_MCP_SERVERS, tuple)
    assert set(mcp_cleanup.CODEXCREW_BIN_MCP_SERVERS) == set(agent._MANAGED_MCP_SERVERS)
...
def test_server_key_is_slash_free():
    ...
    A slash in the key would make ``@codexcrew-computer/...`` unparseable and the
...
def test_cli_exposes_the_hidden_subcommand():
    """``codexcrew mcp-computer`` must parse and dispatch.
...
def test_computer_use_sources_are_scrub_lint_clean():
    ...
    id (``io.github.mikutellyourworld.codexcrew``). That id is genuinely needed by the self-denylist
    (CodexCrew's dashboard can flip this feature's own primary enable, so driving our
    ...
    sources = sorted((repo / "src" / "codex_crew" / "computer_use").rglob("*.py"))
    mcp_shim = repo / "src" / "codex_crew" / "mcp_computer.py"
    if mcp_shim.exists():
...
class TestGatedEntryIsNotPreserved:
    ...
    def _isolated_sidecar(self, tmp_path, monkeypatch):
        import codex_crew.config.paths as paths

        monkeypatch.setattr(paths, "_resolved_home", None)
        monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path / "home"))
    ...
    def _refresh(cfg: dict, *, macos: bool) -> None:
        with ExitStack() as stack:
            stack.enter_context(
                patch.multiple("codex_crew.agent", _MANAGED_MCP_SERVERS=_GATED_MANAGED)
            )
            stack.enter_context(
                patch("codex_crew.agent._prompt_path", return_value=Path("/tmp/p.md"))
            )
            stack.enter_context(patch("codex_crew.platform_compat.IS_MACOS", macos))
            stack.enter_context(
                patch(
                    "codex_crew.computer_use.backend.platform_could_be_supported",
                    return_value=macos,
                )
            )
            if macos:
                stack.enter_context(
                    patch("codex_crew.computer_use.enable_state.is_enabled", return_value=True)
                )
...
class TestGatedRefsAreLeftALONE:
    ...
    def test_a_NARROWED_ref_survives_an_off_on_cycle_byte_for_byte(self):
        ...
        re-adds refs, the bare ``@codexcrew-computer`` appears here and the user who
        ...
        cfg = {
            ...
            "mcpServers": {CU_SERVER: {"command": "/usr/bin/codexcrew", "args": [CU_SUBCOMMAND]}},
        }
    ...
    def test_a_fresh_build_also_keeps_the_shipped_ref(self):
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch("codex_crew.platform_compat.IS_MACOS", False))
            stack.enter_context(
                patch(
                    "codex_crew.computer_use.backend.platform_could_be_supported",
                    return_value=False,
...
class _FakeSupport:
    ...
    def status(self):
        from codex_crew.computer_use.types import BackendStatus
...
class TestSpecEmissionGate:
    ...
    def _build(tmp_path: Path, *, macos: bool, keystone: "dict | None") -> dict:
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(
                patch(
                    "codex_crew.computer_use.enable_state.computer_use_state_path",
    ...
    def test_an_unsupported_platform_gets_no_entry(self, tmp_path: Path):
        ...
        assert "codexcrew-core" in config["mcpServers"]
        assert "codexcrew-cron" in config["mcpServers"]
    ...
    def test_an_override_file_cannot_smuggle_the_entry_past_the_gate(
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(
                patch("codex_crew.agent._user_overrides_path", return_value=override)
            )
            stack.enter_context(
                patch("codex_crew.agent._prompt_path", return_value=cfg_dir / "prompt.md")
            )
            ...
            stack.enter_context(patch("codex_crew.platform_compat.IS_MACOS", supported))
            stack.enter_context(
                patch(
                    "codex_crew.computer_use.backend.platform_could_be_supported",
    ...
    def test_a_refresh_RETRACTS_an_entry_written_while_the_gate_was_open(self):
        ...
        cfg = {
            "mcpServers": {
                CU_SERVER: {"command": "/usr/bin/codexcrew", "args": [CU_SUBCOMMAND]},
                "codexcrew-core": {"command": "/usr/bin/codexcrew", "args": ["mcp-core"]},
            },
            "tools": ["ReadFile", CU_REF, "@codexcrew-core"],
            "allowedTools": [f"{CU_REF}/computer_get_state"],
        }
        with ExitStack() as stack:
            stack.enter_context(patch.multiple("codex_crew.agent", _MANAGED_MCP_SERVERS=self._GATED))
            stack.enter_context(
                patch("codex_crew.agent._prompt_path", return_value=Path("/tmp/p.md"))
            )
            stack.enter_context(patch("codex_crew.platform_compat.IS_MACOS", False))
            agent._refresh_dynamic_fields(cfg)
        assert CU_SERVER not in cfg["mcpServers"]
        assert "codexcrew-core" in cfg["mcpServers"]
    ...
    def test_the_gate_fails_CLOSED_when_the_keystone_cannot_be_read(self, tmp_path: Path):
        ...
        """
        with ExitStack() as stack:
            stack.enter_context(patch("codex_crew.platform_compat.IS_MACOS", True))
            stack.enter_context(
                patch(
    ...
    def test_a_gate_that_raises_is_treated_as_closed(self):
        ...
        with patch.multiple("codex_crew.agent", _MANAGED_MCP_SERVERS=managed):
    ...
    def test_only_computer_use_is_gated(self):
        ...
        A ``spec_gate`` accidentally added to ``codexcrew-core`` would remove the
    ...
    def test_enabling_rebuilds_the_spec_before_resetting_sessions(self):
        ...
        from codex_crew.dashboard.handlers import computer_use as handler
    ...
    def test_the_WHOLE_install_withholds_the_server_and_audits_the_decision(self, tmp_path: Path):
        ...
        with ExitStack() as stack:
            stack.enter_context(patch("codex_crew.platform_compat.IS_MACOS", False))
            stack.enter_context(patch("codex_crew.agent.sel", lambda: _Sel()))
...
class TestDataHomePin:
    """``CODEXCREW_HOME`` must reach the stdio shims, or they read a DIFFERENT home.

    A child process does NOT inherit the gateway's ``CODEXCREW_HOME``; the spec's
    ...
    def _entries(monkeypatch, home: "str | None") -> dict:
        ...
        if home is None:
            ...
            monkeypatch.setenv("CODEXCREW_HOME", home)
        monkeypatch.setattr(paths, "_resolved_home", None)
        with patch.multiple("codex_crew.agent", _MANAGED_MCP_SERVERS=_MANAGED):
            with patch("codex_crew.agent._prompt_path", return_value=Path("/tmp/p.md")):
    ...
    def test_every_managed_server_is_pinned_under_an_override(self, tmp_path, monkeypatch):
        ...
        for name in _MANAGED:
            assert servers[name]["env"]["CODEXCREW_HOME"] == str(tmp_path.resolve()), name
    ...
    def test_a_default_install_emits_no_env_at_all(self, monkeypatch):
        ...
        would churn every existing user's ``codexcrew.json`` for nothing.
        """
    ...
    def test_the_pin_always_AGREES_with_the_resolver(self, tmp_path, monkeypatch):
        ...
        for candidate in (str(tmp_path), "/", "/usr", "/etc", "/System"):
            monkeypatch.setenv("CODEXCREW_HOME", candidate)
            monkeypatch.setattr(paths, "_resolved_home", None)
            expected = paths._valid_override_home()
            pinned = agent._managed_mcp_env().get("CODEXCREW_HOME")
    ...
    def test_a_refresh_pins_an_existing_config_and_keeps_user_env(self, tmp_path, monkeypatch):
        ...
        assert env["CODEXCREW_HOME"] == str(tmp_path.resolve())
    ...
    def test_a_refresh_CLEARS_a_stale_pin_when_the_override_is_gone(self, monkeypatch):
        ...
        cfg = {
            "mcpServers": {
                CU_SERVER: {"command": "old", "args": [], "env": {"CODEXCREW_HOME": "/stale"}},
            }
        }
        with patch.multiple("codex_crew.agent", _MANAGED_MCP_SERVERS=_MANAGED):
            with patch("codex_crew.agent._prompt_path", return_value=Path("/tmp/p.md")):
