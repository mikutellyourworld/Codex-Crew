...
def cfg(tmp_path, monkeypatch) -> PodConfig:
    ...
    monkeypatch.setenv("CODEXCREW_POD_ENV_DIR", str(tmp_path / "pods-env"))
    monkeypatch.setenv("CODEXCREW_POD_ARTIFACTS_DIR", str(tmp_path / "artifacts"))
...
def test_plist_boots_the_named_pod_through_python(cfg):
    ...
    assert body["Label"] == "dev.codexcrew.pod.codexcrew-pod.smoke"
...
def test_plist_is_valid_and_written_per_pod(cfg):
    ...
    assert dst.name == "dev.codexcrew.pod.codexcrew-pod.smoke.plist"
    ...
    assert parsed["Label"] == "dev.codexcrew.pod.codexcrew-pod.smoke"
...
def test_label_honours_a_hermetic_unit_prefix(cfg, monkeypatch):
    ...
    monkeypatch.setenv("CODEXCREW_POD_UNIT_PREFIX", "codexcrew-pod-test")
    hermetic = PodConfig.load()
    assert launchd.pod_label(hermetic, "smoke") == "dev.codexcrew.pod.codexcrew-pod-test.smoke"
    ...
    assert launchd.pod_label(cfg, "smoke") == "dev.codexcrew.pod.codexcrew-pod.smoke"
...
def test_env_selection_is_shared_with_the_systemd_backend(cfg):
    ...
    from codex_crew.pod.config import environment_vars
...
def test_active_names_filters_to_our_prefix_and_liveness(cfg, monkeypatch):
    domain_dump = (
        "dev.codexcrew.pod.codexcrew-pod.alpha\n"
        "dev.codexcrew.pod.codexcrew-pod.beta\n"
        "com.apple.something\n"
        "dev.other.pod.gamma\n"
    )
