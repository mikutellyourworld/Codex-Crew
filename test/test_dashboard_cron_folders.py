...
from codex_crew.dashboard.handlers.cron import (
    api_cron_folders,
...
from codex_crew.validation import MAX_SHORT_STRING
...
class TestCronFoldersList:
    ...
    async def test_list_serializes_a_snapshot_not_the_live_dicts(self, tmp_path):
        ...
        with patch("codex_crew.dashboard.handlers.cron.web.json_response", side_effect=_capture):
...
class TestCronFolderDeleteStateMethod:
    ...
    def test_delete_removes_folder_and_clears_jobs(self, tmp_path, monkeypatch):
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path, raising=False)
...
class TestCronFoldersAsyncPersistence:
    ...
    async def test_create_calls_state_method_via_to_thread(self, tmp_path):
        ...
        with patch(
            "codex_crew.dashboard.handlers.cron.asyncio.to_thread", new_callable=AsyncMock
        ) as mock_to_thread:
...
class TestCronFoldersPersistence:
    ...
    def test_startup_wiring_calls_load_cron_folders(self):
        ...
        import codex_crew.dashboard.server as server_mod
