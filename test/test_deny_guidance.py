...
drive the REAL producers in :mod:`codex_crew.security` rather than asserting on
...
from codex_crew import deny_guidance as dg
...
def _rule_tier_reason(rule: security.DeniedCommandRule) -> str:
    ...
    :mod:`codex_crew.security` emits through, precisely so the micro-format cannot
...
class TestRuleIdentityRoutesTheRegexTier:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("credential-exfil-codexcrew-token", dg.DENY_CLASS_SELF_PROTECTION),
            ("credential-exfil-codexcrew-token-argv", dg.DENY_CLASS_SELF_PROTECTION),
    ...
    def test_a_rule_tier_self_protection_refusal_matches_what_the_floor_says(self):
        ...
        ``codexcrew token`` is enforced by the regex tier AND the argv-structural
        ...
        """
        floor_reason = security.is_denied("codexcrew token", denied_regexes=_builtin_regexes())
        assert floor_reason
        rule = _rule_by_id("credential-exfil-codexcrew-token")
        assert (
...
class TestRemediationText:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "Running: rm -rf ~/.codex-crew/lessons.json",
...
class TestCredentialToolHint:
    ...
    def test_real_server_ids_are_still_resolved_for_the_operator_surface(self):
        ...
        """
        for server_id in ("creds-agent", "codexcrew-core", "local-chorus-mcp", "mochi:mochi"):
            rows = [{"server_id": server_id, "description": "vends credentials"}]
