...
``codexcrew`` inside a pod used to resolve the GLOBAL launcher shim — meaning a pod
...
def test_pod_env_scrubs_the_live_gateways_bound_port(tmp_path, monkeypatch):
    """CODEXCREW_BOUND_PORT must never cross the pod boundary.
    ...
    the pod's own CODEXCREW_PORT is the target — and resolve_client_port reads
    ...
    assert "CODEXCREW_BOUND_PORT" not in env
    assert env["CODEXCREW_PORT"] == "7900"
...
def test_pod_env_still_isolates_home_and_port(tmp_path):
    ...
    assert env["CODEXCREW_PORT"] == "7900"
    assert env["CODEXCREW_PROJECT_DIR"] == str(checkout)
...
def test_pod_context_resolves_the_pods_own_binary_and_env(tmp_path, monkeypatch):
    ...
    assert env["CODEXCREW_HOME"] == str(cfg.home_dir("wt-feature"))
...
def test_exec_in_pod_execs_the_pods_binary_with_the_pod_env(tmp_path, monkeypatch):
    ...
    assert seen["env"]["CODEXCREW_HOME"] == str(cfg.home_dir("wt-feature"))  # type: ignore[index]
...
def test_the_refusal_names_a_pod_native_equivalent_where_one_exists():
    ...
    assert "codexcrew pod down wt-feature" in str(exc.value)
...
def test_logs_is_refused_and_points_at_the_pod_journal():
    ...
    assert "codexcrew pod logs wt-feature" in str(exc.value)
...
def test_snapshot_is_refused_because_its_destination_is_configurable():
    ...
    assert hasattr(CodexCrewConfig, "__dataclass_fields__")
    assert "snapshot_dir" in CodexCrewConfig.__dataclass_fields__
...
# neither CODEXCREW_WORKSPACE nor config_dir()/workspace_dir is present — which a
...
def test_pod_env_scopes_the_workspace_into_the_pod_home(tmp_path):
    ...
    assert env["CODEXCREW_WORKSPACE"] == str(home / "workspace")
    # Inside the pod HOME, so zero-residue teardown removes it.
    assert Path(env["CODEXCREW_WORKSPACE"]).is_relative_to(home)
...
def test_the_pod_workspace_is_not_the_live_workspace(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("CODEXCREW_WORKSPACE", raising=False)
    live = loader.workspace_root()

    assert Path(env["CODEXCREW_WORKSPACE"]).resolve() != live.resolve()


def test_the_override_actually_drives_workspace_root(tmp_path, monkeypatch):
    """Pin that CODEXCREW_WORKSPACE is the mechanism workspace_root() honours, so
    ...
    monkeypatch.setenv("CODEXCREW_WORKSPACE", env["CODEXCREW_WORKSPACE"])

    assert loader.workspace_root().resolve() == Path(env["CODEXCREW_WORKSPACE"]).resolve()
...
def test_run_is_refused_because_it_writes_beside_the_spec():
    ...
    from codex_crew import task_reporter
...
def test_chat_and_tui_verbs_stay_refused_in_a_pod():
    ...
    port with a literal 5486 fallback, never `CODEXCREW_PORT`, so a pod landed on
