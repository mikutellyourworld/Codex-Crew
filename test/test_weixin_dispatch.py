...
from codex_crew.messaging.driver import APPROVAL_AUTO
from codex_crew.messaging.transport import InboundMessage
from codex_crew.session_allocation import SessionClosingError
from codex_crew.weixin.client import ContextTokenStore, TypingTicketCache
from codex_crew.weixin.commands import parse_command
from codex_crew.weixin.transport import WEIXIN_CAPABILITIES
from codex_crew.weixin.transport_dispatch import WeixinDispatcher
...
class FakeProvider:
    ...
    async def stream(self, message):
        ...
        from codex_crew.messaging.driver import EVENT_COMPLETE, EVENT_TEXT_CHUNK
...
def test_a_mid_turn_attachment_is_refused_instead_of_ingested(tmp_path, monkeypatch):
    ...
    """
    import codex_crew.weixin.transport_dispatch as td
...
def test_a_turn_starting_during_the_download_still_refuses_the_attachment(tmp_path, monkeypatch):
    ...
    """
    import codex_crew.weixin.transport_dispatch as td
    from codex_crew.messaging.attachments import IngestResult
...
def test_dispatcher_agent_falls_back_to_codexcrew(tmp_path):
    d, _, _ = _make(tmp_path)
    assert d._resolve_agent() == "codexcrew"
...
def test_governance_deny_drops_the_message_before_any_turn(tmp_path, monkeypatch):
    ...
    """
    import codex_crew.messaging.dispatch as mod
...
def test_pipeline_enforces_the_gate_even_if_a_channel_forgets(tmp_path, monkeypatch):
    ...
    asyncio.run(
        mod.drive_turn(
            mod.ChannelTurn(
                channel_type="weixin",
                session_key="weixin:codexcrew:direct:u1",
...
def test_every_alias_in_the_weixin_spec_is_reachable():
    ...
    from codex_crew.weixin.commands import COMMANDS
...
def test_every_visible_weixin_command_appears_in_help():
    from codex_crew.weixin.commands import COMMANDS, build_help
...
def test_weixin_help_lists_no_link_command():
    ...
    from codex_crew.weixin.commands import build_help
