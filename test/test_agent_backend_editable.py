...
``PATCH /api/config/codexcrew``, so it has to be in ``_EDITABLE_CONFIG`` at all —
...
from codex_crew.dashboard.handlers.agents import _supply_live_enum
...
def test_the_default_backend_is_accepted_by_its_own_allowlist():
    ...
    assert CodexCrewConfig().agent.acp_backend in _EDITABLE_CONFIG[FIELD]["values_fn"]()
...
def test_a_registered_backend_reaches_the_allowlist(restore_registry):
    ...
    starting state has to be constructed from the OpenAI-compatible adapter. That
    ...
    assert ACP_BACKEND_OPENAI_COMPATIBLE not in _EDITABLE_CONFIG[FIELD]["values_fn"]()

    acp_backends.register_selectable_backend(ACP_BACKEND_OPENAI_COMPATIBLE)

    assert ACP_BACKEND_OPENAI_COMPATIBLE in _EDITABLE_CONFIG[FIELD]["values_fn"]()
...
def test_a_registered_backend_reaches_the_schema_endpoint(restore_registry):
    ...
    entry: Dict[str, Any] = {"path": FIELD, "enumValues": None}
    _supply_live_enum(entry)
    assert ACP_BACKEND_OPENAI_COMPATIBLE in entry["enumValues"]
...
def test_the_field_declares_no_static_enum():
    ...
    """
    meta = CodexCrewConfig().agent.__dataclass_fields__["acp_backend"].metadata
...
def test_baseline_ships_every_known_backend():
    ...
    assert baseline == sorted([ACP_BACKEND_CODEX, ACP_BACKEND_OPENAI_COMPATIBLE])
