...
class TestSeedHomeFromScenario:
    ...
    def test_restores_the_callers_home_env(self, tmp_path: Path, monkeypatch) -> None:
        monkeypatch.setenv("CODEXCREW_POD_ROOT", str(tmp_path / "pods"))
        monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path / "callers-home"))
        rt.seed_home_from_scenario(PodConfig.load(), "wt", "empty")
        assert os.environ["CODEXCREW_HOME"] == str(tmp_path / "callers-home")
...
class TestBootAppliesTheScenario:
    ...
    def booted(self, tmp_path: Path, monkeypatch):
        ...
        monkeypatch.setenv("CODEXCREW_POD_ROOT", str(tmp_path / "pods"))
        monkeypatch.setenv("CODEXCREW_POD_ENV_DIR", str(tmp_path / "envs"))
        checkout = tmp_path / "co"
        binary = rt.prov.venv_bin(checkout)
        binary.parent.mkdir(parents=True)
        binary.write_text("#!/bin/sh\n")
        binary.chmod(0o755)
        (checkout / "src" / "codex_crew" / "static" / "dist").mkdir(parents=True)
        cli_src = checkout / "src" / "codex_crew" / "cli.py"
    ...
    def test_a_scenario_populates_the_home_before_the_exec(self, booted, capsys) -> None:
        ...
        # entry point is `codexcrew.exe`, so a bare `endswith("codexcrew")` asserts
        # the POSIX layout on every platform.
        assert captured["argv"][0].endswith(rt.prov.venv_bin(Path(".")).name)
        assert captured["env"]["CODEXCREW_HOME"] == str(home)
