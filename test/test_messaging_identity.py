...
from codex_crew.messaging import identity
...
def test_publishes_with_host_pid_and_key() -> None:
    ...
    with patch.object(identity, "publish_session_pid") as pub:
        asyncio.run(identity.publish_turn_identity(sessions, "telegram:codexcrew:direct:7"))
        pub.assert_called_once_with(4242, "telegram:codexcrew:direct:7")


def test_no_publish_when_pid_unavailable() -> None:
    sessions = _Sessions(None)  # session not spawned yet -> get_pid None
    with patch.object(identity, "publish_session_pid") as pub:
        asyncio.run(identity.publish_turn_identity(sessions, "slack:codexcrew:C1:T1"))
...
def test_swallows_get_pid_error() -> None:
    ...
    with patch.object(identity, "publish_session_pid") as pub:
        ...
        asyncio.run(identity.publish_turn_identity(sessions, "discord:codexcrew:g:t"))
        pub.assert_not_called()
...
def test_inbound_fail_closed_on_governance_error(monkeypatch) -> None:
    ...
    monkeypatch.setattr("codex_crew.messaging.identity.governance_permits", _boom)
    assert asyncio.run(identity.channel_inbound_permitted("discord")) is False
...
def test_inbound_governed_deny_is_sel_audited(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("codex_crew.messaging.identity.sel", lambda: _Sel())
    try:
        (pdir / "host.json").write_text(
            json.dumps(
                {
