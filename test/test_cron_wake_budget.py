...
from codex_crew.cron import _JOB_TIMEOUT_SECS, CronJob, CronSchedule, CronService
...
class TestCronTransientRetry:
    ...
    def _run(
        ...
        *,
        ...
        with (
            patch("codex_crew.slack.gateway.stream_and_collect", side_effect=mock_stream),
            ...
            patch(
                "codex_crew.slack.gateway.CronService.create",
                new=AsyncMock(side_effect=capture_cron),
            ),
            patch("codex_crew.slack.gateway.transient_retry_delay", return_value=0.0),
        ):
    ...
    def test_transient_retries_exhausted_still_counts(
        ...
        from codex_crew.slack.gateway import _CRON_TRANSIENT_RETRIES
...
class TestCronPostTokenResume:
    ...
    def test_posttoken_transient_recovers_and_records_success(
        ...
        from codex_crew.slack.gateway import _CRON_POSTTOKEN_CONTINUE_MSG
    ...
    def test_resumed_turn_bills_both_prompts(self, gw_and_cb: tuple[Any, Any, Any]) -> None:
        ...
        with (
            ...
            patch("codex_crew.slack.gateway.transient_retry_delay", return_value=0.0),
            patch(
                "codex_crew.slack.gateway.provider_last_turn_usage",
                side_effect=[TurnUsage(credits=3.0), TurnUsage(credits=2.0)],
            ),
            patch(
                "codex_crew.slack.gateway.persist_token_record_async",
...
class TestWakeBudgetSubprocessGuard:
    ...
    def test_add_allows_budget_covering_script_default(self, tmp_path: Path) -> None:
        ...
        job = svc.add_job(
            ...
            script="~/.codex-crew/crons/x.py:f", timeout_secs=60,
        )
