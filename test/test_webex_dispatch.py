"""Tests for codex_crew.webex.transport_dispatch (WebexDispatcher) + commands."""
...
from codex_crew.acp.types import EVENT_COMPLETE, EVENT_TEXT_CHUNK, AcpEvent
from codex_crew.messaging.link import ChannelLink
from codex_crew.session_allocation import SessionClosingError
from codex_crew.webex import cards
from codex_crew.webex import transport_dispatch as webex_dispatch
from codex_crew.webex.client import WebexInbound
from codex_crew.webex.commands import (
    COMMAND_SPEC,
    ConversationState,
    build_help_text,
...
from codex_crew.webex.transport_dispatch import _MAX_COLLAPSE, WebexDispatcher
...
class TestTurn:
    ...
    @pytest.mark.asyncio
    async def test_agent_resolves_to_codexcrew_when_unset(self) -> None:
        provider = FakeProvider([AcpEvent(kind=EVENT_COMPLETE)])
        sessions = FakeSessions(provider)
        d = _dispatcher(sessions, FakeCtx(), FakeClient(), cfg=_cfg(default_agent=""))
        await d.handle_message(_inbound("hi"))
        assert sessions.last_agent == "codexcrew"
...
class TestCommandParsing:
    ...
    @pytest.mark.parametrize(
        "text,arg", [("/yolo on", "on"), ("/yolo", ""), ("/codexcrew dashboard 2h", "dashboard 2h")]
    )
...
class TestApprovals:
    ...
    async def test_interactive_mode_builds_a_decider(self) -> None:
        ...
        with mock.patch("codex_crew.webex.transport_dispatch.drive_turn", _capture):
            await d.handle_message(_inbound("hello"))
    ...
    async def test_the_session_auto_approve_predicate_is_forwarded(self) -> None:
        ...
        with mock.patch("codex_crew.webex.transport_dispatch.safety_override") as so:
    ...
    async def test_a_governance_deny_blocks_approve_but_still_resolves_deny(self) -> None:
        ...
        with mock.patch(
            "codex_crew.webex.transport_dispatch.inbound_permitted",
...
# ------------------------------------------------------------------
# /codexcrew dashboard — presigned login link
# ------------------------------------------------------------------
...
class TestDashboardLink:
    ...
    async def test_a_link_is_minted_capped_and_audited(self, origin: str) -> None:
        ...
        with mock.patch(
            "codex_crew.dashboard.token_auth.generate_token", return_value=self.TOKEN
        ) as gen:
            with mock.patch.object(webex_dispatch, "sel") as sel_mock:
                await d.handle_message(_inbound("/codexcrew dashboard 2h"))
    ...
    async def test_a_non_direct_room_is_refused(self) -> None:
        ...
        inbound = WebexInbound(
            person_email=_EMAIL, room_id="ROOM", text="/codexcrew dashboard", room_type="group"
        )

        with mock.patch("codex_crew.dashboard.token_auth.generate_token") as gen:
    ...
    @pytest.mark.asyncio
    async def test_a_bare_codexcrew_answers_with_usage(self) -> None:
        d = _dispatcher(FakeSessions(FakeProvider([])), FakeCtx(), FakeClient())
        await d.handle_message(_inbound("/codexcrew"))
    ...
    async def test_a_token_failure_reports_the_type_not_the_message(self) -> None:
        ...
        with mock.patch(
            "codex_crew.dashboard.token_auth.generate_token",
            side_effect=RuntimeError("secret=abc"),
        ):
            await d.handle_message(_inbound("/codexcrew dashboard"))
    ...
    async def test_an_unusable_duration_falls_back_to_the_default(self, arg: str) -> None:
        ...
        with mock.patch("codex_crew.dashboard.token_auth.generate_token", return_value="TOK") as gen:
            await d.handle_message(_inbound(f"/codexcrew {arg}"))
    ...
    def test_the_ttl_is_clamped_to_the_server_maximum(self) -> None:
        ...
        from codex_crew.dashboard.token_auth import MAX_SESSION_TTL_SECS, parse_duration
...
class TestDrainIsFlat:
    ...
    async def test_the_replayed_turn_does_not_start_its_own_drain(self) -> None:
        ...
        with mock.patch.object(d, "_drain_queue", _counting):
            with mock.patch("codex_crew.webex.transport_dispatch.drive_turn", _noop_turn):
                await d._drain_queue(_inbound("x"), d._session_key(_EMAIL))
...
class TestOptionsCardPress:
    ...
    async def test_an_unauthorized_press_is_dropped_and_audited(self) -> None:
        ...
        with mock.patch("codex_crew.webex.transport_dispatch.sel") as fake_sel:
...
class TestApprovalCardPress:
    ...
    async def test_governance_blocks_an_approve_but_still_resolves_a_deny(self) -> None:
        ...
        with mock.patch("codex_crew.webex.transport_dispatch.inbound_permitted", _denied):
            await d.handle_message(_approval_press("approve", nonce))
            assert webex_dispatch._APPROVALS.has_pending(key)
            await d.handle_message(_approval_press("deny", nonce))
...
class TestGroupSpaceRouting:
    ...
    async def test_uploads_are_off_in_a_space(self) -> None:
        ...
        with mock.patch("codex_crew.webex.transport_dispatch.WebexRenderer", _spy):
            with mock.patch("codex_crew.webex.transport_dispatch.drive_turn", _noop):
                await d.handle_message(_space("hi"))
                await d.handle_message(_inbound("hi"))
...
class TestSessionsCommand:
    ...
    def _stem(d: WebexDispatcher, route: str, gen: int = 0) -> str:
        from codex_crew.history import transcript_stem
