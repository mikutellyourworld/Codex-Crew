...
from codex_crew.telegram.attachments import (
    _to_attachment,
    append_attachment_context,
    process_telegram_attachments,
)
from codex_crew.telegram.client import TelegramClient, TelegramInbound
...
class TestToAttachment:
    ...
    def test_voice_without_file_name_gets_a_real_audio_suffix(self):
        ...
        """
        from codex_crew.messaging.attachments import safe_suffix
...
class TestProcessTelegramAttachments:
    ...
    async def test_voice_transcribed_when_available(self):
        ...
        with patch("codex_crew.transcribe.is_available", return_value=True), \
             patch("codex_crew.transcribe.transcribe_audio", return_value="hello from voice"):
            result = await process_telegram_attachments(client, attachments)
    ...
    async def test_voice_rejected_when_stt_unavailable(self):
        ...
        with patch("codex_crew.transcribe.is_available", return_value=False):
            result = await process_telegram_attachments(client, attachments)
...
class TestReExport:
    def test_telegram_exports_shared_function(self):
        from codex_crew.messaging.attachments import append_attachment_context as shared_fn
    ...
    def test_transcription_block_is_shared_not_duplicated(self):
        ...
        from codex_crew.discord import attachments as discord_att
        from codex_crew.telegram import attachments as tg_att
