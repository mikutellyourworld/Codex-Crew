"""Tests for ``codex_crew.frontend.ensure_dev_dist_symlink``.
...
* sibling ``CodexCrewWebsite/dist`` is resolved and symlinked
...
from codex_crew import frontend, platform_compat


def _fake_codex_crew_package(root: Path) -> Path:
    """Build the minimal directory shape the resolver walks."""
    pkg = root / "src" / "CodexCrew" / "src" / "codex_crew"
...
def fake_pkg(tmp_path, monkeypatch):
    ...
    Returns the ``codex_crew`` package dir (``<ws>/src/CodexCrew/src/codex_crew``).
    The resolver uses ``Path(__file__)`` from ``codex_crew.frontend`` to locate
    the package; monkeypatching that attribute redirects every probe to the
    temp-dir tree we build in each test.
    """
    pkg = _fake_codex_crew_package(tmp_path)
...
def test_dangling_symlink_is_replaced_when_candidate_exists(fake_pkg, tmp_path, monkeypatch):
    ...
    sibling_dist = _make_dist(fake_pkg.parent.parent.parent / "CodexCrewWebsite" / "dist")
...
def test_sibling_checkout_is_symlinked(fake_pkg, monkeypatch):
    """Sibling CodexCrewWebsite/dist wins even when brazil-path is available."""
    sibling_dist = _make_dist(fake_pkg.parent.parent.parent / "CodexCrewWebsite" / "dist")
...
def _repo_with_build(root: Path) -> Path:
    """Minimal repo shape: src/codex_crew/ plus a built website/dist."""
    (root / "src" / "codex_crew" / "static").mkdir(parents=True)
...
def test_stage_built_dist_reports_failure_when_build_missing(tmp_path):
    ...
    (tmp_path / "src" / "codex_crew" / "static").mkdir(parents=True)
...
def test_build_and_stage_holds_the_lock_across_the_build(tmp_path):
    ...
    def _fake_build(*args, **kwargs):
        ...
        lock_path = tmp_path / "src" / "codex_crew" / "static" / ".dist.staging.lock"
    ...
    staged = tmp_path / "src" / "codex_crew" / "static" / "dist"
...
def test_stage_built_dist_accepts_an_explicit_source_dir(tmp_path):
    ...
    assert (tmp_path / "src" / "codex_crew" / "static" / "dist" / "index.html").is_file()
