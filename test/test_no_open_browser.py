...
def _load_from_raw_string(content: str, tmp_path: Path) -> CodexCrewConfig:
    ...
    with patch("codex_crew.config.loader.config_path", return_value=tmp):
...
class TestNoOpenCliFlag:
    ...
    async def test_run_gateway_passes_no_open(self) -> None:
        ...
        with (
            ...
            patch("codex_crew.slack.gateway.GatewayOrchestrator") as mock_orch_cls,
        ):
            mock_orch = MagicMock()
