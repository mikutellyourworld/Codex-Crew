...
from codex_crew.validation import SPAWN_RUN_SCHEMA, ValidationError, validate_tool_args
...
def _hermetic_cfg(role_models=None, agent_pins=None):
    ...
    ``~/.kiro/agents/codexcrew.json`` from disk) and named-agent pins come from
    *agent_pins* instead of the real agents directory."""
    ...
    cfg = CodexCrewConfig(agent=AgentConfig(role_models=role_models or {}))
    with (
        patch("codex_crew.config.loader.CodexCrewConfig.load", classmethod(lambda c: cfg)),
        patch.object(CodexCrewConfig, "_resolve_agent_model", staticmethod(lambda: "")),
        patch.object(
            CodexCrewConfig,
...
class TestApiSpawnHandler:
    ...
    async def test_value_reaches_spawn(self):
        from codex_crew.dashboard.handlers.messaging import api_spawn
...
class TestResolutionPrecedence:
    ...
    def _run(self, *, info_effort: str = "", role_efforts=None):
        ...
        cfg = CodexCrewConfig(agent=AgentConfig(role_efforts=role_efforts or {}))
...
class TestEffortDropReason:
    ...
    def _reason(
        ...
        from codex_crew.subagent import effort_drop_reason
...
class TestNoSpawnSiteDropWarning:
    ...
    def test_per_call_effort_on_auto_emits_no_spawn_warning(self, caplog):
        with caplog.at_level(logging.WARNING, logger="codex_crew.subagent"):
...
class TestApiSpawnEffortDropped:
    ...
    def _spawn(self, body: dict, *, role_models=None, crews=None, parent_agent="") -> dict:
        ...
        with (
            patch("codex_crew.config.loader.CodexCrewConfig.load", classmethod(lambda c: cfg)),
            patch(
                "codex_crew.dashboard.handlers.messaging.warm_project_agents_for_spawn",
                AsyncMock(),
            ),
        ):
...
class TestEffortAppliedNote:
    ...
    def _note(self, model: str, effort: str, *, role_models=None, agent="", agent_pins=None) -> str:
        from codex_crew.subagent import effort_applied_note
...
class TestVerdictFactoryParity:
    ...
    (``CodexCrewConfig.acp_effective_model``); this drives the REAL factory
    ...
    def test_factory_gate_and_verdict_agree(self, model, role_pin, agent, pins, tmp_path):
        ...
        with _hermetic_cfg(role_models=role_models, agent_pins=pins) as cfg:
            ...
            with patch("codex_crew.providers.acp.AcpProvider") as provider_cls:
...
class TestAppliedLineRendering:
    ...
    def test_api_spawn_returns_applied_for_capable_model(self):
        ...
        cfg = CodexCrewConfig(agent=AgentConfig())
        with (
            patch("codex_crew.config.loader.CodexCrewConfig.load", classmethod(lambda c: cfg)),
            patch(
                "codex_crew.dashboard.handlers.messaging.warm_project_agents_for_spawn",
...
class TestSessionChainResolution:
    ...
    def _reason(self, model: str, effort: str, agent: str = "", *, cfg=None) -> str:
        ...
        cfg = cfg or CodexCrewConfig(agent=AgentConfig())
        with patch("codex_crew.config.loader.CodexCrewConfig.load", classmethod(lambda c: cfg)):
            return effort_drop_reason(model, effort, agent)
    ...
    def _cfg_with_crew(self, crew_model: str):
        ...
        cfg = CodexCrewConfig(agent=AgentConfig())
        cfg.agents["mycrew"] = CodexCrewAgentConfig(model=crew_model)
        return cfg
    ...
    def test_concrete_global_model_carries_the_effort(self):
        ...
        cfg = CodexCrewConfig(agent=AgentConfig(model="sonnet-test-model"))
    ...
    def test_sentinel_global_still_drops(self):
        ...
        cfg = CodexCrewConfig(agent=AgentConfig(model="auto"))
    ...
    def test_api_spawn_uses_the_inherited_parent_agent(self):
        ...
        data = harness._spawn(
            {"task": "x", "reasoning_effort": "high", "parent_session": "d:1"},
            crews={"parentcrew": CodexCrewAgentConfig(model="sonnet-test-model")},
...
class TestVerdictOffTheEventLoop:
    ...
    def test_verdict_resolvers_run_off_the_loop_thread(self):
        ...
        from codex_crew.config.loader import AgentConfig, CodexCrewConfig
        from codex_crew.dashboard.handlers import messaging as messaging_mod
        from codex_crew.dashboard.handlers.messaging import api_spawn
