...
def test_kiro_binary_unresolved_returns_503(tmp_path):
    with patch.object(agents.CodexCrewConfig, "load", return_value=_kiro_cfg()), patch(
        "codex_crew.acp.client._resolve_kiro_bin_for_spawn", return_value=""
    ):
...
def test_list_models_timeout_returns_503(tmp_path):
    with patch.object(agents.CodexCrewConfig, "load", return_value=_kiro_cfg()), patch(
        "codex_crew.acp.client._resolve_kiro_bin_for_spawn", return_value="/usr/bin/kiro-cli"
    ), patch("codex_crew.acp.client._resolve_ssh_auth_sock", lambda env: None), patch(
        "codex_crew.env.augmented_path", lambda p: p
    ), patch(
        "codex_crew.dashboard.handlers.agents.wrap_argv", _stub_wrap_argv
    ), patch(
        "codex_crew.dashboard.handlers.agents.cgroup_scope_argv", lambda argv: argv
    ), patch(
        "codex_crew.sandbox.resource_limit_preexec", lambda: None
    ), patch.object(
...
def test_list_models_nonzero_exit_returns_503(tmp_path):
    ...
    with patch.object(agents.CodexCrewConfig, "load", return_value=_kiro_cfg()), patch(
        ...
        "codex_crew.platform.redact_via_context", lambda text: text
    ), patch.object(
...
def test_unexpected_exception_returns_503(tmp_path):
    ...
    with patch.object(agents.CodexCrewConfig, "load", return_value=_kiro_cfg()), patch(
        "codex_crew.acp.client._resolve_kiro_bin_for_spawn", side_effect=RuntimeError("boom")
    ):
        resp = _run(agents.api_models(_kiro_request(tmp_path)))
    assert resp.status == 503
...
def test_successful_list_launches_resolved_binary_in_place(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.config.loader.env_path", lambda: env_file)
    ...
    with (
        patch.object(agents.CodexCrewConfig, "load", return_value=_kiro_cfg()),
        patch("codex_crew.acp.client._resolve_kiro_bin_for_spawn", return_value=resolved),
        patch("codex_crew.acp.client._resolve_ssh_auth_sock", lambda env: None),
        patch("codex_crew.env.augmented_path", lambda p: p),
        patch("codex_crew.dashboard.handlers.agents.wrap_argv", _stub_wrap_argv),
        patch("codex_crew.dashboard.handlers.agents.cgroup_scope_argv", lambda argv: argv),
        patch("codex_crew.sandbox.resource_limit_preexec", lambda: None),
    ...
    assert env["CODEXCREW_UNRELATED_KEEPME"] == "keep-this-value"
...
def test_list_models_spawns_at_the_configured_sandbox_tier(tmp_path):
    ...
    inside Codex Crew's), taking that default asks for a STRICTER tier than chat
    ...
    with patch.object(agents.CodexCrewConfig, "load", return_value=_kiro_cfg()), patch(
        ...
        "codex_crew.dashboard.handlers.agents.configured_sandbox_mode", lambda: "off"
    ), patch(
        "codex_crew.dashboard.handlers.agents.wrap_argv", _record
    ), patch(
        "codex_crew.dashboard.handlers.agents.cgroup_scope_argv", lambda argv: argv
    ), patch(
        "codex_crew.sandbox.resource_limit_preexec", lambda: None
...
def test_configured_sandbox_mode_is_the_tier_the_chat_path_uses():
    ...
    for configured in ("off", "auto"):
        ...
        with patch("codex_crew.config.loader.CodexCrewConfig.load", return_value=cfg):
...
def test_configured_sandbox_mode_fails_secure_when_config_unreadable(caplog):
    ...
    with patch("codex_crew.config.loader.CodexCrewConfig.load", side_effect=OSError("boom")):
...
def test_sandbox_refusal_is_reported_with_a_machine_readable_code(tmp_path, caplog):
    ...
    with patch.object(agents.CodexCrewConfig, "load", return_value=_kiro_cfg()), patch(
        "codex_crew.acp.client._resolve_kiro_bin_for_spawn", return_value="/usr/bin/kiro-cli"
    ), patch("codex_crew.env.augmented_path", lambda p: p), patch(
        "codex_crew.dashboard.handlers.agents.configured_sandbox_mode", lambda: "auto"
    ), patch(
        "codex_crew.dashboard.handlers.agents.wrap_argv", _refuse
    ):
...
def test_ssh_auth_sock_resolver_runs_off_the_event_loop(tmp_path):
    ...
    with patch.object(agents.CodexCrewConfig, "load", return_value=_kiro_cfg()), patch(
        "codex_crew.acp.client._resolve_kiro_bin_for_spawn", return_value="/usr/bin/kiro-cli"
    ), patch("codex_crew.acp.client._resolve_ssh_auth_sock", _probe), patch(
        "codex_crew.env.augmented_path", lambda p: p
    ), patch(
        "codex_crew.dashboard.handlers.agents.wrap_argv", _stub_wrap_argv
    ), patch(
