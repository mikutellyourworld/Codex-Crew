...
from codex_crew.dashboard.state import row_mid
...
_CHANNELS = [
    ("weixin", "codex_crew.weixin.transport_dispatch"),
    ("telegram", "codex_crew.telegram.transport_dispatch"),
    ("discord", "codex_crew.discord.transport_dispatch"),
    ("feishu", "codex_crew.feishu.transport_dispatch"),
    ("imessage", "codex_crew.imessage.transport_dispatch"),
    ("teams", "codex_crew.teams.transport_dispatch"),
    ("webex", "codex_crew.webex.transport_dispatch"),
    ("wecom", "codex_crew.wecom.transport_dispatch"),
    ("whatsapp", "codex_crew.whatsapp.transport_dispatch"),
]
...
def _persist(cls, host, key, user_text, reply_text, is_new):
    ...
    """
    try:
        cls._persist_turn(host, key, user_text, reply_text, is_new, agent="codexcrew-research")
    except TypeError:
...
class TestDiscordMirroredBranchStaysIdempotent:
    ...
    @staticmethod
    def _discord_cls():
        module = __import__("codex_crew.discord.transport_dispatch", fromlist=["_"])
...
class TestTelegramResumedDurability:
    @staticmethod
    def _telegram_cls():
        module = __import__("codex_crew.telegram.transport_dispatch", fromlist=["_"])
        return _dispatcher_class(module)
...
class TestIdentitySurvivesMaterialization:
    ...
    def _surface(state, messages, *, user: str):
        return channel_slots.surface_channel_session(
            state,
            {"key": f"weixin_codexcrew-research_direct_{user}", "title": "t", "modified": 0.0},
            {},
            [dict(r) for r in messages],
            session_key=f"weixin:codexcrew-research:direct:{user}",
        )
