...
def _extract(tarball: Path, into: Path) -> Path:
    ...
    return next(
        d for d in into.iterdir() if d.name.startswith(("codexcrew-snapshot-", "codexcrew-partial-"))
    )


def _snapshot(out: Path, extra: list[str]) -> Path:
    # A selective bundle is now named `codexcrew-partial-*` so an older restore refuses
    # it rather than silently relocating the core files it lacks; a complete one keeps
    # the `codexcrew-snapshot-*` name. Match both so this helper works for either.
