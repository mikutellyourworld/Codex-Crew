...
from codex_crew import agent, sandbox
...
def _compose_enterprise_context(cfg: CodexCrewConfig, monkeypatch):
    """Compose via the companion if importable, else build the overlay inline."""
    try:
        from codex_crew.platform.discovery import discover_companion_context
...
def test_extra_mcp_server_in_agent_config(enterprise_ctx) -> None:
    ...
    assert "codexcrew-core" in servers
    assert "codexcrew-cron" in servers
...
def test_extra_mcp_server_on_refresh(enterprise_ctx) -> None:
    ...
    existing: dict = {"name": "codexcrew", "mcpServers": {}}
