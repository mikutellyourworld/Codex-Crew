"""The launch config that selects the engine Codex Crew actually installs."""
...
from codex_crew.browser_cli import launch as mod
...
def test_the_launch_config_is_write_protected_from_the_agent(
    ...
    for command in (
        "echo x > ~/.codex-crew/playwright-cli-config.json",
        "echo x > $HOME/.codex-crew/playwright-cli-config.json",
        "echo x > ~/.codexcrew/playwright-cli-config.json",  # legacy data home
        "tee ~/.codex-crew/playwright-cli-config.json",
        "cp /tmp/evil.json ~/.codex-crew/playwright-cli-config.json",
    ):
...
def test_launch_config_shell_protection_matches_an_existing_protected_leaf() -> None:
    ...
    for form in (
        "echo x > ~/.codex-crew/{leaf}",
        "echo x > $HOME/.codex-crew/{leaf}",
        "echo x > ~/.codexcrew/{leaf}",
        "tee ~/.codex-crew/{leaf}",
        "cd ~/.codex-crew && printf x > {leaf}",
    ):
        assert (
...
def test_gateway_startup_merges_the_override(monkeypatch: pytest.MonkeyPatch) -> None:
    """The variable has to reach a command line Codex Crew never constructs, so the
...
def test_an_inherited_generated_name_is_regenerated() -> None:
    ...
    repo's ``codexcrew-worktree-dev`` skill tells an agent to run) inherits its
...
def test_both_agent_spawn_paths_name_their_browser_session() -> None:
    ...
    Codex Crew spawns an agent through two independent paths -- ``AcpClient``
    ...
    from codex_crew.acp import client, runtime
...
def test_browser_socket_env_ignores_an_inherited_root_from_another_home(
    ...
    """The trigger flows change ``CODEXCREW_HOME``, so identity would miss them.

    ``dev-backend.sh`` exports its own ``CODEXCREW_HOME`` and a pod runs an
