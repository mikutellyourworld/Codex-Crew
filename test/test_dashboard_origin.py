...
class TestBindAddressFor:
    """CODEXCREW_BIND overrides ONLY the TCP bind address (container support).
    ...
    def test_default_is_loopback(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.delenv("CODEXCREW_BIND", raising=False)
    ...
    def test_invalid_value_falls_back_to_loopback(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setenv("CODEXCREW_BIND", "all-interfaces-please")
        assert bind_address_for(True) == "127.0.0.1"
    ...
    def test_hostname_rejected(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setenv("CODEXCREW_BIND", "eth0.local")
        assert bind_address_for(True) == "127.0.0.1"
    ...
    def test_override_does_not_touch_local_only_urls(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        origins = build_allowed_origins(5486, True)
        assert "http://localhost:5486" in origins
...
class TestProbePaths:
    ...
    def test_exact_membership(self) -> None:
        from codex_crew.dashboard.origin import PROBE_PATHS
...
class TestBuildAllowedOrigins:
    def test_default_origins(self) -> None:
        origins = build_allowed_origins(5486, local_only=True)
        assert "http://127.0.0.1:5486" in origins
        assert "http://localhost:5486" in origins
        assert "http://codexcrew.localhost:5486" in origins

    def test_configured_host_adds_http_with_port(self) -> None:
        origins = build_allowed_origins(5486, local_only=True, configured_host="myhost")
        assert "http://myhost:5486" in origins

    def test_dashboard_url_empty_no_extra_origin(self) -> None:
        baseline = build_allowed_origins(5486, local_only=True)
        with_empty = build_allowed_origins(5486, local_only=True, dashboard_url="")
        assert baseline == with_empty

    def test_dashboard_url_https_adds_origin(self) -> None:
        origins = build_allowed_origins(
            5486, local_only=True, dashboard_url="https://codexcrew.local"
        )
        assert "https://codexcrew.local" in origins

    def test_dashboard_url_http_with_port(self) -> None:
        origins = build_allowed_origins(5486, local_only=True, dashboard_url="http://myhost:8080")
        assert "http://myhost:8080" in origins

    def test_dashboard_url_no_scheme_normalized(self) -> None:
        origins = build_allowed_origins(5486, local_only=True, dashboard_url="myhost:8080")
        assert "http://myhost:8080" in origins
    ...
    def test_dashboard_url_preserves_existing_origins(self) -> None:
        origins = build_allowed_origins(
            ...
            dashboard_url="https://codexcrew.local",
        )
        assert "http://myhost:5486" in origins
        assert "https://codexcrew.local" in origins
        assert "http://localhost:5486" in origins
    ...
    def test_dashboard_url_strips_default_https_port(self) -> None:
        origins = build_allowed_origins(
            5486, local_only=True, dashboard_url="https://codexcrew.local:443"
        )
        assert "https://codexcrew.local" in origins
        assert "https://codexcrew.local:443" not in origins

    def test_dashboard_url_strips_default_http_port(self) -> None:
        origins = build_allowed_origins(
            5486, local_only=True, dashboard_url="http://codexcrew.local:80"
        )
        assert "http://codexcrew.local" in origins
        assert "http://codexcrew.local:80" not in origins

    def test_dashboard_url_keeps_non_default_port(self) -> None:
        origins = build_allowed_origins(
            5486, local_only=True, dashboard_url="https://codexcrew.local:8443"
        )
        assert "https://codexcrew.local:8443" in origins

    def test_dashboard_url_malformed_port_ignored(self) -> None:
        origins = build_allowed_origins(5486, local_only=True, dashboard_url="https://host:abc")
...
class TestDashboardOrigin:
    ...
    def test_https_url(self) -> None:
        assert dashboard_origin("https://codexcrew.local") == "https://codexcrew.local"
...
class TestParseDashboardUrlMalformed:
    ...
    def test_malformed_ipv6_falls_back_to_defaults(self) -> None:
        ...
        assert host == ""
        assert port == 5486  # _DEFAULT_PORT
...
_MOD = "codex_crew.dashboard.urls"


class TestBuildDashboardUrl:
    def test_token_appended(self) -> None:
        assert (
            build_dashboard_url("http://localhost:5486", "abc") == "http://localhost:5486?token=abc"
        )

    def test_empty_token_returns_bare_url(self) -> None:
        assert build_dashboard_url("http://localhost:5486") == "http://localhost:5486"

    def test_not_local_without_token_raises(self) -> None:
        with pytest.raises(ValueError, match="token is required"):
            build_dashboard_url("http://host:5486", "", local_only=False)

    def test_local_without_token_ok(self) -> None:
        assert (
            build_dashboard_url("http://localhost:5486", "", local_only=True)
            == "http://localhost:5486"
        )

    def test_not_local_with_token_ok(self) -> None:
        url = build_dashboard_url("http://host:5486", "tok", local_only=False)
        assert url == "http://host:5486?token=tok"

    def test_special_chars_in_token_are_encoded(self) -> None:
        url = build_dashboard_url("http://localhost:5486", "a&b=c#d")
        assert url == "http://localhost:5486?token=a%26b%3Dc%23d"
    ...
    def test_truthy_non_bool_local_only_still_requires_token(self) -> None:
        ...
        with pytest.raises(ValueError, match="token is required"):
            build_dashboard_url("http://host:5486", "", local_only="yes")  # type: ignore[arg-type]
...
class TestFormatDashboardUrls:
    ...
    def test_local_direct_url(self, _mh: object, _dp: object) -> None:
        lines = format_dashboard_urls("http://localhost:5486", port=5486)
        assert len(lines) == 2
        assert lines[0] == ">_ Dashboard:"
        assert "http://localhost:5486" in lines[1]

    @patch.dict("os.environ", {}, clear=True)
    @patch(f"{_MOD}.devspaces_proxy_url", return_value=None)
    @patch(f"{_MOD}.machine_hostname", return_value="localhost")
    def test_token_in_url_shown(self, _mh: object, _dp: object) -> None:
        lines = format_dashboard_urls("http://localhost:5486?token=abc", port=5486)
    ...
    def test_remote_ssh_tunnel_instructions(self, _dns: object, _mh: object, _dp: object) -> None:
        lines = format_dashboard_urls("http://localhost:5486?token=t", port=5486)
        assert any("ssh -NL 5486:localhost:5486 myhost" in ln for ln in lines)
        assert any("http://localhost:5486?token=t" in ln for ln in lines)
    ...
    def test_local_with_resolvable_host_adds_remote_hint(
        ...
        lines = format_dashboard_urls("http://localhost:5486", port=5486, local_only=True)
    ...
    def test_custom_host_suppresses_remote_hint(
        ...
        lines = format_dashboard_urls("http://localhost:5486", port=5486, has_custom_host=True)
    ...
    def test_devspaces_proxy_shown_when_not_local(self, _mh: object, _dp: object) -> None:
        lines = format_dashboard_urls("http://host:5486?token=t", port=5486, local_only=False)
    ...
    def test_token_propagated_to_proxy_url(self, _mh: object, _dp: object) -> None:
        lines = format_dashboard_urls("http://host:5486?token=abc", port=5486, local_only=False)
        proxy_line = [ln for ln in lines if "Proxy" in ln][0]
        assert "proxy.devspaces.example.com?token=abc" in proxy_line

    def test_not_local_without_token_raises(self) -> None:
        with pytest.raises(ValueError, match="token is required"):
            format_dashboard_urls("http://host:5486", port=5486, local_only=False)

    def test_not_local_with_non_token_query_raises(self) -> None:
        with pytest.raises(ValueError, match="token is required"):
            format_dashboard_urls("http://host:5486?debug=1", port=5486, local_only=False)

    def test_truthy_non_bool_local_only_raises(self) -> None:
        with pytest.raises(ValueError, match="token is required"):
            format_dashboard_urls("http://host:5486", port=5486, local_only="yes")  # type: ignore[arg-type]
...
class TestCheckOriginLoopbackTrust:
    ...
    def _make_request(
        ...
        *host* sets the request ``Host`` header (used by the same-origin
        ...
        # Only allow port 5486 — simulates the default config
        if allowed is None:
            allowed = {"http://localhost:5486", "http://127.0.0.1:5486"}
    ...
    def test_opted_in_loopback_port_trusted(self) -> None:
        """A loopback port the operator added (via CODEXCREW_ALLOWED_LOOPBACK_PORTS,
    ...
    def test_same_origin_loopback_port_trusted(self) -> None:
        ...
        request = self._make_request(
            "http://codexcrew.localhost:7779",
            host="codexcrew.localhost:7779",
        )
        assert check_origin(request) is True
    ...
    def test_origin_host_mismatch_rejected(self) -> None:
        ...
        request = self._make_request("http://localhost:9999", host="codexcrew.localhost:7779")
        assert check_origin(request) is False
...
class TestAllowedLoopbackPortsEnv:
    """CODEXCREW_ALLOWED_LOOPBACK_PORTS opts specific loopback ports into the allowed set."""

    @patch.dict("os.environ", {"CODEXCREW_ALLOWED_LOOPBACK_PORTS": "8777,9000"}, clear=True)
    ...
    @patch.dict("os.environ", {"CODEXCREW_ALLOWED_LOOPBACK_PORTS": "notaport"}, clear=True)
...
class TestShouldCanonicalizeHost:
    ...
    def test_host_without_port(self) -> None:
        assert should_canonicalize_host(
            "localhost", "codexcrew.localhost", method="GET", sec_fetch_dest="document"
        )
    ...
    def test_ipv6_loopback_bracket_host_without_port(self) -> None:
        assert should_canonicalize_host(
            "[::1]", "codexcrew.localhost", method="GET", sec_fetch_dest="document"
        )
...
class TestBuildHostCanonicalRedirect:
    ...
    async def test_document_nav_302s_preserving_port_path_query(self) -> None:
        ...
        from codex_crew.dashboard.server import build_host_canonical_redirect

        async def _ok(_request: web.Request) -> web.Response:
            return web.Response(text="ok")

        app = web.Application(middlewares=[build_host_canonical_redirect("codexcrew.localhost")])
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            assert loc.hostname == "codexcrew.localhost"  # host converged
...
class TestBuildAllowedHosts:
    ...
    def test_loopback_floor_always_present(self) -> None:
        from codex_crew.dashboard.origin import build_allowed_hosts

        hosts = build_allowed_hosts(set())
        assert {"localhost", "127.0.0.1", "::1", "codexcrew.localhost"} <= hosts

    def test_hostnames_extracted_port_stripped(self) -> None:
        from codex_crew.dashboard.origin import build_allowed_hosts

        hosts = build_allowed_hosts({"http://myhost:8080", "https://codexcrew.example.com"})
...
class TestCheckHost:
    ...
    def test_spoofed_host_rejected(self) -> None:
        from codex_crew.dashboard.origin import check_host
    ...
    def test_localhost_host_accepted(self) -> None:
        ...
        assert check_host(self._make_request("localhost:5486")) is True

    def test_loopback_ip_host_accepted(self) -> None:
        from codex_crew.dashboard.origin import check_host

        assert check_host(self._make_request("127.0.0.1:5486")) is True
    ...
    def test_configured_remote_host_accepted(self) -> None:
        ...
        allowed = {"http://localhost:5486", "https://codexcrew.example.com"}
        req = self._make_request("codexcrew.example.com", allowed=allowed)
    ...
    def test_missing_allowed_origins_denied(self) -> None:
        ...
        request = MagicMock()
        request.headers = {"Host": "localhost:5486"}
...
class TestFrameAncestorsValue:
    ...
    def test_self_is_always_present(self) -> None:
        ...
        """
        from codex_crew.dashboard.origin import frame_ancestors_value
    ...
    def test_an_embedding_ancestor_is_added(self) -> None:
        ...
        assert frame_ancestors_value(["http://localhost:5486"]) == "'self' http://localhost:5486"

    def test_several_ancestors_keep_their_order(self) -> None:
        from codex_crew.dashboard.origin import frame_ancestors_value

        value = frame_ancestors_value(["http://127.0.0.1:5486", "http://localhost:5486"])
        assert value == "'self' http://127.0.0.1:5486 http://localhost:5486"

    def test_a_duplicate_ancestor_is_not_repeated(self) -> None:
        from codex_crew.dashboard.origin import frame_ancestors_value

        value = frame_ancestors_value(["http://localhost:5486", "http://localhost:5486"])
        assert value == "'self' http://localhost:5486"

    @pytest.mark.parametrize(
        "bad",
        [
            "http://[::1]:5486",
            "http://localhost:5486/path",
            "http://localhost:5486; script-src *",
            "http://localhost:5486 http://evil.example",
            "'self'",
            "*",
            "",
        ],
