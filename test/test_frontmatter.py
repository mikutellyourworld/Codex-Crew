...
from codex_crew import history
from codex_crew.frontmatter import (
    ONBOARDING_IMPORT,
    SKILL_LOADER,
    SKILL_UPDATE,
    STEERING_LOADER,
...
from codex_crew.onboarding_import import _column0_activation_declared, _frontmatter
...
class TestTheRepoSkillFileCorpus:
    ...
    NOT_VALID_YAML = frozenset(
        {
            "src/codex_crew/builtin_skills/codexcrew-dev/prepare-pr/SKILL.md",
            "src/codex_crew/builtin_skills/web-verify/SKILL.md",
        }
    )
...
class TestChompingCannotFlipAnActivationFlag:
    ...
    def test_a_block_scalar_repo_scope_still_gates_to_the_same_path(self) -> None:
        ...
        for label, header, body in (
            ("keep chomped", "|+", ["  src/codex_crew", "", ""]),
            ("clip chomped", "|", ["  src/codex_crew", ""]),
            ("plain literal", "|", ["  src/codex_crew"]),
            ("strip chomped", "|-", ["  src/codex_crew"]),
            ("folded", ">", ["  src/codex_crew"]),
        ):
            text = "\n".join(["---", "name: s", f"repo_scope: {header}", *body, "---", "", "# B"])
            value = parse_frontmatter(text, SKILL_LOADER)["repo_scope"]
            assert value.strip() == "src/codex_crew", f"{label}: {value!r}"
    ...
    def test_every_repo_scope_gate_site_normalises(self) -> None:
        ...
        src = (Path(__file__).resolve().parents[1] / "src/codex_crew/skills.py").read_text()
...
def test_removing_the_last_field_keeps_the_body_blank_lines():
    ...
    from codex_crew.frontmatter import STEERING_LOADER, set_frontmatter_fields
...
class TestEmptyFenceIsStillAFence:
    ...
    def test_split_frontmatter_reports_no_fields_and_the_real_body(self):
        from codex_crew.frontmatter import STEERING_LOADER, split_frontmatter
