...
* ``_resolve_checkout`` matched only the worktree DIRECTORY basename, including
  in the branch-matching awk branch, so a short pod name that ``codexcrew pod up``
...
def test_resolver_prefers_feat_over_another_branch_with_the_same_leaf(tmp_path):
    ...
    `codexcrew pod up foo` resolves feat/foo, so the harness must too — otherwise
...
def stub_cli(tmp_path: Path):
    """A fake `codexcrew` whose `pod status --json` health value is injectable."""

    def _make(health: str) -> str:
        path = tmp_path / "codexcrew-stub"
        path.write_text(
            textwrap.dedent(f"""
                #!/usr/bin/env bash
