...
BASE_VERSION = "1.2.3"
RUN_ID = 12345
IMAGE = "ghcr.io/kirodotdev/codexcrew"
...
def _bundle(path: Path, extra: dict[str, bytes] | None = None) -> Path:
    path.mkdir()
    files = {
        "codexcrew-1.2.3rc4-py3-none-any.whl": b"wheel",
        "codexcrew-1.2.3rc4.tar.gz": b"sdist",
        "CodexCrew-x86_64.AppImage": b"appimage",
        "CodexCrew-aarch64.AppImage": b"appimage-arm64",
        "CodexCrew-x86_64.deb": b"deb",
        "CodexCrew-aarch64.deb": b"deb-arm64",
        "CodexCrew-x86_64.rpm": b"rpm",
        "CodexCrew-aarch64.rpm": b"rpm-arm64",
...
def test_manifest_rejects_distribution_filename_from_other_wheel_version(
    ...
    cases = (
        (
            "wheel",
            "codexcrew-1.2.3rc4-py3-none-any.whl",
            "codexcrew-9.9.9-py3-none-any.whl",
        ),
        ("sdist", "codexcrew-1.2.3rc4.tar.gz", "codexcrew-9.9.9.tar.gz"),
    )
...
def test_wheel_filename_binding_requires_exact_match() -> None:
    ...
    promotion._require_wheel_version_filename(
        "wheel", "codexcrew-1.2.3rc4-py3-none-any.whl", "1.2.3rc4"
    )
    for bad in (
        "codexcrew-1.2.3rc40-py3-none-any.whl",
        "codexcrew-1.2.3rc4.post1-py3-none-any.whl",
        "codexcrew-1.2.3rc4-1-py3-none-any.whl",
    ):
        with pytest.raises(
...
def test_tampered_candidate_file_fails_closed(tmp_path: Path) -> None:
    bundle = _bundle(tmp_path / "bundle")
    (bundle / "CodexCrew.dmg").write_bytes(b"different bytes")
...
def _mock_resolver_api(
    ...
    def fake_download(repository: str, artifact_id: int, destination: Path) -> None:
        assert repository == "mikutellyourworld/Codex-Crew"
...
def test_matching_recorded_archive_digest_promotes_candidate(
    ...
    manifest, selected = promotion.resolve_candidate(
        repository="mikutellyourworld/Codex-Crew",
        source_sha=SOURCE_SHA,
        base_version=BASE_VERSION,
...
WINDOWS_ROLES = {
    "CodexCrew-Setup.exe": b"nsis-installer",
    "CodexCrew-Setup.exe.blockmap": b"blockmap",
}
...
def test_a_candidate_with_windows_promotes_the_installer_and_its_blockmap(
    ...
    assert manifest["artifacts"]["windows_installer"]["filename"] == "CodexCrew-Setup.exe"
    assert (
        manifest["artifacts"]["windows_blockmap"]["filename"]
        == "CodexCrew-Setup.exe.blockmap"
    )
...
def test_optionality_does_not_let_an_unlisted_file_ride_along(tmp_path: Path) -> None:
    ...
    bundle = _bundle(tmp_path / "bundle", WINDOWS_ROLES)
    (bundle / "CodexCrew-Setup.exe.sig").write_bytes(b"extra")
...
def test_a_tampered_windows_installer_fails_closed(tmp_path: Path) -> None:
    bundle = _bundle(tmp_path / "bundle", WINDOWS_ROLES)
    (bundle / "CodexCrew-Setup.exe").write_bytes(b"nsis-installeR")
