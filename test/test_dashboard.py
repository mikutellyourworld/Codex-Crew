...
from codex_crew.notifications.bus import payload_from_legacy
...
class TestNotificationPersistence:
    ...
    def test_trim_large_file(self, monkeypatch, tmp_path) -> None:
        ...
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        monkeypatch.setattr("codex_crew.dashboard.state._MAX_PERSISTED_NOTIFICATIONS", 5)
        path = tmp_path / "notifications.jsonl"
        # Write 11 lines (> 2 * 5)
        lines: list[str] = []
        for i in range(11):
    ...
    def test_deliver_note_offloads_persist_on_running_loop(
        ...
        monkeypatch.setattr("codex_crew.dashboard.state.config_dir", lambda: tmp_path)
        from codex_crew.dashboard.state import _notification_io_executor
