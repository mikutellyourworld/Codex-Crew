...
from codex_crew.dashboard.handlers.taskrunner import (
...
from codex_crew.task_models import Project
from codex_crew.taskrunner import Step, StepStatus, TaskRun, TaskRunner, _auto_approve_scope
...
class TestSettersSetAutoApprove:
    ...
    async def test_execute_plan_sets_auto_approve(self, tmp_path: Path) -> None:
        ...
        with patch(
            "codex_crew.taskrunner.asyncio.create_task",
...
class TestAutoApproveRespectsHookDeny:
    ...
    async def test_hook_auto_approve_reason_preserved(self, tmp_path: Path) -> None:
        ...
        with patch("codex_crew.task_executor.self_review", return_value=True), patch(
            "codex_crew.task_executor.sel"
        ) as mock_sel:
            await runner._execute_single_task(run, step)
...
class TestAutoApproveProvenanceGating:
    ...
    async def test_execute_gate_audit_failure_fails_closed(self) -> None:
        ...
        with patch("codex_crew.dashboard.handlers.taskrunner._sel", return_value=boom):
