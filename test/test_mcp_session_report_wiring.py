...
class TestPublish:
    def test_publishes_and_broadcasts(self):
        ...
        report.begin_session([{"name": "codexcrew-core"}])
        report.record_frame(_ready_frame("codexcrew-core"), owned=True)
        acp = MagicMock()
...
class TestLiveEventOwnershipIsWired:
    ...
    def test_an_ownerless_event_does_not_reach_the_report(self):
        from codex_crew.acp.types import EVENT_MCP_SERVER_INITIALIZED
        from codex_crew.dashboard.chat_runner import _record_session_mcp_event
    ...
    def test_the_roster_keeps_the_stall_clocks_ownership_rule(self):
        ...
        from codex_crew.acp import session_handle as handle_mod
...
class TestBothTransportsAreReached:
    ...
    def test_a_provider_with_an_inner_client_reports_too(self):
        ...
        payload = slot.mcp_report_payload()
        assert payload is not None and payload["ready"] == ["codexcrew-core"]
...
class TestSerializerDropsOrphanedReport:
    ...
    def test_a_published_report_survives_its_own_serialize(self):
        ...
        assert payloads[0]["mcp_report"]["ready"] == ["codexcrew-core"]
