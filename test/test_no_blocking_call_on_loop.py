...
lexically inside an ``async def`` body in ``codex_crew`` source. This is the
...
def _src_root() -> pathlib.Path:
    """Locate the codex_crew source tree (see ``source_corpus.src_root``)."""
    return src_root()
...
def collect_repo_violations() -> list[tuple[str, int, str]]:
    """Scan every ``codex_crew/**/*.py`` for on-loop blocking calls."""
...
def test_no_blocking_call_on_event_loop() -> None:
    ...
    if violations:
        ...
        raise AssertionError(
            ...
            "codex_crew.executors, or switch to an async API (e.g. "
