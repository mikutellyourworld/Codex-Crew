...
from codex_crew.platform.governance import resolve
...
def test_missing_profiles_dir_is_absent_no_warning(profiles_dir, monkeypatch, caplog):
    ...
    # (fresh CODEXCREW_HOME). It must publish an empty index (policy-only) WITHOUT
    ...
    gp.reset_store()
    with caplog.at_level(logging.WARNING, logger="codex_crew.platform.governance_profiles"):
        assert gp.resolve_active_scope("cli_chat") is None
...
def test_runtime_hot_added_unrecoverable_marks_health_incident(profiles_dir, monkeypatch):
    ...
    # The reload does `from codex_crew.platform.context import current_context`
    ...
    incidents: list = []
    import codex_crew.platform.governance_health as gh
...
def test_index_published_atomically_as_one_snapshot(profiles_dir):
    ...
    from codex_crew.platform.governance_profiles import _Snapshot
...
def test_metadata_change_reload_walks_dir_once(profiles_dir, monkeypatch):
    ...
    import codex_crew.platform.governance_profiles as gpm
