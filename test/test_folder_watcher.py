...
from codex_crew.knowledge import folder_watcher
from codex_crew.knowledge.connectors.local_folder import LocalFolderConnector
from codex_crew.knowledge.folder_watcher import MAX_SCAN_ATTEMPTS, FolderWatcher
...
class TestHashFileSecurity:
    ...
    def test_hash_sensitive_path_returns_none(self, store, pipeline, tmp_path):
        ...
        f = tmp_path / "normal.md"
        f.write_text("hello")
        with patch("codex_crew.knowledge.folder_watcher.is_sensitive_path", return_value=True):
            result = fw._hash_file(str(f))
...
class TestAsyncToThread:
    ...
    async def test_per_file_dedup_runs_off_the_event_loop(
        ...
        monkeypatch.setattr(
            "codex_crew.knowledge.folder_watcher.dedup_document",
