"""Additional unit coverage for :mod:`codex_crew.apps.backend`.
...
from codex_crew.apps.backend import AppProcess
...
def boot_env(monkeypatch: pytest.MonkeyPatch) -> dict[str, list[Any]]:
    ...
    monkeypatch.setattr("codex_crew.apps.manager._app_activation_denied", lambda _name: None)
    ...
    monkeypatch.setattr("codex_crew.apps.bridges._deregister_mcp_servers", _dereg_mcp)
    monkeypatch.setattr("codex_crew.apps.bridges._deregister_agents", _dereg_agents)
    monkeypatch.setattr("codex_crew.apps.bridges._deregister_skills", _dereg_skills)
    monkeypatch.setattr("codex_crew.apps.bridges.register_app", _register)
    monkeypatch.setattr("codex_crew.apps.bridges.reconcile_app_skills", _reconcile)
    return calls
...
class TestDependencyInstall:
    ...
    def test_the_installer_never_shells_out_to_a_bare_interpreter(
        ...
        from codex_crew.apps.interpreter import venv_python_path
...
class TestSpawnEnvironment:
    def test_platform_overrides_and_the_proxy_secret_reach_the_backend(
        ...
        monkeypatch.setenv("CODEXCREW_PROJECT_DIR", "/checkout")
        monkeypatch.setenv("CODEXCREW_EDITION_DIR", "/edition")
        monkeypatch.setenv("CODEXCREW_DEVFLEET_REPO", "/opt/codexcrew")
        monkeypatch.setenv("CODEXCREW_DEVFLEET_BIN_GH", "/opt/bin/gh")
        seen = _capture_popen(monkeypatch)
        with pytest.raises(_StopSpawn):
            bmod._start_app_backend_body("envapp", _manifest("server.py"))
        env = seen["kwargs"]["env"]
        assert env["CODEXCREW_PROJECT_DIR"] == "/checkout"
        assert env["CODEXCREW_EDITION_DIR"] == "/edition"
        assert env["CODEXCREW_DEVFLEET_REPO"] == "/opt/codexcrew"
        assert env["CODEXCREW_DEVFLEET_BIN_GH"] == "/opt/bin/gh"
        assert env["CODEXCREW_PROXY_SECRET"] == "s3cr3t"
        assert env["CODEXCREW_APP_NAME"] == "envapp"
    ...
    def test_a_missing_proxy_secret_is_tolerated(
        ...
        assert "CODEXCREW_PROXY_SECRET" not in seen["kwargs"]["env"]
...
class TestGateMcpRegistration:
    def test_a_healthy_backend_is_registered_with_its_live_port(
        ...
        monkeypatch.setattr(
            "codex_crew.apps.bridges.reregister_app_mcp_servers",
    ...
    def test_an_unhealthy_backend_has_its_entries_scrubbed(
        ...
        monkeypatch.setattr("codex_crew.apps.bridges._deregister_mcp_servers", lambda _n: 2)
        with caplog.at_level(logging.WARNING):
    ...
    def test_a_reconcile_failure_never_crashes_the_health_loop(
        ...
        monkeypatch.setattr("codex_crew.apps.bridges.reregister_app_mcp_servers", _boom)
        with caplog.at_level(logging.WARNING):
            bmod._gate_mcp_registration("app", 9133, healthy=True)
...
class TestBootMcpReconcile:
    ...
    def test_a_failing_scrub_does_not_crash_boot(
        ...
        monkeypatch.setattr("codex_crew.apps.bridges._deregister_mcp_servers", _boom)
        monkeypatch.setattr(
            bmod,
            "list_apps",
            lambda: [_app("off", enabled=False, manifest={"mcpServers": {"x": {}}})],
...
class TestBootResourceReconcile:
    ...
    def test_registration_errors_are_surfaced_not_swallowed(
        ...
        monkeypatch.setattr(
            "codex_crew.apps.bridges.register_app",
    ...
    def test_a_failing_reconcile_does_not_crash_boot(
        ...
        monkeypatch.setattr("codex_crew.apps.bridges.register_app", _boom)
    ...
    def test_a_denied_app_has_its_derivative_resources_revoked(
        ...
        monkeypatch.setattr(
            "codex_crew.apps.manager._app_activation_denied", lambda _n: "not in allowlist"
        )
    ...
    def test_a_failed_revocation_is_logged_as_an_error(
        ...
        monkeypatch.setattr("codex_crew.apps.bridges._deregister_agents", _boom)
        monkeypatch.setattr(
            "codex_crew.apps.manager._app_activation_denied", lambda _n: "banned"
        )
        monkeypatch.setattr(bmod, "list_apps", lambda: [_app("banned")])
        with caplog.at_level(logging.ERROR):
            bmod.start_enabled_app_backends()
    ...
    def test_an_unavailable_bridges_module_aborts_only_the_reconcile(
        ...
        monkeypatch.setitem(sys.modules, "codex_crew.apps.bridges", SimpleNamespace())
...
class TestBootSpawnGating:
    def test_a_governance_denied_app_is_not_spawned(
        ...
        monkeypatch.setattr(
            "codex_crew.apps.manager._app_activation_denied", lambda _n: "not allowed"
        )
