...
from codex_crew.acp.session_handle import AcpRuntimeError, AcpSessionHandle
...
async def test_agent_switch_extracted_from_result():
    ...
    handle, _ = _make(
        response_result={
            "message": "switched",
            "data": {"agent": {"name": "codexcrew"}},
        }
    )
    events = await _collect(handle, "/agent codexcrew")
    switches = [ev for ev in events if ev.kind == EVENT_AGENT_SWITCHED]
    assert [ev.text for ev in switches] == ["codexcrew"]
...
async def test_agent_switch_not_duplicated_when_notification_arrived():
    ...
    notification = JsonRpcMessage(
        ...
        params={"sessionId": "sA", "agentName": "codexcrew"},
    )
    handle, _ = _make(
        response_result={"data": {"agent": {"name": "codexcrew"}}},
...
async def test_prompt_transport_commands_stay_on_prompt(command):
    ...
    from codex_crew.acp.types import METHOD_PROMPT
...
async def test_kas_backend_falls_back_to_prompt():
    ...
    from codex_crew.acp.types import ACP_BACKEND_KAS, METHOD_PROMPT
