...
from codex_crew.dashboard.handlers import knowledge as kh
from codex_crew.knowledge.store import KnowledgeStore

MODULE = "codex_crew.dashboard.handlers.knowledge"
...
class TestImportBundle:
    ...
    async def test_recursion_error_during_validation_is_400_not_500(
        ...
        from codex_crew.dashboard.handlers import knowledge as knowledge_mod
...
class TestAddAgentDocumentRoute:
    ...
    async def test_disabled_toggle_is_403(self, store, monkeypatch):
        monkeypatch.setattr(f"{MODULE}.CodexCrewConfig.load",
    ...
    async def test_missing_pipeline_is_503(self, store, monkeypatch):
        monkeypatch.setattr(f"{MODULE}.CodexCrewConfig.load", staticmethod(_cfg))
        async with _client(_make_app(store)) as client:
            resp = await client.post("/api/knowledge/agent-document", json={})
            assert resp.status == 503
