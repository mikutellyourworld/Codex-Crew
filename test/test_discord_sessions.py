...
from codex_crew.discord import session_resume
from codex_crew.discord.client import DISCORD_CHUNK_LIMIT, DiscordInteraction
from codex_crew.discord.commands import parse_command, parse_command_argument
from codex_crew.discord.renderer import session_provenance_tag
from codex_crew.discord.transport_dispatch import DiscordDispatcher
from codex_crew.messaging import resume_expectation
from codex_crew.messaging import session_resume as session_resume_core
...
class _Provider:
    ...
    async def stream(self, message: str):
        from codex_crew.acp.types import EVENT_COMPLETE, EVENT_TEXT_CHUNK
...
class _ConversationLog:
    ...
    def search_sessions(self, query: str, limit: int = 50) -> list[dict]:
        """Mirror CodexCrewHistory.search_sessions' FIELD COVERAGE and phrase
...
async def test_sessions_delegates_to_the_shared_search() -> None:
    ...
    The scoring formula belongs to CodexCrewHistory.search_sessions and is tested
...
async def test_sessions_lists_only_persistent_dashboard_sessions_and_redacts() -> None:
    ...
    log = _ConversationLog(
        [
            ...
            {
                "key": "discord_codexcrew_direct_u1",
...
class TestDashboardConnectedConversationResumes:
    """The reported bug: replying to a message from Codex Crew forked a new session.
