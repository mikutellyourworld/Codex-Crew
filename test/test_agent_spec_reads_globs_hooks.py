...
- ``test_oversized_codexcrew_json_degrades_to_empty`` — old handler had no size
  cap and served the oversized file's hooks.
- ``test_non_utf8_codexcrew_json_degrades_to_empty`` — old handler decoded with
...
import codex_crew.hooks as kc_hooks
from codex_crew.agent_discovery import agent_skill_globs
from codex_crew.dashboard.handlers.hooks import api_kiro_hooks
...
# handler's namespace.
_P_AGENTS_DIR = "codex_crew.agent.KIRO_AGENTS_DIR"
_P_DEFAULTS = "codex_crew.dashboard.handlers.hooks._shipped_defaults"
...
class TestAgentSkillGlobsHardenedRead:
    ...
    def test_sensitive_symlink_target_skipped_and_sel_denied(
        ...
        monkeypatch.setattr(
            "codex_crew.agent_discovery.is_sensitive_path", lambda p: str(p) == resolved
        )
...
class TestApiKiroHooksHardenedRead:
    ...
    @pytest.mark.asyncio
    async def test_oversized_codexcrew_json_degrades_to_empty(
        self, kiro_dir: Path, defaults: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """An over-cap ``codexcrew.json`` yields no user hooks — 200, not 500."""
        monkeypatch.setattr(kc_hooks, "MAX_FILE_BYTES", 256)
        body = json.dumps(
            {
        ...
        assert len(body.encode("utf-8")) > 256
        (kiro_dir / "codexcrew.json").write_text(body, encoding="utf-8")
        status, payload = await _get_hooks(kiro_dir, defaults)
        assert status == 200
        assert payload == {"hooks": {}}

    @pytest.mark.asyncio
    async def test_non_utf8_codexcrew_json_degrades_to_empty(
        ...
        (kiro_dir / "codexcrew.json").write_bytes(b'\xff\xfe{"hooks": {}}')
        status, payload = await _get_hooks(kiro_dir, defaults)
        assert status == 200
        assert payload == {"hooks": {}}
    ...
    async def test_non_dict_hooks_value_degrades_to_empty(
        ...
        (kiro_dir / "codexcrew.json").write_text(json.dumps({"hooks": []}), encoding="utf-8")
        status, payload = await _get_hooks(kiro_dir, defaults)
        assert status == 200
        assert payload == {"hooks": {}}

    @pytest.mark.asyncio
    async def test_normal_codexcrew_json_still_serves_hooks(
        self, kiro_dir: Path, defaults: Path
    ) -> None:
