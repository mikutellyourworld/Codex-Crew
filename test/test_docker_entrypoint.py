...
* Sandbox posture is decided by PROBING the product's backend detector on a
  ...
  legacy ``~/.codexcrew`` is owned by the product's migrator.
...
* Explicit ``CODEXCREW_HOME`` / legacy-home resolution mirrors the backend.

These tests execute the REAL script with stubbed ``codexcrew`` and
...
def _run_entrypoint(
    ...
    *args: str,
    ...
    """Run the real entrypoint with fake ``codexcrew``/``python3``/``tini``
    on PATH.
    ...
    ``/usr/bin/false`` to simulate a broken install). The ``codexcrew`` shim
    ...
    env = {
        # Minimal, hermetic environment: no inherited CODEXCREW_HOME and no
...
def test_first_run_without_backend_stays_fail_closed_without_consent(tmp_path: Path) -> None:
    ...
    assert "CODEXCREW_ALLOW_UNSANDBOXED=1" in result.stdout, (
...
def test_first_run_without_backend_seeds_opt_out_with_explicit_consent(tmp_path: Path) -> None:
    """With -e CODEXCREW_ALLOW_UNSANDBOXED=1 the operator has explicitly
    ...
    result = _run_entrypoint(
        ...
        extra_env={"CODEXCREW_ALLOW_UNSANDBOXED": "1"},
    )
    assert result.returncode == 0, result.stderr
    ...
    assert "CODEXCREW_ALLOW_UNSANDBOXED=1 given" in result.stdout
...
def test_explicit_codexcrew_home_env_is_honored(tmp_path: Path) -> None:
    """CODEXCREW_HOME overrides the default seed location (parity with the
    ...
    result = _run_entrypoint(
        ...
        probe_backend_available=False,
        extra_env={
            "CODEXCREW_HOME": str(custom),
            "CODEXCREW_ALLOW_UNSANDBOXED": "1",
        },
    )
    assert result.returncode == 0, result.stderr
...
def test_codexcrew_home_tilde_is_expanded_like_the_backend(tmp_path: Path) -> None:
    """A tilde override (docker -e CODEXCREW_HOME=~/crew-data arrives
    ...
    assert "CODEXCREW_HOME=~/crew-data" in captured


def test_system_dir_override_is_rejected_like_the_backend(tmp_path: Path) -> None:
    """CODEXCREW_HOME pointing at a system directory is ignored by the
