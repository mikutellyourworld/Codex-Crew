...
async def test_question_text_is_redacted_before_broadcast() -> None:
    ...
    with patch(
        "codex_crew.dashboard.state.redact_exfiltration_urls",
        side_effect=lambda s: (
            ("<url-redacted>", 1) if "LEAKY_URL_SENTINEL" in s else (s, 0)
        ),
    ), patch(
        "codex_crew.dashboard.state.redact_credentials",
        side_effect=lambda s: (
...
async def test_handler_rejects_unknown_slot() -> None:
    from codex_crew.dashboard.handlers.ask_question import api_ask_question
...
async def test_answer_handler_resolves_pending_question() -> None:
    from codex_crew.dashboard.handlers.ask_question import api_ask_question_answer
...
async def test_oversized_answer_is_rejected_not_truncated() -> None:
    ...
    """
    from codex_crew.dashboard.handlers.ask_question import api_ask_question_answer
    from codex_crew.validation import _ASK_MAX_ANSWER_LEN
...
async def test_too_many_answer_entries_rejected() -> None:
    from codex_crew.dashboard.handlers.ask_question import api_ask_question_answer
    from codex_crew.validation import _ASK_MAX_QUESTIONS
...
async def test_dashboard_user_token_is_still_allowed() -> None:
    ...
    request.json = _json
    with patch("codex_crew.dashboard.handlers.ask_question.sel"):
        resp = await api_ask_question(request)
...
def test_unblock_pending_waits_releases_both_waits() -> None:
    ...
    from codex_crew.dashboard.chat_handlers import _unblock_pending_waits
    ...
    with patch(
        "codex_crew.dashboard.chat_handlers._reject_pending_approvals"
    ) as rejected:
...
def test_every_stop_path_uses_the_combined_chokepoint() -> None:
    ...
    src = (
        ...
        / "src/codex_crew/dashboard/chat_handlers.py"
...
async def test_pending_endpoint_lists_unanswered_cards() -> None:
    ...
    elapses.
    """
    from codex_crew.dashboard.handlers.ask_question import api_ask_question_pending
...
async def test_reset_chokepoint_cancels_pending_questions() -> None:
    ...
    slot = MagicMock()
    slot.key = "chat-1"
    with patch("codex_crew.dashboard.chat_handlers._reject_pending_approvals"):
...
def test_question_window_stays_under_the_tool_stall_watchdog() -> None:
    ...
    """
    from codex_crew.acp.client import _TOOL_STALL_TIMEOUT
    from codex_crew.validation import ASK_QUESTION_SCHEMA
...
async def test_dismiss_retires_a_stateless_card_status() -> None:
    ...
    """
    from codex_crew.dashboard.handlers.ask_question import api_ask_question_dismiss
    from codex_crew.dashboard.state import _ChatSlot
...
class TestErrorCodes:
    ...
    **What the codes deliberately do NOT add.** The two ``403``s answer for
    ...
    def test_no_refusal_in_this_module_is_prose_only(self) -> None:
        ...
        assert missing == [], (
            "these src/codex_crew/dashboard/handlers/ask_question.py lines refuse "
    ...
    async def test_answer_with_an_over_long_question_key(self) -> None:
        from codex_crew.validation import _ASK_MAX_QUESTION_LEN
    ...
    async def test_every_refusal_carries_both_a_code_and_its_prose(self) -> None:
        ...
        from codex_crew.validation import _ASK_MAX_ANSWER_LEN, _ASK_MAX_QUESTIONS
