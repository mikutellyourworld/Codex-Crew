...
from codex_crew.deploy import webapp_types

# ─── F1: reaper.sh OAC uses fullmatch not startswith ────────────────────────

_REAPER_SH = Path(__file__).resolve().parent.parent / (
    "src/codex_crew/deploy/skills/artifact-deploy/scripts/reaper.sh"
)
...
class TestF3IdentityWiring:
    ...
    def test_writeback_uses_allowed_event_type(self):
        ...
        from pathlib import Path
        src = Path(__file__).parent.parent / "src" / "codex_crew" / "deploy" / "handlers.py"
