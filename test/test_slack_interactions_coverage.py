"""Coverage tests for ``codex_crew.slack.interactions``.
...
writes land in the per-test ``CODEXCREW_HOME`` that ``conftest.py`` pins.
"""
...
from codex_crew.config.loader import ConfigReadError, config_path
from codex_crew.slack import handler as sh
from codex_crew.slack import interactions as ix
...
class TestDispatchRouting:
    ...
    async def test_checkboxes_toggle_is_a_noop(
        ...
        from codex_crew.slack.format import OPTIONS_CHECKBOXES_ACTION
    ...
    async def test_spent_options_marker_is_a_noop(
        ...
        from codex_crew.slack.format import OPTIONS_ACTION_PREFIX
    ...
    async def test_options_blocked_by_channels_governance(
        ...
        from codex_crew.slack.format import OPTIONS_SUBMIT_ACTION
...
class TestDispatchTransportToolApproval:
    ...
    async def test_approve_resolves_and_labels(
        ...
        from codex_crew.slack.renderer import TOOL_APPROVE_ACTION_PREFIX
    ...
    async def test_deny_resolves_false(self, orch: MagicMock, _decider: MagicMock) -> None:
        from codex_crew.slack.renderer import TOOL_DENY_ACTION_PREFIX
    ...
    async def test_trust_grants_session_trust_before_resolving(
        ...
        from codex_crew.slack.renderer import TOOL_TRUST_ACTION_PREFIX
...
class TestResumeChoice:
    ...
    async def test_posts_recent_transcript_context(
        ...
        monkeypatch.setattr("codex_crew.config.loader.data_home", lambda: tmp_path)
