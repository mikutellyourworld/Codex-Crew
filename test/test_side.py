...
6. Agent resolution: the CodexCrew slot agent name (e.g. "default") is resolved
...
from codex_crew import context as context_module
from codex_crew.context import ContextBuilder
from codex_crew.dashboard.handlers.side import (
    _run_side_turn,
    api_side_close,
    api_side_open,
    api_side_turn,
...
def _stub_run_side_turn(monkeypatch, *, answer: str = _SIDE_ANSWER):
    ...
    monkeypatch.setattr("codex_crew.dashboard.handlers.side._run_side_turn", _fake_run)
...
async def test_side_turn_returns_before_run_finishes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.dashboard.handlers.side._run_side_turn", _blocking)
    state = _make_state(tmp_path)
    state.get_or_create_slot("parent")
    app = _make_side_app(state)
...
async def test_side_turn_surfaces_the_actionable_auth_message(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.side.broadcast_side_result",
...
async def test_side_run_id_never_leaks_to_main_channels(tmp_path, monkeypatch):
    ...
    async def _streaming(state, slot, run_id, question, *, is_first_turn):
        from codex_crew.dashboard.ws import broadcast_side_result
    ...
    monkeypatch.setattr("codex_crew.dashboard.handlers.side._run_side_turn", _streaming)
    state = _make_state(tmp_path)
...
async def test_empty_llm_output_produces_visible_fallback(tmp_path, monkeypatch):
    ...
    state.sessions.release = MagicMock()
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.side.stream_and_collect",
...
async def test_side_turn_resolves_slot_agent_to_kiro_agent(tmp_path, monkeypatch):
    """slot.agent (a CodexCrew name like "default") is resolved to the real
    ...
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.side.CodexCrewConfig.load",
        lambda: MagicMock(),
    )
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.side.resolve_agent_bindings",
        lambda cfg, agent, project_dir=None: MagicMock(kiro_agent="codexcrew"),
    )
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.side.stream_and_collect",
    ...
    assert captured["agent"] == "codexcrew", (
...
async def test_side_turn_agent_resolution_falls_back_on_error(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.dashboard.handlers.side.CodexCrewConfig.load", _boom)
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.side.stream_and_collect",
        AsyncMock(return_value="ok"),
    )
...
async def test_side_stream_redacts_credential_split_across_chunks(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.dashboard.handlers.side.stream_and_collect", _fake_stream)
...
async def test_side_turn_refuses_to_substitute_the_default_for_an_app_agent(
    ...
    async def _recover_fails(cfg, _slot, *, project=None):
        ...
        return MagicMock(kiro_agent="codexcrew", requested_resolved=False)

    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.side.CodexCrewConfig.load", lambda: MagicMock()
    )
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.side.resolve_agent_bindings",
        lambda cfg, agent, project_dir=None: MagicMock(
            kiro_agent="codexcrew", requested_resolved=False
        ),
    )
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.side.refresh_materialized_agents",
    ...
    monkeypatch.setattr(
        "codex_crew.dashboard.chat_runner._recover_app_agent_binding",
        _recover_fails,
        raising=False,
    )
    monkeypatch.setattr(
...
async def test_side_turn_self_heals_a_cold_app_agent_and_then_dispatches_it(
    ...
    monkeypatch.setattr("codex_crew.dashboard.handlers.side.resolve_agent_bindings", _resolve)
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.side.refresh_materialized_agents",
    ...
    monkeypatch.setattr(
        "codex_crew.dashboard.chat_runner._recover_app_agent_binding", _recover, raising=False
    )
    monkeypatch.setattr(
        "codex_crew.dashboard.handlers.side.stream_and_collect",
        AsyncMock(return_value="ok"),
...
async def test_side_turn_self_heal_is_scoped_to_app_slots(tmp_path, monkeypatch):
    ...
    assert dispatched == ["codexcrew"], "a non-app slot stopped dispatching: %r" % (dispatched,)
