...
def _cfg(backend: str, model: str) -> CodexCrewConfig:
    ...
    ``~/.kiro/agents/codexcrew.json``) is only consulted for the ``auto``
    sentinel, and the auto case below wants exactly that skipped.
    """
    return CodexCrewConfig(agent=AgentConfig(acp_backend=backend, model=model))
