...
async def test_api_agent_config_put_succeeds(tmp_path):
    ...
    installed.write_text(json.dumps({"name": "codexcrew"}))
    ...
    with (
        ...
        patch(
            "codex_crew.agent.build_agent_config",
...
async def test_api_agent_config_put_strips_governed_grants(tmp_path, monkeypatch):
    ...
    with (
        ...
        patch("codex_crew.dashboard.handlers._reset_all_sessions", new_callable=AsyncMock),
        patch("codex_crew.dashboard.handlers.config_path", return_value=mc_cfg),
        patch("codex_crew.dashboard.handlers.agents.get_shipped_tools", return_value={"tools": [], "allowedTools": []}),
    ):
        response = await api_agent_config(request)
...
async def test_api_agent_config_put_strips_bookkeeping_keys(tmp_path):
    """A dashboard PUT must not re-pollute the kiro spec with Codex Crew keys.
    ...
    with (
        ...
        patch("codex_crew.dashboard.handlers._reset_all_sessions", new_callable=AsyncMock),
        patch("codex_crew.dashboard.handlers.config_path", return_value=mc_cfg),
        patch("codex_crew.agent.get_shipped_tools", return_value={"tools": [], "allowedTools": []}),
    ):
        response = await api_agent_config(request)
...
async def test_api_agent_config_put_does_not_clobber_sidecar(tmp_path):
    ...
    agent_state.set_cc_model("codexcrew", "test-model-stub")
    ...
    assert agent_state.get_cc_model("codexcrew") == "test-model-stub"
...
async def test_api_agent_config_put_uses_atomic_write(tmp_path):
    ...
    with (
        ...
        patch(
            "codex_crew.dashboard.handlers.agents.write_config_atomically",
...
async def test_agent_config_write_holds_the_mcp_transaction_lock(tmp_path):
    ...
    # transaction lock AND by bridges' codexcrew.lock file lock, and the file
    ...
    with (
        ...
        patch(
            "codex_crew.dashboard.handlers.mcp._get_mcp_lock",
            _recording_lock,
        ),
        patch(
            "codex_crew.dashboard.handlers.agents._agent_file_lock",
    ...
    # own flock (codexcrew.lock), which the transaction lock does not cover: a
    ...
    for _path, _held, locked in spec_writes:
        assert locked, "the agent-spec write ran WITHOUT bridges' codexcrew.json file lock"
...
async def test_agent_config_write_goes_through_one_shielded_offload(tmp_path):
    ...
    with (
        ...
        patch("codex_crew.dashboard.handlers._reset_all_sessions", new_callable=AsyncMock),
        patch("codex_crew.dashboard.handlers.config_path", return_value=mc_cfg),
        patch("codex_crew.agent.build_agent_config", return_value={"toolsSettings": {}}),
        patch(
            "codex_crew.dashboard.handlers.agents.get_shipped_tools",
            return_value={"tools": ["a", "c"], "allowedTools": ["b"]},
        ),
        patch("codex_crew.agent_state.lift_and_strip_bookkeeping", _recording_lift),
        patch(
            "codex_crew.dashboard.handlers.agents.write_config_atomically",
            _recording_write,
        ),
        patch(
            "codex_crew.dashboard.handlers.agents.update_config_locked",
            _recording_locked,
        ),
        patch("codex_crew.dashboard.handlers.mcp._offload_config_write", _recording_offload),
    ):
        resp = await api_agent_config(request)
...
async def test_agent_config_put_persists_nothing_before_the_transaction_lock(tmp_path):
    ...
    """
    installed = tmp_path / "codexcrew.json"
    installed.write_text(json.dumps({"name": "codexcrew", "tools": ["a", "c"]}))
    defaults = tmp_path / "defaults.json"
    mc_cfg = tmp_path / "config.json"
    # A baseline that must survive an aborted PUT byte-for-byte.
    mc_cfg.write_text(json.dumps({"default_agent": "codexcrew"}))
...
async def test_agent_config_put_lock_releases_only_after_every_write_landed(tmp_path):
    ...
    with (
        ...
        patch("codex_crew.agent_state.lift_and_strip_bookkeeping", _blocking_bookkeeping),
        patch("codex_crew.dashboard.handlers.mcp._get_mcp_lock", _recording_lock),
        patch(
            "codex_crew.dashboard.handlers.agents.write_config_atomically",
            _recording_write,
        ),
    ...
    # STRIPS Codex Crew keys out of the very dict the spec write persists, so it
...
async def test_agent_config_put_unreadable_config_persists_nothing(tmp_path, monkeypatch):
    ...
    """
    from codex_crew.config.loader import ConfigReadError
    ...
    # sidecar off the developer's real ~/.codex-crew.
    state.write_text(json.dumps({"test": {"model_managed": False}}, indent=2, sort_keys=True) + "\n")
    monkeypatch.setattr("codex_crew.agent_state._state_path", lambda: state)
    ...
    with (
        ...
        patch("codex_crew.dashboard.handlers.agents.update_config_locked", _unreadable),
    ):
        response = await api_agent_config(request)
...
async def test_agent_config_put_keeps_a_concurrent_config_write_that_lands_before_the_worker(
    ...
    with (
        ...
        patch("codex_crew.dashboard.handlers.mcp._get_mcp_lock", _neutral_lock),
        patch(
            "codex_crew.dashboard.handlers.mcp._offload_config_write",
...
async def test_offload_config_write_drains_through_repeated_cancellation():
    ...
    from codex_crew.dashboard.handlers.mcp import _offload_config_write
...
async def test_default_agent_write_holds_the_config_lock(tmp_path):
    ...
    from codex_crew.dashboard.handlers import api_default_agent

    mc_cfg = tmp_path / "config.json"
    mc_cfg.write_text(json.dumps({"default_agent": "codexcrew", "unrelated": "keep me"}))
    ...
    loaded = MagicMock()
    loaded.agents = {"codexcrew": MagicMock()}
    fake_config_cls = MagicMock()
    fake_config_cls.load.return_value = loaded

    with (
        patch("codex_crew.dashboard.handlers.config_path", return_value=mc_cfg),
        patch("codex_crew.dashboard.handlers.agents.CodexCrewConfig", fake_config_cls),
        patch("codex_crew.dashboard.handlers.agents._get_config_lock", _recording_lock),
        patch("codex_crew.dashboard.handlers.agents.update_config_locked", _recording_locked),
    ):
...
async def test_agent_config_put_governs_against_the_ceiling_current_at_commit(
    ...
    with (
        ...
        patch("codex_crew.dashboard.handlers.mcp._get_mcp_lock", _revoking_lock),
    ):
        resp = await api_agent_config(request)
