...
- ``_resolve_skill_root`` cross-source resolution (codexcrew / kiro-user / aim)
...
def fake_home(tmp_path, monkeypatch):
    ...
    Also clears CODEXCREW_HOME so ``skills_dir()`` resolves to
    ``<tmp>/.codex-crew/skills`` rather than any value leaked from the
...
class TestAnnotateSkillsWithAgents:
    ...
    def test_parses_agents_once_regardless_of_skill_count(self, fake_home, monkeypatch):
        ...
        import codex_crew.dashboard.handlers._shared as shared
...
class TestCollectSkillsBlocking:
    ...
    def test_merges_all_sources_and_annotates(self, fake_home):
        ...
        # codexcrew source defaulted, package rows merged, kiro discovered.
        assert by_key["mc"]["source"] == "codexcrew"
...
class TestResolveSkillRoot:
    def test_codexcrew_skill(self, fake_home):
    ...
    def test_nested_codexcrew_skill_resolves(self, fake_home):
    ...
    def test_codexcrew_skill_honors_codexcrew_home(self, tmp_path, monkeypatch):
        """``_resolve_skill_root`` must resolve codexcrew skills under the
        active config home (``skills_dir()``), not a hardcoded
        ``~/.codex-crew``.  An isolated dev gateway sets CODEXCREW_HOME to a
        ...
        # Isolated config home elsewhere, selected via CODEXCREW_HOME.
        mc_home = tmp_path / "dev-home"
        monkeypatch.setenv("CODEXCREW_HOME", str(mc_home))
        skill_dir = _write_skill(mc_home / "skills", "isolated-skill")

        state = MagicMock(_slots={})
        out = _resolve_skill_root("isolated-skill", state)
        assert out == skill_dir.resolve()
        # And nothing was created under the real ~/.codex-crew.
...
def _make_app(state):
    from codex_crew.dashboard.handlers import api_skill_file, api_skill_tree, api_skills
...
class TestEndpoints:
    ...
    async def test_tree_endpoint_returns_entries(self, fake_home):
        ...
        # SkillsLoader will use ~/.codex-crew/skills (empty here) — fine.
    ...
    async def test_endpoints_emit_sel_audit_events(self, fake_home, monkeypatch):
        ...
        _write_skill(fake_home / ".kiro" / "skills", "demo")
        sel_mock = MagicMock()
        monkeypatch.setattr("codex_crew.dashboard.handlers.sel", lambda: sel_mock)
    ...
    async def test_skill_named_tree_hits_detail_not_browser(self, fake_home):
        ...
        # A real skill literally named ``utils/tree`` under the codexcrew root.
...
class TestSessionScopedSkillResolution:
    ...
    async def test_app_skill_catalog_requires_positive_slot_ownership(
        ...
        """
        from codex_crew.dashboard.handlers import prompts
    ...
    async def test_app_projectless_slot_cannot_fall_back_to_foreign_project(
        ...
        from codex_crew.dashboard.handlers import api_skill_detail, prompts
        from codex_crew.skills import SkillsLoader
...
class TestPackageSkillDetailReadsThroughTheGate:
    ...
    async def test_a_hardlinked_package_skill_is_not_served(self, tmp_path, monkeypatch):
        from codex_crew.dashboard.handlers import api_skill_detail
    ...
    async def test_a_refused_package_skill_leaves_an_audit_line(self, tmp_path, monkeypatch):
        ...
        """
        from codex_crew.dashboard import handlers as handlers_pkg
        from codex_crew.dashboard.handlers import api_skill_detail
