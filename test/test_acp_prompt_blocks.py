...
from codex_crew.acp import prompt_blocks
from codex_crew.acp.prompt_blocks import (
    _POSIX_PATH_RE,
    IMAGE_MEDIA_TYPES,
    MAX_IMAGE_BYTES,
    MAX_IMAGE_EDGE_PX,
...
class TestPlatformPathGrammar:
    ...
    def test_windows_pattern_extracts_dashboard_unc_image_markdown(self):
        ...
        """
        text = "![image](//fileserver/home/me/.codex-crew/uploads/shot.png)"
        m = prompt_blocks._WINDOWS_PATH_RE.search(text)
        assert m is not None
        assert m.group(1) == "//fileserver/home/me/.codex-crew/uploads/shot.png"
...
class TestUncProbeGate:
    ...
    def test_attacker_host_is_refused(self, monkeypatch, tmp_path):
        monkeypatch.setattr(
            "codex_crew.config.paths.data_home", lambda: tmp_path / "home"
        )
    ...
    def test_unc_under_a_unc_data_home_is_allowed(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.config.paths.data_home",
            lambda: Path(r"\\fileserver\home\me\.codex-crew"),
        )
        allowed = hooks.unc_probe_allowed(
            r"\\fileserver\home\me\.codex-crew\uploads\shot.png"
        )
        forward = hooks.unc_probe_allowed(
            "//fileserver/home/me/.codex-crew/uploads/shot.png"
        )
    ...
    def _patch_roots(self, monkeypatch, tmp_path, agents_dir):
        ...
        monkeypatch.setattr("codex_crew.config.paths.data_home", lambda: tmp_path / "home")
        monkeypatch.setattr("codex_crew.config.paths.kiro_agents_dir", lambda: agents_dir)
    ...
    def test_broken_agents_root_fails_safe(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.config.paths.kiro_agents_dir", boom)
        monkeypatch.setattr(
            "codex_crew.config.paths.data_home",
            lambda: Path("//fileserver/home/me/.codex-crew"),
        )
        assert hooks.unc_probe_allowed("//fileserver/home/me/.codex-crew/uploads/x.png") is True
        assert hooks.unc_probe_allowed("//evil/share/x.png") is False
    ...
    def test_agents_root_is_resolved_once_per_configuration(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("codex_crew.config.paths.data_home", lambda: tmp_path / "home")
        monkeypatch.setattr("codex_crew.config.paths.kiro_agents_dir", counting_agents_dir)
        assert hooks.unc_probe_allowed(self._UNC_KIRO_HOME + "/agents/foo.json") is True
...
class TestImageEncodedBudget:
    ...
    def test_b64_len_matches_real_encoding(self):
        from codex_crew.imaging import _b64_len
