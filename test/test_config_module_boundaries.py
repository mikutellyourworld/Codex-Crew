...
from codex_crew.config import loader, resolution, sections

_PRE_SPLIT_REEXPORT_NAMES = {
    "codex_crew.config.sections": tuple("""
ACTIVATION_ALWAYS
ACTIVATION_MENTION
ACTIVATION_OBSERVE
ACTIVATION_OFF
...
""".split()),
    "codex_crew.config.resolution": tuple("""
...
def test_extracted_modules_do_not_import_the_loader() -> None:
    ...
    code = (
        "import sys\n"
        "import codex_crew.config.sections\n"
        "import codex_crew.config.resolution\n"
        "forbidden = (\n"
        "    'codex_crew.config.loader',\n"
        "    'codex_crew.config.schema',\n"
        "    'codex_crew.config.validation',\n"
        ")\n"
