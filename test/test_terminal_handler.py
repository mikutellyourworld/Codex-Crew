...
from codex_crew.dashboard import terminal_commands
from codex_crew.dashboard.handlers import terminal
...
def _make_request(
    ...
    app = {"state": state, "allowed_origins": {"http://localhost:5486"}}
    request = MagicMock()
    request.app = app
...
class TestKillSession:
    ...
    async def test_cancels_reader_task(self):
        ...
        with patch("os.close"), patch("codex_crew.dashboard.handlers.terminal.platform_compat.kill_process_tree"):
    ...
    async def test_closes_master_fd(self):
        sess = _make_session()
        sess.master_fd = 42
        with patch("os.close") as mock_close, patch("codex_crew.dashboard.handlers.terminal.platform_compat.kill_process_tree"):
            await terminal._kill_session(sess)
    ...
    async def test_sends_sigterm_to_process_group(self):
        ...
        with patch("os.close"), \
                patch("codex_crew.dashboard.handlers.terminal.platform_compat.kill_process_tree") as mock_kill:
            await terminal._kill_session(sess)
    ...
    async def test_handles_process_lookup_error_on_sigterm(self):
        sess = _make_session(alive=True)
        with patch("os.close"), \
                patch("codex_crew.dashboard.handlers.terminal.platform_compat.kill_process_tree",
    ...
    async def test_handles_os_error_on_close(self):
        sess = _make_session()
        sess.master_fd = 42
        with patch("os.close", side_effect=OSError), patch("codex_crew.dashboard.handlers.terminal.platform_compat.kill_process_tree"):
            await terminal._kill_session(sess)
        assert sess.master_fd == -1
    ...
    async def test_close_runs_on_subprocess_pool_off_loop(self):
        ...
        with patch("os.close", side_effect=_record_close), patch(
            "codex_crew.dashboard.handlers.terminal.platform_compat.kill_process_tree"
        ):
            await terminal._kill_session(sess)
...
class TestListCompletions:
    ...
    def test_sensitive_directory_yields_nothing(self, tmp_path):
        # ~/.codex-crew/profiles is trust-root metadata: enumerating it would
...
class TestSensitiveEntryGate:
    ...
    protected children (``~/.codex-crew`` holds ``security_policy.json``,
...
class TestApiTerminalWsOrigin:
    ...
    async def test_allows_the_dashboard_origin(self):
        ...
        req = _make_request(user=None, origin="http://localhost:5486")
        with patch.object(terminal, "_sel") as mock_sel:
            mock_sel.return_value.log_api_access = MagicMock()
...
class TestApiTerminalWs:
    ...
    async def test_windows_conpty_spawn_failure_sends_error(self, monkeypatch):
        """On Windows a new WS session spawns a ConPTY shell (codex_crew.conpty);
        ...
        with patch.object(terminal.platform_compat, "IS_POSIX", False), \
             ...
             patch("codex_crew.conpty.WindowsPty", side_effect=RuntimeError("boom")), \
             patch.object(terminal, "_get_config", return_value={"enabled": True}), \
             patch.object(terminal.web, "WebSocketResponse", return_value=ws), \
             patch.object(terminal, "_sel") as mock_sel:
            mock_sel.return_value.log_api_access = MagicMock()
...
def _make_app(registry=None, cfg=None, user="testuser"):
    ...
    app["allowed_origins"] = {"http://localhost:5486"}
...
class TestTerminalWsIntegration:
    ...
    async def test_ws_windows_spawns_conpty(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("codex_crew.conpty.WindowsPty", _FakeWinPty)
    ...
    async def test_ws_bash_restores_an_inherited_prompt_command(
        ...
        # The Codex Crew names are gone from the session either way.
        assert b"CODEXCREW_TERMINAL_READY" not in tail
    ...
    async def test_rest_create_list_delete(self, monkeypatch, tmp_path):
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            from codex_crew.dashboard.handlers import terminal as _term
...
class TestBashShellReadiness:
    ...
    def test_ready_hook_is_a_single_shot_self_removing_prompt_command(
        ...
        assert "]697;CodexCrewReady;%s" in hook
    ...
    def test_ready_hook_preserves_an_inherited_prompt_command(self, monkeypatch):
        ...
        assert exported.index("CodexCrewReady") < exported.index("history -a")
...
class TestPtyChildEnvStripsPythonStartupVars:
    ...
    shell makes a user's Python 3.13 venv import Codex Crew's 3.12 site-packages
    ...
    def test_python_startup_vars_are_dropped(self, monkeypatch):
        ...
        env = terminal._pty_child_env(
            {"TERM": "xterm-256color", "CODEXCREW_TERMINAL": "1"}
        )

        assert "PYTHONPATH" not in env
        assert "PYTHONHOME" not in env
        assert "PYTHONPYCACHEPREFIX" not in env
        assert env["CODEXCREW_TERMINAL"] == "1"
    ...
    def test_credential_bearing_vars_survive(self, monkeypatch):
        ...
        env = terminal._pty_child_env({"CODEXCREW_TERMINAL": "1"})
