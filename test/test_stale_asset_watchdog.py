...
async def test_watchdog_shuts_down_when_assets_vanish():
    ...
    from codex_crew.dashboard.stale_asset_watchdog import run_stale_asset_watchdog
    ...
    with patch(
        "codex_crew.dashboard.stale_asset_watchdog.assets_present",
...
def test_assets_present_detects_dist_index(tmp_path: Path):
    ...
    from codex_crew.dashboard import stale_asset_watchdog as mod
...
def test_token_probe_warns_on_stale_dashboard():
    ...
    from codex_crew.cli_server import _probe_dashboard_health
...
def test_token_probe_silent_on_healthy_dashboard():
    ...
    healthy_body = b"<!DOCTYPE html><html><head><title>CodexCrew</title></head></html>"
    mock_resp = MagicMock()
...
def test_token_probe_silent_on_network_error():
    ...
    with patch("codex_crew.cli_server.loopback_urlopen", side_effect=OSError("connection refused")), \
         patch("sys.stderr", stderr_capture):
        _probe_dashboard_health(7777)
...
async def test_watchdog_survives_asset_gap_that_heals_while_draining(caplog):
    ...
    caplog.set_level(logging.CRITICAL, logger="codex_crew.dashboard.stale_asset_watchdog")
