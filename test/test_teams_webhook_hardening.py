...
from codex_crew.teams.client import TEAMS_ACTIVITY_REQUEST_KEY, TEAMS_MAX_ACTIVITY_BYTES
...
class _FakeRequest(dict):
    ...
    def __init__(
        ...
        *,
        ...
        self.app = app if app is not None else {"allowed_origins": {"http://localhost:5486"}}
...
class TestCsrfExemptSet:
    ...
    def test_the_constant_names_a_route_the_app_registers(
        ...
        routes = importlib.import_module(f"codex_crew.dashboard.routes.{route_module}")
        app = web.Application()
        routes.register(app)
        methods = {
            route.method
