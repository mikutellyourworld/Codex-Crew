...
class TestSendMessage:
    ...
    async def test_send_message_session_origin(self):
        ...
        app = _make_send_app(state)
        with patch(
            "codex_crew.dashboard.chat_runner._run_chat", new_callable=AsyncMock
        ) as mock_run, patch(
    ...
    async def test_send_message_session_origin_rehydrate_returns_none_falls_back(self):
        ...
        app = _make_send_app(state)
        with patch(
            "codex_crew.dashboard.handlers.messaging.rehydrate_slot_from_history_async", return_value=None
        ) as mock_rehydrate:
            async with TestClient(TestServer(app)) as client:
                resp = await client.post(
                    "/api/send-message",
    ...
    async def test_send_message_session_origin_stateless_cron(self):
        ...
        with patch(
            "codex_crew.dashboard.chat_runner._run_chat", new_callable=AsyncMock
        ) as mock_run, patch("codex_crew.dashboard.handlers.messaging.rehydrate_slot_from_history_async"):
            async with TestClient(TestServer(app)) as client:
                resp = await client.post(
                    "/api/send-message",
                    json={
