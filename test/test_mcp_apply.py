...
from codex_crew.platform.interfaces import McpScope
...
# ---------------------------------------------------------------------------
# Scope helpers: _set_codexcrew_entry, _set_scope_entry, _remove_codexcrew_entry
# ---------------------------------------------------------------------------
...
class TestSetKirocrewEntry:
    def test_adds_entry_when_enabling_with_spec(self, tmp_path, monkeypatch):
        ...
        mc_path = tmp_path / "codexcrew.mcp.json"
        monkeypatch.setattr(mcp_mod, "_CODEXCREW_MCP_JSON", mc_path)
        action = mcp_mod._set_codexcrew_entry("srv", enabled=True, spec={"command": "x"})
        assert action == "added"
    ...
    def test_disables_existing(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(mcp_mod, "_CODEXCREW_MCP_JSON", mc_path)
        action = mcp_mod._set_codexcrew_entry("srv", enabled=False)
    ...
    def test_enabling_disabled_removes_flag(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(mcp_mod, "_CODEXCREW_MCP_JSON", mc_path)
        action = mcp_mod._set_codexcrew_entry("srv", enabled=True)
    ...
    def test_disabling_missing_with_spec_seeds_entry(self, tmp_path, monkeypatch):
        ...
        mc_path = tmp_path / "codexcrew.mcp.json"
        monkeypatch.setattr(mcp_mod, "_CODEXCREW_MCP_JSON", mc_path)
        action = mcp_mod._set_codexcrew_entry("srv", enabled=False, spec={"command": "x"})
        assert action == "disabled"
...
class TestApplyEndpoint:
    @pytest.mark.asyncio
    async def test_preservation_kiro_to_codexcrew(self, tmp_path, monkeypatch):
        """Turning Kiro off when server was only in Kiro copies to CodexCrew first."""
        from codex_crew.dashboard.handlers import mcp as mcp_mod
        ...
        agent_path = tmp_path / "codexcrew_agent.json"
        ...
        monkeypatch.setattr("codex_crew.dashboard.handlers.mcp.rebuild_agent_config", lambda: None)
        ...
        # CodexCrew mcp.json should now have slack-mcp (preservation happened)
    ...
    async def test_uninstall_removes_from_all_three(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.handlers._shared._capability_manager",
    ...
    async def test_calls_rebuild_agent_config_once(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(mcp_mod, "_CODEXCREW_MCP_JSON", tmp_path / "mc.json")
        ...
        rebuild = MagicMock()
        monkeypatch.setattr("codex_crew.dashboard.handlers.mcp.rebuild_agent_config", rebuild)
        ...
        request = _make_request(
            {
                "changes": [
                    {"name": "a", "codexcrew": True, "kiroGlobal": True, "ccGlobal": False},
                    {"name": "b", "codexcrew": True, "kiroGlobal": False, "ccGlobal": True},
...
class TestHostileNameRejection:
    ...
    async def test_rejects_hostile_names(self, tmp_path, monkeypatch, bad_name):
        ...
        request = _make_request({"changes": [{"name": bad_name, "codexcrew": True}]})
        resp = await mcp_mod.api_mcp_apply(request)
        body = json.loads(resp.body)
...
class TestMcpNameValidator:
    ...
    def test_accepts_well_formed_names(self, name):
        from codex_crew.dashboard.handlers.mcp import _is_valid_mcp_name
...
class TestApplyAcceptsAppProvidedName:
    ...
    async def test_colon_name_passes_the_gate_and_applies(self, tmp_path, monkeypatch):
        ...
        request = _make_request(
            {"changes": [{"name": "auto-improvement:auto-improvement", "codexcrew": True}]}
        )
        resp = await mcp_mod.api_mcp_apply(request)
        body = json.loads(resp.body)
        ...
        assert "codexcrew" in result["actions"]
...
class TestSetStubNameGate:
    ...
    async def test_accepts_app_provided_colon_name(self, tmp_path, monkeypatch):
        ...
        import codex_crew.config.loader as loader_mod
        from codex_crew.dashboard.handlers import mcp as mcp_mod
        ...
        from codex_crew.config.loader import _resolve_stub_servers
...
class TestApplyBatchCap:
    ...
    async def test_accepts_batch_at_cap(self, monkeypatch, tmp_path) -> None:
        ...
        monkeypatch.setattr(mcp_mod, "_CODEXCREW_MCP_JSON", tmp_path / "kc.json")
        monkeypatch.setattr(mcp_mod, "_GLOBAL_MCP_JSON", tmp_path / "kiro.json")
        monkeypatch.setattr(mcp_mod, "_extra_mcp_scopes", lambda: [])
        monkeypatch.setattr("codex_crew.dashboard.handlers.mcp.rebuild_agent_config", lambda: None)
...
class TestBoundedCapabilityManager:
    ...
    async def test_mutation_op_is_time_bounded(self, monkeypatch) -> None:
        ...
        from codex_crew.platform import capability_bound
    ...
    async def test_apply_uninstall_calls_manager_off_lock(self, monkeypatch, tmp_path) -> None:
        ...
        from codex_crew.dashboard.handlers import _shared
        from codex_crew.dashboard.handlers import mcp as mcp_mod
...
class TestUninstallCrashWindowCleanup:
    ...
    async def test_config_purged_even_when_loop_aborts(self, monkeypatch, tmp_path) -> None:
        ...
        # the uninstall change for 'gone' is processed. _set_codexcrew_entry is
        # the first mutation the toggle path calls.
        def _boom(*a, **k):
            raise OSError("disk full")

        monkeypatch.setattr(mcp_mod, "_set_codexcrew_entry", _boom)

        request = _make_request(
            {
                "changes": [
                    {"name": "a", "codexcrew": True},  # aborts the loop
    ...
    async def test_sweep_runs_off_event_loop_thread(self, monkeypatch, tmp_path) -> None:
        ...
        request = _make_request(
            {"changes": [{"name": "a", "codexcrew": True}, {"name": "gone", "uninstall": True}]}
        )
        with pytest.raises(OSError):
            await mcp_mod.api_mcp_apply(request)
