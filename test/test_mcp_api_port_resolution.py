...
and the dashboard server exports ``CODEXCREW_BOUND_PORT`` once its TCP site is
listening.
"""
...
import codex_crew
import codex_crew.mcp_core as mcp_core
from codex_crew.dashboard.server import _export_bound_port

#: Source root of the tree under test, pinned onto the probe subprocess's
#: PYTHONPATH. Without it the child interpreter resolves whatever codex_crew
...
_SRC = str(Path(codex_crew.__file__).resolve().parents[1])
...
def _fresh_caches(monkeypatch: pytest.MonkeyPatch):
    ...
    must start unresolved. ``CODEXCREW_PORT`` is deleted because a dev box (or
...
def _cfg(url: object):
    ...
    return patch("codex_crew.cli_server.CodexCrewConfig.load", return_value=cfg)
...
def _owned(ports: list[int]):
    """Pretend a verified Codex Crew gateway listens on each of *ports*."""
    return patch("codex_crew.cli_server._gateway_owns_port", side_effect=lambda p: p in set(ports))
...
class TestApiBaseResolution:
    ...
    def test_dead_pid_marker_is_ignored(self, monkeypatch: pytest.MonkeyPatch):
        ...
        """
        monkeypatch.setattr("codex_crew.cli_server.run_marker.read_pid", lambda port: 999999999)
        # A dead pid holds no sockets — the listener set for the port is empty.
        monkeypatch.setattr(
            "codex_crew.cli_server.platform_compat.find_listening_pids", lambda port: []
        )
        with _cfg(""), _markers([6776]):
            assert mcp_core._api_base() == "http://127.0.0.1:5486"

    def test_env_var_beats_marker(self, monkeypatch: pytest.MonkeyPatch):
        """The exported ``CODEXCREW_PORT`` (gateway truth) sits above discovery."""
    ...
    def test_unix_socket_not_pinned_on_default_fallthrough(self):
        ...
        with _cfg(""), _markers([]):
            assert mcp_core._api_unix_socket().endswith("dashboard-5486.sock")
    ...
    def test_bound_port_beats_marker_and_config_default(
        ...
        monkeypatch.setenv("CODEXCREW_BOUND_PORT", "7891")
        with _cfg("http://my.host.example"), _markers([6776]), _owned([6776]):
            assert mcp_core._api_base() == "http://127.0.0.1:7891"

    def test_operator_port_beats_bound_port(self, monkeypatch: pytest.MonkeyPatch):
        """An explicit CODEXCREW_PORT is how a caller retargets a child at a
        DIFFERENT gateway: pod exec builds a client env with
        CODEXCREW_PORT=<pod-port> while the inherited CODEXCREW_BOUND_PORT still
        ...
        monkeypatch.setenv("CODEXCREW_PORT", "7891")
        monkeypatch.setenv("CODEXCREW_BOUND_PORT", "5486")
        with _cfg(""), _markers([]):
            assert mcp_core._api_base() == "http://127.0.0.1:7891"
...
class TestExportBoundPort:
    ...
    def test_explicit_port_is_exported(self, monkeypatch: pytest.MonkeyPatch):
        ...
        assert os.environ["CODEXCREW_BOUND_PORT"] == "6776"
        monkeypatch.delenv("CODEXCREW_BOUND_PORT", raising=False)
    ...
    def test_ephemeral_bind_reads_port_back_from_runner(self, monkeypatch: pytest.MonkeyPatch):
        ...
        assert os.environ["CODEXCREW_BOUND_PORT"] == "54321"
        monkeypatch.delenv("CODEXCREW_BOUND_PORT", raising=False)
    ...
    def test_no_readable_address_leaves_env_untouched(self, monkeypatch: pytest.MonkeyPatch):
        ...
        assert "CODEXCREW_BOUND_PORT" not in os.environ
    ...
    def test_export_overwrites_stale_env(self, monkeypatch: pytest.MonkeyPatch):
        ...
        monkeypatch.setenv("CODEXCREW_BOUND_PORT", "1111")
...
class TestPortResolutionStaysLeaf:
    ...
    @pytest.mark.parametrize(
        "module", ["codex_crew.port_resolution", "codex_crew.mcp_core"]
    )
    def test_import_does_not_pull_cli_server(self, module: str) -> None:
        code = (
            "import importlib, sys; "
            f"importlib.import_module({module!r}); "
            "assert 'codex_crew.cli_server' not in sys.modules, "
