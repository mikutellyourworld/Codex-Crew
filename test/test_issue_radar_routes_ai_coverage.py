...
touched, and nothing is written outside the per-test ``CODEXCREW_HOME`` that
...
from codex_crew.apps.builtins.issue_radar.backend import provider, routes, store
...
class TestRunOneshotModel(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_returns_the_models_text_and_tears_the_session_down(self):
        ...
        state.sessions.get_or_create.assert_awaited_once_with("k1", agent="codexcrew-lite")
