...
the recogniser is stubbed at the ``codex_crew.stt`` seam, because the point of the
...
from codex_crew import stt, transcribe
from codex_crew.config.loader import SttConfig
...
def _stub_recogniser_absent(monkeypatch) -> None:
    ...
    """
    from codex_crew.stt import engine as engine_mod
...
class TestFindBrew:
    ...
    def test_found_on_path(self):
        with patch("codex_crew.transcribe.shutil.which", return_value="/opt/homebrew/bin/brew"):
    ...
    def test_found_off_path_via_prefix(self, tmp_path, monkeypatch):
        ...
        brew.chmod(0o755)
        with patch("codex_crew.transcribe.shutil.which", return_value=None):
            monkeypatch.setattr("codex_crew.transcribe._BREW_CANDIDATE_PATHS", [str(brew)])
            assert find_brew() == str(brew)

    def test_not_installed(self, monkeypatch):
        with patch("codex_crew.transcribe.shutil.which", return_value=None):
            monkeypatch.setattr("codex_crew.transcribe._BREW_CANDIDATE_PATHS", ["/nonexistent/brew"])
...
class TestIsAvailable:
    ...
    def test_loads_config_when_none(self):
        ...
        with patch("codex_crew.config.loader.CodexCrewConfig.load", return_value=mock_cfg):
...
class TestAvailabilityDetail:
    ...
    def test_local_missing_extra(self, monkeypatch):
        ...
        # The recogniser distribution, not the extra name: `codexcrew[voice]` is
        # not installable from any index.
        assert "pywhispercpp" in detail.detail
        assert "codexcrew[" not in detail.detail
    ...
    def test_local_ok_ignores_a_model_that_is_not_downloaded(self, monkeypatch):
        ...
        """
        from codex_crew.stt import models
...
class TestTranscribeAudio:
    ...
    def _recogniser(monkeypatch, transcript="Hello world", available=True, seen=None):
        """Stand the recogniser up at the ``codex_crew.stt`` seam.
    ...
    async def test_aws_audio_read_runs_off_event_loop(self, tmp_path, monkeypatch):
        ...
        from codex_crew import transcribe as tr
    ...
    async def test_transcode_targets_the_recogniser_format(self, tmp_path, monkeypatch):
        ...
        with (
            patch(
                "codex_crew.transcribe._open_ffmpeg_for_execution",
    ...
    async def test_missing_ffmpeg_for_a_compressed_memo_returns_none(self, tmp_path, monkeypatch):
        ...
        with (
            patch("codex_crew.transcribe._open_ffmpeg_for_execution", return_value=None),
...
class TestTranscribeFiles:
    ...
    async def test_transcribe_audio_files(self):
        from codex_crew.slack.events import _transcribe_files
        ...
        with patch(
            "codex_crew.slack.events.transcribe_audio", new_callable=AsyncMock, return_value="Hello"
        ):
    ...
    async def test_handles_transcription_failure(self):
        ...
        # does `from codex_crew.transcribe import transcribe_audio`, so it holds its own
        ...
        with patch(
            "codex_crew.slack.events.transcribe_audio", new_callable=AsyncMock, return_value=None
        ):
            result = await _transcribe_files(mock_orch, files)
        assert result == []
...
class TestSensitivePathGuard:
    ...
    async def test_sensitive_path_blocked_before_any_local_decode(self, tmp_path, monkeypatch):
        ...
        with patch("codex_crew.security.is_sensitive_path", return_value=True):
            result = await transcribe_audio(str(audio), cfg)
        assert result is None
...
class TestBundledFfmpeg:
    ...
    def test_linux_memfd_explicitly_requests_executable_mode(self, monkeypatch):
        ...
        assert len(calls) == 1
        assert calls[0][0] == "codexcrew-ffmpeg"
    ...
    def test_macos_snapshot_is_staged_under_agent_denied_root(self, monkeypatch, tmp_path):
        ...
        opened = transcribe._AuthenticatedFfmpeg("source", reader, path, cleanup_path=path)
        try:
            assert calls == [(f".codexcrew-ffmpeg-{os.getpid()}-", str(root))]
        finally:
            opened.close()
    ...
    def test_snapshot_root_is_private_and_pruned_once(self, monkeypatch, tmp_path):
        ...
        cleanup_calls = []
        monkeypatch.setattr(
            "codex_crew.sandbox.prime_voice_runtime_sandbox_paths",
            lambda: str(root),
        )
    ...
    def test_stale_macos_snapshots_are_pruned_without_following_unknown_entries(
        ...
        stale = tmp_path / ".codexcrew-ffmpeg-123-dead"
        stale.mkdir()
        (stale / "ffmpeg").write_bytes(b"stale")
        active = tmp_path / f".codexcrew-ffmpeg-{os.getpid()}-active"
        active.mkdir()
        (active / "ffmpeg").write_bytes(b"active")
        unknown = tmp_path / ".codexcrew-ffmpeg-not-a-pid"
        unknown.mkdir()
        monkeypatch.setattr(
...
class TestTranscribeFormatValidation:
    ...
    async def test_rejects_unsupported_format(self, tmp_path):
        ...
        audio.write_text("fake")
        cfg = SttConfig(enabled=True, provider="transcribe")
        with patch("codex_crew.security.is_sensitive_path", return_value=False):
            result = await transcribe_audio(str(audio), cfg)
        assert result is None
