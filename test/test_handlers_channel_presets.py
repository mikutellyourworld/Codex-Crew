...
from codex_crew.dashboard.handlers_channel import _DEFAULT_PRESETS, api_channel_presets
...
class TestChannelPresets:
    ...
    async def test_returns_defaults_when_config_missing(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.handlers_channel.config_path",
    ...
    async def test_returns_defaults_when_config_has_no_presets_key(
        ...
        monkeypatch.setattr("codex_crew.dashboard.handlers_channel.config_path", lambda: cfg)
        request = MagicMock()
        resp = await api_channel_presets(request)
        body = json.loads(resp.body)
        assert body["presets"] == _DEFAULT_PRESETS
