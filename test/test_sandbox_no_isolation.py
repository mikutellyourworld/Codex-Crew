...
def _neutralize_passthrough(monkeypatch):
    ...
    monkeypatch.setattr(sb, "_inside_codexcrew_sandbox", lambda: False)
...
def test_windows_native_tool_gate_can_own_codex_security(monkeypatch):
    """A verified native gate permits Codex on Windows without a Crew backend."""
    _reset_warned()
    _neutralize_passthrough(monkeypatch)
    ...
    argv, cleanup = sb.wrap_argv(["codex-acp"], mode="standard", native_tool_gate=True)

    assert argv == ["codex-acp"]
    assert cleanup is None


def test_windows_native_tool_gate_does_not_override_governance_floor(monkeypatch):
    """A native tool hook is not an OS sandbox required by policy."""
    _reset_warned()
    _neutralize_passthrough(monkeypatch)
    monkeypatch.setattr(sys, "platform", "win32")
    monkeypatch.setattr(sb, "detect_backend", lambda config_mode="auto": "none")
    monkeypatch.setattr(sb, "_allow_unsandboxed_exec", lambda: False)
    monkeypatch.setattr(sb, "_governance_sandbox_floor", lambda: "standard")

    with pytest.raises(sb.SandboxUnavailableError):
        sb.wrap_argv(["codex-acp"], mode="standard", native_tool_gate=True)
...
def test_strip_python_env_covers_pycache_prefix(monkeypatch):
    ...
    """
    monkeypatch.setenv("PYTHONPYCACHEPREFIX", "/home/x/.codex-crew/cache/pycache")
...
def test_strip_python_env_holds_on_fail_open_path(monkeypatch):
    ...
    base = {"PATH": "/usr/bin", "PYTHONPATH": "/codexcrew/site", "PYTHONHOME": "/py"}
...
def test_strip_python_env_false_keeps_python_env(monkeypatch):
    ...
    sandboxed Python children import codex_crew via it)."""
    _reset_warned()
    _neutralize_passthrough(monkeypatch)
    monkeypatch.setattr(sb, "detect_backend", lambda config_mode="auto": "none")
    monkeypatch.setattr(sb, "_allow_no_isolation", lambda: True)
    ...
    base = {"PATH": "/usr/bin", "PYTHONPATH": "/codexcrew/site"}
    _, env, _ = sb.sandboxed_spawn_argv(["python", "-m", "x"], env=base)
    assert env["PYTHONPATH"] == "/codexcrew/site"
