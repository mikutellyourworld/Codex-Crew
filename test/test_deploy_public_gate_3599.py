...
def test_pending_confirm_denied_without_consuming_the_entry(monkeypatch):
    ...
    from codex_crew.deploy import pending as pending_mod
...
def narrowed_allowlist(tmp_path, monkeypatch):
    ...
    ``CodexCrewConfig.load()``. The gate now reads the FILE, and a mock here would
...
def test_the_config_allowlist_denial_is_audited(narrowed_allowlist, monkeypatch):
    ...
    """
    import codex_crew.publish_governance as pg
...
def test_a_malformed_config_denies_instead_of_reopening_the_path(
    ...
    ``CodexCrewConfig.load()`` catches ``JSONDecodeError``/``OSError``, warns, and
...
def test_a_config_read_failure_is_audited_too(tmp_path, monkeypatch, _quiet_sel):
    ...
    monkeypatch.setattr(pg, "CodexCrewConfig", _Raising)
...
#
# Written as real bytes to a real config.json. Mocking CodexCrewConfig.load()
...
def test_a_scalar_allowed_destinations_never_crashes_load(tmp_path, monkeypatch, _quiet_sel):
    ...
    cfg = CodexCrewConfig.load()  # must not raise
