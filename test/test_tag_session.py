...
def patch_save_slot(tmp_path):
    ...
    with patch(
        "codex_crew.dashboard.chat_auto_tag.save_slot_off_loop",
        mock,
    ):
        ...
        with patch("codex_crew.dashboard.chat_tags.asyncio.to_thread", side_effect=_sync_to_thread):
            with patch("codex_crew.dashboard.state.config_dir", return_value=tmp_path):
                yield mock
...
class TestAutoTagDerivation:
    ...
    @pytest.mark.parametrize("name", ["workspace", "workspaces", "workplace", "codexcrew-workspace", "default"])
    async def test_trivial_workspace_basenames_are_noop(self, patch_save_slot, name):
        ...
        state = _make_state()
        slot = _make_slot(project=f"/home/user/.codex-crew/{name}")
...
class TestAutoTagFlagSetBeforeSave:
    ...
    async def test_flag_is_set_when_slot_is_saved(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_auto_tag.save_slot_off_loop", _recording_save)
...
class TestAutoTagDeleteRace:
    ...
    async def test_no_dangling_reference(self, tmp_path, monkeypatch):
        ...
        for _ in range(iterations):
            ...
            with patch(
                "codex_crew.dashboard.chat_tags.asyncio.to_thread", side_effect=_sync_to_thread
            ):
                with patch(
                    "codex_crew.dashboard.chat_tags.save_slot_off_loop", new_callable=AsyncMock
                ):
                    with patch(
...
class TestAutoTagTrigger:
    ...
    async def test_import_succeeds(self):
        ...
        from codex_crew.dashboard.chat_handlers import maybe_auto_tag as imported
...
class TestAutoTagPersistFailureRollback:
    ...
    async def test_snapshot_failure_rolls_back_definition(self, patch_save_slot, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_auto_tag.persist_tags_snapshot_unlocked",
            _failing_persist,
        )
        await maybe_auto_tag(state, slot)
...
class TestAutoTagTruncationBeforeMatch:
    ...
    async def test_long_basenames_reuse_one_definition(self, patch_save_slot):
        from codex_crew.dashboard.chat_auto_tag import _NAME_MAX
