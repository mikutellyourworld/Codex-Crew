...
class TestAcpPerAgentModel:
    ...
    @staticmethod
    def _acp_cfg():
        # CodexCrew is KiroACP-only, so the factory is always acp; a plain
    ...
    def test_custom_agent_threads_its_declared_model(self):
        cfg = self._acp_cfg()
        with patch.object(
            CodexCrewConfig, "_resolve_named_agent_model", return_value="gpt-5.6-sol"
        ):
    ...
    def test_unresolved_agent_model_falls_back_to_the_global_default(self):
        ...
        with patch.object(CodexCrewConfig, "_resolve_named_agent_model", return_value=""):
            provider = cfg.create_provider_factory()(
    ...
    def test_unresolved_agent_and_unset_global_leaves_the_backend_to_pick(self):
        ...
        # ~/.kiro/agents/codexcrew.json, which pins a model on a dev machine.
        cfg = self._acp_cfg()
        cfg.agent.model = DEFAULT_MODEL
        with (
            patch.object(CodexCrewConfig, "_resolve_named_agent_model", return_value=""),
            patch.object(CodexCrewConfig, "_resolve_agent_model", return_value=DEFAULT_MODEL),
        ):
            provider = cfg.create_provider_factory()(
                session_key="cron:job:run", agent="no-model-agent"
            )
    ...
    def test_kiro_agent_ignores_cc_model_sidecar(self, tmp_path):
        ...
        assert CodexCrewConfig._resolve_named_agent_model(
