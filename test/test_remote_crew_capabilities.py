...
def _all_ok(**overrides):
    ...
    replies = {
        "/api/version": (True, {"version": codex_crew.__version__}),
...
class TestVersionGate:
    async def test_an_equal_version_reports_a_match(self, monkeypatch):
        ...
        assert data["version"] == codex_crew.__version__
        assert data["local_version"] == codex_crew.__version__
...
class TestPeerCapabilityCarrier:
    ...
    def test_the_path_set_holds_exactly_the_five_reads(self):
        from codex_crew.instances.ssh_tunnel_manager import _PEER_CAPABILITY_PATHS
