...
from codex_crew.dashboard.chat_runner import _start_next_queued_turn
from codex_crew.dashboard.chat_utils import SUBAGENT_COMPLETION_KIND, SYNTHETIC_RECOVERY_KIND
...
def _key(content: str) -> str:
    ...
    from codex_crew.dashboard.state import _delivery_key
...
def agent_root(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.subagent_persistence._SUBAGENTS_DIR", tmp_path)
    return tmp_path
...
class TestConsumptionSignalIsPerTurn:
    ...
    def test_run_chat_reports_consumption_through_the_callers_hook(self):
        ...
        from codex_crew.dashboard import chat_runner as mod
    ...
    async def test_a_successor_turn_cannot_unsettle_its_predecessor(self, agent_root, tmp_path):
        ...
        with patch("codex_crew.dashboard.chat_runner.spawn_guarded_turn", _spawn):
...
class TestDrainSettlesDelivery:
    ...
    async def test_nothing_is_written_until_the_turn_completes(self, agent_root, tmp_path):
        ...
        with patch(
            "codex_crew.dashboard.chat_runner.spawn_guarded_turn",
...
class TestReaperDoesNotPruneAQueuedPromise:
    ...
    async def test_result_survives_a_queue_wait_longer_than_the_ttl(self, agent_root):
        ...
        with patch("codex_crew.subagent_persistence.time.time", return_value=time.time() + 4 * ttl):
        ...
        with patch("codex_crew.subagent_persistence.time.time", return_value=time.time() + 2 * ttl):
    ...
    async def test_gateway_queue_route_leaves_the_folder_readable(self, agent_root):
        ...
        _finished_run(info.id, agent_root)
        try:
            with patch("codex_crew.slack.gateway.INJECTION_TIMEOUT", 0.01):
                await on_done(info)
        finally:
            blocker.cancel()
