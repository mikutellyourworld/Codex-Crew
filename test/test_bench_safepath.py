...
import codex_crew.eval.bench.safepath as safepath
from codex_crew.eval.bench import datasets
from codex_crew.eval.bench.corpus import (
    CAT_SINGLE_HOP,
    BenchInstance,
    BenchQuery,
    BenchSession,
...
from codex_crew.eval.bench.ingest import IngestConfig
from codex_crew.eval.bench.retrieval import RetrievalConfig
from codex_crew.eval.bench.run import RunResult, run_retrieval, write_report
from codex_crew.eval.bench.safepath import (
    UnsafePathError,
    guard_output_dir,
    guard_read_path,
    guard_write_path,
...
def _result() -> RunResult:
    return RunResult(
        ...
        metrics=__import__(
            "codex_crew.eval.bench.retrieval", fromlist=["RetrievalAggregate"]
        ).RetrievalAggregate(),
    )
...
def test_the_corpus_cache_root_is_gated_too(
    ...
    """CODEXCREW_BENCH_CACHE can point anywhere, so it needs the same gate as argv.
    ...
    """
    monkeypatch.setenv("CODEXCREW_BENCH_CACHE", str(fake_home / ".aws"))
    with pytest.raises(UnsafePathError):
...
def test_the_file_adapters_gate_their_caller_supplied_path(fake_home: Path) -> None:
    from codex_crew.eval.bench.adapters import load_locomo_file, load_longmemeval_file
