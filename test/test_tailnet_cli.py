"""``codexcrew tailnet`` — the command that makes exposing the dashboard one step.
...
from codex_crew.dashboard.tailnet_serve import ServeResult, ServeState
...
def cfg_file(tmp_path, monkeypatch):
    ...
    Isolation is by ``CODEXCREW_HOME`` rather than by patching ``config_path`` in
...
def _stub_serve(
    monkeypatch, *, publish=None, unpublish=None, state=None, name="d.t.ts.net", marker=5486
):
...
class TestPortResolution:
    ...
    A gateway started with `--port` (or `CODEXCREW_PORT`) is not described by
    ...
    def test_an_explicit_port_outranks_everything(self, cfg_file, monkeypatch) -> None:
        ...
        monkeypatch.setenv("CODEXCREW_PORT", "9001")
    ...
    def test_the_run_marker_outranks_the_configured_url(self, cfg_file, monkeypatch) -> None:
        ...
        # config says 5486; a verified gateway is actually listening on 7788.
...
class TestCorruptConfigIsNeverReplaced:
    ...
    `CodexCrewConfig.load()` swallows a bad file and returns DEFAULTS, and the writer
...
class TestACorruptConfigStopsEveryAction:
    """Validation runs before anything calls ``CodexCrewConfig.load()``.
    ...
    def test_every_action_refuses_rather_than_rewriting(self, cfg_file, monkeypatch, action):
        ...
        `CodexCrewConfig.load()` migration-writes a parseable-but-wrongly-typed config,
    ...
    def test_the_key_path_is_real(self) -> None:
        ...
        assert isinstance(CodexCrewConfig().dashboard.tailscale.enabled, bool)
...
class TestEverySectionIsGuardedNotJustOurs:
    ...
    ``CodexCrewConfig.load()`` performs a migration write-back, so merely *loading* a
    ...
    def test_the_guarded_set_is_derived_from_the_model_not_a_hardcoded_list(self) -> None:
        ...
        from codex_crew.cli_commands import _container_valued_sections
        from codex_crew.config.loader import CodexCrewConfig

        guarded = set(_container_valued_sections())
        expected = {
            f.name
            for f in dataclasses.fields(CodexCrewConfig)
...
class TestUpNeverWritesConfig:
    ...
    def test_no_action_changes_a_single_operator_value(
        ...
        first ``CodexCrewConfig.load()`` on a minimal config performs a one-time
        migration write-back that materialises every default key (98 bytes became 9187
        in a direct measurement), and that happens for `codexcrew status`, `config get`
