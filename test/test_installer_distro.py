...
def _run_cli_with_fake_env(
    ...
    *,
    ...
    env = {
        ...
        "CODEXCREW_HOME": str(tmp_path / "data-home"),
        **(extra_env or {}),
    }
...
def test_cli_reuses_a_recorded_managed_python_choice(tmp_path: Path) -> None:
    ...
    `codexcrew update` performs, which passes only --channel. Without the
...
def test_cli_system_pinned_marker_stays_on_system(tmp_path: Path) -> None:
    ...
    survive a flag-less re-run -- the exact shape `codexcrew update` performs --
...
def test_cli_marker_driven_managed_run_falls_back_on_provision_failure(
    ...
    `codexcrew update` takes after the first successful default install) and a
...
def test_cli_uv_download_honors_a_mirror_base(tmp_path: Path) -> None:
    """CODEXCREW_UV_URL points air-gapped / proxied hosts at a mirror of the uv
    ...
    result, markers = _run_cli_with_fake_env(
        tmp_path,
        extra_env={"CODEXCREW_UV_URL": "https://mirror.example.com/uv/"},
    )
