...
class TestChannelSettingsStore:
    def test_mute_persists_and_reloads(self, monkeypatch, tmp_path):
        monkeypatch.setattr(
            "codex_crew.notifications.settings.config_dir", lambda: tmp_path
        )
        s = ChannelSettings()
...
class TestReviewRegressions:
    ...
    def test_persist_failure_leaves_memory_unchanged(self, settings, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.notifications.settings.atomic_write", boom)
    ...
    def test_readers_are_lock_free(self, settings):
        ...
        import codex_crew.notifications.settings as mod
