...
def _loaded(tmp_path: Path, data: dict) -> CodexCrewConfig:
    """Load *data* as config.json through the real load path.

    Goes through `CodexCrewConfig.load()` rather than constructing dataclasses: every
...
def test_the_completion_keep_ceiling_matches_its_owner() -> None:
    ...
    `context_management` does `from codex_crew.config.loader import config_dir`, so the
