...
from codex_crew.apps.builtins.dev_fleet import fleet_state, runtime
...
_UNIT_A = "codexcrew-pod@alpha.service"
_UNIT_B = "codexcrew-pod@beta.service"
_CANNED_TWO_PODS = f"""\
Id={_UNIT_A}
...
def _cfg():
    ...
    return SimpleNamespace(unit_prefix="codexcrew-pod")
...
def test_a_short_record_list_never_misattributes(monkeypatch, _cfg):
    ...
    """
    unit_c = "codexcrew-pod@gamma.service"
    dump = (
        f"Id={_UNIT_A}\nMemoryCurrent=100\nTasksCurrent=1\n"
        "MemoryAccounting=yes\nCPUAccounting=no\n\n"
        f"Id={unit_c}\nMemoryCurrent=300\nTasksCurrent=3\n"
...
def test_home_size_cache_drops_pods_that_stopped(monkeypatch, _cfg):
    ...
    fleet_state._POD_HOME_SIZE_CACHE["codexcrew-pod@evicted.service"] = (1.0, 999)
    fleet_state._pod_resources_sync(_cfg, ["alpha", "beta"])
    assert _UNIT_A in fleet_state._POD_HOME_SIZE_CACHE
    assert "codexcrew-pod@evicted.service" not in fleet_state._POD_HOME_SIZE_CACHE
...
def test_cpu_percent_null_then_pct():
    unit = "codexcrew-pod@alpha.service"
...
def test_empty_running_set_clears_stale_caches(monkeypatch, _cfg):
    ...
    fleet_state._POD_CPU_SAMPLES["codexcrew-pod@gone.service"] = (1.0, 5, "aaa")
    fleet_state._POD_HOME_SIZE_CACHE["codexcrew-pod@gone.service"] = (1.0, 999)
    assert fleet_state._pod_resources_sync(_cfg, []) == {}
...
async def test_home_size_goes_through_the_routed_chokepoint(monkeypatch, _cfg):
    ...
    await fleet_state._pod_home_size(_cfg, "alpha", "codexcrew-pod@alpha.service", 1.0)
...
async def test_home_size_failure_is_none(monkeypatch, _cfg):
    ...
    assert (
        await fleet_state._pod_home_size(_cfg, "alpha", "codexcrew-pod@alpha.service", 1.0) is None
    )
