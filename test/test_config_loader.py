...
Tests the CodexCrewConfig loader validation logic using hypothesis
...
import codex_crew.config.loader as loader_module
...
from codex_crew.stt import limits as stt_limits
from codex_crew.stt import models as stt_models

# Logger used by the loader module — needed for capturing warnings in tests
logger = logging.getLogger("codex_crew.config.loader")
...
def _load_from_dict(data: object) -> CodexCrewConfig:
    """Write *data* to a temp config file and load via CodexCrewConfig.load()."""
    ...
    try:
        with unittest.mock.patch(
            ...
            return CodexCrewConfig.load()
    finally:
        tmp.unlink(missing_ok=True)


def _load_absent_config() -> CodexCrewConfig:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        with (
            ...
            unittest.mock.patch(
                "codex_crew.config.loader.config_local_path",
...
def _load_from_raw_string(content: str) -> CodexCrewConfig:
...
def _load_from_dict_with_logs(data: object) -> tuple[CodexCrewConfig, list[str]]:
    ...
    try:
        with unittest.mock.patch(
            ...
            with unittest.mock.patch.object(logger, "warning", capture_warning):
                result = CodexCrewConfig.load()
            return result, messages
    finally:
        tmp.unlink(missing_ok=True)


def _default_config() -> CodexCrewConfig:
    """Return a default CodexCrewConfig for comparison."""
    return CodexCrewConfig()
...
def test_max_subagents_defaults_to_auto_sentinel() -> None:
    ...
    """
    from codex_crew.subagent import resolve_max_subagents

    # Bare dataclass default.
    assert CodexCrewConfig().agent.max_subagents == 0
...
def test_sandbox_allow_unsandboxed_exec_loads_from_config() -> None:
    assert CodexCrewConfig().agent.sandbox_allow_unsandboxed_exec is False
...
def test_max_stop_hook_nudges_loads_from_config_and_round_trips() -> None:
    ...
    """
    assert CodexCrewConfig().agent.max_stop_hook_nudges == 100
...
def test_dashboard_tailscale_hydrates_and_survives_a_round_trip() -> None:
    ...
    ``codexcrew config set dashboard.tailscale.enabled true`` would land in
    ...
    """
    assert CodexCrewConfig().dashboard.tailscale.enabled is False
...
def test_sandbox_allow_unsandboxed_exec_default_is_platform_independent(monkeypatch) -> None:
    ...
    discoverable path to the opt-in is the ``codexcrew setup`` consent step
...
class TestMalformedConfigValuesNeverCrashLoad:
    ...
    typed value reached the parse and crashed CodexCrewConfig.load() outright —
...
# Strategy for CodexCrewAgentConfig instances
_codexcrew_agent_config_st = st.builds(
    CodexCrewAgentConfig,
...
# Hypothesis strategy for generating valid CodexCrewConfig instances
...
_codexcrew_config_st = st.builds(
    CodexCrewConfig,
    ...
    agents=st.dictionaries(
        keys=_safe_name_st,
        values=_codexcrew_agent_config_st,
        min_size=0,
        max_size=3,
    ),
...
class TestConfigLoaderProperties:
    ...
    # Feature: config-schema, Property 6: CodexCrewConfig load/to_dict round-trip
    @given(config=_codexcrew_config_st)
    ...
    def test_load_to_dict_round_trip(
        ...
        equivalent CodexCrewConfig instance.
    ...
    # Feature: config-schema, Property 12: load() always returns valid CodexCrewConfig
    @given(
        content=st.one_of(
    ...
    def test_load_always_returns_valid_config(
        ...
        """For any input content, load() must return a CodexCrewConfig
        ...
        **Validates: Requirements 6.6**
        ...
        assert isinstance(result, CodexCrewConfig)
    ...
    def test_deprecated_fields_accepted_during_loading(
        ...
        **Validates: Requirements 8.2**
        """
        from codex_crew.config import schema as schema_mod
...
class TestAgentWorkspaceBindingsProperties:
    ...
    def test_new_dataclass_metadata_completeness(
        ...
        """All fields of CodexCrewAgentConfig, WorkspaceConfig, and
        ...
        **Validates: Requirements 1.1, 3.1, 5.1**
        ...
        classes = [CodexCrewAgentConfig, WorkspaceConfig, MemoryStoreConfig]
        cls = classes[cls_idx]
    ...
    def test_config_serialization_round_trip(
        ...
        """For any valid CodexCrewConfig with agents/workspaces/stores,
    ...
    def test_resolver_correct_bindings(
        ...
        **Validates: Requirements 7.1, 7.2, 7.5**
        ...
        config = CodexCrewConfig(
            agents={
                agent_name: CodexCrewAgentConfig(
    ...
    def test_kiro_agent_validation_warnings(
        self,
        agents_data: dict[str, CodexCrewAgentConfig],
        installed: list[str],
    ) -> None:
        ...
        **Validates: Requirements 8.1, 8.2, 8.3**
        """
        config = CodexCrewConfig(agents=agents_data)
        ...
        for mc_name, mc_agent in agents_data.items():
            if not mc_agent.kiro_agent or mc_agent.kiro_agent in installed_set:
                ...
                prefix = f"CodexCrew agent '{mc_name}' references"
    ...
    @given(
        agents_data=st.dictionaries(
            ...
            values=st.fixed_dictionaries(
                {
                    "kiro_agent": st.sampled_from(["codexcrew", "oncall-agent", "custom", ""]),
                    "workspace": _safe_name_st,
                    "memory_store": _safe_name_st,
                },
            ),
    ...
    def test_agents_parsing_with_duplicate_kiro_agent_values(
        ...
        **Validates: Requirements 1.3, 1.7**
        ...
        for name, raw_entry in agents_data.items():
            ...
            assert isinstance(parsed, CodexCrewAgentConfig)
    ...
    def test_backward_compatibility_with_legacy_configs(
        ...
        **Validates: Requirements 9.1, 9.2, 9.3, 9.5**
        ...
        expected_kiro = legacy_default_agent if legacy_default_agent else "codexcrew"
    ...
    **Validates: Requirements 2.4, 4.3, 4.4, 5.4**
    ...
    def test_empty_agents_empty_default_agent_falls_back(self) -> None:
        ...
        **Validates: Requirement 2.4**
        ...
        # Migrated default agent uses "codexcrew" as kiro_agent (no legacy value)
        assert result.kiro_agent == "codexcrew"
...
class TestMultiAgentOrchestrationProperties:
    ...
    def test_config_load_always_produces_agent_with_valid_default(
        ...
        default_agent), loading via CodexCrewConfig.load() shall produce a config
    ...
    def test_backward_compat_migrated_default_produces_identical_resolution(
        ...
        **Validates: Requirements 7.4**
        ...
        # - kiro_agent = agent.default_agent or "codexcrew"
        # - workspace = default_workspace → workspaces["default"].dir
        # - memory_store = default_memory_store
        expected_kiro = legacy_kiro if legacy_kiro else "codexcrew"
    ...
    def test_agent_resolution_correct_workspace_and_memory_store(
        ...
        """For any CodexCrewConfig with agents and a valid agent name, calling
    ...
    # Feature: multi-agent-orchestration, Property 6: Non-CodexCrew agent names resolve via default agent
    @given(
    ...
    @settings(deadline=None)
    def test_non_codexcrew_agent_names_resolve_via_default(
        self,
        default_name: str,
        unknown_name: str,
        kiro_agent_name: str,
        ...
        **Validates: Requirements 1.2, 2.2, 7.2**
        ...
        config = CodexCrewConfig(
            agents={
                default_name: CodexCrewAgentConfig(
                    kiro_agent=kiro_agent_name,
                    workspace="default",
                    memory_store=store_name,
                ),
...
class TestMultiAgentMigrationEdgeCases:
    ...
    **Validates: Requirements 6.1, 6.2, 6.5, 6.7, 1.4, 3.4**
        ...
        **Validates: Requirement 6.1**
        ...
        try:
            with unittest.mock.patch(
                ...
                assert "default" in cfg.agents
                assert cfg.default_agent == "default"
                assert cfg.agents["default"].kiro_agent == "codexcrew"
                ...
                assert on_disk["agents"]["default"]["kiro_agent"] == "codexcrew"
        finally:
            tmp.unlink(missing_ok=True)
    ...
    def test_setup_writes_default_agent(self) -> None:
        """codexcrew setup creates config with default agent via
        ...
        **Validates: Requirement 6.7**
        ...
        from codex_crew.cli_chat import _ensure_default_agent_in_config
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                ...
                unittest.mock.patch(
                    "codex_crew.cli_chat.config_path",
                    return_value=tmp_config,
                ),
            ):
                _ensure_default_agent_in_config()
    ...
    def test_resolver_with_empty_agent_name_uses_default(self) -> None:
        ...
        **Validates: Requirement 3.4**
        ...
        config = CodexCrewConfig(
            agents={
                "mydefault": CodexCrewAgentConfig(
                    kiro_agent="codexcrew",
                    workspace="default",
                    memory_store="default",
                ),
        ...
        assert result2.kiro_agent == "codexcrew"
...
class TestReactionsEmptyStringFiltering:
    ...
    def test_empty_string_reaction_filtered(self, tmp_path: Path) -> None:
        ...
        with unittest.mock.patch("codex_crew.config.loader.config_dir", return_value=tmp_path):
            cfg = CodexCrewConfig.load()
...
def _loaded_stt(tmp_path: Path, section: dict) -> SttConfig:
    ...
    Goes through ``CodexCrewConfig.load()`` rather than constructing an
    ...
    with unittest.mock.patch("codex_crew.config.loader.config_dir", return_value=tmp_path):
        return CodexCrewConfig.load().stt
...
class TestSttRetiredProviders:
    ...
    def test_retired_provider_degrades_to_local(self, retired: str, caplog) -> None:
        with caplog.at_level(logging.WARNING, logger="codex_crew.config.loader"):
...
class TestSttRemovedFieldsAreInert:
    ...
    def test_a_round_trip_does_not_carry_them_forward(self, tmp_path: Path) -> None:
        ...
        cfg_file.write_text(json.dumps({"stt": dict(self._LEGACY_SECTION)}))
        with unittest.mock.patch("codex_crew.config.loader.config_dir", return_value=tmp_path):
            written = CodexCrewConfig.load().to_dict()["stt"]
...
class TestSttModelCatalog:
    ...
    def test_an_unknown_name_degrades_to_the_default(self, caplog) -> None:
        with caplog.at_level(logging.WARNING, logger="codex_crew.stt.models"):
...
class TestSttIntervalClamps:
    ...
    Every bound here is read from ``codex_crew.stt.limits``, which owns them. A
    ...
    def test_the_vad_does_not_re_clamp_what_the_loader_stored(self, tmp_path: Path, caplog) -> None:
        ...
        """
        from codex_crew.stt.vad import Endpointer

        stt = _loaded_stt(tmp_path, {"silence_ms": 1})
        with caplog.at_level(logging.WARNING, logger="codex_crew.stt.vad"):
            caplog.clear()
            Endpointer(silence_ms=stt.silence_ms)
        assert [r.message for r in caplog.records if r.name == "codex_crew.stt.vad"] == []
...
class TestSoftStopBudget:
    ...
    def test_soft_stop_budget_appears_in_schema(self) -> None:
        ...
        from codex_crew.config.schema import SCHEMA_REGISTRY
...
class TestArchiveRetentionDays:
    ...
    def test_schema_permits_null_sentinel(self) -> None:
        ...
        """
        from codex_crew.config.schema import JSON_SCHEMA
    ...
    def test_validation_preserves_null(self) -> None:
        ...
        from codex_crew.config import validation
...
class TestConfigCache:
    ...
    def _reset_cache(self):
        ...
        from codex_crew.config.loader import _invalidate_config_cache
    ...
    _CANON = {
        "agents": {"default": {"kiro_agent": "codexcrew"}},
    ...
    def test_second_load_skips_validation(self, tmp_path: Path) -> None:
        ...
        with (
            patch("codex_crew.config.loader.config_path", return_value=cfg_file),
            patch(
                "codex_crew.config.loader.config_local_path",
                return_value=tmp_path / "config.local.json",
            ),
            patch("codex_crew.config.loader._validate_config_data", _counting),
        ):
            CodexCrewConfig.load()
            CodexCrewConfig.load()
            CodexCrewConfig.load()
    ...
    def test_edit_busts_cache(self, tmp_path: Path) -> None:
        ...
        with (
            patch("codex_crew.config.loader.config_path", return_value=cfg_file),
            patch("codex_crew.config.loader.config_local_path", return_value=local),
        ):
            self._write(cfg_file, {"agent": {"model": "model-a"}})
            first = CodexCrewConfig.load()
    ...
    def test_returned_config_is_independent(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            first.agents["injected"] = CodexCrewAgentConfig(kiro_agent="x")
    ...
    def test_mid_read_write_does_not_cache_stale(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            first = CodexCrewConfig.load()  # reads v0, writer swaps to v1 mid-read
...
class TestSecurityBoundClamping:
    ...
    def test_subagent_auto_max_clamped_to_ceiling(self) -> None:
        from codex_crew.config.loader import SUBAGENT_AUTO_MAX_CEILING

        with unittest.mock.patch("codex_crew.config.loader._log_config_clamp_event"):
    ...
    def test_max_subagents_below_fixed_floor_raised_to_three(self) -> None:
        ...
        from codex_crew.config.loader import MAX_SUBAGENTS_FIXED_FLOOR
    ...
    def test_subagent_max_turns_clamped_to_ceiling(self) -> None:
        from codex_crew.config.loader import SUBAGENT_MAX_TURNS_CEILING
    ...
    def test_pool_size_clamped_to_max(self) -> None:
        from codex_crew.config.loader import POOL_SIZE_MAX
    ...
    def test_session_start_timeout_floored_to_min(self) -> None:
        ...
        from codex_crew.config.loader import SESSION_START_TIMEOUT_MIN
    ...
    def test_session_start_timeout_clamped_to_max(self) -> None:
        ...
        from codex_crew.config.loader import SESSION_START_TIMEOUT_MAX
    ...
    def test_in_range_values_unchanged(self) -> None:
        with unittest.mock.patch("codex_crew.config.loader._log_config_clamp_event") as mock_event:
            cfg = _load_from_dict(
                {
                    "agent": {
                        "subagent_auto_max": 32,
    ...
    def test_non_int_value_not_clamped(self) -> None:
        ...
        from codex_crew.config.loader import _clamp_security_bounds
    ...
    def test_log_config_clamp_event_is_best_effort(self) -> None:
        from codex_crew.config.loader import _log_config_clamp_event

        with unittest.mock.patch("codex_crew.sel.sel", side_effect=RuntimeError("SEL down")):
...
class TestAutocompactPctLoadClamp:
    ...
    def test_above_range_clamped_to_max(self) -> None:
        from codex_crew.config.loader import AUTOCOMPACT_PCT_MAX
    ...
    def test_negative_clamped_to_min(self) -> None:
        from codex_crew.config.loader import AUTOCOMPACT_PCT_MIN
    ...
    def test_default_when_omitted(self) -> None:
        ...
        from codex_crew.config.loader import DEFAULT_AUTOCOMPACT_PCT
    ...
    def test_load_range_and_dashboard_validator_share_one_constant(self) -> None:
        ...
        from codex_crew.config.loader import AUTOCOMPACT_PCT_MAX, AUTOCOMPACT_PCT_MIN
        from codex_crew.dashboard.handlers.core import _EDITABLE_CONFIG
...
class TestConfigWriteProtection:
    ...
    def test_config_json_is_write_protected(self) -> None:
        ...
        # Data home moved to ~/.codex-crew; the legacy ~/.codexcrew stays gated too.
        assert is_sensitive_write_path("~/.codex-crew/config.json")
        assert is_sensitive_write_path(str(Path.home() / ".kiro" / "crew" / "config.json"))
        assert is_sensitive_write_path("~/.codexcrew/config.json")
        assert is_sensitive_write_path(str(Path.home() / ".codexcrew" / "config.json"))

    def test_config_local_json_is_write_protected(self) -> None:
        from codex_crew.security import is_sensitive_write_path

        assert is_sensitive_write_path("~/.codex-crew/config.local.json")
        assert is_sensitive_write_path(str(Path.home() / ".kiro" / "crew" / "config.local.json"))
        assert is_sensitive_write_path("~/.codexcrew/config.local.json")
        assert is_sensitive_write_path(str(Path.home() / ".codexcrew" / "config.local.json"))

    def test_config_json_reads_still_allowed(self) -> None:
        from codex_crew.security import is_sensitive_bash_command, is_sensitive_path
    ...
    def test_write_protection_superset_of_sensitive(self) -> None:
        ...
        assert is_sensitive_write_path("~/.codex-crew/security_policy.json")
        assert is_sensitive_write_path("~/.codexcrew/security_policy.json")

    def test_non_config_codexcrew_file_not_write_protected(self) -> None:
        from codex_crew.security import is_sensitive_write_path

        assert is_sensitive_write_path("~/.codex-crew/sessions.db") is False
        assert is_sensitive_write_path("~/.codexcrew/sessions.db") is False
...
class TestConfigEditToolBlocked:
    ...
    def test_edit_config_json_denied(self) -> None:
        result = self._hooks().on_tool_call(
            "Editing config.json",
            tool_kind="edit",
            raw_params={"path": "~/.codexcrew/config.json"},
        )
    ...
    def test_edit_normal_workspace_file_allowed(self) -> None:
        result = self._hooks().on_tool_call(
            "Editing notes.md",
            tool_kind="edit",
            raw_params={"path": "~/.codexcrew/workspace/notes.md"},
        )
        assert result.action != "deny"
...
class TestMessagingConfigValidation:
    def test_normalizes_bad_scope_mode_and_clamps_resets(self) -> None:
        from codex_crew.config.loader import MessagingConfig
...
class TestKnowledgeAutoIngest:
    ...
    def test_new_keys_are_dashboard_editable(self) -> None:
        # A key absent from the allowlist is rejected by PATCH /api/config/codexcrew,
...
class TestKnowledgePoolIdleTtl:
    ...
    def test_integral_float_preserves_legacy_coercion(
        ...
        import codex_crew.config.validation as validation
...
class TestSaveRoundTripPreservesAllSections:
    ...
    def test_migration_save_preserves_hand_written_sections(self) -> None:
        ...
        with tempfile.NamedTemporaryFile("w", suffix=".json", delete=False) as f:
            ...
            tmp = Path(f.name)
        try:
            with unittest.mock.patch("codex_crew.config.loader.config_path", return_value=tmp):
                CodexCrewConfig.load()  # migration write-back save fires
...
class TestOrchestratorWatchdogThemeAreParsed:
    ...
    def test_per_agent_watchdog_overrides_default_to_inherit(self) -> None:
        ...
        cfg = _load_from_dict({"agents": {"builder": {"kiro_agent": "codexcrew"}}})
    ...
    def test_factory_resolves_canonical_crew_identity(
        ...
        import codex_crew.providers.acp as acp_mod
...
class TestMalformedConfigNeverBricksLoad:
    ...
    contract for malformed-but-writable values reachable via `codexcrew config
...
class TestAppAgentDispatch(unittest.TestCase):
    ...
    def setUp(self):
        ...
        import codex_crew.config.loader as loader
    ...
    def _config(self):
        ...
        return CodexCrewConfig(
            agents={"default": CodexCrewAgentConfig(kiro_agent="codexcrew")},
            default_agent="default",
    ...
    def test_app_agent_matched_by_name_field_dispatches_itself(self):
        ...
        # bare name. Before this fix the bare name was not a CodexCrew alias, so it
    ...
    def test_namespaced_filename_stem_is_not_dispatchable(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with unittest.mock.patch.object(loader, "kiro_agents_dir", lambda: d):
                ...
                assert loader.resolve_agent_bindings(cfg, "mochi--mochi").kiro_agent == "codexcrew"
    ...
    def test_genuinely_unknown_agent_still_falls_back_to_default(self):
        ...
        assert r.kiro_agent == "codexcrew"

    def test_alias_still_wins_over_materialized_file(self):
        # A CodexCrew alias keeps full control of the binding (and the directory is
        # not even scanned for it).
        import codex_crew.config.loader as loader
        from codex_crew.config.loader import CodexCrewAgentConfig

        cfg = self._config()
        cfg.agents["mochi"] = CodexCrewAgentConfig(kiro_agent="explicitly-bound")
        with tempfile.TemporaryDirectory() as td:
            d = self._agents_dir(Path(td), {"mochi--mochi.json": {"name": "mochi"}})
            with unittest.mock.patch.object(loader, "kiro_agents_dir", lambda: d):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with unittest.mock.patch.object(loader, "kiro_agents_dir", lambda: d):
                ...
                for stem in ("junk", "broken"):
                    assert (
                        loader.resolve_agent_bindings(cfg, agent_name=stem).kiro_agent == "codexcrew"
                    )
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with unittest.mock.patch.object(hooks_mod, "safe_read_file", _guarded):
                with unittest.mock.patch.object(loader, "kiro_agents_dir", lambda: d):
                    ...
                    assert loader.resolve_agent_bindings(cfg, "stolen").kiro_agent == "codexcrew"
                    assert loader.resolve_agent_bindings(cfg, "evil").kiro_agent == "codexcrew"
    ...
    def test_out_of_order_refresh_cannot_resurrect_a_deleted_agent(self):
        ...
        assert loader.resolve_agent_bindings(cfg, "deleted").kiro_agent == "codexcrew"
    ...
    def test_substituting_an_alias_name_round_trips_to_the_same_agent(self):
        ...
        cfg = self._config()
        cfg.agents["default"] = CodexCrewAgentConfig(kiro_agent="worker")
        cfg.agents["worker"] = CodexCrewAgentConfig(kiro_agent="other")
    ...
    def test_refresh_replaces_when_no_publish_intervened(self):
        ...
        assert loader.resolve_agent_bindings(cfg, agent_name="gone").kiro_agent == "codexcrew"
    ...
    def test_publish_makes_names_dispatchable_with_no_filesystem_access(self):
        ...
        with unittest.mock.patch.object(loader, "_scan_materialized_agents", _explode):
            ...
            # completeness.
            assert loader.resolve_agent_bindings(cfg, agent_name="nope").kiro_agent == "codexcrew"
            ...
            with unittest.mock.patch.object(loader, "kiro_agents_dir", lambda: d):
                with unittest.mock.patch.object(loader, "_MATERIALIZED_AGENTS", frozenset()):
                    with unittest.mock.patch.object(loader, "_MATERIALIZED_AGENTS_READY", False):
                        assert asyncio.run(_on_loop()).kiro_agent == "codexcrew"
    ...
    def test_project_agent_dispatches_itself(self):
        # A project-local agent is resolvable by kiro-cli (Codex Crew spawns it with
        # the project dir as cwd) but is not a Codex Crew alias, so without the
    ...
    def test_alias_still_wins_over_a_project_agent(self):
        # An explicit Codex Crew alias is authored config; it must not be displaced by
    ...
    def test_project_lookup_reads_nothing_on_the_loop_with_a_cold_cache(self):
        ...
        import codex_crew.agent_discovery as ad
        import codex_crew.config.loader as loader
...
class TestUnsatisfiableSubagentCwdRoots(unittest.TestCase):
    ...
    def _load(self, data: dict) -> tuple[CodexCrewConfig, dict, bool]:
...
def test_agent_triggers_load() -> None:
    ...
    cfg = _load_from_dict(
        {
            "agents": {
                "oncall": {"kiro_agent": "codexcrew", "triggers": "incident, outage"},
                "research": {"kiro_agent": "codexcrew", "description": "deep research crew"},
                "weird": {"kiro_agent": "codexcrew", "triggers": 1},
            },
...
class TestUpdateConfigLocked:
    ...
    def test_lost_update_without_lock(self, tmp_path: Path) -> None:
        ...
        from codex_crew.config.loader import read_config_for_update, write_config_atomically
    ...
    def test_locked_update_preserves_both_writes(self, tmp_path: Path) -> None:
        ...
        from codex_crew.config.loader import update_config_locked
    ...
    def test_fail_closed_unreadable_config(self, tmp_path: Path) -> None:
        ...
        from codex_crew.config.loader import ConfigReadError, update_config_locked
...
class TestMigrationBackupContainment:
    ...
    def _load_with(self, home: Path, cfg_file: Path) -> CodexCrewConfig:
        with (
            unittest.mock.patch("codex_crew.config.loader.config_dir", return_value=home),
            unittest.mock.patch("codex_crew.config.loader.config_path", return_value=cfg_file),
        ):
            return CodexCrewConfig.load()
    ...
    def test_failed_backup_still_aborts_the_migration_save(self, tmp_path: Path) -> None:
        ...
        with unittest.mock.patch(
            "codex_crew.config.loader.shutil.copy2",
...
class TestWeComSectionSurvivesLoad:
    def test_the_sample_map_covers_every_field(self) -> None:
        ...
        assert not missing, (
            ...
            "and check CodexCrewConfig.load() actually reads the key -- it enumerates them "
    ...
    def test_each_configured_value_survives_load(self, field_name: str) -> None:
        ...
        assert loaded != default, (
            ...
            f"as {sample!r} -- CodexCrewConfig.load() is not reading the key"
        )
...
class TestMigrationWriteBackOrdering:
    ...
    awaits ``asyncio.to_thread(CodexCrewConfig.load)``), so the interleave is
    ...
    def test_a_write_landing_mid_migration_survives(self, tmp_path: Path) -> None:
        ...
        locked config write (the shape the dashboard PATCH and ``codexcrew config
    ...
    def test_the_seeded_agent_takes_its_kiro_agent_from_the_reread(self, tmp_path: Path) -> None:
        ...
        ``codexcrew config set agent.default_agent`` landing in between would
