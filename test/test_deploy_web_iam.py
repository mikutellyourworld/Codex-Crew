...
def test_policy_scoping_levers_present():
    ...
    # Must cover BOTH the CFN template prefix (codexcrew-deploy-*) and the
    # engine.py HTTP-publish prefix (codexcrew-web-*).
    ...
    assert cf["Condition"]["StringEquals"]["aws:ResourceTag/codexcrew:managed"] == "true"  # tag scope
...
def test_policy_prefix_matches_template_bucket():
    ...
    # Template uses !Sub 'codexcrew-deploy-${AWS::AccountId}-${AWS::Region}'
    # The IAM prefix must be 'codexcrew-deploy-*' to cover all account/region combos.
...
def test_policy_covers_engine_bucket_prefix():
    """Regression (R8→R9): engine.py creates codexcrew-web-* buckets; IAM must cover them."""
...
def test_policy_covers_base_stack_audit_controls():
    ...
    assert trail["Resource"] == "arn:aws:cloudtrail:*:*:trail/codexcrew-deploy-trail-*"
...
def test_policy_fullstack_tier_includes_lambda_and_dynamodb():
    ...
    # Resources are scoped to codexcrew-deploy-app-*
    lambda_stmt = next(s for s in doc["Statement"] if s["Sid"] == "LambdaFullstack")
    assert "codexcrew-deploy-app-*" in lambda_stmt["Resource"]
...
def test_policy_reaper_passrole_scoped_to_lambda():
    ...
    assert "codexcrew-deploy-reaper" in pass_stmt["Resource"]
    cond = pass_stmt.get("Condition", {})
    assert cond.get("StringEquals", {}).get("iam:PassedToService") == "lambda.amazonaws.com"
...
def test_unmark_webapp_expired_sets_live():
    ...
    from codex_crew.deploy.webapp_types import WebAppDeployTarget, WebAppLifecycle, WebAppMetadata
...
def test_reaper_template_includes_engine_arch_permissions():
    ...
    # CloudFrontEngineSites must be scoped by the codexcrew:site tag PRESENCE
    # test (Null: false), NOT codexcrew:managed==true (which the shared base-stack
    # distribution also carries, so it wouldn't scope to per-site only).
    # The value of codexcrew:site is the site_id string — a StringEquals "true"
    ...
    assert cf_engine["Condition"]["Null"]["aws:ResourceTag/codexcrew:site"] == "false"

    # S3EngineSites must be scoped to codexcrew-web-*
    s3_engine = next(s for s in stmts if s["Sid"] == "S3EngineSites")
    assert "arn:aws:s3:::codexcrew-web-*" in s3_engine["Resource"]
