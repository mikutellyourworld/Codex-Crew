"""Tests for the CLI ``codexcrew update`` wheel-install dispatch (issue #1871).
...
class TestDetectInstallLayout:
    ...
    def test_an_externally_managed_stamp_beats_a_mounted_checkout(
        ...
        _init_repo(proj)
        monkeypatch.setenv("CODEXCREW_PROJECT_DIR", str(proj))
        monkeypatch.setattr("codex_crew.platform.update_capability.distribution", lambda: "docker")
        monkeypatch.setattr("codex_crew.platform.update_layout.distribution", lambda: "docker")
    ...
    def test_no_project_dir_falls_to_distribution(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("codex_crew.platform.update_layout.distribution", lambda: "wheel")
    ...
    def test_dmg_is_externally_managed(self, monkeypatch) -> None:
        monkeypatch.delenv("CODEXCREW_PROJECT_DIR", raising=False)
        monkeypatch.setattr("codex_crew.platform.update_layout.distribution", lambda: "dmg")
    ...
    def test_source_distribution_treated_as_wheel(self, monkeypatch) -> None:
        ...
        monkeypatch.delenv("CODEXCREW_PROJECT_DIR", raising=False)
        monkeypatch.setattr("codex_crew.platform.update_layout.distribution", lambda: "source")
...
class TestReleaseChannel:
    ...
    def test_reads_channel_file(self, monkeypatch, tmp_path) -> None:
        ...
        monkeypatch.setattr("codex_crew.platform.update_layout.data_home", lambda: tmp_path)

        from codex_crew.platform.update_layout import release_channel
...
class TestWheelUpdateCommand:
    ...
    def test_includes_channel(self, monkeypatch, tmp_path) -> None:
        ...
        monkeypatch.setattr("codex_crew.platform.update_layout.data_home", lambda: tmp_path)
        monkeypatch.delenv("CODEXCREW_CDN_BASE", raising=False)

        from codex_crew.platform.update_layout import wheel_update_command
    ...
    def test_cdn_override(self, monkeypatch, tmp_path) -> None:
        monkeypatch.setenv("CODEXCREW_CDN_BASE", "https://custom.cdn.example")
        monkeypatch.setattr("codex_crew.platform.update_layout.data_home", lambda: tmp_path)
...
class TestUpdateWheelCli:
    ...
    def test_up_to_date_exits_cleanly(self, monkeypatch, tmp_path, capsys) -> None:
        ...
        monkeypatch.setattr("codex_crew.cli_server.__version__", "0.2.0")
        monkeypatch.setattr("codex_crew.__version__", "0.2.0")

        import codex_crew.cli_server as cs
    ...
    def test_newer_version_runs_installer(self, monkeypatch, tmp_path, capsys) -> None:
        ...
        monkeypatch.setattr("codex_crew.cli_server.__version__", "0.1.3")
        monkeypatch.setattr("codex_crew.__version__", "0.1.3")
...
class TestUpdateDispatch:
    ...
    def test_externally_managed_prints_guidance(self, monkeypatch, capsys) -> None:
        ...
        monkeypatch.setattr("codex_crew.platform.update_capability.distribution", lambda: "dmg")
        monkeypatch.setattr("codex_crew.cli_server.distribution", lambda: "dmg")
    ...
    def test_an_externally_managed_stamp_wins_over_a_mounted_checkout(
        ...
        monkeypatch.setattr("codex_crew.platform.update_capability.distribution", lambda: "docker")
        monkeypatch.setattr("codex_crew.cli_server.distribution", lambda: "docker")
    ...
    def test_git_checkout_still_works(self, monkeypatch, tmp_path) -> None:
        ...
        monkeypatch.setattr("subprocess.run", fake_run)
        monkeypatch.setattr(
            "codex_crew.platform.update_governance.resolve_remote_url", lambda *a, **k: ""
        )
        monkeypatch.setattr(
            "codex_crew.platform.update_governance.update_blocked_reason", lambda *a: ""
        )
...
class TestUpdateDivergenceGuard:
    ...
    def _run_update(
        ...
        *,
        ...
        monkeypatch.setattr("codex_crew.cli_server.shutil.which", lambda *_: None)
        monkeypatch.setattr("codex_crew.cli._ensure_node", lambda *_: None)
        monkeypatch.setattr("codex_crew.cli_server.build_frontend_sync", lambda *_: None)
        monkeypatch.setattr("codex_crew.cli_server.dep_sync.sync_or_reinstall", lambda *a, **k: 0)
    ...
    def test_cli_wires_force_flag(self, monkeypatch) -> None:
        """``codexcrew update --force`` reaches ``_update(force=True)``."""
        import sys

        with (
            patch.object(sys, "argv", ["codexcrew", "update", "--force"]),
            ...
            # Windows.
            patch("codex_crew.cli._setup_cli_logging"),
            patch("codex_crew.cli_server._update") as mock_update,
        ):
    ...
    def test_cli_defaults_force_off(self, monkeypatch) -> None:
        """A bare ``codexcrew update`` keeps the guard armed (force=False)."""
        import sys

        with (
            patch.object(sys, "argv", ["codexcrew", "update"]),
            patch("codex_crew.cli._setup_cli_logging"),
            patch("codex_crew.cli_server._update") as mock_update,
        ):
            from codex_crew.cli import main
