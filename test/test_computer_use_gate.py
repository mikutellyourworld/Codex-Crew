...
* the observation ceiling is a pass-through, so a renderer cannot lose fields to
  it;
* the one retained refusal (CodexCrew's own window) lives in ``policy``, NOT here —
...
from codex_crew.computer_use import gate
...
class TestTheOneRetainedRefusalIsNotHere:
    def test_codexcrew_self_is_refused_by_policy_not_by_the_gate(self):
        ...
        from codex_crew.computer_use.types import AppRef, PolicyConfig
...
class TestNothingInTreeStillCallsThisGateFailClosed:
    ...
    def _files(self):
        ...
        return [
            p
            for p in [*root.glob("src/codex_crew/**/*.py"), *root.glob("docs/**/*.md")]
