...
class TestConfig:
    def test_defaults_off_and_tunables(self):
        from codex_crew.config.loader import InstancesConfig
        from codex_crew.instances.constants import (
    ...
    def test_recovery_knob_clamps(self):
        ...
        from codex_crew.instances.constants import MAX_RECOVERY_ATTEMPTS_CEILING
        ...
        from codex_crew.instances.constants import RECOVER_BACKOFF_MAX_CEILING_SECS
...
class TestPortAllocator:
    def test_rejects_bad_base(self):
        from codex_crew.instances.port_allocator import PortAllocator
    ...
    def test_is_port_free_detects_live_listener_even_with_reuseaddr(self):
        ...
        """
        from codex_crew.instances.port_allocator import _is_port_free
    ...
    def test_is_port_free_rejects_port_held_on_ipv6_loopback_only(self):
        ...
        """
        from codex_crew.instances.port_allocator import _is_addr_free, _is_port_free
    ...
    def test_is_port_free_treats_unassignable_address_as_free(self, monkeypatch):
        ...
        """
        import codex_crew.instances.port_allocator as pa
...
class TestTokenMint:
    def test_parse(self):
        from codex_crew.instances.token_mint import parse_token_from_stdout
    ...
    def test_ttl_rejects(self, bad):
        from codex_crew.instances.token_mint import TokenMintError, _validate_ttl

        with pytest.raises(TokenMintError):
            _validate_ttl(bad)

    def test_command_builders(self):
        from codex_crew.instances.token_mint import build_remote_token_command
        ...
        custom = build_remote_token_command("~/bin/codexcrew", ttl="30m")
        assert '"$HOME/bin/codexcrew" token --ttl 30m' in custom
        ...
        with_port = build_remote_token_command("~/bin/codexcrew", ttl="20h", port=7879)
        assert '"$HOME/bin/codexcrew" token --ttl 20h --port 7879' in with_port
        # invalid port is rejected (kept out of the shell command unvalidated)
        from codex_crew.instances.token_mint import TokenMintError

        with pytest.raises(TokenMintError):
            build_remote_token_command("", ttl="20h", port=99999)

    def test_token_command_prefers_run_marker_for_port(self):
        from codex_crew.config.paths import CONFIG_DIR_NAME, LEGACY_CONFIG_DIR_NAME
        from codex_crew.instances.token_mint import (
        ...
        # marker is probed under each candidate data home (CODEXCREW_HOME override,
        # the current default, then the legacy home) so a migrated remote whose
        # non-interactive SSH shell doesn't export CODEXCREW_HOME still hits the
        ...
        assert '"${CODEXCREW_HOME:+$CODEXCREW_HOME/run/gateway-7879.bin}"' in cmd
        ...
        custom = build_remote_token_command("~/bin/codexcrew", ttl="20h", port=7879)
        assert "for __mk in" not in custom and "gateway-" not in custom
        assert '"$HOME/bin/codexcrew" token --ttl 20h --port 7879' in custom
        ...
        # the sibling remote-exec path (restart, via run_remote_codexcrew) also
        # prefers the marker when the caller passes the remote port.
        from codex_crew.instances.token_mint import build_remote_command
        ...
        pinned_restart = build_remote_command("~/bin/codexcrew", "restart", marker_port=7781)
        assert "for __mk in" not in pinned_restart and "gateway-" not in pinned_restart

    def test_ssh_argv_shape(self):
        from codex_crew.instances.token_mint import _build_ssh_argv
        ...
        # don't thread a budget (e.g. run_remote_codexcrew).
    ...
    def test_mint_success_and_no_token_in_logs(self, monkeypatch, caplog):
        from codex_crew.instances import token_mint as tm
    ...
    def test_mint_nonzero_exit_raises(self, monkeypatch):
        ...
        class FakeProc:
            ...
            async def communicate(self):
                return b"", b"codexcrew binary not found"
    ...
    def test_nonzero_exit_surfaces_stdout_tail_when_stderr_empty(self, monkeypatch):
        ...
        Pre-fix ``codexcrew token`` printed its failure prose to stdout, so a
        ...
        self._fake_proc(
            monkeypatch,
            1,
            b"\xe2\x9d\x8c Could not reach gateway on port 5486: <urlopen error refused>\n",
            b"",
        )
        with pytest.raises(tm.TokenMintError) as excinfo:
            asyncio.run(tm.mint_remote_token("cd-1", ttl="20h"))
        msg = str(excinfo.value)
        assert "Could not reach gateway on port 5486" in msg
    ...
    def test_stdout_tail_never_leaks_a_token(self, monkeypatch):
        ...
        self._fake_proc(
            monkeypatch,
            1,
            f"http://localhost:5486?token={minted}\nbare {minted} too\nboom\n".encode(),
            b"",
        )
        with pytest.raises(tm.TokenMintError) as excinfo:
            asyncio.run(tm.mint_remote_token("cd-1", ttl="20h"))
    ...
    def test_stdout_tail_is_bounded_and_absent_when_stdout_empty(self, monkeypatch):
        ...
        self._fake_proc(monkeypatch, 127, b"", b"codexcrew binary not found")
        with pytest.raises(tm.TokenMintError) as excinfo:
            asyncio.run(tm.mint_remote_token("cd-1", ttl="20h"))
        assert "stdout tail:" not in str(excinfo.value)
        assert "codexcrew binary not found" in str(excinfo.value)
    ...
    def test_success_path_never_builds_the_stdout_tail(self, monkeypatch):
        ...
        self._fake_proc(monkeypatch, 0, b"http://localhost:5486?token=eyJa.b\n", b"")
...
class TestRunMarker:
    def test_write_read_clear(self, tmp_path, monkeypatch):
        ...
        # Fabricate a venv layout: <venv>/bin/{python,codexcrew}
        bindir = tmp_path / "venv" / "bin"
        bindir.mkdir(parents=True)
        launcher = bindir / "codexcrew"
    ...
    def test_port_only_marker_when_launcher_absent(self, tmp_path, monkeypatch):
        ...
        bindir.mkdir(parents=True)  # no sibling 'codexcrew' launcher
        monkeypatch.setattr(sys, "executable", str(bindir / "python"))
    ...
    def test_mint_clause_ignores_an_empty_marker(self):
        ...
        from codex_crew.instances.token_mint import build_candidate_command
...
class TestRunMarkerDiscovery:
    ...
    def _marker(self, home, name: str) -> None:
        ...
        (d / name).write_text("/some/venv/bin/codexcrew\n", encoding="utf-8")
    ...
    def test_lists_sorted_ports_and_ignores_non_markers(self, tmp_path, monkeypatch):
        ...
        self._marker(tmp_path, "gateway-5486.bin")
        ...
        assert run_marker.marker_ports() == [5486, 6776]
    ...
    def test_read_launcher_reads_marker_content(self, tmp_path, monkeypatch):
        ...
        assert run_marker.read_launcher(6776) is None
        (d / "gateway-6776.bin").write_text("/opt/venv/bin/codexcrew\n", encoding="utf-8")
        assert run_marker.read_launcher(6776) == "/opt/venv/bin/codexcrew"
    ...
    def test_write_prunes_markers_from_earlier_runs(self, tmp_path, monkeypatch):
        ...
        self._marker(tmp_path, "gateway-5486.bin")
        (tmp_path / "run" / "gateway-5486.pid").write_text("111\n", encoding="utf-8")
        (tmp_path / "run" / "gateway-5486.start").write_text("111-start\n", encoding="utf-8")
        ...
        assert not (tmp_path / "run" / "gateway-5486.pid").exists()
        ...
        assert not (tmp_path / "run" / "gateway-5486.start").exists()
...
class TestValidation:
    ...
    def test_ssh_host_accept(self, good):
        from codex_crew.instances.validation import validate_ssh_host
    ...
    def test_ssh_host_reject(self, bad):
        from codex_crew.instances.validation import SshValidationError, validate_ssh_host

        with pytest.raises(SshValidationError):
            validate_ssh_host(bad)

    def test_remote_bin(self):
        from codex_crew.instances.validation import SshValidationError, validate_remote_bin

        assert validate_remote_bin("") == ""
        assert validate_remote_bin("~/.local/bin/codexcrew") == "~/.local/bin/codexcrew"
...
class TestRegistry:
    ...
    def test_crud_and_collision(self, tmp_path):
        from codex_crew.instances.registry import DuplicateInstanceError

        reg = self._reg(tmp_path)
        a = reg.add(name="Cloud Desktop 1", ssh_host="cd-1-alias")
        assert a.id == "cloud-desktop-1" and a.remote_port == 5486 and a.was_connected is False
    ...
    def test_update_validation_and_hints(self, tmp_path):
        from codex_crew.instances.registry import InstanceNotFoundError, InvalidInstanceError
...
def _patch_port_probe(monkeypatch, *, manager_free: bool = True, allocator_free: bool = True):
    ...
    * ``ssh_tunnel_manager`` holds its own re-bound reference for the advisory
    ...
    """
    import codex_crew.instances.port_allocator as pa
    import codex_crew.instances.ssh_tunnel_manager as stm
...
class _FakeTunnel:
    def __init__(
        ...
        *,
        ...
        from codex_crew.instances.ssh_tunnel_manager import TunnelState, TunnelStatus
...
class TestSshTunnelArgvCompression:
    ...
    def test_compression_flag_present_by_default(self):
        from codex_crew.instances.ssh_tunnel_manager import _build_ssh_tunnel_argv
...
class TestSshTunnelManager:
    ...
    def _mgr(self, tmp_path, *, mint=None, factory=_FakeTunnel):
        from codex_crew.instances.registry import InstancesRegistry
        from codex_crew.instances.ssh_tunnel_manager import SshTunnelManager
    ...
    async def test_connect_persists_and_idempotent(self, tmp_path):
        from codex_crew.instances.ssh_tunnel_manager import TunnelState
    ...
    async def test_disconnect_clears_local_port(self, tmp_path):
        ...
        from codex_crew.instances.registry import _UNALLOCATED_PORT
    ...
    async def test_token_validates_status_mapping(self, tmp_path, monkeypatch):
        from codex_crew.instances import ssh_tunnel_manager as m
...
class TestValidatorsRejectEmbeddedNewlines:
    ...
    def test_a_trailing_newline_never_survives_validation(self, tmp_path):
        ...
        """
        from codex_crew.instances import validation
        from codex_crew.instances.registry import (
            InstancesRegistry,
        ...
        for fn, dirty in (
            ...
            (validation.validate_remote_bin, "/usr/bin/codexcrew\n"),
...
class TestHandlers:
    ...
    def test_disabled_returns_403(self, tmp_path, monkeypatch):
        from codex_crew.dashboard import handlers_instances as handlers
    ...
    def test_add_defaults_remote_port_to_the_stock_gateway_port(self, tmp_path, monkeypatch):
        ...
        """
        from codex_crew.dashboard import handlers_instances as handlers
        from codex_crew.instances.registry import DEFAULT_REMOTE_PORT
        ...
        assert reg.get("cd-1").remote_port == DEFAULT_REMOTE_PORT == 5486
    ...
    def test_ttl_beyond_the_minters_bound_is_refused(self, tmp_path, monkeypatch):
        ...
        from codex_crew.dashboard import handlers_instances as handlers
        from codex_crew.instances.registry import InvalidInstanceError
    ...
    def test_update_locks_addressing_fields_for_correlated_cloud_instance(
        ...
        # resolve an EC2 stack launched by Codex Crew — doing so strands a running,
        # billing instance with no dashboard path to reach it.
        from codex_crew.cloud import launch_job as lj
        from codex_crew.dashboard import handlers_instances as handlers
        ...
        # A job Codex Crew provisioned whose EC2 instance id matches the
        # instance's ssm_target — this is what makes it "correlated".
        store = lj.LaunchJobStore()  # honours CODEXCREW_HOME, same as production
...
class TestTokenMintGeneric:
    def test_ttl_to_seconds(self):
        from codex_crew.instances.token_mint import TokenMintError, ttl_to_seconds
    ...
    def test_generic_builders_and_token_delegation(self):
        ...
        assert '"$HOME/bin/codexcrew" restart' in build_remote_command("~/bin/codexcrew", "restart")
        # token builder emits identical strings via the generic builders it delegates to
        assert 'exec "$b" token --ttl 20h;' in build_remote_token_command("", ttl="20h")

    def test_run_remote_codexcrew(self, monkeypatch):
        from codex_crew.instances import token_mint as tm
        ...
        monkeypatch.setattr(asyncio, "create_subprocess_exec", fake_exec)
        rc, err = asyncio.run(tm.run_remote_codexcrew("cd-1", "restart"))
        assert rc == 0 and err == ""

    def test_run_remote_codexcrew_honors_connect_timeout_secs(self, monkeypatch):
        ...
        monkeypatch.setattr(asyncio, "create_subprocess_exec", fake_exec)
        rc, _ = asyncio.run(tm.run_remote_codexcrew("cd-1", "restart", connect_timeout_secs=45.0))
        assert rc == 0
        assert "ConnectTimeout=45" in captured["argv"]

    def test_run_remote_codexcrew_redacts_stderr(self, monkeypatch):
    ...
    def test_run_remote_codexcrew_redacts_urls_before_credentials(self, monkeypatch):
    ...
    def test_run_remote_codexcrew_timeout_reaps_child_via_communicate_not_wait(self, monkeypatch):
        from codex_crew.instances import token_mint as tm
...
class TestDiagnostics:
    def _set_probes(self, monkeypatch, ssh, remote, local):
        from codex_crew.instances import diagnostics as diag
    ...
    def test_ladder_first_broken_link(self, monkeypatch):
        from codex_crew.instances.diagnostics import (
            NOT_CONNECTED,
            OK,
            REMOTE_DOWN,
            SSH_UNREACHABLE,
    ...
    def test_invalid_host_short_circuits(self):
        from codex_crew.instances.diagnostics import UNKNOWN, diagnose_instance
...
class TestSelfHealRefreshRestart:
    ...
    async def test_wait_until_ready_rejects_child_that_dies_during_probe(self):
        from codex_crew.instances.ssh_tunnel_manager import TunnelState, _SshTunnel
    ...
    def test_exit_error_strips_post_quantum_noise(self):
        from codex_crew.instances.ssh_tunnel_manager import _SshTunnel
    ...
    def test_recover_backoff_grows_and_caps(self):
        from codex_crew.instances.ssh_tunnel_manager import (
    ...
    def test_restart_remote(self, tmp_path, monkeypatch):
        from codex_crew.instances import ssh_tunnel_manager as stm
        ...
        monkeypatch.setattr(stm, "run_remote_codexcrew", fake_run)
        r = asyncio.run(mgr.restart_remote("cd-1"))
        # remote_port defaults to 5486 → threaded so restart uses the marker resolver.
        ...
        assert r["ok"] and calls["a"] == ("cd-1-alias", "restart", 5486, 30.0)
        # validation failure
    ...
    def test_diagnose_caps_connect_timeout_at_the_diagnostics_ceiling(self, tmp_path, monkeypatch):
        ...
        async def fake_diagnose(ssh_host, remote_port, local_port, connect_timeout_secs=10.0):
            ...
            from codex_crew.instances.diagnostics import OK, DiagnosisResult
...
class TestInstancesStartupHooks:
    ...
    def test_register_then_freeze_then_startup_creates_manager(self, tmp_path, monkeypatch):
        ...
        from codex_crew.dashboard.server import _register_instances_hooks
...
class TestPortMirror:
    ...
    async def test_two_instances_share_one_remote_port(self, tmp_path, monkeypatch):
        ...
        reg.add(name="A", ssh_host="host-a", instance_id="cd-a", remote_port=5486)
        reg.add(name="B", ssh_host="host-b", instance_id="cd-b", remote_port=5486)
        mgr = self._mgr(reg, factory, monkeypatch)
        ...
        assert 5486 not in seen.values()
...
class TestStatusForRetainedError:
    ...
    async def test_surfaces_error_when_no_live_tunnel(self, tmp_path):
        ...
        from codex_crew.dashboard.handlers_instances import _status_for
        from codex_crew.instances.token_mint import TokenMintError
...
class TestStartupRevive:
    ...
    async def test_revives_all_was_connected(self, tmp_path):
        import codex_crew.dashboard.server as server
        from codex_crew.instances.ssh_tunnel_manager import TunnelState
    ...
    async def test_instances_startup_schedules_revive_in_background(self, monkeypatch):
        ...
        monkeypatch.setattr(server, "CodexCrewConfig", types.SimpleNamespace(load=lambda: cfg))
        ...
        server._register_instances_hooks(app, state, 5486)
...
class TestSsmValidation:
    ...
    def test_valid_targets(self):
        from codex_crew.instances.validation import validate_ssm_target
    ...
    def test_rejects_bad_targets(self, bad):
        from codex_crew.instances.validation import SsmValidationError, validate_ssm_target

        with pytest.raises(SsmValidationError):
            validate_ssm_target(bad)

    def test_profile_and_region(self):
        from codex_crew.instances.validation import (
            SsmValidationError,
    ...
    def test_ssm_run_as_accepts_unix_usernames_and_defaults_when_empty(self):
        ...
        from codex_crew.instances.validation import validate_ssm_run_as
    ...
    def test_ssm_run_as_rejects_injection_and_bad_usernames(self):
        ...
        from codex_crew.instances.validation import SsmValidationError, validate_ssm_run_as
...
class TestSsmRegistry:
    ...
    def test_aws_profile_regex_is_single_sourced(self):
        ...
        """
        from codex_crew.instances import registry, validation
    ...
    def test_ssm_run_as_defaults_and_round_trips(self, tmp_path):
        ...
        """
        from codex_crew.instances.registry import Instance
...
class TestSsmTunnelArgv:
    ...
    def _bare_resolver(self, monkeypatch):
        ...
        from codex_crew.cloud import ssm
    ...
    def test_argv_shape(self):
        from codex_crew.instances.ssh_tunnel_manager import _build_ssm_tunnel_argv
...
class TestSsmTunnelProcessGroup:
    ...
    async def test_ssm_spawn_passes_both_isolation_kwargs(self, monkeypatch):
        ...
        import codex_crew.instances.ssh_tunnel_manager as mod
    ...
    async def test_ssm_child_gets_plugin_search_path_and_ssh_inherits(self, monkeypatch, tmp_path):
        ...
        """
        import codex_crew.instances.ssh_tunnel_manager as mod
        from codex_crew.deploy import engine
...
class TestSsmTransportSelection:
    ...
    async def test_ssm_instance_uses_ssm_transport_and_ssm_mint(self, tmp_path, monkeypatch):
        import codex_crew.instances.ssh_tunnel_manager as mod
        from codex_crew.instances.constants import DEFAULT_SSM_CONNECT_TIMEOUT_SECS
        ...
        monkeypatch.setattr("codex_crew.cloud.ssm.session_manager_plugin_installed", lambda: True)
    ...
    async def test_mint_timeout_ssm_default_and_override(self, tmp_path, monkeypatch):
        ...
        import codex_crew.instances.ssh_tunnel_manager as mod
        from codex_crew.instances.constants import DEFAULT_SSM_MINT_TIMEOUT_SECS
        from codex_crew.instances.registry import InstancesRegistry
        from codex_crew.instances.ssh_tunnel_manager import SshTunnelManager
    ...
    async def test_plugin_probe_runs_off_the_event_loop(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.cloud.ssm.session_manager_plugin_installed", _probe)
        reg, mgr = self._mgr(tmp_path)
        reg.add(
            name="EC2",
            connection_method="ssm",
    ...
    async def test_ssm_connect_fails_clean_without_plugin(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.cloud.ssm.session_manager_plugin_installed", lambda: False)
        reg, mgr = self._mgr(tmp_path)
        reg.add(
            name="EC2",
            connection_method="ssm",
...
class TestSsmDiagnostics:
    ...
    async def test_unreachable_node_is_first_rung(self, monkeypatch):
        import codex_crew.instances.diagnostics as diag
...
class TestForwarderPidHints:
    ...
    def test_registry_field_default_roundtrip_and_validation(self, tmp_path):
        from codex_crew.instances.registry import Instance, InvalidInstanceError
...
class TestProxyRequest:
    ...
    async def test_not_connected_raises_typed_error(self, tmp_path):
        from codex_crew.instances.ssh_tunnel_manager import ProxyRequestError
...
class TestPeerRequestSharedDance:
    ...
    def test_peer_cookie_header_refuses_when_no_credential(self, tmp_path):
        from codex_crew.instances.ssh_tunnel_manager import _PeerUnavailable
...
class TestProxyHandlerPolicy:
    ...
    async def test_client_disconnect_midstream_does_not_write_after_reset(
        ...
        """
        from codex_crew.dashboard.handlers_instances import api_instances_proxy
        ...
        from codex_crew.dashboard import handlers_instances as hi
    ...
    def test_paths_outside_the_chat_allowlist_are_refused(self, raw):
        ...
        from codex_crew.dashboard.handlers_instances import (
    ...
    def test_bare_chat_prefix_itself_is_forwarded(self):
        ...
        from codex_crew.dashboard.handlers_instances import _proxy_canonical_path
    ...
    def test_allowlist_constant_is_pinned_exactly(self):
        ...
        invariant."""
        from codex_crew.dashboard.handlers_instances import _PROXY_ALLOWED_PREFIXES
