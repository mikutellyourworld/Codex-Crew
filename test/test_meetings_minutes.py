...
class TestOwnership:
    ...
    def test_agent_file_tools_cannot_read_or_overwrite_an_owner_edit(
        ...
        assert is_sensitive_path(f"~/.codex-crew/{relative_edit}") is True
        assert is_sensitive_path(f"~/.codexcrew/{relative_edit}") is True
        assert is_sensitive_bash_command(f"cat ~/.codex-crew/{relative_edit}") is not None
...
class TestWiring:
    def test_both_methods_are_registered_on_the_outputs_path(self):
        ...
        from codex_crew.apps.builtins.meetings.backend.routes import register_routes
