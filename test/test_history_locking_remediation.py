...
class TestOnLoopPersistDiscipline:
    ...
    def test_raw_on_loop_mutator_raises_in_strict_mode(
        ...
        """With strict enforcement on (``CODEXCREW_STRICT_ON_LOOP_PERSIST=1``) a
        ...
        monkeypatch.setenv("CODEXCREW_STRICT_ON_LOOP_PERSIST", "1")
        log = ConversationLog(base_dir=tmp_path)
    ...
    def test_dev_mode_also_enables_strict(
        ...
        """``CODEXCREW_DEV_MODE`` (developer gateway) also turns the guard strict,
        and an explicit falsy ``CODEXCREW_STRICT_ON_LOOP_PERSIST`` overrides it so
        ...
        monkeypatch.setenv("CODEXCREW_DEV_MODE", "1")
        assert history_mod._on_loop_persist_strict() is True
        # Explicit falsy override wins over dev-mode.
        monkeypatch.setenv("CODEXCREW_STRICT_ON_LOOP_PERSIST", "0")
        assert history_mod._on_loop_persist_strict() is False

        log = ConversationLog(base_dir=tmp_path)
        monkeypatch.setenv("CODEXCREW_STRICT_ON_LOOP_PERSIST", "")
        monkeypatch.setenv("CODEXCREW_DEV_MODE", "1")
    ...
    def test_offloaded_call_does_not_trip_guard(
        ...
        from codex_crew.history import append_off_loop
    ...
    def test_e2e_harness_enables_strict_mode_ci_enforcement(self) -> None:
        ...
        boots that gateway with ``CODEXCREW_STRICT_ON_LOOP_PERSIST=1``, so any
        ...
        # env["CODEXCREW_STRICT_ON_LOOP_PERSIST"] = "1" (a truthy string).
        found = False
        for node in ast.walk(tree):
            ...
            for tgt in node.targets:
                if (
                    ...
                    and tgt.slice.value == "CODEXCREW_STRICT_ON_LOOP_PERSIST"
                    and isinstance(node.value, ast.Constant)
                    and str(node.value.value).strip().lower() in {"1", "true", "yes", "on"}
                ):
                    found = True
        assert found, (
            "setup.py test_e2e must export CODEXCREW_STRICT_ON_LOOP_PERSIST=1 into "
...
class TestRotateOversizedFewLines:
    ...
    def test_rotation_declines_when_archiving_the_dropped_lines_fails(
        ...
        """
        from codex_crew import history as history_mod
...
class TestDeleteSessionMissingOk:
    ...
    def test_toctou_removal_does_not_raise(
        ...
        monkeypatch.setattr("codex_crew.history.Path.exists", lambda self: True)
        # Must not raise.
...
class TestUpdateMetadataOffLoop:
    ...
    def test_inline_update_when_no_running_loop(self, tmp_path: Path) -> None:
        ...
        from codex_crew.history import update_metadata_off_loop
...
class TestOnLoopCallersOffload:
    ...
    def test_persist_title_runs_update_metadata_off_loop(
        ...
        from codex_crew.dashboard import chat_title
    ...
    def test_api_session_delete_runs_delete_off_loop(
        ...
        from codex_crew.dashboard.handlers import sessions as sessions_mod
...
class TestDashboardSaveHoldsLock:
    ...
    def test_foreign_append_content_identity_dedup_semantics(
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.state.config_dir", lambda: tmp_path
        )
    ...
    def test_on_loop_cleanup_deferred_to_worker_thread(self, tmp_path: Path) -> None:
        ...
        async def _run() -> None:
            ...
            try:
                ...
                from codex_crew.history import allow_on_loop_persist
...
class TestAppendIfAbsent:
    ...
    def test_off_loop_wrapper_dispatches_to_thread(self, tmp_path: Path) -> None:
        ...
        from codex_crew.history import append_if_absent_off_loop
