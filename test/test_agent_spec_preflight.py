...
``{'code': -32603, ..., 'data': "Mode 'codexcrew' not found"}`` frame.
"""
...
from codex_crew.acp.runtime import _format_runtime_rpc_error
from codex_crew.agent import missing_required_agent_specs
...
class TestMissingRequiredAgentSpecs:
    ...
    def test_empty_dir_reports_every_required_spec(self, tmp_path):
        ...
        agents_dir.mkdir()
        with patch("codex_crew.agent.KIRO_AGENTS_DIR", agents_dir):
    ...
    def test_absent_dir_reports_every_required_spec(self, tmp_path):
        ...
        with patch("codex_crew.agent.KIRO_AGENTS_DIR", tmp_path / "nope"):
            assert missing_required_agent_specs() == list(REQUIRED_KIRO_AGENT_FILES)
...
class TestFormatRuntimeRpcError:
    ...
    @pytest.mark.parametrize("agent", ["codexcrew", "codexcrew-lite"])
    def test_missing_spec_error_is_actionable(self, agent, tmp_path):
        ...
        assert "codexcrew setup --agent-only --clean" in text  # names the repair
...
class TestGatewayInstallVerification:
    ...
    def _run_init_services(orch, *, rebuild):
        ...
        names = [
            "codex_crew.slack.gateway.SkillsLoader",
            "codex_crew.slack.gateway.HookManager",
            "codex_crew.slack.gateway.LessonStore",
            "codex_crew.slack.gateway.ContextBuilder",
            "codex_crew.slack.gateway.SessionManager",
            "codex_crew.slack.gateway.HistoryConsolidator",
            "codex_crew.slack.gateway.ChannelHistory",
        ]
        with contextlib.ExitStack() as stack:
            for name in names:
                stack.enter_context(patch(name))
            for name in ("MemoryStore", "ConversationLog"):
                mock = stack.enter_context(patch(f"codex_crew.slack.gateway.{name}"))
                mock.return_value = MagicMock()
            vector = stack.enter_context(patch("codex_crew.vector_memory.VectorMemoryStore"))
            vector.return_value = MagicMock()
            stack.enter_context(patch("codex_crew.agent.rebuild_agent_config", rebuild))
            proc = MagicMock()
            proc.returncode = 0
    ...
    def test_install_exception_logs_error_not_warning(self, tmp_path, caplog, capsys):
        ...
        with caplog.at_level(logging.ERROR, logger="codex_crew.slack.gateway"):
        ...
        assert "codexcrew setup --agent-only --clean" in capsys.readouterr().out
    ...
    def test_silent_no_write_is_reported(self, tmp_path, caplog, capsys):
        ...
        with caplog.at_level(logging.ERROR, logger="codex_crew.slack.gateway"):
            with patch("codex_crew.agent.KIRO_AGENTS_DIR", agents_dir):
                with patch("codex_crew.slack.gateway.kiro_agents_dir", return_value=agents_dir):
    ...
    def test_healthy_install_is_silent(self, tmp_path, caplog, capsys):
        ...
        assert not [
            r
            for r in caplog.records
            if r.levelno >= logging.ERROR and r.name.startswith("codex_crew")
        ]
