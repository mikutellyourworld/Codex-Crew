...
from codex_crew.dashboard.handlers.notifications_push import api_push_notification
from codex_crew.dashboard.token_auth import app_token_path_allowed
from codex_crew.notifications.bus import NotificationBus
from codex_crew.notifications.rate_limit import RATE_LIMIT_BURST, AppRateLimiter
...
class TestAppRateLimiter:
    ...
    def test_tokens_refill_over_time(self, monkeypatch):
        ...
        now = [1000.0]
        monkeypatch.setattr(
            "codex_crew.notifications.rate_limit.time.monotonic", lambda: now[0]
        )
        for _ in range(RATE_LIMIT_BURST):
            assert limiter.allow("app-a")
        assert not limiter.allow("app-a")
...
def _patch_channels(channels="unset"):
    return patch(
        "codex_crew.dashboard.handlers.notifications_push._resolve_app_channels",
...
class TestPushHandler:
    ...
    async def test_body_size_boundary_exact(self):
        ...
        from codex_crew.dashboard.handlers._shared import _MAX_BODY_BYTES
    ...
    def test_resolve_app_channels_is_read_only(self):
        ...
        from codex_crew.dashboard.handlers.notifications_push import _resolve_app_channels

        with patch("codex_crew.apps.manager.get_app") as get_app_mock, patch(
            "codex_crew.dashboard.handlers.notifications_push.is_app_enabled",
            return_value=False,
        ):
    ...
    async def test_missing_app_token_denial_emits_sel_audit(self):
        ...
        state = _FakeState()
        with patch(
            "codex_crew.dashboard.handlers.notifications_push.sel"
        ) as sel_mock:
            async with TestClient(TestServer(_make_app(state, {"app": ""}))) as client:
                resp = await client.post(
                    "/api/notifications/push",
...
class TestReservedAppName:
    ...
    async def test_push_from_reserved_app_name_denied(self):
        ...
        with patch(
            ...
            "codex_crew.dashboard.handlers.notifications_push.get_app_manifest"
        ) as gm:
