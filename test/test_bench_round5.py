...
from codex_crew.eval.bench.adapters import locomo as locomo_adapter
...
from codex_crew.eval.bench.ingest import (
    IngestConfig,
    IngestedInstance,
    IngestError,
    ingest_instance,
)
from codex_crew.eval.bench.retrieval import QueryRetrieval, aggregate
...
def test_recorded_identity_comes_from_the_live_embedder(monkeypatch) -> None:
    """`CODEXCREW_EMBED_MODEL_PATH` runs a different model, and the width can be
    adopted from the file, so the module constants do not describe the system."""
    from codex_crew.eval.bench import run as run_mod
    ...
    monkeypatch.setattr(
        "codex_crew.embeddings.get_shared_embedder", lambda: FakeEmbedder(), raising=True
    )
...
def test_an_unreadable_identity_refuses_rather_than_claiming_the_bundled_one(
    ...
    monkeypatch.setattr(
        "codex_crew.embeddings.get_shared_embedder", lambda: Mute(), raising=True
    )
...
def test_all_three_corpus_readers_route_through_the_nofollow_helper() -> None:
    ...
    from codex_crew.eval.bench.adapters import longmemeval
