...
from codex_crew.workflows.agent_pool import build_pooled_agent_fn
...
def _patch_stream(monkeypatch):
    monkeypatch.setattr("codex_crew.workflows.agent_pool.stream_and_collect", _fake_stream)
    # redaction is a no-op passthrough for these tests
    monkeypatch.setattr("codex_crew.workflows.agent_pool.redact_credentials", lambda t: (t, []))
    monkeypatch.setattr(
        "codex_crew.workflows.agent_pool.redact_exfiltration_urls", lambda t: (t, [])
    )
...
async def test_pooled_is_faster_than_cold_start_per_call():
    ...
    import codex_crew.workflows.agent_pool as ap
...
async def test_end_to_end_service_pooled_cold_starts_fewer_sessions(monkeypatch):
    ...
    import codex_crew.workflows.agent_exec as ae
    import codex_crew.workflows.agent_pool as ap
...
async def test_send_message_enforces_timeout(monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.workflows.agent_pool.stream_and_collect", _hang)
    ...
    from codex_crew.workflows.agent_pool import _WorkflowSessionWorker
...
async def test_worker_pool_send_timeout_reaches_worker(monkeypatch):
    ...
    sessions = _FakeSessions()
    from codex_crew.acp.worker_pool import WorkerPool
    from codex_crew.workflows.agent_pool import _WorkflowSessionWorker
...
def test_max_turns_constant_is_shared_with_agent_exec():
    ...
    from codex_crew.workflows import agent_exec, agent_pool
