...
(``codex_crew.github_runner.resolve_gh``): the well-known install dirs first,
...
from codex_crew import github_runner  # noqa: E402
from codex_crew.apps.builtins.issue_radar.backend import github_client as gh  # noqa: E402


@pytest.fixture(autouse=True)
def _reset_gh_cache(monkeypatch):
    github_runner.reset_cache()
    monkeypatch.delenv("CODEXCREW_ISSUE_RADAR_GH", raising=False)
...
def test_gh_bin_refuses_a_shim_inside_the_agent_writable_tree(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(
        github_runner,
        "PROVIDER_EXECUTABLE_CANDIDATES",
        {"gh": (binary,), "glab": ("/nonexistent-codexcrew/glab",)},
    )
...
def test_gh_bin_override_failure_is_a_setup_error(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("CODEXCREW_ISSUE_RADAR_GH", str(tmp_path / "missing-gh"))

    with pytest.raises(gh.GhSetupError) as excinfo:
        gh._gh_bin()

    assert excinfo.value.reason == "not_installed"
    assert "CODEXCREW_ISSUE_RADAR_GH" in str(excinfo.value)
