"""Further coverage for ``codex_crew.slack.gateway``.
...
signal, no socket and no write outside the per-test ``CODEXCREW_HOME`` (pinned
...
from codex_crew.autonudge import APPROVAL_STALL_REASON, NudgeLoop
...
class TestWarnIfKiroCliOutdated:
    ...
    async def test_unresolvable_binary_never_spawns(self, capsys):
        ...
        with patch("codex_crew.slack.gateway.resolve_kiro_cli", return_value=None):
    ...
    async def test_path_only_install_is_refused_but_reported(self, caplog):
        ...
        with caplog.at_level("WARNING"):
            with patch("codex_crew.slack.gateway.resolve_kiro_cli", side_effect=_resolve):
                with patch("asyncio.create_subprocess_exec") as spawn:
                    await orch._warn_if_kiro_cli_outdated()
        spawn.assert_not_called()
        assert "CODEXCREW_KIRO_BIN" in caplog.text
        ...
        with caplog.at_level("WARNING"):
            with patch("codex_crew.slack.gateway.resolve_kiro_cli", return_value=None):
                with patch("asyncio.create_subprocess_exec") as spawn:
                    await orch._warn_if_kiro_cli_outdated()
        spawn.assert_not_called()
        assert "CODEXCREW_KIRO_BIN" not in caplog.text
    ...
    async def test_slow_home_directory_cannot_stall_boot(self, caplog):
        ...
        with caplog.at_level("WARNING"):
            with patch.object(gw, "_KIRO_CLI_RESOLVE_TIMEOUT_SECS", 0.01):
                with patch("codex_crew.slack.gateway.resolve_kiro_cli", side_effect=_hang):
                    with patch("asyncio.create_subprocess_exec") as spawn:
                        await orch._warn_if_kiro_cli_outdated()
        spawn.assert_not_called()
...
class TestDeliverResultRouting:
    ...
    async def test_default_deliver_lookup_failure_falls_through(self):
        ...
        with patch.object(gw.CodexCrewConfig, "load", side_effect=RuntimeError("no config")):
...
class TestShutdownExtras:
    ...
    async def test_slot_save_failure_does_not_abort_shutdown(self):
        ...
        with patch(
            "codex_crew.dashboard.chat.save_all_slots_to_history",
...
def _prepared_orchestrator() -> Any:
    orch = _make_orchestrator()
    orch._cfg.dashboard.url = "http://127.0.0.1:5486"
...
class TestDashboardPortSelection:
    ...
    async def test_api_server_ephemeral_port_is_read_back(self):
        ...
        with patch.object(gw, "LessonStore", MagicMock()):
            with patch(
                "codex_crew.dashboard.start_api_server", AsyncMock(return_value=(runner, ds))
            ) as start:
...
class TestAutoMigrateMemory:
            ...
            with patch.object(gw, "model_file_present", return_value=True):
                with patch.object(gw, "make_sync_embed_fn", return_value=lambda s: [0.0]):
                    with patch.object(gw, "reconcile_store_embedding_space") as reconcile:
                        with patch("codex_crew.memory.legacy_memory_present", return_value=True):
    ...
    async def test_set_memory_migrated_delegates_to_handler(self):
        orch = _make_orchestrator()
        with patch(
            "codex_crew.dashboard.handlers.memory._set_migrated", AsyncMock()
        ) as set_migrated:
...
class TestInitCrew:
    ...
    def test_orchestrator_failure_disables_crew_mode(self, caplog):
        ...
        with caplog.at_level("WARNING"):
            with patch(
                "codex_crew.crew_chat.CrewOrchestrator", side_effect=RuntimeError("bad wiring")
            ):
                orch._init_crew()
    ...
    def test_no_dashboard_state_skips_crew_setup(self):
        orch = _make_orchestrator()
        orch.dashboard_state = None
        with patch("codex_crew.crew_chat.CrewOrchestrator") as ctor:
            orch._init_crew()
...
class TestInitMcpDiscovery:
    ...
    def test_configured_servers_are_listed(self, caplog):
        ...
        with caplog.at_level("INFO"):
            with patch("codex_crew.mcp_discovery.list_servers", return_value=servers):
                orch._init_mcp_discovery()
        assert "builder-mcp, playwright-mcp" in caplog.text

    def test_listing_failure_is_swallowed(self):
        orch = _make_orchestrator()
        with patch("codex_crew.mcp_discovery.list_servers", side_effect=RuntimeError("boom")):
...
class TestNotifyNudgeExpired:
    ...
    def test_a_terminal_subject_outranks_the_wall_clock_budget(self):
        ...
        """
        from codex_crew.autonudge import MONITOR_TERMINAL_REASON
        from codex_crew.monitoring.models import MonitorOutcome, MonitorState
    ...
    def test_an_owed_terminal_turn_outranks_the_cycle_cap(self):
        ...
        """
        from codex_crew.monitoring.models import MonitorState
