...
hooks of ``codex_crew.apps.builtins.dev_fleet.server``.
...
from codex_crew.apps.builtins.dev_fleet import (
    fleet_state,
    gateway_service,
    http_api,
    live,
...
def test_load_dev_fleet_cfg_config_dir_failure_is_empty(monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.config.loader.config_dir", _boom)
    assert repository._load_dev_fleet_cfg() == {}
...
@pytest.mark.parametrize(
    ...
    [
        "#!/bin/sh\nexport FOO=1\n",  # no exec line at all
        "#!/bin/sh\nexec '/usr/local/bin/codexcrew' gateway\n",  # not a venv binary
    ],
)
...
def test_launchd_live_worktree_resolves_checkout(monkeypatch, tmp_path):
    ...
    checkout = tmp_path / "codexcrew-wt-alpha"
    kcbin = checkout / ".venv" / "bin" / "codexcrew"
...
async def test_load_fallback_repos_collects_ancestor_remotes(monkeypatch):
    ...
    async def fake_run(cmd, **kw):
        ...
        if "get-url" in cmd:
            ...
            if remote == "origin":
                return 0, "git@github.com:mikutellyourworld/Codex-Crew.git\n", ""
            return 0, "git@github.com:someone/codexcrew-old.git\n", ""
        return 1, "", "unexpected"
    ...
    assert repository._FALLBACK_REPOS == ["someone/codexcrew-old"]
...
async def test_load_fallback_repos_skips_duplicate_upstream_alias(monkeypatch):
    ...
    async def fake_run(cmd, **kw):
        ...
        if "get-url" in cmd:
            ...
            return 0, "https://github.com/mikutellyourworld/Codex-Crew.git\n", ""
        return 1, "", "unexpected"

    monkeypatch.setattr(repository, "_repo", lambda: "/fake/repo")
    monkeypatch.setattr(repository, "_UPSTREAM_REMOTE", "codexcrew")
    monkeypatch.setattr(runtime, "_run_cmd", fake_run)
    await repository._load_fallback_repos()
    assert repository._FALLBACK_REPOS == []
...
async def test_load_fallback_repos_recognizes_scp_and_git_suffix_as_same(monkeypatch):
    ...
    async def fake_run(cmd, **kw):
        ...
        if "get-url" in cmd:
            ...
            if remote == "codexcrew":
                return 0, "https://github.com/mikutellyourworld/Codex-Crew.git\n", ""
            return 0, "git@github.com:KiroDotDev/codexcrew\n", ""
        return 1, "", "unexpected"
...
async def test_load_fallback_repos_dedupes_multiple_aliases_of_one_repo(monkeypatch):
    ...
    async def fake_run(cmd, **kw):
        ...
        if "get-url" in cmd:
            ...
            if remote == "old-a":
                return 0, "git@github.com:someone/codexcrew-old.git\n", ""
            return 0, "https://github.com/someone/CodexCrew-Old\n", ""  # same repo, other spelling
        return 1, "", "unexpected"
...
async def test_load_fallback_repos_skips_same_named_fork(monkeypatch):
    ...
    async def fake_run(cmd, **kw):
        ...
        if "get-url" in cmd:
            ...
            return 0, "git@github.com:someone/CodexCrew.git\n", ""
        return 1, "", "unexpected"
...
async def test_load_fallback_repos_keeps_renamed_repo_alongside_a_fork(monkeypatch):
    ...
    async def fake_run(cmd, **kw):
        ...
        if "get-url" in cmd:
            ...
            if remote == "fork":
                return 0, "git@github.com:someone/CodexCrew.git\n", ""
            return 0, "git@github.com:kirodotdev/codexcrew-old.git\n", ""
        return 1, "", "unexpected"
    ...
    monkeypatch.setattr(runtime, "_run_cmd", fake_run)
    await repository._load_fallback_repos()
    assert repository._FALLBACK_REPOS == ["kirodotdev/codexcrew-old"]


def test_normalize_repo_identity_spellings_and_host():
    """Same repo across https/scp/.git spellings is one identity; host matters."""
    https = repository._normalize_repo_identity("https://github.com/mikutellyourworld/Codex-Crew.git")
    scp = repository._normalize_repo_identity("git@github.com:KiroDotDev/codexcrew")
    assert https == scp == ("github.com", "kirodotdev/codexcrew")
    # Same owner/repo on a different forge is a DISTINCT identity.
    other = repository._normalize_repo_identity("https://gitlab.com/kirodotdev/codexcrew.git")
    assert other == ("gitlab.com", "kirodotdev/codexcrew")
    assert other != https
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({}, "s3cr3t", "missing X-CodexCrew-Proxy header"),
        ({"X-CodexCrew-Proxy": "nocolon"}, "s3cr3t", "malformed X-CodexCrew-Proxy header"),
        ({"X-CodexCrew-Proxy": "abc:sig"}, "s3cr3t", "invalid timestamp in proxy header"),
        ({"X-CodexCrew-Proxy": "1:sig"}, "s3cr3t", "proxy signature expired"),
    ],
)
...
def test_gateway_unit_name_defaults_to_live_unit(monkeypatch, tmp_path):
    monkeypatch.setattr("codex_crew.config.loader.config_dir", lambda: tmp_path / "home")
    assert live._gateway_unit_name() == live._LIVE_GATEWAY_UNIT


def test_gateway_unit_name_uses_pod_instance(monkeypatch, tmp_path):
    home = tmp_path / ".codexcrew-pods" / "feat"
    monkeypatch.setattr("codex_crew.config.loader.config_dir", lambda: home)
    assert live._gateway_unit_name() == "codexcrew-pod@feat.service"
...
def test_gateway_label_uses_pod_agent(monkeypatch, tmp_path):
    launchd = pytest.importorskip("codex_crew.pod.launchd")
    pod_config = pytest.importorskip("codex_crew.pod.config")
    home = tmp_path / ".codexcrew-pods" / "feat"
    monkeypatch.setattr("codex_crew.config.loader.config_dir", lambda: home)
    monkeypatch.delenv("CODEXCREW_POD_UNIT_PREFIX", raising=False)
...
def test_gateway_label_honours_unit_prefix_override(monkeypatch, tmp_path):
    ...
    home = tmp_path / ".codexcrew-pods" / "feat"
    monkeypatch.setattr("codex_crew.config.loader.config_dir", lambda: home)
    monkeypatch.setenv("CODEXCREW_POD_UNIT_PREFIX", "altplane")
    assert live._gateway_label() == f"{launchd.LABEL_PREFIX}.altplane.feat"


@pytest.mark.parametrize(
    ("home_parts", "expected"),
    [((".codexcrew-pods", "feat"), True), (("home", "codexcrew"), False)],
)
def test_in_pod_detection(monkeypatch, tmp_path, home_parts, expected):
    monkeypatch.setattr(
        "codex_crew.config.loader.config_dir", lambda: tmp_path.joinpath(*home_parts)
    )
...
async def test_find_worktree_by_path_matches_known_worktree(monkeypatch, tmp_path):
    wanted = tmp_path / "codexcrew-wt-alpha"
    wanted.mkdir()
    monkeypatch.setattr(
        repository,
        "_discover_worktrees",
...
def test_main_boots_platform_before_serving(monkeypatch):
    ...
    monkeypatch.setattr(mod.CodexCrewConfig, "load", classmethod(lambda cls: SimpleNamespace()))
