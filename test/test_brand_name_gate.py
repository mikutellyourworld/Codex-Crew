...
class TestCaught:
    @pytest.mark.parametrize(
        "line",
        [
            "CodexCrew keeps working while you sleep.",
            "Run CodexCrew on your own hardware.",
            "This is CodexCrew's own sandbox.",
            "the process that launched CodexCrew.",
            "A desktop companion for CodexCrew that paces your day",
            "every CodexCrew-owned file",
            '"about_blurb": "A companion built into CodexCrew."',
            "# CodexCrew needs Python >= 3.10 at runtime",
        ],
    )
    def test_concatenated_prose_is_flagged(self, line: str) -> None:
        assert _hits(line), f"missed: {line}"

    @pytest.mark.parametrize("token", ["CodexCrew", "Kirocrew", "kiroCrew", "KiroCREW"])
    ...
    def test_uncapitalised_spacing_is_flagged(self) -> None:
        ...
        # is still wrong.
        assert _hits("install codex crew first") == ["codex crew"]

    def test_capitalised_spacing_is_accepted(self) -> None:
        assert _hits("Codex Crew keeps working.") == []
        assert _hits("Codex crew keeps working.") == []

    def test_multiple_hits_on_one_line_all_reported(self) -> None:
        assert _hits("CodexCrew talks to CodexCrew over SSH") == ["CodexCrew", "CodexCrew"]
...
class TestExempt:
    @pytest.mark.parametrize(
        ...
        [
            ...
            "run `codexcrew serve` to start the gateway",
            'os.environ["CODEXCREW_HOME"]',
            "from codex_crew.config import loader",
            "state lives under ~/.codex-crew/workspace",
            "mailto:kiro-crew-security-support@example.com",
            # Repo slug and URLs.
            "https://github.com/mikutellyourworld/Codex-Crew/issues",
            "git clone https://github.com/mikutellyourworld/Codex-Crew.git",
            "pushed to `mikutellyourworld/Codex-Crew` itself",
            # Path segments — each separator side alone, since a brand with a
            # separator on both sides is covered by either check.
            "built from ~/src/CodexCrew last night",
            "CodexCrew/website holds the frontend",
            r"installed to C:\Program Files\CodexCrew",
            r"launches CodexCrew\resources\app.asar",
            "artifacts/CodexCrew-notarized-stable/CodexCrew.dmg",
            # One identifier, not two words.
            "resolve CodexCrewApps from the registry",
            "the CodexCrewPublishCDK distribution stack",
            "SHIM = MyCodexCrewShim()",
            # Hyphenated identifier interior — an HTTP header, not prose.
            'assert "X-CodexCrew-Proxy" in headers',
            # Release artifacts.
            "electron-builder signs CodexCrew.exe and Update.exe",
            "publishes CodexCrew-x86_64.AppImage beside the deb",
            "opens /Applications/CodexCrew.app",
            "the CodexCrew-Nightly channel",
        ],
    )
    ...
    def test_sentence_end_is_not_read_as_a_file_extension(self) -> None:
        # `CodexCrew.exe` is exempt; `CodexCrew.` at the end of a sentence is not.
        assert _hits("shipped with CodexCrew.") == ["CodexCrew"]
        assert _hits("shipped with CodexCrew.exe") == []

    def test_hyphen_exemption_does_not_swallow_prose(self) -> None:
        # Only an *interior* hyphen segment is an identifier.
        assert _hits("a CodexCrew-specific workaround") == ["CodexCrew"]

    def test_line_suppression(self) -> None:
        assert _hits("correct = 'CodexCrew'  # brand-ok: transcript fixture") == []
    ...
    def test_channel_qualified_identifier(self) -> None:
        ...
        assert _hits('home / "Library" / "Logs" / "CodexCrew Nightly"') == []
        assert _hits("the CodexCrew Insider channel") == []
        # Any other following word is prose again.
        assert _hits("the CodexCrew dashboard") == ["CodexCrew"]
...
class TestUrlBoundary:
    ...
    def test_wrapped_link_still_exempts_the_slug(self) -> None:
        assert _hits("see [docs](https://github.com/mikutellyourworld/Codex-Crew/issues)") == []
        assert _hits("<https://github.com/mikutellyourworld/Codex-Crew>") == []

    def test_markup_closing_a_url_does_not_exempt_the_text_after_it(self) -> None:
        assert _hits('<a href="https://example.com/">CodexCrew</a>') == ["CodexCrew"]
        assert _hits("[CodexCrew](https://example.com/)") == ["CodexCrew"]

    def test_a_url_earlier_on_the_line_does_not_exempt_later_prose(self) -> None:
        assert _hits("https://example.com/x is where CodexCrew lives") == ["CodexCrew"]
    ...
    def test_a_very_long_line_stays_linear(self, filler: str, glue: str) -> None:
        ...
        line = filler * (200_000 // len(filler)) + glue + "CodexCrew is here"
        started = time.monotonic()
        found = _hits(line, path="big.md")
        assert time.monotonic() - started < 2.0
        assert found == ["CodexCrew"]
    ...
    def test_many_backticks_stay_linear(self) -> None:
        line = "`x`" * 30_000 + " CodexCrew"
        started = time.monotonic()
        assert _hits(line, path="big.md") == ["CodexCrew"]
        assert time.monotonic() - started < 2.0
...
class TestMarkdownCode:
    ...
    def test_tilde_fences_and_reopened_blocks(self) -> None:
        doc = ["a", "~~~", "cd CodexCrew", "~~~", "b", "```", "cd CodexCrew", "```"]
    ...
    def test_a_wider_fence_is_not_closed_by_a_narrower_run(self) -> None:
        ...
        doc = ["````markdown", "```bash", "cd CodexCrew", "```", "````", "Then run CodexCrew."]
    ...
    def test_a_closing_fence_may_not_carry_an_info_string(self) -> None:
        ...
        doc = ["````", "```python", "x = 1", "```", "````", "CodexCrew here"]
        assert gate.fenced_lines(doc) == {1, 2, 3, 4, 5}

    def test_inline_code_span_is_exempt_but_the_rest_of_the_line_is_not(self) -> None:
        assert _hits("clone `CodexCrew` then start CodexCrew.", path="doc.md") == ["CodexCrew"]
    ...
    def test_inline_code_exemption_is_markdown_only(self) -> None:
        ...
        assert _hits("s = 'see `CodexCrew` docs'", path="a.py") == ["CodexCrew"]
    ...
    def test_the_uncapitalised_rule_has_its_own_inline_code_exemption(self) -> None:
        ...
        assert _hits("run `codex crew` from a shell", path="doc.md") == []
        assert _hits("run codex crew from a shell", path="doc.md") == ["codex crew"]
...
class TestScope:
    def test_out_of_scope_paths(self) -> None:
        ...
        assert not gate.in_scope("src/codex_crew/_vendor/libggml-base.so.0")
    ...
    def test_generated_artifacts_are_reported_but_never_enforced(self) -> None:
        ...
        for path in (
            ...
            "src/codex_crew/data/tips_catalog.json",
        ):
...
class TestDiffScopedRun:
    ...
    def test_preexisting_violation_does_not_fail_the_diff_gate(self, tmp_path) -> None:
        ...
        self._commit(root, "doc.md", "Old line about CodexCrew.\n", "base")
        base = subprocess.run(
            ["git", "rev-parse", "HEAD"], cwd=root, capture_output=True, text=True, check=True
        ).stdout.strip()
        self._commit(root, "doc.md", "Old line about CodexCrew.\nA clean new line.\n", "head")
    ...
    def test_a_no_diff_gitattribute_cannot_hide_a_file(self, tmp_path) -> None:
        ...
        self._commit(root, "doc.md", "Run CodexCrew today.\n", "head")
    ...
    def test_a_non_ascii_path_cannot_hide_a_file(self, tmp_path) -> None:
        ...
        base = self._head(root)
        self._commit(root, "日本語.md", "Run CodexCrew today.\n", "head")
    ...
    def test_a_base_with_no_common_ancestor_still_enforces(self, tmp_path) -> None:
        ...
        self._commit(root, "doc.md", "Now with CodexCrew in it.\n", "orphan")
        assert (
            subprocess.run(
    ...
    def test_a_translated_catalog_cannot_fail_the_gate(self, tmp_path) -> None:
        ...
        self._commit(
            root,
            "website/src/i18n/locales/de.json",
            '{\n  "update": "CodexCrew wird aktualisiert"\n}\n',
            "head",
        )
...
class TestExplicitPathFailsClosed:
    ...
    def test_an_undecodable_path_fails_closed(self, tmp_path) -> None:
        ...
        blob.write_bytes(b"\xff\xfe\x00CodexCrew")
    ...
    def test_a_readable_clean_path_still_passes(self, tmp_path) -> None:
        ...
        doc.write_text("Prose about Codex Crew.\n", encoding="utf-8")
    ...
    def test_a_readable_dirty_path_still_fails_on_the_finding(self, tmp_path) -> None:
        ...
        doc.write_text("Prose about CodexCrew.\n", encoding="utf-8")
        result = self._run(str(doc))
        assert result.returncode == 1, result.stdout
...
class TestReportDisclosesTruncation:
    ...
    def test_a_truncated_report_says_so_and_tallies_by_path(self, capsys) -> None:
        violations = [
            gate.Violation(f"{top}/f{i}.md", 1, "CodexCrew", "CodexCrew")
    ...
    def test_an_untruncated_report_adds_no_tally_or_notice(self, capsys) -> None:
        violations = [gate.Violation("a.md", 1, "CodexCrew", "CodexCrew")]
        assert gate.report(violations, enforcing=False, base=None) == 0
        out = capsys.readouterr().out
    ...
    def test_the_enforcing_path_keeps_its_own_cap_and_notice(self, capsys) -> None:
        violations = [
            gate.Violation(f"f{i}.md", 1, "CodexCrew", "CodexCrew") for i in range(205)
        ]
    ...
    def test_a_root_level_path_is_tallied_without_a_trailing_slash(self, capsys) -> None:
        violations = [
            gate.Violation("install.sh" if i % 2 else "docs/a.md", 1, "CodexCrew", "CodexCrew")
            for i in range(50)
        ]
        assert gate.report(violations, enforcing=False, base=None) == 0
        out = capsys.readouterr().out
