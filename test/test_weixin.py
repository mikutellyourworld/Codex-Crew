...
from codex_crew.weixin.client import (
    ILINK_APP_ID,
    ITEM_TEXT,
    MSG_STATE_FINISH,
    MSG_TYPE_BOT,
...
from codex_crew.weixin.renderer import (
    WEIXIN_CHUNK_LIMIT,
    normalize_markdown,
    render_chunks,
...
def test_config_dm_policy_defaults_to_allowlist():
    ...
    from codex_crew.config.loader import WeixinConfig
...
def test_gateway_home_follows_the_configured_data_home(tmp_path, monkeypatch):
    ...
    A hardcoded expanduser() would send isolated profiles (CODEXCREW_HOME, the dev
    backend, tests) to the shared default home, where they would overwrite each
    other's reply state.
    """
    monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path / "isolated"))
    ...
    import codex_crew.weixin.gateway as gw
...
def test_a_mid_response_options_mention_never_deletes_the_body():
    ...
    from codex_crew.messaging.renderer import render_options_as_text
    from codex_crew.weixin.transport import WEIXIN_CAPABILITIES
...
def test_poll_loop_backs_off_on_ret_keyed_session_expiry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("codex_crew.weixin.transport.asyncio.sleep", fake_sleep)
    t._running = True
...
def test_receive_persists_the_context_off_the_event_loop(tmp_path, monkeypatch):
    ...
    import codex_crew.weixin.transport as mod
