...
Covers:
  * resolve_profile.py resolution order (config / codexcrew markers /
    auto-detect / generic) and the bundled CodexCrew profile contents.
...
SKILL_DIR = REPO_ROOT / "src" / "codex_crew" / "builtin_skills" / "codexcrew-dev" / "prepare-pr"
...
def test_codexcrew_markers_load_bundled_profile(tmp_path):
...
def test_opus_profile_model_matches_the_ci_workflow():
    ...
    data = json.loads((PROFILES_DIR / "codexcrew.json").read_text(encoding="utf-8"))
...
def test_ci_blocking_scans_are_covered_by_the_floor():
    ...
    assert not missing, (
        ...
        f"{missing}. Add them to profiles/codexcrew.json gates[] in their "
    ...
    assert not tool_missing, (
        ...
        "Add each to profiles/codexcrew.json gates[] in its CI-exact form, or "
...
def test_floor_typechecks_the_way_ci_does():
    ...
    """
    gates = "\n".join(
        json.loads((PROFILES_DIR / "codexcrew.json").read_text(encoding="utf-8"))["gates"]
    )
...
def test_bundled_codexcrew_profile_is_valid_json():
    data = json.loads((PROFILES_DIR / "codexcrew.json").read_text())
    assert data["name"] == "codexcrew"
...
def test_tree_reader_worktree_and_pinned_parity(tmp_path):
    ...
    assert resolve_profile.detect_codexcrew(wt_reader) is False
    assert resolve_profile.detect_codexcrew(pinned_reader) is False
