...
import codex_crew.dashboard.handlers.files as files_mod
from codex_crew.dashboard.handlers import api_project_git
...
@pytest.fixture()
def mock_sel():
    with patch("codex_crew.dashboard.handlers.sel") as m:
...
class TestProjectGitEndpoint:
    ...
    async def test_recent_projects_entry_is_accepted(self, repo, tmp_path, mock_sel):
        ...
        with patch("codex_crew.dashboard.handlers.files.config_dir", return_value=cfg):
...
class TestNoGitSubprocess:
    ...
    def test_symlinked_head_to_a_blocked_path_is_refused(self, repo, tmp_path):
        ...
        with patch(
            "codex_crew.dashboard.handlers.files.safe_read_prefix", return_value=None
        ) as gated:
            info = _project_git_branch(os.path.realpath(str(repo)))
...
class TestBranchRedaction:
    ...
    def test_branch_name_is_routed_through_redaction(self, repo):
        with patch(
            "codex_crew.dashboard.handlers.files.redact", side_effect=lambda t: f"<{t}>"
        ) as red:
            info = _project_git_branch(os.path.realpath(str(repo)))
        assert info["branch"] == "<trunk>"
    ...
    async def test_no_response_echoes_an_unredacted_path(self, repo, tmp_path, mock_sel):
        ...
        for known, url in cases:
            with patch(
                "codex_crew.dashboard.handlers.files.redact", return_value=marker
            ):
...
class TestResolveProjectGit:
    ...
    def test_sensitive_path_is_reported_without_running_git(self, repo):
        with (
            ...
            patch("codex_crew.dashboard.handlers.files._project_git_branch") as branch,
        ):
