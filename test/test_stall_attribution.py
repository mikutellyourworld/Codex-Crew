...
from codex_crew import cron_inflight, stall_attribution
from codex_crew.dashboard import crash_dump_store
from codex_crew.stall_attribution import (
    attribute_dump,
...
class TestSurface:
    ...
    def test_cron_wins_over_the_slack_gateway_module_it_passes_through(self) -> None:
        ...
        # Codex Crew frame is the cron run, and that is what names the surface.
    ...
    def test_windows_paths_classify_too(self) -> None:
        win = [r'  File "C:\Users\u\venv\Lib\site-packages\codex_crew\cron.py", line 1 in _execute']
    ...
    def test_rules_name_modules_and_functions_that_exist(self) -> None:
        ...
        for label, fragments, funcs in stall_attribution._SURFACE_RULES:
            for fragment in fragments:
                rel = fragment.removeprefix("/codex_crew/")
                assert (pkg / rel).exists(), f"{label}: no such module or package {fragment!r}"
            for func in funcs:
                assert func in defs, f"{label}: no function named {func!r} in codex_crew"
        for gate in stall_attribution._GATE_FILES:
            assert (pkg / gate.removeprefix("/codex_crew/")).is_file(), gate
...
class TestAttribution:
    ...
    def test_single_matching_marker_names_the_job(
        ...
        assert lines[2] == "recommended: codexcrew cron pause caeb441a"
    ...
    def test_latest_picks_the_newest_dump_with_stacks(
        ...
        header_only.write_text(
            "# CodexCrew loop-stall crash dump — opened test\n"  # brand-ok: mirrors the store's header bytes
    ...
    def test_describe_renders_disk_strings_inert(self, tmp_path: Path, dead_pids: set[int]) -> None:
        ...
        stack = [
            '  File "/opt/venv/lib/python3.12/site-packages/codex_crew/security.py\x07", '
