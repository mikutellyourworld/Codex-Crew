...
from codex_crew.platform.interfaces import InboundToken, SessionPrincipal

# ── Static-analysis configuration ──

_SRC_ROOT = Path(codex_crew.__file__).resolve().parent
...
def _core_source_files() -> List[Path]:
    """Every non-excluded ``.py`` file under ``src/codex_crew``."""
    return [
        path
...
def _peek_scanned_files() -> List[Path]:
    """Every ``.py`` under ``src/codex_crew`` except vendored third-party source."""
    return [
        path
        for path in sorted(_SRC_ROOT.rglob("*.py"))
...
def _module_key(path: Path) -> str:
    """``src/codex_crew/platform/context.py`` → ``platform/context.py``."""
...
class TestSeamCoverage:
    def test_every_context_field_is_wired_or_reserved(
        ...
        assert not unaccounted, (
            ...
            "call site, or add it to codex_crew.platform.context.RESERVED_SLOTS with "
...
class TestInstalledContextPeeks:
    ...
    def test_every_peek_is_declared(self, peek_sites) -> None:
        ...
        assert not undeclared, (
            ...
            "codex_crew.platform.context.PEEK_CALLERS stating why that holds for this "
...
class TestReservedSlotWarning:
    ...
    def test_default_context_warns_about_nothing(self, caplog) -> None:
        with caplog.at_level(logging.WARNING, logger="codex_crew.platform.context"):
            build_default_context(CodexCrewConfig())
    ...
    def test_subclass_of_default_adapter_still_warns(self, caplog) -> None:
        ...
        """
        from codex_crew.platform.defaults import DefaultPackageManager
    ...
    def test_warning_never_breaks_composition(self, monkeypatch, caplog) -> None:
        ...
        monkeypatch.setattr("codex_crew.platform.context._reserved_slot_is_default", _boom)
        base = build_default_context(CodexCrewConfig())
        with caplog.at_level(logging.WARNING, logger="codex_crew.platform.context"):
...
class TestAgentIdentitySeam:
    ...
    def test_agent_identity_known_posture_is_stored_on_ceiling(self) -> None:
        from codex_crew.platform.governance import agentcore_posture
    ...
    def test_agent_identity_profile_enabled_without_posture_parses(self) -> None:
        ...
        from codex_crew.platform.governance import CapabilityGate
    ...
    def test_agent_identity_policy_gateway_url_is_stored(self) -> None:
        from codex_crew.platform.governance import agentcore_gateway_url
    ...
    def test_agent_identity_seam_stays_off_when_capability_is_off(self) -> None:
        ...
        from codex_crew.agent import _agent_identity_enabled
        from codex_crew.platform.defaults import DefaultAgentIdentityProvider
