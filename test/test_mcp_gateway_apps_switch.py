...
What remains is ``CODEXCREW_MCP_APPS`` as an absolute kill switch. These tests pin
...
from codex_crew.mcp_gateway.backend import MCP_APPS_ENV_FLAG, _mcp_apps_enabled
...
def test_stored_config_key_is_honoured(monkeypatch, apps_enabled: bool) -> None:
    ...
    real = loader.CodexCrewConfig.load()
    monkeypatch.setattr(real.mcp_gateway, "apps_enabled", apps_enabled, raising=False)
    monkeypatch.setattr(loader.CodexCrewConfig, "load", staticmethod(lambda: real))
...
def test_unreadable_config_fails_closed(monkeypatch) -> None:
    ...
    monkeypatch.setattr(loader.CodexCrewConfig, "load", staticmethod(_boom))
