...
def _make_cfg(
    ...
    """Build a mock CodexCrewConfig with dashboard display fields.
...
async def test_theme_boot_returns_defaults() -> None:
    ...
    cfg = _make_cfg()
    with patch.object(core_mod, "CodexCrewConfig") as mock_cls:
...
async def test_theme_config_put_serializes_full_load_modify_save_transaction() -> None:
    ...
    with patch.object(core_mod.CodexCrewConfig, "load", side_effect=Config):
