...
class TestSaveLoadRoundTrip:
    ...
    def _load(tmp_path: Path, webex: dict) -> Any:
        ...
        old = os.environ.get("CODEXCREW_HOME")
        os.environ["CODEXCREW_HOME"] = str(tmp_path)
        try:
            return CodexCrewConfig.load().webex
        finally:
            if old is None:
                os.environ.pop("CODEXCREW_HOME", None)
            else:
                os.environ["CODEXCREW_HOME"] = old
    ...
    def test_the_loader_reads_every_field_the_dataclass_declares(self, tmp_path: Path) -> None:
        ...
        from codex_crew.config.loader import WebexConfig
