...
def home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect CODEXCREW_HOME so the keystone file writes land in a tmp dir."""
...
def _a_builtin_id() -> str:
    ...
    from codex_crew.security import builtin_denied_rules, floor_enforced_builtin_command_ids
...
def _a_floor_id() -> str:
    ...
    from codex_crew.security import floor_enforced_builtin_command_ids
...
def test_pin_forces_enabled_under_disable_all(home: Path, config_file: Path):
    rid = _a_builtin_id()
    _seed(config_file, {"disable_all": True})
    with patch("codex_crew.security.pinned_builtin_command_ids", return_value={rid}):
        snap = build_denied_commands_snapshot()
...
async def test_builtin_toggle_disable_pinned_is_409(home: Path, mock_sel):
    rid = _a_builtin_id()
    with patch(
        "codex_crew.security.pinned_builtin_command_ids",
        return_value={rid},
    ):
        async with _client() as client:
            resp = await client.patch(
...
async def test_user_add_redos_pattern_is_400(home: Path, mock_sel):
    ...
    from codex_crew.security import _DANGEROUS_AWS_FLAG_RUN, _LINEARIZED_AWS_FLAG_RUN
...
async def test_api_security_stats_uses_effective_count(home: Path, config_file: Path):
    from codex_crew.dashboard.handlers.core import api_security_stats
...
async def _run_write_denied_state(home: Path, config_file: Path, mutate):
    ...
    """
    from codex_crew.dashboard.handlers.security import _write_denied_state
...
def test_write_denied_state_does_not_fall_back_to_chmod_safe(
    ...
    """
    import codex_crew.atomic_write as atomic_write_mod
