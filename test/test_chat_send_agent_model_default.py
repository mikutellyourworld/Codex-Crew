...
from codex_crew.acp.types import EVENT_TEXT_CHUNK
from codex_crew.config.loader import CodexCrewConfig
from codex_crew.dashboard import chat_runner
from codex_crew.dashboard.chat_runner import _eager_spawn
...
def _load_config(tmp_path: Path, data: dict) -> CodexCrewConfig:
    """A real ``CodexCrewConfig`` loaded from *data* (same shape as config.json)."""
    cfg_file = tmp_path / "config.json"
    cfg_file.write_text(json.dumps(data))
    with unittest.mock.patch("codex_crew.config.loader.config_path", return_value=cfg_file):
        return CodexCrewConfig.load()


def _config(tmp_path: Path, *, crew_model: str = "") -> CodexCrewConfig:
    """Global ``agent.model`` set; the crews pin nothing unless *crew_model*."""
    return _load_config(
        tmp_path,
        {
            "agent": {"model": GLOBAL_DEFAULT, "provider": "acp"},
            "agents": {"researcher": {"kiro_agent": "codexcrew", "model": crew_model}},
...
def _runner_config(tmp_path):
    """Serve the real config object to every ``CodexCrewConfig.load()`` in the turn."""

    def _install(cfg: CodexCrewConfig):
        patcher = unittest.mock.patch.object(
            chat_runner.CodexCrewConfig, "load", unittest.mock.MagicMock(return_value=cfg)
        )
        patcher.start()
        return patcher
    ...
    def _use(cfg: CodexCrewConfig) -> None:
...
class TestRunChatDefaultModel:
    ...
    async def test_every_tier_deferring_leaves_the_backend_to_choose(
        ...
        cfg = _load_config(
            tmp_path,
            {"agent": {"model": "auto", "provider": "acp"}, "default_agent": "codexcrew"},
        )
        _runner_config(cfg)
        state, _client = _turn_state(tmp_path)
        slot = _slot()
        ...
        with unittest.mock.patch.object(
            CodexCrewConfig, "_resolve_agent_model", staticmethod(lambda: "")
        ):
            await _drive(state, slot)
