...
A cron created outside a chat -- ``codexcrew cron add``, the dashboard Schedule
...
class TestAdoptTargetVisibility:
    ...
    def _run_adopt_with_known(self, known):
        with (
            patch("codex_crew.cli_commands.CronService") as mock_svc_cls,
            patch("codex_crew.cli_commands.sel"),
            patch("codex_crew.cli_commands.ConversationLog") as mock_log_cls,
        ):
            mock_svc_cls.return_value.adopt_job.return_value = True
    ...
    def test_a_failing_lookup_stays_quiet(self, capsys):
        ...
        with (
            patch("codex_crew.cli_commands.CronService") as mock_svc_cls,
            patch("codex_crew.cli_commands.sel"),
            patch("codex_crew.cli_commands.ConversationLog", side_effect=OSError("no dir")),
        ):
            mock_svc_cls.return_value.adopt_job.return_value = True
...
class TestAdoptIsDeniedFromBash:
    ...
    @pytest.mark.parametrize(
        "command",
        [
            "codexcrew cron adopt abc123 --session-of chat-3",
            "codexcrew cron adopt abc123 --release",
            "kiro-crew cron adopt abc123 --session-of dashboard:chat-3",
            "kiro.crew cron adopt abc123 --session-of dashboard:chat-3",
            "cd /tmp && codexcrew cron  adopt abc123 --session-of chat-3",
            "CODEXCREW_HOME=/tmp codexcrew cron adopt abc123 --session-of chat-3",
            # The module spelling, which is how the CLI is invoked from a venv.
            # `kiro.?crew` covers it because `.` matches the underscore.
            "python3 -m codex_crew cron adopt abc123 --release",
            ...
            # subparsers) -- verified by running `codexcrew -v cron list`, which
            # executes normally.
            "codexcrew -v cron adopt abc123 --session-of chat-3",
            "codexcrew -vv cron adopt abc123 --session-of chat-3",
            "codexcrew --verbose cron adopt abc123 --session-of chat-3",
            "codexcrew --no-jail cron adopt abc123 --session-of chat-3",
            "codexcrew -v --no-jail cron adopt abc123 --session-of chat-3",
            ...
            "codexcrew >/dev/null cron adopt abc123 --release",
            "codexcrew 2>/dev/null cron adopt abc123 --release",
            "codexcrew >/dev/null 2>&1 cron adopt abc123 --release",
            "codexcrew${IFS}cron${IFS}adopt abc123 --release",
            "codexcrew\tcron\tadopt abc123 --release",
            "codexcrew cron >/dev/null adopt abc123 --release",
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # combine with an earlier harmless `codexcrew cron` read.
            "codexcrew cron list; echo adopt",
            "codexcrew cron list && git adopt-nothing",
            "codexcrew cron list | grep adopt",
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            "codexcrew cron list",
            "codexcrew cron add nightly 'sweep' --every 3600",
            "codexcrew cron remove abc123",
