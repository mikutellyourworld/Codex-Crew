...
The `codexcrew-partial-` root exists so released versions refuse a selective bundle.
...
import pytest
from test_snapshot import _setup_fake_codexcrew, unpinnable_argv

from codex_crew import snapshot as snap
...
def home(tmp_path, monkeypatch):
    d = tmp_path / "home"
    d.mkdir()
    monkeypatch.setenv("CODEXCREW_HOME", str(d))
    _setup_fake_codexcrew(d)
    return d
...
class TestAPartialRootWithoutAComponentMapIsRefused:
    def test_it_refuses_rather_than_restoring_everything(self, home, tmp_path, capsys):
        bundle = _archive(tmp_path, {"codexcrew-partial-20260101T000000Z": None})
...
class TestANamedComponentTheBundleDoesNotCarryIsRefused:
    ...
    def test_it_refuses_instead_of_reporting_success(self, home, tmp_path, capsys):
        bundle = _archive_without_memory(tmp_path, "codexcrew-partial-20260101T000000Z")
        rc = snap.restore_main(
    ...
    def test_a_declared_map_still_decides_when_there_is_one(self, home, tmp_path):
        ...
        bundle = _archive(
            tmp_path,
            {
                "codexcrew-partial-20260101T000000Z": {
                    "version": 3,
...
def _extract_root(bundle, tmp_path):
    ...
    def test_a_complete_root_without_a_map_is_still_accepted(self, home, tmp_path):
        ...
        bundle = _archive(tmp_path, {"codexcrew-snapshot-20260101T000000Z": None})
        assert snap.restore_main([str(bundle), "--mode", "replace", "--force"]) == 0
...
class TestTwoRootsAreRefusedRatherThanGuessed:
    def test_more_than_one_root_refuses(self, home, tmp_path, capsys):
        bundle = _archive(
            tmp_path,
            {
                "codexcrew-snapshot-20260101T000000Z": {"version": 3, "components": {}},
                "codexcrew-partial-20260101T000001Z": {"version": 3, "components": {}},
            },
        )
        rc = snap.restore_main([str(bundle), "--mode", "replace", "--force"])
        assert rc == 1
    ...
    def test_the_refusal_names_both_roots(self, home, tmp_path, capsys):
        ...
        out = capsys.readouterr().out
        assert "codexcrew-snapshot-20260101T000000Z" in out
        assert "codexcrew-partial-20260101T000001Z" in out
...
class TestTextIOPinsUtf8:
    ...
    def test_no_text_io_in_the_backup_path_omits_an_encoding(self):
        ...
        import codex_crew.snapshot as sn
        import codex_crew.snapshot_redact as sr
        from codex_crew.apps.builtins.aws_control.backend import backup as app_backup
