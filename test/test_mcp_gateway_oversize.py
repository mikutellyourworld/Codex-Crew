...
from codex_crew.mcp_gateway.spill import (
...
class TestSpillToFile:
    def test_tool_result_spilled(self, tmp_path):
        ...
        assert "[CodexCrew: response truncated" in text
...
class TestSpillFailure:
    def test_unwritable_dir_returns_original(self, tmp_path):
        ...
        with patch.dict(os.environ, {"CODEXCREW_HOME": fake_home}):
...
class TestConfigOverrides:
    def test_read_limit_env_override(self):
        """CODEXCREW_MCP_READ_LIMIT env var overrides the default."""
        from codex_crew.mcp_gateway import pool
        with patch.dict(os.environ, {"CODEXCREW_MCP_READ_LIMIT": "8388608"}):
            val = pool._resolve_read_buffer_limit()
        assert val == 8388608

    def test_read_limit_env_invalid_uses_default(self):
        """Invalid CODEXCREW_MCP_READ_LIMIT falls back to default."""
        from codex_crew.mcp_gateway import pool
        with patch.dict(os.environ, {"CODEXCREW_MCP_READ_LIMIT": "not_a_number"}):
            val = pool._resolve_read_buffer_limit()
        assert val == pool._DEFAULT_READ_BUFFER_LIMIT

    def test_read_limit_env_too_small_uses_default(self):
        """CODEXCREW_MCP_READ_LIMIT below 1024 falls back to default."""
        from codex_crew.mcp_gateway import pool
        with patch.dict(os.environ, {"CODEXCREW_MCP_READ_LIMIT": "100"}):
            val = pool._resolve_read_buffer_limit()
        assert val == pool._DEFAULT_READ_BUFFER_LIMIT

    def test_spill_threshold_env_override(self):
        """CODEXCREW_MCP_SPILL_THRESHOLD env var overrides the default."""
        from codex_crew.mcp_gateway import pool
        with patch.dict(os.environ, {"CODEXCREW_MCP_SPILL_THRESHOLD": "1048576"}):
            val = pool._resolve_spill_threshold()
        assert val == 1048576

    def test_spill_threshold_zero_disables(self):
        """CODEXCREW_MCP_SPILL_THRESHOLD=0 disables spilling."""
        from codex_crew.mcp_gateway import pool
        with patch.dict(os.environ, {"CODEXCREW_MCP_SPILL_THRESHOLD": "0"}):
            val = pool._resolve_spill_threshold()
        assert val == 0
    ...
    def test_read_buffer_limit_reads_config_when_env_unset(self, monkeypatch):
        import codex_crew.mcp_gateway.pool as pool

        monkeypatch.delenv("CODEXCREW_MCP_READ_LIMIT", raising=False)
        with patch(
    ...
    def test_spill_threshold_reads_config_when_env_unset(self, monkeypatch):
        ...
        monkeypatch.delenv("CODEXCREW_MCP_SPILL_THRESHOLD", raising=False)
        with patch(
            "codex_crew.config.loader._raw_config",
    ...
    def test_env_var_still_beats_config(self, monkeypatch):
        ...
        monkeypatch.setenv("CODEXCREW_MCP_READ_LIMIT", "2048")
        with patch(
            "codex_crew.config.loader._raw_config",
