...
from codex_crew.knowledge.llm_pool import (
...
def _config_dir_tracks_patched_home(monkeypatch):
    """Keep ``llm_pool.config_dir()`` pointed at ``<patched home>/.codexcrew``.

    The data home moved from ``~/.codexcrew`` to ``~/.codex-crew`` (``config_dir()``),
    and ``_read_config`` now reads ``config_dir()/config.json`` rather than
    ``Path.home()/".codexcrew"/"config.json"``. These tests patch
    ``llm_pool.Path.home`` per-test and write ``config.json`` under
    ``<home>/.codexcrew`` — but ``config_dir()`` reads ``CODEXCREW_HOME`` (pinned to
    a *different* tmp dir by the conftest ``_isolate_codexcrew_home`` fixture), so
    without this redirect the config would never be found. Redirect
    ``config_dir`` to ``Path.home()/".codexcrew"`` (evaluated lazily, so it tracks
    whatever ``Path.home()`` each test patches), preserving the existing
    ``.codexcrew/config.json`` layout the tests build.
    """
    monkeypatch.setattr(
        "codex_crew.knowledge.llm_pool.config_dir", lambda: Path.home() / ".codexcrew"
    )
...
class TestProviderDetection:
    ...
    def test_reads_claude_code_from_config(self, tmp_path):
        config = tmp_path / ".codexcrew" / "config.json"
...
class TestReadConfig:
    ...
    async def test_start_passes_configured_sandbox_to_client(self, tmp_path):
        ...
        with patch("pathlib.Path.home", return_value=tmp_path), \
             patch("codex_crew.knowledge.llm_pool.AcpClient", return_value=mock_client) as mk:
            worker = AcpWorker()
            await worker.start()
...
class TestAcpWorker:
    ...
    async def test_start_shuts_down_stale_client_before_respawn(self, tmp_path):
        ...
        fresh.is_ready = True
        with patch("pathlib.Path.home", return_value=tmp_path), \
             patch("codex_crew.knowledge.llm_pool.AcpClient", return_value=fresh):
            worker = AcpWorker()
    ...
    async def test_start_registers_pid_shutdown_unregisters(self, tmp_path):
        ...
        with patch("pathlib.Path.home", return_value=tmp_path), \
             patch("codex_crew.knowledge.llm_pool.AcpClient", return_value=fresh), \
             patch("codex_crew.knowledge.llm_pool.register_protected_pid",
                   side_effect=registered.append), \
             patch("codex_crew.knowledge.llm_pool.unregister_protected_pid",
    ...
    async def test_respawn_reshields_new_pid(self, tmp_path):
        ...
        unregistered: list[int] = []
        with patch("pathlib.Path.home", return_value=tmp_path), \
             patch("codex_crew.knowledge.llm_pool.AcpClient", side_effect=[first, second]), \
             patch("codex_crew.knowledge.llm_pool.register_protected_pid",
                   side_effect=registered.append), \
             patch("codex_crew.knowledge.llm_pool.unregister_protected_pid",
                   side_effect=unregistered.append):
...
class TestAcpWorkerEffort:
    ...
    async def test_applies_kiro_effort_after_ready_before_use(self, tmp_path):
        ...
        with patch("pathlib.Path.home", return_value=tmp_path), \
             patch("codex_crew.knowledge.llm_pool.AcpClient", return_value=client):
...
class TestLLMPoolEffort:
    ...
    async def test_passes_effort_to_acp_worker(self):
        ...
        with patch("codex_crew.knowledge.llm_pool.AcpWorker", return_value=fake_worker) as worker_type:
    ...
    async def test_fetch_sized_pool_ignores_extraction_size_config(self):
        ...
        pool._create_worker = _mock_create  # type: ignore[assignment]
        with patch(
            "codex_crew.knowledge.llm_pool._read_config",
...
class TestCCWorker:
    ...
    async def test_start_raises_without_claude(self):
        with patch("codex_crew.knowledge.llm_pool.shutil.which", return_value=None):
            worker = CCWorker()
...
class TestWorkerConversationRecycle:
    ...
    async def test_acp_worker_reset_respawns_the_client(self):
        ...
        with patch("codex_crew.knowledge.llm_pool.AcpClient", return_value=new_client), \
             patch("codex_crew.knowledge.llm_pool.register_protected_pid"), \
             patch("codex_crew.knowledge.llm_pool.unregister_protected_pid"):
...
class TestCCWorkerReapsTheProcessTree:
    ...
    async def test_spawn_requests_its_own_session(self):
        ...
        with patch("codex_crew.knowledge.llm_pool.wrap_argv", lambda cmd, **k: (cmd, None)), \
             patch("codex_crew.knowledge.llm_pool.cgroup_scope_argv", lambda argv: argv), \
             patch(
                 "codex_crew.knowledge.llm_pool.create_subprocess_limited",
                 _fake_spawn,
             ):
            await worker._spawn()
