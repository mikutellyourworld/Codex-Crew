...
class TestSpawnRunSessionKeyRouting:
    def test_uses_env_var_when_set(self):
        """CODEXCREW_SESSION_KEY env var is used as parent_session."""
        with patch("codex_crew.mcp_core._post") as mock_post, patch.dict(
            "os.environ", {"CODEXCREW_SESSION_KEY": "sess-from-env"}
        ):
    ...
    def test_falls_back_to_pid_file(self, tmp_path):
        ...
        with patch("codex_crew.mcp_core._post") as mock_post, patch(
            "pathlib.Path.home", return_value=tmp_path / "fake_home"
        ):
            env = os.environ.copy()
            env.pop("CODEXCREW_SESSION_KEY", None)
            env.pop("CODEXCREW_HOME", None)  # ensure config_dir() uses patched Path.home()
            with patch.dict("os.environ", env, clear=True):
                # The gateway writes session_pid files into the data home
                # (config_dir() -> ~/.codex-crew), which is where the fallback
                # reader looks; there is no legacy fallback for these per-boot
                # runtime files.
                codexcrew_dir = tmp_path / "fake_home" / ".kiro" / "crew"
                codexcrew_dir.mkdir(parents=True)
                (codexcrew_dir / f"session_pid_{os.getppid()}.txt").write_text("sess-from-pid")
...
class TestSendMessageUnfurlForwarding:
    def test_unfurl_params_forwarded_in_payload(self):
        ...
        with patch("codex_crew.mcp_core._post") as mock_post:
...
class TestSendMessageCronSession:
    ...
    def _permit_messaging(self, monkeypatch):
        """Stub governance vets so a real ~/.codexcrew/profiles/cron.json that
        disables messaging doesn't block these payload-routing tests."""
        monkeypatch.setattr("codex_crew.mcp_core._vet_messaging_governance", lambda _sk: None)
        monkeypatch.setattr("codex_crew.mcp_core._vet_channel_governance", lambda _sk, _t: None)
    ...
    def test_default_notification_only(self):
        ...
        """
        with patch("codex_crew.mcp_core._post") as mock_post, patch.dict(
            "os.environ", {"CODEXCREW_SESSION_KEY": "dashboard:chat-1"}
        ):
            mock_post.return_value = {"ok": True}
    ...
    def test_cron_bare_send_attaches_caller_session(self):
        ...
        with patch("codex_crew.mcp_core._post") as mock_post, patch.dict(
            "os.environ", {"CODEXCREW_SESSION_KEY": "cron:abc123"}
        ):
...
class TestKnowledgeSearchCache:
    ...
    def _reset(self):
        import codex_crew.mcp_core as mc

        mc._KNOWLEDGE_CACHE = None

    def test_reuses_store_when_db_unchanged(self, tmp_path):
        from codex_crew.mcp_core import _get_knowledge_search
...
class TestSessionKeyHeaderError:
    ...
    def test_ascii_key_is_header_safe(self):
        from codex_crew.mcp_core import _session_key_header_error
...
class TestDeployArtifactMCPTool:
    ...
    def test_deploy_artifact_blocked_by_scan(self):
        ...
        with patch("codex_crew.mcp_core._post") as mock_post, \
                patch("codex_crew.deploy.pending.add_pending") as mock_pending:
