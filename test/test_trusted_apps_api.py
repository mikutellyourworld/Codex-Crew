...
from codex_crew.dashboard.handlers import security
from codex_crew.dashboard.handlers.security import (
...
# max_subagents). Every distinctive value below is one ``CodexCrewConfig.load()``
...
def home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect CODEXCREW_HOME so config.json + installed apps land in tmp."""
    h = tmp_path / "codexcrew-home"
    h.mkdir()
    monkeypatch.setenv("CODEXCREW_HOME", str(h))
    return h
...
def mock_sel():
    ...
    with patch("codex_crew.dashboard.handlers.security._sel") as m:
        instance = MagicMock()
...
def _install(
    ...
    *,
    ...
    """Install a minimal app record under the tmp CODEXCREW_HOME.
...
async def test_revoke_tears_down_ineffective_legacy_repository_grant(
    ...
    import codex_crew.apps.teardown as appteardown
...
async def test_mutation_on_corrupt_config_409s_without_clobbering(
    ...
    # REGRESSION: ``CodexCrewConfig.load()`` degrades an unparseable config.json to
...
async def test_revoke_stops_the_backend_process_and_the_beacon(
    ...
    from codex_crew.apps import backend as appbackend
...
async def test_revoke_tears_down_even_when_metadata_says_disabled(
    ...
    # pure metadata write, and `codexcrew app disable` calls it from a DIFFERENT
...
async def test_blanket_off_sweeps_an_app_that_calls_itself_builtin(
    ...
    import codex_crew.apps.teardown as appteardown
    from codex_crew.apps.manager import _read_installed, _write_installed
...
async def test_teardown_deregisters_even_when_the_app_calls_itself_self_managed(
    ...
    meta.lifecycle = "app"
    _write_installed(_APP, meta)
    from codex_crew.apps.manager import INSTALLED_META_FILENAME, app_dir
...
async def test_a_disabled_apps_own_code_is_not_launched_by_an_ordinary_disable(
    ...
    # helper survives `codexcrew app disable` (metadata-only, cross-process) with no
...
async def test_a_revoke_always_runs_the_shutdown_code_even_when_disabled(
    ...
    # reopened a hole on the REVOKE path. `codexcrew app disable` is metadata-only and
...
async def test_grant_is_withdrawn_if_the_app_is_uninstalled_mid_write(
    ...
    # handler holds `app_lifecycle_lock(name)`, but `codexcrew app uninstall` runs in a
    ...
    from codex_crew.apps import manager as appmanager
...
async def test_revoke_of_a_builtin_name_leaves_the_builtin_enabled(
    ...
    from codex_crew.apps.execution import builtin_app_names
...
async def test_refused_builtin_grant_is_not_inherited_by_a_later_takeover(
    ...
    from codex_crew.apps.execution import app_execution_denied, builtin_app_names
...
async def test_grant_accepts_a_registry_name_that_is_not_installed(
    ...
    from codex_crew.apps.manager import get_app
...
async def test_snapshot_reports_unenforceable_entries_as_ineffective(
    ...
    from codex_crew.apps.execution import app_execution_denied, trusted_app_names
...
async def test_mutations_refuse_when_config_local_owns_the_trust_setting(
    ...
    # REGRESSION: CodexCrewConfig deep-merges config.local.json OVER config.json and
    ...
    # is user-owned and Codex Crew never writes it.
...
async def test_refused_revoke_does_not_switch_the_app_off(
    ...
    _install(tmp_path, _APP, enabled=True)
    with patch(
        "codex_crew.dashboard.handlers.security.teardown_app_runtime"
    ) as teardown:
        async with _client() as client:
            resp = await client.delete(f"/api/security/trusted-apps/{_APP}")
            assert resp.status == 409
...
def test_uninstall_holds_the_shared_config_lock_across_the_grant_drop(
    ...
    # winning a race.
    from codex_crew.apps import routes as approutes
...
def test_uninstall_withdraws_a_grant_that_landed_during_the_delete(
    ...
    # The cross-process race GPT named: `codexcrew app uninstall` drops the (absent)
...
def test_the_cli_trust_withdrawal_is_audited(home: Path, tmp_path: Path):
    ...
    # from `codexcrew app uninstall`, so without this the grant could disappear with
...
def test_uninstall_writes_no_config_at_all_when_there_is_no_grant(
    ...
    # `codexcrew config set`, whose own reader documents the shape), so every writer
...
async def test_grant_preserves_base_settings_shadowed_by_the_overlay(
    ...
    # The mutation used to run CodexCrewConfig.load() -> cfg.save(), and save()
    ...
    _install(tmp_path, _APP, enabled=False)
    # Migration-complete on purpose: `CodexCrewConfig.load()` write-BACKS a legacy
...
async def test_blanket_off_runs_shutdown_hooks_before_persisting_false(
    ...
    async def _on_app_disable(name: str, record: dict, **_kw: object) -> dict:
        ...
        from codex_crew.apps.execution import third_party_execution_allowed
...
def test_blanket_flag_is_not_editable_through_the_generic_config_patch():
    ...
    # /api/config/codexcrew, and that path performs NO teardown — so flipping it off
    ...
    from codex_crew.dashboard.handlers.core import (
        _EDITABLE_CONFIG,
...
async def test_grant_holds_the_app_lifecycle_lock_across_validate_and_write(
    ...
    from codex_crew.apps import manager as appmanager
    from codex_crew.dashboard.handlers import security as sec
