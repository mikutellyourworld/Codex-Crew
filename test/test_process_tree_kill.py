...
class TestResetProcessTreeKill:
    ...
    async def test_reset_killpg_on_surviving_process(self, cfg):
        ...
        with (
            patch("codex_crew.session.os.kill") as mock_kill,
            patch("codex_crew.session.os.killpg") as mock_killpg,
            patch("codex_crew.session.os.getpgid", return_value=12345),
    ...
    async def test_reset_fallback_kill_when_killpg_fails(self, cfg):
        ...
        with (
            patch("codex_crew.session.os.kill") as mock_kill,
            patch("codex_crew.session.os.killpg", side_effect=OSError),
            patch("codex_crew.session.os.getpgid", return_value=12345),
            patch("codex_crew.acp.client._get_child_pids", return_value=[]),
    ...
    async def test_reset_merges_fresh_child_scan(self, cfg):
        ...
        with (
            patch("codex_crew.session.os.kill", side_effect=ProcessLookupError),
            patch("codex_crew.acp.client._get_child_pids", return_value=[12347, 12348]),
            patch("codex_crew.acp.client._get_start_time", return_value=999),
    ...
    async def test_reset_skips_kill_for_non_int_pid(self, cfg):
        ...
        with patch("codex_crew.session.os.kill") as mock_kill:
            await mgr.reset("t1")
...
class TestSigkillSessionProcessTree:
    ...
    async def test_sigkill_uses_killpg(self):
        ...
        with (
            patch("codex_crew.subagent.os.killpg") as mock_killpg,
            patch("codex_crew.subagent.os.getpgid", return_value=54321),
            patch("codex_crew.acp.client._get_child_pids", return_value=[]),
            patch("codex_crew.acp.client._kill_escaped_children") as mock_sweep,
    ...
    async def test_sigkill_fallback_on_killpg_failure(self):
        ...
        with (
            patch("codex_crew.subagent.os.killpg", side_effect=ProcessLookupError),
            patch("codex_crew.subagent.os.kill") as mock_kill,
            patch("codex_crew.subagent.os.getpgid", return_value=54321),
            patch("codex_crew.acp.client._get_child_pids", return_value=[]),
            patch("codex_crew.acp.client._kill_escaped_children"),
            patch("codex_crew.acp.client._get_start_time", return_value=100),
    ...
    async def test_sigkill_merges_child_pids(self):
        ...
        with (
            patch("codex_crew.subagent.os.killpg"),
            patch("codex_crew.subagent.os.getpgid", return_value=54321),
            patch("codex_crew.acp.client._get_child_pids", return_value=[54323]),
            patch("codex_crew.acp.client._get_start_time", return_value=200),
            patch("codex_crew.acp.client._read_basename", return_value=b"node"),
            patch("codex_crew.acp.client._is_our_child", return_value=True),
            patch("codex_crew.acp.client._kill_escaped_children") as mock_sweep,
        ):
    ...
    async def test_sigkill_noop_when_no_session(self):
        ...
        with patch("codex_crew.subagent.os.killpg") as mock_killpg:
