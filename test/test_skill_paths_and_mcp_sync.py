...
class TestApiSessionsRestartMcpSync:
    ...
    async def test_syncs_new_servers_before_restart(self):
        ...
        from codex_crew.dashboard.handlers.sessions import api_sessions_restart

        fake_server = MagicMock()
        request = _make_restart_request()

        with (
            patch("codex_crew.dashboard.handlers.sessions._reset_all_sessions", new_callable=AsyncMock, return_value=2),
            patch("codex_crew.dashboard.handlers.sessions.sync_discovered_servers", return_value=[fake_server]),
        ):
    ...
    async def test_sync_failure_does_not_block_restart(self):
        ...
        with (
            patch("codex_crew.dashboard.handlers.sessions._reset_all_sessions", new_callable=AsyncMock, return_value=1) as reset,
            patch("codex_crew.dashboard.handlers.sessions.sync_discovered_servers", side_effect=RuntimeError("boom")),
        ):
            resp = await api_sessions_restart(request)
    ...
    async def test_no_servers_to_sync(self):
        ...
        with (
            patch("codex_crew.dashboard.handlers.sessions._reset_all_sessions", new_callable=AsyncMock, return_value=0),
            patch("codex_crew.dashboard.handlers.sessions.sync_discovered_servers", return_value=[]) as mock_sync,
        ):
            resp = await api_sessions_restart(request)
    ...
    async def test_multiple_servers_synced(self):
        ...
        with (
            patch("codex_crew.dashboard.handlers.sessions._reset_all_sessions", new_callable=AsyncMock, return_value=1),
            patch("codex_crew.dashboard.handlers.sessions.sync_discovered_servers", return_value=servers),
        ):
            resp = await api_sessions_restart(request)
