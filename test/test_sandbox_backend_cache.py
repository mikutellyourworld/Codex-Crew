...
def clean_backend(monkeypatch):
    ...
    monkeypatch.setattr(
        sb, "_KIRO_INTERNAL_SETTINGS_PATH", "/nonexistent/codexcrew-test/amazon-internal.json"
    )
    sb.reset_backend()
...
class TestNoBackendGuidanceNamesTheRightRemedy:
    """Which remedy is named depends on HOW Codex Crew was launched."""
    ...
    def test_appimage_launch_names_the_attach_command_with_the_path(self, monkeypatch):
        ...
        monkeypatch.setenv("APPIMAGE", "/home/user/Apps/codexcrew.AppImage")

        msg = sb._no_backend_guidance()

        assert "codexcrew sandbox install-profile" in msg
        assert "/home/user/Apps/codexcrew.AppImage" in msg
    ...
    def test_non_appimage_launch_points_at_the_service(self, monkeypatch):
        ...
        assert "codexcrew service install" in msg
    ...
    @pytest.mark.parametrize("appimage", ["/home/user/Apps/codexcrew.AppImage", None])
    ...
    @pytest.mark.parametrize(
        "hostile",
        [
            "/home/user/CodexCrew-$(touch PWNED).AppImage",
            "/home/user/`id`.AppImage",
            "/home/user/a;rm -rf ~/b.AppImage",
            "/home/user/Bob's Apps/codexcrew.AppImage",
            "/home/user/a b/codexcrew.AppImage",
        ],
    )
    ...
    def test_the_remedy_names_a_cli_the_appimage_user_actually_has(
        ...
        assert "while Codex Crew is open" in msg, "must say the path is live-only"
    ...
    def test_which_is_not_trusted_as_evidence_the_user_has_the_cli(
        ...
        bundled.write_text("#!/bin/sh\n")
        monkeypatch.setattr(sb.sys, "argv", [str(bundled)])
        monkeypatch.setattr(sb.shutil, "which", lambda _n: "/usr/local/bin/codexcrew")
    ...
    def test_falls_back_to_the_bare_name_rather_than_inventing_a_path(
        ...
        assert "codexcrew sandbox install-profile" in msg
        assert "while Codex Crew is open" not in msg
