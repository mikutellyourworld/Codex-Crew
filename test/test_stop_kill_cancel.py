...
from codex_crew.mcp_gateway import abort as abort_mod
from codex_crew.mcp_gateway import gatewayd as gw
from codex_crew.mcp_gateway import transport
from codex_crew.mcp_gateway.backend import Backend, _PendingRequest
...
class TestCancelInFlight:
    ...
    async def test_cancel_sends_notifications_for_in_flight_requests(self):
        ...
        with patch("codex_crew.mcp_gateway.backend._write_json_line", new=fake_write):
    ...
    async def test_cancel_returns_empty_when_no_in_flight(self):
        ...
        with patch("codex_crew.mcp_gateway.backend._write_json_line", new=AsyncMock()):
            cancelled = await backend.cancel_in_flight_for_stub("stub-A")
    ...
    async def test_cancel_handles_broken_pipe_gracefully(self):
        ...
        with patch("codex_crew.mcp_gateway.backend._write_json_line", new=failing_write):
            cancelled = await backend.cancel_in_flight_for_stub("stub-A")
    ...
    async def test_cancel_skips_when_backend_dead(self):
        ...
        with patch("codex_crew.mcp_gateway.backend._write_json_line", new=AsyncMock()) as mock_write:
            cancelled = await backend.cancel_in_flight_for_stub("stub-A")
...
class TestDetachOnCancelFailure:
    ...
    async def test_detach_still_called_when_cancel_raises(
        ...
        from codex_crew.mcp_gateway import socketsec
...
class TestConservativeShutdown:
    ...
    async def test_session_sharing_never_kills_runtime(self):
        ...
        with patch("os.kill") as mock_kill, \
             ...
             patch("codex_crew.subagent.Stats") as mock_stats:
