...
from codex_crew.probes import GH_PR
from codex_crew.probes.targets import infer


def test_a_pr_url_is_inferred():
    target = infer("Babysit https://github.com/mikutellyourworld/Codex-Crew/pull/7491 to green.")
    assert target is not None
    assert target.kind == GH_PR
    assert target.subject == "mikutellyourworld/Codex-Crew#7491"
    assert json.loads(target.message) == {
        "repo": "mikutellyourworld/Codex-Crew",
        "pr": 7491,
...
def test_an_unconvertible_pr_number_is_refused_not_raised():
    ...
    target = infer(
        ...
        "https://github.com/mikutellyourworld/Codex-Crew/pull/7491"
    )
    assert target is not None
    assert target.subject == "mikutellyourworld/Codex-Crew#7491"
...
def test_a_competing_bare_reference_refuses_inference():
    ...
    kept = infer("fix src/codex_crew/autonudge.py#1751 for https://github.com/acme/widgets/pull/42")
    assert kept is not None and kept.subject == "acme/widgets#42"
...
def test_a_competing_shorthand_refuses_the_url_rather_than_gating_on_it():
    ...
    assert (
        infer(
            "Drive mikutellyourworld/Codex-Crew#42 to green; it is blocked on "
            "https://github.com/mikutellyourworld/Codex-Crew/pull/7 merging first."
        )
        is None
    )
    ...
    target = infer(
        "Babysit https://github.com/mikutellyourworld/Codex-Crew/pull/7491; the guard lives "
        "at src/codex_crew/autonudge.py#1751."
    )
    assert target is not None and target.subject == "mikutellyourworld/Codex-Crew#7491"
...
def test_a_shorthand_subject_is_refused_so_the_loop_stays_ungated():
    ...
    direction.
    """
    assert infer("watch mikutellyourworld/Codex-Crew#7491 until it is green") is None
    assert infer("BABYSIT mikutellyourworld/Codex-Crew#7435 (fix/knowledge-sync) to green.") is None
...
def test_the_url_and_shorthand_for_one_pr_are_one_subject():
    ...
    target = infer(
        "Drive mikutellyourworld/Codex-Crew#7491 -- "
        "https://github.com/mikutellyourworld/Codex-Crew/pull/7491 -- to review-ready."
    )
    assert target is not None
    assert target.subject == "mikutellyourworld/Codex-Crew#7491"
...
def test_two_distinct_pull_requests_refuse_rather_than_pick_one():
    ...
    assert (
        infer(
            "PR mikutellyourworld/Codex-Crew#4327 is gated on mikutellyourworld/Codex-Crew#4137 "
...
def test_a_source_path_with_a_line_ref_is_not_a_target():
    """``src/codex_crew/autonudge.py#1751`` names a code location, not a PR.
    ...
    """
    assert infer("fix the guard at src/codex_crew/autonudge.py#1751") is None
...
def test_a_real_babysit_instruction_infers_its_own_pr():
    ...
    text = (
        "BABYSIT PR #7542 (mikutellyourworld/Codex-Crew, branch refactor/delete-dead-fence-"
        "helpers, worktree /home/u/oss/codexcrew-fix-4919, Closes #4919, follow-up "
        "#7540).\n\nEXIT: when 0 red checks, PR Readiness passed and MERGEABLE -- "
        "post a final message whose first line is `GREEN: PR #7542 "
        "https://github.com/mikutellyourworld/Codex-Crew/pull/7542`"
    )
    target = infer(text)
    assert target is not None
    assert target.subject == "mikutellyourworld/Codex-Crew#7542"
