...
specific to writers that rebuild the file from ``CodexCrewConfig.to_dict()``:
...
from codex_crew.config.loader import stamp_config_meta
...
def _run_config_cmd(args: argparse.Namespace, config_dir: Path) -> None:
    ...
    with (
        patch("codex_crew.cli_config.config_path", return_value=config_dir / "config.json"),
        patch(
            "codex_crew.cli_config.config_local_path",
            return_value=config_dir / "config.local.json",
        ),
        patch("codex_crew.config.loader.config_dir", return_value=config_dir),
        patch("codex_crew.cli_config.sel"),
    ):
        _config_cmd(args)
...
class TestCliConfigSetKeepsMeta:
    """``codexcrew config set <key> <value>`` rebuilds the file from the dataclass."""
...
class TestConfigSaveKeepsMeta:
    def test_save_stamps_the_current_build(self, tmp_path: Path) -> None:
        ...
        with (
            patch(
                "codex_crew.config.loader.config_path", return_value=config_dir / "config.json"
            ),
            patch(
...
class TestRefreshConfigMetaStamp:
    ...
    def test_stale_stamp_is_refreshed_and_settings_survive(self, tmp_path: Path) -> None:
        from codex_crew.config.loader import refresh_config_meta_stamp
    ...
    def test_current_stamp_is_not_rewritten(self, tmp_path: Path) -> None:
        ...
        from codex_crew.config.loader import refresh_config_meta_stamp, stamp_config_meta
...
class TestDashboardNeverSurfacesThePersistedMarker:
    ...
    status producer reports ``codex_crew.__version__`` — and this locks that
    ...
    def test_status_producers_report_the_running_version(self) -> None:
        ...
        assert ws._local_version == codex_crew.__version__
        assert updates._local_version == codex_crew.__version__
    ...
    def test_api_status_reads_the_module_version(self) -> None:
        ...
        assert '"version": codex_crew.__version__' in src

    def test_no_dashboard_module_reads_the_marker(self) -> None:
        import re

        dashboard_dir = (
            Path(__file__).resolve().parents[1] / "src" / "codex_crew" / "dashboard"
        )
