...
from codex_crew.subagent import SubagentManager, validate_cwd
...
class TestSpawnCwd:
    ...
    async def test_spawn_with_valid_cwd_stores_resolved_path(
        ...
        with (
            ...
            patch("codex_crew.subagent.CodexCrewConfig.load", return_value=mock_cfg),
        ):
    ...
    async def test_spawn_with_invalid_cwd_rejects_and_emits_sel(
        ...
        with (
            patch("codex_crew.subagent.Stats"),
            patch("codex_crew.subagent.sel", return_value=sel_mock),
            patch("codex_crew.subagent.CodexCrewConfig.load", return_value=mock_cfg),
        ):
    ...
    async def test_prevalidated_app_spawn_at_capacity_is_rejected_not_queued(
        ...
        with (
            patch("codex_crew.subagent.Stats"),
            patch("codex_crew.subagent.sel"),
            patch("codex_crew.subagent._vet_spawn_governance", return_value=None),
            patch("codex_crew.subagent.CodexCrewConfig.load", return_value=mock_cfg),
        ):
            info = manager.spawn(
                "t",
    ...
    async def test_spawn_fails_closed_when_config_load_raises(
        ...
        """If CodexCrewConfig.load raises, reject cwd (fail-closed).
        ...
        with patch("codex_crew.subagent.Stats"), patch("codex_crew.subagent.sel"), load_mock:
            info = manager.spawn("t", cwd=str(project))
