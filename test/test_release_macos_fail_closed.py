...
CHANNEL = "stable"
VERSION = "1.2.3"
ARTIFACT_NAME = f"CodexCrew-notarized-{CHANNEL}-{VERSION}"
...
def _write_valid_zip(path: Path, app_name: str = "CodexCrew.app") -> None:
...
def _write_valid_handoff(root: Path, name: str = ARTIFACT_NAME) -> Path:
    ...
    _write_valid_dmg(artifact / "CodexCrew.dmg")
    return artifact
...
WINDOWS_ASSET = f"CodexCrew-{VERSION}-Setup-x64.exe"  # brand-ok: artifact filename
...
#: installer never collides with the bundle's `CodexCrew-Setup.exe`, and why an
#: extension glob would attach both instead of choosing.
REBUILT_INSTALLER = f"CodexCrew Setup {VERSION}.exe"  # brand-ok: artifact filename
...
def test_missing_exact_gated_artifact_does_not_fall_back_to_unsigned(tmp_path: Path) -> None:
    ...
    _write_valid_zip(unsigned / "CodexCrew-universal-mac.zip")
    _write_valid_dmg(unsigned / "CodexCrew.dmg")
    _write_valid_handoff(tmp_path, "CodexCrew-notarized-stable-1.2.2")
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("CodexCrew.dmg", "Required gated macOS DMG is missing or empty"),
    ),
)
...
def test_non_udif_dmg_fails(tmp_path: Path) -> None:
    artifact = _artifact_dir(tmp_path)
    _write_valid_zip(artifact / "notarized.zip")
    (artifact / "CodexCrew.dmg").write_bytes(b"not a UDIF image")
...
def test_exact_gated_handoff_is_renamed_for_the_release(tmp_path: Path) -> None:
    ...
    release_zip = release / f"CodexCrew-{VERSION}-universal-mac.zip"
    release_dmg = release / f"CodexCrew-{VERSION}-universal.dmg"
    assert release_zip.read_bytes() == (gated / "notarized.zip").read_bytes()
    assert release_dmg.read_bytes() == (gated / "CodexCrew.dmg").read_bytes()
...
def test_promotion_publishes_the_bundled_installer_and_never_the_rebuild(
    ...
    """
    gated = _write_valid_handoff(tmp_path)
    (gated / "CodexCrew-Setup.exe").write_bytes(b"promoted installer")
    (gated / "CodexCrew-Setup.exe.blockmap").write_bytes(b"promoted blockmap")
