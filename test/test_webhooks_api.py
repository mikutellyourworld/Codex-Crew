...
from codex_crew.dashboard.handlers import hooks as H
...
def wired(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        H, "_installed_agent_names", lambda: {"codexcrew", "code-reviewer", "oncall"}
    )
...
class TestTokenEndpoints:
    ...
    async def test_create_rejects_bad_label(self, wired):
        resp = await H.api_webhook_token_create(
            _req("POST", "/api/webhooks/tokens", {"label": "  ", "agent": "codexcrew"})
        )
    ...
    async def test_cap_returns_400_on_twenty_first(self, wired):
        ...
        resp = await H.api_webhook_token_create(
            _req(
                "POST", "/api/webhooks/tokens",
                {"label": "Too many", "agent": "codexcrew"},
            )
        )
        assert resp.status == 400
    ...
    async def test_patch_reports_an_unavailable_store(self, wired):
        ...
        _raw, _secret, entry = store.create("Review Bot", agent="codexcrew")
...
class TestVerifyHookToken:
    ...
    async def test_header_variants_and_legacy(self, wired, monkeypatch):
        ...
        alt = make_mocked_request("POST", "/api/hooks/agent", headers={"x-codexcrew-token": raw})
    ...
    def test_legacy_reader_tolerates_non_dict_config(self, monkeypatch):
        ...
        monkeypatch.setattr(H.CodexCrewConfig, "load", staticmethod(lambda: cfg))
