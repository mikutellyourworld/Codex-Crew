...
response). Every write lands under the autouse-isolated ``CODEXCREW_HOME``
...
from codex_crew import extras
from codex_crew import transcribe as transcribe_mod
...
from codex_crew.dashboard.handlers import _shared as shared_mod
from codex_crew.dashboard.handlers import core as core_mod
from codex_crew.sel import SelVerification as _SelVerification
from codex_crew.stt import models as stt_models
from codex_crew.stt.limits import MIN_PARTIAL_INTERVAL_MS, MIN_SILENCE_MS
...
def fake_sel(monkeypatch) -> MagicMock:
    ...
    """
    recorder = MagicMock()
    monkeypatch.setattr("codex_crew.dashboard.handlers.sel", lambda: recorder)
    return recorder
...
class TestPageAndAssets:
    ...
    async def test_logo_refuses_sensitive_avatar_path(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.handlers.CodexCrewConfig",
            SimpleNamespace(load=lambda: cfg),
        )
        monkeypatch.setattr("codex_crew.dashboard.handlers.is_sensitive_path", lambda _p: True)
    ...
    async def test_logo_serves_validated_custom_avatar(self, monkeypatch, tmp_path) -> None:
        ...
        monkeypatch.setattr("codex_crew.dashboard.handlers.is_sensitive_path", lambda _p: False)
        monkeypatch.setattr(
            "codex_crew.hooks.validate_file_path", lambda p: str(avatar) if p else None
        )
        resp = await core_mod.logo(_req())
    ...
    async def test_logo_prefers_nightly_variant_on_nightly_build(
        ...
        (tmp_path / "codexcrew-logo.png").write_bytes(b"day")
        nightly = tmp_path / "codexcrew-logo-nightly.png"
        ...
        monkeypatch.setattr("codex_crew.dashboard.handlers._STATIC_DIR", tmp_path)
        monkeypatch.setattr("codex_crew.__version__", "9.9.9-nightly.20260812")
        resp = await core_mod.logo(_req())
        assert isinstance(resp, web.FileResponse)
    ...
    async def test_logo_404_when_no_asset_exists(self, monkeypatch, tmp_path) -> None:
        ...
        monkeypatch.setattr("codex_crew.dashboard.handlers._STATIC_DIR", tmp_path / "empty")
        monkeypatch.setattr("codex_crew.__version__", "9.9.9")
        resp = await core_mod.logo(_req())
        assert resp.status == 404
    ...
    async def test_favicon_route_resolves_to_logo_handler(self) -> None:
        ...
        from codex_crew.dashboard.routes import realtime
...
class TestSttProviders:
    def test_apple_hidden_when_framework_unavailable(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "codex_crew.apple_speech.availability",
...
class TestSttPrereqCommands:
    ...
    provider can need beyond an installed Codex Crew is the optional ``voice``
    ...
    def test_local_needs_the_voice_extra_when_the_recogniser_is_absent(self, monkeypatch) -> None:
        ...
        assert len(cmds) == 1
        # The extra's own distributions, never `codexcrew[voice]`: this project is
        # published on no index, so the extras form cannot resolve for anyone.
        assert "codexcrew[" not in cmds[0]
...
class TestSttStatus:
    ...
    def _quiet_probes(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.stt.models.is_present", lambda _m: False)
        monkeypatch.setattr(
            "codex_crew.stt.engine.shared_engine", lambda *a, **k: SimpleNamespace(loaded=False)
        )
    ...
    async def test_status_reports_a_present_model_as_present(
        ...
        monkeypatch.setattr("codex_crew.stt.models.is_present", lambda _m: True)
        body = json.loads((await core_mod.api_stt_status(_req())).body)
    ...
    async def test_status_reads_presence_through_the_owning_module(
        ...
        ``codex_crew.stt`` re-exports its surface through a lazy ``__getattr__``, and
    ...
    async def test_status_reports_whether_a_model_is_resident(
        ...
        monkeypatch.setattr(
            "codex_crew.stt.engine.shared_engine", lambda *a, **k: SimpleNamespace(loaded=True)
        )
...
class TestSttPrepare:
    ...
    def _no_real_download(self, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.stt.session.ensure_model", _fake_ensure)
...
class TestSttFfmpegDownload:
    ...
    def test_the_route_is_registered(self) -> None:
        ...
        from codex_crew.dashboard.routes import memory as memory_routes
...
class TestSttPrewarm:
    ...
    async def test_prewarm_answers_202_without_waiting_for_the_load(
        ...
        monkeypatch.setattr("codex_crew.stt.session.prewarm", _fake_prewarm)
    ...
    async def test_prewarm_failure_does_not_fail_the_request(
        ...
        monkeypatch.setattr("codex_crew.stt.session.prewarm", _boom)
        resp = await core_mod.api_stt_prewarm(_req())
        assert resp.status == 202
...
class TestSttTranscribe:
    ...
    async def test_transcript_is_returned_and_redacted(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "codex_crew.transcribe.transcribe_audio",
...
class TestSecurityStats:
    ...
    async def test_counts_are_derived_from_the_posture_registry(self, monkeypatch) -> None:
        monkeypatch.setattr(
            "codex_crew.dashboard.handlers.security.build_denied_commands_snapshot_async",
...
# ── Agent settings PUT (/api/config/codexcrew) ───────────────────────────


def _agent_cfg_app() -> web.Application:
    app = web.Application()
    app.router.add_route("*", "/api/config/codexcrew", core_mod.api_codexcrew_config)
    return app


async def _put_agent(client, settings: dict):
    return await client.put("/api/config/codexcrew", json={"agent": settings})
...
class TestAgentSettingsPut:
    ...
    async def test_malformed_body_is_denied_and_audited(self, seeded_config, fake_sel) -> None:
        async with TestClient(TestServer(_agent_cfg_app())) as client:
            resp = await client.put("/api/config/codexcrew", data=b"{{{")
            assert resp.status == 400
            assert (await resp.json())["error"] == "invalid JSON"
        assert fake_sel.log_api_access.call_args.kwargs["outcome"] == "denied"
    ...
    async def test_missing_agent_object_is_denied(self, seeded_config, fake_sel) -> None:
        async with TestClient(TestServer(_agent_cfg_app())) as client:
            resp = await client.put("/api/config/codexcrew", json={"agent": "nope"})
            assert resp.status == 400
    ...
    async def test_conductor_enable_regenerates_the_skill(
        ...
        regen = MagicMock()
        monkeypatch.setattr("codex_crew.dashboard.handlers.agents._regen_conductor", regen)
        async with TestClient(TestServer(_agent_cfg_app())) as client:
    ...
    async def test_get_drops_edition_contributed_sections(self, seeded_config) -> None:
        ...
        async with TestClient(TestServer(_agent_cfg_app())) as client:
            body = await (await client.get("/api/config/codexcrew")).json()
...
class TestAgentSettingsPutLockOffload:
    ...
    async def test_write_goes_through_atomic_helper(
        ...
        import codex_crew.config.loader as _loader
...
class TestPatchGuards:
    ...
    async def test_moved_field_names_its_replacement_endpoint(
        ...
        app = web.Application()
        app.router.add_patch("/api/config/codexcrew", core_mod.api_codexcrew_config_patch)
        async with TestClient(TestServer(app)) as client:
            resp = await client.patch(
                "/api/config/codexcrew",
    ...
    async def test_unknown_field_is_refused(self, seeded_config, fake_sel) -> None:
        ...
        async with TestClient(TestServer(app)) as client:
            resp = await client.patch(
                "/api/config/codexcrew", json={"path": "agent.nope", "value": 1}
            )
            assert resp.status == 400
...
class TestAdvertisedModelGuards:
    ...
    def test_provider_rejection_is_surfaced(self, monkeypatch) -> None:
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_handlers._model_rejected_reason",
...
class TestLocalToken:
    ...
    async def test_non_loopback_is_refused(self, monkeypatch, fake_sel) -> None:
        monkeypatch.setattr("codex_crew.dashboard.handlers.is_loopback", lambda _r: False)
    ...
    async def test_unconfigured_secret_is_503(self, monkeypatch, fake_sel) -> None:
        monkeypatch.setattr("codex_crew.dashboard.handlers.is_loopback", lambda _r: True)
    ...
    async def test_issues_credential_with_requested_ttl_and_embed_claim(
        ...
        resp = await core_mod.api_token_local(
            _req(
                ...
                query={"ttl": "2h", "embed_parent_port": "5486"},
            )
        )
        assert resp.status == 200
        assert json.loads(resp.body)["expires_in"] == 7200
        assert minted["owner"] == "owner-1"
        assert minted["extra"] == {"embed_parent_port": "5486"}
...
class TestLogout:
    ...
    async def test_revocation_persist_failure_reports_a_coded_error(
        ...
        monkeypatch.setattr("codex_crew.dashboard.token_auth.revoke_all_sessions", _boom)
        resp = await core_mod.api_logout(
    ...
    async def test_successful_revocation_is_audited(self, monkeypatch, fake_sel) -> None:
        monkeypatch.setattr("codex_crew.dashboard.handlers.is_loopback", lambda _r: True)
        revoke = MagicMock()
        monkeypatch.setattr("codex_crew.dashboard.token_auth.revoke_all_sessions", revoke)
        resp = await core_mod.api_logout(
            _req(app={"local_secret": "right"}, headers={"X-Local-Secret": "right"})
        )
        assert resp.status == 200
...
class TestAppSecretExchange:
    ...
    def app_sel(self, monkeypatch) -> MagicMock:
        recorder = MagicMock()
        monkeypatch.setattr("codex_crew.sel.sel", lambda: recorder)
        return recorder
    ...
    async def test_invalid_secret_is_refused(self, app_sel, monkeypatch) -> None:
        monkeypatch.setattr("codex_crew.dashboard.token_auth.validate_app_secret", lambda *_a: False)
    ...
    async def test_valid_secret_mints_an_app_scoped_credential(self, app_sel, monkeypatch) -> None:
        monkeypatch.setattr("codex_crew.dashboard.token_auth.validate_app_secret", lambda *_a: True)
        seen: dict = {}
        ...
        monkeypatch.setattr("codex_crew.dashboard.token_auth.generate_token", _generate)
        resp = await core_mod.api_app_token(
...
class TestSessionAgentRoutes:
    ...
    async def test_list_returns_workspace_results(self, monkeypatch, fake_sel) -> None:
        monkeypatch.setattr(
            "codex_crew.session_workspace.list_results",
    ...
    async def test_missing_result_is_404(self, monkeypatch, fake_sel) -> None:
        monkeypatch.setattr("codex_crew.session_workspace.read_result", lambda *_a: "")
    ...
    async def test_result_is_returned_after_redaction(self, monkeypatch, fake_sel) -> None:
        monkeypatch.setattr(
            "codex_crew.session_workspace.read_result", lambda *_a: "finished the audit"
        )
        resp = await core_mod.api_session_agent_result(
            _req(match_info={"id": "s1", "agent_id": "a1"})
        )
    ...
    async def test_stream_emits_the_tail_then_a_done_event(
        ...
        monkeypatch.setattr("codex_crew.session_workspace.result_path", lambda *_a: result)
    ...
    async def test_stream_stops_when_the_client_disconnects(self, monkeypatch, fake_sel) -> None:
        ...
        monkeypatch.setattr(
            "codex_crew.session_workspace.result_path",
...
class TestSessionAgentPathIdValidation:
    ...
    async def test_the_accepted_charset_is_not_narrowed(self, good, monkeypatch, fake_sel) -> None:
        ...
        refused one."""
        monkeypatch.setattr("codex_crew.session_workspace.list_results", lambda _s: [])
