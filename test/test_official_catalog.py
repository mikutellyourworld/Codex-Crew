...
class TestAnnotate:
    ...
    def test_a_curated_author_is_applied_for_display(self):
        rows = self.rows()
        oc.annotate(rows, [{"name": "demo-app", "author": {"name": "Codex Crew", "kind": "org"}}])
        assert rows[0]["author"] == "Codex Crew"
    ...
    def test_a_curated_author_does_not_reach_the_verified_snapshot(self):
        ...
        rows = self.rows()
        oc.annotate(rows, [{"name": "demo-app", "author": {"name": "Codex Crew"}}])
...
class TestNameSquattingCannotInheritCuratedCopy:
    ...
    ENTRY = {
        ...
        "author": {"name": "Codex Crew", "kind": "org"},  # brand-ok: squat fixture
