"""Tests for codex_crew.feishu.transport (FeishuTransport, Layer 1)."""
...
from codex_crew.feishu.transport import (
    _SEEN_KEEP,
    _SEEN_MAX,
    FEISHU_CAPABILITIES,
    FeishuTransport,
...
class TestAuthorize:
    ...
    def test_unknown_open_id_denied(self) -> None:
        t = FeishuTransport(FakeClient(), allowed_open_ids=["ou_abc"])
        with patch("codex_crew.feishu.transport.sel") as mock_sel:
...
class TestChatTypeGate:
    ...
    async def test_explicit_p2p_still_serves(self) -> None:
        dispatched: list[LarkInbound] = []
        t = self._t(dispatched)
        with patch("codex_crew.feishu.transport.sel"):
