...
class TestSyncBuiltinConfig:
    def test_sets_enabled_false(self, tmp_path, register_test_builtin):
        ...
        with patch("codex_crew.apps.routes.config_path", return_value=cfg):
    ...
    def test_write_routes_through_update_config_locked(self, tmp_path, register_test_builtin):
        ...
        with patch("codex_crew.apps.routes.config_path", return_value=cfg):
            with patch("codex_crew.config.loader.config_path", return_value=cfg):
                with patch("codex_crew.apps.routes.update_config_locked", side_effect=_spy):
                    _sync_builtin_config(_TEST_BUILTIN, enabled=True)
        assert len(calls) == 1
    ...
    def test_failed_write_propagates_to_the_callers_warning_path(
        ...
        with patch("codex_crew.apps.routes.config_path", return_value=cfg):
            with patch(
                "codex_crew.apps.routes.update_config_locked",
...
class TestHandleDisableBuiltin:
    ...
    async def test_disable_builtin_syncs_config_and_stops_service(self, tmp_path, register_test_builtin):
        ...
        with (
            patch("codex_crew.apps.routes.get_app", return_value={
            ...
            patch("codex_crew.apps.routes.deregister_app"),
            patch("codex_crew.apps.routes.config_path", return_value=cfg),
            patch("codex_crew.apps.routes.sel"),
        ):
...
class TestRedactWarning:
    def test_passes_through_clean_string(self):
        from codex_crew.apps.routes import _redact_warning
...
class TestBuiltinConfigSyncHoldsBothConfigLocks:
    ...
    async def test_disable_holds_the_loop_side_lock_across_the_write(
        ...
        with (
            ...
            patch("codex_crew.apps.routes.config_path", return_value=cfg),
            patch("codex_crew.apps.routes.sel"),
            patch("codex_crew.apps.routes._sync_builtin_config", spy),
        ):
    ...
    async def test_enable_holds_the_loop_side_lock_across_the_write(
        ...
        with (
            ...
            patch("codex_crew.apps.routes.enable_app") as mock_enable,
            patch("codex_crew.apps.routes.on_app_enable", AsyncMock(return_value=None)),
            patch("codex_crew.apps.routes.config_path", return_value=cfg),
            patch("codex_crew.apps.routes.sel"),
            patch("codex_crew.apps.routes._sync_builtin_config", spy),
        ):
