...
from codex_crew.config.loader import CodexCrewConfig, SkillsConfig
from codex_crew.skill_usage import SkillUsageLedger
from codex_crew.skills import _SHORT_DESC_CHARS, SkillsLoader
...
def _isolate_extra_paths(monkeypatch, tmp_path_factory):
    ...
    so a developer's local ~/.codexcrew extra_paths / composed companion roots
    ...
    monkeypatch.setattr(
        CodexCrewConfig,
        "load",
        classmethod(lambda cls: CodexCrewConfig(skills=SkillsConfig(extra_paths=[]))),
    )
...
class TestSkillsLoader:
    ...
    def test_update_auto_skill_invalidates_cache_same_mtime(self, tmp_path):
        ...
        from codex_crew.skills import AutoSkillProvenance
...
class TestRepoScope:
    ...
    def _repo(self, tmp_path: Path, name: str = "checkout") -> Path:
        repo = tmp_path / name
        (repo / "src" / "codex_crew").mkdir(parents=True)
    ...
    def test_scoped_skill_suppressed_without_a_project(self, tmp_path: Path) -> None:
        ...
        self._write_skill(skills, "repo-only", "src/codex_crew")
    ...
    def test_scoped_skill_eligible_inside_repo(self, tmp_path: Path) -> None:
        skills = tmp_path / "skills"
        self._write_skill(skills, "repo-only", "src/codex_crew")
        cfg = CodexCrewConfig(skills=SkillsConfig(max_triggered=3))
        loader = SkillsLoader(skills_path=skills, install_builtins=False, config=cfg)
        subdir = self._repo(tmp_path) / "website"
        subdir.mkdir()
        # ancestor of the project dir contains src/codex_crew
    ...
    def test_always_path_is_gated_identically(self, tmp_path: Path) -> None:
        ...
        # way around the gate.
        skills = tmp_path / "skills"
        self._write_skill(skills, "repo-only", "src/codex_crew", always=True)
        loader = SkillsLoader(skills_path=skills, install_builtins=False)
        other = tmp_path / "some-rust-project"
        other.mkdir()
    ...
    def test_pod_e2e_declares_a_repo_scope(self) -> None:
        ...
        from codex_crew import skills as skills_mod
        ...
        assert "repo_scope: src/codex_crew" in head
        assert "ONLY for developing Codex Crew itself" in head


class TestRelocatedSkillCleanup:
    """Skills moved into skills/codexcrew-dev/ must have their old FLAT copies
    ...
    def test_flat_copy_quarantined_when_nested_present(self, tmp_path: Path) -> None:
        from codex_crew.skills import _ensure_builtin_skills
        ...
        new = base / "codexcrew-dev" / "prepare-pr"
...
class TestTriggeredSkills:
    ...
    def _loader_with_skill(self, tmp_path, triggers, monkeypatch=None):
        ...
        if monkeypatch is not None:
            ...
            monkeypatch.setattr("codex_crew.skills.sel", lambda: MagicMock())
        return loader
...
class TestTriggerMatching:
    ...
    def test_max_triggered_limit(self, tmp_path, monkeypatch):
        ...
        cfg = CodexCrewConfig(skills=SkillsConfig(max_triggered=2))
        loader = SkillsLoader(skills_path=skills_dir, install_builtins=False, config=cfg)
    ...
    def test_sort_by_overlap_score(self, tmp_path, monkeypatch):
        ...
        cfg = CodexCrewConfig(skills=SkillsConfig(max_triggered=5))
        loader = SkillsLoader(skills_path=skills_dir, install_builtins=False, config=cfg)
...
class TestCreateAutoSkill:
    ...
    def test_rejects_oversized_procedure(self, tmp_path):
        from codex_crew.skills import AUTO_SKILL_MAX_PROCEDURE_CHARS
...
class TestListAutoSkills:
    ...
    def test_filters_to_auto_namespace_only(self, tmp_path):
        ...
        loader.create_auto_skill(
            ...
            provenance=(
                __import__(
                    "codex_crew.skills", fromlist=["AutoSkillProvenance"]
                ).AutoSkillProvenance(
                    session_key="x",
                    created_at="2026-05-05T11:30:00+00:00",
                )
...
def _cfg_with_extra(paths):
    ...
    return CodexCrewConfig(skills=SkillsConfig(extra_paths=paths))
...
class TestSkillsLoaderExtraPaths:
    ...
    def test_sensitive_extra_path_skipped(self, tmp_path, monkeypatch):
        extra = tmp_path / "extra"
        extra.mkdir()
        monkeypatch.setattr("codex_crew.skills.is_sensitive_path", lambda p: True)
        loader = SkillsLoader(
    ...
    def test_sensitive_skill_file_skipped(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.skills.validate_file_path",
...
class TestTriggerPerformance:
    ...
    def test_single_audit_event_for_triggered_set(self, tmp_path, monkeypatch):
        ...
        loader = SkillsLoader(
            ...
            install_builtins=False,
            config=CodexCrewConfig(skills=SkillsConfig(max_triggered=3)),
        )

        fake_sel = MagicMock()
        monkeypatch.setattr("codex_crew.skills.sel", lambda: fake_sel)
    ...
    def test_config_not_loaded_per_message(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.config.loader.CodexCrewConfig.load", _counting_load)
        for _ in range(5):
            loader.get_triggered_skills("shorten this url")
...
class TestResolveDollarSkills:
    ...
    def test_cap_enforced(self, tmp_path, monkeypatch):
        ...
        loader = SkillsLoader(skills_path=skills_dir, install_builtins=False)
        monkeypatch.setattr("codex_crew.skills._MAX_DOLLAR_SKILLS", 5)
    ...
    def test_local_precedence_over_aim_extra_path(self, tmp_path):
        ...
        cfg = CodexCrewConfig(skills=SkillsConfig(extra_paths=[str(aim)]))
...
class TestDisabledAppSkillsAreNotTriggered:
    ...
    def _link_shipped_builtin_skill(skills: Path) -> str:
        ...
        """
        import codex_crew.skills as skills_mod
