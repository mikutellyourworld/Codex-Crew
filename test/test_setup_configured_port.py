...
class TestConfiguredPort:
    def test_returns_default_without_config(self, monkeypatch):
        ...
        with patch("codex_crew.cli_server.CodexCrewConfig") as mock_cfg:
            mock_cfg.load.return_value.dashboard.url = ""
            assert _configured_port() == 5486
...
class TestCustomDomainUsesConfiguredPort:
    def test_prints_configured_port(self, monkeypatch, capsys, tmp_path):
        monkeypatch.setenv("CODEXCREW_PORT", "4321")
        monkeypatch.setattr("codex_crew.cli_setup.Path", lambda *a, **kw: tmp_path / "hosts")
        _maybe_setup_custom_domain()
...
class TestDashboardUrlUsesConfiguredPort:
    def test_input_prompt_contains_configured_port(self, monkeypatch, capsys):
        monkeypatch.setenv("CODEXCREW_PORT", "5678")
        ...
        monkeypatch.setattr("codex_crew.cli_setup.CodexCrewConfig.load", lambda: mock_cfg)
        monkeypatch.setattr("codex_crew.cli_setup.config_path", lambda: MagicMock())
        monkeypatch.setattr("codex_crew.cli_setup.socket.gethostbyname", lambda _: "10.0.0.1")
        monkeypatch.setattr("codex_crew.cli_setup.socket.gethostname", lambda: "myhost")
