...
from codex_crew.channel import _match_trusted_channel_command
from codex_crew.dashboard.handlers_channel import api_channel_approve_agent
...
async def _call(body: dict, agent):
    ...
    with (
        patch(
            "codex_crew.dashboard.handlers_channel._mgr",
            return_value=MagicMock(get=MagicMock(return_value=ch)),
        ),
        patch("codex_crew.dashboard.handlers_channel.sel", return_value=MagicMock()),
    ):
...
class TestChannelApproveTrustTiers:
    ...
    async def test_trust_denials_are_sel_audited(self):
        ...
        with (
            ...
            patch("codex_crew.dashboard.handlers_channel.sel", return_value=sel_mock),
        ):
            resp = await api_channel_approve_agent(request)
        assert resp.status == 400
