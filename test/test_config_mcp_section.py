...
consumer (``codex_crew.env.augmented_path``) -- see ``test_env.py`` -- so what is
...
from codex_crew.config.loader import CodexCrewConfig, McpConfig


def _load_from(tmp_path, monkeypatch, data: dict) -> CodexCrewConfig:
...
def test_defaults_to_empty_list():
    assert CodexCrewConfig().mcp.extra_path_dirs == []
...
def test_load_publishes_the_setting_to_the_search_path(tmp_path, monkeypatch):
    ...
    ``codex_crew.env``. Pinned end-to-end: without this call the config field
...
def test_section_is_in_the_schema_registry():
    ...
    from codex_crew.config import schema
...
def test_distinct_from_mcp_gateway():
    ...
    assert not hasattr(CodexCrewConfig().mcp_gateway, "extra_path_dirs")
    assert isinstance(CodexCrewConfig().mcp, McpConfig)
