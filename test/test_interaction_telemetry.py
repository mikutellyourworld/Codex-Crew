...
def stub_telemetry():
    ...
    stub = _StubTelemetry()
    ctx = dataclasses.replace(build_default_context(CodexCrewConfig()), telemetry=stub)
    set_context(ctx)
    yield stub
    reset_context()
...
def _clean_slack_module_state():
    ...
    from codex_crew.slack.handler import _pending_approvals, _thread_agents, _trusted_sessions
