...
from codex_crew.context_blocks import split_blocks
...
def _loader(skills_path: Path, *, cap: int = 3) -> SkillsLoader:
    ...
    return SkillsLoader(
        ...
        config=CodexCrewConfig(skills=SkillsConfig(max_triggered=cap)),
    )
...
class TestDeliveryIsAuditable:
    ...
    def test_sel_metadata_separates_bodies_from_pointers(
        ...
        monkeypatch.setattr("codex_crew.skills.sel", lambda: _Sel())
