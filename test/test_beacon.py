"""Tests for the anonymous usage beacon (codex_crew.beacon).
...
from codex_crew import beacon, platform_compat
from codex_crew.apps import install_receipt
...
class TestInstallId:
    ...
    def test_non_utf8_id_does_not_crash(self, _isolated_home):
        ...
        `codexcrew telemetry status` outright.
        """
...
class TestPayloadAllowlist:
    ...
    def test_no_value_leaks_identity_or_paths(self, _isolated_home, monkeypatch):
        monkeypatch.setenv("CODEXCREW_PROJECT_DIR", "/Users/secret/my-private-repo")
...
class TestDistributionStamp:
    ...
    def test_generated_module_is_gitignored(self):
        ...
        assert "src/codex_crew/_build_info.py" in (root / ".gitignore").read_text()
...
class TestSuppression:
    ...
    def test_non_default_home_suppressed(self, _isolated_home, monkeypatch):
        ...
        ok, reason, _code = beacon.should_send(enabled=True, acked=True)
        assert not ok and "CODEXCREW_HOME" in reason
...
class TestDefaultHomeDetection:
    ...
    def test_dev_home_is_not_default(self, monkeypatch, tmp_path):
        ...
        config_dir() honors CODEXCREW_HOME, so comparing the two would always
        match and the dev-home/pod suppression would never fire. This test
        failed against exactly that bug during development.
        """
        monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path / "dev-home"))
    ...
    def test_real_home_spelled_explicitly_is_default(self, monkeypatch):
        ...
        from codex_crew.config.paths import CONFIG_DIR_NAME

        real = Path.home() / CONFIG_DIR_NAME
        monkeypatch.setenv("CODEXCREW_HOME", str(real))
        assert beacon.is_default_home() is True
...
class TestUrlAndTransport:
    ...
    def test_unwritable_data_home_is_silent(self, _isolated_home, monkeypatch):
        ...
        boot) and made `codexcrew telemetry status` crash — while the module
...
class TestFailOpen:
    ...
    def test_gateway_wiring_is_detached_and_daemon(self):
        ...
        from codex_crew.slack import gateway
...
class TestStatusOutput:
    ...
    1. Codex Crew CLI (recommended)
       codexcrew telemetry disable
...
class TestTelemetryCliWrite:
    ...
    def test_toggle_preserves_unrelated_config(self, _isolated_home, monkeypatch):
        ...
        cfg.write_text(json.dumps({"slack": {"command": "codexcrew"}, "timezone": "UTC"}))
        monkeypatch.setattr("codex_crew.config.loader.config_path", lambda: cfg)
        ...
        assert data["slack"]["command"] == "codexcrew", "must not drop other settings"
    ...
    def test_uses_atomic_write_not_write_text(self, _isolated_home, monkeypatch):
        ...
        because ``CodexCrewConfig.load()`` performs its own migration write-back
    ...
    def test_refuses_rather_than_replacing_a_non_object_section(
        ...
        ``CodexCrewConfig.load`` is stubbed out because it runs FIRST in
        ...
        monkeypatch.setattr("codex_crew.config.loader.config_path", lambda: cfg)
        monkeypatch.setattr(cli_commands.CodexCrewConfig, "load", classmethod(lambda cls: MagicMock()))
...
class TestSnapshotAndPortabilityRegistration:
    ...
    def test_beacon_names_are_not_basename_filtered(self):
        from codex_crew.portability import EXPORT_EXCLUDE
        from codex_crew.snapshot import NEVER_SNAPSHOT_FILES
    ...
    def test_root_export_allowlist_excludes_beacon_state(self):
        ...
        from codex_crew import portability
    ...
    def test_snapshot_components_never_name_beacon_state(self):
        from codex_crew.snapshot import CORE_FILES
    ...
    def test_workspace_file_with_beacon_name_survives_export(self):
        ...
        from codex_crew.portability import _is_excluded
...
class TestGovernancePin:
    ...
    def test_every_enforcement_site_audits_with_its_own_tool_name(
        ...
        from codex_crew.cli_commands import _telemetry
        from codex_crew.cli_config import _config_cmd
        from codex_crew.dashboard.handlers.core import _beacon_governance_pinned_off
...
class TestGenericConfigSetterIsGated:
    """`codexcrew config set` is a FOURTH write path to telemetry.beacon_enabled.
    ...
    def _pin(self, monkeypatch, pinned):
        from codex_crew import beacon as beacon_mod
