...
def agent_root(tmp_path, monkeypatch):
    ...
    root.mkdir()
    monkeypatch.setattr("codex_crew.subagent_persistence._SUBAGENTS_DIR", root)
    return root
...
class TestCreateAgentFolder:
    def test_creates_state_json(self, agent_root):
        path = create_agent_folder("abc123", task="do stuff", agent="codexcrew", parent_session="dashboard:default", max_turns=100)
        state = json.loads((path / "state.json").read_text(encoding="utf-8"))
        assert state["id"] == "abc123"
        assert state["task"] == "do stuff"
        assert state["agent"] == "codexcrew"
...
class TestUpdateState:
    ...
    def test_corrupt_or_nonobject_state_skips_update(self, agent_root, monkeypatch):
        import codex_crew.subagent_persistence as sp
...
class TestPruneStaleTombstones:
    ...
    def test_invalid_tombstone_died_uses_mtime_fallback(
        ...
        cleaned: list[str] = []
        monkeypatch.setattr(
            "codex_crew.subagent_persistence._cleanup_session_files_sync",
...
class TestTombstoneOnAbnormalExit:
    ...
    async def test_tombstone_on_timeout(self, agent_root):
        ...
        from codex_crew.subagent_persistence import create_agent_folder
        ...
        from codex_crew.subagent_persistence import remember_live_cleanup_identity
    ...
    async def test_timeout_skipped_when_already_reaped(self, agent_root):
        ...
        from codex_crew.subagent import SubagentInfo, SubagentManager
        from codex_crew.subagent_persistence import create_agent_folder, write_tombstone
        ...
        with patch.object(manager, "_run_inner", _hang), \
             ...
             patch("codex_crew.subagent.Stats"), \
             patch("codex_crew.subagent.sel"), \
...
class TestFolderCleanupOnSuccess:
    ...
    async def test_folder_kept_on_delivery_failure(self, agent_root):
        ...
        with patch("codex_crew.subagent._ON_DONE_TIMEOUT", 0.01), \
             patch("codex_crew.subagent.Stats"), \
             patch("codex_crew.subagent.sel"):
...
class TestOrphanReconciliation:
    ...
    async def test_dead_pid_with_result_tombstoned_as_delivered(self, agent_root):
        ...
        from codex_crew.subagent import SubagentManager
        from codex_crew.subagent_persistence import create_agent_folder, write_result_chunk
        ...
        from codex_crew.subagent_persistence import update_state
    ...
    async def test_dead_pid_no_result_tombstoned_as_notified(self, agent_root):
        ...
        from codex_crew.subagent import SubagentManager
        from codex_crew.subagent_persistence import create_agent_folder, update_state
...
class TestOrphanNotification:
    ...
    async def test_msg_redacted_before_injection_path(self, agent_root):
        ...
        with patch.object(manager, "_try_inject_orphan_notification", side_effect=_capture_inject), \
             patch("codex_crew.subagent._redact", side_effect=lambda m: f"[REDACTED]{m}") as mock_redact:
...
class TestSpawnStatusReadsFromAgentFolder:
    ...
    def test_read_state_for_orphaned_agent(self, agent_root):
        ...
        from codex_crew.subagent_persistence import create_agent_folder, read_state, write_tombstone
    ...
    async def test_api_spawn_status_fallback_to_disk(self, agent_root):
        ...
        from codex_crew.dashboard.handlers.messaging import api_spawn_status
        from codex_crew.subagent_persistence import (
            create_agent_folder,
            write_result_chunk,
            write_tombstone,
...
class TestPathTraversal:
    def test_dot_agent_id_rejected(self, agent_root):
        from codex_crew.subagent_persistence import _agent_dir
...
class TestRecordSlowCommandRotation:
    ...
    def small_cap(self, monkeypatch):
        monkeypatch.setattr(
            "codex_crew.subagent_persistence._SLOW_LOG_MAX_BYTES", self.CAP
        )
