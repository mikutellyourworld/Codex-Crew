...
under ~/.codexcrew: a fold test that depended on the live trail would pass or fail
...
from codex_crew.apps.builtins.issue_radar.backend import pipeline_fold as fold
from codex_crew.apps.builtins.issue_radar.backend import pipeline_routes as routes
...
def test_pr_extraction_pull_url_anywhere() -> None:
    rec = {"event": "pr_green", "details": {"url": "https://github.com/o/CodexCrew/pull/5600"}}
...
def test_the_shard_name_matches_what_the_writers_produce(tmp_path: Path) -> None:
    ...
    """
    got = fold.queue_path(repo="mikutellyourworld/Codex-Crew", root=tmp_path).name
    assert got == "gh-autofix-dispatch-queue.kirodotdev__CodexCrew.jsonl"
