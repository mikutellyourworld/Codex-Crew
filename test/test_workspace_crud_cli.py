"""Unit tests for the ``codexcrew workspace`` CLI subcommand group.
...
def _mock_sel():
    ...
    with unittest.mock.patch("codex_crew.sel.sel"):
        yield
...
class TestWorkspaceArgparse:
    ...
    def test_workspace_subparser_exists(
        ...
        for subcmd, argv in [
            ("list", ["codexcrew", "workspace", "list"]),
            ("create", ["codexcrew", "workspace", "create", "--name", "newtest"]),
            ("update", ["codexcrew", "workspace", "update", "staging"]),
            ("delete", ["codexcrew", "workspace", "delete", "staging"]),
        ]:
            with (
                unittest.mock.patch("codex_crew.config.loader.config_path", return_value=cfg_path),
                unittest.mock.patch("codex_crew.config.loader.config_dir", return_value=tmp_path),
    ...
    def test_create_requires_name_flag(self) -> None:
        ...
        with (
            unittest.mock.patch("sys.argv", ["codexcrew", "workspace", "create"]),
    ...
    def test_update_accepts_positional_name_and_dir(
        ...
        with (
            ...
            unittest.mock.patch(
                "sys.argv",
                ["codexcrew", "workspace", "update", "staging", "--dir", "new-path"],
            ),
        ):
            main()
        out = capsys.readouterr().out
    ...
    def test_delete_accepts_positional_name(
        ...
        with (
            ...
            unittest.mock.patch(
                "sys.argv",
                ["codexcrew", "workspace", "delete", "staging"],
            ),
        ):
            main()
        out = capsys.readouterr().out
...
class TestWorkspaceDispatch:
    ...
    def test_dispatch_routes_to_handle_workspace(self, tmp_path: Path) -> None:
        ...
        with (
            unittest.mock.patch("codex_crew.config.loader.config_path", return_value=cfg_path),
            unittest.mock.patch("sys.argv", ["codexcrew", "workspace", "list"]),
            unittest.mock.patch("codex_crew.cli_commands._handle_workspace") as mock_handler,
        ):
            main()
...
class TestWorkspaceList:
    """Test ``codexcrew workspace list`` output format."""
...
class TestWorkspaceCreate:
    """Test ``codexcrew workspace create`` error paths."""
    ...
    def test_create_duplicate_name_exits_nonzero(
        ...
        with (
            ...
            unittest.mock.patch(
                "sys.argv",
                ["codexcrew", "workspace", "create", "--name", "default"],
            ),
            pytest.raises(SystemExit) as exc_info,
        ):
            main()
...
class TestWorkspaceUpdate:
    """Test ``codexcrew workspace update`` error paths."""
    ...
    def test_update_nonexistent_exits_nonzero(
        ...
        with (
            ...
            unittest.mock.patch(
                "sys.argv",
                ["codexcrew", "workspace", "update", "nonexistent", "--dir", "/x"],
            ),
            pytest.raises(SystemExit) as exc_info,
        ):
            main()
...
class TestWorkspaceDelete:
    """Test ``codexcrew workspace delete`` error paths."""
    ...
    def test_delete_default_workspace_exits_nonzero(
        ...
        with (
            ...
            unittest.mock.patch(
                "sys.argv",
                ["codexcrew", "workspace", "delete", "default"],
            ),
            pytest.raises(SystemExit) as exc_info,
        ):
            main()
    ...
    def test_delete_agent_referenced_workspace_exits_nonzero(
        ...
        with (
            ...
            unittest.mock.patch(
                "sys.argv",
                ["codexcrew", "workspace", "delete", "oncall"],
            ),
            pytest.raises(SystemExit) as exc_info,
        ):
            main()
    ...
    def test_delete_nonexistent_workspace_exits_nonzero(
        ...
        with (
            ...
            unittest.mock.patch(
                "sys.argv",
                ["codexcrew", "workspace", "delete", "nonexistent"],
            ),
            pytest.raises(SystemExit) as exc_info,
        ):
            main()
...
class TestWorkspaceDirContainmentMessage:
    ...
    def test_create_rejects_escaping_dir_with_actionable_message(
        ...
        with (
            unittest.mock.patch("codex_crew.config.loader.config_path", return_value=cfg_path),
            unittest.mock.patch("codex_crew.config.loader.config_dir", return_value=tmp_path),
            unittest.mock.patch("codex_crew.cli_commands.config_dir", return_value=tmp_path),
            unittest.mock.patch(
                "sys.argv",
                ["codexcrew", "workspace", "create", "--name", "ws1", "--dir", bad_dir],
            ),
            pytest.raises(SystemExit) as exc_info,
        ):
            main()
    ...
    def test_update_rejects_escaping_dir_with_actionable_message(
        ...
        with (
            ...
            unittest.mock.patch(
                "sys.argv",
                ["codexcrew", "workspace", "update", "staging", "--dir", "/etc"],
            ),
            pytest.raises(SystemExit) as exc_info,
        ):
            main()
    ...
    def test_relative_dir_still_accepted(
        ...
        with (
            ...
            unittest.mock.patch(
                "sys.argv",
                ["codexcrew", "workspace", "create", "--name", "ws2", "--dir", "workspace-ws2"],
            ),
        ):
            main()
    ...
    def test_unknown_tilde_user_refuses_instead_of_crashing(
        ...
        with (
            ...
            unittest.mock.patch(
                "sys.argv",
                ["codexcrew", "workspace", "create", "--name", "tildews", "--dir", bad_dir],
            ),
            pytest.raises(SystemExit) as exc_info,
        ):
            main()
    ...
    def test_data_home_root_refused_in_both_forms(self, monkeypatch, tmp_path: Path) -> None:
        ...
        ``config_dir() / ws_dir`` WITHOUT expanding ``~``, so `~/.codex-crew`
        became `<home>/~/.codex-crew` there — unequal to the root, so it slipped
    ...
    def test_root_dir_cli_refuses_with_message(
        ...
        with (
            ...
            unittest.mock.patch(
                "sys.argv",
                ["codexcrew", "workspace", "create", "--name", "rootws", "--dir", str(tmp_path)],
            ),
            pytest.raises(SystemExit) as exc_info,
        ):
            main()
    ...
    def test_keystone_paths_refused_as_workspace_dir(self, monkeypatch, keystone: str) -> None:
        ...
        the literal ``~/.codex-crew`` / ``~/.codexcrew`` prefixes rather than
    ...
    def test_absolute_dir_inside_home_is_accepted(
        ...
        with (
            ...
            unittest.mock.patch(
                "sys.argv",
                ["codexcrew", "workspace", "create", "--name", "insidews", "--dir", inside],
            ),
        ):
            main()
    ...
    def test_help_does_not_advertise_absolute_paths(self) -> None:
        ...
        with (
            unittest.mock.patch("sys.argv", ["codexcrew", "workspace", "create", "--help"]),
