...
- An app agent's ``@codexcrew-cron`` / ``@codexcrew-core`` tool references get
...
class TestManagedRefsMaterialized:
    def test_managed_tool_refs_get_specs(self):
        from codex_crew.apps.bridges import _materialize_managed_refs

        data: dict[str, Any] = {"tools": ["fs_read", "@codexcrew-cron", "@codexcrew-core"]}
        _materialize_managed_refs(data)
        assert data["mcpServers"]["codexcrew-cron"]["args"][-1] == "mcp-cron"
        assert data["mcpServers"]["codexcrew-core"]["args"][-1] == "mcp-core"

    def test_existing_entry_is_not_overwritten(self):
        from codex_crew.apps.bridges import _materialize_managed_refs

        data: dict[str, Any] = {
            "tools": ["@codexcrew-cron"],
            "mcpServers": {"codexcrew-cron": {"command": "custom"}},
        }
        _materialize_managed_refs(data)
        assert data["mcpServers"]["codexcrew-cron"]["command"] == "custom"
...
class TestNamespacedAgentFileCheck:
    ...
    def test_namespaced_file_with_matching_name_counts(self, tmp_path, monkeypatch):
        ...
        from codex_crew.dashboard.handlers.agents import _namespaced_agent_file_exists
