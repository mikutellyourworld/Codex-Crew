"""Tests for codex_crew.dashboard.handlers.usage."""
...
import codex_crew.dashboard.handlers.usage as usage_mod
...
class TestParseSessions:
    ...
    def test_refused_transcripts_are_reported_not_swallowed(self, tmp_path, caplog):
        ...
        with patch.object(usage_mod, "_SESSIONS_DIR", d), patch.object(
            usage_mod, "validate_file_path", return_value=None
        ), caplog.at_level(logging.WARNING, logger="codex_crew.dashboard.handlers.usage"):
            r = _parse_sessions()
        assert r["total_sessions"] == 0
...
class TestGetUsageCache:
    def test_returns_cache(self):
        ...
        with patch.dict(
            "sys.modules",
            {"codex_crew.dashboard.handlers.sessions": MagicMock(_usage_cache=mock_cache)},
        ):
            assert get_usage_cache() == {"credits_used": 42}

    def test_empty_cache(self):
        with patch.dict(
            "sys.modules", {"codex_crew.dashboard.handlers.sessions": MagicMock(_usage_cache={})}
        ):
            assert get_usage_cache() == {}

    def test_import_error(self):
        with patch.dict("sys.modules", {"codex_crew.dashboard.handlers.sessions": None}):
            assert get_usage_cache() == {}
...
class TestReadEffectiveAgent:
    ...
    def test_reads_resolved_agent_off_client(self):
        inner = type("C", (), {"_agent": "codexcrew"})()
        assert read_effective_agent(inner) == "codexcrew"

    def test_walks_nested_handle(self):
        handle = type("H", (), {"_agent": "codexcrew-lite"})()
        mid = type("SessionProvider", (), {"_handle": handle})()
        outer = type("P", (), {"client": mid})()
        assert read_effective_agent(outer) == "codexcrew-lite"
    ...
    def test_reads_agent_off_the_runtime(self):
        ...
        runtime = type("Runtime", (), {"_agent": "codexcrew"})()
        handle = type("Handle", (), {"_runtime": runtime})()
        provider = type("SessionProvider", (), {"_handle": handle})()
        assert read_effective_agent(provider) == "codexcrew"
