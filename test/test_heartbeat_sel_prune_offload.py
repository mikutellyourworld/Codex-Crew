...
import codex_crew.heartbeat as hb_mod
from codex_crew.heartbeat import HeartbeatService

_PRUNE_COUNTER = "codexcrew.sel.prune_failed.count"
...
def _service(
    ...
    *,
    ...
    monkeypatch.setattr(
        hb_mod.CodexCrewConfig, "load", classmethod(lambda cls: MagicMock())
    )
    ran_on: list[str] = []
...
class TestSelPruneFailureIsLoud:
    ...
    async def test_failure_increments_alarmable_counter(
        ...
        # Core metric names must live under the codexcrew.* namespace -- an
