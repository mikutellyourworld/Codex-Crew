...
from codex_crew.dashboard.handlers import api_file_sheet
...
def mock_sel():
    with patch("codex_crew.sel.sel") as m, \
         patch("codex_crew.dashboard.handlers.files.is_sensitive_path", return_value=False):
        instance = MagicMock()
...
async def test_returns_cell_grid_with_sheet_names(tmp_path, mock_sel):
    ...
    with patch("codex_crew.dashboard.handlers._validate_dashboard_path", return_value=str(f)):
...
def test_zip_expansion_cap_refuses_before_parse(tmp_path):
    ...
    with pytest.MonkeyPatch.context() as mp:
        mp.setattr("codex_crew.dashboard.handlers.files._SHEET_MAX_EXPANDED_BYTES", 1024)
...
def test_zip_member_count_cap_refuses_before_parse(tmp_path):
    ...
    with pytest.MonkeyPatch.context() as mp:
        mp.setattr("codex_crew.dashboard.handlers.files._SHEET_MAX_MEMBERS", 10)
        with pytest.raises(_SheetRefusal) as exc:
            _load_sheet_payload(open(f, "rb"), max_bytes=_MAX_UPLOAD_BYTES)
    assert exc.value.status == 413
...
def test_eocd_declared_entry_count_refuses_before_zipfile_construction(tmp_path):
    ...
    from codex_crew.dashboard.handlers.files import _vet_zip_eocd
...
def test_oversized_cell_text_is_truncated():
    from codex_crew.dashboard.handlers.files import _SHEET_MAX_CELL_CHARS
...
def test_workbook_text_budget_refuses_amplified_shared_strings():
    ...
    with pytest.MonkeyPatch.context() as mp:
        mp.setattr("codex_crew.dashboard.handlers.files._SHEET_MAX_TEXT_CHARS", 4000)
        with pytest.raises(_SheetRefusal) as exc:
...
async def test_rejects_sensitive_path(tmp_path):
    ...
    with patch("codex_crew.sel.sel", return_value=MagicMock()), \
         patch("codex_crew.dashboard.handlers.files.is_sensitive_path", return_value=True), \
         patch("codex_crew.dashboard.handlers._validate_dashboard_path", return_value=str(f)):
        async with TestClient(TestServer(_make_app())) as client:
            resp = await client.get(f"/api/file-sheet?path={f}")
...
async def test_missing_file_answers_404(tmp_path, mock_sel):
    ...
    with patch("codex_crew.dashboard.handlers._validate_dashboard_path", return_value=str(ghost)):
        async with TestClient(TestServer(_make_app())) as client:
...
async def test_oversized_file_answers_413(tmp_path, mock_sel):
    ...
    with patch("codex_crew.dashboard.handlers._validate_dashboard_path", return_value=str(f)), \
         patch("codex_crew.dashboard.handlers.files._MAX_UPLOAD_BYTES", 8):
        async with TestClient(TestServer(_make_app())) as client:
            resp = await client.get(f"/api/file-sheet?path={f}")
...
async def test_cancellation_during_parse_still_audits(tmp_path, mock_sel):
    ...
    with patch("codex_crew.dashboard.handlers._validate_dashboard_path", return_value=str(f)), \
         patch(
             "codex_crew.dashboard.handlers.files.asyncio.to_thread",
...
def test_the_sheet_vet_routes_through_the_shared_module(tmp_path, monkeypatch):
    ...
    from codex_crew.zip_vet import ZipInventoryRejected
