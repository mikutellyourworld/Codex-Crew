...
from codex_crew.slack.outbound import PostedOptions, expire_options
...
def _recs(state, key):
    ...
    """
    from codex_crew.dashboard.chat_utils import effective_session_key, options_records
...
def _set_recs(state, key, records):
    ...
    from codex_crew.dashboard.chat_utils import effective_session_key, set_options_records
...
class TestLifecycleOnTheSlot:
    ...
    async def test_a_raising_slot_registry_cannot_break_the_turn(
        ...
        from codex_crew.dashboard.chat_utils import expire_slack_options
    ...
    async def test_a_click_clears_the_record_a_mirroring_session_holds(
        ...
        from codex_crew.slack import interactions
    ...
    def test_linking_registers_the_thread_so_a_click_can_route_back(
        ...
        """
        from codex_crew.dashboard.chat_utils import slack_options_owner_key
...
class TestTurnEntryWiring:
    ...
    async def test_slack_inbound_turn_expires(self, monkeypatch):
        ...
        from codex_crew.slack import transport_dispatch
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("codex_crew.slack.transport_dispatch", "handle_message_transport"),
            ("codex_crew.slack.handler", "handle_message"),
        ],
    )
...
def _slack_app(state):
    from codex_crew.dashboard.chat_slack import api_chat_slot_slack_link
...
class TestNonStringKeyIsRefused:
    ...
    def test_a_non_string_key_returns_none_instead_of_spinning(self):
        from codex_crew.dashboard import chat_utils
...
class TestUnlinkSpendsTheControl:
    ...
    async def test_unlink_expires_a_live_options_control(self, tmp_path, monkeypatch):
        from codex_crew.dashboard.chat_slack import api_chat_slot_slack_unlink

        monkeypatch.setattr("codex_crew.dashboard.chat_slack.sel", lambda: MagicMock())
    ...
    async def test_a_relink_during_the_expiry_await_is_not_clobbered(
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_slack.expire_slack_options", _relink_midway
        )
    ...
    async def test_an_unraced_unlink_still_tears_the_link_down(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_slack.expire_slack_options", _no_relink)
...
class TestEveryOutstandingControlIsExpired:
    ...
    async def test_a_selection_made_under_the_lock_is_not_overwritten(self, tmp_path):
        ...
        from codex_crew.slack.outbound import options_edit_lock
...
class TestControlPostedAfterTheWindowIsSpent:
    ...
    def test_options_are_recorded_under_the_threads_live_owner(self):
        ...
        from codex_crew.slack import handler, transport_dispatch
    ...
    async def test_backfill_expires_when_the_transcript_advanced_mid_drain(
        ...
        """
        from codex_crew.dashboard import chat_slack
        from codex_crew.dashboard.chat_backfill import BackfillSelection
    ...
    def test_an_ansi_split_credential_in_a_choice_does_not_reach_slack(self):
        ...
        """
        from codex_crew.slack.format import strip_ansi
    ...
    async def test_expiry_marks_the_control_terminal(self, tmp_path):
        ...
        from codex_crew.slack.outbound import claim_options_answer
    ...
    def test_send_message_options_use_the_safe_fallback_stub(self):
        ...
        from codex_crew.dashboard.handlers import messaging
    ...
    def test_a_live_control_is_never_evicted_to_bound_the_store(self):
        ...
        from codex_crew.dashboard import chat_utils
        from codex_crew.dashboard.chat_utils import options_records, set_options_records
    ...
    def test_a_channel_mention_in_a_choice_is_inert_in_the_mrkdwn_summary(self):
        ...
        """
        from codex_crew.slack.format import build_options_selected_blocks
