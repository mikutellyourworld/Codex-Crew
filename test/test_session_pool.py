...
def _isolate_config_dir(tmp_path, monkeypatch):
    ...
    the developer's real ``~/.codexcrew``.
    """
    monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path / "codexcrew_home"))


def _make_cfg(
    pool_size: int = 2, pool_agent: str = "codexcrew", pool_ttl_secs: int = 1800
) -> MagicMock:
    cfg = MagicMock()
    ...
    cfg.agent.model = "auto"  # match real CodexCrewConfig default
    return cfg
...
def _make_manager(pool_size: int = 2, pool_agent: str = "codexcrew", pool_ttl_secs: int = 1800):
    ...
    with patch(
        "codex_crew.session.default_project_dir", return_value="/home/user/.codexcrew/workspace"
    ):
...
class TestFillWarmPool:
    ...
    async def test_cancelled_error_cleans_up_via_finally(self):
        ...
        with patch("codex_crew.session._sync_kill_provider") as mock_kill:
...
class TestLivenessDrainLoop:
    ...
    async def test_dead_provider_discarded_healthy_used(self):
        ...
        mgr, _ = _make_manager(pool_agent="codexcrew")
        ...
        pooled = await mgr._drain_and_claim("codexcrew")
    ...
    async def test_unanswerable_liveness_probe_is_discarded_fail_closed(self):
        ...
        """
        mgr, _ = _make_manager(pool_agent="codexcrew", pool_ttl_secs=1)
...
class TestGetOrCreatePoolIntegration:
    ...
    async def test_claims_from_pool_when_agent_matches(self):
        ...
        mgr, factory = _make_manager(pool_agent="codexcrew")
        ...
        provider, is_new, _ = await mgr.get_or_create(
            "test-key", agent="codexcrew", channel_id="ch-1"
        )
    ...
    async def test_claim_forwards_canonical_crew_identity_to_rekey(self):
        ...
        provider, _, _ = await mgr.get_or_create(
            "test-key", agent="codexcrew", channel_id="ch-1", crew_agent="pr-reviewer"
        )
    ...
    async def test_skips_pool_when_resume_sid_set(self):
        ...
        provider, is_new, _ = await mgr.get_or_create("test-key", agent="codexcrew")
    ...
    async def test_skips_pool_when_cwd_set(self):
        ...
        provider, is_new, _ = await mgr.get_or_create(
            "test-key", agent="codexcrew", cwd="/Users/alice/workspace/proj"
        )
    ...
    async def test_claims_pool_with_model_override_and_switches(self):
        ...
        with patch.object(type(mgr), "_resolve_agent_model", return_value="default-model"):
            provider, is_new, _ = await mgr.get_or_create(
                "test-key", agent="codexcrew", model="custom-model"
            )
...
class TestTTLExpiration:
    ...
    async def test_ttl_zero_disables_check(self):
        ...
        mgr, _ = _make_manager(pool_agent="codexcrew", pool_ttl_secs=0)
        old = _make_provider()
        # Very old provider
    ...
    async def test_stale_drain_triggers_replenish(self):
        ...
        await mgr._drain_and_claim("codexcrew")
    ...
    async def test_claim_ttl_discard_logs_info_dead_keeps_warning(self, caplog):
        ...
        with patch("codex_crew.session._sync_kill_provider"):
...
class TestModelMatchesPoolDefault:
    ...
    async def test_pool_claimed_when_model_matches_agent_default(self):
        ...
        with patch.object(type(mgr), "_resolve_agent_model", return_value="claude-opus-4.6"):
            provider, is_new, _ = await mgr.get_or_create(
                "test-key", agent="codexcrew", model="claude-opus-4.6"
            )
    ...
    async def test_pool_claimed_when_model_differs_with_post_switch(self):
        ...
        with patch.object(type(mgr), "_resolve_agent_model", return_value="claude-opus-4.6"):
            provider, is_new, _ = await mgr.get_or_create(
                "test-key", agent="codexcrew", model="claude-sonnet-4.6"
            )
    ...
    async def test_pool_claude_backend_translates_canonical_key_on_switch(self):
        ...
        with patch.object(type(mgr), "_resolve_agent_model", return_value="default-model"):
            await mgr.get_or_create("test-key", agent="codexcrew", model="opus-4.8-1m")
    ...
    async def test_pool_skipped_when_model_set_but_pool_disabled(self):
        ...
        mgr, factory = _make_manager(pool_size=0, pool_agent="codexcrew")
        mgr._drain_and_claim = AsyncMock()

        with patch.object(type(mgr), "_resolve_agent_model", return_value="claude-opus-4.6"):
            await mgr.get_or_create("test-key", agent="codexcrew", model="claude-opus-4.6")
    ...
    async def test_none_model_still_claims_from_pool(self):
        ...
        provider, is_new, _ = await mgr.get_or_create("test-key", agent="codexcrew", model=None)
    ...
    async def test_unusable_model_is_withheld_not_raised_on_claim(self):
        ...
        with patch.object(type(mgr), "_resolve_agent_model", return_value="claude-sonnet-4.6"):
            provider, _is_new, _resumed = await mgr.get_or_create(
                "test-key", agent="codexcrew", model="claude-opus-4.8"
            )
    ...
    async def test_namespaced_pin_resolves_on_claim_like_a_cold_start(self):
        ...
        with patch.object(type(mgr), "_resolve_agent_model", return_value="claude-sonnet-4.6"):
            provider, _is_new, _resumed = await mgr.get_or_create(
                "test-key", agent="codexcrew", model="openrouter::z-ai/glm-5.3-flash"
            )
...
class TestPoolDisabledSkipsClaim:
    ...
    async def test_pool_size_zero_skips_drain_and_claim(self):
        ...
        await mgr.get_or_create("test-key", agent="codexcrew")
...
class TestReloadProviderFactoryRefillsPool:
    ...
    async def test_reload_resets_pool_started_and_refills(self):
        ...
        with patch("codex_crew.session.CodexCrewConfig.load") as mock_load:
...
class TestDefaultProjectDir:
    def test_returns_realpath_of_workspace_dir(self, tmp_path):
        ...
        ws.mkdir()
        with patch("codex_crew.config.loader.workspace_dir_for", return_value=ws):
            from codex_crew.config.loader import default_project_dir
    ...
    def test_returns_empty_when_dir_missing(self, tmp_path):
        ...
        with patch("codex_crew.config.loader.workspace_dir_for", return_value=missing):
            from codex_crew.config.loader import default_project_dir
    ...
    def test_returns_empty_when_sensitive(self, tmp_path):
        ws = tmp_path / "workspace"
        ws.mkdir()
        with patch("codex_crew.config.loader.workspace_dir_for", return_value=ws), patch(
            "codex_crew.security.is_sensitive_path", return_value=True
        ):
            from codex_crew.config.loader import default_project_dir
    ...
    def test_returns_empty_on_exception(self):
        with patch("codex_crew.config.loader.workspace_dir_for", side_effect=RuntimeError("boom")):
            from codex_crew.config.loader import default_project_dir
...
class TestPoolCwd:
    def test_pool_cwd_set_from_default_project_dir(self):
        ...
        cfg = _make_cfg()
        with patch("codex_crew.session.default_project_dir", return_value="/custom/workspace"):
            mgr = SessionManager(cfg)
    ...
    def test_pool_cwd_empty_when_no_workspace(self):
        ...
        cfg = _make_cfg()
        with patch("codex_crew.session.default_project_dir", return_value=""):
            mgr = SessionManager(cfg)
    ...
    async def test_pool_claimed_when_cwd_matches_pool_cwd(self):
        ...
        provider, is_new, _ = await mgr.get_or_create(
            "test-key",
            agent="codexcrew",
            cwd="/home/user/.codexcrew/workspace",  # same as _pool_cwd
        )
    ...
    async def test_fill_warm_pool_passes_pool_cwd(self):
        ...
        factory.assert_called_once()
        assert factory.call_args.kwargs.get("cwd") == "/home/user/.codexcrew/workspace"
...
class TestDiscardReaping:
    ...
    async def test_wedged_shutdown_is_bounded_and_hard_killed(self, monkeypatch):
        ...
        with ThreadPoolExecutor(max_workers=1) as private_executor, patch(
            "codex_crew.session._sync_kill_provider"
        ) as mock_kill, patch(
            "codex_crew.session.subprocess_executor", return_value=private_executor
        ):
            pooled = await asyncio.wait_for(mgr._drain_and_claim("codexcrew"), timeout=5)
    ...
    async def test_dispatch_hard_kill_never_runs_inline_when_executor_down(self):
        ...
        provider = _make_provider()
        with patch("codex_crew.session.subprocess_executor", return_value=dead_executor), \
                patch("codex_crew.session._sync_kill_provider", side_effect=_record_thread):
    ...
    async def test_one_failing_hard_kill_does_not_abort_batch_discard(self):
        ...
        with patch("codex_crew.session._sync_kill_provider", side_effect=_kill):
            await mgr._sweep_warm_pool_once()
