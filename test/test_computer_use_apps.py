...
from codex_crew.computer_use import apps_macos, macos_ffi, policy
from codex_crew.computer_use.types import AppRef, ComputerUseError, PolicyConfig
...
def test_resolve_app_prefers_layer_zero_window_owner_over_decoy_helper(
    ...
    assert app.window_title == "CodexCrew — GitHub"
...
def test_a_plist_on_the_sensitive_path_floor_is_never_read(
    ...
    ``codex_crew.security`` attribute would not be observed and the test would pass
...
def test_identity_for_falls_back_to_the_process_name(monkeypatch: pytest.MonkeyPatch):
    ...
    """
    monkeypatch.setattr(apps_macos, "resolve_identity", lambda pid: apps_macos.AppIdentity())
    from codex_crew.computer_use.types import AppRef
...
class TestBrowserHostedDashboardIsRefused:
    ...
    CodexCrew's own Settings page is where the computer-use enable lives, and that
    enable sits on the keystone precisely so the agent cannot reach it. Driving our
    own window would route around it — which is why ``codexcrew_self`` is the one
    ...
    @pytest.mark.parametrize(
        "title",
        [
            "Codex Crew",  # the plain tab title
            "(3) Codex Crew",  # the unread-badge prefix (App.tsx)
            "Artifacts — Codex Crew",  # a popout frame's suffix
            "codexcrew",  # the no-space spelling
            "CODEX CREW",  # case must not matter
        ],
    )
    ...
    def test_the_native_app_is_still_refused_by_IDENTITY(self):
        ...
        """
        native = AppRef(
            name="Codex Crew", pid=1, bundle_id="dev.kiro.crew", window_title="Settings"
        )
...
class TestMultiWindowHostPrefersTheDeniedTitle:
    ...
    @pytest.mark.parametrize(
        "titles",
        [
            ("Hacker News", "Codex Crew"),  # dashboard in a BACKGROUND window
            ("Codex Crew", "Hacker News"),  # dashboard first
            ("", "Codex Crew"),  # untitled window listed first
            ("Hacker News", "GitHub", "Codex Crew"),  # third of three
        ],
    )
