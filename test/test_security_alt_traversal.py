...
from codex_crew.security import is_sensitive_bash_command
...
@pytest.mark.parametrize(
    ...
    [
        ...
        'D=$HOME/.codex-crew; rg . "$D"',
        'D=$HOME/.codex-crew; grep -r secret "$D"',
        'D=$HOME/.codex-crew; fd . "$D" -x cat',
        ...
        'export D=$HOME/.codex-crew; rg . "$D"',
        'readonly D=$HOME/.codex-crew; rg . "$D"',
        'declare D=$HOME/.codex-crew; rg . "$D"',
        'declare -x D=$HOME/.codex-crew; rg . "$D"',
        'typeset D=$HOME/.codex-crew; rg . "$D"',
        'local D=$HOME/.codex-crew; grep -r x "$D"',
    ],
)
...
def test_root_check_accepts_a_flag_value_as_a_candidate_root() -> None:
    ...
    assert security._alt_root_reaching_fence([f"--search-path={home}/.codex-crew"], {})
    assert security._alt_root_reaching_fence([f"{home}/.codex-crew"], {})
...
def test_root_check_resolves_a_root_held_in_an_assignment() -> None:
    home = os.path.expanduser("~")
    assignments = {"D": f"{home}/.codex-crew"}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        'grep -r --regexp="$HOME/.codex-crew" ./src',
        'grep -r --reg "$HOME/.kiro" ./src',
...
@pytest.mark.parametrize(
    ...
    [
        ...
        'D=$(printf %s "$HOME/.codex-crew"); rg . "$D"',
        'D=$(echo $HOME/.codex-crew); rg . "$D"',
        'D=`echo $HOME/.codex-crew`; rg . "$D"',
        'D=$(printf %s "$HOME/.codex-crew"); grep -r secret "$D"',
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        'D=$HOME/.codex-crew; rg . "$D"; D=/tmp',
        'D=$HOME/.codex-crew; grep -r secret "$D"; D=/tmp; D=/var',
    ],
)
...
def test_a_computed_assignment_is_only_fenced_when_its_stage_names_one() -> None:
    home = os.path.expanduser("~")
    fenced = security._alt_pipeline_stages(f'D=$(printf %s "{home}/.codex-crew"); rg . "$D"')
...
def _fenced_root() -> str | None:
    ...
    * fenced in THIS environment, because the suite pins ``CODEXCREW_HOME`` per test,
      so ``~/.codex-crew`` is not where the fence lives while the tests run -- and the
