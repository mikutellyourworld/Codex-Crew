...
class TestCatalog:
    ...
    def test_token_mint_is_blocked_in_both_the_cli_and_module_forms(self):
        """`codexcrew token` mints a signed dashboard token that authenticates to EVERY gateway
        ...
        The module form is why the union matters. `python -m codex_crew token` mints the
        ...
        for blocked in (
            "codexcrew token",
            "codexcrew token --port 6777",
            'codexcrew "token"',
            "codexcrew -v --no-jail token",
            "kiro-crew token",
            # The module form, in the spellings a shell accepts.
            "python -m codex_crew token",
            "python3 -m codex_crew token --port 6777",
            "python -mcodex_crew token",
            "python -m codex_crew pod token",
            ...
            "python -X dev -m codex_crew token",
            "python -W ignore -m codex_crew token",
            "python -Q new -m codex_crew token",
            "python -X utf8 -X dev -m codex_crew token",
            "python3 -B -X dev -m codex_crew pod token",
            # ATTACHED operands are one token and need no skip — covered because the
            # separate-operand fix must not break them.
            "python -Xdev -m codex_crew token",
            "python -Wignore -m codex_crew token",
            ...
            'python -c "from codex_crew.cli import main; main()" token',
            "python3 -c 'import codex_crew.cli; codex_crew.cli.main()' token",
            'python -c "from codex_crew import cli; cli.main()" token --port 6777',
            # Attached spelling: payload inside the same token.
            'python -c"import codex_crew.cli;codex_crew.cli.main()" token',
            # Behind an interpreter flag that takes a separate operand.
            'python -X dev -c "import codex_crew.cli; codex_crew.cli.main()" token',
            # Reached without a literal `import` statement.
            "python -c \"__import__('codex_crew.cli').cli.main()\" token",
            ...
            # verb requirement holds everywhere else (`codexcrew doctor` is legitimate) but is
            # not enforceable here. Found in review (GPT 5.6).
            "python -c \"import sys; sys.argv.append('token'); "
            'from codex_crew.cli import main; main()"',
            "python -c \"from codex_crew.cli import main; import sys; "
            "sys.argv=['x','token']; main()\"",
            'python -c "from codex_crew.cli import main; main([\'token\'])"',
            'python -c "import codex_crew.cli as c; c.main()"',
            'python -X dev -c "import codex_crew.cli"',
            ...
            "python - <<'PY'\nfrom codex_crew.cli import main; main()\nPY",
            "python3 - <<EOF\nimport codex_crew.cli\nEOF",
            "echo 'from codex_crew.cli import main; main()' | python -",
            "python -X dev - <<'PY'\nimport codex_crew.cli\nPY",
            "python << 'PY'\nimport codex_crew.cli; codex_crew.cli.main()\nPY",
        ):
        ...
        for allowed in (
            ...
            # and `\bcodex_crew\b` does not match inside `codex_crew_notes`.
            "cat codex_crew_notes.txt | python -",
            "python -c 'import json; print(json.dumps({}))'",
            "python -c 'import sys; print(sys.version)'",
            # Mentions the import name as DATA for another program, not as code we will run.
            "grep -r codex_crew src/",
            "echo 'import codex_crew.cli' > /tmp/note.txt",
        ):
...
class TestSelfProtectionFlagInterposition:
    ...
    ``codexcrew -v restart`` runs the same restart as ``codexcrew restart``. Four
    ...
    _TEMPLATES = {
        "self-protection-restart": "codexcrew {flags} restart",
        "self-protection-update": "codexcrew {flags} update",
        "self-protection-gateway-restart": "codexcrew {flags} gateway restart",
        "self-protection-cloud": "codexcrew {flags} cloud destroy",
        ...
        "self-protection-cron-adopt": "codexcrew {flags} cron adopt",
        ...
        "self-protection-dev-mode-out-of-root-confirm": (
            "codexcrew {flags} app dev my-app --confirm-out-of-install-root"
        ),
        # The kill rules key on the kill TARGET, not a CLI subcommand; their gap
        # is between the kill verb and the product name.
        "self-protection-kill": "pkill {flags} codexcrew",
        "self-protection-kill-interpreter": (
            "python -c \"import os; os.system('pkill {flags} -f codexcrew')\""
        ),
    }
    ...
    def test_widened_patterns_still_require_the_subcommand_token(self):
        ...
        for allowed in (
            "codexcrew -v",
            "codexcrew --verbose",
            "codexcrew --no-jail doctor",
            "codexcrew -v status",
            "codexcrew -vv cloud status",
            ...
            "codexcrew doctor restart",
            "codexcrew gateway status restart",
            "codexcrew cloud status destroy",
            "python -m codex_crew doctor restart",
            "python -m codex_crew gateway status restart",
            "python -m codex_crew cloud status destroy",
        ):
            assert not security.is_denied(
                allowed, denied_regexes=effective
            ), f"false positive on {allowed!r}"
    ...
    def _dressings(words):
        ...
        Every entry reaches the shell as ``codexcrew [<flag>] <words...>`` after
        ...
        return {
            "bare": f"codexcrew {rest}",
            "real-flag": f"codexcrew -v {rest}",
            "backslash-escaped-flag": f"codexcrew -\\v {rest}",  # -\v -> -v
            "escaped-verb-letter": f"codexcrew \\{first}{tail}",  # \restart -> restart
            "line-continuation-flag": f"codexcrew -\\\nv {rest}",
            "continuation-before-verb": f"codexcrew \\\n{first}{tail}",
            "each-word-quoted": f"codexcrew {each_quoted}",
            "each-word-single-quoted": f"codexcrew {each_single_quoted}",
            ...
            "double-quoted-flag": f'codexcrew "-v" {rest}',  # "-v" -> -v
            "single-quoted-flag": f"codexcrew '-v' {rest}",
            "quoted-flag-and-quoted-verb": f'codexcrew "-v" {each_quoted}',
        }
    ...
    def test_self_protection_denied_under_the_full_quoting_cross(self):
        ...
        effective = self._effective()
        for rule_id, words in self._SUBCOMMANDS.items():
            for cmd in self._quoting_cross("codexcrew", list(words)):
                assert security.is_denied(
                    cmd, denied_regexes=effective
    ...
    def test_self_protection_floor_covers_every_subcommand_rule(self):
        ...
        a floor-listed rule whose template names a ``codexcrew`` CLI subcommand
        ...
        ``codexcrew ...`` template, so the derivation excludes them. The
        dev-mode confirm rule's template does start with ``codexcrew``, but its
        ...
        floor_subcommand_ids = {
            ...
            if self._TEMPLATES.get(rule_id, "").startswith("codexcrew ")
            and rule_id not in flag_keyed_floor_ids
        }
        assert set(self._SUBCOMMANDS) == floor_subcommand_ids, (
            "every floor-listed codexcrew-subcommand rule must register its "
        ...
        assert security._is_self_restart("codexcrew -\\v restart")
        assert security._is_self_update("codexcrew \\update")
        assert security._is_self_gateway_restart("codexcrew -\\v gateway restart")
        assert security._is_self_cloud_destructive("codexcrew -\\v cloud destroy")
        assert security._is_dev_mode_out_of_root_confirm(
            "codexcrew app dev x --confirm-out-of-install-'root'"
        )
    ...
    def test_self_protection_denied_under_interposed_redirection(self):
        ...
        effective = self._effective()
        for cmd in (
            "codexcrew 2>/tmp/x restart",  # attached redirect leaves fd residue
            "codexcrew > /tmp/x restart",  # separate target
            "codexcrew 2>&1 restart",
            "codexcrew restart 2>/tmp/log",  # redirect AFTER the subcommand
            "codexcrew >/dev/null -v update",  # redirect + flag
            "codexcrew > 'audit;log' restart",  # quoted ';' in the target is a filename, not a boundary
            "codexcrew 2> 'x|y' restart",  # quoted '|' in the target
        ):
            assert security.is_denied(
                cmd, denied_regexes=effective
        ...
        assert not security._is_self_restart("codexcrew > restart")
    ...
    def test_self_protection_denied_under_dollar_quoting(self):
        ...
        effective = self._effective()
        for cmd in (
            "codexcrew $'-v' restart",  # ANSI-C flag
            "codexcrew $'\\x2d\\x76' restart",  # ANSI-C hex -> -v
            'codexcrew $"-v" restart',  # locale flag
            "codexcrew $'restart'",  # ANSI-C on the verb
            "codexcrew $'-v' cloud destroy",
        ):
            assert security.is_denied(
                cmd, denied_regexes=effective
    ...
    def test_self_protection_module_form_denied_under_shell_dressing(self):
        """``python -m codex_crew <subcommand>`` dispatches the same self-action. The
        escaped module form (``python -m codex_crew -\\v restart``) slips past the
        ...
        effective = self._effective()
        for cmd in (
            "python -m codex_crew restart",
            r"python -m codex_crew -\v restart",  # escaped: regex misses, floor catches
            r"python -mcodex_crew -\v restart",  # attached -m spelling
            r"python -m codex_crew \update",
            "python -m codex_crew gateway restart",
            r"python -m codex_crew -\v cloud destroy",
        ):
            assert security.is_denied(
                cmd, denied_regexes=effective
        ...
        assert not security._is_self_restart("python -m codex_crew status")
        assert not security._is_self_cloud_destructive("python -m codex_crew cloud status")
    ...
    def test_self_protection_module_form_denied_under_version_launchers(self):
        """Every interpreter launcher spelling of ``-m codex_crew`` dispatches the
        ...
        operands (``-X dev``), and the attached ``-mcodex_crew`` form. Each is
        ...
        launchers = (
            "python -m codex_crew",
            "python3 -B -m codex_crew",
            "python3.12 -X dev -m codex_crew",
            "py -3.12 -m codex_crew",
            "python -mcodex_crew",
        )
        ...
        for allowed in (
            "py -3.12 -m codex_crew status",
            "python3.12 -X dev -m codex_crew doctor",
    ...
    def test_self_protection_floor_is_not_over_broad(self):
        ...
        assert not security._is_self_restart("codexcrew -v status")
        assert not security._is_self_cloud_destructive("codexcrew cloud status")
        assert not security._is_self_cloud_destructive("codexcrew -vv cloud status")
        ...
        assert not security._is_self_restart("echo codexcrew restart")
        assert not security._is_self_restart("grep restart /var/log/codexcrew.log")
        # gateway-restart is a distinct rule from bare restart
        assert not security._is_self_restart("codexcrew gateway restart")
...
class TestLazyPossessiveGapSplit:
    ...
    def test_split_absorbs_lazy_and_possessive_modifier(self):
        from codex_crew.security import _split_deny_frags
    ...
    def test_lazy_gap_rule_still_matches_end_to_end(self):
        from codex_crew.security import _DenyMatcher
...
class TestGreedyFragmentUnderConsume:
    ...
    def test_underconsume_detector(self):
        from codex_crew.security import _frags_can_underconsume
...
class TestUserPatternExactSemantics:
    ...
    def test_documented_bound_applies_only_where_the_bounded_engine_is_needed(self):
        ...
        from codex_crew.security import _DENY_FALLBACK_SCAN_MAX_CHARS, _DenyMatcher
...
class TestIsDeniedReDoSResistance:
    ...
    def test_mid_dotstar_chain_spam_stays_linear(self, monkeypatch):
        ...
        """
        from codex_crew.security import _DENY_MATCHER_CACHE, _deny_matcher
...
class TestBuiltinRuleMatcherShape:
    ...
    def test_no_regex_tier_matcher_is_disabled(self):
        from codex_crew.security import _deny_matcher
...
class TestSelfProtectionFloorIsAdditive:
    ...
    * **Nested shell payloads.** ``bash -c "<script>"`` hands the whole script
    ...
    def test_retained_pattern_is_a_subset_of_its_predicate(self, rule_id):
        ...
        from codex_crew.security import _is_credential_mint, _is_self_kill
    ...
    def test_home_expansion_tolerates_a_backslash_home(self, monkeypatch):
        ...
        from codex_crew.security import normalize_shell_command
...
class TestInterpreterArgvLiteralMint:
    ...
    def test_a_dynamic_exec_inline_payload_is_denied_as_opaque(self):
        ...
        `-c` invocation denied. `os.system("codexcrew token")` with the verb literal is caught by
...
class TestNameAsDataIsNotAnInvocation:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            f"sudo {_KA} {_NAME}",
            f"CODEXCREW_HOME=/tmp/h {_NAME} {_TOK}",
...
class TestCredentialMintSegmentScoping:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            f"CODEXCREW_HOME=/tmp/h CODEXCREW_PORT=6777 {_NAME} {_TOK}",
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            f"cd /workplace/x/{_NAME}-wt-y && grep -n mint src/codex_crew/{_TOK}_auth.py",
            f"tail -20 /tmp/{_NAME}-gw.log | sed 's/{_TOK}=.*/REDACTED/'",
            f"CODEXCREW_HOME=/tmp/h ./bin/{_NAME} gateway  # banner prints the auth {_TOK}",
...
class TestSelfFloorShortCircuit:
    ...
    def test_name_carrying_command_still_descends(self, monkeypatch):
        ...
        assert self._descent_calls(monkeypatch, "codexcrew token") >= 1
        assert self._descent_calls(monkeypatch, "pkill -f codexcrew") >= 1
    ...
    def test_gate_is_a_necessary_condition_not_a_name_grep(self):
        ...
        for evasive in (
            "python -m codex_crew token",  # underscored module spelling
            ...
            "kill $(pgrep -f codexcrew)",  # bare kill via substitution
    ...
    def test_gated_predicates_still_deny_the_obfuscation_corpus(self):
        ...
        assert security._is_self_kill("kill $(pgrep -f codexcrew)")
        assert security._is_self_kill("pkill -f codexcrew")
...
class TestStdinProgramTextScoping:
    ...
    ``codex_crew`` path read as a credential mint, with no ``token`` word anywhere.
    """
    ...
    BENIGN_NEIGHBOUR = (
        ...
        "isort src/codex_crew/mcp_core.py\npython3 - <<'PY'\nprint(1)\nPY",
        # The same shape behind the other two separators a frame preserves.
        "isort src/codex_crew/x.py && python3 -",
        "black src/codex_crew/security.py; python3 - <<'PY'\nprint(2)\nPY",
        # Order does not matter: the neighbour may follow the interpreter too.
        "python3 - <<PY\nprint(1)\nPY\nisort src/codex_crew/x.py",
    ...
    REAL_STDIN_REACH = (
        ...
        "echo 'import codex_crew' | python3 -",
        "echo 'import codex_crew'|python3 -",
        "echo 'import codex_crew' |python3 -",
        "echo 'import codex_crew'| python3 -",
        "cat src/codex_crew/cli.py | python3 -",
        "cat src/codex_crew/cli.py|python3 -",
        "printf 'import codex_crew'|python3",
        "echo 'import codex_crew' | python3",
        # Stdin redirect -- the file's CONTENT becomes the program.
        "python3 - < src/codex_crew/cli.py",
        "python3 -<src/codex_crew/cli.py",
        "python3 - 0< src/codex_crew/cli.py",
        ...
        "python3 - < <(echo 'import codex_crew')",
        'python3 - <<<$(printf %s "import codex_crew")',
        "python3 - <<<`printf %s 'import codex_crew'`",
        'python3 - <<<"${x:-import codex_crew}"',
        'python3 - < $(printf %s "src/codex_crew/cli.py")',
        "python3 - <<<$(cat src/codex_crew/cli.py)",
        ...
        "python3 - <<<$(true ')'; printf %s \"import codex_crew\")",
        'python3 - <<<$(echo ")" ; printf %s "import codex_crew")',
        ...
        'python <<< $(printf %s "import codex_crew")',
        'python3 <<< $(printf %s "import codex_crew")',
        'python3 < $(printf %s "src/codex_crew/cli.py")',
        ...
        "<<'PY' python -\nimport codex_crew\nPY",
        "<<PY python3 -\nimport codex_crew\nPY",
        "<src/codex_crew/cli.py python3 -",
        "< src/codex_crew/cli.py python3 -",
        "<<<'import codex_crew' python3 -",
    ...
    def test_carriers_are_the_only_search_space(self):
        ...
        piped = security.normalize_shell_command("echo 'import codex_crew' | python3 -")
        j = piped.index("python3")
        assert "import codex_crew" in list(security._stdin_program_text(piped, j))
    ...
    def test_bare_interpreter_with_a_heredoc_is_recognised_as_reading_stdin(self):
        ...
        for cmd, expect_stdin in (
            ...
            ("python <<PY\nimport codex_crew\nPY", True),
            ("python <<-PY\nimport codex_crew\nPY", True),
            ("python <<<'import codex_crew'", True),
            ("python <<< 'import codex_crew'", True),
            ('python <<< $(printf %s "import codex_crew")', True),
            ("python < prog.py", True),
            ("python script.py", False),
            ("python script.py < input.txt", False),
            ("python -c 'print(1)'", False),
            ("python -m codex_crew gateway", False),
        ):
    ...
    def test_a_pipe_anywhere_left_is_a_known_over_block(self):
        ...
        assert security.is_denied("grep codex_crew src | head; python3 -") is not None

    def test_rule_does_not_fire_on_its_own_pattern_text(self):
        """Quoting this rule must not trip it.

        ``credential-exfil-codexcrew-token``'s code comment claims this exemption
        ...
        rule = next(
            r for r in security.BUILTIN_DENIED_RULES if r.id == "credential-exfil-codexcrew-token"
        )
        for cmd in (
...
class TestDevModeConfirmFlagIsAgentInaccessible:
    ...
    def test_the_flag_is_denied_in_direct_and_nested_forms(self):
        ...
        for cmd in (
            "codexcrew app dev my-app --confirm-out-of-install-root",
            'bash -c "codexcrew app dev my-app --confirm-out-of-install-root"',
    ...
    def test_dev_mode_confirm_flag_denied_under_quote_splitting(self):
        ...
        for cmd in (
            ...
            'codexcrew app dev my-app "--confirm-out-of-install-root"',
            "codexcrew app dev my-app '--confirm-out-of-install-root'",
            'codexcrew app dev my-app --confirm-out-of-install-ro""ot',
            "codexcrew app dev my-app --confirm\\-out-of-install-root",
            "codexcrew app dev my-app --'confirm'-out-of-install-root",
            "bash -c \"codexcrew app dev my-app --confirm-out-of-install-'root'\"",
        ):
    ...
    def test_ordinary_dev_toggles_stay_allowed(self):
        ...
        assert security.is_denied("codexcrew app dev my-app") is None
        assert security.is_denied("codexcrew app dev my-app --off") is None
...
class TestSelfModuleIndexIsLinear:
    ...
    which is why the shape below carries ``codexcrew``.  Measured on base:
    ...
    SHAPES: "list[tuple[list[str], object]]" = [
        (["python", "-m", "codex_crew", "restart"], 2),
        (["python", "-mcodex_crew", "restart"], 1),
        ...
        (["python", "-mjson", "-m", "codex_crew"], 3),
        (["python", "-msomething", "-mcodex_crew"], 2),
        (["python", "-u", "-O", "-m", "codex_crew"], 4),
        ...
        (["python", "-m", "'codex_crew'"], 2),
        (["python", "-m", "codex_crew.cli"], 2),
    ]
    ...
    def test_the_floor_verdicts_are_unchanged(self):
        ...
        for text in (
            "codexcrew restart",
            "python -m codex_crew restart",
            "python -mcodex_crew restart",
            "python -mjson -m codex_crew restart",
            "python -msomething -m codex_crew restart",
            "python -u -O -m codex_crew restart",
            "python -m 'codex_crew' restart",
            "python -m codex_crew -v restart",
            "python python -m codex_crew restart",
        ):
    ...
    def test_the_scan_is_linear_not_quadratic(self, monkeypatch):
        ...
        def build(n: int) -> str:
            return " ".join(["python"] * n + ["codexcrew", "restart"])
    ...
    def test_the_padded_shape_that_does_not_open_the_gate_stays_cheap(self):
        ...
        assert security._self_floor_can_fire("python codexcrew")
...
class TestPythonStdinDetectorStepsOverOutputRedirects:
    ...
    def test_the_floor_denies_the_stdin_program_behind_a_redirect(self):
        ...
        payload = "from codex_crew.cli import main; main()"
        for cmd in (
...
class TestOutputRedirectScanQuoting:
    ...
    def test_the_quoted_paren_bypass_is_denied_end_to_end(self):
        ...
        prog = "'from codex_crew.cli import main; main()'"
