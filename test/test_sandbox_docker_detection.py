...
Related issue: https://github.com/mikutellyourworld/Codex-Crew/issues/1617
"""
...
def _clear_cache() -> None:
    from codex_crew.sandbox import is_docker_container
...
class TestIsDockerContainer:
    """Unit tests for :func:`codex_crew.sandbox.is_docker_container`."""
...
class TestWrapArgvDockerGuidance:
    ...
    def test_docker_guidance_mentions_seccomp(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        from codex_crew.sandbox import SandboxUnavailableError, wrap_argv
    ...
    def test_docker_guidance_mentions_allow_unsandboxed(
        ...
        """Error message references CODEXCREW_ALLOW_UNSANDBOXED."""
        from codex_crew.sandbox import SandboxUnavailableError, wrap_argv

        self._patch_no_backend_docker(monkeypatch)
        with pytest.raises(SandboxUnavailableError) as exc_info:
            wrap_argv(["kiro-cli", "chat"], mode="auto")
        assert "CODEXCREW_ALLOW_UNSANDBOXED" in str(exc_info.value)
    ...
    def test_bare_metal_no_backend_gets_generic_guidance(
        ...
        assert "CODEXCREW_ALLOW_UNSANDBOXED" not in str(exc_info.value)
    ...
    def test_apparmor_restricted_host_gets_profile_guidance(
        ...
        assert message.index("codexcrew service install") < message.index("last resort")
