"""Agent-record free text must leave GET /api/config/codexcrew masked (#8717).
...
from codex_crew.config import loader as L
from codex_crew.config.loader import CodexCrewConfig
from codex_crew.config.sections import CodexCrewAgentConfig
...
def _cfg_with_agents() -> CodexCrewConfig:
    cfg = CodexCrewConfig()
    cfg.agents["shady"] = CodexCrewAgentConfig(
        description=_CRED_DESCRIPTION, triggers=_EXFIL_TRIGGERS
    )
    cfg.agents["plain"] = CodexCrewAgentConfig(
...
def test_every_unguarded_str_field_is_in_the_masked_set():
    ...
    cover every ``str`` field of ``CodexCrewAgentConfig``.
    ...
    str_fields = {f.name for f in dataclasses.fields(CodexCrewAgentConfig) if f.type in ("str", str)}
...
class TestMaskedConfigDict:
    ...
    def test_extended_fields_mask_when_suspicious(self):
        ...
        cfg = CodexCrewConfig()
        cfg.agents["odd"] = CodexCrewAgentConfig(
        ...
        assert record["source"] == "codexcrew"
    ...
    def test_suspicious_record_key_is_removed_not_leaked(self):
        ...
        cfg = CodexCrewConfig()
        cfg.agents[_CRED_DESCRIPTION] = CodexCrewAgentConfig(description="x")
        cfg.agents["plain"] = CodexCrewAgentConfig(description=_BENIGN_DESCRIPTION)
        masked = _masked_config_dict(cfg)
    ...
    def test_name_references_to_removed_record_are_masked(self):
        ...
        cfg = CodexCrewConfig()
        cfg.agents[_CRED_DESCRIPTION] = CodexCrewAgentConfig()
...
class TestConfigEndpointWire:
    ...
    async def test_get_response_masks_agent_free_text(self, tmp_path, monkeypatch):
        ...
        async with TestClient(TestServer(app)) as client:
            resp = await client.get("/api/config/codexcrew")
            assert resp.status == 200
            body = await resp.json()
        assert body["agents"]["codexcrew"]["description"] == _SENSITIVE_MASK
        assert body["agents"]["codexcrew"]["triggers"] == _SENSITIVE_MASK
    ...
    async def test_patch_echo_masks_agent_free_text(self, tmp_path, monkeypatch):
        ...
        with patch("codex_crew.config.loader.config_path", return_value=cfg_path):
            async with TestClient(TestServer(app)) as client:
                resp = await client.patch(
                    "/api/config/codexcrew", json={"path": "auto_update", "value": True}
                )
                assert resp.status == 200
                body = await resp.json()
        assert body["agents"]["codexcrew"]["description"] == _SENSITIVE_MASK
        ...
        assert stored["agents"]["codexcrew"]["description"] == _CRED_DESCRIPTION
