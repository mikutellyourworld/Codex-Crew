...
from codex_crew import mcp_grant
from codex_crew.connections.tool_aliases import normalized_endpoint
...
from codex_crew import agent as agent_mod  # noqa: E402
from codex_crew import mcp_discovery  # noqa: E402
from codex_crew.config import paths as connections_paths  # noqa: E402
from codex_crew.connections import get_provider  # noqa: E402
from codex_crew.connections import mint  # noqa: E402
from codex_crew.connections import ownership  # noqa: E402
from codex_crew.connections import warm  # noqa: E402
from codex_crew.dashboard.handlers import connections  # noqa: E402
from codex_crew.dashboard.handlers import mcp as mcp_handlers  # noqa: E402
...
def _wire(
    ...
    *,
    ...
    default_raw = {"codexcrew": {s.name: {"url": s.url} for s in inventory}}
...
async def test_a_query_string_variant_shares_the_grant_and_blocks_the_revoke(
    ...
    _wire(
        ...
        revoked=revoked,
        raw_specs={
            "codexcrew": {_SLUG: {"url": _provider_url()}},
...
async def test_a_disabled_entry_still_counts_as_a_grant_sharer(
    ...
    ``list_servers`` drops disabled entries outside the Codex Crew scope, so the
...
async def test_a_custom_agent_config_sharing_the_endpoint_blocks_the_revoke(
    ...
    """Blocker 1: the census must include agent specs Codex Crew does not own.

    Discovery merges only ``codexcrew.json``, so a spec the user wrote by hand is
...
async def test_a_mismatched_entry_in_another_scope_survives_the_purge(
    ...
    assert body["entryRemoved"] is True
    assert scopes == [("codexcrew",)], "the purge reached a scope it never judged"
...
def _agents_dir(monkeypatch: pytest.MonkeyPatch, tmp_path: pathlib.Path) -> pathlib.Path:
    ...
    agents.mkdir()
    monkeypatch.setattr(mcp_discovery, "_MCP_SOURCES", ((tmp_path / "mcp.json", "codexcrew"),))
...
def test_the_census_reads_agent_specs_codexcrew_does_not_own(
...
async def test_an_owned_trailing_slash_variant_revokes_both_pairs(
    ...
    _wire(
        ...
        purged=purged,
        revoked=revoked,
        raw_specs={"codexcrew": {_SLUG: {"url": base + "/"}}},
    )
    body = await _disconnect()
...
async def test_a_project_local_agent_spec_is_never_a_purge_target(
    ...
    restricted to, because Codex Crew does not write version-controlled files.
    """
    ...
    assert scopes == [("codexcrew",)], "purge reached outside the mcp.json scopes"
    assert revoked == []
    assert body["grantSharedWith"] == [_SLUG]
...
async def test_the_rendered_agent_mirror_does_not_block_the_revoke(
    ...
    ``<agents>/codexcrew.json`` and every ``scope.agent_mcp_file`` in the SAME
    ...
    _wire(
        ...
        raw_specs={
            "codexcrew": {_SLUG: {"url": base}},
            "mirror:codexcrew.json": {_SLUG: {"url": base}},
        },
    )
    body = await _disconnect()
    ...
    assert scopes == [("codexcrew",)], f"mirror label leaked into the purge scopes: {scopes}"
...
async def test_a_mirror_entry_still_blocks_when_no_scope_is_purged(
    ...
    _wire(
        ...
        purged=purged,
        revoked=revoked,
        raw_specs={"mirror:codexcrew.json": {_SLUG: {"url": base}}},
    )
    body = await _disconnect()
    assert revoked == []
    assert body["grantSharedWith"] == [_SLUG]
...
def test_agent_sources_separate_purge_owned_mirrors_from_user_specs(
    ...
    """The rendered Codex Crew agent file is a MIRROR; a user's agent spec is not.

    ``_purge_server_config`` strips ``<agents>/codexcrew.json`` itself, so its
    entries are reflections of the scopes being purged rather than independent
    grant holders. A hand-written ``custom.json`` is the opposite: Codex Crew
    ...
    """
    agents = _agents_dir(monkeypatch, tmp_path)
    (agents / "codexcrew.json").write_text('{"mcpServers": {}}', encoding="utf-8")
    ...
    assert "mirror:codexcrew.json" in labels, f"rendered mirror not labelled: {labels}"
...
async def test_a_probe_row_mirroring_a_census_entry_does_not_vote_twice(
    ...
    _wire(
        ...
        raw_specs={
            "codexcrew": {_SLUG: {"url": base}},
            "mirror:codexcrew.json": {_SLUG: {"url": variant}},
        },
    )
    body = await _disconnect()
...
def test_the_mirror_strip_serializes_against_the_agent_files_own_lock(
    ...
    entry from ``<agents>/codexcrew.json`` too (``_remove_from_agent_file``), and
    ...
    (``codexcrew.lock``). The MCP transaction lock the purge runs under is a
    ...
    from codex_crew.dashboard.handlers.mcp import _remove_from_agent_file

    mirror = tmp_path / "codexcrew.json"
