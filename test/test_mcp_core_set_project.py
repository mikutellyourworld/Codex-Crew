...
2. The stateless ``set_project`` path (#755). ``_call_tool_inner`` no longer
   ...
   and returns a session directive (see ``codex_crew.session_directive``). The
   session-aware consumer applies it via
   ``codex_crew.dashboard.session_directive_apply.apply_session_directive``,
...
from codex_crew import mcp_core, session_directive
...
class TestResolveSessionKeyStrict:
    """Strict resolver: the ``CODEXCREW_SESSION_KEY`` env var, or the direct
    ``CODEXCREW_HOST_PID`` -> ``session_pid_<pid>.txt`` lookup — the latter
    ...
    def _signed_env(self, monkeypatch, tmp_path, pid: str, session_key: str):
        ...
        from codex_crew import session_pid_sig

        monkeypatch.delenv("CODEXCREW_SESSION_KEY", raising=False)
        monkeypatch.setenv("CODEXCREW_HOST_PID", pid)
    ...
    def test_env_var_used(self, monkeypatch):
        monkeypatch.setenv("CODEXCREW_SESSION_KEY", "dashboard:slot-B")
    ...
    def test_env_var_wins_over_host_pid(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("CODEXCREW_SESSION_KEY", "dashboard:env-slot")
        with patch.object(session_pid_sig, "config_dir", return_value=tmp_path), \
             patch.object(
                 session_pid_sig,
                 "sel_hmac_key_path",
    ...
    def test_unsigned_host_pid_file_refused(self, monkeypatch, tmp_path):
        ...
        monkeypatch.delenv("CODEXCREW_SESSION_KEY", raising=False)
        monkeypatch.setenv("CODEXCREW_HOST_PID", "4242")
        (tmp_path / "sel_hmac.key").write_bytes(b"k" * 32)
    ...
    def test_replayed_sidecar_for_other_pid_refused(self, monkeypatch, tmp_path):
        ...
        monkeypatch.delenv("CODEXCREW_SESSION_KEY", raising=False)
        monkeypatch.setenv("CODEXCREW_HOST_PID", "2000")
        with patch.object(session_pid_sig, "config_dir", return_value=tmp_path), \
             patch.object(
                 session_pid_sig,
                 "sel_hmac_key_path",
    ...
    def test_host_pid_without_file_returns_empty(self, monkeypatch, tmp_path):
        ...
        monkeypatch.delenv("CODEXCREW_SESSION_KEY", raising=False)
        monkeypatch.setenv("CODEXCREW_HOST_PID", "5555")
        with patch.object(session_pid_sig, "config_dir", return_value=tmp_path), \
             patch.object(
                 session_pid_sig,
                 "sel_hmac_key_path",
    ...
    def test_non_numeric_host_pid_ignored(self, monkeypatch, tmp_path):
        ...
        monkeypatch.delenv("CODEXCREW_SESSION_KEY", raising=False)
        monkeypatch.setenv("CODEXCREW_HOST_PID", "../../etc/passwd")
        with patch.object(session_pid_sig, "config_dir", return_value=tmp_path), \
             patch.object(
                 session_pid_sig,
                 "sel_hmac_key_path",
...
class TestSetProjectApplier:
    ...
    async def test_sensitive_path_denied_without_mutating_slot(self, tmp_path, monkeypatch):
        ...
        state = _FakeState()
        # _set_project imports is_sensitive_path lazily from codex_crew.security,
        # so patch it on the source module.
        monkeypatch.setattr(
            "codex_crew.security.is_sensitive_path", lambda *a, **k: True
        )
        result = await apply_session_directive(
            state,
            slot,
    ...
    async def test_data_home_overlap_refused_without_mutating_slot(self, tmp_path, monkeypatch):
        ...
        _set_project imports it lazily from codex_crew.sandbox."""
        slot = _FakeSlot(project="/existing/project")
        state = _FakeState()
        monkeypatch.setattr(
            "codex_crew.sandbox.voice_runtime_workspace_conflict",
...
def sel_spy(monkeypatch):
    ...
    ``_audit`` does ``from codex_crew.sel import sel; sel().log_tool_invocation``,
    so patching the factory on the source module intercepts every path."""
    spy = _SelSpy()
    monkeypatch.setattr("codex_crew.sel.sel", lambda: spy)
    return spy
...
class TestApplierAuditAndFailSoft:
    ...
    async def test_success_emits_one_mcp_directive_event(self, tmp_path, monkeypatch, sel_spy):
        ...
        monkeypatch.setattr("codex_crew.security.is_sensitive_path", lambda *a, **k: False)
        saved: list[str] = []
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_handlers._save_recent_project",
    ...
    async def test_denied_path_audits_denied_and_returns_error(self, tmp_path, monkeypatch, sel_spy):
        ...
        monkeypatch.setattr("codex_crew.security.is_sensitive_path", lambda *a, **k: True)
    ...
    async def test_internal_exception_is_caught_and_audited(self, monkeypatch, sel_spy):
        ...
        # _monitor_start does `from codex_crew.autonudge import get_instance`
        # then calls it first — patch the source symbol so it raises.
        monkeypatch.setattr("codex_crew.autonudge.get_instance", _boom)
        slot = _FakeSlot()
        state = _FakeState()
        result = await apply_session_directive(
    ...
    @pytest.mark.parametrize(
        "session_key", ["telegram:codexcrew:direct:123:gen1", "slack:C123.456", "discord:guild1:chan2"]
    )
    ...
    async def test_refused_monitor_update_audits_denied(self, monkeypatch, sel_spy):
        ...
        monkeypatch.setattr("codex_crew.autonudge.get_instance", lambda: _Svc())
        monkeypatch.setattr("codex_crew.autonudge.binding_key_for", lambda sk: "bind-1")
        slot = _FakeSlot()
        state = _FakeState()
        result = await apply_session_directive(
