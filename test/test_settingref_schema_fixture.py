...
actually appears as a literal string in the backend source (src/codex_crew/).
...
BACKEND_SRC_ROOT = Path(__file__).resolve().parent.parent / "src" / "codex_crew"
...
def _scan_backend_source_for_literal(name: str) -> bool:
    """Recursively search src/codex_crew/**/*.py for the literal env var name."""
...
class TestSettingRefEnvVarsDrift:
    ...
    def test_all_env_vars_found_in_backend_source(self, env_vars_fixture):
        ...
        assert not missing, (
            ...
            f"src/codex_crew/**/*.py: {missing}. Either the var was removed "
