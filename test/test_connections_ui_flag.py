...
/api/config/codexcrew`` (``website/src/hooks/useConnectionsUi.ts``:
...
top-level key is captured into ``CodexCrewConfig._extra_sections`` at load and
...
def _masked(cfg: CodexCrewConfig) -> dict:
    from codex_crew.dashboard.handlers.core import _masked_config_dict
...
def test_flag_absent_leaves_the_gallery_off(tmp_path, monkeypatch):
    ...
    masked = _masked(CodexCrewConfig.load())
...
def test_a_non_bool_value_fails_closed(tmp_path, monkeypatch):
    ...
    held-for-release surface, a value Codex Crew cannot read means "off", never
...
def test_flag_survives_a_config_write(tmp_path, monkeypatch):
    ...
    assert _frontend_says_enabled(_masked(CodexCrewConfig.load()))
