...
in core ``codex_crew`` source is NOT declared in ``setup.cfg [options]
...
Scope: only core codex_crew modules (excludes apps/builtins/, knowledge/,
...
# Sub-trees within codex_crew that are NOT core startup and have their own
...
def _src_root() -> pathlib.Path:
    """Locate the codex_crew source tree."""
    try:
        import codex_crew  # noqa: PLC0415

        return pathlib.Path(codex_crew.__file__).resolve().parent
    except Exception:
        return pathlib.Path(__file__).resolve().parent.parent / "src" / "codex_crew"
...
def test_otlp_extra_declares_exact_http_exporter_version():
    """The documented codexcrew[otlp] install path must remain usable."""
    cfg = _read_setup_cfg()
    requirements = [
        line.strip()
...
def test_pyproject_declares_optional_dependencies_dynamic():
    ...
    omission also broke the published wheel's ``codexcrew[voice]`` install path.
    """
...
def test_dev_extra_pins_agree_with_the_dev_dependency_group():
    ...
    ``codex_crew.config.validation`` imports jsonschema behind a try/except, so
...
def test_all_unguarded_third_party_imports_are_declared():
    """Every module-level unguarded third-party import in core codex_crew
...
def test_noop_recorder_when_otel_missing(monkeypatch):
    ...
    to_remove = [
        ...
        or k
        in (
            "codex_crew.metrics.provider",
            "codex_crew.metrics.recorder",
            "codex_crew.metrics.local_exporter",
        )
    ]
    ...
    try:
        ...
        spec = importlib.util.find_spec("codex_crew.metrics.provider")
        assert spec is not None
        prov = importlib.util.module_from_spec(spec)
        sys.modules["codex_crew.metrics.provider"] = prov
        ...
        sys.modules.pop("codex_crew.metrics.provider", None)
        # Re-import cleanly
        importlib.import_module("codex_crew.metrics.provider")
