...
def test_windows_address_is_a_pipe_name(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert addr.startswith(r"\\.\pipe\codexcrew-mcp-")
...
def test_windows_address_is_deterministic_and_path_scoped(
    ...
    two installations (different $CODEXCREW_HOME) must not collide on the
    machine-global pipe namespace.
    """
    monkeypatch.setattr(pc, "IS_WINDOWS", True)
    a1 = transport.resolve_address("/home/alice/.codex-crew/mcp-gateway/gateway.sock")
    a2 = transport.resolve_address("/home/alice/.codex-crew/mcp-gateway/gateway.sock")
    b = transport.resolve_address("/home/bob/.codex-crew/mcp-gateway/gateway.sock")
    assert a1 == a2
    assert a1 != b
...
def test_windows_address_folds_case_and_separators(
    ...
    upper = transport.resolve_address(r"C:\Users\Foo\.codex-crew\gateway.sock")
    lower = transport.resolve_address(r"c:\users\foo\.codex-crew\gateway.sock")
    assert upper == lower, "case-different spellings of one path gave two pipe names"

    # Distinct installations must still not collide.
    other = transport.resolve_address(r"C:\Users\Bar\.codex-crew\gateway.sock")
    assert upper != other
...
def test_windows_address_folds_case_natively() -> None:
    ...
    upper = transport.resolve_address(r"C:\Users\Foo\.codex-crew\gateway.sock")
    lower = transport.resolve_address(r"c:\users\foo\.codex-crew\gateway.sock")
    slashes = transport.resolve_address("C:/Users/Foo/.codex-crew/gateway.sock")
