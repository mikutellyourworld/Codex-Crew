...
* ``session_pid._is_sweepable_orphan_gatewayd`` + ``_kill_orphan_gatewayd`` —
  ...
  from disk, TERM-first, while ``codex_crew.cli`` / ``codex_crew.__main__`` and
...
from codex_crew.mcp_gateway import gatewayd as gw
...
class TestSocketLivenessSweeper:
    ...
    async def test_inconclusive_stat_failure_never_triggers_exit(
        ...
        stop = asyncio.Event()
        with patch("codex_crew.mcp_gateway.gatewayd.os.stat", side_effect=denied):
            task = asyncio.create_task(gw._socket_liveness_sweeper(sock, 0.01, stop))
    ...
    async def test_non_consecutive_misses_never_trigger_exit(
        ...
        stop = asyncio.Event()
        with patch("codex_crew.mcp_gateway.gatewayd.os.stat", side_effect=flaky):
            task = asyncio.create_task(gw._socket_liveness_sweeper(sock, 0.01, stop))
...
def _gatewayd_cmdline(sock: Path | str, socket_form: str = "pair") -> bytes:
    parts = [b"/usr/bin/python3", b"-m", b"codex_crew.mcp_gateway.gatewayd"]
    if socket_form == "pair":
...
class TestIsSweepableOrphanGatewayd:
    ...
    def test_duplicate_socket_flags_use_the_last_occurrence(
        ...
        base = [b"/usr/bin/python3", b"-m", b"codex_crew.mcp_gateway.gatewayd"]
    ...
    def test_cli_and_main_entrypoints_are_never_sweepable(self, tmp_path: Path) -> None:
        gone = tmp_path / "vanished.sock"
        for module in (b"codex_crew.cli", b"codex_crew.__main__"):
            cmdline = b"\x00".join(
    ...
    def test_marker_as_path_fragment_does_not_match(self, tmp_path: Path) -> None:
        """`vim codex_crew.mcp_gateway.gatewayd` shapes are structural misses."""
        gone = tmp_path / "vanished.sock"
        cmdline = b"\x00".join(
            [
                b"vim",
                b"codex_crew.mcp_gateway.gatewayd",
                b"--socket",
    ...
    def test_inconclusive_stat_failure_fails_closed(self, tmp_path: Path) -> None:
        ...
        with patch("codex_crew.session_pid.os.stat", side_effect=denied):
...
class TestFindOrphanGatewaydCandidates:
    ...
    def test_live_socket_gatewayd_is_excluded(self, tmp_path: Path) -> None:
        ...
        live.write_text("")
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[701]),
            patch("codex_crew.session_pid.sys") as mock_sys,
    ...
    def test_age_floor_still_applies(self, tmp_path: Path) -> None:
        gone = tmp_path / "vanished.sock"
        with (
            patch("codex_crew.session_pid._our_orphan_pids", return_value=[702]),
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=_gatewayd_cmdline(gone)),
            patch("os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=10.0),
        ):
            mock_sys.platform = "linux"
            result = sp.find_orphan_mcp_candidates(active_pids=set())
        assert result == []
...
class TestKillOrphanGatewayd:
    ...
    def test_term_suffices_when_the_daemon_exits(self, tmp_path: Path) -> None:
        ...
        with (
            patch("codex_crew.session_pid.os.kill", side_effect=fake_kill),
            patch("codex_crew.session_pid.os.killpg") as killpg,
        ):
    ...
    def test_wedged_daemon_is_escalated_to_killpg(
        ...
        with (
            patch("codex_crew.session_pid.os.kill", side_effect=fake_kill),
            patch("codex_crew.session_pid.os.killpg") as killpg,
            patch("codex_crew.session_pid.os.getpgid", return_value=901),
            patch("codex_crew.session_pid.os.getpgrp", return_value=1234),
    ...
    def test_kill_orphan_mcps_routes_gatewayd_through_term_first(
        ...
        with (
            patch("codex_crew.session_pid.platform_compat") as pc,
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=cmdline),
            patch("codex_crew.session_pid.os.killpg") as killpg,
            patch("codex_crew.session_pid.os.getpgrp", return_value=1234),
            patch("codex_crew.session_pid.os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=300.0),
        ):
            pc.IS_WINDOWS = False
    ...
    def test_kill_phase_age_floor_protects_a_recycled_pid(
        ...
        with (
            ...
            patch("codex_crew.session_pid.sys") as mock_sys,
            patch.object(Path, "read_bytes", return_value=cmdline),
            patch("codex_crew.session_pid.os.kill") as kill,
            patch("codex_crew.session_pid.os.killpg") as killpg,
            patch("codex_crew.session_pid.os.getpgrp", return_value=1234),
            patch("codex_crew.session_pid.os.getpid", return_value=1),
            patch("codex_crew.session_pid._linux_pid_age", return_value=3.0),
        ):
            pc.IS_WINDOWS = False
            mock_sys.platform = "linux"
