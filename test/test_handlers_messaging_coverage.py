...
``codex_crew.dashboard.handlers.messaging`` that the existing per-channel test
...
from codex_crew.subagent import AGENT_NOT_FOUND_CODE
...
def _track(monkeypatch, tracked: bool) -> None:
    monkeypatch.setattr("codex_crew.slack.handler.is_tracked_channel", lambda cid: tracked)
...
class TestTeamsConfigSave:
    ...
    def test_clear_config_write_failure_does_not_leave_env_cleared(
        ...
        import codex_crew.agent as _agent
...
class TestSlackManifest:
    ...
    def test_defaults_to_a_non_identifying_alias(self) -> None:
        assert _payload(_run(mod.api_slack_manifest, _Req(_state())))["alias"] == "codexcrew"
