...
class TestPathExists:
    ...
    def test_reports_a_path_that_names_no_file(self, tree: Path) -> None:
        _write(tree, "docs/ok.md", "# Ok\n\nSee `src/codex_crew/ghost.py`.\n")
        assert "src/codex_crew/ghost.py" in _tokens(tree, gate.CHECK_PATH_EXISTS)

    @pytest.mark.parametrize(
        "shape",
        [
            "src/codex_crew/ghost.py",
            "scripts/ghost.py",
            "scripts/ghost.sh",
    ...
    def test_silent_when_the_path_exists(self, tree: Path) -> None:
        _write(tree, "src/codex_crew/real.py", "a = 1\n")
        _write(tree, "docs/ok.md", "# Ok\n\nSee `src/codex_crew/real.py`.\n")
    ...
    def test_silent_for_a_path_that_resolves_one_root_down(self, tree: Path) -> None:
        ...
        _write(tree, "src/codex_crew/builtin_skills/demo/scripts/reaper.sh", "#!/bin/sh\n")
        _write(tree, "docs/ok.md", "# Ok\n\nRun `scripts/reaper.sh`.\n")
        assert not _tokens(tree, gate.CHECK_PATH_EXISTS)

    def test_silent_for_a_path_that_is_only_a_longer_path_s_tail(self, tree: Path) -> None:
        # `src/codex_crew/docs/tips.md` must not also be read as `docs/tips.md`.
        _write(tree, "src/codex_crew/docs/tips.md", "# Tips\n")
        _write(tree, "docs/ok.md", "# Ok\n\nSee `src/codex_crew/docs/tips.md`.\n")
        assert not _tokens(tree, gate.CHECK_PATH_EXISTS)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("src/codex_crew/types.pyi", "src/codex_crew/types.py"),
    ...
    def test_a_local_coordinate_is_still_validated(self, tree: Path) -> None:
        ...
        _write(tree, "docs/ok.md", "# Ok\n\nSee `src/codex_crew/gone.py::missing_symbol`.\n")
        assert "src/codex_crew/gone.py" in _tokens(tree, gate.CHECK_PATH_EXISTS)

    def test_a_resolvable_local_coordinate_is_silent(self, tree: Path) -> None:
        _write(tree, "src/codex_crew/mcp_tools/workflows.py", "def workflow_run():\n    pass\n")
        _write(
            tree,
            "docs/ok.md",
            "# Ok\n\nSee `src/codex_crew/mcp_tools/workflows.py::workflow_run`.\n",
        )
        assert not _tokens(tree, gate.CHECK_PATH_EXISTS)
    ...
    def test_a_line_citation_is_still_seen_after_the_boundary(self, tree: Path) -> None:
        ...
        _write(tree, "src/codex_crew/small.py", "a = 1\n")
        _write(tree, "docs/ok.md", "# Ok\n\nSee `small.py:1`.\n")
    ...
    def test_silent_for_a_bare_path_outside_backticks(self, tree: Path) -> None:
        ...
        _write(tree, "docs/ok.md", "# Ok\n\nThe handler is in src/codex_crew/ghost.py.\n")
        assert not _tokens(tree, gate.CHECK_PATH_EXISTS)

    def test_silent_inside_a_fenced_block(self, tree: Path) -> None:
        _write(tree, "docs/ok.md", "# Ok\n\n```\ncat src/codex_crew/ghost.py\n```\n")
        assert not _tokens(tree, gate.CHECK_PATH_EXISTS)
...
class TestLineRef:
    ...
    def test_reports_a_range_and_a_list(self, tree: Path) -> None:
        _write(tree, "src/codex_crew/small.py", "\n".join(f"x = {i}" for i in range(50)) + "\n")
...
class TestFencedPath:
    ...
    def test_reports_a_dead_codexcrew_run_argument(self, tree: Path) -> None:
        _write(tree, "docs/ok.md", "# Ok\n\n```bash\ncodexcrew run specs/gone.md\n```\n")
    ...
    def test_silent_when_the_pasted_path_exists(self, tree: Path) -> None:
        ...
        _write(
            tree,
            "docs/ok.md",
            "# Ok\n\n```bash\ncodexcrew run docs/task-specs/2026/01/live/spec.md\n```\n",
        )
    ...
    def test_silent_in_the_packaged_user_docs(self, tree: Path) -> None:
        ...
        _write(
            tree,
            "src/codex_crew/docs/task-runner.md",
            "# Runner\n\n```bash\ncodexcrew run docs/task-specs/2026/03/my-task/spec.md\n```\n",
        )
        assert not _tokens(tree, gate.CHECK_FENCED_PATH)

    def test_one_pasted_command_is_one_finding(self, tree: Path) -> None:
        # Both patterns match a `codexcrew run docs/task-specs/...` line.
        _write(
            tree,
            "docs/ok.md",
            "# Ok\n\n```bash\ncodexcrew run docs/task-specs/gone/spec.md\n```\n",
        )
...
class TestCouplingCompleteness:
    ...
    def test_reports_an_unrecorded_consumer(self, tree: Path) -> None:
        _write(tree, "src/codex_crew/docs/ghost-integration.md", "# Ghost\n")
        _write(
            tree,
    ...
    def test_silent_when_named_only_in_a_comment(self, tree: Path) -> None:
        ...
        _write(tree, "src/codex_crew/docs/iframe-hosts.md", "# Hosts\n")
        _write(
            tree,
            "website/src/pages/ChatPage.tsx",
            "// See `src/codex_crew/docs/iframe-hosts.md`.\nexport const x = 1\n",
        )
        assert not _tokens(tree, gate.CHECK_COUPLING_COMPLETENESS)

    def test_silent_when_named_only_in_a_test(self, tree: Path) -> None:
        _write(tree, "src/codex_crew/docs/fixture-doc.md", "# Fixture\n")
    ...
    def test_silent_for_an_ambiguous_filename(self, tree: Path) -> None:
        ...
        _write(tree, "src/codex_crew/docs/README.md", "# Index\n")
...
class TestDeadIdentifier:
    ...
    def test_reports_a_name_absent_from_every_code_tree(self, tree: Path) -> None:
        _write(tree, "src/codex_crew/real.py", "def live_handler():\n    return 1\n")
    ...
    def test_silent_for_a_live_identifier(self, tree: Path) -> None:
        _write(tree, "src/codex_crew/real.py", "def live_handler_name():\n    return 1\n")
...
class TestBaseline:
    ...
    def test_a_triple_that_no_longer_fires_is_reported_not_fatal(self, tree: Path) -> None:
        gone = (gate.CHECK_PATH_EXISTS, "docs/ok.md", "src/codex_crew/already-fixed.py")
        findings = gate.run(tree)
