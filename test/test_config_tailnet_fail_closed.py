...
def _trust(cfg: CodexCrewConfig) -> TailnetTrust:
...
class TestALostOverlayCannotReadmitAnyone:
    ...
    def test_the_refusal_names_the_overlay_not_the_base(self, tmp_path: Path, caplog) -> None:
        ...
        with unittest.mock.patch(
            "codex_crew.dashboard.tailnet.is_governance_pinned_off", return_value=False
        ):
            with caplog.at_level("ERROR", logger="codex_crew.dashboard.tailnet"):
                asyncio.run(
                    governed_tailnet_trust(
                        False,
                        (),
...
class TestAnExplicitNullIsNotAnAbsentKey:
    ...
    Safe to treat this way because Codex Crew's own ``to_dict()`` writes concrete
...
class TestGovernanceCeilingStillWins:
    ...
    def test_a_pinned_ceiling_clears_unknown_identity(self) -> None:
        with unittest.mock.patch(
            "codex_crew.dashboard.tailnet.is_governance_pinned_off", return_value=True
        ):
