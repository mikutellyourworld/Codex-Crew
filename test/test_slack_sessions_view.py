...
Covers the helpers in ``codex_crew.slack.sessions_view`` that the slash command
``/<command> sessions``, the ``sessions`` keyword in DMs, and the App
Home Tab all share, plus the keyword handler in
``codex_crew.slack.handler`` which delegates to those helpers.
"""
...
from codex_crew.slack import sessions_view
from codex_crew.slack.handler import _handle_sessions_command
...
class TestCollectRecentSessions:
    ...
    def sess_dir(self, tmp_path, monkeypatch):
        d = tmp_path / "sessions"
        d.mkdir()
        monkeypatch.setattr("codex_crew.slack.sessions_view._SESSIONS_DIR", d)
        return d

    def test_missing_dir_returns_empty(self, tmp_path, monkeypatch):
        # Point at a directory that doesn't exist
        monkeypatch.setattr("codex_crew.slack.sessions_view._SESSIONS_DIR", tmp_path / "nope")
    ...
    def test_reads_taskrunner_session_with_default_title(self, sess_dir):
        ...
        # No metadata agent → defaults to codexcrew
        assert row["agent"] == "codexcrew"
...
class TestHandleSessionsCommandDelegation:
    ...
    async def test_logs_sel_audit_on_access(self, tmp_path, monkeypatch):
        ...
        slack = MockSlackClient()
        with patch("codex_crew.slack.handler.sel") as mock_sel:
...
class TestSlashSessionsAudit:
    ...
    async def test_slash_logs_sel_audit_with_caller_and_count(
        ...
        from codex_crew.slack.events import _handle_sessions
    ...
    async def test_slash_authorized_via_allowed_user_branch(
        ...
        with (
            patch("codex_crew.slack.events.sel") as mock_sel,
            patch("codex_crew.slack.events.is_owner", return_value=False),
            patch("codex_crew.slack.events.is_allowed_user", return_value=True),
        ):
            mock_sel.return_value.log_api_access = MagicMock()
...
class TestWithMessagesBoundsTheRead:
    ...
    def test_it_reads_only_the_metadata_line(self, tmp_path, monkeypatch):
        from codex_crew.messaging import sessions_view as sv
...
class TestOffLoopStructuralRatchet:
    ...
    def _src_modules() -> list[Path]:
        ...
        pkg_root = Path(codex_crew.__file__).parent
    ...
    def test_resume_context_has_no_direct_read_text_call(self):
        ...
        import codex_crew.slack.interactions as interactions_mod
