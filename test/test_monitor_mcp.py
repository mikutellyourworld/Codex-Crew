...
from codex_crew.mcp_tools import control


def test_monitor_watch_is_stateless_and_canonical(gateway_posts):
    with patch("codex_crew.mcp_core._resolve_session_key_strict", return_value="dashboard:chat-1"):
...
def test_monitor_watch_rejects_native_subagent_binding(gateway_posts):
    with patch("codex_crew.mcp_core._resolve_session_key_strict", return_value="subagent:child"):
        result = control.monitor_watch(
            "monitor_watch",
            {
                "kind": "github_pull_request",
...
def test_monitor_watch_rejects_webex_while_finite_legacy_loop_remains_available(gateway_posts):
    ...
    with (
        patch(
            ...
            return_value="webex:codexcrew:direct:operator@example.com",
        ),
        patch("codex_crew.mcp_core.sel", return_value=audit),
    ):
...
def test_structured_monitor_session_refusals_are_failed_and_audited(tool_name, args):
    audit = MagicMock()
    with (
        patch("codex_crew.mcp_core._resolve_session_key", return_value="subagent:child"),
        patch("codex_crew.mcp_core._resolve_session_key_strict", return_value="subagent:child"),
        patch("codex_crew.mcp_core.sel", return_value=audit),
        patch("codex_crew.mcp_shared.sel", return_value=audit),
    ):
...
def test_monitor_inspect_passes_strict_identity_without_fallback():
    with (
        patch("codex_crew.mcp_core._resolve_session_key_strict", return_value="slack:123"),
        patch(
...
def test_monitor_inspect_reports_internal_read_failure_as_error():
    with (
        patch("codex_crew.mcp_core._resolve_session_key_strict", return_value="slack:123"),
        patch("codex_crew.mcp_core._get", return_value={"error": "gateway unavailable"}),
    ):
        result = control.monitor_inspect("monitor_inspect", {})
