...
from codex_crew.dashboard.file_index import FileIndex
from codex_crew.dashboard.handlers import api_file_search
...
class TestFileIndexDirs:
    ...
    async def test_index_dir_symlink_resolved_before_sensitive_check(self, tmp_path):
        ...
        with patch("codex_crew.dashboard.file_index.is_sensitive_path", side_effect=fake_sensitive):
            entries, _ = idx._walk()
...
class TestApiFileSearchDirs:
    ...
    async def test_walk_fallback_file_symlink_resolved_before_sensitive_check(
        ...
        with patch(
            ...
            # codex_crew.security, so the source module is the patch target.
            "codex_crew.security.is_sensitive_path",
