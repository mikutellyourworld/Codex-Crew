...
import codex_crew.mcp_shared as mcp_shared
from codex_crew.mcp_shared import _read_message, respond
...
class TestCallToolWithLoggingRedaction:
    ...
    def test_args_redacted_before_sel_log(self):
        from codex_crew.mcp_shared import call_tool_with_logging
        ...
        with patch("codex_crew.mcp_shared.sel", return_value=_FakeSel()):
...
def _tools_call_with_caller(req_id, tool_name: str, session_key: str) -> dict:
    from codex_crew.mcp_caller import CallerContext, build_caller_meta
...
def _tools_call_with_tenant(req_id, tool_name: str, nonce: str) -> dict:
    ...
    from codex_crew.mcp_caller import build_tenant_meta
...
class TestStdioLoopCallerIdentity:
    ...
    def test_initialize_omits_capability_by_default(self, monkeypatch):
        # codexcrew-cron does NOT consume per-call identity — it must stay
...
class TestPerSessionToolPolicy:
    ...
    def test_cache_is_keyed_per_session(self, monkeypatch):
        ...
        monkeypatch.setattr(
            mcp_shared.CodexCrewConfig,
            "load",
            classmethod(lambda cls: type("C", (), {"dashboard": type("D", (), {"url": "http://localhost:5486"})()})()),
        )
