...
PREPARE_PR_SKILL = (
    ROOT / "src" / "codex_crew" / "builtin_skills" / "codexcrew-dev" / "prepare-pr" / "SKILL.md"
)
PREPARE_PR_FINDINGS = (
    ROOT
    / "src"
...
class TestFirstPrinciplesScopeGateBehavior:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("src/codex_crew/session.py", True),
            ...
            ("src/codex_crew/apps/builtins/meetings/tests/test_routes.py", False),
...
class TestUxScopeGateSurvivesAWideDiff:
    ...
    def test_a_ui_change_is_detected_regardless_of_diff_width(
        ...
        touched = "website/src/App.tsx\n" + "\n".join(
            f"src/codex_crew/module_{i}.py" for i in range(tail_files)
        )
    ...
    def test_a_non_ui_diff_still_skips(self, lane: str, tmp_path: Path) -> None:
        ...
        if bash is None:
            pytest.skip("the scope gate runs only under Bash")
        touched = "src/codex_crew/session.py\ndocs/ci/ci-and-reviews.md"
        github_output = tmp_path / "github_output"
        github_output.touch()  # the Actions runtime pre-creates $GITHUB_OUTPUT
        script = (
            "set -euo pipefail\n"
...
class TestBlockAdjudicationContract:
    ...
    def test_the_fence_reads_only_the_consequence_chain_line(
        ...
        harness.write_text(
            "BLOCKING src/codex_crew/sandbox.py:44 -- the adapted harness"
            " skips the seam.\n"
        ...
        event_loop.write_text(
            "BLOCKING src/codex_crew/thing.py:9 -- a sync read on the loop.\n"
...
DOWN = "DOWNGRADE {fid} codexcrew/x.py:10 reason=disproportionate-remedy"
UP = "UPHOLD {fid} codexcrew/x.py:10 reason=harm-warrants-remedy"
FFOOTER = "[ADJUDICATION-FENCED] deadbeef fenced={n} flagged={k}"
FMARKER = "[GPT-ADJUDICATED-FENCED] deadbeef"
FLAGLINE = (
    "FLAG {fid} codexcrew/x.py:10 -- requires a timestamp collision "
    "the monotonic writer cannot produce"
)
UPFENCED = "UPHOLD-FENCED {fid} codexcrew/x.py:10 -- plausible in real operation"
        ...
        [
            ...
            (
                ...
                _adjudication(
                    FOOTER.format(total=2, uph=0, dwn=2),
                    DOWN.format(fid="F1"),
                    "DOWNGRADE F2 codexcrew/x.py:10 reason=security-class",
                    MARKER,
                ),
...
class TestBlockAdjudicationExtraction:
    ...
    VERDICT = "\n".join(
        [
            "**One blocking issue.**",
            "",
            "**BLOCKING — codexcrew/session/replay.py:120**",
