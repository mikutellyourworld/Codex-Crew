...
``codex_crew.cron`` resolves the global default timezone on the event loop --
...
value is PUBLISHED by each successful ``CodexCrewConfig.load`` rather than read
...
from codex_crew.cron import CronJob, _job_tz
...
def _load_with_timezone(tz_name: str) -> CodexCrewConfig:
...
class TestTheSnapshotCarriesTheConfiguredZone:
    def test_load_publishes_the_timezone_it_resolved(self) -> None:
        ...
        """
        with unittest.mock.patch("codex_crew.config.loader.publish_config_timezone") as publish:
...
class TestTheSnapshotFailsToUnsetRatherThanToAGuess:
    def test_a_cold_snapshot_reads_as_unset(self) -> None:
        ...
        ``""`` is exactly what an unset ``CodexCrewConfig.timezone`` says, so a
        ...
        with unittest.mock.patch("codex_crew.cron.published_config_timezone", return_value=""):
            job = CronJob(id="j1", name="t", message="m", timezone="")
    ...
    def test_a_defaults_load_overwrites_a_richer_snapshot(self) -> None:
        ...
        try:
            ...
            publish_config_timezone(CodexCrewConfig(), first + 1)
...
def _cfg_with_tz(tz_name: str) -> CodexCrewConfig:
    cfg = CodexCrewConfig()
...
def _live_default() -> str:
    ...
    """
    return CodexCrewConfig.load().timezone
