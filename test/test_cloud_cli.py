...
from codex_crew import cli_cloud
...
class TestDispatch:
    ...
    def test_no_action_prints_help(self, capsys):
        ...
        assert "codexcrew cloud" in capsys.readouterr().out
    ...
    def test_setup_cloud_step_skips_on_eof(self, monkeypatch, capsys):
        # Piped `codexcrew setup` (no stdin) must skip the cloud step, not crash.
...
class TestListStatus:
    def test_list_empty(self, monkeypatch, capsys):
        ...
        assert "No CodexCrew cloud instances" in capsys.readouterr().out
...
class TestDestroy:
    def test_destroy_dry_run(self, monkeypatch, capsys):
        monkeypatch.setattr(
            ...
            "destroy",
            lambda *a, **k: {
                "argv": ["cloudformation", "delete-stack", "--stack-name", "codexcrew-kc-1"]
            },
        )
    ...
    def test_destroy_warns_when_source_cleanup_fails(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(
            ...
            lambda *a, **k: {
                "removed": False,
                "uri": "s3://codexcrew-src-1/kc-1/codexcrew-src.tar.gz",
                "error": "AccessDenied",
            },
        )
        monkeypatch.setattr(
        ...
        assert "aws s3 rm s3://codexcrew-src-1/kc-1/codexcrew-src.tar.gz" in out
...
class TestCloudLogin:
    ...
    def test_login_surfaces_device_url_and_waits(self, monkeypatch, capsys):
        from codex_crew.cloud.login import LoginPrompt
    ...
    def test_logout_signs_out_and_points_at_login(self, monkeypatch, capsys):
        ...
        assert "codexcrew cloud login" in out
