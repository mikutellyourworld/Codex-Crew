...
def tmp_config(tmp_path):
    with patch("codex_crew.context_management.config_dir", return_value=tmp_path):
        yield tmp_path


def test_cap_result_file_no_truncation(tmp_path):
    from codex_crew.context_management import cap_result_file
...
def test_cap_result_file_truncates(tmp_path):
    from codex_crew.context_management import RESULT_FILE_MAX_BYTES, cap_result_file
...
def test_cap_streaming_text_short():
    from codex_crew.context_management import cap_streaming_text

    assert cap_streaming_text("short") == "short"


def test_cap_streaming_text_long():
    from codex_crew.context_management import STREAMING_TEXT_MAX_CHARS, cap_streaming_text
...
def test_cap_history():
    from codex_crew.context_management import HISTORY_MAX_ENTRIES, cap_history
...
def test_check_session_budget_under(tmp_path):
    from codex_crew.context_management import check_session_budget
...
def test_check_session_budget_over(tmp_path):
    from codex_crew.context_management import SESSION_MAX_BYTES, check_session_budget
...
def test_evict_completed_agents():
    from codex_crew.context_management import evict_completed_agents
...
def test_cleanup_stale_sessions(tmp_config):
    ...
    from codex_crew.context_management import cleanup_stale_sessions
...
def test_looks_like_plan_true():
    from codex_crew.context_management import looks_like_plan
...
async def test_rephrase_plan_not_a_plan_returns_none():
    ...
    from codex_crew.context_management import rephrase_plan

    client = AsyncMock()
    client.send_message = AsyncMock(return_value=None)

    with patch("codex_crew.llm_helpers.stream_and_collect", new_callable=AsyncMock) as mock_stream:
...
def test_validate_plan_format_valid():
    from codex_crew.context_management import validate_plan_format
...
def test_strip_plan_markers():
    from codex_crew.context_management import strip_plan_markers
