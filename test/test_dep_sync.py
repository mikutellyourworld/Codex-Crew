...
def _origin_inside(target_py):
    ...
    return str(Path(_origin_inside.repo) / "src" / "codex_crew" / "__init__.py")
...
def test_normalize_folds_the_spellings_pep503_treats_as_one():
    ...
    assert dep_sync.normalize("CODEXCREW") == "codexcrew"
...
def test_console_script_target_prefers_the_pyproject_declaration(repo):
    ...
    (repo / "pyproject.toml").write_text(
        '[project]\nname = "codexcrew"\ndynamic = ["dependencies"]\n\n'
        '[project.scripts]\ncodexcrew = "codex_crew.new:main"\n',
        encoding="utf-8",
    )

    assert dep_sync.console_script_target(repo, "codexcrew") == "codex_crew.new:main"


def test_console_script_target_falls_back_to_setup_cfg(repo):
    (repo / "setup.cfg").write_text(
        _SETUP_CFG + "\n\n[options.entry_points]\nconsole_scripts =\n"
        "    codexcrew = codex_crew.old:main\n",
        encoding="utf-8",
    )

    assert dep_sync.console_script_target(repo, "codexcrew") == "codex_crew.old:main"
...
def test_console_script_target_reports_a_removal_rather_than_reading_a_stale_copy(repo):
    ...
    (repo / "pyproject.toml").write_text(
        '[project]\nname = "codexcrew"\ndynamic = ["dependencies"]\n\n'
        '[project.scripts]\nsomething-else = "codex_crew.other:main"\n',
        encoding="utf-8",
    )

    assert dep_sync.console_script_target(repo, "codexcrew") == dep_sync.SCRIPT_REMOVED
...
def test_rejected_specs_refuses_paths_archives_and_the_project_itself():
    ...
    # Only spellings PEP 503 actually folds onto this project's name. `codex_crew`
    # normalizes to `kiro-crew`, which is a DIFFERENT distribution, so it is not
    # claimed here.
    for spelling in [
        "codexcrew",
        "CodexCrew",  # brand-ok: a PEP 503 spelling of the distribution name
        "CODEXCREW",
        "codexcrew>=1",
    ]:
...
def test_installed_package_origin_reports_where_the_package_resolves():
    ...
    class _Proc:
        returncode = 0
        stdout = "/checkouts/main/src/codex_crew/__init__.py\n"
...
def test_venv_serving_another_checkout_is_reported(tmp_path):
    ...
    reason = dep_sync.venv_not_mapped_to(
        str(tmp_path / "other" / "src" / "codex_crew" / "__init__.py"), tmp_path / "main"
    )
...
def test_a_venv_serving_this_checkout_passes(tmp_path):
    repo = tmp_path / "main"
    origin = repo / "src" / "codex_crew" / "__init__.py"
...
def test_a_sibling_directory_sharing_the_prefix_does_not_count_as_inside(tmp_path):
    ...
    sibling = tmp_path / "main-wt" / "src" / "codex_crew" / "__init__.py"
...
def test_main_refuses_a_venv_serving_another_checkout(repo, capsys):
    ...
    with (
        patch.object(
            ...
            return_value="/checkouts/other/src/codex_crew/__init__.py",
        ),
        patch.object(dep_sync, "subprocess") as sp,
    ):
        rc = dep_sync.main([str(repo), "py"])
...
def test_main_reports_a_repointed_console_script_after_installing(repo, capsys):
    ...
    with (
        patch.object(dep_sync, "console_script_target", return_value="codex_crew.new:main"),
        patch.object(
            dep_sync, "installed_console_script_target", return_value="codex_crew.old:main"
        ),
    ...
    assert rc == 1
    err = capsys.readouterr().err
    assert "codex_crew.new:main" in err
    assert "codex_crew.old:main" in err
...
def test_main_tolerates_an_unreadable_installed_entry_point(repo):
    ...
    with (
        patch.object(dep_sync, "console_script_target", return_value="codex_crew.cli:main"),
        patch.object(dep_sync, "installed_console_script_target", return_value=None),
        patch.object(dep_sync.subprocess, "run", return_value=_Proc()),
    ):
...
def test_locked_console_scripts_is_a_posix_noop(tmp_path, monkeypatch):
    ...
    py = _make_scripts(tmp_path, "codexcrew.exe")
    monkeypatch.setattr(sys, "platform", "linux")
    _raise_on(monkeypatch, "codexcrew.exe", PermissionError(13, "in use"))
...
def test_locked_console_scripts_flags_a_locked_script(tmp_path, monkeypatch):
    ...
    assert len(locked) == 1
    assert locked[0].endswith("codexcrew.exe")
...
def test_locked_console_scripts_ignores_unrelated_executables(tmp_path, monkeypatch):
    ...
    """
    py = _make_scripts(tmp_path, "codexcrew.exe", "unrelated.exe")
    monkeypatch.setattr(sys, "platform", "win32")
...
def test_locked_console_scripts_lets_pip_judge_other_errors(tmp_path, monkeypatch):
    ...
    py = _make_scripts(tmp_path, "codexcrew.exe")
    monkeypatch.setattr(sys, "platform", "win32")
    _raise_on(monkeypatch, "codexcrew.exe", OSError(5, "I/O error"))
...
def test_sync_or_reinstall_substitutes_when_a_script_is_locked(tmp_path):
    ...
    with (
        _origin_stub(),
        _maps(),
        patch.object(dep_sync, "locked_console_scripts", return_value=[r"C:\v\codexcrew.exe"]),
    ...
    assert any("codexcrew.exe" in m and "dependency-only" in m for m, _ in messages)
...
def _decoy_package(tmp_path):
    """A directory holding a decoy ``codex_crew`` package, as a checkout's src/ does."""
    decoy = tmp_path / "decoy-src"
    (decoy / "codex_crew").mkdir(parents=True)
    (decoy / "codex_crew" / "__init__.py").write_text("", encoding="utf-8")
    return decoy
...
def test_origin_probe_ignores_a_decoy_package_in_the_callers_cwd(tmp_path, monkeypatch):
    ...
    with its CWD inside a checkout's ``src/``, which contains ``codex_crew/``.
...
def test_every_interpreter_probe_runs_isolated_with_a_neutral_cwd(tmp_path):
    ...
    with patch.object(dep_sync.subprocess, "run", side_effect=fake_run):
        ...
        dep_sync.installed_console_script_target(target, "codexcrew")
...
def test_module_imports_stdlib_only():
    """The invariant ``codex_crew._bootstrap`` depends on, enforced.
