...
for Codex Crew's own MCP servers WITHOUT spawning them: on a host where the probe
...
#: ``codexcrew-core`` is the odd one out — the other three all call theirs
#: ``run_mcp_server``.
_SERVE_ENTRY = {
    "codexcrew-core": "run_mcp_core_server",
    "codexcrew-cron": "run_mcp_server",
    "codexcrew-computer": "run_mcp_server",
    "codexcrew-dashboard": "run_mcp_server",
    "codexcrew-consult": "run_mcp_server",
}
...
def test_the_concrete_verdicts_are_spelled_out() -> None:
    ...
    every managed server flipped at once. ``codexcrew-core``, ``codexcrew-cron``
    and ``codexcrew-dashboard`` consume the injected caller block and refuse or
    safely namespace an unidentified caller, so none is session-bound.
    ``codexcrew-computer`` also advertises and consumes the block (#4659 — its
    ...
    """
    assert managed_server_is_session_bound("codexcrew-core") is False
    assert managed_server_is_session_bound("codexcrew-cron") is False
    assert managed_server_is_session_bound("codexcrew-computer") is True
    assert managed_server_is_session_bound("codexcrew-dashboard") is False
