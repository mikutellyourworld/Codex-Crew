...
def _wire_common(monkeypatch: pytest.MonkeyPatch, topo: ProcessTopology, view: str) -> None:
    ...
    # A leaked CODEXCREW_HOST_PID (e.g. when the test itself runs inside a
    ...
    monkeypatch.setattr("codex_crew.mcp_caller._FROM_ENV_CACHE", None)
...
def test_from_env_resolves_session_key(topo, monkeypatch, view) -> None:
    from codex_crew import mcp_caller
    ...
    monkeypatch.setattr("codex_crew.config.loader.config_dir", lambda: topo.cfg_dir)
...
def test_mcp_shared_policy_walk_reaches_gateway(topo, monkeypatch, view) -> None:
    from codex_crew import mcp_shared
    ...
    cfg = MagicMock()
    cfg.dashboard.url = "http://localhost:5486/"
    monkeypatch.setattr(mcp_shared.CodexCrewConfig, "load", classmethod(lambda cls: cfg))
    monkeypatch.setattr(mcp_shared, "parse_dashboard_url", lambda url: ("localhost", 5486))
...
def test_stub_caller_block_carries_session_key(topo, monkeypatch, view) -> None:
    from codex_crew import mcp_caller
    from codex_crew.mcp_gateway import stub
...
# ---------------------------------------------------------------------------
# Resolution path 5: CODEXCREW_HOST_PID env shortcut.
...
def test_from_env_host_pid_env_resolves_in_any_view(topo, monkeypatch, view) -> None:
    """CODEXCREW_HOST_PID resolution must succeed under BOTH pid views — it
    ...
    monkeypatch.setenv("CODEXCREW_HOST_PID", str(SESSION_HOST))
...
#: file (relative to src/codex_crew) -> declared role / namespace assumption
_REGISTERED_CALL_SITES: dict[str, str] = {
    ...
    "mcp_core.py": (
        ...
        "(HMAC-verified, direct CODEXCREW_HOST_PID lookup, no walk)"
    ),
    "mcp_shared.py": (
    ...
    "sandbox.py": (
        "writer-adjacent: launcher exports CODEXCREW_HOST_PID (its own HOST pid — "
    ...
    "mcp_computer.py": (
        ...
        "(HMAC-verified, direct CODEXCREW_HOST_PID lookup) rather than the lenient "
...
def _src_root() -> Path:
    return Path(__file__).resolve().parent.parent / "src" / "codex_crew"
...
def test_reflexive_tools_route_through_the_strict_gate() -> None:
    from codex_crew.mcp_core import REFLEXIVE_TOOL_MODULES
