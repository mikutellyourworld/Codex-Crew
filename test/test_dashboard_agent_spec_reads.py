...
import codex_crew.dashboard.handlers.agents as agents_mod
...
async def test_refused_under_lock_reread_returns_agent_changed(
    ...
    path.write_text(json.dumps({"name": "codexcrew", "model": "old"}), encoding="utf-8")
    ...
    with patch("codex_crew.agent.KIRO_AGENTS_DIR", tmp_path):
        response = await api_agent_detail(_patch_request("codexcrew", {"model": "new"}))
