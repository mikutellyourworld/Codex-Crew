...
from codex_crew.apps.manifest import RESERVED_APP_NAME_CODE
...
async def test_reserved_route_name_refused_before_any_setup() -> None:
    with (
        patch("codex_crew.apps.registry._resolve_install_entry") as resolve,
        patch("codex_crew.apps.registry._clone_build_app", new_callable=AsyncMock) as clone,
    ):
...
async def test_non_reserved_invalid_name_refused_without_reserved_code() -> None:
    with patch("codex_crew.apps.registry._clone_build_app", new_callable=AsyncMock) as clone:
