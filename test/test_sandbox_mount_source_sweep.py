...
Two halves lock the fix in:
  (a) the generated launcher tags every staging site with a pid-bearing
      ``_src_prefix`` (``codexcrew_sb_<pid>_``) assigned in the post-fork CHILD
  ...
  (b) ``_cleanup_stale_sandbox_mount_sources`` reclaims layered by cost: plain
      ...
      ``codexcrew_sandbox_*`` launcher scripts are preserved, without the sweep
      ever raising.
"""
...
def _pin(
    ...
    *,
    ...
    monkeypatch.setattr("codex_crew.sandbox._mount_pinned_source_names", _fake)
...
class TestMountSourceSweep:
    ...
    def test_reclaims_dead_pid_dir_including_non_empty(
        ...
        empty = _make_dir(tmp_path, f"codexcrew_sb_{_DEAD_PID}_abc123")
        shadow = _make_dir(tmp_path, f"codexcrew_sb_{_DEAD_PID}_ssh456", populate=True)
    ...
    def test_reclaims_dead_pid_file(self, tmp_path: Path):
        stale = _make_file(tmp_path, f"codexcrew_sb_{_DEAD_PID}_file01")
    ...
    def test_age_backstop_reclaims_empty_dir_and_file_despite_live_pid(
        ...
        """
        _pin(monkeypatch, set())
        recycled_dir = _make_dir(tmp_path, f"codexcrew_sb_{_LIVE_PID}_old999", old=True)
        recycled_file = _make_file(tmp_path, f"codexcrew_sb_{_LIVE_PID}_old888", old=True)
    ...
    def test_mount_pinned_entry_is_preserved_however_old_and_whatever_the_pid(
        ...
        pin.
        """
        live_name = f"codexcrew_sb_{_LIVE_PID}_shadow1"
        dead_name = f"codexcrew_sb_{_DEAD_PID}_held01"
        empty_name = f"codexcrew_sb_{_DEAD_PID}_mask01"
    ...
    def test_recycled_pid_non_empty_dir_is_reclaimed_when_unpinned(
        ...
        """
        _pin(monkeypatch, set())
        recycled = _make_dir(tmp_path, f"codexcrew_sb_{_LIVE_PID}_recyc1", populate=True, old=True)
    ...
    def test_incomplete_and_uncovered_pin_scan_blocks_dir_removal(
        ...
        held = _make_dir(tmp_path, f"codexcrew_sb_{_DEAD_PID}_part01", populate=True)
        empty = _make_dir(tmp_path, f"codexcrew_sb_{_DEAD_PID}_part03")
        plain = _make_file(tmp_path, f"codexcrew_sb_{_DEAD_PID}_part02")
    ...
    def test_dirs_are_reclaimed_once_every_own_uid_task_was_read(
        ...
        stale = _make_dir(tmp_path, f"codexcrew_sb_{_DEAD_PID}_inherit1", populate=True, old=True)
        live_name = f"codexcrew_sb_{_DEAD_PID}_inherit2"
    ...
    def test_preserves_fresh_live_pid_entry(self, tmp_path: Path):
        live = _make_dir(tmp_path, f"codexcrew_sb_{_LIVE_PID}_live01")
    ...
    def test_rmtree_raising_non_oserror_does_not_kill_the_sweep(
        ...
        _make_dir(tmp_path, f"codexcrew_sb_{_DEAD_PID}_deep01", populate=True)
        plain = _make_file(tmp_path, f"codexcrew_sb_{_DEAD_PID}_zfile1")
    ...
    def test_preserves_foreign_and_unparseable_names(self, tmp_path: Path):
        ...
        Foreign ``tmp*`` (the unprefixed backlog), ``codexcrew_sandbox_*``
        launcher scripts, the ``codexcrew_sbprobe_*`` tmpfs probe, and a
        ...
        keep = [
            ...
            _make_file(tmp_path, f"codexcrew_sandbox_{_DEAD_PID}_x.py", old=True),
            _make_dir(tmp_path, "codexcrew_sbprobe_ab12cd", old=True),
            _make_dir(tmp_path, f"codexcrew_sb_pid{_DEAD_PID}_z", old=True),
        ]
    ...
    def test_planted_hostile_pid_segments_neither_crash_nor_stall_the_sweep(
        ...
        """
        _pin(monkeypatch, set())
        superscript = _make_dir(tmp_path, "codexcrew_sb_\u00b2_x", old=True)
        arabic_indic = _make_dir(tmp_path, "codexcrew_sb_\u0663_x", old=True)
        zero = _make_dir(tmp_path, "codexcrew_sb_0_x", populate=True, old=True)
        padded = _make_dir(tmp_path, f"codexcrew_sb_0{_DEAD_PID}_x", old=True)
        oversized = _make_dir(tmp_path, f"codexcrew_sb_{'9' * 40}_x", old=True)
        genuine = _make_dir(tmp_path, f"codexcrew_sb_{_DEAD_PID}_real1")
    ...
    def test_second_sweep_is_a_no_op(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
        _pin(monkeypatch, set())
        _make_dir(tmp_path, f"codexcrew_sb_{_DEAD_PID}_one", populate=True)
        _make_file(tmp_path, f"codexcrew_sb_{_DEAD_PID}_two")
        survivor = _make_dir(tmp_path, f"codexcrew_sb_{_LIVE_PID}_three")
    ...
    def test_wired_into_periodic_profile_sweep(
        ...
        root = tmp_path / "tmpfs"
        root.mkdir()
        _make_dir(root, f"codexcrew_sb_{_DEAD_PID}_wired", populate=True)
        monkeypatch.setattr("codex_crew.sandbox._mount_source_candidate_roots", lambda: [str(root)])
        ...
        assert removed >= 1
        assert not (root / f"codexcrew_sb_{_DEAD_PID}_wired").exists()
...
class TestMountPinnedSourceNames:
    ...
    def test_extracts_prefix_shaped_basenames_from_the_root_field(self, tmp_path: Path):
        ...
        self._write_mountinfo(
            proc,
            "101",
            "36 35 0:22 /codexcrew_sb_123_ab /home/u/.aws rw,nosuid - tmpfs tmpfs rw\n"
            "37 35 0:22 /codexcrew_sb_123_cd /home/u/.ssh rw,nosuid - tmpfs tmpfs rw\n"
            "38 35 8:1 / / rw - ext4 /dev/sda1 rw\n",
        )

        pinned, complete = _mount_pinned_source_names(proc_root=str(proc))

        assert pinned == {"codexcrew_sb_123_ab", "codexcrew_sb_123_cd"}
    ...
    def test_prefix_in_the_mount_point_field_alone_pins_nothing(self, tmp_path: Path):
        ...
        self._write_mountinfo(
            proc,
            "102",
            "36 35 0:22 /elsewhere /tmp/codexcrew_sb_123_ab rw,nosuid - tmpfs tmpfs rw\n",
        )
    ...
    def test_vanished_process_is_not_a_coverage_gap(self, tmp_path: Path):
        ...
        self._write_mountinfo(
            proc,
            "104",
            "36 35 0:22 /codexcrew_sb_9_zz /home/u/.aws rw - tmpfs tmpfs rw\n",
        )

        pinned, complete = _mount_pinned_source_names(proc_root=str(proc))

        assert pinned == {"codexcrew_sb_9_zz"}
        assert complete is True
    ...
    def test_successor_appearing_after_a_vanish_is_scanned(
        ...
        self._write_mountinfo(
            proc,
            "202",
            "36 35 0:22 /codexcrew_sb_7_hh /home/u/.ssh rw - tmpfs tmpfs rw\n",
        )
        ...
        assert pinned == {"codexcrew_sb_7_hh"}
        assert complete is True
    ...
    def test_namespaceless_task_is_not_a_coverage_gap(
        ...
        self._write_mountinfo(
            proc,
            "301",
            "36 35 0:22 /codexcrew_sb_5_aa /home/u/.aws rw - tmpfs tmpfs rw\n",
        )
        zombie = proc / "302"
        zombie.mkdir()
        ...
        assert pinned == {"codexcrew_sb_5_aa"}
        assert complete is True
    ...
    def test_namespaceless_task_still_forces_a_rescan_for_its_successor(
        ...
        (successor / "mountinfo").write_text(
            "36 35 0:22 /codexcrew_sb_11_bb /home/u/.ssh rw - tmpfs tmpfs rw\n"
        )
        real_listdir = os.listdir
        calls = {"n": 0}
        ...
        assert pinned == {"codexcrew_sb_11_bb"}
        assert complete is True
    ...
    def test_zombie_leader_with_a_live_thread_is_pinned_not_forgiven(
        ...
        (thread_dir / "mountinfo").write_text(
            "36 35 0:22 /codexcrew_sb_21_tt /home/u/.aws rw - tmpfs tmpfs rw\n"
        )

        pinned, complete = _mount_pinned_source_names(proc_root=str(proc))

        assert pinned == {"codexcrew_sb_21_tt"}
        assert complete is True
    ...
    def test_a_readable_sibling_must_not_mask_an_unreadable_one(
        ...
        (readable / "mountinfo").write_text(
            "36 35 0:22 /codexcrew_sb_31_rr /home/u/.aws rw - tmpfs tmpfs rw\n"
        )
        ...
        assert pinned == {"codexcrew_sb_31_rr"}, "the pin that was read must be kept"
    ...
    def test_a_departed_sibling_sinks_coverage_even_beside_a_readable_one(
        ...
        readable.mkdir(parents=True)
        (readable / "mountinfo").write_text(
            "36 35 0:22 /codexcrew_sb_41_aa /home/u/.aws rw - tmpfs tmpfs rw\n"
        )
        ...
        assert pinned == {"codexcrew_sb_41_aa"}, "the pin that was read must be kept"
    ...
    def test_filtered_procfs_without_pid_1_reports_incomplete_but_still_pins(self, tmp_path: Path):
        ...
        (d / "mountinfo").write_text(
            f"1932 1355 0:55 /codexcrew_sb_{_DEAD_PID}_live1 /home/u/.ssh rw - tmpfs tmpfs rw\n"
        )

        pinned, complete = _mount_pinned_source_names(proc_root=str(proc))

        assert complete is False
        assert pinned == {f"codexcrew_sb_{_DEAD_PID}_live1"}
    ...
    def test_a_sibling_thread_in_another_namespace_pins(self, tmp_path: Path):
        ...
        line = "100 99 0:40 /codexcrew_sb_777_home /root/home rw - tmpfs tmpfs rw\n"
        ...
        assert "codexcrew_sb_777_home" in pinned
    ...
    def test_a_sibling_thread_departing_on_the_final_pass_clears_coverage(
        ...
        monkeypatch.setattr("codex_crew.sandbox._PIN_SCAN_MAX_PASSES", 1)
        proc = tmp_path / "proc"
        self._proc_task(proc, 1, mountinfo="")
    ...
    def test_a_sibling_thread_departure_is_re_read_on_the_next_pass(self, tmp_path: Path):
        ...
        with pytest.MonkeyPatch.context() as mp:
            mp.setattr("codex_crew.sandbox.os.listdir", _settling_listdir)
            coverage = _PinScanCoverage()
            _, complete = _mount_pinned_source_names(proc_root=str(proc), coverage=coverage)
    ...
    def _stat_uid_of(monkeypatch: pytest.MonkeyPatch, pid: int, uid: int) -> None:
        ...
        monkeypatch.setattr("codex_crew.sandbox.os.stat", _fake_stat)
    ...
    def test_foreign_uid_departure_keeps_coverage(
        ...
        monkeypatch.setattr("codex_crew.sandbox._PIN_SCAN_MAX_PASSES", 1)
        monkeypatch.setattr("codex_crew.sandbox._overflow_uid", lambda: 65534)
        proc = tmp_path / "proc"
        self._proc_task(proc, 1, mountinfo="")
        self._proc_task(proc, 4242, mountinfo=None)
    ...
    def test_unknown_overflow_uid_makes_a_foreign_departure_clear_coverage(
        ...
        monkeypatch.setattr("codex_crew.sandbox._PIN_SCAN_MAX_PASSES", 1)
        monkeypatch.setattr("codex_crew.sandbox._overflow_uid", lambda: None)
        proc = tmp_path / "proc"
        self._proc_task(proc, 1, mountinfo="")
        self._proc_task(proc, 4242, mountinfo=None)
        self._stat_uid_of(monkeypatch, 4242, (os.getuid() if hasattr(os, "getuid") else 1000) + 1)
    ...
    def test_gate_retains_a_pinned_dir_under_an_incomplete_but_covered_scan(
        ...
        name = f"codexcrew_sb_{_DEAD_PID}_setsid1"
...
class TestLegacyResidueSweep:
    ...
    def _fence(
        ...
        *,
        ...
        if covered is None:
            covered = complete
        monkeypatch.setattr("codex_crew.sandbox._launcher_tmpfs_roots", lambda: [str(root)])
        ...
        monkeypatch.setattr("codex_crew.sandbox._bound_source_basenames", _fake)
        # Each test below pins ONE fence; the pile threshold has its own tests.
        monkeypatch.setattr("codex_crew.sandbox._LEGACY_PILE_THRESHOLD", 1)
    ...
    def test_a_planted_symlink_at_the_marker_is_not_followed(
        ...
        from codex_crew.sandbox import _LEGACY_RESIDUE_MARKER, config_dir
    ...
    def test_unproven_bind_coverage_removes_nothing_and_does_not_retire_the_pass(
        ...
        with caplog.at_level(logging.INFO, logger="codex_crew.sandbox"):
            assert _cleanup_legacy_mount_source_residue() == 0
        assert held.exists()
    ...
    def test_an_absent_root_skips_the_pin_scan_without_a_warning(
        ...
        monkeypatch.setattr(
            "codex_crew.sandbox._launcher_tmpfs_roots", lambda: [str(tmp_path / "absent")]
        )
    ...
    def test_bound_scan_keys_on_the_tmpfs_relative_root_field(
        ...
        monkeypatch.setattr(
            "codex_crew.sandbox._mount_pinned_source_names",
        ...
        (proc / "202" / "mountinfo").write_text(
            ...
            "1932 1355 0:55 /codexcrew_sb_7_x /home/u/.ssh rw,nosuid - tmpfs tmpfs rw\n"
    ...
    def test_fresh_foreign_and_wrong_shaped_entries_survive(
        ...
        keyed = _make_dir(tmp_path, f"codexcrew_sb_{_DEAD_PID}_keyed01")
    ...
    def test_below_the_pile_threshold_everything_is_retained_and_the_pass_retires(
        ...
        """
        self._fence(monkeypatch, tmp_path)
        monkeypatch.setattr("codex_crew.sandbox._LEGACY_PILE_THRESHOLD", 4)
    ...
    def test_wired_into_the_periodic_entry_point(
        ...
        self._fence(monkeypatch, tmp_path)
        monkeypatch.setattr(
            "codex_crew.sandbox._mount_source_candidate_roots", lambda: [str(tmp_path)]
        )
    ...
    def test_reclaim_starts_at_boot_without_holding_the_loop(self):
        ...
        """
        import codex_crew.session_cleanup as cleanup_mod
...
class TestSweepTimeBudget:
    ...
    def _spent_budget(self) -> pytest.MonkeyPatch:
        ...
        — including the ``CODEXCREW_HOME`` pin — and this sweep then stamps its
        ...
        budget.setattr("codex_crew.sandbox._SWEEP_TIME_BUDGET_SECONDS", -1.0)
        budget.setattr("codex_crew.sandbox._SWEEP_BUDGET_CHECK_EVERY", 2)
        return budget
    ...
    def test_keyed_pass_stops_at_the_budget_and_resumes(
        ...
        for index in range(4):
            _make_file(tmp_path, f"codexcrew_sb_{_DEAD_PID}_file{index:02d}")
    ...
    def test_truncated_legacy_pass_does_not_stamp_the_one_shot_marker(
        ...
        monkeypatch.setattr("codex_crew.sandbox._launcher_tmpfs_roots", lambda: [str(tmp_path)])
        monkeypatch.setattr(
            "codex_crew.sandbox._bound_source_basenames",
...
class TestHeldBackDiagnostic:
    ...
    def test_unprovable_coverage_is_reported_as_a_fault(
        ...
        _pin(monkeypatch, set(), complete=False)
        _make_dir(tmp_path, f"codexcrew_sb_{_DEAD_PID}_held01")
    ...
    def test_a_live_pin_stays_ordinary_information(
        ...
        name = f"codexcrew_sb_{_DEAD_PID}_pinned1"
    ...
    def test_every_tempfile_call_in_the_launcher_carries_a_known_prefix(self, level: str):
        ...
        for call in calls:
            ...
            ok_probe = (
                ...
                and prefix_kw.value.value == "codexcrew_sbprobe_"
            )
