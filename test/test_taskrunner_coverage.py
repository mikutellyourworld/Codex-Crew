"""Coverage-focused tests for the :mod:`codex_crew.taskrunner` orchestrator.
...
from codex_crew import taskrunner as tr
from codex_crew.safety_override import safety_override
from codex_crew.task_models import Project, Task, TaskStatus
from codex_crew.taskrunner import TaskRunner, _auto_approve_scope, _resolve_workspace_dir
...
class TestLogTask:
    ...
    async def test_on_loop_failure_is_reported_not_raised(
        ...
        runner = _runner(tmp_path, conversation_log=log)
        run = _seed_run(runner, tmp_path)
        with caplog.at_level(logging.DEBUG, logger="codex_crew.taskrunner"):
            runner._log_task("hist", run, Task(index=1, title="T", description="d"))
            for _ in range(100):
