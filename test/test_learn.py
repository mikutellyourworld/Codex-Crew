...
import codex_crew.learn as learn_mod
from codex_crew.atomic_write import replace_with_retry
from codex_crew.learn import Lesson, LessonStore

# Read through the MODULE, never `from codex_crew.learn import _DEFAULT_DIR`.
...
class TestLessonStoreSecurity:
    ...
    def test_sensitive_base_dir_emits_sel_audit(self, tmp_path: Path) -> None:
        ...
        sensitive.mkdir()
        with (
            patch("codex_crew.security.is_sensitive_path", return_value=True),
            patch("codex_crew.sel.SecurityEventLog.log_tool_invocation") as mock_log,
        ):
    ...
    def test_sel_failure_does_not_bypass_fallback(self, tmp_path: Path) -> None:
        ...
        with (
            patch("codex_crew.security.is_sensitive_path", return_value=True),
            patch(
                "codex_crew.sel.SecurityEventLog.log_tool_invocation",
    ...
    def test_sensitive_config_dir_falls_back_to_default(self, tmp_path: Path) -> None:
        sensitive = tmp_path / ".codexcrew-sensitive"
        sensitive.mkdir()
        with (
            patch("codex_crew.learn._config_dir", return_value=sensitive),
            patch("codex_crew.security.is_sensitive_path", return_value=True),
        ):
            store = LessonStore()
        assert store._dir == _default_dir()
    ...
    def test_config_dir_exception_falls_back_to_default(self) -> None:
        with patch("codex_crew.learn._config_dir", side_effect=OSError("broken loader")):
            store = LessonStore()
        assert store._dir == _default_dir()

    def test_config_dir_none_falls_back_to_default(self) -> None:
        with patch("codex_crew.learn._config_dir", None):
            store = LessonStore()
        assert store._dir == _default_dir()
...
class TestImportPurity:
    def test_importing_learn_never_calls_config_dir(self) -> None:
        ...
        with patch(
            "codex_crew.config.loader.config_dir",
...
class TestSaveOrEnrich:
    ...
    def test_the_rename_goes_through_the_windows_retry_helper(self, tmp_path: Path) -> None:
        ...
        store = LessonStore(base_dir=tmp_path)
        with patch(
            "codex_crew.atomic_write.replace_with_retry", wraps=replace_with_retry
        ) as spy:
            store.save(_make_lesson("Pin the port", "tool"))
...
class TestAtomicWrite:
    ...
    def test_a_failed_swap_leaves_the_store_and_cache_intact(self, tmp_path: Path) -> None:
        ...
        with patch(
            "codex_crew.atomic_write.replace_with_retry", side_effect=OSError("disk full")
        ):
