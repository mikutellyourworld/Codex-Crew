...
from codex_crew.computer_use import (
    macos_driver,
    macos_ffi,
    permissions,
    policy,
...
_APP = AppRef(name="Fixture", pid=4242, bundle_id="dev.codexcrew.fixture", window_id=77)
...
class TestWindowSelection:
    def test_selects_by_title_stem_when_ids_do_not_correlate(self, fakes: _Fakes):
        ...
        app = AppRef(
            name="Fixture",
            pid=4242,
            bundle_id="dev.codexcrew.fixture",
            window_id=77,
...
class TestMacOSPointerPaths:
    ...
    def _owns_the_point(self, monkeypatch):
        ...
        """
        from codex_crew.computer_use import apps_macos
...
class TestChildCapMarksTheWalkTruncated:
    ...
    def test_a_capped_walk_suppresses_the_screenshot(self, fakes: _Fakes, monkeypatch):
        ...
        """
        from codex_crew.computer_use import capture_macos
...
class TestDenylistCaseFolding:
    ...
    gone, so this exercises ``codexcrew_self`` — the rule that keeps the keystone
    ...
    def test_matching_is_case_insensitive_on_both_sides(self):
        ...
        for name, bundle in (
            ("CODEX CREW", "DEV.CODEX.CREW"),
            ("codex crew", "dev.kiro.crew"),
            ("Codex Crew", "Dev.Kiro.Crew"),
        ):
...
class TestReviewerFoundRegressions:
    ...
    def test_a_real_pointer_click_outside_the_authorized_window_is_refused(
        ...
        """
        from codex_crew.computer_use import apps_macos, macos_driver
        from codex_crew.computer_use.types import ClickRequest
