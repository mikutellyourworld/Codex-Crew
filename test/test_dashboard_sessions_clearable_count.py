...
from codex_crew.dashboard.handlers import api_sessions_clear, api_sessions_clearable_count
from codex_crew.dashboard.handlers.sessions import _clearable_history_keys
...
def _history_key_for(key: str) -> str:
    from codex_crew.dashboard.chat import _history_key_for as _hkf
...
async def test_count_and_bulk_delete_resolve_the_same_set() -> None:
    ...
    with (
        patch(
            "codex_crew.dashboard.handlers._remove_slot_for_history_key",
            new=AsyncMock(return_value=None),
        ),
        patch("codex_crew.dashboard.handlers.sel"),
    ):
