"""Tests for codex_crew.validation — tool input/output validation."""
...
class TestValidateToolArgs:
    ...
    def test_cron_add_still_accepts_posix_script_path(self):
        result = validate_tool_args(
            {"name": "p", "script": "~/.codex-crew/crons/job.py:run", "every": 300},
            CRON_ADD_SCHEMA,
        )
        assert result["script"] == "~/.codex-crew/crons/job.py:run"
...
class TestDeployArtifactSchema:
    ...
    def test_schema_accepts_valid_artifact_slug(self):
        from codex_crew.validation import DEPLOY_ARTIFACT_SCHEMA
