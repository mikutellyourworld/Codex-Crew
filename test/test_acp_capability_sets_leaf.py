"""The capability sets must be askable without importing ``codex_crew.acp``.

They used to be defined in ``codex_crew.acp.types``. Importing anything under
``codex_crew.acp`` executes that package's ``__init__`` (client + runtime), and
``codex_crew.acp`` is a FORBIDDEN_ROOT for the agent-SDK boundary gate -- so every
...
Definitions now live in the leaf ``codex_crew.acp_backends`` and are re-exported from
...
SRC = Path(__file__).resolve().parent.parent / "src" / "codex_crew"
...
def test_defined_in_the_leaf_not_in_the_acp_package(name: str) -> None:
    ...
    assert f"\n{name} = frozenset(" not in types_mod, (
        ...
        f"acp_backends.py so a consumer can read it without importing codex_crew.acp"
    )


@pytest.mark.parametrize("name", CAPABILITY_SETS)
def test_the_re_export_is_the_same_object(name: str) -> None:
    """Existing ``from codex_crew.acp.types import ...`` call sites keep working.

    Identity, not equality: a copy would drift the moment one side is edited.
    """
    import codex_crew.acp_backends as leaf
    from codex_crew.acp import types as acp_types
...
def test_reading_a_capability_set_does_not_load_the_acp_package() -> None:
    ...
    avoidable. If the sets drifted back behind ``codex_crew.acp``, every consumer
    ...
    probe = (
        ...
        "from codex_crew.acp_backends import ACP_BACKENDS_STEER, ACP_BACKENDS_ACP_RUNTIME\n"
        "assert isinstance(ACP_BACKENDS_STEER, frozenset)\n"
        "print(repr({'acp': 'codex_crew.acp' in sys.modules,\n"
        "            'providers': 'codex_crew.providers' in sys.modules}))\n"
    )
    ...
    assert loaded["acp"] is False, (
        "reading a capability set loaded codex_crew.acp; the sets must stay in the "
        "leaf so an outside consumer can ask without a forbidden-root import"
    )
    assert loaded["providers"] is False, "reading a capability set loaded codex_crew.providers"
