...
from codex_crew.task_models import MAX_RETRIES
...
class TestScenarioReviewModes:
    ...
    async def test_review_uses_diff_when_branch_exists(self, tmp_path: Path) -> None:
        ...
        with patch(
            "codex_crew.task_executor.stream_and_collect_json", return_value={"ok": True}
        ) as mock_json, patch(
            "codex_crew.task_executor.git_coord.get_step_diff", return_value=fake_diff
        ):
...
class TestScenarioStepPromptWithGit:
    ...
    async def test_prompt_includes_git_log(self, tmp_path: Path) -> None:
        ...
        with patch("codex_crew.git_coord.get_state_summary", return_value=fake_summary):
