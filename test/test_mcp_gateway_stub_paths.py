...
def _clear_home_env(monkeypatch: pytest.MonkeyPatch) -> None:
    """Reproduce a Windows environment: no CODEXCREW_HOME and no HOME."""
    monkeypatch.delenv("CODEXCREW_HOME", raising=False)
    monkeypatch.delenv("HOME", raising=False)


def test_codexcrew_home_wins_when_set(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path))
    assert _crew_home() == tmp_path
    assert _default_socket_path() == str(tmp_path / "codexcrew-mcp-gateway.sock")
...
def test_unresolvable_home_degrades_instead_of_raising(
    ...
    assert _default_socket_path().endswith("codexcrew-mcp-gateway.sock")
