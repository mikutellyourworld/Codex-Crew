...
from codex_crew import kiro_cli as kiro_cli_mod
...
MANAGED = ("codexcrew-core", "codexcrew-cron", "codexcrew-computer")

# ``codexcrew-computer`` carries a ``spec_gate`` and is therefore absent from an
...
def _point_config_at(tmp_path: Path, monkeypatch) -> None:
    ...
    the loader, which resolves from ``CODEXCREW_HOME``; the fallback path reads the
    ...
    if hasattr(CodexCrewConfig.load, "cache_clear"):
        CodexCrewConfig.load.cache_clear()
...
class TestRegistryModeDeclaration:
    ...
    def test_config_loader_round_trips_the_field(self, tmp_path, monkeypatch):
        _write_config(tmp_path, registry_mode=True)
        monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path))
        CodexCrewConfig.load.cache_clear() if hasattr(CodexCrewConfig.load, "cache_clear") else None
    ...
    def test_local_overlay_declaration_is_honoured(self, tmp_path, monkeypatch):
        """`codexcrew config set --local` writes config.local.json, which deep-merges
...
class TestIdentityProbeIsAudited:
    ...
    def test_probe_emits_an_audit_event(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.hooks.emit_internal_read_audit", _spy)
    ...
    def test_probe_fails_closed_when_the_audit_is_unavailable(self, tmp_path, monkeypatch):
        ...
        db = kiro_cli_state_dbs("linux", tmp_path, {})[0]
        TestIdcDetection._make_db(db, ("auth.idc.start-url",))
        monkeypatch.setattr("codex_crew.hooks.emit_internal_read_audit", lambda *_: False)
...
class TestFreshInstallMarker:
    ...
    def test_marker_does_not_replace_the_command(self, monkeypatch):
        ...
        monkeypatch.setattr(agent_mod, "_mcp_registry_mode", lambda: True)
        entry = agent_mod.build_agent_config(gated_off=NO_GATES)["mcpServers"]["codexcrew-core"]
...
class TestRefreshMarker:
    ...
    def test_refresh_preserves_a_user_transport_hint(self, monkeypatch):
        ...
        monkeypatch.setattr(agent_mod, "_mcp_registry_mode", lambda: False)
        config = {"mcpServers": {"codexcrew-core": {"command": "x", "args": [], "type": "stdio"}}}
        agent_mod._refresh_dynamic_fields(config, gated_off=NO_GATES)
        assert config["mcpServers"]["codexcrew-core"]["type"] == "stdio"
...
class TestDoctorGovernanceSection:
    def _spec(self, tmp_path: Path, *, marked: bool) -> Path:
        entry: dict[str, object] = {"command": "codexcrew", "args": ["mcp-core"]}
        if marked:
            entry["type"] = "registry"
        path = tmp_path / "codexcrew.json"
        path.write_text(
    ...
    def test_silent_on_a_personal_account(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(
            cli_doctor.CodexCrewConfig, "load", staticmethod(lambda: _cfg(registry_mode=False))
        )
        issues: list[str] = []
    ...
    def test_declared_on_a_non_idc_account_is_flagged(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(cli_doctor, "mcp_governance_may_apply", lambda: False)
        monkeypatch.setattr(
            cli_doctor.CodexCrewConfig, "load", staticmethod(lambda: _cfg(registry_mode=True))
        )
        issues: list[str] = []
...
class TestDoctorSurvivesAMalformedSpec:
    ...
    @pytest.mark.parametrize(
        "servers", ["not-a-dict", ["codexcrew-core"], 7, True, "", [], {}]
    )
