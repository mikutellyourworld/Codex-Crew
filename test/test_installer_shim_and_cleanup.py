"""Regression gates for the packaged-install path of the Codex Crew installer.
...
Property A — the packaged launcher must resolve
    ``_resolve_codexcrew_bin()`` reaches the desktop bundle's launcher by walking
    up from ``codex_crew.__file__``: the bundle is a python-build-standalone
    ...
    back to the bare string ``"codexcrew"`` — which is not on ``PATH`` inside a
    bundle — and ``build_agent_config`` / ``rebuild_agent_config`` then DROP
    ``codexcrew-core`` and ``codexcrew-cron``, taking ``spawn_run``, ``cron_add``,
    ``learn_add`` … offline.

    NOTE: a live ``codexcrew mcp-core`` stdio handshake PASSES even when this is
...
Defect B — stale predecessor MCP entries
    ``clean_stale_managed_mcp()`` only removes ``codexcrew-*`` entries unless an
...
import codex_crew
import codex_crew.agent as agent
import codex_crew.mcp_cleanup as mcp_cleanup
from codex_crew.config.loader import CodexCrewConfig
from codex_crew.platform.bootstrap import build_default_context
from codex_crew.platform.context import reset_context, set_context
from codex_crew.platform.interfaces import ImportSource

# The ~/.local/bin symlink shim is POSIX-only: ensure_codexcrew_on_path returns
# early on Windows (pip's Scripts\codexcrew.exe is the launcher there, and a
...
_posix_shim_only = pytest.mark.skipif(
    sys.platform == "win32",
    reason="POSIX ~/.local/bin symlink shim; Windows uses pip's Scripts\\codexcrew.exe",
)
...
def _fake_bundle_launcher(tmp_path: Path) -> Path:
    ...
    The path comes from :func:`agent._codexcrew_bin_subpath`, the same helper the
    resolver uses, so the fixture cannot drift from the layout under test (or from
    the per-OS naming: ``bin/codexcrew`` on POSIX, ``Scripts\\codexcrew.exe`` on
    Windows).
    """
    root = tmp_path / "backend-dist" / "codexcrew-backend"
    launcher = agent._codexcrew_bin_subpath(root)
...
def _simulate_bundled_app(monkeypatch, launcher: Path) -> None:
    ...
    resolver's walk-up from ``codex_crew.__file__`` is what reaches the bundle's
    ...
    depend on the tree it runs in, and leave nothing named ``codexcrew`` on PATH.
    """
    root = launcher.parent.parent
    pkg_init = root / "lib" / "python3.12" / "site-packages" / "codex_crew" / "__init__.py"
    pkg_init.parent.mkdir(parents=True, exist_ok=True)
    pkg_init.touch()
    monkeypatch.setattr(agent, "_CODEXCREW_BIN", "", raising=False)
    monkeypatch.setattr(codex_crew, "__file__", str(pkg_init))
    monkeypatch.setattr(agent, "_bin_is_usable", lambda p: str(p) == str(launcher))
    # `codexcrew` is not on PATH; only absolute paths resolve.
    monkeypatch.setattr(
...
def test_resolver_finds_the_bundled_launcher(tmp_path, monkeypatch):
    ...
    bundle's own launcher instead of the bare ``"codexcrew"`` sentinel."""
    launcher = _fake_bundle_launcher(tmp_path)
    _simulate_bundled_app(monkeypatch, launcher)

    resolved = agent._resolve_codexcrew_bin()

    assert resolved == str(launcher), (
        "bundled app must resolve to its own launcher, not bare 'codexcrew' "
        f"(got {resolved!r})"
    )
...
def test_managed_servers_survive_in_the_desktop_bundle(tmp_path, monkeypatch):
    """build_agent_config() must give codexcrew-core/codexcrew-cron an absolute,
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(patch.multiple("codex_crew.agent", _BUNDLED_CFG_DIR=cfg_dir))
        stack.enter_context(
            patch("codex_crew.agent._user_overrides_path", return_value=_missing_overrides)
        )
        stack.enter_context(
            patch("codex_crew.agent._prompt_path", return_value=cfg_dir / "prompt.md")
        )
        stack.enter_context(
            patch("codex_crew.agent._mc_config_path", return_value=tmp_path / "missing_mc.json")
        )
        config = agent.build_agent_config()

    servers = config.get("mcpServers", {})
    for name in ("codexcrew-core", "codexcrew-cron"):
...
# --------------------------------------------------------------------------
@_posix_shim_only
def test_ensure_codexcrew_on_path_creates_shim(tmp_path, monkeypatch):
    """A bundled app with no `codexcrew` on PATH must get a shim pointing at the
    bundle's launcher."""
    exe = _fake_bundle_launcher(tmp_path)
    _simulate_bundled_app(monkeypatch, exe)
    bin_dir = tmp_path / "localbin"

    created = agent.ensure_codexcrew_on_path(bin_dir=bin_dir)
...
@_posix_shim_only
def test_ensure_codexcrew_on_path_idempotent(tmp_path, monkeypatch):
    """Re-running setup when the shim is already correct is a no-op."""
    exe = _fake_bundle_launcher(tmp_path)
    _simulate_bundled_app(monkeypatch, exe)
    bin_dir = tmp_path / "localbin"

    assert agent.ensure_codexcrew_on_path(bin_dir=bin_dir) is not None
    assert agent.ensure_codexcrew_on_path(bin_dir=bin_dir) is None
    assert os.path.realpath(bin_dir / "codexcrew") == os.path.realpath(exe)


def test_ensure_codexcrew_on_path_is_noop_on_windows(tmp_path, monkeypatch):
    """On Windows the POSIX symlink shim must be skipped entirely — pip's
    Scripts\\codexcrew.exe is the launcher, and attempting the symlink raises
    ...
    with patch.object(agent, "_resolve_codexcrew_bin", return_value=str(tmp_path / "kc")):
        assert agent.ensure_codexcrew_on_path(bin_dir=bin_dir) is None
...
# The desktop app launches `codexcrew gateway` (never `codexcrew setup`), so
...
def _seed_global_mcp(path: Path) -> None:
    ...
    path.write_text(
        json.dumps(
            {
                "mcpServers": {
                    "codexcrew-core": {"command": "codexcrew", "args": ["mcp-core"]},
                    "codexcrew-cron": {"command": "codexcrew", "args": ["mcp-cron"]},
                    "predecessor-core": {
...
def test_first_run_delivers_shim_and_purge(tmp_path, monkeypatch):
    ...
    link = tmp_path / ".local" / "bin" / "codexcrew"
    ...
    assert {"predecessor-core", "predecessor-cron", "codexcrew-core", "codexcrew-cron"}.isdisjoint(
        remaining
    )
...
def test_first_run_purge_is_one_time(tmp_path, monkeypatch):
    ...
    assert (tmp_path / ".local" / "bin" / "codexcrew").is_symlink()
...
def test_first_run_is_best_effort(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(agent, "ensure_codexcrew_on_path", _raise)
    monkeypatch.setattr("codex_crew.mcp_cleanup.clean_stale_managed_mcp", _raise_purge)
...
def _spy_backfill(monkeypatch, calls, *, raises=None):
    ...
    monkeypatch.setattr("codex_crew.apps.manager.backfill_default_on_builtins", _fake)
...
def test_first_run_survives_a_failing_backfill(tmp_path, monkeypatch):
    ...
    pip's `Scripts\\codexcrew.exe`), and skipping the whole case on Windows would
...
def test_resolver_never_returns_the_interpreter(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        agent.shutil, "which", lambda c, **kw: str(path_bin) if c == "codexcrew" else None
    )
...
# --------------------------------------------------------------------------
# ensure_codexcrew_on_path — edge cases
# --------------------------------------------------------------------------
def test_ensure_shim_noop_when_no_binary(tmp_path, monkeypatch):
    ...
    bin_dir = tmp_path / "localbin"
    assert agent.ensure_codexcrew_on_path(bin_dir=bin_dir) is None
    assert not (bin_dir / "codexcrew").exists()
...
def test_ensure_shim_refreshes_stale_symlink(tmp_path, monkeypatch):
    ...
    stale.write_text("x")
    stale.chmod(0o755)
    (bin_dir / "codexcrew").symlink_to(stale)

    created = agent.ensure_codexcrew_on_path(bin_dir=bin_dir)
    assert created == str(bin_dir / "codexcrew")
    assert os.path.realpath(bin_dir / "codexcrew") == os.path.realpath(exe)
...
def test_ensure_shim_noop_when_already_on_path(tmp_path, monkeypatch):
    ...
    # `codexcrew` already resolves on PATH to the SAME binary.
    monkeypatch.setattr(
        agent.shutil, "which", lambda c, **kw: str(exe) if c == "codexcrew" else None
    )
    bin_dir = tmp_path / "localbin"
    assert agent.ensure_codexcrew_on_path(bin_dir=bin_dir) is None
    assert not (bin_dir / "codexcrew").exists()
...
# --------------------------------------------------------------------------
# ensure_codexcrew_on_path — never follow an ephemeral git worktree
#
# `git worktree remove` deletes the tree's .venv with it, so a shim pointing
# there dangles and `codexcrew` breaks machine-wide, not just in that tree.
# --------------------------------------------------------------------------
def _checkout_with_codexcrew(root: Path, *, linked_worktree: bool, bare_parent: bool = False) -> Path:
    """Build a fake checkout at *root* whose venv holds a `codexcrew` entrypoint.
    ...
    **bare** repo produces — ``<repo>.git/worktrees/<name>``, with no ``.git``
    ...
    """
    binary = root / ".venv" / "bin" / "codexcrew"
...
def _resolve_to(monkeypatch, binary: Path) -> None:
    ...
    monkeypatch.setattr(agent, "_CODEXCREW_BIN", "", raising=False)
    monkeypatch.setattr(agent, "_resolve_codexcrew_bin", lambda: str(binary))
    monkeypatch.setattr(agent.shutil, "which", lambda c, **kw: None)
...
def test_in_linked_git_worktree_distinguishes_marker_kind(tmp_path):
    ...
    wt = _checkout_with_codexcrew(tmp_path / "wt-feature", linked_worktree=True)
    clone = _checkout_with_codexcrew(tmp_path / "clone", linked_worktree=False)
    ...
    assert agent._in_linked_git_worktree(tmp_path / "nowhere" / "bin" / "codexcrew") is False
...
def test_ensure_shim_declines_a_worktree_target(tmp_path, monkeypatch):
    ...
    binary = _checkout_with_codexcrew(tmp_path / "wt-feature", linked_worktree=True)
    _resolve_to(monkeypatch, binary)
    bin_dir = tmp_path / "localbin"

    assert agent.ensure_codexcrew_on_path(bin_dir=bin_dir) is None
    assert not (bin_dir / "codexcrew").exists(), "worktree venv must not be linked"
...
def test_ensure_shim_declines_a_bare_repo_worktree_target(tmp_path, monkeypatch):
    ...
    binary = _checkout_with_codexcrew(
        tmp_path / "wt-from-bare", linked_worktree=True, bare_parent=True
    )
    _resolve_to(monkeypatch, binary)
    bin_dir = tmp_path / "localbin"
...
def test_ensure_shim_links_an_ordinary_clone_target(tmp_path, monkeypatch):
    ...
    binary = _checkout_with_codexcrew(tmp_path / "clone", linked_worktree=False)
    _resolve_to(monkeypatch, binary)
    bin_dir = tmp_path / "localbin"
    ...
    assert created == str(bin_dir / "codexcrew")
    assert os.path.realpath(bin_dir / "codexcrew") == os.path.realpath(binary)
...
def test_ensure_shim_keeps_a_working_shim_when_target_is_a_worktree(tmp_path, monkeypatch):
    ...
    good = _checkout_with_codexcrew(tmp_path / "clone", linked_worktree=False)
    bin_dir = tmp_path / "localbin"
    bin_dir.mkdir()
    (bin_dir / "codexcrew").symlink_to(good)

    worktree_binary = _checkout_with_codexcrew(tmp_path / "wt-feature", linked_worktree=True)
    _resolve_to(monkeypatch, worktree_binary)

    assert agent.ensure_codexcrew_on_path(bin_dir=bin_dir) is None
    assert os.path.realpath(bin_dir / "codexcrew") == os.path.realpath(good)
    assert os.path.exists(bin_dir / "codexcrew"), "shim must not be left dangling"
...
def test_clean_stale_purges_deleted_playwright_proxy(tmp_path, monkeypatch):
    ...
    p.write_text(
        json.dumps(
            {
                "mcpServers": {
                    "@playwright/mcp": {"command": "codexcrew", "args": ["mcp-playwright-proxy"]},
                    # Launched by codexcrew but with a verb that still EXISTS,
                    # and deliberately NOT one of CODEXCREW_BIN_MCP_SERVERS (those
                    # are purged by the separate install-path rule). Proves the
                    # new purge matches the deleted VERB, not "anything codexcrew
                    # launches".
                    "operator-own-server": {"command": "codexcrew", "args": ["mcp-core"]},
                    "ai-community-slack-mcp": {"command": "ai-community-slack-mcp", "args": []},
                }
            },
            indent=2,
    ...
    assert "operator-own-server" in remaining  # other codexcrew-launched verb kept
...
# re-injected from ~/.codex-crew/mcp.json by the merge passes.
# --------------------------------------------------------------------------
...
def test_rebuild_purge_drops_reinjected_entry_on_second_rebuild():
    ...
    # Simulate re-injection from ~/.codex-crew/mcp.json on next rebuild.
...
def test_in_ephemeral_tree_matches_the_runtime_mount(tmp_path):
    ...
    mount = tmp_path / ".mount_CodexCrewAbc123"
    binary = mount / "resources" / "backend-dist" / "codexcrew-backend" / "bin" / "codexcrew"
    assert agent._in_ephemeral_tree(binary) is True
    # A durable install is not condemned by the same check.
    assert agent._in_ephemeral_tree(Path("/opt/CodexCrew/resources/bin/codexcrew")) is False
    assert agent._in_ephemeral_tree(tmp_path / "clone" / "bin" / "codexcrew") is False
...
def test_in_ephemeral_tree_honors_appdir(tmp_path):
    ...
    assert agent._in_ephemeral_tree(tmp_path / "elsewhere" / "codexcrew", env) is False


def test_shim_declines_an_appimage_mount_target(tmp_path, monkeypatch):
    """The guard's payoff: ensure_codexcrew_on_path() runs on EVERY gateway start,
    so without it an AppImage re-creates a dangling ~/.local/bin/codexcrew every
    time. Declining leaves whatever already worked in place."""
    mount = tmp_path / ".mount_CodexCrewXyz789"
    binary = mount / "bin" / "codexcrew"
    binary.parent.mkdir(parents=True)
    binary.write_text("#!/bin/sh\nexit 0\n")
    binary.chmod(0o755)
    _resolve_to(monkeypatch, binary)
    ...
    bin_dir = tmp_path / "localbin"
    assert agent.ensure_codexcrew_on_path(bin_dir) is None
    assert not (bin_dir / "codexcrew").exists()


def test_launcher_whose_venv_is_gone_is_not_usable(tmp_path):
    """The reaped-work-directory case, observed live: a clone's `bin/codexcrew`
    survives while its `.venv` is deleted, so the file is readable and executable
    but fails at run time. Publishing it as the machine-wide `codexcrew` writes a
    command that is broken the moment it is written."""
    root = tmp_path / "kc-work-dir"
    launcher = root / "bin" / "codexcrew"
    launcher.parent.mkdir(parents=True)
    launcher.write_text('#!/bin/sh\nexec "$(dirname "$0")/../.venv/bin/python" -m codex_crew "$@"\n')
    launcher.chmod(0o755)
...
def test_console_script_is_judged_by_its_own_shebang(tmp_path):
    ...
    script = tmp_path / "localbin" / "codexcrew"
...
def test_launcher_naming_no_interpreter_stays_usable(tmp_path):
    ...
    plain = tmp_path / "bin" / "codexcrew"
    plain.parent.mkdir(parents=True)
    plain.write_text("#!/bin/sh\nexit 0\n")
    plain.chmod(0o755)
    assert agent._bin_is_usable(plain) is True

    compiled = tmp_path / "bin" / "codexcrew.exe"
...
# and `ensure_codexcrew_on_path` runs on every gateway start.
# --------------------------------------------------------------------------
...
def test_explicit_setup_claims_the_name(tmp_path, monkeypatch):
    """`codexcrew setup` names an install deliberately, so it MAY take over."""
    exe = _fake_bundle_launcher(tmp_path)
    _simulate_bundled_app_honest(monkeypatch, tmp_path, exe)
    foreign = _foreign_working_launcher(tmp_path)
    bin_dir = tmp_path / "localbin"
    ...
    created = agent.ensure_codexcrew_on_path(bin_dir=bin_dir, claim_existing=True)
...
def test_gateway_start_repairs_a_dangling_launcher(tmp_path, monkeypatch):
    ...
    bin_dir.mkdir()
    link = bin_dir / "codexcrew"
    link.symlink_to(tmp_path / "reaped" / "bin" / "codexcrew")
...
def test_gateway_start_replaces_launcher_whose_interpreter_vanished(tmp_path, monkeypatch):
    ...
    This is the shape that made the live host's `codexcrew` fail -- a readable,
...
def test_gateway_start_does_not_shadow_a_working_launcher_elsewhere_on_path(tmp_path, monkeypatch):
    ...
    `codexcrew` while ~/.local/bin holds none. Creating one there would shadow it
    ...
    foreign = _foreign_working_launcher(tmp_path)
    _simulate_bundled_app_honest(monkeypatch, tmp_path, exe)
    # `codexcrew` resolves on PATH to the other install, and nothing is in bin_dir.
    monkeypatch.setattr(
        agent.shutil, "which", lambda c, **kw: str(foreign) if c == "codexcrew" else None
    )
    bin_dir = tmp_path / "localbin"
...
def test_explicit_setup_may_shadow_a_launcher_elsewhere_on_path(tmp_path, monkeypatch):
    """`codexcrew setup` names this install, so it may publish into bin_dir."""
    exe = _fake_bundle_launcher(tmp_path)
    foreign = _foreign_working_launcher(tmp_path)
    _simulate_bundled_app_honest(monkeypatch, tmp_path, exe)
    monkeypatch.setattr(
