...
:mod:`codex_crew.changelog` by path rather than re-implementing it, so there is no
...
from codex_crew.changelog import base_version
...
def test_the_gate_reads_the_grammar_from_the_renderer_it_is_given() -> None:
    ...
    renderer = gate._load_renderer_source(
        (_REPO / "src" / "codex_crew" / "changelog.py").read_text(encoding="utf-8")
    )
