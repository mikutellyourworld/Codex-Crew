...
Advisory by construction: an unconfigured AWS profile is not a Codex Crew fault, so
...
class TestCredentialsSection:
    ...
    def test_nothing_under_dot_aws_is_opened(self, fake_home, monkeypatch, capsys):
        ...
        ``codexcrew doctor`` is reachable from a tool call — so parsing the config
...
class TestNeitherProbeResolvesThroughPath:
    """The probes run under an OPERATOR's ``codexcrew doctor``, outside the sandbox.
...
class TestOperatorCopyPointsSomewhereReachable:
    ...
    def test_the_doc_pointer_is_a_url_not_a_dashboard_page_or_repo_path(
        ...
        GitHub links), and ``src/codex_crew/...`` is a repo path absent from a wheel
        ...
        assert "src/codex_crew/docs/blocked-commands.md." not in out, "bare repo path"
        assert issues == []
