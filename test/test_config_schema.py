...
from codex_crew.config import schema as _schema_module
...
def _all_fields_recursive(
    ...
    user-facing config — e.g. ``CodexCrewConfig._extra_sections``, which
    ...
    for f in dataclasses.fields(cls):
        ...
        if isinstance(tp, str):
            import codex_crew.config.loader as _mod
...
class TestConfigSchemaProperties:
    ...
    def test_all_fields_carry_required_metadata(self) -> None:
        ...
        **Validates: Requirements 1.1**
        """
        all_fields = _all_fields_recursive(CodexCrewConfig)
        ...
        **Validates: Requirements 3.2, 2.6**
        ...
        for entry in SCHEMA_REGISTRY:
            ...
            assert entry.path in reachable_paths, (
                ...
                f"dataclasses.fields() recursion on CodexCrewConfig"
            )
...
class TestDeclaredDictProperties:
    ...
    def test_declared_key_type_violation_keeps_value_and_dict(self) -> None:
        ...
        from codex_crew.config.validation import validate_config_data
