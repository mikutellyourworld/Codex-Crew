...
from codex_crew.slack.blocks import build_working_blocks, session_task_card
...
class TestSessionTaskCard:
    def test_end_button_present(self):
        blocks = session_task_card(
            idx=0, key="sess-1", title="Test", agent="codexcrew",
...
def setup_interactions(mock_orch, monkeypatch):
    ...
    import codex_crew.slack.interactions as interactions
...
class TestHandleInlineStop:
    ...
    async def test_denied_for_non_owner(self, setup_interactions, mock_orch):
        ...
        with patch("codex_crew.slack.interactions.sel") as mock_sel:
