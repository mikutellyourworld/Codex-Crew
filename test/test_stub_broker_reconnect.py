...
def _clean_env() -> dict[str, str]:
    ...
    return {k: v for k, v in os.environ.items() if k != "CODEXCREW_CHANNEL_ID"}
...
async def _spawn_stub(
    *,
    ...
    env = {**_clean_env(), "CODEXCREW_HOME": str(home)}
    env["CODEXCREW_SESSION_KEY"] = session_key
    return await asyncio.create_subprocess_exec(
        ...
        cwd=str(work_dir),
        # CODEXCREW_HOME redirects the stub's fallback audit log into the test's
...
def _session_with_captured_init(init_result: dict | None) -> object:
    from codex_crew.mcp_gateway.stub import StubSession
...
async def test_replay_accepts_a_generation_that_answers_identically() -> None:
    ...
    from codex_crew.mcp_gateway.stub import _REPLAY_OK, _replay_initialize
...
async def test_replay_refuses_a_generation_that_answers_differently() -> None:
    ...
    """
    from codex_crew.mcp_gateway.stub import _REPLAY_REFUSE, _replay_initialize
...
async def test_a_connection_lost_during_replay_is_retryable_not_terminal() -> None:
    ...
    """
    from codex_crew.mcp_gateway.stub import _REPLAY_RETRY, _replay_initialize
...
async def test_reconnect_retries_past_a_daemon_that_dies_mid_replay(
    ...
    """
    from codex_crew.mcp_gateway import stub as stub_mod
...
async def test_a_closed_stdin_is_not_a_reconnectable_ending() -> None:
    ...
    """
    from codex_crew.mcp_gateway.stub import StubSession, run_bridge
...
def test_abandoned_calls_are_failed_once_when_no_replay_can_answer_them() -> None:
    from codex_crew.mcp_gateway.stub import StubSession, _split_abandoned_ids
...
def test_an_unanswered_initialize_is_deferred_to_the_replay() -> None:
    ...
    from codex_crew.mcp_gateway.stub import _split_abandoned_ids
