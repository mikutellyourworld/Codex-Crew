...
from codex_crew.dashboard.handlers import whatsapp_setup as mod
...
def cfg_file(tmp_path: Path, monkeypatch) -> Path:
    ...
    ``CodexCrewConfig.load`` resolves the same binding.
    """
...
def test_config_save_persists_policy_and_preserves_other_sections(
    ...
    cfg_file.write_text(
        json.dumps({"slack": {"command": "codexcrew"}, "agent": {"model": "auto"}}),
        encoding="utf-8",
    )
    status, body = _call(
        _serve(_FakeState()),
    ...
    stored = json.loads(cfg_file.read_text(encoding="utf-8"))
    assert stored["slack"] == {"command": "codexcrew"}
