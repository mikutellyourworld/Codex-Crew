...
from codex_crew.messaging import TransportCapabilities, TurnDriver
from codex_crew.messaging.dispatch import _ChannelDirectiveState, build_directive_consumer
...
def no_dashboard_tabs():
    ...
    from codex_crew import session_surface
...
class TestChannelApplierBoundary:
    ...
    async def test_monitor_start_not_supported_on_non_nudgeable_channel(
        ...
        result = await apply_session_directive(
            state, None, "telegram:codexcrew:direct:42", "monitor_start", dict(MONITOR_ARGS)
        )
    ...
    async def test_not_supported_paths_audit_denied_never_success(
        ...
        monkeypatch.setattr("codex_crew.sel.sel", lambda: _SelSpy())
        monkeypatch.setattr("codex_crew.autonudge.get_instance", lambda: _ArmSvc())
        result = await apply_session_directive(
            _ChannelDirectiveState(sessions=_ChannelSessions("x")),
            None,
            "telegram:codexcrew:direct:42",
            kind,
...
class TestBuildDirectiveConsumer:
    ...
    async def test_prefers_dispatcher_dashboard_state(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.session_directive_apply.apply_session_directive", _spy
        )
        ...
        consume = build_directive_consumer(
            session_key="discord:codexcrew:direct:42",
            sessions=object(),
        ...
        assert slot is None
        assert session_key == "discord:codexcrew:direct:42"
