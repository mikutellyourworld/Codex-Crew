...
from codex_crew.config.schema import (
    SCHEMA_REGISTRY,
...
def _make_app() -> web.Application:
    ...
    from codex_crew.dashboard.handlers import api_config_schema
...
# ---------------------------------------------------------------------------
# CodexCrew Agent CRUD API tests (Tasks 5.3 + 5.4)
# ---------------------------------------------------------------------------


def _make_crud_app() -> web.Application:
    """Minimal aiohttp app with CodexCrew Agent CRUD endpoints."""
...
class TestAgentCrudProperties:
    """Property-based tests for CodexCrew Agent CRUD round-trips."""
    ...
    @given(
        ...
        kiro_agent=st.sampled_from(["codexcrew", "oncall", "research", "coding"]),
    ...
    async def test_crud_update_round_trip(self, data: st.DataObject) -> None:
        ...
        new_kiro = data.draw(st.sampled_from(["codexcrew", "oncall", "research"]))
            ...
            with unittest.mock.patch("codex_crew.config.loader.config_path", return_value=tmp):
                async with TestClient(TestServer(_make_crud_app())) as client:
                    ...
                    if update_kiro:
                        ...
                        assert agent["kiro_agent"] == "codexcrew"
                    if update_ws:
...
class TestAgentCrudEdgeCases:
    """Unit tests for CodexCrew Agent CRUD error handling."""
        ...
        try:
            with unittest.mock.patch("codex_crew.config.loader.config_path", return_value=tmp):
                async with TestClient(TestServer(_make_crud_app())) as client:
                    resp = await client.post(
                        "/api/agents",
                        json={"name": "default", "kiro_agent": "codexcrew"},
                    )
        ...
        try:
            with unittest.mock.patch("codex_crew.config.loader.config_path", return_value=tmp):
                async with TestClient(TestServer(_make_crud_app())) as client:
                    resp = await client.post(
                        "/api/agents",
                        json={"name": "", "kiro_agent": "codexcrew"},
                    )
        ...
        try:
            with unittest.mock.patch("codex_crew.config.loader.config_path", return_value=tmp):
                with unittest.mock.patch(
                    "codex_crew.dashboard.handlers.agents.CodexCrewConfig.load",
