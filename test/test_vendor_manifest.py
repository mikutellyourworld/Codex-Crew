...
Everything under ``src/codex_crew/_vendor`` is excluded from source-level
...
class TestRoundTrip:
    ...
    def test_manifest_is_deterministic_and_sha256sum_compatible(self, tmp_path: Path) -> None:
        ...
        ``<64-hex>␣␣src/codex_crew/_vendor/<relpath>`` with a trailing newline —
        ...
        for line in lines:
            ...
            assert name.startswith("src/codex_crew/_vendor/")
...
class TestDivergenceDetection:
    ...
    def test_detects_a_modified_file(self, green_baseline, capsys) -> None:
        ...
        assert "MODIFIED: src/codex_crew/_vendor/llama_cpp_libs/linux_x86_64/libllama.so" in err
    ...
    def test_detects_a_missing_file(self, green_baseline, capsys) -> None:
        ...
        assert _run(_args(vendor, manifest)) == 1
        assert "MISSING: src/codex_crew/_vendor/llama_cpp/llama.py" in capsys.readouterr().err
    ...
    def test_detects_an_unexpected_extra_file(self, green_baseline, capsys) -> None:
        ...
        assert _run(_args(vendor, manifest)) == 1
        err = capsys.readouterr().err
        assert "UNEXPECTED: src/codex_crew/_vendor/llama_cpp/sitecustomize.py" in err
    ...
    def test_reports_every_class_in_one_run(self, green_baseline, capsys) -> None:
        ...
        assert _run(_args(vendor, manifest)) == 1
        err = capsys.readouterr().err
        assert "MODIFIED: src/codex_crew/_vendor/README.md" in err
        assert "MISSING: src/codex_crew/_vendor/llama_cpp/llama.py" in err
        assert "UNEXPECTED: src/codex_crew/_vendor/extra.bin" in err
    ...
    def test_a_symlink_is_refused_not_skipped(self, green_baseline, capsys) -> None:
        ...
        assert _run(_args(vendor, manifest)) == 1
        err = capsys.readouterr().err
        assert "SYMLINK: src/codex_crew/_vendor/llama_cpp_libs/linux_x86_64/evil.so" in err
    ...
    def test_pycache_is_refused_never_silently_skipped(self, green_baseline, capsys) -> None:
        ...
        assert _run(_args(vendor, manifest)) == 1
        err = capsys.readouterr().err
        assert "PYCACHE: src/codex_crew/_vendor/llama_cpp/__pycache__/llama.cpython-312.pyc" in err
...
@pytest.mark.parametrize(
    "pattern",
    (
        "src/codex_crew/_vendor/**/*.py text eol=lf",
        "src/codex_crew/_vendor/**/*.md text eol=lf",
        "src/codex_crew/_vendor/**/*.typed text eol=lf",
    ),
)
