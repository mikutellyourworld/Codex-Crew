"""Tests for :mod:`codex_crew.skill_trust` -- the project-skills consent record.
...
Isolation: ``CODEXCREW_HOME`` is pinned to this test's ``tmp_path`` (the same
...
from codex_crew import platform_compat, security, skill_trust
...
class TestOffSwitch:
    def test_the_off_switch_beats_a_recorded_grant(self, tmp_path: Path) -> None:
        ...
        disabled = CodexCrewConfig.load()
        object.__setattr__(disabled.skills, "project_skills_enabled", False)
        with patch("codex_crew.config.loader.CodexCrewConfig.load", return_value=disabled):
            skill_trust.reset_cache_for_tests()
