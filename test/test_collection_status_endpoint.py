...
``CODEXCREW_TELEMETRY`` overrides ``telemetry.enabled`` inside the collector, so a
...
def _make_app() -> web.Application:
    from codex_crew.dashboard.handlers import api_collection_status
...
class TestCollectionStatusEndpoint:
    ...
    async def test_env_var_on_beats_a_false_config(self, _home, monkeypatch) -> None:
        ...
        assert body["env_var"] == "CODEXCREW_TELEMETRY"
    ...
    async def test_unreadable_config_reports_off_rather_than_raising(self, _home) -> None:
        ...
        with patch(
            "codex_crew.config.loader.CodexCrewConfig.load", side_effect=OSError("boom")
        ):
            async with TestClient(TestServer(_make_app())) as c:
                body = await _get(c)
        assert body["enabled"] is False
