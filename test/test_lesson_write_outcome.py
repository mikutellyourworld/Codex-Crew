...
class TestCliLearnAddReportsTheOutcome:
    """``codexcrew learn add`` no longer writes a second record on a decline.
    ...
    def _run(self, tmp_path, negative=None, rule="prefer ruff over flake8", pre=None):
        ...
        with (
            ...
            patch.object(cli_commands.CodexCrewConfig, "load", return_value=MagicMock()),
        ):
            try:
    ...
    def test_no_dedup_log_line_carries_lesson_text(self, tmp_path, caplog):
        ...
        for label, (stored, submitted) in cases.items():
            ...
            try:
                ...
                caplog.clear()
                with caplog.at_level(logging.DEBUG, logger="codex_crew.vector_memory"):
