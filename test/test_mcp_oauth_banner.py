...
from codex_crew.connections import get_all_registry_providers, get_visible_providers
...
class TestIsSafeOAuthUrl:
    ...
    def test_http_allowed(self):
        assert _is_safe_oauth_url("http://localhost:5486/callback")
...
class TestOAuthParamCredentialScan:
    ...
    def test_superhuman_endpoint_is_in_builtin_allowlist(self):
        ...
        from codex_crew.security import _OAUTH_AUTHORIZATION_ENDPOINTS
...
def _own(tmp_path, monkeypatch, servers) -> None:
    """Point discovery's codexcrew scope at a temp store holding ``servers``.

    Patches the real read path rather than stubbing ``codexcrew_managed_names``, so
    the store's own parsing (and its fail-open branches) is what the annotation is
    tested against. An unrecognized path buckets as ``SCOPE_CODEXCREW``, which is
...
class TestAnnotationFailsOpen:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("broken", ["codexcrew_managed_names", "get_visible_providers"])
    async def test_either_lookup_raising_fails_open(self, tmp_path, monkeypatch, broken):
        ...
        slot = _ChatSlot("s1")
        with patch(
            f"codex_crew.dashboard.chat_runner.{broken}", side_effect=RuntimeError("boom")
        ):
            await _drain_session_init_oauth_requests(MagicMock(), slot, _pending(CARDED_SLUG))
        assert _owned_flags(slot) == [False]
...
class TestConnectionsManagedMcpNames:
    ...
    def test_ownership_discriminator_is_not_reimplemented(self, tmp_path, monkeypatch):
        ...
        assert CARDED_SLUG not in mcp_discovery.codexcrew_managed_names()
