...
def test_shutdown_event_importable_without_running_loop() -> None:
    ...
    assert hasattr(codex_crew, "shutdown_event")
    assert callable(codex_crew.shutdown_event.set)
    assert callable(codex_crew.shutdown_event.clear)
    assert callable(codex_crew.shutdown_event.is_set)
...
def test_shutdown_event_survives_fresh_asyncio_run() -> None:
    ...
    async def main() -> None:
        ...
        await codex_crew.shutdown_event.wait()

    asyncio.run(main())
    assert codex_crew.shutdown_event.is_set()
    codex_crew.shutdown_event.clear()
...
def test_shutdown_event_wait_for_timeout() -> None:
    ...
    async def main() -> str:
        try:
            await asyncio.wait_for(codex_crew.shutdown_event.wait(), timeout=0.05)
...
def test_shutdown_event_does_not_bind_to_default_loop_via_get_event_loop() -> None:
    ...
    codex_crew.shutdown_event.clear()
    codex_crew.shutdown_event._event = None  # type: ignore[attr-defined]
    codex_crew.shutdown_event._loop = None  # type: ignore[attr-defined]
...
def test_shutdown_event_pending_set_preserved_across_loops() -> None:
    ...
    async def main() -> bool:
        return codex_crew.shutdown_event.is_set()
...
def test_shutdown_event_get_raises_without_loop() -> None:
    ...
    codex_crew.shutdown_event._loop = None  # type: ignore[attr-defined]
    try:
        codex_crew.shutdown_event._get()  # type: ignore[attr-defined]
