...
from codex_crew.deploy.iam import boundary_policy_document, policy_document

_TEMPLATES = (
    Path(__file__).resolve().parents[1]
    / "src/codex_crew/deploy/skills/artifact-deploy/templates"
)
...
def test_fullstack_policy_grants_scoped_log_group_perms():
    ...
    assert all("/codexcrew-deploy-app/" in r for r in res), res
