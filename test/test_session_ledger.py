...
from codex_crew import session_ledger as sl
...
def test_ledger_root_is_behind_the_agent_file_gate():
    ...
    home = Path.home()
    assert is_sensitive_path(str(home / ".codex-crew/ledger/chat-1-abc12345/state.json"))
    assert is_sensitive_path(str(home / ".codexcrew/ledger/x-deadbeef/state.json"))
...
class TestBoundedWriteIsLoudAboutLoss:
    ...
    def test_a_record_that_fits_logs_nothing(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        with caplog.at_level(logging.WARNING, logger="codex_crew.session_ledger"):
...
async def test_compose_nudge_body_prefixes_snapshot():
    from codex_crew.dashboard.handlers.autonudge import compose_nudge_body
...
def _open_route(monkeypatch):
    ...
    from codex_crew.dashboard.handlers import session_ledger as routes
...
async def test_remove_slot_for_history_key_purges_ledger():
    from codex_crew.dashboard.handlers.sessions import _remove_slot_for_history_key
...
def test_mcp_tools_refuse_without_strict_identity(monkeypatch):
    ...
    from codex_crew.mcp_tools import ledger as tools
