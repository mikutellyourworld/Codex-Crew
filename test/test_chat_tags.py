...
from codex_crew.dashboard import chat_tags as chat_tags_module
from codex_crew.dashboard.chat_tags import _normalize_column, _valid_color
...
class TestTagVocabulary:
    ...
    async def test_delete_tag_persists_vocab_before_slots(self, tmp_path, monkeypatch):
        ...
        original_write = __import__(
            "codex_crew.dashboard.chat_tags", fromlist=["_write_tags_snapshot"]
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            with patch("codex_crew.dashboard.chat_tags.save_slot_off_loop", _tracking_save):
                with patch("codex_crew.dashboard.chat_tags._write_tags_snapshot", _tracking_write):
    ...
    async def test_delete_tag_strips_from_slots_and_columns(self, tmp_path, monkeypatch):
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            with patch("codex_crew.dashboard.chat_tags.save_slot_off_loop"):
                resp = await client.delete(f"/api/chat/tags/{tag['id']}")
            assert resp.status == 200
    ...
    async def test_delete_strips_all_slots_despite_partial_save_failure(
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            with patch("codex_crew.dashboard.chat_tags.save_slot_off_loop", _partial_failing_save):
                resp = await client.delete(f"/api/chat/tags/{tag['id']}")
            assert resp.status == 200
...
class TestUpdateDeleteRace:
    ...
    async def test_concurrent_delete_causes_update_to_404(self, tmp_path, monkeypatch):
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            with patch(
                "codex_crew.dashboard.chat_tags._write_tags_snapshot",
