...
_SITE_TAGS = [
    {"ResourceARN": "arn:aws:s3:::codexcrew-web-aaaa1111",
     "Tags": [{"Key": "codexcrew:managed", "Value": "true"}, {"Key": "codexcrew:site", "Value": "cr-dash"}]},
...
def test_recall_empties_bucket_and_invalidates(monkeypatch):
    ...
    assert result["bucket"] == "codexcrew-web-aaaa1111"
...
def test_list_sites_groups_by_tag_with_live_status(monkeypatch):
    def run(args, profile, timeout=30):  # noqa: ANN001
        if args[0] == "resourcegroupstaggingapi":
            return _tag_resp(_SITE_TAGS + [
                {"ResourceARN": "arn:aws:s3:::codexcrew-web-bbbb2222",
                 "Tags": [{"Key": "codexcrew:managed", "Value": "true"},
                          {"Key": "codexcrew:site", "Value": "blog"}]},
            ])
        if args[:2] == ["cloudfront", "get-distribution"]:
    ...
    assert by_id["cr-dash"]["bucket"] == "codexcrew-web-aaaa1111"
