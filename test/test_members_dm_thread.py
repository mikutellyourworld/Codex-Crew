...
* ``dm.json`` binding read/write in the existing per-member space
  (``$CODEXCREW_HOME/members/<slug>/``, isolated per test by the autouse
  ``_isolate_codexcrew_home`` fixture).
...
from codex_crew.dashboard.chat_handlers import _history_key_for
...
def _fake_config(names, default=CREW):
    return SimpleNamespace(
        agents={name: CodexCrewAgentConfig(kiro_agent=name) for name in names},
...
def _make_members_app(state) -> web.Application:
    from codex_crew.dashboard.handlers.members import (
        api_member_activity,
        api_member_thread,
        api_members,
    )
...
def _patched_config(names, default=CREW):
    return patch(
        "codex_crew.dashboard.handlers.members.CodexCrewConfig.load",
...
class TestResumeGuards:
    ...
    async def test_concurrent_member_resume_does_not_duplicate_history(self, tmp_path):
        ...
        with _patch(
            "codex_crew.members.read_dm_binding",
    ...
    async def test_resume_refuses_when_metadata_changes_across_the_binding_read(
        ...
        import codex_crew.members as members_real
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_handlers.members_mod.read_dm_binding", _racing_read
        )
        async with TestClient(TestServer(_make_app(state))) as client:
            resp = await client.post(
                f"/api/chat/slots/{member_slot_key(CREW)}/resume", json={"key": key}
...
class TestOpenAiCompatPin:
    ...
    async def test_completions_refuses_member_agent_mismatch(self):
        ...
        from codex_crew.dashboard.openai_compat import api_completions
...
class TestRegistryDriftWithoutLiveSlot:
    ...
    async def test_drifted_binding_fails_closed_even_with_no_live_slot(self, tmp_path):
        ...
        with patch(
            "codex_crew.dashboard.handlers.members._member_names_for_slug",
            return_value=[CREW],
        ):
            with _patched_config([CREW]):
                async with TestClient(TestServer(_make_members_app(state))) as client:
...
class TestPersistenceRestoreGate:
    def test_member_identity_resolves_from_the_binding(self):
        from codex_crew.dashboard.chat_persistence import _member_restore_identity
    ...
    def test_open_slot_restore_round_trips_a_bound_member_thread(self, tmp_path):
        ...
        """
        from codex_crew.dashboard.chat_persistence import _rehydrate_slot_from_history
...
class TestMemberActivityRoute:
    ...
    async def test_unreadable_timestamps_are_skipped_not_sorted_as_garbage(self, tmp_path):
        ...
        state = _make_state(tmp_path)
        from codex_crew.members import ACTIVITY_FILE_NAME, member_dir, record_activity
    ...
    async def test_display_cap_reports_capped_and_keeps_newest(self, tmp_path):
        ...
        state = _make_state(tmp_path)
        from codex_crew.dashboard.handlers import members as handler_mod
        from codex_crew.members import record_activity
...
class TestDenialAuditOffload:
    ...
    async def test_app_denial_audit_runs_inline(self, tmp_path, monkeypatch):
        state = _make_state(tmp_path)
        record: dict = {}
        monkeypatch.setattr("codex_crew.dashboard.handlers.sel", lambda: self._recording_sel(record))
    ...
    def test_no_members_sel_audit_is_offloaded(self):
        ...
        from codex_crew.dashboard.handlers import members as members_mod_py
