"""Tests for codex_crew.portability — export/import zip feature."""
...
from codex_crew.jsonl_util import UnreadableRecord
from codex_crew.portability import (
    EXPORT_EXCLUDE,
    _is_excluded,
    apply_import_zip,
    create_export_zip,
...
@pytest.fixture
def fake_codexcrew_home(tmp_path):
    """Create a realistic ~/.codexcrew directory structure for testing."""
    mc = tmp_path / ".codexcrew"
    mc.mkdir()
...
@pytest.fixture
def patched_config_dir(fake_codexcrew_home):
    """Patch config_dir() to return our fake directory."""
    with patch("codex_crew.portability.config_dir", return_value=fake_codexcrew_home):
        with patch.dict(os.environ, {"CODEXCREW_HOME": str(fake_codexcrew_home)}):
            yield fake_codexcrew_home
...
class TestExport:
    ...
    def test_export_empty_codexcrew_dir(self, tmp_path):
        mc = tmp_path / "empty_mc"
        mc.mkdir()
        with patch("codex_crew.portability.config_dir", return_value=mc):
            with patch.dict(os.environ, {"CODEXCREW_HOME": str(mc)}):
                zip_bytes, manifest = create_export_zip()
        assert len(zip_bytes) > 0
...
class TestImportMerge:
    def _make_export(self, source_dir):
        ...
        with patch("codex_crew.portability.config_dir", return_value=source_dir):
            with patch.dict(os.environ, {"CODEXCREW_HOME": str(source_dir)}):
                zip_bytes, _ = create_export_zip()
        tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".zip")
        tmp.write(zip_bytes)
        tmp.close()
    ...
    def test_import_merge_into_empty(self, patched_config_dir, tmp_path):
        """Import into a fresh (empty) CodexCrew instance."""
        zip_path = self._make_export(patched_config_dir)
        try:
            # Target: empty directory
            target = tmp_path / "target_mc"
            target.mkdir()
            with patch("codex_crew.portability.config_dir", return_value=target):
                with patch.dict(os.environ, {"CODEXCREW_HOME": str(target)}):
...
def test_import_zip_bomb_member_cap(tmp_path, monkeypatch):
    ...
    import codex_crew.portability as port
...
def test_a_dropped_cron_command_is_audited(tmp_path):
    ...
    import codex_crew.mcp_cron as mcp_cron
...
def test_merge_crons_returns_the_outcome_on_every_path(tmp_path):
    ...
    """
    from codex_crew.snapshot import _merge_crons
...
async def test_import_handler_outcome_reflects_a_refused_merge(
    ...
    import codex_crew.dashboard.handlers.portability as ph
