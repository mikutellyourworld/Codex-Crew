...
* ``store.*`` directory helpers -- redirected into ``tmp_path`` so nothing is written
  to the operator's Codex Crew data home.
"""
...
from codex_crew.apps.builtins.auto_improvement.backend import profile_normalize as PN
from codex_crew.apps.builtins.auto_improvement.backend import store
from codex_crew.apps.builtins.auto_improvement.profiles.github_repo import profile as gh
...
def test_write_protected_targets_masks_existing_parent_dirs(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: (".codex-crew/config.json", ".absent/dir/file.json"),
    )
...
def test_profile_propose_never_fabricates_a_mechanical_edit(tmp_path, monkeypatch):
    prof = _profile(tmp_path, monkeypatch)
    from codex_crew.apps.builtins.auto_improvement.spine.contracts import Candidate
