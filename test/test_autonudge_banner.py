...
from codex_crew import autonudge_authz as authz
from codex_crew import session_directive
from codex_crew.autonudge import AutoNudgeService, NudgeLoop
from codex_crew.autonudge_authz import MAX_BANNER_CHARS
...
async def _fire_full(loop: NudgeLoop, *, ledger: str = "") -> tuple[str, str, dict]:
    ...
    with (
        ...
        patch("codex_crew.dashboard.chat._run_chat", new=run_chat),
    ):
...
class TestMonitorToolsCanSetTheBanner:
    ...
    async def test_the_applier_hands_the_banner_to_the_authorised_seam(self) -> None:
        ...
        """
        from codex_crew.dashboard import session_directive_apply as sda

        svc = MagicMock()
        authz = AsyncMock(return_value=(SimpleNamespace(id="loop-1"), None, 200))
        with (
            patch.object(sda, "_binding", return_value="chat-9-1"),
            patch("codex_crew.autonudge.get_instance", return_value=svc),
            patch("codex_crew.autonudge_authz.authorize_and_add_nudge", authz),
        ):
