"""Tests for the authenticated ``codexcrew pod api`` front door."""
...
def stub_gateway(monkeypatch: pytest.MonkeyPatch) -> _RecordingUnixServer:
    ...
    with contextlib.ExitStack() as stack:
        ...
        monkeypatch.setenv("CODEXCREW_POD_ROOT", str(root))
        cfg = PodConfig.load()
...
class TestPodUnprovenRemediation:
    ...
    def test_no_record_at_all_is_told_to_restart(self, cfg: PodConfig) -> None:
        ...
        assert "codexcrew pod down demo && codexcrew pod up demo" in remedy
    ...
    def test_a_record_with_no_start_identity_is_told_to_rebuild(self, cfg: PodConfig) -> None:
        ...
        assert "codexcrew pod provision demo" in remedy
    ...
    def test_the_mint_refusal_carries_the_matching_remedy(
        ...
        assert "codexcrew pod provision demo" in message
...
class TestPodApiUnixTransport:
    ...
    def test_an_absent_socket_refuses_instead_of_falling_back_to_tcp(
        ...
        assert "codexcrew pod status wt" in rendered
...
class TestPodApiAuditAndCli:
    ...
    def test_an_unrecognised_method_reaches_the_envelope_not_argparse_prose(
        ...
        """
        src = str(Path(codex_crew.__file__).resolve().parents[1])
        env = os.environ.copy()
        env.update(
            {
                "HOME": str(tmp_path / "os-home"),
                "USERPROFILE": str(tmp_path / "os-home"),
                "CODEXCREW_HOME": str(tmp_path / "crew-home"),
                "PYTHONPATH": src,
            }
        )
        result = subprocess.run(
            [sys.executable, "-m", "codex_crew", "pod", "api", "wt", "OPTIONS", "sessions"],
            capture_output=True,
            encoding="utf-8",
            env=env,
            cwd=str(tmp_path),
