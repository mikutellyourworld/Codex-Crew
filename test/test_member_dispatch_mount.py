...
- ``members.member_dispatch_session_server`` — the session-level ``mcpServers``
  element, carrying strict identity via ``CODEXCREW_SESSION_KEY`` in its env
...
import codex_crew.validation  # noqa: F401 - break the legacy import cycle first
from codex_crew.acp.client import AcpClient
from codex_crew.acp.kas_agents import to_client_custom_agent
...
class TestMemberDispatchSessionServer:
    ...
    def test_identity_env_carries_the_session_key(self):
        ...
        resolver reads ``CODEXCREW_SESSION_KEY``, and the session-level param is
        the one path the KAS projection's env stripping never touches."""
        entry = member_dispatch_session_server(MEMBER_KEY)
        assert entry is not None
        assert {"name": "CODEXCREW_SESSION_KEY", "value": MEMBER_KEY} in entry["env"]
    ...
    def test_env_carries_the_exported_bound_port(self, monkeypatch):
        ...
        monkeypatch.setenv("CODEXCREW_BOUND_PORT", "7779")
        entry = member_dispatch_session_server(MEMBER_KEY)
        assert entry is not None
        assert {"name": "CODEXCREW_BOUND_PORT", "value": "7779"} in entry["env"]
    ...
    def test_env_falls_back_to_the_serving_resolver(self, monkeypatch):
        ...
        from codex_crew import port_resolution
        ...
        entry = member_dispatch_session_server(MEMBER_KEY)
        assert entry is not None
        assert {"name": "CODEXCREW_BOUND_PORT", "value": "6123"} in entry["env"]

    def test_port_does_not_displace_the_identity_pair(self, monkeypatch):
        """Both env pairs, and no CODEXCREW_PORT: that name means "the port an
        ...
        assert "CODEXCREW_SESSION_KEY" in names
        assert "CODEXCREW_BOUND_PORT" in names
        assert "CODEXCREW_PORT" not in names

    def test_unresolvable_command_degrades_to_none(self, monkeypatch):
        import codex_crew.agent as agent_mod

        monkeypatch.setattr(agent_mod, "_codexcrew_mcp_invocation", lambda _sub: ("", []))
...
class TestKasMemberProjection:
    ...
    def test_member_grants_join_allowed_tools(self):
        ...
        from codex_crew.agent import _MEMBER_DASHBOARD_GRANTS
    ...
    def test_spec_is_not_mutated(self):
        ...
        assert spec["tools"] == ["@codexcrew-core"]
        assert spec["allowedTools"] == ["@codexcrew-core"]
...
def _base_servers() -> list[dict]:
    return [{"name": "codexcrew-core", "command": "x", "args": [], "env": [], "type": "stdio"}]
...
class TestClaudeMemberAppend:
    ...
    def test_member_session_gains_the_entry(self):
        ...
        assert {"name": "CODEXCREW_SESSION_KEY", "value": MEMBER_KEY} in out[-1]["env"]
...
class TestRuntimeMemberThreading:
    ...
    async def test_member_flag_is_forwarded(self, monkeypatch):
        ...
        await rt._kas_custom_agents("codexcrew", member_dispatch=True)
        assert seen == [True]
...
class TestMemberServerJoinsSubtraction:
    ...
    An agent spec that already declares codexcrew-dashboard (the opt-in
...
class TestSessionHistoryWriteProtected:
    ...
    def test_sessions_dir_stays_readable(self):
        ...
        from codex_crew.security import sensitive_home_dirs
