...
from codex_crew import autonudge as _an
from codex_crew import autonudge_authz as _autonudge_mod
...
from codex_crew.dashboard.handlers.autonudge import render_nudge_message
...
def _enable(monkeypatch):
    monkeypatch.setenv("CODEXCREW_AUTONUDGE", "1")
...
async def test_a_retarget_does_not_overwrite_a_future_version_monitor(tmp_path):
    ...
    """
    from codex_crew.monitoring.models import MONITOR_STATE_VERSION
...
def test_runtime_budget_exceeded_predicate():
    ...
    from codex_crew.autonudge import runtime_budget_exceeded
...
async def test_replace_stopped_preserves_a_quarantined_monitor_record(svc):
    ...
    from codex_crew.monitoring.models import MONITOR_STOP_INVALID_RECORD
...
async def test_disabled_when_flag_off(tmp_path, monkeypatch):
    monkeypatch.setenv("CODEXCREW_AUTONUDGE", "0")
    ...
    from codex_crew.autonudge import enabled
...
def test_is_channel_key():
    from codex_crew.autonudge import is_channel_key

    assert is_channel_key("slack:1700000000.123456")
    assert is_channel_key("discord:codexcrew:direct:42")
    assert is_channel_key("unified:codexcrew")
...
@pytest.mark.parametrize(
    "key",
    [
        "telegram:codexcrew:direct:4242",
        "webex:codexcrew:direct:user@example.com",
        "teams:codexcrew:direct:29:1abcdef",
        "weixin:codexcrew:direct:oUserOpenId",
        "imessage:codexcrew:direct:+15550100",
    ],
)
...
def test_wecom_is_classified_because_it_gained_a_proactive_send_path():
    ...
    """
    from codex_crew.autonudge import is_channel_key
    from codex_crew.wecom.transport import WECOM_CAPABILITIES

    assert WECOM_CAPABILITIES.supports_proactive_send is True
    assert is_channel_key("wecom:codexcrew:direct:oUserOpenId")
...
def test_channel_key_prefixes_mirror_the_shipped_namespaces():
    ...
    name ``codex_crew.messaging.link``, whose package ``__init__`` pulls the
    driver/renderer/transport layer and with it the ACP client, agent, hooks,
    artifacts, metrics and sqlite: 48 extra ``codex_crew`` modules to obtain one
    ...
    """
    from codex_crew.autonudge import _CHANNEL_KEY_PREFIXES
    from codex_crew.messaging.link import CHANNEL_SESSION_NAMESPACES
...
async def test_unrouted_channel_namespace_degrades_instead_of_raising(svc, monkeypatch):
    ...
    await svc.start()
    loop = await svc.add(
        slot_key="whatsapp:codexcrew:direct:15550100", message="watch", idle_secs=60
    )
    for _ in range(500):
        if removed.is_set() and loop.id not in svc._timers:
            break
...
async def test_cross_surface_ladder_still_refuses_unroutable_channels(tmp_path):
    ...
    from codex_crew.dashboard.chat_runner import _deliver_cross_surface_reply
    from codex_crew.messaging.link import ChannelLink
    from codex_crew.messaging.transport import TransportCapabilities
    ...
    await _deliver_cross_surface_reply(state, "whatsapp:codexcrew:direct:15550100", "cycle 1 output")
    ...
    await _deliver_cross_surface_reply(state, "telegram:codexcrew:direct:4242", "cycle 1 output")
...
def test_binding_key_for_does_not_widen_with_the_classifier():
    ...
    """
    from codex_crew.autonudge import binding_key_for, is_channel_key
    ...
    assert binding_key_for("discord:codexcrew:direct:42") == "discord:codexcrew:direct:42"
    assert (
        binding_key_for("webex:codexcrew:direct:user@example.com")
        == "webex:codexcrew:direct:user@example.com"
    )
...
class TestAutonudgeDisabledSettingLink:
    ...
    def _app(self, monkeypatch):
        ...
        from codex_crew.dashboard.handlers import autonudge as _handler
...
class TestAutonudgeUpdateChokepoint:
    ...
    async def test_unknown_loop_is_audited_as_denied(self, monkeypatch):
        ...
        from codex_crew import autonudge_authz as _authz
...
class TestSentinelPathRepair:
    ...
    ``~/.codexcrew`` → ``~/.codex-crew`` migration is re-armed on the next start
    ...
    def test_legacy_rooted_path_is_rehomed(self, tmp_path, monkeypatch):
        """A ~/.codexcrew-rooted sentinel is rewritten onto the current home."""
        home = tmp_path / "home"
        legacy = home / ".codexcrew"
    ...
    def test_legacy_home_as_current_home_is_noop(self, tmp_path, monkeypatch):
        """When the live home IS ~/.codexcrew (override / migration fallback),
    ...
    def test_nested_current_home_inside_legacy_is_not_rehomed(self, tmp_path, monkeypatch):
        """CODEXCREW_HOME may legally point INSIDE the legacy root.

        ``~/.codexcrew/dev`` is lexically under ``~/.codexcrew`` but is the live
        home, so its sentinel is already correct. Re-homing it would yield
        ``~/.codexcrew/dev/dev/workspace/...``, persist that over the correct
    ...
    def test_unnormalized_path_escaping_legacy_is_preserved(self, tmp_path, monkeypatch):
        """``~/.codexcrew/../workspace/STOP`` normalizes OUTSIDE the legacy root.
        ...
        original = str(home / ".codexcrew" / ".." / "workspace" / "STOP")
        ...
        assert ".codex-crew" not in repaired
    ...
    def test_live_legacy_rooted_workspace_is_not_rehomed(self, tmp_path, monkeypatch):
        ...
        path under ``~/.codexcrew``, and the legacy root can survive the migration
...
def test_a_webex_session_is_nudge_able():
    ...
    key = "webex:codexcrew:direct:kyle@example.com"
    assert is_channel_key(key)
...
def test_the_channels_without_a_fire_adapter_stay_excluded():
    ...
    for channel in ("wecom", "teams", "weixin", "imessage"):
        key = f"{channel}:codexcrew:direct:someone"
...
def test_a_unified_scope_key_is_never_bindable():
    ...
    assert is_channel_key("unified:codexcrew")
    assert binding_key_for("unified:codexcrew") is None
