"""Coverage tests for ``codex_crew.dashboard.handlers.themes`` — HTTP surface.
...
Every test points ``CODEXCREW_HOME`` at ``tmp_path`` so ``_themes_dir()``
...
import codex_crew.platform.governance_profiles as gov_mod
...
def themes_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect CODEXCREW_HOME into tmp_path and return the themes directory."""
...
class TestListThemesSync:
    def test_missing_directory_is_empty(
        ...
        monkeypatch.setenv("CODEXCREW_HOME", str(tmp_path / "nowhere"))
...
class TestDoInstallPromotion:
    def test_source_containing_the_themes_dir_is_rejected(self, themes_dir: Path) -> None:
        # The themes directory lives under CODEXCREW_HOME, so installing FROM
