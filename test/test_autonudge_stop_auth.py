...
from codex_crew import autonudge_authz, session_directive
...
from codex_crew.mcp_tools._limits import _MONITOR_DEFAULT_MAX_CYCLES
...
# from ``codex_crew.autonudge`` / ``codex_crew.autonudge_authz``, so they are
...
def _install_svc(monkeypatch, svc):
    monkeypatch.setattr("codex_crew.autonudge.get_instance", lambda: svc)
...
def _record_add(monkeypatch, *, loop=None, error=None):
    ...
    monkeypatch.setattr("codex_crew.autonudge_authz.authorize_and_add_nudge", _fake)
    return calls
...
def _record_update(monkeypatch, *, loop=None, error=None):
    ...
    monkeypatch.setattr("codex_crew.autonudge_authz.authorize_and_update_nudge", _fake)
    return calls
...
def test_a_probe_state_double_is_a_legacy_loop_only_while_it_carries_the_gate():
    ...
    """
    from codex_crew.autonudge import is_structured_monitor_loop
...
def test_applier_autonudge_stop_miss_logs_caller_binding_and_active_slot_keys(monkeypatch, caplog):
    ...
    svc = _FakeSvc(None, all_loops=loops)
    _install_svc(monkeypatch, svc)
    with caplog.at_level(logging.WARNING, logger="codex_crew.dashboard.session_directive_apply"):
        result = asyncio.run(
            apply_session_directive(
                _fake_state(), _fake_slot(), _SESSION, "autonudge_stop", {"reason": "done"}
            )
    ...
    warnings = [
        ...
        if r.levelno >= logging.WARNING and r.name == "codex_crew.dashboard.session_directive_apply"
    ]
...
def test_applier_autonudge_stop_logs_nothing_when_no_loop_exists(monkeypatch, caplog):
    ...
    assert "nothing to stop" in result.lower()
    assert [
        r for r in caplog.records if r.name == "codex_crew.dashboard.session_directive_apply"
    ] == []
