"""The one-way dependency of ``codex_crew.messaging``, enforced over every edge.
...
lived inside ``test_teams_transport.py`` and named ``codex_crew.teams`` only, so
...
bodies, and a guarded ``try: from codex_crew.dashboard import x`` is the same edge
...
import codex_crew.messaging as messaging_pkg
...
def _channel_packages() -> tuple[str, ...]:
    ...
    return tuple(f"codex_crew.{name}" for name in names)


#: Every package ``codex_crew.messaging`` may not depend on. The channels are
...
_FORBIDDEN = ("codex_crew.dashboard",) + _channel_packages()
...
_KNOWN_VIOLATIONS: dict[tuple[str, str], str] = {
    ("dispatch.py", "codex_crew.dashboard"): (
...
def _forbidden_prefix(module: str) -> str:
    ...
    Prefix-matched on a dotted boundary so a future ``codex_crew.slackbot`` is not
    mistaken for ``codex_crew.slack``.
    """
...
class TestMessagingImportPurity:
    def test_messaging_imports_no_channel_and_no_dashboard(self) -> None:
        ...
        assert not offenders, (
            "codex_crew.messaging must not import a channel package or dashboard; "
    ...
    def test_a_violation_outside_the_table_is_still_caught(self) -> None:
        ...
        assert _is_known("dispatch.py", "dispatch.py:1: from codex_crew.dashboard.x import y")
        assert not _is_known("dispatch.py", "dispatch.py:1: from codex_crew.slack.x import y")
        assert not _is_known("driver.py", "driver.py:1: from codex_crew.dashboard.x import y")
    ...
    def test_a_type_checking_only_import_is_still_refused(self) -> None:
        ...
        is `Any`-shaped. An actual ``from codex_crew.slack import ...`` inside the
        ...
        """
        src = "from typing import TYPE_CHECKING\nif TYPE_CHECKING:\n    from codex_crew.slack import x\n"
    ...
    def test_a_dynamic_import_does_not_escape_the_scan(self) -> None:
        ...
        src = 'import importlib\nm = importlib.import_module("codex_crew.dashboard.handlers")\n'
    ...
    def test_a_lookalike_package_is_not_a_false_positive(self) -> None:
        ...
        assert _forbidden_prefix("codex_crew.slackbot") == ""
        assert _forbidden_prefix("codex_crew.slack") == "codex_crew.slack"
        assert _forbidden_prefix("codex_crew.slack.handler") == "codex_crew.slack"
    ...
    def test_every_rostered_channel_is_covered(self) -> None:
        ...
        for descriptor in builtin_channel_descriptors():
            package = f"codex_crew.{descriptor.channel_type}"
