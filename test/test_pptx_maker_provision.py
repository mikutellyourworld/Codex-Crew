...
from codex_crew.apps.builtins.pptx_maker.backend import provision
...
class TestEnginePin:
    ...
    def test_the_pin_is_re_exported_from_the_verifying_module(self):
        ...
        from codex_crew.apps.builtins.pptx_maker.backend import engine_source
...
class TestRunSandboxing:
    ...
    def test_registration_is_skipped_when_the_app_was_disabled(self, tmp_path: Path):
        ...
        log: list[str] = []
        with (
            mock.patch("codex_crew.apps.manager.is_app_enabled", return_value=False),
            mock.patch(
    ...
    def test_registration_runs_while_the_app_is_enabled(self, tmp_path: Path):
        ...
        with (
            ...
            mock.patch("codex_crew.apps.bridges.register_app", side_effect=_register),
        ):
...
class TestRenderAgents:
    ...
    def test_the_tool_is_installed_before_its_path_is_baked_in(self, tmp_path: Path):
        ...
        with (
            ...
            mock.patch(
                "codex_crew.apps.builtins.pptx_maker.backend.preview_tools.install_pdftoppm",
    ...
    def test_a_windows_style_path_still_renders_parseable_json(self, tmp_path: Path):
        ...
        """
        install_dir = tmp_path / "install"
        win_root = r"C:\Users\runneradmin\.codex-crew\apps\pptx-maker\data\vendor\sdpm"
        log: list[str] = []
        with (
...
class TestProvision:
    ...
    def test_the_resolved_uv_is_threaded_into_the_build(self):
        ...
        with (
            mock.patch.object(provision, "resolve_uv", return_value="/opt/codexcrew/uv"),
            ...
            provision.provision()
        assert fetch.call_args.args[2] == "/opt/codexcrew/uv"
        assert venv.call_args.args[2] == "/opt/codexcrew/uv"
    ...
    def test_a_successful_run_reports_the_resolved_tag(self):
        with (
            ...
            mock.patch("codex_crew.apps.manager.is_app_enabled", return_value=True),
            mock.patch("codex_crew.apps.bridges.register_app") as register,
            mock.patch(
                "codex_crew.apps.builtins.pptx_maker.backend.engine.scan_new_templates",
                return_value=["corp"],
            ),
        ):
    ...
    def test_a_registration_failure_is_reported_not_raised(self):
        ...
        with (
            mock.patch("codex_crew.apps.manager.is_app_enabled", return_value=True),
            mock.patch(
                "codex_crew.apps.bridges.register_app", side_effect=RuntimeError("manifest gone")
            ),
        ):
            provision._register_resources(log)
...
class TestShippedAgentsDoNotPreAuthorizeTools:
    ...
    ``codexcrew-computer`` server, which is deliberately in ``tools`` but NOT
    ...
    def test_every_at_server_grant_resolves_to_a_declared_server(self) -> None:
        ...
        - a HOST-MANAGED server — read from ``agent._MANAGED_MCP_SERVERS``
          ...
          only the bare ``@server`` form resolves — ``@codexcrew-core/tool``
