...
five of them were missed by the ``~/.codexcrew`` -> ``~/.codex-crew`` data-home
...
* ``packages/codexcrew-client-py`` -- the shipped client defaulted to the legacy
...
four shapes -- shell ``${CODEXCREW_HOME:-...}`` fallbacks, bare legacy literals on
non-comment shell lines, Python ``CODEXCREW_HOME`` defaults in shipped packages,
...
DEFAULTS_JSON = REPO_ROOT / "src" / "codex_crew" / "config" / "defaults.json"

# ``.codexcrew`` NOT followed by ``-`` or ``.`` -- so the top-level siblings that
# genuinely did not move (``.codexcrew-pods``, ``.codexcrew-dev``,
# ``.codexcrew.breadcrumb``, ``.codexcrew.archived``) are not false positives.
LEGACY_HOME = re.compile(r"\.codexcrew(?![-.])")

CURRENT_HOME = "/.codex-crew"
...
LEGACY_READER_SCRIPTS = frozenset(
    {
        ...
        "src/codex_crew/deploy/skills/artifact-deploy/scripts/_common.sh",
    }
)
...
def _shell_scripts() -> list[Path]:
    ...
    home (``.codexcrew-dev/``, which legitimately contains installed skill
...
def _shipped_python() -> list[Path]:
    ...
    """
    roots = [REPO_ROOT / "src" / "codex_crew", REPO_ROOT / "packages"]
    files: list[Path] = []
    for root in roots:
        if not root.is_dir():
            continue
...
def test_no_shell_home_default_points_at_the_legacy_home() -> None:
    """Every ``${CODEXCREW_HOME:-...}`` fallback must name the current home."""
    scripts = _shell_scripts()
    assert len(scripts) > 5, f"guard scanned too few scripts: {len(scripts)}"

    pattern = re.compile(r"\$\{CODEXCREW_HOME:-([^}]*)\}")
    offenders: list[str] = []
    for path in scripts:
    ...
    assert not offenders, (
        "A shell CODEXCREW_HOME fallback still names the pre-move home. Unset is "
...
def test_no_shipped_python_defaults_to_the_legacy_home() -> None:
    """A shipped ``CODEXCREW_HOME`` default must not resolve to the legacy home.
    ...
    pattern = re.compile(
        r"""(?:os\.environ\.get|os\.getenv)\(\s*["']CODEXCREW_HOME["']\s*,(?P<default>.*)$"""
    )
    offenders: list[str] = []
    ...
    assert not offenders, (
        "Shipped Python defaults CODEXCREW_HOME to the pre-move home. Unset is the "
        "normal case, so the default is what runs:\n  " + "\n  ".join(offenders)
    )


def test_no_python_expands_a_hardcoded_legacy_home() -> None:
    """No Python may build a path by expanding a literal ``~/.codexcrew``.

    Fourth shape, and the one the other three could not see: a hardcoded
    ``os.path.expanduser("~/.codexcrew/...")`` is not a shell script, need not be
    in a shipped package, and never mentions ``CODEXCREW_HOME`` -- which is how
    ...
    """
    pattern = re.compile(r"expanduser\(\s*[^)]*\.codexcrew(?![-.])")
    offenders: list[str] = []
