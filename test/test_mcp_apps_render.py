...
def test_find_marker_valid_id():
    sid = _hex()
    assert mcp_apps_render.find_marker(f"done [codexcrew-mcp-app:{sid}] ok") == sid
...
def test_find_marker_rejects_wrong_length():
    short = "a" * 31
    long = "a" * 33
    assert mcp_apps_render.find_marker(f"[codexcrew-mcp-app:{short}]") is None
    ...
    assert mcp_apps_render.find_marker(f"[codexcrew-mcp-app:{long}]") is None


def test_find_marker_rejects_uppercase():
    upper = "A" * 32
    assert mcp_apps_render.find_marker(f"[codexcrew-mcp-app:{upper}]") is None
    mixed = "abcdef0123456789ABCDEF0123456789"
    assert mcp_apps_render.find_marker(f"[codexcrew-mcp-app:{mixed}]") is None


def test_find_marker_rejects_non_hex():
    bad = "g" * 32
    assert mcp_apps_render.find_marker(f"[codexcrew-mcp-app:{bad}]") is None


def test_strip_marker_removes_all():
    sid1, sid2 = _hex(), _hex()
    text = f"a[codexcrew-mcp-app:{sid1}]b[codexcrew-mcp-app:{sid2}]c"
...
def spool(tmp_path, monkeypatch):
    d = tmp_path / "mcp-apps"
    d.mkdir()
    monkeypatch.setenv("CODEXCREW_MCP_APPS_SPOOL", str(d))
    return d
...
async def test_handle_tool_result_broadcasts_and_strips(spool):
    ...
    st = _FakeState()
    text = f"result [codexcrew-mcp-app:{sid}] tail"
    out = await mcp_apps_render.handle_tool_result(
...
async def test_handle_tool_result_marker_but_missing_spool_still_strips(spool):
    ...
    st = _FakeState()
    text = f"x [codexcrew-mcp-app:{sid}] y"
    out = await mcp_apps_render.handle_tool_result(
...
async def test_handle_tool_result_broadcast_exception_degrades_gracefully(spool):
    ...
    text = f"a [codexcrew-mcp-app:{sid}] b"
...
async def test_handle_tool_result_offloads_spool_read(spool, monkeypatch):
    ...
    out = await mcp_apps_render.handle_tool_result(
        ...
        text=f"pre [codexcrew-mcp-app:{sid}] post",
    )
    assert sid not in out
...
async def test_handle_tool_result_refuses_cross_session_marker(spool):
    ...
    st = _FakeState()
    out = await mcp_apps_render.handle_tool_result(
        st, slot_key="dashboard:B", tool_call_id="tc", text=f"[codexcrew-mcp-app:{sid}]"
    )
    assert st.calls == []
    assert sid not in out
...
async def test_handle_tool_result_renders_in_bound_session(spool):
    ...
    await mcp_apps_render.handle_tool_result(
        st, slot_key="dashboard:A", tool_call_id="tc", text=f"[codexcrew-mcp-app:{sid}]"
    )
    assert len(st.calls) == 1
...
def test_default_spool_dir_uses_config_dir(monkeypatch, tmp_path):
    monkeypatch.delenv("CODEXCREW_MCP_APPS_SPOOL", raising=False)
...
def test_env_override_spool_dir(monkeypatch, tmp_path):
    monkeypatch.setenv("CODEXCREW_MCP_APPS_SPOOL", str(tmp_path / "custom"))
...
def test_module_has_no_import_side_effect_on_env(monkeypatch):
    ...
    monkeypatch.setenv("CODEXCREW_MCP_APPS_SPOOL", "/tmp/a")
    assert mcp_apps_render._spool_dir() == Path("/tmp/a")
    monkeypatch.setenv("CODEXCREW_MCP_APPS_SPOOL", "/tmp/b")
...
async def test_handle_tool_result_redacts_credentials_in_leaves(spool):
    ...
    st = _FakeState()
    await mcp_apps_render.handle_tool_result(
        st, slot_key="dashboard:1", tool_call_id="tc", text=f"[codexcrew-mcp-app:{sid}]"
    )
    _, data = st.calls[0]
...
async def test_binding_uses_producing_session_key_not_slot(spool):
    ...
    st = _FakeState()
    await mcp_apps_render.handle_tool_result(
        st, slot_key="9", tool_call_id="tc", text=f"[codexcrew-mcp-app:{sid}]",
    ...
    st2 = _FakeState()
    await mcp_apps_render.handle_tool_result(
        st2, slot_key="9", tool_call_id="tc", text=f"[codexcrew-mcp-app:{sid2}]",
