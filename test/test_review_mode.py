...
def owner_patch():
    ...
    with patch("codex_crew.slack.interactions.is_owner", side_effect=_is_owner):
        yield
...
def sel_mock():
    ...
    with patch("codex_crew.slack.interactions.sel", return_value=mock_sel):
        yield mock_log
...
def auth_err_mock():
    ...
    mock = AsyncMock()
    with patch("codex_crew.slack.interactions._post_review_auth_error", mock):
        yield mock
...
class TestHandleReviewApprove:
    ...
    async def test_no_orch_returns(self) -> None:
        with patch("codex_crew.slack.interactions._orch", None):
...
class TestHandleReviewReviseSubmit:
    ...
    async def test_requester_submit_spawns_handle_message(
        ...
        with (
            patch(
                "codex_crew.slack.interactions.handle_message",
                new_callable=AsyncMock,
            ) as mock_hm,
            patch(
                "codex_crew.slack.interactions.asyncio.create_task",
