...
decisions on a host with no sandbox backend: allowing Codex Crew's OWN managed
...
from codex_crew.sandbox import SandboxUnavailableError, reset_backend, wrap_argv

_ARGV = ["/opt/codexcrew/bin/codexcrew", "mcp-core"]
...
def clean_state(monkeypatch):
    ...
    monkeypatch.delenv("CODEXCREW_ALLOW_UNSANDBOXED", raising=False)
    monkeypatch.setattr(
...
class TestCarveOutAllowedPath:
    ...
    def test_sel_unconfined_event_is_emitted(self, no_backend):
        ...
        with patch("codex_crew.sel.sel", return_value=sel_instance):
...
class TestCarveOutStillRaises:
    ...
    def test_governance_floor_raises_despite_flag(self, no_backend, monkeypatch):
        ...
        """
        monkeypatch.setattr(
            "codex_crew.platform.governance_profiles.governance_floor_ordinal",
