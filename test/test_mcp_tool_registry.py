...
A ``codexcrew-core`` tool is declared twice in the same domain module under
:mod:`codex_crew.mcp_tools` -- a descriptor in ``schemas()`` (what ``tools/list``
...
from codex_crew.mcp_tools import DOMAIN_MODULES, build_tool_list, dispatch


def _domain(name: str):
    return importlib.import_module(f"codex_crew.mcp_tools.{name}")
