...
The data home moved from a top-level ``~/.codexcrew`` to ``~/.codex-crew`` (nested
...
#   ~/.codexcrew-pods       -> CODEXCREW_POD_ROOT default (pod/config.py)
#   ~/.codexcrew-dev        -> per-worktree dev data dir (dev-backend.sh)
#   ~/.codexcrew.breadcrumb -> recovery pointer written beside the home
# The negative lookahead below is what distinguishes them from the old home.
LEGACY_HOME = re.compile(r"\.codexcrew(?![-.])")

CURRENT_HOME_POSIX = "~/.codex-crew"

# A skill may legitimately *mention* the legacy home when it is documenting the
# migration itself ("legacy installs auto-migrate from ~/.codexcrew on first
...
def _skill_files() -> list[Path]:
    ...
    roots = [REPO_ROOT / "skills", REPO_ROOT / "src" / "codex_crew" / "builtin_skills"]
    roots += sorted((REPO_ROOT / "src" / "codex_crew" / "apps" / "builtins").glob("*/skills"))
    files: list[Path] = []
    for root in roots:
        if not root.is_dir():
            continue
...
def test_no_skill_references_the_legacy_data_home() -> None:
    """No skill file may *instruct* against the pre-move ``~/.codexcrew`` home."""
    offenders: list[str] = []
    ...
    assert not offenders, (
        ...
        f"({CURRENT_HOME_POSIX}), not the legacy ~/.codexcrew. If a line is "
...
def test_sibling_paths_that_did_not_move_are_preserved() -> None:
    """The fix must not have rewritten ``.codexcrew-pods`` / ``.codexcrew-dev``.
    ...
    assert ".codexcrew-pods" in joined, "pod-root references should be unchanged"
    assert ".codex-crew-pods" not in joined, "pod root was wrongly rewritten"
    assert ".codex-crew-dev" not in joined, "worktree dev dir was wrongly rewritten"


def test_windows_examples_use_windows_separators() -> None:
    """A Windows path example must not end up with a mixed ``\\.codex-crew\\`` form."""
    offenders: list[str] = []
    for path in _skill_files():
        for lineno, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
            if "\\.codex-crew" in line:
...
def test_migration_exemption_is_narrow() -> None:
    ...
    assert not _is_migration_prose("SECRET=$(cat ~/.codexcrew/.local_secret)")
    assert LEGACY_HOME.search("SECRET=$(cat ~/.codexcrew/.local_secret)")
    # ...while prose about the move is allowed.
    assert _is_migration_prose("legacy installs auto-migrate from ~/.codexcrew")
...
AGENT_INSTRUCTION_FILES = (
    ...
    "src/codex_crew/config/prompt-orchestrator.md",
    "src/codex_crew/mcp_cron.py",
    "AUTOSDE.yaml",
...
def test_agent_instruction_surfaces_do_not_name_the_legacy_home() -> None:
    ...
    every agent that cron scripts "must live under ``~/.codexcrew/crons/``" while
...
def test_shipped_user_docs_do_not_name_the_legacy_home() -> None:
    ...
    ``src/codex_crew/docs/`` is packaged and surfaced in-product, so a stale path
    ...
    """
    root = REPO_ROOT / "src" / "codex_crew" / "docs"
    if not root.is_dir():
