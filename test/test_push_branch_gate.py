...
Covers the always-on Python gate in ``codex_crew.security`` and its kiro-cli
...
CodexCrew protects the git default branch names only (enumerated at line 9
...
class TestIsPushToProtectedBranch:
    ...
    def test_nondefault_integration_branches_not_protected(self) -> None:
        """CodexCrew protects only git defaults; other integration branch names
...
class TestDefaultsJsonPushRegexes:
    """Git-publish enforcement at the CodexCrew hooks gate (``is_denied``).
...
class TestTokenizerContractBetweenTheLayers:
    ...
    def test_the_argument_scan_is_handed_raw_words(self):
        ...
        from codex_crew.security import _git_push_args
    ...
    def test_a_quoted_separator_does_not_split_the_segment(self):
        ...
        from codex_crew.security import _split_push_command_segments
    ...
    def test_a_wrapped_payload_is_one_word_not_a_bare_git_token(self):
        ...
        from codex_crew.security import _git_push_args, _split_shell_words, is_denied
...
class TestNestedPayloadExtractionSpansTheProvenBoundary:
    ...
    def test_the_extracted_body_is_the_whole_body(self):
        from codex_crew.security import _substitution_bodies
    ...
    def test_the_span_helper_is_quote_aware_and_reports_proof(self):
        from codex_crew.security import _matching_close_paren
    ...
    def test_the_self_protection_substitution_scan_is_quote_aware_too(self):
        ...
        from codex_crew.security import _protected_name_in_substitution
        ...
        tokens = ["$(true", "X=')'", "codexcrew", "token)"]
        assert _protected_name_in_substitution(tokens, 0) == "codexcrew"
        # Still stops at the REAL closer rather than reading a later command.
        assert _protected_name_in_substitution(["$(true)", ";", "codexcrew"], 0) == ""
