"""Regression tests for Round-23 findings (CodexCrew PR #6).

F1: the fullstack IAM policy conditions iam:CreateRole on the
    codexcrew-deploy-app-boundary permissions boundary and denies boundary
...
_TPL = _ROOT / "src/codex_crew/deploy/skills/artifact-deploy/templates"
...
def test_f1_create_role_requires_boundary():
    ...
    for st in create_stmts:
        ...
        assert "codexcrew-deploy-app-boundary" in bound
...
def test_f1_boundary_policy_document_is_scoped():
    ...
    assert "codexcrew-deploy-app-*" in body
...
def test_f1_all_template_roles_carry_boundary():
    for name in ("app-apigw.yaml", "app-apigw-ddb.yaml", "app-lambda.yaml"):
        ...
        assert "codexcrew-deploy-app-boundary" in src
...
def test_f2_all_staging_rejections_convert_to_409():
    src = (_ROOT / "src/codex_crew/deploy/handlers.py").read_text(encoding="utf-8")
