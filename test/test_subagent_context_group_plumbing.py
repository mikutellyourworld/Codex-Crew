...
from codex_crew.subagent_manager.continuation import ContinuationCoordinator
from codex_crew.subagent_persistence import create_agent_folder, read_state
...
class TestContinuationInheritsScope:
    ...
    def test_falls_back_to_persisted_scope(self, monkeypatch):
        mgr = _mgr()
        monkeypatch.setattr(
            "codex_crew.subagent.read_state",
    ...
    def test_all_groups_withheld_is_not_confused_with_a_legacy_run(self, monkeypatch):
        ...
        mgr = _mgr()
        monkeypatch.setattr(
            "codex_crew.subagent.read_state", lambda _id: {"context_groups": ""}
        )
    ...
    def test_run_predating_the_field_defaults_to_all_on(self, monkeypatch):
        mgr = _mgr()
        monkeypatch.setattr("codex_crew.subagent.read_state", lambda _id: {"id": "old"})
    ...
    def test_continue_conversation_runs_where_the_run_ran(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "codex_crew.subagent.read_state", lambda _id: {"cwd": str(proj)}
        )
        captured.clear()
    ...
    def test_recorded_cwd_is_resolved_off_loop_not_inside_continue(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "codex_crew.subagent.read_state", lambda _id: {"cwd": str(gone)},
        )
        ...
        monkeypatch.setattr("codex_crew.subagent.read_state", lambda _id: {})
...
class TestScopePersistence:
    ...
    def test_folder_creation_records_the_scope(self, tmp_path, monkeypatch):
        monkeypatch.setattr("codex_crew.subagent_persistence._subagents_dir", lambda: tmp_path)
