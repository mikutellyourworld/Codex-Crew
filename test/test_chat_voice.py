...
def _make_voice_app(state):
    from codex_crew.dashboard.chat_voice import api_voice_config, api_voice_synthesize
...
class TestVoiceConfig:
    ...
    async def test_get_config(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_voice._vc", mock_vc)
    ...
    async def test_put_config_updates_voice(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_voice.config_path", lambda: cfg_path)
        state = _make_state(tmp_path)
        async with TestClient(TestServer(_make_voice_app(state))) as client:
    ...
    async def test_synthesize_routes_piper_through_nonstreaming(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_voice.synthesize_speech", _fake_synth)
        monkeypatch.setattr("codex_crew.dashboard.chat_voice.streaming_voice_reply", _fake_stream)
        state = _make_state(tmp_path)
...
class TestVoiceSynthesize:
    ...
    async def test_synthesize_success(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_voice.streaming_voice_reply", mock_stream)
        monkeypatch.setattr("codex_crew.dashboard.chat_voice.stitch_mp3s", AsyncMock(return_value=None))
    ...
    async def test_the_polly_chunks_are_written_and_read_off_the_event_loop(
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_voice.streaming_voice_reply", _mock_stream)
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_voice.stitch_mp3s", AsyncMock(return_value=str(final))
        )
    ...
    async def test_synthesize_exception_returns_500_and_broadcasts_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_voice.streaming_voice_reply", mock_stream_error)
...
class TestVoiceVoices:
    ...
    async def test_voices_returns_list(self, tmp_path, monkeypatch):
        ...
        # Reset cache
        monkeypatch.setattr("codex_crew.dashboard.chat_voice._voices_cache", None)
        monkeypatch.setattr("codex_crew.dashboard.chat_voice._voices_cache_ts", 0)
        ...
        monkeypatch.setattr(
            "codex_crew.dashboard.chat_voice.resolve_polly_cli", lambda: "/usr/local/bin/aws"
        )

        from codex_crew.dashboard.chat_voice import api_voice_voices
        app = web.Application()
    ...
    async def test_voices_uses_cache(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_voice._voices_cache", cached)
        monkeypatch.setattr("codex_crew.dashboard.chat_voice._voices_cache_ts", time.time())
    ...
    async def test_voices_aws_not_found(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("codex_crew.dashboard.chat_voice.resolve_polly_cli", lambda: None)
        spawn = AsyncMock()
        ...
        from codex_crew.dashboard import chat_voice
