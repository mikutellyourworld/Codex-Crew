"""Tests for codex_crew.wecom.transport (WeComTransport, Layer 1)."""
...
from codex_crew.wecom.transport import (
    WECOM_CAPABILITIES,
    WeComSendError,
    WeComTransport,
)
...
class TestAuthorize:
    ...
    def test_unknown_denied(self) -> None:
        ...
        with patch("codex_crew.wecom.transport.sel") as mock_sel:
            assert t.authorize(_msg("stranger")) is False
        mock_sel().log_api_access.assert_called_once()

    def test_empty_userid_denied(self) -> None:
        t = WeComTransport(FakeClient(), owner_id="Wei")
        with patch("codex_crew.wecom.transport.sel"):
