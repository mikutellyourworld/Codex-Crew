...
class TestMaxSubagentsPlaceholder:
    ...
    def _resolve_cap(prompt, session_key, *, cap=None, raises=False):
        ...
        if raises:
            sub = patch(
                "codex_crew.subagent.resolve_max_subagents",
                side_effect=RuntimeError("boom"),
            )
        else:
            sub = patch("codex_crew.subagent.resolve_max_subagents", return_value=cap)
        with patch("codex_crew.context.CodexCrewConfig.load", return_value=fake_cfg), sub:
            return ContextBuilder._resolve_prompt_templates(prompt, session_key)
    ...
    def test_absent_token_skips_resolver(self):
        ...
        fake_cfg = SimpleNamespace(dashboard=SimpleNamespace(widget_density="more"))
        with (
            patch("codex_crew.context.CodexCrewConfig.load", return_value=fake_cfg),
            patch("codex_crew.subagent.resolve_max_subagents") as resolver,
        ):
