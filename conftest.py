...
``src/codex_crew/apps/builtins`` (~108 test modules that ship inside the package,
...
* **The data home.** ``CODEXCREW_HOME`` is pinned per test, and the ``~/.kiro``
  ...
  under ``src/codex_crew/apps/builtins/*/tests/`` -- which see this conftest and no
  other -- write the operator's live ``~/.codex-crew`` the moment they touch
  ``config_dir()``.
...
* **The agent-spec home.** ``kiro_agents_dir()`` is a LAZY resolver, so neither of
  ...
  the machine-wide ``<kiro home>/agents/codexcrew.json`` -- the file that decides
...
be *cancelled* rewrote the operator's real ``codexcrew-gateway.service`` drop-in
...
``CODEXCREW_HOME`` or its own temp dir keeps winning.

Imports at MODULE level are stdlib + pytest only, on purpose: a rootdir conftest is
imported before every collection, so pulling ``codex_crew`` in here would make the
whole suite depend on import-time side effects of the package under test. The
fixtures that do need ``codex_crew`` import it in their own body, which runs at test
...
#:   ``systemd-run --user --scope --slice=codexcrew-agents.slice -p MemoryMax=…``
...
def _refuse(reason: str, argv: object) -> None:
    ...
    raise AssertionError(
        ...
        f"  - codex_crew.service.*: patch `service.<platform>.subprocess.run`\n"
...
def _isolate_xdg_config_home(_xdg_config_root, monkeypatch):
    ...
    ``$XDG_CONFIG_HOME/systemd/user/codexcrew-gateway.service.d/make-live.conf``,
...
def _isolate_launchd_paths(_xdg_config_root, monkeypatch):
    ...
    module globals bound at IMPORT time from ``Path.home()``::
        ...
        PLIST_PATH   = PLIST_DIR / "dev.codexcrew.gateway.plist"
    ...
    ``write_live_program(render_live_program(codexcrew_bin()))`` with no path
    ...
    eager: dict[str, dict[str, object]] = {
        ...
        "codex_crew.service.common": {"launchd_live_program": lambda: launcher},
    }
    lazy: dict[str, dict[str, object]] = {
        "codex_crew.apps.builtins.dev_fleet.gateway_service": {
            "PLIST_PATH": plist_path,
...
def _isolate_sandbox_mount_source_roots(_sandbox_mount_source_root, monkeypatch):
    ...
    """
    try:
        sandbox_mod = importlib.import_module("codex_crew.sandbox")
    except Exception:  # pragma: no cover - a partial checkout must not break collection
        return
    ...
    # matches names no Codex Crew build has created since #6268: an unpinned test
...
def _pin_kill_and_reap_group_probe(monkeypatch):
    ...
    """
    try:
        platform_compat = importlib.import_module("codex_crew.platform_compat")
    except Exception:  # pragma: no cover - a partial checkout must not break collection
        return
...
def _no_credential_env_residue():
    ...
    ``CodexCrewConfig.load_credentials()`` deliberately propagates every credential
    ...
    """
    from codex_crew.config.loader import _CREDENTIAL_KEYS, _JIRA_TOKEN_RE
...
def _block_host_service_mutation(request, monkeypatch):
    ...
    ``systemctl --user restart codexcrew-gateway.service``. Those restart the
...
def _no_leaked_telemetry_exporter():
    ...
    if not leaked:
        return
    provider = sys.modules.get("codex_crew.metrics.provider")
...
def _restore_log_queue_listener():
    ...
    its reasons. The ``_CliLogQueueHandler`` left on the ``codex_crew`` logger therefore
    ...
    """
    cli = sys.modules.get("codex_crew.cli")
...
def _restore_logger_levels():
    ...
    level left on ``codex_crew`` decides what every ``codex_crew.*`` logger in the worker
    ...
    drives the real ``cli.main()``, whose ``_setup_cli_logging`` pins ``codex_crew`` at
    ...
    **HANDLERS are deliberately not restored, and the boundary is not squeamishness.** A
    ...
    The handlers ``_setup_cli_logging`` leaves on ``codex_crew`` do accumulate -- each open
...
def pytest_runtest_setup(item):
    ...
    so the ~1490 tests under ``src/codex_crew/apps/builtins/*/tests/`` had no warm
...
def pytest_collection_modifyitems(config, items):
    ...
    ids under ``src/codex_crew/apps/builtins/auto_improvement/tests/``, and a hook rooted
...
def platform_compat_or_none():
    """``codex_crew.platform_compat``, or ``None`` when it cannot be imported.
...
#: ``CODEXCREW_TMP_PER_TEST=1`` and the residue's parent directory IS the test id.
_TMP_PER_TEST_ENV = "CODEXCREW_TMP_PER_TEST"
...
#: * ``codexcrew-computer-shots`` -- the computer-use screenshot spool, which production
...
_TMP_RESIDUE_STRICT_ENV = "CODEXCREW_TMP_RESIDUE_STRICT"
...
def _remove_tree(path: pathlib.Path) -> bool:
    ...
    """
    try:
        from codex_crew import platform_compat as _pc
    except ImportError:  # pragma: no cover - partial checkout
...
def _isolate_tempfile_base(tmp_path_factory):
    ...
    The report WARNS by default and fails only under ``CODEXCREW_TMP_RESIDUE_STRICT`` --
...
def _isolate_tempfile_base_per_test(_isolate_tempfile_base, request):
    ...
    Inert unless ``CODEXCREW_TMP_PER_TEST`` is set, so the steady-state cost is one
...
def _isolation_root(tmp_path_factory):
    ...
    everything a test writes under ``CODEXCREW_HOME`` nests inside here.
    """
...
@pytest.fixture(autouse=True)
def _isolate_codexcrew_home(_isolation_dirs, monkeypatch):
    """Pin ``CODEXCREW_HOME`` to a per-test tmp dir, for EVERY testpath.
    ...
    modules under ``src/codex_crew/apps/builtins/*/tests/`` ship inside the package and
    ...
    can run the one-time ``~/.codexcrew`` -> ``~/.codex-crew`` migration as a side
    ...
    A test that sets its own ``CODEXCREW_HOME`` still wins: ``monkeypatch.setenv``
    ...
    ``CODEXCREW_PROJECT_DIR`` is cleared for a different reason -- to match CI on a dev
    ...
    ``CODEXCREW_BOUND_PORT`` is cleared because ``_export_bound_port`` writes it into
    ...
    ``CODEXCREW_DEV_MODE`` / ``CODEXCREW_STRICT_ON_LOOP_PERSIST`` are cleared so a
    ...
    """
    monkeypatch.setenv("CODEXCREW_HOME", str(_isolation_dirs("codexcrew-home")))
    monkeypatch.delenv("CODEXCREW_PROJECT_DIR", raising=False)
    monkeypatch.delenv("CODEXCREW_BOUND_PORT", raising=False)
    monkeypatch.delenv("CODEXCREW_DEV_MODE", raising=False)
    monkeypatch.delenv("CODEXCREW_STRICT_ON_LOOP_PERSIST", raising=False)
    paths = sys.modules.get("codex_crew.config.paths")
...
def _isolate_sel_default_dir(tmp_path_factory):
    ...
    * Whichever test calls ``sel()`` FIRST fixes the directory for the whole
    ...
    the module resolves its default lazily so that importing ``codex_crew.sel`` never
    ...
    """
    try:
        from codex_crew import sel as _sel
    except ImportError:  # pragma: no cover - partial checkout
        yield
        return
...
#: ``~/.kiro`` paths production binds at IMPORT time, which ``CODEXCREW_HOME`` cannot
...
#: ``Path.home()`` bindings ``src/codex_crew`` actually has, so a new one cannot land
#: unpinned.
_SHARED_KIRO_PATHS: tuple[tuple[str, str, str], ...] = (
    ("codex_crew.agent", "_KIRO_MCP_JSON", ".kiro/settings/mcp.json"),
    ("codex_crew.agent", "_CC_MCP_JSON", ".claude.json"),
    ("codex_crew.agent", "_DEFAULT_KIRO_HOOKS_DIR", ".kiro/hooks"),
    ("codex_crew.learn", "_DEFAULT_DIR", ".codex-crew"),
    ("codex_crew.apps.bridges", "_LEGACY_SHARED_MCP_PATH", ".kiro/settings/mcp.json"),
    ("codex_crew.dashboard.handlers.mcp", "_GLOBAL_MCP_JSON", ".kiro/settings/mcp.json"),
    ...
    ("codex_crew.dashboard.handlers.mcp", "_MCP_LOCK_PATH", ".kiro/settings/mcp.lock"),
)
...
def _isolate_subagents_dir(_isolation_dirs, monkeypatch):
    ...
    ``codex_crew.subagent_persistence._SUBAGENTS_DIR`` is bound at import time to
    ``config_dir() / "subagents"``, so the ``CODEXCREW_HOME`` safety net above
    ...
    stub agent folders into the operator's real ``~/.codexcrew/subagents/``. On the
    ...
    """
    monkeypatch.setattr(
        "codex_crew.subagent_persistence._SUBAGENTS_DIR",
...
_AGENT_SPEC_HOOKS: tuple[tuple[str, str], ...] = (
    ("codex_crew.agent", "KIRO_AGENTS_DIR"),
    ("codex_crew.agent_discovery", "_KIRO_AGENTS_DIR"),
    ("codex_crew.apps.bridges", "KIRO_AGENTS_DIR"),
    ("codex_crew.cli_doctor", "KIRO_AGENTS_DIR"),
    ("codex_crew.doctor_deadpath", "KIRO_AGENTS_DIR"),
)
...
def _isolate_agent_spec_home(_agent_spec_seam_modules, _isolation_dirs, monkeypatch):
    ...
    ``CODEXCREW_HOME`` nor ``_SHARED_KIRO_PATHS`` reaches it -- the shared-path ratchet's
    ...
    operator's machine-wide ``<kiro home>/agents/codexcrew.json``. Confirmed live
    ...
    ``codexcrew-core`` from a deleted venv against a data home recreated empty ->
    ...
    ``_isolate_codexcrew_home`` for why pinning that variable is refused: ~35 tests
...
def _no_model_download(monkeypatch, _isolation_dirs):
    ...
    """
    monkeypatch.setenv("CODEXCREW_SKIP_MODEL_DOWNLOAD", "1")
    ...
    # `CodexCrewConfig.load` with a bare MagicMock, whose `telemetry.enabled` is TRUTHY,
    ...
    monkeypatch.setenv("CODEXCREW_TELEMETRY", "0")
...
def _no_central_policy_fetch(monkeypatch):
    ...
    ``src/codex_crew/apps/builtins/*/tests/`` never see that file, and several of them
    call ``boot_platform``.

    **The environment.** ``CODEXCREW_POLICY_URL`` and its siblings make
    ...
    **The thread.** ``policy_distribution`` keeps a module-global refresher whose
    ...
    for var in [k for k in os.environ if k.startswith("CODEXCREW_POLICY_")]:
    ...
    module = sys.modules.get("codex_crew.platform.policy_distribution")
...
def _isolate_agent_state_sidecar(_isolation_dirs, monkeypatch):
    ...
    ``codex_crew.agent_state`` stores per-agent bookkeeping (model_managed,
    cc_model) in ``~/.codexcrew/agent_model_state.json`` via ``config_dir()``.
    ...
    monkeypatch.setattr("codex_crew.agent_state.config_dir", lambda: sidecar_root)
