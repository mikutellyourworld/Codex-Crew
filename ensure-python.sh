...
# manager. The data home is "$CODEXCREW_HOME" when set, else
# "$HOME/.codex-crew" (the current default) — NOT the pre-move "$HOME/.codexcrew",
# which is not the data home and must not be written to.

MIN_MAJOR=3
MIN_MINOR=12
TARGET_PY="${CODEXCREW_PYTHON_VERSION:-3.12}"
...
_record() {
    home="${CODEXCREW_HOME:-$HOME/.codex-crew}"
    mkdir -p "$home"
