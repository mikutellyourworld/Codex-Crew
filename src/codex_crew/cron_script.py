...
Usage:
    ...
    def run(ctx):
        data = ctx.call_tool("codexcrew-core", "local_knowledge_search", {"query": "..."})
        if not ready(data):
...
from codex_crew.config.loader import config_dir, read_local_secret
from codex_crew.config.paths import kiro_agents_dir
from codex_crew.env import sanitize_spec_env
from codex_crew.github_runner import prevalidated_gh_env
from codex_crew.hooks import FileTooLargeError, safe_read_file_bytes_nolink
...
from codex_crew.security import is_sensitive_path, redact
...
# CODEXCREW_OWNER_ID; CODEXCREW_INTERNAL_SECRET is handed to scripts via a 0600
# temp file instead of the env (defense-in-depth item 4).
_CRON_ENV_DENY: frozenset[str] = frozenset({"CODEXCREW_INTERNAL_SECRET", *_AGENT_DENIED_ENV_KEYS})
...
#
# A grant maps an env-var NAME -> a vault secret NAME (codex_crew.secrets
...
_SECRET_ENV_DENIED_PREFIXES: tuple[str, ...] = (
    "CODEXCREW",  # product-internal, incl. _CODEXCREW_* dial/secret plumbing
    "_CODEXCREW",
...
def _read_script_body(file_path: str) -> bytes:
    ...
    Routes through :func:`codex_crew.hooks.safe_read_file_bytes_nolink`: the
...
#: epochs and removals also run from the CLI (``codexcrew cron remove``), so
...
def _filter_grant_env(resolved: dict[str, str]) -> dict[str, str]:
    ...
    grant can never name a product-internal key (``_CODEXCREW*``) or a loader
...
if TYPE_CHECKING:
    from codex_crew.cron import CronJob
...
class ScriptContext:
    ...
    def __post_init__(self) -> None:
        ...
        # keeps credential and dial target from one resolution; CODEXCREW_PORT is the
        # fallback for a directly-constructed context and is 5486 on a --port auto
        # gateway, which is a SIBLING rather than this instance.
        self._port = int(
            os.environ.pop("_CODEXCREW_DIAL_PORT", "") or os.environ.get("CODEXCREW_PORT", "5486")
        )
        ...
        secret_file = os.environ.pop("_CODEXCREW_SECRET_FILE", "")
        if secret_file and Path(secret_file).exists():
            ...
            self._secret = os.environ.pop("CODEXCREW_INTERNAL_SECRET", "")
...
class McpToolClient:
    ...
    def __init__(self, server_name: str, session_key: str = ""):
        ...
        #   * sanitize_spec_env() ALSO drops the reserved CODEXCREW_ namespace
        ...
        #     `codexcrew-cron` itself, whose ownership checks resolve the calling
        #     session from CODEXCREW_SESSION_KEY / CODEXCREW_HOST_PID when no
        ...
        if self._session_key:
            ...
            proc_env["CODEXCREW_SESSION_KEY"] = self._session_key
        ...
        try:
            self._rpc(
                "initialize",
                {
                    ...
                    "capabilities": {},
                    "clientInfo": {"name": "codexcrew-cron-script", "version": "0.1"},
                },
            )
...
def _resolve_mcp_server(name: str) -> tuple[tuple[str, ...], dict[str, str]] | None:
    ...
    """
    cfg_path = kiro_agents_dir() / "codexcrew.json"
    if not cfg_path.exists():
        # Fall back to any codexcrew-named agent spec in the same agents dir.
        for p in kiro_agents_dir().glob("*codexcrew*.json"):
            cfg_path = p
            break
    if not cfg_path.exists():
        return None
...
def _resolve_internal_secret(port: int) -> str:
    ...
    home-wide fallback); the ``CODEXCREW_INTERNAL_SECRET`` env var is normally unset,
    ...
    ``_CODEXCREW_DIAL_PORT``. Resolving twice -- once for the credential, once for the
    ...
    unrepresentable, which is what the ``_CODEXCREW_DIAL_PORT`` mechanism promised.
    """
    env_secret = os.environ.get("CODEXCREW_INTERNAL_SECRET", "")
    if env_secret:
        return env_secret
...
def _resolve_dial_port() -> int:
    ...
    ``CODEXCREW_PORT``, which is 5486 on a ``--port auto`` gateway, while the parent
    would have minted for the real ephemeral port -- credential for one gateway,
    request to another. One resolution, injected as ``_CODEXCREW_DIAL_PORT``, makes
    that mismatch unrepresentable.

    Delegates to :func:`resolve_serving_port`, the shared gateway-side resolver that
    prefers ``CODEXCREW_BOUND_PORT`` over an inherited ``CODEXCREW_PORT`` -- the cron
    scheduler runs inside the gateway, so the bound port is ground truth and a
    sibling-naming ``CODEXCREW_PORT`` must not win.
    """
...
def run_script_sandboxed(
    ...
    if secret_env:
        ...
        pinned_dir = tempfile.mkdtemp(prefix="codexcrew_cron_pin_")
    ...
    # Isolation also means ``codex_crew`` may no longer be importable via an
    # inherited PYTHONPATH (dev checkouts), so the TRUSTED package parent —
    # computed here in the gateway from codex_crew's own location, never from
    ...
    if stdin_payload is not None:
        ...
        prelude = (
            ...
            # codex_crew — otherwise a stray sibling like /tmp/struct.py or
    ...
    launcher = prelude + (
        ...
        "from codex_crew.platform.bootstrap import boot_platform\n"
        "boot_platform(CodexCrewConfig.load())\n"
        "from codex_crew.cron_script import ScriptContext, Skip, Done, Report\n"
        ...
        "if _payload:\n"
        "    import codex_crew.cron_script as _kcs\n"
    ...
    fd, launcher_path = tempfile.mkstemp(
        suffix=".py", prefix="codexcrew_cron_", dir=pinned_dir if stdin_payload else None
    )
    sandbox_cleanup: str | None = None
    # Resolve the dial port ONCE: the credential written below and the
    # _CODEXCREW_DIAL_PORT the child dials must come from the same resolution, or a
    ...
    secret_fd, secret_path = tempfile.mkstemp(prefix="codexcrew_secret_")
    try:
        try:
        ...
        # The launcher prelude seeds the trusted codex_crew package parent
        ...
        clean_env["_CODEXCREW_SECRET_FILE"] = secret_path
        ...
        clean_env["_CODEXCREW_DIAL_PORT"] = str(dial_port)
        # Give the child the SAME identity the gateway hands every agent
        # subprocess (acp/client.py injects CODEXCREW_SESSION_KEY for agent crons
        ...
        # the gateway-injected caller block, this env var, or CODEXCREW_HOST_PID
        ...
        # launcher's CODEXCREW_HOST_PID pointed at an unsigned pid and every write
        ...
        clean_env["CODEXCREW_SESSION_KEY"] = f"cron:{job_id}"
...
def run_command_sandboxed(
    ...
    # .netrc, .git-credentials, .npmrc, .pypirc, .codexcrew/.env) and scrubs the
