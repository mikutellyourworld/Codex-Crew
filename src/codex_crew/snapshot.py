"""CodexCrew snapshot and restore — portable state management."""
...
from codex_crew import pinned_fs, platform_compat
from codex_crew.jsonl_util import (
    RECORD_CAP,
    UndecodableRecord,
    UnreadableRecord,
    strict_raw_records,
...
if TYPE_CHECKING:
    from codex_crew import snapshot_redact
...
try:
    from codex_crew.config.loader import DASHBOARD_PORT as _DASHBOARD_PORT
except Exception:  # pragma: no cover - optional during early/standalone import
    _DASHBOARD_PORT = int(os.environ.get("CODEXCREW_PORT", 5486))
...
def _redactor() -> Any:
    ...
    actually prepares an outbound copy asks for it, so `codexcrew gateway` reaches
    readiness without loading it. A ratchet compares the boot module set against the base
    branch's.
    """
    return importlib.import_module("codex_crew.snapshot_redact")
...
def _default_snapshot_dir() -> str:
    ...
    try:
        ...
        d = CodexCrewConfig.load().snapshot_dir
        if d:
...
def _mc_dir() -> Path:
    ...
    # CODEXCREW_HOME override (and the same ~/.codex-crew default) as every other
    # module — not an undocumented CODEXCREW_DIR, which would make snapshots
...
def _redacted_upload_copy(
    ...
    try:
        ...
        raise RedactionFailed(
            ...
            "unaffected. Re-run `codexcrew snapshot` once the database is readable, or "
...
def _copytree_safe(
    ...
    **kwargs,
    ...
    :func:`codex_crew.pinned_fs.stage_tree_pinned`, including the chain above the root.
...
def _build_snapshot(
    ...
    *,
    ...
    and the caller turns a :class:`codex_crew.pinned_fs.PinnedPathRefusal` into an exit
    ...
    *name* names the TARBALL and *root_name* the directory inside it, and they are two
    ...
    glob ``codexcrew-snapshot-*.tar.gz`` -- so every partial bundle became invisible to
...
def snapshot_main(
    ...
    if parsed is None:
        p = argparse.ArgumentParser(
            prog="codexcrew-snapshot",
            description="Create a portable .tar.gz snapshot of Codex Crew state.",
        )
    ...
    if args.list_snapshots:
        if not out.is_dir():
            ...
            return 0
        snaps = sorted(
            out.glob("codexcrew-snapshot-*.tar.gz"), key=lambda x: x.stat().st_mtime, reverse=True
        )
        for s in snaps:
            print(s)
        if not snaps:
    ...
    # consumer, because the consumer has already shipped: a released `codexcrew restore`
    ...
    # `codexcrew-snapshot-`, and print "Invalid snapshot format" and exit 1 otherwise --
    ...
    # `codexcrew-snapshot-*.tar.gz`, and a partial bundle still needs to be found and
    # rotated by them. Only the directory inside it carries the marker.
    complete = set(selected) == set(COMPONENTS)
    name = f"codexcrew-snapshot-{ts}"
    root_name = name if complete else f"codexcrew-partial-{ts}"
    ...
    # `mode=ro`, so `codexcrew snapshot` cannot modify the data it was asked to copy.
    ...
    remaining = len(list(out.glob("codexcrew-snapshot-*.tar.gz")))
...
def _is_gateway_running() -> bool:
    """Check if the CodexCrew gateway is listening on its dashboard port."""
    # Deterministic override (used by tests / scripted restores) — avoids a real
    # socket probe whose result is environment-dependent.
    override = os.environ.get("CODEXCREW_ASSUME_GATEWAY_RUNNING")
...
def restore_main(argv: list[str] | None = None, *, parsed: argparse.Namespace | None = None) -> int:
    if parsed is None:
        p = argparse.ArgumentParser(
            prog="codexcrew-restore", description="Restore CodexCrew state from a snapshot."
        )
    ...
    if not force and _is_gateway_running():
        ...
        print("❌ Gateway is running. Stop it first (codexcrew stop) or use --force.")
        return 1
    ...
    with tempfile.TemporaryDirectory() as work_str:
        ...
        # Both roots: `codexcrew-snapshot-` for a complete bundle and
        # `codexcrew-partial-` for a selective one. The second name exists so that
        ...
        snap_dirs = [
            ...
            if d.is_dir()
            and (d.name.startswith("codexcrew-snapshot-") or d.name.startswith("codexcrew-partial-"))
        ]
        if not snap_dirs:
        ...
        if len(snap_dirs) > 1:
            ...
            return 1
        snap = snap_dirs[0]
        partial_root = snap.name.startswith("codexcrew-partial-")
    ...
    print("\n⚠️  Restart codexcrew gateway to pick up changes: codexcrew restart")
    return 0
