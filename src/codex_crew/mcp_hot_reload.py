...
* a ``@server`` ref ADDED to ``tools`` is honoured from the next turn, but a ref
  ...
  ref — see :func:`codex_crew.dashboard.handlers.mcp._sync_mcp_to_agent`.
"""
...
from codex_crew.acp_backends import ACP_BACKENDS_MCP_CONFIG_HOT_RELOAD
...
class DeclaresMcpHotReload(Protocol):
    ...
    (the agent-SDK import boundary keeps application code off ``codex_crew.acp``
    and ``codex_crew.providers``).
    """
