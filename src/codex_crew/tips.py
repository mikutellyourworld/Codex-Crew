...
from codex_crew.platform import PROFILE_STANDALONE, current_context, safe_context_call
...
from codex_crew.tips_text import first_prose_paragraph, strip_decoration, truncate_summary
...
# Doc selection is allowlist-based (see codex_crew.tips_allowlist) — internal
# architecture/incident docs must never surface as user-facing tips.
_MAX_GENERATED_TIPS = 8

_PROMPT_TEMPLATE = """\
You are generating personalized feature tips for a developer assistant called Codex Crew.
...
ACTION — every tip must be immediately usable. Many Codex Crew features are \
...
`codexcrew config set session.pool_size 2`), or a ready-to-send message
...
def _scan_docs_catalog() -> list[CatalogEntry]:
    """Scan codex_crew/docs/*.md for H1 + first paragraph."""
...
class TipsState:
    ...
    # (see codex_crew.tips_pool.TipsPool). A host that gains or loses an edition
...
def _state_path() -> Path:
    # Use the shared path helper: CODEXCREW_HOME tilde-expansion/resolution and
...
def _load_curated_tips() -> list[dict]:  # type: ignore[type-arg]
    ...
    These cover CodexCrew-native features (keyboard shortcuts, Settings toggles,
...
async def generate_tips(state: DashboardState) -> list[dict]:  # type: ignore[type-arg]
    ...
    cfg = await loop2.run_in_executor(None, CodexCrewConfig.load)
...
async def api_tips_next(request: web.Request) -> web.Response:
    ...
    loop = asyncio.get_running_loop()
    cfg = await loop.run_in_executor(None, CodexCrewConfig.load)
...
async def api_tips_status(request: web.Request) -> web.Response:
    ...
    - opted_out: per-instance opt-out flag (CodexCrew is a single-owner
