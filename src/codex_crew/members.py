"""Per-crew-member space: ``$CODEXCREW_HOME/members/<slug>/``.
...
:func:`~codex_crew.artifacts.slugify` so both surfaces normalize names the same
way.
...
from codex_crew.artifacts import slugify
from codex_crew.atomic_write import atomic_write, fsync_dir
...
MEMBER_DISPATCH_SERVER = "codexcrew-dashboard"
...
def member_dispatch_session_server(session_key: str) -> dict[str, object] | None:
    ...
    The entry carries ``CODEXCREW_SESSION_KEY`` so the server's strict identity
    ...
    The env also carries ``CODEXCREW_BOUND_PORT`` — the port this gateway is
    ...
    connection refused on every dispatch call. ``CODEXCREW_BOUND_PORT`` rather
    than ``CODEXCREW_PORT`` because the latter means "the port an operator
    ...
    try:
        command, args = _codexcrew_mcp_invocation("mcp-dashboard")
    ...
    if not command:
        return None
    env: list[dict[str, str]] = [{"name": "CODEXCREW_SESSION_KEY", "value": session_key}]
    # resolve_serving_port() reads CODEXCREW_BOUND_PORT first and only then falls
    ...
    # than forwarded.
    env.append({"name": "CODEXCREW_BOUND_PORT", "value": str(resolve_serving_port())})
    return {
...
def member_briefing_supported() -> bool:
    ...
    descriptor-relative pinned walk (:func:`~codex_crew.pinned_fs
...
def read_member_briefing(slug: str) -> str:
    ...
    * **No symlink following anywhere on the path, no non-regular files, no
      ...
      opened through :func:`~codex_crew.pinned_fs.open_in_pinned_parent`, which
...
def _read_activity_checked(slug: str, limit: int = 0) -> tuple[list[dict], bool]:
    ...
    :func:`codex_crew.jsonl_util.strict_records` stops the read instead of
