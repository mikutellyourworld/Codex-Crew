...
import codex_crew.validation as _validation
from codex_crew import security
from codex_crew import sel as _sel_mod
from codex_crew.executors import governance_executor
...
_REDACTION_SINKS: tuple[tuple[str, str, str], ...] = (
    (
        ...
        "cli_server.py",
        "The failure text `codexcrew update` prints when a managed-venv shadow "
        ...
        "CODEXCREW_CDN_BASE either would land credentials in terminal "
    ...
    (
        ...
        "surfaces: `codexcrew policy fetch` on stdout and a `logger` line (durable, "
    ...
    (
        ...
        "(`identityDetail` over `GET /api/aws/consent`) and `codexcrew aws-consent "
    ...
    (
        ...
        "into bug reports via `codexcrew logs`) and the Settings > Browser error card. "
    ...
    (
        ...
        "Transcript content copied to another Codex Crew instance over an Instances "
    ...
    (
        "Profile artifact",
        "perf_sampler.py",
        "Folded-stack profiles written by `codexcrew perf sample`. Frame labels are "
    ...
    (
        ...
        "Imported foreign-agent history and config before it enters Codex Crew.",
    ),
    (
    ...
    (
        ...
        "diagnostics.py",
        "The redacted zip built by `codexcrew doctor --bundle` and Settings › About › "
...
NON_EGRESS_REDACTION_MODULES: frozenset[str] = frozenset(
    {
        ...
        # The codexcrew-core tool handlers, moved out of mcp_core.py into their
        ...
        # mirrored/presigned CODEXCREW_PPTX_ENGINE_URL override cannot leak
        ...
        "builtin_skills/codexcrew-dev/prepare-pr/scripts/pr_findings.py",
...
def _own_namespace_prefixes() -> tuple[str, ...]:
    """Home-relative prefixes that belong to CodexCrew / kiro-cli itself.
...
def _sensitive_path_items() -> list[PostureItem]:
    ...
    for entry in security.sensitive_home_dirs():
        # Path-boundary match, so a sibling like `.codexcrew-notes` is not counted
        # as ours just because it shares a string prefix.
        first = entry.split("/", 1)[0]
        if first in own:
            detail = "Codex Crew trust root — the agent can neither read nor write it"
        else:
...
def _token_auth_items() -> list[PostureItem]:
    # circular import: token_auth lives under codex_crew.dashboard, whose package
...
_CONTROLS: tuple[PostureControl, ...] = (
    PostureControl(
        ...
        source="src/codex_crew/security.py",
    ...
    PostureControl(
        ...
        summary=(
            ...
            "are surfaced by the `codexcrew` history scan, NOT blocked at the "
    ...
    PostureControl(
        ...
        source="src/codex_crew/validation.py",
    ...
    PostureControl(
        ...
        source="src/codex_crew/dashboard/token_auth.py",
