...
2. **The session-map provider label** -- ``PROVIDER_LABEL_CLAUDE`` in
   :mod:`codex_crew.acp.types`. Same string, different job: it indexes resume
...
3. **A model-registry index key** -- inside :mod:`codex_crew.model_registry`,
...
4. **An onboarding import-source id** -- in :mod:`codex_crew.onboarding_import`,
...
Deliberately dependency-free: only stdlib, no ``codex_crew`` imports, so this
module can never be the module that closes an import cycle.

Note what that does NOT buy. Importing ``codex_crew.agent_sdk.provider_identity``
...
is stdlib-only, and ``drivers.acp`` defers every ``codex_crew.acp`` import into a
function body, so ``codex_crew.acp`` itself stays unloaded. That last property is
