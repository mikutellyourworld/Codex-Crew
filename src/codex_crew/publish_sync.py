...
- Rendering a CodexCrew artifact to ``(bytes-on-disk, content_type)`` — including
  wrapping ``widget`` artifacts in a standalone HTML document.
- The version-bump → push chokepoint and the 1:1 CodexCrew↔destination version
  invariant. Reverts mirror as a new version (never a pointer-rollback).
- Best-effort sync: a ``push_version`` failure NEVER fails the CodexCrew update;
...
from codex_crew.artifacts import (
    Artifact,
    ArtifactPublication,
...
from codex_crew.publish_provider import (
    DEFAULT_PROVIDER,
    NON_TEXT_KINDS,
    Capability,
    DriveNotFound,
...
# dashboard theme). Mirrors CodexCrewWebsite/src/lib/widgetSrcdoc.ts.
...
# in a sandboxed iframe, which the provider owns and Codex Crew cannot enforce;
...
def unwrap_widget_html(html: str) -> str | None:
    ...
    Used on pull: a sentinel-bearing document was a CodexCrew widget, so we strip
...
def _publication_summary(pub: ArtifactPublication) -> dict[str, object]:
    ...
    return {
        ...
        "last_synced_codexcrew_version": pub.last_synced_codexcrew_version,
...
async def _publish_unlocked(
    ...
    *,
    ...
    pub = ArtifactPublication(
        ...
        last_synced_codexcrew_version=art.version,
...
async def push_version(art: Artifact, *, force: bool = False) -> None:
    ...
    exactly once per CodexCrew version (idempotent + the 1:1 invariant); a force
    ...
    # Push exactly once per CodexCrew version. If this version was already synced
    ...
    if not force and fresh.version == pub.last_synced_codexcrew_version and not wrapper_stale:
        return
    ...
    await asyncio.to_thread(
        lambda: store.update_publication(
            ...
            last_synced_codexcrew_version=fresh.version,
    ...
    logger.info(
        "artifact version synced: slug=%s provider=%s codexcrew_v=%s dest_v=%s",
        art.slug,
        pub.provider,
        fresh.version,
        res.version_number,
...
_DRIFT_PREFIX = "The remote copy changed outside Codex Crew"
...
_DRIFT_PREFIXES = (_DRIFT_PREFIX, "The remote copy changed outside CodexCrew")
...
async def refresh_publication(slug: str) -> Artifact:
    ...
    # mirror it into CodexCrew's append-only history; instead surface a re-sync
    # prompt (the conflict banner's "Force re-sync" re-asserts CodexCrew's
    # current content as a new top version). CodexCrew stays authoritative.
    expected_dest_v = pub.version_map.get(str(pub.last_synced_codexcrew_version))
    ...
    if drifted:
        ...
        drift_msg = (
            f"{_DRIFT_PREFIX}: it is showing {cur_v_str} (Codex Crew published "
            f"{expected_str}). Force re-sync to re-publish Codex Crew's current version."
        )
...
def _kind_for_content_type(content_type: str) -> str:
    """Map an upstream content-type back to a CodexCrew artifact kind.
...
def _source_target(art: Artifact, source: str):
    ...
    if source == "publication" and art.publication is not None:
        pub = art.publication
        expected = pub.version_map.get(str(pub.last_synced_codexcrew_version))
...
async def upstream_status(slug: str) -> dict[str, object]:
    ...
    if is_pub and art.publication is not None and art.publication.collab_mode == "live":
        ...
        local_ahead_live = (
            art.version > pub.last_synced_codexcrew_version
    ...
    local_ahead = (
        ...
        and (
            art.version > art.publication.last_synced_codexcrew_version
            # Wrapper envelope changed since last push — widget needs re-render (#3373).
...
async def pull_upstream(
    ...
    if is_pub and updated.publication is not None:
        ...
        await asyncio.to_thread(
            lambda: store.update_publication(
                slug,
                last_synced_codexcrew_version=updated.version,
