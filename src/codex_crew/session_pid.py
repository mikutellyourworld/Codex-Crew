...
from codex_crew.mcp_gateway.shutdown_budget import TOTAL_SHUTDOWN_BUDGET_SECS
...
# Basenames of agent runtimes whose lifecycle Codex Crew manages through PID-file
...
def _is_managed_agent_process(pid: int) -> bool:
    """Check if a PID belongs to an agent process managed by CodexCrew (guards against PID recycling)."""
...
# Catches CodexCrew-spawned MCP subtrees that escaped PID-file tracking.
...
# CODEXCREW_SPAWNED environ marker — see _is_sweepable_orphan_work). Work
...
# Entrypoints that positively identify a CodexCrew-spawned MCP/worker process.
# Each marker MUST be unique to a process CodexCrew itself launches — the sweep
...
_MCP_ENTRYPOINT_MARKERS = (
    b"codexcrew_sandbox_",  # sandbox wrapper script (session-spawned)
    b"codex_crew.mcp_gateway.stub",  # gateway pool worker (not gatewayd itself)
)
...
_GATEWAYD_MODULE = b"codex_crew.mcp_gateway.gatewayd"
_GATEWAY_MARKERS = (
    _GATEWAYD_MODULE,
    b"codex_crew.cli",
    b"codex_crew.__main__",
)

# MCP launcher cmdline shapes that carry NO CodexCrew fingerprint (a user's own
# shell can produce identical cmdlines), so matching them requires the
# ``CODEXCREW_SPAWNED`` environ marker as positive identity — the public fork's
...
# its argv0 (``npx``/``node``) nor its args mention CodexCrew, so a grandchild
...
#   so its EXEC-TIME environ carries both ``CODEXCREW_SPAWNED`` and the
...
#: Must track :data:`codex_crew.browser_cli.launch.SESSION_ENV`. Duplicated as a
...
def _is_generated_browser_session(name: bytes) -> bool:
    """True for a Codex-Crew-generated ``kc-<8hex>`` session name.
...
def _env_value(pid: int, key: str) -> bytes | None:
    ...
    :func:`_env_has_codexcrew_marker`: the callers here need to tell "read
...
def _browser_session_owner_alive(pid: int, session: bytes) -> bool:
    ...
    Codex Crew injects the generated ``PLAYWRIGHT_CLI_SESSION`` into exactly
...
def _is_sweepable_orphan_browser_daemon(pid: int, cmdline: bytes, age_seconds: float) -> bool:
    ...
    2. that same name in the process's exec-time environ, which ties this
       daemon to a name Codex Crew itself generated rather than one an agent
       passed with ``-s=``;
    3. the ``CODEXCREW_SPAWNED`` environ marker, proving Codex Crew spawned the
       tree (:func:`_env_has_codexcrew_marker`, Linux-only, fail-closed);
    ...
    if not _env_has_codexcrew_marker(pid):
        return False
...
def _is_orphan_mcp(cmdline: bytes) -> bool:
    """True if cmdline matches a CodexCrew MCP entrypoint (not a peer gateway)."""
...
def _is_marked_mcp_launcher(cmdline: bytes) -> bool:
    ...
    :func:`_env_has_codexcrew_marker` because a user's own shell produces
...
def _env_has_codexcrew_marker(pid: int) -> bool:
    """True if *pid*'s environment carries the ``CODEXCREW_SPAWNED`` marker.
    ...
    if sys.platform != "linux":
        return False
    needle = f"{CODEXCREW_SPAWNED_ENV}={CODEXCREW_SPAWNED_VALUE}".encode()
    try:
        environ = Path(f"/proc/{pid}/environ").read_bytes()
    except OSError:
        return False
...
def _gatewayd_socket_arg(cmdline: bytes) -> bytes | None:
    ...
    Accepts the two-token ``--socket <path>`` form (the shape every Codex Crew
...
def _is_sweepable_orphan_gatewayd(cmdline: bytes) -> bool:
    ...
    1. a structural ``-m codex_crew.mcp_gateway.gatewayd`` argv pair — never
       ``codex_crew.cli`` / ``codex_crew.__main__``, which stay unconditionally
...
def _is_sweepable_orphan_work(pid: int, cmdline: bytes, age_seconds: float) -> bool:
    ...
    2. The ``CODEXCREW_SPAWNED`` environ marker (:func:`_env_has_codexcrew_marker`,
       Linux-only, fail-closed elsewhere). The marker is only ever injected
       into environments Codex Crew itself spawns (sandbox wrapper, ACP client
    ...
    return _env_has_codexcrew_marker(pid)
...
def _is_untracked_managed_agent_orphan(pid: int, cmdline: bytes, tracked_pids: set[int]) -> bool:
    ...
    * :func:`_is_sweepable_orphan_work` NEGATIVE-gates
    ...
    ``CODEXCREW_SPAWNED`` environ marker (:func:`_env_has_codexcrew_marker`,
        ...
        if _is_untracked_managed_agent_orphan(pid, cmdline, tracked_pids):
            ...
            if pid not in _reported_untracked_agent_pids:
                ...
                logger.error(
                    ...
                    "to init/systemd with a CODEXCREW_SPAWNED environ marker but "
    ...
    for pid in pids:
        ...
        try:
            ...
            if gw_age >= _ORPHAN_MIN_AGE_SECONDS and _is_sweepable_orphan_gatewayd(cmdline):
                ...
                continue
            # Work-class orphan (CODEXCREW_SPAWNED marker, no launcher shape).
            ...
            try:
                ...
                # a module-level `from codex_crew.sel import sel` would be circular.
...
def _pid_cmdline(pid: int) -> bytes:
    ...
    feeds a decision that also requires :func:`_env_has_codexcrew_marker`, which
...
def _pid_parent_and_token(pid: int) -> tuple[int | None, str | None]:
    ...
    reap is already a no-op because :func:`_env_has_codexcrew_marker` is
...
def _prune_from_orphan_walk(pid: int) -> bool:
    ...
    enumerated, and each carries ``CODEXCREW_SPAWNED`` with no gateway marker in
...
def _kill_orphan_mcp_descendants(
    ...
    * ``CODEXCREW_SPAWNED`` in the member's exec-time environ, proving it
      belongs to a tree Codex Crew spawned (:func:`_env_has_codexcrew_marker`,
    ...
    * The SAME process the walk saw -- its ``_pid_start_token`` must still
      ...
      hands that PID to a NEW Codex-Crew-spawned worker, and the stale entry
    ...
    for target, walk_token in reversed(descendants):
        ...
        if any(marker in cmdline.replace(b"\x00", b" ") for marker in _GATEWAY_MARKERS):
            ...
            continue
        if not _env_has_codexcrew_marker(target):
            continue  # not provably part of a Codex Crew tree
    ...
    try:
        ...
        sel().log_tool_invocation(
            ...
            metadata={
                ...
                "reason": "descendants of CODEXCREW_SPAWNED MCP launcher orphan",
            },
        )
    except Exception:
...
def _kill_orphan_work_tree(pid: int, cmdline: bytes, age_seconds: float, budget: int) -> int:
    ...
    of a marked work orphan inherited the ``CODEXCREW_SPAWNED`` environment
        ...
        try:
            ...
            if target == pid:
                logger.info(
                    ...
                    "reason=CODEXCREW_SPAWNED work orphan (reparented to init)",
                    target,
                    basename,
                    int(age_seconds),
                )
    ...
    try:
        ...
        sel().log_tool_invocation(
            ...
            metadata={
                "cmdline": cmdline[:200].decode("utf-8", errors="replace"),
                "killed_in_tree": killed,
                "reason": "CODEXCREW_SPAWNED orphan work process",
            },
        )
    except Exception:
