...
CodexCrew never performs an IAM write. Verification is **read-only reachability only**
...
# The bucket template creates codexcrew-deploy-${AccountId}-${Region}; the
# engine.py HTTP-publish path creates codexcrew-web-<random>. The IAM policy
# must cover BOTH prefixes to work with both code paths.
S3_PREFIX = "codexcrew-deploy-*"
...
BOUNDARY_POLICY_NAME = "codexcrew-deploy-app-boundary"
...
def boundary_policy_document() -> dict[str, Any]:
    ...
    --policy-name codexcrew-deploy-app-boundary --policy-document file://...
    """
    return {
        "Version": "2012-10-17",
        "Statement": [
            ...
            },
            {
                ...
                "Resource": "arn:aws:dynamodb:*:*:table/codexcrew-deploy-app-*",
            },
            {
            ...
            },
            {
                ...
                # codexcrew-managed resources only.
            ...
            },
            {
                ...
                "Resource": "arn:aws:cloudformation:*:*:stack/codexcrew-deploy-app-*/*",
            },
            {
                "Sid": "ReaperStsCap",
                "Effect": "Allow",
            ...
            },
            {
                ...
                "Resource": [
                    "arn:aws:lambda:*:*:function:codexcrew-deploy-app-*",
                    "arn:aws:dynamodb:*:*:table/codexcrew-deploy-app-*",
                ],
            },
            {
                ...
                "Resource": [
                    "arn:aws:logs:*:*:log-group:/codexcrew-deploy-app/*",
                    "arn:aws:logs:*:*:log-group:/codexcrew-deploy-app/*:*",
                ],
            },
            {
            ...
            },
            {
                ...
                "Resource": "arn:aws:iam::*:role/codexcrew-deploy-app-*",
            },
            {
                "Sid": "ReaperApiGwCap",
                "Effect": "Allow",
...
def _drive_statements() -> list[dict[str, Any]]:
    ...
    pinned to the ``codexcrew-drive-*`` naming scheme the engine creates and
    ...
    drive_arn = "arn:*:s3:::codexcrew-drive-*"
    return [
        {
...
def policy_document(*, include_custom_domain: bool = False, tier: str = "static") -> dict[str, Any]:
    ...
    Tiers:
      ...
      - fullstack: adds scoped Lambda, API Gateway, DynamoDB, IAM PassRole
        for codexcrew-deploy-app-* stacks, and CloudFront invalidation.
      - drive: a SELF-CONTAINED statement set for the AWS Control drive —
        S3 on codexcrew-drive-* buckets only, tag discovery, identity, and
    ...
    statements: list[dict[str, Any]] = [
        {
            ...
            "Action": [
                ...
                # and the codexcrew-deploy-logs-* target bucket match the
                # codexcrew-deploy-* resource prefix below.
                "s3:PutBucketLogging",
                "s3:GetBucketLogging",
            ],
        ...
        },
        {
            ...
            # codexcrew-deploy-* wildcard, which also matches the append-only
            # audit bucket codexcrew-deploy-logs-* (CloudTrail data events + S3
            ...
            "Resource": [
                "arn:aws:s3:::codexcrew-deploy-logs-*",
                "arn:aws:s3:::codexcrew-deploy-logs-*/*",
            ],
        },
        {
        ...
        },
        {
            ...
            # so the codexcrew:managed tag condition can never match and no
        ...
        },
        {
            ...
            "Resource": "arn:aws:cloudformation:*:*:stack/codexcrew-deploy-*/*",
        },
        {
        ...
        },
        {
            ...
            # creates (codexcrew-deploy-trail-*) — all these actions support the
            # trail resource type.
            ...
            "Resource": "arn:aws:cloudtrail:*:*:trail/codexcrew-deploy-trail-*",
        },
    ]
    statements.append(
        {
        ...
        statements += [
            {
                ...
                "Resource": "arn:aws:lambda:*:*:function:codexcrew-deploy-app-*",
            },
            {
                # CWE-778 access logging: the app-apigw*.yaml stages create an
                # AWS::Logs::LogGroup (/codexcrew-deploy-app/<slug>/apigw*) and
            ...
            {
                # Mutations gated on the codexcrew:managed tag — the
                # templates tag every AWS::ApiGatewayV2::Api at creation, so
                # only CodexCrew-created APIs are mutable/deletable. An
                ...
                "Condition": {"StringEquals": {"aws:ResourceTag/codexcrew:managed": "true"}},
            },
            {
            ...
            },
            {
                ...
                # the codexcrew-deploy-app-boundary permissions boundary — this
            ...
            },
            {
                ...
                "Resource": [
                    "arn:aws:iam::*:role/codexcrew-deploy-app-*",
                    "arn:aws:iam::*:role/codexcrew-deploy-reaper*",
                ],
            },
            {
            ...
            },
            {
                ...
                "Resource": "arn:aws:lambda:*:*:function:codexcrew-deploy-reaper*",
            },
            {
                ...
                "Resource": "arn:aws:cloudformation:*:*:stack/codexcrew-deploy-reaper/*",
            },
            {
                ...
                "Effect": "Allow",
                "Action": ["iam:CreateRole"],
                "Resource": "arn:aws:iam::*:role/codexcrew-deploy-reaper*",
                "Condition": {
                    "ArnLike": {
                        "iam:PermissionsBoundary": (f"arn:aws:iam::*:policy/{BOUNDARY_POLICY_NAME}")
                    }
            ...
            },
            {
                ...
                "Condition": {
                    ...
                    "ArnLike": {
                        "iam:AssociatedResourceArn": (
                            "arn:aws:lambda:*:*:function:codexcrew-deploy-reaper*"
                        )
                    },
                },
            },
            {
                ...
                "Resource": "arn:aws:events:*:*:rule/codexcrew-deploy-reaper*",
            },
            {
