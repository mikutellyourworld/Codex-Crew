...
the existing bucket + distribution by the ``codexcrew:site`` tag and reuses them
(idempotent), so the public URL stays stable. The physical bucket name is an
opaque ``codexcrew-web-<random>`` (no account id); idempotency never relies on
re-deriving it.
"""
...
BUCKET_PREFIX = "codexcrew-web-"
TAG_MANAGED = "codexcrew:managed"
TAG_SITE = "codexcrew:site"
...
def resolve_aws_tool_bin(name: str) -> str:
    ...
    :func:`codex_crew.github_runner.validate_provider_executable` — the repo's
    ...
    if found:
        from codex_crew.github_runner import validate_provider_executable
...
def aws_spawn_env(aws_bin: str) -> dict[str, str]:
    ...
    also why this is not :func:`codex_crew.env.augmented_path`, which PREPENDS a
...
def run_aws(
    ...
    *,
    ...
    resolved by the CLI's own provider chain; CodexCrew passes only the profile name.
...
def find_site_by_tag(site_id: str, profile: str, region: str = DEFAULT_REGION) -> Optional[dict[str, str]]:
    """Resolve an existing site's bucket + distribution by the codexcrew:site tag.
    ...
    (1) require BOTH codexcrew:site=<id> AND codexcrew:managed=true (filters
    ...
    managed naming scheme (codexcrew-web-*); (3) multiple matching buckets or
    ...
    policy = {
        ...
        "Statement": [
            ...
            # this service-principal Deny does not affect.
            {
                ...
                "Action": "s3:GetObject",
                "Resource": [
                    f"arn:aws:s3:::{bucket}/*/.codexcrew-deploy.json",
...
def list_sites(profile: str, region: str = DEFAULT_REGION) -> list[dict[str, Any]]:
    ...
    Groups managed resources by the codexcrew:site tag; status/domain read live
