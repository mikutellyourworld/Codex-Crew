...
`codexcrew-` prefix, so a first deploy with the new tooling creates a second,
...
gates (OAC name fullmatch, `codexcrew:site` tag checks, bucket-prefix
...
lands behind the `codexcrew-deploy-base` distribution, then verify the new URL
