...
# CodexCrew One-Click Deploy (MVP)
...
registry** at `~/.codex-crew/deploy/profiles.json`
...
3. **Legacy fallback**: if `profiles.json` doesn't exist, read the old
   `~/.codex-crew/deploy/config.json (legacy: ~/.codex-crew/apps/deploy-web/data/config.json)` (`{"profile", "region"}`).
...
- **One shared base stack per account** (`codexcrew-deploy-base`): a private S3
...
- Cost lives in the **user's** account and is scale-to-zero (S3 storage +
  CloudFront requests). Idle ≈ $0. AWS earns the consumption; CodexCrew pays
  nothing.
...
- **POSIX platform (Linux / macOS)** — deploy scripts require `bash`.
  ...
  CodexCrew gateway if your host OS is Windows. The backend returns HTTP 400
  with a clear message on unsupported platforms.
- AWS access configured **once in the Artifact Deploy app** (profiles registered
  in `~/.codex-crew/deploy/profiles.json`, verified via the app page).
...
- **M3 lifecycle** — DONE: `list.sh` / `persist.sh` / `detach_backend.py`, plus the
  **in-account scheduled reaper** (`install-reaper.sh` → EventBridge-timed Lambda;
  ...
  role scoped to `codexcrew-deploy-app-*`. Runs in-account with no local creds —
