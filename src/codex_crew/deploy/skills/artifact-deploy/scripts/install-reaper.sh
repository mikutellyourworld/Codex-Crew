#!/usr/bin/env bash
# CodexCrew One-Click Deploy - install the in-account scheduled reaper ONCE per
...
_STACK_STATUS="$(aws_cli cloudformation describe-stacks --stack-name codexcrew-deploy-reaper \
  --query 'Stacks[0].StackStatus' --output text 2>/dev/null || echo "DOES_NOT_EXIST")"
if [[ "$_STACK_STATUS" == "ROLLBACK_COMPLETE" ]]; then
  echo "error: stack codexcrew-deploy-reaper is in ROLLBACK_COMPLETE state." >&2
  echo "  This dead stack blocks new deployments. Delete it first:" >&2
  echo "    aws cloudformation delete-stack --stack-name codexcrew-deploy-reaper \\" >&2
  echo "      ${PROFILE:+--profile $PROFILE }--region $REGION" >&2
  exit 1
fi
...
echo ""
echo "✅ reaper installed: Lambda codexcrew-deploy-reaper runs $RATE in-account."
...
echo "   Test one run:  aws lambda invoke --function-name codexcrew-deploy-reaper /tmp/reap.json --profile <p> --region <r> && cat /tmp/reap.json"
