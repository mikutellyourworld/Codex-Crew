"""CodexCrew One-Click Deploy - in-account reaper Lambda.

Triggered by an EventBridge schedule (see reaper.yaml). Each run scans the
shared deploy bucket for per-slug `.codexcrew-deploy.json` manifests and, for any
NON-persistent deploy whose `expires_at` is in the past, reaps it:
  ...
  3. delete its backend CloudFormation stack codexcrew-deploy-app-<slug>
...
def _manifest(slug):
    try:
        obj = s3.get_object(Bucket=BUCKET, Key=f"{slug}/.codexcrew-deploy.json")
...
def _stack_site_tag_ok(name, slug):
    """A stack may only be deleted when its codexcrew:site tag equals
    ...
    try:
        ...
        return False
    if tags.get("codexcrew:site") != slug:
        print(json.dumps({"app_reap_stack_tag_mismatch": slug, "stack": name,
                          "actual": tags.get("codexcrew:site", "<missing>")}))
        return False
    return True
...
def _quarantine_manifest(slug):
    ...
    key = f"{slug}/.codexcrew-deploy.json"
    try:
        obj = s3.get_object(Bucket=BUCKET, Key=key)
        s3.put_object(
            Bucket=BUCKET,
            Key=f"_quarantine/{slug}.codexcrew-deploy.json",
            Body=obj["Body"].read(),
...
def _del_stack(slug):
    name = f"codexcrew-deploy-app-{slug}"
...
def _reap_engine_arch(man, slug, now):
    ...
    if man.get("oac_pending"):
        ...
        try:
            s3.delete_object(Bucket=BUCKET, Key=f"{slug}/.codexcrew-deploy.json")
    ...
    if dist_id:
        # Verify codexcrew:site tag matches the slug being reaped BEFORE
        # any destructive operation. The tag VALUE is the site_id (NOT "true" —
        # that's on codexcrew:managed). engine.py sets this at creation via
        ...
        try:
            ...
            expected_site = slug
            if tags.get("codexcrew:site") != expected_site:
    ...
    if site_bucket:
        ...
        if not _owner:
            ...
            return "reaping"
        # Verify codexcrew:site tag matches the slug being reaped.
        # engine.py tags buckets at creation: codexcrew:site=<site_id>.
        try:
            ...
            # which deployment's resources the reaper targets.
            expected_site = slug
            if bucket_tags.get("codexcrew:site") != expected_site:
                print(json.dumps({"engine_reap_bucket_tag_mismatch": slug,
                                  "bucket": site_bucket,
                                  "expected": expected_site,
                                  "actual": bucket_tags.get("codexcrew:site", "<missing>")}))
                _quarantine_manifest(slug)
            ...
            if code == "NoSuchBucket":
                ...
                # point at any untagged codexcrew-web-* bucket (e.g. someone
...
def handler(event, context):
    ...
    for slug in _slugs():
        ...
        if man and man.get("reaping"):
            ...
            stack_name = f"codexcrew-deploy-app-{slug}"
            try:
        ...
        # (tagged codexcrew:site). The existing shared-bucket reap path doesn't touch them.
        if man.get("arch") == "engine":
            result = _reap_engine_arch(man, slug, now)
            if result == "reaped":
                reaped.append(slug)
        ...
        if has_stack:
            # Verify the stack's codexcrew:site tag matches the slug
            # being reaped BEFORE deletion — same identity gate as the
            # engine-arch path. deploy-backend.sh tags stacks at
            # creation (codexcrew:site=<slug>); a stack without a matching tag
            # is not provably this deployment's — skip destructive ops.
            if stack_tags.get("codexcrew:site") != slug:
                print(json.dumps({
                    "app_reap_stack_tag_mismatch": slug,
                    "stack": stack_name,
                    "actual": stack_tags.get("codexcrew:site", "<missing>"),
                }))
