#!/usr/bin/env bash
# CodexCrew One-Click Deploy - cost ESTIMATE from usage metrics (NOT the AWS bill;
...
if [[ -n "$SLUG" ]]; then
  _man="$(aws_cli s3 cp "s3://$BUCKET/$SLUG/.codexcrew-deploy.json" - 2>/dev/null || true)"
  if [[ -n "$_man" ]]; then
...
echo "CodexCrew deploy cost - ESTIMATE from usage (NOT the AWS bill; billing lags ~24h)"
...
if [[ -n "$SLUG" ]]; then
  ...
  if aws_cli cloudformation describe-stacks --stack-name "codexcrew-deploy-app-$SLUG" >/dev/null 2>&1; then
    lfn="$(aws_cli cloudformation describe-stacks --stack-name "codexcrew-deploy-app-$SLUG" --query 'Stacks[0].Outputs[?OutputKey==`FunctionName`].OutputValue' --output text 2>/dev/null)"
