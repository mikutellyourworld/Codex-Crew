#!/usr/bin/env bash
# CodexCrew One-Click Deploy - deploy a pre-built static site under a slug.
#
# Usage:
...
# Falls back to a builtin grep set ONLY when python3/codex_crew is unavailable;
# the fallback's failure is also fatal (no `|| true`).
_SCAN_RC=0
if python3 -c "import codex_crew.security" 2>/dev/null; then
  CRED_HITS=$(python3 - "$SNAPSHOT_DIR" <<'PYEOF'
import sys, os, re
from pathlib import Path
from codex_crew.security import get_credential_patterns, is_sensitive_path
...
PYEOF
  ...
  # Fallback: python3/codex_crew unavailable — use grep (failure = abort).
...
if [[ "$TTL_HOURS" != "0" ]]; then
  if ! "${AWS[@]}" cloudformation describe-stacks --stack-name "codexcrew-deploy-reaper" >/dev/null 2>&1; then
    echo ""
    echo "ERROR: The reaper stack (codexcrew-deploy-reaper) is NOT deployed."
...
"${AWS[@]}" s3 cp "$MANIFEST" "s3://$BUCKET/$SLUG/.codexcrew-deploy.json" \
