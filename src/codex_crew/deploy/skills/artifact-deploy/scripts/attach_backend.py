#!/usr/bin/env python3
"""Attach a per-app backend origin to the shared CodexCrew CloudFront distribution.
...
OAC_NAME = "codexcrew-deploy-lambda-oac"


def aws(profile, region, *args):
    # Every AWS spawn from these LLM-facing helpers MUST route through
    # the sandbox chokepoint. Failing open when codex_crew is not importable
    ...
    # invocation), never bare python3 without codex_crew on sys.path.
    try:
        from codex_crew.deploy.engine import resolve_aws_bin
        from codex_crew.sandbox import run_limited, sandboxed_spawn_argv
    except ImportError:
        sys.stderr.write(
            "error: codex_crew package not importable — refusing to spawn AWS "
            "commands unsandboxed. Run this script with the CodexCrew venv "
...
def ensure_lambda_oac(profile, region):
    ...
    cfg = {
        "Name": OAC_NAME,
        "Description": "CodexCrew deploy Lambda OAC",
...
def _validate_args(profile: str, region: str, dist_id: str, slug: str) -> None:
    ...
    # VERBATIM copy of codex_crew.constants.AWS_PROFILE_NAME_PATTERN (#6063):
...
# Standalone list (duplicated here for standalone execution where codex_crew
# may not be importable) — covers both the current CodexCrew data home
# (~/.codex-crew) and the legacy ~/.codexcrew home, so a not-yet-migrated box
# is covered too.
_SENSITIVE_PREFIXES: tuple[str, ...] = (
    ...
    ".codex-crew/trust",
    ".codex-crew/security_events.jsonl",
    ".codexcrew/.env",
    ".codexcrew/sel_hmac.key",
    ".codexcrew/trust",
    ".codexcrew/security_events.jsonl",
)
...
def main():
    ...
    ap.add_argument(
        ...
        default="",
        help="random secret for x-codexcrew-origin-verify header (API GW authorizer)",
    )
    ap.add_argument(
    ...
    if a.origin_verify_secret_file:
        ...
        # Preferred path: codex_crew.hooks.safe_read_file does is_sensitive_path +
        # O_NOFOLLOW open in one shot, closing the TOCTOU between validation and
        # read. Fallback: standalone execution where codex_crew is not importable
        ...
        try:
            ...
            # codex_crew.hooks.safe_read_file. Reading a user-controlled secret
            ...
            sys.stderr.write(
                "error: codex_crew.hooks is unavailable — cannot read "
                "--origin-verify-secret-file without the shared sensitive-path "
                "gate. Run this script from a codex_crew environment (fail-closed).\n"
            )
            sys.exit(2)
    # Argument validation — FieldSpec patterns with ImportError fallback for
    # standalone operator execution where codex_crew is not on sys.path.
    ...
    if origin_verify_secret:
        custom_headers = {
            ...
            "Items": [
                {
                    "HeaderName": "x-codexcrew-origin-verify",
