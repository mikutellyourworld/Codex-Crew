#!/usr/bin/env bash
# Shared helpers for CodexCrew One-Click Deploy scripts.
#   source "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/_common.sh"
# Provides: STACK_NAME, PROFILE, REGION, aws_cli(), read_base_outputs(), validate_slug(),
#           scan_source_dir()
STACK_NAME="codexcrew-deploy-base"
PROFILE="${PROFILE:-}"
...
# scan_source_dir <dir>
# Fail-closed pre-package/upload scan. Uses codex_crew.security (canonical) with
...
validate_source_dir() {
  ...
  # Canonical check (preferred): codex_crew.security.is_sensitive_path.
...
VSD_PYEOF
    ...
    for sens in "$HOME/.aws" "$HOME/.ssh" "$HOME/.kube" "$HOME/.gnupg" \
                "$HOME/.config/gcloud" "/etc" "$HOME/.codexcrew/keys" "$HOME/.kiro"; do
