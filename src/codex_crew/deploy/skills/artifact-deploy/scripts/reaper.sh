#!/usr/bin/env bash
# CodexCrew One-Click Deploy - TTL reaper. Deletes non-persistent deploys whose
# expires_at is in the past (S3 prefix + invalidation, and any backend stack).
# Safe to run on a schedule (CodexCrew cron / in-account EventBridge).
...
aws_cli s3 ls "s3://$BUCKET/" 2>/dev/null | awk '/ PRE /{print $2}' | while read -r pfx; do
  ...
  man="$(aws_cli s3 cp "s3://$BUCKET/$slug/.codexcrew-deploy.json" - 2>/dev/null)"
  [[ -n "$man" ]] || continue
  ...
  if [[ "$_arch" == "engine" ]]; then
    ...
    if [[ -n "$_site_dist" ]]; then
      ...
      if [[ "$_enabled" == "GONE" ]]; then
        ...
        # attacker-writable — verify the distribution's codexcrew:site tag
        ...
        _dist_site_tag="$(aws_cli cloudfront list-tags-for-resource --resource "$_dist_arn" \
          --query "Tags.Items[?Key=='codexcrew:site'].Value | [0]" --output text 2>/dev/null || echo "TAG_LOOKUP_FAILED")"
        if [[ "$_dist_site_tag" != "$slug" ]]; then
          echo "   error: distribution $_site_dist codexcrew:site tag ('$_dist_site_tag') != slug '$slug' -- identity mismatch, quarantining manifest, skipping" >&2
          continue
        fi
      fi
      if [[ "$_enabled" == "GONE" ]]; then
        ...
        aws_cli s3 cp "$_tmp" "s3://$BUCKET/$slug/.codexcrew-deploy.json" --content-type application/json --only-show-errors
        rm -f "$_tmp"
    ...
    if [[ -n "$_site_bucket" ]]; then
      # R27 F1 (twin): verify the bucket's codexcrew:site tag matches the slug
      ...
      _btag="$(aws_cli s3api get-bucket-tagging --bucket "$_site_bucket" \
        --query 'TagSet[?Key==`codexcrew:site`].Value | [0]' --output text 2>/dev/null || echo '')"
      if [[ "$_btag" != "$slug" ]]; then
        if aws_cli s3api head-bucket --bucket "$_site_bucket" 2>/dev/null; then
          echo "   bucket $_site_bucket identity tag mismatch (codexcrew:site='$_btag') -- skipping"
...
" "$slug"; then
    ...
    if [[ "$_engine_all_clean" == "true" ]]; then
      aws_cli s3 rm "s3://$BUCKET/$slug/.codexcrew-deploy.json" --only-show-errors 2>/dev/null || true
  ...
  if [[ "$_reaping" == "true" ]]; then
    ...
    stack_name="codexcrew-deploy-app-$slug"
    ...
    if [[ "$stack_status" == "GONE" || "$stack_status" == "DELETE_COMPLETE" ]]; then
      ...
      _stack_tag="$(aws_cli cloudformation describe-stacks --stack-name "$stack_name" \
        --query 'Stacks[0].Tags[?Key==`codexcrew:site`].Value | [0]' --output text 2>/dev/null || echo '')"
      if [[ "$_stack_tag" != "$slug" ]]; then
        echo "   stack $stack_name tag mismatch (codexcrew:site='$_stack_tag') -- skipping retry"
        continue
      fi
  ...
  if aws_cli cloudformation describe-stacks --stack-name "codexcrew-deploy-app-$slug" >/dev/null 2>&1; then
    # R21 F1: verify the stack's codexcrew:site tag matches the slug being
    ...
    _stack_tag="$(aws_cli cloudformation describe-stacks --stack-name "codexcrew-deploy-app-$slug" \
      --query 'Stacks[0].Tags[?Key==`codexcrew:site`].Value | [0]' --output text 2>/dev/null || echo '')"
    if [[ "$_stack_tag" != "$slug" ]]; then
      echo "   stack codexcrew-deploy-app-$slug tag mismatch (codexcrew:site='$_stack_tag') -- skipping"
      continue
    fi
    # Two-phase: initiate stack deletion and write reaping marker.
    if ! aws_cli cloudformation delete-stack --stack-name "codexcrew-deploy-app-$slug"; then
      echo "   stack delete failed for $slug -- will retry next run"
      continue
    fi
    echo "   backend stack codexcrew-deploy-app-$slug scheduled for deletion"
