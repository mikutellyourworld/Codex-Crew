# deploy-web — CodexCrew App Design Doc (v1)

**Status:** ✅ Implemented (v1) — built-in app at `codex_crew/apps/builtins/deploy_web/`
(engine/render/scan/iam/handlers) + UI `CodexCrewWebsite/src/apps/deploy-web/DeployWebPage.tsx`,
...
Extend CodexCrew's scope — including **public usage** — by collaborating with AWS,
**without CodexCrew ever managing AWS accounts or credentials**, to **safely deploy
...
your own AWS" is the deliberate bar. The *user-owns-their-own-cloud* model (CodexCrew
...
1. **One-time AI-guided setup** (§12): the agent walks the user through AWS *access* —
   ...
   **once ever**, reused for everything after. CodexCrew stores only the profile name.
...
### Packaging — built-in app, ships with CodexCrew
deploy-web is a **built-in app that ships *with* CodexCrew** (like Research Lab / Team
...
CodexCrew package and **disabled / opt-in by default** so users without AWS never see AWS
...
**Flagship use case:** publish an existing CodexCrew artifact — **HTML, markdown file,
...
**YES — build a `deploy-web` built-in CodexCrew app** (ships with CodexCrew, opt-in/disabled
...
CodexCrew's code-verified posture.

### One-picture flow
> User authenticates **once** (`aws configure sso`) → CodexCrew stores only the
> **profile name** → `deploy-web --artifact <dir>` runs a deterministic 6-step flow
> on the user's credentials → returns a live HTTPS URL. CodexCrew never sees or stores a key.
...
deploy-web mostly **extends code CodexCrew already ships**:
...
upload the module runs the rendered output through CodexCrew's **existing
...
- **AWS account is the source of truth.** CodexCrew holds **no** local record — **no
  SQLite cache in v1** (Q8). `list-sites` queries the Resource Groups Tagging API live
  every time (`tag:GetResources Key=codexcrew:managed`), so there is zero drift.
- **Identity vs naming (Q2):**
  - **Logical site id** = the artifact slug (or user-supplied name) → drives the
    `codexcrew:site=<id>` tag and the UI label. This is what makes re-deploy idempotent.
  - **Physical bucket name** = `codexcrew-web-<random ~12hex>` — opaque, **no account
...
- Mandatory tags on every resource: `codexcrew:managed=true` + `codexcrew:site=<id>`,
...
- CodexCrew **never persists AWS credentials** — every key reference in source is
...
of the following is ever done by CodexCrew:
...
- ❌ Running `aws configure`, `aws sso login`, `ada`, or any credential-establishing
  command on the user's behalf — these remain **user-run, outside CodexCrew**.
- ❌ Assuming roles, minting STS tokens, or brokering cross-account access.

What CodexCrew **does**, and the entire trust surface, is narrow:
...
- ✅ Pass `--profile <name>` to the **local** AWS CLI and let the OS-resident provider
  chain resolve credentials entirely outside CodexCrew's process boundary.
...
This makes the credential/account boundary identical to CodexCrew's existing,
...
Two scoping levers: an **S3 name prefix** (`codexcrew-web-*`) and a **resource tag**
(`aws:ResourceTag/codexcrew:managed=true`) confining CloudFront mutate/delete actions.
...
## 8. Build shape — built-in app (ships with CodexCrew)
...
- **Backend: a Python builtin module** — runs the §4 6-step deploy flow + recall/destroy
  ...
  credential resolution entirely outside CodexCrew's process, preserving the §6.1
...
- **No per-app update cron.** As a **built-in** app, deploy-web is versioned and updated
  with the CodexCrew package itself (`codexcrew update`) — there is no independent upstream
...
- **Built-in packaging:** ships inside the CodexCrew package (registered like Research
...
- CodexCrew **never reads, stores, or persists** AWS credentials — verified across the
...
- Scoped customer-managed policy (§7): S3 confined to the `codexcrew-web-*` prefix;
  CloudFront mutate/delete confined by the `codexcrew:managed=true` resource tag.
...
- The approval prompt **states the public nature** of the URL being created
  (a world-readable site), per CodexCrew's security-awareness norm.
...
- **One-time AWS prep (~10–15 min, once ever):** `aws configure sso` (~3 min) +
  create the IAM role/policy (~5–10 min). CodexCrew stores only the profile name.
...
- **Chat-native path:** saying "deploy this artifact publicly" in chat triggers the same
  deploy-web **skill workflow** in codexcrew-core (no custom tool involved) — same backend
  endpoint, same gates.
...
is good at: CodexCrew already has a live agent in the loop, so "installation" doesn't
...
- Generate the cycle-006 scoped IAM policy JSON (S3 + CloudFront only, confined to
  `codexcrew-web-*` + `codexcrew:managed` tag) and show it to the user.
- The **user applies it themselves** (console or their own `aws iam` command) and
  attaches it to their assumable role. CodexCrew does **not** create, attach, or modify
...
- **Verify (read-only reachability only, Q3):** after the user applies it, CodexCrew runs
...
```
>_ Let's get deploy-web connected to your AWS account. I'll never
...
Step 2/3 — Permissions
   ...
   (You apply it — CodexCrew never edits your IAM.)
...
- **Safety boundary intact.** The agent guides, *generates* policy text, and
  *read-only verifies* — it never reads/stores keys and **never performs an IAM write**.
  ...
  themselves; CodexCrew never creates/attaches/modifies roles or policies.
- **No account management.** CodexCrew does not create AWS accounts, IAM users/roles,
  or credentials, and does not rotate or transmit them. All account/IAM mutation is the
  user's own action, outside CodexCrew.
...
- Built as a **Python builtin module** (aws CLI subprocess, not boto3) + UI page +
  thin chat skill (§8) — no per-app update cron (built-in; updated with CodexCrew).
