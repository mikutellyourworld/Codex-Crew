...
from codex_crew.deploy import iam as iam_mod
from codex_crew.deploy import pricing as pricing_mod
from codex_crew.deploy import profiles as profiles_mod
from codex_crew.deploy.render import render_standalone
from codex_crew.deploy.scan import Finding, is_credential_finding, scan_content, summarize
...
from codex_crew.validation import FieldSpec, ValidationError, validate_field
...
def _deny_restricted(request: "web.Request", operation: str) -> "web.Response | None":
    ...
    """
    from codex_crew.dashboard.handlers._shared import _is_restricted_session
...
def _reaper_remediation(profile: str, region: str) -> str:
    ...
    (the stack creates an IAM role, which CodexCrew never does itself).
    """
    ...
    if not roots:
        for cand in (Path.home() / "workplace", Path.home() / "workspace",
            ...
            try:
                ...
                pass
    # Always allow the agent's own config-dir workspace (e.g. ~/.codex-crew/workspace).
    try:
...
async def _do_deploy(params: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    ...
    if os.name == "nt":
        return 400, {
            ...
            "Use WSL (Windows Subsystem for Linux) to run the Codex Crew gateway on Windows."
        }
    try:
    ...
    try:
        ...
        try:
            base_rc, base_out, _ = await asyncio.to_thread(
                ...
                ["cloudformation", "describe-stacks",
                 "--stack-name", "codexcrew-deploy-base",
        ...
        if not manifest_bucket and ttl_hours != 0:
            ...
            return 409, {
                "error": (
                    ...
                    "(codexcrew-deploy-base). Use ttl_hours=0 for persistent "
        ...
        if ttl_hours != 0:
            try:
                reaper_rc, _, _ = await asyncio.to_thread(
                    engine.run_aws,
                    ["cloudformation", "describe-stacks",
                     "--stack-name", "codexcrew-deploy-reaper",
            ...
            if reaper_rc != 0:
                return 409, {
                    "error": (
                        ...
                        "(codexcrew-deploy-reaper). Install the reaper "
        ...
        # Write the .codexcrew-deploy.json manifest (same shape that deploy.sh
                ...
                def _write_deploy_manifest() -> int:
                    ...
                    try:
                        ...
                        rc, _, _ = engine.run_aws(
                            ["s3", "cp", f.name,
                             f"s3://{target_bucket}/{site_id}/.codexcrew-deploy.json",
...
def _check_reaper_installed(profile: str, region: str) -> bool:
    """Check if the codexcrew-deploy-reaper stack exists in the account.
...
async def _expire_manifest_best_effort(art: Any) -> str:
    ...
    try:
        rc_m, out_m, _ = await asyncio.to_thread(
            engine.run_aws,
            ["s3", "cp", f"s3://{bucket}/{slug}/.codexcrew-deploy.json", "-",
             "--region", region],
            profile, 15,
        )
        if rc_m == 0 and out_m:
    ...
    try:
        ...
        rc, _, err = await asyncio.to_thread(
            engine.run_aws,
            ["s3", "cp", tmp_path,
             f"s3://{bucket}/{slug}/.codexcrew-deploy.json",
             "--content-type", "application/json",
             "--region", region],
            profile, 15,
        )
...
async def _handle_teardown(request: web.Request) -> web.Response:
    ...
    from codex_crew.dashboard.handlers.artifacts import _serialize
...
async def _handle_pending_list(request: web.Request) -> web.Response:
    ...
    """
    from codex_crew.deploy.pending import list_pending
...
async def _handle_pending_confirm(request: web.Request) -> web.Response:
    ...
    from codex_crew.deploy.pending import add_pending, claim_pending
...
async def _handle_pending_dismiss(request: web.Request) -> web.Response:
    ...
    if denied:
        return denied
    entry_id = request.match_info["id"]
    from codex_crew.deploy.pending import remove_pending
