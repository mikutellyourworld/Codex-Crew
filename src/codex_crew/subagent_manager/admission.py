...
class SpawnAdmissionCoordinator(ManagerComponent):
    ...
    def spawn_impl(
        ...
        Args:
            ...
            agent (str): Agent name override (default: "codexcrew").
        ...
        try:
            min_mem = CodexCrewConfig.load().agent.spawn_min_memory_gb
        except Exception:
            min_mem = 4.0
        ...
        if cwd:
            try:
                allowed_roots = CodexCrewConfig.load().agent.subagent_cwd_allowed_roots
            except Exception:
    ...
    async def _spawn_with_approval_impl(self, info: SubagentInfo) -> None:
        ...
        try:
            ...
            # ``codexcrew logs`` held no record keyed to the affected run id —
    ...
    def _log_spawned_impl(self, info: SubagentInfo) -> None:
        ...
        try:
            from codex_crew.metrics.events import SUBAGENTS_SPAWNED, emit_counter
