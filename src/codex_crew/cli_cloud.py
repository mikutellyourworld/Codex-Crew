"""CLI ``codexcrew cloud`` command group — thin dispatchers into :mod:`cloud`.
...
from codex_crew.cloud import ec2, iam
from codex_crew.cloud import login as login_mod
from codex_crew.cloud import sizes, ssm, ui, wizard
from codex_crew.cloud.aws import AWSError, CloudActionDenied
from codex_crew.cloud.config import DEFAULT_REGION, CloudConfig
from codex_crew.deploy.engine import resolve_aws_bin
...
def _resolve_tag(args: argparse.Namespace) -> str:
    ...
    if not cfg.last_tag:
        ...
        ui.detail("Pass --tag <tag>, or run `codexcrew cloud list` to see instances.")
        sys.exit(1)
    return cfg.last_tag
...
def _cloud_list(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        return 1
    if not rows:
        ui.info("No CodexCrew cloud instances found.")
        ui.detail("Launch one with: codexcrew cloud launch")
        return 0
    ui.note(f"{ui.BOLD}CodexCrew cloud instances ({region}):{ui.RESET}")
    for r in rows:
...
def _cloud_connect(args: argparse.Namespace) -> int:
    ...
    if conn.ready and conn.url:
        if not conn.token:
            ...
            ui.detail("The page will ask for a token. Retry: codexcrew cloud connect")
...
def _cloud_login(args: argparse.Namespace) -> int:
    ...
    if signed:
        ui.ok(
            ...
            "was already open: codexcrew cloud connect"
        )
        return 0
    ui.warn(
...
def _cloud_logout(args: argparse.Namespace) -> int:
    ...
    if not signed_out:
        ...
        ui.detail("The session may still be active — retry, or check with: codexcrew cloud connect")
        return 1
    ...
    ui.detail("Sign in with another account: codexcrew cloud login")
    return 0
...
def _cloud_stop(args: argparse.Namespace) -> int:
    ...
    ui.detail("Resume with: codexcrew cloud start")
    return 0
...
def _cloud_start(args: argparse.Namespace) -> int:
    ...
    ui.detail("Reopen the dashboard with: codexcrew cloud connect")
    return 0
...
def _cloud_destroy(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "yes", False):
        if not ui.confirm(f"Remove CodexCrew instance '{tag}' from AWS?", default=False):
    ...
    if not res.get("destroyed"):
        ...
        ui.detail("Check `codexcrew cloud status` (and the AWS console); re-run destroy if needed.")
        return 1
    ...
    from codex_crew.cloud import source as source_mod
...
def _cloud_doctor(args: argparse.Namespace) -> int:
    ...
    profile, region = _resolve(args)
    ui.note(f"{ui.BOLD}CodexCrew cloud — diagnostics{ui.RESET}")
    ...
    # what `codexcrew cloud` commands actually run under a GUI-launched
...
def handle_cloud(args: argparse.Namespace) -> int:
    """Entry point for ``codexcrew cloud <action>``."""
    action = getattr(args, "cloud_action", None)
    if not action:
        ui.note(f"{ui.BOLD}codexcrew cloud{ui.RESET} — run CodexCrew on your own AWS EC2")
        print()
        ui.detail("launch      Provision + configure an instance (interactive)")
        ui.detail("list        List your CodexCrew cloud instances")
