...
from codex_crew import identity_stores, platform_compat
...
def api_key_configured(environ: Mapping[str, str] | None = None) -> bool:
    ...
    credential loader lifted out of Codex Crew's ``.env``.
    """
...
def signed_in_via_idc(
    ...
    from codex_crew.hooks import emit_internal_read_audit
...
def find_kiro_cli_candidates(
    ...
    *,
    ...
    candidates: list[str] = []
    override = environ.get("CODEXCREW_KIRO_BIN", "")
    if override:
...
def resolve_kiro_cli(
    *,
    ...
    explicit ``CODEXCREW_KIRO_BIN`` override, which is deliberately still
