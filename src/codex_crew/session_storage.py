...
Codex Crew and kiro-cli each own:
...
from codex_crew import hooks, pinned_fs, platform_compat
from codex_crew.atomic_write import atomic_write, fsync_dir, replace_with_retry
...
from codex_crew.history import ARCHIVE_DIR_NAME, ARCHIVE_SEGMENT_DELIMITER, SESSIONS_DIR_NAME
from codex_crew.session_map import SESSION_MAP_FILENAME

logger = logging.getLogger(__name__)

# Trash lives under the data home (not beside kiro-cli's store) because it holds
# Codex Crew's own staged deletions: it must survive a kiro-cli upgrade, and
...
class SessionIndex:
    ...
    *stem_to_sid* maps a transcript filename stem to the kiro-cli session id it
    ...
    :func:`codex_crew.history.transcript_stems`, never by re-deriving the naming
...
def _crew_sessions_dir() -> Path:
    """Codex Crew's own transcript directory."""
...
def _pod_root() -> Path:
    raw = os.environ.get("CODEXCREW_POD_ROOT")
    return Path(raw).expanduser() if raw else Path.home() / ".codexcrew-pods"
...
def _replay_store_cotenants() -> list[str]:
    ...
    ``CODEXCREW_HOME`` cannot be, and stays a documented limitation.
    """
    try:
...
def _cotenant_key() -> tuple[object, ...]:
    ...
    of :func:`_scan_key`: it is resolved per call (``CODEXCREW_POD_ROOT`` overrides
    ...
    sensitive-path gate re-anchors on ``CODEXCREW_HOME`` / ``KIRO_HOME`` — the
    ...
    two, the read tier re-anchors on ``CODEXCREW_HOME``; ``KIRO_HOME`` reaches
...
def _has_own_replay_store(pod_home: Path) -> bool:
    ...
    """
    return (pod_home / CONFIG_DIR_NAME.lstrip(".")).is_dir()
...
def reclaim_block_reason(*, cached: bool = False) -> str:
    ...
    ``KIRO_HOME=/etc`` alongside an isolated ``CODEXCREW_HOME`` leaves this process
    ...
    *cached* permits reusing a recent pass over co-tenant pod mappings,
    ...
    defaults = {home / CONFIG_DIR_NAME, _norm(legacy_home())}
    ...
    if data in defaults:
        ...
        if _norm(kiro_home()) == home / CONFIG_DIR_NAME:
            ...
            if refusals:
                ...
                return (
                    ...
                    "`codexcrew pod down <name>` to reclaim from here."
                )
        return ""
    ...
    return (
        ...
        "one cannot see. Point KIRO_HOME inside CODEXCREW_HOME to reclaim from here."
    )
...
def _remove_emptied_batch(batch: Path, what: str, *, expect: tuple[int, int] | None) -> bool:
    ...
    :func:`codex_crew.pinned_fs.remove_tree_pinned` is the same mechanism the delete path
...
#:
#: The pinned-walk half is asked of :mod:`codex_crew.pinned_fs` rather than restated here.
...
#: approval - so they now live in :mod:`codex_crew.pinned_fs` with the rest of the
...
def _open_absolute_nofollow(path: Path) -> tuple[int, int]:
    ...
    The walk itself is :func:`codex_crew.pinned_fs.pin_parent` - one ``openat`` per
...
def _remove_scanned_dirs(
    ...
    :func:`codex_crew.pinned_fs.remove_dir_verified`, which is where the batch directory and
...
def _remove_pinned_batch(parent_fd: int, batch_fd: int, name: str) -> None:
    ...
    :func:`codex_crew.pinned_fs.remove_dir_verified`, the same primitive the interior
