...
numpy on the import path of ``codex_crew.cli``, which ``test_cli_lazy_imports``
correctly refuses: a ``codexcrew --help`` has no business loading an array
library.
...
#: that cuts them off. Enforced in :mod:`codex_crew.stt.vad` and clamped to by
...
#: Floor on the partial cadence, enforced in :mod:`codex_crew.stt.session`.
