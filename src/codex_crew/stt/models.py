...
no edit in ``security``: the pattern covers any ``ggml-*.bin``. Codex Crew's own downloader writes here directly and does not route
...
the doctrine and the ``CODEXCREW_SKIP_MODEL_DOWNLOAD`` escape hatch are mirrored
...
#: GGUF, Codex Crew serves no mirror of these, so the pin below is doing the whole
...
MODEL_URL_ENV = "CODEXCREW_WHISPER_MODEL_BASE_URL"
...
SKIP_DOWNLOAD_ENV = "CODEXCREW_SKIP_MODEL_DOWNLOAD"
...
def models_dir() -> Path:
    """Where whisper weights live. Respects ``CODEXCREW_HOME``.
...
def stream_pinned_payload(
    ...
    *,
    ...
    (:mod:`codex_crew.stt.decoder`), because every property below is a property of
...
def _download_blocking(
    ...
    #   server and a `codexcrew` CLI each run their own store over the same data home --
