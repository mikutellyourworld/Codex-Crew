...
- :mod:`codex_crew.stt.limits`: the numbers configuration can set, and nothing else.
- :mod:`codex_crew.stt.models`: the sha256-pinned model catalog and its downloader.
- :mod:`codex_crew.stt.hallucinations`: suppression of the recogniser's own artefacts.
- :mod:`codex_crew.stt.vad`: voice-activity detection and utterance endpointing.
- :mod:`codex_crew.stt.engine`: the resident model, and the lock discipline around it.
- :mod:`codex_crew.stt.session`: a live session turning PCM into partials and a final.

The public surface resolves lazily through :pep:`562` ``__getattr__``, for the
same reason :mod:`codex_crew.config` does it. ``vad``, ``session`` and ``engine``
...
``config.loader`` reading a field default and therefore ``codex_crew.cli`` printing
``--help``. ``test_cli_lazy_imports`` guards exactly that. ``from codex_crew.stt
...
(``from codex_crew.stt import models as stt_models``). Mixing the two is the bug,
...
if TYPE_CHECKING:
    from codex_crew.stt.engine import (
        CODE_DECODE_FAILED,
        CODE_EXTRA_MISSING,
        CODE_IMPORT_FAILED,
        CODE_MODEL_MISSING,
    ...
    from codex_crew.stt.models import (
        ModelStore,
        WhisperModel,
        is_present,
        models_dir,
    )
    from codex_crew.stt.session import (
        DECODE_FAILED_ADVISORY,
        KIND_ERROR,
        KIND_FINAL,
        KIND_PARTIAL,
    ...
    from codex_crew.stt.vad import SAMPLE_RATE_HZ


def availability() -> Availability:
    """Whether local recognition can run on this host, and if not, why.

    A thin alias for :func:`codex_crew.stt.engine.probe` so callers outside the
    ...
    :func:`codex_crew.stt.models.is_present`, because a missing model is a
    condition that resolves itself on first use.
    """
    from codex_crew.stt.engine import probe as _probe
...
def resolve_model(name: str) -> WhisperModel:
    ...
    from codex_crew.stt.models import resolve as _resolve
...
def model_store() -> ModelStore:
    ...
    from codex_crew.stt.models import store as _store
