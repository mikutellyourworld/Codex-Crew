...
**The trust anchor is the digest, not the path.** :mod:`codex_crew.transcribe`
...
def store_dir() -> Path:
    """Where a fetched decoder lives. Respects ``CODEXCREW_HOME``.
...
def _extract_pinned_member(archive: Path, artifact: DecoderArtifact, target_dir: Path) -> Path:
    ...
    place. Raises :class:`~codex_crew.stt.models.ModelDownloadError` on anything
