...
from codex_crew.acp._dispatch import is_shell_kind
from codex_crew.acp.client import AcpError, AcpTimeoutError
...
from codex_crew.constants import BANNER, DATA_WARNING
...
#: the same value so a task-bound ``codexcrew`` profile cannot be skipped merely
#: because the CLI relied on the provider's default.
_DEFAULT_KIRO_AGENT = "codexcrew"
...
class _ToolGate:
    """Codex Crew's own PreToolUse gate, plus the identity it is asked under.
...
def _build_tool_gate(agent: str = "") -> _ToolGate:
    ...
    """
    return _ToolGate(
        hooks=HookManager(hooks_config_from_config_dict(CodexCrewConfig.load().hooks)),
        agent=agent,
    )
...
async def _chat(message: str | None, model: str | None, agent: str | None = None) -> None:
    ...
    channel_id = os.environ.get("CODEXCREW_CHANNEL_ID") or None
    # Keep the provider and governance gate on one canonical identity.  ACP
    # resolves an omitted agent to ``codexcrew``; passing an empty string only to
...
async def _read_line(prompt: str) -> str:
    ...
    threading.Thread(target=_worker, daemon=True, name="codexcrew-permission-prompt").start()
    try:
        return await future
...
async def _answer_permission(
    ...
    *,
    ...
    Answering one is an authorization decision, so Codex Crew's own PreToolUse
    gate runs first and a human is asked only about what survives it. That gate
    is the shared :class:`~codex_crew.hooks.HookManager` -- sensitive paths, the
    ...
    if not can_prompt:
        ...
        try:
            ...
            _print_permission_notice(
                ...
                "   Run `codexcrew chat` from a terminal to approve tool calls."
            )
        except Exception:
...
async def _interactive(
    provider: LLMProvider, cfg: CodexCrewConfig, *, gate: _ToolGate | None = None
) -> None:
...
def _ensure_default_agent_in_config() -> None:
    """Ensure config.json includes a default Codex Crew agent for fresh installs.
    ...
    ``codexcrew config set`` write landing between an outside read and this
