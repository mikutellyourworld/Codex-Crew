...
from codex_crew.embeddings import make_sync_embed_fn
from codex_crew.frontmatter import ONBOARDING_IMPORT, parse_block_scalar_header, split_frontmatter
from codex_crew.hooks import FileTooLargeError, safe_read_file_bytes_nolink
from codex_crew.learn import _MAX_LESSONS_TOTAL, Lesson, LessonStore
...
# Directories that exist under the shared home but have no Codex Crew
...
def _add_instruction_files(
    ...
    """Project user-authored instruction documents onto CodexCrew's memory tiers.
    ...
    deliberately NOT imported: CodexCrew's persona surface is theme-pack persona,
...
def _column0_activation_declared(text: str) -> bool:
    ...
    :func:`~codex_crew.frontmatter.parse_block_scalar_header` rather than
...
def _normalized_gemini_spec(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    * drop ``env`` only when it is empty
    ...
    being reshaped into something Codex Crew cannot actually run.
    """
...
def _scan_lineage_install(scan: _Scan) -> None:
    """Scan an agent that shares Codex Crew's OWN on-disk layout.
...
def _sqlite_snapshot(
    ...
    snapshot_dir = Path(tempfile.mkdtemp(prefix="codexcrew-import-sqlite-"))
    try:
...
def _row_is_workspace_scoped(value: Any) -> bool:
    ...
    CodexCrew's own memory tables have no workspace column, so a genuinely
...
def _managed_mcp_names() -> frozenset[str]:
    """MCP server names owned by Codex Crew or by a known foreign agent, casefolded.
...
def _mcp_lock(_path: Path) -> Iterator[None]:
    """Coordinate with dashboard and app writers of the CodexCrew MCP file."""
    ...
    from codex_crew.dashboard.handlers.mcp import _get_mcp_lock_sync
...
def _write_mcp(
    ...
    *,
    ...
    with _mcp_lock(path):
        ...
        from codex_crew.mcp_discovery import configured_mcp_aliases
