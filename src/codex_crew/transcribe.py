...
:mod:`codex_crew.stt` holds loaded in this process. Keeping the model resident is
...
binary and it works on every OS Codex Crew supports.
...
- ``apple``: Apple's on-device SpeechAnalyzer (macOS 26+), which downloads no
  model because the OS ships the assets. Owned by :mod:`codex_crew.apple_speech`.
- ``transcribe``: AWS Transcribe Streaming, a paid service, gated on the recorded
  operator consent in :mod:`codex_crew.aws_consent`.
...
digest-verified store :mod:`codex_crew.stt.decoder` fetches the same pinned upstream
...
from codex_crew import aws_consent, platform_compat, stt
...
# Re-exported: the hallucination filter lives in codex_crew.stt.hallucinations so
...
from codex_crew.stt.hallucinations import filter_hallucinations  # noqa: F401
...
# Apple team identifier of the Developer ID certificate that signs Codex Crew
...
_FFMPEG_SNAPSHOT_PREFIX = ".codexcrew-ffmpeg-"
_FFMPEG_SNAPSHOT_NAME_RE = re.compile(r"^\.codexcrew-ffmpeg-(\d+)-[A-Za-z0-9_-]+$")
...
def _ffmpeg_snapshot_root() -> str:
    ...
    from codex_crew.sandbox import prime_voice_runtime_sandbox_paths
...
def _new_executable_snapshot() -> tuple[int, int, bool, str | None]:
    ...
    if platform_compat.IS_LINUX and hasattr(os, "memfd_create"):
        ...
        try:
            descriptor = os.memfd_create("codexcrew-ffmpeg", flags | getattr(os, "MFD_EXEC", 0x0010))
        except OSError as exc:
            if exc.errno != errno.EINVAL:
                raise
            descriptor = os.memfd_create("codexcrew-ffmpeg", flags)
...
# The adapted providers answer in :class:`codex_crew.stt.Availability`, the same
...
# :mod:`codex_crew.stt.engine`. They travel to the browser in JSON, so renaming
...
def availability_detail(stt_config=None) -> stt.Availability:  # type: ignore[no-untyped-def]
    ...
    if stt_config is None:
        ...
        stt_config = CodexCrewConfig.load().stt
...
async def _pcm_via_ffmpeg(audio_path: str, timeout_secs: int) -> np.ndarray | None:
    ...
    if not ffmpeg_bin:
        logger.error(
            "the audio decoder is unavailable for %s; reinstall the Codex Crew "
...
async def _transcribe_apple(audio_path: str, stt_config) -> str | None:  # type: ignore[no-untyped-def]
    ...
    Delegates to :mod:`codex_crew.apple_speech`, which owns the Swift-helper seam.
