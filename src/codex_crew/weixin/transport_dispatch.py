...
never imports ``codex_crew.slack``.
...
from codex_crew.messaging.commands import compact_unsupported_backend
...
from codex_crew.messaging.link import build_dm_session_key, seed_generation
from codex_crew.messaging.transport import InboundMessage
from codex_crew.safety_override import safety_override
from codex_crew.weixin.attachments import process_weixin_attachments
from codex_crew.weixin.commands import ConversationState, build_help, parse_command
...
if TYPE_CHECKING:
    ...
    from codex_crew.weixin.client import ContextTokenStore, TypingTicketCache, WeixinClient

logger = logging.getLogger(__name__)

# Canonical kiro-cli agent fallback so Weixin sessions load codexcrew-core
...
class WeixinDispatcher:
    ...
    async def _drive(self, inbound: InboundMessage, user_id: str, text: str) -> None:
        ...
        await drive_turn(
            ChannelTurn(
                ...
                # grant the dashboard toggle and Slack's `/codexcrew yolo` drive,
    ...
    def _persist_turn(
        ...
        :func:`codex_crew.history.mint_row_mid` for why the identity has to be
