...
DELIBERATELY SEPARATE FROM ``codex_crew.metrics`` (the OTEL trunk). Four reasons,
each independently disqualifying:
  ...
  1. ``MetricsRecorder._guard()`` runs EVERY attribute through
     ...
     metric.
  2. OTLP egress lives in the ``codexcrew[otlp]`` package extra, NOT the default
...
``CODEXCREW_TELEMETRY_DISABLED`` is truthy, ``telemetry.beacon_enabled`` is
false, the process looks like CI, or ``CODEXCREW_HOME`` is non-default (dev
...
from codex_crew.config.paths import CONFIG_DIR_NAME, config_dir
...
# Opt-out env var. Truthy disables; mirrors the CODEXCREW_TELEMETRY convention
# in metrics/provider.py so operators only learn one spelling.
DISABLE_ENV = "CODEXCREW_TELEMETRY_DISABLED"
...
# install cannot change. ``CODEXCREW_DISTRIBUTION`` is inherited by every child
...
DIST_ENV = "CODEXCREW_DISTRIBUTION"
...
def is_env_opted_out() -> bool:
    """Return whether ``CODEXCREW_TELEMETRY_DISABLED`` pins the beacon off.
...
def is_default_home() -> bool:
    ...
    A non-default ``CODEXCREW_HOME`` means a dev home (``.codexcrew-dev``), a pod,
    or a worktree preview — one operator's own extra instances, so counting them
    would inflate DAU.

    Compared against ``~/.codex-crew`` directly rather than against
    ``config_dir()``: ``config_dir()`` *honors* ``CODEXCREW_HOME``, so comparing
    ...
    """
    raw = os.environ.get("CODEXCREW_HOME", "").strip()
    if not raw:
        return True
    try:
        default = Path.home() / CONFIG_DIR_NAME
...
def install_id(*, create: bool = True) -> str:
    ...
    ``codexcrew telemetry status`` so merely inspecting status cannot
    ...
    try:
        ...
        # a caller inspecting state (``codexcrew telemetry status``) needs "the
...
def telemetry_permitted(
    *, enabled: bool, acked: bool, audit_tool: str = ""
    ...
    chapter, or an explicit ``codexcrew telemetry enable|disable``). It gates the
    ...
    if not is_default_home():
        return Verdict(
            False,
            "non-default CODEXCREW_HOME (dev home / pod / preview)",
            "non_default_home",
        )
...
def status(
    ...
    """Return the exact state for ``codexcrew telemetry status``.
    ...
    (which the Privacy panel refetches) and from ``codexcrew telemetry status``.
