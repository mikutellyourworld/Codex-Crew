...
``CODEXCREW_READY``. ``warn_if_wecom_uncredentialed`` remains as the
...
from codex_crew.messaging.driver import APPROVAL_AUTO, APPROVAL_INTERACTIVE
...
def warn_if_channel_uncredentialed(
    ...
    ``CODEXCREW_READY``, off the boot-path window), once per collapsed-flag
    ...
    channel_logger = logging.getLogger(f"codex_crew.{channel_type}.gateway")
