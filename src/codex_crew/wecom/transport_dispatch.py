...
if TYPE_CHECKING:
    ...
    from codex_crew.wecom.client import WeComClient, WeComInbound

logger = logging.getLogger(__name__)

# Canonical kiro-cli agent fallback so WeCom sessions load codexcrew-core
...
# Telegram paths' _DEFAULT_CODEXCREW_AGENT.
...
class WeComDispatcher:
    ...
    async def handle_message(self, inbound: "WeComInbound") -> None:
        ...
        # configured default, else the canonical "codexcrew" agent -- so the
        # session loads codexcrew-core (spawn_run) instead of kiro-cli's bare
