"""``codexcrew bench`` -- external memory benchmarks (LongMemEval, LoCoMo).

A separate command from ``codexcrew eval`` because they answer different questions.
...
measurement lives in :mod:`codex_crew.eval.bench`.
"""
...
from codex_crew import cli_help
...
def register_bench_parser(sub: argparse._SubParsersAction) -> None:
    """Wire ``codexcrew bench`` into the top-level parser."""
    parser = cli_help.add_command(
        sub,
        "bench",
        description=(
            "Measures the Codex Crew memory layer against published benchmarks. The "
...
def bench_cmd(args: argparse.Namespace) -> int:
    ...
    """
    from codex_crew.eval.bench.errors import BenchRefusal
...
def _bench_dispatch(args: argparse.Namespace) -> int:
    ...
    if action is None:
        print("usage: codexcrew bench {list,fetch,retrieval,kb-retrieval,compare}")
        return 2

    # Deferred deliberately, and measured. `cli.py` imports this module at module
    # scope, so every `codexcrew` invocation of every subcommand loads it. Hoisting
    ...
    # `codex_crew.vector_memory` plus `sqlite3` into the boot path of commands that
    ...
    from codex_crew.eval.bench import datasets
...
def _load_report(path: str, label: str) -> dict:
    ...
    machine -- an agent can run any CLI command, so ``codexcrew bench compare
    ...
    try:
        ...
        print(
            ...
            "'codexcrew bench retrieval' to regenerate it."
        )
        raise _BenchError(1) from None
...
def _compare(args: argparse.Namespace) -> int:
    from codex_crew.eval.bench import compare_reports
...
def _load_corpus(key: str):  # noqa: ANN202 - Corpus, but imported lazily
    ...
    from codex_crew.eval.bench import datasets
    from codex_crew.eval.bench.adapters import load_locomo, load_longmemeval
...
def _retrieval(args: argparse.Namespace) -> int:
    from codex_crew.eval.bench import IngestConfig, RetrievalConfig, run_retrieval, write_report
    from codex_crew.eval.bench.ingest import IngestError
    from codex_crew.eval.bench.retrieval import RetrievalNotMeasurable
    from codex_crew.eval.bench.safepath import UnsafePathError
    ...
    if args.toy_embedder:
        from codex_crew.eval.bench.toy_embedder import TOY_EMBEDDER_ID, toy_embed_fn
    ...
    from codex_crew.eval.bench import format_report
...
def _kb_retrieval(args: argparse.Namespace) -> int:
    ...
    path of every unrelated ``codexcrew`` subcommand -- the perf regression
    ``test/test_perf_boot_path.py`` pins.
    """
    from codex_crew.eval.bench.kb_retrieval import (
        KBGoldenSet,
        KBGoldenSetError,
    ...
    if args.real_embedder:
        from codex_crew.knowledge.embedder import InProcessEmbedder
