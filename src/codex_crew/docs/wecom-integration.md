...
Talk to your Codex Crew agent from WeCom — through a WeCom (企业微信) AI bot. Create
the bot in your WeCom console, drop in two values, and you're chatting. Replies
stream back live.

> **WeChat vs. WeCom.** Codex Crew connects through **WeCom (企业微信)**, the work
> edition, using its AI-bot API. It does **not** sign in to a personal WeChat
> account — people message the bot from inside WeCom.

Like Telegram, the connection is outbound-only: Codex Crew opens a secure
...
Codex Crew tells you where to create the WeCom AI bot, then writes your Bot ID and
Secret into `~/.codex-crew/.env` and `config.json` and restarts the gateway for
...
You'll need a running gateway (`codexcrew gateway`) and admin access to your
WeCom console.
...
3. **Save the credentials** to `~/.codex-crew/.env`:
   ```
...
Those two values — **Bot ID** and **Secret** — are all Codex Crew needs. There's
...
- Authorized senders: the **owner** (`CODEXCREW_OWNER_ID`) plus anyone listed in
...
Credentials go in `~/.codex-crew/.env`: `WECOM_BOT_ID` and `WECOM_SECRET`.
