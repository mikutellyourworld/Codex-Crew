...
Codex Crew can spawn background subagents to handle tasks in parallel. This is
...
Codex Crew uses the `spawn_run` MCP tool to create subagents.
...
1. Codex Crew spawns one or more subagent processes
...
4. Codex Crew synthesizes the results into a final response
...
- **Timeout**: 30 minutes per subagent task (`agent.subagent_timeout_secs`), 20 minutes delivery (semaphore wait + injection), 15 minutes per injection attempt (`CODEXCREW_INJECTION_TIMEOUT`, clamped to the delivery cap)
...
The full transcript lives at `~/.codex-crew/subagents/<id>/result.txt` and is
...
Set via `codexcrew config set agent.completion_keep tail` or by editing
`~/.codex-crew/config.json` directly.
