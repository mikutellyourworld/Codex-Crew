...
Codex Crew can run tasks on a schedule — recurring checks, daily briefings, periodic monitoring, or one-shot reminders.
...
Codex Crew uses the `cron_add` MCP tool to create the job.
...
```bash
codexcrew cron add "pipeline-check" "check pipeline health" --every 1800
codexcrew cron add "weekday-9am" "check tickets" --cron "0 9 * * MON-FRI" --approval-mode auto
codexcrew cron update <job-id> --name "new name" --message "new prompt" --approval-mode auto
codexcrew cron list
codexcrew cron remove <id>
```
...
- **Script cron**: set `script` to `~/.codex-crew/crons/file.py:function`; it bypasses the LLM and cannot be combined with `command`.
- **Command cron**: set `command` to a shell command; it bypasses the LLM and cannot be combined with `script`.

For script crons, `message` is available as `ctx.message`. Import the context helpers and flow-control exceptions from `codex_crew.cron_script`:

```python
from codex_crew.cron_script import Done, Report, Skip
...
```bash
codexcrew cron preview ~/.codex-crew/crons/my.py:run --message "arguments for ctx.message"
```
...
```bash
codexcrew cron add "auto-check" "check pipeline" --every 300 --approval-mode auto
```
...
To avoid traffic spikes from thousands of users' jobs all firing at the same instant, Codex Crew adds a small random delay before executing scheduled jobs:
...
The CLI has no flag for this. `codexcrew cron add` leaves `strict_schedule` at its default of `false`, so a job created there takes the jitter for its frequency; use MCP or the dashboard when you need exact firing.

## Managing Jobs

| Action | Dashboard | Slack | CLI |
|--------|-----------|-------|-----|
| List | Schedule page | `cron list` | `codexcrew cron list` |
| Pause | Pause button | `cron pause <id>` | `codexcrew cron pause <id>` |
| Resume | Resume button | `cron resume <id>` | `codexcrew cron resume <id>` |
| Delete | Delete button | `cron remove <id>` | `codexcrew cron remove <id>` |
| Adopt / release | — | — | `codexcrew cron adopt <id> --session-of <slot>` / `--release` |

### Which chat session owns a job

A job's `session_key` names the chat session it belongs to, and that one field carries one meaning: it is also where the job's output is delivered (`session="origin"` sends and script results both resolve a slot from it). A job created outside a chat — `codexcrew cron add`, the dashboard Schedule page, an onboarding import — therefore has no owning session, which is truthful rather than a defect: there is no chat to deliver into.

The consequence is worth stating plainly, because it is easy to read as a job that disappeared. A job with no owning session is outside every chat session's scope: `cron_list` from chat does not list it, and the mutating cron tools answer a deliberately vague `job not found` so the reply cannot be used to enumerate jobs the caller may not see. Those jobs are managed from the operator surfaces instead: `codexcrew cron list`, which is the only surface that shows *who* owns a job, and the dashboard Schedule page, which lists and manages every job regardless of owner but does not yet display ownership (its API payload does not carry `session_key`).

`codexcrew cron adopt` is the way across that line, in both directions:

```
codexcrew cron adopt <id> --session-of chat-3-1712793600   # hand it to a session
codexcrew cron adopt <id> --session-of dashboard:chat-3-1712793600
codexcrew cron adopt <id> --release                        # back to operator-only
```
