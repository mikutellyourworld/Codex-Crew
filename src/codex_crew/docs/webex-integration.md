...
Chat with your Codex Crew agent from Cisco Webex — on your phone, your laptop,
anywhere. Create a bot on the Webex developer portal, paste one token, and
you're talking.

Webex needs no public URL and no webhooks: Codex Crew registers a device with
...
and your email into `~/.codex-crew/.env` and `config.json` and restarts the
...
You'll need a running gateway (`codexcrew gateway`) and a Webex account.
...
2. **Save the token** to `~/.codex-crew/.env`:
   ```
   WEBEX_BOT_TOKEN=YmFzZTY0…
   ```
3. **Turn it on** in `~/.codex-crew/config.json` — your own Webex account email
   is the allow-list:
   ```json
...
| `/codexcrew dashboard [2h]` | Get a dashboard login link (**DM only**) |
...
- **`allowed_emails` IS the trust boundary — every entry is an operator.** There
  ...
  toggle and CLI drive the same grant) and `/codexcrew dashboard` (mints a
  presigned dashboard login for YOUR Codex Crew, DM-only). So add a second address
