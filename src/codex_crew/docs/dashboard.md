...
The dashboard is a React SPA at `http://localhost:5486` (or your configured URL). It provides chat, configuration, scheduled jobs, agent capabilities, and system/developer views.
...
- **SSH tunnel**: `ssh -NL 5486:localhost:5486 <host>` then open localhost:5486.
...
- **Custom domain**: after `codexcrew setup`, optionally use `http://codexcrew.localhost:5486`.
...
1. Confirm the gateway is running: `codexcrew status`.
2. Test the API: `curl http://localhost:5486/api/status`.
3. Check for port conflicts: `lsof -i :5486`.
4. On remote dev desktops, you must use an SSH tunnel — the dashboard binds to localhost by default.
5. Run `codexcrew gateway -vv` for debug output.
