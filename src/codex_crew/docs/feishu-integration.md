...
Talk to your Codex Crew agent from Feishu (Lark / 飞书) — through a Feishu custom
app bot. Create the app in the Feishu developer console, drop in two values, and
you're chatting.

Like Telegram and WeCom, the connection is outbound-only: Codex Crew opens a
...
You'll need a running gateway (`codexcrew gateway`), `lark-oapi` installed, and
...
7. **Paste it into Settings** — open **Settings → Channels → Feishu**, put the
   ...
   `~/.codex-crew/.env` (owner-only, `0600`) and the rest to `config.json`, so
...
- **Every denial is audited.** Rejected inbound messages write a SEL audit
  ...
  `codexcrew security posture` rather than vanishing.
