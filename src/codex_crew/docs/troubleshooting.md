...
When something is wrong, start with `codexcrew doctor` rather than guessing: it
...
`codexcrew doctor` reports the binary and the login state on separate lines, so
check both.
...
relaunch — see the
[macOS troubleshooting guide](https://github.com/mikutellyourworld/Codex-Crew/blob/main/docs/guides/macos-troubleshooting.md)
...
than `kiro-cli login`. `codexcrew doctor` prints a signed-in state while the
...
codexcrew restart           # or restart however you run the gateway
```
...
key in `~/.codex-crew/.env` is loaded into the gateway's environment at startup;
...
`/etc/codexcrew/codexcrew.env` — both are readable by any local user.
`codexcrew service install` warns when it sees a key in your shell that the
...
This regenerates `~/.kiro/agents/codexcrew.json` while preserving your own
customizations in it.

### MCP tools not working

`codexcrew doctor` auto-appends missing `tools` / `allowedTools` entries for the
...
1. Check `~/.kiro/agents/codexcrew.json` for `codexcrew-core`, `codexcrew-cron`,
   and `codexcrew-computer` under `mcpServers`, and for the matching `@`-prefixed
   entries under `tools`
2. Check `~/.kiro/settings/mcp.json` for globally configured servers
3. Re-run `codexcrew setup --agent-only`
...
Your administrator also has to add `codexcrew-core`, `codexcrew-cron` and
`codexcrew-computer` to the registry under those exact names. `codexcrew doctor`
...
Codex Crew never holds it. The dashboard's status probe therefore connects without a
...
- If tool calls fail, use the server in chat once. The Kiro CLI starts the OAuth
  flow on the `401` and Codex Crew shows the consent link as a banner; approve it
...
```bash
codexcrew status                          # is the gateway running?
curl http://localhost:5486/api/status    # is it answering on the expected port?
```

If the port is taken by something else, either stop that process or run Codex Crew
on another port with `CODEXCREW_PORT`.

### Slack not responding

- Verify `~/.codex-crew/.env` has current `SLACK_APP_TOKEN` and
  `SLACK_BOT_TOKEN` values
- Check that `CODEXCREW_OWNER_ID` is your user ID **in the workspace where the
...
- Run `codexcrew gateway -vv` for debug output

### Context window filling up

Codex Crew auto-compacts at `session.autocompact_pct` context usage (70% by
...
`90.0`; check with `codexcrew config get session.autocompact_pct`). If
...
the Codex Crew CDN on gateway startup and is sha256-verified. A failed download
retries with exponential backoff (up to 6 attempts) and again on every gateway
start. If it keeps failing:

- Run `codexcrew doctor`, which probes the resolved model URL and reports
  reachability
- Check outbound HTTPS connectivity. No git or cloud SDK is involved
- Mirrored or airgapped hosts: point `CODEXCREW_EMBED_MODEL_URL` (or
...
- Coming from an install that used Ollama for embeddings? The download is
  usually skipped: Codex Crew finds the identical model in the local Ollama blob
  store and copies it (sha256-verified) instead of re-downloading

### Embeddings not working

- Run `codexcrew doctor`, which checks the bundled embedding runtime and whether
  the model file is present. Embeddings themselves are always on and cannot be
  disabled, so there is no switch to check
- If `CODEXCREW_SKIP_MODEL_DOWNLOAD=1` is set, the model never downloads. Unset
...
Point `memory.embed_model_path` (or `CODEXCREW_EMBED_MODEL_PATH`) at an absolute
...
`CODEXCREW_EMBED_MODEL_PATH` is set, the dashboard refuses to change the model,
...
```bash
codexcrew config set agent.completion_keep tail        # keep the conclusion
codexcrew config set agent.completion_keep_chars 5000  # 0 disables truncation
```

The full transcript lives at `~/.codex-crew/subagents/<agent_id>/result.txt` and
...
```bash
codexcrew config set agent.subagent_result_ttl_secs 21600   # 6 hours
```
...
```bash
codexcrew gateway          # WARNING only (default)
codexcrew gateway -v       # INFO: session lifecycle, context %
codexcrew gateway -vv      # DEBUG: full ACP events, message traces
```
...
Tail a background gateway's output with `codexcrew logs -f`.

## Emergency Recovery

1. Stop the gateway: Ctrl+C, or `codexcrew stop` if it is running detached
2. Check the logs for the actual error: `codexcrew logs -n 200`
3. Reset sessions: delete `~/.codex-crew/session_map.json`
4. Fix or reset config: `codexcrew config edit`, or delete
   `~/.codex-crew/config.json` to fall back to defaults
5. Reconfigure from scratch: `codexcrew setup`
