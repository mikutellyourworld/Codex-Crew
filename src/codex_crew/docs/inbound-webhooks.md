...
This is the inbound counterpart to cron jobs: cron fires on a schedule Codex Crew
owns, a webhook fires when something outside Codex Crew decides it is time.
...
```bash
curl -X POST http://127.0.0.1:5486/api/hooks/agent \
  -H "Origin: http://127.0.0.1:5486" \
  -H "Authorization: Bearer $CODEXCREW_WEBHOOK_TOKEN" \
...
`X-CodexCrew-Token: <token>` is accepted as an equivalent.
...
| `401` | `{"error": "…"}` | Signing failure on a token that requires signatures: `X-CodexCrew-Timestamp` or `X-CodexCrew-Signature` missing, timestamp unparseable or more than 300 seconds from now, digest mismatch, or a signature already seen inside the window (replay). Each cause has its own `error` string. |
...
- A legacy single token set as `hooks.webhook_token` in `config.json` continues
  ...
  the key from the config file — Codex Crew does not rewrite your config for you.
...
```bash
ssh -NL 5486:127.0.0.1:5486 <gateway-host>
```
...
```
X-CodexCrew-Timestamp: 1785372000            # unix seconds, integer
X-CodexCrew-Signature: sha256=<hex hmac>     # hex is case-insensitive
```
...
SIG=$(printf '%s.%s' "$TS" "$BODY" \
  | openssl dgst -sha256 -hmac "$CODEXCREW_WEBHOOK_SIGNING_SECRET" -hex \
  | sed 's/^.* //')

curl -X POST http://127.0.0.1:5486/api/hooks/agent \
  -H "Origin: http://127.0.0.1:5486" \
  -H "Authorization: Bearer $CODEXCREW_WEBHOOK_TOKEN" \
  -H "X-CodexCrew-Timestamp: $TS" \
  -H "X-CodexCrew-Signature: sha256=$SIG" \
  -H "Content-Type: application/json" \
  -d "$BODY"
```
...
Registrations live in `~/.codex-crew/hooks.json`, written under an exclusive lock
...
Webhook URL: http://127.0.0.1:5486/api/hooks/agent
```
...
curl -X POST http://127.0.0.1:5486/api/hooks/agent \
  ...
  -H "Content-Type: application/json" \
  -d '{
        "message": "Analysis complete on PR 123.\nHIGH src/codex_crew/dashboard/handlers/files.py:212 unbounded read before limit check\nMEDIUM test/test_files.py:88 missing negative case",
        "sessionKey": "hook:review:pr-123",
        "name": "Review Bot",
        "timeoutSeconds": 1800
      }'
...
- **Keep the gateway on loopback.** It binds to `127.0.0.1` by default. Reach it
  from elsewhere with an SSH tunnel (`ssh -NL 5486:127.0.0.1:5486 <host>`) rather
...
- Every call is written to the security event log — accepted, rejected for
  capacity, and denied for a bad token — so `codexcrew security events` shows you
