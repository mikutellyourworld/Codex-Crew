...
Codex Crew connects to Slack via Socket Mode. You interact with it through DMs
...
Only the owner (set via `CODEXCREW_OWNER_ID`) can use these:
...
| `!agent off` | Switch back to default codexcrew agent |
...
`!allowlist` and `/codexcrew @user` are not accepted while access is owner-only, and
...
| Command | Description |
|---------|-------------|
| `/codexcrew dashboard` | Same as `!dashboard` |

## Tool Approval Flow

When Codex Crew needs to run a tool (file write, bash command, etc.):
...
When Codex Crew is executing and you need to halt it immediately, type `!stop`
...
When Codex Crew presents choices, they render as interactive Block Kit buttons.
Click a button to send that choice back to the conversation. You can select
multiple options before submitting.

## Access control

Codex Crew on Slack answers the bot owner (`CODEXCREW_OWNER_ID`) and nobody else.
...
limit and no expiry. `!allowlist`, `/codexcrew @user` and `slack.allowed_users` are all
...
When `slack.tracking_channels` is configured, Codex Crew watches for new members
...
Codex Crew connects to Slack as a Socket Mode app that you create and install in
your own workspace.
...
5. **Set credentials** in `~/.codex-crew/.env` (`SLACK_APP_TOKEN`, `SLACK_BOT_TOKEN`, `CODEXCREW_OWNER_ID`). Slack has no `enabled` setting: both Slack tokens opt the channel in.
6. **Slash command** (optional) — the command name is configurable via
   `slack.command` in config.json (default: `codexcrew`). Each app instance
