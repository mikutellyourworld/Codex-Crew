...
Codex Crew can run as a Microsoft Teams bot, so you can DM your agent from Teams
...
> Because Teams uses the Bot Framework model, Codex Crew exposes an **inbound
...
Framework **pushes** activities to a messaging endpoint you host. Codex Crew:
...
- A public HTTPS URL that reaches your gateway. Options:
  ...
  - A dev tunnel for local testing: `devtunnel host -p 5486` (Microsoft Dev
    Tunnels) or `ngrok http 5486`.
...
- The JWT dependency installed (for inbound token validation), in the
  ...
  the form that works on every layout: Codex Crew is not published on PyPI, so
  `pip install "codexcrew[teams]"` cannot resolve, and the direct-URL form below
  reinstalls Codex Crew itself rather than just adding the dependency.

  If the channel is enabled without it, Codex Crew logs an actionable error and
 ...
 >
 > ```bash
 > pip install "codexcrew[teams] @ git+https://github.com/mikutellyourworld/Codex-Crew.git"
 > ```
 > ### Alternative Solutions
 >
 > #### Option 1: Install Subdirectory / Local Clone (Editable Mode)
 >
 > If you have cloned the [Codex Crew GitHub repository](https://github.com/mikutellyourworld/Codex-Crew) locally, navigate to the repo root and run:
 >
 > ```bash
 > pip install -e ".[teams]"
 > ```
 ...
 >
 > ```bash
 > pip install "git+https://github.com/mikutellyourworld/Codex-Crew.git"
...
5. Set the bot's **Messaging endpoint** to your public URL plus the Codex Crew
   route:
...
Then enable the channel and add your allow-list in `~/.codex-crew/config.json`:
...
Codex Crew: turning it on here turns it on for your dashboard chats and scheduled
