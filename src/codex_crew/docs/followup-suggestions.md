...
The agent calls the `suggest_followup` MCP tool (codexcrew-core) with up to three
items:
  ...
  "items": [
    {
      ...
      "prompt": "In src/codex_crew/dashboard/handlers/files.py, add a per-caller token-bucket limiter to api_file_upload ... (full standalone instruction)",
