...
a route the policy anticipated. The protection is enforced by Codex Crew, not by
...
`codexcrew secrets import` migrates plaintext credentials from `.env` into the
...
```bash
codexcrew secrets import           # report only
codexcrew secrets import --apply   # store the secrets and rewrite .env
```
