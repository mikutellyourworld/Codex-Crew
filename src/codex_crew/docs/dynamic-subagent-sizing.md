...
Codex Crew sizes the concurrent sub-agent cap **automatically** by default
(`agent.max_subagents = 0`): at gateway startup it computes a sensible cap from
the host's actual memory and CPU, plus a per-agent cost Codex Crew *learns* from
...
```
codexcrew config set agent.max_subagents 8
```
...
Codex Crew doesn't hard-code how much an agent costs — it measures it:
...
- At exit, one sample `{agent, mem_gb, cpu_cores, ts}` is appended to
  `~/.codex-crew/subagents/cost_samples.jsonl`.
- At the next startup, Codex Crew takes the **p90 of the last N samples per
...
raise it. Codex Crew does **not** yet measure provider saturation — that's a
...
- Design rationale and worked examples:
  [`docs/system-specs/modules/subagent.md`](https://github.com/mikutellyourworld/Codex-Crew/blob/main/docs/system-specs/modules/subagent.md).
