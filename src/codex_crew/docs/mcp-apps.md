...
Codex Crew renders **MCP Apps** natively: when an MCP tool returns a `ui://` resource
...
`ui` extension — Codex Crew targets the **Stable 2026-01-26** revision — and it works with any conforming command-based stdio server: nothing is hardcoded per vendor.
...
| Condition | Result |
|---|---|
| `CODEXCREW_MCP_APPS` = `0`/`false`/`no`/`off` | disabled (explicit kill-switch, wins over everything) |
| stored `mcp_gateway.apps_enabled` = `false` | disabled, even with `CODEXCREW_MCP_APPS` on — a released opt-out is still honoured |
| config unreadable | disabled (fails closed — the stored preference cannot be confirmed) |
| `CODEXCREW_MCP_APPS` = `1`/`true`/`yes`, no stored opt-out | enabled (explicit override — tests, e2e harness) |
...
Check `CODEXCREW_MCP_APPS` is not set to an off value and that
...
Nothing specific to Codex Crew. Conform to SEP-1865:
...
2. **Associate it with the tool.** Either form works:
   - on the **tool definition**: `_meta.ui.resourceUri` (SEP-1865's primary form —
     preferred, and what Codex Crew harvests at backend spawn), or
...
Codex Crew harvests declared URIs from `tools/list` when the backend starts — a
tool that declared a `ui://` resource renders even when its individual results
carry no `_meta`.

## Deviations from SEP-1865

Codex Crew targets the Stable 2026-01-26 revision. Two things an app author should
...
handshake. Codex Crew does not do this. It renders app HTML in a **single
...
3. The gateway writes the payload to a spool file at
   `$CODEXCREW_HOME/mcp-apps/<uuid4hex>.json` and injects an opaque marker
   `[codexcrew-mcp-app:<uuid4hex>]` at the START of the tool result *text*. It
...
Worth understanding before you point Codex Crew at an unfamiliar server, because
...
|---|---|
| Tool output renders as plain text | the server has no stub in MCP Management, or `CODEXCREW_MCP_APPS` is set to an off value |
...
[Dashboard iframe hosts](https://github.com/mikutellyourworld/Codex-Crew/blob/main/docs/architecture/dashboard-iframe-hosts.md)
