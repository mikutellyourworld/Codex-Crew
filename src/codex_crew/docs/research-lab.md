...
> **Research Lab is not a permission boundary.** Its `codexcrew-research` worker is
...
```
~/.codex-crew/workspace/research/<campaign_id>/
...
Research Lab writes campaign state, findings, and reports in its campaign directory. Its `codexcrew-research` worker derives from the main Codex Crew agent configuration rather than a research-only tool allowlist. Treat Research Lab as an orchestration feature, not a permission boundary; review the tools enabled for the generated agent before running an unattended campaign.
