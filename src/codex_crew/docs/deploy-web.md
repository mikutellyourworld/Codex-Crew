...
to be configured on the machine running the Codex Crew gateway, not on your laptop if the gateway
...
Run one of these in a terminal on the gateway host. Codex Crew never sees the keys:
...
1. Register the **profile name** and **region**. Registered profiles are kept in
   `~/.codex-crew/deploy/profiles.json`, and one of them is the default. Optionally supply a
   12-digit account plus a role name and Codex Crew writes a `credential_process` entry that assumes
...
3. Click **Get IAM policy** and apply the generated least-privilege policy **yourself** to a
   dedicated role or identity. **Codex Crew never writes IAM.** For the `fullstack` tier the console
   also emits a permissions-boundary policy that must exist as `codexcrew-deploy-app-boundary`
...
behind one shared per-account base stack (`codexcrew-deploy-base`: one private bucket plus one global distribution), each served under its own `/<slug>/` prefix.
...
`~/.codex-crew/skills/artifact-deploy/`. It is an operator step by design: the stack creates an IAM
role, and Codex Crew never writes IAM. When a finite-TTL deploy is refused, the 409 body carries
that exact command with your profile and region already filled in.

The reaper only touches resources that carry both the `codexcrew:site=<id>` and
`codexcrew:managed=true` tags and match the managed naming scheme. It verifies the tag against the
...
Artifact Deploy is built to keep Codex Crew out of credential and account management entirely, and
to serve content from a bucket that is never itself public.

### 6.1 Credentials never touch Codex Crew
- The local registry stores a **profile name**, region, last-verified account metadata, and the selected default in `~/.codex-crew/deploy/profiles.json`; it does not store credentials.
...
- Codex Crew **never writes IAM** and never creates or manages accounts, users, or roles. You apply
...
- Per-site bucket names are random and opaque (`codexcrew-web-<hex>`) and are hidden from the
...
`aws:ResourceTag/codexcrew:managed=true`, and Codex Crew tags each resource at creation. An unrelated
...
scoped to the `codexcrew-deploy-*` prefix, and the audit-log bucket is covered by an explicit Deny.

### 6.5 Pre-publish content scan
- Before upload, content runs through Codex Crew's credential patterns plus data-leak heuristics
...
| Two base stacks, or duplicate buckets and distributions, in one account | A pre-rename install left a parallel set of stacks. Follow `~/.codex-crew/skills/artifact-deploy/MIGRATION.md` to move live sites over and remove the old set. |
