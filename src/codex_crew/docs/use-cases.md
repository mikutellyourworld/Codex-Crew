...
Real-world workflows from the Codex Crew community. These combine Codex Crew's
capabilities — cron jobs, subagents, memory, chat channels, and task runner —
into end-to-end automation.

## Backlog Crusher

The most powerful workflow: Codex Crew digs through your issue backlog, picks up
...
```
codexcrew run backlog-crusher.md
```

`codexcrew run --help` lists the flags that matter for a long run: `--fresh`
ignores an existing checkpoint, `--no-test` skips verification between steps,
and `--timeout` caps the whole run.

For unattended operation with the `yolo` approval mode, use an isolated `CODEXCREW_HOME`; the gateway refuses `--approval yolo` for the main data home.

## Repetitive Refactors

Automate repetitive cleanup across a codebase. Codex Crew reads a config or
...
coding Codex Crew instance picks them up automatically.
...
Codex Crew spawns subagents — one per region or source — and synthesizes the
results into a single summary.

## Auto-Collect Information

Replace manual information-gathering workflows. Use Codex Crew cron jobs to
...
```
codexcrew agent list
codexcrew chat --agent <installed-review-agent>
```
...
List available agents with `codexcrew agent list`, then select one for a CLI chat with `codexcrew chat --agent <agent-name>`.
...
- **Unattended overnight runs**: Use an isolated `CODEXCREW_HOME` when running the gateway with `--approval yolo`. Review the CRs in the morning.
- **Workspace isolation**: Use `CODEXCREW_HOME` and `CODEXCREW_PORT` env vars to run multiple Codex Crew instances with separate data.
