...
`codexcrew snapshot` packs everything Codex Crew has learned about you into a single
portable `.tar.gz`, and `codexcrew restore` unpacks it, on this machine or a
...
```bash
codexcrew snapshot                                     # write to ~/.codex-crew/snapshots
codexcrew snapshot ~/my-snapshots --keep 3             # custom dir, prune to 3
codexcrew snapshot --components memory                 # just memory, ~20 MB
codexcrew snapshot --list                              # list existing snapshots
codexcrew restore snapshot.tar.gz                      # auto-detects replace vs merge
codexcrew restore snapshot.tar.gz --components memory,crons
codexcrew restore snapshot.tar.gz --dry-run            # preview, write nothing
codexcrew restore --list-components                    # show component names
```

Stop the gateway before restoring. `codexcrew restore` refuses to run while a
...
A snapshot written only to `~/.codex-crew/snapshots` does not survive losing the machine,
...
`codexcrew restore` takes a **local path**. An `s3://` argument is refused, with a message
...
The mode is auto-detected from whether `~/.codex-crew/memory.db` exists, so a
...
After a restore, run `codexcrew restart` so the gateway picks up the new state.
...
`codexcrew snapshot` runs only when something runs it. OFF-HOST backups are scheduled
...
schedule `codexcrew snapshot --keep 7` daily. Verify it afterwards with
`codexcrew snapshot --list`: an unverified backup job is the same as no backup.
