...
Everything Codex Crew remembers about how it should behave lives in one JSON file,
`~/.codex-crew/config.json`, created automatically on the first `codexcrew gateway`
...
```bash
codexcrew config get                    # print full config
codexcrew config get agent.model        # print a specific value
codexcrew config set agent.model auto   # set a value (auto type detection)
codexcrew config set --local agent.model auto   # write config.local.json instead
...
The dashboard port is **not** a config key: set `CODEXCREW_PORT` instead.
...
whatever was written the last time it saved. On startup Codex Crew prints one line
naming any key still holding an old default.

`codexcrew config defaults` shows each one with its stored value, the current
default, and the release that changed it. Two ways to answer it:

```bash
codexcrew config defaults --adopt       # take the current defaults
codexcrew config defaults --keep        # affirm your values, stop the notice
```

Both accept specific keys — `codexcrew config defaults --keep session.autocompact_pct`
...
you affirmed, so changing one later brings the notice back. `codexcrew doctor` lists
everything, affirmed values included.

The same command also clears a stored value Codex Crew has to replace — a retired
...
`agent.sandbox` controls whether Codex Crew wraps the agent process in its own
OS-level sandbox (a user namespace on Linux, `sandbox-exec` on macOS).

| Value | Behavior |
|-------|----------|
| `auto` (default) | Add the Codex Crew OS-level sandbox; on macOS it defers to the kiro-cli internal sandbox when that is enabled |
| `off` | Skip the Codex Crew OS-level sandbox |

The two layers are mutually exclusive on macOS because a nested seatbelt sandbox fails with `EPERM`. The default is `auto`: it uses the Codex Crew sandbox where available and defers to the kiro-cli internal sandbox on macOS when that sandbox is enabled.

Set via `codexcrew config set agent.sandbox auto`.

## ACP Backend

`agent.acp_backend` selects which ACP agent Codex Crew drives. `agent.provider`
...
the separate `_kiro.dev/*` methods kiro-cli uses, and Codex Crew maps them back to
the same displays.
...
- Native subagent progress reporting (subagents run; their live progress does not
  surface in the UI).
- Slash commands: KAS advertises them (`available_commands_update`), but Codex Crew
...
- Model selection is unverified: KAS advertises no model list on an
  unauthenticated session, and Codex Crew only sends a model the session
...
notification for Codex Crew to surface. A resumable-session existence probe would
...
**KAS is served by kiro-cli's own ACP relay.** Codex Crew spawns
...
- **Credentials stay in kiro-cli.** `--auth-method cli` makes the relay resolve
  access tokens from kiro-cli's own store, so Codex Crew never handles a KAS
  token. This works on any machine where `kiro-cli login` has succeeded; sign in
  with kiro-cli before switching.
- **No KAS assets to locate.** Codex Crew does not read kiro-cli's extracted KAS
  ...
  `codexcrew doctor` reports that when `agent.acp_backend` is `kas`.
...
Set via `codexcrew config set agent.acp_backend kas`.
...
| `agent.sandbox` | `"auto"` (use Codex Crew OS-level sandbox, or defer to the kiro-cli internal sandbox on macOS) or `"off"` (skip the Codex Crew sandbox) | `"auto"` |
...
| `agent.log_level` | Persistent log level for the `codex_crew` logger, applied at startup. The `--verbose` CLI flag overrides it | `"WARNING"` |
...
| `slack.command` | Slash-command name | `"codexcrew"` |
...
Only the owner (`CODEXCREW_OWNER_ID`) is authorized to interact over Slack.
...
| `stt.enter_sends` | Let Enter send while dictation is live, from anywhere rather than only when the composer holds focus. Starting a recording puts focus on the mic button, so without this the panel's own "Enter to send" hint does nothing. Off makes Enter insert a newline while recording | `true` |
...
the first time you dictate Codex Crew fetches the model named by `stt.model` and
...
- **Intel Macs have no prebuilt recognizer.** Every other platform Codex Crew
...
- Compressed audio still passes through ffmpeg internally: a voice note arrives
  ...
  for Codex Crew's use. If that download fails, the page offers to hand the failure
...
| `knowledge.auto_add_documents` | Let the agent add documents it reads while working to the Knowledge Library (one aggregate "Auto-added" source). The agent fetches the content with its own tools under your approval; Codex Crew fetches nothing, so `doc_ingest_hosts` does not apply. Renamed from `auto_ingest_doc_links`, which is still accepted on read | `false` |
...
| `snapshot_dir` | Where `codexcrew snapshot` writes tarballs | `""` (`~/.codex-crew/snapshots`) |

## Environment Variables

| Variable | Purpose | Default |
|----------|---------|---------|
| `CODEXCREW_HOME` | Override the config/data directory | `~/.codex-crew` |
| `CODEXCREW_PORT` | Override the dashboard port | `5486` |
| `CODEXCREW_PROJECT_DIR` | Override the agent-config/skills project directory | Auto-detected |
| `CODEXCREW_WORKSPACE` | Override the workspace root, used as-is with no subdirectory appended | Saved `workspace_dir`, else a platform default |
| `CODEXCREW_SKIP_MODEL_DOWNLOAD` | Set to `1` to skip the background embedding-model download at gateway startup (tests, CI, airgapped hosts) | unset |
| `CODEXCREW_EMBED_MODEL_URL` | Override HTTPS URL for the embedding-model GGUF; wins over `memory.embed_model_url` and the CDN default | unset |
| `CODEXCREW_EMBED_MODEL_PATH` | Absolute path to a local GGUF to use instead of the bundled model; wins over `memory.embed_model_path` and suppresses the default download entirely | unset |
...
- cron schedule display (`codexcrew cron list`, the Slack Home Tab)
...
`~/.codex-crew/.env` holds messaging-channel credentials and the owner ID. For
Slack:

```
SLACK_APP_TOKEN=xapp-...
SLACK_BOT_TOKEN=xoxb-...
CODEXCREW_OWNER_ID=UXXXXXXXX
```

## Denied Commands

The built-in destructive-command deny rules are enforced at Codex Crew's own
...
| Path | Purpose |
|------|---------|
| `~/.codex-crew/config.json` | Main config |
| `~/.codex-crew/config.local.json` | Local overrides that survive upgrades |
| `~/.codex-crew/.env` | Slack credentials |
| `~/.codex-crew/skills/` | User skills |
| `~/.codex-crew/crons.json` | Scheduled jobs |
| `~/.codex-crew/hooks.json` | Script hooks |
| `~/.codex-crew/lessons.jsonl` | Learned corrections |
| `~/.codex-crew/notifications.jsonl` | Notification history |
| `~/.codex-crew/models/` | Embedding model, downloaded in the background at startup |
| `~/.codex-crew/history/` | Chat history (JSONL) |
| `~/.codex-crew/workspace/memory/` | Memory files |
| `~/.codex-crew/session_map.json` | Session resume mapping |
| `~/.codex-crew/snapshots/` | Default output of `codexcrew snapshot` |
| `~/.kiro/agents/codexcrew.json` | Installed agent config |
