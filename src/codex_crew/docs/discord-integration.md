...
| **Every other server channel** | Ignored, including a channel ID entered in the *thread* allow-list by mistake: Codex Crew verifies the Discord channel type before running a turn. |
...
Codex Crew requests the guild/message-content Gateway intents when either `allowed_thread_ids` or `allowed_channel_ids` is configured, so DM-only installs keep the narrower intent set.
...
server. Codex Crew publishes its command set to Discord at startup, so the menu
is populated for you. The scope is **not** required to use Codex Crew: every
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| Enabled, but no Discord activity | Token missing or gateway not restarted | Set `DISCORD_BOT_TOKEN`, then `codexcrew restart` |
...
| DMs are ignored | User ID is missing/wrong | Add the numeric user ID; inspect `codexcrew security events` |
...
- **Channel-type verification.** Codex Crew resolves an approved guild channel
...
- **Global intent scope.** Enabling any server thread or allowed parent channel turns on Discord's global Message Content intent. Discord then delivers content from every server channel the bot can see; Codex Crew immediately discards traffic outside approved threads and parent channels and does not audit routine background chatter.
...
- **Token handling.** The token lives in `~/.codex-crew/.env` (mode 0600), is
...
another channel, Codex Crew refuses and tells you where it lives, rather than
moving it silently.
...
or more allowed users Codex Crew cannot tell which one owns the workspace and
