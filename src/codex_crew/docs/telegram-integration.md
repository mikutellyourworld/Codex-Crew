...
Chat with your Codex Crew agent right from Telegram — on your phone, your laptop,
...
works from behind a firewall — Codex Crew reaches out to Telegram, so there's
nothing to expose.

## The easy way: just ask Codex Crew

You don't have to edit anything by hand. In any Codex Crew session — the
dashboard, Slack, or the CLI — say something like *"set up the Telegram
channel."* Codex Crew walks you through creating the bot, then writes the token
and your user ID into `~/.codex-crew/.env` and `config.json` and restarts the
...
You'll need a running gateway (`codexcrew gateway`) and a Telegram account.
...
3. **Save the token** to `~/.codex-crew/.env`:
   ```
   TELEGRAM_BOT_TOKEN=123456789:AA…
   ```
4. **Turn it on** in `~/.codex-crew/config.json`:
   ```json
...
> **Codex Crew runs on your machine, with your files and credentials.** So it only
...
- `/yolo [on|off|renew]` — report or change the auto-approve grant. This is the
  SAME process-wide grant the dashboard toggle and Slack's `/codexcrew yolo`
...
- `/codexcrew dashboard [<N>h|<N>m]` — get a dashboard login link (DM only).
...
A restart no longer replays your last few messages: the `getUpdates` cursor is kept in `~/.codex-crew/routing/telegram_offset.json`. Delete that file only if you want a deliberate replay of whatever Telegram still holds.
