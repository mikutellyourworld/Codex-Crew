...
Chat with Codex Crew on WhatsApp from your own account: scan a QR code once and
...
2. Enable the channel in `~/.codex-crew/config.json`:

   ```json
   { "whatsapp": { "enabled": true } }
   ```

   Or run `codexcrew setup --whatsapp`, which reports whether the optional extra
...
4. The badge flips to **Connected**. Pairing state persists in
   `~/.codex-crew/whatsapp/session.db`, so you scan once rather than per restart.
...
- **Not sure whether the channel can run at all** - run `codexcrew doctor`. Its
  **WhatsApp Integration** section reports whether the `whatsapp` extra is
  ...
  session store exists at `~/.codex-crew/whatsapp/session.db`. An unpaired store is
