...
Codex Crew gives an agent real tool access, so some commands are refused at the
...
reading the file. If you still see the old conclusion, run `codexcrew doctor` —
...
| Self-protection | minting a dashboard token, killing the gateway, an inline interpreter that imports Codex Crew | Prevents the agent from escalating its own access or shutting down its supervisor. |
...
`doctor` run — a missing AWS setup is not a Codex Crew fault.
...
codexcrew policy explain <scope> <item>
```
