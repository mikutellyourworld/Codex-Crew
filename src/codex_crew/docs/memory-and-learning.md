...
Codex Crew has persistent memory that survives across sessions. It remembers your
...
Corrections and rules you teach Codex Crew. Two ways to create:
1. **Explicit**: say "remember to always use pytest" → saved immediately
2. **Implicit**: correct Codex Crew during conversation → extracted during consolidation
...
first time the gateway starts, over HTTPS from the Codex Crew CDN — failed
...
The bundled model is `qwen3-embedding:0.6b` (1024 dimensions). `CODEXCREW_EMBED_MODEL_URL` overrides `memory.embed_model_url` for the download URL; `CODEXCREW_EMBED_MODEL_PATH` or `memory.embed_model_path` selects a local GGUF instead of the bundled model.

## Consolidation

Codex Crew automatically consolidates conversations into memory:
...
- `codexcrew memory show [preferences|projects|history]` — print the markdown
...
- `codexcrew memory export --include-markdown` — add a `markdown` collection to
...
- **Chat**: ask Codex Crew to update its memory files directly
