...
# called from ``CodexCrewConfig.load``) rather than read here. That is not
...
def register_mcp_path_dirs(*dirs: str) -> tuple[str, ...]:
    ...
    packaged distribution, or a host embedding Codex Crew can name the directories
...
def publish_config_path_dirs(values: object) -> None:
    ...
    Called by ``CodexCrewConfig.load`` on every load, which is what keeps the
...
# unofficial glibc-2.17 build, i.e. a Node Codex Crew installed itself. Relative
...
_NODE_BIN_DIR_ENV = "CODEXCREW_NODE_BIN_DIR"
...
def _validated_bin_dir(val: str) -> str | None:
    ...
    ``CODEXCREW_NODE_BIN_DIR`` override and the ``ensure-node.sh`` marker file --
...
def node_bin_dirs() -> tuple[str, ...]:
    ...
    1. ``CODEXCREW_NODE_BIN_DIR`` -- explicit operator override.
    ...
    the very node Codex Crew installed for it.
    ...
    # Under a CODEXCREW_HOME override data_home() mkdirs, so it can raise on an
...
def _ensure_node_script() -> Path | None:
    ...
    Search order mirrors :func:`codex_crew.cli._ensure_node`: the explicit
    ``CODEXCREW_PROJECT_DIR`` first, then the source-tree root two levels above
...
def is_toolbox_install() -> bool:
    """Return True if the running codexcrew binary was installed via Toolbox."""
...
def git_build_info() -> tuple[str, str]:
    ...
    Reads ``CODEXCREW_PROJECT_DIR`` (the git tree the gateway runs from) and
    ...
    pip-wheel installs (no ``CODEXCREW_PROJECT_DIR`` or no ``.git``) — so callers
...
def augmented_path(base_path: str = "", *, home: str | None = None) -> str:
    ...
    When CodexCrew runs under systemd or another non-login shell the
    ...
    to resolve the ``codexcrew`` / ``codexcrew-core`` console-script wrappers
...
# * The ``PYTHON`` namespace matters because Codex Crew's Linux sandbox launcher
...
# it, for the same reason ``CODEXCREW_`` is below: an enumeration cannot cover the
...
# The env namespace Codex Crew reserves for ITSELF. A spec-declared ``env`` may
# not set any key in it.
#
# This is an AUTHORIZATION boundary, and it is a different class from the loader
# prefixes above — not a variant of them. Several ``CODEXCREW_*`` variables are
# how the gateway tells a process it spawns WHO IS CALLING, and the consumers
# treat them as vouched-for:
#
# * ``CODEXCREW_CLI`` — no consumer reads it. It is covered because the deny is on
...
# * ``CODEXCREW_SESSION_KEY`` / ``CODEXCREW_HOST_PID`` — two of the three sources
...
# * ``CODEXCREW_OWNER_ID`` / ``CODEXCREW_INTERNAL_SECRET`` — the Slack owner
...
# ``CODEXCREW_*`` variables are read across the tree today, several of them
...
# ``dict(os.environ)``), so a gateway-authored ``CODEXCREW_*`` value is INHERITED
# either way. Denying it here removes only a config file's ability to OVERRIDE
# one — exactly the capability being abused, and nothing else. Codex Crew's own
# spawn paths set these variables directly on the child env
# (``apps.backend``'s ``CODEXCREW_APP_NAME``, the gateway's
# ``CODEXCREW_MCP_TARGET_*``), never by declaring them in an ``mcpServers``
...
_SPEC_ENV_RESERVED_PREFIXES: tuple[str, ...] = ("CODEXCREW_",)
...
def sanitize_spec_env(pairs: Iterable[tuple[str, str]]) -> dict[str, str]:
    ...
    * ``_SPEC_ENV_RESERVED_PREFIXES`` — Codex Crew's own namespace, which carries
      ...
      whose jobs Codex Crew believes the child is entitled to. That is why "the
      ...
      reaches the identical consumer (``sh -c 'CODEXCREW_SESSION_KEY=... exec
      ...
      ``CODEXCREW_SESSION_KEY`` / ``CODEXCREW_HOST_PID`` have legitimate producers,
    ...
    for key, value in pairs:
        ...
        if any(folded.startswith(p) for p in _SPEC_ENV_RESERVED_PREFIXES):
            # Distinct message on purpose: reporting a forged CODEXCREW_CLI as a
            ...
            logger.warning(
                "dropping spec env key %r: the CODEXCREW_ namespace is reserved for "
...
def activate_mise(env: MutableMapping[str, str] | None = None) -> list[str]:
    ...
    installed, when disabled via ``CODEXCREW_NO_MISE``, or when invoking /
    ...
    if target.get("CODEXCREW_NO_MISE"):
        logger.debug("mise activation skipped: CODEXCREW_NO_MISE set")
        return []
    mise = _mise_bin()
    if not mise:
        return []
