// AppleTranscribe.swift — on-device speech-to-text helper for Codex Crew's `apple` STT provider.
//
// WHY A SEPARATE BINARY: Apple's SpeechAnalyzer / SpeechTranscriber (macOS 26+) is a
// Swift-only framework. The Codex Crew gateway is Python, so the only way to reach it is
// a small out-of-process helper. This file is compiled on demand by
// ``codex_crew.apple_speech`` into the data home's cache dir; a shipped build would
