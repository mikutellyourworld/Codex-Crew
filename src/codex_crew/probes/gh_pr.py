...
:mod:`codex_crew.irq`. This module owns only the two things that are
...
scheduler, which drives it in-process through :func:`codex_crew.irq.poll` for a
...
from codex_crew.github_runner import resolve_gh, run_gh
...
if __name__ == "__main__":  # pragma: no cover -- cron-only entry point
    print("pr_watch.py is a Codex Crew script cron; register it with cron_add.")
    sys.exit(2)
