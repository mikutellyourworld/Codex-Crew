"""CodexCrew — open-source personal AI agent."""
...
# ``/api/health``, ``codexcrew --version``, the diagnostics report, the
...
# is theirs to set, so ``CODEXCREW_BUILD_VERSION=0.6.0.999`` would have waved a
...
def _apply_build_version_file(base: str, package_dir: "str | None") -> str:
    ...
    if override is None:
        logging.getLogger(__name__).warning(
            "%s %r does not name a build of codex_crew %s "
