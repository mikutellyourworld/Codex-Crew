...
The kiro agents directory (:func:`codex_crew.config.paths.kiro_agents_dir`) is
...
* aged backups ``*.bak-<digits>`` and ``*.json.bak.<digits>`` — OPT-IN
  (``sweep_backups=True``) and swept at a much longer 14-day window. Codex Crew
...
surface so it can be wired into ``codexcrew doctor`` and gateway boot in one or
...
#: ``codexcrew doctor``, the command you run *when something is broken*, is one of
...
#:   ``.tmp``.
#: * ``tmp<alnum>.tmp`` — Codex Crew's OWN atomic-write residue. ``agent.py``'s
...
def sweep_agents_dir(
    ...
    *,
    ...
    :param sweep_backups: when false (the default), only temps are swept and
        recognized backups are left entirely alone. Codex Crew authors no
    ...
    :param dry_run: when true, identify eligible files and populate
        ...
        ``codexcrew doctor``) REPORT what a sweep would reclaim without mutating
