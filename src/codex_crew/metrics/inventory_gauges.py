...
does this install actually have set up". An operator running Codex Crew across
...
the ``codexcrew[otlp]`` extra, so any aggregate over these gauges describes one
...
serves the same job the rest of ``codexcrew.*`` serves: an operator observing
machines they run.
...
Both are properties of the shared resource -- every ``codexcrew.process.*`` gauge
...
Instruments (all in the core ``codexcrew.`` namespace, validated against
``schema.validate_name`` at registration):

=========================================  =========  ==============  ==================
name                                       value      attributes      source
=========================================  =========  ==============  ==================
``codexcrew.inventory.crons.active``        count      —               ``crons.json``
``codexcrew.inventory.monitor_loops.active`` count     —               ``autonudge`` service
``codexcrew.inventory.skills.installed``    count      —               skills loader
``codexcrew.inventory.memory.migrated``     0/1        —               ``memory.migrated``
``codexcrew.inventory.knowledge.documents`` count      —               ``knowledge.db``
``codexcrew.inventory.lessons``             count      —               ``lessons.jsonl``
``codexcrew.inventory.mcp.servers``         count      ``class``       MCP roster
``codexcrew.inventory.config.toggle``       0/1        ``key``         config flags
``codexcrew.inventory.probe.failures``      count      ``probe``       this module
=========================================  =========  ==============  ==================
...
from codex_crew.metrics.schema import validate_name
...
GAUGE_CRONS_ACTIVE = "codexcrew.inventory.crons.active"
GAUGE_MONITOR_LOOPS_ACTIVE = "codexcrew.inventory.monitor_loops.active"
GAUGE_SKILLS_INSTALLED = "codexcrew.inventory.skills.installed"
GAUGE_MEMORY_MIGRATED = "codexcrew.inventory.memory.migrated"
GAUGE_KNOWLEDGE_DOCUMENTS = "codexcrew.inventory.knowledge.documents"
GAUGE_LESSONS = "codexcrew.inventory.lessons"
GAUGE_MCP_SERVERS = "codexcrew.inventory.mcp.servers"
GAUGE_CONFIG_TOGGLE = "codexcrew.inventory.config.toggle"
GAUGE_PROBE_FAILURES = "codexcrew.inventory.probe.failures"
...
#: :mod:`codex_crew.metrics.process_gauges`, and declared the same way so the
...
#: COUNTER, which made it the last CUMULATIVE series Codex Crew exported: one is
...
#: giant delta (see :mod:`codex_crew.metrics.temporality`).
...
# case: it boots with its own ``CODEXCREW_HOME`` and never copies crons, sessions or
...
def mark_install_reporter() -> None:
    ...
    ``codexcrew.inventory.*`` series is absent, with nothing to say why (see
    ``test_reporter_claim_is_unconditional``). Every other telemetry-enabled
    process in the install leaves it unset and publishes no ``codexcrew.inventory.*``
...
def read_active_crons() -> Optional[int]:
    ...
    **missing** store — a genuine 0, since a fresh install has no crons.
    """
    from codex_crew.cron import _CRONS_FILE, _default_dir, enabled_count_from_disk
...
def read_memory_migrated() -> Optional[int]:
    ...
    return int(bool(CodexCrewConfig.load().memory.migrated))
