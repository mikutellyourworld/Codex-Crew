...
existing session instruments cannot: ``codexcrew.session.startup.duration``
measures the cold start of the agent PROCESS, and
``codexcrew.session.idle_expired`` counts one specific teardown cause, but
...
(``codexcrew.session.idle_expired``, ``codexcrew.watchdog.recovery.outcome``), and
...
def _crumb_path(session_key: str, started_at: float) -> Path:
    ...
    **The name identifies the WRITER and the GENERATION, not just the key, and
    ...
    a ``codexcrew run`` and the gateway hold the same key at the same time, each
...
async def record_session_started(session_key: str) -> None:
    ...
    **Why this is a coroutine, and awaited under the caller's lock.** Two
    ...
    try:
        from codex_crew.metrics.events import emit_counter
        ...
        payload = json.dumps(
            {
                ...
                # sibling process -- `codexcrew run` and the eval runner each build
