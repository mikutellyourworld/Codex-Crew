...
name                                             kind        source
===============================================  ==========  ====================
``codexcrew.process.threads.python``              gauge       ``threading.active_count()``
``codexcrew.process.threads.os``                  gauge       ``/proc/self/task`` (Linux only)
``codexcrew.process.open_fds``                    gauge       ``/proc/self/fd`` / ``/dev/fd``
``codexcrew.process.memory.rss_bytes``            gauge       ``platform_compat.proc_rss_bytes``
``codexcrew.process.memory.peak_rss_bytes``       gauge       ``platform_compat.proc_peak_rss_bytes``
``codexcrew.process.cpu.seconds``                 gauge       ``platform_compat.proc_cpu_seconds``
``codexcrew.process.gc.collections``              gauge       ``gc.get_stats()`` per generation
``codexcrew.process.gc.collected``                gauge       ``gc.get_stats()`` per generation
``codexcrew.process.gc.uncollectable``            gauge       ``gc.get_stats()`` per generation
===============================================  ==========  ====================

The last four report a monotonic PROCESS-LIFETIME total and were observable
COUNTERS until they were the only cumulative series Codex Crew exported. One
...
whole total as one giant delta (see :mod:`codex_crew.metrics.temporality`).
...
GAUGE_THREADS_PYTHON = "codexcrew.process.threads.python"
GAUGE_THREADS_OS = "codexcrew.process.threads.os"
GAUGE_OPEN_FDS = "codexcrew.process.open_fds"
GAUGE_RSS = "codexcrew.process.memory.rss_bytes"
GAUGE_PEAK_RSS = "codexcrew.process.memory.peak_rss_bytes"
GAUGE_CPU_SECONDS = "codexcrew.process.cpu.seconds"
GAUGE_GC_COLLECTIONS = "codexcrew.process.gc.collections"
GAUGE_GC_COLLECTED = "codexcrew.process.gc.collected"
GAUGE_GC_UNCOLLECTABLE = "codexcrew.process.gc.uncollectable"
