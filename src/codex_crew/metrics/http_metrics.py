...
API server):
  ...
  * ``codexcrew.gateway.boot.duration`` — a histogram (ms) of how long
  ...
  * ``codexcrew.gateway.request.duration`` — a histogram (ms) of per-request
...
# --- Metric names (core codexcrew.* namespace; validated by the recorder) ------
BOOT_METRIC = "codexcrew.gateway.boot.duration"
REQUEST_METRIC = "codexcrew.gateway.request.duration"
...
def record_boot_to_ready(
    ...
    """Emit the ``codexcrew.gateway.boot.duration`` histogram (best-effort).
