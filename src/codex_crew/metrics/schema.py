...
OSS-CLEAN: stdlib only, plus the first-party ``codex_crew.security`` scrubbers.
...
RESOURCE_ATTR_PROCESS_START_TIME = "codexcrew.process.start_time"
...
def redact(value: str) -> str:
    ...
    """
    # First-party scrubbers (codex_crew.security): scan for exfiltration URLs
...
def validate_name(name: str, *, app_id: Optional[str] = None) -> str:
    ...
    Args:
        ...
        app_id: If provided, the caller is an app; the name MUST start with
                ``app.<app_id>.`` and MUST NOT start with ``codexcrew.`` or
                ``gen_ai.``.
    ...
    if app_id is not None:
        ...
        if name.startswith(NS_CORE):
            raise ValueError(f"App {app_id!r} cannot emit metrics in the codexcrew.* namespace")
        ...
        # Core callers: must use codexcrew.* or gen_ai.*
