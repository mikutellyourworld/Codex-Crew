"""Per-query timing for CodexCrew's SQLite stores.

``codexcrew.db.query.duration`` — a histogram (ms) of how long individual store
operations take, labelled by which store and which kind of operation.

Why this exists: route latency (``codexcrew.gateway.request.duration``) tells you
...
``telemetry.enabled`` (or ``CODEXCREW_TELEMETRY``) is set, so an unopted host pays
...
DB_METRIC = "codexcrew.db.query.duration"
