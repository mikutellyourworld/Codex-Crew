...
``codexcrew.turn.duration`` (histogram) plus the turn's own usage —
``codexcrew.turn.tokens`` (counter, ``direction`` attr) and the two billing
histograms ``codexcrew.turn.credits`` / ``codexcrew.turn.cost_usd``. All four are
...
ACP ``sessionId``, not the Codex Crew session key this metric must group by, and
...
# imported from ``codex_crew.acp.types``. Application code must not reach the ACP
...
TURN_METRIC = "codexcrew.turn.duration"
...
#: unbounded.
TURN_TOKENS_METRIC = "codexcrew.turn.tokens"
...
TURN_CREDITS_METRIC = "codexcrew.turn.credits"
TURN_COST_METRIC = "codexcrew.turn.cost_usd"
...
def turn_outcome(stop_reason: str | None, *, exhausted: bool = False) -> str:
    ...
    ``codexcrew.watchdog.recovery.outcome``), so folding it into ``error`` would
...
def emit_turn_duration(
    ...
    *,
    ...
    """Emit one ``codexcrew.turn.duration`` sample (best-effort, never raises).
    ...
    :func:`codex_crew.messaging.link.telemetry_channel_of`, which exists for
...
def emit_turn_usage(
    ...
    **Every value is gated on being positive**, and that gate is the point rather
    ...
    they exist on ``codexcrew.turn.duration``, which samples the same turns, so
