...
to ``<dir>/metrics-YYYY-MM-DD-<pid>.jsonl``. This is CodexCrew's default metrics
sink: data stays on the local disk (default ``~/.codex-crew/metrics``) and never
...
the ``~/.codex-crew`` file-permission convention -- so no other local user on a
...
from codex_crew.metrics.temporality import delta_preference
    ...
    def export(
        ...
        **kwargs: object,
        ...
        try:
            ...
            # ~/.codex-crew convention: telemetry stays private (dir 0o700, file
