...
Every core or app metric flows through this facade so CodexCrew's namespace and
privacy guardrails (``validate_name`` / ``validate_attrs`` / ``redact`` from
``codex_crew.metrics.schema``) run *before* the value reaches an OTEL instrument.
...
the first-party ``codex_crew.metrics.schema`` guardrails. No Amazon-internal
imports.
"""
...
from codex_crew.metrics.schema import validate_attrs, validate_name
