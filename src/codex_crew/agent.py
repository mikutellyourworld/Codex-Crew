"""CodexCrew kiro-cli agent configuration.

Generates and installs ``codexcrew.json`` into ``~/.kiro/agents/``.

Configuration files (edit these, then ``codexcrew setup --agent-only``):

  ``src/codex_crew/config/defaults.json``
      Base agent config — tools, model, allowedTools, toolsSettings, etc.

  ``src/codex_crew/config/prompt.md``
      System prompt.

  ``~/.codex-crew/agent.json``
      User overrides merged on top of defaults (optional).

  ``~/.codex-crew/prompt.md``
...
Dynamic fields resolved at install time:
  ...
  - ``mcpServers.codexcrew-cron.command`` — absolute path to ``codexcrew`` binary
"""
...
from codex_crew import agent_state, platform_compat
from codex_crew.agent_discovery import _read_agent_spec
from codex_crew.agent_files import (
    AGENT_FILENAME,
)
from codex_crew.agent_files import CONDUCTOR_AGENT_FILENAME as _CONDUCTOR_AGENT_FILENAME
from codex_crew.agent_files import HEARTBEAT_AGENT_FILENAME as _HEARTBEAT_AGENT_FILENAME
from codex_crew.agent_files import KNOWLEDGE_AGENT_FILENAME as _KNOWLEDGE_AGENT_FILENAME
from codex_crew.agent_files import LITE_AGENT_FILENAME as _LITE_AGENT_FILENAME
from codex_crew.agent_files import (
...
from codex_crew.agent_files import RESEARCH_AGENT_FILENAME as _RESEARCH_AGENT_FILENAME
from codex_crew.atomic_write import replace_with_retry
from codex_crew.config import config_dir
from codex_crew.config import config_path as _mc_config_path
from codex_crew.config.paths import (
    _in_ephemeral_tree,
...
from codex_crew.mcp_cleanup import purge_deleted_proxy_from_config
from codex_crew.mcp_provenance import (
    DERIVED_KEY,
    command_is_ours,
    record_derived,
    recorded_source,
...
from codex_crew.mcp_utils import kiro_oauth_wire_entry, mcp_server_alias
from codex_crew.platform import current_context
from codex_crew.platform import redact_via_context as redact
from codex_crew.platform import safe_context_call
from codex_crew.platform.governance import (
    CU_MCP_SERVER,
    agentcore_posture,
    may_skip_gate_now,
    strip_ungoverned_auto_approve,
)
from codex_crew.platform.governance_profiles import governance_permits
from codex_crew.security import is_sensitive_path
from codex_crew.sel import (  # circular import: sel imports config which imports agent
    SecurityEvent,
    sel,
)
from codex_crew.validation import _AGENT_NAME_RE
...
_MAIN_AGENT_NAME = "codexcrew"
# Cheap Claude Code model for CodexCrew's background agents (lite / heartbeat).
...
def _background_agent_model() -> str:
    ...
    try:
        ...
        return CodexCrewConfig.load().agent.resolve_model("background")
    except Exception:
...
# Bundled fallback — inside the codex_crew.config package
_BUNDLED_CFG_DIR = Path(__file__).resolve().parent / "config"


def _project_dir() -> Path | None:
    """Return the project root from CODEXCREW_PROJECT_DIR, or None."""
    val = os.environ.get("CODEXCREW_PROJECT_DIR")
    if val:
        p = Path(val)
        if p.is_dir():
            return p
...
# via cli_doctor — must NOT fire config_dir(), or it would create $CODEXCREW_HOME
...
# codexcrew binary path — resolved lazily to handle gateway restarts
# where PATH may not include the virtualenv at import time.
_CODEXCREW_BIN: str | None = None
...
def _bin_is_usable(path: Path) -> bool:
    ...
    makes ``ensure_codexcrew_on_path`` publish a machine-wide ``codexcrew`` that is
    ...
    if not head.startswith(b"#!"):
        ...
        # batch shim (`bin\codexcrew.cmd` starts with `@`). The shim DOES name
        # its interpreter (`"%~dp0..\python.exe"`), but we choose not to parse
        # the batch body here; the consumer that spawns it
        # (`_codexcrew_mcp_invocation`) resolves and validates that sibling
    ...
    bin_dir = path.parent
    # `<venv>/bin/codexcrew` (already inside the venv) vs `<root>/bin/codexcrew`
...
def _launcher_works(path: Path) -> bool:
    ...
    ``~/.local/bin/codexcrew`` that points somewhere ELSE is a working install's
    launcher — which must be left alone — or a dead one we should replace.

    Deliberately not folded into ``ensure_codexcrew_on_path``'s gate on its OWN
    resolved target: that gate additionally requires an absolute path, and its
    interpreter check already happened inside :func:`_resolve_codexcrew_bin`.
    """
    return path.is_file() and os.access(path, os.X_OK) and _bin_is_usable(path)


def _codexcrew_bin_subpath(root: Path) -> Path:
    """The console-script path under an install ``root`` for this OS.

    A venv exposes its entry points under ``bin/codexcrew`` on POSIX but
    ``Scripts/codexcrew.exe`` on Windows — pip generates a ``.exe`` launcher
    ...
    ``codexcrew-cron`` / ``codexcrew-core`` MCP servers (``command not found:
    .../bin/codexcrew``). Branch on the platform so both layouts resolve.

    On Windows a relocatable ``bin\\codexcrew.cmd`` shim is preferred over the
    pip-generated ``Scripts\\codexcrew.exe`` when it exists. The desktop bundle
    ...
    in sync. Plain pip installs ship no ``bin\\codexcrew.cmd``, so they keep
    resolving ``Scripts\\codexcrew.exe`` via the fallback.
    """
    if platform_compat.IS_WINDOWS:
        cmd_shim = root / "bin" / "codexcrew.cmd"
        if cmd_shim.is_file():
            return cmd_shim
        return root / "Scripts" / "codexcrew.exe"
    return root / "bin" / "codexcrew"


def _resolve_codexcrew_bin() -> str:
    """Resolve the absolute path of the ``codexcrew`` executable.

    Resolution order (first existing + executable wins):

    1. A sibling ``.venv`` entrypoint, for a source-tree install (an editable
       install next to its own venv, e.g. ``project/src/codex_crew`` plus
       ``project/.venv``). Bounded by the first ``pyvenv.cfg`` walking up, so a
       pip-into-venv install falls through to step 2 instead.
    2. Same install as the current process: walk up from ``codex_crew.__file__``
       looking for a sibling console script (see
       :func:`_codexcrew_bin_subpath` for the per-OS layout). Covers venv-based
    ...
    4. ``shutil.which('codexcrew')`` — respects PATH order.
    5. Bare ``"codexcrew"`` — last resort, may fail but surfaces the problem
    ...
    """
    global _CODEXCREW_BIN
    if _CODEXCREW_BIN:
        return _CODEXCREW_BIN
    ...
    #    install with a sibling .venv directory, e.g. project/src/codex_crew
    #    + project/.venv/bin/codexcrew).
    ...
    try:
        # Circular import: codex_crew.agent is loaded during codex_crew
        # package initialization, so importing codex_crew at module level
        # would create a circular dependency. Deferring here resolves
        # after the package is fully loaded.
        import codex_crew as _mc  # noqa: PLC0415  circular import

        pkg_dir = Path(_mc.__file__).resolve().parent
        for parent in pkg_dir.parents:
            venv_candidate = _codexcrew_bin_subpath(parent / ".venv")
            if _usable(venv_candidate):
                _CODEXCREW_BIN = str(venv_candidate)
                return _CODEXCREW_BIN
            if (parent / "pyvenv.cfg").exists():
                break
    except Exception:
        logger.debug("codexcrew venv bin check failed", exc_info=True)
    ...
    try:
        ...
        pkg_dir = Path(_mc.__file__).resolve().parent
        for parent in pkg_dir.parents:
            candidate = _codexcrew_bin_subpath(parent)
            if _usable(candidate):
                _CODEXCREW_BIN = str(candidate)
                return _CODEXCREW_BIN
            if (parent / "pyvenv.cfg").exists():
                break  # reached venv root without finding the binary
    except Exception:
        logger.debug("codexcrew bin walk failed", exc_info=True)
    ...
    #    ``<root>/lib/python3.12/site-packages/codex_crew`` and the script at
    #    ``<root>/python3.12/bin/codexcrew``, which is not an ancestor of the
    #    package dir at all. The walk then finds nothing and resolution falls
    #    through to PATH, where an unrelated ``codexcrew`` from some earlier
    #    install wins and gets written into ``codexcrew.json`` as the command for
    ...
    #    handing it to :func:`_codexcrew_bin_subpath` yields the same directory
    ...
    #    ``codex_crew`` package init, which can run with a user project on
    #    ``sys.path``, and a project-local ``sysconfig.py`` would then execute.
    try:
        candidate = _codexcrew_bin_subpath(Path(sys.exec_prefix))
        if _usable(candidate):
            _CODEXCREW_BIN = str(candidate)
            return _CODEXCREW_BIN
    except Exception:
        logger.debug("codexcrew exec-prefix bin check failed", exc_info=True)

    # 4. PATH lookup (also validated)
    found = shutil.which("codexcrew")
    if found and _usable(found):
        _CODEXCREW_BIN = found
        return _CODEXCREW_BIN

    # 5. Last resort — don't cache, so a future call can retry
    logger.warning(
        "Could not resolve codexcrew binary to an existing file; "
        "falling back to bare 'codexcrew' (MCP probes may fail)"
    )
    return "codexcrew"


def _managed_mcp_env() -> dict[str, str]:
    """Env every managed CodexCrew MCP server is launched with.

    Pins ``CODEXCREW_HOME`` when the gateway is running under an override, because
    ...
    return {"CODEXCREW_HOME": str(override)} if override else {}
...
# Codex Crew's whole reserved namespace plus the loader/interpreter channels.
#
...
# owns, and a name list fails open for the next CODEXCREW_ variable somebody
# adds -- the reachable case GPT found on this PR was CODEXCREW_CLI, which
...
# namespace. CODEXCREW_HOME is stripped by that same namespace rule and then
...
# managed shim resolves its data home when no ``CODEXCREW_HOME`` pin is present.
...
def _mcp_registry_mode() -> bool:
    ...
    ``config.local.json`` deep-merges over it and is where ``codexcrew config set
    ...
    try:
        ...
        return CodexCrewConfig.load().agent.mcp_registry_mode is True
    except Exception:
...
def _codexcrew_mcp_invocation(subcommand: str) -> tuple[str, list[str]]:
    """Resolve a CWD- and shebang-independent invocation for a built-in
    MCP server (``codexcrew-cron`` / ``codexcrew-core``).

    Prefers a standalone ``codexcrew`` binary when one resolves. Falls back
    to ``<interpreter> -m codex_crew <subcommand>`` when
    :func:`_resolve_codexcrew_bin` cannot find a usable standalone binary --
    ...
    ``_resolve_codexcrew_bin`` returns the bare ``"codexcrew"`` sentinel, the
    command fails to validate, and the server gets dropped from
    ``codexcrew.json`` on every config refresh.
    ...
    codex_crew`` dispatches the same CLI as the ``codexcrew`` console script.

    A resolved ``bin\\codexcrew.cmd`` (the Windows bundle's relocatable shim,
    see :func:`_codexcrew_bin_subpath`) is unwrapped to the sibling
    interpreter — ``<root>\\python.exe -P -s -m codex_crew <sub>`` — instead of
    ...
    """
    bin_path = _resolve_codexcrew_bin()
    if bin_path == "codexcrew":  # unresolved sentinel from _resolve_codexcrew_bin
        return sys.executable, ["-m", "codex_crew", subcommand]
    if bin_path.endswith(".cmd"):
        ...
        if _interpreter_runnable(interpreter):
            ...
            # so with ``-m`` alone a cloned repo carrying a ``codex_crew/``
            ...
            return str(interpreter), ["-P", "-s", "-m", "codex_crew", subcommand]
        return sys.executable, ["-m", "codex_crew", subcommand]
    return bin_path, [subcommand]


def _computer_use_spec_gate() -> bool:
    """Whether ``codexcrew-computer`` belongs in an EMITTED agent spec.
    ...
    **Neither condition loads a native library**, which matters because this gate runs
    ...
    try:
        ...
        from codex_crew.computer_use import backend as cu_backend
        from codex_crew.computer_use import enable_state
...
_MANAGED_MCP_SERVERS: dict[str, dict] = {
    "codexcrew-cron": {"invocation_fn": lambda: _codexcrew_mcp_invocation("mcp-cron")},
    "codexcrew-core": {"invocation_fn": lambda: _codexcrew_mcp_invocation("mcp-core")},
    ...
    "codexcrew-computer": {
        "invocation_fn": lambda: _codexcrew_mcp_invocation("mcp-computer"),
    ...
    # entry nor an ``@codexcrew-dashboard`` ref in ``tools`` — and kiro-cli loads a
    ...
    "codexcrew-dashboard": {
        "invocation_fn": lambda: _codexcrew_mcp_invocation("mcp-dashboard"),
        "opt_in": True,
    },
    # External model consultation is paid data egress, so it is a separate
    # assignable set. The API key stays in the gateway vault and this server
    # receives only prompts and redacted answers.
    "codexcrew-consult": {
        "invocation_fn": lambda: _codexcrew_mcp_invocation("mcp-consult"),
        "opt_in": True,
    },
}
...
def managed_mcp_spec_entry(name: str) -> dict[str, Any] | None:
    ...
    one caller here (the claude MCP translation, :mod:`codex_crew.acp.session_mcp`)
...
def ensure_codexcrew_on_path(
    bin_dir: Path | None = None, *, claim_existing: bool = False
) -> str | None:
    """Ensure a ``codexcrew`` launcher is reachable on the user's PATH.

    The source ``install.sh`` symlinks ``~/.local/bin/codexcrew`` → the venv
    entry point, but install paths that don't run it (notably the packaged
    Electron app) leave no ``codexcrew`` on PATH — breaking the ``codexcrew``
    terminal command. This mirrors that symlink step in Python so it runs from
    ``codexcrew setup``. Best-effort and idempotent:

    * No-op if ``codexcrew`` already resolves on PATH to the same binary.
    ...
    * Otherwise (re)create ``<bin_dir>/codexcrew`` → the resolved binary.
    ...
    Args:
        ...
        claim_existing: Take the name over from another install's working
            launcher. ``codexcrew setup`` passes True because the user named this
    ...
    # OSError [WinError 1314] mid-wizard. pip's Scripts\codexcrew.exe console
    ...
    # filesystem attempt so `codexcrew setup` never prints a traceback for it.
    if platform_compat.IS_WINDOWS:
        return None

    target = _resolve_codexcrew_bin()
    # Nothing concrete to point at — bare "codexcrew" or a non-executable file.
    ...
    # `codexcrew` stops working EVERYWHERE — not just in the tree that went away.
    # Any process running out of a worktree's venv (a pod gateway, a dev run, a
    # `codexcrew setup` invoked from that tree) resolves its own venv entrypoint
    ...
    if _in_linked_git_worktree(Path(target).resolve()):
        logger.info(
            "Not installing a codexcrew launcher: %s is inside a linked git worktree, "
            "which is ephemeral (removing the worktree would break `codexcrew` "
            "machine-wide). Install from your primary clone, or link it yourself: "
            "ln -sfn <clone>/.venv/bin/codexcrew ~/.local/bin/codexcrew",
            target,
        )
        return None
    ...
    if _in_ephemeral_tree(Path(target).resolve()):
        logger.info(
            "Not installing a codexcrew launcher: %s is inside an ephemeral tree (an "
            ...
            "`codexcrew` on PATH, or link a persistent install yourself.",
            target,
        )
        return None

    # Already reachable on PATH as the same binary? Then there's nothing to do.
    existing = shutil.which("codexcrew")
    if existing and os.path.realpath(existing) == os.path.realpath(target):
        return None

    # Ownership, checked on PATH before the target path: a working `codexcrew`
    # ANYWHERE on PATH already belongs to some install — a pipx bin dir, a distro
    # package, /usr/local/bin — and writing <bin_dir>/codexcrew would shadow it or
    ...
    # sitting AT <bin_dir>/codexcrew while <bin_dir> is not on PATH at all.
    if existing and not claim_existing:
        existing_on_path = Path(os.path.realpath(existing))
        if _launcher_works(existing_on_path):
            logger.info(
                "Leaving `codexcrew` on PATH alone: %s -> %s still works and belongs "
                "to another install. Run `codexcrew setup` from the install you want "
    ...
    try:
        ...
        if link.is_symlink() or link.exists():
            ...
            # the cli.sh wheel under ~/.codex-crew-venv — and taking the name from
            ...
            # would then land on a path nothing points at: `codexcrew` keeps
            ...
            #
            # An explicit `codexcrew setup` DOES claim the name: the user named
            ...
            if not claim_existing and _launcher_works(existing_target):
                logger.info(
                    "Leaving the existing codexcrew launcher alone: %s -> %s still "
                    "works and belongs to another install. Run `codexcrew setup` "
        ...
        logger.warning("Could not create codexcrew shim at %s: %s", link, exc)
        return None
    logger.info("Linked codexcrew shim: %s -> %s", link, target)
    return str(link)


# One-time migrations performed automatically on gateway first-run (so the
# desktop app, which never runs `codexcrew setup`, still gets them). Lazy
...
def run_first_run_setup() -> None:
    ...
    The Electron app only runs ``codexcrew gateway`` — never ``codexcrew
    ...
    * **PATH shim** — ``ensure_codexcrew_on_path()`` is idempotent and only
      writes ``~/.local/bin/codexcrew``, so it runs on every start. It is called
    ...
    * **Stale predecessor MCP purge** — ``clean_stale_managed_mcp()`` mutates
      ...
      guarded by a marker file, to honor the "CodexCrew owns only the agent
    ...
    try:
        shim = ensure_codexcrew_on_path()
        if shim:
            logger.info("First-run: linked codexcrew shim at %s", shim)
    except Exception:
    ...
    try:
        from codex_crew.platform.admission import seed_default_policy  # noqa: PLC0415
    ...
    try:
        from codex_crew.apps.manager import (  # noqa: PLC0415
    ...
    if stale_marker.exists():
        return
    try:
        from codex_crew.mcp_cleanup import clean_stale_managed_mcp  # noqa: PLC0415
...
def _all_skill_paths() -> list[str]:
    ...
    - ``CODEXCREW_PROJECT_DIR/skills`` (project-level)
    - ``~/.codex-crew/skills`` (user-created)
    """
    paths: set[str] = set()
    ...
    # User-created skills (CodexCrew convention)
...
# Codex Crew-internal hook keys that must NOT appear in generated kiro-cli agent  # brand-ok
...
# only legacy keys Codex Crew serialized. A new event added to defaults.json is
...
# Repair is subtractive against the runtime-only key Codex Crew is known to have
# serialized into its generated specs. Unknown keys may belong to a newer
# kiro-cli schema or to the user.
_LEGACY_CODEXCREW_HOOK_KEYS = frozenset({"auto_approve_tools"})
...
def _strip_legacy_denied_commands(config: dict) -> None:
    ...
    Denied commands are now enforced solely at CodexCrew's hooks.py PreToolUse
    ...
    in its ``codexcrew.json``. kiro-cli would keep enforcing those stale rules
...
def _apply_user_kiro_hooks(config: dict, mc_cfg: dict) -> None:
    """Merge user-defined kiro_hooks from codexcrew config into *config* (additive).

    Two sources, explicit first then auto-discovered:

      1. ``agent.kiro_hooks`` in ``~/.codex-crew/config.json`` -- explicit entries
...
def _enforce_managed_mcp_ownership(
    ...
    *,
    ...
    """Strip/re-pin the fields Codex Crew owns on one managed-server entry.
    ...
    * ``"preserve"`` (refresh, entry present) -- leave it alone, so a user who
    ...
    # takes every Codex Crew tool down with it. Dropping the key we cannot honour
    ...
    # agent.json value. sanitize_spec_env then drops Codex Crew's whole reserved
    ...
    # (see _HOME_DERIVING_ENV_KEYS), and CODEXCREW_HOME is re-pinned to the
...
def build_agent_config(*, gated_off: "frozenset[str] | None" = None) -> dict:
    ...
    LONGER injected here — command denial is enforced at CodexCrew's own
    hooks.py PreToolUse gate, not via the kiro agent spec. User-defined
    ``kiro_hooks`` from ``~/.codex-crew/config.json`` are then additively merged;
    ...
    # Strip Codex Crew-internal keys (auto_approve_tools etc.) that kiro-cli  # brand-ok
    ...
    # Merge user-defined kiro_hooks from ~/.codex-crew/config.json (additive).
...
def _refresh_dynamic_fields(config: dict, *, gated_off: "frozenset[str] | None" = None) -> None:
    ...
    Called when ``codexcrew.json`` already exists so user customizations are
    ...
    # Self-heal: lift any stray CodexCrew bookkeeping keys into the sidecar and
    ...
    from codex_crew.config.loader import DEFAULT_MODEL, normalize_agent_model
    ...
    # config.json agent.model is the user-facing authority (codexcrew config set
...
def _load_existing_config(
    ...
    """Load and refresh an existing codexcrew.json.
...
def _norm_mcp_spec(spec: Any) -> Any:
    ...
    :data:`~codex_crew.mcp_provenance.DERIVED_KEY` is excluded for the same reason,
...
def _apply_connection_tool_aliases(
    ...
    :mod:`codex_crew.connections.tool_aliases` owns invariants 1-6, which decide
    ...
    * **The generated subset is read from a PERSISTED record, not inferred from a
      ...
      default. See :mod:`codex_crew.connections.alias_record` for the generation
    ...
    * **A generated alias never lands on a name already in use.** The destination
    ...
    try:
        # Imported here, not at module scope: codex_crew.connections.registry
        # validates the committed registry at MODULE level, so a missing or
        # malformed registry.json would raise on `import codex_crew.agent` --
        ...
        from codex_crew.connections.alias_record import (  # noqa: PLC0415
            emitted_from_alias_map,
            is_recorded_emission,
            spec_fingerprint,
        )
        from codex_crew.connections.tool_aliases import (  # noqa: PLC0415
...
def migrate_agent_specs() -> int:
    """Strip CodexCrew bookkeeping keys from kiro agent specs into the sidecar.
    ...
    if cleaned:
        logger.info("Cleaned %d kiro agent spec(s) of CodexCrew bookkeeping keys", cleaned)
    return cleaned
...
def clear_model_pin(config: MutableMapping[str, object], name: str) -> None:
    ...
    the editor, or ``codexcrew agent reset-model`` -- and never by a heuristic
...
def _conflicting_spec_for(name: str, chosen: Path, agents_dir: Path) -> Path | None:
    ...
    The runtime resolver (``CodexCrewConfig._resolve_named_agent_model``) accepts
...
def reset_agent_model(name: str) -> tuple[Path, str]:
    ...
    The explicit, narrow counterpart to ``codexcrew setup --clean``, which also
...
def _decline_shared_agent_home(*, audit: bool = True) -> Path | None:
    ...
    An **ephemeral** CodexCrew instance — one booted from a linked git worktree, or
    one running on its own isolated ``CODEXCREW_HOME`` (a pod) — is throwaway by
    ...
    Declining mirrors ``ensure_codexcrew_on_path``'s worktree guard: leave whatever
    ...
    * A **pod running from the primary checkout** is not in a linked worktree at
      ...
      ``CODEXCREW_POD`` (set in ``build_pod_env``) and that counts as ephemeral on
      its own. Note what is deliberately NOT used as the signal: merely *having*
      an isolated ``CODEXCREW_HOME``. A CI test gateway (the offline E2E suite boots
    ...
    * A globally exported ``KIRO_HOME`` moves the shared directory, so comparing
    ...
    relocates kiro-cli's session storage while CodexCrew still reads the host path
    ...
    if own_home is not None and target == isolated_agents_dir(own_home).resolve():
        ...
        # (CODEXCREW_HOME=$HOME suffices), handing an ephemeral instance the very
    ...
    # "Has an isolated CODEXCREW_HOME" is NOT that: a CI test gateway and a user
    ...
    # (``Mode 'codexcrew' not found``). ``_in_ephemeral_tree`` is the same
    ...
    # dead (every turn fails with ``Mode 'codexcrew' not found``). #4781's harm is
    ...
    if audit:
        logger.warning(
            ...
            "KIRO_HOME also relocates kiro-cli's session storage, which Codex Crew still "
...
def _seed_kas_permissions(config: dict[str, Any]) -> None:
    ...
    **Seed, never refresh.** Once the key exists it belongs to whoever edits the
    ...
    # permitted to import ``codex_crew.acp``, and agent.py's direct-import count
    ...
    # field.
    from codex_crew.agent_sdk.drivers.acp import (  # noqa: PLC0415 - boot path
...
def _apply_allowed_tools_ceiling(config: dict, *, source: str) -> None:
    ...
    which writes only ``codexcrew.json`` (#7401).
...
def _ceiling_filtered_spec(ref: str, spec: dict[str, Any]) -> dict[str, Any]:
    ...
    that can come from outside this repo — rather than by CodexCrew or the user.
...
def _collect_app_mcp_servers() -> dict[str, Any]:
    ...
    the app. CodexCrew sessions only ever read the agent config
    ...
    try:
        # Imported lazily: codex_crew.apps imports back into agent/security, so a
        # module-level import here would close a cycle.
        from codex_crew.apps.bridges import registered_app_mcp_servers
        from codex_crew.apps.manager import get_app_manifest, is_app_enabled, list_apps
...
def _durable_tool_aliases(path: Path) -> tuple[bool, object]:
    ...
    Returns:
        ...
        :func:`~codex_crew.connections.alias_record.spec_fingerprint` both read as the
...
def reproject_for_ceiling_change() -> None:
    ...
    forbids — the tool short-circuits inside the harness and never reaches Codex Crew's own
    PreToolUse gate.
...
def rebuild_agent_config(*, clean: bool = False) -> Path:
    """Rebuild and write the merged codexcrew.json to ~/.kiro/agents/.
    ...
    Merge priority (highest wins):
      1. ~/.codex-crew/mcp.json (agent-specific overrides)
      2. ~/.kiro/settings/mcp.json (kiro global, fills gaps)
      3. Existing codexcrew.json (preserves user customizations)
    ...
    codexcrew MCP server commands) are refreshed from defaults.
    ...
    # One-time (idempotent) self-heal: strip CodexCrew bookkeeping keys from
    ...
    # config) FIRST.  CodexCrew is kiro-first (ACP/kiro-cli only), so Kiro
    ...
    # entries.  Write-through is never done here (CodexCrew reads globals but
    # never mutates them).
    #
    ...
    for name, spec in shared_mcp.items():
        if isinstance(spec, dict) and name not in managed_names:
            ...
            # (codexcrew merge) must not mutate shared_mcp, which is reused as a
    ...
    # ~/.codex-crew/mcp.json overrides kiro mcp.json for the codexcrew agent —
    # codexcrew-specific config wins in a tie.
    # Uses update() to merge into existing specs, preserving user-set fields
    # like autoApprove while letting codexcrew's command/args/env win.
    # Skip managed servers for the same reason as above.
    codexcrew_mcp = _load_json(_user_dir() / "mcp.json").get("mcpServers", {})
    for name, spec in codexcrew_mcp.items():
        if isinstance(spec, dict) and name not in managed_names:
    ...
    # (codexcrew > kiro-global > provider-global).  This prevents one source's
    ...
    def _resolve_command(cmd: str, env: dict | None) -> tuple[str | None, str]:
        ...
        config ("command not found: codexcrew"). The value EMITTED into the spec
        ...
        fallback for pip-generated wrappers like ``codexcrew``.
        """
        if not cmd:
            return None, ""
    ...
    for _n, _s in codexcrew_mcp.items():
    ...
    for name, spec in _cfg_servers.items():
        ...
        if spec.get("url"):
            ...
            # in the agent config itself (``kiro-cli mcp add --agent codexcrew``, a
            ...
            _store_entry = codexcrew_mcp.get(name)
    ...
    # re-injected from ~/.codex-crew/mcp.json by the merges above.  The
    ...
    #
    # ``codexcrew_mcp`` is in this chain for the same reason: it holds every entry
    ...
    _disabled_anywhere = {
        ...
        for scope in (extra_shared_mcp, shared_mcp, codexcrew_mcp)
    ...
    for name, spec in itertools.chain(
        extra_shared_mcp.items(), shared_mcp.items(), codexcrew_mcp.items()
    ):
    ...
    # branch above is the only place it is added — so ``codexcrew-computer`` is
    ...
    # codexcrew.json has TWO independent-locked writers: this regenerating one and
    ...
    # (Only for codexcrew.json — every other agent file this may write has a single
    # writer.) The re-read uses the UNLOCKED reader because we already hold the lock.
    from codex_crew.apps.bridges import (
        _mcp_json_path,
        _mcp_lock,
    ...
    def _finalize_and_write() -> None:
        ...
        # generated pass does.
        #
        # Imported inside a try: `codex_crew.connections.alias_record` is a submodule,
        # so importing it executes `codex_crew.connections.__init__`, which eagerly
    ...
    try:
        is_codexcrew_json = path.resolve() == _mcp_json_path().resolve()
    except OSError:
        is_codexcrew_json = False
    if is_codexcrew_json:
        with _mcp_lock():
            ...
            if isinstance(on_disk, dict):
                ...
                try:
                    from codex_crew.apps.manager import is_app_enabled
    ...
    # Install CodexCrew AIM capabilities package (includes codexcrew-lite)
    _install_aim_capabilities()

    # Install codexcrew-knowledge agent (used by Knowledge Library LLMPool)
    try:
        _install_knowledge_agent()
    except Exception:
        logger.debug("codexcrew-knowledge agent install failed", exc_info=True)

    # Install codexcrew-research agent (used by the Research Lab campaign loop)
    try:
        _install_research_agent()
    except Exception:
        logger.debug("codexcrew-research agent install failed", exc_info=True)

    # Install codexcrew-heartbeat agent (used by HeartbeatService for unattended polling)
    try:
        _install_heartbeat_agent()
    except Exception:
        logger.debug("codexcrew-heartbeat agent install failed", exc_info=True)

    # Install codexcrew-conductor agent (goal decomposition + session-control dispatch)
    try:
        _install_conductor_agent()
    except Exception:
        logger.debug("codexcrew-conductor agent install failed", exc_info=True)

    # Install codexcrew-pipeline-conductor agent (repository pipeline fleet supervision)
    try:
        _install_pipeline_conductor_agent()
    except Exception:
        logger.debug("codexcrew-pipeline-conductor agent install failed", exc_info=True)
...
def ensure_agent_materialized(agent: str | None) -> bool:
    ...
    Normally ``codexcrew setup --agent-only`` writes it, but a source checkout /
    dev launch that skips setup leaves it absent — this makes the runtime
    self-sufficient regardless.

    Only the managed default (``AGENT_FILENAME`` → ``codexcrew.json``) is
...
def _install_aim_capabilities() -> None:
    """Write a bare ``codexcrew-lite`` agent config.
    ...
    Amazon-internal package manager); the generic ``codexcrew-lite`` fallback
...
def _install_lite_agent_fallback() -> None:
    """Write a bare codexcrew-lite config (cheap background agent)."""
    ...
    # stays schema-clean).
    agent_state.set_cc_model("codexcrew-lite", _background_cc_model())


_KNOWLEDGE_SYSTEM_PROMPT = (
    "You are a knowledge extraction specialist for CodexCrew's Knowledge Library. "
...
def _install_knowledge_agent() -> None:
    """Generate and install the codexcrew-knowledge agent config.
...
_RESEARCH_SYSTEM_PROMPT = """# CodexCrew Research Worker

You are `codexcrew-research`, an autonomous research worker. You run ONE research
...
def _install_research_agent() -> None:
    """Generate and install the codexcrew-research agent config.

    Derives from the codexcrew agent (MCP servers, security, tools) but swaps in a
    ...
    config["name"] = "codexcrew-research"
    config["description"] = (
...
_CONDUCTOR_SYSTEM_PROMPT = """# Codex Crew Conductor

You are `codexcrew-conductor`. You own a long-horizon goal: you decompose it into
...
#: by one rather than as the whole ``@codexcrew-dashboard`` server.
#:
...
#:   non-destructive).
#:
#: Every withheld verb stays MOUNTED (``@codexcrew-dashboard`` is still in
...
_CONDUCTOR_DASHBOARD_GRANTS: tuple[str, ...] = (
    "@codexcrew-dashboard/chat_folder_tree",
    "@codexcrew-dashboard/chat_folder_create",
    "@codexcrew-dashboard/session_create",
    "@codexcrew-dashboard/session_read_message",
)
...
_MEMBER_DASHBOARD_GRANTS: tuple[str, ...] = _CONDUCTOR_DASHBOARD_GRANTS + (
    "@codexcrew-dashboard/session_send",
    "@codexcrew-dashboard/session_stop",
)


#: The codexcrew-core verbs the goal conductor may call WITHOUT an approval
#: prompt. Named one by one rather than as the whole ``@codexcrew-core`` server,
...
#: the tools that actually register on ``codexcrew-core`` — the ``session_*``
#: and ``chat_folder_*`` verbs the charter also names are
#: ``@codexcrew-dashboard`` and are granted by the tuple above, while
...
_CONDUCTOR_CORE_GRANTS: tuple[str, ...] = (
    "@codexcrew-core/monitor_start",
    "@codexcrew-core/monitor_update",
    "@codexcrew-core/autonudge_stop",
    "@codexcrew-core/wait",
    "@codexcrew-core/resource_status",
    "@codexcrew-core/list_sessions",
    "@codexcrew-core/session_ledger_read",
    "@codexcrew-core/session_ledger_record",
    "@codexcrew-core/skill_search",
    "@codexcrew-core/skill_fetch",
    "@codexcrew-core/select_crew",
    "@codexcrew-core/send_message",
    "@codexcrew-core/send_notification",
    "@codexcrew-core/ask_question",
)


def _install_conductor_agent() -> None:
    """Generate and install the codexcrew-conductor agent config.

    Derives from the codexcrew agent (resolved MCP invocations, security hooks)
    ...
    rather than of the prompt. The ``codexcrew-dashboard`` server is the opt-in
    ...
    ``@codexcrew-core`` and ``@codexcrew-dashboard`` are both MOUNTED whole but
    ...
    false — the default. Sharing the name would let ``codexcrew setup`` erase the
    packaged skill on a stock install, and quarantine the user's delegation
    skill when the flag is on.
    """
    config = build_agent_config()
    config["name"] = "codexcrew-conductor"
    config["description"] = (
    ...
    core_entry = mcp.get("codexcrew-core")
    narrowed: dict = {}
    if core_entry:
        narrowed["codexcrew-core"] = core_entry
    dash_cmd, dash_args = _codexcrew_mcp_invocation("mcp-dashboard")
    ...
    # dead with no local error; without the `CODEXCREW_HOME` pin the shim reads the
    ...
    narrowed["codexcrew-dashboard"] = dash_entry
    ...
    # its KAS rule with it (a hand-written ``codexcrew-core/*`` allow would have
    ...
    # layer permitted to import ``codex_crew.acp``, and agent.py's direct-import
...
_PIPELINE_CONDUCTOR_SYSTEM_PROMPT = """# Codex Crew Pipeline Conductor

You are `codexcrew-pipeline-conductor`. You run ONE pipeline on ONE repository:
...
#: The codexcrew-core verbs the pipeline conductor may call WITHOUT an approval
#: prompt. Named one by one rather than as the whole ``@codexcrew-core`` server,
...
#: too; it registers on ``codexcrew-cron``, which neither conductor mounts, so
...
def _install_pipeline_conductor_agent() -> None:
    """Generate and install the codexcrew-pipeline-conductor agent config.
    ...
    the codexcrew agent, **no dedicated file-writing tool** (neither ``fs_write``
    nor ``code``), ``@codexcrew-dashboard`` mounted whole but auto-approved only
    ...
    """
    config = build_agent_config()
    config["name"] = "codexcrew-pipeline-conductor"
    config["description"] = (
    ...
    # to import ``codex_crew.acp``, and agent.py's direct-import count is a
...
_HEARTBEAT_SYSTEM_PROMPT = """# CodexCrew Heartbeat Worker

You are `codexcrew-heartbeat`, an unattended polling worker that runs one task
...
and side-effect-free codexcrew-core reads). Anything outside that list is
...
def _install_heartbeat_agent() -> None:
    """Generate and install the codexcrew-heartbeat agent config.

    A dedicated agent for HeartbeatService.  Minimal MCP surface — only
    ``codexcrew-core`` (learn/cron/spawn list, recall, artifacts read) on
    ...
    # Pull the ``codexcrew-core`` entry from the main agent config so the
    ...
    agent_state.set_cc_model("codexcrew-heartbeat", _background_cc_model())
...
def repair_agent_configs() -> None:
    """Remove legacy Codex Crew hook keys from agent configs owned by Codex Crew."""
...
def _sanitize_agent_hooks() -> None:
    ...
    default agent — losing codexcrew-core, codexcrew-cron.

    Auto-repairs configs carrying keys Codex Crew wrote in prior versions. Files
    outside :data:`OWNED_KIRO_AGENT_FILES` and unrecognized hook keys are left
    untouched because Codex Crew does not own their schema or contents.
    """
    agents_dir = kiro_agents_dir_path()
    for filename in OWNED_KIRO_AGENT_FILES:
        ...
        if not isinstance(hooks, dict):
            ...
            continue
        removed_keys = [key for key in hooks if key in _LEGACY_CODEXCREW_HOOK_KEYS]
        if not removed_keys:
            _hooks_sanitized_mtimes[str(f)] = mtime
            continue
        data["hooks"] = {
            key: value for key, value in hooks.items() if key not in _LEGACY_CODEXCREW_HOOK_KEYS
        }
        _atomic_json_write(f, data)
        _hooks_sanitized_mtimes[str(f)] = f.stat().st_mtime
        logger.info("Removed legacy Codex Crew hook keys %s from %s", removed_keys, f.name)
        sel().log_api_access(
            caller="system",
