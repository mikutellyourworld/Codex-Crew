"""Explicit operator consent before Codex Crew spends money in an AWS account.

Two optional features reach a PAID AWS service through the provider's own
credential chain: Amazon Polly (text-to-speech, :mod:`codex_crew.voice_reply`)
and Amazon Transcribe (speech-to-text, :mod:`codex_crew.transcribe` and
:mod:`codex_crew.dashboard.stt_stream`). Both omit ``--profile`` / pass no
...
from codex_crew.constants import AWS_PROFILE_FIRST_CHARS
...
async def probe_identity(profile: str, region: str, *, use_cache: bool = True) -> Identity:
    ...
    Delegated to :func:`codex_crew.cloud.aws.run_aws` rather than spawning the CLI
...
def _run_aws(args: list[str], profile: str, region: str) -> tuple[int, str, str]:
    ...
    Imported at call time, not module scope: ``codex_crew.cloud`` is an optional
    provisioning subsystem, and the voice/STT paths that import THIS module for
    the local gate must not pay for it.
    """
    from codex_crew.cloud.aws import run_aws
