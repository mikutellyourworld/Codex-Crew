"""``codexcrew seed`` — copy a hand-authored fixture into ``$CODEXCREW_HOME``.

Fixtures ship as package data at ``src/codex_crew/tests_fixtures/<name>/``.
Each fixture is a valid ``CODEXCREW_HOME`` tree with a ``fixture.yaml``
declaring ``schema-version``. Shipping inside the package is what lets
``codexcrew seed`` work after ``pip`` installs the wheel into
``site-packages/codex_crew/`` (a source-tree-relative path walk would
...
except ImportError:  # pragma: no cover — CodexCrew targets py3.10.
    _resource_files = None  # type: ignore[assignment]

# ``codex_crew.sel`` is a first-party module shipped in the same package, so
# we import it at top level like every other caller (``subagent.py``,
# ``taskrunner.py``, ``dashboard/stt_stream.py``, ...). An ImportError here
# would mean the CodexCrew install itself is broken — there's no scenario
...
def _fixtures_root() -> Path:
    ...
    workspaces (``src/codex_crew/tests_fixtures/``) and in the pip-installed
    layout where the package lives under
    ``site-packages/codex_crew/``. ``files()`` returns a ``Traversable``
    which, for the regular filesystem loader CodexCrew uses, is a real
    ...
    return Path(str(_resource_files("codex_crew") / _FIXTURES_PKG))
...
def _resolve_fixture(name: str) -> Path:
    ...
    - Empty or ``.``-only names resolve to the fixtures root itself — would
      copy the whole ``tests_fixtures/`` tree into ``$CODEXCREW_HOME``.
    ...
    if not candidate.is_dir():
        ...
        # ``src/codex_crew/tests_fixtures/`` or the PRD. Sorted for stable
...
def _protected_homes() -> set[Path]:
    ...
    * ``_default_home()`` → ``~/.codex-crew`` — the current main gateway home.
    * ``_legacy_home()`` → ``~/.codexcrew`` — the pre-move home. A box that
    ...
    purpose: seeding requires ``$CODEXCREW_HOME`` to be set, so ``config_dir()``
...
def _resolve_target(*, for_main_home_check: bool = False) -> Path:
    """Return ``$CODEXCREW_HOME`` as a ``Path``, or raise ``SeedError``.

    ``expanduser()`` is applied so ``CODEXCREW_HOME=~/dev`` works. Pass
    ``for_main_home_check=True`` to additionally ``resolve()`` the path so
    a symlinked ``CODEXCREW_HOME`` pointing at ``~/.codex-crew`` is caught by
    ...
    """
    raw = os.environ.get("CODEXCREW_HOME")
    if not raw:
        raise SeedError(
            "$CODEXCREW_HOME is not set. Point it at a dev directory "
            "(e.g. CODEXCREW_HOME=~/.codexcrew-dev codexcrew gateway --seed empty).",
    ...
    if for_main_home_check:
        ...
        # catches ``$CODEXCREW_HOME -> ~/.codex-crew`` even when the symlink
        ...
        # ``~/.codex-crew`` that ``resolve()`` couldn't evaluate, the
        ...
        try:
            ...
            raise SeedError(
                f"cannot resolve $CODEXCREW_HOME ({raw!r}) for main-home "
...
def seed(fixture_name: str, *, replace: bool = False) -> None:
    """Copy fixture ``fixture_name`` into ``$CODEXCREW_HOME``.

    Raises ``SeedError`` on guardrail violations. Enforces, in order:

    1. **Main-home guardrail** — refuses when ``$CODEXCREW_HOME`` resolves to
       ``~/.codex-crew`` (the dev's live gateway home). This guardrail is
    ...
    # ``$CODEXCREW_HOME`` into its target and we'd copy/rmtree the wrong
    # location; conversely, the raw path can't be trusted for the main-home
    # guardrail because ``CODEXCREW_HOME=~/my-link`` where ``my-link -> ~/.codex-crew``
    ...
    if dst_resolved in _protected_homes():
        raise SeedError(
            ...
            "Point $CODEXCREW_HOME at a separate dev directory "
            "(e.g. ~/.codexcrew-dev).",
    ...
    if dst.is_symlink() and not dst.exists():
        raise SeedError(
            f"$CODEXCREW_HOME is a dangling symlink: {dst}. "
    ...
    # ``$CODEXCREW_HOME`` pointing at a regular file (or a symlink to one) would
    ...
    if dst.exists() and not dst.is_dir():
        raise SeedError(
            f"$CODEXCREW_HOME is not a directory: {dst}. "
    ...
    if dst.exists() and any(dst.iterdir()):
        ...
        if dst.is_symlink():
            raise SeedError(
                f"refusing to seed into a symlinked $CODEXCREW_HOME: {dst}. "
                "Point it at a real directory.",
                guardrail=SeedError.GUARDRAIL_SYMLINK_REPLACE,
            )
        if not replace:
            raise SeedError(
                f"$CODEXCREW_HOME is not empty: {dst}. "
...
def seed_cmd(args) -> int:  # noqa: ANN001 — argparse.Namespace at call site
    ...
    directory tree into ``$CODEXCREW_HOME`` and 1.B's ``--seed-replace`` will
    ...
    # this flag is always ``True`` on the success path (if ``CODEXCREW_HOME``
    ...
    target_set = "CODEXCREW_HOME" in os.environ
    try:
