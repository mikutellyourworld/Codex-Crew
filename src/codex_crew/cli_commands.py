...
from codex_crew import __version__, beacon, platform_compat
from codex_crew.agent import reset_agent_model
...
from codex_crew.apps.scaffold import scaffold_app
from codex_crew.cli_server import _marker_port, resolve_client_port
...
from codex_crew.cron import CronSchedule, CronService, CronStoreUnreadable, format_schedule
from codex_crew.cron_trigger import trigger_cron_job
from codex_crew.dashboard import tailnet, tailnet_serve
from codex_crew.dashboard.origin import parse_dashboard_url
from codex_crew.eval.judge import LLMJudge
from codex_crew.eval.runner import EvalRunner, format_results, score_by_dimension
from codex_crew.eval.scenario import AssertionType, load_scenario, load_scenarios
...
from codex_crew.secrets.migrate import (
...
from codex_crew.vector_memory import LessonWriteOutcome, VectorMemoryStore, _lesson_display_text
...
_WS_DIR_OUTSIDE_HOME = (
    "Error: --dir must resolve inside the CodexCrew data home ({home}); got {given!r}. "
...
def _ws_dir_resolves_inside_home(ws_dir: str) -> bool:
    ...
    ``config_dir() / ws_dir`` WITHOUT expanding ``~``, so ``~/.codex-crew`` used to
    become ``<home>/~/.codex-crew`` there — unequal to the root, hence accepted —
...
def _internal_secret(port: int) -> str:
    ...
    The gateway writes ``~/.codex-crew/.local_secret`` (mode 0600) after a
    ...
    browser cookie. Mirrors `codex_crew.mcp_core._internal_secret`.
...
def _spawn(args: argparse.Namespace) -> None:
    ...
    print("Usage: codexcrew spawn {run|list}")
...
def _handle_workspace(args: argparse.Namespace) -> None:
    ...
    if action == "list":
        ...
        print("Usage: codexcrew workspace {list|create|update|delete}")
...
def _run_app_mcp_server(app_name: str) -> None:
    ...
    """
    module_name = f"codex_crew.apps.builtins.{app_name.replace('-', '_')}.mcp_server"
    try:
    ...
    if action == "install":
        ...
        if result.ok:
            ...
            if reg.errors:
                for e in reg.errors:
                    print(f"   ⚠️  {e}")
            print(f"\n   Run: codexcrew app enable {result.name}")
        else:
        ...
        from codex_crew.apps.dev_mode import set_dev_mode
        ...
        if enabled:
            ...
            print(f"   Turn off with: codexcrew app dev {args.name} --off")
        else:
        ...
        print(f"   codexcrew app install {app_dir}")

    else:
        print("Usage: codexcrew app {install|list|enable|disable|uninstall|info|init}")
    ...
    if action == "list":
        ...
        for name, agent in cfg.agents.items():
            ...
            print(
                ...
                f"{getattr(agent, 'source', 'codexcrew'):<12}"
            )
        ...
        if args.name in cfg.agents:
            ...
            sys.exit(1)
        cfg.agents[args.name] = CodexCrewAgentConfig(
        ...
        print("Usage: codexcrew agent {list|create|update|delete|reset-model}")


def _agent_reset_model(args: argparse.Namespace) -> None:
    """Clear an agent spec's pinned model (``codexcrew agent reset-model``).
    ...
    editor (clear the model) and ``codexcrew setup --clean``, which also
    regenerates the whole spec and discards every other customization with it.
    """
    name = getattr(args, "agent", None) or "codexcrew"
    try:
...
def _cron_dispatch(args: argparse.Namespace) -> None:
    ...
    if action == "list":
        ...
        # CODEXCREW_PORT only, so on a --port auto gateway it names a sibling, and the
        ...
        print("Usage: codexcrew cron {list|add|update|remove|pause|resume|trigger|preview}")
...
def _cron_preview(args: argparse.Namespace) -> None:
    ...
    from codex_crew.cron_script import Done, McpToolClient, Report, Skip, resolve_script_path

    # Resolve and validate script path (same validation as production cron runner:
    # format, existence, sensitive path, containment under ~/.codex-crew/crons/)
    try:
    ...
    class _LiveTestCtx:
        ...
        Acceptable because: scripts are constrained to ~/.codex-crew/crons/ via
...
def _security(args: argparse.Namespace) -> None:
    ...
    if action == "deny-list":
        ...
        print("Usage: codexcrew security {audit|deny-list|events|verify}")
...
def _policy(args: argparse.Namespace) -> None:
    ...
    """
    from codex_crew.platform.context import current_context
    ...
    if action == "show":
        ...
        from codex_crew.platform.governance_profiles import _profiles_dir
        ...
        if prof is None:
            print(f"No profile named {args.name!r} in ~/.codex-crew/profiles/.")
            return
        ...
        print(
            "Usage: codexcrew policy {show|validate|explain <scope> <item>|"
...
def _learn(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if action == "add":
            ...
            print("Usage: codexcrew learn {add|list|remove}")
    finally:
        vs.close()
...
def _memory_cmd(args: argparse.Namespace) -> None:
    ...
    try:
        if action == "list":
            ...
            if not import_file:
                print("Usage: codexcrew memory import <file>")
                return
            ...
            print("Usage: codexcrew memory {list|search|show|stats|audit|export|migrate|import}")
    finally:
        store.close()
...
def _artifact(args: argparse.Namespace) -> None:
    ...
    if action == "save":
        ...
        if taken:
            print(
                ...
                f"codexcrew artifact update {taken}",
                file=sys.stderr,
            )
    ...
    print(
        "Usage: codexcrew artifact {list|show|save|update|delete|versions}",
        file=sys.stderr,
    )
    sys.exit(2)
...
def _pod(args: argparse.Namespace) -> None:
    """Dispatch ``codexcrew pod <verb>`` to the pod verb layer (isolated worktree
    test instances)."""
    from codex_crew.pod.cli import dispatch
...
def _container_valued_sections() -> dict[str, type]:
    ...
    ``CodexCrewConfig`` later is covered without anyone remembering to edit this.
    ...
    """
    out: dict[str, type] = {}
    for field in dataclasses.fields(CodexCrewConfig):
        ann = field.type
...
def _assert_config_sections_are_objects(raw: dict) -> None:
    ...
    ``CodexCrewConfig.load()``, and ``load()`` is itself destructive on a file it
...
def _tailnet(args: argparse.Namespace) -> None:
    """Publish, withdraw, or inspect tailnet dashboard access (``codexcrew tailnet``).
    ...
    Codex Crew only ever did the second. Doing one without the other is the failure
    ...
    # Validate the raw file BEFORE anything calls ``CodexCrewConfig.load()``, for
    ...
    # ``resolve_client_port`` → ``CodexCrewConfig.load()``, so there is no version of
    ...
    # marker wins: it only reports a port where a Codex Crew gateway process is actually
    ...
    #
    # An explicit ``--port``/``CODEXCREW_PORT`` still wins over both, because that is
    ...
    if not port and os.environ.get("CODEXCREW_PORT") is not None:
        port_source = "CODEXCREW_PORT"
    ...
    if action == "up" and not port_source:
        ...
        # the run marker only reports a port a Codex Crew gateway is actually listening
        ...
        print(
            ...
            f"(`codexcrew dashboard`) and re-run, or name the port yourself with "
            f"`codexcrew tailnet up --port <port>`.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if action == "status":
        ...
        print(">_ Tailnet dashboard access")
        if pinned:
            print(
        ...
        if name and enabled and published is not True:
            print("   Next:       codexcrew tailnet up")
        return
    ...
    if not enabled:
        ...
        #    enables the setting, `codexcrew tailnet up` is a single command forever;
        ...
        print(
            ...
            "`codexcrew config set --local dashboard.tailscale.enabled true`.)",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print(">_ Restart the gateway for the tailnet origin to be trusted.")


def _telemetry(args: argparse.Namespace) -> None:
    """Inspect or toggle the anonymous usage beacon (``codexcrew telemetry``).
    ...
    try:
        effective = CodexCrewConfig.load().telemetry.beacon_enabled
    ...
    if want:
        ...
        print("   Run 'codexcrew telemetry status' to see exactly what is sent.")
    else:
    ...
    if action == "import":
        ...
        try:
            ...
            print(
                ...
                f"store, then re-run `codexcrew secrets import --apply`.",
                file=sys.stderr,
            )
            sys.exit(1)
        print(format_report(report))
    else:
        print("Usage: codexcrew secrets import [--apply]", file=sys.stderr)
        sys.exit(1)
