...
from codex_crew.credential_patterns import AWS_KEY_ID_PREFIXES
...
def _default_dir() -> Path:
    ...
    :mod:`codex_crew.sel` never triggers the one-time data-home migration as an
...
def _redact_text(text: str) -> str:
    ...
    """
    # circular import: codex_crew.security imports SecurityEvent/SecurityEventLog
...
class SecurityEvent:
    ...
    agent: str  # agent name (codexcrew, custom, etc.)
...
class SecurityEventLog:
    ...
    def _forward_event(self, callback: Callable[[dict], None], event: SecurityEvent) -> None:
        ...
        try:
            # circular import: codex_crew.security imports SecurityEvent/
...
def sel_hmac_key_path() -> Path:
    ...
    (e.g. via ``config_dir()``; ``_default_dir()`` honors ``CODEXCREW_HOME`` the
