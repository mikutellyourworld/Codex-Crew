...
from codex_crew.mcp_grant import grant_observed
from codex_crew.mcp_provenance import ABSENT, resolve_write
from codex_crew.mcp_utils import kiro_entry_client_id, kiro_entry_scopes, mcp_server_alias
...
def _get_probe_timeout() -> int:
    try:
        ...
        return CodexCrewConfig.load().dashboard.mcp_probe_timeout_secs
    except Exception:
...
def _warn_probe_sandbox_unavailable_once(name: str) -> None:
    ...
    logger.warning(
        ...
        "Codex Crew cannot spawn the server to enumerate its tools. The server "
...
# the dashboard badges (codexcrew / kiroGlobal / ccGlobal) and are the
# source of truth for the ``presence`` field on each server.
SCOPE_CODEXCREW = "codexcrew"
...
def _mcp_sources() -> tuple[tuple[Path, str], ...]:
    ...
    The tuple is in merge/priority order, highest first: the codexcrew-specific
    ...
    if _MCP_SOURCES is not None:
        return _MCP_SOURCES
    return (
        (data_home() / "mcp.json", SCOPE_CODEXCREW),
...
def _extra_scope_sources() -> list[tuple[Path, str]]:
    ...
    Each returned :class:`~codex_crew.platform.interfaces.McpScope` becomes a
...
# codexcrew-specific file, then the Kiro global. ``ccGlobal`` (and every other
...
# (codexcrew > kiro-global > seam provider globals) — discovery, apply, and
# rebuild all agree, so the dashboard never shows a spec the agent won't run.
_CORE_SCOPE_ORDER: tuple[str, ...] = (SCOPE_CODEXCREW, SCOPE_KIRO_GLOBAL)
...
class McpServerInfo:
    ...
    # authorization exchange. Codex Crew never enforces scopes and never registers
...
def _load_agent_config(*, user_home: Path | None = None) -> dict[str, Any]:
    ...
    AND the installed codexcrew.json — because defaults.json may not have
    mcpServers (they're added dynamically at install time by ``codexcrew setup``).
    """
    ...
    from codex_crew.agent import AGENT_FILENAME  # circular import: agent imports mcp_discovery
...
def configured_mcp_aliases(*, data_home: Path, user_home: Path) -> set[str]:
    """Return canonical names reserved by every effective CodexCrew MCP source."""
    names: set[str] = set()
...
def _load_mcp_json_by_source() -> dict[str, dict[str, Any]]:
    ...
    unknown path falls back to :data:`SCOPE_CODEXCREW`.
    """
    ...
    for p in scan_paths:
        scope = path_to_scope.get(p, SCOPE_CODEXCREW)
        if not p.is_file():
            continue
        try:
...
def _load_mcp_json() -> dict[str, Any]:
    ...
    # codexcrew-specific file > kiro global > any seam provider globals.
...
def _spec_scopes(spec: dict) -> list[str]:
    ...
    Reads kiro-cli's ``oauthScopes`` as well as Codex Crew's internal ``scopes``:
...
# Managed server name -> the ``codexcrew`` CLI subcommand that serves it.
_MANAGED_SERVER_SUBCOMMANDS = {
    "codexcrew-core": "mcp-core",
    "codexcrew-cron": "mcp-cron",
    "codexcrew-computer": "mcp-computer",
    "codexcrew-dashboard": "mcp-dashboard",
    "codexcrew-consult": "mcp-consult",
}
...
_MANAGED_SERVER_TOOL_MODULES = {
    "codexcrew-core": "codex_crew.mcp_core",
    "codexcrew-cron": "codex_crew.mcp_cron",
    "codexcrew-computer": "codex_crew.mcp_computer",
    "codexcrew-dashboard": "codex_crew.mcp_dashboard",
    "codexcrew-consult": "codex_crew.mcp_consult",
}


#: Managed servers that advertise ``codexcrew.caller-identity`` AND are safe to
...
_MANAGED_SERVERS_CALLER_AWARE: frozenset[str] = frozenset(
    {"codexcrew-core", "codexcrew-cron", "codexcrew-dashboard", "codexcrew-consult"}
)
...
#: not-session-bound classification but not sufficient. ``codexcrew-computer``
...
#: #5322 follow-up.
#:
#: Contrast ``codexcrew-dashboard``, which refuses an unidentified caller and is
...
def managed_server_is_session_bound(name: str) -> bool:
    ...
    False for anything not managed by Codex Crew: a third-party server's identity
...
def _fix_stale_managed_command(name: str, spec: dict) -> None:
    ...
    Delegates to :func:`codex_crew.agent._codexcrew_mcp_invocation`, the single
    source of truth for the managed invocation. That handles every layout:
    a standalone ``bin/codexcrew`` (POSIX) / ``Scripts\\codexcrew.exe`` (Windows
    pip install) console script when one resolves, the Windows bundle's
    ``bin\\codexcrew.cmd`` shim (unwrapped to ``<root>\\python.exe -P -s -m
    codex_crew <sub>``), and otherwise the ``<interpreter> -m codex_crew <sub>``
    fallback. Both ``command`` AND ``args``
    are rewritten — the fallback needs ``["-m", "codex_crew", <sub>]``, so
    re-resolving the command alone (the old behavior) silently dropped the args
    and spawned a bare ``codexcrew`` that isn't on PATH (Windows: ``command not
    found: codexcrew``; the built-in cron/core tools then never load).
    """
    ...
    if invocation is None:
        try:
            from codex_crew.agent import _codexcrew_mcp_invocation  # circular import

            invocation = _codexcrew_mcp_invocation(subcommand)
        except Exception:
...
def _is_first_party_managed_argv(
    ...
    invocation (:func:`codex_crew.agent._codexcrew_mcp_invocation`, the single
    ...
    *env* must equal the package-derived managed env too
    (:func:`codex_crew.agent._managed_mcp_env` — ``{}`` on a default install, the
    ``CODEXCREW_HOME`` pin under an override home). ``probe_server`` merges the
    ...
    try:
        ...
        from codex_crew.agent import _codexcrew_mcp_invocation, _managed_mcp_env
    ...
    # Refuse the interpreter fallback (`<python> -m codex_crew <sub>`): `python
    ...
    # gateway's cwd — a planted `codex_crew/` tree there would shadow the
    ...
    if expected_args[:2] == ["-m", "codex_crew"]:
        return False
    return (
...
def list_servers() -> list[McpServerInfo]:
    ...
    #    don't overwrite it.  Order = codexcrew-specific > Kiro global >
    ...
    for scope in _scope_priority(by_source):
        for name, spec in by_source.get(scope, {}).items():
            ...
            elif scope == SCOPE_CODEXCREW and spec.get("disabled") and name not in servers:
                # Consent-disabled entries (registry installs and custom adds
                # land with ``disabled: true`` until the user enables them)
                # live ONLY in the CodexCrew scope. They must still get a row:
    ...
    #
    #    MC presence = "will this load in CodexCrew sessions after the next
    ...
    #    CodexCrew has an explicit ``disabled: true`` override.
    #    Kiro/CC presence = raw membership in that provider's global config.
    agent_names = set(agent_cfg.get("mcpServers", {}).keys())
    codexcrew_own = by_source.get(SCOPE_CODEXCREW, {})
    # Every scope other than codexcrew is a raw-membership global scope
    ...
    global_scopes = [s for s in _scope_priority(by_source) if s != SCOPE_CODEXCREW]
    for name, server in servers.items():
        mc_disabled = (
            isinstance(codexcrew_own.get(name), dict) and codexcrew_own[name].get("disabled") is True
        )
        in_any_source = name in agent_names or any(
            name in by_source.get(scope, {}) for scope in by_source
        )
        presence: dict[str, bool] = {SCOPE_CODEXCREW: in_any_source and not mc_disabled}
    ...
    #     spawn, not only the branch above that INTRODUCES a Codex-Crew-scope row
...
async def _runtime_grant_present(mcp_url: str, name: str) -> bool | None:
    ...
    The probe holds no token of its own (Codex Crew stores no credentials), so the
    ...
    try:
        init_body = {
            ...
            "params": {
                ...
                "capabilities": {},
                "clientInfo": {"name": "codexcrew-probe", "version": "1.0.0"},
            },
        }
        hdrs = {
            **server.headers,
            ...
            async with session.post(server.url, json=init_body, headers=hdrs) as resp:
                if resp.status != 200:
                    ...
                    if _needs_authorization(resp.status, resp.headers, server.headers):
                        ...
                        # sees that token (Codex Crew keeps no credentials), so
...
async def probe_server(
    ...
    *client_info* overrides the ``clientInfo`` sent in the handshake. The
    ...
    try:
        ...
        # above). ``strip_python_env`` keeps CodexCrew's PYTHONPATH/PYTHONHOME out
        ...
        # (``agent._codexcrew_mcp_invocation`` + ``agent._managed_mcp_env`` via
        ...
        if not _declared_temp_upper:
            try:
                from codex_crew.mcp_gateway.backend_tmp import (
                    allocate_probe_tmp,
                    probe_child_scratch,
                )
                ...
                if probe_tmp is not None and probe_scratch is None:
                    ...
                    # for this one.
                    from codex_crew.mcp_gateway.backend_tmp import sweep_backend_tmp
        ...
        try:
            if probe_scratch is not None:
                from codex_crew.mcp_gateway.backend_tmp import tmp_env
        ...
        if probe_tmp is not None:
            ...
            from codex_crew.mcp_gateway.backend_tmp import record_owner
            ...
            json.dumps(
                {
                    ...
                    "params": {
                        ...
                        if client_info
                        else {"name": "codexcrew-probe", "version": "1.0.0"},
                    },
                }
            )
            + "\n"
        ...
        #     does go stale ("command not found: codexcrew; the built-in cron/core
        ...
        if managed_tools is not None:
            ...
            return server
        #
        # The wrap is deliberately KEPT rather than skipped for Codex Crew's own
        ...
        server.status = "error"
        server.error = (
            f"mcp_probe_sandbox_unavailable: Codex Crew could not probe this server "
        ...
        if probe_tmp is not None:
            if platform_compat.IS_POSIX:
                ...
                try:
                    from codex_crew.mcp_gateway.backend_tmp import sweep_all_backend_tmp
...
def _envs_agree(agent_env: dict, source_env: dict) -> bool:
    ...
    compared through :func:`codex_crew.env.spec_env_path` — the agent config
    ...
    for name, spec in mcp_servers.items():
        ...
        if name not in agent_names:
            ...
            if info.is_remote:
                ...
                # consumer guards its own surface, so the two that Codex Crew does
...
def sync_to_agent_config(servers: list[McpServerInfo]) -> bool:
    ...
    that reads all source files (``~/.codex-crew/mcp.json``,
    ...
    """
    from codex_crew.agent import install_agent  # circular import

    install_agent()

    # Audit: log which servers triggered the config rebuild
    try:
        from codex_crew.sel import sel  # circular import
...
def codexcrew_managed_names() -> set[str]:
    """Server names the dashboard store owns.

    A usable dict under the ``codexcrew`` scope (``<data home>/mcp.json``) is the
    one signal that Codex Crew manages a name -- the same discriminator the
    ...
    sync set exactly like a managed one; without the gate a Codex Crew sync would
    ...
    :func:`codex_crew.mcp_provenance.resolve_write`, which reads the marker on the
    ...
    return {
        name
        for name, spec in by_source.get(SCOPE_CODEXCREW, {}).items()
        if isinstance(spec, dict)
    }
...
def register_servers_for_cc(
    ...
    # name we own -- see codexcrew_managed_names.
