...
rewrite ``Scripts\\codexcrew.exe`` while the gateway is served by the very venv it
...
gateway's auto-update, ``codexcrew update``, and the console-script bootstrap. Four
...
rather than a stylistic preference. ``codex_crew._bootstrap`` reaches for this
...
def locked_console_scripts(target_py: Path) -> list[str]:
    """The venv's ``codexcrew`` console scripts that cannot currently be rewritten.
    ...
    ordinary single-checkout setup -- pip cannot replace ``Scripts\\codexcrew.exe``,
    ...
    if sys.platform != "win32":
        return []
    locked: list[str] = []
    for exe in sorted(Path(target_py).parent.glob("codexcrew*.exe")):
        try:
            with exe.open("r+b"):
                pass
...
def installed_console_script_target(target_py: Path, script: str) -> str | None:
    ...
    Reads the entry points off the ``codexcrew`` distribution rather than the
