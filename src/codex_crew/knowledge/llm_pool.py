...
# because this pool does NOT ride the shared codex_crew.acp.worker_pool engine
...
FETCH_TIMEOUT = 120.0
AGENT_NAME = "codexcrew-knowledge"
...
# Sandbox modes accepted by ``codex_crew.sandbox.wrap_argv`` (see its docstring).
...
def _read_config() -> dict:
    ...
    ``config_dir()`` resolves to ``~/.codex-crew`` (or ``$CODEXCREW_HOME``), so
...
def _get_sandbox_mode(config: Optional[dict] = None) -> str:
    ...
    - ``sandbox`` **present but malformed/unrecognised** (typo, wrong type) ->
      ...
      intent to run unsandboxed, so we re-enable CodexCrew's OS-level confinement
...
class AcpWorker(Worker):
    """Long-lived AcpClient session with codexcrew-knowledge agent."""
    ...
    async def start(self) -> None:
        if AcpClient is None:
            raise RuntimeError("AcpClient not available (codex_crew.acp.client not installed)")
...
class CCWorker(Worker):
    ...
    async def _spawn(self) -> None:
        ...
        # vanilla machine). Users can opt in by setting CODEXCREW_KNOWLEDGE_FETCH_TOOLS
        # to a comma-separated list of MCP tool names their backend exposes.
        fetch_tools = os.environ.get("CODEXCREW_KNOWLEDGE_FETCH_TOOLS", "").strip()
        if fetch_tools:
