...
from codex_crew.on_loop_db import STORE_STRICT_ENV, OnLoopDBGuard

from .._sqlite_compat import fts5_cjk_match_groups, fts5_segment_for_index

logger = logging.getLogger(__name__)

# Marker in a source row's properties for a source Codex Crew created itself rather
...
# Marker recording that a row Codex Crew registered itself has been adopted by the
...
def is_auto_registered(props: dict) -> bool:
    """True when *props* belong to a source Codex Crew registered itself, unadopted.
...
#   ``CODEXCREW_STRICT_ON_LOOP_PERSIST`` switch, which ``setup.py``'s ``test_e2e``
...
#   developer to unset ``CODEXCREW_DEV_MODE`` -- silencing history.py's guard too.
#
...
_ON_LOOP_DB_GUARD = OnLoopDBGuard(
    ...
    remedy=(
        ...
        "codex_crew.executors) so the busy wait runs off the loop."
    ),
