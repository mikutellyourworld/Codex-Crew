...
from codex_crew.embeddings import PRIORITY_BULK, PRIORITY_NORMAL, bulk_pace_delay
...
def get_embed_rate_limiter() -> EmbedRateLimiter:
    ...
    try:
        ...
        rate = max(0, int(CodexCrewConfig.load().knowledge.embed_rate_limit))
    except Exception:
        rate = 0
...
def _max_ingest_file_mb() -> float:
    ...
    from codex_crew.config.loader import CodexCrewConfig  # circular import
    try:
        return CodexCrewConfig.load().knowledge.max_ingest_file_mb
    except Exception:
...
async def rebuild_embeddings(store, embedder, *, job_id: str | None = None,
    ...
    ``memory.embedding_bulk_duty`` (see :func:`codex_crew.embeddings.bulk_pace_delay`).
