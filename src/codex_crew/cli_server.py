...
from codex_crew import __version__, dep_sync, platform_compat
from codex_crew.beacon import distribution, is_default_home
...
from codex_crew.constants import DATA_WARNING
from codex_crew.context import ContextBuilder
from codex_crew.dashboard import tailnet_serve
from codex_crew.dashboard.handlers.core import DASHBOARD_HTML_NOT_FOUND_MARKER
from codex_crew.dashboard.origin import (
    dashboard_origin,
    resolve_dashboard_host,
)
from codex_crew.dashboard.tailnet import is_governance_pinned_off, tailnet_origin
from codex_crew.dashboard.token_auth import parse_duration
from codex_crew.embeddings import (
    make_sync_embed_fn,
    model_file_present,
    store_embedding_space_is_stale,
)
from codex_crew.env import activate_mise
from codex_crew.frontend import build_frontend_sync, ensure_dev_dist_symlink
from codex_crew.git_divergence import (
...
from codex_crew.hooks import HookManager, hooks_config_from_config_dict
from codex_crew.instances import run_marker
from codex_crew.learn import LessonStore
from codex_crew.loopback_http import loopback_urlopen
...
# Client-side port resolution lives in codex_crew.port_resolution, a light leaf
...
# import and patch these names as ``codex_crew.cli_server.<name>``, and the
...
# intercepting.
from codex_crew.port_resolution import (  # noqa: F401
    _CODEXCREW_SERVER_SUBCOMMANDS,
    _args_look_like_codexcrew,
    _basename_stem,
    _config_url_port,
    _gateway_owns_port,
    _is_codexcrew_process,
    _marker_port,
    resolve_client_port,
...
from codex_crew.service.common import SERVICE_NAME, Platform, current_platform
...
# pulls the manifest-verify / crypto path, and `codexcrew gateway` imports this
...
def _token(args: argparse.Namespace) -> None:
    ...
    remote-mint path (:func:`codex_crew.instances.token_mint.mint_remote_token`)
    ...
    # default = no checks; see codex_crew.preflight.
    ...
    if not secret:
        print("❌ Gateway not running — start it with: codexcrew gateway", file=sys.stderr)
        sys.exit(1)
...
def _emit_session_urls(port: int, token: str) -> None:
    ...
    # to the Tailscale CLI with a multi-second timeout, and `codexcrew to`+`ken` is a
        ...
        if tailnet_url and tailnet_url != origin:
            ...
            if state.published is True:
                ...
                print(
                    ...
                    "`codexcrew tailnet up` to publish this dashboard, then re-run.",
                    file=sys.stderr,
                )
...
def _logout(port: int) -> None:
    ...
    secret = read_local_secret(port)
    if not secret:
        print("❌ Gateway not running — start it with: codexcrew gateway")
        sys.exit(1)
...
def _stop(cli_port: int | None = None) -> None:
    """Stop a running CodexCrew gateway.
    ...
    if cli_port is None and service_controller.stop_service():
        ...
        print("✅ Stopped codexcrew service. To remove it: codexcrew service uninstall")
        return
    ...
    # `codexcrew stop` a no-op on Windows).
    ...
    if not pids:
        ...
        if not platform_compat.listening_pid_tool_available():
            ...
            if _unpinned:
                print(
                    ...
                    f"Codex Crew resolves it from, so it cannot look up the gateway process "
        ...
        print(f"No Codex Crew gateway currently running on port {port}.")
        sys.exit(1)

    # Only kill processes that are actually CodexCrew gateways.
    ...
    pids = [p for p in pids if _is_codexcrew_process(p)]
    if not pids:
        sel().log_api_access(
            caller="cli",
            operation="gateway_stop",
            outcome="no_target",
            source="cli",
            resources=f"port={port} reason=no_codexcrew_process",
        )
        print(f"No Codex Crew gateway currently running on port {port}.")
        sys.exit(1)
    ...
    if denied:
        ...
        print(
            f"❌ No permission to stop pid {', '.join(str(p) for p in denied)} — try: sudo codexcrew stop"
        )
        sys.exit(1)
    if not sent:
        sel().log_api_access(
        ...
        print(f"No Codex Crew gateway currently running on port {port} (process already exited).")
        sys.exit(1)
...
def _wait_for_pids_exit(pids: list[int], timeout: float) -> list[int]:
    ...
    The incumbent holds an exclusive ``flock`` on ``<CODEXCREW_HOME>/gateway.lock``
    (see :mod:`codex_crew.gateway_lock`) for its whole lifetime, and the release
    ...
    CodexCrew gateways once, before the stop, and a pid recycled onto an unrelated
...
def _own_console_script() -> str | None:
    ...
    :func:`_spawn_detached_gateway` prefers this over ``shutil.which("codexcrew")``
    so a restart replaces the gateway with the *same* entry point that asked for
    the restart. ``which`` returns whatever ``codexcrew`` sits earliest on
    ...
    path (``python -m codex_crew restart``, a frozen bundle, a launcher that
    rewrites argv).
    """
    argv0 = sys.argv[0] if sys.argv else ""
    if not argv0 or _basename_stem(argv0) != "codexcrew":
        return None
    path = Path(argv0)
    if not path.is_absolute():
        # argv[0] may be a bare name found on PATH ("codexcrew") or a relative
        # path; resolve it the way the shell did.
        resolved = shutil.which(argv0)
        if not resolved:
            return None
        # MUST be absolutized. ``shutil.which`` returns an argument that already
        # has a directory component *unchanged*, so ``.venv/bin/codexcrew``
        # (a `cd ~/checkout && .venv/bin/codexcrew restart` invocation) comes back
...
def _spawn_detached_gateway(port: int | None = None) -> subprocess.Popen[bytes]:
    """Spawn a detached ``codexcrew gateway`` so the calling shell returns.
    ...
    - Drops stdin to ``/dev/null`` and redirects stdout/stderr to
      ``~/.codex-crew/gateway.log`` (same file the existing ``logs``
    ...
    - Resolves the console script this CLI was invoked as
      ...
      *same* ``codexcrew`` rather than whichever one happens to sit
      earliest on ``PATH``; then ``shutil.which("codexcrew")``, falling
      back to ``sys.executable -m codex_crew`` so editable/source-tree
      dev installs also work without a global ``codexcrew`` symlink.
    ...
    would bind 5486 while the parent polls 6776 and prints a 6776 URL. Naming
    ...
    replacement refused by the ``CODEXCREW_HOME`` ownership guard exits 1 within
    ...
    bin_path = _own_console_script() or shutil.which("codexcrew")
    if bin_path:
        ...
        argv = [sys.executable, "-m", "codex_crew", "gateway"]
    if port is not None:
...
def _probe_gateway_ready(port: int, timeout: int = 3) -> int:
    ...
    Same contract as :func:`codex_crew.pod.runtime.health` (the status code, or
...
def _replacement_is_serving(port: int, prior_pid: int | None) -> bool:
    ...
    **No marker means not proven, never proven.** An absent marker is the state
    ...
    accurate ``codexcrew token``, whereas a false success leaves the operator
...
def _wait_gateway_ready(
    ...
    * **Early death short-circuits the wait.** A replacement refused by the
      ``CODEXCREW_HOME`` ownership guard exits within milliseconds; polling the
...
def _print_token_url(port: int) -> None:
    ...
    while time.monotonic() < deadline:
        try:
            ...
            with loopback_urlopen(req, timeout=3) as resp:
                ...
                token = data.get("token", "")
            if token:
                # Print the canonical loopback host (codexcrew.localhost when it
                ...
                origin = dashboard_origin(CodexCrewConfig.load().dashboard.url)
                if origin and "localhost" not in origin:
                    print(f"   {origin}/?token={token}")
                return
    ...
    print("\n⚠️  Could not generate token (gateway still starting?). Run: codexcrew token")


def _restart(cli_port: int | None = None) -> None:
    """Restart a running CodexCrew gateway.
    ...
    2. Otherwise, SIGTERM the foreground gateway via the existing
       lsof+SIGTERM path used by ``codexcrew stop``, then spawn a
    ...
    if cli_port is None:
        if service_controller.restart_service():
            ...
            print("✅ Restarted codexcrew service.")
        ...
        if service_controller.is_service_active():
            ...
            # CODEXCREW_HOME lock refuses, and the original gateway keeps
            ...
            print(
                "❌ A codexcrew service is installed and running, but the service "
    ...
    # `codexcrew restart` after the gateway crashed should still get a fresh
    ...
    # the CODEXCREW_HOME lock (see _wait_for_pids_exit). Filter with _stop()'s own
    # codexcrew-process predicate so an unrelated listener on this port never
    ...
    incumbents = [p for p in listeners if _is_codexcrew_process(p)]
    ...
    if wait_for_incumbents:
        ...
        if alive:
            ...
            print(
                ...
                f"   To inspect the shutdown, run: codexcrew logs -f\n"
    ...
    if verdict != _READY_OK:
        ...
        if verdict == _READY_DIED:
            print(
                ...
                f"   To see why it exited, run: codexcrew logs -f"
            )
        else:
            print(
                ...
                f"   To follow its startup, run: codexcrew logs -f"
            )
        sys.exit(1)
    ...
    print(f"✅ Started detached gateway (pid {pid}). Logs: codexcrew logs -f")
    _print_token_url(port)


def _update(force: bool = False) -> None:
    """Update Codex Crew — dispatches based on install layout.
    ...
    * **wheel / cli.sh** — fetch the release feed, compare versions, and
      ...
      caused the ``CODEXCREW_PROJECT_DIR not set`` error for cli.sh installs.
    * **externally managed** (desktop app, Docker) — print guidance on how
    ...
    """
    from codex_crew.platform.update_layout import InstallLayout

    print(">_ Updating Codex Crew…\n")

    # A policy-defined provider OWNS the update on this host. Checked before any
    # layout dispatch so a manual `codexcrew update` cannot run the built-in
    # git/CDN mechanism the administrator excluded.
    from codex_crew.platform.update_provider import apply_policy_update
    ...
    from codex_crew.platform.update_governance import resolve_remote_url, update_blocked_reason
    ...
    from codex_crew.cli import _ensure_node  # circular import: cli -> cli_server -> cli
    ...
    # Install the pulled revision into this CLI's own venv. `codexcrew update` is
    ...
    # script — that is the one case still needing a terminal without codexcrew
    # running).
...
def _refresh_agent_config(proj: str) -> None:
    ...
    * ``TimeoutExpired`` is caught. It is raised, not returned, so without a
      handler a slow refresh would traceback out of ``codexcrew update`` right
      after the success banner printed.
    """
    print("  🔒 Refreshing agent config…")
    try:
        r = subprocess.run(
            [sys.executable, "-m", "codex_crew", "setup", "--agent-only"],
            cwd=proj,
            stdin=subprocess.DEVNULL,
            capture_output=True,
            timeout=30,
        ...
        print("  ⚠️  Agent config refresh timed out — run: codexcrew setup --agent-only")
        return
    if r.returncode == 0:
        print("  ✅ Agent config refreshed (deniedCommands + hooks updated)")
    else:
        print("  ⚠️  Agent config refresh failed — run: codexcrew setup --agent-only")


def _update_wheel(layout) -> None:
    """Update a wheel/cli.sh install by checking the release feed and re-running the installer.

    This is the path taken when CODEXCREW_PROJECT_DIR is unset or has no .git —
    the standard state for ``curl | sh`` installs where the venv at
    ``~/.codex-crew-venv`` has no source tree.
    """

    from codex_crew import __version__ as local_version
    from codex_crew.platform.update_governance import update_blocked_reason
    from codex_crew.platform.update_layout import (
        cdn_bases,
        cdn_bases_are_safe,
        release_channel,
        wheel_update_command,
    )
    from codex_crew.platform.wheel_engine import (
        WheelUpdateError,
        apply_wheel_update,
    ...
    # Shell safety: cdn_bases() reads CODEXCREW_CDN_BASE which is operator-set.
    ...
    if not feed_url.startswith("https://"):
        ...
        sys.exit(1)
    try:
        req = urllib.request.Request(feed_url, headers={"User-Agent": "codexcrew-update/1"})
    ...
    if manifest.get("schema") != "codexcrew-cli-artifact-manifest-v1":
    ...
    from codex_crew.dashboard.handlers.updates import _is_newer
    ...
    if running_from_managed_venv():
        ...
        try:
            ...
            # credentials (a token-bearing CODEXCREW_CDN_BASE), and this
            ...
            print(f"    {fallback}")
            sys.exit(1)
        print(f"\n✅ Codex Crew {remote_version} installed at {promoted}")
    ...
    print(f"\n✅ Codex Crew updated to {remote_version}!")
    print("\n  Restart the gateway to use the new version:")
    print("    codexcrew restart")
...
def _update_approve() -> None:
    ...
    update overlay.
    """
    from codex_crew.platform.update_stepup import read_pending

    print(">_ Approving the pending in-app update…\n")
    ...
    try:
        from codex_crew.dashboard.urls import dashboard_socket_path
    ...
    try:
        ...
        print("❌ Gateway is not running — start it, or update directly with: codexcrew update")
        sys.exit(1)
...
def _status(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if e.code in (401, 403):
            print("Codex Crew gateway is running (token auth enabled).")
            print("  For detailed stats, see the Overview page in the dashboard.")
        else:
            print(f"Codex Crew gateway is running but returned HTTP {e.code}.")
        return
    except (urllib.error.URLError, OSError):
        print("Codex Crew gateway is not running.")
        print("  Start it with: codexcrew gateway")
        return
    except Exception:
        print("Codex Crew gateway is running but returned an unexpected response.")
        return

    print(f"Codex Crew v{__version__} >_\n")
...
def _should_reconcile_launchd_launcher() -> bool:
    ...
    ``CODEXCREW_HOME`` does not scope, so a dev, pod, or worktree gateway
...
async def _gateway(
    *,
    ...
    from codex_crew.cli import _ensure_node, _node_ok  # circular import: cli -> cli_server -> cli
    ...
    if not config_path().exists():
        cfg = CodexCrewConfig()
        cfg.save()
        print(f">_ Created default config: {config_path()}")
...
async def _run_task(args: argparse.Namespace) -> None:
    ...
    # is already present. Deliberately NO download kick here — `codexcrew run`
    ...
    # standalone `codexcrew run` has no gateway to own the sync, so run the
    ...
    if fresh:
        print(f">_ Running spec (fresh): {spec_path}")
    else:
        print(f">_ Running spec: {spec_path}")
...
def _service_cmd(args: argparse.Namespace) -> int:
    """Dispatch ``codexcrew service {install,uninstall,status}``.

    Wraps :mod:`codex_crew.service.controller` so that platform detection
    ...
    if action == "status":
        ...
        return rc
    print("Usage: codexcrew service {install|uninstall|status}", file=sys.stderr)
    return 2


def _sandbox_cmd(args: argparse.Namespace) -> int:
    """Dispatch ``codexcrew sandbox {install-profile,remove-profile,status}``.

    Mirrors :func:`_service_cmd`: platform detection and the privileged calls
    live in :mod:`codex_crew.service.controller`, and this layer only parses
    ...
    print(
        "Usage: codexcrew sandbox {install-profile|remove-profile|status}",
        file=sys.stderr,
    )
    return 2
...
def _logs_cmd(args: argparse.Namespace) -> None:
    ...
    Order of preference:
      ...
      3. ``~/.codex-crew/gateway.log`` (foreground gateway)
    """
    ...
    if plat == Platform.SYSTEMD and svc_linux.UNIT_PATH.exists():
        ...
        # this fall-through, `codexcrew logs` would hang on hosts without
        ...
        if not sys.stdin.isatty():
            print(
                ">_ Insufficient permissions to read the journal without sudo, "
    ...
    # fall-through to the config_dir() log below and `codexcrew logs` exits 0
    ...
    if not fallback.exists():
        print(
            ">_ No gateway logs found. Either install the service "
            "(`codexcrew service install`) or start the gateway "
            "(`codexcrew gateway`).",
            file=sys.stderr,
        )
        sys.exit(1)
