...
from codex_crew.acp.runtime import AcpRuntime, AcpRuntimeError
from codex_crew.acp.session_handle import AcpRequestTimeout, AcpSessionHandle
...
from codex_crew.acp.types import (
    ...
    PROVIDER_LABEL_KAS,
    PROVIDER_LABEL_KIMI,
    PROVIDER_LABEL_OPENAI_COMPATIBLE,
    STOP_REASON_CANCELLED,
    STOP_REASON_END_TURN,
)
from codex_crew.acp_backends import POLICY_ID_BY_BACKEND
...
from codex_crew.effort import (
    EFFORT_LEVELS,
    effort_settings_key,
    model_supports_effort,
    resolve_effort_for_model,
)
from codex_crew.mcp_hot_reload import mcp_hot_reload_supported, parse_kiro_cli_version
from codex_crew.messaging.link import telemetry_channel_of
...
from codex_crew.providers.cleanup import _is_safe_path
...
def _write_tool_search_overlay(
    ...
    Codex Crew toggle stays authoritative regardless of any value in the user's
...
# to a fresh session + CodexCrew history replay (see _start_kiro_runtime_impl).
...
class AcpProvider(LLMProvider):
    ...
    def __init__(
        ...
        # Canonical Codex Crew identity (a cfg.agents key), resolved by the
        ...
        # cleared). Signals SessionManager.get_or_create to replay CodexCrew's
        ...
        # CodexCrew toggle is authoritative over any global kiro setting.
    ...
    @property
    def is_kimi_backend(self) -> bool:
        """True when this ACP provider talks to Kimi Code's native ACP server."""
        return self._client.backend == ACP_BACKEND_KIMI

    @property
    def is_openai_compatible_backend(self) -> bool:
        """True when this provider uses a configured Chat Completions profile."""
        return self._client.backend == ACP_BACKEND_OPENAI_COMPATIBLE
    ...
    async def _start_kiro_runtime(self) -> None:
        ...
        already emits ``codexcrew.session.startup.duration`` for the *claude* path
    ...
    def _emit_kiro_startup_metric(
        ...
        try:
            ...
            if resume_outcome:
                rec.counter(
                    "codexcrew.session.resume.outcome",
                    1,
                    attrs={
    ...
    def _member_session_key(self) -> str:
        ...
        from codex_crew.acp_backends import ACP_BACKENDS_MEMBER_DISPATCH
        from codex_crew.members import is_member_session_key
    ...
    async def _load_session_with_retry(
        ...
        * request timeout          → re-raise; its outcome is ambiguous, so a
                                       fresh session could duplicate the load.
        * runtime dies mid-retry   → return ``None`` (caller's respawn handles it).
        ...
        for attempt in range(_RESUME_MAX_ATTEMPTS):
            try:
                ...
                # The backend may have restored the session before its response
                # missed our deadline. Never turn that unknown outcome into a
                # second session/new request.
                logger.warning(
                    "Resume of kiro session %s timed out with an unknown outcome; "
                    "refusing to start a duplicate session",
                    resume_sid,
                )
                raise
        ...
        # the caller migrates to a fresh session with CodexCrew history replay.
        logger.warning(
            ...
            "to a fresh session with CodexCrew history replay",
            resume_sid,
            _RESUME_MAX_ATTEMPTS,
        )
        return None
        ...
        try:
            ...
            if resume_sid:
                ...
                if should_load:
                    ...
                    # to a fresh session/new below WITH CodexCrew history replay
    ...
    async def change_effort(self, level: str) -> bool:
        ...
        from codex_crew.dashboard.chat_persistence import get_reasoning_effort_values
...
def provider_label(provider: Any) -> str:
    ...
    if backend == ACP_BACKEND_KIMI:
        return PROVIDER_LABEL_KIMI
    if backend == ACP_BACKEND_OPENAI_COMPATIBLE:
        return PROVIDER_LABEL_OPENAI_COMPATIBLE
    return PROVIDER_LABEL_DEFAULT
