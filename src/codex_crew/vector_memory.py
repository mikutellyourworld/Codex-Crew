...
Storage: ~/.codex-crew/memory.db (SQLite, WAL mode)
FAISS index: ~/.codex-crew/memory.faiss (optional, for vector search)
...
from codex_crew.metrics.db_metrics import timed
from codex_crew.project_scope import (
    canonical_scope,
    project_scope_satisfied,
    scope_is_admissible,
)
from codex_crew.security import redact_and_truncate
from codex_crew.validation import ALLOWED_LESSON_CATEGORIES, normalize_lesson_category
...
# existing `from codex_crew.vector_memory import _MAX_*` paths keep working.
from codex_crew.vector_memory_constants import (  # noqa: F401
    _INJECTION_PATTERNS,
...
class LessonWriteOutcome(str, Enum):
    ...
    The vocabulary matches :meth:`codex_crew.learn.LessonStore.save_or_enrich`, which
...
class VectorMemoryStore:
    ...
    def _embed_bulk_row(self, text: str, *, pace: bool) -> "list[float] | None":
        ...
        :func:`codex_crew.embeddings.bulk_pace_delay`).
                ...
                if self.embed_fn is None:
                    ...
                    if (
                        ...
                        if candidate is not None:
                            ...
                            try:
                                probe = candidate("_codexcrew_embed_probe")
                            except Exception:
                                probe = None
    ...
    def backfill_missing_embeddings(
        ...
        *pace* (default on) idles between rows so the sweep targets
        ...
        more than :data:`~codex_crew.embeddings._MAX_BULK_PACE_SLEEP` of idle is
    ...
    def migrate_from_markdown(self) -> dict[str, int]:
        ...
        # Honor CODEXCREW_HOME via config_dir() so the source directory matches
