...
Runs as ``codexcrew mcp-cron`` — kiro-cli spawns it as a child process
...
from codex_crew.cron_trigger import _JOB_ID_RE, trigger_cron_job
from codex_crew.mcp_caller import current_caller
from codex_crew.mcp_core import (
...
from codex_crew.sandbox import _AGENT_DENIED_ENV_KEYS
...
from codex_crew.validation import MCP_CRON_SCHEMAS, ValidationError, validate_tool_args
...
def _vet_cron_capability_governance(session_key: str | None = None) -> str | None:
    ...
    *body* under the ``commands`` scope): this is the on/off *capability* gate.
    ...
    try:
        ...
        # log_warning=False: this runs inside the codexcrew-cron stdio MCP server,
        ...
        # and hard-fail the stdio codexcrew-cron tool call.
        try:
...
def _vet_command_governance(command: str) -> str | None:
    ...
    try:
        ...
        # log_warning=False: stdio codexcrew-cron server (see _vet_cron_capability_
...
def _vet_shell_command(command: str) -> str | None:
    ...
    from codex_crew.hooks import effective_denied_regexes_from_config
...
def _vet_script_contents(text: str) -> str | None:
    ...
    A ``cron_add`` ``script`` job points at a file under ``~/.codex-crew/crons/``
...
def _vet_script_file(file_path: str) -> str | None:
    ...
    ``~/.codex-crew/crons/``), but this function does NOT trust that — it
    ...
    return [
        {
            ...
            "description": (
                ...
                "onboarding importer), are managed with `codexcrew cron list` "
            ...
            "inputSchema": {
                ...
                "properties": {
                    ...
                    "script": {
                        ...
                        "'~/.codex-crew/crons/file.py:function'. Scripts must be under "
                        "~/.codex-crew/crons/. Function receives a "
...
def _call_tool(name: str, raw_args: dict[str, Any]) -> str:
    ...
    return call_tool_with_logging(
        ...
        session_key=_resolve_session_key() or "mcp_cron",
        downstream_service="codexcrew-cron",
    )
...
def _caller_channel_id() -> str:
    ...
    ``CODEXCREW_CHANNEL_ID`` has the same defect the session key had: process
...
def _authz_session_key() -> str:
    ...
    ``CODEXCREW_SESSION_KEY``, or ``CODEXCREW_HOST_PID`` plus its HMAC sidecar --
...
#: session to name: ``codexcrew cron add`` from the CLI (``cli_commands``, which
...
#: ``codexcrew cron add`` does not appear in ``cron_list`` from chat. The CLI
...
def _unidentified_caller_refusal(tool_name: str) -> str:
    ...
    work: ``codexcrew cron ...`` drives :class:`CronService` directly and never
    ...
    return (
        ...
        "refused. Manage jobs from the CLI (`codexcrew cron ...`), which carries "
        "admin authority." + strict_identity_diagnosis("codexcrew-cron")
    )
...
def _not_found(job_id: str) -> str:
    ...
    return (
        ...
        f"it as proof the job is gone. `codexcrew cron list` shows every job with its "
        f"owner; if the job should belong to this session, ask the user to run "
        f"`codexcrew cron adopt {job_id} --session-of <session>`."
    )
...
_SCOPED_EMPTY = (
    ...
    "scope and are not listed here. `codexcrew cron list` and the dashboard Schedule "
    "page show every job with its owner; to bring one into this session's scope for "
    "good, ask the user to run `codexcrew cron adopt <job_id> --session-of <session>`."
)
...
def _owner_unusable_caveat(svc: "CronService", session_key: str, job_id: str) -> str:
    ...
    just written a row only ``codexcrew cron adopt`` can rescue. Both branches below
    ...
    if session_key.startswith("subagent:"):
        return (
            ...
            "user: either have the parent create the job, or run `codexcrew cron adopt "
    ...
    return (
        ...
        "recognise the job on its next run. Tell the user to run `codexcrew cron "
...
def _ephemeral_authority_caveat(persistent: bool, is_agent_job: bool, sequence_len: int) -> str:
    ...
    * NOT an agent job. A script cron is launched with
      ``CODEXCREW_SESSION_KEY=cron:<job_id>`` unconditionally and a command cron
...
def _call_tool_inner(name: str, args: dict[str, Any]) -> str:
    ...
    if name == "cron_add":
        ...
        if channel is None:
            channel = _caller_channel_id() or os.environ.get("CODEXCREW_CHANNEL_ID") or None
        ...
        if not session_key:
            ...
            # does `codexcrew cron add`, which reaches the store directly.
    ...
    if name == "cron_trigger":
        ...
        # not the client resolver: DASHBOARD_PORT reads CODEXCREW_PORT only, and the
        # client resolver reads it FIRST -- both give 5486 on a --port auto gateway,
