...
Jobs are stored in the config directory (``~/.codex-crew/crons.json`` by default,
overridden by ``CODEXCREW_HOME``) and executed by a background
...
Jobs are created via MCP tools (``cron_add``) or the CLI (``codexcrew cron add``).
...
from codex_crew.constants import env_flag_enabled
from codex_crew.cron_history import CronHistoryStore, CronRunRecord
from codex_crew.executors import _CRON_QUEUE_WAIT_SECS, cron_gate_budget, subprocess_executor
from codex_crew.metrics.events import CRON_FIRES, emit_counter
from codex_crew.resource_status import admission_check
from codex_crew.validation import CHANNEL_MAX_LEN, MAX_CRON_MESSAGE, MAX_SHORT_STRING
...
def _read_job_records(path: Path) -> tuple[list[dict[str, Any]], bool]:
    ...
    * ``UnicodeError`` — the store is bytes on disk and can hold invalid
      ...
      one :func:`~codex_crew.atomic_write.atomic_write` writes, rather than to
      the caller's locale.
    ...
    * :func:`~codex_crew.portability._sanitize_imported_crons` rewrites an
      unreadable import to an empty store and reports it to the caller.
    * :func:`~codex_crew.snapshot._merge_crons` prints which path it could not
...
#
# Gating mirrors the repo's other strict rails (e.g. CODEXCREW_STRICT_ON_LOOP_
...
# CODEXCREW_STRICT_LOOP_SAFETY=1 to escalate the warning to a hard failure fleet-
# wide.
_STRICT_LOOP_SAFETY_ENV = "CODEXCREW_STRICT_LOOP_SAFETY"
...
class CronLoopSafetyError(RuntimeError):
    ...
    mode (``CODEXCREW_STRICT_LOOP_SAFETY``); otherwise the guard warns.
    """
...
class CronJob:
    ...
    # vault secret NAME (codex_crew.secrets.SecretVault; plaintext never touches
...
def unhealthy_jobs_from_disk() -> tuple[list[tuple[str, str]], list[tuple[str, str]], bool]:
    ...
    scheduler. ``codexcrew doctor`` is the caller, and a diagnostic whose purpose
...
class CronService:
    ...
    def __init__(
        ...
        *,
        ...
        _cfg = CodexCrewConfig.load().cron_history
        ...
        # cron_add/cron_list/cron_trigger and `codexcrew cron list` alike, none of
    ...
    async def _sigkill_session(self, session_key: str) -> None:
        ...
        :func:`codex_crew.executors.subprocess_executor` via
    ...
    def _apply_loop_stall_breaker(self) -> str | None:
        ...
        in-flight marker (:mod:`codex_crew.cron_inflight`) and the dump names the
        ...
        *settled* is False ONLY when the verdict could not be recorded -- an
        ...
        with self._file_lock():
            ...
            job.last_error = (
                ...
                f"`codexcrew cron resume {job.id}`."
            )
            try:
                self._save()
            except CronStoreUnreadable as exc:
        ...
        logger.error(
            ...
            "loop-stall watchdog while running it (%s). Resume with `codexcrew cron resume %s` "
    ...
    def _guard_off_event_loop() -> None:
        ...
        loop-park hazard. Under strict mode (``CODEXCREW_STRICT_LOOP_SAFETY``)
        ...
        :class:`~codex_crew.apps.cron_sdk.CronSDK` facade offloads to a worker
