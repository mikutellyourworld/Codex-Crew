...
# tree CodexCrew spawns (the ACP provider, MCP probes, gateway pool backends).
...
# identify escaped MCP launcher processes whose *cmdline* carries no CodexCrew
...
CODEXCREW_SPAWNED_ENV = "CODEXCREW_SPAWNED"
CODEXCREW_SPAWNED_VALUE = "1"

# Canonical truthy set for boolean environment variables (CODEXCREW_NO_JAIL,
# CODEXCREW_DEV_MODE, …).  Use ``env_flag_enabled`` rather than ``bool(os.environ
# .get(...))`` — a bare bool() treats ``"0"``/``"false"`` as truthy, which for a
# security toggle (e.g. CODEXCREW_NO_JAIL) is a silent-bypass footgun.
ENV_TRUTHY = frozenset({"1", "true", "yes", "on"})


# Minimum supported Node.js MAJOR version for every Python-side check
# (``codexcrew doctor``, the frontend-build probe in ``cli.py``, the TUI
...
DATA_WARNING = (
    "⚠️  Do not enter sensitive, secret, or regulated data into CodexCrew.\n"
...
#
# ONE definition on purpose. Every Codex Crew identifier that becomes a path
...
# the two most-seen surfaces (bare `codexcrew`, the chat REPL). Import it; never
# re-inline it. `cloud/ui.py` keeps its own art because it renders a different
# wordmark ("Codex Crew Cloud") with ANSI color.
BANNER = r"""
   _  ___            ___
