...
-------------------------------------------------------------------
The engine's ``list_sites`` groups resources by ``codexcrew:site`` and SKIPS anything
...
The drive tags ``codexcrew:publish-drive`` instead, so the deploy surface cannot see it
...
``engine.create_distribution`` hard-codes ``codexcrew:site=<id>`` on the resource it
...
from codex_crew.deploy import engine, profiles
...
TAG_DRIVE = "codexcrew:publish-drive"
TAG_DRIVE_VALUE = "default"

# What the drive hands engine.create_distribution as its site id. It is only used for the
# OAC name now that the drive supplies its own tag set, so the codexcrew:site tag is never
...
SITE_TAG_VALUE = "codexcrew-personal-drive"
...
# least-privilege policy already writes (`iam.py` grants `codexcrew-web-*`), so the drive
...
_SANDBOX_POLICY_NAME = "codexcrew-publish-drive-sandbox"
...
class PersonalDriveProvider(PublishProvider):
        ...
        if not bucket:
            if dist_id:
                ...
                raise PublishError(
                    ...
                    "Codex Crew can safely rebuild -- publishing would create a second "
    ...
    def _sandbox_headers_policy(self, profile: str) -> str:
        ...
        for item in items:
            ...
            raise PublishError(
                ...
                f"browser origin. Policy names are account-global, so Codex Crew cannot "
    ...
    def _serving_notice(self, dist_id: str, status: str, profile: str) -> tuple[str, str] | None:
        ...
        if not enabled:
            return (
                ...
                "not resolve. This is not a state Codex Crew sets -- an account "
    ...
    def _read_payload(self, file_path: str) -> bytes:
        # Deferred, and it must stay deferred. ``codex_crew.artifacts`` resolves config
    ...
    async def push_version(
        ...
        if conflict:
            return PushResult(
                conflict=True,
                error=(
                    "The copy in the drive changed since Codex Crew last pushed it, so "
    ...
    def sync_model(self) -> SyncModel:
        """Codex Crew owns the content; the drive is a token-guarded mirror of it."""
        return SyncModel(authority="codexcrew", concurrency="token")
