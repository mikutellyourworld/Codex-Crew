...
# State lives under the ACTIVE data home, not a hardcoded ~/.codex-crew, so a
# dev instance (CODEXCREW_HOME=~/.codexcrew-dev) keeps its own pidfiles instead of
# sharing mirror state with a production install.
def _resolve_home() -> Path:
    """Resolve the data home to an absolute path, or refuse.

    A RELATIVE CODEXCREW_HOME cannot be honored safely: this launcher is invoked
    ...
    """
    raw = os.environ.get("CODEXCREW_HOME") or ""
    if not raw:
        return Path.home() / ".kiro" / "crew"
    home = Path(raw).expanduser()
    if not home.is_absolute():
        die(f"CODEXCREW_HOME must be an absolute path, got {raw!r} "
...
def resolve_device(want: str | None) -> dict:
    ...
    if booted:
        return booted[0]
    phones = [d for d in devs if d["name"].startswith(("iPhone", "CodexCrew"))]
    if phones:
        return phones[-1]
    if devs:
        return devs[-1]
...
def cmd_start(args: argparse.Namespace) -> None:
    ...
    # here tear down a live mirror another CODEXCREW_HOME owns for the device.
...
def _live_owned_pid(udid: str) -> int | None:
    ...
    CODEXCREW_HOME legitimately owns for the same device, since the vendor's
