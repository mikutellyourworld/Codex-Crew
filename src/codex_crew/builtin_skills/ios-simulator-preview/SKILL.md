...
Mirror a running iOS Simulator into CodexCrew's Browser side panel so the user
...
→ the `codexcrew:preview` marker → Browser panel iframe.** Nothing leaves the
...
browser via a browser tool, while CodexCrew's panel is driven by the hidden
...
```bash
SKILL_DIR="${CODEXCREW_HOME:-$HOME/.codex-crew}/skills/ios-simulator-preview"
```

Call the script by path; do not `cd` into the skill folder.

`CODEXCREW_HOME`, when set, must be an ABSOLUTE path (`~` is fine — the launcher
