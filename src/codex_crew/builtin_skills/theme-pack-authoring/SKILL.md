...
description: Build, validate, and install Codex Crew theme packs -- pack anatomy, the 56-variable palette, role-tagged fonts, the overrides.css allowlist (what installs vs what actually renders), and the install-verify cycle. Use when the user wants to create or edit a theme pack.
...
# Codex Crew theme-pack authoring

House rules for building theme packs. The authoritative contract is
[`website/docs/theming-contract.md`](https://github.com/mikutellyourworld/Codex-Crew/blob/main/website/docs/theming-contract.md);
...
Codex Crew maps these names to bundled Lucide symbols and keeps the stock
...
- **System always stays the OS font** — a pack cannot reach it. Unfilled roles
  fall back to Codex Crew's own stacks.
...
`_THEME_CSS_VARS` in `codex_crew/dashboard/theme_validate.py`.
...
In a Codex Crew dev checkout, `_validate_theme_dir(pack_dir, installing=True)`
from `codex_crew.dashboard.theme_validate` runs the same check programmatically.
