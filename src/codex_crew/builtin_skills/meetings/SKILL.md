...
tool-less `codexcrew-lite` call per line in an ephemeral session — deliberately
...
All paths are under `~/.codex-crew/apps/meetings/data/`:
...
Transcription uses CodexCrew's own streaming endpoint (`/api/ws/stt`), driven from
