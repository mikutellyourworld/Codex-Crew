...
tags: [skill, codexcrew, monitor, babysit, autonudge, loop]
---
...
and tools on every cycle. Loops persist to `~/.codex-crew/autonudge.json` and
...
`${CODEXCREW_HOME:-$HOME/.codex-crew}/workspace/.babysit-key-<loop-id>`, which is the
...
Confirm against state, not the reply: read `~/.codex-crew/autonudge.json` (or
...
chat. Cron scripts must live under `~/.codex-crew/crons/`, so copy the synced
skill asset there first (re-copy on every arm — it keeps the copy current
with skill updates):

```
CREW_HOME="${CODEXCREW_HOME:-$HOME/.codex-crew}"
cp "$CREW_HOME/skills/codexcrew-dev/babysit/scripts/pr_watch.py" \
   "$CREW_HOME/crons/pr_watch.py"

cron_add(
  name="pr-watch #1234",
  script="~/.codex-crew/crons/pr_watch.py:watch",
  every=300,
  timeout=120,
...
The `cp` runs in your shell, so it has to resolve `CODEXCREW_HOME` -- an install
that moved its data home has no `~/.codex-crew/skills` at all and a hardcoded
...
```bash
SKILL_DIR="${CODEXCREW_HOME:-$HOME/.codex-crew}/skills/codexcrew-dev/prepare-pr"
...
> already distributed. For mikutellyourworld/Codex-Crew that record is #2548; #2550 moved
...
3. **Confirm it armed.** Read `~/.codex-crew/autonudge.json` and check your
...
monitor_start(
  message="Check https://github.com/owner/repo/pull/247. FIRST read
           ...
           Then run
           python3 \"${CODEXCREW_HOME:-$HOME/.codex-crew}/skills/codexcrew-dev/prepare-pr/scripts/pr_status.py\" 247
