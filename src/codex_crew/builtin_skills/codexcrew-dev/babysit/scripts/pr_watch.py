...
- :mod:`codex_crew.irq` -- state persistence, per-head reset, time-bounded
  dedupe, the convergence coalescing window, the consecutive-failure backstop.
- :mod:`codex_crew.probes.gh_pr` -- the only GitHub knowledge: how to observe a
...
via :func:`codex_crew.irq.poll`. Keeping a copy here would have meant two
...
resolved ``CODEXCREW_HOME`` -- so the copy a reader happened to follow decided
...
from codex_crew.irq import run
from codex_crew.probes.gh_pr import PrWatchProbe
...
def watch(ctx) -> None:
    ...
    :meth:`~codex_crew.probes.gh_pr.PrWatchProbe.identity`, and the kernel reads
