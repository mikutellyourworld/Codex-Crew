---
name: codexcrew-worktree-dev
description: "HARD RULE for developing the Codex Crew source repo ITSELF (not for users' own projects): every change is built and verified inside a git worktree, never against the live gateway. Covers worktree creation, the blocking local build gates (pytest + isort + flake8 + mypy + tsc + vitest), the built-dist gotcha, feature flags, live preview paths (isolated pods, dev-backend.sh as fallback), and the PR workflow. Use only when building, testing, switching, or verifying a change to Codex Crew's own codebase."
triggers: codexcrew worktree, codexcrew build gate, codexcrew dev, codexcrew source, contribute to codexcrew, codexcrew repo
repo_scope: src/codex_crew
---

# HARD RULE: Codex Crew development happens inside a git worktree

> **Scope guard: this skill applies ONLY when the working directory is the
> Codex Crew source repository (or a worktree of it).** If you are working in any
> other project, ignore this skill entirely — its rules (worktree mandate,
> build gates, single-commit squash, force-push) are conventions of the Codex Crew repo
> and may be wrong or harmful elsewhere.

Every local Codex Crew change — frontend, backend, or both — is developed, built,
...
- **Every** Codex Crew change happens in a dedicated worktree. Never edit the
  live/production checkout, and never develop against the running gateway.
- A Codex Crew feature spans **two layers**: `src/codex_crew/` (backend, Python)
...
- **Single-active model.** Making a worktree "live" swaps the *code* behind the
  same dashboard URL and the same shared data home — `~/.codex-crew` by default
  (legacy installs auto-migrate from `~/.codexcrew` on first launch;
  `CODEXCREW_HOME` overrides) — including your REAL DB and sessions. Only one
...
```bash
# From your main CodexCrew clone:
git fetch origin main
git worktree add ../codexcrew-wt-<name> -b feat/<name> origin/main
cd ../codexcrew-wt-<name>
```
...
To clean up after merging: `git worktree remove ../codexcrew-wt-<name>`
...
isort --check-only src/codex_crew test
...
```bash
python3 -m venv ~/.codex-crew/venvs/mypy-ci
~/.codex-crew/venvs/mypy-ci/bin/pip install -e ".[voice]" --group dev   # from repo root; never add faiss
# then from any worktree root:
~/.codex-crew/venvs/mypy-ci/bin/mypy src/codex_crew/
```
...
- **Gate model (full floor).** The real blocking floor is the `gates[]` list in
  `prepare-pr/profiles/codexcrew.json` (dozens of entries, pinned to `ci.yml` and
...
- The gateway serves the frontend from `src/codex_crew/static/dist/` (a compiled
...
- After frontend changes, build AND stage (the build outputs to `website/dist`;
  ...
  rm -rf src/codex_crew/static/dist && cp -R website/dist src/codex_crew/static/dist
  ```
...
## Rule 4 — Feature flags live in the active instance's `$CODEXCREW_HOME/config.json`

- Flags belong in the config of the instance you are actually looking at.
  Each runtime has its own home: the live gateway uses `~/.codex-crew/` (the
  default since the data-home move; legacy `~/.codexcrew` auto-migrates),
  `dev-backend.sh` uses the worktree's `.codexcrew-dev/`, and each pod has its
  own isolated `CODEXCREW_HOME`. Editing `~/.codex-crew/config.json` while
...
`.codexcrew-dev/` directory you manage yourself. Reach for `dev-backend.sh` only
where a pod cannot run.

1. **Isolated pod (preferred).** Preview the full stack on its own port without
   touching the live gateway:
   ```bash
   codexcrew pod up <worktree-name> --json   # own CODEXCREW_HOME, own port, no crons
   codexcrew pod down <worktree-name>        # zero residue
   ```
   ...
   `codexcrew pod --help` remains authoritative; the verbs are `up`, `down`, `ls`,
   ...
   built first (venv + dist); `codexcrew pod up
   ...
   ```bash
   codexcrew pod up <worktree-name> --seed minimal --json
   ```
   ...
   Pods need Linux `systemd --user` or macOS `launchd` (`codexcrew pod install`
   once per machine) — see [`codex_crew/pod/README.md`](../../../pod/README.md)
...
2. **`dev-backend.sh` (fallback where a pod cannot run).** From the worktree
   ...
   ./dev-backend.sh
   ```
   Starts the gateway on its own dev port using `.codexcrew-dev/` as its data
   directory (isolated from your production `~/.codex-crew/`). It uses
   ...
   (or before running `codexcrew pod install`), or when you specifically need a
...
forthcoming as absent until `codexcrew pod --help` lists it. They also record the
...
`CODEXCREW_HOME` isolates config, DB, sessions and workspace. It does **not**
...
but not self-contained — a change to Codex Crew's own managed MCP servers
...
and Codex Crew still reads the host paths for most of those, so setting it breaks
...
- A flagged feature "missing" is usually a flag set in the wrong instance's
  `CODEXCREW_HOME`, not a missing bundle (Rule 4).
