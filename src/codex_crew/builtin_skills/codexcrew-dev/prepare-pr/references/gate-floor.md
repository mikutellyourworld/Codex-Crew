...
Maintainer-facing rationale for `profiles/codexcrew.json` `setup[]` and `gates[]`. The loop does
...
| matching a data file by its **bare basename** | `prepare-pr/profiles/codexcrew.json` selected 38 test files that all mention `codexcrew.json` meaning Codex Crew's own config file in a different directory |
...
| classifying a file by **location** instead of role | `src/codex_crew/apps/builtins` is a source tree AND a configured testpath, so "inside a root" was read as "is a test", making production modules look like helpers |
