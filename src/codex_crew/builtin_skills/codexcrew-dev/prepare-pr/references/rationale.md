...
`.prepare-pr.toml`, the Codex Crew markers, and the workflow globs from the base ref
...
`docs/` and `src/codex_crew/**` ship in the wheel, the sdist, and the desktop DMG, so
