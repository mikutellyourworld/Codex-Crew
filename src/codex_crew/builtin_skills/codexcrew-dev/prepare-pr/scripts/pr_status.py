...
def _normalize_repo_key(value):
    ...
    must be too: otherwise ``Fixes KiroDotDev/CodexCrew#7`` would read as naming
    a different repository from the host's own ``mikutellyourworld/Codex-Crew`` and the
...
def resolve_readiness_context(argv, environ):
    ...
    env var > the CodexCrew default. Lets a project profile name a non-default
