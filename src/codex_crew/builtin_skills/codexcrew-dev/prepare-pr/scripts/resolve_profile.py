...
Resolution order (most-specific-wins):
  ...
  2. CodexCrew markers present            -> bundled profiles/codexcrew.json
...
Every resolved profile has the SAME shape:
  {
    "source":        "config" | "codexcrew" | "auto-detect" | "generic",
...
# Files whose combined presence identifies the CodexCrew repo (or a faithful
...
def _safe_regular_file(path):
    ...
    to repos without codex_crew installed (it cannot import the gateway helpers).
    """
...
def detect_codexcrew(reader: TreeReader) -> bool:
    """True iff all Codex Crew marker files are present at the given tree reader."""
    return all(reader.has(m) for m in _CODEXCREW_MARKERS)
...
def resolve(root, base_ref=None):
    ...
    With ``base_ref``, EVERY profile input -- the config file, the Codex Crew
    ...
    if raw_config is not None:
        ...
        return profile
    if detect_codexcrew(reader):
        return normalize(load_bundled("codexcrew"), "codexcrew")
