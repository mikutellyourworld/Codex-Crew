---
name: writing-tests
description: "How to write a Codex Crew backend test that has NO side effects and does not flake. Use when adding, editing, reviewing, or debugging a pytest test in the Codex Crew source repo: which conftest is under your file, what leaks (temp dirs, the real data home, ~/.kiro, cron, threads, child processes), how to tell which of the five flake classes you have and where each one's fix is written, and the cross-platform traps on macOS/Linux/Windows and arm64. Also covers diagnosing a residue failure and keeping the parallel suite fast."
triggers: write a test, add a test, fix a flaky test, test is flaky, test side effect, temp dir residue, tmp residue, codexcrew test, pytest codexcrew, test isolation, conftest, xdist, test leaked
repo_scope: src/codex_crew
---

# Writing a Codex Crew test that does not leak and does not flake

> **Scope guard: this skill applies ONLY to the Codex Crew source repository** (or a
...
| Running the build gate in a worktree, and deciding whether a red is yours | **codexcrew-worktree-dev** |
| Driving a branch to a review-ready PR and through CI | **prepare-pr** |
| Polling that PR until it is green | **babysit** |

The line that matters most in practice: **codexcrew-worktree-dev** tells you whether a
...
`src/codex_crew/apps/builtins/*/tests/` gets the rootdir `conftest.py` plus that app's
...
under `CODEXCREW_TMP_RESIDUE_STRICT=1`. So a leak of yours will not necessarily turn CI red;
...
`CODEXCREW_HOME` is pinned per test at the rootdir, which is what makes `config_dir()`
safe — and it needs to be, because resolving it is **not a read**: it creates the home
and its marker, and can run the `~/.codexcrew` → `~/.codex-crew` migration.
...
- **A `MagicMock` config reads TRUTHY.** Patching `CodexCrewConfig.load` with a bare
  ...
  into the repo. That is why `CODEXCREW_TELEMETRY=0` is forced for the whole suite. Any
