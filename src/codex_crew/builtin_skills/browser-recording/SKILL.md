...
python3 <skill-dir>/scripts/record_browser.py \
  ...
  --scenario $CODEXCREW_SCRATCH/demo-scenario.mjs \
  --project /path/to/frontend \
  --size 1280x800 --name settings-flow --out $CODEXCREW_SCRATCH/rec
```
...
```
WEBM $CODEXCREW_SCRATCH/rec/settings-flow.webm
MP4 $CODEXCREW_SCRATCH/rec/settings-flow.mp4
GIF $CODEXCREW_SCRATCH/rec/settings-flow.gif
```
