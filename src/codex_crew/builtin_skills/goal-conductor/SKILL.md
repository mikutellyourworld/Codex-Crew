...
1. `session_create` with a title that says what the item is FOR, `folder` set to
   ...
   default":** the value inherited is YOUR agent, `codexcrew-conductor`, whose
...
2. **Evaluate every open item's acceptance condition with the bundled
   ...
   **Resolve `<this skill's dir>` from where this SKILL.md was actually loaded
   ...
   `~/.codex-crew/skills/goal-conductor`: a `CODEXCREW_HOME` override moves the
...
- **The session tools may not be in your tool list yet.** With MCP Tool Search
  ...
  `tool_search(tool_id="codexcrew-dashboard::session_create")` — `tool_search` is
  ...
  `codexcrew-core`, so its id is `codexcrew-core::monitor_start`.
...
- **Never dispatch a work item onto `codexcrew-conductor` itself** — it is an
...
- **The grant is the agent's MCP mount, not a feature switch.** The session tools
  come from `@codexcrew-dashboard`; an agent whose spec does not mount it never sees
...
- **Reads and creates do not prompt; anything that touches another session does.**
  ...
  `@codexcrew-core` verbs are granted by name too, and only these: `monitor_start`,
...
- **Shell is for the bundled scripts only, and the evaluator runs no command
  ...
  command would turn it into a general way to run one, and Codex Crew's
