...
`chrome`: Codex Crew gives every agent process its own `PLAYWRIGHT_CLI_SESSION`, and
...
```bash
playwright-cli state-save ~/.codex-crew/browser-state/example.json
```

**Replay it later.**

```bash
playwright-cli state-load ~/.codex-crew/browser-state/example.json
playwright-cli goto https://internal.example.com/dashboard
```
