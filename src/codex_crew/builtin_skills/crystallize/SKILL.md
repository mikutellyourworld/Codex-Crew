...
4. **Choose the destination — candidate by default, live only on an explicit
   request.** First resolve your CodexCrew skills directory — the SAME directory
   that holds the `auto/` group you inspected in step 2 (honor `$CODEXCREW_HOME`
   if set; do **not** assume a literal `~/.codexcrew`, since migrated installs
   live elsewhere).
