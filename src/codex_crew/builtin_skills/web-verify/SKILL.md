...
>
> ```bash
> python3 "${CODEXCREW_HOME:-$HOME/.codex-crew}/skills/web-verify/scripts/downscale_image.py" "/abs/path/shot.png"
> ```
>
...
> rewrites only files actually over the cap, and re-execs itself under Codex Crew's
...
1. **Get a loopback URL serving your change.** Never the live gateway. For the
   Codex Crew repo that means an isolated instance from the worktree you edited
   (`./dev-backend.sh`, or `codexcrew pod up <worktree> --json` for a
   `{base_url, token}` handle: see the `codexcrew-worktree-dev` and `pod-e2e`
...
2. **Confirm it is actually up** (HTTP 200/401/403) before navigating, and emit the
   ...
   `<!-- codexcrew:preview url="http://127.0.0.1:PORT" -->`
3. **Open it:** `playwright-cli open "http://127.0.0.1:PORT/?token=…"`. Include the
   ...
   auto-approved: every local control plane lives there, Codex Crew's own
