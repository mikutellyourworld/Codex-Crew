...
if TYPE_CHECKING:  # pragma: no cover - typing only
    ...
    # RECOVERABLE state here (main re-execs under Codex Crew's venv, which has
...
#: ``codex_crew/acp/prompt_blocks.py``, which enforces the same ceiling on images
...
_REEXEC_ENV = "CODEXCREW_DOWNSCALE_REEXEC"
...
def bundled_python() -> str | None:
    ...
    Codex Crew's own venv ships Pillow (``prompt_blocks`` depends on it), and the
    ``codexcrew`` console script lives in that venv's script directory — so the
    ...
    """
    launcher = shutil.which("codexcrew")
    if not launcher:
        return None
...
def _reexec_with_pillow(argv: list[str]) -> int:
    ...
    if os.environ.get(_REEXEC_ENV):
        print(
            ...
            "or run this under Codex Crew's venv interpreter.",
            file=sys.stderr,
        )
        return 1
    ...
    if not interpreter:
        print(
            "downscale: Pillow is unavailable and no Codex Crew venv interpreter "
            "was found next to the 'codexcrew' launcher. Install Pillow into "
