...
description: Conversational workflow for gathering user feedback and filing GitHub Issues on the Codex Crew repository. Load when the user clicks "Request a Feature", wants to report a bug, or suggest an improvement.
...
```
https://github.com/mikutellyourworld/Codex-Crew
```
...
Every `gh` call below targets Codex Crew's own public repository — a fixed target on
...
```
https://github.com/mikutellyourworld/Codex-Crew/issues/new?title=URL_ENCODED_TITLE&body=URL_ENCODED_BODY&labels=URL_ENCODED_LABELS
```
