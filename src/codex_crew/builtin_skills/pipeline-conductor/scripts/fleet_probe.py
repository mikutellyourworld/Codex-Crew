...
reach:
  ...
  * transcripts are read from ``<data home>/sessions`` only, where the data
    home is ``$CODEXCREW_HOME`` (else ``~/.codex-crew``) -- the gateway this
    conductor belongs to, not an arbitrary directory;
  * the handled-set state file is ALWAYS ``<config path>.state.json``;
  * the banned-process scan reads ``/proc`` (``$CODEXCREW_PROBE_PROC_ROOT``
...
The data home is ``$CODEXCREW_HOME`` when set, else ``~/.codex-crew`` — the same
...
def data_home() -> Path:
    env = os.environ.get("CODEXCREW_HOME")
...
def _host_lines(cfg: dict[str, Any]) -> tuple[list[str], str]:
    ...
    proc_root = Path(os.environ.get("CODEXCREW_PROBE_PROC_ROOT") or "/proc")
    banned = 0
    foreign = 0
    if proc_root.is_dir():
