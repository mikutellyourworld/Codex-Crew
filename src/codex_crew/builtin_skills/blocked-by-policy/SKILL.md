...
refused. Codex Crew's own gate produces most refusals, and the real reason
arrives separately as a `[Codex Crew host notice]` message in the same turn.
...
(`python -c 'import codex_crew …'`), and a compound command that mixes a product
...
- `codexcrew doctor` — reports the credential posture: which AWS profiles are
  configured, whether a `credential_process` is in play, and whether this host
  mounts an MCP server that vends credentials.
- `codexcrew policy show [--ids]` / `policy explain <scope> <item>` /
