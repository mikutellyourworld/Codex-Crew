...
``codex_crew`` and never touches ctypes, so it loads identically on macOS,
...
# ── MCP tool names (server ``codexcrew-computer``) ──
...
# of verbs that change the world outside Codex Crew, and starting a process is the
...
# Keystone opt-in for the ``codexcrew computer apps`` / ``call`` diagnostics. Those
...
# Ceiling deliberately equal to the inline-image cap (codex_crew.imaging
...
SCREENSHOT_DIR_NAME = "codexcrew-computer-shots"
...
REFUSAL_DISABLED = (
    "computer use is disabled. Enable it in the CodexCrew dashboard under "
...
OVERLAY_MODULE = "codex_crew.computer_use.overlay_proc"
...
OVERLAY_READY_LINE = "CODEXCREW_OVERLAY_READY"
...
class DeniedApp:
    ...
    # because a host application can display someone else's UI: CodexCrew's own
