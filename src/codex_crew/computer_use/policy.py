...
to every other security plane in CodexCrew. ``is_sensitive_path`` cannot see a
...
CATEGORY_CODEXCREW_SELF = "codexcrew_self"
...
_DENIED_BUNDLE_PREFIXES: tuple[DeniedApp, ...] = (
    DeniedApp(
        ...
        reason=(
            "CodexCrew's own dashboard can change the agent's security settings, "
            "which must only be done by the operator out-of-band"
        ),
        bundle_prefixes=("io.github.mikutellyourworld.codexcrew", "dev.kiro.crew"),
        name_substrings=("codex crew", "codexcrew"),
        ...
        # prefix ("(3) Codex Crew") and popouts a "<label> — Codex Crew" suffix.
        title_substrings=("codex crew", "codexcrew"),
    ),
)
...
def denied_rule_for(app: AppRef) -> DeniedApp | None:
    ...
    * ``title_substrings`` — the resolved WINDOW TITLE. Load-bearing for the
      self-target rule: CodexCrew's dashboard served into a browser tab presents
    ...
    title carries a badge prefix (``(3) Codex Crew``) and a page suffix
    (``… — Codex Crew``), and popout frames prepend their own label.

    The known false-positive cost, stated rather than discovered: any window whose
    title merely CONTAINS "codex crew" is refused — a code editor with this repo open,
