...
whatever launched the tree — ``CodexCrew.app`` when packaged, ``Terminal.app`` for a
dev ``codexcrew gateway``, an IDE if launched from one. A probe inspects only its
...
should actually grant, because "grant Accessibility to CodexCrew" is unactionable
...
HINT_RESPONSIBLE = (
    ...
    "that launched Codex Crew, not to Codex Crew itself. Grant them to {process} — "
...
_UNKNOWN_PROCESS = "the application that launched Codex Crew"
