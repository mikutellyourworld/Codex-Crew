"""``codexcrew computer`` — human-facing computer-use diagnostics and a debug driver.
...
Deliberately NO ``codexcrew computer state <app>``. That would be a second,
...
singleton) is per-process with a 90s TTL. Two ``codexcrew computer call``
...
from codex_crew.computer_use import enable_state, service, tools
from codex_crew.computer_use.backend import platform_id_for_current_os
from codex_crew.computer_use.policy import blocked_app_categories
...
_USAGE = """codexcrew computer — desktop automation (computer use) diagnostics
...
def main() -> None:
    """Console entry point (``codexcrew computer ...``)."""
    run_computer(sys.argv[1:])


def run_computer(args: list[str]) -> None:
    """Entry point for ``codexcrew computer <subcommand>``."""
    if not args:
        print(_USAGE)
        return
    cmd = args[0]
    ...
    if cmd == "call":
        _cmd_call(args[1:])
        return
    print(f"Unknown command: {cmd}. Run 'codexcrew computer' for help.", file=sys.stderr)
...
def _cmd_doctor(*, as_json: bool) -> None:
    ...
    if report["platform"] == "macos":
        ...
        print(
            ...
            "grant to the process that launched Codex Crew, so 'missing' does not\n"
...
def _cmd_apps() -> None:
    ...
    IS the diagnostic — and ``codexcrew computer doctor`` (which reads only the
