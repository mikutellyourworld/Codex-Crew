...
sidecar. :mod:`codex_crew.mcp_computer` is a thin shim that resolves the caller's
...
* The refusals that DO hold for computer use are enforced HERE, in band, on the
  ...
  CodexCrew's-own-window denylist, the secure-field and sensitive-text checks. They
...
5. **the always-on target policy (``policy.check_app``) — the real refusal.** The
   ...
   widen. This is what stops the agent driving Codex Crew's own window.
...
from codex_crew.computer_use import enable_state, gate, keymap, overlay, policy, render, service
...
# has no single target) and ``computer_end_turn`` touches CodexCrew's own cache, so
...
def dispatch_tool(
    ...
    *session_key* is the identity the shim resolved with
    ``mcp_core._resolve_session_key_strict`` (env var, or ``CODEXCREW_HOST_PID``
