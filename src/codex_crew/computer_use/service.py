...
:class:`~codex_crew.computer_use.backend.ComputerUseBackend`, which on macOS means
...
event loop. :mod:`codex_crew.computer_use.tools` is the async half — it offloads
...
* walk a window into a :class:`Snapshot`, persist any captured pixels, and cache
  the result in the shared :class:`~codex_crew.computer_use.index.SnapshotIndex`;
...
from codex_crew.computer_use import index as snapshot_index
from codex_crew.computer_use import render
from codex_crew.computer_use.backend import ComputerUseBackend, get_shared_backend
...
def _config_section() -> object:
    ...
    try:
        ...
        return getattr(CodexCrewConfig.load(), "computer_use", object())
    except Exception:
