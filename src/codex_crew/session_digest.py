...
from codex_crew.jsonl_util import RECORD_CAP, bounded_records

logger = logging.getLogger(__name__)

# The shared reader's cap (see :data:`codex_crew.jsonl_util.RECORD_CAP`, which
...
def digest(uid: str, stems: tuple[str, ...], sid: str) -> SessionDigest:
    ...
    *sid* is the kiro-cli session id (UUID).
    ...
    Reads the Codex Crew transcript(s) for first_message and turns, then
    ...
    # --- Codex Crew transcripts: first_message + turns ---
...
def _scan_transcript(path: Path) -> tuple[str, int]:
    ...
    bytes (see :func:`codex_crew.jsonl_util.bounded_records`, which also owns
...
def _scan_cli_log(path: Path) -> tuple[int, int]:
    ...
    :func:`codex_crew.jsonl_util.bounded_records`).
    Never raises.
    """
    turns = 0
    images = 0
