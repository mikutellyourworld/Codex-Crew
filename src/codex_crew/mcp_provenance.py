"""Authorship marker for MCP entries Codex Crew writes into shared config files.

Codex Crew writes MCP server entries into two files it does not own -- the
...
DERIVED_KEY = "x-codexcrew-derived"
...
def resolve_write(
    *,
    ...
    predicate (see :func:`codex_crew.mcp_discovery.codexcrew_managed_names`) and
    ...
    * **decline** -- the entry is present and unmarked. Nothing proves we wrote
    ...
    logger.warning(
        ...
        "the entry carries no Codex Crew marker, so it reads as hand-authored and is "
        "left as-is",
        name,
        surface,
    )
