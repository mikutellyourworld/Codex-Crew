"""Sidecar store for CodexCrew's per-agent bookkeeping.
...
"no agent with name X found" line). CodexCrew therefore keeps its private
...
State file (``~/.codex-crew/agent_model_state.json``, honoring ``CODEXCREW_HOME``)::

    {
      "codexcrew":           {"model_managed": true},
      "codexcrew-heartbeat": {"cc_model": "auto"}
    }
...
def _state_path() -> Path:
    """Return the sidecar path (resolved fresh so CODEXCREW_HOME is honored)."""
...
def lift_and_strip_bookkeeping(config: MutableMapping[str, object], name: str) -> bool:
    ...
    Codex Crew, never land on disk. When the sidecar already holds a value, a
