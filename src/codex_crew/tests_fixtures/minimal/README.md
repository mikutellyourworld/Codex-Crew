...
Populated `$CODEXCREW_HOME` used by tests that need a non-empty
...
Running `codexcrew gateway --seed minimal` against a fresh
`$CODEXCREW_HOME` lands you on a clickable dashboard: memory tab shows
the workspace content, cron tab shows the two jobs, sessions tab shows
the pinned **Welcome to CodexCrew** starter. Useful as the default
...
`$CODEXCREW_HOME` but are intentionally **not** included here. Text and
