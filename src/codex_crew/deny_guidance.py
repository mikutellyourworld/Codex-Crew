...
def _anchor_matcher(anchor: str, *, allow_plural: bool = False) -> re.Pattern[str]:
    ...
    refusal text produced by :mod:`codex_crew.security`, whose wording is fixed, so
...
_RULE_CLASSES: dict[str, str] = {
    ...
    "credential-exfil-codexcrew-token": DENY_CLASS_SELF_PROTECTION,
    "credential-exfil-codexcrew-token-argv": DENY_CLASS_SELF_PROTECTION,
...
def _rule_class_index() -> "Mapping[str, str]":
    ...
    :func:`codex_crew.security.edition_denied_rules` already validates that seam
...
#: would arrive wearing Codex Crew's own voice — the framing is the escalation, not
...
#: (``creds-agent``, ``codexcrew-core``, ``local-chorus-mcp``, ``mochi:mochi``).
