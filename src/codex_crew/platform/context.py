...
CodexCrew uses the Composed Platform Providers (CPP) model to share one core
...
``codex_crew.platform.interfaces`` — and ships a ``Default*`` adapter for each
...
if TYPE_CHECKING:  # avoid import cycles — config.loader imports heavy modules
    ...
    from codex_crew.platform.governance import GovernanceCeiling
    from codex_crew.platform.interfaces import (
    ...
    from codex_crew.platform.security_authority import PolicyAuthority
...
def _reserved_slot_is_default(field_name: str, value: Any) -> bool:
    ...
    try:
        ...
        # ``codex_crew`` import — every one above is inside ``TYPE_CHECKING`` —
        # because it is the leaf the rest of the platform imports.  ``defaults``
        # pulls in ``codex_crew.security`` and ``sso_status``, which reach back
        ...
        from codex_crew.platform import defaults as _defaults
...
class PlatformContext:
    ...
    Built once at boot by :func:`codex_crew.platform.bootstrap.bootstrap_context`
...
#
# Keys are ``"<path under src/codex_crew>::<enclosing function>"``.  Each value
...
def current_context() -> PlatformContext:
    ...
    probe or ``CODEXCREW_PROFILE=enterprise``) but no context was installed — meaning
    ...
    if _ACTIVE is None:
        ...
        # importing codex_crew.platform stays cheap; only the lazy-default path needs it.
        from codex_crew.config.loader import CodexCrewConfig
        from codex_crew.platform.bootstrap import (  # circular import: bootstrap imports context
            build_default_context,
        )
        from codex_crew.platform.discovery import (  # circular import: discovery imports context
            plugin_entry_points,
        )
        from codex_crew.platform.profile import (
...
def redact_via_context(text: str) -> str:
    ...
    try:
        ...
        from codex_crew.security import redact as _security_redact
