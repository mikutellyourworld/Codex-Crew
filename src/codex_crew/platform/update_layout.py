"""Install-layout detection shared between ``codexcrew update`` and the dashboard.
...
class InstallLayout(NamedTuple):
    """Describes how this Codex Crew instance was installed."""

    kind: str  # "git", "wheel", "dmg", "appimage", "deb", "rpm", "docker", or "source"
    proj: str  # CODEXCREW_PROJECT_DIR value (may be empty for non-git)
    is_git: bool
...
def detect_install_layout() -> InstallLayout:
    ...
    ``is_git_worktree`` FIRST classified a container whose ``CODEXCREW_PROJECT_DIR``
...
def release_channel() -> str:
    """The release channel this install follows, from ``$CODEXCREW_HOME/channel``.
...
def cdn_bases() -> tuple[str, str]:
    ...
    Respects ``CODEXCREW_CDN_BASE`` override for alternate CDNs / testing.
    """
    override = (os.environ.get("CODEXCREW_CDN_BASE") or "").strip().rstrip("/")
    if override:
        return override, override
    base = "https://github.com/mikutellyourworld/Codex-Crew/releases/latest/download"
    return base, base


#: Characters a CDN base may contain. ``CODEXCREW_CDN_BASE`` is operator-set and
