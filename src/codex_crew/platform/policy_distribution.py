...
* ``CODEXCREW_POLICY_URL`` (plus the ``CODEXCREW_POLICY_*`` siblings below) — the
  **fleet lever**, the same role ``CODEXCREW_SECURITY_POLICY`` already plays.  A
...
1. ``CODEXCREW_SECURITY_POLICY`` — an explicit LOCAL file.
...
from codex_crew.atomic_write import atomic_write, read_bytes_with_retry
from codex_crew.config.paths import config_dir
from codex_crew.constants import ENV_TRUTHY
...
POLICY_URL_ENV = "CODEXCREW_POLICY_URL"
...
POLICY_HEADERS_ENV = "CODEXCREW_POLICY_HEADERS"
POLICY_REFRESH_ENV = "CODEXCREW_POLICY_REFRESH_SECS"
POLICY_TIMEOUT_ENV = "CODEXCREW_POLICY_TIMEOUT_SECS"
POLICY_MAX_AGE_ENV = "CODEXCREW_POLICY_MAX_CACHE_AGE_SECS"
POLICY_UNAVAILABLE_ENV = "CODEXCREW_POLICY_ON_UNAVAILABLE"
...
POLICY_CACHE_ONLY_ENV = "CODEXCREW_POLICY_CACHE_ONLY"
...
#: conftest sweeps the ``CODEXCREW_POLICY_`` prefix so an autouse fixture need not import
#: this module.
...
def _fetch_file(request: FetchRequest) -> FetchedPolicy:
    ...
    **Neither the file NOR any ancestor directory may be writable by this process.**  A
    ...
    policy file has the channel designed for it — ``CODEXCREW_SECURITY_POLICY``, tier 1,
    ...
    **Opened ONCE, and the validator is a DIGEST of the bytes read.**  Stat-then-read is
    ...
    try:
        ...
        if stat_writable_by_current_user(st) or path_writable_by_current_user(path):
            raise PlatformCompositionError(
                f"policy source {request.url!r} is writable by the account Codex Crew runs "
                ...
                "CODEXCREW_SECURITY_POLICY instead — that is the channel designed for it, "
...
def _env_number(name: str, *, whole: bool) -> Optional[float]:
    ...
    ``CODEXCREW_POLICY_REFRESH_SECS=15m`` asked for a refresh, and silently giving
...
def write_cache(
    ...
    *,
    ...
    Written through the shared :func:`codex_crew.atomic_write.atomic_write` (never a
...
def _cache_write_lock(directory: Path) -> Iterator[None]:
    ...
    on its interval while ``codexcrew policy fetch --force`` runs from a shell.
...
def load_distributed_policy(
    ...
    **This function is not called only at boot**, which is what the fetch window
    ...
    # ``refresh_now`` does not come through here, so `codexcrew policy fetch` remains the
...
def effective_refresh_interval() -> int:
    ...
    ``codexcrew policy fetch`` that process exits immediately — so "applied" without a
    qualifier would claim something the command cannot do.
    """
    from codex_crew.platform.governance import active_policy_distribution
...
def _sanitize_detail(text: str, source: str) -> str:
    ...
    * the request CREDENTIAL, which the operator also configured, substituted by exact
      ...
      ``CODEXCREW_POLICY_HEADERS`` is deliberately arbitrary: an ``X-Fleet-Key`` holding an
    ...
    * anything the ENDPOINT put in the message. A malformed document reaches the CLI
      ...
      credential into ``codexcrew policy fetch`` and into ``GET /api/logs``. The bytes are
...
def _redact_source(text: str, source: str) -> str:
    ...
    **This module emits the source URL nowhere.**  One rule rather than a per-surface
    ...
    error text is what diagnoses a failure, and ``codexcrew policy source`` reports the
    scheme by design.
...
def tier1_local_policy() -> str:
    ...
    ``CODEXCREW_SECURITY_POLICY`` is precedence tier 1 and the documented rollback
    ...
    """
    raw = os.environ.get("CODEXCREW_SECURITY_POLICY", "").strip()
    if not raw:
        return ""
...
def _installed_digest() -> str:
    ...
    processes (gatewayd's reload, an app backend's boot, ``codexcrew policy fetch``)
...
def validate_ceiling(ceiling: GovernanceCeiling) -> None:
    ...
    """
    from codex_crew.platform.governance import assert_policy_signature_satisfied
    from codex_crew.platform.governance_profiles import assert_profile_floor

    tier1 = tier1_local_policy()
    if tier1:
        raise PlatformCompositionError(
            f"refusing to install a centrally fetched ceiling while {tier1} supplies "
            "one: CODEXCREW_SECURITY_POLICY is precedence tier 1 and the rollback lever, "
...
def apply_ceiling(ceiling: GovernanceCeiling) -> None:
    ...
    """
    from codex_crew.platform.context import current_context, set_context
    ...
    # ``resolve_selected_backend`` runs inside ``CodexCrewConfig.load()``, and reading a
...
def refresh_now(*, force: bool = False) -> RefreshOutcome:
    ...
    if tier1:
        detail = (
            f"{tier1} (CODEXCREW_SECURITY_POLICY) outranks the central source; "
    ...
    # `codexcrew policy fetch` — the step the operator guide recommends for verifying a
    ...
    if fetched.not_modified:
        if cached is None:
            ...
            # Reporting that as UNCHANGED made ``codexcrew policy fetch --force`` exit 0 having
    ...
    #
    # Migrating a source is a BOOTSTRAP change: move ``CODEXCREW_POLICY_URL`` (or the
...
def _audit_refresh(status: str, dist: PolicyDistribution) -> None:
    ...
    **Nothing here names the endpoint.**  The source is recorded as its SCHEME, and
    ...
    gateway log and from ``codexcrew policy fetch``, which are operator surfaces the
...
class _Refresher:
    ...
    def _next_interval(current: int) -> int:
        ...
        if tier1_local_policy():
            ...
            # the ladder afresh.
            logger.warning(
                "a local CODEXCREW_SECURITY_POLICY file now outranks the central source; "
                "stopped polling"
            )
            return 0
