"""The CodexCrew Governance Model — archetypes, ceiling, evaluator, loader.
...
from codex_crew.platform.admission import (
...
from codex_crew.platform.governance_health import mark_governance_incident
from codex_crew.platform.tool_paths import TARGET_PATH_KEYS, target_paths
...
_POLICY_ENV = "CODEXCREW_SECURITY_POLICY"
...
_POLICY_DISTRIBUTION_URL_ENV = "CODEXCREW_POLICY_URL"
#: Named here only so the declared-source refusal can point at it; the engine owns it.
_POLICY_DISTRIBUTION_HEADERS_ENV = "CODEXCREW_POLICY_HEADERS"
...
_POLICY_DISTRIBUTION_CACHE_ONLY_ENV = "CODEXCREW_POLICY_CACHE_ONLY"
...
# document claims.  Reported verbatim by ``codexcrew policy show`` so an operator
...
_CU_ACTION_CLASSES: Dict[str, Tuple[str, ...]] = {
    ...
    # Control plane: drops CodexCrew's OWN cached snapshots and touches no other
...
CU_MCP_SERVER = "codexcrew-computer"
...
def computer_use_action_from_title(tool_name: str) -> str:
    ...
    ``mcp__codexcrew-computer__computer_click`` -> ``computer_click``.  Returns
...
# (``codex_crew.platform.tool_paths.TARGET_PATH_KEYS``) so the governance
...
# them: a policy cannot deny a capability by leaving it out.  ``codexcrew policy
...
SCOPE_CATALOG: Dict[str, ScopeSpec] = {
    ...
    # by codex_crew.safety_override at the activation seam, against the HOST
    ...
    #
    # Distinct from the selectable-backend REGISTRY in ``codex_crew.acp_backends``:
    ...
    #
    # Consulted by ``codex_crew.agent_backend_governance`` at exactly ONE place: it
    ...
    #   * ``codexcrew telemetry enable`` — exits 1 without writing, for the same
    ...
    #   * ``codexcrew config set dashboard.tailscale.enabled true`` — exits 1
...
class PolicyDistribution:
    ...
    redeploy.  ``codex_crew.platform.policy_distribution`` is the engine; this
    ...
    **No credentials live here.**  ``headers`` is deliberately absent: a document
    ...
    from the ``CODEXCREW_POLICY_HEADERS`` environment variable instead — the same
    channel that already carries ``CODEXCREW_SECURITY_POLICY``.
    """
    ...
    def from_dict(d: Mapping[str, object]) -> "PolicyDistribution":
        ...
        # credential travels in `CODEXCREW_POLICY_HEADERS`, which "a published document must
...
def active_policy_distribution() -> PolicyDistribution:
    ...
    """
    try:
        from codex_crew.platform.context import installed_context
...
def _apply_agentcore_gateway_url(
    ...
    """
    from codex_crew.platform.agentcore_schema import normalize_agentcore_gateway_url
...
def _apply_agentcore_workload_name(
    ...
    from codex_crew.platform.agentcore_schema import normalize_agentcore_workload_name
...
# ``codexcrew policy validate``, rendered as a governed scope in
# ``codexcrew policy show``, and left ``sandbox.min_level`` absent — so
...
def _policy_trust_settings() -> Tuple[bool, Dict[str, str]]:
    ...
    Sourced from the **admission policy** (``CODEXCREW_ADMISSION_POLICY`` env path,
...
def _policy_signature_required() -> bool:
    ...
    ``read_policy_trust_root`` already does.  ``codexcrew doctor`` surfaces it.
...
def load_security_policy(
    *, bundled_loader: Optional[Callable[[], Optional[Mapping[str, object]]]] = None
    ...
    2. **the centrally distributed document** — fetched from ``CODEXCREW_POLICY_URL``
       ...
       :mod:`codex_crew.platform.policy_distribution`.  Tier 1 stays above it so an
    ...
    **All three tiers are verified — none is exempt.**  The plugin-admission
    ...
    if (
        ...
        from codex_crew.platform.policy_distribution import load_distributed_policy
...
def _declared_distribution(
    ...
    settings and no ``source`` is legitimate when ``CODEXCREW_POLICY_URL`` supplies the
...
def strip_ungoverned_auto_approve(servers: Mapping[str, object]) -> Dict[str, object]:
    ...
    ``agent.py``'s managed-server block states the rule for the servers CodexCrew
...
def may_skip_gate_now(ref: str) -> bool:
    ...
    try:
        from codex_crew.platform.governance_profiles import any_configured_profile_governs
...
def sanitize_agent_config_governance(config: MutableMapping[str, object]) -> None:
    ...
    config about to be written to ``codexcrew.json``.
...
def assert_governance_paths_protected() -> None:
    ...
    # The data home moved to ~/.codex-crew; check the CURRENT home's trust-root
    # paths are gated (the legacy ~/.codexcrew forms remain in the keystone too,
    ...
    required = (
        ...
        ".codex-crew/denied_commands.json",
        ...
        f".codex-crew/{_POLICY_CACHE_LEAF}",
    )
