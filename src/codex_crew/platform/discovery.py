...
``codexcrew.plugins`` entry point pointing at its composition root
...
PLUGIN_GROUP = "codexcrew.plugins"
...
def plugin_entry_points() -> List[importlib.metadata.EntryPoint]:
    """Return the registered ``codexcrew.plugins`` entry points (may be empty).
...
def discover_companion_context(
    profile: str, cfg: "CodexCrewConfig", *, policy: "AdmissionPolicy | None" = None
    ...
    if not eps:
        raise PlatformCompositionError(
            ...
            "companion package, or set CODEXCREW_PROFILE=standalone to run the "
