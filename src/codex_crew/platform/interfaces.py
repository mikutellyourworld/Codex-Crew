...
if TYPE_CHECKING:
    ...
    from codex_crew.security import DeniedCommandRule
    from codex_crew.skill_providers.base import SkillProvider
    from codex_crew.tips_pool import TipsPool
...
class ProviderRegistry(Protocol):
    ...
    def create_factory(self, cfg: "CodexCrewConfig") -> Callable[..., Any]:
...
class AgentExecutableResolver(Protocol):
    ...
    ``sandbox.py`` calls this before applying CodexCrew's OS-level sandbox so an
    ...
    ``kiro-cli`` paths and the explicit ``CODEXCREW_KIRO_BIN`` override are used
...
class IdentityProvider(Protocol):
    ...
    def preflight_checks(self) -> List[Callable[[], None]]:
        ...
        WIRED: ``codex_crew.preflight.run_preflight_checks`` (called from the
...
class SkillDiscoveryProvider(Protocol):
    ...
    def skill_providers(self) -> List["SkillProvider"]:
        ...
        ``codex_crew.skill_providers.base.SkillProvider`` (async ``search`` /
...
class ExternalAccessPolicy(Protocol):
    ...
    * **Cloud deployment.** ``codex_crew/deploy/`` provisions real infrastructure
...
class AppsLoader(Protocol):
    ...
    def default_registries(self) -> List[Dict[str, Any]]:
        ...
        inside ``CodexCrewConfig`` is deliberate: an edition default must never be
...
class KnowledgeProvider(Protocol):
    ...
    def extra_connectors(self, cfg: "CodexCrewConfig") -> Dict[str, Any]:
        """Return ``{source_type: BaseConnector}`` to merge into the sync map.

        The values are ``codex_crew.knowledge.connectors.base.BaseConnector``
...
class TunnelProvider(Protocol):
    ...
    def status_snapshot(self) -> Optional[Dict[str, Any]]:
        ...
        Edition-neutral primitives only (no import of ``codex_crew.tunnel``): keys
    ...
    async def ensure_available(self, *, install: bool = True) -> str:
        ...
        demand (the ``/codexcrew dashboard`` command, the setup wizard).
...
class JailProvider(Protocol):
    ...
    **Re-entry contract.** Before a successful re-exec the core gate sets the
    ``CODEXCREW_JAILED`` env marker; the jailed CHILD re-runs the gate, sees the
    ...
    def maybe_reexec_into_jail(self, argv: List[str], mode: str) -> Optional[int]:
        ...
        **mode contract:** ``"off"`` / already-jailed / disabled → ``None``.
        ...
        gate's ``CODEXCREW_JAILED`` re-entry guard short-circuits before this is
