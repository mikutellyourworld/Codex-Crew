...
Precedence (first match wins):
  1. ``CODEXCREW_PROFILE`` env var (explicit operator/dev override).
  2. A non-empty ``codexcrew.plugins`` entry-point group (companion installed) —
     the cheap, authoritative signal: capability comes from the installed
     companion, so its presence is what actually matters.
  3. Identity signal: a present SSO-marker directory (``CODEXCREW_SSO_MARKER_PATH``,
     default ``~/.midway``), but ONLY when the opt-in ``CODEXCREW_SSO_PROFILE_PROBE``
     env var (or the legacy ``CODEXCREW_MIDWAY_PROFILE_PROBE``, still set by
...
from codex_crew.platform.context import PROFILE_ENTERPRISE, PROFILE_STANDALONE
...
# Back-compat aliases for the ``CODEXCREW_PROFILE`` env value: an operator or a
...
#
# Two names are honored: the current ``CODEXCREW_SSO_PROFILE_PROBE`` and the
# legacy ``CODEXCREW_MIDWAY_PROFILE_PROBE`` still set by already-deployed managed
...
_SSO_PROBE_ENV = "CODEXCREW_SSO_PROFILE_PROBE"
_LEGACY_SSO_PROBE_ENV = "CODEXCREW_MIDWAY_PROFILE_PROBE"
...
_SSO_MARKER_ENV = "CODEXCREW_SSO_MARKER_PATH"
_DEFAULT_SSO_MARKER_NAME = ".midway"


def resolve_profile(cfg: "CodexCrewConfig", *, entry_points: "Sequence[object]") -> str:
    """Resolve the active profile.  See module docstring for precedence."""
    # 1. Explicit env override.
    env = os.environ.get("CODEXCREW_PROFILE", "").strip().lower()
    if env in _VALID_PROFILES:
        return env
    if env in _PROFILE_ALIASES:
        return _PROFILE_ALIASES[env]
    if env:
        logger.warning("Unknown CODEXCREW_PROFILE=%r; falling back to standalone", env)
