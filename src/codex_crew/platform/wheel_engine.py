...
The managed venv (``${CODEXCREW_HOME}-venv``) is the one install whose bytes the
...
provenance story covers the Codex Crew wheel itself, not the dependency set.
...
class ManagedVenvLayout:
    ...
    def is_managed_tree(self, path: Path) -> bool:
        ...
        # branch below, which every real install satisfies (bin/codexcrew ships
        ...
        # `CODEXCREW_VENV=/srv/crew` makes the prefix "crew-", which a sibling
        ...
        # carries this engine's own artifacts: the codexcrew console script
        ...
        try:
            if resolved.is_relative_to(parent.resolve()):
                ...
                return (candidate / "bin" / "codexcrew").exists() or (
...
def managed_venv_layout() -> ManagedVenvLayout:
    ...
    Mirrors cli.sh's path rule exactly: ``CODEXCREW_VENV`` wins, else the venv
    lives BESIDE the data home (``${CODEXCREW_HOME%/}-venv``) — never inside
    it, for the blast-radius reason cli.sh documents.
    """
    override = os.environ.get("CODEXCREW_VENV", "").strip()
    if override:
...
def _download_to_file(url: str, dest: Path, cap: int, timeout: float, expected_sha: str) -> None:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "codexcrew-update/1"})
...
def download_verified_wheel(payload: dict[str, str], dest_dir: Path) -> Path:
    ...
    wheel_path = dest_dir / f"codexcrew-{payload['version']}-py3-none-any.whl"
...
_SHADOW_SENTINEL = ".codexcrew-shadow-incomplete"
...
def build_shadow_venv(wheel_path: Path, shadow_dir: Path, stable_link: Path | None = None) -> None:
    ...
    if shadow_dir.exists() or shadow_dir.is_symlink():
        ...
        # a versioned name (a custom CODEXCREW_VENV shares a parent with
    ...
    try:
        ...
        (shadow_dir / _SHADOW_SENTINEL).write_text(
            "created by codex_crew.platform.wheel_engine; removed after verification\n",
            encoding="utf-8",
        )
    except OSError as exc:
...
def verify_shadow_venv(shadow_dir: Path, expected_version: str) -> None:
    ...
    try:
        proc = subprocess.run(
            [
                ...
                "utf8",
                "-c",
                "import codex_crew; print(codex_crew.__version__)",
            ],
            capture_output=True,
            text=True,
            encoding="utf-8",
    ...
    if proc.returncode != 0:
        ...
        raise WheelUpdateError(
            "shadow venv cannot import codex_crew — not promoting"
    ...
    if not (shadow_dir / "bin" / "codexcrew").exists():
        raise WheelUpdateError("shadow venv is missing the codexcrew console script")
...
def repoint_launcher_symlink(layout: ManagedVenvLayout) -> bool:
    """Point ``~/.local/bin/codexcrew`` through the stable link.
    ...
    """
    launcher = Path.home() / ".local" / "bin" / "codexcrew"
    target = layout.stable_link / "bin" / "codexcrew"
    ...
    *,
    ...
    try:
        if not try_acquire_lock(lock_fd, exclusive=True):
            raise WheelUpdateError(
                "another codexcrew update is already in progress — wait for it "
...
def _apply_locked(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="codexcrew-update-", dir=str(_staging_dir())) as tmp:
        workdir = Path(tmp)
        ...
        progress(f"Downloading codexcrew {version}…")
        ...
        if not repoint_launcher_symlink(layout):
            ...
            progress(
                "Note: the ~/.local/bin/codexcrew launcher was not repointed; "
