...
if TYPE_CHECKING:
    from codex_crew.publish_provider import PublishProvider
...
from codex_crew import security, sso_status
from codex_crew.platform.interfaces import (
    CapabilityResult,
    InterceptDecision,
    MobileConnectMethod,
...
# import ``codex_crew.platform`` at module-load time, so importing them at the top
...
# ``codex_crew.platform.context`` (the ``sel.py`` deferred pattern).  Because that
...
class DefaultPublishRegistry:
    ...
    def register_publish_providers(self) -> None:
        ...
        """
        from codex_crew.publish_provider import register_provider

        def _build() -> PublishProvider:
            from codex_crew.publish import personal_drive
...
class DefaultAgentRuntime:
    ...
    def managed_mcp_servers(self) -> Dict[str, dict]:
        ...
        from codex_crew import agent  # circular import: agent imports platform
...
class DefaultSandboxPolicy:
    ...
    def strict_dirs(self) -> List[str]:
        from codex_crew import sandbox  # circular import: sandbox imports platform
...
class DefaultEmbeddingSource:
    ...
    def registry_model(self) -> str:
        from codex_crew import embeddings  # circular import: embeddings imports platform
...
class DefaultAppRegistryPolicy:
    ...
    def public_git_hosts(self) -> "frozenset[str]":
        from codex_crew.apps import registry  # circular import: apps.registry imports platform
