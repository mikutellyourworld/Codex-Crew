...
source** (`CODEXCREW_ADMISSION_POLICY` env path, or
`~/.codex-crew/admission_policy.json`), never from the plugin being admitted, so a
...
_POLICY_ENV = "CODEXCREW_ADMISSION_POLICY"
...
_MANIFEST_FILENAME = "codexcrew_plugin.json"
...
def canonical_signing_bytes(body: Mapping[str, object]) -> bytes:
    """The ONE canonicalization every CodexCrew trust-root signature is taken over.
...
_DEFAULT_POLICY_BODY: Dict[str, object] = {
    ...
    "capability_ceiling": {},
    "_comment": (
        "Default permissive admission policy seeded by CodexCrew at first run. "
...
def seed_default_policy() -> bool:
    ...
    is skipped entirely when the ``CODEXCREW_ADMISSION_POLICY`` env override is
...
def _record_admission_posture(posture: str) -> None:
    ...
    try:
        from codex_crew.platform.governance_health import set_admission_posture
...
def load_admission_policy() -> AdmissionPolicy:
    ...
    A MISSING policy (no ``CODEXCREW_ADMISSION_POLICY`` env override and no file at
...
def read_policy_trust_root() -> "AdmissionPolicy":
    ...
    here is a reliability event: log it, stay predictable, let ``codexcrew doctor``
    ...
    try:
        ...
        logger.warning(
            ...
            "policy-signature requirement as unset (codexcrew doctor reports this)",
            path,
            exc_info=True,
        )
        return AdmissionPolicy()
...
def evaluate_admission(
    ...
    #    package spoof an approved identity via codexcrew_plugin.json; checking
