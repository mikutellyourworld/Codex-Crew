...
def audit_governance_degraded(
    ...
    *,
    ...
    safe inside the stdio ``codexcrew-core`` MCP server whose stray stdout/stderr
...
def _infer_surface(session_key: str) -> str:
    ...
    """
    from codex_crew.sel import _infer_source
...
def _ceiling_token() -> Tuple[bool, int]:
    ...
    * **Which ceiling is installed?** ``context.governance_generation()`` changes on
    ...
    """
    try:
        from codex_crew.platform.context import current_context, governance_generation
...
class ProfileStore:
    """Loads + caches profiles from ``~/.codex-crew/profiles`` with mtime hot-reload.
    ...
    def _reload(self, directory: Path) -> None:
        ...
        try:
            ...
            # case (a fresh CODEXCREW_HOME has no profiles/ dir). This is ABSENT,
        ...
        if unrec:
            try:
                ...
                if getattr(current_context(), "governance", None) is not None:
                    ...
                    try:
                        from codex_crew.platform.governance_health import (
                            mark_governance_incident,
                        )
...
def assert_profile_floor(ceiling: "object") -> None:
    ...
    if ceiling is None:
        return
    from codex_crew.platform.governance import GovernanceCeiling, assert_governance_floor
...
def governance_permits(
    ...
    *into this call* from a stdio site so the degrade WARNING (whose stderr is
    ...
    from codex_crew.platform.governance import Decision, resolve
    ...
    try:
        ...
        from codex_crew.platform.governance import Decision as _D
...
def vet_and_audit(
    ...
    *,
    ...
    try:
        ...
        # the test suite's convention is patch("codex_crew.sel.sel"), which a
...
def governance_floor_ordinal(
    ...
    *,
    ...
    from codex_crew.platform.governance import resolve_ordinal
