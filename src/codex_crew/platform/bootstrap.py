...
from codex_crew.platform.defaults import (
...
from codex_crew.platform.discovery import discover_companion_context, plugin_entry_points
from codex_crew.platform.governance import (
    assert_governance_paths_protected,
    assert_policy_signature_satisfied,
    load_security_policy,
)
from codex_crew.platform.profile import resolve_profile
from codex_crew.platform.security_authority import PolicyAuthority, assert_security_floor
...
def boot_platform(cfg: "CodexCrewConfig") -> PlatformContext:
...
def build_default_context(
    cfg: "CodexCrewConfig", *, profile: str = PROFILE_STANDALONE
) -> PlatformContext:
...
def bootstrap_context(cfg: "CodexCrewConfig") -> PlatformContext:
    ...
    # ceiling.
    from codex_crew.platform.governance_profiles import assert_profiles_within_ceiling
    ...
    try:
        from codex_crew.agent_backend_governance import narrow_selectable_backends
        ...
        if narrow_selectable_backends():
            ...
            # every later ``CodexCrewConfig.load()`` (which re-reads from disk) sees
