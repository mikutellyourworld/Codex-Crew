...
MANAGED_BY_CODEXCREW = "codexcrew"
MANAGED_BY_GIT = "git"
...
def is_git_worktree(root: str) -> bool:
    ...
    **When git cannot answer, this degrades to the on-disk markers of a
    ...
    unrelated repository that ``CODEXCREW_PROJECT_DIR`` happens to name — that
...
def running_from_checkout(root: str) -> bool:
    """Is *root* the checkout the ``codex_crew`` package THIS process runs from?
    ...
    release install (a wheel in its own venv) whose ``CODEXCREW_PROJECT_DIR``
    ...
    Delegates to :func:`codex_crew._bootstrap._source_checkout_root`, which
...
class UpdateCapability:
    ...
    def for_channel(self, channel: str) -> "UpdateCapability":
        ...
        """
        if self.managed_by != MANAGED_BY_CODEXCREW or self.remediation is None:
            return self
...
def derive_capability(
    ...
    *install_root* defaults to ``CODEXCREW_PROJECT_DIR`` and *dist* to the value
    ...
    if install_root is None:
        install_root = os.environ.get("CODEXCREW_PROJECT_DIR", "")
    ...
    if is_git_worktree(install_root) and running_from_checkout(install_root):
        ...
        return UpdateCapability(
            ...
            remediation=_command_remediation(
                "Update this checkout from a terminal.", "codexcrew update"
            ),
        )
    ...
    from codex_crew.platform.update_layout import release_channel, wheel_update_command

    return UpdateCapability(
        supported=True,
        managed_by=MANAGED_BY_CODEXCREW,
        mode=MODE_NOTIFY,
        can_download=True,
