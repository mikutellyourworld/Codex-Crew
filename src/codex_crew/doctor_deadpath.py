...
* **Managed specs** (the ones ``install_agent`` / ``rebuild_agent_config`` own,
  i.e. :data:`~codex_crew.agent_files.OWNED_KIRO_AGENT_FILES`) with dead paths are
...
* **Foreign specs** (other tools' agents sharing the directory) are report-only,
  naming the spec file, the server name, and the dead path. Codex Crew never
...
def kiro_agents_dir_path() -> Path:
    ...
    Resolved live via :func:`codex_crew.config.paths.kiro_agents_dir`, which
...
class DeadPath:
    ...
    ``env[CODEXCREW_HOME]``) so a report points at exactly what to fix.
    """

    spec: str  # spec filename (e.g. "codexcrew.json")
...
def _sanitize_for_terminal(value: str) -> str:
    ...
    ``unreadable`` reason — are printed to the operator's terminal by ``codexcrew
...
def _default_repair() -> None:
    ...
    (:func:`codex_crew.agent.rebuild_agent_config`), which re-resolves every
...
def check_dead_paths(*, agents_dir: Path | None = None, repair=_default_repair) -> DeadPathReport:
    ...
    Args:
        agents_dir: The agents directory to scan. Defaults to
            ...
            themselves (notably ``codexcrew doctor``, which has its OWN override
...
def doctor_dead_paths(issues: list[str], *, agents_dir: Path | None = None) -> None:
    """Render the ``Agent Spec Paths`` section of ``codexcrew doctor``.
    ...
    appended to *issues* so ``codexcrew doctor`` exits nonzero and the failure
