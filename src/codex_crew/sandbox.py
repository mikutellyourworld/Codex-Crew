...
The parent CodexCrew process is completely unaffected — isolation applies
only to the spawned child.  Falls back gracefully to no sandbox when the
OS mechanism is unavailable (logged as warning).

Config: ``"sandbox": "auto" | "off"`` in ``~/.codex-crew/config.json``.
...
from codex_crew.constants import CODEXCREW_SPAWNED_ENV, CODEXCREW_SPAWNED_VALUE
from codex_crew.identity_stores import AUTH_SQLITE_DB, AUTH_SQLITE_SIDECAR_SUFFIXES
from codex_crew.pinned_fs import fd_real_path
...
# its own pid ("codexcrew_sb_<pid>_..."); the pid is the liveness key the
...
_MOUNT_SOURCE_PREFIX = "codexcrew_sb_"
...
_CREW_HOME_DEFAULT = ".codex-crew"

#: Both data-home spellings every crew-relative rule below has to cover: a host that
#: has not run the ``~/.codexcrew`` -> ``~/.codex-crew`` migration still holds the real
#: bytes at the legacy path, and ``config_dir()`` can resolve to either.
_CREW_HOME_PREFIXES: tuple[str, ...] = (".codex-crew", ".codexcrew")
...
#
# Reconciling the two is NOT a union, and the reason is specific: Codex Crew's own MCP
...
_CREW_HIDDEN_LEAVES: tuple[str, ...] = (
    ...
    # the in-sandbox CLI reads the STAGED store under ``.codex-crew-auth-staging``, and
...
_CREW_SANDBOX_VISIBLE_LEAVES: tuple[str, ...] = (
    # Holds this launcher itself (``<config_dir>/run/codexcrew_sandbox_*.py``), so the
...
def app_backend_visible_targets(app_name: str, mode: str = "standard") -> tuple[str, ...]:
    ...
    paths when ``CODEXCREW_HOME`` moves the data home — so each returned path matches
...
def carveout_shadowed_by_foreign_mask(path: str, mode: str = "standard") -> bool:
    ...
    ``CODEXCREW_HOME`` under ``~/.aws``) would hand every spawn that asks for a
...
def _sealable_absent_ceilings() -> tuple[list[str], list[str]]:
    ...
    requirement, since a stub in the deprecated ``~/.codexcrew`` of a migrated host is a
...
def _publish_empty_ceiling(target: str, parent: str) -> bool:
    ...
    tmp = ""
    try:
        fd, tmp = tempfile.mkstemp(dir=parent, prefix=".codexcrew-ceiling-", suffix=".tmp")
...
_STRICT_DIRS: list[str] = [
    ...
    ".codex-crew/policy_cache",
    ".codexcrew/policy_cache",
    ".codex-crew/run/voice-runtime",
    ".codexcrew/run/voice-runtime",
]
...
def _relocated_crew_targets(leaves: tuple[str, ...]) -> list[str]:
    ...
    ``CODEXCREW_HOME=/srv/crew`` moves the data home out from under all of them and no rule
...
def _relocated_policy_cache_dirs() -> list[str]:
    ...
    ``Path.home()``, so ``CODEXCREW_HOME=/srv/crew`` moves the data home out from under
...
def prime_voice_runtime_sandbox_paths() -> str:
    ...
    with _voice_runtime_paths_lock:
        ...
        if not stat.S_ISDIR(home_info.st_mode) or stat.S_ISLNK(home_info.st_mode):
            raise OSError("Codex Crew data home does not resolve to a real directory")
...
_VOICE_GUARD_REMEDY = "Pick a project subdirectory that does not contain the Codex Crew data home."
...
def _voice_runtime_guard_message(
    ...
    who picked ``~`` (an ancestor of the default ``~/.codex-crew`` data home)
    ...
    if relationship == "contains":
        return (
            f"macOS agent workspace {workspace_path!r} overlaps Codex Crew's "
    ...
    if relationship == "alias":
        return (
            f"macOS agent workspace {workspace_path!r} aliases Codex Crew's "
    ...
    return (
        ...
        f"separate from Codex Crew's protected voice runtime {runtime_path!r}: "
...
def assert_voice_runtime_outside_agent_workspace(workspace: str | os.PathLike[str]) -> None:
    ...
    Kiro's internal macOS sandbox cannot nest inside Codex Crew's Seatbelt
...
def _is_policy_cache_dir(path: str) -> bool:
    ...
    ``~/.codexcrew`` entry that the deny lists must keep covering, and the relocated
...
_CC_FILES: list[str] = [
    ...
    ".netrc",
    ".git-credentials",
    # CodexCrew's channel-credential file. The data home moved to ~/.codex-crew,
    # so the live .env is now ~/.codex-crew/.env; the legacy ~/.codexcrew/.env is
...
# shell runs).
#  - PYTHONPATH / PYTHONHOME: Codex Crew's runtime may export PYTHONPATH
#    pointing at its own site-packages; a foreign server that inherits it
#    prepends Codex Crew's site-packages to sys.path and imports Codex Crew's
...
# ``_SENSITIVE_ENV_PREFIXES`` because CodexCrew's OWN sandboxed Python
# subprocesses (cron scripts, app backends, code-review workers) import
# ``codex_crew`` via PYTHONPATH -- and on the packaged app run the BUNDLED
...
_AGENT_DENIED_ENV_KEYS: list[str] = [
    ...
    # deliberately:
    #
    # * ``CODEXCREW_POLICY_HEADERS`` is a live bearer credential for the fleet's own
    ...
    # * ``CODEXCREW_POLICY_URL`` is credential-bearing in its own right whenever the
    ...
    #
    # Spelled as CONCRETE NAMES rather than a ``CODEXCREW_POLICY_`` prefix, because this
    ...
    "CODEXCREW_POLICY_URL",
    "CODEXCREW_POLICY_HEADERS",
    "CODEXCREW_POLICY_REFRESH_SECS",
    "CODEXCREW_POLICY_TIMEOUT_SECS",
    "CODEXCREW_POLICY_MAX_CACHE_AGE_SECS",
    "CODEXCREW_POLICY_ON_UNAVAILABLE",
    "CODEXCREW_POLICY_CACHE_ONLY",
]
...
_LINUX_REMEDY_GUIDANCE = {
    REMEDY_APPARMOR_USERNS: (
        ...
        "AppArmor profile carries no CAP_SYS_ADMIN. Run `codexcrew service "
        "install` to install the narrow codexcrew-userns AppArmor profile (it "
        "grants only `userns` and applies to the codexcrew service alone). "
...
#:
#: Kept deliberately free of ``codex_crew`` imports and run under ``-I -S``, like
...
def _probe_sandbox_exec() -> bool:
    ...
    # binary: the probe runs under (allow default) with CodexCrew's credentials,
    ...
    if not os.path.exists(target):
        return False
    fd, profile_path = tempfile.mkstemp(suffix=".sb", prefix="codexcrew_probe_")
    try:
...
def _resolve_agent_executable(executable: str) -> str:
    ...
    launcher with the direct executable it ultimately invokes so CodexCrew can
...
def _build_launcher_script(
    ...
    *,
    ...
    if sandbox_level in ("cc", "strict"):
        ...
        # (the file-level bind-mount of ~/.codex-crew/.env hides them on disk;
    ...
    if strip_python_env:
        ...
        # CodexCrew's PYTHONPATH/PYTHONHOME leak in and shadow their own deps.
    ...
    # (`sel_hmac.key`, `token_signing.key`, `.codex-crew/.env` are files), and Papyrus
...
"""Namespace sandbox launcher — spawned by CodexCrew."""
import sys
# Harden against stdlib shadowing. This launcher runs as
# ``python <config_dir>/run/codexcrew_sandbox_*.py``, so CPython prepends the
...
def main():
    ...
    os.environ["CODEXCREW_HOST_PID"] = str(os.getpid())
    ...
    if pid > 0:
        ...
        # binds (e.g. /tmp on ext4 over ~/.codex-crew/.env on ext4) can corrupt the
        ...
        for _candidate in (f"/run/user/{{REAL_UID}}", "/dev/shm"):
            try:
                ...
                _probe = tempfile.mkdtemp(dir=_candidate, prefix="codexcrew_sbprobe_")
                try:
                    os.rmdir(_probe)
                except FileNotFoundError:
        ...
        # tmpfs probe above deliberately uses the sibling "codexcrew_sbprobe_"
        # prefix, OUTSIDE the pid-parsed family, so the janitor never races
        # its mkdtemp/rmdir window.
        _src_prefix = "codexcrew_sb_%d_" % os.getpid()
        ...
        os.environ["CODEXCREW_SANDBOX_ACTIVE"] = "1"
        ...
        os.environ["CODEXCREW_SANDBOX_LEVEL"] = SANDBOX_LEVEL
        ...
        if _libc.prctl:
            ...
            if _machine == "x86_64":
                ...
                # exact escape Step 6 exists to deny. _inside_codexcrew_sandbox()
                ...
                sys.exit(
                    ...
                    "agent.sandbox_allow_unsandboxed_exec=true in "
                    "~/.codex-crew/config.json." % _machine
                )
        ...
        # so without this check two ordinary directories — `~/.codex-crew-auth-staging`
...
def namespace_argv(
    ...
    *,
    ...
    fd, path = tempfile.mkstemp(
        suffix=".py", prefix=f"codexcrew_sandbox_{os.getpid()}_", dir=run_dir
    )
...
# cannot nest inside CodexCrew's sandbox-exec wrap — the kernel returns EPERM
...
# kiro's internal sandbox ON  -> CodexCrew's seatbelt OFF for kiro-cli spawns
# kiro's internal sandbox OFF -> CodexCrew's seatbelt ON (unchanged default)
# (``~/.kiro/settings`` is the kiro-cli backend's own directory, distinct from
# CodexCrew's data home ``~/.codex-crew``; the filename is the literal kiro-cli ships.)
...
def kiro_internal_sandbox_enabled() -> bool:
    ...
    False, which keeps CodexCrew's own sandbox engaged — failure resolves
    ...
    try:
        ...
        # Every failure resolves toward CodexCrew's own sandbox.
        return False
...
def _spawns_kiro_cli(argv: list[str]) -> bool:
    ...
    cron script) has no internal sandbox of its own and MUST keep CodexCrew's
...
def _delegate_to_kiro_internal_sandbox(
    ...
    *,
    ...
    mutual exclusion with Codex Crew's seatbelt; on Windows it is restricted to a
    positive first-party Kiro backend classification because Codex Crew has no
    ...
    try:
        ...
        sel().log_tool_invocation(
            ...
            resources=(
                ...
                "this spawn; Codex Crew has no native Windows sandbox backend"
                if sys.platform == "win32"
                else "macOS sandbox mutual exclusion: kiro internal sandbox on -> "
                "CodexCrew seatbelt off for this kiro-cli spawn"
            ),
        ...
        # be audited does not happen. The caller continues through Codex Crew's
        ...
        logger.warning(
            ...
            "delegation; falling back to Codex Crew's sandbox policy",
            exc_info=True,
        )
        return None
    ...
    if not _kiro_delegation_warned:
        ...
        logger.warning(
            ...
            "sandbox and skipping Codex Crew's OS wrapper. Env scrubbing still "
            "applies.",
...
def sandbox_exec_argv(
    ...
    *,
    ...
    run_dir = _ensure_run_dir()
    fd, path = tempfile.mkstemp(
        suffix=".sb", prefix=f"codexcrew_sandbox_{os.getpid()}_", dir=run_dir
    )
    ...
    # its own env scrub (see the export beside ``CODEXCREW_HOST_PID``). Without
    # this, an in-sandbox ``wrap_argv`` call cannot tell that CodexCrew's own
...
def _sandbox_env_unset_args(sandbox_level: str, strip_python_env: bool) -> list[str]:
    ...
    env-scrub guarantee is identical whether or not CodexCrew's own seatbelt is
...
def cleanup_stale_sandbox_profiles(*, legacy_dir: str | None = None) -> int:
    ...
    Also sweeps legacy /tmp/codexcrew_sandbox_*.py files that predate the
    ...
    if os.path.isdir(run_dir):
        for entry in os.listdir(run_dir):
            if not entry.startswith("codexcrew_sandbox_"):
                continue
            ...
            middle = entry[len("codexcrew_sandbox_") : -len(suffix)]
    ...
    # ── Sweep legacy /tmp/codexcrew_sandbox_*.py (age only, no PID segment) ──
    if os.path.isdir(legacy_dir):
        try:
            with os.scandir(legacy_dir) as it:
                for dentry in it:
                    if not dentry.name.startswith("codexcrew_sandbox_"):
                        continue
...
def _mount_pinned_source_names(
    ...
    *,
    ...
    # Default: the keyed ``codexcrew_sb_<pid>_`` shape. ``matcher`` lets the
...
def _cleanup_stale_sandbox_mount_sources(*, roots: Sequence[str] | None = None) -> int:
    ...
    known-hosts copy, all named ``codexcrew_sb_<pid>_*`` on a tmpfs root. The
    ...
    ``codexcrew_sb_<pid>_`` shape with an ASCII positive pid is touched.
    Foreign ``tmp*`` names carry no liveness key and are left alone, as are
    ``codexcrew_sandbox_*`` launcher scripts, the ``codexcrew_sbprobe_*`` tmpfs
...
def _cleanup_retired_acp_snapshot_dir() -> int:
    ...
    CodexCrew used to copy the whole kiro-cli binary here per ACP spawn generation
    ...
    the sweep above only matches `codexcrew_sandbox_*` FILES in this same dir, and
...
def _allow_no_isolation() -> bool:
    ...
    try:
        from codex_crew.config.loader import (
            CodexCrewConfig,  # circular import: sandbox is a low-level dep of config.loader
        )

        return bool(getattr(CodexCrewConfig.load().agent, "sandbox_allow_no_isolation", False))
    except Exception:
        return False
...
def _allow_unsandboxed_exec() -> bool:
    ...
    try:
        ...
        return bool(getattr(CodexCrewConfig.load().agent, "sandbox_allow_unsandboxed_exec", False))
    except Exception:
        return False
...
def configured_sandbox_mode() -> str:
    ...
    inside Codex Crew's (macOS Seatbelt returns EPERM) — a spawn that takes the
    ...
    try:
        ...
        return str(getattr(CodexCrewConfig.load().agent, "sandbox", _SANDBOX_MODE_FALLBACK))
    except Exception:
        logger.warning(
...
# CodexCrew namespace sandbox. Deny-by-default: the gate keys ONLY on the
# explicit, single-purpose ``CODEXCREW_SANDBOX_ACTIVE``, which is exported at
# exactly one site — the namespace launcher main() (see the export beside
# ``CODEXCREW_HOST_PID``). We deliberately do NOT key on ``CODEXCREW_HOST_PID``:
...
# launcher sets ``CODEXCREW_SANDBOX_ACTIVE`` at the same site, no fallback marker
...
# environment CodexCrew already sanitised, which is what makes the passthrough
# below safe for callers that use ``wrap_argv`` directly rather than
# ``sandboxed_spawn_argv``.
_IN_SANDBOX_MARKER = "CODEXCREW_SANDBOX_ACTIVE"
...
# scrub / ``-u`` flags). The marker alone proves "a Codex Crew sandbox is active"
...
# as ``unknown``.
_IN_SANDBOX_LEVEL_VAR = "CODEXCREW_SANDBOX_LEVEL"
...
def _bundled_cli_invocation() -> str | None:
    ...
    ``codexcrew`` on their PATH at all — the CLI is bundled INSIDE the AppImage.
    ...
    ``shutil.which("codexcrew")`` is deliberately NOT trusted as evidence here:
    ...
    if not name.startswith("codexcrew") or not os.path.isfile(resolved):
        return None
...
def _apparmor_userns_restricted() -> bool:
    ...
    :mod:`codex_crew.service.apparmor`: ``sandbox`` is a low-level dependency of
...
def _no_backend_guidance() -> str:
    ...
    The remedy differs by HOW Codex Crew was launched, so it is named per shape:

    * AppImage / desktop app — nothing applies a profile to a directly launched
      binary, so attach one to it (``codexcrew sandbox install-profile``).
    * anything else on such a host — the profile must be applied by systemd
      (``codexcrew service install``), because the only executable in a foreground
    ...
    optout = (
        ...
        "~/.codex-crew/config.json allows unsandboxed execution, but that removes "
    ...
    if sys.platform.startswith("linux") and _apparmor_userns_restricted():
        ...
        if appimage:
            # Name the CLI by ABSOLUTE PATH, not as `codexcrew`. An AppImage user
            # has no codexcrew on PATH (see _bundled_cli_invocation), so the bare
            ...
            cli = _bundled_cli_invocation() or "codexcrew"
            where = (
                " (that path is inside the running app, so run it while Codex Crew " "is open)"
                if cli != "codexcrew"
                else ""
            )
            ...
            # AppImage named `Codex-Crew-$(...).AppImage` would otherwise have its
        ...
        return (
            base
            + (
                "Run `codexcrew service install` to install the profile and have "
    ...
    return (
        ...
        "agent.sandbox_allow_unsandboxed_exec=true in "
        "~/.codex-crew/config.json to explicitly allow unsandboxed "
...
def unavailable_kind() -> str:
    ...
    isolation back to Codex Crew rather than disable it.
    """
...
def _inside_codexcrew_sandbox() -> bool:
    """True when this process already runs inside a CodexCrew OS sandbox.
    ...
    ``CODEXCREW_SANDBOX_ACTIVE`` marker (see ``_IN_SANDBOX_MARKER``).
    """
...
def _macos_sandbox_state() -> bool | None:
    ...
    ``CODEXCREW_SANDBOX_ACTIVE`` marker proves the marker was forged or inherited
...
def _inside_macos_sandbox() -> bool:
    ...
    CodexCrew did NOT create — notably kiro-cli >= 2.13's own internal seatbelt
    ...
    sandbox is active, not that CodexCrew built it or scrubbed the environment.
...
def agent_confinement_evidence() -> str | None:
    ...
    the direction of refusal*. A present ``CODEXCREW_SANDBOX_ACTIVE`` marker in
    ...
    of "Seatbelt-confined" for a non-CodexCrew sandbox (kiro-cli's internal
...
def _warn_no_isolation(mode: str) -> None:
    ...
    logger.warning(
        ...
        "agent.sandbox_allow_no_isolation=true in ~/.codex-crew/config.json to "
...
def _command_log_label(argv: list[str]) -> str:
    ...
    if name in {"node", "npm", "npx"}:
        return "node"
    if name in {"kiro", "kiro-cli", "codexcrew"}:
        return "kiro"
...
def _warn_first_party_unconfined_once(argv: list[str]) -> None:
    ...
    logger.warning(
        ...
        "first-party fixed-argv Codex Crew helper UNCONFINED (its full command "
...
def _first_party_no_backend_passthrough(
    ...
    try:
        from codex_crew.sel import sel  # circular import: sandbox is low-level
...
class SandboxUnavailableError(RuntimeError):
    ...
    ``"foreign_sandbox"`` (an outer Seatbelt sandbox CodexCrew did not create
...
def credential_mask_applies(mode: str) -> bool:
    ...
    if effective == "off":
        return False
    # Already inside a Codex Crew sandbox: a nested re-wrap is impossible by design,
    ...
    if _inside_codexcrew_sandbox() and _macos_sandbox_state() is not False:
        return False
...
def _governance_sandbox_floor() -> str | None:
    ...
    """
    from codex_crew.platform.context import PlatformCompositionError

    try:
        from codex_crew.platform.governance_profiles import governance_floor_ordinal
...
def _floor_mandates_sandbox(floor: str | None) -> bool:
    ...
    if not floor:
        return False
    from codex_crew.platform.governance import _ORDINAL_SCALES
...
def _clamp_sandbox_mode_to_floor(mode: str, floor: str | None) -> str:
    ...
    """
    from codex_crew.platform.governance import _ORDINAL_SCALES, OrdinalControl
...
def wrap_argv(
    ...
    *,
    ...
    Args:
        ...
        native_tool_gate: True only when the caller injected a fail-closed
            PreToolUse hook into the native agent runtime. On Windows, where
            Codex Crew has no OS sandbox backend, this permits the adapter spawn
            unless governance requires an OS sandbox. It does not bypass a
            governance floor and cannot carry Crew-specific path masks.
    ...
    if (
        sys.platform == "win32"
        and native_tool_gate
        and not _floor_mandates_sandbox(governance_floor)
    ):
        if extra_hidden_dirs or extra_visible_dirs or extra_writable_dirs:
            raise RuntimeError("A native tool gate cannot enforce Codex Crew sandbox path masks")
        logger.info(
            "wrap_argv: Windows agent security delegated to a verified native " "PreToolUse gate"
        )
        return list(argv), None
    ...
    if mode == "off":
        ...
        # Codex Crew's seatbelt is off, kiro-cli's internal sandbox is ON —
    ...
    # Already inside a CodexCrew sandbox (script cron, sandboxed agent child, app
    ...
    # other's blind spot: the marker proves CodexCrew built the outer sandbox and
    ...
    if _inside_codexcrew_sandbox() and _macos_sandbox_state() is not False:
        if not getattr(wrap_argv, "_nested_passthrough_logged", False):
            ...
            logger.info(
                "wrap_argv: already inside a CodexCrew sandbox — nested OS "
        ...
        # to CodexCrew's own seatbelt, an equally-safe audited layer — a nested
    ...
    # initialize nested inside CodexCrew's seatbelt (kernel EPERM even under an
    ...
    # CodexCrew's sandbox stays on for everything else and whenever kiro's is off.
    # Windows has no Codex Crew OS sandbox backend. Official Kiro ACP spawns are
    ...
    if delegate_to_kiro:
        if extra_hidden_dirs or extra_visible_dirs or extra_writable_dirs:
            # A delegated sandbox cannot enforce CodexCrew-specific path hides.
            ...
            if sys.platform == "darwin":
                ...
                # refused, Codex Crew's own seatbelt remains the safe owner.
    ...
    if backend == "none":
        ...
        # outer sandbox is NOT one CodexCrew built: a CodexCrew-built one carries
        # CODEXCREW_SANDBOX_ACTIVE and was already passed through above. The
        ...
        if floor_mandates_sandbox or not opted_in:
            ...
            if transient:
                ...
                # config-level fix that hands isolation back to CodexCrew's own
                ...
                guidance = (
                    ...
                    "CodexCrew did not create, and Seatbelt cannot nest, so "
                    "sandbox-exec fails with EPERM. Spawns under CodexCrew's OWN "
                    ...
                    "~/.kiro/settings/amazon-internal.json so CodexCrew's own "
                ...
                guidance = (
                    ...
                    "  (a) Use the Codex Crew custom seccomp profile (adds "
                    ...
                    "seccomp=docker/seccomp/codexcrew-seccomp.json ...\n"
                    "        # Without a checkout (image-only):\n"
                    "        curl -fsSL https://raw.githubusercontent.com/"
                    "mikutellyourworld/Codex-Crew/main/docker/seccomp/codexcrew-seccomp.json"
                    " -o codexcrew-seccomp.json\n"
                    "        docker run --security-opt seccomp=codexcrew-seccomp.json ...\n"
                    "  (b) Restart with explicit unsandboxed consent "
                    "(the container is then the only isolation boundary):\n"
                    "        docker run -e CODEXCREW_ALLOW_UNSANDBOXED=1 ...\n"
                    "  (c) Manually set agent.sandbox_allow_unsandboxed_exec=true "
                    "in ~/.codex-crew/config.json inside the container.\n"
            ...
            if policy_overrode_opt_in:
                guidance = (
                    ...
                    "also withholds the first-party carve-out, so Codex Crew's own "
...
def sandboxed_spawn_argv(
    ...
    *,
    ...
    agent-influenced spawn in ``src/codex_crew`` to route through.
    ...
    Args:
        ...
        strip_python_env: Strip ``PYTHONPATH``/``PYTHONHOME`` so a foreign
            Python child does not inherit CodexCrew's interpreter paths. Applied
    ...
    # as CodexCrew-spawned even when its cmdline carries no CodexCrew fingerprint
    # (e.g. ``npx @playwright/mcp``).
    scrubbed[CODEXCREW_SPAWNED_ENV] = CODEXCREW_SPAWNED_VALUE
...
async def shielded_prepare_off_loop(
    ...
    *,
    ...
    :mod:`codex_crew.executors` partitions blocking work into named pools so such
...
# codexcrew.slice/codexcrew-agents.slice inside the user manager). It is also
# the aggregate enforcement boundary — see ensure_agents_slice_limits().
_CGROUP_AGENTS_SLICE = "codexcrew-agents.slice"
...
def _agents_slice_name() -> str:
    ...
    several gateways at once — ``codexcrew pod up`` creates one per pod by
...
def _cgroup_limits_from_config() -> tuple[int, int, int, int]:
    ...
    try:
        ...
        # config/security consumers — importing codex_crew.config.loader at
        # module load would create an import cycle, so it stays function-level
        # (same pattern as resource_limit_preexec below).
        from codex_crew.config.loader import ResourceLimitsConfig, _raw_config
...
# ── aggregate agent-slice soft ceiling (memory.high on codexcrew-agents.slice) ──
...
def _slice_memory_events_high() -> int | None:
    ...
    understands systemd's dash-hierarchy (``codexcrew-agents.slice`` nests
    under ``codexcrew.slice`` in the user manager's subtree). ``None`` when it
...
def _agents_slice_cgroup_dir() -> Path | None:
    ...
    systemd's dash-hierarchy places ``codexcrew-agents.slice`` under
    ``codexcrew.slice`` inside the user manager's subtree; the direct
    ...
    direct = base / "codexcrew.slice" / _CGROUP_AGENTS_SLICE
    if direct.is_dir():
        return direct
    try:
...
def resource_limit_preexec() -> "Callable[[], None] | None":
    ...
    Returns the callable from :func:`codex_crew.security.apply_resource_limits`,
    ...
    if _RESOURCE_PREEXEC is _UNSET:
        ...
        from codex_crew.security import apply_resource_limits
        ...
        try:
            ...
            from codex_crew.config.loader import _raw_config
...
def resource_limit_supervisor_argv() -> "tuple[str, ...]":
    ...
    if _RESOURCE_SUPERVISOR_ARGV is _UNSET:
        if os.name != "posix":
            ...
            return ()
        from codex_crew.security import resource_limit_spec
...
def build_resource_limit_preexec() -> "Callable[[], None] | None":
    ...
    if _BUILD_RESOURCE_PREEXEC is _UNSET:
        ...
        from codex_crew.config.loader import ResourceLimitsConfig
...
def _pinned_spawn_path(
    ...
    * **Identity** (when *chdir_fd* is given) -- an absolute entry that IS the
      ...
      ``PATH=/home/me/.codex-crew/workspace/bin:/usr/bin`` passes the lexical
    ...
    if bound_identity is not None:
        ...
        from codex_crew.hooks import _fd_real_path
