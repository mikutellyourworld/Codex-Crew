"""App registry — curated list of available CodexCrew apps.
...
own ``app.json`` — they never need to update the CodexCrew registry JSON
...
from codex_crew.apps import install_receipt, official_catalog
from codex_crew.apps.admission import app_admission_denied, verified_signer
...
from codex_crew.apps.manager import list_apps as list_installed_apps
from codex_crew.apps.manager import (
    registry_source_repository,
    set_app_provenance,
    update_app,
)
from codex_crew.apps.manifest import (
...
try:
    from codex_crew.sel import sel as _sel_fn
except ImportError:
...
from codex_crew.platform import PlatformCompositionError, current_context
...
# :func:`codex_crew.service.common` for the same reason). This is a
...
def _pinned_registries() -> list[Any]:
    ...
    from codex_crew.config.loader import ExternalRegistryConfig
...
def _effective_registries() -> list[Any]:
    ...
    from codex_crew.config.loader import (
        CodexCrewConfig,  # circular import: loader.py imports from apps/ at module level; deferring avoids ImportError
    )

    try:
        configured = list(CodexCrewConfig.load().registries or [])
...
def _edition_registry_rows() -> list[dict[str, Any]]:
    ...
    from codex_crew.platform.context import safe_context_call
...
async def _fetch_app_manifest(
    ...
    *,
    ...
    Tries, in order:
      1. The persistent clone under ``~/.codex-crew/app-sources/{app_name}/``
    ...
    *commit*: a pinned commit. When set, the manifest is read from THAT tree
    ...
    # is_clone_host_trusted() loads config from disk (CodexCrewConfig.load), so
    ...
    try:
        tmp_root = await asyncio.to_thread(tempfile.mkdtemp, prefix="codexcrew-manifest-")
...
def _enrich_with_install_status(
    ...
    *detected* is a set of app names that were found via ``detectInstalled``
    shell commands (installed outside CodexCrew's app manager).
    """
...
# published catalog both state ``Codex Crew``; the historical bundled spelling
# was the single token ``codexcrew``. Both are us, so both mint the mark.
FIRST_PARTY_AUTHORS: frozenset[str] = frozenset(
    {"codexcrew", "codex crew"}  # brand-ok: folded values, lower-cased by contract
)
...
def _apply_trust_fields(
    ...
    *,
    ...
    - ``verified``: ``True`` only when provenance is NOT ``"external"`` AND
      ...
      repo publishing ``"author": "codexcrew"`` in its manifest does not
...
def _read_external_registry_cache(
    ...
    *,
    ...
    try:
        ...
        from codex_crew.apps.manifest import KEBAB_RE
...
async def _fetch_external_registry_index(
    ...
    *repo* is a git-cloneable URL (https/ssh/git/scp-style).  The repo is
    ...
    tmp_root: str | None = None
    try:
        tmp_root = await asyncio.to_thread(tempfile.mkdtemp, prefix="codexcrew-registry-")
...
async def list_registry() -> list[dict[str, Any]]:
    ...
    5. Enrich with install status from CodexCrew's app manager
    ...
    # the app author's — a repo publishing ``"author": "codexcrew"`` in its
...
def app_source_dir(name: str) -> Path:
    """Return ~/.codex-crew/app-sources/{name}/ — persistent clone directory."""
...
async def _refuse_identity_mismatch(
    ...
    *,
    ...
    been written under ``~/.codex-crew/apps/`` at this point, so removing the
...
def _read_git_metadata_bounded(path: Path, limit: int) -> str | None:
    ...
    :func:`codex_crew.hooks.safe_read_prefix`, which canonicalizes via ``realpath``
...
async def _clone_build_app(
    ...
    *,
    ...
    Source is cloned to ``~/.codex-crew/app-sources/{app_name}/`` (persistent;
...
async def _unpoison_rejected_checkout(
    ...
    *manifest_relpath* is the untrusted registry-declared manifest path the
    ...
    then runs unsandboxed as the Codex Crew process, so it must not trust that
...
async def _run_app_build(
    ...
    if (build_dir / "package.json").is_file():
        ...
        # PATH, which is routinely a different interpreter: `bin/codexcrew` execs
        # `.venv/bin/codexcrew` WITHOUT putting the venv's `bin/` on PATH, and
...
async def _retained_startup_refusal(
    ...
    from codex_crew.apps.hooks_integration import stop_retained_startup_hooks
...
async def install_from_registry(
    ...
    Source code is cloned to ``~/.codex-crew/app-sources/{name}/`` (persistent,
    survives reboots, used by app update scripts).

    For self-managed apps (``managed: "self"`` in registry), only the clone +
    install script is run — CodexCrew does NOT copy files to ``~/.codex-crew/apps/``
    or register resources via bridges.  The app registers itself at runtime.

    For codexcrew-managed apps, files are copied to ``~/.codex-crew/apps/{name}/``
    ...
    2. Clone the repo to ~/.codex-crew/app-sources/{name}/ (timeout: 60s)
    3. Build it (npm/pip, auto-detected) then run the install script from
       app.json if any (timeout: 300s)
    4. For codexcrew-managed: call install_app() or update_app()
    ...
    # Bundled (curated, CodexCrew-shipped) entries have no ``_registry`` marker
    ...
    # the bundled (CodexCrew-shipped) catalog, which is itself a distinct source.
    ...
    # CodexCrew is running on an incompatible platform, return client install
    ...
    # Validate minCodexCrewVersion if declared
    min_version = (manifest or {}).get("minCodexCrewVersion", "")
    if min_version:
        from codex_crew.apps.version import check_min_version
        ...
        try:
            ...
            if proc.returncode == 0:
                return {
                    ...
                    f"Launch it to register with Codex Crew automatically.",
                }
        except (asyncio.TimeoutError, OSError):
            pass
    ...
    # the codexcrew-managed path below already uses `result` for the
    ...
    try:
        ...
        if deps_data and isinstance(deps_data, dict):
            from codex_crew.apps.dependencies import resolve_dependencies as _resolve_deps
            from codex_crew.apps.manifest import Dependencies as _Deps
        ...
        # Step 4: Register with CodexCrew
        if is_self_managed:
        ...
        # Kirocrew-managed: copy to ~/.codex-crew/apps/ and register resources
