"""App Manager — install, uninstall, enable, disable lifecycle for CodexCrew apps.

Apps are installed to ``~/.codex-crew/apps/{name}/``.  Each installed app has an
...
from codex_crew.pinned_fs import supports_pinned_walk
...
def apps_dir() -> Path:
    """Return the root directory for installed apps: ``~/.codex-crew/apps/``."""
...
def app_data_dir(name: str) -> Path:
    """Return the app-scoped data directory: ``~/.codex-crew/apps/{name}/data/``."""
    d = app_dir(name) / "data"
...
class InstalledApp:
    ...
    ``origin`` — where the app came from (read-only, set at install time):
      - ``"builtin"``: baked into the CodexCrew dashboard
    ...
    ``resources`` — who manages agent/skill/cron registration:
      - ``"gateway"``: CodexCrew manages via bridges.py symlinks
      - ``"app"``: the app manages its own resource registration

    ``lifecycle`` — who manages updates and uninstall:
      - ``"gateway"``: CodexCrew handles updates and uninstall
...
def _credential_free_source_metadata(value: str) -> str:
    ...
    from codex_crew.apps.registry import _strip_git_target_userinfo
...
def _validate_source_path(source: Path) -> list[str]:
    ...
    if manifest.minCodexCrewVersion:
        ver_err = _check_min_version(manifest.minCodexCrewVersion)
        if ver_err:
...
def _check_min_version(min_version: str) -> str | None:
    """Return error string if current CodexCrew version is too old, else None."""
...
_REGISTRY_SOURCE_REPOSITORY: ContextVar[str | None] = ContextVar(
    "codexcrew_registry_source_repository", default=None
)
...
def install_app(
    ...
    *,
    ...
    2. Copy to ``~/.codex-crew/apps/{name}/``
    ...
    from codex_crew.dashboard.token_auth import generate_app_secret, write_app_secret
...
def uninstall_app(name: str, *, keep_data: bool = True) -> AppResult:
    ...
    # because this runs under `codexcrew app uninstall` in a DIFFERENT process:
    #
    ...
    try:
        from codex_crew.apps.dev_mode import remove_dev_app
...
def _drop_trust_grant(name: str) -> None:
    ...
    endpoints do: ``CodexCrewConfig.load()`` degrades a corrupt file to defaults, so
    ...
    #
    # `CodexCrewConfig.load()` deep-merges `config.local.json` OVER `config.json`,
    ...
    # base branch has two dozen writers in it, `codexcrew config set` among them, so
    ...
    # endpoint emits its own SEL event, but this path runs from `codexcrew app
...
def register_external_app(
    ...
    *,
    ...
    """Register a self-managed app with CodexCrew's app system.

    Self-managed apps (``resources="app"``) handle their own agent/skill/MCP
    registration.  CodexCrew only tracks metadata so the dashboard can display them.
    ...
    if origin == "builtin":
        ...
        return AppResult(
            ok=False,
            name=name,
            error="builtin origin is reserved for CodexCrew-shipped apps",
        )
    ...
    if existing and _builtin_owns_install(existing):
        ...
        return AppResult(
            ...
            name=name,
            error=(
                f"{name!r} is a CodexCrew-shipped builtin and cannot be replaced "
...
# Built-in apps are features baked into the CodexCrew dashboard that we
...
_DEFAULT_ON_BUILTINS: frozenset[str] = frozenset(
    {
        ...
        "drive-sentinel",  # Measure-first storage safety is part of the base product.
...
# routes stay fully intact and it is enabled with ``codexcrew app enable channels``.
...
def resolve_mcp_backend_url(mcp_servers: Any) -> str | None:
    ...
    if not isinstance(mcp_servers, dict):
        return None
    gateway_port = int(os.environ.get("CODEXCREW_PORT", "5486"))
    ...
    for app_data in all_builtins:
        ...
        if _app_declares_backend(app_data):
            ...
            try:
                ...
                # codex_crew.apps.routes imports from codex_crew.apps.manager
                # at module load, so we cannot import routes at the top of
                # this file without creating a cycle.
                from codex_crew.apps.routes import invalidate_app_secret_cache
