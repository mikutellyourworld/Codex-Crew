...
with CodexCrew's existing file locking patterns.  Read-modify-write cycles
hold a single exclusive lock across the entire operation to prevent lost
updates.

Storage: ``~/.codex-crew/dependency-ledger.json``
"""
...
#: CodexCrew can neither install nor clean them up. Their ledger rows must SURVIVE
...
def declared_capability_keys(deps_data: dict[str, Any]) -> list[str]:
    ...
    alias (see :class:`codex_crew.apps.manifest.Dependencies`).  Shared by the
        ...
        for dep in result["removable"]:
            ...
            if dep["id"] in keep or uncleanable:
                ...
                if entry:
                    ...
                    # was installed at all (CodexCrew has no operation to remove
