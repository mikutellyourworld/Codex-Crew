...
(:func:`codex_crew.apps.hooks_integration.on_gateway_startup`) and on the
dashboard enable/disable HTTP handlers — and thereafter serves whatever module
object is cached in ``sys.modules`` under the app's ``_codexcrew_app_<name>.*``
namespace.

The CLI ``codexcrew app {enable,disable,install,uninstall}`` subcommands are a
...
``codexcrew restart`` picked up the change, and nothing said so.
...
reconciler. This module is it, modelled on :mod:`codex_crew.apps.dev_mode`'s
...
:mod:`codex_crew.apps.hooks_integration` (``record_loaded_hook_signature`` /
...
from codex_crew.apps.lifecycle import app_has_retained_startup, apps_with_retained_startup
from codex_crew.apps.manager import app_enabled_state, app_lifecycle_lock, get_app, list_apps
from codex_crew.apps.module_loader import unload_app_modules
from codex_crew.apps.teardown import forget_app_hooks
