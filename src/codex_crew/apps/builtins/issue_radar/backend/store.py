...
Everything lives under ``~/.codex-crew/apps/issue-radar/data/`` (via
``codex_crew.apps.manager.app_data_dir``, the platform-standard app-scoped data
dir). Nothing is stored on a CodexCrew-hosted backend and no GitHub App/PAT is
...
#   notify_on_new_issue      push a CodexCrew notification when a new issue opens
#
...
def issue_ai_cache_path(
    ...
    :func:`codex_crew.context.normalize_ui_language_tag`, which admits nothing
...
#
# Clicking "Investigate" on an issue opens a CodexCrew chat session, seeds it
