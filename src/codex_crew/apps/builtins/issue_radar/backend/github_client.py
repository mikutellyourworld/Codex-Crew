...
No GitHub App, no PAT, no CodexCrew-hosted credential storage: every call
...
def _gh_bin() -> str:
    ...
    ``CODEXCREW_ISSUE_RADAR_GH`` to an absolute path to override (still
    validated), or ``CODEXCREW_PROVIDER_BIN_STRICT=1`` to require a root-owned
...
# for 50 open PRs on mikutellyourworld/Codex-Crew), so the enrichment is one extra call
...
# simply are not in the window (mikutellyourworld/Codex-Crew has 363 closed PRs, so a PR
...
# Codex Crew's own store: the comment is the authority, so the claim survives a
...
#
#   <!-- codexcrew-crew id=c_7f3a phase=implementing pr=2271 updated=2026-08-08T20:44:12Z -->
#
...
# ``<!-- codexcrew-crew-brief v1 -->`` from matching: the next character there is a
