...
:data:`POLL_INTERVAL_SEC` seconds it makes two passes over the connected repos:
  ...
  1. **new-item notification** — each repo that has opted in (per-repo setting
     ...
     Azure DevOps work items) created since the last check, and a Codex Crew
...
runs while CodexCrew is running (and the machine is awake). It is also
...
from codex_crew.apps.builtins.issue_radar.backend import crew_runtime, provider, store
...
async def _poll_once(app: web.Application) -> None:
    ...
    * the crew sweep, which runs for every connected repo that has a live crew.
    ...
    # disables nothing can tell us about: ``codexcrew app disable`` in another
