...
OAuth app, no PAT stored by Codex Crew, no hosted backend. Every call shells out
...
def _az_bin() -> str:
    ...
    Set ``CODEXCREW_ISSUE_RADAR_AZ`` to an absolute path to override (still
    validated), or ``CODEXCREW_PROVIDER_BIN_STRICT=1`` to require a system-dir
    ...
    if sys.platform == "win32":
        raise ProviderCliError(
            ...
            "Windows is not supported -- use WSL to run the Codex Crew gateway. "
    ...
    override = os.environ.get("CODEXCREW_ISSUE_RADAR_AZ")
    if override:
        try:
            ...
            raise ProviderSetupError(
                f"CODEXCREW_ISSUE_RADAR_AZ={override!r} failed validation: {exc}",
    ...
    raise ProviderSetupError(
        ...
        "CODEXCREW_ISSUE_RADAR_AZ to an absolute az path",
        reason="not_installed",
    )
