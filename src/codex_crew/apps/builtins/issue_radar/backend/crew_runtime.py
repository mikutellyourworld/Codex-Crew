...
from codex_crew.apps.teardown import register_slot_close_hook, register_slot_close_undo_hook
...
BRIEF_SENTINEL = "<!-- codexcrew-crew-brief v1 -->"
...
def brief_is_present(slot: Any) -> bool:
    ...
    ``<!-- codexcrew-crew-brief v1 -->`` and a work list…"), and a sentinel-only
...
async def ensure_crew_session(state: Any, owner: str, repo: str, crew: dict[str, Any]) -> Any:
    ...
    slot = state.get_or_create_slot(
        name=slot_key,
        agent=str(crew.get("agent") or "codexcrew"),
...
async def suspend_crews(state: Any) -> int:
    ...
    * ``watch._poll_once``'s disabled branch — the BACKSTOP, for the disables this
      module can never be told about: ``codexcrew app disable`` in another process,
