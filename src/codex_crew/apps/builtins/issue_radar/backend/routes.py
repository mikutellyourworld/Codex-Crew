...
from codex_crew.apps.builtins.issue_radar.backend import (
    github_client,
    pipeline_routes,
    provider,
    store,
...
from codex_crew.context import normalize_ui_language_tag, ui_language_tag
from codex_crew.loop_lock import LoopBoundLock
from codex_crew.sel import sel

logger = logging.getLogger("codexcrew.app.issue-radar")
...
async def _handle_connect(request: web.Request) -> web.Response:
    ...
    try:
        ...
        # ``dashboard.gitlab_hosts`` allowlist, and ``CodexCrewConfig.load()`` is
...
def _ui_language() -> str:
    ...
    unshipped stored value — see :func:`codex_crew.context.ui_language_tag`. The
    ...
    **Call this OFF the event loop** (``asyncio.to_thread``): ``CodexCrewConfig
    ...
    """
    try:
        return ui_language_tag(CodexCrewConfig.load())
    except Exception:
...
def _hint_language(raw: object) -> str:
    ...
    (:func:`codex_crew.context.normalize_ui_language_tag`) — this value arrives
...
async def _run_oneshot_model(request: web.Request, key: str, prompt: str) -> str:
    ...
    ``codexcrew-lite`` background agent — the same lever workflows / title-gen /
    ...
    so no kiro-cli subprocess leaks. It reuses the user's own CodexCrew backend, so
    there is no separate API key or cloud account (the app's whole premise).
    """
    from codex_crew.llm_helpers import ToolApprovalPolicy, stream_and_collect

    state = request.app.get("state")
    if state is None:
        raise RuntimeError("session manager unavailable")

    provider, _is_new, _resumed = await state.sessions.get_or_create(key, agent="codexcrew-lite")
    try:
...
async def _compute_issue_ai(
    ...
    *,
    ...
    from codex_crew.llm_helpers import parse_llm_json
...
#
# "Investigate" opens a CodexCrew chat session (seeded with an investigation
...
async def _compute_label_recommendations(
    ...
    *,
    ...
    Mirrors :func:`_compute_issue_ai` exactly (``codexcrew-lite`` background agent,
    ...
    between them."""
    from codex_crew.llm_helpers import ToolApprovalPolicy, parse_llm_json, stream_and_collect
    from codex_crew.security import redact
...
def register_routes(app: web.Application) -> None:
    ...
    # job) that polls opted-in repos every ~60s and pushes a CodexCrew
