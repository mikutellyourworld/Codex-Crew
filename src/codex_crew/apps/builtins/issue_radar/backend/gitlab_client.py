...
GitLab App, no PAT stored by CodexCrew, no hosted backend. Every call shells out
...
def _glab_bin() -> str:
    ...
    same binary is untrusted. Set ``CODEXCREW_ISSUE_RADAR_GLAB`` to an absolute
    path to override (still validated), or ``CODEXCREW_PROVIDER_BIN_STRICT=1`` to
    ...
    override = os.environ.get("CODEXCREW_ISSUE_RADAR_GLAB")
    if override:
        try:
            ...
            raise ProviderSetupError(
                f"CODEXCREW_ISSUE_RADAR_GLAB={override!r} failed validation: {exc}",
    ...
    raise ProviderSetupError(
        ...
        "manager) and run `glab auth login`, or set CODEXCREW_ISSUE_RADAR_GLAB to "
...
def allowed_hosts() -> frozenset[str]:
    ...
    """
    try:
        return frozenset(CodexCrewConfig.load().dashboard.gitlab_hosts)
