...
**Nothing has to be installed by hand.** ``pip install codexcrew`` is the only
...
``~/.codex-crew/apps/<name>/`` — it never stages a builtin's other files there. A
...
from codex_crew.apps.builtins.pptx_maker.backend import engine_source, paths
from codex_crew.apps.manager import app_dir
from codex_crew.atomic_write import atomic_write
from codex_crew.sandbox import cgroup_scope_argv, run_limited, sandboxed_spawn_argv

logger = logging.getLogger("codexcrew.app.pptx-maker")
...
def resolve_uv() -> str | None:
    ...
    ``pip install codexcrew`` always has the binary — but not necessarily on
...
def mcp_tools_path() -> str:
    ...
    from codex_crew.apps.builtins.pptx_maker.backend import paths as _paths
...
def _run(argv: list[str], *, cwd: str, timeout: int) -> tuple[int, str]:
    ...
    Uses the :func:`codex_crew.sandbox.sandboxed_spawn_argv` chokepoint so the
...
def _render_agents(install_dir: Path, log: list[str]) -> int:
    ...
    which is itself overridable via ``CODEXCREW_HOME``).
...
def _stage_static(install_dir: Path, log: list[str]) -> None:
    ...
    ``src/codex_crew/builtin_skills/pptx-maker/``, which the gateway copies into
...
def provision() -> ProvisionOutcome:
    ...
    if uv_bin is None:
        log.append(
            "`uv` could not be found. It ships with Codex Crew as a Python "
    ...
    try:
        ...
        from codex_crew.apps.builtins.pptx_maker.backend import preview_tools
    ...
    try:
        ...
        from codex_crew.apps.builtins.pptx_maker.backend import engine, library
