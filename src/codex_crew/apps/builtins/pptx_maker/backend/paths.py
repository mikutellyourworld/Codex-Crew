...
* ``DECK_ROOT`` — where the presentation engine writes decks. Resolved from
  (in order) ``CODEXCREW_PPTX_DECK_ROOT`` (dev/test override), the engine's own
...
DECK_ROOT_ENV = "CODEXCREW_PPTX_DECK_ROOT"
...
def app_root() -> Path:
    """The app's writable data dir (``~/.codex-crew/apps/pptx-maker/data``).
