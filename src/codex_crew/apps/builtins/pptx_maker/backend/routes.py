...
from codex_crew.apps.builtins.pptx_maker.backend import (
    decks,
    engine,
    library,
    paths,
...
from codex_crew.credential_patterns import AWS_KEY_ID
...
# (:mod:`codex_crew.messaging.raster`). Verifying the first few bytes is enough
...
# `CODEXCREW-BITMAP-…` in the artifact text restores to nothing (its index is out
...
_BITMAP_PLACEHOLDER_RE = re.compile(
    r"\x00CODEXCREW-BITMAP-" + _BITMAP_PLACEHOLDER_NONCE + r"-(\d+)\x00"
)
...
async def off_loop(fn: Callable[..., _T], *args: Any, **kwargs: Any) -> _T:
    ...
    DNS — saturating it stalls the gateway (see ``codex_crew.executors``).
    """
...
def _deps_status() -> dict:
    ...
    which is what lets the UI say "installed by CodexCrew" rather than implying
    the host changed.
...
def _redact_artifact(raw: bytes) -> bytes:
    ...
    def _stash(match: re.Match[str]) -> str:
        ...
        return f"\x00CODEXCREW-BITMAP-{_BITMAP_PLACEHOLDER_NONCE}-{len(stash) - 1}\x00"
