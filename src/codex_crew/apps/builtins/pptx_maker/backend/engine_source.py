...
**No system prerequisite.** ``pip install codexcrew`` must leave nothing for the
...
Follows :mod:`codex_crew.embeddings` and
:mod:`codex_crew.apps.builtins.papyrus.backend.tectonic`, the two in-tree
...
#: accepted.
ENGINE_URL_ENV = "CODEXCREW_PPTX_ENGINE_URL"

#: Escape hatch for tests and CI: a test run must never reach the network.
SKIP_DOWNLOAD_ENV = "CODEXCREW_PPTX_SKIP_ENGINE_DOWNLOAD"
...
SOURCE_MARKER_FILENAME = ".codexcrew-engine.json"
...
def _reject_member_name(name: str) -> str | None:
    ...
    * an empty name.
    ...
    Same rules as
    :func:`codex_crew.apps.builtins.papyrus.backend.tectonic._reject_member_name`.
    """
    if not name:
