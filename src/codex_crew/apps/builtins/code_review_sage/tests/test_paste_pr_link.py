...
class TestPullRequestRef(unittest.TestCase):
    def test_parses_a_pull_request_url(self):
        ref = routes._pull_request_ref(
            "https://github.com/mikutellyourworld/Codex-Crew/pull/777")
        assert ref is not None
        self.assertEqual(ref["owner"], "kirodotdev")
        self.assertEqual(ref["repo"], "CodexCrew")  # brand-ok: repo name
        ...
        self.assertEqual(ref["change_id"], "GH-kirodotdev-CodexCrew-777")

    def test_a_repo_url_is_not_a_pull_request(self):
        self.assertIsNone(
            routes._pull_request_ref("https://github.com/mikutellyourworld/Codex-Crew"))

    def test_a_bare_slug_is_not_a_pull_request(self):
        self.assertIsNone(routes._pull_request_ref("mikutellyourworld/Codex-Crew"))
    ...
    def test_extra_path_segments_do_not_confuse_it(self):
        ref = routes._pull_request_ref(
            "https://github.com/mikutellyourworld/Codex-Crew/pull/777/files#r123")
        assert ref is not None
        self.assertEqual(ref["number"], 777)
...
class TestRepoEndpointWithPullRequest(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self._old = os.environ.get("CODEXCREW_HOME")
    ...
    def tearDown(self):
        if self._old is None:
            ...
            os.environ["CODEXCREW_HOME"] = self._old
    ...
    async def test_pins_the_repo_and_reports_the_pull_request(self):
        resp = await routes._handle_repos(_FakeRequest(  # type: ignore[arg-type]
            {"repo": "https://github.com/mikutellyourworld/Codex-Crew/pull/777"}))
        ...
        self.assertIn("mikutellyourworld/Codex-Crew", pinned)

    async def test_a_repo_url_reports_no_pull_request(self):
        resp = await routes._handle_repos(_FakeRequest(  # type: ignore[arg-type]
            {"repo": "https://github.com/mikutellyourworld/Codex-Crew"}))
        self.assertEqual(resp.status, 200)
    ...
    async def test_a_hostile_host_is_still_refused(self):
        resp = await routes._handle_repos(_FakeRequest(  # type: ignore[arg-type]
            {"repo": "https://evil.test/mikutellyourworld/Codex-Crew/pull/777"}))
