...
A self-evolving deep code reviewer packaged as a built-in CodexCrew app. Reviews
...
dashboard's fallback for a failed resume is to replay Codex Crew's own conversation
...
App-local runtime data lives under `~/.codex-crew/apps/code-review-sage/data/`
(honors `CODEXCREW_HOME`; created on first use, never committed).

## Enable

Code Review Sage is a built-in app (listed in `codex_crew.apps.builtins`). Enable
it from the dashboard Apps page, or:

```bash
codexcrew app enable code-review-sage
```
...
`~/.codex-crew/apps/code-review-sage/data/config.json` (the list replaces the
