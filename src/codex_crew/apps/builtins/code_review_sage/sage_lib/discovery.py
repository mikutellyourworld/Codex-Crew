...
other gh surface via ``codex_crew.github_runner`` (trusted-binary resolution +
...
# the standalone path, where `codex_crew` is not importable. A bare module-level
...
_GH_OVERRIDE_ENV = "CODEXCREW_SAGE_GH"
...
def gh_env() -> dict[str, str]:
    ...
    if github_runner is None:  # pragma: no cover - standalone fallback
        raise RuntimeError("gh_env requires the Codex Crew runtime")
...
def gh_bin() -> str:
    ...
    Set ``CODEXCREW_SAGE_GH`` to an absolute path to override (still validated).
    Raises :class:`GhSetupError` when no acceptable executable is found."""
    if github_runner is None:  # pragma: no cover - standalone fallback
        raise RuntimeError("gh_bin requires the Codex Crew runtime")
    try:
...
def _run_gh(argv: list[str], *, timeout: float) -> subprocess.CompletedProcess:
    ...
    if github_runner is None:  # pragma: no cover - standalone fallback
        raise RuntimeError("gh execution requires the Codex Crew runtime")
    try:
