...
def runtime_preflight() -> str:
    ...
    if AcpRuntime is None:
        return ("the reviewer cannot run: the ACP runtime "
                "(codex_crew.acp.runtime) is not importable in this install")
...
_FALLBACK_AGENT = "codexcrew"     # default agent when the reviewer agent isn't installed
...
try:
    from codex_crew.config.loader import DEFAULT_MODEL as _SYSTEM_DEFAULT_MODEL
...
# Valid concrete effort levels — sourced from codex_crew.effort (single source of
# truth), not a hardcoded list. "" (inherit default) is handled separately.
try:
    from codex_crew.effort import EFFORT_LEVELS as VALID_EFFORTS
...
def _resolve_review_agent(preferred: str = REVIEW_AGENT) -> str:
    ...
    `codexcrew` agent. GitHub posting runs the `gh` CLI, so the chosen agent needs
...
def _write_effort_overlay(work_dir: str, model: str, effort: str = REVIEW_EFFORT) -> None:
    ...
    ``codex_crew/providers/acp.py:_write_cli_overlay``)::
...
class _BatchRuntimeHolder:
    ...
    async def _ensure_runtime_locked(self) -> "AcpRuntime":
        ...
        if rt is not None:                          # reap a dead one first
            await self._kill(rt)
        if AcpRuntime is None:
            raise RuntimeError("AcpRuntime unavailable (codex_crew.acp.runtime not importable)")
