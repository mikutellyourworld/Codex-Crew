...
``~/.codex-crew/apps/code-review-sage/data/`` by default) and is safe to run on
...
# Canonical CodexCrew data-root accessor. Imported at module top but kept guarded
# so the store stays importable standalone (outside the CodexCrew runtime) — the
# fallback mirrors ``config_dir()``'s default of ``~/.codex-crew``, honoring
# ``CODEXCREW_HOME`` when it is set.
try:
    from codex_crew.config.paths import config_dir as _config_dir
...
try:
    from codex_crew.platform_compat import restrict_to_owner as _runtime_restrict
except ImportError:  # pragma: no cover - standalone fallback
...
def crew_home() -> Path:
    """Resolve the active CodexCrew data root.

    Delegates to ``codex_crew.config.paths.config_dir()`` when the runtime is
    importable (so it follows the ``~/.codex-crew`` root and honors
    ``CODEXCREW_HOME`` uniformly, including the one-time legacy-home migration).
    Falls back to a standalone resolution when run outside the CodexCrew package.
    """
...
def app_root() -> Path:
    """Resolve the installed app root under the CodexCrew home dir.

    Derives from ``crew_home()`` (``config_dir()`` → ``~/.codex-crew`` by
    default, honoring ``CODEXCREW_HOME``); ``crew_home`` keeps a standalone
    fallback so the store stays importable outside the CodexCrew runtime."""
...
def restrict_to_owner(path: str | os.PathLike) -> None:
    ...
    ``config_dir`` above: outside the Codex Crew runtime there is no stdlib way to
...
# Optional Codex Crew redaction. Lives here rather than in `pipeline` because readers
...
def redact_text(text: str) -> str:
    ...
    system it posts to, and the dashboard it renders in. No-op when the Codex Crew
