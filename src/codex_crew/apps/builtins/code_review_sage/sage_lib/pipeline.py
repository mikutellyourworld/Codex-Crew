...
# Optional CodexCrew redaction — used to scrub LLM-generated text before it is
# posted to an external surface. Imported at module top (absent only when run
# fully standalone outside the CodexCrew runtime).
try:
...
def resolve_rule_pack(pack_name: str) -> str | None:
    """Find a rule-pack SKILL.md by skill name under the CodexCrew skills dir
    (``<config_dir>/skills/`` — ``~/.codex-crew/skills/`` by default, honoring
    ``CODEXCREW_HOME`` — this is where the app framework symlinks app + user
