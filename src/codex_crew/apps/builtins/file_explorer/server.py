...
from codex_crew.hooks import safe_read_file_bytes
from codex_crew.platform import boot_platform
from codex_crew.sandbox import cgroup_scope_argv, run_limited, wrap_argv
...
APP_NAME = os.environ.get("CODEXCREW_APP_NAME", "file-explorer")
...
# CodexCrew data-home subdirectories that are safe for the file explorer to
...
def _is_safe_crew_subdir(name: str) -> bool:
    ...
    the same question.
    """
    return name in _CODEXCREW_SAFE_SUBDIRS


# The CodexCrew data home is ~/.codex-crew (nested under kiro-cli's ~/.kiro), with
# a legacy location at ~/.codexcrew. The granular deny-by-default listing policy
# must recognize the home wherever it lives: the current ~/.codex-crew, and the
# legacy ~/.codexcrew.
# Each marker is the tuple of trailing path segments that identify the home dir;
# the segment(s) AFTER the marker are matched against _CODEXCREW_SAFE_SUBDIRS.
...
# Windows) ``~/.KIRO/crew/config.json`` opens the SAME inode as ``~/.codex-crew``
...
def _crew_home_index(parts: tuple[str, ...]) -> int:
    ...
    ``"skills"``; for ``(..., ".codexcrew", "config.json")`` the index of
    ``"config.json"``. Returns -1 when no crew-home marker is present. The
    longest (most specific) marker wins so ``.codex-crew`` is preferred over any
    shorter match.
...
def _is_sensitive(p: Path) -> bool:
    ...
    For the CodexCrew data home (``~/.codex-crew``, plus the legacy variant), a
    granular policy applies: only subdirectories listed in
    ``_CODEXCREW_SAFE_SUBDIRS`` are accessible; everything else (keys, tokens,
    ...
    # subdir of the crew home (~/.codex-crew or the legacy variant).
    after = _crew_home_index(p.parts)
    if after != -1:
        # The home root itself is blocked (deny-by-default). Listing endpoints
        # use _codexcrew_safe_children() to enumerate only safe subdirs.
...
def _codexcrew_safe_children(codexcrew_dir: Path) -> list[dict]:
    """Return entry metadata for only the safe subdirs of a .codexcrew/ directory.

    This is the single enforcement point for deny-by-default .codexcrew listing:
    callers get back only entries that are in ``_CODEXCREW_SAFE_SUBDIRS``.
    ...
    codexcrew_dir = _contain_in_allowed_roots(codexcrew_dir, operation="tree_list")
    out: list[dict] = []
    try:
        children = sorted(codexcrew_dir.iterdir(), key=lambda x: (not x.is_dir(), x.name.lower()))
...
def _search_rg(root: Path, query: str, include: str, exclude: str) -> list[dict]:
    ...
    #    ~/.codex-crew and the legacy home.
    ...
    # separately from CodexCrew itself, which is why the same folder gets prompted
...
class FileExplorerHandler(BaseHTTPRequestHandler):
    server_version = "CodexCrew-FileExplorer/0.1"
    ...
    def _authorized_or_health(self, method: str) -> bool:
        """Verify the gateway's X-CodexCrew-Proxy HMAC before dispatch (CWE-306).
    ...
    def _dispatch(self, method: str) -> None:
        ...
        # CodexCrew's reverse proxy forwards requests as /api/<path>;
    ...
    def _h_tree(self, qs: dict[str, list[str]]) -> None:
        ...
        # read (``.exists()``/``.is_dir()``/``_codexcrew_safe_children``), so no
        ...
        if _is_crew_home_root(expanded):
            ...
            if resolved.exists() and resolved.is_dir():  # lgtm[py/path-injection]
                ...
                entries = _codexcrew_safe_children(resolved)
                return self._json(
