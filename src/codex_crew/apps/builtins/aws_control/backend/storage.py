...
Everything routes through :func:`codex_crew.deploy.engine.run_aws` — the AWS
...
* **Stateless-by-tag discovery.** The bucket carries an opaque generated name
  (``codexcrew-drive-<12hex>``) and is found by tags, requiring BOTH
  ``codexcrew:managed=true`` AND ``codexcrew:drive=default``, plus the naming
...
from codex_crew.deploy.engine import AWSError, _checked, _harden_bucket
...
_BUCKET_NAME_RE = re.compile(r"codexcrew-drive-[0-9a-f]{12}")
TAG_DRIVE = "codexcrew:drive"
...
def find_drive(profile: str, region: str, *, account: str) -> Optional[str]:
    ...
    for mapping in data.get("ResourceTagMappingList", []):
        ...
        # "codexcrew-drive-company-data" that somehow carries both tags must
