...
* **Snapshot** (the mockup's "Memory & workspace" row): the existing
  ``codex_crew.snapshot`` engine builds its portable ``.tar.gz`` (memory,
...
from codex_crew import snapshot
...
from codex_crew.history import SESSIONS_DIR_NAME
from codex_crew.platform_compat import file_lock, is_link_or_junction
...
def _authorize_upload(account: str, profile: str, region: str, *, caller: str) -> None:
    ...
    from codex_crew.deploy.engine import _checked
...
def run_snapshot_backup(
    ...
    with tempfile.TemporaryDirectory(prefix="kc-backup-") as tmp:
        ...
        archives = sorted(Path(tmp).glob("codexcrew-snapshot-*.tar.gz"))
        if not archives:
        ...
        key = f"snapshots/codexcrew-snapshot-{_stamp()}.tar.gz"
