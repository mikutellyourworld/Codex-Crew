...
from codex_crew.apps.builtins.dev_fleet import gateway_service, repository, runtime
...
from codex_crew.service import live_target
...
def _own_checkout_path() -> str | None:
    """Checkout root the RUNNING process's codex_crew package resolves into.
    ...
    ``<checkout>/src/codex_crew/__init__.py``.
    """
    try:
        import codex_crew as _pkg
...
def _launchd_live_worktree() -> str | None:
    ...
    the system-wide ``codexcrew``. ``None`` correctly means "no row is live"
    ...
    # <checkout>/.venv/bin/codexcrew -> <checkout>
...
_LIVE_GATEWAY_UNIT = "codexcrew-gateway.service"
# The launchd counterpart of the live systemd unit. Same agent
# `codexcrew service install` writes (codex_crew.service.common.LAUNCHD_LABEL);
...
_LIVE_GATEWAY_LABEL = "dev.codexcrew.gateway"
...
def _gateway_unit_name() -> str:
    ...
    Inside a pod (config home under ``.codexcrew-pods/<name>``) the owning unit
    ...
    try:
        ...
        home = config_dir()
        if home.parent.name == ".codexcrew-pods":
            return f"codexcrew-pod@{home.name}.service"
...
def _gateway_label() -> str:
    ...
    try:
        from codex_crew.config.loader import config_dir
        from codex_crew.pod.config import DEFAULT_UNIT_PREFIX
        from codex_crew.pod.launchd import LABEL_PREFIX

        home = config_dir()
        if home.parent.name == ".codexcrew-pods":
            prefix = os.environ.get("CODEXCREW_POD_UNIT_PREFIX", DEFAULT_UNIT_PREFIX)
...
def _in_pod() -> bool | None:
    ...
    ``.codexcrew-pods/<name>``) — same detection as _gateway_unit_name.
    ...
    try:
        ...
        return config_dir().parent.name == ".codexcrew-pods"
...
async def _live_user_unit_status() -> str:
    ...
    unit. A ``codexcrew service install`` SYSTEM unit
    (``/etc/systemd/system/codexcrew.service``) is NOT controllable that way:
...
def _make_live_status_error(code: str) -> str:
    ...
    return {
        ...
        "no_user_unit": (
            ...
            "`codexcrew service install` system unit needs root to bounce)"
        ),
        "user_unit_inactive": (
        ...
        "agent_not_indirected": (
            ...
            "can safely bounce. Re-run `codexcrew service install` to refresh "
        ...
        "agent_restart_contract_outdated": (
            ...
            "`codexcrew service install` to refresh the agent definition"
        ),
        "live_program_missing": (
            ...
            "agent definition, whereas codexcrew service install would rewrite "
...
def _manual_restart_command() -> str:
    ...
    Always the service-aware ``codexcrew restart``: it resolves whatever manager
...
async def _make_live(path: str, dry_run: bool = False, expected_staged: str | None = None) -> dict:
    ...
    ``.venv/bin/codexcrew`` (else Provision -> ``missing_venv`` when absent,
    ...
    is what keeps a ``codexcrew service install`` host (a SYSTEM unit, needing
    ...
    # manager. Restarting still does, and that is the one thing a `codexcrew
    ...
    # drivable at all, a detached `codexcrew restart` can still finish the
    ...
    kcbin = real / ".venv" / "bin" / "codexcrew"
    dist_index = real / "src" / "codex_crew" / "static" / "dist" / "index.html"
    ...
    def _validate_artifacts_sync() -> tuple[str, str] | None:
        ...
        if not kcbin.is_file():
            return (
                "missing_venv",
                (
                    f"{real.name} has no .venv/bin/codexcrew — Provision it first "
        ...
        if not os.access(kcbin, os.X_OK):
            return (
                "venv_not_executable",
                (
                    f"{real.name} has a non-executable .venv/bin/codexcrew — run "
        ...
        if not dist_index.is_file():
            return (
                ...
                (
                    ...
                    "(src/codex_crew/static/dist/index.html) — run Pull+Build "
        ...
        if not can_restart:
            ...
            if foreground is not None:
                ...
                if restarted:
                    ...
                    # while the detached `codexcrew restart` is acting on it.
                    _MAKE_LIVE_COMMITTED = True
                    return {
                        "ok": True,
                        "cutover": True,
