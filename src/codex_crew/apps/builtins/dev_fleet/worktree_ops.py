...
from codex_crew import dep_sync, frontend, hooks, platform_compat
from codex_crew.apps.builtins.dev_fleet import fleet_state, live, npm_preflight, repository, runtime
from codex_crew.executors import subprocess_executor
from codex_crew.loop_lock import LoopBoundLock
from codex_crew.sandbox import sandboxed_spawn_argv, shielded_prepare_off_loop
...
def _sync_base_ref() -> str:
    ...
    """
    return f"refs/codexcrew/sync-base-{os.getpid()}"
...
async def _prune_dead_sync_base_refs(repo: str) -> None:
    ...
    """
    listed = await repository._git(repo, "for-each-ref", "--format=%(refname)", "refs/codexcrew/")
    if not listed:
        return
    mine = _sync_base_ref()
    prefix = "refs/codexcrew/sync-base-"
...
def _pod_env() -> dict:
    ...
    return {**runtime._build_env(), "CODEXCREW_POD_REPO": repository._repo()}
        ...
        if runtime._POD_AVAILABLE and cfg:
            ...
            try:
                ...
                runtime.logger.warning(
                    ...
                    "`codexcrew pod down %s` once the backend is back",
                    name,
                    residue,
                    name,
                )
                    ...
                    if name in active:
                        ...
                        try:
                            ...
                            runtime.logger.warning(
                                ...
                                "leftover with `codexcrew pod prune`",
                                name,
...
async def _sync_start_locked() -> dict:
    ...
    if npm_bin is None:
        ...
        return {
            "ok": False,
            "error": (
                "npm not found. Codex Crew looks for a Node toolchain in "
                ...
                "CODEXCREW_NODE_BIN_DIR=/abs/path/to/node/bin in the gateway's "
    ...
    # that is incidental. `-m codex_crew...dep_sync` would import the module from
    ...
    if frontend_half:
        ...
        # the untracked-`codex_crew/`-at-the-checkout-root shadow -- but the
        # install is EDITABLE, so `import codex_crew...` still resolves into the
        ...
        try:
            snap_dir = Path(tempfile.mkdtemp(prefix="codexcrew-npm-preflight-"))
    ...
    if locked_scripts:
        ...
        snapshot_dir = Path(tempfile.mkdtemp(prefix="codexcrew-dep-sync-"))
    ...
    # CODEXCREW_EDITION_DIR and CODEXCREW_ALLOW_EDITION, so on an edition
    ...
    # which strips CODEXCREW_EDITION_DIR, so the guard would read "stock" on every
    # install and never fire. apps/backend.py therefore propagates that one var
    # explicitly, the same way it already propagates CODEXCREW_PROJECT_DIR.
        ...
        raw_steps += [
            ...
            (
                [
                    sys.executable,
                    "-c",
                    "import sys;from codex_crew.frontend import build_and_stage;"
    ...
    # mkdtemp and run BY PATH with `-I`. Running `-m codex_crew...sync_runner`
    ...
    try:
        runner_dir = Path(tempfile.mkdtemp(prefix="codexcrew-sync-runner-"))
    ...
    # codex_crew -- what it does cannot change with the revision merged under it.
    cmd = [
        sys.executable,
        "-I",
        "-c",
...
_DISABLE_BACKGROUND_ENV = "CODEXCREW_DEVFLEET_NO_BACKGROUND"
