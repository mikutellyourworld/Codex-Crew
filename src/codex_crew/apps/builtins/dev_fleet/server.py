"""Dev Fleet — standalone aiohttp backend for CodexCrew feature worktrees.

Manages CodexCrew feature worktrees (git worktrees of the main repo) and their
isolated pod test instances. Runs as a subprocess spawned by the CodexCrew app
backend system (apps/backend.py). The gateway proxies /apps/dev-fleet/api/* to
this process with X-CodexCrew-Proxy HMAC signing; HMAC middleware validates
...
async def dev_fleet_startup(app: web.Application) -> None:
    ...
    if discovered:
        ...
        valid, hint = await loop.run_in_executor(
            subprocess_executor(),
            lambda: (repository._is_codexcrew_checkout(discovered), repository._repo_source_hint()),
        )
        repository._REPO_INVALID_MSG = (
            None
            if valid
            else (
                f"not a Codex Crew checkout: {discovered} exists but does not carry the "
                f"markers (.git, src/codex_crew/, pyproject.toml). {hint}"
            )
        )
