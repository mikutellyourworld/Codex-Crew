---
name: pod-e2e
description: "ONLY for developing Codex Crew itself -- if the project you are working on is anything else, ignore this skill: it drives Codex Crew's own pod tooling, which does not exist in another repository. Runs end-to-end tests (backend API + frontend Playwright) for a Codex Crew feature worktree against an ISOLATED throwaway pod instance, without touching the live gateway. Use when asked to e2e-test / smoke-test / verify a Codex Crew worktree's feature hands-off, run API + browser tests on a pod, or prove a new backend route + UI work together. NOT for testing the live instance, and NOT a general-purpose e2e or smoke-test skill."
...
on one port** — exactly like a Docker container. The `codexcrew pod` CLI is the
...
ARTIFACT_DIR=~/.codexcrew-pods/.e2e-artifacts/<worktree-name>
```
...
```bash
codexcrew pod ls          # should be empty (torn down)
curl -s -o /dev/null -w '%{http_code}\n' http://127.0.0.1:5486/api/sessions   # live plane still alive
```

## The interface — `codexcrew pod` CLI

```bash
# 1. bring the pod up, get a handle (JSON: base_url + token + port)
codexcrew pod up <wt> --json
...
codexcrew pod down <wt>
```

Other verbs: `ls` (list running pods) · `status <wt>` · `token <wt>` · `url <wt>`
· `logs <wt>` · `provision <wt>`. Run `codexcrew pod --help` for the full list.

**Isolation guarantees** (enforced by the pod runtime):
own `CODEXCREW_HOME`, own port, **no tunnel** (can't grab the real Slack identity),
...
1. **up** — `codexcrew pod up <wt> --json`. If already active, reuses it (and
   won't stop it on exit). Boots the worktree's own gateway with `--no-crons`,
   blank-seed DB, isolated HOME.
2. **health** — polls `codexcrew pod status <wt> --json` until its `health` is
...
3. **auth** — proves auth: `/api/sessions` → 200 with token, 403 without.
   Token comes from `codexcrew pod up --json` output (no manual minting needed).
...
6. **collect** — all logs + screenshots land in
   `~/.codexcrew-pods/.e2e-artifacts/<wt>/`. Per-phase results are appended to
   ...
   screenshot.
7. **stop** — `codexcrew pod down <wt>`: stops the service, waits for its process
...
`<worktree>/.pod-test.sh` then `<worktree>/src/codex_crew/.pod-test.sh`.
...
The **pod** isolates the gateway under test (own `CODEXCREW_HOME`, own port,
...
```
spawn_run(task="""
You are a QA engineer verifying the CodexCrew feature in worktree '<wt>'.
...
`.venv/bin/codexcrew` (editable install) + a built SPA bundle (`static/dist`).
The pod boot refuses without them.

```bash
codexcrew pod up <wt>                 # auto-builds the venv; FAILS LOUD if no dist
codexcrew pod up <wt> --provision     # full on-ramp: venv + build, then up
codexcrew pod provision <wt>          # just the on-ramp (venv + dist)
codexcrew pod provision <wt> --venv-only
```

Every failure teaches the next step: no worktree → create one; no venv → auto;
no dist → build-or-`--provision`.

- Playwright venv: controlled by env `CODEXCREW_PW_PY`.
  ...
  export CODEXCREW_PW_PY=<path>/bin/python
  ```
...
3. **On approval, attach to the PR automatically -- do NOT ask again.**
   - Copy the approved files into `<worktree>/temp-screenshots/<feature>/`
     ...
     for the full convention; NEVER under `docs/` or `src/codex_crew/**` --
...
`~/.codexcrew-pods/.e2e-artifacts/<wt>/` -- **outside** the worktree -- by design;
...
codexcrew-worktree-dev skill, "Rule 9 -- Leave the worktree clean (so prune can
reap it)."
