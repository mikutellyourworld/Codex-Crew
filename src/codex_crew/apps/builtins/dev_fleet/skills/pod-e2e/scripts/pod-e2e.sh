...
#
#   codexcrew pod up --json  →  health poll  →  auth check  →  API tests  →  Playwright  →  pod down
#
# Everything runs on the pod's own port + its own CODEXCREW_HOME. The live
...
# Resolve the codexcrew CLI
CODEXCREW_CLI=""
_kc="$(command -v codexcrew 2>/dev/null || true)"
if [ -n "$_kc" ] && "$_kc" pod --help >/dev/null 2>&1; then
  CODEXCREW_CLI="$_kc"
fi
if [ -z "$CODEXCREW_CLI" ]; then
  for _cand in "$HOME/.local/bin/codexcrew" "/usr/local/bin/codexcrew"; do
    if [ -x "$_cand" ] && "$_cand" pod --help >/dev/null 2>&1; then
      CODEXCREW_CLI="$_cand"; break
    fi
  done
fi
[ -n "$CODEXCREW_CLI" ] || { echo "FATAL: codexcrew CLI with pod subcommand not found on PATH" >&2; exit 65; }

# Resolve the checkout path for $NAME via `git worktree list --porcelain`.
# We search from either CODEXCREW_POD_REPO or the script's own directory.
#
# This MUST mirror pod/runtime.py resolve_checkout(), or the harness can test a
# different checkout than `codexcrew pod up` booted — a silently wrong QA verdict.
...
_resolve_checkout() {
  local name="$1"
  local repo_hint="${CODEXCREW_POD_REPO:-$HERE}"
  local wt_path=""
  ...
  # Final fallback: CODEXCREW_POD_WORKTREES_ROOT/<name>
  if [ -z "$wt_path" ] && [ -n "${CODEXCREW_POD_WORKTREES_ROOT:-}" ] && [ -d "$CODEXCREW_POD_WORKTREES_ROOT/$name" ]; then
    wt_path="$CODEXCREW_POD_WORKTREES_ROOT/$name"
  fi
  echo "$wt_path"
}
...
if [ -z "$CHECKOUT" ] || [ ! -d "$CHECKOUT" ]; then
  ...
  echo "  Ensure a git worktree with basename '$NAME' exists, or set CODEXCREW_POD_REPO / CODEXCREW_POD_WORKTREES_ROOT" >&2
  exit 66
fi
...
_wt_kc="$CHECKOUT/.venv/bin/codexcrew"
if [ ! -x "$_wt_kc" ]; then
  echo "provisioning the worktree venv so the suite runs its own build..."
  "$CODEXCREW_CLI" pod provision "$NAME" --venv-only || true
fi
if [ ! -x "$_wt_kc" ] || ! "$_wt_kc" pod --help >/dev/null 2>&1; then
  ...
  echo "  Build it: codexcrew pod provision $NAME --venv-only" >&2
  exit 67
fi
CODEXCREW_CLI="$_wt_kc"
echo "codexcrew CLI: $CODEXCREW_CLI"

# Playwright runner (sibling script)
PW_PY="${CODEXCREW_PW_PY:-}"
...
E2E_ARTIFACT_BASE="$(_realpath_dir "$HOME/.codexcrew-pods/.e2e-artifacts")" \
  || E2E_ARTIFACT_BASE="$HOME/.codexcrew-pods/.e2e-artifacts"
...
# that skips the FE phase (--api-only, unhealthy pod, no CODEXCREW_PW_PY) or
...
MANIFEST=""
for _m in "$CHECKOUT/.pod-test.sh" "$CHECKOUT/src/codex_crew/.pod-test.sh"; do
...
_pod_down_best_effort() {
  if [ "$ALREADY_UP" -eq 0 ] && [ "$KEEP" -eq 0 ] && [ "$NO_STOP" -eq 0 ] && [ -n "$NAME" ]; then
    "$CODEXCREW_CLI" pod down "$NAME" >/dev/null 2>&1 || true
  fi
}
...
_pod_status_json=$("$CODEXCREW_CLI" pod status "$NAME" --json 2>/dev/null || echo '{}')
...
if [ "$_pod_is_up" = "yes" ]; then
  ...
  POD_JSON=$("$CODEXCREW_CLI" pod up "$NAME" --json 2>"$ARTIFACT_DIR/pod-up.log")
  if [ $? -ne 0 ]; then
...
if [ -z "$BASE_URL" ] || [ -z "$TOKEN" ]; then
  ...
  TOKEN=$("$CODEXCREW_CLI" pod token "$NAME" 2>/dev/null | tail -1)
  BASE_URL=$("$CODEXCREW_CLI" pod url "$NAME" 2>/dev/null | tail -1)
fi
...
# routinely held by another pod or by the live gateway -- and every Codex Crew
...
while :; do
  CODE=$("$CODEXCREW_CLI" pod status "$NAME" --json 2>/dev/null \
...
if [ "$HEALTHY" -eq 0 ]; then
  "$CODEXCREW_CLI" pod logs "$NAME" -n 50 > "$ARTIFACT_DIR/boot-fail.log" 2>&1 || true
...
if [ "$RUN_API" -eq 1 ] && [ "$HEALTHY" -eq 1 ]; then
  ...
  if [ ! -x "$API_PY" ]; then
    fail "api-tests — worktree venv python not found at $API_PY (run: codexcrew pod provision $NAME)"
  else
...
if [ "$RUN_FE" -eq 1 ] && [ "$HEALTHY" -eq 1 ]; then
  if [ -z "$PW_PY" ] || [ ! -x "$PW_PY" ]; then
    ...
    log "FAIL: Playwright python not found (set CODEXCREW_PW_PY)"
    log "  python3 -m venv <path> && <path>/bin/pip install playwright==1.61.0"
    log "  export CODEXCREW_PW_PY=<path>/bin/python"
    log "  (or re-run with --api-only to skip the frontend phase deliberately)"
    fail "playwright — no usable CODEXCREW_PW_PY, so zero screenshots were captured (see fix above)"
    ...
    CODEXCREW_POD_TOKEN="$TOKEN" "${PW_CMD[@]}" > "$ARTIFACT_DIR/playwright.log" 2>&1
    PW_RC=$?
    if [ "$PW_RC" -eq 0 ]; then
