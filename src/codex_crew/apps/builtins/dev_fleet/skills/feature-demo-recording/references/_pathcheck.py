...
LLM-influenced CLI arguments. Direct open() would bypass codex_crew path
controls, so every path is vetted through the CENTRALIZED sensitive-path
gate (codex_crew.security.is_sensitive_path) -- never a local copy of the
denylist, which would drift (miss .docker, .npmrc, .netrc, trust roots...).

Fail-closed: these scripts record CodexCrew demos, so the worktree venv they
run in has codex_crew installed. If the import fails, path safety cannot be
...
def _sensitive_gate(real_path, write=False):
    ...
    try:
        ...
        print(
            "FATAL: codex_crew is not importable -- path safety cannot be "
...
def read_json_input(path):
    ...
    try:
        ...
        print(
            "FATAL: codex_crew is not importable -- input reads require the "
...
def open_media_input(path, workdir=None):
    ...
    Delegates to codex_crew.hooks.safe_copy_file_nolink: open(O_NOFOLLOW) ->
    ...
    Fails closed when codex_crew is not importable. Returns
    ...
    try:
        ...
        print(
            "FATAL: codex_crew is not importable -- media input reads require "
