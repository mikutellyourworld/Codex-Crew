...
1. **Use an interpreter that can import `codex_crew`.** The gate consults the
   ...
   is NOT guaranteed to exist. Do NOT trust `command -v codexcrew` to name one: on
   ...
   `<py> -c "import codex_crew"`. Exit 78 with "cannot import codex_crew" means the
...
import `codex_crew`. Two interpreters is the normal case, and a
`ModuleNotFoundError: playwright` there is not the exit-78 gate.

```bash
PY=<worktree>/.venv/bin/python      # must satisfy: $PY -c "import codex_crew"
