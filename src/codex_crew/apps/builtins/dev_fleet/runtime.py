...
from codex_crew.apps.builtins.dev_fleet import npm_preflight, sync_runner
from codex_crew.env import find_node_tool, node_bin_dirs
...
logger = logging.getLogger("codex_crew.apps.builtins.dev_fleet.server")
...
#   _POD_IMPORTED  — the ``codex_crew.pod`` modules are importable, so the
...
def _find_cli() -> list[str]:
    """Invoke the codexcrew CLI as a module of OUR interpreter.

    Never resolved through the filesystem: a `codexcrew` shim planted in an
    ...
    Targets the ``codex_crew`` PACKAGE (its ``__main__``), NOT ``codex_crew.cli``:
    ``cli.py`` has no ``if __name__ == "__main__"`` guard, so
    ``python -m codex_crew.cli <cmd>`` imports the module, runs no ``main()`` and
    ...
    before ``codex_crew.cli`` is imported, so it is the only correct ``-m`` entry.
    """
...
def _bin_override_var(name: str) -> str:
    ...
    """
    return f"CODEXCREW_DEVFLEET_BIN_{name.upper().replace('-', '_')}"
...
def _toolchain_bin(name: str) -> str | None:
    ...
    * Codex Crew's own supported installer (``install.sh --mise`` /
      ``ensure-node.sh``) puts node under ``$HOME``, so ``_trusted_bin`` returned
      ``None`` for npm on exactly the hosts Codex Crew set up itself, and Pull+Build
...
def _sel():
    ...
    codex_crew.sel directly (no _handlers_pkg indirection needed)."""
    from codex_crew.sel import sel as _sel_singleton
