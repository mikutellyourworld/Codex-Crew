...
* **Stdlib only.** It imports nothing from ``codex_crew``. Everything that would
...
* **Run by path, never by module.** ``server`` copies this file's bytes into an
  ...
  ``-m codex_crew...sync_runner`` would import it from the working tree AFTER the
...
_BACKUP_SUFFIX = ".codexcrew-sync-backup"
...
def main(argv: list[str] | None = None) -> int:
    ...
    module needs to import nothing from ``codex_crew``.
    """
