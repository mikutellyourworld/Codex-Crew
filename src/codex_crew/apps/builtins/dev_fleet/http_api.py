...
from codex_crew.apps.builtins.dev_fleet import fleet_state, live, repository, runtime, worktree_ops
from codex_crew.apps.proxy_auth import raw_request_target

# --- standalone backend config ---
PORT = int(os.environ.get("PORT", 9100))
APP_NAME = os.environ.get("CODEXCREW_APP_NAME", "dev-fleet")
...
async def hmac_proxy_middleware(request: web.Request, handler) -> web.Response:
    """Verify X-CodexCrew-Proxy HMAC on every request except /health.
    ...
    header = request.headers.get("X-CodexCrew-Proxy")
    if not header:
        return _deny("missing X-CodexCrew-Proxy header")

    parts = header.split(":", 1)
    if len(parts) != 2:
        return _deny("malformed X-CodexCrew-Proxy header")
    ...
    try:
        ...
        # a readable directory without the Codex Crew markers) — distinct code so a
