...
that indirection (``codexcrew service install`` from a build that has it).
...
``codexcrew restart`` instead of a service manager. Strictly ordered
...
from codex_crew.service.common import launchd_live_program
from codex_crew.service.macos import (
    PLIST_PATH,
...
#                          be a silent no-op. Remedy: `codexcrew service install`
...
def atomic_write_text(path: Path, content: str) -> None:
    ...
    Thin delegate to :func:`codex_crew.atomic_write.atomic_write`, kept as a
...
#: run-marker (see :mod:`codex_crew.instances.run_marker`).
...
def default_confinement() -> "str | None":
    ...
    ``codexcrew-agents.slice`` cgroup scope. Namespaces and cgroups are
    INHERITED by children regardless of ``start_new_session``, so a
    ``codexcrew restart`` spawned from here would produce a replacement gateway
    ...
    (c) — because ``cli.main()`` clears the inherited ``CODEXCREW_SANDBOX_ACTIVE``
    ...
    * ``CODEXCREW_SANDBOX_ACTIVE`` — exported only by the namespace launcher /
      seatbelt env prefix, i.e. only ever present INSIDE the sandbox.
    * ``codexcrew-agents.slice`` in ``/proc/self/cgroup`` — the transient scope
    ...
    """
    if os.environ.get("CODEXCREW_SANDBOX_ACTIVE"):
        return "the Dev Fleet backend runs inside the Codex Crew OS sandbox"
    try:
        cgroup = Path("/proc/self/cgroup").read_text(encoding="utf-8")
    except OSError:
        return None
    if "codexcrew-agents.slice" in cgroup:
        return "the Dev Fleet backend runs inside the codexcrew-agents cgroup scope"
    return None
...
def default_detached_spawn(argv: list[str]) -> None:
    ...
    The command must OUTLIVE us — it is ``codexcrew restart``, which kills the
...
class ForegroundBackend:
    ...
    telling the operator to run ``codexcrew restart`` themselves. This backend
    ...
    **Identity.** There is no ``ExecMainStartTimestampMonotonic`` here, so
    ...
    replacement) — the same identity ``codexcrew restart`` itself waits on, and
    ...
    **FAIL SAFE.** This class never signals any process itself. Its only
    mutation is establishing the detached ``codexcrew restart``; when the binary
    ...
    **Confinement gate.** The Dev Fleet backend usually runs inside the
    gateway-applied OS sandbox and ``codexcrew-agents.slice`` cgroup scope,
    ...
    def _usable_launcher(path: "str | None") -> "str | None":
        """*path* when it is an absolute, executable ``codexcrew`` file."""
        if not path:
            return None
        p = Path(path)
        ...
        # PATH entry happens to name. .stem so a Windows ``codexcrew.exe`` would
        # also pass, though callers currently construct this backend POSIX-only.
        if p.stem != "codexcrew" or not p.is_absolute():
            return None
        try:
    ...
    def _resolve_binary(self, port: int) -> "str | None":
        """The ``codexcrew`` launcher to run the restart with, or ``None``.
        ...
        can have planted. A ``shutil.which("codexcrew")`` fallback is
        ...
        no launcher (a source-tree ``python -m codex_crew`` launch) keeps the
    ...
    async def restart_detached(self) -> "tuple[bool, str]":
        """Establish a detached ``codexcrew restart``. ``(ok, error)``.
        ...
        if kcbin is None:
            return False, "codexcrew binary could not be resolved"
