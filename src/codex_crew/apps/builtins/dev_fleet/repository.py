...
from codex_crew.apps.builtins.dev_fleet import runtime
...
class RepoNotConfigured(RepoUnavailable):
    """No Codex Crew checkout could be found, so there is no fleet to manage.
...
class RepoUnreadable(RepoUnavailable):
    ...
    directory that does not carry the Codex Crew markers. Carries the same
...
#: Set at startup when the resolved checkout does not carry the Codex Crew markers,
...
def _repo() -> str:
    ...
    """
    if not MAIN_REPO:
        raise RepoNotConfigured("no Codex Crew checkout found to manage")
...
def _own_source_checkout() -> str | None:
    ...
    for parent in here.parents:
        if parent.name == "src" and (parent / "codex_crew").is_dir():
            return str(parent.parent)
    return None


def _is_codexcrew_checkout(path: str) -> bool:
    """Whether *path* is a Codex Crew source checkout. Blocking — stats only.
...
# A candidate is adopted solely when it passes _is_codexcrew_checkout, so an
...
_CHECKOUT_DIR_NAMES = frozenset({"codexcrew", "kiro-crew"})
...
def _configured_main_repo() -> str:
    ...
    by a discovered one.
    """
    explicit = os.environ.get("CODEXCREW_DEVFLEET_REPO", "").strip()
    if explicit:
        return explicit
...
def _repo_source_hint() -> str:
    ...
    """
    if os.environ.get("CODEXCREW_DEVFLEET_REPO", "").strip():
        return "It is set by the CODEXCREW_DEVFLEET_REPO environment variable."
    configured = _load_dev_fleet_cfg().get("repo_path")
    if isinstance(configured, str) and configured.strip():
        return "It is set by dev_fleet.repo_path in config.json."
    return (
        "Point Dev Fleet at your Codex Crew checkout with the "
        "CODEXCREW_DEVFLEET_REPO environment variable, or with "
...
def _discover_main_repo() -> str:
    ...
    can only ever be pointed at a real Codex Crew checkout.
    ...
    if configured:
        return configured
    for candidate in (
        os.environ.get("CODEXCREW_PROJECT_DIR", ""),
        _own_source_checkout() or "",
        *_candidate_checkouts(),
    ):
        if _is_codexcrew_checkout(candidate):
            return candidate
    return ""
...
def _default_main_repo() -> str:
    ...
    if explicit:
        return explicit
    for candidate in (os.environ.get("CODEXCREW_PROJECT_DIR", ""), _own_source_checkout() or ""):
        if _is_codexcrew_checkout(candidate):
            return candidate
    return ""
...
async def _upstream_remote() -> str:
    ...
    'codexcrew' instead of 'origin') are honoured automatically. Cached at
...
def _load_dev_fleet_cfg() -> dict:
    ...
    directly rather than through CodexCrewConfig (a separate process owns the
...
async def _discover_worktrees() -> list[dict]:
    ...
    if rc != 0:
        ...
        # carries the Codex Crew markers, so an unverifiable one came from the
