...
# `CODEXCREW_PROXY_SECRET` is the whole of this backend's authentication: the
# gateway HMACs every proxied request with it (`codex_crew.apps.proxy_auth`), and
...
# script is untrusted project code, not Codex Crew's own: an inherited SSH agent
...
# (`CODEXCREW_HOME`, `CODEXCREW_APP_*`, operator-declared `CODEXCREW_DEVFLEET_BIN_*`,
# `CODEXCREW_PROJECT_DIR`, …). None of them are the child's business, and a
# forward-compatible prefix strip means a var added upstream later cannot leak
# through this seam by default.
CHILD_ENV_STRIP_PREFIXES = ("CODEXCREW_", "CODEX_CREW_")
