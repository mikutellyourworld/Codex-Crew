...
from codex_crew.apps.builtins.design_tweak.backend import (
    dev_preview,
    http_api,
    preview_files,
    request_state,
...
APP_NAME = os.environ.get("CODEXCREW_APP_NAME") or "design-tweak"
...
_DATA_ENV = os.environ.get("CODEXCREW_APP_DATA_DIR") or os.environ.get("CODEXCREW_APP_DATA", "")
if _DATA_ENV:
    DATA_DIR = Path(_DATA_ENV).expanduser().resolve()
else:
    _home = os.environ.get("CODEXCREW_HOME")
...
# `is_sensitive_path("~/.codex-crew")` is False and every unlisted file under it is
# servable. Registering `~` as a project is a natural pick when a site lives at
# `~/index.html`, and that alone put `~/.codex-crew/apps/<app>/.app_secret` — the
# proxy-auth HMAC credential shared by every app backend — and
# `~/.codex-crew/history/*.jsonl` (chat transcripts) one same-origin `fetch()` away
...
# `CODEXCREW_HOME` left the rest of the home — `history/*.jsonl` transcripts,
...
_CREW_HOME_ENV = os.environ.get("CODEXCREW_HOME")
_CREW_HOME = (
    Path(_CREW_HOME_ENV).expanduser() if _CREW_HOME_ENV else Path(_HOME_REAL) / ".kiro" / "crew"
)
_CODEXCREW_INTERNAL_DIRS: tuple[str, ...] = tuple(
    os.path.realpath(path)
    for path in (
        os.path.join(_HOME_REAL, ".kiro"),  # kiro-cli's dir; crew home nests inside
        os.path.join(_HOME_REAL, ".codexcrew"),  # pre-move legacy data home
        str(_CREW_HOME),  # a relocated crew home (CODEXCREW_HOME)
        str(DATA_DIR),  # a relocated app data home (CODEXCREW_APP_DATA_DIR)
    )
)
...
# design_critique's local-target guard via codex_crew.security. The shared
...
def _is_codexcrew_internal(target: Path) -> bool:
    return request_state.is_codexcrew_internal(runtime, target)
...
def _static_response(
    ...
    """Serve only after containment and the _is_codexcrew_internal policy floor."""
