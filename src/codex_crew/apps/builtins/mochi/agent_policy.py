...
from codex_crew.apps.builtins.mochi.soul_loader import rendered_bg_prompt_path, rendered_prompt_path
from codex_crew.atomic_write import atomic_write

logger = logging.getLogger(__name__)

#: Filename the framework reads from the app's data dir. Must match
#: ``codex_crew.apps.bridges.AGENT_MCP_POLICY_FILE``.
...
#: which live in ``codexcrew-core`` / ``codexcrew-cron``. Leaving those to the
...
_BUILTIN_GRANTS: dict[str, list[str]] = {
    CHAT_AGENT: ["codexcrew-core", "codexcrew-cron"],
}
...
def apply_policy(data_dir: Path, settings: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        from codex_crew.apps.bridges import get_app_manifest, refresh_app_agents
