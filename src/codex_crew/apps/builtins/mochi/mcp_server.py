...
def _data_dir() -> Path:
    ...
    server, a separate short-lived process, and ``codex_crew.apps.manager``
...
def _call_tool(name: str, args: dict[str, Any]) -> str:
    ...
    return call_tool_with_logging(
        ...
        downstream_service=f"codexcrew-{SERVER_NAME}",
    )
...
def run_mcp_server() -> None:
    """Run the stdio MCP server. Entry point for ``codexcrew app mcp mochi``."""
    # Keep logging off stdout — stdout is the JSON-RPC channel.
    logging.basicConfig(level=os.environ.get("CODEXCREW_LOG_LEVEL", "WARNING"), stream=None)
