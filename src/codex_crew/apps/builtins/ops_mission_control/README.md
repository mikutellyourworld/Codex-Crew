...
An autonomous ops first responder, shipped as a built-in Codex Crew app. It polls your
...
- **Other providers'** tokens go to a keystone file the agent cannot read or overwrite
  (it is on Codex Crew's sensitive-path floor). The API never returns a stored secret —
...
updates in place, diagnosis in the thread. It uses the Slack connection **Codex Crew
already has** and stores no token of its own — so if Slack is not set up for Codex Crew,
...
registered with Codex Crew's session map, so answering in the thread steers the running
...
Codex Crew. The public core never imports it and never branches on which edition is
running.
...
`src/codex_crew/builtin_skills/ops-mission-control/`, because a builtin app's own
