...
**Claims are atomic.** Two heartbeats (two Codex Crew instances, or an overlapping
...
def _redacted(text: str) -> str:
    ...
    from codex_crew.platform.context import redact_via_context as core_redact
...
#: a non-CodexCrew reader can be handed, so deleting it because the *index* got long would
...
def write_log(incident: Incident, *, diagnosis: str, actions: str, next_steps: str) -> Path:
    ...
    human reads at 2am, and the only artifact a reader who does not run Codex Crew can
