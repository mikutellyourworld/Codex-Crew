...
secret store. Codex Crew already holds one for its Slack gateway, and the live
...
operator has not configured Slack for Codex Crew itself, this channel is simply
...
from codex_crew.apps.builtins.ops_mission_control.backend import policy_store, store
...
from codex_crew.platform.context import redact_via_context as redact
...
def configured() -> bool:
    ...
    Does not check that Codex Crew's own Slack client exists — that is a runtime
...
def client_from_state(state: Any | None) -> Any | None:
    ...
    than fetched from a module global, because Codex Crew has no global state
...
def status(client: Any | None = None) -> dict[str, Any]:
    ...
    Codex Crew itself (configure the gateway's Slack integration — this app cannot
    ...
    if not enabled:
        ...
        detail = (
            "Codex Crew's own Slack integration is not connected, so there is "
