...
push channel Codex Crew offers that needs no credential and no inbound URL was inert,
...
``~/.codex-crew/apps/<name>/.app_secret``, and ``register_builtin_apps`` writes that
...
from codex_crew.apps.manager import get_app_manifest, is_app_enabled
...
def bus_from_state(state: Any | None) -> Any | None:
    ...
    is: Codex Crew has no global state accessor (state is per ``web.Application``), and an
