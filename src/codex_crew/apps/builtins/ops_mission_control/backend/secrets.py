...
Why not ``config.json``? Because Codex Crew serves an app's ``data/config.json``
...
from codex_crew.apps.builtins.ops_mission_control.backend.models import CorruptDocumentError
...
def secrets_path() -> Path:
    """Absolute path to the keystone secret file (honors ``CODEXCREW_HOME``)."""
...
class KeystoneFileBackend:
    ...
    def __init__(self, path: Path | None = None) -> None:
        ...
        # later ``CODEXCREW_HOME`` change is ignored and the whole process shares one
