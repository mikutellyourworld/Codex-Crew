...
from codex_crew.apps.builtins.ops_mission_control.backend.providers import webhook as webhook_mod
from codex_crew.apps.builtins.ops_mission_control.backend.registry import get_registry
from codex_crew.apps.builtins.ops_mission_control.backend.secrets import (
    delete_secret,
    describe_secrets,
    put_secret,
    redact_tokens,
...
def _slack_client(request: web.Request) -> Any | None:
    ...
    Passed explicitly into slack_out rather than fetched from a global: Codex Crew
...
async def _handle_put_settings(request: web.Request) -> web.StreamResponse:
    ...
    # reuses Codex Crew's own client (see slack_out for why).
    #
...
def _index_ledger_safely() -> dict[str, int]:
    ...
    """
    from codex_crew.apps.builtins.ops_mission_control.backend import ledger_index
    ...
    try:
        ...
        store_obj = VectorMemoryStore(embedding_dim=CodexCrewConfig.load().memory.embedding_dim)
        store_obj.init()
