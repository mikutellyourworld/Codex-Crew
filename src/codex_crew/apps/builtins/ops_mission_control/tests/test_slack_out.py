...
1. **No credential of our own.** The app must never grow a Slack token field; it
   reuses Codex Crew's client. A test asserts the secret store stays untouched,
...
from codex_crew.apps.builtins.ops_mission_control.backend import slack_out, store
    ...
    def test_module_defines_no_token_config_key(self) -> None:
        ...
        for banned in ('"slack_token"', "'slack_token'", '"bot_token"', "'bot_token'"):
            self.assertNotIn(
                banned,
                source,
                "Slack output must reuse Codex Crew's client, never store its own token",
            )
...
class TestPublish(_HomeIsolated):
    ...
    async def test_no_client_publishes_nothing(self) -> None:
        """Slack not configured on Codex Crew itself: quiet no-op, never a crash."""
...
class TestSlackLinkTargetIsValidated(unittest.TestCase):
    ...
    def test_the_block_omits_the_link_rather_than_rendering_it_broken(self):
        from codex_crew.apps.builtins.ops_mission_control.backend import models, slack_out
