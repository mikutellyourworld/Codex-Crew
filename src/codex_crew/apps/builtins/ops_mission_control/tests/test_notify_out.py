...
from codex_crew.apps.builtins.ops_mission_control.backend import dispatch, notify_out
from codex_crew.apps.builtins.ops_mission_control.backend.models import Signal
from codex_crew.notifications.bus import NotificationPayload, NotificationValidationError
...
class _NotifyBase(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        os.environ["CODEXCREW_HOME"] = self._tmp.name
        # Patched ON THE MODULE UNDER TEST, not as "codex_crew.apps.manager.is_app_enabled".
...
class TestNothingIsPushedOnAClaim(unittest.TestCase):
    ...
    def test_no_claim_path_calls_the_notifier(self) -> None:
        for module in (dispatch, __import__(
            "codex_crew.apps.builtins.ops_mission_control.backend.routes",
            fromlist=["routes"],
        )):
