...
class TestKeystoneProtection(unittest.TestCase):
    ...
    def _secret_path(self) -> str:
        return os.path.expanduser(f"~/.codex-crew/{secrets.SECRETS_FILENAME}")
    ...
    def test_every_home_prefix_is_covered(self):
        ...
        written as a literal ``~/.codexcrew/...``: ``test_runtime_home_write_paths``
...
class TestRedaction(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_a_leaky_adapter_cannot_reach_the_investigation_brief(self):
        ...
        prev = os.environ.get("CODEXCREW_HOME")
        os.environ["CODEXCREW_HOME"] = tmp
        ...
        try:
            ...
            if prev is None:
                ...
                os.environ["CODEXCREW_HOME"] = prev
...
class TestSecretStoreLockdownOrdering(unittest.TestCase):
    ...
    def test_write_lockdown_precedes_content(self):
        ...
        with mock.patch("codex_crew.platform_compat.restrict_to_owner", _measuring):
            self.backend.put("pagerduty", "api_token", "u+secretvalue")
    ...
    def test_a_failed_lockdown_preserves_the_previous_store(self):
        ...
        with mock.patch("codex_crew.platform_compat.restrict_to_owner", _refuse):
    ...
    def test_a_failed_payload_write_preserves_the_previous_store(self):
        ...
        with mock.patch(
            "codex_crew.atomic_write.tempfile", types.SimpleNamespace(mkstemp=_no_space)
        ):
            with self.assertRaises(OSError):
                self.backend.put("datadog", "api_key", "x" * 32)
...
class TestCrossPlatform(unittest.TestCase):
    ...
    def _sources(self):
        from codex_crew.apps.builtins.ops_mission_control.backend import ledger_sync
    ...
    def test_timezone_lookup_degrades_instead_of_raising(self):
        ...
        tmp = Path(tempfile.mkdtemp())
        prev = os.environ.get("CODEXCREW_HOME")
        os.environ["CODEXCREW_HOME"] = str(tmp)
        try:
...
class TestTheScheduleIsWriteProtectedButReadable(unittest.TestCase):
    ...
    def _path(self) -> str:
        from codex_crew.apps.builtins.ops_mission_control.backend.providers import schedule_file
    ...
    def test_the_shell_path_is_closed_too(self):
        ...
        bash gate is a STRING matcher over `_CREW_HOME_PREFIXES` (`.codex-crew`, `.codexcrew`),
        ...
        `config_dir()`, so it DOES follow a non-default `CODEXCREW_HOME`.

        That asymmetry means a custom-`CODEXCREW_HOME` install (a pod, `dev-backend.sh`) has
        ...
        for home in ("~", "$HOME", "/home/alice", "/Users/alice"):
            path = f"{home}/.codex-crew/apps/ops-mission-control/data/rotation.yaml"
            for cmd in (
...
class TestEveryRedactionSinkUsesTheSameSeam(unittest.TestCase):
    ...
    def _source(self, name: str) -> str:
        ...
        mod = importlib.import_module(f"codex_crew.apps.builtins.ops_mission_control.backend.{name}")
...
class TestTheIncidentIndexIsWriteProtectedButReadable(unittest.TestCase):
    ...
    def test_the_shell_path_is_closed_too(self):
        ...
        """
        for home in ("~", "$HOME", "/home/alice", "/Users/alice"):
            path = f"{home}/.codex-crew/apps/ops-mission-control/data/incidents/index.json"
            for cmd in (
                f"echo '{{}}' > {path}",
    ...
    def test_the_shell_matcher_is_verb_independent_including_reads(self):
        ...
        """
        path = "~/.codex-crew/apps/ops-mission-control/data/incidents/index.json"
        ...
        schedule = "~/.codex-crew/apps/ops-mission-control/data/rotation.yaml"
