...
from codex_crew.apps.builtins.ops_mission_control.backend import ledger, ledger_index
...
class TestSearch(_Env):
    def test_search_is_scoped_to_ledger_rows(self) -> None:
        """The index is shared with the rest of Codex Crew; an ops query wants ops
...
class TestSemanticRecallWiring(_Env):
    ...
    def _claimed(title: str = "DLQ fills on AccessDenied", resource: str = "my-queue"):
        from codex_crew.apps.builtins.ops_mission_control.backend import store as inc_store
        from codex_crew.apps.builtins.ops_mission_control.backend.dispatch import (
            ClaimedIncident,
        )
