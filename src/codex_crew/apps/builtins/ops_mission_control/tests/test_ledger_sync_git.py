...
class _TwoInstances(unittest.IsolatedAsyncioTestCase):
    ...
    def tearDown(self) -> None:
        ...
        # ``codex_crew.apps.manager`` also SETS ``manager`` as an attribute on the
        # ``codex_crew.apps`` package object, and that package was never evicted — so
        ...
        # So a later test doing ``import codex_crew.apps.manager as manager`` patched the
    ...
    def _use(self, home: Path):
        ...
        """
        os.environ["CODEXCREW_HOME"] = str(home)
        # `bridges` must go WITH `manager`: it does `from codex_crew.apps.manager import …`,
        ...
        for name in list(sys.modules):
            if (
                ...
                or name.startswith("codex_crew.apps.manager")
                or name.startswith("codex_crew.apps.bridges")
            ):
                del sys.modules[name]
...
class TestWhatGetsCommitted(_TwoInstances):
    async def test_the_rotation_schedule_syncs(self):
        ...
        from codex_crew.apps.builtins.ops_mission_control.backend.providers import (  # noqa: E501
            schedule_file,
        )
