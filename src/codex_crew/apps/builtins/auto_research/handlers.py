...
from codex_crew.apps.builtins.auto_research import subquestion_queue as _sq
...
from codex_crew.autonudge import get_instance as _autonudge_instance
from codex_crew.config.paths import data_home
from codex_crew.dashboard.chat_utils import (
    slot_history_key,
)
from codex_crew.knowledge.llm_pool import LLMPool
from codex_crew.llm_helpers import _extract_json_of_type
from codex_crew.on_loop_db import OnLoopDBGuard
from codex_crew.platform_compat import is_link_or_junction, unlink_link_or_junction

try:
    from codex_crew.artifacts import ArtifactNotFoundError, ArtifactStore
...
# the codexcrew-research agent's system prompt, so this only needs to name the cycle.
_RESEARCH_AGENT = "codexcrew-research"
_RESEARCH_NUDGE = (
...
# implementation in ``codex_crew.on_loop_db``. Defaults are deliberate: this
# surface IS fully offloaded, so it stays on the shared
# ``CODEXCREW_STRICT_ON_LOOP_PERSIST`` switch (which the e2e harness exports) and
