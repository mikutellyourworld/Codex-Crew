...
* ``-no-shell-escape`` is passed **explicitly** on every pdflatex invocation.
  ...
  ask.
* The compiler spawn is routed through :func:`codex_crew.sandbox.sandboxed_spawn_argv`
  — the OS-level sandbox + credential-scrubbed env chokepoint — and carries
  :func:`codex_crew.sandbox.create_subprocess_limited`, so a runaway macro expansion
...
from codex_crew.apps.builtins.papyrus.backend import procio, store, tectonic
...
logger = logging.getLogger("codexcrew.app.papyrus")
...
def _sensitive_hidden_dirs() -> tuple[str, ...]:
    ...
    Resolved per call, not cached at import: ``CODEXCREW_HOME`` is set per test and
    ...
    #
    # `sensitive_home_dirs()` returns paths relative to `$HOME`, so its `.codex-crew/*`
    # entries name the DEFAULT data home. With `CODEXCREW_HOME` pointed elsewhere — a dev
...
async def _run(
    ...
    ``~/.codex-crew/.env`` — but not the REST of CodexCrew's own trust root. The
    ...
    ``\\verbatiminput{~/.codex-crew/.local_secret}`` would typeset the gateway's own
...
async def compile_project(project: Path, main_file: str) -> CompileResult:
    ...
    # mount (`CODEXCREW_HOME` on NFS/SMB) those block for as long as the mount takes to
