...
Every paper lives under ``~/.codex-crew/apps/papyrus/data/projects/<name>/`` (via
:func:`codex_crew.apps.manager.app_data_dir`, the platform-standard app-scoped
...
def is_reparse_link(path: Path) -> bool:
    ...
    CodexCrew owns has to refuse a junction too, or the refusal is POSIX-only.
...
def safe_project_dir(name: str, root: Path | None = None) -> Path:
    ...
    # reasoning as `_config_path`: this is a directory CodexCrew owns and creates.
...
def _config_path(project: Path) -> Path | None:
    ...
    # file is one CodexCrew owns and writes, so a link at that name is illegitimate
    # wherever it points.
    #
...
def pdf_path(project: Path, main_file: str) -> Path | None:
    ...
    A repo shipping ``main.pdf -> ~/.codex-crew/.local_secret`` had that file served
