...
* **Sandbox chokepoint.** The repository is agent- and user-influenced content
  ...
  :func:`codex_crew.sandbox.sandboxed_spawn_argv` (OS isolation + scrubbed env)
  and spawns through :func:`codex_crew.sandbox.create_subprocess_limited`.
...
from codex_crew.apps.builtins.papyrus.backend import procio, store
from codex_crew.atomic_write import atomic_write
from codex_crew.executors import subprocess_executor
from codex_crew.git_divergence import divergence_count_args, parse_divergence_counts
...
def _pin_attributes_sync(cwd: Path) -> None:
    ...
    **Appended, never written over.** Overwriting with
    ...
    # written. Both are paths CodexCrew owns and writes BY NAME, so a link there is
    ...
    # Verified before the fix. The rule has to hold for every segment CodexCrew
    ...
    if (
        ...
        # security DECISION (a link where CodexCrew owns the name), and a denial that
