...
which on a stalled network filesystem (a ``CODEXCREW_HOME`` on NFS/SMB) block for
...
from codex_crew.apps.builtins.papyrus.backend import gitops, latex, store, tectonic
...
async def _handle_pdf(request: web.Request) -> web.StreamResponse:
    ...
    def _resolve() -> tuple[Path, Path | None]:
        ...
        # `main.pdf -> ~/.codex-crew/.local_secret` would otherwise have that file
