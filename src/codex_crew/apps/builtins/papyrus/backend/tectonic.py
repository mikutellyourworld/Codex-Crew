...
A stock machine has no LaTeX compiler, so ``pip install codexcrew`` shipped this
...
follows :mod:`codex_crew.embeddings`, the in-tree precedent for exactly this
...
from codex_crew.apps.builtins.papyrus.backend import store
...
SKIP_DOWNLOAD_ENV = "CODEXCREW_PAPYRUS_SKIP_TECTONIC_DOWNLOAD"
...
TECTONIC_URL_ENV = "CODEXCREW_PAPYRUS_TECTONIC_URL"
...
def reset_provision_state() -> None:
    """Forget the job state (tests, and ``CODEXCREW_HOME`` changes)."""
    global _job_thread
    with _state_lock:
...
def _reject_member_name(name: str) -> str | None:
    ...
    * an empty name.
    ...
    Mirrors :func:`codex_crew.apps.builtins.papyrus.backend.store.safe_child`'s
...
def _make_ssl_context() -> ssl.SSLContext:
    ...
    Same shape and reason as :func:`codex_crew.embeddings._make_ssl_context`: a
...
def provision_in_background(root: Path | None = None) -> bool:
    ...
    reasoning as :func:`codex_crew.embeddings._run_download_on_daemon_thread`. The
...
def _provision_job(root: Path | None) -> None:
    ...
    if installed:
        ...
        from codex_crew.apps.builtins.papyrus.backend import latex
