...
Codex Crew property of the target, not the spine; 08_safety §2.1 generalization note).
...
def _hash_path(path: Path, exclude: frozenset[str] = frozenset()) -> str:
    ...
    UNDER the Codex Crew data home, so the host's own log/ledger/activity writes during the boot
