...
Target: native Codex Crew builtin app `auto-improvement`
...
| Req | Decision |
|---|---|
| 1. GitHub as native code host | `gh pr create --draft` replaces `cr --new-review`. The source's own `spine/profile.py` names this exact substitution as the intended external-host path, so the seam already exists. PR status/checks come from Codex Crew's existing `source_providers.fetch_pull_request{,_checks}` — no new API client. |
...
ACP event-constant module, both repointed at `codex_crew`). Codex Crew's ACP event constants
...
`backend/deps.py`, `_vendor/`, `ui/` vite lib build. Reasons: Codex Crew builtins
...
forget, always a new session, no slot identity. Codex Crew offers three; we use all
...
```
src/codex_crew/apps/builtins/auto_improvement/
