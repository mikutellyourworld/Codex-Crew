...
authenticated every request with a proxy HMAC header. A Codex Crew builtin
...
_CONFIG_WRITABLE = frozenset(
    {
        ...
        # `~/.aws`/`~/.gnupg`/`gh` stores; the PROVIDER path drives a Codex Crew session
...
async def _handle_finding_detail(request: web.Request) -> web.StreamResponse:
    ...
    def _gather() -> dict[str, Any]:
        ...
        # ``codex_crew_apps_builtins_auto_improvement_spine_contracts_py_proposal`` and the
