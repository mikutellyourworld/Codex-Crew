...
def _credentials_are_unconfined() -> str:
    ...
    try:
        ...
        mode = str(getattr(CodexCrewConfig.load(), "sandbox", "") or "").strip().lower()
...
class RunSupervisor:
    ...
    def _build_runner(self, *, stop_check) -> Any:
        ...
        ``import codex_crew.acp`` FIRST: there is a known circular import that only
        ...
        """
        import codex_crew.acp  # noqa: F401  # circular import — must precede the factory
        ...
        if SessionAgentRunner.available():
            ...
            # PROVIDER path does not: it drives a Codex Crew session, so isolation is whatever
            ...
            # silently activates the DEFAULT agent — which carries the full codexcrew-core
