...
def _write_protected_targets() -> tuple[str, ...]:
    """Directories to bind-mount empty so agent-authored tests cannot write Codex Crew's config.
    ...
    * The PARENT DIRECTORY is masked, not the file. ``sandboxed_spawn_argv``'s
      ...
      ``~/.codex-crew/config.json`` and exit 0; passing the parent blocked it.
    * Only EXISTING directories are returned — the launcher mounts over each target, and a
    ...
    Masking the whole ``~/.codex-crew`` directory is safe *for this caller* specifically: the
    sandboxed child is the TARGET REPOSITORY's pytest, which has no legitimate reason to read
    Codex Crew's own configuration. It is a superset of the 18 per-file entries ``strict``
...
def _run(
    ...
    # Also MASK Codex Crew's own write-protected files. `mode="strict"` hides 52 credential
    ...
    # `~/.codex-crew/config.json` and exited 0. Those paths are `security.
    ...
    # instead. Scoped to Codex Crew's OWN control files (config.json, config.local.json
    # under both `.codex-crew` and `.codexcrew`), i.e. the one-way doors that
...
class RepoIsolation:
    ...
    def do_not_pollute_paths(self) -> list[Path]:
        ...
        The app's own data dir and the user's Codex Crew home: the two places a leak
    ...
    def do_not_pollute_excludes(self) -> list[Path]:
        ...
        The app's data dir lives UNDER the Codex Crew config home, and the orchestrator
...
class GitHubRepoProfile(ProfileFieldAliases):
    ...
    def _test_dir(self) -> str:
        ...
        Not just "the first one that exists": a repo can have BOTH (Codex Crew has 776
