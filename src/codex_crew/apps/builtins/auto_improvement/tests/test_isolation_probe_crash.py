...
* every other nonzero exit keeps its existing fail-closed meaning — git's own
  ...
  filename substring, which a repository named ``codexcrew_sandbox_x`` can put
  there legally;
...
_LAUNCHER_TRACEBACK = (
    ...
    '  File "/home/user/.config/codexcrew/run/codexcrew_sandbox_ab12cd.py", '
    "line 293, in main\n"
...
class TestProbeCrashShape:
    ...
    def test_a_repo_named_like_the_launcher_cannot_spoof_the_signature(
        ...
        A repo may legally be NAMED ``codexcrew_sandbox_x`` (the owner/repo
        ...
        probe_result["proc"] = _proc(
            128,
            stderr=(
                "fatal: bad config line 1 in file " "/scratch/owner--codexcrew_sandbox_x/.git/config"
            ),
        )
        assert clone_setup._origin_urls(Path("/nonexistent/clone"), push=True) is None
        assert clone_setup._push_disabled(Path("/nonexistent/clone")) is False
