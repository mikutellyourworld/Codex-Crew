"""Notes backend — git-backed markdown vaults for CodexCrew.
...
from codex_crew import hooks, platform_compat, security
from codex_crew.apps.builtins.md_notebook import git_ops
from codex_crew.apps.builtins.md_notebook import notes as notes_mod
from codex_crew.apps.proxy_auth import raw_request_target, verify_proxy_request
from codex_crew.atomic_write import atomic_write, replace_with_retry
from codex_crew.config.paths import config_dir
from codex_crew.loop_lock import LoopBoundLock
from codex_crew.platform_compat import restrict_to_owner
from codex_crew.sel import sel

logger = logging.getLogger(__name__)

PORT = int(os.environ.get("PORT", 9137))
APP_NAME = os.environ.get("CODEXCREW_APP_NAME", "md-notebook")


# Data-home paths are resolved LAZILY, never at import time (issue #874): the
# active CodexCrew home depends on CODEXCREW_HOME, which a pod, the legacy-home
...
def _default_home() -> Path:
    """The app's data root under the active CodexCrew home.

    ``config_dir()`` follows ``CODEXCREW_HOME`` (default ``~/.codex-crew``), so an
    isolated dev instance (``CODEXCREW_HOME=.codexcrew-dev``) keeps its own vaults
...
def _crew_data_home() -> Path:
    """The app's data root under the CodexCrew data home, IGNORING
    ``MD_NOTEBOOK_HOME``.
    ...
    # resolver. It honors CODEXCREW_HOME on every call.
    try:
        base = config_dir()
    except Exception:
...
def _safe_error(exc: BaseException) -> str:
    ...
    like ``[Errno 2] No such file or directory: '/home/<user>/.codex-crew/...'``,
...
async def api_vault_knowledge(request: web.Request) -> web.Response:
    """Persist a vault's 'sync to Codex Crew knowledge' preference.
...
def create_app() -> web.Application:
    ...
    from codex_crew.apps.builtins.md_notebook import syncer
