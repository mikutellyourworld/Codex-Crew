...
- **Codex Crew memory is not this store.** A session may inject remembered facts about the user, so you can legitimately recall something without reading the Preferences tab. Say which source it came from — crediting the tab for a memory recall is the same false impression as claiming to have saved something.
