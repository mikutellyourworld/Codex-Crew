...
from codex_crew.apps.builtins.personal_shopper.backend import routes as routes_mod
from codex_crew.apps.builtins.personal_shopper.backend.store import PreferenceStore
...
class RoutesTestCase(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self) -> None:
        ...
        # writing sites.json into the OPERATOR'S real ~/.codex-crew. Redirect the
...
class TestTheSuiteHasNoSideEffects(RoutesTestCase):
    ...
    ``~/.codex-crew`` every time the suite ran — silently replacing whatever store
    ...
    async def test_the_real_data_home_is_never_resolved(self) -> None:
        ...
        self.assertNotIn(".codex-crew/apps", resolved)
        self.assertNotIn(".codexcrew/apps", resolved)
