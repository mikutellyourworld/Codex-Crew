...
import codex_crew
from codex_crew import link_unfurl, platform_compat, sandbox
...
# "import codex_crew"` SHELL command, which the gateway's own security filter
# hard-blocks because the string contains the package name.
_SKILL_DIR = (
    Path(codex_crew.__file__).parent / "apps/builtins/design_critique/skills/design-critique"
)
...
# local-path guard via codex_crew.security. The is_sensitive_path floor covers the
...
def _uploads_dir() -> Path:
    # Read at call time, not import: CODEXCREW_HOME can relocate the data home and
...
def _tool(name: str) -> str | None:
    ...
    try:
        ...
        for _var in ("CODEXCREW_SCRATCH", "TMPDIR"):
