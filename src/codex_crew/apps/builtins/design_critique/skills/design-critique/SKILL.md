...
- **Code or markup file** (HTML, JSX/TSX, Vue, Svelte, a CSS+HTML pair, etc.): **render it, then
  ...
  printed by the resolution command (`python3 -c "import codex_crew, pathlib; print(pathlib.Path(codex_crew.__file__).parent / 'apps/builtins/design_critique/skills/design-critique')"`), and the scripts live in `<skill-dir>/scripts`:
