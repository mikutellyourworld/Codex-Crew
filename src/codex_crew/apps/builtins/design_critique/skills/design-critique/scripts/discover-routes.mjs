...
function detect() {
  ...
  // CodexCrew app with an embedded UI — no routes of its own.
  ...
  if (ajRaw) {
    try {
      ...
      if (aj.ui || aj.mcpServers || aj.agents) {
        ...
        notes.push('This is a CodexCrew app with an embedded UI — it has no routes to crawl by itself. Run the whole CodexCrew dashboard and open its page' + (hostPage ? ` (${hostPage})` : '') + ', or give a screenshot.')
        return { framework: 'CodexCrew app (embedded UI)', routing: 'host-embedded', routes: [], hostPage }
      }
