...
on the cheap ``codexcrew-lite`` background agent (the lever workflows, title
...
async def run_oneshot_translation(sessions: Any, prompt: str) -> str:
    ...
    for this: ``codexcrew-lite`` scopes the session to ``tools: []`` and resolves a
    ...
    It reuses the user's own Codex Crew backend, so live translation needs no
    ...
    provider, _is_new, _resumed = await sessions.get_or_create(key, agent="codexcrew-lite")
    try:
...
def sess_is_noise(text: str) -> bool:
    ...
    """
    from codex_crew.apps.builtins.meetings.backend.domain.session import is_noise
