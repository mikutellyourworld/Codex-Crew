...
Transcribes a live meeting through CodexCrew's own streaming speech-to-text, fans
...
# ``importlib.import_module("codex_crew.apps.builtins.meetings")`` then checks
# ``hasattr(_mod, "register_routes")`` on the PACKAGE itself (not the
# backend.routes submodule) — same as issue_radar/__init__.py.
from codex_crew.apps.builtins.meetings.backend.routes import register_routes  # noqa: F401
