...
from codex_crew.apps.builtins.spec_builder import backend as backend_package
from codex_crew.apps.builtins.spec_builder.backend.handlers import _ClientClaim
from codex_crew.apps.builtins.spec_builder.tests.routes_facade import (
    BACKEND_MODULES,
    backend_namespace,
    routes,
    routes_source,
...
def _live_state_snapshot() -> dict[str, int]:
    ...
    module-level ``routes._state_dir()`` freezes whichever ``CODEXCREW_HOME``
...
def _never_touch_the_real_state(monkeypatch, tmp_path):
    ...
    the USER's live ~/.codex-crew/workspace/spec-builder/.
...
def test_security_helper_is_imported_at_module_scope():
    ...
    src = routes_source()
    assert "    from codex_crew.security import is_sensitive_path" not in src
...
def test_gateway_helpers_are_imported_at_module_scope():
    ...
    for node in ast.walk(tree):
        ...
        for inner in ast.walk(node):
            if isinstance(inner, ast.ImportFrom) and (inner.module or "").startswith("codex_crew"):
    ...
    for mod in local_imports:
        assert mod.startswith("codex_crew.dashboard."), f"undocumented local import: {mod}"
...
async def test_nudge_loop_removal_keeps_the_fsync_off_the_loop(tmp_path, monkeypatch):
    ...
    from codex_crew import autonudge as an
...
def test_seed_prompt_is_self_contained_and_type_aware():
    ...
    (that path symlinks from ~/.codex-crew/apps/<name>/, which a wheel-shipped
...
def test_route_registration_touches_no_filesystem():
    ...
    loop during start_dashboard, so a CODEXCREW_HOME on stalled network storage
...
def test_slack_ts_regex_is_bounded():
    ...
    from codex_crew.messaging import link as link_mod
...
async def test_delete_restores_the_spec_when_archiving_fails(tmp_path, monkeypatch):
    ...
    import codex_crew.dashboard.chat_persistence as cp
...
async def test_delete_discards_queued_work_before_cancelling(monkeypatch):
    ...
    monkeypatch.setitem(
        sys.modules,
        "codex_crew.dashboard.chat_persistence",
...
@pytest.mark.parametrize(
    "path",
    [
        "~/.codex-crew/trust/spec-builder-decisions.json",
        "~/.codexcrew/trust/spec-builder-decisions.json",
...
@pytest.mark.parametrize(
    "cmd",
    [
        "ln -s /tmp/evil ~/.codex-crew/trust",
        "ln -sf /tmp/evil ~/.codex-crew/trust/spec-builder-decisions.json",
        "mv ~/.codex-crew/trust /tmp/x",
        "mv /tmp/evil ~/.codex-crew/trust/spec-builder-decisions.json",
        "rm -rf ~/.codex-crew/trust",
        "cp /tmp/evil ~/.codex-crew/trust/spec-builder-decisions.json",
    ],
)
