...
from codex_crew.platform_compat import RENAME_NOREPLACE_AVAILABLE, rename_noreplace
...
_DUPLICATE_MARKER = ".codexcrew-duplicate"
...
def _safe_dir(raw: str, *, must_exist: bool = True) -> Path | None:
    ...
    must pass through, so the guarantees hold uniformly:
      ...
      * must not be a sensitive path (credential stores, ``.ssh``, ``.aws``,
        policy files) per ``codex_crew.security.is_sensitive_path``;
