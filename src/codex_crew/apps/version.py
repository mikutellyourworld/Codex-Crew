...
def check_min_version(min_version: str) -> str | None:
    """Return an error string if the current CodexCrew version is too old.
    ...
    if not min_version:
        return None
    try:
        from codex_crew import __version__ as current

        if parse_version(current) < parse_version(min_version):
            return (
                f"App requires Codex Crew >= {min_version}, "
                f"but current version is {current}. "
                f"Please update Codex Crew first."
            )
