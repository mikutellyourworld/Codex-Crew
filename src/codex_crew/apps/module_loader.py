...
from codex_crew.apps.execution import app_execution_denied, is_builtin_app
from codex_crew.sel import sel

logger = logging.getLogger(__name__)

# Builtin apps ship inside the CodexCrew package and are trusted like core code.
...
def _app_namespace_root(app_name: str) -> str:
    ...
    prefix of another's -- ``_codexcrew_app_foo`` and ``_codexcrew_app_foo-bar``
    ...
    """
    return f"_codexcrew_app_{app_name}"
...
def load_app_module(app_name: str, app_dir: Path, module_path: str) -> Callable[..., Any]:
    ...
    Module is registered in sys.modules as ``_codexcrew_app_{app_name}.{module_name}``
