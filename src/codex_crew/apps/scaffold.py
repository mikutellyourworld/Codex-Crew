"""App scaffolding — generate a new app skeleton with `codexcrew app init`.
...
_BACKEND_TEMPLATE = '''"""Minimal backend for {name} — a CodexCrew app.

Run with: python backend/server.py
Or let CodexCrew manage it via the app manifest backend section.
"""
import json
import os
...
APP_NAME = os.environ.get("CODEXCREW_APP_NAME", "{name}")
  ...
  build: {{
    ...
    rollupOptions: {{
      external: [
        ...
        '@codexcrew/app-sdk', '@codexcrew/app-sdk/ui', 'lucide-react',
      ],
    }},
  }},
}})
...
import {{ useAppApi }} from '@codexcrew/app-sdk'
...
```bash
codexcrew app install /path/to/{name}
codexcrew app enable {name}
```
...
def scaffold_app(
    ...
    *,
    ...
    # `codexcrew app install` refuses that manifest, so the error surfaces one
    ...
    if not description:
        description = f"A Codex Crew app: {display_name}"
    if not author:
