"""App manifest — static metadata for CodexCrew apps.

An app manifest (``app.json``) declares an app's identity, resources, and
requirements without executing any app code.  CodexCrew reads it during
install to register agents, skills, crons, UI pages, and backend config.

Design follows the same pattern as :class:`codex_crew.plugins.manifest.PluginManifest`
...
from codex_crew.constants import WINDOWS_DEVICE_STEMS
from codex_crew.cron import is_valid_skip_date, is_valid_timezone
...
# ``codex_crew.apps.routes.setup_routes``; this set is mirrored by
# ``codex_crew.dashboard.token_auth.RESERVED_APP_PATH_SEGMENTS`` — keep both in
...
@dataclass
class Dependencies:
    """External dependencies that CodexCrew should resolve during install.

    ``managedBy`` controls the default installation strategy:
      - ``"gateway"``: CodexCrew resolves each dependency through the edition's
        ``CapabilityManager`` seam
      - ``"app"``: CodexCrew only checks existence, does not install
...
class ClientInstallConfig:
    ...
    Used when CodexCrew runs remotely (e.g. cloud desktop) and the app
...
class PlatformConfig:
    ...
    - ``"server"`` (default): CodexCrew clones + installs on the server.
    - ``"client"``: Must be installed on the user's local machine.
      When CodexCrew is on an incompatible platform, the App Store shows
    ...
    (``window.codexcrew.isElectron``, set by the shell's preload) is client-side
    ...
    #
    # ``windows`` is in both directions because CodexCrew itself supports Windows
...
@dataclass
class AppManifest:
    """Static metadata for a CodexCrew app — readable without executing app code.

    Parsed from ``app.json`` at the root of an app package.  Follows the same
    pattern as :class:`~codex_crew.plugins.manifest.PluginManifest`: dataclass
    ...
    author: str = ""
    license: str = ""
    minCodexCrewVersion: str = ""  # noqa: N815
    ...
    def validate(self, app_root: Path | None = None) -> list[str]:
        ...
        # 'codex_crew.apps.builtins.x.server' has no '..' and is not absolute, so
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.minCodexCrewVersion:
            d["minCodexCrewVersion"] = self.minCodexCrewVersion
        if self.signer:
    ...
    def from_dict(cls, data: dict[str, Any]) -> AppManifest:
        ...
        return cls(
            ...
            minCodexCrewVersion=str(data.get("minCodexCrewVersion", "")),  # noqa: N815
