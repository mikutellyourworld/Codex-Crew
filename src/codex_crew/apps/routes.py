"""App management REST API endpoints for the CodexCrew dashboard.
...
from codex_crew.apps import official_catalog
from codex_crew.apps.backend import (
...
from codex_crew.apps.builtins import BUILTIN_NAMES
from codex_crew.apps.dependencies import clean_dependencies
from codex_crew.apps.dependencies import resolve_dependencies as _resolve_deps
from codex_crew.apps.dependency_ledger import (
    canonical_dep_key,
    classify_and_clean_for_uninstall,
    classify_for_uninstall,
    declared_capability_keys,
)
from codex_crew.apps.dev_mode import dev_mode_granted_root, is_dev_mode_cached
from codex_crew.apps.event_bus import build_broadcast_fn
from codex_crew.apps.execution import app_execution_denied
...
from codex_crew.apps.lifecycle_scripts import run_lifecycle_script as _run_lifecycle_script
...
from codex_crew.apps.manifest import Dependencies, PlatformConfig
from codex_crew.apps.official_category_order import forget_cache as forget_category_order_cache
from codex_crew.apps.official_category_order import load_category_order
from codex_crew.apps.official_editorial import forget_cache as forget_editorial_cache
from codex_crew.apps.official_editorial import load_sections
...
from codex_crew.apps.teardown import forget_app_hooks, teardown_app_runtime
from codex_crew.apps.version import check_min_version as _check_min_version_str
...
from codex_crew.pinned_fs import (
    PinnedPathRefusal,
    fd_real_path,
    is_reparse_point,
...
from codex_crew.publish_governance import DEPLOY_WEB_PROVIDER_ID, publish_denied_reason
...
def _check_min_version(manifest_data: dict[str, Any]) -> str | None:
    """Check if the app requires a newer CodexCrew version.

    Returns an error message if the current version is too old, or None if OK.
    """
    return _check_min_version_str(manifest_data.get("minCodexCrewVersion", ""))
...
async def handle_publish_providers(request: web.Request) -> web.Response:
    ...
    * the publish-governance chokepoint denies its destination id (governance
    ...
    from codex_crew.dashboard.handlers._shared import admits_cloud_deployment
    ...
    try:
        from codex_crew.deploy import profiles as _deploy_profiles
...
async def _register_app_off_loop(name: str) -> RegistrationResult:
    ...
    ``CODEXCREW_HOME`` — manifest reads, skill-dir symlink walks, agent JSON
...
async def handle_install_app(request: web.Request) -> web.Response:
    ...
    # Check minCodexCrewVersion before installing
...
async def handle_register_external(request: web.Request) -> web.Response:
    ...
    CodexCrew only tracks metadata so the dashboard can display them.
...
async def handle_uninstall_app(request: web.Request) -> web.Response:
    ...
    if is_registry_source(info.get("source", "")):
        ...
        if app_reg_name:
            from codex_crew.apps.registry import app_source_dir
    ...
    async with app_lifecycle_lock(name):
        ...
        if origin == "builtin" and name in _BUILTIN_SERVICE_APPS:
            try:
                ...
                from codex_crew.dashboard.chat_utils import run_config_write
...
async def handle_disable_app(request: web.Request) -> web.Response:
    ...
    async with app_lifecycle_lock(name):
        ...
        if module_name in BUILTIN_NAMES:
            try:
                mod = importlib.import_module(f"codex_crew.apps.builtins.{module_name}")
        ...
        return web.json_response(
            {
                ...
                "remote": True,
                "command": open_cmd,
                "message": f"Codex Crew is running remotely. Run this on your local machine: {open_cmd}",
            }
        )
...
async def handle_registry_install_stream(request: web.Request) -> web.StreamResponse:
    ...
    from codex_crew.apps.registry import StreamingLogLines
...
async def handle_app_config(request: web.Request) -> web.Response:
    ...
    Reads/writes ``~/.codex-crew/apps/{name}/data/config.json``.
    ...
    from codex_crew.apps.manager import app_data_dir
    from codex_crew.atomic_write import atomic_write
...
async def handle_app_dev_mode(request: web.Request) -> web.Response:
    ...
    ``codexcrew app dev <name> --confirm-out-of-install-root``.
    """
...
async def _fetch_git_blob(
    ...
    *,
    ...
    (mirroring how :mod:`codex_crew.apps.registry` already clones), read the
    ...
    from codex_crew.apps.registry import is_clone_host_trusted
    ...
    try:
        tmp_root = await asyncio.to_thread(tempfile.mkdtemp, prefix="codexcrew-blob-")
        ...
        if owner_designated:
            ...
            # ``CodexCrewConfig.load`` (an unbounded ``read_text`` + ``json.loads`` +
...
async def handle_blob_proxy(request: web.Request) -> web.Response:
    ...
    Caches fetched blobs to ~/.codex-crew/cache/blobs/{repo}/{ref}/{path}.
...
# Strip sensitive auth headers — app backends use X-CodexCrew-Proxy HMAC, not user cookies
...
async def handle_app_api_proxy(request: web.Request) -> web.StreamResponse:
    ...
    # Header format: X-CodexCrew-Proxy: <timestamp>:<hmac-sha256>
    ...
    try:
        ...
        headers["X-CodexCrew-Proxy"] = f"{ts}:{sig}"
    except OSError as exc:
...
async def handle_registries_refresh(request: web.Request) -> web.Response:
    ...
    """
    from codex_crew.apps.registry import refresh_registries
