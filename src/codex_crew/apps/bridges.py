"""Registration bridges — wire app resources into CodexCrew's runtime.

When an app is installed or enabled, its agents, skills, and cron jobs need
to be registered with CodexCrew's existing systems.  This module provides
...
from codex_crew.apps.cron_sdk import CronSDK
from codex_crew.apps.execution import (
    app_execution_denied,
    shipped_builtin_app_root,
)
from codex_crew.apps.interpreter import resolve_app_python, venv_provided_command
...
from codex_crew.cron_script import resolve_script_path
from codex_crew.env import emit_env
from codex_crew.executors import maintenance_executor
from codex_crew.platform.governance import may_skip_gate_now, strip_ungoverned_auto_approve
...
# Where CodexCrew loads skills from
...
def _ceiling_filtered_allowed(refs: object, agent_name: str = "") -> list[Any]:
    ...
    :func:`~codex_crew.platform.governance.may_skip_gate`, because this is one of
...
_HOST_CLI_COMMAND = "codexcrew"


def _pin_host_cli_command(app_name: str, cfg: dict[str, Any]) -> dict[str, Any]:
    """Rewrite ``command: "codexcrew"`` to THIS gateway's interpreter.
    ...
    * the launcher on PATH may belong to a DIFFERENT CodexCrew install, so the
    ...
    * that launcher may simply be broken (a stale wrapper whose interpreter is
    ...
    Pinning ``sys.executable -m codex_crew`` plus PYTHONPATH and CODEXCREW_HOME makes
    ...
    pkg_parent = str(Path(codex_crew_file()).parent.parent)
    env = dict(cfg.get("env") or {})
    existing = env.get("PYTHONPATH", "")
    env["PYTHONPATH"] = f"{pkg_parent}{os.pathsep}{existing}" if existing else pkg_parent
    env.setdefault("CODEXCREW_HOME", str(app_dir(app_name).parent.parent))
    cfg["command"] = sys.executable
    cfg["args"] = ["-s", "-m", "codex_crew", *list(cfg.get("args") or [])]
    cfg["env"] = env
    return cfg


def codex_crew_file() -> str:
    """Path to the running ``codex_crew`` package's ``__init__``."""
    import codex_crew

    return str(codex_crew.__file__)
...
def _own_mcp_servers(app_name: str) -> dict[str, Any]:
    ...
    reference: ``_register_mcp_servers`` writes the server into CodexCrew's own
...
def _managed_mcp_spec(name: str) -> dict[str, Any] | None:
    ...
    """
    from codex_crew.agent import _MANAGED_MCP_SERVERS
...
#: config must name a RUNTIME path — one under the data home, which ``CODEXCREW_HOME``
...
def _placeholder_values(app_name: str) -> dict[str, str]:
    ...
    try:
        from codex_crew.apps.builtins.pptx_maker.backend import paths as pptx_paths
        from codex_crew.apps.builtins.pptx_maker.backend import provision as pptx_provision
    ...
    * a copy is regenerated from the template on every registration, and the
    ...
    with _health_reconcile_guard():
        ...
        for agent_path_str in manifest.agents:
            ...
            # an arbitrary JSON file such as ~/.codex-crew/config.json.
            if (
            ...
            # Drop a legacy SYMLINK from an older Codex Crew (which pointed at a file
            ...
            try:
                ...
                # (@codexcrew-cron / @codexcrew-core) in `tools`. Those specs live in
...
def _register_skills(app_name: str, manifest: AppManifest, app_root: Path) -> list[str]:
    """Symlink app skill directories into ~/.codex-crew/skills/.
    ...
    for skill_path_str in manifest.skills:
        ...
        # Namespaced link: ~/.codex-crew/skills/{app_name}/{skill_name}
        ...
        # Flat link: ~/.codex-crew/skills/{skill_name} (for skill scanner)
...
def _deregister_skills(app_name: str) -> int:
    """Remove the app's skill symlinks from ~/.codex-crew/skills/.
...
async def register_app_crons_with_service(app_name: str, cron_service: Any) -> list[str]:
    ...
    from codex_crew.mcp_cron import _vet_script_file, _vet_shell_command
...
# App MCP servers are written to CodexCrew's OWN agent config, not to the shared
...
# HTTP entry there breaks EVERY kiro session, see backend.py's warning). CodexCrew
...
def _mcp_json_path() -> Path:
    """CodexCrew's own agent config. A function, not an import-time constant:
    ...
    return _kiro_agents_dir() / "codexcrew.json"
...
def _mcp_lock(*, exclusive: bool = True, target: Optional[Path] = None) -> Iterator[None]:
    ...
    WHICH file's sidecar to lock (default: CodexCrew's own agent config); pass the
...
def _resolve_live_mcp_url(app_name: str, url: str, live_port: int | None = None) -> str:
    ...
    try:
        if live_port is None:
            ...
            from codex_crew.apps.backend import get_app_backend_port
...
def _health_reconcile_guard() -> Any:
    ...
    """
    from codex_crew.apps.backend import health_reconcile_lock
...
#: one guaranteed to import ``codex_crew``.
...
def resolve_stdio_command(cfg: dict, app_root: Path | None = None) -> dict:
    ...
    ``codex_crew`` at all. ``mcp_gateway/rewriter.py`` bakes ``sys.executable`` into its stub
    ...
    (see :mod:`codex_crew.apps.interpreter`): prefer the app's own venv interpreter — that is
    ...
    if base in _BARE_PYTHON:
        if _targets_gateway_module(cfg):
            # The server runs Codex Crew's OWN code (``-m codex_crew...``). App
            # venvs are created WITHOUT --system-site-packages, so codex_crew is
            # not importable there and the venv interpreter would die on
            # import; and even a venv that pip-installed its own codex_crew is a
...
def _targets_gateway_module(cfg: dict) -> bool:
    """True when a python stdio entry launches a ``codex_crew``-owned module.
    ...
    - a non-dash token: the script operand — everything after belongs to the
      SCRIPT (``python3 server.py -m codex_crew.mode``), never to CPython.
    ...
    (``-sm codex_crew``) reads here as an unknown flag followed by an operand
        ...
        if arg == "-m":
            if i + 1 < len(args):
                ...
                return isinstance(module, str) and (
                    module == "codex_crew" or module.startswith("codex_crew.")
                )
            return False
        if arg.startswith("-m") and len(arg) > 2:
            module = arg[2:]
            return module == "codex_crew" or module.startswith("codex_crew.")
...
def _register_mcp_servers(
    ...
    """Register app-provided MCP servers into CodexCrew's agent config.
...
def _scrub_legacy_shared_mcp(app_name: str) -> int:
    ...
    try:
        ...
        # CodexCrew process), and an unlocked stale read-modify-write here would
        ...
        # codexcrew.json), so pass the legacy path explicitly.
