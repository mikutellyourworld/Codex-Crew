...
from codex_crew.apps.execution import shipped_builtin_app_root
...
class LifecycleDispatcher:
    ...
    def _resolve_hook(app_name: str, hook_path: str):
        ...
        if shipped_root is not None:
            ...
            mod = importlib.import_module(f"codex_crew.apps.builtins.{shipped_root.name}.{dotted}")
    ...
    async def _invoke(self, app_name: str, hook_path: str, ctx: AppContext, *, phase: str) -> bool:
        ...
        try:
            ...
            # module object (`_codexcrew_app_<app>.<mod>`), so an on_startup hook
