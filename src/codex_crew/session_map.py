...
from codex_crew.config.paths import config_dir, kiro_sessions_dir
...
from codex_crew.sel import _infer_source, sel

logger = logging.getLogger(__name__)

# Public because another instance's map is read by file path, not through this
# class: :func:`codex_crew.session_storage.cotenant_sids` opens the map belonging
...
def _normalize_unbind_reason(reason: str) -> str:
    ...
    The runtime guard behind :data:`~codex_crew.messaging.link.UNBIND_REASONS`. An
...
class SessionMap:
    ...
    Stored as ``~/.codex-crew/session_map.json``. Atomic write via tmp+rename.
    ...
    def mapped_sids_by_key(self) -> dict[str, str]:
        ...
        A session ID present here is one Codex Crew can still resume. Callers that
    ...
    def _mirror_key(self, key: str) -> str:
        ...
        (:func:`~codex_crew.messaging.link.legacy_dashboard_mirror_key`); when
    ...
    def channel_key_for_stem(self, stem: str) -> str:
        ...
        ``discord_codexcrew_direct_123`` there is no way to tell which
