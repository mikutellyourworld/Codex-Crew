...
State is persisted to ``~/.codex-crew/autonudge.json`` (fcntl-locked, atomic
...
Feature-flagged via env ``CODEXCREW_AUTONUDGE`` (on by default; set to ``0`` to disable).
"""
...
from codex_crew import irq, platform_compat, probes, shutdown_event
from codex_crew.atomic_write import replace_with_retry
from codex_crew.config.loader import config_dir, data_home
from codex_crew.config.paths import legacy_home
from codex_crew.constants import MAX_BANNER_CHARS
from codex_crew.monitoring.decision import decide_monitor, monitor_budget_reason
...
from codex_crew.probes import targets
from codex_crew.security import is_sensitive_path, redact_credentials, redact_exfiltration_urls

if TYPE_CHECKING:
    from codex_crew.monitoring.github_pull_request import GitHubPullRequestProbeResult
...
#    ``codex_crew.messaging.link`` runs ``messaging/__init__``, which pulls the
...
#    ``codex_crew`` modules to obtain one tuple of string literals.
...
def enabled() -> bool:
    """Feature flag — on by default. Set ``CODEXCREW_AUTONUDGE=0`` to disable."""
    return os.environ.get("CODEXCREW_AUTONUDGE", "1").lower() not in ("0", "false", "no")
...
def repair_sentinel_path(raw: str) -> str:
    ...
    That store survives the one-time ``~/.codexcrew`` → ``~/.codex-crew`` data-home
    ...
    1. **Pass through a path already under the CURRENT home.** Checked FIRST,
       because ``CODEXCREW_HOME`` may legally point *inside* the legacy root
       (e.g. ``~/.codexcrew/dev``). Such a path is lexically under
       ``~/.codexcrew`` yet already live and correct; re-homing it would produce
       ``~/.codexcrew/dev/dev/workspace/…``, persist that, and — since the
    ...
    2. **Re-home a STRANDED legacy-rooted path.** A path under ``~/.codexcrew``
       ...
       Skipped when the current home IS the legacy home (``CODEXCREW_HOME``
       ...
       test, so an unnormalized value like ``~/.codexcrew/../workspace/STOP``
    ...
    try:
        ...
        if norm_candidate.is_relative_to(norm_current):
            ...
            # CODEXCREW_HOME inside the legacy root) — nothing to re-home.
            pass
            ...
            # gone. A path under ``~/.codexcrew`` is not necessarily a migration
            # casualty: ``workspaces.<name>.dir`` may legitimately be configured
            # as an absolute path inside that tree (and the legacy root can
            # survive the migration as debris, which `codexcrew doctor` reports).
...
class AutoNudgeService:
    ...
    async def start(self) -> None:
        if not enabled():
            logger.info("AutoNudge disabled (CODEXCREW_AUTONUDGE not set)")
            return
