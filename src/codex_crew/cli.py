"""Codex Crew CLI — personal AI agent.

Commands:
    codexcrew chat -m "message"    Send a single message
    codexcrew chat                 Interactive chat mode
    codexcrew gateway              Start the Codex Crew server (dashboard + messaging channels)
    codexcrew gateway --seed NAME  Populate $CODEXCREW_HOME from fixture NAME, then start the gateway
    codexcrew status               Show runtime stats
    codexcrew run TASK.md          Run an autonomous task from a spec file
    codexcrew update               Update Codex Crew via git fetch + rebuild
    codexcrew cron list|add|remove Manage scheduled jobs
    codexcrew spawn run "task"     Spawn a background subagent
    codexcrew spawn list           List subagents
    codexcrew learn add|list|remove Save and manage learned corrections
    codexcrew setup                Interactive setup wizard
    codexcrew doctor               Verify setup
"""

from __future__ import annotations

# Ensure SSL certs are found before any library caches its SSL context.
# The ``codexcrew`` entry-point (console_scripts) bypasses ``__main__.py``,
# so we must run this here as well.
from codex_crew._ssl_compat import _ensure_ssl_certs
...
from codex_crew import __version__, cli_help, platform_compat
from codex_crew.apps import builtins as _builtins_pkg
from codex_crew.apps.builtins import BUILTIN_NAMES as _BUILTIN_NAMES
from codex_crew.config import CodexCrewConfig, config_dir, ensure_data_home
from codex_crew.config.loader import (
    DASHBOARD_PORT,
    build_provider_factory,
)
from codex_crew.config.paths import _default_home, _legacy_home
from codex_crew.constants import BANNER, MIN_NODE_MAJOR, env_flag_enabled
from codex_crew.crash_guard import install as _install_crash_guard
from codex_crew.env import git_build_info
from codex_crew.gateway_lock import GatewayLock, GatewayLockError
from codex_crew.history import ConversationLog, HistoryConsolidator
from codex_crew.knowledge.dedup import dedup_sweep
from codex_crew.knowledge.store import KnowledgeStore
from codex_crew.log_redaction import install_log_redaction
from codex_crew.memory import MemoryStore
from codex_crew.platform import (
    PlatformCompositionError,
    boot_platform,
    current_context,
)
from codex_crew.preflight import run_preflight_checks
from codex_crew.seed import seed_cmd
...
from codex_crew.skills import SkillsLoader

logger = logging.getLogger(__name__)

# Markers that uniquely identify the CodexCrew repo root for project-dir
# auto-detection. ``skills/`` + ``src/codex_crew/`` is the stable signature:
# ``skills/`` is editable-at-root and ``src/codex_crew/`` pins this to the
# CodexCrew package repo (not just any directory that happens to contain a
# ``skills/`` folder).
_PROJECT_MARKERS = ("skills", "src/codex_crew")
...
_JAILED_ENV_MARKER = "CODEXCREW_JAILED"
...
def _child_argv() -> "list[str]":
    """The argv the jail should re-exec — this same codexcrew invocation.

    Reuses ``agent._resolve_codexcrew_bin`` (the same self-invocation resolver
    codexcrew-core/codexcrew-cron use) so the jailed child inherits its venv
    ``bin/`` walk and ``os.access(X_OK)`` validation — rather than
    re-implementing a bare ``shutil.which`` that misses those cases.
    The resolver returns the bare ``"codexcrew"`` sentinel when it finds no usable
    binary; in that case fall back to ``python -m codex_crew`` so a non-PATH /
    editable run still re-execs correctly.
    """
    from codex_crew.agent import _resolve_codexcrew_bin

    exe = _resolve_codexcrew_bin()
    if exe != "codexcrew":  # a resolved, validated absolute path
        return [exe, *sys.argv[1:]]
    return [sys.executable, "-m", "codex_crew", *sys.argv[1:]]
...
def _refuse_unjailed(command: str, reason: str) -> NoReturn:
    ...
    """
    logging.getLogger("codex_crew").error(
...
def _jail_reexec_gate(command: str, no_jail_flag: bool) -> None:
    ...
    * Already inside the jail (``CODEXCREW_JAILED`` marker set by a prior re-exec)
    ...
    * ``off`` (``--no-jail`` or ``CODEXCREW_NO_JAIL`` truthy) → early return, no
    ...
    * With a backend present + ``mode != "off"``, a single fail-closed floor
    ...
    """
    from codex_crew.config.loader import _normalize_jail

    log = logging.getLogger("codex_crew")
    ...
    # ``off`` per-invocation: --no-jail OR the CODEXCREW_NO_JAIL env hatch (the
    ...
    if no_jail_flag or env_flag_enabled("CODEXCREW_NO_JAIL"):
...
def _project_dir_file() -> Path:
    """Return the path to the saved project_dir file, respecting CODEXCREW_HOME."""
...
def _ensure_node(proj_dir: str = "") -> bool:
    ...
    script = None
    env_dir = os.environ.get("CODEXCREW_PROJECT_DIR")
    for candidate in [
...
def _detect_project_dir() -> str | None:
    ...
    Search order:
    1. Walk up from CWD
    2. Read saved path from config_dir()/project_dir (respects CODEXCREW_HOME)
    """
...
def _install_child_watcher() -> None:
    ...
    no attribute 'set_child_watcher'`` and killed ``codexcrew gateway`` on
    ...
    ``codexcrew gateway`` path satisfies this (``asyncio.run`` on the main thread
    ...
    # raise AttributeError during `codexcrew gateway` startup. This is a true
...
def _resolve_gateway_args(args: argparse.Namespace) -> dict:
    ...
    if port is not None:
        if str(port).lower() == "auto":
            ...
            try:
                ...
                print(
                    f">_ --port must be an integer or 'auto', got {port!r}.",
                    file=sys.stderr,
                )
                sys.exit(2)
            if not 1 <= port_int <= 65535:
                print(
                    f">_ --port {port_int} out of range (1..65535).",
                    file=sys.stderr,
                )
                sys.exit(2)
            port = str(port_int)
    ...
    if approval == "yolo":
        home_env = os.environ.get("CODEXCREW_HOME", "")
        if not home_env:
            print(
                ">_ --approval yolo refused: CODEXCREW_HOME must be explicitly set "
                "to an isolated path (not the default ~/.codex-crew).",
                file=sys.stderr,
            )
            sys.exit(2)
        try:
            ...
            # config_dir() here: CODEXCREW_HOME is already set, so config_dir() would
            # return the override itself and the rail would always fire. We must
            # reject the legacy ~/.codexcrew too, not just ~/.codex-crew: on an
            # unmigrated or downgraded install the legacy home still holds the LIVE
            # data, so CODEXCREW_HOME=~/.codexcrew would otherwise enable unrestricted
            ...
            print(
                f">_ --approval yolo refused: failed to resolve CODEXCREW_HOME: {exc}",
                file=sys.stderr,
            )
            sys.exit(2)
        if home_resolved in protected_homes:
            print(
                ">_ --approval yolo refused: CODEXCREW_HOME resolves to a main "
                f"gateway home ({home_resolved}). Set CODEXCREW_HOME to an isolated "
...
def _diagnostic_port(gw_kwargs: dict) -> int | None:
    ...
    ``dashboard.url``, with ``CODEXCREW_PORT`` and then ``--port`` overriding it)
    ...
    try:
        ...
        from codex_crew.dashboard.urls import parse_dashboard_url

        return parse_dashboard_url(CodexCrewConfig.load().dashboard.url)[1]
    except Exception:
...
def _knowledge(args) -> None:
    """``codexcrew knowledge dedup [--apply]`` -- collapse cross-source duplicate docs."""
    if getattr(args, "knowledge_action", None) != "dedup":
        print("Usage: codexcrew knowledge dedup [--apply]")
        return
...
def _consolidate_cmd(args) -> None:
    ...
    print("\nDone. Check ~/.codex-crew/skills/auto/ for new skills.")
...
async def drain_log_queue_before_hard_exit(timeout: float = 2.0) -> None:
    ...
    :func:`~codex_crew.executors.subprocess_executor` -- the pool reserved for
    ...
    """
    from codex_crew.executors import subprocess_executor
...
def _setup_cli_logging(command: str | None, verbose: int) -> None:
    ...
    of every ``codex_crew`` record into the SAME file the file handler writes
    ...
    # Codex Crew loggers: --verbose CLI flag takes precedence, otherwise
    # fall back to the persistent log_level from config.
    if verbose == 0:
        try:
            _cfg = CodexCrewConfig.load()
    ...
    logging.getLogger("codex_crew").setLevel(level)
    ...
    # encoding="utf-8" is REQUIRED on Windows: Codex Crew logs non-ASCII glyphs and
    ...
    # at WARNING so third-party WARNINGs keep flowing even when codex_crew's
    # own configured level is stricter (codex_crew records below `level` are
    # already filtered at the codex_crew logger, so this cannot over-log).
    ...
    if command in _LONG_LIVED_COMMANDS:
        ...
        for lgr in (logging.getLogger(), logging.getLogger("codex_crew")):
    ...
    # Install secret redaction filter — scrubs Bearer tokens from all codex_crew
...
def _builtin_mcp_server_available(name: str) -> bool:
    """True when ``codex_crew.apps.builtins.<name>.mcp_server`` resolves.
...
def main() -> None:
    ...
    # On Windows, force stdout/stderr to UTF-8 BEFORE anything prints — CodexCrew's
    # non-ASCII output otherwise raises UnicodeEncodeError under the cp1252
    # default when stdout is a pipe (detached gateway, CodexCrewHub client). No-op
    # on POSIX. Must be the first statement: the CODEXCREW_PORT error below prints
    # non-ASCII glyphs.
    platform_compat.ensure_utf8_console()

    # Clear any INHERITED sandbox-active marker before any argv-wrapping path
    # can run. CODEXCREW_SANDBOX_ACTIVE tells the sandbox layer that OS isolation
    ...
    # CODEXCREW_SANDBOX_LEVEL gets the same treatment: a stale inherited value
    # would be read as the ACTIVE tier by a descendant's passthrough and
    # corrupt its downgrade audit.
    os.environ.pop("CODEXCREW_SANDBOX_ACTIVE", None)
    os.environ.pop("CODEXCREW_SANDBOX_LEVEL", None)

    # Validate CODEXCREW_PORT early — fail fast before anything else loads.
    # Range as well as type: an in-range check that lives only in the binder
    # would let `CODEXCREW_PORT=70000 codexcrew service install` bake an
    ...
    _raw_port = os.environ.get("CODEXCREW_PORT")
    if _raw_port is not None:
        try:
            _port_val = int(_raw_port)
        except ValueError:
            print(
                f"❌ CODEXCREW_PORT={_raw_port!r} is not a valid integer.\n"
                f"   Unset it or provide a numeric port (e.g. CODEXCREW_PORT=6777).",
                file=sys.stderr,
            )
            sys.exit(1)
        if not 1 <= _port_val <= 65535:
            print(
                f"❌ CODEXCREW_PORT={_raw_port!r} is outside the valid port range 1-65535.\n"
                f"   Unset it or provide a bindable port (e.g. CODEXCREW_PORT=6777).",
                file=sys.stderr,
            )
            sys.exit(1)

    if not os.environ.get("CODEXCREW_PROJECT_DIR"):
        detected = _detect_project_dir()
        if detected:
            os.environ["CODEXCREW_PROJECT_DIR"] = detected

    parser = argparse.ArgumentParser(
        prog="codexcrew",
        description="Codex Crew — personal AI agent",
        usage=cli_help.TOP_USAGE,
        epilog=cli_help.render_epilog(),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("--version", action="version", version=f"codexcrew {__version__}")
    parser.add_argument(
        "--verbose",
        "-v",
        action="count",
    ...
    # parser, so BOTH ``codexcrew --no-jail <cmd>`` and ``codexcrew <cmd> --no-jail``
    ...
    # every ``codexcrew <cmd> --help`` with the whole top-level usage line.
    ...
    chat_parser = cli_help.add_command(
        ...
        epilog="""
Examples:
  codexcrew chat                      # Interactive mode
  codexcrew chat -m 'check my CRs'    # Single message
...
""",
    ...
    gw_parser.add_argument(
        ...
        metavar="FIXTURE",
        help=(
            "Seed $CODEXCREW_HOME from the named fixture BEFORE starting the "
            "gateway (dev tool). Fixture must exist under "
            "src/codex_crew/tests_fixtures/. The gateway then runs normally "
            "against the populated $CODEXCREW_HOME. Refuses when "
            "$CODEXCREW_HOME is the main gateway home (~/.codex-crew) or "
    ...
    gw_parser.add_argument(
        ...
        action="store_true",
        help=(
            "When used with --seed, wipe $CODEXCREW_HOME (rmtree) before "
            "copying the fixture. Ignored without --seed. Does NOT "
            "override the main-gateway-home rail — ~/.codex-crew is refused "
            "regardless."
        ),
    )
    gw_parser.add_argument(
    ...
    gw_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Print a single line `CODEXCREW_READY:{...}` to stdout once the "
            "dashboard is bound. Payload includes port, token, pid, and "
            "CODEXCREW_HOME. Used by test harnesses to discover the bound "
    ...
    gw_parser.add_argument(
        ...
        help=(
            ...
            "auto-approves all tools (refused unless CODEXCREW_HOME is "
    ...
    setup_parser.add_argument(
        "--electron-only",
        action="store_true",
        help="Only install the Codex Crew desktop app (macOS), skip other setup",
    )
    setup_parser.add_argument(
        "--clean",
        action="store_true",
...
Examples:
  codexcrew cron list
  codexcrew cron add 'daily-status' 'show status' --every 86400
  codexcrew cron add 'weekday-9am' 'check tickets' --cron '0 9 * * MON-FRI' --approval-mode auto
  codexcrew cron add 'c360-check' 'check pipeline' --every 600 --agent customer360-code-agent
  codexcrew cron update <job-id> --approval-mode auto
  codexcrew cron update <job-id> --agent oncall-agent
  codexcrew cron remove <job-id>
""",
        formatter_class=_fmt,
    )
    ...
    cron_add.add_argument(
        ...
        "Empty or omitted uses the default codexcrew agent.",
    )
    cron_add.add_argument(
        "--silent",
        action="store_true",
    ...
    cron_update.add_argument(
        ...
        dest="agent",
        default=None,
        help="New agent name (empty string resets to default codexcrew agent)",
    )
    cron_update.add_argument(
        "--approval-mode",
        dest="approval_mode",
    ...
    adopt_target.add_argument(
        ...
        'session key ("dashboard:chat-3-1712793600"); see `codexcrew cron list`',
    )
    adopt_target.add_argument(
        "--release",
        action="store_true",
    ...
    cron_preview.add_argument(
        "script", help="Script path in module:function format (e.g. ~/.codex-crew/crons/my.py:run)"
    )
    ...
    spawn_parser = cli_help.add_command(
        ...
        epilog="""
Examples:
  codexcrew spawn run 'check my open CRs'        # Wait for result
  codexcrew spawn run --async 'analyze logs'     # Fire-and-forget
  codexcrew spawn list                           # Show active subagents
""",
        formatter_class=_fmt,
    )
    ...
    run_parser = cli_help.add_command(
        ...
        epilog="""
Examples:
  codexcrew run TASK.md                  # Run task with auto-resume
  codexcrew run TASK.md --fresh          # Start from scratch
  codexcrew run TASK.md --no-test        # Skip test verification
  codexcrew run TASK.md --timeout 3600   # 1 hour timeout
""",
        formatter_class=_fmt,
        parents=[_jail_opts],
    )
    ...
    eval_parser = cli_help.add_command(
        ...
        epilog="""
Examples:
  codexcrew eval                         # smoke test (~30s)
  codexcrew eval memory_recall_basic     # specific scenario
  codexcrew eval --all                   # all scenarios (slow)
""",
        formatter_class=_fmt,
        parents=[_jail_opts],
    )
    ...
    pod_up.add_argument(
        "--approval",
        # Literal mirrors codex_crew.pod.runtime.APPROVAL_MODES, which is the
        # enforcement point; this parser imports no pod module at startup.
        choices=["reads", "yolo", "interactive"],
        help=(
            "Approval mode the pod's gateway boots with, forwarded to "
            "`codexcrew gateway --approval`. Persisted per pod so it survives a "
    ...
    pod_api = pod_sub.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  codexcrew pod api my-wt GET sessions\n"
            "  codexcrew pod api my-wt GET /api/health\n"
            '  codexcrew pod api my-wt POST config --data \'{"key":"agent.model"}\' '
            "--allow-write\n"
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    ...
    pod_exec = pod_sub.add_parser(
        "exec",
        help="Run a codexcrew command against a pod, using the pod's own binary and data",
    )
    ...
    stop_parser.add_argument(
        ...
        default=None,
        help=(
            "Dashboard port (default: resolved from CODEXCREW_PORT env or "
    ...
    sbx_parser = cli_help.add_command(
        ...
        epilog="""
Examples:
  codexcrew sandbox status                      # is THIS launch covered?
  codexcrew sandbox install-profile             # attach to $APPIMAGE (sudo)
  codexcrew sandbox install-profile --path P    # attach to an explicit executable
  codexcrew sandbox remove-profile              # unload and delete it (sudo)
...
""",
    ...
    # cloud — provision + run CodexCrew on the user's own AWS EC2 (bring-your-own
    # AWS; credentials resolved by the aws CLI, never stored by CodexCrew).
    cloud_parser = cli_help.add_command(
        sub,
        "cloud",
        epilog="""
Examples:
  codexcrew cloud launch                  # interactive: provision + configure + open dashboard
  codexcrew cloud launch --size power     # non-interactive size
  codexcrew cloud launch --new            # create a separate new instance
  codexcrew cloud launch --subnet subnet-0abc…  # pin the launch to an exact subnet
  codexcrew cloud list                    # list your cloud instances
  codexcrew cloud connect                 # reopen the dashboard over SSM
  codexcrew cloud stop | start            # pause / resume (save cost)
  codexcrew cloud destroy                 # remove EVERYTHING from AWS
  codexcrew cloud iam-policy              # print the least-privilege IAM policy
  codexcrew cloud doctor                  # check prerequisites + AWS reachability
""",
        formatter_class=_fmt,
    )
    ...
    _c_list = cloud_sub.add_parser("list", help="List your Codex Crew cloud instances")
    ...
    logout_parser.add_argument(
        ...
        type=int,
        default=None,
        help="Dashboard port (default: resolved from CODEXCREW_PORT env or dashboard.url config)",
    )
    ...
    token_parser.add_argument(
        ...
        help=(
            ...
            "multi-instance embed (the embedding gateway's CODEXCREW_PORT). Loopback "
    ...
    # mcp-consult (opt-in external-model second-opinion tool).
    sub.add_parser("mcp-consult")
    ...
    # them in the ``codexcrew-computer`` MCP server instead.
    computer_parser = cli_help.add_command(
        sub,
        "computer",
        epilog="""
Examples:
  codexcrew computer doctor                     # Support + permission report
  codexcrew computer doctor --json              # The same report as JSON
  codexcrew computer apps                       # Apps with an on-screen window
...
""",
    ...
    learn_parser = cli_help.add_command(
        ...
        epilog="""
Examples:
  codexcrew learn list
  codexcrew learn add 'use snake_case for variables' --category tool
  codexcrew learn remove 'snake_case'
""",
        formatter_class=_fmt,
    )
    ...
    art_parser = cli_help.add_command(
        ...
        epilog="""
Examples:
  codexcrew artifact list
  codexcrew artifact list --tag op --kind widget
  codexcrew artifact save --name "CR Queue" --content-file widget.html --tags ops,cr
  cat widget.html | codexcrew artifact save --name "Pipeline Health"
  codexcrew artifact show cr-queue
  codexcrew artifact show cr-queue --version 2
  codexcrew artifact show cr-queue --meta
  codexcrew artifact update cr-queue --content-file widget.html
  codexcrew artifact versions cr-queue
  codexcrew artifact delete cr-queue
""",
        formatter_class=_fmt,
    )
    ...
    agent_sub.add_parser("list", help="List Codex Crew agents")
    agent_create = agent_sub.add_parser("create", help="Create a Codex Crew agent")
    agent_create.add_argument("--name", required=True, help="Agent name")
    agent_create.add_argument("--kiro-agent", default="codexcrew", help="Kiro agent name")
    ...
    agent_update = agent_sub.add_parser("update", help="Update a Codex Crew agent")
    ...
    agent_delete = agent_sub.add_parser("delete", help="Delete a Codex Crew agent")
    ...
    agent_reset_model.add_argument(
        ...
        default="codexcrew",
        help="Kiro agent spec to reset (default: codexcrew)",
    )
    ...
    ws_create.add_argument(
        ...
        default=None,
        help=(
            "Workspace directory NAME, relative to the CodexCrew data home "
    ...
    ws_update.add_argument(
        "--dir",
        help=(
            "New workspace directory NAME, relative to the CodexCrew data home. "
    ...
    app_parser = cli_help.add_command(
        ...
        epilog="""
Examples:
  codexcrew app install /path/to/oncall-watchtower
  codexcrew app list
  codexcrew app enable oncall-watchtower
  codexcrew app disable oncall-watchtower
  codexcrew app info oncall-watchtower
  codexcrew app uninstall oncall-watchtower
""",
        formatter_class=_fmt,
    )
...
Examples:
  ...
  codexcrew config get agent.provider    # Get a specific value
  codexcrew config set dashboard.url http://localhost:5486
  codexcrew config edit                  # Open in $EDITOR
  codexcrew config defaults              # Stored values holding a superseded default
  codexcrew config defaults --adopt      # Take the current defaults for all of them
  codexcrew config defaults --keep session.autocompact_pct   # Affirm one as intentional

The dashboard port is set with the CODEXCREW_PORT env var, not a config key.
""",
        formatter_class=_fmt,
    )
    ...
    if args.command in _JAILED_COMMANDS:
        from codex_crew.kiro_prerequisite import register_process_start_override_attestation
    ...
    # ``gateway --seed <fixture>`` populates $CODEXCREW_HOME from a hand-authored
    ...
    # ``CodexCrewConfig.load()`` and the file-log handler attach at line ~603):
    # both of those call ``config_dir()`` which ``mkdir``s $CODEXCREW_HOME, which
    ...
    # the gateway against a half-seeded or wrong-state $CODEXCREW_HOME would be
    ...
    # asyncio loop. The legacy→~/.codex-crew migration blocks (copytree + os.walk
    ...
    # an empty target) and before CodexCrewConfig.load()/the log handler, both of
    # which call config_dir(). Skipped for the seed path above, which set up its
    # own $CODEXCREW_HOME (an override → migration is a no-op there anyway).
    ensure_data_home()
    ...
    # No subcommand given (`codexcrew` with no args) — show banner + help and exit.
    ...
    # services.  Standalone (no companion, no CODEXCREW_PROFILE) composes the
    ...
    try:
        boot_platform(CodexCrewConfig.load())
        ...
        if isinstance(exc, PlatformCompositionError):
            if args.command == "doctor":
                logging.getLogger("codex_crew").debug(
            ...
            # config.
            logging.getLogger("codex_crew").debug("platform boot deferred", exc_info=True)
    ...
    if args.command == "chat":
        ...
        from codex_crew.cli_server import _gateway
        from codex_crew.dashboard.state import set_build_info

        # Resolve the running build's git branch+commit ONCE here in the sync
        # entrypoint: provably AFTER CODEXCREW_PROJECT_DIR detection (top of main())
        ...
        # CODEXCREW_HOME so two ConversationLog writers can never clobber the same
        # session file. Held for the process lifetime; released by the kernel on
        # death (POSIX record lock — see codex_crew.gateway_lock). The port is
        ...
        from codex_crew.cli_server import _run_task

        asyncio.run(_run_task(args))
    elif args.command == "learn":
        from codex_crew.cli_commands import _learn
        ...
        from codex_crew.cli_commands import _memory_cmd

        _memory_cmd(args)
    elif args.command == "mcp-cron":
        from codex_crew.mcp_cron import run_mcp_server as run_mcp_cron_server

        run_mcp_cron_server()
    elif args.command == "mcp-core":
        from codex_crew.mcp_core import run_mcp_core_server

        run_mcp_core_server()
    elif args.command == "mcp-computer":
        from codex_crew.mcp_computer import run_mcp_server as run_mcp_computer_server
        ...
        # below: `codexcrew gateway` boots through this module, and a default-off
        # optional subsystem must not be imported to start it.
        importlib.import_module("codex_crew.mcp_dashboard").run_mcp_server()
    elif args.command == "mcp-consult":
        importlib.import_module("codex_crew.mcp_consult").run_mcp_server()
        ...
        # the `codexcrew app mcp <name>` manifest path uses (clean stderr line +
        ...
        from codex_crew.cli_commands import _run_app_mcp_server
        ...
        from codex_crew.computer_use.cli import run_computer

        run_computer(getattr(args, "computer_args", []))
    elif args.command == "eval":
        from codex_crew.cli_commands import _run_eval

        asyncio.run(_run_eval(args))
    elif args.command == "security":
        from codex_crew.cli_commands import _security

        _security(args)
    elif args.command == "tailnet":
        from codex_crew.cli_commands import _tailnet

        _tailnet(args)
    elif args.command == "telemetry":
        from codex_crew.cli_commands import _telemetry

        _telemetry(args)
    elif args.command == "policy":
        from codex_crew.cli_commands import _policy
        ...
        # function-local `from codex_crew.cli_commands import …`: the latter trips
        ...
        importlib.import_module("codex_crew.cli_commands")._handle_secrets(args)
    elif args.command == "pod":
        from codex_crew.cli_commands import _pod

        _pod(args)
    elif args.command == "update":
        if getattr(args, "action", None) == "approve":
            from codex_crew.cli_server import _update_approve
        ...
        from codex_crew.cli_server import _service_cmd

        sys.exit(_service_cmd(args))
    elif args.command == "sandbox":
        from codex_crew.cli_server import _sandbox_cmd

        sys.exit(_sandbox_cmd(args))
    elif args.command == "cloud":
        sys.exit(handle_cloud(args))
    elif args.command == "logs":
        from codex_crew.cli_server import _logs_cmd
        ...
        from codex_crew.cli_server import _logout, resolve_client_port
        ...
        from codex_crew.snapshot import snapshot_main

        rc = snapshot_main(parsed=args)
        if rc:
            raise SystemExit(rc)
    elif args.command == "restore":
        from codex_crew.snapshot import restore_main

        rc = restore_main(parsed=args)
        if rc:
            raise SystemExit(rc)
    elif args.command == "agent":
        from codex_crew.cli_commands import _handle_agent

        _handle_agent(args)
    elif args.command == "workspace":
        from codex_crew.cli_commands import _handle_workspace

        _handle_workspace(args)
    elif args.command == "app":
        from codex_crew.cli_commands import _handle_app
...
# (``codexcrew mcp-core`` / ``mcp-cron`` / ``mcp-computer``) — which dispatch
...
from codex_crew.cli_bench import bench_cmd, register_bench_parser  # noqa: E402
from codex_crew.cli_chat import _run_chat  # noqa: E402
from codex_crew.cli_cloud import add_size_choices as _cloud_size_choices  # noqa: E402
from codex_crew.cli_cloud import handle_cloud  # noqa: E402
from codex_crew.cli_config import _config_cmd  # noqa: E402
from codex_crew.cli_desktop import desktop_cmd, register_desktop_parser  # noqa: E402
from codex_crew.cli_doctor import _doctor  # noqa: E402
from codex_crew.cli_perf import perf_cmd, register_perf_parser  # noqa: E402
from codex_crew.cli_setup import (  # noqa: E402, F401
    _fix_shell_profiles,
    _manifest,
    _setup,
)
