...
SERVICE_NAME = "codexcrew"  # systemd unit name (without .service)
LAUNCHD_LABEL = "dev.codexcrew.gateway"  # launchd Label
...
def launchd_live_program() -> "os.PathLike[str]":
    ...
    """
    return (
        Path.home() / "Library" / "Application Support" / "CodexCrew"
        / "live-gateway"
    )


def codexcrew_bin() -> str:
    """Return the resolved codexcrew executable path, or fall back to sys.argv[0].

    Used by both the systemd unit and the launchd plist as ``ExecStart`` /
    ``ProgramArguments``. Resolution order:

    1. ``CODEXCREW_SERVICE_BIN`` if set — an explicit operator override. This
       ...
       rather than silently falling back to whatever ``codexcrew`` happens to be
    ...
    2. ``shutil.which("codexcrew")`` — the installed console script.
    3. ``sys.argv[0]`` — for development installs where ``codexcrew`` isn't on
       the global PATH.
    """
    override = os.environ.get("CODEXCREW_SERVICE_BIN", "").strip()
    if override:
...
def service_environment(home: str) -> "dict[str, str]":
    ...
    * ``CODEXCREW_KIRO_BIN`` — propagated only when the installer already has it
    ...
    env = {
        ...
        # ``codexcrew doctor`` and regenerated with ``codexcrew service install``.
        "CODEXCREW_SERVICE_MANAGED": "1",
    }
    kiro_bin = os.environ.get("CODEXCREW_KIRO_BIN", "").strip()
    if kiro_bin:
        env["CODEXCREW_KIRO_BIN"] = os.path.abspath(kiro_bin)
    # CODEXCREW_PORT is the ONLY input DASHBOARD_PORT reads, so a service that
    ...
    # same way CODEXCREW_KIRO_BIN is:
    # captured from the installer's environment, so
    # `CODEXCREW_PORT=5477 codexcrew service install` bakes 5477 into the unit.
    #
    # No validation here on purpose. `cli.py`'s main() already rejects a
    # CODEXCREW_PORT that is not an integer in 1-65535 before any subcommand
    ...
    port = os.environ.get("CODEXCREW_PORT", "").strip()
    if port:
        env["CODEXCREW_PORT"] = port
    return env
...
def headless_auth_warning(environ: "Mapping[str, str] | None" = None) -> str:
    ...
    ``codexcrew service install`` is not there when the service starts: the probe
    ...
    misconfiguration for a durable disclosure. ``~/.codex-crew/.env`` is the
    ...
    if _home_override_is_set(environ):
        lines.append("")
        lines.append(
            "   CODEXCREW_HOME is set here but is also not inherited, so confirm"
        )
...
def _home_override_is_set(environ: "Mapping[str, str] | None" = None) -> bool:
    ...
    source = os.environ if environ is None else environ
    return bool(source.get("CODEXCREW_HOME", "").strip())
...
def service_path(home: str) -> str:
    ...
    interactive shell that ran ``codexcrew service install``. Common
...
def restart_command_hint() -> str:
    ...
    * ``SYSTEMD`` — the unit is **system-level** at
      ``/etc/systemd/system/codexcrew.service`` (see
      :mod:`codex_crew.service.linux`). ``systemctl --user`` fails on AL2
    ...
    * ``LAUNCHD`` / ``UNSUPPORTED`` — defer to the service-aware
      ``codexcrew restart`` CLI, which resolves the right mechanism itself.
