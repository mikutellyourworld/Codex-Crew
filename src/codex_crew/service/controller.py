...
importing :mod:`codex_crew.service.linux` or :mod:`codex_crew.service.macos`
...
from codex_crew.service import linux, macos
...
def installed_unit_path() -> "Path | None":
    ...
    environment: a host running ``codexcrew gateway`` in the foreground has none,
...
def installed_service_has_managed_marker() -> "bool | None":
    ...
    try:
        if plat == Platform.SYSTEMD:
            expected = 'Environment="CODEXCREW_SERVICE_MANAGED=1"'
        ...
        if plat == Platform.LAUNCHD:
            ...
            # inspection path instead of crashing `codexcrew doctor`.
            payload = macos._plist_payload(path)
            if payload is None:
                return False
            environment = payload.get("EnvironmentVariables")
            return (
                isinstance(environment, dict) and environment.get("CODEXCREW_SERVICE_MANAGED") == "1"
            )
...
def _unsupported_message() -> None:
    print(
        "❌ codexcrew service management is only supported on Linux (systemd)\n"
        "   and macOS (launchd). On other platforms run `codexcrew gateway`\n"
...
def install_service() -> int:
    ...
    ``/etc/systemd/system/codexcrew.service`` and to run
    ``systemctl daemon-reload / enable / restart``. The gateway itself
    runs as ``User=$USER`` once started — codexcrew code is never
    ...
    if plat == Platform.SYSTEMD:
        try:
            ...
            return 1
        print("✅ codexcrew service installed and started.")
        ...
        print()
        print("   Status: codexcrew service status")
        print("   Logs:   codexcrew logs -f")
        print("   Remove: codexcrew service uninstall")
        return 0
    if plat == Platform.LAUNCHD:
        try:
            macos.install()
...
def uninstall_service() -> int:
    ...
    if plat == Platform.SYSTEMD:
        ...
        try:
            ...
            print(f"❌ {exc}", file=sys.stderr)
            return 1
        print("✅ codexcrew service stopped and removed.")
        if profile.message:
            print(f"   {'' if profile.ok else '⚠️ '}{profile.message}")
        return 0
    if plat == Platform.LAUNCHD:
...
def is_service_active() -> bool:
    """Return True if a codexcrew service is installed and currently running."""
    plat = current_platform()
    if plat == Platform.SYSTEMD:
...
def manual_restart_hint() -> str:
    ...
    not have. Unlike :func:`codex_crew.service.common.restart_command_hint`,
    this must never answer ``codexcrew restart``: that is the command that just
    failed, so a circular hint would send the operator straight back into the
    same refusal.
    """
    plat = current_platform()
    if plat == Platform.SYSTEMD:
        # "sudo systemctl restart codexcrew" — shared with the update path and
