...
* a systemd **system** unit (what ``codexcrew service install`` writes on Linux)
...
checkout the process ``execve``s into that checkout's own ``codexcrew``, moving
...
EXEC_MARKER = "CODEXCREW_LIVE_EXECED"
...
def pointer_path() -> Path:
    ...
    # ``codex_crew.config.loader.config_dir`` patch seam still governs where the
...
def target_bin(checkout: Path) -> Path:
    """The ``codexcrew`` a live target is executed through.
    ...
    if sys.platform == "win32":
        return checkout / ".venv" / "Scripts" / "codexcrew.exe"
    return checkout / ".venv" / "bin" / "codexcrew"
...
def validate(raw: str) -> Path:
    ...
    if not (checkout / "src" / "codex_crew").is_dir():
        return _reject(
            f"the live target does not look like a Codex Crew checkout "
            f"(no src/codex_crew): {checkout}"
        )
    return checkout
...
def maybe_reexec(argv: list[str], *, log: object = None) -> None:
    ...
    env = {
        **os.environ,
        ...
        "CODEXCREW_PROJECT_DIR": str(target),
        # The target's venv leads PATH. Without this, a bare ``codexcrew`` in a
    ...
    try:
        ...
        # inside a Codex Crew checkout. argv is this process's own argv, and env is
...
def _warn(log: object, message: str) -> None:
    ...
    if callable(warn):
        ...
        return
    print(f"codexcrew: live-target: {message}", file=sys.stderr)
