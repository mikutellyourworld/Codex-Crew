...
profile carrying no ``CAP_SYS_ADMIN``. CodexCrew's Linux sandbox needs a user
...
``ch-run``, ``QtWebEngineProcess``, ``1password``, ``Discord``). CodexCrew was the
outlier.
...
1. *Attach to the interpreter.* ``~/.codex-crew-venv/bin/python3`` is a *symlink*
...
chain (``~/.local/bin/codexcrew`` → ``~/.codex-crew-venv/bin/codexcrew``, itself a
...
script (e.g. ``~/.codex-crew-venv/bin/codexcrew``) is a stable, per-install,
non-symlinked path — narrower in scope than the shared interpreter, since only
the codexcrew entry point gets ``userns``, not every uv-managed tool sharing that
...
# fake `apparmor_parser` executed as root by `codexcrew service install` — a local
...
# Codex Crew's Linux sandbox isolates the agent by entering a user namespace and
...
# blocks that second step, so Codex Crew fail-closes and refuses to run the agent
...
def render_profile(abi: str | None, exec_path: Path) -> str:
    ...
    """
    comment = f"""# Managed by Codex Crew — regenerated by `codexcrew service install`.
#
...
# `codexcrew service install` re-renders it against the current path on every
...
#
{_FLAGS_COMMENT}# Removing this file re-breaks Codex Crew's sandbox on this host: the gateway
# will refuse to spawn the agent instead of running it without isolation.
# `codexcrew service uninstall` removes it for you.
"""
...
def validate(parser: str, text: str) -> tuple[bool, str]:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", suffix=".aa", prefix="codexcrew_profile_", delete=False, encoding="utf-8"
        ) as handle:
            handle.write(text)
            tmp = handle.name
        proc = subprocess.run(
...
# import codex_crew: this runs under sudo, and importing our own package would
...
def verify_enforcement(
    ...
    2. **It must NOT run anything user-writable as root.** So the interpreter is a
       ...
       import codex_crew — otherwise ``sudo`` would execute user-controlled
       site-packages.
...
def install(
    ...
    return ProfileOutcome(
        ...
        "codexcrew launcher path (the service's ExecStart, and any launch that "
...
LAUNCHER_PROFILE_NAME = "codexcrew-launcher"
...
def default_exec_path() -> str | None:
    ...
    Returning None is the correct answer for a foreground ``codexcrew gateway`` in
    ...
    the machine. That case is served by ``codexcrew service install`` instead.
    """
...
def _substitutable_by_others(resolved: Path, expected_uid: int | None = None) -> str | None:
    ...
    * **Owned by the expected user.** This is the rule that makes the whole check
    ...
    case: an unprivileged user runs ``codexcrew sandbox install-profile`` on their
    own account). The systemd service case (#3463) is different: ``codexcrew
...
def validate_exec_path(raw: str, expected_uid: int | None = None) -> tuple[Path | None, str]:
    ...
    ``codexcrew_bin()``). ``expected_uid`` is forwarded to
    ...
    if _SHARED_INTERPRETER_RE.match(text):
        return (
            ...
            "use `codexcrew service install`, which now attaches to the installed "
...
def render_launcher_profile(abi: str | None, exec_path: Path) -> str:
    """Render the attached profile for a directly launched Codex Crew.
    ...
    """
    comment = f"""# Managed by Codex Crew — regenerated by `codexcrew sandbox install-profile`.
#
...
# spawns.
#
# The attachment below is a grant keyed on a path, so `codexcrew sandbox
...
# profile from matching; `codexcrew sandbox status` reports that, and re-running
# `codexcrew sandbox install-profile` re-points it.
#
{_FLAGS_COMMENT}# Removing this file re-breaks Codex Crew's sandbox for this app: the gateway
# will refuse to spawn the agent instead of running it without isolation.
# `codexcrew sandbox remove-profile` removes it for you.
"""
...
def installed_attachment(
    ...
    Defaults to the launcher profile (``codexcrew sandbox status``'s original
...
def install_launcher(
    ...
    """Attach the userns profile to a directly launched Codex Crew. NEVER raises.
    ...
    if not target:
        return ProfileOutcome(
            ...
            "--path with the app's executable, or run `codexcrew service install` "
...
def launcher_status(exec_path: str | None = None) -> tuple[bool, str]:
    ...
    if attached is None:
        if not target:
            return (
                ...
                "is not an AppImage launch. Use `codexcrew service install` to "
                "confine the gateway as a systemd unit.",
            )
        return (
            False,
            f"no launcher profile is installed, so {target} runs unconfined and the "
            "agent sandbox cannot be built. Run `codexcrew sandbox install-profile`.",
        )
    if not target:
    ...
    if attached != current:
        return (
            ...
            + ". Re-run `codexcrew sandbox install-profile` to re-point it.",
        )
