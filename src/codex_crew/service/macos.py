...
The plist lives at ``~/Library/LaunchAgents/dev.codexcrew.gateway.plist``
...
LOG_DIR = Path.home() / "Library" / "Logs" / "CodexCrew"
STDOUT_LOG = LOG_DIR / "gateway.log"
STDERR_LOG = LOG_DIR / "gateway.err"
#: See :func:`codex_crew.service.common.launchd_live_program` for why the agent
...
def _write_plist_atomic(contents: str) -> None:
    ...
    Delegates to :func:`codex_crew.atomic_write.atomic_write`: it writes a
...
def render_live_program(target_bin: str, *, working_dir: str | None = None,
    ...
    that re-invokes ``codexcrew`` by name, resolves to the checkout that is now
    ...
    lines = [
        "#!/bin/sh",
        "# Generated by CodexCrew (service install / Dev Fleet make-live).",
...
def write_live_program(contents: str, path: Path | None = None) -> None:
    ...
    *path* defaults to :data:`LIVE_PROGRAM`. Dev Fleet's backend passes the path
    ...
    :func:`codex_crew.atomic_write.atomic_write` applies *mode* to the
...
def _repairer_bin() -> str:
    ...
    Resolution is deliberately narrow: an explicit ``CODEXCREW_SERVICE_BIN``, else
    the ``codexcrew`` console script sitting beside the running interpreter. There
    is no ``PATH`` fallback, because ``PATH`` cannot answer the question being
    asked — "which install is running" — and an unrelated or older ``codexcrew``
    ...
    if override:
        resolved = codexcrew_bin()
        if not _is_executable_file(Path(resolved)):
            raise OSError(
                f"refusing to restore the launchd launcher: CODEXCREW_SERVICE_BIN "
                f"resolves to {resolved!r}, which is not an executable file."
            )
        return resolved
    sibling = Path(sys.executable).parent / (
        "codexcrew.exe" if os.name == "nt" else "codexcrew"
    )
    if _is_executable_file(sibling):
        return str(sibling)
    raise OSError(
        "refusing to restore the launchd launcher: no codexcrew console script "
        f"beside the running interpreter ({sys.executable}). PATH is not "
        "consulted, because an unrelated codexcrew ahead of this install on PATH "
        "would be persisted into the agent. Set CODEXCREW_SERVICE_BIN to the "
...
def ensure_live_program() -> bool:
    ...
    ``EnvironmentVariables`` (a non-default ``CODEXCREW_PORT``, for instance), so
...
def install() -> None:
    ...
    write_live_program(render_live_program(codexcrew_bin()))
...
def status() -> str:
    ...
    res = _launchctl("list", LAUNCHD_LABEL)
    if res.returncode != 0:
        return f"codexcrew service is not loaded ({res.stderr.strip() or 'no entry'})\n"
    return res.stdout
