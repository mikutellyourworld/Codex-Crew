...
On an SELinux-enforcing host where codexcrew is installed under ``$HOME`` — the
...
**Mechanism-based, never distro-based** — the same rule :mod:`codex_crew.service
...
wrongly fire on a Fedora host that installed codexcrew system-wide, or one whose
...
that file goes on to exec at RUNTIME. A ``CODEXCREW_SERVICE_BIN`` override naming a
...
:func:`codex_crew.service.linux.install` covers the residue at the point of failure
...
def _file_context(path: str) -> str | None:
    ...
    (``~/.local/bin/codexcrew`` → ``~/.codex-crew-venv/bin/codexcrew``). Reading the
