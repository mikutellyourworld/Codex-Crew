...
The unit lives at ``/etc/systemd/system/codexcrew.service`` and is
...
anything the installer writes: an SELinux-enforcing host whose codexcrew lives
...
so the unit fails every start with ``203/EXEC`` (#7165). :mod:`codex_crew.service
...
``sudo codexcrew service install`` it reuids to 0, which is a no-op, and only on
...
no ``codex_crew``, so no MCP / LLM / agent code is reached deliberately.
...
from codex_crew.service import apparmor, selinux
from codex_crew.service.common import (
    SERVICE_NAME,
    codexcrew_bin,
    service_environment,
)
from codex_crew.service.common import systemd_quote as _sd_quote
...
# ``Environment=CODEXCREW_PORT`` was frozen at `service install` time, so before
# this file there was no supported way to change it on a running unit.
ENV_DIR = Path("/etc/codexcrew")
ENV_FILE_PATH = ENV_DIR / "codexcrew.env"
...
_ENV_FILE_TEMPLATE = """\
# Codex Crew service environment overrides.
#
# This file is read by the systemd unit (EnvironmentFile=) AFTER the values
# baked in at `codexcrew service install` time, so anything set here WINS. Edit
# it, then apply without reinstalling:
#
#     sudo systemctl restart codexcrew
#
# Bind a non-default dashboard port (e.g. to run a second crew beside the
# default 5486, or when 5486 is already taken):
#CODEXCREW_PORT=5477
"""
...
def _current_user() -> str:
    ...
    invoking human, never root, so ``sudo codexcrew service install`` must target
...
def _home_for_user(user: str) -> str:
    ...
    ``sudo -H codexcrew service install`` the process's ``HOME`` is ``/root``
...
def render_unit(*, user_scope: bool = False) -> str:
    ...
    has access to ``$HOME/.codex-crew``, the user's config, etc. The PATH
    ...
    return (
        "[Unit]\n"
        "Description=Codex Crew gateway (dashboard + Slack + cron)\n"
        "Documentation=https://github.com/mikutellyourworld/Codex-Crew\n"
        ...
        # CODEXCREW_PORT) without a re-install. The leading "-" makes a missing
        # file non-fatal, so the unit still starts where install could not write
        # /etc/codexcrew (the baked Environment= values then apply).
...
def _write_unit_via_sudo(contents: str) -> subprocess.CompletedProcess[str]:
    ...
    ``daemon-reload``.
    """
    fd, tmp_path = tempfile.mkstemp(prefix="codexcrew-unit-", suffix=".service")
    try:
...
def _install_file_via_sudo(contents: str, dest: Path, mode: str = "0644") -> None:
    ...
    Same escalation path as the unit file — no second mechanism, and no codexcrew
    or LLM-influenced code runs under sudo; only ``install`` is invoked.
    """
    fd, tmp_path = tempfile.mkstemp(prefix="codexcrew-", suffix=".tmp")
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as fh:
            fh.write(contents)
...
def _seed_env_file() -> None:
    ...
    ``Path.exists()``: a pre-existing root-owned ``/etc/codexcrew`` with a
...
def _user_scope_remedy() -> str:
    ...
    **Every path and account is spelled out, and none is taken from the pasting
    ...
    in :mod:`codex_crew.service.common`: these lines are copy-pasted verbatim, and
    ...
    return (
        ...
        f"\n"
        f"     mkdir -p {unit_dir}\n"
        f"     cat > {unit_file} <<'CODEXCREW_UNIT'\n"
        f"{unit_body}"
        f"CODEXCREW_UNIT\n"
        ...
        f"   `journalctl --user -u {SERVICE_NAME} -f`. `codexcrew service "
        f"status|uninstall`\n"
...
def selinux_refusal(reason: str) -> str:
    ...
    return (
        ...
        f"\n"
        f"   Installing codexcrew outside $HOME (onto a system-labelled path such\n"
...
def selinux_start_failure_hint() -> str:
    ...
    follow what that file execs at runtime, so a ``CODEXCREW_SERVICE_BIN`` override
    ...
    :mod:`codex_crew.service.selinux`), name SELinux here, where the unit has
    ...
    return (
        ...
        f"   the pre-flight found no proven denial for {codexcrew_bin()}, but it only\n"
        ...
        f"   docs/guides/install.md, \"SELinux-enforcing hosts with codexcrew under\n"
...
def install() -> apparmor.ProfileOutcome:
    ...
    No codexcrew / LLM / agent code runs under sudo deliberately — see the module
    ...
    if user == "root":
        raise ServiceInstallError(
            ...
            "as that user (e.g. via their login, or `sudo -u <user> codexcrew "
            "service install`), or set $USER to a non-root account."
        )

    # Last gate before anything is written: on an SELinux-enforcing host whose
    # codexcrew lives under $HOME, PID 1's domain is denied execute on the binary
    ...
    blocked, selinux_reason = selinux.blocks_system_unit(codexcrew_bin())
    if blocked:
    ...
    # `CODEXCREW_PORT=...` edit + restart works without re-installing.
    _seed_env_file()
...
def install_apparmor_profile(expected_uid: int | None) -> apparmor.ProfileOutcome:
    ...
    Attaches the profile to ``codexcrew_bin()`` — the same resolved path
    ...
    if expected_uid is None:
        return apparmor.ProfileOutcome(
            ...
            "codexcrew service install`), which would accept a root-owned shared "
            "launcher and hand the grant to every account on this host. The "
            "service was installed without the profile; re-run `codexcrew "
...
def install_launcher_profile(exec_path: str | None = None) -> apparmor.ProfileOutcome:
    ...
    ``aa-exec`` running under sudo. No codexcrew or LLM-influenced code does.
...
def uninstall() -> None:
    ...
    # operator-authored or -edited /etc/codexcrew/codexcrew.env (including one
    ...
    # dir, so it never removes anything else parked under /etc/codexcrew.
...
def status() -> str:
    ...
    ``codexcrew service status`` doesn't prompt for a password just to
