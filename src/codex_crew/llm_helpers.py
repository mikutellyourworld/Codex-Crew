...
from codex_crew.acp.client import AcpError, AcpPromptBusy, advertised_model_ids
from codex_crew.acp.types import EVENT_STEER_CONSUMED, TurnUsage
from codex_crew.config.loader import CodexCrewConfig
from codex_crew.hooks import fire_tool_hooks, get_global_hook_store
...
# tried instead of surfacing the error. This lives entirely on the Codex Crew
# side (kiro-cli
...
def configured_fallback_chain() -> tuple[str, ...]:
    ...
    ``CodexCrewConfig.load()`` is fingerprint-cached (mtime/size/mode of both
    ...
    """
    try:
        fm = CodexCrewConfig.load().agent.fallback_model
    except Exception:
        return ()
    if not fm:
        return ()
...
# command. Tracked in https://github.com/mikutellyourworld/Codex-Crew/issues/8053.
#
    ...
    try:
        ...
        try:
            try:
                ...
                from codex_crew.dashboard.handlers.usage import persist_token_record_async
...
def _provider_label(provider: Any) -> str:
    ...
    """
    try:
        from codex_crew.acp.types import PROVIDER_LABEL_DEFAULT
...
async def background_turn(
    ...
    *,
    ...
    """
    from codex_crew.session import BACKGROUND_AGENT, BACKGROUND_KEY  # circular import
...
def save_conversation_turn(
    ...
    ``/codexcrew sessions`` displays the correct agent name.
    """
    log.append(
        key,
        "user",
...
async def save_conversation_turn_off_loop(
    ...
    raises :class:`~codex_crew.history.HistoryLockTimeout` on any concurrent
    ...
    Unlike :func:`~codex_crew.history.append_off_loop`, this **awaits** the write
    ...
    The whole turn is written under one :meth:`~codex_crew.history.ConversationLog.atomic_appends`
