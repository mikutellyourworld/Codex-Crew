...
from codex_crew.config.superseded_defaults import (
    acked_superseded,
    coerced_value_drift,
    coercion_summary,
    drift_summary,
    ...
    if action == "get":
        ...
        if file_path:
            ...
            if not key or value is None:
                print("Usage: codexcrew config set <key> <value>", file=sys.stderr)
                print("       codexcrew config set --local <key> <value>", file=sys.stderr)
                print("       codexcrew config set --file <path.json>", file=sys.stderr)
                sys.exit(1)
            ...
            if use_local:
                top_key = key.split(".")[0]
                _known_sections = {f.name for f in dataclasses.fields(CodexCrewConfig)}
        ...
        if not p.exists():
            cfg = CodexCrewConfig()
            cfg.save()
            print(f">_ Created default config: {p}")
        sel().log_api_access(
            caller="cli",
        ...
        print("Usage: codexcrew config {get,set,edit,defaults}", file=sys.stderr)
        sys.exit(1)
...
def _defaults_cmd(args: argparse.Namespace) -> None:
    ...
    if wanted:
        ...
        if unknown:
            ...
            print(
                "   Run 'codexcrew config defaults' with no arguments to see the list.",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if coerced or any(e.dotted_key not in acked for e in drifted):
        print("Take the current defaults:  codexcrew config defaults --adopt")
        print("Affirm your values:         codexcrew config defaults --keep")
