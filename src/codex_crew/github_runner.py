...
This module MUST NOT import ``codex_crew.dashboard.*`` (at module scope or
lazily): it exists to dissolve that cross-layer dependency. Heavy runtime deps
(``codex_crew.sel``, ``codex_crew.apps.registry``, ``codex_crew.config.loader``)
are imported lazily inside functions so Code Review Sage's standalone import
path (``sage_lib`` without the Codex Crew runtime) keeps working for callers
...
from codex_crew import platform_compat, windows_acl
...
STRICT_PROVIDER_BIN_ENV = "CODEXCREW_PROVIDER_BIN_STRICT"
...
PROVIDER_EXECUTABLE_DIRS = (
    "/usr/local/libexec/codexcrew",
    "/usr/libexec/codexcrew",
    "/opt/homebrew/bin",
    "/usr/local/bin",
    "/usr/bin",
...
# its own caller-specific override (CODEXCREW_ISSUE_RADAR_GH / CODEXCREW_SAGE_GH).
GH_BIN_ENV = "CODEXCREW_GH_BIN"
...
GH_PREVALIDATED_ENV = "_CODEXCREW_GH_PREVALIDATED"
...
def agent_writable_roots() -> tuple[Path, ...]:
    ...
    """
    raw_roots = [os.environ.get("CODEXCREW_PROJECT_DIR")]
    try:
        from codex_crew.config.loader import workspace_root
...
def check_provider_path_component_windows(
    ...
    information on Windows (see :mod:`codex_crew.windows_acl`).
...
def validate_provider_executable(candidate: str) -> str:
    ...
    * anything inside the agent's project checkout or workspace root, the one
    ...
    Set ``CODEXCREW_PROVIDER_BIN_STRICT=1`` on shared or multi-tenant hosts to
    ...
    there (see :mod:`codex_crew.windows_acl`). An **elevated** gateway is refused
...
# CODEXCREW_GH_BIN value) so a changed override never serves a stale binary
...
def resolve_gh(*, override_env: str = "", cache: bool = True) -> str:
    ...
    generic ``CODEXCREW_GH_BIN``, then :func:`provider_executable_candidates`
...
def _audit_run(
    ...
    try:
        from codex_crew.sel import sel  # lazy: heavy runtime dep, see module docstring
