...
from codex_crew.messaging.conversation import ConversationState
...
from codex_crew.teams.approvals import TeamsApprovalDecider
from codex_crew.teams.attachments import append_attachment_context, process_teams_attachments
from codex_crew.teams.cards import (
    DECISION_APPROVE,
    DECISION_DENY,
    DECISION_TRUST,
    KIND_APPROVAL,
...
from codex_crew.teams.client import TeamsSendError
from codex_crew.teams.commands import (
    DIRECTIVE_USAGE,
    HELP_TEXT,
    command_argument,
    parse_command,
...
from codex_crew.teams.renderer import TeamsRenderer
from codex_crew.teams.session_resume import (
    ResumeReleaseError,
    RoutingDecision,
    TeamsSessionResume,
)
from codex_crew.teams.transport import TEAMS_CAPABILITIES
...
if TYPE_CHECKING:
    ...
    from codex_crew.teams.client import TeamsClient, TeamsInbound

logger = logging.getLogger(__name__)

# Canonical kiro-cli agent fallback so Teams sessions load codexcrew-core
...
# Telegram / WeCom / Webex paths' _DEFAULT_CODEXCREW_AGENT.
...
class TeamsDispatcher:
    ...
    async def _handle_dashboard(self, inbound: "TeamsInbound", arg: str) -> None:
        ...
        ``/codexcrew dashboard 2h`` cannot disagree about durations.
        """
        ...
        try:
            ...
            if not origin:
                ...
                # parse_dashboard_url resolves with the CODEXCREW_PORT override.
    ...
    def _bind_origin_mirror(self, session_key: str, inbound: "TeamsInbound") -> None:
        ...
        :func:`~codex_crew.messaging.link.bind_origin_mirror`; this only supplies
