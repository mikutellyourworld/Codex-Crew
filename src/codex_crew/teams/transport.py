...
from codex_crew.teams.client import TEAMS_MAX_TEXT, TeamsClient, TeamsInbound, TeamsSendError
        ...
        for identity in sorted(self._allowed):
            ...
            targets.append(
                ConfiguredChannelTarget(
                    ...
                    available=available,
                    unavailable_reason=(
                        "" if available else "Send a message to Codex Crew in Teams first"
                    ),
                )
            )
        return targets
