"""Single-writer guard for a ``CODEXCREW_HOME``.

Two ``codexcrew gateway`` processes bound to the same home each open the same
...
:mod:`codex_crew.kiro_prerequisite`.
...
Isolated homes (``--test-mode``/``--seed`` with a distinct ``CODEXCREW_HOME``)
...
class GatewayLockError(RuntimeError):
    """Raised when another process already owns this ``CODEXCREW_HOME``."""
    ...
    def __init__(self, home: Path, holder_pid: int | None, diagnosis: str | None = None) -> None:
        ...
        super().__init__(f"{detail}; stop it first or set CODEXCREW_HOME to an isolated directory")


class GatewayLock:
    """Process-lifetime exclusive lock on a single ``CODEXCREW_HOME``.
    ...
    matching the single-host scope of ``CODEXCREW_HOME``.
    ...
    *port* is diagnostic only. When given, a refusal reports whether the holder
    ...
    def _diagnose(self, recorded_pid: int | None) -> tuple[int | None, str | None]:
        ...
        return recorded_pid, (
            ...
            "Stop the running gateway, or set CODEXCREW_HOME to an isolated directory."
        )
    ...
    def _describe_live_owner(self, pid: int, recorded_pid: int | None) -> str:
        ...
        message = (
            ...
            f"{self._home}; stop it first (codexcrew stop) or set CODEXCREW_HOME to an "
            "isolated directory"
        )
    ...
    def _describe_orphaned_lock(self, dead_owner: int, openers: list[int] | None) -> str:
        ...
        if orphaned and not serving:
            ...
            lines.append(
                ...
                "gateway, not a wedged leftover -- stop it with codexcrew stop instead."
            )
        else:
...
def _port_answers_http(port: int, timeout: float = 1.5) -> bool:
    ...
    mirrors :func:`codex_crew.dashboard.port_reclaim._probe_gateway_healthy` in a
