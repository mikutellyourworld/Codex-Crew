...
tips runtime, and so ``codex_crew.platform.interfaces`` can name the type under
``TYPE_CHECKING`` without pulling the runtime into the platform package.

``codex_crew.tips`` re-exports both names, so ``tips.CatalogEntry`` keeps
...
class CatalogEntry:
    ...
    The public build derives these from ``codex_crew/docs/*.md`` (H1 + first
