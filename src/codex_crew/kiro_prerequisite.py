...
The public CodexCrew provider is KiroACP-only, so a healthy, authenticated
...
Codex Crew neither installs nor authenticates on the user's behalf, so there is no
...
repair write (which touches Codex Crew's own files, never the CLI).
"""
...
from codex_crew import hooks, identity_stores, platform_compat
from codex_crew._sqlite_compat import sqlite3
from codex_crew.agent_files import AGENT_FILENAME
from codex_crew.atomic_write import atomic_write
from codex_crew.config.loader import CRED_KIRO_API_KEY, read_env_file_credential
from codex_crew.config.paths import config_dir
from codex_crew.kiro_cli import (
...
# above, which Codex Crew only ever NAMES for the user to run, this one IS run on
...
# when the installed CLI is too old to expose the ``acp`` subcommand Codex Crew
...
# The subcommand Codex Crew launches every ACP session through (see
...
FAKE_ACP_TEST_MODE_ENV = "CODEXCREW_FAKE_ACP_TEST_MODE"
...
class PrerequisiteStatus:
    ...
    # (the user installs it from OFFICIAL_INSTALL_DOCS_URL, which Codex Crew has no
    ...
    # What the user runs to sign in. Codex Crew never runs it for them.
    ...
    # Whether the installed CLI exposes the ``acp`` subcommand every Codex Crew
    ...
    # ``login_command`` (which Codex Crew only names), this one is also run FOR the
    ...
    # Codex Crew's own agent specs (~/.kiro/agents/codexcrew*.json). ``ready``
    ...
    # Codex Crew's own specs that are PRESENT but which kiro-cli refuses to load.
    ...
    # ``--agent codexcrew`` resolves to the default agent with only a line on
    # stderr — every Codex Crew MCP server silently absent from the session. That
...
class TrustedAcpExecutableSnapshot:
    ...
    CodexCrew never copies the CLI and runs the copy, because a multi-call Kiro
...
def _acp_executable_is_runnable(
    ...
    *,
    ...
    (CodexCrew is not the authority on where Kiro CLI is installed).
...
def snapshot_trusted_acp_executable(
    ...
    *,
    ...
    CodexCrew always executes the Kiro CLI the user installed, at the path it was
    ...
    # ``CODEXCREW_KIRO_BIN=./kiro-cli`` resolves against the GATEWAY's cwd, but ACP
...
def register_process_start_override_attestation(
    *,
    ...
    override = active_environ.get("CODEXCREW_KIRO_BIN", "")
...
def _established_installation(data_home: Path) -> bool:
    """Recognize an existing Codex Crew home when migrating onto the setup marker."""
...
def _default_spec_lister() -> list[tuple[str, Path]]:
    ...
    """
    from codex_crew.agent import present_required_agent_specs  # circular import
...
class KiroPrerequisiteService:
    ...
    def __init__(
        ...
        *,
        ...
        if data_home is not None:
            ...
            configured_home = self._environ.get("CODEXCREW_HOME", "")
            self._data_home = (
        ...
        # Codex Crew's own secret home is always hidden from a probed CLI. The
    ...
    async def _agent_spec_overlay(self, result: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            logger.debug("Could not check Codex Crew agent specs", exc_info=True)
            return result
        ...
        async with self._repair_lock:
            ...
            if not error and (result.get("missing_agent_specs") or []):
                error = (
                    ...
                    "Run `codexcrew setup --agent-only --clean` on the gateway host."
                )
    ...
    async def _probe_spec_acceptance(self, executable: str) -> tuple[list[str], str]:
        """Ask kiro-cli whether it accepts each required Codex Crew spec on disk.
        ...
        try:
            ...
            logger.debug("Could not enumerate Codex Crew agent specs", exc_info=True)
            return [], ""
        rejected: list[str] = []
        detail = ""
    ...
    async def _probe_acp_support(self, executable: str) -> bool:
        """Ask kiro-cli whether it exposes the ``acp`` subcommand Codex Crew uses.
        ...
        Codex Crew launches every session as ``kiro-cli acp ...`` (see
    ...
    async def _run_auth_command(
        ...
        *,
        ...
        do not gate sign-in); CodexCrew is not the authority on where Kiro CLI is
        ...
        if not isolate_home:
            ...
            # session already reads. CodexCrew stages nothing and publishes
            # nothing. Only Codex Crew's own secret home is hidden, and the user's
