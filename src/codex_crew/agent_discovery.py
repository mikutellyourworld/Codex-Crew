...
agents, including CodexCrew's own agent and any agents shipped by
...
from codex_crew.agent_files import AGENT_FILENAME, LITE_AGENT_FILENAME
from codex_crew.config.paths import kiro_agents_dir, project_agents_dir, project_kiro_dir
from codex_crew.executors import discovery_executor
from codex_crew.hooks import FileTooLargeError, safe_read_file_bytes
...
# ``--agent`` against ``$PWD/.kiro/agents`` before ``~/.kiro/agents``. Codex Crew
...
class AgentInfo:
    ...
    source: str = "builtin"  # "codexcrew" | "package" | "builtin"
...
# Codex Crew-only spec convention, predating ``.kiro/agents/`` and still used by
...
def _read_agent_spec(
    ...
    *,
    ...
    :func:`codex_crew.hooks.safe_read_file_bytes` — the hardened gate every other
...
def list_agents(
    ...
    and is spawned by Codex Crew with the session's project dir as that cwd. The
    ...
    # before the user-level dir, and Codex Crew spawns it with the session's project
