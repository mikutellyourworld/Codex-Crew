...
Every request Codex Crew makes to its own gateway carries ``X-Internal-Secret``,
...
environments -- a request to ``http://127.0.0.1:5486`` is sent to the proxy in
...
``socket``) and nothing from ``codex_crew``. That is what lets the cheapest
callers use it -- ``cron_trigger`` imports nothing from ``codex_crew`` at all,
