"""CLI doctor subcommand — verify CodexCrew setup and diagnose issues."""
...
from codex_crew import __version__ as _mc_version
from codex_crew import agent as _agent
from codex_crew import agent_state, dep_sync, diagnostics, platform_compat, sandbox, stt
from codex_crew._bootstrap import _source_checkout_root
from codex_crew.acp.client import KIRO_CLI_BIN
from codex_crew.acp.kas_transport import (
    KAS_RELAY_ENGINE,
    KAS_RELAY_ENGINE_FLAG,
    build_kas_argv,
)
from codex_crew.acp.types import ACP_BACKEND_KAS
from codex_crew.agent import AGENT_FILENAME
from codex_crew.agent_discovery import (
    _read_agent_spec,
    project_agent_files,
    project_agent_name,
)
from codex_crew.agent_sdk.provider_identity import is_claude_code
...
from codex_crew.config.superseded_defaults import render_doctor_section
from codex_crew.constants import MIN_NODE_MAJOR
from codex_crew.cron import job_pause_state_from_disk, unhealthy_jobs_from_disk
from codex_crew.dashboard.crash_dump_store import (
    dump_age_seconds,
...
from codex_crew.deny_guidance import credential_vendor_server_ids
from codex_crew.discord import install_url, intent_probe
from codex_crew.doctor_deadpath import doctor_dead_paths
...
from codex_crew.extras import install_hint
from codex_crew.kiro_cli import mcp_governance_may_apply, resolve_kiro_cli
from codex_crew.mcp_cleanup import ALWAYS_ON_BIN_MCP_SERVERS as _ALWAYS_ON_MCPS
from codex_crew.mcp_cleanup import CODEXCREW_BIN_MCP_SERVERS as _MANAGED_MCPS
from codex_crew.mcp_cleanup import OPT_IN_BIN_MCP_SERVERS as _OPT_IN_MCPS
from codex_crew.mcp_discovery import McpServerInfo, probe_server
from codex_crew.model_registry import acp_id_correction
from codex_crew.platform import (
    PlatformCompositionError,
)
from codex_crew.platform import context as platform_context
from codex_crew.platform import (
    current_context,
    safe_context_call,
)
from codex_crew.platform.capability_bound import bind_capability_manager
from codex_crew.platform.defaults import DefaultCapabilityManager
from codex_crew.platform.governance import CU_MCP_SERVER, may_skip_gate_now
from codex_crew.sandbox import warm_backend
from codex_crew.security import is_sensitive_path
from codex_crew.sel import sel
from codex_crew.service import apparmor
from codex_crew.service import common as common_service
from codex_crew.service import controller as service_controller
from codex_crew.service import linux as service_linux
from codex_crew.session_pid_sig import signing_health
from codex_crew.stall_attribution import attribute_dump, describe
from codex_crew.subprocess_utf8 import UTF8_TEXT
from codex_crew.transcribe import _find_ffmpeg, availability_detail, ensure_ffmpeg_in_path
...
# attribute directly (``patch("codex_crew.cli_doctor.KIRO_AGENTS_DIR", tmp)``),
...
def _doctor_effective_model(cfg: CodexCrewConfig, project_dir: str, issues: list[str]) -> None:
    ...
    user's explicit call (``codexcrew agent reset-model``).
    """
    print("\nModel")
    try:
    ...
    # only after the global defers (tier 4). Reading codexcrew.json in both cases
    ...
    try:
        ...
        bound = bindings.kiro_agent or "codexcrew"
    ...
    if decided_by.startswith("bound agent pin"):
        ...
        pinned_agent, pinned_value = "codexcrew", default_model
    else:
    ...
    # upward walk, and Codex Crew's own resolver never reads that directory. So a
    # project-local spec can decide what actually RUNS while every Codex Crew
    ...
    if pinned_value:
        # pinned_agent is either the literal "codexcrew" or a configured kiro
        ...
        flag = "" if pinned_agent == "codexcrew" else f" --agent {_safe_display(pinned_agent)}"
        ...
        print(f"      Fix: codexcrew agent reset-model{flag}   (clears the pin, tracks the default)")
...
def _spec_gate_closed(name: str) -> bool:
    ...
    "missing from mcpServers (re-run `codexcrew setup`)" loop on every host
...
def _doctor_gated_off_mcps() -> frozenset[str]:
    ...
    "markers missing — re-run `codexcrew setup --agent-only`". Not reused from
...
def _doctor_mcp_tools(
    ...
    """Render the `MCP Tools` section of `codexcrew doctor`.

    Two passes scoped to the managed servers (`codexcrew-core`,
    `codexcrew-cron`, `codexcrew-computer`):
    ...
    if not isinstance(agent_data, dict):
        ...
        print("  ❌ agent spec is not a JSON object — re-run `codexcrew setup`")
        agent_data = {}
    ...
    for name in _MANAGED_MCPS:
        ...
        if name not in mcps:
            ...
            # means `codexcrew setup` did not finish.
            ...
            if gate_closed:
                ...
                continue
            print(f"  {ref}: ❌ missing from mcpServers (re-run `codexcrew setup`)")
        ...
        if gate_closed:
            ...
            print(
                ...
                "retracted on the next `codexcrew setup` or gateway start"
            )
        ...
        if not may_skip_gate_now(ref):
            ...
            # revoking would leave `codexcrew doctor` reporting a repaired config
...
def _doctor_mcp_governance(
    ...
    """Render the `MCP Governance` section of `codexcrew doctor`.
    ...
    access mode. Codex Crew's own handshake probe spawns each server directly and
    ...
    """
    try:
        declared = CodexCrewConfig.load().agent.mcp_registry_mode
    except Exception:
    ...
    if not governed_capable:
        ...
        print("      fix:  codexcrew config set agent.mcp_registry_mode false")
    ...
    if declared:
        ...
        if len(marked) < len(expected):
            print("  ❌ markers missing — re-run `codexcrew setup --agent-only`")
        ...
        print(
            ...
            "registry-governed — try `codexcrew config set agent.mcp_registry_mode false`"
        )
        return
    ...
    print("      Declare it:  codexcrew config set agent.mcp_registry_mode true")
...
# ``~/.codexcrew`` home, so a leftover legacy dir may still contain the running
...
def _doctor_data_home() -> None:
    """Report the data home and any leftover top-level ``~/.codexcrew`` directory.

    The data root is ``~/.codex-crew`` (or a valid ``CODEXCREW_HOME`` override). A
    leftover top-level ``~/.codexcrew`` is not the data home unless an override
    ...
    if override_home is not None:
        ...
        if points_at_legacy:
            ...
            print(
                ...
                f"(CODEXCREW_HOME override points to it)"
            )
            return
    ...
    if venvs:
        ...
        # interpreter with it (`which codexcrew` may resolve through it).
        print(
            f"  legacy:      ✅ {legacy} retained to hold a Codex Crew "
...
def _doctor_managed_service_policy(issues: list[str]) -> None:
    ...
    print("               Fix: run `codexcrew service install` once, then restart the service")
...
def _doctor_claude_backend() -> None:
    ...
    try:
        ...
        from codex_crew.agent_sdk import INSTALLED, MISSING, probe_backend
...
def _doctor_path_launcher() -> None:
    """Report which install the ``codexcrew`` command on PATH actually belongs to.

    A gateway never takes the name from another install's working launcher (see
    ``agent.ensure_codexcrew_on_path``), which is the right call — but it leaves a
    ...
    ``codexcrew`` can run a different install, at a different version or channel,
    ...
    on_path = shutil.which("codexcrew")
    if not on_path:
        ...
        print("  codexcrew CLI: ⏹ not on PATH (run `codexcrew setup` to link it)")
        return
    running = _resolve_codexcrew_bin()
    if not os.path.isabs(running) or os.path.realpath(on_path) == os.path.realpath(running):
        print(f"  codexcrew CLI: ✅ {on_path}")
        return
    print("  ⚠ codexcrew CLI on PATH belongs to a different install than this one.")
    ...
    _print_wrapped(
        "Both can coexist — the wheel keeps its own updates — but `codexcrew` in a "
        "terminal runs the one on PATH, which may be a different version or "
        "channel. Run `codexcrew setup` from the install you want to own the name."
    )
...
#: Mirrors ``mcp_core._STRICT_IDENTITY_SERVERS``; ``codexcrew-dashboard`` is
#: opt-in per agent, so it is reported only when an agent actually references it.
_STRICT_IDENTITY_SERVERS = ("codexcrew-core", "codexcrew-dashboard")


def _doctor_strict_identity(cfg: CodexCrewConfig) -> None:
    ...
    ``codexcrew doctor`` exit 1 on every stock install — the same failure the
    ...
    launcher exports ``CODEXCREW_HOST_PID``, so routing is not what decides
...
def _process_apparmor_confinement() -> str:
    ...
    ``codexcrew-userns (enforce)``. The per-LSM path is tried first; the bare
...
def _service_profile_applies(profile_path: Path, profile_name: str) -> bool:
    ...
    is now attached BY PATH to ``codexcrew_bin()`` (the same path ``ExecStart``
    ...
    ``codexcrew service install`` no longer writes it, and this check follows —
    ...
    re-pointed) makes this False until ``codexcrew service install`` re-renders
    the profile against the new path — the same staleness
    ``codexcrew sandbox status`` already reports for the launcher profile.
    """
    attached = apparmor.installed_attachment(profile_path, profile_name)
    if attached is None:
        return False
    try:
        current = str(Path(service_linux.codexcrew_bin()).resolve(strict=True))
    except OSError:
        return False
    if attached != current:
        return False
    ...
    * profile installed and attached to the resolved launcher script, and this
    ...
    if not apparmor.PROFILE_PATH.is_file():
        ...
        _print_wrapped(
            ...
            f"on this host can build the sandbox. Run `codexcrew service install` to "
    ...
    if confinement and confinement.split(" ")[0] == apparmor.PROFILE_NAME:
        ...
        _print_wrapped(
            ...
            f"`codexcrew service install` to re-render and reload the profile."
        )
    ...
    if not _service_profile_applies(apparmor.PROFILE_PATH, apparmor.PROFILE_NAME):
        ...
        _print_wrapped(
            ...
            f"attached to the codexcrew launcher script this host currently resolves — "
            ...
            f"way nothing on this host runs confined by it. Run `codexcrew service "
    ...
    _print_wrapped(
        ...
        f"the codexcrew launcher script this host resolves, but this process was not "
    ...
    try:
        launcher = str(Path(service_linux.codexcrew_bin()).resolve(strict=True))
    except OSError:
        launcher = service_linux.codexcrew_bin()
...
def _doctor_sandbox(issues: list[str]) -> None:
    ...
    if kind == "foreign_sandbox":
        ...
        _print_wrapped(
            "Codex Crew cannot nest its own sandbox inside it. Launch the gateway "
            "outside that sandbox to hand isolation back to Codex Crew's own profile."
        )
        return
...
def _doctor_pod_session_bus(issues: list[str]) -> None:
    ...
    unit (``codexcrew service install``) inherits no login-session environment,
    and if the per-user instance is not running at all there is nothing for
    CodexCrew to point at — every pod verb then fails with "Failed to connect to
    ...
    from codex_crew.pod.runtime import has_session_bus, session_bus_socket
    ...
    if not has_session_bus():
        ...
        print("               Pods are systemd --user units, so `codexcrew pod` is")
...
def _doctor_cron_health(issues: list[str]) -> None:
    ...
    if not auto_paused and not errored:
        ...
        if not loadable:
            ...
            print("               on startup.")
            print("               Fix: restore it from a snapshot (`codexcrew restore`)")
    ...
    if auto_paused:
        ...
        print("               Fix: `codexcrew cron resume <id>` once the cause is fixed.")
    ...
    if errored:
        ...
        print("               Fix: `codexcrew cron trigger <id>` to retry now. The recorded")
...
def _doctor_model_url_reachable(issues: list[str]) -> None:
    ...
    """
    del issues  # advisory-only diagnostic; keeps the call-site signature uniform
    from codex_crew.embeddings import redact_model_url  # circular-safe (no loader)
    ...
    try:
        ...
        print("               Fix: set CODEXCREW_EMBED_MODEL_URL (or memory.embed_model_url)")
...
#: links, the base `TipCard` uses), and `src/codex_crew/...` does not exist on a
...
_BLOCKED_COMMANDS_DOC_URL = (
    "https://github.com/mikutellyourworld/Codex-Crew/blob/main/src/codex_crew/docs/blocked-commands.md"
)
...
def _aws_profile_names() -> list[str] | None:
    ...
    ``codexcrew doctor`` is reachable from a tool call — so opening it here would
    ...
    venv's ``bin``), and this runs when an OPERATOR types ``codexcrew doctor`` —
...
def _aws_auto_refreshes() -> bool:
    ...
    probe: this runs under an operator's ``codexcrew doctor``, and a ``PATH`` that
...
def _doctor_headless_auth(issues: list[str]) -> None:
    ...
    currently serving, so a stopped unit beside a foreground ``codexcrew gateway``
...
def _doctor_kas(issues: list[str]) -> None:
    ...
    :mod:`codex_crew.acp.kas_transport`), so the thing that makes a selected KAS
    ...
    if CodexCrewConfig.load().agent.acp_backend == ACP_BACKEND_KAS:
...
def _doctor_agents_janitor(issues: list[str], sweep_backups: bool) -> None:
    ...
    them. ``codexcrew doctor`` REPORTS what a sweep would reclaim but never
...
def _discord_intent_grants(token: str) -> intent_probe.IntentGrants:
    ...
    :func:`~codex_crew.discord.intent_probe.probe_intent_grants`; the guard here
...
def _discord_msg_content_line(
    ...
    if not needs_content:
        ...
        print(f"{_INDENT}then `codexcrew restart`.")
...
def _discord_unused_intent_line(label: str, name: str, state: str) -> None:
    """Flag a privileged intent nothing in Codex Crew reads, if it is granted.
...
def _doctor_discord(
    cfg: CodexCrewConfig, creds: dict[str, str], port: int | None, issues: list[str]
) -> None:
    ...
    if not dc.enabled:
        ...
        print(f"{_INDENT}{env_path()}, then `codexcrew restart`")
        return
    ...
    if token:
        ...
        print(f"{_INDENT}DISCORD_BOT_TOKEN=<token> to {env_path()}, then `codexcrew restart`")
    ...
    if users:
        ...
        print(f"{_INDENT}right-click your name → Copy User ID), then `codexcrew restart`")
...
def _doctor_whatsapp(cfg: CodexCrewConfig, issues: list[str]) -> None:
    ...
    # every `codexcrew` invocation, since cli.py imports this module at its own
    # module scope for all subcommands.
    from codex_crew.whatsapp.client import (
        MISSING_EXTRA_HINT,
        default_db_path,
        neonize_available,
    )
    ...
    if not wa.enabled:
        print("  status:      ⏭  not enabled (optional)")
        print("  setup:       run 'codexcrew setup --whatsapp', or enable it from")
    ...
    if store.exists():
        ...
        # failing here would break the documented `codexcrew doctor && codexcrew
...
def _doctor(platform_boot_error: "Exception | None" = None, bundle: bool = False) -> None:
    """Verify CodexCrew setup — check dependencies, config, credentials, connectivity.
    ...
    if bundle:
        ...
        try:
            ...
            print("     Check that ~/.codex-crew is writable and has free space.")
            sys.exit(1)
    ...
    if git:
        ...
        print("  git:         ❌ not found (needed for codexcrew update)")
        issues.append("git")
    ...
    if proj and Path(proj).is_dir():
        ...
        print("  project dir: ⚠️  not set (run codexcrew setup from project root)")
    ...
    if agent_path.exists():
        ...
        print("  config:      ❌ not found (run codexcrew setup)")
    ...
    # trivially. Walks EVERY spec in the agents dir (not just codexcrew.json),
    ...
    if is_venv_install:
        ...
        print(f"  codex_crew:   ✅ {_mc_version}")
        try:
    ...
    try:
        from codex_crew._sqlite_compat import fts5_available
    ...
    # Gated on the checkout markers themselves (setup.cfg + src/codex_crew, via
    ...
    if _load_llama_class() is not None:
        ...
        if _absent:
            ...
            print("               defect, not an unsupported platform) — reinstall Codex Crew")
    ...
    if ffmpeg_bin:
        ...
        if platform_compat.is_bundled_interpreter():
            print("               Fix: reinstall Codex Crew (the bundled audio decoder is missing)")
        else:
            print(
                "               Fix: "
                + _os_fix_hint(
    ...
    # and reporting it as a note would have `codexcrew doctor` exit 0 on a
    ...
    print("\nSlack Integration")
    if _has_slack:
        has_owner = bool(creds.get("CODEXCREW_OWNER_ID"))
        print("  tokens:      ✅ configured")
        if has_owner:
            print(f"  owner:       ✅ {creds['CODEXCREW_OWNER_ID']}")
        else:
            print("  owner:       ⚠️  CODEXCREW_OWNER_ID not set")
        ...
        print("  setup:       run 'codexcrew setup --slack', or connect any channel")
    ...
    for row in rows:
        ...
        if row.ready:
            ...
            print(
                ...
                "Settings > Channels, or in ~/.codex-crew/.env"
            )
    ...
    try:
        ...
        if _latest is not None:
            ...
            if _age_s < 7 * 86400:  # Less than 7 days old
                ...
                # reach past the asyncio plumbing into the Codex Crew frame
    ...
    if _port:
        ...
        if is_remote:
            ...
            print("     Then run: codexcrew token")
