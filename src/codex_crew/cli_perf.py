"""``codexcrew perf`` -- debug-only performance sampling commands.

Thin CLI layer: argument handling, gate enforcement, process resolution and
output. The sampling itself lives in :mod:`codex_crew.perf_sampler`.

Two shapes, because they answer different questions and have different
requirements:

* ``codexcrew perf sample --pid <PID>`` (default: the running gateway) attaches
...
* ``codexcrew perf sample --call <module:callable>`` runs that callable in this
...
from codex_crew import cli_help, platform_compat
from codex_crew.atomic_write import atomic_write
from codex_crew.config.loader import config_dir
from codex_crew.gateway_lock import LOCK_FILENAME
from codex_crew.perf_sampler import (
...
def _read_gateway_pid() -> int | None:
    """PID of a **live** gateway from ``$CODEXCREW_HOME/gateway.lock``, or None.
...
def _write_artifact(output: Path, text: str) -> bool:
    ...
    symlink, so a planted ``codexcrew-profile.folded`` link would have the linked
...
def _sample_in_process(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        # sys.exit on their normal path (`--call codex_crew.cli_doctor:_doctor`
...
def _sample_out_of_process(args: argparse.Namespace, pid: int) -> int:
    ...
    * py-spy's raw output has to be re-read and redacted anyway (it embeds
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="codexcrew-perf-") as tmpdir:
...
def _perf_sample(args: argparse.Namespace) -> int:
    """Entry point for ``codexcrew perf sample``."""
...
def perf_cmd(args: argparse.Namespace) -> int:
    ...
    print("Usage: codexcrew perf sample [--pid PID | --call module:callable]", file=sys.stderr)
    return 2


def register_perf_parser(sub: argparse._SubParsersAction) -> None:
    """Wire ``codexcrew perf`` into the top-level parser.

    Named ``perf`` rather than ``profile``: ``codexcrew policy profile`` already
    ...
    sample = perf_sub.add_parser(
        ...
        description=(
            "Debug-only stack sampler. Requires CODEXCREW_DEBUG=1. With no "
    ...
    sample.add_argument(
        "--output",
        type=Path,
        default=Path("codexcrew-profile.folded"),
        help="Where to write the profile (default: ./codexcrew-profile.folded)",
    )
