...
Every AWS interaction in :mod:`codex_crew.cloud` goes through :func:`run_aws`.
...
Credentials are resolved by the ``aws`` CLI's own provider chain — CodexCrew
...
(:func:`codex_crew.sandbox.wrap_argv`, ``mode="standard"``) exactly like the
...
from codex_crew.sandbox import cgroup_scope_argv, popen_limited, wrap_argv
...
class CloudActionDenied(Exception):
    ...
    ``deniedCommands`` block the ``aws`` CLI verbs and the ``codexcrew cloud``
    wrappers, but an agent could still import ``codex_crew.cloud`` and call a
...
def _in_agent_session() -> bool:
    ...
    return bool(os.environ.get("CODEXCREW_SESSION_KEY"))


def assert_human_action(action: str) -> None:
    """Refuse a destructive/sensitive cloud mutation from an agent session.

    The gateway injects ``CODEXCREW_SESSION_KEY`` into every agent/ACP subprocess
    (and MCP tools key off it). A human running ``codexcrew cloud …`` in their own
    ...
    ``CODEXCREW_SESSION_KEY`` is an ordinary env var the agent's own code can
    ...
    credentials (see :mod:`codex_crew.cloud.iam`); to truly wall the agent off
    ...
    if _in_agent_session():
        raise CloudActionDenied(
            ...
            "from an agent session — run `codexcrew cloud …` yourself in a terminal."
        )
...
def assert_chokepoint_allowed(args: list[str]) -> None:
    ...
    in-process ``import codex_crew.cloud`` path. All three are bypassable by a
    determined, code-executing agent (obfuscated shell string; ``del
    os.environ['CODEXCREW_SESSION_KEY']``; raw ``aws``/boto3 when ``~/.aws`` is
    ...
    credentials (:mod:`codex_crew.cloud.iam` — tag/ARN-scoped, no IAM writes), plus
    ...
    if _in_agent_session() and (tuple(args[:2]) not in _AGENT_READ_ALLOWLIST):
        ...
        raise CloudActionDenied(
            ...
            "`codexcrew cloud …` yourself in a terminal."
        )
    ...
    *,
    ...
    if rc != 0:
        ...
        if missing:
            ...
            hint = " — add the missing permission (see `codexcrew cloud iam-policy`) and retry"
        raise AWSError(
