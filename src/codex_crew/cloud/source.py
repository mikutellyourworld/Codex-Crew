"""Ship the local CodexCrew source to the instance via S3 (private-repo safe).

The public CodexCrew repo may be private, so the EC2 box can't ``git clone`` it
...
is created once per account/region (``codexcrew-src-<account12>-<region>``) and
reused; each launch uploads to ``<tag>/codexcrew-src.tar.gz``.
"""
...
SOURCE_KEY_NAME = "codexcrew-src.tar.gz"
_BUCKET_PREFIX = "codexcrew-src-"
...
# keep out of the git-archive path — notably the CodexCrew dev-mode data dir
...
_EXCLUDE_DIRS = {
    ...
    # CodexCrew's own data home ~/.codex-crew after the data-root move); excluding
    # the whole ``.kiro`` segment covers both. The legacy ~/.codexcrew stays
...
def find_repo_root() -> Optional[Path]:
    """The Codex Crew source root, or ``None`` when this is not a checkout.
...
def repo_root() -> Path:
    """The CodexCrew source root to package (the installed package's repo).
    ...
    if found is not None:
        return found
    raise aws.AWSError(
        "could not locate the CodexCrew source root (no install.sh + setup.cfg "
...
def _use_git_archive(root: Path) -> Optional[Path]:
    ...
    try:
        out = tempfile.NamedTemporaryFile(  # noqa: SIM115 - handed to caller
            prefix="codexcrew-src-", suffix=".tar.gz", delete=False
        )
        out.close()
...
def _custom_home_rel_parts(root: Path) -> Optional[tuple]:
    """``CODEXCREW_HOME``'s path parts relative to the repo root, if it's under it.

    Dev mode (AGENTS.md) allows a custom-named data dir (e.g. ``.codexcrew-dev`` or
    ...
    actual (possibly nested) path, not just the hardcoded ``.codexcrew*`` entries.
    Returns e.g. ``("data", "kc-home")`` for ``root/data/kc-home``; ``None`` when
    ``CODEXCREW_HOME`` is unset or resolves outside the repo (an absolute
    ``~/.codexcrew`` isn't in the tarball anyway).
    """
...
def _tracked_tree_is_dirty(root: Path) -> bool:
    ...
    tracked file without committing, ``codexcrew cloud launch`` would silently
...
def _tar_fallback(root: Path) -> Path:
    ...
    a custom ``CODEXCREW_HOME`` under the repo is dropped. If the tracked-file
    ...
    if tracked is None:
        raise aws.AWSError(
            ...
            ".gitignore) — run `codexcrew cloud launch` from a git checkout, or "
...
def _account_from_bucket(bucket: str) -> str:
    """Extract the 12-digit account id embedded in a ``codexcrew-src-<account>-<region>`` name.
    ...
    shape (e.g. the ``codexcrew-src-unknown-*`` fallback), so the caller fails
...
def ensure_bucket(profile: str, region: str) -> str:
    ...
    if not account:
        ...
        # closed rather than mint an unprotected `codexcrew-src-unknown-*` bucket.
        raise aws.AWSError(
    ...
    # A pre-existing codexcrew-src-* bucket (older CodexCrew version, or one whose
...
def ensure_instance_boundary(profile: str = "", region: str = "") -> str:
    ...
    ``codexcrew-ec2-boundary`` (see :mod:`cloud.iam`) — the launcher CODE creates
    ...
    **fetch its default version and compare it to the expected content-fixed
    ...
    (``codexcrew cloud iam-boundary``) and drop the ``iam:CreatePolicy`` grant.
    """
    from codex_crew.cloud import iam
    ...
    create = [
        ...
        iam.BOUNDARY_NAME,
        "--description",
        "CodexCrew EC2 instance permissions ceiling (SSM core + launcher source read).",
        "--policy-document",
...
def _verify_instance_boundary_content(arn: str, account: str, profile: str, region: str) -> None:
    ...
    An existing ``codexcrew-ec2-boundary`` must ONLY be reused if its content is
    ...
    if _canonical(actual) != _canonical(expected):
        raise aws.AWSError(
            ...
            "or re-run `codexcrew cloud iam-boundary`.",
            action="iam:GetPolicyVersion",
        )
...
def delete_instance_boundary(profile: str = "", region: str = "") -> dict:
    ...
    call this only after every codexcrew-ec2-* role is gone.
    """
    from codex_crew.cloud import iam
...
def delete_source(tag: str, profile: str = "", region: str = "") -> dict:
    ...
    # back to `codexcrew-src-unknown-*` because the account didn't resolve), fail
