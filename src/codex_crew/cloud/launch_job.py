...
* :class:`LaunchJobStore`, a disk-backed store (one JSON file per job, atomic
  writes, ``CODEXCREW_HOME``-aware via ``config_dir()``);
...
from codex_crew.cloud import sizes
...
_STEP_LABELS: tuple = (
    ...
    (STEP_PROVISION, "Create the instance and install Codex Crew"),
...
class LaunchJobStore:
    ...
    def __init__(self, root: Optional[Path] = None) -> None:
        if root is not None:
            ...
            # config_dir() is called (not imported) late, so a CODEXCREW_HOME
    ...
    def reap_orphans(self) -> List[str]:
        ...
        for job in self.list():
            ...
            job.status = FAILED
            job.error = (
                "Interrupted — Codex Crew restarted while this setup was running. "
