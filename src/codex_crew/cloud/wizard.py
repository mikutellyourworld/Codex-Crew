"""The interactive ``codexcrew cloud launch`` wizard.

One command → answer at most a question or two → CodexCrew running on a
...
from codex_crew.cloud import connect as connect_mod
from codex_crew.cloud import ec2, iam, login, sizes, ssm, ui
from codex_crew.cloud.aws import AWSError
from codex_crew.cloud.config import DEFAULT_REGION, CloudConfig
...
def _deploy_with_progress(
    *,
    ...
    exists, the tail of ``/var/log/codexcrew-setup.log`` over SSM. This replaces
...
def _ensure_running_and_ssm_ready(instance_id: str, profile: str, region: str) -> bool:
    ...
    if state in ("stopped", "stopping"):
        ...
        if state == "terminated":
            ui.fail("the saved instance is terminated — run `codexcrew cloud launch --new`.")
            return False
    ...
    ui.fail(
        ...
        "`codexcrew cloud status`; the agent may still be starting — retry shortly."
    )
    return False
...
def aws_start_instance(instance_id: str, profile: str, region: str) -> None:
    ...
    from codex_crew.cloud import aws as _aws
...
def _verify_operational(
    ...
    try:
        ...
        ui.detail(f"{exc} — check later with: codexcrew cloud login")
        return False
...
def _fetch_bootstrap_log(
    ...
    try:
        res = ssm.run_command(
            instance_id,
            "tail -n %d /var/log/codexcrew-setup.log 2>/dev/null "
            "| tr -d '\\r' "
            "| grep -avE 'Installing (npm|codexcrew and) depend|building React app' || true" % lines,
            profile,
            region,
            run_as="root",
            total_wait=30,
...
def launch(
    ...
    *,
    ...
    wizard is embedded in a larger flow (``codexcrew setup``) that still has
    ...
    if not profile:
        ...
        ui.detail("CodexCrew never stores AWS credentials — the aws CLI resolves them.")
    ...
    ui.detail("  codexcrew cloud iam-policy   → prints the least-privilege policy")
    ...
    try:
        ...
        if exc.missing_action:
            ui.detail(f"Grant `{exc.missing_action}` (see `codexcrew cloud iam-policy`) and retry.")
        return 1
    ...
    if result is None:
        ...
        if subnet_id:
            if assume_yes:
                ...
                ui.detail("Use `codexcrew cloud launch --new --subnet …` for a fresh instance.")
                return 1
    ...
    if result is None:
        ...
        ui.info("Provisioning EC2 + installing CodexCrew (this takes a few minutes)…")
        try:
            ...
            if exc.missing_action:
                ui.detail(
                    f"Grant `{exc.missing_action}` (see `codexcrew cloud iam-policy`) and retry."
                )
            else:
                ...
                if not keep_on_failure:
                    ui.detail(
                        ...
                        "inspection (`codexcrew cloud launch --keep-on-failure`)."
                    )
            return 1
        ...
        cfg.profile, cfg.region, cfg.last_tag = profile, region, tag
        cfg.save()
        ui.ok(f"Instance {result.instance_id} is up and CodexCrew is healthy.")
    elif not result.instance_id:
        ui.warn("Previous cloud stack exists but the instance is not ready yet.")
        ui.detail("Check progress with: codexcrew cloud status")
        return 1
    else:
        ui.ok(f"Resuming instance {result.instance_id}.")
        # A resumed stack may be STOPPED (after `codexcrew cloud stop`). All the
    ...
    if login.is_logged_in(instance_id, profile, region):
        ...
        if prompt.already_logged_in:
            ...
            if signed:
                ...
                ui.detail("Re-run: codexcrew cloud connect (then sign in from the dashboard/SSM).")
        else:
            prompt.close()
    ...
    if not _verify_operational(instance_id, profile, region, assume_yes=assume_yes):
        ui.warn(
            ...
            "sign in. Run: codexcrew cloud login"
        )
    ...
    connect_mod.register_instance(
        instance_id,
        name=f"Codex Crew Cloud ({result.tag})",
        profile=profile,
        region=region,
    )
    ...
    if dashboard_ready:
        ui.note(f"{ui.GREEN}{ui.BOLD}CodexCrew is live on AWS.{ui.RESET}")
    else:
        ui.warn("CodexCrew is running on AWS, but the dashboard tunnel is not open.")
        ui.detail("Fix the local SSM tunnel issue, then run: codexcrew cloud connect")
    print()
    ui.note(f"{ui.BOLD}Manage it:{ui.RESET}")
    ui.detail("codexcrew cloud status            # state + cost estimate")
    ui.detail("codexcrew cloud connect           # reopen the dashboard")
    ui.detail("codexcrew cloud stop | start      # pause / resume (saves cost)")
    ui.detail("codexcrew cloud destroy           # remove EVERYTHING from AWS")
    print()
    ...
    # When embedded in `codexcrew setup` (hold_tunnel=False) we must return so
    ...
    if conn and conn.ready and conn.process is not None and conn.process.poll() is None:
        if not hold_tunnel:
            conn.close()
            ui.detail("Dashboard tunnel closed — reopen anytime: codexcrew cloud connect")
        else:
            ui.detail(
                "Keeping the dashboard tunnel open — press Ctrl+C to exit "
                "(reopen later with `codexcrew cloud connect`)."
            )
            try:
                conn.process.wait()
            except KeyboardInterrupt:
                conn.close()
                ui.info("Tunnel closed. CodexCrew keeps running on AWS.")
...
def _ensure_session_manager_plugin(*, assume_yes: bool = False) -> bool:
    ...
    ui.detail("CodexCrew can install AWS's official Session Manager plugin locally.")
    if not assume_yes and not ui.confirm("Install session-manager-plugin now?", default=True):
        ui.detail("Install it later with: codexcrew cloud doctor")
        return False
...
def _choose_existing_launch(
    ...
    if assume_yes:
        if len(launches) == 1 or cfg.last_tag:
            ...
            ui.info(
                f"Existing CodexCrew cloud stack found: {chosen.stack_name}. "
                "Keeping it by default."
            )
            ui.detail("Use `codexcrew cloud launch --new` to create a separate instance.")
            return chosen
        tags = ", ".join(launch.tag for launch in launches)
        raise AWSError(
            "multiple existing CodexCrew cloud stacks found; rerun interactively "
            f"to choose one ({tags}), or pass `codexcrew cloud launch --new` "
    ...
    if len(launches) == 1:
        launch = launches[0]
        idx = ui.choose(
            "Existing CodexCrew cloud deployment",
            [
    ...
    idx = ui.choose("Existing CodexCrew cloud deployments", options, default_index=0)
    if idx < len(launches):
        return launches[idx]
