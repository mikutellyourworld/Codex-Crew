...
from codex_crew.deploy.engine import aws_spawn_env, resolve_aws_bin, resolve_aws_tool_bin
...
def run_command(
    ...
    *,
    ...
    it runs as the CodexCrew user with a login shell (PATH, home). Blocks until
...
def session_manager_plugin_install_hint() -> str:
    ...
    return (
        ...
        f"Session Manager plugin, then rerun `codexcrew cloud connect`: "
...
def install_session_manager_plugin() -> PluginInstallResult:
    ...
    with tempfile.TemporaryDirectory(prefix="codexcrew-ssm-plugin-") as tmp:
        tmpdir = Path(tmp)
...
def open_port_forward(
    ...
    The child gets :func:`~codex_crew.deploy.engine.aws_spawn_env` rather than a
