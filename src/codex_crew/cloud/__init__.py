"""CodexCrew cloud launcher — provision + run CodexCrew on the user's own EC2.
...
- **Bring-your-own-AWS, store nothing.** Every AWS call shells to the ``aws``
  ...
  CodexCrew persists only a profile name, region, and stack name.
...
- Provisioning is a **human/installer action, never an LLM tool** — the cloud
  ...
  ``aws cloudformation delete-stack``) **and** the ``codexcrew cloud
...
- ``templates/codexcrew-ec2.yaml`` — the CloudFormation template.
"""

from __future__ import annotations

from codex_crew.cloud.aws import AWSError, run_aws
