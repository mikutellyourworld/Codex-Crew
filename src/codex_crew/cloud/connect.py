"""Connect to a launched CodexCrew instance — SSM tunnel + token + open browser.
...
2. Mint a short-lived dashboard token on the instance (``codexcrew token``) over
...
DEFAULT_REMOTE_DASHBOARD_PORT = 5486
# Local port the launcher forwards to by default (kept distinct from a local
# gateway's own 5486 so both can coexist).
DEFAULT_LOCAL_PORT = 5599

# `codexcrew token` prints a URL like http://localhost:5486/?token=<jwt>;
...
def mint_token(instance_id: str, profile: str = "", region: str = "", *, ttl: str = "6h") -> str:
    ...
    Runs ``codexcrew token --ttl <ttl>`` on the box and parses the token out of
    ...
    res = ssm.run_command(
        ...
        f"codexcrew token --ttl {_safe_ttl(ttl)} 2>/dev/null | grep -m1 'token=' || true",
        profile,
        region,
        total_wait=90,
    )
    ...
    if not ready:
        ...
        try:
            ...
            error = (
                ...
                f"({exc}) — check `codexcrew cloud status` / your IAM permissions, "
                "then retry."
            )
            _terminate(proc)
            return Connection(
        ...
        if not token:
            # The tunnel is up but we couldn't mint a token (e.g. `codexcrew token`
            ...
            error = (
                ...
                "instance — check `codexcrew cloud status` / the box's gateway, then retry."
            )
            _terminate(proc)
            ready = False
        else:
...
def is_launched_instance(ssm_target: str) -> bool:
    """True if *ssm_target* (an EC2 instance id) was provisioned by a Codex Crew
    ...
    if not ssm_target:
        return False
    try:
        from codex_crew.cloud.launch_job import LaunchJobStore
...
def _safe_ttl(ttl: str) -> str:
    """Constrain a TTL string to the ``<n>[hm]`` shape codexcrew token accepts."""
...
def _browser_open_supported() -> bool:
    ...
    if os.environ.get("CODEXCREW_NO_BROWSER"):
        return False
