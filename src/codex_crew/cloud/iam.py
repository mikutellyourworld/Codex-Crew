...
Same philosophy as ``deploy_web/iam.py``: **CodexCrew never performs an IAM
...
from codex_crew.cloud import aws

# Resource tag every launcher-created resource carries (mirrors deploy-web's
# codexcrew:managed). Scopes the tag-conditioned statements below.
MANAGED_TAG_KEY = "codexcrew:managed"

# The IAM role name pattern the template uses; PassRole is scoped to it.
ROLE_NAME_PREFIX = "codexcrew-ec2-"
...
# permissive. iam:CreateRole is gated on this name so a codexcrew-ec2-* role MUST
...
BOUNDARY_NAME = "codexcrew-ec2-boundary"
...
def boundary_policy_document(account: str = "*") -> dict[str, Any]:
    ...
    launcher bucket prefix (``codexcrew-src-<account>-*/*``) instead of the
    ...
    resource is region-agnostic (``codexcrew-src-<account>-*``) — a byte-stable
    ...
    return {
        ...
        "Statement": [
            ...
            },
            {
                ...
                "Effect": "Allow",
                "Action": ["s3:GetObject"],
                "Resource": f"arn:aws:s3:::codexcrew-src-{account}-*/*",
            },
        ],
    }
...
def policy_document() -> dict[str, Any]:
    ...
    statements: list[dict[str, Any]] = [
        {
            # Stack-mutating verbs scoped to codexcrew-* stacks so the applied
        ...
        },
        {
            ...
            # BOTH the changeSet and stack ARN forms, still codexcrew-*-scoped.
        ...
        {
            # `codexcrew cloud list` discovers instances via the tagging API.
            "Sid": "TagDiscovery",
            "Effect": "Allow",
        ...
        },
        {
            ...
            # is evaluated per-resource, so requiring codexcrew:managed=true on the
        ...
        },
        {
            ...
            # CreateSecurityGroup creates the SG (tagged codexcrew:managed=true at
        ...
        {
            # SG rule mutation is gated to CodexCrew-tagged security groups: the
            # stack's SG carries codexcrew:managed=true from creation
        ...
        },
        {
            ...
            # codexcrew:managed=true and thereby bring it under the tag-gated
        ...
        {
            # Destructive verbs, gated to CodexCrew-tagged resources so a leaked
            # launcher credential can't delete/retag security groups it never
            # created. The stack's SG carries codexcrew:managed=true from creation.
        ...
        },
        {
            ...
            # iam:CreateRole is the enforcement point: a codexcrew-ec2-* role can
            # ONLY be created WITH our permissions boundary (iam:PermissionsBoundary
            # must ArnLike-match arn:...:policy/codexcrew-ec2-boundary). Because the
        ...
        {
            # PutRolePolicy is scoped to the codexcrew-ec2-* role ARN AND gated on
            # the role carrying our creation tag (aws:ResourceTag/
            # codexcrew:managed=true). The name-prefix scope alone let a leaked
            # launcher credential target a PRE-EXISTING codexcrew-ec2-* role that a
            ...
            # tagged codexcrew:managed=true, and CreateRole is boundary-gated
        ...
        },
        {
            ...
            # unbounded codexcrew-ec2-* role as codexcrew:managed=true, then inline
        ...
        },
        {
            ...
            # 403 and the launch can't tag the role codexcrew:managed=true (which the
            ...
            # codexcrew-ec2-* role codexcrew:managed=true, then inline admin + pass it
            # to EC2 (the tag is only non-spoofable if the same policy can't apply
            # it to an arbitrary role).
            #
            # Gate: aws:ResourceTag/codexcrew:managed=true — the role must ALREADY be
            ...
            #     codexcrew:managed=true tag is already "present" in context → match.
            #   * (b) A STANDALONE tag-role on a pre-existing unbounded victim
            #     (which does NOT yet carry codexcrew:managed) is DENIED — the key is
        ...
        },
        {
            ...
            # admin (codexcrew cloud iam-boundary) and drop this statement — the
        ...
        },
        {
            ...
            # a codexcrew-ec2-* role and pass it to EC2 — a full escalation. The
        ...
        {
            # PassRole is scoped to the codexcrew-ec2-* role ARN, gated on the EC2
            # service (iam:PassedToService) AND on our creation tag
            # (aws:ResourceTag/codexcrew:managed=true) — the same non-spoofable
            # constraint as PutRolePolicy. Without the tag, a leaked launcher
            # credential could pass a PRE-EXISTING, unbounded codexcrew-ec2-* role
        ...
        },
        {
            ...
            # and SendCommand (effectively RCE) — are gated to CodexCrew-tagged
