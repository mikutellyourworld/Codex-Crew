...
+ code, and open that URL in the user's *local* browser. CodexCrew stores **no**
...
_LOGIN_LOG_PATH = "/tmp/codexcrew-kiro-login.log"
_LOGIN_PID_PATH = "/tmp/codexcrew-kiro-login.pid"
_LOGIN_FIFO_PATH = "/tmp/codexcrew-kiro-login.stdin"
...
def social_login_hint(prompt: Optional[LoginPrompt]) -> str:
    ...
    return (
        ...
        f"CodexCrew could not automate the SSM port-forward for {ports}; "
        "run `codexcrew cloud connect` and retry sign-in from the instance."
    )
...
def _open_browser(url: str) -> bool:
    ...
    try:
        ...
        from codex_crew.cloud.connect import redact_token
