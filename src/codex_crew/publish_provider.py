"""Publish-provider interface for sharing CodexCrew artifacts to external
destinations.
...
Mirrors CodexCrew's ``LLMProvider`` ABC pattern.
...
- ``push_version`` is **best-effort** and NEVER raises for upstream failures —
  ...
  fails the CodexCrew update" invariant.
"""
...
class SharingModel:
    ...
    #: Can CodexCrew set sharing via the provider API? When False the UI shows an
...
class SyncModel:
    ...
    - ``authority="codexcrew"`` + ``concurrency="token"`` → MIRROR_GUARDED
      (sha256 guard).
    - ``authority="codexcrew"`` + ``concurrency="lww"`` → MIRROR_LWW
      (blind last-write-wins; no Force-push).
    - ``authority="remote"`` + ``concurrency="crdt"`` → LIVE_CRDT
      (CodexCrew is a participant; no version-conflict UI).
    """

    #: "codexcrew" (MIRROR / collab_mode=mirror) | "remote" (LIVE / collab_mode=live)
    authority: str = "codexcrew"
