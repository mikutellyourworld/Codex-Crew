...
def _kiro_windows_cache_path() -> Path:
    ...
    Resolved lazily (not at import) so tests / CODEXCREW_HOME overrides are
    ...
    ``~/.codex-crew`` instead of writing to the now-archived legacy ``~/.codexcrew``
...
def persist_kiro_windows() -> None:
    ...
    synchronous. Atomic via the shared :func:`codex_crew.atomic_write.atomic_write`
...
def _advertised_models_cache_path() -> Path:
    ...
    :func:`_kiro_windows_cache_path`: honour ``CODEXCREW_HOME`` / test overrides
...
def persist_advertised_models() -> None:
    ...
    update synchronous. Atomic via :func:`codex_crew.atomic_write.atomic_write`; a
...
def to_acp_id(canonical_or_id: str) -> str:
    ...
    ``codexcrew-knowledge`` agent on Sonnet. Only the canonical keys (which kiro
