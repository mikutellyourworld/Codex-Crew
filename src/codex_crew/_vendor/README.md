...
- `llama_cpp.llama_cache`'s module-level `import diskcache` is satisfied by a
  `sys.modules` stub installed in `codex_crew.embeddings._install_diskcache_stub`
  (CodexCrew never uses disk-backed LLM state caching; a real installed
...
| `linux_aarch64/` | `llama_cpp_python-0.3.34-py3-none-manylinux2014_aarch64.manylinux_2_17_aarch64.whl` (sha256 725d8a324032b3f1143c20eee62e47415486eb85127e8a134e3a431d666d21d1) |
...
source with a `codex_crew DIVERGENCE FROM UPSTREAM` comment, so
`grep -rn 'codex_crew DIVERGENCE' llama_cpp/` lists what must be re-applied.
