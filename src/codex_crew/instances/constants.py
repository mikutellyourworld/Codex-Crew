...
in ``codex_crew.config.loader``; a user can override them via
``codexcrew config set instances.<key> <value>``. Keeping the canonical default
...
# ``codex_crew.instances.warm_set.resolve_warm_set_cap``), so below that ceiling no
...
# ``codexcrew config set instances.ssh_compression false``. See §5.2.
...
# ``codexcrew config set instances.connect_timeout_secs <value>`` so operators on
...
# How long (secs) to wait for the remote `codexcrew token` to return before
...
# user-tunable via ``codexcrew config set instances.mint_timeout_secs <value>``.
...
# `codexcrew restart` that invalidates tokens); an iframe loaded with a stale
