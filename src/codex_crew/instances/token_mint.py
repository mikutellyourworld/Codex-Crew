"""Mint a remote CodexCrew dashboard token over SSH.

Build the shell snippet that runs ``codexcrew token`` on the remote desktop and
...
from codex_crew.instances.constants import DEFAULT_MINT_TIMEOUT_SECS, TTL_PATTERN
from codex_crew.platform_compat import kill_and_reap
...
DEFAULT_REMOTE_BIN = "$HOME/.local/bin/codexcrew"
...
REMOTE_BIN_CANDIDATES: tuple[str, ...] = (
    "$HOME/.local/bin/codexcrew",  # install.sh / source install
    "$HOME/.codexcrew-app/.venv/bin/codexcrew",  # one-liner installer venv
)

# ttl accepted by `codexcrew token --ttl`: a positive integer with an h/m suffix
# (e.g. "20h", "30m"). Validated to keep it out of the remote command unchecked.
_TTL_RE = re.compile(TTL_PATTERN)

# Extract the JWT from a `codexcrew token` URL: http://localhost:7777?token=eyJ...
# (also matches https://.../?token=...&foo=bar).
_TOKEN_RE = re.compile(r"[?&]token=([^\s&]+)")

# Bare CodexCrew-token / JWT shape, used to scrub a token that reached stdout
...
# How long to wait for the remote `codexcrew token` to return before giving up.
...
def parse_token_from_stdout(stdout: str) -> str:
    """Extract the JWT from a ``codexcrew token`` URL, or ``""`` if absent."""
...
def _token_subcommand(
    ...
    """Return the ``codexcrew token`` subcommand (ttl/port pre-validated).

    ``--port`` is essential when the remote gateway isn't on the default 7777:
    ``codexcrew token`` calls its own dashboard to mint, so the port must match
...
def build_candidate_command(
    ...
    *,
    ...
    Generic over the codexcrew *subcommand* (e.g. ``token``, ``restart``).
    ...
    *running* gateway wrote for that port (``<data-home>/run/gateway-<port>.bin``,
    probing ``$CODEXCREW_HOME`` then ``$HOME/.codex-crew`` then the legacy
    ``$HOME/.codexcrew`` — see :func:`_run_marker_clause`) and execs the launcher
    it names — so mint uses the same venv as the live gateway instead of whatever
    ``~/.local/bin/codexcrew`` happens to point at. Falls through to the candidate
    search when the marker is absent or doesn't name an executable (older
    remotes, or gateway not running), so nothing regresses.
    See :mod:`codex_crew.instances.run_marker`.
    """
    ...
    return prefix + " ".join(
        [
            ...
            "  fi;",
            "done;",
            f'echo "codexcrew binary not found in any of: {", ".join(candidates)}" >&2;',
            "exit 127",
        ]
    )
...
def _run_marker_clause(subcommand: str, port: int) -> str:
    ...
    :func:`codex_crew.instances.run_marker.write_marker`) under each candidate
    data home and ``exec``s the first launcher it names that is an executable
    file:

    1. ``$CODEXCREW_HOME`` — an explicit override, if exported into this shell.
    ...
    :data:`codex_crew.config.paths.CONFIG_DIR_NAME` /
    :data:`~codex_crew.config.paths.LEGACY_CONFIG_DIR_NAME` — the same constants
    ...
    ``$HOME``/``$CODEXCREW_HOME`` expand at remote-shell parse time; *port* is a
    ...
    process's ``config_dir()``, which now defaults to ``~/.codex-crew`` after the
    ``~/.codexcrew`` -> ``~/.codex-crew`` data-home move. This prelude runs in the
    *remote* non-interactive SSH shell, which usually does NOT export
    ``CODEXCREW_HOME`` — so a single ``${CODEXCREW_HOME:-$HOME/.codexcrew}`` default
    ...
    search (which lands on whatever ``~/.local/bin/codexcrew`` points at, e.g. an
    uninstalled worktree). Probing both the new default and the legacy home — and
    still honoring an explicit ``CODEXCREW_HOME`` above both — restores the marker
    ...
    # ``${CODEXCREW_HOME:+...}`` expands to the value only when CODEXCREW_HOME is
    ...
    homes = " ".join(
        [
            f'"${{CODEXCREW_HOME:+$CODEXCREW_HOME/{fname}}}"',
...
def _redacted_output_tail(stdout: str, limit: int = _OUTPUT_TAIL_CHARS) -> str:
    ...
    Why this exists: ``codexcrew token`` on a remote that predates the
    stdout->stderr split prints its failure reasons (``❌ Could not reach gateway
    on port 5486: ...``) to **stdout**, so an error built only from stderr came
...
async def mint_remote_token(
    ...
    *,
    ...
    """SSH to *ssh_host*, run ``codexcrew token``, and return the parsed JWT.

    *remote_port* is passed to ``codexcrew token --port`` so the remote mint
    targets the gateway the instance actually runs on (not the default 7777).

    *embed_parent_port* is passed to ``codexcrew token --embed-parent-port`` so the
    ...
    if proc.returncode != 0:
        ...
        # Belt-and-suspenders for older remotes: pre-fix `codexcrew token` printed
...
async def run_remote_codexcrew(
    ssh_host: str,
    subcommand: str,
    *,
    remote_bin: str = "",
    ...
    """Run ``codexcrew <subcommand>`` on *ssh_host* over SSH.
    ...
    *marker_port* — when the caller knows the remote dashboard port, pass it so
    ...
    ``~/.local/bin/codexcrew`` points at an uninstalled worktree.
    ...
    *connect_timeout_secs* — this is the same one-shot ssh-exec shape as
    ...
    logger.info("Running 'codexcrew %s' on %s", subcommand, ssh_host)
    try:
        proc = await asyncio.create_subprocess_exec(
            *argv,
            stdout=asyncio.subprocess.PIPE,
