"""Instances registry — persistent store of remote CodexCrew instances.

Backs the *Instances* feature (multi-instance management). The registry is a
small JSON file at ``~/.codex-crew/instances.json``. Each record describes how to
reach one remote Codex Crew over **either SSH or AWS SSM Session Manager**
...
* Writes go through :func:`codex_crew.atomic_write.atomic_write` (temp file +
...
in-memory copy, so a live gateway and an out-of-band ``codexcrew`` CLI edit don't
...
from codex_crew.config.loader import _DEFAULT_PORT, config_dir
from codex_crew.instances.constants import TTL_PATTERN
from codex_crew.instances.validation import _AWS_PROFILE_RE as _validation_aws_profile_re

logger = logging.getLogger(__name__)

# Monkeypatchable in tests via ``monkeypatch.setattr`` alongside CODEXCREW_HOME,
# per the shared-state test-isolation lesson. ``None`` means "derive from
# config_dir() at call time" so CODEXCREW_HOME overrides are always honoured.
...
@dataclass
class Instance:
    """One remote Codex Crew instance reachable over SSH or SSM.
