"""Mint a remote Codex Crew dashboard token over AWS SSM (SSM transport).

The SSM-transport sibling of :mod:`codex_crew.instances.token_mint`. Where the
SSH transport runs ``codexcrew token`` over ``ssh <host> <command>``, the SSM
transport runs the same subcommand over ``aws ssm send-command`` (via
:mod:`codex_crew.cloud.ssm`, the launcher's existing send-command chokepoint) so
...
* The *remote* command is the same ``codexcrew token``/``restart`` subcommand
...
def _redacted_tail(text: str, limit: int = _OUTPUT_TAIL_CHARS) -> str:
    ...
    Mirrors :func:`codex_crew.instances.token_mint._redacted_output_tail`'s
...
async def mint_remote_token_ssm(
    ...
    *,
    ...
    """Run ``codexcrew token`` on *ssm_target* over SSM and return the parsed JWT.

    Mirrors :func:`codex_crew.instances.token_mint.mint_remote_token`'s contract
...
async def run_remote_codexcrew_ssm(
    ssm_target: str,
    subcommand: str,
    *,
    aws_profile: str = "",
    ...
    """Run ``codexcrew <subcommand>`` on *ssm_target* over SSM.

    The SSM-transport sibling of
    :func:`codex_crew.instances.token_mint.run_remote_codexcrew`. Returns
    ...
    logger.info("Running 'codexcrew %s' on %s over SSM", subcommand, target)
    try:
        result = await asyncio.wait_for(
            asyncio.to_thread(
                cloud_ssm.run_command,
