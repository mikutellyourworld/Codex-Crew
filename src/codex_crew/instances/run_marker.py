...
The problem this solves:
    Token mint SSHes to the remote desktop and runs ``codexcrew token`` resolved
    from a fixed PATH candidate list (:data:`token_mint.REMOTE_BIN_CANDIDATES`,
    first entry ``$HOME/.local/bin/codexcrew``). When that launcher symlinks into
    an *uninstalled* git worktree (no ``.venv``), every mint fails with
    "CodexCrew venv not found" — even though the gateway itself is up and healthy,
...
The fix:
    At startup the gateway records the absolute path to *its own* ``codexcrew``
...
#: emits, which is :func:`codex_crew.platform_compat.get_process_start_id`.
...
def pid_start_token(pid: int) -> str:
    ...
    - :func:`codex_crew.platform_compat.get_process_start_id` is preferred. It is
    ...
    - :func:`codex_crew.platform_compat.process_start_time` is the fallback, and
...
def read_launcher(port: int) -> str | None:
    ...
    ``codexcrew`` launcher (see :func:`write_marker`), or ``None`` when the
    marker is absent, unreadable, or empty (a source-tree ``python -m
    codex_crew`` launch records no launcher). Same trust argument as
...
def gateway_launcher_path() -> str | None:
    """Absolute path to the running gateway's own ``codexcrew`` launcher.

    In an OSS venv install ``sys.executable`` is ``<venv>/bin/python`` and the
    console script ``pip install -e .`` creates is its sibling
    ``<venv>/bin/codexcrew`` (``codexcrew.exe`` on Windows). Returns ``None`` when
    ...
    if not exe:
        return None
    base = Path(exe).with_name("codexcrew.exe" if os.name == "nt" else "codexcrew")
    try:
...
def write_marker(port: int) -> None:
    ...
    * ``gateway-<port>.bin`` — the launcher path for the SSH token mint, or
      **empty** when no console script sits beside ``sys.executable`` (a
      source-tree ``python -m codex_crew`` launch). Mint's shell clause requires
    ...
    * ``gateway-<port>.pid`` — this process's pid and nothing else, the identity
    ...
    All three go through :func:`codex_crew.atomic_write.atomic_write`, which uses a
    ...
    if not launcher:
        logger.debug(
            "No venv codexcrew launcher next to %s — writing port-only run-marker", sys.executable
        )
    pid = os.getpid()
    try:
