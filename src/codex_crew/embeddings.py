...
(``codex_crew/_vendor``) — no external Ollama server, no HTTP hop, no
...
to ``~/.codex-crew/models/``. Sources are tried in order: a byte-identical
blob salvaged from a legacy Ollama install, then the public CloudFront CDN
(plain HTTPS — no git access, no cloud SDK). ``CODEXCREW_EMBED_MODEL_URL``
...
``CODEXCREW_EMBED_MODEL_PATH`` (or ``memory.embed_model_path``) at a local
...
from codex_crew._ssl_compat import _ssl_context_has_ca_trust
from codex_crew.config.loader import config_path
...
# codexcrew-core MCP server — the GGUF weights themselves are mmap'd and
...
_SKIP_DOWNLOAD_ENV = "CODEXCREW_SKIP_MODEL_DOWNLOAD"
# Distribution: public CloudFront CDN in front of CodexCrew's OWN model bucket
# (codexcrew-models, reachable only through the distribution's OAC — the bucket
...
# CodexCrew first-install, uncontaminated by another product's traffic.
...
# object can only fail verification. Resolution order: CODEXCREW_EMBED_MODEL_URL
...
_MODEL_URL_ENV = "CODEXCREW_EMBED_MODEL_URL"
# Custom-model escape hatch: an absolute path to a user-supplied GGUF. When set
# (env, or the memory.embed_model_path config knob) CodexCrew runs THAT model and
...
_MODEL_PATH_ENV = "CODEXCREW_EMBED_MODEL_PATH"
...
def _install_diskcache_stub() -> None:
    ...
    CodexCrew never constructs a disk-backed LLM state cache (we only use
    ...
    class Cache:  # pragma: no cover - never constructed by CodexCrew
        def __init__(self, *args: object, **kwargs: object) -> None:
            raise RuntimeError(
                "diskcache is stubbed out by codex_crew.embeddings — disk-backed "
    ...
    if _LIB_PATH_ENV not in os.environ:
        ...
        if absent:
            logger.warning(
                ...
                "Codex Crew from a current release, or point %s at a complete lib "
        ...
        if libs_dirname == "win_amd64":
            ...
            if missing_runtime:
                logger.warning(
                    ...
                    "and restart Codex Crew. Memory falls back to keyword search until "
...
def models_dir() -> Path:
    """Directory where downloaded embedding models live (respects CODEXCREW_HOME)."""
...
def _emit_embed_timing(
    ...
    *,
    ...
    try:
        ...
        recorder.histogram("codexcrew.embed.queue_wait", wait_ms, unit="ms")
        recorder.histogram(
            "codexcrew.embed.inference",
            infer_ms,
            unit="ms",
...
def _is_sensitive_model_path(path: Path) -> bool:
    ...
    try:
        ...
        # shared. The exact path stays available on demand: `codexcrew doctor`
        ...
        logger.warning(
            ...
            "embedding model — refusing it. Run 'codexcrew doctor' for the path.",
            exc_info=True,
        )
        return True
...
def resolve_custom_model() -> "CustomModelSpec | None":
    ...
    Resolution order for the path: ``CODEXCREW_EMBED_MODEL_PATH`` env, then the
...
def default_embedding_space_signature() -> str:
    ...
    :meth:`~codex_crew.vector_memory.VectorMemoryStore.reconcile_embedding_space`
...
def reconcile_store_embedding_space(store: "_ReconcilableStore") -> int:
    ...
    non-mutating ``store_embedding_space_is_stale`` probe instead — ``codexcrew
...
class EmbeddingBackend(abc.ABC):
    ...
    - ``model_id`` + ``dim`` identify the vector space. Vectors produced under
      ...
      keys off :func:`codex_crew.knowledge.embedder.embed_signature`, which
        ...
        with self._load_lock:
            ...
            if _is_sensitive_model_path(self._model_path):
                ...
                logger.error(
                    ...
                    "Run 'codexcrew doctor' for the path."
                )
    ...
    def _load_model(self) -> None:
        ...
        try:
            ...
            llm = llama_cls(
                ...
                # src/codex_crew/_vendor/llama_cpp/llama.py, issue #6827), so
...
def reset_shared_embedder() -> None:
    """Drop the singleton (tests, disable-embeddings, CODEXCREW_HOME changes)."""
    global _shared_embedder
    with _shared_embedder_lock:
...
def _resolve_model_url() -> str:
    ...
    The ``CODEXCREW_EMBED_MODEL_URL`` env var wins (mirrored/airgapped
...
class ModelDownloadManager:
    ...
    async def ensure_model(self, attempts: int = 1) -> bool:
        ...
        ``CODEXCREW_SKIP_MODEL_DOWNLOAD=1`` escape hatch is set — test runs
...
def reset_download_manager() -> None:
    """Drop the singleton (tests, CODEXCREW_HOME changes)."""
