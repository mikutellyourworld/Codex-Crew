...
from codex_crew.mcp_shared import ToolCancelled, is_tool_cancelled
...
from codex_crew.subagent_persistence import agent_dir_for_display
...
def schemas() -> list[dict[str, Any]]:
    ...
    try:
        _max_sub = resolve_max_subagents(CodexCrewConfig.load())
    except Exception:
        _max_sub = 0
    _cap_hint = (
...
def spawn_run(name: str, args: dict[str, Any]) -> str:
    ...
    # CODEXCREW_SESSION_KEY/CODEXCREW_CHANNEL_ID) so a cron running with
    ...
    # responder for.
    approval_mode = os.environ.get("CODEXCREW_APPROVAL_MODE", "")
    ...
    if not parent_session and agent_ids:
        ...
        spawn_lines.append(
            ...
            "Poll spawn_list and read ~/.codex-crew/subagents/<id>/result.txt "
            "instead. (Identity plumbing issue — check CODEXCREW_HOST_PID / "
...
def spawn_sub_agents(name: str, args: dict[str, Any]) -> str:
    ...
    poll_interval = 2.0
    try:
        max_wait = float(os.environ.get("CODEXCREW_SPAWN_SUB_AGENTS_MAX_WAIT", "7200"))
