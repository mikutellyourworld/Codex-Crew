...
from codex_crew import file_delivery_consent, mcp_core
...
from codex_crew.security import BINARY_MIME_ALLOWLIST, redact_credentials, redact_exfiltration_urls
from codex_crew.validation import _SLACK_TS_RE, CHANNEL_ID_RE
...
def schemas() -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "description": (
                "Publish a notification to the Codex Crew notification center "
        ...
        },
        {
            ...
            "description": (
                ...
                "messages authored by the Codex Crew bot itself (Slack API constraint). "
...
def send_message(name: str, args: dict[str, Any]) -> str:
    ...
    if channel_type:
        ...
        # ``CODEXCREW_SESSION_KEY`` into every agent/ACP subprocess
    ...
    # auto-approved codexcrew-core call (default allowedTools) emits no
