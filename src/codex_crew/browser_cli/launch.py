"""The launch config that makes `playwright-cli` open the browser Codex Crew provisions.

Codex Crew installs, gates on, and offers downloads for **Chromium**:
...
Codex Crew never provisions and cannot install without root. So on a host that has
...
- ``--config`` is accepted only on the session-establishing commands (``open``,
  ...
  :func:`codex_crew.browser_cli.snapshots.cli_env_overrides` use an env var.
- ``PLAYWRIGHT_MCP_CONFIG`` names a config FILE and applies to every invocation
  uniformly. That is the one channel that reaches a command line Codex Crew never
...
from codex_crew.browser_cli.install import cli_lifecycle_env_supported
...
#: The engine Codex Crew provisions and gates on. Kept as one named constant so
...
#: Marks a generated name as Codex Crew's in ``playwright-cli list``, so an
...
def _operator_base(configured: str) -> Path | None:
    ...
    (``dev-backend.sh`` exports ``CODEXCREW_HOME``, and a pod runs an isolated
...
def browser_socket_env(env: Mapping[str, str]) -> dict[str, str]:
    ...
    ``playwright-cli`` launches its daemon after Codex Crew has pointed
    ...
    the corresponding session registry location, letting Codex Crew find and
    validate the exact generated session file at controlled teardown without
    executing the user-writable CLI wrapper.

    This helper is called only when Codex Crew generated ``SESSION_ENV``.
...
def launch_config_path() -> Path:
    ...
    working directory an agent turn ran in, and so an isolated ``CODEXCREW_HOME``
...
def desired_config() -> dict[str, object]:
    """The config Codex Crew generates.
...
def write_config() -> Path | None:
    ...
    try:
        ...
        logger.warning(
            ...
            "fall back to its own default browser channel, which Codex Crew does "
            "not install",
            path,
        )
        return None
...
def browser_session_env(env: Mapping[str, str]) -> dict[str, str]:
    ...
    :meth:`codex_crew.session.SessionManager.get_or_create`). Uniqueness per
    ...
    an agent process — which this repo's own ``codexcrew-worktree-dev`` skill
