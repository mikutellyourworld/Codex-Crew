...
Node is located through :func:`codex_crew.env.find_node_tool` rather than bare
...
from codex_crew.browser_cli import os_deps
from codex_crew.config.paths import config_dir
from codex_crew.env import augmented_path, find_node_tool, node_augmented_path
...
_STANDALONE_PREFIX_ENV = "CODEXCREW_PLAYWRIGHT_CLI_HOME"
...
_INSTALLER_BASE = "https://raw.githubusercontent.com/mikutellyourworld/Codex-Crew/main"
...
def _step(
    ...
    and the log is the longer-lived of the two surfaces: `codexcrew logs` output
...
def _download_browser(path: str, engine: str | None = None) -> list[dict[str, Any]]:
    ...
    :mod:`codex_crew.browser_cli.os_deps`), and even there a refusal is not fatal.
