"""Whether a harness's tool decisions reach Codex Crew's PreToolUse gate.
...
:mod:`codex_crew.acp_backends` and nothing from ``codex_crew.acp``, which the SDK
boundary gate treats as a forbidden root. The callers that need a verdict --
``acp/client.py`` on the session path, and later ``codexcrew doctor`` and the
dashboard -- can therefore all reach it, and a consumer naming a verdict does not
buy a forbidden edge to do it.

**Enforcement scope.** Only :data:`~codex_crew.acp_backends.Routing.SESSION_CONFIG`
...
class Verdict(str, Enum):
    """Whether a harness's tool decisions reach Codex Crew's gate.
...
def adapter_hidden_credential_dirs(backend: str) -> tuple:
    ...
    from codex_crew.security import sandbox_credential_targets

    # Delegated rather than projected under ``Path.home()`` here: a credential the
    # operator relocated with ``CODEXCREW_HOME`` / ``CLAUDE_CONFIG_DIR`` /
...
def enforce_sandbox_floor(backend: str, mode: str, *, native_tool_gate: bool = False) -> None:
    ...
    if not is_enforced(backend):
        return
    if native_tool_gate:
        # The caller may set this only after injecting a fail-closed native
        # PreToolUse hook. That hook closes the passive-read gap before Codex
        # runs a tool, so the adapter mask is defense in depth rather than the
        # sole control. Governance still decides whether an OS sandbox is
        # mandatory in sandbox.wrap_argv.
        return
    # Local import for the same reason as the mask builder: this is a leaf that
    # ``acp/client.py`` imports at import time.
    from codex_crew.sandbox import credential_mask_applies
...
def label_for(backend: str) -> str:
    ...
    return _LABELS.get(backend) or (backend or "Agent backend")
...
def routing_verdict(backend: str) -> tuple:
    ...
    return (
        Verdict.INDETERMINATE,
        "Codex Crew has not established how this harness routes tool calls",
    )
...
def enforce_runtime_routing(
    ...
    **There is deliberately no opt-out.** An earlier revision carried
    ...
    raise ToolGateUnroutable(
        f"{label_for(backend)} tool calls would not reach Codex Crew's security gate "
