"""``codexcrew pod <verb>`` — kubectl-style control of worktree test pods.

Thin verb layer over :mod:`codex_crew.pod.runtime` / :mod:`codex_crew.pod.unit`.
Dispatched from :func:`codex_crew.cli_commands._pod`.
"""
...
from codex_crew.tips_text import truncate_summary
...
def _verify_seed_landed(cfg: PodConfig, name: str, scenario: str, home_was_populated: bool) -> None:
    ...
    if home_was_populated:
        ...
        _die(
            ...
            f"codexcrew pod up {name}. To boot it fresh: "
            f"codexcrew pod down {name} && codexcrew pod up {name} --seed {scenario}"
        )
    if landed == scenario:
        return
    ...
    _die(
        ...
        f"  Its boot log: codexcrew pod logs {name}\n"
        f"  Then retry:   codexcrew pod down {name} && codexcrew pod up {name} "
        f"--seed {scenario}"
    )
...
def _up(cfg: PodConfig, args: argparse.Namespace) -> None:
    ...
    if getattr(args, "provision", False):
        ...
        if not prov.has_dist(checkout):
            _die(
                ...
                f"  Or let pod do the full chain: codexcrew pod up {name} --provision"
            )
        ...
        if not was_active:
            ...
            with rt.pod_plane_mutex(cfg):
                ...
                if displaced_from is not None:
                    print(
                        ...
                        f"agrees; `codexcrew pod url {name}` prints it.",
                        file=sys.stderr,
                    )
                if env_updates:
            ...
            if boot_flags:
                ...
                print(
                    ...
                    f"(codexcrew pod down {name} && codexcrew pod up {name} {joined}).",
                    file=sys.stderr,
                )
        ...
        if code not in (200, 401, 403):
            ...
            if code == rt.HEALTH_FOREIGN:
                ...
                _die(
                    ...
                    f"  Which pods hold which ports: codexcrew pod ls\n"
                    f"  Give this pod its own port:  add PORT=<free port> to "
                    f"{cfg.env_file(name)}, then `codexcrew pod up {name}` again."
                )
            if code == -1:
                _die(
                    f"{name}: the worktree's gateway failed to start (see journal above). "
                    f"This is the worktree build, not pod — fix it, then `codexcrew pod up {name}` again."
                )
            _die(
    ...
    if args.json:
        ...
        print(f"  stop     : codexcrew pod down {name}")
...
def _print_orphans(cfg: PodConfig, orphans: list[str]) -> None:
    ...
    for n in orphans:
        ...
        try:
            ...
            age = "age unknown"
        print(f"  {n:<26} {age:<12} reclaim: codexcrew pod down {n}")
    print("  bulk reclaim: codexcrew pod prune [--all] [--dry-run] (default keeps the last 3d)")
...
def _exec(cfg: PodConfig, args: argparse.Namespace) -> None:
    ...
    if not argv:
        _die("nothing to run — usage: codexcrew pod exec <name> -- <args…>")
...
def _install(cfg: PodConfig, args: argparse.Namespace) -> None:
    ...
    print(msg)
    print("ready. Next: codexcrew pod up <worktree>")
...
def _cleanup_internal(cfg: PodConfig, args: argparse.Namespace) -> None:
    ...
    Not wired to a service-manager hook (see :mod:`codex_crew.pod.unit` for why
    ...
    ``codexcrew pod down <name>``, which stops the service first.
    """
...
def _scenarios(cfg: PodConfig, args: argparse.Namespace) -> None:
    ...
    print(f"\nseed one with: codexcrew pod up <worktree> --seed {rows[0]['name']}")
