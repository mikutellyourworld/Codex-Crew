# `codexcrew pod` — isolated worktree test instances

Spin up a **throwaway, full-stack CodexCrew gateway** for any feature worktree —
its own port, its own `CODEXCREW_HOME` (own DB / sessions / memory), no Slack
...
live gateway or your shared `~/.codex-crew` data**.
...
host-side (`main/live Codex Crew -> target test pod`); a pod does not create or control
a child pod.
...
codexcrew pod up   <wt> [--json]   # bring up an isolated pod → {base_url, token, port}
codexcrew pod up   <wt> --provision# provision (if needed) then bring it up
codexcrew pod up   <wt> --approval reads  # boot its gateway in an approval mode
codexcrew pod up   <wt> --crons          # boot its gateway with the cron scheduler on
codexcrew pod up   <wt> --seed minimal  # pre-populate its HOME from a named scenario
codexcrew pod scenarios [--json]        # list named scenarios and their descriptions
codexcrew pod api  <wt> GET sessions    # authenticated request → fixed-key JSON
codexcrew pod ls                   # what's running (≈ kubectl get pods) + orphaned HOMEs (with age)
codexcrew pod prune [--all] [--dry-run]  # bulk-reclaim orphaned HOMEs (default: older than 3d; --all for every age)
codexcrew pod status <wt>          # up/down + health
codexcrew pod token  <wt> [--ttl]  # (re)mint a dashboard token for a running pod
codexcrew pod url    <wt>          # print its base_url
codexcrew pod logs   <wt> [-n N]   # tail its journal
codexcrew pod down   <wt>          # evict → delete its HOME, verified (zero residue)
```

`<wt>` is a friendly worktree name. It is resolved to a checkout **git-natively**:
`codexcrew pod up <name>` matches a linked worktree by its directory basename, its
branch (`<name>` or `feat/<name>`), or an exact path — run it from inside any
CodexCrew checkout (or set `CODEXCREW_POD_REPO`). The resolved path is pinned so the
...
`.venv/bin/codexcrew` and a built SPA bundle (`src/codex_crew/static/dist`). These
...
```bash
codexcrew pod scenarios
codexcrew pod scenarios --json
codexcrew pod up my-wt --seed minimal
codexcrew pod up my-wt --seed ~/.codex-crew
```
...
A bare name selects a fixture shipped under `codex_crew/tests_fixtures/<name>/`
...
codexcrew pod api my-wt GET '/api/sessions?limit=20'
codexcrew pod api my-wt POST config --data '{"key":"agent.model"}' --allow-write
```
...
- **Control plane** — the `codexcrew pod` verbs (resolution, port derivation, unit
  management, token mint, boot *prep*). These run from the **stable, globally
  installed** `codexcrew`, so they never break just because a worktree's code is broken.
- **Payload** — the booted pod *is* the worktree's `.venv/bin/codexcrew gateway`. If
...
`codexcrew pod install` writes a template unit `codexcrew-pod@.service` whose
`ExecStart` re-enters `codexcrew pod _run <wt>` (boot logic lives in
`codex_crew.pod.runtime.boot`). Before each start, `pod up` writes a per-instance
drop-in that replaces the template's `ExecStart` with the resolved checkout's
own `.venv/bin/codexcrew`; it refuses to fall back to a global install that may
...
of its sessions and config. So `codexcrew pod down` owns reclamation on every
...
`PORT=` is pinned in `~/.codex-crew/pods/<name>.env`. `pod up` refuses if a derived
...
## Configuration (`PodConfig`, all `CODEXCREW_POD_*`-overridable)

| env | default | meaning |
|---|---|---|
| `CODEXCREW_POD_REPO` | invoking cwd | repo git is queried from to resolve worktree names |
| `CODEXCREW_POD_WORKTREES_ROOT` | (unset) | optional `name→path` fallback root (hermetic planes) |
| `CODEXCREW_POD_ROOT` | `~/.codexcrew-pods` | isolated pod HOMEs (reclaimed by `pod down`) |
| `CODEXCREW_POD_ENV_DIR` | `~/.codex-crew/pods` | per-pod `CHECKOUT=`/`PORT=`/`SEED=` files |
| `CODEXCREW_POD_BASE_PORT` | `7810` | port derivation base |
| `CODEXCREW_POD_LIVE_PORT` | `5486` | the port a pod must never bind |
| `CODEXCREW_POD_UNIT_PREFIX` | `codexcrew-pod` | systemd unit prefix |
| `CODEXCREW_POD_BIN` | (auto) | the `codexcrew` binary the unit boots |
...
- A pod runs its own `CODEXCREW_HOME` and binds `127.0.0.1` only; it never touches
  the shared `~/.codex-crew` data and refuses the live port.
- Every pod's `config.json` forces `enabled=false` on the tunnel and on every
  ...
  seeded one, which is the point: `--seed ~/.codex-crew` clones the real config.
...
systemd **system** unit — which is how `codexcrew service install` runs the
...
that names neither cause nor remedy. `codexcrew doctor` reports the same three
