...
The unit's ``ExecStart`` re-enters a ``codexcrew`` binary as ``codexcrew pod _run
%i``, so the boot logic lives in Python (see :func:`codex_crew.pod.runtime.boot`)
...
(:func:`install_dropin`) pinning ``ExecStart`` to its OWN checkout's ``codexcrew``
...
:func:`codex_crew.pod.runtime.stop_pod`, which runs it after the service is
...
from codex_crew.atomic_write import atomic_write_at
from codex_crew.pod import provision as prov
from codex_crew.pod.config import PodConfig, environment_vars
from codex_crew.service.common import systemd_quote

_UNIT_TEMPLATE = """\
[Unit]
Description=CodexCrew pod (%i) — isolated worktree gateway, throwaway
# Multi-active: many run at once, one per worktree, each on its own port + own
# CODEXCREW_HOME. Orthogonal to the live gateway; refuses to bind the live port.
...
# codexcrew binary; boot logic is codex_crew.pod.runtime.boot (no shipped shell).
ExecStart={codexcrew_bin} pod _run %i
...
# bringing the pod back on a wiped home. `codexcrew pod down` owns reclamation
# (codex_crew.pod.runtime.stop_pod); `codexcrew pod ls` reports what a crash left.
...
def _codexcrew_argv() -> tuple[str, ...]:
    """Argv prefix that re-enters the codexcrew entry point the unit should boot.

    Resolution order:
      1. ``CODEXCREW_POD_BIN`` — explicit override (used when installing a unit that
         must boot a specific build, e.g. a worktree's own ``.venv``).
      2. the console-script on PATH.
      3. ``<this python> -m codex_crew`` so it works from a bare checkout.
    """
    override = os.environ.get("CODEXCREW_POD_BIN")
    if override:
        return (override,)
    found = shutil.which("codexcrew")
    if found:
        return (found,)
    return (sys.executable, "-m", "codex_crew")


def _environment_block(cfg: PodConfig) -> str:
    """``Environment=`` lines for the shared pod-plane env selection.

    The *selection* lives in :func:`codex_crew.pod.config.environment_vars` so the
...
def render_unit(cfg: PodConfig) -> str:
    return _UNIT_TEMPLATE.format(
        codexcrew_bin=" ".join(systemd_quote(arg) for arg in _codexcrew_argv()),
...
# bake ONE codexcrew binary: whichever one ``pod install`` happened to resolve,
# normally the globally installed ``~/.local/bin/codexcrew``. Every pod therefore
...
_DROPIN_FILENAME = "50-codexcrew-pod.conf"

_DROPIN_TEMPLATE = """\
# Written by `codexcrew pod up` — removed by `codexcrew pod down`. Do not edit.
[Service]
# Reset the template's ExecStart (a list directive, so an unreset second value
# would APPEND) and boot this pod through its own checkout's codexcrew. The pod
...
def render_dropin(checkout: Path) -> str:
    """The drop-in pinning a pod's boot to *checkout*'s own ``codexcrew``."""
    return _DROPIN_TEMPLATE.format(codexcrew_bin=systemd_quote(str(prov.venv_bin(checkout))))
...
def remove_dropin(cfg: PodConfig, name: str) -> bool:
    """Delete only Codex Crew's drop-in. True when its file and empty dir are gone.
...
def unit_exec_ok(cfg: PodConfig) -> bool:
    ...
    The unit bakes an absolute codexcrew path at install time (often a
...
def unit_is_current(cfg: PodConfig) -> bool:
    ...
    machine that installed an older Codex Crew would keep the teardown hook, and
