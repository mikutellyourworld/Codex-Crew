"""macOS ``launchd`` backend for pods — the darwin twin of :mod:`codex_crew.pod.unit`.
...
(``codexcrew-pod@.service``) and instantiates it per pod via ``%i``. launchd has
no such concept, so each pod gets its own plist under the pod plane's own directory
(``cfg.pods_dir/dev.codexcrew.pod.<name>.plist`` — deliberately NOT
...
absolute ``codexcrew`` path at *install* time, which goes stale when the worktree
...
stop half of a ``Restart=``, so :func:`codex_crew.pod.runtime.stop_pod` owns
...
:func:`codex_crew.pod.runtime.orphan_homes` exists so ``pod ls`` can REPORT those
...
Every other isolation property is unchanged on macOS: own ``CODEXCREW_HOME``, own
...
from codex_crew.pod.unit import _codexcrew_argv as _shared_codexcrew_argv
...
LABEL_PREFIX = "dev.codexcrew.pod"
...
def require_backend() -> None:
    ...
    if shutil.which("launchctl") is None:
        raise LaunchdError(
            ...
            "like macOS but is missing launchd; run `codexcrew pod` from a normal "
            "user session."
        )
...
def pod_label(cfg: PodConfig, name: str) -> str:
    ...
    (``CODEXCREW_POD_UNIT_PREFIX``) cannot collide with a developer's real pods,
    exactly as it cannot on systemd.
    """
    # EVERY plane carries its prefix segment — including the default one.
    # Omitting it for the default made "dev.codexcrew.pod." a strict prefix of
...
def _codexcrew_argv() -> list[str]:
    """Return the shared entry-point prefix in launchd's list form."""
    return list(_shared_codexcrew_argv())
...
def render_plist(cfg: PodConfig, name: str) -> dict[str, object]:
    ...
    body: dict[str, object] = {
        ...
        # boot logic stays in Python (codex_crew.pod.runtime.boot), so nothing
        # shell-shaped is shipped, same as the systemd unit.
        "ProgramArguments": [*_codexcrew_argv(), "pod", "_run", name],
...
def stop(cfg: PodConfig, name: str, *, timeout: float = 15.0) -> subprocess.CompletedProcess:
    ...
    **A per-pod plist must not outlive its pod.** systemd's template unit is
    ...
    it behind makes :func:`codex_crew.pod.runtime.orphan_homes` classify the leftover HOME as "installed,
...
def active_names(cfg: PodConfig) -> set[str]:
    ...
    prefix = pod_label(cfg, "")  # e.g. "dev.codexcrew.pod." (trailing dot kept)
    names: set[str] = set()
