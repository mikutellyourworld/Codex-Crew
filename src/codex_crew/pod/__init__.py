"""CodexCrew pod — isolated, throwaway, full-stack test instances per worktree.

A *pod* is an ephemeral CodexCrew gateway booted from one feature worktree's own
``.venv``, on its own deterministic port, with its own ``CODEXCREW_HOME`` (own DB /
...
gateway or the shared ``~/.codex-crew`` data**. Think ``kubectl`` for local worktree
test rigs.
...
The user-facing surface is ``codexcrew pod <verb>`` (see :mod:`codex_crew.pod.cli`):
...
:func:`codex_crew.pod.runtime.resolve_checkout`) and pinned so the systemd-booted
gateway never re-resolves. Mechanism, per platform:

* **Linux (``systemd --user``)**: one template unit ``codexcrew-pod@<wt>.service``
  whose ``ExecStart`` re-enters ``codexcrew pod _run <wt>`` (boots the worktree's own
...
* **macOS (``launchd``)**: one agent plist per pod under the pod plane's own directory (not
  ...
  enforced** — see :mod:`codex_crew.pod.launchd` for why a weaker key is
...
from codex_crew.pod.runtime import (
    PodError,
    derive_port,
    pod_home,
    pod_unit,
