...
A pod boots the worktree's OWN ``.venv/bin/codexcrew gateway`` serving its OWN
...
module collapses them into one command (``codexcrew pod provision`` /
``pod up --provision``).
...
from codex_crew.env import find_node_tool, node_augmented_path
...
def venv_bin(checkout: Path) -> Path:
    """Path to the worktree venv's ``codexcrew`` entry point.
    ...
    """
    name = "codexcrew.exe" if platform_compat.IS_WINDOWS else "codexcrew"
    return venv_bin_dir(checkout) / name


def dist_dir(checkout: Path) -> Path:
    return checkout / "src" / "codex_crew" / "static" / "dist"
...
def _npm_bin() -> str | None:
    ...
    if npm:
        return npm
    _say(
        "FATAL: npm not found. Codex Crew looks for a Node toolchain in "
        ...
        "Node, or set CODEXCREW_NODE_BIN_DIR=/abs/path/to/node/bin."
    )
    return None
...
def build_dist(checkout: Path) -> bool:
    ...
    ``src/codex_crew/static/dist``. Returns True if the dist exists afterward."""
...
def provision(checkout: Path, build: bool = True) -> bool:
    ...
    if not build_dist(checkout):
        return False
    _say(f"[provision] {checkout.name} is ready — `codexcrew pod up {checkout.name}`")
    return True
