...
pod's ``.local_secret``) lives here so :mod:`codex_crew.pod.cli` stays a thin verb
...
from codex_crew import pinned_fs
from codex_crew import seed as seed_mod
from codex_crew.atomic_write import atomic_write, atomic_write_at
from codex_crew.dashboard.urls import dashboard_socket_name
from codex_crew.instances import run_marker
from codex_crew.loopback_http import loopback_urlopen, unix_socket_urlopen
from codex_crew.platform_compat import (
    IS_LINUX,
    IS_MACOS,
    IS_POSIX,
    find_port_listeners,
...
from codex_crew.pod import launchd
from codex_crew.pod import provision as prov
from codex_crew.pod import unit as unit_mod
from codex_crew.pod.config import PodConfig
from codex_crew.seed import SeedError
...
# ``codexcrew gateway --approval``. This tuple is the ENFORCEMENT point: the env
...
def resolve_checkout(
    ...
    (from ``CODEXCREW_POD_REPO`` else *cwd*), matching a worktree's basename, then
    its branch (``name`` or ``feat/<name>``), then an exact path → optional
    ``CODEXCREW_POD_WORKTREES_ROOT/name`` fallback → :class:`PodError`.
    """
    ...
    raise PodError(
        ...
        f"  (run `codexcrew pod up {name}` from inside a codexcrew checkout, "
        f"or set CODEXCREW_POD_REPO to point at one)"
    )
...
def pod_socket_path(cfg: PodConfig, name: str, port: int) -> Path:
    ...
    ``CODEXCREW_HOME``, which is :func:`pod_home` -- so the socket lands in the
...
def _systemctl_env() -> dict[str, str]:
    ...
    a systemd SYSTEM unit — which is how ``codexcrew service install`` runs the
...
def main_pid(cfg: PodConfig, name: str) -> int | None:
    ...
    of how a pod boots: the unit is ``Type=simple`` running ``codexcrew pod _run
    %i``, and :func:`_run` finishes with ``os.execve`` of the worktree's
    ``codexcrew gateway``. The gateway therefore REPLACES the unit's main process
...
def unit_state(cfg: PodConfig, name: str) -> tuple[str, int]:
    ...
    :func:`codex_crew.pod.launchd.unit_state` for how the crash signal is
...
def _write_and_load_unit(cfg: PodConfig) -> subprocess.CompletedProcess | None:
    ...
    systemd.* :func:`codex_crew.pod.unit.unit_is_current` reads the file, but what
...
def _refresh_stale_unit(cfg: PodConfig) -> subprocess.CompletedProcess | None:
    ...
    return subprocess.CompletedProcess(
        ...
        stderr=(
            ...
            "start or stop a pod until the unit is loaded: run `codexcrew pod install` "
            f"and retry.{detail}"
        ),
    )
...
def _install_pod_dropin(cfg: PodConfig, name: str) -> subprocess.CompletedProcess | None:
    ...
    if not checkout:
        return subprocess.CompletedProcess(
            ...
            stderr=(
                ...
                f"worktree's own codexcrew. Run `codexcrew pod up {name}` from inside "
                "the checkout."
            ),
        )
    try:
    ...
    return subprocess.CompletedProcess(
        ...
        stderr=(
            ...
            "boot the globally installed codexcrew. Refusing to start it; retry or "
            f"run `codexcrew pod install`.{detail}"
        ),
    )
...
def start_pod(cfg: PodConfig, name: str) -> subprocess.CompletedProcess:
    ...
    with pod_name_mutex(cfg, name):
        ...
        # an absolute codexcrew path at install time (a pruned worktree leaves it
        ...
        if survivors:
            ...
            return subprocess.CompletedProcess(
                ...
                stderr=(
                    ...
                    f"NOT zero-residue. Reclaim it with `codexcrew pod down {name}` "
        ...
        if rc != 0 or leftover.exists():
            return subprocess.CompletedProcess(
                ...
                stderr=(
                    ...
                    f"Reclaim it with `codexcrew pod down {name}` once nothing is "
                    "writing there."
                ),
            )
        if not dropin_gone:
...
def orphan_homes(cfg: PodConfig) -> list[str]:
    ...
    :func:`cleanup_home`'s re-validation via ``codexcrew pod down <name>``.
    """
    try:
...
def install_backend(cfg: PodConfig) -> tuple[str, subprocess.CompletedProcess | None]:
    ...
    if IS_MACOS:
        return (
            ...
            "pod's agent plist is written at `codexcrew pod up <worktree>`.",
            None,
        )
...
# answer from any Codex Crew gateway on the host, and its identity fields say
# `app=codexcrew` plus a version — true of the squatter as well. Reading a 200 as
...
def _unproven_remedy(cfg: PodConfig, name: str, port: int) -> str:
    ...
    if record is not None and not record[1]:
        return (
            ...
            f"(`codexcrew pod provision {name}`), then `codexcrew pod down {name} && "
            f"codexcrew pod up {name}`."
        )
    return (
        f"A record left behind by a crash cannot attest, so restarting it is what "
        f"re-establishes the proof: `codexcrew pod down {name} && "
        f"codexcrew pod up {name}`."
    )
    ...
    if owner != OWNER_POD:
        ...
        if owner == OWNER_FOREIGN:
            raise PodError(
                ...
                f"by another process, not this pod's gateway. `codexcrew pod status "
...
def _read_capped(stream: object, method: str, path: str, name: str) -> str:
    ...
    try:
        ...
        raise PodError(
            ...
            f"  Is it healthy? codexcrew pod status {name}\n"
            f"  Its logs:      codexcrew pod logs {name}"
        ) from exc
    ...
    *,
    ...
    if method not in API_READ_METHODS and not allow_write:
        raise PodError(
            ...
            f"  Retry: codexcrew pod api {name} {method} {normalized} --allow-write"
        )
    if not is_active(cfg, name):
        raise PodError(
            f"pod {name!r} is not running, so there is nothing to call.\n"
            f"  Start it:   codexcrew pod up {name}\n"
            f"  What is up: codexcrew pod ls"
        )
    port = derive_port(cfg, name)
    ...
    if not socket_path.exists():
        ...
        raise PodError(
            ...
            f"  Is it healthy? codexcrew pod status {name}\n"
            f"  Its logs:      codexcrew pod logs {name}\n"
            f"  Restart it:    codexcrew pod down {name} && codexcrew pod up {name}"
        )
...
# ``--seed ~/.codex-crew`` workflow) and a pod that answers real people as the
    ...
    try:
        ...
        try:
            ...
            if entries:
                ...
                if recorded != scenario:
                    ...
                    raise PodError(
                        ...
                        "overwrite it. Run `codexcrew pod down "
            ...
            raise PodError(
                ...
                f"`codexcrew pod down {name}` before retrying."
            ) from exc
    finally:
        if home_fd >= 0:
            os.close(home_fd)
...
def build_pod_env(cfg: PodConfig, home_dir: Path, port: int, checkout: Path) -> dict[str, str]:
    ...
    env = {
        **os.environ,
        ...
        "CODEXCREW_HOME": str(home_dir),
        "CODEXCREW_PORT": str(port),
        "CODEXCREW_PROJECT_DIR": str(checkout),
        ...
        # keys on THIS marker rather than on "has an isolated CODEXCREW_HOME",
        ...
        # real lessons). Safe only because every CodexCrew reader of the transcripts
        # dir now resolves through ``kiro_sessions_dir()``; without that the pod
        # would write sessions somewhere CodexCrew never looks and lose resume.
        ...
        # finds no `CODEXCREW_WORKSPACE` and no `config_dir()/workspace_dir` file in
        ...
        # WRITE the live workspace. `CODEXCREW_WORKSPACE` is the documented override
        ...
        "CODEXCREW_WORKSPACE": str(home_dir / "workspace"),
        ...
        # bound. An inherited CODEXCREW_BIND (the official image exports
        ...
        "CODEXCREW_BIND": "127.0.0.1",
        # The pod's OWN venv leads PATH, ahead of cfg.gateway_path (which starts
        # with ~/.local/bin). Without this a bare `codexcrew` inside a pod — an
        # agent bash turn, a subprocess, `_codexcrew_bin()`'s "console-script on
    ...
    # gateway's CODEXCREW_BOUND_PORT (dashboard.server._export_bound_port).
    # Inside a pod env it would name the wrong plane — the pod's own
    # CODEXCREW_PORT above is the target — so drop it unconditionally rather
    # than rely on resolution precedence alone.
    env.pop("CODEXCREW_BOUND_PORT", None)
    return env
...
def write_pod_config(home_dir: Path, seed: str) -> None:
    ...
    # The pod's own workspace root (see build_pod_env's CODEXCREW_WORKSPACE).
...
def pod_context(cfg: PodConfig, name: str) -> tuple[Path, dict[str, str]]:
    """Resolve pod *name* to ``(its own codexcrew binary, its isolated env)``.
    ...
    if not checkout_str:
        raise PodError(
            f"pod {name!r} has no pinned checkout — run `codexcrew pod up {name}` "
            f"from inside a codexcrew checkout first"
        )
    ...
    if not (bin_path.exists() and os.access(bin_path, os.X_OK)):
        raise PodError(f"no codexcrew venv at {bin_path} (provision {name} first)")
    env = build_pod_env(cfg, cfg.home_dir(name), derive_port(cfg, name), checkout)
    return bin_path, env


# `pod exec` forwards to a real codexcrew, so it inherits the WHOLE CLI — including
...
# `service`): every verb below acts only on `CODEXCREW_HOME` state, which
...
#                    HOST registry, which is NOT covered by CODEXCREW_HOME or
...
#   browse         — writes browser auth state outside CODEXCREW_HOME
...
#                    `getattr(args, "port", None) or cfg…get("port", 5486)` — the
#                    CONFIG dashboard port, falling back to literal 5486, never
#                    `CODEXCREW_PORT`. A pod's config names no dashboard port, so it
...
#                    `CODEXCREW_PORT` — so this hazard is confined to `_tui`.
...
#                    `codexcrew pod logs NAME` reads the pod's own unit.
...
_POD_EQUIVALENT: dict[str, str] = {
    "stop": "codexcrew pod down {name}",
    "restart": "codexcrew pod down {name} && codexcrew pod up {name}",
    "gateway": "codexcrew pod up {name}",
    "logs": "codexcrew pod logs {name}",
}
...
def exec_in_pod(cfg: PodConfig, name: str, argv: list[str]) -> int:
    """``exec`` *argv* as the pod's own codexcrew, in the pod's isolated env.
    ...
    # edit land outside the pod even with CODEXCREW_WORKSPACE set correctly.
    workspace = Path(env["CODEXCREW_WORKSPACE"])
    try:
...
# --------------------------------------------------------------------------- #
# Boot — the ExecStart body. Re-entered as ``codexcrew pod _run <name>`` by the
...
def target_supports_flag(checkout: Path, flag: str) -> bool:
    ...
    resolves ONE codexcrew at ``pod install`` time and every instance re-enters it
    ...
    """
    try:
        src = (checkout / "src" / "codex_crew" / "cli.py").read_text(encoding="utf-8")
    except OSError:
        return False
...
def boot(cfg: PodConfig, name: str) -> int:
    ...
    if not checkout_str:
        print(
            ...
            f"`codexcrew pod up {name}` from inside a codexcrew checkout first"
        )
        return 3
    checkout = Path(checkout_str).expanduser()
    home_dir = cfg.home_dir(name)
    ...
    if not (bin_path.exists() and os.access(bin_path, os.X_OK)):
        print(f"FATAL: no codexcrew venv at {bin_path} (provision {name} first)")
        return 3
    if not (checkout / "src" / "codex_crew" / "static" / "dist").is_dir():
    ...
    if approval and approval not in APPROVAL_MODES:
        ...
        print(
            f"codexcrew-pod: ignoring unknown APPROVAL={approval!r} "
    ...
    if crons_raw and not crons:
        ...
        print(
            f"codexcrew-pod: ignoring unrecognised CRONS={crons_raw!r} "
    ...
    if scenario:
        try:
            ...
            return 3
        print(
            f"codexcrew-pod: seeded home from scenario {scenario!r}"
            if fresh
            else f"codexcrew-pod: home already populated — scenario {scenario!r} not re-applied"
        )
    if not scenario:
    ...
    print(f"codexcrew-pod: name={name} port={port} home={home_dir} checkout={checkout}")
    ...
    if target_supports_flag(checkout, "--no-tunnel"):
        ...
        # tried and the premise does not hold: ``CodexCrewConfig.load()`` deep-merges
        # ``config.local.json`` OVER ``config.json`` with the overlay winning, and
        # ``codexcrew config set`` writes that overlay by default -- so pinning
        ...
        print(
            "codexcrew-pod: --no-tunnel not found in this checkout's cli.py, so the "
