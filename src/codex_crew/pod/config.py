...
Every value is ``CODEXCREW_POD_*``-overridable so a test harness can stand up a
...
DEFAULT_UNIT_PREFIX = "codexcrew-pod"
...
def environment_vars(cfg: PodConfig) -> dict[str, str]:
    ...
    from the built-in default are emitted (plus ``CODEXCREW_POD_PATH``, always
    ...
    candidates: list[tuple[str, str, str | None]] = [
        ("CODEXCREW_POD_ROOT", str(cfg.pod_root), str(home / ".codexcrew-pods")),
        ("CODEXCREW_POD_ENV_DIR", str(cfg.pods_dir), str(_default_home() / "pods")),
        (
            "CODEXCREW_POD_ARTIFACTS_DIR",
            str(cfg.artifacts_dir),
            str(cfg.pod_root / ".e2e-artifacts"),
        ),
        ("CODEXCREW_POD_BASE_PORT", str(cfg.base_port), str(DEFAULT_BASE_PORT)),
        ("CODEXCREW_POD_LIVE_PORT", str(cfg.live_port), str(DEFAULT_LIVE_PORT)),
        ("CODEXCREW_POD_UNIT_PREFIX", cfg.unit_prefix, DEFAULT_UNIT_PREFIX),
        ("CODEXCREW_POD_PATH", cfg.gateway_path, None),  # always pin PATH
    ]
    out = {
        key: val
    ...
    if cfg.repo_hint is not None:
        out["CODEXCREW_POD_REPO"] = str(cfg.repo_hint)
    if cfg.worktrees_root is not None:
        out["CODEXCREW_POD_WORKTREES_ROOT"] = str(cfg.worktrees_root)
    return out
...
class PodConfig:
    ...
    checkout by :func:`codex_crew.pod.runtime.resolve_checkout` (git-native).
    """
    ...
    def load(cls) -> "PodConfig":
        home = Path.home()
        pod_root = _env_path("CODEXCREW_POD_ROOT", home / ".codexcrew-pods")
        # Pod env files are HOST-side state, so they follow the data-home move to
        # ~/.codex-crew. Use the DEFAULT home (not config_dir()) so a pod process
        # that has its own isolated CODEXCREW_HOME set can't redirect the host's
        # pod registry into the pod's throwaway home.
        pods_dir = _env_path("CODEXCREW_POD_ENV_DIR", _default_home() / "pods")
        artifacts_dir = _env_path("CODEXCREW_POD_ARTIFACTS_DIR", pod_root / ".e2e-artifacts")
        ...
        return cls(
            ...
            base_port=_env_int("CODEXCREW_POD_BASE_PORT", DEFAULT_BASE_PORT),
            live_port=_env_int("CODEXCREW_POD_LIVE_PORT", DEFAULT_LIVE_PORT),
            unit_prefix=os.environ.get("CODEXCREW_POD_UNIT_PREFIX", DEFAULT_UNIT_PREFIX),
            gateway_path=os.environ.get("CODEXCREW_POD_PATH", default_path),
            repo_hint=_env_path_opt("CODEXCREW_POD_REPO"),
            worktrees_root=_env_path_opt("CODEXCREW_POD_WORKTREES_ROOT"),
        )

    # ---- derived locations -------------------------------------------------
    def home_dir(self, name: str) -> Path:
        """Isolated ``CODEXCREW_HOME`` for pod *name* (nuked on stop)."""
