...
the shared ``codex_crew.messaging`` pipeline; the protocol layer is `neonize
<https://github.com/krypton-byte/neonize>`_ (Apache-2.0 Python bindings to
whatsmeow), an optional dependency installed with ``codexcrew[whatsapp]``.

Because the agent sends *as the operator*, two properties are load-bearing:

- **Echo discipline** (:mod:`codex_crew.whatsapp.echo`): the account's own
...
- **Deny-by-default reach** (:mod:`codex_crew.whatsapp.transport`): only the
  ...
  tool approval or session control (:mod:`codex_crew.whatsapp.group_gate`).
