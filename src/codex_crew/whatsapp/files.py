...
The channel-neutral half is :mod:`codex_crew.messaging.outbound_files`: it finds
...
memoized Pillow handle in :mod:`codex_crew.imaging`, and the refusal is spoken by
...
from codex_crew.imaging import image_dimensions, pil_available
...
def _decode_refusal(file: OutboundFile) -> Rejection | None:
    ...
    Header-only inspection: :func:`codex_crew.imaging.image_dimensions` reads the
...
def rejection_note(rejections: list[Rejection]) -> str:
    ...
    Written so it survives :func:`codex_crew.whatsapp.renderer.to_whatsapp_text`
