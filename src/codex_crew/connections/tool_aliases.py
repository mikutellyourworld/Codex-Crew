...
what the last pass emitted (:mod:`codex_crew.connections.alias_record`), so every
...
inertness); see :func:`codex_crew.agent._apply_connection_tool_aliases`.
...
5. **Provider identity is proven by endpoint, not assumed from the key.** A
   ...
   ``mcp_url``. (The ``x-codexcrew`` provenance marker cannot serve here: by
   design it appears only in files Codex Crew does not own, and is stripped on the
   way into both the store and the rendered spec -- see
   :mod:`codex_crew.mcp_provenance`.)
...
7. **Ownership of an already-written pair is PERSISTED, never inferred from its
   ...
   :mod:`codex_crew.connections.alias_record`, which now owns the decision. This
...
from codex_crew.connections.registry import get_all_registry_providers
...
def derived_alias(slug: str, tool: str) -> str:
    ...
    record in :mod:`codex_crew.connections.alias_record`.
    """
