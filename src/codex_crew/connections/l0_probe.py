...
human approval; see :mod:`codex_crew.connections.l0_record`.
...
CONSECUTIVE failures (:mod:`codex_crew.connections.l0_drift`) so one flaky night
...
from codex_crew.connections import l0_drift
from codex_crew.connections.l0_record import ObservedBaseline, RecordError, record_baselines
from codex_crew.connections.registry import (
...
def _staleness(*, fail_on_stale: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "overdue": overdue,
        "runbook": (
            "run `python -m codex_crew.connections.l0_probe --record` on a "
            "networked machine and commit src/codex_crew/connections/registry.json"
        ),
    }
