...
class Provider(_RequiredProviderFields, total=False):
    ...
    :mod:`codex_crew.connections.tool_aliases`), so declaring one does not rename
...
# Public so the baseline recorder (codex_crew.connections.l0_record) rewrites the
...
# refresh is `python -m codex_crew.connections.l0_probe --record` run by a human
