...
5. **The TRANSITION belongs to the writer of the spec, not to the alias pass.**
   :func:`~codex_crew.agent.rebuild_agent_config` opens and commits it around its own
...
6. **Both record writes are LOUD, and the pending write FAILS CLOSED.**
   :func:`~codex_crew.atomic_write.atomic_write` publishes through a temp file and
...
:func:`~codex_crew.agent.rebuild_agent_config` writes one canonical spec, and that
...
# Sidecar under the Codex Crew data home. kiro-cli validates agent specs with
# ``deny_unknown_fields``, so an in-spec ownership marker is impossible -- the
# record lives out of band in a directory Codex Crew owns outright, mirroring the
# ``owned-mcp-keys.json`` manifest that answers the same question for MCP server
# keys (see :mod:`codex_crew.browser.setup`).
