...
Relocated verbatim from ``codex_crew.dashboard.handlers.connections`` so the
...
``codex_crew.connections.__init__``: a package-level re-export would put this
module on the import path of every ``from codex_crew.connections import …``,
...
_RENDERED_AGENT_FILE = "codexcrew.json"
...
def _census_source_name(label: str) -> str:
    ...
    (``codexcrew``, the edition's extra scopes) and ``mirror:`` labels are left
    verbatim, because they name a Codex Crew-managed source rather than a file the
...
def agent_spec_sources(project_dirs: tuple[Path, ...] = ()) -> list[tuple[str, Path]]:
    ...
    ``codexcrew.json``, see :func:`mcp_discovery._load_agent_config` -- so a spec
    ...
    block a revoke, never purge targets (nothing in Codex Crew writes a
    ...
    is the specs Codex Crew does NOT own. Provider agent sidecars
    ...
    """
    from codex_crew.config.paths import kiro_agents_dir, project_agents_dir
    from codex_crew.dashboard.handlers.mcp import _extra_mcp_scopes
    ...
    # rendered ``codexcrew.json`` and from every ``scope.agent_mcp_file`` itself,
    ...
    # Codex Crew never writes it, so its entry keeps its own grant and must be
...
def spec_census(
    ...
    from codex_crew.mcp_discovery import SCOPE_CODEXCREW, _extra_scope_sources, _mcp_sources
    ...
    specs.setdefault(SCOPE_CODEXCREW, {})
...
async def remove_provider_entry(
    ...
    * a census that missed custom agent configs revoked a grant an agent outside
      Codex Crew was authorized by (see :func:`agent_spec_sources`);
    ...
    * a revoke that ran after the lock released deleted the grant of an entry
    ...
    the pair the card matches on and the rule :mod:`codex_crew.connections.l1_smoke`
    ...
    disabled entries outside the Codex Crew scope, and a user's switched-off server
    ...
    see :func:`codex_crew.connections.warm.rearm_invalidated_provider` -- and only
    ...
    from codex_crew.mcp_grant import grant_key, revoke_local_grant, surviving_grant_artifacts
    ...
    def _judge(
        ...
        a purge target -- Codex Crew does not own a user's agent file.
    ...
    async with _get_mcp_lock():
        ...
        # ``~/.kiro/agents/codexcrew.lock`` (apps/bridges._mcp_lock), so they are
    ...
    if not residue:
        from codex_crew.connections.warm import rearm_invalidated_provider
