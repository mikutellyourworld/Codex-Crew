...
def _save_connected_since(recorded: dict[str, str]) -> bool:
    ...
    """
    from codex_crew.agent import _atomic_json_write  # circular import at module scope
...
def _reconcile_connected_since_locked(statuses: list[ConnectionStatus], now: str) -> dict[str, str]:
    ...
    if fresh:
        ...
        from codex_crew import hooks as _hooks  # circular import at module scope
...
def _classify(granted: bool | None, mint_state: str, *, shared: bool = False) -> tuple[str, str]:
    ...
    click (:mod:`codex_crew.connections.warm`), holding a URL nobody has asked for. It
...
async def collect_connection_statuses() -> list[ConnectionStatus]:
    ...
    """
    from codex_crew.connections.mint import pending_mint_for
