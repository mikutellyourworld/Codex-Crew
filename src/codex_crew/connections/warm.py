...
The cold path (:mod:`codex_crew.connections.mint`) pays a full kiro-cli spawn PER provider
...
``generation`` / ``activation`` on :class:`~codex_crew.connections.mint.MintState`) and the
...
the same reasoning
:func:`~codex_crew.connections.mint._new_mint_token` records for the cold engine. WITHDRAWAL
...
from codex_crew import hooks, platform_compat
...
from codex_crew.config.paths import project_agents_dir
from codex_crew.connections.mint import (
    _MINT_AGENT_PREFIX,
...
from codex_crew.connections.registry import Provider, get_visible_providers
from codex_crew.connections.tool_aliases import declared_tool_aliases, resolve_tool_aliases
from codex_crew.mcp_discovery import list_servers
from codex_crew.mcp_grant import grant_presence as grant_present
...
_WARM_SPEC_SENTINEL = "Codex Crew warm mint spec (machine-written; safe to delete)"
...
_WARM_GENERATION_SENTINEL = "codexcrew.connections.warm.generation"
_WARM_GENERATION_MARKER_VERSION = 1
_WARM_GENERATION_MARKER_MAX_BYTES = 4096
_WARM_RUNTIME_DIR_ATTR = "_codexcrew_warm_generation_dir"
...
def _acp_runtime_factory() -> Any:
    ...
    this module carries no ``codex_crew.acp`` import: application code may not reach the
    ACP layer (``scripts/check_agent_sdk_boundary.py``), and the sanctioned surface
    ``codex_crew.agent_sdk`` is deliberately EMPTY -- its RFC schedules runtime ownership
    ...
    """
    from codex_crew.session import _load_bg_runtime_types
...
def _warm_activation_candidates(universe: list[Provider]) -> list[Provider]:
    ...
    the two-pass race :func:`~codex_crew.mcp_grant.grant_presence` refuses -- a failure that
...
def _is_stale_warm_spec(stem: str, plan_names: frozenset[str]) -> bool:
    ...
    (``codexcrew-mint-warm-foo-4821-9ab3c1de``), so prefix plus not-in-plan alone would
...
def scavenge_warm_mint_artifacts() -> int:
    ...
    An unsentinelled global file is deliberately retained: its name does not prove Codex Crew
...
async def _adopt_shared_row(slug: str, mcp_url: str) -> str | None:
    ...
    every write in :func:`~codex_crew.connections.mint._mint_watcher` is guarded on the
...
def _credential_bearing_slugs(urls: dict[str, str]) -> set[str]:
    ...
    The gate is :func:`~codex_crew.security.oauth_url_contains_credential` -- the same
