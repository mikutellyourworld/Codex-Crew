...
Runs as ``codexcrew mcp-computer``: kiro-cli spawns it as a child process and calls
...
``codexcrew.caller-identity``, so gatewayd injects one whenever it can name the
caller — the only identity source that works on a pooled backend serving many
sessions), then the env var, then ``CODEXCREW_HOST_PID`` plus the HMAC sidecar
...
``CODEXCREW_SESSION_KEY`` reaches a child only from a launcher that already knows
which session it spawns for (the ACP spawn path in ``acp/client.py``, the
script-cron launcher in ``cron_script.py``) and ``CODEXCREW_HOST_PID`` only from the
...
from codex_crew.mcp_caller import current_tenant_nonce
...
from codex_crew.validation import MCP_COMPUTER_SCHEMAS, ValidationError, validate_tool_args

logger = logging.getLogger(__name__)

SERVER_NAME = "codexcrew-computer"
...
ERR_GATEWAY_UNREACHABLE = (
    "the CodexCrew gateway is not reachable, so computer use cannot run "
        ...
        },
        {
            ...
            "inputSchema": {
                ...
                "properties": {
                    ...
                    # only to an element CodexCrew can inspect: with no index there
