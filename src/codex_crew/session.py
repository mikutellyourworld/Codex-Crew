...
from codex_crew import model_registry, platform_compat, shutdown_event
...
from codex_crew.acp_backends import selectable_backends
from codex_crew.agent import kiro_agents_dir_path
from codex_crew.agent_discovery import _read_agent_spec, spec_model
from codex_crew.agent_sdk.backend_identity import is_claude_backend_name
...
from codex_crew.constants import COMPACT_WAIT_TIMEOUT_SECS
from codex_crew.executors import maintenance_executor, subprocess_executor
from codex_crew.mcp_gateway.abort import schedule_abort
...
from codex_crew.metrics.events import SESSION_IDLE_EXPIRED, emit_counter
from codex_crew.metrics.provider import get_recorder
from codex_crew.providers.base import CancelOutcome, LLMProvider
from codex_crew.pycache_gc import PYCACHE_GC_INTERVAL_SECS, prune_pycache
from codex_crew.sandbox import cleanup_stale_sandbox_profiles
from codex_crew.sel import sel
from codex_crew.session_allocation import (
    AllocationConstants,
    AllocationDeps,
    SessionAllocationService,
)
from codex_crew.session_allocation import SessionBusyError as SessionBusyError  # noqa: F401
from codex_crew.session_allocation import (
    SessionClosingError,
    SessionRegistryState,
)
from codex_crew.session_allocation import (  # noqa: F401
    SpeculativeResumeRefused as SpeculativeResumeRefused,
)
from codex_crew.session_allocation import (
    _collect_parent_runtime_kwargs,
)
from codex_crew.session_background import (
    BackgroundRuntimeDeps,
    BackgroundSessionRuntime,
    _ProviderBgSession,
)
from codex_crew.session_cleanup import CleanupDeps, CleanupState, SessionCleanup
from codex_crew.session_compaction import (
    CompactionCoordinator,
    CompactionDeps,
    CompactionState,
)
from codex_crew.session_lifecycle import (
    SessionLifecycleConstants,
    SessionLifecycleDeps,
    SessionLifecycleService,
    SessionLifecycleState,
)
from codex_crew.session_map import _kiro_sessions_dir  # noqa: F401
from codex_crew.session_map import MIRROR_OPT_OUT_FLAG
from codex_crew.session_map import SessionMap as SessionMap  # noqa: F401
from codex_crew.session_map import UnbindListener, set_unbind_listener
...
from codex_crew.session_pid import _track_child_pids as _track_child_pids  # noqa: F401
from codex_crew.session_pid import _track_pid as _track_pid  # noqa: F401
from codex_crew.session_pid import _track_session_pid as _track_session_pid  # noqa: F401
from codex_crew.session_pid import _untrack_child_pids as _untrack_child_pids  # noqa: F401
from codex_crew.session_pid import _untrack_pid as _untrack_pid  # noqa: F401
from codex_crew.session_pid import _untrack_session_pid as _untrack_session_pid  # noqa: F401
from codex_crew.session_pid import (
    cleanup_orphaned_session_roots,
)
from codex_crew.session_pid import (  # noqa: F401
...
from codex_crew.session_pool import WarmPoolDeps, WarmSessionPool
from codex_crew.stats import Stats
from codex_crew.watchdog import CleanupHook, SessionWatchdog
...
def _is_claude_backend(provider: Any) -> bool:
    ...
    """
    from codex_crew.providers.acp import AcpProvider  # circular import: providers -> session
...
def _provider_label(provider: Any) -> str:
    ...
    """
    from codex_crew.providers.acp import provider_label  # circular: providers -> session
...
def _resolve_allocation_crew_identity(
    ...
    from codex_crew.config.loader import resolve_crew_identity

    return resolve_crew_identity(cfg, agent, crew_agent)


def _load_allocation_watchdog_settings(crew: str) -> object:
    from codex_crew.acp.session_handle import _load_watchdog_settings
...
def detect_provider_switch(session_map: "SessionMap", session_key: str, new_provider: str) -> bool:
    ...
    is achieved via CodexCrew's own history replay (build_session_replay), never
...
#: Every value the ``codexcrew.session.pool.decision`` counter can report. A
...
BACKGROUND_AGENT = "codexcrew-lite"
...
def _load_bg_runtime_types() -> tuple[Any, type[BaseException]]:
    ...
    from codex_crew.acp.runtime import AcpRuntime, AcpRuntimeDead

    return AcpRuntime, AcpRuntimeDead


# Heartbeat session key — used by HeartbeatService.  Spawned with the full
# ``codexcrew`` agent so polled tasks can call read-only MCP tools (CR/ticket
...
def _session_model(cfg: "CodexCrewConfig", agent: str | None) -> "str | None":
    ...
    jobs and spawned agents pass a CodexCrew agent (crew) name. Both are handled
...
class FirstTurnState(Enum):
    ...
    NOTHING_ARMED = auto()
    # Fresh session: the first real turn injects Codex Crew history.
    FRESH = auto()
...
class _Session:
    ...
    # from CodexCrew's conversation_log. Ensures replay fires exactly once
...
class SessionManager:
    ...
    def _allocation_deps(self) -> AllocationDeps:
        ...
        return AllocationDeps(
            ...
            load_config=lambda: CodexCrewConfig.load(),
    ...
    def _sync_autocompact_pct(self) -> None:
        ...
        ``CodexCrewConfig.load`` now publishes it, and prompt assembly loads
        config once per turn, so a write from ANY writer -- the dashboard PATCH
        handler or ``codexcrew config set`` -- is in force by the next context
    ...
    def _record_pool_decision(self, decision: str, key: str) -> None:
        ...
        try:
            get_recorder().counter(
                "codexcrew.session.pool.decision",
                1,
                attrs={
    ...
    def _companion_runtime_pids(self) -> set[int]:
        ...
        - ``self._bg_runtime`` — the background runtime backing ``get_bg_session``
          (codexcrew-lite title-gen / memory consolidation), plus any
