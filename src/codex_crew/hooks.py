"""Config-driven hook system for CodexCrew's message pipeline.
...
from codex_crew import platform_compat, security, webhooks
...
from codex_crew.config import paths as _config_paths
...
from codex_crew.pinned_fs import fd_real_path as _fd_real_path
from codex_crew.platform import current_context, redact_via_context
...
from codex_crew.platform.tool_paths import (  # noqa: F401  (re-exported for callers)
...
from codex_crew.session_directive import CORE_MCP_SERVER
from codex_crew.validation import _bounded_pattern_search
...
class ToolHookResult:
    ...
    def _count(action: str, security_deny: bool) -> None:
        ...
        """
        try:
            from codex_crew.metrics.events import APPROVAL_DECISIONS, emit_counter
...
class HookManager:
    ...
    def on_tool_call(
        ...
        *,
        ...
        # CodexCrew-side read-only auto-approve — the LAST branch before allow(),
        ...
        if is_shell:
            ...
            from codex_crew.dashboard.state import is_read_only_bash

            if command and is_read_only_bash(command):
                return ToolHookResult.auto_approve()
        else:
            from codex_crew.slack.gateway import _is_read_only_tool
            ...
            # leading read-ish verb and rejects EVERY `mcp__codexcrew-computer__*` title
...
def _governance_denial(
    ...
    *tool_name* because the title grammar cannot encode every identity; both are
    ...
    try:
        from codex_crew.platform.governance import gate_decision
        from codex_crew.platform.governance_profiles import resolve_active_scope
...
def _app_owns_mcp_server(mcp_server_name: str, app: str) -> bool:
    ...
    ``<app>:``-prefixed (host/managed servers such as ``codexcrew-cron`` never
    match).
    """
...
def _builtin_app_for_agent(resolved_agent: str) -> str:
    ...
    ``codexcrew`` — so an alias NAMED after a builtin's agent would otherwise lend
...
# ``codex_crew.platform.tool_paths`` (imported at module top) so the governance
...
def _pinned_replace(
    ...
    # this call created.
    tmp_name = f".{base}.codexcrew-{os.getpid()}-{uuid.uuid4().hex}.tmp"
...
_INTERNAL_READ_ALLOWLIST: dict[str, str] = {
    # ``codex_crew.dashboard.handlers.kiro_usage_api`` reads the kiro-cli SSO
    ...
    # trusted CodexCrew with the session by running ``kiro-cli login`` outside
...
def register_internal_read_path(read_id: str, rel_path: str) -> None:
    ...
    * the resolved ``~/<rel_path>`` must already be classified sensitive by
      :func:`codex_crew.security.is_sensitive_path` — the carve-out is only valid
...
_AUDIT_ONLY_READ_IDS: dict[str, str] = {
    ...
    # Read read-only by ``codex_crew.dashboard.handlers.kiro_usage_api`` for the
    ...
    # Same store, read by ``codex_crew.kiro_cli.signed_in_via_idc`` to answer one
    ...
    # Same store, read by ``codex_crew.kiro_prerequisite.identity_fingerprint`` to
    ...
    # ``codex_crew.mcp_grant.grant_present`` STATS the paired
    ...
    # status module (``codex_crew.connections.status``) STATS the identical
...
_HOOK_BASE_ENV_KEYS: frozenset[str] = frozenset(
    {
        ...
        # CODEXCREW_HOOK_* metadata vars). A hook that depended on an ambient var
        ...
        "LOCALAPPDATA",
        "XDG_CONFIG_HOME",
        # Data home — a hook that invokes the ``codexcrew`` CLI (or otherwise
        # reads the instance's data dir) must resolve the gateway's overridden
        # home, not the default ``~/.codex-crew``. It is a path, not a credential,
...
def _hook_subprocess_env(hook: "ScriptHook", context: str) -> dict[str, str]:
    ...
    ``CODEXCREW_HOOK_CONTEXT`` is capped for a Stop event only: the env var is
    ...
    env["CODEXCREW_HOOK_EVENT"] = hook.event
    env["CODEXCREW_HOOK_CONTEXT"] = context[:500] if hook.event == HOOK_EVENT_STOP else context
    return env
...
class ScriptHookStore:
    """Persist script hooks to ~/.codex-crew/hooks.json."""

    def __init__(self, config_dir: Path | None = None):
        from codex_crew.config.loader import config_dir as _cfg_dir
    ...
    async def fire(
        ...
        only the ``CODEXCREW_HOOK_CONTEXT`` env var is length-capped downstream in
