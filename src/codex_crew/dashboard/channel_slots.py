...
* **Recency-bounded.** Only sessions modified within the dashboard's configured
  ...
  window, matching :func:`~codex_crew.dashboard.chat_persistence.restore_recent_sessions`.
...
from codex_crew.dashboard.channel_folders import lookup_channel_folder
from codex_crew.dashboard.chat_title import _persist_title
from codex_crew.dashboard.chat_utils import effective_session_key
from codex_crew.dashboard.state import _normalize_slot_key, durable_row_count, row_mid
from codex_crew.history import carry_provenance, is_incognito_transcript
from codex_crew.loop_lock import LoopBoundLock
from codex_crew.messaging.link import channel_namespace_of, is_channel_session_key
from codex_crew.messaging.upload_gate import live_dashboard_slot
...
def channel_slot_name(session_key: str) -> str:
    ...
    :meth:`~codex_crew.session.SessionManager.channel_key_for_stem`.
    """
...
def surface_channel_session(
    ...
    *folder_id* is the channel's configured session folder (see
    :mod:`codex_crew.dashboard.channel_folders`), applied only when the session
    ...
    *meta* (written atomically with the filing marker by the reconcile pass) are
    ...
    if meta.get("autocompact_pct") is not None:
        ...
        from codex_crew.dashboard.chat_persistence import _validate_autocompact_pct
    ...
    from codex_crew.dashboard.chat_tags import validate_folder_tag_ids
    ...
    for s in pending:
        ...
        if to_file:
            ...
            try:
                ...
                from codex_crew.dashboard.chat_tags import (
                    tags_write_lock,
