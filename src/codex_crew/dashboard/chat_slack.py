...
from codex_crew.dashboard.chat_backfill import (
    backfill_content,
    gap_summary,
...
from codex_crew.dashboard.state import DashboardState, _log_task_exception
from codex_crew.messaging.link import SLACK_NAMESPACE
...
from codex_crew.slack.channel_resolver import _CACHE_FILENAME, ChannelNameResolver
...
from codex_crew.sync_bridge import handoff_to_slack
...
def _get_channel_resolver(state: DashboardState) -> ChannelNameResolver:
    ...
    resolving to the real ``~/.codex-crew`` dir in production.
    """
...
def _format_backfill_parts(content: str, icon: str) -> list[str]:
    ...
    Thin delegate to :func:`codex_crew.slack.format.render_for_slack`, which owns
...
async def drain_slack_backfill(
    ...
    if selection.skipped_turns and selection.recent:
        ...
        link = ""
        try:
            # Offloaded: CodexCrewConfig.load() reads and validates the config
