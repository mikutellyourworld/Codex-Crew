...
def register(app: web.Application) -> None:
    ...
    _is_dev_env = os.environ.get("CODEXCREW_HOME", "").endswith("-dev")
    if _is_dev_env or env_flag_enabled("CODEXCREW_DEV_MODE"):
    ...
    # Session revocation (called by `codexcrew logout` CLI)
    ...
    for _builtin_name in BUILTIN_NAMES:
        try:
            _mod = importlib.import_module(f"codex_crew.apps.builtins.{_builtin_name}")
            if hasattr(_mod, "register_routes"):
                _mod.register_routes(app)
        except ModuleNotFoundError as exc:
            if exc.name != f"codex_crew.apps.builtins.{_builtin_name}":
                raise
