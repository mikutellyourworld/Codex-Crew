...
from codex_crew.dashboard.chat_compaction_notice import deliver_channel_compaction_notice
from codex_crew.dashboard.dashboard_persistence import DashboardPersistenceCoordinator
from codex_crew.dashboard.folder_repository import FOLDERS_FILE, FolderRepository
from codex_crew.dashboard.interaction_coordinator import (
    ApprovalCoordinator,
    QuestionCoordinator,
)
from codex_crew.dashboard.notification_coordinator import NotificationCoordinator
from codex_crew.dashboard.remote_mirror import mirror_frame as _mirror_relay_frame
from codex_crew.dashboard.session_pulse_counter import increment_user_session_count_off_loop
from codex_crew.dashboard.side_state import SideState
from codex_crew.dashboard.slot_buffers import SlotBufferCoordinator
from codex_crew.dashboard.slot_projection import SlotProjection
from codex_crew.dashboard.slot_queue_repository import SlotQueueRepository
from codex_crew.dashboard.slot_registry import SlotRegistry
from codex_crew.dashboard.system_notices import is_system_notice
from codex_crew.dashboard.websocket_hub import WebSocketHub
from codex_crew.deny_guidance import remediation_for
from codex_crew.history import latest_transcript_ts, mint_row_mid, monotonic_transcript_ts
...
from codex_crew.messaging.renderer import display_safe
...
from codex_crew.notifications.rate_limit import AppRateLimiter
from codex_crew.notifications.resource_pressure import ResourcePressureNotifier
from codex_crew.notifications.settings import ChannelSettings
from codex_crew.preview_text import strip_markdown_preview
from codex_crew.release_channel import channel as _release_channel_of_build
from codex_crew.safety_override import cached_disabled_approval_modes, safety_override
...
if TYPE_CHECKING:
    ...
    from codex_crew.dashboard.loop_watchdog import LoopStallWatchdog  # noqa: F401
    from codex_crew.messaging.transport import MessagingTransport  # noqa: F401
    from codex_crew.power import SleepInhibitor  # noqa: F401
    from codex_crew.slack.outbound import PostedOptions  # noqa: F401
...
# entrypoint via set_build_info() — AFTER CODEXCREW_PROJECT_DIR is detected and
...
def build_refusal_recovery_prompt(
    ...
    lines = [
        (
            ...
            "Codex Crew safety policy, which ended the turn early. This was NOT a "
...
_DENY_CAUSE_TEXT: dict[str, tuple[str, str]] = {
    DENY_CAUSE_POLICY: (
        "was blocked by a Codex Crew safety policy",
...
def build_refusal_steer_notice(
    ...
    *cause* selects the wording. The distinction is not cosmetic: a policy block
    ...
    return (
        f"[Codex Crew host notice] The tool call you just made {clause}. "
...
def build_tool_stall_recovery_prompt(
    ...
    lines = [
        ...
        f"~{idle_mins} minute(s) and the turn was ended by a Codex Crew watchdog. "
...
def _ascii_slot_key(name: str) -> str:
    """Return *name* with any character outside printable ASCII replaced by ``-``.
     ...
     codexcrew-core sends as the ``X-Session-Key`` HTTP header on every gateway
...
class _ChatSlot:
    ...
    def __init__(
        ...
        # conversation (see codex_crew.dashboard.channel_folders). Persisted,
        ...
        # ceiling cut the turn (codexcrew.turn.timeout.cause).
    ...
    def set_mcp_report(self, report: dict[str, Any] | None, session_id: str = "") -> bool:
        ...
        :class:`codex_crew.acp.mcp_session_report.McpSessionReport`, which owns
...
class DashboardState:
    ...
    def __init__(
        ...
        # Published after the serving boundary and durable session restoration,
        # but before optional remote tunnel authentication. The listener binds
        # earlier; /api/ready stays 503 while required dashboard state is restored,
        # then turns 200 even if an optional tunnel is still connecting.
        ...
        self._tunnel_start_task: Any = None  # optional post-readiness connect task
        self._tunnel_shutting_down: bool = False  # blocks late startup publication
        ...
        from codex_crew.dashboard.file_index import FileIndexRegistry
    ...
    def wire_session_compact_callback(self) -> None:
        ...
        async def _on_compacted(key: str, pct: float, *, success: bool) -> None:
            from codex_crew.dashboard.chat_utils import dashboard_slot_key
    ...
    def get_or_create_slot(
        ...
        *linked_session_key* binds a new slot to the session its conversation
        ...
        try:
            if self.sessions:
                ...
                if slot._slack_linked:
                    ...
                    from codex_crew.history import _safe_key
        ...
        if self.sessions:
            try:
                from codex_crew.dashboard.chat_utils import _sync_dashboard_slots
    ...
    def load_chat_pins(self) -> None:
        ...
        from codex_crew.dashboard.chat_pins import (
...
def _governance_status() -> str:
    ...
    try:
        from codex_crew.platform.governance_health import governance_status
...
def _cached_check_status(url: str) -> dict | None:
    ...
    from codex_crew.dashboard.handlers.source_providers import get_cached_check_status
...
def _repo_is_public(url: str) -> bool | None:
    ...
    """
    from codex_crew.dashboard.handlers.source_providers import is_repo_public
