...
from codex_crew.dashboard.create_rate_limit import FOLDER_CREATE, allow_create
from codex_crew.dashboard.state import DashboardState
from codex_crew.dashboard.token_auth import caller_names_a_missing_slot, derive_caller_app
...
#: :data:`~codex_crew.dashboard.state.MAX_LIVE_SLOTS`. Folder creation had no bound
...
_KNOWN_INTERNAL_CALLERS = frozenset({"codexcrew-dashboard"})
...
async def api_chat_slot_mode(request: web.Request) -> web.Response:
    ...
    # (gateway -> codex_crew.dashboard -> chat_folders), and crew is a
    ...
    from codex_crew.crew_chat import CrewOrchestrator, is_crew_capable_slot_key
