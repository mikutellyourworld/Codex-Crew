...
Split out of :mod:`codex_crew.dashboard.origin` so callers that only need URL
...
Why it matters: ``codexcrew`` CLI invocations and every MCP stdio subprocess
...
_CANONICALIZABLE_LOOPBACK_HOSTS = frozenset(
    {"127.0.0.1", "::1", "localhost", "codexcrew.localhost"}
)
...
def is_loopback(host: str) -> bool:
    ...
    if host in ("localhost", "127.0.0.1", "::1", "codexcrew.localhost"):
        return True
    try:
...
def dashboard_socket_name(port: int) -> str:
    ...
    whose ``CODEXCREW_HOME`` is the pod's isolated home, so it must join this name
...
def parse_dashboard_url(url: str) -> tuple[str, int]:
    ...
    ``CODEXCREW_PORT`` env var always overrides the port (dev mode).
    """
    if not url:
    ...
    if env_port:
        try:
            ...
            logger.warning(
                "CODEXCREW_PORT=%r is not a valid integer; using port %d from config",
                env_port,
                port,
            )
    return host, port
...
def devspaces_proxy_url(port: int) -> str | None:
    ...
    its origin via ``dashboard.url`` or ``CODEXCREW_CORS_ORIGINS``.
    """
    return None
...
_ENV_BIND = "CODEXCREW_BIND"


def bind_address_for(local_only: bool) -> str:
    """Return the TCP bind address string for aiohttp.

    ``CODEXCREW_BIND`` (env) overrides the bind ADDRESS only — nothing else.
    ...
    the port to be exposed. The official image sets ``CODEXCREW_BIND=0.0.0.0``;
    ...
    * ``local_only`` semantics are UNCHANGED — dashboard URLs, the CSRF
      ...
      ``-p 5486:5486`` + ``http://localhost:5486`` mapping). Only the TCP
      bind widens.
...
def resolve_dashboard_host(local_only: bool, configured_host: str = "") -> str:
    ...
    if local_only:
        ...
        # ``codexcrew.localhost`` (and other ``*.localhost`` names) are NOT
        ...
        # honor — so canonicalizing onto ``codexcrew.localhost`` would 302 those
...
def should_canonicalize_host(
    ...
    *,
    ...
    Converges loopback aliases (127.0.0.1 / localhost / codexcrew.localhost) onto
...
def format_dashboard_urls(
    ...
    *,
    ...
    if _is_remote:
        ...
        lines: list[str] = [
            f">_ Dashboard: ssh -NL {port}:localhost:{port} {mh}",
            f"             then open http://localhost:{port}{_qs}",
        ]
    else:
        lines = [">_ Dashboard:", f"   {authed_url}"]
        ...
        if mh_local and mh_local != "localhost":
            try:
                ...
                if ip and ip != "127.0.0.1":
                    lines.append(f">_ Remote:    ssh -NL {port}:localhost:{port} {mh_local}")
            except Exception:
                pass

    proxy = devspaces_proxy_url(port)
    if proxy and not local_only:
        lines.append(f">_ Proxy:     {proxy}{_qs}")

    if _is_remote:
        lines.append(">_ Run 24/7:  see docs/guides/remote-and-mobile.md for systemd service setup")
...
def build_allowed_origins(
    ...
    *tailnet_host* is this machine's MagicDNS name when tailnet access is
    ...
    origins: set[str] = {
        ...
        f"http://codexcrew.localhost:{port}",
    }
    ...
    for _co in os.environ.get("CODEXCREW_CORS_ORIGINS", "").split(","):
        if _co.strip():
    ...
    for _p in os.environ.get("CODEXCREW_ALLOWED_LOOPBACK_PORTS", "").split(","):
        _p = _p.strip()
        if _p.isdigit():
            origins.add(f"http://127.0.0.1:{_p}")
            origins.add(f"http://localhost:{_p}")
            origins.add(f"http://[::1]:{_p}")
            origins.add(f"http://codexcrew.localhost:{_p}")
    return origins
...
def build_allowed_hosts(allowed_origins: set[str]) -> set[str]:
    ...
    rejected.
    """
    hosts: set[str] = {"localhost", "127.0.0.1", "::1", "codexcrew.localhost"}
