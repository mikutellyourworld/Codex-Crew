...
i.e. ``~/.codex-crew`` or ``$CODEXCREW_HOME``)
...
from codex_crew.platform_compat import pid_exists, process_start_time
...
def get_dumps_dir() -> Path:
    ...
    ``config_dir()`` resolves to ``~/.codex-crew`` (or ``$CODEXCREW_HOME`` when
...
def open_dump_file(dumps_dir: Path | None = None) -> DumpFile:
    ...
    f.write(f"# CodexCrew loop-stall crash dump — opened {ts}\n")
...
def dump_first_stack_lines(dump_path: Path, max_lines: int = 5) -> list[str]:
    ...
    ``codexcrew doctor`` does) actively misleads whoever is diagnosing the
