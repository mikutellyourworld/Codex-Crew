...
- Persistence: ``~/.codex-crew/refresh_chains.json`` (mode ``0600``).
"""
...
from codex_crew.dashboard.revocation_gen import (
    current_revocation_gen,
    current_revocation_gen_or_none,
)
from codex_crew.dashboard.token_secret import _get_secret
...
# will be forced through the regular ``codexcrew token`` URL flow again
...
class RefreshStateManager:
    ...
    window (the user simply re-mints via the ``codexcrew token`` URL) and keeping
    ...
    def _persist(self) -> None:
        ...
        with self._lock:
            ...
            try:
                ...
                # window and a local co-tenant able to enumerate ~/.codex-crew
...
def generate_refresh_token(
    ...
    *,
    ...
    Omit it to start a fresh chain (initial mint after ``codexcrew token``
    URL is consumed).
    ...
    payload_dict = {
        ...
        # (codexcrew logout) ends refresh chains, not just access cookies.
...
def validate_refresh_token(token: str) -> tuple[bool, str, str, str, str, float]:
    ...
    # cookie types — revoke_all_sessions() (codexcrew logout) bumps it once and
