...
tool's OWN return over the MCP pipe; this string is recorded on CodexCrew's
...
def _audit(session_key: str, kind: str, outcome: str) -> None:
    ...
    """
    try:
        # Local import: codex_crew.sel transitively pulls config -> apps ->
...
def _binding(session_key: str) -> str | None:
    from codex_crew.autonudge import binding_key_for

    return binding_key_for(session_key)


def _structured_binding(session_key: str) -> str | None:
    from codex_crew.autonudge import structured_monitor_binding_key_for
...
async def _monitor_watch(state: Any, session_key: str, args: dict[str, Any]) -> str:
    from codex_crew.autonudge import get_instance
    from codex_crew.autonudge_authz import authorize_and_add_nudge
    from codex_crew.monitoring.models import MonitorBudgets, MonitorState
...
async def _monitor_update(state: Any, session_key: str, args: dict[str, Any]) -> str:
    from codex_crew.autonudge import get_instance, is_structured_monitor_loop
    from codex_crew.autonudge_authz import authorize_and_update_nudge
...
async def _structured_monitor_update(state: Any, svc: Any, loop: Any, patch: dict[str, Any]) -> str:
    from codex_crew.autonudge_authz import authorize_and_update_monitor
...
def _structured_stop_reason(args: dict[str, Any]) -> str:
    from codex_crew.monitoring.models import MAX_MONITOR_STOP_REASON_CHARS
    from codex_crew.security import redact_and_truncate
...
async def _monitor_stop(session_key: str, args: dict[str, Any]) -> str:
    from codex_crew.autonudge import get_instance, is_structured_monitor_loop
    from codex_crew.autonudge_authz import authorize_and_stop_monitor
...
async def _set_project(state: Any, slot: Any, args: dict[str, Any]) -> str:
    ...
    if rp != old_project:
        slot._pending_reset_history_key = effective_session_key(slot)
        try:
            from codex_crew.dashboard.chat_handlers import _save_recent_project
...
async def _suggest_followup(state: Any, slot: Any, args: dict[str, Any]) -> str:
    from codex_crew.dashboard.chat_handlers import _redact_followup_item
