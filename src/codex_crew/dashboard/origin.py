...
:mod:`codex_crew.dashboard.urls` leaf and is re-exported below, so ``from
codex_crew.dashboard.origin import ...`` keeps working for every existing caller
...
from codex_crew.dashboard.urls import (  # noqa: F401
    _BIND_ALL,
    _BIND_LOCAL,
...
def is_direct_local_request(request: web.Request) -> bool:
    ...
    proxies (including managed tunnel providers, nginx, Caddy, and ALB) attach
...
def is_proxied_request(request: web.Request) -> bool:
    ...
    * **upstream** — a proxy on *another* host (an nginx box, a container
      ...
      (``CODEXCREW_BIND``), which presents a NON-loopback peer.
...
def check_origin(
    ...
    *,
    ...
    # ``CODEXCREW_ALLOWED_LOOPBACK_PORTS`` (folded into the allowed set above).
    return False
...
# also NOT expressible — ``http://[::1]:5486`` is refused by the browser with "the
...
def frame_ancestors_value(extra: "Iterable[str]" = ()) -> str:
    ...
    * **``'self'`` alone is not enough, because the directive is matched against
      EVERY ancestor.** Codex Crew frames at depth: the Instances embed puts a remote
