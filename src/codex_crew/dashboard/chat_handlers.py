...
from codex_crew import members as members_mod
from codex_crew import model_registry
from codex_crew.acp.client import AcpModelUnavailable
from codex_crew.agent_discovery import cached_project_agent_names, warm_project_agent_names
...
from codex_crew.dashboard import remote_mirror
from codex_crew.dashboard.channel_slots import channel_slot_name, note_slot_closed
from codex_crew.dashboard.chat_auto_tag import maybe_auto_tag
...
from codex_crew.dashboard.chat_folders import (
    _resolve_folder_project_dir,
    _unhide_folder,
)
from codex_crew.dashboard.chat_orchestrator import _stage_loop
...
from codex_crew.dashboard.chat_tags import tags_write_lock, validate_folder_tag_ids
from codex_crew.dashboard.chat_title import _maybe_auto_title
...
from codex_crew.dashboard.remote_relay import (
    RemoteTurnError,
    create_peer_slot,
...
from codex_crew.dashboard.system_notices import SESSION_RELOAD_KIND, is_system_notice
from codex_crew.dashboard.turn_dispatch import spawn_guarded_turn
from codex_crew.history import carry_provenance, is_incognito_transcript, transcript_stems
from codex_crew.messaging.link import is_channel_session_key
...
from codex_crew.sandbox import voice_runtime_workspace_conflict
...
from codex_crew.session_summary import count_user_turns_in_records
from codex_crew.trust_patterns import (
...
async def api_chat(request: web.Request) -> web.StreamResponse:
    ...
    if requested_mode == "crew" and slot_name:
        ...
        from codex_crew.crew_chat import is_crew_capable_slot_key
    ...
    if slot.mode == "member":
        ...
        _member_cfg = await asyncio.to_thread(CodexCrewConfig.load)
    ...
    if slot.agent not in (None, ""):
        ...
        if agent and slot.agent != agent:
            ...
            try:
                ...
                _cfg = await asyncio.to_thread(CodexCrewConfig.load)
    ...
    # (the system prompt and the codexcrew-commands / web-browse skills tell it
...
def _generate_state(cfg: CodexCrewConfig, slot: Any) -> str:
    ...
    for aid, fut in list(slot._approval_futures.items()):
        if not fut.done():
            ...
            sel().log_tool_invocation(
                ...
                agent=getattr(slot, "agent", "") or "codexcrew",
                source="dashboard",
...
def _replacement_shares_transcript(state: DashboardState, name: str, slot: _ChatSlot) -> bool:
    ...
    ``thread_ts`` stem (:func:`~codex_crew.history.transcript_stems` carries both).
...
async def _restore_slot_nudge_loop(
    ...
    if not loop.active:
        return
    try:
        from codex_crew import autonudge  # circular: autonudge -> dashboard.chat -> chat_handlers
...
async def close_slot(
    ...
    *,
    ...
    if slot._app:
        from codex_crew.apps.teardown import (
...
async def api_chat_slot_agent(request: web.Request) -> web.Response:
    ...
    async with contextlib.AsyncExitStack() as _stack:
        ...
        logger.info(
            "Slot %s agent switched to %r, resetting session", name, agent_name or "codexcrew"
        )
...
def _model_rejected_reason(model_name: str, provider: str | None = None) -> str | None:
    ...
    *provider* lets a caller that has already loaded the config supply it, so
    this adds no read of its own: ``CodexCrewConfig.load()`` deep-copies the
    ...
    if provider is None:
        try:
            provider = CodexCrewConfig.load().agent.provider
...
async def api_chat_slot_project(request: web.Request) -> web.Response:
    ...
    if project:
        ...
        # workspace that contains (or sits inside) the Codex Crew data home is
