...
from codex_crew.embeddings import get_shared_embedder, model_file_present
...
def _yolo_duration_fields() -> tuple[str, bool, list[str]]:
    ...
    """
    try:
        label = str(CodexCrewConfig.load().agent.yolo_duration)
    except Exception:
...
async def api_status(request: web.Request) -> web.Response:
    ...
    from codex_crew.dashboard.handlers import updates as _updates_mod
    ...
    data.update(
        {
            ...
            "version": codex_crew.__version__,
...
def _scan_mcp_processes() -> dict[str, object]:
    ...
    matches both "codexcrew_sandbox" and "kiro-cli"); per-category counts can
    ...
    Sandbox counting platform differences:
      Linux:  The namespace launcher (python3 ~/.codex-crew/run/codexcrew_sandbox_*.py ...)
              forks — the parent stays alive with "codexcrew_sandbox" in its
      ...
      macOS:  sandbox-exec execs the target command, replacing the process
              ...
              "codexcrew_sandbox" string (only in the -f path arg) is lost.
...
def _collect_system_metrics() -> dict[str, object]:
    ...
    try:
        from codex_crew.resource_status import probe as _resource_probe
        from codex_crew.subagent import compute_max_subagents
...
def _channel_members() -> tuple[str, ...]:
    ...
    from codex_crew.messaging.registry import governed_members
