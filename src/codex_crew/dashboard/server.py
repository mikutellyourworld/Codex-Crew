...
from codex_crew import platform_compat, port_resolution
from codex_crew.apps.backend import start_enabled_app_backends
from codex_crew.apps.hook_reconcile import init_hook_reconciler, stop_hook_reconciler
from codex_crew.apps.hooks_integration import (
    init_hooks_system,
    on_gateway_shutdown,
    on_gateway_startup,
)
from codex_crew.apps.manager import cleanup_migrated_builtin, register_builtin_apps
from codex_crew.autonudge import get_instance as _autonudge_get
from codex_crew.autonudge_authz import authorize_and_add_nudge
from codex_crew.browser_cli import launch as browser_cli_launch
from codex_crew.browser_cli import snapshots as browser_cli_snapshots
from codex_crew.browser_cli import token as browser_cli_token
from codex_crew.browser_cli import view as browser_cli_view
from codex_crew.channel_transcript_migration import migrate_channel_transcripts
from codex_crew.config import data_home
...
from codex_crew.dashboard import (
    cautious_boot,
    channel_slots,
    chat,
    handlers,
...
from codex_crew.dashboard.handlers.artifacts import (
    api_artifact_asset,
...
from codex_crew.dashboard.handlers.feedback import setup_feedback_routes
from codex_crew.dashboard.handlers.knowledge import setup_knowledge_routes
from codex_crew.dashboard.handlers.link_meta import setup_link_meta_routes
from codex_crew.dashboard.handlers.openai_profiles import setup_openai_profile_routes
from codex_crew.dashboard.handlers.secrets import setup_secrets_routes
from codex_crew.dashboard.handlers.source_providers import (
    register_status_delta_sink,
    unregister_status_delta_sink,
)
from codex_crew.dashboard.handlers.weixin_qr import setup_weixin_routes
from codex_crew.dashboard.handlers.whatsapp_setup import setup_whatsapp_routes
from codex_crew.dashboard.loop_watchdog import LoopStallWatchdog
...
from codex_crew.dashboard.port_reclaim import (
    FOREIGN_HOLDER,
    HEALTHY_PEER,
    RECLAIMED,
    reclaim_stale_gateway_port,
)
from codex_crew.dashboard.routes import register_all
from codex_crew.dashboard.slowloris import build_hardened_runner
from codex_crew.dashboard.state import _DEFAULT_PORT, DashboardState
from codex_crew.dashboard.token_auth import (
...
from codex_crew.deploy import _register_core_skills as _register_deploy_skills
from codex_crew.deploy.handlers import register_routes as _register_deploy_routes
from codex_crew.executors import subprocess_executor
from codex_crew.hooks import ScriptHookStore, set_global_hook_store
from codex_crew.instances import run_marker
from codex_crew.instances.registry import InstancesRegistry
from codex_crew.instances.ssh_tunnel_manager import SshTunnelManager, TunnelState
from codex_crew.mcp_gateway.socketsec import chmod_socket_0600
from codex_crew.metrics.http_metrics import (
...
from codex_crew.power import SleepInhibitor
from codex_crew.safety_override import (
...
from codex_crew.sel import sel, sel_is_warm, warm_sel_singleton
from codex_crew.skill_usage import register_skill_read_observer
from codex_crew.skills import SkillsLoader, set_pending_consumed_hook, set_pending_staged_hook
from codex_crew.stall_attribution import attribute_dump, describe
from codex_crew.tunnel.setup import setup_tunnel

if TYPE_CHECKING:
    from codex_crew.dashboard._types import (  # noqa: F401
        ContextBuilder,
        ConversationLog,
        CronService,
        HistoryConsolidator,
...
async def _should_prevent_sleep(state: DashboardState, port: int) -> bool:
    ...
    * **The dashboard is published on this machine's tailnet**, and
    ...
    """
    try:
        # CodexCrewConfig.load() does a stat and, on a cache miss, a JSON read +
...
_STRICT_INTERNAL_API_PATHS = frozenset(
    {
        ...
        # Computer use: the ``codexcrew-computer`` stdio shim's forwarding leg.
        ...
        # only legitimate caller is a Codex Crew directive tool in an MCP
        ...
        # caller is `codexcrew update approve` on the gateway host presenting the
        ...
        # MCP-only external-model consultation. The handler re-asserts the
        # internal-secret grant before reading any vault entry or making egress.
        "/api/model-consult",
        "/api/mcp/servers",
...
async def _audit_denied(caller: str, request: web.Request, error: str) -> None:
    ...
    * BEST-EFFORT — a trust root too short to sign the chain makes construction
    ...
    (:func:`codex_crew.sel.warm_sel_singleton`, awaited by both start paths
    ...
    that case, gated on :func:`codex_crew.sel.sel_is_warm`: two attribute
...
_MIXED_INTERNAL_API_PATHS = frozenset(
    {
        ...
        # httpOnly, ``CODEXCREW_INTERNAL_SECRET`` is stripped from agent env by
...
def _extra_frame_ancestors(
    ...
    (embedding) dashboard's port — its ``CODEXCREW_PORT`` — so the embedded remote
    ...
    return [f"http://{host}:{port}" for host in ("127.0.0.1", "localhost", "codexcrew.localhost")]
...
def _register_dist_static_routes(app: web.Application, dist_dir: Path) -> None:
    ...
    # (generic lucide icon / "CODEXCREW" hero). Stable, un-hashed filenames, so
...
def _precompute_telemetry(state: "DashboardState") -> None:
    ...
    from codex_crew.dashboard.handlers_system import _get_owner_hash, _get_static_system_info
...
def _deferred_session_control(handler_name: str) -> Callable:
    ...
    async def _route(request: web.Request) -> web.StreamResponse:
        from codex_crew.dashboard.handlers import session_control
...
def _register_mcp_routes(app: web.Application) -> None:
    ...
    app.router.add_post("/api/model-consult", handlers.api_model_consult)
    ...
    # Computer use: the thin ``codexcrew-computer`` stdio shim's only call. Lives
    ...
    # Auto-nudge (feature-flagged — returns 503 when CODEXCREW_AUTONUDGE unset)
    ...
    from codex_crew.dashboard.handlers.ask_question import (
        api_ask_question,
    ...
    from codex_crew.dashboard.handlers.workflows import (
        api_workflow_author,
    ...
    from codex_crew.dashboard.handlers.webapp_preview import register_webapp_preview_routes
    ...
    from codex_crew.dashboard.handlers.sandbox_doc import register_sandbox_doc_routes
...
def _export_bound_port(runner: web.AppRunner, port: int) -> None:
    ...
    Sets ``CODEXCREW_BOUND_PORT`` in this process's environment once the TCP
    ...
    Deliberately a DISTINCT variable from ``CODEXCREW_PORT``: that one means
    ...
    ``CODEXCREW_BOUND_PORT`` carries ephemeral truth only: consumed by
    ...
    *port* is ``0`` for an OS-assigned ephemeral bind (``--port auto``); the
    ...
    if bound:
        os.environ["CODEXCREW_BOUND_PORT"] = str(bound)
        logger.debug("Exported CODEXCREW_BOUND_PORT=%d for child processes", bound)
    else:
        logger.warning(
...
async def _start_site(
    ...
    *,
    ...
    non-CodexCrew process is never touched; those (and any case where the holder
        ...
        try:
            ...
            if attempt == 0:
                ...
                if outcome == RECLAIMED:
                    logger.warning(
                        "Reclaimed port %d from a stale CodexCrew gateway — rebinding.",
                        port,
                    )
    ...
    logger.error(
        "Port %d still in use after %.0fs — is another CodexCrew gateway running?\n"
        "Stop it with: codexcrew stop  or  sudo systemctl stop codexcrew",
        port,
        retries * delay,
    )
...
def _override_expiry_dm_text(source: str) -> str:
    ...
    ``approval_modes`` gate refuses the very ``/codexcrew yolo`` this message
    ...
    return "\U0001f512 Safety override expired. Tools now require approval. Reply `/codexcrew yolo` to re-authorize."
...
def _dispatch_override_expiry_notification(
    ...
    """
    if not CodexCrewConfig.load().agent.notify_override_expiry:
        return False
    try:
        loop = asyncio.get_running_loop()
    except RuntimeError:
...
def _register_connections_warm_lifecycle(app: web.Application, state: DashboardState) -> None:
    ...
    async def _connections_warm_shutdown(_app: web.Application) -> None:
        try:
            from codex_crew.connections.warm import shutdown_warm_mint
...
def _kick_connections_warm_scavenge(state: DashboardState) -> None:
    ...
    def _scavenge_in_thread() -> None:
        from codex_crew.connections.warm import scavenge_warm_mint_artifacts
...
def _register_instances_hooks(app: web.Application, state: DashboardState, port: int) -> None:
    ...
    async def _instances_startup(app_: web.Application) -> None:
        ...
        # http://127.0.0.1:<port> (NOT localhost / codexcrew.localhost) — mixing
...
def _wire_tunnel_shutdown(app: web.Application, state: DashboardState) -> None:
    ...
    async def _stop_bounded(stop: Callable[[], Awaitable[None]], what: str) -> None:
        """Start *stop* and observe it for at most the shared shutdown bound.

        ``asyncio.wait_for`` is deliberately not used: after its timeout it waits
        for cancellation to finish, so a provider that suppresses
        ``CancelledError`` can wedge cleanup indefinitely. The task stays tracked
        until it really exits, and its callback consumes any late exception.
        """

        async def _run_stop() -> None:
            await stop()

        stop_task = asyncio.create_task(_run_stop())
        state._background_tasks.add(stop_task)

        def _stop_done(task: "asyncio.Task[None]") -> None:
            state._background_tasks.discard(task)
            if task.cancelled():
                return
            exc = task.exception()
            if exc is not None:
                logger.warning("%s failed during shutdown", what, exc_info=exc)

        stop_task.add_done_callback(_stop_done)
        done, _pending = await asyncio.wait({stop_task}, timeout=_TUNNEL_STOP_TIMEOUT_SECS)
        if not done:
            stop_task.cancel()
            logger.warning(
    ...
    async def _tunnel_shutdown(_app: web.Application) -> None:
        # Tunnel authentication/connect runs after dashboard readiness. Latch
        # shutdown BEFORE cancellation so a provider that suppresses
        # CancelledError cannot publish a late manager after teardown.
        state._tunnel_shutting_down = True
        start_task = getattr(state, "_tunnel_start_task", None)
        if start_task is not None and not start_task.done():
            start_task.cancel()
            done, _pending = await asyncio.wait({start_task}, timeout=_TUNNEL_STOP_TIMEOUT_SECS)
            if not done:
                # asyncio.wait is the hard observation bound. wait_for is not:
                # it waits for cancellation to finish, so a non-cooperative
                # provider can overrun the timeout indefinitely.
                logger.warning(
                    "Tunnel startup did not cancel within %.0fs — continuing shutdown",
                    _TUNNEL_STOP_TIMEOUT_SECS,
                )
...
def _import_stt_engine() -> Any:
    ...
    """
    from codex_crew.stt import engine
...
def _register_stt_hooks(app: web.Application) -> None:
    ...
    async def _stt_shutdown(app_: web.Application) -> None:
        ...
        if "codex_crew.stt.engine" not in sys.modules:
            return
        from codex_crew import stt
...
def _arm_prevent_sleep_poll(state: DashboardState, port: int) -> None:
    ...
    *port* is the port this server actually bound, needed because one of the two
    ...
    the configured one for the same reason ``codexcrew tailnet up`` insists on
...
def _tailnet_origin_enabled() -> bool:
    ...
    return bool(CodexCrewConfig.load().dashboard.tailscale.enabled)
...
async def start_dashboard(
    ...
    if consolidator is None and conversation_log is not None:
        try:
            from codex_crew import history as _hist_mod
            ...
            _scfg = CodexCrewConfig.load().skills
    ...
    # --- Dynamic Workflows ---
    try:
        from codex_crew.dashboard.handlers import workflows as wf_handlers
        from codex_crew.dashboard.workflow_inject import inject_workflow_result
        from codex_crew.security import redact_credentials, redact_exfiltration_urls
        from codex_crew.workflows.service import WorkflowService
        ...
        try:
            _wf_timeout_secs = int(CodexCrewConfig.load().agent.workflow_run_timeout_secs)
        except Exception:
    ...
    from codex_crew.slack.handler import load_voice_reply_config
    ...
    async def _warm_builtin_app_names() -> None:
        try:
            from codex_crew.apps.execution import (
                builtin_app_agents,
                builtin_app_mcp_servers,
                builtin_app_names,
            )
            from codex_crew.hooks import (
    ...
    async def _reconcile_app_resources() -> None:
        from codex_crew.apps.bridges import reconcile_enabled_app_resources
    ...
    from codex_crew.apps.builtins import _MIGRATED_BUILTINS
    ...
    from codex_crew.apps.event_bus import build_broadcast_fn
    from codex_crew.apps.spawn_sdk import build_spawn_impl
    ...
    async def _hooks_startup(app_: web.Application) -> None:
        ...
        from codex_crew.apps.dev_mode import init_dev_mode_watcher

        await init_dev_mode_watcher(state.broadcast_ws)

        # App hook reconciler: the CLI (`codexcrew app enable/disable/install/
    ...
    async def _hooks_shutdown(app_: web.Application) -> None:
        ...
        from codex_crew.apps.dev_mode import stop_dev_mode_watcher
    ...
    # codexcrew.localhost) onto a single origin so the SPA's per-origin
    ...
    # dashboard on "localhost" one time and "codexcrew.localhost" the next (e.g.
    # `codexcrew token` historically printed localhost while the gateway
    # auto-opens codexcrew.localhost) lands the browser in a different, empty
    ...
    # ~/.codex-crew/logs/crash-dumps/ so they are findable via `codexcrew doctor`
    ...
    if _prior_dump is not None:
        ...
        if _age_h < 168:  # Only surface dumps less than 7 days old
            logger.warning(
                ...
                "Run `codexcrew doctor` for details.",
                _prior_dump,
                _age_h,
            )
    ...
    # Name the engine Codex Crew actually installs. The CLI's own default is the
    ...
    def _on_override_expired(source: str) -> None:
        ...
        if state.sessions is not None:
            from codex_crew.dashboard.chat_utils import effective_session_key
        ...
        try:
            ...
            from codex_crew.messaging.session_trust import clear_trusted_sessions
    ...
    from codex_crew.channel import ChannelManager, run_channel_agent
    from codex_crew.dashboard.handlers_channel import _spawn_agent_task
    ...
    # Boot-to-ready (rec #1): every required dashboard subsystem and durable
    # session restore is complete. Remote publication is optional connectivity,
    # not a prerequisite for the local dashboard Electron is waiting on.
    state.ready = True
    record_boot_to_ready((time.time() - state.start_time) * 1000.0, server="dashboard")

    if _tunnel_enabled:

        async def _start_tunnel_after_ready() -> None:
            try:
                ...
                if not tunnel_mgr:
                    return
                if state._tunnel_shutting_down:
                    # A provider may suppress cancellation and finish after the
                    # cleanup hook's hard observation bound. Never publish that
                    # late manager; stop it from this detached startup task.
                    try:
                        await tunnel_mgr.stop()
                    except Exception:
                        logger.warning("Late tunnel startup teardown failed", exc_info=True)
                    return
                state.tunnel_manager = tunnel_mgr
            except asyncio.CancelledError:
                raise
            except Exception:
                # Optional publication must not retract local readiness. The
                # provider owns its retry/status surface and can recover later.
                logger.warning("Tunnel startup failed after dashboard readiness", exc_info=True)

        tunnel_start_task = asyncio.create_task(_start_tunnel_after_ready())
        state._tunnel_start_task = tunnel_start_task
        state._background_tasks.add(tunnel_start_task)

        def _tunnel_start_done(task: "asyncio.Task[None]") -> None:
            state._background_tasks.discard(task)
            if getattr(state, "_tunnel_start_task", None) is task:
                state._tunnel_start_task = None

        tunnel_start_task.add_done_callback(_tunnel_start_done)
...
async def start_api_server(
    ...
    # widened it (dashboard.url opt-out of local_only, or the CODEXCREW_BIND
