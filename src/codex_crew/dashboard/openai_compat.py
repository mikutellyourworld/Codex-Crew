...
Translates OpenAI API format into CodexCrew's slot-based chat system,
allowing any OpenAI SDK client to talk to CodexCrew agents by setting
`model` to the agent name (e.g. "router", "lite").

Limitations:
- ``usage`` fields are hardcoded to zero; CodexCrew does not track token
...
from codex_crew.context import _neutralize_structural_markers
...
from codex_crew.dashboard.state import DashboardState, _normalize_slot_key
from codex_crew.dashboard.turn_dispatch import chat_turn_timeout_secs
...
async def api_completions(request: web.Request) -> web.StreamResponse:
    ...
    if blocked is not None:
        return web.json_response(
            {
                "error": {
                    "message": "The selected agent backend must be installed and signed in before starting a session.",
...
async def _blocking_response(
    ...
    Note: ``usage`` is hardcoded to zero — CodexCrew does not expose token
