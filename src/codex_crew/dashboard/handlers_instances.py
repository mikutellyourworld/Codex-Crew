...
from codex_crew.dashboard.session_transfer import (
    SnapshotUnstable,
    build_transfer_bundle_async,
    local_instance_label,
)
from codex_crew.history import SEARCH_MIN_CHARS
...
from codex_crew.instances.ssh_tunnel_manager import ProxyRequestError, TunnelState
from codex_crew.instances.warm_set import resolve_warm_set_cap
...
def _is_correlated_cloud_instance(ssm_target: str) -> bool:
    """True if *ssm_target* was provisioned by a Codex Crew cloud launch.
    ...
    """
    try:
        from codex_crew.cloud.connect import is_launched_instance
    ...
    return web.json_response(
        {
            ...
            "warm_set_cap": resolve_warm_set_cap(
                CodexCrewConfig.load().instances.warm_set_cap, eligible
            ),
        }
    )
...
async def api_instances_update(request: web.Request) -> web.Response:
    ...
    # editing them on an instance Codex Crew launched would strand a running,
        ...
        return web.json_response(
            {
                "error": (
                    ...
                    "edited on an instance Codex Crew launched — Stop/Start/Delete "
...
async def api_instances_connect(request: web.Request) -> web.Response:
    ...
    if status.state.value == "connected":
        ...
        # (a failed self-heal re-mint, or a remote `codexcrew restart` that
...
def _cap_str(value: object, limit: int = _CAP_MAX_STR) -> str:
    ...
    from codex_crew.dashboard.remote_relay import redact_peer_text
    ...
    return web.json_response(
        {
            ...
            "local_version": codex_crew.__version__,
            ...
            "version_match": bool(peer_version) and peer_version == codex_crew.__version__,
            "agents": _cap_rows(
