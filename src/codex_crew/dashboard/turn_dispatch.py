...
from codex_crew.acp.client import resolve_prompt_timeout
from codex_crew.config.loader import (
    APPROVAL_TURN_MARGIN_SECS,
    TOOL_APPROVAL_TIMEOUT_MIN,
    CodexCrewConfig,
)
from codex_crew.constants import CHAT_TURN_TIMEOUT, TOOL_APPROVAL_TIMEOUT
...
_TURN_DEADLINE: ContextVar[float | None] = ContextVar("codexcrew_turn_deadline", default=None)
...
def _acp_prompt_ceiling() -> float:
    ...
    Resolves through :func:`~codex_crew.acp.client.resolve_prompt_timeout`, which
...
def tool_approval_timeout_secs() -> float:
    ...
    * against the budget REMAINING in the turn that is already running. A prompt
    ...
    """
    try:
        configured = float(CodexCrewConfig.load().agent.tool_approval_timeout_secs)
    except Exception:
...
async def bounded_chat_turn(coro: "Coroutine[Any, Any, Any]") -> Any:
    ...
    the :func:`chat_turn_timeout_secs` resolution — a ``CodexCrewConfig.load()``
