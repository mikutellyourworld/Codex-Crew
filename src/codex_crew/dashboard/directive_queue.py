...
The directive marker (:mod:`codex_crew.session_directive`) is model-visible text,
so the consumer may only honour it when the tool CALL it arrived under was
recorded as an MCP call served by Codex Crew's own core server. That recording
...
arguments, POSTs them to the gateway over Codex Crew's own internal API declaring
...
from codex_crew.session_directive import DIRECTIVE_TOOLS, content_free_digest
...
def publish(session_key: str, kind: str, args: dict[str, Any]) -> str:
    ...
    did not come from one of Codex Crew's own directive tools.
    """
