...
Both share the collector engine in :mod:`codex_crew.diagnostics`, the exact same
code path the ``codexcrew doctor --bundle`` CLI uses. Auth + host validation are
...
def _sel():
    ...
    # circular import: the parent package `codex_crew.dashboard.handlers` imports
