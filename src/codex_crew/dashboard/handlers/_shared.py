...
from codex_crew import extras, platform_compat
...
from codex_crew.dashboard.state import VALID_MEMORY_MODES, DashboardState
...
from codex_crew.messaging.privacy_mode import hydrate as _hydrate_conv_flags
from codex_crew.messaging.privacy_mode import is_incognito as is_thread_incognito
from codex_crew.messaging.privacy_mode import is_temporary as is_thread_temporary
from codex_crew.security import is_sensitive_path, redact_credentials, redact_exfiltration_urls
from codex_crew.skill_trust import is_project_trusted as _is_project_trusted
from codex_crew.skills import skills_dir

if TYPE_CHECKING:
    from codex_crew.platform.interfaces import CapabilityManager
...
#: :func:`codex_crew.security.redact` before egress.
#:
...
def _audit_admission(surface: str, resource: str, allowed: bool, error: str = "") -> None:
    ...
    """
    from codex_crew.sel import sel as _sel  # circular import: sel imports config
...
def _canonical_skill_roots() -> list[Path]:
    ...
    try:
        ...
        out.extend(Path(p).expanduser().resolve() for p in CodexCrewConfig.load().skills.extra_paths)
    except Exception:
...
def _expand_resource_uri(uri: str, agent_path: Path) -> str | None:
    ...
    Thin alias for :func:`codex_crew.agent_discovery.expand_skill_uri` — the
...
def _resolve_loaded_by_agents(
    ...
    ``skill://`` URIs (it may still be loaded via CodexCrew text-injection or
...
def collect_skills_blocking(
    ...
    1. ``skills_loader.list_skills()`` — codexcrew skills (default source).
...
def _resolve_skill_root(name: str, state: DashboardState, session_key: str = "") -> Path | None:
    ...
    - ``foo`` → ``~/.codex-crew/skills/foo``
    - ``utils/tiny-url`` → ``~/.codex-crew/skills/utils/tiny-url``
    ...
    *session_key* scopes ``kiro-workspace/`` to the requesting chat slot's
    ...
    if name.startswith("kiro-user/"):
        ...
        # (consistent with the codexcrew/kiro branches below).
        if is_sensitive_path(str(resolved)):
            return None
        return resolved
    else:
        # ``codexcrew`` skills live under the active config home, which honors
        # CODEXCREW_HOME (e.g. isolated dev gateways).  Hardcoding
        # ``~/.codexcrew`` here would 404 every skill in a CODEXCREW_HOME-isolated
        ...
        if not rel or ".." in rel or rel.startswith("/") or rel.startswith("~"):
            return None
        # Root precedence must match SkillsLoader.load_skill(): codexcrew ->
        ...
        roots = [skills_dir()]
        try:
            roots.extend(Path(p).expanduser() for p in CodexCrewConfig.load().skills.extra_paths)
        except Exception:
            logger.debug("failed to load extra skill paths from config", exc_info=True)
...
def _persisted_session_paths(slot_name: str) -> list["Path"]:
    ...
    if (
        ...
        # Defence-in-depth against path traversal; ``CODEXCREW_SESSION_KEY``
...
def _session_has_persisted_history(slot_name: str) -> bool:
    ...
    * A legitimate MCP subprocess whose in-memory slot was evicted by the
      ...
      ``CODEXCREW_SESSION_KEY`` env var, so it keeps sending the same
...
def pip_extra_install_command(extra: str) -> str:
    ...
    Thin wrapper over :func:`codex_crew.extras.pip_install_command`, which owns
    the two things that make this string correct: it names the extra's real
    distributions rather than ``codexcrew[extra]`` (this project is not on any
