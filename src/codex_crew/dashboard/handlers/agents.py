...
from codex_crew import agent_state, model_registry
from codex_crew.acp.client import advertised_model_ids, model_is_unusable
from codex_crew.acp_backends import selectable_backend_values
...
from codex_crew.agent_sdk.drivers.acp import resolve_pin_spelling
...
from codex_crew.apps.bridges import _registration_source
from codex_crew.apps.manager import (
...
from codex_crew.config.schema import SCHEMA_REGISTRY, config_entry_to_dict
from codex_crew.config.sections import (
    _AVATAR_FILE_PIN_RE,
    _AVATAR_GHOST_BOOL_TRAITS,
    _AVATAR_GHOST_STR_TRAITS,
)
from codex_crew.config.sections import _AVATAR_IMAGE_EXTS as _LOADER_AVATAR_IMAGE_EXTS
from codex_crew.config.sections import (
    _safe_avatar,
)
from codex_crew.dashboard.chat_persistence import get_reasoning_effort_ordered
...
from codex_crew.dashboard.handlers._shared import (
    MAX_AGENT_SKILLS,
    _capability_manager,
    _read_session_key,
    active_project_dir,
...
from codex_crew.dashboard.handlers.discover import _redact_external
from codex_crew.dashboard.kiro_readiness import reject_if_kiro_unverified
from codex_crew.dashboard.state import DashboardState
from codex_crew.effort import EFFORT_LEVELS, EFFORT_VALUES
from codex_crew.executors import discovery_executor, maintenance_executor, subprocess_executor
...
def _sel():
    ...
    import codex_crew.dashboard.handlers as _pkg  # noqa: F811
...
async def _require_owner(request: web.Request, operation: str) -> web.Response | None:
    ...
    """
    from codex_crew.dashboard.handlers._shared import require_owner_dashboard_request
...
def _installed_agent_config() -> Path:
    """Return the installed agent config path (~/.kiro/agents/codexcrew.json).
...
def _app_declared_server_names() -> frozenset[str]:
    ...
    reach an ``opt_in`` server (``codexcrew-dashboard``: never auto-emitted, and a
    ...
    (``codexcrew-computer`` on an unsupported platform, or with computer use off),
...
def _app_or_host_owned(names: dict[str, Any]) -> frozenset[str]:
    ...
    * HOST-managed, and only the entries a rebuild would actually RE-ADD:
      ...
      grant (``codexcrew-dashboard``) that no rebuild re-introduces, and a
      server whose ``spec_gate`` is shut (``codexcrew-computer``), which both spec
...
def _commit_agent_config(
    ...
    * The bookkeeping lift runs AFTER the ``config.json`` write (so prefix 2
      ...
      the spec write, because it STRIPS Codex Crew keys (``model_managed`` /
    ...
    from codex_crew.platform.governance import sanitize_agent_config_governance
...
async def api_agent_config(request: web.Request) -> web.Response:
    ...
    Reads/writes ``~/.kiro/agents/codexcrew.json`` — the live config that
    kiro-cli actually uses at runtime.  Falls back to ``agents/defaults.json``
    if the installed config doesn't exist yet.
    """
    import codex_crew.dashboard.handlers as _h  # noqa: F811
    ...
    if request.method == "PUT":
        ...
        try:
            ...
            #    (~/.kiro/agents/codexcrew.lock) is the FILE lock. The transaction
            ...
            # so they don't reappear on upgrade.  Stored in ~/.codex-crew/config.json
            # (NOT codexcrew.json — kiro-cli rejects unknown fields).
            ...
            from codex_crew.dashboard.handlers.mcp import (
                _get_mcp_lock,
            ...
            if changed:
                logger.info(
                    "Stripped Codex Crew bookkeeping keys from a PUT to agent config for %r",
                    name,
                )
...
async def api_default_agent(request: web.Request) -> web.Response:
    ...
    if request.method == "PUT":
        ...
        try:
            ...
            known = set((await asyncio.to_thread(CodexCrewConfig.load)).agents.keys())
        except Exception:
            known = set()
...
async def api_capability_mcp_install(request: web.Request) -> web.Response:
    ...
    try:
        ...
        from codex_crew.dashboard.handlers.mcp import (  # noqa: E402 circular: mcp imports agents
            _sync_mcp_to_agent,
        )

        async with _get_config_lock():
            # Off the loop: _sync_mcp_to_agent acquires bridges' synchronous
            # _mcp_lock and does a full RMW of codexcrew.json. If a concurrent app
...
async def api_agents_installed(request: web.Request) -> web.Response:
    ...
    codexcrew is always first; codexcrew-lite is excluded.
    ...
    def _collect() -> list[Any]:
        ...
        agents.sort(key=lambda a: (0 if a.name == "codexcrew" else 1, a.name))
        return agents
...
def _cc_models(request: web.Request, configured_default: str = "") -> list[dict]:
    ...
    static catalog of everything CodexCrew knows how to name, so a free-tier user
...
def _wrap_list_models_argv(argv: list[str]) -> tuple[list[str], str | None]:
    ...
    shim, or a ``CODEXCREW_KIRO_BIN`` pointing at a nonstandard launch path all
...
async def api_models(request: web.Request) -> web.Response:
    ...
    try:
        from codex_crew.acp.client import (  # noqa: F811
            _resolve_kiro_bin_for_spawn,
            _resolve_ssh_auth_sock,
        )
        from codex_crew.env import augmented_path  # noqa: F811
        ...
        if proc.returncode != 0:
            from codex_crew.platform import redact_via_context  # noqa: F811
            ...
            if data.get("name") == name or f.stem == name:
                if request.method == "DELETE":
                    if f.name in (
                        ...
                        return web.json_response({"error": "cannot delete codexcrew"}, status=400)
                    ...
                    async with _get_config_lock():
                        ...
                        if "model" in patch_body:
                            ...
                            if data["model"] is None:
                                ...
                                # Shared with `codexcrew agent reset-model` so the
                        ...
                        # Never persist Codex Crew bookkeeping into the kiro spec —
...
def _roster_mask(value: object) -> str:
    ...
    the same job on ``GET /api/config/codexcrew``. A non-string (the loader lets
    ...
    * **Stale-view skew.** If the stored value changes between the GET and the
    ...
    from codex_crew.dashboard.handlers.core import _SENSITIVE_MASK
...
def _agent_roster_row(
    name: str, scope: str, agent_cfg: CodexCrewAgentConfig, *, redact: bool
) -> dict[str, object]:
    ...
    **Key half.** Explicit allowlist -- never a ``dataclasses.asdict`` spread,
    ...
    spread makes that contract "every field ``CodexCrewAgentConfig`` has now, plus
    ...
    **Value half.** Every record value goes through ``_roster_mask``, for every
    ...
    ``api_codexcrew_agent_update``; neither is correct alone, and an end-to-end
    ...
    wrong, because ``api_codexcrew_agent_update`` accepts ``description`` and
    ``source`` too.
    ...
    return {
        ...
        # ``api_codexcrew_agent_update`` and ``api_codexcrew_agent_delete`` 404 on a
        ...
        # ``GET /api/config/codexcrew``, where they are the ``agents`` map's KEYS
...
async def api_codexcrew_agents(request: web.Request) -> web.Response:
    """GET /api/agents — list all Codex Crew agent definitions, most-used first.
    ...
    # "mask", not "show".
    from codex_crew.dashboard.handlers.source_providers import is_owner_dashboard_request
    ...
    if project_dir:
        try:
            project_names = await asyncio.get_running_loop().run_in_executor(
                ...
                functools.partial(
                    project_agent_names,
                    project_dir,
                    operation="api_codexcrew_agents",
                    source="dashboard",
                ),
            )
        except Exception:
        ...
        project_default = CodexCrewAgentConfig()
        agents.extend(
...
async def api_codexcrew_agents_sync(request: web.Request) -> web.Response:
    ...
    try:
        ...
        for disc in discovered_agents:
            if (
                ...
                cfg.agents[disc.name] = CodexCrewAgentConfig(
        ...
        # Only prune package-installed agents (never user-created or codexcrew-owned).
...
async def api_codexcrew_agent_resolved_model(request: web.Request) -> web.Response:
    ...
    CodexCrew agent name; omitted falls back to the configured default agent.
...
def _effort_inputs(crew: CodexCrewAgentConfig | None) -> tuple[str, str] | None:
...
async def _refresh_session_defaults(request: web.Request, crew: str) -> None:
    ...
    ``api_codexcrew_config_patch`` already handles for ``agent.reasoning_effort``
...
def _model_pin_rejected(model: str, request: web.Request, provider: str) -> str | None:
    ...
    *provider* is passed in rather than resolved here so this whole path adds no
    ...
    ``CodexCrewConfig.load()`` deep-copies the validated dict even on a cache
    ...
    from codex_crew.dashboard.handlers.core import _validate_role_model

    return _validate_role_model(model, request, provider=provider)


async def api_codexcrew_agents_create(request: web.Request) -> web.Response:
    """POST /api/agents — create a new CodexCrew agent."""
    ...
    # The template pointer must be EXPLICIT. It used to default to "codexcrew",
    ...
    # report behind #1684. "codexcrew" is still a perfectly valid CHOICE here (a
        ...
        return web.json_response(
            {
                ...
                "from (pass 'codexcrew' for the built-in agent)",
    ...
    async with _get_config_lock():
        ...
        cfg.agents[name] = CodexCrewAgentConfig(
            ...
            source=body.get("source", "codexcrew"),
...
async def api_codexcrew_agent_update(request: web.Request) -> web.Response:
    """PUT /api/agents/{name} — update a CodexCrew agent."""
...
async def api_codexcrew_agent_delete(request: web.Request) -> web.Response:
    """DELETE /api/agents/{name} — delete a CodexCrew agent."""
...
async def api_codexcrew_agent_avatar_get(request: web.Request) -> web.Response:
...
async def api_codexcrew_agent_avatar_upload(request: web.Request) -> web.Response:
...
def _regen_conductor() -> None:
    ...
    try:
        ...
        if not cfg.agent.conductor_skill:
            return
        from codex_crew.conductor_skill import generate_conductor_skill  # noqa: F811
        from codex_crew.skills import SkillsLoader  # noqa: F811
