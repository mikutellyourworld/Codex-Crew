...
under ``~/.codex-crew/metrics/metrics-YYYY-MM-DD-<pid>.jsonl`` (see
``codex_crew.metrics.local_exporter``). Each line is one export cycle serialized
...
and generically surfaces every other ``codexcrew.*`` metric so newly-added emit
...
from codex_crew import sel as _sel_mod
from codex_crew.config.loader import CodexCrewConfig
from codex_crew.config.paths import config_dir
from codex_crew.dashboard.chat_utils import slot_transcript_key
...
from codex_crew.dashboard.state import NEW_SESSION_TITLE
from codex_crew.hooks import validate_file_path
from codex_crew.jsonl_util import bounded_records
from codex_crew.metrics.provider import TELEMETRY_ENV_VAR, env_pin, otlp_egress_active
from codex_crew.metrics.schema import RESOURCE_ATTR_PROCESS_START_TIME
from codex_crew.metrics.turns import TURN_COST_METRIC, TURN_CREDITS_METRIC, TURN_METRIC
from codex_crew.security import redact_credentials, redact_exfiltration_urls

logger = logging.getLogger(__name__)

_STARTUP_METRIC = "codexcrew.session.startup.duration"
...
# every attribute present: ``codexcrew.gateway.request.duration`` carries
...
def _lifetime_total_gauge_names() -> "frozenset[str]":
    ...
    if _lifetime_total_gauges is None:
        from codex_crew.metrics.inventory_gauges import LIFETIME_TOTAL_METRICS as _inventory
        from codex_crew.metrics.process_gauges import LIFETIME_TOTAL_METRICS as _process
...
# codexcrew.watchdog.recovery.outcome. Counting them as faults would inflate the
...
def _telemetry_cfg() -> _TelemetryState:
    ...
    ``enabled`` is the EFFECTIVE state, not the stored flag: ``CODEXCREW_TELEMETRY``
...
def _iter_metric_points(
    ...
    Every ``codexcrew.*`` data point is yielded; the name filter is load-bearing
    rather than defensive. One meter carries all three namespaces the recorder
    accepts — ``codexcrew.*``, ``gen_ai.*`` and ``app.<app_id>.*``
        ...
        for rm in obj.get("resource_metrics", []) or []:
            ...
            for sm in rm.get("scope_metrics", []) or []:
                for metric in sm.get("metrics", []) or []:
                    ...
                    if not name.startswith("codexcrew."):
                        continue
...
def _gauge_series(
    ...
    Shards are per-PID and several codexcrew processes (gateway, MCP daemons)
...
def _aggregate(shard_paths: list[Path]) -> dict[str, Any]:
    ...
    # generic surface for every other codexcrew.* metric
    other_hist: dict[str, _Hist] = {}
    ...
    # wins WITHIN one process. Shards are per-PID and several codexcrew
...
async def api_telemetry_startup(request: web.Request) -> web.Response:
    """GET /api/telemetry/startup — session-startup latency + all codexcrew.* metrics.
    ...
    list surfacing every other emitted codexcrew.* metric.
...
async def api_context_trace(request: web.Request) -> web.Response:
    ...
    Returns what CodexCrew added to each turn of one session, block by block, so
    ...
    :func:`codex_crew.dashboard.handlers.usage.context_trace`.
...
async def api_beacon_status(request: web.Request) -> web.Response:
    ...
    ``CODEXCREW_TELEMETRY_DISABLED``, a CI host, a non-default data home, or a
    ...
    try:
        # to_thread, not a bare load(): CodexCrewConfig.load() stats and reads
...
async def api_collection_status(request: web.Request) -> web.Response:
    ...
    ``CODEXCREW_TELEMETRY`` overrides ``telemetry.enabled`` inside the collector: a
