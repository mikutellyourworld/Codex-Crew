...
from codex_crew.browser.command_bus import (
...
from codex_crew.constants import CHANNEL_SEND_NAMESPACES
from codex_crew.cron import CronStoreBusy, CronStoreUnreadable
from codex_crew.dashboard.channel_folders import (
    LIVE_RELOAD_FIELDS,
...
from codex_crew.dashboard.origin import is_direct_local_request, is_proxied_request
from codex_crew.dashboard.state import (
    CRON_NOTIFY_END,
    CRON_NOTIFY_PREFIX,
    DashboardState,
)
from codex_crew.dashboard.token_auth import caller_names_a_missing_slot
from codex_crew.messaging.display_safety import redact_for_display
from codex_crew.messaging.link import SLACK_NAMESPACE, ChannelLink
...
from codex_crew.messaging.transport import delivery_confirmed
from codex_crew.notifications.bus import (
    NotificationPayload,
    NotificationValidationError,
)
from codex_crew.platform.governance_profiles import HOST_SESSION_KEY
from codex_crew.platform_compat import IS_MACOS
from codex_crew.security import is_sensitive_path, redact_credentials, redact_exfiltration_urls
from codex_crew.slack.format import build_options_blocks, extract_options
from codex_crew.slack.outbound import OPTIONS_FALLBACK_TEXT, PostedOptions
from codex_crew.spawn_warm import warm_project_agents_for_spawn
from codex_crew.subagent import effort_applied_note, effort_drop_reason
from codex_crew.subagent_persistence import _agent_dir, read_state
...
async def api_spawn_status(request: web.Request) -> web.Response:
    ...
    if info.done:
        ...
        # api_spawn_list. This endpoint is the one a blocking `codexcrew spawn
...
async def api_spawn_list(request: web.Request) -> web.Response:
    ...
    for info in state.subagents.all_agents:
        ...
        if info.done:
            ...
            # which is how #6484 presented: `codexcrew spawn list` showed the
...
async def api_notification_channels(request: web.Request) -> web.Response:
    ...
    """
    from codex_crew.notifications.settings import PROTECTED_CHANNELS
...
async def api_notification_channel_settings(request: web.Request) -> web.Response:
    ...
    """
    from codex_crew.notifications.settings import ChannelSettingsError
...
async def _deliver_channel_dm(
    ...
    # codex_crew.dashboard.handlers, so a top-level import closes that cycle. Only
    ...
    from codex_crew.dashboard.chat_runner import _resolve_channel_target
...
async def api_send_message(request: web.Request) -> web.Response:
    ...
    from codex_crew.slack.handler import is_allowed_user, is_tracked_channel  # noqa: F811
    from codex_crew.validation import USER_ID_RE  # noqa: F811
            ...
            if slot_key:
                ...
                if slot:
                    ...
                    if slot.running or slot._in_stage_execution:
                        ...
                        # codex_crew.dashboard.handlers (for MAX_PROMPT_BYTES,
                        ...
                        from codex_crew.dashboard.turn_dispatch import spawn_guarded_turn
...
async def api_slack_pins(request: web.Request) -> web.Response:
    ...
    from codex_crew.slack.handler import is_tracked_channel  # noqa: F811
...
async def api_slack_profile(request: web.Request) -> web.Response:
    ...
    from codex_crew.slack.handler import is_allowed_user  # noqa: F811
...
def _deny_non_owner_browser_request(request: web.Request, operation: str) -> web.Response | None:
    ...
    * the view endpoints return an unauthenticated dashboard URL, i.e. control
    ...
    from codex_crew.dashboard.handlers._shared import _owner_denial_response
...
async def api_browser_view_start(request: web.Request) -> web.Response:
    ...
    def _start_view() -> None:
        ...
        pinned = CodexCrewConfig.load().dashboard.browser_view_port
...
def _write_env_updates(updates: dict[str, str | None]) -> None:
    ...
    through :func:`codex_crew.atomic_write.atomic_write` with
    ...
    # writers (the `codexcrew secrets import` migrator and the WeChat/Weixin QR
...
async def api_slack_manifest(request: web.Request) -> web.Response:
    ...
    Mirrors ``codexcrew manifest --url`` so the settings UI can offer one-click
    ...
    alias = request.query.get("alias", "").strip() or "codexcrew"
...
async def _slack_config_save_locked(request: web.Request) -> web.Response:
    ...
    if "command" in body:
        ...
        new_cmd = cmd or "codexcrew"
        if new_cmd != slack_cfg.get("command", "codexcrew"):
...
def _threshold_pct_rejection(body: dict[str, Any], key: str) -> tuple[str, str] | None:
    ...
    if key not in body:
        return None
    from codex_crew.config.loader import THRESHOLD_PCT_MAX, THRESHOLD_PCT_MIN
...
async def api_teams_activity(request: web.Request) -> web.Response:
    ...
    from codex_crew import webhooks  # noqa: F811
    from codex_crew.teams.client import (  # noqa: F811
...
async def api_teams_config_get(request: web.Request) -> web.Response:
    ...
    from codex_crew.teams.client import HAS_JWT  # noqa: F811
    ...
    return web.json_response(
        {
            ...
            # PyJWT ships in the optional `codexcrew[teams]` extra and the channel
...
async def api_teams_config_save(request: web.Request) -> web.Response:
    ...
    if credential_touched:
        ...
        # than via CodexCrewConfig.load(), which triggers _load_resolved() ->
    ...
    async with _get_config_lock():
        ...
        if verified_triple is not None:
            ...
            # avoid CodexCrewConfig.load() which would trigger migration write-back).
...
async def api_webex_config_save(request: web.Request) -> web.Response:
    ...
    # torn read and a side-effecting one: ``CodexCrewConfig.load()`` normalizes and
...
async def api_feishu_config_get(request: web.Request) -> web.Response:
    ...
    def _read() -> "tuple[CodexCrewConfig, dict, tuple[bool, bool, str]]":
...
async def _feishu_config_save_locked(request: web.Request) -> web.Response:
    ...
    # so a concurrent ``codexcrew config set`` in ANOTHER PROCESS cannot land between
    ...
    def _restore_feishu(fresh: dict) -> dict:
        ...
        concurrent ``codexcrew config set feishu.*`` that arrived between our write
