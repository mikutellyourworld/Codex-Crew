...
import codex_crew
from codex_crew import beacon, platform_compat, stt
...
from codex_crew.computer_use.types import MAX_TREE_NODES_LIMIT as _CU_MAX_TREE_NODES_LIMIT
...
from codex_crew.context_management import RESULT_FILE_MAX_BYTES
from codex_crew.dashboard.handlers._shared import (
    _pip_install_channel_available,
    pip_extra_install_command,
)
from codex_crew.dashboard.origin import check_host, is_direct_local_request
from codex_crew.dashboard.state import DashboardState
from codex_crew.dashboard.stt_stream import _STREAMING_PROVIDERS, PROVIDER_LOCAL
from codex_crew.dashboard.token_auth import MAX_SESSION_TTL_SECS, generate_token, parse_duration
from codex_crew.effort import EFFORT_LEVELS
from codex_crew.executors import discovery_executor
from codex_crew.metrics import provider as _metrics_provider
from codex_crew.security_posture import build_posture_snapshot_async, posture_counts_async
from codex_crew.session_workspace import is_valid_id
from codex_crew.stt import decoder as stt_decoder
from codex_crew.stt import models as stt_models
from codex_crew.stt.limits import (
    MAX_IDLE_EVICT_SECS,
    MAX_INTERVAL_MS,
    MIN_IDLE_EVICT_SECS,
...
from codex_crew.transcribe import (
    _find_ffmpeg,
    _whisper_language,
    availability_detail,
...
#
# Scope: every ``str`` field of ``CodexCrewAgentConfig`` whose LOAD path does not
...
def _masked_config_dict(cfg: CodexCrewConfig) -> dict:
    """Return ``cfg.to_dict()`` with sensitive string values masked.

    Applied ONLY to the GET /api/config/codexcrew response — never to the value
    ...
    from codex_crew.config.validation import _is_sensitive_path

    masked = copy.deepcopy(cfg.to_dict())

    # Drop unknown/edition-contributed top-level sections (CodexCrewConfig.
...
# probes (e.g. `codexcrew token`'s stale-dashboard warning) can detect that
...
_DASHBOARD_HTML_NOT_FOUND = (
    ...
    "<p><strong>Try restarting Codex Crew.</strong> The exact restart step"
    " depends on your environment: if you installed it as a service use"
    " <code>codexcrew restart</code> (systemd / launchd); otherwise"
    " stop the running <code>codexcrew gateway</code> process and start it"
    " again.</p>"
)
...
def _sel():
    ...
    import codex_crew.dashboard.handlers as _pkg  # noqa: F811 — circular import
...
async def logo(request: web.Request) -> web.StreamResponse:
    ...
    from codex_crew.hooks import validate_file_path  # noqa: F811

    cfg = _h.CodexCrewConfig.load()
    ...
    from codex_crew import __version__

    names = ["codexcrew-logo.png"]
    if "-nightly." in __version__:
        names.insert(0, "codexcrew-logo-nightly.png")
    for name in names:
...
async def api_branding(request: web.Request) -> web.Response:
    ...
    return web.json_response(
        {
            "bot_name": cfg.dashboard.bot_name or "Codex Crew",
            "avatar": "/logo.png",
...
def _liveness_payload(request: web.Request) -> dict[str, object]:
    ...
    if is_direct_local_request(request) and check_host(request):
        ...
        payload.update({"app": "codexcrew", "version": codex_crew.__version__})
    return payload
...
async def api_version(request: web.Request) -> web.Response:
    ...
    """
    return web.json_response({"version": codex_crew.__version__})
...
async def api_ready(request: web.Request) -> web.Response:
    ...
    * **Shutdown requested** — when SIGTERM/SIGINT or ``POST /api/shutdown``
    ...
    shutting_down = codex_crew.shutdown_event.is_set()
...
def _theme_payload(cfg: CodexCrewConfig) -> dict[str, object]:
...
async def pwa_file(request: web.Request) -> web.StreamResponse:
    ...
    # at CodexCrewWebsite/dist) still passes the traversal guard.
...
def _stt_providers() -> list[str]:
    ...
    if "apple" in providers:
        from codex_crew import apple_speech
...
async def api_stt_config(request: web.Request) -> web.Response:
    ...
    if request.method == "PUT":
        ...
        path = config_path()
        from codex_crew.agent import _atomic_json_write  # noqa: F811
        ...
        async with _get_config_lock():
            ...
            if "enter_sends" in body and isinstance(body["enter_sends"], bool):
                stt_section["enter_sends"] = body["enter_sends"]
            # Every bound comes from codex_crew.stt.limits, which is what the
    ...
    return web.json_response(
        {
            ...
            "enter_sends": cfg.stt.enter_sends,
...
async def api_stt_status(request: web.Request) -> web.Response:
    ...
    def _probe() -> tuple[stt.Availability, list[dict[str, object]], bool, str | None]:
        # codex_crew.stt.engine is imported HERE rather than at module scope: it
        ...
        from codex_crew.stt import engine as stt_engine
...
def _transcribe_extra_importable() -> bool:
    ...
    Reads ``codex_crew.transcribe``'s own import outcome (its module-level
    ...
    """
    from codex_crew import transcribe
...
async def api_stt_transcribe(request: web.Request) -> web.Response:
    ...
    from codex_crew.dashboard import part_stream
    from codex_crew.transcribe import transcribe_audio  # noqa: F811
    ...
    try:
        ...
        if text:
            from codex_crew.security import (  # noqa: F811
                redact_credentials,
...
async def api_security_stats(_request: web.Request) -> web.Response:
    ...
    denied = 0
    try:
        from codex_crew.dashboard.handlers.security import build_denied_commands_snapshot_async
...
async def api_codexcrew_config(request: web.Request) -> web.Response:
    """GET/PUT /api/config/codexcrew — read or update CodexCrew config."""
    # Re-imported at call time (not reused from the module-level binding) so a
    # test that redirects ``codex_crew.config.loader.config_path`` at a temp path
    # is observed by this handler.
    from codex_crew.config.loader import config_path  # noqa: F811
    ...
    if request.method == "PUT":
        ...
        from codex_crew.config.loader import ConfigReadError, update_config_locked  # noqa: F811
        from codex_crew.dashboard.handlers.agents import _get_config_lock  # noqa: F811
            ...
            async with _get_config_lock():
                ...
                if "conductor_skill" in applied:
                    if agent.get("conductor_skill"):
                        from codex_crew.dashboard.handlers.agents import (  # noqa: F811
                            _regen_conductor,
                        )
...
def _agent_values() -> set[str]:
    ...
    return {"", *CodexCrewConfig.load().agents}
...
def _active_advertised_ids(request: web.Request) -> list[str] | None:
    ...
    """
    from codex_crew.acp.client import advertised_model_ids
...
def _validate_role_model(
    ...
    *provider* is forwarded to :func:`_model_rejected_reason` so a caller holding
    ...
    if not value or value == "auto":
        return None
    from codex_crew.acp.client import model_is_unusable
    from codex_crew.dashboard.chat_handlers import _model_rejected_reason
...
_EDITABLE_CONFIG: dict[str, dict] = {
    ...
    # toggle), the GUI twin of `codexcrew telemetry disable`. Only the boolean
    ...
    # ~/.codex-crew/metrics. It is NOT unconditionally local, though —
...
def _tailnet_governance_pinned_off() -> bool:
    ...
    """
    from codex_crew.dashboard import tailnet  # noqa: F811 - local: keeps the import edge lazy

    return tailnet.is_governance_pinned_off(audit_tool="config_patch_dashboard_tailnet")


async def api_codexcrew_config_patch(request: web.Request) -> web.Response:
    """PATCH /api/config/codexcrew — update a single config field."""
    from codex_crew.config.loader import ConfigReadError, config_path, update_config_locked
    ...
    if path_key == "agent.provider":
        ...
        # For claude_code this (re)writes ~/.claude/agents/codexcrew.mcp.json —
        ...
        try:
            from codex_crew.agent import rebuild_agent_config  # noqa: F811  circular import
    ...
    # codexcrew processes pick the value up when they next start.
...
async def api_token_local(request: web.Request) -> web.Response:
    ...
    Requires a per-session secret written to ~/.codex-crew/.local_secret at
...
def _invalid_session_path_id(session_id: str, agent_id: str | None = None) -> web.Response | None:
    ...
    the existing tests' ``monkeypatch`` of ``codex_crew.session_workspace.<fn>``
...
async def api_session_agents_list(request: web.Request) -> web.Response:
    ...
    if (_e := _invalid_session_path_id(session_id)) is not None:
        return _e
    from codex_crew.session_workspace import list_results  # noqa: F811
...
async def api_session_agent_result(request: web.Request) -> web.Response:
    ...
    if (_e := _invalid_session_path_id(session_id, agent_id)) is not None:
        return _e
    from codex_crew.session_workspace import read_result  # noqa: F811

    content = read_result(session_id, agent_id)
    if not content:
        return web.json_response({"error": "not found"}, status=404)
    from codex_crew.security import redact_credentials, redact_exfiltration_urls  # noqa: F811
...
async def api_session_agent_stream(request: web.Request) -> web.StreamResponse:
    ...
    from codex_crew.session_workspace import result_path  # noqa: F811
...
async def api_logout(request: web.Request) -> web.Response:
    ...
    Called by ``codexcrew logout`` CLI. Requires loopback + local secret
    (same auth as /api/token/local) to prevent unauthorized revocation.
    """
    import codex_crew.dashboard.handlers as _h  # noqa: F811
    from codex_crew.dashboard.token_auth import revoke_all_sessions  # noqa: F811
...
async def api_shutdown(request: web.Request) -> web.Response:
    ...
    """
    import codex_crew.dashboard.handlers as _h  # noqa: F811
    from codex_crew import shutdown_event  # noqa: F811
...
async def api_app_token(request: web.Request) -> web.Response:
    ...
    """
    from codex_crew.dashboard.token_auth import generate_token, validate_app_secret
