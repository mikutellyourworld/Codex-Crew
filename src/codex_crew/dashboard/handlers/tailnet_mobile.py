...
The dashboard already had every *piece* of tailnet access — :mod:`codex_crew.
dashboard.tailnet` resolves the MagicDNS name, :mod:`codex_crew.dashboard.
tailnet_serve` publishes and withdraws ``tailscale serve``, and the CLI wires
them together as ``codexcrew tailnet up`` — and no way to get there from the UI.
...
:func:`codex_crew.dashboard.handlers.tailnet._derive_state` gives: one owner for
...
from codex_crew.dashboard.handlers._shared import _caller_bounds, _is_restricted_session
from codex_crew.dashboard.handlers.agents import _get_config_lock
from codex_crew.dashboard.handlers.mobile_connect import mint_denied_reason
...
from codex_crew.qr import render_qr_data_uri
...
def _derive_step(
    *,
    ...
    6. ``occupied`` — serve holds this port/mount for something that is not this
       ...
       (``codexcrew tailnet up``) for the operator to run deliberately.
...
def _dashboard_port(request: web.Request) -> int:
    ...
    ``tailscale serve`` whatever else holds it — the hazard ``codexcrew tailnet
...
def _effective_mobile_setup(cfg: CodexCrewConfig, login: str) -> tuple[bool, list[str]]:
...
async def api_tailnet_mobile_configure(request: web.Request) -> web.Response:
    ...
    effective = await asyncio.to_thread(CodexCrewConfig.load)
...
async def api_tailnet_mobile_qr(request: web.Request) -> web.Response:
    ...
    # `codexcrew logout` all still apply unchanged.
    #
