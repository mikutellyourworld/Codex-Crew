...
from codex_crew._sqlite_compat import fts5_segment_for_index, sqlite3
from codex_crew.artifacts import get_default_store
from codex_crew.config.loader import CodexCrewConfig, config_dir, data_home
from codex_crew.dashboard import part_stream
from codex_crew.dashboard.handlers._shared import read_bounded_json
from codex_crew.dashboard.handlers.files import (
    _ZIP_CONTAINER_EXTS,
    _content_matches_ext,
)
from codex_crew.executors import run_in_embed_pool
from codex_crew.knowledge.agent_fetch import fetch_url_content
from codex_crew.knowledge.agent_source import add_agent_document
from codex_crew.knowledge.artifact_ingest import ArtifactKnowledgeSync
...
from codex_crew.knowledge.connectors.local_folder import LocalFolderConnector
...
from codex_crew.knowledge.folder_watcher import (
    estimate_scan_cost,
    folder_chunk_budget,
    max_files_prop,
    walk_filters,
)
from codex_crew.knowledge.ingestion import (
    IngestionPipeline,
    _redact,
    rebuild_embeddings,
    start_rebuild_job,
)
from codex_crew.knowledge.llm_pool import DEFAULT_EXTRACTION_EFFORT, LLMPool
from codex_crew.knowledge.readers import FileReader
from codex_crew.knowledge.retrieval import HybridRetriever
from codex_crew.knowledge.spend import source_spend
from codex_crew.knowledge.store import (
...
from codex_crew.zip_vet import ZipInventoryRejected, vet_zip_inventory
...
def _create_embedder(app):
    """Create embedder from CodexCrew config. Returns None if disabled/unavailable."""
...
async def _start_artifact_ingest_async(app: web.Application) -> None:
    ...
    ``codex_crew.knowledge.artifact_ingest`` for the full design.
    """
...
async def confirm_source(request: web.Request) -> web.Response:
    ...
    # adopted in the same write that activates it. Without this, a row Codex Crew
...
def _inspect_zip_archive(path: str) -> str | None:
    ...
    The member cap runs TWICE, deliberately. The shared vet (codex_crew.zip_vet)
