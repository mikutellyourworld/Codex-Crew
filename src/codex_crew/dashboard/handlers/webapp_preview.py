...
- Dotfiles (and thus ``.codexcrew-deploy.json`` style manifests / VCS dirs)
  are never served.
"""
...
from codex_crew.artifacts import ArtifactNotFoundError, ArtifactValidationError, get_default_store
from codex_crew.dashboard.origin import frame_ancestors_value, is_direct_local_request
from codex_crew.deploy.handlers import _allowed_local_roots
from codex_crew.hooks import safe_read_file_bytes_nolink
                ...
                if "X-Frame-Options" not in resp.headers:
                    ...
                    for directive in csp.split(";"):
                        ...
                        if d.lower().startswith("frame-ancestors"):
                            fa = d.lower()
                            break
                    framable = not fa or "localhost" in fa or "127.0.0.1" in fa or "codexcrew.localhost" in fa
...
async def serve_artifact_app_file(request: web.Request) -> web.Response:
    ...
    from codex_crew.dashboard.server import _extra_frame_ancestors
