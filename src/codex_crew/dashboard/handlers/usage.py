...
from codex_crew.config.paths import data_home, kiro_sessions_dir
from codex_crew.context_blocks import USER_LABEL
...
from codex_crew.metrics.turns import (
...
#
# ``config_dir()`` / ``kiro_sessions_dir()`` read ``CODEXCREW_HOME`` on every
...
# pod isolation (a pod sets ``CODEXCREW_HOME`` for its own process), the one-time
# ``~/.codexcrew`` -> ``~/.codex-crew`` migration (deliberately lazy), and test
# isolation -- the autouse ``_isolate_codexcrew_home`` fixture runs *after*
...
# in the same unit as CodexCrew's own exactly-counted blocks. Never applied to
...
def context_trace(slot: str, days: int = 14) -> dict[str, Any]:
    ...
    CodexCrew did NOT inject — kiro-cli's own base prompt, its tool catalogue and
    its steering files. It is an ESTIMATE and labelled as one everywhere it is
    surfaced: the provider reports occupancy in tokens while every CodexCrew
...
def read_effective_agent(source: object) -> str:
    ...
    can be served by ``codexcrew``. Recording the alias would attribute the turn
...
def _build_token_record(
    ...
    *,
    ...
    return {
        ...
        # codex_crew.context_blocks.split_blocks. Sizes are characters (exact and
...
def persist_token_record(
    ...
    *,
    ...
    not emit ``codexcrew.turn.duration``. A future direct caller of this variant
...
async def persist_token_record_async(
    ...
    *,
    ...
    ALSO emits the per-turn OTEL family — ``codexcrew.turn.duration`` plus the
    turn's usage (``codexcrew.turn.tokens`` and whichever of
    ``codexcrew.turn.credits`` / ``codexcrew.turn.cost_usd`` the backend billed in)
...
def _emit_turn_histogram(
    ...
    ``codexcrew.turn.duration`` plus the turn's usage family
    (``codexcrew.turn.tokens`` and whichever of ``codexcrew.turn.credits`` /
    ``codexcrew.turn.cost_usd`` the backend billed in). Split out so the emit is
...
def get_usage_cache() -> dict:
    ...
    try:
        from codex_crew.dashboard.handlers.sessions import _usage_cache
