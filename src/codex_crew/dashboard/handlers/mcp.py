...
from codex_crew import mcp_quarantine, platform_compat
...
from codex_crew.config.paths import data_home, kiro_agents_dir
from codex_crew.dashboard.handlers._shared import read_bounded_json
from codex_crew.dashboard.state import DashboardState
from codex_crew.env import emit_env
from codex_crew.loop_lock import LoopBoundLock
...
from codex_crew.mcp_gateway import hazards, is_gateway_supported
from codex_crew.mcp_gateway.hashing import hash_command
from codex_crew.mcp_gateway.rewriter import records_dir
from codex_crew.mcp_gateway.shareability import ShareEvidence, ShareVerdict, assess
from codex_crew.mcp_gateway.verdict_cache import load_cache
from codex_crew.mcp_hot_reload import live_sessions_hot_reload
from codex_crew.mcp_provenance import ABSENT, resolve_write, stamp
from codex_crew.mcp_utils import (
    INTERNAL_CLIENT_ID_KEY,
    INTERNAL_SCOPES_KEY,
    apply_kiro_oauth_hints,
    mcp_server_alias,
)
from codex_crew.platform.governance import may_skip_gate_now
...
def _sync_mcp_to_agent(name: str, enabled: bool, *, remove: bool = False) -> None:
    """Serialize the codexcrew.json read-modify-write with bridges' app-MCP
    ...
    """
    from codex_crew.apps.bridges import _mcp_lock
    from codex_crew.dashboard.handlers.agents import _installed_agent_config
...
def _sync_mcp_to_agent_unlocked(name: str, enabled: bool, *, remove: bool = False) -> None:
    """Sync MCP server state to codexcrew.json mcpServers (not tools/allowedTools)."""
    from codex_crew.dashboard.handlers.agents import (  # noqa: F811 circular: agents imports mcp
    ...
    if enabled and not remove:
        # Ensure server exists in codexcrew.json mcpServers when enabled
    ...
    if not enabled and not remove:
        ...
        # (see :mod:`codex_crew.mcp_hot_reload`) leaves a still-running server's
...
def _sync_mcp_to_agent_batch_unlocked(names: list[str], enabled: bool) -> None:
    """Batch sync multiple MCP servers to codexcrew.json in a single read-modify-write."""
    from codex_crew.dashboard.handlers.agents import (  # noqa: F811 circular: agents imports mcp
        _installed_agent_config,
    )
    ...
    changed = False
    if enabled:
        # Ensure all servers exist in codexcrew.json mcpServers
        mcp_servers = cfg.setdefault("mcpServers", {})
        try:
            gdata = json.loads(_GLOBAL_MCP_JSON.read_text(encoding="utf-8"))
        except (FileNotFoundError, json.JSONDecodeError):
...
async def _run_mcp_probe() -> None:
    ...
    try:
        # circular import: mcp_discovery defers imports of codex_crew.agent
        ...
        from codex_crew.mcp_discovery import probe_all  # noqa: F811
...
async def api_mcp_servers(request: web.Request) -> web.Response:
    ...
    ``mcpServers``, the scope-tagged ``mcp.json`` files (Codex Crew data home
    ...
    try:
        ...
        pass
    # CodexCrew-scope entries: disabled state (consent-disabled installs and
    # custom adds live only here) + which rows the JSON editor can manage.
    codexcrew_mcps = _load_json_or_empty(_codexcrew_mcp_json()).get("mcpServers", {})
    result: list[dict] = []
    for s in servers:
        d = s.to_dict()
        ...
        spec = global_mcps.get(s.name, {})
        kc_spec = codexcrew_mcps.get(s.name)
        d["codexcrewManaged"] = isinstance(kc_spec, dict)
        is_disabled = (
...
async def api_mcp_active(request: web.Request) -> web.Response:
    ...
    For non-codexcrew agents, reads ``mcpServers`` from the agent's config
    in ``~/.kiro/agents/`` — these are the only servers kiro-cli loads
    when ``--agent <name>`` is passed.  For codexcrew (or no agent),
    reads from global ``~/.kiro/settings/mcp.json`` as before.
    """
    agent = request.query.get("agent", "")

    # Resolve CodexCrew agent name → kiro agent name so "default" → "codexcrew"
    if agent:
        try:
            from codex_crew.config.loader import CodexCrewConfig, resolve_agent_bindings  # noqa: F811
    ...
    # Non-codexcrew agent: read from agent config
    if agent and agent != "codexcrew":
    ...
    from codex_crew.mcp_discovery import list_servers  # noqa: F811
    ...
    # Also include the managed CodexCrew servers (always enabled). NOTE for
    # ``codexcrew-computer``: "enabled" here means the SERVER is registered, not
    ...
    for builtin in ("codexcrew-cron", "codexcrew-core", "codexcrew-computer"):
...
async def _evaluate_shareability(servers: list[Any]) -> None:
    ...
    from codex_crew.mcp_gateway.evaluate import evaluate_new_servers
    ...
    await evaluate_new_servers(
        list(servers), records_dir(CodexCrewConfig.load().mcp_gateway.socket_path)
    )
...
async def _bg_measure_all() -> None:
    ...
    try:
        ...
        await evaluate_new_servers(
            list(servers),
            records_dir(CodexCrewConfig.load().mcp_gateway.socket_path),
            budget=None,
            on_progress=report,
        )
...
async def api_mcp_quarantine_clear(request: web.Request) -> web.Response:
    ...
    the count Codex Crew accumulated on its own, so a server the user had switched
...
async def api_mcp_sync(request: web.Request) -> web.Response:
    ...
    2. Adds them to both codexcrew agent config AND global mcp.json.
    3. Makes the change reach the sessions. When every live process reconciles
       the agent file itself (:mod:`codex_crew.mcp_hot_reload`) the write has
    ...
    """
    from codex_crew.mcp_discovery import (  # noqa: F811
        codexcrew_managed_names,
    ...
    if to_sync:
        ...
        async with _get_mcp_lock():
            ...
            _managed = codexcrew_managed_names()
            for s in to_sync:
                if s.is_remote:
        ...
        from codex_crew.dashboard.handlers.agents import (
    ...
    # codexcrew.json, and on a harness without live reconcile only a restart
    ...
    if _mcp_hot_reload_active(request):
        ...
        from codex_crew.dashboard.handlers.sessions import _reset_all_sessions  # noqa: F811
...
async def api_mcp_toggle(request: web.Request) -> web.Response:
    ...
    2. Syncs ``tools``/``allowedTools`` in ``codexcrew.json`` (non-ACP mode).
    """
    body, body_err = await read_bounded_json(request)
    if body_err is not None:
        return body_err
    ...
    async with _get_mcp_lock():
        ...
        if name not in servers:
            ...
            from codex_crew.mcp_discovery import (
                list_servers as _ls,  # circular import: mcp_discovery defers imports of codex_crew.agent which shares state with this module
            )
        ...
        # 2. Sync to codexcrew.json tools/allowedTools (lock prevents lost updates vs agents.py)
        from codex_crew.dashboard.handlers.agents import _get_config_lock  # noqa: F811
...
async def api_mcp_toggle_all(request: web.Request) -> web.Response:
    ...
    async with _get_mcp_lock():
        ...
        # Batch sync: single read-modify-write of codexcrew.json
        from codex_crew.dashboard.handlers.agents import _get_config_lock  # noqa: F811
...
async def api_mcp_remove(request: web.Request) -> web.Response:
    ...
    codexcrew.json.  If the optional ``aim`` package manager happens to be
    ...
    from codex_crew.dashboard.handlers._shared import _capability_manager
...
async def api_mcp_server_detail(request: web.Request) -> web.Response:
    ...
    if request.method == "DELETE":
        ...
        # would otherwise both read codexcrew.json and the last write would drop
    ...
    # Sync to codexcrew.json (enable by default). Config lock across the offloaded
...
_CODEXCREW_MCP_JSON: Path | None = None


def _codexcrew_mcp_json() -> Path:
    """CodexCrew-scope ``mcp.json`` path, resolved against the live data home."""
    return _CODEXCREW_MCP_JSON if _CODEXCREW_MCP_JSON is not None else data_home() / "mcp.json"
...
def _atomic_write(path: Path, data: dict) -> None:
    """Atomic JSON write; secret-aware for the store owned by Codex Crew.

    That store is secret-bearing by construction (``env`` values and remote
    ``headers`` carry credentials), so it is published through
    :func:`codex_crew.atomic_write.atomic_write` with ``restrict_to_owner=True``
...
def _find_server_spec_anywhere(name: str) -> dict | None:
    ...
    Search order matches the CodexCrew merge: agent config → <data home>/mcp.json
    (``config_dir()/mcp.json``, i.e. ~/.codex-crew/mcp.json) → kiro global →
    ...
    found = _usable(
        _read_agent_spec(
            kiro_agents_dir() / "codexcrew.json",
...
def _set_codexcrew_entry(name: str, *, enabled: bool, spec: dict | None = None) -> str:
    ...
    ``"disabled"``, or ``"noop"``.
    """
    data = _load_json_or_empty(_codexcrew_mcp_json())
    servers = data.setdefault("mcpServers", {})
    ...
    _atomic_write(_codexcrew_mcp_json(), data)
    return action


def _get_codexcrew_entry(name: str) -> dict | None:
...
def _replace_codexcrew_spec(name: str, spec: dict) -> bool:
    """Replace the full spec of an existing CodexCrew-managed entry.
...
def _remove_codexcrew_entry(name: str) -> bool:
...
def _remove_from_agent_file(path: Path, name: str) -> bool:
    ...
    ``~/.kiro/agents/codexcrew.json`` / ``~/.claude/agents/codexcrew.mcp.json``
    ...
    file under ``~/.kiro/agents/codexcrew.lock`` (app enable/disable, MCP
    ...
    settings-lock holder uses when it reaches a codexcrew.json writer, and no
    ...
    if not path.is_file():
        return False
    from codex_crew.apps.bridges import _mcp_lock as _agent_file_lock
...
def _purge_server_config(name: str, *, scopes: Collection[str] | None = None) -> dict[str, str]:
    ...
    at all otherwise: they are Codex Crew's own merge output rather than a scope a
    user edits, and leaving the entry there lets the next rebuild resurrect what
    was just removed.
    """
    actions: dict[str, str] = {}
    if scopes is None or SCOPE_CODEXCREW in scopes:
        actions[SCOPE_CODEXCREW] = "removed" if _remove_codexcrew_entry(name) else "noop"
    ...
    _remove_from_agent_file(kiro_agents_dir() / "codexcrew.json", name)
    for scope in _extra_mcp_scopes():
...
async def _do_mcp_apply(request: web.Request) -> web.Response:
    ...
    Request body::

        {
          "changes": [
            {
              "name": "slack-mcp",
              "codexcrew": true,     // desired MC visibility
    ...
    so the provider-native agent files (``~/.kiro/agents/codexcrew.json`` and
    ``~/.claude/agents/codexcrew.md`` + ``codexcrew.mcp.json``) reflect the
    ...
    try:
        if uninstall_names:
            from codex_crew.dashboard.handlers._shared import _capability_manager
            from codex_crew.platform.capability_bound import (
        ...
        async with _get_mcp_lock():
            for change in changes:
                ...
                desired_mc = bool(change.get("codexcrew", True))
                ...
                if desired_mc and not desired_kiro and not any(desired_extra.values()):
                    has_mc = _scope_has_entry(name, _codexcrew_mcp_json())
                    if not has_mc:
                ...
                outcome["actions"]["codexcrew"] = await _offload_config_write(
                    _set_codexcrew_entry,
                    name,
                    enabled=desired_mc,
                    spec=preserved_spec,
                )
                ...
                # reachable from CodexCrew sessions — a permission-shaping
...
async def api_mcp_gateway_status(request: web.Request) -> web.Response:
    ...
    """
    from codex_crew.config.loader import CodexCrewConfig  # noqa: F811
    ...
    cfg = CodexCrewConfig.load().mcp_gateway
    return web.json_response(
        {
            "enabled": cfg.enabled,
...
def _local_overlay_section() -> dict:
    ...
    ``CodexCrewConfig.load``), so ``config.json`` alone is not the effective
    config. Any read-modify-write on the base file has to account for it or it
    reasons about a view the runtime never sees.
    """
    from codex_crew.config.loader import config_local_path
...
async def api_mcp_gateway_enable(request: web.Request) -> web.Response:
    ...
    """
    from codex_crew.config.loader import config_path  # circular import
    from codex_crew.dashboard.handlers.agents import _get_config_lock  # circular import
...
def _stub_eligibility(
    ...
    *,
    ...
    """
    from codex_crew.config.loader import CodexCrewConfig  # noqa: F811
    from codex_crew.mcp_gateway.evaluate import identity_for
    ...
    try:
        runtime = records_dir(CodexCrewConfig.load().mcp_gateway.socket_path)
...
async def api_mcp_gateway_servers(request: web.Request) -> web.Response:
    ...
    gw_cfg = CodexCrewConfig.load().mcp_gateway
...
def _load_shareability_state() -> tuple[
    ...
    """
    try:
        rt = records_dir(CodexCrewConfig.load().mcp_gateway.socket_path)
    ...
    *,
    ...
    return assess(
        ShareEvidence(
            ...
            # when it declines the caller-identity extension: codexcrew-core
            # consumes the injected caller block and shares a backend safely,
            # while codexcrew-cron reads process identity. Asking the server's own
...
async def api_mcp_gateway_set_stub(request: web.Request) -> web.Response:
    ...
    """
    from codex_crew.config.loader import (  # noqa: F811
        ConfigReadError,
        config_path,
