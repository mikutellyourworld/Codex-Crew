...
from codex_crew.apps.manager import disable_app, get_app, list_apps
from codex_crew.apps.official_catalog import CatalogUnavailable
...
from codex_crew.apps.routes import app_lifecycle_lock
from codex_crew.apps.teardown import teardown_app_runtime
...
from codex_crew.security import DENY_REASON_MATCH_PREFIX, edition_denied_rules
...
def _denied_state(data: dict) -> dict:
    ...
    *data* is the file root (the opt-out object itself), not a config wrapper.
    """
    from codex_crew.hooks import _coerce_bool
...
async def _pinned_ids_for_snapshot_async() -> set:
    ...
    """
    from codex_crew.security import pinned_builtin_command_ids_for_snapshot
...
def _reload_live_hooks(request: web.Request, denied_state: dict) -> None:
    ...
    from codex_crew.hooks import HooksConfig
...
def build_trusted_apps_snapshot() -> dict:
    ...
    """
    agent = CodexCrewConfig.load().agent
...
class TrustSettingOverlayOwned(Exception):
    ...
    ``CodexCrewConfig`` deep-merges the user-owned ``config.local.json`` overlay OVER
    ...
    Codex Crew — so the caller is told which key and which file to edit.
    """
...
async def _mutate_agent_config(mutate) -> None:
    ...
    **Why a targeted raw edit and not ``CodexCrewConfig.load()`` → ``save()``.**
    ...
    parse as a JSON object. ``CodexCrewConfig.load()`` degrades a corrupt file to
...
async def api_trusted_app_grant(request: web.Request) -> web.Response:
    ...
    # against other in-process lifecycle work, but `codexcrew app uninstall` runs in a
    ...
    async with app_lifecycle_lock(name):
        ...
        if was_granted and not is_builtin:
            ...
            if record:
                ...
                # (``codexcrew app disable``) calls it from a DIFFERENT PROCESS
...
def _distribution_posture() -> dict:
    ...
    :func:`codex_crew.platform.policy_distribution.distribution_posture`, which is
    ...
    """
    try:
        from codex_crew.platform.policy_distribution import distribution_posture
