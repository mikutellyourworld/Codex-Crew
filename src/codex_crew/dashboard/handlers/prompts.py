...
from codex_crew.skill_trust import ReviewedProjectChanged as _ReviewedProjectChanged
from codex_crew.skill_trust import (
    TrustStoreFull,
...
def _list_aim_prompts(project_dir=None):
    ...
    import codex_crew.dashboard.handlers as _pkg
...
async def api_prompts(request: web.Request) -> web.Response:
    ...
    # process-wide CODEXCREW_PROJECT_DIR — which on a source install names the
    # Codex Crew tree itself and on a wheel install names nothing, so a prompt
...
def _pin_prompt_dir(d: Path, *, create: bool = False) -> int:
    ...
    translation, the capability probe — lives in :mod:`codex_crew.pinned_fs`,
...
def _user_prompt_dir(scope: str, project_dir: Path | None) -> Path | None:
    ...
    ``CODEXCREW_PROJECT_DIR``, which names the Codex Crew tree on a source install
...
async def _assemble_skills_catalog_uncached(
    ...
    offloads ALL blocking filesystem work — codexcrew ``list_skills()`` (os.walk +
...
async def api_skills(request: web.Request) -> web.Response:
    ...
    Sources:
    - ``codexcrew``: ``~/.codex-crew/skills/`` (managed by SkillsLoader; editable)
    ...
    still be loaded via CodexCrew text-injection or an external MCP server).
...
async def api_skills_pending(request: web.Request) -> web.Response:
    ...
    def _prune_and_list() -> list:
        ...
        try:
            ttl = CodexCrewConfig.load().skills.pending_ttl_days
...
async def api_skill_pending_approve(request: web.Request) -> web.Response:
    ...
    def _approve_and_bound() -> str | None:
        ...
        if nm:
            ...
            try:
                cfg = CodexCrewConfig.load().skills
