...
auditing — happen gateway-side in :mod:`codex_crew.mcp_gateway.app_call`, so a
...
from codex_crew.dashboard.chat_utils import _history_key_for, effective_session_key
from codex_crew.dashboard.handlers._shared import _is_restricted_session, _read_session_key
from codex_crew.mcp_apps_render import load_spool
from codex_crew.mcp_gateway import transport
from codex_crew.mcp_gateway.rewriter import default_socket_path
...
def _socket_path() -> str:
    ...
    override (empty -> the standard ``$CODEXCREW_HOME`` location)."""
    try:
        # circular import: config.loader pulls dashboard modules; keep lazy.
        from codex_crew.config.loader import CodexCrewConfig

        configured = (CodexCrewConfig.load().mcp_gateway.socket_path or "").strip()
        if configured:
            return configured
