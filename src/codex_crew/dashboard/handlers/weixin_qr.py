...
from codex_crew.config.loader import CRED_WEIXIN_TOKEN, CodexCrewConfig, config_path, env_path
...
from codex_crew.dashboard.handlers.messaging import is_direct_local_request
from codex_crew.weixin.client import ILINK_BASE_URL, WeixinClient
...
def _render_qr_data_uri(scan_data: str) -> str:
    ...
    gateway event loop. Delegates to :mod:`codex_crew.qr`, the single owner of QR
    encoding, so this surface and tailnet mobile access cannot drift into two
    different encoders.
    """
    from codex_crew.qr import render_qr_data_uri  # noqa: PLC0415 - heavy import, QR path only
...
def _atomic_write(path: Path, text: str, *, secret: bool = False) -> None:
    ...
    Delegates to :func:`codex_crew.atomic_write.atomic_write`, which every
...
def _write_env_secret(key: str, value: str) -> None:
    """Upsert ``KEY=VALUE`` in ~/.codex-crew/.env (0600), preserving other lines."""
    ep = env_path()
    lines: list[str] = []
    found = False
    if ep.exists():
...
def _read_env_value(key: str) -> Optional[str]:
    """Return the current value of ``key`` in ~/.codex-crew/.env, or None if absent.
...
def _delete_env_key(key: str) -> None:
    """Remove ``key`` from ~/.codex-crew/.env, preserving every other line."""
    ep = env_path()
    if not ep.exists():
        return
    kept: list[str] = []
