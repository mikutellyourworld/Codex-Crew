...
runtime service (RTS). This module calls that same API so CodexCrew surfaces the
...
from codex_crew.identity_stores import Trust, sqlite_dbs
...
_USER_AGENT = "AmazonQ-For-CLI/1.24.0 ua/2.0 os/linux lang/rust CodexCrew"
_TIMEOUT_SECS = 15
...
# ``codex_crew.hooks._INTERNAL_READ_ALLOWLIST``.
...
def _map_response(data: dict) -> dict | None:
    """Translate a GetUsageLimits response into CodexCrew's canonical usage dict.
