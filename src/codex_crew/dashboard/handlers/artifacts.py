...
from codex_crew import hooks, publish_sync
from codex_crew import sel as _sel_mod
from codex_crew.artifact_source import LINK, classify_source
...
from codex_crew.hooks import FileTooLargeError, safe_read_file_bytes_with_identity, stat_identity
...
from codex_crew.validation import infer_use_case


def sel():
    """Late-resolved sel() — calls the module function so test patching of
    ``codex_crew.sel.sel`` (the canonical patch target) continues to work."""
    return _sel_mod.sel()
...
def _validate_inbound_webapp_metadata(body: dict[str, Any]) -> str | None:
    ...
    if body.get("webapp_metadata") is None:
        return None
    from codex_crew.validation import ValidationError, _validate_artifact_save
...
def _publish_governance_denied(request: web.Request, provider_name: str) -> str | None:
    ...
    Thin alias for :func:`codex_crew.publish_governance.publish_denied_reason`,
