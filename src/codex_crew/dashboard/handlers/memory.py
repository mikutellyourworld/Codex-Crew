...
from codex_crew.executors import embed_executor, run_in_embed_pool
...
def _get_vector_store(state: DashboardState):
    ...
    if not hasattr(state, "_standalone_vector"):
        ...
        # observe it. ``CodexCrewConfig`` deliberately shadows this module's own
        ...
        from codex_crew.vector_memory import VectorMemoryStore  # noqa: F811
...
async def api_memory_semantic_write(request: web.Request) -> web.Response:
    ...
    if err is not None:
        ...
        from codex_crew.vector_memory import SemanticRejectCode
    ...
    try:
        ...
        if not ready:
            if candidate_installed:
                ...
                reset_shared_embedder()
            prog.fail(
                "the model did not load — run 'codexcrew doctor' for the reason "
...
async def api_memory_embedding_model(request: web.Request) -> web.Response:
    ...
    # CODEXCREW_EMBED_MODEL_PATH wins over memory.embed_model_path for the PATH,
    ...
    if os.environ.get("CODEXCREW_EMBED_MODEL_PATH", "").strip():
        return web.json_response(
            {"ok": False,
             "error": "CODEXCREW_EMBED_MODEL_PATH is set, so it overrides the configured "
    ...
    async with _faiss_install_lock:
        try:
            ...
            try:
                ...
                # `codexcrew doctor` points the user at a manual
...
async def api_memory_stats(request: web.Request) -> web.Response:
    ...
    # ``CodexCrewConfig`` at call time is what lets a test substitute it on the
    # source module.
    from codex_crew.config.loader import CodexCrewConfig  # noqa: F811
    ...
    from codex_crew.memory import legacy_memory_present  # noqa: F811
...
def _build_memory_graph(mem: Any, lessons: list) -> tuple[list[dict], list[dict]]:
    ...
    # "CodexCrew (Public)"): the full header almost never occurs verbatim inside
    ...
    # parenthetical qualifier stripped: "CodexCrew (Public)" -> "codexcrew",
