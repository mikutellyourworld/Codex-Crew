...
Codex Crew is self-hosted, open-source software: every install runs on its own
arbitrary origin (``localhost:5486``, a self-hosted domain, whatever a user
...
(``SessionPulseSurveyCard.tsx``) to the Codex Crew backend instead: the frontend
...
(``beacon.telemetry_permitted``). When that gate says no (``CODEXCREW_TELEMETRY_DISABLED``,
...
from codex_crew.dashboard.session_pulse_counter import (
...
_FORM_CATEGORY = "CodexCrew"  # brand-ok: literal Aperture portal identifier
...
_SERVICE_ID = "CodexCrew"  # brand-ok: literal Aperture portal identifier
...
# template (GET rendering.../form/template for category=CodexCrew,  # brand-ok: registered category id
...
_RATING_QUESTION = "How would you rate your experience with CodexCrew today?"  # brand-ok: verbatim registered template text, ingestion 400s on mismatch
...
def _telemetry_permitted(audit_tool: str = "") -> bool:
    ...
    same two config inputs the beacon and ``codexcrew telemetry status`` use.
...
async def api_feedback_submit(request: web.Request) -> web.Response:
    ...
    codex_crew_version = str(body.get("kiroCrewVersion") or "")
    codex_crew_version, _ = redact_exfiltration_urls(codex_crew_version)
    codex_crew_version, _ = redact_credentials(codex_crew_version)
    ...
    payload = {
        ...
        # GET rendering.../form/template?category=CodexCrew&name=SessionFeedback&version=1.0.1,  # brand-ok: registered category id
        # whose own metadataList lists userId, sessionId, codex_crew_version in
        # that order.
        "metadataList": [
            {"key": "userId", "value": user_id, "pii": True},
            {"key": "sessionId", "value": session_id, "pii": False},
            {"key": "codex_crew_version", "value": codex_crew_version, "pii": False},
        ],
    }
