...
lock + ``_set_codexcrew_entry`` / ``_replace_codexcrew_spec``) so there is
...
from codex_crew.dashboard.handlers import mcp as _mcp
from codex_crew.dashboard.handlers.mcp import (
    _find_server_spec_anywhere,
    _get_codexcrew_entry,
    _get_mcp_lock,
    _is_valid_mcp_name,
    _replace_codexcrew_spec,
)
from codex_crew.mcp_discovery import MCP_REDACTED_HEADER_VALUE, redact_mcp_headers
...
# Codex Crew validates only their SHAPE and never interprets them: the runtime
...
def _load_codexcrew_config_strict() -> dict | None:
    ...
    """
    try:
        text = _mcp._codexcrew_mcp_json().read_text(encoding="utf-8")
...
async def api_mcp_custom_add(request: web.Request) -> web.Response:
    ...
    async with _get_mcp_lock():
        ...
        data = _load_codexcrew_config_strict()
        if data is None:
            sel().log_api_access(
                caller="dashboard",
            ...
            return web.json_response(
                {
                    ...
                    f" remove {_mcp._codexcrew_mcp_json()} and retry"
                },
                status=500,
            )
        ...
        await _mcp._offload_config_write(_mcp._atomic_write, _mcp._codexcrew_mcp_json(), data)
...
async def api_mcp_custom_get(request: web.Request) -> web.Response:
    ...
    async with _get_mcp_lock():
        entry = _get_codexcrew_entry(name)
    if entry is None:
...
async def api_mcp_custom_update(request: web.Request) -> web.Response:
    ...
    404 when the name isn't a CodexCrew-managed entry.  The server's
    ...
    The authorship marker is the one exception: it records that Codex Crew
    ...
    async with _get_mcp_lock():
        existing = _get_codexcrew_entry(name)
        if existing is None:
        ...
        replaced = await _mcp._offload_config_write(_replace_codexcrew_spec, name, spec)
    if not replaced:
        return web.json_response({"error": f"server '{name}' not found"}, status=404)
