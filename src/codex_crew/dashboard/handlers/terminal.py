...
from codex_crew.dashboard.origin import check_origin, mark_audit_claimed
from codex_crew.executors import discovery_executor, subprocess_executor
from codex_crew.hooks import validate_file_path
from codex_crew.sandbox import _PYTHON_ENV_PREFIXES
...
def _sel():
    import codex_crew.dashboard.handlers as _pkg  # circular import: __init__ imports terminal
...
class _ConptyBackend(Protocol):
    """Structural type for the Windows ConPTY backend (:class:`codex_crew.conpty.WindowsPty`).
...
_READY_TOKEN_VAR = "CODEXCREW_TERMINAL_READY_TOKEN"
_READY_HOOK_VAR = "CODEXCREW_TERMINAL_READY_HOOK"
...
def _pty_child_env(extra: dict[str, str]) -> dict[str, str]:
    ...
    *extra*, minus Codex Crew's own Python startup variables.
    ...
    Python 3.13 venv import Codex Crew's 3.12 site-packages and its C extensions
...
def _bash_ready_env(token: str) -> dict[str, str]:
    ...
    hook = (
        ...
        f"builtin printf '\\033]697;CodexCrewReady;%s\\007' "
        f'"${{{_READY_TOKEN_VAR}}}"; '
...
async def api_terminal_ws(request: web.Request) -> web.WebSocketResponse | web.Response:
    ...
    if existing:
        ...
        # no POSIX pty/fork; codex_crew.conpty drives the Win32 pseudo-console via
        # ctypes (stdlib, no extra dependency).
        from codex_crew.conpty import WindowsPty
        ...
        env = _pty_child_env({"CODEXCREW_TERMINAL": "1"})
        ...
        try:
            ...
            if _is_bash_shell(shell):
                ...
                ready_marker = (
                    f"\x1b]697;CodexCrewReady;{token}\x07".encode("ascii")
                )
...
def _entry_is_sensitive(canonical_dir: str, entry: os.DirEntry) -> bool:
    ...
    Vetting only the DIRECTORY is not enough: ``~/.codex-crew`` is not itself on
...
def _list_vetted_completions(
    ...
    ``CodexCrew-terminal-completion``. Each entry reports ``at``, the offset the
