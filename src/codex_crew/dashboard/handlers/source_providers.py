...
from codex_crew import github_runner, platform_compat
from codex_crew.config.loader import CodexCrewConfig, config_dir, read_env_file_credential
...
# owned by the shared hardened runner (codex_crew.github_runner) so every
...
from codex_crew.github_runner import GH_ENV_PASSTHROUGH as _GH_ENV_PASSTHROUGH
from codex_crew.github_runner import (
    PROVIDER_EXECUTABLE_CANDIDATES as _PROVIDER_EXECUTABLE_CANDIDATES,
)
from codex_crew.github_runner import STRICT_PROVIDER_BIN_ENV as _STRICT_PROVIDER_BIN_ENV
from codex_crew.github_runner import (
    provider_executable_candidates,
)
from codex_crew.github_runner import strict_provider_bins as _strict_provider_bins
from codex_crew.github_runner import validate_provider_executable as _validate_provider_executable
from codex_crew.history_search import register_search_ref_resolver as _register_search_ref_resolver
from codex_crew.history_search import (
...
def _sel():
    import codex_crew.dashboard.handlers as _pkg  # circular import: package exports this module
...
def _provider_setup_message(executable: str, override_name: str, last_error: str) -> str:
    ...
    return (
        ...
        "available to the Codex Crew gateway.\n"
        "\n"
...
def register_source_provider(plugin: SourceProviderPlugin) -> None:
    ...
    # gate and the `codexcrew memory search` CLI -- and a process that never ran a
...
def _load_source_link_settings() -> tuple[frozenset[str], frozenset[str], bool]:
    ...
    BLOCKING -- never on the loop. ``CodexCrewConfig.load()`` stats, reads,
    ...
    try:
        ...
        dashboard = CodexCrewConfig.load().dashboard
        return (
...
def _get_jira_auth(host: str) -> tuple[str, str] | None:
    ...
    ``codexcrew secrets import``), falling back to the protected .env file /
    ...
    try:
        ...
        # CodexCrewConfig.load() / load_credentials() runs.  load_credentials()
    ...
    for entry in entries:
        ...
        if entry_host == normalized:
            ...
            # populated by `codexcrew secrets import`), then the legacy .env /
            ...
            # var is the effective credential at runtime — and `codexcrew secrets
...
async def _fetch_jira_issue(ref: SourceRef) -> dict[str, Any]:
    ...
    """
    # Offload config I/O to a thread — CodexCrewConfig.load() is synchronous
...
# before ``CODEXCREW_OWNER_ID`` was configured carries `local-app` /
    ...
    if not owner_id:
        ...
        if is_local_dashboard:
            ...
            return web.json_response(
                {
                    "error": (
                        "this action needs a configured owner, which Codex Crew"
