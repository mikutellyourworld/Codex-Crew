...
-----------------------
* ``branch`` must satisfy :func:`~codex_crew.validation.is_valid_followup_branch`,
...
from codex_crew.dashboard.chat_handlers import deny_non_dashboard_caller
...
from codex_crew.validation import MAX_FOLLOWUP_BRANCH, is_valid_followup_branch
...
# (`.git/codexcrew-no-hooks`) is resolved relative to the repo, so whoever prepared
...
_SANDBOX_REFUSAL = (
    "This host has no OS sandbox backend, so Codex Crew will not run git for you. "
