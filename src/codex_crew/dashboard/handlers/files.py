...
from codex_crew import file_delivery_consent, pinned_fs, platform_compat
from codex_crew.atomic_write import (
    atomic_write,
    open_access_control_source,
    pinned_parent_replace_supported,
)
from codex_crew.config import loader as config_loader
from codex_crew.config.loader import CodexCrewConfig, WorkspaceConfig, config_dir, data_home
from codex_crew.dashboard import part_stream, upload_destination
from codex_crew.dashboard.chat_utils import dashboard_slot_key
from codex_crew.dashboard.file_index import _SKIP_DIRS as _WALK_SKIP_DIRS
from codex_crew.dashboard.handlers._shared import _probe_persisted_session, read_bounded_json
from codex_crew.dashboard.origin import is_direct_local_request
from codex_crew.dashboard.state import DashboardState, append_and_surface
from codex_crew.doc_parser import extract_text
from codex_crew.hooks import FileTooLargeError, safe_read_file_bytes, safe_read_prefix
...
from codex_crew.messaging.raster import SNIFF_BYTES, sniff_raster_mime
from codex_crew.platform import redact_via_context as redact
from codex_crew.sandbox import popen_limited, sandboxed_spawn_argv
...
from codex_crew.slack.handler import is_tracked_channel
from codex_crew.validation import (
    FILE_READ_SCHEMA,
    ValidationError,
    validate_tool_args,
)
from codex_crew.zip_vet import ZipInventoryRejected, vet_zip_inventory_bytes
...
async def api_outbox_notify(request: web.Request) -> web.Response:
    ...
    from codex_crew.config.loader import outbox_dir  # noqa: F811
    from codex_crew.hooks import FileTooLargeError, safe_read_file_bytes  # noqa: F811
...
async def api_outbox_download(request: web.Request) -> web.StreamResponse:
    ...
    if is_text:
        ...
        if redacted != text:
            ...
            from codex_crew.dashboard.handlers.source_providers import (  # lazy: import cycle
...
async def api_slack_upload_file(request: web.Request) -> web.Response:
    ...
    (:func:`codex_crew.dashboard.upload_destination.resolve_slack`), which holds
...
async def api_channel_upload_file(request: web.Request) -> web.Response:
    ...
    Destination and authorization come from the shared oracle
    (:func:`codex_crew.dashboard.upload_destination.resolve_channel`), which for
...
# verified by the shared sniffer (:mod:`codex_crew.messaging.raster`), so all
...
async def _stream_video_part(
    ...
    All the file handling lives in :func:`~codex_crew.dashboard.part_stream.
...
async def api_workspaces_create(request: web.Request) -> web.Response:
    ...
    from codex_crew.validation import WORKSPACE_NAME_RE  # noqa: F811
...
async def api_file_read(request: web.Request) -> web.Response:
    ...
    from codex_crew.validation import (  # noqa: F811
        FILE_READ_SCHEMA,
        ValidationError,
        validate_tool_args,
    )
...
def _open_checked_file(
    ...
    *log_open_failure* keeps log volume a per-endpoint decision: a
    ...
    """
    import codex_crew.dashboard.handlers as _h  # noqa: F811  # circular import
...
# Extensions previewable via codex_crew.doc_parser (OOXML docx/pptx). Legacy
...
async def api_file_office_preview(request: web.Request) -> web.Response:
    ...
    Uses ``codex_crew.doc_parser.extract_text`` which parses the .docx /
...
async def api_file_raw(request: web.Request) -> web.Response:
    ...
    # (codex_crew.messaging.raster), which requires the full PNG signature and
...
def _resolve_project_relative(raw: str) -> tuple[str, str | None]:
    """Resolve a relative path against CODEXCREW_PROJECT_DIR (resolve=1).
...
async def api_file_search(request: web.Request) -> web.Response:
    ...
    # test that stubs ``codex_crew.security.is_sensitive_path`` is observed by the
    # project-root rejection below.
    from codex_crew.security import is_sensitive_path  # noqa: F811
    ...
    if project:
        ...
        from codex_crew.config.loader import workspace_dir_for  # noqa: F811
    ...
    if not search_roots:
        # Fallback: project dir, then the codexcrew workspace.
        #
...
async def api_browse_dirs(request: web.Request) -> web.Response:
    ...
    if is_sensitive_path(base):
        ...
        return web.json_response({"error": "Access denied"}, status=403)
    skip = {".git", "node_modules", "__pycache__", ".cache", ".venv", "venv", "env", ".codexcrew", ".kiro", ".aim"}
...
def _project_git_branch(base: str) -> dict:
    ...
    for _ in range(_GIT_ROOT_WALK_LIMIT):
        ...
        # probe for existence rather than is_dir() — otherwise every CodexCrew
...
async def api_browse_files(request: web.Request) -> web.Response:
    ...
    if is_sensitive_path(base):
        ...
        return web.json_response({"error": "Access denied"}, status=403)
    skip = {".git", "node_modules", "__pycache__", ".cache", ".venv", "venv", "env", ".codexcrew", ".kiro", ".aim", "build", "dist", ".next"}
...
async def api_dashboard_config(request: web.Request) -> web.Response:
    ...
    # SEL audit on denial) compared to the CodexCrewConfig.load() thread hop
    ...
    # Offloaded: CodexCrewConfig.load() stats, reads, parses, and validates config
    ...
    if request.method == "PUT":
        ...
        #    `codexcrew config set` (which takes that same file lock) cannot land
        ...
        from codex_crew.config.loader import update_config_locked  # noqa: F811
        from codex_crew.dashboard.handlers.agents import (  # lazy: import cycle
            _get_config_lock,
        )
    ...
    from codex_crew.dashboard import social_share
...
def _vet_zip_eocd(data: bytes) -> None:
    ...
    Delegates to the shared vet (codex_crew.zip_vet) so this endpoint, knowledge
