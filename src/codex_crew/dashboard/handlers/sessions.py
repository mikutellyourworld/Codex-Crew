...
if TYPE_CHECKING:
    from codex_crew.providers.base import LLMProvider  # noqa: F811
...
from codex_crew import session_ledger
from codex_crew.acp.client import _resolve_kiro_bin_for_spawn
from codex_crew.config.paths import kiro_agents_dir
from codex_crew.dashboard import directive_queue
from codex_crew.dashboard.handlers import kiro_usage_api
from codex_crew.dashboard.handlers._shared import SESSION_SEARCH_TEXT_FIELDS
from codex_crew.dashboard.kiro_readiness import reject_if_kiro_unverified
from codex_crew.dashboard.session_memory import SessionMemorySampler
from codex_crew.dashboard.state import DashboardState
from codex_crew.executors import subprocess_executor
from codex_crew.history import SEARCH_MIN_CHARS, _archive_dir, is_incognito_transcript
from codex_crew.llm_helpers import run_bg_oneliner
from codex_crew.mcp_discovery import sync_discovered_servers
...
from codex_crew.security import redact, redact_credentials, redact_exfiltration_urls
from codex_crew.validation import sanitize_string
    ...
    if now - _health_cache_ts > _HEALTH_REFRESH_SECS:
        async with _health_lock:
            ...
            if time.monotonic() - _health_cache_ts > _HEALTH_REFRESH_SECS:
                try:
                    from codex_crew.dashboard import session_health
...
def _text_scrape_enabled() -> bool:
    ...
    try:
        ...
        return bool(CodexCrewConfig.load().dashboard.usage_text_scrape_enabled)
    except Exception:
...
def _wrap_argv_usage_scrape(kiro_bin: str) -> tuple[list[str], str | None]:
    ...
    return _wrap_argv_at_configured_tier(
        [kiro_bin, "chat", "--no-interactive", "--agent", "codexcrew-lite", "/usage"]
    )
...
def _open_slot_transcript_keys(state: DashboardState) -> set[str]:
    ...
    ``discord:codexcrew:direct:123`` mapped to a stem that does not exist.
    ...
    """
    from codex_crew.dashboard.chat_utils import slot_history_key, slot_transcript_key
...
async def _remove_slot_for_history_key(state: DashboardState, key: str) -> None:
    ...
    """
    from codex_crew.dashboard.state import _normalize_slot_key
...
async def api_session_directive(request: web.Request) -> web.Response:
    ...
    a Codex Crew directive tool (``mcp_tools.control._emit_directive``), which has
    ...
    :mod:`codex_crew.dashboard.directive_queue` for why that is not weaker than the
    ...
    Codex Crew's own directive tools, so an unrecognized kind means the request did
...
def _service_wait_ping(
    ...
    # MCP gateway disabled (the default) CODEXCREW_SESSION_KEY is unset and one MCP
    ...
    # Tracked in https://github.com/mikutellyourworld/Codex-Crew/issues/2347, which also
...
async def api_session_tool_policy(request: web.Request) -> web.Response:
    ...
    Used by managed MCP servers (codexcrew-core, codexcrew-cron) to filter
...
async def api_sessions_restart(request: web.Request) -> web.Response:
    ...
    Also syncs MCP servers from mcp.json → codexcrew.json so newly
...
async def api_session_archive_list(request: web.Request) -> web.Response:
    ...
    from codex_crew.history import _archive_dir, _safe_key
