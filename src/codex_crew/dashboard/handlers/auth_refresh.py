...
from codex_crew.dashboard.origin import check_origin, is_https_request
...
async def api_auth_refresh(request: web.Request) -> web.Response:
    ...
    # (atomic-rename write to ~/.codex-crew/refresh_chains.json) — must
...
async def api_auth_logout(request: web.Request) -> web.Response:
    ...
    2. Access cookie: ``revoke_access_cookie`` adds the access cookie's nonce
       ...
       ``revoke_all_sessions`` (codexcrew logout) which kills every session.
