...
from codex_crew import __version__ as _local_version
from codex_crew import dep_sync, shutdown_event
from codex_crew.changelog import Release, base_version, build_release_list, release_of_build
...
from codex_crew.dashboard.handlers._shared import read_capped_response
from codex_crew.dashboard.state import DashboardState, chat_message_frame
...
from codex_crew.platform import feed_trust
...
from codex_crew.platform.update_layout import cdn_bases as _cdn_bases
from codex_crew.platform.update_layout import detect_install_layout
from codex_crew.platform.update_layout import release_channel as _release_channel
from codex_crew.platform.update_layout import set_release_channel, wheel_update_command
from codex_crew.platform.update_provider import CommandProvider, resolve_provider
from codex_crew.platform_compat import reexec_python_module
from codex_crew.safety_override import flush_breadcrumb_writes
...
_CLI_MANIFEST_SCHEMA = "codexcrew-cli-artifact-manifest-v1"
...
def status_update_fields() -> dict[str, object]:
    ...
    return {
        ...
        # installed ``codex_crew.__version__``, which is never folded either
...
def _invalidate_update_check(channel: str) -> None:
    ...
    ``$CODEXCREW_HOME/channel`` would be both redundant and a synchronous read on a
...
async def _do_update_check() -> None:
    ...
    :func:`~codex_crew.platform.update_capability.derive_capability`, the one
    ...
    * **git checkout** — fetch the remote, then report an update when the
      ...
      when the remote ``src/codex_crew/__init__.py`` ``__version__`` outranks the
    ...
    try:
        ...
        try:
            if generation != _check_generation:
                ...
                # Offloaded: this reads $CODEXCREW_HOME/channel, and the data home can
...
async def _check_git_checkout(proj: str, capability: UpdateCapability) -> None:
    ...
    show = await asyncio.create_subprocess_exec(
        "git",
        "show",
        f"{target_sha}:src/codex_crew/__init__.py",
        cwd=proj,
        stdout=asyncio.subprocess.PIPE,
        stderr=asyncio.subprocess.DEVNULL,
    )
...
async def _check_release_feed(capability: UpdateCapability) -> None:
    ...
    **Security posture — the manifest is UNTRUSTED display metadata, with one
    ...
    # `managed_by == "codexcrew"` also covers bare source installs the arm
    # endpoint refuses, so shipping the wider signal would render a dead button.
    # Offloaded — the probe resolves venv paths on disk.
    from codex_crew.platform.wheel_engine import running_from_managed_venv
    ...
    _set_update_info(
        **base,
        ...
        # ``codex_crew/__init__``) is a build OF ``0.6.0``, which the lane did
...
def _changelog_path() -> Path | None:
    ...
    1. ``CODEXCREW_PROJECT_DIR/CHANGELOG.md`` — dev / git installs.
    2. Bundled ``codex_crew/CHANGELOG.md`` — pip-wheel installs where
    ...
    if proj:
        ...
        if p.is_file():
            return p
    # updates.py lives at codex_crew/dashboard/handlers/ — parents[2] == codex_crew/
...
async def api_releases(request: web.Request) -> web.Response:
    ...
    belongs to; :mod:`codex_crew.changelog` owns that rule and the reasoning.
...
async def _build_frontend(proj: str, state: DashboardState) -> None:
    ...
    Delegates to the shared :func:`codex_crew.frontend.build_frontend_async`
    helper so the build/stage logic is not duplicated across the three update
    paths (CLI ``codexcrew update``, this dashboard endpoint, and the gateway
    auto-update). That helper runs ``npm ci`` (fallback ``npm install``) then
    ``npm run build`` in ``<proj>/website`` and copies ``website/dist`` into
    the served ``src/codex_crew/static/dist`` — without which the dashboard
    ...
    redaction.
    """
    from codex_crew import frontend
...
async def _venv_pip_install(proj: str, state: DashboardState) -> bool:
    ...
    Delegates the choice of HOW to :func:`codex_crew.dep_sync.sync_or_reinstall`:
...
async def _restart_gateway(state: DashboardState) -> bool:
    ...
    try:
        ...
        if not os.path.isfile(exe) or not os.access(exe, os.X_OK):
            ...
            return False
        # circular import: codex_crew.dashboard.chat imports from
        # codex_crew.dashboard.handlers (which re-exports this module), so this
        ...
        reexec_python_module("codex_crew", sys.argv[1:], executable=exe)
        return True
    finally:
...
async def api_update_apply(request: web.Request) -> web.Response:
    ...
    proj = os.environ.get("CODEXCREW_PROJECT_DIR", "")
    if not proj:
        return web.json_response({"error": "CODEXCREW_PROJECT_DIR not set"}, status=400)
    ...
    if capability.managed_by != MANAGED_BY_GIT:
        return web.json_response(
            {"error": "Not a git checkout — update by redeploying (e.g. `codexcrew cloud launch`)"},
            status=409,
        )
...
async def api_log_level(request: web.Request) -> web.Response:
    """POST /api/logs/level — change the codex_crew logger level at runtime.
    ...
    root = logging.getLogger("codex_crew")
...
async def api_log_level_get(request: web.Request) -> web.Response:
    """GET /api/logs/level — current codex_crew logger level."""
    root = logging.getLogger("codex_crew")
...
def install_log_ring_handler() -> _RingLogHandler | None:
    ...
    logging.getLogger("codex_crew").addHandler(handler)
...
def _loopback_peer(request: web.Request) -> bool:
    ...
    if request.get("internal_auth") or request.get("peer_verified"):
        return True
    from codex_crew.dashboard.origin import request_is_unix_socket
    from codex_crew.dashboard.urls import is_loopback
...
async def api_update_arm(request: web.Request) -> web.Response:
    ...
    from codex_crew.platform import update_stepup
    from codex_crew.platform.wheel_engine import running_from_managed_venv
...
async def api_update_approve(request: web.Request) -> web.Response:
    ...
    Called by ``codexcrew update approve`` on the gateway host, which read the
    ...
    from codex_crew.platform import update_stepup
    from codex_crew.platform.update_layout import cdn_bases as _cdn
    from codex_crew.platform.update_layout import cdn_bases_are_safe as _cdn_safe
    from codex_crew.platform.wheel_engine import WheelUpdateError, apply_wheel_update
    ...
    from codex_crew.sel import sel as _sel
    ...
    async def _apply() -> None:
        ...
        try:
            ...
            # basic-auth credentials in the URL), and the codex_crew logger
