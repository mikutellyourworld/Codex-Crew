...
from codex_crew import webhooks
from codex_crew.agent import _VALID_HOOK_EVENTS, _shipped_defaults, kiro_agents_dir_path
from codex_crew.agent_discovery import _read_agent_spec, list_agents
from codex_crew.config.loader import CodexCrewConfig, data_home
...
def _get_hook_store(state: DashboardState):
    ...
    if state._hook_store is None:
        from codex_crew.hooks import (  # noqa: F811  # circular import
            ScriptHookStore,
...
async def api_kiro_hooks(request: web.Request) -> web.Response:
    """GET /api/kiro-hooks — read-only view of kiro-cli agent hooks from codexcrew.json."""
    from codex_crew.platform import redact_via_context as redact

    agent_cfg = kiro_agents_dir_path() / "codexcrew.json"
    # ``codexcrew.json`` lives in the user-writable, tool-shared agents dir, so
...
async def api_hook_test(request: web.Request) -> web.Response:
    ...
    # circular import: codex_crew.hooks pulls dashboard state at module load, so
    # this handler defers the import to call time (matches _get_hook_store above).
    from codex_crew.hooks import HOOK_EVENT_STOP, run_script_hook  # noqa: F811
...
def _load_hook_context(hook_id: str) -> str:
    ...
    lives in :func:`codex_crew.webhooks.resolve_context` so the dashboard's
...
def _verify_hook_token(request: web.Request) -> str | None:
    ...
    if auth.startswith("Bearer "):
        ...
        candidate = request.headers.get("x-codexcrew-token", "")
    if not candidate:
        return None
...
async def api_hooks_agent(request: web.Request) -> web.Response:
    ...
    Payload:
        ...
        agent (str): agent name for routing (default: codexcrew)
        deliver (bool): send result to Slack DM + dashboard notification
        timeoutSeconds (int): max agent run duration

    Headers:
        Authorization: Bearer <token>  (or X-CodexCrew-Token)
        X-CodexCrew-Timestamp: unix seconds — required when the token requires
            signatures
        X-CodexCrew-Signature: sha256=<hex hmac of "<timestamp>.<raw body>">
    """
...
async def _run_hook_inner(
    ...
    from codex_crew.providers.base import EVENT_COMPLETE, EVENT_TEXT_CHUNK  # noqa: F811
    ...
    if is_new and state.context_builder:
        ...
        full_message, _ = await run_in_embed_pool(
            ...
            provider_type=CodexCrewConfig.load().agent.provider,
        )
    result_text = ""
    ...
    try:
        # circular import: reached while codex_crew.slack.handler is still
...
async def api_webhook_test(request: web.Request) -> web.Response:
    ...
    if not message:
        message = (
            "This is a Codex Crew webhook test request. Reply with a one-line "
            "confirmation that you received it; no other action is needed."
        )
    agent = body.get("agent") or "codexcrew"
