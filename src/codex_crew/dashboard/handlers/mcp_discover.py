...
(``_set_codexcrew_entry`` + the file lock) so there is exactly one code
...
from codex_crew.mcp_providers.base import ProviderRegistry, ProviderUnavailableError
...
def _build_registry() -> ProviderRegistry:
    ...
    from codex_crew.dashboard.handlers._shared import admits_registry
...
def _configured_server_names() -> set[str]:
    """Names of all MCP servers CodexCrew currently knows about.

    Same source as the ``/api/mcp`` list endpoint so the ``installed``
    cross-ref matches what the config table shows.
    """
    # circular import: mcp_discovery defers imports of codex_crew.agent which
...
async def api_mcp_discover(request: web.Request) -> web.Response:
    ...
    # Cross-ref against CodexCrew's configured servers so the UI can badge
...
async def api_mcp_discover_install(request: web.Request) -> web.Response:
    ...
    write it into the CodexCrew scope enabled=true (reusing the mcp.py write
...
async def _install_from_official(request: web.Request, server_id: str) -> web.Response:
    """Fetch the registry entry, translate, and write into CodexCrew scope."""
    registry = await asyncio.to_thread(_get_registry)
    ...
    async with _get_mcp_lock():
        ...
        if existing is None:
            ...
            await _offload_config_write(_set_codexcrew_entry, name, enabled=enable_now, spec=spec)
        else:
    ...
    try:
        # circular import: codex_crew.agent imports dashboard handlers.
...
async def _install_via_capability(request: web.Request, server_id: str) -> web.Response:
    ...
    async with _get_config_lock():
        ...
        # for a full codexcrew.json RMW; a direct call would freeze the gateway if
