"""Reclaim the dashboard bind port from a *stale* CodexCrew gateway.

Background
----------
The gateway binds its dashboard port once at startup (:func:`_start_site` in
:mod:`codex_crew.dashboard.server`). If the port is already taken it retries for
...
dump-then-exit from :mod:`codex_crew.dashboard.loop_watchdog`, or a ``kill -9``)
...
* the holder is a *CodexCrew gateway* process — reusing the same structural
  command-line check that gates ``codexcrew stop`` (:func:`_is_codexcrew_process`)
...
* the holder does **not** answer an HTTP liveness probe — a *responsive*
  ...
  ``CODEXCREW_HOME`` sharing the port) and is never killed.

Complements — does not replace — :class:`codex_crew.gateway_lock.GatewayLock`,
...
:class:`codex_crew.dashboard.loop_watchdog.LoopStallWatchdog`.
"""
...
logger = logging.getLogger("codex_crew.dashboard.port_reclaim")
...
FOREIGN_HOLDER = "foreign_holder"  # held by a non-CodexCrew process — never touched
...
async def _terminate_pids(
    ...
    *,
    ...
    silently looping).
    """
    from codex_crew.cli_server import _pid_exited
    ...
    def _signal(pid: int, sig: int) -> bool:
        if platform_compat.IS_WINDOWS:
            ...
            # Windows branch of `codexcrew stop`.
            try:
...
async def reclaim_stale_gateway_port(
    ...
    *,
    ...
    is_codexcrew: Callable[[int], bool] | None = None,
    ...
    """Reclaim *port* iff it is held by a stale/unresponsive CodexCrew gateway.
    ...
    # as `codexcrew stop`). Bound to an explicit non-Optional local so the call
    # sites below type-check cleanly.
    checker: Callable[[int], bool]
    if is_codexcrew is not None:
        checker = is_codexcrew
    else:
        try:
    ...
    # ``identity_timeout``: the shared ``_is_codexcrew_process`` shells out to
    ...
    if not kiro_pids:
        log.error(
            "Port %d is held by a non-CodexCrew process (pid %s) — refusing to "
    ...
    if await probe_healthy(port, probe_timeout):
        log.error(
            "Port %d is owned by a live, responsive CodexCrew gateway (pid %s) — "
            "not reclaiming. Stop it with `codexcrew stop` or use a different port.",
            port,
            kiro_pids,
        )
        return HEALTHY_PEER
    ...
    log.warning(
        "Port %d is held by an unresponsive CodexCrew gateway (pid %s) left by an "
