...
the counter (``revoke_all_sessions()``, i.e. ``codexcrew logout``) therefore
...
``codexcrew`` subcommand). Both validators read the LIVE value through
...
def _warn_unreadable_counter(path: Path, reason: str, *, exc_info: bool = False) -> None:
    ...
    logger.warning(  # nosemgrep: python-logger-credential-disclosure
        ...
        "(this re-enables unexpired sessions revoked by codexcrew logout)",
        path,
        reason,
        path,
        exc_info=exc_info,
