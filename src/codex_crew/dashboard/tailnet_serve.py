...
The write half of tailnet access. Until this module existed, Codex Crew never ran
...
Deliberately a **separate module from** :mod:`codex_crew.dashboard.tailnet`, whose
...
from codex_crew.dashboard.tailnet import _cli_path, is_governance_pinned_off
from codex_crew.sandbox import scrub_env
...
def publish(port: int, *, audit_tool: str = "tailnet_publish") -> ServeResult:
    ...
    if rc == -2:
        ...
        detail = (
            ...
            "check `codexcrew tailnet status` before retrying."
        )
        if said:
            detail += f" Before the deadline it printed: {said}"
