...
* **mirrored WebSocket frames** — everything a turn says about itself that is not
  ...
  ``?relay=1``. See :mod:`codex_crew.dashboard.remote_mirror`. These are
...
from codex_crew.dashboard.chat_utils import _redact_deep
from codex_crew.dashboard.remote_mirror import MIRROR_CLS_PREFIX
...
async def ensure_version_parity(mgr: Any, instance_id: str) -> None:
    ...
    local = codex_crew.__version__
    if not ok:
        if value == "capability_peer_too_old":
            raise RemoteTurnError(
                f"This crew is running an older Codex Crew than this machine "
                f"({local}) and cannot report its version. Update it to {local} "
                f"to run sessions on it."
            )
        raise RemoteTurnError(
            "Could not confirm this crew's Codex Crew version, so the session was "
    ...
    if mismatch:
        ...
        raise RemoteTurnError(
            f"This crew runs Codex Crew {shown} but this machine runs {local}. "
