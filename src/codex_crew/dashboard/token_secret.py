...
``$CODEXCREW_HOME``. The CLI imports token_auth (and thus this module)
transitively for every ``codexcrew`` subcommand, so an import-time write
...
def _load_or_create_secret() -> bytes:
    ...
    :func:`codex_crew.atomic_write.atomic_write` helper -- owner-only before the
...
def _get_secret() -> bytes:
    ...
    does not write ``token_signing.key`` into ``$CODEXCREW_HOME`` — see the
