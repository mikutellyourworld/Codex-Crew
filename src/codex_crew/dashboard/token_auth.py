...
from codex_crew.dashboard.boot_id import current_boot_id
...
from codex_crew.dashboard.revocation_gen import (  # noqa: F401  # re-exports
    _REVOCATION_FILE,
...
from codex_crew.dashboard.tailnet import (
    ForwardedPeer,
    TailnetTrust,
...
# token_signing.key into $CODEXCREW_HOME (the CLI imports token_auth for every
# codexcrew subcommand; an import-time write would break gateway --seed and
# pollute the home for read-only commands).
from codex_crew.dashboard.token_secret import (  # noqa: F401  # re-exports
    _SECRET_KEY_FILE,
    _get_secret,
    _load_or_create_secret,
)
from codex_crew.executors import subprocess_executor
from codex_crew.mcp_gateway.socketsec import PeerCredResult, check_peer_is_self, get_peer_pid
...
class TokenStateManager:
    ...
    and sync contexts (CLI commands like `codexcrew token`). The lock hold time
...
_BYPASS_EXACT = {
    ...
    "/api/token/local",
    "/api/shutdown",
    # `codexcrew logout` (CLI) authenticates with loopback + the local secret via
    ...
    # without this entry every `codexcrew logout` is denied 403 by the middleware
...
# (handle_app_api_proxy in codex_crew/apps/routes.py) and continues to
...
_403_HTML = (
    ...
    "<p>No other signed-in device? Run <code>codexcrew token</code> in your terminal, then paste the URL below.</p>"
...
def validate_token(token: str, *, use_session_exp: bool = False) -> tuple[bool, str, str]:
    ...
    # Revocation generation: an explicit revoke_all_sessions() (e.g. codexcrew
...
def token_embed_parent_port(token: str) -> int | None:
    ...
    parent dashboard's port (the embedding desktop app's ``CODEXCREW_PORT``) is
...
def validate_app_secret(app_name: str, provided_secret: str) -> bool:
    ...
    Reads ``~/.codex-crew/apps/{app_name}/.app_secret`` and performs
...
def write_app_secret(app_name: str, secret: str) -> None:
    """Write an app secret to ``~/.codex-crew/apps/{app_name}/.app_secret``.
...
# ``codex_crew.apps.routes.setup_routes`` (the ``add_get``/``add_post`` calls for
...
def _unix_request_socket(request: web.Request) -> Any:
    ...
    :func:`~codex_crew.dashboard.origin.request_is_unix_socket` discriminator
...
def derive_caller_app(
    ...
    **Why this exists.** App-ownership checks gate on ``request["app"]``, which
    ...
    that transport, and an app agent granted ``@codexcrew-dashboard`` arrived
...
def token_auth_middleware(
    ...
    *internal_paths* are exact paths that internal processes (mcp-core,
    ...
    ``X-Internal-Secret`` header (read from ``~/.codex-crew/.local_secret``).
