...
from codex_crew import mcp_apps_render, model_registry, session_directive
...
from codex_crew.acp_backends import ACP_BACKENDS_COMPACT
from codex_crew.agent_discovery import warm_project_agent_names
from codex_crew.agent_sdk.provider_identity import is_claude_code
from codex_crew.autonudge import get_instance
from codex_crew.autonudge_authz import normalize_banner
...
from codex_crew.connections import get_visible_providers
from codex_crew.constants import strip_control_comments
from codex_crew.context_blocks import (
    PHASE_PER_TURN,
    PHASE_SESSION_START,
    USER_LABEL,
...
from codex_crew.dashboard import directive_queue
from codex_crew.dashboard.chat_delivery import (
    STEER_STATE_CONSUMED,
    STEER_STATE_REQUEUED,
    find_written_steer_row,
)
from codex_crew.dashboard.chat_persistence import _build_history_prefix, save_slot_off_loop
from codex_crew.dashboard.chat_summary import generate_session_summary
from codex_crew.dashboard.chat_title import (
...
from codex_crew.dashboard.handlers import (
    MAX_PROMPT_BYTES,
    _find_prompt,
    _get_skills,
    _list_aim_prompts,
...
from codex_crew.dashboard.session_directive_apply import apply_session_directive
...
from codex_crew.dashboard.steer_settle import settle_consumed_steers
from codex_crew.dashboard.turn_dispatch import (
    format_approval_no_budget_card,
    format_approval_timeout_card,
    spawn_guarded_turn,
    tool_approval_timeout_secs,
)
from codex_crew.deny_guidance import (
    DENY_CLASS_AWS_CREDENTIAL,
    DENY_CLASS_SSO_CREDENTIAL,
    classify_deny,
    resolve_credential_tool_hint,
)
from codex_crew.executors import run_in_embed_pool, subprocess_executor
...
from codex_crew.image_artifacts import register_images_off_loop
...
from codex_crew.mcp_discovery import codexcrew_managed_names
from codex_crew.members import member_lifecycle, record_activity
from codex_crew.messaging.display_safety import redact_for_display
from codex_crew.messaging.identity import publish_turn_identity
...
from codex_crew.messaging.renderer import chunk_for_transport
from codex_crew.metrics.events import TURN_TIMEOUT_CAUSE, emit_counter
from codex_crew.metrics.provider import get_recorder
from codex_crew.metrics.turns import emit_turn_duration, emit_turn_usage, turn_outcome
...
from codex_crew.name_grant import (
    Refusal,
    log_decline,
    pin_human_approval,
...
from codex_crew.providers.acp import is_claude_backend
...
from codex_crew.quick_prompts import QUICK_PROMPTS
...
from codex_crew.session import SessionClosingError, SpeculativeResumeRefused
from codex_crew.slack.handler import post_linked_approval, resolve_linked_approval
from codex_crew.slack.outbound import PostedOptions
from codex_crew.trust_patterns import (  # noqa: F401 -- compatibility re-export
    _mask_quoted_separators,
    approval_command,
    approval_display_command,
)
from codex_crew.trust_patterns import extract_base_command as _extract_base_command
from codex_crew.trust_patterns import extract_bash_command as _extract_bash_command
from codex_crew.trust_patterns import extract_full_command as _extract_full_command
from codex_crew.trust_patterns import matches_trusted_pattern as _matches_trusted_pattern
from codex_crew.trust_patterns import (  # noqa: F401 -- compatibility re-export
    split_command_segments as _split_command_segments,
)
from codex_crew.validation import ValidationError, validate_ask_user_question
from codex_crew.widget_artifacts import register_widgets_off_loop
...
from codex_crew.dashboard.chat_utils import (  # noqa: E402
...
def _empty_auto_continue_enabled() -> bool:
    ...
    try:
        return bool(CodexCrewConfig.load().session.empty_response_auto_continue)
...
def _turn_outcome(stop_reason: str | None, *, exhausted: bool = False) -> str:
    ...
    Thin delegate to :func:`codex_crew.metrics.turns.turn_outcome`, which is the
...
def _emit_turn_metric(
    ...
    *,
    ...
    Thin delegate to :func:`codex_crew.metrics.turns.emit_turn_duration` and
    :func:`~codex_crew.metrics.turns.emit_turn_usage`. The family is emitted for
    EVERY dispatch surface, and by two owners that between them sample each turn
    exactly once — see :mod:`codex_crew.metrics.turns`.
...
def _emit_recovery_outcome(mechanism: str, outcome: str, attempts: int) -> None:
    """Emit codexcrew.watchdog.recovery.outcome (best-effort).
    ...
    try:
        get_recorder().counter(
            "codexcrew.watchdog.recovery.outcome",
            attrs={
                "mechanism": mechanism,
                "outcome": outcome,
...
def _default_session_model(
    cfg: "CodexCrewConfig | None", slot: "_ChatSlot", agent_model: str
) -> str:
    ...
    way (``codexcrew config set``, a hand edit) was therefore shown by the chip
...
def _connections_managed_mcp_names() -> frozenset[str]:
    ...
    * :func:`codexcrew_managed_names` -- our own MCP store wrote the entry. This is
    ...
    * :func:`get_visible_providers` -- the name is a Connections provider with a
    ...
    """
    try:
        managed = codexcrew_managed_names()
...
#: :mod:`codex_crew.name_grant` so every surface that honours a name-based grant
...
async def _name_grant_refusal_for(event: object) -> Refusal | None:
    ...
    A thin wrapper over :func:`codex_crew.name_grant.refusal_for_event` rather
    ...
    :func:`codex_crew.name_grant.refusal_for_command_off_loop` (the chokepoint
...
def _audit_name_grant_refusal(
    *, session_key: str, slot: Any, event: Any, refusal: Refusal, tier: str
    ...
    A thin wrapper over :func:`codex_crew.name_grant.log_decline`, which owns
...
def _resolve_channel_target(
    ...
    *principal* lets a caller that has ALREADY established the recipient
    ...
    try:
        ...
        from codex_crew.platform.governance_profiles import vet_and_audit
...
def _strip_yaml_frontmatter(content: str) -> str:
    ...
    ``codex_crew.frontmatter`` (same stance as ``SkillsLoader.strip_frontmatter``).
...
async def _recover_app_agent_binding(
    cfg: "CodexCrewConfig", slot: "_ChatSlot", *, project: str | None
) -> Any:
    ...
    from codex_crew.apps.bridges import register_app
    from codex_crew.apps.manager import app_lifecycle_lock, is_app_enabled
...
async def _eager_spawn(
    ...
    try:
        ...
        async with _eager_spawn_sem:
            ...
            loaded_cfg: CodexCrewConfig | None = None
            resolved_ok = False
            try:
                cfg = CodexCrewConfig.load()
                loaded_cfg = cfg
...
async def _handle_goal_command(state: "DashboardState", slot: "_ChatSlot", message: str) -> None:
    ...
    if _goal_svc is None:
        body = (
            ...
            "Set `CODEXCREW_AUTONUDGE=1` and restart the gateway."
        )
...
def _requeue_unconsumed_steers(state: "DashboardState", slot: "_ChatSlot") -> None:
    ...
    from codex_crew.dashboard.session_control import containment_meta
...
def _arm_queued_delivery_settlement(
    ...
    sub-agent completion (``CodexCrewGateway._defer_queued_delivery``), keyed on the
...
def _drop_stale_admissions(state: DashboardState, slot: _ChatSlot) -> None:
    ...
    if not slot._queue:
        return
    # circular import: session_control imports this package's modules at module level.
    from codex_crew.dashboard import session_control as _sc
...
async def _start_next_queued_turn(state: DashboardState, slot: _ChatSlot) -> bool:
    ...
    try:
        merge = CodexCrewConfig.load().dashboard.merge_queued_messages
    except Exception:
        logger.warning(
    ...
    # circular import: session_control imports this package's modules at module level.
    from codex_crew.dashboard.session_control import (
...
def _emit_ttft_metric(t0: float, session_key: str, *, is_new: bool, resumed: bool) -> None:
    ...
    try:
        ...
        # ``codex_crew.metrics.provider.get_recorder`` reach this emit.
        from codex_crew.metrics.provider import get_recorder

        get_recorder().histogram(
            "codexcrew.chat.first_token.duration",
...
async def _run_chat(
    ...
    *,
    ...
    _is_cc_provider = is_claude_code(CodexCrewConfig.load().agent.provider)
    ...
    if first_word == "/prompts":
        ...
        if not prompts:
            slot.append(
                "assistant",
                "No prompts found. Create prompts in `~/.kiro/prompts/` (or `~/.codex-crew/prompts/`).",
                "msg msg-a",
            )
            sel().log_tool_invocation(
                session_key="",
    ...
    if first_word == "/compact":
        ...
        if _live_provider is not None:
            ...
            _cfg_backend = getattr(CodexCrewConfig.load().agent, "acp_backend", "")
    ...
    try:
        ...
        # agent name (e.g. "codexcrew") instead of the CodexCrew slot name
        # (e.g. "default") which has no matching ~/.kiro/agents/ config.
        kiro_agent: str | None = None
        memory_store: str | None = None
        # The CodexCrew agent's own default model ("" = inherit). Ranks below the
        ...
        if is_slash:
            ...
            # Provider-agnostic session replay: CodexCrew's conversation_log
            ...
            if is_new and not _provider_has_history and state.context_builder.conversation_log:
                ...
                if state.sessions.consume_replay_suppression(session_key):
                    ...
                    from codex_crew.context import (  # circular: context -> chat
            ...
            if _session is not None and getattr(_session, "prev_turn_cancelled", False):
                ...
                if state.context_builder and state.context_builder.conversation_log:
                    from codex_crew.context import (
            ...
            # Use resolved kiro agent name (e.g. "codexcrew"), not the slot
            # name (e.g. "default"), so build_message's is_custom check
            # correctly identifies codexcrew sessions and enables skills.
            ...
            # default behavior).
            from codex_crew.context import window_for_provider_client  # circular: context -> chat
        ...
        async for event in event_stream:
            ...
            if event.kind == EVENT_TEXT_CHUNK:
                ...
                if event.tool_call_id:
                    ...
                    # TurnDriver) requires Codex Crew's OWN core MCP server and a
                ...
                # this turn, then record the applier's real outcome on CodexCrew's
                ...
                if (slot_trusted or yolo_active) and _child_grant_eligible:
                    ...
                    if not _pre_tool_hooks_fired:
                        ...
                        if _pre_tool_hooks_should_block(pre_hook_results):
                            ...
                            continue
                    # always=False — CodexCrew owns trust scope; per-call request_permission
                ...
                # `codex_crew.agent_sdk` — the boundary's sanctioned surface, and an
                ...
                if usage_has_billing(_u):
                    ...
                    await persist_token_record_async(
                        ...
                        # maps e.g. "default" to "codexcrew" before dispatch, so the
                ...
                # codexcrew.turn.duration → turn latency p50/p90 + fault rate.
        ...
        # ``assistant_text``; run_script_hook caps ONLY the CODEXCREW_HOOK_CONTEXT
        ...
        if should_queue_hook_continuation(slot._stopping, needs_session_reset, _stop_reason) and (
            ...
            if _hook_reasons:
                _nudge_cap = (
                    await asyncio.to_thread(CodexCrewConfig.load)
        ...
        if assistant_text and state.slack_client and _mirror_thread and _mirror_chan:
            try:
                from codex_crew.slack.format import (  # circular: slack.format -> dashboard.state -> chat
        ...
        # codexcrew.turn.timeout.cause for the ceiling path.
