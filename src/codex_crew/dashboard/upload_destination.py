...
(:func:`~codex_crew.dashboard.handlers.files._gate_upload_file`: containment, the
...
:func:`codex_crew.messaging.upload_gate.uploads_restricted` uses for its probe.
...
from codex_crew.messaging.link import SLACK_NAMESPACE, is_channel_session_key
from codex_crew.platform.context import PlatformCompositionError
from codex_crew.platform.governance_profiles import HOST_SESSION_KEY, vet_and_audit
from codex_crew.validation import FILE_SEND_SCHEMA, ValidationError, validate_tool_args
...
async def resolve_slack(
    ...
    *,
    ...
    if target_channel:
        ...
        try:
            creds = CodexCrewConfig.load().load_credentials()
            owner_id = creds.get("CODEXCREW_OWNER_ID", "")
            if owner_id:
...
async def resolve_channel(
    ...
    *,
    ...
    from codex_crew.dashboard.chat_runner import _resolve_mirror_target
