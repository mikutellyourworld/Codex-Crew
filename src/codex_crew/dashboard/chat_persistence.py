...
from codex_crew.agent_discovery import agent_model_map
...
from codex_crew.dashboard.channel_slots import slot_closed_since
...
from codex_crew.validation import ARTIFACT_SLUG_RE
...
def _load_restore_cfg() -> "CodexCrewConfig | None":
    ...
    ``CodexCrewConfig.load()`` reads and parses ``config.json`` from disk, which
...
def restore_open_slots(state: DashboardState) -> int:
    ...
    Path resolves through ``config_dir()`` (honors ``CODEXCREW_HOME``) so
...
def _member_restore_identity(slot_name: str) -> tuple[str, str] | None:
    ...
    """
    # Function-local ON PURPOSE: codex_crew.members imports codex_crew.artifacts
...
def _apply_recent_session(
    ...
    *,
    ...
    restore_cfg: "CodexCrewConfig | None",
    member_identity: tuple[str, str] | None = _IDENTITY_UNRESOLVED,
) -> None:
...
def _save_slot_to_history(
    ...
    *,
    ...
    :data:`~codex_crew.history.ROWS_ONLY_OWNED_META_KEYS`. It exists for the one
    ...
    :data:`~codex_crew.history.ROWS_ONLY_DEFERRED_META_KEYS`, which is wider than the
...
async def save_slot_off_loop(
    ...
    *,
    ...
    non-blocking acquire and raises :class:`~codex_crew.history.HistoryLockTimeout`
    under any concurrent holder (e.g. a workflow/cron result appending via
    :func:`~codex_crew.history.append_off_loop`) — so calling the save inline on
    ...
    This helper mirrors :func:`~codex_crew.history.append_off_loop`: on a running
