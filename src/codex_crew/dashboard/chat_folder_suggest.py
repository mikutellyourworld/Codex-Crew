...
# cheap one: the ``codexcrew-lite`` spec's model is ``_background_agent_model()``,
...
def _folder_sample_titles(state: DashboardState) -> dict[str, list[str]]:
    ...
    for a model to file "Fix the render gate flake" under "Codex Crew › i18n".
...
def _build_prompt(
    *,
    ...
    ``labels`` are breadcrumbs (``Codex Crew › feature``) so a nested folder is
