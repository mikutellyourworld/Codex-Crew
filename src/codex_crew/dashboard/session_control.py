...
from codex_crew.dashboard.chat_delivery import sanitize_outbound
from codex_crew.dashboard.chat_folders import _unhide_folder
from codex_crew.dashboard.chat_persistence import _TRANSIENT_ROLES as _PERSISTENCE_TRANSIENT_ROLES
from codex_crew.dashboard.chat_utils import effective_session_key, slot_history_key
from codex_crew.dashboard.create_rate_limit import SESSION_CREATE, allow_create
...
from codex_crew.dashboard.stop_retry import allow_escalation
from codex_crew.history import metadata_now_iso, transcript_stem
from codex_crew.security import redact, redact_and_truncate
from codex_crew.sel import sel
from codex_crew.validation import MAX_LONG_STRING
...
def _member_caller(caller_key: str) -> bool:
    ...
    """
    from codex_crew.members import DM_SLOT_KEY_PREFIX
...
def session_control_enabled() -> bool:
    ...
    """
    try:
        return bool(CodexCrewConfig.load().agent.session_control)
    except Exception:
        logger.warning(
...
async def prewarm_enabled_check() -> None:
    ...
    ``CodexCrewConfig.load()`` re-reads and validates the file on the FIRST call
...
async def create_session(
    ...
    *,
    ...
    # `CodexCrewConfig.load()`, which is filesystem work that must not run on the
...
async def close_target(
    ...
    *,
    ...
    from codex_crew.dashboard.chat_handlers import SlotCloseError, close_slot
