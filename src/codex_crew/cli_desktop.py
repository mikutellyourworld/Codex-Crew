"""``codexcrew desktop`` -- read the Electron app's debug metrics artifact.
...
running with ``CODEXCREW_DEBUG`` set, there is no artifact at all.

Debug-only, and gated by the same ``CODEXCREW_DEBUG`` check as ``codexcrew perf``.
"""
...
from codex_crew.perf_sampler import gate_refusal_message, profiling_enabled
...
# `-c.productName=CodexCrew Nightly` for nightly stamps. Nightly is not a cosmetic
...
PRODUCT_NAMES = ("CodexCrew", "CodexCrew Nightly")
...
def artifact_missing_message() -> str:
    ...
    return (
        ...
        f"CODEXCREW_DEBUG set, so the usual cause is that the running app was "
        "launched without it -- restart the app with CODEXCREW_DEBUG=1 and let it "
...
def _desktop_metrics(args: argparse.Namespace) -> int:
    ...
    if version != SUPPORTED_VERSION:
        ...
        print(
            ...
            f"version {SUPPORTED_VERSION}. Update codexcrew to read it.",
            file=sys.stderr,
        )
        return 5
...
def desktop_cmd(args: argparse.Namespace) -> int:
    """Dispatch a ``codexcrew desktop`` subcommand."""
...
def register_desktop_parser(sub: argparse._SubParsersAction) -> None:
    """Register ``codexcrew desktop`` and its subcommands."""
