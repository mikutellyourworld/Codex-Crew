...
:mod:`codex_crew.mcp_gateway.socketsec`). Walking that PID's */proc* ancestry
...
from codex_crew.config.loader import config_dir as _default_config_dir
from codex_crew.mcp_caller import _parent_pid as _default_ppid
from codex_crew.session_pid_sig import read_session_pid_txt, verify_session_pid
...
def resolve_peer_identity(
    ...
    * the full HOST ancestor PID chain (peer first). gatewayd indexes stub
    ...
    implementations. Reads go through :func:`~codex_crew.session_pid_sig.
    ...
    verify (:func:`~codex_crew.session_pid_sig.verify_session_pid`, pid bound
