...
the same in-session model the rest of CodexCrew uses (no new model plumbing). It
...
from codex_crew.acp.runtime import AcpRequestTimeout, AcpRuntimeDead
from codex_crew.llm_helpers import (
    ToolApprovalPolicy,
    acp_error_is_transient,
    stream_and_collect,
)
from codex_crew.task_planner import decompose_yaml
...
_AUTHOR_SYSTEM = """\
You are authoring a CodexCrew DYNAMIC WORKFLOW: one Python module that orchestrates
...
class WorkflowService:
    ...
    async def author(
        ...
        *,
        ...
        # uses the tool-less ``codexcrew-lite`` agent. That is the lever that makes a
        ...
        for startup_attempt in range(1, _AUTHOR_STARTUP_ATTEMPTS + 1):
            ...
            try:
                provider, *_ = await self._sessions.get_or_create(key, agent="codexcrew-lite")
