...
from codex_crew.metrics.events import WORKFLOW_RUNS, emit_counter
...
try:
    from codex_crew.security import redact_credentials as _redact_credentials
    from codex_crew.security import redact_exfiltration_urls as _redact_exfil
...
# ``codex_crew.sel`` security event log; tests inject a capturing stub. Kept as a
...
class _RunContext:
    ...
    The ports native to Codex Crew (cron/memory/learn/knowledge) are None unless the host
    ...
    def __init__(
        ...
        *,
        ...
        # Ports native to Codex Crew — injected per run; None when the host did
    ...
    # --- ports native to Codex Crew: delegate to injected port fns; clear error if a
