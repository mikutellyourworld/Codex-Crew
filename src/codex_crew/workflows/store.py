...
try:
    ...
    CodexCrewConfig = None  # type: ignore[assignment,misc]
...
def default_workflows_dir() -> Path:
    ...
    ``<config_dir>/workflows`` (e.g. ``~/.codex-crew/workflows``, or the
    ``CODEXCREW_HOME`` override used by the dev instance).
    """
    # CodexCrewConfig is an optional dependency resolved at module top (gate F1);
    # when present, honor workflows.dir, else fall back to config_dir()/workflows.
    if CodexCrewConfig is not None:
        try:
