...
Lessons are saved via the ``codexcrew learn`` CLI (called by the LLM via bash)
...
from codex_crew.project_scope import canonical_scope, project_scope_satisfied
...
# ``CODEXCREW_HOME`` only insofar as this fallback is rarely reached — the normal
...
class Lesson:
    ...
    # ever withheld. See ``codex_crew.project_scope``.
...
class LessonStore:
    ...
    def save(self, lesson: Lesson) -> None:
        ...
        explicit refinement (the /api/lessons route, ``codexcrew learn add``)
