...
from codex_crew import platform_compat, slack_manifest
...
from codex_crew.conductor_skill import generate_conductor_skill
...
from codex_crew.constants import DATA_WARNING, MIN_NODE_MAJOR
from codex_crew.sandbox import unavailable_kind
...
def _get_alias() -> str:
    ...
    if not alias:
        print(
            ...
            "`codexcrew manifest --alias <alias>`.",
            file=sys.stderr,
        )
        sys.exit(1)
    return alias
...
def _manifest(alias: str | None = None, output: str | None = None, url: bool = False) -> None:
    ...
    if url:
        ...
        print(f"✅ Manifest written to {output} (name: CodexCrew-{alias})")
    else:
        print(rendered)


def _fix_shell_profiles() -> None:
    """Remove stale CodexCrew PATH entries from shell profiles."""
    home = Path.home()
    profiles = [
        home / ".zshrc",
        home / ".bashrc",
    ...
    for profile in profiles:
        ...
        try:
            ...
            if removed:
                ...
                print(f"  🔧 Cleaned stale Codex Crew PATH from {profile.name}")
...
def _find_electron_dir() -> Path | None:
    ...
    only resolves from a source checkout: first via CODEXCREW_PROJECT_DIR, then
...
def _setup_electron() -> None:
    """Build and install the CodexCrew desktop app (macOS only)."""
    if platform.system() != "Darwin":
        print("  ⚠️  Codex Crew desktop app is only available on macOS.")
        return

    if not shutil.which("node"):
        print("  ❌ Node.js not found — required to build the desktop app.")
        print("     Install Node.js and re-run: codexcrew setup --electron-only")
        return
    ...
    print("  🔨 Building Codex Crew desktop app…")
    ...
    app_src = electron_dir / "dist" / arch / "CodexCrew.app"
    if not app_src.is_dir():
        for candidate in ("mac-arm64", "mac", "mac-x64"):
            app_src = electron_dir / "dist" / candidate / "CodexCrew.app"
            if app_src.is_dir():
                break
    if not app_src.is_dir():
        print("  ❌ Build succeeded but CodexCrew.app not found in dist/")
        return

    app_dest = Path.home() / "Applications" / "CodexCrew.app"
    app_dest.parent.mkdir(parents=True, exist_ok=True)
    if app_dest.is_dir():
        shutil.rmtree(app_dest)
    shutil.copytree(str(app_src), str(app_dest))
    print("  ✅ CodexCrew.app installed to ~/Applications")
    print("     Launch via Spotlight (⌘+Space → CodexCrew) or Finder → ~/Applications")
...
def _setup_impl(
    ...
    from codex_crew.agent import install_agent  # circular import: agent imports cli
    from codex_crew.cli import _project_dir_file  # circular import: cli -> cli_setup -> cli
    ...
    # 2a. Ensure `codexcrew` is reachable on PATH (the packaged Electron app
    ...
    from codex_crew.agent import ensure_codexcrew_on_path
    from codex_crew.mcp_cleanup import clean_stale_managed_mcp

    # `claim_existing`: this is the explicit setup path, so the user has named
    # THIS install as the one they want `codexcrew` to mean. Gateway startup
    # deliberately does not, so a background start never takes the command away
    # from a working install (see ensure_codexcrew_on_path).
    shim = ensure_codexcrew_on_path(claim_existing=True)
    if shim:
        print(f"  ✅ Linked codexcrew on PATH: {shim}")
    removed_mcp = clean_stale_managed_mcp()
    if removed_mcp:
        print(f"  ✅ Removed stale MCP entries: {', '.join(removed_mcp)}")

    # 2b. Ensure config.json has default CodexCrew agent for fresh installs
    ...
    if agent_only:
        ...
        for flag in [name for name, on in requested if on]:
            print(
                ...
                f"'codexcrew setup {flag}' for its guided setup."
            )
        print("\n>_ Done! Try: codexcrew gateway")
        return

    # 3. Messaging channels (optional, configured after setup by default).
    #    Channel prompts run only on explicit opt-in (`codexcrew setup --slack`,
    #    `codexcrew setup --whatsapp`); the dashboard and CLI need no channel
    ...
    if slack or whatsapp:
        ...
        print("  'codexcrew setup --slack' or 'codexcrew setup --whatsapp' for a")
        print("  guided setup.\n")
    ...
    if platform.system() == "Darwin":
        print("── Desktop App ──\n")
        answer = input("  Install CodexCrew desktop app to ~/Applications? [Y/n]: ").strip().lower()
        if answer in ("", "y", "yes"):
            _setup_electron()
        else:
            print("  ⏭  Skipped. Install later: codexcrew setup --electron-only\n")

    # 7. Cloud (run CodexCrew on the user's own AWS EC2) — optional, delegated.
    _maybe_setup_cloud()

    print("\n>_ Done! Try: codexcrew doctor && codexcrew gateway")


def _maybe_setup_cloud() -> None:
    """Offer to launch CodexCrew on the user's own AWS EC2.

    A thin delegating step — all AWS/CloudFormation/SSM logic lives in the
    testable ``codex_crew.cloud`` module. This just asks and hands off to the
    launcher wizard (``codexcrew cloud launch``).
    """
    print("\n── Run on AWS (optional) ──\n")
    print("  Codex Crew can run 24/7 on your own AWS EC2 instance (bring your own")
    print("  AWS account; credentials stay in the aws CLI — never stored here).")
    try:
        answer = input("  Launch CodexCrew on AWS now? [y/N]: ").strip().lower()
    ...
    if answer not in ("y", "yes"):
        print("  ⏭  Skipped. Launch later: codexcrew cloud launch\n")
        return
    try:
        import argparse

        from codex_crew.cli_cloud import handle_cloud
        ...
        print("  Run it directly: codexcrew cloud launch\n")
...
def _setup_workspace_dir() -> None:
    ...
    # made `codexcrew setup < /dev/null` fail before any later guard could help.
...
def _setup_slack_tokens() -> None:
    ...
    # `codexcrew secrets import` migrator, the WeChat/Weixin QR handler, and the
...
def _setup_whatsapp() -> None:
    ...
    # ``codexcrew config set`` landing between the two is carried forward instead
...
class _SetupAborted(Exception):
    ...
    ``codexcrew setup < /dev/null`` — or a Windows console quirk that closes
...
def _setup_slash_command() -> None:
    ...
    if slack_section is not None and not isinstance(slack_section, dict):
        ...
        return
    current = (slack_section or {}).get("command", "codexcrew")
...
def _setup_sandbox_consent() -> None:
    ...
    sandbox works and the remedy hands isolation back to Codex Crew rather than
    ...
    if kind != "no_backend":
        ...
        # Codex Crew rather than disabling it. Neither warrants this opt-in.
        return
    # A prompt nobody can see is a hang, not consent: `codexcrew update` runs
    ...
    if not (sys.stdin.isatty() and sys.stdout.isatty()):
        ...
        print("     refused. Run `codexcrew setup` from a terminal to decide, or set")
    ...
    print("  or macOS sandbox-exec), so Codex Crew currently REFUSES to run agent")
    ...
    print("  confinement. Codex Crew still scrubs credential environment variables,")
