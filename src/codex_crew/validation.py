...
# nothing from ``codex_crew`` and never touches ctypes), so there is no cycle and
...
from codex_crew.computer_use import types as _cu_types
...
from codex_crew.effort import EFFORT_VALUES
...
from codex_crew.project_scope import SCOPE_FRAGMENT_RE
...
# Artifact slug grammar — mirrors codex_crew.artifacts._SLUG_RE (kept here so
...
def infer_use_case(session_key: str) -> str:
    """Map a CodexCrew session_key to a categorical useCase label.
...
# stdio loop, killing the whole codexcrew-core MCP server for the session.
...
# Artifact tools — slug pattern matches codex_crew.artifacts._SLUG_RE.
...
from codex_crew.artifacts import MAX_CONTENT_BYTES as ARTIFACT_CONTENT_MAX  # noqa: E402
...
SEND_MESSAGE_SCHEMA = ToolSchema(
    ...
    fields=[
        ...
        # Importing it costs no cycle: the roster is homed in ``codex_crew.constants``,
...
MCP_CORE_SCHEMAS: dict[str, ToolSchema] = {
    ...
    # the whole codexcrew-core server for the session.
    ...
    # crash that takes the whole codexcrew-core server down for the session.
...
# ── Tool Schemas (MCP Computer Use — server ``codexcrew-computer``) ──
#
...
# ── Tool Schemas (MCP Dashboard — server ``codexcrew-dashboard``) ──
#
