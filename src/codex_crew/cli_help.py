"""Grouped top-level help for the ``codexcrew`` CLI.
...
COMMAND_GROUPS: tuple[tuple[str, tuple[tuple[str, str], ...]], ...] = (
    (
        "Start here",
        (
            ("gateway", "Start Codex Crew in this terminal (dashboard + messaging channels)"),
    ...
    (
        ...
        (
            ...
            ("update", "Update Codex Crew to the latest version"),
        ),
    ),
    (
        "Set it up",
    ...
    (
        "Extend it",
        (
            ("app", "Manage Codex Crew apps"),
            ("agent", "Manage Codex Crew agent definitions"),
    ...
    (
        ...
        (
            ("cloud", "Run Codex Crew on your own AWS EC2 instance"),
            ("tailnet", "Publish this dashboard on your tailnet (Tailscale)"),
            ("snapshot", "Create a portable backup of Codex Crew state"),
            ("restore", "Restore Codex Crew state from a snapshot"),
        ),
    ),
    (
        "Develop Codex Crew itself",
        (
            ...
            ("desktop", "Debug-only desktop app diagnostics (requires CODEXCREW_DEBUG)"),
        ),
    ),
)
...
TOP_USAGE = "codexcrew [-h] [--version] [-v] [--no-jail] <command> [<args>]"
...
gateway vs. service -- the same server, two lifetimes:
  codexcrew gateway          runs in the foreground and stops on Ctrl-C or when
                            the terminal closes. Best for a first look and for
                            development.
  codexcrew service install  registers a systemd unit (Linux, needs sudo) or a
                            launchd agent (macOS) that runs the SAME gateway
                            detached: it survives logout, restarts on crash and
                            starts at boot. Then use `codexcrew service status`,
                            `codexcrew restart`, `codexcrew logs`.
  Run only one of them at a time -- both bind the same port.

Ports: the dashboard is the only port Codex Crew opens, and it binds loopback
  ...
  with `codexcrew gateway --port N`, CODEXCREW_PORT=N, or the `dashboard.url`
  config value; for the service, set CODEXCREW_PORT when you run
  `service install` (later, edit /etc/codexcrew/codexcrew.env and restart)."""
...
def add_command(
    ...
    **kwargs: Any,
    ...
    which is what ``codexcrew <command> --help`` prints. A caller that wants a
...
def render_epilog(width: int = 13) -> str:
    ...
    lines.append("Run `codexcrew <command> -h` for a command's own options.")
    return "\n".join(lines)
...
class _VisibleCommandChoices(Mapping[str, Any]):
    ...
    while iteration is filtered, and ``codexcrew mcp-core`` keeps working.
