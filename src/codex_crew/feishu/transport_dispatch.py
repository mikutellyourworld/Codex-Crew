...
from codex_crew.feishu.client import CHAT_GROUP
...
if TYPE_CHECKING:
    ...
    from codex_crew.feishu.client import LarkClient, LarkInbound
    from codex_crew.history import ConversationLog
    from codex_crew.session import SessionManager

logger = logging.getLogger(__name__)

# Canonical kiro-cli agent fallback so Feishu sessions load codexcrew-core
# (spawn_run etc.) instead of kiro-cli's bare built-in default.  Mirrors the
# Slack / Telegram / WeCom paths' _DEFAULT_CODEXCREW_AGENT.
...
class FeishuDispatcher:
    ...
    async def handle_message(self, inbound: "LarkInbound") -> None:
        ...
        async def _surface_new_session() -> None:
            from codex_crew.dashboard.channel_slots import (  # noqa: PLC0415
        ...
        await drive_turn(
            ChannelTurn(
                ...
                # dashboard toggle and Slack's `/codexcrew yolo` drive, so it needs no
