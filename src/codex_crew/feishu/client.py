...
is not on an index, so ``pip install codexcrew[feishu]`` cannot resolve).
It is imported lazily inside this module's methods so that every other module
in the package -- including :mod:`codex_crew.feishu.transport` and the channel
roster in :mod:`codex_crew.channels` -- imports cleanly on a build that does not
