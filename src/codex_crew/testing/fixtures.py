"""Test helpers for consumers that need a seeded ``$CODEXCREW_HOME``.
...
1. ``seeded_home(fixture_name)`` — plain context manager. Use from any
   ...
   the previous ``CODEXCREW_HOME`` env var on exit.
2. ``seeded_home_fixture`` — pytest fixture wrapping (1). Parameterised
   ...
   ``codex_crew.testing.fixtures`` to their ``conftest.py`` via
...
# ``pytest`` is an OPTIONAL runtime dependency. ``codex_crew.testing`` ships in
# the runtime wheel (not ``dev_requirements``) so downstream consumers can do
# ``pip install codexcrew`` and use ``seeded_home`` from any test harness
...
from codex_crew.seed import seed
...
def seeded_home(fixture_name: str) -> Iterator[Path]:
    """Context manager yielding a populated ``$CODEXCREW_HOME`` tempdir.

    On enter: creates a fresh temp directory, points ``CODEXCREW_HOME`` at
    it, and runs ``seed(fixture_name)`` to populate the tree. On exit:
    restores the previous ``CODEXCREW_HOME`` (or unsets if it wasn't set)
    ...
    Example:
        with seeded_home("minimal") as home:
            ...
            # agent code under test reads $CODEXCREW_HOME here
    """
    previous = os.environ.get("CODEXCREW_HOME")
    tmpdir = Path(tempfile.mkdtemp(prefix="codexcrew-seeded-home-"))
    try:
        ...
        os.environ["CODEXCREW_HOME"] = str(tmpdir)
        seed(fixture_name)
        yield tmpdir
    finally:
        ...
        if previous is None:
            ...
            os.environ["CODEXCREW_HOME"] = previous
        if tmpdir.exists():
...
if pytest is not None:
    ...
    def seeded_home_fixture(request: "pytest.FixtureRequest") -> Iterator[Path]:
        ...
        Defined only when pytest is importable — ``codex_crew.testing`` is in
