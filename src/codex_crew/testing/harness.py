"""Test harness for spawning isolated CodexCrew gateways.

Companion to the ``--test-mode`` / ``--json-ready`` / ``--port`` / ``--approval``
CLI flags on ``codexcrew gateway``. Provides a context manager that spins up
an isolated, headless gateway from the current workspace's source tree (not
the system-installed ``codexcrew``), reads the ``CODEXCREW_READY:{...}`` line
...
Usage:
    ...
    from codex_crew.testing.harness import spawn_feature_gateway

    with spawn_feature_gateway() as handle:
        # drive Playwright / urllib / etc against handle.url
        ...
    # subprocess and tmp CODEXCREW_HOME are gone by here
...
from codex_crew.kiro_prerequisite import FAKE_ACP_TEST_MODE_ENV
...
# Override via ``CODEXCREW_HARNESS_READY_TIMEOUT``.
...
class GatewayHandle:
    ...
    Attributes:
        ...
        home: Path to the throwaway ``CODEXCREW_HOME`` directory the gateway
...
def _resolve_workspace_src() -> Path:
    ...
    (``<pkg>/src/codex_crew/testing/harness.py``) to find the package's
    ``src/`` directory. We deliberately avoid using the system-installed
    ``codexcrew`` for two reasons:
    ...
    # <pkg>/src/codex_crew/testing/harness.py -> <pkg>/src
    src = here.parent.parent.parent
    if not (src / "codex_crew" / "__init__.py").exists():
        raise GatewaySpawnError(
            f"Could not locate codex_crew package at {src}. "
            f"Harness expects harness.py at <pkg>/src/codex_crew/testing/, "
            f"with the source tree at <pkg>/src/codex_crew/."
        )
    return src


def parse_ready_line(line: str) -> dict[str, Any]:
    """Parse and validate a single ``CODEXCREW_READY:{...}`` line.
...
def _wait_for_ready_line(
    ...
    *,
    ...
    """Read stdout until we see ``CODEXCREW_READY:{...}`` or hit the timeout.
        ...
        while True:
            ...
            if remaining <= 0:
                ...
                raise GatewaySpawnError(
                    ...
                    f"{timeout:.1f}s. Override with CODEXCREW_HARNESS_READY_TIMEOUT.\n"
                    f"--- stderr (last) ---\n{stderr_text[-4000:]}"
                )
    ...
    *,
    ...
    Args:
        fixture: Named fixture (``empty`` / ``minimal`` / ``rich``) passed
            through to ``codexcrew gateway --seed`` so the tmp CODEXCREW_HOME
        ...
        approval: Approval mode to pass through ``--approval``. ``"reads"``
            ...
            ``CODEXCREW_HOME`` (the harness always provides one); pass
        ...
        timeout: Override the ready-line timeout in seconds. Falls back to
            ``CODEXCREW_HARNESS_READY_TIMEOUT`` env var, then to
            ``DEFAULT_READY_TIMEOUT``.

    Yields:
        ``GatewayHandle`` once the gateway has bound its dashboard port
        and emitted the ``CODEXCREW_READY:{...}`` line. The handle is only
    ...
    home = Path(tempfile.mkdtemp(prefix="codexcrew-harness-"))
    ...
    try:
        if timeout is None:
            env_timeout = os.environ.get("CODEXCREW_HARNESS_READY_TIMEOUT")
        ...
        env = {
            **os.environ,
            ...
            "CODEXCREW_HOME": str(home),
            # Isolate the AGENT-SPEC home too, not just the data home (issue #4912).
            # ``codexcrew gateway`` boot runs ``rebuild_agent_config``, which writes the
            ...
            "CODEXCREW_SKIP_MODEL_DOWNLOAD": "1",
        }
        ...
        cmd = [
            ...
            "gateway",
            "--test-mode",
            # ``--seed`` populates the (empty) tmp CODEXCREW_HOME from the named
