"""Test utilities for CodexCrew consumers.

Everything under ``codex_crew.testing`` is intended for downstream test
suites that want to exercise code against a populated ``$CODEXCREW_HOME``
without hand-rolling setup. The module is in the runtime wheel (not
``dev_requirements``) so third-party packages can ``pip install codexcrew``
and use it immediately.

Public entry points:

- :mod:`codex_crew.testing.fixtures` — ``seeded_home`` plain context manager
  and ``seeded_home_fixture`` pytest fixture for setting up an isolated
  ``$CODEXCREW_HOME`` from a named fixture.
- :mod:`codex_crew.testing.harness` — ``spawn_feature_gateway`` context
...
- :mod:`codex_crew.testing.channel_fixtures` — vendor-API fixtures carrying
...
- :mod:`codex_crew.testing.fake_channel_wire` — ``FakeWireSession`` /
...
Import submodules directly (``from codex_crew.testing.fixtures import ...``,
``from codex_crew.testing.harness import ...``); no top-level re-exports,
...
# can traverse paths like ``codex_crew.testing.harness.TERMINATE_GRACE_SECONDS``
...
from codex_crew.testing import fake_channel_wire, fixtures, harness  # noqa: E402,F401
