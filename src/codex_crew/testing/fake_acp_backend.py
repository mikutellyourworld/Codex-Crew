...
newline-delimited stdio) that ``codex_crew.acp.client.AcpClient`` drives:
...
the ``CODEXCREW_KIRO_BIN`` override (the provider stays ``acp``); it is never
selectable from a real gateway. Run standalone as
``python -m codex_crew.testing.fake_acp_backend`` (the pytest harness and the
live-test harness both point ``CODEXCREW_KIRO_BIN`` at a launcher that runs it).
