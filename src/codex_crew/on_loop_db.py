...
:class:`~codex_crew.history.OnLoopPersistError` subclasses ``AssertionError``,
...
same cleanup. ``CODEXCREW_STRICT_ON_LOOP_PERSIST`` is already exported into the
...
STRICT_ENV = "CODEXCREW_STRICT_ON_LOOP_PERSIST"
...
STORE_STRICT_ENV = "CODEXCREW_STRICT_ON_LOOP_STORE"

#: Turning strict on for a developer gateway run.
DEV_MODE_ENV = "CODEXCREW_DEV_MODE"
...
def strict_enabled(*, strict_env: str = STRICT_ENV, include_dev_mode: bool = True) -> bool:
    ...
    3. otherwise, a truthy ``CODEXCREW_DEV_MODE`` -> strict, but only when the
...
class OnLoopDBGuard:
    ...
    def __init__(
        ...
        *,
        ...
        :param dev_mode_arms_strict: whether ``CODEXCREW_DEV_MODE`` alone turns
            ...
            ``CODEXCREW_DEV_MODE`` -- which silences every OTHER surface's guard
        ...
        self._allow_on_loop: contextvars.ContextVar[bool] = contextvars.ContextVar(
            "codexcrew_allow_on_loop_db_" + "_".join(label.split()), default=False
        )
