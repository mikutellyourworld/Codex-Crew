"""Portable zip export/import for CodexCrew state (dashboard endpoint).

Creates a zip archive of all CodexCrew settings and memory for download
...
from codex_crew.mcp_cron import _log_cron_denial, _vet_shell_command
...
def _mc_dir() -> Path:
    return Path(os.environ.get("CODEXCREW_HOME", config_dir()))
...
def create_export_zip() -> tuple[bytes, dict]:
    """Create a zip archive of CodexCrew state. Returns (zip_bytes, manifest_dict)."""
    mc = _mc_dir()
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    prefix = f"codexcrew-export-{ts}"
