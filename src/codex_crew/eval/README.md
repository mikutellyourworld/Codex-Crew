# CodexCrew Eval Harness

Multi-session evaluation harness for benchmarking CodexCrew's cross-session memory, lesson application, and context accumulation.
...
codexcrew eval memory_recall_basic
codexcrew eval memory_recall_basic lesson_application
...
Scenarios are JSON files in `src/codex_crew/eval/scenarios/`. Each defines sessions with turns and assertions (YAML is also supported for backward compatibility):
...
```
codexcrew eval            CLI entry point — scenario selection, output
...
Key design: each session creates a **new provider instance**, simulating a user closing and reopening CodexCrew. Cross-session context must come from persisted memory, not conversation history.
