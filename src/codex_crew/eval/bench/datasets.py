...
def cache_dir() -> Path:
    ...
    Kept off ``CODEXCREW_HOME`` deliberately: these are hundreds of megabytes of
    reproducible third-party data, and sweeping them into the directory that
    ``codexcrew snapshot`` backs up would bloat every snapshot with bytes that a
    URL already describes.
    """
    override = os.environ.get("CODEXCREW_BENCH_CACHE")
    if override:
        return Path(override).expanduser()
    xdg = os.environ.get("XDG_CACHE_HOME")
    base = Path(xdg).expanduser() if xdg else Path.home() / ".cache"
    return base / "codexcrew" / "bench-data"
...
def ensure(spec: DatasetSpec | str, *, allow_download: bool = True) -> Path:
    ...
    # CODEXCREW_BENCH_CACHE can point this anywhere, so it gets the same treatment
    # as an argv path: a cache root of ~/.codex-crew would drop corpus files and
    ...
    if not dest.exists():
        if not allow_download:
            raise CorpusFetchError(
                ...
                f"Fetch it with: codexcrew bench fetch {spec.key}   "
