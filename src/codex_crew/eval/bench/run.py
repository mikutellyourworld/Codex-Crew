...
def _production_embedder_id() -> str:
    ...
    describe the bundled model; ``CODEXCREW_EMBED_MODEL_PATH`` (and the
    ``memory.embed_model_path`` config knob) make Codex Crew run a different one,
    ...
    """
    from codex_crew.embeddings import get_shared_embedder
...
def run_retrieval(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="codexcrew_bench_") as tmp:
...
def write_report(
    ...
    prose. Mirrors what ``codexcrew eval`` already does.
    """
