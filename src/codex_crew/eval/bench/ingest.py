...
measurement of Codex Crew. Everything here is deliberate about one thing: the store
...
def prepare_embedder(*, timeout_s: float = DEFAULT_EMBED_TIMEOUT_S) -> EmbedFn:
    ...
    real call.
    """
    from codex_crew.embeddings import get_shared_embedder, make_sync_embed_fn
    ...
    if not probe:
        raise IngestError(
            ...
            "model file is present (codexcrew doctor) before benchmarking."
        )
    return embed_fn
...
def ingest_instance(
    ...
    *,
    ...
    # and gates every write on it, so a custom model -- `CODEXCREW_EMBED_MODEL_PATH`
