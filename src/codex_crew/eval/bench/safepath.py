...
``--out-dir``'s ``mkdir``, and the corpus cache root, which ``CODEXCREW_BENCH_CACHE``
...
human who owns the machine: an agent can run any CLI command. So
    ...
    codexcrew bench retrieval --out-dir ~/.codex-crew --stem security_policy
...
def guard_output_dir(path: str | Path, *, what: str) -> Path:
    ...
    """
    from codex_crew.security import is_sensitive_path, path_contains_sensitive
...
def _supports_pinned_walk() -> bool:
    ...
    Delegates to :func:`codex_crew.pinned_fs.supports_pinned_walk`. Kept as a
...
def _open_in_pinned_parent(
    ...
    Thin wrapper over :func:`codex_crew.pinned_fs.open_in_pinned_parent` that keeps
...
def _pin_parent(resolved_parent: str, *, what: str) -> int:
    ...
    Thin wrapper over :func:`codex_crew.pinned_fs.pin_parent`, which owns the
...
def _refuse_hardlink_alias(fd: int, *, what: str, name: str) -> None:
    ...
    Thin wrapper over :func:`codex_crew.pinned_fs.refuse_hardlink_alias`, which owns
...
def _is_reparse_point(path: Path) -> bool:
    ...
    Thin wrapper over :func:`codex_crew.pinned_fs.is_reparse_point`, which explains
