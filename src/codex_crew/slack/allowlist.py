...
1. **User allowlist** — when a user joins a tracked channel
   (``member_joined_channel``) or is nominated via ``/codexcrew @user``,
   the owner gets a DM with Allow / Deny buttons.
2. **Tracking channel** — ``/codexcrew #channel`` sends an Add / Ignore
...
from codex_crew.dashboard.token_auth import LINK_WINDOW_SECS, MAX_SESSION_TTL_SECS, generate_token
from codex_crew.sel import sel
from codex_crew.slack.handler import is_allowed_user, is_tracked_channel
from codex_crew.tunnel import get_tunnel_url, publish_disabled
...
async def prompt_allowlist(
    ...
    When the user is already allowed (manual ``/codexcrew @user`` recall),
...
async def send_dashboard_link(
    ...
    if cfg.slack.use_tunnel_url and local_only and not tunnel_url and publish_disabled():
        ...
        from codex_crew.platform.context import async_safe_context_call
...
# inline. `send_dashboard_link()` above (the explicit `/codexcrew dashboard`
