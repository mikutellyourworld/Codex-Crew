...
def _read_allowlist() -> tuple[set[str] | None, str]:
    ...
    **One reader.** The same validated read decides BOTH "is this config
    ...
    disagree.  Asking ``CodexCrewConfig.load()`` for the value while probing the
...
def _load_allowed_team_ids() -> bool:
    ...
    own ``CodexCrewConfig.load()`` -- may widen the allowlist afterwards, or the
...
def validate_enterprise(
    ...
    *,
    ...
    pass their own ``CodexCrewConfig.load()`` snapshot of
    ...
    :class:`~codex_crew.platform.interfaces.SlackEnterpriseGate` protocol, which
    ...
    if extra and degraded:
        ...
        # ``CodexCrewConfig.load()``-derived value -- which degrades a torn
        ...
        # ``CodexCrewConfig.load()`` snapshot of the same
