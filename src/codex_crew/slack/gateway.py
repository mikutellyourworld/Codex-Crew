"""Slack Socket Mode gateway orchestrator for CodexCrew.
...
import codex_crew
import codex_crew.crash_guard as crash_guard
from codex_crew import agent_scratch, beacon, dep_sync, name_grant, platform_compat, shutdown_event
from codex_crew.acp.client import AcpError, AcpProcessDied
from codex_crew.agent_sdk import AgentTurnUsage
from codex_crew.agents_janitor import sweep_agents_dir
from codex_crew.autonudge import (
    APPROVAL_STALL_REASON,
    MONITOR_TERMINAL_REASON,
    AutoNudgeService,
    NudgeLoop,
)
from codex_crew.autonudge import enabled as autonudge_enabled
from codex_crew.autonudge import (
    is_channel_key,
    is_structured_monitor_loop,
    runtime_budget_exceeded,
)
from codex_crew.beacon import distribution
from codex_crew.channel_history import ChannelHistory
from codex_crew.channels import builtin_channel_descriptors
...
from codex_crew.constants import DATA_WARNING, SUBAGENT_COMPLETION_META_KEY, strip_control_comments
from codex_crew.context import ContextBuilder
from codex_crew.context_management import summarize_result
from codex_crew.cron import (
...
from codex_crew.cron_script import delivery_fingerprint, run_command_sandboxed, run_script_sandboxed
from codex_crew.dashboard import cautious_boot, start_dashboard
from codex_crew.dashboard.chat_persistence import rehydrate_slot_from_history_async
from codex_crew.dashboard.chat_runner import (
...
from codex_crew.dashboard.cron_inject import (
    context_meter_reading,
    inject_cron_result_to_dashboard,
    prefetch_cron_history,
)
from codex_crew.dashboard.handlers import MAX_PROMPT_BYTES
from codex_crew.dashboard.handlers.autonudge import (
    _redact_monitor_value,
    compose_nudge_body,
    render_nudge_message,
)
from codex_crew.dashboard.handlers.updates import remediation_command as _remediation_command
from codex_crew.dashboard.handlers.usage import (
    persist_token_record_async,
    read_context_tokens,
    read_effective_agent,
)
from codex_crew.dashboard.origin import (
    build_dashboard_url,
...
from codex_crew.dashboard.stale_asset_watchdog import (
...
from codex_crew.dashboard.token_auth import MAX_SESSION_TTL_SECS, generate_token
from codex_crew.dashboard.turn_dispatch import bounded_chat_turn, spawn_guarded_turn
from codex_crew.embeddings import (
...
from codex_crew.executors import (
    CronQueueTimeout,
...
from codex_crew.frontend import build_frontend_async
from codex_crew.gateway_shutdown_budget import GRACEFUL_SHUTDOWN_SECS
from codex_crew.heartbeat import (
...
from codex_crew.hooks import HookManager, HooksConfig, hooks_config_from_config_dict
from codex_crew.kiro_cli import resolve_kiro_cli
from codex_crew.learn import LessonStore
from codex_crew.llm_helpers import (
...
from codex_crew.mcp_cron import vet_job_at_fire_time
from codex_crew.mcp_gateway import is_gateway_supported
from codex_crew.mcp_gateway.manager import (
    GatewayManager,
    GatewaySpec,
)
from codex_crew.mcp_gateway.resolve_once import prefetch as resolve_prefetch
from codex_crew.mcp_gateway.rewriter import (
    default_socket_path,
    resolve_overlay_dir,
    rewrite_agents,
)
from codex_crew.mcp_hot_reload import parse_kiro_cli_version
from codex_crew.memory import MemoryStore
from codex_crew.messaging import APPROVAL_INTERACTIVE, TurnDriver, registry
from codex_crew.messaging.dispatch import build_directive_consumer, build_tool_gate
from codex_crew.messaging.display_safety import redact_for_display
from codex_crew.messaging.identity import channel_inbound_permitted, publish_turn_identity
from codex_crew.messaging.link import (
...
from codex_crew.messaging.renderer import SilentRenderer, chunk_for_transport
from codex_crew.messaging.transport import InboundMessage, delivery_confirmed
from codex_crew.monitoring.completion import (
...
from codex_crew.platform import boot_platform
...
from codex_crew.platform.update_capability import (
    CHECK_SUCCEEDED,
    CHECK_UNCHECKED,
    EXTERNALLY_MANAGED_STAMPS,
)
from codex_crew.platform.update_governance import (
    commits_ahead,
    git_command_env,
...
from codex_crew.safety_override import flush_breadcrumb_writes, safety_override
from codex_crew.sandbox import ensure_agents_slice_limits, warm_backend
...
from codex_crew.service.common import restart_command_hint
from codex_crew.session import (
    HEARTBEAT_KEY,
    SessionBusyError,
    SessionClosingError,
    SessionManager,
...
from codex_crew.slack.client import RealSlackClient
from codex_crew.slack.format import (
...
from codex_crew.slack.handler import (
    _get_agent_for_session,
    build_timing_footer,
    is_thread_incognito,
    is_thread_temporary,
)
from codex_crew.slack.outbound import PostedOptions
from codex_crew.slack.retry import open_dm_with_retry
from codex_crew.slack.scope_probe import warn_unreadable_tracked_channels
from codex_crew.subagent import (
...
from codex_crew.subagent_completion_meta import (
    OUTCOME_FAILED,
    OUTCOME_OK,
    OUTCOME_STOPPED,
...
from codex_crew.tunnel import set_publish_disabled
from codex_crew.wecom.gateway import warn_if_channel_uncredentialed

if TYPE_CHECKING:
    from codex_crew.dashboard.state import _ChatSlot
    from codex_crew.discord.client import DiscordClient
    from codex_crew.imessage.client import IMessageClient
    from codex_crew.messaging.registry import ChannelDescriptor
    from codex_crew.providers.base import LLMProvider
    from codex_crew.subagent_scale import SubagentEventCoalescer
    from codex_crew.task_models import Task
    from codex_crew.teams.client import TeamsClient
    from codex_crew.telegram.client import TelegramClient
    from codex_crew.webex.client import WebexClient
    from codex_crew.wecom.client import WeComClient
    from codex_crew.weixin.client import WeixinClient
    from codex_crew.whatsapp.client import WhatsAppClient
...
async def _persist_turn_row(
    ...
    *,
    ...
    so a cold-cache ``CodexCrewConfig.load()`` neither runs on every turn nor
...
# Tunable via CODEXCREW_SUBAGENT_DIGEST_CHUNK_SIZE. Guarded parse: a malformed
...
def _digest_chunk_size() -> int:
    try:
        return max(1, min(int(os.environ.get("CODEXCREW_SUBAGENT_DIGEST_CHUNK_SIZE", "10")), 1000))
...
# codex_crew.subagent) instead of adding a third hand-maintained copy next to
...
# CODEXCREW_INJECTION_TIMEOUT) which only covers a single injected continuation turn.
...
HEARTBEAT_SAFE_TOOLS = frozenset(
    {
        ...
        "WorkspaceSearch",
        # CodexCrew-core reads (no side effects)
        "learn_list",
        "cron_list",
        "spawn_list",
        "spawn_status",
...
class CronVetOverran(Exception):
    ...
    :data:`~codex_crew.cron._SUBPROC_CLEANUP_ALLOWANCE_SECS`, so the deadline
...
def claim_vet_bound(job: CronJob) -> float:
    ...
    ``codex_crew.cron._vet_allowance`` adds to the run deadline.  Read from
...
def _claim_backstop(job: CronJob, subprocess_bound: int) -> float:
    ...
    :data:`~codex_crew.cron._SUBPROC_CLEANUP_ALLOWANCE_SECS`; reading the constant
...
async def _pinned_kiro_cli(purpose: str) -> str | None:
    ...
    ``CODEXCREW_KIRO_BIN``, with the inherited ``PATH`` excluded.
    ...
    if pinned is None and unpinned_exists:
        logger.warning(
            ...
            "not trust, so %s is skipped. Point CODEXCREW_KIRO_BIN at the binary "
...
class GatewayOrchestrator:
    ...
    def __init__(
        ...
        *,
        ...
        # client handle is owned by the channel registry (``codex_crew.channels``),
        ...
        # returns early when `CODEXCREW_AUTONUDGE=0`, BEFORE its only assignment,
        ...
        # The rewriter's ``CODEXCREW_MCP_TARGET_*`` mapping from the last broker
    ...
    async def _check_missing_deps(self) -> None:
        ...
        print(f">_ Installing missing dependencies: {', '.join(missing)}")
        ...
        try:
            ...
            print("❌ pip install timed out — run manually: codexcrew update")
        ...
        if proc.returncode == 0:
            ...
            print("❌ pip install failed — run manually: codexcrew update")
    ...
    async def _auto_open_dashboard(self, dashboard_url: str) -> None:
        ...
        try:
            ...
            print(
                ">_ Browser was slow to open — skipping auto-open.\n"
                "   Dashboard is running. Open this URL manually:\n"
                f"   {dashboard_url}\n"
                "   Or run: codexcrew token"
            )
    ...
    async def _init_services(self) -> None:
        ...
        try:
            from codex_crew.agent import rebuild_agent_config  # circular import
            ...
            # desktop app launches the gateway but never runs `codexcrew setup`.
            #
            ...
            print(
                ...
                "Repair with: codexcrew setup --agent-only --clean"
            )
        ...
        try:
            from codex_crew.agent import missing_required_agent_specs  # circular import
        ...
        from codex_crew.vector_memory import VectorMemoryStore
        ...
        from codex_crew.config.loader import ACTIVATION_OBSERVE
    ...
    async def _init_cron(self) -> None:
        ...
        async def _alert_cron_failure(job: CronJob, detail: str, *, denied: bool = False) -> None:
            ...
            * Never raises. Every call site is inside an ``except`` block whose
            ...
            try:
                ...
                # laptop and a cloud desktop can both run Codex Crew, and the
        ...
        async def _cron_callback(job: CronJob) -> str | None:
            ...
            def _cron_extra_env() -> dict[str, str] | None:
                """job.env plus CODEXCREW_APPROVAL_MODE when the job runs auto.
                ...
                CODEXCREW_SESSION_KEY/CODEXCREW_CHANNEL_ID) lets the spawn_run MCP
                tool (mcp_core.py) read and forward its own approval_mode
                explicitly, independent of whether parent_session resolution
                succeeds.

                CODEXCREW_APPROVAL_MODE is a RESERVED control var: it is stripped
                ...
                """
                env = {k: v for k, v in (job.env or {}).items() if k != "CODEXCREW_APPROVAL_MODE"}
                if job.approval_mode == "auto":
                    env["CODEXCREW_APPROVAL_MODE"] = "auto"
                return env or None
            ...
            if len(agents) > 1:
                ...
                for agent in agents:
                    ...
                    try:
                        ...
                        # CODEXCREW_SESSION_KEY and macOS sets no
                        # CODEXCREW_HOST_PID, so the ancestor PID-walk over the
            ...
            try:
                ...
                # laptop + a cloud desktop both running CodexCrew) can tell which
        ...
        if self._no_crons:
            ...
            from codex_crew.apps.bridges import reconcile_app_crons_for_execution
    ...
    async def _init_heartbeat(self) -> None:
        ...
        async def _heartbeat_task(task_text: str, deliver: str) -> str | None:
            ...
            _acquired = False
            try:
                # Use the dedicated ``codexcrew-heartbeat`` agent — minimal
                # MCP surface (codexcrew-core only on public installs) so cycle
                ...
                await _persist_turn_row(
                    ...
                    surface="heartbeat",
                    agent_fallback=lambda: "codexcrew-heartbeat",
                    t0=_turn_t0,
                )
            except asyncio.TimeoutError:
    ...
    async def _fire_webex_nudge(self, loop: NudgeLoop) -> bool:
        ...
        from codex_crew.webex.client import WebexInbound
        from codex_crew.webex.transport import ROOM_DIRECT
    ...
    async def _fire_dashboard_nudge(
        ...
        if banner and wake_message is None:
            ...
            visible = tagged
        from codex_crew.dashboard.chat import (
    ...
    async def _init_autonudge(self) -> None:
        ...
        from codex_crew.monitoring.controller import MonitorController
    ...
    async def _persist_slot_title(self, slot: "_ChatSlot") -> None:
        ...
        from codex_crew.dashboard.chat_utils import slot_history_key
    ...
    async def _deliver_result(
        ...
        if not deliver:
            try:
                if CodexCrewConfig.load().heartbeat.default_deliver == "dashboard":
                    deliver = "dashboard"
            except Exception:
    ...
    def _init_mcp_discovery(self) -> None:
        ...
        """
        try:
            from codex_crew.mcp_discovery import list_servers  # circular import
    ...
    def _init_subagents(self) -> None:
        ...
        async def _subagent_done(info: SubagentInfo) -> None:
            ...
            if _crew is not None:
                from codex_crew.crew_chat import CrewOrchestrator
    ...
    async def _init_api_server(self) -> None:
        ...
        from codex_crew.dashboard import start_api_server
    ...
    async def _start_embeddings(self) -> None:
        ...
        if model_file_present():
            ...
            logger.warning(
                ...
                "search. Run 'codexcrew doctor' for the reason."
            )
        else:
            logger.info(
    ...
    async def _auto_migrate_memory(self) -> None:
        ...
        """
        from codex_crew.memory import legacy_memory_present
    ...
    async def _set_memory_migrated(self, value: bool) -> None:
        ...
        from codex_crew.dashboard.handlers.memory import _set_migrated
    ...
    async def _refresh_mcp_resolutions(self) -> dict:
        ...
        self._cfg = CodexCrewConfig.load()
    ...
    async def _shutdown(self) -> None:
        ...
        with contextlib.suppress(Exception):
            from codex_crew.platform.policy_distribution import stop_refresher
        ...
        if self.dashboard_state:
            from codex_crew.dashboard.chat import save_all_slots_to_history
        ...
        # clean CodexCrew exit it and its pooled MCP subprocesses would
    ...
    async def _check_for_updates(self) -> None:
        ...
        Delegates to the resolved :class:`~codex_crew.platform.update_provider.CommandProvider`
        ...
        provider = None
        try:
            from codex_crew.platform.update_provider import resolve_provider
            ...
            try:
                ...
                if self.dashboard_state:
                    self.dashboard_state.push_update_progress(
                        "failed",
                        "Update provider failed — see logs; run manually: codexcrew update",
                    )
            return
    ...
    def _publish_provider_update_state(self, result: object) -> None:
        ...
        """
        from codex_crew.dashboard.handlers.updates import _update_info
    ...
    async def _check_for_updates_via_provider(self, provider: object) -> None:
        ...
        from codex_crew import __version__ as _running_version
        from codex_crew.config import CodexCrewConfig
        from codex_crew.platform.update_governance import min_version, update_required
        from codex_crew.platform.update_provider import UpdateProvider
        ...
        from codex_crew.platform.wheel_engine import respawn_executable
        ...
        if update_required(_running_version):
            ...
            if success:
                ...
                if self.dashboard_state:
                    self.dashboard_state.push_update_progress(
                        "failed", "Update apply failed — run manually: codexcrew update"
                    )
            return
        ...
        if result.available:
            ...
            print(">_ Already on latest version")
    ...
    async def _restart_after_update(self, respawn: Callable[[], str]) -> None:
        ...
        ``respawn`` is :func:`codex_crew.platform.wheel_engine.respawn_executable`,
        ...
        platform_compat.reexec_python_module("codex_crew", sys.argv[1:], executable=exe)

    async def _check_for_updates_legacy(self) -> None:
        """Legacy update check — the existing layout-aware logic."""
        try:
            from codex_crew import __version__ as _running_version
            from codex_crew.dashboard.handlers import _do_update_check, _update_info
    ...
    async def _auto_apply_update(self) -> None:
        ...
        The public OSS flow is the same one used by ``codexcrew update`` and the
        ...
        from codex_crew.platform.update_provider import _kill_and_reap
        ...
        try:
            ...
            if _git is None:
                logger.warning(
                    ...
                    "Run `codexcrew update` to apply this manually."
                )
                if self.dashboard_state:
            ...
            # needs a deliberate `codexcrew update`, and a detached HEAD has no
            ...
            # `codexcrew update`, where a human is deciding.
            ...
            if ahead != 0:
                logger.warning(
                    ...
                    "a reset would discard them. Run `codexcrew update` to decide.",
                    branch,
                    branch,
            ...
            #    up already refuses for COMMITTED work and defers to `codexcrew
            ...
            if status_proc.returncode != 0:
                ...
                logger.warning(
                    ...
                    "a reset could discard uncommitted changes. Run `codexcrew update`.",
                    loggable_path(proj),
                )
                if self.dashboard_state:
                    self.dashboard_state.clear_update_progress()
            ...
            if hidden is None or hidden:
                logger.warning(
                    ...
                    "them. Run `codexcrew update` to decide.",
                    loggable_path(proj),
            ...
            if added_proc.returncode != 0:
                logger.warning(
                    ...
                    "a reset could overwrite untracked files. Run `codexcrew update`.",
                    branch,
                )
                if self.dashboard_state:
                    self.dashboard_state.clear_update_progress()
            ...
            if collisions:
                logger.warning(
                    ...
                    "Run `codexcrew update` to decide.",
                    branch,
                    len(collisions),
            ...
            if pip_rc != 0:
                ...
                #     `-m codex_crew` so it would survive, but the next restart
                #     through the service manager runs `codexcrew` and does not.
                #
            ...
            importlib.reload(codex_crew)
            new_ver = codex_crew.__version__
            print(f">_ New version {new_ver} available — auto-updating and restarting…")
            if self.dashboard_state:
                self.dashboard_state.push_update_progress("restarting", "Restarting server…")
                from codex_crew.dashboard.chat import save_all_slots_to_history
            ...
            # Use -m codex_crew rather than sys.argv[0] so the restart resolves
    ...
    async def _auto_apply_wheel_update(self) -> None:
        ...
        * The install is NOT a git checkout (no ``can_apply``) and NOT externally
        ...
        :func:`codex_crew.platform.update_layout.wheel_update_command` from a
        ...
        """
        from codex_crew.dashboard.handlers import _update_info
        ...
        from codex_crew.platform.update_governance import update_blocked_reason
        from codex_crew.platform.update_layout import cdn_bases, cdn_bases_are_safe

        # The installer command embeds the CDN bases and is handed to a shell, and
        # CODEXCREW_CDN_BASE is operator-set: a metacharacter could close the URL
        # and append a second command, and an http:// override would make the
        # piped installer interceptable. Same gate `codexcrew update` applies.
        ...
        from codex_crew.platform.update_provider import (
            _kill_and_reap,
            _read_bounded_output,
            _trusted_path_env,
        )
        from codex_crew.platform_compat import trusted_system_bin
        ...
        if not _sh:
            ...
            if self.dashboard_state:
                self.dashboard_state.push_update_progress(
                    "failed", "No trusted shell — run manually: codexcrew update"
                )
            return
        ...
        if _env is None:
            ...
            if self.dashboard_state:
                self.dashboard_state.push_update_progress(
                    "failed", "No trusted PATH — run manually: codexcrew update"
                )
            return
        ...
        try:
            ...
            if self.dashboard_state:
                self.dashboard_state.push_update_progress(
                    "failed", "Installer timed out — run manually: codexcrew update"
                )
            return
        except OSError:
            ...
            if self.dashboard_state:
                self.dashboard_state.push_update_progress(
                    "failed", "'sh' not available — run manually: codexcrew update"
                )
            return
        ...
        if proc.returncode != 0:
            ...
            if self.dashboard_state:
                self.dashboard_state.push_update_progress(
                    "failed",
                    f"Installer failed (exit {proc.returncode}) — " "run manually: codexcrew update",
                )
            return
    ...
    async def _connect_slack(self) -> bool:
        ...
        try:
            ...
            print(">_ Codex Crew gateway connected to Slack")
            return True
        except Exception as exc:
    ...
    def _write_marker_worker(self, run_marker: Any, port: int) -> None:
        ...
        ``run_marker`` is the :mod:`codex_crew.instances.run_marker` module,
    ...
    async def run(self) -> None:
        ...
        from codex_crew.session import cleanup_orphaned_sessions
        ...
        async def _backfill_unclean_session_telemetry() -> None:
            try:
                from codex_crew.metrics.sessions import backfill_crashed_sessions
        ...
        from codex_crew.slack.events import SeenCache, init_socket_mode
        from codex_crew.slack.interactions import init as init_interactions
        ...
        # Record this gateway's own codexcrew launcher, keyed by the port it
        # serves, so a remote token-mint execs THIS install's venv instead of
        # a stale ~/.local/bin/codexcrew that may point at an uninstalled
        # worktree. See codex_crew.instances.run_marker. Written for headless
        # API-only gateways too: the marker's filename is what lets a client
        # or MCP child discover a non-default port when neither CODEXCREW_PORT
        ...
        # CODEXCREW_READY: prefix matcher.
        if self._json_ready:
            ...
            print(f"CODEXCREW_READY:{json.dumps(ready_payload)}", flush=True)
        ...
        if not self._test_mode:
            with contextlib.suppress(Exception):
                from codex_crew.agent import (
                    prime_ceiling_projection,
                    reproject_for_ceiling_change,
                )
                from codex_crew.dashboard.tailnet_serve import (
                    revoke_if_governance_now_pins_off,
                )
                from codex_crew.platform.policy_distribution import (
        ...
        # under CODEXCREW_AUTONUDGE=0, so a claim made in there means no process ever
        ...
        try:
            from codex_crew.metrics.inventory_gauges import mark_install_reporter
        ...
        def _on_signal(*_args: object) -> None:
            ...
            if _shutting_down:
                ...
                try:
                    from codex_crew.cli import _stop_log_queue_listener
        ...
        if not self._no_dashboard:
            try:
                ...
                if self._no_open or not self._cfg.dashboard.auto_open_browser:
                    ...
                    print(">_ Headless remote session — skipping browser auto-open")
                else:
                ...
                # the service, and `codexcrew token` still produces a URL.
        ...
        from codex_crew.dashboard.handlers import _bg_mcp_probe

        print(">_ Probing MCP servers…")
        ...
        try:
            ...
            print(">_ MCP probe timed out — continuing without full probe")
        ...
        print(">_ Codex Crew gateway starting…")
        print(f"\n{DATA_WARNING}\n")
        ...
        from codex_crew.cli import drain_log_queue_before_hard_exit
    ...
    async def _start_channel_transports(
        ...
        :func:`codex_crew.channels.builtin_channel_descriptors` and the loop
        lives in :mod:`codex_crew.messaging.registry` — adding a channel no
        ...
        # (issues #304, #5418). Runs after CODEXCREW_READY, outside the
    ...
    def _badge_unready_channels(self, bootable: "tuple[ChannelDescriptor, ...]") -> None:
        ...
        """
        try:
            from codex_crew.channels import channel_readiness
...
async def run_gateway(
    ...
    *,
    ...
    # kiro agents directory (see codex_crew.agents_janitor). Scheduled as a
    ...
    # (see codex_crew.agent_scratch). Liveness-keyed, never age-keyed, so a
    ...
    if not test_mode:
        with contextlib.suppress(Exception):
            threading.Thread(
                target=beacon.send,
                args=(cfg.telemetry.beacon_endpoint, codex_crew.__version__),
                kwargs={
                    "enabled": cfg.telemetry.beacon_enabled,
                    "acked": cfg.dashboard.privacy_acked,
                },
                name="codexcrew-beacon",
                daemon=True,
            ).start()
