...
from codex_crew.slack.allowlist import (
...
from codex_crew.slack.outbound import (
    PostedOptions,
...
from codex_crew.slack.renderer import (
...
def _probe_tracked_channel_scope(channel_ids: set[str]) -> None:
    ...
    the probe (see :mod:`codex_crew.slack.scope_probe`) turns that silent-dead
...
async def dispatch(payload: dict) -> None:
    ...
    # ── Cron acknowledge ──
    from codex_crew.slack.format import CRON_ACK_ACTION_PREFIX
    ...
    from codex_crew.slack.format import SUBAGENT_ACK_ACTION_PREFIX
...
async def _refresh_channels_modal(view_id: str) -> None:
    ...
    if not _orch or not _orch.slack:
        return
    from codex_crew.slack.blocks import channels_modal
    ...
    from codex_crew.slack.events import _get_agent_names
...
async def _handle_ch_activation(payload: dict, action: dict) -> None:
    ...
    from codex_crew.slack.handler import _persist_channel_config
    ...
    if _orch:
        ...
        _orch._cfg = CodexCrewConfig.load()
    sel().log_api_access(
        caller=caller,
...
async def _handle_ch_remove(payload: dict, action: dict) -> None:
    ...
    from codex_crew.slack.allowlist import persist_tracking_channel
...
async def _handle_voice_config_submission(payload: dict) -> None:
    ...
    if not is_owner(caller):
        return
    from codex_crew.slack.handler import _vc
        ...
        if _orch.slack:
            try:
                ...
                await _orch.slack.post_message(
                    ...
                    "⚠️ *Do not enter sensitive or confidential data into Codex Crew.*"
...
async def _handle_users_select(
    ...
    from codex_crew.slack.handler import is_owner, set_allowed_users
...
async def _handle_channels_select(
    ...
    # Imported at call time on purpose: tests patch ``handler.is_owner`` to drive
    # the non-owner rejection, and only a call-time rebind observes that patch.
    from codex_crew.slack.handler import is_owner, set_tracking_channels
...
async def _handle_stop_confirm(payload: dict, channel: str, msg_ts: str, user_id: str) -> None:
    ...
    if has_session or active_task:
        ...
        async def _on_soft() -> None:
            from codex_crew.slack.blocks import build_stopped_blocks
        ...
        async def _on_hard() -> None:
            from codex_crew.slack.blocks import build_stop_failed_blocks
...
async def _handle_allowlist_remove(
    ...
    from codex_crew.slack.blocks import allowlist_list_block
...
async def _handle_channel_remove(
    ...
    from codex_crew.slack.blocks import channel_list_block
...
async def _handle_resume_choice(
    ...
    async with lock:
        ...
        try:
            from codex_crew.config.loader import data_home
...
async def _handle_session_end(
    ...
    # Also try treating value as a direct session key (from /codexcrew sessions buttons)
...
async def _handle_review_approve(payload: dict, action: dict) -> None:
    ...
    from codex_crew.slack.handler import _review_drafts_get, _review_drafts_pop
...
async def _handle_review_edit(payload: dict, action: dict) -> None:
    ...
    from codex_crew.slack.blocks import review_edit_modal
    from codex_crew.slack.handler import _review_drafts_get
...
async def _handle_review_revise(payload: dict, action: dict) -> None:
    ...
    from codex_crew.slack.blocks import review_revise_modal
    from codex_crew.slack.handler import _review_drafts_get
