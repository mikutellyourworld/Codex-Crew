...
Both modes live in :mod:`codex_crew.messaging.privacy_mode`, keyed by session key,
...
from codex_crew import name_grant
from codex_crew.acp.client import AcpError, AcpProcessDied, AcpPromptBusy, AcpTimeoutError
from codex_crew.acp.types import (
    STOP_REASON_CANCELLED,
    STOP_REASON_COMPACTION_FAILED,
    STOP_REASON_END_TURN,
)
from codex_crew.agent_discovery import project_agent_files, project_agent_name
...
from codex_crew.dashboard.state import append_and_surface
from codex_crew.executors import run_in_embed_pool
...
from codex_crew.messaging.commands import (
...
from codex_crew.messaging.link import canonical_key
from codex_crew.messaging.renderer import credential_redaction_notice
from codex_crew.messaging.session_trust import _trusted_sessions as _shared_trusted_sessions
from codex_crew.messaging.session_trust import add_trusted_session as _add_trusted_session
from codex_crew.messaging.session_trust import clear_trusted_sessions, is_session_trusted
...
from codex_crew.session import SessionClosingError, SessionManager
from codex_crew.session_map import SessionMap
from codex_crew.slack.blocks import build_working_blocks, deprecation_warning_block
from codex_crew.slack.client import SlackClientOps
...
from codex_crew.slack.sessions_view import (
    _SESSIONS_DEFAULT_LIMIT,
    _build_sessions_blocks,
    _collect_recent_sessions_off_loop,
)
from codex_crew.stats import Stats
from codex_crew.subagent import SubagentManager
from codex_crew.task import Task
from codex_crew.taskrunner import TaskRunner
from codex_crew.voice_reply import DEFAULT_PROVIDER, VALID_PROVIDERS
from codex_crew.voice_reply import is_available as _tts_available
from codex_crew.voice_reply import validate_length_scale as _validate_length_scale
from codex_crew.voice_reply import voice_reply as _voice_reply_fn

logger = logging.getLogger(__name__)

# Mapping of bang commands to their /codexcrew slash equivalents.
_BANG_TO_SLASH: dict[str, str] = {
    "!yolo": "/codexcrew yolo",
    "!stop": "/codexcrew stop",
    "!voice": "/codexcrew voice",
    "!agent": "/codexcrew agent",
    "!dashboard": "/codexcrew dashboard",
    "!ta": "/codexcrew agent",
    # "!allowlist" removed — multi-user access disabled for security
    "!channel": "/codexcrew channel",
    "!link-to-dashboard": "/codexcrew link-to-dashboard",
    "!restart": "/codexcrew restart",
}
...
try:
    _overrides = CodexCrewConfig.load().slack.reactions
except Exception:
...
# Falls back to single CODEXCREW_OWNER_ID for backward compatibility.
...
_orch_cfg: CodexCrewConfig | None = None
...
def _is_slack_restricted(session_key: str) -> bool:
    ...
    :func:`codex_crew.messaging.privacy_mode.is_restricted`); the Slack spelling
...
def _conv_state_map(sessions: object) -> "SessionMap | None":
    ...
    Thin wrapper over :func:`codex_crew.messaging.privacy_mode.conv_state_map`,
...
def _get_default_agent() -> str:
    ...
    if _cached_default_agent is None:
        _cached_default_agent = CodexCrewConfig.load().agent.default_agent
...
def _list_all_agent_names(cc_plugins_dir: Path | None = None) -> str:
    ...
    :func:`_iter_cc_agent_names`. The internal ``codexcrew-lite`` variant is
    ...
    agents_dir = kiro_agents_dir()
    if agents_dir.is_dir():
        # Hide the internal codexcrew-lite variant from BOTH sources — a
        # ~/.kiro/agents/codexcrew-lite.json would otherwise leak into the list.
        names.extend(f.stem for f in sorted(agents_dir.glob("*.json")) if f.stem != "codexcrew-lite")
    seen = set(names)
    for agent_name in _iter_cc_agent_names(cc_plugins_dir):
        if agent_name not in seen and agent_name != "codexcrew-lite":
...
def set_orch_cfg(cfg: CodexCrewConfig) -> None:
...
def load_voice_reply_config(cfg: "CodexCrewConfig | None" = None) -> None:
...
def get_orch_cfg() -> "CodexCrewConfig | None":
...
def is_allowed_user(user_id: str) -> bool:
    ...
    (CODEXCREW_OWNER_ID) is authorized to interact via Slack.
    """
    if not user_id:
        return False
...
async def _handle_slash_command(
    ...
    if cmd == "!stop":
        ...
        from codex_crew.slack.blocks import build_stopping_blocks
    ...
    if cmd == "!voice":
        from codex_crew.voice_reply import VALID_ENGINES, _validate_pitch, _validate_rate
    ...
    if cmd == "!agent":
        parts = cmd_text.split()
        if len(parts) == 1:
            name = _get_default_agent() or "codexcrew"
            await slack.post_message(
                channel,
    ...
    if cmd == "!dashboard":
        ...
        from codex_crew.slack.allowlist import send_dashboard_link
    ...
    if cmd == "!link-to-dashboard":
        ...
        from codex_crew.slack.interactions import _import_thread_to_slot
    ...
    if cmd == "!allowlist":
        await slack.post_message(
            channel,
            "⛔ Multi-user access is disabled for security. Only the owner can use Codex Crew via Slack.",
            reply_ts,
        )
        return ""
    ...
    if cmd == "!channel":
        if not is_owner(user_id):
            ...
            return ""
        from codex_crew.config.loader import _VALID_ACTIVATIONS
    ...
    await slack.post_message(
        channel,
        f"❌ Unknown command `{cmd}`. Type `/codexcrew help` for available commands.",
        reply_ts,
    )
    return ""
...
def _append_footer_actions(
    ...
    *staleness_token* must be minted by the caller, which is async and can do the
    ...
    """
    if options:
        from codex_crew.slack.format import build_options_blocks
    ...
    if thread_ts and not linked_session_key and dashboard_state:
        from codex_crew.slack.format import build_link_dashboard_button
...
async def maybe_route_linked_thread(
    ...
    ``/codexcrew link-to-dashboard`` behaves identically on both.
...
async def handle_message(
    ...
    *channel_agent* overrides the default agent for this channel (set via
    ...
    # @mention: "<@UBOT|codexcrew> !agent foo"   → "!agent foo"      (strip prefix)
    ...
    status_ctrl = StatusReactionController(
        ...
        channel,
        msg_ts,
        enabled=CodexCrewConfig.load().slack.reactions_enabled,
    )
    ...
    _show_thinking = CodexCrewConfig.load().slack.show_thinking
    ...
    if channel_activation == ACTIVATION_REVIEW:
        from codex_crew.slack.blocks import review_draft_blocks
...
async def handle_interaction(
    ...
    if not pending:
        ...
        if action_id == _ACTION_TRUST and thread_ts:
            ...
            # ``codex_crew.session.SessionMap`` to drive the fail-closed path, and
...
async def _handle_sessions_command(
    ...
    *,
    ...
    Delegates to
    :func:`codex_crew.slack.sessions_view._collect_recent_sessions_off_loop`
    and :func:`codex_crew.slack.sessions_view._build_sessions_blocks` so the
