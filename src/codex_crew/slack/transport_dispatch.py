...
from codex_crew.hooks import HOOK_REPLY, TOOL_AUTO_APPROVE, TOOL_DENY
from codex_crew.llm_helpers import save_conversation_turn_off_loop
from codex_crew.messaging import auto_title
from codex_crew.messaging.dispatch import build_directive_consumer
...
from codex_crew.slack.renderer import PARTIAL_TURN_MARKER, SlackApprovalDecider, SlackRenderer
...
#: Canonical kiro-cli agent name for CodexCrew. Used as the final agent
#: fallback so the transport session loads the same MCP surface (incl. the
#: codexcrew-core server that provides ``spawn_run``) as the dashboard/native
#: path. Without it, an empty ``agent.default_agent`` config makes kiro-cli
#: launch under its bare built-in default (no codexcrew-core -> no spawn_run).
#: Mirrors the native path's fallback (``handler.py``: ``_get_default_agent()
#: or "codexcrew"``) and the many ``agent="codexcrew"`` call sites.
...
async def _refresh_dashboard_tab(session_key: str) -> None:
    ...
    try:
        ...
        from codex_crew.dashboard.channel_slots import surface_channel_state
...
async def handle_message_transport(
    ...
    *,
    ...
    # /codexcrew link-to-dashboard routes into its dashboard slot (with the same
    ...
    try:
        ...
        # default win; otherwise fall back to the canonical "codexcrew" agent so
        # the session loads codexcrew-core (spawn_run) rather than kiro-cli's bare
        ...
        driver = TurnDriver(
            ...
            audit_agent=_agent or "codexcrew",
