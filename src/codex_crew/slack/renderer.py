...
``codex_crew.messaging.TurnDriver`` onto Slack's streaming + Block Kit surface
...
* **Length splitting** belongs to
  :func:`codex_crew.messaging.split.split_markdown_safe`, the shared fence-safe
...
* **Outbound local-image extraction** belongs to
  :mod:`codex_crew.messaging.outbound_files`, with Slack's per-file ceiling, count
  cap and ``files_upload_v2`` call in :mod:`codex_crew.slack.files`. Extraction
...
from codex_crew.messaging.outbound_files import (
    OutboundFile,
    Rejection,
...
from codex_crew.messaging.renderer import Renderer, chunk_text
...
from codex_crew.slack.files import UPLOAD_LIMITS, upload_outbound_files
from codex_crew.slack.format import SLACK_MSG_LIMIT, extract_options, strip_thinking_tags
