...
onto :class:`~codex_crew.messaging.attachments.Attachment`, and supplying an
...
in :mod:`codex_crew.messaging.attachments` so other channels reuse them instead of
...
The OUTBOUND half is the mirror image: :mod:`codex_crew.messaging.outbound_files`
...
from codex_crew.messaging.attachments import (
    Attachment,
    IngestLimits,
    ingest_attachments,
    safe_suffix,
...
from codex_crew.platform_compat import restrict_dir_to_owner
...
#: the blocks :func:`codex_crew.messaging.attachments.transcribe_audio_attachments`
...
async def upload_outbound_files(
    ...
    **The bytes travel, not the path.** ``OutboundFile.data`` is what every gate
    in :mod:`codex_crew.messaging.outbound_files` was applied to, and the
...
async def _upload_one(
    ...
    """
    staging = await asyncio.to_thread(tempfile.mkdtemp, prefix="codexcrew-slack-upload-")
    try:
