...
from codex_crew.cron import format_schedule
from codex_crew.dashboard.chat_utils import run_config_write
from codex_crew.dashboard.handlers import get_update_info
from codex_crew.dashboard.token_auth import LINK_WINDOW_SECS, MAX_SESSION_TTL_SECS, parse_duration
from codex_crew.executors import subprocess_executor
from codex_crew.hooks import safe_read_file
from codex_crew.mcp_discovery import list_servers
from codex_crew.messaging.identity import channel_inbound_permitted
from codex_crew.platform import current_context, safe_context_call
from codex_crew.platform.interfaces import InterceptDecision
from codex_crew.safety_override import safety_override, yolo_policy_verdict
...
from codex_crew.session import unlink_queued_temp_paths
from codex_crew.skills import SkillsLoader
from codex_crew.slack.allowlist import prompt_track_channel, send_dashboard_link
from codex_crew.slack.blocks import (
...
from codex_crew.slack.enterprise import validated_self_bot_id
from codex_crew.slack.files import (
    VOICE_MEMO_FAILED,
...
from codex_crew.slack.interactions import dispatch as dispatch_interactive
...
from codex_crew.slack.transport_dispatch import handle_message_transport
from codex_crew.stats import Stats
from codex_crew.transcribe import is_available as stt_available
from codex_crew.transcribe import transcribe_audio
...
def register_slash_command(name: str, handler: SlashHandler, description: str = "") -> None:
    """Register a sub-command for ``/codexcrew <name>``."""
    SLASH_REGISTRY[name] = (handler, description)


def _build_help_text(cmd_name: str = "codexcrew") -> str:
...
async def _handle_agent(
    ...
    from codex_crew.slack.handler import (  # circular import: handler.py imports events.py for command dispatch, creating runtime circular dependency
        _get_default_agent,
        _resolve_agent_name,
        _set_default_agent,
        is_owner,
...
async def _handle_config(
    ...
    view = {
        ...
        "title": {"type": "plain_text", "text": "Codex Crew Config"},
...
def _get_agent_names() -> list[str]:
    ...
    for f in agents_dir.glob("*.json"):
        try:
            ...
            # `/codexcrew channels` handler / channel-modal refresh task before
...
async def _handle_restart(
    ...
    if not is_owner(caller_id):
        sel().log_tool_invocation(
            session_key="", source="slack", tool_name="/codexcrew restart",
...
def init_socket_mode(orch: GatewayOrchestrator, seen: SeenCache) -> None:
    ...
    if not orch._owner_id:
        logger.error("CODEXCREW_OWNER_ID is not set — Slack disabled for security")
...
async def _publish_home_tab(orch: GatewayOrchestrator, user_id: str) -> None:
    ...
    try:
        ...
        blocks.append(
            {
                ...
                "text": {
                    ...
                    "text": (
                        ...
                        " into Codex Crew.* Follow your organization's data handling"
        ...
        yolo = is_yolo_mode()
        blocks.append(
            {"type": "header", "text": {"type": "plain_text", "text": ">_ Codex Crew Status"}}
        )
        status_lines = [
            "*Gateway:* ✅ Online",
        ...
        if orch.cron_svc is not None:
            ...
            if jobs:
                try:
                    _tz = CodexCrewConfig.load().timezone
                except Exception:
                    _tz = ""
        ...
        if vs is not None and callable(getattr(vs, "get_lessons", None)):
            ...
            if isinstance(all_vs, list):
                ...
                from codex_crew.vector_memory import _lesson_display_text
        ...
        # ── Version ──
        version_text = f"📦 Codex Crew v{__version__}"
...
async def _handle_slash(orch: GatewayOrchestrator, payload: dict) -> None:
    """Route ``/codexcrew <sub-command>`` via :data:`SLASH_REGISTRY`.
    ...
    if user_match:
        _spawn_tracked(
            _respond("⛔ Multi-user access is disabled. Only the owner can use Codex Crew via Slack.")
        )
        return
    ...
    try:
        if _use_transport:
            await handle_message_transport(
                ...
                reactions_enabled=CodexCrewConfig.load().slack.reactions_enabled,
                show_thinking=CodexCrewConfig.load().slack.show_thinking,
...
async def _route_message(
    ...
    # ── !restart: bang alias for /codexcrew restart — intercept here so it
    ...
    # the native path. (CodexCrew has no challenge-redirect path, so this simply
