"""Caller-identity protocol extension for CodexCrew MCP servers.

BACKGROUND
----------
The base MCP protocol (Anthropic spec 2024-11-05) models a 1:1 stdio
transport: one client, one server. When CodexCrew introduces a broker that
...
Previously, the backend read this identity from the ``CODEXCREW_SESSION_KEY``
...
2. **Capability negotiation** — backends that support pooled operation
   advertise the extension in their ``initialize`` response::
       ...
       "capabilities": {
           ...
           "experimental": {
               "codexcrew.caller-identity": {"schemaVersion": 1}
           }
       }
...
3. **Typed context** — tool handlers receive a ``CallerContext`` dataclass
   ...
   constructed from the ambient ``CODEXCREW_SESSION_KEY`` env var so
...
any future CodexCrew-owned MCP server must go through this module.
"""
...
CALLER_META_KEY = "codexcrew.caller"

#: Capability key backends advertise in ``initialize`` response under
#: ``capabilities.experimental``. Presence indicates the backend
#: understands the ``codexcrew.caller`` ``_meta`` block and is safe to pool.
CALLER_CAPABILITY_KEY = "codexcrew.caller-identity"
...
TENANT_META_KEY = "codexcrew.tenant"
...
def _parent_pid(pid: int) -> int:
    ...
    Delegates to :func:`codex_crew.platform_compat.get_ppid`, which reads
...
class CallerContext:
    """Identity of the originating CodexCrew session for a single MCP call.
    ...
    def from_env(cls) -> "CallerContext":
        """Synthesize a context from the ``CODEXCREW_SESSION_KEY`` env var, or
        ...
        Fallback order:
          1. ``CODEXCREW_SESSION_KEY`` env var (primary, per-session mode)
...
# ``codexcrew.caller`` block from every inbound frame (``backend.py``
...
_CURRENT_CALLER: ContextVar["CallerContext | None"] = ContextVar(
    "codexcrew_current_caller", default=None
)
...
_CURRENT_TENANT_NONCE: ContextVar[str] = ContextVar(
    "codexcrew_current_tenant_nonce", default=""
)
