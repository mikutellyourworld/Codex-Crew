...
from codex_crew import pinned_fs, skill_trust
...
from codex_crew.cron import referenced_skill_names
from codex_crew.frontmatter import SKILL_LOADER, parse_frontmatter
...
from codex_crew.project_scope import project_scope_satisfied
...
from codex_crew.skill_usage import SKILL_USAGE_FILENAME, SkillUsageLedger
from codex_crew.skills_script_validator import validate_scripts
...
# across all three sources (codexcrew builtin, workspace, extra paths).
...
# hand-authored skills.  Final path: ``~/.codex-crew/skills/auto/<name>/SKILL.md``.
...
# Bundled fallback — inside the codex_crew package
...
def _project_skills_dir() -> Path | None:
    """Return project-level skills/ dir from CODEXCREW_PROJECT_DIR, or None."""
...
def _trusted_skill_roots() -> tuple[str, ...]:
    ...
    * the installed ``codex_crew`` package — built-in apps keep their skills
...
def _disabled_app_names() -> frozenset[str]:
    ...
    """
    try:
        from codex_crew.apps.manager import list_apps
...
# Skills RELOCATED into the codexcrew-dev/ folder (the Codex Crew development
...
_RELOCATED_SKILLS: dict[str, str] = {
    "prepare-pr": "codexcrew-dev/prepare-pr",
    "babysit": "codexcrew-dev/babysit",
    "codexcrew-worktree-dev": "codexcrew-dev/codexcrew-worktree-dev",
}
...
def _skill_tree_fingerprint(root: Path) -> str | None:
    ...
    :func:`codex_crew.hooks.safe_read_file_bytes_nolink` with the tree root as
...
def deployed_cron_script_sources() -> list[CronScriptSource]:
    ...
    bodies are LLM-writeable by design (see :mod:`codex_crew.cron_script`) and
...
def _claim_dir_for_replacement(dest_dir: Path) -> Path | None:
    ...
    Codex Crew instance syncing the same home) can swap the directory between
...
def _ensure_builtin_skills(base: Path) -> None:
    ...
    stale_builtins = {"learn", "subagent", "cron", "codexcrew-core"} - source_names
    if base.exists():
...
class SkillsLoader:
    """Load skill markdown files from ~/.codex-crew/skills/.
    ...
    def __init__(
        ...
        config: CodexCrewConfig | None = None,
    ):
        ...
        cfg = config or CodexCrewConfig.load()
        ...
        # Co-located with the skills root's parent (the CodexCrew home) so it
    ...
    def list_skills(self, project_dir: str | Path | None = None) -> list[dict]:
        ...
        ``owned`` says whether Codex Crew may rewrite the file. A skill reached
    ...
    def _owning_app(self, name: str, skill_file: Path) -> str | None:
        ...
        * the flat ``skills/<skill>`` link, whose name says nothing — so the
        ...
        if head != name:
            return head
        try:
            from codex_crew.apps.manager import apps_dir
    ...
    def _owned_hint(self, skill_file: Path) -> bool:
        """Whether *skill_file* sits under the directory Codex Crew owns.
    ...
    def _emit_lazy_load_metric(t0: float, *, hit: bool) -> None:
        ...
        try:
            ...
            get_recorder().histogram(
                "codexcrew.skill.lazy_load.duration",
                elapsed_ms,
                unit="ms",
                attrs=attrs,
            )
            get_recorder().counter("codexcrew.skill.lazy_load.count", attrs=attrs)
    ...
    def _repo_scope_satisfied(relpath: str, project_dir: str | Path | None) -> bool:
        ...
        — e.g. ``repo_scope: src/codex_crew`` restricts a skill to sessions
        whose active project IS the Codex Crew source tree. This is the
        ...
        *project_dir* is the SESSION's active project — the same value the
        ...
        The rule itself lives in ``codex_crew.project_scope`` because lessons are
    ...
    def set_inject_on_trigger(self, name: str, inject: bool) -> bool:
        ...
        kiro-cli user/workspace skill dirs — directories Codex Crew does not own
    ...
    def strip_frontmatter(content: str) -> str:
        ...
        ``codex_crew.frontmatter``. Its closer grammar matches
