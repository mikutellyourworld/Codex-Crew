...
from codex_crew.frontmatter import SKILL_UPDATE, frontmatter_value
from codex_crew.llm_helpers import (
    ToolApprovalPolicy,
    background_turn,
)
from codex_crew.security import redact_credentials, redact_exfiltration_urls
from codex_crew.skills import AUTO_SKILL_MAX_PROCEDURE_CHARS, AutoSkillProvenance
from codex_crew.skills_dedupe import (
    VERDICT_DUP,
    VERDICT_NEW,
    VERDICT_UPDATE,
)
from codex_crew.skills_script_validator import validate_skill_script
from codex_crew.vector_memory_constants import (
...
_HISTORY_LOGGER = logging.getLogger("codex_crew.history")
...
def _facade_sel() -> Any:
    from codex_crew import history as history_facade
...
_SENSITIVE_TOOL_PATTERNS: tuple[str, ...] = (
    ...
    ".netrc",
    ".git-credentials",
    # Codex Crew's own credential file. The data home moved to ~/.codex-crew, so the
    # LIVE secret is ~/.codex-crew/.env; cover the pre-move legacy home too
...
def _strip_skill_frontmatter(text: str | None) -> str:
    ...
    field parser — deliberately outside ``codex_crew.frontmatter``; editing
...
class HistoryConsolidator:
    ...
    def _logger(self) -> logging.Logger:
        """Keep the pre-extraction ``codex_crew.history`` logger category."""
    ...
    def consolidate_session(self, key: str) -> None:
        ...
        and the ``codexcrew consolidate`` CLI command.  Skips if the session
    ...
    async def _dedupe_judge(self, prompt: str) -> str:
        ...
        try:
            async with background_turn(
                self._sessions, task="skill_dedupe", agent="codexcrew-lite"
            ) as client:
    ...
    async def _merge_skill_update(
        ...
        try:
            async with background_turn(
                self._sessions, task="skill_merge", agent="codexcrew-lite"
            ) as client:
                text = await _facade_stream_and_collect(
                    client, prompt, approval_policy=ToolApprovalPolicy.REJECT_ALL
                )
        ...
        async with contextlib.AsyncExitStack() as stack:
            try:
                client = await stack.enter_async_context(
                    background_turn(self._sessions, task="consolidation", agent="codexcrew-lite")
                )
            ...
            # scopes the codexcrew-lite session to tools:[] via set_mode, but the
            # Claude Code backend skips set_mode and injects the full
            # codexcrew-core/cron toolset — without REJECT_ALL a background
