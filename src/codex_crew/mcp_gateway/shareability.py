...
* The probe connects as ``clientInfo: {"name": "codexcrew-probe"}`` while the
  gateway connects as ``codexcrew-gateway``. A server that negotiates its
...
------------------------------------------------------------------
``codexcrew.caller-identity`` is an extension this project invented; the MCP base
...
CALLER_IDENTITY_CAPABILITY = "codexcrew.caller-identity"
...
class ShareEvidence:
    ...
    #
    # Deliberately NOT "is this one of ours". Codex Crew's own managed servers
    # differ from each other here: ``codexcrew-core`` advertises the
    # caller-identity extension and consumes the injected caller, while
    # ``codexcrew-cron`` does not and still reads process identity. Keying this on
...
def assess(
    ...
    #    ``codexcrew-computer`` advertises yet stays session-bound deliberately
