...
from codex_crew.mcp_gateway.rewriter import _WRAPPER_MARKER, _WRAPPER_MARKER_LEGACY
...
def _acp_server_entry(
    ...
    ``-m codex_crew.mcp_gateway.stub`` and anything inserted ahead of that would
...
def pooled_session_servers(
    ...
    ``_meta.codexcrew.caller``. It is deliberately NOT a pool dimension (see
    :mod:`codex_crew.mcp_gateway.pool`), so passing it does not split backends —
