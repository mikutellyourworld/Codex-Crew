"""Supervise the CodexCrew MCP gateway subprocess.

Lifecycle:

1. :meth:`GatewayManager.start` — spawn ``python -m
   codex_crew.mcp_gateway.gatewayd``, wait until the unix socket appears,
...
Gateway failures are non-fatal for CodexCrew. If the daemon crashes, the
...
from codex_crew.env import resolve_krb5_ccname
from codex_crew.mcp_gateway import transport
from codex_crew.mcp_gateway.pool import READ_BUFFER_LIMIT_BYTES
from codex_crew.mcp_gateway.shutdown_budget import TOTAL_SHUTDOWN_BUDGET_SECS
from codex_crew.sandbox import _SENSITIVE_ENV_PREFIXES as _SANDBOX_SENSITIVE_ENV_PREFIXES
...
_GATEWAYD_MODULE = "codex_crew.mcp_gateway.gatewayd"
...
# (SLACK_BOT_TOKEN/APP/USER, CODEXCREW_OWNER_ID). config/loader.py seeds those
...
class GatewayManager:
    ...
    async def _spawn_once(self) -> None:
        ...
        # Capture gatewayd stdout/stderr to the canonical CodexCrew log path.
    ...
    def _required_target_stems(self) -> set[str]:
        ...
        for key in self._spec.mcp_target_env:
            for prefix in ("CODEXCREW_MCP_TARGET_", "MC_MCP_TARGET_"):
    ...
    def _adoption_drift(self, pong: dict) -> list[str]:
        ...
        silently removed ``codexcrew-core``'s entire tool surface from every
        session.
        """
