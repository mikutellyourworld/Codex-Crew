...
advertised the ``codexcrew.caller-identity`` capability (pooled operation),
...
from codex_crew.executors import image_executor, maintenance_executor
from codex_crew.mcp_caller import (
...
from codex_crew.mcp_gateway import hazards
from codex_crew.mcp_gateway.apps import (
    WithheldTools,
    append_marker,
...
from codex_crew.mcp_gateway.image_budget import (
    line_may_carry_image_block,
    parse_image_bearing_frame,
    rewrite_image_frame,
)
from codex_crew.mcp_gateway.pool import READ_BUFFER_LIMIT_BYTES, RESPONSE_SPILL_THRESHOLD_BYTES
from codex_crew.mcp_gateway.spill import maybe_spill_response
from codex_crew.mcp_gateway.tool_surface import ToolSurface, project_tool_surface
from codex_crew.security import redact
from codex_crew.sel import SecurityEventLog

if False:  # typing-only import guard
    from codex_crew.mcp_gateway.pool import PoolKey
...
#    "pool": "example-mcp::codexcrew::...", "pid": 12345, "ok": true}
#
...
# than this. However, shared backends (codexcrew-core) host legitimately long
...
# would kill a high-refcount codexcrew-core backend that is still answering
# fresh pings.
...
def _strip_caller_meta(msg: dict[str, Any]) -> dict[str, Any]:
    ...
    ``params._meta.codexcrew.caller`` / ``params._meta.codexcrew.tenant``
    ...
    ``_meta.codexcrew.caller`` in the request. This function is called on
    ...
    # ("codexcrew.caller") — exactly the shape build_caller_meta writes and
    # CallerContext.from_meta reads. Strip that flat key. (An earlier nested
    # "_meta[codexcrew][caller]" strip was a no-op against the real wire format,
...
MCP_APPS_ENV_FLAG = "CODEXCREW_MCP_APPS"
...
def _mcp_apps_enabled() -> bool:
    ...
    1. ``CODEXCREW_MCP_APPS`` off -> disabled. Absolute kill switch, for tests,
    ...
    Read per-call (``CodexCrewConfig.load`` is fingerprint-cached, so this is a
    ...
    try:
        ...
        gw = CodexCrewConfig.load().mcp_gateway
...
def _inject_caller_meta(msg: dict[str, Any], caller: CallerContext) -> dict[str, Any]:
    """Return a shallow copy of ``msg`` with ``params._meta.codexcrew.caller``
...
def _inject_tenant_meta(msg: dict[str, Any], nonce: str) -> dict[str, Any]:
    """Return a shallow copy of ``msg`` with ``params._meta.codexcrew.tenant``
...
class Backend:
    ...
    # :mod:`codex_crew.mcp_gateway.tool_surface` for why a snapshot must never
    # become one.
    ...
    async def forward_from_stub(
        ...
        *,
        ...
        3. Caller-identity injection — ``tools/call`` requests get a
           ``params._meta.codexcrew.caller`` block when the backend
           advertised the capability at initialize time. The block is
           built via :func:`codex_crew.mcp_caller.build_caller_meta` so
    ...
    async def _handle_initialize(
        ...
        # path — without this a stub could forge _meta.codexcrew.caller at init.
        forward_msg = _strip_caller_meta(msg)
        # MCP Apps: advertise the ui extension to the backend (no-op unless
        # the CODEXCREW_MCP_APPS flag is on). Must follow the strip so the
    ...
    async def probe_tool_surface(
        ...
        *,
        ...
        :func:`~codex_crew.mcp_gateway.tool_surface.project_tool_surface` can
        ...
        connection's ``_meta.codexcrew.tenant`` namespace instead of falling into
    ...
    async def run_stdout_pump(self) -> None:
        ...
        try:
            while True:
                ...
                # session (see codex_crew.imaging MAX_IMAGE_EDGE_PX). Two
    ...
    async def _maybe_intercept_ui_result(
        ...
        if pending.method == "tools/list":
            ...
            if isinstance(result, dict):
                ...
                # :mod:`codex_crew.mcp_gateway.tool_surface`.
    ...
    def _audit_visibility_withhold(
        ...
        in :mod:`codex_crew.mcp_gateway.app_call`) audits every outcome — so the
    ...
    async def _heartbeat_once(self, now: float) -> str:
        ...
        * ``"wedged"`` -- a stub is attached AND BOTH: (1) an in-flight request
          ...
          of ping freshness. Shared backends (codexcrew-core) host long tools
...
async def spawn_backend(
    ...
    1. ``command`` is taken verbatim from ``CODEXCREW_MCP_TARGET_<SERVER>`` env
       vars populated at CodexCrew startup by the rewriter from the user's
    ...
    spawn_env = dict(env)
    spawn_env[CODEXCREW_SPAWNED_ENV] = CODEXCREW_SPAWNED_VALUE
...
async def send_initialize(
    ...
    *,
    ...
    ``capabilities.experimental.codexcrew.caller-identity`` in the response.
    ...
    spawn for them (no cross-tenant injection of ``_meta.codexcrew.caller``).
    ...
    request = {
        ...
        "params": {
            ...
            "capabilities": {},
            "clientInfo": dict(client_info or {"name": "codexcrew-gateway", "version": "0"}),
        },
    }
...
async def _pump_stderr(reader: asyncio.StreamReader, label: str) -> None:
    ...
    while True:
        ...
        # stack trace) does not land verbatim in the CodexCrew log.
        logger.debug(
            "backend[%s] stderr: %s",
            label,
