...
pattern-matches ``[codexcrew-mcp-app:<id>]`` on the wire, reads the spool file
...
Everything here is invoked only behind the ``CODEXCREW_MCP_APPS`` feature flag
that lives in :mod:`codex_crew.mcp_gateway.backend`; this module has no flag of
...
from codex_crew.mcp_apps_render import MAX_SPOOL_BYTES, SPOOL_SCHEMA_VERSION

logger = logging.getLogger(__name__)

# Env override for the spool directory (tests point this at a tmp dir so they
# never touch a developer's real ~/.codex-crew). Read per-call, never cached.
SPOOL_ENV = "CODEXCREW_MCP_APPS_SPOOL"
...
MARKER_PREFIX = "[codexcrew-mcp-app:"
MARKER_SUFFIX = "]"
...
def spool_dir() -> Path:
    """Return the spool directory (``CODEXCREW_MCP_APPS_SPOOL`` override, else
    ``$CODEXCREW_HOME/mcp-apps``). Not created here — :func:`write_spool` does the
    ...
    reader) — both go through :func:`codex_crew.config.paths.config_dir` so a
    ``CODEXCREW_HOME`` override (pods, tests) moves writer and reader together.
