"""Warm-pool prewarming for the CodexCrew MCP gateway.
...
A PoolKey carries no channel dimension (see :mod:`codex_crew.mcp_gateway.pool`),
...
#: gateway socket (``$CODEXCREW_HOME/mcp-gateway/``) so it shares the
...
class HotKeyStore:
    ...
    def record_outcome(self, *, hit: bool) -> None:
        ...
        get_recorder().counter(
            "codexcrew.mcp.warm_pool.acquire",
