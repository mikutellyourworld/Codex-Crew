...
* It is redundant with a mechanism that already works. ``gatewayd`` stamps
  ``_meta.codexcrew.caller`` (session key, session type, principal, channel)
...
There is deliberately NO per-principal dimension either. Codex Crew is
...
here historically, but nothing ever populated its ``CODEXCREW_PRINCIPAL``
...
# ``CODEXCREW_MCP_READ_LIMIT``. Asyncio's stdlib default is 64 KiB which is
...
def _resolve_read_buffer_limit() -> int:
    ...
    Precedence: env var ``CODEXCREW_MCP_READ_LIMIT`` (bytes) → config key
    ...
    """
    raw = os.environ.get("CODEXCREW_MCP_READ_LIMIT")
    if raw:
        try:
            val = int(raw)
            if val >= 1024:
...
def _resolve_spill_threshold() -> int:
    ...
    Precedence: env var ``CODEXCREW_MCP_SPILL_THRESHOLD`` → config key
    ``mcp_gateway.response_spill_threshold_bytes`` → hard-coded default. Config
    read mirrors _resolve_read_buffer_limit.
    """
    raw = os.environ.get("CODEXCREW_MCP_SPILL_THRESHOLD")
    if raw:
        try:
            val = int(raw)
            if val >= 0:
