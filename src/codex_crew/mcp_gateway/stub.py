"""CodexCrew MCP stub — shim between kiro-cli and the gateway daemon.
...
record to ``$CODEXCREW_HOME/logs/stub_fallback.jsonl`` and ``execvpe``\u200bs
...
from codex_crew.executors import configure_default_executor, subprocess_executor
from codex_crew.jsonl_util import bounded_records, rotate_jsonl_at
from codex_crew.mcp_caller import CallerContext, _parent_pid
from codex_crew.mcp_gateway import transport
from codex_crew.mcp_gateway.hashing import hash_command, hash_effective_env
from codex_crew.mcp_gateway.pool import READ_BUFFER_LIMIT_BYTES, PoolKey
from codex_crew.metrics.events import MCP_RECONNECTS, emit_counter
...
def _default_socket_path() -> str:
    """Resolve the default gateway socket under CODEXCREW_HOME (0700 dir)."""
    home = _crew_home()
    new_path = home / "codexcrew-mcp-gateway.sock"
...
def _parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    :func:`codex_crew.mcp_gateway.rewriter.rewrite_agents`. ``--real-stub``
    ...
    p = argparse.ArgumentParser(
        prog="codexcrew-mcp-stub",
        description="CodexCrew MCP shim: proxies kiro-cli stdio to the local gateway",
    )
    ...
    p.add_argument(
        "--socket",
        default=os.environ.get("CODEXCREW_MCP_SOCKET") or os.environ.get("MC_MCP_SOCKET") or _default_socket_path(),
    )
...
def _resolve_channel_id(cli_value: Optional[str]) -> Optional[str]:
    """``--channel-id`` wins; else ``CODEXCREW_CHANNEL_ID`` env; else None."""
    return cli_value or os.environ.get("CODEXCREW_CHANNEL_ID") or None
...
def _ancestor_pids() -> list[int]:
    ...
    Uses :func:`codex_crew.mcp_caller._parent_pid`, which delegates to
...
def _build_caller_block(channel_id: Optional[str]) -> dict[str, str]:
    """Assemble caller-identity from ``CODEXCREW_*`` env vars.

    ``session_key`` is resolved via :meth:`CallerContext.from_env`, which
    reads ``CODEXCREW_SESSION_KEY`` first and falls back to the warm-pool PID
    ...
    # (A ``CODEXCREW_PRINCIPAL`` override existed historically but nothing ever
    # set it — Codex Crew is single-operator, so it was deleted.)
    principal = (
...
#: already changed across versions (``MC_MCP_TARGET_`` -> ``CODEXCREW_MCP_TARGET_``).
...
async def run_bridge(
    ...
    *,
    ...
    tasks = {
        asyncio.create_task(stdin_pump(), name="codexcrew-mcp-stub-stdin"),
        asyncio.create_task(stdout_pump(), name="codexcrew-mcp-stub-stdout"),
        asyncio.create_task(stop_event.wait(), name="codexcrew-mcp-stub-stop"),
        ...
        # check.
        asyncio.create_task(
            writer_failed_evt.wait(), name="codexcrew-mcp-stub-writer-failed"
        ),
    }
    ...
    if peer_supports_ping:
        tasks.add(
            asyncio.create_task(
                _liveness_monitor(), name="codexcrew-mcp-stub-liveness"
            )
        )
    try:
...
async def _amain(argv: Optional[list[str]] = None) -> int:
    ...
    _log_level = os.environ.get("CODEXCREW_MCP_LOG", os.environ.get("MC_MCP_LOG", "warning")).upper()
    ...
    while True:
        ...
        if not payload.get("session_key"):
            recaller_task = asyncio.create_task(
                ...
                name="codexcrew-mcp-stub-recaller",
            )
        try:
...
def _hard_exit(code: int) -> NoReturn:
    ...
    atexit-registered thread-pool joins in ``codex_crew.executors`` hold no
...
def main() -> None:
    """Sync entry point for ``python -m codex_crew.mcp_gateway.stub``."""
    try:
