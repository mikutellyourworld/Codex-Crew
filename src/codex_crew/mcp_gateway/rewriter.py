...
``CODEXCREW_SESSION_KEY`` stay unwrapped only because nothing lists them in the
stub allowlist.
...
from codex_crew import __version__, platform_compat
from codex_crew.atomic_write import atomic_write
from codex_crew.config.paths import config_dir
from codex_crew.env import mcp_search_path, spec_path_key
from codex_crew.mcp_gateway import STUB_MODULE
from codex_crew.mcp_gateway.hashing import hash_command, is_secret_env_key
from codex_crew.mcp_gateway.manager import is_credential_env_key
from codex_crew.mcp_utils import mcp_server_alias
from codex_crew.sandbox import scrub_agent_denied_env
...
# — a backend that does not advertise ``codexcrew.caller-identity`` in its
...
_WRAPPER_MARKER = "_codexcrew_mcp_gateway_wrapped"
...
#: The stub is launched as a module by the interpreter running CodexCrew.
...
#: that can import ``codex_crew`` is guaranteed.
#:
...
def _resolve_target_command(
    ...
    :func:`codex_crew.env.mcp_search_path` — literally the same composition the
...
def _build_stub_entry(
    *,
    ...
    wrapped.update({
        ...
        # ``-m codex_crew.mcp_gateway.stub`` leads; the stub's own flags follow.
    ...
    *,
    ...
    for name, entry in servers.items():
        ...
        if name in UNPOOLABLE_SERVERS:
            # Leave unchanged — these bind to CODEXCREW_SESSION_KEY.
            new_servers[name] = entry
            continue
        ...
        # would put filesystem IO behind every ``CodexCrewConfig.load()`` (244
    ...
    for name, entry in inject.items():
        ...
        # command from CODEXCREW_MCP_TARGET_<SERVER>, keyed only by server name with
        ...
        if target_env is not None:
            env_key = "CODEXCREW_MCP_TARGET_" + alias.replace("-", "_").upper()
...
def _rewrite_inputs_fingerprint(
    ...
    * ``path_env`` / ``pathext`` / ``path_augment`` — feed the
      ...
      :func:`codex_crew.env.mcp_search_path` over an empty spec PATH — the
...
def rewrite_agents(
    *,
    ...
    Codex Crew startup. When no input changed since the last completed run
    ...
    Args:
        ...
        sandbox_mode: Value from ``config.agent.sandbox`` — fed through
            so the stub's PoolKey matches CodexCrew's sandbox policy.
    ...
    Returns:
        ...
        * ``target_env``: mapping ``{CODEXCREW_MCP_TARGET_<SERVER>: "cmd arg arg"}``
    ...
    # bypasses the gateway unless wrapped (the "codexcrew-lite bypass" class of
...
def _collect_target_env(
    ...
    """Populate ``target_env`` with ``CODEXCREW_MCP_TARGET_<SERVER>`` entries
    for every wrapped server in ``mcp_servers``.

    Two kinds of entry are written per wrapped server:

    * ``CODEXCREW_MCP_TARGET_<SERVER>`` — first-wins across calls, kept as a
      backward-compatible fallback for any pool key whose
      ``command_args_hash`` has no disambiguated entry.
    * ``CODEXCREW_MCP_TARGET_<SERVER>__<command_args_hash>`` — one per distinct
    ...
    for server_name, entry in mcp_servers.items():
        if not isinstance(entry, dict) or not (
            ...
            continue
        env_key = "CODEXCREW_MCP_TARGET_" + server_name.replace("-", "_").upper()
        ...
        if target_cmd:
            ...
            if existing is not None and existing != spec:
                logger.warning(
                    "mcp-gateway rewriter: CODEXCREW_MCP_TARGET env-key collision on "
...
def default_overlay_dir() -> Path:
    """Return ``$CODEXCREW_HOME/mcp-gateway/agents`` (follows ``config_dir``)."""
...
def forward_declared_env_enabled() -> bool:
    ...
    try:
        ...
        return bool(CodexCrewConfig.load().mcp_gateway.forward_declared_env)
    except Exception:
...
def pool_identity_env_keys() -> frozenset[str]:
    ...
    try:
        ...
        declared = CodexCrewConfig.load().mcp_gateway.pool_identity_env or []
    except Exception:
