...
This package wires an asyncio local-IPC broker into CodexCrew as a
...
1. Writes rewritten kiro agent JSON into an overlay directory
   (``~/.codex-crew/mcp-gateway/agents/``) — never touches the user's
   ``~/.kiro/agents/`` files on disk.
2. Spawns ``python -m codex_crew.mcp_gateway.gatewayd`` at CodexCrew
...
#: ``<python> -m codex_crew.mcp_gateway.stub …``).
#:
...
STUB_MODULE = "codex_crew.mcp_gateway.stub"
...
if TYPE_CHECKING:  # names for static type checkers only
    from codex_crew.mcp_gateway.backend import Backend, send_initialize, spawn_backend
    from codex_crew.mcp_gateway.gatewayd import run_gatewayd
    ...
    from codex_crew.mcp_gateway.stub import main as stub_main
