"""Asyncio unix-socket server for the CodexCrew MCP gateway.

This module is the entry point for ``python -m
codex_crew.mcp_gateway.gatewayd`` and for in-process use by
:class:`codex_crew.mcp_gateway.manager.GatewayManager`.
...
from codex_crew.config.loader import config_dir as _config_dir
from codex_crew.executors import (
    configure_default_executor,
    maintenance_executor,
    subprocess_executor,
)
from codex_crew.mcp_caller import CallerContext
from codex_crew.mcp_caller import _parent_pid as _ppid_fn
from codex_crew.mcp_caller import new_tenant_nonce
from codex_crew.mcp_gateway import credwatch, hazards, socketsec, tool_surface, transport
from codex_crew.mcp_gateway.apps import sweep_spool as apps_sweep_spool
from codex_crew.mcp_gateway.backend import (
...
from codex_crew.mcp_gateway.breaker import CircuitBreaker
from codex_crew.mcp_gateway.hashing import hash_effective_env, non_secret_env
from codex_crew.mcp_gateway.manager import _scrub_sensitive_env, is_credential_env_key
from codex_crew.mcp_gateway.pool import (
    DRAIN_DEADLINE_SECS,
...
from codex_crew.mcp_gateway.prewarm import (
    HotKeyStore,
    default_hot_keys_path,
    prewarm_from_payloads,
)
from codex_crew.mcp_gateway.resolve_once import resolved_launch
...
from codex_crew.mcp_gateway.secret_uri import resolve_secret_uris
from codex_crew.mcp_gateway.shutdown_budget import DRAIN_SECS, POOL_SHUTDOWN_SECS
from codex_crew.mcp_gateway.spill import cleanup_old_spill_files
from codex_crew.mcp_gateway.stub import fallback_counts as stub_fallback_counts
from codex_crew.metrics.provider import get_recorder
from codex_crew.peer_resolve import resolve_peer_identity
from codex_crew.platform_compat import IS_WINDOWS
from codex_crew.platform_compat import count_open_fds as _shared_count_open_fds
from codex_crew.platform_compat import get_process_start_id as _get_process_start_id
from codex_crew.platform_compat import proc_rss_bytes as _proc_rss_bytes
from codex_crew.sandbox import _PYTHON_ENV_PREFIXES, warm_backend
...
def _emit_backend_acquire_metric(acquire_ms: float, *, warm: bool) -> None:
    """Emit codexcrew.mcp.backend.acquire.duration (best-effort).
    ...
    try:
        get_recorder().histogram(
            "codexcrew.mcp.backend.acquire.duration",
            acquire_ms,
            unit="ms",
            attrs={"warm": warm},
        )
...
def _emit_lazy_load_metrics(elapsed_ms: float, *, warm: bool) -> None:
    ...
    try:
        rec = get_recorder()
        rec.counter("codexcrew.mcp.lazy_load.count", attrs={"transport": "stdio"})
        rec.histogram(
            "codexcrew.mcp.lazy_load.duration",
            elapsed_ms,
            unit="ms",
...
# ``mcp_gateway.read_buffer_limit_bytes`` / ``CODEXCREW_MCP_READ_LIMIT``. Anything
...
_DEFAULT_SOCKET_SUBDIR = "codexcrew"
...
#: ``CODEXCREW_MCP_TARGET_<SERVER>`` env vars (accepts legacy ``MC_MCP_TARGET_<SERVER>``
...
def _default_cli_socket_path() -> Path:
    ...
    This is used ONLY when ``python -m codex_crew.mcp_gateway.gatewayd`` is
    invoked without an explicit ``--socket`` flag — a rare operator path,
    typically ad-hoc debugging. The CodexCrew production path always
    derives the socket from ``McpGatewayConfig.socket_path`` / the
    ``default_socket_path()`` in :mod:`codex_crew.mcp_gateway.rewriter`,
    which returns ``$CODEXCREW_HOME/mcp-gateway/gateway.sock``.

    Preference order for this CLI fallback:
    1. ``$XDG_RUNTIME_DIR/codexcrew/mcp-gateway.sock`` when XDG is set.
    2. ``$CODEXCREW_HOME``/config-dir ``/mcp-gateway/mcp-gateway.sock``.
    ...
    home = os.environ.get("CODEXCREW_HOME")
...
async def run_gatewayd(
    ...
    *,
    ...
    try:
        ...
        # window. Defense-in-depth on top of the 0700 $CODEXCREW_HOME directory;
...
def _declared_env_pairs(pool_key: PoolKey, identity_keys: Collection[str]) -> dict[str, str]:
    ...
    """
    try:
        overlay_dir = resolve_overlay_dir(CodexCrewConfig.load().mcp_gateway.overlay_dir)
    except Exception:
...
_TARGET_ENV_PREFIXES = ("CODEXCREW_MCP_TARGET_", "MC_MCP_TARGET_")
...
def resolvable_target_stems(env: Optional[dict[str, str]] = None) -> list[str]:
    ...
    suffix removed -- e.g. both ``CODEXCREW_MCP_TARGET_CODEXCREW_CORE`` and
    ``CODEXCREW_MCP_TARGET_CODEXCREW_CORE__61774e20...`` yield ``CODEXCREW_CORE``.
...
def env_target_resolver(pool_key: PoolKey) -> Optional[tuple[str, list[str], dict[str, str], str]]:
    """Look up ``CODEXCREW_MCP_TARGET_<SERVER>`` in the process env and return the
    spawn tuple, or ``None`` if no mapping is set.

    Wire format: ``CODEXCREW_MCP_TARGET_SLACK_MCP="slack-mcp --stdio"``.
    The server name is upper-cased with ``-`` replaced by ``_``. Env is
    inherited from the gateway process with ``CODEXCREW_CHANNEL_ID``
    ...
    :func:`codex_crew.mcp_gateway.manager._scrub_sensitive_env` so even if
    the gateway process somehow inherited credential vars, backends won't.
    """
    base = "CODEXCREW_MCP_TARGET_" + pool_key.server_name.upper().replace("-", "_")
    ...
    # Strip the Codex Crew process's own Python env vars so they don't leak into
    ...
    for key in _PYTHON_ENV_PREFIXES:
        env.pop(key, None)
    # No CODEXCREW_CHANNEL_ID is exported into the backend env. It used to be
    ...
    # _meta.codexcrew.caller (see _inject_caller_meta).
...
def _audit_peer_denied(reason: str) -> None:
    ...
    (:mod:`codex_crew.sel`) in addition to the WARNING log line. Wrapped
...
def _audit_peer_allowed(caller: str, pool_label: str) -> None:
    ...
    HMAC-chained security event log (:mod:`codex_crew.sel`) alongside the
...
def _audit_caller_rekey(caller: str, pool_label: str) -> None:
    ...
    connection from effectively unauthorized (no ``_meta.codexcrew.caller`` on
...
def _resolve_peer_identity(peer_pid: int) -> tuple[str, list[int]]:
    ...
    Delegates to the shared :func:`codex_crew.peer_resolve.resolve_peer_identity`
...
async def _handle_connection(
    ...
    *,
    ...
    if register.get("type") == "app-call":
        ...
        from codex_crew.mcp_gateway.app_call import _audit, handle_app_call
        from codex_crew.mcp_gateway.backend import _mcp_apps_enabled
    ...
    try:
        while True:
            ...
            # for subsequent forwards so ``_meta.codexcrew.caller`` carries the
    ...
    *,
    ...
    if target is None:
        raise _TargetUnknown(
            ...
            "set CODEXCREW_MCP_TARGET_<SERVER> env var or pass a target_resolver"
        )
    command, args, env, work_dir = target
...
class _ReplacementRefused(Exception):
    ...
    :func:`~codex_crew.mcp_gateway.tool_surface.describe_surface_change` already
...
def _zombie_diagnostic_path() -> Path:
    ...
    ``$CODEXCREW_HOME/logs/gatewayd_zombie_diagnostic.jsonl`` so a single
...
def _count_open_fds() -> int:
    ...
    ``codexcrew.process.open_fds`` gauge — so this diagnostic cannot drift
...
def _build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        description="CodexCrew MCP gateway daemon — pools MCP backends across sessions",
    )
    p.add_argument(
        "--socket",
        dest="socket",
        default=str(_default_cli_socket_path()),
        help="Unix socket path to bind. Default: $XDG_RUNTIME_DIR/codexcrew/mcp-gateway.sock",
    )
    p.add_argument(
        "--max-backends",
        dest="max_backends",
...
def main() -> None:
    """Sync entry point for ``python -m codex_crew.mcp_gateway.gatewayd``."""
    ...
    # is spawned directly as ``python -m codex_crew.mcp_gateway.gatewayd``), so its
    ...
    # cfg_dir lookup) and the blocking legacy→~/.codex-crew migration (copytree +
    ...
    from codex_crew.config.paths import ensure_data_home
