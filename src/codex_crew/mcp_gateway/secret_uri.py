...
against the local :class:`~codex_crew.secrets.SecretVault`. Resolution is
...
#: importer (``codex_crew.secrets.migrate``) writes references with this prefix
...
#: ``codex_crew.dashboard.handlers.secrets``). No character-class policy is
    ...
    for key, secret_name in pending.items():
        ...
        if secret_value is None:
            raise ValueError(
                ...
                f"migrate it with `codexcrew secrets import`)."
            )
