...
Deliberately NOT part of ``codexcrew-core``. Core is the tool surface EVERY
...
matching ``@codexcrew-dashboard`` reference in ``tools`` — kiro-cli loads a server
...
server's requests carry ``X-Internal-Caller: codexcrew-dashboard`` (attached
...
read the agent does not already have (``list_sessions`` in ``codexcrew-core`` is
...
each grants reach OUTSIDE Codex Crew — desktop input synthesis, the operator's
...
``codexcrew.caller-identity`` so gatewayd injects one; see
...
SERVER_NAME = "codexcrew-dashboard"
