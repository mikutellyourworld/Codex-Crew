...
Some CodexCrew MCP tools act on *the session that called them* — arm a monitor
...
(one ``codexcrew-core`` serves the whole runtime, and the ``/proc`` walk is
...
CodexCrew observing the tool CALL — as an MCP-served call whose CANONICAL name
...
# The MCP server name CodexCrew registers its own tools under (kiro-cli reports
...
# server name.)
CORE_MCP_SERVER = "codexcrew-core"
...
_SENTINEL = "[[CODEXCREW_SESSION_DIRECTIVE]]"
...
_REFUSAL_SENTINEL = "[[CODEXCREW_SESSION_DIRECTIVE_REFUSED]]"
...
_DEFANGED = "[[codexcrew-marker-removed]]"
...
def decode(text: str, expected_tool: str) -> dict[str, Any] | None:
    ...
    *expected_tool* (the name CodexCrew recorded for this tool call) matches the
...
def directive_tool_for(mcp_server_name: str, tool_name: str) -> str:
    ...
    served by Codex Crew's OWN core MCP server (:data:`CORE_MCP_SERVER`) AND its
