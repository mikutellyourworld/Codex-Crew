...
Runs as ``codexcrew mcp-core`` — kiro-cli spawns it as a child process
...
from codex_crew.agent_discovery import list_agents
from codex_crew.autonudge import binding_key_for, structured_monitor_binding_key_for
...
from codex_crew.dashboard.origin import dashboard_socket_path
from codex_crew.history import _SEARCH_SCAN_WINDOW as SEARCH_SCAN_WINDOW
from codex_crew.history import ConversationLog, is_incognito_transcript, snippet_needles
from codex_crew.knowledge.dedup import dedup_sweep
from codex_crew.knowledge.embedder import create_embedder_from_config
from codex_crew.knowledge.retrieval import HybridRetriever
from codex_crew.knowledge.store import KnowledgeStore
from codex_crew.loopback_http import loopback_urlopen
from codex_crew.mcp_caller import current_caller
from codex_crew.mcp_shared import (
    call_tool_with_logging,
    internal_caller,
    run_mcp_stdio_loop,
)
from codex_crew.mcp_tools import build_tool_list, dispatch
from codex_crew.members import record_activity
from codex_crew.messaging.link import is_legacy_slack_key, legacy_key
from codex_crew.platform import redact_via_context as redact
from codex_crew.port_resolution import port_is_gateway_owned, resolve_client_port_src
from codex_crew.security import (
    redact_credentials,
    redact_exfiltration_urls,
)
from codex_crew.sel import sel
from codex_crew.session_directive import refuse_if_markerless
...
# ``patch("codex_crew.mcp_core.sel")``, ``patch("codex_crew.mcp_core.time")`` --
...
def _resolve_api_port() -> tuple[int, str]:
    ...
    Delegates to :func:`codex_crew.port_resolution.resolve_client_port_src`, the
    same precedence every client CLI command applies: ``CODEXCREW_PORT``, then a
...
def _reverify_refused_target(refused_base: str) -> tuple[str, str] | None:
    ...
    * the port is not PROVEN to be held by this user's gateway. The source
      label alone cannot carry that proof: ``CODEXCREW_BOUND_PORT`` is
    ...
    that carries it does not hold there. ``--port`` / ``CODEXCREW_PORT`` users on
...
def _list_tools() -> list[dict[str, Any]]:
    ...
    Declared per domain under :mod:`codex_crew.mcp_tools`; this stays the
...
def _get_ppid(pid: int) -> int:
    ...
    - macOS: ``proc_pidinfo`` via libproc (see ``_ppid_via_libproc``). The old
      ...
      their parent session key (empty ``CODEXCREW_SESSION_KEY``) and surfacing
...
def _resolve_session_key() -> str:
    ...
    topology): gatewayd strips client-forged ``codexcrew.caller`` blocks and
    ...
    Warm-pool kiro-cli processes have no CODEXCREW_SESSION_KEY env var (the pool
    ...
    sk = os.environ.get("CODEXCREW_SESSION_KEY", "")
    if sk:
        return sk
    try:
        from codex_crew.session_pid_sig import read_session_pid_txt
        ...
        host_pid = os.environ.get("CODEXCREW_HOST_PID", "")
        if host_pid.isdigit():
...
def _resolve_session_key_strict() -> str:
    ...
    1. The gateway-injected ``CODEXCREW_SESSION_KEY`` env var.
    2. The direct ``CODEXCREW_HOST_PID`` -> ``session_pid_<pid>.txt``
       lookup, but ONLY when the HMAC sidecar written by the gateway
       verifies (:func:`codex_crew.session_pid_sig.verify_session_pid`).
       PID-namespace sandboxing strips ``CODEXCREW_SESSION_KEY`` from the
    ...
    client-forged ``codexcrew.caller`` block on every inbound frame and
    ...
    try:
        host_pid = os.environ.get("CODEXCREW_HOST_PID", "")
        if host_pid.isdigit():
            from codex_crew.session_pid_sig import verify_session_pid
...
def strict_identity_diagnosis(server: str = "codexcrew-core") -> str:
    ...
    if _resolve_session_key_strict():
        return ""
    if os.environ.get("CODEXCREW_HOST_PID", "").isdigit():
        ...
        return (
            ...
            f"verify. Check `codexcrew doctor` (trust root) — {server} does not need "
    ...
    return (
        ...
        f"a verifiable identity. `codexcrew doctor` reports the same check."
    )
...
#: the gate silently. Paths are relative to ``src/codex_crew``.
...
def require_strict_session_key(
    refusal: str, server: str = "codexcrew-core"
) -> tuple[str, str]:
    ...
    caller context, ``CODEXCREW_SESSION_KEY``, or the HMAC-verified host-pid
...
def _deny_channel_agent_messaging(caller_session: str, tool_name: str) -> str | None:
    ...
    an AUTO-APPROVED call (codexcrew-core is in the default ``allowedTools``)
    ...
    if not caller_session.startswith("channel:"):
        return None
    try:
        # Resolved from ``codex_crew.sel`` at call time, not through the
...
def _vet_messaging_governance(
    ...
    ``codexcrew-core`` stdio subprocess, which DOES boot the platform via
    ...
    try:
        ...
        # No logger here: this runs inside the codexcrew-core stdio MCP server,
        ...
        try:
            from codex_crew.platform.governance_profiles import audit_governance_degraded
...
def _vet_browse_governance(caller_session: str) -> str | None:
    ...
    on a default install). Runs inside the ``codexcrew-core`` stdio subprocess,
...
def _governance_app() -> str:
    ...
    An app backend process carries ``CODEXCREW_APP_NAME`` (set in
    ...
    (``bind:{type:"app"}``) resolve.  NOTE: the managed ``codexcrew-core`` MCP
    ...
    """
    return os.environ.get("CODEXCREW_APP_NAME", "")
...
def _refused_message(refused_base: str, exc: BaseException) -> str:
    ...
    return (
        f"Codex Crew gateway not reachable on {host} — it may be down or "
...
def _http_error_body(exc: urllib.error.HTTPError) -> dict:
    ...
    An HTTP response body is content originating outside CodexCrew, so the
    ...
    if code == "internal_auth_mismatch":
        ...
        message = (
            "this client authenticated against the wrong Codex Crew instance. "
...
def _current_session_thread_ts() -> str | None:
    ...
    ``CODEXCREW_HOST_PID`` lookup. It deliberately drops the ``/proc`` ancestor
...
def _call_tool(name: str, raw_args: dict[str, Any]) -> str:
    ...
    return refuse_if_markerless(
        ...
        call_tool_with_logging(
            ...
            downstream_service="codexcrew-core",
        ),
    )
...
def _history_is_incognito(meta: dict) -> bool:
    ...
    Dict-shaped convenience over :func:`codex_crew.history.is_incognito_transcript`,
...
def _crew_machine_markers() -> list[tuple[str, str]]:
    ...
    Longest-first matters: the Codex Crew home normally sits inside the user's
    home, so scrubbing the home first would leave ``<home>/.codex-crew/...`` —
    still a directory layout — instead of collapsing the whole prefix.
    """
    markers: list[tuple[str, str]] = []
    for value, placeholder in (
        (str(config_dir()), "<codexcrew-home>"),
        (str(Path.home()), "<home>"),
...
def _call_tool_inner(name: str, args: dict[str, Any]) -> str:
    ...
    Handlers live beside their descriptors in :mod:`codex_crew.mcp_tools`; this
...
#: Whether this server advertises ``codexcrew.caller-identity`` — i.e. whether it
...
def run_mcp_core_server() -> None:
    ...
    run_mcp_stdio_loop(
        ...
        _list_tools,
        _call_tool,
        # Pooled-operation opt-in: codexcrew-core consumes the per-call
        # ``codexcrew.caller`` identity (see _resolve_session_key*), so it is
        # safe to share one backend across sessions. All four managed servers
        # advertise today -- codexcrew-cron since #4622, codexcrew-computer and
        # codexcrew-dashboard since #4659 -- and what advertising buys is the
