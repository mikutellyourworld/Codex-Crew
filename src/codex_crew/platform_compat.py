...
from codex_crew import windows_acl
...
def _ensure_utf8_process_environment() -> None:
    ...
    streams are constructed before Codex Crew code runs, so the encoding must be
    ...
    deliberately: Codex Crew's process tree emits Unicode as part of its normal
...
#
# Nothing CodexCrew does at startup needs these folders: they are only ever
...
def ensure_utf8_console() -> None:
    """Keep Codex Crew's process tree UTF-8 and repair current Windows streams.

    CodexCrew prints non-ASCII glyphs throughout its CLI/gateway output. On
    ...
    CodexCrewHub client) Python encodes prints as cp1252 — so the FIRST non-ASCII
    ...
    for name in ("stdout", "stderr"):
        ...
        try:
            ...
            # observed in the 3-layer Windows gateway spawn: codexcrew.exe launcher
...
def probe_file_persistence(directory: Path) -> str | None:
    """Verify that *directory* supports every primitive the Codex Crew
...
def _log_tool_outside_trusted_dirs(name: str, directories: tuple[str, ...]) -> None:
    ...
    tool they already have, and ``codexcrew stop`` would quietly no-op. The
...
def find_port_listeners(port: int) -> list[PortListener]:
    ...
    # silently drops IPv6 listeners entirely, so `codexcrew stop` /
    # `codexcrew restart` no-op on a dual-stack or `[::]`-bound gateway
...
def process_command_line(pid: int) -> str:
    ...
    Used to confirm a listener PID is actually a CodexCrew gateway when the image
    name alone is ambiguous (the venv ``codexcrew.exe`` re-execs ``python.exe``).
    """
    try:
        if sys.platform == "linux":
            raw = Path(f"/proc/{pid}/cmdline").read_bytes()
...
async def kill_pid_async(pid: int, sig: int = SIGTERM) -> bool:
    ...
    :func:`codex_crew.executors.subprocess_executor` (the same bounded pool the
...
async def kill_process_tree_async(pid: int, sig: int = SIGTERM) -> bool:
    ...
    off the loop via :func:`codex_crew.executors.subprocess_executor`. Raises
...
async def kill_and_reap(proc: asyncio.subprocess.Process, *, timeout: float | None = None) -> None:
    ...
    async def _cleanup() -> None:
        ...
        if not _shares_own_process_group(proc.pid):
            ...
            # call time, so tests patching ``codex_crew.platform_compat.
...
def stat_writable_by_current_user(st: os.stat_result) -> bool:
    ...
    (``platform/policy_distribution.py``): a distribution source the account Codex Crew
...
# POSIX execute bit). A hook is a small script CodexCrew shells out to; on
...
def is_executable_file(
    ...
    *,
    ...
    determined when CodexCrew actually invokes it, not by a meaningless bit.
...
def find_python_interpreter(reject: Optional[Callable[[str], bool]] = None) -> str | None:
    ...
    need *an* interpreter to re-exec CodexCrew itself should prefer
...
def count_open_fds() -> int | None:
    ...
    The one shared probe behind both the ``codexcrew.process.open_fds`` gauge
