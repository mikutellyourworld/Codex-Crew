...
from codex_crew.agent import _prompt_path
from codex_crew.agent_discovery import agent_skill_globs
...
from codex_crew.cron import get_local_tz
...
from codex_crew.members import (
    MemberLifecycle,
    MemberSlugError,
...
from codex_crew.quick_prompts import expand_quick_prompt
...
from codex_crew.session_surface import has_dashboard_surface
...
def _member_backend_can_dispatch(cfg: "CodexCrewConfig | None" = None) -> bool:
...
# Docs directory bundled inside the codex_crew package
...
_RUNTIME_DISPLAY = {
    ...
    "cron": "CodexCrew cron job",
    "subagent": "CodexCrew subagent",
    "taskrunner": "CodexCrew task runner",
    "background": "CodexCrew background",
    "heartbeat": "CodexCrew heartbeat",
    "cli": "CLI terminal",
    "slack": "Slack",
    "discord": "Discord",
    "telegram": "Telegram",
...
def _build_docs_section() -> str:
    ...
    return (
        "[DOCUMENTATION]\n"
        f"CodexCrew docs: {_BUNDLED_DOCS_DIR}\n"
        "\n"
        "For CodexCrew behavior, commands, config, or architecture: "
        "consult local docs first.\n"
        "When diagnosing issues, run `codexcrew status` or "
        "`codexcrew doctor` yourself when possible.\n"
        "[END DOCUMENTATION]\n\n"
    )
...
def _role_description(cfg: "CodexCrewConfig") -> str:
...
def _build_user_profile_section(cfg: "CodexCrewConfig") -> str:
...
def ui_language_tag(cfg: "CodexCrewConfig") -> str:
...
def _build_ui_language_section(cfg: "CodexCrewConfig") -> str:
...
def _agent_includes_crew_context(agent: str | None) -> bool:
    ...
    Opt-out, defaulting to inject. The built-in ``codexcrew`` agent and an empty
    ...
    """
    if not agent or agent == "codexcrew":
        return True
...
async def compress_thread_history(
    ...
    *exclude_last_n* is forwarded to ``conversation_log.recent`` to drop
    ...
    """
    from codex_crew.llm_helpers import (  # circular import
        background_turn,
        stream_and_collect,
    )
    ...
    try:
        async with background_turn(
            sessions, task="thread_compress", agent="codexcrew-lite"
        ) as client:
...
def build_session_replay(
    ...
    *,
    ...
    """Build session replay from CodexCrew's conversation_log.
...
def _skills_injection_plan(agent: str | None, *, is_cc: bool) -> tuple[bool, list[str]]:
    ...
    is_custom = bool(agent) and agent != "codexcrew"
    ...
    *marks* is an ordered list of ``(label, monotonic)`` checkpoints. The first
    ...
    try:
        ...
        for label, ms in timings.items():
            recorder.histogram(
                "codexcrew.context.section.duration",
                ms,
                unit="ms",
...
class ContextBuilder:
    ...
    def __init__(
        ...
        if bot_name:
            ...
            self._bot_name = "CodexCrew" if is_claude_code(provider) else "Kiro"  # brand-ok
    ...
    def _resolve_prompt_templates(prompt: str, session_key: str) -> str:
        ...
        if "{{MAX_SUBAGENTS}}" in prompt:
            # Lazy import: codex_crew.subagent imports this module, so a
            # top-level import would cycle.
            try:
                from codex_crew.subagent import (  # circular import: subagent -> context
                    resolve_max_subagents,
                )

                cap = resolve_max_subagents(CodexCrewConfig.load())
            except Exception:
                cap = 0
        ...
        if density == "more":
            widget_block = (
                ...
                "`@codexcrew-core/artifact_save` one you rendered. The user's "
            ...
            widget_block = (
                ...
                "`@codexcrew-core/artifact_save` one you rendered. Load the "
    ...
    def _build_member_section(self, member: str) -> str:
        ...
        :class:`~codex_crew.members.MemberRulesUnreadable` and ABORTS the
        ...
        try:
            ...
            return ""
        try:
            crew = CodexCrewConfig.load().agents.get(member)
        except Exception:
            crew = None
    ...
    def build_session_context(
        ...
        *context_groups* selects which switchable groups are injected (see
        ...
        For custom agents (non-codexcrew), skills and workspace identity are
        ...
        """
        is_custom = agent and agent != "codexcrew"
        ...
        if minimal_context:
            ...
            agent_label = agent or "codexcrew"
            ...
            parts.append(_build_ui_language_section(CodexCrewConfig.load()))
            logger.debug(
        ...
        if is_custom:
            ...
            logger.debug("Building session context for codexcrew agent")
        ...
        # links). These are the built-in codexcrew assistant's dashboard/Slack UI
        ...
        # and injecting the codexcrew assistant's mandates on top both
        ...
        # Honour CodexCrewConfig.timezone (e.g. "Asia/Tokyo") so the LLM sees
        ...
        from codex_crew.members import DM_SLOT_MODE as _member_mode
        ...
        # delivery branch consults (codex_crew.members.member_turn_context).
        ...
        # Workspace identity — codexcrew-only (custom agents don't use workspaces)
        if not is_custom:
        ...
        # Documentation pointer — codexcrew-only, lightweight reference
        ...
        #    CodexCrew injects the mapped set itself, scoped by ``only=``.
        # 2. No mapping + the codexcrew agent -> the whole catalog (unchanged).
        ...
        # Mapped: CC only (kiro loads them natively). Unmapped: codexcrew only.
            ...
            if lessons_ctx:
                if len(lessons_ctx) > caps.lessons:
                    ...
                    parts.append(
                        ...
                        "also suggest they run `codexcrew learn remove <substring>` from "
                        "their terminal.\n"
    ...
    def build_message(
        ...
        *,
        ...
        # chokepoint — codex_crew.members.member_turn_context — which every
        ...
        if is_new_session:
            ...
            if slim_resume:
                ...
                # CC gets the SAME CodexCrew persona prompt as kiro — including
            ...
            # history from CodexCrew's conversation_log — provider-agnostic.
            if compressed_history:
                parts.append(
        ...
        if not minimal_context:
            try:
                from codex_crew.resource_status import probe as _probe_resources
