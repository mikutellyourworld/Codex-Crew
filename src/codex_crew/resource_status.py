...
* the **pressure-gated context line** injected in
  :meth:`codex_crew.context.ContextManager.build_message` — a compact
...
The memory figure reuses :func:`codex_crew.subagent._available_memory_gb`, the
...
def _read_available_gb() -> float:
    ...
    Lazily reuses :func:`codex_crew.subagent._available_memory_gb` (the same
    ...
    """
    try:
        from codex_crew.subagent import _available_memory_gb
...
def probe(cfg: object | None = None) -> ResourceStatus:
    ...
    *cfg* is an optional pre-loaded ``CodexCrewConfig``; when omitted it is
