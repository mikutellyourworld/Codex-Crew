...
at all? The public baseline registers Codex and the OpenAI-compatible adapter;
an edition plugin adds its own from
``ProviderRegistry.register_acp_backends`` by calling
:func:`register_selectable_backend`, the structural twin of
``publish_provider.register_provider``.

A LEAF module on purpose. ``codex_crew/acp/__init__.py`` imports the ACP client and
runtime, so reaching ``codex_crew.acp.types`` executes that package init and lands
...
of the three could import the others. Nothing here imports ``codex_crew.acp``,
``codex_crew.config`` or ``codex_crew.platform``, so all three now derive from this
...
``CodexCrewConfig.load()``, re-enters that load and recurses. Keeping this module
...
ACP_BACKEND_OPENAI_COMPATIBLE = "openai_compatible"
...
#: :mod:`codex_crew.agent_sdk.backend_install` probes for the two binaries and the
...
DEFAULT_ACP_BACKEND = ACP_BACKEND_CODEX
...
POLICY_ID_BY_BACKEND: dict = {
    # Every known id needs an entry: a policy author has to be able to name and
    # deny any id this build can spell, and the mapping is what makes the id
    # nameable in a rule at all.
    ACP_BACKEND_CODEX: ACP_BACKEND_CODEX,
    ACP_BACKEND_OPENAI_COMPATIBLE: ACP_BACKEND_OPENAI_COMPATIBLE,
}
...
#:
#: Codex is the public product floor and default. Its public ACP adapter includes
#: a compatible Codex CLI, so one installable component provides the complete
#: harness while keeping the OpenAI agent as the stable recovery choice.
GOVERNANCE_FLOOR_BACKEND: str = DEFAULT_ACP_BACKEND
...
def resolve_selected_backend(value: object) -> str:
    ...
    Runs inside ``CodexCrewConfig.load()``, so it must stay free of anything that
    ...
    if isinstance(value, str) and value in selectable:
        return value
    if value not in (None, DEFAULT_ACP_BACKEND):
        logger.warning(
...
ACP_BACKENDS_SESSION_SHARING: FrozenSet[str] = frozenset()
...
# direction.
ACP_BACKENDS_MEMBER_DISPATCH: FrozenSet[str] = frozenset()
...
ACP_BACKENDS_STEER: FrozenSet[str] = frozenset()
...
# status unchanged.
ACP_BACKENDS_COMPACT: FrozenSet[str] = frozenset()

# Backends carrying their OWN internal OS sandbox, which on macOS cannot nest
# inside Codex Crew's seatbelt (kernel EPERM) — so ``sandbox.wrap_argv`` skips
...
# a layer that does not exist. See :mod:`codex_crew.acp.kas_transport`.
#
...
# nest inside.
ACP_BACKENDS_INTERNAL_SANDBOX: FrozenSet[str] = frozenset()
...
ACP_BACKENDS_ACP_RUNTIME: FrozenSet[str] = frozenset()
...
# --auth-method cli`` (see :mod:`codex_crew.acp.kas_transport`), and that
...
# no reason.
ACP_BACKENDS_KIRO_IDENTITY_STORE: FrozenSet[str] = frozenset()
...
ACP_BACKENDS_MODEL_VIA_CONFIG_OPTION = frozenset({ACP_BACKEND_CODEX})
...
# never advertised.
ACP_BACKENDS_EFFORT_VIA_CONFIG_OPTION = frozenset({ACP_BACKEND_CODEX})
...
ACP_BACKENDS_ADVERTISED_MODEL_SELECTION: FrozenSet[str] = frozenset()
...
ACP_BACKENDS_SEED_LOCAL_SETTINGS: FrozenSet[str] = frozenset()
...
# this load-path leaf free of a ``codex_crew.model_registry`` dependency.
_MODEL_REGISTRY_NAMESPACE_BY_BACKEND: dict = {
    ACP_BACKEND_CODEX: "acp",
    ACP_BACKEND_OPENAI_COMPATIBLE: "acp",
}
...
ACP_BACKENDS_KIRO_SLASH_COMMANDS: FrozenSet[str] = frozenset()
...
# with nothing red to tell them why. :mod:`codex_crew.mcp_hot_reload` owns the
...
# ``session/new`` (:mod:`codex_crew.acp.kas_agents`), so nothing on disk
...
ACP_BACKENDS_MCP_CONFIG_HOT_RELOAD: FrozenSet[str] = frozenset()


# ── How a harness is made to ask ──
# Codex Crew's PreToolUse gate -- the bundled denied-command rules, the
...
class Routing(str, Enum):
    ...
    enforced value makes privileged tools ask. Codex Crew verifies the option is
    ...
    ``UNVERIFIED`` -- Codex Crew has NOT established how, or whether, this harness
...
ACP_BACKEND_ROUTING: dict = {
    ...
    ACP_BACKEND_OPENAI_COMPATIBLE: Routing.UNVERIFIED,
}
