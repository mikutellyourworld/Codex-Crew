...
Commands:
  ...
  /codexcrew dashboard [<N>h|<N>m] — send a presigned dashboard login link
...
from codex_crew.messaging.conversation import ConversationState  # noqa: F401
...
# Two-token command: ``/codexcrew dashboard [<TTL>]``. The bare ``/codexcrew``
# token is deliberately NOT a command on its own (see parse_command).
_DASHBOARD_ALIASES = frozenset(("/codexcrew",))
...
def _strip_bot_mention(cmd: str, bot_username: str) -> str:
    ...
    ``/new@CodexCrewBot`` instead of bare ``/new``. This is standard,
...
def parse_command(text: str, bot_username: str = "") -> str | None:
    ...
    *bot_username* (this bot's own registered username, from ``getMe``) gates
    ...
    if cmd in _STOP_ALIASES:
        return "stop"
    # /codexcrew dashboard [<TTL>] -- requires the explicit "dashboard"
    # subcommand, so a bare "/codexcrew" (typo or menu tap) falls through as
...
def parse_dashboard_argument(text: str) -> str:
    """Return the argument of ``/codexcrew dashboard [<N>h|<N>m]`` (``""`` if none).
    ...
    :func:`codex_crew.messaging.commands.parse_dashboard_ttl`, which takes this
...
#: documented in the help card's footer instead. ``/codexcrew`` is absent for
...
_HELP_HEADER = "🦞 Codex Crew — Telegram"
_HELP_FOOTER = (
