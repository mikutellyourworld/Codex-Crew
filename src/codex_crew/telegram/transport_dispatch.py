...
(channel-neutral) so this module never imports ``codex_crew.slack``.
"""
...
from codex_crew.config.loader import ACTIVATION_MENTION, ACTIVATION_OFF
from codex_crew.executors import run_in_embed_pool
from codex_crew.history import mint_row_mid
from codex_crew.hooks import TOOL_AUTO_APPROVE, TOOL_DENY
from codex_crew.messaging import auto_title, privacy_mode
from codex_crew.messaging.attachments import IngestLimits, append_attachment_context
from codex_crew.messaging.attachments import cleanup as cleanup_attachments
...
from codex_crew.messaging.dispatch import (
    build_auto_approve,
    build_directive_consumer,
    delivery_is_muted,
)
from codex_crew.messaging.driver import APPROVAL_INTERACTIVE, TurnDriver
...
from codex_crew.messaging.session_resume import (
    ResumeReleaseError,
    RoutingDecision,
    persisted_session_agent,
)
from codex_crew.messaging.session_trust import add_trusted_session, is_session_trusted
from codex_crew.messaging.transport import InboundMessage
from codex_crew.messaging.upload_gate import (
    session_blocks_reads,
    session_is_restricted,
    uploads_restricted,
)
from codex_crew.safety_override import safety_override
from codex_crew.security import redact, redact_local_paths
...
from codex_crew.telegram.attachments import process_telegram_attachments
...
from codex_crew.telegram.session_resume import TelegramSessionResume
from codex_crew.telegram.transport import (
    TELEGRAM_CAPABILITIES,
    TelegramInboundMessage,
    forum_gate_outcome,
)
from codex_crew.voice_reply import synthesis_settings, synthesize_and_deliver
...
if TYPE_CHECKING:
    ...
    from codex_crew.telegram.client import TelegramCallback, TelegramClient

from codex_crew.messaging.queue_receipt import MAX_COLLAPSE as _MAX_COLLAPSE
from codex_crew.messaging.queue_receipt import STEER_ACK_EMOJI as _STEER_ACK_EMOJI
from codex_crew.messaging.queue_receipt import (
    ReceiptQueue,
    ReceiptSurface,
)
...
# Canonical kiro-cli agent fallback so Telegram sessions load codexcrew-core
...
# path's _DEFAULT_CODEXCREW_AGENT.
_DEFAULT_CODEXCREW_AGENT = "codexcrew"
...
def _mention_re(handle: str) -> "re.Pattern[str]":
    ...
    ``(?![A-Za-z0-9_])`` is the whole point: without it `@codexcrewbot` matches
    inside `@codexcrewbot2`, so a message aimed at another bot in the same Topic
...
class TelegramDispatcher:
    ...
    def __init__(
        ...
        *,
        ...
        # codex_crew.telegram.commands._strip_bot_mention for why an unset
        ...
        *origin_tag* is the posting session encoded into an ``[OPTIONS:]``
        ...
        try:
            ...
            driver = TurnDriver(
                ...
                audit_agent=agent or "codexcrew",
            ...
            try:
                ...
                # below.
                from codex_crew.dashboard.channel_slots import project_channel_turn_live
            ...
            if is_new_own_session:
                try:
                    ...
                    from codex_crew.dashboard.channel_slots import (
    ...
    async def _handle_dashboard(
        ...
        Mirrors the Slack ``/codexcrew dashboard`` implementation: calls
        ``generate_token`` directly (never via shell) and builds the URL from
        the ``dashboard.url`` config (``CODEXCREW_PORT`` overrides the port,
        ...
        from codex_crew.dashboard.urls import dashboard_origin, parse_dashboard_url
        ...
        try:
            ...
            if not origin:
                ...
                # (parse_dashboard_url applies the CODEXCREW_PORT override).
            ...
            logger.warning("telegram /codexcrew dashboard: token generation failed", exc_info=True)
            try:
                sel().log_api_access(
                    caller=str(user_id),
                    operation="telegram.dashboard_token",
    ...
    async def _handle_stop(
        ...
        *,
        ...
        :func:`~codex_crew.messaging.commands.stop_running_turn`; this supplies
    ...
    async def _handle_yolo(
        ...
        :func:`~codex_crew.messaging.commands.run_yolo_command`. Reachable only by
    ...
    def _addresses_this_bot(self, msg: InboundMessage) -> bool:
        ...
        * the message replies to one sent by this bot, matched on ``bot_id``.
        ...
        # makes it exact — Telegram usernames extend one another (`@codexcrewbot`,
        # `@codexcrewbot2`, `@codexcrewbot_dev` may all sit in one Topic), and an
        ...
        # `_strip_bot_mention` uses, so `@codexcrewbot` does not match inside
        # `@codexcrewbot2`.
    ...
    async def _blocks_memory_reads(self, session_key: str) -> bool:
        ...
        """
        from codex_crew.dashboard.handlers._shared import _probe_persisted_session
    ...
    async def _session_restricted(self, session_key: str) -> bool:
        ...
        (:func:`codex_crew.messaging.upload_gate.session_is_restricted`), asked here
    ...
    async def _uploads_restricted(self, session_key: str) -> bool:
        ...
        :func:`codex_crew.messaging.upload_gate.uploads_restricted`, shared with the
    ...
    async def _handle_title(
        ...
        *,
        ...
        try:
            ...
            from codex_crew.dashboard.channel_slots import rename_channel_title_live
    ...
    def _configured_agent(self) -> str:
        ...
        return self.agent or self.cfg.agent.default_agent or _DEFAULT_CODEXCREW_AGENT
    ...
    async def _require_direct_chat(
        ...
        *,
        ...
        were never allow-listed at all. Same rule `/codexcrew dashboard` follows.
    ...
    def _bind_origin_mirror(self, session_key: str, route: tuple[str, str], chat_id: int) -> None:
        ...
        :func:`~codex_crew.messaging.link.bind_origin_mirror`, shared with the
    ...
    async def _handle_link(
        ...
        *,
        ...
        :func:`~codex_crew.messaging.link.rebind_conversation_location`, the
