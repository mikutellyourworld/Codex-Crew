...
Presence) are toggled in the Developer Portal, never in Codex Crew's config, so
the two can disagree without anything on the Codex Crew side noticing. A thread
...
This is the Discord analogue of :mod:`codex_crew.slack.scope_probe`, and it
...
* **Never fatal.** Every failure path (no token, rejected token, HTTP error,
  ...
  reason instead of raising, because its caller is ``codexcrew doctor``, and a
...
caller's own event loop (``codexcrew doctor`` gives it a throwaway
``asyncio.run``), so it never borrows the gateway's loop or the live
:class:`~codex_crew.discord.client.DiscordClient` session, whose REST ladder
