...
from codex_crew.discord.attachments import (
    append_attachment_context,
    process_discord_attachments,
)
from codex_crew.discord.commands import (
    ConversationState,
    build_help_text,
...
from codex_crew.discord.renderer import (
    DiscordApprovalDecider,
    DiscordRenderer,
    build_model_components,
    session_provenance_tag,
)
from codex_crew.discord.session_resume import (
    DiscordSessionResume,
    ResumeReleaseError,
    RoutingDecision,
)
from codex_crew.discord.transport import DISCORD_CAPABILITIES
...
from codex_crew.messaging.renderer import Renderer, SilentRenderer
from codex_crew.messaging.session_resume import persisted_session_agent
from codex_crew.messaging.transport import InboundMessage
from codex_crew.messaging.upload_gate import session_is_restricted, uploads_restricted
from codex_crew.monitoring.completion import MonitorCompletionHook
from codex_crew.monitoring.models import MonitorDispatchResult
from codex_crew.safety_override import describe_grant_lifetime, safety_override
from codex_crew.security import redact_credentials, redact_exfiltration_urls
from codex_crew.sel import sel
from codex_crew.session import SessionBusyError
...
if TYPE_CHECKING:
    ...
    from codex_crew.discord.client import DiscordClient, DiscordInteraction
...
# Canonical kiro-cli agent fallback so Discord sessions load codexcrew-core
...
class DiscordDispatcher:
    ...
    async def handle_message(
        ...
        *,
        ...
        (see :func:`~codex_crew.discord.renderer.session_provenance_tag`). When
    ...
    def _render_config(self) -> tuple[bool, bool]:
        ...
        """
        try:
            discord_cfg = CodexCrewConfig.load().discord
    ...
    async def _handle_link(
        ...
        live in the shared
        :func:`~codex_crew.messaging.link.rebind_conversation_location`; this
    ...
    async def _surface_own_session(self) -> None:
        ...
        from codex_crew.dashboard.channel_slots import surface_dispatcher_session
    ...
    async def _handle_status(self, reply: "ReplyFn") -> None:
        """Report runtime stats, from the same source Slack's ``/codexcrew status`` uses.
