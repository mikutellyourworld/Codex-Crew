...
permissions Codex Crew never uses). The named bits below are therefore the
...
``src/codex_crew/docs/discord-integration.md`` documents, and
...
def build_install_url(client_id: str, *, dm_only: bool = False) -> str:
    ...
    a URL Codex Crew printed must be able to trust every parameter in it.
    """
