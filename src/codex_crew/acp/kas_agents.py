...
holding ``tools: ["@codexcrew-core", ...]`` and no definition of what
``codexcrew-core`` is — refs naming nothing, and every Crew tool silently absent.
...
:mod:`codex_crew.acp.kas_permissions` for the mapping and for why an entry it
...
from codex_crew.acp.kas_permissions import allowed_tools_to_permissions
...
#: Codex Crew's OWN managed MCP servers — the ones whose ``env`` may retain a
#: single Crew-authored key through projection (see :func:`_project_mcp_servers`).
#:
#: Imported from :mod:`codex_crew.mcp_cleanup`, which already pins this set to
...
MANAGED_MCP_SERVER_NAMES = frozenset(CODEXCREW_BIN_MCP_SERVERS)
...
_MANAGED_ENV_KEYS_KEPT = frozenset({"CODEXCREW_HOME"})
...
_WRAPPER_MARKERS = ("_codexcrew_mcp_gateway_wrapped", "_mc_mcp_gateway_wrapped")
...
#: :mod:`codex_crew.acp.kas_permissions` translates it into ``permissions``, so
...
#: agent that ships ``"prompt": ""`` (today only Crew's ``codexcrew-lite``, but
...
You are a Codex Crew lightweight background worker. You are dispatched by the
...
def resolve_prompt(
    ...
    * The resolved path must not be a credential/governance location or a
    ...
    spec with no prompt (Crew's own utility agents such as ``codexcrew-lite``
...
def _project_mcp_servers(
    ...
    * **``env`` and ``headers``** — projection puts these on the wire, and a
      ...
      ``CODEXCREW_HOME`` out of its env and nothing else, because that key is the
...
def to_client_custom_agent(
    ...
    *member_dispatch* widens the projection for a crew member's DM session:
    ``@codexcrew-dashboard`` joins ``tools`` (the server itself arrives as a
    ...
    if member_dispatch:
        ...
        # ``"*"`` already covers it.
        tools = out["tools"]
        if isinstance(tools, list) and "@codexcrew-dashboard" not in tools:
            out["tools"] = [*tools, "@codexcrew-dashboard"]
...
def load_agent_spec(agents_dir: Path, agent_id: str) -> dict[str, Any]:
    ...
    stays free of :mod:`codex_crew.agent`, which imports the config loader and
...
def build_kas_custom_agents(
    ...
    *,
    ...
    A prompt-less spec (e.g. ``codexcrew-lite``) is projected with the small
