...
Codex Crew holds two different views of MCP, and reading one as the other is the
...
spec's own servers as well as the ones Codex Crew injects on the wire, so a name
...
:mod:`codex_crew.acp.runtime`: those bound an exception string built from
...
from codex_crew.acp._dispatch import classify_notification
...
class McpSessionReport:
    ...
    #: Server names Codex Crew put on the wire in this session's ``session/new``.
    #: Empty means Codex Crew injected none — NOT that the session has none, since
