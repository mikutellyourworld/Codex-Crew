...
existing ``from codex_crew.acp.runtime import AcpSessionHandle`` call sites keep
working.
"""
...
from codex_crew.acp import kas_wire
...
from codex_crew.acp.prompt_blocks import build_prompt_blocks, summarize_prompt_structure
...
class WatchdogSettings:
    ...
    # snapshot. Telemetry-only (the codexcrew.watchdog.action attr): a BOOLEAN,
...
def _clamp_to_prompt_ceiling(key: str, value: float, chat_ceiling: float) -> float:
    ...
    Resolved via :func:`~codex_crew.acp.client.prompt_timeout_for_ceiling` on the
...
def _load_watchdog_settings(crew_agent: str = "") -> WatchdogSettings:
    ...
    ``crew_agent`` is the CANONICAL Codex Crew agent name — a ``cfg.agents``
...
class AcpSessionHandle:
    ...
    def _emit_watchdog_metric(
        ...
        """Emit codexcrew.watchdog.action + codexcrew.watchdog.idle.duration (best-effort).
        ...
        try:
            ...
            rec = get_recorder()
            rec.counter("codexcrew.watchdog.action", attrs=attrs)
            # ms, like every other codexcrew duration histogram: the dashboard's
            # generic aggregation reports all histograms under *_ms keys, so a
            # seconds-unit instrument would render 1000x off there.
            rec.histogram(
                "codexcrew.watchdog.idle.duration",
                float(idle) * 1000.0,
                unit="ms",
    ...
    def _emit_tool_interrupted_sel(self, site: str) -> None:
        ...
        CodexCrew observes but does not control (kiro-cli denied tool execution).
        Best-effort — a failed audit must not break the turn. This handle has no
        CodexCrew session_key, so the ACP sessionId is recorded in metadata for
        correlation.
        """
        logger.warning(
