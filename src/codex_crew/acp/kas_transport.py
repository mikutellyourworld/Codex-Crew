"""How Codex Crew reaches KAS: kiro-cli's own ACP relay, not a private spawn.
...
forwards ACP frames to KAS in both directions, so Codex Crew speaks ordinary ACP
...
:mod:`codex_crew.acp.kas_wire` matches on), ``session/request_permission``
...
:mod:`codex_crew.acp.types`). KAS implements its own OS sandbox -- seatbelt on
...
def build_kas_argv(kiro_bin: str) -> list[str]:
    ...
    :mod:`codex_crew.acp.kas_agents`), which the relay forwards. Passing an
