...
* :meth:`codex_crew.acp.session_handle.AcpSessionHandle.prompt` -- the live path
...
* :meth:`codex_crew.acp.client.AcpClient._send_prompt` -- the direct-client path.
...
from codex_crew.hooks import is_unc_shape, safe_read_file_bytes, unc_probe_allowed

# The budget constants and Pillow machinery live in the LEAF module
# codex_crew.imaging (shared with the gateway's tool-result rewrite, which must
...
from codex_crew.imaging import (  # noqa: F401 -- constants re-exported, see comment
    MAX_IMAGE_B64_BYTES,
    MAX_IMAGE_EDGE_PX,
...
# dashboard uploads in %USERPROFILE%\.codex-crew\uploads, so on Windows the
    ...
    *,
    ...
    if allow_image:
        ...
        for match in _PATH_RE.finditer(message):
            ...
            # is platform-gated anyway. See codex_crew.hooks.unc_probe_allowed.
