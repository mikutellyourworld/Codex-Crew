...
here so ``from codex_crew.acp.runtime import AcpSessionHandle`` (and the
...
from codex_crew import agent_scratch, platform_compat
...
from codex_crew.acp._dispatch import reject_option_id as _reject_option_id
from codex_crew.acp._dispatch import (
    set_mode_params,
)
...
from codex_crew.acp.kas_transport import build_kas_argv
from codex_crew.acp.session_handle import (
    AcpRequestTimeout,
    AcpRuntimeDead,
    AcpRuntimeError,
    AcpRuntimeProtocol,
...
from codex_crew.env import augmented_path, resolve_krb5_ccname
...
from codex_crew.metrics.events import (
...
from codex_crew.validation import MODEL_ID_RE
...
def _format_runtime_rpc_error(error: object) -> str:
    ...
    error', 'data': "Mode 'codexcrew' not found"}`` — which names an internal ACP
    ...
    if isinstance(error, dict):
        ...
        if match:
            ...
            return (
                ...
                f"is restored — repair with `codexcrew setup --agent-only --clean`, "
...
def _get_rss_tree_mb(pid: int) -> float | None:
    ...
    On Linux the codexcrew-lite background runtime is spawned through the
...
class AcpRuntime:
    ...
    def __init__(
        ...
        self._agent = agent
        # Canonical Codex Crew agent identity (a cfg.agents key) resolved by the
        ...
        # KNOWN to have zero MCP servers — the codexcrew-lite background runtime,
        # whose config Codex Crew itself writes with an empty mcpServers map —
        ...
        # runtimes (e.g. the codexcrew-lite background runtime) have no
        ...
        # (never in __init__: CodexCrewConfig.load() is a synchronous disk
    ...
    async def _is_stale(self) -> str | None:
        ...
        codexcrew-lite background runtime observed growing unbounded (multi-GB
    ...
    async def _spawn_admitted(self) -> None:
        ...
        # Delegated Kiro agents on macOS do not inherit Codex Crew's Seatbelt
        ...
        try:
            ...
            init_resp = await self._send_and_await(
                "initialize",
                {
                    ...
                    # telemetry — bucketed as "(none)" instead of "codexcrew". Nest it to
    ...
    def _audit_denied_off_loop(
        ...
        *,
        ...
        import: a module-level import of ``codex_crew.sel`` would be circular
    ...
    async def _reader_loop(self) -> None:
        ...
        try:
            while True:
                ...
                # :mod:`codex_crew.acp.kas_transport`). A request that still
        ...
        if self._acp_backend == ACP_BACKEND_KAS and agent:
            ...
            def _build() -> list[dict[str, Any]]:
                ...
                if member_dispatch:
                    ...
                    from codex_crew.members import MEMBER_DISPATCH_SERVER
    ...
    async def _session_start_budget(self) -> float:
        ...
        ``_resolve_session_start_timeout`` calls ``CodexCrewConfig.load()``,
    ...
    async def create_session(
        ...
        ``crew_agent`` is the canonical Codex Crew identity for THIS session;
        ...
        session-level entry (identity via ``CODEXCREW_SESSION_KEY``), and the
        ...
        if member_session_key:
            ...
            from codex_crew.members import member_dispatch_session_server
        ...
        if mode_agent and self._mode_available(mode_agent, resp):
            ...
            raise AcpRuntimeError(
                ...
                f"Run `codexcrew setup --agent-only` to materialize the agent config."
            )
