...
# leaf module
# ``codex_crew.acp_backends`` (it imports nothing from this package, which is what
# lets the config loader and the dashboard read them). Re-exported here so every
# existing ``from codex_crew.acp.types import ACP_BACKEND_*`` call site is
# unchanged — see the "ACP Backend Identifiers" section below for why they moved.
from codex_crew.acp_backends import (  # noqa: F401 - re-exported for existing importers
    ACP_BACKEND_CLAUDE,
    ACP_BACKEND_CODEX,
    ACP_BACKEND_KAS,
    ACP_BACKEND_KIMI,
...
# bridge.
#
# `fs` and `terminal` stay false: CodexCrew does not serve the agent's file or
...
# DEFINED in :mod:`codex_crew.acp_backends` and re-exported from the import block
# at the top of this module, so the ~19 existing
# ``from codex_crew.acp.types import ACP_BACKEND_*`` call sites are unchanged.
#
# The definitions had to move out of this package: importing anything under
# ``codex_crew.acp`` executes its ``__init__`` (client + runtime), so the loader's
...
# ``codex_crew.acp_backends`` and re-exported by the import above, so
# ``from codex_crew.acp.types import ACP_BACKENDS_STEER`` still resolves. They moved
# for the same reason the backend identifiers did: a consumer outside this package
# must be able to ask a capability question without importing ``codex_crew.acp``,
...
PROVIDER_LABEL_KIMI = "kimi"
PROVIDER_LABEL_OPENAI_COMPATIBLE = "openai_compatible"
...
# client to service them — and Codex Crew implements none, so leaving them
...
class AcpEvent:
    ...
    stop_reason: str = ""
    #: True when Codex Crew fabricated this terminal event because the provider
