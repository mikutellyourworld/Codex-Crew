"""ACP client for Codex, Claude, Kimi, OpenAI-compatible, and legacy Kiro harnesses.
...
from codex_crew import acp_tool_gate, agent_scratch, model_registry, platform_compat
from codex_crew.acp import seed_provenance
...
from codex_crew.acp.mcp_session_report import McpSessionReport
from codex_crew.acp.prompt_blocks import build_prompt_blocks
from codex_crew.acp.session_mcp import session_mcp_deny_rules
...
from codex_crew.kiro_cli import known_kiro_cli_dirs, resolve_kiro_cli
from codex_crew.mcp_gateway.claim import schedule_claim
from codex_crew.mcp_gateway.session_servers import injection_server_names, pooled_session_servers
from codex_crew.metrics.tool_calls import note_tool_call_started, record_tool_call_finished
from codex_crew.providers.mirrors import mirror_for
from codex_crew.resource_status import inject_xdist_auto_cap
from codex_crew.sandbox import (
...
from codex_crew.security import is_sensitive_path, redact_credentials, redact_exfiltration_urls
from codex_crew.sel import sel
from codex_crew.session_directive import content_free_digest
from codex_crew.skill_usage import get_global_skill_read_observer

logger = logging.getLogger(__name__)

CLIENT_NAME = "codexcrew"
...
# The desktop shell sets this to its own executable. Electron runs ordinary Node
# programs when ELECTRON_RUN_AS_NODE=1, so the installed app needs no separate
# system Node installation for its bundled adapter.
_ENV_CODEXCREW_NODE_EXECUTABLE = "CODEXCREW_NODE_EXECUTABLE"

# Kimi Code ships a native ACP stdio server, so it needs no translation package.
KIMI_CLI_BIN = "kimi"
KIMI_CLI_SUBCMD = "acp"
...
def _resolve_kiro_bin(
    *,
    ...
    Returns the installed binary's own path. CodexCrew never copies the CLI and
    ...
    *environ* and *home* exist so a caller that also needs to REPORT the search
    ...
    from codex_crew.kiro_prerequisite import snapshot_trusted_acp_executable
...
def kiro_cli_not_found_message(
    ...
    * The message never says "in PATH". Resolution also walks
      ...
      ``CODEXCREW_KIRO_BIN`` override, so "not found in PATH" sent a reader whose
      ...
      the app bundle did contain a ``codexcrew.exe`` — Codex Crew's OWN console
    ...
    from codex_crew.kiro_prerequisite import OFFICIAL_INSTALL_DOCS_URL
    ...
    return (
        ...
        f"Kiro CLI is a separate prerequisite Codex Crew does not bundle: install it "
        f"from {OFFICIAL_INSTALL_DOCS_URL}, or point CODEXCREW_KIRO_BIN at the binary."
    )
...
def _mise_node_installs_dir() -> Path:
    ...
    The data root comes from :func:`codex_crew.env.mise_data_dir` so that
...
def _vendored_acp_roots(pkg_dir: Path | None = None) -> list[Path]:
    ...
    *pkg_dir* (the installed ``codex_crew`` package directory) defaults to this
    ...
    if pkg_dir is None:
        pkg_dir = Path(__file__).resolve().parent.parent  # .../codex_crew
    roots.append(pkg_dir / "_vendor" / "node_modules")

    # 2. Explicit project dir (CODEXCREW_PROJECT_DIR points at the repo root):
...
def _resolve_codex_acp_bin() -> tuple[list[str] | None, str]:
    ...
    for script in candidates:
        ...
        if packaged_node and Path(packaged_node).is_file():
            return [str(Path(packaged_node).resolve()), resolved], search_path
        node = _resolve_node_for_script(resolved)
        if node:
            return [node, resolved], search_path
        if platform_compat.is_executable_file(script):
...
def _resolve_kimi_bin() -> str | None:
    """Resolve Kimi Code CLI, including its standard Windows user install."""
    override = os.environ.get(_ENV_KIMI_CLI_BIN)
    if override and platform_compat.is_executable_file(override):
        return _normalize_exe_casing(override) or override

    search_path = augmented_path(os.environ.get("PATH", ""))
    on_path = shutil.which(KIMI_CLI_BIN, path=search_path)
    if on_path:
        return _normalize_exe_casing(on_path) or on_path

    names = ("kimi.exe", "kimi") if os.name == "nt" else ("kimi",)
    for name in names:
        candidate = Path.home() / ".kimi-code" / "bin" / name
...
def _claude_settings_usable(path: Path) -> bool:
    ...
    :func:`~codex_crew.agent_discovery._read_agent_spec` applies to agent specs).
...
def _resolve_spawn_env(env: dict[str, str], *, kiro_api_key: bool = False) -> dict[str, str]:
    ...
    from codex_crew.config.loader import inject_kiro_cli_api_key, strip_kiro_cli_api_key
...
# ``session/prompt`` — so without special handling Codex Crew waits the full prompt timeout.
...
def prompt_timeout_for_ceiling(configured: float) -> float:
    ...
    against the ceiling without a second synchronous ``CodexCrewConfig.load()``.
    """
    if configured <= 0:
...
def resolve_prompt_timeout() -> float:
    ...
    try:
        ...
        configured = float(CodexCrewConfig.load().agent.chat_turn_timeout_secs)
    except Exception:
...
async def _effective_prompt_timeout_async(timeout: float | None) -> float:
    ...
    → ``CodexCrewConfig.load()``: stat, read, validate), so resolving it inline
...
class AcpTimeoutError(AcpError):
    """ACP request timed out, optionally after returning partial prompt output."""

    def __init__(
        self,
        partial_output: str = "",
        *,
        message: str = "ACP prompt timed out",
        method: str = "",
    ) -> None:
...
class AcpToolGateUnroutable(AcpError):  # noqa: N818
    """The harness's tool calls would not reach Codex Crew's PreToolUse gate.
    ...
    Wraps :class:`codex_crew.acp_tool_gate.ToolGateUnroutable`, which cannot
...
# The second alternative is the machine-readable reason code the same rejection
# carries when the backend reports it structurally instead of in prose. Prose is
# localizable and gets reworded between releases; the code does not, so matching
# both means a reworded sentence cannot silently turn a momentary capacity
# rejection back into a terminal one.
_RE_MODEL_TEMP_UNAVAILABLE = re.compile(
    r"[Tt]he model you['\u2019]ve selected is temporarily unavailable"
    r"|MODEL_TEMPORARILY_UNAVAILABLE"
)
...
# ``ModelThrottleError`` must be named here rather than left to the generic
# pattern below: its word-boundary anchors cannot see the ``Throttle`` buried
# inside a CamelCase identifier, so a capacity failure that arrives only under
# this name reads as terminal and no retry or fallback ever engages.
_RE_THROTTLE_NAMED = re.compile(
    r"\b(ThrottlingException|TooManyRequestsException|ServiceQuotaExceededException"
    r"|ModelThrottleError)\b"
)
# "high volume of traffic" is the backend's own user-facing phrasing for a
# capacity rejection and names neither a rate limit nor a throttle, so it needs
# its own alternative to reach the retry ladder.
_RE_THROTTLE_GENERIC = re.compile(
    r"\b(rate.?limit|throttl(?:e|ed|ing)|high volume of traffic)\b", re.IGNORECASE
)
_RE_AUTH = re.compile(
...
# ``InternalServerException`` is the spelling the model backend actually returns
# for a 5xx; it is the same fault as ``InternalServerError`` and shares the one
# suffix group so the pair cannot drift apart.
_RE_5XX_NAMED = re.compile(
    r"\b(InternalServer(?:Error|Exception)|InternalFailure|ServiceUnavailable(?:Exception)?"
    ...
    if isinstance(error, dict):
        ...
        # deprecated, etc.).
            ...
            formatted = (
                ...
                f"'auto' in ~/.codex-crew/config.json, or (3) wait a minute and "
                f"retry."
                f"{req_id_suffix}"
            )
            ...
            formatted = (
                ...
                "'auto' in ~/.codex-crew/config.json, or (3) wait a minute and "
                "retry."
                f"{req_id_suffix}"
            )
...
def _make_unified_diff(old: str, new: str, path: str, max_len: int = 65536) -> str:
    ...
    Thin delegate to :func:`codex_crew.acp._dispatch.make_unified_diff`, kept as
...
def _select_tool_title(
    ...
    *,
    ...
    (e.g. "List CodexCrew ACP module files" rather than `ls /workplace/...`).
...
def _sandbox_preflight(
    backend: str, mode: str, *, native_tool_gate: bool = False
) -> tuple[str, ...]:
    ...
    """
    try:
        acp_tool_gate.enforce_sandbox_floor(backend, mode, native_tool_gate=native_tool_gate)
        if native_tool_gate and sys.platform == "win32":
            return ()
...
class AcpClient:
    ...
    def _is_kimi(self) -> bool:
        return self.backend == ACP_BACKEND_KIMI

    @property
    def _is_openai_compatible(self) -> bool:
        return self.backend == ACP_BACKEND_OPENAI_COMPATIBLE
    ...
    def _append_member_dispatch_server(self, servers: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        carries ``CODEXCREW_SESSION_KEY`` for strict identity — the same value this
        ...
        from codex_crew.members import is_member_session_key, member_dispatch_session_server
    ...
    def _session_mcp_servers(self) -> list[dict[str, Any]]:
        ...
        :mod:`codex_crew.acp.session_mcp`).
    ...
    def _claude_session_mcp_servers(self) -> list:
        ...
        ``codexcrew.mcp.json`` on its own, so a claude session had zero MCP tools.
        ...
        (:mod:`codex_crew.providers.mirrors.claude_code`), not here: projecting the
    ...
    def _codex_session_mcp_servers(self) -> list:
        ...
        registry in :mod:`codex_crew.providers.mirrors` records that as codex's
    ...
    def _expected_settings_fingerprint(self) -> tuple[int, str] | None:
        ...
        durable record in :mod:`codex_crew.acp.seed_provenance`, which is what lets
    ...
    def _write_claude_local_settings(self) -> None:
        ...
        2. ``permissions.deny``, from
           :func:`~codex_crew.acp.session_mcp.session_mcp_deny_rules`: the agent
        ...
        3. ``availableModels`` plus the resolved ``model``, and ONLY once the
           ...
           in. See :func:`~codex_crew.model_registry.seed_available_models`.
        ...
        **Crew touches only the file it owns.** Ownership is not the path -- a
        ...
        :mod:`codex_crew.acp.seed_provenance` for a seed an earlier session left
    ...
    def _capture_available_models(self, session_resp: dict) -> None:
        ...
        :func:`codex_crew.acp.session_handle.parse_advertised_models` so this
        ...
        from codex_crew.acp.session_handle import parse_advertised_models
    ...
    async def _reseed_after_capture(self) -> None:
        ...
        :mod:`codex_crew.acp.seed_provenance`) was not even allowed to correct it.
    ...
    async def _apply_session_permission_routing(self) -> None:
        ...
        * the option was not advertised -> INDETERMINATE, because Codex Crew cannot
        ...
        * the write was rejected -> BYPASSED, an observed failure rather than an
        ...
        if issue:
            ...
            # ask anyway; Codex Crew simply has no evidence, and the enforcement
    ...
    def _sync_effort_levels(self) -> None:
        ...
        if levels:
            ...
            from codex_crew.dashboard.chat_persistence import update_reasoning_effort_values
    ...
    async def _spawn(self) -> None:
        ...
        Every supported ACP backend reaches here. Each branch owns only the
        backend-specific executable and environment while the process lifecycle
        below stays shared.
        """
        ...
        # Codex Crew's Seatbelt profile. Refuse a delegated agent workspace that
        ...
        if self._is_claude:
            ...
            from codex_crew.codex_tool_gate import (
                CodexToolGateConfigError,
                build_codex_config,
            )

            try:
                codex_config_json = build_codex_config(
                    os.environ.get("CODEX_CONFIG"),
                    python_executable=sys.executable,
                    agent=self._agent,
                )
            except CodexToolGateConfigError as exc:
                raise AcpError("Codex security hook could not be installed: " + str(exc)) from exc
            ...
            adapter_hidden_dirs = await asyncio.to_thread(
                functools.partial(
                    ...
                    self.backend,
                    self._sandbox_mode,
                    native_tool_gate=True,
                )
            )
        elif self._is_kimi:
            self._session_mcp_cache = await asyncio.to_thread(self._resolve_session_mcp_servers)
            kimi_bin = await asyncio.to_thread(_resolve_kimi_bin)
            if not kimi_bin:
                raise AcpError(
                    f"{KIMI_CLI_BIN} not found. Install Kimi Code CLI or set "
                    f"{_ENV_KIMI_CLI_BIN} to its executable."
                )
            argv = [kimi_bin, KIMI_CLI_SUBCMD]
        elif self._is_openai_compatible:
            self._session_mcp_cache = await asyncio.to_thread(self._resolve_session_mcp_servers)
            argv = [sys.executable, "-m", "codex_crew.acp_adapters.openai_compatible_server"]
        else:
        ...
        argv, self._sandbox_cleanup = await wrap_argv_async(
            ...
            native_tool_gate=self._is_codex,
            _prepare=wrap_argv,
        )
        ...
        if codex_config_json is not None:
        ...
        if self._session_key:
            env["CODEXCREW_SESSION_KEY"] = self._session_key
        else:
            env.pop("CODEXCREW_SESSION_KEY", None)
        if self._channel_id:
            env["CODEXCREW_CHANNEL_ID"] = self._channel_id
        else:
            env.pop("CODEXCREW_CHANNEL_ID", None)
        ...
        env[CODEXCREW_SPAWNED_ENV] = CODEXCREW_SPAWNED_VALUE
        ...
        try:
            ...
            from codex_crew.session import (  # circular: session -> config.loader -> providers.acp -> acp.client
                _track_child_pids,
                _track_pid,
                _track_session_pid,
            )
    ...
    async def _snapshot_process_tree(self) -> None:
        ...
        if self._child_pids:
            from codex_crew.session import _track_child_pids
    ...
    def _reset_state(self) -> None:
        ...
        # These untrack helpers live in codex_crew.session, which imports this
        # module transitively, so they must be imported inline.
        from codex_crew.session import _untrack_child_pids, _untrack_pid, _untrack_session_pid
        ...
        new_params: dict = {
            ...
            "mcpServers": [
                ...
                *(
                    ...
                    if self._is_kimi or self._is_openai_compatible
                    else []
                ),
    ...
    async def _initialize_session(self) -> None:
        ...
        if resume_sid and self._can_load_session:
            ...
            if self._is_claude:
                # Claude session/load takes no file path, and the
                ...
                session_file = ""
                file_ok = True
            elif self._is_codex or self._is_kimi:
            ...
            if file_ok:
                try:
                    ...
                    # A timed-out load has an unknown outcome: the backend may
                    # have restored the session even though its response was
                    # delayed. Starting a new session here would duplicate it.
                    raise
                except AcpError:
                    logger.info(
        ...
        if self._is_kiro:
            if not self._modes_advertised or self._agent in self._available_mode_ids:
                ...
                raise AcpError(
                    ...
                    f"`codexcrew setup --agent-only` to materialize the agent config."
                )

        # 5. Set model — override if CodexCrew config specifies non-default.
    ...
    async def ensure_ready(self) -> None:
        ...
        # Telemetry (codexcrew.session.startup.duration): time the cold-start work
        ...
        try:
            ...
            for attempt in range(2):
                try:
                    ...
                    # Once a session request times out, its outcome is unknown;
                    # once a session id exists, a later startup step failed after
                    # creation. Replaying either path can manufacture a duplicate
                    # session, so only failures known to precede session creation
                    # receive the one fresh-process retry.
                    session_outcome_ambiguous = bool(self._session_id) or (
                        isinstance(exc, AcpTimeoutError)
                        and exc.method in {METHOD_SESSION_NEW, METHOD_SESSION_LOAD}
                    )
                    non_retryable = isinstance(exc, AcpAuthRequired) or session_outcome_ambiguous
                    if attempt == 0 and not non_retryable:
            ...
            try:
                ...
                # -> config.loader (provider reads CodexCrewConfig). Keep it lazy so
                # provider is never loaded during config.loader's import chain.
                from codex_crew.metrics.provider import get_recorder

                get_recorder().histogram(
                    "codexcrew.session.startup.duration",
    ...
    async def _wait_for_response(
        ...
        *,
        ...
        to ``session/new``.
        """
        from codex_crew import shutdown_event
        ...
        if method in {METHOD_SESSION_NEW, METHOD_SESSION_LOAD}:
            ...
            if progress:
                message += f" ({progress})"
        raise AcpTimeoutError(message=message, method=method)
    ...
    async def _reject_unknown_server_request(self, msg: JsonRpcMessage) -> None:
        ...
        CodexCrew implements only ``session/request_permission`` as an inbound
    ...
    def _emit_tool_interrupted_sel(self, site: str) -> None:
        ...
        that CodexCrew observes but does not control.  Logged so the audit trail reflects
        that tools were blocked even though the decision was made outside CodexCrew.
        ...
        try:
            ...
            # ``codex_crew.sel.sel`` and lets a substituted emitter be observed.
            from codex_crew.sel import sel
    ...
    def _extract_tool_event(self, msg: JsonRpcMessage) -> AcpEvent | None:
        ...
        if update.get("sessionUpdate") == UPDATE_TOOL_CALL:
            ...
            # the first moment Codex Crew sees the call, and the same id's terminal
            ...
            # some backends' Bash tool emits "List CodexCrew ACP module files"
    ...
    def _extract_tool_call_refinement(self, msg: JsonRpcMessage) -> AcpEvent | None:
        ...
        (e.g. `"ls /local/home/user/.codex-crew/workspace"`).
        ...
        # Bash's "List CodexCrew ACP module files" rather than `ls /workplace/...`).
