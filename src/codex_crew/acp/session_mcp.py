...
For a harness in :data:`~codex_crew.acp_backends.ACP_BACKENDS_SESSION_MCP_ARRAY`,
...
the translation here such a session runs with ZERO Codex Crew tools -- the harness
...
from codex_crew.agent_discovery import _read_agent_spec, project_agent_files, project_agent_name
...
_CONTROL_PLANE_SERVERS = ("codexcrew-core", "codexcrew-cron")
...
def _agent_spec_for(agent: str, work_dir: str | Path | None = None) -> dict[str, Any] | None:
    ...
    **Project-nearest first.** kiro-cli resolves ``--agent`` against the project
    ...
    (``codexcrew.json -> ~/.aws/credentials``) is refused and audited, an oversized
...
def session_mcp_servers(
    ...
    *stub_server_names* are the servers that will ALSO arrive in this array as
    ...
    # administrator who omits ``codexcrew-core`` from the catalog has it dropped
    ...
    # kiro-cli drops ``codexcrew-core`` from a spec whose ``tools`` stops naming
