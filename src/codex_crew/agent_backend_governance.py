...
(:func:`~codex_crew.acp_backends.apply_selectable_denials`), so it is idempotent and
...
* **H3** — the one gate ``resolve_selected_backend`` runs inside
  ``CodexCrewConfig.load()`` and must never read the platform context, because
...
claude**, not **only claude**: :data:`~codex_crew.acp_backends.GOVERNANCE_FLOOR_BACKEND`
...
def narrow_selectable_backends() -> list[str]:
    ...
    ``CodexCrewConfig.load()``, so ``governance_permits`` can read the ceiling without
