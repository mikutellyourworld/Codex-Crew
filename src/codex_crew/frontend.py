"""Shared helpers for building the CodexCrew website frontend assets.
...
and must be staged into ``<repo-root>/src/codex_crew/static/dist`` so the
...
``CodexCrewWebsite/dist`` clone is honored as a last-resort fallback when
...
from codex_crew.node_modules_txn import NodeModulesBackup
...
_DIR_NAME = "website"
_SIBLING_DIR_NAME = "CodexCrewWebsite"
...
# ``website/docs/extension-seams.md``). ``CODEXCREW_EDITION_DIR`` names the
# edition's own ``extensions.tsx``; ``CODEXCREW_ALLOW_EDITION=1`` is the
# fail-closed opt-in that must accompany it.
_EDITION_DIR_ENV = "CODEXCREW_EDITION_DIR"
_EDITION_OPT_IN_ENV = "CODEXCREW_ALLOW_EDITION"
...
def edition_sources_missing() -> bool:
    ...
    Absent ``CODEXCREW_EDITION_DIR`` this is ``False`` and the stock path is
    untouched.
    """
...
def _edition_build_env() -> Optional[dict[str, str]]:
    ...
    The runtime rebuild (``POST /api/update``, ``codexcrew update``, and the
    ...
    **The opt-in is READ, never synthesized.** ``CODEXCREW_ALLOW_EDITION=1`` is the
    ...
    ``CODEXCREW_EDITION_DIR``-without-opt-in error still fires when it should.
...
def _repo_root(codex_crew_pkg_dir: Path) -> Path:
    """Return the repo root given the ``codex_crew`` package directory.

    Layout: ``<repo-root>/src/codex_crew/`` is *codex_crew_pkg_dir*, so two
    ``.parent`` hops land on the repo root (parent of ``src/``).
    """
    return codex_crew_pkg_dir.parent.parent


def _resolve_website_dist(codex_crew_pkg_dir: Path) -> Optional[Path]:
    ...
    2. A sibling checkout — ``<repo-root>/../CodexCrewWebsite/dist`` (legacy
       side-by-side dev layout). Last-resort only.

    Returns the resolved dist path on success, ``None`` otherwise.
    """
    repo_root = _repo_root(codex_crew_pkg_dir)
    ...
    # 2. Sibling CodexCrewWebsite/dist (legacy fallback).
...
def ensure_dev_dist_symlink() -> Optional[Path]:
    ...
    The dashboard serves its SPA from ``<codex_crew>/static/dist/index.html``.
    ...
    plain source-tree run (``PYTHONPATH=src python -m codex_crew gateway``,
    ...
    3. Missing → resolve the in-tree ``website/dist`` (or a sibling
       ``CodexCrewWebsite`` checkout as a last resort) and symlink to it.
    ...
    """
    codex_crew_pkg_dir = Path(__file__).resolve().parent
    tree_dist = codex_crew_pkg_dir / "static" / "dist"
    ...
    # local build landing in the source tree / user ran codexcrew init --ui).
    ...
    candidate = _resolve_website_dist(codex_crew_pkg_dir)
...
def _staging_lock(static_parent: Path) -> Iterator[None]:
    ...
    Serializes every build or stage of the frontend initiated by Codex Crew: Dev
...
def _npm_build_and_stage_locked(
    ...
    if proc.returncode != 0:
        ...
        return False
    static_dist = proj_path / "src" / "codex_crew" / "static" / "dist"
...
def build_and_stage(
    ...
    if not npm_bin:
        ...
        return False
    try:
        with _staging_lock(root / "src" / "codex_crew" / "static"):
...
def build_frontend_sync(
    ...
    ``website/dist`` into ``src/codex_crew/static/dist``. Graceful no-op when
    ...
    #
    # RESIDUAL: this closes races between Codex Crew's own Python flows. Dev Fleet's
    # Pull+Build takes this same lock for its build+stage child, but its `npm ci`
    # step runs from a generated stdlib-only script that cannot import codex_crew
    ...
    proc = None
    try:
        with _staging_lock(proj_path / "src" / "codex_crew" / "static"):
            if not backup.begin():
                return
            try:
                try:
...
async def build_frontend_async(
    ...
    ``src/codex_crew/static/dist``. Graceful no-op when there is no
    ...
    static_parent = proj_path / "src" / "codex_crew" / "static"
    messages: list[str] = []
    staged = False
    try:
