...
the ``TMPDIR``/``TMP``/``TEMP`` triple plus ``CODEXCREW_SCRATCH`` pointing at
...
Sweep hygiene follows the house rules of :mod:`codex_crew.agents_janitor`:
...
def scratch_env(path: Path) -> dict[str, str]:
    ...
    both platforms; ``CODEXCREW_SCRATCH`` is the prompt-visible name for
    deliberate work products (clones, logs, screenshots).
    """
    value = str(path)
    return {"TMPDIR": value, "TMP": value, "TEMP": value, "CODEXCREW_SCRATCH": value}
