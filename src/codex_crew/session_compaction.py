"""Context-compaction policy and execution for :mod:`codex_crew.session`.

The coordinator deliberately depends on a small owner facade instead of
importing ``codex_crew.session``.  Session allocation and teardown still own the
...
from codex_crew.metrics.events import CONTEXT_COMPACTIONS, emit_counter
from codex_crew.metrics.sessions import END_REASON_RECYCLED, record_session_ended
...
class CompactionDeps:
    ...
    companions patch names in ``codex_crew.session`` after constructing a
    ...
    async def _compact_in_place(self, key: str, session: Any, pct: float) -> str:
        ...
        try:
            ...
            async def _run() -> None:
                ...
                from codex_crew.acp.types import EVENT_COMPACTION_STATUS
