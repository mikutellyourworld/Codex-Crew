...
2. **Which cron job**, when the surface is cron -- joined by PID to the
   in-flight markers :mod:`codex_crew.cron_inflight` leaves behind: a marker
...
from codex_crew.cron_inflight import RunningMarker, read_markers, read_recorded_attribution
...
#: outermost Codex Crew frame is the one that says who started the turn: a cron
...
_SURFACE_RULES: tuple[tuple[str, tuple[str, ...], tuple[str, ...]], ...] = (
    ...
    ("cron", ("/codex_crew/cron.py",), ("_cron_callback", "_run_job_isolated")),
    ("heartbeat", ("/codex_crew/heartbeat.py",), ()),
    ("task runner", ("/codex_crew/task_executor.py", "/codex_crew/task_planner.py"), ()),
    ("subagent", ("/codex_crew/subagent_manager/",), ()),
    ("workflow", ("/codex_crew/workflows/",), ()),
    ("dashboard chat", ("/codex_crew/dashboard/chat_runner.py",), ()),
    ("dashboard side panel", ("/codex_crew/dashboard/handlers/side.py",), ()),
    ("slack", ("/codex_crew/slack/handler.py", "/codex_crew/slack/transport_dispatch.py"), ()),
    ("discord", ("/codex_crew/discord/",), ()),
    ("telegram", ("/codex_crew/telegram/",), ()),
    ("teams", ("/codex_crew/teams/",), ()),
    ("webex", ("/codex_crew/webex/",), ()),
    ("messaging", ("/codex_crew/messaging/dispatch.py",), ()),
)

#: Gate frames worth naming as "stuck in": the security gate and its callers.
_GATE_FILES = ("/codex_crew/security.py", "/codex_crew/hooks.py")
...
class StallAttribution:
    dump: Path
    surface: str = "unknown"
    #: The innermost Codex Crew frame of the wedged stack (what the loop was
...
def _inert(text: str) -> str:
    ...
    ``codexcrew doctor`` prints on, or spoof the line around it. Printable text,
...
def describe(attribution: StallAttribution) -> list[str]:
    """Human-readable lines for ``codexcrew doctor`` and the boot notification.
    ...
    if a.is_cron:
        if a.job is not None:
            ...
            lines.append(f"recommended: codexcrew cron pause {_inert(a.job.job_id)}")
