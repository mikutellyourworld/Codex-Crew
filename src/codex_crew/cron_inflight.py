...
process died, so ``codexcrew doctor`` could show the stack but not the job, and
...
(:mod:`codex_crew.stall_attribution`) joins it to the crash dump by PID; the
...
from codex_crew.dashboard.crash_dump_store import current_process_identity, pid_identity_alive
...
def record_attribution(
    ...
    later reader of the same dump -- ``codexcrew doctor``, the restart
