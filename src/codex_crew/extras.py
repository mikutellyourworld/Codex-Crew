...
``codexcrew`` is NOT published on any index, so the obvious advice --
``pip install "codexcrew[feishu]"`` -- is not merely unhelpful, it cannot resolve
at all: pip fails with *No matching distribution found for codexcrew* however the
user installed Codex Crew. On a hypothetical index it would be worse than an
error, because installing ``codexcrew[extra]`` REPLACES an editable/source
...
exactly the missing dependency without touching Codex Crew itself.
