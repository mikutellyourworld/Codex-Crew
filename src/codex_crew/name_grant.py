...
* **A name no approval has identified.** The two rules above cannot help a name
  the system directories do not carry -- ``gh``, ``node``, ``codexcrew``, a
...
from codex_crew.github_runner import agent_writable_roots
...
def _pin_refusal(name: str, found: str, real: str, witness: bool) -> Refusal | None:
    ...
    carry -- ``gh``, ``node``, ``codexcrew``, a version manager's ``python``.
