...
from codex_crew.constants import OPTIONS_RE_TRAILER, strip_control_comments
from codex_crew.messaging.display_safety import redact_for_display
from codex_crew.messaging.tables import render_tables, render_tables_with_metadata
...
def chunk_for_transport(text: str, capabilities: TransportCapabilities) -> list[str]:
    ...
    :func:`~codex_crew.messaging.split.split_markdown_bytes`, the char path via
    :func:`~codex_crew.messaging.split.split_markdown_safe`. A blind fixed-width
    ...
    if max_bytes > 0:
        from codex_crew.messaging.split import split_markdown_bytes
