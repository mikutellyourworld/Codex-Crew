...
from codex_crew.hooks import HOOK_REPLY, TOOL_AUTO_APPROVE, TOOL_DENY, event_is_spawn_run
from codex_crew.messaging.driver import DirectiveConsumer, TurnDriver
...
from codex_crew.messaging.renderer import SilentRenderer
from codex_crew.security import redact, redact_credentials, redact_exfiltration_urls
from codex_crew.sel import sel

# Imported from the leaf that DEFINES it rather than through codex_crew.session:
...
class ChannelTurn:
    ...
    constructed channel-side via :func:`codex_crew.messaging.link.build_dm_session_key`
    ...
    user_text: str
    renderer: Any
    """A :class:`codex_crew.messaging.renderer.Renderer`."""
    ...
    """A :class:`~codex_crew.messaging.link.ChannelLink` naming THIS conversation.
    ...
    :func:`codex_crew.messaging.link.bind_origin_mirror` for why the bind must be
...
def build_tool_gate(ctx_builder: Any, *, session_key: str, agent: str) -> Callable[[Any], str]:
    ...
    no channel package needs to import ``codex_crew.slack``.
    """
...
async def drive_turn(turn: ChannelTurn, *, sessions: Any, ctx_builder: Any) -> None:
    ...
    try:
        ...
        driver = TurnDriver(
            ...
            audit_agent=turn.agent or "codexcrew",
