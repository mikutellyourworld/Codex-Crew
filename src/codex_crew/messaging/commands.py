...
* HERE -- the decision plus the reply TEXT, the same shape
  :func:`~codex_crew.messaging.link.release_conversation_location` already uses for
...
* NOT here -- anything address-shaped. Nothing below accepts a chat id, a thread
  ...
  already-bound :class:`~codex_crew.messaging.queue_receipt.ReceiptSurface`, which
...
**Services are duck-typed on purpose.** ``codex_crew.subagent`` and
``codex_crew.taskrunner`` both reach ``codex_crew.slack`` transitively, so importing
...
module additionally imports nothing from ``codex_crew.dashboard`` -- see
...
async def run_yolo_command(
    ...
    *,
    ...
    and Slack's ``/codexcrew yolo`` drive, so a grant taken in one channel shows up
...
def parse_dashboard_ttl(arg: str, *, parse_duration: DurationParser) -> int:
    ...
    *arg* is everything after the channel's own command tokens -- ``"2h"``,
    ...
    is ``/codexcrew dashboard <ttl>`` (the TTL is the third word) and Teams' is
    ...
    *parse_duration* supplies the duration vocabulary and is injected rather than
    imported: it lives in ``dashboard/token_auth.py``, and ``codex_crew.messaging``
    imports nothing from ``codex_crew.dashboard`` -- that one-way direction is what
