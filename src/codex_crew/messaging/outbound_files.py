...
limit downstream (:mod:`codex_crew.messaging.split`). Extraction has to run
...
which offsets are fenced comes from :func:`codex_crew.messaging.split.iter_fence_spans`
...
:mod:`codex_crew.messaging.attachments` on the inbound side: silently dropping a
...
* a denylist check (:func:`codex_crew.security.is_sensitive_path`), so
...
on its own. :mod:`codex_crew.discord.renderer` is the first consumer: it seals a
...
from codex_crew.platform_compat import first_linked_ancestor, is_link_or_junction
from codex_crew.security import (
    is_sensitive_path,
    redact_credentials,
    redact_exfiltration_urls,
)
from codex_crew.widget_parse import mask_inline_code
...
class ExtractLimits:
    ...
    :mod:`codex_crew.image_artifacts`, which happen to carry the same numbers:
...
async def extract_local_refs_off_loop(
    ...
    :func:`codex_crew.image_artifacts.register_images_off_loop`: that pool is
