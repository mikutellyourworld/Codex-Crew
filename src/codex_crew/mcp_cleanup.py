...
CodexCrew does NOT write CodexCrew-managed MCP servers to the user's global
provider MCP config (``~/.kiro/settings/mcp.json``) during normal
operation — the CodexCrew agent file is authoritative, and provider
globals are user-owned.  Remaining helpers here clean up stale
codexcrew-binary entries left over from older install methods.
...
from codex_crew.config.paths import kiro_home
...
# Managed servers whose command is the codexcrew binary itself.
# Only these are affected by install-method path changes.
# Ordered tuples (not sets) so consumers that iterate — e.g. `codexcrew
...
OPT_IN_BIN_MCP_SERVERS = ("codexcrew-dashboard", "codexcrew-consult")

# Every managed-binary server name, regardless of how it reaches a spec. This is
# the cleanup view: Codex Crew never legitimately writes any of them into the
# user's global mcp.json, so a stray entry is purgeable either way.
CODEXCREW_BIN_MCP_SERVERS = ALWAYS_ON_BIN_MCP_SERVERS + OPT_IN_BIN_MCP_SERVERS

# Every managed-binary server name CodexCrew is responsible for removing from
# the user's global mcp.json (Codex Crew never legitimately writes these there).
#
...
# opt-in server is never written there by any version of Codex Crew, since the
...
def _predecessor_mcp_names() -> frozenset[str]:
    ...
    from codex_crew.onboarding_import import predecessor_mcp_names  # noqa: PLC0415
...
def _stale_mcp_binaries() -> frozenset[str]:
    ...
    from codex_crew.onboarding_import import stale_mcp_binaries  # noqa: PLC0415

    return stale_mcp_binaries()


# The argv token the deleted Playwright MCP proxy was registered with. An entry
# still carrying it spawns `codexcrew mcp-playwright-proxy`, a subcommand this
...
def _invokes_deleted_playwright_proxy(spec: object) -> bool:
    ...
    is called, and purging by name would delete a server Codex Crew never wrote --
...
def clean_stale_managed_mcp() -> list[str]:
    ...
    Runs from explicit setup (``codexcrew setup``) and once on first gateway
    start (marker-guarded by ``run_first_run_setup``) — never on every startup,
    which would violate the "CodexCrew owns only the agent file" boundary.
    ...
    * **By name** — ``codexcrew-cron`` / ``codexcrew-core`` (written there by an
      older install method; Codex Crew now keeps these in the agent file), plus
...
def purge_deleted_proxy_from_config(config: dict) -> list[str]:
    ...
    marker) because the entry can be re-injected from ~/.codex-crew/mcp.json
