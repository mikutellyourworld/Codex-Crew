"""Configuration loader for CodexCrew.

Config location: ~/.codex-crew/config.json (overridden by CODEXCREW_HOME)
Credentials:    ~/.codex-crew/.env (overridden by CODEXCREW_HOME)

CodexCrew uses ACP adapters for Codex, Claude, Kimi, legacy public Kiro support,
and OpenAI-compatible Chat Completions profiles. This module handles session
timeouts, hook rules, and the dashboard URL via the config file. (The dashboard *port* is set with the
``CODEXCREW_PORT`` env var, not a config key.)
"""
...
from codex_crew import __version__, model_registry, platform_compat, windows_acl
from codex_crew.acp_backends import ACP_BACKEND_CLAUDE

# Leaf module (stdlib + platform_compat only) — no import cycle with config.
from codex_crew.atomic_write import atomic_write, on_event_loop
...
# ``codex_crew`` — so this cannot create an import cycle with the loader, and the
# ``computer_use`` package's ``__init__`` pulls in only ``platform_compat`` /
# ``executors`` (both stdlib-only), never ``config``.
from codex_crew.computer_use.types import DEFAULT_ATTACH_SCREENSHOT as _CU_DEFAULT_ATTACH_SCREENSHOT
from codex_crew.computer_use.types import DEFAULT_MAX_TREE_DEPTH as _CU_DEFAULT_MAX_TREE_DEPTH
from codex_crew.computer_use.types import DEFAULT_MAX_TREE_NODES as _CU_DEFAULT_MAX_TREE_NODES
from codex_crew.computer_use.types import (
    DEFAULT_SCREENSHOT_JPEG_QUALITY as _CU_DEFAULT_SCREENSHOT_JPEG_QUALITY,
)
from codex_crew.computer_use.types import DEFAULT_SCREENSHOT_MAX_PX as _CU_DEFAULT_SCREENSHOT_MAX_PX
from codex_crew.computer_use.types import DEFAULT_TEXT_LIMIT as _CU_DEFAULT_TEXT_LIMIT
from codex_crew.computer_use.types import MAX_SCREENSHOT_MAX_PX as _CU_MAX_SCREENSHOT_MAX_PX
from codex_crew.computer_use.types import MAX_TEXT_LIMIT as _CU_MAX_TEXT_LIMIT
from codex_crew.computer_use.types import MAX_TREE_DEPTH_LIMIT as _CU_MAX_TREE_DEPTH
from codex_crew.computer_use.types import MAX_TREE_NODES_LIMIT as _CU_MAX_TREE_NODES
from codex_crew.computer_use.types import MIN_SCREENSHOT_MAX_PX as _CU_MIN_SCREENSHOT_MAX_PX

# Post-split section internals are reached through the module: the name-level
# `from codex_crew.config.sections import (...)` block below is a FROZEN
# pre-split snapshot (test_config_module_boundaries pins it), so a coercer added
# after the split must not join it.
from codex_crew.config import sections as _sections

# Pure path primitives live in the leaf module ``config.paths`` (stdlib-only,
# no ``codex_crew`` imports) so the modules that only need ``config_dir()`` can
...
# these from ``codex_crew.config.loader``.
#
...
# ``patch("codex_crew.config.loader.config_dir", ...)`` (used across the suite).
from codex_crew.config.paths import (  # noqa: F401, kiro_agents_dir
    _WORKSPACE_DIR_NAME,
    CONFIG_DIR_NAME,
    OUTBOX_DIR_NAME,
...
from codex_crew.config.resolution import (  # noqa: F401
...
# CodexCrewConfig remains the compatibility facade and owns read/merge/save.
from codex_crew.config.sections import (  # noqa: F401
    _BOT_NAME_MAX,
    _BOT_NAME_RE,
    _COLOR_HEX_RE,
...
from codex_crew.config.superseded_defaults import drift_summary, superseded_default_drift
...
# reference these as ``codex_crew.config.loader.X`` (e.g. the cache tests patch
# ``codex_crew.config.loader._validate_config_data``). ``validate_config_data``
...
from codex_crew.config.validation import (  # noqa: F401
    _CONFIG_CACHE,
    _CONFIG_CACHE_LOCK,
    _HAS_JSONSCHEMA,
    _actual_type_name,
...
from codex_crew.config.validation import validate_config_data as _validate_config_data  # noqa: F401
from codex_crew.effort import is_valid_effort, model_supports_effort
from codex_crew.instances.constants import DEFAULT_CONNECT_TIMEOUT_SECS as _DEFAULT_CONNECT_TIMEOUT
from codex_crew.instances.constants import DEFAULT_MAX_RECOVERY_ATTEMPTS as _DEFAULT_MAX_RECOVERY
from codex_crew.instances.constants import DEFAULT_MINT_TIMEOUT_SECS as _DEFAULT_MINT_TIMEOUT
from codex_crew.instances.constants import DEFAULT_PROBE_FAILURE_THRESHOLD as _DEFAULT_PROBE_FAILS
from codex_crew.instances.constants import DEFAULT_RECOVER_BACKOFF_MAX_SECS as _DEFAULT_BACKOFF_MAX
from codex_crew.instances.constants import DEFAULT_SSH_COMPRESSION as _DEFAULT_SSH_COMPRESSION
from codex_crew.instances.constants import DEFAULT_TUNNEL_BASE_PORT as _DEFAULT_TUNNEL_BASE_PORT
from codex_crew.instances.constants import DEFAULT_WARM_SET_CAP as _DEFAULT_WARM_SET_CAP
from codex_crew.mcp_gateway.rewriter import default_overlay_dir, default_socket_path
from codex_crew.openai_profiles import normalize_profiles
...
# ``codex_crew.stt`` is the leaf ``config.paths``, never this module.
from codex_crew.stt.limits import DEFAULT_IDLE_EVICT_SECS as _STT_DEFAULT_IDLE_EVICT_SECS
from codex_crew.stt.limits import DEFAULT_PARTIAL_INTERVAL_MS as _STT_DEFAULT_PARTIAL_INTERVAL_MS
from codex_crew.stt.limits import DEFAULT_SILENCE_MS as _STT_DEFAULT_SILENCE_MS
from codex_crew.stt.limits import DEFAULT_TIMEOUT_SECS as _STT_DEFAULT_TIMEOUT_SECS
from codex_crew.stt.limits import MAX_IDLE_EVICT_SECS as _STT_IDLE_EVICT_SECS_MAX
from codex_crew.stt.limits import MAX_INTERVAL_MS as _STT_INTERVAL_MS_MAX
from codex_crew.stt.limits import MAX_TIMEOUT_SECS as _STT_MAX_TIMEOUT_SECS
from codex_crew.stt.limits import MIN_IDLE_EVICT_SECS as _STT_IDLE_EVICT_SECS_MIN
from codex_crew.stt.limits import MIN_PARTIAL_INTERVAL_MS as _STT_MIN_PARTIAL_INTERVAL_MS
from codex_crew.stt.limits import MIN_SILENCE_MS as _STT_MIN_SILENCE_MS
from codex_crew.stt.limits import MIN_TIMEOUT_SECS as _STT_MIN_TIMEOUT_SECS
from codex_crew.stt.models import DEFAULT_MODEL as _STT_DEFAULT_MODEL
...
CRED_OWNER_ID = "CODEXCREW_OWNER_ID"
...
# CODEXCREW_PORT is validated at CLI entry (cli.py main()).
# By the time loader.py is imported the env var is a valid int or absent.
DASHBOARD_PORT: int = int(os.environ.get("CODEXCREW_PORT", _DEFAULT_PORT))
...
# ``patch("codex_crew.config.loader.config_dir", ...)`` test seam working.


def _workspace_dir_file() -> Path:
    """Return the path to the saved workspace_dir file, respecting CODEXCREW_HOME."""
...
def workspace_root() -> Path:
    ...
    Resolution order:
    1. ``CODEXCREW_WORKSPACE`` env var (used as-is, no subdirectory appended)
    2. Saved path in ``config_dir()/workspace_dir`` (written by ``codexcrew setup``)
    3. Platform default with ``codexcrew-workspace`` subdirectory
    ...
    """
    override = os.environ.get("CODEXCREW_WORKSPACE")
    if override:
...
#: :meth:`CodexCrewConfig._load_resolved` and re-checked against the on-disk
...
def _persist_config_migration(
    ...
    *,
    ...
    a dashboard PATCH or ``codexcrew config set`` landing in between was silently
    ...
    ``asyncio.to_thread(CodexCrewConfig.load)``), so the interleave is reachable.
...
def denied_commands_path() -> Path:
    ...
    which do not route through the agent tool gate. Respects ``CODEXCREW_HOME``.
    """
...
def computer_use_state_path() -> Path:
    ...
    agent tool gate. Respects ``CODEXCREW_HOME``.
...
def oauth_endpoints_path() -> Path:
    ...
    the operator hand-edits the file out-of-band. Respects ``CODEXCREW_HOME``.
    """
...
def aws_consent_path() -> Path:
    ...
    ``codexcrew aws-consent`` CLI, both of which open the path directly rather
    than through this gate. Respects ``CODEXCREW_HOME``.
    """
...
def read_config_for_update(path: Path | None = None) -> dict:
    ...
    Pair this with :func:`codex_crew.atomic_write.atomic_write` on the way out so
...
def write_config_atomically(path: Path, data: dict, *, fsync: bool = False) -> None:
    ...
    **Windows gets a real owner-only DACL, not just the inert mode.** This used
    ...
    ``CodexCrewConfig.save()``. That constraint no longer exists: the lockdown is
    ...
    **On a network-homed data home the DACL turns on the CALLER, not the volume.**
    ...
    one, via :func:`codex_crew.atomic_write.on_event_loop`. A caller that has
...
def update_config_locked(
    ...
    *,
    ...
    ``config.json``".  :meth:`CodexCrewConfig.save` calls
    ...
    * Writers that reach ``config_path()`` through
      ``codex_crew.agent._atomic_json_write`` (``messaging.py``'s per-channel
    ...
    * Writers that go through :meth:`CodexCrewConfig.save` (``updates.py``'s
...
def _report_superseded_defaults(base_data: dict) -> None:
    ...
    *base_data* is the ``config.json`` document as read, BEFORE the
    ...
    short-lived ``codexcrew`` invocation, where the once-per-process guard below
    buys nothing because there the process IS the invocation. The per-key detail
    belongs on the surface the operator asked for: ``codexcrew config defaults``,
    ...
    logger.warning(
        ...
        "Run 'codexcrew config defaults' to see each one, '--adopt' to take the "
...
def workspace_dir_for(workspace: str | None = None) -> Path:
    ...
    Otherwise the value is relative to ``config_dir()`` (``~/.codex-crew/``).
...
def default_project_dir(workspace: str | None = None) -> str:
    ...
    """
    from codex_crew.security import is_sensitive_path  # circular import
...
def read_env_file_credential(key: str, env_file: Path | None = None) -> str:
    ...
    Same line format :meth:`CodexCrewConfig.load_credentials` parses (one pair
...
def inject_kiro_cli_api_key(env: MutableMapping[str, str]) -> MutableMapping[str, str]:
    ...
    consumed in-process from :meth:`CodexCrewConfig.load_credentials`, but this
...
# ---------------------------------------------------------------------------
# Validation helpers — used by CodexCrewConfig.load()
# ---------------------------------------------------------------------------
...
def resolve_loop_stall_exit_after(
    ...
    # INVOCATION_ID; old definitions are reported by ``codexcrew doctor`` with
...
def load_loop_stall_exit_after(
    ...
    """
    configured = CodexCrewConfig.load().dashboard.loop_stall_exit_after_secs
...
def _log_config_clamp_event(field: str, file_value: int, clamped: int, lo: int, hi: int) -> None:
    ...
    """
    try:
        from codex_crew.sel import SecurityEvent, sel
...
class CodexCrewConfig:
    ...
    instances: InstancesConfig = field(
        ...
        metadata=_meta(
            "Instances", "Multi-instance management — manage/switch remote Codex Crews over SSH."
        ),
    )
    ...
    tunnel: TunnelConfig = field(
        ...
        metadata=_meta("Tunnel", "Optional remote dashboard tunnel settings."),
    )
    hooks: dict = field(
    ...
    agents: dict[str, CodexCrewAgentConfig] = field(
        default_factory=dict,
        metadata=_meta("Agents", "Named Codex Crew agent definitions."),
    )
    default_agent: str = field(
        default="",
        metadata=_meta("Default Agent", "Active Codex Crew agent name from the agents section."),
    )
    ...
    #: /api/config/codexcrew`` (``useConnectionsUi.ts``), and that response drops
    ...
    #:
    #: Parsed through ``_safe_bool`` and defaulting False: a value Codex Crew
    ...
    snapshot_dir: str = field(
        ...
        metadata=_meta(
            "Snapshot Directory",
            "Directory for codexcrew snapshot output. "
            "Defaults to ~/.codex-crew/snapshots if empty.",
        ),
    )
    ...
    @classmethod
    def load(cls) -> CodexCrewConfig:
        """Load config from ~/.codex-crew/config.json, falling back to defaults.
        ...
        # than read there because codex_crew.env.mcp_search_path is reached from
        ...
        try:
            from codex_crew.env import publish_config_path_dirs
    ...
    @classmethod
    def _load_resolved(cls) -> tuple[CodexCrewConfig, int]:
        ...
        if cached_data is not None:
            ...
            # the default "codexcrew" agent in-memory (matching the on-disk
            ...
            if not loaded_base and not local_data:
                ...
                kiro = cfg.agent.default_agent or "codexcrew"
                cfg.agents["default"] = CodexCrewAgentConfig(
                    kiro_agent=kiro,
                    workspace="default",
                    memory_store="default",
                )
        ...
        # normalized form (see CodexCrewConfig.degraded_sections).
        ...
        # Parse agents section into dict[str, CodexCrewAgentConfig]
        raw_agents = data.get("agents", {})
        agents: dict[str, CodexCrewAgentConfig] = {}
        if isinstance(raw_agents, dict):
            for name, entry in raw_agents.items():
                if isinstance(entry, dict):
                    ...
                    agents[name] = CodexCrewAgentConfig(
                        ...
                        source=entry.get("source", "codexcrew"),
        ...
        cfg = cls(
            agent=AgentConfig(
                ...
                member_acp_backend=_normalize_acp_backend(
                    agent_data.get("member_acp_backend", "codex")
                ),
                openai_compatible_profile=str(
                    agent_data.get("openai_compatible_profile", "freechain") or "freechain"
                ),
                openai_compatible_profiles=normalize_profiles(
                    agent_data.get("openai_compatible_profiles", [])
                ),
                ...
                # from the `codexcrew-dashboard` MCP server, so an agent that does
            ...
            slack=SlackConfig(
                ...
                command=slack_data.get("command", "codexcrew"),
            ...
            stt=SttConfig(
                ...
                enter_sends=_safe_bool(stt_data.get("enter_sends"), True),
                timeout_secs=_safe_int(
            ...
            mcp=McpConfig(
                ...
                # codex_crew.env.augmented_path, so the ONE gate the built-in
        ...
        try:
            ...
            if needs_migration and not cfg._degraded_sections:
                _persist_config_migration(
                    path,
                    frozenset(pending),
                    default_kiro_agent=cfg.agent.default_agent or "codexcrew",
                )
    ...
    def save(self) -> None:
        """Write current config to ~/.codex-crew/config.json.
    ...
    def _resolve_agent_model() -> str:
        ...
        """
        agent_json = kiro_agents_dir() / "codexcrew.json"
    ...
    def acp_effective_model(
        ...
        verdict (``codex_crew.subagent._spawn_effective_model``) shares the code
        ...
        (``codexcrew`` itself and the no-agent case use the global directly) >
        ...
        elif not agent or agent == "codexcrew":
            m = global_model
        else:
    ...
    def resolve_session_effort(self, agent: str | None, crew_agent: str | None = None) -> str:
        ...
        worker agent (``codexcrew-lite`` / ``codexcrew-heartbeat``) takes the
    ...
    def load_credentials(self) -> dict[str, str]:
        """Load credentials from ~/.codex-crew/.env and environment variables.
        ...
        # ~/.codex-crew/.env is a bind-mounted empty file. setdefault() preserves
        # any value the caller already set explicitly.
        #
        # EXCEPTION: when the Docker entrypoint has deliberately scrubbed
        # credentials from the process environ (setting _CODEXCREW_CREDS_SCRUBBED=1),
        ...
        scrubbed = bool(os.environ.get("_CODEXCREW_CREDS_SCRUBBED"))
    ...
    def create_provider_factory(self) -> Callable:
        ...
        The configured ACP backend selects Codex, Claude, Kimi, a legacy public
        Kiro harness, or a named OpenAI-compatible profile. The factory accepts
        an optional ``session_key`` to create a per-session subdirectory under
        ``workspace_root()``.
        """
        from codex_crew.providers.acp import (
        ...
        def _acp(
            ...
            **_kwargs: object,
            ...
            #      passes the slot's own model, else the CodexCrew agent's
            ...
            #      applies to every agent, not just "codexcrew": an agent that
            ...
            from codex_crew.members import select_provider_backend
            ...
            _provider_env = dict(extra_env or {})
            _provider_model = m
            from codex_crew.acp_backends import ACP_BACKEND_OPENAI_COMPATIBLE

            if _backend == ACP_BACKEND_OPENAI_COMPATIBLE:
                from codex_crew.openai_profiles import profile_by_id
                from codex_crew.secrets import SecretVault

                profile = profile_by_id(
                    self.agent.openai_compatible_profiles,
                    self.agent.openai_compatible_profile,
                )
                profile_model = str(profile.get("model") or DEFAULT_MODEL)
                if not model_override:
                    _provider_model = profile_model
                _provider_env.update(
                    {
                        "CODEXCREW_OPENAI_BASE_URL": str(profile["base_url"]),
                        "CODEXCREW_OPENAI_MODEL": _provider_model or DEFAULT_MODEL,
                        "CODEXCREW_OPENAI_PROFILE_ID": str(profile["id"]),
                    }
                )
                secret_name = str(profile["secret_name"])
                try:
                    secret = SecretVault(config_dir()).get(secret_name)
                except (OSError, ValueError):
                    logger.warning(
                        "Could not read API key for OpenAI-compatible profile %s",
                        profile["id"],
                        exc_info=True,
                    )
                    secret = None
                api_key = secret.reveal().strip() if secret is not None else ""
                if not api_key:
                    raise ValueError(
                        "OpenAI-compatible profile "
                        f"{profile['id']} requires an API key in the Codex Crew secret vault"
                    )
                _provider_env["CODEXCREW_OPENAI_API_KEY"] = api_key
            return AcpProvider(
                work_dir=wdir,
...
def build_provider_factory(cfg: "CodexCrewConfig") -> Callable:
    ...
    """
    from codex_crew.platform.context import current_context, safe_context_call
...
def _workspace_name_for_dir(config: CodexCrewConfig, ws_dir: Path) -> str:
...
def refresh_materialized_agents() -> None:
    ...
    try:
        from codex_crew.context import invalidate_include_crew_context_cache
...
def _materialized_kiro_agent(agent_name: str | None, project_dir: str | None = None) -> str:
    ...
    CodexCrew alias. Without this lookup :func:`resolve_agent_bindings` would fall
    ...
    *project_dir* adds the session's own ``<project>/.kiro/agents`` scope, which
    ...
    against its cwd, and Codex Crew spawns it with the project dir as cwd). It
...
# Snapshot of the Codex Crew agent ALIAS table as ONE immutable
...
# by every successful :meth:`CodexCrewConfig.load`, exactly like
...
def publish_agent_alias_snapshot(config: "CodexCrewConfig") -> None:
    ...
    from :meth:`CodexCrewConfig.load` so every successful load refreshes it,
...
# :meth:`CodexCrewConfig.load`, for the same reason as the two snapshots above:
...
# it had changed something -- so a ``codexcrew config set`` landed on disk while
...
def publish_autocompact_pct(config: "CodexCrewConfig", ticket: int | None = None) -> None:
    ...
    :meth:`CodexCrewConfig.load` so every successful load refreshes it, including
...
# :meth:`CodexCrewConfig.load`, for the same reason as the three snapshots above:
# its read path runs on the event loop. ``codex_crew.cron._job_tz`` is reached
...
#
# Defaults to "" -- the dataclass default for :attr:`CodexCrewConfig.timezone` --
...
def publish_config_timezone(config: "CodexCrewConfig", ticket: int | None = None) -> None:
...
def published_config_timezone() -> str:
    ...
    unset :attr:`CodexCrewConfig.timezone` says, and both consumers already
...
def resolve_effective_agent(agent_name: str | None, project_dir: str | None = None) -> str:
    ...
    *project_dir* widens the "honored" set to the session's own ``.kiro`` scope
    ...
    if agent_name in aliases:
        # A Codex Crew alias resolves to itself (step 1 of resolve_agent_bindings).
        return ""
    if not _MATERIALIZED_AGENTS_READY:
...
def _project_scope_excludes(agent_name: str, project_dir: str) -> bool:
    ...
    """
    try:
        # circular import: agent_discovery imports codex_crew.hooks (the hardened
        # file-read gate), whose import closure reaches back into
        # codex_crew.config.loader — the same cycle documented at length on
        ...
        from codex_crew.agent_discovery import cached_project_agent_names
...
def _read_hardened_agent_spec(path: Path) -> dict | None:
    ...
    ``agent_discovery`` imports ``codex_crew.hooks``, whose closure reaches
...
def _project_declares_agent(agent_name: str, project_dir: str) -> bool:
    ...
    * **On the loop** — read the cache and nothing else, via a helper that makes no
    ...
    try:
        ...
        # codex_crew.config.loader in sys.modules. A module-scope import here would
        # therefore be a cycle; the deferral is load-bearing, not stylistic.
        from codex_crew.agent_discovery import (
...
def resolve_crew_identity(
    config: "CodexCrewConfig", agent: str | None, crew_agent: str | None
) -> str:
    """Canonical Codex Crew identity (a ``config.agents`` key) for a session.
...
def resolve_agent_bindings(
    ...
    *project_dir* is the session's active project directory, which widens step 2 to
    that project's own ``.kiro`` scope. It must be the same directory Codex Crew
    ...
    # An app agent is resolvable by kiro-cli but is not a CodexCrew alias, so it
...
def resolve_effective_model(
    ...
    1. the CodexCrew agent's own ``model``
    2. the bound kiro agent's pinned ``model`` (skipped for the built-in
       ``codexcrew`` agent, which tracks the global by design)
    3. the global ``agent.model`` default
    4. the installed ``codexcrew.json`` / bundled ``defaults.json`` model
    ...
    if kiro_agent and kiro_agent != "codexcrew":
...
def validate_kiro_agent_references(
    ...
    for mc_name, mc_agent in config.agents.items():
        if mc_agent.kiro_agent and mc_agent.kiro_agent not in installed_names:
            logger.warning(
                "CodexCrew agent '%s' references kiro agent '%s' " "which is not installed",
                mc_name,
                mc_agent.kiro_agent,
            )
