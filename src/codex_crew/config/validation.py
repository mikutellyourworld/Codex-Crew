"""Config validation + the validated-data cache for CodexCrew.
...
- ``_config_fingerprint()`` — it reads ``config_path()`` / ``config_local_path()``,
  which the test suite patches via ``codex_crew.config.loader.config_path``; keeping
...
``codex_crew.config.loader`` logger — not this module's ``__name__`` logger — to
...
#: ``CodexCrewConfig.degraded_sections``. Both halves exist for every path
...
#
# CodexCrewConfig.load() is called on per-message / per-request hot paths (skill
...
# name. Exposed so any lingering `codex_crew.config.loader._CONFIG_CACHE_LOCK`
...
def validate_config_data(data: dict) -> dict:
    ...
    # circular import: schema.py imports CodexCrewConfig from config.loader, which
    ...
    from codex_crew.config.schema import JSON_SCHEMA, SCHEMA_REGISTRY
    ...
    # must be excluded — otherwise every load of a config CodexCrew has ever
    # saved warns about CodexCrew's own bookkeeping.
