"""Pure filesystem path primitives for CodexCrew configuration.

This is a **leaf module**: it depends only on the standard library
(``os``, ``sys``, ``pathlib``, ``logging``) and imports nothing from
``codex_crew``. Modules that only need to locate ``~/.codex-crew/`` should import
from here directly::
...
factory) the way ``from codex_crew.config.loader import config_dir`` does.
...
``outbox_dir``, ``env_path``, …) remain in :mod:`codex_crew.config.loader` so that
their ``config_dir()`` lookups resolve in the loader namespace — preserving the
``patch("codex_crew.config.loader.config_dir", ...)`` test seam used across the
suite.

All names here are also re-exported from ``codex_crew.config.loader`` for
...
# Codex Crew owns a top-level home so it never reads or writes another agent's
# credentials or state by accident.
CONFIG_DIR_NAME = ".codex-crew"

# The pre-move top-level home (``~/.codexcrew``). Retained as a constant (not an
...
LEGACY_CONFIG_DIR_NAME = ".codexcrew"

# Recovery breadcrumb written beside the default data home. It records the
# resolved data location for support tooling but contains no secret. An explicit
# ``CODEXCREW_HOME`` override needs no breadcrumb.
RECOVERY_BREADCRUMB_NAME = ".codex-crew.breadcrumb"

OUTBOX_DIR_NAME = "outbox"

# Cross-platform workspace root for LLM working directories.
# Override: CODEXCREW_WORKSPACE env var or <config_dir>/workspace_dir
# macOS: /Volumes/workplace/codexcrew-workspace (fallback ~/workplace)
# Linux: ~/workplace/codexcrew-workspace
_WORKSPACE_DIR_NAME = "codexcrew-workspace"
...
# Memo for ``config_dir()``: ``(raw CODEXCREW_HOME, the home the entry was built
...
# keeps the override honoured the moment it changes (``CODEXCREW_HOME`` is
...
def _default_home() -> Path:
    """Resolve the default (non-override) data root: ``~/.codex-crew``."""
    return Path.home() / CONFIG_DIR_NAME


def _legacy_home() -> Path:
    """Resolve the pre-move top-level home: ``~/.codexcrew``."""
    return Path.home() / LEGACY_CONFIG_DIR_NAME


def legacy_home() -> Path:
    """Public alias for the pre-move top-level home (``~/.codexcrew``).
...
def _resolve_default_home() -> Path:
    """Resolve the default data root (``~/.codex-crew``), caching it for the process.
...
def _write_recovery_breadcrumb(data_home: Path) -> None:
    """Drop a recovery-pointer breadcrumb at ``~/.codexcrew.breadcrumb`` (best effort).
    ...
    try:
        ...
        content = (
            "CodexCrew data-home location pointer (safe to delete).\n"
            "\n"
            "CodexCrew stores its data (config, credentials, history, DBs) at:\n"
            f"    {data_home}\n"
            "\n"
            "This pointer lives beside the data home so support tools can find\n"
            "the configured location after a reinstall. It contains no secrets.\n"
...
# ``CODEXCREW_HOME=/etc/codexcrew`` accepted on macOS only, so the two platforms
...
def _is_unsafe_home(p: Path) -> bool:
    ...
    through, and CodexCrew would then create agent JSON inside ``/etc``.
    ...
    Shared by :func:`_valid_override_home` (``CODEXCREW_HOME``) and
...
def _valid_override_home() -> Path | None:
    """Return the resolved ``CODEXCREW_HOME`` override iff it is set AND valid.
    ...
    actually selected.
    """
    override = os.environ.get("CODEXCREW_HOME")
    if not override:
        return None
...
def shared_kiro_settings_writable() -> bool:
    ...
    Codex Crew data home, so it is resolved from the real home and a throwaway
    ...
    Keyed on ``CODEXCREW_POD`` rather than on the presence of a data-home override,
    because a custom ``CODEXCREW_HOME`` is a normal single-instance install (the
    ...
    what it sees.
    """
    return not os.environ.get("CODEXCREW_POD")


def config_dir() -> Path:
    global _config_dir_memo
    override_raw = os.environ.get("CODEXCREW_HOME")
    ...
    if p is not None:
        ...
        return p
    if os.environ.get("CODEXCREW_HOME"):
        logger.warning(
            "CODEXCREW_HOME=%s is a system directory, ignoring",
            os.environ.get("CODEXCREW_HOME"),
        )
    d = _resolve_default_home()
    d.mkdir(parents=True, exist_ok=True)
    # Drop the recovery-pointer breadcrumb beside the default data home.
...
def data_home() -> Path:
    ...
    1. A **valid** ``CODEXCREW_HOME`` override -> delegate to :func:`config_dir`
...
def config_package_dir() -> Path:
    """Return the installed ``codex_crew/config/`` directory.
...
def kiro_home() -> Path:
    ...
    but CodexCrew currently resolves the host ``~/.kiro`` for most of those readers
    ...
    moving where CodexCrew READS them, which breaks session resume. Only the agents
...
def kiro_sessions_dir() -> Path:
    ...
    so an instance that redirects its agent specs and then has CodexCrew read
...
def isolated_agents_dir(data_home: Path) -> Path:
    ...
    happens to be an ancestor of it (``CODEXCREW_HOME=$HOME`` is enough), which
...
def kiro_agents_dir() -> Path:
    ...
    Lives in this leaf module so :mod:`codex_crew.config.loader` can locate
    installed agent JSONs without importing :mod:`codex_crew.agent` — which
...
def project_agents_dir(project_dir: str | Path) -> Path:
    ...
    not find the repo root's agents. Codex Crew launches kiro-cli with the session's
    project directory as its cwd, so this is exactly the directory the backend
    itself searches for that session.

    Read-only by construction: nothing in Codex Crew writes here, because the
...
def project_kiro_dir(project_dir: str | Path) -> Path:
    ...
    Distinct from :func:`project_agents_dir` because Codex Crew additionally honors
    ``<project>/.kiro/*.agent-spec.json`` — a Codex Crew-only convention that
