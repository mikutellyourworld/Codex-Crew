...
# it does NOT reach ``codex_crew.acp`` — the package init (client + runtime) imports
...
# literal.
from codex_crew.acp_backends import resolve_selected_backend
...
from codex_crew.config.resolution import _OBSERVED_DEGRADED_SECTIONS, DEGRADED_TAILSCALE
from codex_crew.effort import EFFORT_LEVELS, is_valid_effort
from codex_crew.instances.constants import CONNECT_TIMEOUT_CEILING_SECS as _CONNECT_TIMEOUT_CEILING
...
from codex_crew.instances.constants import MAX_RECOVERY_ATTEMPTS_CEILING as _MAX_RECOVERY_CEILING
from codex_crew.instances.constants import MINT_TIMEOUT_CEILING_SECS as _MINT_TIMEOUT_CEILING
from codex_crew.instances.constants import MINT_TIMEOUT_FLOOR_SECS as _MINT_TIMEOUT_FLOOR
from codex_crew.instances.constants import (
    RECOVER_BACKOFF_MAX_CEILING_SECS as _RECOVER_BACKOFF_CEILING,
)
from codex_crew.instances.constants import WARM_SET_CAP_AUTO as _WARM_SET_CAP_AUTO
...
from codex_crew.stt.models import CATALOG as _STT_CATALOG
from codex_crew.stt.models import DEFAULT_MODEL as _STT_DEFAULT_MODEL
from codex_crew.stt.models import resolve as _resolve_stt_model
...
BACKGROUND_WORKER_AGENTS: tuple[str, ...] = ("codexcrew-lite", "codexcrew-heartbeat")
...
# Default endpoint for the anonymous usage beacon (see codex_crew/beacon.py).
...
class AgentConfig:
    ...
    provider: str = field(
        default="acp",
        metadata=_meta("Provider", "LLM provider transport (ACP).", enum=["acp"]),
    )
    mcp_registry_mode: bool = field(
        ...
        metadata=_meta(
            ...
            "name, so Codex Crew stamps that marker on the servers it manages. "
            ...
            "The administrator must also allow-list codexcrew-core, codexcrew-cron "
            "and codexcrew-computer in the registry by those exact names.",
        ),
    )
    ...
    acp_backend: str = field(
        ...
        metadata=_meta(
            "ACP Backend",
            "Which agent backend to drive. Codex is the default; authenticated "
            "OpenAI-compatible Chat Completions profiles are also available.",
    ...
    member_acp_backend: str = field(
        ...
        metadata=_meta(
            ...
            "Backend for crew-member DM sessions. Codex is the default; "
            "authenticated OpenAI-compatible profiles are also selectable. "
            "Members dispatch work through session-control tools mounted per "
    ...
    openai_compatible_profile: str = field(
        default="freechain",
        metadata=_meta(
            "OpenAI-compatible profile",
            "Named Chat Completions profile used by the OpenAI-compatible backend.",
        ),
    )
    openai_compatible_profiles: list[dict[str, str]] = field(
        default_factory=list,
        metadata=_meta(
            "OpenAI-compatible profiles",
            "Provider metadata only. API keys are stored separately in the encrypted vault.",
        ),
    )
    ...
    sweep_agents_backups: bool = field(
        ...
        metadata=_meta(
            ...
            "from the shared kiro agents directory. OFF by default: Codex Crew "
    ...
    sandbox: str = field(
        ...
        metadata=_meta(
            ...
            "Set to 'off' to skip Codex Crew's own OS-level sandbox — delegation "
    ...
    sandbox_allow_unsandboxed_exec: bool = field(
        ...
        metadata=_meta(
            ...
            "Windows host, a Linux kernel refusing user namespaces) `codexcrew "
    ...
    jail: str = field(
        ...
        metadata=_meta(
            ...
            "it. Disable per-invocation with --no-jail or CODEXCREW_NO_JAIL=1.",
    ...
    yolo_duration: str = field(
        ...
        metadata=_meta(
            ...
            "until_shutdown to keep it on with no timed expiry until Codex Crew "
    ...
    session_control: bool = field(
        ...
        metadata=_meta(
            ...
            "config: the tools come from the codexcrew-dashboard MCP server, so an "
    ...
    log_level: str = field(
        ...
        metadata=_meta(
            "Log Level",
            "Persistent log level for the codex_crew logger. "
    ...
    def __post_init__(self) -> None:
        ...
        # Clamp to [0.5, 60.0] to match ``CodexCrewConfig.load()`` behavior
...
class MessagingConfig:
    use_transport: bool = field(
        ...
        metadata=_meta(
            ...
            "monolith. Default ON in Codex Crew (the transport abstraction is the canonical "
...
class MemoryConfig:
    ...
    embed_model_url: str = field(
        ...
        metadata=_meta(
            ...
            "deployments). Empty uses the public Codex Crew CDN default; the "
            "CODEXCREW_EMBED_MODEL_URL env var wins over both. The download is "
    ...
    embed_model_path: str = field(
        ...
        metadata=_meta(
            ...
            "CODEXCREW_EMBED_MODEL_PATH env var wins over this.",
        ),
    )
...
class KnowledgeConfig:
    ...
    auto_add_documents: bool = field(
        ...
        metadata=_meta(
            ...
            "text -- Codex Crew fetches nothing itself, so the doc-ingest host "
    ...
    doc_ingest_hosts: list[str] = field(
        ...
        metadata=_meta(
            ...
            "Exact hostnames whose links may be fetched by CODEXCREW ITSELF and "
            ...
            "approval and Codex Crew fetches nothing. Applying it there would make "
...
class SlackConfig:
    ...
    use_tunnel_url: bool = field(
        ...
        metadata=_meta(
            ...
            "When true, dashboard links posted to Slack (e.g. via /codexcrew dashboard) "
...
class PublishConfig:
    ...
    relocate_roots: list[str] = field(
        ...
        metadata=_meta(
            ...
            "The sensitive-path denylist (~/.aws, ~/.ssh, ~/.codex-crew, …) still "
...
_MANAGED_SERVICE_ENV = "CODEXCREW_SERVICE_MANAGED"
...
class DashboardConfig:
    ...
    qr_session_until_restart: bool = field(
        ...
        metadata=_meta(
            ...
            "whether or not the gateway is still running. Either way `codexcrew "
    ...
    use_builtin_browser: bool = field(
        ...
        metadata=_meta(
            ...
            "When on, the browser tool opens pages in Codex Crew's built-in panel "
...
class CodexCrewAgentConfig:
    ...
    source: str = field(
        default="codexcrew",
        metadata=_meta("Source", "Agent origin: codexcrew or builtin."),
    )
...
class SkillsConfig:
    ...
    # ``codexcrew config set skills.auto_create_from_sessions true`` or the
    ...
    extra_paths: list[str] = field(
        ...
        metadata=_meta(
            ...
            "Local ~/.codex-crew/skills/ takes precedence for duplicate names.",
        ),
    )
...
class TelemetryConfig:
    ...
    enabled, a local-first JSONL sink under ``~/.codex-crew/metrics`` is activated;
    remote / OTLP egress is a separate opt-in requiring ``codexcrew[otlp]``.
    """
    ...
    enabled: bool = field(
        ...
        metadata=_meta(
            "Enabled",
            "Main switch for Codex Crew metrics telemetry. Off by default: metric "
            "call sites are no-ops and nothing is written. When on, a local-first "
            "JSONL sink under ~/.codex-crew/metrics is enabled (no network egress).",
        ),
    )
    local_dir: str = field(
        default="",
        metadata=_meta(
            "Local Metrics Dir",
            "Directory for local JSONL metric shards. Empty = ~/.codex-crew/metrics. "
            "Supports ~ expansion.",
        ),
    )
    ...
    beacon_enabled: bool = field(
        ...
        metadata=_meta(
            ...
            "and for a non-default CODEXCREW_HOME. Opt out with "
            "CODEXCREW_TELEMETRY_DISABLED=1 or by turning this off; an "
...
# GET /api/config/codexcrew response (which serializes a freshly loaded config)
...
# it: ``context_management`` does ``from codex_crew.config.loader import config_dir``, so
...
def stt_provider_is_coerced(value: object) -> bool:
    ...
    offers to remove it (``codexcrew config defaults``) cannot come to disagree with
...
def _validated_stt_provider(value: object) -> str:
    ...
    if value in _RETIRED_STT_PROVIDERS:
        logger.warning(
            ...
            "recognising the same speech. Run 'codexcrew config defaults --adopt' "
        ...
        logger.warning(
            ...
            "Run 'codexcrew config defaults --adopt' to drop the stored value.",
            value,
            STT_PROVIDER_LOCAL,
...
def _normalize_jail(value: object) -> str:
    ...
    ``off`` per-invocation is expressed via ``--no-jail`` / ``CODEXCREW_NO_JAIL``,
...
def _normalize_acp_backend(value: object) -> str:
    ...
    Delegates to :func:`codex_crew.acp_backends.resolve_selected_backend`, which owns
    ...
    that imports nothing from ``codex_crew.acp``, so it does not reproduce the
    package-init cycle (``codex_crew.acp.__init__`` -> client + runtime -> this
...
class ResolvedBindings:
    ...
    kiro_agent: str
    # The Codex Crew agent's own default model, "" when it pins none. Ranks below
    ...
    # The Codex Crew ALIAS whose bindings these are ("" when no alias applied). A
...
class SttConfig:
    ...
    enter_sends: bool = field(
        default=True,
        metadata=_meta(
            "Enter Sends While Dictating",
            "Let Enter send the message while dictation is live, from anywhere — not only when "
            "the composer happens to hold focus. Starting a recording puts focus on the mic "
            "button, so without this the panel's own 'Enter to send' hint does nothing. Turn it "
            "off to make Enter insert a newline while recording instead.",
        ),
    )
    timeout_secs: int = field(
        default=300,
...
class McpGatewayConfig:
    ...
    apps_enabled: bool = field(
        ...
        metadata=_meta(
            ...
            "Apps off stays off (tightest-wins: it beats CODEXCREW_MCP_APPS=1, and an "
    ...
    socket_path: str = field(
        ...
        metadata=_meta(
            ...
            "$CODEXCREW_HOME/mcp-gateway/gateway.sock. A unix socket at this path "
    ...
    overlay_dir: str = field(
        ...
        metadata=_meta(
            ...
            "Empty -> $CODEXCREW_HOME/mcp-gateway/agents.",
        ),
    )
    ...
    #: ``save()``. Carrying the roster lets :meth:`CodexCrewConfig.to_dict` emit the
    ...
    read_buffer_limit_bytes: int = field(
        ...
        metadata=_meta(
            ...
            "Env override: CODEXCREW_MCP_READ_LIMIT.",
        ),
    )
    response_spill_threshold_bytes: int = field(
        ...
        metadata=_meta(
            ...
            "written to ~/.codex-crew/mcp_spill/ and truncated inline to 16 KiB + "
            "a file path marker. Default 256 KiB. Set 0 to disable spilling. "
            "Env override: CODEXCREW_MCP_SPILL_THRESHOLD.",
        ),
    )
...
class McpConfig:
    ...
    extra_path_dirs: list[str] = field(
        ...
        metadata=_meta(
            ...
            "MCP launchers somewhere Codex Crew does not know about: a server "
...
class InstancesConfig:
    ...
    remote Codex Crew instances over SSH tunnels. Off by default — opt-in only,
    ...
    ``codex_crew.instances.constants`` so their canonical values cannot drift
    from this dataclass.
    """
    ...
    enabled: bool = field(
        ...
        metadata=_meta(
            ...
            "to remote Codex Crews and embed their dashboards. Default off (opt-in). "
    ...
    mint_timeout_secs: float | None = field(
        ...
        metadata=_meta(
            "Mint Timeout (secs)",
            "How long to wait for the remote `codexcrew token` mint to return "
...
class HeartbeatConfig:
    """Heartbeat background task queue (~/.codex-crew/workspace/HEARTBEAT.md)."""
...
class TunnelConfig:
    ...
    name_mode: str = field(
        ...
        metadata=_meta(
            "Name Mode",
            "Tunnel naming: 'username' uses 'codexcrew', "
            "'hash' uses 'codexcrew-<hostHash>' for multi-host disambiguation.",
            enum=["username", "hash"],
        ),
    )
...
def _parse_telegram_accounts(raw: object) -> dict[str, "TelegramAccountConfig"]:
    ...
    through :meth:`CodexCrewConfig.save` with its tokens and allow-lists intact;
...
class WhatsAppConfig:
    ...
    (``pip install 'neonize==0.4.3.post0'``; see :mod:`codex_crew.extras`).
