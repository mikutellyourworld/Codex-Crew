...
the stored document means. ``codexcrew config defaults`` lives in ``cli_config``,
...
def ack_file_path() -> Path:
    ...
    """
    from codex_crew.config.loader import config_dir  # circular import
...
def record_acks(dotted_keys: list[str]) -> list[str]:
    ...
    """
    from codex_crew.config.loader import config_path, update_config_locked  # circular import
...
def _stt_provider_is_coerced(value: object) -> bool:
    ...
    """
    from codex_crew.config.sections import stt_provider_is_coerced  # circular import
...
def coercion_summary(entry: CoercedValue, stored: object) -> str:
    ...
    return (
        ...
        f"that Codex Crew stops saying so on every load."
    )
...
def render_doctor_section(issues: list[str]) -> None:
    """Print the ``Stored Defaults`` section of ``codexcrew doctor``.
    ...
    if unacked:
        print("  fix:         codexcrew config defaults --adopt      (take the current defaults)")
        print("  keep:        codexcrew config defaults --keep       (affirm yours, stop reporting)")
