...
Session files: ~/.codex-crew/sessions/{safe_key}.jsonl
...
from codex_crew.executors import run_in_embed_pool  # noqa: F401 - facade re-export
from codex_crew.frontmatter import (  # noqa: F401 - facade re-exports
    SKILL_UPDATE,
    frontmatter_value,
)
from codex_crew.history_cache import (
    _METADATA_CACHE_MAX,
...
from codex_crew.history_consolidation import (  # noqa: F401 - facade re-exports
...
from codex_crew.history_projection import (
    SessionMetadataProjection,
    TranscriptReadProjection,
)
from codex_crew.history_rewrite import HistoryRewriteCoordinator
from codex_crew.history_search import (  # noqa: F401 - facade re-exports
    _CJK_CHAR_WEIGHT,
...
from codex_crew.llm_helpers import (  # noqa: F401 - facade re-exports
    ToolApprovalPolicy,
    background_turn,
    stream_and_collect,
    stream_and_collect_json,
)
from codex_crew.messaging.link import canonical_key, legacy_key
from codex_crew.preview_text import strip_markdown_preview  # noqa: F401 - facade re-export
from codex_crew.security import redact_credentials, redact_exfiltration_urls
from codex_crew.sel import sel  # noqa: F401 - facade re-export
from codex_crew.skills import (  # noqa: F401 - facade re-export
    AUTO_SKILL_MAX_PROCEDURE_CHARS,
    AutoSkillProvenance,
)
from codex_crew.skills_dedupe import (  # noqa: F401 - facade re-exports
    VERDICT_DUP,
    VERDICT_NEW,
    VERDICT_UPDATE,
    metadata_dedupe_verdict,
)
from codex_crew.skills_script_validator import validate_skill_script  # noqa: F401 - facade re-export
from codex_crew.vector_memory_constants import (  # noqa: F401 - facade re-exports
...
class OnLoopPersistError(AssertionError):
    ...
    ``CODEXCREW_STRICT_ON_LOOP_PERSIST`` or ``CODEXCREW_DEV_MODE``) any on-loop
...
_allow_on_loop_persist: contextvars.ContextVar[bool] = contextvars.ContextVar(
    "codexcrew_allow_on_loop_persist", default=False
)
...
def _on_loop_persist_strict() -> bool:
    ...
    - ``CODEXCREW_STRICT_ON_LOOP_PERSIST`` is truthy (explicit opt-in — the knob
    ...
    - ``CODEXCREW_DEV_MODE`` is truthy (developer gateway runs — where all
    ...
    A truthy ``CODEXCREW_STRICT_ON_LOOP_PERSIST`` wins; an explicit falsy value
    ...
    """
    explicit = os.environ.get("CODEXCREW_STRICT_ON_LOOP_PERSIST", "").strip().lower()
    if explicit in _ON_LOOP_TRUTHY:
        return True
    if explicit in _ON_LOOP_FALSY:
        return False
    return os.environ.get("CODEXCREW_DEV_MODE", "").strip().lower() in _ON_LOOP_TRUTHY
...
def on_loop_persist_strict() -> bool:
    ...
    The strictness knob (``CODEXCREW_STRICT_ON_LOOP_PERSIST`` /
    ``CODEXCREW_DEV_MODE``) governs the on-loop persistence discipline for every
...
def _resolve_retention_days() -> int:
    ...
    """
    try:
        return int(CodexCrewConfig.load().session.archive_retention_days)
    except Exception:
...
def _cleanup_old_archives(retention_days: int | None = None, base: Path | None = None) -> int:
    ...
    # the expensive CodexCrewConfig.load() disk read + parse.
    now = _time.time()
    ...
    # re-run the expensive CodexCrewConfig.load().
    _last_cleanup = now
