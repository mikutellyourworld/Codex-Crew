...
All atomic-write sites in CodexCrew should use this helper instead of
...
def _owned_roots() -> tuple[Path, ...]:
    """The directory roots Codex Crew itself creates and owns.
...
def _link_trust_anchor(parent: Path) -> tuple[Path, tuple[str, ...]] | None:
    ...
    directory is created by Codex Crew's own ``mkdir`` calls, so a link there was
...
def _refuse_linked_parent(path: Path) -> None:
    ...
    * and the resolved parent must EQUAL the path rebuilt from the resolved
    ...
    if split is None:
        # Outside every directory Codex Crew creates, a link is indistinguishable
...
def atomic_write_at(
    ...
    *,
    ...
    not expose and Codex Crew's pod backend does not use there.
    """
