...
from codex_crew.acp.types import PROVIDER_LABEL_CLAUDE, PROVIDER_LABEL_DEFAULT
...
from codex_crew import name_grant, platform_compat
from codex_crew.agent_discovery import cached_project_agent_names, list_agents
from codex_crew.config.loader import DEFAULT_MODEL, CodexCrewConfig
from codex_crew.constants import SUBAGENT_COMPLETION_PREFIX
...
from codex_crew.effort import effort_settings_key, model_supports_effort
...
from codex_crew.mcp_gateway import STUB_MODULE
from codex_crew.metrics.events import CHILD_PERMISSION_DENIED, emit_counter
from codex_crew.platform.context import redact_via_context
...
from codex_crew.resource_status import cached_admission_check
...
from codex_crew.session_workspace import result_path as _ws_result_path
from codex_crew.slack.format import extract_options
...
from codex_crew.subagent_cost import (
    append_cost_sample,
    compact_cost_log,
    read_learned_cost,
)
from codex_crew.subagent_manager import (
...
from codex_crew.subagent_persistence import (
    _agent_dir,
...
UNADVERTISED_AGENTS = frozenset({"codexcrew", "codexcrew-conductor", "codexcrew-pipeline-conductor"})
...
# Default raised to 900s and made tunable via ``CODEXCREW_INJECTION_TIMEOUT``
...
def _resolve_injection_timeout() -> float:
    ...
    val = _env_float("CODEXCREW_INJECTION_TIMEOUT", _DEFAULT_INJECTION_TIMEOUT)
...
def _subagent_default_model() -> str:
    ...
    """
    try:
        from codex_crew.config.loader import CodexCrewConfig, normalize_agent_model

        return normalize_agent_model(CodexCrewConfig.load().agent.role_models.get("subagent", ""))
    except Exception:
        return ""
...
def _subagent_default_effort() -> str:
    ...
    try:
        ...
        val = CodexCrewConfig.load().agent.role_efforts.get("subagent", "")
...
def _spawn_effective_model(model: str, agent: str) -> str:
    ...
    :meth:`CodexCrewConfig.acp_effective_model`, the same function the factory
    ...
    try:
        ...
        from codex_crew.config.loader import CodexCrewConfig
        from codex_crew.session import _session_model
...
#
# Tunable via ``CODEXCREW_SUBAGENT_DIGEST_HOLD_SECS``; 0/negative disables the
...
def _digest_hold_secs() -> float:
    try:
        val = float(os.environ.get("CODEXCREW_SUBAGENT_DIGEST_HOLD_SECS", ""))
...
def compute_max_subagents(cfg: CodexCrewConfig) -> int:
...
def resolve_max_subagents(cfg: CodexCrewConfig) -> int:
...
class SubagentManager:
    ...
    def __init__(
        ...
        try:
            self._global_approval_mode = CodexCrewConfig.load().agent.approval_mode
        except Exception:
            logger.warning(
                "Failed to load CodexCrewConfig for approval_mode; defaulting to interactive",
                exc_info=True,
            )
        ...
        try:
            self._result_ttl_secs = int(CodexCrewConfig.load().agent.subagent_result_ttl_secs)
        except Exception:
        ...
        try:
            self._spawn_stagger_secs = max(
                0.0, float(CodexCrewConfig.load().agent.subagent_spawn_stagger_secs)
            )
        except Exception:
    ...
    def _record_slow_command(info: SubagentInfo, idle: float) -> None:
        ...
        Writes to ``~/.codex-crew/subagents/slow_commands.jsonl`` (rotated at
    ...
    def _provider_label_of(provider: object) -> str:
        ...
        from codex_crew.providers.acp import provider_label
...
# existing integrations patch ``codex_crew.subagent.*`` after manager creation.
