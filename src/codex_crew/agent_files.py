"""Canonical filenames of the agent configs CodexCrew generates.

Single source of truth for the on-disk names CodexCrew writes into
...
Playwright convergence sweep must touch only CodexCrew-owned files) can import it
...
# The primary CodexCrew agent spec.
AGENT_FILENAME = "codexcrew.json"

# Background/auxiliary managed agent specs CodexCrew writes under ~/.kiro/agents/.
LITE_AGENT_FILENAME = "codexcrew-lite.json"
CONDUCTOR_AGENT_FILENAME = "codexcrew-conductor.json"
PIPELINE_CONDUCTOR_AGENT_FILENAME = "codexcrew-pipeline-conductor.json"
KNOWLEDGE_AGENT_FILENAME = "codexcrew-knowledge.json"
RESEARCH_AGENT_FILENAME = "codexcrew-research.json"
HEARTBEAT_AGENT_FILENAME = "codexcrew-heartbeat.json"

# Collective allowlists — the EXACT filenames CodexCrew owns in each dir. Used by
# the Playwright convergence sweep (browser/setup.py) so it rewrites only files
# CodexCrew generates, never a user's own agent config that happens to share a
# prefix (e.g. a hand-authored ``codexcrew-custom.json``).
...
#   * ``codexcrew.json``      — the agent behind user-facing chat.
#   * ``codexcrew-lite.json`` — the cheap background agent (auto-titles,
