...
file. :class:`~codex_crew.artifacts.ArtifactStore` re-validates ``source_path``
...
def project_root_marker(directory: str) -> str | None:
    ...
    if os.path.dirname(directory) == directory:
        return None
    # A HOME directory is never a project root either. CodexCrew's OWN data home
    # is ``~/.codex-crew``, so ``~/.kiro`` exists for every user -- which would
...
def classify_source(path: str) -> tuple[str, str]:
    ...
    0. **Unusable input → copy.** Not a non-empty ``str``, not absolute after
       ...
       :func:`~codex_crew.security.is_sensitive_path`. Nothing here is
