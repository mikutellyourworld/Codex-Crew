...
Codex Crew stored the Jira API token as a plaintext ``KEY=VALUE`` line in the
data home's ``.env`` (``~/.codex-crew/.env``). The encrypted vault
(:class:`~codex_crew.secrets.SecretVault`) supersedes that store, and the
...
from codex_crew.config.loader import _JIRA_TOKEN_RE, CRED_JIRA_API_TOKEN, config_dir, env_path
from codex_crew.mcp_gateway.secret_uri import SECRET_URI_PREFIX
...
def _env_lock_path(ep: Path) -> Path:
    ...
    Used by:
      ...
      * :func:`codex_crew.dashboard.handlers.weixin_qr._commit_credential_and_config`
        — the QR sign-in handler (the only other in-tree ``.env`` writer)
      * :func:`codex_crew.dashboard.handlers.messaging._write_env_updates`
...
def _parse_env_lines(text: str) -> list[tuple[str, str]]:
    ...
    Matches :meth:`CodexCrewConfig.load_credentials` parsing: strips whitespace,
...
def migrate_env_secrets(
    *,
    ...
    monkeypatching :func:`codex_crew.secrets.migrate.env_path` and
    :func:`codex_crew.secrets.migrate.config_dir`.
    """
    ep = env_path()
    ...
    for key in to_migrate:
        ...
        try:
            ...
            raise MigrationConflictError(
                ...
                f"the vault entry, then re-run `codexcrew secrets import --apply`."
            ) from exc
        if secret is None:
            ...
            raise MigrationConflictError(
                ...
                f"`codexcrew secrets import --apply` to reconcile."
            )
        ...
        if stored != to_migrate[key]:
            raise MigrationConflictError(
                ...
                f"`codexcrew secrets import --apply`."
            )
    ...
    try:
        if not platform_compat.try_acquire_lock(lock_fd, exclusive=True):
            raise MigrationConflictError(
                ...
                f"performed. Re-run `codexcrew secrets import --apply` to finish."
            )
        try:
            ...
            for key, value in to_store.items():
                ...
                if entry is None:
                    raise MigrationConflictError(
                        ...
                        f"overwritten. Re-run `codexcrew secrets import --apply` "
                        f"to reconcile."
                    )
            ...
            with vault.hold_cross_process_lock():
                ...
                for _key, _expected in to_migrate.items():
                    ...
                    if _entry is None:
                        raise MigrationConflictError(
                            ...
                            f"— re-run `codexcrew secrets import --apply`."
                        )
                ...
                if final_bytes != original_bytes:
                    raise MigrationConflictError(
                        ...
                        f"change is not clobbered. Re-run `codexcrew secrets import "
...
def format_report(report: MigrationReport) -> str:
    ...
    if report.dry_run:
        ...
        if not keys:
            ...
            lines.append("Codex Crew does not recognize as credentials were left untouched. If")
