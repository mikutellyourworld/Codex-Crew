...
Agent isolation:
    ...
    Codex Crew-mediated read of these files — tool reads (is_sensitive_path) and
    shell commands (is_sensitive_bash_command), including a scripted
    ``python -c "open('~/.codex-crew/.vault/...')"``. This is the same
...
from codex_crew.platform_compat import file_lock, restrict_to_owner
...
class SecretVault:
    ...
    def _encrypt_entry(self, name: str, plaintext: bytes) -> dict[str, str]:
        # Imported HERE, not at module top. `codex_crew.secrets` is reached from
