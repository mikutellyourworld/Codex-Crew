...
``CODEXCREW_HOST_PID`` (PID-namespace sandboxing strips ``CODEXCREW_SESSION_KEY``
...
* OUT OF SCOPE (unchanged from the env-only baseline): a same-uid agent
  ...
  (``CODEXCREW_HOST_PID=<victim pid>``) to reuse a legitimate sidecar. This
  is not a capability the sidecar adds — the identical attack defeats
  env-only resolution (``CODEXCREW_SESSION_KEY=<victim key>`` with the key
  ...
  ``~/.codex-crew`` — the exact capability that already lets it read the
  ...
  ``CODEXCREW_HOST_PID`` is launcher-declared, not attacker-chosen.
...
from codex_crew.sel import _sel_hmac_key_bytes, sel_hmac_key_path
...
# from :func:`codex_crew.sel.sel_hmac_key_path`, the single source of truth
...
# CODEXCREW_HOME while SEL's default dir does not — deriving the path
...
# sidecar directly.
_SUBKEY_DOMAIN = b"codexcrew.session_pid.sig.v1"
...
def _load_hmac_key() -> bytes | None:
    ...
    comes from :func:`codex_crew.sel.sel_hmac_key_path` so both protocols
    ...
    (:func:`codex_crew.sel.sel_hmac_key_bytes`). Without that fallback this
    protocol dies permanently the moment the resolved path stops resolving.
    :func:`codex_crew.sel.sel_hmac_key_path` now re-resolves per call (#2588), so
...
def signing_health() -> tuple[bool, Path]:
    ...
    For diagnostic surfaces (``codexcrew doctor``), which is the only place that
    ...
    construct :class:`~codex_crew.sel.SecurityEventLog` — creating the trust
...
def publish_session_pid(pid: int, session_key: str) -> None:
    ...
    Both files are written via :func:`codex_crew.atomic_write.atomic_write`
