...
CodexCrew has aggregate duration histograms (``codex_crew.metrics``) and a stall
...
def profiling_enabled(env: dict[str, str] | None = None) -> bool:
    ...
    stray ``CODEXCREW_DEBUG=0`` reads as disabled rather than as "the name is
...
class StackSampler:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run, name="codexcrew-perf-sampler", daemon=True
        )
        self._thread.start()
