#!/bin/sh
# Codex Crew first-time setup script (public build)
# Usage: source setup.sh   (bash or zsh), or: bash setup.sh
#
# Sets up Codex Crew using only public tooling:
#   1. Node.js (via ensure-node.sh)
#   2. Optional tools (git-lfs, ffmpeg for voice)
#   3. Agent backend: Codex ACP adapter (npm i -g)
#   4. Build frontend (npm/vite) + backend (pip)
#   5. PATH config
#   6. Agent config (codexcrew setup --agent-only)

# Resolve script directory (works in bash and zsh, sourced or executed)
if [ -n "$BASH_SOURCE" ]; then
    _codexcrew_dir="$(cd "$(dirname "$BASH_SOURCE")" && pwd)"
elif [ -n "$ZSH_VERSION" ]; then
    _codexcrew_dir="$(cd "$(dirname "${(%):-%x}")" && pwd)"
else
    _codexcrew_dir="$(pwd)"
fi
cd "$_codexcrew_dir" || return 1

ACP_NPM_PKG="@agentclientprotocol/codex-acp@1.10.0"

echo ">_ Codex Crew Setup"
echo ""
...
if [ -z "$_py" ] && [ -f "$_codexcrew_dir/ensure-python.sh" ] && _has_provisioner; then
    ...
    bash "$_codexcrew_dir/ensure-python.sh" >/dev/null 2>&1 || true
    _recorded="$(cat "${CODEXCREW_HOME:-$HOME/.codex-crew}/python-bin" 2>/dev/null || true)"
...
if [ -x "$_codexcrew_dir/ensure-node.sh" ]; then
    bash "$_codexcrew_dir/ensure-node.sh"
    ...
    # consume that marker the way the Makefile and codex_crew.env.node_bin_dirs()
    ...
    _nbd="$(cat "${CODEXCREW_HOME:-$HOME/.codex-crew}/node-bin-dir" 2>/dev/null || true)"
...
# ── 3. Agent backend (Codex ACP) ──

echo "── Step 3: Agent Backend ──"
if _check codex-acp; then
    echo "  ✅ codex-acp ($(which codex-acp))"
...
# Frontend: vite emits to website/dist; stage into src/codex_crew/static/dist
if _check node && [ -d "$_codexcrew_dir/website" ]; then
    echo "→ Building frontend (website/)..."
    if (cd "$_codexcrew_dir/website" \
            && { [ -f package-lock.json ] && npm ci --no-audit --no-fund --loglevel=error \
                 || npm install --no-audit --no-fund --loglevel=error; } \
            && npm run build); then
        _dist_src="$_codexcrew_dir/website/dist"
        _dist_dst="$_codexcrew_dir/src/codex_crew/static/dist"
        if [ -d "$_dist_src" ]; then
            rm -rf "$_dist_dst"
            mkdir -p "$(dirname "$_dist_dst")"
            cp -R "$_dist_src" "$_dist_dst"
            echo "  ✅ Frontend built and staged → src/codex_crew/static/dist"
        else
...
echo "→ Installing codexcrew (pip)..."
"$_venv/bin/pip" install --upgrade pip setuptools wheel -q 2>/dev/null || true
if CODEXCREW_SKIP_FRONTEND=1 "$_venv/bin/pip" install -e "$_codexcrew_dir" -q; then
    echo "  ✅ Build succeeded"
    # Record install method for tooling that branches on it
    echo "pip" > "$_codexcrew_dir/.install-method"
else
    echo "  ❌ pip install failed"
    cd - > /dev/null 2>&1
    return 1 2>/dev/null || exit 1
fi
# Symlink CLI so `codexcrew` works on PATH
mkdir -p "$HOME/.local/bin"
ln -sf "$_venv/bin/codexcrew" "$HOME/.local/bin/codexcrew"
if _check codexcrew; then
    echo "  ✅ codexcrew command available ($(which codexcrew))"
else
    echo "  ✅ codexcrew symlinked → ~/.local/bin/codexcrew (restart shell or fix PATH in Step 5)"
fi
echo ""
...
_add_to_rc() {
    ...
    # Remove any old Codex Crew PATH entry and replace it with the current one.
    if grep -qF "Codex Crew" "$_rc" 2>/dev/null; then
        sed -i.bak '/# Codex Crew/d;/Codex Crew.*bin/d;/\.local\/bin/d' "$_rc"
        rm -f "${_rc}.bak"
    fi
...
CODEXCREW_PROJECT_DIR="$_codexcrew_dir" codexcrew setup --agent-only \
    || echo "  ⚠️  codexcrew setup --agent-only failed (run manually later)"

echo ""
echo ">_ Setup complete!"
echo ""
echo "  codexcrew doctor     # verify everything"
echo "  codexcrew gateway    # start dashboard + gateway"
echo "  codexcrew chat       # interactive chat"
echo ""
