...
REQUIRED_ARTIFACT_NAMES = {
    "wheel": re.compile(r"^codexcrew-[A-Za-z0-9_.]+-py3-none-any\.whl$"),
    "sdist": re.compile(r"^codexcrew-[A-Za-z0-9_.]+\.tar\.gz$"),
    "appimage": re.compile(r"^CodexCrew-x86_64\.AppImage$"),
    "appimage_arm64": re.compile(r"^CodexCrew-aarch64\.AppImage$"),
    "deb": re.compile(r"^CodexCrew-x86_64\.deb$"),
    "deb_arm64": re.compile(r"^CodexCrew-aarch64\.deb$"),
    "rpm": re.compile(r"^CodexCrew-x86_64\.rpm$"),
    "rpm_arm64": re.compile(r"^CodexCrew-aarch64\.rpm$"),
    "mac_zip": re.compile(r"^notarized\.zip$"),
    "dmg": re.compile(r"^CodexCrew\.dmg$"),
}
...
OPTIONAL_ARTIFACT_NAMES = {
    "windows_installer": re.compile(r"^CodexCrew-Setup\.exe$"),
    "windows_blockmap": re.compile(r"^CodexCrew-Setup\.exe\.blockmap$"),
}
...
def _require_wheel_version_filename(logical_name: str, filename: str, wheel_version: str) -> None:
    prefix = f"codexcrew-{wheel_version}"
    if logical_name == "wheel":
        ...
        # spelling (codexcrew-1.2.3rc4-1-...), publishing bytes whose version
...
def create_manifest(
    ...
    *,
    ...
    if not re.fullmatch(r"ghcr\.io/[a-z0-9_.-]+/codexcrew", docker_image):
...
def validate_manifest(
    ...
    *,
    ...
    if not re.fullmatch(r"ghcr\.io/[a-z0-9_.-]+/codexcrew", image):
...
def resolve_candidate(
    *,
    ...
    docker_image = f"ghcr.io/{repository.split('/', 1)[0].lower()}/codexcrew"
