...
Codex Crew drives one first-class agent harness, ``kiro-cli``
...
## Usage
    ...
    python3 scripts/check_harness_parity.py src/codex_crew/acp/runtime.py
...
# ``codex_crew.acp`` executes that package's ``__init__`` (client + runtime), so the
...
# ``codex_crew.acp``, so every consumer can name the constants instead of copying
...
VOCABULARY_PATH = "src/codex_crew/acp_backends.py"
...
PROBES: tuple[tuple[str, str, str, str | None], ...] = (
    (
        "negative-property",
        "src/codex_crew/providers/acp.py",
    ...
    (
        ...
        "src/codex_crew/config/loader.py",
    ...
    (
        "sandbox-literal",
        "src/codex_crew/dashboard/handlers/agents.py",
