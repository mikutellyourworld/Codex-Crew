...
common_asserts() {
  cat <<ASSERTS
    ...
    stamp=\$(printf '%s\\n' "\$files" | grep -E '/codex_crew/_build_info[.]py\$' | head -1)
    ...
    # `/opt/CodexCrew Nightly/...` does. Parsing it would work on stable and return
    ...
    # cannot offer, and what makes \`codexcrew service install\` usable.
    printf '%s\\n' "\$files" | grep -qE '/backend-dist/codexcrew-backend/bin/codexcrew\$'
ASSERTS
}
...
docker run --rm -v "${ABS_DIST}:/dist:ro" -w /dist ubuntu:24.04 bash -euxc "
  ...
  apt-get update -qq
  # The package's OWN declared name, so a nightly build (codexcrew-nightly) is
