...
GENERIC_USER='user|users|otheruser|someuser|u|me|you|your|youruser|dev|developer|example|someone|somebody|alice|bob|carol|dave|eve|jane|john|foo|bar|baz|secret|weird|root|runner|ubuntu|node|builder|linuxbrew|codexcrew|codexcrew-workspace|mcp-gateway|nimbus|src|admin|test|tester|testuser|du|voce|você|tu|vous|usuario|usuário|utente|utilisateur|benutzer|<user>|\$USER|\$\{USER\}|\{user\}|%USERNAME%'
...
if [[ -n "$identity_matches" ]]; then
  ...
  red "  Replace with a generic placeholder (e.g. /home/user, redacted-email),"
