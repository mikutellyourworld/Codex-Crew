...
``src/codex_crew/computer_use/permissions.py``). Skipping one of those is how a
...
#   1. string literals   -- '../../../src/codex_crew/connections/registry.json'
...
_FRONTEND_FOREIGN = re.compile(
    ...
    | \.\.[\\/]\.\.[\\/]\.\.                          # ../../.. joined form
    | (?:['"]\.\.['"]\s*,\s*){2,}                     # '..', '..', ... segments
    | codex_crew                                       # backend package by name
    | \bpackaging\b
    | \bskills\b
...
_BACKEND_ROOTS = ("test", "transfer", "src/codex_crew/apps/builtins")
