...
* A file inside a NESTED DISTRIBUTION — some ancestor directory below the repo
  ...
  ``packages/codexcrew-client-py/``). That subtree has its own test runner and
...
_CFG_WITH_ROOTS = "[tool:pytest]\ntestpaths = test src/codex_crew/apps/builtins\n"
...
_PROBES = [
    ...
    (
        ...
        ["src/codex_crew/apps/builtins/foo/tests/test_x.py"],
        {},
        0,
    ),
    (
...
def self_test() -> int:
    ...
    assert roots == ["test", "src/codex_crew/apps/builtins"], roots
