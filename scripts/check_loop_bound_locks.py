...
``codex_crew.loop_lock.LoopBoundLock``, which keeps one inner lock per running
...
## Usage
    ...
    python3 scripts/check_loop_bound_locks.py          # scan src/codex_crew
...
SCAN_ROOT = os.path.join("src", "codex_crew")
MARKER = "loop-lock-ok"
PRIMITIVES = {"Lock", "Event", "Queue"}

REMEDY = (
    "Declare it as `codex_crew.loop_lock.LoopBoundLock()` (locks), or create the "
