...
splat (the shared mapping from ``codex_crew.subprocess_utf8``, which carries the
encoding).
...
# `encoding="utf-8"` / `**UTF8_TEXT` (see src/codex_crew/subprocess_utf8.py), or
...
def _fix_hint(path: str) -> str:
    return (
        ...
        'encoding="utf-8" / **UTF8_TEXT (src/codex_crew/subprocess_utf8.py), '
...
def _self_test() -> int:
    ...
    flagged_probes = {
        ...
        "wrapper run_limited": (
            "from codex_crew.sandbox import run_limited\n" "run_limited(['git', 'st'], text=True)\n"
        ),
    ...
    clean_probes = {
        ...
        "attribute UTF8_TEXT splat": (
            "import subprocess\n"
            "from codex_crew import subprocess_utf8\n"
