...
The product is **Codex Crew**: two words, a space between them, capital ``K``.
Everything else in the tree — the GitHub repo ``mikutellyourworld/Codex-Crew``, the
``codexcrew`` CLI, the ``codex_crew`` Python package, ``CODEXCREW_*`` environment
variables, the ``CodexCrew.dmg`` artifact — is an *identifier*, and identifiers
keep the spelling their system gave them. Only the **prose** name is gated.

## Why diff-scoped and not whole-tree

The tree carries thousands of prose ``CodexCrew``s that predate the convention.
...
# rendering of the *brand* rather than of an identifier: `codexcrew` all-lower is
# the CLI command and `CODEXCREW` all-upper is the env-var prefix, and neither is
# ever prose, so neither appears here. `(?<![A-Za-z0-9_])` / `(?![A-Za-z0-9_])`
# keep `CodexCrewApps` and `MyCodexCrewShim` out: those are single identifiers.
CONCATENATED = re.compile(
    r"(?<![A-Za-z0-9_])(?:CodexCrew|Kirocrew|kiroCrew|KiroCREW)(?![A-Za-z0-9_])"
)

# Spaced but uncapitalised. What keeps the `~/.codex-crew` data directory and the
...
UNCAPITALISED = re.compile(r"(?<![A-Za-z0-9_./\\-])codex crew(?![A-Za-z0-9_])")
...
# Shipped artifact filenames: CodexCrew.dmg, CodexCrew.exe, CodexCrew.app ...
# The list is closed on purpose. `CodexCrew.` followed by anything else — most
...
# Release-artifact suffixes: CodexCrew-x86_64.AppImage, CodexCrew-notarized-...,
# CodexCrew-Nightly. Deliberately a closed list rather than "hyphen means
# artifact", so prose like `CodexCrew-specific` is still reported.
...
# directory ``~/Library/Logs/CodexCrew Nightly`` and the Windows config directory
...
# it, so `<a href="https://example.com/">CodexCrew` must not read as "inside a URL"
...
def hyphen_interior(line: str, start: int) -> bool:
    ...
    The ``X-CodexCrew-Proxy`` header is the case that matters. Only the hyphen
    itself is required: demanding a word character before it as well would let
    this rule carry an untestable branch, since the tree holds no ``-CodexCrew``
    preceded by anything other than a word character. Prose is unaffected either
    way — ``CodexCrew-owned`` has a *space* before the brand, not a hyphen.
...
def url_spans(line: str) -> list[tuple[int, int]]:
    ...
    ``<a href="https://example.com/">CodexCrew`` out of the span: the brand there
...
# `tips_catalog.json` is derived from `src/codex_crew/docs/*.md`. Both are JSON, so
...
GENERATED_PATHS = (
    ...
    re.compile(r"^src/codex_crew/data/tips_catalog\.json$"),
)
...
def fenced_lines(lines: list[str]) -> set[int]:
    ...
    spellings in the docs (``cd CodexCrew``, ``git clone .../CodexCrew.git``), and
...
def scan_line(path: str, line_no: int, line: str, *, in_code: bool) -> Iterator[Violation]:
    ...
    for m in CONCATENATED.finditer(line):
        ...
        if hyphen_interior(line, start):
            continue  # X-CodexCrew-Proxy and friends
...
PROBES: tuple[tuple[str, str, bool], ...] = (
    ("prose", "Run CodexCrew on your laptop.", True),
    ("possessive", "This is CodexCrew's own sandbox.", True),
    ("sentence-end", "shipped with CodexCrew.", True),
    ("uncapitalised", "Install codex crew first.", True),
    ("cli-command", "Run `codexcrew serve` to start it.", False),
    ("env-var", 'os.environ["CODEXCREW_HOME"]', False),
    ("module", "from codex_crew.config import loader", False),
    ("repo-slug", "https://github.com/mikutellyourworld/Codex-Crew/issues", False),
    ("wrapped-slug", "see [docs](https://github.com/mikutellyourworld/Codex-Crew/issues)", False),
    ("url-query", "https://example.com/d?app=CodexCrew&v=2", False),
    ("markup-after-url", '<a href="https://example.com/">CodexCrew</a>', True),
    ("url-then-prose", "https://example.com/x is where CodexCrew lives", True),
    ...
    ("path-before", "built from ~/src/CodexCrew last night", False),
    ("path-after", "CodexCrew/website holds the frontend", False),
    ("windows-path-before", r"installed to C:\Program Files\CodexCrew", False),
    ("windows-path-after", r"launches CodexCrew\resources\app.asar", False),
    ("glued-identifier", "resolve CodexCrewApps from the registry", False),
    ("artifact-ext", "signs CodexCrew.exe and Update.exe", False),
    ("artifact-suffix", "publishes CodexCrew-x86_64.AppImage", False),
    ("channel-identifier", 'home / "Library" / "Logs" / "CodexCrew Nightly"', False),
    ("http-header", 'assert "X-CodexCrew-Proxy" in headers', False),
    ("hyphen-prose", "every CodexCrew-owned file", True),
    ("data-dir", "state lives under ~/.codex-crew/workspace", False),
    ("suppressed", "correct = 'CodexCrew'  # brand-ok: dictionary fixture", False),
)
...
def self_test() -> int:
    ...
    doc = ["Run CodexCrew.", "```bash", "cd CodexCrew", "```", "CodexCrew is done."]
    ...
    inline = list(scan_line("doc.md", 1, "clone `CodexCrew` then run CodexCrew.", in_code=False))
    if len(inline) != 1:
    ...
    for filler, glue in ((".", " "), ("a-", " "), ("x.com", " "), ("`", " "), ("a.", "")):
        long_line = filler * (200_000 // len(filler)) + glue + "CodexCrew"
    ...
    if list(scan_line("doc.md", 1, "run `codex crew` from a shell", in_code=False)):
    ...
    def ratio_of(base: int) -> tuple[float, float, int, int]:
        ...
        def once(count: int) -> tuple[float, int]:
            ...
            hits = len(list(scan_line("big.md", 1, "!CodexCrew" * count, in_code=False)))
    ...
    nested = ["````markdown", "```bash", "cd CodexCrew", "```", "````", "Then run CodexCrew."]
...
def report(violations: Iterable[Violation], *, enforcing: bool, base: str | None) -> int:
    ...
    if enforcing:
        print(
            ...
            "mikutellyourworld/Codex-Crew slug, CodexCrew.dmg-style artifacts, the CodexCrew Nightly "
            "channel, CODEXCREW_* env vars, the codexcrew CLI, codex_crew imports, and "
            ...
            "'CodexCrew-owned' does not become 'Codex Crew-owned' — hyphenating an open "
            "two-word name reads wrong. Write 'owned by Codex Crew' instead."
        )
