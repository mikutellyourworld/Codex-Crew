...
def _self_test() -> int:
    ...
    check("broad impact: ordinary file is not broad", has_broad_impact(["src/codex_crew/session.py"]) is None)
    ...
    check(
        ...
        has_broad_impact(["src/codex_crew/apps/builtins/auto_improvement/backend/clone_setup.py"]) is None,
    )
    ...
    diff_out = f"src/codex_crew/a.py\0{hostile}\0{newlined}\0"
    ...
    status_out = f" M src/codex_crew/a.py\0?? {hostile}\0 M {newlined}\0"
    sparsed = _parse_status_z(status_out)
    check("status -z strips only the 3-char prefix", "src/codex_crew/a.py" in sparsed)
    ...
    check("bucket: src is backend", surface_bucket("src/codex_crew/session.py") == "backend")
