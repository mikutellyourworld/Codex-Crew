...
API. ``src/codex_crew/docs/*.md`` is packaged and read at runtime, and specific
...
* ``fenced-path`` -- the same path question inside fenced blocks, for the two
  ...
  path and the argument of ``codexcrew run``.
...
# ``docs/`` is repo-only contributor/architecture material; ``src/codex_crew/docs/``
...
CODE_COUPLED_DOCS: dict[str, tuple[str, ...]] = {
    "src/codex_crew/docs/discord-integration.md": ("website/src/pages/settings/DiscordPanel.tsx",),
    "src/codex_crew/docs/feishu-integration.md": ("website/src/pages/settings/FeishuPanel.tsx",),
    "src/codex_crew/docs/imessage-integration.md": ("website/src/pages/settings/IMessagePanel.tsx",),
    "src/codex_crew/docs/slack-integration.md": ("website/src/pages/settings/SlackPanel.tsx",),
    "src/codex_crew/docs/teams-integration.md": ("website/src/pages/settings/TeamsPanel.tsx",),
    "src/codex_crew/docs/telegram-integration.md": ("website/src/pages/settings/TelegramPanel.tsx",),
    "src/codex_crew/docs/webex-integration.md": ("website/src/pages/settings/WebexPanel.tsx",),
    "src/codex_crew/docs/wecom-integration.md": ("website/src/pages/settings/WeComPanel.tsx",),
    "src/codex_crew/docs/weixin-integration.md": ("website/src/pages/settings/WeixinPanel.tsx",),
    "src/codex_crew/docs/whatsapp-integration.md": ("website/src/pages/settings/WhatsAppPanel.tsx",),
...
# The tips catalog scans ``src/codex_crew/docs/*.md`` but only surfaces docs named
# in this allowlist module; every allowlisted name must therefore still resolve.
TIPS_ALLOWLIST_MODULE = "src/codex_crew/tips_allowlist.py"
...
_UNRESOLVABLE_REF_OK: frozenset[str] = frozenset(
    {
        # -- This crew's own RUNTIME data, under ~/.codex-crew. Written by the code
        ...
        # Optional dev-only override read from CODEXCREW_PROJECT_DIR by
        ...
        # Codex Crew (`_scan_lineage_install`) -- all three name this relative path.
        "cron/jobs.json",
        #
        #
        ...
        # ~/.codex-crew/apps/pptx-maker/data/vendor/sdpm, a checkout `provision.py`
...
# ``src/codex_crew/run_coordinator/sqlite.py``" is correct writing, and 13 of the 20
...
# ``src/codex_crew/docs/tips.md`` would also be read as a repo-root
...
# (``src/codex_crew/mcp_tools/workflows.py::workflow_run``), and skipping on syntax
...
# spec's path, and whatever ``codexcrew run`` is handed.
_FENCED_TASK_SPEC_RE = re.compile(r"(?<![A-Za-z0-9_./-])docs/task-specs/[A-Za-z0-9_./-]*\.md")
_CODEXCREW_RUN_RE = re.compile(r"codexcrew\s+run\s+(?:-[^\s]*\s+)*([A-Za-z0-9_][A-Za-z0-9_./-]*)")
...
_PACKAGED_DOCS_DIR = "src/codex_crew/docs"
...
# (it lives in test/) and `CodexCrewClient` dead (it lives in packages/). The tail
...
def _is_regular_file(path: Path) -> bool:
    ...
    ``src/codex_crew/evil.py -> /dev/zero`` plus a citation naming it makes the read
            ...
            for name in sorted(filenames):
                ...
                for lineno, line in enumerate(text.splitlines(), start=1):
                    ...
                    for match in _CODE_DOC_REF_RE.finditer(line):
                        ...
                        if (
                            (root / ref).exists()
                            or (root / "src" / "codex_crew" / ref).exists()
...
def check_code_coupled_docs(root: Path, findings: Findings) -> None:
    ...
    ``src/codex_crew/docs/`` is packaged and read at runtime; specific filenames
...
def check_fenced_paths(root: Path, docs: list[Path], findings: Findings) -> None:
    ...
    ``docs/task-specs/...`` path and the argument of ``codexcrew run``. Both go
    ...
    (``codexcrew run TASK.md``), so only a path WITH a directory component is a
    ...
    for doc in docs:
        ...
        for _lineno, line in _iter_fenced_lines(text):
            ...
            tokens += [m.group(1) for m in _CODEXCREW_RUN_RE.finditer(line)]
            # Both patterns match a `codexcrew run docs/task-specs/...` line, and one
...
def _write_baseline(path: Path, triples: set[tuple[str, str, str]]) -> None:
    ...
    ``codex_crew.atomic_write`` is the repo's helper for this shape and is not used
...
def _self_test() -> int:
    ...
    def plant_missing_repo_path(root: Path) -> str:
        (root / "docs" / "ok.md").write_text(
            "# Ok\n\nThe entry point is `src/codex_crew/ghost.py`.\n", encoding="utf-8"
        )
        return "src/codex_crew/ghost.py"
    ...
    def allow_repo_path_that_exists(root: Path) -> None:
        ...
        (pkg / "real.py").write_text("a = 1\n", encoding="utf-8")
        (root / "docs" / "ok.md").write_text(
            "# Ok\n\nThe entry point is `src/codex_crew/real.py`.\n", encoding="utf-8"
        )
    ...
    def allow_repo_path_resolvable_one_root_down(root: Path) -> None:
        ...
        skill = root / "src" / "codex_crew" / "builtin_skills" / "demo" / "scripts"
    ...
    def allow_repo_path_in_a_proposal(root: Path) -> None:
        ...
        (rfc / "rfc-x.md").write_text(
            "# Rfc\n\nThis adds `src/codex_crew/planned.py`.\n", encoding="utf-8"
        )
    ...
    def plant_unrecorded_coupling(root: Path) -> str:
        ...
        (panel / "GhostPanel.tsx").write_text(
            ...
            "src/codex_crew/docs/ghost-integration.md'\n",
            encoding="utf-8",
        )
    ...
    def allow_coupling_named_only_in_a_comment(root: Path) -> None:
        ...
        (page / "ChatPage.tsx").write_text(
            "// See `src/codex_crew/docs/iframe-hosts.md` for the host list.\n"
            "export const x = 1\n",
            encoding="utf-8",
        )
    ...
    def allow_placeholder_identifier(root: Path) -> None:
        ...
        (root / "src" / "codex_crew" / "real.py").write_text("a = 1\n", encoding="utf-8")
        (root / "docs" / "ok.md").write_text(
    ...
    fact_immunity_probe(
        "bare filename after `codexcrew run`",
        CHECK_FENCED_PATH,
        allow_bare_filename_after_codexcrew_run,
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Lint the Codex Crew documentation trees and their indexes."
    )
    parser.add_argument(
        "--test",
        action="store_true",
