#!/usr/bin/env bash
# Write src/codex_crew/_build_info.py so a packaged build reports which install
# path it came from in the usage beacon's `dist` field.
#
# WHY A GENERATED MODULE, not an env var: `CODEXCREW_DISTRIBUTION` is inherited by
...
if [ -z "$PKG_DIR" ]; then
  PKG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/src/codex_crew"
fi
