...
Every file under ``src/codex_crew/builtin_skills/`` installs on every machine that
installs Codex Crew. A skill body that names THIS repository -- its GitHub slug, a
source path only a dev checkout has, a test file, a workflow file -- is giving an
instruction that resolves for exactly one reader: someone working on Codex Crew
...
The rule is absolute: a marker outside ``codexcrew-dev/`` fails. There is no
...
* ``mikutellyourworld/Codex-Crew`` -- this repository by name, including a github.com URL.
* ``src/codex_crew/...`` -- a SOURCE-CHECKOUT path. An installed wheel has
  ``codex_crew/`` with no ``src/`` prefix, so this form resolves only in a clone.
* ``test/test_*.py`` -- this repository's test layout.
* ``.github/workflows/*.yml`` -- this repository's CI.

## What is deliberately NOT a marker

The PRODUCT surface. ``codexcrew`` CLI commands, ``~/.codex-crew/...`` runtime
...
``src/codex_crew/``, and ``theme-pack-authoring`` legitimately cites the first one
...
## The codexcrew-dev family is exempt, structurally

``builtin_skills/codexcrew-dev/`` is the family FOR developing this repository --
``prepare-pr``, ``babysit``, ``writing-tests``, ``codexcrew-worktree-dev``. A
Codex Crew path there is the subject matter, not a leak, and 26 of the tree's 28
...
any path segment: a nested ``widgets/codexcrew-dev/`` would otherwise inherit the
...
SKILL_ROOT = Path("src") / "codex_crew" / "builtin_skills"

# The family whose subject matter IS this repository.
EXEMPT_FAMILY = "codexcrew-dev"
...
MARKER_CLASSES: tuple[tuple[str, str, str], ...] = (
    (
        ...
        # mikutellyourworld/Codex-Crew") is an instruction only a contributor can act on,
        # but an https://github.com/mikutellyourworld/Codex-Crew/... URL RESOLVES on every
...
REMEDY = (
    ...
    "codexcrew-dev/ if it is genuinely a skill for developing this repository."
)
...
def is_exempt(rel_path: Path) -> bool:
    ...
    segment would exempt a nested ``widgets/codexcrew-dev/SKILL.md``, and matching
    a prefix would exempt a ``codexcrew-devtools/`` sibling -- both would ship a
...
_PROBES: tuple[tuple[str, str, int], ...] = (
    ...
    ("product CLI is not a marker", "Run `codexcrew pod up mypod --provision`.\n", 0),
    ("runtime path is not a marker", "Scripts live under `~/.codex-crew/crons/`.\n", 0),
    (
        "installed package path is not a marker",
        "See `codex_crew/dashboard/theme_validate.py`.\n",
        0,
    ),
    ("repo slug", "File it on mikutellyourworld/Codex-Crew please.\n", 1),
    (
        "a resolvable public URL to this repo is NOT a marker",
        "See https://github.com/mikutellyourworld/Codex-Crew/blob/main/README.md\n",
        0,
    ),
    ("checkout path", "Edit `src/codex_crew/dashboard/state.py`.\n", 1),
    ...
    (
        ...
        "`src/codex_crew/x.py` is covered by `test/test_x.py`.\n",
        2,
    ),
    (
        "two markers on separate lines",
        "First `src/codex_crew/a.py`.\nThen `src/codex_crew/b.py`.\n",
        2,
    ),
)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        cases = (
            ...
            (
                ...
                f"{SKILL_ROOT.as_posix()}/codexcrew-devtools/SKILL.md" in flagged,
            ),
            (
