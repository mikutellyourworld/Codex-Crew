...
Codex Crew drives one first-class agent harness — `kiro-cli` — and adapts the
others. This gate is the mechanical half of that rule: it reads the lines a
change ADDS under `src/codex_crew/` and fails on any that let a harness other
...
| `sandbox-delegation` | H7 | `is_kiro_cli=` derived from a negation. This flag makes `wrap_argv` SKIP Codex Crew's seatbelt, so it fails OPEN |
...
- **`src/codex_crew/acp/types.py`**, for the two vocabulary rules only. It is the
