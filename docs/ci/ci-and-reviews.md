...
(`src/codex_crew/builtin_skills/codexcrew-dev/prepare-pr/SKILL.md`) is the agent
...
`.github/CODEOWNERS` assigns every repository path to `@kirodotdev/codexcrew-team`
...
| `builtin-skill-scope` | `scripts/check_builtin_skill_scope.py`, self-test first. Fails on a marker for THIS repository (its GitHub slug, a `src/` checkout path, a test or workflow file) inside a skill body under `src/codex_crew/builtin_skills/`, because those install on every machine and resolve for exactly one of them. The `codexcrew-dev/` family is exempt by directory, since this repository is its subject matter |
| `loop-bound-locks` | `scripts/check_loop_bound_locks.py`, self-test first. Fails on any module-global `asyncio.Lock()`/`Event()`/`Queue()` declaration — those bind to the import-time (or first-use) event loop and raise `RuntimeError` when acquired from another loop (Python 3.10+). #4800 converted the tree to `codex_crew.loop_lock.LoopBoundLock`; whole-tree, since the backlog is zero |
...
| `fenced-path` | A `docs/task-specs/**/*.md` path or a `codexcrew run` argument inside a fenced block that names no file. A fence is a sample everywhere else, but a reader PASTES these two |
...
skips the packaged user docs under `src/codex_crew/docs/`, where a task-spec path is
...
- **The e2e gateway boots with `CODEXCREW_STRICT_ON_LOOP_PERSIST=1`**, so an
  un-offloaded session-JSONL mutator that enters the lock on the event loop raises
  and fails the gate at PR time. `CODEXCREW_E2E_REQUIRE=1` turns an
...
- **`build-wheel`** builds the frontend, stages it into the package, builds the
  wheel, then `pip install dist/*.whl` and `codexcrew --version` as a smoke test.
...
`build-desktop.yml` in the release lane both build the real `codexcrew-backend`
...
then only upload the artifact. The wheel lane at least runs `codexcrew --version`.
...
`CODEXCREW_BIND=0.0.0.0` makes the gateway reachable from the host, that token auth
