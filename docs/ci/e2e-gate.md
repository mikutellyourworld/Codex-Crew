...
- `CODEXCREW_E2E=1` lifts the `skipif` on both files. Neither runs in a bare
  `pytest` invocation, which is deliberate: the browser leg takes minutes per
  interpreter, far too slow for the per-commit gate.
- `CODEXCREW_STRICT_ON_LOOP_PERSIST=1` turns the on-loop session-JSONL persistence
...
2. Points `CODEXCREW_KIRO_BIN` at `codex_crew.testing.fake_acp_backend`. That is
...
3. Boots a real gateway with `spawn_feature_gateway(fixture="minimal",
   approval="reads")`, on an isolated temporary `CODEXCREW_HOME` seeded
...
4. Exports the harness env into the Playwright child: `PLAYWRIGHT_BASE_URL`
   ...
   `CODEXCREW_E2E_EPHEMERAL=1`, `CI=1`, and `PLAYWRIGHT_JSON_OUTPUT_NAME`.
...
`CODEXCREW_KIRO_BIN` is restored (or removed) in a `finally` block, so the test
...
| `baseURL` | `process.env.PLAYWRIGHT_BASE_URL` or `http://localhost:5486` | 5486 is the default dashboard port, so an ad-hoc local run against a normal gateway works. |
...
## `CODEXCREW_E2E_REQUIRE=1`: why a graceful skip needs a marker
...
- **`CODEXCREW_E2E_REQUIRE` set** (the CI gate): `pytest.fail`. A skip counts as
...
`.github/workflows/ci.yml`'s `e2e` job sets `CODEXCREW_E2E_REQUIRE: "1"`. Set it
...
`website/dist` into `src/codex_crew/static/dist` so the specs render the real
