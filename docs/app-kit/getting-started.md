# Getting Started with CodexCrew Apps

Build, install, and run your first CodexCrew app in 5 minutes.

## Prerequisites

- CodexCrew installed and running (`codexcrew gateway`)
...
```json
{
  ...
  "description": "A CodexCrew app: My Dashboard",
  "author": "yourname",
...
`@codexcrew/app-sdk` hooks and `@codexcrew/app-sdk/ui` shared components.

> **You do not `npm install` `@codexcrew/app-sdk`.** The dashboard host provides
> it (and React, ReactDOM, lucide-react) at runtime through its import map: the
> bare `@codexcrew/app-sdk` specifier resolves to the host's vendored copy via
> `window.__codexcrew_modules`. This guarantees your app shares the host's exact
...
import { Card, CardTitle, PageHeader, StatCard } from '@codexcrew/app-sdk/ui'
...
Install via the CodexCrew dashboard REST API or the App Store UI:
...
curl -X POST http://localhost:5486/api/apps/my-dashboard/enable
```

Or open the CodexCrew dashboard → App Store → install from local path.

Your app now appears in the CodexCrew dashboard sidebar.
...
3. Update the installed app:
   ```bash
   curl -X POST http://localhost:5486/api/apps/my-dashboard/update
   ```
...
Available in `@codexcrew/app-sdk`:
...
| `useNavigate()` | Navigate to CodexCrew routes |
...
Also in `@codexcrew/app-sdk`, for an app that renders agent messages itself. An agent puts follow-up
...
Available in `@codexcrew/app-sdk/ui`:
...
For Python apps, CLI tools, or services that need to talk to CodexCrew Gateway:
...
The `codexcrew-client` package is async (uses `aiohttp`) and standalone — no
dependency on the CodexCrew main package. It covers the full Gateway API surface.
...
CodexCrew users can install it with one click.
