# API Reference — CodexCrew Gateway API & Client

Reference for the CodexCrew Gateway HTTP and WebSocket APIs, and how apps consume
them.

How you talk to the Gateway depends on where your code runs:

- **Dashboard UI pages (TypeScript/React)** — use the `@codexcrew/app-sdk` hooks
  ...
  specifier `@codexcrew/app-sdk` resolves to the host's vendored copy via
  `window.__codexcrew_modules`). See
...
- **Python apps / external CLI tools / services** — use the standalone
  `codexcrew-client` package, carried in this repository under
  `packages/codexcrew-client-py/`. It is async (`aiohttp`) and has no dependency on
  the Codex Crew main package, but it is not published to PyPI — use it from a source
...
The `@codexcrew/app-sdk` hooks are real and callable — see the next section; they
resolve from the host import map. The `codexcrew-client` Python package is **not
published**: it lives in this repository under `packages/codexcrew-client-py/`, is
outside the installed distribution, and has no release on PyPI, so `pip install
codexcrew-client` does not work. Use it from a source checkout, or call the
endpoints directly with `fetch` or `aiohttp`. Its method list is in
[Python Client](#python-client).

## App SDK Hooks (dashboard UI)

Dashboard UI pages import permission-scoped hooks from `@codexcrew/app-sdk`,
...
`ChatPanel` mounts Codex Crew's native chat experience for an existing session. The required
`slotKey` selects the session. By default, the component keeps the standard embedded ChatPage
behavior.

```tsx
import { ChatPanel } from '@codexcrew/app-sdk'
...
|---|---|---|---|
| `slotKey` | `string` | yes | Select the Codex Crew session rendered by the panel |
...
Apps resolve `@codexcrew/app-sdk` through the host import map, the same way they get the hooks:

```tsx
import { parseOptions, extractSteeringAcks, deriveFollowUpOptions } from '@codexcrew/app-sdk'
import type { ChatMessage, ParsedOptions } from '@codexcrew/app-sdk'
...
```jsx
import { ChatMessageList } from '@codexcrew/app-sdk'
...
`codexcrew-client` Python package covers part of this surface, and
...
the app secret from `~/.codex-crew/apps/{name}/.app_secret` and exchanges it
...
#### Watching something without paying for a model call (`codex_crew.irq`)

> **Provisional surface.** `codex_crew.irq` has exactly one probe today
...
> them, but expect `Observation` / `Tick` to gain fields, and pin the Codex Crew
...
Schedule a **script** cron instead and build it on `codex_crew.irq`, the
...
from codex_crew.irq import Observation, Probe, Severity, Tick, run
...
| `registerAppMcp(name, entry)` | `—` (no body) | Write MCP entry to `~/.codex-crew/mcp.json` (Node.js only) |
| `unregisterAppMcp(name)` | `—` (no body) | Remove MCP entry from `~/.codex-crew/mcp.json` (Node.js only) |
...
| `installSkill(name, srcDir)` | `void` | Copy skill directory to `~/.codex-crew/skills/` |
...
Verify that an incoming request was signed by the CodexCrew gateway reverse proxy. Use in app backends to authenticate proxied requests.
...
`packages/codexcrew-client-py/`. It is not published to PyPI, so use it from a
...
```python
CodexCrewClient(
    base_url="",              # default: http://localhost:{CODEXCREW_PORT or 5486}
...
Validate and serialize app.json manifests, via the `codexcrew-client` package.

```python
from codexcrew_client import AppManifest
...
```python
from codexcrew_client import CodexCrewClient, AppLifecycle

async with CodexCrewClient() as mc:
...
Manage the CodexCrew Gateway process (start, stop, health check).

```python
from codexcrew_client import GatewayManager

gm = GatewayManager(port=5486)
await gm.start()
...
All `codexcrew-client` errors are `CodexCrewError` instances with `code`,
`message`, `status`, `body`.
...
The `useAppApi()` hook and the `codexcrew-client` package wrap these Gateway
...
The gateway signs each proxied request with `X-CodexCrew-Proxy: <timestamp>:<hmac-sha256>`. The
...
A Python app backend whose environment can import `codex_crew` (the built-in app backends run as child processes and still import it) verifies this with the gateway's own helper:

```python
from codex_crew.apps.proxy_auth import raw_request_target, verify_proxy_request

body = await request.read()
if not verify_proxy_request(
    request.headers.get('X-CodexCrew-Proxy', ''),
...
A backend that cannot import `codex_crew` (a different language, or a Python
...
the value in the `X-CodexCrew-Proxy` header (constant-time), rejecting stale timestamps.
...
`~/.codex-crew/apps/<name>/ui` → your source tree — so the watcher sees edits at
...
- **CLI — `codexcrew app dev <name> [--off] [--confirm-out-of-install-root]`**:
...
(`~/.codex-crew/apps/.dev-grants.json`) mapping the app name to the ui root's
...
- **Out-of-install roots are refused over HTTP; confirm from the host.**
  ...
  the fix: run `codexcrew app dev <name> --confirm-out-of-install-root` on the
...
- **The flag is operator-only on the agent side too — three tiers.** First,
  ...
  (`~/.codex-crew/apps/.dev-grants.json`) is sealed read-only inside the
...
unstable cache** (a small sentinel file under `~/.codex-crew/apps/`, plus an
