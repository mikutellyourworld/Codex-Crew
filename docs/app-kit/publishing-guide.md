...
servers, cron jobs, backend routes, or dashboard UI pages to Codex Crew.
...
```bash
codexcrew app init my-app --ui --backend --cron
```
...
mirrored into the Codex Crew repo. Push a new version and the store picks it up on
the next refresh.
...
- Installing from a **local path** (`POST /api/apps/install`, `codexcrew app
...
Only declare `onUninstall` for state Codex Crew cannot see: app binaries outside
...
| Field | Description |
|-------|-------------|
| `managedBy` | Default resolution strategy. `"gateway"` resolves each entry through the edition's `CapabilityManager` seam; `"app"` means Codex Crew only checks existence. A per-entry object (`{"id": ..., "managedBy": ...}`) overrides it. |
...
Codex Crew's own agent config. `capabilities.mcp` are external servers your app
merely consumes.

Resolved dependencies are recorded in a reference-counting ledger at
`~/.codex-crew/dependency-ledger.json`. On uninstall each declared dependency is
...
```json
{
  "platform": {
    ...
    "installMode": "client",
    "clientInstall": {
      "shell": "git clone https://github.com/you/MyApp.git ~/MyApp && cd ~/MyApp && CODEXCREW_HOST={{gateway_host}} bash setup.sh",
...
| `installMode` | `"server"` | `"server"`: Codex Crew clones and installs. `"client"`: the app must be installed on the user's own machine. |
...
curl -X POST http://localhost:5486/api/apps/install \
  -H 'Content-Type: application/json' \
  -d '{"source": "./my-app"}'

curl -X POST http://localhost:5486/api/apps/my-app/enable
```
...
1. Open the dashboard (`codexcrew token`, then the printed URL).
...
```bash
curl http://localhost:5486/api/apps | python3 -m json.tool
curl http://localhost:5486/api/apps/my-app/manifest | python3 -m json.tool
```
...
curl -X POST http://localhost:5486/api/apps/my-app/update
```

For a tighter loop, turn on dev mode (`codexcrew app dev my-app`, or `POST
...
Install and update copy the source tree into `~/.codex-crew/apps/{name}/` with two
safeguards:
...
Code shipped inside the Codex Crew package (a built-in app) is exempt, but every
...
makes your app appear in the store *and installable*, with **no Codex Crew
...
[listing request template](https://github.com/mikutellyourworld/Codex-Crew/issues/new?template=app-store-listing.yml).
...
**The bundled seed** (`src/codex_crew/apps/app-registry.json` in the Codex Crew
...
Codex Crew release train. A catalog row for the same repository supersedes the
...
is not publicly writable. A seed change in the Codex Crew repo, by contrast,
follows the normal contribution flow and ships with the next release.

## 11. Federated external registries

A team can host its own registry without Codex Crew review per app. Users opt in by
...
(the PUT blocks adding the Codex Crew repo itself) and `POST
...
3. Check `minCodexCrewVersion`.
4. Clone into `~/.codex-crew/app-sources/{name}/` (persistent, one workspace per
...
7. For a gateway-managed app: copy into `~/.codex-crew/apps/{name}/`, register
...
- `minCodexCrewVersion` is checked on install and update; too-old gateways get a
  clear error telling the user to update Codex Crew first.
...
- [ ] `onUninstall` cleans up everything created outside
      `~/.codex-crew/apps/{name}/`, and nothing the gateway already manages
...
useful to Codex Crew users.

## 15. Quick reference

| Stage | Command or action |
|-------|-------------------|
| Scaffold | `codexcrew app init my-app` |
| Build UI | `cd ui && npm run build` |
| Install locally | `POST /api/apps/install`, or `codexcrew app install <dir>` |
| Enable | `POST /api/apps/{name}/enable`, or `codexcrew app enable <name>` |
| Live reload | `codexcrew app dev <name>` |
| Update local copy | `POST /api/apps/{name}/update` |
| List a registry app | Official catalog: open an [App Store listing request](https://github.com/mikutellyourworld/Codex-Crew/issues/new?template=app-store-listing.yml) issue (maintainer-curated). Bundled seed or a federated registry: add an entry to `app-registry.json`, open a pull request |
| User install | Apps page, Discover, Install |
| Ship an update | Bump `version`, push |

Bugs and feature requests: [GitHub
issues](https://github.com/mikutellyourworld/Codex-Crew/issues).
