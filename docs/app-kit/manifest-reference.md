...
| `minCodexCrewVersion` | string | Minimum Gateway version required |
...
- **A bare Python launcher** (`python`, `python3`, `py`, or the same with `.exe`)
  ...
  whose `args` launch a `codex_crew` module (`-m codex_crew...`) always gets the
  gateway's interpreter, since app venvs cannot import `codex_crew`.
...
- The host CLI name `codexcrew` is pinned to the running gateway before any of
  the above applies.
...
registration Codex Crew runs at startup and cannot be self-reported. Treat this as the
...
**If your app is SIGNED, set `minCodexCrewVersion`.** Contributions are covered by the
...
`codexcrew app init` scaffolds `assets/icon.png` and this field, so a new app
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `backend.entryPoint` | string | | Script to run (relative to app root), or a dotted Python module path launched via `python -m` (used by built-in apps like `file-explorer`, e.g. `codex_crew.apps.builtins.file_explorer.server`) |
...
`codexcrew app enable` is not that path. The CLI is a separate process with no
...
shipping a `config.py` would end up sharing one module. `from codex_crew...`
...
Control how CodexCrew manages the app:

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `lifecycle` | string | `"gateway"` | `"gateway"` (managed), `"app"` (self-managed), or `"locked"` (cannot uninstall) |
| `resources` | string | `"gateway"` | `"gateway"` (CodexCrew registers agents/skills/MCP) or `"app"` (app handles its own) |
...
when CodexCrew runs on a remote host).
...
coexist with older CodexCrew versions without breaking validation.
