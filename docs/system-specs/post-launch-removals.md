...
## Legacy `~/.codexcrew` security-path spelling

The one-time `~/.codexcrew` → `~/.codex-crew` data-home migration has been removed
...
"delete the code and its row together" rule above: the `.codexcrew`
**security-path spelling** still gates credentials in any leftover legacy home.

**Remove after:** confirming no supported machine can still have a `~/.codexcrew`
on disk — i.e. every legacy home has already been removed by its owner. Because
nothing migrates or deletes `~/.codexcrew` any more, a legacy home persists until
...
**What to delete then:** the `.codexcrew` spelling in `src/codex_crew/security.py`
(`_CREW_HOME_PREFIXES`, the `sensitive-file-read-cat-codexcrew-env` rule, and its
`test/fixtures/denied_commands_golden.json` entry). Keep `.codex-crew`. The
`codexcrew ... token` credential-exfil rule (`.*codexcrew.*token`) matches the CLI
*name*, not the path — leave it.

**Do NOT confuse with (these stay — permanent, not migration scaffolding):**
- the `~/.codex-crew` resolution itself, the `CODEXCREW_HOME` override, and the
  security keystone for `~/.codex-crew`;
...
- the recovery breadcrumb (`_write_recovery_breadcrumb`,
  ...
  move. (Its message string still names `~/.codexcrew` only as the file's own
