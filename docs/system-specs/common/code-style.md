...
Paths below are relative to `src/codex_crew/`.
...
| Dashboard port | `config/loader.py` | `_DEFAULT_PORT` (5486) and `DASHBOARD_PORT` (honors `CODEXCREW_PORT`). `dashboard/origin.py` imports `_DEFAULT_PORT` rather than restating it. |
...
| Cross-cutting shared constants | `constants.py` | `CODEXCREW_SPAWNED_ENV`, `ENV_TRUTHY`, `CHAT_TURN_TIMEOUT`, `COMPACT_WAIT_TIMEOUT_SECS` (one budget, shared by manual and automatic compaction), the `[OPTIONS:]` parse regexes, `DATA_WARNING`, `BANNER`. |
...
| Built-in skills | `builtin_skills/<name>/SKILL.md` | Frontmatter (`always`, `triggers`, `dir`) is the skill's own contract. This is the only tree copied into a user's `~/.codex-crew/skills/`, so a skill any shipped feature, tool or doc references MUST live here. The top-level `skills/` tree is repo-checkout-only and reaches no installed user. |
...
| Module-global asyncio primitives | Never a bare `asyncio.Lock()`/`Event()`/`Queue()` at module scope — it binds to the import-time (or first-use) loop and raises `RuntimeError` from any other loop (Python 3.10+). Use `codex_crew.loop_lock.LoopBoundLock` for locks, or create the primitive inside the coroutine. CI enforces this (`loop-bound-locks` gate). |
| Dataclasses | `@dataclass` for data containers |
| Product name | The product is **Codex Crew**: two words, a space, capital `K`. Identifiers keep the spelling their own system gave them (the `mikutellyourworld/Codex-Crew` repo slug, `CodexCrew.dmg` artifacts, the `CodexCrew Nightly` OS identifier, the `codexcrew` CLI, `CODEXCREW_*` env vars, `codex_crew` imports). CI gates the lines a change ADDS, so an existing spelling nearby does not exempt a new one; run `BRAND_BASE_REF=origin/main python3 scripts/check_brand_name.py` before pushing. |
...
```bash
python3 scripts/check_black_formatting.py && python3 scripts/check_subprocess_encoding.py && isort src/codex_crew test
flake8 src/codex_crew test && mypy src/codex_crew
python -m pytest
```
...
**On macOS, run `mypy --platform linux src/codex_crew`.** CI type-checks on Linux, and
...
and excludes `src/codex_crew/_vendor` from flake8 entirely. mypy and isort exclude
...
definition in `codex_crew.subprocess_utf8`: splat `**UTF8_TEXT` into the call
