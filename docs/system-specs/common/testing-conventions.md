...
re-export (e.g. `codex_crew.dashboard.handlers.X`, imported there from
...
monkeypatch.setattr("codex_crew.dashboard.handlers._SHUTDOWN_TIMEOUT_SECS", 0.05)

# RIGHT — patch where the constant is defined and read:
monkeypatch.setattr("codex_crew.dashboard.handlers.sessions._SHUTDOWN_TIMEOUT_SECS", 0.05)
```
...
against the dev machine (process-table scans, `~/.codex-crew` PID files), which
...
src/codex_crew/apps/builtins`) and they do **not** get the same fixtures. Know which
...
| `src/codex_crew/apps/builtins/*/tests/` | the rootdir `conftest.py`, plus that app's own `tests/conftest.py` where one exists (`auto_improvement`, `code_review_sage`, `spec_builder` have one; the other five apps do not) |
...
funnels against service mutation, pins `CODEXCREW_HOME` and the import-time `~/.kiro`
...
* `_restore_logger_levels` puts every logger's level and `disabled` flag back. A level is
  process-global AND hierarchical, so an explicit one left on `codex_crew` decides what
  ...
  `cli._setup_cli_logging` pins `codex_crew` at WARNING, and test modules across the suite
...
- Tests MUST NOT depend on `~/.codex-crew/` existing
- Tests MUST NOT write into the operator's real data dir. `CODEXCREW_HOME` is pinned
  ...
  1. **Bound at import time from `config_dir()`** — e.g.
     ...
     each use (e.g. `agent_state`) already honor `CODEXCREW_HOME`. A test that spawns
     subagents without isolating the import-time global leaks stub folders into
     `~/.codex-crew/subagents/`, which a running gateway then sweeps as orphans on its
     next restart.
  2. **Bound at import time from `Path.home()`** — `~/.kiro` is *kiro-cli's* home,
     ...
     `src/codex_crew` grows a module-level `Path.home()` binding that is neither in the
...
- Tests MUST NOT reconfigure or restart a real host service. This is enforced,
  ...
  `transfer` and `src/codex_crew/apps/builtins`) pins `$XDG_CONFIG_HOME` to a tmp
  dir so `dev_fleet._dropin_path()` cannot name the operator's real
  `~/.config/systemd/user/codexcrew-gateway.service.d/`, and traps every stdlib
...
- **Register the destruction of anything you create, in the same scope.** Prefer
  ...
  work is a guard somebody deletes. Set `CODEXCREW_TMP_RESIDUE_STRICT=1` to make it fatal,
  ...
  **Finding the culprit.** The residue report runs in a session-fixture teardown, so it
  ...
  Re-run the suspect subset with `CODEXCREW_TMP_PER_TEST=1` and each residue name
  becomes the id of the test that leaked it:

  ```bash
  CODEXCREW_TMP_PER_TEST=1 pytest src/codex_crew/apps/builtins/<app>/tests -n0 -q
...
- **~77 MiB is spent before collection starts** — interpreter, pytest, its
  ...
  `test/conftest.py` adds the rest, mostly `hypothesis` and `codex_crew.slack`.
- **~320 MiB imports the ~1,540 test modules** and, through them, most of
  `codex_crew`. The package's ~960 modules cost ~145 MiB to import on their own, so
  the product is a sixth of the floor, not a rounding error — `import codex_crew`
...
`~/.cache/codexcrew/test-slots/<hostname>`, one file per worker a run intends to
...
| `CODEXCREW_MAX_TEST_WORKERS` | Per-run ceiling, default 32. |
| `PYTEST_XDIST_AUTO_NUM_WORKERS` | xdist's own ceiling. Honoured here, because this hook replaces xdist's default implementation. Codex Crew seeds it with a memory-aware cap at every agent spawn boundary. |
| `CODEXCREW_TEST_SLOT_DIR` | Where the slot locks live. Point it at a throwaway dir to measure without contending with another run. |
...
```bash
python3 src/codex_crew/builtin_skills/codexcrew-dev/prepare-pr/scripts/prove.py
...
async def main():
    ...
    await wait_response(await send("initialize", {
        ...
        "clientInfo": {"name": "codexcrew", "version": "0.1.0"},
    }))
