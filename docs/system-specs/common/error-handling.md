...
`AcpTimeoutError` retains partial prompt output and carries the request `method`
when the timeout came from a named JSON-RPC request. Session-start callers use
that metadata to avoid replaying an ambiguous `session/new` or `session/load`:
the backend may have committed the request before its response missed the
deadline. `AcpRequestTimeout` is the shared-runtime equivalent and is likewise
propagated from session-start requests instead of being converted into a fresh
session.
