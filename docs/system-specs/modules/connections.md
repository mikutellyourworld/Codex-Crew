...
`src/codex_crew/connections/` (`registry.py`, `mint.py`, `warm.py`, `status.py`,
...
**Codex Crew never holds a connection's credential.** kiro-cli owns the OAuth chain
end to end; Codex Crew observes grant presence by `stat`, and every rule below follows
...
`accountLabel` is deliberately **absent**. Codex Crew never sees a provider
...
Cold mint (`codex_crew.connections.mint`) spawns one kiro-cli process per provider for one
approval URL: ~7.5s per card. `codex_crew.connections.warm` serves the whole gallery from one
process, and every rule below answers an observed failure.

**Placement.** Warm engine code remains in `src/codex_crew/connections/warm.py`; the dashboard
...
- **The census was incomplete.** Discovery merges exactly one agent spec
  (`codexcrew.json`), so a spec the user wrote by hand, or one an app materialized,
  ...
  target: Codex Crew does not own a user's agent file.
...
`~/.kiro/agents/codexcrew.lock` (`apps/bridges._mcp_lock`). They are different
...
rendered `<agents>/codexcrew.json` and from every `scope.agent_mcp_file` itself, so
...
A hand-written agent spec is the opposite: Codex Crew never writes it, so its entry
...
**Codex Crew holds no token.** kiro-cli owns the OAuth chain and injects the
...
promptless kiro-cli ACP session under the real `codexcrew` agent and executes two
...
`python3 -m codex_crew.connections.l1_smoke --report /tmp/l1.json` (under a
