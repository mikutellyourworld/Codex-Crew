...
Frontend invariants are covered in CodexCrewWebsite under `src/test/`:
...
- Build strategy: native CodexCrew implementation reusing only the
  upstream wire format for the ``chat.side_result`` event.
- Memory mode: side conversations are born ephemeral and never write to
  vector store, learn store, CodexCrew session JSONL, or the consolidation
