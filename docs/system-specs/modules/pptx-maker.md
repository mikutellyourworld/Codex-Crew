...
This app supplies the CodexCrew integration: the agents that drive the engine over
...
ported here. See `src/codex_crew/apps/builtins/pptx_maker/ATTRIBUTION.md`.
...
`deck_root()` resolves on every call — env override (`CODEXCREW_PPTX_DECK_ROOT`,
...
does not stage a BUILTIN app's non-manifest files into `~/.codex-crew/apps/<name>/`
...
`src/codex_crew/builtin_skills/pptx-maker/SKILL.md` (bundled, NOT the repo-only
...
`pip install codexcrew` always HAS the binary — but not necessarily on `PATH`: a
...
locked-down host may have none. Requiring it contradicts "`pip install codexcrew`
and nothing else".
...
`ENGINE_URL_ENV` (`CODEXCREW_PPTX_ENGINE_URL`) allows a mirrored/air-gapped
...
`engine_source.write_source_marker()` writes `.codexcrew-engine.json` (tag, commit,
...
a session, CodexCrew's `[OPTIONS: …]` question affordance in place of the engine's
...
`src/codex_crew/apps/builtins/...` would never be collected by CI):
