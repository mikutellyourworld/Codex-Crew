...
The `codex_crew.platform` package defines the **Composed Platform Providers
...
> Authoring note: CodexCrew is the public edition of this seam. The daily
...
reproduces today's CodexCrew behavior. An enterprise companion package (module
separate from `codex_crew`) depends on the public wheel and supplies enterprise
...
| `cfg` | carrier (`CodexCrewConfig`) | loaded config | same |
...
> **cloud deployment**: `codex_crew/deploy/` provisions S3, CloudFront, IAM roles and
...
1. `CODEXCREW_PROFILE` env (`standalone` | `enterprise`; unknown → standalone).
2. Non-empty `codexcrew.plugins` entry-point group (companion installed).
3. Identity signal: a present `~/.midway` directory (a cheap stat, no
   subprocess) — **only when the opt-in `CODEXCREW_MIDWAY_PROFILE_PROBE` env var
   ...
   The companion's managed launcher sets `CODEXCREW_MIDWAY_PROFILE_PROBE=1`.
...
`codexcrew.plugins` entry-point group via `importlib.metadata`:
...
3. **Verify-before-run signature** (`require_signature`) — the plugin ships a
   signed `codexcrew_plugin.json` manifest; admission verifies the signature
...
(`CODEXCREW_ADMISSION_POLICY` env path, else `~/.codex-crew/admission_policy.json`),
...
```toml
[project.entry-points."codexcrew.plugins"]
enterprise = "codexcrew_enterprise.compose:build_enterprise_context"
[project.scripts]
codexcrew-enterprise = "codexcrew_enterprise.cli:main"
dependencies = ["codexcrew"]
```
The `codexcrew-enterprise` binary sets `CODEXCREW_PROFILE=enterprise` and delegates to the
...
`codex_crew/__init__.py`** at packaging time (declared as optional package data;
absent in this repo and in the wheels it publishes). `codex_crew/__init__`
...
`codexcrew --version`, the diagnostics report, and the governance minimum-version
...
`cli_server.py` bind `from codex_crew import __version__` copies that nothing
...
the helpers in `codex_crew/__init__` are private. The accepted shape parses as a
...
- Credential redaction — all egress scrubs route through the single
  `codex_crew.platform.redact_via_context` shim (the one canonical
...
- Gate-side LOG lines (`codex_crew.platform.redact_log_via_context`) — the same
...
- Exfil exact-host heuristic exemption (`CredentialPolicy.exempt_exact_hosts()`) —
  ...
  through a FUNCTION-LOCAL import of `codex_crew.platform.context` (the `sel.py`
...
- Preflight checks (`IdentityProvider.preflight_checks()`) —
  `codex_crew.preflight.run_preflight_checks()` runs seam-supplied pre-launch
...
- `tunnel/manager.py` — the tunnel **lifecycle** routes through the seam. The
  ...
  **`ensure_available(*, install=True) -> str`** is a further v1 addition (no
  ...
  `codex_crew.platform.context`; `platform/` keeps zero imports of `codex_crew.tunnel`.
...
- `cli.py` `main` (wave 3 jail gate, factored into `_jail_reexec_gate`) +
  ...
  guard** — if the `CODEXCREW_JAILED` marker is PRESENT (any non-empty value) we
  ...
  invocation (`--no-jail` OR `CODEXCREW_NO_JAIL` truthy — `1`/`true`/`yes`/`on`
...
- `AppsLoader.default_registries() -> List[Dict]` — external app registries the
  ...
  consumption sites, never inside `CodexCrewConfig`, so a config save can never
...
- `AgentCatalogProvider.builtin_agents() -> List[Dict[str, Any]]` — ADD-only
  ...
  `--agent` against its cwd first, and which Codex Crew spawns with the session's
  project directory as that cwd, so the project entry is the one that would
  actually run. Codex Crew's legacy `<project>/.kiro/*.agent-spec.json` convention is
...
- `SkillDiscoveryProvider.skill_providers() -> List[SkillProvider]` — WIRED: the
  ...
  `codex_crew.skill_providers.base.SkillProvider` protocol and SHOULD expose an
...
`"codexcrew"` for `codexcrew.json` / `codexcrew-lite.json`, `"package"` for a
...
- **There is no browser-auth provider hook.** Browsing runs through the external
  `playwright-cli` binary (`codex_crew/browser_cli/`), and a logged-in session
...
- `config.knowledge.doc_ingest_hosts` (list) — SSRF-safe allowlist for the
  ...
  on it: the agent hands over text it already fetched, Codex Crew fetches nothing.
- `CodexCrewConfig._extra_sections` (private) — unknown top-level config.json
...
3. **An anti-rot gate.** `test/test_platform_cpp_seam_coverage.py` drives off
   ...
   `ast` analysis of `src/codex_crew` (excluding `platform/` itself and
