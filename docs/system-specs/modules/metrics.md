...
Source: the `src/codex_crew/metrics/` package. The tables below cover the modules
...
| File | Purpose |
|------|---------|
| `schema.py` | Namespace constants (`NS_CORE = "codexcrew."`, `NS_GENAI = "gen_ai."`, `NS_APP_PREFIX = "app."`) + `validate_name` / `validate_attrs` / `redact` guardrails. Documents the low-cardinality contract. |
...
That is what makes `codexcrew config set telemetry.enabled true` — a write from a
...
meantime. `CodexCrewConfig.load()` is a fingerprint-cache hit in the steady state
...
(`CODEXCREW_TELEMETRY`) and, when set, decides the effective state regardless of the
...
- **Namespace**: core callers must use `codexcrew.*` or `gen_ai.*`; app callers
...
- **Privacy**: string attribute values pass `redact()` — AKIA/ASIA keys,
  ...
  `codex_crew.security` scrubbers (`redact_credentials`,
...
`~/.codex-crew/config.json`):

| Field | Default | Meaning |
|-------|---------|---------|
| `enabled` | `false` | Main switch. Off = no-op recorder, nothing written. Editable from the dashboard (Settings → Privacy) as well as the config file, `codexcrew config set`, and the env var; re-resolved live, so a change takes effect without a restart. |
| `local_dir` | `""` | JSONL shard dir; empty = `~/.codex-crew/metrics`. `~` expansion supported. |
...
- **Config flag:** set `"telemetry": {"enabled": true}` in `~/.codex-crew/config.json`.
- **CLI:** `codexcrew config set telemetry.enabled true`.
- **Dashboard:** the recording switch in Settings → Privacy, which writes the same
  key through `PATCH /api/config/codexcrew` (`telemetry.enabled` is in
...
- **Env var:** export `CODEXCREW_TELEMETRY=1` (also accepts `true`/`yes`/`on`;
...
metrics are recorded; other codexcrew processes pick the value up on their own
...
|-----------|-------|-------|
| `service.name` | `codexcrew` | constant |
...
**Why the install id may egress while `codexcrew.process.start_time` may not.**
...
Scope of the `codexcrew.inventory.*` family, stated here because the names invite
...
the `codexcrew[otlp]` extra, so any aggregate describes one operator's opted-in
...
`CODEXCREW_AUTONUDGE=0`, so one env var left no process publishing inventory at all.
...
`CODEXCREW_HOME` and copies no crons, sessions, or databases, so its inventory belongs
...
bug. `codexcrew.approval.decisions` is the sharpest case: it counts gate VERDICTS,
...
acted. `codexcrew.subagent.spawned` fires on the confirmed-start funnel after
...
not see `codexcrew.embed.queue_wait` / `codexcrew.embed.inference` (millisecond
...
sized for session startup, so the first `codexcrew.turn.duration` sample ever
...
test_provider_bucket_views.py` scans the source for `codexcrew.*.duration` metric
...
### Bounded cardinality of `codexcrew.gateway.request.duration` (rec #1)
...
every other `codexcrew.*` metric (`other` list) so new emit call-sites appear
...
of the key is the resource-level `codexcrew.process.start_time` token the
...
Note that `codexcrew.mcp.lazy_load.*` is NOT the cold-spawn signal even though its
...
Separate from the OTEL histogram sink above (`~/.codex-crew/metrics/`, DELTA
...
| **Beacon (`beacon.py`)** | **Product analytics** | One anonymous ping per install per day | **Yes — to the CodexCrew endpoint** | `telemetry.beacon_enabled` (**on**) |
...
`src/codex_crew/beacon.py`, tests `test/test_beacon.py`. Fired from
...
2. **OTLP egress is an extra.** `opentelemetry-exporter-otlp-proto-http` ships
   in `codexcrew[otlp]`, not the default dependency set, so a beacon riding it
...
- The over-count is **larger here than for a typical CLI**: CodexCrew supports
  pods, worktree previews, and `CODEXCREW_HOME` overrides, so one person on one
...
1. `codex_crew/_build_info.py`, generated by `scripts/stamp-distribution.sh` and
   ...
   the default `-n auto`.
2. `CODEXCREW_DISTRIBUTION`, kept as a build/test override.
...
`telemetry_permitted()` suppresses both when `CODEXCREW_TELEMETRY_DISABLED` is
truthy, an enterprise **governance ceiling** pins `capabilities.telemetry` off,
the config toggle is false, the process looks like **CI**, `CODEXCREW_HOME` is
...
| `PATCH /api/config/codexcrew` | **403** on `telemetry.beacon_enabled=true`; writing `false` is always allowed (tightest-wins — a narrower local choice composes with the ceiling) |
| `codexcrew telemetry enable` | Exits **1** without writing config.json; `disable` still works |
| `codexcrew config set [--local] telemetry.beacon_enabled true` | Exits **1** without writing. Easy to miss: the *generic* setter reaches the same key, and `--local` writes `config.local.json`, which takes **precedence** over the base file — so leaving it ungated would make it the one remaining way to store `true` on a pinned host |
...
`is_default_home()` compares against `~/.codex-crew` **directly, never against
`config_dir()`** — `config_dir()` *honors* `CODEXCREW_HOME`, so comparing the two
always matches and the suppression would never fire (a real bug caught by
`TestDefaultHomeDetection`).

`codexcrew telemetry status | disable | enable` — `status` prints the exact
payload and never materializes an id (`install_id(create=False)`). Its numbered,
choose-one opt-out list leads with `codexcrew telemetry disable` because that choice
...
The GUI twin of `codexcrew telemetry disable`. It writes the **same** key —
`telemetry.beacon_enabled` via `PATCH /api/config/codexcrew` — so the two controls
...
- `env_override` reports specifically whether `CODEXCREW_TELEMETRY_DISABLED` pins
...
- `overlay_override` does the same for a `config.local.json` entry.
  ...
  the toggle and names the file (the same case `codexcrew telemetry disable`
...
- The handler routes `CodexCrewConfig.load()` and the overlay probe through
...
- **The first heartbeat is withheld until that chapter is acknowledged.** The
  ...
  `codexcrew telemetry enable|disable` sets it too, for headless hosts), which
...
consent ladder (`telemetry.beacon_enabled`, `CODEXCREW_TELEMETRY_DISABLED`, the
...
| `v` | CodexCrew release normalized by `beacon.release()`; build stamps are removed. |
...
3. **Lenient decode** (`errors="replace"`). A strict decode raises
   ...
   escaped the callers' handlers and killed `codexcrew telemetry status`.
...
client ─GET /b/1/<id>?v&py&dist&first_seen─> CloudFront E1YM983XX3ASBM
                                     ...
                                     ▼
        standard logging v2 → s3://codexcrew-beacon-logs (PERMANENT, tiered)
                                     ▼
              Athena codexcrew_analytics.beacon_logs (partition projection)
                                     ▼
        codexcrew-beacon-aggregator Lambda (daily 00:20 UTC) writes BOTH:
                    ├── CloudWatch CodexCrew/Product  → dashboard (~15-month view)
                    └── codexcrew_analytics.beacon_daily → PERMANENT record
```
...
- **Raw logs** — `s3://codexcrew-beacon-logs`. The lifecycle policy has **no
...
- **Daily rollup** — `codexcrew_analytics.beacon_daily` (Parquet, stays in
...
`s3:PutObject`/`s3:DeleteObject` on `codexcrew-beacon-logs/rollup/*` **only** —
...
`codexcrew-models` (distribution E2UX23B48LKM6V, OAC-only bucket access). The
