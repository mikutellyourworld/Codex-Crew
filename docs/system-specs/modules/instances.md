...
Lets a single Codex Crew gateway (the **hub**) manage and switch between several
**remote** Codex Crew instances (dev hosts, EC2, home servers) over SSH **or AWS
...
> **Codex Crew** and from an agent **crew** (an assistant with its own
...
> **Section numbers in this document are an API.** `src/codex_crew/cloud/connect.py`
...
Code: `src/codex_crew/instances/` (registry, tunnel manager, port allocator, token
mint, diagnostics, injection validation, run-marker) plus
`src/codex_crew/dashboard/handlers_instances.py` (control plane) and the frontend
...
A Codex Crew gateway normally binds the dashboard to loopback only. The Instances
...
```bash
codexcrew config set instances.enabled true
codexcrew restart
```

Settings → Remote Instances offers the same toggle (it PATCHes
`instances.enabled` through `/api/config/codexcrew`) and then shows a
...
```
 ...
 |            |                                                          |
 |  instances/ package                                                   |
 |   |- registry.py         ~/.codex-crew/instances.json                  |
 |   |- port_allocator.py   free-loopback-port probe (base 7778)         |
 |   |- token_mint.py       ssh <host> codexcrew token -> JWT (never logged)|
 ...
 |  codexcrew gateway bound to 127.0.0.1:<remote_port> (registry default  |
 |  5486, the port a stock gateway binds)                                |
 +-----------------------------------------------------------------------+
```
...
| `token_mint.py` | Runs `codexcrew token --ttl --port --embed-parent-port` on the remote over SSH (run-marker first, then a bin-candidate ladder) and parses the JWT out of the printed URL. Token is returned in memory only, **never logged**. |
...
| `run_marker.py` | Records the running gateway's own `codexcrew` launcher (and pid) keyed by port, so a remote mint execs the same venv the live gateway runs from. Also backs zero-config client port discovery. See §12. |
...
6. **Stored-token liveness probe.** A token can go stale while the tunnel stays
   CONNECTED (a failed self-heal re-mint, or a remote `codexcrew restart` that
...
7. **Diagnose / restart.** `?diagnose=1` runs the probe ladder on demand;
   `POST .../restart` runs `codexcrew restart` on the **remote** over SSH
...
Transport defaults and bounds live in `codex_crew.instances.constants` and are
...
```bash
codexcrew config set instances.warm_set_cap 3
codexcrew config set instances.ssh_compression false
codexcrew config set instances.connect_timeout_secs 45
codexcrew config set instances.mint_timeout_secs 60
```
...
`~/.codex-crew/instances.json`, one record per instance:

```
id, name, ssh_host, remote_port (default 5486), local_port (0 = unallocated),
...
- **Addressing fields locked for a correlated cloud instance.** `PATCH
  ...
  cloud launch job store (`codex_crew.cloud.connect.is_launched_instance()`) —
  i.e. Codex Crew provisioned it, as opposed to a hand-added SSM record. These
...
1. **Enable** on the hub: `codexcrew config set instances.enabled true && codexcrew restart`
...
3. **Add** an instance:
   ...
   - *Remote codexcrew path*: only needed when `codexcrew` lives somewhere
...
> has `codexcrew` installed with a gateway running on its loopback port.
...
`BatchMode` will not prompt), and `codexcrew` installed with a gateway running on
...
**The cloud launcher registers instances here.** `codexcrew cloud launch`
...
no SSH key, no inbound port, and no hand-edited `~/.ssh/config`. `codexcrew cloud
...
| Every token mint fails on one remote, though its gateway is healthy | The remote's `~/.local/bin/codexcrew` probably points at an uninstalled checkout. See §12: the run-marker is what makes mint follow the *running* gateway's install. |
...
`<data-home>/run/gateway-<port>.bin` (the running gateway's own `codexcrew`
...
Token mint SSHes to the remote and resolves `codexcrew` from a fixed PATH
candidate list whose first entry is `$HOME/.local/bin/codexcrew`. When that
...
remote's non-interactive SSH shell usually does not export `CODEXCREW_HOME`:

1. `$CODEXCREW_HOME` when set and non-empty,
2. `$HOME/<CONFIG_DIR_NAME>` (the current default, `.codex-crew`),
3. `$HOME/<LEGACY_CONFIG_DIR_NAME>` (`.codexcrew`, for a not-yet-migrated remote).

Those two home segments are **interpolated from the shared
`codex_crew.config.paths` constants**, the same ones the marker *writer* derives
...
`restart_remote()` resolves `codexcrew restart` through the same path, keyed by
the instance's `remote_port`.

The launcher path is derived from `sys.executable`'s sibling console script
(`codexcrew`, or `codexcrew.exe` on Windows) and is deliberately **not** resolved
...
script exists (a source-tree `python -m codex_crew` launch) the marker is written
...
entirely. Resolution order is `--port`, then `CODEXCREW_PORT`, then a port named
by `dashboard.url`, then the sole gateway-owned marker, then the default 5486.
...
`CODEXCREW_PORT`. This module deliberately offers no bare "is something
...
|--------|----------------|----------------------|-----------|
| `ssh` (default) | `ssh -N -L 127.0.0.1:LP:127.0.0.1:RP <ssh_host>` | non-interactive SSH access | `ssh <host> codexcrew token` |
| `ssm` | `aws ssm start-session --document-name AWS-StartPortForwardingSession --target <ssm_target> --parameters portNumber=RP,localPortNumber=LP` | AWS CLI + `session-manager-plugin`; `ssm:StartSession`, `ssm:SendCommand`, `ssm:GetCommandInvocation` | `aws ssm send-command` → `codexcrew token` |
...
- **Process-tree teardown (all platforms).** The SSM child gets process-group
  ...
  wrapper-only termination on Windows, which Codex Crew supports.
...
in the gateway process, which has no `CODEXCREW_SESSION_KEY`, so the launcher's
...
the reachability lives in ssh config and Codex Crew is unaware of it.
...
Code: `src/codex_crew/dashboard/session_transfer.py` (bundle + importer),
