...
`codex_crew.history` remains the compatibility facade and defines the real
...
an executable CodexCrew session key. The normal ConversationLog metadata/message
...
- **On-loop offload discipline is enforced, not convention-only**: the offload
  ...
  `CODEXCREW_STRICT_ON_LOOP_PERSIST=1` or `CODEXCREW_DEV_MODE`) so an un-offloaded
...
- **Foreign-append merge & id-first dedup** (`_frozen_prefix_and_foreign_appends`):
  ...
  **count-bounded** (deques of window-entry indices; each disk line matches at
  ...
  - **Successor identity, landed on the save side.** The **creation-time
    ...
    [issue #381](https://github.com/mikutellyourworld/Codex-Crew/issues/381)) is now the
...
- **Archive location**: `~/.codex-crew/sessions/archive/{key}__{YYYYMMDD-HHMMSS}.jsonl`,
