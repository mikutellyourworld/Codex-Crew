...
| [harness-parity.md](harness-parity.md) | The invariants keeping Codex and OpenAI-compatible sessions on their intended transport and governance paths. |
...
| [agent-interrupt-controller.md](agent-interrupt-controller.md) | `codex_crew.irq`: masking, coalescing, epoch resets and an error backstop for script-cron pollers, so a cheap probe interrupts an expensive agent turn instead of the turn polling. Also the app-facing probe SDK. |
