...
The config package loads runtime configuration from `~/.codex-crew/config.json`
...
CodexCrew's data root nests **under kiro-cli's own `~/.kiro/` base** so all
Kiro-family apps share a single directory a user can secure. `config_dir()`
(in `codex_crew/config/paths.py`, re-exported from `codex_crew/config/loader.py`)
is the single accessor and resolves to:

1. `$CODEXCREW_HOME` when set (used as-is; refuses system directories like `/`,
   `/usr`, `/System`, `/etc`), else
2. `~/.codex-crew` (the default).

**No migration — net-new users only.** All supported installs start directly in
`~/.codex-crew`; there is no `~/.codexcrew` to relocate, so `config_dir()` simply
resolves and `mkdir`s the home above. The one-time `~/.codexcrew` → `~/.codex-crew`
data-home migration that earlier releases carried has been **removed** (see
`docs/system-specs/post-launch-removals.md`). A leftover top-level `~/.codexcrew`
from an old install is never read, migrated, or deleted; it is left in place —
still credential-gated by the `.codexcrew` security-path spelling — and `codexcrew
...
repository preserves the CodexCrew data home by default. `codexcrew service
...
resolving or removing the CodexCrew home. App Kit uninstall also preserves the
...
`~/.kiro/` would also remove `~/.codex-crew` and take CodexCrew's data — config,
...
existed — there is now no `~/.codexcrew.archived` fallback for ANY install
...
`~/.codex-crew` from a `~/.kiro/`-wide wipe, or prompt before deleting it.
Independently, a user who wants the data home entirely outside `~/.kiro/` can set
`CODEXCREW_HOME` to relocate it.

**Technical hedge — recovery-pointer breadcrumb.** `config_dir()` writes a small,
non-secret `~/.codexcrew.breadcrumb` pointer file at the top-level home
...
written only on the default path (a `CODEXCREW_HOME` override carries no `~/.kiro/`
...
> data under `~/.kiro/`, the CodexCrew product owner MUST confirm the
> Kiro-family uninstaller either excludes `~/.codex-crew` or prompts — because
> there is no `~/.codexcrew.archived` fallback for any install, so a
...
> [issue #355](https://github.com/mikutellyourworld/Codex-Crew/issues/355)** (label
...
`config_dir()` re-reads `$CODEXCREW_HOME` on every call and the migration above
...
its own `CODEXCREW_HOME`) and test isolation — `conftest.py`'s autouse `_isolate_codexcrew_home`
...
`test/test_lazy_data_home_paths.py`, which walks the AST of `src/codex_crew` for
...
| branch | behaviour |
| --- | --- |
| a **valid** `CODEXCREW_HOME` override | delegates to `config_dir()` every call, so an override set *after* import is honoured. That branch performs no breadcrumb refresh — only a cheap `mkdir`. |
...
Resolution order:
1. `CODEXCREW_WORKSPACE` env var — used as-is (no `codexcrew-workspace` subdirectory appended)
2. Saved path in `~/.codex-crew/workspace_dir` (written by `codexcrew setup`; re-running setup preserves the existing value as the prompt default)
3. Platform default:

| Platform | Path |
|----------|------|
| macOS | `/Volumes/workplace/codexcrew-workspace` (falls back to `~/workplace/codexcrew-workspace` if `/Volumes/workplace` doesn't exist) |
| Linux | `~/workplace/codexcrew-workspace` |

Each session/task gets an isolated subdirectory under this root via `_session_work_dir(key)`:
- Chat sessions: `codexcrew-workspace/cli_chat`, `codexcrew-workspace/{thread_ts}`
- Background: `codexcrew-workspace/_bg`
- Cron: `codexcrew-workspace/cron_{job_id}`
- TaskRunner: `codexcrew-workspace/taskrunner_main`
- Background session: `codexcrew-workspace/_bg`

The parent directory is created on first call if it doesn't exist.

## Project Directory Resolution

`CODEXCREW_PROJECT_DIR` env var controls where agent config and skills are loaded from:

1. Env var `CODEXCREW_PROJECT_DIR` (if set and valid)
2. CWD walk-up — CLI walks up from CWD looking for `skills/` + `src/codex_crew/` (the `agents/` dir was removed in commit bbbc1f6e when agent config moved into `src/codex_crew/config/`)
3. Saved path in `~/.codex-crew/project_dir` (written by `codexcrew setup`)
...
- The load path emits **one** warning naming every drifted key plus the command to
  ...
  and it lands on every short-lived `codexcrew` invocation, where the
...
- `codexcrew doctor` prints a `Stored Defaults` section reading `config.json`
...
`codexcrew config defaults` is the surface that resolves the ambiguity the load path
...
returns. Acks live in `~/.codex-crew/superseded_acked.json` (`{"acked": {...}}`),
...
User overrides can be placed in `~/.codex-crew/config.local.json`. This file is
deep-merged on top of `config.json` at load time and is never touched by
`codexcrew setup` or package upgrades.

Resolution order:
1. Load `config.json` (managed by CodexCrew, may be regenerated on upgrade)
...
codexcrew config set --local agent.yolo true

# Save to config.json (may be overwritten on upgrade):
codexcrew config set agent.yolo true
```

### `config_local_path() -> Path`
Returns `~/.codex-crew/config.local.json` (or `$CODEXCREW_HOME/config.local.json`).
...
### `CodexCrewConfig.load() -> CodexCrewConfig`
...
### `CodexCrewConfig._resolve_agent_model() -> str`
Reads model from installed agent config (`~/.kiro/agents/codexcrew.json`),
falling back to the bundled `config_package_dir()/defaults.json` (i.e.
`src/codex_crew/config/defaults.json`), then `DEFAULT_MODEL`.

### `CodexCrewConfig._resolve_named_agent_model(agent, agents_dir=None) -> str`
...
locate installed agent JSONs without importing `codex_crew.agent` — which imports
...
agents dir) and `<project>/.kiro` (which holds Codex Crew's older
`*.agent-spec.json` convention). kiro-cli resolves `--agent` against
`$PWD/.kiro/agents` before the user-level dir with **no upward walk**, and Codex Crew
...
- **No filesystem I/O**, for the same reason rung 2 has none: it is called from
  ...
  materialized snapshot, the alias snapshot published by `CodexCrewConfig.load()`
...
### `CodexCrewConfig.create_provider_factory() -> Callable`
Returns a factory for LLMProvider instances. Resolves `"auto"` model
before creating the provider.

### `CodexCrewConfig.to_dict() -> dict`
Serializes config to the JSON structure used by `config.json`. Uses `_configured_port`
(the file value) instead of `dashboard_port` (which may be overridden by `CODEXCREW_PORT`
env var) to avoid clobbering the saved port on write-back.

### `CodexCrewConfig.save() -> None`
Writes current config to `~/.codex-crew/config.json` via `to_dict()`, through
...
Many callers do not hold a whole `CodexCrewConfig` — they flip one toggle
...
One deliberate exception: the interactive `codexcrew config set --local` path
...
### `config_dir() -> Path`
Returns `~/.codex-crew/` (nested under kiro-cli's `~/.kiro/` base). Overridden by
`CODEXCREW_HOME` env var (refuses system directories like `/`, `/usr`, `/System`,
`/etc`). On the default (non-override) path, a pre-move `~/.codexcrew` is migrated
once into `~/.codex-crew` — see "Data Home Location & Migration" above.

### `config_path() -> Path`
Returns `~/.codex-crew/config.json` (or `$CODEXCREW_HOME/config.json` if overridden).

### Agent Bookkeeping Sidecar (`agent_model_state.json`)

CodexCrew tracks two pieces of per-agent state that are **not** part of the
...
schema-valid, this state lives in a CodexCrew-owned sidecar
`~/.codex-crew/agent_model_state.json` (honoring `CODEXCREW_HOME`), keyed by agent
name:

```json
{
  "codexcrew":           {"model_managed": true},
  "codexcrew-heartbeat": {"cc_model": "claude-sonnet-4.6"}
}
```

- Read/written via `codex_crew/agent_state.py` (atomic, lock-guarded near-leaf
...
Note: CodexCrew is KiroACP (kiro-cli) only — the deleted `claude_code` provider
...
class AgentConfig:
    ...
    sandbox: str = "auto"          # default "auto" (namespace on Linux, seatbelt on macOS; delegates to kiro-cli's internal sandbox on macOS when enabled); "off" skips Codex Crew's sandbox
    ...
    enforce_denied_commands: str = "all"  # "all" or "codexcrew"
...
class SttConfig:
    ...
    model: str = "base"            # a codex_crew.stt.models CATALOG name; a superseded name resolves via its alias table
...
class TelemetryConfig:
    ...
    local_dir: str = ""            # local JSONL shard dir; empty = ~/.codex-crew/metrics
...
# CodexCrewAgentConfig, SlackConfig.
...
class CodexCrewConfig:
    ...
    snapshot_dir: str = ""         # snapshot output dir (default ~/.codex-crew/snapshots)
...
`CodexCrewAgentConfig.avatar` is a sparse override, exactly like the per-crew
...
`from codex_crew.config.sections import (...)` list is a frozen pre-split snapshot
...
`~/.codex-crew/computer_use.json` (path via `config.loader.computer_use_state_path()`,
...
`is_sensitive_write_path("~/.codex-crew/config.json")` is `True` (the *tool* path is
protected), but `is_sensitive_bash_command("echo x > ~/.codex-crew/config.json")` is
...
CodexCrew value on collision, and reports unsupported or secret-bearing source
...
### `CodexCrewConfig.channel_config(channel_id: str) -> ChannelConfig`
...
| `CODEXCREW_PORT` | Override dashboard port (dev mode — run dev + prod side by side) | `5486` |
| `CODEXCREW_WORKSPACE` | Override workspace root directory | Platform-dependent |
...
The `dashboard.url` field controls where the dashboard is reachable. From it, the system derives the port to bind on, the bind address (`0.0.0.0` for non-loopback hosts, `127.0.0.1` otherwise), and the allowed origins for CSRF/WebSocket checks. When omitted, defaults to `localhost:5486`.

A **malformed** `dashboard.url` (e.g. an unterminated IPv6 literal `http://[::1` or a non-numeric port `http://host:notaport`) does **not** abort startup: `parse_dashboard_url` degrades to the defaults (`""` host, port `5486`) and logs a warning, so a single typo in the config can never take the gateway down on boot. `CODEXCREW_PORT` still overrides the port regardless.

Once the dashboard's TCP site is listening, the gateway **exports the
actually-bound port as `CODEXCREW_BOUND_PORT`** into its own environment, so
...
`CODEXCREW_PORT`** on purpose: `CODEXCREW_PORT` means operator intent and is
persisted by `service_environment()` into unit files, while
`CODEXCREW_BOUND_PORT` is ephemeral observed truth that must never be frozen
...
1. `~/.kiro/agents/codexcrew.json` → `model` field (installed agent config)
2. `config_package_dir()/defaults.json` → `model` field (bundled `src/codex_crew/config/defaults.json`)
