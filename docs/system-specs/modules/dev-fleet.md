...
Dev Fleet is a builtin App Store app (`codex_crew/apps/builtins/dev_fleet/`) for
managing CodexCrew feature worktrees (git worktrees of the main repo) and their isolated
...
carries an HMAC signature (`X-CodexCrew-Proxy: <ts>:<hmac>` over
...
|------|--------|----------------|
| 1 | `CODEXCREW_DEVFLEET_REPO` env var | no — taken verbatim |
| 2 | `dev_fleet.repo_path` in `config.json` / `config.local.json` | no — taken verbatim |
| 3 | `CODEXCREW_PROJECT_DIR` | yes |
| 4 | the checkout this gateway is executing from (`src/codex_crew` layout walk) | yes |
| 5 | conventional clone locations under `$HOME` (`codexcrew`, `CodexCrew`, `kiro-crew` directly and under `Repos`, `repos`, `src`, `Projects`, `projects`, `dev`, `git`, `code`, `workplace`) | yes |
...
The marker test (`_is_codexcrew_checkout`) requires `.git`, `src/codex_crew/` and
...
Relies on `codex_crew.pod` subpackage (optional import — degrades gracefully if unavailable):
...
`_find_cli()` = `[sys.executable, "-m", "codex_crew"]` — the **package** entry
(`codex_crew/__main__`, which also runs the required SSL-cert / UTF-8-console
setup), never `-m codex_crew.cli`. `codex_crew/cli.py` has no
`if __name__ == "__main__"` guard, so `python -m codex_crew.cli <cmd>` imports the
...
Removing a worktree reclaims the isolated `CODEXCREW_HOME` of that worktree's pod
...
([issue #639](https://github.com/mikutellyourworld/Codex-Crew/issues/639)).
...
`codexcrew logs`") instead of spinning forever.
...
runs under `_build_env()`, whose allowlist drops `CODEXCREW_EDITION_DIR` and
`CODEXCREW_ALLOW_EDITION`, so on an edition composition root it can only compile
...
`src/codex_crew/static/dist` under the Dev Fleet backend's OWN interpreter, with
...
writes the tip it brought to a per-process ref (`refs/codexcrew/sync-base-<pid>`),
...
[#7132](https://github.com/mikutellyourworld/Codex-Crew/issues/7132) tracks the stronger
...
own `codexcrew` binary — moving the working directory and `PATH` with it. No
...
typically `~/.codex-crew/live_target.json`). Contents:
...
| `exec` | the target worktree's `codexcrew` binary that the gateway execs into |
...
| `missing_venv` | the worktree has no `.venv/bin/codexcrew` (Provision it first) |
| `venv_not_executable` | the worktree's `.venv/bin/codexcrew` exists but is **not executable** (`chmod +x` it or re-Provision) — a non-executable binary would stop the live gateway but could not start the replacement, leaving no gateway running |
| `missing_dist` | the worktree has no built `src/codex_crew/static/dist/index.html` (Pull+Build first) — a cutover without a built dist serves a broken dashboard |
| `unsafe_path` | the worktree path cannot be used as a live target (control characters, unresolvable, missing binary, no `src/codex_crew` dir) |
...
- **Foreground last resort**: when the manager probe reports one of
  ...
  establishing a **detached `codexcrew restart --port <port>`** (new session, so
  ...
  backend (no `CODEXCREW_SANDBOX_ACTIVE` marker, not inside
  `codexcrew-agents.slice` — a replacement spawned from inside the sandbox or
  ...
  recorded `codexcrew` launcher (keystone-fenced `run/` dir — there is
  deliberately NO `PATH` fallback, which an agent-planted `~/.local/bin/codexcrew`
...
- **Staged only** (`can_restart = False`, no usable foreground gateway): no
  drivable service manager is available (system unit via `codexcrew service
...
`target_bin` (`.venv/bin/codexcrew`, or `.venv/Scripts/codexcrew.exe` on Windows);
`target_bin` not executable; no `src/codex_crew` directory in the checkout.
...
back to staged-only Make Live and names `codexcrew service install` as the repair.
...
|---|---|---|
| `_POD_IMPORTED` | the `codex_crew.pod` modules imported, so its platform-neutral helpers are callable | the import succeeded (any platform) |
...
- **macOS / Windows / Linux without `systemctl`** — the Fleet view, per-branch PR
  ...
  hidden — `codexcrew pod provision` does not touch systemd, so building a
...
`codexcrew-worktree-dev` carries no app-bridged copy: the canonical copy is
owned by the `codexcrew-dev` development-skills suite under
`src/codex_crew/builtin_skills/`, and the app-bridged duplicate was removed
...
`CODEXCREW_PROJECT_DIR`, which a repo checkout provides, but a `pip install` from
the wheel or sdist does not — and neither does the desktop bundle, whose builder
stages no top-level `skills/` tree.

Skills are registered as symlinks into `~/.codex-crew/skills/` via the app bridge at
