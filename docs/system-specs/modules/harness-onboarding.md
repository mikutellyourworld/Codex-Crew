...
`src/codex_crew/acp_backends.py`, which imports no ACP and therefore may be
...
| `ACP_BACKENDS_INTERNAL_SANDBOX` | The harness sandboxes itself, so Codex Crew's own wrapper stands down. Security-relevant: wrong membership hands isolation to a layer that never starts (H7). |
...
- **`scripts/check_agent_sdk_boundary.py`** is shrink-only. A new import of
  `codex_crew.acp` or `codex_crew.providers` from a consumer fails even though the
...
| Windows isolation | Codex Crew has no Windows OS sandbox backend. Codex may still start there because its native tool hook closes the passive-read gap. This narrow delegation cannot carry path masks and cannot override a governance `sandbox.min_level` requirement. Linux and macOS continue to use the outer OS sandbox as defense in depth. |
