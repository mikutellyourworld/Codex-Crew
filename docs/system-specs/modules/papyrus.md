...
install (see Managed compiler), so `pip install codexcrew` does not ship a
...
co-author panel is a real CodexCrew chat session scoped to the paper, so the agent
edits the LaTeX while the user watches the PDF update.

Ported from a standalone app by **tricatte**; see
`src/codex_crew/apps/builtins/papyrus/ATTRIBUTION.md` for what was kept and what
changed.
...
- **`safe_project_dir(name)`** — the name must match `PROJECT_NAME_RE`; a
  **symlink OR Windows junction at the project entry is refused outright** (like
  `_config_path`: this is a directory CodexCrew creates, so a link there is
...
- **the path-validation gate is offloaded too, because validation is itself
  ...
  (`is_dir`/`exists`). With `CODEXCREW_HOME` on a stalled network mount those
...
default on a fresh `pip install codexcrew`: every compile answered *"No LaTeX
...
The module follows `codex_crew/embeddings.py`, the in-tree precedent for the same
...
`CODEXCREW_PAPYRUS_TECTONIC_URL` overrides the download URL for a mirrored or
...
`CODEXCREW_PAPYRUS_SKIP_TECTONIC_DOWNLOAD=1` refuses provisioning outright, which
...
- **A symlink at the name.** `read_text`/`write_text` both follow one, so
  ...
  **All three** of `.git`, `.git/info` and `.git/info/attributes` are now refused
  ...
  The rule holds for every segment CodexCrew traverses by name, not just the leaf.
...
`src/codex_crew/builtin_skills/papyrus-writing/SKILL.md` — bundled (NOT the
...
| Path | Purpose |
|------|---------|
| `src/codex_crew/apps/builtins/papyrus/app.json` | Manifest (opt-in, author `tricatte`, Apache-2.0) |
...
| `src/codex_crew/builtin_skills/papyrus-writing/SKILL.md` | The co-author's LaTeX skill |
...
`CODEXCREW_PAPYRUS_SKIP_TECTONIC_DOWNLOAD=1` for the whole module as a second belt,
