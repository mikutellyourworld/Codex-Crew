...
Five MCP tools on `codexcrew-dashboard`, five strict-internal routes, one config
...
`codexcrew-dashboard` rather than `codexcrew-core`, because these tools are not a
...
| Refusal | Status | Why |
|---------|--------|-----|
| Config switch off (`agent.session_control` explicitly `false`) | 403 | Operator withdrew the capability from every agent at once. Defaults to true — the agent's `codexcrew-dashboard` mount is the grant. **Exception:** a crew-member DM slot (`member-*` caller key) bypasses this switch — see "Member callers" below |
...
The member-facing tool surface is the ordinary `codexcrew-dashboard` `session_*`
...
`members.member_dispatch_session_server`, identity via `CODEXCREW_SESSION_KEY`
in the entry's env, plus `CODEXCREW_BOUND_PORT` — the entry's env is built from
...
`@codexcrew-dashboard` is an opt-in per-agent server, absent from the default
...
(`CODEXCREW_INTERNAL_SECRET` is stripped from agent env), which is why these are
...
The config read fails **closed**: `CodexCrewConfig.load()` raising resolves to
...
from the `codexcrew-dashboard` MCP server, so an agent whose spec does not mount it
