...
| `session_workspace.py` | `~/.codex-crew/sessions/<id>/` layout for sub-agent result files |
...
7. `_capture_stage_result` concatenates the assistant messages back to this
   ...
   `~/.codex-crew/sessions/<slot>/stage_<n>_result.md`; the path is recorded on
...
orchestrator prompt in order: `~/.codex-crew/prompt-orchestrator.md`, then
`<project>/agents/prompt-orchestrator.md`, then the bundled
`src/codex_crew/config/prompt-orchestrator.md`; it falls back to the normal
