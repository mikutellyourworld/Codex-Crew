...
by Codex Crew) and provides a 3-column workbench: browse/filter issues (work
...
passthrough, and Codex Crew stores NO credential of its own.** `gh api`, `glab api`
...
it would mean raw REST plus a credential Codex Crew stores itself, which is exactly
...
| GET | `/issue-ai` | AI summary + suggested labels (codexcrew-lite) |
...
The Investigate / Review buttons open a CodexCrew chat session seeded with a
...
- `CODEXCREW_INTERNAL_SECRET` is stripped from agent env by
...
`codexcrew-core` MCP server, which holds the internal secret legitimately, so the
...
All data under `app_data_dir("issue-radar")` (typically `~/.codex-crew/apps/issue-radar/data/`):
...
- **Spawn hardening**: All `gh` calls funnel through `_gh_run`, a thin wrapper
  over the shared hardened runner (`codex_crew.github_runner.run_gh`), which
  resolves a canonical `gh` via `github_runner.resolve_gh`
  (`CODEXCREW_ISSUE_RADAR_GH` override, then `CODEXCREW_GH_BIN`, then the
  ...
  outright in both modes. `CODEXCREW_PROVIDER_BIN_STRICT=1` restores the
...
- **Every provider has the same chokepoint, one per CLI.** `gitlab_client._glab_run`
  ...
  `_validate_provider_executable` policy, with `CODEXCREW_ISSUE_RADAR_GLAB` /
  `CODEXCREW_ISSUE_RADAR_AZ` as the override), a list argv rather than `shell=True`,
...
- **GitHub and GitLab work on macOS, Linux and Windows.** The provider-CLI trust
  ...
  `codex_crew.windows_acl`. An **elevated** Windows gateway is refused for the same
...
- **Azure DevOps is POSIX only (macOS/Linux).** `azure_client._az_bin` refuses
  ...
  Codex Crew gateway against Azure DevOps.
...
- Any CLI the user can run from their terminal is accepted: the well-known dirs
  ...
  managed `libexec/codexcrew` dirs, and on Windows the `GitHub CLI` subdirectory
  of each Program Files root) are searched first, then `PATH`. No `sudo`
  copy is required. Override with `CODEXCREW_ISSUE_RADAR_GH` /
  `CODEXCREW_ISSUE_RADAR_GLAB` / `CODEXCREW_ISSUE_RADAR_AZ`; harden with
  `CODEXCREW_PROVIDER_BIN_STRICT=1`. All three executables carry the same
