...
The `codex_crew.platform.governance` + `codex_crew.platform.governance_profiles`
modules implement CodexCrew's **two-level security governance model**. Governance
...
implementation companion to the design doc (Pippin `codexcrew/MVTDhLpm2SSW`).

> Scope: this governs **CodexCrew's own** security boundaries — what the host
...
> (`~/.kiro/agents/*.json`) is **out of scope**: CodexCrew enforces its own
...
1. `CODEXCREW_SECURITY_POLICY` env path — fleet hot-override, highest.
2. **the centrally distributed document** — fetched from `CODEXCREW_POLICY_URL` or
...
3. companion-bundled resource (the `amazon` edition packages it; the public core
   passes `None`).
4. `~/.codex-crew/security_policy.json` — standalone operator-authored.
...
(`POST /api/update`, `codexcrew update`, the gateway-boot auto-apply) so they
...
that would stop booting. Recovery is a manual `codexcrew update`.
...
```json
"distribution": {
  "source": "https://config.corp.example/codexcrew/policy.json",
...
| Channel | Where | For |
|---|---|---|
| `CODEXCREW_POLICY_URL` (+ the `CODEXCREW_POLICY_*` siblings) | per-machine env | The **fleet lever** — the same role `CODEXCREW_SECURITY_POLICY` plays. A config-management push sets one variable; no file to place, no package to rebuild. |
...
`CODEXCREW_POLICY_HEADERS` (a JSON object) instead. There is likewise no
...
- `fail_closed` (**the default**) aborts boot. A fleet that pointed a host at a
  ...
  as "run unbounded". Recovery is `CODEXCREW_POLICY_ON_UNAVAILABLE=degrade`, unsetting
  `CODEXCREW_POLICY_URL`, or the tier-1 local file.
...
**Tier 1 wins against a refresh, not only against a boot.** `CODEXCREW_SECURITY_POLICY`
...
boot, `codexcrew policy fetch`), so "the source has nothing newer than the cache"
...
`CODEXCREW_SECURITY_POLICY` file, say — and a poll must never displace it, so that
...
It also **refuses a source the account Codex Crew runs as can write — the file or any
...
`CODEXCREW_SECURITY_POLICY` as the channel designed for a local, editable policy. The
...
`CODEXCREW_POLICY_HEADERS`, which "a published document must not" carry) made enforceable
...
[`atomic_write`](../../../src/codex_crew/atomic_write.py) — never a hand-rolled temp +
...
`codexcrew policy fetch --force` runs from a shell, and a forced write (new document, new
...
while `codexcrew policy fetch --force` published v3 would write v2 over v3 and then *install*
...
slow boot fetch of v2 racing a `codexcrew policy fetch --force` that publishes v3 would
...
prevent is the model-driven agent learning the deny patterns, and this is Codex Crew's own
...
`$HOME`-relative, legacy `~/.codexcrew`, and relocated spellings alike) means a future
...
Those dir entries are `$HOME`-relative, so `CODEXCREW_HOME` moves the cache out from under
...
`codexcrew policy fetch` stays the operator's explicit lever in this mode.
...
the fleet now forbids: the tool short-circuits inside the harness and never reaches Codex Crew's
...
outage. It is retained whenever `CODEXCREW_POLICY_URL` names the address the settings are for,
...
`CODEXCREW_POLICY_URL` supplies one — the ordinary split, where the fleet publishes the cadence
...
that as `unchanged` made `codexcrew policy fetch --force` exit 0 having established nothing, which
...
`CODEXCREW_POLICY_URL` or the placed policy's `distribution.source`, and the new address is
...
`CODEXCREW_POLICY_URL` instead. The test reads the FORMATTED traceback rather than
...
`codexcrew policy fetch` prints, and not in a log line either. One rule rather than a
...
what diagnoses a failure, and `codexcrew policy source` reports the scheme by design. `_sanitize_detail` also **substitutes the request credential by exact value**, before the
generic pass. `security.redact` recognises credential *shapes*, and
`CODEXCREW_POLICY_HEADERS` is deliberately arbitrary — an `X-Fleet-Key` holding an opaque blob
...
`codexcrew policy source` reports the posture; `codexcrew policy fetch [--force]`
...
Every `CODEXCREW_POLICY_*` variable is on `sandbox._AGENT_DENIED_ENV_KEYS` — not just
the header. `CODEXCREW_POLICY_HEADERS` is a live bearer credential, and with it an agent
could read the document the keystone exists to keep it from reading on disk;
`CODEXCREW_POLICY_URL` is credential-bearing in its own right whenever the fleet uses a
...
They are listed as **concrete names, not a `CODEXCREW_POLICY_` prefix**, because that
...
`apps/backend.py` puts an app backend in **cache-only mode** (`CODEXCREW_POLICY_CACHE_ONLY`)
...
`CODEXCREW_POLICY_CACHE_ONLY` is also in the tier's **entry** condition in
...
same file, and `codexcrew doctor` reports it.
...
`codexcrew policy show` prints the verdict verbatim
...
(`~/.codex-crew/security_policy.json`, `~/.codex-crew/profiles`,
`~/.codex-crew/admission_policy.json`) — `is_sensitive_path` is the shared
...
claim. Verified on the current tree: `is_sensitive_path("~/.kiro/agents/codexcrew.json")`
is `False` and `echo x > ~/.kiro/agents/codexcrew.json` is not blocked. Since
...
> [issue #418](https://github.com/mikutellyourworld/Codex-Crew/issues/418) — see
...
- **Plane B — kiro agent JSON**: out of scope (v1). CodexCrew no longer writes
  ...
  is authoritative; CodexCrew does not regenerate `~/.kiro/agents/*.json`.
...
- The strict settings allowlist excludes governance and security controls.
  Preserving an existing CodexCrew value on collision cannot be overridden by
  foreign precedence.
...
- **Per-app profile binding via MCP chokepoints is best-effort.** The managed
  `codexcrew-core` MCP server is spawned by kiro-cli, not by an app backend, so
  `CODEXCREW_APP_NAME` is absent there — `learn_add`/`send_message` resolve the
  per-SURFACE profile + policy ceiling (the enforced path), not a per-app
  profile. An app's own in-process tool calls (which carry `CODEXCREW_APP_NAME`)
...
- **Raster capture has two channels and neither is governed.** Computer use has no
  ...
  means "no raster capture" must deny `@codexcrew-computer` via the `mcp` scope
  **and** the browser CLI via the `commands` scope.
- **The `mcp`-scope deny is now the ONLY governance lever over computer use, and it
  is keyed on a renameable alias.** `mcp.deny: ["@codexcrew-computer"]` works on
  unmodified shipped code, but the server key is derived by `mcp_server_alias()` from
  an agent-mutable config: verified `mcp__codexcrew-computer2__click` and
...
- **`codexcrew computer call` is subject to the same checks as an agent call.** The
...
> `false`. CodexCrew instead treats an omitted scope as *not governed by the
...
tracked with mikutellyourworld/Codex-Crew#312.) The decision is consulted at the
injection site
...
- Activation requires a locally installed pack (filesystem access to
  `~/.codex-crew/themes/`) plus a per-content sha grant — an actor with that
...
ships `capability_default=True`. Rationale: CodexCrew is a single-user,
...
mikutellyourworld/Codex-Crew#312 and MAY tighten the model later without breaking this
...
and the `CODEXCREW_TELEMETRY_DISABLED` env var are all *operator* controls: anyone on
...
| `PATCH /api/config/codexcrew` (`handlers/core.py`) | **403** on `telemetry.beacon_enabled=true` |
| `codexcrew telemetry enable` (`cli_commands.py`) | Exits **1** without writing config.json |
| `codexcrew config set [--local] …` (`cli_config.py`) | Exits **1** without writing. The *generic* setter reaches the same key, and `--local` writes the overlay that takes PRECEDENCE over the base file |
...
| `PATCH /api/config/codexcrew` (`handlers/core.py`) | **403** on `dashboard.tailscale.enabled=true` |
| `codexcrew config set [--local] …` (`cli_config.py`) | Exits **1** without writing. The generic setter reaches the same key, and `--local` writes the overlay that takes PRECEDENCE over the base file |
...
[issue #6622](https://github.com/mikutellyourworld/Codex-Crew/issues/6622).**
...
- **H3** — the single selectability gate `resolve_selected_backend` runs inside
  `CodexCrewConfig.load()` and must never read the platform context:
...
/api/config/codexcrew` allowlist, `GET /api/config/schema`, `GET /api/acp-backends`
...
cannot drive CodexCrew's own window either (`computer_use/policy.py`), so it cannot
...
`mcp` scope still works as a blunt instrument: denying `@codexcrew-computer` removes
...
`codexcrew policy {show | validate | explain <scope> <item> | profile <name>}` —
