...
Themes are a **standalone subsystem built on `useTheme`**, not CodexCrew apps. This document is the **source of truth** for the end-to-end subsystem.
...
explicit *"this pack requires a newer version of CodexCrew"* message — never the
opaque generic-validation errors — so an older CodexCrew degrades honestly when
...
4. **Promote** — the validated snapshot is atomically renamed into
   `~/.codex-crew/themes/<slug>/`. Concurrent installs of the same slug are
...
- **CSS containment** — install-time denylist (no `@import`, external `url()`,
  ...
  tracked in mikutellyourworld/Codex-Crew#316 (milestone-bound to the first L1-pack
...
image/font/video files behind in `src/codex_crew/static/`, which
`MANIFEST.in`'s `recursive-include src/codex_crew/static *` kept shipping in the
sdist, wheel and DMG. They are gone. When retiring a theme, delete its art in
the same change: every file under `src/codex_crew/static/` ships, so an orphan
