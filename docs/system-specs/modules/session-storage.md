...
`src/codex_crew/session_storage.py` measures what conversations cost on disk and
reclaims that space when a user asks. `src/codex_crew/session_digest.py` reads a
single session's content for the detail view. `src/codex_crew/dashboard/handlers/session_storage.py`
...
its session instead of being orphaned. Codex Crew is a co-owner of that directory's
...
The freshness floor narrows the window but does not establish ownership, so the report exposes `reclaim_blocked_reason` for a client to explain rather than offering an action that `move_to_trash()` must refuse. Tests that inspect this host state isolate `CODEXCREW_POD_ROOT` with the homes.
...
The dashboard handlers offload every store-accessing path with `asyncio.to_thread`; `codex_crew.session_storage` itself is synchronous, so any other async caller must do the same. `GET .../empty` is the exception that can run on the event loop because it reads only process-local job counters and touches no store.
...
`codex_crew.session_storage` owns the trash layout, manifest schema, age policy, and mutation-lock constants; `codex_crew.dashboard.handlers.session_storage` owns the request selection bound. Callers import those constants rather than duplicating their literals, so code and tests remain the source of truth when the policy changes.
