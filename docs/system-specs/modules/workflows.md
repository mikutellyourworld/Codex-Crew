...
Codex Crew agents through ordered phases. The module declares a pure-literal `META`
...
This module also provides Codex Crew's shared orchestration substrate: run
...
The subsystem lives in `src/codex_crew/workflows/` (13 modules). This file is the
...
The same test forbids the engine from importing `codex_crew.dashboard.state` or
`codex_crew.dashboard.ws` directly, so progress leaves the engine only through the
injected event sink. `agent_exec`, `agent_pool`, and `store` are **optional
adapters**, not engine layers: `store.py` and `runner.py` import
`codex_crew.config` / `codex_crew.sel` / `codex_crew.security` inside `try/except
...
actually uses. Codex Crew's provider layer has no native schema enforcement and the
...
config key when present, else `<config_dir>/workflows` (so `~/.codex-crew/workflows`
by default, honoring `CODEXCREW_HOME`).
...
`<CODEXCREW_HOME>/workflow_library/<workflow_id>.json`, which is global to the
Codex Crew data home and therefore shared by Kiro and adapted harnesses such as
...
same in-session model plumbing as the rest of Codex Crew. Session creation has its
...
uses the tool-less `codexcrew-lite` agent with `ToolApprovalPolicy.REJECT_ALL`: the
...
1. `src/codex_crew/workflows/__init__.py` (the contract itself),
2. this spec, and
...
### Group D: Codex Crew's own `ctx` primitives
