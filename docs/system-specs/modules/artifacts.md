...
2. Agent (or user) calls `artifact_save` — the widget is persisted under
   `~/.codex-crew/artifacts/<slug>/current.html`
...
### Python (`codex_crew.artifacts`)

```python
from codex_crew.artifacts import ArtifactStore, get_default_store
...
route, the `artifact_save` MCP tool, the `codexcrew artifact save` CLI) infers
...
### CLI (`codexcrew artifact`)

```
codexcrew artifact list [--tag T] [--kind K] [--q SUBSTR]
codexcrew artifact show <slug> [--version N] [--meta]
codexcrew artifact save --name N [--slug S] [--kind K] [--content C | --content-file F] [--tags A,B] [--description D]
codexcrew artifact update <slug> [--content C | --content-file F] [--name N] [--description D] [--tags A,B]
codexcrew artifact versions <slug>
codexcrew artifact delete <slug>
```

The CLI proxies through the gateway HTTP API (matches `codexcrew learn`).
...
`~/.codex-crew/artifact_folders.json` — create/rename/reparent (cycle- and
...
- `src/codex_crew/widget_slug.py` → `derive_widget_slug`
...
adding a parallel watcher (see `codex_crew.knowledge.artifact_ingest`):
...
2. **Manifest expiry (best-effort):** The teardown handler rewrites the S3
   deploy manifest (`.codexcrew-deploy.json`) with `expires_at=now`,
...
### Auto-registration from chat (`codex_crew.image_artifacts`)
