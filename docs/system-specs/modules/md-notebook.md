...
Notes is a builtin App Store app (`codex_crew/apps/builtins/md_notebook/`) for keeping a
...
proxied request carries an HMAC signature (`X-CodexCrew-Proxy: <ts>:<hmac>` over
...
Rooted at `MD_NOTEBOOK_HOME`, defaulting to `~/.codex-crew/workspace/md-notebook/`:
