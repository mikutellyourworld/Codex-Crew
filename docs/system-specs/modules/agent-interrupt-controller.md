# Agent Interrupt Controller (`codex_crew.irq`)

Status: implemented

Owners: `codex_crew.irq`; in-tree GitHub pull-request probe:
`codex_crew.probes.gh_pr`.

## Purpose

`codex_crew.irq` lets a driver perform cheap observation and request an agent
...
(`builtin_skills/codexcrew-dev/babysit/scripts/pr_watch.py:watch`, via `irq.run`)
