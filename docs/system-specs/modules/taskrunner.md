...
The task runner is split into an orchestrator plus 4 focused helper modules under `src/codex_crew/`:
...
```python
from codex_crew.task_models import Task, TaskStatus, WorkingMemory, Project, NotifyCallback  # noqa: F401
...
These files import from `codex_crew.taskrunner` and require no changes:
...
| CLI `codexcrew run` | `"file"` (default) | ✅ |
...
class Project:
    ...
    branch_name: str       # git branch for task (e.g. codexcrew/task/{task_id})
...
- `cli_server.py` constructs the standalone `codexcrew run TASK.md` runner without an approval callback. Use `force_approval`, not `requires_approval`, for an action that must not execute unattended.
...
|------|-------------|----------|
| CLI | `codexcrew run TASK.md` | Blocking, stdout progress, `--no-test` flag |
...
```json
{
  ...
  "runs": [{
    ...
    "branch_name": "codexcrew/task/my-task_1771822344"
  }]
}
```
...
- **No repo**: `git init` in work_dir, then `git checkout -b codexcrew/task/{task_id}`
