...
An AI meeting assistant. Transcribes a live meeting through Codex Crew's own
...
| Path | What it is |
|---|---|
| `src/codex_crew/apps/builtins/meetings/app.json` | manifest (`backend.routes`, `ui.pages`, agents, permissions) |
...
| `src/codex_crew/builtin_skills/meetings/SKILL.md` | the bundled skill (data layout, lifecycle, provider config) |
...
All under `app_data_dir("meetings")` (`~/.codex-crew/apps/meetings/data/`):
...
per-meeting queue running one tool-less call on `codexcrew-lite` per line with the
...
Both follow `codex_crew.embeddings`' `EmbeddingBackend` /
...
Codex Crew's own `/api/ws/stt` (`dashboard/stt_stream.py`).
...
* **Owner-edit isolation.** User-edited minutes live under the fixed
  `<CODEXCREW_HOME>/apps/meetings/data/edits/` sensitive root, outside the meeting
...
* **Model-generated HTML.** The sketch artist writes HTML *from* the transcript,
  ...
  by CodexCrew's own bootstrap from the declarative `div.mermaid` / fenced
...
deleted in favour of CodexCrew's own, the standalone server became in-gateway
