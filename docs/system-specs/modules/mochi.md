...
render only in the CodexCrew desktop (Electron) shell; the dashboard page is a
...
| Path | What it is |
|---|---|
| `src/codex_crew/apps/builtins/mochi/app.json` | manifest (`backend.routes`, `backend.hooks`, `backend.mcpServers`, `ui.pages`, agents, permissions, `platform.requiresDesktopApp`) |
