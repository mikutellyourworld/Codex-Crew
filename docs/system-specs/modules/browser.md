...
drives a browser by running shell commands; Codex Crew owns the install flow, the
...
known prefix (`CODEXCREW_PLAYWRIGHT_CLI_HOME`, else `<data home>/playwright-cli`).
...
and then uses the ordinary `playwright-cli -s=<name> close` protocol. Codex Crew
...
`PLAYWRIGHT_CLI_SESSION` and `CODEXCREW_SPAWNED` in its exec-time environ, which
...
exec-time environ; the `CODEXCREW_SPAWNED` marker; no live process outside the
...
Codex-Crew-owned lifecycle directories are created
...
Codex Crew never provisions and cannot install without root. So on a host that did
...
invocation Codex Crew never constructs. The config is written under the data home
...
when `PLAYWRIGHT_MCP_CONFIG` is **already set** in the environment, Codex Crew adds
...
(`CODEXCREW_NPM_REGISTRY`, `PLAYWRIGHT_DOWNLOAD_HOST`), where `/proc/<pid>/environ` is
...
- [web-browse](../../../src/codex_crew/builtin_skills/web-browse/SKILL.md) for
  opening a page so the user can see it.
- [web-verify](../../../src/codex_crew/builtin_skills/web-verify/SKILL.md) for
