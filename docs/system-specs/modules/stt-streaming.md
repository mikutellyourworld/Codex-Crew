...
|---|---|---|---|
| `local` (default) | this process, whisper.cpp held loaded by [`codex_crew.stt`](../../../src/codex_crew/stt/__init__.py) | free | desktop builds include the runtime; select a model and click **Download now** |
...
| Component | File | Role |
|---|---|---|
| WS endpoint | `src/codex_crew/dashboard/stt_stream.py` | One provider session per connection, plus the caps and the SEL audit pair |
| Local recogniser | `src/codex_crew/stt/engine.py` | One resident whisper.cpp context, serialised decodes, idle eviction |
| Local session | `src/codex_crew/stt/session.py` | Turns a PCM stream into partials and a final |
| Endpointing VAD | `src/codex_crew/stt/vad.py` | Adaptive-RMS speech detection and end-of-utterance |
| Model catalog | `src/codex_crew/stt/models.py` | The offered models, their sizes, and the sha256-pinned download |
| Apple helper | `src/codex_crew/apple_speech/` | Swift `AppleTranscribe.swift` plus its Python driver |
| Config fields | `src/codex_crew/config/loader.py` | `SttConfig`, and the degradation rules for a stored provider or model |
...
the pin, and the operator can point `CODEXCREW_WHISPER_MODEL_BASE_URL` at any host, so
...
what is installed — and Codex Crew's own downloader writes directly without routing
...
Legacy config fields such as `whisper_path`, `mlx_model`, `parakeet_model`, and `device` are ignored by `CodexCrewConfig.load` because `SttConfig` does not consume them. `config/superseded_defaults.py` records migrated defaults for the config surface.
