...
`author: "CodexCrew"`, and self-award the first-party mark next to a button
...
## 1. App MCP servers land in CodexCrew's agent config, never the shared kiro file

An app's `mcpServers` are written into CodexCrew's own agent config
(`<kiro agents dir>/codexcrew.json`, resolved through `config.paths.kiro_agents_dir`
...
just the app's. CodexCrew sessions read only the agent config (`includeMcpJson` is
...
re-added and stay deletable: `codexcrew-dashboard` carries `opt_in`, an assignable
...
surface that can revoke it; and `codexcrew-computer` carries a `spec_gate` that both
...
| host-managed server the rebuild always emits (`codexcrew-cron`, `codexcrew-core`, edition extras), absent | **preserved** — the rebuild re-adds it anyway, so removing it here never stuck |
| host-managed server flagged `opt_in` (`codexcrew-dashboard`), absent | **deleted** — an assignable grant no rebuild re-adds (a refresh keeps an existing one current but never re-grants), so revocation must stick here |
| host-managed server whose `spec_gate` is CLOSED (`codexcrew-computer` off or unsupported), absent | **deleted** — both spec writers `pop` it, so the rebuild would not re-add it and preserving it resurrects the backend the gate withholds |
...
run `codexcrew app disable <name>` while no gateway process can execute app code,
...
`~/.codex-crew/dependency-ledger.json` records which apps caused which capability
...
and missing. The publisher's `EditorialAssets.add` in CodexCrewApps documents the
...
### Drive Sentinel release boundary

The bundled Drive Sentinel backend follows the same authenticated app-proxy
boundary as other built-in backends. Only `/health` is reachable directly for
the gateway's loopback liveness probe. Every functional GET and POST verifies
the per-launch `X-CodexCrew-Proxy` HMAC, and browser requests marked cross-site
are refused before dispatch. POST bodies must be JSON and are size-bounded.
Its manifest requests an automatic app-manager port and the backend binds the
injected `PORT`, so it cannot drift outside the permitted backend range.
Non-Windows volume inventory returns empty without touching Win32 APIs, keeping
imports and rejection paths fail-closed on unsupported hosts.
On a host without an OS sandbox, a module-style backend proceeds through the
audited first-party fixed-argv carve-out only when immutable package provenance
resolves the module inside that exact shipped app. File backends and forged
built-in names remain on the ordinary fail-closed sandbox path.

Startup leaves the index root empty. The periodic refresher is armed only after
an explicit scan records a root, so enabling the default app does not scan a
drive. HTTP sweeps default to dry-run. Byte-changing actions first prove that
the append-only audit log can open beneath `CODEXCREW_HOME`; audit failures are
reported and are never silently swallowed.
...
- **A promotion requires a positively confirmed ENABLED app; a demotion never does.**
  `codexcrew app disable` runs in its own process: it deregisters the app's resources and
  ...
  the watch — but `codexcrew app disable` runs in ANOTHER process and cannot pop it, so it
...
Shared host capability reaches an app through `@codexcrew/app-sdk`, which the host
...
`src/codex_crew/docs/mcp-apps.md` covers the iframe contract itself.
