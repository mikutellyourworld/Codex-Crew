...
and teammates on *different Codex Crew builds* pull it, so an older instance can be handed a
...
Importing `codex_crew.apps.manager` also sets `manager` as an **attribute on the
`codex_crew.apps` package object**, and that package is never evicted — so putting the table
...
`sys.modules`. So a later test doing `import codex_crew.apps.manager as manager` patched the
...
(`~/.kiro/settings/amazon-internal.json` → `{"sandbox": true}`), a layer Codex Crew
...
`codexcrew token`, which reads the sensitive-path-fenced `.local_secret` in a SUBPROCESS the
...
mint. That rule (`credential-exfil-codexcrew-token`) matched `.*codexcrew.*token` but not the
identical `python -m codex_crew token` module form, which was a way for a prompt-injected agent
...
Review then found the same escape one interpreter flag over: `python -c "from codex_crew.cli
...
codex_crew.cli import main; main()"` names no `token` argv word at all, and neither does
...
`codexcrew doctor` is legitimate and only `codexcrew token` mints.
...
`"import sys; …; from codex_crew.cli import main"` to `import sys` and hid the import entirely.

The `-c`/`-m` spellings pin 15 blocked and 13 allowed, including `python -c 'print(1)' token` and
`grep -r codex_crew src/` to show the deny is scoped to code we are about to run, and a payload
that imports the package WITHOUT the mint verb (`python -c "import codex_crew.cli" && echo ok`)
...
than pretending to close it. `\bcodex_crew\b` misses a name assembled at runtime —
...
neighbouring command naming the package in a FILE PATH (`isort src/codex_crew/mcp_core.py`
...
`python -c …`, and `cat codex_crew_notes.txt | python -`
...
Codex Crew already holds a Slack token for its own gateway, and the live
...
unconfigured on Codex Crew itself, this channel is unavailable, and `status()`
...
There is no module-level gateway-state accessor in Codex Crew (state is per
...
token whose secret lives at `~/.codex-crew/apps/<name>/.app_secret`, and
...
It deliberately has **no per-channel mute**: Codex Crew renders that centrally (`GET
...
**It is the only thing this app produces for a reader who does not run Codex Crew** —
...
- `src/codex_crew/apps/builtins/ops_mission_control/app.json` — manifest
...
- `src/codex_crew/builtin_skills/ops-mission-control/` — the agent skill AND the
  ...
  **They live here, not under the app**, because `register_builtin_apps` copies only
  ...
  reference `~/.codex-crew/skills/ops-mission-control/sops/<name>.md` accordingly.
...
- The app's development journals (research sweeps, feature ledger, parity report) are
  ...
  `MANIFEST.in` special case, because `recursive-include src/codex_crew/apps *.md` — which
...
Group is `codexcrew.ops_providers`, deliberately **distinct** from
...
```toml
[project.entry-points."codexcrew.ops_providers"]
...
`src/codex_crew/apps/builtins/ops_mission_control/tests/` — 647 tests:
