...
The CLI module (`codex_crew/cli.py`) provides the `codexcrew` command using stdlib `argparse`.

## Import Weight Contract

`cli.py` is the shared dispatcher for every subcommand — including the
long-lived MCP stdio servers (`codexcrew mcp-core` / `mcp-cron` /
...
fresh `import codex_crew.cli` from ~1.3 s / ~112 MB to ~0.5 s / ~54 MB, paid
per CLI invocation and per MCP backend process.
`test/test_cli_lazy_imports.py` ratchets the contract: after
`import codex_crew.cli` in a fresh interpreter, none of those modules may be
...
(`codexcrew <sub>`, `python -m codex_crew <sub>`, and the frozen desktop
binary) land in `cli.main()`, whose prelude runs `boot_platform()`
(fail-closed for non-standalone profiles), sandbox env hygiene, and
`CODEXCREW_PROJECT_DIR` resolution before any dispatch.

## Source Checkout Launcher

The POSIX wrapper at `bin/codexcrew` resolves symlinks to find the real checkout,
sets `CODEXCREW_PROJECT_DIR` to that checkout unless the caller already supplied
one, and delegates every argument to `.venv/bin/codexcrew`. The virtualenv entry
point comes from the editable install created by the setup scripts, so it makes
`src/codex_crew` importable without adding the source tree to `PYTHONPATH`. Any
caller-provided `PYTHONPATH` is inherited unchanged.

If `.venv/bin/codexcrew` is unavailable, the wrapper exits with source-install
...
- Schema: `codexcrew-cli-artifact-manifest-v1`.
...
At startup, `main()` auto-detects the project root and sets `CODEXCREW_PROJECT_DIR`:

1. If `CODEXCREW_PROJECT_DIR` env var is already set, use it
2. Walk up from CWD looking for a directory with both `skills/` and `src/codex_crew/` (`_PROJECT_MARKERS`). The project-level `agents/` dir was removed when agent config was consolidated into `src/codex_crew/config/` (commit bbbc1f6e), so the marker no longer references it — a stale `agents/` requirement left detection (and the dashboard changelog) silently broken.
3. Read saved path from `~/.codex-crew/project_dir` (written by `codexcrew setup`); the saved path is re-validated against the same markers

This allows `codexcrew` to find project-level agent config and skills from any directory.

## Commands

### Top-level help

`codexcrew --help` (and a bare `codexcrew`, which prints the banner first) does NOT
...
- Its notes answer the two questions the flat list never did: how `gateway`
  ...
  a time), and that the dashboard on loopback `5486` is the **only** port opened
...
- `cli.py` sets `help=argparse.SUPPRESS` on the subparsers action to hide
  ...
  `prog="codexcrew"` on the action so each subcommand's own usage line is
  `usage: codexcrew <cmd> …` rather than the whole top-level usage string.
- Every user-facing command is registered with `cli_help.add_command(sub, name)`,
  ...
  subparser's `description`, which is what `codexcrew <cmd> --help` prints, so the
...
- Internal `mcp-*` servers call `sub.add_parser(name)` with no `help`, which keeps
  ...
  Membership is unfiltered and `_name_parser_map` is untouched, so `codexcrew
...
| Command | Description |
|---------|-------------|
| `codexcrew chat -m "msg"` | Send a single message, print streaming response |
| `codexcrew chat` | Interactive chat mode (readline, exit with Ctrl+D) |
| `codexcrew chat --model X` | Override model for this session |
| `codexcrew gateway` | Start the Codex Crew server (dashboard + messaging channels) |
| `codexcrew gateway --slack-only` | Start without dashboard or SSH tunnel instructions |
| `codexcrew gateway --no-crons` | Start without cron scheduler (use when another instance handles crons) |
| `codexcrew gateway --no-tunnel` | Never publish a tunnel: refuses to start or provision one for the life of the process, whatever `tunnel.enabled` says. SCOPED TO TUNNELS — it does not change where the dashboard binds, so a config that widens `dashboard.url` off loopback still does, with token auth as the control there; do not read `publish_disabled()` as "no published surface of any kind". Reach the instance on the loopback port it binds (`ssh -L` from another host). A Dev Fleet pod boots with this whenever its own checkout declares the flag — the pod's argv is built by the control plane but executed by the target worktree's gateway, so `pod.runtime.target_supports_flag` probes that checkout first and DROPS the flag when it is absent (passing it would make argparse exit 2, which the unit's `Restart=on-failure`/`RestartSec=5` turns into a 5s restart loop). Such a checkout keeps the tunnel behaviour it had before this flag existed and is not given the guarantee — see `security.md` for why no config-side substitute is applied. |
| `codexcrew setup` | Install agent config, save project dir, configure credentials |
| `codexcrew setup --agent-only` | Only install agent config (skip credentials) |
| `codexcrew setup --slack` | Run the guided Slack credential + slash-command setup (opt-in) |
| `codexcrew setup --whatsapp` | Run the guided WhatsApp opt-in: report the optional `whatsapp` extra and the pairing state, then enable the channel (opt-in) |
| `codexcrew doctor` | Verify kiro-cli is installed and config is valid |
| `codexcrew cron add/list/remove` | Manage cron jobs |
| `codexcrew spawn run/list` | Manage background subagents |
| `codexcrew app install/list/enable/disable/uninstall` | Manage App Kit apps. Uninstall preserves `apps/<name>/data/` by default. |
| `codexcrew app uninstall NAME --purge-data` | Explicitly uninstall an app and permanently delete its app data. |
| `codexcrew app dev <name> [--off] [--confirm-out-of-install-root]` | Toggle an installed app into/out of dev mode (no-store UI serving + live reload on file change). See [App Dev Mode](#app-dev-mode). |
| `codexcrew learn add/list/remove` | Manage learned corrections |
| `codexcrew run TASK.md` | Run an autonomous task from a spec file |
| `codexcrew token` | Print a dashboard access URL with auth token |
| `codexcrew logout` | Revoke all active dashboard sessions, refresh chains included |
| `codexcrew manifest` | Generate Slack manifest with user alias auto-populated |
| `codexcrew update` | Update to latest version (git fetch + hard reset to upstream + rebuild; a diverged checkout is refused — `--force` discards its local commits) |
| `codexcrew status` | Show runtime stats from running gateway |
| `codexcrew stop` | Stop a running gateway (service-aware: stops the systemd/launchd service if active, otherwise terminates the gateway found by a cross-platform port lookup — lsof on POSIX, netstat on Windows). Pass `--port N` to bypass the service short-circuit and target a specific gateway. |
| `codexcrew restart` | Restart a running gateway (service-aware: restarts the systemd/launchd service if active, otherwise terminates the foreground gateway and respawns it detached). Pass `--port N` to bypass the service short-circuit and target a specific gateway. |
| `codexcrew service install` | Install gateway as a system-level systemd service (Linux, requires sudo for `tee` + `systemctl` only) or launchd LaunchAgent (macOS, no sudo). Auto-restarts on crash, auto-starts on boot. |
| `codexcrew service uninstall` | Stop and remove the systemd unit / launchd plist. |
| `codexcrew service status` | Show service status (`systemctl status` or `launchctl list`). No sudo required. |
| `codexcrew logs` | Tail gateway logs from the systemd journal, launchd stdout file, or `~/.codex-crew/gateway.log`. |
| `codexcrew logs -f` | Follow logs live (long-running tail). |
| `codexcrew cloud launch/list/status/connect/stop/start/destroy/iam-policy/doctor` | Provision, connect to, and manage a CodexCrew EC2 instance in the user's AWS account. |
| `codexcrew security events` | Show recent SEL audit events (`-n N` for count) |
| `codexcrew security verify` | Verify SEL HMAC chain integrity |
| `codexcrew snapshot` | Create a .tar.gz snapshot of all CodexCrew state |
| `codexcrew snapshot --keep N` | Auto-prune to N most recent snapshots (default 7) |
| `codexcrew snapshot --list` | List existing snapshots |
| `codexcrew restore <file>` | Restore from a snapshot (auto-detects replace vs merge) |
| `codexcrew restore <file> --mode replace\|merge` | Force restore mode; merge skips malformed incoming or local cron JSON with a file-specific warning |
| `codexcrew restore <file> --components X,Y` | Selective component restore |
| `codexcrew restore <file> --dry-run` | Preview restore without writing |
| `codexcrew restore --list-components` | Show available component names |
| `codexcrew snapshot --allow-unpinned-staging` | Stage by path name where a directory cannot be pinned by descriptor |
| `codexcrew restore <file> --allow-unpinned-staging` | Same, for the restore side |
| `codexcrew agent list/create/update/delete/reset-model` | Manage Codex Crew agent definitions (`kiro_agent`, `workspace`, `memory_store` bindings). `reset-model` clears a spec's pinned model, the narrow way back to the shipped default — see [providers.md](providers.md). |
| `codexcrew artifact list/show/save/update/versions/delete` | Manage saved artifacts (LLM-generated UI). Same store the MCP tools and dashboard use — see [artifacts.md](artifacts.md). |
| `codexcrew consolidate [session_key] [--all]` | Force history consolidation, which triggers skill extraction. Omit the key to list sessions with unconsolidated messages. |
| `codexcrew eval [scenarios…] [--all] [--judge]` | Run multi-session evaluation scenarios; a bare invocation is the ~30s smoke test — see [knowledge.md](knowledge.md). |
| `codexcrew sandbox install-profile/status/remove-profile` | Manage the AppArmor profile the agent sandbox needs (Linux) — see [security.md](security.md). |
| `codexcrew tailnet status/up/down` | Publish the dashboard on your tailnet (Tailscale) and trust its origin, or stop publishing — see [remote-and-mobile](../../guides/remote-and-mobile.md). |
| `codexcrew telemetry status/disable/enable` | Inspect exactly what the anonymous beacon sends, or turn it off permanently — see [metrics.md](metrics.md) and [governance.md](governance.md). |

### Staging is descriptor-pinned, and refuses rather than degrading silently

Snapshot and restore stage through `codex_crew.pinned_fs`: the parent chain is resolved
...
carries `"staging": "unpinned"` and `codexcrew restore --dry-run` prints that the
...
remote-mint path (`codex_crew.instances.token_mint.mint_remote_token`) runs
`codexcrew token` on a remote host over SSH and regex-extracts the JWT from its
...
`codexcrew setup` performs:

1. Saves `CODEXCREW_PROJECT_DIR` to `~/.codex-crew/project_dir`
2. Installs agent config to `~/.kiro/agents/codexcrew.json`
...
4. Offers to set up custom domain `codexcrew.localhost` (macOS/Linux)

The saved project dir enables running `codexcrew` from any directory.

### First-run Kiro CLI prerequisite onboarding

CodexCrew exposes the same two-step readiness contract on every supported
...
- Missing CLI: the setup page offers an explicit install action on macOS,
  ...
  changed upstream script therefore fails closed until a CodexCrew release
...
- Installed but signed out: the setup page names the commands the USER runs and
  ...
  as a peer of organization SSO; Codex Crew does not detect which tier applies,
...
additionally hide the configured data home, `~/.codex-crew`, `~/.codexcrew`, and
...
not install source, owner, or fixed path (CodexCrew is not the authority on where
...
standard sandbox against the user's real home, with only the Codex Crew data
homes hidden, and the CLI writes its own credential store exactly as it does
from a terminal. CodexCrew stages nothing and publishes nothing, so no staged
...
Status requests never publish a discovered path by mutating `CODEXCREW_KIRO_BIN`.
...
After credentials, `codexcrew setup` offers to add `127.0.0.1 codexcrew.localhost` to the system hosts file so the dashboard is accessible at `http://codexcrew.localhost:5486`:

- **macOS/Linux**: Uses `sudo tee -a /etc/hosts` for safe append

Skipped if `codexcrew.localhost` is already present or user declines.

## Cloud Command

`codexcrew cloud` is a human installer/control-plane surface for running
CodexCrew on the user's own AWS EC2 instance. Provisioning and teardown are not
LLM-facing tools. AWS credentials are resolved by the AWS CLI; CodexCrew stores
only profile, region, and the most recent instance tag in `cloud.json`.

`codexcrew cloud launch` runs a six-step wizard: check AWS reachability, explain
...
`codexcrew-*` CloudFormation stacks with `cloudformation:ListStacks` and offers a
choice to resume one or create a new installation. `codexcrew cloud launch --new`
...
`codexcrew cloud connect` mints a dashboard token over SSM, opens an
...
launch returns non-zero and tells the user to rerun `codexcrew cloud connect`
after fixing the local SSM tunnel issue.

## Config Command

`codexcrew config` manages `~/.codex-crew/config.json`:
...
`codexcrew gateway` creates `~/.codex-crew/config.json` with defaults if the file doesn't exist. Does nothing if it already exists.
...
`console_scripts` in `setup.cfg` maps `codexcrew` → `codex_crew._bootstrap:main`.
...
Linux pidfd branch raised `AttributeError` and `codexcrew gateway` died before
...
if args.command == "gateway":
    from codex_crew.service.live_target import maybe_reexec
...
`codexcrew` binary. This runs before anything is written to `$CODEXCREW_HOME`,
...
`CODEXCREW_LIVE_EXECED` marker already in env) causes the function to return, and
the currently-installed build boots normally. A bad pointer can never leave the
host with no gateway.

Gateway only — a plain CLI invocation (`codexcrew doctor`, `codexcrew chat`, etc.)
...
| Variable | Purpose |
|----------|---------|
| `CODEXCREW_HOME` | Override config/data directory (default `~/.codex-crew`) |
| `CODEXCREW_PORT` | Override dashboard port (default `5486`, validated as int at CLI startup) |
| `CODEXCREW_PROJECT_DIR` | Override agent config/skills directory |
| `CODEXCREW_WORKSPACE` | Override workspace root directory |

For local dev:
- **macOS/Linux**: `bin/codexcrew` (POSIX shell wrapper); `source setup.sh` adds `bin/` to PATH

The wrapper sets `CODEXCREW_PROJECT_DIR` and routes to the right runtime based on install type:

- **One-liner install** (`install.sh` clones the repo into `~/.codexcrew-app/`): if a sibling `.venv/bin/codexcrew` exists, the wrapper execs it directly.
...
8. `codexcrew setup --agent-only` (install kiro-cli agent config)
9. Optional Slack credential configuration (`codexcrew setup --slack`)
...
5. **MCP tools**: `@codexcrew-cron` and `@codexcrew-core` in `tools`, `allowedTools`, and `mcpServers` — auto-fixes missing entries
6. **Global mcp.json**: codexcrew MCP servers present with valid binary paths — auto-fixes stale paths
...
`codexcrew token` / `status` / `logout` / `stop` / `restart` must find the port
...
2. `CODEXCREW_PORT`, when it parses as an int. Deliberately above the bound
   export: an explicitly-set `CODEXCREW_PORT` is how a caller retargets a
   child at a DIFFERENT gateway — `pod exec` builds a client env with
   `CODEXCREW_PORT=<pod-port>` while the inherited `CODEXCREW_BOUND_PORT`
   ...
   (`build_pod_env` additionally scrubs `CODEXCREW_BOUND_PORT` outright.)
3. `CODEXCREW_BOUND_PORT`, when it parses as an int — the port the parent
...
4. A port **explicitly written** in `dashboard.url`. A portless URL
   ...
   substitutes `5486` for the server's benefit, so the client re-splits the URL
...
5. The sole **gateway-owned run-marker**. A running gateway records
   ...
   `codex_crew.instances.run_marker`, written for the SSH token-mint), so its
   ...
   - **Ownership, not reachability** — `clear_marker()` only runs on graceful
     ...
     `/tmp/codexcrew gateway` would pass it. `_gateway_owns_port()` therefore
     ...
     3. that pid must be owned by the calling uid
        (`platform_compat.process_owner_uid`) and look like a CodexCrew process.
     ...
     **On non-POSIX platforms the step denies outright.** `process_owner_uid`
     cannot report an owner on Windows, and a `CODEXCREW_HOME` writable by another
     ...
     users keep `--port` / `CODEXCREW_PORT`, exactly where they were before this
   ...
   - **Ambiguity** — with several gateways up there is no basis to pick one, so
     ...
     `CODEXCREW_PORT` hint to stderr, and falls through.
6. `_DEFAULT_PORT` (`5486`).

Steps 3 and 5 are what make a single gateway started on a non-default port
(`codexcrew gateway --port 6776`) reachable from a bare `codexcrew token` with
zero configuration; before it existed, the client hit a dead 5486 while the
...
child re-resolves independently, so without that the replacement could bind 5486
while the parent waited on the discovered port.

The marker is written for **every** dashboard-serving gateway, including a
source-tree `python -m codex_crew` launch with no console script beside
...
`codexcrew stop [--port PORT]` stops a running gateway:
...
2. Otherwise (no service active, or `--port` was passed explicitly to
   ...
   `codexcrew stop` a no-op on Windows). Both binaries are resolved through
...
3. `platform_compat.process_command_line(pid)` to verify it's a CodexCrew process —
   `/proc/<pid>/cmdline` (Linux), `ps -o command=` (macOS), `Win32_Process.CommandLine`
   via WMI (Windows). The Windows venv `codexcrew.exe` re-execs `python.exe`, so the
   match is on the command line (`-m codex_crew gateway` / `\Scripts\codexcrew.exe gateway`),
   not the image name.
...
`codexcrew restart [--port PORT]` restarts a running gateway. Mirrors
...
1. If a systemd/launchd service is active **and** the caller did not
   ...
   `sudo systemctl restart codexcrew.service` (single
...
2. Otherwise (foreground gateway, no service, or `--port` passed
   ...
   - Spawn a detached `codexcrew gateway` via `subprocess.Popen`, stdin set
     to `subprocess.DEVNULL`, and stdout + stderr redirected to
     `~/.codex-crew/gateway.log` (the same file the `codexcrew logs` command
     ...
     follow logs via `codexcrew logs -f`.
3. SEL audit event logged with `via=service` or `via=fork pid=<n>` so
   the audit trail distinguishes the two paths.

## Service Management

`codexcrew service {install,uninstall,status}` registers the gateway
with the OS service manager so it survives SSH disconnects, restarts
on crash, and starts on boot. Implemented in `src/codex_crew/service/`.

- **Linux** (`current_platform() == SYSTEMD`):
  - Unit file: `/etc/systemd/system/codexcrew.service` (root-owned).
  - Install: `sudo install` writes the unit, then `sudo systemctl
    daemon-reload && sudo systemctl enable --now codexcrew.service`.
  ...
  - The gateway runs as `User=$USER Group=$(id -gn)`. Every elevated
    executable is a stock system program, not a codexcrew one; the module
  ...
  - **Environment**: values are captured from the installer's environment
    ...
    `CODEXCREW_PORT=5477 codexcrew service install` binds a non-default port.
    The unit also reads `EnvironmentFile=-/etc/codexcrew/codexcrew.env`, an
    ...
    restart codexcrew` changes a value (e.g. the port) without reinstalling.
    Uninstall removes the file and its `/etc/codexcrew` directory.
  - **Credentials are deliberately NOT captured.** Both baked locations are
    ...
    `PATH`, `CODEXCREW_KIRO_BIN` and `CODEXCREW_PORT` (it also returns `HOME`,
    ...
    variable and the remedy when it detects that case, and `~/.codex-crew/.env`
    ...
    is already written and started by the time it runs. `codexcrew doctor` reports
    ...
    foreground `codexcrew gateway`, both leave the host healthy while the check
    fires.
  ...
  - Logs are read from the journal: `sudo journalctl -u codexcrew -f`,
    or unprivileged if the user is in `systemd-journal` / `adm`.
- **macOS** (`current_platform() == LAUNCHD`):
  - Plist: `~/Library/LaunchAgents/dev.codexcrew.gateway.plist`
...
2. launchd stdout file if a plist exists on macOS and that file is
   ...
   and tail nothing.
3. `~/.codex-crew/gateway.log` for foreground gateways
...
`codexcrew status` queries the running gateway's `/api/status` endpoint
and prints uptime, sessions, messages, tool calls, subagents, crons, lessons.

## App Dev Mode

`codexcrew app dev <name> [--off] [--confirm-out-of-install-root]` toggles an
...
- **Persisted schema — `installed.json` `dev: bool`** (default `false`): a
  per-app flag in each app's `~/.codex-crew/apps/<name>/installed.json`. Tolerant
...
- **Endpoint — `POST /api/apps/{name}/dev`**, body `{"enabled": <bool>}`,
  ...
  (`~/.codex-crew/apps/.dev-grants.json`) is sealed read-only inside the agent
  OS sandbox
...
An internal, unstable sentinel cache under `~/.codex-crew/apps/` mirrors the set of
...
`codexcrew computer {doctor [--json] | apps | call}` — hand-rolled dispatch
...
`~/.codex-crew/computer_use.json` is on, and the macOS TCC probe
...
decision into many. There is deliberately **no** `codexcrew computer state <app>` —
...
```bash
codexcrew gateway --test-mode          # bundle: ephemeral port + json-ready + reads approval
codexcrew gateway --port auto          # OS-assigned port, avoiding a collision with a real gateway
codexcrew gateway --json-ready         # print CODEXCREW_READY:{port,token,pid,home} once listening
codexcrew gateway --approval reads     # auto-approve read-only tools
codexcrew gateway --approval yolo      # auto-approve ALL tools
```

`--json-ready` is what makes the harness race-free: the caller waits for the
`CODEXCREW_READY` line instead of polling a port, and reads the token from it rather
than minting one.

**`--approval yolo` refuses to start unless `CODEXCREW_HOME` is explicitly set to a
