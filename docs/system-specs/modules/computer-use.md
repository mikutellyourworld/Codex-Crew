...
> the compression pipeline and the security floors are all CodexCrew's own.
...
```
kiro-cli
  └─ spawns  codexcrew mcp-computer            (stdio MCP server: codexcrew-computer)
...
`agent._computer_use_spec_gate()` decides whether `codexcrew-computer` appears in
...
read-path driver shipped: a hardcoded OS check left `@codexcrew-computer` in `tools`,
...
`tools` is **not** touched. The `@codexcrew-computer` ref the shipped
...
`CODEXCREW_HOME` — the managed spec's `env` map is the only channel, so
...
about different files. Found by running the feature under `CODEXCREW_HOME` in dev mode.
...
block first (the shim advertises `codexcrew.caller-identity`, so gatewayd injects
one whenever it can name the caller — the only source that holds on a pooled
backend serving many sessions), else the env `CODEXCREW_SESSION_KEY`, else
`CODEXCREW_HOST_PID` plus the HMAC sidecar signed with the keystone-protected
...
with a driver. `CODEXCREW_SESSION_KEY` reaches a child only from a launcher that
...
process per job under `cron:<job id>`) — and `CODEXCREW_HOST_PID` only from the Linux
...
alongside the caller block (`_meta."codexcrew.tenant".nonce`, #5322). The nonce is
...
window is why `codexcrew-computer` stays in `_MANAGED_SERVERS_ADVERTISING_BUT_WITHHELD`
...
### Why a separate MCP server rather than folding into `codexcrew-core`

`config/defaults.json` blanket-allowlists `@codexcrew-core` in `allowedTools`.
Riding inside it would inherit that blanket auto-approve for `computer_click`. A
separate slash-free server key also lets a fleet deny `@codexcrew-computer` with
one `mcp`-scope pattern. `@codexcrew-computer` is added to `tools` but
...
`codex_crew/testing/fake_computer_use.py` ships a `FakeComputerUseBackend` in the
...
Importing `codex_crew.computer_use` is side-effect free: no framework load, no
...
Server `codexcrew-computer` (slash-free: kiro-cli splits an agent `@server`
...
`computer_end_turn` is control-plane (it drops CodexCrew's *own* cached snapshots
...
Screenshot: /var/folders/…/codexcrew-computer-shots/shot-1769472013411.jpeg
...
The primary enable lives at **`~/.codex-crew/computer_use.json`**, NOT in
`config.json`:
...
- `permissions` comes from shelling `codexcrew computer doctor --json`
...
| CodexCrew's own window | `policy.check_app` | Driving our own Settings UI would let the agent click the enable above — a self-escalation loop that would make the keystone pointless. Matched on bundle id, process name **and window title** — see below. |
...
rule — the one that keeps the agent out of Codex Crew's own Settings — matches on name
...
CodexCrew's own desktop app appear permanently empty. Order of operations:
...
clipboard is out of band: CodexCrew never reads it, so nothing can classify what it
...
Files land in `os.path.join(tempfile.gettempdir(), "codexcrew-computer-shots")`
...
through the external `playwright-cli` binary (`codex_crew/browser_cli/`), which
...
is therefore its own process (`python -m codex_crew.computer_use.overlay_proc`,
...
cross-loop hazard `codex_crew/__init__.py` documents.
...
- **Refusals go through the SAME exit as results** (`tools._refusal`). A refusal is
  ...
  Refusals that are 100% CodexCrew's own static prose (the primary-enable refusal, the
...
- **The secure-field floor is re-read LIVE at the driver**, not inherited from the
  ...
  dispatch chokepoint (`codexcrew computer call`). `tools._SECURE_TARGET_TOOLS` also
...
`codexcrew computer doctor --json` is what the gateway shells for the Settings
...
## The CLI (`codexcrew computer`) — and what is deliberately not ported
...
a per-process `SnapshotIndex` with a 90s TTL. Two separate `codexcrew computer call`
...
| `turn-ended [--previous-notify]` (a host notify hook) | same intent, different shape: `computer_end_turn` is an MCP tool, so the model drops its own snapshot cache rather than relying on a host lifecycle hook CodexCrew does not have. |
...
* once the operator enables the feature, prompt injection that reaches the agent
  ...
  to contain it;
* the one structural defence is that the agent cannot drive CodexCrew's own window
  ...
  as a substring — the tab title takes a badge prefix (`(3) Codex Crew`) and popouts a
  `… — Codex Crew` suffix. And because input is delivered per-PID
  ...
  merely contains "codex crew" is refused — an editor with this repo open, a browser
...
| `computer_use/policy.py` | The one retained app refusal (CodexCrew's own window) + the operator's allow/deny lists, secure-target + text refusals, the click-target/method/button refusals + `resolve_click_method`, `redact_result` |
...
| `computer_use/cli.py` | `codexcrew computer doctor [--json] \| apps \| call` |
| `mcp_computer.py` | The thin stdio shim (`codexcrew mcp-computer`) |
...
| `src/codex_crew/builtin_skills/computer-use/SKILL.md` | The agent-facing workflow. **Bundled**, not in the top-level `skills/` dir: `config/prompt.md` tells the model to read it by name, so per AGENTS.md it is load-bearing and must reach every pip/DMG install |
