...
The subagent module (`codex_crew/subagent.py`) spawns isolated background agents for parallel task execution. Each subagent gets its own LLM session via `SessionManager`, runs a focused task, and announces the result via callback.
...
| `INJECTION_TIMEOUT` | 900 | Inner cap: max seconds for a single `stream_and_collect` call (15 minutes); default `_DEFAULT_INJECTION_TIMEOUT = 900.0`, tunable via `CODEXCREW_INJECTION_TIMEOUT` (float seconds, clamped to `_ON_DONE_TIMEOUT`) |
...
3. If still empty **and subagent has no parent session key** → use the cached `CodexCrewConfig.agent.approval_mode` (snapshotted at `SubagentManager` init); if `"auto"` → `"auto"`
...
class SubagentInfo:
    ...
    result_path: str      # ~/.codex-crew/subagents/<id>/result.txt (full transcript)
...
| Inner cap | `gateway.py _subagent_done()` | 900s (`INJECTION_TIMEOUT`, tunable via `CODEXCREW_INJECTION_TIMEOUT`) | Single `stream_and_collect` call |
...
The gateway sets the `CODEXCREW_SESSION_KEY` env var when spawning kiro-cli,
and `mcp_core.py` reads it via `os.environ.get()`. If the env var is missing
(e.g. older gateway), it falls back to reading
`~/.codex-crew/session_pid_{getppid()}.txt` for backward compatibility. The
...
`skills/subagent/SKILL.md` (project-level) triggers on keywords: `background`, `spawn`, `bg`, `subtask`, `parallel`, `separately`, `concurrently`. Instructs the LLM to use `codexcrew spawn "task"` via bash to spawn subagents.

### CLI: `codexcrew spawn "task"`

POSTs to `http://localhost:5486/api/spawn` (dashboard API). Returns immediately with subagent ID. Gateway runs the task async and posts result to Slack when done.

### MCP Tool: `spawn_run`

Exposed via `codexcrew-core` MCP server. Always fire-and-forget — results
...
Exposed via `codexcrew-core` MCP server. Unlike fire-and-forget `spawn_run`,
...
Each sub-agent runs as its own CodexCrew-owned ACP session (via
...
kiro-cli process rather than as CodexCrew-owned sessions. CodexCrew surfaces
...
- `max_wait` defaults to 7200s (2 hours), clamped to `[60, 7200]`, and is configurable via the `CODEXCREW_SPAWN_SUB_AGENTS_MAX_WAIT` environment variable. The deadline uses `time.monotonic()`.
...
Folder-per-agent persistence at `~/.codex-crew/subagents/{id}/`:

```
~/.codex-crew/subagents/{id}/
...
On startup, `SubagentManager` scans `~/.codex-crew/subagents/` and reconciles:
...
The full transcript stays in `~/.codex-crew/subagents/<id>/result.txt` for a
...
Unknown `agent.completion_keep` values cause `codexcrew gateway` to fail
...
[`src/codex_crew/docs/configuration.md`](../../../src/codex_crew/docs/configuration.md),
[`src/codex_crew/docs/subagents.md`](../../../src/codex_crew/docs/subagents.md),
