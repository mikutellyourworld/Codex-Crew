...
`POST /api/kiro-prerequisite/repair-specs` — Codex Crew neither installs the CLI
...
sequenceDiagram
    ...
    Browser->>MW: GET /api/status (cookie: mc_token_5486=xyz789)
...
Location: `src/codex_crew/dashboard/token_auth.py`
...
- **Turning identity trust off later ends these sessions**, not just their
  ...
  the `codexcrew token` URL after identity trust is switched off. This is the same
...
def revoke_all_sessions() -> None: ...
    ...
    # (for all users) is rejected. Used by `codexcrew logout`. The counter is
...
re-enable unexpired sessions previously revoked by `codexcrew logout`.
...
Location: `src/codex_crew/dashboard/origin.py`
...
def parse_dashboard_url(url: str) -> tuple[str, int]: ...
    ...
    # CODEXCREW_PORT env var always overrides port
...
def resolve_dashboard_host(local_only: bool, configured_host: str = "") -> str: ...
    ...
    # Returns codexcrew.localhost directly for local-only mode (RFC 6761)
...
Location: `src/codex_crew/slack/handler.py` → `_handle_slash_command`
...
Location: `src/codex_crew/slack/allowlist.py`
...
`~/.codex-crew/.local_secret` (or the explicit `CODEXCREW_HOME`), sets
...
`dashboard_socket_path(port)` (`~/.codex-crew/dashboard-<port>.sock`,
...
`CODEXCREW_PORT` env var overrides the port (dev mode).

## Cookies

### Access cookie
- Name: `mc_token_{port}` (e.g. `mc_token_5486`)
...
### Refresh cookie
- Name: `mc_refresh_{port}` (e.g. `mc_refresh_5486`)
...
`codexcrew token` CLI recovery path. The middleware never raises unhandled exceptions.
...
10. Explicit revocation via `codexcrew logout` — clears all nonces, IP bindings, and consumed tokens, and bumps the persisted revocation generation, ending every outstanding access cookie and refresh chain
