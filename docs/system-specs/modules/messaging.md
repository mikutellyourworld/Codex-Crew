...
`codex_crew.messaging` is the channel-neutral transport abstraction used by the shipped Slack, Discord, Telegram, Webex, WeCom, Microsoft Teams, Weixin, iMessage, WhatsApp, and Feishu integrations; its conservative contract also leaves room for a further channel. It avoids re-implementing streaming, tool approval, session identity, or rendering for each integration. It holds the channel-neutral core of the Slack turn loop (`slack/handler.py::handle_message`) so a new channel implements only two small interfaces (a `MessagingTransport` + a `Renderer`) and inherits everything else.

**Dependency direction is one-way:** `slack` / `dashboard` → `messaging`, never the reverse. The `codex_crew.messaging` package imports nothing from `codex_crew.slack` or `codex_crew.dashboard`; its only first-party dependencies are the shared lower-level helpers — `acp.types` event constants, the `security` redactors (`redact_credentials` / `redact_exfiltration_urls`), and `sel` for audit.

Slack's transport path is gated behind the `messaging.use_transport` config flag (default `true` in Codex Crew, so the abstraction is the canonical path); when off, Slack's native `handle_message` path runs instead.
...
> map from `!cmd` to `/codexcrew cmd` — and neither is replicated on another channel.
...
| `messaging/raster.py` | `sniff_raster_mime` — what counts as a raster, decided by leading bytes. Dependency-free (no `codex_crew` imports) so both the inbound sniff and the outbound extractor can share it |
...
dependency direction intact: `codex_crew.messaging` imports nothing from
`codex_crew.slack` / `codex_crew.discord` / `codex_crew.dashboard`, and a ladder that
...
the gateway injects `CODEXCREW_SESSION_KEY` into every agent/ACP subprocess and a
...
- Session-scoped commands (`/stop`, `/compact`, `/model`, `/title`, `/spawn`,
  ...
  `/status`, `/ping`, `/cron`, `/yolo`, `/codexcrew dashboard`, `/voice`, or
  `/agent`.
...
- **`parse_duration` is injected, not imported.** It lives in
  `dashboard/token_auth.py`, and `codex_crew.messaging` imports nothing from
  `codex_crew.dashboard` at any nesting depth (`test_messaging_commands.py`
...
- `GET /api/slack/manifest` — public manifest template rendered with
  `?alias=` (default `codexcrew`, never `$USER`) plus Slack's one-click
  create deep link.
...
**Transport (`codex_crew/discord/`).** A concrete `MessagingTransport` over a
...
the bot can see, although Codex Crew immediately discards traffic outside
...
It reports `Stats().summary()`, the same source Slack's `/codexcrew status` uses,
...
**Store.** `$CODEXCREW_HOME/trust/discord_resume_expectations.json` holds channel-id → `{key, title, version, retired}` rows under agent-blocked `trust/`, with an owner-only directory and `restrict_to_owner` file write because modes do not protect files on Windows. `retired` defaults false when loading an older row. Every filesystem step, including `config_dir()`, runs in a worker; an `asyncio.Lock` serializes read-modify-write without spanning Discord I/O.
...
`/codexcrew dashboard`, so this capability does not claim inbound session binding
...
`$CODEXCREW_HOME/routing/telegram_offset.json` (atomic write, off-loop, written only
...
**Transport (`codex_crew/webex/`).** A concrete `MessagingTransport` over a
...
`/codexcrew dashboard`, Teams' `/yolo` and Slack's `!yolo` are all reachable by any
...
both commands SEL-audit the sender's email, and `/codexcrew dashboard` refuses
...
`CodexCrew`). A mention later in the sentence is content and is left alone.
...
`/codexcrew dashboard`, `/model`, `/sessions`, plus the `/queue` and `/steer`
per-message overrides),
...
**Transport (`codex_crew/wecom/`).** A concrete `MessagingTransport` over WeCom's
...
`CODEXCREW_OWNER_ID` is compared against each channel's own id space, so on a host
...
**Transport (`codex_crew/teams/`).** A concrete `MessagingTransport` over a
pure-`aiohttp` Bot Framework client (`client.py`) plus `PyJWT` for inbound token
validation — no Bot Framework SDK dependency (the optional `codexcrew[teams]`
...
`$CODEXCREW_HOME/routing/teams_service_urls.json`. Loading is lazy and off-loop —
...
**Transport (`codex_crew/imessage/`).** A concrete `MessagingTransport` over the
...
cursor persisted to `$CODEXCREW_HOME/imessage_cursor.json`, so a gateway restart
...
protocol (`neonize`, an optional extra installed with `codexcrew[whatsapp]`). There
...
**Transport (`codex_crew/feishu/`).** A concrete `MessagingTransport` over
