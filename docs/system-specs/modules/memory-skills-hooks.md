...
Structured files under `~/.codex-crew/workspace/memory/`:
...
`~/.codex-crew/config.json` → `"memory"` section:
```json
...
notification, and it does not coordinate across separate Codex Crew processes
...
- Downloads `qwen3-embedding-0.6b-q8_0.gguf` (Q8_0 quantized, 610MB) over plain HTTPS from the public Codex Crew CDN — URL resolution order: `CODEXCREW_EMBED_MODEL_URL` env var, then the `memory.embed_model_url` config knob, then the built-in `_DEFAULT_MODEL_URL` CDN constant. No git, no cloud SDK. Streaming sha256 is computed while downloading and byte-level progress (`bytes_downloaded`/`bytes_total`) is written to `status` every ~16MB for the dashboard's determinate progress bar
...
- Installs persistently to `~/.codex-crew/models/qwen3-embedding-0.6b.gguf` — atomic install: stages into a per-process unique file in the TARGET directory (same filesystem) then `os.replace`, so two concurrent processes (gateway + one-shot CLI) can never interleave writes into a shared staging file
...
- **Retry ladder**: background startup task = up to 6 attempts with exponential backoff (60s base, 30min cap, may span hours); every gateway restart retries; dashboard Enable/Retry click = `DOWNLOAD_ATTEMPTS_INTERACTIVE` (3) attempts for fast feedback. `codexcrew run` (one-shot CLI) never kicks downloads — only the long-lived gateway does
- Escape hatch: `CODEXCREW_SKIP_MODEL_DOWNLOAD=1` skips the download entirely (tests/CI must never trigger a 610MB download; tests additionally pin `OLLAMA_MODELS` to a tmp dir so the salvage path can't fire)
...
| Source | public Codex Crew CDN (`_DEFAULT_MODEL_URL`; sha256-pinned; `CODEXCREW_EMBED_MODEL_URL` / `memory.embed_model_url` for mirrors) |
| Runtime | Vendored llama-cpp-python 0.3.34 (MIT license, `codex_crew/_vendor/`) |
...
5. Model weights sourced from the sha256-pinned Codex Crew release bucket (integrity-verified download at runtime)

### Why llama.cpp (not TEI)

TEI (Text Embeddings Inference) uses the candle Rust framework with a Metal backend that has an [unmerged memory bug](https://github.com/huggingface/candle/pull/3197) causing unbounded GPU buffer allocation on macOS. The process consumes 4+ GB RAM and never becomes healthy. This affects ALL models on TEI/Metal, not just Qwen3. llama.cpp works correctly on all supported platforms (macOS Metal, Linux CPU) — Codex Crew vendors it directly via llama-cpp-python, which also removes the external Ollama server the previous design depended on.
...
| POST | `/api/memory/embedding-model` | Change the embedding model. `{"path", "validate_only": true}` validates only; **omitting `validate_only` applies** (no `apply` flag exists). Empty path reverts to bundled. 403 restricted session, 409 while re-embedding, 409 `env_override_active` under `CODEXCREW_EMBED_MODEL_PATH` |
...
`codexcrew memory {list,search,show,stats,audit,export,migrate,import}` — manage memory from the command line:
...
- `codexcrew security audit` also scans vector memory for injection patterns
...
The backend `POST /api/memory/migrate` endpoint and the `codexcrew memory migrate` CLI remain as a manual escape hatch, but the dashboard no longer calls them.

### Cross-Platform

macOS (Apple Silicon and Intel), Linux (x86_64, arm64/Graviton), and Windows supported. All paths use `pathlib.Path`. GGUF model downloaded over sha256-pinned HTTPS from the Codex Crew CDN. No runtime install step — native llama.cpp libraries are vendored per platform in `_vendor/llama_cpp_libs/` and selected via `LLAMA_CPP_LIB_PATH` (the old Docker fallback is gone).
...
through the same Codex Crew memory writers, key allowlists, per-entry size/count
limits, injection screening, conflict resolution, deduplication, audit events,
and active-entry caps described above. Existing Codex Crew memories/preferences
...
Codex Crew's own tiers by the `instructions` category: each directive paragraph
...
is the persona *role*: a foreign persona document never becomes Codex Crew's
...
2. **JSONL fallback** (`~/.codex-crew/lessons.jsonl`): only used when vector memory is not initialized. Read-only migration source once vector memory is active.
...
`~/.codex-crew/history/<channel_id>.jsonl` (path-validated: refused if it escapes
...
Markdown files at `~/.codex-crew/skills/{name}/SKILL.md` with optional YAML frontmatter (`name`, `description`, `always`).
...
**Source precedence** (project-level wins): `$CODEXCREW_PROJECT_DIR/skills/` → `builtin_skills/` (bundled). Auto-copied to `~/.codex-crew/skills/` on first run. Copies entire skill directories (scripts, assets, etc.).

**Project skills (`<project>/.kiro/skills`) — a different source from the one above.**
`$CODEXCREW_PROJECT_DIR/skills/` is a *sync* source: its contents are copied into
`~/.codex-crew/skills/` and thereafter are ordinary local skills. `<project>/.kiro/skills`
...
**Registry discovery — `skill_discover` / `skill_fetch` MCP tools (`codexcrew-core`).**
...
- the `max_triggered` cap is snapshotted on the loader in `__init__`
  (`self._max_triggered`) — no `CodexCrewConfig.load()` per message — refreshed
...
listing can show those skills, but rewriting a `SKILL.md` Codex Crew does not own —
...
Codex Crew owns. A skill reached through `skills.extra_paths` still reports
`source: codexcrew`, so source alone cannot gate the toggle; the UI hides the
...
**Enable/Disable**: `POST /api/mcp/toggle` adds/removes `@name` from `tools` and `allowedTools` arrays in installed config (`~/.kiro/agents/codexcrew.json`). Does NOT modify `agents/defaults.json`. Disabled servers stay in `mcpServers` but kiro-cli won't load their tools.
...
scan/apply output or written to Codex Crew config. If the destination
...
server aliases and reserves names from every effective source: the Codex Crew
...
collision, and CodexCrew-managed servers (including `codexcrew-core` and
`codexcrew-cron`) are protected from replacement, deletion, or shadowing by an
...
Hermes-style autonomous skill creation from completed sessions. **Opt-in, and STAGED for approval** — generation is **off by default** (`skills.auto_create_from_sessions` defaults **false**; enable via `codexcrew config set skills.auto_create_from_sessions true` or dashboard Settings → Skills). When on, candidates land in a pending-approval queue (`skills.approval_required` defaults **true**) and nothing goes live unattended. Pipeline: detect (during consolidation) → generate → metadata dedupe → pending queue → human approval → live → archive-if-unused.
...
Auto-generated skills live under `~/.codex-crew/skills/auto/<slug>/SKILL.md`. Slug validated against `^[a-z0-9][a-z0-9-]{1,62}[a-z0-9]$`. The `auto/` prefix:
...
- Off by default (opt-in). Enable: `codexcrew config set skills.auto_create_from_sessions true` (or dashboard Settings → Skills); auto-approve prose-only: `codexcrew config set skills.approval_required false`
- Review pending candidates: dashboard Skills → Pending review, or `GET /api/skills/-/pending`
- List auto skills: filter `codexcrew` skill listings to those under `auto/`, or use `SkillsLoader.list_auto_skills()` in code
- Remove unwanted auto skill: `rm -rf ~/.codex-crew/skills/auto/<slug>` (or dashboard skill delete when UI lands)
- Audit trail: `codexcrew security events -n 20 | grep auto_skill`
...
`~/.codex-crew/hooks.json`. It runs in that platform's native shell language, and
a hook is therefore **not portable across platforms**:

| | Shell | Env var in a command | Quote grouping |
|---|---|---|---|
| POSIX | `/bin/sh -c <command>` | `$CODEXCREW_HOOK_EVENT` | `'…'` and `"…"` |
| Windows | `%ComSpec% /c "<command>"` | `%CODEXCREW_HOOK_EVENT%` | `"…"` only (cmd.exe gives `'` no meaning) |

Both platforms receive the same `CODEXCREW_HOOK_EVENT` / `CODEXCREW_HOOK_CONTEXT`
...
`CODEXCREW_HOME` data-home vars, temp-dir and locale vars, and TLS-trust
(`SSL_CERT_*`, `NO_PROXY`) — plus the two `CODEXCREW_HOOK_*` metadata vars set last.
...
User-defined kiro-cli hooks that persist across `codexcrew update`. Follows the
`removedTools` precedent — a raw key in `~/.codex-crew/config.json` read by
...
Beyond Codex Crew's own assembly, kiro-cli manages its own context window:
`_kiro.dev/compaction/status` notifications signal that it summarized older turns,
and Codex Crew resets its context-usage accounting at that chokepoint. Separately,
...
| Memory + skills + lessons | ❌ Keep | CodexCrew-specific, not in kiro-cli |
