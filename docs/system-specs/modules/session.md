...
- **any other backend** — falls back to a `_ProviderBgSession` over the shared
  ...
  Codex Crew edition `agent.provider` is fixed to `acp` and only kiro and KAS are
...
`~/.codex-crew/session_map.json`. Enables `session/load` to restore full
...
`/side` conversations never resume across CodexCrew restarts — each cold-start
...
2. **Phase 2 — fresh session + history replay (backstop).** If the lock never
   ...
   injects CodexCrew's `conversation_log` into the new native session on the first
...
5. On the first prompt after the switch, `chat_runner` detects the flag and
   injects history from `compress_thread_history()` (CodexCrew's conversation_log)
...
- `/codexcrew sessions` — lists active sessions with Slack link status
- `/codexcrew sessions resume <key>` — resumes a session in the current thread
...
every module under `src/codex_crew` that mentions `_sessions`, recognising the
...
Command denial is enforced by Codex Crew's bundled `hooks.py` `PreToolUse` gate,
...
session teardown.  kiro-cli-chat spawns MCP servers (codex_crew mcp-core/cron,
...
- **Sweep-side reap (defense in depth)**: `_is_sweepable_orphan_gatewayd` is
  ...
  `-m codex_crew.mcp_gateway.gatewayd` argv whose `--socket` path is gone
  ...
  `codex_crew.cli` / `codex_crew.__main__` stay unconditionally excluded. The
...
- **Key**: a purpose-specific subkey derived from the SEL trust root via a
  domain-separation label (`HMAC(sel_hmac.key, "codexcrew.session_pid.sig.v1")`).
...
- **Consumers**: STRICT identity resolvers accept the direct
  `CODEXCREW_HOST_PID` → mapping lookup only via
...
Gateway-off (the default topology this targets), the model's tool result is the tool's OWN returned line delivered over kiro-cli's MCP pipe; the applier's confirmation string and SEL audit are recorded on CodexCrew's own surfaces (transcript / WS / hooks) and do NOT rewrite the model's tool result. Each tool therefore phrases its own message as a *request* that the consumer applies (and may refuse — no interactive session, invalid/sensitive path, capped/paused loop) rather than asserting the effect already landed.

```mermaid
sequenceDiagram
    participant M as Model
    participant T as MCP tool (codexcrew-core)
...
`session_pid.py` go through `codex_crew.platform_compat` so CodexCrew runs natively on
