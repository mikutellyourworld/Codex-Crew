...
Storage: `~/.codex-crew/security_events.jsonl` (append-only JSONL with HMAC-SHA256 chain).
...
| `downstream_service` | MCP server name if applicable (`codexcrew-core`, `codexcrew-cron`, `internal-mcp`) |
...
- **Second protocol anchored on this key — domain-separated:** `session_pid_sig.py`
  ...
  (`HMAC(sel_hmac.key, "codexcrew.session_pid.sig.v1")`) so the sidecar MAC and
...
```
codexcrew security events [-n 20]   # Show recent events
codexcrew security verify            # Verify HMAC chain integrity
```
