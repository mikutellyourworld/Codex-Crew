...
`src/codex_crew/cloud/` runs CodexCrew on the user's **own** AWS EC2 instance with a
...
- (2) the shell `deniedCommands` in `config/defaults.json` (kiro-cli
  ...
  the `codexcrew cloud destroy|stop|start|launch|connect|tunnel|login|logout` wrappers
  ...
  to `codexcrew-src-*` would be an agent exfiltration path. It also blocks the
...
- (3) an in-layer chokepoint — `run_aws` calls `assert_chokepoint_allowed()`,
  which under an agent session (`CODEXCREW_SESSION_KEY` set) allows only an
...
`del os.environ['CODEXCREW_SESSION_KEY']` before an in-process call, or — since
...
| `ec2.py` | `deploy`/`status`/`stop`/`start`/`destroy` via `aws cloudformation` + `ec2`; AZ- **and egress-**aware `discover_network` + `resolve_explicit_subnet` (`--subnet` pin, same guarantees); tag-based stateless discovery; `_validate_cidr`. `find_stack` verifies BOTH `codexcrew:managed=true` AND `codexcrew:instance==<tag>` before status/stop/start/destroy touch a stack — so a same-prefix managed stack with a different instance tag can't be acted on by the wrong `--tag`. |
| `iam.py` | Least-privilege launcher policy generator (applied by the user, never by CodexCrew) + read-only reachability check + the **content-fixed instance permissions-boundary document** (`boundary_policy_document`/`boundary_arn`) and its constants (`BOUNDARY_NAME`). |
...
| `templates/codexcrew-ec2.yaml` | The CloudFormation stack. |
...
`/usr/local/sbin/codexcrew-bootstrap`, installs and enables the
`codexcrew-bootstrap.service` systemd oneshot, starts it without blocking, and
...
under `/var/lib/codexcrew-bootstrap/` checkpoint installation separately from
...
(`codexcrew-src-<account>-<region>`); the instance downloads it with its own IAM
...
- **IMDSv2 enforced** on the instance (`HttpTokens: required`, hop limit 1):
  CodexCrew runs a prompt-injectable agent, so an SSRF/injection that reaches the
...
- **Least-privilege, tag-/prefix-scoped IAM.** The RCE-adjacent SSM verbs
  ...
  require `codexcrew:managed=true`; CloudFormation stack mutation/delete is scoped
  to `stack/codexcrew-*/*`, and the change-set verbs (which `aws cloudformation
  deploy` authorizes on the **changeSet ARN**, not just the stack ARN) are a
  separate statement scoped to both `changeSet/codexcrew-*/*` and
  `stack/codexcrew-*/*` (scoping to `stack/*` alone would deny the launch under
  the generated policy); only enumerate/`GetTemplateSummary` stay on `*`.
  `iam:PassRole` is scoped to `codexcrew-ec2-*`; S3 is scoped to `codexcrew-src-*`.
  Command-history
...
- **Create verbs require the managed request-tag (per-resource-ARN split).**
  ...
  `aws:RequestTag/codexcrew:managed=true`, so a leaked launcher credential can't
...
- **Escalation primitives constrained + immutable, pre-created permissions
  ...
  can't tag an *existing* resource `codexcrew:managed=true` to pull it under the
  ...
  self-authorship gap:
  - The boundary is a **single** managed policy named `codexcrew-ec2-boundary`
    ...
    **not** per-launch CloudFormation. It is create-if-not-exists (tolerates
    ...
    `codexcrew-src-<account>-*/*` (region-agnostic — IAM is global; the
  ...
  - The template no longer creates the boundary; the `InstanceRole` references it
    ...
    `^arn:aws:iam::[0-9]{12}:policy/codexcrew-ec2-boundary$`), which the launcher
    fills with `arn:aws:iam::<account>:policy/codexcrew-ec2-boundary`.
  ...
  - `iam:CreateRole` remains gated on `ArnLike iam:PermissionsBoundary ==
    arn:…:policy/codexcrew-ec2-boundary` (`ArnLike`, NOT `StringEquals` — the
  ...
  - **Residual (first-write race), tracked in as-built:** the very first
    ...
    hole. Operators who want it gone entirely run `codexcrew cloud iam-boundary`
  ...
  launcher path (`codexcrew-src-${AccountId}-${Region}/${StackTag}/…`), not the
  ...
  tag-gated (`aws:ResourceTag/codexcrew:managed=true`) so a leaked credential
...
- **`PutRolePolicy`/`PassRole` tag-gated (not just name-prefix).** Both
  ...
  `aws:ResourceTag/codexcrew:managed=true` on the target role — not just the
  `codexcrew-ec2-*` name prefix. Without the tag gate, a leaked launcher credential
  could target a **pre-existing** `codexcrew-ec2-*` role that a third party created
  ...
  template's `InstanceRole.Tags`) — carries `codexcrew:managed=true`, and the tag
...
- **`iam:TagRole` gated so the tag gate can't be self-defeated.** The tag gate
  ...
  unbounded `codexcrew-ec2-*` role `codexcrew:managed=true`, then inline admin +
  pass it. `iam:TagRole` is therefore **not** unconditioned in the role-management
  statement; it is its own statement gated on
  `aws:ResourceTag/codexcrew:managed=true` (`IamTagRoleOnManaged`). `TagRole` is
...
- **Anti-squat bucket pin, end to end.** The launcher source bucket name is
  deterministic (`codexcrew-src-<account>-<region>`) and thus globally
  ...
  verification; if the account can't be derived (the `codexcrew-src-unknown-*`
  ...
  reused — not just on create, so a pre-existing `codexcrew-src-*` bucket whose
...
- **Secret hygiene in source shipping.** Both packaging paths ship only
  ...
  **non-recursively** and skips gitlink directories: `git ls-files` lists a
  ...
  a directory for us. On top of that, both paths run the denylist (`.codexcrew*`,
  `.aws`, `.ssh`, `.gnupg`, `.env*`, `*.pem`/`*.key`/`*.p12`, credential
  filenames) and drop a custom-named `CODEXCREW_HOME` (incl. nested) under the
...
`codexcrew cloud launch`. They install *client* prerequisites only — the gateway
...
`codexcrew cloud launch` runs `python -m codex_crew`, which imports the whole CLI —
