...
| Mirror | Upstream |
|---|---|
| [kiro-cli/](kiro-cli/README.md) | The `kiro-cli` documentation, taken from the 2.x line. kiro-cli is Codex Crew's required agent backend, so its ACP surface, agent-spec schema, and MCP config shape are load-bearing here. |

Where Codex Crew's own behavior differs from a mirrored page, Codex Crew's docs win:
