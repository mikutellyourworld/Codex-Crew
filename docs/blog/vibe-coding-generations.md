...
- 2.0: there are too many sessions to hold in your head, and you have stopped
  ...
  Codex Crew's interface is here today.
...
Codex Crew already has the first part and the fourth, plus a narrow version of the
...
(`src/codex_crew/vector_memory.py:1562` and `:1651`, on `4506e9c92`), which is a
half-life of about 23 days. The score is then rounded to four decimals
(`src/codex_crew/vector_memory.py:1565`), so once a memory is roughly a year old a
typical score underflows to `0.0000` and the sort order is gone with it.
Retrieval benchmarking with the harness in
[#2123](https://github.com/mikutellyourworld/Codex-Crew/pull/2123) measures turn-level
