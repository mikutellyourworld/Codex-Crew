...
loaded, plus the two invariants every new Codex Crew MCP tool must satisfy: it
...
> **Design invariant: Codex Crew does NOT write to provider globals.**
> `~/.kiro/settings/mcp.json` is user-owned. Codex Crew reads it and never mutates
> it. Codex Crew's own additions go into the per-agent file it fully owns,
> `~/.kiro/agents/codexcrew.json`. That keeps tools scoped to Codex Crew out of every
> interactive kiro-cli and Kiro IDE session the user runs outside Codex Crew. If
> `codexcrew-core` / `codexcrew-cron` ever appear in a provider global, that is
> leftover state from an older install: clean it from the dashboard MCP panel,
> or run `codexcrew cli-setup`, which calls the narrowly-scoped
...
|------|-------|---------|---------|
| `~/.kiro/agents/codexcrew.json` | Codex Crew gateway (`agent.rebuild_agent_config`) | The rendered Kiro agent: model + tools + merged `mcpServers` | kiro-cli, when spawned as the `codexcrew` agent |
| `~/.kiro/settings/mcp.json` | User | Kiro global MCP servers | kiro-cli for all agents; merged into Codex Crew's agent file at render time |
| `~/.codex-crew/mcp.json` | User, via the dashboard MCP panel | specific to Codex Crew additions and per-server tool disables | Codex Crew gateway only |

`rebuild_agent_config()` writes exactly **one** file, `~/.kiro/agents/codexcrew.json`.
There is no second rendered agent file and no agent-file renderer for any other
provider: Codex Crew is KiroACP-only.
...
The existing `~/.kiro/agents/codexcrew.json` is the merge **base** when one
...
4. **`~/.codex-crew/mcp.json`** via `update()` on an existing entry, so
   Codex Crew's `command`/`args`/`env` win while user-set fields such as
   `autoApprove` survive.

Kiro global outranks any seam-contributed provider global because Codex Crew is
...
same server's spec from the other sources in priority order (codexcrew, then
...
```json
{
  ...
  "mcpServers": {
    "codexcrew-core":     { "command": "…", "args": ["mcp-core"] },
    "codexcrew-cron":     { "command": "…", "args": ["mcp-cron"] },
    "codexcrew-computer": { "command": "…", "args": ["mcp-computer"] }
  }
}
```
...
Codex Crew forces `false` on every agent it manages (the primary agent and every
app agent). Plain kiro-cli agents outside Codex Crew keep kiro-cli's own default.

### Managed servers

`agent._MANAGED_MCP_SERVERS` holds the three servers the gateway owns end to
end: `codexcrew-cron`, `codexcrew-core`, `codexcrew-computer`. Each is refreshed on
every rebuild by `_refresh_dynamic_fields()`, which rewrites `command`/`args`
from the live `codexcrew` binary, strips stale remote-transport fields (`url`,
`headers`) left by older builds, and re-pins `env.CODEXCREW_HOME` to the home the
...
EMISSION time. `codexcrew-computer` is the one row that has one, and the
...
`codexcrew doctor` is a third, read-only consumer: its MCP sections resolve the
...
`codexcrew-computer` carries **no `autoApprove` key and none may ever be added.**
...
`~/.kiro/agents/codexcrew.json` has two independent writers: this whole-file
...
`list_servers()` reads `~/.kiro/agents/codexcrew.json`, then each scope file with
...
dashboard can render per-scope badges. The `codexcrew` badge is the **effective**
state after the merge minus explicit `disabled: true` overrides in
`~/.codex-crew/mcp.json`; the other badges are raw membership in that scope's
file.
...
- **A probe that could not RUN is reported as a probe limitation, not a server
  ...
  - **A managed server FALLS BACK to its declared tool list when — and only when —
    the sandbox refuses.** Every managed server declares its tools statically in
    this package (`mcp_core._list_tools()` and friends, the very functions the
...
`_fix_stale_managed_command()` re-resolves the `codexcrew` binary on every
`list_servers()` call, because the stored absolute path goes stale after an
update: first `agent._resolve_codexcrew_bin()`, then `shutil.which("codexcrew")`
...
servers a machine runs ships with Codex Crew and nothing leaves the host.
...
1. **Uninstalls first.** `_purge_server_config()` removes the entry from
   `~/.codex-crew/mcp.json`, the Kiro global, every seam-contributed scope, and
   directly from `~/.kiro/agents/codexcrew.json`. That last targeted delete is
...
3. **Scope removes** strip it. If the server would no longer be inherited into
   Codex Crew but the user kept the Codex Crew badge on, the full spec is first
   copied into `~/.codex-crew/mcp.json` (the **preservation rule**), which is why
...
kiro-cli re-read a file it already watches. `codex_crew/mcp_hot_reload.py` owns
...
`apps.bridges._register_mcp_servers()` writes them into Codex Crew's agent config
...
An app agent that references a host-managed server (`@codexcrew-core`,
`@codexcrew-cron`) in its `tools` gets the launch spec copied in by
...
`~/.kiro/agents/codexcrew.json`:

| Server | Process | Tools |
|--------|---------|-------|
| `codexcrew-cron` | `codexcrew mcp-cron` (`mcp_cron.py`) | `cron_add`, `cron_list`, `cron_update`, `cron_remove`, `cron_remove_all`, `cron_pause`, `cron_resume`, `cron_trigger` |
| `codexcrew-core` | `codexcrew mcp-core` (`mcp_core.py` + `mcp_tools/`) | spawn/subagent, learn, task, messaging, artifact, workflow, knowledge and session-directive tools (see below) |
| `codexcrew-computer` | `codexcrew mcp-computer` (`mcp_computer.py`) | `computer_list_apps`, `computer_launch_app`, `computer_get_state`, `computer_click`, `computer_drag`, `computer_type_text`, `computer_press_key`, `computer_set_value`, `computer_scroll`, `computer_perform_action`, `computer_end_turn` |
| `codexcrew-dashboard` | `codexcrew mcp-dashboard` (`mcp_dashboard.py`) | `chat_folder_tree`, `chat_folder_create`, `chat_folder_move`, `chat_folder_move_session` |
| `codexcrew-consult` | `codexcrew mcp-consult` (`mcp_consult.py`) | `consult_model` (one-shot OpenAI-compatible second opinion) |

CLI commands and their MCP twins:

| CLI command | MCP tool | Server |
|-------------|----------|--------|
| `codexcrew cron add` | `cron_add` | `codexcrew-cron` |
| `codexcrew cron list` | `cron_list` | `codexcrew-cron` |
| `codexcrew cron update` | `cron_update` | `codexcrew-cron` |
| `codexcrew cron remove` | `cron_remove` | `codexcrew-cron` |
| `codexcrew cron remove-all` | `cron_remove_all` | `codexcrew-cron` |
| `codexcrew cron pause` | `cron_pause` | `codexcrew-cron` |
| `codexcrew cron resume` | `cron_resume` | `codexcrew-cron` |
| `codexcrew cron trigger` | `cron_trigger` | `codexcrew-cron` |
| `codexcrew spawn run` | `spawn_run` | `codexcrew-core` |
| `codexcrew spawn list` | `spawn_list` | `codexcrew-core` |
| `codexcrew learn add` | `learn_add` | `codexcrew-core` |
| `codexcrew learn list` | `learn_list` | `codexcrew-core` |
| `codexcrew learn remove` | `learn_remove` | `codexcrew-core` |
| `codexcrew run TASK.md` | `task_run` | `codexcrew-core` |
| `codexcrew computer apps` | `computer_list_apps` | `codexcrew-computer` |

`codexcrew-core` tools with no CLI twin, grouped by concern (authoritative list:
`codex_crew.mcp_tools.build_tool_list()`, which is what `mcp_core._list_tools`
...
### A `codexcrew-core` tool has two halves

Each tool is declared twice in the same per-domain module under
`codex_crew/mcp_tools/` (`spawn.py`, `artifacts.py`, `workflows.py`, …), and
...
that rebinds one (`patch("codex_crew.mcp_core._post")`, `setattr(mcp_core, "sel",
...
## What belongs in `codexcrew-core`, and what does not

`codexcrew-core` is the surface EVERY session carries. kiro-cli reads `tools/list`
...
use it. With `agent.tool_search` on (the default) Codex Crew forces kiro's deferral
...
- **Its own server** is for a capability an agent is granted on purpose. Give it
  the `codexcrew-dashboard` shape: an **assignable set**, marked `opt_in` in
...
`@<server>` ref in `tools`. Only `codexcrew.json` is rewritten on install, and a
...
### OpenAI-compatible consultation

`codexcrew-consult` is an opt-in intelligence source, not another provider: Kiro
still owns the session, tools, approvals and final answer, while `consult_model`
requests one untrusted second opinion. In **Settings → Secrets**, create one
entry named `OPENAI_COMPAT_CONFIG`; its value is one atomic JSON record so a
provider switch cannot pair a new key with an old endpoint:

```json
{"version":1,"endpoint":"https://api.example.com/v1/chat/completions","model":"provider-model-id","api_key":"<provider key>"}
```

The endpoint must use public HTTPS on port 443, end in `/chat/completions`, and
carry no query or fragment.

Then grant the agent the `codexcrew-consult` managed server (the
`mcpServers.codexcrew-consult` entry plus `@codexcrew-consult` in `tools`). It is
not added to a default agent and carries no `autoApprove`; assignment is the
explicit tool-surface consent for paid external egress. The loopback route uses
the gateway's shared internal secret like other managed tools, so the server
assignment—not a separate route credential—is the per-agent boundary. The MCP
process sees only the prompt and a redacted answer. The gateway reads the atomic
vault record, strips known credentials and exfiltration URLs from the prompt,
pins the vetted public DNS address, disables redirects, sends no tools or
history, and bounds config/DNS/request/response under one 30-second deadline.
It never returns upstream error bodies. Local/private/plain-HTTP endpoints are
refused rather than treated as OpenAI-compatible exceptions.
...
something outside Codex Crew (desktop input synthesis, the operator's logged-in
...
`_MANAGED_SERVER_TOOL_MODULES`, `mcp_cleanup.CODEXCREW_BIN_MCP_SERVERS`,
...
`codexcrew computer call <tool>` has **no MCP twin, on purpose.** It is not a
...
**A new `codexcrew-core` or `codexcrew-cron` tool MUST NOT keep per-caller or
...
**1. Identity is not the process, it is the call.** `CODEXCREW_SESSION_KEY` and
...
- `_resolve_session_key_strict` for **anything that mutates or targets a
  ...
  client-forged `codexcrew.caller` block), the injected `CODEXCREW_SESSION_KEY`, or
  a `CODEXCREW_HOST_PID` lookup whose HMAC sidecar verifies against the
...
`_meta.kiro.mcpServerName` equal to `codexcrew-core`) is in `DIRECTIVE_TOOLS`. The
...
**MCP tools not working.** Check that `~/.kiro/agents/codexcrew.json` contains
`codexcrew-core` and `codexcrew-cron`, that `includeMcpJson` is `false`, then run
`codexcrew doctor` (which checks probe status) and read the live probe results in
...
**Tools present in Codex Crew but absent in interactive kiro-cli.** That is correct.
`codexcrew-core` / `codexcrew-cron` / `codexcrew-computer` are agent-scoped and must
...
entry never reached `~/.kiro/agents/codexcrew.json` — check `codexcrew doctor` —
...
Apply & Restart, or `codexcrew config set`, which triggers a restart.
