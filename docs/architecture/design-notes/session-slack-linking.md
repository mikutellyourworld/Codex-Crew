...
How a Slack thread maps onto a Codex Crew session, and how messages mirror in both
...
`~/.codex-crew/session_map.json`), not in a gateway-lifetime dict, so it survives a
...
| Slack thread imported to dashboard | `!link-to-dashboard` (`/codexcrew link-to-dashboard`) fetches the thread, redacts each message, imports up to the last 50 into a fresh slot, then `link_slack`. Idempotent: an already-linked thread returns its existing slot. |
| `/codexcrew sessions` resume | Posts a resume header in-thread or in a DM, then `set_slack_link` plus `dashboard_state.link_slack`. |
