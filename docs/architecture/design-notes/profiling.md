...
Codex Crew ships aggregate duration histograms (`codex_crew.metrics`, off by
default) and a stall detector that dumps thread stacks when the event loop wedges
(`codex_crew.dashboard.loop_watchdog`). Neither attributes time to call paths.
`codexcrew perf sample` fills that gap: it turns a window of execution into
...
```bash
export CODEXCREW_DEBUG=1
```

Without it every `perf` command refuses and explains how to enable it. A value of
`0`, `false`, `no` or `off` also reads as disabled, so `CODEXCREW_DEBUG=0` does
...
```bash
CODEXCREW_DEBUG=1 codexcrew perf sample \
  --call codex_crew.history:ConversationLog.list_sessions \
  --interval 0.002 \
...
running gateway's PID read from `$CODEXCREW_HOME/gateway.lock`.

```bash
pip install 'py-spy>=0.3,<1'
CODEXCREW_DEBUG=1 codexcrew perf sample --seconds 30 --output /tmp/gateway.folded
```
...
## Profiling the desktop app (`codexcrew desktop metrics`)
...
`codexcrew desktop metrics` covers that half:

```
CODEXCREW_DEBUG=1 codexcrew desktop metrics
```
...
Rather than add one, the app **records**: when it starts with `CODEXCREW_DEBUG`
...
- The app must have been **started** with `CODEXCREW_DEBUG` set. Setting the
...
| Platform | Path |
|---|---|
| macOS | `~/Library/Logs/CodexCrew/desktop-metrics.json` |
| Linux | `${XDG_CONFIG_HOME:-~/.config}/CodexCrew/logs/desktop-metrics.json` |
| Windows | `%APPDATA%\CodexCrew\logs\desktop-metrics.json` |

Nightly builds install under their own product name (`CodexCrew Nightly`), so they
log to a sibling directory -- `~/Library/Logs/CodexCrew Nightly/` and equivalents.
...
```
run (codex_crew/cli.py:702);search (codex_crew/history.py:412) 184
```
...
| `--output PATH` | Where to write the profile (default `./codexcrew-profile.folded`) |
