...
| `computer_use/types.py` `MAX_SCREENSHOT_MAX_PX` | Codex Crew's own window captures, bounded by the inline cap |
...
Codex Crew code is on the path and there is nothing to intercept.
...
```bash
python -m codex_crew.session_image_repair ~/.kiro/sessions/cli/<session-id>.jsonl
```

Then, with the session ENDED (not merely idle -- see below):

```bash
python -m codex_crew.session_image_repair --apply ~/.kiro/sessions/cli/<session-id>.jsonl
```
...
- It is not a sweeper and not a `codexcrew` subcommand, on purpose: it rewrites
