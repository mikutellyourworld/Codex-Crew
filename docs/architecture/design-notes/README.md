...
| [session-slack-linking.md](session-slack-linking.md) | How a Slack thread maps onto a Codex Crew session, and how thread state stays in sync. |
| [mcp-oauth-ownership.md](mcp-oauth-ownership.md) | The kiro-cli credential boundary for an OAuth MCP server: which path establishes a grant, the paired token and registration artifacts it leaves, and the stat-only surface Codex Crew observes them through. |
