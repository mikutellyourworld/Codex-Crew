...
`codexcrew bench` measures the Codex Crew memory layer against two published benchmarks.
It exists to answer one question the existing `codexcrew eval` cannot: **did this
change make the agent's memory better or worse?**

## Why not extend `codexcrew eval`

`codexcrew eval` runs 4 hand-written scenarios carrying 19 substring assertions, in
...
alone). nltk is not a Codex Crew dependency, so the port uses `snowballstemmer`'s
...
`CODEXCREW_EMBED_MODEL_PATH` (and `memory.embed_model_path`) make Codex Crew run a
...
```bash
codexcrew bench list                    # corpora, sizes, what is cached
codexcrew bench fetch locomo10          # download + verify (2.8 MB)

# the deterministic ruler
codexcrew bench retrieval locomo10

# isolate semantic ranking from the recency decay
codexcrew bench retrieval locomo10 --timeline now

# relax near-duplicate rejection (byte-identical text is always dropped regardless)
codexcrew bench retrieval locomo10 --no-dedup

# smoke the harness on a host that cannot load the embedder
codexcrew bench retrieval locomo10 --instances 1 --queries 25 --toy-embedder

# A/B two commits
codexcrew bench retrieval locomo10 --stem before   # on main
codexcrew bench retrieval locomo10 --stem after    # on the branch
codexcrew bench compare bench_results/before.json bench_results/after.json
```
