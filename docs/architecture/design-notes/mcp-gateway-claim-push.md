...
gatewayd stamps a caller identity (`_meta.codexcrew.caller`) on every MCP call
...
- `src/codex_crew/mcp_gateway/claim.py` — frame builder + sender (stdlib-only)
- `src/codex_crew/mcp_gateway/gatewayd.py` — `_StubConn`, `_CONN_INDEX`,
  `_apply_claim`, claim first-frame dispatch, per-frame caller pickup
- `src/codex_crew/mcp_gateway/stub.py` — `ancestor_pids` on register; unbounded
  backoff recaller
- `src/codex_crew/acp/client.py`, `src/codex_crew/acp/session_provider.py` —
  `rekey()` claim hooks
