...
3. Write the **full original response** to `~/.codex-crew/mcp_spill/<server>-<request_id>-<timestamp>.json`.
4. Truncate each text item to the first 16 KiB.
5. Append a marker: `[CodexCrew: response truncated -- full <N> bytes at <path>. Read with bash: head/grep/jq.]`
...
per-image budget the prompt path enforces (`codex_crew/imaging.py`:
...
hop Codex Crew owns between an MCP server and kiro-cli's history). Two shapes
deliberately stay outside it: images produced by kiro-cli's own built-in
tools (e.g. its file-read tool in image mode) never transit Codex Crew and
...
- **Directory:** `~/.codex-crew/mcp_spill/` (mode 0700)
...
```
[CodexCrew: response truncated -- full 1482937 bytes at /home/user/.codex-crew/mcp_spill/example-mcp-gw-12345-7-1721200000.json. Read with bash: head/grep/jq.]
```
...
2. **Raise the limit** — set `CODEXCREW_MCP_READ_LIMIT=134217728` (128 MiB) in your env, or add to `~/.codex-crew/config.json`:
   ```json
   {
     "mcp_gateway": {
   ...
   ```
3. **Restart the gateway** — `codexcrew restart`
...
3. To raise the threshold: `CODEXCREW_MCP_SPILL_THRESHOLD=1048576` (1 MiB)
