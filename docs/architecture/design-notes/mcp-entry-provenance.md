...
Codex Crew writes MCP server entries into two config files it does not own — the
...
> An entry in a shared config file is Codex Crew–managed **iff** it carries our
> marker. Name presence in the store is necessary, never sufficient. An
> unmarked entry is the user's and is never rewritten.

## The marker

Entries Codex Crew writes into shared files carry one reserved key:

    "x-codexcrew": { "managed": true }
...
that a field the user set and Codex Crew never models survives a rebuild. One field
...
(#4955).
    ...
    "x-codexcrew-derived": { "from": "npx", "emitted": "/abs/npx" }
...
Deliberately a second key rather than a field inside `x-codexcrew`. The two records
