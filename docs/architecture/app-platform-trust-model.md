...
Codex Crew's app platform loads app Python directly into the gateway process
(`apps/module_loader.py` → `importlib` → `exec_module`). This page documents the
resulting trust boundary and how Codex Crew makes it explicit.
...
> **Admitting and enabling an app is therefore equivalent to running that code with the same privileges as Codex Crew itself.** Only trust apps you trust.

## How Codex Crew makes the boundary explicit
