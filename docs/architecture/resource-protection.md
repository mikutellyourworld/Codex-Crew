...
Codex Crew runs long-lived LLM sessions that spawn OS processes (kiro-cli, MCP servers) across
...
| Subagent injection inner cap | `slack/gateway.py` | Per `stream_and_collect` | 900s (`INJECTION_TIMEOUT`, from `_DEFAULT_INJECTION_TIMEOUT`; override with `CODEXCREW_INJECTION_TIMEOUT`, clamped down to `_ON_DONE_TIMEOUT`) | No | `_inject_with_retry` up to 3 attempts with backoff, bounded by the outer 1200s cap |
...
`src/codex_crew`. A synchronous spawn wedges a worker thread rather than the event
...
### The aggregate slice ceiling (`memory.high` on `codexcrew-agents.slice`)
...
**`MemoryHigh`** on `codexcrew-agents.slice`, always **75% of physical RAM**
...
set-property --runtime codexcrew-agents.slice MemoryHigh=<N>M`, run by the unprivileged user
...
- **`MemoryMax`** plus **`MemorySwapMax=0`** on `codexcrew-agents.slice`, default **80% of
...
- **The periodic sweep's active set unions live shared-runtime PIDs.** Every `AcpRuntime`
  ...
  `codexcrew-lite` runtime (`_bg_runtime`). The sweep unions
