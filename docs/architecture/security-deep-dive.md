...
The security **architecture**: what Codex Crew defends against, where its trust
...
Codex Crew runs an LLM agent with filesystem and shell access on the operator's own
...
|---|---|---|---|
| Gateway process ↔ agent subprocess | Codex Crew gateway | `kiro-cli` + every tool/MCP descendant | OS sandbox (`sandbox.py`), env scrub, cgroup scope |
...
Codex Crew's own gate, not the agent's.** Denied commands and the governance
...
on the way in. Windows has no Codex Crew OS wrapper, so positively identified
...
`dashboard/handlers/core.py`). `"off"` skips Codex Crew's own sandbox but still
delegates to `kiro-cli`'s internal agent sandbox on macOS when it is enabled,
which cannot nest inside Codex Crew's
...
re-enables Codex Crew's own sandbox.
...
- **Failure is refusal, not degradation.** With no sandbox backend available and
  ...
  **platform-independent** — a platform-derived default would grant unconfined
  ...
  the discoverable path is instead a consent step in `codexcrew setup`, which
...
- **Delegation is audited, never silent.** When `kiro-cli`'s internal sandbox owns
  ...
  the audit cannot be written, the delegation is refused. Codex Crew's own Seatbelt
...
`SSH_AUTH_SOCK` is scrubbed whenever a Codex Crew sandbox tier is active, so
...
`hooks.safe_read_file()` is the guarded read used by Codex Crew's own non-tool file
...
legacy home is fenced identically to the current `~/.codex-crew`.
...
- **Denied-command rules** (`BUILTIN_DENIED_RULES`): first-class
  ...
  self-protection (the agent disabling Codex Crew or minting its own dashboard
...
surface: they back the `codexcrew security audit` history scan and the posture
...
is a per-surface, narrow-only ceiling. Both are enforced at Codex Crew's own
...
back the explicit `/codexcrew dashboard` link command.
...
does not reopen the control: because `CodexCrewConfig.load()` degrades a torn
...
`load_credentials()` tightens `~/.codex-crew/.env` to owner-only mode at load time
...
**Write protection covers Codex Crew's own trust root, not the user's shell
