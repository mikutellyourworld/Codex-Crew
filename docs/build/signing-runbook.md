...
Operational reference for Codex Crew's macOS signing chain: signing via the
...
Published **filenames** are pinned to the `CodexCrew` basename on every channel,
even though the nightly bundle is `CodexCrew Nightly.app`. CDN keys and the
latest-DMG permalink (`desktop/<channel>/latest/CodexCrew.dmg`) are a public
...
- **CI copy: AWS Secrets Manager**, secret id `codexcrew/signing/apple-notary`,
...
- **Local copy: the macOS Keychain**, via
  `xcrun notarytool store-credentials "CodexCrewNotary" ...`. Every local command
  then uses `--keychain-profile "CodexCrewNotary"`.
...
`xcrun notarytool log <submission-id> --keychain-profile CodexCrewNotary`. Every
