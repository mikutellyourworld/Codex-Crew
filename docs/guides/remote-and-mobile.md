...
Run Codex Crew on an always-on host (a VPS, a cloud VM, a Linux desktop, a spare
...
- **Node.js**: needed to build the dashboard bundle. `website/package.json`
  declares `"node": ">=22"`; `codexcrew doctor` warns below Node 22.
...
Codex Crew drives the `kiro-cli` agent over ACP, and it is the only provider
...
The credentials Codex Crew itself reads from `~/.codex-crew/.env` are chat-platform
and owner-identity credentials (`SLACK_BOT_TOKEN`, `SLACK_APP_TOKEN`,
`CODEXCREW_OWNER_ID`, and the equivalents for Discord / Telegram / Teams / WeCom
...
cp -R website/dist src/codex_crew/static/dist
...
# Configure
codexcrew setup
codexcrew doctor            # verify everything, including kiro-cli and Node
```

`codexcrew setup` writes the resolved project directory to
`~/.codex-crew/project_dir` so the CLI works from any working directory.
...
```bash
tmux new -s codexcrew
codexcrew gateway
# Ctrl+B, D to detach; reattach with: tmux attach -t codexcrew
```
...
gateway can own the dashboard port, which defaults to `5486`
(`config/loader.py` `_DEFAULT_PORT`, overridable with the `CODEXCREW_PORT`
environment variable). Two gateways on one port means the second one fails to
bind.

```bash
tmux kill-session -t codexcrew
```
...
| Category | Path | Why |
|---|---|---|
| Structured memory | `~/.codex-crew/workspace/memory/` | `preferences.md`, `projects.md`, daily `history/` |
| Vector + FTS databases | `~/.codex-crew/memory.db`, `memory_index.db` | Semantic/episodic memory (`vector_memory.py`) and the FTS5 index (`memory.py`) |
| Lessons | `~/.codex-crew/memory.db`, falling back to `lessons.jsonl` | See the note below |
| Config | `~/.codex-crew/config.json` | Chat-platform tokens, model prefs, dashboard settings |
| Skills | `~/.codex-crew/skills/` | Custom skill definitions |
| Webhook hooks | `~/.codex-crew/hooks.json` | Script-hook definitions (`hooks.py` `ScriptHookStore`) |
| Cron jobs | `~/.codex-crew/crons.json` | Scheduled recurring jobs (`cron.py`) |
...
- `~/.codex-crew/kiro_pids.txt`, `kiro_session_pids.txt`, and the
  `session_pid_<pid>.txt` / `.sig` sidecars: live PID tracking
  (`session_pid.py`), meaningless on another host and actively misleading if
  copied.
- `~/.codex-crew/security_events.jsonl`: the tamper-evident SEL audit chain
  (`sel.py`); it belongs to the host that wrote it.
- `~/.codex-crew/.env`, `.local_secret`, `sel_hmac.key`: secrets. Re-enter the
  `.env` credentials with `codexcrew setup`; the other two are regenerated.

Keeping two hosts loosely in sync afterwards is just rsync (replace the SSH
target):

```bash
alias mc-sync='rsync -avz ~/.codex-crew/workspace/memory/ user@your-host.example.com:~/.codex-crew/workspace/memory/ \
  && rsync -avz ~/.codex-crew/memory.db ~/.codex-crew/memory_index.db user@your-host.example.com:~/.codex-crew/'
```
...
bind override is `CODEXCREW_BIND` (an IP address, validated by
...
```bash
ssh -L 5486:localhost:5486 user@your-host.example.com
```

Then open `http://localhost:5486` locally. For a non-default remote port, match
both sides:
...
compression by default, while Codex Crew's own tunnel manager turns it on for
exactly this traffic (`src/codex_crew/instances/ssh_tunnel_manager.py`): the
...
Host your-host.example.com
    LocalForward 5486 localhost:5486
```
...
remote one (`ssh -L 8777:localhost:5486`), the browser sends Origin
`http://localhost:8777`, which is not in the default allowlist. Opt that port in
with `CODEXCREW_ALLOWED_LOOPBACK_PORTS` on the gateway host; the CSRF check
...
cloudflared tunnel create codexcrew
cloudflared tunnel route dns codexcrew codexcrew.example.com
cloudflared tunnel --url http://localhost:5486 run codexcrew
```

With [ngrok](https://ngrok.com/docs) the equivalent is
`ngrok http --domain=codexcrew.example.com 5486`.
...
- [`tailscale serve`](https://tailscale.com/kb/1242/tailscale-serve) publishes the
  ...
  ```bash
  codexcrew config set dashboard.tailscale.enabled true   # once per machine
  codexcrew restart
  codexcrew tailnet up
  codexcrew token                                         # the link to open on the phone
  ```
  ...
  allowlist.
  `codexcrew tailnet up` runs `tailscale serve` for you — HTTPS on 443 in front of
  ...
  login you cannot complete from the phone. `codexcrew token` is the step that hands you
  ...
  takes `--port`, `CODEXCREW_PORT`, or the run marker of a gateway that is actually
  ...
  `codexcrew tailnet status` shows the three things that are independently
  ...
  being wrong looks identical from your phone. `codexcrew tailnet down` stops
  ...
  ```bash
  tailscale serve --bg --https 443 http://127.0.0.1:5486
  codexcrew config set dashboard.tailscale.enabled true
  codexcrew restart
  ```
...
`~/.codex-crew/config.json` and restart:

```json
{
  "dashboard": {
    "url": "https://codexcrew.example.com"
  }
}
```
...
> keeps being used.
>
> **`codexcrew logout` ends those sessions.** It bumps a persisted revocation
...
> auth layer or tear the tunnel down** — logout ends Codex Crew's sessions, not
> the tunnel itself.
...
1. In your Codex Crew DM, send `/codexcrew dashboard` (or `/codexcrew dashboard 6h`).
...
`codexcrew token` does the same thing from a shell on the gateway host.
...
**You re-run `/codexcrew dashboard` or `codexcrew token` roughly once per 30 _idle_
...
pass a duration to raise it (`/codexcrew dashboard 6h`,
`/codexcrew dashboard 20h`). `parse_duration` accepts `<N>h` or `<N>m` and clamps
to the 20-hour ceiling, so asking for more silently gets you 20 hours rather than
an error. `codexcrew token` defaults straight to `20h`. The 5-minute click window
...
- **Signing out in the dashboard** (`POST /api/auth/logout`) — revokes that
  ...
  sessions alive.
- **`codexcrew logout`** — ends **all** sessions globally, refresh chains
...
trust off, and a fresh `codexcrew token` link gets you going again.

Chains persist in `~/.codex-crew/refresh_chains.json` (mode `0600`) — including
...
`http://<host>:5486` from another device on the LAN you get the manifest but no
...
- **Notifications need the app open.** The dashboard raises them through the
  ...
  [issue #2267](https://github.com/mikutellyourworld/Codex-Crew/issues/2267); the Android
  symptom is [issue #1828](https://github.com/mikutellyourworld/Codex-Crew/issues/1828).
...
[`assets/com.codexcrew.tunnel.plist`](assets/com.codexcrew.tunnel.plist):

```bash
cp docs/guides/assets/com.codexcrew.tunnel.plist ~/Library/LaunchAgents/
sed -i '' 's|ALIAS@DEV_DESKTOP_HOSTNAME|user@your-host.example.com|g' \
  ~/Library/LaunchAgents/com.codexcrew.tunnel.plist
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.codexcrew.tunnel.plist
```
...
```bash
curl -s http://localhost:5486/api/health     # {"ok": true, ...}
```

Manage it:

```bash
tail -f /tmp/codexcrew-tunnel.log
launchctl kickstart -k gui/$(id -u)/com.codexcrew.tunnel                            # restart
launchctl bootout gui/$(id -u)/com.codexcrew.tunnel                                 # stop
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.codexcrew.tunnel.plist  # start
```
...
# @raycast.title Open CodexCrew
# @raycast.mode compact
# Optional parameters:
# @raycast.packageName CodexCrew Utils
# Documentation:
# @raycast.description Get a token and open the dashboard
REMOTE_HOST="user@your-host.example.com"
REMOTE_CMD='source ~/.zshrc; codexcrew token'
DEBUG_LOG="/tmp/codexcrew_debug.log"

if lsof -i :5486 -sTCP:LISTEN > /dev/null 2>&1; then
  echo "Tunnel already open"
else
  ssh -fNT -L 5486:localhost:5486 "${REMOTE_HOST}"
  echo "Tunnel opened"
fi

# `codexcrew token` prints only URL(s) on stdout (failures go to stderr). Keep the
# localhost one: that is what routes through the SSH tunnel opened above.
URL="$(ssh -o ConnectTimeout=10 "${REMOTE_HOST}" "${REMOTE_CMD}" 2>"${DEBUG_LOG}" | grep -m1 'localhost:5486')"
...
```bash
codexcrew service install
```

On Linux this writes a **system-level** systemd unit at
`/etc/systemd/system/codexcrew.service` and enables it, so the gateway survives
...
`~/Library/LaunchAgents/dev.codexcrew.gateway.plist` with `RunAtLoad`,
...
`codexcrew stop` unloads the agent so it stays down for the current login session.

```bash
codexcrew service status      # service state
codexcrew logs -f             # tail live logs
codexcrew stop                # stop
codexcrew restart             # restart (service-aware)
codexcrew service uninstall   # remove the unit / plist
```
...
restart). No codexcrew, MCP, or LLM code path runs under sudo. Once started the
...
script [`assets/setup.sh`](assets/setup.sh). It resolves the `codexcrew` binary
and the Node version, renders [`assets/codexcrew.service`](assets/codexcrew.service)
...
cp docs/guides/assets/codexcrew.service ~/.config/systemd/user/
sed -i "s|CODEXCREW_BIN|$(command -v codexcrew)|g" ~/.config/systemd/user/codexcrew.service
sed -i "s/%u/$(whoami)/g" ~/.config/systemd/user/codexcrew.service
sed -i "s/NVM_NODE_VERSION/$(node --version)/g" ~/.config/systemd/user/codexcrew.service
systemctl --user daemon-reload
systemctl --user enable codexcrew
systemctl --user start codexcrew
```

`systemctl --user status codexcrew` should report `active (running)`.
...
| Action | Command |
|---|---|
| Status | `systemctl --user status codexcrew` |
| Restart | `systemctl --user restart codexcrew` |
| Logs | `journalctl --user -u codexcrew -f` |
| Uninstall | `systemctl --user disable --now codexcrew` |

### Hand-rolled system unit

If you want to customize the unit rather than let `codexcrew service install`
generate it:

```bash
CODEXCREW_BIN=$(command -v codexcrew 2>/dev/null || echo "$HOME/.local/bin/codexcrew")

sudo tee /etc/systemd/system/codexcrew.service << EOF
[Unit]
Description=CodexCrew AI Agent Gateway
...
Type=simple
User=$(whoami)
ExecStart=$CODEXCREW_BIN gateway
Restart=always
RestartSec=10
LimitNOFILE=65536
WorkingDirectory=$HOME
Environment=HOME=$HOME
Environment=PATH=$(dirname $CODEXCREW_BIN):$HOME/.local/bin:$HOME/.nvm/versions/node/$(node -v 2>/dev/null || echo v20.0.0)/bin:/usr/local/bin:/usr/bin
...
sudo systemctl daemon-reload
sudo systemctl enable --now codexcrew
```

Tail it with `sudo journalctl -u codexcrew -f`. Getting `PATH` right matters:
...
| Symptom | Fix |
|---|---|
| `codexcrew: command not found` after install | Put pip's script dir on `PATH` (often `~/.local/bin`), then `source ~/.bashrc` or re-login |
| Agent backend errors or timeouts | Confirm `kiro-cli` is on `PATH` and logged in (`kiro-cli login`); `codexcrew doctor` reports its status |
| Service will not start | `sudo journalctl -u codexcrew -n 50` (system unit) or `journalctl --user -u codexcrew -n 50` (user unit) |
| Service restart-loops then gives up | `StartLimitBurst=3` within 5 minutes stops the loop on purpose. Read the logs, fix the cause, then `sudo systemctl reset-failed codexcrew` |
| `systemctl --user` says `Failed to get D-Bus connection` | `export XDG_RUNTIME_DIR=/run/user/$(id -u)` |
| Gateway will not bind the port | Something else already owns it, usually a tmux gateway. `tmux kill-session -t codexcrew`, then `ss -ltnp \| grep 5486` |
| SSH tunnel connection refused | Confirm the gateway is running and listening: `ss -ltnp \| grep 5486` on the remote host |
...
| "Embeddings not ready" in the dashboard | The ~610MB model downloads in the background over HTTPS on gateway start. `codexcrew doctor` probes the resolved URL; set `CODEXCREW_EMBED_MODEL_URL` for a mirror. Memory falls back to keyword search until it lands, and the agent keeps working |

## See also

- [install.md](install.md): all build and install methods
- [docker.md](docker.md): container deployment, including `CODEXCREW_BIND`
