...
These traces prove a Codex Crew change against an isolated gateway running the
...
- `codexcrew pod up/down/ls/status/logs/prune`
- `src/codex_crew/apps/builtins/dev_fleet/skills/pod-e2e/scripts/pod-e2e.sh`

`codexcrew pod api` is not on `main` yet. It arrives with PR #8218. This guide is
...
```bash
codexcrew pod --help | grep -qw api || {
  echo "This recipe requires codexcrew pod api from PR #8218" >&2
  exit 1
}
```
...
`codexcrew pod scenarios` prints each name plus its description, and that listing
...
owned by [`minimal/crons.json`](../../src/codex_crew/tests_fixtures/minimal/crons.json).
...
HANDLE="$(codexcrew pod up "$WT" --seed minimal --json)"
trap 'codexcrew pod down "$WT"' EXIT
...
RESPONSE="$(codexcrew pod api "$WT" GET "$PATH_TO_ASSERT")"
printf '%s\n' "$RESPONSE" | jq -e '
  .status == 200 and
  .ok == true and
  ([.body.jobs[].name] | sort) ==
...
The [pod-e2e skill](../../src/codex_crew/apps/builtins/dev_fleet/skills/pod-e2e/SKILL.md)
...
set -euo pipefail
WT="$(basename "$PWD")"
HARNESS="$PWD/src/codex_crew/apps/builtins/dev_fleet/skills/pod-e2e/scripts/pod-e2e.sh"
ARTIFACT_DIR="$HOME/.codexcrew-pods/.e2e-artifacts/$WT"

HANDLE="$(codexcrew pod up "$WT" --seed rich --json)"
trap 'codexcrew pod down "$WT"' EXIT
printf '%s\n' "$HANDLE" | jq -e '
...
CREATED="$(codexcrew pod api "$WT" POST /api/session-control/create \
  --data '{"title":"cron verification agent"}' --allow-write)"
TARGET="$(printf '%s\n' "$CREATED" | jq -er \
  '.status == 200 and .body.ok == true and .body.target')"

SENT="$(codexcrew pod api "$WT" POST /api/session-control/send \
...
codexcrew pod api "$WT" POST /api/session-control/stop \
  --data "$(jq -nc --arg target "$TARGET" '{target:$target}')" \
  --allow-write >/dev/null
codexcrew pod api "$WT" POST /api/session-control/close \
  --data "$(jq -nc --arg target "$TARGET" '{target:$target}')" \
  --allow-write >/dev/null
```
...
codexcrew pod ls
STATUS="$(codexcrew pod status "$WT" --json)"
printf '%s\n' "$STATUS" | jq -e --arg wt "$WT" \
  '.name == $wt and .status == "up" and (.health == 200 or .health == 401 or .health == 403)' \
  >/dev/null
codexcrew pod logs "$WT" --lines 100
codexcrew pod prune --dry-run --json | jq .
```
...
Use `codexcrew pod prune --all` only when every reported orphan should be
