...
4. Click **Authorize** on the banner Codex Crew raises in chat.
...
error, the provider's authorization host is not on Codex Crew's recognized set
...
Codex Crew renders one agent file it fully owns —
`~/.kiro/agents/codexcrew.json` — and `kiro-cli` loads MCP servers with this
precedence (highest wins for a same-named server; different names are additive):

| Priority | Source | Owner |
|---|---|---|
| 1 (highest) | `mcpServers` in the agent JSON (`~/.kiro/agents/codexcrew.json`) | Codex Crew gateway (`agent.rebuild_agent_config`) |
...
`mcp.json`" instinct fails for the `codexcrew` agent: the agent pins
...
> then go through the approval path like any other. Codex Crew treats this list
...
> `codexcrew doctor` will **revoke** a `@ref` the ceiling disallows and log an
...
> write it into the scope files it manages. Hand-editing `codexcrew.json` is the
...
global shadow the fresh path the gateway just resolved. Codex Crew forces
...
`codexcrew` agent.** The symptom is that no session ever offers the server's
tools and no authentication banner is raised for it — the agent never connects,
so there is no `401` to trigger OAuth.

What makes that hard to read is the dashboard: the MCP panel's `codexcrew` badge
reads **green** for such a server anyway. That badge does not mean "loaded now"
— `mcp_discovery.list_servers()` computes it as *"will this load in Codex Crew
...
(`codexcrew.json`) and add the `@miro` ref to `tools`.
...
end, and Codex Crew's only view of it is the one-directional `_kiro.dev/*`
notifications.
...
failure. kiro-cli hands the URL to Codex Crew over the ACP notification
`_kiro.dev/mcp/oauth_request`
([../reference/kiro-cli/acp.md](../reference/kiro-cli/acp.md), "MCP server
events"), and Codex Crew renders it as an **inline banner** in chat — a lock
...
in the Codex Crew data home; otherwise ask the server owner to fix the URL.
```
...
by Codex Crew's gate, not by the provider.
Codex Crew scans consent URLs for credential-exfiltration patterns, and it
...
`CODEXCREW_HOME` (typically `~/.codex-crew/oauth_endpoints.json`):
...
- **Agent-fenced keystone.** The file sits on Codex Crew's protected keystone
...
scanner), so there is nothing to copy out of Codex Crew here.
...
`clientId` on the entry: Codex Crew maps it to the wire field `oauth.clientId`
...
session**. Codex Crew keeps a **warm session pool**, and a running `kiro-cli acp`
...
## Where the credentials live (and what Codex Crew can see)
...
Codex Crew **stats these files for presence** (`mcp_grant.grant_presence`) to
render the "Signed in" badge, but **nothing in that module opens a token file**
(`mcp_grant.py`): it reads no token value. So Codex Crew can report neither the
...
`connections/registry.json`, and **Miro is not one of them**, so Codex Crew
...
- [../architecture/mcp.md](../architecture/mcp.md) — config hierarchy,
  ...
  probing badges.
- [../architecture/design-notes/mcp-oauth-ownership.md](../architecture/design-notes/mcp-oauth-ownership.md)
  — why kiro-cli owns token custody and what Codex Crew can and cannot see.
- [../reference/kiro-cli/mcp/oauth-token-storage.md](../reference/kiro-cli/mcp/oauth-token-storage.md)
