# Codex Crew behind enterprise MCP governance
...
you hand every host in a fleet the same Codex Crew `security_policy.json`, and it
works on any account type.

## The symptom

Codex Crew starts, the dashboard works, chat works — and a large part of the
...
no tools to work with. Nothing errors. `codexcrew doctor` reports the MCP servers
healthy.
...
- Codex Crew's own probe **spawns each server directly** and completes an MCP
...
- The match is on the **`mcpServers` map key**, not on the command, not on a
  registry id. `codexcrew-core` in your spec must be `codexcrew-core` in the
  registry file.
...
### 1. Declare registry mode on the Codex Crew side

```bash
codexcrew config set agent.mcp_registry_mode true
codexcrew restart
```

Codex Crew then stamps `"type": "registry"` on the servers it manages, so they
...
Verify with `codexcrew doctor`, which grows an `MCP Governance (enterprise)`
section whenever the local identity came from Identity Center.

### 2. Have the administrator allow-list the servers

Codex Crew needs three servers, and they must appear in the registry file under
**exactly** these names:

| Server | What is lost without it |
|---|---|
| `codexcrew-core` | `spawn_run`, `learn_add`, artifacts, knowledge, monitoring — the bulk of the product |
| `codexcrew-cron` | every scheduled job (`cron_add` and the whole cron surface) |
| `codexcrew-computer` | desktop automation (inert unless separately enabled, but still filtered) |
...
and — because all three Codex Crew servers live behind one package — a
...
launches `uvx codexcrew` with no subcommand, which prints CLI help instead of
...
```json
{
  "servers": [
    {
      "name": "codexcrew-core",
      "description": "Codex Crew orchestration: subagents, memory, artifacts, monitoring",
      "version": "0.3.0",
      "packages": [
        {
          "registryType": "pypi",
          "identifier": "codexcrew",
    ...
    {
      "name": "codexcrew-cron",
      "description": "Codex Crew scheduled jobs",
      "version": "0.3.0",
      "packages": [
        {
          "registryType": "pypi",
    ...
    {
      "name": "codexcrew-computer",
      "description": "Codex Crew desktop automation (macOS, opt-in)",
      "version": "0.3.0",
      "packages": [
        {
          "registryType": "pypi",
...
Set `version` to the Codex Crew version your fleet runs.

## Known limitation: the registry launches the server, not your install

Codex Crew's MCP servers are not standalone tools — they are the gateway's own
process, reached through subcommands (`codexcrew mcp-core`, `mcp-cron`,
...
means `uvx` fetching Codex Crew from PyPI into its own ephemeral environment — so
the process serving your MCP tools can be a *different* Codex Crew from the
gateway serving your dashboard. Your `env` overrides (including `CODEXCREW_HOME`)
...
organisation pins Codex Crew centrally, that pin now governs the MCP side too.
...
`mcp.json` servers load alongside registry-managed ones. Codex Crew's servers
live in an agent spec (`~/.kiro/agents/codexcrew.json`), not in personal
`mcp.json`, so that last change does not exempt them.

## Central policy distribution: one security policy, every host

`security_policy.json` is Codex Crew's **enterprise ceiling**: the trust-root
...
different mechanism from the MCP registry above — it governs Codex Crew rather
...
- **The document is the whole ceiling, not a patch.** The fetched policy replaces
  the local one outright; there is no merge with `~/.codex-crew/security_policy.json`
...
| | `CODEXCREW_POLICY_URL` | a `distribution` block in a policy file |
|---|---|---|
...
| Carries a credential | yes, via `CODEXCREW_POLICY_HEADERS` | never — see below |
...
```bash
CODEXCREW_POLICY_URL=https://config.corp.example/codexcrew/security_policy.json
CODEXCREW_POLICY_HEADERS='{"Authorization":"Bearer <per-machine token>"}'
CODEXCREW_POLICY_REFRESH_SECS=900          # 0 = fetch at boot only; floor is 60
CODEXCREW_POLICY_TIMEOUT_SECS=10           # per request; default 10
CODEXCREW_POLICY_MAX_CACHE_AGE_SECS=86400  # 0 = no staleness bound
CODEXCREW_POLICY_ON_UNAVAILABLE=fail_closed  # or: degrade
```
...
itself — that is Codex Crew reaching for its own ceiling before any agent exists,
...
such trap: `CODEXCREW_POLICY_URL` is read from the environment on every poll. Either
way, `refresh_interval_secs` (or `CODEXCREW_POLICY_REFRESH_SECS`) must be non-zero
...
credentials belong in `CODEXCREW_POLICY_HEADERS` (a JSON object of header name to
value).
...
Codex Crew runs as — the file *and* every directory above it**: a source that account can write is one an agent subprocess can
...
`CODEXCREW_SECURITY_POLICY` (tier 1), not this channel. The validator is a content digest,
...
#    any request credential in CODEXCREW_POLICY_HEADERS rather than in the URL, so
...
aws s3 cp security_policy.json s3://corp-config/codexcrew/security_policy.json

# 2. Point the host at it, through whatever already sets environment variables
#    for the Codex Crew service, then restart it once.
export CODEXCREW_POLICY_URL=https://corp-config.s3.amazonaws.com/codexcrew/security_policy.json
export CODEXCREW_POLICY_REFRESH_SECS=900
codexcrew restart

# 3. Verify. `source` reports the posture; `fetch` proves the round trip.
codexcrew policy source
codexcrew policy fetch --force
codexcrew policy show
```

`codexcrew policy source` reports whether central distribution is active, the
...
`codexcrew policy fetch` fetches now, validates the document, and on success
...
`codexcrew policy show` then prints the ceiling actually in effect, and
`codexcrew policy validate` load-checks the policy plus every profile. No `policy`
...
|---|---|
| `fail_closed` (**the default**) | **Codex Crew refuses to start.** A fleet that pointed a host at a central ceiling meant that ceiling to bind, so "we could not reach it" must not read as "run unbounded". |
...
- `CODEXCREW_POLICY_ON_UNAVAILABLE=degrade` — boot, and report the degradation.
- unset `CODEXCREW_POLICY_URL` — stop fetching centrally on this host.
- `CODEXCREW_SECURITY_POLICY=/path/to/local.json` — govern from a local file,
  which outranks the central tier entirely.

A refusal to establish the ceiling aborts every `codexcrew` command on that host,
`policy source` included, because each of them boots the same platform context.
`codexcrew doctor` is the exception and the diagnostic to reach for: it is exempt
...
`CODEXCREW_SECURITY_POLICY` — an explicit **local** file path — outranks the
...
- **On a live refresh, a bad document is refused and the running ceiling is
  ...
  does not outlive the push once you correct it. `codexcrew policy source` reports
...
- **A host that RESTARTS while a bad document is published will try to adopt
  ...
  (with `CODEXCREW_POLICY_URL` pointed at a canary object) before you publish it
  to the fleet's URL.
...
- **No signing runbook ships.** There is no `codexcrew policy sign`, no key
...
- **No MDM or directory integration.** Codex Crew reads an environment variable
...
- **No fleet-compliance reporting.** There is no console that lists which hosts
  ...
  `codexcrew policy source`, the dashboard's policy viewer, and its own audit log
  (which records refresh outcomes that changed something or failed, by scheme, not
  by URL). Aggregation is yours to build — `codexcrew policy fetch`'s exit code is
  the intended hook.
...
- **No staged or percentage rollout.** One URL serves one document to everyone
  ...
  hosts at it with `CODEXCREW_POLICY_URL`.
...
- [../architecture/mcp.md](../architecture/mcp.md) — how Codex Crew composes the
...
- [../../src/codex_crew/docs/troubleshooting.md](../../src/codex_crew/docs/troubleshooting.md)
