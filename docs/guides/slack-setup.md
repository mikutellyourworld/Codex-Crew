...
How to create a Slack app for Codex Crew and connect it.

> **Slack is optional**: If you don't need Slack, skip this entirely. The default `codexcrew setup` configures no messaging channels, and the gateway runs the web dashboard without any. When you are ready to connect Slack, run `codexcrew setup --slack` to enter the tokens created below.

Codex Crew connects to Slack using **Socket Mode**, so it runs entirely from your own machine over an outbound WebSocket: no public URL, no inbound webhooks, and no hosting required. You just need a Slack workspace where you can install an app.
...
- Python and Codex Crew installed through a [supported install path](install.md#install-paths), so you can run the `codexcrew` CLI.
...
```bash
codexcrew manifest -o ~/.codex-crew/slack-manifest.yaml
```

Then:
1. Go to <https://api.slack.com/apps> → **Create New App** → **From a manifest**
2. Select your workspace
3. Paste the contents of `~/.codex-crew/slack-manifest.yaml`
4. Click **Create**
...
> ⚠️ **Your Member ID is per-workspace.** If you install the app in a different workspace, your Member ID changes, so use the ID from the workspace where Codex Crew is installed.

### Step 6. Verify & Run

```bash
codexcrew doctor    # verify tokens and config
codexcrew gateway   # start CodexCrew
```
...
2. Name: something unique (e.g. `codexcrew`). Generic names may conflict with existing apps in the same workspace
...
These scopes belong to the installing user's `xoxp-...` token. The Codex Crew
...
The command name you choose here must match the `slack.command` value in `~/.codex-crew/config.json` (default: `codexcrew`):
...
### Step 10. Configure Codex Crew

Same as [Path A, Step 5](#step-5-configure-codexcrew).
...
If you prefer to configure tokens manually instead of using `codexcrew setup`:

```bash
mkdir -p ~/.codex-crew
cat > ~/.codex-crew/.env << 'EOF'
SLACK_APP_TOKEN=xapp-your-app-token-here
SLACK_BOT_TOKEN=xoxb-your-bot-token-here
CODEXCREW_OWNER_ID=your-slack-member-id
EOF
chmod 600 ~/.codex-crew/.env
```

### Owner-Only Access

Only the owner (`CODEXCREW_OWNER_ID`) can interact with Codex Crew via Slack.
...
> **Setting or changing `CODEXCREW_OWNER_ID` invalidates existing dashboard
...
> Either way, run `codexcrew token` and open the fresh link to re-authenticate
> under the new owner.

### Reaction Emojis

Codex Crew adds phase-aware emoji reactions during message processing (queued → thinking → coding → done). Customize or disable:
...
4. Re-generate the App Token (Socket Mode) and Bot Token, then re-run `codexcrew setup` with the new tokens and your Member ID for that workspace
...
/codexcrew dashboard     # Slash command: 1-hour session
/codexcrew dashboard 30m # Slash command: 30-minute session
```
...
5. **Every** request needs a valid token or cookie, including requests that
   ...
   carve-out is a small set of internal API paths reserved for Codex Crew's own
   processes (doctor, the MCP servers), and those additionally require a
   matching `X-Internal-Secret` read from `~/.codex-crew/.local_secret`

### Dashboard URL Configuration

Set `dashboard.url` in `~/.codex-crew/config.json` to the host and port you reach
the dashboard on:
...
From this single URL, Codex Crew derives:
- **Port** to bind on (8080 in this example)
- **Allowed origins** for the CSRF / WebSocket checks
- **Dashboard link hostname** for `!dashboard` and `/codexcrew dashboard`

When omitted, it defaults to port 5486 and the `localhost` hostname.

The gateway itself always binds loopback in this build: publishing it is your
reverse proxy's or tunnel's job, not the gateway's. `CODEXCREW_BIND` widens the
bind address only (the container image sets `CODEXCREW_BIND=0.0.0.0` so a
...
The slash command name is configurable via `slack.command` in config (default: `codexcrew`).
...
These `!`-prefixed commands are restricted to `CODEXCREW_OWNER_ID`.
...
| If a token is exposed, run `codexcrew logout` (ends all sessions, refresh chains included) and revoke at your tunnel or reverse-proxy auth layer | Paste tokens in Slack channels, shared docs, or wikis |
| Avoid showing the browser URL bar during screen shares | Leave dashboard links in screen-share recordings |
| Leave the built-in `codexcrew token` deny rules enabled | Trust an AI agent that asks to run `codexcrew token` |

`codexcrew logout` ends every issued session, not just in-memory state: it bumps
...
> document that trick your agent into running `codexcrew token` and exfiltrating
> the output. Codex Crew ships built-in denied-command rules covering that mint,
...
| `missing_scope` error | Add the scope in OAuth & Permissions, reinstall app, re-run `codexcrew setup` |
| Bot doesn't respond | Check `codexcrew doctor` output. Ensure gateway is running (`codexcrew gateway`) |
