...
You run the Codex Crew gateway on an EC2 box and drive it from your laptop. The
...
plus the EC2-specific gotchas people actually hit (from `codexcrew doctor`).
...
1. On your laptop, forward the gateway's port over SSH (use the **real** gateway
   ...
   ```bash
   ssh -N -L 5486:localhost:5486 <user>@<ec2-host>
   ```

   Then open `http://localhost:5486`. Run `codexcrew token` on the box to mint the
...
These map to warnings in `codexcrew doctor`.
...
- **Or opt into unsandboxed execution (trades isolation — only on a box you
  trust).** Run `codexcrew setup` (it offers this interactively), or set
  `agent.sandbox_allow_unsandboxed_exec: true` in `~/.codex-crew/config.json`, then
...
kiro-cli must be authenticated **on the box**. Run `codexcrew cloud login` from
...
`codexcrew cloud login` short-circuits when the box already has a session ("already
signed in"), so switching accounts is a sign-out first:

```bash
codexcrew cloud logout    # drops the kiro-cli session on the instance
codexcrew cloud login     # device-code flow for the new account
```
...
`codexcrew doctor`'s "Remote access" hint and its `dashboard: http://localhost:5486`
line show the **defaults**. If your service or shell sets `CODEXCREW_PORT` (e.g.
`7777`), the gateway actually listens **there**, not on 5486 — the doctor line just
didn't see that env var. Your tunnel, browser, and token must all use the **same,
real** port:

```bash
# service has CODEXCREW_PORT=7777 → tunnel 7777, not 5486:
...
- **`ffmpeg: not found`** — only needed for speech-to-text. Drop a static ffmpeg
  build into `~/.local/bin` (it's not in the AL2023 repos; Codex Crew auto-detects
  it).
- **`Vector Memory … vendored runtime failed to load`** — the in-process embedding
  ...
  local vector memory.
- **`project dir: not set`** — cosmetic. Run `codexcrew setup` from a project root
