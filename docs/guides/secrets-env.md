...
runtime.  Codex Crew deliberately keeps secrets **out** of
...
keep working without change — run `codexcrew secrets import --apply` to move the
...
children.  The file itself (`/etc/codexcrew/secrets.env`) is owned by root
...
`_AGENT_DENIED_ENV_KEYS` in `src/codex_crew/sandbox.py` to prevent the
...
// ~/.kiro/mcp.json
{
  "mcpServers": {
    "my-server": {
      ...
      "args": [
        "-c",
        "set -a; . /etc/codexcrew/mcp-secrets.env; set +a; exec my-mcp-server --stdio"
      ]
    }
  }
}
...
| `. /etc/codexcrew/mcp-secrets.env` | Source secrets from a root-owned file. |
...
```bash
sudo install -m 600 /dev/null /etc/codexcrew/mcp-secrets.env
sudo sh -c 'read -rp "Secret: " val && printf "MY_MCP_SECRET=%s\n" "$val" >> /etc/codexcrew/mcp-secrets.env'
```
...
- **Do not** add custom keys to `~/.codex-crew/.env` expecting them to be
...
- **Do not** store MCP secrets in user-readable paths — a file at
  `~/.codex-crew/mcp-secrets.env` or `~/.kiro/.env` is accessible to the
  agent via filesystem reads.  Use root-owned paths (`/etc/codexcrew/`)
  or the vault.
