...
loopback `:5486`, never public), signs kiro-cli in over SSM, registers the box
...
1. **One-time prep.** Install the AWS CLI and `session-manager-plugin` — on a bare
   ...
   `codexcrew cloud doctor`. Attach the least-privilege policy printed by
   `codexcrew cloud iam-policy` to the AWS profile you'll launch with.
2. **Launch.** Run `codexcrew cloud launch` — interactive (size picker + confirm),
   or non-interactive, e.g. `codexcrew cloud launch --size power --region us-west-2
...
3. **Reach it — and the ports.** Launch ends by opening an SSM port-forward and
   ...
   **remote** port: the gateway always listens on loopback `:5486` on the box. The
   **local** forwarded port defaults to `5599` (deliberately not `5486`, so it
   ...
   in place; `codexcrew cloud destroy` removes both the box and this registration.
...
```
Codex Crew Cloud (kc-3f9a)
SSM  i-0abc123456789def0 (us-west-2)   port 5486   TTL 20h · token 19h58m left
```
