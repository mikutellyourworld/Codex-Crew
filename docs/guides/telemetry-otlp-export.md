# Exporting Codex Crew metrics over OTLP

Codex Crew records its metrics through the OpenTelemetry SDK. By default they go
nowhere but this machine: a local JSONL sink under `~/.codex-crew/metrics`, which
...
| `codexcrew[otlp]` extra | not installed | The OTLP exporter is not even importable. |
...
If the extra is missing but an endpoint is configured, Codex Crew logs a warning
and stays local-only rather than failing to start.

### 2. Enable collection

Either edit `~/.codex-crew/config.json`:
...
```bash
CODEXCREW_TELEMETRY=1 codexcrew gateway
```
...
```
telemetry enabled; local JSONL sink at /home/you/.codex-crew/metrics (otlp=on)
...
| `telemetry.local_dir` | `~/.codex-crew/metrics` | Local JSONL shard directory. |
...
Codex Crew exports **delta** by default, for counters, up-down counters and
...
variable. When it is set, Codex Crew passes no preference of its own and the
...
- **Gauges have no temporality at all.** Every instrument reporting a
  ...
  apply to them. Nothing Codex Crew exports is a cumulative sum, which is what
...
- `codexcrew.process.cpu.seconds`
- `codexcrew.process.gc.collections`
- `codexcrew.process.gc.collected`
- `codexcrew.process.gc.uncollectable`
- `codexcrew.inventory.probe.failures`
...
Codex Crew's own dashboard is not exposed to this: the local JSONL exporter stamps
...
The examples below are collector configs. Point Codex Crew at the collector's OTLP
...
exporters:
  awsemf:
    region: us-west-2
    namespace: CodexCrew
    log_group_name: /codexcrew/metrics
...
Two families of instruments, all under the `codexcrew.` namespace:

- **`codexcrew.process.*`** — this process's own resource behavior: Python and OS
  thread counts, open file descriptors, current and peak RSS, cumulative CPU
  seconds, and per-generation GC counters.
- **`codexcrew.inventory.*`** — what this install has configured: active cron jobs,
  ...
  fixed set of feature switches. Plus `codexcrew.inventory.probe.failures`, which is
...
your queries, so you can move that line without a Codex Crew change.

Resource attributes identify the sending process and are attached to every
series, so they are what you group by. `service.name` is always `codexcrew`.
...
pinned for the `codexcrew[otlp]` extra), while the declared floor is
...
This applies to every `codexcrew.*` metric, not just the inventory family, and it is
...
switches default off and OTLP additionally needs the `codexcrew[otlp]` extra, so an
...
measurement of how a feature is used in general. Codex Crew's own install analytics
are a separate, deliberately unrelated channel (`beacon.py`), for reasons its
docstring sets out.

One practical consequence: `codexcrew.inventory.*` comes from the **gateway process
only**, while `codexcrew.process.*` comes from every telemetry-enabled process
...
unless `codexcrew[otlp]` is installed.

### Against a real collector

The automated tiers stop at the process boundary: they prove what Codex Crew
...
Point a collector at Codex Crew using the [Shared receiver](#shared-receiver)
...
interval, and read what arrived. Expect the `codexcrew.process.*` and
`codexcrew.inventory.*` families and the resource attributes described above.
...
server, or armed a loop. `codexcrew.inventory.probe.failures` is the series that
...
ls ~/.codex-crew/metrics/
python3 -m json.tool < ~/.codex-crew/metrics/metrics-*.jsonl | head -40
```
...
|---|---|
| No `telemetry OTLP export active` line | `otlp_endpoint` empty, or `codexcrew[otlp]` not installed — check for the warning naming the missing extra. |
...
| Nothing recorded at all | Collection consent: `telemetry.enabled` is `false` and `CODEXCREW_TELEMETRY` is unset. |
