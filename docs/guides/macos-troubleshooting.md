...
Fixes for problems specific to the macOS desktop app (`CodexCrew.app`). For
install and build steps see [install.md](install.md); for problems that are not
macOS-specific, start with `codexcrew doctor`.
...
1. From a Terminal window whose `PATH` is correct (one where the command
   ...
   Codex Crew. You are copying the same value your shells already use, but if
...
`~/Library/LaunchAgents/dev.codexcrew.path.plist`:
...
<plist version="1.0">
<dict>
  <key>Label</key><string>dev.codexcrew.path</string>
...
`launchctl load ~/Library/LaunchAgents/dev.codexcrew.path.plist`; from then on
...
- Directories read from the launchd domain are **appended after** the app's
  inherited `PATH`. Inside Codex Crew they can make a name resolve that
...
- Only absolute entries are added: a relative entry, or one containing a `..`
  segment, is ignored.
- A Gateway started from a terminal (`codexcrew gateway`) is unaffected — it
