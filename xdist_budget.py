...
_MAX_WORKERS_ENV = "CODEXCREW_MAX_TEST_WORKERS"
_SLOT_DIR_ENV = "CODEXCREW_TEST_SLOT_DIR"
...
def _slot_root() -> pathlib.Path:
    ...
    return pathlib.Path.home() / ".cache" / "codexcrew" / "test-slots"
...
def _slot_dir() -> pathlib.Path:
    ...
    Deliberately host-global and *not* derived from ``CODEXCREW_HOME``: the point
...
def _claim_worker_slots(capacity: int, cap: int) -> int:
    ...
    try:
        ...
        # CODEXCREW_TEST_SLOT_DIR and could redirect our writes.
    ...
    for index in range(capacity):
        ...
        try:
            ...
            # CODEXCREW_TEST_SLOT_DIR somewhere writable -- is only obvious once the cause
            # is named.
            warnings.warn(
...
def resolve_workers() -> int:
    ...
    **Per-run cap** -- the most this single run may take, the tightest of:
    ...
    1. ``CODEXCREW_MAX_TEST_WORKERS``, default 32. The optimal worker count for this
    ...
    2. ``PYTEST_XDIST_AUTO_NUM_WORKERS``, xdist's OWN knob. Honoured as a ceiling
       ...
       implementation and would otherwise discard it silently -- and Codex Crew itself
    ...
    intends to spawn (under ``~/.cache/codexcrew/test-slots/<hostname>``, root
    overridable with ``CODEXCREW_TEST_SLOT_DIR``): a run alone takes the whole
