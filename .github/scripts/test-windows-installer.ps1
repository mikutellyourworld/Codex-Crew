...
$backendRoot = Join-Path $installLocation "resources\backend-dist\codexcrew-backend"
...
$savedGatewayEnv = @{
  CODEXCREW_HOME = $env:CODEXCREW_HOME
  CODEXCREW_PROJECT_DIR = $env:CODEXCREW_PROJECT_DIR
...
try {
  $env:CODEXCREW_HOME = $gatewayHome
  $env:CODEXCREW_PROJECT_DIR = $installLocation
  ...
  $gatewayProcess = Start-Process -FilePath $bundledPython -ArgumentList @(
    "-s", "-m", "codex_crew", "gateway", "--no-open", "--port", "$gatewayPort"
