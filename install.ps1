# ======================================================================
#  CodexCrew - Windows setup (cloud mode)
# ======================================================================
#  This is the CLOUD path: Codex Crew runs on an EC2 Linux instance in YOUR OWN
#  AWS account and this machine is the thin client. Use it when you want the
#  gateway always-on and off your laptop.
#
#  It is NOT the only Windows option. Codex Crew also builds and runs natively
#  on Windows from source (`.\make.ps1 build`, then `codexcrew gateway`) - see
...
Write-Host ""
Write-Host "  CodexCrew - Windows setup (cloud mode)" -ForegroundColor Magenta
Write-Host "  Runs CodexCrew on your own AWS EC2; this PC is the client." -ForegroundColor DarkGray
...
# --- 4. CodexCrew client + launch ------------------------------------------
Write-Step 4 4 "CodexCrew client"
$repoRoot = Split-Path -Parent $MyInvocation.MyCommand.Path
Push-Location $repoRoot
try {
    Write-Info "Installing the Codex Crew client (pip)..."
    & $PyExe -m pip install --upgrade pip *> $null
    $env:CODEXCREW_SKIP_FRONTEND = "1"
    ...
    Write-Ok "CodexCrew client installed"
} finally {
    Pop-Location
}

if (-not (Have "aws")) {
    Write-Warn "Restart your terminal so aws / session-manager-plugin are on PATH, then run: codexcrew cloud launch"
    exit 0
}
if (-not (Have "session-manager-plugin")) {
    ...
    Write-Warn "session-manager-plugin is not on PATH yet. Restart your terminal, then run: codexcrew cloud launch"
    exit 0
}
...
if (-not $identity) {
    ...
    Write-Info "Then run:  codexcrew cloud launch"
    exit 0
}
Write-Ok "AWS credentials resolve."

if ($NonInteractive) {
    Write-Info "Prerequisites ready. Launch when you're set:  codexcrew cloud launch"
    exit 0
}

Write-Host ""
Write-Host "  Launching CodexCrew on AWS..." -ForegroundColor Cyan
if ($Size -ne "") {
    & $PyExe -m codex_crew cloud launch --size $Size
} else {
    & $PyExe -m codex_crew cloud launch
}
