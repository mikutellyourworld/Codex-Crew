"""CodexCrewClient — async Python client for the CodexCrew Gateway.

Standalone package (no dependency on codex_crew main package).
Uses aiohttp for HTTP and WebSocket communication.

Mirrors the TypeScript @codexcrew/client API surface.
"""
...
from codexcrew_client.errors import CodexCrewError, ErrorCode, http_error

logger = logging.getLogger(__name__)

_DEFAULT_BASE_URL = "http://localhost:5486"
_DEFAULT_TIMEOUT = 30
...
def _read_app_secret(app_name: str) -> str:
    ...
    home = os.environ.get("CODEXCREW_HOME", str(Path.home() / ".kiro" / "crew"))
...
class CodexCrewClient:
    """Async HTTP client for the CodexCrew Gateway.
    ...
    def __init__(
        ...
        *,
        ...
        port = os.environ.get("CODEXCREW_PORT", "5486")
    ...
    async def __aenter__(self) -> CodexCrewClient:
    ...
    async def _request(
        ...
        last_error: CodexCrewError | None = None
        ...
        raise last_error or CodexCrewError(ErrorCode.NETWORK_ERROR, "Request failed")
    ...
    async def register_mcp_server(
        ...
        if not name or not command:
            raise CodexCrewError(ErrorCode.VALIDATION_ERROR, "MCP server requires name and command")
    ...
    async def inject_context(
        ...
        *,
        ...
        await self._post(
            f"/api/chat/slots/{slot_id}/context",
            {
                "content": content,
                "source": source,
                "ephemeral": ephemeral,
