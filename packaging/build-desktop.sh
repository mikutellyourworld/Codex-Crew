#!/usr/bin/env bash
# Build the standalone CodexCrew desktop app end-to-end.
#
...
#   3. pip-install codex_crew + deps INTO the bundled interpreter
...
# ships as TWO complete trees (backend-dist/codexcrew-backend-arm64/ and
# .../codexcrew-backend-x64/), selected at launch by find-bin.js via
...
KC_VERSION="$(grep -m1 '__version__' "$ROOT/src/codex_crew/__init__.py" \
  | sed -E 's/.*=[[:space:]]*"([^"]+)".*/\1/')"
if [ -z "$KC_VERSION" ]; then
  echo "ERROR: could not parse __version__ from src/codex_crew/__init__.py" >&2
  exit 1
fi
...
# identity; unstamped dev builds and insider/stable stamps build "CodexCrew".
case "$KC_VERSION" in
  *-nightly.*) PRODUCT_NAME="Codex Crew Nightly" ;;
  *)           PRODUCT_NAME="Codex Crew" ;;
esac
...
#
# The runtime consequence is handled in codex_crew.transcribe: the packaged decoder
...
from codex_crew.stt.engine import probe
from codex_crew.transcribe import _packaged_ffmpeg_version_probe
...
restamp_backends() {
  ...
  while IFS= read -r sp; do
    ...
    found=$((found + 1))
  done < <(find "$ELECTRON_DIR/backend-dist" -type d -path "*/site-packages/codex_crew" 2>/dev/null)
  if [ "$found" -eq 0 ]; then
...
# Install the pinned Codex ACP adapter inside a staged Python package. The
# desktop shell supplies its own Electron executable as the Node runner, so this
# keeps the installed app self-contained without shipping a second Node runtime.
stage_codex_adapter() {
  local sp="$1" target_arch="$2" target_os="$3" vendor_root npm_arch npm_os
  vendor_root="$sp/codex_crew/_vendor"
  case "$target_arch" in
    x86_64|x64|"") npm_arch="x64" ;;
  ...
  log "Staging pinned Codex ACP adapter (${npm_os}/${npm_arch})..."
  mkdir -p "$vendor_root"
  npm_config_os="$npm_os" npm_config_arch="$npm_arch" \
    npm install --prefix "$vendor_root" --omit=dev --no-save \
    --no-package-lock --no-audit --no-fund \
    "@agentclientprotocol/codex-acp@1.10.0"
  test -f "$vendor_root/node_modules/@agentclientprotocol/codex-acp/dist/index.js" || {
    echo "ERROR: staged Codex ACP adapter has no entry script" >&2
    exit 1
  }
  test -d "$vendor_root/node_modules/@agentclientprotocol/sdk" || {
    echo "ERROR: staged Codex ACP adapter is missing its SDK dependency" >&2
    exit 1
  }
}
...
# Copies the interpreter, pip-installs codex_crew (full closure), stages the
# dashboard, writes the relocatable launcher, gates self-containment, prunes.
build_backend() {
  local pbs_dir="$1" out="$2" want_arch="$3" sp

  log "Installing codex_crew into the bundled interpreter ($(basename "$out"))…"
  mkdir -p "$(dirname "$out")"
  cp -R "$pbs_dir" "$out"
  ...
  env PYTHONNOUSERSITE=1 PYTHONPATH= CODEXCREW_SKIP_FRONTEND=1 \
  ...
  #
  # `--only-binary pywhispercpp` (scoped to that one name, so codexcrew itself still
  ...
  if ! env PYTHONNOUSERSITE=1 PYTHONPATH= CODEXCREW_SKIP_FRONTEND=1 \
      "$out/bin/python3.12" -m pip install --prefer-binary \
  ...
  stage_codex_adapter "$sp" "$want_arch" "$OS"
  log "Staging dashboard dist into codex_crew/static/dist…"
  mkdir -p "$sp/codex_crew/static"
  ( cd "$sp/codex_crew/static" && rm -rf dist && cp -R "$ROOT/website/dist" dist )
  [ -f "$sp/codex_crew/static/dist/index.html" ] || {
  ...
  bash "$ROOT/scripts/stamp-distribution.sh" "$KC_DISTRIBUTION" "$sp/codex_crew"

  # Relocatable launcher script.
  cat > "$out/bin/codexcrew" <<'LAUNCH'
#!/bin/bash
set -euo pipefail
# Resolve symlinks before deriving DIR. When this launcher is reached through a
# symlink (e.g. the ~/.local/bin/codexcrew shim planted on the user's PATH),
...
DIR="$(cd -P "$(dirname "$SOURCE")" && pwd)"
exec "$DIR/python3.12" -s -m codex_crew "$@"
LAUNCH
  chmod +x "$out/bin/codexcrew"
  ...
  PYTHONNOUSERSITE=1 "$out/bin/python3.12" -m codex_crew --version >/dev/null \
...
# bin/codexcrew.cmd on win32; Electron unwraps the shim and spawns
...
build_backend_windows() {
  ...
  # Run the backend pip install through powershell.exe rather than directly from
  # this MSYS bash. On Git-for-Windows bash, `"$out/python.exe" -m pip install
  # <target>` resolves the deps but SILENTLY skips installing the codex_crew
  # project itself — site-packages/codex_crew ends up with only data files and no
  # modules, so the self-containment gate below trips on "No module named
  # codex_crew.__main__". The identical pip invocation from PowerShell installs
  # all modules correctly, so the install is delegated there. Paths are handed
  # over in native Windows form (backslashes) via cygpath -w.
  #
  # CODEXCREW_PREBUILT_WHEEL: install a prebuilt codexcrew wheel (built host-side)
  # instead of building the project from "$ROOT"; either target installs fine
  # once the invocation runs under PowerShell.
  local _win_py _win_target
  _win_py="$(cygpath -w "$out/python.exe")"
  if [ -n "${CODEXCREW_PREBUILT_WHEEL:-}" ] && [ -f "$CODEXCREW_PREBUILT_WHEEL" ]; then
    _win_target="$(cygpath -w "$CODEXCREW_PREBUILT_WHEEL")"
  else
    _win_target="$(cygpath -w "$ROOT")"
  fi
  PYTHONNOUSERSITE=1 PYTHONPATH= CODEXCREW_SKIP_FRONTEND=1 \
    powershell.exe -NoProfile -ExecutionPolicy Bypass -Command \
    "\$env:PYTHONNOUSERSITE='1'; \$env:PYTHONPATH=''; \$env:CODEXCREW_SKIP_FRONTEND='1'; & '$_win_py' -m pip install --prefer-binary --no-warn-script-location --disable-pip-version-check '$_win_target'; exit \$LASTEXITCODE"
  ...
  env PYTHONNOUSERSITE=1 PYTHONPATH= CODEXCREW_SKIP_FRONTEND=1 \
    ...
    --no-warn-script-location --disable-pip-version-check \
    "codexcrew[voice-aws] @ $root_uri" "imageio-ffmpeg==0.6.0"
  env PYTHONNOUSERSITE=1 PYTHONPATH= CODEXCREW_SKIP_FRONTEND=1 \
    "$out/python.exe" -m pip install --prefer-binary \
    --only-binary pywhispercpp \
    --no-warn-script-location --disable-pip-version-check \
    "codexcrew[voice] @ $root_uri"

  sp="$out/Lib/site-packages"
  stage_codex_adapter "$sp" "$HOST_ARCH" "$OS"
  log "Staging dashboard dist into codex_crew/static/dist…"
  mkdir -p "$sp/codex_crew/static"
  ( cd "$sp/codex_crew/static" && rm -rf dist && cp -R "$ROOT/website/dist" dist )
  [ -f "$sp/codex_crew/static/dist/index.html" ] || {
  ...
  mkdir -p "$out/bin"
  printf '@echo off\r\n"%%~dp0..\\python.exe" -s -m codex_crew %%*\r\n' > "$out/bin/codexcrew.cmd"

  log "Verifying self-containment ($(basename "$out"))…"
  PYTHONNOUSERSITE=1 "$out/python.exe" -s -m codex_crew --version >/dev/null \
    || { echo "ERROR: bundled backend is NOT self-contained (missing dep under PYTHONNOUSERSITE=1)" >&2; exit 1; }
  ...
  ( cd "$out"
    ...
    rm -rf Lib/test Lib/idlelib Lib/tkinter Lib/turtledemo Lib/ensurepip Lib/lib2to3 \
           include libs tcl \
           Lib/site-packages/codex_crew/_vendor/llama_cpp_libs/linux_aarch64 \
           Lib/site-packages/codex_crew/_vendor/llama_cpp_libs/linux_x86_64 \
           Lib/site-packages/codex_crew/_vendor/llama_cpp_libs/macos_arm64 \
           Lib/site-packages/codex_crew/_vendor/llama_cpp_libs/macos_x86_64 \
           2>/dev/null || true
  ...
  env PYTHONDONTWRITEBYTECODE=1 PYTHONNOUSERSITE=1 PYTHONPATH= \
    ...
    --root "$out" --module codex_crew.cli_server
...
# "codexcrew" PATH fallback -> spawn ENOENT).
...
if [ "$UNIVERSAL" = "1" ]; then
  ...
  build_backend "$PBS_ARM64" "$ELECTRON_DIR/backend-dist/codexcrew-backend-arm64" "arm64"
  build_backend "$PBS_X64" "$ELECTRON_DIR/backend-dist/codexcrew-backend-x64" "x86_64"

  resolver_gate "$ELECTRON_DIR/backend-dist/codexcrew-backend-arm64/bin/codexcrew" "arm64"
  resolver_gate "$ELECTRON_DIR/backend-dist/codexcrew-backend-x64/bin/codexcrew" "x64"
else
  ...
  if [ "$OS" = "windows" ]; then
    build_backend_windows "$PBS_DIR" "$ELECTRON_DIR/backend-dist/codexcrew-backend"
    resolver_gate "$ELECTRON_DIR/backend-dist/codexcrew-backend/bin/codexcrew.cmd" ""
  else
    build_backend "$PBS_DIR" "$ELECTRON_DIR/backend-dist/codexcrew-backend" ""
    resolver_gate "$ELECTRON_DIR/backend-dist/codexcrew-backend/bin/codexcrew" ""
  fi
fi
...
if [ -n "${CODEXCREW_MANAGED_INSTALL_MARKER:-}" ]; then
  MARKER_SRC="$CODEXCREW_MANAGED_INSTALL_MARKER"
  test -f "$MARKER_SRC" || { echo "❌ CODEXCREW_MANAGED_INSTALL_MARKER does not name a file: $MARKER_SRC" >&2; exit 1; }
  node -e '
    const fs = require("fs");
  ...
  ' "$MARKER_SRC" || { echo "❌ CODEXCREW_MANAGED_INSTALL_MARKER rejected: $MARKER_SRC" >&2; exit 1; }
...
( cd "$ELECTRON_DIR"
  ...
  if [ "$PRODUCT_NAME" = "Codex Crew Nightly" ]; then
    # Same appId (io.github.mikutellyourworld.codexcrew) as production ON PURPOSE:
    ...
    # Cost accepted: shared TCC/notification identity, and a codexcrew:// URL
    # scheme could not disambiguate the two apps (none is registered today).
    EB_ARGS+=(
      "-c.productName=CodexCrew Nightly"
      ...
      "-c.mac.extendInfo.CFBundleDisplayName=Codex Crew Nightly"
      ...
      "-c.deb.packageName=codexcrew-nightly"
      "-c.rpm.packageName=codexcrew-nightly"
      "-c.linux.executableName=codexcrew-desktop-nightly"
      "-c.extraMetadata.desktopName=codexcrew-desktop-nightly.desktop"
      ...
      # do not cover.
      "-c.extraMetadata.name=codexcrew-desktop-nightly"
      ...
      #    hypothetical `io.github.mikutellyourworld.codexcrew.nightly` appId, so it is stable,
      ...
      "-c.win.appId=io.github.mikutellyourworld.codexcrew.nightly"
    )
  fi
...
echo ""
echo "    The .app embeds the backend, so it runs with no PATH codexcrew needed."
