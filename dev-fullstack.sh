#!/bin/bash
# Start the full CodexCrew dev stack in ONE terminal:
#   1. Backend gateway from live source (dev-backend.sh, port 6777, .codexcrew-dev home)
...
GATEWAY_PORT="${CODEXCREW_PORT:-6777}"
...
if [ ! -x "$RUNTIME_PYTHON" ]; then
    echo "[dev] ERROR: cannot find the CodexCrew venv Python at $SCRIPT_DIR/.venv/bin/python."
...
if curl -fsS -o /dev/null --max-time 2 "http://127.0.0.1:$GATEWAY_PORT/" 2>/dev/null; then
    ...
    echo "[dev] Stop it first (codexcrew stop --port $GATEWAY_PORT) or set CODEXCREW_PORT to a free port."
    exit 1
fi
# One EFFECTIVE data home for the whole stack. dev-backend.sh honors a
# caller-provided CODEXCREW_HOME, so the token mint below must use the same
# home the backend booted with — hardcoding .codexcrew-dev for one of them
# yields a token signed with a different secret (auth silently fails).
# Same default + relative-path handling as dev-backend.sh.
CODEXCREW_HOME="${CODEXCREW_HOME:-.codexcrew-dev}"
case "$CODEXCREW_HOME" in
    /*) ;;
    *) CODEXCREW_HOME="$SCRIPT_DIR/$CODEXCREW_HOME" ;;
esac
export CODEXCREW_HOME
...
echo "[dev] starting backend (port $GATEWAY_PORT, home .codexcrew-dev) -> $BACKEND_LOG"
...
# export CODEXCREW_ALLOWED_LOOPBACK_PORTS=3000,3001,3002 before running this.
...
( cd website && CODEXCREW_PORT="$GATEWAY_PORT" exec npm run dev > "$VITE_LOG" 2>&1 ) &
PIDS+=($!)
...
# `codexcrew token` can print a SECOND, externally-advertised URL when
# dashboard.url is configured — always take the localhost one.
TOKEN_URL="$(PYTHONPATH="$SCRIPT_DIR/src" \
    "$RUNTIME_PYTHON" -m codex_crew token --port "$GATEWAY_PORT" 2>/dev/null \
...
if [ -n "$TOKEN_URL" ]; then
    ...
    echo "[dev]   CODEXCREW_HOME=$CODEXCREW_HOME PYTHONPATH=src .venv/bin/python -m codex_crew token --port $GATEWAY_PORT"
