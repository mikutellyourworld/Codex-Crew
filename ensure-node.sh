...
# Called by: setup.sh, codexcrew update, codexcrew gateway, Makefile (frontend)
...
# "$CODEXCREW_HOME" when set, else "$HOME/.codex-crew" (the current default) —
# NOT the pre-move "$HOME/.codexcrew", which is not the data home and must not
# be written to.
...
_glibc217_node_dir() {
    echo "${CODEXCREW_HOME:-$HOME/.codex-crew}/node-glibc217"
}
...
# the data home ($CODEXCREW_HOME, else ~/.codex-crew) — never the legacy
# ~/.codexcrew (see header).
_record_node_bin() {
    if command -v node >/dev/null 2>&1; then
        local home="${CODEXCREW_HOME:-$HOME/.codex-crew}"
        mkdir -p "$home"
