#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────────
# Codex Crew, minimal public quickstart
#
# The canonical public install. Builds Codex Crew from a local clone using
...
echo ">_ Codex Crew, minimal install"
echo "  Repo:    $REPO_DIR"
...
# Vite emits to website/dist; setup.py copies src/codex_crew/static/dist into
...
if [ -d "$REPO_DIR/website" ]; then
    ...
    _dist_dst="$REPO_DIR/src/codex_crew/static/dist"
    if [ -d "$_dist_src" ]; then
        rm -rf "$_dist_dst"
...
if [ "$WITH_VOICE" -eq 1 ]; then
    ...
    CODEXCREW_SKIP_FRONTEND=1 "$_venv/bin/pip" install -e "$REPO_DIR"'[voice]' \
        || die "pip install failed"
else
    CODEXCREW_SKIP_FRONTEND=1 "$_venv/bin/pip" install -e "$REPO_DIR" \
        || die "pip install failed"
fi
...
echo "→ Checking agent backend (codex-acp)…"
if has codex-acp; then
    echo "✓ codex-acp already on PATH"
elif has npm; then
    echo "  Installing @agentclientprotocol/codex-acp@1.10.0 via npm…"
    npm install -g @agentclientprotocol/codex-acp@1.10.0 >/dev/null 2>&1 \
        && echo "✓ codex-acp installed" \
        || echo "⚠ codex-acp install failed — install it manually before running"
else
    echo "⚠ npm not found — install the agent backend later:"
    echo "    npm i -g @agentclientprotocol/codex-acp@1.10.0"
fi
echo ""
...
echo "✓ Symlinked codexcrew → ~/.local/bin/codexcrew"
echo ""

# ── Done ──
echo ">_ Codex Crew installed!"
echo ""
echo "  Next steps:"
echo "    1. Run setup:    $HOME/.local/bin/codexcrew setup"
echo "    2. Start it:     $HOME/.local/bin/codexcrew gateway"
echo "       (or add ~/.local/bin to PATH and just run: codexcrew gateway)"
echo ""
