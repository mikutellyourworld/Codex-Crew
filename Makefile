# CodexCrew — public build targets (pip + npm/vite + pytest).
...
frontend:
	...
	cd website && \
	  NBD="$$(cat "$${CODEXCREW_HOME:-$$HOME/.codex-crew}/node-bin-dir" 2>/dev/null || true)"; \
	  ...
	  ( cd electron && \
	    if [ -f package-lock.json ]; then npm ci --no-audit --no-fund; else npm install --no-audit --no-fund; fi )
	rm -rf src/codex_crew/static/dist
	mkdir -p src/codex_crew/static
...
backend:
	...
	PY="$$(cat "$${CODEXCREW_HOME:-$$HOME/.codex-crew}/python-bin" 2>/dev/null || true)"; [ -n "$$PY" ] || PY="$(PY)"; \
	...
	# wheel (macOS, AL2023).
	CODEXCREW_SKIP_FRONTEND=1 $(PIP) install --prefer-binary -e ".[dev]"
...
clean:
	rm -rf build dist *.egg-info src/*.egg-info \
	       src/codex_crew/static/dist website/dist \
