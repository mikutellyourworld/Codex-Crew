...
Skills are markdown files that teach the LLM agent how to use CodexCrew capabilities.

## This directory is repo-checkout-only

Skills here are synced into `~/.codex-crew/skills` only when `CODEXCREW_PROJECT_DIR`
...
`codex_crew/builtin_skills/` instead** — that tree is packaged and copied into every
