---
name: self-update
description: Check for and apply CodexCrew updates. Use when user says "update yourself", "check for updates", "are you up to date", "keep yourself updated", or "auto-update".
...
Check for available CodexCrew updates, apply them, and optionally set up automatic update checking via cron.

`codexcrew update` behaves differently on three install layouts, and which one the
...
no fallback — `codexcrew update` exits 1 on provider failure. Report the provider's
...
codexcrew update --force   # git installs only
```
...
2. **Add a random minute** (0-59) to the cron expression to avoid a thundering herd of all CodexCrew instances checking simultaneously.
...
```python
cron_add(
    name="codexcrew-update-check",
    cron_expr="37 14 * * 3",  # Wednesday at 2:37pm (random minute)
    timezone="America/Los_Angeles",  # from user's Slack profile
    command="codexcrew --version",
    message="Report the installed CodexCrew version and check whether a newer one is available",
)
```

A notify-only cron must NOT run `codexcrew update` (that applies the update). Use
`codexcrew --version` to report the installed version, and have the LLM-mode job
...
```python
cron_add(
    name="codexcrew-auto-update",
    cron_expr="42 2 * * 4",  # Thursday at 2:42am (off-hours, random minute)
    timezone="America/Los_Angeles",
    message="Apply a CodexCrew update by scheduling it server-side, then follow the gateway-restart skill to restart the gateway. If already up to date, report briefly.",
)
```

An explicit `codexcrew update` is the supported way a source install updates; a
...
- BOTH `codexcrew restart` AND `codexcrew update` are blocked when run directly from an agent session, by the `self-protection-restart` and `self-protection-update` deny rules.
- A policy-defined update provider, where configured, owns updating the host and there is no fallback: `codexcrew update` exits 1 if it fails. Report that failure rather than working around it.
...
1. Run `codexcrew --version` to show the current version
...
2. If an update is available, apply it (the agent shell cannot run `codexcrew update` directly — schedule it server-side via cron, or ask the user to run it)
