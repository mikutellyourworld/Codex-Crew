---
name: codexcrew-app-dev
description: "Build, package, and publish CodexCrew external apps. Covers app.json manifest, UI components, crons, skills, self-healing install, git publishing, and common pitfalls."
---

# CodexCrew App Development

Guide for building external CodexCrew apps that work reliably across CodexCrew versions without depending on buggy lifecycle hooks or skill scanner behavior.

## When to Use

- User says "create a codexcrew app", "build an app", "make an app for codexcrew"
...
**Never depend on:**
- `onInstall` hook firing (unreliable across CodexCrew versions)
...
"message": "FIRST (self-heal, run these bash commands silently):
  (1) Skill symlink: if ~/.codex-crew/skills/MY-SKILL/SKILL.md does not exist,
      run: ln -sfn ~/.codex-crew/apps/MY-APP/skills/MY-SKILL ~/.codex-crew/skills/MY-SKILL
  (2) State dir: mkdir -p ~/.codex-crew/workspace/MY-APP && [ -f ~/.codex-crew/workspace/MY-APP/state.json ] || echo '{}' > ~/.codex-crew/workspace/MY-APP/state.json
  (3) Config for UI: mkdir -p ~/.codex-crew/apps/MY-APP/data && [ -f ~/.codex-crew/apps/MY-APP/data/config.json ] || echo '{...resolved paths...}' > ~/.codex-crew/apps/MY-APP/data/config.json
...
All Codex Crew apps MUST use the same visual language for consistency. Colors come
...
`src/codex_crew/apps/builtins/` — they follow the same manifest, UI, and cron
...
```html
<!--
  This is what a CodexCrew app header + card should look like.
...
import { useNavigate } from '@codexcrew/app-sdk'
...
const appJsonPath = '/Users/.../.codex-crew/apps/my-app/app.json'
...
| Use `useNavigate()` from `@codexcrew/app-sdk` for navigation | Use `window.location` (causes full reload) |
...
- `react-dom`
- `@codexcrew/app-sdk` — hooks (`useAppApi`, `useAppEvents`, `useTheme`, `useAppInfo`,
...
Open a pull request to the Codex Crew repo adding an entry to
`src/codex_crew/apps/app-registry.json`:
```json
{
  "name": "my-app",
...
from the public repository catalog, so a listing lands there
...
```json
{
  ...
  "message": "Fetch remote app.json via git archive, compare version to installed. Write result to ~/.codex-crew/workspace/my-app/update-status.json. Always silent.",
  "every": 86400,
  "silent": true,
  "persistent_session": false
}
...
{
  "name": "my-app-update-check",
  "message": "Check if a newer version of MY-APP is available. READ-ONLY. Steps: (1) Remote version: run `git archive --remote=https://github.com/<org>/my-app main app.json | tar -xO` from $HOME. Parse 'version'. (2) Installed version: read ~/.codex-crew/apps/my-app/app.json. (3) Compare semver. Write ONLY to ~/.codex-crew/workspace/my-app/update-status.json: {checked:true, installedVersion, remoteVersion, updateAvailable:bool, checkedAt:ISO}. Silent.",
  "every": 86400,
  "silent": true,
  "persistent_session": false
}
...
1. `codexcrew app install /path/to/my-app` — the CLI verb takes a local directory containing `app.json`. A git-hosted app is installed from the App Store / registry, which is the path that clones.
2. Gateway restart: `codexcrew restart` (`codexcrew gateway restart` is not a command — `gateway` has flags, not subcommands). An agent cannot run it: the shell layer blocks the spelling under `self-protection-restart`, so use the gateway-restart skill's scheduled-restart flow instead.
...
The installed app at `~/.codex-crew/apps/MY-APP/` IS a git repo with origin pointing to the source repository. You can commit and push directly from there:

```bash
cd ~/.codex-crew/apps/my-app && git add -A && git commit -m "message" && git push origin main
```
...
| Skills don't load after install | Scanner only checks flat `~/.codex-crew/skills/<name>/SKILL.md` | Self-heal symlink in cron |
| UI shows stale data after app.json change | Browser caches manifest API response | Hard refresh |
| `onInstall` doesn't run | CodexCrew lifecycle hook bug | Don't depend on it — self-heal instead |
...
2. Install: `codexcrew app install /path/to/my-app`
3. Restart gateway: `codexcrew restart`
...
- `register_routes(ctx)` receives an AppContext and MUST return `list[AppRoute]`
  (`from codex_crew.apps.route_registry import AppRoute`) with paths RELATIVE to
...
- **Preferred: dev mode** — `codexcrew app dev <name>` (off: `--off`). Serves that
...
- To edit in your source tree, symlink the installed UI dir to source:
  `mv ~/.codex-crew/apps/<n>/ui ~/.codex-crew/apps/<n>/ui.bak && ln -s <src>/ui ~/.codex-crew/apps/<n>/ui`
  ...
  `Rename-Item "$env:USERPROFILE\.codex-crew\apps\<n>\ui" ui.bak; New-Item -ItemType Junction -Path "$env:USERPROFILE\.codex-crew\apps\<n>\ui" -Target "<src>\ui"`
...
- **⚠️ Symlinks do NOT survive the app lifecycle.** `install_app`/`update_app`
  ...
  `ls -l ~/.codex-crew/apps/<n>/ui` shows a real dir, not a link. Fix: re-create
...
- Same clobber applies to locally-edited SHIPPED skills: installed skill files
  under `~/.codex-crew/skills/` re-sync from the CodexCrew package on update —
  durable skill changes must land in the repo (`skills/` in the CodexCrew source).
...
- **Host components over hand-rolled ones**: the `@codexcrew/ui` module-map
  ...
  (`window.__codexcrew_modules?.['@codexcrew/ui']`) and keep a small fallback for
...
- Access: `const sdk = window.__codexcrew_modules?.['@codexcrew/app-sdk']`, then
...
When an app proves out and should ship with CodexCrew, port it into the repo —
the contracts CHANGE on both sides. Template: `src/codex_crew/apps/builtins/issue_radar/`.

- Layout: `src/codex_crew/apps/builtins/<snake_name>/` with `app.json`,
...
- Full worktree + build-gate discipline applies (see the codexcrew-worktree-dev
  skill) — this is now CodexCrew source.
