...
# "Verify the outcome").
LOG_DIR="${CODEXCREW_HOME:-$HOME/.codex-crew}/logs"
...
if [ -n "${CODEXCREW_RESTART_STATUS_FILE:-}" ]; then
  case "$CODEXCREW_RESTART_STATUS_FILE" in
    "$LOG_DIR"/restart-status.*)
      suffix="${CODEXCREW_RESTART_STATUS_FILE#"$LOG_DIR"/restart-status.}"
      case "$suffix" in
        */* | *..*) : ;;
        *) STATUS_FILE="$CODEXCREW_RESTART_STATUS_FILE" ;;
      esac
      ;;
  esac
fi
...
# attempt's verdict.
rm -f -- "$STATUS_FILE"
sleep "${CODEXCREW_RESTART_DELAY:-10}"
...
codexcrew restart 2>&1 | sed -E 's/([?&]token=)[^[:space:]]+/\1REDACTED/g' >&3
status="${PIPESTATUS[0]}"
