...
# Usage (from agent):
#   Start-Process -WindowStyle Hidden powershell -ArgumentList "-ExecutionPolicy", "Bypass", "-File", "<path>\do-restart.ps1", "-KirocrewBin", "<resolved-path-to-codexcrew.exe>"
#
...
if ($KirocrewBin -ne "codexcrew" -and (Test-Path $KirocrewBin)) {
    $bin = $KirocrewBin
} else {
    $found = Get-Command codexcrew -ErrorAction SilentlyContinue
...
try {
    ...
    if (Test-Path $venvPython) {
        & $venvPython -m codex_crew.cli restart 2>&1 | Out-Null
