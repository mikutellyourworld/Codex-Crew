...
One page. Tells you how to launch an autonomous AutoNudge loop in this CodexCrew session for **$PROJECT**.
...
curl -X POST "http://127.0.0.1:5486/api/autonudge?token=\$TOKEN" \\
...
| Symptom | Check |
|---|---|
| UI icon dim | \`echo \$CODEXCREW_AUTONUDGE\` should be \`1\`. |
| Nudge never fires | \`cat ~/.codex-crew/autonudge.json\` — loop \`active:true\`? Right slot_key? |
...
Skill reference: \`~/.codex-crew/skills/self-nudge-loop/SKILL.md\` §"Hardened Loop Pattern".
EOF
)
...
3. Read the skill: \`~/.codex-crew/skills/self-nudge-loop/SKILL.md\` §"Hardened Loop Pattern".
