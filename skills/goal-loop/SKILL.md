...
**CodexCrew `autonudge_stop` MCP tool** — shipped with CodexCrew ≥ the
...
the path below exists only where `CODEXCREW_PROJECT_DIR` names such a checkout):

```bash
cd ~/.codex-crew/skills/goal-loop
./scaffold.sh \
