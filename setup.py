"""CodexCrew packaging — plain setuptools build.
...
the pre-built frontend assets from ``src/codex_crew/static/dist`` into the
package.

The frontend is built separately with npm/Vite in the ``website/`` directory
and the resulting ``dist/`` is copied into ``src/codex_crew/static/dist`` before
...
class E2eTestCommand(Command):
    ...
    (the Makefile ``test`` target / plain ``pytest``) on purpose:
      * sets ``CODEXCREW_E2E=1`` to lift the ``skipif`` gate in
    ...
    def run(self) -> None:
        ...
        env["CODEXCREW_STRICT_ON_LOOP_PERSIST"] = "1"
        cmd = [
            ...
            # CODEXCREW_KIRO_BIN) and shells `playwright test` against it, so
        ...
        print("[test_e2e] CODEXCREW_E2E=1 " + " ".join(cmd))
...
class BuildWithFrontend(build_py):
    ...
    Expects ``src/codex_crew/static/dist`` to already exist in-tree (built by
    ...
    def run(self) -> None:
        super().run()
        base = os.path.dirname(os.path.abspath(__file__))
        src_dist = os.path.join(base, "src", "codex_crew", "static", "dist")
        if os.path.isdir(src_dist):
            build_dist = os.path.join(
                self.build_lib, "codex_crew", "static", "dist"
            )
            ...
            print(
                ...
                "         then copy website/dist into src/codex_crew/static/dist."
            )
    ...
    def _copy_changelog(self, base: str) -> None:
        ...
        back to a bundled ``codex_crew/CHANGELOG.md``. Copy it in here rather than
        via package_data because it lives at the repo root, outside the
        ``src/codex_crew`` package tree that setuptools globs.
        """
        src_changelog = os.path.join(base, "CHANGELOG.md")
        if os.path.isfile(src_changelog):
            pkg_dir = os.path.join(self.build_lib, "codex_crew")
